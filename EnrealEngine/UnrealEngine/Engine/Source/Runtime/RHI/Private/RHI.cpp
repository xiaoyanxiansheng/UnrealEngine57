...
bool RHIGetShaderPlatformConfigurationString(FString& OutSetting, FConfigCacheIni* Config, EShaderPlatform Platform, const TCHAR* SettingName)
{
	...
	{
		// Get the runtime shader platform's category name via the format->platform conversion
		// This is used to simulate "PREVIEW_USE_RUNTIME_VALUE" from DDSPI. A "ShaderPlatformConfig" system would provide this for us.
		const FName ShaderFormat = FDataDrivenShaderPlatformInfo::GetShaderFormat(Platform);
		const EShaderPlatform RuntimePlatform = ShaderFormatToLegacyShaderPlatform(ShaderFormat);
		FDataDrivenShaderPlatformInfo::GetName(RuntimePlatform).ToString(CategoryName);
	}
	else
#endif
	{

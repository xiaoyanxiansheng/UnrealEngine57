...
void RHIDetectAndWarnOfBadDrivers(bool bHasEditorToken)
{
	...
	if (DriverInfo.IsValid() && !GRHIDeviceIsAMDPreGCNArchitecture)
	{
		...
		if (bShowPrompt)
		{
			...
			{
				VendorString = TEXT("Qualcomm");
				HyperlinkText = NSLOCTEXT("MessageDialog", "DriverDownloadLinkQualcomm", "https://softwarecenter.qualcomm.com/catalog/item/Windows_Graphics_Driver");
			}

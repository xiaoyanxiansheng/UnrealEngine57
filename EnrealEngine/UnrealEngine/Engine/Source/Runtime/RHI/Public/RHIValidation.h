...
public:
	...
	*/
	...
	virtual FRHIResourceCollectionRef RHICreateResourceCollection(FRHICommandListBase& RHICmdList, TConstArrayView<FRHIResourceCollectionMember> InMembers) override final
	{
		return RHI->RHICreateResourceCollection(RHICmdList, InMembers);
	}

	virtual void RHIUpdateResourceCollection(FRHICommandListBase& RHICmdList, FRHIResourceCollection* InResourceCollection, uint32 InStartIndex, TConstArrayView<FRHIResourceCollectionMember> InMemberUpdates) override final
	{
		RHI_VALIDATION_CHECK(InStartIndex < static_cast<uint32>(InResourceCollection->GetMembers().Num()) && InStartIndex + InMemberUpdates.Num() <= static_cast<uint32>(InResourceCollection->GetMembers().Num()),
			*FString::Printf(TEXT("ResourceCollection update range of [%d, %d) is invalid for a ResourceCollection of size '%d'"), InStartIndex, InStartIndex + InMemberUpdates.Num(), InResourceCollection->GetMembers().Num()));

		RHI->RHIUpdateResourceCollection(RHICmdList, InResourceCollection, InStartIndex, InMemberUpdates);
	}
	...
	*/
	...
	virtual uint32 RHIGetHTilePlatformConfig(const FRHITextureDesc& DepthDesc) const override final
	{
		return RHI->RHIGetHTilePlatformConfig(DepthDesc);
	}
	...
	// FlushType: Thread safe
	virtual void RHIAdvanceFrameForGetViewportBackBuffer(FRHIViewport* Viewport, bool bPresent) override final
	{
		RHI->RHIAdvanceFrameForGetViewportBackBuffer(Viewport, bPresent);
	}
	...
	*/
	...
	virtual FRayTracingClusterOperationSize RHICalcRayTracingClusterOperationSize(const FRayTracingClusterOperationInitializer& Initializer) override final
	{
		return RHI->RHICalcRayTracingClusterOperationSize(Initializer);
	}

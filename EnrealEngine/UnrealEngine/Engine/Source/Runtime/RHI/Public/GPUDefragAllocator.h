...
*	Returns the current size (in uint8s), or the final size if it has a reallocating request.
*/
inline int64 FGPUDefragAllocator::FMemoryChunk::GetFinalSize() const
{
	return Size;
}
...
* @return			true if the allocation can be relocated at this time
*/
inline bool FGPUDefragAllocator::CanRelocate(const FMemoryChunk* Chunk) const
{
	if (Chunk->IsLocked())
	{
		return false;
...
* @param Request	Request to wait for. Must be a valid request.
*/
inline void FGPUDefragAllocator::BlockOnAsyncReallocation(FAsyncReallocationRequest* Request)
{

...
#if WITH_GAMEPLAY_DEBUGGER_MENU
void UGameplayDebuggerLocalController::OnDebugDraw(UCanvas* Canvas, APlayerController* PlayerController)
{
	check(Canvas);
	...
	// Draw on canvas only when OnDebugDraw (bound to global multicast delegate; no world differentiation)
	// is called without a specific player controller or with the one associated to our replicator.
	// This validation is required for multiplayer PIE, including split-screen.
	if (PlayerController != nullptr && CachedReplicator->GetReplicationOwner() != PlayerController)
	{
		return;
	}
...
{
	APlayerController* OwnerPC = CachedReplicator ? CachedReplicator->GetReplicationOwner().Get() : nullptr;

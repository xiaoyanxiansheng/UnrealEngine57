...
void FStaticMeshOperations::AppendMeshDescriptions(const TArray<const FMeshDescription*>& SourceMeshes, FMeshDescription& TargetMesh, const FAppendSettings& AppendSettings)
{
	...
	TPolygonAttributesRef<FName> TargetPolygonObjectNames;
	if (AppendSettings.bMergeObjectName)
	{
		// If any of the source meshes have an object name attribute, but the target mesh doesn't, we register it here so that we can
		// actually honor the bMergeObjectNames flag.
		if (!TargetAttributes.GetPolygonObjectNames().IsValid())
		{
			for (const FMeshDescription* SourceMesh: SourceMeshes)
			{
				FStaticMeshConstAttributes SourceAttributes(*SourceMesh);
				if (SourceAttributes.GetPolygonObjectNames().IsValid())
				{
					TargetAttributes.RegisterPolygonObjectNameAttribute();
					break;
				}
			}
		}
		TargetPolygonObjectNames = TargetAttributes.GetPolygonObjectNames();
	}
	...
	for (const FMeshDescription* SourceMeshPtr : SourceMeshes)
	{
		...
		TPolygonAttributesConstRef<FName> SourcePolygonObjectNames;
		if (TargetPolygonObjectNames.IsValid())
		{
			SourcePolygonObjectNames = SourceAttributes.GetPolygonObjectNames();
		}
		...
		if (AppendSettings.MeshTransform)
		{
			...
			for (const TPair<FVertexInstanceID, FVertexInstanceID>& VertexInstanceIDPair : SourceToTargetVertexInstanceID)
			{
				...
				Normal = static_cast<FVector3f>(FVector(TransformInverseTransposeMatrix.TransformVector(static_cast<FVector>(Normal)).GetSafeNormal()));

				FVector3f& Tangent = TargetVertexInstanceTangents[InstanceID];
				Tangent = static_cast<FVector3f>(FVector(TransformInverseTransposeMatrix.TransformVector(static_cast<FVector>(Tangent)).GetSafeNormal()));
		...
		{
			SourceMesh.GetPolygonVertexInstances(SourcePolygonID, PolygonVertexInstanceIDs);

			//Find the polygonGroupID
			FPolygonGroupID TargetPolygonGroupID = SinglePolygonGroup != INDEX_NONE ? SinglePolygonGroup : RemapPolygonGroup[SourceMesh.GetPolygonPolygonGroup(SourcePolygonID)];

			TArray<FVertexInstanceID, TInlineAllocator<4>> VertexInstanceIDs;
			VertexInstanceIDs.Reserve(4);
			if (bReverseCulling)
			{
				for (int32 ReverseVertexInstanceIdIndex = PolygonVertexInstanceIDs.Num()-1; ReverseVertexInstanceIdIndex >= 0; ReverseVertexInstanceIdIndex--)
				{
					VertexInstanceIDs.Add(SourceToTargetVertexInstanceID[PolygonVertexInstanceIDs[ReverseVertexInstanceIdIndex]]);
				}
			}
			else
			{
			...
			FPolygonID TargetPolygonID = TargetMesh.CreatePolygon(TargetPolygonGroupID, VertexInstanceIDs);

			// Copy the source object name, if requested. If the attribute doesn't exist on the source, just leave the name at default.
			// SourcePolygonObjectNames can only be valid if TargetPolygonObjectNames, so we only check that here.
			if (SourcePolygonObjectNames.IsValid())
			{
				TargetPolygonObjectNames.Set(TargetPolygonID, SourcePolygonObjectNames.Get(SourcePolygonID));
			}
		}
	}
...
void FStaticMeshOperations::AppendMeshDescription(const FMeshDescription& SourceMesh, FMeshDescription& TargetMesh, const FAppendSettings& AppendSettings)
{
...
float FStaticMeshOperations::ComputeMeshArea(const FMeshDescription& InMeshDescription)
{
	...
	{
		TArrayView<const FVertexID> TriangleVertices = InMeshDescription.GetTriangleVertices(TriangleID);
		const FVector3f& PointA = InMeshDescription.GetVertexPosition(TriangleVertices[0]);
		const FVector3f& PointB = InMeshDescription.GetVertexPosition(TriangleVertices[1]);
		const FVector3f& PointC = InMeshDescription.GetVertexPosition(TriangleVertices[2]);

		AreaSum += TriangleUtilities::ComputeTriangleArea(PointA, PointB, PointC);
	}

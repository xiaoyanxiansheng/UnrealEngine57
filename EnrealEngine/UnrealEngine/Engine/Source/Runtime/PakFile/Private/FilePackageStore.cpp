...
bool FFilePackageStoreBackend::GetPackageRedirectInfo(FPackageId PackageId, FName& OutSourcePackageName, FPackageId& OutRedirectedToPackageId)
{
	...
	if (!GIsEditor) // Skip localized asset redirection in Editor
	{
...
{
	if (const FEntryHandle* Entry = PackageEntries.Find(PackageId); Entry != nullptr)
	{
		if (Entry->HasSoftRefs)
		{
			...
			{
				// to do: figure out a better way to make sure softreference information wasn't loaded for this container, also this should make the outer request fail completely and error with appropriate information
				if (!Container->ContainerHeader->SoftPackageReferences.bLoadedSoftPackageReferences)
				{
					UE_LOG(LogFilePackageStore, Error, TEXT("Attempting to read softreference information for package %s but container %s never loaded softreference information"), *LexToString(PackageId), *LexToString(Container->ContainerHeader->ContainerId));
				}
		...
		{
			// make sure tha tit's containers mounted soft refs

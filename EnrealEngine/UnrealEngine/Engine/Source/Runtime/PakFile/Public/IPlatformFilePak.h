...
inline FString ChunkHashToString(const TPakChunkHash& InHash)
{
#if PAKHASH_USE_CRC
...
inline FArchive& operator<<(FArchive& Ar, FPakCompressedBlock& Block)
{
...
struct FPakEntry
{
	...
	inline void SetFlag( uint8 InFlag, bool bValue )
	{
		if( bValue )
		{
			Flags |= InFlag;
	...
	inline bool GetFlag( uint8 InFlag ) const
	{
		return (Flags & InFlag) == InFlag;
	}
	
	inline bool IsEncrypted() const             { return GetFlag(Flag_Encrypted); }
	inline void SetEncrypted( bool bEncrypted ) { SetFlag( Flag_Encrypted, bEncrypted ); }

	inline bool IsDeleteRecord() const                { return GetFlag(Flag_Deleted); }
	inline void SetDeleteRecord( bool bDeleteRecord ) { SetFlag(Flag_Deleted, bDeleteRecord ); }
...
inline FArchive& operator<<(FArchive& Ar, FPakEntryLocation& PakEntryLocation)
{
...
public:
	...
	bool operator==(TYPE_OF_NULLPTR) { return Archive == nullptr; }
	bool operator!=(TYPE_OF_NULLPTR) { return Archive != nullptr; }
...
typedef TMap<FUtf8String, FPakEntryLocation> FPakDirectory;
...
public:
	...
	static inline int64 AlignReadRequest(int64 Size) 
	{
		return Size;
	}

	static inline void DecryptBlock(void* Data, int64 Size, const FGuid& EncryptionKeyGuid)
	{
...
class FPakPlatformFile : public IPlatformFile
{
	struct FPakListEntry
	{
		...
		inline bool operator < (const FPakListEntry& RHS) const
		{
	 ...
	 */
	inline void GetMountedPaks(TArray<FPakListEntry>& Paks)
	{
...
public:
	...
	*/
	inline void GetMountedPakFilenames(TArray<FString>& PakFilenames)
	{
		UE::TScopeLock ScopedLock(PakListCritical);
	...
	* Get a list of all pak files which have been successfully mounted
	*/
	inline void GetMountedPakFilenames(TSet<FString>& PakFilenames)
	{
		UE::TScopeLock ScopedLock(PakListCritical);
		PakFilenames.Empty(PakFiles.Num());
		for (FPakListEntry& Entry : PakFiles)
	...
	*/
	inline void GetMountedChunkIds(TSet<int32>& OutChunkIds)
	{
		UE::TScopeLock ScopedLock(PakListCritical);
	...
	*/
	 ...
	 * @return true if the file was found, false otherwise.
	 */
	PAKFILE_API bool FindFileInPakFiles(const TCHAR* Filename, TRefCountPtr<FPakFile>* OutPakFile = nullptr,
		...
		if (FindFileInPakFiles(Filename, &PakFile, &FileEntry))
		{
			...
			if (PakDirectory != nullptr)
			{
				for (FPakDirectory::TConstIterator FileIt(*PakDirectory); FileIt; ++FileIt)
				{
					...
					{
						const FUtf8String& RealFilename = FileIt.Key();
						return Path / FString(RealFilename);
					}
				}
			}

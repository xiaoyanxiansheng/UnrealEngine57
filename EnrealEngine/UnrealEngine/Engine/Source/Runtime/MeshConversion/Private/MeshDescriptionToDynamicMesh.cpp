...
{
	TVertexAttributesConstRef<FVector3f> MeshDescAttribRef;
	FDynamicMeshMorphTargetAttribute* DynaMeshAttribRef = nullptr;
};
...
void FMeshDescriptionToDynamicMesh::Convert(const FMeshDescription* MeshIn, FDynamicMesh3& MeshOut, bool bCopyTangents )
{
	...
	// If needed, compute mapping to 'compacted' versions of the Polygon Group IDs
	TArray<int32> CompactedPolygonGroupMap;
	if (bUseCompactedPolygonGroupIDValues && MeshIn->PolygonGroups().Num() != MeshIn->PolygonGroups().GetArraySize())
	{
		const FPolygonGroupArray& PolygonGroups = MeshIn->PolygonGroups();
		CompactedPolygonGroupMap.Init(INDEX_NONE, PolygonGroups.GetArraySize());
		int32 SectionIndex = 0;
		for (FPolygonGroupID GroupID : PolygonGroups.GetElementIDs())
		{
			CompactedPolygonGroupMap[GroupID.GetValue()] = SectionIndex;
			++SectionIndex;
		}
	}
	auto MapPolygonGroupID = [&CompactedPolygonGroupMap](FPolygonGroupID GroupID) -> int32
	{
		return CompactedPolygonGroupMap.IsEmpty() ? GroupID.GetValue() : CompactedPolygonGroupMap[GroupID.GetValue()];
	};
	...
	for (const FTriangleID TriangleID : MeshIn->Triangles().GetElementIDs())
	{
		...
		int32 MappedPolygonGroupID = MapPolygonGroupID(MeshIn->GetTrianglePolygonGroup(TriangleID));
		...
		switch (GroupMode)
		{
			...
			case EPrimaryGroupMode::SetToPolygonGroupID:
				GroupID = MappedPolygonGroupID + 1; // Shift IDs up by 1 to leave ID 0 as a default/unassigned group
				break;
		...
		TriData.PolygonGroupID = MappedPolygonGroupID;
	...
	TArray<FMorphTargetAttribCopyInfo> MorphTargetAttribs;
	if (!bDisableAttributes)
	{
		...
		// Copy all morph targets
		MeshIn->VertexAttributes().ForEach([&](const FName InAttributeName, auto InAttributesRef)
		{
			if (FSkeletalMeshAttributes::IsMorphTargetAttribute(InAttributeName))
			{
				const FName MorphTargetName = FSkeletalMeshAttributes::GetMorphTargetNameFromAttribute(InAttributeName);
				
				FDynamicMeshMorphTargetAttribute *MorphTargetAttribute = new FDynamicMeshMorphTargetAttribute(&MeshOut);
				MorphTargetAttribs.Add({
					Attributes.GetVertexMorphPositionDelta(MorphTargetName),
					MorphTargetAttribute                    
				});
				
				MeshOut.Attributes()->AttachMorphTargetAttribute(MorphTargetName, MorphTargetAttribute);
			}
		});
	...
	if (!bDisableAttributes)
	{
		...
		for (const FMorphTargetAttribCopyInfo& MorphTargetAttribInfo : MorphTargetAttribs)
		{
			auto MorphTargetTask = UE::Tasks::Launch(UE_SOURCE_LOCATION, [&, MorphTargetAttribInfo]() -> void
			{
				for (int32 VertexID: MeshOut.VertexIndicesItr())
				{
					const FVector3f& V = MorphTargetAttribInfo.MeshDescAttribRef.Get(VertIDMap[VertexID]);
					MorphTargetAttribInfo.DynaMeshAttribRef->SetValue(VertexID, V);
				}
			});
			Pending.Add(MoveTemp(MorphTargetTask));
		}

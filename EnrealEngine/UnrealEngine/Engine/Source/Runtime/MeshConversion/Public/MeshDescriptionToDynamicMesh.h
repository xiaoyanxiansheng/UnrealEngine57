...
public:
	...
	/** 
	 * Set this flag to transform Polygon Group IDs to their compacted indices, before using them as material indices --
	 * in other words, 'holes' in the Mesh Description's PolygonGroups array are removed by shifting the subsequent IDs down
	 * 
	 * Note: For Static Mesh LODs, this transforms Polygon Group IDs to Section Indices.
	 * This flag should typically be set to 'true' when converting Static Mesh assets.
	 */
	 ...
	 *
	 * Note: If bUseCompactedPolygonGroupIDValues is true, this remapping applies *after* the compact step --
	 *  so this mapping is a Section Index -> Material Index map for Static Mesh assets.
	 */

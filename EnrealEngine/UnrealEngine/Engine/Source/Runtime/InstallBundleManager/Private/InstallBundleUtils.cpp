...
namespace InstallBundleUtil
{
	...
	void FContentRequestStatsMap::StatsBegin(const FContentRequestStatsKey& Key)
	{
		FContentRequestStats& Stats = StatsMap.FindOrAdd(Key);
		if (false == ensureAlwaysMsgf(Stats.bOpen, TEXT("StatsBegin - Stat closed for %s"), *Key.GetBundleName().ToString()))
		{
	...
	void FContentRequestStatsMap::StatsEnd(const FContentRequestStatsKey& Key)
	{
		FContentRequestStats& Stats = StatsMap.FindOrAdd(Key);

		ensureAlwaysMsgf(Stats.bOpen && Stats.StartTime > 0, TEXT("StatsEnd - Stat closed for %s"), *Key.GetBundleName().ToString());
		if (Stats.bOpen)
		{
			ensureAlwaysMsgf(
				Algo::AllOf(Stats.StateStats, 
					[](const TPair<FString, FContentRequestStateStats>& Pair) { return !Pair.Value.bOpen; }),
				TEXT("StatsEnd - StateStat open for %s"), *Key.GetBundleName().ToString());
	...
	void FContentRequestStatsMap::StatsReset(const FContentRequestStatsKey& Key)
	{
		if (FContentRequestStats* Stats = StatsMap.Find(Key))
		{
			ensureAlwaysMsgf(!Stats->bOpen, TEXT("StatsReset - Stat open for %s"), *Key.GetBundleName().ToString());
			ensureAlwaysMsgf(
				Algo::AllOf(Stats->StateStats,
					[](const TPair<FString, FContentRequestStateStats>& Pair) { return !Pair.Value.bOpen; }),
				TEXT("StatsReset - StateStat open for %s"), *Key.GetBundleName().ToString());

			StatsMap.Remove(Key);
		}
	}

	void FContentRequestStatsMap::StatsBegin(const FContentRequestStatsKey& Key, const TCHAR* State)
	{
		FContentRequestStats& Stats = StatsMap.FindOrAdd(Key);
		if (false == ensureAlwaysMsgf(Stats.bOpen, TEXT("StatsBegin - Stat closed for %s - %s"), *Key.GetBundleName().ToString(), State))
		{
			Stats = FContentRequestStats();
			Stats.StartTime = FPlatformTime::Seconds();
		}
		...
		if (false == ensureAlwaysMsgf(StateStats.bOpen, TEXT("StatsBegin - StateStat closed for %s - %s"), *Key.GetBundleName().ToString(), State))
		{
	...
	void FContentRequestStatsMap::StatsEnd(const FContentRequestStatsKey& Key, const TCHAR* State, uint64 DataSize /*= 0*/)
	{
		FContentRequestStats& Stats = StatsMap.FindOrAdd(Key);
		if (false == ensureAlwaysMsgf(Stats.bOpen && Stats.StartTime > 0, TEXT("StatsEnd - Stat closed for %s - %s"), *Key.GetBundleName().ToString(), State))
		{
			Stats = FContentRequestStats();
			Stats.StartTime = FPlatformTime::Seconds();
		}
		...
		FContentRequestStateStats& StateStats = Stats.StateStats.FindOrAdd(State);
		if (ensureAlwaysMsgf(StateStats.bOpen && StateStats.StartTime > 0, TEXT("StatsEnd - StateStat closed for %s - %s"), *Key.GetBundleName().ToString(), State))
		{

...
namespace InstallBundleUtil
{
	...
	public:
		FContentRequestStatsKey(FName InBundleName, void* InCookie)
			: BundleName(InBundleName), Cookie(InCookie)
		{}

		FName GetBundleName() const { return BundleName; }
		void* GetCookie() const { return Cookie; }

		[[nodiscard]] bool operator==(const FContentRequestStatsKey& Other) const
		{
			return BundleName == Other.BundleName && Cookie == Other.Cookie;
		}

		[[nodiscard]] friend uint32 GetTypeHash(const FContentRequestStatsKey& Key)
		{
			return HashCombineFast(GetTypeHash(Key.GetBundleName()), GetTypeHash(Key.GetCookie()));
		}
	};
	...
	{
	private:
		TMap<FContentRequestStatsKey, FContentRequestStats> StatsMap;
	...
	public:
		...
		{
			return StatsMap.Find(Key);
		}

		const FContentRequestStats& FindStatsChecked(const FContentRequestStatsKey& Key) const
		{
			return StatsMap.FindChecked(Key);
		}

		UE_DEPRECATED(5.7, "Use FContentRequestStatsKey version")
		void StatsBegin(FName BundleName) { StatsBegin(FContentRequestStatsKey(BundleName, nullptr)); }
		UE_DEPRECATED(5.7, "Use FContentRequestStatsKey version")
		void StatsEnd(FName BundleName) { StatsEnd(FContentRequestStatsKey(BundleName, nullptr)); }
		UE_DEPRECATED(5.7, "Use FContentRequestStatsKey version")
		void StatsBegin(FName BundleName, const TCHAR* State) { StatsBegin(FContentRequestStatsKey(BundleName, nullptr)); }
		UE_DEPRECATED(5.7, "Use FContentRequestStatsKey version")
		void StatsEnd(FName BundleName, const TCHAR* State, uint64 DataSize = 0) { StatsEnd(FContentRequestStatsKey(BundleName, nullptr)); }
		UE_DEPRECATED(5.7, "Use FContentRequestStatsKey version")
		void StatsReset(FName BundleName) { StatsReset(FContentRequestStatsKey(BundleName, nullptr)); }

		UE_DEPRECATED(5.7, "Use FindStats and FindStatsChecked")
		const TMap<FContentRequestStatsKey, FContentRequestStats>& GetMap() { return StatsMap; }
	};

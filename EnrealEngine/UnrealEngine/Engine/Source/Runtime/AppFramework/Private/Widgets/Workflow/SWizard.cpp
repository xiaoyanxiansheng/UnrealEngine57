...
void SWizard::Construct( const FArguments& InArgs )
{
	...
	bShowHistoryButtons = InArgs._ShowHistoryButtons;
...
TSharedPtr<SWidget> SWizard::GetWidgetToFocusOnActivate()
{
	return FinishButton;
}
...
{
	if (bShowHistoryButtons && WidgetSwitcher->GetActiveWidgetIndex() == 0 && OnFirstPageBackClicked.IsBound())
	{
		return true;
	}

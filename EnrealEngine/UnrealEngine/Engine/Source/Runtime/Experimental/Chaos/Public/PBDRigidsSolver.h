	...
	*/
	...
	public:
		...
		*/
		...
		UE_DEPRECATED(5.7, "Deprecated, This functionality is now handled by FRewindData::ProcessDirtyPTParticles")
	...
	private:
		...
		int32 DispatchTasksBufferPhysicsResults(int32 DispatchBatchIndex, int32 NumDirty, LambdaType ReturnHandle);

		CHAOS_API void BufferPhysicsResults();
		CHAOS_API void BufferPhysicsResultsSingleThreaded(
		...
		CHAOS_API void ClearCollisionEventsForProxy(const IPhysicsProxyBase* Proxy);
		...
		// Task dispatcher manage the task graph logic
		TUniquePtr<Private::FTaskDispatcherSolver> TaskDispatcher;
		...
		// NOTE: Must be before MEvolution because they are passed to it in the constructor
		...
		TArray<FPendingProxyDestroyInfo<FGeometryCollectionPhysicsProxy>> PendingDestroyGeometryCollectionPhysicsProxy;
		TArray<FPendingProxyDestroyInfo<FClusterUnionPhysicsProxy>> PendingDestroyClusterUnionProxy;

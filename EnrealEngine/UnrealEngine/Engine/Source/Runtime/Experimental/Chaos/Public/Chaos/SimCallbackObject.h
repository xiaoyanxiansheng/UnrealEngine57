...
namespace Utilities
{
	CHAOS_API FReal GetSolverPhysicsResultsTime(const FPhysicsSolverBase*);
}
...
public:
	...
	{
		return bIsResim ? ExecuteOnStep == Step : ExecuteOnStep <= Step;
	}

	const bool ShouldDelay(const int32 Step, const bool bIsResim) const
	{
		return !bIsResim && ExecuteOnStep > Step;
	}

	const int32 GetExecuteOnStep() const
	{
		return ExecuteOnStep;
	}
...
public:
	...
	*/
	...
	/**
	* Gets the output data produced in order up to and including ResultsTime. Typical usage is:
	* while(auto Output = PopOutputData_External()) { //process output }
	*/
...
private:
	...
	template <typename T>
	T* NewDataHelper(TArray<TUniquePtr<T>>& Backing, TTransactionallySafeSpscQueue<T*>& Queue)
	{
		T* Result;
	...
	TTransactionallySafeSpscQueue<TInputType*> InputPool;
	TArray<TUniquePtr<TInputType>> InputBacking;

	TTransactionallySafeSpscQueue<TOutputType*> OutputPool;
	TArray<TUniquePtr<TOutputType>> OutputBacking;
	TTransactionallySafeSpscQueue<TOutputType*> OutputQueue;	//holds the outputs in order

...
namespace Chaos
{
	...
	CHAOS_API extern int32 AccelerationStructureUseDynamicTree;
	...
	public:
		...
		{ 
			FPBDRigidsEvolutionBase::SetName(InName);
			CollisionConstraints.SetName(EvolutionName);
		}
		...
		*/
		...
		/*
		 * [EXPERIMENTAL] Similar to ApplyParticleTransformCorrectionDelta, but supplied an absolute transform to jump to. This is used for snaps.
		 */
		...
		*/
		...
		// Create a joint constraint connecting two particles
		...
		// Destroy a joint constraint
		CHAOS_API void DestroyJointConstraint(FPBDJointConstraintHandle* InJointHandle);

		// Change the settings of a joint constraint.
		// NOTE: May recreate the joint which would change the handle pointer. (Currently only when changing linear/non-linear solver type)
		...
		// Internal used only
		UE_INTERNAL virtual void WaitIntegrationComplete() override;
	...
	protected:
		...
		// Used to track whether we have applied the latest Inertia Conditioning Cvars

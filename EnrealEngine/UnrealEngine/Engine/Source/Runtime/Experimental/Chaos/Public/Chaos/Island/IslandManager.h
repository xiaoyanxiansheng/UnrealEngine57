...
namespace Chaos::Private
{
	...
	private:
		...
		public:
			...
			struct
			{
				...
				uint32 bWasTransformSetExplicitly : 1;
			};
	...
	*/
	...
	private:
		...
		// When SleepTypeSwitchCounter hits a threshold, the island sleep type (partial or full) 
		// will change if CVars::bChaosSolverPartialIslandSleep == true.
		int32 SleepTypeSwitchCounter = 0;
	...
	*/
	...
	private:
		...
		CHAOS_API void UpdateIslandPartialSleepAllowed(FPBDIsland* Island, const FGeometryParticleHandle* Particle);
		CHAOS_API void ResetIslandPartialSleepAllowed();
		CHAOS_API void UpdateIslandHasSleepingNode(FPBDIsland* Island, const FGeometryParticleHandle* Particle);
		...
		CHAOS_API bool RewakeConstraintsNotSupportingPartialSleep(FPBDIsland* Island);
		CHAOS_API void SleepConstraintsBetweenNonAwakeParticles(FPBDIsland* Island);
		CHAOS_API void PostStepPartialWake(FPBDIsland* Island);
		CHAOS_API void SwitchIslandSleepType(FPBDIsland* Island, const int32 MotionlessCount);
		CHAOS_API void ValidateConstraintSleepState(const FPBDIsland* Island);

		// Partial Island Waking
		CHAOS_API void ProcessPartialIslandWake(FPBDIsland* Island, const bool bProcessEdgeBasedWake);
		CHAOS_API bool PropagatePartialIslandWake(TArray<FPBDIslandParticle*>& NodeQueue, TArray<FPBDParticleMomentumSq>& MomentumQueue,
			TArray<FGeometryParticleHandle*>& ParticlesToWakeUp, const int32 NumNodesConstraintWake, const bool bTriggerNodesAlwaysPropagate);
		CHAOS_API bool ProcessTriggerParticle(FPBDIslandParticle* Node, TArray<FPBDIslandParticle*>& NodeQueue, TArray<FPBDParticleMomentumSq>& MomentumQueue, const bool bAlwaysTrigger);
		CHAOS_API bool ProcessParticleWake(FPBDIslandParticle* Node, const FPBDIslandParticle* OtherNode, const FPBDParticleMomentumSq* OtherMomentum,
			TArray<FPBDIslandParticle*>& NodeQueue, TArray<FPBDParticleMomentumSq>& MomentumQueue,
			TArray<FGeometryParticleHandle*>& WakeUpQueue, const bool bAlwaysPropagate);
		...
		CHAOS_API void ResetEdgeFlags();
		CHAOS_API void ResetNodeFlags();

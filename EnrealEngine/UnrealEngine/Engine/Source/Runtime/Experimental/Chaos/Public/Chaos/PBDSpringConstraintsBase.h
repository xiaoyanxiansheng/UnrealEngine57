...
public:
	...
	{
		// Update distances
		Dists.SetNumUninitialized(Constraints.Num());
		CalculateRestLengths(Particles.XArray(), Dists);
	}
...
protected:
	...
	void CalculateRestLengths(const TConstArrayView<FSolverVec3>& Positions, const TArrayView<FSolverReal>& DistsToCompute) const
	{
		...
		{
			DistsToCompute[Index] = (Positions[Constraints[Index][0]] - Positions[Constraints[Index][1]]).Size();
		}
	}

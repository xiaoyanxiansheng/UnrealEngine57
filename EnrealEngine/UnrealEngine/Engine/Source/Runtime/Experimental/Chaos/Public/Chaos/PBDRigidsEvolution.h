...
#include "Chaos/TaskDispatcherEvolution.h"
...
public:
	...
	void SetCaptureRewindKinematicTargetFunction(FCaptureRewindKinematicTargetRule Rule) { CaptureRewindKinematicTarget = Rule; }
	...
	*/
	...
	FORCEINLINE_DEBUGGABLE void DirtyParticle(TGeometryParticleHandleImp<FReal, 3, bPersistent>& Particle, const EPendingSpatialDataOperation Op = EPendingSpatialDataOperation::Update)
	{
		...
		if (Particle.HasCollision() || ForceNoCollisionIntoSQ)
		{
			...
			FPendingSpatialData& PendingData = InternalAccelerationQueue.Add(0, UniqueIdx);
			PendingData.Operation = Op;
			PendingData.AccelerationHandle = FAccelerationStructureHandle(Particle);
			PendingData.SpatialIdx = Particle.SpatialIdx();


			FPendingSpatialData& AsyncSpatialData = AsyncAccelerationQueue.FindOrAdd(UniqueIdx);
			AsyncSpatialData = PendingData;
	...
	*/
	...
	virtual void SetName(const FString& InName) { EvolutionName = FSharedDebugName(InName); }
	const FString& GetName() const { return EvolutionName.Value(); }

	// Internal used only 
	UE_INTERNAL virtual void WaitIntegrationComplete() { check(false); };

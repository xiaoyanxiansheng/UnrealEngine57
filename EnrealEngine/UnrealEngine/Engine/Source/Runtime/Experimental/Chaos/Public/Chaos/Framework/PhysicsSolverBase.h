...
namespace Chaos
{
	...
	public:
		...
		/** Enqueue a lambda function (command) to run on the physics thread on a specific physics step
		* The command will be reapplied during physics resimulation on the same step */
		template <typename Lambda>
		void EnqueueCommandScheduled_External(const int32 ExecuteOnStep, Lambda&& Func)
		{
			FSimCallbackCommandObject* CommandObject = new FSimCallbackCommandObject(MoveTemp(Func), ExecuteOnStep);
			MarshallingManager.RegisterSimCommand_External(CommandObject);
		}

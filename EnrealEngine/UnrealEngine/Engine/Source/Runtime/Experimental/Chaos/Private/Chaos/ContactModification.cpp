...
namespace Chaos
{
	...
	bool FContactPairModifier::GetIsInitialContact() const
	{
		return Constraint->IsInitialContact();
	}

	void FContactPairModifier::SetIsInitialContact(bool bIsInitialContact) const
	{
		Constraint->SetModifierApplied();

		Constraint->SetIsInitialContact(bIsInitialContact);
	}
	...
	bool FContactPairModifier::GetIsCCD() const
	{
		return Constraint->GetCCDEnabled() && Constraint->GetCCDSweepEnabled();
	}
	...
	void FCollisionContactModifier::ConvertToNonProbeConstraint(FPBDCollisionConstraint& Constraint)
	{
		...
		const FRigidTransform3& ShapeWorldTransform0 = Constraint.GetContact().GetShapeRelativeTransform0() * FParticleUtilities::GetActorWorldTransform(FConstGenericParticleHandle(Constraint.GetParticle0()));
		const FRigidTransform3& ShapeWorldTransform1 = Constraint.GetContact().GetShapeRelativeTransform1() * FParticleUtilities::GetActorWorldTransform(FConstGenericParticleHandle(Constraint.GetParticle1()));

...
namespace Chaos
{
	...
	namespace CVars
	{
		FRealSingle Chaos_Collision_CullDistanceScaleInverseSize = 0.01f;	// 100cm
	...
	void ReactivateCollisionConstraint(FPBDCollisionConstraint* Constraint)
	{
		...
		Constraint->ConcreteContainer()->GetConstraintAllocator().AddActiveConstraint(Constraint);

		// Re-enable the constraint
		// NOTE: The Enabled/Disabled flag can also be controlled by the user in a
		// contact modifier. There is no danger of re-enabling a constraint that the 
		// user disabled because a disabled constraint is removed from the graph and 
		// therefore will not get reactivated if an island wakes.
		// @todo(chaos): Despite the note, we should separate Internal and Extenal Enabled flags.
		// Internal-Enabled would be controlled by collision detection and waking. 
		// External-Enabled would be controlled by the user via a contact callback.
		// We would only be enabled when both are set. Probably call them Activated and Enabled.
	...
	void FSingleShapePairCollisionDetector::WakeCollision(const int32 SleepEpoch, const int32 CurrentEpoch)
	{
		if (Constraint.IsValid() && IsUsedSince(SleepEpoch))
		{
			...
			{
				ReactivateCollisionConstraint(Constraint.Get());
		...
		FImplicitSphere3& OutSphere)
	{
		...
		const FReal RadiusAlpha = FMath::Clamp(ChaosOneWayPairSphereRadiusAlpha, 0.0f, 1.0f);
		const FReal Radius = 0.5f * FMath::Lerp(Extents.GetMin(), Extents.GetMax(), RadiusAlpha);

...
namespace Chaos
{
	...
	void FClusterUnionManager::HandleAddOperation(FClusterUnionIndex ClusterIndex, const TArray<FPBDRigidParticleHandle*>& InParticles, bool bReleaseClustersFirst)
	{
		...
		Chaos::FReal MaxLinearEtherDrag = Cluster->InternalCluster->LinearEtherDrag();
		Chaos::FReal MaxAngularEtherDrag = Cluster->InternalCluster->AngularEtherDrag();
		bool bEnableGravity = Cluster->InternalCluster->GravityEnabled();
		...
		for (FPBDRigidParticleHandle* Particle : FinalParticlesToAdd)
		{
			...
			MaxLinearEtherDrag = FMath::Max(MaxLinearEtherDrag, Particle->LinearEtherDrag());
			MaxAngularEtherDrag = FMath::Max(MaxAngularEtherDrag, Particle->AngularEtherDrag());
			bEnableGravity &= Particle->GravityEnabled(); // favor disabled gravity 
		...
		Cluster->InternalCluster->SetLinearEtherDrag(MaxLinearEtherDrag);
		Cluster->InternalCluster->SetAngularEtherDrag(MaxAngularEtherDrag);
		Cluster->InternalCluster->SetGravityEnabled(bEnableGravity);

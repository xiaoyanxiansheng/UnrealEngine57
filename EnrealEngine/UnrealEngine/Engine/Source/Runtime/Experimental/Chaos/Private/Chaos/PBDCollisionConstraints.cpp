...
namespace Chaos
{
	...
	void FPBDCollisionConstraints::AddConstraintsToGraph(Private::FPBDIslandManager& IslandManager)
	{
		...
		for (FPBDCollisionConstraintHandle* ConstraintHandle : TempCollisions)
		{
			...
			// Check for nodes whose transform was set explicitly, estimate the wake-up momentum with new constraints
			// Update the smooth velocity to an estimate of the motion which would have lead to the penetration detected.
			const auto UpdateSmoothVelocity = [](FGeometryParticleHandle* Particle, const FPBDCollisionConstraint& Contact)  -> void
				{
					if (FPBDRigidParticleHandle* Rigid = Particle->CastToRigidParticle())
					{
						// @todo(Chaos): Not great because
						// 1) The time step is hard-coded which might lead to unwanted side effect for frame rate changes
						// 2) VSmooth logic is spread out over various files and classes (IslandManager, PBDRigidsEvolutionGBF)
						// 3) Penetration velocity estimation is not what we would expect when calling AddToConstraintGraph
						const FReal FakeInvDt = 30.0; // hard-coded inverse time step
						const FReal Phi = Contact.GetPhi();
						const FVec3 Normal = Contact.CalculateWorldContactNormal();
						const FVec3 PenetrationVelocity = (Phi * Normal) * FakeInvDt;
						if (Rigid->VSmooth().SizeSquared() + UE_KINDA_SMALL_NUMBER < PenetrationVelocity.SizeSquared())
						{
							Rigid->SetVSmooth(PenetrationVelocity);
						}
					}
				};


			const FParticlePair& ParticlePair = ConstraintHandle->GetConstrainedParticles();
			const FPBDCollisionConstraint& Contact = ConstraintHandle->GetContact();

			if (IslandManager.WasParticleTransformSetExplicitly(ParticlePair[0]))
			{
				UpdateSmoothVelocity(ParticlePair[0], Contact);
			}
			if (IslandManager.WasParticleTransformSetExplicitly(ParticlePair[1]))
			{
				UpdateSmoothVelocity(ParticlePair[1], Contact);
			}
		}

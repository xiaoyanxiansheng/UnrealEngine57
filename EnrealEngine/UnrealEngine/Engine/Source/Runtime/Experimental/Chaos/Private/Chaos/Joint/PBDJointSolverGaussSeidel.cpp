...
namespace Chaos
{
		...
		FReal& Lambda)
	{
		...
		else
		{
			...
			FReal DLambda = SolverStiffness * Multiplier * (S * Delta - D * VelDt - Lambda);
		...
		const FPBDJointSettings& JointSettings)
	{
		...
		if (Error > PositionTolerance)
		{
			...
			else
			{
				...
				ApplyPositionConstraintSoft(Dt, JointStiffness, JointDamping, bAccelerationMode, Axis, Error, 0.0f, 0.0f, LinearSoftLambda);
			}
		...
		const FPBDJointSettings& JointSettings)
	{
		...
		if (AxialError > PositionTolerance)
		{
			if ((AxialMotion == EJointMotionType::Limited) && FPBDJointUtilities::GetSoftLinearLimitEnabled(SolverSettings, JointSettings))
			{
				...
				const FReal JointDamping = FPBDJointUtilities::GetSoftLinearDamping(SolverSettings, JointSettings);
				const bool bAccelerationMode = FPBDJointUtilities::GetLinearSoftAccelerationMode(SolverSettings, JointSettings);
				ApplyPositionConstraintSoft(Dt, JointStiffness, JointDamping, bAccelerationMode, Axis, AxialError, 0.0f, 0.0f, LinearSoftLambda);
			}
		...
		if (RadialError > PositionTolerance)
		{
			if ((RadialMotion == EJointMotionType::Limited) && FPBDJointUtilities::GetSoftLinearLimitEnabled(SolverSettings, JointSettings))
			{
				...
				const FReal JointDamping = FPBDJointUtilities::GetSoftLinearDamping(SolverSettings, JointSettings);
				const bool bAccelerationMode = FPBDJointUtilities::GetLinearSoftAccelerationMode(SolverSettings, JointSettings);
				ApplyPositionConstraintSoft(Dt, JointStiffness, JointDamping, bAccelerationMode, RadialAxis, RadialError, 0.0f, 0.0f, LinearSoftLambda);
			}
		...
		const FReal DeltaVel)
	{
		...
		const FReal LambdaLimit = JointSettings.LinearDriveMaxForce[AxisIndex] * Dt * Dt;

		if ((FMath::Abs(DeltaPos) > PositionTolerance) || (JointDamping > 0.0f))
		{
			FReal ReturnedLambda = LinearDriveLambdas[AxisIndex];
			ApplyPositionConstraintSoft(Dt, JointStiffness, JointDamping, bAccelerationMode, Axis, DeltaPos, DeltaVel, LambdaLimit, ReturnedLambda);

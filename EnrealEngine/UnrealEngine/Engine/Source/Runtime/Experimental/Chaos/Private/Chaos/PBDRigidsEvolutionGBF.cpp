...
namespace Chaos
{
	...
	namespace CVars
	{
		...
		// Gets incremented when one of the Inertia Conditioning CVars changes so we can refresh the system.
		// Uses a change counter rather than a boolean to handle multiple Evolutions (e.g., Client+Server PIE)
		...
		FAutoConsoleVariableRef  CVarChaosSolverInertiaConditioningEnabled(
			TEXT("p.Chaos.Solver.InertiaConditioning.Enabled"), 
		...
		FAutoConsoleVariableRef  CVarChaosSolverInertiaConditioningDistance(
		...
		// NOTE: A value of 2.5 means that spheres (and cubes) are not effected by inertia conditioning (unless they have constraints outside their bounds).
		FRealSingle ChaosSolverInertiaConditioningRotationRatio = 2.5f;
		FAutoConsoleVariableRef  CVarChaosSolverInertiaConditioningRotationRatio(
			TEXT("p.Chaos.Solver.InertiaConditioning.RotationRatio"), 
		...
		FAutoConsoleVariableRef  CVarChaosSolverInertiaConditioningMaxInvInertiaComponentRatio(
		...
		// NOTE: ChaosInertiaConditioningInvInertiaTolerance of 1e-4 effectively disables inertia conditioning for medium and larger object.
		// If you want inertia conditioning on large objects, make ChaosInertiaConditioningInvInertiaTolerance very small (say, 1e-30).
		FInertiaConditioningTolerances ChaosInertiaConditioningTolerances = FInertiaConditioningTolerances();
		FAutoConsoleVariableRef  CVarChaosInertiaConditioningInvMassTolerance(
			TEXT("p.Chaos.InertiaConditioning.InvMassTolerance"),
			...
			OnInertiaConditioningCVarChangedDelegate);
		FAutoConsoleVariableRef  CVarChaosInertiaConditioningInvInertiaTolerance(
			TEXT("p.Chaos.InertiaConditioning.InvInertiaTolerance"),
			...
			OnInertiaConditioningCVarChangedDelegate);
		FAutoConsoleVariableRef  CVarChaosInertiaConditioningExtentTolerance(
			TEXT("p.Chaos.InertiaConditioning.ExtentTolerance"),
...
{
	CSV_SCOPED_TIMING_STAT(PhysicsVerbose, StepSolver_AdvanceOneTimeStepImpl);
	...
	{
		QUICK_SCOPE_CYCLE_COUNTER(STAT_Evolution_StepSolver_IntegrateKinematicSpatial);
		CSV_SCOPED_TIMING_STAT(PhysicsVerbose, StepSolver_IntegrateKinematicSpatial);
		...
		{
			if (CaptureRewindKinematicTarget)
			{
				CaptureRewindKinematicTarget(Particles.GetActiveMovingKinematicParticlesView());
			}
...
void FPBDRigidsEvolutionGBF::Integrate(FReal Dt)
{
	...
	const TArray<FPBDRigidParticleHandle*>& ParticlesArray = Particles.GetActiveParticlesArray();

	auto IntegrateWork = [this, &ParticlesArray,
		Dt, HackMaxAngularSpeedSq, HackMaxLinearSpeedSq, bAllowMACD, bForceMACD, VelocityBoundsMultiplierMACD, MaxVelocityBoundsExpansionMACD,
		VelocityBoundsMultiplier, MaxVelocityBoundsExpansion, BoundsThickness](int32 BatchIndex, int32 StartIndex, int32 EndIndex)
	{
		for (int32 Index = StartIndex; Index < EndIndex; Index++)
		{
			//question: can we enforce this at the API layer? Right now islands contain non dynamic which makes this hard
			if (FPBDRigidParticleHandle* PBDParticle = ParticlesArray[Index]->CastToRigidParticle();  PBDParticle)
			{
				if (PBDParticle->ObjectState() == EObjectStateType::Dynamic)
				{
					FPBDRigidParticleHandle& Particle = *PBDParticle;
				...
				if (!Base::IsADisableCluster(*PBDParticle) && (PBDParticle->HasCollision() || ForceNoCollisionIntoSQ))
				{
					const FUniqueIdx UniqueIdx = PBDParticle->UniqueIdx();
					FPendingSpatialData& PendingData = InternalAccelerationQueue.Add(BatchIndex + 1, UniqueIdx);
					PendingData.Operation = EPendingSpatialDataOperation::Update;
					PendingData.AccelerationHandle = FAccelerationStructureHandle(*PBDParticle);
					PendingData.SpatialIdx = PBDParticle->SpatialIdx();
				}
			}
		}
	};

	auto DirtyAsync = [this, &ParticlesArray]()
	{
		// The async data structure is for the static particle. Here we update the dynamic particles.
		// We need to update them only if dynamic and static particles are sharing the same spatial structure
		// If the dynamic tree is not used we need to update the static one.
		if (AccelerationStructureSplitStaticAndDynamic != 1 || AccelerationStructureUseDynamicTree == 0)
		{
			for (FPBDRigidParticleHandle* Particle : ParticlesArray)
			{
				Base::DirtyParticleAsync(*Particle);
			}
		}
	};
	

	if (GSingleThreadedPhysics)
	{
		// In single threaded mode keep same logic with three batches inside PendingDataArrays: 
		// 0 Dirty particles from the PushData
		// 1 Dynamic particles added in Integrate
		// 2 Kinematic particles added in Apply Kinematic Target
		InternalAccelerationQueue.PendingDataArrays.SetNum(3);
		InternalAccelerationQueue.KinematicBatchStartIndex = 2;
		const int32 NumDynParticles = ParticlesArray.Num();
		IntegrateWork(0, 0, NumDynParticles);
		DirtyAsync();
	}
	else
	{
		TaskDispatcher.DispatchIntegrate(IntegrateWork);
		TaskDispatcher.DispatchDynAsyncDirty(DirtyAsync);
	}
}
...
void FPBDRigidsEvolutionGBF::ApplyKinematicTargets(const FReal Dt, const FReal StepFraction)
{
	...
	const bool bIsLastStep = (FMath::IsNearlyEqual(StepFraction, (FReal)1, (FReal)UE_KINDA_SMALL_NUMBER));

	TParticleView<FPBDRigidParticles>& KienamticParticles = Particles.GetActiveMovingKinematicParticlesView();
	...
	for (int32 ViewIndex = 0; ViewIndex < KienamticParticles.SOAViews.Num(); ++ViewIndex)
	{
		const TSOAView<TSOA>& SOAView = KienamticParticles.SOAViews[ViewIndex];
		...
		if (const TArray<THandle*>* CurHandlesArray = SOAView.HandlesArray)
		{
			...
			DispatchKinematicTargetsTasks<LambdaType>(DispatchBatchIndex++, NumHandles, Dt, StepFraction, bIsLastStep, ReturnHandle);
		}
		else if (TSOA* Soa = SOAView.SOA)
		{
			auto ReturnHandle = [Soa](int32 Index) { return THandleBase(Soa, Index); };
			using LambdaType = decltype(ReturnHandle);
			DispatchKinematicTargetsTasks<LambdaType>(DispatchBatchIndex++, NumParticles, Dt, StepFraction, bIsLastStep, ReturnHandle);
		}
	}

	auto DirtyKinAsync = [this]()
	{
		// The async data structure is for the static particle. Here we update the kinematic particles.
		// We need to update them only if kinematic and static particles are sharing the same spatial structure
		// If the dynamic tree is not used we need to update the static one.
		if (AccelerationStructureSplitStaticAndDynamic != 1 || AccelerationStructureUseDynamicTree == 0)
		{
	...
	{
		TaskDispatcher.DispatchKinAsyncDirtyAndUpdateKinematic(DirtyKinAsync, bIsLastStep);
	}
}

void FPBDRigidsEvolutionGBF::WaitIntegrationComplete()
{
	TaskDispatcher.WaitIntegrationComplete();
}

template <typename LambdaType>
void FPBDRigidsEvolutionGBF::DispatchKinematicTargetsTasks(int32 DispatchBatchIndex, int32 NumParticles, FReal Dt, FReal StepFraction, bool bIsLastStep, LambdaType ReturnHandle)
{
	using TSOA = typename TParticleView<FPBDRigidParticles>::TSOA;
	using THandle = typename TSOA::THandleType;
	using THandleBase = typename THandle::THandleBase;

	auto KinematicTargetsLambda = [this, ReturnHandle, Dt, StepFraction, bIsLastStep](int32 StartIndex, int32 EndIndex, int32 BatchIndex)
	{
		InternalAccelerationQueue.PendingDataArrays[InternalAccelerationQueue.KinematicBatchStartIndex + BatchIndex].Reserve(EndIndex- StartIndex);
		for (int32 Index = StartIndex; Index < EndIndex; Index++)
		{
			THandleBase Handle = ReturnHandle(Index);
			FTransientPBDRigidParticleHandle& Particle = static_cast<FTransientPBDRigidParticleHandle&>(Handle);
			if (!Particle.LightWeightDisabled())
			{
			...
			if (!Base::IsADisableCluster(Particle) && (Particle.HasCollision() || ForceNoCollisionIntoSQ))
			{
				const FUniqueIdx UniqueIdx = Particle.UniqueIdx();
				FPendingSpatialData& PendingData = InternalAccelerationQueue.Add(InternalAccelerationQueue.KinematicBatchStartIndex + BatchIndex, UniqueIdx);
				PendingData.Operation = EPendingSpatialDataOperation::Update;
				PendingData.AccelerationHandle = FAccelerationStructureHandle(Particle);
				PendingData.SpatialIdx = Particle.SpatialIdx();
			}
		}
	};
	...
	if (GSingleThreadedPhysics)
	{
		KinematicTargetsLambda(0, NumParticles, 0);
	}
	else
	{
		TaskDispatcher.DispatchKinematicsTarget(KinematicTargetsLambda, NumParticles, DispatchBatchIndex); 
	}
}
...
FPBDJointConstraintHandle* FPBDRigidsEvolutionGBF::CreateJointConstraint(const FParticlePair& InConstrainedParticles, const FPBDJointSettings& InJointSettings)
{
	FPBDJointConstraintHandle* JointHandle = nullptr;

	if (InJointSettings.bUseLinearSolver)
	{
		JointHandle = JointConstraints.LinearConstraints.AddConstraint(InConstrainedParticles, InJointSettings.ConnectorTransforms);
	}
	else
	{
		JointHandle = JointConstraints.NonLinearConstraints.AddConstraint(InConstrainedParticles, InJointSettings.ConnectorTransforms);
	}

	if (!InJointSettings.bCollisionEnabled)
	{
		GetBroadPhase().GetIgnoreCollisionManager().AddIgnoreCollisions(InConstrainedParticles[0], InConstrainedParticles[1]);
	}

	JointHandle->SetSettings(InJointSettings);

	// Register the constraint with each particle
	InConstrainedParticles[0]->AddConstraintHandle(JointHandle);
	InConstrainedParticles[1]->AddConstraintHandle(JointHandle);

	// We added a joint to the particles, so we need to modify the inertia (See UpdateInertiaConditioning)
	FGenericParticleHandle(InConstrainedParticles[0])->SetInertiaConditioningDirty();
	FGenericParticleHandle(InConstrainedParticles[1])->SetInertiaConditioningDirty();

	return JointHandle;
}

void FPBDRigidsEvolutionGBF::DestroyJointConstraint(FPBDJointConstraintHandle* InJointHandle)
{
	FParticlePair ConstrainedParticles = InJointHandle->GetConstrainedParticles();

	const FPBDJointSettings& Settings = InJointHandle->GetSettings();
	const bool bValidPair = ConstrainedParticles[0] && ConstrainedParticles[1];
	...
	{
		GetBroadPhase().GetIgnoreCollisionManager().RemoveIgnoreCollisions(ConstrainedParticles[0], ConstrainedParticles[1]);
	}

	// Unegister the constraint from each particle
	if (ConstrainedParticles[0])
	{
		ConstrainedParticles[0]->RemoveConstraintHandle(InJointHandle);
	}
	if (ConstrainedParticles[1])
	{
		ConstrainedParticles[1]->RemoveConstraintHandle(InJointHandle);
	}

	// Remove from the constraint graph
	if (InJointHandle->IsInConstraintGraph())
	{
		RemoveConstraintFromConstraintGraph(InJointHandle);
	}

	// Destroy the constraint
	if (Settings.bUseLinearSolver)
	{
		JointConstraints.LinearConstraints.RemoveConstraint(InJointHandle->GetConstraintIndex());
	}
	else
	{
		JointConstraints.NonLinearConstraints.RemoveConstraint(InJointHandle->GetConstraintIndex());
	}
}

FPBDJointConstraintHandle* FPBDRigidsEvolutionGBF::SetJointConstraintSettings(FPBDJointConstraintHandle* InJointHandle, const FPBDJointSettings& InJointSettings)
{
	const FPBDJointSettings& CurrentJointSettings = InJointHandle->GetSettings();
	FParticlePair ConstrainedParticles = InJointHandle->GetConstrainedParticles();

	// Handle changes to the CollisionEnabled flag
	if (CurrentJointSettings.bCollisionEnabled != InJointSettings.bCollisionEnabled)
	{
		if (InJointSettings.bCollisionEnabled)
		{
			GetBroadPhase().GetIgnoreCollisionManager().RemoveIgnoreCollisions(ConstrainedParticles[0], ConstrainedParticles[1]);
		...
		{
			GetBroadPhase().GetIgnoreCollisionManager().AddIgnoreCollisions(ConstrainedParticles[0], ConstrainedParticles[1]);
		}
	}

	// NOTE: Changing the solver type requires us to destroy and recreate the joint, which results in
	// relocating the joint handle. This also resets any state in the joint, e.g., whether it is broken.
	// @todo(chaos): Improve suport for this feature, but for now, just make it not crash!
	if (CurrentJointSettings.bUseLinearSolver != InJointSettings.bUseLinearSolver)
	{
		DestroyJointConstraint(InJointHandle);
		return CreateJointConstraint(ConstrainedParticles, InJointSettings);
	}

	// Update the joint settings
	InJointHandle->SetSettings(InJointSettings);
	return InJointHandle;
}
...
void FPBDRigidsEvolutionGBF::SetParticleTransform(FGeometryParticleHandle* InParticle, const FVec3& InPos, const FRotation3& InRot, const bool bIsTeleport, const bool bWakeUp)
{
	...
	// We only want to set the IsParticleTransformSetExplicitly flag if the particle will be woken up and this is a teleport.
	// Since only then we want to estimate an overlap velocity based on penetration.
	// (see @FPBDCollisionConstraints::AddConstraintsToGraph)
	if (bWakeUp && bIsTeleport)
	{
		GetIslandManager().FlagParticleTransformSetExplicitly(InParticle);
	}
	GetIslandManager().FlagParticleUpdated(InParticle);
...
void FPBDRigidsEvolutionGBF::OnParticleMoved(FGeometryParticleHandle* InParticle, const FVec3& PrevX, const FRotation3& PrevR, const bool bIsTeleport, const bool bWakeUp)
{
	...
	// If the particle transform was set explicitly (contains check for partial island sleep), 
	// we also want to manually set VSmooth to make use of momentum propagation in FPBDIslandManager::UpdateExplicitSleep.
	if (!bIsTeleport || GetIslandManager().WasParticleTransformSetExplicitly(InParticle))
	{
...
void FPBDRigidsEvolutionGBF::SetParticleDynamics(FPBDRigidParticleHandle* Rigid, const FParticleDynamics& Dynamics)
{
	Rigid->SetDynamics(Dynamics);
	ResetVSmoothFromForces(*Rigid);
	GetIslandManager().FlagParticleUpdated(Rigid->Handle());
}

void FPBDRigidsEvolutionGBF::SetParticleMassProps(FPBDRigidParticleHandle* Rigid, const FParticleMassProps& MassProperties)
{
	Rigid->SetMassProps(MassProperties);
	GetIslandManager().FlagParticleUpdated(Rigid->Handle());
}
...
void FPBDRigidsEvolutionGBF::UpdateInertiaConditioning()
{
	...
	// Tolerances (currently controlled by cvar)
	const FInertiaConditioningTolerances& Tolerances = ChaosInertiaConditioningTolerances;

	// Did the settings change?
	const bool bSettingsChanged = (InertiaConditioningEpoch != ChaosSolverInertiaConditioningEpoch);
	InertiaConditioningEpoch = ChaosSolverInertiaConditioningEpoch;

	// @todo(chaos): we should only visit particles that are dirty
	for (auto& Rigid : Particles.GetNonDisabledDynamicView())
	{
		// Update the inertia conditioning if some property of the body, or the system settings have changed
		if (Rigid.InertiaConditioningDirty() || (bSettingsChanged && Rigid.InertiaConditioningEnabled()))
		{
			...
			{
				const FVec3f InvInertiaScale = CalculateParticleInertiaConditioning(Rigid.Handle(), MaxDistance, MaxRotationRatio, MaxInvInertiaComponentRatio, Tolerances);

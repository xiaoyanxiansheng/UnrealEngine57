...
namespace Chaos
{
		...
		virtual TUniquePtr<ISpatialAcceleration<FAccelerationStructureHandle, FReal, 3>> CreateAccelerationPerBucket_Threaded(const TConstParticleView<FSpatialAccelerationCache>& Particles, uint16 BucketIdx, bool ForceFullBuild, bool bDynamicTree, bool bBuildOverlapCache) override
		{
			...
			case 0:
			{
				...
				else if (BroadPhaseConfig.BroadphaseType == FBroadPhaseConfig::Tree || BroadPhaseConfig.BroadphaseType == FBroadPhaseConfig::TreeAndGrid)
				{
					...
					const int MaxNumToProcess = ForceFullBuild ? 0 : BroadPhaseConfig.IterationsPerTimeSlice;
					if (bDynamicTree)
					{
						return MakeUnique<AABBDynamicTreeType>(Particles, BroadPhaseConfig.MaxChildrenInLeaf, BroadPhaseConfig.MaxTreeDepth, BroadPhaseConfig.MaxPayloadSize, MaxNumToProcess, bDynamicTree, bUseDirtyTree, bBuildOverlapCache);
					}
					bUseDirtyTree = AccelerationStructureUseDirtyTreeInsteadOfGrid == 1;
					return MakeUnique<AABBTreeType>(Particles, BroadPhaseConfig.MaxChildrenInLeaf, BroadPhaseConfig.MaxTreeDepth, BroadPhaseConfig.MaxPayloadSize, MaxNumToProcess, bDynamicTree, bUseDirtyTree, bBuildOverlapCache);
				}
				else if (BroadPhaseConfig.BroadphaseType == FBroadPhaseConfig::TreeOfGridAndGrid || BroadPhaseConfig.BroadphaseType == FBroadPhaseConfig::TreeOfGrid)
				{
	...
	FPBDRigidsEvolutionBase::FPBDRigidsEvolutionBase(FPBDRigidsSOAs& InParticles, THandleArray<FChaosPhysicsMaterial>& InSolverPhysicsMaterials, bool InIsSingleThreaded)
		...
		, TaskDispatcher(Particles, InternalAccelerationQueue)
	...
	void FPBDRigidsEvolutionBase::PruneInternalPendingData(FAccelerationStructure& AccelerationStructure)
	{
		...
		auto HasToBeUpdated = [&AccelerationStructure](const FPendingSpatialData& PendingData) -> bool
		{
			FGeometryParticleHandle* UpdateParticle = PendingData.AccelerationHandle.GetGeometryParticleHandle_PhysicsThread();
			return (PendingData.Operation == EPendingSpatialDataOperation::Delete ||
		...
		{
			TaskDispatcher.PruneInternalPendingData(HasToBeUpdated);
		}
		else
		{
			InternalAccelerationQueue.CleanUpDuplicated();
			for (TArray<FPendingSpatialData>& PendingDataArray : InternalAccelerationQueue.PendingDataArrays)
			{
				TArray<FPendingSpatialData> NewPendingData;
				NewPendingData.Reserve(PendingDataArray.Num());
				for (const FPendingSpatialData& PendingData : PendingDataArray)
				{
				...
				PendingDataArray = MoveTemp(NewPendingData);
			}
		}
	}
	...
	void FPBDRigidsEvolutionBase::FlushInternalAccelerationQueue()
	{
		...
		// Could be linked by dependent task, but could start as soon a previous task is finished
		for (const TArray<FPendingSpatialData>& PendingDataArray : InternalAccelerationQueue.PendingDataArrays)
		{
			for (const FPendingSpatialData& PendingData : PendingDataArray)
			{
	...
	{
		auto FlushAsyncAccelerationQueueLambda = [this]() 
		{
		...
		TaskDispatcher.FlushAccelerationQueue(FlushAsyncAccelerationQueueLambda);
	}
	...
	void FPBDRigidsEvolutionBase::ComputeIntermediateSpatialAcceleration(bool bBlock)
	{
		...
		if (!AccelerationStructureTaskComplete)
		{
			...
			TaskDispatcher.WaitAsyncQueueTask();
		...
		if (AsyncComplete && !IsResimming())
		{
			...
			if (AccelerationStructureTaskComplete && AsyncInternalAcceleration->IsAllAsyncTasksComplete())
			{
				...
				else
				{
					...
					// Wait for the integration to finish before reseting the internal acceleration queue. 
					// Note: We could even not fill the Internal Acceleration Queue when only one acceleration structure 
					// is used (AccelerationStructureSplitStaticAndDynamic is disabled)
			...
			if (bCanStartAsyncTasks)
			{
				...
				bool ForceFullBuild = AsyncAccelerationQueue.Num() > AccelerationStructureTimeSlicingMaxQueueSizeBeforeForce;
	...
	void FPBDRigidsEvolutionBase::WakeParticle(FPBDRigidParticleHandle* Particle)
	{
		...
		IslandManager.FlagParticleUpdated(Particle);
	}

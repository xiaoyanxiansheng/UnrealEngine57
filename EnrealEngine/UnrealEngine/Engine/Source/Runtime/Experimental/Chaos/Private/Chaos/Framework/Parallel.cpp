...
bool Chaos::ShouldExecuteTasksInParallel(int32 InTaskNum)
{
	...
	{
		// Single threaded physics should not run in parallel
		return false;
	}
	else if (InTaskNum >= MinParallelTaskSize)
	{
		// Meets threshold so run in parallel
		return true;
	}
	else if (IsInGameThread())
	{
		// Game thread should always use workers
		return true;
	}

...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PhysicsObjectInterface)
...
namespace Chaos
{
	...
	template<EThreadContext Id>
	void FWritePhysicsObjectInterface<Id>::SetLinearVelocity(TArrayView<const FPhysicsObjectHandle> InObjects, const FVector& InVelocity, bool bAddToCurrent)
	{
		if (InObjects.IsEmpty())
		{
			return;
		...
		for (const FPhysicsObjectHandle Object : InObjects)
		{
			...
			if (TThreadParticle<Id>* Particle = Object->GetParticle<Id>())
			{
				if (Chaos::TThreadRigidParticle<Id>* Rigid = Particle->CastToRigidParticle())
				{
					FVector Final = InVelocity;

					if (bAddToCurrent)
					{
						Final += Rigid->LinearImpulseVelocity();
					}

					Rigid->SetLinearImpulseVelocity(Final, false);
				}
			}
		}
		...
		if constexpr (Id == EThreadContext::External)
		{
			...
			{
				Solver->EnqueueCommandImmediate(
					[AllObjects = TArray<FPhysicsObjectHandle>{ InObjects }, InVelocity, bAddToCurrent]() {
						Chaos::FWritePhysicsObjectInterface_Internal Interface = Chaos::FPhysicsObjectInternalInterface::GetWrite();
						Interface.SetLinearVelocity(AllObjects, InVelocity, bAddToCurrent);
					}
				);
			}
		}
	...
	template<EThreadContext Id>
	void FWritePhysicsObjectInterface<Id>::SetAngularVelocityInRadians(TArrayView<const FPhysicsObjectHandle> InObjects, const FVector& InAngularVelocity, bool bAddToCurrent)
	{
		if (InObjects.IsEmpty())
		{
			return;
		...
		for (const FPhysicsObjectHandle Object : InObjects)
		{
			...
			if (TThreadParticle<Id>* Particle = Object->GetParticle<Id>())
			{
				if (Chaos::TThreadRigidParticle<Id>* Rigid = Particle->CastToRigidParticle())
				{
					FVector Final = InAngularVelocity;

					if (bAddToCurrent)
					{
						Final += Rigid->AngularImpulseVelocity();
					}

					Rigid->SetAngularImpulseVelocity(Final, false);
				}
			}
		}
		...
		if constexpr (Id == EThreadContext::External)
		{
			...
			{
				Solver->EnqueueCommandImmediate(
					[AllObjects = TArray<FPhysicsObjectHandle>{ InObjects }, InAngularVelocity, bAddToCurrent]() {
						Chaos::FWritePhysicsObjectInterface_Internal Interface = Chaos::FPhysicsObjectInternalInterface::GetWrite();
						Interface.SetAngularVelocityInRadians(AllObjects, InAngularVelocity, bAddToCurrent);
					}
				);
			}
		}
	...
	{
		Filter::FCombinedShapeFilterData ShapeFilterData = Chaos::Filter::FShapeFilterBuilder::BuildFromLegacyShapeFilter(QueryData, SimData);
		UpdateCombinedShapeFilterData(InObjects, ShapeFilterData);
	}

	template<EThreadContext Id>
	void FWritePhysicsObjectInterface<Id>::UpdateCombinedShapeFilterData(TArrayView<const FPhysicsObjectHandle> InObjects, const Chaos::Filter::FCombinedShapeFilterData& CombinedShapeFilter)
	{
		for (const FPhysicsObjectHandle Object : InObjects)
		{
			if (!Object)
			...
			if (TThreadParticle<Id>* Particle = Object->GetParticle<Id>())
			{
				for (const TUniquePtr<Chaos::FPerShapeData>& ShapeData : Particle->ShapesArray())
				{
					ShapeData->SetCombinedShapeFilterData(CombinedShapeFilter);
				}
			}
		}
	}

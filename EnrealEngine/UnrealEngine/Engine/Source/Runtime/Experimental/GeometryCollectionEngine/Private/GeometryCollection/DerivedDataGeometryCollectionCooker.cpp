...
bool FDerivedDataGeometryCollectionRenderDataCooker::Build(TArray<uint8>& OutData)
{
	...
	if (FGeometryCollection* Collection = GeometryCollection.GetGeometryCollection().Get())
	{
		...
		FGeometryCollectionRenderData::FCreateDesc CreateDesc = {
			.bEnableNanite = GeometryCollection.EnableNanite,
			.bEnableNaniteFallback = GeometryCollection.bEnableNaniteFallback,
			.bUseFullPrecisionUVs = GeometryCollection.bUseFullPrecisionUVs, 
			.bConvertVertexColorsToSRGB = GeometryCollection.bConvertVertexColorsToSRGB,
			.NaniteMinimumResidencyInKB = GeometryCollection.NaniteMinimumResidencyInKB
		};

		TUniquePtr<FGeometryCollectionRenderData> RenderData = FGeometryCollectionRenderData::Create(*Collection, CreateDesc);
...
const TCHAR* FDerivedDataGeometryCollectionRenderDataCooker::VersionString = TEXT("2087D33DBF524754A1F8FD20DCB7BEAB");

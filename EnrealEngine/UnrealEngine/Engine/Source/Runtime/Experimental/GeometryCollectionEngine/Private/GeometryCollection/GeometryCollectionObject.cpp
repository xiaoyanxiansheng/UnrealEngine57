...
{
	ProxyMeshes.Reset();
	MeshTransforms.Reset();
	MeshOverrideMaterials.Reset();
}
...
void UGeometryCollection::CacheMaterialDensity()
{
	...
	{
		PhysicsMaterialForDensity = GEngine ? GEngine->DefaultPhysMaterial.Get() : nullptr;
	}
	if (PhysicsMaterialForDensity)
	{
...
void UGeometryCollection::RebuildRenderData()
{
	if (RenderDataGuid != StateGuid)
	{
		...
		FGeometryCollectionRenderData::FCreateDesc CreateDesc = {
			.bEnableNanite = EnableNanite,
			.bEnableNaniteFallback = bEnableNaniteFallback,
			.bUseFullPrecisionUVs = bUseFullPrecisionUVs,
			.bConvertVertexColorsToSRGB = bConvertVertexColorsToSRGB,
			.NaniteMinimumResidencyInKB = NaniteMinimumResidencyInKB
		};

		RenderData = FGeometryCollectionRenderData::Create(*GetGeometryCollection(), CreateDesc);
		InitResources();
...
void UGeometryCollection::SetRootProxiesFromGeometrySources()
{
	...
	for (const FGeometryCollectionSource& Source : GeometrySource)
	{
		...
		if (UStaticMesh* SourceStaticMesh = Cast<UStaticMesh>(SourceMesh))
		{
			...
			FGeometryCollectionProxyMeshMaterials& MeshMaterials = RootProxyData.MeshOverrideMaterials.AddDefaulted_GetRef();
			MeshMaterials.Materials = Source.SourceMaterial;
		}
	}
}
...
TSharedPtr<FGeometryCollection, ESPMode::ThreadSafe> UGeometryCollection::GenerateMinimalGeometryCollection() const
{
	...
	// TODO(chaos) : for now, let's fix up the missing attributes and we should consolidate GenerateMinimalGeometryCollection and CopyCollectionAndRemoveGeometry for later releases
	// we should also probably have a way to specify which attributes are needed in the final collection instead of hardcoding it here
	static const FName AnchoredAttributeName(TEXT("Anchored"));
	if (GeometryCollection->HasAttribute(AnchoredAttributeName, FTransformCollection::TransformGroup))
	{
		DuplicateGeometryCollection->AddAttribute<bool>(AnchoredAttributeName, FTransformCollection::TransformGroup);
	}
	static const FName AutoInstanceMeshIndexAttributeName(TEXT("AutoInstanceMeshIndex"));
	if (GeometryCollection->HasAttribute(AutoInstanceMeshIndexAttributeName, FTransformCollection::TransformGroup))
	{
		DuplicateGeometryCollection->AddAttribute<int32>(AutoInstanceMeshIndexAttributeName, FTransformCollection::TransformGroup);
	}
	static const FName LevelAttributeName(TEXT("Level"));
	if (GeometryCollection->HasAttribute(LevelAttributeName, FTransformCollection::TransformGroup))
	{
		DuplicateGeometryCollection->AddAttribute<int32>(LevelAttributeName, FTransformCollection::TransformGroup);
	}
	static const FName VolumeAttributeName(TEXT("Volume"));
	if (GeometryCollection->HasAttribute(VolumeAttributeName, FTransformCollection::TransformGroup))
	{
		DuplicateGeometryCollection->AddAttribute<float>(VolumeAttributeName, FTransformCollection::TransformGroup);
	}
	static const FName SizeAttributeName(TEXT("Size"));
	if (GeometryCollection->HasAttribute(SizeAttributeName, FTransformCollection::TransformGroup))
	{
		DuplicateGeometryCollection->AddAttribute<float>(SizeAttributeName, FTransformCollection::TransformGroup);
	}
...
void UGeometryCollection::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyChangedEvent.Property)
	{
		...
		else if (PropertyChangedEvent.Property->GetFName() == GET_MEMBER_NAME_CHECKED(UGeometryCollection, NaniteMinimumResidencyInKB))
		{
			bDoInvalidateCollection = true;
			bRebuildRenderData = true;
		}
...
void UGeometryCollection::AppendToClassSchema(FAppendToClassSchemaContext& Context)
{
	Super::AppendToClassSchema(Context);

	FStringView RenderDataVersionString(FDerivedDataGeometryCollectionRenderDataCooker::VersionString);
	Context.Update(RenderDataVersionString.GetData(), RenderDataVersionString.Len() * sizeof(RenderDataVersionString[0]));
}

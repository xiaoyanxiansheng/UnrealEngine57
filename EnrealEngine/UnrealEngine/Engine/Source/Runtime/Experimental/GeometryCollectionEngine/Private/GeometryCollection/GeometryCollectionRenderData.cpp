...
{
	Ar << NumVertices << NumTriangles << Sections << SectionsNoInternal;
...
bool BuildMeshDataFromGeometryCollection(FGeometryCollection& InCollection, FGeometryCollectionRenderData::FCreateDesc const& InDesc, FGeometryCollectionBuiltMeshData& OutMeshData)
{
	// Vertices Group
...

	{
		...
		ParallelForWithTaskContext(Bounds, NumGeometry, [&](FBounds3f& BatchBounds, int32 GeometryGroupIndex)
		{
			...
			ParallelForWithTaskContext(TEXT("GC:BuildVertices"), GeometryBounds, VertexCount, 500, [&](FBounds3f& BatchGeometryBounds, int32 VertexIndex)
			{
				...
				if (bHasColors)
				{
					BuildVertexData.Color[DestVertexStart + VertexIndex] = ColorArray[VertexStart + VertexIndex].ToFColor(InDesc.bConvertVertexColorsToSRGB /* sRGB */);
				}
...
void CreateMeshData(FGeometryCollectionBuiltMeshData const& InMeshData, FGeometryCollectionRenderData::FCreateDesc const& InDesc, FGeometryCollectionRenderData& OutRenderData)
{
...

	{
		...
		OutRenderData.MeshResource.StaticMeshVertexBuffer.SetUseFullPrecisionUVs(InDesc.bUseFullPrecisionUVs);
...
void CreateNaniteData(FGeometryCollectionBuiltMeshData&& InMeshData, FGeometryCollectionRenderData::FCreateDesc const& InDesc, FGeometryCollectionRenderData& OutRenderData)
{
	...
	NaniteSettings.TargetMinimumResidencyInKB = InDesc.NaniteMinimumResidencyInKB;
...
TUniquePtr<FGeometryCollectionRenderData> FGeometryCollectionRenderData::Create(FGeometryCollection& InCollection, FCreateDesc const& InDesc)
{
	...
	if (BuildMeshDataFromGeometryCollection(InCollection, InDesc, MeshBuildData))
	{
		if (!InDesc.bEnableNanite || InDesc.bEnableNaniteFallback)
		{
			CreateMeshData(MeshBuildData, InDesc, *RenderData.Get());
		}

		if (InDesc.bEnableNanite)
		{
			CreateNaniteData(MoveTemp(MeshBuildData), InDesc, *RenderData.Get());
		}

		RenderData->PreSkinnedBounds = BuildPreSkinnedBounds(InCollection);
	}
...
{
#if WITH_EDITOR
	// Optionally don't cook the rendering data.
	...
	const bool bStripAllRenderDataFromCook = Owner.bStripRenderDataOnCook && bIsArchiveValidCandidateForStrip;
	const bool bStripMeshDataFromCook = bStripAllRenderDataFromCook;
	// Also strip nanite data for non-nanite platforms.
	const bool bStripNaniteDataFromCook = bStripAllRenderDataFromCook || (Ar.IsCooking() && !DoesTargetPlatformSupportNanite(Ar.CookingTarget()));
#else
	const bool bStripMeshDataFromCook = false;
	const bool bStripNaniteDataFromCook = false;
#endif

	if (bStripMeshDataFromCook)
	{
		bool bDummy = false;
		Ar << bDummy;
	}
	...
	if (bHasMeshData && !bStripMeshDataFromCook)
	{

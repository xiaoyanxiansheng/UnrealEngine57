...
struct FGeometryCollectionProxyMeshData
{
	...
	*/
	...
	/**
	* Optional arrays of override material matching each proxy Mesh
	* if an entry is missing or if the arary is empty , then no material will be assigned
	*/
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Rendering")
	TArray<FGeometryCollectionProxyMeshMaterials> MeshOverrideMaterials;
...
public:
...
#if WITH_EDITOR
	GEOMETRYCOLLECTIONENGINE_API static void AppendToClassSchema(FAppendToClassSchemaContext& Context);
	...
	*/
	...
	/**
	 * How much of the nanite resource should always be resident (In KB). Approximate due to paging. 0: Minimum size (single page). MAX_uint32: Entire mesh.
	 */
	UPROPERTY(EditAnywhere, Category = "Nanite", Meta = (EditCondition = "EnableNanite", DisplayName = "Nanite Minimum Residency"))
	uint32 NaniteMinimumResidencyInKB;

...
public:
	...
	virtual void ResolveBoundObjects(FMovieSceneSequenceID LocalSequenceID, TSharedRef<UE::MovieScene::FSharedPlaybackState> SharedPlaybackState, UObject* SubObject = nullptr)  PURE_VIRTUAL(ResolveBoundObjects);
	...
	virtual void InitConstraint(UWorld * InWorld)  PURE_VIRTUAL(UTickableConstraint::InitConstraint);
	/** Teardown and unregister the constraint with that world*/
	virtual void TeardownConstraint(UWorld* InWorld)  PURE_VIRTUAL(UTickableConstraint::DeInitConstraint);
	/** */
	virtual void AddedToWorld(UWorld* InWorld) PURE_VIRTUAL(UTickableConstraint::AddedToWorld);
...
public:
	...
	CONSTRAINTS_API void SetConstraintsDependencies(
		...
		const FGuid& InGuidToTickAfter) const;
...
public:
	...
	CONSTRAINTS_API static FOnSceneComponentConstrained& OnSceneComponentConstrained();

	/** Delegate to trigger changes in the constraints manager. */
	CONSTRAINTS_API static FConstraintsManagerNotifyDelegate& GetNotifyDelegate();

	/** Notify from changes in the constraints manager. */
	CONSTRAINTS_API static void Notify(EConstraintsManagerNotifyType InNotifyType, UObject* InObject);

...
void UTransformableComponentHandle::SetGlobalTransform(const FTransform& InGlobal) const
{
	if (Component.IsValid())
	{
		...
		{
			FConstraintsManagerController::OnSceneComponentConstrained().Broadcast(Component.Get());
		}
...
{
	USceneComponent* SceneComponent = Component.Get();
	if (!SceneComponent)
	{
	...
	if (TransformableHandleUtils::SkipTicking())
	{
		...
		const UE::Anim::FAnimationEvaluator& AnimationEvaluator = TransformableHandleUtils::EvaluateComponent(SceneComponent);
		if (AnimationEvaluator.IsValid(bValidForTransforms))
		{
			return AnimationEvaluator.GetGlobalTransform(SocketName);
		}
		
		return TransformableHandleUtils::GetGlobalTransform(SceneComponent, SocketName);
	}

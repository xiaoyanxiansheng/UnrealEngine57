...
#include UE_INLINE_GENERATED_CPP_BY_NAME(ChaosVDRecording)
...
int32 FChaosVDRecording::FindFirstSolverKeyFrameNumberFromFrame_AssumesLocked(int32 SolverID, int32 StartFrameNumber)
{
	if (TArray<int32>* KeyFrameNumbersPtr = RecordedKeyFramesNumberPerSolver.Find(SolverID))
	{
		...
		// IndexFound will be outside the array's bounds. In that case we want to use the last key frame available
...
void FChaosVDRecording::CollapseSolverFramesRange_AssumesLocked(int32 SolverID, int32 StartFrame, int32 EndFrame, FChaosVDSolverFrameData& OutCollapsedFrameData)
{
	...
	{
		auto ProcessDestroyedParticles = [](const TSet<int32>& InDestroyedParticleIDs, TMap<int32, TSharedPtr<FChaosVDParticleDataWrapper>>& InOutParticlesMap)
		{
			for (int32 DestroyedPArticleID : InDestroyedParticleIDs)
			{
				InOutParticlesMap.Remove(DestroyedPArticleID);
			}
		};
		...
		if (const FChaosVDSolverFrameData* SolverFrameData = GetSolverFrameData_AssumesLocked(SolverID, CurrentFrameNumber, bRequestingKeyFrameOnly))
		{
			...
			{
				for (const FChaosVDFrameStageData& SolverStageData : SolverFrameData->SolverSteps)
				{
					for (const TSharedPtr<FChaosVDParticleDataWrapper>& ParticleData : SolverStageData.RecordedParticlesData)
					{
						if (!ParticleData)
						{
							continue;
						...
						// Remove any particle data that was remoted in this stage
						ProcessDestroyedParticles(SolverStageData.ParticlesDestroyedIDs, ParticlesOnCurrentGeneratedKeyframe);	
					}
				}
			}

			// Remove any particle data that was remove outside a recorded stage
			ProcessDestroyedParticles(SolverFrameData->ParticlesDestroyedIDs, ParticlesOnCurrentGeneratedKeyframe);

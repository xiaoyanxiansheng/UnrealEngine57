...
bool FDependsNodeTest::RunTest(const FString& Parameters)
{
	...
	for (ESortTestType SortTestType : { ESortTestType::AlwaysSorted, ESortTestType::AlwaysUnsorted, ESortTestType::UnsortedThenSorted })
	{
		...
		FTestDependency TestDependencies[] = {
			...
			{&ScratchNodes[4], EDependencyCategory::Manage, EDependencyProperty::Direct | EDependencyProperty::CookRule},
			{&ScratchNodes[5], EDependencyCategory::Manage, EDependencyProperty::Direct},
			{&ScratchNodes[6], EDependencyCategory::Manage, EDependencyProperty::None }
		};

		constexpr int NumUsedScratchNodes = UE_ARRAY_COUNT(TestDependencies);
		static_assert(NumUsedScratchNodes <= UE_ARRAY_COUNT(ScratchNodes));
		if (SortTestType == ESortTestType::AlwaysUnsorted || SortTestType == ESortTestType::UnsortedThenSorted)
		{
			...
			for (int n = 0; n < NumUsedScratchNodes; ++n)
			{
		...
		// Manage All
		{
			FTestDependency MatchingDeps[] = { TestDependencies[4], TestDependencies[5], TestDependencies[6] };
			FDependsNode* Matching[] = { &ScratchNodes[4], &ScratchNodes[5], &ScratchNodes[6] };
			Dependencies.Reset();
			...
			A.IterateOverDependencies([this, &IterateCount, &ScratchNodes](FDependsNode* Dependency, EDependencyCategory Category, EDependencyProperty Properties, bool bDuplicate)
				{
					...
					{
						TestTrue(TEXT("IterateDependencies Manage, no flags"), Properties == (EDependencyProperty::Direct | EDependencyProperty::CookRule) && !bDuplicate);
					}
					...
					else if (Dependency == &ScratchNodes[6])
					{
		...
		// Manage Direct
		{
			...
			A.IterateOverDependencies([this, &IterateCount, &ScratchNodes](FDependsNode* Dependency, EDependencyCategory Category, EDependencyProperty Properties, bool bDuplicate)
				{
					...
					if (Dependency == &ScratchNodes[4])
					{
						TestTrue(TEXT("IterateDependencies Manage, Direct"), Properties == (EDependencyProperty::Direct | EDependencyProperty::CookRule) && !bDuplicate);
					}
					else if (Dependency == &ScratchNodes[5])
					{
			...
			TestEqual(TEXT("IterateDependencies Manage, Direct, Count"), IterateCount, 2);
		}
		// Manage Indirect
		{
			FTestDependency MatchingDeps[] = { TestDependencies[6] };
			FDependsNode* Matching[] = { &ScratchNodes[6] };
			FDependsNode* NonMatching[] = { &ScratchNodes[4], &ScratchNodes[5] };
			Dependencies.Reset();
		...
		TestEqual(TEXT("RemoveDependency in dependency and referencer filtering loop"), A.GetConnectionCount(), NumUsedScratchNodes);
		for (int n = 0; n < NumUsedScratchNodes; ++n)
		{
			ScratchNodes[n].RemoveReferencer(&A);
			ScratchNodes[n].SetIsReferencersSorted(true);
			A.RemoveDependency(&ScratchNodes[n]);
			TestEqual(TEXT("RemoveDependency in dependency and referencer filtering loop"), A.GetConnectionCount(), NumUsedScratchNodes - 1 - n);
		}
	}
	...
	// RemoveLinks
	{
		...
		for (int n = 0; n < NumLinkNodes; ++n)
		{
			...
			ManageProperties[n] |= (n & 0x2) ? EDependencyProperty::CookRule : EDependencyProperty::None;
		}
		...
		for (bool bKeepManageDependencies : { true, false})
		{
			for (bool bKeepNameDependencies : { true, false})
			{
...

				{
					...
					LoadedA.SerializeLoad<true>(Reader, GetNodeFromSerializeIndex, LoadScratch, FAssetRegistryVersion::LatestVersion);
					if (KeepNode2 == EKeepNode2::KeepAlways)
					{
						LoadedB.SerializeLoad<true>(Reader, GetNodeFromSerializeIndex, LoadScratch, FAssetRegistryVersion::LatestVersion);
					}
				}

...
template<typename RealType, int ElementSize>
void TDynamicMeshOverlay<RealType, ElementSize>::CreatePerVertex(RealType InitElementValue)
{
	ClearElements(); // deletes all elements and initializes triangles to be 1:1 w/ parentmesh IDs
	TArray<int32> VIDtoEID;
	bool bIsCompactV = ParentMesh->IsCompactV();
	if (!bIsCompactV)
	{
	...
	{
		int32 ElementID = AppendElement(InitElementValue);
	...
	for (int32 TriangleID : ParentMesh->TriangleIndicesItr())
	{
		...
		for (int32 SubIdx = 0; SubIdx < 3; ++SubIdx)
		{
			...
			int32 EID = bIsCompactV ? VID : VIDtoEID[VID];
...
void TDynamicMeshOverlay<RealType, ElementSize>::SplitBowties(bool bParallel)
{
	...
	if (bParallel)
	{
		// Bowties are typically rare, so we can test for bowties in parallel and only run the split on the (hopefully) few needed cases
		TArray<int32> BowtieVerts;
		FMutex BowtieCS;
		ParallelFor(ParentMesh->MaxVertexID(), [this, &BowtieVerts, &BowtieCS](int32 VertexID)
			{
...
void TDynamicMeshOverlay<RealType, ElementSize>::SplitBowtiesAtVertex(int32 VertexID, TArray<int32>* NewElementIDs)
{
	...
	FDynamicMesh3::FLocalIntArray GroupElementIDs, // stores element IDs of this vertex, 1:1 w/ contiguous triangles in the parent mesh

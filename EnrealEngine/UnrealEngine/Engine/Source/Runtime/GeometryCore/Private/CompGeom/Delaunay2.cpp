...
namespace DelaunayInternal
{
	...
	template<typename RealType>
	void Update(FRandomStream& Random, FDelaunay2Connectivity& Connectivity,
		TArrayView<const TVector2<RealType>> Vertices, TArray<int32> VertexOrder, FIndex3i StartSearchTri, bool bAssumeDelaunay)
	{
		FIndex3i SearchTri = StartSearchTri;
		for (int32 OrderIdx = 0; OrderIdx < VertexOrder.Num(); OrderIdx++)
		{
			int32 Vertex = VertexOrder[OrderIdx];
			int32 DuplicateOf = -1;
		...
		TArrayView<const TVector2<RealType>> Vertices, TArrayView<const FIndex2i> Edges, bool bKeepFastEdgeAdjacencyData, FDelaunay2::EResult& OutResult)
	{
		...
		Update(Random, Connectivity, Vertices, Order, FirstTri, bAssumeDelaunay);
...
bool FDelaunay2::Update(TArrayView<const TVector2<double>> Vertices, int32 FirstNewIdx)
{
	// Check if a triangulation already exists
	if (Connectivity->NumTriangles() == 0)
	{
		return false;
	}

	// Construct BRIO for new vertices
	FBRIOPoints InsertOrder;
	InsertOrder.Compute(Vertices.RightChop(FirstNewIdx));
	TArray<int32>& Order = InsertOrder.Order;
	for (int32& VertexIndex : Order)
	{
		VertexIndex += FirstNewIdx;
	}

	// Get any existing triangle as a bootstrap one
	FIndex3i SearchTri;
	Connectivity->EnumerateTriangles([&SearchTri](const FIndex2i& Edge, int32 Vertex) -> bool
	{
		SearchTri.A = Vertex;
		SearchTri.B = Edge.A;
		SearchTri.C = Edge.B;
	...
	bool bAssumeDelaunay = !bIsConstrained; // this may have previously constrained edges in the original triangulation so we cannot assume Delaunay
	DelaunayInternal::Update<double>(RandomStream, *Connectivity, Vertices, Order, SearchTri, bAssumeDelaunay);
	return true;
}

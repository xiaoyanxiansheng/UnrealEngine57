...
bool FMeshBoundaryLoops::Compute()
{
	...
	for (int eid = 0; eid < NE; ++eid)
	{
		...
		if (bIsOpenSpan || bOnlyComputeSpans)
		{
			bSawOpenSpans = true;
...
{
	const int32 NumSpans = Spans.Num();
	for (int32 SpanIndex = 0; SpanIndex < NumSpans; ++SpanIndex)
	{
		if (Spans[SpanIndex].Edges.Contains(EdgeID))
		{
			return SpanIndex;
		}
	}

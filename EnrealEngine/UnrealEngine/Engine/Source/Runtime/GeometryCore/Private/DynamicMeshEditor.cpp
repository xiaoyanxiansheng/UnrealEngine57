	...
	bool bReverseOrientation)
{
	...
	if (AppendMesh->HasAttributes() && Mesh->HasAttributes())
	{
		...
		for (const TPair<FName, TUniquePtr<FDynamicMeshMorphTargetAttribute>>& AttribPair : AppendMesh->Attributes()->GetMorphTargetAttributes())
		{
			FDynamicMeshMorphTargetAttribute* ToAttrib = Mesh->Attributes()->GetMorphTargetAttribute(AttribPair.Key);
			if (ToAttrib)
			{
				ToAttrib->CopyThroughMapping(AttribPair.Value.Get(), IndexMapsOut);	
			}
...
static void AppendVertexAttributes(const FDynamicMesh3* FromMesh, FDynamicMesh3* ToMesh, FMeshIndexMappings& IndexMaps)
{
	...
	for (const TPair<FName, TUniquePtr<FDynamicMeshMorphTargetAttribute>>& AttribPair : FromMesh->Attributes()->GetMorphTargetAttributes())
	{
		FDynamicMeshMorphTargetAttribute* ToAttrib = ToMesh->Attributes()->GetMorphTargetAttribute(AttribPair.Key);
		if (ToAttrib)
		{
			ToAttrib->CopyThroughMapping(AttribPair.Value.Get(), IndexMaps);
		}

...
{
	FCustomInsideMeshTest WindingTest(
		[Threshold = WindingThreshold](FVector3d Pt, const FCustomInsideTestContext& Context) -> bool
		{
			return Context.OptionalWindingTree->IsInside(Pt, Threshold);
		}, true);
	return ComputeWithCustomInside(WindingTest, WindingTest);
}

bool FMeshBoolean::ComputeWithCustomInside(FCustomInsideMeshTest InsideMeshA, FCustomInsideMeshTest InsideMeshB)
{
	// copy meshes
	...
	{ // (just for scope)
		...
		for (int MeshIdx = 0; MeshIdx < NumMeshesToProcess; MeshIdx++)
		{
			FCustomInsideMeshTest& UseInsideTest = MeshIdx == 0 ? InsideMeshB : InsideMeshA;
			FDynamicMeshAABBTree3& OtherSpatial = Spatial[1 - MeshIdx];
			if (!OtherSpatial.IsValid(false))
			{
				OtherSpatial.Build();
			}
			TFastWindingTree<FDynamicMesh3> LocalWinding(&OtherSpatial, UseInsideTest.bRequiresWinding);
			TFastWindingTree<FDynamicMesh3>* UseWinding = UseInsideTest.bRequiresWinding ? &LocalWinding : nullptr;
			FCustomInsideTestContext InsideTestContext
			{
				.Spatial = &OtherSpatial,
				.OptionalWindingTree = UseWinding
			};
			...
			ParallelFor(MaxTriID, [&](int TID)
				{
					...
					KeepTri[MeshIdx][TID] = (UseInsideTest.IsPointInsideFn(Centroid, InsideTestContext)) != bRemoveInside;
				});
		}
	...
	if (NumMeshesToProcess > 1)
	{
		...
		if (TrackPerTriangleSourceMesh.IsSet())
		{
			TrackPerTriangleSourceMesh->SetNumZeroed(Result->MaxTriangleID());
			for (const TPair<int32, int32> TriSrcDst : IndexMaps.GetTriangleMap().GetForwardMap())
			{
				(*TrackPerTriangleSourceMesh)[TriSrcDst.Value] = 1;
			}
		}

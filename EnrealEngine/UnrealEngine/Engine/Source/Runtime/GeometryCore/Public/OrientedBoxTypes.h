...
struct TOrientedBox3
{
	...
	/** @return true if box is valid (has non-negative extents) */
	bool IsValid() const
	{
	...
	RealType SignedDistance(TVector<RealType> Point) const
	{
	 ...
	 /**
	  * Transform the frame of the oriented box so its local axes are closer to the corresponding world axes
	  * (Does not change the shape of the box, just re-orders the axes if needed)
	  */
	 [[nodiscard]] GEOMETRYCORE_API TOrientedBox3<RealType> ReparameterizedCloserToWorldFrame() const;

	 /**
	  * @param OtherBox An oriented box to test for intersection against this box
	  * @param ParallelAxesTolerance Tolerance for handling the special case where the box axes are nearly parallel
	 ...
	 /**
	  * @param OtherBox An axis-aligned box to test for intersection against this box
	  * @param ParallelAxesTolerance Tolerance for handling the special case where the box axes are nearly parallel
	  * @return Whether the boxes overlap
	  */

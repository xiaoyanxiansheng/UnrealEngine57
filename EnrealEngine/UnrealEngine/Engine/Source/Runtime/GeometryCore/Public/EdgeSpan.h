...
public:
	 ...
	 * @param AngleThresholdDeg max angle between adjacant edges to mark a vertex as a "corner" (in degrees). Angle is measured between edge vectors extending from their
	 *							common vertex, so a value of 180 means all vertices would be labelled as corners.
	 * @param MinSpanSize minimum number of edges necessary for a subspan, regardless of angles between edges. Subspans that would be too small are merged to adjacent subspans.
	 * @param OutSpan output subspans
	 */

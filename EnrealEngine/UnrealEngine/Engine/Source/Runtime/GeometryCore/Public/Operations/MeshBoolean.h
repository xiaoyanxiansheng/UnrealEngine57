...
public:
	...
	// If set, will be populated with an array of source mesh indices per TriangleID in the result mesh,
	// indicating if that triangle was from the Meshes[0] or Meshes[1] source.
	// Note: For non-compact meshes, invalid triangle IDs will have arbitrary indices.
...
public:
	...
	{
		// AABB Tree for the test mesh; always non-null
		FDynamicMeshAABBTree3* Spatial;
		// Note: OptionalWindingTree will be null when FCustomInsideMeshTest has bRequiresWinding == false
		TFastWindingTree<FDynamicMesh3>* OptionalWindingTree;
	};

	// Function type for custom inside/outside query functions
	// Note: If implementing a custom function that needs to access the underlying mesh, it is accessible via Context.Spatial->GetMesh()
	...
	struct FCustomInsideMeshTest
	{
		...
		/**
		 * Construct a custom class for testing whether a point is inside a mesh.
		 * Note: This will be called in the local space of the boolean, which is not the initial / world-space of the input meshes
		 * 
		 * @param IsPointInsideFn Custom function reference to test if a given point is inside a mesh.
		 * @param bRequiresWinding Whether the winding tree needs to be computed for the custom function. If false, the OptionalWindingTree passed to IsPointInsideFn will be null.
		 */
		FCustomInsideMeshTest(TestInsideFunctionType IsPointInsideFn, bool bRequiresWinding = true)
			: IsPointInsideFn(IsPointInsideFn), bRequiresWinding(bRequiresWinding)
		{}
	};

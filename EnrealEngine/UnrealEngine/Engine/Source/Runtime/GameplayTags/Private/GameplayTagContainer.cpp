...
private:
	...
	bool EvalAnyTagsExactMatch(FGameplayTagContainer const& Tags, bool bSkip);
	bool EvalAllTagsExactMatch(FGameplayTagContainer const& Tags, bool bSkip);
...
bool FQueryEvaluator::EvalAnyTagsExactMatch(FGameplayTagContainer const& Tags, bool bSkip)
{
	bool bShortCircuit = bSkip;
	bool Result = false;
	...
	for (int32 Idx = 0; Idx < NumTags; ++Idx)
	{
		...
		if (bShortCircuit == false)
		{
			...
			bool bHasTag = Tags.HasTagExact(Tag);
...
bool FQueryEvaluator::EvalAllTagsExactMatch(FGameplayTagContainer const& Tags, bool bSkip)
{
	bool bShortCircuit = bSkip;
	...
	for (int32 Idx = 0; Idx < NumTags; ++Idx)
	{
		...
		{
			FGameplayTag const Tag = Query.GetTagFromIndex(TagIdx);
			bool const bHasTag = Tags.HasTagExact(Tag);
...
bool FGameplayTagContainer::NetSerialize(FArchive& Ar, class UPackageMap* Map, bool& bOutSuccess)
{
	...
	if (Ar.IsSaving())
	{
		uint16 NumTags = IntCastChecked<uint16, int32>(GameplayTags.Num());
		const uint16 MaxSize = (1 << NumBitsForContainerSize) - 1;
		if (!ensureMsgf(GameplayTags.Num() <= MaxSize, TEXT("TagContainer has %d elements when max is %d! Tags: %s"), GameplayTags.Num(), MaxSize, *ToStringSimple()))
		{
			NumTags = MaxSize;
		}
...
bool FGameplayTag::MatchesTag(const FGameplayTag& TagToCheck) const
{
	...
	ensureMsgf(!IsValid(), TEXT("MatchesTag called on an invalid gameplay tag %s, only registered tags can be used in containers"), *GetTagName().ToString());
...
bool FGameplayTag::MatchesAny(const FGameplayTagContainer& ContainerToCheck) const
{
	...
	// If a non-empty tag has not been registered, it will not exist in the tag database so this function may return the incorrect value
	// All tags must be registered from code or data before being used in matching functions and this tag may have been deleted with active references
	ensureMsgf(!IsValid(), TEXT("MatchesAny called on an invalid gameplay tag %s, only registered tags can be used in containers"), *GetTagName().ToString());
...
UEditableGameplayTagQueryExpression* FQueryEvaluator::ReadEditableQueryExpr(UObject* ExprOuter)
{
	...
	case EGameplayTagQueryExprType::NoExprMatch:
		...
		break;
	case EGameplayTagQueryExprType::AnyTagsExactMatch:
		ExprClass = UEditableGameplayTagQueryExpression_AnyTagsExactMatch::StaticClass();
		break;
	case EGameplayTagQueryExprType::AllTagsExactMatch:
		ExprClass = UEditableGameplayTagQueryExpression_AllTagsExactMatch::StaticClass();
		break;
	}
...
void FQueryEvaluator::ReadEditableQueryTags(UEditableGameplayTagQueryExpression* EditableQueryExpr)
{
	...
	else if (EditableQueryExpr->IsA(UEditableGameplayTagQueryExpression_AnyTagsExactMatch::StaticClass()))
	{
		Tags = &((UEditableGameplayTagQueryExpression_AnyTagsExactMatch*)EditableQueryExpr)->Tags;
	}
	else if (EditableQueryExpr->IsA(UEditableGameplayTagQueryExpression_AllTagsExactMatch::StaticClass()))
	{
		Tags = &((UEditableGameplayTagQueryExpression_AllTagsExactMatch*)EditableQueryExpr)->Tags;
	}
	ensure(Tags);
...
void UEditableGameplayTagQueryExpression_AnyTagsExactMatch::EmitTokens(TArray<uint8>& TokenStream, TArray<FGameplayTag>& TagDictionary,
	FString* DebugString) const
{
	TokenStream.Add((uint8)EGameplayTagQueryExprType::AnyTagsExactMatch);

	if (DebugString)
	{
		DebugString->Append(TEXT(" ANY_EXACT("));
	}
...
void UEditableGameplayTagQueryExpression_AllTagsExactMatch::EmitTokens(TArray<uint8>& TokenStream, TArray<FGameplayTag>& TagDictionary,
	FString* DebugString) const
{
	TokenStream.Add((uint8)EGameplayTagQueryExprType::AllTagsExactMatch);

	if (DebugString)
	{
		DebugString->Append(TEXT(" ALL_EXACT("));
	}

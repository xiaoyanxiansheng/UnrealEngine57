...
struct FGameplayTag
{
	...
	inline friend uint32 GetTypeHash(const FGameplayTag& Tag)
	{
...
struct FGameplayTagContainer
{
	 ...
	 */
	inline bool HasTag(const FGameplayTag& TagToCheck) const
	{
		if (!TagToCheck.IsValid())
		{
			return false;
	 ...
	 * @return True if TagToCheck is in this container, false if it is not
	 */
	inline bool HasTagExact(const FGameplayTag& TagToCheck) const
	{
		if (!TagToCheck.IsValid())
		{
			return false;
	 ...
	 * @return True if this container has ANY of the tags of in ContainerToCheck
	 */
	inline bool HasAnyExact(const FGameplayTagContainer& ContainerToCheck) const
	{
		if (ContainerToCheck.IsEmpty())
		{
			return false;
	 ...
	 */
	inline bool HasAll(const FGameplayTagContainer& ContainerToCheck) const
	{
		if (ContainerToCheck.IsEmpty())
		{
			return true;
	 ...
	 * @return True if this container has ALL of the tags of in ContainerToCheck, including if ContainerToCheck is empty
	 */
	inline bool HasAllExact(const FGameplayTagContainer& ContainerToCheck) const
	{
		if (ContainerToCheck.IsEmpty())
		{
			return true;
...
private:
	...
	inline friend TArray<FGameplayTag>::TConstIterator begin(const FGameplayTagContainer& Array) { return Array.CreateConstIterator(); }
	inline friend TArray<FGameplayTag>::TConstIterator end(const FGameplayTagContainer& Array) { return TArray<FGameplayTag>::TConstIterator(Array.GameplayTags, Array.GameplayTags.Num()); }
};
...
struct FGameplayTagQueryExpression
{
	...
	inline bool UsesTagSet() const
	{
		return (ExprType == EGameplayTagQueryExprType::AllTagsMatch) || (ExprType == EGameplayTagQueryExprType::AnyTagsMatch) || (ExprType == EGameplayTagQueryExprType::NoTagsMatch) || (ExprType == EGameplayTagQueryExprType::AnyTagsExactMatch) || (ExprType == EGameplayTagQueryExprType::AllTagsExactMatch);
	}
	/** Returns true if this expression uses the expression list data. */
	inline bool UsesExprSet() const
	{
...
UCLASS(BlueprintType, editinlinenew, collapseCategories, meta = (DisplayName = "Any Tags Exact Match"))
class UEditableGameplayTagQueryExpression_AnyTagsExactMatch : public UEditableGameplayTagQueryExpression
{
	GENERATED_BODY()
public:
	UPROPERTY(EditDefaultsOnly, Category = Expr)
...
UCLASS(BlueprintType, editinlinenew, collapseCategories, meta = (DisplayName = "All Tags Exact Match"))
class UEditableGameplayTagQueryExpression_AllTagsExactMatch : public UEditableGameplayTagQueryExpression
{
	GENERATED_BODY()
public:
	UPROPERTY(EditDefaultsOnly, Category = Expr)

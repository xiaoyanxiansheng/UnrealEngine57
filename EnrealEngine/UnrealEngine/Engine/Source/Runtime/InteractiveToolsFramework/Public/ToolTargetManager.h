...
public:
	...
	/**
	 * Remove existing ToolTargetFactories by predicate
	 */
	INTERACTIVETOOLSFRAMEWORK_API virtual void RemoveTargetFactoriesByPredicate(TFunctionRef<bool(UToolTargetFactory*)> Predicate);
	...
	/** Additional filter function that will be applied in CanBuildTarget and BuildTarget. */
	virtual void SetInputFilterFunction(TFunction<bool(const UObject*)> ShouldAllowBuildingFromInputObject) 
	{ 
		InputFilterFunction = ShouldAllowBuildingFromInputObject; 
	}
...
private:
	TFunction<bool(const UObject*)> InputFilterFunction;
};

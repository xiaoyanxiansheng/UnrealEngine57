...
public:
	...
	UPROPERTY(EditAnywhere, Category = Brush, meta = (DisplayName = "Size", UIMin = "0.0", UIMax = "1.0", ClampMin = "0.0", ClampMax = "10.0", DisplayPriority = 1, HideEditConditionToggle, EditCondition = "bSpecifyRadius == false", ModelingQuickSettings = 100))
	float BrushSize;
	...
	//~Set by tool and determines whether the bEnablePressureSensitivity flag below is shown and used.
	...
	/** When set, stylus pressure is used to adjust radius */
	UPROPERTY(EditAnywhere, Category = Brush, meta = (EditCondition = "bToolSupportsPressureSensitivity", EditConditionHides))
	bool bEnablePressureSensitivity = false;

	/** Strength of the brush (0.0 - 1.0) */
	UPROPERTY(EditAnywhere, Category = Brush, meta = (DisplayName = "Strength",
		UIMin = "0.0", UIMax = "1.0", ClampMin = "0.0", ClampMax = "1.0", 
		HideEditConditionToggle, EditConditionHides, EditCondition = "bShowStrength", DisplayPriority = 2,
		ModelingQuickEdit, ModelingQuickSettings = 200))
	float BrushStrength;

	/** Amount of falloff to apply (0.0 - 1.0) */
	UPROPERTY(EditAnywhere, Category = Brush, meta = (DisplayName = "Falloff", 
		UIMin = "0.0", UIMax = "1.0", ClampMin = "0.0", ClampMax = "1.0", 
		HideEditConditionToggle, EditConditionHides, EditCondition="bShowFalloff", DisplayPriority = 3,
		ModelingQuickEdit, ModelingQuickSettings = 300))
	float BrushFalloffAmount;


	/** Allow the Brush to hit the back-side of the mesh */
	UPROPERTY(EditAnywhere, Category = Brush, meta = (DisplayPriority = 6, HideEditConditionToggle, EditConditionHides, EditCondition = "bShowHitBackFaces"))
	...
	/** If false, then bHitBackFaces will not be shown in DetailsView panels (otherwise no effect) */
	UPROPERTY(meta = (TransientToolProperty))
	bool bShowHitBackFaces = false;
};
...
public:
	...
	INTERACTIVETOOLSFRAMEWORK_API virtual void OnTick(float DeltaTime) override;
...
public:
	...
	INTERACTIVETOOLSFRAMEWORK_API virtual double GetCurrentBrushRadius() const;
	virtual double GetCurrentBrushRadiusLocal() const { return GetCurrentBrushRadius() * WorldToLocalScale; }

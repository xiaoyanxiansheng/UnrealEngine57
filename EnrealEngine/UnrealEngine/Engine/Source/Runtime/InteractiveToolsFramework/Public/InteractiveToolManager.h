...
public:
	...
	/**
	 * Determines the preferred shutdown type for the active tool based on the tool's type and the tool manager's configuration.
	 */
	INTERACTIVETOOLSFRAMEWORK_API EToolShutdownType GetPreferredShutdownType(EToolSide Side,
		EToolShutdownReason ShutdownReason = EToolShutdownReason::Unspecified);
	...
	/**
	 * Returns how tool changes emit change events. See EToolChangeTrackingMode for details.
	 */
	INTERACTIVETOOLSFRAMEWORK_API virtual EToolChangeTrackingMode GetChangeTrackingMode() const;

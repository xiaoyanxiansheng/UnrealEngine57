	...
	if (bVisibleViewDependent && Vertices.Num() > 1)
	{
		...
		if(bDrawLineStrip)
		{
			for(int32 Index = 0; Index < ProjectedVertices.Num() - 1; Index++)
			{
				...
				PDI->DrawLine(A, B, LineColor, SDPG_Foreground, CurrentLineThickness, LineDepthBias, bScreenSpaceLine);
			}
		}
		else
		{
...
FInputRayHit UGizmoElementLineStrip::LineTrace(const UGizmoViewContext* ViewContext, const FLineTraceTraversalState& LineTraceState, const FVector& RayOrigin, const FVector& RayDirection, FLineTraceOutput& OutLineTraceOutput)
{

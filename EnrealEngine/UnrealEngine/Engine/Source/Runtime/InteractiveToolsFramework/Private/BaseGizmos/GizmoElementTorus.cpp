	...
	if (bVisibleViewDependent)
	{
		if (const UMaterialInterface* UseMaterial = CurrentRenderState.GetCurrentMaterial())
		{
			...
			DrawTorus(PDI, CurrentRenderState.LocalToWorldTransform.ToMatrixWithScale(),
				BeginAxis, TorusSideAxis, Color, GetCurrentRadius(), GetCurrentInnerRadius(), NumSegments, NumInnerSlices,  
...
//
FInputRayHit UGizmoElementTorus::LineTrace(const UGizmoViewContext* ViewContext, const FLineTraceTraversalState& LineTraceState, const FVector& RayOrigin, const FVector& RayDirection, FLineTraceOutput& OutLineTraceOutput)
{
	...
	if (bHittableViewDependent)
	{
		...
		const double WorldOuterRadius = GetCurrentRadius() * CurrentLineTraceState.LocalToWorldTransform.GetScale3D().X;
		const double WorldInnerRadius = GetCurrentInnerRadius() * CurrentLineTraceState.LocalToWorldTransform.GetScale3D().X;
		...
		if (IsGlancingAngle(ViewContext, WorldCenter, WorldNormal))
		{
			...
			const double ExactCylinderRadius = FMath::Max(CurrentLineTraceState.PixelToWorldScale, WorldInnerRadius + MinimumPixelHitThresholdAdjust);
			...
			for (int32 i = 0; i < NumCylinders; i++)
			{
				...
				if (bIntersects && (HitDepth < UE_DOUBLE_SMALL_NUMBER || HitDepth > RayParam))
				{
					HitDepth = RayParam;
		...
		else
		{
			...
			bIsExactHit = (Distance <= FMath::Max(CurrentLineTraceState.PixelToWorldScale * 2.0f, WorldInnerRadius));
		...
		if (HitDepth >= UE_DOUBLE_SMALL_NUMBER)
		{
			...
			OutLineTraceOutput.bIsSurfaceHit = bIsExactHit;
...
double UGizmoElementTorus::GetCurrentInnerRadius() const
{
	auto GetMultipliedInnerRadius = [this](const float InMultiplier) -> float
	{
		return (InnerRadius > UE_SMALL_NUMBER ? InnerRadius * InMultiplier : InMultiplier);
	};

	return GetMultipliedInnerRadius(InnerRadiusMultiplier.GetValueForState(ElementInteractionState));
}
...
void UGizmoElementTorus::SetHoverInnerRadiusMultiplier(double InHoverInnerRadiusMultiplier)
{
	InnerRadiusMultiplier.Hover = InHoverInnerRadiusMultiplier;
}

double UGizmoElementTorus::GetHoverInnerRadiusMultiplier() const
{
	return InnerRadiusMultiplier.GetHoverValue();
}

void UGizmoElementTorus::SetInteractInnerRadiusMultiplier(double InInteractInnerRadiusMultiplier)
{
	InnerRadiusMultiplier.Interact = InInteractInnerRadiusMultiplier;
}

double UGizmoElementTorus::GetInteractInnerRadiusMultiplier() const
{
	return InnerRadiusMultiplier.GetInteractValue();
}

void UGizmoElementTorus::SetSelectInnerRadiusMultiplier(double InSelectInnerRadiusMultiplier)
{
	InnerRadiusMultiplier.Select = InSelectInnerRadiusMultiplier;
}

double UGizmoElementTorus::GetSelectInnerRadiusMultiplier() const
{
	return InnerRadiusMultiplier.GetSelectValue();
}

void UGizmoElementTorus::SetSubdueInnerRadiusMultiplier(double InSubdueInnerRadiusMultiplier)
{
	InnerRadiusMultiplier.Subdue = InSubdueInnerRadiusMultiplier;
}

double UGizmoElementTorus::GetSubdueInnerRadiusMultiplier() const
{
	return InnerRadiusMultiplier.GetSubdueValue();
}

...
bool UInteractiveToolManager::ActivateTool(EToolSide Side)
{
	...
	if (ActiveLeftTool != nullptr)
	{
		DeactivateTool(EToolSide::Left, GetPreferredShutdownType(EToolSide::Left, EToolShutdownReason::SwitchTool));
	}
...
EToolShutdownType UInteractiveToolManager::GetPreferredShutdownType(EToolSide Side, EToolShutdownReason ShutdownReason)
{
	...
	case EToolManagerToolSwitchMode::CustomizableAcceptIfAble:
		ShutdownType = 
			CanAcceptActiveTool(Side) ? EToolShutdownType::Accept : 
			CanCancelActiveTool(Side) ? EToolShutdownType::Cancel : EToolShutdownType::Completed;
		break;
	case EToolManagerToolSwitchMode::CancelIfAble:
	case EToolManagerToolSwitchMode::CustomizableCancelIfAble:
		ShutdownType = CanCancelActiveTool(Side) ? EToolShutdownType::Cancel : EToolShutdownType::Completed;
		break;
	default:
		ensure(false);
	}
...
EToolChangeTrackingMode UInteractiveToolManager::GetChangeTrackingMode() const
{
	return ActiveToolChangeTrackingMode;
}

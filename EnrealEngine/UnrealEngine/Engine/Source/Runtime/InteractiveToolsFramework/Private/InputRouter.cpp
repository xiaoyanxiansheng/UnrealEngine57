...
void UInputRouter::HandleCapturedKeyboardInput(const FInputDeviceState& Input)
{
	...
	// Perform Capture Stealing if needed
	bool bStoleCapture = false;
	if (ActiveKeyboardCapture->AllowsCaptureStealing())
	{
		TArray<FInputCaptureRequest> CaptureRequests;
		ActiveInputBehaviors->CollectWantsCapture(Input, CaptureRequests);
		...
		if (!CaptureRequests.IsEmpty())
		{
			...
			const FInputCaptureRequest& TopRequest = CaptureRequests[0];
			UInputBehavior* const TopCapture = TopRequest.Source;

			if (TopCapture != ActiveKeyboardCapture)
			{
				if (TopCapture->GetPriority() < ActiveKeyboardCapture->GetPriority())
				{
					...
					FInputCaptureUpdate Result = TopRequest.Source->BeginCapture(Input, EInputCaptureSide::Left);
					if (Result.State == EInputCaptureState::Begin)
					{
						ActiveKeyboardCapture = Result.Source;
						ActiveKeyboardCaptureOwner = TopRequest.Owner;
						ActiveKeyboardCaptureData = Result.Data;
					}
...
void UInputRouter::HandleCapturedMouseInput(const FInputDeviceState& Input)
{
	if (Input.IsFromDevice(EInputDevices::Mouse))
	{
		// Calling this to make sure behaviors treating mouse buttons as modifiers are always updated,
		// even if there is some drag-based behavior currently capturing
		TArray<FInputCaptureRequest> CaptureRequests;
		ActiveInputBehaviors->CollectWantsCapture(Input, CaptureRequests);
	}
	...
	// Perform Capture Stealing if needed
	bool bStoleCapture = false;
	if (ActiveLeftCapture->AllowsCaptureStealing())
	{
		TArray<FInputCaptureRequest> CaptureRequests;
		ActiveInputBehaviors->CollectWantsCapture(Input, CaptureRequests);
		...
		if (!CaptureRequests.IsEmpty())
		{
			...
			if (TopCapture != ActiveLeftCapture)
			{
				if (TopCapture->GetPriority() < ActiveLeftCapture->GetPriority())
				{
					...
					{
						ActiveLeftCapture = Result.Source;
						ActiveLeftCaptureOwner = TopRequest.Owner;
						ActiveLeftCaptureData = Result.Data;
					}

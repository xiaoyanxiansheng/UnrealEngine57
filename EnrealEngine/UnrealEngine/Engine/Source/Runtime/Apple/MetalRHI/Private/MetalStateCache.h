...
#pragma mark - Public Shader Resource Mutators -
	 ...
	 */
	void SetShaderBuffer(EMetalShaderStages const Frequency, FMetalAccelerationStructure* AccelerationStructure, NS::UInteger const Index, TArray<TTuple<MTL::Resource*, MTL::ResourceUsage>> BLAS);
#endif
...
private:
    ...
    void Validate();
    bool ValidateFunctionBindings(FMetalShaderPipeline* Pipeline, EMetalShaderFrequency Frequency, uint8 SideTableIndex);
...
#pragma mark - Private Type Declarations -
	struct FMetalBufferBinding
	{
		...
		FMetalBufferData* Bytes = nullptr;
...
private:
	void InternalResetStageBindings();

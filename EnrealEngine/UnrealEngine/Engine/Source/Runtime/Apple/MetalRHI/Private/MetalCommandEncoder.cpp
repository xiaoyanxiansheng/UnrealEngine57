...
void FMetalCommandEncoder::BeginAccelerationStructureCommandEncoding(void)
{
	...
	AccelerationStructureCommandEncoder = NS::RetainPtr(CommandBuffer->GetMTLCmdBuffer()->accelerationStructureCommandEncoder());
	
	if(GetEmitDrawEvents())
	{
		Label = FStringToNSString(FString::Printf(TEXT("AccelerationStructureEncoder: %s"), DebugGroups.Num() > 0 ? *NSStringToFString(DebugGroups.Last()) : TEXT("InitialPass")));
...
#if METAL_RHI_RAYTRACING
void FMetalCommandEncoder::SetShaderAccelerationStructure(MTL::FunctionType const FunctionType, MTLAccelerationStructurePtr AccelerationStructure, NS::UInteger const index)
{
...
void FMetalCommandEncoder::SetShaderBuffer(MTL::FunctionType const FunctionType, FMetalBufferPtr Buffer, NS::UInteger const Offset, NS::UInteger const Length, NS::UInteger index, MTL::ResourceUsage const Usage, EPixelFormat const Format, NS::UInteger const ElementRowPitch, TArray<TTuple<MTL::Resource*, MTL::ResourceUsage>> ReferencedResources)
{
    ...
    if(Buffer && (ShaderBuffers[uint32(FunctionType)].Bound & (1 << index)) &&
...
void FMetalCommandEncoder::SetShaderTexture(MTL::FunctionType FunctionType, MTL::Texture* Texture, NS::UInteger index, MTL::ResourceUsage Usage)
{
	...
	if (Texture)
	{
		...
		if (Texture->pixelFormat() == MTL::PixelFormatX32_Stencil8 || Texture->pixelFormat() == MTL::PixelFormatX24_Stencil8)
		{
...
#if METAL_RHI_RAYTRACING
	MTLAccelerationStructurePtr AS = ShaderBuffers[uint32(Function)].AccelerationStructure[Index];
	if (AS)
	{
		switch (Function)
		{
		case MTL::FunctionTypeKernel:
			...
			check(ComputeCommandEncoder);
			ComputeCommandEncoder->useResource(AS.get(), MTL::ResourceUsageRead);
			ComputeCommandEncoder->setAccelerationStructure(AS.get(), Index);
			break;
		default:
			checkNoEntry();
			break;

...
, Storage((MTL::StorageMode)((Options & MTL::ResourceStorageModeMask) >> MTL::ResourceStorageModeShift))
{
	...
	// If we are using heaps use shared storage
	if(bSupportsHeaps)
	{
		Storage = MTL::StorageModeShared;
		Options = (Options & ~MTL::ResourceStorageModeMask) | (MTL::ResourceStorageModeShared << MTL::ResourceStorageModeShift);
	}
	else
	{
		Options = (MTL::ResourceOptions)FMetalCommandQueue::GetCompatibleResourceOptions(Options);
...
MTLTexturePtr FMetalTexturePool::CreateTexture(FMetalDevice& Device, MTL::TextureDescriptor* Desc)
{
	...
	check(Descriptor.usage != MTL::TextureUsageUnknown);
...
void FMetalResourceHeap::ReleaseBuffer(FMetalBuffer* Buffer)
{
	...
	if (It && *It != UINT32_MAX)
	{
		...
		static bool bSupportsHeaps = Device.SupportsFeature(EMetalFeaturesHeaps);
		if (GMetalResourcePurgeOnDelete && !bSupportsHeaps)
		{
...
FMetalAccelerationStructure* FMetalResourceHeap::CreateAccelerationStructure(uint32 Size)
{
	LLM_SCOPE_METAL(ELLMTagMetal::AccelerationStructures);
	LLM_PLATFORM_SCOPE_METAL(ELLMTagMetal::AccelerationStructures);

	static bool bSupportsHeaps = Device.SupportsFeature(EMetalFeaturesHeaps);
	check(bSupportsHeaps);
	...
	
	{
		...
		MTL::SizeAndAlign SizeAndAlign = Device.GetDevice()->heapAccelerationStructureSizeAndAlign(Size);
		uint32 ASStructSize = SizeAndAlign.size;
		uint32 ASStructAlignment = SizeAndAlign.align;
		
		uint32_t ResourceOptions = MTL::ResourceStorageModePrivate | MTL::ResourceHazardTrackingModeTracked;
		
		auto BlockIt = FindOrAllocateBlock(ASStructSize, ASStructAlignment, ResourceOptions);
		MemoryBlock& Block = *BlockIt;
		
		check(Block.Resource == nil && Block.Size == ASStructSize);
		...
		AccelerationStructure = NS::TransferPtr(Block.Heap->newAccelerationStructure(ASStructSize, Block.Offset));
		
		AllocationHandlesLUT.Add(AccelerationStructure.get(), HeapAllocationHandle);
		
		Block.Resource = AccelerationStructure.get();
		Block.Options = ResourceOptions;
	}
	
	FMetalAccelerationStructure* AS = new FMetalAccelerationStructure(AccelerationStructure, Size);
	
	return AS;
}

void FMetalResourceHeap::ReleaseAccelerationStructure(FMetalAccelerationStructure* AccelerationStructure)
{
	if (AccelerationStructure)
	{
		FScopeLock ScopeLock(&InUseResourcesCS);
		
		MTLAccelerationStructurePtr AS = AccelerationStructure->GetAccelerationStructure();
		if (AS)
		{
			auto It = AllocationHandlesLUT.Find(AS.get());
			if (It && *It != UINT32_MAX)
			{
				FreeBlock(*It);
				AllocationHandlesLUT.Remove(AS.get());
			}
		}

...
void FWebBrowserTextureResource::Render(FRHICommandList& RHICmdList, const FRenderParams& Params)
{
	...
	if (SampleSource.IsValid())
	{
		...
		// render the sample
		CopySample(RHICmdList, Sample, Params.ClearColor);
	...
	{
		ClearTexture(RHICmdList, Params.ClearColor);
...
void FWebBrowserTextureResource::ClearTexture(FRHICommandList& RHICmdList, const FLinearColor& ClearColor)
{
...
void FWebBrowserTextureResource::CopySample(FRHICommandListBase& RHICmdList, const TSharedPtr<FWebBrowserTextureSample, ESPMode::ThreadSafe>& Sample, const FLinearColor& ClearColor)
{
	...
	if (SampleTexture2D != nullptr)
	{
		...
		if (TextureRHI != SampleTexture2D)
		{
			...
			UpdateTextureReference(RHICmdList, SampleTexture2D);
...
void FWebBrowserTextureResource::UpdateTextureReference(FRHICommandListBase& RHICmdList, FRHITexture* NewTexture)
{

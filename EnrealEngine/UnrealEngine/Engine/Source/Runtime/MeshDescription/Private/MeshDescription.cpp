...
void FMeshDescriptionBulkData::LoadMeshDescription( FMeshDescription& MeshDescription)
{
	...
	if (BulkData.HasPayloadData())
	{
		...
		constexpr bool bIsPersistent = true;
		UE::Serialization::FEditorBulkDataReader Ar(BulkData, bIsPersistent);
	
		// Only attempt to load the mesh description if the payload actually exists. HasPayloadData only checks if the payload size is greater
		// than zero, not that the payload actually got loaded (which can happen asynchronously). 

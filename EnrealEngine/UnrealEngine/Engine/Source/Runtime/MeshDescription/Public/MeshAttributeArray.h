...
public:
	...
	inline int32 Num() const { return Container.Num() / Extent; }

	/** Return base of data */
	UE_DEPRECATED(5.0, "This method will be removed.")
	inline const AttributeType* GetData() const { return Container.GetData(); }

	/** Initializes the array to the given size with the default value */
	inline void Initialize(const int32 ElementCount, const AttributeType& Default)
	{
	...
	inline const AttributeType& operator[](const int32 Index) const { return Container[Index]; }

	UE_DEPRECATED(5.0, "Please use GetElementBase() instead.")
	inline AttributeType& operator[](const int32 Index) { return Container[Index]; }

	inline const AttributeType* GetElementBase(const int32 Index) const { return &Container[Index * Extent]; }
	inline AttributeType* GetElementBase(const int32 Index) { return &Container[Index * Extent]; }

	inline uint32 GetExtent() const { return Extent; }
...
public:
	/** Constructor */
	inline FMeshAttributeArraySetBase(const uint32 InType, const EMeshAttributeFlags InFlags, const int32 InNumberOfElements, const uint32 InExtent)
		: Type(InType),
		  Extent(InExtent),
...
//	UE_DEPRECATED(5.0, "Please use RemoveChannel().")
	...
	inline void SetFlags(const EMeshAttributeFlags InFlags) { Flags = InFlags; }
...
public:
	/** Constructors */
	inline explicit TMeshAttributeArraySet(const int32 Extent = 1)
		: Super(TTupleIndex<AttributeType, AttributeTypes>::Value, EMeshAttributeFlags::None, 0, Extent)
	{}

	inline explicit TMeshAttributeArraySet(const int32 NumberOfChannels, const AttributeType& InDefaultValue, const EMeshAttributeFlags InFlags, const int32 InNumberOfElements, const uint32 Extent)
	...
	inline const TMeshAttributeArrayBase<AttributeType>& GetArrayForIndex( const int32 Index ) const { return ArrayForChannels[ Index ]; }
	UE_DEPRECATED(5.0, "Please use GetArrayForChannel().")
	inline TMeshAttributeArrayBase<AttributeType>& GetArrayForIndex( const int32 Index ) { return ArrayForChannels[ Index ]; }

	/** Return the TMeshAttributeArrayBase corresponding to the given attribute channel */
	inline const TMeshAttributeArrayBase<AttributeType>& GetArrayForChannel( const int32 Index ) const { return ArrayForChannels[ Index ]; }
	inline TMeshAttributeArrayBase<AttributeType>& GetArrayForChannel( const int32 Index ) { return ArrayForChannels[ Index ]; }
...
public:
	/** Constructors */
	inline TMeshUnboundedAttributeArraySet()
		: Super(TTupleIndex<AttributeType, AttributeTypes>::Value, EMeshAttributeFlags::None, 0, 0)
	{}

	inline explicit TMeshUnboundedAttributeArraySet(const int32 NumberOfChannels, const AttributeType& InDefaultValue, const EMeshAttributeFlags InFlags, const int32 InNumberOfElements)
	...
	/** Return the TMeshAttributeArrayBase corresponding to the given attribute channel */
	inline const TAttributeArrayContainer<AttributeType>& GetArrayForChannel( const int32 Index ) const { return ArrayForChannels[ Index ]; }
	inline TAttributeArrayContainer<AttributeType>& GetArrayForChannel( const int32 Index ) { return ArrayForChannels[ Index ]; }
...
public:
	...
	inline const FMeshAttributeArraySetBase* Get() const { return Ptr.Get(); }
	inline const FMeshAttributeArraySetBase* operator->() const { return Ptr.Get(); }
	inline const FMeshAttributeArraySetBase& operator*() const { return *Ptr; }
	inline FMeshAttributeArraySetBase* Get() { return Ptr.Get(); }
	inline FMeshAttributeArraySetBase* operator->() { return Ptr.Get(); }
	inline FMeshAttributeArraySetBase& operator*() { return *Ptr; }

	/** Object can be coerced to bool to indicate if it is valid */
	inline explicit operator bool() const { return Ptr.IsValid(); }
	inline bool operator!() const { return !Ptr.IsValid(); }
...
#if DO_CHECK
			// Log out some more debug info when this fails, as it's failing in public builds with no known repro
			if (MapEntry.Value->GetNumElements() != NumElements)
			{
				UE_LOGFMT(LogMeshDescription, Error, "NumElements mismatch in FAttributesSetBase::Insert");
				UE_LOGFMT(LogMeshDescription, Error, "NumElements: {NumElements}", NumElements);
				UE_LOGFMT(LogMeshDescription, Error, "Index: {Index}", Index);
				UE_LOGFMT(LogMeshDescription, Error, "Mismatched attribute: {Attribute}\n", MapEntry.Key.ToString());

				for (const TPair<FName, FAttributesSetEntry>& Pair : Map)
				{
					UE_LOGFMT(LogMeshDescription, Error, "Attribute {Attribute} NumElements: {NumElements}\n", Pair.Key.ToString(), Pair.Value->GetNumElements());
				}

				// Add some info to the assert message anyway, in case the logs are suppressed
				checkf(false, TEXT("NumElements mismatch in FAttributesSetBase::Insert: %s: %i != %i"), *MapEntry.Key.ToString(), MapEntry.Value->GetNumElements(), NumElements);
			}
#endif
		}
	}

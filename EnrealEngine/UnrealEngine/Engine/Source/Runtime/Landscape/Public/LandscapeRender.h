...
/** vertex factory for VTF-heightmap terrain  */
class UE_DEPRECATED(5.7, "XYOffset deprecated with the removal of non-edit layer landscapes") FLandscapeXYOffsetVertexFactory : public FLandscapeVertexFactory
{
...
#if RHI_RAYTRACING
	virtual void RayTracingNotifyViewDeleted(uint32 ViewKey) = 0;
#endif
...
protected:
	...
	LANDSCAPE_API virtual void OnForceHiddenChanged() override;
...
public:
	...
	LANDSCAPE_API virtual void GetSectionCenterAndVectors(FVector& OutSectionCenterWorldSpace, FVector& OutSectionXVectorWorldSpace, FVector& OutSectionYVectorWorldSpace) const override;
	LANDSCAPE_API virtual const FPrimitiveSceneInfo* GetPrimitiveSceneInfo() const override;
	LANDSCAPE_API virtual bool ShouldInvalidateShadows(const FSceneView& InView, float InLODValue, float InLastShadowInvalidationLODValue) const override;

#if RHI_RAYTRACING
	LANDSCAPE_API virtual void RayTracingNotifyViewDeleted(uint32 ViewKey) override;
#endif

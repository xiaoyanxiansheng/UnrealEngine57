			...
			if (!CurrentWorld->IsGameWorld())
			{
				...
				for (auto& Pair : LandscapeInfoMap.Map)
				{
					if (Pair.Value && Pair.Value->SupportsLandscapeEditing())
					{
						...
						if (Pair.Value->LandscapeActor.IsValid() && !Pair.Value->LandscapeActor->IsTemplate())
						{
...
bool ULandscapeSplinesComponent::IsUsingLayerInfo(const ULandscapeLayerInfoObject* LayerInfo) const
{
	for (ULandscapeSplineControlPoint* ControlPoint : ControlPoints)
	{
		if (ControlPoint->LayerName == LayerInfo->GetLayerName())
		{
			return true;
		}
	}
	...
	for (ULandscapeSplineSegment* Segment : Segments)
	{
		if (Segment->LayerName == LayerInfo->GetLayerName())
		{
			return true;
		}
	}
...
void ULandscapeSplinesComponent::RequestSplineLayerUpdate()
{
	if (IsValidChecked(this))
	{
		...
		//  This can get called on a deleted spline, when redoing a deletion.
		if (SplineOwner)
		{
...
{
	ULandscapeSplinesComponent* SplineComponent = GetOuterSafe();
	return bPlaceSplineMeshesInStreamingLevels && SplineComponent && SplineComponent->GetSplineOwner()->SupportsForeignSplineMesh();
}
...
TMap<ULandscapeSplinesComponent*, UControlPointMeshComponent*> ULandscapeSplineControlPoint::GetForeignMeshComponents()
{
	...
	ULandscapeSplinesComponent* OuterSplines = GetOuterSafe();
	if (OuterSplines)
	{
...
void ULandscapeSplineControlPoint::UpdateSplinePoints(bool bUpdateCollision, bool bUpdateAttachedSegments, bool bUpdateMeshLevel)
{
	...
	ULandscapeSplinesComponent* OuterSplines = GetOuterChecked();
...
void ULandscapeSplineControlPoint::DeleteSplinePoints()
{
	...
	// Make this control point transient to prevent duplicate from picking the disconnected, but un-garbage-collected
	// instance and exporting it.  Otherwise the non-standard code in PostEditImport will re-attach the point to the
	// component and it will re-appear as part of the copy.
...
void ULandscapeSplineControlPoint::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	{
		GetOuterChecked()->RequestSplineLayerUpdate();
	}
}
#endif // WITH_EDITOR
...
void ULandscapeSplineSegment::DeleteSplinePoints()
{
	...
	// Make this segment transient to prevent duplicate from picking the disconnected, but un-garbage-collected
	// instance and exporting it.  Otherwise the non-standard code in PostEditImport will re-attach the segment to the
	// component and it will re-appear as part of the copy.
	Rename(nullptr, GetTransientPackage());
...
ULandscapeSplinesComponent* ULandscapeSplineSegment::GetOuterSafe() const
{
	return !HasAnyFlags(RF_ClassDefaultObject) ? Cast<ULandscapeSplinesComponent>(GetOuter()) : nullptr;
}

ULandscapeSplinesComponent* ULandscapeSplineSegment::GetOuterChecked() const
{
	check(!HasAnyFlags(RF_ClassDefaultObject));
	return CastChecked<ULandscapeSplinesComponent>(GetOuter());
}

ULandscapeSplinesComponent* ULandscapeSplineControlPoint::GetOuterSafe() const
{
	return !HasAnyFlags(RF_ClassDefaultObject) ? Cast<ULandscapeSplinesComponent>(GetOuter()) : nullptr;
}

ULandscapeSplinesComponent* ULandscapeSplineControlPoint::GetOuterChecked() const
{
	check(!HasAnyFlags(RF_ClassDefaultObject));
	return CastChecked<ULandscapeSplinesComponent>(GetOuter());
}
...
void ULandscapeInfo::MoveSpline(ULandscapeSplineControlPoint* InControlPoint, TScriptInterface<ILandscapeSplineInterface> InNewOwner)
{
	...
	ULandscapeSplinesComponent* FromSplineComponent = InControlPoint->GetOuterChecked();

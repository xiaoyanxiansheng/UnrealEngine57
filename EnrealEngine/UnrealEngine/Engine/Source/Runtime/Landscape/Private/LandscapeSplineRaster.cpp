...
public:
	static TSharedPtr<FModulateAlpha> CreateFromLayerInfo(ULandscapeLayerInfoObject* InLayerInfo, int32 InLandscapeMinX, int32 InLandscapeMinY)
	{
		...
		if (!InLayerInfo->GetSplineFalloffModulationTexture()->Source.IsValid())
		{
			UE_LOG(LogLandscape, Error, TEXT("Invalid source data for spline falloff modulation texture (%s). Alpha modulation will be disabled."), *InLayerInfo->GetSplineFalloffModulationTexture()->GetPathName());
			return nullptr;
		}
...
private:
	FModulateAlpha(ULandscapeLayerInfoObject* InLayerInfo, int32 InLandscapeMinX, int32 InLandscapeMinY)
	{
		...
		verify(InLayerInfo->GetSplineFalloffModulationTexture()->Source.GetMipData(MipData, 0));
		TextureWidth = InLayerInfo->GetSplineFalloffModulationTexture()->Source.GetSizeX();
		TextureHeight = InLayerInfo->GetSplineFalloffModulationTexture()->Source.GetSizeY();
		...
		ChannelOffset = ChannelOffsets[(int32)InLayerInfo->GetSplineFalloffModulationColorMask()];
	}

...
static TAutoConsoleVariable<int32> CVarRayTracingLandscapeGrass(
	...
	0,
	TEXT("Defines if landscape grass meshes are added to the ray tracing scene.\n")
	TEXT(" 0: raytracing cached MDCs for landscape grass mesh are not created (default).\n")
	TEXT(" 1: raytracing cached MDCs for landscape grass mesh are created and instances are added to the RayTracing scene (default).\n")
	TEXT(" 2: landscape grass meshes are runtime culled from the RayTracing scene, but keeps the cached state for fast toggling."));
...
bool ULandscapeComponent::CanRenderGrassMap() const
{
	...
	FMaterialResource* MaterialResource = MaterialInstance != nullptr ? MaterialInstance->GetMaterialResource(GetFeatureLevelShaderPlatform_Checked(ComponentWorld->GetFeatureLevel())) : nullptr;
...
TArray<uint16> ULandscapeComponent::RenderWPOHeightmap(int32 LOD)
{
	...
	if (ensure(SceneProxy))
	{
		...
		{
			GetMaterialInstance(0)->GetMaterialResource(GetFeatureLevelShaderPlatform_Checked(GetWorld()->GetFeatureLevel()))->FinishCompilation();
...
struct FGrassBuilderBase
{
	...
	FGrassBuilderBase(ALandscapeProxy* Landscape, ULandscapeComponent* Component, const FGrassVariety& GrassVariety, ERHIFeatureLevel::Type FeatureLevel, int32 SqrtSubsections = 1, int32 SubX = 0, int32 SubY = 0, bool bEnableDensityScaling = true)
	{
		...
		LandscapeSectionOffset = Landscape->GetSectionBase();
...
struct FAsyncGrassBuilder : public FGrassBuilderBase
{
	...
	FAsyncGrassBuilder(ALandscapeProxy* Landscape, ULandscapeComponent* Component, const ULandscapeGrassType* GrassType, const FGrassVariety& GrassVariety, ERHIFeatureLevel::Type FeatureLevel, UGrassInstancedStaticMeshComponent* GrassInstancedStaticMeshComponent, int32 SqrtSubsections, int32 SubX, int32 SubY, uint32 InHaltonBaseIndex, TArray<FBox>& InExcludedBoxes)
		...
		, bAlignToTriangleNormals(GrassVariety.bAlignToTriangleNormals)
	...
	void Build()
	{
		...
		auto ApplyNormalAlignmentTransform = [](const FVector& Normal, FMatrix& InOutXForm)
			{
				// Note: NewZ should never be perpendicular to Z axis, as it is constructed from a heightmap.
				// Therefore it should never have a zero cross product with (0,-1,0)
				const FVector NewZ = Normal * FMath::Sign(Normal.Z);
		...
		if (HaltonBaseIndex)
		{
			...
			for (int32 InstanceIndex = 0; InstanceIndex < MaxNum; InstanceIndex++)
			{
				...
				if (bKeep)
				{
					...
					FMatrix OutXForm = FScaleRotationTranslationMatrix(Scale, FRotator(0.0f, Rot, 0.0f), FVector::ZeroVector);
					if (bAlignToSurface && !ComputedNormal.IsNearlyZero())
					{
						ApplyNormalAlignmentTransform(ComputedNormal, OutXForm);
					}
					OutXForm = OutXForm.ConcatTranslation(LocationWithHeight) * XForm;
			...
			if (InstanceTransforms.Num())
			{
				...
				for (int32 InstanceIndex = 0; InstanceIndex < InstanceTransforms.Num(); InstanceIndex++)
				{
					...
					// NOTE: the random fraction parameter is not used, but we keep it to avoid changing the deterministic random behavior
		...
		else
		{
			...
			Instances.AddUninitialized(SqrtMaxInstances * SqrtMaxInstances);
			{
				...
				for (int32 xStart = 0; xStart < SqrtMaxInstances; xStart++)
				{
					for (int32 yStart = 0; yStart < SqrtMaxInstances; yStart++)
					{
						...
						float Weight = 0.f;
						SampleLandscapeAtLocationLocal(Location, Instance.Pos, Weight, (bAlignToSurface && bAlignToTriangleNormals) ? &Instance.Normal : nullptr);
			...
			// A second pass is necessary because we need derivatives to calculate the smooth grid-based normal
			if (NumKept)
			{
					...
					for (int32 xStart = 0; xStart < SqrtMaxInstances; xStart++)
					{
						for (int32 yStart = 0; yStart < SqrtMaxInstances; yStart++)
						{
							...
							if (Instance.bKeep)
							{
								...
								if (bAlignToSurface)
								{
									if (bAlignToTriangleNormals)
									{
										ApplyNormalAlignmentTransform(Instance.Normal, OutXForm);
									}
									else
									{
										// NOTE: This calculation of smooth normals uses jittered grid points to compute the derivatives.
										// The jitter is generally ok, though may ocasionally result in a small delta that is sensitive to error.
										// However, the grid may be at any resolution, so the derivative will be estimated from taking steps at that resolution.
										// which may result in normals that don't match the local terrain well, especially for low resolution grids that take large steps.
										...
										// this only fails if your grid is 1x1, or if the jittered offsets of neighboring grids randomly intersected
										// in either case, we can't align, so leave the grass transform alone
										if (PosX1 != PosX2 && PosY1 != PosY2)
										{
											FVector Normal = ((PosX1 - PosX2) ^ (PosY1 - PosY2)).GetSafeNormal();
											ApplyNormalAlignmentTransform(Normal, OutXForm);
										}
									}
								}

								// apply translation and outer transform
								OutXForm = OutXForm.ConcatTranslation(Instance.Pos) * XForm;
	...
	FORCEINLINE_DEBUGGABLE void SampleLandscapeAtLocationLocal(const FVector& InLocation, FVector& OutLocation, float& OutLayerWeight, FVector* OutNormal = nullptr)
	{
		...
		if (OutNormal)
		{
			const FVector P11 = FVector((float)X1, (float)Y1, SampleHeight11) * DrawScale;
			const FVector P22 = FVector((float)X2, (float)Y2, SampleHeight22) * DrawScale;
			
			// Choose triangle and compute normal
			if (LerpX > LerpY)
			{
				const FVector P21 = FVector((float)X2, (float)Y1, SampleHeight21) * DrawScale;
				*OutNormal = (P11 != P21 && P22 != P21) ? FVector(((P22 - P21) ^ (P11 - P21)).GetSafeNormal()) : FVector::UpVector;
			}
			else 
			{
				const FVector P12 = FVector((float)X1, (float)Y2, SampleHeight12) * DrawScale;
				*OutNormal = (P11 != P12 && P22 != P12) ? FVector(((P11 - P12) ^ (P22 - P12)).GetSafeNormal()) : FVector::UpVector;
			}
		}
	}
};

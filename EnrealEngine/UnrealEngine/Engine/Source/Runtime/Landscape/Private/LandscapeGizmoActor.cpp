...
void ALandscapeGizmoActiveActor::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	{
		SetTargetLandscape(TargetLandscapeInfo.Get());
	}
...
{
	ULandscapeInfo* PrevInfo = TargetLandscapeInfo.Get();
	if (!LandscapeInfo || LandscapeInfo->HasAnyFlags(RF_BeginDestroyed))
	{
		...
		{
			for (const TPair<FGuid, TObjectPtr<ULandscapeInfo>>& InfoMapPair : ULandscapeInfoMap::GetLandscapeInfoMap(CurrentWorld).Map)
			{
	...
	if (TargetLandscapeInfo.Get() != PrevInfo)
	{
		// if there's no copied data, try to move somewhere useful
		if (TargetLandscapeInfo.IsValid() && DataType == LGT_None)
		{
...
{
	if (TargetLandscapeInfo.IsValid())
	{
		// Find fit size
...
{
	if (TargetLandscapeInfo.IsValid() && GizmoTexture)
	{

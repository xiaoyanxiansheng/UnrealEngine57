...
public:
	...
	/** Use GetOuterSafe instead of GetOuterULandscapeSplinesComponent if there is any danger that this segment has been deleted (via DeleteSplinePoints).
	  * This will return nullptr in that case, because the outer is no longer a ULandscapeSplinesComponent */
	LANDSCAPE_API ULandscapeSplinesComponent* GetOuterSafe() const;
	/** Use GetOuterChecked when the call is always expected to succeed.  Will assert if it fails. */

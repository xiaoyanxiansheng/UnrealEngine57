...
public:
	...
	// False if landscape editing not supported, in PIE world, or in a Commandlet
	LANDSCAPE_API bool IsLandscapeEditableWorld() const;
	...
	UE_DEPRECATED(5.7, "Use ApplySplines without bOnlySelected. Edit layer landscapes update all splines regardless of selection state")
	...
	LANDSCAPE_API bool ApplySplines(TSet<TObjectPtr<ULandscapeComponent>>* OutModifiedComponents = nullptr, bool bMarkPackageDirty = true);
	...
	LANDSCAPE_API static int32 RemoveLandscapeInfo(UWorld* InWorld, const FGuid& LandscapeGuid);
	...
	/** Returns a const reference to the sorted registered streaming proxies */
	LANDSCAPE_API const TArray<TWeakObjectPtr<ALandscapeStreamingProxy>>& GetSortedStreamingProxies() const;

	/**
	 * Runs the given function on the root landscape actor and all streaming proxies, with the possibility of early exit
...
private:
	...
	/** Sorts registered streaming proxies by their section base offset */
	void SortStreamingProxies();

	UPROPERTY()
	TArray<TWeakObjectPtr<ALandscapeStreamingProxy>> SortedStreamingProxies;
};

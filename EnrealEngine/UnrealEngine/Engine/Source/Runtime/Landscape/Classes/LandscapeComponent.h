...
struct FLandscapeComponentGrassData
{
	...
	UE_DEPRECATED(all, "GenerationHash is now used") 
...
UENUM()
enum UE_DEPRECATED(5.7, "Use ELandscapeToolTargetTypeFlags instead.") ELandscapeClearMode : int
{
...
class ULandscapeComponent : public UPrimitiveComponent
{
	...
	UE_DEPRECATED(all, "MaterialInstance has been deprecated, use MaterialInstances instead.")
	UPROPERTY()
	...
	UPROPERTY()
	TObjectPtr<UTexture2D> XYOffsetmapTexture_DEPRECATED;
...
#if WITH_EDITORONLY_DATA
	UE_DEPRECATED(all, "CollisionComponent has been deprecated")
	UPROPERTY()
...
private:
	...
	/** Obsolete edit layers (Edit layers not tracked by ALandscape). During registration, moves obsolete layer data out of active LayersData into
	* this transient storage. When obsolete layers exists, creates an additional duplicate of the final merged data on initial registration mapped to FGuid() */
	UPROPERTY(Transient)
	TMap<FGuid, FLandscapeLayerComponentData> ObsoleteEditLayerData;
	...
	// Physical material has been invalidated and no new physical material task has been started since then
	bool bPendingPhysicalMaterialInvalidation = false;
...
public:
	...
	UE_DEPRECATED(all, "IrrelevantLights is officially deprecated now")
	UPROPERTY()
	...
	UE_DEPRECATED(all, "MobileMaterialInterface has been deprecated and will be removed in a future version")
	UPROPERTY(NonPIEDuplicateTransient)
	TObjectPtr<UMaterialInterface> MobileMaterialInterface_DEPRECATED;

    UE_DEPRECATED(all, "MobileCombinationMaterialInstance has been deprecated and will be removed in a future version")
	UPROPERTY(NonPIEDuplicateTransient)
...
public:
	...
	void CopyComponentDataToEditLayer(const FGuid& InSourceEditLayerGuid, const FGuid& InDestEditLayerGuid, bool bInUseObsoleteLayerData, TSet<UTexture2D*>& OutProcessedHeightmaps, TSet<UTexture2D*>& OutProcessedWeightmaps);

	// Obsolete edit layer helpers
	const TMap<FGuid, FLandscapeLayerComponentData>& GetObsoleteEditLayersData() const;
	const FLandscapeLayerComponentData* GetObsoleteEditLayerData(const FGuid& InLayerGuid) const; 
	void RemoveObsoleteEditLayerData(const FGuid& InLayerGuid);
	void InitializeObsoleteEditLayerData(const TSet<FGuid>& InParentEditLayerGuids);
	...
	UFUNCTION(BlueprintCallable, Category = "Landscape|Grass", meta=(DisplayName = "GetGrassTypes", Tooltip = "Returns the grass types used by the landscape material on this component"))
	const TArray<ULandscapeGrassType*>& GetGrassTypesBP() const { return ObjectPtrDecay(GrassTypes); }
	...
	class UMaterialInterface* GetCurrentRuntimeMaterialInterface(int32 InIndex) const;
	...
	* @param TextureDataInfo - FLandscapeTextureDataInfo pointer, to notify of the mip data region updated.
	 ...
	 */
	void UpdateCollisionHeightData(const FColor* HeightmapTextureMipData, const FColor* SimpleCollisionHeightmapTextureData, int32 ComponentX1=0, int32 ComponentY1=0, int32 ComponentX2=MAX_int32, int32 ComponentY2=MAX_int32, bool bUpdateBounds=false, bool bInUpdateHeightfieldRegion=true);
	 ...
	 */
	LANDSCAPE_API void UpdateCollisionData(bool bInUpdateHeightfieldRegion = true);
	...
	/** Invalidate current or pending physical material data */
...
protected:
	...
	void UpdateDominantLayerBuffer(int32 InComponentX1, int32 InComponentY1, int32 InComponentX2, int32 InComponentY2, int32 InCollisionMipLevel, int32 InWeightmapSizeU, int32 InVisibilityLayerIdx, const TArray<uint8*>& InCollisionDataPtrs, const TArray<ULandscapeLayerInfoObject*>& InLayerInfos, uint8* DominantLayerData);
#endif

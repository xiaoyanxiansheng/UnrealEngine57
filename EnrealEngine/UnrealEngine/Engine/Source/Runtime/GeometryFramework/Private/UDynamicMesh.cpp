...
{
	FScopeLock PoolScopeLock(&PoolAccessCs);
	...
	if (!ensure(AllCreatedMeshes.Num() < CVarDynamicMeshPoolMaxPoolSizeThreshold.GetValueOnAnyThread()))
	{
		UE_LOG(LogGeometry, Warning, TEXT("UDynamicMeshPool Threshold of %d Allocated Meshes exceeded! Releasing references to all current meshes and forcing a garbage collection."), CVarDynamicMeshPoolMaxPoolSizeThreshold.GetValueOnAnyThread());
		AllCreatedMeshes.Reset();
		// We generally expect the mesh pool to be used from game-thread blueprint code, but in case it is not, take extra care that ForceGarbageCollection is only called from the game thread
		if (IsInGameThread())
		{

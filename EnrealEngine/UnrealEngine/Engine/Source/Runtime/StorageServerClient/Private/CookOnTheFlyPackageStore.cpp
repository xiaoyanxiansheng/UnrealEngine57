...
void FCookOnTheFlyPackageStoreBackend::GetCookedPackagesFromServer()
{
...
void FCookOnTheFlyPackageStoreBackend::GetAllAvailablePackagesFromServer()
{
	using namespace UE::Cook;
	using namespace UE::ZenCookOnTheFly::Messaging;

	FCookOnTheFlyRequest Request(ECookOnTheFlyMessage::GetAllOnDiskPackages);
	FCookOnTheFlyResponse Response = CookOnTheFlyServerConnection.SendRequest(Request).Get();
	if (Response.IsOk())
	{
		FGetAllOnDiskPackagesRequest GetAllOnDiskPackagesResponse = Response.GetBodyAs<FGetAllOnDiskPackagesRequest>();
		UE_LOG(LogCookOnTheFly, Display, TEXT("Received list of %d total on-disk packages from server"), GetAllOnDiskPackagesResponse.PackageIds.Num());

		AddOnDiskPackages(GetAllOnDiskPackagesResponse.PackageIds);
	}
	else
	{
		UE_LOG(LogCookOnTheFly, Warning, TEXT("Failed to send 'GetAllOnDiskPackages' request"));
	}
}
...
bool FCookOnTheFlyPackageStoreBackend::DoesPackageExist(FPackageId PackageId)
{
	...
	const FEntryInfo* EntryInfo = PackageIdToEntryInfo.Find(PackageId);
	if (EntryInfo == nullptr)	
	{
		// We didn't encounter a cooked version of this package yet, but it may still exist on the server
		return AllServerOnDiskPackages.Find(PackageId) != nullptr;
	}
	else
	{
...
void FCookOnTheFlyPackageStoreBackend::AddOnDiskPackages(const TArray<FPackageId>& Packages)
{
	FWriteScopeLock WriteLock(EntriesLock);
	AllServerOnDiskPackages.Reserve(Packages.Num());
	for (const FPackageId& PackageID : Packages)
	{
		AllServerOnDiskPackages.Emplace(PackageID);
	}
}

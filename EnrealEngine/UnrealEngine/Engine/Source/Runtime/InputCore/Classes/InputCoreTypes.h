...
struct FKey
{
	 ...
	 * @return			True if this FKey's name or its virtual key name (if it has one) is the same as the other.
	 */
...
struct FKeyDetails
{
	...
	inline bool IsAxis2D() const { return AxisType == EInputAxisType::Axis2D; }
	inline bool IsAxis3D() const { return AxisType == EInputAxisType::Axis3D; }
	inline bool IsButtonAxis() const { return AxisType == EInputAxisType::Button; }	// Analog 1D axis emulating a digital button press.
	inline bool IsAnalog() const { return IsAxis1D() || IsAxis2D() || IsAxis3D(); }
	inline bool IsDigital() const { return !IsAnalog(); }
	...
	inline FName GetMenuCategory() const { return MenuCategory; }
	INPUTCORE_API FText GetDisplayName(const bool bLongDisplayName = true) const;
	inline const FKey& GetKey() const { return Key; }

	// Key pairing
	inline EPairedAxis GetPairedAxis() const { return PairedAxis; }
	inline const FKey& GetPairedAxisKey() const { return PairedAxisKey; }
	 ...
	 */
	inline const FKey& GetVirtualKey() const { return bIsVirtual ? VirtualKeyValue : Key; }
...
struct EKeys
{
	...
	UE_DEPRECATED(5.7, "Use Virtual_Gamepad_Back.GetVirtualKey() instead")

...
#if WITH_CEF3
	/** Returns the time in seconds that an external caller should wait after previous tick, before externally calling Tick(). */
	WEBBROWSER_API float GetExternalTickCallWaitSeconds() const;

	/** Returns the time of the most recent call to Tick(). */
	WEBBROWSER_API double GetPreviousTickTimeSeconds() const;
...
private:
...
#if !WITH_CEF3 && (PLATFORM_IOS || PLATFORM_MAC || (PLATFORM_ANDROID && USE_ANDROID_JNI))
	...
	/* Time when our Tick() was last called. */
	double PreviousTickTimeSeconds = 0.0;
	
	/** How long an external caller of our Tick() function should wait, after the previous tick time, before calling our Tick(). */ 
	float ExternalTickCallWaitSeconds = 0.0;
};

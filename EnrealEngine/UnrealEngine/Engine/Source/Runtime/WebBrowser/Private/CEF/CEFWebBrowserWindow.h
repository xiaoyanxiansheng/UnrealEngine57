...
public:
	...
	*/
	...
	/**
	 * Tells us whether this has the correct CEF image buffer.
	 */
	WEBBROWSER_API bool HasCorrectNativeCefBuffer() const;
	
	/**
	 * Whether we should use a workaround for this bug -
	 * https://github.com/chromiumembedded/cef/issues/3826
	 */
	WEBBROWSER_API void SetUseCapturedCefBuffer(const bool bValue);
	
	/**
	 * Whether the CEF version we're using has a resize bug that sometimes requires requesting new frames after a resize.
	 */
	WEBBROWSER_API bool HasResizeBug() const;
	
	/**
	 * Gets an undistorted buffer captured from CEF. Format is PF_B8G8R8A8.
	 */
	WEBBROWSER_API TSharedPtr<FCapturedCefBuffer> GetCapturedCefBuffer();

	/**
	 * Set if a resize is needed.
	 */
	WEBBROWSER_API void SetNeedsResize(const bool bInValue);
...
private:
	...
	TUniquePtr<FCEFWebBrowserWindowRHIHelper> RHIRenderHelper;

#if PLATFORM_WINDOWS || PLATFORM_MAC
	bool bInDirectHwndMode;
#endif

	bool bHasCorrectNativeCefBuffer = true;
	TSharedPtr<FCapturedCefBuffer> CapturedCefBuffer;
};

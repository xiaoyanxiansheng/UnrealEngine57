...
namespace UE { namespace TasksTests
{
	...
	TEST_CASE_NAMED(FTasksEventTest, "System::Core::Tasks::TriggerTaskEventConcurrently", "[ApplicationContextMask][EngineFilter]")
	{
		// Regression test to ensure we do not assert when triggering an event from more than one thread at a time.
		for (int i = 0; i < 100000; ++i)
		{
			FTaskEvent Event{ UE_SOURCE_LOCATION };
			FTask DeferredTask{ Launch(UE_SOURCE_LOCATION, [] {}, Event) };
			ParallelFor(64, [Event, DeferredTask] (int32) mutable
			{
				Event.Trigger();
				DeferredTask.Wait();
			});
	...
	// This test will force the scheduling of a background task from the normal task which exercise some code in the scheduler
	// to send the task to the global queue and always wake up a worker since background tasks can't run on normal threads.
	TEST_CASE_NAMED(FTaskGraphBackgroundWithNormalAsPrereq, "System::Core::Async::Tasks::BackgroundWithNormalAsPrereq", "[.][ApplicationContextMask][EngineFilter]")
	{
		for (int32 Index = 0; Index < 100; ++Index)
		{
			UE::Tasks::FTask Task =
				UE::Tasks::Launch(
					TEXT("Task"),
					[] { },
					UE::Tasks::Launch(TEXT("Task"), [] { }), // Launch Normal task first and use it as prereq for the background one
			...
			while (!Task.IsCompleted())
			{
				if (FPlatformTime::Seconds() - StartTime > 10.0f)
				{
					FAIL_CHECK(TEXT("Test is likely deadlocked, aborting."));
					return;
				}
			}
		}

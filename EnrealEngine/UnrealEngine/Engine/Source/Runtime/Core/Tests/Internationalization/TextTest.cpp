...
TEST_CASE_NAMED(FTextNumericParsingTest, "System::Core::Misc::TextNumericParsing", "[.][EditorContext][ClientContext][EngineFilter][ServerContext][CommandletContext][SupportsAutoRTFM]")
{
	FTextNumericParsingTestClass Instance;
	Instance.TextNumericParsingTest();
}

TEST_CASE_NAMED(FTextStringificationTest, "System::Core::Misc::TextStringification", "[.][EditorContext][ClientContext][EngineFilter][ServerContext][CommandletContext][SupportsAutoRTFM]")
{
	FInternationalization& I18N = FInternationalization::Get();
...
TEST_CASE_NAMED(FTextFormatArgModifierTest, "System::Core::Misc::TextFormatArgModifiers", "[.][EditorContext][ClientContext][EngineFilter][ServerContext][CommandletContext][SupportsAutoRTFM]")
{
...
TEST_CASE_NAMED(FICUTextUtilities, "System::Core::Misc::ICUTextUtilities", "[.][EditorContext][ClientContext][EngineFilter]")
{
	FInternationalization& I18N = FInternationalization::Get();
	...
	if (I18N.SetCurrentCulture("fr"))
	{
		if (FTextTransformer::ToLower(FString(TEXT("ÔTE"))) != FString(TEXT("ôte")))
		{
			FAIL_CHECK(FString::Printf(TEXT("FTextTransformer::ToLower doesn't work correctly")));
		}
		
		if (FTextChar::IsWhitespace(TEXT('\u0212')) == true)
		{
			FAIL_CHECK(FString::Printf(TEXT("FTextChar::IsWhitespace can't detect whitespace")));
		}

		if (FTextChar::IsAlpha(TEXT('\u0212')) == false)
		{
			FAIL_CHECK(FString::Printf(TEXT("FTextChar::IsAlpha can't detect fr letter")));
		}

		if (FTextChar::IsAlpha(TEXT('7')) == true || FTextChar::IsAlpha(TEXT('A')) == false)
		{
			FAIL_CHECK(FString::Printf(TEXT("FTextChar::IsAlpha can't detect not letter")));
		}
		
		TCHAR EmojiChars[] = { 0xD83D, 0xDE00, 0x0000 };
		if (FTextChar::IsAlpha(FTextChar::GetCodepoint(EmojiChars)) == true)
		{
			FAIL_CHECK(FString::Printf(TEXT("FTextChar::IsAlpha can't detect surrogate pair")));
		}
	}

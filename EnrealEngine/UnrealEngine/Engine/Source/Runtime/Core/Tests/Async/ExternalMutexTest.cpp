...
{
	constexpr static uint8 IsLockedFlag = 1;
	constexpr static uint8 MayHaveWaitingLockFlag = 2;
};
...
TEST_CASE("Core::Async::ExternalMutex", "[Core][Async][Slow][LinuxSkip]")
{
	...
	SECTION("TExternalMutex IsLocked() and TryLock()")
	{
	...
	SECTION("TExternalMutex with TUniqueLock which uses SlowLock() and SlowUnlock()")
	{
		FThread Threads[TaskCount];
		std::atomic<uint32> TasksComplete = 0;
		const uint8 ThirdBit = 1 << 2;

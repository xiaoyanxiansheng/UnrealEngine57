...
FExtendedPlatformMemoryStats FUnixPlatformMemory::GetExtendedStats()
{
	...
	{
		QUICK_SCOPE_CYCLE_COUNTER(STAT_GetExtendedStats_KsmStat);
		
		if (access("/sys/kernel/mm/ksm/run", F_OK) == 0)
		{
			MemoryStats.bIs_KSM_Enabled = true;

			FProcField Field = { "ksm_process_profit", (uint64 *)&MemoryStats.KSM_Process_Profit };
			if (ReadProcFields("/proc/self/ksm_stat", &Field, 1))
			{
				// ReadProcFields expects KiB so has already multiplied by 1024
				MemoryStats.KSM_Process_Profit /= 1024;
			}
		}
	}

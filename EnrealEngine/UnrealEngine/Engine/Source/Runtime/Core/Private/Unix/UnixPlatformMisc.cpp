...
public:
	...
	static uint32 GetCPUModelID()			{ ParseCPUInfoFile(); return GCPUModelID; }
	static uint32 GetCPUMicrocodeRevision()	{ ParseCPUInfoFile(); return GCPUMicrocodeRevision; }
...
#undef _XTAG
	...
	static uint32 GCPUModelID;
	static uint32 GCPUMicrocodeRevision;
};
...
uint32 FPlatformProcCPUInfo::GCPUModelID = 0;
uint32 FPlatformProcCPUInfo::GCPUMicrocodeRevision = 0;
...
bool FPlatformProcCPUInfo::ParseCPUInfoFile()
{
	...
	if (ParseCPUInfoEntries("/proc/cpuinfo", CPUInfoValues))
	{
		...
		// Build a model ID from the family, model and stepping. Works for x64 CPUs.
		{
			const FString& strFamily = CPUInfoValues[TokenCPUfamily];
			const FString& strCpuModel = CPUInfoValues[TokenCPUmodel];
			const FString& strStepping = CPUInfoValues[TokenCPUstepping];
			if (!strFamily.IsEmpty() && !strCpuModel.IsEmpty() && !strStepping.IsEmpty())
			{
				uint32 FamilyID = FCString::Atoi(*strFamily);
				uint32 ModelID = FCString::Atoi(*strCpuModel);
				uint32 SteppingID = FCString::Atoi(*strStepping);
				// Use the same packing logic as FWindowsPlatformMisc::GetCPUModelID().
				GCPUModelID = (FamilyID << 12) | (ModelID << 4) | SteppingID;
			}
		}

		{
			const FString& strMicrocode = CPUInfoValues[TokenCPUmicrocode];
			if (strMicrocode.StartsWith(TEXT("0x")))
			{
				TCHAR* endPtr;
				GCPUMicrocodeRevision = FCString::Strtoi(*strMicrocode + 2, &endPtr, 16);
				if (*endPtr != 0)
				{
					GCPUMicrocodeRevision = 0;
				}
			}
		}
...
{
	return FPlatformProcCPUInfo::GetCPUModelID();
}

uint32 FUnixPlatformMisc::GetCPUMicrocodeRevision()
{
	return FPlatformProcCPUInfo::GetCPUMicrocodeRevision();
}

...
#include "Android/AndroidPlatformManualResetEvent.h"
...
FORCENOINLINE void FAndroidManualResetEvent::WaitSlow()
{
...
FORCENOINLINE bool FAndroidManualResetEvent::WaitForSlow(FMonotonicTimeSpan WaitTime)
{
	if (State.load(std::memory_order_acquire))
	{
		return true;
	...
	if (syscall(SYS_futex, &State, FUTEX_WAIT_PRIVATE, 0, WaitTimeSpecPtr, nullptr, 0) == -1 && errno == ETIMEDOUT)
	{
		return false;
	}
...
FORCENOINLINE bool FAndroidManualResetEvent::WaitUntilSlow(FMonotonicTimePoint WaitTime)
{
	LowLevelTasks::FOversubscriptionScope _;
	struct timespec WaitTimeSpec;
	const struct timespec* WaitTimeSpecPtr = SecondsToTimeSpec(WaitTime.ToSeconds(), WaitTimeSpec);
	...
	for (;;)
	{
		...
		if (syscall(SYS_futex, &State, FUTEX_WAIT_BITSET_PRIVATE, 0, WaitTimeSpecPtr, nullptr, FUTEX_BITSET_MATCH_ANY) == -1 && errno == ETIMEDOUT)
		{
			return false;
		}
	}
...
void FAndroidManualResetEvent::Notify()
{

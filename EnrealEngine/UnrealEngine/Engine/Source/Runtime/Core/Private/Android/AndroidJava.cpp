...
{
	return {UE::Jni::Class<UE::Jni::FGameActivity>, AndroidJavaEnv::GetGameActivityThis()};
}
...
FJavaClassObject::FJavaClassObject(jclass LocalClass, jobject LocalObject)
{
	check(LocalClass);
	check(LocalObject);

	Class = UE::Jni::Env.NewGlobalRef(LocalClass);
	Object = UE::Jni::Env.NewGlobalRef(LocalObject);
}

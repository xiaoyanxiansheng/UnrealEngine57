...
#endif
static inline void SetMmapName(const void* Ptr, const size_t Size, const char* Name)
{
	prctl(PR_SET_VMA, PR_SET_VMA_ANON_NAME, Ptr, Size, Name);
}
#else
static inline void SetMmapName(const void*, const size_t, const char*) {}
#endif
...
FPlatformMemoryStats FAndroidPlatformMemory::GetStats()
{
	...
	if (FILE* ProcMemStats = fopen("/proc/self/status", "r"))
	{
		...
		do
		{
			...
			{
				MemoryStats.RssFile = AndroidPlatformMemory::GetBytesFromStatusLine(Line);
				++FieldsSetSuccessfully;
			}
...
void* FAndroidPlatformMemory::BinnedAllocFromOS(SIZE_T Size)
{
	...
	if (USE_MALLOC_BINNED2)
	{
		...
		if (End - Base < Size)
		{
			...
			SetMmapName(UnalignedBase, SizeToAlloc, "UE");
...
FAndroidPlatformMemory::FPlatformVirtualMemoryBlock FAndroidPlatformMemory::FPlatformVirtualMemoryBlock::AllocateVirtual(size_t InSize, size_t InAlignment)
{
	...
	SetMmapName(Result.Ptr, Result.GetActualSize(), "UE");
	return Result;
}
...
void* LLMAlloc(size_t Size)
{
	...
	{
		const int ErrNo = errno;
		UE_LOG(LogHAL, Error, TEXT("mmap(Size=%llu) failed with errno = %d (%s)"), Size,
			ErrNo, StringCast< TCHAR >(strerror(ErrNo)).Get());
	}
	else
	{
...
static std::atomic<bool> GAndroidMemoryProbeTickRun = {};
static FTSTicker::FDelegateHandle GAndroidMemoryProbeTicker = {};
static UE::Tasks::TTask<void> GAndroidMemoryProbeTask = {};

static float GAndroidMemoryProbeInterval = 10.0f;
static FAutoConsoleVariableRef CVarAndroidMemoryProbeInterval(
	TEXT("android.MemoryProbeInterval"),
	GAndroidMemoryProbeInterval,
	TEXT("/proc/self/smaps polling interval in seconds, default 10 seconds."),
	ECVF_Default
);

void FAndroidPlatformMemory::AndroidMemoryProbeStart()
{
	if (GAndroidMemoryProbeInterval <= 0.0f)
	{
		return;
	}

	if (GAndroidMemoryProbeTicker.IsValid())
	{
		return;
	}

	GAndroidMemoryProbeTickRun = true;

	GAndroidMemoryProbeTicker = FTSTicker::GetCoreTicker().AddTicker(FTickerDelegate::CreateLambda([](float)
	{
		if (!GAndroidMemoryProbeTask.IsValid() || GAndroidMemoryProbeTask.IsCompleted())
		{
			GAndroidMemoryProbeTask = UE::Tasks::Launch(
				TEXT("AndroidMemoryProbeTick"),
...
void FAndroidPlatformMemory::AndroidMemoryProbeStop()
{
	...
	GAndroidMemoryProbeTickRun = false;

	if (GAndroidMemoryProbeTask.IsValid() && !GAndroidMemoryProbeTask.IsCompleted() && GAndroidMemoryProbeTask.IsAwaitable())
	{
		GAndroidMemoryProbeTask.Wait();
	}

	GAndroidMemoryProbeTask = {};

	FTSTicker::RemoveTicker(GAndroidMemoryProbeTicker);
	GAndroidMemoryProbeTicker = {};
}

void FAndroidPlatformMemory::AndroidMemoryProbeTick()
{
	if (!GAndroidMemoryProbeTickRun)
	{
		return;
	}

	TRACE_CPUPROFILER_EVENT_SCOPE(AndroidMemoryProbeTick2);
	...
	bool bIsProgramSize = false;
	uint64 ProgramSize = 0;

	while (GAndroidMemoryProbeTickRun)
	{
		char* LinePtr = fgets(LineBuffer, UE_ARRAY_COUNT(LineBuffer), Smaps);
		if (!LinePtr)
		{
			break; // eof or an error
		}
		FAnsiStringView Line(LinePtr);

		if (Line.Contains("-"))
		{
			// parsing region entry
			// 7fffcccb4000-7fffcccd8000 rw-p 00000000 00:00 0                          [stack]
			bIsProgramSize =
				FAnsiStringView(Line).EndsWith("/libUnreal.so\n") ||     // our own .so in case if it's unpacked
				FAnsiStringView(Line).EndsWith("/base.apk\n") ||         // our own .so in case if it's stored with-in .apk
				FAnsiStringView(Line).EndsWith("[anon:.bss]\n") ||
				FAnsiStringView(Line).EndsWith("[anon:linker_alloc]\n");
		}
		else if (bIsProgramSize && Line.StartsWith("Rss:"))
		{
			ProgramSize += AndroidPlatformMemory::GetBytesFromStatusLine(LinePtr);
		}
	}
...
#endif
	FPlatformMemory::ProgramSize = ProgramSize;
}

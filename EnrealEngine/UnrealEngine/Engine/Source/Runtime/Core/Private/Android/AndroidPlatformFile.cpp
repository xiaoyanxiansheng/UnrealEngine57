...
void JNICALL UE::Jni::FGameActivity::nativeSetObbInfo(JNIEnv* jenv, jobject thiz, jstring ProjectName, jstring PackageName, jint Version, jint PatchVersion, jstring AppType)
{
...
//This function is declared in the Java-defined class, AsisGameActivity.java: "public native String nativeGetObbComment();"
jstring JNICALL UE::Jni::Asis::FAsisGameActivity::nativeGetObbComment(JNIEnv* jenv, jclass clazz)
{
...
public:
	...
	virtual bool Read(uint8* Destination, int64 BytesToRead) override
	{
		...
		check(BytesToRead >= 0);
		check(Destination);
		bool bAtEndOfFile = CurrentOffset >= Start + Length;
		if (bAtEndOfFile)
		{
			// Fail if we tried reading when already at the end of the file.
			return false;
		}
		...
		do
		{
			...
			bAtEndOfFile = CurrentOffset >= Start + Length;
		} while (BytesToRead > 0 && !bAtEndOfFile);

		return BytesToRead == 0 || bAtEndOfFile;
	}
...
public:
	...
	{
		const bool bForceAllowLocal = false;
		return OpenWriteInternal(Filename, bAppend, bAllowRead, bForceAllowLocal);
	}

	IFileHandle* OpenWriteInternal(const TCHAR* Filename, bool bAppend, bool bAllowRead, bool bForceAllowLocal)
	{
#if LOG_ANDROID_FILE

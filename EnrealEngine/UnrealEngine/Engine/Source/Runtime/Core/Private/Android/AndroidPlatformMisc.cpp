...
namespace UE::Jni
{
	struct FHeadsetReceiver: Java::Lang::FObject
	{
		static constexpr FAnsiStringView ClassName = "com/epicgames/unreal/HeadsetReceiver";
		
		static void JNICALL stateChanged(JNIEnv * jni, jclass clazz, jint state)
		{
	...
	struct FVolumeReceiver: Java::Lang::FObject
	{
		static constexpr FAnsiStringView ClassName = "com/epicgames/unreal/VolumeReceiver";
		
		static void JNICALL volumeChanged(JNIEnv * jni, jclass clazz, jint volume)
		{
...
{
	{ UE::Jni::FVolumeReceiver::ClassName.GetData(),{ "volumeChanged", "(I)V",  (void*)UE::Jni::FVolumeReceiver::volumeChanged } },
	{ UE::Jni::FBatteryReceiver::ClassName.GetData(),{ "dispatchEvent", "(III)V",(void*)UE::Jni::FBatteryReceiver::dispatchEvent } },
	{ UE::Jni::FHeadsetReceiver::ClassName.GetData(),{ "stateChanged",  "(I)V",  (void*)UE::Jni::FHeadsetReceiver::stateChanged } },
};
...
void FAndroidMisc::PlatformInit()
{
	...
	FAndroidPlatformMemory::AndroidMemoryProbeStart();
}
...
{
	FAndroidPlatformMemory::AndroidMemoryProbeStop();
...
{ 
	return ANDROID_DEFAULT_DEVICE_PROFILE_NAME; 
}
...
static FString VulkanDeviceNameString;
...
// Only valid if EDeviceVulkanSupportStatus is EDeviceVulkanSupportStatus::Supported
...
static EDeviceVulkanSupportStatus AttemptVulkanInit(void* VulkanLib)
{
	...
	// Try to create instance to verify driver available. Logic here matches that in VulkanRHI.cpp.
	// Drivers need to detect the app and potentially return VK_ERROR_INCOMPATIBLE_DRIVER to disable Vulkan support for known incompatibility
	...
	vkGetPhysicalDeviceProperties(PhysicalDevices[0], &VulkanPhysicalDeviceProperties);
	...
	VulkanVersionString = FString::Printf(TEXT("%d.%d.%d"), VK_VERSION_MAJOR(VulkanPhysicalDeviceProperties.apiVersion), VK_VERSION_MINOR(VulkanPhysicalDeviceProperties.apiVersion), VK_VERSION_PATCH(VulkanPhysicalDeviceProperties.apiVersion));
	vkDestroyInstance(Instance, nullptr);
	VulkanDeviceNameString = VulkanPhysicalDeviceProperties.deviceName;
...
FString FAndroidMisc::GetVKQualityRecommendation()
{
	...
	if (VulkanSupport == EDeviceVulkanSupportStatus::Supported)
	{
		if (AndroidVKQuality::LoadVkQuality(TCHAR_TO_UTF8(*FAndroidMisc::GetGLVersion()), &AndroidPlatformMisc::VulkanPhysicalDeviceProperties))
		{
			Result = AndroidVKQuality::GetVKQualityRecommendation();
			bCachedResult = true;
			AndroidVKQuality::UnloadVkQuality();
		}
	}
#endif
	return Result;
...
bool FAndroidMisc::IsExternalMemoryAndroidHardwareBufferExtensionLoaded()
{
	if (GIsRHIInitialized && ShouldUseVulkan())
	{
		...
		{
			UE_LOG(LogAndroid, Log, TEXT("VK_KHR_SAMPLER_YCBCR_CONVERSION_EXTENSION is not available!"));
		}
		else if (!LoadedDeviceExtensions.Find(FAnsiString(VK_ANDROID_EXTERNAL_MEMORY_ANDROID_HARDWARE_BUFFER_EXTENSION_NAME)))
		{
			UE_LOG(LogAndroid, Log, TEXT("VK_ANDROID_EXTERNAL_MEMORY_ANDROID_HARDWARE_BUFFER_EXTENSION is not available!"));
		}
		else
		{
			UE_LOG(LogAndroid, Log, TEXT("VK_ANDROID_EXTERNAL_MEMORY_ANDROID_HARDWARE_BUFFER_EXTENSION is loaded."));
			return true;
		}
	}
...
#if USE_ANDROID_JNI
void JNICALL UE::Jni::FGameActivity::nativeSetConfigRulesVariables(JNIEnv* jenv, jobject thiz, Java::Lang::TArray<jstring>* KeyValuePairs)
{
...
#if USE_ANDROID_JNI
void JNICALL UE::Jni::FGameActivity::nativeSetAndroidStartupState(JNIEnv* jenv, jobject thiz, jboolean bDebuggerAttached)
{
...
void JNICALL UE::Jni::FGameActivity::nativeNetworkChanged(JNIEnv* jenv, jobject thiz)
{
	LastNetworkConnectionType = PrivateGetNetworkConnectionType();
	bLastConnectionTypeValid = true;

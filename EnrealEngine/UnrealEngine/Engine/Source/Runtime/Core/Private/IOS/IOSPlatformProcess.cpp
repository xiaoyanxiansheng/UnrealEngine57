...
void FIOSPlatformProcess::LaunchURL( const TCHAR* URL, const TCHAR* Parms, FString* Error )
{
	...
	[[UIApplication sharedApplication] openURL: [NSURL URLWithString: CFUrl] options:@{} completionHandler:
		^(BOOL Success)
		{
			if (!Success)
			{
				UE_LOG(LogIOS, Warning, TEXT("LaunchURL of %s was NOT successful"), URL);
			}
		}
	];
...
/**
* Opens the specified URL and waits for it to open, up to the Timeout (s).  If there's an error with the opening
* that's found within the timeout period, or a timeout occurs, Error is set to "Unable to open url"
*
* @param URL The url to open
* @param Parms Not currently used
* @param Timeout The length of time to wait (in seconds) before it times out
* @param Error Either blank or with an error message if fails to open or times out
**/
void FIOSPlatformProcess::LaunchSyncURL( const TCHAR* URL, const TCHAR* Parms, float Timeout, FString* Error )
{
	UE_LOG(LogIOS, Log, TEXT("LauncSynchURL %s %s"), URL, Parms?Parms:TEXT("") );

	if (FCoreDelegates::ShouldLaunchUrl.IsBound() && !FCoreDelegates::ShouldLaunchUrl.Execute(URL))
	{
		if (Error)
		{
			*Error = TEXT("LauncSynchURL cancelled by delegate");
		}
		return;
	}
	...
	if (Timeout <= 0)
	{
		if (Error)
		{
			*Error = TEXT("LauncSynchURL invalid Timeout specified");
		}
		return;
	}
	...
	// wait for a result, up to "Timeout" seconds
	if (dispatch_semaphore_wait(Semaphore, dispatch_time(DISPATCH_TIME_NOW, (uint64_t)(Timeout*(double)NSEC_PER_SEC))))
	{

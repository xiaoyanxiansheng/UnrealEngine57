...
FGPUDriverInfo FMacPlatformMisc::GetGPUDriverInfo(const FString& DeviceDescription, bool bVerbose)
{
	SCOPED_AUTORELEASE_POOL;
	...
	if (bVerbose)
	{
		...
		UE_LOG(LogMac, Log, TEXT("Attached monitors:"));
		int NumMonitors = 0;
		
		for (NSScreen* Screen in NSScreen.screens)
		{
			NSDictionary* ScreenDescription = Screen.deviceDescription;
			CGSize PointSize = Screen.frame.size;
			NSSize PixelSize = [[ScreenDescription objectForKey:NSDeviceSize] sizeValue];
			NSRect WorkArea = Screen.visibleFrame;
			NSNumber* Scale = ScreenDescription[NSDeviceResolution];
			
			UE_LOG(LogMac, Log, TEXT("Screen: %s, Resolution: %.0fx%.0f, Points: %.0fx%.0f, WorkArea: (%.0f, %.0f) -> (%.0f, %.0f), Scale: %.2f")
				   , Screen.localizedName ? *FString((__bridge CFStringRef)Screen.localizedName) : TEXT("(Unnamed)")
				   , PixelSize.width, PixelSize.height
				   , PointSize.width, PointSize.height
				   , WorkArea.origin.x, WorkArea.origin.y, WorkArea.origin.x + WorkArea.size.width, WorkArea.origin.y + WorkArea.size.height
				   , Screen.backingScaleFactor);
			NumMonitors++;
		}
		
		UE_LOG(LogMac, Log, TEXT("Found %d attached monitors."), NumMonitors);
	}
...
int32 FMacPlatformMisc::VersionCompare(const NSOperatingSystemVersion& VersionA, const NSOperatingSystemVersion& VersionB)
{
...
{
	return VersionCompare(GMacAppInfo.OSXVersion, {Major, Minor, Revision});
}
...
{
	return VersionCompare(GMacAppInfo.XcodeVersion, {Major, Minor, Revision});
}
...
void FMacPlatformMisc::SetCrashHandler(void (* CrashHandler)(const FGenericCrashContext& Context))
{
	...
	bool bEnableCustomSignalHandling = true;
	// For packaged games, only use custom signal handling when CRC is opt-in. Otherwise let macOS process crash without UE interference
#if !WITH_EDITOR
	FString ReportClient = FPaths::ConvertRelativePathToFull(FPlatformProcess::GenerateApplicationPath(TEXT("CrashReportClient"), EBuildConfiguration::Development));
	char CrashReportClient[PATH_MAX+1];
	FCStringAnsi::Strncpy(CrashReportClient, TCHAR_TO_UTF8(*ReportClient), PATH_MAX + 1);
	struct stat Stat;
	stat(CrashReportClient, &Stat);
	bEnableCustomSignalHandling = (S_ISREG(Stat.st_mode)) && ((Stat.st_mode & S_IXUSR) != 0);	// CRC exists and executable
#endif

	if (bEnableCustomSignalHandling && !FMacApplicationInfo::CrashReporter && !GCrashMalloc)
	{
...
void FMacCrashContext::GenerateInfoInFolder(char const* const InfoFolder) const
{
	...
	if(!mkdir(CrashInfoFolder, 0766))
	{
		...
		// generate additional files from providers.
		DumpAdditionalContext(*FString(CrashInfoFolder));
		...
		ConvertMinidump(FilePath, GMacAppInfo.PLCrashReportPath);

...
bool FMacPlatformProcess::ExecProcess( const TCHAR* URL, const TCHAR* Params, int32* OutReturnCode, FString* OutStdOut, FString* OutStdErr, const TCHAR* OptionalWorkingDirectory, bool bShouldEndWithParentProcess)
{
	...
	FProcHandle ProcHandle = FPlatformProcess::CreateProcInternal(*ExecutableFileName, *CmdLineParams, bLaunchDetached, bLaunchHidden, bLaunchReallyHidden, NULL, 0, OptionalWorkingDirectory, PipeStdOutWrite, nullptr, PipeStdErrWrite, false);
...
TTuple<FProcHandle, UE::HAL::EProcessId> FMacPlatformProcess::CreateProc(const UE::HAL::FProcessStartInfo& StartInfo)
{
	uint32 Id;
...
FProcHandle FMacPlatformProcess::CreateProcInternal(const TCHAR* URL, const TCHAR* Parms, bool bLaunchDetached, bool bLaunchHidden, bool bLaunchReallyHidden, uint32* OutProcessID, int32 PriorityModifier, const TCHAR* OptionalWorkingDirectory, void* PipeStdOutChild, void* PipeStdInChild, void* PipeStdErrChild, bool bInheritHandles)
{
	...
	short int SpawnFlags = !bInheritHandles ? POSIX_SPAWN_CLOEXEC_DEFAULT : 0;
	...
	else if (!bInheritHandles)
	{
		if (bLaunchDetached)
		{
			posix_spawn_file_actions_addopen(&FileActions, STDERR_FILENO, "/dev/null", O_RDWR, 0);
		}
		else
		{
	...
	{
		UE_LOG(LogHAL, Warning, TEXT("FMacPlatformProcess::CreateProc: posix_spawn(\"%s\") failed (%d, %s)"), *ProcessPath, PosixSpawnErrNo, UTF8_TO_TCHAR(strerror(PosixSpawnErrNo)));
		return FProcHandle();
	}
	...
	return FProcHandle(new FProcState(ChildPid));
}
...
namespace UE::Mac
{
	namespace
	{
		bool bProcessReaperStarted = false;
		// Using semaphore_t as it's async-signal-safe
		semaphore_t ProcessReaperSemaphore;
		std::atomic_bool bProcessReaperSignaled = false;
		// Using pthread_mutex_t so it's not dynamic initialized
		pthread_mutex_t ProcessesToReapMutex = PTHREAD_MUTEX_INITIALIZER;
		// [[clang::no_destroy]] required here to not potentially crash on exit
		[[clang::no_destroy]] TSet<pid_t> ProcessesToReap;
		
		void ReleaseProcessReaper()
		{
			if (!bProcessReaperSignaled.exchange(true, std::memory_order_seq_cst))
			{
				verify(semaphore_signal(ProcessReaperSemaphore) == KERN_SUCCESS);
			}
		}
		
		void AcquireProcessReaper()
		{
			verify(semaphore_wait(ProcessReaperSemaphore) == KERN_SUCCESS);
			bProcessReaperSignaled.store(false, std::memory_order_seq_cst);
		}
	}

	void StartProcessReaper()
	{
		if (bProcessReaperStarted)
		{
			return;
		}
		
		bProcessReaperStarted = true;
		
		verify(semaphore_create(mach_task_self(), &ProcessReaperSemaphore, SYNC_POLICY_FIFO, 0) == KERN_SUCCESS);

		// Using pthread_t so it runs forever
		pthread_t Thread;
		verify(pthread_create(&Thread, nullptr, [](void*) -> void*
		{
			auto Get = []() -> pid_t
			{
				siginfo_t SigInfo;
				SigInfo.si_pid = 0;
				while (waitid(P_ALL, 0, &SigInfo, WEXITED | WNOHANG | WNOWAIT) && errno == EINTR);
				return SigInfo.si_pid;
			};
			
			auto Reap = [](pid_t PId) -> bool
			{
				do
				{
					if (pid_t Result = waitpid(PId, nullptr, WNOHANG); Result >= 0)
					{
						return Result;
					}
				}
			...
			for (;;)
			{
				AcquireProcessReaper();
				
				while (pid_t PId = Get())
				{
					verify(pthread_mutex_lock(&ProcessesToReapMutex) == 0);
					ON_SCOPE_EXIT
					{
						verify(pthread_mutex_unlock(&ProcessesToReapMutex) == 0);
					};

					if (!ProcessesToReap.Remove(PId))
					{
						for (TSet<pid_t>::TIterator Iterator = ProcessesToReap; Iterator; ++Iterator)
						{
							if (Reap(*Iterator))
							{
		...
		{
			ReleaseProcessReaper();
		};
		sigemptyset(&Action.sa_mask);
		Action.sa_flags = SA_NOCLDSTOP | SA_RESTART;
		verify(sigaction(SIGCHLD, &Action, nullptr) == 0);
	}

	void AddToProcessReaper(pid_t PId)
	{
		verify(pthread_mutex_lock(&ProcessesToReapMutex) == 0);
		ON_SCOPE_EXIT
		{
...
FProcState::~FProcState()
{
	if (IsRunning())
	{
		UE::Mac::AddToProcessReaper(ProcessId);
	}
}
...
void FProcState::Wait()
{
	...
	{
		siginfo_t SignalInfo;
		if (waitid(P_PID, GetProcessId(), &SignalInfo, WEXITED | WNOWAIT))
		{
			if (errno != EINTR)
			{
				int ErrNo = errno;
...
bool FMacPlatformProcess::CreatePipe( void*& ReadPipe, void*& WritePipe, bool bWritePipeLocal )
{
	...
	else
	{
		...
		fcntl(pipefd[0], F_SETFL, O_CLOEXEC | O_NONBLOCK);
	}

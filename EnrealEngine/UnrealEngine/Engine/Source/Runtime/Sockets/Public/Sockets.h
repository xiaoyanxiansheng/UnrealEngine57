...
public:
	...
	/**
	 * Releases the underlying socket implementation for use with native APIs.
	 * The user of this class must know the underlying implementation details in order to cast the return value properly.
	 * If this function succeeds, the FSocket instance no longer controls the connection (it will not Close()).
	 * @return a non-zero value upon success, or zero if not implemented/failure to release ownership.
	 */
	[[nodiscard]] virtual UPTRINT ReleaseNativeSocket() { return 0; }
	...
	*/
	 ...
	 */
	inline ESocketType GetSocketType() const
	{
		return SocketType;
	}
	 ...
	 * @see GetDescription, GetPortNo
	 */
	inline FName GetProtocol() const
	{

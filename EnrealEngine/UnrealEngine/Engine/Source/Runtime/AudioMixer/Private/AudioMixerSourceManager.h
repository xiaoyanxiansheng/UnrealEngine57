...
namespace Audio
{
	...
	public:
		...
		{
			PendingAudioBusConnections.Enqueue(FPendingAudioBusConnection{ FPendingAudioBusConnection::FPatchVariant(TInPlaceType<FPatchInput>(), MoveTemp(PatchInput)), MoveTemp(AudioBusKey), NumChannels, bIsAutomatic, InstanceID });
		}
		...
		{
			PendingAudioBusConnections.Enqueue(FPendingAudioBusConnection{ FPendingAudioBusConnection::FPatchVariant(TInPlaceType<FPatchOutputStrongPtr>(), MoveTemp(PatchOutputStrongPtr)), MoveTemp(AudioBusKey), NumChannels, bIsAutomatic, InstanceID });
		}

		FAudioRenderScheduler& GetRenderScheduler() const
		{
			return MixerDevice->GetRenderScheduler();
		}
...
#define INVALID_AUDIO_RENDER_THREAD_ID static_cast<uint32>(-1)
		...
		void ComputeBuses();
		void UpdateBuses();
		void AddBusPatchDependency(const uint64 InstanceID, const FAudioRenderStepId FirstStep, const FAudioRenderStepId SecondStep);
		void RemoveBusPatchDependencies(const uint64 InstanceID);
		...
		TMultiMap<uint64, TPair<FAudioRenderStepId, FAudioRenderStepId>> BusPatchDependencies;

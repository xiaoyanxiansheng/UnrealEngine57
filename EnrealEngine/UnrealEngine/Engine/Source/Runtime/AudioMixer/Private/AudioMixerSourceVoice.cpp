...
namespace Audio
{
	...
	*/
	...
	void FMixerSourceVoice::SetAudioBusSendInfo(EBusSendType InBusSendType, uint32 AudioBusId, float BusSendLevel, const FString& InBusName)
	{
		AUDIO_MIXER_CHECK_GAME_THREAD(MixerDevice);
		...
		SourceManager->SetBusSendInfo(SourceId, InBusSendType, AudioBusId, BusSendLevel, InBusName);
	}

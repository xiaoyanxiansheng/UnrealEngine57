...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AudioMixerDevice)
...
UE_TRACE_EVENT_BEGIN(Audio, SubmixAlivePing)
	...
	UE_TRACE_EVENT_FIELD(bool, IsMainSubmix)
UE_TRACE_EVENT_END()
...
namespace Audio
{
		...
		, bSubmixRegistrationDisabled(true)
	{
		...
		RenderScheduler = MakeUnique<FAudioRenderScheduler>(this);
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
		FTraceAuxiliary::OnTraceStarted.AddRaw(this, &FMixerDevice::OnTraceStarted);
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
	}
	...
	FMixerDevice::~FMixerDevice()
	{
		...
		// The rendering thread has been stopped now, so it is safe to run any cleanup code that
		//  would normally be run only on the rendering thread.
		ResetAudioRenderingThreadId();
	}

#if UE_AUDIO_PROFILERTRACE_ENABLED
	void FMixerDevice::OnTraceStarted(FTraceAuxiliary::EConnectionType TraceType, const FString& TraceDestination)
	{
		SendSubmixesAlivePing();
	}

	void FMixerDevice::SendSubmixesAlivePing()
	{
		const bool bChannelEnabled = UE_TRACE_CHANNELEXPR_IS_ENABLED(AudioChannel);
		if (bChannelEnabled)
		{
			auto TraceSubmixLoadedMessage = [this](const FMixerSubmixPtr& InSubmix)
			{
				UE_TRACE_LOG(Audio, SubmixAlivePing, AudioChannel)
					<< SubmixAlivePing.DeviceId(DeviceID)
					<< SubmixAlivePing.SubmixId(GetTypeHash(InSubmix->GetPathName()))
					<< SubmixAlivePing.Timestamp(FPlatformTime::Cycles64())
					<< SubmixAlivePing.Name(*InSubmix->GetPathName())
					<< SubmixAlivePing.NumChannels(InSubmix->NumChannels)
					<< SubmixAlivePing.IsMainSubmix(InSubmix == GetMainSubmix());
			};

			Submixes.Iterate([this, &TraceSubmixLoadedMessage](const FSubmixMap::FPair& SubmixPair)
			{
				const FMixerSubmixPtr& Submix = SubmixPair.Value;
				if (Submix.IsValid())
				{
					TraceSubmixLoadedMessage(Submix);
				}
			});

			for (const FMixerSubmixPtr& RequiredSubmix : RequiredSubmixInstances)
			{
				if (RequiredSubmix.IsValid())
				{
					TraceSubmixLoadedMessage(RequiredSubmix);
				}
			}
		}
	}

	void FMixerDevice::UpdateSubmixesAlivePingTime()
	{
		SubmixesAlivePingTimeSec += AudioClockDelta;

		if (SubmixesAlivePingTimeSec > SubmixesAlivePingWaitTimeSec)
		{
			SendSubmixesAlivePing();
			SubmixesAlivePingTimeSec = 0.0;
		}
	}

	void FMixerDevice::RegisterSubmixEnvelopeFollowerCVars()
	{
		// StartSubmixEnvelopeFollowerCVar
		StartSubmixEnvelopeFollowerCVar = IConsoleManager::Get().RegisterConsoleVariable(
			*FString::Printf(TEXT("au.StartSubmixEnvelopeFollower_AD%d"), DeviceID),
			FString(),
			TEXT("Starts a submix envelope follower (args= submix name)\n"),
			ECVF_Default
		);

		if (StartSubmixEnvelopeFollowerCVar)
		{
			StartSubmixEnvelopeFollowerCVar->SetOnChangedCallback(FConsoleVariableDelegate::CreateRaw(this, &FMixerDevice::OnStartSubmixEnvelopeFollowerCVarChanged));
		}

		// StopSubmixEnvelopeFollowerCVar
		StopSubmixEnvelopeFollowerCVar = IConsoleManager::Get().RegisterConsoleVariable(
			*FString::Printf(TEXT("au.StopSubmixEnvelopeFollower_AD%d"), DeviceID),
			FString(),
			TEXT("Stops a submix envelope follower (args= submix name)\n"),
			ECVF_Default
		);

		if (StopSubmixEnvelopeFollowerCVar)
		{
			StopSubmixEnvelopeFollowerCVar->SetOnChangedCallback(FConsoleVariableDelegate::CreateRaw(this, &FMixerDevice::OnStopSubmixEnvelopeFollowerCVarChanged));
		}
	}

	void FMixerDevice::UnregisterSubmixEnvelopeFollowerCVars()
	{
		IConsoleManager::Get().UnregisterConsoleObject(StartSubmixEnvelopeFollowerCVar);
		IConsoleManager::Get().UnregisterConsoleObject(StopSubmixEnvelopeFollowerCVar);
	}

	void FMixerDevice::OnStartSubmixEnvelopeFollowerCVarChanged(IConsoleVariable* InStartSubmixEnvelopeFollowerCVar)
	{
		ensure(IsInGameThread());

		if (InStartSubmixEnvelopeFollowerCVar)
		{
			FAudioThread::RunCommandOnAudioThread([this, SubmixName = InStartSubmixEnvelopeFollowerCVar->GetString()]()
			{
				StartSubmixEnvelopeFollowerFromCVar(SubmixName);
			});
		}
	}

	void FMixerDevice::OnStopSubmixEnvelopeFollowerCVarChanged(IConsoleVariable* InStopSubmixEnvelopeFollowerCVar)
	{
		ensure(IsInGameThread());

		if (InStopSubmixEnvelopeFollowerCVar)
		{
			FAudioThread::RunCommandOnAudioThread([this, SubmixName = InStopSubmixEnvelopeFollowerCVar->GetString()]()
			{
				StopSubmixEnvelopeFollowerFromCVar(SubmixName);
			});
		}
	}

	void FMixerDevice::StartSubmixEnvelopeFollowerFromCVar(const FString& InSubmixName)
	{
		ensure(IsInAudioThread());

		// Attack/Release default times used in USoundSubmix constructor
		constexpr int32 AttackTime  = 10;
		constexpr int32 ReleaseTime = 500;

		Submixes.Iterate([this, &InSubmixName, AttackTime, ReleaseTime](const FSubmixMap::FPair& SubmixPair)
		{
			const FMixerSubmixPtr& Submix = SubmixPair.Value;

			if (Submix.IsValid() && Submix->GetName() == InSubmixName)
			{
				Submix->StartEnvelopeFollowing(AttackTime, ReleaseTime);
			}
		});

		for (const FMixerSubmixPtr& RequiredSubmix : RequiredSubmixInstances)
		{
			if (RequiredSubmix.IsValid() && RequiredSubmix->GetName() == InSubmixName)
			{
				RequiredSubmix->StartEnvelopeFollowing(AttackTime, ReleaseTime);
			}
		}
	}

	void FMixerDevice::StopSubmixEnvelopeFollowerFromCVar(const FString& InSubmixName)
	{
		ensure(IsInAudioThread());

		Submixes.Iterate([this, &InSubmixName](const FSubmixMap::FPair& SubmixPair)
		{
			const FMixerSubmixPtr& Submix = SubmixPair.Value;

			if (Submix.IsValid() && Submix->GetName() == InSubmixName)
			{
				Submix->StopEnvelopeFollowing();
			}
		});
		...
		for (const FMixerSubmixPtr& RequiredSubmix : RequiredSubmixInstances)
		{
			if (RequiredSubmix.IsValid() && RequiredSubmix->GetName() == InSubmixName)
			{
				RequiredSubmix->StopEnvelopeFollowing();
			}
		}
	}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
				RegisterSubmixEnvelopeFollowerCVars();
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
	...
	void FMixerDevice::TeardownHardware()
	{
		...
		{
#if UE_AUDIO_PROFILERTRACE_ENABLED
			UnregisterSubmixEnvelopeFollowerCVars();
#endif // UE_AUDIO_PROFILERTRACE_ENABLED

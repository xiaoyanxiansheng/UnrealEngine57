...
namespace Audio
{
	...
	static void* GetSoundFileDllHandle()
	{
		void* DllHandle = nullptr;
			...
			const FString PlatformPath = TEXT("Win64/arm64/");
		#else
	...
	*/
	...
	public:
		...
		ESoundFileError::Type WriteBytes(const void* DataPtr, SoundFileCount NumBytes, SoundFileCount& OutNumBytesWritten) override
		{
			...
			if (BulkData.Num() + NumExtraBytesNeeded > INT32_MAX)
			{
				UE_LOG(LogAudioMixer, Error, TEXT("Error encountered while writing bytes, file too large... skipping"));
			...
			{
				check(BulkData.Num() + NumExtraBytesNeeded <= INT32_MAX);

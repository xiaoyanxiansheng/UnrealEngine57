...
UE_TRACE_EVENT_BEGIN(Audio, SubmixLoaded)
...
UE_TRACE_EVENT_BEGIN(Audio, SubmixHasActivity)
	UE_TRACE_EVENT_FIELD(uint32, DeviceId)
	UE_TRACE_EVENT_FIELD(uint32, SubmixId)
	UE_TRACE_EVENT_FIELD(uint64, Timestamp)
...
UE_TRACE_EVENT_BEGIN(Audio, SubmixEnvelopeFollowerEnabled)
	UE_TRACE_EVENT_FIELD(uint32, DeviceId)
	UE_TRACE_EVENT_FIELD(uint32, SubmixId)
	UE_TRACE_EVENT_FIELD(uint64, Timestamp)
...
UE_TRACE_EVENT_BEGIN(Audio, SubmixEnvelopeValues)
	UE_TRACE_EVENT_FIELD(uint32, DeviceId)
	UE_TRACE_EVENT_FIELD(uint32, SubmixId)
	UE_TRACE_EVENT_FIELD(uint64, Timestamp)
...
UE_TRACE_EVENT_BEGIN(Audio, SubmixUnloaded)
	UE_TRACE_EVENT_FIELD(uint32, DeviceId)
	UE_TRACE_EVENT_FIELD(uint32, SubmixId)
	UE_TRACE_EVENT_FIELD(uint64, Timestamp)
UE_TRACE_EVENT_END()
...
namespace Audio
{
	...
	{
		bool IsAudioBufferSilent(const FAlignedFloatBuffer& InAudioBuffer)
		{
			...
			const int32 SimdNum = InAudioBuffer.Num() & 0xFFFFFFF0;
			for (; Index < SimdNum; Index += 16)
			{
				const VectorRegister4x4Float Samples = VectorLoad16(&InAudioBuffer[Index]);
			...
			for (; Index < InAudioBuffer.Num(); ++Index)
			{
				// As soon as we hit a non-silent sample, we're not silent
				if (FMath::Abs(InAudioBuffer[Index]) > SMALL_NUMBER)
				{
					bIsSilent = false;
					break;
				}
	...
	FMixerSubmix::~FMixerSubmix()
	{
		...
		if (bChannelEnabled)
		{
			UE_TRACE_LOG(Audio, SubmixUnloaded, AudioChannel)
				<< SubmixUnloaded.DeviceId(MixerDevice->DeviceID)
				<< SubmixUnloaded.SubmixId(GetTypeHash(OwningSubmixObject->GetPathName()))
				<< SubmixUnloaded.Timestamp(FPlatformTime::Cycles64());
		}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
	}
	...
	void FMixerSubmix::Init(const USoundSubmixBase* InSoundSubmix, bool bAllowReInit)
	{
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
			SubmixPathName = InSoundSubmix->GetPathName();
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
		...
		if (bChannelEnabled)
		{
			UE_TRACE_LOG(Audio, SubmixLoaded, AudioChannel)
				<< SubmixLoaded.DeviceId(MixerDevice->DeviceID)
				<< SubmixLoaded.SubmixId(GetTypeHash(OwningSubmixObject->GetPathName()))
				<< SubmixLoaded.Timestamp(FPlatformTime::Cycles64())
				<< SubmixLoaded.Name(*OwningSubmixObject->GetPathName())
				<< SubmixLoaded.NumChannels(NumChannels)
				<< SubmixLoaded.IsMainSubmix(InSoundSubmix == &MixerDevice->GetMainSubmixObject());
		}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
	}
		...
		else if (const USoundfieldEndpointSubmix* SoundfieldEndpointSubmix = Cast<const USoundfieldEndpointSubmix>(OwningSubmixObject))
		{
			...
			if (IsSoundfieldSubmix())
			{
				...
				if (!EndpointSettings)
				{
					...
					if (!ensureMsgf(EndpointSettings, TEXT("Soundfield Endpoint %s did not return default endpoint settings! Is ISoundfieldEndpointFactory::GetDefaultEndpointSettings() implemented?"), *SoundfieldFactory->GetEndpointTypeName().ToString()))
					{
						return;
					}
				}
	...
	void FMixerSubmix::ProcessAudio(FAlignedFloatBuffer& OutAudioBuffer)
	{
		...
		if (bAutoDisable && !IsRenderingAudio())
		{
			if (!bIsCurrentlyDisabled)
			{
				...
				VolumeMod.ClearModulators();
				WetLevelMod.ClearModulators();
				DryLevelMod.ClearModulators();
				
#if UE_AUDIO_PROFILERTRACE_ENABLED
				const bool bChannelEnabled = UE_TRACE_CHANNELEXPR_IS_ENABLED(AudioChannel);
				if (bChannelEnabled && OwningSubmixObject.IsValid())
				{
					TArray<float> SilentEnvelopeValues;
					SilentEnvelopeValues.Init(0.0f, NumChannels);

					UE_TRACE_LOG(Audio, SubmixEnvelopeValues, AudioChannel)
						<< SubmixEnvelopeValues.DeviceId(MixerDevice->DeviceID)
						<< SubmixEnvelopeValues.SubmixId(GetTypeHash(OwningSubmixObject->GetPathName()))
						<< SubmixEnvelopeValues.Timestamp(FPlatformTime::Cycles64())
						<< SubmixEnvelopeValues.EnvelopeValues(SilentEnvelopeValues.GetData(), NumChannels);
				}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED

				UE_CLOG(LogSubmixEnablementCVar == 1, LogAudioMixer, Verbose,
		...
		if (bIsCurrentlyDisabled)
		{
			if (const USoundSubmix* SoundSubmix = Cast<const USoundSubmix>(OwningSubmixObject))
			{
				const TSet<TObjectPtr<USoundModulatorBase>>& VolumeModulator = SoundSubmix->OutputVolumeModulation.Modulators;
				const TSet<TObjectPtr<USoundModulatorBase>>& WetLevelModulator = SoundSubmix->WetLevelModulation.Modulators;
				const TSet<TObjectPtr<USoundModulatorBase>>& DryLevelModulator = SoundSubmix->DryLevelModulation.Modulators;
				UpdateModulationSettings(VolumeModulator, WetLevelModulator, DryLevelModulator);
			}
			
			bIsCurrentlyDisabled = false;
			UE_CLOG(LogSubmixEnablementCVar == 1, LogAudioMixer, Verbose, TEXT("Submix Re-Enabled: %s"), *SubmixName);
		}
			...
			if (NumChannels > 0)
			{
				...
				if (bChannelEnabled)
				{
					UE_TRACE_LOG(Audio, SubmixEnvelopeValues, AudioChannel)
						...
						<< SubmixEnvelopeValues.SubmixId(GetTypeHash(OwningSubmixObject->GetPathName()))
						<< SubmixEnvelopeValues.Timestamp(FPlatformTime::Cycles64())
						<< SubmixEnvelopeValues.EnvelopeValues(EnvelopeValues, NumChannels);
				}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
			}
		...
		bool bIsNowSilent = false;

#if UE_AUDIO_PROFILERTRACE_ENABLED
		const bool bShouldTraceHasActivityMsg = UE_TRACE_CHANNELEXPR_IS_ENABLED(AudioChannel) && OwningSubmixObject.IsValid();
#endif // UE_AUDIO_PROFILERTRACE_ENABLED

		if (bAutoDisable)
		{
			bIsNowSilent = FMixerSubmixPrivate::IsAudioBufferSilent(OutAudioBuffer);
		...
		if (bShouldTraceHasActivityMsg)
		{
			UE_TRACE_LOG(Audio, SubmixHasActivity, AudioChannel)
				<< SubmixHasActivity.DeviceId(MixerDevice->DeviceID)
				<< SubmixHasActivity.SubmixId(GetTypeHash(OwningSubmixObject->GetPathName()))
	...
	void FMixerSubmix::StartEnvelopeFollowing(int32 AttackTime, int32 ReleaseTime)
	{
		if (!bIsEnvelopeFollowing)
		{
			...
			if (bChannelEnabled && OwningSubmixObject.IsValid())
			{
				UE_TRACE_LOG(Audio, SubmixEnvelopeFollowerEnabled, AudioChannel)
					<< SubmixEnvelopeFollowerEnabled.DeviceId(MixerDevice->DeviceID)
					<< SubmixEnvelopeFollowerEnabled.SubmixId(GetTypeHash(OwningSubmixObject->GetPathName()))
					<< SubmixEnvelopeFollowerEnabled.Timestamp(FPlatformTime::Cycles64())
					<< SubmixEnvelopeFollowerEnabled.EnvelopeFollowerEnabled(true);
			}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
		}
	}
	...
	void FMixerSubmix::StopEnvelopeFollowing()
	{
		...
		if (bChannelEnabled && OwningSubmixObject.IsValid())
		{
			UE_TRACE_LOG(Audio, SubmixEnvelopeFollowerEnabled, AudioChannel)
				...
				<< SubmixEnvelopeFollowerEnabled.SubmixId(GetTypeHash(OwningSubmixObject->GetPathName()))
				<< SubmixEnvelopeFollowerEnabled.Timestamp(FPlatformTime::Cycles64())
				<< SubmixEnvelopeFollowerEnabled.EnvelopeFollowerEnabled(false);
		}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
	}
	...
	void FMixerSubmix::GetPhaseForFrequencies(const TArray<float>& InFrequencies, TArray<float>& OutPhases)
	{
		...
		{
			UE_LOG(LogAudioMixer, Warning, TEXT("Call StartSpectrumAnalysis before calling GetPhaseForFrequencies."));
		}
	}

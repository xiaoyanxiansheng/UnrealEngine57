...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AudioMixerSourceManager)
...
static bool bDumpCallstackOnStallDiagnostics = true;
FAutoConsoleVariableRef CVarDumpCallstackOnStallDiagnostics(
	TEXT("au.DumpCallstackOnStallDiagnostics"),
...
static int32 PerSourceResampling = 1;
...
UE_TRACE_EVENT_BEGIN(Audio, SoundVolume)
...
UE_TRACE_EVENT_BEGIN(Audio, SoundDistanceAttenuation)
	UE_TRACE_EVENT_FIELD(uint32, DeviceId)
	UE_TRACE_EVENT_FIELD(uint64, Timestamp)
	UE_TRACE_EVENT_FIELD(uint32, PlayOrder)
	UE_TRACE_EVENT_FIELD(uint32, ActiveSoundPlayOrder)
...
UE_TRACE_EVENT_BEGIN(Audio, SoundPitch)
	UE_TRACE_EVENT_FIELD(uint32, DeviceId)
	UE_TRACE_EVENT_FIELD(uint64, Timestamp)
	UE_TRACE_EVENT_FIELD(uint32, PlayOrder)
	UE_TRACE_EVENT_FIELD(uint32, ActiveSoundPlayOrder)
...
UE_TRACE_EVENT_BEGIN(Audio, SoundSourceFilters)
	UE_TRACE_EVENT_FIELD(uint32, DeviceId)
	UE_TRACE_EVENT_FIELD(uint64, Timestamp)
	UE_TRACE_EVENT_FIELD(uint32, PlayOrder)
	UE_TRACE_EVENT_FIELD(uint32, ActiveSoundPlayOrder)
...
UE_TRACE_EVENT_BEGIN(Audio, SoundSourceEnvelope)
	UE_TRACE_EVENT_FIELD(uint32, DeviceId)
	UE_TRACE_EVENT_FIELD(uint64, Timestamp)
	UE_TRACE_EVENT_FIELD(uint32, PlayOrder)
	UE_TRACE_EVENT_FIELD(uint32, ActiveSoundPlayOrder)
...
// Audio Bus messages
UE_TRACE_EVENT_BEGIN(Audio, AudioBusStart)
	UE_TRACE_EVENT_FIELD(uint32, DeviceId)
	UE_TRACE_EVENT_FIELD(uint32, AudioBusId)
	UE_TRACE_EVENT_FIELD(uint64, Timestamp)
	UE_TRACE_EVENT_FIELD(UE::Trace::WideString, Name)
	UE_TRACE_EVENT_FIELD(int32, NumChannels)
UE_TRACE_EVENT_END()

UE_TRACE_EVENT_BEGIN(Audio, AudioBusStop)
	UE_TRACE_EVENT_FIELD(uint32, DeviceId)
	UE_TRACE_EVENT_FIELD(uint32, AudioBusId)
	UE_TRACE_EVENT_FIELD(uint64, Timestamp)
UE_TRACE_EVENT_END()
...
UE_TRACE_EVENT_BEGIN(Audio, AudioBusEnvelopeFollowerEnabled)
	UE_TRACE_EVENT_FIELD(uint32, DeviceId)
	UE_TRACE_EVENT_FIELD(uint32, AudioBusId)
	UE_TRACE_EVENT_FIELD(uint64, Timestamp)
	UE_TRACE_EVENT_FIELD(bool, EnvelopeFollowerEnabled)
UE_TRACE_EVENT_END()

UE_TRACE_EVENT_BEGIN(Audio, AudioBusEnvelopeValues)
	UE_TRACE_EVENT_FIELD(uint32, DeviceId)
	UE_TRACE_EVENT_FIELD(uint32, AudioBusId)
	UE_TRACE_EVENT_FIELD(uint64, Timestamp)
	UE_TRACE_EVENT_FIELD(float[], EnvelopeValues)
UE_TRACE_EVENT_END()
...
namespace Audio
{
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
	void TraceAudioBusStartMessage(const uint32 InDeviceId, const uint32 InObjectId, const FString& InAudioBusName, const int32 InNumChannels)
	{
		const bool bChannelEnabled = UE_TRACE_CHANNELEXPR_IS_ENABLED(AudioMixerChannel);
		if (bChannelEnabled)
		{
			UE_TRACE_LOG(Audio, AudioBusStart, AudioChannel)
				<< AudioBusStart.DeviceId(InDeviceId)
				<< AudioBusStart.AudioBusId(InObjectId)
				<< AudioBusStart.Timestamp(FPlatformTime::Cycles64())
				<< AudioBusStart.Name(InAudioBusName.IsEmpty() ? *FString::Printf(TEXT("%d"), InObjectId) : *InAudioBusName)
				<< AudioBusStart.NumChannels(InNumChannels);
		}
	}

	void TraceAudioBusStopMessage(const uint32 InDeviceId, const uint32 InObjectId)
	{
		const bool bChannelEnabled = UE_TRACE_CHANNELEXPR_IS_ENABLED(AudioMixerChannel);
		if (bChannelEnabled)
		{
			UE_TRACE_LOG(Audio, AudioBusStop, AudioChannel)
				<< AudioBusStop.DeviceId(InDeviceId)
				<< AudioBusStop.AudioBusId(InObjectId)
				<< AudioBusStop.Timestamp(FPlatformTime::Cycles64());
		}
	}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
	...
	**************************************************************************/
	...
	FMixerSourceManager::~FMixerSourceManager()
	{
		...
		UnregisterAudioBusEnvelopeFollowerCVars();
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
	void FMixerSourceManager::OnTraceStarted(FTraceAuxiliary::EConnectionType TraceType, const FString& TraceDestination)
	{
		for (const auto& [AudioBusKey, AudioBus] : AudioBuses)
		{
			if (AudioBus.IsValid())
			{
 				TraceAudioBusStartMessage(MixerDevice->DeviceID, AudioBusKey.ObjectId, AudioBus->BusName, AudioBus->GetNumChannels());
			}
		}
	}

	void FMixerSourceManager::RegisterAudioBusEnvelopeFollowerCVars()
	{
		// StartAudioBusEnvelopeFollowerCVar
		StartAudioBusEnvelopeFollowerCVar = IConsoleManager::Get().RegisterConsoleVariable(
			*FString::Printf(TEXT("au.StartAudioBusEnvelopeFollower_AD%d"), MixerDevice->DeviceID),
			FString(),
			TEXT("Starts an audio bus envelope follower (args= audio bus id)\n"),
			ECVF_Default
		);

		if (StartAudioBusEnvelopeFollowerCVar)
		{
			StartAudioBusEnvelopeFollowerCVar->SetOnChangedCallback(FConsoleVariableDelegate::CreateRaw(this, &FMixerSourceManager::OnStartAudioBusEnvelopeFollowerCVarChanged));
		}

		// StopAudioBusEnvelopeFollowerCVar
		StopAudioBusEnvelopeFollowerCVar = IConsoleManager::Get().RegisterConsoleVariable(
			*FString::Printf(TEXT("au.StopAudioBusEnvelopeFollower_AD%d"), MixerDevice->DeviceID),
			FString(),
			TEXT("Stops an audio bus envelope follower (args= audio bus id)\n"),
			ECVF_Default
		);

		if (StopAudioBusEnvelopeFollowerCVar)
		{
			StopAudioBusEnvelopeFollowerCVar->SetOnChangedCallback(FConsoleVariableDelegate::CreateRaw(this, &FMixerSourceManager::OnStopAudioBusEnvelopeFollowerCVarChanged));
		}
	}

	void FMixerSourceManager::UnregisterAudioBusEnvelopeFollowerCVars()
	{
		IConsoleManager::Get().UnregisterConsoleObject(StartAudioBusEnvelopeFollowerCVar);
		IConsoleManager::Get().UnregisterConsoleObject(StopAudioBusEnvelopeFollowerCVar);
	}

	void FMixerSourceManager::OnStartAudioBusEnvelopeFollowerCVarChanged(IConsoleVariable* InStartAudioBusEnvelopeFollowerCVar)
	{
		ensure(IsInGameThread());

		if (InStartAudioBusEnvelopeFollowerCVar)
		{
			uint32 AudioBusId = INDEX_NONE;
			LexFromString(AudioBusId, *InStartAudioBusEnvelopeFollowerCVar->GetString());

			FAudioThread::RunCommandOnAudioThread([this, AudioBusId]()
			{
				StartAudioBusEnvelopeFollowerFromCVar(AudioBusId);
			});
		}
	}

	void FMixerSourceManager::OnStopAudioBusEnvelopeFollowerCVarChanged(IConsoleVariable* InStopAudioBusEnvelopeFollowerCVar)
	{
		ensure(IsInGameThread());

		if (InStopAudioBusEnvelopeFollowerCVar)
		{
			uint32 AudioBusId = INDEX_NONE;
			LexFromString(AudioBusId, *InStopAudioBusEnvelopeFollowerCVar->GetString());

			FAudioThread::RunCommandOnAudioThread([this, AudioBusId]()
			{
				StopAudioBusEnvelopeFollowerFromCVar(AudioBusId);
			});
		}
	}

	void FMixerSourceManager::StartAudioBusEnvelopeFollowerFromCVar(const uint32 InAudioBusId)
	{
		ensure(IsInAudioThread());

		constexpr float AttackTime  = 10.0f;
		constexpr float ReleaseTime = 500.0f;

		for (auto& [AudioBusKey, AudioBus] : AudioBuses)
		{
			if (AudioBusKey.ObjectId == InAudioBusId && AudioBus.IsValid())
			{
				AudioBus->StartEnvelopeFollower(AttackTime, ReleaseTime, MixerDevice->GetSampleRate());

				const bool bChannelEnabled = UE_TRACE_CHANNELEXPR_IS_ENABLED(AudioChannel);
				if (bChannelEnabled && AudioBus->bIsEnvelopeFollowing)
				{
					UE_TRACE_LOG(Audio, AudioBusEnvelopeFollowerEnabled, AudioChannel)
						<< AudioBusEnvelopeFollowerEnabled.DeviceId(MixerDevice->DeviceID)
						<< AudioBusEnvelopeFollowerEnabled.AudioBusId(AudioBusKey.ObjectId)
						<< AudioBusEnvelopeFollowerEnabled.Timestamp(FPlatformTime::Cycles64())
						<< AudioBusEnvelopeFollowerEnabled.EnvelopeFollowerEnabled(true);
				}
	...
	void FMixerSourceManager::StopAudioBusEnvelopeFollowerFromCVar(const uint32 InAudioBusId)
	{
		ensure(IsInAudioThread());
			...
			if (AudioBusKey.ObjectId == InAudioBusId && AudioBus.IsValid())
			{
				...
				if (bChannelEnabled && !AudioBus->bIsEnvelopeFollowing)
				{
					UE_TRACE_LOG(Audio, AudioBusEnvelopeFollowerEnabled, AudioChannel)
						...
						<< AudioBusEnvelopeFollowerEnabled.AudioBusId(AudioBusKey.ObjectId)
						<< AudioBusEnvelopeFollowerEnabled.Timestamp(FPlatformTime::Cycles64())
						<< AudioBusEnvelopeFollowerEnabled.EnvelopeFollowerEnabled(false);
				}
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
		RegisterAudioBusEnvelopeFollowerCVars();
		FTraceAuxiliary::OnTraceStarted.AddRaw(this, &FMixerSourceManager::OnTraceStarted);
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
	}
	...
	void FMixerSourceManager::ReleaseSource(const int32 SourceId)
	{
		...
		if (SourceInfo.AudioBusId != INDEX_NONE)
		{
			...
			if (AudioBusPtr.IsValid())
			{
				...
				if (AudioBusPtr->RemoveInstanceId(SourceId, SourceInfo.MixerSourceBuffer->GetInstanceID()))
				{
#if UE_AUDIO_PROFILERTRACE_ENABLED
					TraceAudioBusStopMessage(MixerDevice->DeviceID, SourceInfo.AudioBusId);
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
		...
		for (int32 AudioBusSendType = 0; AudioBusSendType < (int32)EBusSendType::Count; ++AudioBusSendType)
		{
			for (uint32 AudioBusId : SourceInfo.AudioBusSends[AudioBusSendType])
			{
				...
				if (AudioBusPtr.IsValid())
				{
					...
					{
#if UE_AUDIO_PROFILERTRACE_ENABLED
						TraceAudioBusStopMessage(MixerDevice->DeviceID, AudioBusId);
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
		...
		if (SourceInfo.MixerSourceBuffer.IsValid())
		{
			...
			{
				RemoveBusPatchDependencies(SourceInfo.MixerSourceBuffer->GetInstanceID());
			}
		...
		]() mutable
		{
			...
			if (InitParams.NumInputChannels <= InitParams.SourceEffectChainMaxSupportedChannels)
			{
				...
				if (InitParams.AudioBusId != INDEX_NONE)
				{
					...
					TSharedRef<FMixerAudioBus> AudioBusPtr = FindOrAddAudioBus(SourceInfo.AudioBusId, InitParams.AudioBusChannels, true, &bAlreadyExisted);
					AudioBusPtr->AddInstanceId(SourceId, SourceInfo.MixerSourceBuffer->GetInstanceID(), InitParams.NumInputChannels);

#if UE_AUDIO_PROFILERTRACE_ENABLED
					if (!bAlreadyExisted)
					{
						AudioBusPtr->BusName = InitParams.AudioBusName;
						TraceAudioBusStartMessage(MixerDevice->DeviceID, InitParams.AudioBusId, InitParams.AudioBusName, InitParams.AudioBusChannels);
					}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
				}
			...
			for (int32 BusSendType = 0; BusSendType < (int32)EBusSendType::Count; ++BusSendType)
			{
				for (const FInitAudioBusSend& AudioBusSend : InitParams.AudioBusSends[BusSendType])
				{
					...
					NewAudioBusSend.TransmitterID = SourceInfo.MixerSourceBuffer->GetInstanceID();
					NewAudioBusSend.SendLevel = AudioBusSend.SendLevel;

					// Get existing BusId and add the send, or create new bus registration. A newly created bus won't output audio, but 
					// we register the send anyway in the event that this bus does play, we'll know to send this
					// source's audio to it.
					bool bAlreadyExisted = false;
					TSharedRef<FMixerAudioBus> AudioBusPtr = FindOrAddAudioBus(AudioBusSend.AudioBusId, AudioBusSend.BusChannels, true, &bAlreadyExisted);
					AudioBusPtr->AddSend((EBusSendType)BusSendType, NewAudioBusSend);
#if UE_AUDIO_PROFILERTRACE_ENABLED
					if (!bAlreadyExisted)
					{
						AudioBusPtr->BusName = AudioBusSend.AudioBusName;
						TraceAudioBusStartMessage(MixerDevice->DeviceID, AudioBusSend.AudioBusId, AudioBusSend.AudioBusName, AudioBusSend.BusChannels);
					}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
			...
			if (AudioMixerCVars::UseRenderScheduler)
			{
				MixerDevice->GetRenderScheduler().AddStep(FAudioRenderStepId::FromTransmitterID(SourceInfo.MixerSourceBuffer->GetInstanceID()), &SourceInfo);
			}
	...
	void FMixerSourceManager::ReleaseSourceId(const int32 SourceId)
	{
		...
		AudioMixerThreadCommand([this, SourceId]()
		{
			...
			{
				FSourceInfo& SourceInfo = SourceInfos[SourceId];
				check(SourceInfo.MixerSourceBuffer.IsValid());
				MixerDevice->GetRenderScheduler().RemoveStep(FAudioRenderStepId::FromTransmitterID(SourceInfo.MixerSourceBuffer->GetInstanceID()));
			}
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
		TraceAudioBusStartMessage(MixerDevice->DeviceID, InAudioBusKey.ObjectId, InAudioBusName, InNumChannels);
#endif

		AudioMixerThreadCommand([this, InAudioBusKey, InNumChannels, bInIsAutomatic, InAudioBusName]()
		{
			// If this audio bus id already exists, set it to not be automatic and return it
			bool bAlreadyExisted = false;
			TSharedRef<FMixerAudioBus> AudioBusPtr = FindOrAddAudioBus(InAudioBusKey, InNumChannels, bInIsAutomatic, &bAlreadyExisted);
			if (bAlreadyExisted)
			{
			...
			{
#if UE_AUDIO_PROFILERTRACE_ENABLED
				AudioBusPtr->BusName = InAudioBusName;
#endif
			}
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
		TraceAudioBusStopMessage(MixerDevice->DeviceID, InAudioBusKey.ObjectId);
#endif
	...
	void FMixerSourceManager::SetBusSendInfo(const int32 SourceId, EBusSendType InAudioBusSendType, uint32 AudioBusId, float BusSendLevel, const FString& InBusName)
	{
		AUDIO_MIXER_CHECK(SourceId < NumTotalSources);
		AUDIO_MIXER_CHECK(GameThreadInfo.bIsBusy[SourceId]);
		AUDIO_MIXER_CHECK_GAME_THREAD(MixerDevice);

		AudioMixerThreadCommand([this, SourceId, InAudioBusSendType, AudioBusId, BusSendLevel, InBusName]()
		{
			FSourceInfo& SourceInfo = SourceInfos[SourceId];
			...
			BusSend.TransmitterID = SourceInfo.MixerSourceBuffer->GetInstanceID();
			BusSend.SendLevel = BusSendLevel;

			// Retrieve the bus we want to send audio to. A newly created bus won't output audio, but 
			// we register the send anyway in the event that this bus does play, we'll know to send this
			// source's audio to it.
			bool bAlreadyExisted = false;
			TSharedRef<FMixerAudioBus> AudioBusPtr = FindOrAddAudioBus(AudioBusId, SourceInfo.NumInputChannels, true, &bAlreadyExisted);
			AudioBusPtr->AddSend(InAudioBusSendType, BusSend);

#if UE_AUDIO_PROFILERTRACE_ENABLED
			if (!bAlreadyExisted)
			{
				AudioBusPtr->BusName = InBusName;
				TraceAudioBusStartMessage(MixerDevice->DeviceID, AudioBusId, InBusName, SourceInfo.NumInputChannels);
			}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
	...
	void FMixerSourceManager::ComputeSourceBuffer(const bool bGenerateBuses, const int32 SourceId)
	{
		...
		else
		{
			...
			if (bChannelEnabled)
			{
				UE_TRACE_LOG(Audio, SoundPitch, AudioMixerChannel)
					<< SoundPitch.DeviceId(MixerDevice->DeviceID)
					<< SoundPitch.Timestamp(FPlatformTime::Cycles64())
					<< SoundPitch.PlayOrder(SourceInfo.PlayOrder)
					<< SoundPitch.ActiveSoundPlayOrder(SourceInfo.ActiveSoundPlayOrder)
					<< SoundPitch.Pitch(FinalPitch);
			}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
	...
	void FMixerSourceManager::ConnectBusPatches()
	{
		while (TOptional<FPendingAudioBusConnection> PendingAudioBusConnection = PendingAudioBusConnections.Dequeue())
		{
			...
			bool bAlreadyExisted = false;
			TSharedRef<FMixerAudioBus> AudioBusPtr = FindOrAddAudioBus(AudioBusKey, NumChannels, bIsAutomatic, &bAlreadyExisted);
			if (bAlreadyExisted)
			{
				// If this audio bus already existed, make sure the num channels lines up
			...
			case FPendingAudioBusConnection::FPatchVariant::IndexOfType<FPatchInput>():
				if (AudioMixerCVars::UseRenderScheduler)
				{
					const uint64 InstanceID = PendingAudioBusConnection->InstanceID;
					if (InstanceID != INDEX_NONE)
					{
						FAudioRenderStepId FirstStep = FAudioRenderStepId::FromTransmitterID(InstanceID);
						FAudioRenderStepId SecondStep = FAudioRenderStepId::FromAudioBusKey(AudioBusKey);
						AddBusPatchDependency(InstanceID, FirstStep, SecondStep);
					}
				}
			...
			case FPendingAudioBusConnection::FPatchVariant::IndexOfType<FPatchOutputStrongPtr>():
				if (AudioMixerCVars::UseRenderScheduler)
				{
					...
					if (InstanceID != INDEX_NONE)
					{
						FAudioRenderStepId FirstStep = FAudioRenderStepId::FromAudioBusKey(AudioBusKey);
						FAudioRenderStepId SecondStep = FAudioRenderStepId::FromTransmitterID(InstanceID);
						AddBusPatchDependency(InstanceID, FirstStep, SecondStep);
					}
				}
	...
	void FMixerSourceManager::UpdateBuses()
	{
		...
		for (auto& [AudioBusKey, AudioBus] : AudioBuses)
		{
			...
			if (bChannelEnabled)
			{
				...
				if (AudioBus->bIsEnvelopeFollowing)
				{
					UE_TRACE_LOG(Audio, AudioBusEnvelopeValues, AudioChannel)
						<< AudioBusEnvelopeValues.DeviceId(MixerDevice->DeviceID)
						<< AudioBusEnvelopeValues.AudioBusId(AudioBusKey.ObjectId)
						<< AudioBusEnvelopeValues.Timestamp(FPlatformTime::Cycles64())
						<< AudioBusEnvelopeValues.EnvelopeValues(AudioBus->EnvelopeValues, AudioBus->GetNumChannels());
				}
			}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
		}
	...
	void FMixerSourceManager::AddBusPatchDependency(const uint64 InstanceID, const FAudioRenderStepId FirstStep, const FAudioRenderStepId SecondStep)
	{
		check(AudioMixerCVars::UseRenderScheduler);
		if (FirstStep.IsValid() && SecondStep.IsValid())
		{
			GetRenderScheduler().AddDependency(FirstStep, SecondStep);
			BusPatchDependencies.Add(InstanceID, { FirstStep, SecondStep });
		}
	}

	void FMixerSourceManager::RemoveBusPatchDependencies(const uint64 InstanceID)
	{
		check(AudioMixerCVars::UseRenderScheduler);
		TArray<TPair<FAudioRenderStepId, FAudioRenderStepId>> Dependencies;
		BusPatchDependencies.MultiFind(InstanceID, Dependencies, false);
		if (!Dependencies.IsEmpty())
		{
			for (TPair<FAudioRenderStepId, FAudioRenderStepId>& Dependency : Dependencies)
			{
				GetRenderScheduler().RemoveDependency(Dependency.Get<0>(), Dependency.Get<1>());
			}
			BusPatchDependencies.Remove(InstanceID);
		}
	}
	...
	void FMixerSourceManager::ComputePostSourceEffectBuffer(const bool bGenerateBuses, const int32 SourceId)
	{
		...
		// Update volume fade information if we're stopping
		{
			...
			if (bChannelEnabled)
			{
				UE_TRACE_LOG(Audio, SoundVolume, AudioMixerChannel)
					<< SoundVolume.DeviceId(MixerDevice->DeviceID)
					<< SoundVolume.Timestamp(FPlatformTime::Cycles64())
					<< SoundVolume.PlayOrder(SourceInfo.PlayOrder)
					<< SoundVolume.ActiveSoundPlayOrder(SourceInfo.ActiveSoundPlayOrder)
					<< SoundVolume.Volume(VolumeDestination);

				UE_TRACE_LOG(Audio, SoundDistanceAttenuation, AudioMixerChannel)
					<< SoundDistanceAttenuation.DeviceId(MixerDevice->DeviceID)
					<< SoundDistanceAttenuation.Timestamp(FPlatformTime::Cycles64())
					<< SoundDistanceAttenuation.PlayOrder(SourceInfo.PlayOrder)
					<< SoundDistanceAttenuation.ActiveSoundPlayOrder(SourceInfo.ActiveSoundPlayOrder)
					<< SoundDistanceAttenuation.DistanceAttenuation(SourceInfo.DistanceAttenuationSourceDestination);
			}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
		}
		...
		if (SourceInfo.IsRenderingToSubmixes() || bUpdateModFilters)
		{
			...
			if (bChannelEnabled)
			{
				...
				UE_TRACE_LOG(Audio, SoundSourceFilters, AudioMixerChannel)
					<< SoundSourceFilters.DeviceId(MixerDevice->DeviceID)
					<< SoundSourceFilters.Timestamp(FPlatformTime::Cycles64())
					<< SoundSourceFilters.PlayOrder(SourceInfo.PlayOrder)
					<< SoundSourceFilters.ActiveSoundPlayOrder(SourceInfo.ActiveSoundPlayOrder)
					<< SoundSourceFilters.HPFFrequency(HPFFrequency)
					<< SoundSourceFilters.LPFFrequency(LPFFrequency);
				UE_TRACE_LOG(Audio, SoundSourceEnvelope, AudioMixerChannel)
					<< SoundSourceEnvelope.DeviceId(MixerDevice->DeviceID)
					<< SoundSourceEnvelope.Timestamp(FPlatformTime::Cycles64())
					<< SoundSourceEnvelope.PlayOrder(SourceInfo.PlayOrder)
					<< SoundSourceEnvelope.ActiveSoundPlayOrder(SourceInfo.ActiveSoundPlayOrder)
					<< SoundSourceEnvelope.Envelope(SourceInfo.SourceEnvelopeValue);
			}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
	...
	{
		// This check had to be disabled for now. It is safe to call this both directly from the render thread and also from render tasks.
		// AUDIO_MIXER_CHECK_AUDIO_PLAT_THREAD(MixerDevice);
	...
	void FMixerSourceManager::ComputeNextBlockOfSamples()
	{
		...
		if (AudioMixerCVars::UseRenderScheduler)
		{
			...
			MixerDevice->GetRenderScheduler().RenderBlock(DisableParallelSourceProcessingCvar != 0);
		}
		else
		{
	...
	void FMixerSourceManager::DoStallDiagnostics()
	{
		...
		if (bDumpCallstackOnStallDiagnostics)
		{
			LogCallstacks();
		}
	}
	...
	TSharedRef<FMixerAudioBus> FMixerSourceManager::FindOrAddAudioBus(const FAudioBusKey BusKey, const int32 InNumChannels, const bool bIsAutomatic, bool* bOutAlreadyExisted)
	{
		TSharedPtr<FMixerAudioBus> AudioBusPtr = AudioBuses.FindRef(BusKey);
		if (bOutAlreadyExisted != nullptr)
		{
			*bOutAlreadyExisted = AudioBusPtr.IsValid();
		}
		if (!AudioBusPtr.IsValid())
		{
			AudioBusPtr = MakeShared<FMixerAudioBus>(this, bIsAutomatic, InNumChannels, BusKey);
			AudioBuses.Add(BusKey, AudioBusPtr);
		}

		return AudioBusPtr.ToSharedRef();
	}
	...
	void FMixerSourceManager::FSourceInfo::DoRenderStep()
	{
		const int32 SourceId = this - SourceManager->SourceInfos.GetData();
		SourceManager->RenderSource(SourceId);
	}

	const TCHAR* FMixerSourceManager::FSourceInfo::GetRenderStepName()
	{
#if AUDIO_MIXER_ENABLE_DEBUG_MODE
		return *DebugName;
#else
		return TEXT("Render mixer source");
#endif
	}
}
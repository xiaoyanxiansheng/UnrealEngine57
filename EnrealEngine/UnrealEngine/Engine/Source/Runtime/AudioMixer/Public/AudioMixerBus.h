...
namespace Audio
{
	...
	class FMixerAudioBus : public IAudioMixerRenderStep
	{
	public:
		...
		// InBusKey         The unique key used to identify this bus.
	...
	private:
		...
		// IAudioMixerRenderStep interface
		virtual void DoRenderStep() override;
		virtual const TCHAR* GetRenderStepName() override;

#if UE_AUDIO_PROFILERTRACE_ENABLED
		void StartEnvelopeFollower(const float InAttackTime, const float InReleaseTime, const float InSampleRate);

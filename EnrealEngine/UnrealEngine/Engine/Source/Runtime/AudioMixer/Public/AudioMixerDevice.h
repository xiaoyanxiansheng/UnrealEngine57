...
namespace Audio
{
	...
	public:
		...
		inline int32 GetNumDeviceChannels() const { return PlatformInfo.NumChannels; }
		inline int32 GetNumDirectOutChannels() const { return PlatformInfo.NumDirectOutChannels; }
	...
	private:
		...
		void SendSubmixesAlivePing();
		void UpdateSubmixesAlivePingTime();

		void RegisterSubmixEnvelopeFollowerCVars();
		void UnregisterSubmixEnvelopeFollowerCVars();

		void OnStartSubmixEnvelopeFollowerCVarChanged(IConsoleVariable* InStartSubmixEnvelopeFollowerCVar);
		void OnStopSubmixEnvelopeFollowerCVarChanged(IConsoleVariable* InStopSubmixEnvelopeFollowerCVar);

		void StartSubmixEnvelopeFollowerFromCVar(const FString& InSubmixName);
		void StopSubmixEnvelopeFollowerFromCVar(const FString& InSubmixName);
		...
		/** Scheduler responsible for launching rendering tasks */
		TUniquePtr<FAudioRenderScheduler> RenderScheduler;

		/** The mixer source manager. Should be cleaned up before the scheduler so dependencies can be removed correctly. */
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
		IConsoleVariable* StartSubmixEnvelopeFollowerCVar = nullptr;
		IConsoleVariable* StopSubmixEnvelopeFollowerCVar  = nullptr;

		double SubmixesAlivePingTimeSec = 0.0;
		static constexpr double SubmixesAlivePingWaitTimeSec = 1.0;
#endif // UE_AUDIO_PROFILERTRACE_ENABLED

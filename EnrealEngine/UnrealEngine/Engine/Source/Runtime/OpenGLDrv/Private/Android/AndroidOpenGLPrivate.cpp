...
void FAndroidGPUInfo::ReadGPUFamily()
{
	...
	// If we're ANGLE, get the raw GPU string from Vulkan.
	if (GPUFamily.StartsWith(TEXT("ANGLE")) && FAndroidMisc::HasVulkanDriverSupport())
	{
		UE_LOG(LogAndroid, Log, TEXT("FAndroidGPUInfo ANGLE detected with GL_RENDERER string %s"), *GPUFamily);
		GPUFamily = FAndroidMisc::GetVulkanDeviceNameString();
		UE_LOG(LogAndroid, Log, TEXT("FAndroidGPUInfo Using Vulkan deviceName string %s"), *GPUFamily);
	}

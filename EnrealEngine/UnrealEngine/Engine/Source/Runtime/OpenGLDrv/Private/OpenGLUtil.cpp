...
void SetOpenGLResourceName(FOpenGLTexture* Texture, const ANSICHAR* Name)
{
	...
	{
		Texture->EvictionParamsPtr->SetDebugLabelName(Name);
	}
	else
	{
		FOpenGL::LabelObject(GL_TEXTURE, Texture->GetResource(), Name);
	}
#endif
}
...
{
#if GLDEBUG_LABELS_ENABLED
	const FAnsiString AnsiName(Name);
	SetOpenGLResourceName(Texture, *AnsiName);
#endif
}

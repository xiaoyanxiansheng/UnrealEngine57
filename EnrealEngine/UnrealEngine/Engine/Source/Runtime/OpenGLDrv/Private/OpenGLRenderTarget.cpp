...
public:
		...
		,	NumRenderingSamples(InNumRenderingSamples)
	{
		...
		for( RenderTargetIndex = 0; RenderTargetIndex < InNumRenderTargets; ++RenderTargetIndex )
		{
			...
			RenderTargets[RenderTargetIndex].Texture = InRenderTargets ? InRenderTargets[RenderTargetIndex] : nullptr;
			RenderTargets[RenderTargetIndex].Resource = (InRenderTargets && InRenderTargets[RenderTargetIndex]) ? InRenderTargets[RenderTargetIndex]->GetResource() : 0;
...
void FOpenGLDynamicRHI::RHIEndRenderPass()
{
	...
	{
		DiscardRenderTargets(bDiscardDepthStencil, bDiscardDepthStencil, ColorMask);
	}

	...
	for( int32 TextureStageIndex = 0; TextureStageIndex <= MaxProgramTexture; ++TextureStageIndex )
	{
		...
		else
		{
			...
			if (!bExternalTexture)
			{
				...
				if (TextureStage.Texture && TextureStage.Texture->bInStencilTextureMode)
				{
					//set the texture to return the depth component, and then force the components to match D3D
					glTexParameteri(TextureStage.Target, GL_DEPTH_STENCIL_TEXTURE_MODE, GL_DEPTH_COMPONENT);
					glTexParameteri(TextureStage.Target, GL_TEXTURE_SWIZZLE_R, GL_RED);
					glTexParameteri(TextureStage.Target, GL_TEXTURE_SWIZZLE_G, GL_ZERO);
					glTexParameteri(TextureStage.Target, GL_TEXTURE_SWIZZLE_B, GL_ZERO);
					glTexParameteri(TextureStage.Target, GL_TEXTURE_SWIZZLE_A, GL_ZERO);

					TextureStage.Texture->bInStencilTextureMode = false;
				}
			}
...
void FOpenGLDynamicRHI::DiscardRenderTargets(bool Depth, bool Stencil, uint32 ColorBitMaskIn)
{

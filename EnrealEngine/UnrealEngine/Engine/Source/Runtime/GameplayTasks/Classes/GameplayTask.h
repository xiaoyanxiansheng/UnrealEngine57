...
public:
	...
	template <class T>
	inline static T* NewTask(TScriptInterface<IGameplayTaskOwnerInterface> TaskOwner, FName InstanceName = FName())
	{
	...
	template <class T>
	inline static T* NewTaskUninitialized()
	{
		return NewObject<T>();
	}
	...
	inline FName GetInstanceName() const { return InstanceName; }
	inline bool IsTickingTask() const { return (bTickingTask != 0); }
	inline bool IsSimulatedTask() const { return (bSimulatedTask != 0); }
	inline bool IsSimulating() const { return (bIsSimulating != 0); }
	inline bool IsPausable() const { return (bIsPausable != 0); }
	inline bool HasOwnerFinished() const {	return (bOwnerFinished != 0); }
	inline uint8 GetPriority() const { return Priority; }
	inline bool RequiresPriorityOrResourceManagement() const { return bCaresAboutPriority == true || RequiredResources.IsEmpty() == false || ClaimedResources.IsEmpty() == false; }
	inline FGameplayResourceSet GetRequiredResources() const { return RequiredResources; }
	inline FGameplayResourceSet GetClaimedResources() const { return ClaimedResources; }
	
	inline EGameplayTaskState GetState() const { return TaskState; }
	inline bool IsActive() const { return (TaskState == EGameplayTaskState::Active); }
	inline bool IsPaused() const { return (TaskState == EGameplayTaskState::Paused); }
	inline bool IsFinished() const { return (TaskState == EGameplayTaskState::Finished); }

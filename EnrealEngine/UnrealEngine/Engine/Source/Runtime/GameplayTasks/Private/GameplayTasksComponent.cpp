...
void UGameplayTasksComponent::OnRep_SimulatedTasks()
{
	if (IsUsingRegisteredSubObjectList())
	{
		...
		for (TWeakObjectPtr<UGameplayTask> OldSimulatedTaskPtr : PreviousOnRepSimulatedTasks)
		{
			UGameplayTask* OldSimulatedTask = OldSimulatedTaskPtr.Get();
			if (OldSimulatedTask)
			{
	...
	for (UGameplayTask* SimulatedTask : GetSimulatedTasks())
	{
		if (SimulatedTask)
		{
			...
			if (IsUsingRegisteredSubObjectList())
			{
				const bool bIsNew = PreviousOnRepSimulatedTasks.Find(SimulatedTask) == INDEX_NONE;
				if (bIsNew)
				{
					AddReplicatedSubObject(SimulatedTask, COND_SkipOwner);
				}

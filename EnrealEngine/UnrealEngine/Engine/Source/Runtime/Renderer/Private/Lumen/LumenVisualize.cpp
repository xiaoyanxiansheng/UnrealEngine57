	...
	EReflectionsMethod ReflectionsMethod)
{
	...
	FLumenVisualizeSceneSoftwareRayTracingParameters VisualizeParameters;
	SetupVisualizeParameters(GraphBuilder, View, Output.ViewRect, ReflectionsMethod, ColorGradingTexture, EyeAdaptationBuffer, VisualizeMode, VisualizeTileIndex, VisualizeParameters);
...
FScreenPassTexture AddVisualizeLumenScenePass(FRDGBuilder& GraphBuilder, const FViewInfo& View, EDiffuseIndirectMethod DiffuseIndirectMethod, EReflectionsMethod ReflectionsMethod, const FVisualizeLumenSceneInputs& Inputs, FLumenSceneFrameTemporaries& FrameTemporaries)
{
			...
			if (VisualizeMode == VISUALIZE_MODE_OVERVIEW)
			{
				...
				for (int32 TileIndex = 0; TileIndex < LumenVisualize::NumOverviewTilesPerRow; ++TileIndex)
				{
					VisualizeLumenScene(
						...
						FrameTemporaries,
						Output,
						Inputs.ColorGradingTexture,
						Inputs.EyeAdaptationBuffer,
						Inputs.SceneTextures,
						VisualizeTiles[TileIndex].Mode,
						TileIndex,
						DiffuseIndirectMethod,
						ReflectionsMethod);
				}
	...
	if (FrameTemporaries.DebugData != nullptr)
	{
		...
		FLumenVisualizeSceneSoftwareRayTracingParameters VisualizeParameters;
		SetupVisualizeParameters(GraphBuilder, View, Output.ViewRect, ReflectionsMethod, Inputs.ColorGradingTexture, Inputs.EyeAdaptationBuffer, 0, 0, VisualizeParameters);

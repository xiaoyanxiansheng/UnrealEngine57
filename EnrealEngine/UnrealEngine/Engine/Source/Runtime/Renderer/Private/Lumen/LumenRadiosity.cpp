...
TAutoConsoleVariable<int32> CVarLumenSceneRadiosityVisualizeProbesShowInvalid(
	TEXT("r.LumenScene.Radiosity.VisualizeProbes.ShowInvalid"),
	0,
...
BEGIN_SHADER_PARAMETER_STRUCT(FLumenRadiosityTexelTraceParameters, )
	...
	SHADER_PARAMETER_RDG_BUFFER_SRV(StructuredBuffer<uint>, RadiosityTileData)
	...
	SHADER_PARAMETER(uint32, ProbeSpacingInCardTexels)
	SHADER_PARAMETER(uint32, ProbeSpacingInCardTexelsDivideShift)
	SHADER_PARAMETER(uint32, CardTileSizeInProbes)
	SHADER_PARAMETER(uint32, RadiosityTileSizeInProbes)
	...
	SHADER_PARAMETER(uint32, MaxRadiosityTiles)
...
class FBuildRadiosityTilesCS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_RDG_BUFFER_UAV(RWStructuredBuffer<uint>, RWRadiosityTileAllocator)
		SHADER_PARAMETER_RDG_BUFFER_UAV(RWStructuredBuffer<uint>, RWRadiosityTileData)
...
class FLumenRadiosityIndirectArgsCS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_RDG_BUFFER_UAV(RWBuffer<uint>, RWDispatchRadiosityTilesIndirectArgs)
		SHADER_PARAMETER_STRUCT_INCLUDE(FLumenRadiosityTexelTraceParameters, RadiosityTexelTraceParameters)
	...
	ERDGPassFlags ComputePassFlags)
{
	...
	// A radiosity tile is a sub tile containing a single probe when spacing is >=4 but falls back to a whole card tile otherwise.
	// There are a lot of probes in a card tile when spacing is small. Generating a tile per probe seems excessive
	const uint32 RadiosityTileSize = RadiosityFrameTemporaries.ProbeSpacing >= 4 ? RadiosityFrameTemporaries.ProbeSpacing : Lumen::CardTileSize;
	const uint32 MaxRadiosityTiles = MaxCardTiles * FMath::Square(Lumen::CardTileSize / RadiosityTileSize);
	...
	FRDGBufferRef CardTiles = GraphBuilder.CreateBuffer(FRDGBufferDesc::CreateStructuredDesc(sizeof(uint32), MaxCardTiles), TEXT("Lumen.CardTiles"));
	AddClearUAVPass(GraphBuilder, GraphBuilder.CreateUAV(CardTileAllocator), 0, ComputePassFlags);

	FRDGBufferRef RadiosityTileAllocator = GraphBuilder.CreateBuffer(FRDGBufferDesc::CreateStructuredDesc(sizeof(uint32), NumViewOrigins), TEXT("Lumen.RadiosityTileAllocator"));
	FRDGBufferRef RadiosityTiles = GraphBuilder.CreateBuffer(FRDGBufferDesc::CreateStructuredDesc(sizeof(uint32), MaxRadiosityTiles * NumViewOrigins), TEXT("Lumen.RadiosityTiles"));
	AddClearUAVPass(GraphBuilder, GraphBuilder.CreateUAV(RadiosityTileAllocator), 0, ComputePassFlags);
	...
	FRDGBufferRef RadiosityIndirectArgs  = GraphBuilder.CreateBuffer(
		FRDGBufferDesc::CreateIndirectDesc<FRHIDispatchIndirectParameters>((uint32)ERadiosityIndirectArgs::MAX * NumViewOrigins),
		TEXT("Lumen.RadiosityIndirectArgs"));

	CardTileUpdateContext.CardTileAllocator = CardTileAllocator;
	CardTileUpdateContext.CardTiles = CardTiles;
	CardTileUpdateContext.DispatchCardTilesIndirectArgs = DispatchCardTilesIndirectArgs;
	...
	const uint32 CardTileSizeInProbes = Lumen::CardTileSize / RadiosityFrameTemporaries.ProbeSpacing;
	const uint32 RadiosityTileSizeInProbes = RadiosityTileSize / RadiosityFrameTemporaries.ProbeSpacing;
	...
	FLumenRadiosityTexelTraceParameters RadiosityTexelTraceParameters;
	{
		...
		RadiosityTexelTraceParameters.RadiosityTileAllocator = GraphBuilder.CreateSRV(RadiosityTileAllocator);
		RadiosityTexelTraceParameters.RadiosityTileData = GraphBuilder.CreateSRV(RadiosityTiles);
		...
		RadiosityTexelTraceParameters.ProbeSpacingInCardTexels = RadiosityFrameTemporaries.ProbeSpacing;
		RadiosityTexelTraceParameters.ProbeSpacingInCardTexelsDivideShift = FMath::FloorLog2(RadiosityFrameTemporaries.ProbeSpacing);
		RadiosityTexelTraceParameters.CardTileSizeInProbes = CardTileSizeInProbes;
		RadiosityTexelTraceParameters.RadiosityTileSizeInProbes = RadiosityTileSizeInProbes;
		...
		RadiosityTexelTraceParameters.MaxRadiosityTiles = MaxRadiosityTiles;
	...
	// Build a list of radiosity tiles for future processing
	{
		...
		PassParameters->RWRadiosityTileAllocator = GraphBuilder.CreateUAV(RadiosityTileAllocator);
		PassParameters->RWRadiosityTileData = GraphBuilder.CreateUAV(RadiosityTiles);
	...
	// Setup indirect args for future passes
	{
		...
		PassParameters->RWDispatchRadiosityTilesIndirectArgs = GraphBuilder.CreateUAV(RadiosityIndirectArgs);
		PassParameters->RadiosityTexelTraceParameters = RadiosityTexelTraceParameters;
#if RHI_RAYTRACING
	...
	if (Lumen::UseHardwareRayTracedRadiosity(*FirstView.Family))
	{
		...
		for (int32 OriginIndex = 0; OriginIndex < NumViewOrigins; ++OriginIndex)
		{
			...
			SetLumenHardwareRayTracingSharedParameters(
				GraphBuilder,
				GetSceneTextureParameters(GraphBuilder, *ViewOrigin.ReferenceView),
				*ViewOrigin.ReferenceView,
				TracingParameters,
			...
			PassParameters->MaxTraceDistance = ViewOrigin.MaxTraceDistance;
			...
			if (bInlineRayTracing)
			{
				FLumenRadiosityHardwareRayTracingCS::AddLumenRayTracingDispatchIndirect(
					...
					*ViewOrigin.ReferenceView,
					...
					(uint32)ERadiosityIndirectArgs::HardwareRayTracingThreadPerTrace + OriginIndex * (uint32)ERadiosityIndirectArgs::MAX * sizeof(FRHIDispatchIndirectParameters),
					ComputePassFlags);
			}
			else
			{
...

	{
		...
		PassParameters->IndirectArgs = DispatchCardTilesIndirectArgs;
		PassParameters->RadiosityCommonParameters = RadiosityCommonParameters;
		PassParameters->LumenCardScene = FrameTemporaries.LumenCardSceneUniformBuffer;
		PassParameters->RadiosityTexelTraceParameters = RadiosityTexelTraceParameters;
...
class FVisualizeRadiosityProbesVS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(uint32, ProbeTileSize)
		SHADER_PARAMETER(uint32, MaxVisualizeProbes)
		SHADER_PARAMETER(int32, bShowInvalidProbes)
	END_SHADER_PARAMETER_STRUCT()
...
void FDeferredShadingSceneRenderer::RenderLumenRadiosityProbeVisualization(FRDGBuilder& GraphBuilder, const FMinimalSceneTextures& SceneTextures, const FLumenSceneFrameTemporaries& FrameTemporaries)
{
		...
		&& CVarLumenSceneRadiosityVisualizeProbes.GetValueOnRenderThread() != 0)
	{
		...
		// Gather and prepare probes for visualization
		{
			...
			PassParameters->MaxVisualizeProbes = MaxVisualizeProbes;
			PassParameters->ProbeSpacingInCardTexels = ProbeSpacing;
			PassParameters->FixedJitterIndex = GLumenRadiosityFixedJitterIndex;
			PassParameters->MaxFramesAccumulated = LumenRadiosity::UseTemporalAccumulation() ? GLumenRadiosityTemporalMaxFramesAccumulated : 1;
		...
		PassParameters->VS.bShowInvalidProbes = CVarLumenSceneRadiosityVisualizeProbesShowInvalid.GetValueOnRenderThread() != 0;

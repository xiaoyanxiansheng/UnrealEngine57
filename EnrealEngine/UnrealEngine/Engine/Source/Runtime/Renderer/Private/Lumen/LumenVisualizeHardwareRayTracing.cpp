...
#if RHI_RAYTRACING
{
	...
	// Generate rays
	const uint32 RayCount = MaxTileCount * FLumenVisualizeCreateRaysCS::GetThreadGroupSize1D();
	...
	FRDGBufferRef TraceDataPackedBuffer = GraphBuilder.CreateBuffer(FRDGBufferDesc::CreateStructuredDesc(sizeof(LumenVisualize::FTraceDataPacked), RayCount), TEXT("Lumen.Visualize.TraceDataPacked"));
	{
		...
		FIntPoint DispatchResolution = FIntPoint(FLumenVisualizeHardwareRayTracing::GetGroupSize(), FMath::DivideAndRoundUp(RayCount, FLumenVisualizeHardwareRayTracing::GetGroupSize()));

...
FAutoConsoleVariableRef CVarLumenReflectionsDistantScreenTraces(
	...
	TEXT("0 - Off\n")
	TEXT("1 - Enable when not using far field (default)\n")
	TEXT("2 - Enable"),
	ECVF_Scalability | ECVF_RenderThreadSafe
);
...
{
	return !bUseRadianceCache
		&& (GLumenReflectionsDistantScreenTraces == 2 || (GLumenReflectionsDistantScreenTraces != 0 && !bUseFarField))
		&& UseScreenTraces(View)
...
class FReflectionTraceVoxelsCS : public FGlobalShader
{
	...
	static EShaderPermutationPrecacheRequest ShouldPrecachePermutation(const FGlobalShaderPermutationParameters& Parameters)
	{
		...
		const bool bDistantScreenTraces = GLumenReflectionsDistantScreenTraces != 0 && bScreenTraces;
	...
	ERDGPassFlags ComputePassFlags)
{
	...
	{
		const bool bDistantScreenTraces = LumenReflections::UseDistantScreenTraces(View, false /*bUseFarField*/, bUseRadianceCache);

			...
			if (bAnyInstanceValid)
			{
				...
				else
				{
					...
					else
					{
						...
						PrimitiveGroup.bValidMeshCards = true;
						PrimitiveGroup.bFarField = SceneProxy->IsRayTracingFarField();
						PrimitiveGroup.bHeightfield = SceneProxy->SupportsHeightfieldRepresentation() || SceneProxy->IsLumenHeightfield();
						PrimitiveGroup.LightingChannelMask = SceneProxy->GetLightingChannelMask();
						PrimitiveGroup.bEmissiveLightSource = SceneProxy->IsEmissiveLightSource();
						PrimitiveGroup.bOpaqueOrMasked = SceneProxy->IsOpaqueOrMasked();
						PrimitiveGroup.bLandscape = SceneProxy->IsLandscapeProxy();
						PrimitiveGroup.Primitives.Reset();
						PrimitiveGroup.Primitives.Add(ScenePrimitiveInfo);
...
void FLumenSceneData::DumpStats(const FDistanceFieldSceneData& DistanceFieldSceneData, bool bDumpMeshDistanceFields, bool bDumpPrimitiveCullingInfos, bool bDumpPrimitiveGroups)
{
	...
	{
#if STATS
		UE_LOG(LogRenderer, Log, TEXT("*** LumenScene PrimitiveCullingInfos ***"));
		...
		for (const FLumenPrimitiveGroupCullingInfo& CullingInfo : PrimitiveCullingInfos)
		{
			if (CullingInfo.PrimitiveGroupIndex > 0)
			{
				if (PrimitiveGroups.IsAllocated(CullingInfo.PrimitiveGroupIndex))
				{
					...
					if (PrimitiveGroup.Primitives.Num() > 0)
					{
						const FPrimitiveSceneInfo* ScenePrimitive = PrimitiveGroup.Primitives[0];
						if (ScenePrimitive && ScenePrimitive->Proxy)
						{
							UE_LOG(LogRenderer, Log, TEXT("NumInstances:%d FarField:%d Heightfield:%d %s"),

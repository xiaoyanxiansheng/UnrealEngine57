...
void FDeferredShadingSceneRenderer::FinishGatheringLumenSurfaceCacheFeedback(FRDGBuilder& GraphBuilder, const FViewInfo& View, FLumenSceneFrameTemporaries& FrameTemporaries, const FFrontLayerTranslucencyData& FrontLayerTranslucencyData, const FMinimalSceneTextures& SceneTextures)
{
	const FPerViewPipelineState& ViewPipelineState = GetViewPipelineState(View);
	const bool bLumenActive = ViewPipelineState.DiffuseIndirectMethod == EDiffuseIndirectMethod::Lumen || ViewPipelineState.ReflectionsMethod == EReflectionsMethod::Lumen;
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ++ViewIndex)
	{
		...
		if (ViewInfo.ViewState && !ViewInfo.bStatePrevViewInfoIsReadOnly)
		{
			if (FrontLayerTranslucencyData.SceneDepth)
			{
				GraphBuilder.QueueTextureExtraction(FrontLayerTranslucencyData.SceneDepth, &ViewInfo.ViewState->Lumen.TranslucentReflectionState.LayerSceneDepthHistory);
			}

			if (FrontLayerTranslucencyData.Normal)
			{
				GraphBuilder.QueueTextureExtraction(FrontLayerTranslucencyData.Normal, &ViewInfo.ViewState->Lumen.TranslucentReflectionState.LayerSceneNormalHistory);
			}
		}
	}
}
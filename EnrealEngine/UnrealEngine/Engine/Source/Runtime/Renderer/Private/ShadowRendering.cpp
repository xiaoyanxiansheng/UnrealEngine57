...
bool FProjectedShadowInfo::UseShadowStencilCulling(const FViewInfo& View) const
{
	...
	if (MegaLights::IsEnabled(*View.Family))
	{
		...
		FSceneRenderer::GetLightNameForDrawEvent(GetLightSceneInfo().Proxy, LightNameWithLevel);

		GEngine->AddOnScreenDebugMessage(uint64(GetLightSceneInfo().Proxy), 1.f, FColor::Yellow,
			*FString::Printf(TEXT("r.Shadow.StencilCulling is not compatible with MegaLights. Modify '%s' to use MegaLights, enable VSM or disable r.Shadow.StencilCulling."),
				*LightNameWithLevel));

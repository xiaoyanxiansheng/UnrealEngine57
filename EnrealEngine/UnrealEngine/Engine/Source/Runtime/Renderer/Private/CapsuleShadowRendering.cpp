	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		...
		if (CapsuleShapeData.Num() > 0)
		{
...

			{
				...
				{
					PassParameters->RenderTargets[0] = FRenderTargetBinding(ScreenShadowMaskTexture, GetLoadActionIfProduced(ScreenShadowMaskTexture, ERenderTargetLoadAction::EClear), /*MipIndex*/ 0, /*ArraySlice*/ ViewIndex);
				}

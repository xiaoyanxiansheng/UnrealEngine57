...
class FRescaleVariableRateShadingCS : public FGlobalShader
{
		...
		FRDGTextureUAVRef ScaledConservateSRIUAV)
	{
		...
		Parameters.View = ViewInfo.GetShaderParameters();
	...
	bool bCreateHardwareImages, bool bCreateSoftwareImages)
{
	...
	for (int Index = 0; Index < ViewFamily.Views.Num(); Index++)
	{
...

		{
			...
			const FTonemapperOutputDeviceParameters OutputDeviceParameters = GetTonemapperOutputDeviceParameters(*ViewInfo.Family);
			const EDisplayOutputFormat OutputFormat = (EDisplayOutputFormat) OutputDeviceParameters.OutputDevice;
			const bool bIsHDR10 = OutputFormat == EDisplayOutputFormat::HDR_ACES_1000nit_ST2084 
...
{
	// See also the requirments under FVariableRateShadingImageManager::IsVRSCompatibleWithView
	return !IsMobilePlatform(View.GetShaderPlatform());
}

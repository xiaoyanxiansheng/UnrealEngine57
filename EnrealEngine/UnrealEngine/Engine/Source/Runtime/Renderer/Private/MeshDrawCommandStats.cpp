...
DECLARE_DWORD_COUNTER_STAT(TEXT("Total Rendered Vertices"), STAT_Culling_TotalNumVertices, STATGROUP_Culling);
...
DECLARE_DWORD_COUNTER_STAT(TEXT("InstanceCulling Indirect Rendered Vertices"), STAT_Culling_InstanceCullingIndirectNumVertices, STATGROUP_Culling);
...
DECLARE_DWORD_COUNTER_STAT(TEXT("Custom Indirect Rendered Vertices"), STAT_Culling_CustomIndirectNumVertices, STATGROUP_Culling);
...
static TAutoConsoleVariable<int32> CVarMeshDrawCommandUseVertexStats(
	TEXT("r.MeshDrawCommands.UseVertexCount"),
	0,
	TEXT("Use vertex count instead of primitive count for budget reporting."),
	ECVF_RenderThreadSafe
);
...
FMeshDrawCommandStatsManager::FMeshDrawCommandStatsManager()
{
	...
	ScreenMessageDelegate = FRendererOnScreenNotification::Get().AddLambda([this](TMultiMap<FCoreDelegates::EOnScreenMessageSeverity, FText >& OutMessages)
		{	
			...
			auto NumberToK = [](uint64 InV) -> uint64 { return InV / 1000; };

			int64 TotalTracked = 0;
			
			const bool bShowStats = CVarMeshDrawCommandStats->GetInt() != (int)MeshDrawStatsCollection::None;
			if (bShowStats)
			{
				const bool bUseVertexCount = CVarMeshDrawCommandUseVertexStats->GetInt() > 0;
				if (bUseVertexCount)
				{
					OutMessages.Add(FCoreDelegates::EOnScreenMessageSeverity::Info, FText::FromString(FString::Printf(TEXT("MeshDrawCommandStats (Vertices / Budget - Category)"))));
				}
				else
				{
				...
				if (StatCollection)
				{
					...
					{
						FDrawData* DrawData = BudgetedDrawData.Find(Category.Name);
						if (DrawData && DrawData->PrimitiveCount > 0)
						{
							uint64 Count = bUseVertexCount ? DrawData->VertexCount : DrawData->PrimitiveCount;
							if (Count > 0)
							{
								uint64 Budget = bUseVertexCount ? Category.DrawBudgets.VertexCount : Category.DrawBudgets.PrimitiveCount;
								FCoreDelegates::EOnScreenMessageSeverity Severity = Budget < Count ? FCoreDelegates::EOnScreenMessageSeverity::Warning : FCoreDelegates::EOnScreenMessageSeverity::Info;
					...
					if (TotalTracked > 0)
					{
						int32 CollectionBudget = StatCollection ? (bUseVertexCount ? StatCollection->DrawBudgets.VertexCount : StatCollection->DrawBudgets.PrimitiveCount) : INT32_MAX;

						FCoreDelegates::EOnScreenMessageSeverity Severity = TotalTracked < CollectionBudget ? FCoreDelegates::EOnScreenMessageSeverity::Info : FCoreDelegates::EOnScreenMessageSeverity::Warning;
						OutMessages.Add(Severity, FText::FromString(FString::Printf(TEXT("%5dK - Total Tracked"), NumberToK(TotalTracked))));
					}
				}

				// Categories which are not tracked by specific budgets
				int32 TotalUntracked = 0;
				...
				for (const TPair<FName, FDrawData>& Pair : UntrackedDrawData)
				{
					const FName& Name = Pair.Key;
					uint64 Count = bUseVertexCount ? Pair.Value.VertexCount : Pair.Value.PrimitiveCount;
					if (Count > 0)
					{
						OutMessages.Add(FCoreDelegates::EOnScreenMessageSeverity::Warning, FText::FromString(FString::Printf(TEXT("\t%5dK - %s%s"), NumberToK(Count), *(Name.ToString()), *PassFriendlyNames)));
						TotalUntracked += Count;
					}					
				}

				if (TotalUntracked > 0 && !bShowPassNameStats)
				{
					OutMessages.Add(FCoreDelegates::EOnScreenMessageSeverity::Warning, FText::FromString(FString::Printf(TEXT("%5dK - Total Untracked"), NumberToK(TotalUntracked))));
				}

				int32 TotalBudget = StatCollection ? (bUseVertexCount ? StatCollection->DrawBudgets.VertexCount : StatCollection->DrawBudgets.PrimitiveCount) : 0;
				int32 TotalCount = bUseVertexCount ? Stats.TotalDrawData.VertexCount : Stats.TotalDrawData.PrimitiveCount;
				if (TotalBudget)
				{
					FCoreDelegates::EOnScreenMessageSeverity Severity = TotalBudget < TotalCount ? FCoreDelegates::EOnScreenMessageSeverity::Warning : FCoreDelegates::EOnScreenMessageSeverity::Info;
					OutMessages.Add(Severity, FText::FromString(FString::Printf(TEXT("%5dK / %5dK - Total"), NumberToK(TotalTracked + TotalUntracked), NumberToK(TotalBudget))));
				}
				else
				{
					OutMessages.Add(FCoreDelegates::EOnScreenMessageSeverity::Info, FText::FromString(FString::Printf(TEXT("\t%5dK - Total"), NumberToK(TotalCount))));
				}
			}
		});
}
...
void FMeshDrawCommandStatsManager::Update()
{
	...
	for (int32 Index = Frames.Num() - 1; Index >= 0; --Index)
	{
		...
		if (FrameData->IsCompleted())
		{
			if (!bHasProcessedFrame)
			{
				...
				using PassCategoryStats = TMap<FName, FDrawData>;
				...
				for (FMeshDrawCommandPassStats* PassStats : FrameData->PassData)
				{
					...
					for (int32 CmdIndex = 0; CmdIndex < PassStats->DrawData.Num(); ++CmdIndex)
					{
						...
						check(DrawData.StatsData.NumVertices > 0);
						...
						if (DrawData.CustomIndirectArgsBuffer)
						{
							...
							if (ensure(IndirectArgsBufferResult))
							{
								...
								Stats.CustomIndirectDrawData.VertexCount += DrawData.VisibleInstanceCount * DrawData.StatsData.NumVertices;
								Stats.CustomIndirectDrawData.PrimitiveCount += DrawData.VisibleInstanceCount * DrawData.PrimitiveCount;
							}
						}
						else if (IndirectArgsPtr && DrawData.UseInstantCullingIndirectBuffer > 0 && InstanceCullingReadBackData)
						{
							ensure(DrawData.NumBatches > 0);
							DrawData.VisibleInstanceCount = 0;
							for (int32 BatchIndex = 0; BatchIndex < DrawData.NumBatches; BatchIndex++)
							{
								const FRHIDrawIndexedIndirectParameters& IndirectArgs = IndirectArgsPtr[PassStats->IndirectArgParameterOffset + IndirectCommandIndex + BatchIndex];
							...
							Stats.InstanceCullingIndirectDrawData.VertexCount += DrawData.VisibleInstanceCount * DrawData.StatsData.NumVertices;
							Stats.InstanceCullingIndirectDrawData.PrimitiveCount += DrawData.VisibleInstanceCount * DrawData.PrimitiveCount;
						}

						Stats.TotalInstances += DrawData.VisibleInstanceCount;
						Stats.TotalDrawData.VertexCount += DrawData.VisibleInstanceCount * DrawData.StatsData.NumVertices;
						Stats.TotalDrawData.PrimitiveCount += DrawData.VisibleInstanceCount * DrawData.PrimitiveCount;

						FName StatName = bShowPassNameStats ? PassStats->PassName : DrawData.StatsData.CategoryName;
						FDrawData& CategoryDrawData = CategoryStats.FindOrAdd(StatName);
						CategoryDrawData.PrimitiveCount += DrawData.VisibleInstanceCount * DrawData.PrimitiveCount;
						CategoryDrawData.VertexCount += DrawData.VisibleInstanceCount * DrawData.StatsData.NumVertices;
					}
	...
	SET_DWORD_STAT(STAT_Culling_TotalNumPrimitives, Stats.TotalDrawData.PrimitiveCount);
	SET_DWORD_STAT(STAT_Culling_TotalNumVertices, Stats.TotalDrawData.VertexCount);
	SET_DWORD_STAT(STAT_Culling_TotalNumInstances, Stats.TotalInstances);
	SET_DWORD_STAT(STAT_Culling_InstanceCullingIndirectNumPrimitives, Stats.InstanceCullingIndirectDrawData.PrimitiveCount);
	SET_DWORD_STAT(STAT_Culling_InstanceCullingIndirectNumVertices, Stats.InstanceCullingIndirectDrawData.VertexCount);
	SET_DWORD_STAT(STAT_Culling_InstanceCullingIndirectNumInstances, Stats.InstanceCullingIndirectInstances);
	SET_DWORD_STAT(STAT_Culling_CustomIndirectNumPrimitives, Stats.CustomIndirectDrawData.PrimitiveCount);
	SET_DWORD_STAT(STAT_Culling_CustomIndirectNumVertices, Stats.CustomIndirectDrawData.VertexCount);
	...
	if (bCollectStats)
	{
		...
		if (!StatCollections.Num())
		{
			...
			for (const FMeshDrawCommandStatsBudget& CategoryBudget : Settings->Budgets)
			{
				...
				Category->DrawBudgets.PrimitiveCount = CategoryBudget.PrimitiveBudget;
				Category->DrawBudgets.VertexCount = CategoryBudget.VertexBudget;
			...
			for (const FMeshDrawCommandStatsBudgetTotals& BudgetTotal : Settings->BudgetTotals)
			{
				...
				if (Collection)
				{
					Collection->DrawBudgets.PrimitiveCount = BudgetTotal.PrimitiveBudget;
					Collection->DrawBudgets.VertexCount = BudgetTotal.VertexBudget;
				}
			}
		...
		BudgetedDrawData.Reset();
		UntrackedDrawData.Reset();
			...
			{ 
				...
				if (CategoryIndices)
				{
					...
					{ 
						...
						{
							FDrawData& DrawData = BudgetedDrawData.FindOrAdd(Category.Name);
							DrawData.PrimitiveCount += CategoryStat.DrawData.PrimitiveCount;
							DrawData.VertexCount += CategoryStat.DrawData.VertexCount;
						}
					}
				}
				...
				{
					FDrawData& DrawData = UntrackedDrawData.FindOrAdd(CategoryStat.CategoryName);
					DrawData.PrimitiveCount += CategoryStat.DrawData.PrimitiveCount;
					DrawData.VertexCount += CategoryStat.DrawData.VertexCount;
				}
			}
	...
	if (bCsvExport)
	{
		...
		for (const TPair<FName, FDrawData>& Pair : BudgetedDrawData)
		{
			TRACE_CSV_PROFILER_INLINE_STAT(TCHAR_TO_ANSI(*Pair.Key.ToString()), CSV_CATEGORY_INDEX(MeshDrawCommandStats));
			FCsvProfiler::RecordCustomStat(Pair.Key, CSV_CATEGORY_INDEX(MeshDrawCommandStats), IntCastChecked<int32>(Pair.Value.PrimitiveCount), ECsvCustomStatOp::Set);
		}
		...
		for (const TPair<FName, FDrawData>& Pair : UntrackedDrawData)
		{
			TotalUntracked += Pair.Value.PrimitiveCount;
		}
...
void FMeshDrawCommandStatsManager::DumpStats(FFrameData* FrameData)
{
	...
	for (FMeshDrawCommandPassStats* PassStats : FrameData->PassData)
	{
		for (FVisibleMeshDrawCommandStatsData& DrawData : PassStats->DrawData)
		{
			if (DrawData.VisibleInstanceCount > 0)
			{
				...
				StatEntry.VisibileVertexCount = DrawData.VisibleInstanceCount * DrawData.StatsData.NumVertices;
				StatEntry.VisibleInstance = DrawData.VisibleInstanceCount;
				StatEntry.PrimitiveCount = DrawData.PrimitiveCount;
				StatEntry.VertexCount = DrawData.StatsData.NumVertices;
				...
				StatEntry.TotalVertexCount = DrawData.TotalInstanceCount * DrawData.StatsData.NumVertices;
	...
	const TCHAR* Header = TEXT("Pass,VisiblePrimitiveCount,VisibleVertices,VisibleInstances,Category,ResourceName,LODIndex,SegmentIndex,MaterialName,PrimitiveCount,VertexCount,TotalInstanceCount,TotalPrimitiveCount,TotalVertexCount\n");
	...
	for (FStatEntry& StatEntry : StatEntries)
	{
		...
		FString Row = FString::Printf(TEXT("%s,%d,%d,%d,%s,%s,%d,%d,%s,%d,%d,%d,%d,%d\n"),
			PassNameBuffer,
			StatEntry.VisibilePrimitiveCount,
			StatEntry.VisibileVertexCount,

	  ...
	  DependentView(InDependentView)
{
	...
	for (int32 ClipmapIndex = 0; ClipmapIndex < LevelCount; ++ClipmapIndex)
	{
		FLevelData& Level = LevelData[ClipmapIndex];
		const int32 AbsoluteLevel = ClipmapIndex + FirstLevel;		// Absolute (virtual) level index
		...
		FVirtualShadowMapCacheEntry* ClipmapLevelEntry = &PerLightCacheEntry->ShadowMapEntries[ClipmapIndex];
		...
		ClipmapLevelEntry->ProjectionData = ComputeProjectionShaderData(ClipmapIndex);
	}
...
FVirtualShadowMapProjectionShaderData FVirtualShadowMapClipmap::ComputeProjectionShaderData(int32 ClipmapIndex) const
{
	...
	Data.LightId = GetLightSceneInfo().GetPersistentIndex();

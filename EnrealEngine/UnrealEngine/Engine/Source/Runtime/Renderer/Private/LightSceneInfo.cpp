...
bool FLightSceneInfo::SetupMobileMovableLocalLightShadowParameters(const FViewInfo& View, TConstArrayView<FVisibleLightInfo> VisibleLightInfos, FMobileMovableLocalLightShadowParameters& MobileMovableLocalLightShadowParameters) const
{
	...
	MobileMovableLocalLightShadowParameters.ShadowedBits = GetShadowedBits(View, *this);
...
uint32 FLightSceneInfo::PackExtraData(bool bAllowStaticLighting, bool bLightFunction, bool bMegaLight, bool bClusteredDeferredSupported) const
{
...

	{
		...
		Result |= ShadowMapChannelMask << LIGHT_EXTRA_DATA_BIT_OFFSET_SHADOW_MAP_CHANNEL_MASK;
	}

	Result |= Proxy->GetLightingChannelMask() << LIGHT_EXTRA_DATA_BIT_OFFSET_LIGHTING_CHANNEL_MASK;

	// pack light type in this uint32 as well
	Result |= ((uint32)Proxy->GetLightType()) << LIGHT_EXTRA_DATA_BIT_OFFSET_LIGHT_TYPE;

	const uint32 CastShadows = Proxy->CastsDynamicShadow() ? 1 : 0;
	Result |= CastShadows << LIGHT_EXTRA_DATA_BIT_OFFSET_CAST_SHADOW;

	uint32 HasLightFunction = bLightFunction ? 1 : 0;
	Result |= HasLightFunction << LIGHT_EXTRA_DATA_BIT_OFFSET_HAS_LIGHT_FUNCTION;

	Result |= Proxy->LightFunctionAtlasLightIndex << LIGHT_EXTRA_DATA_BIT_OFFSET_LIGHT_FUNCTION_ATLAS_LIGHT_INDEX;

	uint32 AffectsTranslucentLighting = Proxy->AffectsTranslucentLighting() ? 1 : 0;
	Result |= AffectsTranslucentLighting << LIGHT_EXTRA_DATA_BIT_OFFSET_AFFECT_TRANSLUCENT_LIGHTING;

	uint32 MegaLight = bMegaLight ? 1 : 0;
	Result |= MegaLight << LIGHT_EXTRA_DATA_BIT_OFFSET_MEGA_LIGHT;

	uint32 ClusteredDeferredSupported = bClusteredDeferredSupported ? 1 : 0;
	Result |= ClusteredDeferredSupported << LIGHT_EXTRA_DATA_BIT_OFFSET_CLUSTERED_DEFERRED_SUPPORTED;

	const uint32 CastVolumetricShadows = Proxy->CastsVolumetricShadow() ? 1 : 0;
	Result |= CastVolumetricShadows << LIGHT_EXTRA_DATA_BIT_OFFSET_CAST_VOLUMETRIC_SHADOWS;

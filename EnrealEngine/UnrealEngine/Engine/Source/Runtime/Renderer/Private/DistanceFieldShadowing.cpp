...
class FCullObjectsForShadowCS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_ARRAY(FVector4f,ShadowConvexHull,[MAX_NUM_SHADOW_CONVEX_HULL_PLANES])
	...
	FLightTileIntersectionParameters& LightTileIntersectionParameters)
{
	...
	ensureMsgf(NumPlanes <= MAX_NUM_SHADOW_CONVEX_HULL_PLANES, TEXT("NumPlanes (%d) is expected to be less or equal to %d."), NumPlanes, MAX_NUM_SHADOW_CONVEX_HULL_PLANES);
	NumPlanes = FMath::Clamp(NumPlanes, 0, MAX_NUM_SHADOW_CONVEX_HULL_PLANES);

	{
		...
		for (int32 Index = 0; Index < NumPlanes; ++Index)
		{
			// translated planes from translated-shadow-space to translated-world-space
			const FPlane4f Plane(PlaneData[Index].TranslateBy(View.ViewMatrices.GetPreViewTranslation() - PrePlaneTranslation));
			PassParameters->ShadowConvexHull[Index] = FVector4f(FVector3f(Plane), Plane.W);
		}
	...
	FTiledShadowRendering* TiledShadowRendering)
{
	...
	if (RayTracedShadowsTexture.IsValid())
	{
		...
		PassParameters->RenderTargets[0] = FRenderTargetBinding(ScreenShadowMaskTexture, GetLoadActionIfProduced(ScreenShadowMaskTexture, ERenderTargetLoadAction::EClear));

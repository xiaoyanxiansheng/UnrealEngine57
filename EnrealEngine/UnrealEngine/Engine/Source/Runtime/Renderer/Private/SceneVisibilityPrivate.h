...
private:
	...
	bool bUsesMotionVectorWorldOffset = false;
...
public:
	...
	struct FRecordVisitor
	{
		...
		{
			VisualizeQueries.Emplace(TPair<FBox, FColor>(Box, FColor(50, 255, 50)));
		}

		void AddVisualizeQuery(const FBox& Box, const FColor& Color)
		{
			VisualizeQueries.Emplace(TPair<FBox, FColor>(Box, Color));
		}
	...
	struct FProcessVisitor
	{
		...
		void AddVisualizeQuery(const FBox& Box, const FColor& Color)
		{
			DrawWireBox(&Packet.ViewElementPDI, Box, Color, SDPG_Foreground);
		}
		...
		void Replay(const FRecordVisitor& RecordVisitor)
		{
			...
			for (const TPair<FBox, FColor>& Query : RecordVisitor.VisualizeQueries)
			{
				AddVisualizeQuery(Query.Key, Query.Value);
			}

...
bool FMobileDirectionalLightAndCSMPolicy::ShouldCompilePermutation(const FMeshMaterialShaderPermutationParameters& Parameters)
{
	if (IsMobileDeferredShadingEnabled(Parameters.Platform) && 
		!(Parameters.MaterialParameters.ShadingModels.HasShadingModel(MSM_SingleLayerWater) || Parameters.MaterialParameters.bIsTranslucentSurfaceLighting))
	{
		return false;
	}
	...
	return Parameters.MaterialParameters.ShadingModels.IsLit() &&
		(!IsTranslucentBlendMode(Parameters.MaterialParameters) || !!Parameters.MaterialParameters.bIsTranslucentSurfaceLighting) && // receive CSM when opaque or translucent surface lighting
...
bool FMobileDirectionalLightCSMAndLightMapPolicy::ShouldCompilePermutation(const FMeshMaterialShaderPermutationParameters& Parameters)
{
	...
	return (!IsTranslucentBlendMode(Parameters.MaterialParameters) || !!Parameters.MaterialParameters.bIsTranslucentSurfaceLighting) && // receive CSM when opaque or translucent surface lighting
		Super::ShouldCompilePermutation(Parameters);
}

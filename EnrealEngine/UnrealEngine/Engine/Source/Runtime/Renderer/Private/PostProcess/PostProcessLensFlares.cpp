...
class FLensFlareTransitionBuffer : public FVertexBuffer
{
public:
	virtual void InitRHI(FRHICommandListBase& RHICmdList) override
	{
		const FRHIBufferCreateDesc CreateDesc =
			FRHIBufferCreateDesc::CreateStructured(TEXT("LensFlareTransitionBuffer"), sizeof(uint32) * 4, sizeof(uint32))
	...
	const FLensFlareInputs& Inputs)
{
	...
	// Lens flare blur pass. Rasterizes a bokeh quad for each pixel on the screen based on the intensity threshold.
	{
		...
		ERDGPassFlags LensFlareBlurFlags = ERDGPassFlags::Raster;
		LensFlareBlurFlags |= GRHINeedsSRVGraphicsNonPixelWorkaround ? ERDGPassFlags::SkipRenderPass : ERDGPassFlags::None;

		GraphBuilder.AddPass(
			RDG_EVENT_NAME("LensFlareBlur %dx%d", FlareViewSize.X, FlareViewSize.Y),
			PassParameters,
			LensFlareBlurFlags,
			[VertexShader, PixelShader, VertexParameters, PixelParameters, PassParameters, AdditiveBlendState, FlareViewport, TileCount](FRDGAsyncTask, FRHICommandList& RHICmdList)
			{
				...
				// Force a transition to SRVGraphicsNonPixel on an Empty Structured Buffer to account for VS accessing the texture (using NON_PIXEL_SRV in struct gets merged)
				if (GRHINeedsSRVGraphicsNonPixelWorkaround)
				{
					RHICmdList.Transition(FRHITransitionInfo(GLensFlareTransitionBuffer->GetRHI(), ERHIAccess::UAVMask, ERHIAccess::VertexOrIndexBuffer | ERHIAccess::SRVGraphicsNonPixel));

					FRHIRenderPassInfo RPInfo(PassParameters->RenderTargets[0].GetTexture()->GetRHI(), ERenderTargetActions::Clear_Store);
					RHICmdList.BeginRenderPass(RPInfo, TEXT("LensFlareBlur"));
				}
				...
				if (GRHINeedsSRVGraphicsNonPixelWorkaround)
				{
					...
					RHICmdList.Transition(FRHITransitionInfo(GLensFlareTransitionBuffer->GetRHI(), ERHIAccess::VertexOrIndexBuffer | ERHIAccess::SRVGraphicsNonPixel, ERHIAccess::UAVMask));
				}
			});
	}

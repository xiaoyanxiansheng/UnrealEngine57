...
struct FCachedLUTSettings
{
	...
	bool UpdateCachedValues(FRDGBuilder& GraphBuilder, const FViewInfo& View, const FTexture* const* Textures, const float* Weights, uint32 BlendCount, bool bInUseCompute)
	{
		...
		UPDATE_CACHE_SETTINGS(bUseCompute, bInUseCompute, bHasChanged);
...
public:
	...
	static void ModifyCompilationEnvironment(const FGlobalShaderPermutationParameters& Parameters, FShaderCompilerEnvironment& OutEnvironment)
	{
		...
		const int32 UseVolumeLUT = PipelineVolumeTextureLUTSupportGuaranteedAtRuntime(Parameters.Platform) ? 1 : 0;
...
{
public:
	static constexpr int32 GroupSize2D = 8;
	static constexpr int32 GroupSize3D = 4;
	...
	static void ModifyCompilationEnvironment(const FGlobalShaderPermutationParameters& Parameters, FShaderCompilerEnvironment& OutEnvironment)
	{
		FLUTBlenderShader::ModifyCompilationEnvironment(Parameters, OutEnvironment);

		const int32 GroupSize = PipelineVolumeTextureLUTSupportGuaranteedAtRuntime(Parameters.Platform) ? GroupSize3D : GroupSize2D;
...
FRDGTextureRef AddCombineLUTPass(FRDGBuilder& GraphBuilder, const FViewInfo& View)
{
	...
	const bool bHasChanged = CachedLUTSettings.UpdateCachedValues(GraphBuilder, View, LocalTextures, LocalWeights, LocalCount, bUseComputePass);
	...
	if (bUseComputePass)
	{
		...
		const int32 GroupSize = bUseVolumeTextureLUT ? FLUTBlenderCS::GroupSize3D : FLUTBlenderCS::GroupSize2D;
		const uint32 GroupSizeXY = FMath::DivideAndRoundUp(OutputViewSize.X, GroupSize);

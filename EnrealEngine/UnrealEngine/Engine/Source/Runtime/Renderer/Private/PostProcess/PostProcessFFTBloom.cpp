	...
	FRDGTextureRef BlurredLogLuminanceTexture)
{
	...
	// Generate the apply constant buffer for the tone-maping pass.
	{
		...
		const float BloomIntensity = View.FinalPostProcessSettings.BloomIntensity * View.FinalPostProcessSettings.BloomConvolutionIntensity;
		...
		PassParameters->ScatterDispersionIntensity = View.FinalPostProcessSettings.BloomConvolutionScatterDispersion * BloomIntensity;
		PassParameters->KernelConstantsBuffer = GraphBuilder.CreateSRV(KernelConstantsBuffer);

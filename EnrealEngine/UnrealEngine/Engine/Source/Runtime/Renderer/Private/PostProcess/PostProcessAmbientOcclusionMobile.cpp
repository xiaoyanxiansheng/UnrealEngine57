...
// NOTE: this only applies to GTAO
...
static TAutoConsoleVariable<int32> CVarMobileAmbientOcclusionSSAOCompute(
	TEXT("r.Mobile.AmbientOcclusionSSAOCompute"),
	0,
	TEXT("Whether to use the compute shader variant of mobile SSAO\n")
	TEXT("0: Disabled (PS).\n")
	TEXT("1: Compute (graphics pipe)\n")
	TEXT("2: Compute (async pipe)\n"),
	ECVF_Scalability | ECVF_RenderThreadSafe
);
...
{
	int32 ShaderType = FMath::Clamp(CVarMobileAmbientOcclusionShaderType.GetValueOnRenderThread(), 0, 2);
...
public:
	...
	static bool ShouldCompilePermutation(const FGlobalShaderPermutationParameters& Parameters, const FCommonPermutationDomain& CommonPermutationVector)
	{
		...
		return IsMobilePlatform(Parameters.Platform) && (MobileGTAOPreIntegratedTextureType == LUTTextureType);
	}
...
static void RenderGTAO(FRDGBuilder& GraphBuilder, const FSceneTextures& SceneTextures, FRDGTextureRef AmbientOcclusionTexture, const TArray<FViewInfo>& Views)
{
	RDG_EVENT_SCOPE_STAT(GraphBuilder, MobileGTAO, "MobileGTAO");
	RDG_GPU_STAT_SCOPE(GraphBuilder, MobileGTAO);
...
class FMobileAmbientOcclusionCS : public FGlobalShader
{
public:
	DECLARE_GLOBAL_SHADER(FMobileAmbientOcclusionCS);
	SHADER_USE_PARAMETER_STRUCT(FMobileAmbientOcclusionCS, FGlobalShader);
	...
	using FPermutationDomain = TShaderPermutationDomain<FUseUpsampleDim, FShaderQualityDim>;
	...
	static void ModifyCompilationEnvironment(const FGlobalShaderPermutationParameters& Parameters, FShaderCompilerEnvironment& OutEnvironment)
	{
		...
		OutEnvironment.SetDefine(TEXT("SSAO"), 1);
		OutEnvironment.SetDefine(TEXT("USE_AO_SETUP_AS_INPUT"), 0);
	}
	...
	FScreenPassRenderTarget Output)
{
	...
	const bool bDepthBoundsTestEnabled = CVarMobileAmbientOcclusionDepthBoundsTest.GetValueOnRenderThread()
		&& !bHalfResolution
	...
	int32 ComputeMethod = CVarMobileAmbientOcclusionSSAOCompute.GetValueOnRenderThread();
	if (ComputeMethod>0)
	{
		FRDGEventName EventName(TEXT("AmbientOcclusionCS %dx%d"),
		OutputViewport.Rect.Width(), OutputViewport.Rect.Height());

		// Compute Shader Path
		FMobileAmbientOcclusionCS::FParameters* PassParameters = GraphBuilder.AllocParameters<FMobileAmbientOcclusionCS::FParameters>();
		PassParameters->SharedParameters = MoveTemp(SharedParameters);
		PassParameters->OutTexture = GraphBuilder.CreateUAV(Output.Texture);

		FMobileAmbientOcclusionCS::FPermutationDomain PermutationVector;
		PermutationVector.Set<FMobileAmbientOcclusionCS::FUseUpsampleDim>(bHalfResolution);
		PermutationVector.Set<FMobileAmbientOcclusionCS::FShaderQualityDim>(MobileAmbientOcclusionQuality);

		TShaderMapRef<FMobileAmbientOcclusionCS> ComputeShader(View.ShaderMap, PermutationVector);
		FComputeShaderUtils::AddPass(
			GraphBuilder,
			MoveTemp(EventName),
			ComputeMethod == 1 ? ERDGPassFlags::Compute : ERDGPassFlags::AsyncCompute,
			ComputeShader,
			PassParameters,
	...
	else
	{
		...
		GraphBuilder.AddPass(
			...
			PassParameters,
			ERDGPassFlags::Raster,
			[&View, Output, OutputViewport, InputViewport, VertexShader, PixelShader, PassParameters, bHalfResolution, bDepthBoundsTestEnabled, DepthFar](FRHICommandListImmediate& RHICmdList)
			{
	...
	FScreenPassRenderTarget Output)
{
	...
	if (bDepthBoundsTestEnabled)
	{
		...
		GraphBuilder.AddPass(
			...
			ClearParameters,
			ERDGPassFlags::Raster,
			[Output, OutputViewport, DepthFar](FRHICommandListImmediate& RHICmdList)
			{
				RHICmdList.SetViewport(OutputViewport.Rect.Min.X, OutputViewport.Rect.Min.Y, 0.0f, OutputViewport.Rect.Max.X, OutputViewport.Rect.Max.Y, 1.0f);

				// We must clear all pixels that won't be touched by AO shader.
				FClearQuadCallbacks Callbacks;
				Callbacks.PSOModifier = [Output](FGraphicsPipelineStateInitializer& PSOInitializer)
				{
					PSOInitializer.bDepthBounds = true;
				};
				Callbacks.PreClear = [DepthFar](FRHICommandList& InRHICmdList)
...
static void RenderSSAO(FRDGBuilder& GraphBuilder, const FSceneTextures& SceneTextures, FRDGTextureRef AmbientOcclusionTexture, const TArray<FViewInfo>& Views)
{
	...
	if (bHalfResolution)
	{
		...
		const EPixelFormat Format = bBilateralUpsample ? PF_R8G8B8A8 : PF_R8;
		const FIntPoint Extent = GetDownscaledExtent(AmbientOcclusionTexture->Desc.Extent, 2);
		ETextureCreateFlags TextureCreateFlags = TexCreate_ShaderResource | TexCreate_RenderTargetable;
		if (CVarMobileAmbientOcclusionSSAOCompute.GetValueOnRenderThread() > 0)
		{
			TextureCreateFlags |= TexCreate_UAV;
		}
		HalfResolutionTexture = GraphBuilder.CreateTexture(
			FRDGTextureDesc::Create2D(Extent, Format, FClearValueBinding::None, TextureCreateFlags),
			TEXT("HalfResolutionScreenSpaceAO"));
	}

	for (FViewInfo const& View : Views)
	{
		FMobileSSAOCommonParameters Parameters = GetMobileSSAOCommonParameters(GraphBuilder, View, SceneDepthTexture, SceneTextures.MobileUniformBuffer);
...
{
	const int32 Quality = CVarMobileAmbientOcclusionQuality.GetValueOnRenderThread();
	return Quality > 0
...
FRDGTextureRef CreateMobileScreenSpaceAOTexture(FRDGBuilder& GraphBuilder, const FSceneTexturesConfig& Config)
{
	...
	const EMobileAmbientOcclusionShaderType GtaoShaderType = GetCVarMobileAmbientOcclusionShaderTypeOnRenderThread();

	const bool bComputeShader = ( bGTAO && IsComputeShaderType(GtaoShaderType) ) || CVarMobileAmbientOcclusionSSAOCompute.GetValueOnRenderThread() > 0;
...
FRDGTextureRef FMobileSceneRenderer::RenderAmbientOcclusion(FRDGBuilder& GraphBuilder, const FSceneTextures& SceneTextures)
{
	...
	{
	case 0:
		RenderGTAO(GraphBuilder, SceneTextures, AmbientOcclusionTexture, Views);
		break;
	case 1:
		RenderSSAO(GraphBuilder, SceneTextures, AmbientOcclusionTexture, Views);
		break;
	}

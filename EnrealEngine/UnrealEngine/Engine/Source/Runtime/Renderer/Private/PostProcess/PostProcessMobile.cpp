...
public:
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(FVector4f, BufferViewRect)
...
FMobileBloomSetupOutputs AddMobileBloomSetupPass(FRDGBuilder& GraphBuilder, const FViewInfo& View, const FEyeAdaptationParameters& EyeAdaptationParameters, const FMobileBloomSetupInputs& Inputs)
{
	...
	PSShaderParameters->BufferViewRect = FVector4f(
		(Inputs.SceneColor.ViewRect.Min.X + 0.5f) / BufferSize.X,
		(Inputs.SceneColor.ViewRect.Min.Y + 0.5f) / BufferSize.Y,
		(Inputs.SceneColor.ViewRect.Max.X - 0.5f) / BufferSize.X,
		(Inputs.SceneColor.ViewRect.Max.Y - 0.5f) / BufferSize.Y);
...
FScreenPassTexture AddMobileSunMergePass(FRDGBuilder& GraphBuilder, const FViewInfo& View, const FMobileSunMergeInputs& Inputs)
{
	...
	FLinearColor BloomColor = Settings.Bloom1Tint * BloomGaussianIntensity * 0.5;

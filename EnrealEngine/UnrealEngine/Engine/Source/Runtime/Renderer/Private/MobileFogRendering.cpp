...
void FMobileSceneRenderer::RenderFog(FRHICommandList& RHICmdList, const FViewInfo& View)
{
	...
	const bool bUseAerialPerspective = !bSimpleFog && ShouldRenderSkyAtmosphere(Scene, View.Family->EngineShowFlags);
	...
	const bool bUseFogSecondTerm = !bSimpleFog && (View.ExponentialFogParameters2.Z > 0); // Fog Density in Z
	const bool bUseFogDirectionalInscatering = !bSimpleFog && !bUseFogInscatteringColorCubemap && (View.DirectionalInscatteringColor.GetLuminance() > 0 || View.bUseDirectionalInscattering);

	...
	bool bAllowSkipDrawCommand)
{
	...
	if (MeshDrawCommand.CachedPipelineId.GetId() != StateCache.PipelineId)
	{
		...
		{
			UE_LOG(LogRenderer, Log, TEXT("Missing Preload shaders. Material Friendly: %s Material Proxy: %s Static: %s"), *MeshDrawCommand.DebugData.Material->GetFriendlyName(), *MeshDrawCommand.DebugData.MaterialRenderProxyName, *MeshDrawCommand.StatsData.CategoryName.ToString());

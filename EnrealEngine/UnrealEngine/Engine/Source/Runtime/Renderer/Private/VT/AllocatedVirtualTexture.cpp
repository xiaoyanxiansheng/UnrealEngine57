	...
	, Space(nullptr)
{
	...
	// We gate this on the config support because using both 16bit and 32bit spaces can increase the total number of virtual texture spaces used and there is a hard limit.
	// But private spaces always use their own space, so we ignore the config for that case.
	bool bSupport16BitPageTable = VirtualTexturePool::Support16BitPageTables() || InDesc.bPrivateSpace;
	for (int32 Index = 0; bSupport16BitPageTable && Index < UniquePageTableLayers.Num(); ++Index)
	{
...
bool FAllocatedVirtualTexture::TryMapLockedTiles(FVirtualTextureSystem* InSystem) const
{
	...
	// Display a message if we've failed to map pages for this after a set number of frames
	...
	{
		UE_LOG(LogVirtualTexturing, Verbose, TEXT("Failed to map lowest resolution mip for AllocatedVT %s (%d frames)"), *Description.Name.ToString(), InSystem->GetFrame() - FrameAllocated);
	}
...
{
	// Force invalidate the page table when the space is no longer in the supported feedback range.
	// This will cause us to sample the fallback color only.
	if (UNLIKELY(SpaceID >= VIRTUALTEXTURE_MAX_FEEDBACK_SPACES))
	{
		OutUniform[0] = OutUniform[1] = FUintVector4(0, 0, 0, 0);
		return;
	}

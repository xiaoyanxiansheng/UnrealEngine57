...
{
	if (!SpaceFillingCurve.IsValid())
	{
		// Initialize the texture the first time it is used
		constexpr int Res = 256;
		FRHITextureCreateDesc Desc = FRHITextureCreateDesc::Create2D(TEXT("SpaceFillingCurve"), Res, Res, PF_R16_UINT)
			.SetFlags(ETextureCreateFlags::ShaderResource);
		TArray<FIntVector> Temp;
		Temp.Reserve(Res * Res);
		for (int Dy = 0; Dy < Res; Dy++)
		{
			for (int Dx = 0; Dx < Res; Dx++)
			{
				...
				// The paper describes a space filling curve from Sierpinski that passes through each pixel in a Res x Res square.
				// This routine only gives results that are in the right _order_, but do not form a strict permutation
				// so we need to store the inverse mapping and explicitly number the points after sorting them.
				const unsigned M = Res - 1;
				unsigned Result = 0;
				if (X > Y)
				{
					Result++;
				...
				Temp.Add(FIntVector(Dx, Dy, Result));
			}
		}
		Temp.Sort([](FIntVector A, FIntVector B) { return A.Z < B.Z; });
		GRenderTargetPool.FindFreeElement(GraphBuilder.RHICmdList, Desc, SpaceFillingCurve, Desc.DebugName);
		check(SpaceFillingCurve.IsValid());
		uint32 DestStride;
		uint16* DestBuffer = (uint16*) GraphBuilder.RHICmdList.LockTexture2D(SpaceFillingCurve->GetRHI(), 0, RLM_WriteOnly, DestStride, false);
		DestStride /= sizeof(uint16);
		for (int Index = 0; Index < Res * Res; Index++)
		{
			FIntVector P = Temp[Index];
			DestBuffer[P.X + DestStride * P.Y] = Index;
		}
		GraphBuilder.RHICmdList.UnlockTexture2D(SpaceFillingCurve->GetRHI(), 0, false);
	}
	return GraphBuilder.RegisterExternalTexture(GSystemTextures.SpaceFillingCurve, TEXT("SpaceFillingCurve"), ERDGTextureFlags::SkipTracking);
}
	...
	TClearValue Value)
{
	...
	else if (Dimension == ETextureDimension::Texture3D)
	{
		...
		FTextureRHIRef Texture = GraphBuilder.RHICmdList.CreateTexture(Desc);

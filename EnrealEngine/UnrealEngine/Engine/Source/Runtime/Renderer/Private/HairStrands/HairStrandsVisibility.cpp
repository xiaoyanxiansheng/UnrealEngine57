...
static int32 GHairResolutionScaleAware = 1;
static FAutoConsoleVariableRef CVarHairResolutionScaleAware(TEXT("r.HairStrands.Visibility.ResolutionScaleAware"), GHairResolutionScaleAware, TEXT("Scale hair strands based on resolution scale"));
...
static uint32 GetMaxSamplePerPixel(EShaderPlatform InPlatform)
{
	switch (GetHairVisibilityRenderMode(InPlatform))
	{
		...
		{
			// Mac platfoms don't properly support R32_UINT format for for MSAA render target. To alleviate this, force sample count to 1.
			#if PLATFORM_MAC
			{
				return 1;
			}
...
void SetUpViewHairRenderInfo(const FViewInfo& ViewInfo, bool bEnableMSAA, FVector4f& OutHairRenderInfo, uint32& OutHairRenderInfoBits, uint32& OutHairComponents)
{
	...
	const FIntRect EffectiveViewRect = GHairResolutionScaleAware > 0 ? ViewInfo.ViewRect : ViewInfo.UnconstrainedViewRect;
	const FMinHairRadiusAtDepth1 MinHairRadius = ComputeMinStrandRadiusAtDepth1(
		FIntPoint(EffectiveViewRect.Width(), EffectiveViewRect.Height()), ViewInfo.FOV, HairVisibilitySampleCount, RasterizationScaleOverride, ViewInfo.ViewMatrices.GetOrthoDimensions().X);

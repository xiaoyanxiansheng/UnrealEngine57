	...
	EDecalRenderStage DecalRenderStage)
{
	...
	// When using SubstrateDbufferPass, the DBuffer are rendered after the BasePass. In this case, the toplayer data can be piped to the DBuffer base pass for normal reading.
	if (DecalRenderStage == EDecalRenderStage::Emissive || (DecalRenderStage == EDecalRenderStage::BeforeBasePass && Substrate::IsDBufferPassEnabled(View.GetShaderPlatform())))
	{
	...
	TArray<FPSOPrecacheData>& PSOInitializers)
{
	...
	if (FeatureLevel == ERHIFeatureLevel::ES3_1)
	{
		...
		GraphicsPSOInit.SubpassIndex = (GraphicsPSOInit.SubpassHint != ESubpassHint::None) ? 1 : 0;

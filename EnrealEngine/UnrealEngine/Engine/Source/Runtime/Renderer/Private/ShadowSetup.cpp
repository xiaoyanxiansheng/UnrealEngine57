...
static TAutoConsoleVariable<int32> CVarForceOnlyVirtualShadowMaps(
	...
	TEXT("This variable should be enabled for optimal performance and memory usage, but geometric primitives that cannot be rendered into the virtual shadow map will not cast shadows.\n")
	TEXT("Disabling may reduce performance. This variable is deprecated and may be removed in a future version."),
	FConsoleVariableDelegate::CreateLambda(
		[](IConsoleVariable* CVar)
		{
			bool bForceOnlyVirtualShadowMaps = CVar->GetInt() != 0;
			if (!bForceOnlyVirtualShadowMaps)
			{
				UE_LOG(LogRenderer, Warning, 
					   TEXT("Disabling r.Shadow.Virtual.ForceOnlyVirtualShadowMaps may reduce performance. ")
					   TEXT("This variable is deprecated and may be removed in a future version."));
			}
		}
	),
	ECVF_RenderThreadSafe
	...
	TArray<FProjectedShadowInfo*, SceneRenderingAllocator>& OutPreShadows)
{
		...
		&& LightSceneInfo->Proxy->GetPerObjectProjectedShadowInitializer(Bounds, ShadowInitializer))
	{
		...
		if (CVarAllowPerObjectShadows.GetValueOnRenderThread() && MaxFadeAlpha > 1.0f / 256.0f)
		{
			...
			const int32 SizeX = MaxDesiredResolution >= MaxShadowResolution ? MaxShadowResolution : 
				bCanUseShadowDepthAtlas ? 
				(1 << (FMath::CeilLogTwo(MaxDesiredResolution) - 1)) : 
...
struct FGatherShadowPrimitivesPacket
{
	...
	void AnyThreadTask(FDynamicShadowsTaskData& TaskData)
	{
		...
		if (TaskData.FeatureLevel < ERHIFeatureLevel::SM5)
		{
			bSupportMobileInsetShadows = CVarMobileInsetShadows.GetValueOnRenderThread() 
				&& (MobileUsesFullDepthPrepass(TaskData.ShaderPlatform) || (IsMobileDeferredShadingEnabled(TaskData.ShaderPlatform) && !MobileAllowFramebufferFetch(TaskData.ShaderPlatform)));
		}
	...
	void FilterPrimitiveForShadows(FDynamicShadowsTaskData& TaskData, const FPrimitiveSceneInfoCompact& PrimitiveSceneInfoCompact)
	{
		...
		for (int32 ShadowIndex = 0, Num = TaskData.ViewDependentWholeSceneShadows.Num();ShadowIndex < Num;ShadowIndex++)
		{
				...
				&& ProjectedShadowInfo->CascadeSettings.ShadowBoundsAccurate.IntersectBox(PrimitiveBounds.Origin, PrimitiveBounds.BoxExtent))
			{
				...
				bool bCastsInsetShadow = DoesPrimitiveCastInsetShadow(TaskData, PrimitiveSceneInfo, PrimitiveProxy);

				// if we are on mobile also check if inset shadows are supported
				if (bCastsInsetShadow && TaskData.FeatureLevel < ERHIFeatureLevel::SM5)
				{
					bCastsInsetShadow &= bSupportMobileInsetShadows;
				}
				...
				if (LightSceneInfoCompact.bCastStaticShadow || LightSceneInfoCompact.bCastDynamicShadow)
				{
					...
					if (bIsVisibleInAnyView)
					{
						...
						if ((!LightSceneInfo->Proxy->HasStaticLighting() && LightSceneInfoCompact.bCastDynamicShadow) || bCreateShadowToPreviewStaticLight)
						{
							...
							if (!bMobile || bMobileInsetShadows || IsMobileCapsuleShadowsEnabled(ShaderPlatform) || (LightSceneInfo->Proxy->CastsModulatedShadows() && !LightSceneInfo->Proxy->UseCSMForDynamicObjects() && LightSceneInfo->Proxy->HasStaticShadowing()))
							{

...
private:
	...
	void RegisterHighPrecisionMouseMode();

	/** Unregisters for raw input */
	void UnregisterHighPrecisionMouseMode();
...
private:
	...
	bool bCachedUseWorkerThreadForRawInput;
	HWND HighPrecisionMouseInputWindowHandle;
	HWND CaptureWindowHandle;

	UE::FEventCount AsyncCursorEventCount;
	std::atomic<bool> bAsyncCursorTaskActive = false;
	UE::Tasks::FTask AsyncCursorTask;
	mutable FRWLock AsyncCursorRWLock;
	BOOL bAsyncCursorPosValid{};
	POINT AsyncCursorPos{};
	HWND AsyncCursorWindowHandle = NULL;
	...
	RECT ClipCursorRect;
	DWORD CachedDragAndDropKeyState;

	double RepeatTime[EMouseButtons::Invalid];
};

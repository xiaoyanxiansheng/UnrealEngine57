	...
	if( !GSplashScreenThread && FParse::Param(FCommandLine::Get(),TEXT("NOSPLASH")) != true )
	{
		...
		if ( GetSplashPath(SplashImage, SplashPath, IsCustom ) == true )
		{
			...
			if( GIsEditor )
			{
				...
				{
					const FString CopyrightInfo = NSLOCTEXT( "UnrealEd", "SplashScreen_CopyrightInfo", "Copyright \u00a9   Epic Games, Inc.   All rights reserved." ).ToString();
...
#if	STATS
			AutoRTFM::UnreachableIfClosed("TODO: Could remove if we marked `FWindowsPlatformSplash` as `AUTORTFM_DISABLE`!");

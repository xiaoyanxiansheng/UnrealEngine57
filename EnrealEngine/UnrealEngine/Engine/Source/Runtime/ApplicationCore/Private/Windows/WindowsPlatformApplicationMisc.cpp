...
void FWindowsPlatformApplicationMisc::SetHighDPIMode()
{
	if (IsHighDPIAwarenessEnabled())
	{
		...
		if (void* ShCoreDll = FPlatformProcess::GetDllHandle(TEXT("shcore.dll")))
		{
			...
			if (SetProcessDpiAwareness && GetProcessDpiAwareness && !IsRunningCommandlet() && (!FApp::IsUnattended() || bAllowHighDpiWhenUnattended))
			{
		...
		else if (void* User32Dll = GetModuleHandle(L"user32.dll"))
		{
			...
			if (SetProcessDpiAware && !IsRunningCommandlet() && (!FApp::IsUnattended() || bAllowHighDpiWhenUnattended))
			{

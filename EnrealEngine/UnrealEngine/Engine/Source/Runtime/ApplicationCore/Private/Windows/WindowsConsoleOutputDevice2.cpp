...
public:
	...
	~FConsoleWindow()
	{
		FTrackedActivity::UnregisterEventListener();
	...
	void CreateLogEntryText(TStringBuilderBase<TCHAR>& OutString, const LogEntry& E, bool ForFilter)
	{
		...
		if (bConsoleShowVerbosity)
		{
			const TCHAR* VerbosityStr = ToString(E.Verbosity);
			const TCHAR VerbositySpace[] = TEXT("           ");
			if (FCString::Strlen(VerbosityStr) >= UE_ARRAY_COUNT(VerbositySpace))
				OutString.Append(VerbosityStr, UE_ARRAY_COUNT(VerbositySpace) - 1);
			else
				OutString.Append(VerbosityStr).Append(VerbositySpace + FCString::Strlen(VerbosityStr));
			OutString.Append(": ");
		}
	...
	LRESULT LogHwndWndProc(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_RBUTTONUP:
		{
			...
			if (bConsoleShowVerbosity)
				AddItem(TEXT("Hide Verbosity"), 134);
			if (!bConsoleShowVerbosity)
				AddItem(TEXT("Show Verbosity"), 135);
		...
		case WM_COMMAND:
		{
			...
			case 134:
				bConsoleShowVerbosity = 0;
				RefreshLogHwnd();
				break;

			case 135:
				bConsoleShowVerbosity = 1;
				RefreshLogHwnd();
				break;
			}
			break;
	...
	LRESULT MainWinProc(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_COMMAND:
			...
			else if (LOWORD(wParam) == ID_COMMANDBUTTON)
			{
				...
				bool bOldConsoleShowVerbosity = bConsoleShowVerbosity;
				if (FCString::Stricmp(Command, TEXT("red")) == 0)
					...
					ConsoleShowDateTime = 0;
				else if (FCString::Stricmp(Command, TEXT("showverbosity")) == 0)
					bConsoleShowVerbosity = true;
				else if (FCString::Stricmp(Command, TEXT("hideverbosity")) == 0)
					bConsoleShowVerbosity = false;

				if (ConsoleShowDateTime != OldConsoleShowDateTime
					|| bConsoleShowVerbosity != bOldConsoleShowVerbosity)
					RefreshLogHwnd();
...
void FWindowsConsoleOutputDevice2::SaveToINI()
{
	...
	bool bConsoleShowVerbosity = Window->bConsoleShowVerbosity;
	WindowRWLock.WriteUnlock();
	...
	Config.SetBool(Selection, TEXT("ConsoleShowVerbosity"), bConsoleShowVerbosity, Filename);
...
void FWindowsConsoleOutputDevice2::Show( bool ShowWindow )
{
	if (ShowWindow)
	{
		...
		Config.GetBool(Selection, TEXT("ConsoleShowVerbosity"), bConsoleShowVerbosity, Filename);
		...
		Window->bConsoleShowVerbosity = bConsoleShowVerbosity;

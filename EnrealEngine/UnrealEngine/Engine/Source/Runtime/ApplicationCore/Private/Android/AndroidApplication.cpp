...
void FAndroidApplication::AddExternalInputDevice(TSharedPtr<IInputDevice> InputDevice)
{
	...
	{
		InputInterface->AddExternalInputDevice(MoveTemp(InputDevice));
	}
}
...
void FAndroidApplication::PollGameDeviceState( const float TimeDelta )
{
	if (!bHasLoadedInputPlugins)
	{
		...
		IModularFeatures::Get().OnModularFeatureRegistered().AddRaw(this, &FAndroidApplication::OnInputDeviceModuleRegistered);
...
void FAndroidApplication::OnInputDeviceModuleRegistered(const FName& Type, IModularFeature* ModularFeature)
{

...
#if USE_ANDROID_JNI
	const EScreenPhysicalAccuracy Accuracy  = GetNativePhysicalScreenDensity(OutScreenDensity);
...
EScreenPhysicalAccuracy FAndroidApplicationMisc::ComputePhysicalScreenDimensions(FVector2f& OutScreenDimensions, float& OutScreenSize)
{
	FIntVector2 NativeScreenResolution;
	GetNativePhysicalScreenResolution(NativeScreenResolution);

	int32 ScreenDensity;
	const EScreenPhysicalAccuracy Accuracy = GetNativePhysicalScreenDensity(ScreenDensity);

	if (ScreenDensity <= 0 || Accuracy == EScreenPhysicalAccuracy::Unknown)
	{
		UE_LOG(LogAndroid, Warning, TEXT("ComputePhysicalScreenDimensions : Incorrect ScreenDensity or Unknown ScreenPhysicalAccuracy. ScreenDensity: %d, Accuracy: %d"),
			ScreenDensity,
			Accuracy);
	...
	OutScreenDimensions = FVector2f(NativeScreenResolution) / (float)ScreenDensity;
	OutScreenSize = OutScreenDimensions.Size();

	UE_LOG(LogAndroid, Display, TEXT("ComputePhysicalScreenDimensions : NativeScreenResolution: %s, ScreenDensity: %d, OutScreenDimensions (inches): %s, OutScreenSize (inches): %f"),
		*NativeScreenResolution.ToString(),
		ScreenDensity,
		*OutScreenDimensions.ToString(),
		OutScreenSize);
...
#if USE_ANDROID_JNI
	extern FString AndroidThunkCpp_GetMetaDataString(const FString& Key);
	FString DisplayString = AndroidThunkCpp_GetMetaDataString(FString(TEXT("unreal.display")));
	TArray<FString> DisplayValues;
	DisplayString.ParseIntoArray(DisplayValues, TEXT(";"));

	FString WidthString = DisplayValues[DisplayValues.IndexOfByKey(TEXT("realWidth")) + 1];
	FString HeightString = DisplayValues[DisplayValues.IndexOfByKey(TEXT("realHeight")) + 1];

	int Width = FCString::Atoi(*WidthString);
	int Height = FCString::Atoi(*HeightString);

	OutScreenResolution = FIntVector2(Width, Height);
#endif
}

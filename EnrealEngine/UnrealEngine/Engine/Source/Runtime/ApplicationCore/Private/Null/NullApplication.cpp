...
void FNullApplication::PollGameDeviceState(const float TimeDelta)
{
	...
	if (!bHasLoadedInputPlugins)
	{
		...
		IModularFeatures::Get().OnModularFeatureRegistered().AddRaw(this, &FNullApplication::OnInputDeviceModuleRegistered);
...
void FNullApplication::OnInputDeviceModuleRegistered(const FName& Type, IModularFeature* ModularFeature)
{

...
FAutoConsoleVariableRef CVarNotificationActivationProtocolKey(
		TEXT("ios.notification.activationProtocolKey"),
...
FAutoConsoleVariableRef CVarDelayRotationUntilPresent(
		TEXT("ios.DelayRotationUntilPresent"),
...
static FAutoConsoleCommand CForceRotation(
	TEXT("ios.ForceRotation"),
	TEXT("Force a rotation with the integer representing a EDeviceScreenOrientation value passed as a parameter."),
	FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& Args)
	{
		if (Args.Num() > 0)
		{
			int32 RequestRotation = (int32)FCString::Atod(*Args[0]);
			FPlatformMisc::SetAllowedDeviceOrientation((EDeviceScreenOrientation)RequestRotation);
		}
	}),
	ECVF_Default);
#endif
...
@interface IOSNotificationHandler : NSObject
+(void)handleWhenReady:(UNNotification*)notification withCompletionHandler:(void (^)()) completionHandler;
@end
...
-(void)MainAppThread:(NSDictionary*)launchOptions
{
	...
	while(self.bIsSuspended || [UIApplication sharedApplication].applicationState != UIApplicationStateActive)
	{
		usleep(100);
	}
...
/*	dispatch_async(dispatch_get_main_queue(), ^{
	...
	// Ask iOS to re-evaluate allowed orientations if we change the ios.DelayRotationUntilPresent
	CVarDelayRotationUntilPresent->OnChangedDelegate().AddLambda([](IConsoleVariable* Variable)
	{
...
- (void)InitializeAudioSession
{
	...
	{
		UE_LOG(LogIOSAudioSession, Log, TEXT("AVAudioSessionRouteChange: UserInfo: [%s] CurrentRoute: [%s]"), *FString([[notification userInfo]description]), *FString([[[AVAudioSession sharedInstance] currentRoute] description]));
...
-(void)SetPendingActivationProtocol:(NSString *)NewProtocol
{
	...
	self.PendingActivationProtocol = NewProtocol;
	...
	{
		FTSTicker::RemoveTicker(ProtocolActivationTickerHandle);
		ProtocolActivationTickerHandle.Reset();
	}
}
...
- (void)ToggleSuspend:(bool)bSuspend
{
	...
	else
	{
		...
		// Check rotation state after returning from background
		[self didRotate:nil];
	}
...
-(BOOL)application:(UIApplication*)application continueUserActivity:(NSUserActivity*)userActivity restorationHandler:(void(^)(NSArray<id<UIUserActivityRestoring>>* _Nullable)) restorationHandler 
{
	...
	{
		[self SetPendingActivationProtocol:userActivity.webpageURL.absoluteString];
		return YES;
	}
	return NO;
}
...
{   
	...
	if(!self.bPlatformInit)
	{
		FPlatformMisc::LowLevelOutputDebugStringf(TEXT("[rotation] didRotate ignoring call before bPlatformInit self.InterfaceOrientation=%d windowScene=%d device=%d\n"), self.InterfaceOrientation, (int)[self.Window.windowScene interfaceOrientation], (int)[UIDevice currentDevice].orientation);
		return;
	}

	FPlatformMisc::LowLevelOutputDebugStringf(TEXT("[rotation] didRotate self.InterfaceOrientation=%d windowScene=%d device=%d\n"), self.InterfaceOrientation, (int)[self.Window.windowScene interfaceOrientation], (int)[UIDevice currentDevice].orientation);
	
	UIInterfaceOrientationMask PotentialMask;
	UIInterfaceOrientation NewOrientation = self.InterfaceOrientation;
	bool bForceUpdate = false;
	if (GIOSDelayRotationUntilPresent)
	{
		// Force a refresh if the current and cached orientations are not the same. This can happen at startup.
		bForceUpdate = [self.Window.windowScene interfaceOrientation] != self.InterfaceOrientation;

		UIInterfaceOrientationMask AllowedMask = [self.IOSController supportedInterfaceOrientations_Internal];

		// See if we support the current device orientation
		UIInterfaceOrientation PotentialOrientation = [IOSAppDelegate GetUIInterfaceOrientationFromUIDeviceOrientation: [UIDevice currentDevice].orientation];	
		PotentialMask = [IOSAppDelegate GetMaskFromUIInterfaceOrientation: PotentialOrientation];
		if (PotentialOrientation != UIInterfaceOrientationUnknown && (PotentialMask & AllowedMask) != 0)
		{
			FPlatformMisc::LowLevelOutputDebugStringf(TEXT("[rotation] didRotate: using device orientation\n"));
			NewOrientation = PotentialOrientation;
		}
		else
		{
			// If not, check if we support the current windowScene interfaceOrientation. It could have been programmatically set.
			PotentialOrientation = [self.Window.windowScene interfaceOrientation];
			PotentialMask = [IOSAppDelegate GetMaskFromUIInterfaceOrientation: PotentialOrientation];
			if ((PotentialMask & AllowedMask) != 0)
			{
				NewOrientation = PotentialOrientation;
				FPlatformMisc::LowLevelOutputDebugStringf(TEXT("[rotation] didRotate: using windowScene interfaceOrientation\n"));
			}
			else
			{			
				// Force rotation to the first allowed orientation
				if ((AllowedMask & UIInterfaceOrientationMaskLandscape) == UIInterfaceOrientationMaskLandscape)
				{
					...
					// Preferred default is LandscapeRight as per 
					if (PreferredLandscapeOrientation.Equals("LandscapeLeft"))
					{
						NewOrientation =  UIInterfaceOrientationLandscapeLeft;
						PotentialMask = UIInterfaceOrientationMaskLandscapeLeft;
					}
					else
					{
						NewOrientation =  UIInterfaceOrientationLandscapeRight;
						PotentialMask = UIInterfaceOrientationMaskLandscapeRight;
					}
				}
				else if (AllowedMask & UIInterfaceOrientationMaskLandscapeRight)
				{
					NewOrientation =  UIInterfaceOrientationLandscapeRight;
					PotentialMask = UIInterfaceOrientationMaskLandscapeRight;
				}
				else if (AllowedMask & UIInterfaceOrientationMaskLandscapeLeft)
				{
					NewOrientation =  UIInterfaceOrientationLandscapeLeft;
					PotentialMask = UIInterfaceOrientationMaskLandscapeLeft;
				}
				else if (AllowedMask & UIInterfaceOrientationMaskPortrait)
				{
					NewOrientation = UIInterfaceOrientationPortrait;
					PotentialMask = UIInterfaceOrientationMaskPortrait;
				}
				else if (AllowedMask & UIInterfaceOrientationMaskPortraitUpsideDown)
				{
					NewOrientation = UIInterfaceOrientationPortraitUpsideDown;
					PotentialMask = UIInterfaceOrientationMaskPortraitUpsideDown;
				}
				FPlatformMisc::LowLevelOutputDebugStringf(TEXT("[rotation] didRotate: using first allowed orientation %d mask %d (AllowedMask was %d)\n"), NewOrientation, PotentialMask, AllowedMask);
			}
		}
	}
	else
	{
		NewOrientation = [self.Window.windowScene interfaceOrientation];
	}

	if (self.InterfaceOrientation != NewOrientation || bForceUpdate)
	{
		static int32 Count=0;
		int LocalCount = ++Count;
		FPlatformMisc::LowLevelOutputDebugStringf(TEXT("[rotation] didRotate: rotating to new orientation %d, count %d\n"), (int32)NewOrientation, LocalCount);

		self.InterfaceOrientation = NewOrientation;
		FIOSApplication* Application = self.IOSApplication;
		
		FFunctionGraphTask::CreateAndDispatchWhenReady([NewOrientation, Application, LocalCount]()
		{
			FPlatformMisc::LowLevelOutputDebugStringf(TEXT("[rotation] didRotate: calling OrientationChanged(%d) from GT, count %d"), (int32)NewOrientation, LocalCount);
			Application->OrientationChanged(NewOrientation);
		}, TStatId(), NULL, ENamedThreads::GameThread);
	}
#endif
}
...
- (BOOL)application:(UIApplication *)application openURL:(NSURL *)url sourceApplication:(NSString *)sourceApplication annotation:(id)annotation
{
	...
	//    Save openurl information before engine initialize.
...
- (void)applicationDidEnterBackground:(UIApplication *)application
{
	...
	BackgroundTaskId = [[UIApplication sharedApplication] beginBackgroundTaskWithExpirationHandler:^{
		UE_LOG(LogIOS, Warning, TEXT("Background task expired before completing FCoreDelegates::ApplicationWillEnterBackgroundDelegate broadcast"));
...
{
#if !UE_BUILD_SHIPPING
	if (FParse::Param(FCommandLine::Get(), TEXT("metalshadervalidation")))
	{
		FPlatformMisc::SetEnvironmentVar(TEXT("MTL_SHADER_VALIDATION"), TEXT("1"));
		FPlatformMisc::SetEnvironmentVar(TEXT("MTL_SHADER_VALIDATION_DEFAULT_STATE"), TEXT("none"));
	}
	
	if (FParse::Param(FCommandLine::Get(), TEXT("metalapivalidation")))
	{
		FPlatformMisc::SetEnvironmentVar(TEXT("MTL_DEBUG_LAYER"), TEXT("1"));
	}
#endif
...
- (void)application:(UIApplication *)application handleEventsForBackgroundURLSession:(NSString *)identifier completionHandler:(void(^)(void))completionHandler
{
	...
	[FBackgroundURLSessionHandler::BackgroundSessionEventCompleteDelegateMap setObject:completionHandler forKey:identifier];
...
+(UIInterfaceOrientation) GetUIInterfaceOrientationFromUIDeviceOrientation:(UIDeviceOrientation)Orientation
{
	...
	{
		default:
		case UIDeviceOrientationUnknown : return UIInterfaceOrientationUnknown; break;
		case UIDeviceOrientationPortrait : return UIInterfaceOrientationPortrait; break;
		case UIDeviceOrientationPortraitUpsideDown : return UIInterfaceOrientationPortraitUpsideDown; break;
		// Note UIDeviceOrientation and UIInterfaceOrientation have have lansdcape left and right switched
		// https://developer.apple.com/documentation/uikit/uiinterfaceorientation
		case UIDeviceOrientationLandscapeLeft : return UIInterfaceOrientationLandscapeRight; break;
		case UIDeviceOrientationLandscapeRight : return UIInterfaceOrientationLandscapeLeft; break;
	}
}
...
+(UIInterfaceOrientationMask) GetMaskFromUIInterfaceOrientation:(UIInterfaceOrientation)Orientation
{
	...
	{
		default:
		case UIInterfaceOrientationUnknown : return UIInterfaceOrientationMaskAll; break;
		case UIInterfaceOrientationPortrait : return UIInterfaceOrientationMaskPortrait; break;
		case UIInterfaceOrientationPortraitUpsideDown : return UIInterfaceOrientationMaskPortraitUpsideDown; break;
		case UIInterfaceOrientationLandscapeLeft : return UIInterfaceOrientationMaskLandscapeLeft; break;
		case UIInterfaceOrientationLandscapeRight : return UIInterfaceOrientationMaskLandscapeRight; break;
	}
}
...
+(UIInterfaceOrientationMask) GetMaskFromEDeviceScreenOrientation:(EDeviceScreenOrientation)Orientation
{
	...
	{
		default:
		case EDeviceScreenOrientation::Unknown : return UIInterfaceOrientationMaskAll; break;
		case EDeviceScreenOrientation::Portrait : return UIInterfaceOrientationMaskPortrait; break;
		case EDeviceScreenOrientation::PortraitUpsideDown : return UIInterfaceOrientationMaskPortraitUpsideDown; break;
		case EDeviceScreenOrientation::LandscapeLeft : return UIInterfaceOrientationMaskLandscapeLeft; break;
		case EDeviceScreenOrientation::LandscapeRight : return UIInterfaceOrientationMaskLandscapeRight; break;
	}
}
...
static bool HandleReceivedNotification(UNNotification* notification)
{
	bool IsLocal = false;
	bool IsReadyToHandle = [IOSAppDelegate GetDelegate].bEngineInit;
	if (IsReadyToHandle)
	{
		 ...
		 withCompletionHandler:(void (^)(UNNotificationPresentationOptions options))completionHandler
{
	...
	[IOSNotificationHandler handleWhenReady:notification withCompletionHandler:^{ completionHandler(UNNotificationPresentationOptionNone); }];
}
		 ...
		 withCompletionHandler:(void (^)())completionHandler
{
	...
	if(userInfo != nullptr)
	{
		...
		// Try to find the activation protocol
		id ActivationProtocolId = userInfo[(NSString*)FPlatformString::TCHARToCFString(*GNotificationActivationProtocolKey)];
		if ([ActivationProtocolId isKindOfClass:[NSString class]])
		{
			NSString* ActivationProtocol = ActivationProtocolId;
			[self SetPendingActivationProtocol:ActivationProtocol];
		}
	}
	
	[IOSNotificationHandler handleWhenReady:response.notification withCompletionHandler:completionHandler];
}

#else
static bool HandleReceivedNotification(UNNotification* notification) { return true; }
#endif
...
@implementation IOSNotificationHandler
{
...
+(void)handleWhenReady:(UNNotification*)notification withCompletionHandler:(void (^)()) completionHandler
{
	[[[[IOSNotificationHandler alloc] initWithNotification:notification andCompletionHandler:completionHandler] autorelease] handle];
}

-(id)initWithNotification:(UNNotification*)_notification andCompletionHandler:(void (^)()) _completionHandler
{
	self = [super init];
	if(self)
	{
		completionHandler = [_completionHandler copy];
		notification = [_notification retain];
		retryCount = 0;
	}
	return self;
}
...
-(void)handle
{
	if (HandleReceivedNotification(notification))
	{
		completionHandler();
	}
	// retry for up to 10s
	...
	{
		[self performSelector:@selector(handle) withObject:nil afterDelay:0.1];
	}
	else
	{
		UE_LOG(LogIOS, Warning, TEXT("Failed to deliver notification after retrying for 10 seconds."));

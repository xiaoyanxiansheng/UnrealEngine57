...
EScreenPhysicalAccuracy FIOSPlatformApplicationMisc::ComputePhysicalScreenDimensions(FVector2f& OutScreenDimensions, float& OutScreenSize)
{
#if PLATFORM_VISIONOS
	...
	int32 ScreenDensity = CVarPhysicalScreenDensity.GetValueOnAnyThread();
	if (ScreenDensity > 0)
	{
	...
	{
		UE_LOG(LogIOS, Warning, TEXT("ComputePhysicalScreenDimensions : Approximating ScreenDensity due to CVarPhysicalScreenDensity not set or unknown (%d)."), ScreenDensity);

		const double NativeScale = [[UIScreen mainScreen] scale];
		ScreenDensity = FMath::TruncToInt(163 * NativeScale);
		if (ScreenDensity <= 0)
		{
			UE_LOG(LogIOS, Warning, TEXT("ComputePhysicalScreenDimensions : Incorrect ScreenDensity resulting in Unknown ScreenPhysicalAccuracy. ScreenDensity: %d"), ScreenDensity);
			return EScreenPhysicalAccuracy::Unknown;
		}
	...
	const FVector2f NativeResolution = FVector2f((float)NativeBounds.size.width, (float)NativeBounds.size.height);
	OutScreenDimensions = NativeResolution / (float)ScreenDensity;
	OutScreenSize = OutScreenDimensions.Size();

	UE_LOG(LogIOS, Display, TEXT("ComputePhysicalScreenDimensions - NativeResolution: %s, ScreenDensity: %d, OutScreenDimensions (inches): %s, OutScreenSize (inches): %f"),

...
void FLinuxSplashState::DrawCharacter(int32 PenX, int32 PenY, FT_GlyphSlot Glyph, int32 CurTypeIndex, float Red, float Green, float Blue)
{
	...
	const SDL_PixelFormatDetails *PixelFormat = SDL_GetPixelFormatDetails(SplashSurface->format);
	const int SplashBPP = PixelFormat->bytes_per_pixel;
...
SDL_Surface* FLinuxSplashState::LoadImage(const FString &ImagePath)
{
	...
	if (!ImagePath.EndsWith(TEXT("bmp"), ESearchCase::IgnoreCase) && FFileHelper::LoadFileToArray(RawFileData, *ImagePath))
	{
		...
		if (ImageWrapper.IsValid() && ImageWrapper->SetCompressed(RawFileData.GetData(), RawFileData.Num()))
		{
			...
			{
				SDL_Surface *Surface = SDL_CreateSurface(ImageWrapper->GetWidth(), ImageWrapper->GetHeight(), SDL_PIXELFORMAT_BGRA32);
				if (Surface)
				{
					const int Width = Surface->w;
					const int Height =  Surface->h;
					const SDL_PixelFormatDetails *PixelFormat = SDL_GetPixelFormatDetails(Surface->format);
					const int BytesPerPixel = PixelFormat->bytes_per_pixel;
...
bool FLinuxSplashState::InitSplashResources(const FText &AppName, const FString &SplashPath, const FString &IconPath)
{
	...
	if (Props == 0)
	{
		...
		UE_LOG(LogHAL, Error, TEXT("Failed to create SDL properties"));
		return false;
	}
	SDL_SetStringProperty(Props, SDL_PROP_WINDOW_CREATE_TITLE_STRING, TCHAR_TO_UTF8(*AppName.ToString()));
	SDL_SetNumberProperty(Props, SDL_PROP_WINDOW_CREATE_WIDTH_NUMBER, SplashWidth);
	SDL_SetNumberProperty(Props, SDL_PROP_WINDOW_CREATE_HEIGHT_NUMBER, SplashHeight);
	...
	SplashWindow = SDL_CreateWindowWithProperties(Props);
	...
	if (!IconPath.IsEmpty())
	{
		...
		else
		{
			...
			SDL_DestroySurface(SplashIconImage);
		}
	}
	...
	SplashTexture = SDL_CreateTexture(SplashRenderer, SDL_GetPixelFormatDetails(SplashSurface->format)->format, SDL_TEXTUREACCESS_STATIC, SplashSurface->w, SplashSurface->h);
...
void FLinuxSplashState::Redraw()
{
	...
	if (bNeedsRedraw || bStringsChanged)
	{
		...
		RenderStrings();
		SDL_RenderTexture(SplashRenderer, SplashTexture, NULL, NULL);
...
void FLinuxSplashState::Pump()
{
	if (SplashWindow)
	{
		...
		{
			if ((Event.type == SDL_EVENT_WINDOW_EXPOSED) && (SDL_GetWindowID(SplashWindow) == Event.window.windowID))
			{
				bNeedsRedraw = true;
			}
		}
...
void FLinuxPlatformSplash::Show( )
{
	...
	if ( GIsEditor )
	{
		...
		{
			const FText CopyrightInfo = NSLOCTEXT( "UnrealEd", "SplashScreen_CopyrightInfo", "Copyright \u00a9   Epic Games, Inc.   All rights reserved." );

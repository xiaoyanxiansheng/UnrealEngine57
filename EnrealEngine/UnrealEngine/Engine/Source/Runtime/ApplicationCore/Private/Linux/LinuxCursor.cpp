...
FLinuxCursor::FLinuxCursor()
	: 	bHidden(false)
	,	CachedGlobalXPosition(0.0f)
	,	CachedGlobalYPosition(0.0f)
	,	bPositionCacheIsValid(false)
{
	...
	for( int32 CurCursorIndex = 0; CurCursorIndex < EMouseCursor::TotalCursorCount; ++CurCursorIndex )
	{
		...
		case EMouseCursor::Default:
			CursorHandle = SDL_CreateSystemCursor( SDL_SYSTEM_CURSOR_DEFAULT );
			break;

		case EMouseCursor::TextEditBeam:
			CursorHandle = SDL_CreateSystemCursor( SDL_SYSTEM_CURSOR_TEXT );
			break;

		case EMouseCursor::ResizeLeftRight:
			CursorHandle = SDL_CreateSystemCursor( SDL_SYSTEM_CURSOR_EW_RESIZE );
			break;

		case EMouseCursor::ResizeUpDown:
			CursorHandle = SDL_CreateSystemCursor( SDL_SYSTEM_CURSOR_NS_RESIZE );
			break;

		case EMouseCursor::ResizeSouthEast:
			CursorHandle = SDL_CreateSystemCursor( SDL_SYSTEM_CURSOR_NWSE_RESIZE );
			break;

		case EMouseCursor::ResizeSouthWest:
			CursorHandle = SDL_CreateSystemCursor( SDL_SYSTEM_CURSOR_NESW_RESIZE );
			break;

		case EMouseCursor::CardinalCross:
			CursorHandle = SDL_CreateSystemCursor( SDL_SYSTEM_CURSOR_MOVE );
			break;
		...
		case EMouseCursor::Hand:
			CursorHandle = SDL_CreateSystemCursor( SDL_SYSTEM_CURSOR_POINTER );
			break;
		...
		case EMouseCursor::SlashedCircle:
			CursorHandle = SDL_CreateSystemCursor( SDL_SYSTEM_CURSOR_NOT_ALLOWED );
			break;
...
FLinuxCursor::~FLinuxCursor()
{
	...
	for( int32 CurCursorIndex = 0; CurCursorIndex < EMouseCursor::TotalCursorCount; ++CurCursorIndex )
	{
		switch( CurCursorIndex )
		{
			...
			case EMouseCursor::GrabHandClosed:
			case EMouseCursor::EyeDropper:
				SDL_DestroyCursor(CursorHandles[CurCursorIndex]);
				break;
...
void* FLinuxCursor::CreateCursorFromRGBABuffer(const FColor* Pixels, int32 Width, int32 Height, FVector2D InHotSpot)
{
	...
	SDL_Surface* Surface = SDL_CreateSurfaceFrom(Width, Height, SDL_GetPixelFormatForMasks(32, Rmask, Gmask, Bmask, Amask),(void*) Pixels, Width*4);
	if (Surface)
	{
...
FVector2D FLinuxCursor::GetPosition() const
{
	...
	float CursorX = CachedGlobalXPosition;
	float CursorY = CachedGlobalYPosition;
...
void FLinuxCursor::SetCachedPosition( const float X, const float Y )
{
...
void FLinuxCursor::SetPosition( const int32 X, const int32 Y )
{
	...
	if (WndFocus)
	{
		...
		SDL_WarpMouseInWindow(WndFocus, (float)(X - WndX), (float)(Y - WndY));
	}
	else
	{
	...
	SetCachedPosition((float)X, (float)Y);
}

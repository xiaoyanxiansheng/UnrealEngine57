...
void FLinuxWindow::Initialize( FLinuxApplication* const Application, const TSharedRef< FGenericWindowDefinition >& InDefinition, const TSharedPtr< FLinuxWindow >& InParent, const bool bShowImmediately )
{
	...
	UE_LOG(LogLinuxWindowType, Verbose, TEXT("Linux Window: X=%d Y=%d, Width=%d, Height=%d"), X, Y, ClientWidth, ClientHeight);
	...
	{
		// We don't set SDL_PROP_WINDOW_CREATE_TOOLTIP_BOOLEAN to true here because it requires a parent and Slate isn't set up
		// to provide one for tooltips
		...
		Definition->Type == EWindowType::Notification)
	{
		...
		SDL_SetBooleanProperty( Props, SDL_PROP_WINDOW_CREATE_UTILITY_BOOLEAN, true );
		bIsNotificationWindow = true;
		UE_LOG(LogLinuxWindowType, Verbose, TEXT("*** New Window is a parentless Notification Window ***"));
	}
	...
	if ( !Definition->HasOSWindowBorder )
	{
		...
		// MODAL and NOTIFICATION windows will already not be shown in the task bar
...
SDL_HitTestResult FLinuxWindow::HitTest( SDL_Window *SDLwin, const SDL_Point *point, void *data )
{
	...
	if ( !self.IsValid() ) 
	{
		...
		UE_LOG(LogLinuxWindow, Warning, TEXT("BAD HIT TEST EVENT: SDL window = %p\n"), SDLwin);
		return SDL_HITTEST_NORMAL;
	}

...
void FMacApplication::PollGameDeviceState(const float TimeDelta)
{
	if (!bHasLoadedInputPlugins)
	{
		...
		IModularFeatures::Get().OnModularFeatureRegistered().AddRaw(this, &FMacApplication::OnInputDeviceModuleRegistered);
...
void FMacApplication::ProcessEvent(const FDeferredMacEvent& Event)
{
	...
	if (Event.Type)
	{
		if(LastNativeEvent != nil)
		{
			[LastNativeEvent release];
			LastNativeEvent = nil;
		}
		...
		switch (Event.Type)
		{
			...
			case NSEventTypeKeyDown:
				// retain LastNativeEvent in the event of a ResendLastEvent
				if(Event.Event != nil)
				{
					[Event.Event retain];
				}
				LastNativeEvent = Event.Event;
				ConditionallyUpdateModifierKeys(Event);
...
void FMacApplication::OnInputDeviceModuleRegistered(const FName& Type, IModularFeature* ModularFeature)
{

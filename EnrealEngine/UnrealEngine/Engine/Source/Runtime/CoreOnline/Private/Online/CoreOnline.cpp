...
#include UE_INLINE_GENERATED_CPP_BY_NAME(CoreOnline)
...
{
	if (FParse::Param(FCommandLine::Get(), TEXT("EnableEOSSplitPlugins")))
	{
		return true;
	}

	bool bShouldUseEpicModularPlugins = false;
	GConfig->GetBool(TEXT("CoreOnline"), TEXT("bShouldUseEpicModularPlugins"), bShouldUseEpicModularPlugins, GEngineIni);
	return bShouldUseEpicModularPlugins;
}
...
FAccountId FOnlineIdRegistryRegistry::ToAccountId(EOnlineServices Services, const FString& StringData) const
{
	FAccountId AccountId;
	if (IOnlineAccountIdRegistry* Registry = GetAccountIdRegistry(Services))
	{
		AccountId = Registry->FromStringData(StringData);
	}

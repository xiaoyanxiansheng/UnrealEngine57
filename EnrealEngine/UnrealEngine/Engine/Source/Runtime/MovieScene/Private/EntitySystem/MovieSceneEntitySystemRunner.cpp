...
#if !NO_LOGGING
	if (LogMovieSceneECS.GetVerbosity() >= ELogVerbosity::VeryVerbose)
	{
		using namespace UE::MovieScene;

		const FInstanceRegistry* InstanceRegistry = GetLinker()->GetInstanceRegistry();
		const FSequenceInstance& SequenceInstance = InstanceRegistry->GetInstance(Instance);
		const TSharedRef<FSharedPlaybackState> SharedPlaybackState = SequenceInstance.GetSharedPlaybackState();
		const UMovieSceneSequence* Sequence = SharedPlaybackState->GetSequence(SequenceInstance.GetSequenceID());

		UE_LOG(LogMovieSceneECS, VeryVerbose, TEXT("Queue update: Instance(%d,%d); Status='%s'; Range=%s; Sequence='%s'"),
				Instance.InstanceID, Instance.InstanceSerial,
				*UEnum::GetValueAsString(Context.GetStatus()),
				*LexToString(Context.GetRange()),
...
bool FMovieSceneEntitySystemRunner::QueueFinalUpdateImpl(FInstanceHandle InInstanceHandle, FSimpleDelegate&& InOnLastFlushDelegate, bool bDestroyInstance)
{
	...
	if (LogMovieSceneECS.GetVerbosity() >= ELogVerbosity::VeryVerbose)
	{
		...
		const TSharedRef<FSharedPlaybackState> SharedPlaybackState = Instance.GetSharedPlaybackState();
		const UMovieSceneSequence* Sequence = SharedPlaybackState->GetSequence(Instance.GetSequenceID());

		UE_LOG(LogMovieSceneECS, VeryVerbose, TEXT("Queue update: Instance(%d,%d); %s; Sequence='%s'"),
				InInstanceHandle.InstanceID, InInstanceHandle.InstanceSerial,
				bDestroyInstance ? TEXT("Destroy Instance") : TEXT("Finish Instance"),
				*GetNameSafe(Sequence));
	}
#endif
...
void FMovieSceneEntitySystemRunner::FlushOutstanding(double BudgetMs, UE::MovieScene::ERunnerFlushState TargetState)
{
	...
	UE_LOG(LogMovieSceneECS, VeryVerbose, TEXT("Flushing ECS runner state '0x%#08x' with budget %fms"), FlushState, BudgetMs);
	...
	if (DissectedUpdates.Num() == 0)
	{
		...
		for (int32 UpdateIndex = 0; UpdateIndex < TempUpdateQueue.Num(); ++UpdateIndex)
		{
			...
			if (Dissections.Num() != 0)
			{
				...
				bool bHasJumped = Request.Context.HasJumped();
				...
				for (int32 Index = 0; Index < Dissections.Num() - 1; ++Index)
				{
					...
					FDissectedUpdate Dissection{
						...
						FMovieSceneContext(FMovieSceneEvaluationRange(Dissections[Index], Request.Context.GetFrameRate(), Request.Context.GetDirection()), Request.Context.GetStatus()).SetHasJumped(bHasJumped),
					...
					// Only the first dissection needs to know if we've jumped
					bHasJumped = false;
				...
				FDissectedUpdate Dissection{
					...
					FMovieSceneContext(FMovieSceneEvaluationRange(Dissections.Last(), Request.Context.GetFrameRate(), Request.Context.GetDirection()), Request.Context.GetStatus()).SetHasJumped(bHasJumped),
					Request.Params.InstanceHandle,
					MAX_int32,
...
#if !NO_LOGGING
	if (LogMovieSceneECS.GetVerbosity() >= ELogVerbosity::VeryVerbose && DissectedUpdates.Num() > 0)
	{
		UE_LOG(LogMovieSceneECS, VeryVerbose, TEXT("Executing %d dissected updates"), DissectedUpdates.Num());

		for (int32 Index = 0; Index < DissectedUpdates.Num(); ++Index)
		{
			const FDissectedUpdate& DissectedUpdate(DissectedUpdates[Index]);
			const FSequenceInstance& Instance = InstanceRegistry->GetInstance(DissectedUpdate.InstanceHandle);
			const TSharedRef<FSharedPlaybackState> SharedPlaybackState = Instance.GetSharedPlaybackState();
			const UMovieSceneSequence* Sequence = SharedPlaybackState->GetSequence(Instance.GetSequenceID());

			UE_LOG(LogMovieSceneECS, VeryVerbose,
					TEXT("[%d] Order='%s'; Instance(%d,%d); Status='%s'; Range=%s; OnFlushed='%s'; Flags='%#03x', Sequence='%s'"),
					Index,
					DissectedUpdate.Order == MAX_int32 ? TEXT("MAX") : *LexToString(DissectedUpdate.Order),
					DissectedUpdate.InstanceHandle.InstanceID, DissectedUpdate.InstanceHandle.InstanceSerial,
					*UEnum::GetValueAsString(DissectedUpdate.Context.GetStatus()),
					*LexToString(DissectedUpdate.Context.GetRange()),
					DissectedUpdate.OnFlushed.IsBound() ? TEXT("BOUND") : TEXT("unbound"),
					DissectedUpdate.UpdateFlags,
					*GetNameSafe(Sequence));
		}
	}
#endif
...
#if !NO_LOGGING
	if (LogMovieSceneECS.GetVerbosity() >= ELogVerbosity::VeryVerbose && CurrentInstances.Num() > 0)
	{
		UE_LOG(LogMovieSceneECS, VeryVerbose, TEXT("Running evaluation for %d current instances to update"), CurrentInstances.Num());

		for (int32 Index = 0; Index < CurrentInstances.Num(); ++Index)
		{
			const FQueuedUpdateParams& UpdateParams(CurrentInstances[Index]);
			const FSequenceInstance& Instance = InstanceRegistry->GetInstance(UpdateParams.InstanceHandle);
			const TSharedRef<FSharedPlaybackState> SharedPlaybackState = Instance.GetSharedPlaybackState();
			const UMovieSceneSequence* Sequence = SharedPlaybackState->GetSequence(Instance.GetSequenceID());

			UE_LOG(LogMovieSceneECS, VeryVerbose,
					TEXT("[%d] Instance(%d,%d); Status='%s'; Range=%s; Flags='%#03x', Sequence='%s'"),
					Index,
					UpdateParams.InstanceHandle.InstanceID, UpdateParams.InstanceHandle.InstanceSerial,
					*UEnum::GetValueAsString(Instance.GetContext().GetStatus()),
					*LexToString(Instance.GetContext().GetRange()),
					UpdateParams.UpdateFlags,
					*GetNameSafe(Sequence));
		}
	}
#endif

...
FGuid UMovieScene::AddSpawnable( const FString& Name, UObject& ObjectTemplate )
{
	...
	int32 NewBindingIndex = InsertSorted(ObjectBindings, FMovieSceneBinding(Spawnables[NewSpawnableIndex].GetGuid()));
...
FGuid UMovieScene::AddPossessable( const FString& Name, UClass* Class )
{
	...
	int32 NewBindingIndex = InsertSorted(ObjectBindings, FMovieSceneBinding(Possessables[NewPossessableIndex].GetGuid()));
...
{
	const FGuid PossessableID = InNewPossessable.GetGuid();
	const FGuid BindingID     = InNewBinding.GetObjectGuid();

	FGuid FinalID = PossessableID.IsValid() ? PossessableID : BindingID;

	Modify();

	FMovieScenePossessable NewPossessable = InNewPossessable;
	FMovieSceneBinding     NewBinding     = InNewBinding;

	if (!ensureMsgf(FinalID.IsValid() && PossessableID == BindingID, TEXT("GUIDs for possessable and binding objects are Invalid or do not match")))
	{
		if (!FinalID.IsValid())
		{
			FinalID = FGuid::NewGuid();
		}

		NewBinding.SetObjectGuid_Private(FinalID);
		NewPossessable.SetGuid(FinalID);
	}
...
FText UMovieScene::GetObjectDisplayName(const FGuid& ObjectId) const
{
#if WITH_EDITORONLY_DATA
	const FText* Result = ObjectsToDisplayNames.Find(ObjectId.ToString());
	...
	int32 SpawnableIndex = IndexOfSpawnable(ObjectId);
	if (SpawnableIndex != INDEX_NONE)
	{
		return FText::FromString(Spawnables[SpawnableIndex].GetName());
	}

	int32 PossessableIndex = IndexOfPossessable(ObjectId);
	if (PossessableIndex != INDEX_NONE)
	{
		return FText::FromString(Possessables[PossessableIndex].GetName());
	}
...
void UMovieScene::UpgradeTimeRanges()
{
	...
	{
		// Update the working and view ranges to encompass the new range
		const double RangeStartSeconds = PlaybackRange.Value.GetLowerBoundValue() / TickResolution;
		const double RangeEndSeconds = PlaybackRange.Value.GetUpperBoundValue() / TickResolution;
...
void UMovieScene::ReplaceBinding(const FGuid& OldGuid, const FGuid& NewGuid, const FString& Name)
{
	...
	{
		Binding->SetObjectGuid_Private(NewGuid);

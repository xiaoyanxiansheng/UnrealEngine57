...
TOptional<FFrameTime> UMovieSceneSequencePlayer::GetPauseTimeForNewPosition(const FFrameTime& NewPosition) const
{
	TOptional<TRange<FFrameTime>> PauseRange = GetPauseRange(NewPosition);

	if (PauseRange.IsSet())
	{
		if (bReversePlayback)
		{
			return PauseRange.GetValue().GetLowerBoundValue();
		}
		else
		{
			return PauseRange.GetValue().GetUpperBoundValue();
		}
	}
...
void UMovieSceneSequencePlayer::UpdateTimeCursorPosition_Internal(FFrameTime NewPosition, EUpdatePositionMethod Method, bool bHasJumpedOverride)
{
	...
	const TOptional<FFrameTime> PauseAtTime = GetPauseTimeForNewPosition(NewPosition);
	if (!PauseAtTime.IsSet() && Method == EUpdatePositionMethod::Play && ShouldStopOrLoop(NewPosition))
	{
		...
		if (bLoopIndefinitely || CurrentNumLoops + NumTimesLooped <= PlaybackSettings.LoopCount.Value)
		{
			...
			const bool bHasLooped = true;
			UpdateMovieSceneInstance(Range, StatusOverride, bHasJumped, bHasLooped);
	...
	{
		// If the desired evaluation will take us past where we want to go we need to use a clipped range provided by PauseAtTime, otherwise use the normal one.
		FFrameTime ActualNewPosition = PauseAtTime.IsSet() ?
			FFrameRate::TransformTime(PauseAtTime.GetValue(), PlayPosition.GetOutputRate(), PlayPosition.GetInputRate()) :
			NewPosition;
		FMovieSceneEvaluationRange Range = UpdatePlayPosition(
				PlayPosition,
				ActualNewPosition,
				Method,
				bReversePlayback);
...
void UMovieSceneSequencePlayer::UpdateMovieSceneInstance(FMovieSceneEvaluationRange InRange, EMovieScenePlayerStatus::Type PlayerStatus, bool bHasJumped, bool bHasLooped)
{
	FMovieSceneUpdateArgs Args;
	Args.bHasJumped = bHasJumped;
	Args.bHasLooped = bHasLooped;
...
void UMovieSceneSequencePlayer::UpdateMovieSceneInstance(FMovieSceneEvaluationRange InRange, EMovieScenePlayerStatus::Type PlayerStatus, const FMovieSceneUpdateArgs& Args)
{
	...
	Context.SetHasLooped(Args.bHasLooped);
...
FString UMovieSceneSequencePlayer::GetSequenceName(bool bAddClientInfo) const
{
	if (Sequence)
	{
		FString SequenceName = GetNameSafe(Sequence);
		if (bAddClientInfo)
		{

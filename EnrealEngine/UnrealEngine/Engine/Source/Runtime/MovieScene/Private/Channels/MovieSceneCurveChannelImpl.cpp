...
template<typename ChannelType>
void TMovieSceneCurveChannelImpl<ChannelType>::PopulateCurvePoints(const ChannelType* InChannel, double StartTimeSeconds, double EndTimeSeconds, double TimeThreshold, CurveValueType ValueThreshold, FFrameRate InTickResolution, TArray<TTuple<double, double>>& OutPoints)
{
	if (InChannel)
	{
		UE::MovieScene::DrawBezierCurve(
			*InChannel, 
			InTickResolution, 
			TimeThreshold, 
			ValueThreshold, 

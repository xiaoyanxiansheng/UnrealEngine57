...
void FMovieSceneChannelData::ReplaceKeyHandlesInternal(TConstArrayView<int32> KeyIndices, TConstArrayView<FKeyHandle> NewHandles)
{
	if (ensureMsgf(KeyHandles, TEXT("This channel does not contain key handles"))
		&& ensure(KeyIndices.Num() == NewHandles.Num()))
	{
		for (int32 Index = 0; Index < KeyIndices.Num(); ++Index)
		{
			KeyHandles->ReplaceHandle(KeyIndices[Index], NewHandles[Index]);
		}
	}
}

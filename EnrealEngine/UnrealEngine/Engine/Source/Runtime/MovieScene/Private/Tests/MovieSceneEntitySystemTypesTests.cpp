...
bool FMovieSceneBlendTargetComponentTest::RunTest(const FString& Parameters)
{
	...
	// Simple test - use inline allocation
	{
		...
		// Verify that calling GetTypeHash does not segfault on the manual memory management
		(void)GetTypeHash(BlendTarget);
	}
	...
	// Test move and copy operators for heap allocations
	{
		...
		// Make some copies
		{
			...
			// Verify that calling GetTypeHash does not segfault on the manual memory management
			(void)GetTypeHash(BlendTargetCopy);
			(void)GetTypeHash(BlendTarget[0]);
			...
			// Verify that calling GetTypeHash does not segfault on the manual memory management
			(void)GetTypeHash(BlendTargetCopy);
			(void)GetTypeHash(BlendTarget[1]);
		}
		...
		// Make some moves
		{
			...
			// Verify that calling GetTypeHash does not segfault on the manual memory management
			(void)GetTypeHash(BlendTargetMoved);
			(void)GetTypeHash(BlendTargetCopy1);
			...
			// Verify that calling GetTypeHash does not segfault on the manual memory management
			(void)GetTypeHash(BlendTargetMoved);
			(void)GetTypeHash(BlendTargetCopy2);
		}
	}

...
struct FMovieSceneChannelData
{
	 ...
	 */
	inline TArrayView<FFrameNumber> GetTimes()
	{
		return *Times;
	}
...
protected:
	 ...
	 *
	 * @param KeyIndices	The index of the key to replace
	 * @param NewHandles	The key handle instance that should be at the given index.
	 */
...
struct TMovieSceneChannelData : FMovieSceneChannelData
{
	 ...
	 */
	inline TArrayView<const ValueType> GetValues() const
	{
		return *Values;
	}
	 ...
	 */
	inline TArrayView<ValueType> GetValues()
	{
		return *Values;
	}
	...
	{
		return MoveKey(KeyIndex, NewTime, bRemoveDuplicateKeys, [](const FKeyHandle& KeyHandle){ return true; });
	}

	template<typename TCanRemove> requires std::is_invocable_r_v<bool, TCanRemove, const FKeyHandle&>
	int32 MoveKey(int32 KeyIndex, FFrameNumber NewTime, bool bRemoveDuplicateKeys, TCanRemove&& CanRemove)
	{
		if (bRemoveDuplicateKeys)
		{
			...
			KeysToRemove.SetNum(Algo::RemoveIf(KeysToRemove, CanRemove));
	 ...
	 * Replaces the key handle at the given Index with NewHandle. Fails if the handle is already in use.
	 *
	 * @param KeyIndex	The index of the key to replace
	 * @param NewHandle	The key handle instance that should be at the given index.
	 */
	void ReplaceKeyHandle(int32 KeyIndex, FKeyHandle NewHandle)
	{
		ReplaceKeyHandles(TConstArrayView<int32>(&KeyIndex, 1), TConstArrayView<FKeyHandle>(&NewHandle, 1));
	}
	/**
	 * Replaces the key handles at the given KeyIndices with NewHandles. Handles already in use are skipped.
	 *
	 * @param KeyIndices	The indidces of the keys to replace
	 * @param NewHandles	The key handle instances that should be at the given indices.
	 */
	void ReplaceKeyHandles(TConstArrayView<int32> KeyIndices, TConstArrayView<FKeyHandle> NewHandles)
	{
		ReplaceKeyHandlesInternal(KeyIndices, NewHandles);
	}

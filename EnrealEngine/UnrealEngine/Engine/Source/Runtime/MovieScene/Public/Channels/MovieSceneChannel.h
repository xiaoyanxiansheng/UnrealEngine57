...
struct FCycleParams
{
	...
	inline void ComputePreValueOffset(double FirstValue, double LastValue)
	{
		// CycleCount is negative for pre-extrap
		ValueOffset = (LastValue - FirstValue) * CycleCount;
	}
	inline void ComputePostValueOffset(double FirstValue, double LastValue)
	{
		ValueOffset = (LastValue - FirstValue) * CycleCount;
	}
	inline bool ShouldMirrorCurve() const
	{
		return bMirrorCurve;
	}
	inline void Oscillate(int32 MinFrame, int32 MaxFrame)
	{

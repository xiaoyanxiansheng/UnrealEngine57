...
namespace Private
{
	template<typename...> struct TComponentTypeIDsImpl;
}
template<typename...T>
...
{
	template<typename T>
	friend inline FEntityIterationResult operator,(T, FEntityIterationResult)
	{
		return FEntityIterationResult { true };
	}

	friend inline FEntityIterationResult operator,(bool In, FEntityIterationResult)
	{
...
struct TEntityTaskComponentsImpl<TIntegerSequence<int, Indices...>, T...>
{
	...
	{
		return ReadAllOf(TypeIDs, TMakeIntegerSequence<int, sizeof...(U)>());
	}
	template<typename... U, int... ExtraIndices>
	...
	{
		return TEntityTaskComponents<T..., TReadAccess<U>... >( CommonParams, Accessors.template Get<Indices>()..., TReadAccess<U>(TypeIDs.GetType(ExtraIndices).template ReinterpretCast<U>())... );
	}
...
{
	template<typename... T>
	inline static void ForEachAllocation(T&&...)
	{}

	template<typename... T>
	inline static void ForEachEntity(T&&...)
	{}
};
...
{
	template<typename TaskImpl>
	inline static void ForEachEntityImpl(TaskImpl& TaskImplInstance, ...)
	{
		static_assert(!std::is_same_v<TaskImpl, TaskImpl>, "non-expanded entity iteration is not supported");
	}

	template<typename TaskImpl, typename... AccessorTypes>
	inline static void ForEachAllocationImpl(TaskImpl& TaskImplInstance, FEntityAllocationIteratorItem Item, TArrayView<const FPreLockedDataPtr> PreLockedData, FEntityAllocationWriteContext WriteContext, const TEntityTaskComponents<AccessorTypes...>& Components)
	{
		TaskImplInstance.ForEachAllocation(Item, Components, PreLockedData);
	}

	template<typename TaskImpl, typename... AccessorTypes>
	inline static void ForEachAllocationImpl(TaskImpl& TaskImplInstance, FEntityAllocationIteratorItem Item, const TEntityTaskComponents<AccessorTypes...>& Components)
	{

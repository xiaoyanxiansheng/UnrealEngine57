...
{
	inline static constexpr bool IsGarbage(...)
	{
		return false;
	}
	template<typename T>
	inline static std::enable_if_t<TPointerIsConvertibleFromTo<T, const UObject>::Value, bool> IsGarbage(T* InObject)
	{
	...
	inline static bool IsGarbage(FObjectKey InObject)
	{
		return FBuiltInComponentTypes::IsBoundObjectGarbage(InObject.ResolveObjectPtr());
	}
	template<typename T>
	inline static bool IsGarbage(TObjectPtr<T>& InObject)
	{
		return FBuiltInComponentTypes::IsBoundObjectGarbage(InObject);
	}
	inline static bool IsGarbage(FObjectComponent& InComponent)
	{
		return FBuiltInComponentTypes::IsBoundObjectGarbage(InComponent.GetObject());
	}
	
	template<typename T>
	inline static void AddReferencedObjects(FReferenceCollector& ReferenceCollector, T* In)
	{

...
protected:
	...
	MOVIESCENE_API void UpdateMovieSceneInstance(FMovieSceneEvaluationRange InRange, EMovieScenePlayerStatus::Type PlayerStatus, bool bHasJumped = false, bool bHasLooped = false);
	...
	* then a range of time that should be evaluated to reach there will be returned, in tick resolution.
	* If we should not pause then the TOptional will be unset.
	* */
	MOVIESCENE_API TOptional<TRange<FFrameTime>> GetPauseRange(const FFrameTime& NewPosition) const;
	/**
	 * Same as GetPauseRange but returns the next time to update to (the upper or lower bound value of
	 * the range returned by GetPauseRange, depending on the playback direction), in tick resolution.
	 */
	MOVIESCENE_API TOptional<FFrameTime> GetPauseTimeForNewPosition(const FFrameTime& NewPosition) const;

...
public:
	 ...
	 */
	virtual bool IsCompatibleSubSequence(const UMovieSceneSequence& SubSequence) const { return true; }

	/**
	 * Sequences can determine if they would be compatible as a subsequence to the given parent sequence.
	 */
	virtual bool IsCompatibleAsSubSequence(const UMovieSceneSequence& ParentSequence) const { return true; }
};

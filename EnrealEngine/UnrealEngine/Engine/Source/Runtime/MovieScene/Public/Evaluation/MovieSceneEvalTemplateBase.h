...
public:
	 ...
	 */
	inline bool RequiresSetup() const { return (OverrideMask & RequiresSetupFlag) != 0; }

	/**
	 * Check whether this entity requires tear up when it no longer being evaluated
	 */
	inline bool RequiresTearDown() const { return (OverrideMask & RequiresTearDownFlag) != 0; }
	 ...
	 * @param Player				The player that is responsible for playing back this template
	 */
	inline void OnEndEvaluation(FPersistentEvaluationData& PersistentData, IMovieScenePlayer& Player) const
	{
		if (RequiresTearDown())
		{

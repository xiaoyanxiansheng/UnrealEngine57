...
struct TPreAnimatedPropertyTraits<PropertyTraits, TIntegerSequence<int, MetaDataIndices...>, MetaDataTypes...> : FBoundObjectPreAnimatedStateTraits
{
	...
	TPreAnimatedPropertyTraits(const PropertyTraits* InPropertyTraitsInstance)
		: PropertyTraitsInstance(InPropertyTraitsInstance)
	{}
	...
	void RestorePreAnimatedValue(const KeyType& InKey, StorageType& CachedValue, const FRestoreStateParams& Params)
	{
		...
		{
			PropertyTraitsInstance->SetObjectPropertyValue(Object, CachedValue.MetaData.template Get<MetaDataIndices>()..., *FastOffset, CachedValue.Data);
		}
		...
		{
			PropertyTraitsInstance->SetObjectPropertyValue(Object, CachedValue.MetaData.template Get<MetaDataIndices>()..., Bindings->Get(), CachedValue.Data);
		}
		else if (const FCustomPropertyAccessor* CustomAccessor = CachedValue.Binding.template Get<const FCustomPropertyAccessor*>())
		{
			PropertyTraitsInstance->SetObjectPropertyValue(Object, CachedValue.MetaData.template Get<MetaDataIndices>()..., *CustomAccessor, CachedValue.Data);
		}
	}
};
...
struct TPreAnimatedPropertyTraits<PropertyTraits, TPropertyMetaData<>, TIntegerSequence<int>> : FBoundObjectPreAnimatedStateTraits
{
	...
	void RestorePreAnimatedValue(const KeyType& InKey, StorageType& CachedValue, const FRestoreStateParams& Params)
	{
		...
		if (const uint16* FastOffset = CachedValue.Binding.template TryGet<uint16>())
		{
			PropertyTraitsInstance->SetObjectPropertyValue(Object, *FastOffset, CachedValue.Data);
		}
		else  if (const TSharedPtr<FTrackInstancePropertyBindings>* Bindings = CachedValue.Binding.template TryGet<TSharedPtr<FTrackInstancePropertyBindings>>())
		{
			PropertyTraitsInstance->SetObjectPropertyValue(Object, Bindings->Get(), CachedValue.Data);
		}
		else if (const FCustomPropertyAccessor* CustomAccessor = CachedValue.Binding.template Get<const FCustomPropertyAccessor*>())
		{
			PropertyTraitsInstance->SetObjectPropertyValue(Object, *CustomAccessor, CachedValue.Data);
		}
	}
};
	...
	, IPreAnimatedObjectPropertyStorage
{
	...
	TPreAnimatedPropertyStorageImpl(const FPropertyDefinition& InPropertyDefinition)
		: TPreAnimatedStateStorage<StorageTraits>(static_cast<const PropertyTraits*>(InPropertyDefinition.TraitsInstance))
		, PropertyTraitsInstance(static_cast<const PropertyTraits*>(InPropertyDefinition.TraitsInstance))
		...
		for (int32 Index = 0; Index < Num; ++Index)
		{
			...
			if (!this->IsStorageRequirementSatisfied(Entry.ValueHandle.StorageIndex, StorageRequirement))
			{
				...
				if (Fast)
				{
					...
					PropertyTraitsInstance->GetObjectPropertyValue(BoundObjects[Index], MetaData.template Get<MetaDataIndices>()[Index]..., Fast[Index], NewValue.Data);
				}
				else if (Custom)
				{
					...
					PropertyTraitsInstance->GetObjectPropertyValue(BoundObjects[Index], MetaData.template Get<MetaDataIndices>()[Index]..., Accessor, NewValue.Data);
				}
				else if (Slow)
				{
					...
					PropertyTraitsInstance->GetObjectPropertyValue(BoundObjects[Index], MetaData.template Get<MetaDataIndices>()[Index]..., Bindings.Get(), NewValue.Data);
				}

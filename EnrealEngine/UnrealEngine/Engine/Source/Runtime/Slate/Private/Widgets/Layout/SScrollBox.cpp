...
#include UE_INLINE_GENERATED_CPP_BY_NAME(SScrollBox)
...
void SScrollPanel::Construct(const FArguments& InArgs, TArray<SScrollBox::FSlot::FSlotArguments> InSlots)
{
	...
	bAllowContentToShrink = InArgs._AllowContentToShrink;
}
...
void SScrollPanel::OnArrangeChildren(const FGeometry& AllottedGeometry, FArrangedChildren& ArrangedChildren) const
{
	...
	{
		ArrangeChildrenInStack<EOrientation::Orient_Horizontal, TPanelChildren<SScrollBox::FSlot>, SScrollBox::FSlot>(GSlateFlowDirection, this->Children, AllottedGeometry, ArrangedChildren, ChildrenOffset, bAllowContentToShrink, ArrangedSize);
	}
	else
	{
		ArrangeChildrenInStack<EOrientation::Orient_Vertical, TPanelChildren<SScrollBox::FSlot>, SScrollBox::FSlot>(GSlateFlowDirection, this->Children, AllottedGeometry, ArrangedChildren, ChildrenOffset, bAllowContentToShrink, ArrangedSize);
	}
}
...
UE::Slate::FDeprecateVector2DResult SScrollBox::GetScrollPanelContentSize() const
{
	...
	if (!bAllowContentToShrink || Size.IsNearlyZero())
	{
		Size = ScrollPanel->GetDesiredSize();
	}
...
void SScrollBox::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{
	...
	{
		ScrollIntoViewRequest(CachedGeometry);
	...
	float NewPhysicalOffset = GetScrollComponentFromVector(CurrentViewOffset * GetScrollPanelContentSize());
	if ( AllowOverscroll == EAllowOverscroll::Yes )
	{
		NewPhysicalOffset += Overscroll.GetOverscroll(CachedGeometry);
	}
...
FSlateColor SScrollBox::GetEndShadowOpacity() const
{
	...
	const float ShadowOpacity = (ScrollBar->DistanceFromBottom() * GetScrollComponentFromVector(GetScrollPanelContentSize()) / ShadowFadeDistance);
...
void SScrollBox::EndInertialScrolling()
{
	...
	// Sets the desired scroll offset to the current one so the panel does not continue moving
	DesiredScrollOffset = ScrollPanel->PhysicalOffset;
}

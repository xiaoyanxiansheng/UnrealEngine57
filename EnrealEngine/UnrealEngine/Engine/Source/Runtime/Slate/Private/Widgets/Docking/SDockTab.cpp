...
ETabRole SDockTab::GetVisualTabRole() const
{
	...
	if (this->TabRole == ETabRole::NomadTab)
	{
		...
		// If we have a parent docking area but no docking tab well. We are in drawer. In this case we will not use the major style.
		else if (!GetParent().IsValid() && !ParentDockingAreaPtr.IsValid())
		{
			// We are dragging, but we are not dragging over anything, assume major
...
void SDockTab::RemoveTabFromParent()
{
	...
	RemoveTabFromParent_Internal();
}

TSharedPtr<SDockingArea> SDockTab::GetDockArea() const
{
	if (TSharedPtr<SDockingArea> DockingArea = ParentDockingAreaPtr.Pin())
	{
		return DockingArea;
	}

	if (TSharedPtr<SDockingTabWell> Parent = ParentPtr.Pin())
	{
		return Parent->GetDockArea();
	}
...
void SDockTab::ActivateInParent(ETabActivationCause InActivationCause)
{
	...
	if (InActivationCause != UserClickedOnTab || !GetContent()->SupportsKeyboardFocus())
	{
		return;
	}

	FSlateApplication::Get().SetKeyboardFocus(GetContent(), EFocusCause::SetDirectly);
}
	...
	SBorder::Construct( SBorder::FArguments()
		...
		[
			...
			[
				...
				.AutoWidth()
				.VAlign(VAlign_Center)
				.Padding(IsTabNameHidden() ? FMargin(0) : FMargin(0, 0, 5, 0))
				[
					SNew(SBorder)
				...
				+ SHorizontalBox::Slot()
				.AutoWidth()
				.HAlign(IsTabNameHidden() ? HAlign_Left : HAlign_Right)
				.VAlign(VAlign_Center)
				[
					SNew(SButton)
...
void SDockTab::RemoveTabFromParent_Internal()
{
	// Keep this object alive until the end of this method since it may die by the time RemoveAndDestroyTab returns
	TSharedRef<SDockTab> This = SharedThis(this);
	if (ParentPtr.IsValid())
	{
		ParentPtr.Pin()->RemoveAndDestroyTab(This, SDockingNode::TabRemoval_Closed);
	}
	else if (TSharedPtr<SDockingArea> DockingArea = ParentDockingAreaPtr.Pin())
	{
		// Tab is hosted in a panel drawer
		DockingArea->ClosePanelDrawerForTransfer();
		DockingArea->RemoveHiddenInactivePanelDrawerTab(This);
	}

	ParentPtr.Reset();
	ParentDockingAreaPtr.Reset();
}
...
EVisibility SDockTab::HandleIsCloseButtonVisible() const
{
	...
	return bCanEverClose && !IsTabLocked() && bIsVisibled && bIsTabCloseable ? EVisibility::Visible : EVisibility::Hidden;
}
...
{
	const TSharedPtr<SDockingTabStack>& StackTab = GetParentDockTabStack();
	return StackTab.IsValid() ? StackTab->IsTabLocked(SharedThis(this)) : false;
}
...
{
	return bIsTabNameHidden;
}

void SDockTab::SetIsTabNameHidden(bool bInIsTabNameHidden)
{
	bIsTabNameHidden = bInIsTabNameHidden;
}

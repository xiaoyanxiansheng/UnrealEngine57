...
void SVirtualJoystick::SetJoystickVisibility(const bool bInVisible, const bool bInFade)
{
	...
	//Update slate visibility - we still want to tick, so we use HitTestInvisible to avoid blocking other UI
	SetVisibility(bVisible ? EVisibility::Visible : EVisibility::HitTestInvisible);
}

...
void SExpanderArrow::PrivateRegisterAttributes(FSlateAttributeInitializer& AttributeInitializer)
{
	...
	SLATE_ADD_MEMBER_ATTRIBUTE_DEFINITION(AttributeInitializer, BaseIndentLevelAttribute, EInvalidateWidgetReason::Layout);
	SLATE_ADD_MEMBER_ATTRIBUTE_DEFINITION(AttributeInitializer, IndentAmountAttribute, EInvalidateWidgetReason::Layout);
}

SExpanderArrow::SExpanderArrow()
	: StyleSet(nullptr)
	, ShouldDrawWires(*this, false)
	, IndentAmountAttribute(*this, 10.0f)
	, BaseIndentLevelAttribute(*this, 0)
{
...
void SExpanderArrow::Construct( const FArguments& InArgs, const TSharedPtr<class ITableRow>& TableRow  )
{
	...
	SetBaseIndentLevel(InArgs._BaseIndentLevel);
...
int32 SExpanderArrow::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
	...
	const float Indent = IndentAmountAttribute.Get();
...
void SExpanderArrow::SetIndentAmount(TAttribute<float> InIndentAmount)
{
	IndentAmountAttribute.Assign(*this, MoveTemp(InIndentAmount));
}

void SExpanderArrow::SetBaseIndentLevel(TAttribute<int32> InBaseIndentLevel)
{
	BaseIndentLevelAttribute.Assign(*this,  MoveTemp(InBaseIndentLevel));
}
...
{
	const int32 NestingDepth = FMath::Max(0, OwnerRowPtr.Pin()->GetIndentLevel() - BaseIndentLevelAttribute.Get());
	const float Indent = IndentAmountAttribute.Get();

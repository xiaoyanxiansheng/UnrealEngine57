...
void SRadialBox::Construct( const FArguments& InArgs )
{
	...
	bClockwiseOrder = InArgs._bClockwiseOrder;
...
void SRadialBox::FChildArranger::Arrange()
{
	...
	for (int32 ChildIndex = 0; ChildIndex < NumItems; ++ChildIndex)
	{
		...
		DegreeOffset += RadialBox.bClockwiseOrder ? DegreeIncrements : (-1 * DegreeIncrements);
...
void SRadialBox::SetSectorCentralAngle(float InSectorCentralAngle)
{
	SectorCentralAngle = InternalNormalizeAngle(InSectorCentralAngle);
}
...
float SRadialBox::InternalNormalizeAngle(float Angle) const
{
	...
	return NormalizedAngle < UE_SMALL_NUMBER ? NormalizedAngle + 360.f : NormalizedAngle;
}

...
FReply SButton::OnFocusReceived(const FGeometry& MyGeometry, const FFocusEvent& InFocusEvent)
{
	FReply ReturnReply = SBorder::OnFocusReceived(MyGeometry, InFocusEvent);
...
/**/
SLATE_API FReply SButton::OnDragDetected(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	//	If the drag detected handler is bound, we want to use the response up the chain
	if (OnSlateButtonDragDetected.IsBound())
	{
		return OnSlateButtonDragDetected.Execute(MyGeometry, MouseEvent);
	}
	else
	{
		return SBorder::OnDragDetected(MyGeometry, MouseEvent);
	}
}

SLATE_API void SButton::OnDragEnter(FGeometry const& MyGeometry, FDragDropEvent const& DragDropEvent)
{
	if (OnSlateButtonDragEnter.IsBound())
	{
		OnSlateButtonDragEnter.Execute(MyGeometry, DragDropEvent);
	}
	SBorder::OnDragEnter(MyGeometry, DragDropEvent);
}

SLATE_API void SButton::OnDragLeave(FDragDropEvent const& DragDropEvent)
{
	if (OnSlateButtonDragLeave.IsBound())
	{
			OnSlateButtonDragLeave.Execute(DragDropEvent);
	}
	SBorder::OnDragLeave(DragDropEvent);
}

SLATE_API FReply SButton::OnDragOver(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent)
{
	if (OnSlateButtonDragOver.IsBound())
	{
		return OnSlateButtonDragOver.Execute(MyGeometry, DragDropEvent);
	}
	else
	{
...
SLATE_API FReply SButton::OnDrop(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent)
{
	if (OnSlateButtonDrop.IsBound())
	{
		return OnSlateButtonDrop.Execute(MyGeometry, DragDropEvent);
	}
	else
	{
...
void SButton::SetOnFocusReceived(FSimpleDelegate InOnFocusReceived)
{
	OnReceivedFocus = InOnFocusReceived;
}

void SButton::SetOnFocusLost(FSimpleDelegate InOnFocusLost)
{
	OnLostFocus = InOnFocusLost;
}
...
SLATE_API void SButton::SetAllowDragDrop(bool bInAllowDragDrop)
{

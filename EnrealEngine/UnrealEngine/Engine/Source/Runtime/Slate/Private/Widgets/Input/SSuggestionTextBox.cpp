...
void SSuggestionTextBox::Construct( const FArguments& InArgs )
{
	...
	CommitsOnSuggestionSelected = InArgs._CommitOnSuggestionSelected.Get();
...
FReply SSuggestionTextBox::OnKeyDown( const FGeometry& MyGeometry, const FKeyEvent& KeyEvent )
{
	...
	if (MenuAnchor->IsOpen())
	{
		...
		else if (KeyEvent.GetKey() == EKeys::Enter && CommitsOnSuggestionSelected && SuggestionListView->GetNumItemsSelected() > 0)
		{
			HandleTextBoxTextCommitted(FText::FromString(*Suggestions[SelectedSuggestion]), ETextCommit::OnEnter);
		}
	...
	if (!IgnoreUIUpdate)
	{
		for (int32 i = 0; i < Suggestions.Num(); ++i)
		{
			if (NewValue == Suggestions[i])
			{
				...
				if (CommitsOnSuggestionSelected)
				{
					HandleTextBoxTextCommitted(FText::FromString(*NewValue.Get()), ETextCommit::OnEnter);
				}

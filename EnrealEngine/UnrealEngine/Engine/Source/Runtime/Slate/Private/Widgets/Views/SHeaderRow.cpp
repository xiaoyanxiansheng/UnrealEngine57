...
template <>
struct TWidgetTypeTraits<class STableColumnHeader>
{
...
public:
	...
	void Construct( const STableColumnHeader::FArguments& InArgs, const SHeaderRow::FColumn& Column, const TSharedRef<SWidget>& OverrideHeaderMenuContent, const FMargin DefaultHeaderContentPadding )
	{
		...
		CanManuallySort = Column.CanManuallySort;
		...
		{
			TSharedRef<SWidget> SortedHeaderContent = SNew(SBox)
			...
			{
				// Variation that shows sorting direction without allowing user controls over sorting
				Box->AddSlot()
					.FillWidth(1.0f)
					.HAlign(HAlign_Fill)
					[
	...
	{
		if (SortMode.Get() == EColumnSortMode::None && !CanManuallySort.Get() )
		{
			return GetInitialSortMode();
		}
		else
		{
			return SortMode.Get();
...
private:
	...
	const FSlateBrush* GetSortingBrush() const
	{
		...
		return (GetSortMode() == EColumnSortMode::Ascending ?
	...
	{
		EColumnSortMode::Type ColumnSortMode = GetSortMode();
		const bool bShow = ColumnSortMode != EColumnSortMode::None && bHasBeenManuallySorted && !IsSorting.Get();
...
private:
	...
	/** Whether or not the sorting mode should be under manual user control. */
	...
	bool bHasBeenManuallySorted = false;
};
...
bool SHeaderRow::ShouldGenerateEmptyColumnForSpacing(const FName& InColumnId) const
{
	for (const FColumn& SomeColumn : Columns)
	{
		if (SomeColumn.ColumnId == InColumnId)
		{
			return SomeColumn.ShouldGenerateWidget.Get(true) && SomeColumn.ShouldGenerateEmptyWidgetForSpacing.Get(true) && SomeColumn.bIsVisible;
		}
	}
	return false;
}

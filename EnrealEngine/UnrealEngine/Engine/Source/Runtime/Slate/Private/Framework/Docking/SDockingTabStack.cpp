...
{
	const FTabManager::FTab* Tab = Tabs.FindByPredicate(FTabMatcher(InTab->GetLayoutIdentifier()));
	if (Tab)
	{
		return Tab->bIsLockedInPlace.Get(false);
	}

	return false;
}

void SDockingTabStack::SetTabLocked(const TSharedRef<const SDockTab>& InTab, TOptional<bool> bInLocked)
{
	FTabManager::FTab* Tab = Tabs.FindByPredicate(FTabMatcher(InTab->GetLayoutIdentifier()));
	if (Tab)
	{
		Tab->bIsLockedInPlace = bInLocked;
	}
}
...
bool SDockingTabStack::IsTabAllowedInSidebar(TSharedPtr<SDockTab> Tab) const
{
	...
	if (Tab.IsValid())
	{
		...
		{
			return !Tab->IsTabLocked() && Tab->GetVisualTabRole() != ETabRole::MajorTab && TabManager->IsTabAllowedInSidebar(Tab->GetLayoutIdentifier());
		}
	}
	return false;
}

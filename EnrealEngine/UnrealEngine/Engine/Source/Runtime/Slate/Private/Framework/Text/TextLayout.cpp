...
FTextLayout::FBreakCandidate FTextLayout::CreateBreakCandidate( int32& OutRunIndex, FLineModel& Line, int32 PreviousBreak, int32 CurrentBreak )
{
	...
	for (; OutRunIndex < Line.Runs.Num(); OutRunIndex++)
	{
		...
		while( WhitespaceStopIndex > BeginIndex && FTextChar::IsWhitespace( (*Line.Text)[ WhitespaceStopIndex - 1 ] ) )
		{
			--WhitespaceStopIndex;
		}
...
void FTextLayout::CreateLineWrappingCache(FLineModel& LineModel)
{
	...
	{
		LineBreakIterator = GSlateUseSharedBreakIterator  ? FBreakIterator::GetLineBreakIterator() : FBreakIterator::CreateLineBreakIterator();
	}
...
FTextLayout::FTextLayout()
	...
	, WordBreakIterator(GSlateUseSharedBreakIterator  ? FBreakIterator::GetWordBreakIterator() : FBreakIterator::CreateWordBreakIterator())
...
FTextSelection FTextLayout::GetWordAt(const FTextLocation& Location) const
{
	...
	while ((CurrentBreak = WordBreakIterator->MoveToPrevious()) != INDEX_NONE)
	{
		...
		{
			if (!FTextChar::IsWhitespace((*LineModel.Text)[Index]))
			{
				HasLetter = true;
				break;
			}

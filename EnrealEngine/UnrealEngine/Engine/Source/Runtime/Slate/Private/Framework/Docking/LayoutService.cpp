...
{
	SaveSectionToConfig(InConfigFileName, InSectionName, InSectionValue.ToString());
}

void FLayoutSaveRestore::SaveSectionToConfig(const FString& InConfigFileName, const FString& InSectionName, const FString& InSectionValue)
{
	GConfig->SetString(GetEditorLayoutsSectionName(), *InSectionName, *InSectionValue, InConfigFileName);
	...
	if (JsonObject.IsValid())
	{
		JsonObject->SetStringField(InSectionName, *InSectionValue);
...
bool FLayoutSaveRestore::DuplicateConfig(const FString& SourceConfigFileName, const FString& TargetConfigFileName)
{
	...
	// The right layout config to be duplicated is already in a json file
	// Let's load it and use that as source for duplication
	if (TSharedPtr<FJsonObject> SourceJson = LoadJsonFile(GetLayoutJsonFileName(SourceConfigFileName)))
	{
		RootObject = SourceJson;
	}
	else
	{
		// If we end up here, there is no json file for the specified ini layout:
		// convert this layout to a JSON file
		TArray<FString> SectionPairs;
		GetConfigSection(SectionPairs, TargetConfigFileName);
		RootObject = ConvertSectionToJson(SectionPairs);
	}

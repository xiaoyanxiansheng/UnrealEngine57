...
FDockingDragOperation::FDockingDragOperation( const TSharedRef<SDockTab>& InTabToBeDragged, const FVector2D InTabGrabOffsetFraction, TSharedRef<class SDockingArea> InTabOwnerArea, const FVector2D& OwnerAreaSize )
	...
	, TabStackOfOrigin(InTabToBeDragged->GetParent() ? InTabToBeDragged->GetParent()->GetParentDockTabStack() : nullptr)
	, HoveredTabPanelPtr( )
	, HoveredDockTarget()
	, LastContentSize( OwnerAreaSize )
{
	...
	if (!TabStackOfOrigin.IsValid())
	{
		// No parent, open the window right away
		CursorDecoratorWindow->Resize(OwnerAreaSize);
		CursorDecoratorWindow->ShowWindow();
		CursorDecoratorWindow->ReshapeWindow(FSlateRect(FVector2D(0.0, 0.0), OwnerAreaSize));

...
#include UE_INLINE_GENERATED_CPP_BY_NAME(SlateApplication)
...
struct FScopedNavigationTransition
{
	FScopedNavigationTransition(EUINavigation NavigationType, TSharedPtr<FSlateUser> User)
		...
		, User(User)
	{
		OldFocusedWidget = GetFocusedWidget();
	}

	~FScopedNavigationTransition()
	{
		NewFocusedWidget = GetFocusedWidget();

		ExecuteNavigationTransition(OldFocusedWidget, ENavigationTransitionDirection::Outgoing);
		ExecuteNavigationTransition(NewFocusedWidget, ENavigationTransitionDirection::Incoming);
	}

	TWeakPtr<SWidget> GetFocusedWidget()
	{
		if (User.IsValid() && User->GetWeakFocusPath().IsValid())
		{
			return User->GetWeakFocusPath().GetLastWidget().Pin();
		}
		return nullptr;
	}
	...
	void ExecuteNavigationTransition(TWeakPtr<SWidget> Widget, const ENavigationTransitionDirection& Direction)
	{
		...
		// Notify widget hierarchy of navigation event
		while(Widget.IsValid())
		{
			TSharedPtr<SWidget> PinnedWidget = Widget.Pin();
			if (TSharedPtr<FNavigationTransitionMetadata> Metadata = PinnedWidget->GetMetaData<FNavigationTransitionMetadata>())
			{
				const FNavigationTransition Transition(NavigationType, OldFocusedWidget, NewFocusedWidget, Direction);
				Metadata->OnNavigationTransition.ExecuteIfBound(Transition);
			}
			Widget = PinnedWidget->GetParentWidget();
		}
	}
...
void FSlateApplication::ProcessReply( const FWidgetPath& CurrentEventPath, const FReply& TheReply, const FWidgetPath* WidgetsUnderMouse, const FPointerEvent* InMouseEvent, const uint32 UserIndex )
{
	...
	if ((bDisableLastDragOnDragEnter && bStartingDragDrop && SlateUser->IsDragDropping()) || TheReply.ShouldEndDragDrop())
	{
	...
	if (bHandleDeviceInputWhenApplicationNotActive || bAppIsActive || bIsVirtualInteraction)
	{
		...
		if ( !bIsVirtualInteraction && CurrentEventPath.IsValid() && RequestedMouseCaptor.IsValid())
		{
			...
			if (TheReply.ShouldUseHighPrecisionMouse() && !PlatformApplication->IsUsingHighPrecisionMouseMode())
			{
	...
	if (TheReply.GetNavigationDestination().IsValid() || TheReply.GetNavigationType() != EUINavigation::Invalid)
	{
		...
		if (NavigationSource.IsValid())
		{
			...
			{
				FScopedNavigationTransition ScopedNavigationTransition(TheReply.GetNavigationType(), SlateUser);
...
void FSlateApplication::EnterDebuggingMode()
{
	...
	auto AddNotification = [Self=this](const FText& SubText)
	{
		...
		if (bDoDebugBreak)
		{
			//The application was not able to enter debugging mode.
			//The user will see a notification, but Blueprint breakpoint will not stop the execution.
			//We detected that the debugging mode would prevent the editor from ticking.
			//Entering debugging mode could make the application unresponsive or corrupt the application's message stack.
			// 
			//This can occur when you are debugging an editor blueprint that is executed inside a modal.
			//ie.
			// Engine::Tick
			//   RenamePrompt. ShowModal that blocks execution until the user presses a button.
			//    Inside the rename modal window, code executes that triggers a blueprint.
			// 
			//This can occur when you are debugging a Widget that is painting.
			//Game widgets are sadly included in the editor paint. It is related to how Slate rendering works.
			//ie.
			// World::Tick (no problem here)
			// SlateApplication::TickAndPaint
			//  SWindow::Paint // paint the editor
			//    SMyWidget::Paint (includes OnPaint, Tick, and other callbacks).
			UE_DEBUG_BREAK();
		}
	};
...
{
	bDisableLastDragOnDragEnter = bInDisableLastDragOnDragEnter;
}

bool FSlateApplication::IsLastDragOnDragEnterDisabled() const
{
	return bDisableLastDragOnDragEnter;
}
...
void FSlateApplication::ShowVirtualKeyboard( bool bShow, int32 UserIndex, TSharedPtr<IVirtualKeyboardEntry> TextEntryWidget )
{
	...
	{
		SlateTextField = IPlatformTextFieldFactory::TryCreateInstance();
	}
...
void FSlateApplication::NavigateToWidget(const uint32 UserIndex, const TSharedPtr<SWidget>& NavigationDestination, ENavigationSource NavigationSource, EUINavigation NavigationType)
{
...
bool FSlateApplication::AttemptNavigation(const FWidgetPath& NavigationSource, const FNavigationEvent& NavigationEvent, const FNavigationReply& NavigationReply, const FArrangedWidget& BoundaryWidget)
{
	...
	FScopedNavigationTransition ScopedNavigationTransition(NavigationType, GetUser(NavigationEvent.GetUserIndex()));

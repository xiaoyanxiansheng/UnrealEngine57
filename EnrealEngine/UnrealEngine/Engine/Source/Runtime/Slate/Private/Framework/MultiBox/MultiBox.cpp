	...
	for (const TWeakPtr<SWidget>& WeakClippedWidget : ClippedWidgets)
	{
		...
		for (const TPair<TSharedPtr<SWidget>, TSharedPtr<const FMultiBlock>>& Pair : FinalMultiBoxWidgets)
		{
			if (ClippedWidget == Pair.Key)
			{
				...
				// Entries that are followed by simple combo boxes have both widgets merged together to create a single button with an "options" dropdown.
				// When overflowing, the primary button of this pair needs to handle placing this consumed child.
				const TArray<TSharedRef<const FMultiBlock>>& Blocks = GetMultiBox()->GetBlocks();
				int32 NextBlockIndex = Blocks.Find(Pair.Value.ToSharedRef()) + 1;
				if (NextBlockIndex < Blocks.Num())
				{
					const TSharedRef<const FMultiBlock>& NextBlock = Blocks[NextBlockIndex];
					if (NextBlock->GetType() == EMultiBlockType::ToolBarComboButton)
					{
						...
						if (NextToolBarComboButtonBlock->IsSimpleComboBox())
						{
							NextBlock->CreateMenuEntry(MenuBuilder);
						}
					}
				}

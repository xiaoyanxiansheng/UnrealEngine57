...
void SToolBarButtonBlock::BuildMultiBlockWidget(const ISlateStyle* StyleSet, const FName& StyleName)
{
	...
	if (ToolBarButtonBlock->CustomWidget)
	{
		ButtonContent = ToolBarButtonBlock->CustomWidget.ToSharedRef();
	}
	else if (MultiBox->GetType() == EMultiBoxType::SlimHorizontalToolBar 
		...
		|| bIsSlimWrappingToolBar)
	{
		...
		.Padding(ToolBarStyle.LabelPadding)
		.VAlign(VAlign_Center))
		.MaxWidth(ToolBarStyle.ButtonTextMaxWidth)
		.MinWidth(ToolBarStyle.ButtonTextMinWidth)
		[ TextBlock ];
	}
	else
	{

...
public:
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(STreeView::Tick);
	...
	bool PopulateLinearizedItems(
		...
		int32 ParentIndex)
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(STreeView::PopulateLinearizedItems);
		...
		for ( int32 ItemIndex = 0; ItemIndex < InItemsSource.Num(); ++ItemIndex )
		{
			...
			{
				TRACE_CPUPROFILER_EVENT_SCOPE(STreeView_GetChildren);

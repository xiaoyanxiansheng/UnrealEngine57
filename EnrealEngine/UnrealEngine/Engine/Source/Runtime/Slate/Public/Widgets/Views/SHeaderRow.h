...
template <>
struct TWidgetTypeTraits<class SHeaderRow>
{
...
public:
	...
	public:
		...
		SLATE_BEGIN_ARGS(FColumn)
			...
			/** Set the minimum column size when resizing. */
			SLATE_ARGUMENT(float, MinSize)
			...
			SLATE_ATTRIBUTE( bool, CanManuallySort)
			SLATE_EVENT( FOnSortModeChanged, OnSort )

			SLATE_ATTRIBUTE(bool, ShouldGenerateWidget)
			SLATE_ATTRIBUTE(bool, ShouldGenerateEmptyWidgetForSpacing)
		...
		FColumn( const FArguments& InArgs )
			...
			, CanManuallySort(InArgs._CanManuallySort)
			, OnSortModeChanged( InArgs._OnSort )
			, ShouldGenerateWidget(InArgs._ShouldGenerateWidget)
			, ShouldGenerateEmptyWidgetForSpacing(InArgs._ShouldGenerateEmptyWidgetForSpacing)
			, bIsVisible(true)
			{
	...
	/** @return if we should generate this column even if empty, some users want to skip this to allow for large headers */

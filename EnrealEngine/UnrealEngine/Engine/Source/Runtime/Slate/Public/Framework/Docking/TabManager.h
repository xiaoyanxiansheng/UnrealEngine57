...
{
	inline bool IsHomeScreenEnabled()
	{
		static bool bIsHomeScreenEnabled = false;
		UE_CALL_ONCE([&]()
		{
			if (const IConsoleVariable* CVar = IConsoleManager::Get().FindConsoleVariable(TEXT("HomeScreen.EnableHomeScreen")))
			{
				ensureAlwaysMsgf(!EnumHasAnyFlags(CVar->GetFlags(), ECVF_Default), TEXT("The CVar should have already been set from commandline."));
				bIsHomeScreenEnabled = CVar->GetBool();
			}
		});
		return bIsHomeScreenEnabled;
	}
}
...
struct FTabSpawnerEntry : public FWorkspaceItem
{
	FTabSpawnerEntry(const FName& InTabType, const FOnSpawnTab& InSpawnTabMethod, const FCanSpawnTab& InCanSpawnTab)
		...
		, bCanSidebarTab(true)
		, bHiddenTabName(false)
		, SpawnedTabPtr()
		, ReadOnlyBehavior(ETabReadOnlyBehavior::Disabled)
	{
		// Special treatment for the HomeScreen tab to not have its name displayed and to be locked, this is currently intended to not be a public feature
		if (InTabType == FName(TEXT("HomeScreen")))
		{
			bHiddenTabName = true;
			bIsLocked = true;
		}

		// Special treatment for the LevelEditor tab to be locked, this is currently intended to not be a public feature
		if (InTabType == FName(TEXT("LevelEditor")))
		{
			bIsLocked = true;
		}
	}
...
private:
	...
	bool bCanSidebarTab;
	/** Whether the tab name should be hidden */
	bool bHiddenTabName;
	/** Whether the tab default state should be locked or not */
	TOptional<bool> bIsLocked;
...
class FTabManager : public TSharedFromThis<FTabManager>
{
	...
	public:
		...
		struct FTab
		{
			FTab(const FTabId& InTabId, ETabState::Type InTabState)
				...
				, bIsLockedInPlace(TOptional<bool>())
			{
		...
		{
			float MainContentCoefficient = 0.75f;
			float PanelDrawerCoefficient = 0.25f;
		};
		...
		class FArea : public FSplitter
		{
			...
			public:
				...
				{
					ActivePanelDrawerTab = InPanelDrawerTab;
					return SharedThis(this);
				}
				...
				{
					InactivePanelDrawerTabs.Add(InPanelDrawerTab);
					return SharedThis(this);
				}
				...
				{
					return InactivePanelDrawerTabs;
				}
	...
	public:
		...
		/**
		 * Try to open tab in the PanelDrawer. If it already exists elsewhere, it will draw attention to the tab.
		 * If the tab is open in another PanelDrawer it will transfer it to this drawer
		 * If no PanelDrawer is available in the manager we will just open a new standard tab
		 */
		UE_EXPERIMENTAL(5.7, "The panel drawer is still in development and might be removed. The api, implementation details and its form may change in near releases.")
		...
		/**
		 * Try to open tab in the PanelDrawer. If it already exists elsewhere, it will draw attention to the tab.
		 * If the PanelDrawer is already open with the tab it will close the drawer
		 * If the tab is open in another PanelDrawer it will transfer it to this drawer
		 * If no PanelDrawer is available in the manager we will just open a new standard tab
		 */
		UE_EXPERIMENTAL(5.7, "The panel drawer is still in development and might be removed. The api, implementation details and its form may change in near releases.")
...
PRAGMA_DISABLE_EXPERIMENTAL_WARNINGS
		UE_EXPERIMENTAL(5.7, "The panel drawer is still in development and might be removed. The api, implementation details and its form may change in near releases.")
		SLATE_API FDelegateHandle RegisterOnPanelDrawerStateChanges(FOnPanelDrawerStateChanged::FDelegate&& InDelegate);
PRAGMA_ENABLE_EXPERIMENTAL_WARNINGS

		UE_EXPERIMENTAL(5.7, "The panel drawer is still in devel1opment and might be removed. The api, implementation details and its form may change in near releases.")
		SLATE_API void UnregisterOnPanelDrawerStateChanges(FDelegateHandle InHandle);
...
protected:
		...
		/*
		 * Search the docking area to see if we have one that is part of the window.
		 */
		TSharedPtr<SDockingArea> GetDockingAreaForWindow(const TSharedRef<SWindow>& InWindow) const;

		/*
		 * Search the docking area to see if we have one that is part of the window. With some customized Logic for the panel drawer.
		 */
		TSharedPtr<SDockingArea> GetDockingAreaForPanelDrawer(const TSharedPtr<SWindow>& InWindow) const;
	...
	/** If the HomeScreen tab is enabled this will force the primary area to contain the HomeScreen and the LevelEditor next to each other otherwise it will just force the LevelEditor to be the first tab */
	void FixLayoutLoadingPrimaryArea(const TSharedRef<FArea>& InPrimaryArea);
	...
	/** Based on if the tab can have a hidden display name it will return the correct Label to use for the tab */
	FText GetTabLabelBasedOnSpawner(const TSharedPtr<FTabSpawnerEntry>& InSpawnerEntry) const;

	/** Does the shared logic between InvokeTab_Internal and InvokeTabInPanelDrawer */
	bool InvokeTab_CanInvokeTab(const FTabId& TabId) const;
	TSharedPtr<SDockTab> InvokeTab_FindOrReuseExistingTab(const FTabId& TabId) const;
	void InvokeTab_DrawAttentionToTab(const TSharedPtr<SDockTab>& InvokedTab);

	TSharedPtr<SDockTab> InvokeTabInPanelDrawer(const FTabId& TabId, bool bToggleIfActiveInDrawer, const TSharedPtr<SWindow>& ParentWindow, bool bForceInDrawerPanel);

	/** Check if the tab can be invoked in a PanelDrawer, also return the tab since we need it to do some test */
	bool CanInvokeInPanelDrawer(const FTabId& TabId, const TSharedPtr<SDockingArea>& DockingArea, TSharedPtr<SDockTab>& OutTab, bool bForceInDrawerPanel);

	/** Find the right area to host the tab and setup its PanelDrawer to host the tab */
	void SetTabInPanelDrawer(const TSharedPtr<SDockTab>& InTab, const TSharedPtr<SDockingArea>& TargetDockingArea, const TSharedPtr<SWindow>& InWindow);

	/** Broadcasted when a PanelDrawer is opened or closed */
	void OnPanelDrawerStateChanged() const;

	/** Remove the tab from keep alive list of the PanelDrawers. Return true if found in the PanelDrawer keep alive */
	bool RemoveFromHiddenPanelDrawerTabs(TSharedPtr<SDockTab> ExistingTab);

	/** Transfer the KeepAlive tab to TabManager and clean the PanelDrawer data */
	void HandleClosingAreaPanelDrawerData(const TSharedPtr<SDockingArea>& ClosingDockingArea);
...
PRAGMA_DISABLE_EXPERIMENTAL_WARNINGS
		...
		FOnPanelDrawerStateChanged OnPanelDrawerStateChangedDelegate;
PRAGMA_ENABLE_EXPERIMENTAL_WARNINGS

		/** Keep alive the PanelDrawer tabs that have no area */
		TMap<FTabId, TSharedRef<SDockTab>> PanelDrawerActiveHiddenTabs;
};
...
private:
	...
	using FTabAndManagerForDrawer = TPair<TSharedPtr<SDockTab>, TSharedPtr<FTabManager>>;
	void RelocatePanelDrawerNomadTab(FTabAndManagerForDrawer ForegroundTabAndSubManager, FTabAndManagerForDrawer BackgroundedTabAndSubManager);

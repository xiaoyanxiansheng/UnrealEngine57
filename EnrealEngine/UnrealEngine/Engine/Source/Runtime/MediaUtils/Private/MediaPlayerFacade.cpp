...
void FMediaPlayerFacade::FBlockOnRange::SetRange(const TRange<FTimespan>& NewRange)
{
	...
	{
		if (NewRange.HasLowerBound() && NewRange.HasUpperBound() && NewRange.GetLowerBoundValue().GetTicks() <= NewRange.GetUpperBoundValue().GetTicks())
		{
...
bool FMediaPlayerFacade::FBlockOnRange::IsSet() const
{
	...
	return !CurrentTimeRange.IsEmpty() && CurrentPlayer.IsValid() && CurrentPlayer->GetControls().CanControl(EMediaControl::BlockOnFetch);
}
...
const TRange<FMediaTimeStamp>& FMediaPlayerFacade::FBlockOnRange::GetRange() const
{
	...
	if (!RangeIsDirty || !CurrentPlayer.IsValid())
	{
		return BlockOnRange;
	}

	// If the range is empty or the player can't support blocked playback or the range interval is invalid: reset everything & return empty block range...
	if (CurrentTimeRange.IsEmpty() || !CurrentPlayer->GetControls().CanControl(EMediaControl::BlockOnFetch) ||
		CurrentTimeRange.GetLowerBoundValue().GetTicks() > CurrentTimeRange.GetUpperBoundValue().GetTicks() ||
		CurrentPlayer->GetControls().GetDuration().GetTicks() <= 0)
	{
		LastProcessedTimeRange = TRange<FTimespan>::Empty();
		...
		if (Player->GetPlayerFeatureFlag(IMediaPlayer::EFeatureFlag::UsePlaybackTimingV2))
		{
			...
			while(1)
			{
				...
				if ((FPlatformTime::Seconds() - BlockingStart) > static_cast<double>(UE::MediaUtils::Private::GBlockOnFetchTimeout))
				{
					...
					Url = Player->GetUrl();
#endif // !UE_BUILD_SHIPPING
					UE_LOG(LogMediaUtils, Error, TEXT("Blocking media playback timed out on range [%s (seq:%d, loop:%d), %s (seq:%d, loop:%d)]. Disabling it for this playback session. URL:%s"),
						*TimeRange.GetLowerBoundValue().Time.ToString(TEXT("%h:%m:%s.%t")), TimeRange.GetLowerBoundValue().GetSequenceIndex(), TimeRange.GetLowerBoundValue().GetLoopIndex(),
						*TimeRange.GetUpperBoundValue().Time.ToString(TEXT("%h:%m:%s.%t")), TimeRange.GetUpperBoundValue().GetSequenceIndex(), TimeRange.GetUpperBoundValue().GetLoopIndex(),
						*Url);
					BlockOnRangeDisabled = true;
					break;
				}
...
void FMediaPlayerFacade::TickTickable()
{
	...
	{
		FScopeLock Lock1(&TimeValueLock);
	...
	if (HaveVideoPlayback())
	{
		if (IsSeeking())
		{
			...
			if (VideoTimeStamp.IsValid() && VideoTimeStamp.GetSequenceIndex() >= NextSequenceIndex.Get(0))
			{
				...
				FScopeLock LockLT(&TimeValueLock);
...
{
	TimeValueLock.Lock();
	auto nst = NextSeekTime;
	TimeValueLock.Unlock();

	if (nst.IsSet() && !IsSeeking())
	{
		UE_LOG(LogMediaUtils, Verbose, TEXT("PlayerFacade: Internal Seek at %s."), *nst.GetValue().ToString(TEXT("%h:%m:%s.%t")));

		if (!Seek(nst.GetValue()))
		{
			UE_LOG(LogMediaUtils, Error, TEXT("PlayerFacade: Failed Internal Seek at %s."), *nst.GetValue().ToString(TEXT("%h:%m:%s.%t")));
		}
	}
}
...
bool FMediaPlayerFacade::IsVideoSampleStillGood(const TRange<FMediaTimeStamp>& LastSampleTimeRange, const TRange<FMediaTimeStamp>& TimeRange, bool bReverse) const
{
	...
	if (!TimeRange.IsEmpty() && !IsSeeking() && !LastSampleTimeRange.IsEmpty())
	{
...
#if !NO_LOGGING
		if (VideoSampleSinks.IsEmpty())
		{
			UE_LOG(LogMediaUtils, Verbose, TEXT("PlayerFacade: No Video Sample Sinks available, Dropping Sample %s Sequence %d Loop %d"),
				*SampleTime.GetTime().ToString(TEXT("%h:%m:%s.%t")), SampleTime.GetSequenceIndex(), SampleTime.GetLoopIndex());
		}
		else
		{
			UE_LOG(LogMediaUtils, VeryVerbose, TEXT("PlayerFacade: Enqueuing Sample %s Sequence %d Loop %d"),
				*SampleTime.GetTime().ToString(TEXT("%h:%m:%s.%t")), SampleTime.GetSequenceIndex(), SampleTime.GetLoopIndex());
		}
#endif

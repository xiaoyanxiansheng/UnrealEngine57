...
FText FAndroidErrorReport::DiagnoseReport() const
{
	auto MergeIntoReport = [](const FString & MergeFilePathName, FMergeInfo::EMergeType MergeType)
	{
		...
		{
			bool bSuccess = false;
			FXmlNode* Root = MergeNode.GetRootNode();
			if(ensure(Root))
			{
				switch (MergeType)
				{
					...
					{
						if (Root->GetTag().Contains(TEXT("Threads")))
						{
							FPrimaryCrashProperties::Get()->Threads = Root;
							bSuccess = true;
						}
						break;
					}
					...
					{
						FPrimaryCrashProperties::Get()->PlatformPropertiesExtras = Root;
						bSuccess = true;
						break;
					}
				}
			...
			if( bSuccess ) // Keep unmerged data for inspection.
			{

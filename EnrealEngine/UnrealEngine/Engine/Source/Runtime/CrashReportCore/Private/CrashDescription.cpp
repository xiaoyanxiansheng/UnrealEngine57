...
{
	// clear EpicAccountId if user doesn't want to be contacted
...
void FPrimaryCrashProperties::MakeCrashEventAttributes(TArray<FAnalyticsEventAttribute>& OutCrashAttributes)
{
	...
	if (XmlFile->IsValid())
	{
		// Add arbitrary platform specific data
		const FXmlNode* PlatformPropertiesNode = XmlFile->GetRootNode()->FindChildNode( FGenericCrashContext::PlatformPropertiesTag );
		if (PlatformPropertiesNode)
		{
			FString PlatformPropertiesAttributes;
			CrashDescriptionInternal::FAnalyticsJsonWriter JsonWriter(&PlatformPropertiesAttributes);
			JsonWriter.WriteObjectStart();
			for (const FXmlNode* ChildNode : PlatformPropertiesNode->GetChildrenNodes())
			{
				JsonWriter.WriteValue(ChildNode->GetTag(), ChildNode->GetContent());
			}
			JsonWriter.WriteObjectEnd();
			JsonWriter.Close();
			OutCrashAttributes.Add(FAnalyticsEventAttribute(TEXT("PlatformProperties"), FJsonFragment(MoveTemp(PlatformPropertiesAttributes))));
		}

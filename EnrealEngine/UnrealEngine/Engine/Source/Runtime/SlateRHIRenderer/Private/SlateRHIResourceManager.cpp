...
FSlateShaderResource* FSlateRHIResourceManager::GetFontShaderResource( int32 InTextureAtlasIndex, FSlateShaderResource* FontTextureAtlas, const class UObject* FontMaterial )
{
	...
	else if (!FontMaterial->IsA<UMaterialInterface>())
	{
#if SLATE_CHECK_UOBJECT_RENDER_RESOURCES
		FString ErrorMsg = FString::Format(TEXT("Cannot render object '{0}' as a material because it's of type {1} instead of type MaterialInterface. Make sure all materials passed for rendering in slate are Material Interfaces. This is caused by bad user data (a UObject that is not a UMaterialInterface) passed for rendering."), { FontMaterial->GetFullName(), FontMaterial->GetClass()->GetName() });

		if (GSlateCheckUObjectRenderResourcesShouldLogFatal)
		{
			UE_LOG(LogSlate, Fatal, TEXT("%s"), *ErrorMsg);
		}
		else
		{
		...
		if ( UObject* ResourceObject = InBrush.GetResourceObject() )
		{
			...
			else
			{
				...
				if ( !FailedTextures.Contains(ResourceObject) )
				{
					...
					{
						ensureMsgf(false, TEXT("Slate RHI Error - Invalid Texture2D, ResourceName: '%s', Path: '%s', Class: '%s'"),
					...
					{
						ensureMsgf(false, TEXT("Slate RHI Error - Invalid Texture2D, ResourceName: '%s'. UObject not valid."), *ResourceName.ToString());
					}
				}

...
public:
	...
	{
		Publish(Message, MessageType::StaticStruct(), EMessageScope::Network, Annotations, FTimespan::Zero(), FDateTime::MaxValue());
	}
	...
	{
		Publish(Message, MessageType::StaticStruct(), Scope, Annotations, FTimespan::Zero(), FDateTime::MaxValue());
	}
	...
	{
		Publish(Message, MessageType::StaticStruct(), Scope, Annotations, Delay, Expiration);
	}
...
protected:
	...
	*/
	 ...
	 */
	inline TSharedPtr<IMessageBus, ESPMode::ThreadSafe> GetBusIfEnabled() const
	{
		if (Enabled)
		{
			return BusPtr.Pin();

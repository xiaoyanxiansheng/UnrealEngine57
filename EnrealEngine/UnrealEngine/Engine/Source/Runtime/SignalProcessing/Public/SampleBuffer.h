...
namespace Audio
{
	...
	public:
		...
		inline TSampleBuffer(const TSampleBuffer& Other)
		{
		...
		inline TSampleBuffer(const FAlignedFloatBuffer& InData, int32 InNumChannels, int32 InSampleRate)
		{
		...
		inline TSampleBuffer(const int16* InBufferPtr, int32 InNumSamples, int32 InNumChannels, int32 InSampleRate)
		{
			NumSamples = InNumSamples;
			NumFrames = NumSamples / InNumChannels;
			NumChannels = InNumChannels;
		...
		inline const SampleType* GetData() const
		{
			return RawPCMData.GetData();
		}

		inline TArrayView<SampleType> GetArrayView()
		{
			return MakeArrayView(RawPCMData);
		}

		inline TArrayView<const SampleType> GetArrayView() const
		{
			return MakeArrayView(RawPCMData);
		}
		...
		inline int32 GetSampleRate() const
		{
			return SampleRate;
		}

		inline float GetSampleDuration() const
		{

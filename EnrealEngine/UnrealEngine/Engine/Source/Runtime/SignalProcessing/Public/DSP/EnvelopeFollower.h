...
namespace Audio
{
	...
	public:
		...
		inline bool GetAnalog() const { return bIsAnalog; }

		/** Get the attack time in samples. */
		inline float GetAttackTimeSamples() const { return AttackTimeSamples; }

		/** Get the release time in samples. */
		inline float GetReleaseTimeSamples() const { return ReleaseTimeSamples; }

		/** Get the attack time in milliseconds. */
		inline float GetAttackTimeMsec() const { return AttackTimeMsec; }

		/** Get the release time in milliseconds. */
		inline float GetReleaseTimeMsec() const { return ReleaseTimeMsec; }
	...
	public:
		...
		inline void ProcessBuffer(const float* InSamples, const int32 InNumSamples, float* OutSamples)
		{
	...
	private:
		inline float NormalizeSample(float InSample)
		{
			switch (Mode)
			{
				case EPeakMode::Peak:
		...
		inline void NormalizeSamples(const float* InSamples, const int32 InNumSamples, float* OutSamples)
		{

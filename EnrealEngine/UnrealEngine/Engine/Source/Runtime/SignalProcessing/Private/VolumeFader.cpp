...
namespace Audio
{
	...
	float FVolumeFader::AlphaToVolume(float InAlpha, EFaderCurve InCurve, float InStartingValue, float InTarget)
	{
		switch (InCurve)
		{
		...
		{
			const float NormalizedAlpha = FMath::GetMappedRangeValueClamped(FVector2f(InStartingValue, InTarget), FVector2f(0.0f, 1.0f), InAlpha);
			float Volume = 0.5f * Audio::FastSin(PI * NormalizedAlpha - HALF_PI) + 0.5f;
			Volume = FMath::GetMappedRangeValueClamped(FVector2f(0.0f, 1.0f), FVector2f(InStartingValue, InTarget), Volume);
		...
		{
			const float NormalizedAlpha = FMath::GetMappedRangeValueClamped(FVector2f(InStartingValue, InTarget), FVector2f(0.0f, 1.0f), InAlpha);
			float Volume = Audio::FastSin(HALF_PI * NormalizedAlpha);
			Volume = FMath::GetMappedRangeValueClamped(FVector2f(0.0f, 1.0f), FVector2f(InStartingValue, InTarget), Volume);
			return FMath::Max(0.0f, Volume);
		}
	...
	{
		return AlphaToVolume(Alpha, FadeCurve, StartingValue, Target);
	}
	...
	float FVolumeFader::GetVolumeAfterTime(float InDeltaTime) const
	{
		...
		return AlphaToVolume(FutureAlpha, FadeCurve, StartingValue, Target);
	}

...
public:
	...
	inline const ProcessedPacket Incoming(uint8* Packet, int32 CountBytes)
	{
	 ...
	 */
	inline EIncomingResult Incoming(FReceivedPacketView& PacketView)
	{
	 ...
	 */
	inline const ProcessedPacket Outgoing(uint8* Packet, int32 CountBits, FOutPacketTraits& Traits)
	{
		const TSharedPtr<const FInternetAddr> EmptyAddress = nullptr;
	...
	inline const ProcessedPacket IncomingConnectionless(const TSharedPtr<const FInternetAddr>& Address, uint8* Packet, int32 CountBytes)
	{
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
	 ...
	 * @return				Returns Success/Failure
	 */
	inline EIncomingResult IncomingConnectionless(FReceivedPacketView& PacketView)
	{
	 ...
	 * @return				Returns the final packet
	 */
	inline const ProcessedPacket OutgoingConnectionless(const TSharedPtr<const FInternetAddr>& Address, uint8* Packet, int32 CountBits, FOutPacketTraits& Traits)
	{
...
public:
	 ...
	 */
	inline void QueuePacketForRawSending(BufferedPacket* PacketToQueue)
	{
	 ...
	 * @param PacketToQueue		The packet to be queued
	 */
	inline void QueueHandlerPacketForSending(BufferedPacket* PacketToQueue)
	{
	...
	*/
	 ...
	 */
	inline void SetRawSend(bool bInEnabled)
	{
		bRawSend = bInEnabled;
	}
	 ...
	 */
	inline bool IsFullyInitialized()
	{

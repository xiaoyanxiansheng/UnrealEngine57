...
namespace UE::Interchange
{
		...
		void EndSetupAssetData(FFactoryCommon::FUpdateImportAssetDataParameters& Parameters, UInterchangeAssetImportData* AssetImportData)
		{
			...
			for (const UObject* Pipeline : Parameters.Pipelines)
			{
				...
				if(bPipelineSupportReimport)
				{
					...
					if (DupPipeline)
					{
						// CAUTION: Python Pipeline adds this flag to the Package in PostLoad, 
						// and StaticDuplicateObject calls PostLoad on the duplicated asset. But here as the Outer is AssetImportData
						// The package is the imported asset. This will only show references in the reference viewer when the mode is set to
						// Editor Only References. Hence removing that particular flag from the package.
						DupPipeline->GetPackage()->ClearPackageFlags(PKG_EditorOnly);
	...
	bool FFactoryCommon::CanProceedWithFactoryNode(UInterchangeFactoryBaseNode& FactoryNode, UObject* ReimportObject, UObject* ObjectToReimport, const FString& PackageName, const FString& AssetName, const FString& SubPathString)
	{
		// If ReimportObject is null, proceed with factory node
		if (!ReimportObject)
		{
			return true;
		}

		// Object associated with the factory node was not part of the previous import, proceed with factory node 
		if (!GetFactoryNode(ReimportObject, PackageName, AssetName, SubPathString))
		{
			return true;
		}

		// Associated object was not found (deleted or renamed) and factory node has been flagged as to be reimported. Proceed with factory node
		if (!ObjectToReimport && FactoryNode.ShouldForceNodeReimport())
		{
			return true;
		}

		// If ObjectToReimport is null but still tracked in the UInterchangeSceneImportAsset, it means that
		// the associated object was part of a previous import/reimport but deleted or renamed since.
		// The UInterchangeSceneImportAsset only tracks factory nodes with reference object.
		// Consequently keep track of it for future reimport.
		if (!ObjectToReimport)
		{
			const FTopLevelAssetPath AssetPath(*PackageName, *AssetName);
			FactoryNode.SetCustomReferenceObject(FSoftObjectPath(AssetPath, SubPathString));

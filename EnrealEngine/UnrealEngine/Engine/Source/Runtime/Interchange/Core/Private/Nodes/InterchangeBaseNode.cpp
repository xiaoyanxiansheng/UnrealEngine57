...
void UInterchangeBaseNode::InitializeNode(const FString& UniqueID, const FString& DisplayLabel, const EInterchangeNodeContainerType NodeContainerType)
{
	...
	if (!SetDisplayLabel(DisplayLabel))
	{
...
bool UInterchangeBaseNode::SetDisplayLabel(const FString& InDisplayLabel)
{
	//Having a non-FName-friendly display label (like "None" or "_123") will most certainly cause issues in the editor when creating any kind of asset.
	//It's preferable to be very defensive and avoid that value completely.
	//This can still be bypassed by using the Attributes system directly.
	FString Sanitized = UE::Interchange::MakeSanitizedName(InDisplayLabel);

	UE::Interchange::EAttributeStorageResult Result = Attributes->RegisterAttribute(UE::Interchange::FBaseNodeStaticData::DisplayLabelKey(), Sanitized);
	if (IsAttributeStorageResultSuccess(Result))
	{
	...
	do
	{
		...
		if (RestOfPropertyPath.Len() > 0)
		{
			...
			else if (FObjectProperty* ObjectProperty = CastField<FObjectProperty>(Property))
			{
				...
				if (Container.IsType<UObject*>())
				{
					DestObject = *Property->ContainerPtrToValuePtr<UObject*>(Container.Get<UObject*>());
				}
				else
				{
					DestObject = *Property->ContainerPtrToValuePtr<UObject*>(Container.Get<uint8*>());
				}
				Outer = DestObject->GetClass();
				Container.Set<UObject*>(DestObject);
			}
		}

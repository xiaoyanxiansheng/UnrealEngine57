...
namespace UE
{
	namespace Vulkan
	{
		...
		{
			return GetVulkanDevice()->GetHandle();
		}
...
// 		TArray<uint64> PSOsToBeFlushed;
		...
		uint64 GetPrecacheKey(const FVulkanGraphicsPipelineState* GraphicsPipelineState)
		{
...
class FVulkanChunkedPipelineCacheManagerImpl
{
	...
	using FVulkanPipelineCacheChunkLRUNode = UE::Vulkan::FVulkanPipelineCacheChunkLRUNode;
	using FVulkanGraphicsPipelineStateLRU = UE::Vulkan::FVulkanGraphicsPipelineStateLRU;

	UE::Vulkan::FVulkanGraphicsPipelineStateLRU CacheChunkLRU;
...
private:
	...
	static uint64 GetPrecacheHash(const FVulkanGraphicsPipelineState* GFXState) { return GFXState->PrecacheKey; }
...
template VkResult FVulkanChunkedPipelineCacheManager::CreatePSO(FVulkanGraphicsPipelineState* GraphicsPipelineState, bool bIsPrecompileJob, FPSOCreateCallbackFunc<FVulkanGraphicsPipelineState> PSOCreateFunc);

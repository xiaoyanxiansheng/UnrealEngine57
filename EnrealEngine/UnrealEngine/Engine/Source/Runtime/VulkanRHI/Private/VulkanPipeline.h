...
struct FGfxPipelineDesc
{
	...
	struct FVertexBinding
	{
		...
		void WriteInto(VkVertexInputBindingDescription2EXT& OutState) const;
	...
	struct FVertexAttribute
	{
		...
		void WriteInto(VkVertexInputAttributeDescription2EXT& OutState) const;
...
private:
	...
	bool CreateGfxPipelineFromEntry(FVulkanGraphicsPipelineState* PSO, FVulkanShader* Shaders[ShaderStage::NumGraphicsStages], FGraphicsPipelineStateInitializer::EPSOPrecacheCompileType PSOCompileType);
	bool CreateGfxPipelineFromLibrary(FVulkanGraphicsPipelineState& GraphicsPipelineState, FVulkanShader* Shaders[ShaderStage::NumGraphicsStages], FGraphicsPipelineStateInitializer::EPSOPrecacheCompileType PSOCompileType);

	void CreateVertexInputLibraries();
	VkPipeline VertexInputPipelineLibraryVS = VK_NULL_HANDLE;
	VkPipeline VertexInputPipelineLibraryMS = VK_NULL_HANDLE;

	void CreateEmptyFragmentStageLibrary();
	VkPipeline EmptyFragmentStageLibrary = VK_NULL_HANDLE;

	VkPipeline FindOrCreateFragmentOutputStateLibrary(const FVulkanGraphicsPipelineState& GraphicsPipelineState);
	TMap<uint32, VkPipeline> FragmentOutputStatePipelineLibraries;
	FCriticalSection FragmentOutputStateCS;

	VkResult CreateVKPipeline(FVulkanGraphicsPipelineState* PSO, FVulkanShader* Shaders[ShaderStage::NumGraphicsStages], const VkGraphicsPipelineCreateInfo& PipelineInfo, FGraphicsPipelineStateInitializer::EPSOPrecacheCompileType PSOCompileType);
	...
	void LRUAdd(FVulkanGraphicsPipelineState* PSO);
	void LRUTouch(FVulkanGraphicsPipelineState* PSO);
	bool LRUEvictOne(bool bOnlyOld = false);
	void LRURemoveAll();
	void LRUDump();
	void LRUDebugEvictAll(); //evict all that are safe to evict without stalling..
	void LRURemove(FVulkanGraphicsPipelineState* PSO);
	void LRUCheckNotInside(FVulkanGraphicsPipelineState* PSO);
	...
	TMap<FVulkanPSOKey, FVulkanGraphicsPipelineState*> GraphicsPSOLockedMap;

	
	FCriticalSection LRUCS;
	FVulkanGraphicsPipelineStateLRU LRU;
...
public:
	...
	FVulkanGraphicsPipelineStateLRUNode* LRUNode = nullptr;

	...
	VkDebugUtilsMessageTypeFlagsEXT MsgType, const VkDebugUtilsMessengerCallbackDataEXT* CallbackData, void* UserData)
{
	...
	if (!FCStringAnsi::Strcmp(CallbackData->pMessageIdName, "UNASSIGNED-CoreValidation-Shader-OutputNotConsumed") ||
		...
		!FCStringAnsi::Strcmp(CallbackData->pMessageIdName, "Undefined-Value-ShaderOutputNotConsumed-DynamicRendering"))
	{
...
void FWrapLayer::CmdBeginRenderingKHR(VkResult Result, VkCommandBuffer CommandBuffer, const VkRenderingInfo* RenderingInfo)
{
	if (Result == VK_RESULT_MAX_ENUM)
	{
#if VULKAN_ENABLE_DUMP_LAYER
		CmdPrintfBegin(CommandBuffer, TEXT("----- vkCmdBeginRenderingKHR()"));
#endif
	}
}

void FWrapLayer::CmdEndRenderingKHR(VkResult Result, VkCommandBuffer CommandBuffer)
{
	if (Result == VK_RESULT_MAX_ENUM)
	{
#if VULKAN_ENABLE_DUMP_LAYER
		CmdPrintfBegin(CommandBuffer, TEXT("----- vkCmdEndRenderingKHR()"));
#endif
	}
}
...
void FWrapLayer::CreateShadersEXT(VkResult Result, VkDevice Device, uint32_t CreateInfoCount, const VkShaderCreateInfoEXT* CreateInfos, const VkAllocationCallbacks* Allocator, VkShaderEXT* Shaders)
{
	if (Result == VK_RESULT_MAX_ENUM)
	{
#if VULKAN_ENABLE_DUMP_LAYER
		PrintfBeginResult(FString::Printf(TEXT("CreateShadersEXT()")));
#endif
	}
}

void FWrapLayer::DestroyShaderEXT(VkResult Result, VkDevice Device, VkShaderEXT Shader, const VkAllocationCallbacks* Allocator)
{
	if (Result == VK_RESULT_MAX_ENUM)
	{
#if VULKAN_ENABLE_DUMP_LAYER
		PrintfBeginResult(FString::Printf(TEXT("DestroyShaderEXT()")));
#endif
	}
}

void FWrapLayer::GetShaderBinaryDataEXT(VkResult Result, VkDevice Device, VkShaderEXT Shader, size_t* DataSize, void* Data)
{
	if (Result == VK_RESULT_MAX_ENUM)
	{
#if VULKAN_ENABLE_DUMP_LAYER
		PrintfBeginResult(FString::Printf(TEXT("GetShaderBinaryDataEXT()")));
#endif
	}
}

void FWrapLayer::CmdBindShadersEXT(VkResult Result, VkCommandBuffer CommandBuffer, uint32_t StageCount, const VkShaderStageFlagBits* Stages, const VkShaderEXT* Shaders)
{
	if (Result == VK_RESULT_MAX_ENUM)
	{
#if VULKAN_ENABLE_DUMP_LAYER
		PrintfBeginResult(FString::Printf(TEXT("CmdBindShadersEXT()")));
#endif
	}
}

void FWrapLayer::CmdBindVertexBuffers2EXT(VkResult Result, VkCommandBuffer CommandBuffer, uint32_t FirstBinding, uint32_t BindingCount, const VkBuffer* Buffers, const VkDeviceSize* Offsets, const VkDeviceSize* Sizes, const VkDeviceSize* Strides)
{
	if (Result == VK_RESULT_MAX_ENUM)
	{
#if VULKAN_ENABLE_DUMP_LAYER
		PrintfBeginResult(FString::Printf(TEXT("CmdBindVertexBuffers2EXT()")));
#endif
	}
}

void FWrapLayer::CmdSetCullModeEXT(VkResult Result, VkCommandBuffer CommandBuffer, VkCullModeFlags CullMode)
{
	if (Result == VK_RESULT_MAX_ENUM)
	{
#if VULKAN_ENABLE_DUMP_LAYER
		PrintfBeginResult(FString::Printf(TEXT("CmdSetCullModeEXT()")));
#endif
	}
}

void FWrapLayer::CmdSetColorBlendEnableEXT(VkResult Result, VkCommandBuffer CommandBuffer, uint32_t FirstAttachment, uint32_t AttachmentCount, const VkBool32* ColorBlendEnables)
{
	if (Result == VK_RESULT_MAX_ENUM)
	{
#if VULKAN_ENABLE_DUMP_LAYER
		PrintfBeginResult(FString::Printf(TEXT("CmdSetColorBlendEnableEXT()")));
#endif
	}
}

void FWrapLayer::CmdSetColorBlendEquationEXT(VkResult Result, VkCommandBuffer CommandBuffer, uint32_t FirstAttachment, uint32_t AttachmentCount, const VkColorBlendEquationEXT* ColorBlendEquations)
{
	if (Result == VK_RESULT_MAX_ENUM)
	{
#if VULKAN_ENABLE_DUMP_LAYER
		PrintfBeginResult(FString::Printf(TEXT("CmdSetColorBlendEquationEXT()")));
#endif
	}
}

void FWrapLayer::CmdSetColorWriteMaskEXT(VkResult Result, VkCommandBuffer CommandBuffer, uint32_t FirstAttachment, uint32_t AttachmentCount, const VkColorComponentFlags* ColorWriteMasks)
{
	if (Result == VK_RESULT_MAX_ENUM)
	{
#if VULKAN_ENABLE_DUMP_LAYER
		PrintfBeginResult(FString::Printf(TEXT("CmdSetColorWriteMaskEXT()")));
#endif
	}
}

void FWrapLayer::CmdSetFrontFaceEXT(VkResult Result, VkCommandBuffer CommandBuffer, VkFrontFace FrontFace)
{
	if (Result == VK_RESULT_MAX_ENUM)
	{
#if VULKAN_ENABLE_DUMP_LAYER
		PrintfBeginResult(FString::Printf(TEXT("CmdSetFrontFaceEXT()")));
#endif
	}
}

void FWrapLayer::CmdSetPolygonModeEXT(VkResult Result, VkCommandBuffer CommandBuffer, VkPolygonMode PolygonMode)
{
	if (Result == VK_RESULT_MAX_ENUM)
	{
#if VULKAN_ENABLE_DUMP_LAYER
		PrintfBeginResult(FString::Printf(TEXT("CmdSetPolygonModeEXT()")));
#endif
	}
}

void FWrapLayer::CmdSetPrimitiveTopologyEXT(VkResult Result, VkCommandBuffer CommandBuffer, VkPrimitiveTopology PrimitiveTopology)
{
	if (Result == VK_RESULT_MAX_ENUM)
	{
#if VULKAN_ENABLE_DUMP_LAYER
		PrintfBeginResult(FString::Printf(TEXT("CmdSetPrimitiveTopologyEXT()")));
#endif
	}
}

void FWrapLayer::CmdSetRasterizationSamplesEXT(VkResult Result, VkCommandBuffer CommandBuffer, VkSampleCountFlagBits RasterizationSamples)
{
	if (Result == VK_RESULT_MAX_ENUM)
	{
#if VULKAN_ENABLE_DUMP_LAYER
		PrintfBeginResult(FString::Printf(TEXT("CmdSetRasterizationSamplesEXT()")));
#endif
	}
}

void FWrapLayer::CmdSetScissorWithCountEXT(VkResult Result, VkCommandBuffer CommandBuffer, uint32_t ScissorCount, const VkRect2D* Scissors)
{
	if (Result == VK_RESULT_MAX_ENUM)
	{
#if VULKAN_ENABLE_DUMP_LAYER
		PrintfBeginResult(FString::Printf(TEXT("CmdSetScissorWithCountEXT()")));
#endif
	}
}

void FWrapLayer::CmdSetStencilOpEXT(VkResult Result, VkCommandBuffer CommandBuffer, VkStencilFaceFlags FaceMask, VkStencilOp FailOp, VkStencilOp passOp, VkStencilOp depthFailOp, VkCompareOp compareOp)
{
	if (Result == VK_RESULT_MAX_ENUM)
	{
#if VULKAN_ENABLE_DUMP_LAYER
		PrintfBeginResult(FString::Printf(TEXT("CmdSetStencilOpEXT()")));
#endif
	}
}

void FWrapLayer::CmdSetStencilTestEnableEXT(VkResult Result, VkCommandBuffer CommandBuffer, VkBool32 StencilTestEnable)
{
	if (Result == VK_RESULT_MAX_ENUM)
	{
#if VULKAN_ENABLE_DUMP_LAYER
		PrintfBeginResult(FString::Printf(TEXT("CmdSetStencilTestEnableEXT()")));
#endif
	}
}

void FWrapLayer::CmdSetVertexInputEXT(VkResult Result, VkCommandBuffer CommandBuffer, uint32_t VertexBindingDescriptionCount, const VkVertexInputBindingDescription2EXT* pVertexBindingDescriptions, uint32_t vertexAttributeDescriptionCount, const VkVertexInputAttributeDescription2EXT* pVertexAttributeDescriptions)
{
	if (Result == VK_RESULT_MAX_ENUM)
	{
#if VULKAN_ENABLE_DUMP_LAYER
		PrintfBeginResult(FString::Printf(TEXT("CmdSetVertexInputEXT()")));
#endif
	}
}

void FWrapLayer::CmdSetViewportWithCountEXT(VkResult Result, VkCommandBuffer CommandBuffer, uint32_t ViewportCount, const VkViewport* pViewports)
{
	if (Result == VK_RESULT_MAX_ENUM)
	{
#if VULKAN_ENABLE_DUMP_LAYER
		PrintfBeginResult(FString::Printf(TEXT("CmdSetViewportWithCountEXT()")));
#endif
	}
}

void FWrapLayer::CmdSetDepthBiasEnableEXT(VkResult Result, VkCommandBuffer CommandBuffer, VkBool32 DepthBiasEnable)
{
	if (Result == VK_RESULT_MAX_ENUM)
	{
#if VULKAN_ENABLE_DUMP_LAYER
		PrintfBeginResult(FString::Printf(TEXT("CmdSetDepthBiasEnableEXT()")));
#endif
	}
}

void FWrapLayer::CmdSetDepthBoundsTestEnableEXT(VkResult Result, VkCommandBuffer CommandBuffer, VkBool32 DepthBoundsTestEnable)
{
	if (Result == VK_RESULT_MAX_ENUM)
	{
#if VULKAN_ENABLE_DUMP_LAYER
		PrintfBeginResult(FString::Printf(TEXT("CmdSetDepthBoundsTestEnableEXT()")));
#endif
	}
}

void FWrapLayer::CmdSetDepthClampEnableEXT(VkResult Result, VkCommandBuffer CommandBuffer, VkBool32 DepthClampEnable)
{
	if (Result == VK_RESULT_MAX_ENUM)
	{
#if VULKAN_ENABLE_DUMP_LAYER
		PrintfBeginResult(FString::Printf(TEXT("CmdSetDepthClampEnableEXT()")));
#endif
	}
}

void FWrapLayer::CmdSetDepthCompareOpEXT(VkResult Result, VkCommandBuffer CommandBuffer, VkCompareOp DepthCompareOp)
{
	if (Result == VK_RESULT_MAX_ENUM)
	{
#if VULKAN_ENABLE_DUMP_LAYER
		PrintfBeginResult(FString::Printf(TEXT("CmdSetDepthCompareOpEXT()")));
#endif
	}
}

void FWrapLayer::CmdSetDepthTestEnableEXT(VkResult Result, VkCommandBuffer CommandBuffer, VkBool32 DepthTestEnable)
{
	if (Result == VK_RESULT_MAX_ENUM)
	{
#if VULKAN_ENABLE_DUMP_LAYER
		PrintfBeginResult(FString::Printf(TEXT("CmdSetDepthTestEnableEXT()")));
#endif
	}
}

void FWrapLayer::CmdSetDepthWriteEnableEXT(VkResult Result, VkCommandBuffer CommandBuffer, VkBool32 DepthWriteEnable)
{
	if (Result == VK_RESULT_MAX_ENUM)
	{
#if VULKAN_ENABLE_DUMP_LAYER
		PrintfBeginResult(FString::Printf(TEXT("CmdSetDepthWriteEnableEXT()")));
#endif
	}
}

void FWrapLayer::CmdSetRasterizerDiscardEnableEXT(VkResult Result, VkCommandBuffer CommandBuffer, VkBool32 RasterizerDiscardEnable)
{
	if (Result == VK_RESULT_MAX_ENUM)
	{
#if VULKAN_ENABLE_DUMP_LAYER
		PrintfBeginResult(FString::Printf(TEXT("CmdSetRasterizerDiscardEnableEXT()")));
#endif
	}
}

void FWrapLayer::CmdSetPrimitiveRestartEnableEXT(VkResult Result, VkCommandBuffer CommandBuffer, VkBool32 PrimitiveRestartEnable)
{
	if (Result == VK_RESULT_MAX_ENUM)
	{
#if VULKAN_ENABLE_DUMP_LAYER
		PrintfBeginResult(FString::Printf(TEXT("CmdSetPrimitiveRestartEnableEXT()")));
#endif
	}
}

void FWrapLayer::CmdSetLogicOpEnableEXT(VkResult Result, VkCommandBuffer CommandBuffer, VkBool32 LogicOpEnable)
{
	if (Result == VK_RESULT_MAX_ENUM)
	{
#if VULKAN_ENABLE_DUMP_LAYER
		PrintfBeginResult(FString::Printf(TEXT("CmdSetLogicOpEnableEXT()")));
#endif
	}
}

void FWrapLayer::CmdSetAlphaToCoverageEnableEXT(VkResult Result, VkCommandBuffer CommandBuffer, VkBool32 AlphaToCoverageEnable)
{
	if (Result == VK_RESULT_MAX_ENUM)
	{
#if VULKAN_ENABLE_DUMP_LAYER
		PrintfBeginResult(FString::Printf(TEXT("CmdSetAlphaToCoverageEnableEXT()")));
#endif
	}
}

void FWrapLayer::CmdSetAlphaToOneEnableEXT(VkResult Result, VkCommandBuffer CommandBuffer, VkBool32 AlphaToOneEnable)
{
	if (Result == VK_RESULT_MAX_ENUM)
	{
#if VULKAN_ENABLE_DUMP_LAYER
		PrintfBeginResult(FString::Printf(TEXT("CmdSetAlphaToOneEnableEXT()")));
#endif
	}
}

void FWrapLayer::CopyMemoryToImageEXT(VkResult Result, VkDevice Device, const VkCopyMemoryToImageInfo* CopyMemoryToImageInfo)
{
	if (Result == VK_RESULT_MAX_ENUM)
	{
#if VULKAN_ENABLE_DUMP_LAYER
		PrintfBeginResult(FString::Printf(TEXT("CopyMemoryToImageEXT()")));
#endif
	}
}

void FWrapLayer::TransitionImageLayoutEXT(VkResult Result, VkDevice Device, uint32_t TransitionCount, const VkHostImageLayoutTransitionInfo* Transitions)
{
	if (Result == VK_RESULT_MAX_ENUM)
	{
#if VULKAN_ENABLE_DUMP_LAYER
		PrintfBeginResult(FString::Printf(TEXT("TransitionImageLayoutEXT()")));
#endif
	}
}

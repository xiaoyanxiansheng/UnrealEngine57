...
#endif
	VERIFYVULKANRESULT(VulkanRHI::vkCreateDescriptorPool(Device->GetHandle(), &PoolInfo, VULKAN_CPU_ALLOCATOR, &DescriptorPool));
...
FVulkanDescriptorPool::~FVulkanDescriptorPool()
{
	...
	{
		VulkanRHI::vkDestroyDescriptorPool(Device->GetHandle(), DescriptorPool, VULKAN_CPU_ALLOCATOR);
		DescriptorPool = VK_NULL_HANDLE;
	}
}
...
void FVulkanDescriptorPool::Reset()
{
	if (DescriptorPool != VK_NULL_HANDLE)
	{
		VERIFYVULKANRESULT(VulkanRHI::vkResetDescriptorPool(Device->GetHandle(), DescriptorPool, 0));
	}
...
bool FVulkanDescriptorPool::AllocateDescriptorSets(const VkDescriptorSetAllocateInfo& InDescriptorSetAllocateInfo, VkDescriptorSet* OutSets)
{
	...
	return VK_SUCCESS == VulkanRHI::vkAllocateDescriptorSets(Device->GetHandle(), &DescriptorSetAllocateInfo, OutSets);
}
...
{
	TMap<FVulkanGraphicsPipelineState*, FVulkanGraphicsPipelineDescriptorState*> Temp;
...
void FVulkanPendingGfxState::PrepareForDraw(FVulkanCommandListContext& Context)
{
	...
	if (bDirtyVertexStreams)
	{
		...
		struct FTemporaryIA
		{
			...
			VkDeviceSize VertexSizes[MaxVertexElementCount];
			VkDeviceSize VertexStrides[MaxVertexElementCount];
			int32 NumUsed = 0;

			void Add(VkBuffer InBuffer, VkDeviceSize InOffset, VkDeviceSize InStride)
			{
				check(NumUsed < MaxVertexElementCount);
				VertexBuffers[NumUsed] = InBuffer;
				VertexOffsets[NumUsed] = InOffset;
				VertexSizes[NumUsed] = VK_WHOLE_SIZE;
				VertexStrides[NumUsed] = InStride;
				++NumUsed;
			}
		} TemporaryIA;
		...
		for (uint32 BindingIndex = 0; BindingIndex < VertexInputStateInfo.BindingsNum; BindingIndex++)
		{
			...
			TemporaryIA.Add(CurrStream.Stream, CurrStream.BufferOffset, CurrBinding.stride);
		}
...
void FVulkanPendingGfxState::UpdateDynamicStates(FVulkanCommandBuffer& CommandBuffer)
{
	...
	if (bNeedsUpdateViewport)
	{
		...
		if (Device.GetOptionalExtensions().HasEXTExtendedDynamicState1)
		{
			VulkanRHI::vkCmdSetViewportWithCountEXT(CommandBuffer.GetHandle(), FlippedViewports.Num(), FlippedViewports.GetData());
		}
		else
		{
	...
	if (bNeedsUpdateScissor)
	{
		if (Device.GetOptionalExtensions().HasEXTExtendedDynamicState1)
		{
			VulkanRHI::vkCmdSetScissorWithCountEXT(CommandBuffer.GetHandle(), Scissors.Num(), Scissors.GetData());
		}
		else
		{
	...
	const bool bForceUpdate = CommandBuffer.bNeedsFullDynamicStateUpdate;
	FGfxPipelineDesc* LastDynamicStateUpdate = CommandBuffer.LastDynamicStateUpdate;
	const FGfxPipelineDesc& NewDynamicState = CurrentPipeline->Desc;
	const bool bHasMeshShader = (CurrentPipeline->GetVulkanShader(SF_Mesh) != nullptr);
	const bool bHasGeometryShader = (CurrentPipeline->GetVulkanShader(SF_Geometry) != nullptr);

	// Bunch of macros to make dynamic state comparions below easier to read
...
#define VULKAN_DYNAMIC_STATE_BLOCK_END()      }
	...
	// NOTE: All of the different VK_DYNAMIC_STATE_* values being set can be found in VulkanPipeline.cpp

	if (Device.GetOptionalExtensions().HasEXTExtendedDynamicState1)
	{
		check(LastDynamicStateUpdate);

		if (!bHasMeshShader)
		{
			// VK_DYNAMIC_STATE_PRIMITIVE_TOPOLOGY
			VULKAN_DYNAMIC_STATE_BLOCK_START(Topology);
			VulkanRHI::vkCmdSetPrimitiveTopologyEXT(CommandBuffer.GetHandle(), (VkPrimitiveTopology)NewDynamicState.Topology);
			VULKAN_DYNAMIC_STATE_BLOCK_END();
		}
		else
		{
			// The state will be invalidated, make sure we set it again
			LastDynamicStateUpdate->Topology = VK_PRIMITIVE_TOPOLOGY_MAX_ENUM;
		}

		// VK_DYNAMIC_STATE_CULL_MODE
		VULKAN_DYNAMIC_STATE_BLOCK_START(Rasterizer.CullMode);
		VulkanRHI::vkCmdSetCullModeEXT(CommandBuffer.GetHandle(), (VkCullModeFlags)NewDynamicState.Rasterizer.CullMode);
		VULKAN_DYNAMIC_STATE_BLOCK_END();

		// VK_DYNAMIC_STATE_STENCIL_TEST_ENABLE
		VULKAN_DYNAMIC_STATE_BLOCK_START(DepthStencil.bStencilTestEnable);
		VulkanRHI::vkCmdSetStencilTestEnableEXT(CommandBuffer.GetHandle(), NewDynamicState.DepthStencil.bStencilTestEnable ? VK_TRUE : VK_FALSE);
		VULKAN_DYNAMIC_STATE_BLOCK_END();

		// VK_DYNAMIC_STATE_STENCIL_OP (front)
		VULKAN_DYNAMIC_STATE_BLOCK_START(DepthStencil.FrontFailOp, DepthStencil.FrontPassOp, DepthStencil.FrontDepthFailOp, DepthStencil.FrontCompareOp);
		VulkanRHI::vkCmdSetStencilOpEXT(
			CommandBuffer.GetHandle(),
			VK_STENCIL_FACE_FRONT_BIT,
			(VkStencilOp)NewDynamicState.DepthStencil.FrontFailOp,
			(VkStencilOp)NewDynamicState.DepthStencil.FrontPassOp,
			(VkStencilOp)NewDynamicState.DepthStencil.FrontDepthFailOp,
			(VkCompareOp)NewDynamicState.DepthStencil.FrontCompareOp
		);
		VULKAN_DYNAMIC_STATE_BLOCK_END();

		// VK_DYNAMIC_STATE_STENCIL_OP (back)
		VULKAN_DYNAMIC_STATE_BLOCK_START(DepthStencil.BackFailOp, DepthStencil.BackPassOp, DepthStencil.BackDepthFailOp, DepthStencil.BackCompareOp);
		VulkanRHI::vkCmdSetStencilOpEXT(
			CommandBuffer.GetHandle(),
			VK_STENCIL_FACE_BACK_BIT,
			(VkStencilOp)NewDynamicState.DepthStencil.BackFailOp,
			(VkStencilOp)NewDynamicState.DepthStencil.BackPassOp,
			(VkStencilOp)NewDynamicState.DepthStencil.BackDepthFailOp,
			(VkCompareOp)NewDynamicState.DepthStencil.BackCompareOp
		);
		VULKAN_DYNAMIC_STATE_BLOCK_END();

		// VK_DYNAMIC_STATE_DEPTH_COMPARE_OP
		VULKAN_DYNAMIC_STATE_BLOCK_START(DepthStencil.DepthCompareOp);
		VulkanRHI::vkCmdSetDepthCompareOpEXT(CommandBuffer.GetHandle(), (VkCompareOp)NewDynamicState.DepthStencil.DepthCompareOp);
		VULKAN_DYNAMIC_STATE_BLOCK_END();

		// VK_DYNAMIC_STATE_DEPTH_TEST_ENABLE
		VULKAN_DYNAMIC_STATE_BLOCK_START(DepthStencil.bDepthTestEnable);
		VulkanRHI::vkCmdSetDepthTestEnableEXT(CommandBuffer.GetHandle(), NewDynamicState.DepthStencil.bDepthTestEnable ? VK_TRUE : VK_FALSE);
		VULKAN_DYNAMIC_STATE_BLOCK_END();

		// VK_DYNAMIC_STATE_DEPTH_BOUNDS_TEST_ENABLE
		VULKAN_DYNAMIC_STATE_BLOCK_START(DepthStencil.bDepthBoundsTestEnable);
		VulkanRHI::vkCmdSetDepthBoundsTestEnableEXT(CommandBuffer.GetHandle(), NewDynamicState.DepthStencil.bDepthBoundsTestEnable ? VK_TRUE : VK_FALSE);
		VULKAN_DYNAMIC_STATE_BLOCK_END();

		// VK_DYNAMIC_STATE_DEPTH_WRITE_ENABLE
		VULKAN_DYNAMIC_STATE_BLOCK_START(DepthStencil.bDepthWriteEnable);
		VulkanRHI::vkCmdSetDepthWriteEnableEXT(CommandBuffer.GetHandle(), NewDynamicState.DepthStencil.bDepthWriteEnable ? VK_TRUE : VK_FALSE);
		VULKAN_DYNAMIC_STATE_BLOCK_END();
	}

	if (Device.GetOptionalExtensions().HasEXTExtendedDynamicState2)
	{
		// VK_DYNAMIC_STATE_DEPTH_BIAS_ENABLE
		const float OldDepthBiasConstantFactor = NewDynamicState.Rasterizer.DepthBiasConstantFactor;
		VULKAN_DYNAMIC_STATE_BLOCK_START(Rasterizer.DepthBiasConstantFactor);
		VulkanRHI::vkCmdSetDepthBiasEnableEXT(CommandBuffer.GetHandle(), (NewDynamicState.Rasterizer.DepthBiasConstantFactor != 0.0f) ? VK_TRUE : VK_FALSE);
		// Put back the old value if VK_DYNAMIC_STATE_DEPTH_BIAS is int use so that it gets updated
		// Note: this does mean we call enable for each DepthBiasConstantFactor
		if (Device.GetOptionalExtensions().HasEXTExtendedDynamicState3)
		{
			LastDynamicStateUpdate->Rasterizer.DepthBiasConstantFactor = OldDepthBiasConstantFactor;
		}
		VULKAN_DYNAMIC_STATE_BLOCK_END();
	}
	...
	if (Device.GetOptionalExtensions().HasEXTExtendedDynamicState3)
	{
		...
		VULKAN_DYNAMIC_STATE_BLOCK_START(Rasterizer.PolygonMode);
		VulkanRHI::vkCmdSetPolygonModeEXT(CommandBuffer.GetHandle(), (VkPolygonMode)NewDynamicState.Rasterizer.PolygonMode);
		VULKAN_DYNAMIC_STATE_BLOCK_END();

		// VK_DYNAMIC_STATE_RASTERIZATION_SAMPLES_EXT
		VULKAN_DYNAMIC_STATE_BLOCK_START(RasterizationSamples);
		const VkSampleCountFlagBits RasterizationSamples = (VkSampleCountFlagBits)FMath::Max<uint16>(1u, NewDynamicState.RasterizationSamples);
		VulkanRHI::vkCmdSetRasterizationSamplesEXT(CommandBuffer.GetHandle(), RasterizationSamples);
		VULKAN_DYNAMIC_STATE_BLOCK_END();

		//VK_DYNAMIC_STATE_ALPHA_TO_COVERAGE_ENABLE_EXT
		VULKAN_DYNAMIC_STATE_BLOCK_START(UseAlphaToCoverage);
		VulkanRHI::vkCmdSetAlphaToCoverageEnableEXT(CommandBuffer.GetHandle(), NewDynamicState.UseAlphaToCoverage ? VK_TRUE : VK_FALSE);
		VULKAN_DYNAMIC_STATE_BLOCK_END();
		...
		if (bForceUpdate || 
			(LastDynamicStateUpdate->ShaderKeyShared != NewDynamicState.ShaderKeyShared) ||
			(LastDynamicStateUpdate->ColorAttachmentStates != NewDynamicState.ColorAttachmentStates))
		{
			if (NewDynamicState.ColorAttachmentStates.Num())
			{
				check(NewDynamicState.ColorAttachmentStates.Num() <= MaxSimultaneousRenderTargets);

				VkBool32 BlendEnabled[MaxSimultaneousRenderTargets];
				VkColorBlendEquationEXT ColorBlendEquation[MaxSimultaneousRenderTargets];
				VkColorComponentFlags ColorWriteMask[MaxSimultaneousRenderTargets];
				uint32 TargetWriteMask = 0xffffffff;
				if (const FVulkanShader* PixelShader = CurrentPipeline->GetVulkanShader(SF_Pixel))
				{
					TargetWriteMask = PixelShader->GetCodeHeader().InOutMask;
				}
				for (int32 Index = 0; Index < NewDynamicState.ColorAttachmentStates.Num(); ++Index)
				{
					const FGfxPipelineDesc::FBlendAttachment& BlendAttachment = NewDynamicState.ColorAttachmentStates[Index];

					BlendEnabled[Index] = BlendAttachment.bBlend ? VK_TRUE : VK_FALSE;

					ColorBlendEquation[Index].colorBlendOp = (VkBlendOp)BlendAttachment.ColorBlendOp;
					ColorBlendEquation[Index].srcColorBlendFactor = (VkBlendFactor)BlendAttachment.SrcColorBlendFactor;
					ColorBlendEquation[Index].dstColorBlendFactor = (VkBlendFactor)BlendAttachment.DstColorBlendFactor;
					ColorBlendEquation[Index].alphaBlendOp = (VkBlendOp)BlendAttachment.AlphaBlendOp;
					ColorBlendEquation[Index].srcAlphaBlendFactor = (VkBlendFactor)BlendAttachment.SrcAlphaBlendFactor;
					ColorBlendEquation[Index].dstAlphaBlendFactor = (VkBlendFactor)BlendAttachment.DstAlphaBlendFactor;

					ColorWriteMask[Index] = (VkColorComponentFlags)BlendAttachment.ColorWriteMask;
					if (0 == (TargetWriteMask & 1)) //clear write mask of rendertargets not written by pixelshader.
					{
						ColorWriteMask[Index] = 0;
					}
					TargetWriteMask >>= 1;
				}

				VulkanRHI::vkCmdSetColorBlendEnableEXT(
					CommandBuffer.GetHandle(),
					0, NewDynamicState.ColorAttachmentStates.Num(),
					BlendEnabled);

				VulkanRHI::vkCmdSetColorBlendEquationEXT(
					CommandBuffer.GetHandle(),
					0, NewDynamicState.ColorAttachmentStates.Num(),
					ColorBlendEquation);

				VulkanRHI::vkCmdSetColorWriteMaskEXT(
					CommandBuffer.GetHandle(),
					0, NewDynamicState.ColorAttachmentStates.Num(),
					ColorWriteMask);
			}
	...
	if (Device.GetOptionalExtensions().HasEXTVertexInputDynamicState)
	{
		bDirtyDynamicVertexInput = bDirtyDynamicVertexInput || bForceUpdate;

		if (bDirtyDynamicVertexInput ||
			(LastDynamicStateUpdate->VertexAttributes != NewDynamicState.VertexAttributes) ||
			(LastDynamicStateUpdate->VertexBindings != NewDynamicState.VertexBindings))
		{
			if (!bHasMeshShader)
			{
				TArray<VkVertexInputBindingDescription2EXT, TInlineAllocator<32>> BindingDescription;
				for (const FGfxPipelineDesc::FVertexBinding& SourceBinding : NewDynamicState.VertexBindings)
				{
					VkVertexInputBindingDescription2EXT& Binding = BindingDescription.AddZeroed_GetRef();
					Binding.sType = VK_STRUCTURE_TYPE_VERTEX_INPUT_BINDING_DESCRIPTION_2_EXT;
					SourceBinding.WriteInto(Binding);
				}

				TArray<VkVertexInputAttributeDescription2EXT, TInlineAllocator<32>> AttributeDescription;
				for (const FGfxPipelineDesc::FVertexAttribute& SourceAttr : NewDynamicState.VertexAttributes)
				{
					VkVertexInputAttributeDescription2EXT& Attr = AttributeDescription.AddZeroed_GetRef();
					Attr.sType = VK_STRUCTURE_TYPE_VERTEX_INPUT_ATTRIBUTE_DESCRIPTION_2_EXT;
					SourceAttr.WriteInto(Attr);
				}

				VulkanRHI::vkCmdSetVertexInputEXT(CommandBuffer.GetHandle(),
					BindingDescription.Num(), BindingDescription.GetData(),
					AttributeDescription.Num(), AttributeDescription.GetData());

				LastDynamicStateUpdate->VertexAttributes = NewDynamicState.VertexAttributes;
				LastDynamicStateUpdate->VertexBindings = NewDynamicState.VertexBindings;
				bDirtyDynamicVertexInput = false;
			}
		}

		// Pipelines with mesh or geometry shaders invalidate the dynamic state
		if (bHasMeshShader || bHasGeometryShader)
		{
			bDirtyDynamicVertexInput = true;
		}
	}

#undef VULKAN_DYNAMIC_STATE_COMPARE
#undef VULKAN_DYNAMIC_STATE_UPDATE
#undef VULKAN_DYNAMIC_STATE_BLOCK_START
#undef VULKAN_DYNAMIC_STATE_BLOCK_END

	CommandBuffer.bNeedsFullDynamicStateUpdate = false;
}
...
bool FVulkanGenericDescriptorPool::AllocateDescriptorSet(VkDescriptorSetLayout Layout, VkDescriptorSet& OutSet)
{
	...
	return (VK_SUCCESS == VulkanRHI::vkAllocateDescriptorSets(Device->GetHandle(), &DescriptorSetAllocateInfo, &OutSet));
}
...
#endif
		VulkanRHI::vkUpdateDescriptorSets(Pool.GetDevice()->GetHandle(),
...
bool FVulkanPendingGfxState::SetGfxPipeline(FVulkanGraphicsPipelineState* InGfxPipeline, bool bForceReset)
{

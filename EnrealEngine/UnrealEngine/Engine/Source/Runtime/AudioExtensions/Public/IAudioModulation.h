...
namespace Audio
{
	...
	struct FModulationParameter
	{
		...
		friend inline uint32 GetTypeHash(const FModulationParameter& InModulationParameter)
		{
	...
	struct FModulatorHandle
	{
		...
		friend inline uint32 GetTypeHash(const FModulatorHandle& InModulatorHandle)
		{
			return HashCombineFast(InModulatorHandle.HandleId, InModulatorHandle.ModulatorId);
		}

		inline bool operator==(const FModulatorHandle& Other) const
		{

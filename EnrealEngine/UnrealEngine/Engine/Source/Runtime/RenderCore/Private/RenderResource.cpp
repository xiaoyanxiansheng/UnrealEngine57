...
static thread_local FName GRenderResourceScopeName = NAME_None;
...
{
	FName Old = GRenderResourceScopeName;
	GRenderResourceScopeName = Name;
	return Old;
}
...
FRenderResource::~FRenderResource()
{
	...
	if (IsInitialized() && !GIsCriticalError)
	{
		...
		UE_LOG(LogRendererCore, Fatal, TEXT("A FRenderResource was deleted without being released first! [Resource=%s, Owner=%s, Scope=%s]"), *GetResourceName().ToString(), *GetOwnerName().ToString(), *GRenderResourceScopeName.ToString());
	}
}
...
void FTextureReference::InitRHI(FRHICommandListBase& RHICmdList)
{
	SCOPED_LOADTIMER(FTextureReference_InitRHI);
	TextureReferenceRHI = RHICmdList.CreateTextureReference();
}

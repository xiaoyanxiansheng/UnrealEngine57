...
bool UE::PipelineCacheUtilities::SaveStableKeysFile(const FStringView& Filename, const FStableShaderSet& Values)
{
	...
	*Archive << Header;
	...
	for (const TUniquePtr<FStableShaderKeyAndValue>& Item : Values)
	{
	...
	*Archive << NumHashes;
	...
	for (const TUniquePtr<FStableShaderKeyAndValue>& ConstItem : Values)
	{
...
bool UE::PipelineCacheUtilities::LoadStablePipelineCacheFile(const FString& Filename, const FStableShaderSet& StableMap, TSet<FPipelineCacheFileFormatPSO>& OutPSOs, FName& OutTargetPlatform, int32& OutPSOsRejected, int32& OutPSOsMerged)
{
	...
	*Archive << Header;
	...
	int32 NumKeysPerTask = FMath::Max(SavedStableKeys.Num() / NumRemappingTasks, 1);
		...
		RemappingStableKeysTasks.Add(
			FFunctionGraphTask::CreateAndDispatchWhenReady([FirstKey, KeysToRemapThisTask, &SavedStableKeys, &StableMap, &HashesForStableKeys, &OutTargetPlatform]
			{
				for (int32 IdxKey = 0; IdxKey < KeysToRemapThisTask; ++IdxKey)
				{
					...
					if (const TUniquePtr<FStableShaderKeyAndValue>* ShaderKeyPtr = StableMap.Find(SavedStableKeys[AbsKeyIdx]))
					{
						check((*ShaderKeyPtr)->OutputHash != FSHAHash());
						check(OutTargetPlatform == (*ShaderKeyPtr)->TargetPlatform);

	...
	FCoreDelegates::FAttachShaderReadRequestFunc* AttachShaderReadRequestFuncPtr)
{
	...
	else if (PreloadEntry.PreloadEvent.IsValid() && PreloadEntry.PreloadEvent->IsCompleted() && PreloadEntry.IoRequest.GetResult() == nullptr)
	{
...
TRefCountPtr<FRHIShader> FIoStoreShaderCodeArchive::CreateShader(int32 ShaderIndex, bool bRequired)
{
	...
	// we should not have this situation, but if we do, log a little more info about it before dying
	if (UNLIKELY(PreloadEntryPtr->IoRequest.GetResult() == nullptr))
	{
		FString ShaderGroupIoHash;
		Header.ShaderGroupIoHashes[GroupIndex].ToString(ShaderGroupIoHash);
		UE_LOG(LogShaderLibrary, Warning, TEXT("IoRequest backing buffer is not there when we need it, about to crash. Library '%s', shader hash '%s' (freq=%d), group IoChunkId '%s', started preloading at frame %d, current frame is %d, preloading %s"),
			*GetName(),

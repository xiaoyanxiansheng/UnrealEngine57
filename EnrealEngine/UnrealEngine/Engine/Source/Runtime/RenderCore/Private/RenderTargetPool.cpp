...
FPooledRenderTarget* FRenderTargetPool::CreateRenderTarget(FRHICommandListBase& RHICmdList, const FRHITextureCreateInfo& Desc, uint32 DescHash, const TCHAR* Name)
{
	...
	if (GRenderTargetPoolLogCreationSizes > 0)
	{
		...
		int32 CurrentCaptureFrame = 0;
		if (!FCsvProfiler::Get())
		{
			CurrentCaptureFrame = -1;
		}
		else if (FCsvProfiler::Get()->IsCapturing_Renderthread())
		{
			CurrentCaptureFrame = FCsvProfiler::Get()->GetCaptureFrameNumberRT();
		}
		UE_LOG(LogRenderTargetPool, Warning, TEXT("FRenderTargetPool::CreateRenderTarget{Frame %d}: %s create with expected size - %ld"), CurrentCaptureFrame, Name, RHICalcTexturePlatformSize(Desc).Size);
#else
		UE_LOG(LogRenderTargetPool, Warning, TEXT("FRenderTargetPool::CreateRenderTarget: %s create with expected size - %ld"), Name, RHICalcTexturePlatformSize(Desc).Size);
#endif
	}

...
FAutoConsoleCommandWithArgsAndOutputDevice GListShaderCmd(
	...
	FConsoleCommandWithArgsAndOutputDeviceDelegate::CreateStatic(
		[](const TArray<FString>& Params, FOutputDevice& Out)
		{
			...
			{
				TCHAR ShaderMapHash[FSHAHash::GetStringLen()+1] = {0};
				if (ShaderMap->GetResource())
				{
					ShaderMap->GetResource()->GetShaderMapHash().ToString(ShaderMapHash, true);
				}
				...
				Report.BindLambda(
					[&Out, &ShaderMapHash](
						...
						void* Ptr)
					{
						Out.Logf(TEXT("%s,%s,%s,%s,%s,%d,%.2f,%s,%p"), FriendlyName, OwnerName, ShaderMapHash, VertexFactory, ShaderType, Permutation, Bytes / 1024.f, Hash, Ptr);
					}
				);

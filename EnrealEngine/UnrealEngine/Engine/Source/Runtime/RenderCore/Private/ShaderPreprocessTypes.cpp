...
void FShaderDiagnosticRemapper::Remap(FShaderCompilerError& Diagnostic) const
{
	...
	while (FilenameIndex >= 0)
	{
		...
		while (LineNumberStart < OriginalMessage.Len() && !FChar::IsDigit(OriginalMessage[LineNumberStart]))
		{
			++LineNumberStart;
		}
...
void FShaderDiagnosticRemapper::Shrink()
{
	Blocks.Shrink();
	StrippedLineOffsets.Shrink();
}

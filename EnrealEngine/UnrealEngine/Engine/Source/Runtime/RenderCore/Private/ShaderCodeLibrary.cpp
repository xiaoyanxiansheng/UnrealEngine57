...
static FAutoConsoleVariableRef CVarUploadShaderDebugArtifactsRef(
	TEXT("r.ShaderCodeLibrary.UploadArtifacts"),
	GUploadShaderDebugArtifacts,
	TEXT("If true, uploads artifacts that were dumped to the Saved/Shaders/ folder when running on a build machine"),
	ECVF_Default
);

static FString GetShaderCodeLibraryDebugInfoPath()
{
	const FString BaseDebugInfoPath = GIsBuildMachine && GUploadShaderDebugArtifacts ? GetBuildMachineArtifactBasePath() : FPaths::ProjectSavedDir();
	return BaseDebugInfoPath / TEXT("Shaders");
}
...
struct FEditorShaderCodeArchive
{
	...
	int32 PrepareAddShaderCode(FShaderCodeStats& CodeStats,
	...
	void ReadShaderCodeBlock(FArchive& Ar, int64 StartOffset, const TArrayView<FShaderCodeReadInfo>& BlockReadInfo)
	{
	...
	void ReadShaderCode(FArchive& Ar, TArray<FShaderCodeReadInfo>& ReadInfo)
	{
		if (ReadInfo.Num() > 0)
		{
			// Sort by offset
			...
			for (int64 i = 1; i < ReadInfo.Num(); ++i)
			{
				...
				{
					ReadShaderCodeBlock(Ar, PrevCookedShadersCodeStart, MakeArrayView(&ReadInfo[BlockStartIndex], IndexCount));
					BlockStartIndex = i;
					IndexCount = 1;
				}
				else
			...
			*PrevCookedAr << Version;
			...
			if (Version == GShaderCodeArchiveVersion)
			{
				...
				*PrevCookedAr << PrevCookedShaders;
				...
				{
					PrepareAddShaderCode(CodeStats, PrevShaderMapIndex, PrevCookedShaders, ReadInfo);
				}
				ReadShaderCode(*PrevCookedAr, ReadInfo);
			}
	...
	void AddExistingShaderCodeLibrary(FString const& OutputDir, FShaderCodeStats& CodeStats)
	{
		...
		const FString ShaderIntermediateLocation = GetShaderCodeLibraryDebugInfoPath() / FormatNameStr;
		...
		for (const FString& ShaderFileName : ShaderFiles)
		{
			...
			if (FindArchiveFormat.FindNext())
			{
				...
				{
					AddShaderCodeLibraryByName(OutputDir, FoundArchiveName, CodeStats);
				}
			}
		}
	}
	...
	bool SaveToDisk(const FString& OutputDir, const FString& MetaOutputDir, bool bSaveOnlyAssetInfo = false, TArray<FString>* OutputFilenames = nullptr)
	{
		...
		if (bSuccess && SerializedShaders.GetNumShaderMaps() > 0)
		{
			...
			const FString DebugInfoPath = GetShaderCodeLibraryDebugInfoPath();
			FString IntermediateFormatPath = GetShaderCodeFilename(DebugInfoPath / FormatName.ToString(), LibraryName, FormatName);
			FString AssetInfoIntermediatePath = GetShaderAssetInfoFilename(DebugInfoPath / FormatName.ToString(), LibraryName, FormatName);
			FString TypeInfoIntermediatePath = GetShaderTypeInfoFilename(DebugInfoPath / FormatName.ToString(), LibraryName, FormatName);
	...
	bool PackageNativeShaderLibrary(const FString& ShaderCodeDir, TArray<FString>* OutputFilenames = nullptr)
	{
		...
		FString IntermediateFormatPath = GetShaderDebugFolder(GetShaderCodeLibraryDebugInfoPath() / FormatName.ToString(), LibraryName, FormatName);
	...
	void DumpStatsAndDebugInfo()
	{
		...
		if (bUseExtendedDebugInfo)
		{
			...
			const FString DebugInfoPath = GetShaderCodeLibraryDebugInfoPath();
			const FString DebugLibFolder = GetShaderDebugFolder(DebugInfoPath / FormatName.ToString(), LibraryName, FormatName);
...

			{
				...
				if (TUniquePtr<FArchive> DumpAr = TUniquePtr<FArchive>(IFileManager::Get().CreateFileWriter(*DumpFilename)))
				{
					...
					UE_LOG(LogShaderLibrary, Display, TEXT("Textual dump saved to '%s'"), *DumpFilename);
				}
			}
#if 0 // creating a graphviz graph - maybe one day we'll return to this
			FString DebugGraphFolder = GetShaderDebugFolder(DebugInfoPath / FormatName.ToString(), LibraryName, FormatName);
...
struct FEditorShaderStableInfo
{
	...
	void AddExistingShaderCodeLibrary(FString const& OutputDir)
	{
		...
		{
			const FString ShaderIntermediateLocation = GetShaderCodeLibraryDebugInfoPath() / FormatName.ToString();
			TArray<FString> ShaderFiles;
	...
	bool SaveToDisk(FString const& OutputDir, FString& OutSCLCSVPath)
	{
		...
		{
			// Write to a intermediate file
			FString IntermediateFormatPath = GetStableInfoArchiveFilename(GetShaderCodeLibraryDebugInfoPath() / FormatName.ToString(), LibraryName, FormatName);
...
public:
	...
	void CleanDirectories(TArray<FName> const& ShaderFormats)
	{
		...
		{
			const FString DebugInfoPath = GetShaderCodeLibraryDebugInfoPath();
			FString ShaderIntermediateLocation = DebugInfoPath / Format.ToString();
			FString ShaderPlatformIntermediateLocation = DebugInfoPath / GetFormatAndPlatformName(Format);
		...
		for (const FShaderCodeStats& PlatformCodeStats : EditorShaderCodeStats)
		{
			if (PlatformCodeStats.NumShaderMaps > 0)
			{
				...
				for (const TPair<uint64, FShaderCodeStats::FPerTypeStats>& Pair : PlatformCodeStats.ShaderTypeStats)
				{
					...
					checkf(ShaderType != nullptr, TEXT("Shader library contains a reference to an FShaderType with hash %llu which does not exist.\n"
												       "If it has been removed or renamed, please bump CookIncrementalVersion to force a rebuild"), Pair.Key);
				...
				{
					UE_LOG(LogShaderLibrary, Warning, TEXT("Failed to copy from '%s' to '%s'"), *Filename, *MetaPath);
				}
			}
			PlatformId++;
		}

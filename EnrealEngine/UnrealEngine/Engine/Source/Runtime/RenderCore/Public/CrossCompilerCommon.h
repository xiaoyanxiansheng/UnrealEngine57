...
namespace CrossCompiler
{
	...
	static inline uint8 ShaderStageIndexToTypeName(uint8 ShaderStage)
	{
		switch (ShaderStage)
		{
	...
	static inline uint8 PackedTypeIndexToTypeName(uint8 ArrayType)
	{
		switch (ArrayType)
		{
	...
	static inline uint8 PackedTypeNameToTypeIndex(uint8 ArrayName)
	{
		switch (ArrayName)
		{
	...
	static inline bool IsValidPackedTypeName(EPackedTypeName TypeName)
	{
		switch (TypeName)
		{
	...
	static inline EPackedTypeName PackedTypeIndexToTypeName(EPackedTypeIndex TypeIndex)
	{
		switch (TypeIndex)
		{
	...
	static inline EPackedTypeIndex PackedTypeNameToTypeIndex(EPackedTypeName TypeName)
	{
		switch (TypeName)
		{
...
#pragma warning(error : 4596)
	struct FShaderBindingInOutMask
	{
		...
		inline void EnableField(int32 Index)
		{
		...
		inline bool IsFieldEnabled(int32 Index) const
		{
			ensure(Index >= 0 && Index <= FShaderBindingInOutMask::MaxIndex);

...
// A global dummy texture used for manual transition (Ex: when the VS is sampling produce in a previous pass, and needs a barrier)
UE_DEPRECATED(5.7, "This is no longer being used.")
extern RENDERCORE_API FTexture* GDummyTransitionTexture;

// An empty vertex buffer with a UAV
extern RENDERCORE_API FVertexBufferWithSRV* GEmptyVertexBufferWithUAV;

// An empty uint4 vertex buffer with a UAV
extern RENDERCORE_API FVertexBufferWithSRV* GEmptyVertexBufferUInt4WithUAV;
...
class FEmptyResourceCollection : public FRenderResource
{
public:
...
extern RENDERCORE_API TGlobalResource<FEmptyResourceCollection, FRenderResource::EInitPhase::Pre> GEmptyResourceCollection;
...
public:
	...
	inline FAllocationEx Allocate(uint32 NumIndices)
	{

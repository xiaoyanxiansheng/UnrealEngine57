...
struct FPixelShaderUtils
{
	...
	class FRasterizeToRectsVS : public FGlobalShader
	{
		...
		BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
			...
			SHADER_PARAMETER(float, DepthIncrement)
		...
		float DepthIncrement = 0.0f)
	{
		...
		Parameters->VS.DepthIncrement = DepthIncrement;
		...
		GraphBuilder.AddPass(
			...
			Parameters,
			PassFlags,
			[Parameters, GlobalShaderMap, VertexShader, PixelShader, ViewportSize, BlendState, RasterizerState, DepthStencilState, StencilRef, bSkipRenderPass, NumRects](FRDGAsyncTask, FRHICommandList& RHICmdList)
		{
			if (bSkipRenderPass)
			{
			...
			const uint32 NumInstances = NumRects;

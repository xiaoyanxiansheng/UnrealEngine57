...
namespace FAISystem
{
	...
	inline bool IsValidDirection(const FVector& TestVector)
	{
		return IsValidLocation(TestVector) == true && TestVector.IsZero() == false;
	}

	inline bool IsValidRotation(const FRotator& TestRotation)
	{
		return TestRotation != InvalidRotation;
	}

	inline bool IsValidOrientation(const FQuat& TestOrientation)
	{
...
namespace FAISystem
{
	inline bool PickAIOption(EAIOptionFlag::Type Option, bool DefaultOption)
	{
		return Option == EAIOptionFlag::Default ? DefaultOption : (Option == EAIOptionFlag::Enable);
	}

	inline EAIOptionFlag::Type BoolToAIOption(bool Value)
	{
...
public:
	...
	friend inline uint32 GetTypeHash(const FAINamedID& ID)
	{
...
public:
	...
	friend inline uint32 GetTypeHash(const FAIGenericID& ID)
	{
		return GetTypeHash(ID.Index);
	}
};
...
struct FAIResourceLock
{
	...
	inline bool IsLockedBy(EAIRequestPriority::Type LockPriority) const
	{
	...
	inline bool IsAvailableFor(EAIRequestPriority::Type LockPriority) const
	{

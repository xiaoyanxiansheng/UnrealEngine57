...
public:
	...
	inline uint8* GetKeyRawData(const FName& KeyName) { return GetKeyRawData(GetKeyID(KeyName)); }
	inline uint8* GetKeyRawData(FBlackboard::FKey KeyID) { return ValueMemory.Num() && ValueOffsets.IsValidIndex(KeyID) ? (ValueMemory.GetData() + ValueOffsets[KeyID]) : NULL; }

	inline const uint8* GetKeyRawData(const FName& KeyName) const { return GetKeyRawData(GetKeyID(KeyName)); }
	inline const uint8* GetKeyRawData(FBlackboard::FKey KeyID) const { return ValueMemory.Num() && ValueOffsets.IsValidIndex(KeyID) ? (ValueMemory.GetData() + ValueOffsets[KeyID]) : NULL; }

	PRAGMA_DISABLE_DEPRECATION_WARNINGS // re BlackboardAsset
	inline bool IsValidKey(FBlackboard::FKey KeyID) const { check(BlackboardAsset); return BlackboardAsset->IsValidKey(KeyID); }
...
inline bool UBlackboardComponent::HasValidAsset() const
{
...
public:
	...
	inline bool SetValue(UBlackboardComponent& BBComponent, const T2 InValue)
	{
		return SetValue(BBComponent, FStoredType(InValue));
	}

	/** @return True is value has changed*/
	inline bool SetValue(UBlackboardComponent& BBComponent, const FStoredType InValue)
	{
	...
	inline const FStoredType& Get() const
	{
		ensure(BBKey != FBlackboard::InvalidKey);
		return CachedValue;
	}

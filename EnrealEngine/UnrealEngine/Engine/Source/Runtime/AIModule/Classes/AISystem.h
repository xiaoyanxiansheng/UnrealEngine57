...
public:
	...
	inline UBehaviorTreeManager* GetBehaviorTreeManager() { return BehaviorTreeManager; }
	/** Behavior tree manager const getter */
	inline const UBehaviorTreeManager* GetBehaviorTreeManager() const { return BehaviorTreeManager; }

	/** Environment Query manager getter */
	inline UEnvQueryManager* GetEnvironmentQueryManager() { return EnvironmentQueryManager; }
	/** Environment Query manager const getter */
	inline const UEnvQueryManager* GetEnvironmentQueryManager() const { return EnvironmentQueryManager; }

	inline UAIPerceptionSystem* GetPerceptionSystem() { return PerceptionSystem; }
	inline const UAIPerceptionSystem* GetPerceptionSystem() const { return PerceptionSystem; }

	inline UAIHotSpotManager* GetHotSpotManager() { return HotSpotManager; }
	inline const UAIHotSpotManager* GetHotSpotManager() const { return HotSpotManager; }

	inline UNavLocalGridManager* GetNavLocalGridManager() { return NavLocalGrids; }
	inline const UNavLocalGridManager* GetNavLocalGridManager() const { return NavLocalGrids; }

	inline static UAISystem* GetCurrentSafe(UWorld* World) 
	{ 
		return World != nullptr ? Cast<UAISystem>(World->GetAISystem()) : NULL;
	}

	inline static UAISystem* GetCurrent(UWorld& World)
	{
		return Cast<UAISystem>(World.GetAISystem());
	}

	inline UWorld* GetOuterWorld() const { return Cast<UWorld>(GetOuter()); }

	virtual UWorld* GetWorld() const override { return GetOuterWorld(); }
	
	inline void AddReferenceFromProxyObject(UAIAsyncTaskBlueprintProxy* BlueprintProxy) { AllProxyObjects.AddUnique(BlueprintProxy); }

	inline void RemoveReferenceToProxyObject(UAIAsyncTaskBlueprintProxy* BlueprintProxy) { AllProxyObjects.RemoveSwap(BlueprintProxy); }
	...
	*/
	...
	public:
		...
		inline FBlackboardDataToComponentsIterator& operator++()
		{
			++GetCurrentIteratorRef();
			TryMoveIteratorToParentBlackboard();
			return *this;
		}
		inline FBlackboardDataToComponentsIterator operator++(int)
		{
		...
		inline explicit operator bool() const { return CurrentIteratorIndex < Iterators.Num() && (bool)GetCurrentIteratorRef(); }
		inline bool operator !() const { return !(bool)*this; }

		inline UBlackboardData* Key() const { return GetCurrentIteratorRef().Key().Get(); }
		inline UBlackboardComponent* Value() const { return GetCurrentIteratorRef().Value().Get(); }

	private:
		inline const FBlackboardDataToComponentsMap::TConstKeyIterator& GetCurrentIteratorRef() const { return Iterators[CurrentIteratorIndex]; }
		inline FBlackboardDataToComponentsMap::TConstKeyIterator& GetCurrentIteratorRef() { return Iterators[CurrentIteratorIndex]; }

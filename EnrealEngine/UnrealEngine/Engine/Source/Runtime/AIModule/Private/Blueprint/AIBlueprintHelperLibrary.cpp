...
{
	if (!ensureMsgf(WorldContextObject == nullptr || IsValid(WorldContextObject), TEXT("Unusable WorldContextObject pointer provided (i.e., garbage)")))
	{
		return nullptr;
	}

	if (!ensureMsgf(Pawn == nullptr || IsValid(Pawn), TEXT("Unusable Pawn pointer provided (i.e., garbage)")))
	{
		return nullptr;
	}
	...
	if (Pawn == nullptr
		|| !ensureMsgf(IsValid(Pawn), TEXT("Unusable Pawn pointer fetched from AIController (i.e., garbage)")))
	{
		return nullptr;
	}
	...
	if (AIController != nullptr
		&& ensureMsgf(IsValid(AIController), TEXT("Unusable AIController pointer fetched from Pawn (i.e., garbage)")))
	{

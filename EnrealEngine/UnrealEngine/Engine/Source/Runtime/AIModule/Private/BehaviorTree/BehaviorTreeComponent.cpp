...
void UBehaviorTreeComponent::EvaluateBranch(EBTNodeResult::Type ContinueWithResult)
{
	if (InstanceStack.IsValidIndex(ActiveInstanceIdx))
	{
		...
		UBTCompositeNode* ExecuteParent = (ActiveInstance.ActiveNode == NULL) ? ActiveInstance.RootNode.Get() :
			(ActiveInstance.ActiveNodeType == EBTActiveNode::Composite) ? (UBTCompositeNode*)ActiveInstance.ActiveNode.Get() :
											  ...
											  const int32 RequestedByChildIndex, const EBTNodeResult::Type ContinueWithResult, const bool bStoreForDebugger)
{
	...
	if (bSwitchToHigherPriority)
	{
		...
		{
			FBehaviorTreeInstance& ActiveInstance = InstanceStack[ActiveInstanceIdx];
			CurrentNode = (ActiveInstance.ActiveNode == NULL) ? ActiveInstance.RootNode.Get() :
				(ActiveInstance.ActiveNodeType == EBTActiveNode::Composite) ? (UBTCompositeNode*)ActiveInstance.ActiveNode.Get() :
				ActiveInstance.ActiveNode->GetParentNode();
...
void UBehaviorTreeComponent::AddReferencedObjects(UObject* InThis, FReferenceCollector& Collector)
{
	UBehaviorTreeComponent* This = CastChecked<UBehaviorTreeComponent>(InThis);
	for (FBehaviorTreeInstance& ActiveInstance : This->InstanceStack)
	{
		Collector.AddReferencedObject(ActiveInstance.RootNode);
		Collector.AddReferencedObject(ActiveInstance.ActiveNode);
		Collector.AddReferencedObjects(ActiveInstance.ActiveAuxNodes);
		for (FBehaviorTreeParallelTask& ParallelTask : ActiveInstance.ParallelTasks)
		{
			Collector.AddReferencedObject(ParallelTask.TaskNode);
		}
	}

	for (FBehaviorTreeSearchUpdate& PendingUpdate : This->SearchData.PendingUpdates)
	{
		Collector.AddReferencedObject(PendingUpdate.TaskNode);
		Collector.AddReferencedObject(PendingUpdate.AuxNode);
	}

	Collector.AddReferencedObject(This->ExecutionRequest.ExecuteNode);
	Collector.AddReferencedObject(This->PendingExecution.NextTask);

	for (FBranchActionInfo& PendingBranchActionRequest : This->PendingBranchActionRequests)
	{
		Collector.AddReferencedObject(PendingBranchActionRequest.Node);
	}

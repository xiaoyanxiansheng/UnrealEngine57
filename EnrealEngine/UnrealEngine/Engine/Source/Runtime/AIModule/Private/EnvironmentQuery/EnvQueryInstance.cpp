...
bool FEnvQueryInstance::PrepareContext(UClass* ContextClass, FEnvQueryContextData& ContextData)
{
	...
	{
		const FEnvQueryInstanceContextCacheItem* CachedData = ContextCache.FindByPredicate([ContextClass](const FEnvQueryInstanceContextCacheItem& Item)
			{
				return Item.Class == ContextClass;
			});
		...
		{
			ContextData = CachedData->ContextData;
		}
	}
...
void FEnvQueryInstance::FinalizeQuery()
{
	if (NumValidItems > 0)
	{
		EEnvQueryResultNormalizationOption NormalizationOption = EEnvQueryResultNormalizationOption::Default;
		if (const UEnvQueryGenerator* Generator = Options[OptionIndex].Generator)
		{
			NormalizationOption = Generator->GetNormalizationOption();
		}
		...
		if (Mode == EEnvQueryRunMode::SingleResult)
		{
			...
			if (bFoundSingleResult == false && bPassOnSingleResult == false)
			{
				...
				{
				case EEnvQueryResultNormalizationOption::Default: // falling through on purpose
				case EEnvQueryResultNormalizationOption::Unaltered:
					break;
				case EEnvQueryResultNormalizationOption::Normalized:
					NormalizeScores();
					break;
				}
...
uint32 FEnvQueryInstance::GetContextAllocatedSize() const
{
	...
	for (const FEnvQueryInstanceContextCacheItem& CacheItem : ContextCache)
	{
		MemSize += CacheItem.ContextData.GetAllocatedSize();
	}

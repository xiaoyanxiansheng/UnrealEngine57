...
void UPathFollowingComponent::UpdatePathSegment()
{
	...
	checkf(NavMovementInterface.IsValid() , TEXT("Temporary comment: if you hit this, check other threads to see what could stump the NavMovementInterface."));
	const FVector CurrentLocation = NavMovementInterface->GetFeetLocation();
		...
		&& MoveRequestId == GetCurrentRequestId())
	{
		...
		if (bHasNewSample && IsBlocked())
		{
			...
			{
				if ((GetOwner() != nullptr) && (NavMovementInterface != nullptr))
				{
...
bool UPathFollowingComponent::IsFollowingNavLink() const
{
	return IsCurrentSegmentNavigationLink();
}

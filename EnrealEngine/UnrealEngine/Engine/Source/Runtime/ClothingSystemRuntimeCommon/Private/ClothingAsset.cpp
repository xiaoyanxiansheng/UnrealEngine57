...
#if WITH_EDITOR
void ClothingAssetUtils::UnbindFromSkeletalMesh(const UClothingAssetBase& Asset, USkeletalMesh& SkeletalMesh)
{
	if (HasAnyMeshClothingAssetBindings(&SkeletalMesh))
	{
		FScopedSkeletalMeshPostEditChange SkeletalMeshPostEditChange(&SkeletalMesh);

		if (const FSkeletalMeshModel* const Mesh = SkeletalMesh.GetImportedModel())
		{
			const int32 NumLods = Mesh->LODModels.Num();
			for (int32 LodIndex = 0; LodIndex < NumLods; ++LodIndex)
			{
				UnbindFromSkeletalMesh(Asset, SkeletalMesh, LodIndex);
			}
		}
	}
}

void ClothingAssetUtils::UnbindFromSkeletalMesh(const UClothingAssetBase& Asset, USkeletalMesh& SkeletalMesh, const int32 MeshLodIndex)
{
	if (HasAnyLodMeshClothingAssetBindings(&SkeletalMesh, MeshLodIndex))
	{
		FSkeletalMeshModel* const Mesh = SkeletalMesh.GetImportedModel();
		if (!Mesh || !Mesh->LODModels.IsValidIndex(MeshLodIndex))
		{
			UE_LOG(LogClothingAsset, Warning, TEXT("Failed to remove clothing asset [%s] from mesh LOD%d as that LOD doesn't exist."),
				*Asset.GetName(),
				MeshLodIndex);
			return;
		}
		...
		FSkeletalMeshLODModel& LodModel = Mesh->LODModels[MeshLodIndex];
		...
		for (int32 SectionIdx = LodModel.Sections.Num() - 1; SectionIdx >= 0; --SectionIdx)
		{
			...
			if (Section.HasClothingData() && Section.ClothingData.AssetGuid == Asset.GetAssetGuid())
			{
				...
				// Log the LOD bias mapping data that will be removed
				for (int32 LodIndex = 0; LodIndex < MeshLodIndex; ++LodIndex)
				{
					const int32 LODBias = MeshLodIndex - LodIndex;
				...
				if (FSkelMeshSourceSectionUserData* const UserSectionData = LodModel.UserSectionsData.Find(Section.OriginalDataSectionIndex))
				{
				...
				ClothingAssetUtils::ClearLODBiasMappings(SkeletalMesh, MeshLodIndex, SectionIdx);
...
bool ClothingAssetUtils::HasAnyMeshClothingAssetBindings(const USkeletalMesh* SkeletalMesh)
{
	if (const FSkeletalMeshModel* const MeshModel = SkeletalMesh ? SkeletalMesh->GetImportedModel() : nullptr)
	{
		for (int32 LodIndex = 0; LodIndex < MeshModel->LODModels.Num(); ++LodIndex)
		{
			if (MeshModel->LODModels[LodIndex].HasClothData())
			{
				return true;
			}
		}
...
bool ClothingAssetUtils::HasAnyLodMeshClothingAssetBindings(const USkeletalMesh* SkeletalMesh, int32 LodIndex)
{
	if (const FSkeletalMeshModel* const MeshModel = SkeletalMesh ? SkeletalMesh->GetImportedModel() : nullptr)
	{
		if (MeshModel->LODModels.IsValidIndex(LodIndex) && MeshModel->LODModels[LodIndex].HasClothData())
		{
			return true;
		}
	}
	...
	if (MeshLODModel.HasClothData())
	{
		...
		for (int32 SectionIndex = 0; SectionIndex < SectionNum; ++SectionIndex)
		{
			...
			if (Section.HasClothingData())
			{
				...
				ClothBinding.Asset = SkeletalMesh->GetClothingAsset(Section.ClothingData.AssetGuid);
				ClothBinding.AssetInternalLodIndex = Section.ClothingData.AssetLodIndex;
...
void ClothingAssetUtils::ClearSectionClothingData(FSkelMeshSection& Section)
{
	Section.ClothingData.AssetGuid = FGuid();
	Section.ClothingData.AssetLodIndex = INDEX_NONE;
	Section.CorrespondClothAssetIndex = INDEX_NONE;
...
void ClothingAssetUtils::ClearLODBiasMappings(USkeletalMesh& SkeletalMesh, const int32 UpdatedLodIndex, const int32 SectionIndex)
{
	if (FSkeletalMeshModel* const SkeletalMeshModel = SkeletalMesh.GetImportedModel())
	{
		if (SkeletalMeshModel->LODModels.IsValidIndex(UpdatedLodIndex))
		{
			for (int32 LODIndex = UpdatedLodIndex + 1; LODIndex < SkeletalMeshModel->LODModels.Num(); ++LODIndex)
			{
				...
				if (BiasLodModel.Sections.IsValidIndex(SectionIndex))
				{
					...
					if (BiasSection.ClothMappingDataLODs.IsValidIndex(LODBias))
					{
						...
						const UClothingAssetBase* const Asset = SkeletalMesh.GetClothingAsset(BiasSection.ClothingData.AssetGuid);

						UE_LOG(LogClothingAsset, Verbose, TEXT("Removed deformer data for [%s/%s], section %d LODBias = %d, render mesh LOD = %d, sim mesh LOD = %d"),
							*SkeletalMesh.GetName(),
							Asset ? *Asset->GetName() : TEXT("?"),
							SectionIndex,
							LODBias,
							LODIndex,
							UpdatedLodIndex);
...
void UClothingAssetCommon::UnbindFromSkeletalMesh(USkeletalMesh* SkeletalMesh)
{
	// Clear the entire LOD map to allow for rebinding a new asset
	for (int32& LodMapIndex : LodMap)
	{
		LodMapIndex = INDEX_NONE;  // Note: This must be done prior calling UnbindFromSkeletalMesh(), so that the info is correct when rebuilding the mesh
	}

	// Unbind from the Skeletal Mesh
	if (SkeletalMesh)
	{
...
void UClothingAssetCommon::UnbindFromSkeletalMesh(USkeletalMesh* SkeletalMesh, const int32 MeshLodIndex)
{
	// Clear the LOD map entry to allow for rebinding a new asset
	if (LodMap.IsValidIndex(MeshLodIndex))
	{
		LodMap[MeshLodIndex] = INDEX_NONE;  // Note: This must be done prior calling UnbindFromSkeletalMesh(), so that the info is correct when rebuilding the mesh
	}
	// Unbind from the Skeletal Mesh
	if (SkeletalMesh)
	{
	...
	if(USkeletalMesh* OwnerMesh = Cast<USkeletalMesh>(GetOuter()))
	{
		for(TObjectIterator<USkeletalMeshComponent> It; It; ++It)
		{
			if(USkeletalMeshComponent* Component = *It)
			{
				if(Component->GetSkeletalMeshAsset() == OwnerMesh)
				{
					if(UClothingSimulationInteractor* CurInteractor = Component->GetClothingSimulationInteractor(GetFName()))
					{
						Func(CurInteractor);
					}
				}
...
void UClothingAssetCommon::PostLoad()
{
	...
	{
		// Use the skeletal mesh version here to prevent a circular dependency between ClothingAssetCommon and Engine/SkeletalMesh
		// and to unify all cached data invalidations, while the InvalidateCachedData method is also deprecated from ClothingAssetBase.
		if (SkeletalMesh->GetLinkerCustomVersion(FSkeletalMeshCustomVersion::GUID) < FSkeletalMeshCustomVersion::CachedClothInverseMasses)
		{
...
bool UClothingAssetCommon::AddClothConfigs()
{
	...
	for (IClothingSimulationFactoryClassProvider* Provider : ClassProviders)
	{
		...
		{
			const UClothingSimulationFactory* const ClothingSimulationFactory = ClothingSimulationFactoryClass->GetDefaultObject<UClothingSimulationFactory>();
			if (!ClothingSimulationFactory || !ClothingSimulationFactory->SupportsAsset(this))
			{
				continue;  // No need to add configs for simulations that don't support this asset
			}
			for (TSubclassOf<UClothConfigBase> ClothConfigClass : ClothingSimulationFactory->GetClothConfigClasses())
			{
				...
				if (!ClothConfigPtr || !*ClothConfigPtr)
				{
					...
					checkf(!StaticFindObject(ClothConfigClass, this, *ClothConfigClass->GetName(), EFindObjectFlags::ExactClass), TEXT("Found an existing instance of ClothConfigClass in %s. Class:%s"), *GetPathNameSafe(this), *ClothConfigClass->GetName());

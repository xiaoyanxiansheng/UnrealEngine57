...
TArray<uint8> FAnalyticsProviderETEventCache::CreateImmediatePayload(const FString& EventName, const TArray<FAnalyticsEventAttribute>& Attributes) const
{
	TSharedRef<FAnalyticsProviderETEventCache> Clone = CloneForImmediateMode();
	Clone->AddToCache(EventName, Attributes);
	return Clone->FlushCacheUTF8();
	// the Clone will be released here
}
...
TSharedRef<FAnalyticsProviderETEventCache> FAnalyticsProviderETEventCache::CloneForImmediateMode() const
{
	// need to make this because FAnalyticsProviderETEventCache is not allowed to be copyed because
	// FTransactionallySafeCriticalSection CachedEventsCS depends on FPlatformSharedMutex which has deleted copy and assignment operators.
	TSharedRef<FAnalyticsProviderETEventCache> Copy = MakeShared<FAnalyticsProviderETEventCache>(MaximumPayloadSize, PreallocatedPayloadSize);
	Copy->CachedDefaultAttributeUTF8Stream = CachedDefaultAttributeUTF8Stream;
	return Copy;
}

...
/**
 * Enum describing the supported record event modes
 * @enum Cached use the existing system where the events are cached and flushed when possible
 * @enum Immediate use the new system where events are immediately flushed to a HTTP request
 */
enum class EAnalyticsRecordEventMode : uint8
{
	Cached,
	Immediate
};
...
public:
	 ...
	 * @param EventName name of the event
	 * @param Attributes array of attribute name/value pairs
	 * @param Mode the record event mode, Cached or Immediate @see EAnalyticsRecordEventMode
	 */

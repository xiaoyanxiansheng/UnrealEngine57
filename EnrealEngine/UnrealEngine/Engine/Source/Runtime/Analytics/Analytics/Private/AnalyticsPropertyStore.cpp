...
{
	/** The key/value storage format version defining the memory layout of the store. */
	...
	/** Conditionally set a value in the store. */
	...
	/** Conditionally update a value in the store. */
	template<typename T, typename TGetter, typename TSetter, typename TCompare>
...
FAnalyticsPropertyStore::~FAnalyticsPropertyStore()
{
	...
	{
		// The FAnalyticsPropertyStore must not be destroyed from within a transaction, 
		// since any queued-up flush tasks won't be run. 
		AutoRTFM::PopAllOnCommitHandlers(this);
		ensure(!"Destroying FAnalyticsPropertyStore from within a transaction!");
	}
	else
	{
		// Ensure any running async flush has completed.
		UE::TScopeLock Lock(StoreLock);
...
bool FAnalyticsPropertyStore::Flush(bool bAsync, const FTimespan& Timeout)
{
	...
	// If we are inside an AutoRTFM transaction, we can't flush yet, since this potentially involves writing
	// to disk and spinning up threads. 
	if (AutoRTFM::IsClosed())
	{
		// If we have the option to decline, just return false.
		if (Timeout < FTimespan::MaxValue())
		{
			return false;
		}

		// The caller expects the flush to succeed; queue up a blocking flush.
		AutoRTFM::PushOnCommitHandler(this, [this, bAsync]
		{
			Flush(bAsync, FTimespan::MaxValue());
		});
		return true;
	}
	...
	auto FlushLockedFn = [this, bAsync](bool bNoWaiting)
	{
		...
		if (FlushTask.IsValid())
		{
			// If the task completed or can be successfully canceled.
	...
	if (Timeout == FTimespan::MaxValue())
	{
		UE::TScopeLock ScopeLock(StoreLock);

...
int32 UUserObjectListEntryLibrary::GetListItemIndex(TScriptInterface<IUserObjectListEntry> UserObjectListEntry)
{
	if (UUserWidget* EntryWidget = Cast<UUserWidget>(UserObjectListEntry.GetObject()))
	{
		if (const UListView* OwningListView = Cast<UListView>(UUserListEntryLibrary::GetOwningListView(EntryWidget)))
		{
			if (UObject* Item = UUserObjectListEntryLibrary::GetListItemObject(UserObjectListEntry))
			{
				return OwningListView->GetIndexForItem(Item);
			}
		}
	}
	return INDEX_NONE;
...
bool UUserObjectListEntryLibrary::IsFirstWidget(TScriptInterface<IUserObjectListEntry> UserObjectListEntry)
{
	return UUserObjectListEntryLibrary::GetListItemIndex(UserObjectListEntry) == 0;
}

bool UUserObjectListEntryLibrary::IsLastWidget(TScriptInterface<IUserObjectListEntry> UserObjectListEntry)
{
	if (UUserWidget* EntryWidget = Cast<UUserWidget>(UserObjectListEntry.GetObject()))
	{
		if (const UListView* OwningListView = Cast<UListView>(UUserListEntryLibrary::GetOwningListView(EntryWidget)))
		{
			if (UObject* Item = UUserObjectListEntryLibrary::GetListItemObject(UserObjectListEntry))
			{
				const int32 Index = OwningListView->GetIndexForItem(Item);
				return Index != INDEX_NONE && Index == OwningListView->GetNumItems() - 1;
			}
		}
	}
	return false;

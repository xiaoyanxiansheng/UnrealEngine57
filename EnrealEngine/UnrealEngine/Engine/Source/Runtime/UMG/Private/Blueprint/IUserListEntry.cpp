...
UMG_API void IUserListEntry::UpdateEntryDropIndicator(UUserWidget& ListEntryWidget, TOptional<EItemDropZone> DropZone)
{
	if (ListEntryWidget.Implements<UUserListEntry>() && DropZone.IsSet())
	{
		Execute_BP_OnUpdateEntryDropIndicator(&ListEntryWidget, UDragDropOperation::ConvertSlateDropZoneToUMG(DropZone));
	}
}

UMG_API void IUserListEntry::UpdateEntryDragOverState(UUserWidget& ListEntryWidget, bool bIsDraggingOver)
{
	if (ListEntryWidget.Implements<UUserListEntry>())
	{
		Execute_BP_OnEntryDragOverChanged(&ListEntryWidget, bIsDraggingOver);
	}
}

UMG_API void IUserListEntry::EndEntryDropOperation(UUserWidget& ListEntryWidget, bool bSuccess)
{
	if (ListEntryWidget.Implements<UUserListEntry>())
	{
		Execute_BP_OnEndEntryDropOperation(&ListEntryWidget, bSuccess);
	}
}

UMG_API void IUserListEntry::HandleEntryDragged(UUserWidget& ListEntryWidget, UDragDropOperation* Operation)
{
	if (ListEntryWidget.Implements<UUserListEntry>())
	{
		Execute_BP_OnEntryDragged(&ListEntryWidget, Operation);
	}
}

UMG_API void IUserListEntry::HandleEntryDropped(UUserWidget& ListEntryWidget, UDragDropOperation* Operation)
{
	if (ListEntryWidget.Implements<UUserListEntry>())
	{
		Execute_BP_OnEntryDropped(&ListEntryWidget, Operation);
	}
}

...
UWidget* UWidgetTree::FindWidget(TSharedRef<SWidget> InWidget) const
{
	...
	ForEachWidget([&] (UWidget* Widget) {
		if ( Widget->GetCachedWrappedWidget() == InWidget )
		{
			FoundWidget = Widget;
		}
	});
...
void UWidgetTree::ForEachWidgetUntil(TFunctionRef<bool(UWidget*)> Predicate) const
{
	// Mimics UWidgetTree::ForWidget
	if ( RootWidget )
	{
		...
		if (!ForEachWidgetAndChildrenUntil(RootWidget, Predicate))
		{
			return;
		}
	}
	...
	for (const auto& NamedSlotsEntry : NamedSlotBindings)
	{
		if (UWidget* NamedSlotContent = NamedSlotsEntry.Value)
		{
			if(!Predicate(NamedSlotContent))
			{
				return;
			}
			
			if(!ForEachWidgetAndChildrenUntil(NamedSlotContent, Predicate))
			{
				return;
			}
		}
...
bool UWidgetTree::ForEachWidgetAndChildrenUntil(UWidget* Widget, TFunctionRef<bool(UWidget*)> Predicate)
{
	// Mimics UWidgetTree::ForWidgetAndChildren
	if ( INamedSlotInterface* NamedSlotHost = Cast<INamedSlotInterface>(Widget) )
	{
		TArray<FName> SlotNames;
		NamedSlotHost->GetSlotNames(SlotNames);
		...
		for ( FName SlotName : SlotNames )
		{
			if ( UWidget* SlotContent = NamedSlotHost->GetContentForSlot(SlotName) )
			{
				...
				if (!ForEachWidgetAndChildrenUntil(SlotContent, Predicate))
				{
					return false;
				}
			}
	...
	if ( UPanelWidget* PanelParent = Cast<UPanelWidget>(Widget) )
	{
		for ( int32 ChildIndex = 0; ChildIndex < PanelParent->GetChildrenCount(); ChildIndex++ )
		{
			if ( UWidget* ChildWidget = PanelParent->GetChildAt(ChildIndex) )
			{
				...
				if (!ForEachWidgetAndChildrenUntil(ChildWidget, Predicate))
				{
					return false;
				}
			}

...
void UWidget::SetNavigationMethod(const TInstancedStruct<FNavigationMethod>& NavigationMethod)
{
	...
	Navigation->NavigationMethod = NavigationMethod;
...
TSharedRef<SWidget> UWidget::TakeWidget_Private(ConstructMethodType ConstructMethod)
{
		...
		if (const UUserWidget* UserWidget = WidgetTree ? Cast<UUserWidget>(WidgetTree->GetOuter()) : nullptr)
		{
			if (UUIComponentUserWidgetExtension* Extension = UserWidget->GetExtension<UUIComponentUserWidgetExtension>())
			{
				for (UUIComponent* Component : Extension->GetComponentsFor(this))
				{
					if (bNewlyCreated)
					{
						TSharedRef<SWidget> SafeComponentWidget = Component->RebuildWidgetWithContent(PublicWidget.ToSharedRef());
						...
						if (SafeComponentWidget != PublicWidget)
						{
							...
							if (IsDesignTime())
							{
								SafeComponentWidget->SetVisibility(EVisibility::Visible);
							}
#endif						
	...
	if (IsDesignTime())
	{
		if (bNewlyCreated)
		{
			TSharedRef<SWidget> SafeDesignWidget = RebuildDesignWidget(PublicWidget.ToSharedRef());

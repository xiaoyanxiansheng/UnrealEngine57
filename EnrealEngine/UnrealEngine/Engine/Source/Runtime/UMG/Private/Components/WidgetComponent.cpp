...
void UWidgetComponent::CollectPSOPrecacheData(const FPSOPrecacheParams& BasePrecachePSOParams, FMaterialInterfacePSOPrecacheParamsList& OutParams)
{
	if (MaterialInstance)
	{
		...
		else
		{
			...
			VtxElements.Add(FVertexElement(2, 0, VET_Color, 3, sizeof(FColor)));
			VtxElements.Add(FVertexElement(3, 0, VET_Float2, 4, sizeof(FVector2f)));
			VtxElements.Add(FVertexElement(3, 0, VET_Float2, 5, sizeof(FVector2f)));
			VtxElements.Add(FVertexElement(3, 0, VET_Float2, 6, sizeof(FVector2f)));
			VtxElements.Add(FVertexElement(3, 0, VET_Float2, 7, sizeof(FVector2f)));
			VtxElements.Add(FVertexElement(3, 0, VET_Float2, 15, sizeof(FVector2f)));
			if (IsGPUSkinPassThroughSupported(GMaxRHIShaderPlatform))
			{
				VtxElements.Add(FVertexElement(4, 0, VET_Float4, 14, 0));
				VtxElements.Add(FVertexElement(5, 0, VET_UInt, 13, 0, true));
			}
			else
			{
				VtxElements.Add(FVertexElement(4, 0, VET_UInt, 13, 0, true));
			}
...
void UWidgetComponent::OnWorldUserEntered(int32 SlateUserIndex)
{
	UsersEnteredWorldWidget.Add(SlateUserIndex);
}

void UWidgetComponent::ResetWorldUserFocusOnWidget()
{
	// we want to go through all the users which could've focused
	// on our widget to release any focus paths/strongs refs they might have to our widget.
	for(int32 UserIndex : UsersEnteredWorldWidget)
	{
		...
		TSharedPtr<SWidget> CurrentWidgetInPath = SlateUser->GetFocusedWidget();
		while(CurrentWidgetInPath != nullptr)
		{
			if(CurrentWidgetInPath == SlateWindow)
			{
				FSlateApplication::Get().ClearUserFocus(UserIndex, EFocusCause::SetDirectly);

				break;
			}
			
			CurrentWidgetInPath = CurrentWidgetInPath->GetParentWidget();
		}
	}
	UsersEnteredWorldWidget.Reset();
}

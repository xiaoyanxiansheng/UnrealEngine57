...
UMG_API void UListViewBase::CancelListViewDragDrop()
{
	if (bIsDragging)
	{
		bIsDragging = false;
...
UDragDropOperation* UListViewBase::CreateDragDropOperation(UObject* Item)
{
	if (DragDropOperationClass)
	{
		UDragDropOperation* Operation = UWidgetBlueprintLibrary::CreateDragDropOperation(DragDropOperationClass);
		Operation->Pivot = DragDropVisualPivot;
		Operation->Offset = DragDropVisualOffset;
		...
		{
			TSubclassOf<UUserWidget> DragVisualClass = (DragDropVisualEntryClass) ? DragDropVisualEntryClass : EntryWidgetClass;
			Operation->DefaultDragVisual = CreateWidget<UUserWidget>(GetWorld(), DragVisualClass);
		}
		return Operation;
	}
...
void UListViewBase::FinishGeneratingEntry(UUserWidget& GeneratedEntry)
{
	if (!IsDesignTime())
	{
		...
		if (!EntryGenAnnouncementDelegateHandle.IsValid())
		{
			if (UWorld* World = GetWorld())
			{
				EntryGenAnnouncementDelegateHandle = FTSTicker::GetCoreTicker().AddTicker(FTickerDelegate::CreateWeakLambda(this, [this](float InDelta)
				{
...
{
	EntryGenAnnouncementDelegateHandle.Reset();

...
#if WITH_EDITOR
	FWorldDelegates::OnPIEEnded.AddUObject(this, &UWidgetInteractionComponent::OnPIEEnded);
#endif
}
...
#if WITH_EDITOR
void UWidgetInteractionComponent::OnPIEEnded(UGameInstance*)
{
	if(LastWidgetPath.IsValid() && VirtualUser.IsValid())
	{
		TSharedPtr<SWidget> LastVirtualUserWidget = LastWidgetPath.GetLastWidget().Pin();
		int32 UserIndex = VirtualUser->GetUserIndex();
		
		if (LastVirtualUserWidget != nullptr && LastVirtualUserWidget->HasUserFocus(UserIndex))
		{
...
FWidgetPath UWidgetInteractionComponent::DetermineWidgetUnderPointer()
{
	...
	if ( NewHoveredWidgetComponent != OldHoveredWidgetComponent )
	{
		...
		if ( NewHoveredWidgetComponent )
		{
			if(VirtualUser.IsValid())
			{
				NewHoveredWidgetComponent->OnWorldUserEntered(VirtualUser->GetUserIndex());
			}
		}

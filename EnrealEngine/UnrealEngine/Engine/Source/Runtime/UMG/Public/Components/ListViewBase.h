...
protected:
		...
		const FListViewConstructArgs& Args = FListViewConstructArgs())
	{
		...
		TSharedRef<ListViewT<ItemType>> ListView = SNew(ListViewT<ItemType>)
			...
			.bEnableShadowBoxStyle(Args.bEnableShadowBoxStyle)
			.ListViewStyle(Args.ListViewStyle)
			.ShadowBoxStyle(Args.ShadowBoxStyle)
...
public:
	...
	// Creates the drag drop operation for the list view.
	UFUNCTION(Category = ListViewBase)
	UDragDropOperation* CreateDragDropOperation(UObject* Item);
...
protected:
	...
	virtual UDragDropOperation* HandleListEntryDragDetected(const FGeometry& MyGeometry, const FPointerEvent& PointerEvent, UUserWidget& EntryWidget) { return nullptr; };
	virtual TOptional<EItemDropZone> HandleListEntryCanAcceptDrop(const FDragDropEvent& DropEvent, EItemDropZone DropZone, UUserWidget& EntryWidget) { return NullOpt; };
	virtual FReply HandleListEntryAcceptDrop(const FDragDropEvent& DropEvent, EItemDropZone DropZone, UUserWidget& EntryWidget) { return FReply::Unhandled(); };
	...
	*/
		...
		WidgetEntryT* ListEntryWidget = EntryWidgetPool.GetOrCreateInstance<WidgetEntryT>(*WidgetClass,
			[this, &OwnerTable] (UUserWidget* WidgetObject, TSharedRef<SWidget> Content)
			{
				return SNew(ObjectTableRowT, OwnerTable, *WidgetObject, this)
					...
					.bAllowDragDrop(bAllowDragDrop)
					.bAllowKeepPreselectedItems(bAllowKeepPreselectedItems)
					...
					.OnRowDragDetected_UObject(this, &UListViewBase::HandleListEntryDragDetected)
					.OnRowCanAcceptDrop_UObject(this, &UListViewBase::HandleListEntryCanAcceptDrop)
					.OnRowAcceptDrop_UObject(this, &UListViewBase::HandleListEntryAcceptDrop)
					.OnRowDragEnter_UObject(this, &UListViewBase::HandleListEntryDragEnter)
					.OnRowDragLeave_UObject(this, &UListViewBase::HandleListEntryDragLeave)
					.OnRowDragCancelled_UObject(this, &UListViewBase::HandleListEntryDragCancelled)
					[
						Content
					];
			});
	...
	/** Allow drag and drop operations to be performed on the list. */
	...
	/** The class which will be used to generate the drag drop visual.  If none is specified, the EntryWidgetClass will be used. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Drag and Drop", meta = (AllowPrivateAccess = true, MustImplement = "/Script/UMG.UserListEntry"))
	TSubclassOf<UUserWidget> DragDropVisualEntryClass;

	/** The drag drop operation class for this listview - if this is not specified, drag drop operations will not be created. */
	...
	/** This is the actual drag visual widget that will be used by the drag drop operation.This can be set in blueprint via override of CreateDragDropOperation(should call parent)*/
	UPROPERTY(BlueprintReadWrite, Transient, Category = "Drag and Drop")
	TObjectPtr<class UWidget> DragVisualWidget = nullptr;

	/** This indicates whether the ListViewBase is currently in a dragging state*/
	UPROPERTY(BlueprintReadOnly, Transient, Category = "Drag and Drop")
	...
	/** If true, when selecting an item via mouse button, we allow pre-selected items to remain selected (applicable to "Multi" Selection Mode only). */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = ListView)
...
private:
	...
	FTSTicker::FDelegateHandle EntryGenAnnouncementDelegateHandle;
...
private:	\
	...
	mutable FSimpleListItemEvent OnItemDragDetectedEvent;	\
	mutable FSimpleListItemEvent OnItemDragEnterEvent;	\
	mutable FSimpleListItemEvent OnItemDragLeaveEvent;	\
	mutable FOnItemDragCancelled OnItemDragCancelledEvent;	\
	mutable FOnItemCanAcceptDrop OnItemCanAcceptDropEvent;	\
	mutable FSimpleListItemEvent OnItemAcceptDropEvent;	\
...
public:	\
	...
	virtual FSimpleListItemEvent& OnItemDragDetected() const override { return OnItemDragDetectedEvent; }	\
	virtual FSimpleListItemEvent& OnItemDragEnter() const override { return OnItemDragEnterEvent; }	\
	virtual FSimpleListItemEvent& OnItemDragLeave() const override { return OnItemDragLeaveEvent; }	\
	virtual FOnItemDragCancelled& OnItemDragCancelled() const override { return OnItemDragCancelledEvent; }	\
	virtual FOnItemCanAcceptDrop& OnItemCanAcceptDrop() const override { return OnItemCanAcceptDropEvent; }	\
	virtual FSimpleListItemEvent& OnItemAcceptDrop() const override { return OnItemAcceptDropEvent; }	\

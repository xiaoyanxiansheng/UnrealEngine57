...
public:
	 ...
	 *	@param NavigationMethodStruct. Must be the StaticStruct of type FNavigationMethod
	 */
...
public:
	...
	inline EWidgetDesignFlags GetDesignerFlags() const
	{
		return static_cast<EWidgetDesignFlags>(DesignerFlags);
	}

	/** Tests if any of the flags exist on this widget. */
	inline bool HasAnyDesignerFlags(EWidgetDesignFlags FlagsToCheck) const
	{
...
protected:
	inline bool CanSafelyRouteCall() { return !(GIntraFrameDebuggingGameThread || IsUnreachable() || FUObjectThreadContext::Get().IsRoutingPostLoad); }

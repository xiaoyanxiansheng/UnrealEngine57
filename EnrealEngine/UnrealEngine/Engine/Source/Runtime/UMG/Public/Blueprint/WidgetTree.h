...
public:
	...
	inline WidgetT* FindWidget(const FName& Name) const
	{
	...
	/**
	 * Iterates widgets according to ForEachWidget until Predicate returns false.
	 */
	void ForEachWidgetUntil(TFunctionRef<bool(UWidget*)> Predicate) const;

	/**
	 * Iterates widgets according to ForWidgetAndChildren until Predicate returns false.
	 */
	static UMG_API bool ForEachWidgetAndChildrenUntil(UWidget* Widget, TFunctionRef<bool(UWidget*)> Predicate);

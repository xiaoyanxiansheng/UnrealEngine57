...
public:
	...
	static UUserWidget* GetOuterUserWidget(const UWidget* Widget);
};
...
private:
	...
	/** True when we are ticking our active animations */
	bool bIsTickingAnimations = false;
...
protected:
	...
	inline FVector2D GetMinimumDesiredSize() const { return MinimumDesiredSize; }
...
protected:
	...
	UE_DEPRECATED(5.7, "InitializeInputComponent is deprecated. bAutomaticallyRegisterInputOnConstruction will be set on compilation of this widget and automatically create the input component instead.")
	UMG_API virtual void InitializeInputComponent() final;
...
protected:
	 ...
	 * 
	 * bAutomaticallyRegisterInputOnConstruction is true when there are input event nodes in the graph
	 * of this widget, which would in turn need to be bound in order to receive their callbacks.
	 *
	 * This should only happen once per widget when it is spawned and initialized.
	 */
	UMG_API virtual void CreateInputComponent();

	/**
	 * Pushes the input component to the input stack and allows for any
	 * script delegates on this widget to get called.
	 */
	UMG_API virtual void StartProcessingInputScriptDelegates();

	/**
	 * Removes this widget's input component from the input stack,
	 * stopping any script delegates from being called.
	 * 
	 * This does not actually destroy or unbind any of the delegates yet, because the widget may be added 
	 * to the viewport again the future, and the binding process can be expensive.
	 */
	UMG_API virtual void StopProcessingInputScriptDelegates();

	/**
	* Unbinds all script delegates that this widget has bound to on its input component,
	* removes it from the input stack, and deletes the actual input component object.
	*/
	UMG_API virtual void DestroyInputComponent();

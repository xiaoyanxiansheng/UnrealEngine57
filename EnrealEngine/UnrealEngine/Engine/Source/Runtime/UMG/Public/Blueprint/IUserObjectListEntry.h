...
public:
	...
	UFUNCTION(BlueprintPure, Category = UserObjectListEntry, meta = (DefaultToSelf = UserObjectListEntry))
	static UMG_API int32 GetListItemIndex(TScriptInterface<IUserObjectListEntry> UserObjectListEntry);

	/** Returns true if the widget list entry is the first item in the list view */
	UFUNCTION(BlueprintPure, Category = UserObjectListEntry, meta = (DefaultToSelf = UserObjectListEntry))
	static UMG_API bool IsFirstWidget(TScriptInterface<IUserObjectListEntry> UserObjectListEntry);

	/** Returns true if the widget list entry is the last item in the list view */
	UFUNCTION(BlueprintPure, Category = UserObjectListEntry, meta = (DefaultToSelf = UserObjectListEntry))
	static UMG_API bool IsLastWidget(TScriptInterface<IUserObjectListEntry> UserObjectListEntry);
};

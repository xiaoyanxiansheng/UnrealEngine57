...
void FAppEventManager::SetAppIsActive(bool bIsActive)
{
	bAppIsActive = bIsActive;
}

void FAppEventManager::ResumePausedGameThread()
{
	if (bAppIsActive || bTempRunningTriggered)
	{
		//UE_LOG(LogAndroid, Log, TEXT("Resume_GT_Temp bypassed. %d, %d, %d"), (bool)bAppIsActive, (bool)bRunning, (bool)bTempRunningTriggered);
		return;
	}

	EnqueueAppEvent(APP_EVENT_STATE_APP_ACTIVATED);
	...
	bTempRunningTriggered = true;

	//UE_LOG(LogAndroid, Log, TEXT("Resume_GT_Temp done. %d, %d, %d"), (bool)bAppIsActive, (bool)bRunning, (bool)bTempRunningTriggered);
}

void FAppEventManager::KeepGameThreadPaused()
{
	if (bRunning && !bAppIsActive && bTempRunningTriggered)
	{
		EnqueueAppEvent(APP_EVENT_STATE_APP_SUSPENDED);
		//UE_LOG(LogAndroid, Log, TEXT("Pause_GT_Temp done. %d, %d, %d"), (bool)bAppIsActive, (bool)bRunning, (bool)bTempRunningTriggered);
	}
	else
	{
		//UE_LOG(LogAndroid, Log, TEXT("Pause_GT_Temp bypassed. %d, %d, %d"), (bool)bAppIsActive, (bool)bRunning, (bool)bTempRunningTriggered);
	}

	bTempRunningTriggered = false;
}
...
void FAppEventManager::Tick()
{
	...
	while (!Queue.IsEmpty())
	{
		...
		case APP_EVENT_STATE_WINDOW_CHANGED:
			...
			// see 'UE::Jni::FGameActivity::nativeOnOrientationChanged' for event thread/game thread mismatches.
			ExecWindowResized();
		break;

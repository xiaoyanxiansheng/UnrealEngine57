...
FD3D11Texture* FD3D11Viewport::GetSwapChainSurface(FD3D11DynamicRHI* D3DRHI, EPixelFormat PixelFormat, uint32 SizeX, uint32 SizeY, IDXGISwapChain* SwapChain)
{
	...
	bool const bQuadBufferStereo = D3DRHI->IsQuadBufferStereoEnabled();
	...
	const FRHITextureCreateDesc CreateDesc =
		FRHITextureCreateDesc::Create(TEXT("FD3D11Viewport::GetSwapChainSurface"),
			bQuadBufferStereo ? ETextureDimension::Texture2DArray : ETextureDimension::Texture2D)
		.SetArraySize(bQuadBufferStereo ? 2 : 1)
		.SetExtent(FIntPoint(TextureDesc.Width, TextureDesc.Height))
...
void FD3D11DynamicRHI::RHIAdvanceFrameForGetViewportBackBuffer(FRHIViewport* Viewport, bool bPresent)
{
}

...
struct FD3D11ResourceBinder
{
	...
	void SetSRV(FRHIShaderResourceView* InShaderResourceView, uint8 Index)
	{
		...
		ID3D11ShaderResourceView* D3D11SRV = D3D11ShaderResourceView ? D3D11ShaderResourceView->View.GetReference() : nullptr;
...
void FD3D11DynamicRHI::InternalSetUAVCS(uint32 BindIndex, FD3D11UnorderedAccessView* UnorderedAccessViewRHI)
{
	...
	ID3D11UnorderedAccessView* D3D11UAV = UnorderedAccessViewRHI ? UnorderedAccessViewRHI->View.GetReference() : nullptr;
	...
	const FRHIDepthRenderTargetView* NewDepthStencilTargetRHI)
{
	...
	for(uint32 RenderTargetIndex = 0;RenderTargetIndex < MaxSimultaneousRenderTargets;++RenderTargetIndex)
	{
		...
		if(RenderTargetIndex < NewNumSimultaneousRenderTargets && NewRenderTargetsRHI[RenderTargetIndex].Texture != nullptr) //-V522
		{
...
void FD3D11GPUFence::Wait(FRHICommandListImmediate& RHICmdList, FRHIGPUMask GPUMask) const
{
	if (Event && !Event->IsComplete())
	{
		...
		//
		// Debug code to investigate FORT-856705 and FORT-900082
		// 
		//  - Canary value to determine if the below EnqueueLambda command has actually executed before the hang detector fires.
		//  - Volatile to ensure this appears in the minidump on the render thread's stack.
		//  - Critical section and TSharedPtr logic to ensure the RHI thread doesn't set the value on the RT's stack if the RT returns before the command runs.
		//
		volatile uint64 Canary = 0x3344334433443344;

		struct FDebugScope
		{
			FCriticalSection CS;
			volatile uint64* Ptr = nullptr;
		};

		TSharedPtr<FDebugScope> ScopeRef = MakeShared<FDebugScope>();
		ScopeRef->Ptr = &Canary;

		{
			RHICmdList.EnqueueLambda([Event = Event, Ref = ScopeRef.ToWeakPtr()](FRHICommandListImmediate&)
			{
				if (Event == nullptr)
				{
					UE_LOG(LogD3D11RHI, Fatal, TEXT("FD3D11GPUFence::Wait Lambda - Event is nullptr."));
				}
				...
				// Mark this command as having executed
				if (TSharedPtr<FDebugScope> Ptr = Ref.Pin())
				{
					FScopeLock Lock(&Ptr->CS);
					if (Ptr->Ptr != nullptr)
					{
						*Ptr->Ptr = 0x7788778877887788;
					}
				}
			});
		...
		FScopeLock Lock(&ScopeRef->CS);
		ScopeRef->Ptr = nullptr;
		Canary = 0x5566556655665566;
	}
}
...
void FD3D11GPUFence::PollFencesUntil(FGraphEvent* Target)
{
	...
	// Flush the queue if we have a specific fence to wait for, otherwise the commands that signal the
	// fence may be stuck in the driver's command queue and never reach the GPU, causing an infinite loop.
	UINT const Flags = Target ? 0 : D3D11_ASYNC_GETDATA_DONOTFLUSH;
	...
	Retry:
		...
		bool bFoundTarget = Sync->Event == Target;
	...
	UE_CLOG(Target, LogD3D11RHI, Fatal, TEXT("FD3D11GPUFence::PollFencesUntil - Attempt to poll for a specific fence (0x%p), but it was not found in the queue."), Target);
}

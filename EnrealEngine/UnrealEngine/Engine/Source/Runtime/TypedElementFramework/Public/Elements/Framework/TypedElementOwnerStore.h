...
{
public:
	explicit TTypedElementOwnerScopedAccessImpl(FTransactionallySafeCriticalSection* InSynchObject)
	...
	inline ElementOwnerType& operator*() const
	{
		check(ElementOwner);
		return *ElementOwner;
	}
...
private:
	FTransactionallySafeCriticalSection* SynchObject = nullptr;
...
public:
	...
	TTypedElementOwner<ElementDataType> UnregisterElementOwner(const KeyDataType& InKey)
	{
		...
		{
			UE::TScopeLock Lock(ElementOwnerMapCS);
...
private:
	...
	mutable FTransactionallySafeCriticalSection ElementOwnerMapCS;
};

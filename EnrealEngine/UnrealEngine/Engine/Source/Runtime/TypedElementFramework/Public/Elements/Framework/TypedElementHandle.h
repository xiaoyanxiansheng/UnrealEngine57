...
public:
	...
	inline bool IsDataOfType() const
	{
	 ...
	 */
	template <typename ElementDataType>
	inline const ElementDataType& GetDataChecked() const
	{
	...
	inline friend bool operator==(const FTypedElementHandle& InLHS, const FTypedElementHandle& InRHS)
	{
	...
	inline void Private_InitializeNoRef(const FTypedElementInternalData& InData)
	{
		DataPtr = &InData;
	}

	inline void Private_InitializeAddRef(const FTypedElementInternalData& InData)
	{
		Private_InitializeNoRef(InData);
		RegisterRef();
	}

	inline void Private_DestroyNoRef()
	{
		DataPtr = nullptr;
#if UE_TYPED_ELEMENT_HAS_REFTRACKING
	...
	inline void Private_DestroyReleaseRef()
	{
		UnregisterRef();
		Private_DestroyNoRef();
	}

	inline const FTypedElementInternalData* Private_GetInternalData() const
	{
		return DataPtr;
	}

private:
	inline void RegisterRef()
	{
		if (DataPtr)
		{
#if !UE_TYPED_ELEMENT_HAS_REFTRACKING
...
public:
	 ...
	 */
	inline BaseInterfaceType* GetInterface() const
	{
		return InterfacePtr;
	}
	...
	inline U& GetInterfaceChecked() const
	{
	...
	inline friend bool operator==(const TTypedElementBase& InLHS, const TTypedElementBase& InRHS)
	{
	...
	inline void Private_InitializeNoRef(const FTypedElementInternalData& InData, BaseInterfaceType* InInterfacePtr)
	{
		FTypedElementHandle::Private_InitializeNoRef(InData);
		InterfacePtr = InInterfacePtr;
	}

	inline void Private_InitializeAddRef(const FTypedElementInternalData& InData, BaseInterfaceType* InInterfacePtr)
	{
...
public:
	...
	inline friend bool operator==(const TTypedElementOwner& InLHS, const TTypedElementOwner& InRHS)
	{
		return InLHS.DataPtr == InRHS.DataPtr;
	}
	...
	inline void Private_InitializeNoRef(TTypedElementInternalData<ElementDataType>& InData)
	{
		DataPtr = &InData;
	}

	inline void Private_InitializeAddRef(TTypedElementInternalData<ElementDataType>& InData)
	{
		Private_InitializeNoRef(InData);
		RegisterRef();
	}
	...
	inline const TTypedElementInternalData<ElementDataType>* Private_GetInternalData() const
	{
		return DataPtr;
	}
...
public:
	...
	inline bool operator==(const FScriptTypedElementHandle& InOther) const
	{
	...
	inline void Private_Initialize(FScriptTypedElementInternalDataPtr&& InInternalData)
	{

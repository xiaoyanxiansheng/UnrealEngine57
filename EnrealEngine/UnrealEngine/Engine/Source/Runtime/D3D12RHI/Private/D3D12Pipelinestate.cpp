...
FD3D12PipelineState::~FD3D12PipelineState()
{
	...
	if (UsePSORefCounting())
	{
		PipelineState->AddRef(); // AddRef for DeferredDelete to take over the actual deletion
		FD3D12DynamicRHI::GetD3DRHI()->DeferredDelete(PipelineState);
	}

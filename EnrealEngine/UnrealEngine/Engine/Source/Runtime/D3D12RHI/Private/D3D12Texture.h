...
public:
	...
	void EmplaceSRV(D3D12_SHADER_RESOURCE_VIEW_DESC const& SRVDesc, FD3D12Texture* FirstLinkedObject)
	{
		...
		const ERHIDescriptorType DescriptorType = RHIDescriptorTypeFromD3D12ViewDesc(SRVDesc);
		...
		ShaderResourceView = MakeShared<FD3D12ShaderResourceView>(GetParentDevice(), FirstLinkedObject ? FirstLinkedObject->ShaderResourceView.Get() : nullptr, DescriptorType);

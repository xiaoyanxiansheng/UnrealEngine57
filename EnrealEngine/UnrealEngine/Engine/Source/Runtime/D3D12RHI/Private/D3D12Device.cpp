...
FD3D12Queue::FD3D12Queue(FD3D12Device* Device, ED3D12QueueType QueueType, int32 QueueIndex)
	: Device(Device)
	, QueueType(QueueType)
	...
	, TextureAllocator         (this, FRHIGPUMask::All())
{
	...
	for (uint32 QueueIndex = 0; QueueIndex < (uint32)ED3D12QueueType::Count; ++QueueIndex)
	{
		Queues.Emplace(this, (ED3D12QueueType)QueueIndex, QueueIndex);
	}
...
{
	ObjectPool.Contexts.CleanupResources();
	ObjectPool.Allocators.CleanupResources();
	ObjectPool.Lists.CleanupResources();
}
...
void FD3D12Device::BlockUntilIdle()
{
	...
	{
		FD3D12SyncPointRef SyncPoint = FD3D12SyncPoint::Create(ED3D12SyncPointType::GPUAndCPU, TEXT("BlockUntilGPUIdle"));

...
FD3D12UnorderedAccessView::FD3D12UnorderedAccessView(FD3D12Device* InDevice, FD3D12UnorderedAccessView* FirstLinkedObject, ERHIDescriptorType InDescriptorType)
...
{
	ID3D12Resource* Resource = ResourceInfo.Resource->GetResource();
	...
	const D3D12_RESOURCE_DESC* AliasResourceDesc =
		ResourceInfo.Resource->GetUAVAccessResourceDesc();

	if (AliasResourceDesc)
	{
		GetParentDevice()->CreateUnorderedAccessViewAlias(
			Resource,
			Counter,
			*AliasResourceDesc,
			D3DViewDesc,
...
FD3D12UnorderedAccessView_RHI::FD3D12UnorderedAccessView_RHI(FD3D12Device* InDevice, FRHIViewableResource* InResource, FRHIViewDesc const& InViewDesc, FD3D12UnorderedAccessView_RHI* FirstLinkedObject)
	...
	, FD3D12UnorderedAccessView(InDevice, FirstLinkedObject, RHIDescriptorTypeFromViewDesc(InViewDesc))
{
}
	...
	if (UAVDesc.ViewDimension == D3D12_UAV_DIMENSION_BUFFER)
	{
		if (UAVDesc.Format == DXGI_FORMAT_UNKNOWN || (UAVDesc.Buffer.Flags & D3D12_BUFFER_UAV_FLAG_RAW) != 0)
		{
			...
			RHICmdList.RunOnContext([UnorderedAccessView, ClearValues](FD3D12CommandContext& Context)
			{
				...
				Context.OpenIfNotAlready();

				// The DescriptorType argument specifies which descriptor heap to allocate the view's descriptor from.
				// If we won't actually need the descriptor (ie bindless is disabled), we can signal to not allocate it in the first place by specifying an Invalid descriptor type.
				ERHIDescriptorType DescriptorType = ERHIDescriptorType::Invalid;
#if PLATFORM_SUPPORTS_BINDLESS_RENDERING
				if (Context.StateCache.GetDescriptorCache()->IsUsingBindlessHeap())
				{
					DescriptorType = ERHIDescriptorType::BufferUAV;
				}
#endif

				// Scoped view will free the offline CPU handle once we return
				// Always single GPU object, so FirstLinkedObject is nullptr
				FD3D12UnorderedAccessView UAV(ParentDevice, nullptr, DescriptorType);
				...
				if (Context.StateCache.GetDescriptorCache()->IsUsingBindlessHeap())
				{
					...
					// Make sure a descriptor heap is actually bound. This will noop if they were bound previously.
					Context.StateCache.GetDescriptorCache()->SetDescriptorHeaps(ED3D12SetDescriptorHeapsFlags::Bindless);
...
#endif
				{
					...
					// Make sure a descriptor heap is actually bound. This will noop if they were bound previously.
					Context.StateCache.GetDescriptorCache()->SetDescriptorHeaps(ED3D12SetDescriptorHeapsFlags::None);

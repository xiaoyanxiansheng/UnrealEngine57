...
struct FD3D12ResourceInitConfig
{
	static FD3D12ResourceInitConfig CreateUpload()
	{
		...
		Config.InitialD3D12Access = ED3D12Access::GenericRead;
		return Config;
	}
	...
	bool operator==(const FD3D12ResourceInitConfig& InOther) const
	{
		return HeapType == InOther.HeapType &&
			...
			InitialD3D12Access   == InOther.InitialD3D12Access;
	}
...
public:
	...
	virtual void AllocateResource(
		...
		const TCHAR* InName,
		FD3D12ResourceLocation& ResourceLocation) override;
...
protected:
	...
	virtual FD3D12Resource* CreatePlacedResource(

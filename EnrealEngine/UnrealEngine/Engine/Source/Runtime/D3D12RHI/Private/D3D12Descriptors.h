...
inline D3D12_DESCRIPTOR_HEAP_TYPE Translate(ERHIDescriptorType InDescriptorType)
{
	const ERHIDescriptorTypeMask TypeMask = RHIDescriptorTypeMaskFromType(InDescriptorType);
	if (EnumHasAnyFlags(TypeMask, ERHIDescriptorTypeMask::CBV | ERHIDescriptorTypeMask::SRV | ERHIDescriptorTypeMask::UAV))
	{
		return D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
	}

	if (EnumHasAnyFlags(TypeMask, ERHIDescriptorTypeMask::Sampler))
	{
		return FD3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER;
	}
...
inline ERHIDescriptorTypeMask D3D12DescriptorTypeMaskFromHeapType(ERHIDescriptorHeapType InHeapType)
{
...
public:
	...
	ED3D12DescriptorHeapFlags   GetFlags() const { return Flags; }

	ERHIDescriptorTypeMask GetDescriptorTypeMask() const
	{
		return D3D12DescriptorTypeMaskFromHeapType(GetType());
	}
	...
	bool   IsSuballocation()   const { return bIsSuballocation; }

	uint32 GetMemorySize() const { return DescriptorSize * NumDescriptors; }
	...
	D3D12_GPU_DESCRIPTOR_HANDLE GetGPUSlotHandle(uint32 Slot) const { return CD3DX12_GPU_DESCRIPTOR_HANDLE(GpuBase, Slot, DescriptorSize); }

	bool CanBePooled() const { return EnumHasAnyFlags(GetFlags(), ED3D12DescriptorHeapFlags::Poolable); }
...
public:
	...
	{
		return HandlesAllocations(D3D12DescriptorTypeMaskFromHeapType(InHeapType)) && Heap->GetFlags() == InHeapFlags;
	}

	bool HandlesAllocationWithFlags(ERHIDescriptorType InDescriptorType, ED3D12DescriptorHeapFlags InHeapFlags) const
	{
		return HandlesAllocation(InDescriptorType) && Heap->GetFlags() == InHeapFlags;
	}
...
private:
	...
	TArray<FD3D12DescriptorManager, TFixedAllocator<2>> GlobalHeaps; // Use TFixedAllocator to prevent move of CRITICAL_SECTION

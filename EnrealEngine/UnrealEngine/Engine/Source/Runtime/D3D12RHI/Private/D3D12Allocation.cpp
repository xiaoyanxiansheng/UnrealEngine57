	...
	, HeapFullMessageDisplayed(false)
{
	...
	check(0 == (MinBlockSize % 16)); // Always 16 byte aligned
	...
	{
		{
			...
			VERIFYD3D12RESULT(
				Adapter->CreateBuffer(
					...
					TEXT("Resource Allocator Underlying Buffer"),
...
void FD3D12BuddyAllocator::Allocate(uint32 SizeInBytes, uint32 Alignment, FD3D12ResourceLocation& ResourceLocation)
{
	...
	// Need to always align to a multiple of 16
	Alignment = Align(FMath::Max(Alignment, 16u), 16u);

	// If the alignment doesn't match the block size
	if (MinBlockSize % Alignment != 0)
	{
	...
	if (AllocationBlockOffset % Alignment != 0)
	{
...
bool FD3D12BucketAllocator::TryAllocate(uint32 SizeInBytes, uint32 Alignment, FD3D12ResourceLocation& ResourceLocation)
{
	...
	// Alignment needs to be a multiple of 16 bytes
	Alignment = Align(FMath::Max(Alignment, 16u), 16u);
	...
	if (AlignedBlockOffset % Alignment != 0)
	{
...
void* FD3D12UploadHeapAllocator::AllocUploadResource(uint32 InSize, uint32 InAlignment, FD3D12ResourceLocation& ResourceLocation)
{
	...
	else
	{
		...
		BigBlockAllocator.AllocateResource(
			GetParentDevice()->GetGPUIndex(),
			D3D12_HEAP_TYPE_UPLOAD,
			ResourceDesc,
			InSize,
			InAlignment,
			ED3D12Access::GenericRead,
			ED3D12ResourceStateMode::SingleState,
			ED3D12Access::GenericRead,
			nullptr,
			nullptr,
...
FD3D12ResourceInitConfig FD3D12DefaultBufferPool::GetResourceAllocatorInitConfig(D3D12_HEAP_TYPE InHeapType, D3D12_RESOURCE_FLAGS InResourceFlags, EBufferUsageFlags InBufferUsage)
{
	...
	{
		// TODO: Can we do better than READ + WRITE all the time?
		// should only have this flag and no other flags
		check(InBufferUsage == BUF_AccelerationStructure);
		check(EnumHasAnyFlags(InResourceFlags, D3D12_RESOURCE_FLAG_RAYTRACING_ACCELERATION_STRUCTURE));
...
void FD3D12DefaultBufferPool::AllocDefaultResource(
	...
	FD3D12ResourceLocation& ResourceLocation)
{
	...
	{
		// @TODO - This isn't true anymore when using enhanced barriers and when using legacy barriers, it's the 
		// implementations responsibility now to deal with the API restriction. This can be re-written to use
		// BVHRead and BVHWrite as appropriate so that barriers implementations that can do better, will.
		...
		if (Allocator->TryAllocate(Desc.Width, Alignment, ResourceLocation))
		{
			if (bPlacedResource)
			{
				...
				VERIFYD3D12RESULT(Adapter->CreatePlacedResource(
					...
					HeapOffset,
					InCreateD3D12Access,
					// @TODO - Is this right? We're ignoring InResourceStateMode
...
FD3D12BufferPool* FD3D12DefaultBufferAllocator::CreateBufferPool(D3D12_HEAP_TYPE InHeapType, D3D12_RESOURCE_FLAGS InResourceFlags, EBufferUsageFlags InBufferUsage, ED3D12ResourceStateMode InResourceStateMode, uint32 Alignment)
{
	...
	if (EnumHasAnyFlags(InitConfig.InitialD3D12Access, ED3D12Access::BVHRead | ED3D12Access::BVHWrite))
	{
...
ED3D12Access FD3D12DefaultBufferAllocator::GetDefaultInitialD3D12Access(
	D3D12_HEAP_TYPE InHeapType,
...
// Grab a buffer from the available buffers or create a new buffer if none are available
void FD3D12DefaultBufferAllocator::AllocDefaultResource(
	D3D12_HEAP_TYPE InHeapType,
	...
	const TCHAR* Name)
{
	...
	if (EnumHasAnyFlags(InBufferUsage, BUF_ReservedResource))
	{
		...
		VERIFYD3D12RESULT(Adapter->CreateReservedResource(
			Desc,
			GetGPUMask(),
			InCreateD3D12Access,
			InResourceStateMode,
	...
	BufferPool->AllocDefaultResource(
		InHeapType,
		ResourceDesc,
		InBufferUsage,
		InResourceStateMode,
...
{	
	...
	SharedInitConfig.InitialD3D12Access = ED3D12Access::Common;
	...
	const TCHAR* Name)
{
	...
	PoolAllocators[(int)PoolType]->AllocateResource(
		GetParentDevice()->GetGPUIndex(),
...
HRESULT FD3D12TextureAllocator::AllocateTexture(
	FD3D12ResourceDesc Desc,
	const D3D12_CLEAR_VALUE* ClearValue,
	FD3D12ResourceLocation& TextureLocation,
	ED3D12Access InInitialD3D12Access,
	...
	const TCHAR* Name)
{
	...
	if (Info.SizeInBytes < D3D12_DEFAULT_RESOURCE_PLACEMENT_ALIGNMENT)
	{
		if (TryAllocate(Info.SizeInBytes, Info.Alignment, TextureLocation))
		{
			...
			hr = Adapter->CreatePlacedResource(
				Desc,
				BackingHeap,
				HeapOffset,
				InInitialD3D12Access,
	...
	hr = Adapter->CreateCommittedResource(
		Desc,
		GetGPUMask(),
		HeapProps,
		InInitialD3D12Access,
	...
	const TCHAR* Name)
{
	if (FD3D12Texture::CanBe4KAligned(Desc, (EPixelFormat)UEFormat))
	{
		Desc.Alignment = D3D12_SMALL_RESOURCE_PLACEMENT_ALIGNMENT; // request 4k alignment
			return ReadOnlyTexturePool.AllocateTexture(
				Desc,
				ClearValue,
				TextureLocation,
				InInitialD3D12Access,
	...
	HRESULT hr = Adapter->CreateCommittedResource(Desc, GetGPUMask(), HeapProps, InInitialD3D12Access, ClearValue, &Resource, Name, false);
...
FD3D12FastAllocatorPage* FD3D12FastAllocatorPagePool::RequestFastAllocatorPage()
{
	...
	const ED3D12Access InitialD3D12Access = DetermineInitialBufferD3D12Access(HeapProperties.Type, &HeapProperties);
	VERIFYD3D12RESULT(Adapter->CreateBuffer(
		HeapProperties,
		GetGPUMask(),
		InitialD3D12Access,
		...
		InitialD3D12Access,
		PageSize,
		Page->FastAllocBuffer.GetInitReference(),
		TEXT("Fast Allocator Page")));

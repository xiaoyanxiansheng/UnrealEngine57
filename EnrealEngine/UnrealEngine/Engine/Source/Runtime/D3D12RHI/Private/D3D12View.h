...
inline ERHIDescriptorType RHIDescriptorTypeFromD3D12ViewDesc(const D3D12_SHADER_RESOURCE_VIEW_DESC& ViewDesc)
{
	if (ViewDesc.ViewDimension == D3D12_SRV_DIMENSION_UNKNOWN)
	{
	...
	if (ViewDesc.ViewDimension == D3D12_SRV_DIMENSION_RAYTRACING_ACCELERATION_STRUCTURE)
	{
...
inline ERHIDescriptorType RHIDescriptorTypeFromD3D12ViewDesc(const D3D12_UNORDERED_ACCESS_VIEW_DESC& ViewDesc)
{
	if (ViewDesc.ViewDimension == D3D12_UAV_DIMENSION_UNKNOWN)
	{
		return ERHIDescriptorType::Invalid;
	}
...
protected:
	...
	TD3D12View(FD3D12Device* InDevice, ERHIDescriptorType InDescriptorType, ERHIDescriptorHeapType InHeapType, TD3D12View* FirstLinkedObject)
		: FD3D12View(InDevice, InDescriptorType, InHeapType, FirstLinkedObject)
	{}

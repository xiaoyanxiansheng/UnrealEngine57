 ...
 */
struct UE_DEPRECATED(5.7, "Unused since virtual texture support was never fully submitted.") FMediaTileCoordinate
{
	uint16 X, Y;

	FMediaTileCoordinate() = default;

	inline explicit FMediaTileCoordinate(uint16 InX, uint16 InY)
		: X(InX)
		, Y(InY)
	{
	}

	inline explicit FMediaTileCoordinate(int32 InX, int32 InY)
		: X(IntCastChecked<uint16>(InX))
		, Y(IntCastChecked<uint16>(InY))
	{
	}

	inline FMediaTileCoordinate(EForceInit)
		: X(0u)
		, Y(0u)
	{
	}

	inline bool operator==(const FMediaTileCoordinate& Other) const
	{
	...
	inline static FMediaTileCoordinate Zero()
	{
...
inline uint32 GetTypeHash(const FMediaTileCoordinate& Coord)
{
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	 ...
	 * @see SetVisibleTiles
	 */
	UE_DEPRECATED(5.7, "Unused since virtual texture support was never fully submitted.")

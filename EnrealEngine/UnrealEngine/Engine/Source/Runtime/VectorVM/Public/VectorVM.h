...
struct FDataSetMeta
{
	...
	inline void Init(FInputRegisterView InInputRegisters, FOutputRegisterView InOutputRegisters, int32 InInstanceOffset, TArray<int32>* InIDTable, TArray<int32>* InFreeIDTable, int32* InNumFreeIDs, int32 *InNumSpawnedIDs, int32* InMaxUsedID, int32 InIDAcquireTag, TArray<int32>* InSpawnedIDsTable)
	{
...
public:
	...
	inline FRandomStream& GetRandStream() { return *RandStream; }
	inline void* GetUserPtrTable(int32 UserPtrIdx) { check(UserPtrIdx < NumUserPtrs);  return UserPtrTable[UserPtrIdx]; }
	template<uint32 InstancesPerOp> inline int32  GetNumLoops() const { static_assert(InstancesPerOp == 4); return NumLoops; };
...
namespace VectorVM
{
	...
	public:
		...
		inline FExternalFuncInputHandler(FVectorVMExternalFunctionContext& Context)
		{
			Init(Context);
		}
		...
		inline bool IsConstant() const { return !IsRegister(); }
		inline bool IsRegister() const { return AdvanceOffset > 0; }
		inline void Reset(){ InputPtr = StartPtr; }

		inline const T Get() { return *InputPtr; }
		inline const T* GetDest() { return InputPtr; }
		inline void Advance(int32 Count=1) { InputPtr += AdvanceOffset * Count; }
		inline const T GetAndAdvance()
		{
			const T* Ret = InputPtr;
			InputPtr += AdvanceOffset;
			return *Ret;
		}
		inline const T* GetDestAndAdvance()
		{
			const T* Ret = InputPtr;
			InputPtr += AdvanceOffset;
			return Ret;
	...
	public:
		...
		inline bool IsValid() const { return AdvanceOffset > 0; }
		inline const T Get() { return *Register; }
		inline T* GetDest() { return Register; }
		inline void Advance() { Register += AdvanceOffset; }
		inline void Advance(int32 Count) { Register += AdvanceOffset * Count; }
		inline const T GetAndAdvance()
		{
			T* Ret = Register;

...
uint32 OptimizeVectorVMScript(const uint8 *InBytecode, int InBytecodeLen, FVectorVMExtFunctionData *ExtFnIOData, int NumExtFns, FVectorVMOptimizerContext* OptContext, uint64 HashId, uint32 Flags)
{
	...
	while (OpPtrIn < OpPtrInEnd)
	{
		...
		switch (OpCode)
		{
			...
			{
				uint16 ExtFnIdx = *reinterpret_cast<const uint16*>(OpPtrIn);
				check(ExtFnIdx < NumExtFns);

				const uint16* ExtFuncVecIndices = reinterpret_cast<const uint16*>(OpPtrIn + sizeof(ExtFnIdx));
				...
				{
					if (ExtFuncVecIndices[i] == 0xFFFF)
					{
						VVMPushRegUsage(0xFFFF, VVM_RT_INVALID, 0);
					}
					else
					{
						if (ExtFuncVecIndices[i] & 0x8000) //register: high bit means input is a register in the original bytecode
						{ 
							VVMPushRegUsage(ExtFuncVecIndices[i] & 0x7FFF, VVM_RT_TEMPREG, 0);
						}
						else //constant
						{
							VVMPushRegUsage(ExtFuncVecIndices[i], VVM_RT_CONST, 0);
						}
					}
				}
				for (int i = 0; i < ExtFnIOData[ExtFnIdx].NumOutputs; ++i)
				{
					...
					check((ExtFuncVecIndices[Idx] & 0x8000) == 0 || ExtFuncVecIndices[Idx] == 0xFFFF); //can't output to a const... 0xFFFF is invalid
					VVMPushRegUsage(ExtFuncVecIndices[Idx], VVM_RT_TEMPREG, 1);
				}
				...
				OpPtrIn += sizeof(ExtFnIdx) + (ExtFnIOData[ExtFnIdx].NumInputs + ExtFnIOData[ExtFnIdx].NumOutputs) * 2;
			}
			break;

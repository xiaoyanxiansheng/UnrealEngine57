...
void FPreLoadScreenManager::StaticRenderTick_RenderThread(FRHICommandListImmediate& RHICmdList)
{
	...
	{
		FPreLoadScreenManager::Get()->RenderTick_RenderThread(RHICmdList);
	}
}
...
void FPreLoadScreenManager::RenderTick_RenderThread(FRHICommandListImmediate& RHICmdList)
{
	...
	if (PinnedActivePreloadScreen && bRenderingEnabled)
	{
		if (MainWindow.IsValid() && VirtualRenderWindow.IsValid() && !PinnedActivePreloadScreen->IsDone())
		{
			...
			PinnedActivePreloadScreen->RenderTick(RHICmdList, DeltaTime);
			RHICmdList.EndFrame();
			...
			ENQUEUE_RENDER_COMMAND(BeginPreLoadScreenFrame)(
				[this, SlateDeltaTime](FRHICommandListImmediate& RHICmdList)
				{
					...
					if (FPreLoadScreenManager::bRenderingEnabled && PinnedActivePreloadScreen && !bHasRenderPreLoadScreenFrame_RenderThread)
					{
						...
						PinnedActivePreloadScreen->RenderTick(RHICmdList, SlateDeltaTime);
					}
				});

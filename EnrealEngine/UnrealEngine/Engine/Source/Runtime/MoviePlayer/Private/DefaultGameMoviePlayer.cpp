...
void FDefaultGameMoviePlayer::WaitForMovieToFinish(bool bAllowEngineTick)
{
	...
	if (LoadingScreenIsPrepared() && IsMovieCurrentlyPlaying())
	{
			...
			||	(bEnforceMinimumTime && (FPlatformTime::Seconds() - LastPlayTime) < LoadingScreenAttributes.MinimumLoadingScreenDisplayTime)))
		{
			...
			// unless waiting for manual stop that hasn't been signaled yet.
			if (ActiveMovieStreamer.IsValid() && LoadingScreenAttributes.PlaybackType == MT_LoadingLoop && ActiveMovieStreamer->IsLastMovieInPlaylist() && (!bWaitForManualStop || (bWaitForManualStop && bUserCalledFinish)))
			{
				break;
			}
			...
			if (FSlateApplication::IsInitialized())
			{
				...
				ENQUEUE_RENDER_COMMAND(BeginLoadingMovieFrameAndTickMovieStreamer)(
					[InMoviePlayer, DeltaTime](FRHICommandListImmediate& RHICmdList)
					{
						...
						InMoviePlayer->TickStreamer(RHICmdList, DeltaTime);
					}
				);
...
void FDefaultGameMoviePlayer::Tick( FRHICommandListImmediate& RHICmdList, float DeltaTime )
{
	...
	if (MainWindow.IsValid() && VirtualRenderWindow.IsValid() && !IsLoadingFinished() && GDynamicRHI && !GDynamicRHI->RHIIsRenderingSuspended())
	{
		...
		if(SyncMechanism)
		{
			...
			{
				GFrameNumberRenderThread++;
				TickStreamer(RHICmdList, DeltaTime);
...
void FDefaultGameMoviePlayer::TickStreamer(FRHICommandListImmediate& RHICmdList, float DeltaTime)
{
	if (MovieStreamingIsPrepared() && ActiveMovieStreamer.IsValid() && !IsMovieStreamingFinished())
	{
		const bool bMovieIsDone = ActiveMovieStreamer->Tick(RHICmdList, DeltaTime);
		if (bMovieIsDone)
		{
			MovieStreamingIsDone.Set(1);
		}

...
bool inline IsCustomDeltaProperty(const FStructProperty* StructProperty)
{
	return EnumHasAnyFlags(StructProperty->Struct->StructFlags, STRUCT_NetDeltaSerializeNative);
}

bool inline IsCustomDeltaProperty(const FProperty* Property)
{
...
public:
	...
	/**
	 * @param Reader the buffer that contains the RPC (this function reads RPC parameters from the current position)
	 * @param OutUnmappedGuids any GUIDs that are not yet resolvable because we do not have the data (yet)
	 * @param bOutSkippedRpcExec did we skip execution of this RPC? If so, the caller must handle execution (e.g. delay).
	 * @param RepFlags the replication flags (state) of the replicated object
	 * @param FieldCache the RPC FieldCache used to speed-up property lookups.  This cannot be null.
	 * @param SkipRpcBehavior under what circumstances should we skip the execution of the RPC?
	 *
	 * @return True if the Reader is still valid. False if the Reader should be abandoned.
	 * @see ReceivedBunch(), UpdateUnmappedObjects()
	 */
	...
	inline bool ReceivedRPC(
		...
		TSet<FNetworkGUID>& OutUnmappedGuids)
	{
		return ReceivedRPC(Reader, OutUnmappedGuids, bOutDelayRPC, RepFlags, FieldCache, bCanDelayRPC ? ESkipRpcBehavior::SkipIfNotReady : ESkipRpcBehavior::NeverSkip);
	}
	 ...
	 * @see UNetDriver::PostTickDispatch
	 */
	ENGINE_API void ExecutePendingTransactionalRPCs();

	inline TWeakObjectPtr<UObject>	GetWeakObjectPtr() const
	{
	...
	inline void SetObject(UObject* NewObj)
	{
		ObjectPtr = NewObj;
...
private:
	...
	/**
	 * These are RPCs that are saved so that they may be executed transactionally at a later time
	 * @see ExecutePendingTransactionalRPCs
	 */
	TArray<FRPCPendingLocalCall> PendingTransactionalRPCs;

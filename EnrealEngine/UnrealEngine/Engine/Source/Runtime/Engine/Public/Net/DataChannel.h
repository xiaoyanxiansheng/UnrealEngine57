...
DEFINE_CONTROL_CHANNEL_MESSAGE(IrisNetRefHandleError, 30, UE::Net::ENetRefHandleError, uint64, TArray<uint64>); // a specific handle caused an error and we want the remote connection to log all information it has on it
...
//
// Relevancy: Since network relevancy will be based on the location of ANoPawnPlayerController, it's important to set the location of
//			  the actor using the RPC ANoPawnPlayerController::SetViewTargetPosition(), either from the server or client using this connection.
//
...
// Close a child connection remotely without closing all other child and parent connections.
//
// The argument to this message is the value of NetPlayerIndex associated with the child connection that must be closed.
//
// It is expected that the local child connection will be cleaned up manually and does not depend on this message being 

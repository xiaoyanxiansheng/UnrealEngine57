...
public:
	 ...
	 */
	inline FKeyHandle AddKey(TimeTypeRef InTime)
	{
	 ...
	 */
	inline void SetKeyTime(FKeyHandle KeyHandle, TimeTypeRef NewTime)
	{
	 ...
	 */
	inline void RemoveKey(FKeyHandle KeyHandle)
	{
	 ...
	 */
	inline TOptional<TimeType> GetKeyTime(FKeyHandle KeyHandle) const
	{
	 ...
	 */
	inline TOptional<FKeyHandle> FindKey(TimeTypeRef KeyTime, TimeTypeRef KeyTimeTolerance) const
	{
...
struct TKeyTimeIterator
{
	...
	inline TKeyTimeIterator& operator++()
	{
		++Index;
		return *this;
	}
	...
	inline bool operator==(const TKeyTimeIterator& RHS) const
	{
		return Manipulator.KeyTimes == RHS.Manipulator.KeyTimes && Index == RHS.Index;
	}

	inline friend bool operator!=(const TKeyTimeIterator& LHS, const TKeyTimeIterator& RHS)
	{
		return !(LHS == RHS);
	}
	...
	inline friend TKeyTimeIterator<TimeType> begin(const TKeyTimeIterator<TimeType>& Iter)
	{
		TKeyTimeIterator<TimeType> NewIter = Iter;
		NewIter.Index = Iter.GetStartIndex();
		return NewIter;
	}
	inline friend TKeyTimeIterator<TimeType> end  (const TKeyTimeIterator<TimeType>& Iter)
	{
		TKeyTimeIterator<TimeType> NewIter = Iter;

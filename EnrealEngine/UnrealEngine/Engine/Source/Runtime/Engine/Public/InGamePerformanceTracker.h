...
struct FInGameCycleHistory
{
	...
	inline void AddCycles(uint64 NewCycles)
	{
		CurrFrameCycles += NewCycles;
	}
	
	inline uint64 GetAverageCycles()const
	{
...
public:
	...
	inline void AddCycles(uint32 Cycles)
	{
		return History.AddCycles(Cycles);
	}

	inline double GetAverageTimeSeconds() const
	{

...
public:
	...
	*/
	inline FRenderTarget* GetRenderTarget() const
	{
		return RenderTarget;
	}
...
public:
	...
	public:
		...
		inline void SetMatrix(const FMatrix& InMatrix)
		{
			Matrix = InMatrix;
			MatrixCRC = FCrc::MemCrc_DEPRECATED(&Matrix, sizeof(FMatrix));
		}
		...
		inline uint32 GetMatrixCRC() const
		{
			return MatrixCRC;
		}
	private:
	...
	inline const TArray<FTransformEntry>& GetTransformStack() const
	{
		return TransformStack;
	}
	inline const FIntRect& GetViewRect() const
	{
		return ViewRect;
	}

	inline void SetScaledToRenderTarget(bool bScale = true)
	{
		bScaledToRenderTarget = bScale;
	}
	inline bool IsScaledToRenderTarget() const { return bScaledToRenderTarget; }

	inline void SetStereoRendering(bool bStereo = true)
	{
		bStereoRendering = bStereo;
	}
	inline bool IsStereoRendering() const { return bStereoRendering; }

	inline void SetUseInternalTexture(const bool bInUseInternalTexture)
	{
		bUseInternalTexture = bInUseInternalTexture;
	}

	inline bool IsUsingInternalTexture() const { return bUseInternalTexture; }

	inline void SetWriteDestinationAlpha(const bool bInForceWriteAlpha)
	{
		bWriteDestinationAlpha = bInForceWriteAlpha;
	}

	inline bool IsWriteDestinationAlphaSet() const { return bWriteDestinationAlpha; }

	inline void SetParentCanvasSize(FIntPoint InParentSize)
	{
		ParentSize = InParentSize;
	}

	inline FIntPoint GetParentCanvasSize() const { return ParentSize; }
...
private:
	...
	/** true, if the destination alpha channel must be written */
	bool bWriteDestinationAlpha;

...
struct TStaticMeshVertexTangentDatum
{
	...
	inline FVector3f GetTangentX() const
	{
		return FVector3f(TangentX.ToFVector());
	}

	inline FVector4f GetTangentZ() const
	{
		return TangentZ.ToFVector4f();
	}

	inline FVector3f GetTangentY() const
	{
		return FVector3f(GenerateYAxis(TangentX, TangentZ));
	}

	inline void SetTangents(FVector3f X, FVector3f Y, FVector3f Z)
	{
		TangentX = X;
...
struct TStaticMeshVertexUVsDatum
{
	...
	inline FVector2f GetUV() const
	{
		return UVs;
	}

	inline void SetUV(FVector2f UV)
	{
		UVs = UV;
	}
...
public:
	...
	*/
	...
	inline FVector4f VertexTangentX_Typed(uint32 VertexIndex)const
	{
	...
	inline FVector4f VertexTangentX(uint32 VertexIndex) const
	{
	...
	template<EStaticMeshVertexTangentBasisType TangentBasisTypeT>
	inline FVector4f VertexTangentZ_Typed(uint32 VertexIndex)const
	{
		typedef TStaticMeshVertexTangentDatum<typename TStaticMeshVertexTangentTypeSelector<TangentBasisTypeT>::TangentTypeT> TangentType;
		TangentType* ElementData = reinterpret_cast<TangentType*>(TangentsDataPtr);
		check((void*)((&ElementData[VertexIndex]) + 1) <= (void*)(TangentsDataPtr + TangentsData->GetResourceSize()));
	...
	inline FVector4f VertexTangentZ(uint32 VertexIndex) const
	{
		checkSlow(VertexIndex < GetNumVertices());
	...
	template<EStaticMeshVertexTangentBasisType TangentBasisTypeT>
	inline FVector4f VertexTangentY_Typed(uint32 VertexIndex)const
	{
		typedef TStaticMeshVertexTangentDatum<typename TStaticMeshVertexTangentTypeSelector<TangentBasisTypeT>::TangentTypeT> TangentType;
		TangentType* ElementData = reinterpret_cast<TangentType*>(TangentsDataPtr);
		check((void*)((&ElementData[VertexIndex]) + 1) <= (void*)(TangentsDataPtr + TangentsData->GetResourceSize()));
	...
	*/
	inline FVector3f VertexTangentY(uint32 VertexIndex) const
	{
		checkSlow(VertexIndex < GetNumVertices());
	...
	inline void SetVertexTangents(uint32 VertexIndex, FVector3f X, FVector3f Y, FVector3f Z)
	{
		checkSlow(VertexIndex < GetNumVertices());
	...
	*/
	inline void SetVertexUV(uint32 VertexIndex, uint32 UVIndex, const FVector2f& Vec2D, bool bUseBackwardsCompatibleF16TruncUVs = false)
	{
		checkSlow(VertexIndex < GetNumVertices());
	...
	inline FVector2f GetVertexUV_Typed(uint32 VertexIndex, uint32 UVIndex)const
	{
	...
	* @param 2D UV values
	*/
	inline FVector2f GetVertexUV(uint32 VertexIndex, uint32 UVIndex) const
	{
		checkSlow(VertexIndex < GetNumVertices());
		checkSlow(UVIndex < GetNumTexCoords());
	...
	inline void* GetTangentData() { return TangentsDataPtr; }
	inline const void* GetTangentData() const { return TangentsDataPtr; }

	inline void* GetTexCoordData() { return TexcoordDataPtr; }
	inline const void* GetTexCoordData() const { return TexcoordDataPtr; }

	ENGINE_API int GetTangentSize() const;

	ENGINE_API int GetTexCoordSize() const;

	inline bool GetAllowCPUAccess() const
	{
	...
	// Allows for creating SRV on demand
	// Note: consider cache the SRV locally if multiple compute tasks all need access to one in a single frame

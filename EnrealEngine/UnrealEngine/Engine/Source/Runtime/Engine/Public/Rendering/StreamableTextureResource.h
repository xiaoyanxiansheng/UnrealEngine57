...
public:
	...
	inline FStreamableRenderResourceState GetState() const
	{ 
	...
	inline FStreamableRenderResourceState GetPostInitState() const
	{ 
	...
	UE_DEPRECATED(5.7, "A RHICmdList argument is now required.")
	...
	inline EPixelFormat GetPixelFormat() const { return PixelFormat; }
	...
	inline ETextureCreateFlags GetCreationFlags() const { return CreationFlags; }

	inline const FName& GetTextureName() const { return TextureName; }

	inline TextureGroup GetLODGroup() const { return LODGroup; }
	...
	inline int32 GetCurrentFirstMip() const
	{
		return State.IsValid() ? State.ResidentFirstLODIdx() : 0;
	}

	inline uint32 GetExtData() const { return PlatformData->GetExtData(); }

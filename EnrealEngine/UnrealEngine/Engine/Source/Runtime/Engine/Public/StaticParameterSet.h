...
struct FStaticTerrainLayerWeightParameter
{
	...
	// These are necessary to prevent deprecation warnings from UE_DEPRECATED on some compilers because of auto-generated code
	...
	UE_DEPRECATED(all, "ParameterInfo_DEPRECATED remains in the code for backwards-compatibility reasons but shall not be used")
	UPROPERTY()
	FMaterialParameterInfo ParameterInfo_DEPRECATED;

	UE_DEPRECATED(5.7, "ExpressionGUID_DEPRECATED is now unused and will be removed in a future version")
	UPROPERTY()
	FGuid ExpressionGUID_DEPRECATED;

	UE_DEPRECATED(all, "bOverride_DEPRECATED remains in the code for backwards-compatibility reasons but shall not be used")
	UPROPERTY()
	...
	UE_DEPRECATED(5.7, "bIsRepeatedLayer is now unused and will be removed in a future version")
	UPROPERTY()
	bool bIsRepeatedLayer = false;

	UE_DEPRECATED(5.7, "bWeightBasedBlend is now unused and will be removed in a future version")
	UPROPERTY()
	bool bWeightBasedBlend = false;

	UE_DEPRECATED(5.7, "bWeightBasedBlend being deprecated, this constructor is now useless")
	...
	{
		return LayerName == Reference.LayerName && WeightmapIndex == Reference.WeightmapIndex;
	}
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
		...
		{
			// For backwards compatibility reasons, we now serialize a bogus value and this code shall remain here forever, but the member 
			//  variable bWeightBasedBlend can be removed after the deprecation grace period has passed
			bool bDummyWeightBasedBlend = false;
			Ar << bDummyWeightBasedBlend;
		}
		...
		if (Ar.CustomVer(FFortniteMainBranchObjectVersion::GUID) < FFortniteMainBranchObjectVersion::TerrainLayerWeightsAreNotParameters)
		{
			...
			// For backwards compatibility reasons, we now serialize a bogus value and this code shall remain here forever, but the member 
			//  variable ExpressionGUID_DEPRECATED can be removed after the deprecation grace period has passed
			FGuid DummyExpressionGUID_DEPRECATED;
			Ar << DummyExpressionGUID_DEPRECATED;
		}
#endif // WITH_EDITORONLY_DATA
		return Ar;
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	void UpdateHash(FSHA1& HashState) const
	{
		...
		HashState.Update((const uint8*)&WeightmapIndex, sizeof(WeightmapIndex));
	}
	ENGINE_API void AppendKeyString(FString& KeyString) const;
	ENGINE_API void Append(FShaderKeyGenerator& KeyGen) const;
};

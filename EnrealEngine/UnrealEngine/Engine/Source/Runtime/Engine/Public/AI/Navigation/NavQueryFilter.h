...
public:
	...
	inline uint32 GetMaxSearchNodes() const { return MaxSearchNodes; }
	...
	*  (find path from End to Start, but all links works like on path from Start to End) */
	...
	inline void SetFilterImplementation(const INavigationQueryFilterInterface* InQueryFilterImpl)
	{
		QueryFilterImpl = MakeShareable(InQueryFilterImpl->CreateCopy());
	}

	inline const INavigationQueryFilterInterface* GetImplementation() const { return QueryFilterImpl.Get(); }
	inline INavigationQueryFilterInterface* GetImplementation() { return QueryFilterImpl.Get(); }
	void Reset() { GetImplementation()->Reset(); }

	ENGINE_API FSharedNavQueryFilter GetCopy() const;

	inline bool operator==(const FNavigationQueryFilter& Other) const
	{

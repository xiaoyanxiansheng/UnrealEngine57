...
private:
	...
	public:
		 ...
		 */
		inline void Allocate(FAllocationInfo& OutInfo, int32 ValueCount, EGPUSortFlags Flags);
	};
	...
	struct FDynamicValueBuffer
	{
		...
		inline int32 GetUsedCount() const { return ValueBuffers.Num() ? ValueBuffers.Last().UsedCount : 0; }
		/** Get the buffer allocated size . */
		inline int32 GetAllocatedCount() const { return ValueBuffers.Num() ? ValueBuffers.Last().AllocatedCount : 0; }
	};
	...
	struct FSortBatch
	{
		...
		inline int32 GetUsedValueCount() const { return DynamicValueBuffer ? DynamicValueBuffer->GetUsedCount() : 0; }
		/** Get the batch buffer allocated size. */
		inline int32 GetAllocatedValueCount() const { return DynamicValueBuffer ? DynamicValueBuffer->GetAllocatedCount() : 0; }
	};
...
private:
	...
	static inline bool HasEither(EGPUSortFlags Flags, EGPUSortFlags A, EGPUSortFlags B);
	/** Create the initial batch flags from a task flags. */
	static inline EGPUSortFlags GetBatchFlags(EGPUSortFlags TaskFlags);
	/** Update a batch flags after adding a task into it. */
	static inline EGPUSortFlags CombineBatchFlags(EGPUSortFlags BatchFlags, EGPUSortFlags TaskFlags);
	/** Test whether a batch flags are compatible with a given task. Used to know if the task can be merged in the batch.  */
	static inline bool TestBatchFlags(EGPUSortFlags BatchFlags, EGPUSortFlags TaskFlags);
	/** Convert the precision flags into a string, used for GPU markers. */
	static inline auto GetPrecisionString(EGPUSortFlags BatchFlags) -> TCHAR const(&)[1];
	...
	/** Used to ensure no concurrent access during AddTask */
	UE::FMutex AddTaskMutex;

...
public:
	...
	inline int32 Num() const { return NumIndices; }
...
protected:
	...
	UE_DEPRECATED(5.7, "Use IsSRVCreatedByDefault or IsShaderResource instead, which gives more granular control over platform specific behavior")	
...
public:	
	...
	*/
	virtual void InitRHI(FRHICommandListBase& RHICmdList) override
	{
		...
		if (IsSRVCreatedByDefault(Indices.GetAllowCPUAccess()))
		{
			SRVValue = GetOrCreateSRV(RHICmdList);
		}
	}
	...
	virtual FShaderResourceViewRHIRef GetOrCreateSRV(FRHICommandListBase& RHICmdList) const override
	{
		...
		if (!IsShaderResource(Indices.GetAllowCPUAccess()))
		{
			return {};
		}
		
		FShaderResourceViewRHIRef ExternalSRV = RHICmdList.CreateShaderResourceView(
				IndexBufferRHI,
	...
	FBufferRHIRef CreateRHIBuffer(FRHICommandListBase& RHICmdList)
	{
		if (CachedNumIndices)
		{
			...
			return CreateRHIIndexBufferInternal(
				...
				sizeof(INDEX_TYPE),
				&Indices,
				IsShaderResource(Indices.GetAllowCPUAccess())
			);
		}
		return nullptr;
	}

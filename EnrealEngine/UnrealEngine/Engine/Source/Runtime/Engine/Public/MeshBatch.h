...
{
public:
	inline bool IsBound() const { return GPUSceneWriteDelegate != nullptr; }

	inline FGPUSceneWriteDelegate* operator->() const
	{
		check( IsBound() );
...
struct FMeshBatchDynamicPrimitiveData
{
	...
	inline void SetPayloadDataFlags(uint16 Flags, bool bValue)
	{
		if (bValue)
		{
	...
	inline void EnableInstanceDynamicData(bool bEnable)  { SetPayloadDataFlags(INSTANCE_SCENE_DATA_FLAG_HAS_DYNAMIC_DATA, bEnable); }
	inline void EnableInstanceSkinningData(bool bEnable) { SetPayloadDataFlags(INSTANCE_SCENE_DATA_FLAG_HAS_SKINNING_DATA, bEnable); }
	inline void EnableInstanceLocalBounds(bool bEnable)  { SetPayloadDataFlags(INSTANCE_SCENE_DATA_FLAG_HAS_LOCAL_BOUNDS, bEnable); }
	inline void SetNumInstanceCustomDataFloats(uint32 NumFloats)
	{
	 ...
	 **/
	inline uint32 GetPayloadFloat4Stride() const
	{
		uint32 Total = 0;
	...
	inline void Validate(uint32 NumInstances) const
	{
#if DO_CHECK
...
struct FMeshBatch
{
	...
	UE_DEPRECATED(5.6, "This is not used by the renderer.")
	...
	uint32 bForceTwoSidedInRayTracing : 1;	// Whether segment should be considered two sided in ray tracing regardless of material.
#endif
	...
	inline bool HasAnyDrawCalls() const
	{
		for (int32 ElementIdx = 0; ElementIdx < Elements.Num(); ++ElementIdx)
		{
...
#else
	inline bool Validate(const FPrimitiveSceneProxy* SceneProxy, ERHIFeatureLevel::Type FeatureLevel) const { return true; }
#endif

	// Disable deprecation warnings for TessellationDisablingShadowMapMeshSize.
	...
	,	bForceTwoSidedInRayTracing(false)
#endif
	...
	// Need to do these explicitly as clang otherwise produces deprecation warnings for the _implicit_ ctors & operators

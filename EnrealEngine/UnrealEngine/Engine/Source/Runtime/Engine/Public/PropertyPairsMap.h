...
public:
	 ...
	 */
	inline void AddProperty(FName InName, FName InValue = NAME_None)
	{
	 ...
	 */
	inline bool HasProperty(FName InName) const
	{
		return GetProperty(InName);
	}

	inline bool HasProperty(const TCHAR* InName) const
	{
	 ...
	 * @returns True if the property exist in the map.
	 */
	inline bool GetProperty(FName InName, FName* OutValue = nullptr) const
	{
	...
	inline bool GetProperty(const TCHAR* InName, FName* OutValue = nullptr) const
	{
		FName PropertyName(InName, FNAME_Find);
	 ...
	 */
	inline friend FArchive& operator<<(FArchive& InAr, FPropertyPairsMap& InPropertyPairsMap)
	{
	 ...
	 */
	template <class Func>
	inline void ForEachProperty(Func InFunc)
	{

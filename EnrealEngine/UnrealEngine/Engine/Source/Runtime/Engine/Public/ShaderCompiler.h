...
extern ENGINE_API void GetShaderCompilerCurrentMemoryLimits(FShaderCompilerMemoryLimits& limits);
extern ENGINE_API void SetShaderCompilerTargetMemoryLimitInMiB(int32 NewLimit);
...
public:
	...
	/**
	 * Invokes the specified callback on each pending job this shader compile thread is processing.
	 * Use this to find compile jobs that are already sent to a worker, since once a worker processes them,
	 * they are no longer in the "pending list" of the shader compile manager (i.e. FShaderCompileJobCollection specifically).
	 * @param PendingJobCallback Function callback that is invoked on each pending job. Returns false to exit the iteration early.
	 */
...
private:
	...
	/** Queries the memory status of all worker processes. Uses FResourceProcessTreeMemory. */
	bool QueryMemoryStatus(FShaderCompilerJobMemoryLimitInfo& OutInfo);

	/** Queries the status if the worker processes have violated the memory limitation. Either uses FResourceProcessTreeMemory. */
	bool QueryMemoryLimitViolationStatus(FShaderCompilerJobMemoryLimitInfo& OutInfo);
};

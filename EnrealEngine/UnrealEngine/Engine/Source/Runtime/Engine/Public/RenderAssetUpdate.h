...
extern ENGINE_API bool GEnableAssetUpdateAbandons;
...
public:
	...
	*/
	...
	/**
	* Allows the update to be decoupled from its UStreamableRenderAsset so it can be independently garbage collected.
	* This is called when we query IsReadyForFinishDestroy on the UStreamableRenderAsset.
	* Returns true if the asset update can handle being abandoned.
	*/

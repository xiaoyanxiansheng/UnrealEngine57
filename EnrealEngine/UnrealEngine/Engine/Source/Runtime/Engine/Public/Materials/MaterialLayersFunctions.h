...
struct FMaterialLayersFunctionsEditorOnlyData
{
	...
	inline bool operator==(const FMaterialLayersFunctionsEditorOnlyData& Other) const
	{
...
struct FMaterialLayersFunctionsRuntimeData
{
	...
	// In multiple locations there are const references to this runtime data/it's children, 
	// so this mutable entry is required to ensure we can keep available layers/blend up to date across all instances.
	mutable TSharedPtr<FMaterialLayerStackFunctionsCache> LayerStackCache;
	...
	FMaterialLayersFunctionsRuntimeData(const FMaterialLayersFunctionsRuntimeData& Rhs)
		: Layers(Rhs.Layers)
		, Blends(Rhs.Blends)
		, LayerStackCache(Rhs.LayerStackCache)
	{}
	...
	void Empty()
	{
		Layers.Empty();
		Blends.Empty();
		LayerStackCache.Reset();
	}

	inline bool operator==(const FMaterialLayersFunctionsRuntimeData& Other) const
	{
		...
		if (LayerStackCache || Other.LayerStackCache)
		{
			if (!LayerStackCache || !Other.LayerStackCache 
				|| LayerStackCache->AvailableLayerPaths != Other.LayerStackCache->AvailableLayerPaths
				|| LayerStackCache->AvailableBlendPaths != Other.LayerStackCache->AvailableBlendPaths)
			{
				return false;
			}
		}
...
struct FMaterialLayersFunctions : public FMaterialLayersFunctionsRuntimeData
{
	...
	inline bool operator==(const FMaterialLayersFunctions& Other) const
	{

...
{
	struct FFXInterfaceDeletor
	{
		void operator()(FFXSystemInterface* ObjectToDelete) const;
	};
...
public:
	...
	/**
	 * Notify the effect system instance that it should release resources in preperation for destruction.
	 * Once the interface is marked pending kill using the interface is undefined, as the scene (for example) may be invalid.
	 * The destruction of the instance will happen later on the render thread, and may be much later due to shared references to the interface.
	 */
	ENGINE_API static void MarkPendingKill(FFXSystemInterface* FXSystem);
	...
	/**
	 * Gamethread callback when MarkPendingKill is called, allows to clean up references.
	 * Once marked pending kill the interface can release resources and is in an undefined state.
	 */
	virtual void OnMarkPendingKill() { bIsPendingKill = true; }
	...
	UE_DEPRECATED(5.7, "Destroy is no longer supported, please update your code call MarkPendingKill")
	ENGINE_API static void Destroy(FFXSystemInterface* FXSystem);

protected:
	UE_DEPRECATED(5.7, "OnDestroy is no longer supported, please update your code call/implement OnMarkPendingKill")
...
public:
	...
	inline void SetSceneInterface(FSceneInterface* InSceneInterface) { SceneInterface = InSceneInterface; }

	inline FScene* GetScene()const { return Scene; }
	inline void SetScene(FScene* InScene) { Scene = InScene; }

...
public:
	...
	ENGINE_API void RecordLevelStreamingCountsToCSV();
#endif
...
#if (CSV_PROFILER_STATS && !UE_BUILD_SHIPPING)
	struct FWorldLevelStreamingStats
	{
		void Remove(const ULevelStreaming* StreamingLevel)
		{
			LevelsLoading.Remove(StreamingLevel);
			LevelsMakingVisible.Remove(StreamingLevel);
			LevelsMakingInvisible.Remove(StreamingLevel);
		}
		TSet<const ULevelStreaming*> LevelsLoading;
		TSet<const ULevelStreaming*> LevelsMakingVisible;
		TSet<const ULevelStreaming*> LevelsMakingInvisible;
	};

	FWorldLevelStreamingStats WorldLevelStreamingStats;
#endif
};

...
struct FBitManip
{
	 ...
	 */
	static inline uint32 CountTrailingZeros(ElementType Value);
};


template <>
inline uint32 FBitManip<uint32>::CountTrailingZeros(uint32 Value)  
{ 
	return FMath::CountTrailingZeros(Value); 
}
template <>
inline uint32 FBitManip<uint64>::CountTrailingZeros(uint64 Value)  
{ 
...
public:
	 ...
	 */
	inline void MarkRemoved(FPrimitiveInstanceId Id)
	{
	 ...
	 */
	inline void RemoveAtSwap(FPrimitiveInstanceId Id, int32 Index, int32 MaxInstanceIndex)
	{
		Validate();
	 ...
	 */
	inline void RemoveAt(FPrimitiveInstanceId Id, int32 Index, int32 MaxInstanceIndex)
	{
		Validate();
	 ...
	 */
	template <EFlag Flag>
	inline void MarkIndex(int32 Index, int32 MaxInstanceIndex)
	{
		Validate();
	 ...
	 */
	inline ElementType GetFlags(int32 Index) const
	{
		check(Index < MaxIndex);
	 ...
	 */
	template <EFlag Flag>
	inline bool TestFlag(int32 Index) const
	{
	...
	/**
	 */
	inline void SetNum(int32 InstanceIndexMax)
	{
		Validate();
	...
	{
	public:
		inline FAnyValidIterator() = default;
		inline FAnyValidIterator(const FInstanceAttributeTracker *InTracker, int32 StartIndex) : Tracker(InTracker)
		{
		...
		inline FAnyValidIterator &operator++()
		{
			AdvanceToNext();
			return *this;
		}
	...
	template <uint32 Mask = AnyFlagMask>
	inline FAnyValidIterator<Mask> GetChangedIterator() const
	{
	...
	public:
		...
		inline FDeltaRange() = default;

		inline FDeltaRange(bool bInFullUpdate, int32 InNumItems, const FInstanceAttributeTracker *InInstanceUpdateTracker)
		...
		/**
		 */
		inline bool IsDelta() const { return !bFullUpdate; }
		...
		struct FConstIterator
		{
			...
			inline FConstIterator(int32 InIndex, int32 InMaxNum)
			:	bUseIterator(false),
				ItemIndex(InIndex),
				MaxNum(InMaxNum)
			{
			...
			inline FConstIterator(FAnyValidIterator<IteratorMask> &&InIt)
			:	bUseIterator(true),
				ItemIndex(0),
				MaxNum(),
				It(MoveTemp(InIt))
			 ...
			 */
			inline int32 GetItemIndex() const
			{
				return ItemIndex;
			}
		...
		inline FConstIterator GetIterator() const
		{
			if (bFullUpdate)
			{
	...
	template <EFlag Flag>
	inline FDeltaRange<Flag> GetDeltaRange(bool bForceFullUpdate, int32 InNumItems) const
	{
...
private:
	...
	/**
	 */
	inline void SetMaskTo(int32 Index, ElementType Mask)
	{
		check(Index < MaxIndex);
	 ...
	 */
	template <EFlag Flag>
	inline void DecChangedCount(uint32 Flags)
	{
	...
	/**
	 */
	inline void DecChangedCounts(int32 Index)
	{
	...
	/**
	 */
	inline void CountMaskChanges(ElementType Element, uint32 Offset, uint32 NumMasksInElement)
	{
	 ...
	 */
	inline ElementType LoadCountAndMask(int32 ElementIndex, bool bHasNextElement)
	{
		if (bHasNextElement)
		{

...
public:
	...
	UE_DEPRECATED(5.7, "CreateTexture now requires a command list.")
	ENGINE_API void CreateTexture(FIntVector Dimensions);
	UE_DEPRECATED(5.7, "CreateTargetTexture now requires a command list.")
...
public:
	...
	UE_DEPRECATED(5.7, "InitRHIForSubLevelResources now requires a command list.")
...
struct FVolumetricLightmapBrickTextureSet : public FVolumetricLightmapBasicBrickDataLayers
{
	...
	void Initialize(FIntVector InBrickDataDimensions, VolumetricLightmapBrickDataType& BrickData)
	{
		...
		AmbientVector.CreateTargetTexture(RHICmdList, BrickDataDimensions);
		AmbientVector.CreateUAV(RHICmdList);

		for (int32 i = 0; i < UE_ARRAY_COUNT(SHCoefficients); i++)
		{
			SHCoefficients[i].CreateTargetTexture(RHICmdList, BrickDataDimensions);
			SHCoefficients[i].CreateUAV(RHICmdList);
		}

		if (BrickData.SkyBentNormal.Texture.IsValid())
		{
			SkyBentNormal.CreateTargetTexture(RHICmdList, BrickDataDimensions);
			SkyBentNormal.CreateUAV(RHICmdList);
		}

		DirectionalLightShadowing.CreateTargetTexture(RHICmdList, BrickDataDimensions);

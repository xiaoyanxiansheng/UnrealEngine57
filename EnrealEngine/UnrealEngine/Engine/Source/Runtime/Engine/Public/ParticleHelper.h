...
struct FBeamParticleModifierPayloadData
{
	...
	// Helper functions
	inline void UpdatePosition(FVector& Value)
	{
	...
	inline void UpdateTangent(FVector& Value, bool bAbsolute)
	{
	...
	inline void UpdateStrength(float& Value)
	{
 ...
 */
inline void GetDynamicValueFromPayload(int32 InDynamicPayloadOffset, const FBaseParticle& InParticle, FVector4f& OutDynamicData)
{
...
struct FMacroUVOverride
{
	...
	friend inline FArchive& operator<<(FArchive& Ar, FMacroUVOverride& O)
	{
		Ar << O.bOverride;
		Ar << O.Radius;
		Ar << O.Position;
...
inline FVector3f GetParticleBaseSize(const FBaseParticle& Particle, bool bKeepFlipScale = false)
{
...
inline FVector2D GetParticleSizeWithUVFlipInSign(const FBaseParticle& Particle, const FVector2D& ScaledSize)
{
	return FVector2D(

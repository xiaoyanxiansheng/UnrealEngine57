...
{
public:
	inline TWorldPartitionHandle()
		: ContainerInstance(nullptr)
		, ActorDescInstance(nullptr)
	{}

	inline TWorldPartitionHandle(TUniquePtr<FWorldPartitionActorDescInstance>* InActorDescInstance)
	...
	inline TWorldPartitionHandle(FWorldPartitionActorDescInstance* InActorDescInstance)
		: ContainerInstance(Impl::GetActorDescContainerInstance(InActorDescInstance))
		, ActorDescInstance(nullptr)
	{
	...
	inline TWorldPartitionHandle(UActorDescContainerInstance* InContainerInstance, const FGuid& InActorGuid)
	...
	template<class U>
	inline TWorldPartitionHandle(TActorDescContainerInstanceCollection<U>* ContainerCollection, const FGuid& ActorGuid)
	...
	inline TWorldPartitionHandle(TUniquePtr<FWorldPartitionActorDesc>* InActorDesc)
		: ContainerInstance(nullptr)
		, ActorDescInstance(nullptr)
	{
	}
	...
	inline TWorldPartitionHandle(UActorDescContainer* InContainer, const FGuid& ActorGuid)
		: ContainerInstance(nullptr)
		, ActorDescInstance(nullptr)
	{
	}
	...
	template<class U>
	inline TWorldPartitionHandle(TActorDescContainerCollection<U>* ContainerCollection, const FGuid& ActorGuid)
		: ContainerInstance(nullptr)
		, ActorDescInstance(nullptr)
	{
	...
	inline TWorldPartitionHandle(const TWorldPartitionHandle& Other)
		: ContainerInstance(nullptr)
		, ActorDescInstance(nullptr)
	{
		*this = Other;
	...
	inline TWorldPartitionHandle(TWorldPartitionHandle&& Other)
		: ContainerInstance(nullptr)
		, ActorDescInstance(nullptr)
	{
		*this = MoveTemp(Other);
	...
	inline TWorldPartitionHandle<FWorldPartitionHandleImpl> ToHandle() const
	{
		return Impl::ToHandle(*this);
	}

	inline TWorldPartitionHandle<FWorldPartitionReferenceImpl> ToReference() const
	{
		return Impl::ToReference(*this);
	}

	template <typename T>
	inline TWorldPartitionHandle<Impl>(TWorldPartitionHandle<T>&& Other)
		: ActorDescInstance(nullptr)
	{
		*this = MoveTemp(Other);
	}
	...
	inline TWorldPartitionHandle& operator=(const TWorldPartitionHandle& Other)
	{
		if (this != &Other)
		{
			if (IsValid())
	...
	inline TWorldPartitionHandle<Impl>& operator=(TWorldPartitionHandle&& Other)
	{
		if (this != &Other)
		{
			if (IsValid())
	...
	// Conversions
	template <typename T>
	inline TWorldPartitionHandle<Impl>& operator=(const TWorldPartitionHandle<T>& Other)
	{
		if (IsValid())
		{
			DecRefCount();
	...
	template <typename T>
	inline TWorldPartitionHandle<Impl>& operator=(TWorldPartitionHandle<T>&& Other)
	{
		if (IsValid())
		{
			DecRefCount();
	...
	inline FWorldPartitionActorDescInstance* operator->() const
	{
		return GetInstance();
	}
	...
	inline FWorldPartitionActorDescInstance* GetInstance() const
	{
	...
	friend inline uint32 GetTypeHash(const TWorldPartitionHandle<Impl>& HandleBase)
	{
		return ::PointerHash(HandleBase.ActorDescInstance);
	}

	inline bool operator==(const TWorldPartitionHandle& Other) const
	{
	...
	// Conversions
	template <typename T>
	inline bool operator==(const TWorldPartitionHandle<T>& Other) const
	{

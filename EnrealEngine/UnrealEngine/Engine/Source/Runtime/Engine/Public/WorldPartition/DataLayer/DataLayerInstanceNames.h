...
struct FDataLayerInstanceNames
{
	...
	{
		const int32 NonExternalDataLayersOffset = bInIsFirstDataLayerExternal && InDataLayers.Num() ? 1 : 0;
		const int32 NumNonExternalDataLayers = InDataLayers.Num() - NonExternalDataLayersOffset;
		const FName ExternalDataLayer = (NonExternalDataLayersOffset > 0) ? InDataLayers[0] : NAME_None;
		Initialize(MakeArrayView(NumNonExternalDataLayers ? &InDataLayers[NonExternalDataLayersOffset] : nullptr, NumNonExternalDataLayers), ExternalDataLayer);
	}

	FDataLayerInstanceNames(const TArray<FName>& InNonExternalDataLayers, FName InExternalDataLayer)
	{
		Initialize(InNonExternalDataLayers, InExternalDataLayer);
	}
...
private:
	...
	void Initialize(TArrayView<const FName> InNonExternalDataLayers, FName InExternalDataLayer)
	{
#if WITH_EDITOR
		bIsForcedEmptyNonExternalDataLayers = false;
#endif
		...
		DataLayers.Reserve(InNonExternalDataLayers.Num() + (bIsFirstDataLayerExternal ? 1 : 0));
		if (bIsFirstDataLayerExternal)
		{
			DataLayers.Add(InExternalDataLayer);
		}
		for (FName DataLayer : InNonExternalDataLayers)
		{
			if (DataLayer != InExternalDataLayer)
			{

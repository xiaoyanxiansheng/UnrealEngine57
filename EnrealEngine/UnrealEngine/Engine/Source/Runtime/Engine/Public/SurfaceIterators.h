...
{
public:
	static inline bool IsSuitable(const ULevel* Level)
	{
		return true;
	}
};
...
public:
	...
	inline UModel* GetModel()
	{
		check( !bReachedEnd );
		return World->GetLevel(LevelIndex)->Model;
	}

	inline int32 GetSurfaceIndex() const
	{
		check( !bReachedEnd );
		return SurfaceIndex;
	}

	inline int32 GetLevelIndex() const
	{
		check( !bReachedEnd );
		return LevelIndex;
	}
...
{
public:
	static inline bool IsSuitable(const FBspSurf* Surface)
	{
		return true;
	}
};

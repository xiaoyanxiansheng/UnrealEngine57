...
public:
	...
	void inline SetAudioDeviceID(Audio::FDeviceId DeviceID) { OwningDeviceID = DeviceID; }
...
public:
	...
	{
		UClass* ArgClass = nullptr;
		if constexpr (TIsIInterface<ArgType>::Value)
		{
			ArgClass = ArgType::UClassType::StaticClass();
		}
		else
		{
			ArgClass = ArgType::StaticClass();
		}

		ForEachSubsystemOfClass(ArgClass, [Function=MoveTemp(InFunction)](USubsystem* Subsystem)
		{
			Function(CastChecked<ArgType>(Subsystem));
		});
	}
};

...
public:
	...
	inline void SetTimer(FTimerHandle& InOutHandle, UserClass* InObj, typename FTimerDelegate::TMethodPtr< UserClass > InTimerMethod, float InRate, bool InbLoop = false, float InFirstDelay = -1.f)
	{
	...
	template< class UserClass >
	inline void SetTimer(FTimerHandle& InOutHandle, UserClass* InObj, typename FTimerDelegate::TConstMethodPtr< UserClass > InTimerMethod, float InRate, bool InbLoop = false, float InFirstDelay = -1.f)
	{
		InternalSetTimer(InOutHandle, FTimerUnifiedDelegate( FTimerDelegate::CreateUObject(InObj, InTimerMethod) ), InRate, InbLoop, InFirstDelay);
	}

	/** Version that takes any generic delegate. */
	inline void SetTimer(FTimerHandle& InOutHandle, FTimerDelegate const& InDelegate, float InRate, bool InbLoop, float InFirstDelay = -1.f)
	{
	...
	inline void SetTimer(FTimerHandle& InOutHandle, FTimerDynamicDelegate const& InDynDelegate, float InRate, bool InbLoop, float InFirstDelay = -1.f)
	{
	...
	inline void SetTimer(FTimerHandle& InOutHandle, float InRate, bool InbLoop, float InFirstDelay = -1.f)
	{
		InternalSetTimer(InOutHandle, FTimerUnifiedDelegate(), InRate, InbLoop, InFirstDelay);
	}
	/** Version that takes a TFunction */
	inline void SetTimer(FTimerHandle& InOutHandle, TFunction<void(void)>&& Callback, float InRate, bool InbLoop, float InFirstDelay = -1.f )
	{
	 ...
	 */
	template <class UserClass>
	inline void SetTimer(FTimerHandle& InOutHandle, UserClass* InObj, typename FTimerDelegate::TMethodPtr<UserClass> InTimerMethod, float InRate, const FTimerManagerTimerParameters& InTimerParameters)
	{
	...
	template< class UserClass >
	inline void SetTimer(FTimerHandle& InOutHandle, UserClass* InObj, typename FTimerDelegate::TConstMethodPtr<UserClass> InTimerMethod, float InRate, const FTimerManagerTimerParameters& InTimerParameters)
	{
		InternalSetTimer(InOutHandle, FTimerUnifiedDelegate(FTimerDelegate::CreateUObject(InObj, InTimerMethod)), InRate, InTimerParameters);
	}

	/** Version that takes any generic delegate. */
	inline void SetTimer(FTimerHandle& InOutHandle, FTimerDelegate const& InDelegate, float InRate, const FTimerManagerTimerParameters& InTimerParameters)
	{
		InternalSetTimer(InOutHandle, FTimerUnifiedDelegate(InDelegate), InRate, InTimerParameters);
	}
	/** Version that takes a dynamic delegate (e.g. for UFunctions). */
	inline void SetTimer(FTimerHandle& InOutHandle, FTimerDynamicDelegate const& InDynDelegate, float InRate, const FTimerManagerTimerParameters& InTimerParameters)
	{
		InternalSetTimer(InOutHandle, FTimerUnifiedDelegate(InDynDelegate), InRate, InTimerParameters);
	}
	/*** Version that doesn't take a delegate */
	inline void SetTimer(FTimerHandle& InOutHandle, float InRate, const FTimerManagerTimerParameters& InTimerParameters)
	{
		InternalSetTimer(InOutHandle, FTimerUnifiedDelegate(), InRate, InTimerParameters);
	}
	/** Version that takes a TFunction */
	inline void SetTimer(FTimerHandle& InOutHandle, TFunction<void(void)>&& Callback, float InRate, const FTimerManagerTimerParameters& InTimerParameters)
	{
	 ...
	 */
	template< class UserClass >
	inline FTimerHandle SetTimerForNextTick(UserClass* inObj, typename FTimerDelegate::TMethodPtr< UserClass > inTimerMethod)
	{
		return InternalSetTimerForNextTick(FTimerUnifiedDelegate(FTimerDelegate::CreateUObject(inObj, inTimerMethod)));
	}
	template< class UserClass >
	inline FTimerHandle SetTimerForNextTick(UserClass* inObj, typename FTimerDelegate::TConstMethodPtr< UserClass > inTimerMethod)
	{
		return InternalSetTimerForNextTick(FTimerUnifiedDelegate(FTimerDelegate::CreateUObject(inObj, inTimerMethod)));
	}

	/** Version that takes any generic delegate. */
	inline FTimerHandle SetTimerForNextTick(FTimerDelegate const& InDelegate)
	{
		return InternalSetTimerForNextTick(FTimerUnifiedDelegate(InDelegate));
	}
	/** Version that takes a dynamic delegate (e.g. for UFunctions). */
	inline FTimerHandle SetTimerForNextTick(FTimerDynamicDelegate const& InDynDelegate)
	{
		return InternalSetTimerForNextTick(FTimerUnifiedDelegate(InDynDelegate));
	}
	/** Version that takes a TFunction */
	inline FTimerHandle SetTimerForNextTick(TFunction<void(void)>&& Callback)
	{
	...
	*/
	inline void ClearTimer(FTimerHandle& InHandle)
	{
	 ...
	 */
	inline bool IsTimerActive(FTimerHandle InHandle) const
	{
		FTimerData const* const TimerData = FindTimer( InHandle );
	...
	*/
	 ...
	 */
	inline float GetTimerElapsed(FTimerHandle InHandle) const
	{
		FTimerData const* const TimerData = FindTimer(InHandle);
	 ...
	 */
	inline float GetTimerRemaining(FTimerHandle InHandle) const
	{
		FTimerData const* const TimerData = FindTimer(InHandle);
		return InternalGetTimerRemaining(TimerData);
	}

	bool inline HasBeenTickedThisFrame() const
	{
...
protected:
	...
	inline FTimerData const* FindTimer(FTimerHandle const& InHandle) const
	{
...
private:
	...
	inline const FTimerData& GetTimer(const FTimerHandle& InHandle) const
	{

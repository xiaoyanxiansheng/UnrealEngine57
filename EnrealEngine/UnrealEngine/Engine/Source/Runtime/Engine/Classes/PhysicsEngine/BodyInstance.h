...
struct FCollisionResponse
{
	...
	ECollisionResponse GetResponse(ECollisionChannel Channel) const { return ResponseToChannels.GetResponse(Channel); }
...
struct FBodyInstance : public FBodyInstanceCore
{
	...
	UPROPERTY(EditAnywhere, AdvancedDisplay, BlueprintReadOnly, Category=Physics, meta = (editcondition = "bOverrideIterationCounts", InlineEditConditionToggle, ClampMin = "0", UIMin = "0"))
...
private:
	...
	/** [EXPERIMENTAL] If true Partial Island Sleeping is permitted for the entire island this body is in */
	UPROPERTY(EditAnywhere, AdvancedDisplay, Category = Physics)
	uint8 bAllowPartialIslandSleep : 1;
...
public:
	...
	bool IsPartialIslandSleepAllowed() const { return bAllowPartialIslandSleep; }
...
public:
	...
	*/
	...
	/**
	 * [EXPERIMENTAL] Enable/disable Partial Island Sleeping for the entire island this body is in.
	 * Partial Island Sleeping permits the solver to freeze the body's dynamics ("sleep") if it has not moved significantly
	 * in the previous simulation frames even if connected bodies are still moving. The body will be unfrozen ("wake up")
	 * due to several possible triggers such as the detection of new collisions, creation of new constraints, and user interactions.
	 */
	ENGINE_API void SetAllowPartialIslandSleep(bool bInAllowPartialIslandSleep);

	// Set the per-body position iterations (only used if bOverrideIterationCounts is true)
	ENGINE_API void SetPositionSolverIterationCount(uint8 PositionSolverIterationCountIn);

	// Set the per-body velocity iterations (only used if bOverrideIterationCounts is true)
	ENGINE_API void SetVelocitySolverIterationCount(uint8 VelocitySolverIterationCountIn);

	// Set the per-body projection iterations (only used if bOverrideIterationCounts is true)
	ENGINE_API void SetProjectionSolverIterationCount(uint8 ProjectionSolverIterationCountIn);

	// Enable or disable the use of iteration count overrides
	ENGINE_API void SetOverrideIterationCounts(bool bOverride);

	// Get the per-body position iterations (or -1 if bOverrideIterationCounts is false)
	int32 GetPositionSolverIterationCount() const { return (!!bOverrideIterationCounts) ? PositionSolverIterationCount : -1; }

	// Get the per-body velocity iterations (or -1 if bOverrideIterationCounts is false)
	int32 GetVelocitySolverIterationCount() const { return (!!bOverrideIterationCounts) ? VelocitySolverIterationCount : -1; }

	// Get the per-body projection iterations (or -1 if bOverrideIterationCounts is false)
	int32 GetProjectionSolverIterationCount() const { return (!!bOverrideIterationCounts) ? ProjectionSolverIterationCount : -1; }
	...
	bool GetAllowPartialIslandSleep() const { return bAllowPartialIslandSleep != 0; }
...
public:
	...
	inline ECollisionResponse GetResponseToChannel(ECollisionChannel Channel) const { return CollisionResponses.GetResponse(Channel); }
	...
	inline const FCollisionResponseContainer& GetResponseToChannels() const { return CollisionResponses.GetResponseContainer(); }
	...
	inline ECollisionChannel GetObjectType() const { return ObjectType; }
...
public:
	...
	inline ECollisionEnabled::Type GetCollisionEnabled(bool bCheckOwner = true) const
	{
	...
	inline FMaskFilter GetMaskFilter() const { return MaskFilter; }
...
inline bool FBodyInstance::OverlapTestForBodies(const FVector& Position, const FQuat& Rotation, const TArray<FBodyInstance*>& Bodies, bool bTraceComplex) const
{
	return OverlapTestForBodiesImpl(Position, Rotation, Bodies, bTraceComplex);
}

inline bool FBodyInstance::OverlapTestForBody(const FVector& Position, const FQuat& Rotation, FBodyInstance* Body, bool bTraceComplex) const
{
...
inline bool FBodyInstance::IsInstanceSimulatingPhysics() const
{

...
public:
	...
	ENGINE_API void ApplyMigratedPhysicsState(Chaos::FSerializedDataBufferPtr MigratedStateData = nullptr);
	ENGINE_API bool HasValidPhysicsState() const;
#endif

	UFUNCTION()
	ENGINE_API void ReInitConstraintAndApplyPhysicsState(UPrimitiveComponent* ChangedComponent, EComponentPhysicsStateChange StateChange);
...
private:
...
#if UE_WITH_REMOTE_OBJECT_HANDLE
	bool CanReInitConstraintAndApplyPhysicsState() const;

	void MigratePhysicsState(FArchive& Ar);
	...
	void CheckAndUnbindPhysicsStateDelegates();
#endif
};

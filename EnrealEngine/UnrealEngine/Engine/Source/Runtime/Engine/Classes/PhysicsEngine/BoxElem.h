...
struct FKBoxElem : public FKShapeElem
{
	...
	FVector::FReal GetVolume(const FVector& Scale3D) const { return GetScaledVolume(Scale3D); }

	FVector::FReal GetScaledVolume(const FVector& Scale3D) const { return FMath::Abs(Scale3D.X * Scale3D.Y * Scale3D.Z * X * Y * Z); }

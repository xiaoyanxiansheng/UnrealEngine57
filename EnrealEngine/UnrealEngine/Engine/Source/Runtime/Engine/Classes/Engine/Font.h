...
class UFont : public UObject, public IFontProviderInterface
{
	...
	/** Where is the runtime font data for this Font stored? */
	UPROPERTY(EditAnywhere, AdvancedDisplay, Category=RuntimeFont)
	ERuntimeFontSource RuntimeFontSource = ERuntimeFontSource::Asset;
	...
	UE_DEPRECATED(5.7, "Public access to CompositeFont is deprecated. Use GetCompositeFont(), GetInternalCompositeFont(), or GetMutableInternalCompositeFont() instead.")
	UPROPERTY(EditAnywhere, Category = RuntimeFont)
...
public:
	...
	ENGINE_API virtual const FCompositeFont* GetCompositeFont() const override;

	inline const FCompositeFont& GetInternalCompositeFont() const
	{
	...
	inline FCompositeFont& GetMutableInternalCompositeFont()
	{
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		return CompositeFont;
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	{
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		return GET_MEMBER_NAME_CHECKED(UFont, CompositeFont);
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
	}

	/** Get the info needed to use this UFont with Slate, using the fallback data for legacy Canvas APIs */
	ENGINE_API FSlateFontInfo GetLegacySlateFontInfo() const;
	 ...
	 */
	inline void SetFontScalingFactor(float InScalingFactor)
	{
	 ...
	 */
	inline float GetFontScalingFactor() const
	{

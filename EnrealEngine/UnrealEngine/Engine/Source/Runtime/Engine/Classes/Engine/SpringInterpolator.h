...
{
	static inline bool IsValidValue(float Value, float MaxAbsoluteValue = RK4_SPRING_INTERPOLATOR_MAX_VALUE)
	{
		return FMath::Abs(Value) < MaxAbsoluteValue;
	}

	static inline bool IsValidValue(FVector Value, float MaxAbsoluteValue = RK4_SPRING_INTERPOLATOR_MAX_VALUE)
	{
		return Value.GetAbsMax() < MaxAbsoluteValue;
	}

	static inline bool AreEqual(float A, float B, float ErrorTolerance = UE_KINDA_SMALL_NUMBER)
	{
		return FMath::IsNearlyEqual(A, B, ErrorTolerance);
	}

	static inline bool AreEqual(FVector A, FVector B, float ErrorTolerance = UE_KINDA_SMALL_NUMBER)
	{

...
public:
	...
	inline AssetType* GetPrimaryAssetObject(const FPrimaryAssetId& PrimaryAssetId) const
	{
	...
	template<class AssetType>
	inline TSubclassOf<AssetType> GetPrimaryAssetObjectClass(const FPrimaryAssetId& PrimaryAssetId) const
	{
	...
	/**
	 * Returns PrimaryAssetIds that manage whether a package is included in the cook, with property describing the
	 * reference (direct or indirect).
	 */
	ENGINE_API virtual bool GetPackageCookRuleManagers(FName PackageName, bool bRecurseToParents,
		TMap<FPrimaryAssetId, UE::AssetRegistry::EDependencyProperty>& Managers) const;

	/**
	 * Returns PrimaryAssetIds that manage a package in any way, either chunk assignment or inclusion in the cook.
	 * Chunk assignments are a superset of inclusion in the cook assignments, so this is a supserset of
	 * GetPackageCookRuleManagers.
	 * Includes property describing the reference (direct or indirect, and cookrule or not).
	 */
...
protected:
	ENGINE_API bool GetPackageManagersInternal(FName PackageName,
...
public:
	...
	ENGINE_API virtual bool GetPrimaryAssetSetChunkIds(const TSet<FPrimaryAssetId>& PrimaryAssetSet, const ITargetPlatform* TargetPlatform,
		TArrayView<const int32> ExistingChunkList, TArray<int32>& OutChunkList) const;
	ENGINE_API virtual bool GetPrimaryAssetSetChunkIds(TConstArrayView<FPrimaryAssetId> PrimaryAssets, const ITargetPlatform* TargetPlatform,
		TArrayView<const int32> ExistingChunkList, TArray<int32>& OutChunkList) const;

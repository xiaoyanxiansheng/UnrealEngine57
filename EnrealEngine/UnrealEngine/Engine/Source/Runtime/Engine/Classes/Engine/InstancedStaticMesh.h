...
public:
	...
	inline void GetInstanceCustomDataValues(int32 InstanceIndex, TArray<float>& InstanceCustomData) const
	{
		InstanceData->GetInstanceCustomDataValues(InstanceIndex, InstanceCustomData);
	}
	
	inline FStaticMeshInstanceData* GetInstanceData() const
	{
...
public:
	...
	const FStaticMeshRayTracingProxyLODArray* RayTracingProxyLODs = nullptr;
...
public:
	...
	ENGINE_API virtual void SetEvaluateWorldPositionOffsetInRayTracing(FRHICommandListBase& RHICmdList, bool bEvaluate) override;
...
#if RHI_RAYTRACING
	ENGINE_API virtual void GetRayTracingWPOConfig(bool& bOutHasRayTracingRepresentation, bool& bOutDynamicRayTracingGeometry) const override;

	ENGINE_API void SetupRayTracingDynamicInstances(FRHICommandListBase& RHICmdList, int32 NumDynamicInstances, int32 LODIndex);
	ENGINE_API void ReleaseRayTracingDynamicInstances();

	struct FRayTracingDynamicInstanceData
	{
		FRayTracingGeometry Geometry;
		FRWBuffer VertexBuffer;
	};

	TArray<FRayTracingDynamicInstanceData> RayTracingDynamicInstanceData;
...
private:
	...
	bool CanSetupRayTracingInstancedMeshBatch(int32 LODIndex) const;
	...
	/** Stores a loose uniform buffer used for ray tracing. */
	FInstancedStaticMeshVFLooseUniformShaderParametersRef RayTracingLooseUniformBuffer;

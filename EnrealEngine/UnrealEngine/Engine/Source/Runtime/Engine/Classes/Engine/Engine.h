...
private:
	UPROPERTY()
	TObjectPtr<class UFont> MonospaceFont;

public:
	/** Sets the font used for monospace engine text, e.g. in debug displays */
	UPROPERTY(globalconfig, EditAnywhere, Category=Fonts, meta=(AllowedClasses="/Script/Engine.Font", DisplayName="Monospace Font", ConfigRestartRequired=true), AdvancedDisplay)
...
public:
	...
	UPROPERTY()
	TObjectPtr<class UMaterialInterface> ClayMaterial;

	/** A material used to render the geometry inspection clay mode. */
	UPROPERTY(globalconfig)
	FSoftObjectPath ClayMaterialName;

	UPROPERTY()
	TObjectPtr<class UMaterial> ZebraMaterial;

	UPROPERTY()
	TObjectPtr<class UMaterialInstanceDynamic> ZebraMaterialInstance;

	/** A material used to render the geometry inspection zebra mode. */
	UPROPERTY(globalconfig)
	FSoftObjectPath ZebraMaterialName;
	
	UPROPERTY()
	TObjectPtr<class UMaterialInterface> FrontBackFaceMaterial;

	/** A material used to render the geometry inspection front/back face mode. */
	UPROPERTY(globalconfig)
	FSoftObjectPath FrontBackFaceMaterialName;

	UPROPERTY()
	TObjectPtr<class UMaterialInterface> RandomColorMaterial;

	/** A material used to render the geometry inspection random color mode. */
	UPROPERTY(globalconfig)
	FSoftObjectPath RandomColorMaterialName;
...
public:
	...
	inline void ResumeDynamicResolution()
	{
	...
	inline class IDynamicResolutionState* GetDynamicResolutionState()
	{
		#if !WITH_DYNAMIC_RESOLUTION
			return nullptr;
		#else
	...
	inline bool GetDynamicResolutionUserSetting() const
	{
		#if !WITH_DYNAMIC_RESOLUTION
			return false;
		#else
	...
	inline void SetDynamicResolutionUserSetting(bool Enable)
	{
		#if WITH_DYNAMIC_RESOLUTION
...
public:
	 ...
	 */
	static ENGINE_API class UFont* GetMonospaceFont();

...
struct FLocalPlayerContext
{
	...
	template<class T>
	inline T* GetPlayerController(bool bCastChecked = false) const
	{
		if (bCastChecked)
		{ 
	...
	template<class T>
	inline T* GetGameState(bool bCastChecked = false) const
	{
		if (bCastChecked)
		{
	...
	template<class T>
	inline T* GetPlayerState(bool bCastChecked = false) const
	{
		if (bCastChecked)
		{
	...
	template<class T>
	inline T* GetHUD(bool bCastChecked = false) const
	{
		if (bCastChecked)
		{
	...
	template<class T>
	inline T* GetPawn(bool bCastChecked = false) const
	{
		if (bCastChecked)
		{
...
public:
	...
	/** Whether or not we're emulating splitscreen with this player. Should only be set from screenshot functional tests. */
	bool bEmulateSplitscreen = false;
	
	/** 
	 * Set the connection identifier for this local user. 
	 *
	 * The connection identifier is a unique identifier for this local user on a client and is used to 
	 * associate the local player that began a connection handshake with a player controller replicated
	 * to the client when the connection handshake is complete.
	 *
	 * Only set this if you are connecting to a game server outside of the normal connection flow in the 
	 * engine (e.g. you are sending NMT_Join manually to initiate a connection).
	 * 
	 ...
	 */
	ENGINE_API void SetConnectionIdentifier(const int32 Id);

	/** Get the client connection identifier for this local user. */
	ENGINE_API int32 GetConnectionIdentifier() const;
...
public:
	 ...
	 */
	template <typename TSubsystemClass>
	static inline TSubsystemClass* GetSubsystem(const ULocalPlayer* LocalPlayer)
	{
		if (LocalPlayer)
		{
	...
	/**
	 * Get a subsystem of the specificied type from the provided Player Controller's owning ULocalPlayer.
	 * Returns nullptr if the Subsystem cannot be found or the given controller or its local player are null.
	 */
	template <typename TSubsystemClass>

...
public:
	...
	std::atomic<uint64> FixupOverrideVertexColorsTimeMS = 0;
	std::atomic<uint32> FixupOverrideVertexColorsCount = 0;
	...
	ENGINE_API static bool GetIsLevelCookedFromAsset(const FAssetData& Asset);
	ENGINE_API static bool GetIsLevelCookedFromPackage(FName LevelPackage);

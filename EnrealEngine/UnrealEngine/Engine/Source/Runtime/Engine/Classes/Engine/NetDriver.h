...
{
	inline bool operator()( const FActorPriority& A, const FActorPriority& B ) const
	{
...
public:
	...
	*/
	...
	UPROPERTY()
	TObjectPtr<UClass> ChildNetConnectionClass;
	...
	inline bool IsKnownChannelName(const FName& ChName) const
	{
...
protected:
	 ...
	 *
	 * If a sub-class needs to create new metrics, those metrics should be added to the metrics database in the sub-class
	 * implementation of this function. Metrics should only be added to the database after calling Super::SetupNetworkMetrics().
	 */
	ENGINE_API virtual void SetupNetworkMetrics();
...
private:
	...
	void SetupNetworkMetricsListeners(bool bIsClient);
...
private:
	...
	/** A set of replicators that have queued RPCs for transactions (server only) */
	TSet< FObjectReplicator* > ReplicatorsWithPendingTransactions;
...
public:
	 ...
	 */
	virtual bool IsAvailable() const PURE_VIRTUAL( UNetDriver::IsAvailable, return false;)
...
public:
	 ...
	 */
	virtual void LowLevelSend(TSharedPtr<const FInternetAddr> Address, void* Data, int32 CountBits, FOutPacketTraits& Traits)
	...
	ENGINE_API virtual void NotifyActorDestroyed(AActor* Actor, bool IsSeamlessTravel = false, EChannelCloseReason CloseReason = EChannelCloseReason::Destroyed);
	...
	ENGINE_API virtual void RemoveNetworkActor(AActor* Actor);
	...
	*/
	...
	inline bool IsUsingIrisReplication() const
	{
		return bIsUsingIris;
	}
...
public:
	...
	ENGINE_API virtual bool ShouldClientDestroyActor(AActor* Actor, EChannelCloseReason CloseReason = EChannelCloseReason::Destroyed) const;
...
private:
	/** Reset all the per-frame net connection stats */
	void ResetNetConnectionStats();
...
public:
	...
	/**
	 * Used by UNetConnection after receiving a packet to determine whether to send an ack packet on the next
	 * connection flush regardless of whether there's any other data to send. Set via the config property bSendImmediateAcks.
	 */
	bool IsSendingImmediateAcks() const
	{
		return bSendImmediateAcks;
	}
...
private:
	...
	/** Private helper for ServerReplicateActors */
	void ServerReplicateActors_ForConnection(UE::Net::FServerReplicateActors_ForConnectionParams& Params);
...
private:
	...
	bool bSkipStartReplicatingWorldForIris = false;
	...
	/** Transient ConsiderList only used inside ServerReplicateActors. It's a member so that it can be easily updated between transactions when bReplicateTransactionally is true to remove stale entries. */
	TArray<FNetworkObjectInfo*> CurrentConsiderList;
	...
	/**
	 * If true, after receiving a packet, send an ack packet on the next connection flush regardless of whether
	 * there's any other data to send. This is the default.
	 * If false, the ack won't be sent until other data is ready or the KeepAliveTime is reached. This can help
	 * reduce the number of individual packets sent.
	 */
	UPROPERTY(Config)
	bool bSendImmediateAcks = true;
};

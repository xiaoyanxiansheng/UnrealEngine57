...
struct FTextureSource
{
	...
	inline static bool IsHDR(ETextureSourceFormat Format) { return UE::TextureDefines::IsHDR(Format); }
	...
	*/
	...
	inline bool GetMipData(TArray64<uint8>& OutMipData, int32 MipIndex, class IImageWrapperModule* ImageWrapperModule = nullptr)
	{
	...
	inline bool GetMipImage(FImage & OutImage, int32 MipIndex)
	{
	...
	inline ETextureSourceCompressionFormat GetSourceCompression() const { return CompressionFormat; }
	inline bool IsSourceCompressed() const { return GetSourceCompression() != ETextureSourceCompressionFormat::TSCF_None; }
	...
	inline int64 GetSizeX() const { return SizeX; }
	inline int64 GetSizeY() const { return SizeY; }
	inline int32 GetNumSlices() const { return NumSlices; }
	inline int32 GetNumMips() const { return NumMips; }
	inline int32 GetNumLayers() const { return NumLayers; }
	inline int32 GetNumBlocks() const { return Blocks.Num() + 1; }
	inline ETextureSourceFormat GetFormat(int32 LayerIndex = 0) const { return (LayerIndex == 0) ? Format : LayerFormat[LayerIndex]; }
	...
	inline int64 GetSizeOnDisk() const { return BulkData.GetPayloadSize(); }
	inline bool HasPayloadData() const { return BulkData.HasPayloadData(); } // this is the same as GetSizeOnDisk() != 0
	...
	inline bool IsBulkDataLoaded() const { return true; }
	...
	inline bool LoadBulkDataWithFileReader() { return true; }
	...
	inline int64 CalcMipSize(int32 MipIndex) const { return CalcMipSize(0, 0, MipIndex); }
	/** Lock a mip for reading. */
	inline const uint8* LockMipReadOnly(int32 MipIndex) { return LockMipReadOnly(0, 0, MipIndex); }
	/** Lock a mip for editing. */
	inline uint8* LockMip(int32 MipIndex) { return LockMip(0, 0, MipIndex); }
	inline void UnlockMip(int32 MipIndex) { UnlockMip(0, 0, MipIndex); }
...
public:
	...
	inline void ReleaseSourceMemory() { }

	UE_DEPRECATED(5.5, "Remove call to HasHadBulkDataCleared.")
	inline bool HasHadBulkDataCleared() const { return false; }
#endif // WITH_EDITOR
...
public:
	...
	void TouchStreamingMipDDCData(int32 InMipCount, FStringView DebugContext);
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	 ...
	 */
	UPROPERTY(EditAnywhere, Category = Texture, AdvancedDisplay, meta=(EditCondition = "VirtualTextureStreaming"))
...
public:
	...
	/** Get Texture Class */
	virtual ETextureClass GetTextureClass() const PURE_VIRTUAL(UTexture::GetTextureClass, return ETextureClass::Invalid; );
	...
	*/
	...
	{
#if WITH_EDITORONLY_DATA
		LightingGuid = BuildLightingGuid();
#else
...
#if WITH_EDITORONLY_DATA
	/**
	 * Creates a Guid for the texture based on the current texture values. Default version returns a new unique Guid.
	 * Some subclasses override this to calculate it from a hash.
	 */
	ENGINE_API virtual FGuid BuildLightingGuid();
	/** Helper for subclasses that want to implement BuildLightingGuid from a deterministic hash of the texture. */
	ENGINE_API FGuid BuildLightingGuidFromHash();
#endif
	...
	static inline int32 GetStaticMinTextureResidentMipCount()
	{

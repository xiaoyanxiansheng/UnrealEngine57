...
public:
	...
	/**
	 * Declare a delegate funtion signiture for a function that is registered in Initialize() to be called if we call Initialize() without calling Release().
	 * This can happen for the case where the RuntimeVirtualTexture is registered twice. For example in PIE and the main editor.
	 * In that case we want to notify the first caller.
	 */
	DECLARE_DELEGATE(FOnReInitDelegate);

	/** (Re)Initialize this object. Call this whenever we modify the producer or transform. */
	ENGINE_API void Initialize(IVirtualTexture* InProducer, FVTProducerDescription const& InProducerDesc, FTransform const& InVolumeToWorld, FBox const& InWorldBounds, FOnReInitDelegate InDelegate);
	UE_DEPRECATED(5.6, "Use the version that takes a FOnReInitDelegate instead.")
...
private:
	...
	/** Delegate to be called on ReInitialize(). */
	FOnReInitDelegate OnReInitDelegate; 
};

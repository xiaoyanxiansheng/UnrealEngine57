...
public:
	...
	inline bool IsTickManaged()const { return ManagerHandle != INDEX_NONE && !IsPendingManagerRemove(); }
	inline int32 GetManagerHandle()const { return ManagerHandle; }
	inline void SetManagerHandle(int32 InHandle) { ManagerHandle = InHandle; }

	inline int32 IsPendingManagerAdd()const { return bPendingManagerAdd; }
	inline void SetPendingManagerAdd(bool bValue) { bPendingManagerAdd = bValue; }
	inline int32 IsPendingManagerRemove()const { return bPendingManagerRemove; }
	inline void SetPendingManagerRemove(bool bValue) { bPendingManagerRemove = bValue; }
...
public:
	...
	inline void SetManagingSignificance(bool bManageSignificance)
	{
	...
	inline const FTransform& GetAsyncComponentToWorld()
	{
	...
	inline const TArray<struct FParticleSysParam>& GetAsyncInstanceParameters()
	{
		if (!bParallelRenderThreadUpdate && !IsInGameThread())
		{
			check(bAsyncDataCopyIsValid); 
	...
	inline const FBoxSphereBounds& GetAsyncBounds()
	{
		if (!bParallelRenderThreadUpdate && !IsInGameThread())
		{
			check(bAsyncDataCopyIsValid);
	...
	inline const FVector& GetAsyncPartSysVelocity()
	{
		if (!bParallelRenderThreadUpdate && !IsInGameThread())
		{
			check(bAsyncDataCopyIsValid);
	...
	inline const TArray<FVector>& GetPlayerLocations()
	{
		return PlayerLocations;
	}

	inline const TArray<float>& GetPlayerLODDistanceFactor()
	{
...
private:
	...
	inline void SetAsyncWork(FGraphEventRef& InAsyncWork) { AsyncWork = InAsyncWork; }
...
public:
	...
	inline void ForceAsyncWorkCompletion(EForceAsyncWorkCompletion Behavior, bool bDefinitelyGameThread, bool InSkipUpdateDynamicDataDuringTick)
	{
	...
	/** If there is async work outstanding, force it to be completed now **/
	inline void ForceAsyncWorkCompletion(EForceAsyncWorkCompletion Behavior, bool bDefinitelyGameThread = true) const
	{
		if (AsyncWork.GetReference())
		{
			WaitForAsyncAndFinalize(Behavior, bDefinitelyGameThread);
...
public:
	inline int32 GetCurrentLODIndex() const
	{
		return LODLevel;
	}
...
public:
	inline FParticlePerfStatsContext GetPerfStatsContext(){ return FParticlePerfStatsContext(GetWorld(), Template, this); }
};
...
inline void UParticleSystemComponent::SetAutoAttachParams(USceneComponent* Parent, FName SocketName, EAttachLocation::Type LocationType)
{

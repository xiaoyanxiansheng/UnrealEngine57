...
using FReplacementComponentMap = TMap<UObject*, UObject*>;
DECLARE_MULTICAST_DELEGATE_OneParam(FActorOnComponentsReplaced, const FReplacementComponentMap&);
#endif
...
#if WITH_EDITORONLY_DATA
	/** If true, this actor can be migrated to another server even if it's been loaded from disk */
	UPROPERTY(Category = Actor, EditAnywhere, AdvancedDisplay)
	uint8 bMigratingAsset : 1;
#endif
...
private:
	...
	/** Set for actors created from networking that are not yet fully initialized */
	uint8 bActorIsPendingPostNetInit : 1;	
...
public:
	...
	inline const FTransform& GetActorTransform() const
	{
	...
	inline FRotator GetActorRotation() const
	{
	...
	inline FQuat GetActorQuat() const
	{
	...
	inline bool ShouldCookOptimizedBPComponentData() const
	{
	...
	UE_DEPRECATED(5.7, "SetBrowseToAssetOverride is deprecated as it was unreliable. Register a callback with UBrowseToAssetOverrideSubsystem instead, and store the override as a property on your actor.")
	...
	UE_DEPRECATED(5.7, "GetBrowseToAssetOverride is deprecated. Use UBrowseToAssetOverrideSubsystem::GetBrowseToAssetOverride instead.")
...
public:
	...
	inline bool CanEverTick() const { return PrimaryActorTick.bCanEverTick; }
...
public:
	...
	inline bool HasDeferredComponentRegistration() const { return bHasDeferredComponentRegistration; }
	 ...
	 */
	ENGINE_API virtual void FillReplicationParams(const FFillReplicationParamsContext& Context, FActorReplicationParams& OutParams);
...
protected:
	 ...
	 */
	ENGINE_API virtual void OnReplicationStartedForIris(const FOnReplicationStartedParams&);
	 ...
	 */
	ENGINE_API virtual void OnStopReplicationForIris(const FOnStopReplicationParams&);
	...
	*/
	UE_DEPRECATED(5.7, "This is being deprecated as part of iris beta. Use OnReplicationStartedForIris or FillReplicationParams overrides instead.")
	...
	*/
	UE_DEPRECATED(5.7, "This is being deprecated as part of iris beta. Override OnStopReplicationForIris instead.")
	...
	*/
	UE_DEPRECATED(5.7, "This is being deprecated as part of iris beta. Do not call directly override FillReplicationParams instead to provide custom parameters.")
	...
	friend struct UE::Net::FReplicationSystemUtil;
...
#if WITH_EDITOR
	FActorOnComponentsReplaced OnComponentsReplaced;
#endif
...
private:
	...
	template<class T>
	static inline const FTransform& TemplateGetActorTransform(const T* RootComponent)
	{
	...
	template<class T>
	static inline FVector TemplateGetActorScale(const T* RootComponent)
	{
		return (RootComponent != nullptr) ? RootComponent->GetComponentScale() : FVector(1.f,1.f,1.f);
	}

	template<class T>
	static inline FQuat TemplateGetActorQuat(const T* RootComponent)
	{
		return (RootComponent != nullptr) ? RootComponent->GetComponentQuat() : FQuat(ForceInit);
	}

	template<class T>
	static inline FVector TemplateGetActorForwardVector(const T* RootComponent)
	{
		return (RootComponent != nullptr) ? RootComponent->GetForwardVector() : FVector::ForwardVector;
	}

	template<class T>
	static inline FVector TemplateGetActorUpVector(const T* RootComponent)
	{
		return (RootComponent != nullptr) ? RootComponent->GetUpVector() : FVector::UpVector;
	}

	template<class T>
	static inline FVector TemplateGetActorRightVector(const T* RootComponent)
	{
...
public:
	...
	{
		bActorIsPendingPostNetInit = bInIsPendingPostNetInit;
	}
...
inline FVector AActor::K2_GetActorLocation() const
{
	return GetActorLocation();
}

inline FRotator AActor::K2_GetActorRotation() const
{
	return GetActorRotation();
}

inline USceneComponent* AActor::K2_GetRootComponent() const
{
	return GetRootComponent();
}

inline FVector AActor::GetActorForwardVector() const
{
	return TemplateGetActorForwardVector(ToRawPtr(RootComponent));
}

inline FVector AActor::GetActorUpVector() const
{
	return TemplateGetActorUpVector(ToRawPtr(RootComponent));
}

inline FVector AActor::GetActorRightVector() const
{
	return TemplateGetActorRightVector(ToRawPtr(RootComponent));
}


inline float AActor::GetSimpleCollisionRadius() const
{
...
inline float AActor::GetSimpleCollisionHalfHeight() const
{
	float Radius, HalfHeight;
	GetSimpleCollisionCylinder(Radius, HalfHeight);
	return HalfHeight;
...
inline FVector AActor::GetSimpleCollisionCylinderExtent() const
{
	float Radius, HalfHeight;
	GetSimpleCollisionCylinder(Radius, HalfHeight);
...
inline bool AActor::GetActorEnableCollision() const
{
...
inline AActor* AActor::GetOwner() const
{ 
	return Owner; 
}
...
inline ENetRole AActor::GetRemoteRole() const
{
	return RemoteRole;
}

inline ENetMode AActor::GetNetMode() const
{
...
inline bool AActor::IsNetMode(ENetMode Mode) const
{
#if UE_EDITOR

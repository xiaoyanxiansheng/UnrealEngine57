...
protected:
	...
	*/
	...
	{
		return GetDeviceSpecificData<TDataLayout>(UserId, DeviceId, InMap);
	}
...
const TDataLayout* UInputDeviceProperty::GetDeviceSpecificData(const FPlatformUserId UserId, const FInputDeviceId DeviceId, const TMap<FName, TDataLayout>& InDeviceData) const
{
	...
	{
		// If we were given a valid device, use that. Otherwise, default to the player's most recently used device
		const FHardwareDeviceIdentifier Hardware = DeviceId.IsValid() ? SubSystem->GetInputDeviceHardwareIdentifier(DeviceId) : SubSystem->GetMostRecentlyUsedHardwareDevice(UserId);

	...
	: public UObject
{
	 ...
	 */
	UE_DEPRECATED(5.7, "bEnableDynamicComponentInputBinding is deprecated.")
	UPROPERTY(config, meta = (DeprecatedProperty, DeprecationMessage = "bEnableDynamicComponentInputBinding is deprecated, it will be treated as always being true."))
	...
	/**
	 * If true, then any input action or axis mappings which are marked as deprecated will throw a warning.
	 */
	UPROPERTY(config, EditAnywhere, Category="Bindings")
	uint8 bEnabledLegacyMappingDeprecationWarnings : 1;
...
private:
	...
	/**
	 * List of FNames, where each name is either an Action or Axis mapping, which are marked as deprecated.
	 * 
	 * Deprecated action and axis mappings will trigger Blueprint compilation warnings and log warnings when bound
	 * to in C++ code.
	 */
	UPROPERTY(config, VisibleAnywhere, Category="Bindings")
	TSet<FName> DeprecatedActionAndAxisNames;

	/** List of Speech Mappings */
	UE_DEPRECATED(5.7, "SpeechMappings are deprecated. These were only used by a now deprecated platform.")
...
public:
	...
	ENGINE_API bool IsActionMappingDeprecated(const FName ActionName) const;
	ENGINE_API void SetActionMappingDeprecated(const FName ActionName, const bool bIsDeprecated);
	...
	UE_DEPRECATED(5.7, "Speech Mappings are deprecated. These were only used by a now deprecated platform.")
	PRAGMA_DISABLE_DEPRECATION_WARNINGS

...
protected:
	ENGINE_API virtual void OnReplicationStartedForIris(const FOnReplicationStartedParams&) override;
...
public:
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Setter=SetShowMouseCursor, Category=MouseInterface)
...
public:
	...
	*/
	...
	/** Generates a timestamp for the upcoming physics step. Useful for synchronizing client and server events on a specific physics step 
	* On the server LocalFrame and ServerFrame are the same, on the client LocalFrame is the upcoming physics frame and ServerFrame is the server-side frame that the LocalFrame corresponds to.
	*/
	ENGINE_API FAsyncPhysicsTimestamp GetPhysicsTimestamp();

	UE_DEPRECATED(5.7, "Deprecated, use the GetPhysicsTimestamp() without the DeltaSeconds parameter, it will return the upcoming frame as intended.")
	FAsyncPhysicsTimestamp GetPhysicsTimestamp(float DeltaSeconds) { return GetPhysicsTimestamp(); }
...
private:
	...
	int32 LocalPlayerConnectionIdentifier;
};
...
public:
	...
	GENERATED_BODY()
	ANoPawnPlayerController(const FObjectInitializer& ObjectInitializer);
	...
	UFUNCTION(Server, Unreliable)
	void ServerSetViewTargetPosition(FVector ViewTargetPos);
	...
	void ServerEnableReplicationToConnection(bool bEnable);

	bool IsEnableReplicationToConnection() const { return bEnableReplicationToConnection; }

private:
	
	bool bEnableReplicationToConnection = true;
};

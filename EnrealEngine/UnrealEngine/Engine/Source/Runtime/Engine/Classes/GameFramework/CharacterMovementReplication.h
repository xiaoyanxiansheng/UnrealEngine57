...
public:
	...
	inline FCharacterNetworkMoveData* GetNewMoveData() const		{ return NewMoveData; }
	inline FCharacterNetworkMoveData* GetPendingMoveData() const	{ return PendingMoveData; }
	inline FCharacterNetworkMoveData* GetOldMoveData() const		{ return OldMoveData; }
...
public:
	...
	TObjectPtr<UPrimitiveComponent> NewBase;
#else
	...
	void Serialize(FArchive& Archive)
	{
		if (Archive.IsMigratingRemoteObjects())
		{
			...
			Archive << NewVel;
			Archive << NewRot;
			Archive << GravityDirection;
			Archive << NewBase;
			Archive << NewBaseBoneName;
			Archive.SerializeBits(&bAckGoodMove, 1);
			Archive.SerializeBits(&bBaseRelativePosition, 1);
			Archive.SerializeBits(&bBaseRelativeVelocity, 1);

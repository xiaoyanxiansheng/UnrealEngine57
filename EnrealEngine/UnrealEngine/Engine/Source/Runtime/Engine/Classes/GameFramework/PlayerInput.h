...
public:
	...
	FInputActionSpeechMapping(const FInputActionSpeechMapping&) = default;
	FInputActionSpeechMapping(FInputActionSpeechMapping&&) = default;
	FInputActionSpeechMapping& operator=(const FInputActionSpeechMapping&) = default;
	FInputActionSpeechMapping& operator=(FInputActionSpeechMapping&&) = default;
...
public:
	...
	* Use + to addmodifiers; do not include any spaces between modifiers, so that it is not conflated with the command to be bound.
	* example usage: 
	* SetBind k serverexec slomo 0.1
	* SetBind shift+k serverexec slomo 1
	*/
	UFUNCTION(exec)
...
protected:
	...
	/** 
	* Called just before iteration every input component on the stack to process their bound listener delegates.
	* This provides an extension point to handle modifying the current input state before determining which input delegates should be fired.
	*/
	 ...
	 * 
	 * @return True if this input component should BLOCK the rest of the input stack, preventing further components from evaluating their delegates.
	 */
	ENGINE_API virtual bool EvaluateInputComponentDelegates(UInputComponent* const IC, const TArray<TPair<FKey, FKeyState*>>& KeysWithEvents, const float DeltaTime, const bool bGamePaused);
	
	/** 
	* Called on any input component which was not evaluated due to the input being blocked by another component earlier in the stack.
	* These input components will not have had the EvaluateInputComponentDelegates flow executed, because they are blocked.
	* This is a good spot to reset any delegate values if desired to reflect that they didn't execute for next frame.
	*/
	ENGINE_API virtual void EvaluateBlockedInputComponent(UInputComponent* InputComponent);
...
public:
	...
	ENGINE_API virtual const TArray<FInputActionKeyMapping>& GetKeysForAction(const FName ActionName) const;

	/** Returns the list of keys mapped to the specified Axis Name */
	ENGINE_API virtual const TArray<FInputAxisKeyMapping>& GetKeysForAxis(const FName AxisName) const;
...
private:
	...
	inline void ConditionalBuildKeyMappings() const
	{
		if (!bKeyMapsBuilt)
		{

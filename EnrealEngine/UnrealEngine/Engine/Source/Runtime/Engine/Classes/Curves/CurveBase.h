...
public:
	...
	{
		// @todo: remove this entire implementation, since the default behavior should be to not return anything,
		// once the GetCurves 5.6 deprecation gets removed
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		const TArray<FRichCurveEditInfoConst> TempCurves = GetCurves();
		PRAGMA_ENABLE_DEPRECATION_WARNINGS

		for (const FRichCurveEditInfoConst& TempCurve : TempCurves)
		{
	...
	ENGINE_API virtual void SetOnCurveChangedIsInteractive(bool bInteractive) override;
...
public:
	...
	bool bOnCurveUpdateInteractive = false;
#endif
};

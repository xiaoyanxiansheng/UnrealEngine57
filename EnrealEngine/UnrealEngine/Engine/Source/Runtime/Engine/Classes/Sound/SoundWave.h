...
{
	inline bool operator()(const FSoundWaveSpectralData& A, const FSoundWaveSpectralData& B) const
	{
...
public:
	...
	ENGINE_API const ESoundWaveCuePointOrigin GetCuePointOrigin() const;
	ENGINE_API void SetCuePointOrigin(const ESoundWaveCuePointOrigin InCuePointOrigin);

	/** Set gated LUFS and SamplePeakDB values */
	ENGINE_API void SetLoudnessValues(const float InLUFS, const float InSamplePeakDB);
#endif //WITH_EDITOR
	...
	/**
	* For use when creating a new soundwave with another soundwave used as a template
...
public:
	...
	*/
	...
	// Pass in a Transformations array to overwrite this soundwave with another soundwaves transformation data
	ENGINE_API void OverwriteSoundWaveDataWithTransformationData(const TArrayView<TObjectPtr<UWaveformTransformationBase>> InTransformations = {});
...
public:
	...
	/** Gated Loudness Units Full Scale of the SoundWave. Can be modified by transformations. */
	UPROPERTY(Category = Info, AssetRegistrySearchable, VisibleAnywhere)
	float LUFS;

	/** Highest Sample Peak in db of the SoundWave. Can be modified by transformations. */
	UPROPERTY(Category = Info, AssetRegistrySearchable, VisibleAnywhere)
	float SamplePeakDB;
...
public:
	...
	friend inline uint32 GetTypeHash(const FSoundWaveProxy& InProxy)
	{

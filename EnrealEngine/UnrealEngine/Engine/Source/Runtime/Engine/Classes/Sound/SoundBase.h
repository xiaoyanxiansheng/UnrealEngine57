...
/**
 * There is a editor setting to use submixes when previewing sounds. This gives the user
 * the ability to override that locally for this specific asset.
 */
UENUM(BlueprintType)
...
public:
	/** Override the Editor Preference to use submixes on preview playback */
	UPROPERTY(EditAnywhere, Transient, SkipSerialization, Category = Developer, meta = (DisplayName = "Use Submixes on preview"))
	EUseSubmixOnPreviewMode OverrideUseSubmixesOnPreview = EUseSubmixOnPreviewMode::UseEditorPreference;
};
...
public:
	...
	/** Supplement the USoundBase with a list of transient editor-only properties */
	UPROPERTY(EditAnywhere, Transient, SkipSerialization, Category = Developer, meta = (DisplayName = "Editor Data", ShowOnlyInnerProperties))
	FSoundBaseEditorData EditorData;
...
public:
	...
	*/
	...
	UE_DEPRECATED(5.7, "This is no longer used.") virtual bool EnableSubmixSendsOnPreview() const { return false; }

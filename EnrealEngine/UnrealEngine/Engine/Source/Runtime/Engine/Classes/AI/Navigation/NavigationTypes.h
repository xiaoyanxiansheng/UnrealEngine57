...
namespace FNavigationSystem
{
	...
	inline bool IsValidLocation(const FVector& TestLocation)
	{
		return TestLocation != InvalidLocation;
	}

	inline bool BoxesAreSame(const FBox& A, const FBox& B)
	{
...
struct FNavigationPortalEdge
{
	...
	inline FVector GetPoint(const int32 Index) const
	{
		check(Index >= 0 && Index < 2);
		return ((FVector*)&Left)[Index];
	}

	inline FVector::FReal GetLength() const { return FVector::Dist(Left, Right); }

	inline FVector GetMiddlePoint() const { return Left + (Right - Left) / 2; }
};
...
struct FNavAgentProperties : public FMovementProperties
{
	...
	inline bool IsValid() const { return AgentRadius >= 0.f && AgentHeight >= 0.f; }
	inline bool HasStepHeightOverride() const { return AgentStepHeight >= 0.0f; }

	ENGINE_API bool IsNavDataMatching(const FNavAgentProperties& Other) const;

	inline bool IsEquivalent(const FNavAgentProperties& Other, float Precision = 5.f) const
	{
...
namespace NavMeshMemory
{
	...
	public:
		...
		public:
			...
			inline void ResizeAllocation(int32 PreviousNumElements, int32 NumElements, int32 NumBytesPerElement, uint32 AlignmentOfElement)
			{
...
struct FNavHeightfieldSamples
{
	...
	inline bool IsEmpty() const { return Heights.IsEmpty() && Holes.IsEmpty();  }
};

...
namespace FNavigationSystem
{
	...
	inline TNavSys* GetCurrent(UWorld* World)
	{
		return World ? Cast<TNavSys>(World->GetNavigationSystem()) : (TNavSys*)nullptr;
	}

	template<typename TNavSys>
	inline const TNavSys* GetCurrent(const UWorld* World)
	{
...
public:
	...
	*/
	 ...
	 */
	virtual bool ShouldCreateNavigationSystemInstance(const UWorld* World) const PURE_VIRTUAL(UNavigationSystemBase::ShouldCreateNavigationSystemInstance, return true;);

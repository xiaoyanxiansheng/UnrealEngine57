...
public:
	...
	ENGINE_API virtual FReply OnDoubleClick(UBlueprint* InBlueprint);
...
class UEdGraphSchema : public UObject
{
	...
	*/
	...
	/**
	 * Gets the draw color of a pin based on the UEdGraphPin.
	 * Will return GetPinTypeColor by default
	 *
	 * @param	InPin The pin to convert into a color.
	 *
	 * @return	The color representing the passed in pin
	 */
	virtual FLinearColor GetPinColor(const UEdGraphPin* InPin) const { return InPin ? GetPinTypeColor(InPin->PinType) : FLinearColor::Black; }
	...
	ENGINE_API virtual FReply BeginGraphDragAction(TSharedPtr<FEdGraphSchemaAction> InAction) const;

	/*
	* Begins a drag and drop action to drag a graph action into another graph
	*/
	ENGINE_API virtual FReply BeginGraphDragAction(TSharedPtr<FEdGraphSchemaAction> InAction, const FPointerEvent& MouseEvent) const;
	 ...
	 */
	ENGINE_API virtual FReply TrySetGraphCategory(const UEdGraph* InGraph, const FText& InCategory) const;

...
inline FArchive& operator<<(FArchive& Ar, FSimpleMemberReference& Data)
{
...
public:
	...
	inline bool IsContainer() const { return (ContainerType != EPinContainerType::None); }
	inline bool IsArray() const { return (ContainerType == EPinContainerType::Array); }
	inline bool IsSet() const { return (ContainerType == EPinContainerType::Set); }
	inline bool IsMap() const { return (ContainerType == EPinContainerType::Map); }
...
#if WITH_EDITORONLY_DATA
struct FSnappedEdGraphPin
{
	UEdGraphPin* ParentPin = nullptr;
	UEdGraphPin* ChildPin = nullptr;

	friend auto operator<=>(const FSnappedEdGraphPin&, const FSnappedEdGraphPin&) = default;
};
#endif
...
private:
	/** True if we have a snapped child pin in our LinkedTo list, corresponding pin will have bHasSnappedParent set */
	uint8 bHasSnappedChild:1;
	
	/** True if we are snapped within a parent pin in our LinkedTo list, corresponding pin will have bHasSnappedChild set */
	uint8 bHasSnappedParent:1;
public:
	...
	/** The deprecated message of the pin. Used when the pin is deprecated. */
...
#if WITH_EDITORONLY_DATA
	ENGINE_API void SetSnappedChild(UEdGraphPin* SnappedChild);
	ENGINE_API void ClearSnappedChild();
	ENGINE_API bool IsSnapped() const;
	ENGINE_API bool IsSnappedParentOf(const UEdGraphNode* Node) const;
	ENGINE_API FSnappedEdGraphPin GetSnapDescription() const;
	ENGINE_API UEdGraphNode* GetSnappedChildNode() const;
	ENGINE_API UEdGraphPin* GetSnappedChildPin() const;
	ENGINE_API UEdGraphPin* GetSnappedParentPin() const;
#endif
...
private:
	...
	template<class T>
	static void TransferPersistentDataFromOldPin(UEdGraphPin& DestPin, T& SourcePin, const ETransferPersistentDataMode TransferMode);
...
public:
	...
	*/
	...
	inline class UEdGraphNode* GetOwningNode() const { check(OwningNode); return OwningNode; }
	inline class UEdGraphNode* GetOwningNodeUnchecked() const { return OwningNode; }
	...
	inline bool WasTrashed() const { return bWasTrashed; }
...
#if WITH_EDITORONLY_DATA
	static FString bHasSnappedChildName;
	static FString bHasSnappedParentName;
#endif
};

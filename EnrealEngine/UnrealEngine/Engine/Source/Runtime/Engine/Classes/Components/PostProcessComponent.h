...
class UPostProcessComponent : public USceneComponent, public IInterface_PostProcessVolume
{
	...
	/** Automatically generated GUID for this post process volume, used for deterministic sorting of equal priority volumes. */
	UPROPERTY(TextExportTransient, NonPIEDuplicateTransient, NonTransactional)
	FGuid VolumeGuid;
	...
	virtual FPostProcessVolumeProperties GetProperties() const override
	{
		...
		Ret.Size = Ret.bIsUnbound ? DBL_MAX : Cast<UShapeComponent>(GetAttachParent())->CalcLocalBounds().GetBox().GetVolume();

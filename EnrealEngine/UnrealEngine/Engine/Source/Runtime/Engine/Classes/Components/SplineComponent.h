...
struct FSplineCurves
{
	...
	static FSplineCurves FromSplineInterface(const FSpline& InProxy)
	{
		...
		NewCurve.Position = InProxy.GetSplinePointsPosition();
		NewCurve.Rotation = InProxy.GetSplinePointsRotation();
		NewCurve.Scale = InProxy.GetSplinePointsScale();
...
public:
	...
	/** Return the name of the authoritative spline property. */
	ENGINE_API static FName GetSplinePropertyName();

	/** Return the set of names of the backing spline properties. */
	ENGINE_API static TSet<FName> GetSplinePropertyNames();
...
private:
	...
	/** Material used for rendering spline segments. */
	UPROPERTY(Transient)
	TSoftObjectPtr<UMaterialInterface> LineMaterial = nullptr;

	/** Material used for rendering spline points. */
	UPROPERTY(Transient)
...
public:
	...
	UPROPERTY(EditAnywhere, Replicated, BlueprintReadWrite, Category = Spline, meta=(Spatial))
	...
	ENGINE_API FSpline GetSpline() const;
	ENGINE_API void SetSpline(const FSpline& InSpline);
	ENGINE_API void SetSpline(const FSplineCurves& InSplineCurves);

	UE_EXPERIMENTAL(5.7, "Spline change notifications are experimental while the API is under active development.")
	FSimpleMulticastDelegate& GetOnSplineChanged() { return OnSplineChanged; }

	UE_EXPERIMENTAL(5.7, "Spline update notifications are experimental while the API is under active development.")
	FSimpleMulticastDelegate& GetOnSplineUpdated() { return OnSplineUpdated; }

	UE_EXPERIMENTAL(5.7, "Spline display setting change notifications are experimental while the API is under active development.")
	FSimpleMulticastDelegate& GetOnSplineDisplayChanged() { return OnSplineDisplayChanged; }
	
	/** Reset all spline data to match the current state of SplineCurves */
	void SynchronizeSplines() { SetSpline(WarninglessSplineCurves()); }
	...
	ENGINE_API void ApplyComponentInstanceData(struct FSplineComponentInstanceData* ComponentInstanceData, const bool bPostUCS);
	...
	ENGINE_API static FSpline GetDefaultSpline();
	...
	UFUNCTION(BlueprintCallable, Category = Spline)
	ENGINE_API float GetInputKeyValueAtTime(float Time) const;
	...
	ENGINE_API bool SupportsAttributes() const;
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Spline|Metadata")
	ENGINE_API bool RemovePropertyChannel(FName PropertyName);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Spline|Metadata")
	ENGINE_API int32 GetNumberOfPropertyValues(FName PropertyName) const;
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Spline|Metadata")
	ENGINE_API void CreateFloatPropertyChannel(FName PropertyName);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Spline|Metadata")
	ENGINE_API int32 SetFloatPropertyAtSplineInputKey(float InKey, float Value, FName PropertyName) const;
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Spline|Metadata")
	ENGINE_API float GetFloatPropertyInputKeyAtIndex(int32 Index, FName PropertyName) const;
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Spline|Metadata")
	ENGINE_API int32 SetFloatPropertyInputKeyAtIndex(int32 Index, float InKey, FName PropertyName);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Spline|Metadata")
	ENGINE_API float GetFloatPropertyAtIndex(int32 Index, FName PropertyName) const;
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Spline|Metadata")
	ENGINE_API void SetFloatPropertyAtIndex(int32 Index, float Value, FName PropertyName);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Spline|Metadata")
	ENGINE_API void RemovePropertyAtIndex(int32 Index, FName PropertyName);
...
protected:
	...
	/** Delegate broadcast any time the spline has potentially been mutated. */
	FSimpleMulticastDelegate OnSplineChanged;

	/** Delegate broadcast any time UpdateSpline is called (regardless of whether or not the spline has changed). */
	FSimpleMulticastDelegate OnSplineUpdated;

	/** Delegate broadcast any time the spline's visualization properties have changed. */
	FSimpleMulticastDelegate OnSplineDisplayChanged;
	
private:
	
	/** Returns an interval representing the full range of valid input keys, or an empty interval if the spline is empty. */
	UE::Geometry::FInterval1f GetInputKeyRange() const;
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	UPROPERTY()
	ELastAuthority LastAuthority = ELastAuthority::Unset;
	
	/** Checks for consistency between Spline and SplineCurves. */
	bool Validate() const;
...
/** Used to store spline data during RerunConstructionScripts */
USTRUCT()
struct FSplineComponentInstanceData : public FSceneComponentInstanceData
{
	GENERATED_BODY()
public:
	FSplineComponentInstanceData()
		: bSplineHasBeenEdited(false)
	{}
	explicit FSplineComponentInstanceData(const USplineComponent* SourceComponent)
		: FSceneComponentInstanceData(SourceComponent)
		, bSplineHasBeenEdited(false)
	{}
	explicit FSplineComponentInstanceData(const USplineComponent* SourceComponent, const FSpline& SourceSpline)
		: FSceneComponentInstanceData(SourceComponent)
		, bSplineHasBeenEdited(false)
		, Spline(SourceSpline)
	{}

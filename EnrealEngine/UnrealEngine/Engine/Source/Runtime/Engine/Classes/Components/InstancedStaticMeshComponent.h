...
public:
	...
	inline int32 GetRenderIndex(int32 InInstanceIndex) const { return InstanceReorderTable.IsValidIndex(InInstanceIndex) ? InstanceReorderTable[InInstanceIndex] : InInstanceIndex; }
	...
	inline int32 GetInstanceIndexForId(FPrimitiveInstanceId InstanceId) const { return PrimitiveInstanceDataManager.IdToIndex(InstanceId); }
...
public:
	...
	// Deprecated in 5.7
	inline FPrimitiveMaterialPropertyDescriptor GetUsedMaterialPropertyDesc(EShaderPlatform ShaderPlatform) const
	{
		return UPrimitiveComponent::GetUsedMaterialPropertyDesc(ShaderPlatform);
	}
	...
	{
		return GetUsedMaterialPropertyDesc(GetFeatureLevelShaderPlatform(FeatureLevel));
	}
...
protected:
	...
	ENGINE_API virtual void BuildComponentInstanceData(EShaderPlatform InShaderPlatform, FInstanceUpdateComponentDesc& OutData);
...
private:
	...
	/** Temporary flag used to skip Navigation CachedBounds invalidation.
	 * This member was added to avoid deprecating the virtual PartialNavigationUpdates() (which can't be deprecated nicely). */ 
	bool bNavigationCachedBoundsUpdated = false;
};

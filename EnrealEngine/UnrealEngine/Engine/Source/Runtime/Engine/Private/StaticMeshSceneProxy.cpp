...
static TAutoConsoleVariable<int32> CVarRayTracingStaticMeshes(
	...
	1,
	TEXT("Defines if static meshes are added to the ray tracing scene.\n")
	TEXT(" 0: raytracing cached MDCs for static mesh are not created.\n")
	TEXT(" 1: raytracing cached MDCs for static mesh are created and instances are added to the RayTracing scene (default).\n")
	TEXT(" 2: static meshes are runtime culled from the RayTracing scene, but keeps the cached state for fast toggling."));
...
#endif
{
	...
	for (; FirstAvailableLOD < RenderData->LODResources.Num()-1; FirstAvailableLOD++)
	{
	...
	bDynamicRayTracingInstances = false;
	bDynamicRayTracingGeometry = false;
	bCreateDynamicRayTracingGeometries = true;

	if (bSupportRayTracing && !ensureMsgf(RenderData->RayTracingProxy, TEXT("StaticMesh [%s] missing RayTracingProxy."), StaticMesh ? *StaticMesh->GetName() : TEXT("None")))
	{
	...
	if (InProxyDesc.ShouldCreateMaterialCacheProxy())
	{
		for (UMaterialCacheVirtualTexture* MaterialCacheTexture : InProxyDesc.MaterialCacheTextures)
		{
			MaterialCacheRenderProxies.Emplace(MaterialCacheTexture->CreateRenderProxy(InProxyDesc.MaterialCacheTextureCoordinateIndex));
		}
	}
}
...
void FStaticMeshSceneProxy::SetEvaluateWorldPositionOffsetInRayTracing(FRHICommandListBase& RHICmdList, bool bEvaluate)
{
	...
	const bool bWantsRayTracingWPO = bEvaluateWorldPositionOffset && MaterialRelevance.bUsesWorldPositionOffset && bEvaluate;
...
void FStaticMeshSceneProxy::GetRayTracingWPOConfig(bool& bOutHasRayTracingRepresentation, bool& bOutDynamicRayTracingGeometry) const
{
	GetStaticMeshRayTracingWPOConfig(bOutHasRayTracingRepresentation, bOutDynamicRayTracingGeometry);
}

bool FStaticMeshSceneProxy::ShouldEvaluateWPOInRayTracing(FVector ViewCenter) const
{
	return ShouldStaticMeshEvaluateWPOInRayTracing(ViewCenter, GetBounds());
}

void FStaticMeshSceneProxy::CreateDynamicRayTracingGeometries(FRHICommandListBase& RHICmdList)
{
	if (!bCreateDynamicRayTracingGeometries)
	{
		return;
	}
							...
							for (int32 SectionIndex = 0; SectionIndex < LODModel.Sections.Num(); SectionIndex++)
							{
								...
								for (int32 BatchIndex = 0; BatchIndex < NumBatches; BatchIndex++)
								{
									...
									if (GetMeshElement(LODIndex, BatchIndex, SectionIndex, SDPG_World, bSectionIsSelected, true, MeshElement))
									{
										...
										// Only draw the selection outline for selected items which only write to runtime virtual texture.
										if (bProxyIsSelected && DrawInRuntimeVirtualTextureOnly(ViewFamily.EngineShowFlags.Editor))
										{
											if (FColoredMaterialRenderProxy* DebugMaterialProxy = new FColoredMaterialRenderProxy(GEngine->EmissiveMeshMaterial->GetRenderProxy(), FColor::Transparent))
											{
												Collector.RegisterOneFrameMaterialProxy(DebugMaterialProxy);
												MeshElement.MaterialRenderProxy = DebugMaterialProxy;
												bDebugMaterialRenderProxySet = true;
											}
										}
	#endif // WITH_EDITOR
...
ERayTracingPrimitiveFlags FStaticMeshSceneProxy::GetCachedRayTracingInstance(FRayTracingInstance& OutRayTracingInstance)
{
	...
	if (!EnumHasAnyFlags(Flags, DisableCacheMeshCommandsFlags))
	{
		...
		const FStaticMeshRayTracingProxyLODArray& RayTracingLODs = RenderData->RayTracingProxy->LODs;
			...
			for (int32 BatchIndex = 0; BatchIndex < NumBatches; BatchIndex++)
			{
				for (int32 SectionIndex = 0; SectionIndex < RayTracingLOD.Sections->Num(); SectionIndex++)
				{
					...
					if (bResult)
					{
						...
						OutRayTracingInstance.Materials.Add(MeshBatch);
					}
				}
			}
		}
...
void FStaticMeshSceneProxy::GetDynamicRayTracingInstances(FRayTracingInstanceCollector& Collector)
{
	...
	int32 RayTracingMinLOD = RenderData->RayTracingProxy->bUsingRenderingLODs ? NumLODs - 1 : 0;
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		...
		bEvaluateWPO |= ShouldEvaluateWPOInRayTracing(ViewCenter);

		if (RenderData->RayTracingProxy->bUsingRenderingLODs)
		{
			// we use same LOD for all views
			RayTracingMinLOD = FMath::Min(RayTracingMinLOD, GetLOD(View));
		}
	}

	if (RenderData->RayTracingProxy->bUsingRenderingLODs)
	{
		// when using rendering LODs also need to limit based on currently loaded LOD
		RayTracingMinLOD = FMath::Max(RayTracingMinLOD, (int32)GetCurrentFirstLODIdx_RenderThread());
	}
	...
	if (bEvaluateWPO && !RenderData->RayTracingProxy->bUsingRenderingLODs)
	{
		...
		{
			const FStaticMeshRayTracingProxyLOD& CurrentRayTracingLOD = RayTracingLODs[LODIndex];

			if (CurrentRayTracingLOD.bBuffersInlined || CurrentRayTracingLOD.AreBuffersStreamedIn())
			{
				break;
			}
		}
...
{   
	...
	if(
		...
		(IsSelected() && DrawInRuntimeVirtualTextureOnly(View->Family->EngineShowFlags.Editor)) ||
#endif
#if STATICMESH_ENABLE_DEBUG_RENDERING
		bDrawMeshCollisionIfComplex ||
		bDrawMeshCollisionIfSimple ||
	...
	, bUsesMeshModifyingMaterials(false)
{
	...
	const EShaderPlatform ShaderPlatform = InProxyDesc.Scene->GetShaderPlatform();
	...
	for(int32 SectionIndex = 0;SectionIndex < LODModel.Sections.Num();SectionIndex++)
	{
		...
		FMaterialResource const* MaterialResource = const_cast<UMaterialInterface const*>(SectionInfo.Material)->GetMaterial_Concurrent()->GetMaterialResource(ShaderPlatform);
		if(MaterialResource)
		{

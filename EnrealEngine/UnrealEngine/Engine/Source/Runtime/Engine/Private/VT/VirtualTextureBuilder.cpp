...
void UVirtualTextureBuilder::Serialize(FArchive& Ar)
{
	...
	if (Ar.IsCooking() && Ar.IsSaving())
	{
		...
		int32 OverrideSVTTexture = Ar.CookingTarget()->GetConfigSystem()->GetIntOrDefault(TEXT("/Script/Engine.RendererSettings"), TEXT("OverrideSVTTexture"), -1, GEngineIni);

		if (OverrideSVTTexture >= 0)
		{
			if (OverrideSVTTexture == 0)
			{
				TextureMobile = Texture;
			}
			else
			{
				Texture = TextureMobile;
			}
		}

...
FVTRequestPageResult FVirtualTextureChunkStreamingManager::RequestTile(FRHICommandListBase& RHICmdList, FUploadingVirtualTexture* VTexture, const FVirtualTextureProducerHandle& ProducerHandle, uint8 LayerMask, uint8 vLevel, uint32 vAddress, EVTRequestPagePriority Priority)
{
	...
	if (TileOffset == ~0u)
	{
		UE_LOG(LogConsoleResponse, Verbose, TEXT("vAddr %i@%i has an invalid tile (-1)."), vAddress, vLevel);
...
IVirtualTextureFinalizer* FVirtualTextureChunkStreamingManager::ProduceTile(FRHICommandListBase& RHICmdList, uint32 SkipBorderSize, uint8 NumLayers, uint8 LayerMask, uint64 RequestHandle, const FVTProduceTargetLayer* TargetLayers)
{

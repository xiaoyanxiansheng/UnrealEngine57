...
void URuntimeVirtualTexture::GetProducerDescription(FVTProducerDescription& OutDesc, FInitSettings const& InitSettings, FTransform const& VolumeToWorld) const
{
	...
	OutDesc.FullNameHash = GetTypeHash(GetName().ToLower());
	OutDesc.Dimensions = 2;
	OutDesc.DepthInTiles = 1;
	OutDesc.WidthInBlocks = 1;
	OutDesc.HeightInBlocks = 1;
	...
	// Disable splitting physical pools to give 16bit page tables when r.VT.PageTableMode=2.
	// This is only useful for virtual texture spaces with many small allocated page tables, and that isn't typically the case for RVT.
	OutDesc.bRequiresSinglePhysicalPool = true;
...
void URuntimeVirtualTexture::Initialize(IVirtualTexture* InProducer, FVTProducerDescription const& InProducerDesc, FTransform const& InVolumeToWorld, FBox const& InWorldBounds, FOnReInitDelegate InDelegate)
{
	OnReInitDelegate.ExecuteIfBound();
	OnReInitDelegate = InDelegate;
...
{
	Initialize(InProducer, InProducerDesc, InVolumeToWorld, InWorldBounds, {});
}

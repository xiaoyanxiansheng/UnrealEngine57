...
/** Fill virtual texture borders (or full tiles) with mip colors. */
static TAutoConsoleVariable<int32> CVarVirtualTextureBorders(
	TEXT("r.VT.Borders"),
	0,
	TEXT("Sets the virtual texture tile border size, in texels, that is filled with debug mip colors.\n")
	TEXT("Size may be rounded up for block compressed textures."),
	FConsoleVariableDelegate::CreateLambda([](IConsoleVariable* InVariable) { GetRendererModule().FlushVirtualTextureCache(); }),
	ECVF_RenderThreadSafe
);

/** Select a single mip level to apply mip colors to. This can be useful for highlighting mip 0 sampling issues. */
static TAutoConsoleVariable<int32> CVarVirtualTextureBordersMip(
	TEXT("r.VT.Borders.Mip"),
	-1,
	TEXT("Set a single mip level that r.VT.Borders will apply to.\n")
	TEXT("-1 (default) applies to all mip levels."),
	FConsoleVariableDelegate::CreateLambda([](IConsoleVariable* InVariable) { GetRendererModule().FlushVirtualTextureCache(); }),
	ECVF_RenderThreadSafe
);
...
struct FTranscodeTask
{
	...
	void DoTask(bool bAsync)
	{
		...
		for (uint32 LayerIndex = 0u; LayerIndex < NumLayers; ++LayerIndex)
		{
			...
			const int32 DebugBorderSize = CVarVirtualTextureBorders.GetValueOnAnyThread();
			if (DebugBorderSize > 0)
			{
				const int32 DebugBorderMip = CVarVirtualTextureBordersMip.GetValueOnAnyThread();
				if (DebugBorderMip < 0 || DebugBorderMip == vLevel)
				{
					BakeDebugInfo(StagingBufferForLayer, TilePixelSize, TilePixelSize, TileBorderPixelSize + DebugBorderSize, LayerFormat, vLevel);
				}
			}
		}
...
void FVirtualTextureTranscodeCache::RetireOldTasks(FRHICommandListBase& RHICmdList, FVirtualTextureUploadCache& InUploadCache)
{

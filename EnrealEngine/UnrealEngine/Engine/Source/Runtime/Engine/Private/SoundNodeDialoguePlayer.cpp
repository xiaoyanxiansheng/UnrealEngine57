...
const UDialogueSoundWaveProxy* USoundNodeDialoguePlayer::GetContextualDialogueSoundWaveProxy() const
{
	const UDialogueWave* DialogueWave = GetDialogueWave();

	if (IsValid(DialogueWave))
	{
		const USoundBase* SoundBase = DialogueWave ? DialogueWave->GetWaveFromContext(DialogueWaveParameter.Context) : NULL;
		if (IsValid(SoundBase))
		{
			const UDialogueSoundWaveProxy* Proxy = Cast<UDialogueSoundWaveProxy>(SoundBase);
			check(IsValid(Proxy));	// GetWaveFromContext() specifically uses DialogueSoundWaveProxy, but casts to SoundBase before returning.
			return Proxy;
		}
	}
	return nullptr;

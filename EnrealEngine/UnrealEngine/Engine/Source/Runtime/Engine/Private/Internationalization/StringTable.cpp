...
namespace UE::StringTable
{
	...
	namespace Private
	{
		bool ShouldAutoRegister(const FName InTableId)
		{
			return !FPackageName::IsTempPackage(FNameBuilder(InTableId).ToView());
		}
	}
}
...
public:
	...
	{
		KeepAliveStringTables.RemoveAllSwap(
			[&PluginPackageRoot](const TObjectPtr<UStringTable>& InStringTableAsset)
			{
				if (!InStringTableAsset || InStringTableAsset->HasAnyFlags(RF_MirroredGarbage | RF_NewerVersionExists))
				{
					return true;
				}

				const FNameBuilder PackageName(InStringTableAsset->GetPackage()->GetFName());
				if (PackageName.ToView().StartsWith(PluginPackageRoot))
				{
					UE_LOG(LogStringTable, Log, TEXT("Removing string table '%s' from keep-alive list"), *InStringTableAsset->GetName());
					return true;
				}
...
private:
	...
	virtual void CollectStringTableAssetSearchableNameImpl(const FName InTableId, const FTextKey InKey, const FArchive& Ar) override
	{
		if (Ar.IsSaving() && !InTableId.IsNone() && !InKey.IsEmpty())
		{
			if (UStringTable* StringTableAsset = FStringTableRegistry::Get().FindStringTableAsset(InTableId))
			{
				// Note which key we are pointing to for later searching
				FNameBuilder KeyStr;
				InKey.ToString(KeyStr);
				Ar.MarkSearchableName(StringTableAsset, FName(KeyStr));
			}
		}
	}
	...
	{
		FScopeLock KeepAliveStringTablesLock(&KeepAliveStringTablesCS);
		KeepAliveStringTables.RemoveAllSwap([](const TObjectPtr<UStringTable>& InStringTableAsset) { return !InStringTableAsset || InStringTableAsset->HasAnyFlags(RF_MirroredGarbage | RF_NewerVersionExists); });
	...
	, StringTableId(*GetPathName()) // Note: If you change this ID format, also fix HandleStringTableAssetAsyncLoadCompleted
{
	...
	if (IsAsset() && UE::StringTable::Private::ShouldAutoRegister(GetStringTableId()))
	{
...
{
	if (!HasAnyFlags(RF_ClassDefaultObject)) // Needed to avoid a crash during shutdown for programs that don't call RequestEngineExit
	{
		FStringTableEngineBridge::Get().UnregisterForGC(this);
		if (!HasAnyFlags(RF_NewerVersionExists))
		{
...
bool UStringTable::Rename(const TCHAR* NewName, UObject* NewOuter, ERenameFlags Flags)
{
	...
	if (bRenamed)
	{
		const bool bWasRegistered = FStringTableRegistry::Get().UnregisterStringTable(GetStringTableId());
		const bool bWasManuallyRegistered = bWasRegistered && !UE::StringTable::Private::ShouldAutoRegister(GetStringTableId());

		StringTableId = *GetPathName();
		
		if (bWasRegistered && (bWasManuallyRegistered || UE::StringTable::Private::ShouldAutoRegister(GetStringTableId())))
		{
			FStringTableRegistry::Get().RegisterStringTable(GetStringTableId(), StringTable.ToSharedRef());
		}
	}
...
void UStringTable::OnPluginUnloaded(FStringView PluginName)
{
	auto PackageNameBasis = WriteToString<256>(TEXT("/"), PluginName, TEXT("/"));
	FStringTableEngineBridge::Get().UnregisterForGCUnderPath(PackageNameBasis);
}

...
void AHUD::ShowDebug(FName DebugType)
{
	...
	else
	{
		...
		if (!bRemoved)
		{
			// Since we prevent removing items when bShowDebugInfo is false, add them only once.
			DebugDisplay.AddUnique(DebugType);
		}
...
void AHUD::ShowDebugInfo(float& YL, float& YPos)
{
	if (DebugCanvas != nullptr && DebugCanvas->Canvas != nullptr)
	{
		...
		// Draw Header.
		{
			...
			DisplayDebugManager.DrawString(TEXT("Press [Right Shift] or [Left Shift] to move the debug menu a column over to see more of the debug menu"));
		}

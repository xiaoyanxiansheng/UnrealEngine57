...
void FVirtualShadowMapVisualizationData::Initialize()
{
	if (!bIsInitialized)
	{
		...
		AddVisualizationMode(
			...
			LOCTEXT("ShadowCastersDesc", "Visualize objects that cast shadows, and which type of shadow map they invalidate."),
			FModeType::Standard,
			VIRTUAL_SHADOW_MAP_VISUALIZE_SHADOW_CASTERS,
			VMI_ShadowCasters);
...
)
{
	...
	Record.ViewModeIndex        = ViewModeIndex;
}
...
{
	return ViewModeIndex == VMI_VisualizeVirtualShadowMap

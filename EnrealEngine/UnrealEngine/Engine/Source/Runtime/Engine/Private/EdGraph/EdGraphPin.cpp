...
#include UE_INLINE_GENERATED_CPP_BY_NAME(EdGraphPin)
#if WITH_EDITOR
...
namespace PinHelpers
{
	...
	static void ResolveReferencesToPin(UEdGraphPin* Pin, TArray<FUnresolvedPinData>& UnresolvedPinData, bool bStrictValidation = true);
}
...
#if WITH_EDITORONLY_DATA
void UEdGraphPin::SetSnappedChild(UEdGraphPin* SnappedChild)
{
	check(SnappedChild);
	if( !ensure(SnappedChild->bHasSnappedParent == false && bHasSnappedChild == false) ||
		!ensure(LinkedTo.Contains(SnappedChild)))
	{
		return;
	}

	SnappedChild->bHasSnappedParent = true;
	bHasSnappedChild = true;
}

void UEdGraphPin::ClearSnappedChild()
{
	if(UEdGraphPin* OldSnappedChild = GetSnappedChildPin())
	{
		OldSnappedChild->bHasSnappedParent = false;
	}
	bHasSnappedChild = false;
}

bool UEdGraphPin::IsSnapped() const
{
	return bHasSnappedChild || bHasSnappedParent;
}

bool UEdGraphPin::IsSnappedParentOf(const UEdGraphNode* Node) const
{
	if(!bHasSnappedChild)
	{
		return false;
	...
	if(UEdGraphPin* SnappedChild = GetSnappedChildPin())
	{
		return SnappedChild->GetOwningNode() == Node;
	}

	return false;
}

FSnappedEdGraphPin UEdGraphPin::GetSnapDescription() const
{
	if(!IsSnapped())
	{
		return {};
	...
	// if we have a child attachment return that:
	FSnappedEdGraphPin Result;
	if(bHasSnappedChild)
	{
		if(UEdGraphPin* ChildPin = GetSnappedChildPin())
		{
			Result.ParentPin = const_cast<UEdGraphPin*>(this);
			Result.ChildPin = ChildPin;
		}
	}
	else
	{
		// terminal attachment - fall back to describing our relationship with the parent pin:
		if(UEdGraphPin* SnappedParentPin = GetSnappedParentPin())
		{
			Result.ParentPin = SnappedParentPin;
			Result.ChildPin = const_cast<UEdGraphPin*>(this);
		}
	}
	return Result;
}

UEdGraphNode* UEdGraphPin::GetSnappedChildNode() const
{
	if(UEdGraphPin* SnappedChild = GetSnappedChildPin())
	{
		return SnappedChild->GetOwningNode();
	}
	return nullptr;
}

UEdGraphPin* UEdGraphPin::GetSnappedChildPin() const
{
	if(!bHasSnappedChild)
	{
		return nullptr;
	...
	{
		if(LinkedPin->bHasSnappedParent)
		{
			return LinkedPin;
		}
	}
...
UEdGraphPin* UEdGraphPin::GetSnappedParentPin() const
{
	if(!bHasSnappedParent)
	{
		return nullptr;
	}
...
#if WITH_EDITORONLY_DATA
			// update snapping state - if any:
			if(bHasSnappedParent && ToPin->bHasSnappedChild)
			{
				ToPin->bHasSnappedChild = false;
				bHasSnappedParent = false;
			}
			else if(bHasSnappedChild && ToPin->bHasSnappedParent)
			{
				bHasSnappedChild = false;
				ToPin->bHasSnappedParent = false;
			}
...
template<class T>
void UEdGraphPin::TransferPersistentDataFromOldPin(UEdGraphPin& DestPin, T& SourcePin, const UEdGraphPin::ETransferPersistentDataMode TransferMode)
{
...
#if WITH_EDITORONLY_DATA
	DestPin.bHasSnappedChild = SourcePin.bHasSnappedChild;
	DestPin.bHasSnappedParent = SourcePin.bHasSnappedParent;
#endif//WITH_EDITORONLY_DATA
...
void UEdGraphPin::MovePersistentDataFromOldPin(UEdGraphPin& SourcePin)
{
	...
	TransferPersistentDataFromOldPin(*this, SourcePin, UEdGraphPin::ETransferPersistentDataMode::Move);
...
{
	TransferPersistentDataFromOldPin(*this, SourcePin, UEdGraphPin::ETransferPersistentDataMode::Copy);
}
...
bool UEdGraphPin::ExportTextItem(FString& ValueStr, int32 PortFlags) const
{
...
#if WITH_EDITORONLY_DATA
	{
		...
		// avoid exporting bHasSnappedChild unless it's true, as it defaults to false
		// and older code that processes T3D nodes may be confused by extra properties:
		if(bHasSnappedChild)
		{
			ValueStr += UEdGraphPin::bHasSnappedChildName + TEXT("=");
			bool LocalHasSnappedChildName = bHasSnappedChild;
			BoolPropCDO->ExportTextItem_Direct(ValueStr, &LocalHasSnappedChildName, nullptr, nullptr, PortFlags, nullptr);
			ValueStr += PinHelpers::ExportTextPropDelimiter;
		}
		
		if(bHasSnappedParent)
		{
			ValueStr += UEdGraphPin::bHasSnappedParentName + TEXT("=");
			bool LocalHasSnappedParentName = bHasSnappedParent;
			BoolPropCDO->ExportTextItem_Direct(ValueStr, &LocalHasSnappedParentName, nullptr, nullptr, PortFlags, nullptr);
			ValueStr += PinHelpers::ExportTextPropDelimiter;
		}
	}
#endif //WITH_EDITORONLY_DATA
...
bool UEdGraphPin::ImportTextItem(const TCHAR*& Buffer, int32 PortFlags, class UObject* Parent, FOutputDevice* ErrorText)
{
	...
	while (*Buffer != ')')
	{
		...
		else if (PropertyToken == UEdGraphPin::bHasSnappedChildName)
		{
			bool LocalHasSnappedChild = bHasSnappedChild;
			Buffer = BoolPropCDO->ImportText_Direct(Buffer, &LocalHasSnappedChild, Parent, PortFlags, ErrorText);
			bParseSuccess = (Buffer != nullptr);
			bHasSnappedChild = LocalHasSnappedChild;
		}
		else if (PropertyToken == UEdGraphPin::bHasSnappedParentName)
		{
			bool LocalHasSnappedParent = bHasSnappedParent;
			Buffer = BoolPropCDO->ImportText_Direct(Buffer, &LocalHasSnappedParent, Parent, PortFlags, ErrorText);
			bParseSuccess = (Buffer != nullptr);
			bHasSnappedParent = LocalHasSnappedParent;
		}
#endif
...
void UEdGraphPin::ResolveAllPinReferences()
{
	for(TMap<FPinResolveId, TArray<FUnresolvedPinData>>::TIterator Iter(PinHelpers::UnresolvedPins); Iter; ++Iter)
	{
		...
		if (TargetPin)
		{
			PinHelpers::ResolveReferencesToPin(*TargetPin, Iter->Value);
		}
	}
...
bool UEdGraphPin::Serialize(FArchive& Ar)
{
	...
	if (!Ar.IsFilterEditorOnly())
	{
		...
		BitField |= bHasSnappedChild << 6;
		BitField |= bHasSnappedParent << 7;

		const int32 PersistentBits = 8;
...
void UEdGraphPin::ResolveReferencesToPin(UEdGraphPin* Pin, bool bStrictValidation)
{
	...
	if (UnresolvedPinData)
	{
		PinHelpers::ResolveReferencesToPin(Pin, *UnresolvedPinData, bStrictValidation);
		PinHelpers::UnresolvedPins.Remove(ResolveId);
	}
}

void PinHelpers::ResolveReferencesToPin(UEdGraphPin* Pin, TArray<FUnresolvedPinData>& UnresolvedPinData, bool bStrictValidation)
{
	for (const FUnresolvedPinData& PinData : UnresolvedPinData)
	{
		UEdGraphPin* ReferencingPin = PinData.ReferencingPin;
...
#if WITH_EDITORONLY_DATA
FString UEdGraphPin::bHasSnappedChildName = GET_MEMBER_NAME_STRING_CHECKED(UEdGraphPin, bHasSnappedChild);
FString UEdGraphPin::bHasSnappedParentName = GET_MEMBER_NAME_STRING_CHECKED(UEdGraphPin, bHasSnappedParent);
#endif

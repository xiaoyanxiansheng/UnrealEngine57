...
#include UE_INLINE_GENERATED_CPP_BY_NAME(EdGraphSchema)
...
FReply UEdGraphSchema::BeginGraphDragAction(TSharedPtr<FEdGraphSchemaAction> InAction) const
{
	return FReply::Unhandled();
}

FReply UEdGraphSchema::BeginGraphDragAction(TSharedPtr<FEdGraphSchemaAction> InAction, const FPointerEvent& MouseEvent) const
{
	return FReply::Unhandled();
}
...
FReply UEdGraphSchema::TrySetGraphCategory(const UEdGraph* InGraph, const FText& InCategory) const
{
	return FReply::Unhandled();
}

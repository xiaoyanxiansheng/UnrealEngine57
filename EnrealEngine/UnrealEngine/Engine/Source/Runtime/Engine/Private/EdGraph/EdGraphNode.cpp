...
#include UE_INLINE_GENERATED_CPP_BY_NAME(EdGraphNode)
#if WITH_EDITOR
...
void UEdGraphNode::DiffProperties(UStruct* StructA, UStruct* StructB, uint8* DataA, uint8* DataB, FDiffResults& Results, FDiffSingleResult& Diff) const
{
	...
	for (TFieldIterator<FProperty> PropertyIt(StructA, EFieldIteratorFlags::IncludeSuper); PropertyIt; ++PropertyIt)
	{
		...
		auto ShouldSkipProperty = [](const FProperty& Prop)
			{
				// if the property is an inner property (e.g. for a container) then use the outer property for visibility:
				const FProperty* PropertyToTest = &Prop;
				const FProperty* Owner = Prop.GetOwnerProperty();
				if(Owner)
				{
					PropertyToTest = Owner;
				}

				return !PropertyToTest->HasAnyPropertyFlags(CPF_Edit | CPF_BlueprintVisible) ||
					PropertyToTest->HasAnyPropertyFlags(CPF_Transient) ||
					PropertyToTest->HasAnyPropertyFlags(CPF_DisableEditOnInstance) ||
					PropertyToTest->IsA(FDelegateProperty::StaticClass()) ||
					PropertyToTest->IsA(FMulticastDelegateProperty::StaticClass());
			};
...
bool UEdGraphNode::IsSnappedToAny(const TSet<TObjectPtr<UObject>>& NodeSet ) const
{
	for (const UEdGraphPin* Pin : Pins)
	{
		for (UEdGraphPin* Connection : Pin->LinkedTo)
		{
			if(Connection->IsSnappedParentOf(this))
			{
				if( NodeSet.Contains(Connection->GetOwningNode()))
				{
					return true;
				}
				else if(Connection->GetOwningNode()->IsSnappedToAny(NodeSet))
				{
					return true;
				}
			}
...
UEdGraphNode* UEdGraphNode::GetRootSnappedParent(const TSet<TObjectPtr<UObject>>& NodeSet) const
{
	for (const UEdGraphPin* Pin : Pins)
	{
		for (UEdGraphPin* Connection : Pin->LinkedTo)
		{
			if(Connection->IsSnappedParentOf(this))
			{
				UEdGraphNode* OwningNode = Connection->GetOwningNode();
				if( NodeSet.Contains(OwningNode))
				{
					return OwningNode;
				}
				OwningNode = OwningNode->GetRootSnappedParent(NodeSet);
				if(OwningNode)
				{
					return OwningNode;
				}

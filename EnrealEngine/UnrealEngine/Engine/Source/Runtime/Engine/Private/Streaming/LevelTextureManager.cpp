...
{
	return Level ? FRenderAssetStreamingManagerHelper::GetWorldTime(Level->GetWorld()) : 0.f;
}
...
void FLevelRenderAssetManager::IncrementalBuild(FDynamicRenderAssetInstanceManager& DynamicComponentManager, FStreamingTextureLevelContext& LevelContext, bool bForceCompletion, int64& NumStepsLeft)
{
	...
	case EStaticBuildStep::BuildTextureLookUpMap:
	{
		...
		LevelContext.UpdateContext(GetCurrentMaterialQualityLevelChecked(), Level, &TextureGuidToLevelIndex);
		break;
	}
	...
	bool bUseDynamicStreaming) 
{
	...
	{
		FStreamingTextureLevelContext LevelContext(GetCurrentMaterialQualityLevelChecked(), Level, &TextureGuidToLevelIndex);
		do
		{

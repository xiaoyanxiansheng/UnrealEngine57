	...
	, CurrentThread(InCurrentThread)
{
	if (InTexture)
	{
		...
		Resource = Texture->GetResource() ? const_cast<UTexture2D*>(Texture)->GetResource()->GetTexture2DResource() : nullptr;
		if (Resource)
		{
...
void FTexture2DUpdate::DoConvertToVirtualWithNewMips(const FContext& Context)
{
	...
	if (!IsCancelled() && Context.Resource)
	{
		...
		if (!Context.Resource->IsTextureRHIPartiallyResident())
		{
			...
			if (Context.Resource->ResourceMem)
			{
				Desc.SetInitActionBulkData(Context.Resource->ResourceMem);
			}

			Desc.DetermineInititialState();

			IntermediateTextureRHI = RHICmdList.CreateTexture(Desc);
		...
		RHICmdList.VirtualTextureSetFirstMipInMemory(IntermediateTextureRHI, PendingFirstLODIdx);
	}
}

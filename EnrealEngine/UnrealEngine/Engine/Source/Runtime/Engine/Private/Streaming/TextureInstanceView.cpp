...
void FBounds4::Set(int32 Index, const FBoxSphereBounds& InBounds, uint32 InPackedRelativeBox, float InLastRenderTime, const FVector& InRangeOrigin, float InMinDistanceSq, float InMinRangeSq, float InMaxRangeSq)
{
	Set(Index, InBounds, InPackedRelativeBox, InBounds.SphereRadius, InLastRenderTime, InRangeOrigin, InMinDistanceSq, InMinRangeSq, InMaxRangeSq);
}
void FBounds4::Set(int32 Index, const FBoxSphereBounds& InBounds, uint32 InPackedRelativeBox, float InRadiusOrComponentScale, float InLastRenderTime, const FVector& InRangeOrigin, float InMinDistanceSq, float InMinRangeSq, float InMaxRangeSq)
{
	check(Index >= 0 && Index < 4);

	OriginX.Component(Index) = InBounds.Origin.X;
	OriginY.Component(Index) = InBounds.Origin.Y;
	OriginZ.Component(Index) = InBounds.Origin.Z;
	RangeOriginX.Component(Index) = InRangeOrigin.X;
	RangeOriginY.Component(Index) = InRangeOrigin.Y;
	RangeOriginZ.Component(Index) = InRangeOrigin.Z;
	ExtentX.Component(Index) = InBounds.BoxExtent.X;
	ExtentY.Component(Index) = InBounds.BoxExtent.Y;
	ExtentZ.Component(Index) = InBounds.BoxExtent.Z;
	RadiusOrComponentScale.Component(Index) = InRadiusOrComponentScale;
...
void FRenderAssetInstanceView::FRenderAssetLinkConstIterator::OutputToLog(float MaxNormalizedSize, float MaxNormalizedSize_VisibleOnly, const TCHAR* Prefix) const
{
	...
	FRenderAssetInstanceView::OutputToLog(Bounds, GetBoundsIndex(), State.Bounds4, GetTexelFactor(), GetForceLoad(), MaxNormalizedSize, MaxNormalizedSize_VisibleOnly, Prefix, GetComponent());
#endif
}

void FRenderAssetInstanceView::OutputToLog(const FBoxSphereBounds& Bounds, int32 BoundsIndex, const TArray<FBounds4>& Bounds4, float TexelFactor, bool bForceLoad, float MaxNormalizedSize, float MaxNormalizedSize_VisibleOnly, const TCHAR* Prefix, const UPrimitiveComponent* Component)
{
#if !UE_BUILD_SHIPPING
	...
	else if (TexelFactor >= 0)
	{
		...
		{
			UE_LOG(LogContentStreaming, Log, TEXT("    Size=%f, BoundIndex=%d"), TexelFactor * FMath::Max(MaxNormalizedSize, MaxNormalizedSize_VisibleOnly), BoundsIndex);
		}
		else if (MaxNormalizedSize_VisibleOnly > 0)
		{
			UE_LOG(LogContentStreaming, Log, TEXT("    OnScreenSize=%f, BoundIndex=%d"), TexelFactor * MaxNormalizedSize_VisibleOnly, BoundsIndex);
		}
		else
		{
			if (Bounds4.IsValidIndex(BoundsIndex / 4))
			{
				float LastRenderTime = Bounds4[BoundsIndex / 4].LastRenderTime[BoundsIndex % 4];
				UE_LOG(LogContentStreaming, Log, TEXT("    OffScreenSize=%f, LastRenderTime= %.3f, BoundIndex=%d"), TexelFactor * MaxNormalizedSize, LastRenderTime, BoundsIndex);
			}
			else
			{
	...
	float& OutMaxLevelRenderAssetScreenSize)
{
	...
	const bool bConsiderVisibility = !FSimpleStreamableAssetManager::IsEnabled() || FSimpleStreamableAssetManager::ShouldConsiderVisibility();
	...
	for (int32 Bounds4Index = 0; Bounds4Index < NumBounds4; ++Bounds4Index)
	{
		...
		for (int32 ViewIndex = 0; ViewIndex < NumViews; ++ViewIndex)
		{
			...
			// When using SimpleStreamableAssetManager, LastRenderTime is only valid when FSimpleStreamableAssetManager::ShouldConsiderVisibility() == true

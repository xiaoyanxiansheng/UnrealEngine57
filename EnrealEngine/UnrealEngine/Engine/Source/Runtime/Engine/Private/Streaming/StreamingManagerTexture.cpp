...
void FRenderAssetStreamingManager::ProcessAddedRenderAssets()
{
	...
	for (int32 Idx = 0; Idx < PendingStreamingRenderAssets.Num(); ++Idx)
	{
		...
		if (Asset)
		{
			...
			new (StreamingRenderAssets) FStreamingRenderAsset(Asset, Asset->SimpleStreamableAssetManagerIndex, NumStreamedMips, NumLODGroups, Settings);
		}
	}
...
void FRenderAssetStreamingManager::NotifyPrimitiveUpdated( const UPrimitiveComponent* Primitive )
{
	...
	if (IsInGameThread() && bUseDynamicStreaming && Primitive && !Primitive->bIgnoreStreamingManagerUpdate)
	{
		...
		if (bUpdatePrimitive)
		{
			FStreamingTextureLevelContext LevelContext(GetCurrentMaterialQualityLevelChecked(), Primitive);
...
void FRenderAssetStreamingManager::SetLastUpdateTime()
{
	...
	if (FSimpleStreamableAssetManager::IsEnabled())
	{
		if (FSimpleStreamableAssetManager::ShouldConsiderVisibility())
		{
			for (const FWorldContext& Context : GEngine->GetWorldContexts())
			{
				WorldTime = FRenderAssetStreamingManagerHelper::GetWorldTime(Context.World());
				if (WorldTime > 0)
				{
					break;
				}
	...
	{
		for (FLevelRenderAssetManager* LevelRenderAssetManager : LevelRenderAssetManagers)
		{
			// Update last update time only if there is a reasonable threshold to define visibility.
			WorldTime = LevelRenderAssetManager ? LevelRenderAssetManager->GetWorldTime() : 0.f;
			if (WorldTime > 0)
			{
				break;
			}
...
void FRenderAssetStreamingManager::UpdateResourceStreaming( float DeltaTime, bool bProcessEverything/*=false*/ )
{
...
#if STATS
		UpdateStats();
#elif UE_WITH_RENDER_ASSET_STREAMING_CSV_STATS
		UpdateCSVOnlyStats();
#endif // STATS
	}
	...
	if (GEnableAssetUpdateAbandons && GTickAbandonedRenderAssetUpdatesOnStreamingUpdate)
	{
		AbandonedRenderAssetUpdateManager.TickAbandoned();
	}
...
void FRenderAssetStreamingManager::OnRenderAssetUpdateAbandoned(UStreamableRenderAsset* OwningRenderAsset, TRefCountPtr<class FRenderAssetUpdate>& RenderAssetUpdate)
{
	check(GEnableAssetUpdateAbandons);
	AbandonedRenderAssetUpdateManager.OnAbandoned(OwningRenderAsset, RenderAssetUpdate);
}
...
bool FRenderAssetStreamingManager::HandleListStreamingRenderAssetsCommand( const TCHAR* Cmd, FOutputDevice& Ar )
{
	...
	for (TMap<FString, int32>::TConstIterator It(SortedRenderAssets); It; ++It)
	{
		...
		else
		{
			...
			if (StaticMesh)
			{
				LODGroupName = StaticMesh->GetLODGroup().ToString();
			}
#endif
...
bool FRenderAssetStreamingManager::HandleInvestigateRenderAssetCommand(const TCHAR* Cmd, FOutputDevice& Ar, UWorld* InWorld)
{
	...
	if (InvestigateAssetName.Len())
	{
		...
		const bool bInvestigateAll = InvestigateAssetName == TEXT("*");
		TArray<int32> AssetIndices;
		AssetIndices.Reserve(StreamingRenderAssets.Num());
		for (int32 AssetIndex = 0; AssetIndex < StreamingRenderAssets.Num(); ++AssetIndex)
		{
			AssetIndices.Add(AssetIndex);
		}

		// When investigating all assets, sort render asset based on their full name to facilitate diffing
		if (bInvestigateAll)
		{
			AssetIndices.Sort([&StreamingRenderAssets](const int32& A, const int32& B)
			{
				return StreamingRenderAssets[A].RenderAsset->GetFullName() < StreamingRenderAssets[B].RenderAsset->GetFullName();
			});
		}

		for (int32 i = 0; i < AssetIndices.Num(); ++i)
		{
			int32 AssetIndex = AssetIndices[i];
			FStreamingRenderAsset& StreamingRenderAsset = StreamingRenderAssets[AssetIndex];
			FString AssetName = StreamingRenderAsset.RenderAsset->GetFullName();
			if (bInvestigateAll || AssetName.Contains(InvestigateAssetName))
			{
...
float FRenderAssetStreamingManagerHelper::GetWorldTime(UWorld* InWorld)
{
	// When paused, updating the world time sometimes break visibility logic.
	if (InWorld && !InWorld->IsPaused())
	{
		// In the editor, we only return a time for the PIE world.
		if (!GIsEditor || (InWorld->IsPlayInEditor() && InWorld->Scene && InWorld->Scene->GetFrameNumber()))
		{

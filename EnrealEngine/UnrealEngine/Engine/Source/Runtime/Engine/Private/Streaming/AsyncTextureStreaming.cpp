...
void FAsyncRenderAssetStreamingData::UpdatePerfectWantedMips_Async(FStreamingRenderAsset& StreamingRenderAsset, const FRenderAssetStreamingSettings& Settings, bool bOutputToLog) const
{
	...
	else
	{
		...
		if (FSimpleStreamableAssetManager::IsEnabled() && *StreamingRenderAsset.SimpleStreamableAssetManagerIndex != INDEX_NONE)
		{
			...
			if (!bAlreadyAtMaximumResolution)
			{
				...
				FSimpleStreamableAssetManager::GetRenderAssetScreenSize(AssetType, *StreamingRenderAsset.SimpleStreamableAssetManagerIndex, TmpMaxSize, TmpMaxVisibleOnly, TmpMaxNumForcedLODs, MaxAssetSize, StreamingRenderAsset.MaxAllowedMips, bOutputToLog ? TEXT("Static") : nullptr);
		...
		if (StreamingRenderAsset.bUseUnkownRefHeuristic)
		{
			...
			if (StreamingRenderAsset.bUseUnkownRefHeuristic && StreamingRenderAsset.LastRenderTime < 90.0f && ((Settings.DropMips != 3 && Settings.DropMips != 4) || AssetType == EStreamableRenderAssetType::Texture))
			{
...
void FRenderAssetStreamingMipCalcTask::UpdateBudgetedMips_Async()
{
	...
	//*************************************
#if !UE_BUILD_SHIPPING
	if (Settings.DropMips > 0 && Settings.DropMips != 4)
	{
		for (FStreamingRenderAsset& StreamingRenderAsset : StreamingRenderAssets)
		{
			if (IsAborted()) break;
...
void FRenderAssetStreamingMipCalcTask::UpdateLoadAndCancelationRequests_Async()
{
	...
	if (!IsAborted())
	{
		StreamOutRequests.Reserve(PrioritizedRenderAssets.Num());
		StreamInRequests.Reserve(PrioritizedRenderAssets.Num());
	}
	...
	LoadRequests.Reset(StreamOutRequests.Num() + StreamInRequests.Num());
...
void FRenderAssetStreamingMipCalcTask::DoWork()
{
...

	{
		...
		uint64 LengthCycles64 = (FPlatformTime::Cycles64() - StartTime);
		double ElapsedMSTime = FPlatformTime::ToMilliseconds64(LengthCycles64);

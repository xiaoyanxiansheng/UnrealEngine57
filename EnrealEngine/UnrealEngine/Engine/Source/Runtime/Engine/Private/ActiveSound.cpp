...
void FActiveSound::Stop()
{
	if (AudioDevice)
	{
		...
		Audio::Trace::EventLog::SendActiveSoundEvent(*this, Audio::Trace::EventLog::ID::StopRequestActiveSound);
...
{
	// If we haven't cached the closest listener index, just directly query it from the audio device.
	if (ClosestListenerIndex == INDEX_NONE && AudioDevice)
	{
		CacheClosestListener();
	}
	return ClosestListenerIndex;
}

void FActiveSound::CacheClosestListener() const
{
	// Early outs.

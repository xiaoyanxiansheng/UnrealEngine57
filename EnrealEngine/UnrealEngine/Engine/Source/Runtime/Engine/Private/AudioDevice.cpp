...
UE_TRACE_EVENT_BEGIN(Audio, SoundIsAlivePing)
	UE_TRACE_EVENT_FIELD(uint32, DeviceId)
	UE_TRACE_EVENT_FIELD(uint64, Timestamp)
	UE_TRACE_EVENT_FIELD(uint32, PlayOrder)
	UE_TRACE_EVENT_FIELD(UE::Trace::WideString, AssetPath)
...
bool FAudioDevice::HandleAudioMemoryInfo(const TCHAR* Cmd, FOutputDevice& Ar)
{
	...
	if (SoundWaveClass != nullptr)
	{
		...
		for (TObjectIterator<USoundWave> It; It; ++It)
		{
			...
			ELoadingType LoadType = SoundWave->IsStreaming()? ELoadingType::Streaming : ELoadingType::CompressedInMemory;
...
void FAudioDevice::OnTraceStarted(FTraceAuxiliary::EConnectionType TraceType, const FString& TraceDestination)
{
	for (const FActiveSound* ActiveSound : ActiveSounds)
	{
		...
		SendSoundIsAlivePingTraceMessage(*ActiveSound);
	}
...
void FAudioDevice::UpdateSoundClassProperties(float DeltaTime)
{
	...
	TArray<TObjectPtr<USoundMix>, TInlineAllocator<32>> SoundMixesToClear;

	for (TPair<TObjectPtr<USoundMix>, FSoundMixState>& SoundMix :SoundMixModifiers)
	{
		FSoundMixState* SoundMixState = &SoundMix.Value;
		...
		else if (AudioTime >= SoundMixState->FadeOutStartTime && AudioTime < SoundMixState->EndTime)
		{
			...
			if (SoundMixState->CurrentState != ESoundMixState::FadingOut)
			{
				...
				TryClearingEQSoundMix(SoundMix.Key);
			}
		}
		else
		{
		...
		ApplyClassAdjusters(SoundMix.Key, SoundMixState->InterpValue, DeltaTime);

		if (SoundMixState->CurrentState == ESoundMixState::AwaitingRemoval && SoundMixState->PassiveRefCount == 0)
		{
			SoundMixesToClear.Add(SoundMix.Key);
		}
	}

	for (USoundMix* SoundMix : SoundMixesToClear)
	{
		ClearSoundMix(SoundMix);
	}
}
...
{
	// DEPRECATED in 5.7
	// Calling SubsystemCollection.ForEachSubsystem here is not safe because
	// it is on the Game Thread while all other calls are on the Audio Thread.
	// There would be risk of concurrent calls, which will corrupt SubsystemCollection bIsIterating.
}
...
void FAudioDevice::ClearSoundMixModifiers()
{
	...
	// Work on local data since ClearSoundMixModifier might modify SoundMixModifiers
	TArray<TObjectPtr<USoundMix>, TInlineAllocator<32>> Keys;
	SoundMixModifiers.GenerateKeyArray(Keys);

	// Clear all sound mix modifiers
	for (USoundMix* Key : Keys)
	{
		ClearSoundMixModifier(Key);
	}
}
		...
		else
		{
			...
			if (ActiveSoundWorldPtr == nullptr || ActiveSoundWorldPtr->AllowAudioPlayback())
			{
				...
				if (!bStopped)
				{
					...
					ActiveSound->CacheClosestListener();
...
{
#if UE_AUDIO_PROFILERTRACE_ENABLED
	auto ShouldSendIsAliveTraceMessage = [](const FActiveSound& ActiveSound, const float DeltaTime) -> bool
	{
		if (ActiveSound.IsPlayingAudio())
		{
			// Send a ping to Audio Insights every second to confirm the sound is still playing.
			...
			return FMath::Floor(ActiveSound.PlaybackTimeUnscaled + DeltaTime) - FMath::Floor(ActiveSound.PlaybackTimeUnscaled) > 0.0f;
		}
	...
	if (bIsGameTicking)
	{
		...
		{
#if UE_AUDIO_PROFILERTRACE_ENABLED
			if (ShouldSendIsAliveTraceMessage(*ActiveSound, DeltaTime))
			{
				SendSoundIsAlivePingTraceMessage(*ActiveSound);
			}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
			...
			ActiveSound->PlaybackTimeUnscaled += DeltaTime;
	...
	else if (GIsEditor)
	{
		...
		for (FActiveSound* ActiveSound : ActiveSounds)
		{
			...
			{
#if UE_AUDIO_PROFILERTRACE_ENABLED
				if (ShouldSendIsAliveTraceMessage(*ActiveSound, DeviceDeltaTime))
				{
					SendSoundIsAlivePingTraceMessage(*ActiveSound);
				}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
...
void FAudioDevice::SendSoundStartedTraceMessage(const FActiveSound& InActiveSound) const
{
	...
	if (bChannelEnabled)
	{
		const TObjectPtr<UObject> SoundObjectPtr = Audio::Trace::Util::GetSoundObjectPointer(InActiveSound);
		if (SoundObjectPtr == nullptr)
		{
			return;
		}

		const FString OwnerActorLabel = Audio::Trace::Util::GetOwnerActorLabel(InActiveSound);
		...
		UE_TRACE_LOG(Audio, SoundStart, AudioMixerChannel)
			...
			<< SoundStart.AssetPath(*SoundObjectPtr->GetPathName())
			<< SoundStart.EventLogName(*Audio::Trace::EventLog::ID::SoundStart)
			<< SoundStart.SoundClassName(*Audio::Trace::Util::GetSoundBaseAssetName(SoundObjectPtr->GetClass()))
			<< SoundStart.ActorLabel(*Audio::Trace::Util::GetOwnerActorLabel(InActiveSound))
			<< SoundStart.ActorIconName(*Audio::Trace::Util::GetOwnerActorIconName(InActiveSound).ToString());
	}
}

void FAudioDevice::SendSoundIsAlivePingTraceMessage(const FActiveSound& InActiveSound) const
{
	const bool bChannelEnabled = UE_TRACE_CHANNELEXPR_IS_ENABLED(AudioMixerChannel);
	...
	if (bChannelEnabled)
	{
		...
		const FString SoundBaseAssetName = Audio::Trace::Util::GetSoundBaseAssetName(SoundObjectPtr->GetClass());
		const FString OwnerActorLabel = Audio::Trace::Util::GetOwnerActorLabel(InActiveSound);

		UE_TRACE_LOG(Audio, SoundIsAlivePing, AudioMixerChannel)
			<< SoundIsAlivePing.DeviceId(InActiveSound.AudioDevice->DeviceID)
			<< SoundIsAlivePing.Timestamp(FPlatformTime::Cycles64())
			<< SoundIsAlivePing.PlayOrder(InActiveSound.PlayOrder)
			<< SoundIsAlivePing.AssetPath(*SoundObjectPtr->GetPathName())
			<< SoundIsAlivePing.SoundClassName(*SoundBaseAssetName)
			<< SoundIsAlivePing.ActorLabel(*OwnerActorLabel);
	}
}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
	Audio::Trace::EventLog::SendEvent(DeviceID, Audio::Trace::EventLog::ID::StopAllRequested);
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
...
void FAudioDevice::AddNewActiveSound(const FActiveSound& ActiveSound, const TArray<FAudioParameter>* InDefaultParams, const FString* EventLogID)
{
	...
	//Previously the copy was done in AddNewActiveSoundInternal when creating the Lamba function. 
	TSharedRef<FActiveSound> NewUniqueActiveSound = MakeShared<FActiveSound>(ActiveSound);

	AddNewActiveSound(NewUniqueActiveSound, MoveTemp(Params), EventLogID);
}
...
{
	//Previously the copy was done in AddNewActiveSoundInternal when creating the Lamba function. 
	TSharedRef<FActiveSound> NewUniqueActiveSound = MakeShared<FActiveSound>(NewActiveSound);

	AddNewActiveSound(NewUniqueActiveSound, MoveTemp(InDefaultParams));
}
...
void FAudioDevice::AddNewActiveSoundInternal(const FActiveSound& InNewActiveSound, TArray<FAudioParameter>&& InDefaultParams, FAudioVirtualLoop* InVirtualLoopToRetrigger)
{
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
		Audio::Trace::EventLog::SendActiveSoundEvent(InNewActiveSound, Audio::Trace::EventLog::ID::PlayFailedNotPlayable);
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
			Audio::Trace::EventLog::SendActiveSoundEvent(InNewActiveSound, Audio::Trace::EventLog::ID::PlayFailedOutOfRange);
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
				Audio::Trace::EventLog::SendActiveSoundEvent(InNewActiveSound, Audio::Trace::EventLog::ID::PlayFailedDebugFiltered);
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
				Audio::Trace::EventLog::SendActiveSoundEvent(InNewActiveSound, Audio::Trace::EventLog::ID::PlayFailedConcurrency);
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
			}
		}
		return;
	...
	if (InVirtualLoopToRetrigger)
	{
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
		Audio::Trace::EventLog::SendActiveSoundEvent(VirtualActiveSound, Audio::Trace::EventLog::ID::SoundRealized);
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
		...
		{
			ActiveSound->SeekRestartTimes = InVirtualLoopToRetrigger->GetEffectivePlaybackTimes();
		}
	}
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
	Audio::Trace::EventLog::SendActiveSoundEvent(ActiveSound, Audio::Trace::EventLog::ID::SoundVirtualized);
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
...
void FAudioDevice::StopActiveSound(const uint64 AudioComponentID, const FString* EventLogID)
{
	if (!IsInAudioThread())
	{
		...
		FAudioDevice* AudioDevice = this;
		FAudioThread::RunCommandOnAudioThread([AudioDevice, AudioComponentID, EventLogID]()
		{
			AudioDevice->StopActiveSound(AudioComponentID, EventLogID);
	...
	SendCommandToActiveSounds(AudioComponentID, [AudioDeviceID, EventLogID](FActiveSound& ActiveSound)
	{
		...
		if (AudioDeviceManager)
		{
			...
			if (AudioDevice.IsValid())
			{
				...
				if (EventLogID)
				{
					Audio::Trace::EventLog::SendActiveSoundEvent(ActiveSound, *EventLogID);
				}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
...
void FAudioDevice::StopActiveSound(FActiveSound* ActiveSound, const FString* EventLogID)
{
	check(IsInAudioThread());

#if UE_AUDIO_PROFILERTRACE_ENABLED
	if (ActiveSound && EventLogID)
	{
		Audio::Trace::EventLog::SendActiveSoundEvent(*ActiveSound, *EventLogID);
	}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
...
void FAudioDevice::PauseActiveSound(const uint64 AudioComponentID, const bool bInIsPaused)
{
	...
	SendCommandToActiveSounds(AudioComponentID, [bInIsPaused, AudioDeviceID](FActiveSound& ActiveSound)
	{
		...
		if (GEngine)
		{
			...
			if (AudioDeviceManager)
			{
				...
				if (AudioDevice.IsValid())
				{
					Audio::Trace::EventLog::SendActiveSoundEvent(ActiveSound, bInIsPaused ? Audio::Trace::EventLog::ID::PauseSoundRequested : Audio::Trace::EventLog::ID::ResumeSoundRequested);
				}
			}
		}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
...
void FAudioDevice::RemoveActiveSound(FActiveSound* ActiveSound)
{
	...
	if (bChannelEnabled)
	{
		const TObjectPtr<UObject> SoundObjectPtr = Audio::Trace::Util::GetSoundObjectPointer(*ActiveSound);
		if (SoundObjectPtr)
		{
			UE_TRACE_LOG(Audio, SoundStop, AudioMixerChannel)
				...
				<< SoundStop.AssetPath(*SoundObjectPtr->GetPathName())
				<< SoundStop.EventLogName(*Audio::Trace::EventLog::ID::SoundStop)
				<< SoundStop.SoundClassName(*Audio::Trace::Util::GetSoundBaseAssetName(SoundObjectPtr->GetClass()))
				<< SoundStop.ActorLabel(*Audio::Trace::Util::GetOwnerActorLabel(*ActiveSound))
				<< SoundStop.ActorIconName(*Audio::Trace::Util::GetOwnerActorIconName(*ActiveSound).ToString());
		}
	}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
...
void FAudioDevice::PlaySoundAtLocation(USoundBase* Sound, UWorld* World, float VolumeMultiplier, float PitchMultiplier, float StartTime, const FVector& Location, const FRotator& Rotation, USoundAttenuation* AttenuationSettings, USoundConcurrency* Concurrency, TArray<FAudioParameter>&& Params, const AActor* OwningActor)
{
	check(IsInGameThread());
	...
	if (Sound->IsLooping() || Sound->IsPlayWhenSilent() || SoundIsAudible(Sound, World, Location, AttenuationSettingsToApply, MaxDistance, FocusFactor))
	{
		...
		TSharedRef<FActiveSound> NewSharedActiveSound = MakeShared<FActiveSound>();
		FActiveSound& NewActiveSound = NewSharedActiveSound.Get();
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
		const FString* EventLogID = &Audio::Trace::EventLog::ID::PlayRequestSoundAtLocation;
#else
		const FString* EventLogID = nullptr;
#endif // UE_AUDIO_PROFILERTRACE_ENABLED

		AddNewActiveSound(MoveTemp(NewSharedActiveSound), MoveTemp(Params), EventLogID);
	}
	else
	{
...
void FAudioDevice::PlaySoundAtLocation(USoundBase* Sound, UWorld* World, float VolumeMultiplier, float PitchMultiplier, float StartTime, const FVector& Location, const FRotator& Rotation, USoundAttenuation* AttenuationSettings, USoundConcurrency* Concurrency, const TArray<FAudioParameter>* Params, const AActor* OwningActor)
{
	...
	TArray<FAudioParameter> ParamsCopy;
	if (Params)
	{
		ParamsCopy.Append(*Params);
	}

	PlaySoundAtLocation(Sound, World, VolumeMultiplier, PitchMultiplier, StartTime, Location, Rotation, AttenuationSettings, Concurrency, MoveTemp(ParamsCopy), OwningActor);
}
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
	Audio::Trace::EventLog::SendEvent(DeviceID, Audio::Trace::EventLog::ID::FlushAudioDeviceRequested);
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
...
float FAudioDevice::GetCompressionDurationThreshold(const FSoundGroup &SoundGroup)
{
	...
	float CompressedDurationThreshold = 0.f; // related cvar has been removed
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
				Audio::Trace::EventLog::SendActiveSoundEvent(*ActiveSound, Audio::Trace::EventLog::ID::StopRequestSoundsUsingResource);
#endif // UE_AUDIO_PROFILERTRACE_ENABLED

	...
	FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& Args)
	{
		...
		for (const FAssetData& WorldAsset : WorldAssets)
		{
			...
			if (Percentage != CurrentPercentage)
			{
				...
				UE_LOG(LogWorld, Log, TEXT("Processing world packages (%02d%%)"), Percentage);
			}
		}

		if (InconsistentWorlds.Num())
		{
			UE_LOG(LogWorld, Warning, TEXT("Found %d inconsistent worlds:"), InconsistentWorlds.Num());

			for (TPair<FName, uint32> InconsistentWorld : InconsistentWorlds)
			{
				UE_LOG(LogWorld, Warning, TEXT("\t %s (%d)"), *InconsistentWorld.Key.ToString(), InconsistentWorld.Value);
			}
		}
	})
);
...
namespace UE::Private::World
{
	...
	int32 GetLocalPlayerIdentifier(FURL& InUrl)
	{
		if (const TCHAR* LocalPlayerIdStr = InUrl.GetOption(TEXT("LocalPlayerId="), nullptr))
		{
			...
			{
				UE_LOG(LogNet, Warning, TEXT("Invalid local player id."));
			}
		}
...
void UWorld::FinishDestroy()
{
	...
	if (bHasEverBeenInitialized)
	{
		...
		if (FXSystem)
		{
			FFXSystemInterface::MarkPendingKill( FXSystem );
			FXSystem = NULL;
		}
...
void UWorld::InternalUpdateStreamingState()
{
	...
	if (HasBegunPlay())
	{
		static FName TransactionalWorkName(TEXT("UWorld::InternalUpdateStreamingState"));
	...
	if( NetDriver->ServerConnection )
	{
		...
		switch (MessageType)
		{
			case NMT_CloseChildConnection:
			{
				...
				{
					UE_LOG(LogNet, Log, TEXT("Received NMT_CloseChildConnection with NetPlayerIndex %d on the client"), NetPlayerIndexToClose);
	...
	else
	{
		...
		switch (MessageType)
		{
			case NMT_CloseChildConnection:
			{
				...
				if (FNetControlMessage<NMT_CloseChildConnection>::Receive(Bunch, NetPlayerIndexToClose))
				{
					UE_LOG(LogNet, Log, TEXT("Received NMT_CloseChildConnection with NetPlayerIndex %d on the server"), NetPlayerIndexToClose);
...
bool UWorld::IsPlayInMobilePreview() const
{
	...
	if (FPIEPreviewDeviceModule::IsRequestingPreviewDevice()
		...
		|| FDataDrivenShaderPlatformInfo::GetIsPreviewPlatform(Scene ? Scene->GetShaderPlatform(): GMaxRHIShaderPlatform))
	{
		return true;
	}
#endif // WITH_EDITOR
...
void UWorld::InsertPostProcessVolume(IInterface_PostProcessVolume* InVolume)
{
	...
	for (int32 Index = InsertIndex + 1; Index < NumVolumes; Index++)
	{
		if (PostProcessVolumes[Index] == InVolume)
		{
			return;
		}
	}
...
{
	int32 RemoveIndex;
	if (PostProcessVolumes.Find(InVolume, RemoveIndex))
	{
		PostProcessVolumes.RemoveAt(RemoveIndex);
		PostProcessVolumeCachedSizes.RemoveAt(RemoveIndex);
		return true;
	}
	return false;
}

...
extern bool TrackRenderAssetEvent( FStreamingRenderAsset* StreamingTexture, UStreamableRenderAsset* Texture, bool bForceMipLevelsToBeResident, const FRenderAssetStreamingManager* Manager );
...
void UTexture2D::BeginDestroy()
{
	...
	TrackRenderAssetEvent(nullptr, this, false, 0);
}
...
FVirtualTexture2DResource::FVirtualTexture2DResource(const UTexture2D* InOwner, FVirtualTextureBuiltData* InVTData, int32 InFirstMipToUse)
{
	...
	FullNameHash = GetTypeHash(InOwner->GetName().ToLower());
...
#if WITH_EDITOR
void FVirtualTexture2DResource::InitializeEditorResources(FRHICommandListBase& RHICmdList, IVirtualTexture* InVirtualTexture)
{
	...
	if (GIsEditor && !IsRunningCommandlet())
	{
		...
		if (bUseFallbackTexture)
		{
			...
			const FRHITextureCreateDesc TextureDesc =
				FRHITextureCreateDesc::Create2D(*Name, 1, 1, EPixelFormat::PF_B8G8R8A8)
				.AddFlags(TexCreateFlags)
...

			{
				...
				Subresource.WriteColor(VTData->LayerFallbackColors[0].ToFColor(true));
			}

			Texture2DRHI = Initializer.Finalize();
		}
		else
		{
			...
			if (FRHITexture* ReferencedTexture = TextureReferenceRHI->GetReferencedTexture())
			{
			...
			if (Texture2DRHI == nullptr)
			{
				...
				Texture2DRHI = RHICmdList.CreateTexture(Desc);
			}
			...
			for (uint32 TileY = 0u; TileY < MipHeightInTiles; ++TileY)
			{
				for (uint32 TileX = 0u; TileX < MipWidthInTiles; ++TileX)
				{
					...
					const FVTRequestPageResult RequestResult = InVirtualTexture->RequestPageData(RHICmdList, ProducerHandle, LayerMask, MipLevel, vAddress, EVTRequestPagePriority::High);
			...
			RHICmdList.BindDebugLabelName(Texture2DRHI, *Name);
		}
	...
	if (!HasPendingInitOrStreaming() && CachedSRRState.StreamIn(NewMipCount) && ensure(Texture2DResource))
	{
		...
		if (!CustomMipDataProvider && GUseGenericStreamingPath != 1)
		{
			...
			if (HasDerivedData(this))
			{
				...
				if (Texture2DResource && Texture2DResource->bUsePartiallyResidentMips)
				{

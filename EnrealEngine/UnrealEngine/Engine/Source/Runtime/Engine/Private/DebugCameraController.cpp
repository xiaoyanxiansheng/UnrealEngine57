	...
	if (Player != nullptr)
	{
		...
		while (FParse::Line(&Command, Line, CmdLen+1))	// The FParse::Line function expects the full array size, including the NULL character.
		{
			if (Player->Exec( GetWorld(), Line, StrOut) == false)
			{
				...
				// The Player pointer could be null if the calling console command has changed the player controller back to the
				// original one. In that case, the Player pointer will be null so we can't switch the player controller back

...
void FScreenshotRequest::RequestScreenshot(bool bInShowUI)
{
	...
	RequestScreenshot(TEXT(""), bInShowUI, /*bAddUniqueSuffix*/ true, /*bHdrScreenshot*/ false, FIntRect());
}

void FScreenshotRequest::RequestScreenshot(const FString& InFilename, bool bInShowUI, bool bAddUniqueSuffix, bool bHdrScreenshot, const FIntRect& InViewRect)
{
...
FIntRect FScreenshotRequest::ViewRect = FIntRect();
...
int32 FStatUnitData::DrawStat(FViewport* InViewport, FCanvas* InCanvas, int32 InX, int32 InY)
{
	...
	for (uint32 GPUIndex : FRHIGPUMask::All())
	{
		...
		if (GRHISupportsGPUUsage)
		{
			...
			RawGPUUsageFraction[GPUIndex] = GPUUsageFractions.CurrentProcessMHz;
			GPUUsageFraction[GPUIndex] = 0.9 * GPUUsageFraction[GPUIndex] + 0.1 * RawGPUUsageFraction[GPUIndex];

			RawGPUMemoryUsage[GPUIndex] = GPUUsageFractions.CurrentProcessMemoryUsage;
			GPUMemoryUsage[GPUIndex] = 0.9 * GPUMemoryUsage[GPUIndex] + 0.1 * RawGPUMemoryUsage[GPUIndex];

			RawGPUExternalUsageFraction[GPUIndex] = GPUUsageFractions.ExternalProcessesMHz;
			GPUExternalUsageFraction[GPUIndex] = 0.9 * GPUExternalUsageFraction[GPUIndex] + 0.1 * RawGPUExternalUsageFraction[GPUIndex];

			RawGPUExternalMemoryUsage[GPUIndex] = GPUUsageFractions.ExternalProcessMemoryUsage;
			GPUExternalMemoryUsage[GPUIndex] = 0.9 * GPUExternalUsageFraction[GPUIndex] + 0.1 * RawGPUExternalMemoryUsage[GPUIndex];
		}
	}
	...
	// Draw unit.
	{
		...
		// Draw all GPU informations
		{
			...
			const int32 GPURowCount = 1 + (GRHISupportsGPUUsage ? 5 : 0) + (bDisplayGPUIndexes ? 1 : 0);
			...
			// Draw the different timings:
			{
				...
				if (GRHISupportsGPUUsage)
				{
					...
					DrawRowTitle(GPURowId++, GPUColumnCount, TEXT("GPU Mem"), NoUnitGraphColor);
					DrawRowTitle(GPURowId++, GPUColumnCount, TEXT("GPUExt Usage"), NoUnitGraphColor);
					DrawRowTitle(GPURowId++, GPUColumnCount, TEXT("GPUExt Mem"), NoUnitGraphColor);
				}
			...
			for (uint32 GPUIndex : FRHIGPUMask::All())
			{
				...
				{
					constexpr float GIGABYTE = 1024.0f * 1024.0f * 1024.0f;
					{
					...
					{
						const FColor Color = (GPUMemoryUsage[GPUIndex] > GIGABYTE * 3) ? StatRed : ((GPUMemoryUsage[GPUIndex] > GIGABYTE * 2) ? StatOrange : StatGreen);
						DrawCell(
							GPURowId++, GPUIndex* ColumnPerGPU + AvgUnitColumnId, GPUColumnCount,
							GetMemoryString((double)GPUMemoryUsage[GPUIndex]), Color);
					}

					{
						const FColor Color = (GPUExternalUsageFraction[GPUIndex] > 0.5f) ? StatRed : ((GPUExternalUsageFraction[GPUIndex] > 0.25f) ? StatOrange : StatGreen);
						DrawCell(
							GPURowId++, GPUIndex* ColumnPerGPU + AvgUnitColumnId, GPUColumnCount,
					...
					{
						const FColor Color = (GPUExternalMemoryUsage[GPUIndex] > GIGABYTE * 3) ? StatRed : ((GPUExternalMemoryUsage[GPUIndex] > GIGABYTE * 2) ? StatOrange : StatGreen);
						DrawCell(
							GPURowId++, GPUIndex* ColumnPerGPU + AvgUnitColumnId, GPUColumnCount,
							GetMemoryString((double)GPUExternalMemoryUsage[GPUIndex]), Color);
					}
				}
				check(GPURowId == GPURowCount);
			}
...
void FDummyViewport::InitRHI(FRHICommandListBase& RHICmdList)
{
	...
	RenderTargetTextureRHI = RHICmdList.CreateTexture(Desc);
}

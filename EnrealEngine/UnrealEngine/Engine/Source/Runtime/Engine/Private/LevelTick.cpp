...
static TAutoConsoleVariable<int32> CVarAllowAsyncRenderThreadUpdates(
	...
	TEXT("0: GameThread")
	TEXT("1: GameThread with ParallelFor")
	TEXT("2: Async task with ParallelFor"));
...
enum class EComponentEndOfFrameUpdateMode : uint8
{
	// Game thread only
	GameThread,

	// Parallel tasks with the game thread helping out
	Parallel,
...
{
	const auto GetMode = [] (int32 Value)
	{
		return FMath::Clamp<EComponentEndOfFrameUpdateMode>((EComponentEndOfFrameUpdateMode)Value, EComponentEndOfFrameUpdateMode::GameThread, EComponentEndOfFrameUpdateMode::Async);
	};

#if WITH_EDITOR
	if (World->IsGameWorld())
	{
		return GetMode(CVarAllowAsyncRenderThreadUpdatesEditorGameWorld.GetValueOnAnyThread());
	}
	else
	{
		return GetMode(CVarAllowAsyncRenderThreadUpdatesEditor.GetValueOnAnyThread());
	}
#else
	return GetMode(CVarAllowAsyncRenderThreadUpdates.GetValueOnAnyThread());
#endif
}
...
private:
	...
	FORCEINLINE static void ClearReadyForEarlyEndOfFrameUpdate(UActorComponent* Component)
	{
		Component->bReadyForEarlyEndOfFrameUpdate = false;
	}
	...
	FORCEINLINE static void SetUpdating(UActorComponent* Component, bool bUpdating)
	{
		Component->bRenderStateUpdating = bUpdating;
	}
};
...
{
	int32 MarkedIndex = GetComponentsThatNeedEndOfFrameUpdate(Component).IndexOfByKey(Component);
...
void UWorld::AddPrimitiveComponentPSOPrecacheCompleted(UPrimitiveComponent* Component)
{
	if (Component->IsRenderStateUpdating())
	{
		ComponentsThatNeedEndOfFrameMarkRenderStateDirty.Emplace(Component);
	}
	else
	{
...
void UWorld::ClearActorComponentEndOfFrameUpdate(UActorComponent* Component)
{
	...
	if (Component->IsRenderStateUpdating())
	{
		UE_LOG(LogLevel, Fatal, TEXT("Calling ClearActorComponentEndOfFrameUpdate on component %s but it is currently being updated. The component was marked for early update with MarkForNeededEndOfFrameUpdate but is being modified after the early update task was kicked. This is a race condition. Use AllowAsyncRenderThreadUpdates{Editor, EditorGameWorld} 1 to disable async updates to work around this issue."), *Component->GetReadableName());
	}

	const uint32 CurrentState = Component->GetMarkedForEndOfFrameUpdateState();

	auto& ComponentsToUpdate = GetComponentsThatNeedEndOfFrameUpdate(Component);
	auto& ComponentsToSync   = GetComponentsThatNeedEndOfFrameSync(Component);
	...
	if (CurrentState == EComponentMarkedForEndOfFrameUpdateState::Marked)
	{
		...
		check(ComponentsToUpdate.IsValidIndex(ArrayIndex));
		check(ComponentsToUpdate[ArrayIndex] == Component);
		ComponentsToUpdate[ArrayIndex] = nullptr;
		FMarkComponentEndOfFrameUpdateState::ClearReadyForEarlyEndOfFrameUpdate(Component);
	}
	...
	{
		ComponentsToSync.Remove(Component);
...
void UWorld::MarkActorComponentForNeededEndOfFrameUpdate(UActorComponent* Component, bool bForceGameThread)
{
	...
	if (CurrentState == EComponentMarkedForEndOfFrameUpdateState::Unmarked)
	{
		...
		if (!bForceGameThread)
		{
			bForceGameThread = GetComponentEndOfFrameUpdateMode(this) == EComponentEndOfFrameUpdateMode::GameThread;
		}
		...
		{
			FMarkComponentEndOfFrameUpdateState::Set(Component, ComponentsToUpdate.Num(), EComponentMarkedForEndOfFrameUpdateState::Marked);
		...
		if (Component->RequiresPreEndOfFrameSync())
		{
			...
			ComponentsToSync.Add(Component);
		}
	}
}
...
{
	return ComponentsThatNeedEndOfFrameUpdate_OnGameThread.Num() > 0 || ComponentsThatNeedEndOfFrameUpdate_AsyncTick.Num() > 0 || ComponentsThatNeedEndOfFrameUpdate.Num() > 0;
}
...
struct UWorld::FEndOfFrameUpdateContext
{
	...
	void SetUpdating(bool bUpdating)
	{
		for (UActorComponent* Component : Components)
		{
			if (Component)
			{
				FMarkComponentEndOfFrameUpdateState::SetUpdating(Component, bUpdating);
			}
		}
	}
	...
	void UpdateComponent_Parallel(FRenderCommandList* RenderCommandList, int32 Index)
	{
		...
		FRenderCommandList::FRecordScope RecordScope(RenderCommandList);
		FSceneInterface::FPrimitiveTransformUpdateScope TransformScope(PrimitiveTransformUpdater);
		...
		UActorComponent* NextComponent = Components[Index];
		if (NextComponent)
		{
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(DeferredRenderUpdates_GameThread);
		FSceneInterface::FPrimitiveTransformUpdateScope TransformScope(PrimitiveTransformUpdater);
		...
		DeferredUpdates.Reserve(Components_OnGameThread.Num());

		for (UActorComponent* Component : Components_OnGameThread)
		{
			if (Component)
			{
		...
		FStaticMeshComponentBulkReregisterContext ReregisterContext(InScene, DeferredUpdates, EBulkReregister::RenderState);

		for (UActorComponent* Component : DeferredUpdates)
		{
			FMarkComponentEndOfFrameUpdateState::SetUpdating(Component, true);
			Component->DoDeferredRenderUpdates_Concurrent();
			FMarkComponentEndOfFrameUpdateState::SetUpdating(Component, false);
		}
	}
};
...
void UWorld::SendAllEndOfFrameUpdatesInternal(EEndOfFrameUpdateLocation Location)
{
	...
	const bool bAsyncInTick = Location == EEndOfFrameUpdateLocation::Async && bInTick;
	...
	if ((bAsyncInTick && ComponentsThatNeedEndOfFrameUpdate_AsyncTick.IsEmpty()) || !HasEndOfFrameUpdates())
	{
		return;
	}

	const auto MergeArray = [](TArray<TObjectPtr<UActorComponent>>& Dst, TArray<TObjectPtr<UActorComponent>>& Src)
	{
		if (Dst.IsEmpty())
		{
			Dst = MoveTemp(Src);
	...
	const auto MergeSet = [](TArray<TObjectPtr<UActorComponent>>& Dst, TSet<TObjectPtr<UActorComponent>>& Src)
	{
		Dst.Reserve(Dst.Num() + Src.Num());
		for (TObjectPtr<UActorComponent> Ptr : Src)
		{
			Dst.Emplace(Ptr);
		}
		Src.Reset();
	...
	FEndOfFrameUpdateContext* Context = new FEndOfFrameUpdateContext();
	MergeArray(Context->Components, ComponentsThatNeedEndOfFrameUpdate_AsyncTick);
	MergeSet(Context->ComponentsToSync, ComponentsThatNeedPreEndOfFrameSync_AsyncTick);

	if (!bAsyncInTick)
	{
		MergeArray(Context->Components, ComponentsThatNeedEndOfFrameUpdate);
		MergeSet(Context->ComponentsToSync, ComponentsThatNeedPreEndOfFrameSync);
		Context->Components_OnGameThread = MoveTemp(ComponentsThatNeedEndOfFrameUpdate_OnGameThread);
		ComponentsThatNeedEndOfFrameUpdate_OnGameThread.Empty(Context->Components_OnGameThread.Num());
	}

	for (UActorComponent* Component : Context->ComponentsToSync)
	{
		if (Component)
		{
	...
	// If we have a previous context active, then force this one to run asynchronously as well. They are not allowed to touch
	// the same components (validated via IsInAsyncEndOfFrameUpdate) so it is safe to run the tasks concurrently.
	const bool bLaunchAsyncTask = Scene && (!EndOfFrameUpdateContexts.IsEmpty() || Location == EEndOfFrameUpdateLocation::Async);
	...
	if (bInTick)
	{
		...
		for (UActorComponent* Component : Context->Components)
		{
			if (Component)
			{
		...
		for (UActorComponent* Component : Context->Components_OnGameThread)
		{
			if (Component)
			{
				Component->OnEndOfFrameUpdateDuringTick();
	...
	const auto ParallelWork = [Context](FRenderCommandList* InRenderCommandList, int32 Index)
	{
		Context->UpdateComponent_Parallel(InRenderCommandList, Index);
	};

	const EParallelForFlags ParallelForFlags = EParallelForFlags::Unbalanced;
	...
	if (Scene)
	{
		PrimitiveTransformUpdater = Scene->CreatePrimitiveTransformUpdater(Context->Components.Num() + Context->Components_OnGameThread.Num());
		Context->PrimitiveTransformUpdater = PrimitiveTransformUpdater;
	}

	if (bLaunchAsyncTask)
	{
		const int32 NumContexts = ParallelForImpl::GetNumberOfThreadTasks(Context->Components.Num(), 1, ParallelForFlags);

		if (EndOfFrameUpdateContexts.IsEmpty())
		{
			Scene->GetSkeletalMeshUpdater()->BeginAsyncPushCommands();

			TRACE_BEGIN_REGION(UE_END_OF_FRAME_UPDATE_REGION);
#if WITH_EDITOR
			FObjectCacheEventSink::BeginQueueNotifyEvents();
#endif
		}

		EndOfFrameUpdateContexts.Emplace(Context);
		Context->SetUpdating(true);

		FRenderCommandList* TaskRenderCommandList = FRenderCommandList::Create();
		...
		FRenderCommandDispatcher::Submit(TaskRenderCommandList, RootRenderCommandList);
	...
	else
	{
		...
		const auto GameThreadWork = [Context, Scene = Scene, RootRenderCommandList]
		{
			Context->Update_GameThread(Scene, RootRenderCommandList);
		};
	...
	if (Scene)
	{
		Scene->UpdatePrimitiveTransforms(PrimitiveTransformUpdater);
	}
...
{
	if (GetComponentEndOfFrameUpdateMode(this) != EComponentEndOfFrameUpdateMode::Async)
	{
		return;
	}

	SendAllEndOfFrameUpdatesInternal(EEndOfFrameUpdateLocation::Async);
}
...
void UWorld::FinishAsyncSendAllEndOfFrameUpdates()
{
	if (!EndOfFrameUpdateContexts.IsEmpty())
	{
		Scene->GetSkeletalMeshUpdater()->EndAsyncPushCommands();

		EndOfFrameUpdateTasks.Emplace(MoveTemp(EndOfFrameUpdatePrerequisiteTask));
		UE::Tasks::Wait(EndOfFrameUpdateTasks);
		EndOfFrameUpdateTasks.Reset();

		for (FEndOfFrameUpdateContext* Context : EndOfFrameUpdateContexts)
		{
			Context->SetUpdating(false);
			delete Context;
		}
		EndOfFrameUpdateContexts.Reset();
		...
		TRACE_END_REGION(UE_END_OF_FRAME_UPDATE_REGION);
	}

	for (UActorComponent* Component : ComponentsThatNeedEndOfFrameMarkRenderStateDirty)
	{
		Component->MarkRenderStateDirty();
	}
	ComponentsThatNeedEndOfFrameMarkRenderStateDirty.Empty();
}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(SendAllEndOfFrameUpdates);
	SendAllEndOfFrameUpdatesInternal(EEndOfFrameUpdateLocation::Immediate);
	FlushDeferredParameterCollectionInstanceUpdates();

	if (!EndOfFrameUpdateTasks.IsEmpty())
	{
		UE::Tasks::FTaskEvent TaskEvent{ UE_SOURCE_LOCATION };
		TaskEvent.AddPrerequisites(EndOfFrameUpdateTasks);
		TaskEvent.Trigger();

		EndOfFrameUpdateTasks.Reset();
		EndOfFrameUpdatePrerequisiteTask = Scene->GetSkeletalMeshUpdater()->AddPushCommandsTask(TaskEvent);
	}
}
...
extern TSAN_ATOMIC(int32) CSVActorTotalCount;
...
void UWorld::Tick( ELevelTick TickType, float DeltaSeconds )
{
	...
	for (int32 i = 0; i < LevelCollections.Num(); ++i)
	{
		...
		if (!bHasTickedACollection)
		{
			...
			// Kick early end of frame updates after the major tick managers run, as they are the most likely to contain systems that touch components.
	...
	{
		if (ULevelStreamingProfilingSubsystem* LevelStreamingProfilingSubsystem = GetSubsystem<ULevelStreamingProfilingSubsystem>())
		{
			LevelStreamingProfilingSubsystem->RecordLevelStreamingCountsToCSV();
		}
	}
#endif // (CSV_PROFILER_STATS && !UE_BUILD_SHIPPING)

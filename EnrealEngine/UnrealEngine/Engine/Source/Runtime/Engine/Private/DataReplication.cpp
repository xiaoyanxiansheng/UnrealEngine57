...
static TAutoConsoleVariable<int32> CVarQueueRpcsForTransactions(
	TEXT("net.QueueRpcsForTransactions"),
	1,
	TEXT("When set to 1, any transactional connection will queue RPCs and execute them in transactions allowing for rescheduling"));
...
void FObjectReplicator::CleanUp()
{
	...
	if ( Connection != nullptr )
	{
		...
		Connection->Driver->ReplicatorsWithPendingTransactions.Remove( this );
...
void FObjectReplicator::ReceivedNak( int32 NakPacketId )
{
	...
	else if (!RepLayout->IsEmpty())
	{
		if (FSendingRepState* SendingRepState = RepState.IsValid() ? RepState->GetSendingRepState() : nullptr)
		{
			// This is used to determine if we can skip the update,
			// unconditionally decrementing leads to objects with no custom delta serializer
			// to be unable to be skipped for replication since their value is never updated elsewhere
			if (SendingRepState->CustomDeltaChangeIndex != 0)
			{
...
bool FObjectReplicator::ReceivedBunch(FNetBitReader& Bunch, const FReplicationFlags& RepFlags, const bool bHasRepLayout, bool& bOutHasUnmapped)
{
	...
	const bool bQueueRpcForTransactions = ConnectionNetDriver->ShouldReplicateTransactionally() && (CVarQueueRpcsForTransactions.GetValueOnGameThread() > 0);
#else
	constexpr bool bQueueRpcForTransactions = false;
#endif
	...
	while (true)
	{
		...
		{
			ESkipRpcBehavior SkipRpcBehavior = bCanDelayRPCs ? ESkipRpcBehavior::SkipIfNotReady : ESkipRpcBehavior::NeverSkip;
			if (bQueueRpcForTransactions)
			{
				SkipRpcBehavior = ESkipRpcBehavior::AlwaysSkip;

				// To debug why RPC property deserialization is aborting:
				//UE_AUTORTFM_ONABORT()
				//{
				//	ensureMsgf(false, TEXT("Expecting that we never abort when we are using queued transactional RPCs"));
				//};
			}

			bool bSkippedRpcExec = false;
			TSet<FNetworkGUID> UnmappedGuids;
			bool bSuccess = ReceivedRPC(Reader, UnmappedGuids, bSkippedRpcExec, RepFlags, FieldCache, SkipRpcBehavior);

			if (!bSuccess)
			{
				return false;
			}
			else if (bSkippedRpcExec)
			{
				if (bQueueRpcForTransactions && UnmappedGuids.IsEmpty())
				{
					// We are skipping now and executing later (PostTickDispatch)
					PendingTransactionalRPCs.Emplace(FieldCache, RepFlags, Reader, DriverReplicationFrame, UnmappedGuids);
					ConnectionNetDriver->ReplicatorsWithPendingTransactions.Add(this);
				}
				else
				{
					// Unmapped GUIDs: Either due to not receiving a GUID yet from replication (client only),
					// or this can also happen on the client & server due to async loading.
...
bool FObjectReplicator::ReceivedRPC(FNetBitReader& Reader, TSet<FNetworkGUID>& UnmappedGuids, bool& bOutSkippedRpcExec, const FReplicationFlags& RepFlags, const FFieldNetCache* FieldCache, ESkipRpcBehavior SkipRpcBehavior)
{
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(RPCDosDetection);
	...
	if (bCanExecute)
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(EvaluateCanExecute);
...

	{
		...
		if (WriterBits != Writer.GetNumBits())
		{
			if (bHasDirtyCustomDeltaProperties)
			{
				...
				// If we wrap around, skip 0 as this is used to detect that we never sent any custom delta properties
				if (ChangelistState->CustomDeltaChangeIndex == 0)
				{
					++ChangelistState->CustomDeltaChangeIndex;
				}
			}
		...
		else
		{
			...
			// SendingRepState->CustomDeltaChangeIndex needs to be incremented in all cases just like above,
			// otherwise we cannot skip CustomDeltaProperties for connections on which they do not write
			// if they received a NAK on their last write
...
void FObjectReplicator::UpdateUnmappedObjects(bool & bOutHasMoreUnmapped)
{
	...
	if (PackageMapClient && OwningChannel)
	{
		...
		for (int32 RPCIndex = 0; RPCIndex < PendingLocalRPCs.Num(); RPCIndex++)
		{
			...
			{
				// Force execute if none of our RPC guids are pending, even if other guids are. This is more consistent behavior as it is less dependent on unrelated actors
				ESkipRpcBehavior AllowDelay = bIsGuidPending ? ESkipRpcBehavior::SkipIfNotReady : ESkipRpcBehavior::NeverSkip;

				FunctionName = FieldCache->Field.GetName();
				bSuccess = ReceivedRPC(Reader, UnmappedGuids, bFunctionWasUnmapped, Pending.RepFlags, FieldCache, AllowDelay);
			}
...
{
	ensureMsgf(AutoRTFM::IsClosed(), TEXT("%hs is meant to be executed inside of a transaction"), __func__);
	if (!ensure(Connection) || !ensure(Connection->Driver) || !ensure(Connection->Driver->NetCache))
	{
		return;
	}

	const bool bIsServer = Connection->Driver->IsServer();
	if (!ensureMsgf(bIsServer, TEXT("%hs: Expected to only be executed on the server"), __func__))
	{
		return;
	}
	...
	// Handle pending RPCs, in order
	TArray<FRPCPendingLocalCall> ServiceTheseRPCs;
	Swap(PendingTransactionalRPCs, ServiceTheseRPCs);

	for (const FRPCPendingLocalCall& Pending : ServiceTheseRPCs)
	{
		ensureMsgf(Pending.UnmappedGuids.IsEmpty(), TEXT("PendingTransactionalRPCs are never expected to have UnmappedGuids (UnmappedGuids are in PendingLocalRPCs)"));

		bool bSuccess;
		const FFieldNetCache* FieldCache = ClassCache->GetFromIndex(Pending.RPCFieldIndex);			
		if (FieldCache == nullptr)
		{
			UE_LOG(LogNet, Warning, TEXT("%hs: FieldCache not found. Object: %s"), __func__, *Object->GetFullName());
			bSuccess = false;
		}
		else
		{
			// Help us keep track of this function's execution time
			FName FunctionName = FieldCache->Field.GetFName();
			UE::RemoteExecutor::OnRegionBeginDelegate.ExecuteIfBound(FunctionName);
			AutoRTFM::PushOnAbortHandler(&Pending, []()
			{
			...
			TSet<FNetworkGUID> UnmappedGuids;
			bool bSkipped = false;
			bSuccess = ReceivedRPC(Reader, UnmappedGuids, bSkipped, Pending.RepFlags, FieldCache, ESkipRpcBehavior::NeverSkip);

			ensureMsgf(UnmappedGuids.IsEmpty(), TEXT("%hs: Servers are expected to never have UnmappedGuids"), __func__);
			ensureMsgf(!bSkipped, TEXT("%hs: We should never skip an RPC when requesting NeverSkip"), __func__);

			UE::RemoteExecutor::OnRegionEndDelegate.ExecuteIfBound(TEXT(""));
			AutoRTFM::PopOnAbortHandler(&Pending);
		}

		if (!bSuccess)
		{
			FString FunctionName = FieldCache ? FieldCache->Field.GetName() : TEXT("Unknown");

			const bool bDemoMode = Connection->IsInternalAck();
			if (!bDemoMode)
			{
				// Close our connection and abort rpcs as things are invalid
				UE_LOG(LogNet, Error, TEXT("%hs: Failed executing delayed RPC %s on Object %s, closing connection!"), __func__, *FunctionName, *Object->GetFullName());
			...
			{
				UE_LOG(LogNet, Warning, TEXT("%hs: Failed executing delayed RPC %s on Object %s, skipping RPC!"), __func__, *FunctionName, *Object->GetFullName());
			}
		}
	}
}

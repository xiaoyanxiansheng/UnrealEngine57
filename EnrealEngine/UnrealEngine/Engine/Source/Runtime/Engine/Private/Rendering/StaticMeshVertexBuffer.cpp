...
void FStaticMeshVertexBuffer::InitRHI(FRHICommandListBase& RHICmdList)
{
	...
	if (bCreateTangentsSRV || AreBufferSRVsAlwaysCreatedByDefault(GMaxRHIShaderPlatform))
	{
		TangentsSRV = GetOrCreateTangentsSRV(RHICmdList);
	}
	...
	if (bCreateTexCoordSRV || AreBufferSRVsAlwaysCreatedByDefault(GMaxRHIShaderPlatform))
	{
		TextureCoordinatesSRV = GetOrCreateTexCoordsSRV(RHICmdList);	
	}
}
...
{
	// Get the existing SRV or create a new externally owned SRV
	if (TangentsSRV.IsValid())
	{
		return TangentsSRV;
	}
	...
	FShaderResourceViewRHIRef ExternalTangentsSRV = RHICmdList.CreateShaderResourceView(
		TangentsVertexBuffer.VertexBufferRHI,
		FRHIViewDesc::CreateBufferSRV()
		.SetType(FRHIViewDesc::EBufferType::Typed)
		.SetFormat(Format));
	
	return ExternalTangentsSRV;
}

FShaderResourceViewRHIRef FStaticMeshVertexBuffer::GetOrCreateTexCoordsSRV(FRHICommandListBase& RHICmdList) const
{
	// Get the existing SRV or create a new externally owned SRV
	if (TextureCoordinatesSRV.IsValid())
	{
		return TextureCoordinatesSRV;
	}
	...
	FShaderResourceViewRHIRef ExternalTextureCoordinatesSRV = RHICmdList.CreateShaderResourceView(
		TexCoordVertexBuffer.VertexBufferRHI,
		FRHIViewDesc::CreateBufferSRV()
			.SetType(FRHIViewDesc::EBufferType::Typed)
			.SetFormat(Format));

	return ExternalTextureCoordinatesSRV;
}

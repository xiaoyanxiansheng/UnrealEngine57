...
static const FName NAME_FTexture2DArrayResource(TEXT("FTexture2DArrayResource"));
...
void FTexture2DArrayResource::CreateTexture(FRHICommandListBase& RHICmdList)
{
	...
	const FRHITextureCreateDesc Desc =
		FRHITextureCreateDesc::Create2DArray(*Name, FirstMip.SizeX, FirstMip.SizeY, FirstMip.SizeZ, PixelFormat)
		.SetNumMips(State.NumRequestedLODs)
		.SetFlags(CreationFlags)
		.SetExtData(PlatformData->GetExtData())
		.SetClassName(NAME_FTexture2DArrayResource)
	...
	{		
		...
		{
			uint32 DestStride = 0;
			void* DestData = RHICmdList.GetAsImmediate().LockTexture2DArray(TextureRHI, ArrayIndex, MipIndex, RLM_WriteOnly, DestStride, false);
			if (DestData)
			{
			...
			RHICmdList.GetAsImmediate().UnlockTexture2DArray(TextureRHI, ArrayIndex, MipIndex, false);
		}
	}
...
void FTexture2DArrayResource::CreatePartiallyResidentTexture(FRHICommandListBase& RHICmdList)
{
	unimplemented();

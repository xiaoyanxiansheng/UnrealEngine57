...
static const FName NAME_FTexture3DResource(TEXT("FTexture3DResource"));
...
, InitialData(InState.RequestedFirstLODIdx())
{
		...
		InitialData.GetMipData() + FirstLODIdx, InitialData.GetMipSize() + FirstLODIdx, InOwner->GetPathName()))
	{
		...
		if (GRHIGlobals.UseTexture3DBulkData)
		{
...
void FTexture3DResource::CreateTexture(FRHICommandListBase& RHICmdList)
{
	...
	// Create the RHI texture.
	{
		...
		FRHITextureCreateDesc Desc =
			FRHITextureCreateDesc::Create3D(*Name, FirstMip.SizeX, FirstMip.SizeY, FirstMip.SizeZ, PixelFormat)
			.SetNumMips(State.NumRequestedLODs)
			.SetFlags(CreationFlags)
			.SetExtData(PlatformData->GetExtData())
			.SetClassName(NAME_FTexture3DResource)
			.SetOwnerName(GetOwnerName());

		if (GRHIGlobals.UseTexture3DBulkData)
		{
			InitialData.MergeMips(State.MaxNumLODs);
			Desc.SetInitActionBulkData(&InitialData);
		}
	...
	if (!GRHIGlobals.UseTexture3DBulkData) 
	{
		...
		for (int32 RHIMipIdx = 0; RHIMipIdx < State.NumRequestedLODs; ++RHIMipIdx)
		{
			...
			if (MipData)
			{
				...
				RHICmdList.UpdateTexture3D(TextureRHI, RHIMipIdx, UpdateRegion, NumBlockX * BlockBytes, NumBlockX * NumBlockY * BlockBytes, MipData);
			}
		}
		InitialData.Discard();
	}
...
void FTexture3DResource::CreatePartiallyResidentTexture(FRHICommandListBase& RHICmdList)
{
	unimplemented();

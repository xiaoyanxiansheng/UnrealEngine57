...
{
	return IsFeatureLevelSupported(InShaderPlatform, ERHIFeatureLevel::SM5) && Substrate::IsSpecularProfileEnabled(InShaderPlatform);
}
...
{
	if (!Substrate::IsSubstrateEnabled() || !IsSpecularProfileSupport(ShaderPlatform))
	{
		return nullptr;
	}

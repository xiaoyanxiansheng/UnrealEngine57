...
const static FName NAME_FTexture2DResource(TEXT("FTexture2DResource"));
	...
	else
	{
		...
		for (int32 RHIMipIdx = 0; RHIMipIdx < State.NumRequestedLODs; ++RHIMipIdx)
		{
			...
			if (MipData[ResourceMipIdx])
			{
				uint32 DestPitch = -1;
				uint64 Size = ~0ULL;
				void* TheMipData = RHICmdList.GetAsImmediate().LockTexture2D(TextureRHI, RHIMipIdx, RLM_WriteOnly, DestPitch, false /* bLockWithinMiptail */, true /* bFlushRHIThread */, &Size);
				//check(Size != ~0ULL);

				GetData( ResourceMipIdx, TheMipData, DestPitch, Size );
				RHICmdList.GetAsImmediate().UnlockTexture2D(TextureRHI, RHIMipIdx, false );
			}
		}
	}
}

void FTexture2DResource::CreatePartiallyResidentTexture(FRHICommandListBase& RHICmdList)
{
	const FString Name = TextureName.ToString();
	...
	const FRHITextureCreateDesc Desc =
		FRHITextureCreateDesc::Create2D(*Name, SizeX, SizeY, PixelFormat)
	...
	RHICmdList.GetAsImmediate().VirtualTextureSetFirstMipInMemory(TextureRHI, CurrentFirstMip);
	RHICmdList.GetAsImmediate().VirtualTextureSetFirstMipVisible(TextureRHI, CurrentFirstMip);
	...
	for( int32 MipIndex=CurrentFirstMip; MipIndex < State.MaxNumLODs; MipIndex++ )
	{
		if ( MipData[MipIndex] != NULL )
		{
			uint32 DestPitch = -1;
			uint64 Size = ~0ULL;
			void* TheMipData = RHICmdList.GetAsImmediate().LockTexture2D(TextureRHI, MipIndex, RLM_WriteOnly, DestPitch, false /* bLockWithinMiptail */, true /* bFlushRHIThread */, &Size);
			//check(Size != ~0ULL);

			GetData( MipIndex, TheMipData, DestPitch, Size);
			RHICmdList.GetAsImmediate().UnlockTexture2D(TextureRHI, MipIndex, false);
		}
	}
}
...
uint32 FTexture2DResource::CalculateTightPackedMipSize(int32 MipSizeX, int32 MipSizeY, EPixelFormat PixelFormat, uint32& OutPitch)
{
	const FPixelFormatInfo& Format = GPixelFormats[PixelFormat];

	uint32 NumColumns = FMath::DivideAndRoundUp(MipSizeX, Format.BlockSizeX); // Num-of columns in the source data (in blocks)
	uint32 NumRows    = FMath::DivideAndRoundUp(MipSizeY, Format.BlockSizeY); // Num-of rows in the source data (in blocks)
	...
	const uint32 SrcPitch   = NumColumns * Format.BlockBytes; // Num-of bytes per row in the source data
	const uint32 EffectiveSize = Format.BlockBytes * NumColumns * NumRows;

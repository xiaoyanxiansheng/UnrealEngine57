...
int32 UInstancedStaticMeshComponent::AddInstanceInternal(int32 InstanceIndex, FInstancedStaticMeshInstanceData* InNewInstanceData, const FTransform& InstanceTransform, bool bWorldSpace)
{
	...
	// Indicates if the cached bounds were updated, it's expected to occur in PartialNavigationUpdate().
	bNavigationCachedBoundsUpdated = false;
	...
	const FBoundsCacheElement NavigationCacheElement = CachedBounds[(int32)EBoundsCacheType::Navigation];
	InvalidateCachedBounds();

	// Restore Navigation CachedBounds (the invalidation is not needed if it was updated).
	if (bNavigationCachedBoundsUpdated)
	{
		CachedBounds[(int32)EBoundsCacheType::Navigation] = NavigationCacheElement;
	}
...
void UInstancedStaticMeshComponent::PartialNavigationUpdates(const TConstArrayView<FTransform> InstanceTransforms)
{
	...
	if (InstanceBounds.IsValid)
	{
		...
		CalcAndCacheNavigationBounds();
		bNavigationCachedBoundsUpdated = true;

...
bool UStreamableRenderAsset::IsReadyForFinishDestroy()
{
	...
	if (PendingUpdate)
	{
		...
		if (GEnableAssetUpdateAbandons && PendingUpdate->CanAbandon())
		{
			IStreamingManager::Get().GetRenderAssetStreamingManager().OnRenderAssetUpdateAbandoned(this, PendingUpdate);
			bCanBeReleased = true;
		}
		else if(FRenderAssetUpdateHelper::TickRenderAssetUpdateForGarbageCollection(PendingUpdate))
		{
			bCanBeReleased = true;
		}

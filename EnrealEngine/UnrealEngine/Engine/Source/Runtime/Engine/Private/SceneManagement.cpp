...
void FSimpleElementCollector::DrawBatchedElements(FRHICommandList& RHICmdList, const FMeshPassProcessorRenderState& DrawRenderState, const FSceneView& InView, EBlendModeFilter::Type Filter, ESceneDepthPriorityGroup DepthPriorityGroup, uint32 NumInstances) const
{
...
void FRayTracingInstanceCollector::AddViewMeshArrays(
	const FSceneView* InView,
	FGPUScenePrimitiveCollector* DynamicPrimitiveCollector)
{
	Views.Add(InView);
	RDGPooledBuffers.Add({});

	FMeshElementCollector::AddViewMeshArrays(
		InView,
		nullptr,
		nullptr,
...
void FRayTracingInstanceCollector::SetVisibilityMap(uint32 InVisibilityMap)
{
...
{
	AddRayTracingInstance(0, MoveTemp(Instance));
}

void FRayTracingInstanceCollector::AddRayTracingInstance(int32 ViewIndex, FRayTracingInstance Instance)
{
	FRayTracingInstanceAndViewIndex Tmp;
	Tmp.Instance = MoveTemp(Instance);
	Tmp.ViewIndex = ViewIndex;

	RayTracingInstances.Add(MoveTemp(Tmp));
}
...
{
	AddRayTracingGeometryUpdate(0, MoveTemp(Params));
}

void FRayTracingInstanceCollector::AddRayTracingGeometryUpdate(int32 ViewIndex, FRayTracingDynamicGeometryUpdateParams Params)
{
	FRayTracingDynamicGeometryUpdateRequest UpdateRequest;
	UpdateRequest.Params = MoveTemp(Params);
	UpdateRequest.ViewIndex = ViewIndex;

	RayTracingGeometriesToUpdate.Add(MoveTemp(UpdateRequest));
}

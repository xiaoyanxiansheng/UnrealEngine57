...
#endif
{
	...
	SetDesiredRuntimeBehavior(ELevelInstanceRuntimeBehavior::None);
#endif
}
	...
	if (!HasChildEdit())
	{
		...
		if (bChildChanged)
		{
			...
			if (UBlueprint* BlueprintToPack = GetRootBlueprint())
			{
				...
				// return here because Actor might have been reinstanced
				// @todo_ow: We should only return if actor was reinstanced
				return;
			}
			else
			{

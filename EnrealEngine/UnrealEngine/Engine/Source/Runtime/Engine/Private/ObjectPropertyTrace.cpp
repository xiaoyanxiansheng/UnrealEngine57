	...
	static void TraceObjects()
	{
		...
		for(const TWeakObjectPtr<const UObject>& WeakObject : ObjectPropertyTrace::Objects)
		{
			if(const UObject* TracedObject = WeakObject.Get())
			{
				...
				IterateProperties(TracedObject->GetClass(), TracedObject, [StartCycle, ObjectId](const FString& InTypeString, const FString& InValue, int32 InId, int32 InParentId, FStringBuilderBase & InPropertyNameBuilder)
				{
...
{
	check(ObjectPropertyTrace::TickerHandle.IsValid());
	FTSTicker::RemoveTicker(ObjectPropertyTrace::TickerHandle);
}

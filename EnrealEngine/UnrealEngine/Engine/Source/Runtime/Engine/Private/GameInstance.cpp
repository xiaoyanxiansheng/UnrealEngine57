...
void UGameInstance::OnWorldChanged(UWorld* OldWorld, UWorld* NewWorld)
{
	...
	FWorldDelegates::OnGameInstanceWorldChanged.Broadcast(this, OldWorld, NewWorld);
}
...
ULocalPlayer* UGameInstance::CreateLocalPlayer(FPlatformUserId UserId, FString& OutError, bool bSpawnPlayerController)
{
	...
	else if (LocalPlayers.Num() < MaxSplitscreenPlayers)
	{
		...
		static int32 NextLocalPlayerIdentifier = 0;
		NewPlayer->SetConnectionIdentifier(NextLocalPlayerIdentifier);
		NextLocalPlayerIdentifier++;
...
bool UGameInstance::IsReplayDeferred(const FString& ReplayName) const
{
	if (UReplaySubsystem* ReplaySubsystem = GetSubsystem<UReplaySubsystem>())
	{
		return ReplaySubsystem->IsReplayDeferred(ReplayName);
	}

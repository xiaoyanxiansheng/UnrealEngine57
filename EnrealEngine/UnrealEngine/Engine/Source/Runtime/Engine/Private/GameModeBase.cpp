		...
		if (APlayerController* PlayerController = Iterator->Get())
		{
			if (Cast<UNetConnection>(PlayerController->Player) != nullptr)
			{
				...
				if (!UE::GameModeBase::Private::bAllowListenParamSentToClient)
				{
					...
					static FString ListenString = TEXT("?listen");
					int32 ListenIndex = ClientUrl.Find(ListenString, ESearchCase::IgnoreCase);
					if (ListenIndex >= 0)
					{
						// Only remove if the next character isn't part of a longer option string
						int32 NextIndex = ListenIndex + ListenString.Len();						
						if (NextIndex >= ClientUrl.Len() || ClientUrl[NextIndex] == TEXT('#') || ClientUrl[NextIndex] == TEXT('?'))
						{
							ClientUrl.RemoveAt(ListenIndex, ListenString.Len());
						}
					}
				}

...
void UGameViewportClient::Draw(FViewport* InViewport, FCanvas* SceneCanvas)
{
	...
	uint32 StereoMobilePrimaryScalingMode = 0;
	if (bStereoRendering)
	{
		...
		static const bool bEmulateStereo = FParse::Param(FCommandLine::Get(), TEXT("emulatestereo"));
		EngineShowFlags.StereoRendering = bEmulateStereo ? true : ViewFamily.EngineShowFlags.StereoRendering;

		// Get the stereo mobile primary scaling mode, which determines if primary scaling (e.g. dynres) is redirected to the HMD runtime when otherwise unsupported
		if (World && World->GetFeatureLevel() == ERHIFeatureLevel::ES3_1)
		{
			static const IConsoleVariable* StereoMobilePrimaryScalingModeCVar = IConsoleManager::Get().FindConsoleVariable(TEXT("xr.MobilePrimaryScalingMode"));
			if (StereoMobilePrimaryScalingModeCVar)
			{
				StereoMobilePrimaryScalingMode = StereoMobilePrimaryScalingModeCVar->GetInt();
				if (StereoMobilePrimaryScalingMode < 0 || StereoMobilePrimaryScalingMode > 2)
				{
					ensureMsgf(false, TEXT("Supported values for xr.MobilePrimaryScalingMode are 0-2, %d is unsupported. Using value 0 instead."), StereoMobilePrimaryScalingMode);
					StereoMobilePrimaryScalingMode = 0;
				}
			}
		}
	}
	...
	bool bUsesDynamicResolution = false;
	{
		...
		// Skip if we're using stereo rendering and redirecting all secondary scaling to the HMD runtime (xr.SecondaryScreenPercentage.HMDRenderTarget).
		if (!bStereoRendering && ViewFamily.SupportsScreenPercentage())
		{
			float CustomSecondaryScreenPercentage = CVarSecondaryScreenPercentage.GetValueOnGameThread();
		...
		// Force screen percentage support if we're using stereo rendering and redirecting primary scaling to the HMD runtime.
		bool bDynamicResolutionScalingViaHMDRuntime = false;
		bool bAllResolutionScalingViaHMDRuntime = false;
		switch (StereoMobilePrimaryScalingMode)
		{
		case 0:		// Never
			bDynamicResolutionScalingViaHMDRuntime = false;
			bAllResolutionScalingViaHMDRuntime = false;
			break;
		case 1:		// Only if otherwise unsupported and dynamic resolution driven
			bDynamicResolutionScalingViaHMDRuntime = true;
			bAllResolutionScalingViaHMDRuntime = false;
			break;
		case 2:		// Only if otherwise unsupported
			bDynamicResolutionScalingViaHMDRuntime = true;
			bAllResolutionScalingViaHMDRuntime = true;
			break;
		default:
			checkNoEntry();
			break;
		...
		if (ViewFamily.EngineShowFlags.ScreenPercentage || bDynamicResolutionScalingViaHMDRuntime)
		{
			...
			{
				ViewFamily.EngineShowFlags.ScreenPercentage = true; // If this is false and we're scaling via HMD, need to set it to true now to avoid an assert in FLegacyScreenPercentageDriver()
		...
		ViewFamily.EngineShowFlags.ScreenPercentage = ViewFamily.EngineShowFlags.ScreenPercentage || bUsesDynamicResolution || bAllResolutionScalingViaHMDRuntime;
	...
	for (FLocalPlayerIterator Iterator(GEngine, MyWorld); Iterator; ++Iterator)
	{
		...
		if (LocalPlayer)
		{
			...
			const int32 NumViews = bStereoRendering ? 
				GEngine->StereoRenderingDevice->GetDesiredNumberOfViews(bStereoRendering) 
				: (LocalPlayer->bEmulateSplitscreen ? 2 : 1);

			checkf(!bStereoRendering || !LocalPlayer->bEmulateSplitscreen, TEXT("r.ScreenshotTest.EmulateSplitscreenMode is unsupported when using stereo rendering."));
			...
			for (int32 ViewIndex = 0; ViewIndex < NumViews; ++ViewIndex)
			{
				...
				FSceneView* View = LocalPlayer->CalcSceneView(&ViewFamily, ViewLocation, ViewRotation, InViewport, nullptr, (bStereoRendering || LocalPlayer->bEmulateSplitscreen) ? ViewIndex : INDEX_NONE);
...

	{
		...
		// Change screen percentage method to raw output when using stereo rendering and redirecting primary scaling to the HMD runtime.
		if (bStereoRendering)
		{
			bool bUsingRawOutput = false;
			switch (StereoMobilePrimaryScalingMode)
			{
			case 0:		// Never
				bUsingRawOutput = false;
				break;
			case 1:		// Only if otherwise unsupported and dynamic-resolution driven
				bUsingRawOutput = !ViewFamily.SupportsScreenPercentage() && bUsesDynamicResolution;
				break;
			case 2:		// Only if otherwise unsupported
				bUsingRawOutput = !ViewFamily.SupportsScreenPercentage() && ViewFamily.EngineShowFlags.ScreenPercentage;
				break;
			default:
				checkNoEntry();
				break;
			...
			if (bUsingRawOutput)
			{
				for (FSceneView* View : Views)
				{
					...
					if (bWarnOnce && View->PrimaryScreenPercentageMethod == EPrimaryScreenPercentageMethod::TemporalUpscale)
					{
						UE_LOG(LogViewport, Warning, TEXT("Temporal upscaling is unsupported when redirecting primary scaling to the HMD runtime. The HMD runtime's spatial scaling will be used instead."));
						bWarnOnce = false;
					}
		...
		// Make sure the screen percentage show flag is not set to true if screen percentage is unsupported.
		// If we're using RawOutput due to the setup above, SupportsScreenPercentage() should now return true.
				...
				if( LocalPlayer )
				{
					...
					if (View != NULL)
					{
						...
						if (DebugCanvas != nullptr)
						{
							...
							UDebugDrawService::Draw(ViewFamily.EngineShowFlags, InViewport, View, DebugCanvas, DebugCanvasObject, PlayerController);
...
bool UGameViewportClient::ProcessScreenShots(FViewport* InViewport)
{
	...
	if (GIsDumpingMovie || FScreenshotRequest::IsScreenshotRequested() || GIsHighResScreenshot)
	{
		...
		{
			ensureMsgf(FScreenshotRequest::GetViewRect().IsEmpty(), TEXT("Specifying a screenshot view rect is not supported for slate widget screenshots (bShowUI=true). Using the full window size."));
		...
		{
			// Set up view rect
			FIntRect ViewRect = FScreenshotRequest::GetViewRect();
			if (ViewRect.IsEmpty())
			{
				// If an empty view rect was provided (default), screenshot the entire render target
				ViewRect = FIntRect(0, 0, Size.X, Size.Y);
			}
			else if (!FIntRect(0, 0, Size.X, Size.Y).Contains(ViewRect))
			{
				ensureMsgf(false, TEXT("Requested a screenshot with view rect %s, which does not fit within the viewport size of %dx%d. Using the full viewport size."), *ViewRect.ToString(), Size.X, Size.Y);
				ViewRect = FIntRect(0, 0, Size.X, Size.Y);
			}
			else
			{
				// If a valid view rect was provided, update the output size to match the view rect size
				Size.X = ViewRect.Width();
				Size.Y = ViewRect.Height();
			}

			check(FIntPoint(Size.X, Size.Y) == ViewRect.Size());

			// Take screenshot
			if (bHdrEnabled)
			{
				bScreenshotSuccessful = GetViewportScreenShotHDR(InViewport, BitmapHDR, ViewRect);
				ConvertPixelDataToSCRGB(BitmapHDR, ViewportOutputFormat);
			}
			else if (InViewport->GetShaderResourceTexture() && InViewport->GetShaderResourceTexture()->GetDesc().Dimension == ETextureDimension::Texture2DArray)
			{
				...
				if (ScreenshotTextureArrayMode == 1 || ScreenshotTextureArrayMode == 2)
				{
					...
					for (int CurrentArrayIndex = 0; CurrentArrayIndex < ArraySize; CurrentArrayIndex++)
					{
						...
						bScreenshotSuccessful = GetViewportScreenShot(InViewport, BitmapArray[CurrentArrayIndex], ViewRect, ReadSurfaceFlags);
				...
				else
				{
					...
					bScreenshotSuccessful = GetViewportScreenShot(InViewport, Bitmap, ViewRect, ReadSurfaceFlags);
				}
			}
			else
			{
				bScreenshotSuccessful = GetViewportScreenShot(InViewport, Bitmap, ViewRect);
			}
		}

...
namespace CameraLensEffectCVars
{
	static bool bAttachedEmitterTransformOptimizationEnabled = true;
	static FAutoConsoleVariableRef CVarAttachedEmitterTransformOptimizationEnabled(
		TEXT("CameraLensEffect.AttachedEmitterTransformOptimizationEnabled"),
		bAttachedEmitterTransformOptimizationEnabled,
		TEXT("If true, GetAttachedEmitterTransform avoids doing unnecessary work when the camera FOV matches the emitter's base FOV, and avoids recalculating constant values."),
		ECVF_Default);
}
...
FTransform ICameraLensEffectInterface::GetAttachedEmitterTransform(const AActor* Emitter, const FVector& CamLoc, const FRotator& CamRot, float CamFOVDeg)
{
	if (const ICameraLensEffectInterface* InterfaceEmitter = Cast<ICameraLensEffectInterface>(Emitter))
	{
		...
		const float BaseFOV = InterfaceEmitter->GetBaseFOV();
		if (!CameraLensEffectCVars::bAttachedEmitterTransformOptimizationEnabled || !FMath::IsNearlyEqual(BaseFOV, CamFOVDeg))
		{
			constexpr float HalfFovDegreesToRadians = 0.5f * UE_PI / 180.f;
			AdjustedRelativeLoc.X *= FMath::Tan(BaseFOV * HalfFovDegreesToRadians) / FMath::Tan(CamFOVDeg * HalfFovDegreesToRadians);

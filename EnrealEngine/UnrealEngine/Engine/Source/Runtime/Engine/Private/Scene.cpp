...
#include UE_INLINE_GENERATED_CPP_BY_NAME(Scene)
...
{
	OutPostProcessSettings->bOverride_BloomGaussianIntensity = true;
	...
	OutPostProcessSettings->BloomGaussianIntensity = Intensity;
...
{
	OutPostProcessSettings->bOverride_BloomConvolutionIntensity = true;
	...
	OutPostProcessSettings->BloomConvolutionIntensity = Intensity;
...
bool FLensBloomSettings::Serialize(FArchive& Ar)
{
	Ar.UsingCustomVersion(FFortniteMainBranchObjectVersion::GUID);
	return false;
}

void FLensBloomSettings::PostSerialize(const FArchive& Ar)
{
	if (Ar.IsLoading())
	{
		if (Ar.CustomVer(FFortniteMainBranchObjectVersion::GUID) < FFortniteMainBranchObjectVersion::SpecializeBloomIntensity)
		{
			Intensity = GaussianSum.Intensity;
			GaussianSum.Intensity = 1.0f;
			Convolution.Intensity = 1.0f;
		}
	}
}
...
static void VerifyPostProcessingProperties(
	...
	const TArray<FString>& NewStructPrefixes,
	const TMap<FString, FString>& RenameMap)
{
	check(NewStructs.Num() == NewStructPrefixes.Num());
	...
	for (int Index = 0; Index < NewStructs.Num(); ++Index)
	{
		const UStruct* NewStruct = NewStructs[Index];
		const FString& NewStructPrefix = NewStructPrefixes[Index];

		for (FProperty* Property = NewStruct->PropertyLink; Property; Property = Property->PropertyLinkNext)
		{
			// Make sure there is no duplicate.
			FString Name = NewStructPrefix + Property->GetNameCPP();
			check(!NewPropertySet.Contains(Name));
			NewPropertySet.Add(Name, Property);
		}
	}
...
{
	{
		...
		RenameMap.Add(TEXT("BloomThreshold"), TEXT("GaussianThreshold"));
		RenameMap.Add(TEXT("BloomSizeScale"), TEXT("GaussianSizeScale"));
		RenameMap.Add(TEXT("Bloom1Size"), TEXT("GaussianFilter1Size"));
		RenameMap.Add(TEXT("Bloom2Size"), TEXT("GaussianFilter2Size"));
		RenameMap.Add(TEXT("Bloom3Size"), TEXT("GaussianFilter3Size"));
		RenameMap.Add(TEXT("Bloom4Size"), TEXT("GaussianFilter4Size"));
		RenameMap.Add(TEXT("Bloom5Size"), TEXT("GaussianFilter5Size"));
		RenameMap.Add(TEXT("Bloom6Size"), TEXT("GaussianFilter6Size"));
		RenameMap.Add(TEXT("Bloom1Tint"), TEXT("GaussianFilter1Tint"));
		RenameMap.Add(TEXT("Bloom2Tint"), TEXT("GaussianFilter2Tint"));
		RenameMap.Add(TEXT("Bloom3Tint"), TEXT("GaussianFilter3Tint"));
		RenameMap.Add(TEXT("Bloom4Tint"), TEXT("GaussianFilter4Tint"));
		RenameMap.Add(TEXT("Bloom5Tint"), TEXT("GaussianFilter5Tint"));
		RenameMap.Add(TEXT("Bloom6Tint"), TEXT("GaussianFilter6Tint"));

		VerifyPostProcessingProperties(TEXT("Bloom"),
			TArray<const UStruct*>({
				FGaussianSumBloomSettings::StaticStruct(),
				FConvolutionBloomSettings::StaticStruct(),
...
FPostProcessSettings::FPostProcessSettings()
{
	...
	Bloom6Size = 64.0f;
	BloomConvolutionIntensity = 1.0f;

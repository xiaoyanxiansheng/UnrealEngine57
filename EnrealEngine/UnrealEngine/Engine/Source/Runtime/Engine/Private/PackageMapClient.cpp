...
DEFINE_LOG_CATEGORY_STATIC(LogNetPackageMapRemoteObject, Warning, All);
#endif
...
#if UE_WITH_REMOTE_OBJECT_HANDLE
	UE_LOG(LogNetPackageMapRemoteObject, Log, TEXT("UPackageMapClient::SerializeNewActor: Actor=%s NetGUID=%s"),
		   *GetNameSafe(NewObj), *NetGUID.ToString());
#endif
...
#if UE_WITH_REMOTE_OBJECT_HANDLE
		UE_LOG(LogNetPackageMapRemoteObject, Log, TEXT("Looking up NetGUID %s in the local cache and found '%s'"), *NetGUID.ToString(), *GetNameSafe(Object));

		if (Object == nullptr)
		{
			FRemoteObjectId RemoteObjectId = FNetGUIDCache::GetRemoteObjectIdFromNetworkGUID(NetGUID);
			if (UE::RemoteObject::Handle::IsRemote(RemoteObjectId))
			{
				if (AutoRTFM::IsTransactional())
				{
					UE_LOG(LogNetPackageMapRemoteObject, Log, TEXT("Dereferencing a remote NetGUID %s (%s)."), *NetGUID.ToString(), *RemoteObjectId.ToString());
					TObjectPtr<UObject> RemoteObjectPtr = TObjectPtr<UObject>(FObjectPtr(RemoteObjectId));
					Object = RemoteObjectPtr.Get();
				}
				else
				{
					UE_LOG(LogNetPackageMapRemoteObject, Log, TEXT("Dereferencing a remote NetGUID %s (%s) that is outside a transaction."), *NetGUID.ToString(), *RemoteObjectId.ToString());
				}
			}
		}
#endif
...
#if UE_WITH_REMOTE_OBJECT_HANDLE
FNetworkGUID FNetGUIDCache::GetNetworkGUIDFromRemoteObjectId(const FRemoteObjectId RemoteObjectId, const bool bIsStatic)
{
	// Ignore the first bit of FRemoteObjectId which is reserved for the NetGUID static flag.
	return FNetworkGUID::CreateFromIndex(RemoteObjectId.GetIdNumber() >> 1, bIsStatic);
}

FRemoteObjectId FNetGUIDCache::GetRemoteObjectIdFromNetworkGUID(const FNetworkGUID NetGuid)
{
	// The NetGUID ObjectId is a 64-bit value with the first bit as the static flag. Since this value is ignored by
	// FRemoteObjectId (it's a reserved bit) the value is masked away.
	return FRemoteObjectId::CreateFromInt(NetGuid.ObjectId & ~1).GetLocalized();
}
#endif
...
FNetGUIDCache::FNetGUIDCache(UNetDriver* InDriver, uint64 NetworkGuidSeed) 
	: Driver(InDriver)
...
#if UE_WITH_REMOTE_OBJECT_HANDLE
	const FRemoteObjectId RemoteObjectId = TObjectPtr<UObject>(Object).GetRemoteId();
	const FNetworkGUID NewNetGuid = GetNetworkGUIDFromRemoteObjectId(RemoteObjectId, IsStatic != 0);

	UE_LOG(LogNetPackageMapRemoteObject, Log, TEXT("Assigning object %s to use NetGUID %s based on remote object id %s"),
		   *Object->GetName(),
		   *NewNetGuid.ToString(),
		   *RemoteObjectId.ToString());
...
FNetworkGUID FNetGUIDCache::AssignNewNetGUIDFromPath_Server( const FString& PathName, UObject* ObjOuter, UClass* ObjClass )
{
	...
	// Generate new NetGUID and assign it
#if UE_WITH_REMOTE_OBJECT_HANDLE
	const FRemoteObjectId RemoteObjectId = TObjectPtr<UObject>(ObjOuter).GetRemoteId();
	const FNetworkGUID NewNetGuid = FNetworkGUID::CreateFromIndex(RemoteObjectId.GetIdNumber(), true);

	UE_LOG(LogNetPackageMapRemoteObject, Log, TEXT("Assigning object %s from path %s to use NetGUID %s based on remote object id %s"),
		   *ObjOuter->GetName(),
		   *PathName,
		   *NewNetGuid.ToString(),
		   *RemoteObjectId.ToString());
...
{
	FNetworkGUID* NetGUIDPtr = NetGUIDLookup.Find(Object);
	if (NetGUIDPtr)
	{
		FNetworkGUID NetGUID = *NetGUIDPtr;

		ensure(ObjectLookup.Contains(NetGUID));
		ObjectLookup.Remove(NetGUID);
...
void FNetGUIDCache::RemoveActorNetGUIDs(const AActor* Actor)
{
	...
	auto RemoveSubObjectsFromGuidCache = [this](const FSubObjectRegistry* SubObjectRegistry)
	{
		if (SubObjectRegistry)
		{
			for (const FSubObjectRegistry::FEntry& SubObjectInfo : SubObjectRegistry->GetRegistryList())
			{
				RemoveNetGUID(SubObjectInfo.GetSubObject());
			}
		}
	};

	RemoveNetGUID(Actor);
	RemoveSubObjectsFromGuidCache(&FSubObjectRegistryGetter::GetSubObjects(Actor));

	for (UActorComponent* Component : Actor->GetComponents())
	{
		RemoveNetGUID(Component);
		RemoveSubObjectsFromGuidCache(FSubObjectRegistryGetter::GetSubObjectsOfActorComponent(Actor, Component));
	}
}

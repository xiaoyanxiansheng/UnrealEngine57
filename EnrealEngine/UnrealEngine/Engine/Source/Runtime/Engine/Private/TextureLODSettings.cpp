...
void UTextureLODSettings::SetPreviewTextureGroupRedirect(UTextureLODSettings* InPreviewTextureGroupRedirect)
{
	if (PreviewTextureGroupRedirect != InPreviewTextureGroupRedirect)
	{
		...
		for (FThreadSafeObjectIterator ObjIterator(UTexture::StaticClass(), /*bOnlyGCedObjects = */false, /*AdditionalExclusionFlags = */RF_NoFlags); ObjIterator; ++ObjIterator)
		{
			...
			if (!Texture->IsA<UTextureRenderTarget>())
			{

...
{
	static const FName Name_ActorLabel = GET_MEMBER_NAME_CHECKED(AActor, ActorLabel);
	...
	static const FName Name_MigratingAsset = GET_MEMBER_NAME_CHECKED(AActor, bMigratingAsset);	
	...
	const bool bReregisterComponents = (MemberPropertyName != Name_ActorLabel) && (((GEditor && GEditor->bIsSimulatingInEditor && GetWorld() != nullptr) || ReregisterComponentsWhenModified()));
	...
	if (MemberPropertyName == Name_MigratingAsset)
	{
		if (bMigratingAsset)
		{
			SetFlags(RF_MigratingAsset);
		}
		else
		{
			ClearFlags(RF_MigratingAsset);
		}
	}
...
void AActor::FixupDataLayers(bool bRevertChangesOnLockedDataLayer /*= false*/)
{
	...
	DataLayerAssets.SetNum(Algo::RemoveIf(DataLayerAssets, [this, &InvalidDataLayerAssets](const TSoftObjectPtr<UDataLayerAsset>& DataLayerAsset)
	{
		return InvalidDataLayerAssets.Contains(DataLayerAsset) ||
			!UDataLayerAsset::CanBeReferencedByActor(DataLayerAsset, this) ||
			// Make sure there are no EDLs in DataLayerAssets
			(DataLayerAsset.IsValid() && DataLayerAsset->IsA<UExternalDataLayerAsset>());
	}));

	// Remove duplicates
	TSet<TSoftObjectPtr<UDataLayerAsset>> UniqueDataLayerAssets(DataLayerAssets);
	DataLayerAssets = UniqueDataLayerAssets.Array();
...
bool AActor::SupportsDataLayerType(TSubclassOf<UDataLayerInstance> InDataLayerType) const
{
	...
	const bool bIsLevelIsPartitioned = Level ? Level->bIsPartitioned || Level->IsWorldPartitionRuntimeCell() : true;

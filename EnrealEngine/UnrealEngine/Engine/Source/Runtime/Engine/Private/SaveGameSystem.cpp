...
void ISaveGameSystem::LoadGameIfExistsAsync(bool bAttemptToUseUI, const TCHAR* Name, FPlatformUserId PlatformUserId, FSaveGameAsyncLoadCompleteCallback Callback)
{
	FString SlotName(Name);
	...
	AsyncTaskPipe.Launch(UE_SOURCE_LOCATION,
		[this, bAttemptToUseUI, SlotName, PlatformUserId, Callback]()
		{
			// load the savegame if it exists
			TSharedRef<TArray<uint8>> Data = MakeShared<TArray<uint8>>();
			int32 UserIndex = FPlatformMisc::GetUserIndexForPlatformUser(PlatformUserId);

			const ESaveExistsResult ExistsResult = DoesSaveGameExistWithResult(*SlotName, UserIndex);
			bool bResult = false;

			if (ExistsResult == ESaveExistsResult::OK)
			{
				bResult = LoadGame(bAttemptToUseUI, *SlotName, UserIndex, Data.Get());
			}
...
void FBaseAsyncSaveGameSystem::LoadGameIfExistsAsync(bool bAttemptToUseUI, const TCHAR* Name, FPlatformUserId PlatformUserId, FSaveGameAsyncLoadCompleteCallback Callback)
{
	// If a platform has a custom async implementation it is not safe to use the generic task pipe. So, fall back to calling the normal async load
	// Platforms can override this if InternalDoesSaveGameExistAsync always needs to be called before InternalLoadGameAsync

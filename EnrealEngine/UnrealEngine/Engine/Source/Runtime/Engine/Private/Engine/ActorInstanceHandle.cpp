...
bool FActorInstanceHandle::IsUnresolvedDehydratedActor() const
{
	if (ResolutionStatus == EResolutionStatus::NeedsResolving)
	{
		if (const UPrimitiveComponent* Component = CastChecked<UPrimitiveComponent>(ReferenceObject.Get(), ECastCheckedType::NullAllowed))
		{
			if (AActor* OwnerActor = Component->GetOwner())
			{
				if (OwnerActor->GetClass()->ImplementsInterface(UActorInstanceManagerInterface::StaticClass()))
				{
					return true;
				}
			}
...
bool FActorInstanceHandle::operator==(const FActorInstanceHandle& Other) const
{
	...
	// If both handles point to the same Manager/InstanceIndex pair, we can assume they'll point
	// to the same ReferenceObject.
	if ((InstanceIndex != INDEX_NONE && Other.InstanceIndex != INDEX_NONE) && (ManagerInterface == Other.ManagerInterface && InstanceIndex == Other.InstanceIndex))
	{
		return true;
	}
	...
	if (MyActor == ManagerInterface.GetObject() && OtherActor == Other.ManagerInterface.GetObject())
	{
		// Both actors point to their own managers, so we can compare their Manager/InstanceIndex instead,
		// since two handles may live inside the same manager. In this case, we'll just return false, since
		// we already verified above that their Manager/InstanceIndex are different.
		return false;
	}

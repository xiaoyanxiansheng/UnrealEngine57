...
void UDemoNetDriver::FinalizeFastForward(const double StartTime)
{
	...
	if (ServerConnection != nullptr)
	{
		// OnReps can cause the channel map to be modified. Make a copy and lookup the object in the active channel map.
		const FActorChannelMap& ChannelMap = ServerConnection->ActorChannelMap();
		const FActorChannelMap ChannelMapCopy = ChannelMap;
		for (const TPair<TWeakObjectPtr<AActor>, UActorChannel*>& ChannelPair : ChannelMapCopy)
		{
			const TWeakObjectPtr<AActor>& Actor = ChannelPair.Key;
			if (!Actor.IsValid())
			{
				continue;
			}

			// Look up the actor in the original channel map
			if (const UActorChannel*const* Channel = ChannelMap.Find(Actor))
			{
				for (const TPair<UObject*, TSharedRef<FObjectReplicator>>& ReplicatorPair : (*Channel)->ReplicationMap)
				{
...
void UDemoNetDriver::NotifyActorDestroyed(AActor* Actor, bool IsSeamlessTravel, EChannelCloseReason CloseReason)
{
	check(Actor != nullptr);
	...
	Super::NotifyActorDestroyed(Actor, IsSeamlessTravel, CloseReason);
}

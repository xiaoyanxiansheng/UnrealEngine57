...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AudioDerivedData)
...
#define STREAMEDAUDIO_DERIVEDDATA_VER		TEXT("479371f6537b4fc1be91786813e83a16")				// This depends on the above key, but will regenerate all streaming chunk data derived from the compressed audio
...
#if WITH_EDITOR
	if (Ar.IsCooking() && Ar.IsSaving() && Ar.GetCookContext() && Ar.GetCookContext()->GetCookTagList() && Owner)
	{
...
{
#if WITH_EDITORONLY_DATA
	return CookInputs->SoundFullName;
#else
	return CookInputs->SoundName;
#endif
}

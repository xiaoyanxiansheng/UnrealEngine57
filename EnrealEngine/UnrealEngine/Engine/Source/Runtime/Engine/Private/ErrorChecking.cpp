...
void ABrush::CheckForErrors()
{
	...
	{		
		...
		if( Brush && !FActorEditorUtils::IsABuilderBrush(this) && !IsBrushShape() )
		{
			...
			{
				// A brush without any polygons in it is invalid.  Should be deleted.
				FFormatNamedArguments Arguments;
                Arguments.Add(TEXT("ActorName"), FText::FromString(GetName()));
                FMessageLog("MapCheck").Warning()
                	->AddToken(FUObjectToken::Create(this))
                	->AddToken(FTextToken::Create(FText::Format(LOCTEXT( "MapCheck_Message_BrushNullPolygons", "{ActorName} : Brush has a NULL polygon reference - please delete" ), Arguments) ))
                	->AddToken(FMapErrorToken::Create(FMapErrors::BrushZeroPolygons));
			}

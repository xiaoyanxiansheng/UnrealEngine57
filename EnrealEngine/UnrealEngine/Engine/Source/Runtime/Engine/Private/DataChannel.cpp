...
void UControlChannel::ReceivedBunch( FInBunch& Bunch )
{
	...
	while (!Bunch.AtEnd() && bStopReadingBunch == false && Connection != nullptr && Connection->GetConnectionState() != USOCK_Closed) // if the connection got closed, we don't care about the rest
	{
		...
		else if (MessageType == NMT_IrisNetRefHandleError)
		{
			...
			if (FNetControlMessage<NMT_IrisNetRefHandleError>::Receive(Bunch, NetRefHandleError, NetRefHandleId, ExtraNetRefs))
			{
				if (UReplicationSystem* IrisRepSystem = Connection->Driver->GetReplicationSystem())
				{
		...
		else if (Connection->Driver->Notify != nullptr)
		{
			...
			if (Connection->Driver->ShouldReplicateTransactionally())
			{
				static FName TransactionalWorkName("UControlChannel::ReceivedBunch NotifyControlMessage");
				UE::RemoteExecutor::ExecuteTransactional(TransactionalWorkName, [this, MessageType, &Bunch]()
					{
		...
		if (Pos == Bunch.GetPosBits() && !Bunch.IsError())
		{
			switch (MessageType)
			{
				...
				case NMT_CloseChildConnection:
					FNetControlMessage<NMT_CloseChildConnection>::Discard(Bunch);
					break;
				default:
...
void UActorChannel::DestroyActorAndComponents(EChannelCloseReason Reason)
{
...
bool UActorChannel::CleanUp(const bool bForDestroy, EChannelCloseReason CloseReason)
{
	...
	if (!bIsServer)
	{
		...
		if (!bHandled && Actor != nullptr)
		{
			...
			else if (!Actor->bNetTemporary && Actor->GetWorld() != NULL && !IsEngineExitRequested() && Connection->Driver->ShouldClientDestroyActor(Actor, CloseReason))
			{
				UE_LOG(LogNetDormancy, Verbose, TEXT("UActorChannel::CleanUp: Destroying Actor (ChIndex: %i): %s"), ChIndex, *GetFullNameSafe(Actor));

				DestroyActorAndComponents(CloseReason);
			}
		}
	}
...
void UActorChannel::SetChannelActor(AActor* InActor, ESetChannelActorFlags Flags)
{
	...
	// We could check Actor->IsPendingKill here, but that would suppress the warning later.
...
void UActorChannel::NotifyActorChannelOpen(AActor* InActor, FInBunch& InBunch)
{
	...
	UWorld* const World = (NetDriver && NetDriver->World) ? NetDriver->World.Get() : nullptr;
...
static void	TestObjectRefSerialize( const TArray<FString>& Args, UWorld* InWorld )
{
	...
	UObject* Object = StaticFindObject( UObject::StaticClass(), NULL, *Args[0], EFindObjectFlags::None );
	if (!Object)
	{

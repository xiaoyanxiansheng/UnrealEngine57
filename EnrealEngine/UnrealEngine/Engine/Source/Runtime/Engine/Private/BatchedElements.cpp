	...
	) const
{
	...
	else
	{
		...
		SetShaderParametersLegacyVS(RHICmdList, VertexShader, ViewMatrices,
			InstancedView ? FDFRelativeViewMatrices::Create(InstancedView->ViewMatrices)
						  : ViewMatrices);
	}
}

void FBatchedElements::DrawPointElements(FRHICommandList& RHICmdList, const FMatrix& Transform, const uint32 ViewportSizeX, const uint32 ViewportSizeY, const FVector& CameraX, const FVector& CameraY, const uint32 NumInstances) const
{
	...
	if( Points.Num() > 0 )
	{
		...
		RHICmdList.DrawPrimitive(0, NumTris, NumInstances);
	}
}
...
bool FBatchedElements::Draw(FRHICommandList& RHICmdList, const FMeshPassProcessorRenderState& DrawRenderState, ERHIFeatureLevel::Type FeatureLevel, const FSceneView& View, bool bHitTesting, float Gamma /* = 1.0f */, EBlendModeFilter::Type Filter /* = EBlendModeFilter::All */, uint32 NumInstances /* = 1 */) const
{
	...
	if( HasPrimsToDraw() )
	{
			...
			&& (Filter & EBlendModeFilter::OpaqueAndMasked))
		{
			...
			if( LineVertices.Num() > 0 )
			{
				...
				*/
				...
				while( MinVertex < TotalVerts )
				{
					...
					RHICmdList.DrawPrimitive(MinVertex, NumLinePrims, NumInstances);
			...
			// Draw points
			DrawPointElements(RHICmdList, WorldToClip, ViewportSizeX, ViewportSizeY, CameraX, CameraY, NumInstances);
			...
			if ( ThickLines.Num() > 0 )
			{
				...
				*/
				...
				while (LineIndex < ThickLines.Num())
				{
					...
					{
						RHICmdList.SetStreamSource(0, VertexBufferRHI, 0);
						RHICmdList.DrawPrimitive(0, AddedTris, NumInstances);
					}
				}
			...
			if (WireTris.Num() > 0)
			{
				...
				*/
				...
				while( MinTri < TotalTris )
				{
					...
					RHICmdList.DrawPrimitive(MinTri * 3, NumTris, NumInstances);
					MinTri = MaxTri;
				}
				VertexBufferRHI.SafeRelease();
				GraphicsPSOInit.RasterizerState = TStaticRasterizerState<FM_Solid, CM_None>::GetRHI();
			...
			if (ValidSpriteCount > 0)
			{
				...
				for (int32 SpriteIndex = 1; SpriteIndex < ValidSpriteCount + 1; SpriteIndex++)
				{
						...
						CurrentOpacityMask != SortedSprites[SpriteIndex].OpacityMaskRefVal)
					{
						...
						RHICmdList.SetStreamSource(0, VertexBufferRHI, 0);
						RHICmdList.DrawPrimitive(BaseVertex, PrimCount, NumInstances);
		...
		if( MeshElements.Num() > 0)
		{
			...
			for(int32 MeshIndex = 0;MeshIndex < MeshElements.Num();MeshIndex++)
			{
				...
				if (Filter & MeshFilter)
				{
					...
					RHICmdList.DrawIndexedPrimitive(IndexBufferRHI, 0, 0, MeshElement.MaxVertex - MeshElement.MinVertex + 1, 0, MeshElement.Indices.Num() / 3, NumInstances);

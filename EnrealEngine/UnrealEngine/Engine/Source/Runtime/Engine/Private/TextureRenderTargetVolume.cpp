...
void UTextureRenderTargetVolume::GetResourceSizeEx(FResourceSizeEx& CumulativeResourceSize)
{
	...
	const int32 NumBlocksZ	= (SizeZ + BlockSizeZ - 1) / BlockSizeZ;
...
{
	return FString::Printf( TEXT("Render to Texture Volume %dx%dx%d[%s]"), SizeX, SizeY, SizeZ, GPixelFormats[GetFormat()].Name);
}

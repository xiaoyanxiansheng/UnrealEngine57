...
{
	// Operator on a copy of the action list since it might be modified while processing it
	using FActionElementArray = TArray<TPair<int32, class FPendingLatentAction*>, TInlineAllocator<32>>;
	FActionElementArray ItemsToRemove;
	for (const TPair<int32, class FPendingLatentAction*>& Element : ObjectActionList)
	{
		ItemsToRemove.Add(Element);
	}

	FLatentResponse Response(DeltaTime);
	for (FActionElementArray::TIterator It = ItemsToRemove.CreateIterator(); It; ++It)
	{
	...
	for (const TPair<int32, class FPendingLatentAction*>& ItemPair : ItemsToRemove)
	{

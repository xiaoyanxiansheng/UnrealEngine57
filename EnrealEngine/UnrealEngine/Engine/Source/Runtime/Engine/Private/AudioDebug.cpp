...
namespace Audio
{
		...
		FString& OutReason) const
	{
		...
		const FString PlainNameString = FName(Name).GetPlainNameString();
	
		// Allow for partial matches of the name.
		auto MatchesPartOfName = [&Name, &PlainNameString](FName i) -> bool
			{
				return Name.Contains(i.ToString()) || (PlainNameString == i.GetPlainNameString());
			};
	...
	bool FAudioDebugger::IsMuteOrSoloActive() const
	{
		return !DebugNames.MuteSoundClass.IsEmpty()
			|| !DebugNames.MuteSoundCue.IsEmpty()
			|| !DebugNames.MuteSoundWave.IsEmpty()
			|| !DebugNames.SoloSoundClass.IsEmpty()
			|| !DebugNames.SoloSoundCue.IsEmpty()
			|| !DebugNames.SoloSoundWave.IsEmpty();
	}

...
public:
	...
	void HashAddAllItems(int32 NumHashBuckets = 0)
	{
		...
		if (NumHashBuckets <= 0)
		{
			NumHashBuckets = FDefaultSparseSetAllocator::GetNumberOfHashBuckets(Array.Num());
		}
		if (Array.IsEmpty())
		{
			HashTable.Empty();
...
public:
	...
	void RenderThread_ClearParameters()
	{
		...
		ParameterCollectionParameterArray.Empty();
	...
	void RenderThread_UpdateParameter(const FHashedMaterialParameterInfo& ParameterInfo, const ValueType& Value )
	{
		...
		else
		{
			...
			const int32 NumHashBuckets = FDefaultSparseSetAllocator::GetNumberOfHashBuckets(ValueArray.Array.Num());
...
private:
	virtual void EvaluateParameterCollections(FRHICommandListBase& RHICmdList, FUniformExpressionCache& OutUniformExpressionCache, const FMaterialRenderContext& Context, FUniformExpressionCacheAsyncUpdater* Updater) const override;
	...
	static bool IsValidParameterValue(const UMaterialParameterCollection* Value) { return Value != nullptr; }
	...
	static bool IsValidParameterValue(FName Value) { return true; }
	...
	/** Parameter collection parameters for this material instance. */
	THashedMaterialParameterMap<const UMaterialParameterCollection*> ParameterCollectionParameterArray;
...
template <> FORCEINLINE THashedMaterialParameterMap<const UMaterialParameterCollection*>& FMaterialInstanceResource::GetValueArray() { return ParameterCollectionParameterArray; }
...
template <> FORCEINLINE const THashedMaterialParameterMap<const UMaterialParameterCollection*>& FMaterialInstanceResource::GetValueArray() const { return ParameterCollectionParameterArray; }

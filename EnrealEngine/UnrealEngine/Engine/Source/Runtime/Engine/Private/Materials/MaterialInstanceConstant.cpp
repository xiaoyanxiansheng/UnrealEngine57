...
void UMaterialInstanceConstant::GetAssetRegistryTags(FAssetRegistryTagsContext Context) const
{
	Super::GetAssetRegistryTags(Context);

	Context.AddTag(FAssetRegistryTag("HasStaticPermutationResource", bHasStaticPermutationResource ? TEXT("True") : TEXT("False"), FAssetRegistryTag::TT_Alphabetical));
}
...
// Walk up the parent chain as long as they are all UMaterialInstanceConstant.  Gather the CRC32 of all parent ParameterStateId values.
static uint32 GetMICParameterStateIdCRCRecursive(const UMaterialInstanceConstant* MIC)
{
	uint32 CRC = 0;
	if (MIC && !MIC->GetReentrantFlag())
	{
		FMICReentranceGuard Guard(MIC);
		UMaterialInstanceConstant* Next = Cast<UMaterialInstanceConstant>(MIC->Parent);
		CRC = GetMICParameterStateIdCRCRecursive(Next);

		CRC = FCrc::TypeCrc32(MIC->ParameterStateId, CRC);
	}
...
uint32 UMaterialInstanceConstant::ComputeAllStateCRC() const
{
	...
	if (BaseMaterial)
	{
		CRC = FCrc::TypeCrc32(BaseMaterial->StateId, CRC);
	}

	CRC = FCrc::TypeCrc32(GetMICParameterStateIdCRCRecursive(this), CRC);

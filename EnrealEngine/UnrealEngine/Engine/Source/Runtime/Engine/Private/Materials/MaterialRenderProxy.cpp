...
bool FMaterialRenderProxy::GetParameterCollectionValue(const FHashedMaterialParameterInfo& ParameterInfo, const UMaterialParameterCollection** OutValue, const FMaterialRenderContext& Context) const
{
	FMaterialParameterValue Value;
	if (GetParameterValue(EMaterialParameterType::ParameterCollection, ParameterInfo, Value, Context))
	{
		*OutValue = Value.ParameterCollection;
		return true;
	}
	return false;
}

bool FMaterialRenderProxy::GetParameterShaderValue(EMaterialParameterType Type, const FHashedMaterialParameterInfo& ParameterInfo, UE::Shader::FValue& OutValue, const FMaterialRenderContext& Context) const
{
	FMaterialParameterValue Value;
	if (GetParameterValue(Type, ParameterInfo, Value, Context))
	{
		OutValue = Value.AsShaderValue();
		return true;
	}
	return false;
}
...
static void InvalidateMaterialRenderProxyExpressionCache(FMaterialRenderProxy* MaterialProxy)
{
...
static void OnVirtualTextureDestroyedCB(const FVirtualTextureProducerHandle&, void* Baton)
{
	InvalidateMaterialRenderProxyExpressionCache(static_cast<FMaterialRenderProxy*>(Baton));
}

static void OnTagChangedCB(void* Baton)
{
	InvalidateMaterialRenderProxyExpressionCache(static_cast<FMaterialRenderProxy*>(Baton));
}
...
void FMaterialRenderProxy::EvaluateUniformExpressions(FRHICommandListBase& RHICmdList, FUniformExpressionCache& OutUniformExpressionCache, const FMaterialRenderContext& Context, FUniformExpressionCacheAsyncUpdater* Updater) const
{
	...
	for (int32 TagIndex = 0; TagIndex < UniformExpressionSet.MaterialCacheTagStacks.Num(); ++TagIndex)
	{
		const FMaterialCacheTagStack& Stack = UniformExpressionSet.MaterialCacheTagStacks[TagIndex];

		// Subscribe to tag changes which require rebinding
		IMaterialCacheTagProvider* TagProvider = GetRendererModule().GetMaterialCacheTagProvider();
		TagProvider->AddTagSceneInvalidationCallback(Stack.TagGuid, &OnTagChangedCB, const_cast<FMaterialRenderProxy*>(this));
		bHasMaterialCacheCallbacks = true;
	}

	for (int32 VTCIndex = 0; VTCIndex < UniformExpressionSet.UniformTextureCollectionParameters.Num(); ++VTCIndex)
	{
		const FMaterialTextureCollectionParameterInfo& Parameter = UniformExpressionSet.UniformTextureCollectionParameters[VTCIndex];
		if (!Parameter.bIsVirtualCollection)
		{
			continue;
		}

		const UTextureCollection* Texture = nullptr;
		UniformExpressionSet.GetTextureCollectionValue(VTCIndex, Context, Context.Material, Texture);

		// May not be bound, but must be a VTC
		if (!Texture || !ensure(Texture->IsVirtualCollection()))
		{
			continue;
		}

		// May not have a valid resource
		FVirtualTextureCollectionResource* Resource = static_cast<FVirtualTextureCollectionResource*>(Texture->GetResource());
		if (!Resource || !ensure(!Resource->bIsBindless))
		{
			continue;
		}

		// Subscribe to VT destruction
		GetRendererModule().AddVirtualTextureProducerDestroyedCallback(Resource->GetProducerHandle(), &OnVirtualTextureDestroyedCB, const_cast<FMaterialRenderProxy*>(this));
		bHasVirtualTextureCallbacks = true;
	}
	...
	EvaluateParameterCollections(RHICmdList, OutUniformExpressionCache, Context, Updater);
...
void FMaterialRenderProxy::EvaluateParameterCollections(FRHICommandListBase& RHICmdList, FUniformExpressionCache& OutUniformExpressionCache, const FMaterialRenderContext& Context, FUniformExpressionCacheAsyncUpdater* Updater) const
{
	// Retrieve the material's uniform expression set.
	FMaterialShaderMap* ShaderMap = Context.Material.GetRenderingThreadShaderMap();
	const FUniformExpressionSet& UniformExpressionSet = ShaderMap->GetUniformExpressionSet();
...
void FMaterialRenderProxy::InvalidateUniformExpressionCache(bool bRecreateUniformBuffer)
{
	...
	if (bHasMaterialCacheCallbacks)
	{
		IMaterialCacheTagProvider* TagProvider = GetRendererModule().GetMaterialCacheTagProvider();
		TagProvider->RemoveTagSceneInvalidationCallbacks(this);
		bHasMaterialCacheCallbacks = false;
	}

...
void UPrimitiveComponent::SetPhysicsLinearVelocity(FVector NewVel, bool bAddToCurrent, FName BoneName)
{
	...
	else if (BoneName == NAME_None)
	{
		...
		Interface->SetLinearImpulseVelocity(PhysicsObjects, NewVel, bAddToCurrent);
	}
	else if (Chaos::FPhysicsObject* Object = GetPhysicsObjectByName(BoneName))
	{
		FLockedWritePhysicsObjectExternalInterface Interface = FPhysicsObjectExternalInterface::LockWrite({ &Object, 1 });
		Interface->SetLinearImpulseVelocity({ &Object, 1 }, NewVel, bAddToCurrent);
	}
}
...
void UPrimitiveComponent::SetPhysicsAngularVelocityInRadians(FVector NewAngVel, bool bAddToCurrent, FName BoneName)
{
	...
	else if (BoneName == NAME_None)
	{
		...
		Interface->SetAngularVelocityInRadians(PhysicsObjects, NewAngVel, bAddToCurrent);
	}
	else if (Chaos::FPhysicsObject* Object = GetPhysicsObjectByName(BoneName))
	{
		FLockedWritePhysicsObjectExternalInterface Interface = FPhysicsObjectExternalInterface::LockWrite({ &Object, 1 });
		Interface->SetAngularVelocityInRadians({ &Object, 1 }, NewAngVel, bAddToCurrent);
	}
}
...
void UPrimitiveComponent::SetAllowPartialIslandSleep(bool bInAllowPartialIslandSleep /* = true */, FName BoneName /* = NAME_None */)
{
	FBodyInstance* BI = GetBodyInstance(BoneName);
	if (BI)
	{
		BI->SetAllowPartialIslandSleep(bInAllowPartialIslandSleep);
	}
}

void UPrimitiveComponent::SetAllAllowPartialIslandSleep(bool bInAllowPartialIslandSleep /* = true */)
{
	SetAllowPartialIslandSleep(bInAllowPartialIslandSleep, NAME_None);
}
...
void GetRootBodyAndBoneName(const UPrimitiveComponent* RootComponent, const FName& SocketName, FBodyInstance*& OutRootBI, FName& OutBoneName)
{
	...
	{
		const USkeletalMeshComponent* RootSkeletal = Cast<USkeletalMeshComponent>(RootComponent);
...
bool UPrimitiveComponent::WeldToImplementation(USceneComponent * InParent, FName ParentSocketName /* = Name_None */, bool bWeldSimulatedChild /* = true */, bool bWeldToKinematicParent /* = false */)
{
	...
	if (RootComponent)
	{
		...
		FName BoneName;
		GetRootBodyAndBoneName(RootComponent, SocketName, RootBI, BoneName);

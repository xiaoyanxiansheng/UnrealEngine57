	...
	if (!PlayerState->IsFromPreviousLevel() && !MustSpectate(PC) && !LocalWorld->bIsTearingDown)
	{
		...
		if (NewPlayerState)
		{
			...
			for (int32 Idx = 0; Idx < InactivePlayerArray.Num(); ++Idx)
			{
				...
				{
					bool bDestroyPlayerState = false;
					if (!bUseUniqueIdCheck)
					{
						if (bHasValidNetworkAddress && CurrentPlayerState->SavedNetworkAddress == NewPlayerState->SavedNetworkAddress)
						{
							bDestroyPlayerState = true;
						}
					}
					else if (CurrentPlayerState->GetUniqueId() == NewPlayerState->GetUniqueId())
					{
						bDestroyPlayerState = true;
					}

					if (bDestroyPlayerState)
					{

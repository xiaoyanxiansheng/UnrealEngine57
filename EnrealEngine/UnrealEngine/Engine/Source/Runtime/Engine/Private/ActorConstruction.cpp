...
void AActor::DestroyConstructedComponents()
{
	...
	// When a constructed component is destroyed, it is removed from this set. We rebuild it from the original set to compact
	// the set and ensure that the iteration order remains stable after re-running actor construction scripts.
	if (PreviouslyAttachedComponents.Num() > OwnedComponents.Num())
	{
		TSet<TObjectPtr<UActorComponent>> OldComponents = MoveTemp(OwnedComponents);
		OwnedComponents.Reserve(OldComponents.Num());
		for (UActorComponent* Component : PreviouslyAttachedComponents)
		{
...
void AActor::RerunConstructionScripts()
{
	...
	if (bAllowReconstruction)
	{
		...
		// NotifyToolsOfObjectReplacement works only with UEditorEngine
		// OnComponentsReplaced delegate broadcasts in standalone editor as well
		if (OldToNewComponentMapping.Num() > 0)
		{
			OnComponentsReplaced.Broadcast(OldToNewComponentMapping);
		}

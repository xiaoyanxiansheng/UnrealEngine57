...
{
	FRealCurve* CurveToDelete = nullptr;
	if (RowMap.RemoveAndCopyValue(CurveName, CurveToDelete))
	{
...
void UCurveTable::GetCurves(TAdderReserverRef<FRichCurveEditInfoConst> Curves) const
{
	for (auto Iter = RowMap.CreateConstIterator(); Iter; ++Iter)
	{
		Curves.Add(FRichCurveEditInfoConst(Iter.Value(), Iter.Key()));

...
#include UE_INLINE_GENERATED_CPP_BY_NAME(SGameLayerManager)
...
void SGameLayerManager::ClearWidgets()
{
	...
	for (const TWeakPtr<SWidget>& GameLayerWeak : GameLayers)
	{
		if (const TSharedPtr<SWidget>& GameLayer = GameLayerWeak.Pin())
		{
			WindowOverlay->RemoveSlot(GameLayer.ToSharedRef());
		}
	}
...
{
	GameLayers.Add(ViewportContent);
...
void SGameLayerManager::AddOrUpdatePlayerLayers(const FGeometry& AllottedGeometry, UGameViewportClient* ViewportClient, const TArray<ULocalPlayer*>& GamePlayers)
{
	...
	for ( int32 PlayerIndex = 0; PlayerIndex < GamePlayers.Num(); PlayerIndex++ )
	{
		...
		if ( SplitType < SplitInfo.Num() )
		{
			...
			TSharedPtr<FPlayerLayer> PlayerLayer = FindOrCreatePlayerLayer(Player);
			if (PlayerIndex < SplitInfo[SplitType].PlayerData.Num())
			{
				FPerPlayerSplitscreenData& SplitData = SplitInfo[SplitType].PlayerData[PlayerIndex];
				Size = FVector2D(SplitData.SizeX, SplitData.SizeY);
			}
			else if (SplitType == ESplitScreenType::None && GetDefault<UGameMapsSettings>()->bShowAllPlayerWidgetsWhenSplitscreenDisabled )
			{
				// If splitscreen is disabled, we should overlay all of the player layers in the viewport
				FPerPlayerSplitscreenData& SplitData = SplitInfo[SplitType].PlayerData[0];
				Size = FVector2D(SplitData.SizeX, SplitData.SizeY);
			}		

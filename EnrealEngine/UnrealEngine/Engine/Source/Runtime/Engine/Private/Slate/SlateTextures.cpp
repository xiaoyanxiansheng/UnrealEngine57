...
const FName NAME_FSlateTexture2DRHIRef(TEXT("FSlateTexture2DRHIRef"));
...
void FSlateTexture2DRHIRef::InitRHI(FRHICommandListBase& RHICmdList)
{
	...
	if( Width > 0 && Height > 0 )
	{
		const bool bValid = TextureData.IsValid();
		const bool bHasUploadData = TextureData.IsValid() && TextureData->GetRawBytes().Num() > 0;

		if (bValid || bHasUploadData || bCreateEmptyTexture)
		{
			...
			FRHITextureCreateDesc CreateDesc =
				...
				.SetClassName(NAME_FSlateTexture2DRHIRef);

			if (bHasUploadData)
			{
				CreateDesc.SetInitActionInitializer();
			}
			...
			if (bHasUploadData)
			{
				...
				const uint32 SourceDataStride = BlocksX * GPixelFormats[PixelFormat].BlockBytes;

				checkf((uint32)TextureData->GetRawBytes().Num() >= SourceDataStride * BlocksY, TEXT("Not enough bytes in source TextureData to complete copy operation"));

				if (Subresource.Stride == SourceDataStride)
				{
					FMemory::Memcpy(Subresource.Data, SourceTextureData, Subresource.Stride * BlocksY);
				}
				else
				{
					uint8* WritePointer = reinterpret_cast<uint8*>(Subresource.Data);

					checkf(SourceDataStride < Subresource.Stride, TEXT("Texture stride of %u is smaller than source data stride of %u, PixelFormat=%s (%d)"), Subresource.Stride, SourceDataStride, GPixelFormats[PixelFormat].Name, (int32)PixelFormat);
					for (uint32 i = 0; i < BlocksY; i++)
					{
						FMemory::Memcpy(WritePointer, SourceTextureData, SourceDataStride);
						WritePointer += Subresource.Stride;
						SourceTextureData += SourceDataStride;
					}
				}
				TextureData->Empty();
			}

			ShaderResource = Initializer.Finalize();
			check(IsValidRef(ShaderResource));
...
void FSlateTextureRenderTarget2DResource::InitRHI(FRHICommandListBase& RHICmdList)
{

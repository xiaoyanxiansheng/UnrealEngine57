...
{
	UE::TWriteScopeLock Lock(EntryRWLock);
...
FVisualLogEntry* FVisualLogger::GetEntryToWrite(const UObject* Object, const double TimeStamp, ECreateIfNeeded ShouldCreate)
{
	...
	else
	{
		...
		{
			UE::TReadScopeLock Lock(EntryRWLock);
...
FVisualLogEntry* FVisualLogger::GetEntryToWriteInternal(const UObject* Object, const double TimeStamp, const ECreateIfNeeded ShouldCreate)
{
	...
	// We were previously using UE_MT_SCOPED_WRITE_ACCESS(CurrentEntryPerObject.EntriesAccessDetector) here
	// but Transactionally safe detectors are not reentrant so it is now the responsibility of the caller.
	...
	if (bInitializeEntry)
	{
		...
		else if (const UActorComponent* ObjectAsActorComponent = Cast<UActorComponent>(LogOwner))
		{
			if (const USceneComponent* ObjectAsSceneComponent = Cast<USceneComponent>(ObjectAsActorComponent))
			{
				CurrentEntry->Location = ObjectAsSceneComponent->GetComponentLocation();
				CurrentEntry->bIsLocationValid = true;
			}
			else if (const AActor* ComponentOwner = ObjectAsActorComponent->GetOwner())
			{
				CurrentEntry->Location = ComponentOwner->GetActorLocation();
				CurrentEntry->bIsLocationValid = true;
			}
		}

...
namespace UE::Gameplay::CVars
{
	...
	bool bSortInputStackByPriority = true;
	static FAutoConsoleVariableRef CVarSortInputStackByPriority(
		TEXT("PlayerController.Input.SortInputStackByPriority"), 
		bSortInputStackByPriority,
		TEXT("If true, APlayerController's input component stack will be sorted by priority after being built. False will maintain 'legacy' ordering."),
		ECVF_Default);
...
FString APlayerController::ConsoleCommand(const FString& Cmd, bool bWriteToLog)
{
	if (Player != nullptr)
	{
		...
		if (!AutoRTFM::IsClosed())
		{
			UE_AUTORTFM_OPEN
			{
				static FName TransactionalWorkName(TEXT("APlyerController::ConsoleComand"));
				UE::RemoteExecutor::ExecuteTransactional(TransactionalWorkName, [this, &Result, Cmd, bWriteToLog]()
				{
					Result = Player->ConsoleCommand(Cmd, bWriteToLog); 
				});
			};
		}
		else
...
void APlayerController::OnActorChannelOpen(FInBunch& InBunch, UNetConnection* Connection)
{
	...
	if (InBunch.EngineNetVer() >= FEngineNetworkCustomVersion::CloseChildConnection)
	{
		InBunch << LocalPlayerConnectionIdentifier;
	}
	...
	if (Connection->Driver != NULL && Connection == Connection->Driver->ServerConnection)
	{
		...
		{
			// Only create a new child connection if there isn't alreayd one associated with a NetPlayerIndex.
			UChildConnection* ChildConnection = Connection->GetChildConnectionWithNetPlayerIndex(NetPlayerIndex);
			if (!ChildConnection)
			{
				ChildConnection = Connection->CreateChildWithNetPlayerIndex(NetPlayerIndex);
			}

			if (ensure(ChildConnection))
			{
				UE_LOG(LogNet, Log, TEXT("[%s:%s] Binding player controller %s to child connection %s"), 
					   *GetNameSafe(Connection->GetDriver()), *GetNameSafe(Connection), *GetName(), *GetNameSafe(ChildConnection));

				ChildConnection->HandleClientPlayer(this, Connection);
			}
		}
	}
}
...
void APlayerController::OnSerializeNewActor(FOutBunch& OutBunch)
{
	...
	OutBunch << LocalPlayerConnectionIdentifier;
}
...
void APlayerController::BuildInputStack(TArray<UInputComponent*>& InputStack)
{
	...
	if (ControlledPawn)
	{
		if (ControlledPawn->InputEnabled())
		{
			...
			for (UActorComponent* ActorComponent : ControlledPawn->GetComponents())
			{
				...
				// Do not add this input component it is already contained on the "CurrentInputStack". Input components which are on the CurrentInputStack
				// already, will have been specifically added via the PushInputComponent function already. Adding them again here would result in them being
				// processed twice
				if (PawnInputComponent && PawnInputComponent != ControlledPawn->InputComponent && !CurrentInputStack.Contains(PawnInputComponent))
				{
	...
	// We need to sort the input stack in order to respect the priority of each input component.
	// This way, all input components and their priorities will have their order respected. Not just those
	// manually added via "PushInputComponent"
	if (UE::Gameplay::CVars::bSortInputStackByPriority)
	{
		Algo::StableSort(InputStack, [](const UInputComponent* A, const UInputComponent* B)
			{
...
void APlayerController::PostMigrate(const FUObjectMigrationContext& MigrationContext)
{
	...
	auto GetNetPlayerIndex = [](UNetConnection* Connection)
	{
		if (UChildConnection* ChildConnection = Cast<UChildConnection>(Connection))
		{
			if (UNetConnection* ParentConnection = ChildConnection->GetParentConnection())
			{
				...
				const bool bFoundNetPlayerIndex = ParentConnection->GetNetPlayerIndex(ChildConnection, &NetPlayerIndex);
				ensure(bFoundNetPlayerIndex);
	...
	if (MigrationContext.MigrationSide == EObjectMigrationSide::Receive)
	{
		...
		UE_LOG(LogPlayerController, Verbose, TEXT("[%s] APlayerController::PostMigrate [%s]: %s bHasOwnership=%d MigrationSide=Receive RecvType=%s PlayerId=%s"), 
			   *FRemoteServerId::GetLocalServerId().ToString(), *FRemoteObjectId(this).ToString(), *GetName(), bHasOwnership, ToString(MigrationRecvType), *CachedConnectionPlayerId.ToDebugString());
		...
		if (bAttachControllerToConnection)
		{
			...
			if (ensure(NetConnection))
			{
				...
				// The ANoPawnPlayerController actor has a low NetUpdateFrequency, so ForceNetUpdate() must be called because
				// it's going to be detached from a connection.
				NetConnection->PlayerController->ForceNetUpdate();
				
				// Detach the previous player controller from the network connection so that any attempts to send an RPC
				// on that controller to the client will not succeed.
				NetConnection->PlayerController->NetConnection = nullptr;
				...
				// Destroy the previous player controller used by the connection.
				UE_LOG(LogPlayerController, Verbose, TEXT("[%s] APLayerController::PostMigrate [%s]: Destroying previous controller %s"), 
					   *FRemoteServerId::GetLocalServerId().ToString(), *FRemoteObjectId(this).ToString(), *GetNameSafe(NetConnection->PlayerController));
				GetWorld()->RemoveController(NetConnection->PlayerController);
				GetWorld()->DestroyActor(NetConnection->PlayerController, true);
				...
				UE_LOG(LogPlayerController, Verbose, TEXT("[%s] APlayerController::PostMigrate [%s]: Binding %s to connection %s"), 
					   *FRemoteServerId::GetLocalServerId().ToString(), *FRemoteObjectId(this).ToString(), 
	...
	else if (MigrationContext.MigrationSide == EObjectMigrationSide::Send)
	{
		...
		UE_LOG(LogPlayerController, Verbose, TEXT("[%s] APlayerController::PostMigrate [%s]: %s bHasOwnership=%d MigrationSide=Send SendType=%s PlayerId=%s"), 
			   *FRemoteServerId::GetLocalServerId().ToString(), *FRemoteObjectId(this).ToString(), *GetName(), bHasOwnership, ToString(MigrationSendType), *CachedConnectionPlayerId.ToDebugString());
		...
		if (bDetachControllerFromConnection)
		{
			...
			if (ensure(PrevNetConnection))
			{
				...
				// Create a new instance of ANoPawnPlayerController for the connection to use while the original instance of APlayerController is migrated
				// on another server. This controller will be destroyed when it's replaced by APlayerController when it returns to the server.
				FActorSpawnParameters SpawnInfo;
				...
				UE_LOG(LogPlayerController, Verbose, TEXT("[%s] APlayerController::PostMigrate [%s]: Replacing %s with new player controller %s on connection %s"), 
					*FRemoteServerId::GetLocalServerId().ToString(), *FRemoteObjectId(this).ToString(), 
		...
		{
			UE_LOG(LogPlayerController, Verbose, TEXT("[%s] APlayerController::PostMigrate [%s]: Removing %s from the world"), 
				   *FRemoteServerId::GetLocalServerId().ToString(), *FRemoteObjectId(this).ToString(), *GetName());
					...
					if (ServerData->ServerTimeStamp != 0.f)
					{
						...
						if (ServerData->bTriggeringForcedUpdates)
						{
							if (ServerData->ServerTimeStamp > ServerData->ServerTimeLastForcedUpdate)
							{
								...
								UE_LOG(LogNetPlayerMovement, Log, TEXT("Movement detected, resetting forced update state (ServerTimeStamp %.6f > ServerTimeLastForcedUpdate %.6f) Pawn: %s"), ServerData->ServerTimeStamp, ServerData->ServerTimeLastForcedUpdate, *GetNameSafe(GetPawn()));
							...
							else
							{
								...
								if (PawnTimeSinceForcingUpdates > PawnTimeForcedUpdateMaxDuration)
								{
									...
									UE_LOG(LogNetPlayerMovement, Log, TEXT("Setting bForcedUpdateDurationExceeded=true (PawnTimeSinceForcingUpdates %.6f > PawnTimeForcedUpdateMaxDuration %.6f) (bLastRequestNeedsForcedUpdates:%d) Pawn: %s"), PawnTimeSinceForcingUpdates, PawnTimeForcedUpdateMaxDuration, (int32)ServerData->bLastRequestNeedsForcedUpdates, *GetNameSafe(GetPawn()));
...
void APlayerController::OnReplicationStartedForIris(const FOnReplicationStartedParams& Params)
{
...
FAsyncPhysicsTimestamp APlayerController::GetPhysicsTimestamp()
{
	...
	if (UWorld* World = GetWorld())
	{
		if (FPhysScene* PhysScene = World->GetPhysicsScene())
		{
			if (FPhysicsSolverBase* Solver = static_cast<FPhysicsSolverBase*>(PhysScene->GetSolver()))
			{
				Timestamp.LocalFrame = Solver->GetMarshallingManager().GetInternalStep_External();
				Timestamp.ServerFrame = Timestamp.LocalFrame + NetworkPhysicsTickOffset; // On the server NetworkPhysicsTickOffset is always 0 so it's safe to always add on both client and server to convert LocalFrame to ServerFrame
			}
		}
	}
...
void APlayerController::SetLocalPlayerConnectionIdentifier(int32 Identifier)
{
	LocalPlayerConnectionIdentifier = Identifier;
}

int32 APlayerController::GetLocalPlayerConnectionIdentifier() const
{
	return LocalPlayerConnectionIdentifier;
}

ANoPawnPlayerController::ANoPawnPlayerController(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	bReplicates = true;
	...
	// This controller's updates don't need to be replicated to the client very frequently because
	// it's only intended to be used by the server for replication, relevancy and establishing the
	// initial connection on the client.
	//
	// This low update frequency means that ForceNetUpdate() must be called when any explicit changes
	// are made to the controller, including being attached or detached from a UNetConnection.
	SetNetUpdateFrequency(0.1f);
}

void ANoPawnPlayerController::PostInitializeComponents()
{
	// Don't call APlayerController::PostInitializeComponents() because it will spawn an instance of APlayerState
	// which doesn't make sense in ANoPawnPlayerController because it doesn't have an associated pawn.

	AController::PostInitializeComponents();
}

AActor* ANoPawnPlayerController::GetViewTarget() const
{
	return const_cast<ANoPawnPlayerController*>(this);
}

class UNetConnection* ANoPawnPlayerController::GetNetConnection() const
{
...
void ANoPawnPlayerController::ServerEnableReplicationToConnection_Implementation(bool bEnable)
{
	UE_LOG(LogNet, Log, TEXT("%s replication for connections using %s (previously %s)."),
		   bEnable ? TEXT("Enabled") : TEXT("Disabled"),
		   *GetName(),
		   bEnableReplicationToConnection ? TEXT("enabled") : TEXT("disabled"));

	bEnableReplicationToConnection = bEnable;
}

			...
			PlatformIni->GetSection(*Section, CVarData, GScalabilityIni);
			{
				...
				for (int32 DataIndex = 0; DataIndex < CVarData.Num(); ++DataIndex)
				{
					...
					if (CVarString.Split(TEXT("="), &CVarName, &CVarValue))
					{
						...
						{
							EConsoleVariableFlags MaxPriority = ECVF_SetByPreview;
							EConsoleVariableFlags MinPriority = ECVF_SetByScalability;
							CVar->ReplaceCurrentPriorityAndTag(*CVarValue, MaxPriority, MinPriority);
						}
					}
				}
			}
...
{
	SetQualityLevels(GetDefaultQualityLevels());
}
#endif
...
#if WITH_EDITOR
	else if (GEditor && GEditor->IsFeatureLevelPreviewActive())
	{
		EConsoleVariableFlags MaxPriority = ECVF_SetByPreview;
		EConsoleVariableFlags MinPriority = ECVF_SetByScalability;
		CVarToEdit.AsVariable()->ReplaceCurrentPriorityAndTag(DesiredValue, MaxPriority, MinPriority);
	}
#endif
	else
	{
		...
		if (DesiredValue != PreviousValue || !bIsInitialized)
		{
...
void SetQualityLevels(const FQualityLevels& QualityLevels, bool bForce/* = false*/)
{
	...
	if (GetQualityLevels() != ClampedLevels || bForce || !bIsInitialized)
	{

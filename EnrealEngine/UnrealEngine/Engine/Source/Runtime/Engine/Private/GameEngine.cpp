...
void UGameEngine::RedrawViewports( bool bShouldPresent /*= true*/ )
{
	...
	if ( GameViewport != NULL )
	{
		...
		{
			// Make sure we don't attempt to render a frame in parallel with the loading screen, which might deadlock because of interleaving two different frames.
			// It could happen because the Game thread invokes RedrawViewports right after blocking loads when the loading thread is still spinning.
...
void UGameEngine::Tick( float DeltaSeconds, bool bIdleMode )
{
	...
	for (FWorldContext& Context : WorldList)
	{
		if (Context.World())
		{
			Context.World()->StartAsyncSendAllEndOfFrameUpdates();
		}
	}
	...
	for (FWorldContext& Context : WorldList)
	{
		if (Context.World())
		{
			Context.World()->FinishAsyncSendAllEndOfFrameUpdates();
		}
	}

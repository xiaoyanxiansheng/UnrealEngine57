...
void DrawDebugCircle(const UWorld* InWorld, const FMatrix& TransformMatrix, double Radius, int32 Segments, const FColor& Color, bool bPersistentLines, float LifeTime, uint8 DepthPriority, float Thickness, bool bDrawAxis)
{
	RETURN_IF_DEBUG_DRAW_DISABLED;
...
void DrawDebugCircle(const UWorld* InWorld, FVector Center, double Radius, int32 Segments, const FColor& Color, bool PersistentLines, float LifeTime, uint8 DepthPriority, float Thickness, FVector YAxis, FVector ZAxis, bool bDrawAxis)
{
	RETURN_IF_DEBUG_DRAW_DISABLED;
...
void DrawDebugCircleArc(const UWorld* InWorld, const FVector& Center, float Radius, const FQuat& Rotation, float AngleWidth, int32 Segments, const FColor& Color, bool PersistentLines, float LifeTime, uint8 DepthPriority, float Thickness)
{
	RETURN_IF_DEBUG_DRAW_DISABLED;
	...
	if (GEngine->GetNetMode(InWorld) != NM_DedicatedServer)
	{
		if (ULineBatchComponent* const LineBatcher = GetDebugLineBatcher(InWorld, PersistentLines, LifeTime, (DepthPriority == SDPG_Foreground)))
		{
			...
			// Need at least 4 segments
			Segments = FMath::Max(Segments, 4);
			const float AngleStep = AngleWidth / static_cast<float>(Segments) * 2.f;

			const FVector AxisX = Rotation.Vector();
			...
			FVector PrevVertex = Center + Radius * (AxisX * FMath::Cos(Angle) + AxisY * FMath::Sin(Angle));
			
			while (Segments--)
			{
				Angle += AngleStep;
				const FVector NextVertex = Center + Radius * (AxisX * FMath::Cos(Angle) + AxisY * FMath::Sin(Angle));
				Lines.Emplace(FBatchedLine(PrevVertex, NextVertex, Color, LineLifeTime, Thickness, DepthPriority));
				PrevVertex = NextVertex;
			}
	...
	{
		UE_DRAW_SERVER_DEBUG_ON_EACH_CLIENT(DrawDebugCircleArc, Center, Radius, Rotation, AngleWidth, Segments, AdjustColorForServer(Color), PersistentLines, LifeTime, DepthPriority, Thickness);
	}
}
...
void DrawCircle(const UWorld* InWorld, const FVector& Base, const FVector& X, const FVector& Y, const FColor& Color, double Radius, int32 NumSides, bool bPersistentLines, float LifeTime, uint8 DepthPriority, float Thickness)
{
	RETURN_IF_DEBUG_DRAW_DISABLED;

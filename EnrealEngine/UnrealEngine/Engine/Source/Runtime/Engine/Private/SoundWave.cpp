...
#include UE_INLINE_GENERATED_CPP_BY_NAME(SoundWave)
...
UE_TRACE_EVENT_BEGIN(Audio, SoundWaveIsAlivePing)
	UE_TRACE_EVENT_FIELD(uint32, DeviceId)
	UE_TRACE_EVENT_FIELD(uint64, Timestamp)
	UE_TRACE_EVENT_FIELD(uint32, PlayOrder)
	UE_TRACE_EVENT_FIELD(uint32, ActiveSoundPlayOrder)
...
#endif
{
	...
	CuePointOrigin = ESoundWaveCuePointOrigin::MarkerTransformation;
#endif
...
void USoundWave::Serialize( FArchive& Ar )
{
	...
	if (Ar.IsLoading() && FPlatformProperties::SupportsAudioStreaming() && !IsRunningDedicatedServer())
	{
...
const ESoundWaveCuePointOrigin USoundWave::GetCuePointOrigin() const
{
	return CuePointOrigin;
}

void USoundWave::SetCuePointOrigin(const ESoundWaveCuePointOrigin InCuePointOrigin)
{
	CuePointOrigin = InCuePointOrigin;
}

void USoundWave::SetLoudnessValues(const float InLUFS, const float InSamplePeakDB)
{
	LUFS = InLUFS;
	SamplePeakDB = InSamplePeakDB;
}
#endif //WITH_EDITOR
...
bool USoundWave::GetImportedSoundWaveData(TArray<uint8>& OutRawPCMData, uint32& OutSampleRate, TArray<EAudioSpeakers>& OutChannelOrder) const
{
	...
	if (RawData.HasPayloadData())
	{
		...
		if (NumChannels > 2)
		{
			...
			TFunction<bool(const FWaveModInfo& WaveModInfo)> ValidateWaveModInfo = [](const FWaveModInfo& WaveModInfo)
			{
				// Check for valid channel count
				if (1 != *WaveModInfo.pChannels)
				{
					UE_LOG(LogAudio, Warning, TEXT("Cannot handle audio format. Expected single channel audio but read %d channels"), *WaveModInfo.pChannels);
					
					return false;
				}

				// Check for valid bit depth
				if (16 != *WaveModInfo.pBitsPerSample)
				{
					UE_LOG(LogAudio, Warning, TEXT("Cannot handle audio format. Expected 16bit audio but found %d bit audio"), *WaveModInfo.pBitsPerSample);
			...
			// Determine which channel is the smallest and read the wave for the number of frames so we can truncate larger channels if needed
			int32 SmallestValidChannelIndex = INDEX_NONE;
			int32 SmallestChannelSize = INT32_MAX;

			for (int32 Index = 0; Index < ChannelOffsets.Num(); Index++)
			{
				if (ChannelSizes[Index] > 0 && ChannelSizes[Index] < SmallestChannelSize)
				{
					SmallestValidChannelIndex = Index;
					SmallestChannelSize = ChannelSizes[Index];
				}
			}
			
			if (SmallestValidChannelIndex == INDEX_NONE)
			{
			...
			FWaveModInfo SmallestChannelWaveInfo;
			if (!SmallestChannelWaveInfo.ReadWaveHeader(Data, ChannelSizes[SmallestValidChannelIndex], ChannelOffsets[SmallestValidChannelIndex]))
			{
				UE_LOG(LogAudio, Warning, TEXT("Failed to read wave data: %s."), *GetFullName());
			...
			if (!ValidateWaveModInfo(SmallestChannelWaveInfo))
			{
				return false;
			}

			check(SmallestChannelWaveInfo.SampleDataSize)

			// The largest number of frames a channel should have in this soundwave
			// Bad chunk data can lead to NumFramesPerChannel being a decimal value but the data truncation to int32 removes the bad sample.
			int32 NumFramesPerChannel = SmallestChannelWaveInfo.SampleDataSize / sizeof(int16);
			int32 NumSamples = NumFramesPerChannel * NumChannels; // Number of total samples across all channels
			OutSampleRate = *SmallestChannelWaveInfo.pSamplesPerSec;
			...
			{
				UE_LOG(LogAudio, Warning, TEXT("Zero samples found in wave data: %s."), *GetFullName());
			...
			for (int32 ChannelIndex = 0; ChannelIndex < ChannelOffsets.Num(); ChannelIndex++)
			{
				...
				if (!ValidateWaveModInfo(WaveInfo))
				{
					return false;
				}
				...
				TArrayView<const int16> ChannelArrayView;
				// If not all channels have the same number of frames we will have to copy the channel data and alter it
				TArray<int16> ChannelArrayCopy;

				if (ThisChannelNumFrames == NumFramesPerChannel)
				{
					ChannelArrayView = TArrayView<const int16>(reinterpret_cast<const int16*>(WaveInfo.SampleDataStart), NumFramesPerChannel);
				}
				else // Adjust if not all channels have the same number of frames
				{
					UE_LOG(LogAudio, Warning, TEXT("Audio channels contain varying number of frames (%d vs %d). Truncating frames."), NumFramesPerChannel, ThisChannelNumFrames);
					check(NumFramesPerChannel < ThisChannelNumFrames);

					// Copy the data instead of referencing it directly so we don't modify the imported WaveInfo
					ChannelArrayCopy.Append(reinterpret_cast<const int16*>(WaveInfo.SampleDataStart), ThisChannelNumFrames);

					constexpr EAllowShrinking AllowShrinking = EAllowShrinking::Yes;
					ChannelArrayCopy.SetNum(NumFramesPerChannel, AllowShrinking); //Truncate 

					ChannelArrayView = TArrayView<const int16>(ChannelArrayCopy);
				}

				// Interleave each channel's data into one multichannel audio buffer
				TArrayView<int16> OutPCM(reinterpret_cast<int16*>(OutRawPCMData.GetData()), NumSamples);

				check(OutChannelOrder.Num() >= 1);
		...
		else
		{
			...
			check(NumChannels > 0);
			const bool InvalidDataSize = (WaveInfo.SampleDataSize % sizeof(int16) != 0);
			uint32 NumSamples = WaveInfo.SampleDataSize / sizeof(int16);

			if (InvalidDataSize || NumSamples % NumChannels != 0)
			{
				UE_LOG(LogAudio, Warning, TEXT("Invalid Sample Data Size compared to NumChannels for: %s. Adjusting sample count"), *GetFullName());

				NumSamples -= NumSamples % NumChannels;
			}

			// NumSamples * sizeof(int16) can be different from WaveInfo.DataSize if the file contains bad data
			const uint32 SampleDataSize = NumSamples * sizeof(int16);

			// Copy the raw PCM data and the header info that was parsed
			OutRawPCMData.AddUninitialized(SampleDataSize);
			FMemory::Memcpy(OutRawPCMData.GetData(), WaveInfo.SampleDataStart, SampleDataSize);
...
TArray<FSoundWaveCuePoint> USoundWave::GetCuePointsScaledForSampleRate(const float InSampleRate) const
{
	...
	case ESoundWaveCuePointOrigin::MarkerTransformation:
		CuePointsScaled = GetWaveformTransformationInfo().AllCuePointsRelativeToStartTime;
		break;
	default:
		CuePointsScaled = CuePoints;
		break;
...
void USoundWave::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	{
		// An empty event property field might mean the update comes from an undo or an import.
		// We are making an educated wish that an import task would still have async tasks running
		// when we get to the PostEditChange and an undo would not. This will help prevent us
		// from updating the asset twice on import, blocking the UI for the previous Update task
		// to complete. 
	...
	if (FProperty* PropertyThatChanged = PropertyChangedEvent.Property)
	{
		...
		else if (Name == AssetUserDataFName && !AssetUserData.IsEmpty())
		{
			...
			if (IsValid(LastModifiedUserData))
			{
				...
				if (IsValid(UserDataAsSubtitle) && !UserDataAsSubtitle->Subtitles.IsEmpty())
				{
					FSubtitleAssetData& Subtitle = UserDataAsSubtitle->Subtitles.Last();
					if(Subtitle.Duration == SubtitleDefaultDuration && Subtitle.Text.IsEmpty())
					{
...
void USoundWave::Parse(FAudioDevice* AudioDevice, const UPTRINT NodeWaveInstanceHash, FActiveSound& ActiveSound, const FSoundParseParameters& ParseParams, TArray<FWaveInstance*>& WaveInstances)
{
	...
	// Override the start time for seek restart virtualization mode
	if (ActiveSound.SeekRestartTimes.Contains(WaveInstance->WaveInstanceHash))
	{
		float SeekRestartTime = ActiveSound.SeekRestartTimes[WaveInstance->WaveInstanceHash];
		// For sounds with a defined duration, we wrap the seek time so that it sounds like it properly looped.
		// Procedural sounds will get the unwrapped time, which they can use how they see fit.
		const bool bWrapSeekTime = !bProcedural && (ParseParams.bLooping || IsLooping());
		if (bWrapSeekTime)
		{
			WaveInstance->StartTime = FMath::Fmod(SeekRestartTime, Duration);
		}
		else
		{
			WaveInstance->StartTime = SeekRestartTime;
		}
	}
	...
	else //if this is a preview sound, ignore Submix sends
	{
		...
		if (!WaveInstance->ActiveSound->bIsDryPreviewPlayback)
		{
#endif //WITH_EDITORONLY_DATA
			WaveInstance->bEnableSubmixSends = ParseParams.bEnableSubmixSends;
			WaveInstance->bEnableBaseSubmix = ParseParams.bEnableBaseSubmix;
	...
	if (bChannelEnabled && bWaveIsActive && WaveInstance->SoundClass != nullptr)
	{
		if (bIsNewWave)
		{
			...
			{
				const FString SoundWaveBaseAssetName = Audio::Trace::Util::GetSoundBaseAssetName(SoundWaveObjectPtr->GetClass());
				const FString OwnerActorLabel = Audio::Trace::Util::GetOwnerActorLabel(ActiveSound);
				...
				UE_TRACE_LOG(Audio, SoundWaveStart, AudioMixerChannel)
					...
					<< SoundWaveStart.AssetPath(*SoundWaveObjectPtr->GetPathName())
					<< SoundWaveStart.SoundClassName(*SoundWaveBaseAssetName)
					<< SoundWaveStart.ActorLabel(*OwnerActorLabel);
			}
		}
		// Occasionally send a ping to Audio Insights to confirm the sound is still playing.
		// This is to ensure long duration sounds (such as ambiences) will still appear in snapshot files if their start action was before the snapshot start
		else if (FMath::Floor(ActiveSound.PlaybackTimeUnscaled + AudioDevice->GetDeviceDeltaTime()) - FMath::Floor(ActiveSound.PlaybackTimeUnscaled) > 0.0f)
		{
			const TObjectPtr<UObject> SoundWaveObjectPtr = FSoftObjectPath(WaveInstance->WaveData->GetPathName()).ResolveObject();
			if (SoundWaveObjectPtr)
			{
				...
				UE_TRACE_LOG(Audio, SoundWaveIsAlivePing, AudioMixerChannel)
					<< SoundWaveIsAlivePing.DeviceId(AudioDevice->DeviceID)
					<< SoundWaveIsAlivePing.Timestamp(FPlatformTime::Cycles64())
					<< SoundWaveIsAlivePing.PlayOrder(WaveInstance->GetPlayOrder())
					<< SoundWaveIsAlivePing.ActiveSoundPlayOrder(ActiveSound.GetPlayOrder())
					<< SoundWaveIsAlivePing.AssetPath(*SoundWaveObjectPtr->GetPathName())
					<< SoundWaveIsAlivePing.SoundClassName(*SoundWaveBaseAssetName)
					<< SoundWaveIsAlivePing.ActorLabel(*OwnerActorLabel);
			}
		}
...
void USoundWave::OverwriteSoundWaveDataWithTransformationData(const TArrayView<TObjectPtr<UWaveformTransformationBase>> InTransformations)
{
	// If InTransformations array is empty, use the current soundwaves transformations
	TArrayView<TObjectPtr<UWaveformTransformationBase>> TransformationsView = InTransformations.IsEmpty() ? 
		TArrayView<TObjectPtr<UWaveformTransformationBase>>(Transformations) : InTransformations;

	for (const TObjectPtr<UWaveformTransformationBase>& Transformation : TransformationsView)
	{
		if (Transformation)
		{
	...
	// CreateCueAndSampleChunks errors out if there are more than 2 channels
	if (NumChannels > 2)
	{
		if (!WaveCues.IsEmpty() && !WaveSampleLoops.IsEmpty())
		{
			UE_LOG(LogAudio, Warning, TEXT("Files with a channel count > 2 are not currently supported so new cue points or loop regions were not written to the RawData in file: %s"), *GetPathName());
		}
...
void USoundWave::SerializeCuePoints(FArchive& Ar, const bool bIsLoadingFromCookedArchive)
{
	...
	if (Ar.IsCooking())
	{
		...
		case ESoundWaveCuePointOrigin::WaveFile:
			...
			break;
		case ESoundWaveCuePointOrigin::MarkerTransformation:
			PlatformCuePoints = GetWaveformTransformationInfo().AllCuePointsRelativeToStartTime;
			break;
		default:
			PlatformCuePoints = CuePoints;
			break;
		...
		float ResampleRatio = -1.0f;
		const ITargetPlatform* CookingTarget = Ar.CookingTarget();
		if (ensure(CookingTarget) && CookingTarget != nullptr)
		{

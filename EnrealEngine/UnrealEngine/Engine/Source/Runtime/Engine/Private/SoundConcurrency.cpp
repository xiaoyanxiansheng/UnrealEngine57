...
{
	static const int32 DefaultMaxCount = 16;
...
bool USoundConcurrency::SetMaxCount(int32 InMaxCount)
{
	return Concurrency.SetMaxCount(InMaxCount);
}

FSoundConcurrencySettings::FSoundConcurrencySettings()
	: MaxCount(SoundConcurrencyPrivate::DefaultMaxCount)
	, PlatformMaxCount(SoundConcurrencyPrivate::DefaultMaxCount)
{
}

int32 FSoundConcurrencySettings::GetMaxCount() const
{
	if (bEnableMaxCountPlatformScaling)
	{
		if (const int32 Count = PlatformMaxCount.GetValue(); Count > 0)
		{
			return Count;
		}
	}
	...
	return SoundConcurrencyPrivate::DefaultMaxCount;
}

bool FSoundConcurrencySettings::SetMaxCount(int32 InMaxCount)
{
	if (InMaxCount <= 0 || bEnableMaxCountPlatformScaling)
	{
		return false;
	}
...
#if WITH_EDITOR
void FSoundConcurrencySettings::SetEnableMaxCountPlatformScaling(bool bEnabled)
{
	bEnableMaxCountPlatformScaling = bEnabled ? 1 : 0;
}

bool FSoundConcurrencySettings::IsMaxCountPlatformScalingEnabled() const
{
	return bEnableMaxCountPlatformScaling > 0;
}

bool FSoundConcurrencySettings::SetPlatformMaxCount(FPerPlatformInt InMaxCount)
{
	bool bIsMaxCountValid = InMaxCount.Default > 0;

#if WITH_EDITORONLY_DATA
	auto IsValidMaxCount = [](const TPair<FName, int32>& Pair) { return Pair.Value > 0; };
	bIsMaxCountValid &= Algo::AllOf(InMaxCount.PerPlatform, IsValidMaxCount);
#endif // WITH_EDITORONLY_DATA

	if (bIsMaxCountValid)
	{
		PlatformMaxCount = MoveTemp(InMaxCount);
		return true;
	}
...
{
	return Settings.GetMaxCount() <= ActiveSounds.Num();
}
...
void FConcurrencyGroup::AddActiveSound(FActiveSound& ActiveSound)
{
	...
	if (Settings.VolumeScaleMode == EConcurrencyVolumeScaleMode::Default)
	{
		...
		SoundConcurrencyPrivate::SetSoundDataTarget(ActiveSound, SoundData, 1.0f, 0.0f);
	}
...
void FConcurrencyGroup::RemoveActiveSound(FActiveSound& ActiveSound)
{
	...
	for (FActiveSound* OtherSound : ActiveSounds)
	{
		...
		if (Settings.bVolumeScaleCanRelease && Settings.VolumeScaleMode == EConcurrencyVolumeScaleMode::Default)
		{
			...
			SoundConcurrencyPrivate::SetSoundDataTarget(*OtherSound, OtherSoundData, NewTargetVolume, Settings.VolumeScaleReleaseTime);
		}
	}
}
...
void FConcurrencyGroup::UpdateGeneration(FActiveSound* NewActiveSound)
{
	...
	for (int32 i = 0; i < ActiveSounds.Num(); ++i)
	{
		...
		if (FConcurrencySoundData* SoundData = ActiveSound->ConcurrencyGroupData.Find(GetGroupID()))
		{
			...
			{
				SoundConcurrencyPrivate::SetSoundDataTarget(*ActiveSound, *SoundData, NewTargetVolume, AttackTime);
			}
		}
	}
}
...
void FConcurrencyGroup::CullSoundsDueToMaxConcurrency()
{
	...
	if (ActiveSounds.Num() <= Settings.GetMaxCount())
	{
		return;
	}
	...
	const int32 NumSoundsToStop = ActiveSounds.Num() - Settings.GetMaxCount();
...
FActiveSound* FSoundConcurrencyManager::CreateNewActiveSound(const FActiveSound& NewActiveSound, bool bIsRetriggering)
{
	...
	for (const FConcurrencyHandle& ConcurrencyHandle : ConcurrencyHandles)
	{
		check(ConcurrencyHandle.Settings.GetMaxCount() > 0);
	}
#endif
		...
		if (Settings.VolumeScaleMode == EConcurrencyVolumeScaleMode::Default)
		{
			...
			if (!FMath::IsNearlyEqual(Volume, 1.0f))
			{
				...
				for (FActiveSound* OtherSound : ActiveSounds)
				{
					...
					SoundConcurrencyPrivate::SetSoundDataTarget(*OtherSound, OtherSoundData, NewTargetVolume, Settings.VolumeScaleAttackTime);
				}
			}
		}
...
void FSoundConcurrencyManager::StopDueToVoiceStealing(FActiveSound& ActiveSound)
{
	...
	else
	{
		...
		Audio::Trace::EventLog::SendActiveSoundEvent(ActiveSound, Audio::Trace::EventLog::ID::StopRequestConcurrency);

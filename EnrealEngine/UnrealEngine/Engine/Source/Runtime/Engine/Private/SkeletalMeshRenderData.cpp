...
void FSkeletalMeshRenderData::Cache(const ITargetPlatform* TargetPlatform, USkinnedAsset* Owner, FSkinnedAssetCompilationContext* ContextPtr)
{
	...
	if (USkeletalMesh* SkelMesh = Cast<USkeletalMesh>(Owner))
	{
		if (SkelMesh->IsNaniteAssembly() && !NaniteAssembliesSupported())
		{
			// Pop up a notification to warn the user about assembly support
			extern void ShowNaniteAssembliesDisabledWarningNotification();
...
FSkeletalMeshRenderData::~FSkeletalMeshRenderData()
{
	FRenderResourceNameScope FatalScope(OwnerName);
...
void FSkeletalMeshRenderData::InitResources(bool bNeedsVertexColors, USkinnedAsset* Owner)
{
	if (!bInitialized)
	{
#if RHI_RAYTRACING
		...
		for (int32 LODIndex = 0; LODIndex < LODRenderData.Num(); LODIndex++)
		{
			...
			{
				RenderData.InitResources(bNeedsVertexColors, LODIndex, Owner);
			}
		}

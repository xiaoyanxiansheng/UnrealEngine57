...
#include UE_INLINE_GENERATED_CPP_BY_NAME(ObjectEditorUtils)
...
namespace FObjectEditorUtils
{
	...
	FText GetCategoryText( const FName InName )
	{
		...
		{
			DisplayName = FText::AsLocalizable_Advanced(CategoryLocalizationNamespace, NativeCategory, FName::NameToDisplayString(NativeCategory, false));
		}
	...
	FText GetCategoryText( const FField* InField )
	{
		...
		if (InField)
		{
			...
			{
				return FText::AsLocalizable_Advanced(CategoryLocalizationNamespace, NativeCategory, NativeCategory);
			}
		}

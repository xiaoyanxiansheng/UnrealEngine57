...
void FSceneView::OverridePostProcessSettings(const FPostProcessSettings& Src, float Weight)
{
...

	{
		...
		LERP_PP(BloomGaussianIntensity);
		...
		LERP_PP(BloomConvolutionIntensity);
...
void FSceneView::EndFinalPostprocessSettings(const FSceneViewInitOptions& ViewInitOptions)
{
	...
	if (State != nullptr)
	{
		State->SetExposureCompensationCurve(FinalPostProcessSettings.AutoExposureBiasCurve);
	}
...
float FSceneView::GetLastAverageSceneLuminance() const
{
	if (EyeAdaptationViewState)
	{
		return EyeAdaptationViewState->GetLastAverageSceneLuminance();
	}
	return 0.0f;
}
...
{
	if (Views.Num() && Views[0]->PrimaryScreenPercentageMethod == EPrimaryScreenPercentageMethod::RawOutput)
	{
		return true;
	}
	...
	if (Scene != nullptr)
	{
		if (GetFeatureLevelShadingPath(Scene->GetFeatureLevel()) == EShadingPath::Mobile)
		{
			...
			// Normal post-processing is disabled when using Mobile LDR or the mobile tonemap subpass
			return IsMobileHDR() && !IsMobileTonemapSubpassEnabled(ShaderPlatform, Aspects.IsMobileMultiViewEnabled());
		}
...
{
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	if (DebugViewShaderMode == DVSM_MAX)
	{

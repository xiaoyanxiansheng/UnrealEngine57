...
void ULevel::Serialize( FArchive& Ar )
{
	...
	else if (Ar.IsPersistent() && (Ar.IsSaving() || Ar.IsObjectReferenceCollector()))
	{
		...
		Algo::CopyIf(Actors, EmbeddedActors, [&](AActor* Actor)
		{
			...
			if ((Ar.GetPortFlags() & PPF_DuplicateForPIE) && !Actor->ShouldDuplicateInPIE())
			{
				return false;
			}
#endif
	...
	for (AActor* Actor : ActorsQueue)
	{
		...
		if (IsValid(Actor))
		{
			...
			if (TedsCompat)
			{
				...
				Actor->ForEachComponent(bIncludeChildActors, [&TedsCompat](UActorComponent* Component)
				{
					TedsCompat->AddCompatibleObject(Component);
				});
			}
		}
	...
	for (AActor* Actor : ActorsQueue)
	{
		...
		if (IsValid(Actor))
		{
			...
			if (TedsCompat)
			{
				...
				Actor->ForEachComponent(bIncludeChildActors, [&TedsCompat](UActorComponent* Component)
				{
					TedsCompat->RemoveCompatibleObject(Component);
				});
...
namespace LevelAssetRegistryHelper
{
	static bool GetLevelInfoFromAssetRegistry(FName LevelPackage, TFunctionRef<bool(const FAssetData & Asset)> Func)
	{
		...
		// Also ignore the invalid path warning as we just want to return false for unmounted level paths.
		AssetRegistry.ScanSynchronous({ }, { LevelPackage.ToString()}, UE::AssetRegistry::EScanFlags::IgnoreInvalidPathWarning);
...
bool ULevel::GetIsLevelCookedFromAsset(const FAssetData& Asset)
{
	return !!(Asset.PackageFlags & PKG_Cooked);
}

bool ULevel::GetIsLevelCookedFromPackage(FName LevelPackage)
{
	return LevelAssetRegistryHelper::GetLevelInfoFromAssetRegistry(LevelPackage, [](const FAssetData& Asset)
	{
		return GetIsLevelCookedFromAsset(Asset);
	});
}
...
bool ULevel::SetUseActorFolders(bool bInEnabled, bool bInInteractiveMode)
{
	...
	{
		UClass* WorldPartitionBuilderCommandletClass = FindObject<UClass>(nullptr, TEXT("/Script/UnrealEd.WorldPartitionBuilderCommandlet"), EFindObjectFlags::ExactClass);

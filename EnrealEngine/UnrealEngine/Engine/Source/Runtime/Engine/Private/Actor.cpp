...
/** Controls if a replicated component should call FReplicationSystemUtil::StopReplicatingActorComponent immediately when updating its replication status. */
static bool GbCallStopReplicationForActorComponentWhenUpdatingReplicatedComponent = false;
static FAutoConsoleVariableRef CVarStopReplicationForActorComponentWhenUpdatingReplicatedComponent(
	TEXT("net.Iris.CallStopReplicationForActorComponentWhenUpdatingReplicatedComponent"),
	GbCallStopReplicationForActorComponentWhenUpdatingReplicatedComponent,
	TEXT("If true, replicated ActorComponentrs will call stop replication if they are no longer replicated after a call to UpdateReplicatedComponent.."));
...
TSAN_ATOMIC(int32) CSVActorTotalCount = 0;
...
void AActor::InitializeDefaults()
{
	...
	bActorIsPendingPostNetInit = false;
...
{
	CSV_CUSTOM_STAT(ActorMigration, Total, 1, ECsvCustomStatOp::Accumulate);
	CSV_CUSTOM_STAT(ActorMigration, Loan, MigrationContext.MigrationSide == EObjectMigrationSide::Send && MigrationContext.GetObjectMigrationSendType(this) == EObjectMigrationSendType::Loan, ECsvCustomStatOp::Accumulate);
	CSV_CUSTOM_STAT(ActorMigration, ReturnBorrowed, MigrationContext.MigrationSide == EObjectMigrationSide::Send && MigrationContext.GetObjectMigrationSendType(this) == EObjectMigrationSendType::ReturnBorrowed, ECsvCustomStatOp::Accumulate);
	CSV_CUSTOM_STAT(ActorMigration, ReassignBorrowed, MigrationContext.MigrationSide == EObjectMigrationSide::Send && MigrationContext.GetObjectMigrationSendType(this) == EObjectMigrationSendType::ReassignBorrowed, ECsvCustomStatOp::Accumulate);
	CSV_CUSTOM_STAT(ActorMigration, ReassignOwnership, MigrationContext.MigrationSide == EObjectMigrationSide::Send && MigrationContext.GetObjectMigrationSendType(this) == EObjectMigrationSendType::ReassignOwnership, ECsvCustomStatOp::Accumulate);
	CSV_CUSTOM_STAT(ActorMigration, Borrowed, MigrationContext.MigrationSide == EObjectMigrationSide::Receive && MigrationContext.GetObjectMigrationRecvType(this) == EObjectMigrationRecvType::Borrowed, ECsvCustomStatOp::Accumulate);
	CSV_CUSTOM_STAT(ActorMigration, ReturnedLoan, MigrationContext.MigrationSide == EObjectMigrationSide::Receive && MigrationContext.GetObjectMigrationRecvType(this) == EObjectMigrationRecvType::ReturnedLoan, ECsvCustomStatOp::Accumulate);
	CSV_CUSTOM_STAT(ActorMigration, AssignedOwnership, MigrationContext.MigrationSide == EObjectMigrationSide::Receive && MigrationContext.GetObjectMigrationRecvType(this) == EObjectMigrationRecvType::AssignedOwnership, ECsvCustomStatOp::Accumulate);
	...
	if (MigrationContext.MigrationSide == EObjectMigrationSide::Send)
	{
		...
		if (MigrationSendType != EObjectMigrationSendType::Loan)
		{
		...
		else 
		{
			...
			if (World)
			{
				...
				UE::Net::FReplicationSystemUtil::StopReplicatingActor(this, FStopReplicatingActorParams(EEndPlayReason::Destroyed));
				...
				if (FWorldContext* Context = GEngine->GetWorldContextFromWorld(World))
				{
					for (FNamedNetDriver& Driver : Context->ActiveNetDrivers)
					{
						...
						{
							Driver.NetDriver->NotifyActorDestroyed(this, false, EChannelCloseReason::Migrated);
						}
					}
				}
...
void AActor::UpdateReplicatedComponent(UActorComponent* Component)
{
	...
	if (HasAuthority())
	{
		...
		{
			// Start replication and set NetCondition, if component already is replicated the NetCondition will be updated
			UE::Net::FReplicationSystemUtil::StartReplicatingActorComponent(this, Component);
		}
		else if (bWasRemovedFromReplicatedComponents || NetCondition == COND_Never)
		{
			if (GbCallStopReplicationForActorComponentWhenUpdatingReplicatedComponent)
			{
				UE::Net::FReplicationSystemUtil::StopReplicatingActorComponent(Component);
			}
			else
			{
...
void AActor::UpdateAllReplicatedComponents()
{
	...
	for (FReplicatedComponentInfo& RepComponentInfo : ReplicatedComponentsInfo)
	{
		...
		if (HasAuthority())
		{
			// Need to stop replication for all components that should no longer replicate
			if (RepComponentInfo.Component && !RepComponentInfo.Component->GetIsReplicated())
			{
				if (GbCallStopReplicationForActorComponentWhenUpdatingReplicatedComponent)
				{
					UE::Net::FReplicationSystemUtil::StopReplicatingActorComponent(RepComponentInfo.Component);
				}
				else
				{
...
void AActor::FinishSpawning(const FTransform& UserTransform, bool bIsDefaultTransform, const FComponentInstanceDataCache* InstanceDataCache, ESpawnActorScaleMethod TransformScaleMethod)
{
	...
	if (ensure(!bHasFinishedSpawning))
	{
		...
		{
			FTransform OriginalSpawnTransform(NoInit);
			const bool bFound = GSpawnActorDeferredTransformCache.RemoveAndCopyValue(this, OriginalSpawnTransform);

			if (bFound && !OriginalSpawnTransform.Equals(UserTransform))
			{
...
{ 
	...
	else if (bReplicates != bInReplicates || RemoteRole != ExpectedRemoteRole) 
	{
		...
		if (bActorInitialized)
		{
			...
			{
				UE::Net::FReplicationSystemUtil::StopReplicatingActor(this, FStopReplicatingActorParams(EEndPlayReason::RemovedFromWorld));
			}
...
void AActor::IncrementalPreUnregisterComponents()
{
	...
	for (UActorComponent* Component : GetComponents())
	{
		if (Component && Component->ShouldIncrementalPreUnregister())
		{
			Components.Add(Component);
		}
	}

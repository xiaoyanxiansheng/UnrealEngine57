...
namespace Audio
{
	...
	bool FSoundWavePCMWriter::BeginWriteToSoundWave(const FString& FileName, const TSampleBuffer<>& InSampleBuffer, FString InPath, TFunction<void(const USoundWave*)> OnSuccess, TObjectPtr<USoundWave> InSoundWaveTransformationTarget)
	{
...
#if WITH_EDITOR
		CurrentSoundWave->ResetInvalidTemplateArguments();

		// CurrentSoundwave::Transformations must be empty because all Transformation data will be written directly to the soundwave, not overlayed as a transformation
		check(CurrentSoundWave->Transformations.IsEmpty());
#endif //WITH_EDITOR
		...
		CurrentOperation.Reset(new FAsyncSoundWavePCMWriterTask(this, ESoundWavePCMWriteTaskType::GenerateAndWriteSoundWave, [this, OnSuccess, InSoundWaveTransformationTarget](const USoundWave* InSoundWave)
		{
...
#if WITH_EDITOR
			if (InSoundWaveTransformationTarget && InSoundWaveTransformationTarget->Transformations.Num() > 0)
			{
				CurrentSoundWave->OverwriteSoundWaveDataWithTransformationData(InSoundWaveTransformationTarget->Transformations);
			}
#endif //WITH_EDITOR

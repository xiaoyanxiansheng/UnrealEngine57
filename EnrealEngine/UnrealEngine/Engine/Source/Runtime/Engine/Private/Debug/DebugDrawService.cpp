...
void UDebugDrawService::Draw(const FEngineShowFlags Flags, FViewport* Viewport, FSceneView* View, FCanvas* Canvas, UCanvas* CanvasObject, APlayerController* PlayerController)
{
	...
	Draw(Flags, CanvasObject, PlayerController);
}

void UDebugDrawService::Draw(const FEngineShowFlags Flags, UCanvas* Canvas, APlayerController* PlayerController)
{
	...
	for (int32 FlagIndex = 0; FlagIndex < Delegates.Num(); ++FlagIndex)
	{
		...
		{
			MulticastDelegate.Broadcast(Canvas, PlayerController);
		}
	}
}

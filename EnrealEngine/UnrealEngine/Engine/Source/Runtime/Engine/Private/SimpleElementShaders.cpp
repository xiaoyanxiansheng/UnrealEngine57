...
void FSimpleElementVS::SetParameters(FRHIBatchedShaderParameters& BatchedParameters, const FDFRelativeViewMatrices& Matrices, const FDFRelativeViewMatrices& InstancedViewMatrices)
{
	const FMatrix44f FinalMats[2] = { Matrices.RelativeWorldToClip , InstancedViewMatrices.RelativeWorldToClip };
	const FVector4f PositionsHigh[2] = { Matrices.PositionHigh, InstancedViewMatrices.PositionHigh };
	SetShaderValue(BatchedParameters, RelativeTransform, FinalMats);
	SetShaderValue(BatchedParameters, TransformPositionHigh, PositionsHigh);
}

...
#include UE_INLINE_GENERATED_CPP_BY_NAME(RichCurve)
...
void FRichCurve::ReadjustTimeRange(float NewMinTimeRange, float NewMaxTimeRange, bool bInsert/* whether insert or remove*/, float OldStartTime, float OldEndTime)
{
	...
	if (bNeedToDeleteKey)
	{
		// Ensure this here as DeleteKey might do it while running through the loop, causing the keyhandle to become invalid
		EnsureAllIndicesHaveHandles();
		for (int32 KeyIndex=0; KeyIndex<Keys.Num(); ++KeyIndex)
		{

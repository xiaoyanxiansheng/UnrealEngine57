...
void FKeyHandleLookupTable::ReplaceHandle(int32 Index, FKeyHandle HandleToReplace)
{
	if (KeyHandles.IsValidIndex(Index) && !KeyHandlesToIndices.Contains(HandleToReplace))
	{
		KeyHandlesToIndices.Remove(KeyHandles[Index]);
		KeyHandlesToIndices.Add(HandleToReplace, Index);
		KeyHandles[Index] = HandleToReplace;
	}
}

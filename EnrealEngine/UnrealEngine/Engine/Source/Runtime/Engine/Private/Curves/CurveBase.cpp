...
void UCurveBase::SetOnCurveChangedIsInteractive(bool bInteractive)
{
#if WITH_EDITOR
	bOnCurveUpdateInteractive = bInteractive;
#endif
}
...
{
#if WITH_EDITOR
	// ideally OnCurveChanged should have taken a EPropertyChangeType as a function arg
	//	instead we pick it up from an extra call to SetOnCurveChangedIsInteractive
	EPropertyChangeType::Type ChangeType = bOnCurveUpdateInteractive ? EPropertyChangeType::Interactive : EPropertyChangeType::ValueSet;
	OnUpdateCurve.Broadcast(this, ChangeType);
#endif // WITH_EDITOR
}

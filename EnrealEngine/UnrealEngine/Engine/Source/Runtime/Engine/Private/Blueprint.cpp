...
#include UE_INLINE_GENERATED_CPP_BY_NAME(Blueprint)
...
#if WITH_EDITORONLY_DATA
namespace BlueprintLocalization
{
	static bool bSkipGraphProperties = true;
	static FAutoConsoleVariableRef CVarSkipGraphProperties(TEXT("Localization.SkipBlueprintGraphProperties"), bSkipGraphProperties, TEXT("True to skip BP graph properties from the localization gather, or false to mark them as editor-only"));

	void GatherForLocalization(const UObject* const Object, FPropertyLocalizationDataGatherer& PropertyLocalizationDataGatherer, const EPropertyLocalizationGathererTextFlags GatherTextFlags)
	{
		...
		EPropertyLocalizationGathererTextFlags BlueprintGatherFlags = GatherTextFlags | (bSkipGraphProperties ? EPropertyLocalizationGathererTextFlags::SkipProperties : EPropertyLocalizationGathererTextFlags::ForceEditorOnlyProperties);
...
{
#if WITH_EDITORONLY_DATA
	{ static const FAutoRegisterLocalizationDataGatheringCallback AutomaticRegistrationOfLocalizationGatherer(UBlueprintCore::StaticClass(), &BlueprintLocalization::GatherForLocalization); }
#endif
...
void UBlueprint::Serialize(FArchive& Ar)
{
	...
	if (Ar.IsLoading())
	{
		for (UClass* ClassIt = ParentClass; ClassIt && !ClassIt->HasAnyClassFlags(CLASS_Native); ClassIt = ClassIt->GetSuperClass())
		{
			...
			if (ClassIt->ClassGeneratedBy)
			{
				ClassIt->ClassGeneratedBy->ConditionalPreload();
			}
		}
	}
	...
	// @todo: temporarily marking all BPs to be uncooked, must check in to get a build overnight
...
{
	FPostLoadAssetRegistryTagsContext Context(InAssetData, OutTagsAndValuesToUpdate, IsInGameThread());
	PostLoadBlueprintAssetRegistryTags(Context);
}

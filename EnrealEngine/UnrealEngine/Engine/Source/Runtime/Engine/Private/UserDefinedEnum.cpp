...
void UUserDefinedEnum::Serialize(FArchive& Ar)
{
	...
	{
		Names.MakeValuesSequentialFromZero();
...
bool UUserDefinedEnum::SetEnums(TArray<TPair<FName, int64>>& InNames, ECppForm InCppForm, EEnumFlags InFlags, bool bAddMaxKeyIfMissing)
{
	...
	const int64 MaxEnumValue = (InNames.Num() == 0) ? 0 : GetMaxEnumValue(InNames, InFlags) + 1;

	// Add a placeholder to the end of Names for the max element we're going to generate so that we don't need to reallocate
	Names.Empty();
	...
	for (int32 TryNum = 0; TryNum < MaxTryNum; ++TryNum)
	{
		...
		if ((MaxEnumItemIndex == INDEX_NONE) && (LookupEnumName(GetPackage()->GetFName(), MaxEnumItem) == INDEX_NONE))
		{
			...
			check(Names.Num() == (InNames.Num() + 1));
			AddNamesToPrimaryList();
			return true;
		}
	}

	Names.DiscardPlaceholder();
	check(Names.Num() == InNames.Num());

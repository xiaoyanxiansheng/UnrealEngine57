...
namespace SoundModulationDestinationPrivate
{
	static bool IsPluginEnabled()
	{
		static bool bEnabled = AudioPluginUtilities::GetDesiredModulationPlugin() != nullptr;
		return bEnabled;
	}
}
...
namespace Audio
{
	...
	void FModulationDestination::Init(FDeviceId InDeviceId, FName InParameterName, bool bInIsBuffered, bool bInValueNormalized)
	{
		...
		{
			UE_CLOG(SoundModulationDestinationPrivate::IsPluginEnabled(),LogAudio, Warning, TEXT("Modulation Parameter %s not found while Initializing Modulation Destination. Using default parameter instead."), *InParameterName.ToString());
	...
	void FModulationDestination::ClearModulators()
	{
		DestinationData->ResetHandles();
	}

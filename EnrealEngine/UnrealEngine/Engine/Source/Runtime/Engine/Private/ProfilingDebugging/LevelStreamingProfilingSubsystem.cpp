...
void ULevelStreamingProfilingSubsystem::RecordLevelStreamingCountsToCSV()
{	
	if (FCsvProfiler::Get()->IsCapturing())
	{
		int32 NumLevelsLoading = WorldLevelStreamingStats.LevelsLoading.Num();
		int32 NumLevelsMakingVisible = 0;
		Algo::ForEach(WorldLevelStreamingStats.LevelsMakingVisible, [&NumLevelsMakingVisible, this](const ULevelStreaming* LevelStreaming) { NumLevelsMakingVisible += GetWorld()->IsLevelMakingVisible(LevelStreaming->GetLoadedLevel()) ? 1 : 0; });
		int32 NumLevelsMakingInvisible = WorldLevelStreamingStats.LevelsMakingInvisible.Num();

		CSV_CUSTOM_STAT(LevelStreaming, NumLevelsLoading, NumLevelsLoading, ECsvCustomStatOp::Set);
		CSV_CUSTOM_STAT(LevelStreaming, NumLevelsMakingVisible, NumLevelsMakingVisible, ECsvCustomStatOp::Set);
		CSV_CUSTOM_STAT(LevelStreaming, NumLevelsMakingInvisible, NumLevelsMakingInvisible, ECsvCustomStatOp::Set);
...
void ULevelStreamingProfilingSubsystem::OnLevelStreamingStateChanged(UWorld* World, const ULevelStreaming* StreamingLevel, ULevel* LevelIfLoaded, ELevelStreamingState PrevState, ELevelStreamingState NewState)
{
	...
	{
		WorldLevelStreamingStats.Remove(StreamingLevel);
		if (NewState == ELevelStreamingState::Loading)
		{
			WorldLevelStreamingStats.LevelsLoading.Add(StreamingLevel);
		}
		else if (NewState == ELevelStreamingState::MakingVisible)
		{
			WorldLevelStreamingStats.LevelsMakingVisible.Add(StreamingLevel);
		}
		else if (NewState == ELevelStreamingState::MakingInvisible)
		{
			WorldLevelStreamingStats.LevelsMakingInvisible.Add(StreamingLevel);
		}
	}
#endif

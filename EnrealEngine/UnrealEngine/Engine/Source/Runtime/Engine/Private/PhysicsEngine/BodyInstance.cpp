...
FBodyInstance::FBodyInstance()
	...
	, bOverrideIterationCounts(false)
...
void FBodyInstance::SetCollisionEnabled(ECollisionEnabled::Type NewType, bool bUpdatePhysicsFilterData)
{
	if (CollisionEnabled != NewType)
	{
		...
		if (bWasPhysicsEnabled != bIsPhysicsEnabled)
		{
			...
			{
				PrimComponent->RecreateInstanceBody(InstanceBodyIndex);
			}
	...
	FPhysicsCommand::ExecuteWrite(GetPhysicsActorWeldRoot(), [&](const FPhysicsActorHandle& Actor)
	{
		...
		for(int32 ShapeIndex = 0; ShapeIndex < NumTotalShapes; ++ShapeIndex)
		{
			...
			FPhysicsCommand::ExecuteShapeWrite(this, Shape, [&](const FPhysicsShapeHandle& InnerShape)
			{
				...
				const FCollisionFilterData& QueryFilter = bIsTrimesh ? FilterData.QueryComplexFilter : FilterData.QuerySimpleFilter;
				Chaos::Filter::FCombinedShapeFilterData CombinedShapeFilter = Chaos::Filter::FShapeFilterBuilder::BuildFromLegacyShapeFilter(QueryFilter, FilterData.SimFilter);
				FPhysicsInterface::SetCombinedShapeFilterData(InnerShape, CombinedShapeFilter);
			});
		}
...
TSharedPtr<TArray<ANSICHAR>> GetDebugDebugName(const UPrimitiveComponent* PrimitiveComp, const UObject* SourceObject, const UBodySetup* BodySetup, FString& DebugName)
{
	...
	if (PrimitiveComp)
	{
		...
		AActor* PrimitiveCompOwner = PrimitiveComp->GetOwner();
		UPackage* ExternalPackage = PrimitiveCompOwner ? PrimitiveCompOwner->GetExternalPackage() : nullptr;
		if (ExternalPackage)
		{
			DebugName += FString::Printf(TEXT("%s "), *ExternalPackage->GetName());
		}
	}
...
void FBodyInstance::TermBody(bool bNeverDeferRelease)
{
	...
	if (ShapeToBodiesMap != nullptr && !ShapeToBodiesMap->IsEmpty())
	{
		...
		int32 BodyIndex = 0;
		for (const TPair<FPhysicsShapeHandle, FWeldInfo>& It : *ShapeToBodiesMap)
		{
...
void FBodyInstance::UpdateInstanceSimulatePhysics()
{
	...
	FPhysicsCommand::ExecuteWrite(GetPhysicsActor(), [&](const FPhysicsActorHandle& Actor)
	{
		...
		FPhysicsInterface::SetPartialIslandSleepAllowed_AssumesLocked(Actor, bAllowPartialIslandSleep);
		FPhysicsInterface::SetOneWayInteraction_AssumesLocked(Actor, bOneWayInteraction);
		FPhysicsInterface::SetPositionSolverIterationCount_AssumesLocked(Actor, GetPositionSolverIterationCount());
		FPhysicsInterface::SetVelocitySolverIterationCount_AssumesLocked(Actor, GetVelocitySolverIterationCount());
		FPhysicsInterface::SetProjectionSolverIterationCount_AssumesLocked(Actor, GetProjectionSolverIterationCount());
...
{
	if (bAllowPartialIslandSleep != bInAllowPartialIslandSleep)
	{
		bAllowPartialIslandSleep = bInAllowPartialIslandSleep;

		FPhysicsCommand::ExecuteWrite(GetPhysicsActor(), [this, bInAllowPartialIslandSleep](const FPhysicsActorHandle& Actor)
			{
				if (FPhysicsInterface::IsValid(Actor) && FPhysicsInterface::IsRigidBody(Actor))
				{
					FPhysicsInterface::SetPartialIslandSleepAllowed_AssumesLocked(Actor, bInAllowPartialIslandSleep);
				}
			});
	}
}
...
{
	if (bOverride != bOverrideIterationCounts)
	{
		bOverrideIterationCounts = bOverride;

		int32 PosCount = GetPositionSolverIterationCount();
		int32 VelCount = GetVelocitySolverIterationCount();
		int32 ProjCount = GetProjectionSolverIterationCount();

		FPhysicsCommand::ExecuteWrite(GetPhysicsActor(), [this, PosCount, VelCount, ProjCount](const FPhysicsActorHandle& Actor)
			{
				if (FPhysicsInterface::IsValid(Actor) && FPhysicsInterface::IsRigidBody(Actor))
				{
					FPhysicsInterface::SetPositionSolverIterationCount_AssumesLocked(Actor, PosCount);
					FPhysicsInterface::SetVelocitySolverIterationCount_AssumesLocked(Actor, VelCount);
					FPhysicsInterface::SetProjectionSolverIterationCount_AssumesLocked(Actor, ProjCount);
				}
			});
	}
}
...
void FBodyInstance::SetPositionSolverIterationCount(uint8 PositionSolverIterationCountIn)
{
	if (PositionSolverIterationCount != PositionSolverIterationCountIn)
	{
		...
		FPhysicsCommand::ExecuteWrite(GetPhysicsActor(), [this, PosCount](const FPhysicsActorHandle& Actor)
			{
				if (FPhysicsInterface::IsValid(Actor) && FPhysicsInterface::IsRigidBody(Actor))
				{
					FPhysicsInterface::SetPositionSolverIterationCount_AssumesLocked(Actor, PosCount);
...
void FBodyInstance::SetVelocitySolverIterationCount(uint8 VelocitySolverIterationCountIn)
{
	if (VelocitySolverIterationCount != VelocitySolverIterationCountIn)
	{
		...
		FPhysicsCommand::ExecuteWrite(GetPhysicsActor(), [this, VelCount](const FPhysicsActorHandle& Actor)
			{
				if (FPhysicsInterface::IsValid(Actor) && FPhysicsInterface::IsRigidBody(Actor))
				{
					FPhysicsInterface::SetVelocitySolverIterationCount_AssumesLocked(Actor, VelCount);
...
void FBodyInstance::SetProjectionSolverIterationCount(uint8 ProjectionSolverIterationCountIn)
{
	if (ProjectionSolverIterationCount != ProjectionSolverIterationCountIn)
	{
		...
		FPhysicsCommand::ExecuteWrite(GetPhysicsActor(), [this, ProjCount](const FPhysicsActorHandle& Actor)
			{
				if (FPhysicsInterface::IsValid(Actor) && FPhysicsInterface::IsRigidBody(Actor))
				{
					FPhysicsInterface::SetProjectionSolverIterationCount_AssumesLocked(Actor, ProjCount);
...
void FBodyInstance::InitDynamicProperties_AssumesLocked()
{
	...
	if(FPhysicsInterface::IsDynamic(GetPhysicsActor()))
	{
		...
		FPhysicsInterface::SetSolverPositionIterationCount_AssumesLocked(GetPhysicsActor(), GetPositionSolverIterationCount());
		FPhysicsInterface::SetSolverVelocityIterationCount_AssumesLocked(GetPhysicsActor(), GetVelocitySolverIterationCount());
		FPhysicsInterface::SetSolverProjectionIterationCount_AssumesLocked(GetPhysicsActor(), GetProjectionSolverIterationCount());

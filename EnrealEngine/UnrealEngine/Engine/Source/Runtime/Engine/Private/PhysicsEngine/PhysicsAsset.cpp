...
void UPhysicsAsset::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	for(UAssetUserData* UserData : AssetUserData)
	{
		if(UserData)
		{
			UserData->PostEditChangeOwner(PropertyChangedEvent);
		}
	}
...
void UPhysicsAsset::AddAssetUserData(UAssetUserData* InUserData)
{
	if(!InUserData)
	{
		return;
...
UAssetUserData* UPhysicsAsset::GetAssetUserDataOfClass(TSubclassOf<UAssetUserData> InUserDataClass)
{
	TObjectPtr<UAssetUserData>* Found = AssetUserData.FindByPredicate([&InUserDataClass](TObjectPtr<UAssetUserData> Item)
		{
			return Item && Item->IsA(InUserDataClass);
		});
...
void UPhysicsAsset::RemoveUserDataOfClass(TSubclassOf<UAssetUserData> InUserDataClass)
{
	AssetUserData.RemoveAll([&InUserDataClass](TObjectPtr<UAssetUserData> Item)
	{
		return Item && Item->IsA(InUserDataClass);
	});
}

const TArray<UAssetUserData*>* UPhysicsAsset::GetAssetUserDataArray() const
{

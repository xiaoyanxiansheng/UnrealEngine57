...
FPhysScene* FPhysInterface_Chaos::GetCurrentSceneForComponent(USceneComponent* InComponent)
{
	...
	if (UPrimitiveComponent* PrimComp = Cast<UPrimitiveComponent>(InComponent))
	{
				...
				for (int32 ShapeIdx = 0; ShapeIdx < NumShapes; ShapeIdx++)
				{
					...
					if ((bSweepComplex && bShapeIsComplex) || (!bSweepComplex && bShapeIsSimple))
					{
						...
						if (bShapeHit && FloatDistance < Hit.Distance)
						{
							...
							ConvertQueryImpactHit(OwnerComponentInst->GetWorld(), Hit, OutHit, DeltaMag, QueryFilter, InStart, InEnd, &QueryGeometry, StartTransform, true, false);
							bSweepHit = true;
						}
					}
				}

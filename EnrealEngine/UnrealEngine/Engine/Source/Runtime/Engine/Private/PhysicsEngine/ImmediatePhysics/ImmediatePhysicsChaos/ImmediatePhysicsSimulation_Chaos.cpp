...
namespace ImmediatePhysics_Chaos
{
	...
	void FSimulation::SetUseMinStepTime(bool bUse)
	{
		Implementation->bUseMinStepTime = bUse;
	}

	void FSimulation::SetUseFixedStepTolerance(bool bUse)
	{
		Implementation->bUseFixedStepTolerance = bUse;
	}
		...
		Chaos::FCollisionDetectorSettings* CollisionDetectorSettings)
	{
		...
		if (Implementation->bUseMinStepTime && (FixedStepTime <= 0.0f) && (StepTime < ChaosImmediate_Evolution_MinStepTime))
		{
		...
		if (FixedStepTime > 0)
		{
			...
			if (!Implementation->bUseFixedStepTolerance || RemainderTime > ChaosImmediate_Evolution_FixedStepTolerance * StepTime)
			{
				++NumSteps;

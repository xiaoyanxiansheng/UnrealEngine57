...
void FPhysScene_Chaos::OnSyncBodies(Chaos::FPhysicsSolverBase* Solver)
{
	...
	auto ExecuteCallback = [this, Solver, &Dispatcher]()
	{
	...
	static FName TransactionalDispatcherWorkName(TEXT("FPhysScene_Chaos::OnSyncBodies-Dispatcher"));
	UE::RemoteExecutor::ExecuteTransactional(TransactionalDispatcherWorkName, ExecuteCallback);
#else
	ExecuteCallback();
#endif
	...
	{
		auto DoWorkCallback = [this, &ComponentTransform]
		{
...
#if UE_WITH_REMOTE_OBJECT_HANDLE
		static FName TransactionalWorkName(TEXT("FPhysScene_Chaos::OnSyncBodies"));

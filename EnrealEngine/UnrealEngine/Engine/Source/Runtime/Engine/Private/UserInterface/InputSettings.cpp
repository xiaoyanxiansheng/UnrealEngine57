...
bool UInputSettings::IsActionMappingDeprecated(const FName ActionName) const
{
	return DeprecatedActionAndAxisNames.Contains(ActionName);
}

void UInputSettings::SetActionMappingDeprecated(const FName ActionName, const bool bIsDeprecated)
{
	if (bIsDeprecated)
	{
		DeprecatedActionAndAxisNames.Add(ActionName);
	}
	else
	{
		DeprecatedActionAndAxisNames.Remove(ActionName);
	}
}

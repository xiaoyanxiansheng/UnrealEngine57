...
DECLARE_CYCLE_STAT(TEXT("CanvasDashedBoxItem Time"),STAT_Canvas_DashedBoxItemTime,STATGROUP_Canvas);
...
{	
	...
	const bool bWriteAlpha = InCanvas->IsWriteDestinationAlphaSet();
	BlendMode = GetTextBlendMode( bHasShadow );
	if (InCanvas->IsUsingInternalTexture() || bWriteAlpha)
	{
...
void FCanvasDashedBoxItem::Draw(class FCanvas* InCanvas)
{
	SCOPE_CYCLE_COUNTER(STAT_Canvas_DashedBoxItemTime);
	...
	TArray<FVector, TInlineAllocator<4>> Corners;
	// Top
	Corners.AddUnique( FVector( Position.X , Position.Y, 0.0f ) );
	// Right
	Corners.AddUnique( FVector( Position.X + Size.X , Position.Y, 0.0f ) );
	...
	FVector CurrentPosition = Corners[0];
	bool bIsDash = true;
	float NextDistance = DashLength;
	
	// Draw the four dashed edges
	for (int32 EdgeIndex = 0; EdgeIndex < Corners.Num() ; EdgeIndex++)
	{
		int32 NextCornerIndex = ( ( EdgeIndex + 1 ) % Corners.Num() );
		
		bool bMoveNext = false;
		
		while (!bMoveNext)
		{
			FVector ToCornerDelta = Corners[NextCornerIndex] - CurrentPosition;
			bool bFlipDash = false;
		
			FVector NextPosition;
			if (ToCornerDelta.SquaredLength() < NextDistance * NextDistance)
			{
				NextPosition = Corners[NextCornerIndex];
				NextDistance = NextDistance - ToCornerDelta.Length();
				bMoveNext = true;
			}
			else
			{
				NextPosition = CurrentPosition + ToCornerDelta.GetSafeNormal() * NextDistance;
				bFlipDash = true;
			...
			if (bIsDash)
			{
				BatchedElements->AddLine( CurrentPosition, NextPosition, Color, HitProxyId, DashThickness );
			}

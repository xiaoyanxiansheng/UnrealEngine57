...
{
	static int32 StoredDebugMenuOffset = 0;
	static int32 LastKnownColumnCount = 0;
	static int32 MaxColumnsCreated = 0;

	static int32 ColumnOffsetIndex = 0;
	static FAutoConsoleVariableRef DebugManagerColumnOffsetIndex(
		TEXT("DebugManager.ColumnOffsetIndex"),
		ColumnOffsetIndex,
		TEXT("")
	);

	static FAutoConsoleCommandWithWorldAndArgs CmdDebugManagerCycleToNextColumn(
		TEXT("DebugManager.CycleToNextColumn"),
		TEXT("Will move the text over to the next column on the debugmanager"),
		FConsoleCommandWithWorldAndArgsDelegate::CreateLambda([](const TArray<FString>& InParams, UWorld* World)
			{
				DisplayDebugManagerCvars::ColumnOffsetIndex++;
				if (DisplayDebugManagerCvars::ColumnOffsetIndex > DisplayDebugManagerCvars::MaxColumnsCreated)
				{
					DisplayDebugManagerCvars::ColumnOffsetIndex = DisplayDebugManagerCvars::MaxColumnsCreated;
				}
			})
	);

	static FAutoConsoleCommandWithWorldAndArgs CmdDebugManagerCycleToPreviousColumn(
		TEXT("DebugManager.CycleToPreviousColumn"),
		TEXT("Will move the text over to the previous column on the debugmanager"),
		FConsoleCommandWithWorldAndArgsDelegate::CreateLambda([](const TArray<FString>& InParams, UWorld* World)
			{
				DisplayDebugManagerCvars::ColumnOffsetIndex--;
				if (DisplayDebugManagerCvars::ColumnOffsetIndex < 0)
				{
					DisplayDebugManagerCvars::ColumnOffsetIndex = 0;
				}
			})
	);
}
...
bool FCanvasWordWrapper::ProcessLine(FWrappingState& WrappingState)
{
	...
	if (WrappingState.StartIndex < StringLength)
	{
		...
		while (BreakIndex > 0 && FTextChar::IsWhitespace(WrappingState.String[BreakIndex - 1]))
		{
			--BreakIndex;
		}
		...
		while (NextStartIndex < StringLength && FTextChar::IsWhitespace(WrappingState.String[NextStartIndex]))
		{
			++NextStartIndex;
		}
...
{	
	...
	if( Data->BatchedElements.HasPrimsToDraw() )
	{
		...
		const FRenderTarget* CanvasRenderTarget = Canvas->GetRenderTarget();
		const float Gamma = 1.0f / CanvasRenderTarget->GetDisplayGamma(); // GetDisplayGamma typically == 2.2
		const bool bAllowDeleteOnRender = Canvas->GetAllowedModes() & FCanvas::Allow_DeleteOnRender;
			...
			[DrawParameters](FRHICommandList& RHICmdList)
		{
			...
			if (DrawParameters.bAllowDeleteOnRender)
			{
				delete DrawParameters.RenderData;
			}
		});

		if (bAllowDeleteOnRender)
		{
			Data = nullptr;
		}
	}
...
void FCanvas::DrawTile(double X, double Y, double SizeX, double SizeY, float U, float V, float SizeU, float SizeV, const FLinearColor& Color,	const FTexture* Texture, bool AlphaBlend)
{
	...
	if (AlphaBlend)
	{
		...
		{
			// If alpha is required, and no internal texture used, replace SE_BLEND_Translucent with SE_BLEND_AlphaBlend for all tiles.
			BlendMode = (bWriteDestinationAlpha ? SE_BLEND_AlphaBlend : SE_BLEND_Translucent);
		}
	}
...
void FDisplayDebugManager::DrawString(const FString& InDebugString, const float& OptionalXOffset)
{
	if (Canvas)
	{
		...
		Canvas->DrawItem(DebugTextItem, FVector2D((CurrentPos.X + OptionalXOffset) - DisplayDebugManagerCvars::StoredDebugMenuOffset, CurrentPos.Y));

		const float EndOfStringPosX = CurrentPos.X + OptionalXOffset + (DebugTextItem.DrawnSize.X / Canvas->GetDPIScale());

		NextColumXPos = FMath::Max(NextColumXPos, EndOfStringPosX);
...
void FDisplayDebugManager::AddColumnIfNeeded()
{
	if (Canvas)
	{
		...
		if ((CurrentPos.Y + YStep) > Canvas->ClipY)
		{
			...
			// Checking and storing the max amount of columns created
			if (ColumnCount == 0)
			{
				DisplayDebugManagerCvars::MaxColumnsCreated = DisplayDebugManagerCvars::LastKnownColumnCount;
			}
			ColumnCount++;
			DisplayDebugManagerCvars::LastKnownColumnCount = ColumnCount;


			// Storing the targeted column offset
			if (DisplayDebugManagerCvars::ColumnOffsetIndex > 0)
			{
				if (ColumnCount == DisplayDebugManagerCvars::ColumnOffsetIndex)
				{
					DisplayDebugManagerCvars::StoredDebugMenuOffset = CurrentPos.X;
				}
			}
			else
			{
				DisplayDebugManagerCvars::StoredDebugMenuOffset = 0;
			}
		}
	}
}
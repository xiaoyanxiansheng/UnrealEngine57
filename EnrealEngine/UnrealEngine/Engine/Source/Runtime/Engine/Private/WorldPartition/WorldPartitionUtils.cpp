...
bool FWorldPartitionUtils::FSimulateCookedSession::SimulateCook(const FSimulateCookSessionParams& InParams)
{
	...
	for (TSharedPtr<FContentBundleEditor>& ContentBundle : ContentBundles)
	{
		if (ContentBundle->HasCookedContent())
		{
			...
			WorldPartition->RuntimeHash->InjectExternalStreamingObject(ExternalStreamingObject);
			InjectedStreamingObjects.Add(ExternalStreamingObject);
		}
	}

	// Simulate EDL injection
	UExternalDataLayerManager* EDLManager = WorldPartition->GetExternalDataLayerManager();
	for (const UDataLayerInstance* Instance : WorldPartition->GetWorld()->GetDataLayerManager()->GetDataLayerInstances())
	{	
		if (const UExternalDataLayerInstance* ExternalInstance = Cast<const UExternalDataLayerInstance>(Instance))
		{
			if (URuntimeHashExternalStreamingObjectBase* EDLStreamingObject = EDLManager->GetExternalStreamingObjectForDataLayerAsset(CastChecked<const UExternalDataLayerAsset>(ExternalInstance->GetAsset())))
			{
				WorldPartition->RuntimeHash->InjectExternalStreamingObject(EDLStreamingObject);
				InjectedStreamingObjects.Add(EDLStreamingObject);
			}
		}
	}
...
FWorldPartitionUtils::FSimulateCookedSession::~FSimulateCookedSession()
{
	if (CookContext)
	{
		// Clean-up External Streaming Objects injection
		for(URuntimeHashExternalStreamingObjectBase* InjectedStreamingObject : InjectedStreamingObjects)
		{
			WorldPartition->RuntimeHash->RemoveExternalStreamingObject(InjectedStreamingObject);
			// Trash external streaming object
			InjectedStreamingObject->Rename(*MakeUniqueObjectName(GetTransientPackage(), URuntimeHashExternalStreamingObjectBase::StaticClass()).ToString(), GetTransientPackage(), REN_NonTransactional | REN_DontCreateRedirectors | REN_DoNotDirty);
		}		

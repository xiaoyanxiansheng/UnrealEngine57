...
bool UWorldPartitionLevelStreamingDynamic::RequestLevel(UWorld* InPersistentWorld, bool bInAllowLevelLoadRequests, EReqLevelBlock InBlockPolicy)
{
	...
	UPackage* LevelPackage = (UPackage*)StaticFindObjectFast(UPackage::StaticClass(), nullptr, WorldAssetPackageFName, EFindObjectFlags::None, RF_NoFlags, EInternalObjectFlags::Garbage);
	...
	else if (bInAllowLevelLoadRequests)
	{
		...
		if (RuntimeLevel == nullptr)
		{
			...
			EWorldType::Type WorldType = RuntimeLevel->OwningWorld->WorldType;
			check(WorldType == EWorldType::PIE ||
				((IsRunningGame() || IsRunningDedicatedServer()) && WorldType == EWorldType::Game) ||
				((WorldType == EWorldType::Editor || WorldType == EWorldType::EditorPreview) && GetShouldBeVisibleInEditor()));

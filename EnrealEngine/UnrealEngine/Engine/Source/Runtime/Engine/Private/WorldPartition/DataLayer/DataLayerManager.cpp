...
#include UE_INLINE_GENERATED_CPP_BY_NAME(DataLayerManager)
...
void UDataLayerManager::DrawDataLayersStatus(UCanvas* Canvas, FVector2D& Offset) const
{
	...
	if (GDrawDataLayersLoadTime)
	{
		...
		auto TransformAndAppendDataLayers = [this](const TSet<FName>& InDataLayerNames, TArray<const UDataLayerInstance*>& OutDataLayerInstances)
		{
		...
		TransformAndAppendDataLayers(LoadedDataLayers, LoadingDataLayers);
		TransformAndAppendDataLayers(ActiveDataLayers, LoadingDataLayers);

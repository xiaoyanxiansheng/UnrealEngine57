...
void FContentBundle::DoInitialize()
{
	...
	if (FPackageName::DoesPackageExist(ExternalStreamingObjectPackagePath))
	{
		...
		{
			UE_LOG(LogContentBundle, Error, TEXT("%s Streaming package %s failed to load. No content will be injected."), *ContentBundle::Log::MakeDebugInfoString(*this), *GetExternalStreamingObjectPackagePath());
		}
	}
	else
	{
		UE_LOG(LogContentBundle, Log, TEXT("%s Has no content for the current World (this can happen if the ContentBundle has no Actors for the current World)."), *ContentBundle::Log::MakeDebugInfoString(*this));
	}
#endif

...
void ITokenizedMessageErrorHandler::OnUnsupportedHLODLayer(const IWorldPartitionActorDescInstanceView& ActorDescView, const UHLODLayer* HLODLayer, EHLODLayerUnsupportedReason Reason)
{
	FText LayerTypeText = HLODLayer ? StaticEnum<EHLODLayerType>()->GetDisplayNameTextByValue((int64)HLODLayer->GetLayerType()) : FText::FromString(TEXT("Unknown"));
	...
	case EHLODLayerUnsupportedReason::ActorClassUnsupportedByLayerType:
		...
		Message->AddToken(FTextToken::Create(LOCTEXT("TokenMessage_WorldPartition_Actor", "Actor")))
			->AddToken(FActorToken::Create(ActorDescView.GetActorSoftPath().ToString(), ActorDescView.GetGuid(), FText::FromString(GetActorName(ActorDescView))))
			->AddToken(FTextToken::Create(FText::Format(LOCTEXT("TokenMessage_WorldPartition_ActorClassUnsupportedByLayerType_Warning", "of class '{0}' is unsupported by HLOD Layer"), FText::FromString(ActorDescView.GetActorNativeClass()->GetName()))))
			->AddToken(FAssetNameToken::Create(ActorDescView.GetHLODLayer().ToString()))
			->AddToken(FTextToken::Create(FText::Format(LOCTEXT("TokenMessage_WorldPartition_ActorClassUnsupportedByLayerType_LayerType", "of type '{0}'"), LayerTypeText)));
		HandleTokenizedMessage(MoveTemp(Message));
		break;
	}
}

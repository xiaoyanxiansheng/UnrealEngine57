...
void FWorldPartitionClassDescRegistry::PrefetchClassDescs(const TArray<FTopLevelAssetPath>& InClassPaths)
{
	...
	// This function makes asset registry request during the cook. The tracker will register those requests as dependencies. The issue
	// is the requests are not always made by the same package so I end up with non deterministic dependencies.
	// Those dependencies are useless. This function makes a list of BP and native classes used by the actor descriptors. Whatever dependencies
	// map/generated packages and actors need should already be tracked by the actors/map themselves.
	UE::CookAssetRegistryAccessTracker::FIgnoreScope IgnoreAssetRegistryTracker;

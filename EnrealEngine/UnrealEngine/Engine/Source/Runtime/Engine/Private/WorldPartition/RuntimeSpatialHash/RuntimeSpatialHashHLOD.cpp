	...
	PartitionedActors.ForEachCells([&](const FSquare2DGridHelper::FGridLevel::FGridCell& GridCell)
	{
		...
		for (const FSquare2DGridHelper::FGridLevel::FGridCellDataChunk& GridCellDataChunk : GridCell.GetDataChunks())
		{
			...
			if (bShouldGenerateHLODActors)
			{
				...
				CreationParams.TargetWorld = WorldPartition->GetWorld();
				CreationParams.CellGuid = CellGuid;
				CreationParams.CellName = CellName;
				CreationParams.GetRuntimeGrid = [HLODLevel](const UHLODLayer* HLODLayer) 
				{ 
					PRAGMA_DISABLE_DEPRECATION_WARNINGS
					bool bIsSpatiallyLoaded = HLODLayer->IsSpatiallyLoaded();
					int32 CellSize = HLODLayer->GetCellSize();
					double LoadingRange = HLODLayer->GetLoadingRange();
					PRAGMA_ENABLE_DEPRECATION_WARNINGS

					return !bIsSpatiallyLoaded ? NAME_None : FName(UWorldPartitionRuntimeSpatialHash::GetHLODGridName(HLODLevel, CellSize, LoadingRange));
				};
...
static TMap<FName, FSpatialHashRuntimeGrid> CreateHLODGrids(TMap<UHLODLayer*, int32>& InHLODLayersLevel)
{
	...
	for (const TPair<UHLODLayer*, int32>& HLODLayerLevel : InHLODLayersLevel)
	{
		...
		const int32 HLODLevel = HLODLayerLevel.Value;

		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		const bool bIsSpatiallyLoaded = HLODLayer->IsSpatiallyLoaded();
		const FName GridName = HLODLayer->GetRuntimeGrid(HLODLayerLevel.Value);
		const int32 CellSize = HLODLayer->GetCellSize();
		const double LoadingRange = HLODLayer->GetLoadingRange();
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
		...
		if (bIsSpatiallyLoaded)
		{
			...
			{
				HLODGrid.CellSize = CellSize;
				HLODGrid.LoadingRange = LoadingRange;

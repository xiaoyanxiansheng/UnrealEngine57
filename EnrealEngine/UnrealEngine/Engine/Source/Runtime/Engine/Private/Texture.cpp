...
void UTexture::ValidateSettingsAfterImportOrEdit(bool * pRequiresNotifyMaterials)
{
	...
	if (Source.IsValid()) // we can have an empty source if the last source in a texture2d array is removed via the editor.
	{
		...
		// ?? bug ?? bIsPowerOfTwo does not consider ResizeDuringBuildX

		// note: NeverStream used to be turned on here if !bIsPowerOfTwo
		//	now we leave it off to reflect artist intent, and rely on it being set at runtime by IsPossibleToStream
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
		...
		{
			PRAGMA_DISABLE_DEPRECATION_WARNINGS
			if (CompositeTexture && CompositeTexture->CompositeTexture == this)
			{
				UE_LOG(LogTexture, Warning, TEXT("Cycle detected: Requested composite texture has this texture as its composite texture!"));
...
#if WITH_EDITORONLY_DATA
FGuid UTexture::BuildLightingGuid()
{
...
{
	// user flags requesting non-streaming :
	if ( NeverStream || LODGroup == TEXTUREGROUP_UI )
	{
		return false;
	}

	// we can only stream pow2 textures at the moment, check for that :

	if ( FApp::CanEverRender() ) // don't do this branch during Cook
	{
		// previously NeverStream was always set in the Editor for nonpow2 textures
		// now it is only set if user wants it
		// but nonpow2 textures must still not stream, so detect it here on the cooked data :
		FTexturePlatformData** PtrPlatformData = const_cast<UTexture *>(this)->GetRunningPlatformData();
		if ( PtrPlatformData && *PtrPlatformData )
		{
			FTexturePlatformData * PlatformData = *PtrPlatformData;

			bool bIsAsyncWorkComplete = true;
			#if WITH_EDITOR
			bIsAsyncWorkComplete = PlatformData->IsAsyncWorkComplete() && ! IsDefaultTexture();
			#endif

			// PlatformData->Mips.Num() == 0 during Cook
			if( bIsAsyncWorkComplete && PlatformData->Mips.Num() > 0 )
			{
				...
				uint32 SizeX = PlatformData->SizeX;
				uint32 SizeY = PlatformData->SizeY;
				uint32 SizeZ = PlatformData->GetNumSlices(); // == 6 for cubes
				if ( !FMath::IsPowerOfTwo(SizeX) || !FMath::IsPowerOfTwo(SizeY) )
				{
					return false;
				}
				if ( GetTextureClass() == ETextureClass::Volume && !FMath::IsPowerOfTwo(SizeZ) )
				{
					// Arrays can stream with no constraint on SizeZ
					return false;
				}
			}
		}
	...
	if ( Source.IsValid() )
	{
		// turn off streaming for non-pow2 sources :

		// note: this replicates the legacy logic, but it is not actually correct
		//	for example it does not account for options like ResizeToSpecific
		// more correct would be to use GetBuiltTextureSize here.

		// AreAllBlocksPowerOfTwo only checks XY :
		bool bIsPowerOfTwo = Source.AreAllBlocksPowerOfTwo();
		if ( ! FMath::IsPowerOfTwo(Source.GetVolumeSizeZ()) )
		{
			bIsPowerOfTwo = false;
		...
		{
			bIsPowerOfTwo = true;
			// @@ not correct for ETexturePowerOfTwoSetting::ResizeToSpecificResolution
		}
		if ( Source.IsLongLatCubemap() )
		{
			// longlat cube always generates pow2 output
...
{
	// returns the streaming method currently in use on this platform
	//	is not a general property of the texture
	//	can vary during texture lifetime (eg. PostInit streaming setup)

	if ( IsCurrentlyVirtualTextured() )
	{
		return TSM_VirtualStreamed;
	}
	else if ( IsStreamable() )
	{
		// StreamableRenderAsset.h
		// this will only be true after GetResourcePostInitState sets up lods
		//	see also IsPossibleToStream
		return TSM_Streamed;
	}
	else
	{
		return TSM_NotStreamed;
	}
}

...
/*static*/ ULocalPlayerSubsystem* USubsystemBlueprintLibrary::GetLocalPlayerSubsystem(UObject* ContextObject, TSubclassOf<ULocalPlayerSubsystem> Class)
{
	...
	else if (const APlayerController* PlayerController = Cast<APlayerController>(ContextObject))
	{
		LocalPlayer = Cast<ULocalPlayer>(PlayerController->Player);
	}
	else if (const APawn* Pawn = Cast<APawn>(ContextObject))
	{
		const APlayerController* PC = Cast<APlayerController>(Pawn->GetController());
		
		LocalPlayer = PC ? PC->GetLocalPlayer() : nullptr;
	}
	else if (const AActor* Actor = Cast<AActor>(ContextObject))
	{
		const APlayerController* PC = Cast<APlayerController>(Actor->GetInstigatorController());
		
		LocalPlayer = PC ? PC->GetLocalPlayer() : nullptr;
	}
	else if (const UActorComponent* ActorComponent = Cast<UActorComponent>(ContextObject))
	{
		const AActor* OwningActor = ActorComponent->GetOwner<AActor>();
		const APlayerController* PC = Cast<APlayerController>(OwningActor->GetInstigatorController());

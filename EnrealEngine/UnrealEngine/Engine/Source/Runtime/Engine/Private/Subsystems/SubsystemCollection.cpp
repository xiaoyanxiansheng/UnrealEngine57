...
void FSubsystemCollectionBase::RemoveSubsystemsInPackages(TConstArrayView<UPackage*> Packages)
{
	...
	{
		UE_CLOG(Pair.Value->bIsIterating, LogSubsystemCollection, Fatal,
			TEXT("FSubsystemCollectionBase::RemoveSubsystemsInPackages called while iterating subsystems of type %s"), *Pair.Key->GetPathName());
	}

	// release any subsystem class & instances that matches the packages being unloaded
	for (auto Iter = SubsystemArrayMap.CreateIterator(); Iter; ++Iter)
	{
		const UClass* KeyClass = Iter.Key();

		if (Packages.Contains(KeyClass->GetPackage()))
		{
			UE_LOG(LogSubsystemCollection, Verbose, TEXT("FSubsystemCollectionBase::RemoveSubsystemsInPackages: removing %s"), *KeyClass->GetFullName());
			
			for (USubsystem* Subsystem : Iter.Value()->Subsystems)
			{

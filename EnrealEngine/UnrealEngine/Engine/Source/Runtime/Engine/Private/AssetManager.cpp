...
int32 UAssetManager::ScanPathsForPrimaryAssets(FPrimaryAssetType PrimaryAssetType, const TArray<FString>& Paths, UClass* BaseClass, bool bHasBlueprintClasses, bool bIsEditorOnly, bool bForceSynchronousScan)
{
	...
	if (!FoundType)
	{
		UE_LOG(LogAssetManager, VeryVerbose, TEXT("UAssetManager::ScanPathsForPrimaryAssets adding TypeData to AssetTypeMap: '%s', (Paths=%s%s)"),
...
void UAssetManager::OnAssetStateChangeCompleted(TSharedPtr<FStreamableHandle> BoundHandle, FPrimaryAssetId PrimaryAssetId)
{
	...
	if (NameData)
	{
		...
		if (!NameData->PendingState.GetHandle().IsValid() || NameData->PendingState.GetHandle() == BoundHandle)
		{
			...
			{
				const UE::UnifiedError::StreamableManager::FStreamableManagerAdditionalContext* AdditionalContext = BoundHandle->GetError()->GetErrorContext<UE::UnifiedError::StreamableManager::FStreamableManagerAdditionalContext>();

				if (AdditionalContext)
				{
					UE_LOG(LogAssetManager, Warning, TEXT("OnAssetStateChangeCompleted: Received error (%s) loading asset! Primary asset id:(%s) erroring asset:%s"), *BoundHandle->GetError()->GetErrorMessage(true).ToString(), *PrimaryAssetId.ToString(), *AdditionalContext->MissingObject);
				}
				else
				{
					UE_LOG(LogAssetManager, Warning, TEXT("OnAssetStateChangeCompleted: Received error (%s) loading asset! Primary asset id:(%s)"), *BoundHandle->GetError()->GetErrorMessage(true).ToString(), *PrimaryAssetId.ToString());
				}
...
bool UAssetManager::VerifyCanCookPackage(UE::Cook::ICookInfo* CookInfo, FName PackageName, bool bLogError) const
{
	...
	if (!bRetVal && bLogError)
	{
		...
		{
			UE_LOG(LogAssetManager, Display, TEXT("  %s"), *PrimaryAssetPair.Key.ToString());
		}

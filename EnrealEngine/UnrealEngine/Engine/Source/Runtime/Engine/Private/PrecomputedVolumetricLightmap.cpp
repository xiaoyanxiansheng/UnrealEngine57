...
void FVolumetricLightmapDataLayer::CreateTexture(FRHICommandListBase& RHICmdList, FIntVector Dimensions)
{
	const FRHITextureCreateDesc Desc =
		...
		.SetInitActionBulkData(this)
		.SetClassName(VolumetricLightmapDataLayerName);

	Texture = RHICmdList.CreateTexture(Desc);
}

void FVolumetricLightmapDataLayer::CreateTargetTexture(FRHICommandListBase& RHICmdList, FIntVector Dimensions)
{
	const FRHITextureCreateDesc Desc =
		FRHITextureCreateDesc::Create3D(TEXT("VolumetricLightmap"), Dimensions, Format)
		.SetFlags(ETextureCreateFlags::ShaderResource | ETextureCreateFlags::UAV)
...
{
	CreateTexture(FRHICommandListImmediate::Get(), Dimensions);
}

void FVolumetricLightmapDataLayer::CreateTargetTexture(FIntVector Dimensions)
{
	CreateTargetTexture(FRHICommandListImmediate::Get(), Dimensions);
}
...
ENGINE_API void FPrecomputedVolumetricLightmapData::InitRHI(FRHICommandListBase& RHICmdList)
{
	if (GetFeatureLevel() >= ERHIFeatureLevel::SM5)
	{
		...
		{
			IndirectionTexture.CreateTexture(RHICmdList, IndirectionTextureDimensions);
		}

		if (BrickDataDimensions.GetMax() > 0)
		{
			BrickData.AmbientVector.CreateTexture(RHICmdList, BrickDataDimensions);

			for (int32 i = 0; i < UE_ARRAY_COUNT(BrickData.SHCoefficients); i++)
			{
				BrickData.SHCoefficients[i].CreateTexture(RHICmdList, BrickDataDimensions);
			}

			if (BrickData.SkyBentNormal.Data.Num() > 0)
			{
				BrickData.SkyBentNormal.CreateTexture(RHICmdList, BrickDataDimensions);
			}

			BrickData.DirectionalLightShadowing.CreateTexture(RHICmdList, BrickDataDimensions);
		}
...
{
	InitRHIForSubLevelResources(FRHICommandListImmediate::Get());
}
...
ENGINE_API void FPrecomputedVolumetricLightmapData::HandleDataMovementInAtlas(int32 OldOffset, int32 NewOffset)
{
	...
	if (IndirectionTextureDimensions.GetMax() > 0)
	{
		...
		for (FPrecomputedVolumetricLightmapData* SceneData : SceneDataAdded)
		{
			if (SceneData->IndirectionTexture.Texture.IsValid())
			{
				...
				NewIndirectionTexture.CreateTargetTexture(RHICmdList, IndirectionTextureDimensions);
	...
	{
		InitRHIForSubLevelResources(RHICmdList);
...
void FVolumetricLightmapBrickAtlas::Insert(int32 Index, FPrecomputedVolumetricLightmapData* Data)
{
	...
	else
	{
		...
		{
			TextureSet.SkyBentNormal.CreateTargetTexture(RHICmdList, TextureSet.BrickDataDimensions);

...
void UFont::BeginDestroy()
{
	if (FontCacheType == EFontCacheType::Runtime && FSlateApplication::IsInitialized())
	{
		...
		if (SlateRenderer)
		{
			...
			FontCache->FlushCompositeFont(GetInternalCompositeFont());
...
void UFont::PostLoad()
{
	...
	if (FontCacheType == EFontCacheType::Runtime)
	{
		...
		FCompositeFont& MutableInternalCompositeFont = GetMutableInternalCompositeFont();
		for (FTypefaceEntry& TypefaceEntry : MutableInternalCompositeFont.DefaultTypeface.Fonts)
		{
			UpgradeLegacyData(TypefaceIndex, TypefaceEntry.Name, TypefaceEntry.Font);
		}

		for (FCompositeSubFont& SubFont : MutableInternalCompositeFont.SubTypefaces)
		{
			++TypefaceIndex;
...
FSlateFontInfo UFont::GetLegacySlateFontInfo() const
{
...
void UFont::GetResourceSizeEx(FResourceSizeEx& CumulativeResourceSize)
{
	...
	if (FontCacheType == EFontCacheType::Runtime && CumulativeResourceSize.GetResourceSizeMode() == EResourceSizeMode::EstimatedTotal)
	{
		...
		const FCompositeFont& InternalCompositeFont = GetInternalCompositeFont();
		GetTypefaceResourceSize(InternalCompositeFont.DefaultTypeface);
		for (const FCompositeSubFont& SubTypeface : InternalCompositeFont.SubTypefaces)
		{
...
const FCompositeFont* UFont::GetCompositeFont() const
{
	if (FontCacheType == EFontCacheType::Runtime)
	{
		switch (RuntimeFontSource)
		{
			case ERuntimeFontSource::Asset:
				return &GetInternalCompositeFont();

			case ERuntimeFontSource::CoreStyleDefault:
				// Note: Returning this as a raw pointer is safe (for use on the stack), as FLegacySlateFontInfoCache keeps the FCompositeFont alive as long as Slate is initialized
				return &FCoreStyle::GetDefaultFont().Get();

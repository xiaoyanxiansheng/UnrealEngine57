...
namespace UE::DataTableJSON
{
	...
	/**
	 * Console variable to be able to run the previous implementation of the exporter
	 * in case issues get reported and validate potential regression.
	 * We could eventually get rid of the FieldIterator implementation.
	 */
	bool bExportUsingPropertyVisitor = true;
	static FAutoConsoleVariableRef CVarRuntimeValidationContext(
		TEXT("DataTableJSON.ExportUsingPropertyVisitor"),
		bExportUsingPropertyVisitor,
		TEXT("True indicates that the PropertyVisitor will be used to export the data table row including support for new types like instanced struct."
				" Otherwise the legacy implementation based on the FieldIterator is used.")
	);
...
{
	if (UE::DataTableJSON::bExportUsingPropertyVisitor)
	{
		return InStruct ? WriteStructUsingVisitor(InStruct, InStructData, FieldToSkip) : false;
	}

	return WriteStructUsingFieldIterator(InStruct, InStructData, FieldToSkip);
}

template <typename CharType>
bool TDataTableExporterJSON<CharType>::WriteStructUsingFieldIterator(
	const UScriptStruct* InStruct
	, const void* InStructData
	, const FString* FieldToSkip)
{
...
bool TDataTableExporterJSON<CharType>::WriteStructEntry(const void* InRowData, const FProperty* InProperty, const void* InPropertyData)
{
	...
	{
		if (EnumHasAnyFlags(DTExportFlags, EDataTableExportFlags::UseJsonObjectsForStructs))
		{
			JsonWriter->WriteObjectStart(Identifier);
...
{
	using namespace UE::DataTableJSON;
...
template <typename CharType>
bool TDataTableExporterJSON<CharType>::WriteStructUsingVisitor(TNotNull<const UScriptStruct*> InStruct, const void* InStructData
	, const FString* FieldToSkip)
{
	using namespace UE::DataTableJSON;

	// Keep a stack of identifier overrides since when entering arrays we don't want struct to output their identifier
	// We always push override for structs and arrays so an empty override list or an unset value on top of the stack
	// indicates that no override is effective and the property export name should be used.
	TArray<TOptional<FString>> IdentifierOverrides;

	auto PushFunc = [this, &IdentifierOverrides](const FPropertyVisitorInfo& InVisitorInfo)
		{
			const FProperty* Property = InVisitorInfo.Property;
			const FString Identifier = DataTableUtils::GetPropertyExportName(Property, DTExportFlags);

			if (InVisitorInfo.PropertyInfo == EPropertyVisitorInfoType::MapKey
				|| InVisitorInfo.PropertyInfo == EPropertyVisitorInfoType::MapValue)
			{
				// Special case for map keys since we won't write them but use their values as string when writing the map values.
				// In order to do so we don't push a new identifier, and we will set it when visiting the key value.
				// Then we can reset when popping the values.
				return;
			}

			// Process static array first to properly handle arrays of structs
			if (Property->ArrayDim != 1)
			{
				ensure(InVisitorInfo.PropertyInfo == EPropertyVisitorInfoType::StaticArrayIndex);
				if (InVisitorInfo.Index == 0)
				{
					JsonWriter->WriteArrayStart(Identifier);

					// Push override (empty identifier) for array elements
					IdentifierOverrides.AddDefaulted_GetRef().Emplace(FString{});
				}
			}
			...
			else if (Property->IsA<FStructProperty>())
			{
				if (EnumHasAnyFlags(DTExportFlags, EDataTableExportFlags::UseJsonObjectsForStructs))
				{
					if (IdentifierOverrides.Num() && IdentifierOverrides.Top().IsSet())
					{
						const FString* Override = IdentifierOverrides.Top().GetValue().IsEmpty() ? nullptr : &IdentifierOverrides.Top().GetValue();
						WriteJSONObjectStartWithOptionalIdentifier<CharType>(*JsonWriter, Override);
					}
					else
					{
						JsonWriter->WriteObjectStart(Identifier);
				...
				// Push a "no override" entry when entering a struct
				IdentifierOverrides.AddDefaulted();
			}
		};

	auto PopFunc = [this, &IdentifierOverrides](const FPropertyVisitorInfo& InVisitorInfo)
		{
			// Map keys don't do anything when pushed in the visitor scope, so nothing to do here
			if (InVisitorInfo.PropertyInfo == EPropertyVisitorInfoType::MapKey)
			{
				return;
			}

			// Map element is done after writing its value; we can reset the override set by the key and close its WriteObjectStart.
			if (InVisitorInfo.PropertyInfo == EPropertyVisitorInfoType::MapValue)
			{
				JsonWriter->WriteObjectEnd();
				return;
			}
			...
			// Process static array last to properly close an array of structs
			if (Property->ArrayDim != 1)
			{
				ensure(InVisitorInfo.PropertyInfo == EPropertyVisitorInfoType::StaticArrayIndex);
				if (InVisitorInfo.Index + 1 == Property->ArrayDim)
				{
					JsonWriter->WriteArrayEnd();
					IdentifierOverrides.Pop();
				}
	...
	// Visitor
	FPropertyVisitorPath Path(PushFunc, PopFunc);
	FPropertyVisitorData Data(const_cast<void*>(InStructData), /*ParentStructData*/nullptr);
	FPropertyVisitorContext Context(Path, Data, FPropertyVisitorContext::EScope::All);

	InStruct->Visit(Context,
		[FieldToSkip, this, &IdentifierOverrides](const FPropertyVisitorContext& InContext)-> EPropertyVisitorControlFlow
		{
			const FProperty* Property = InContext.Path.Top().Property;
			if (FieldToSkip && *FieldToSkip == DataTableUtils::GetPropertyExportName(Property, DTExportFlags))
			{
				// Skip this field
				return EPropertyVisitorControlFlow::StepOver;
			}

			// Special case for map when we use the key to set the identifier for the values
			if (InContext.Path.Top().PropertyInfo == EPropertyVisitorInfoType::MapKey)
			{
				const FString Identifier = (DataTableUtils::GetPropertyValueAsString(Property, static_cast<uint8*>(InContext.Data.PropertyData), DTExportFlags));
				JsonWriter->WriteObjectStart(Identifier);
				return EPropertyVisitorControlFlow::StepOver;
			}

			// Use override identifier if one is set on the stack
			if (IdentifierOverrides.Num() && IdentifierOverrides.Top().IsSet())
			{
				const FString* Override = IdentifierOverrides.Top().GetValue().IsEmpty() ? nullptr : &IdentifierOverrides.Top().GetValue();
				return WriteVisitedEntry(Property, InContext.Data.PropertyData, Override);
			}

			const FString ExportName = DataTableUtils::GetPropertyExportName(Property, DTExportFlags);
			return WriteVisitedEntry(Property, InContext.Data.PropertyData, &ExportName);
		});
...
template<typename CharType>
EPropertyVisitorControlFlow TDataTableExporterJSON<CharType>::WriteVisitedEntry(TNotNull<const FProperty*> InProperty, const void* InValueAddress, const FString* InIdentifier)
{
	using namespace UE::DataTableJSON;

	if (const FEnumProperty* EnumProp = CastField<const FEnumProperty>(InProperty))
	{
		const FString PropertyValue = DataTableUtils::GetPropertyValueAsStringDirect(EnumProp, (uint8*)InValueAddress, DTExportFlags);
		WriteJSONValueWithOptionalIdentifier<CharType>(*JsonWriter, InIdentifier, PropertyValue);
	}
	else if (const FNumericProperty* NumProp = CastField<const FNumericProperty>(InProperty))
	{
		if (NumProp->IsEnum())
		{
			const FString PropertyValue = DataTableUtils::GetPropertyValueAsStringDirect(InProperty, (uint8*)InValueAddress, DTExportFlags);
			WriteJSONValueWithOptionalIdentifier<CharType>(*JsonWriter, InIdentifier, PropertyValue);
		}
		else if (NumProp->IsInteger())
		{
			const int64 PropertyValue = NumProp->GetSignedIntPropertyValue(InValueAddress);
			WriteJSONValueWithOptionalIdentifier<CharType>(*JsonWriter, InIdentifier, PropertyValue);
		}
		else if (NumProp->IsA<FFloatProperty>())
		{
			const float PropertyValue = (float)NumProp->GetFloatingPointPropertyValue(InValueAddress);
			WriteJSONValueWithOptionalIdentifier<CharType>(*JsonWriter, InIdentifier, PropertyValue);
		}
		else
		{
			const double PropertyValue = NumProp->GetFloatingPointPropertyValue(InValueAddress);
			WriteJSONValueWithOptionalIdentifier<CharType>(*JsonWriter, InIdentifier, PropertyValue);
		}
	}
	else if (const FBoolProperty* BoolProp = CastField<const FBoolProperty>(InProperty))
	{
		const bool PropertyValue = BoolProp->GetPropertyValue(InValueAddress);
		WriteJSONValueWithOptionalIdentifier<CharType>(*JsonWriter, InIdentifier, PropertyValue);
	}
	else if (InProperty->IsA<const FStructProperty>())
	{
		if (EnumHasAnyFlags(DTExportFlags, EDataTableExportFlags::UseJsonObjectsForStructs))
		{
...
bool FDataTableImporterJSON::ReadStruct(const TSharedRef<FJsonObject>& InParsedObject, UScriptStruct* InStruct, const FName InRowName, void* InStructData)
{
	...
	for (TFieldIterator<FProperty> It(InStruct); It; ++It)
	{
		...
		{
			const TCHAR* const ParsedPropertyType = UE::DataTableJSON::JSONTypeToString(ParsedPropertyValue->Type);
...
{
	const TCHAR* const ParsedPropertyType = UE::DataTableJSON::JSONTypeToString(InParsedPropertyValue->Type);

...
namespace UE::Blueprint::Private
{
	...
	// Convert the BP-exposed Draw Debug version of Scene Depth Priority Group to the corresponding engine type enum value
	ESceneDepthPriorityGroup ConvertSceneDepthPriorityToEngineType(EDrawDebugSceneDepthPriorityGroup DepthPriority)
	{
		return (DepthPriority == EDrawDebugSceneDepthPriorityGroup::World)
			? ESceneDepthPriorityGroup::SDPG_World : ESceneDepthPriorityGroup::SDPG_Foreground;
	}
}
...
/** Draw a debug line */
void UKismetSystemLibrary::DrawDebugLine(const UObject* WorldContextObject, FVector const LineStart, FVector const LineEnd, FLinearColor Color, float LifeTime, float Thickness, EDrawDebugSceneDepthPriorityGroup DepthPriority)
{
#if ENABLE_DRAW_DEBUG
	if (UWorld* World = GEngine->GetWorldFromContextObject(WorldContextObject, EGetWorldErrorMode::LogAndReturnNull))
	{
		::DrawDebugLine(World, LineStart, LineEnd, Color.ToFColor(true), false, LifeTime, UE::Blueprint::Private::ConvertSceneDepthPriorityToEngineType(DepthPriority), Thickness);
	}
#endif
}

/** Draw a debug circle */
void UKismetSystemLibrary::DrawDebugCircle(const UObject* WorldContextObject,FVector Center, float Radius, int32 NumSegments, FLinearColor LineColor, float LifeTime, float Thickness, FVector YAxis, FVector ZAxis, bool bDrawAxis, EDrawDebugSceneDepthPriorityGroup DepthPriority)
{ 
#if ENABLE_DRAW_DEBUG
	if (UWorld* World = GEngine->GetWorldFromContextObject(WorldContextObject, EGetWorldErrorMode::LogAndReturnNull))
	{
		::DrawDebugCircle(World, Center, Radius, NumSegments, LineColor.ToFColor(true), false, LifeTime, UE::Blueprint::Private::ConvertSceneDepthPriorityToEngineType(DepthPriority), Thickness, YAxis, ZAxis, bDrawAxis);
	}
#endif
}

/** Draw a debug point */
void UKismetSystemLibrary::DrawDebugPoint(const UObject* WorldContextObject, FVector const Position, float Size, FLinearColor PointColor, float LifeTime, EDrawDebugSceneDepthPriorityGroup DepthPriority)
{
#if ENABLE_DRAW_DEBUG
	if (UWorld* World = GEngine->GetWorldFromContextObject(WorldContextObject, EGetWorldErrorMode::LogAndReturnNull))
	{
		::DrawDebugPoint(World, Position, Size, PointColor.ToFColor(true), false, LifeTime, UE::Blueprint::Private::ConvertSceneDepthPriorityToEngineType(DepthPriority));
	}
#endif
}

/** Draw directional arrow, pointing from LineStart to LineEnd. */
void UKismetSystemLibrary::DrawDebugArrow(const UObject* WorldContextObject, FVector const LineStart, FVector const LineEnd, float ArrowSize, FLinearColor Color, float LifeTime, float Thickness, EDrawDebugSceneDepthPriorityGroup DepthPriority)
{
#if ENABLE_DRAW_DEBUG
	UWorld* World = GEngine->GetWorldFromContextObject(WorldContextObject, EGetWorldErrorMode::LogAndReturnNull);
	if (World != nullptr)
	{
		::DrawDebugDirectionalArrow(World, LineStart, LineEnd, ArrowSize, Color.ToFColor(true), false, LifeTime, UE::Blueprint::Private::ConvertSceneDepthPriorityToEngineType(DepthPriority), Thickness);
	}
#endif
}

/** Draw a debug box */
void UKismetSystemLibrary::DrawDebugBox(const UObject* WorldContextObject, FVector const Center, FVector Extent, FLinearColor Color, const FRotator Rotation, float LifeTime, float Thickness, EDrawDebugSceneDepthPriorityGroup DepthPriority)
{
#if ENABLE_DRAW_DEBUG
	if (UWorld* World = GEngine->GetWorldFromContextObject(WorldContextObject, EGetWorldErrorMode::LogAndReturnNull))
	{
		...
		{
			::DrawDebugBox(World, Center, Extent, Color.ToFColor(true), false, LifeTime, UE::Blueprint::Private::ConvertSceneDepthPriorityToEngineType(DepthPriority), Thickness);
		}
		else
		{
			::DrawDebugBox(World, Center, Extent, Rotation.Quaternion(), Color.ToFColor(true), false, LifeTime, UE::Blueprint::Private::ConvertSceneDepthPriorityToEngineType(DepthPriority), Thickness);
		}
	}
#endif
}
...
void UKismetSystemLibrary::DrawDebugCoordinateSystem(const UObject* WorldContextObject, FVector const AxisLoc, FRotator const AxisRot, float Scale, float LifeTime, float Thickness, EDrawDebugSceneDepthPriorityGroup DepthPriority)
{
#if ENABLE_DRAW_DEBUG
	if (UWorld* World = GEngine->GetWorldFromContextObject(WorldContextObject, EGetWorldErrorMode::LogAndReturnNull))
	{
		::DrawDebugCoordinateSystem(World, AxisLoc, AxisRot, Scale, false, LifeTime, UE::Blueprint::Private::ConvertSceneDepthPriorityToEngineType(DepthPriority), Thickness);
	}
#endif
}

/** Draw a debug sphere */
void UKismetSystemLibrary::DrawDebugSphere(const UObject* WorldContextObject, FVector const Center, float Radius, int32 Segments, FLinearColor Color, float LifeTime, float Thickness, EDrawDebugSceneDepthPriorityGroup DepthPriority)
{
#if ENABLE_DRAW_DEBUG
	if (UWorld* World = GEngine->GetWorldFromContextObject(WorldContextObject, EGetWorldErrorMode::LogAndReturnNull))
	{
		::DrawDebugSphere(World, Center, Radius, Segments, Color.ToFColor(true), false, LifeTime, UE::Blueprint::Private::ConvertSceneDepthPriorityToEngineType(DepthPriority), Thickness);
	}
#endif
}

/** Draw a debug cylinder */
void UKismetSystemLibrary::DrawDebugCylinder(const UObject* WorldContextObject, FVector const Start, FVector const End, float Radius, int32 Segments, FLinearColor Color, float LifeTime, float Thickness, EDrawDebugSceneDepthPriorityGroup DepthPriority)
{
#if ENABLE_DRAW_DEBUG
	if (UWorld* World = GEngine->GetWorldFromContextObject(WorldContextObject, EGetWorldErrorMode::LogAndReturnNull))
	{
		::DrawDebugCylinder(World, Start, End, Radius, Segments, Color.ToFColor(true), false, LifeTime, UE::Blueprint::Private::ConvertSceneDepthPriorityToEngineType(DepthPriority), Thickness);
	}
#endif
}
...
void UKismetSystemLibrary::DrawDebugConeInDegrees(const UObject* WorldContextObject, FVector const Origin, FVector const Direction, float Length, float AngleWidth, float AngleHeight, int32 NumSides, FLinearColor Color, float LifeTime, float Thickness, EDrawDebugSceneDepthPriorityGroup DepthPriority)
{
#if ENABLE_DRAW_DEBUG
	if (UWorld* World = GEngine->GetWorldFromContextObject(WorldContextObject, EGetWorldErrorMode::LogAndReturnNull))
	{
		::DrawDebugCone(World, Origin, Direction, Length, FMath::DegreesToRadians(AngleWidth), FMath::DegreesToRadians(AngleHeight), NumSides, Color.ToFColor(true), false, LifeTime, UE::Blueprint::Private::ConvertSceneDepthPriorityToEngineType(DepthPriority), Thickness);
	}
#endif
}

/** Draw a debug capsule */
void UKismetSystemLibrary::DrawDebugCapsule(const UObject* WorldContextObject, FVector const Center, float HalfHeight, float Radius, const FRotator Rotation, FLinearColor Color, float LifeTime, float Thickness, EDrawDebugSceneDepthPriorityGroup DepthPriority)
{
#if ENABLE_DRAW_DEBUG
	if (UWorld* World = GEngine->GetWorldFromContextObject(WorldContextObject, EGetWorldErrorMode::LogAndReturnNull))
	{
		::DrawDebugCapsule(World, Center, HalfHeight, Radius, Rotation.Quaternion(), Color.ToFColor(true), false, LifeTime, UE::Blueprint::Private::ConvertSceneDepthPriorityToEngineType(DepthPriority), Thickness);
	}
#endif
}
...
/** Draws a debug plane. */
void UKismetSystemLibrary::DrawDebugPlane(const UObject* WorldContextObject, FPlane const& P, FVector const Loc, float Size, FLinearColor Color, float LifeTime, EDrawDebugSceneDepthPriorityGroup DepthPriority)
{
#if ENABLE_DRAW_DEBUG
	if (UWorld* World = GEngine->GetWorldFromContextObject(WorldContextObject, EGetWorldErrorMode::LogAndReturnNull))
	{
		::DrawDebugSolidPlane(World, P, Loc, Size, Color.ToFColor(true), false, LifeTime, UE::Blueprint::Private::ConvertSceneDepthPriorityToEngineType(DepthPriority));
	}
#endif
}
...
void UKismetSystemLibrary::DelayUntilNextTick(const UObject* WorldContextObject, FLatentActionInfo LatentInfo)
{
	...
	{
		FLatentActionManager& LatentActionManager = World->GetLatentActionManager();
		if (LatentActionManager.FindExistingAction<FDelayUntilNextTickAction>(LatentInfo.CallbackTarget, LatentInfo.UUID) == NULL)
		{
...
void UKismetSystemLibrary::DelayUntilNextFrame(const UObject* WorldContextObject, FLatentActionInfo LatentInfo)
{
	if (UWorld* World = GEngine->GetWorldFromContextObject(WorldContextObject, EGetWorldErrorMode::LogAndReturnNull))
	{
		FLatentActionManager& LatentActionManager = World->GetLatentActionManager();
		if (LatentActionManager.FindExistingAction<FDelayUntilNextFrameAction>(LatentInfo.CallbackTarget, LatentInfo.UUID) == NULL)
		{
			LatentActionManager.AddNewAction(LatentInfo.CallbackTarget, LatentInfo.UUID, new FDelayUntilNextFrameAction(LatentInfo));
		}
	}
}
...
void UKismetSystemLibrary::LoadAssets(const UObject* WorldContextObject, TArray<TSoftObjectPtr<UObject>> Assets, FOnAssetsLoaded OnLoaded, FLatentActionInfo LatentInfo)
{
	struct FLoadAssetsAction : public FPendingLatentAction
	{
	public:
		...
		int32 OutputLink;
		FWeakObjectPtr CallbackTarget;
		UKismetSystemLibrary::FOnAssetsLoaded OnLoadedCallback;

		FLoadAssetsAction(const TArray<FSoftObjectPath>& InSoftObjectPaths, UKismetSystemLibrary::FOnAssetsLoaded InOnLoadedCallback, const FLatentActionInfo& InLatentInfo)
			: SoftObjectPaths(InSoftObjectPaths)
			, ExecutionFunction(InLatentInfo.ExecutionFunction)
			, OutputLink(InLatentInfo.Linkage)
			, CallbackTarget(InLatentInfo.CallbackTarget)
			, OnLoadedCallback(InOnLoadedCallback)
		...
		virtual FString GetDescription() const override
		{
			FString AggregatedSoftObjectPaths;

			for (auto Path = SoftObjectPaths.CreateConstIterator(); Path; ++Path)
			{
				AggregatedSoftObjectPaths.Append(Path->ToString());

				if (*Path != SoftObjectPaths.Last())
				{
					AggregatedSoftObjectPaths.Append("\n");
				}
			}

			return FString::Printf(TEXT("Load Assets Action: %s"), *AggregatedSoftObjectPaths);
		}
#endif
		...
		void OnLoaded() 
		{
			...
			for (FSoftObjectPath& Path : SoftObjectPaths)
			{
				LoadedObjects.Add(Path.ResolveObject());
			}

			OnLoadedCallback.ExecuteIfBound(LoadedObjects);
		}
	};
	...
	if (UWorld* World = GEngine->GetWorldFromContextObject(WorldContextObject, EGetWorldErrorMode::LogAndReturnNull))
	{
		...
		TArray<FSoftObjectPath> SoftObjectPathsToLoad;

		for (TSoftObjectPtr<UObject> SoftObjectPtr : Assets)
		{
			SoftObjectPathsToLoad.Add(SoftObjectPtr.ToSoftObjectPath());
		}

		// We always spawn a new load even if this node already queued one, the outside node handles this case
		FLoadAssetsAction* NewAction = new FLoadAssetsAction(SoftObjectPathsToLoad, OnLoaded, LatentInfo);
		LatentManager.AddNewAction(LatentInfo.CallbackTarget, LatentInfo.UUID, NewAction);
	}
}

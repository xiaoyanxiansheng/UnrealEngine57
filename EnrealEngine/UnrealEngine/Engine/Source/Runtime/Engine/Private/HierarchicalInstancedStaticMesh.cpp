...
static TAutoConsoleVariable<int32> CVarRayTracingHISM(
	...
	1,
	TEXT("Defines if HierarchicalInstancedStaticMesh are added to the ray tracing scene.\n")
	TEXT(" 0: raytracing cached MDCs for HISMs are not created.\n")
	TEXT(" 1: raytracing cached MDCs for HISMs are created and instances are added to the RayTracing scene (default).\n")
	TEXT(" 2: HISMss are runtime culled from the RayTracing scene, but keeps the cached state for fast toggling."));
#endif
...
, bDitheredLODTransitions(InComponent->SupportsDitheredLODTransitions(GetScene().GetShaderPlatform()))
...
{
	return BuildComponentInstanceData(GetFeatureLevelShaderPlatform_Checked(FeatureLevel), OutData);
}

void UHierarchicalInstancedStaticMeshComponent::BuildComponentInstanceData(EShaderPlatform InShaderPlatform, FInstanceUpdateComponentDesc& OutData)
{
	LLM_SCOPE(ELLMTag::InstancedMesh);

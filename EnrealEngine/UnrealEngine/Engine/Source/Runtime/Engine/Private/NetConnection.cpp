...
static FAutoConsoleVariableRef CVarBufferBloatCooldownTimeInMS(
	TEXT("netEmulation.BufferBloatCooldownTimeInMS"),
	BufferBloatCooldownTimeInMS,
	TEXT("Cooldown time in milliseconds during which buffer bloat won't be active."),
	ECVF_Default);
...
void UChildConnection::CloseAndRemoveChild(UE::Net::ECloseChildFlags Flags)
{
	UE_LOG(LogNet, Log, TEXT("[%s:%s] Closing child connection %s."), *GetNameSafe(GetDriver()), *GetNameSafe(Parent), *GetName());
	
	if (EnumHasAnyFlags(Flags, UE::Net::ECloseChildFlags::SendCloseMessage))
	{
		if (PlayerController)
		{
			int32 NetPlayerIndexParam = static_cast<int32>(PlayerController->NetPlayerIndex);
			FNetControlMessage<NMT_CloseChildConnection>::Send(Parent, NetPlayerIndexParam);

			UE_LOG(LogNet, Log, TEXT("[%s:%s] Sending NMT_CloseChildConnection with NetPlayerIndex %d."), 
				   *GetNameSafe(GetDriver()), *GetNameSafe(Parent), NetPlayerIndexParam);
		}
		else
		{
			UE_LOG(LogNet, Warning, TEXT("[%s:%s] Cannot send NMT_CloseChildConnection because there's no valid player controller."), 
				   *GetNameSafe(GetDriver()), *GetNameSafe(Parent));
		}
	}
	...
	if (PlayerController)
	{
		if (ULocalPlayer* LocalPlayer = PlayerController->GetLocalPlayer())
		{
			if (UWorld* World = GetWorld())
			{
				...
				{
					// A local player may be removed from the game before the proxy driver is cleaned up in the case where the
					// game is shutting down or closing.
					const bool bIsRegisteredLocalPlayer = GameInstance->GetLocalPlayers().Find(LocalPlayer) != INDEX_NONE;

					if (bIsRegisteredLocalPlayer)
					{
						UE_LOG(LogNet, Log, TEXT("[%s:%s] Remove local player %s."), *GetNameSafe(GetDriver()), *GetNameSafe(Parent), *GetNameSafe(LocalPlayer));
	...
	if (Parent)
	{
		...
		if (EnumHasAnyFlags(Flags, UE::Net::ECloseChildFlags::DestroyActors))
		{
			CleanUp();
		}
	}
...
FNetLevelVisibilityTransactionId UNetConnection::UpdateLevelStreamStatusChangedTransactionId(const ULevelStreaming* LevelObject, const FName PackageName, bool bShouldBeVisible)
{
	...
	if (bShouldBeVisible == false)
	{
		...
		if (UReplicationSystem* ReplicationSystem = Driver->GetReplicationSystem())
		{
			...
			{
				const UObjectReplicationBridge* ReplicationBridge = ReplicationSystem->GetReplicationBridge();
...
void UNetConnection::UpdateLevelVisibilityInternal(const FUpdateLevelVisibilityLevelInfo& LevelVisibility)
{
	...
	else
	{
		...
		for (auto It = ActorChannels.CreateIterator(); It; ++It)
		{
			...
			{
				It.RemoveCurrent(); // Remove safely here since Channel->Close will remove unsafely
...
void UNetConnection::SetClientWorldPackageName(FName NewClientWorldPackageName)
{
	...
	if (Driver->IsUsingIrisReplication())
	{
		Driver->UpdateGroupFilterStatusForAllLevelsForConnection(GetConnectionHandle());
	}
	else
	{
...
void UNetConnection::PostTickDispatch()
{
	...
	if (!IsInternalAck())
	{
		...
		// Make sure to clear the AckPacketId for the next frame
		OutFirstAckPacketId = -1;
	}
...
void UNetConnection::FlushNet(bool bIgnoreSimulation)
{
	...
	if (SendBuffer.GetNumBits() || HasDirtyAcks || ( Driver->GetElapsedTime() - LastSendTime > Driver->KeepAliveTime && !IsInternalAck() && GetConnectionState() != USOCK_Closed))
	{
		...
		{
			bWasPacketEmulated = CheckOutgoingPacketEmulation(PacketSentTimeInS, Traits);
		}
...
#if DO_ENABLE_NET_TEST
bool UNetConnection::CheckOutgoingPacketEmulation(double CurrentTimeInS, FOutPacketTraits& Traits)
{
	if (ShouldDropOutgoingPacketForLossSimulation(SendBuffer.GetNumBits()))
	{
		UE_LOG(LogNet, VeryVerbose, TEXT("Dropping outgoing packet at %f"), CurrentTimeInS);
		return true;
	}

	if (PacketSimulationSettings.PktFrameDelay > 0)
	{
		FDelayedPacket& DelayedPacket = Delayed.Emplace_GetRef(FDelayedPacket(SendBuffer.GetData(), SendBuffer.GetNumBits(), Traits));
		DelayedPacket.DelayFrameCount = PacketSimulationSettings.PktFrameDelay;

		// No more emulation applied
		return true;
	}

	if (PacketSimulationSettings.PktOrder)
	{
		Delayed.Emplace(SendBuffer.GetData(), SendBuffer.GetNumBits(), Traits);
	...
	FDelayedPacket* DelayedPacket = nullptr;

	if (PacketSimulationSettings.PktJitter != 0)
	{
		bSendDelayedPacketsOutofOrder = true;

		DelayedPacket = &(Delayed.Emplace_GetRef(FDelayedPacket(SendBuffer.GetData(), SendBuffer.GetNumBits(), Traits)));
		...
		if (bIsLowLatency)
		{
			...
			DelayedPacket->SendTime = CurrentTimeInS + ExtraLag;
		}
		else
		{
	...
	// Delay the packet until the next buffer flush or later
	if (PacketSimulationSettings.PktBufferBloatInMS > 0)
	{
		if (!DelayedPacket)
		{
			DelayedPacket = &(Delayed.Emplace_GetRef(FDelayedPacket(SendBuffer.GetData(), SendBuffer.GetNumBits(), Traits)));
		...
		if (NextBufferBloatFlushOut < CurrentTimeInS && CurrentTimeInS >= NextBufferBloatActiveTimeOut)
		{
			// Find the time at which we flush all delayed packets, and the next time we restart the buffer bloat accumulation
			NextBufferBloatFlushOut = CurrentTimeInS + (PacketSimulationSettings.PktBufferBloatInMS/ 1000.0);
			NextBufferBloatActiveTimeOut = NextBufferBloatFlushOut + (BufferBloatCooldownTimeInMS / 1000.0);
		}

		// Only keep the latest delayed time
		DelayedPacket->SendTime = FMath::Max(NextBufferBloatFlushOut, DelayedPacket->SendTime);
	}
	
	return DelayedPacket != nullptr;
}
#endif
...
void UNetConnection::ReceivedAck(int32 AckPacketId, FChannelsToClose& OutChannelsToClose)
{
	...
	// Store the first Ack we received in the packet
	if (OutFirstAckPacketId == -1)
	{
		OutFirstAckPacketId = AckPacketId;

		// On the first ACK of the first packet of a frame, reset the RawPing
		if (InPacketsThisFrame == 1)
		{
			RawPingInSeconds = 0.0;
		}
	}
...
bool UNetConnection::ReadPacketInfo(FBitReader& Reader, bool bHasPacketInfoPayload, FEngineNetworkCustomVersion::Type EngineNetVer)
{
	...
	if (OutFirstAckPacketId == -1)
	{
		// This means we received a packet without new ACKs in it.
		// Continue reading it but we don't need to update the Ping.
		return true;
	}

	// Find the received time of the packet
	...
	// Use the driver's LastTickDispatchRealtime because it is set closer to the beginning of the frame - we don't care about the time so far of the current frame to process the packet
	// This time needs to be a realtime value based on FPlatformTime::Seconds so that it's compatible with the packet send time.
	const double CurrentTime = (PacketReceiveTime != 0.0 ? PacketReceiveTime : Driver->LastTickDispatchRealtime);

	// Update ping

	const bool bExcludeFrameTime = !!CVarPingExcludeFrameTime.GetValueOnAnyThread();

	// Go trought all the ACKs we received
	for (int32 AckId = OutFirstAckPacketId; AckId <= OutAckPacketId; ++AckId)
	{
		// Map the Id to the Index(0-255) storing the sent time 
		const int32 AckIndex = AckId & (UE_ARRAY_COUNT(OutLagPacketId) - 1);
		
		if (OutLagPacketId[AckIndex] != AckId)
		{
			// The Ids were already processed, nothing more to do here
			return true;
		}

		// Clear the id to tell it was processed.
		OutLagPacketId[AckIndex] = -1;

		const double PacketSendTime = OutLagTime[AckIndex];
		const double RTT			= (CurrentTime - PacketSendTime);
		const double RTTExclFrame	= RTT - (bExcludeFrameTime ? ServerFrameTime : 0.0);
		const double PacketPing		= FMath::Max(RTTExclFrame, 0.0);

		LagAcc += PacketPing;
		LagCount++;

		// Keep the highest ping as the RawPing value for this frame
		RawPingInSeconds = FMath::Max(RawPingInSeconds, PacketPing);
		...
		if (PlayerController)
		{
			...
			{
				PlayerController->UpdatePing(PacketPing);
			}
		}

		if (NetPing.IsValid())
		{
			NetPing->UpdatePing(EPingType::RoundTrip, CurrentTime, PacketPing);
...
#if DO_ENABLE_NET_TEST
bool UNetConnection::CheckIncomingPacketEmulation(double CurrentTimeInS, FBitReader& Reader)
{
	if (PacketSimulationSettings.PktIncomingLoss)
	{
		if (FMath::FRand() * 100.f < PacketSimulationSettings.PktIncomingLoss)
		{
			UE_LOG(LogNet, VeryVerbose, TEXT("Dropped incoming packet at %f"), CurrentTimeInS);
			return true;
		}
	}

	FDelayedIncomingPacket* DelayedPacket = nullptr;
	...
	if (PacketSimulationSettings.PktIncomingLagMin > 0 || PacketSimulationSettings.PktIncomingLagMax > 0)
	{
		...
		DelayedPacket = &(DelayedIncomingPackets.AddDefaulted_GetRef());

		DelayedPacket->PacketData = MakeUnique<FBitReader>(Reader);
		DelayedPacket->ReinjectionTime = CurrentTimeInS + ExtraLagInSec;
	...
	else if (PacketSimulationSettings.PktIncomingFrameDelay)
	{
		...
		// Not compatible with BufferBloat
		return true;
	}

	if (PacketSimulationSettings.PktIncomingBufferBloatInMS > 0)
	{
		if (!DelayedPacket)
		{
			DelayedPacket = &(DelayedIncomingPackets.AddDefaulted_GetRef());
		...
		if (NextBufferBloatFlushIn < CurrentTimeInS && CurrentTimeInS >= NextBufferBloatActiveTimeIn)
		{
			NextBufferBloatFlushIn = CurrentTimeInS + (PacketSimulationSettings.PktIncomingBufferBloatInMS / 1000.f);
			NextBufferBloatActiveTimeIn = NextBufferBloatFlushIn + (BufferBloatCooldownTimeInMS / 1000.0);
		}

		// Only keep the latest delayed time
		DelayedPacket->ReinjectionTime = FMath::Max(NextBufferBloatFlushIn, DelayedPacket->ReinjectionTime);
	}
...
void UNetConnection::ReceivedPacket( FBitReader& Reader, bool bIsReinjectedPacket, bool bDispatchPacket )
{
	...
	{
		const bool bIsPackedEmulated = CheckIncomingPacketEmulation(CurrentReceiveTimeInS, Reader);

		if (bIsPackedEmulated)
		{
			// Packet was delayed or dropped, nothing to do here anymore
			return;
		}
	}
#endif //#if DO_ENABLE_NET_TEST
	...
	else
	{
		...
		// If the packetID is higher then the last one
		if (PacketSequenceDelta > 0)
		{
			...
			// If there is a gap in the Id sequence, cache the packet so they can be processed in the right order
		...
		// PacketID was earlier then a previously read packet, drop it entirely because it is now obsolete
		else
		{
	...
	if(!IsInternalAck())
	{
		...
		if (Driver->IsSendingImmediateAcks())
		{
			// If enabled, we want to let the other side know about the ack, so even if there are no other outgoing data when we tick the connection we will send an ackpacket.
			TimeSensitive = 1;
			++HasDirtyAcks;
		}
	}
...
{
	TObjectPtr<UChildConnection>* ChildConnectionPtr = ChildrenLookup.Find(NetPlayerIndex);
	if (!ChildConnectionPtr)
	{
		return nullptr;
	}

	UChildConnection* ChildConnection = *ChildConnectionPtr;

	ensure(!ChildConnection->PlayerController || (ChildConnection->PlayerController && ChildConnection->PlayerController->NetPlayerIndex == NetPlayerIndex));
	return ChildConnection;
}

bool UNetConnection::GetNetPlayerIndex(UChildConnection* ChildConnection, int32* OutNetPlayerIndex)
{
	if (OutNetPlayerIndex == nullptr)
	{
		return false;
	}

	for (TPair<int32, TObjectPtr<UChildConnection>>& Pair : ChildrenLookup)
	{
		if (Pair.Value == ChildConnection)
		{
			*OutNetPlayerIndex = Pair.Key;
			return true;
		}
	}
...
{
	const int32 NetPlayerIndex = NextServerNetPlayerIndex;
	NextServerNetPlayerIndex++;

	UE_LOG(LogNet, Log, TEXT("[%s:%s] Assigning NetPlayerIndex %d to a new child connection."), *GetNameSafe(GetDriver()), *GetName(), NetPlayerIndex);

	UChildConnection* ChildConnection = CreateChildWithNetPlayerIndex(NetPlayerIndex);

	if (OutNetPlayerIndex)
	{
		*OutNetPlayerIndex = NetPlayerIndex;
	}
...
{
	// A NetPlayerIndex of 0 is reserved for the parent connection so the NetPlayerIndex for
	// child connections should always begin at 1.
	ensure(NetPlayerIndex > 0);

	UChildConnection* ChildConnection = NewObject<UChildConnection>(GetTransientPackage(), GetDriver()->ChildNetConnectionClass);
	ChildConnection->InitChildConnection(GetDriver(), this);
	
	UE_LOG(LogNet, Log, TEXT("[%s:%s] Creating child connection %s NetPlayerIndex %d"), *GetNameSafe(GetDriver()), *GetName(), *GetNameSafe(ChildConnection), NetPlayerIndex);
	
	ensure(Children.Find(ChildConnection) == INDEX_NONE);
	Children.Add(ChildConnection);

	ensure(!ChildrenLookup.Contains(NetPlayerIndex));
	ChildrenLookup.Add(NetPlayerIndex, ChildConnection);
...
void UNetConnection::RemoveChild(UChildConnection* ChildConnection)
{
	...
	Children.Remove(ChildConnection);
		
	int32 NetPlayerIndexToRemove = -1;

	if (PlayerController && ChildConnection->PlayerController)
	{
		NetPlayerIndexToRemove = ChildConnection->PlayerController->NetPlayerIndex;
	}
	else
	{
		for (TPair<int32, TObjectPtr<UChildConnection>>& Pair : ChildrenLookup)
		{
			if (Pair.Value == ChildConnection)
			{
				NetPlayerIndexToRemove = Pair.Key;
				break;
			}
		}
	}
	
	UE_LOG(LogNet, Log, TEXT("Removing child connection %s from parent connection %s using NetPlayerIndex %d."), *GetNameSafe(ChildConnection), *GetNameSafe(this), NetPlayerIndexToRemove);

	if (NetPlayerIndexToRemove >= 0)
	{
		// A NetPlayerIndex of 0 is reserved for the parent connection so it should never be found in the lookup table.
		ensure(NetPlayerIndexToRemove > 0);

		ensure(ChildrenLookup.Contains(NetPlayerIndexToRemove) && *ChildrenLookup.Find(NetPlayerIndexToRemove) == ChildConnection);
		ChildrenLookup.Remove(NetPlayerIndexToRemove);
	}
}
...
void UChildConnection::HandleClientPlayer(APlayerController* PC, UNetConnection* NetConnection)
{
	...
	{
		// Find the local player with the same split screen identifier as the new player controller.
		for (FLocalPlayerIterator It(GEngine, Driver->GetWorld()); It; ++It)
		{
			ULocalPlayer* LocalPlayer = *It;
			if (PC->GetLocalPlayerConnectionIdentifier() == LocalPlayer->GetConnectionIdentifier())
			{
				NewPlayer = *It;
				break;
			}

...
{
	// Call with equal top and bottom radius values
	BuildTaperedCylinderVerts(Base, XAxis, YAxis, ZAxis, Radius, Radius, HalfHeight, Sides, OutVerts, OutIndices);
}
...
{
	const float	AngleDelta = 2.0f * UE_PI / Sides;
	FVector	LastVertex = Base + XAxis * BaseRadius;
	...
	for (uint32 SideIndex = 0; SideIndex < Sides; SideIndex++)
	{
		const FVector Vertex = Base + (XAxis * FMath::Cos(AngleDelta * (SideIndex + 1)) + YAxis * FMath::Sin(AngleDelta * (SideIndex + 1))) * BaseRadius;
	...
	LastVertex = Base + XAxis * BaseRadius;
	TC = FVector2D(0.0f, 1.0f);

	//Compute vertices for the top circle
	for (uint32 SideIndex = 0; SideIndex < Sides; SideIndex++)
	{
		const FVector Vertex = Base + (XAxis * FMath::Cos(AngleDelta * (SideIndex + 1)) + YAxis * FMath::Sin(AngleDelta * (SideIndex + 1))) * TopRadius;
		FVector Normal = Vertex - Base;
		Normal.Normalize();
...
{
	DrawTorus(PDI, Transform, XAxis, YAxis, FColor::White, OuterRadius, InnerRadius, OuterSegments, InnerSegments, MaterialRenderProxy, DepthPriority, bPartial, Angle, bEndCaps);
}

void DrawTorus(FPrimitiveDrawInterface* PDI, const FMatrix& Transform, const FVector& XAxis, const FVector& YAxis, FColor Color,
	...
	)
{
	...
	{	
...
#if WITH_EDITOR
		else if (EngineShowFlags.RandomColor)
		{
			FRandomStream RandomStream(GetTypeHash(PrimitiveSceneProxy->GetPrimitiveComponentId().PrimIDValue) + GetTypeHash(Mesh.MeshIdInPrimitive));

			const uint8 R = static_cast<uint8>(RandomStream.FRandRange(64, 255));
			const uint8 G = static_cast<uint8>(RandomStream.FRandRange(64, 255));
			const uint8 B = static_cast<uint8>(RandomStream.FRandRange(64, 255));
			FLinearColor RandomColor(FColor(R, G, B, 255));
			FMaterialRenderProxy* LevelColorationMaterialInstance = nullptr;

			if (TObjectPtr<UMaterialInterface> RandomMaterial = GEngine->RandomColorMaterial)
			{
				LevelColorationMaterialInstance = new FColoredMaterialRenderProxy(
					RandomMaterial->GetRenderProxy(), 
					GetSelectionColor(RandomColor, bSelected, PrimitiveSceneProxy->IsHovered(), /*bUseOverlayIntensity=*/true), 
					TEXT("Diffuse"));
			}
			else
			{
		...
		else if (EngineShowFlags.Clay)
		{
			if (TObjectPtr<UMaterialInterface> ClayMaterial = GEngine->ClayMaterial)
			{
				FMaterialRenderProxy* LevelColorationMaterialInstance = new FColoredMaterialRenderProxy(
					ClayMaterial->GetRenderProxy(), 
					GetSelectionColor(FLinearColor(1, 1, 1, 1), bSelected, PrimitiveSceneProxy->IsHovered(), /*bUseOverlayIntensity=*/true));

				Mesh.MaterialRenderProxy = LevelColorationMaterialInstance;
				Collector.RegisterOneFrameMaterialProxy(LevelColorationMaterialInstance);
			}
		}
		else if (EngineShowFlags.Zebra)
		{
			if (TObjectPtr<UMaterialInstanceDynamic> ZebraMaterial = GEngine->ZebraMaterialInstance)
			{
				FMaterialRenderProxy* LevelColorationMaterialInstance = new FColoredMaterialRenderProxy(
					ZebraMaterial->GetRenderProxy(), 
					GetSelectionColor(FLinearColor(0, 0, 0, 0), bSelected, PrimitiveSceneProxy->IsHovered(), /*bUseOverlayIntensity=*/true));

				Mesh.MaterialRenderProxy = LevelColorationMaterialInstance;
				Collector.RegisterOneFrameMaterialProxy(LevelColorationMaterialInstance);
			}
		}
		else if (EngineShowFlags.FrontBackFace)
		{
			if (TObjectPtr<UMaterialInterface> FrontBackFaceMaterial = GEngine->FrontBackFaceMaterial)
			{
				FMaterialRenderProxy* LevelColorationMaterialInstance = new FColoredMaterialRenderProxy(
					FrontBackFaceMaterial->GetRenderProxy(),
					GetSelectionColor(FLinearColor(0, 0, 0, 0), bSelected, PrimitiveSceneProxy->IsHovered(), /*bUseOverlayIntensity=*/false));

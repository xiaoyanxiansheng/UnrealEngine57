...
void BuildSignedDistanceFieldBuildSectionData(
	...
	TArray<FSignedDistanceFieldBuildSectionData>& OutData)
{
	...
	for (int32 SectionIndex = 0; SectionIndex < OutData.Num(); SectionIndex++)
	{
		...
		if (!MaterialRemap.IsEmpty())
		{
			MaterialIndex = MaterialRemap.IsValidIndex(MaterialIndex) ? MaterialRemap[MaterialIndex] : 0;
		}
		...
		if (RenderData.NaniteResourcesPtr.IsValid())
		{
			SectionData.bTwoSided |= (RenderData.NaniteResourcesPtr->VoxelMaterialsMask & (1ull << MaterialIndex)) != 0;
		}
	}
}
...
void FDistanceFieldVolumeData::CacheDerivedData(UStaticMesh* Mesh, UStaticMesh* GenerateSource, float DistanceFieldResolutionScale, bool bGenerateDistanceFieldAsIfTwoSided)
{
	...
	BuildSignedDistanceFieldBuildSectionData(Mesh->GetSectionInfoMap(), LODIndex, Mesh->GetStaticMaterials(), {}, *Mesh->GetRenderData(), BuildSectionData);
	...
	if (Mesh->IsNaniteAssembly())
	{
		...
		for (int32 PartIndex = 0; PartIndex < NumParts; ++PartIndex)
		{
			...
			if (Ref)
			{
				...
				// include part section data into DDC key
				for (int32 SectionIndex = 0; SectionIndex < PartData.SectionData.Num(); SectionIndex++)
				{
					DistanceFieldKey += FString::Printf(TEXT("_PM%u_%u_%u"),
						(uint32)PartData.SectionData[SectionIndex].BlendMode,
						PartData.SectionData[SectionIndex].bTwoSided ? 1 : 0,
						PartData.SectionData[SectionIndex].bAffectDistanceFieldLighting ? 1 : 0);
				}
			}
		}
	}
	...
	else if (GDistanceFieldAsyncQueue)
	{
		...
		if (Mesh->IsNaniteEnabled())
		{
			...
			if (Mesh->IsNaniteAssembly())
			{
				...
				check(NumParts == PartDatas.Num());

				for (int32 Index = 0; Index < NumParts; ++Index)
				{
					const TObjectPtr<UStaticMesh>& Ref = PartReferences[Index];

					if (Ref)
					{
						FSignedDistanceFieldBuildPartData& PartData = PartDatas[Index];
...
void FDistanceFieldAsyncQueue::Build(FAsyncDistanceFieldTask* Task, FQueuedThreadPool& BuildThreadPool)
{
	...
	if (Task->StaticMesh && Task->GenerateSource)
	{
		...
		// Begin mesh card representation while we're async
		...
		if (CardTask)
		{
			...
			{
				GCardRepresentationAsyncQueue->AddTask(CardTask);
			}
		}

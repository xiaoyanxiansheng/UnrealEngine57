...
void ULocalPlayer::SendSplitJoin(TArray<FString>& Options, UNetDriver* NetDriver, UE::Net::EJoinFlags Flags)
{
	...
	else if (!bSentSplitJoin)
	{
		...
		if (bNeedToSendJoin)
		{
			...
			URL.AddOption(*FString::Printf(TEXT("LocalPlayerId=%d"), ConnectionIdentifier));
	...
	int32 StereoViewIndex)
{
	...
	// If we are emulating splitscreen rendering for a screenshot functional test, override the layout set in UGameViewportClient::LayoutPlayers()
	if (bEmulateSplitscreen)
	{
		static const TConsoleVariableData<int32>* CVarEmulateSplitscreenMode = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.ScreenshotTest.EmulateSplitscreenMode"));
		const int32 EmulateSplitscreenMode = CVarEmulateSplitscreenMode ? CVarEmulateSplitscreenMode->GetValueOnAnyThread() : 0;

		checkf(EmulateSplitscreenMode >= 1 && EmulateSplitscreenMode <= 3, TEXT("bEmulateSplitscreen should only be set by screenshot functional tests via r.ScreenshotTest.EmulateSplitscreenMode"));
		checkf((StereoViewIndex == 0 || StereoViewIndex == 1), TEXT("bEmulateSplitscreen only supports two splitscreen views"));

		const bool bBaseView = (StereoViewIndex == 0);
		switch (EmulateSplitscreenMode)
		{
			case 1:	// Horizontal
				Origin = bBaseView ? FVector2D(0.0f, 0.0f) : FVector2D(0.5f, 0.0f);
				Size = FVector2D(0.5f, 1.0f);
				break;
			case 2:	// Vertical
				Origin = bBaseView ? FVector2D(0.0f, 0.0f) : FVector2D(0.0f, 0.5f);
				Size = FVector2D(1.0f, 0.5f);
				break;
			case 3:	// Diagonal
				Origin = bBaseView ? FVector2D(0.0f, 0.0f) : FVector2D(0.5f, 0.5f);
				Size = FVector2D(0.5f, 0.5f);
				break;
			default:
				checkNoEntry();
				break;
...
void ULocalPlayer::SetConnectionIdentifier(const int32 Id)
{
	ConnectionIdentifier = Id;
}
	
int32 ULocalPlayer::GetConnectionIdentifier() const
{

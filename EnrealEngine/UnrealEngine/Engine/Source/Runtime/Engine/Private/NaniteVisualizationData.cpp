...
void FNaniteVisualizationData::Initialize()
{
	if (!bIsInitialized)
	{
		...
		AddVisualizationMode(TEXT("ShadowCasters"), LOCTEXT("ShadowCasters", "Shadow Casters"), FModeType::Advanced, NANITE_VISUALIZE_SHADOW_CASTERS, true, false);
		...
		AddVisualizationMode(TEXT("Skinning"), LOCTEXT("Skinning", "Skinning"), FModeType::Advanced, NANITE_VISUALIZE_SKINNING, false);
...
)
{
	...
	Record.bDrawLegend          = bDrawLegend;
	Record.LegendEntries		= MoveTemp(LegendEntries);
}
		...
		if (ActiveVisualizationModeID == INDEX_NONE)
		{
			...
			if (ICVarVisualize)
			{
				...
				if (!ConsoleVisualizationMode.IsEmpty() && !bDisable)
				{
					...
					if (ModeID == INDEX_NONE)
					{
						if (ConsoleVisualizationMode != CurrentConsoleVisualizationMode)
						{
				...
				// Store the last handled ConsoleVisualization mode as a means to prevent console log spam
				CurrentConsoleVisualizationMode = ConsoleVisualizationMode;
			}
		}
		else
		{
			CurrentConsoleVisualizationMode.Reset();
		}
...
TArray<FVisualizationDataLegendEntry> FNaniteVisualizationData::GetLegendEntries(const FName& InModeName) const
{
	if (const FModeRecord* Record = ModeMap.Find(InModeName))
	{
		return Record->LegendEntries;
	}
	else
	{
		return TArray<FVisualizationDataLegendEntry>();
	}
}

...
void ApplyViewMode(EViewModeIndex ViewModeIndex, bool bPerspective, FEngineShowFlags& EngineShowFlags)
{
	...
	EngineShowFlags.SetVisualizeShadowCasters(ViewModeIndex == VMI_ShadowCasters);
	...
	EngineShowFlags.SetClay(ViewModeIndex == VMI_Clay);
	EngineShowFlags.SetZebra(ViewModeIndex == VMI_Zebra);
	EngineShowFlags.SetFrontBackFace(ViewModeIndex == VMI_FrontBackFace);
	EngineShowFlags.SetRandomColor(ViewModeIndex == VMI_RandomColor);
}
...
void EngineShowFlagOverride(EShowFlagInitMode ShowFlagInitMode, EViewModeIndex ViewModeIndex, FEngineShowFlags& EngineShowFlags, bool bCanDisableTonemapper)
{
	...
	// Some view modes want some features off or on (no state)
	{
		...
		if( ViewModeIndex == VMI_Unlit ||
			...
			ViewModeIndex == VMI_VisualizeActorColoration ||
			ViewModeIndex == VMI_VisualizeGPUSkinCache ||
			ViewModeIndex == VMI_LWCComplexity ||
			ViewModeIndex == VMI_LightmapDensity ||
			ViewModeIndex == VMI_ShadowCasters)
		{
		...
		static TOptional<bool> NanitePreviousValue;
		static const auto ICNaniteVar = IConsoleManager::Get().FindConsoleVariable(TEXT("r.Nanite"));

		if (ViewModeIndex == VMI_Clay ||
			ViewModeIndex == VMI_Zebra ||
			ViewModeIndex == VMI_FrontBackFace ||
			ViewModeIndex == VMI_RandomColor)
		{
			static const auto ICVar = IConsoleManager::Get().FindTConsoleVariableDataBool(TEXT("r.GeometryInspection.DrawWireframe"));
			const bool bDrawWireframe = (ICVar && ICVar->GetValueOnGameThread());

			EngineShowFlags.SetMeshEdges(bDrawWireframe);

			EngineShowFlags.SetLighting(false);
			EngineShowFlags.SetCapsuleShadows(false);
			EngineShowFlags.SetContactShadows(false);
			EngineShowFlags.SetDynamicShadows(false);
			...
			if (ICNaniteVar)
			{
				if (!NanitePreviousValue.IsSet())
				{
					NanitePreviousValue = ICNaniteVar->GetBool();
				}
				ICNaniteVar->Set(false);
			}
		}
		else if (ICNaniteVar && NanitePreviousValue.IsSet())
		{
			ICNaniteVar->Set(NanitePreviousValue.GetValue());
		}


		if (ViewModeIndex == VMI_ShadowCasters)
		{
			EngineShowFlags.SetTemporalAA(false);
...
EViewModeIndex FindViewMode(const FEngineShowFlags& EngineShowFlags)
{
	...
	else if (EngineShowFlags.VisualizeShadowCasters)
	{
		return VMI_ShadowCasters;
	}
	...
	{
		return VMI_FrontBackFace;
	}
	else if (EngineShowFlags.RandomColor)
	{
		return VMI_RandomColor;
	}
...
const TCHAR* GetViewModeName(EViewModeIndex ViewModeIndex)
{
	switch (ViewModeIndex)
	{
		...
		case VMI_ShadowCasters:				return TEXT("ShadowCasters");
		case VMI_Clay:						return TEXT("Clay");
		case VMI_Zebra:						return TEXT("Zebra");
		case VMI_FrontBackFace:				return TEXT("FrontBackFace");
		case VMI_RandomColor:				return TEXT("RandomColor");
	}
	return TEXT("");
}

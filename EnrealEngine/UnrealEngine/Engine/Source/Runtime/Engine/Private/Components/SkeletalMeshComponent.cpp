...
bool USkeletalMeshComponent::ShouldRunClothTick() const
{
	...
	if (GetSkeletalMeshAsset()->HasActiveClothingAssetsForLOD(GetPredictedLODLevel()))
	{
		return true;
	}
...
void USkeletalMeshComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction *ThisTickFunction)
{
	...
	{
		// Cloth work must be done after Super::TickComponent so that it has the correct PredictedLOD

		UpdateEndPhysicsTickRegisteredState();
		UpdateClothTickRegisteredState();
		if(!ClothTickFunction.IsTickFunctionRegistered())
		{
			// Do any cloth work that needs to be done when it doesn't tick.
			...
			if (!CVarEnableClothPhysics.GetValueOnGameThread() || bClothingSimulationSuspended)
			{
				for (FClothingSimulationInstance& ClothingSimulationInstance : ClothingSimulationInstances)
				{
					...
					ClothingSimulationInstance.FillContextAndPrepareTick(this, DeltaTime, bIsInitialization);
				}
			...
			{
				// Since cloth isn't ticking, force a teleport and reset the next time it does.
...
/*static*/ void USkeletalMeshComponent::ExcludeHiddenBones(const USkeletalMeshComponent* SkeletalMeshComponent, const USkeletalMesh* SkeletalMesh, TArray<FBoneIndexType>& OutRequiredBones)
{
	...
	{
		if (!ensureMsgf(EditableBoneVisibilityStates.Num() == SkeletalMeshComponent->GetNumComponentSpaceTransforms(),
		TEXT("Skeletal Mesh asset '%s' has incorrect BoneVisibilityStates. # of BoneVisibilityStates (%d), # of SkeletalMeshComponent->GetNumComponentSpaceTransforms() (%d)"),
		*SkeletalMesh->GetName(), EditableBoneVisibilityStates.Num(), SkeletalMeshComponent->GetNumComponentSpaceTransforms()))
		{
			return;
		}

		if (ensureMsgf(EditableBoneVisibilityStates.Num() >= OutRequiredBones.Num(),
			TEXT("Skeletal Mesh asset '%s' has incorrect BoneVisibilityStates. # of BoneVisibilityStates (%d), # of OutRequiredBones (%d)"),
...
{
	if (const IClothingSimulationInterface* const ClothingSimulation = GetClothingSimulationInstances().Num() ? ClothingSimulationInstances[0].GetClothingSimulation() : nullptr)
	{
		return ClothingSimulation->DynamicCastToIClothingSimulation();
	}
	return nullptr;
}
PRAGMA_ENABLE_DEPRECATION_WARNINGS
...
{
	if (IClothingSimulationInterface* const ClothingSimulation = GetClothingSimulationInstances().Num() ? ClothingSimulationInstances[0].GetClothingSimulation() : nullptr)
	{
		return ClothingSimulation->DynamicCastToIClothingSimulation();
	}
	return nullptr;
...
{
	return GetClothingSimulationInstances().Num() ? ClothingSimulationInstances[0].GetClothingSimulationContext() : nullptr;
}
...
UClothingSimulationInteractor* USkeletalMeshComponent::GetClothingSimulationInteractor(const FName ClothingDataName) const
{
	check(IsInGameThread());
	if (ClothingDataName != NAME_None)
	{
		for (const FClothingSimulationInstance& ClothingSimulationInstance : GetClothingSimulationInstances())
		{
			...
			{
				if (ClothingSimulationInteractor->ClothingInteractors.Contains(ClothingDataName))
				{
	...
	else if (ClothingSimulationInstances.Num())
	{
		// For legacy blueprint compatibility only, return the first simulation's interactor
		UE_CLOG(
			ClothingSimulationInstances.Num() > 1,
			LogSkeletalMesh,
			Warning,
			TEXT("GetClothingSimulationInteractor must be passed a valid Clothing Data name to retrieve a Clothing Simulation Interactor for Skeletal Mesh [%s], because this asset requires multiple simulation types."),
			GetSkeletalMeshAsset() ? *GetSkeletalMeshAsset()->GetName() : TEXT(""));
		return ClothingSimulationInstances[0].GetClothingSimulationInteractor();
	}
	return nullptr;
}
...
void USkeletalMeshComponent::UpdateClothSimulationContext(float InDeltaTime)
{
	...
	// Fill the contexts for the next simulation
	const int32 PredictedLOD = GetPredictedLODLevel();
	for (FClothingSimulationInstance& ClothingSimulationInstance : ClothingSimulationInstances)
	{
		constexpr bool bIsInitialization = false;
		ClothingSimulationInstance.FillContextAndPrepareTick(this, InDeltaTime, bIsInitialization);
		if (bNeedsHardReset)
		{
			ClothingSimulationInstance.HardResetSimulation();
		}

		ClothingSimulationInstance.SyncClothingInteractor();
	}
...
void USkeletalMeshComponent::WritebackClothingSimulationData()
{
	if (ClothingSimulationInstances.Num())
	{
		...
		const USkinnedMeshComponent* OverrideComponent = nullptr;
		if (LeaderPoseComponent.IsValid())
		{
		...
		// Reset all existing CurrentSimulationData so that the ClothingSimulationInstances can append their data.
		for (TMap<int32, FClothSimulData>::TIterator DataIter = CurrentSimulationData.CreateIterator(); DataIter; ++DataIter)
		{
			DataIter.Value().Reset();
		}

		for (const FClothingSimulationInstance& ClothingSimulationInstance : ClothingSimulationInstances)
		{
			ClothingSimulationInstance.AppendSimulationData(CurrentSimulationData, this, OverrideComponent);
		}

		for (TMap<int32, FClothSimulData>::TIterator DataIter = CurrentSimulationData.CreateIterator(); DataIter; ++DataIter)
		{
			if (DataIter.Value().IsReset())
			{
...
{
	if (bPostEvaluatingAnimation
#if DO_CHECK
		|| (AnimScriptInstance && (AnimScriptInstance->IsUpdatingAnimation() || AnimScriptInstance->IsPostUpdatingAnimation()))
#endif
		)
	{
		UE_LOG(LogAnimation, Warning, TEXT("USkeletalMeshComponent::SetAnimInstanceClass: Cannot set anim instance class while animation is still updating/evaluating. Apply a delay or perform the switch elsewhere."));
		return;
	}
	if (NewClass != nullptr)
	{
...
bool USkeletalMeshComponent::HandleExistingParallelEvaluationTask(bool bBlockOnTask, bool bPerformPostAnimEvaluation)
{
	...
	)
	{
		if (bBlockOnTask)
		{
			...
			// if the parallel animation evaluation task depended on something that was trying to take a mutex that we
			// had also taken in the transaction, we'd have caused a deadlock if we didn't have this here.
			if (AutoRTFM::IsTransactional())
			{
				(void)AutoRTFM::Close([&]
				{
					AutoRTFM::OnComplete([strong_this = TObjectPtr<USkeletalMeshComponent>(this), bPerformPostAnimEvaluation]
					{
...
void USkeletalMeshComponent::BindClothToLeaderPoseComponent()
{
	if(USkeletalMeshComponent* LeaderComp = Cast<USkeletalMeshComponent>(LeaderPoseComponent.Get()))
	{
		...
		if(ClothingSimulationInstances.Num() && LeaderComp->ClothingSimulationInstances.Num())
		{

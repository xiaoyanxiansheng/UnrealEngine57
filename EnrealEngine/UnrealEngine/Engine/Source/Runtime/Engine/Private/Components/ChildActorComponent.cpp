	...
	if (IsTemplate())
	{
		if (ChildActorClass)
		{
			if (ChildActorTemplate == nullptr || ActorTemplate || (ChildActorTemplate->GetClass() != ChildActorClass))
			{
				...
				// If the CAC's package is ever reloaded, there's a chance that any serialized template with the same class might've also been reloaded.
				// In order to avoid any collisions, we'll move the reloaded one to the transient package.
				if (AActor* ExistingChildActorTemplate = FindObject<AActor>(this, *TemplateName, EFindObjectFlags::None))
				{
					ExistingChildActorTemplate->Rename(nullptr, GetTransientPackage(), REN_DontCreateRedirectors);
				}

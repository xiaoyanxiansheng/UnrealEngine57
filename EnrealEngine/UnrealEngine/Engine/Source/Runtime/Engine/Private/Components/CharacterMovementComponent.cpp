...
void UCharacterMovementComponent::Serialize(FArchive& Archive)
{
	...
	if (Archive.IsMigratingRemoteObjects())
	{
		bool bHasServerPredictionData = false;

		if (Archive.IsSaving())
		{
			bHasServerPredictionData = (ServerPredictionData != nullptr);
			Archive << bHasServerPredictionData;
		}
		else if (Archive.IsLoading())
		{
			Archive << bHasServerPredictionData;
			if (bHasServerPredictionData && ServerPredictionData == nullptr)
			{
				ServerPredictionData = new FNetworkPredictionData_Server_Character(*this);
			}
		}

		if (bHasServerPredictionData)
		{
			ensure(ServerPredictionData);

			ServerPredictionData->Serialize(Archive);
		}
	}
}
...
#if UE_WITH_REMOTE_OBJECT_HANDLE
				PushForceModificator *= PhysicsForceSubsteppingFactor;
#endif
...
bool UCharacterMovementComponent::ForcePositionUpdate(float DeltaTime)
{
	...
	{
		UE_LOG(LogNetPlayerMovement, Log, TEXT("ForcePositionUpdate: %s (DeltaTime %.2f -> ServerTimeStamp %.2f) Pawn: %s"), *GetNameSafe(CharacterOwner), DeltaTime, ServerData->CurrentClientTimeStamp, *GetNameSafe(CharacterOwner));
	}
...
bool UCharacterMovementComponent::VerifyClientTimeStamp(float TimeStamp, FNetworkPredictionData_Server_Character& ServerData)
{
	...
	if (bIsValid)
	{
		...
		if (bFirstMoveAfterForcedUpdates && (ServerData.bForcedUpdateDurationExceeded || CharacterMovementCVars::bNetServerForcePositionUpdateSyncToClient))
		{
			...
			UE_LOG(LogNetPlayerMovement, Log, TEXT("Received a new move after performing ForcedUpdates. Updating CurrentClientTimeStamp from %f to %f Pawn: %s"), ServerData.CurrentClientTimeStamp, TimeStamp, *GetNameSafe(CharacterOwner));
		...
		{
			UE_LOG(LogNetPlayerMovement, VeryVerbose, TEXT("TimeStamp %f Accepted! CurrentTimeStamp: %f Pawn: %s"), TimeStamp, ServerData.CurrentClientTimeStamp, *GetNameSafe(CharacterOwner));
	...
	else
	{
		if (bTimeStampResetDetected)
		{
			UE_LOG(LogNetPlayerMovement, Log, TEXT("TimeStamp expired. Before TimeStamp Reset. CurrentTimeStamp: %f, TimeStamp: %f Pawn: %s"), ServerData.CurrentClientTimeStamp, TimeStamp, *GetNameSafe(CharacterOwner));
		}
		else
		{
...
#if UE_WITH_REMOTE_OBJECT_HANDLE
		SerializeOptionalValue<TObjectPtr<UPrimitiveComponent>>(bIsSaving, Ar, ClientAdjustment.NewBase, nullptr);
#else
...
void FNetworkPredictionData_Server_Character::Serialize(FArchive& Archive)
{
	if (Archive.IsMigratingRemoteObjects())
	{
		FNetworkPredictionData_Server::Serialize(Archive);

		PendingAdjustment.Serialize(Archive);

		Archive << CurrentClientTimeStamp;
		Archive << LastReceivedClientTimeStamp;
		Archive << ServerAccumulatedClientTimeStamp;
		Archive << LastUpdateTime;
		Archive << ServerTimeStampLastServerMove;
		Archive << MaxMoveDeltaTime;
		
		bool bTempForceClientUpdate = bForceClientUpdate;
		Archive.SerializeBits(&bTempForceClientUpdate, 1);
		bForceClientUpdate = bTempForceClientUpdate;

		Archive << LifetimeRawTimeDiscrepancy;
		Archive << TimeDiscrepancy;
		Archive.SerializeBits(&bResolvingTimeDiscrepancy, 1);
		Archive << TimeDiscrepancyResolutionMoveDeltaOverride;
		Archive << TimeDiscrepancyAccumulatedClientDeltasSinceLastServerTick;
		Archive << WorldCreationTime;
	}
};

...
#include UE_INLINE_GENERATED_CPP_BY_NAME(SceneComponent)
...
static bool SceneComponentNeedsLoadForTarget(USceneComponent const* SceneComponentObject, const ITargetPlatform* TargetPlatform, USceneComponent const* ParentThatWillBeCulled = nullptr)
{
	if(UDeviceProfile* DeviceProfile = UDeviceProfileManager::Get().FindProfile(TargetPlatform->IniPlatformName()))
	{
		...
		if(DeviceProfile->GetConsolidatedCVarValue(TEXT("r.CookOutUnusedDetailModeComponents"), CVarCullBasedOnDetailLevel) && CVarCullBasedOnDetailLevel == 1)
		{
			...
			if(DeviceProfile->GetConsolidatedCVarValue(TEXT("r.DetailMode"), CVarDetailMode))
			{
				...
				const bool bKeep = (int32)SceneComponentObject->DetailMode <= CVarDetailMode;
				UE_CLOGFMT(bKeep && ParentThatWillBeCulled, LogSceneComponent, Display, 
					"SceneComponent {ChildComponent} will not be culled because its detail mode {DetailMode} is less than the maximum detail mode for the target platform {PlatformMaxDetailMode}, "
					"but its parent {ParentComponent} will be culled. Make sure that attached components have a detail mode equal or lower to their parent.", 
					FSoftObjectPath(SceneComponentObject), SceneComponentObject->DetailMode, CVarDetailMode, FSoftObjectPath(ParentThatWillBeCulled));
				if (!bKeep)
				{
					return false;
				}					
	...
	const bool bKeep = TargetPlatform->AllowsEditorObjects() || !SceneComponentObject->IsEditorOnly();
	UE_CLOGFMT(bKeep && ParentThatWillBeCulled, LogSceneComponent, Display, 
		"SceneComponent {ChildComponent} will not be culled because it is not marked editor-only, but its parent {ParentComponent} will be culled."
		"Make sure that all attached children of editor-only components are also editor-only.", 
		FSoftObjectPath(SceneComponentObject), FSoftObjectPath(ParentThatWillBeCulled));
	return bKeep;
}

// Returns number of child components not culled
static int32 CheckDescendantsAreAlsoCulledForTarget(USceneComponent const* SceneComponentObject, const ITargetPlatform* TargetPlatform)
{
	if (!ensure(SceneComponentObject != nullptr))
	{
		return 0;
	...
	const TArray<USceneComponent*>& AttachedChildren = SceneComponentObject->GetAttachChildren();
	...
	for (USceneComponent* ChildSceneComponent : AttachedChildren)
	{
		...
		UE_LOGFMT(LogSceneComponent, Verbose, "Checking attached component {ChildComponent} for culling", FSoftObjectPath(ChildSceneComponent));
		if (SceneComponentNeedsLoadForTarget(ChildSceneComponent, TargetPlatform, /* ParentThatWillBeCulled */ SceneComponentObject))
		{
			UE_LOGFMT(LogSceneComponent, Verbose, "Scene component {ChildComponent} will not be culled", FSoftObjectPath(ChildSceneComponent));
			NumNotCulled++;
		}
		else
		{
			UE_LOGFMT(LogSceneComponent, Verbose, "Scene component {ChildComponent} will be culled", FSoftObjectPath(ChildSceneComponent));
		}
	}
...
bool USceneComponent::NeedsLoadForTargetPlatform(const ITargetPlatform* TargetPlatform) const
{
	...
	{
		UE_LOGFMT(LogSceneComponent, Log, "Scene component {ParentComponent} will be culled for target, checking children", FSoftObjectPath(this));
		// Also check whether any of our children are culled.
		int32 DescendantsNotCulled = CheckDescendantsAreAlsoCulledForTarget(this, TargetPlatform);

		// Child not culled, so warn
		if(DescendantsNotCulled != 0)
		{
			UE_LOGFMT(LogSceneComponent, Warning, "Component {ParentComponent} not removed from client data because {NumNotCulled} descendants could not be removed. See prior warnings for details.",
				FSoftObjectPath(this), DescendantsNotCulled);
			return true;
		}
...
#if WITH_EDITOR
		GIsEditor ? !bHiddenEdTemporary && (!Owner || !Owner->IsHiddenEd()) : false;
#else
		false;
#endif
...
#if WITH_EDITOR
void USceneComponent::SetIsTemporarilyHiddenInEditor(const bool bInIsHidden)
{
...
void USceneComponent::Serialize(FArchive& Ar)
{
	...
	if (Ar.IsMigratingRemoteObjects())
	{
		...
		if (Ar.IsLoading())
		{
			// Freshen rotation cache (which would not have been serialized/copied), with our current transform
			RelativeRotationCache.RotatorToQuat(RelativeRotation);
			WorldRotationCache.QuatToRotator(ComponentToWorld.GetRotation());
		}

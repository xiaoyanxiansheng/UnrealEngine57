	...
	if (bMeshPaintTextureUsesEditorOnly)
	{
		...
		if (bSerializeAsCookedData)
		{
			...
			if (Ar.IsSaving())
			{
				...
				UObject* SavedMeshPaintTexture = bCookMeshPaintTexture ? MeshPaintTexture.Get() : nullptr;
#else
...
void UStaticMeshComponent::OnRegister()
{
	...
	if (GetStaticMesh() && StaticMeshImportVersion != GetStaticMesh()->GetImportVersion())
	{
		if (OverrideMaterials.Num())
		{
			uint32 MaterialMapKey = ( (uint32)((StaticMeshImportVersion & 0xffff) << 16) | (uint32)(GetStaticMesh()->GetImportVersion() & 0xffff));
			for (const FMaterialRemapIndex &MaterialRemapIndex : GetStaticMesh()->GetMaterialRemapIndexPerImportVersion())
			{
		...
		StaticMeshImportVersion = GetStaticMesh()->GetImportVersion();
	}
#endif //WITH_EDITORONLY_DATA
...
void UStaticMeshComponent::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyThatChanged)
	{
		...
		// Recreate on material cache changes
		if (PropertyThatChanged->GetFName() == GET_MEMBER_NAME_CHECKED(UStaticMeshComponent, MaterialCacheTileCount))
		{
			ReleaseMaterialCacheTextures();
			UpdateMaterialCacheTextures();
		}
	}
...
void UStaticMeshComponent::ReleaseMaterialCacheTextures()
{
	// Unregister all textures against the scene
...
void UStaticMeshComponent::UpdateMaterialCacheTextures()
{
	for (int32 i = 0; i < GetNumMaterials(); i++)
	{
		...
		for (TObjectPtr Tag : ExpressionData.MaterialCacheTags)
		{
			UMaterialCacheVirtualTextureTag* TagHandle = Tag.Get();
			
			// May already have the tag
			if (GetMaterialCacheVirtualTexture(TagHandle ? TagHandle->Guid : FGuid()))
			{
				continue;
			}
			
			UMaterialCacheVirtualTexture* VirtualTexture = NewObject<UMaterialCacheVirtualTexture>(this);
			VirtualTexture->OwningComponent = this;
			VirtualTexture->Tag = Tag;
			VirtualTexture->TileCount = MaterialCacheTileCount;
			VirtualTexture->UpdateResourceWithParams(UTexture::EUpdateResourceFlags::None);
			MaterialCacheTextures.Add(VirtualTexture);
		}
	}
}

UMaterialCacheVirtualTexture* UStaticMeshComponent::GetMaterialCacheVirtualTexture(FGuid TagGuid) const
{
	// Linear search
	for (TObjectPtr MaterialCacheVirtualTexture : MaterialCacheTextures)
	{
...
{
	return SupportsDitheredLODTransitions(GetFeatureLevelShaderPlatform(FeatureLevel));
}

bool UStaticMeshComponent::SupportsDitheredLODTransitions(EShaderPlatform ShaderPlatform)
{
	if (!AllowDitheredLODTransition(ShaderPlatform))
	{
		return false;
	}
...
void UStaticMeshComponent::ComputeHLODHash(FHLODHashBuilder& HashBuilder) const
{
	...
	if (ForcedLodModel > 0)
	{
		HashBuilder.HashField(ForcedLodModel, GET_MEMBER_NAME_CHECKED(UStaticMeshComponent, ForcedLodModel));
	}
	
	int32 Width, Height;
	if (GetLightMapResolution(Width, Height))
	{
		HashBuilder.HashField(FIntVector2(Width, Height), TEXT("LightMapResolution"));
	}
	...
	for (const FStaticMeshComponentLODInfo& LODInfo : LODData)
	{
		if (LODInfo.OverrideVertexColors)
		{
			...
			HashBuilder.HashField(Ar.GetCrc(), TEXT("OverrideVertexColors"));
		}
	}

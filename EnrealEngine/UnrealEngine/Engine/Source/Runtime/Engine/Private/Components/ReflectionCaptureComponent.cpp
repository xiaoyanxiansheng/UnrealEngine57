...
public:
	...
	virtual void InitRHI(FRHICommandListBase& RHICmdList) override
	{
		...
		TextureCubeRHI = RHICmdList.CreateTexture(Desc);
		...
		if (SourceData.Num())
		{
			...
			for (int32 MipIndex = 0; MipIndex < NumMips; MipIndex++)
			{
				...
				{
					uint32 DestPitch = 0;
					uint8* DestBuffer = (uint8*)RHICmdList.GetAsImmediate().LockTextureCubeFace(TextureCubeRHI, CubeFace, 0, MipIndex, RLM_WriteOnly, DestPitch, false);
					...
					RHICmdList.GetAsImmediate().UnlockTextureCubeFace(TextureCubeRHI, CubeFace, 0, MipIndex, false);
				}

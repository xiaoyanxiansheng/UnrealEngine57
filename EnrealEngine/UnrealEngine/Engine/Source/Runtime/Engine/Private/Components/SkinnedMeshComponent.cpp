...
#include UE_INLINE_GENERATED_CPP_BY_NAME(SkinnedMeshComponent)
...
FAutoConsoleVariableRef CVarSkeletalMeshForceLOD(
	TEXT("r.SkeletalMeshForceLOD"),
		...
		MeshObject)
	{
		...
		if (MaxBonesPerChunk <= MaxSupportedNumBones)
		{
			if (MeshObject->IsNaniteMesh())
			{
				...
				Result = ::new Nanite::FSkinnedSceneProxy(*MeshObject->GetNaniteMaterials(), this, SkelMeshRenderData);
			}
			else
			{
...
void USkinnedMeshComponent::CreateRenderState_Concurrent(FRegisterComponentContext* Context)
{
	...
	if (true) // This branch is here to easily toggle between statestream and not.. will be removed once we are on par
	{
		if (GetSkinnedAsset() && FApp::CanEverRender() && ShouldComponentAddToScene())
		{
			...
			Ss.SetMaterialRelevance(GetMaterialRelevance(GetFeatureLevelShaderPlatform_Checked(ERHIFeatureLevel::SM6)).Raw);
...
void USkinnedMeshComponent::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	{
		// NOTE: From 5.8, this call has been simplified and moved to PostEditUndo in USkinnedMeshComponent.
		// If the PropertyChangedEvent is not a specified change, it may have originated from a transaction such as undo or MU. 
		// Therefore, make sure that any socket attachments are up to date.
...
{
	if (!GEnableGPUSkinCache || !GetScene() || !GetScene()->GetGPUSkinCache())
	{
		return false;
	}
...
{
	return GetActiveMeshDeformers().Deformers.Num() > 0;
}
...
bool USkinnedMeshComponent::HasMeshDeformer(int32 LodIdx) const
{
	...
	const FMeshDeformerSet& Set = GetActiveMeshDeformers();
	if (!Set.DeformerIndexForLOD.IsValidIndex(LodIdx))
	{
		return false;
	}
	return Set.DeformerIndexForLOD[LodIdx] != INDEX_NONE;
}
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
#if !UE_BUILD_SHIPPING
	return GSkeletalMeshForceLOD < 0 ? ForcedLodModel : GSkeletalMeshForceLOD + 1; // Add 1 to CVar because requires the return value is 1-based for legacy reasons.
#else
...
TArray<FName> USkinnedMeshComponent::GetCurrentSkinWeightProfileLayerNames() const
{
	...
	if (USkinnedAsset* Asset = GetSkinnedAsset())
	{
		if (Asset->IsCompiling())
		{
			//Return an array with name none entries for all layers
			return ProfileNames;
		}

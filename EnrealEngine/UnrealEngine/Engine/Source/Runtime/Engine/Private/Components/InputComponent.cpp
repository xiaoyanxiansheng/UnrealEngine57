...
void UInputComponent::LogDeprecatedBindingWarning(const FName BindingName)
{
	const UInputSettings* Settings = GetDefault<UInputSettings>();
	if (Settings->bEnabledLegacyMappingDeprecationWarnings && Settings->IsActionMappingDeprecated(BindingName))
	{
		UE_LOG(LogPlayerInput, Warning, TEXT("Attempting to access a deprecated action/axis mapping '%s'"), *BindingName.ToString());
	}
}
...
{
	LogDeprecatedBindingWarning(InBinding.GetActionName());
...
{
	LogDeprecatedBindingWarning(BindingToRemove.GetActionName());
...
{
	LogDeprecatedBindingWarning(InActionName);
...
{
	LogDeprecatedBindingWarning(AxisName);

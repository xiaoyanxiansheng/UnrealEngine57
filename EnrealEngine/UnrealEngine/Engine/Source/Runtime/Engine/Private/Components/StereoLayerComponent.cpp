...
namespace UE::XRStereoLayerCVars
{
	static bool bUseLegacyFacing = false;
	static FAutoConsoleVariableRef UseUECoordinateSystemCVar(
		TEXT("XR.StereoLayers.UseLegacyFacing"),
		bUseLegacyFacing,
		TEXT("If true the old UE coordinate system will be used where layers facing away from the camera"
			 "are visible instead of the ones seemingly facing it inside of UE"),
		ECVF_Default);
}
...
void UStereoLayerComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction *ThisTickFunction)
{
	...
	if (bIsDirty)
	{
		...
		if (!UE::XRStereoLayerCVars::bUseLegacyFacing)
		{
			...
			LayerDesc.Transform.SetRotation(Transform.GetRotation() * FQuat(0., 0., 1., 0.));
		}

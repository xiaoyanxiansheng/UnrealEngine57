...
void UHLODEngineSubsystem::UnregisterRecreateLODActorsDelegates()
{
	...
	FEditorDelegates::PreSaveWorldWithContext.Remove(OnPreSaveWorlDelegateHandle);
}
...
void UHLODEngineSubsystem::RecreateLODActorsForLevel(ULevel* InLevel, UWorld* InWorld)
{
	...
	// Legacy LODActors are not necessary anymore when using World Partition
...
bool UHLODEngineSubsystem::CleanupHLOD(ALODActor* InLODActor)
{
	...
	if (InLODActor->GetLevel()->GetWorldSettings()->IsPartitionedWorld())
	{
		UE_LOG(LogEngine, Warning, TEXT("Deleting legacy LODActor %s found in World Partition map. Resave %s to silence warning."), *InLODActor->GetName(), *InLODActor->GetOutermost()->GetPathName());
	...
	if (bShouldDestroyActor)
	{
		...
		ReleaseUniquelyNamedObject(InLODActor);
	}
...
void FHLODDependencyGatherer::GatherDependencies(FGatherDependenciesContext& Context) const
{
	...
	FString Wildcard = Utilities->GetWildcardOfHLODPackagesForPackage(Context.GetAssetData().PackageName.ToString());
	FARFilter PossibleAssetsFilter;
	PossibleAssetsFilter.PackagePaths.Add(FName(FPathViews::GetPath(Wildcard)));
	TArray<FAssetData> FilteredAssets;
	Context.GetAssetRegistryState().GetAssets(Context.CompileFilter(PossibleAssetsFilter), {}, FilteredAssets);
	FString PackageName;

...
bool USoundSubmixWithParentBase::DynamicConnect(FAudioDeviceHandle Handle, USoundSubmixBase* InParent)
{
	...
	// Reject anything that's Soundfield related. That system is being retired/
	if (Cast<USoundfieldSubmix>(this) || Cast<USoundfieldSubmix>(InParent))
	{
		UE_LOG(LogAudio, Warning, TEXT("Submix (DynamicConnect): Soundfield submixes do not support dynamic connections. Submix=%s, Parent=%s "), *GetName(), *GetNameSafe(InParent));
		return false;
	}

...
{
	UE::TReadScopeLock ReadLock(CurrentStateLock);
	return CurrentState;
}
...
void ULevelStreaming::SetCurrentState(ELevelStreamingState NewState)
{
	...
	{
		UE::TWriteScopeLock WriteLock(CurrentStateLock);
...
bool ULevelStreaming::RequestLevel(UWorld* PersistentWorld, bool bAllowLevelLoadRequests, EReqLevelBlock BlockPolicy)
{
	...
	UPackage* LevelPackage = (UPackage*)StaticFindObjectFast(UPackage::StaticClass(), nullptr, DesiredPackageName, EFindObjectFlags::None, RF_NoFlags, EInternalObjectFlags::Garbage);
...
#endif
			// Here, if the level should not be re-used, trash the package.
			// This can happen in the case where a streaming level is first removed inside the World Tick,
			// FLevelStreamingGCHelper::RequestUnload will not call PrepareStreamedOutLevelForGC right away 
			// and will process the level in FLevelStreamingGCHelper::OnWorldTickEnd.
			// If we are insde a BlockTillLevelStreamingCompleted (still in the World Tick), the target
			// state of the Level can change to be loaded by a subsystem like UWorldPartitionSubsystem::OnUpdateStreamingState.
			// In this scenario, a request to load the level will happen and the level package will be found in memory.
			if (!ULevelStreaming::ShouldReuseUnloadedButStillAroundLevels(World->PersistentLevel))
			{
				UE_LOG(LogLevelStreaming, Verbose, TEXT("Trying to reuse level %s but ShouldReuseUnloadedButStillAroundLevels returns false. Trash the current package and reload."), *LevelPackage->GetName());
...
{
	// This function can be called from an async task (when wp.runtime.UpdateStreaming.EnableAsyncUpdate = true)
	ELevelStreamingState LocalState = GetLevelStreamingState();
	if (LocalState == ELevelStreamingState::FailedToLoad)
	{
		return LEVEL_FailedToLoad;
	}
	else if (LocalState == ELevelStreamingState::MakingInvisible)
	{
		return LEVEL_MakingInvisible;
	}
	else if (LoadedLevel)
	{
		if (LocalState == ELevelStreamingState::LoadedVisible)
		{
			return LEVEL_Visible;
		}
		else if ((LocalState == ELevelStreamingState::MakingVisible) && (GetWorld()->IsLevelMakingVisible(LoadedLevel)))
		{
	...
	{
		if (LocalState == ELevelStreamingState::Loading)
		{
			return LEVEL_Loading;
		}

		check(LocalState == ELevelStreamingState::Removed || LocalState == ELevelStreamingState::Unloaded);

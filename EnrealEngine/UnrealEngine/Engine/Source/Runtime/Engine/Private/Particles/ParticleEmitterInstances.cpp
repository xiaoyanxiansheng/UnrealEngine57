...
{
	return GatherMaterialRelevance(OutMaterialRelevance, LODLevel, GetFeatureLevelShaderPlatform_Checked(InFeatureLevel));
}
 ...
 */
void FParticleEmitterInstance::GatherMaterialRelevance(FMaterialRelevance* OutMaterialRelevance, const UParticleLODLevel* LODLevel, EShaderPlatform InShaderPlatform) const
{
	// These will catch the sprite cases...
	if (CurrentMaterial)
	{
		(*OutMaterialRelevance) |= CurrentMaterial->GetRelevance_Concurrent(InShaderPlatform);
	}
	else if (LODLevel->RequiredModule->Material)
	{
		(*OutMaterialRelevance) |= LODLevel->RequiredModule->Material->GetRelevance_Concurrent(InShaderPlatform);
	}
	else
	{
		check( UMaterial::GetDefaultMaterial(MD_Surface) );
		(*OutMaterialRelevance) |= UMaterial::GetDefaultMaterial(MD_Surface)->GetRelevance_Concurrent(InShaderPlatform);
	}
}
 ...
 * @param InFeatureLevel - The relevant shader feature level.
 */
void FParticleMeshEmitterInstance::GatherMaterialRelevance( FMaterialRelevance* OutMaterialRelevance, const UParticleLODLevel* LODLevel, EShaderPlatform InShaderPlatform ) const
{
	TArray<UMaterialInterface*,TInlineAllocator<2> > Materials;
	GetMeshMaterials(Materials, LODLevel, true); // Allow log issues since GatherMaterialRelevance is only called when the proxy is created.
	for (int32 MaterialIndex = 0; MaterialIndex < Materials.Num(); ++MaterialIndex)
	{
		(*OutMaterialRelevance) |= Materials[MaterialIndex]->GetRelevance_Concurrent(InShaderPlatform);
	}
}
...
{
	GetMeshMaterials(OutMaterials, LODLevel, bLogWarnings);
}

...
public:
	...
	FTextureRHIRef CreateStateTextureRHI(FRHICommandListBase& RHICmdList, const TCHAR* Name, EPixelFormat PixelFormat)
	{
	...
	virtual void InitRHI(FRHICommandListBase& RHICmdList) override
	{
		...
		PositionTextureRHI = CreateStateTextureRHI(RHICmdList, PaticlePositonTextureName, GetParticlePositionFormat());

		// 16-bit per channel RGBA texture for velocity.
		check(!IsValidRef(VelocityTextureRHI));
		VelocityTextureRHI = CreateStateTextureRHI(RHICmdList, PaticleVelocityTextureName, GetParticleVelocityFormat());
	...
	void ResizeRHI(FRHICommandListImmediate& RHICmdList, int32 SizeX, int32 SizeY)
	{
		...
		FTextureRHIRef NewPositionTextureRHI = CreateStateTextureRHI(RHICmdList, PaticlePositonTextureName, PF_A32B32G32R32F);
		FTextureRHIRef NewVelocityTextureRHI = CreateStateTextureRHI(RHICmdList, PaticleVelocityTextureName, PF_FloatRGBA);
...
public:
	...
	FTextureRHIRef CreateAttributesTextureRHI(FRHICommandListBase& RHICmdList)
	{
		const FRHITextureCreateDesc Desc =
	...
	virtual void InitRHI(FRHICommandListBase& RHICmdList) override
	{
		TextureRHI = CreateAttributesTextureRHI(RHICmdList);

		ClearRenderTarget(RHICmdList.GetAsImmediate(), TextureRHI);
		RHICmdList.GetAsImmediate().Transition(FRHITransitionInfo(TextureRHI, ERHIAccess::RTV, ERHIAccess::SRVMask));
	}
	...
	void ResizeRHI(FRHICommandListImmediate& RHICmdList, int32 SizeX, int32 SizeY)
	{
		...
		// allocate the new textures
		FTextureRHIRef NewAttributesTextureRHI = CreateAttributesTextureRHI(RHICmdList);

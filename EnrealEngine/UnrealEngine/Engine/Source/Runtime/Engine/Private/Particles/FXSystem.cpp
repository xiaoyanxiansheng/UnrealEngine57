...
void FFXSystemInterface::FFXInterfaceDeletor::operator()(FFXSystemInterface* ObjectToDelete) const
{
	ENQUEUE_RENDER_COMMAND(FFXInterfaceDeletor)
	(
...
FFXSystemInterface* FFXSystemInterface::Create(ERHIFeatureLevel::Type InFeatureLevel, FSceneInterface* Scene)
{
	...
	if (CreateCustomFXDelegates.Num())
	{
		...
		Set->FXSystems.Emplace(MakeShareable(new FFXSystem(InFeatureLevel, InShaderPlatform, GPUSortManager), FFXInterfaceDeletor()));
		...
		for (TMap<FName, FCreateCustomFXSystemDelegate>::TConstIterator Ite(CreateCustomFXDelegates); Ite; ++Ite)
		{
			...
			if (CustomFX)
			{
				...
				Set->FXSystems.Emplace(MakeShareable(CustomFX, FFXInterfaceDeletor()));
			}
		}
		return Set;
	}
...
FFXSystem::~FFXSystem()
{
	ensure(GPUSimulations.Num() == 0);
	ensure(VectorFields.Num() == 0);
}
...
void FFXSystem::OnMarkPendingKill()
{
	...
	// Enqueue command to destroy simulations as destruction order can matter for Cascade
	ENQUEUE_RENDER_COMMAND(FOnMarkPendingKill)(

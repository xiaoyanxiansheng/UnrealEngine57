...
#if WITH_EDITOR
bool UCommandlet::ApplyCommandLineSwitches( UObject* Object, const TArray<FString>& Switches )
{
	for (const FString& Switch : Switches)
	{
		...
		// Lookup property
		const FProperty* Property = Object->GetClass()->FindPropertyByName(*Key);

		// If we can't find the property, try for properties with the 'b' prefix
		if (!Property)
		{
			Key = TEXT("b") + Key;
			Property = Object->GetClass()->FindPropertyByName(*Key);
		}
		...
		if (Property)
		{
			...
			// Apply the property
			if (!FBlueprintEditorUtils::PropertyValueFromString(Property, Value, (uint8*)Object, nullptr))
			{
				UE_LOG(LogCommandlet, Error, TEXT("Cannot set value for '%s': '%s'"), *Key, *Value);
				return false;
			}
		}
	}
...
void CommandletHelpers::TickEngine(UWorld* InWorld, double InDeltaTime)
{
	...
	if (InWorld && IsAllowCommandletRendering())
	{
		if (FSceneInterface* Scene = InWorld->Scene)
		{
			...
			ENQUEUE_RENDER_COMMAND(ComputeTaskWorker_EndOfFrameUpdate)([FeatureLevel, ComputeWorkers, Scene](FRHICommandListImmediate& RHICmdList)
			{				

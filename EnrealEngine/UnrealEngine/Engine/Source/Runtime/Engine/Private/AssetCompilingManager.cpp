...
TRACE_DECLARE_MEMORY_COUNTER(AsyncCompilationAvailableMemory, TEXT("AsyncCompilation/AvailableMemory"));
TRACE_DECLARE_MEMORY_COUNTER(AsyncCompilationTotalEstimatedMemory, TEXT("AsyncCompilation/TotalEstimatedMemory"));

TRACE_DECLARE_MEMORY_COUNTER(AsyncCompilationSCWCurrentLimit, TEXT("AsyncCompilation/SCWCurrentLimit"));
TRACE_DECLARE_MEMORY_COUNTER(AsyncCompilationSCWBaseLimit, TEXT("AsyncCompilation/SCWBaseLimit"));

TRACE_DECLARE_INT_COUNTER(AsyncDelayedTask, TEXT("AsyncCompilation/DelayedTask"));
TRACE_DECLARE_INT_COUNTER(AsyncDelayedTaskActive, TEXT("AsyncCompilation/DelayedTaskActive"));
...
public:
	...
	int64 GetMemoryAvailableForAssetCompilation() const
	{
		const FPlatformMemoryStats MemoryStats = FPlatformMemory::GetStats();
		
		// UsedPhysical = "working set"
		// UsedVirtual  = "commit charge"
		UE_LOG(LogAsyncCompilation, Verbose, TEXT("GetMemoryAvailableForAssetCompilation UsedPhysical = %.3f MB (Peak %.3f MB) UsedVirtual = %.3f MB (Peak %.3f MB) AvailPhysical = %.3f MB AvailVirtual = %.3f MB "),
		...
		const int64 SystemAvailableMemory = (int64)FMath::Min3(MemoryStats.AvailablePhysical, MemoryStats.AvailableVirtual, (uint64)INT64_MAX);

		const int64 HardMemoryLimit = GetHardMemoryLimit();
		const int64 LeaveMemoryHeadRoom = 64 * 1024 * 1024; // leave 64 MB head room

		// No hard limit specified, use system memory
		if (HardMemoryLimit == 0) {
			return FMath::Max(0, SystemAvailableMemory - LeaveMemoryHeadRoom);
		}

		const int64 MemoryAvailableAgainstHardLimit = FMath::Max(0, HardMemoryLimit - TotalEstimatedMemory);

		if (SystemAvailableMemory < MemoryAvailableAgainstHardLimit) 
		{
			int64 CurrentDefecit = MemoryAvailableAgainstHardLimit - SystemAvailableMemory;

			if (CurrentDefecit > LargestDefecit) 
			{
				LargestDefecit = CurrentDefecit;
				UE_LOG(LogAsyncCompilation, VeryVerbose, TEXT(
					"Memory available for asset compilation less than exptected: (Hard Limit %.3f MB - SystemAvailableMemory %.3f MB) < MemoryAvailableAgainstHardLimit = %.3f MB"),
					   HardMemoryLimit / (1024 * 1024.f),
					   SystemAvailableMemory / (1024 * 1024.f),
					   MemoryAvailableAgainstHardLimit / (1024 * 1024.f));
			}
		}

		const int64 MemoryAvailableForAssetCompilation = FMath::Min(
			FMath::Max(0, MemoryAvailableAgainstHardLimit - LeaveMemoryHeadRoom), 
			FMath::Max(0, SystemAvailableMemory - LeaveMemoryHeadRoom)
		);

		return MemoryAvailableForAssetCompilation;
	}
	...
	class MemorySharingInfo {
	private:
		const TFunction<int64 ()> GetAvailableMemory;
		mutable FCriticalSection UpdateLock;

		FShaderCompilerMemoryLimits SCWMemoryLimits;
		FShaderCompileMemoryUsage SCWMemoryUsage;

		int64 AvailableAssetCompilationMemory;
	public:
		int64 MemoryToLoan;
		int64 MemoryLoanThreshold;
		int64 MemoryToBorrowFromSCW;

		bool IsMemorySharingEnabled;

		MemorySharingInfo(TFunction<int64()> InGetAvailableMemory)
			: GetAvailableMemory(InGetAvailableMemory)
		{
			MemoryToLoan = GConfig->GetIntOrDefault(TEXT("AssetCompilation"), TEXT("MemoryToLoanToSCW"), 2000, GEngineIni);	
			MemoryLoanThreshold = GConfig->GetIntOrDefault(TEXT("AssetCompilation"), TEXT("MemoryLoanToSCWThreshold"), 3000, GEngineIni);	
			MemoryToBorrowFromSCW = GConfig->GetIntOrDefault(TEXT("AssetCompilation"), TEXT("MemoryToBorrowFromSCW"), 2000, GEngineIni);	

			IsMemorySharingEnabled = GConfig->GetBoolOrDefault(TEXT("AssetCompilation"), TEXT("SCWMemorySharingEnabled"), false, GEngineIni);
		}	

		int32 GetHighSCWMemory() const
		{
			FScopeLock MemLock(&UpdateLock);
			return SCWMemoryLimits.BaseLimit + MemoryToLoan;
		}

		int32 GetNormalSCWMemory() const
		{
			FScopeLock MemLock(&UpdateLock);
			return SCWMemoryLimits.BaseLimit;
		}

		bool IsSCWMemoryWithinLimit() const 
		{ 
			FScopeLock MemLock(&UpdateLock);
			return SCWMemoryUsage.VirtualMemory <= (SCWMemoryLimits.CurrentLimit * 1024 * 1024);
		}
		bool IsMemoryAvailableToLoan() const
		{
			FScopeLock MemLock(&UpdateLock);
			return AvailableAssetCompilationMemory >= MemoryLoanThreshold;
		}
		bool ShouldRetrieveFromSCW() const 
		{
			FScopeLock MemLock(&UpdateLock);
			return IsMemorySharingEnabled && IsMemoryAvailableToLoan() == false && SCWMemoryLimits.CurrentLimit == GetHighSCWMemory();
		}
		bool CanLoanToSCW() const 
		{
			FScopeLock MemLock(&UpdateLock);
			return IsMemorySharingEnabled && IsMemoryAvailableToLoan() && SCWMemoryLimits.CurrentLimit == GetNormalSCWMemory();
		}
		...
		{	
			...
			GetShaderCompilerCurrentMemoryLimits(SCWMemoryLimits);								
			SCWMemoryUsage = GShaderCompilingManager->GetExternalMemoryUsage();
			AvailableAssetCompilationMemory = GetAvailableMemory() / (1024 * 1024);				
		}
	};
	...
	{
	public:
		FMemoryBoundScheduledWork(MemorySharingInfo& InMemoryInfo) 
		: MemoryInfo(InMemoryInfo)
		{
		...
		MemorySharingInfo& MemoryInfo;
	protected:
		void DoThreadedWork() override
		{			
			MemoryInfo.Update();					

			if (MemoryInfo.IsMemorySharingEnabled && FMemoryBoundQueuedThreadPoolWrapper::IsMemoryConstrained())
			{
				auto WaitForSCWMemory = [this]() 
				{
					TRACE_CPUPROFILER_EVENT_SCOPE(FMemoryBoundScheduledWork::WaitForSCWMemory);

					const float MaxWaitTimeSeconds = 2.0f;
					const float PerIterationWaitTimeSeconds  = 0.2f;
					const int WaitIterations = static_cast<int>(MaxWaitTimeSeconds / PerIterationWaitTimeSeconds );

					TRACE_COUNTER_INCREMENT(AsyncDelayedTask);
					TRACE_COUNTER_INCREMENT(AsyncDelayedTaskActive);			

					int Counter = 0;
					for (Counter = 0; Counter < WaitIterations; Counter++) 
					{
						MemoryInfo.Update();

						if (MemoryInfo.IsSCWMemoryWithinLimit())
							break;

						FPlatformProcess::Sleep(PerIterationWaitTimeSeconds);
					}

					if (Counter > 0)
					{
						UE_LOG(LogAsyncCompilation, Display, TEXT("Waited for SCW memory: %d iterations for a total of %f seconds"),
							   Counter,
							   Counter * PerIterationWaitTimeSeconds);


						if (Counter == WaitIterations)
						{
							UE_LOG(LogAsyncCompilation, Display, TEXT("Waited for the maximum allowed time for SCW to reduce memory usage, proceeding with work anyway"));
						}
					}
				
					TRACE_COUNTER_DECREMENT(AsyncDelayedTaskActive);
				};

				if (RequiredMemory > FMemoryBoundQueuedThreadPoolWrapper::GetHardMemoryLimit())
				{
					SetShaderCompilerTargetMemoryLimitInMiB(MemoryInfo.GetNormalSCWMemory() - MemoryInfo.MemoryToBorrowFromSCW);
					WaitForSCWMemory();
					FQueuedThreadPoolWrapper::FScheduledWork::DoThreadedWork();
					SetShaderCompilerTargetMemoryLimitInMiB(MemoryInfo.GetNormalSCWMemory());
				}
				else if (MemoryInfo.ShouldRetrieveFromSCW())
				{
					SetShaderCompilerTargetMemoryLimitInMiB(MemoryInfo.GetNormalSCWMemory());
					WaitForSCWMemory();
					FQueuedThreadPoolWrapper::FScheduledWork::DoThreadedWork();
	...
	{
		return new FMemoryBoundScheduledWork(MemoryInfo);
	}

	void UpdateCounters()
	{
		TRACE_COUNTER_SET(AsyncCompilationAvailableMemory, GetMemoryAvailableForAssetCompilation());
		...
		FShaderCompilerMemoryLimits SCWMemoryLimits; 
		GetShaderCompilerCurrentMemoryLimits(SCWMemoryLimits);
		TRACE_COUNTER_SET(AsyncCompilationSCWCurrentLimit, SCWMemoryLimits.CurrentLimit * 1024 * 1024);
		TRACE_COUNTER_SET(AsyncCompilationSCWBaseLimit, SCWMemoryLimits.BaseLimit * 1024 * 1024);
	}
	...
	void OnUnscheduled(const IQueuedWork* InWork) override
	{
		...
		MemoryInfo.Update();
		if (MemoryInfo.CanLoanToSCW() && IsMemoryConstrained())
		{
			SetShaderCompilerTargetMemoryLimitInMiB(MemoryInfo.GetHighSCWMemory());
		} 		
	}
	...
	int32 GetMaxConcurrency() const override
	{
		...
		int64 AvailableAssetCompilationMemory = GetMemoryAvailableForAssetCompilation();
		...
		if ( NewRequiredMemory > 0 && NewRequiredMemory >= AvailableAssetCompilationMemory )
		{
			if ( Concurrency == 0 )
			{
				...
				if (bHasMemoryEstimate)
				{
					...
					if (NewRequiredMemory > HardMemoryLimit && HardMemoryLimit > 0)
					{
						...
						UE_LOGFMT_NSLOC(LogAsyncCompilation, Warning, "AsyncAssetCompilation", "HardMemoryLimitExceeded",
							...
							"RequiredMemory = {TotalEstimatedMemory} MiB + {RequiredMemory} MiB, AvailableAssetCompilationMemory = {AvailableAssetCompilationMemory} MiB, HardMemoryLimit = {HardMemoryLimit} MiB",
							("TaskName", DebugName),
							...
							("AvailableAssetCompilationMemory", FString::SanitizeFloat(AvailableAssetCompilationMemory / (1024 * 1024.f), 3)),
					...
					else
					{
						UE_LOGFMT_NSLOC(LogAsyncCompilation, Display, "AsyncAssetCompilation", "MemoryLimitExceeded",
							...
							"RequiredMemory = {TotalEstimatedMemory} MiB + {RequiredMemory} MiB, MemoryLimit = {AvailableAssetCompilationMemory} MiB ",
							("TaskName", DebugName),
							("RequiredMemory", FString::SanitizeFloat(NewRequiredMemory / (1024 * 1024.f), 3)),
							("TotalEstimatedMemory", FString::SanitizeFloat(TotalEstimatedMemory / (1024 * 1024.f), 3)),
							("AvailableAssetCompilationMemory", FString::SanitizeFloat(AvailableAssetCompilationMemory / (1024 * 1024.f), 3))
			...
			{
				UE_LOG(LogAsyncCompilation, Verbose, TEXT("CONCURRENCY LIMITED to %d/%d; RequiredMemory = %.3f MB + %.3f MB AvailableAssetCompilationMemory = %.3f MB "),
					Concurrency,DynamicMaxConcurrency,
					NewRequiredMemory/(1024*1024.f),
					TotalEstimatedMemory/(1024*1024.f),
					AvailableAssetCompilationMemory/(1024*1024.f));
	...
	void AddQueuedWork(IQueuedWork* InQueuedWork, EQueuedWorkPriority InPriority) override
	{
		...
		if ( RequiredMemory < 0 )
		{
			...
			UE_LOG(LogAsyncCompilation, Display, TEXT("AddQueuedWork [%s] did not supply a RequiredMemory estimate. This is a bug, all asset types should provide an estimate. When no estimate is provided asset compilation is slowed as the queue has to operate very conservatively."),DebugName);
		}
		else
		{
...
private:
	...
	mutable std::atomic<int64> LargestDefecit {0};
};

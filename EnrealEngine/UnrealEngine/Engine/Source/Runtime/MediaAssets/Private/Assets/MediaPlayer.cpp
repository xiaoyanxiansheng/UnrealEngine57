...
UMediaPlayer::UMediaPlayer(const FObjectInitializer& ObjectInitializer)
	...
	, PlayerGuid(IsTemplate() ? FGuid() : FGuid::NewGuid())
	, PlayOnNext(false)
...
{
	UE_LOG(LogMediaAssets, VeryVerbose, TEXT("%s.SetBlockOnRange [%s, %s]"), *GetFName().ToString(),
		*TimeRange.GetLowerBoundValue().ToString(TEXT("%h:%m:%s.%t")), *TimeRange.GetUpperBoundValue().ToString(TEXT("%h:%m:%s.%t")));
...
{
	UE_LOG(LogMediaAssets, VeryVerbose, TEXT("%s.SetPlaybackTimeRange [%s, %s]"), *GetFName().ToString(),
		*InTimeRange.GetLowerBoundValue().ToString(TEXT("%h:%m:%s.%t")), *InTimeRange.GetUpperBoundValue().ToString(TEXT("%h:%m:%s.%t")));

...
#if WITH_EDITOR
FString UPlatformMediaSource::GetDescriptionString() const
{
	// Guard against reentrant calls.
	static bool bIsGettingConfigText = false;
	if (bIsGettingConfigText)
	{
		UE_LOG(LogMediaAssets, Warning, TEXT("UPlatformMediaSource::GetConfigurationDisplayText - Reentrant calls are not supported. Asset: %s"), *GetPathName());
		return FString();
	}
	TGuardValue<bool> Guard(bIsGettingConfigText, true);

	UMediaSource* PlatformMediaSource = GetMediaSource();
	return (PlatformMediaSource != nullptr) ? PlatformMediaSource->GetDescriptionString() : FString();
}

void UPlatformMediaSource::GetDetailsPanelInfoElements(TArray<FInfoElement>& OutInfoElements) const
{
	// Guard against reentrant calls.
	static bool bIsGettingInfoElements = false;
	if (bIsGettingInfoElements)
	{
		UE_LOG(LogMediaAssets, Warning, TEXT("UPlatformMediaSource::GetDetailsPanelInfoElements - Reentrant calls are not supported. Asset: %s"), *GetPathName());
		return;
	}
	TGuardValue<bool> Guard(bIsGettingInfoElements, true);

	UMediaSource* PlatformMediaSource = GetMediaSource();
	if (PlatformMediaSource != nullptr)
	{
		PlatformMediaSource->GetDetailsPanelInfoElements(OutInfoElements);
	}
}
#endif //WITH_EDITOR

...
UCineCameraComponent::UCineCameraComponent()
{
	...
	ExposureMethod = ECameraExposureMethod::Enabled;
...
void UCineCameraComponent::RecalcDerivedData()
{
	...
	AspectRatio = (CropSettings.AspectRatio > 0.0f ? CropSettings.AspectRatio : Filmback.SensorAspectRatio * LensSettings.SqueezeFactor);
...
{
	if (ExposureMethod == ECameraExposureMethod::DoNotOverride &&
			FocusSettings.FocusMethod == ECameraFocusMethod::DoNotOverride)
	{
		// We want neither dynamic exposure nor focus effects.
		DesiredView.PostProcessSettings.bOverride_DepthOfFieldFstop = false;
	}
	else
	{
		// We want dynamic exposure or focus effects, maybe both.

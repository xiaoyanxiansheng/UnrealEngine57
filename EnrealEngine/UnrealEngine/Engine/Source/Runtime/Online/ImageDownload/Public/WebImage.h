...
public:
	...
	inline FWebImage& SetStandInBrush(TAttribute<const FSlateBrush*> StandInBrushIn) { StandInBrush = StandInBrushIn; DownloadedBrush.Reset(); return *this; }
	...
	inline bool BeginDownload(const FString& InUrl, const FOnImageDownloaded& DownloadCallback)
	{
...
public:
	...
	inline const FSlateBrush* GetBrush() const { return DownloadedBrush.IsValid() ? DownloadedBrush.Get() : StandInBrush.Get(); }

	/** Only returns the downloaded brush. May be null if the download hasn't finished or was unsuccessful */
	inline const FSlateBrush* GetDownloadedBrush() const { return DownloadedBrush.IsValid() ? DownloadedBrush.Get() : nullptr; }

	/** Is there a pending HTTP request */
	inline bool IsDownloadPending() const { return PendingRequest.IsValid(); }

	/** Has the download finished AND was it successful */
	inline bool DidDownloadSucceed() const { return bDownloadSuccess; }

	/** Has the download finished AND did it fail */
	inline bool DidDownloadFail() const { return !IsDownloadPending() && !DidDownloadSucceed(); }

	/** What URL was requested */
	inline const FString& GetUrl() const { return Url; }

	/** What is the ETag of the downloaded resource */
	inline const TOptional<FString>& GetETag() const { return ETag; }

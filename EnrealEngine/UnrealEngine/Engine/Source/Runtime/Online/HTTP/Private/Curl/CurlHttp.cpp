...
TAutoConsoleVariable<bool> CVarHttpAlwaysSetContentLengthEnabled(
	TEXT("http.AlwaysSetContentLengthEnabled"),
	false,
	TEXT("Always set Content-Length header even if there is no content to upload when do the http request"),
	ECVF_SaveForNextBoot
);

extern TAutoConsoleVariable<bool> CVarHttpCurlReuseConnectionEnabled;
	...
	, LeastRecentlyCachedInfoMessageIndex(0)
{
	...
	if (!CVarHttpCurlReuseConnectionEnabled.GetValueOnAnyThread())
	{
...
bool FCurlHttpRequest::SetupRequest()
{
	...
	{
		if (RequestPayload->GetContentLength() != 0 || CVarHttpAlwaysSetContentLengthEnabled.GetValueOnAnyThread())
		{

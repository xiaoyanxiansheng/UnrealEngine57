...
static TAutoConsoleVariable<bool> CVarEnableBackgroundRequests(
	TEXT("AppleHttp.EnableBackgroundRequests"),
	true,
	TEXT("If true, will use the Background URLSession to schedule tasks that can run while the app is in the background. "
			   "If false, will use the current flow scheduling the request to be executed."),
	ECVF_Default);

FRWLock FApplePlatformHttp::TaskRequestMapLock;
TMap<NSURLSessionTask*, TSharedPtr<FAppleHttpRequest>> FApplePlatformHttp::TaskRequestMap;
...
#if PLATFORM_IOS // won't work on Mac, tvOS and visionOS
	if (IsBackgroundRequestFeatureEnabled()) // can be disabled by cvar
	{
		Config = [NSURLSessionConfiguration backgroundSessionConfigurationWithIdentifier:@"com.unrealengine.backgroundhttpsession"];
		
		// Disable cache to mimic WinInet behavior
		Config.requestCachePolicy = NSURLRequestReloadIgnoringLocalCacheData;
		// Set discretionary to false so the OS will send the request ASAP (with fewer considerations on battery level, power consumption, and such)
		Config.discretionary = NO;
		Config.sessionSendsLaunchEvents = YES;

		FApplePlatformHttpBackgroundSessionDelegate* Delegate = [[FApplePlatformHttpBackgroundSessionDelegate alloc] init];
		BackgroundSession = [NSURLSession sessionWithConfiguration:Config delegate:Delegate delegateQueue:nil];
...
IHttpRequest* FApplePlatformHttp::ConstructRequest()
{
	...
	// To avoid breaking the ConstructRequest contract I'll be passing both sessions here
	// If the background feature is disabled, BackgroundSession will be nil and I can check that inside the FAppleHttpRequest
	FAppleHttpRequest* Request = new FAppleHttpRequest(Session, BackgroundSession);
...
{
	return CVarEnableBackgroundRequests.GetValueOnAnyThread();
}

TSharedPtr<FAppleHttpRequest>& FApplePlatformHttp::Add(NSURLSessionTask* Task, const TSharedPtr<FAppleHttpRequest>& Request)
{
	FRWScopeLock ScopeLock(TaskRequestMapLock, FRWScopeLockType::SLT_Write);

	return TaskRequestMap.Add(Task, CopyTemp(Request));
}

bool FApplePlatformHttp::Find(NSURLSessionTask* Task, TSharedPtr<FAppleHttpRequest>& Request)
{
	FRWScopeLock ScopeLock(TaskRequestMapLock, FRWScopeLockType::SLT_ReadOnly);

	if (TSharedPtr<FAppleHttpRequest>* Found = TaskRequestMap.Find(Task))
	{
		Request = *Found;
		return true;
	}
...
bool FApplePlatformHttp::RemoveAndCopyValue(NSURLSessionTask* Task, TSharedPtr<FAppleHttpRequest>& Request)
{
	FRWScopeLock ScopeLock(TaskRequestMapLock, FRWScopeLockType::SLT_Write);

	return TaskRequestMap.RemoveAndCopyValue(Task, Request);
}
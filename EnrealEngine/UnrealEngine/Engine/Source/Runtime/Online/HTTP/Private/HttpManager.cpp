...
TAutoConsoleVariable<FString> CVarHttpUrlPatternsToMockResponse(
	TEXT("http.UrlPatternsToMockResponse"),
	TEXT(""),
...
void FHttpManager::Initialize()
{
	...
	UpdateUrlPatternsToMockResponse(CVarHttpUrlPatternsToMockResponse.AsVariable());
	CVarHttpUrlPatternsToMockResponse.AsVariable()->OnChangedDelegate().AddRaw(this, &FHttpManager::UpdateUrlPatternsToMockResponse);
}
...
void FHttpManager::Shutdown()
{
	...
	CVarHttpUrlPatternsToMockResponse.AsVariable()->OnChangedDelegate().Clear();
...
{
	const FScopeLock CacheLock(&UrlPatternsToMockResponseCriticalSection);
	const FString UrlPatternsToMockResponseStr = CVar->AsVariable()->GetString();

	TArray<FString> UrlPatternsToMockResponseStrings;
	UrlPatternsToMockResponseStr.ParseIntoArray(UrlPatternsToMockResponseStrings, TEXT(" "));

	UrlPatternsToMockResponse.Empty();

	for (const FString& UrlPatternToMockResponseString : UrlPatternsToMockResponseStrings)
	{
		// Example runtime args(NOTE: It can't contain space in body of each url pattern): 
		//  -ini:Engine:[ConsoleVariables]:http.UrlPatternsToMockResponse="www.mydomain.com/mypath++POST->403++{\"friendly_error\":\"online.presence.restricted\",\"error_code\":3456}++Content-Type:application/json"
		TArray<FString> UrlPattern;
		UrlPatternToMockResponseString.ParseIntoArray(UrlPattern, TEXT("->"));
		const FStringView PayloadSeparator = TEXT("++");

		if (UrlPattern.Num() == 2)
		{
			FMockResponse MockResponse;

			FString MockResponseStr = UrlPattern[1];
			const int32 PayloadSeparatorIndex = UE::String::FindFirst(MockResponseStr, PayloadSeparator);
			if (PayloadSeparatorIndex != INDEX_NONE)
			{
				MockResponse.StatusCode = FCString::Atoi(*MockResponseStr.Left(PayloadSeparatorIndex));
				FString ResponsePayload = MockResponseStr.RightChop(PayloadSeparatorIndex + PayloadSeparator.Len());
				const int32 HeaderSeparatorIndex = UE::String::FindFirst(ResponsePayload, PayloadSeparator);
				if (HeaderSeparatorIndex != INDEX_NONE)
				{
					FString HeadersStr = ResponsePayload.RightChop(HeaderSeparatorIndex + PayloadSeparator.Len());
					ResponsePayload = ResponsePayload.Left(HeaderSeparatorIndex);

					TArray<FString> ResponseHeaderStrings;
					HeadersStr.ParseIntoArray(ResponseHeaderStrings, TEXT(","));
					for (const FString& ResponseHeaderString : ResponseHeaderStrings)
					{
						FString HeaderKey;
						FString HeaderValue;
						if (ResponseHeaderString.Split(TEXT(":"), &HeaderKey, &HeaderValue))
						{
							MockResponse.ResponseHeaders.Emplace(MoveTemp(HeaderKey), MoveTemp(HeaderValue));
						}
					}
				}

				MockResponse.ResponsePayload = StringCast<UTF8CHAR>(*ResponsePayload);
			}
			else
			{
				MockResponse.StatusCode = FCString::Atoi(*MockResponseStr);
			}

			TArray<FString> UrlInfo;
			UrlPattern[0].ParseIntoArray(UrlInfo, PayloadSeparator.GetData());

			FUrlToMatch UrlToMatch;
			UrlToMatch.Str = UrlInfo[0];
			if (UrlInfo.Num() > 1)
			{
				UrlToMatch.Verb = UrlInfo[1];
			}
			UrlPatternsToMockResponse.Emplace(UrlToMatch, MoveTemp(MockResponse));
		}
	}
}

TOptional<FHttpManager::FMockResponse> FHttpManager::GetMockResponse(FStringView Url, FStringView Verb) const
{
	TOptional<FMockResponse> Result;
	const FScopeLock CacheLock(&UrlPatternsToMockResponseCriticalSection);

	for (const TPair<FUrlToMatch, FMockResponse>& UrlPatternPair : UrlPatternsToMockResponse)
	{
		const FUrlToMatch& UrlToMatch = UrlPatternPair.Key;
		if (Url.Contains(UrlToMatch.Str) && (UrlToMatch.Verb.IsEmpty() || UrlToMatch.Verb.Compare(FString(Verb), ESearchCase::IgnoreCase) == 0))
		{
			Result = UrlPatternPair.Value;
			break;
		}
	}

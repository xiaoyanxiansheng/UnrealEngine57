...
public:
	...
	UE_DEPRECATED(5.7, "Please use FindTempFilenameMappingForURL with reference out argument instead")
	...
	UE_DEPRECATED(5.7, "Please use FindMappedURLForTempFilename with reference out argument instead")
...
private:
	// Same as ToString but doesn't take a lock 
	BACKGROUNDHTTPFILEHASH_API FString ToStringNoLock() const;

	// Same as InitFromString but doesn't take a lock 
	BACKGROUNDHTTPFILEHASH_API bool InitFromStringNoLock(const FString& StringIn);

	// Helper function that just returns the filepath we use to save/load our URL mapping data
	static BACKGROUNDHTTPFILEHASH_API const FString& GetURLMappingFilePathNoLock();

	// Looks for a temp filename mapping for the given URL. If one isn't found returns nullptr an does NOT generate one.
	BACKGROUNDHTTPFILEHASH_API const FURLTempFileMapping* FindMappingForURLNoLock(const FString& URL) const;
	...
	BACKGROUNDHTTPFILEHASH_API FString FindValidFilenameHashForURLNoLock(const FString& URL) const;

	// Creates a hashed Filename with a modifier for the given collision count.
	static BACKGROUNDHTTPFILEHASH_API FString GenerateHashedFilenameForURLNoLock(const FString& URL, uint32 CollisionCount);

	// Used to make sure that we have called LoadData before any operations that depend on it
	std::atomic<bool> bHasLoadedURLData;
	...
	// Access lock for URLFileMappings
	mutable FRWLock URLFileMappingsLock;
};

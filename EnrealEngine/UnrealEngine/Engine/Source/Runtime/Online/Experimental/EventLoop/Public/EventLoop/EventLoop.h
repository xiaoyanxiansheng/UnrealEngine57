...
struct FEventLoopTraitsBase
{
	...
	static inline FTimespan GetCurrentTime()
	{
		return FTimespan(FPlatformTime::Seconds() * ETimespan::TicksPerSecond);
	}

	static inline bool IsEventLoopThread(uint32 EventLoopThreadId)
	{
		return EventLoopThreadId == GetCurrentThreadId();
	}

	static inline bool IsInitialized(uint32 EventLoopThreadId)
	{
		return (EventLoopThreadId != 0);
	}

	static inline void CheckInitialized(uint32 EventLoopThreadId)
	{
		check(IsInitialized(EventLoopThreadId));
	}

	static inline void CheckNotInitialized(uint32 EventLoopThreadId)
	{
		check(!IsInitialized(EventLoopThreadId));
	}

	static inline void CheckIsEventLoopThread(uint32 EventLoopThreadId)
	{
		check(IsEventLoopThread(EventLoopThreadId));
	}

	static inline bool IsShutdownRequested(bool bShutdownRequested)
	{

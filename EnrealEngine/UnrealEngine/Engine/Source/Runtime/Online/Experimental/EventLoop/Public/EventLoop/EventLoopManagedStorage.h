...
struct FManagedStorageTraitsBase
{
	...
	static inline bool IsManagerThread(uint32 ManagerThreadId)
	{
		return ManagerThreadId == GetCurrentThreadId();
	}

	static inline void CheckNotInitialized(uint32 ManagerThreadId)
	{
		check(ManagerThreadId == 0);
	}

	static inline void CheckIsManagerThread(uint32 ManagerThreadId)
	{
...
public:
	...
	struct TManagedStorageInternalHandle
	{
		...
		friend inline uint32 GetTypeHash(const TManagedStorageInternalHandle& InHandle)
		{
...
public:
	 ...
	 */
	inline bool IsManagerThread() const
	{
...
public:
	...
	inline ElementType* Find(FExternalHandle Handle)
	{
		CheckThreadForStorageAccess();
	...
	inline const ElementType* Find(FExternalHandle Handle) const
	{
		return const_cast<TManagedStorage*>(this)->Find(Handle);
	}

	inline ElementType* Find(FInternalHandle Handle)
	{
		CheckThreadForStorageAccess();
	...
	inline const ElementType* Find(FInternalHandle Handle) const
	{
		return const_cast<TManagedStorage*>(this)->Find(Handle);
	}
	...
	public:
		...
		inline TBaseRangeForIterator& operator++()
		{
			++StorageIterator;
			if (StorageIterator)
			{
		...
		inline bool operator==(const TBaseRangeForIterator& Rhs) const { return StorageIterator == Rhs.StorageIterator; }
		inline bool operator!=(const TBaseRangeForIterator& Rhs) const { return StorageIterator != Rhs.StorageIterator; }
		...
		inline ItElementType& operator*() const { return *ElementAccess; }
		inline ItElementType* operator->() const { return &*ElementAccess; }
	...
	inline TRangedForIterator begin()
	{
		CheckThreadForStorageAccess();
...
private:
	inline void CheckThreadForStorageAccess() const
	{

...
struct FPluginDescriptor
{
	...
	/* Reads the descriptor using the global CustomPluginDescriptorReaderDelegate */
	...
	// Custom hook for providing a plugin descriptor (as an FJsonObject) based on a filename
	DECLARE_DELEGATE_RetVal_FourParams(bool, FPluginDescriptorReaderDelegate, const TCHAR*/*Filename*/, FText*/*OutFailReason*/,  TSharedPtr<FJsonObject>&/*OutJson*/, bool&/*OutRetryWithNative*/);
	PROJECTS_API static FPluginDescriptorReaderDelegate CustomPluginDescriptorReaderDelegate;
};

namespace UE::PluginDescriptor::Internal
{
	PROJECTS_API bool ReadFile(const TCHAR* FileName, FString& Text, FText* OutFailReason = nullptr);
	PROJECTS_API TSharedPtr<FJsonObject> DeserializeJson(const FString& Text, FText* OutFailReason = nullptr);
}

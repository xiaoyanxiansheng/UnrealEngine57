...
FPluginReferenceDescriptor::FPluginReferenceDescriptor(FString InName, bool bInEnabled)
...
TOptional<FText> UE::Projects::Private::Read(UE::Json::FConstObject Object, FPluginReferenceDescriptor& Result)
{
	using namespace UE::Json;

#if WITH_EDITOR
	Result.CachedJson = ConvertRapidJsonToSharedJsonObject(Object);
	Result.AdditionalFieldsToWrite.Reset();
#endif // WITH_EDITOR

	// Get the name
	if (!TryGetStringField(Object, TEXT("Name"), Result.Name))
	{
		return LOCTEXT("PluginReferenceWithoutName", "Plugin references must have a 'Name' field");
	}

	// Get the enabled field
	if (!TryGetBoolField(Object, TEXT("Enabled"), Result.bEnabled))
	{
		return LOCTEXT("PluginReferenceWithoutEnabled", "Plugin references must have an 'Enabled' field");
	}

	// Read the optional field
	TryGetBoolField(Object, TEXT("Optional"), Result.bOptional);

	TryGetBoolField(Object, TEXT("Activate"), Result.bActivate);
	...
	TryGetStringArrayFieldWithDeprecatedFallback(Object, TEXT("PlatformAllowList"), TEXT("WhitelistPlatforms"), /*out*/ Result.PlatformAllowList);
	TryGetStringArrayFieldWithDeprecatedFallback(Object, TEXT("PlatformDenyList"), TEXT("BlacklistPlatforms"), /*out*/ Result.PlatformDenyList);

	// Get the target configuration lists
	TryGetEnumArrayFieldWithDeprecatedFallback(Object, TEXT("TargetConfigurationAllowList"), TEXT("WhitelistTargetConfigurations"), /*out*/ Result.TargetConfigurationAllowList);
	TryGetEnumArrayFieldWithDeprecatedFallback(Object, TEXT("TargetConfigurationDenyList"), TEXT("BlacklistTargetConfigurations"), /*out*/ Result.TargetConfigurationDenyList);

	// Get the target lists
	TryGetEnumArrayFieldWithDeprecatedFallback(Object, TEXT("TargetAllowList"), TEXT("WhitelistTargets"), /*out*/ Result.TargetAllowList);
	TryGetEnumArrayFieldWithDeprecatedFallback(Object, TEXT("TargetDenyList"), TEXT("BlacklistTargets"), /*out*/ Result.TargetDenyList);
	...
	int32 ReadVersion;
	if (TryGetNumberField(Object, TEXT("Version"), ReadVersion))
	{
		Result.RequestedVersion = ReadVersion;
		if (!Result.bEnabled)
		{
			return LOCTEXT("PluginReferenceDisabledWithVersion", "Plugin references cannot be used to disable explicit versions. Remove the 'Version' field when 'Enabled' is false.");
			}
		}
...
void FPluginReferenceDescriptor::UpdateJson(FJsonObject& JsonObject) const
{
	...
	if (bActivate)
	{
		JsonObject.SetBoolField(TEXT("Activate"), bActivate);
	}
	else
	{
		JsonObject.RemoveField(TEXT("Activate"));
	}

...
namespace DiscoveredPluginMapUtils
{
	 ...
	 * This enum is used to distinctualize how a plugin is handled -- should it be promoted as the one "offered" plugin, or suppressed?
	 */
			...
			for (const FModuleDescriptor& ChildModule : Child->GetDescriptor().Modules)
			{
				...
				for (FModuleDescriptor& ParentModule : Parent->Descriptor.Modules)
				{
					if (ParentModule.Name == ChildModule.Name && ParentModule.Type == ChildModule.Type)
					{
						...
						// merge platform architecture allow/deny lists
						auto MergePlatformArchitectureLists = []( TMap<FString,TArray<FString>>& Parent, const TMap<FString,TArray<FString>>& Child )
						{
							for (TPair<FString,TArray<FString>> Pair : Child)
							{
								TArray<FString>* ArchList = Parent.Find(Pair.Key);
								if (ArchList == nullptr)
								{
								...
								{
									for (const FString& Arch : Pair.Value)
									{
						...
						MergePlatformArchitectureLists( ParentModule.PlatformArchitectureAllowList, ChildModule.PlatformArchitectureAllowList);
						MergePlatformArchitectureLists( ParentModule.PlatformArchitectureDenyList, ChildModule.PlatformArchitectureDenyList);
					}
				}
...
void FPluginManager::FindPluginsInDirectory(const FString& PluginsDirectory, TArray<FString>& FileNames, IPlatformFile& PlatformFile)
{
	...
	public:
		...
		virtual bool Visit(const TCHAR* FilenameOrDirectory, bool bIsDirectory) override
		{
			...
			if (bIsDirectory)
			{
				constexpr const TCHAR ContentStr[] = TEXT("/Content");
				constexpr const int32 ContentStrLen = UE_ARRAY_COUNT(ContentStr)-1; // -1 for the string's terminating zero (which would be counted by `UE_ARRAY_COUNT()`)

				if (FileNameLen >= ContentStrLen && FCString::Stricmp(&FilenameOrDirectory[FileNameLen - ContentStrLen], ContentStr) == 0)
				{
					// excluded
				}
				else
			...
			{
				// ideally we'd like to find this .uplugin file first before visiting anything else
...
#if READ_TARGET_ENABLED_PLUGINS_FROM_RECEIPT
static TUniquePtr<FTargetReceipt> FindMatchingTargetFile (const FString& ReceiptWildcard)
{
...
void FPluginManager::FindCompileTimePlugins(FPluginDiscoveryContext& Context)
{
...
#if READ_TARGET_ENABLED_PLUGINS_FROM_RECEIPT
	{
		...
		auto ReadBuildPluginsFromFirstMatchingTargetFile = [&Context](const TCHAR* BaseDir) -> bool
			{
				...
				if (Receipt.IsValid())
				{
					Context.AllowedOptionalDependencies.Append(Receipt->BuildPlugins);
					return true;
				}
				return false;
			};
...
#else
	{
		...
		Context.AllowedOptionalDependencies.Append({ UBT_TARGET_BUILD_PLUGINS });
	}
#endif // READ_TARGET_ENABLED_PLUGINS_FROM_RECEIPT
#endif
}
...
bool FPluginManager::FindCommandLinePlugins(FPluginDiscoveryContext& Context)
{
	...
	{
#if !UE_BUILD_SHIPPING
		Context.AllowedOptionalDependencies.Append(ExtraPluginsToEnable);
#endif
		...
		{
			if (!Context.ConfiguredPluginNames.Contains(EnablePluginName) && !ExceptPlugins.Contains(EnablePluginName) && (!bExceptRestrictedPlugins || !IsRestrictedPlugin(EnablePluginName)))
			{
				if (!ConfigureEnabledPluginForCurrentTarget(FPluginReferenceDescriptor(EnablePluginName, true), Context.EnabledPlugins,
					bAllPluginsEnabledViaCommandLine ? TEXTVIEW("Commandline EnableAllPlugins") : TEXTVIEW("CommandLine EnablePlugins="), Context.AllowedOptionalDependencies))
				{
					if (bAllPluginsEnabledViaCommandLine)
					{
				...
				Context.ConfiguredPluginNames.Add(EnablePluginName);
			}
		}
	}
	...
	{
		if (!Context.ConfiguredPluginNames.Contains(DisablePluginName))
		{
			if (!ConfigureEnabledPluginForCurrentTarget(FPluginReferenceDescriptor(DisablePluginName, false), Context.EnabledPlugins,
				TEXTVIEW("CommandLine DisablePlugins="), Context.AllowedOptionalDependencies))
			{
				return false;
			}
			Context.ConfiguredPluginNames.Add(DisablePluginName);
		}
	}
...
bool FPluginManager::FindTargetPlugins(FPluginDiscoveryContext& Context)
{
	// this is a poorly named param, and probably could be just removed (this code enables engine and project plugins)
	if (!FParse::Param(FCommandLine::Get(), TEXT("NoEnginePlugins")))
	{
		...
		auto ConfigurePluginsFromFirstMatchingTargetFile = [this, &Context, &bAllowEnginePluginsEnabledByDefault]
			(const TCHAR* BaseDir, bool& bOutError) -> bool
			{
				...
				if (Receipt.IsValid())
				{
					...
					for (const TPair<FString, bool>& Pair : Receipt->PluginNameToEnabledState)
					{
						...
						if (!Context.ConfiguredPluginNames.Contains(PluginName))
						{
							if (!ConfigureEnabledPluginForCurrentTarget(FPluginReferenceDescriptor(PluginName, bEnabled), Context.EnabledPlugins,
								SourceDescription, Context.AllowedOptionalDependencies))
							{
								bOutError = true;
								break;
							}
							Context.ConfiguredPluginNames.Add(PluginName);
						}
					}
...
#else
		{
			...
			for (const FStringView& TargetEnabledPlugin : std::initializer_list<FStringView>{UBT_TARGET_ENABLED_PLUGINS}) //-V1078
			{
				if (!Context.ConfiguredPluginNames.ContainsByHash(GetTypeHash(TargetEnabledPlugin), TargetEnabledPlugin))
				{
					if (!ConfigureEnabledPluginForCurrentTarget(FPluginReferenceDescriptor(FString{TargetEnabledPlugin}, true), Context.EnabledPlugins,
						TEXTVIEW("UBT_TARGET_ENABLED_PLUGINS"), Context.AllowedOptionalDependencies))
					{
						return false;
					}
					Context.ConfiguredPluginNames.Add(FString{TargetEnabledPlugin});
				}
			}
		}
		{
			...
			for (const FStringView& TargetDisabledPlugin : std::initializer_list<FStringView>{UBT_TARGET_DISABLED_PLUGINS}) //-V1078
			{
				if (!Context.ConfiguredPluginNames.ContainsByHash(GetTypeHash(TargetDisabledPlugin), TargetDisabledPlugin))
				{
					if (!ConfigureEnabledPluginForCurrentTarget(FPluginReferenceDescriptor(FString{TargetDisabledPlugin}, false), Context.EnabledPlugins,
						TEXTVIEW("UBT_TARGET_ENABLED_PLUGINS"), Context.AllowedOptionalDependencies))
					{
						return false;
					}
					Context.ConfiguredPluginNames.Add(FString{TargetDisabledPlugin});
				}
			}
		}
#endif // READ_TARGET_ENABLED_PLUGINS_FROM_RECEIPT

		auto ProcessPluginConfigurations = [&Context, this]
			(const TArray<FPluginReferenceDescriptor>& PluginReferences, FStringView SourceDescription)->bool
			{
				...
				{
					if (!Context.ConfiguredPluginNames.Contains(PluginReference.Name))
					{
						if (!ConfigureEnabledPluginForCurrentTarget(PluginReference, Context.EnabledPlugins, SourceDescription, Context.AllowedOptionalDependencies))
						{
							return false;
						}
						Context.ConfiguredPluginNames.Add(PluginReference.Name);
					}
				}
				return true;
			};
...

		{
			...
			for (const FString& PluginName : PluginsToConfigure)
			{
				...
				if (Plugin->IsEnabledByDefault(bAllowEnginePluginsEnabledByDefault) && !Context.ConfiguredPluginNames.Contains(PluginName))
				{
					if (!ConfigureEnabledPluginForCurrentTarget(FPluginReferenceDescriptor(PluginName, true),
						Context.EnabledPlugins, SourceDescription, Context.AllowedOptionalDependencies))
					{
						return false;
					}
					Context.ConfiguredPluginNames.Add(PluginName);
...
bool FPluginManager::FindProgramPlugins(FPluginDiscoveryContext& Context)
{
#if IS_PROGRAM
	// Programs can also define the list of enabled plugins in ini
	SCOPED_BOOT_TIMING("AddProgramEnabledPlugins");

	auto EnableProgramPlugin = [this, &Context](const TCHAR* ConfigEntry, bool bOptionalPlugin) mutable
		{
...
void FPluginManager::MarkEnabledPlugins(FPluginDiscoveryContext& Context)
{
	...
	for (TPair<FString, FPlugin*>& Pair : Context.EnabledPlugins)
	{
		...
		{
			PluginLocalizationUtils::GetLocalizationPathsForPlugin(Plugin, Context.AdditionalLocResPaths);
		}
...
void FPluginManager::ProcessEnabledPlugins(FPluginDiscoveryContext& Context)
{
	SCOPED_BOOT_TIMING("ProcessEnabledPlugins");

	PluginsEnabledForTarget.Reserve(Context.EnabledPlugins.Num());
	for (const TPair<FString, FPlugin*>& Pair : Context.EnabledPlugins)
	{
	...
	for (TSharedRef<FPlugin> PluginPtr : PluginsArray)
	{
		...
		FConfigContext ConfigContext = FConfigContext::ReadIntoGConfig();
		ConfigContext.ChangeTracker = &ChangeTracker;
		ConfigContext.ConfigFileTag = *Plugin.Name;
		ConfigContext.Load(*Plugin.Name);
	}
...
void FPluginManager::MountContentPlugins(FPluginDiscoveryContext& Context)
{
...
bool FPluginManager::ConfigureEnabledPlugins()
{
	...
	if(PluginsToConfigure.Num() > 0)
	{
		...
		FPluginDiscoveryContext Context;

		// look for compiletime and commandline plugins
		FindCompileTimePlugins(Context);
		if (!FindCommandLinePlugins(Context))
		{
			return false;
		}

		// look for the engine enabled plugins
		if (!FindTargetPlugins(Context))
		{
			return false;
		}

		// look for program-specific plugins
		if (!FindProgramPlugins(Context))
		{
			return false;
		}

		MarkEnabledPlugins(Context);

		// If we made it here, we have all the required plugins
		bHaveAllRequiredPlugins = true;

		ProcessEnabledPlugins(Context);

		MountContentPlugins(Context);

		if (Context.AdditionalLocResPaths.Num() > 0)
		{
			FTextLocalizationManager::Get().HandleLocalizationTargetsMounted(Context.AdditionalLocResPaths);
		}
...
#if WITH_ENGINE
	// Engine Startup needs to process all phases, even phases not relevant to the current engine configuration,
	// (e.g. PostSplashScreen on Server) because some plugins might load themselves on that phase to ensure
	// they occur before/after related code is intialized on the platforms that have code related to the phase
	// but they still need to be loaded at SOME point on the platforms that do not have any related code. Rather
	// than writing code to find those plugins and have them register on the next phase that occurs, just make
	// sure that every platform processes every phase.
	// Since the constraint enforced above means we can never go backwards in phases, this constraint means that
	// every new phase must be the exact successor of the last completed phase (or a repeat of the last phase).
	UE_CLOG((LastCompletedLoadingPhase == ELoadingPhase::None && LoadingPhase != ELoadingPhase::EarliestPossible)
		|| (LastCompletedLoadingPhase != ELoadingPhase::None
			&& static_cast<int32>(LoadingPhase) > static_cast<int32>(LastCompletedLoadingPhase) + 1),
		LogPluginManager, Error,
		TEXT("LoadModulesForEnabledPlugins called on phase %d after last being called on phase %d, which skipped phase %d."),
		static_cast<int32>(LoadingPhase), static_cast<int32>(LastCompletedLoadingPhase),
		static_cast<int32>(LastCompletedLoadingPhase) + 1);
#endif
...
bool FPluginManager::MountExplicitlyLoadedPlugin(const FString& PluginName, ELoadingPhase::Type MaxLoadingPhase)
{
	bool bSuccess = false;
	...
	{
		MountPluginFromExternalSource(Plugin.ToSharedRef(), MaxLoadingPhase);
		bSuccess = true;
	}
...
void FPluginManager::MountPluginFromExternalSource(const TSharedRef<FPlugin>& Plugin, ELoadingPhase::Type MaxLoadingPhase)
{
	...
	if (Plugin->Descriptor.Modules.Num() > 0)
	{
		...
		for (ELoadingPhase::Type LoadingPhase = (ELoadingPhase::Type)0; LoadingPhase < MaxLoadingPhase; LoadingPhase = (ELoadingPhase::Type)(LoadingPhase + 1))
		{

...
private:
	...
	/**
	 * Helper functions just to split up the massive size of ConfigureEnabledPlugins()
	 */
	struct FPluginDiscoveryContext
	{
	...
	void FindCompileTimePlugins(FPluginDiscoveryContext& Context);
	bool FindCommandLinePlugins(FPluginDiscoveryContext& Context);
	bool FindTargetPlugins(FPluginDiscoveryContext& Context);
	bool FindProgramPlugins(FPluginDiscoveryContext& Context);
	void MarkEnabledPlugins(FPluginDiscoveryContext& Context);
	void ProcessEnabledPlugins(FPluginDiscoveryContext& Context);
	void MountContentPlugins(FPluginDiscoveryContext& Context);
	...
	void MountPluginFromExternalSource(const TSharedRef<FPlugin>& Plugin, ELoadingPhase::Type MaxLoadingPhase = ELoadingPhase::Max);

...
{
	SCOPED_BOOT_TIMING("FProjectDescriptor::Load")
	...
	TValueOrError<UE::Json::FDocument, UE::Json::FParseError> ParseResult = UE::Json::ParseInPlace(Characters);

	if (ParseResult.HasError())
	{
		// have to re-load the file because it was destructively altered
		ensure(FFileHelper::LoadFileToString(FileContents, *FileName));

		// Do not try to localize these messages, a missing project file usually indicates missing data and an
		// access to ICU will result in failure that's harder to diagnose
		OutFailReason = FText::FromString(FString::Printf(TEXT("Failed to read file %s: %s"), *FileName, *ParseResult.GetError().CreateMessage(FileContents)));
		return false;
	}

	TOptional<UE::Json::FConstObject> RootObject = UE::Json::GetRootObject(ParseResult.GetValue());
	if (!RootObject.IsSet())
	{
		return false;
	}

	return Read(*RootObject, FPaths::GetPath(FileName), OutFailReason);
}

bool FProjectDescriptor::Read(const UE::Json::FConstObject Object, const FString& PathToProject, FText& OutFailReason)
{
	SCOPED_BOOT_TIMING("FProjectDescriptor::Read")

	using namespace UE::Json;
	using namespace UE::Projects::Private;

	// Read the file version
	int32 FileVersionInt32 = (int32)EProjectDescriptorVersion::Invalid;
	if(!TryGetNumberField(Object, TEXT("FileVersion"), FileVersionInt32))
	{
		if(!TryGetNumberField(Object, TEXT("ProjectFileVersion"), FileVersionInt32))
		{
	...
	// Read simple fields
	TryGetStringField(Object, TEXT("EngineAssociation"), EngineAssociation);
	TryGetStringField(Object, TEXT("Category"), Category);
	TryGetStringField(Object, TEXT("Description"), Description);
	TryGetBoolField(Object, TEXT("Enterprise"), bIsEnterpriseProject);
	TryGetBoolField(Object, TEXT("DisableEnginePluginsByDefault"), bDisableEnginePluginsByDefault);

#if WITH_EDITOR
	ModuleNamesCache.Reset();
#endif

	if (TOptional<FText> ReadModulesError = ReadArray(Object, TEXT("Modules"), Modules))
	{
		OutFailReason = *ReadModulesError;
		return false;
	}

	if (TOptional<FText> ReadPluginsError = ReadArray(Object, TEXT("Plugins"), Plugins))
	{
		OutFailReason = *ReadPluginsError;
		return false;
	}

	// Read the list of additional plugin directories to scan
	TArray<FString> AdditionalPluginDirectoriesValue;
	if (TryGetStringArrayField(Object, TEXT("AdditionalPluginDirectories"), AdditionalPluginDirectoriesValue))
	{
#if WITH_EDITOR || (IS_PROGRAM && WITH_PLUGIN_SUPPORT)
		for (FString& AdditionalPluginDir : AdditionalPluginDirectoriesValue)
		{
	...
	TArray<FString> AdditionalRootDirectoriesValue;
	if (TryGetStringArrayField(Object, TEXT("AdditionalRootDirectories"), AdditionalRootDirectoriesValue))
	{
		for (FString& AdditionalRoot : AdditionalRootDirectoriesValue)
		{
	...
	TryGetStringArrayField(Object, TEXT("TargetPlatforms"), TargetPlatforms);
	...
	TryGetNumberField(Object, TEXT("EpicSampleNameHash"), EpicSampleNameHash);

	// Read the custom build steps
	PreBuildSteps = ReadCustomBuildSteps(Object, TEXT("PreBuildSteps"));
	PostBuildSteps = ReadCustomBuildSteps(Object, TEXT("PostBuildSteps"));
...
{
	return UE::Projects::Private::ReadFromDefaultJsonHelper(
		Object,
		[&PathToProject, &OutFailReason, this](UE::Json::FConstObject Object)
		{
			return this->Read(Object, PathToProject, OutFailReason);
		}
	);
}

...
struct COREUOBJECT_API FBlueprintContextTracker : TThreadSingleton<FBlueprintContextTracker>
{
	...
	UE_FORCEINLINE_HINT int32 AddRunaway()
	{
		return ++Runaway;
	}
	...
	UE_FORCEINLINE_HINT int32 GetScriptEntryTag() const
	{
	...
	UE_FORCEINLINE_HINT TArray<const FFrame*> GetScriptStack() const
	{
		return TArray<const FFrame*>(GetCurrentScriptStack());
	}

	/** Returns current script stack frame */
	UE_FORCEINLINE_HINT TArrayView<const FFrame* const> GetCurrentScriptStack() const
	{
		return MakeArrayView<const FFrame* const>(ScriptStack.GetData(), ScriptStack.Num());
	}

	UE_FORCEINLINE_HINT TArrayView<FFrame* const> GetCurrentScriptStackWritable() const
	{

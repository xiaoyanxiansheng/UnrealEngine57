...
// Allow UObject reflection info to be initialized in the binary at compile time instead of allocated on the heap at runtime.
#ifndef UE_WITH_CONSTINIT_UOBJECT
	#define UE_WITH_CONSTINIT_UOBJECT 0
#endif

// Allows relaxing uobject constinit declarations to be ordinary globals with dynamic initialization allowed
#ifndef UE_CONSTINIT_UOBJECT_DECL
...
enum EObjectFlags
{
	...
	RF_MigratingAsset			=0x20000000,	///< Objects marked as MigratingAsset can be migrated to another server even if they've been loaded from disk
...
enum class EInternalObjectFlags : int32
{
	...
	Remote = 1 << 0, //< Object is no longer owned by this process
	RemoteReference = 1 << 1, //< Object referenced by remote process
	Borrowed = 1 << 2, //< Object that is not owned by this process 
#else
	Remote = 0,
	RemoteReference = 0,
	Borrowed = 0,
	...
	// NOTE that the minimum flag bit index is currently 14 (unless compiling with UE_WITH_REMOTE_OBJECT_HANDLE). 
	// See EInternalObjectFlags_MinFlagBitIndex and UE_ENABLE_FUOBJECT_ITEM_PACKING in UObjectArray.h
...
namespace UM
{
	...
	enum
	{
		...
		/// [InterfaceMetadata] If this metadata is set to true, then the interface cannot generate generic "Message"
		/// nodes which are used for type-agnostic message passing.
		/// 
		/// This is useful if you want to clean up the blueprint action context menu and force users to use a different method of calling
		/// into your interface.
		///
		/// Example:
		///		UINTERFACE(MinimalAPI, meta=(CannotGenerateMessageNodes=true))
		///		class UFooBarSubsystemInterface : public UInterface { GENERATED_BODY() };
		///
		///	This metadata is read in the BlueprintActionDatabaseImpl::AddClassFunctionActions function
...
public: \
	...
	/** Returns the unique cast flag for this class if any */ \
	inline constexpr static EClassCastFlags StaticClassCastFlags() \
	{ \
		return TStaticCastFlags; \
	} \
	/** Returns all cast flags for this class, including those inherited from parent classes */ \
	inline consteval static EClassCastFlags StaticAllClassCastFlags() \
	{ \
		if constexpr (std::is_same_v<ThisClass, Super>) { return StaticClassCastFlags(); } \
		return StaticClassCastFlags() | Super::StaticAllClassCastFlags(); \
	} \
...
// Namespace should be in the form of "Whatever::" or blank for no namespace
#define DEFINE_VTABLE_PTR_HELPER_CTOR_NS(Namespace, TClass) \
...
#define DEFINE_VTABLE_PTR_HELPER_CTOR_CALLER_DUMMY() \
	...
	#define DEFINE_VTABLE_PTR_HELPER_CTOR_CALLER(TClass) \
		static UObject* __VTableCtorCaller(FVTableHelper& Helper) \
		{ \
			return new (EC_InternalUseOnlyConstructor, GetTransientPackageAsObject(), NAME_None, RF_NeedLoad | RF_ClassDefaultObject | RF_TagGarbageTemp) TClass(Helper); \
		}
...
#define DECLARE_CLASS_INTRINSIC_NO_CTOR(TClass,TSuperClass,TStaticFlags,TPackage) \
	...
	UE_DELETE_COPY_MOVE_CONSTRUCTORS(TClass) \
	DEFINE_DEFAULT_OBJECT_INITIALIZER_CONSTRUCTOR_CALL(TClass) \
...
public:
	...
	explicit consteval FUObjectCppClassStaticFunctions(TClass* Null)
	: UOBJECT_CPPCLASS_STATICFUNCTIONS_FORCLASS(TClass)
	{
	}

	explicit constexpr FUObjectCppClassStaticFunctions(AddReferencedObjectsType InAddReferencedObjects
#if WITH_EDITORONLY_DATA
...
#endif // !UE_WITH_CONSTINIT_UCLASS

#if UE_WITH_CONSTINIT_UOBJECT
#define IMPLEMENT_INTRINSIC_CLASS(TClass, TRequiredAPI, TSuperClass, TSuperRequiredAPI, TPackage, InitCode) \
	IMPLEMENT_INTRINSIC_CLASS_CONSTINIT(TClass, TRequiredAPI, TSuperClass, TSuperRequiredAPI, TPackage, InitCode)
...
#if UE_WITH_CONSTINIT_UOBJECT
// CONSTINIT_UOBJECT_TODO: Should intrinsic classes inherit class flags here rather than waiting until Link?
#define IMPLEMENT_INTRINSIC_CLASS_CONSTINIT(TClass, TRequiredAPI, TSuperClass, TSuperRequiredAPI, TPackage, InitCode) \
	static constinit FCppClassTypeInfoStatic Z_StaticCppClassTypeInfo_##TClass = { \
		TCppClassTypeTraits<TClass>::IsAbstract, \
	}; \
	constinit extern UPackage Z_ConstInit_UPackage__Script_##TPackage; \
	constinit extern TNoDestroy<UClass> Z_ConstInit_UClass_UClass; \
	constinit extern TNoDestroy<UClass> Z_ConstInit_UClass_##TSuperClass; \
	constinit TNoDestroy<UClass> Z_ConstInit_UClass_##TClass{NoDestroyConstEval, \
		UE::CodeGen::ConstInit::FObjectParams{ \
			.Flags = RF_Public|RF_Transient|RF_MarkAsNative|RF_MarkAsRootSet, \
			.Class = std::is_same_v<UClass, TClass> ? nullptr : &Z_ConstInit_UClass_UClass, \
			/* Trim the U/A prefix which is still a single byte in UTF8 */ \
			.NameUTF8 = &UTF8TEXT(UE_STRINGIZE(TClass))[1], \
			.Outer = &Z_ConstInit_UPackage__Script_##TPackage, \
		}, \
		UE::CodeGen::ConstInit::FUFieldParams{}, \
		UE::CodeGen::ConstInit::FStructParams{ \
			.Super = &Z_ConstInit_UClass_##TSuperClass, \
			.BaseChain = TIntrinsicClassBaseChain<TClass>::ToArrayView(), \
			.PropertiesSize = sizeof(TClass), \
			.MinAlignment = alignof(TClass), \
		}, \
		UE::CodeGen::ConstInit::FClassParams{ \
			.ConfigName = TClass::StaticConfigName(), \
			.ClassConstructor = InternalConstructor<TClass>, \
			.ClassVTableHelperCtorCaller = InternalVTableHelperCtorCaller<TClass>, \
			.CppClassStaticFunctions = UOBJECT_CPPCLASS_STATICFUNCTIONS_FORCLASS(TClass), \
			.ClassFlags = TClass::StaticClassFlags, \
			.ClassCastFlags = TClass::StaticAllClassCastFlags(), \
			.CppClassTypeInfo = &Z_StaticCppClassTypeInfo_##TClass, \
			.ClassWithin = UE::Private::GetConstInitClass<TClass::WithinClass>(), \
		}, \
	}; \
	struct Z_Construct_UClass_##TClass##_Statics { \
		static void Construct() \
		{ \
			UClass* SuperClass = TSuperClass::StaticClass(); \
			UClass* Class = TClass::StaticClass(); \
			InitCode; \
		} \
	}; \
	static FRegisterCompiledInObjects Z_Register_UClass_##TClass{ &Z_ConstInit_UClass_##TClass, &Z_Construct_UClass_##TClass##_Statics::Construct }; \
	TRequiredAPI UClass* TClass::GetPrivateStaticClass() \
	{ \
		return &Z_ConstInit_UClass_##TClass; \
	}
#endif // UE_WITH_CONSTINIT_UOBJECT

#if UE_WITH_CONSTINIT_UOBJECT
#define IMPLEMENT_CORE_INTRINSIC_CLASS(TClass, TSuperClass, InitCode) \
	IMPLEMENT_INTRINSIC_CLASS_CONSTINIT(TClass, COREUOBJECT_API, TSuperClass, COREUOBJECT_API, CoreUObject, InitCode)
#else
#define IMPLEMENT_CORE_INTRINSIC_CLASS(TClass, TSuperClass, InitCode) \

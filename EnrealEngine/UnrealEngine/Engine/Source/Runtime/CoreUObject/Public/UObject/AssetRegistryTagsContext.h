...
public:
	...
	/** Removes the tag from the output. */
	...
	/** Removes the cooktag from the output. */
	COREUOBJECT_API void RemoveCookTag(FName TagName);

private:
	bool TryValidateTagName(FName TagName) const;
	FName TransformTagName(FName TagName, FStringView PrefixToAdd, bool bCreateNameIfDoesNotExist = true) const;
	...
	void RemoveTagInternal(FName TagName, FStringView PrefixToAdd);

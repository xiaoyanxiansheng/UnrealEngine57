...
struct FPropertyPathNameSegment
{
	...
	/** Index within a container, or INDEX_NONE if not in a container. 
		Associative containers (Sets and Maps) use virtual indices that skip gaps instead of logical indices. */
	...
	UE_API static bool FromJsonValue(const TSharedPtr<FJsonValue>& JsonValue, FPropertyPathNameSegment& OutSegment);
};
...
public:
	...
	UE_API TSharedRef<FJsonValue> ToJsonValue() const;
	UE_API static bool FromJsonValue(const TSharedPtr<FJsonValue>& JsonValue, FPropertyPathName& OutPath);

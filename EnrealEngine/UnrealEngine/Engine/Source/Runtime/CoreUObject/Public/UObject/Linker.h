...
public:
	 ...
	 */
	inline FObjectResource& ImpExp(FPackageIndex Index)
	{
		check(!Index.IsNull());
		if (Index.IsImport())
		{
	 ...
	 * @return	the resource corresponding to this index
	 */
	inline FObjectResource const& ImpExp(FPackageIndex Index) const
	{
		check(!Index.IsNull());
		if (Index.IsImport())
		{
	 ...
	 */
	inline FObjectResource* ImpExpPtr(FPackageIndex Index)
	{
		if (Index.IsImport())
		{
			return ImpPtr(Index);
	 ...
	 */
	inline FObjectImport& Imp(FPackageIndex Index)
	{
		check(Index.IsImport() && ImportMap.IsValidIndex(Index.ToImport()));
		return ImportMap[Index.ToImport()];
	}
	inline FObjectImport const& Imp(FPackageIndex Index) const
	{
		check(Index.IsImport() && ImportMap.IsValidIndex(Index.ToImport()));
		return ImportMap[Index.ToImport()];
	}
	 ...
	 */
	inline FObjectImport* ImpPtr(FPackageIndex Index)
	{
		if (Index.IsImport())
		{
	 ...
	 */
	inline FObjectExport& Exp(FPackageIndex Index)
	{
		check(Index.IsExport() && ExportMap.IsValidIndex(Index.ToExport()));
		return ExportMap[Index.ToExport()];
	}
	inline FObjectExport const& Exp(FPackageIndex Index) const
	{
		check(Index.IsExport() && ExportMap.IsValidIndex(Index.ToExport()));
		return ExportMap[Index.ToExport()];
	}
	 ...
	 */
	inline FObjectExport* ExpPtr(FPackageIndex Index)
	{
		if (Index.IsExport())
		{
...
template<typename T>
inline T* CastChecked(FLinker* Src)
{

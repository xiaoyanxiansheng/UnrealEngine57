...
public:
	...
	explicit consteval UObject(UE::CodeGen::ConstInit::FObjectParams InParams)
		: UObjectBaseUtility(InParams)
	{
	}
#endif
...
#else
	UE_FORCEINLINE_HINT bool Modify(bool bAlwaysMarkDirty = true) { return false; }
	UE_FORCEINLINE_HINT bool IsCapturingAsRootObjectForTransaction() const { return false; };
#endif
	...
	*/
	...
	struct FPostLoadAssetRegistryTagsContext
	{
		FPostLoadAssetRegistryTagsContext(const FAssetData& InAssetData, TArray<FAssetRegistryTag>& InTagsAndValuesToUpdate,
			bool bInCanCreateObjects)
		: AssetData(InAssetData), TagsAndValuesToUpdate(InTagsAndValuesToUpdate), bCanCreateObjects(bInCanCreateObjects) { }
		...
		bool CanCreateObjects() const { return bCanCreateObjects; }
		bool PostLoadCouldComplete() const { return bPostLoadCouldComplete; }
		void SetPostLoadCouldComplete(bool bValue) { bPostLoadCouldComplete = bValue; }
	private:
		...
		bool bPostLoadCouldComplete = true;
	};
...
public:
	...
	/** 
	 * Returns the list of replicated properties for the class and their replication configuration.
	 * This function used to require every replicated property of the class to be registered in the array. 
	 * But now you can leave it empty (or call the Super) and the class's replicated properties will still be replicated.
	 * The NetDriver will automatically add all properties that are CPF_Net and not found in the array.
	 * Note that these auto-added properties will use default configs: No conditions, PushModel = off, etc.
	 */
...
public:
	 ...
	 * @param bSilenceLogs				If true, this function will not output non-fatal log messages
	 */
...
{
	inline static bool CheckObjectValidBasedOnItsFlags(const UObject* Test)
	{
...
*/
inline bool IsValidChecked(const UObject* Test)
{
	check(Test);

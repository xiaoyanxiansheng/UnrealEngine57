...
struct FObjectMemoryUsage
{
	...
	UE_FORCEINLINE_HINT bool IsRoot() const { return !!(Flags&EObjFlags::IsRoot); }
	UE_FORCEINLINE_HINT bool IsReferencedByRoot() const { return !!(Flags&EObjFlags::IsReferencedByRoot); }
	UE_FORCEINLINE_HINT bool IsProcessed() const { return !!(Flags&EObjFlags::IsProcessed); }
	UE_FORCEINLINE_HINT bool IsReferencedByNonRoot() const { return !!(Flags&EObjFlags::IsReferencedByNonRoot); }
...
struct FObjectMemoryAnalyzer
{
	...
	{		
		...
		inline bool operator()( const FObjectMemoryUsage& A, const FObjectMemoryUsage& B ) const
		{
			switch (SortKey)
			{

...
public:
	...
	{
		Idle,
		Marking,     // Finding roots and traversing the heap
		PostMarking, // Successfully completed a marking pass
		Census,      // Cleaning up weakly referenced objects
		Destroying,  // Running destructors on unreferenced objects
		Sweeping,    // Reclaiming memory
		Finishing,   // Ending the collection process, returning to idle
	};
	...
	{
		return State == EState::Marking;
	}

	static bool IsDestroying()
	{
		return State == EState::Destroying;
	}
...
private:
	...
	COREUOBJECT_API static UE::FConditionVariable ConditionVariable;
	...
	COREUOBJECT_API static EState State;

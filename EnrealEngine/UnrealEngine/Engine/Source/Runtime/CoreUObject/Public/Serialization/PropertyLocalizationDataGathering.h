...
enum class EPropertyLocalizationGathererTextFlags : uint8
{
	 ...
	 * @note Does not apply to properties gathered from script data (see SkipScriptData).
	 */
	SkipProperties = 1<<4,
...
private:
	...
	struct FObjectAndGatherFlags
	{
		...
		inline bool operator==(const FObjectAndGatherFlags& Other) const
		{
			return Object == Other.Object 
				&& GatherTextFlags == Other.GatherTextFlags;
		}

		UE_FORCEINLINE_HINT bool operator!=(const FObjectAndGatherFlags& Other) const
		{
			return !(*this == Other);
		}

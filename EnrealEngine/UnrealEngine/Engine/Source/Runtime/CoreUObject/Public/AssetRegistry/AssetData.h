...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	inline void SerializeForCache(Archive&& Ar)
	{
	 ...
	 */
	template<class Archive>
	inline void SerializeForCacheOldVersion(Archive&& Ar, FAssetRegistryVersion::Type Version = FAssetRegistryVersion::LatestVersion)
	{
...
private:
	 ...
	 */
	UE_FORCEINLINE_HINT void SerializeForCacheInternal(FArchive& Ar, FAssetRegistryVersion::Type Version, void (*SerializeTagsAndBundles)(FArchive& , FAssetData&, FAssetRegistryVersion::Type));
...
public:
	...
	bool IsReadOnly() const { return (Flags & FLAG_PACKAGE_READ_ONLY) != 0; }
	void SetIsReadOnly(bool bValue) { Flags = (Flags & ~FLAG_PACKAGE_READ_ONLY) | (bValue ? FLAG_PACKAGE_READ_ONLY : 0); }
...
private:
	UE_FORCEINLINE_HINT void SerializeForCacheInternal(FArchive& Ar, FAssetPackageData& PackageData, FAssetRegistryVersion::Type Version);

	enum : uint32
	{
		FLAG_LICENSEE_VERSION						= 1u << 0,
		FLAG_HAS_VIRTUALIZED_PAYLOADS				= 1u << 1,
		FLAG_PACKAGE_READ_ONLY						= 1u << 2,

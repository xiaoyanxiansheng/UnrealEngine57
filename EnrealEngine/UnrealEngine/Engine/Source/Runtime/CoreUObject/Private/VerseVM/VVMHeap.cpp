...
void FHeap::Initialize()
{
	if (!bIsInitialized)
	{
		...
		verse_heap_live_bytes_trigger_threshold = static_cast<size_t>(CVarHeapMinimumTrigger.GetValueOnAnyThread());
...
bool FHeap::NormalizeWithoutThreadingAtCollectionStart()
{
	...
	if (bWithoutThreading)
	{
		V_DIE_UNLESS(State == EState::Idle);
...
void FHeap::RunCollectionCycle(FIOContext Context)
{
	...
	do
	{
		V_DIE_UNLESS(State == EState::Marking);

		Mark(Context);
	}
	while (!AttemptToTerminate(Context));

	V_DIE_UNLESS(State == EState::PostMarking);
...
void FHeap::ConductCensus(FIOContext Context)
{
	...
	V_DIE_UNLESS(State == EState::Census);
...
{
	V_DIE_UNLESS(State == EState::Destroying);
	V_DIE_UNLESS(bIsTerminated);
	V_DIE_UNLESS(WeakBarrierState == EWeakBarrierState::CheckMarkedOnRead);
...
{
	using namespace UE;
	V_DIE_UNLESS(State == EState::Sweeping);
	V_DIE_UNLESS(bIsTerminated);
	{
		TUniqueLock Lock(Mutex);
		V_DIE_UNLESS(WeakBarrierState == EWeakBarrierState::CheckMarkedOnRead);
...
{
	using namespace UE;
	V_DIE_UNLESS(State == EState::Finishing);
	V_DIE_UNLESS(bIsTerminated);
	...
	if (CompletedCycleVersion == RequestedCycleVersion)
	{
		...
		NewTrigger = std::max(NewTrigger, static_cast<size_t>(CVarHeapMinimumTrigger.GetValueOnAnyThread()));
...
{
	V_DIE_UNLESS(bIsExternallyControlled);
	return !bIsGCMarkingExternallySignaled && RequestedCycleVersion > CompletedCycleVersion;
}

...
{
	FailureContext->Trail.Enter(Context);
	FailureContext->Transaction.Start(Context);
}

bool FNativeFrame::IsCurrent(FRunningContext Context) const
{
	return &FailureContext->Transaction == Context.CurrentTransaction();
}
...
void FNativeFrame::CommitIfNoAbort(FRunningContext Context) const
{
	if (!FailureContext->Transaction.bHasAborted)
	{
		...
		FailureContext->Trail.Exit(Context);
	}
}

VFailureContext* FNativeFrame::RootFailureContext() const
{
	VFailureContext* Context = FailureContext;
...
void FContextImpl::StartComputationWatchdog()
{
	...
	if (StartTime == 0.0)
	{
		auto WatchdogLambda = [this] {
			while (true)
			{
				UE::FEventCountToken Token = WaitForVerseEvent.PrepareWait();
				if (!bVerseIsRunning)
				{
					WaitForVerseEvent.Wait(Token);
					continue;
				}
		...
		// Set to forkable as otherwise we don't actually run on UEFN's single-threaded servers
		ComputationWatchdog = FThread(
			TEXT("VVMComputationWatchdog"),
			WatchdogLambda,
			0,
...
void FContextImpl::StopComputationWatchdog()
{
	...
	bVerseIsRunning.store(true, std::memory_order_seq_cst);
	WaitForVerseEvent.Notify();
	if (ComputationWatchdog.IsJoinable())
	{
		ComputationWatchdog.Join();
	}
}
...
FOpResult FRunningContext::HandleAutoRTFMFailure(AutoRTFM::EContextStatus Status)
{
	AutoRTFM::UnreachableIfClosed("#jira SOL-8415");

	FNativeFrame* const Native = NativeFrame();

	// Has the failure context already aborted?
	// This can happen if:
	// * The native function called into RaiseVerseRuntimeError(), in which case
	//   all the transactions have already been aborted.
	// * The native function called back into the VM, a failure was raised which
	//   called into HandleAutoRTFMFailure(), and we've unwound back to the
	//   outer native call which called HandleAutoRTFMFailure() again.
	const bool bAlreadyAborted = Native->FailureContext->Transaction.bHasAborted;

	// The AutoRTFM::Close() call will have already aborted and popped the
	// inner AutoRTFM transaction. To prevent a future call to
	// Verse::FTransaction::Abort() attempting to rollback this transaction
	// a second time, set bHasRolledBackAutoRTFMTransaction to true.
	V_DIE_IF(bAlreadyAborted != Native->FailureContext->Transaction.bHasRolledBackAutoRTFMTransaction);
	Native->FailureContext->Transaction.bHasRolledBackAutoRTFMTransaction = true;

	switch (Status)
	{
		case AutoRTFM::EContextStatus::AbortedByRequest:
			// Returning FOpResult::Fail will cause the FRunningContext::Fail()
			// to be called on the current failure context, which will abort the
			// top most transaction.
			return FOpResult::Fail;

		case AutoRTFM::EContextStatus::AbortedByLanguage:
			if (!bAlreadyAborted)
			{
				// Clear the status so we can rollback the parent transactions.
				// The status will be reset to AbortedByCascadingAbort by RaiseVerseRuntimeError().
				AutoRTFM::ForTheRuntime::ClearTransactionStatus();
				// Treat an AutoRTFM language failure (e.g. reaching an unsupported instruction)
				// as a runtime failure.
				const ERuntimeDiagnostic Diagnostic = ERuntimeDiagnostic::ErrRuntime_TransactionAbortedByLanguage;
				const SRuntimeDiagnosticInfo& DiagnosticInfo = GetRuntimeDiagnosticInfo(Diagnostic);
				RaiseVerseRuntimeError(Diagnostic, FText::FromString(DiagnosticInfo.Description));
			}
			return FOpResult::Error;

		case AutoRTFM::EContextStatus::AbortedByCascadingAbort:
			if (!bAlreadyAborted)
			{
				// Clear the status so we can rollback the parent transactions.
				// The status will be restored to AbortedByCascadingAbort by
				// CascadingAbortRollbackTransaction().
				AutoRTFM::ForTheRuntime::ClearTransactionStatus();

				// Rollback all the transactions owned by the VVM.
				Native->RootFailureContext()->Fail(*this);

				// Propagate the cascade-abort to the outer transaction.
				// This immediately rolls back the transaction, but does not
				// immediately pop it from the AutoRTFM transaction stack.
				// Once execution reaches the open -> closed boundary, this
				// transaction is popped and all outer transactions will be aborted.
				AutoRTFM::ForTheRuntime::CascadingAbortRollbackTransaction();
			}
			return FOpResult::Error;

		case AutoRTFM::EContextStatus::AbortedByCascadingRetry:
			if (!bAlreadyAborted)
			{
				// Clear the status so we can rollback the parent transactions.
				// The status will be restored to AbortedByCascadingRetry by
				// CascadingRetryRollbackTransaction().
				AutoRTFM::ForTheRuntime::ClearTransactionStatus();

				// Rollback all the transactions owned by the VVM.
				Native->RootFailureContext()->Fail(*this);

				// Propagate the cascade-retry to the outer transaction.
				// This immediately rolls back the transaction, but does not
				// immediately pop it from the AutoRTFM transaction stack.
				// Once execution reaches the open -> closed boundary, this
				// transaction is popped and all outer transactions will be aborted,
				// then the top-most transaction will be attempted again.
				AutoRTFM::ForTheRuntime::CascadingRetryRollbackTransaction();
			}
			return FOpResult::Error;

		default:
			V_DIE("Unexpected EContextStatus value: %d", static_cast<int>(Status));
			return FOpResult::Error;
	}
}

...
FString FPackageName::GetShortName(FStringView LongName)
{
	...
	return FString(LongName.Mid(IndexOfLastSlash + 1));
}
...
UE::Core::FVersePath FPackageName::LongPackagePathToVersePath(FStringView LongPackagePath)
{
	...
	bool bHadClassesPrefix = false;
	const FStringView MountPointName = FPathViews::GetMountPointNameFromPath(LongPackagePath, &bHadClassesPrefix);
	if (MountPointName.IsEmpty() || bHadClassesPrefix)
	{
		return {};
	}
	...
	// If the plugin doesn't have a root vpath, the object doesn't have a Verse path.
	const FString& PluginVersePath = Plugin->GetVersePath();
	if (PluginVersePath.IsEmpty())
	{
		return {};
	...
	// Don't FPaths::Combine if getting the Verse path for the root plugin path, FPaths::Combine will add a trailing '/' making the Verse path invalid.
	const int32 CharCount = MountPointName.Len() + 1;
	UE::Core::FVersePath Result;
	UE::Core::FVersePath::TryMake(Result, LongPackagePath.Len() == CharCount ? PluginVersePath : FPaths::Combine(PluginVersePath, LongPackagePath.RightChop(CharCount)));
	return Result;
}

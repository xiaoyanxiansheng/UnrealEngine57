...
static bool IsUnrealGCMarkingPassInProgress()
{
	return !GObjUnhashUnreachableIsInProgress && !GObjIncrementalPurgeIsInProgress && IsGarbageCollectingAndLockingUObjectHashTables();
}

static bool IsFrankenGCDestroyingPassInProgress()
{
#if WITH_VERSE_VM
	// The FrankenGC object-destruction pass occurs immediately after the marking 
	// pass of both Verse and Unreal has finished and reached a steady-state. 
	// Unfortunately `IsUnrealGCMarkingPassInProgress` still returns true at that 
	// point, even though all marking has actually concluded. 
	// We detect this case here.
	return UE::GC::GIsFrankenGCCollecting && Verse::FHeap::IsDestroying();
#else
	return false;
#endif
}
...
{
	check(Object);
	check(!IsUnrealGCMarkingPassInProgress() || IsFrankenGCDestroyingPassInProgress());

	UE::TScopeLock ReferencedObjectsLock(Impl->ReferencedObjectsCritical);

...
#include UE_INLINE_GENERATED_CPP_BY_NAME(UserDefinedStruct)
...
void UUserDefinedStruct::SerializeTaggedProperties(FStructuredArchive::FSlot Slot, uint8* Data, const UStruct* DefaultsStruct, const uint8* Defaults, const UObject* BreakRecursionIfFullyLoad) const 
{
	...
	const UUserDefinedStruct* UDDefaultsStruct = Cast<UUserDefinedStruct>(DefaultsStruct);
	...
	if (bUseNewDefaults)
	{
		Defaults = UDDefaultsStruct->GetDefaultInstance();
	}

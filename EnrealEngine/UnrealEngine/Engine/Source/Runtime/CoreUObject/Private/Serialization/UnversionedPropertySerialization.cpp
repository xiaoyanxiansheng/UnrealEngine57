...
struct FDefaultStruct
{
	...
	FDefaultStruct(const uint8* InData, const UStruct* InStruct) : Data(InData), StructSize(InData ? InStruct->GetPropertiesSize() : 0)  {}
#else
	const UStruct* Struct;

	FDefaultStruct(const uint8* InData, const UStruct* InStruct) : Data(InData), Struct(InStruct) {}
#endif
};
...
struct FUnversionedStructSchema
{
...
#if WITH_EDITORONLY_DATA
	static void AppendObjectPath(const UObject* Object, FBlake3& HashBuilder)
	{
	...
	static void AppendClassMetaData(const UStruct* Struct, FBlake3& HashBuilder)
	{
		...
		// Append some properties of the struct that can impact instances' serialization or errors.
		const UClass* StructAsClass = Cast<const UClass>(Struct);
		if (StructAsClass)
		{
			AppendObjectPath(StructAsClass->ClassWithin, HashBuilder);
		}
...
static bool CanUseUnversionedPropertySerialization(const FConfigFile& TargetIni)
{
	bool bAllow;
	return TargetIni.GetBool(TEXT("Core.System"), TEXT("CanUseUnversionedPropertySerialization"), /* out */ bAllow) && bAllow;
}

static bool CanUseUnversionedPropertySerializationForServerOnly(const FConfigFile& TargetIni)
{
	bool bAllow;
...
bool CanUseUnversionedPropertySerialization(const ITargetPlatform* Target)
{
	...
	const FConfigFile& TargetIni = *FConfigCacheIni::ForPlatform(*Target->IniPlatformName())->FindConfigFile(GEngineIni);

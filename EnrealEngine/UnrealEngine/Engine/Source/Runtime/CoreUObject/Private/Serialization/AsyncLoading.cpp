...
FObjectImport* FAsyncPackage::FindExistingImport(int32 LocalImportIndex)
{
	...
	if (!Import->XObject && !Import->bImportSearchedFor)
	{
		...
		{
			Import->XObject = StaticFindObjectFast(UPackage::StaticClass(), nullptr, Linker->GetInstancingContext().RemapPackage(Import->ObjectName), EFindObjectFlags::ExactClass);
		...
		else if (Import->OuterIndex.IsImport())
		{
			...
			{
				Import->XObject = StaticFindObjectFast(UObject::StaticClass(), ImportOuter->XObject, Import->ObjectName, EFindObjectFlags::None);
				if (Import->XObject)
				{
	...
	while (ImportIndex < Linker->ImportMap.Num())
	{
		...
		if (Import.OuterIndex.IsNull())
		{
			if (!Import.bImportFailed)
			{
				...
				if (!ImportPackage)
				{
					ImportPackage = FindObjectFast<UPackage>(NULL, Import.ObjectName, EFindObjectFlags::None);
					if (!ImportPackage)
					{
						Import.bImportFailed = true;
			...
			|| GIsInitialLoad) // during the initial load, we might find the object, but it hasn't yet actually been finished
		{
			...
			if (!ImportPackage)
			{
				ImportPackage = FindObjectFast<UPackage>(nullptr, ImportPackageName, EFindObjectFlags::None);
				if (!ImportPackage)
				{
					Import.bImportFailed = true;
	...
	if (!Import.XObject && !Import.bImportFailed)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					UPackage* ClassPackage = FindObjectFast<UPackage>(NULL, Import.ClassPackage, EFindObjectFlags::None);
					if (ClassPackage)
					{
						UClass*	FindClass = FindObjectFast<UClass>(ClassPackage, Import.ClassName, EFindObjectFlags::None);
						if (FindClass)
						{
	...
	if (!Export.Object && !Export.bExportLoadFailed)
	{
		...
		if (!Linker->FilterExport(Export)) // for some acceptable position, it was not "not for" 
		{
			...
			check(LoadContext->HasStartedLoading());
			{
				...
				UObject* ActualObjectWithTheName = StaticFindObjectFastInternal(NULL, ThisParent, Export.ObjectName, EFindObjectFlags::ExactClass);
...
void FAsyncLoadingThread::NotifyRegistrationEvent(FName PackageName, FName Name, ENotifyRegistrationType NotifyRegistrationType, ENotifyRegistrationPhase NotifyRegistrationPhase, UObject* (*InRegister)(), bool InbDynamic, UObject* FinishedObject)
{
...
EAsyncPackageState::Type FAsyncPackage::LoadImports()
{
	...
	while (LoadImportIndex < Linker->ImportMap.Num() && !IsTimeLimitExceeded())
	{
		...
		UPackage* ExistingPackage = dynamic_cast<UPackage*>(StaticFindObjectFast(UPackage::StaticClass(), nullptr, ImportPackageFName, EFindObjectFlags::ExactClass));

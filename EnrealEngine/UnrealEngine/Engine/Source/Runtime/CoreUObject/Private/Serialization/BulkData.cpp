...
FStringBuilderBase& LexToString(EBulkDataFlags Flags, FStringBuilderBase& Sb)
{
	...
	TEST_AND_ADD_FLAG(Sb, Flags, BULKDATA_PayloadInSeparateFile);
...
void FBulkData::Serialize(FArchive& Ar, UObject* Owner, bool bAttemptFileMapping, int32 ElementSize, EFileRegionType FileRegionType)
{
	...
	if (Ar.IsPersistent() && !Ar.IsObjectReferenceCollector() && !Ar.ShouldSkipBulkData())
	{
		...
		else if (Ar.IsSaving())
		{
			...
			const EBulkDataFlags FlagsToClear = static_cast<EBulkDataFlags>(BULKDATA_PayloadAtEndOfFile | BULKDATA_PayloadInSeparateFile | BULKDATA_WorkspaceDomainPayload | BULKDATA_ForceSingleElementSerialization);
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	const bool bPayloadInSeparateFile = !bPayloadInline && (InBulkDataFlags & BULKDATA_PayloadInSeparateFile);

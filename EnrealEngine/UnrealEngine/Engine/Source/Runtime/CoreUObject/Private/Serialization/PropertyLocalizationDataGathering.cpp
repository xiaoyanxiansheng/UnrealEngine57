	...
	, AllObjectsInPackage()
{
	...
	{
		if (!EnumHasAnyFlags(BytecodeToGather.GatherTextFlags, EPropertyLocalizationGathererTextFlags::SkipScriptData))
		{
...
void FPropertyLocalizationDataGatherer::GatherLocalizationDataFromObjectWithCallbacks(const UObject* Object, const EPropertyLocalizationGathererTextFlags GatherTextFlags)
{
	...
	if (Object->GetClass()->GetPackage()->HasAnyPackageFlags(PKG_EditorOnly) || Object->GetClass()->HasAnyClassFlags(CLASS_Optional))
	{
...
void FPropertyLocalizationDataGatherer::GatherLocalizationDataFromObjectFields(const FString& PathToParent, const UObject* Object, const EPropertyLocalizationGathererTextFlags GatherTextFlags)
{
	...
	for (const FProperty* PropertyField : GatherableFieldsForType.Properties)
	{
		...
		GatherLocalizationDataFromChildTextProperties(PathToParent, PropertyField, ValueAddress, DefaultValueAddress, GatherTextFlags | (IsEditorOnlyProperty(PropertyField) ? EPropertyLocalizationGathererTextFlags::ForceEditorOnly : EPropertyLocalizationGathererTextFlags::None));
	}
...
void FPropertyLocalizationDataGatherer::GatherLocalizationDataFromChildTextProperties(const FString& PathToParent, const FProperty* const Property, const void* const ValueAddress, const void* const DefaultValueAddress, const EPropertyLocalizationGathererTextFlags GatherTextFlags)
{
	...
	const EPropertyLocalizationGathererTextFlags FixedChildPropertyGatherTextFlags = GatherTextFlags | (IsEditorOnlyProperty(Property) ? EPropertyLocalizationGathererTextFlags::ForceEditorOnly : EPropertyLocalizationGathererTextFlags::None);
	...
	for(int32 i = 0; i < Property->ArrayDim; ++i)
	{
		...
		if (TextProperty)
		{
			...
			else if (!EnumHasAnyFlags(GatherTextFlags, EPropertyLocalizationGathererTextFlags::SkipProperties))
			{
				...
				GatherTextInstance(*TextElementValueAddress, PathToElement, !!(GatherTextFlags & EPropertyLocalizationGathererTextFlags::ForceEditorOnlyProperties) || IsEditorOnlyProperty(TextProperty));
			}
		}
...
void FPropertyLocalizationDataGatherer::GatherTextInstance(const FText& Text, const FString& Description, const bool bIsEditorOnly)
{
	...
	if (Text.IsInitializedFromString())
	{
		// We early out as a text initialized from a string does not have a namespace or key and thus can't be gathered. into the array
		ResultFlags |= EPropertyLocalizationGathererResultFlags::HasTextInitializedFromString;
		return;
	}
...
#if WITH_METADATA
	{
		static const FName NAME_EditorOnlyLocalization = "EditorOnlyLocalization";
		if (Property->HasMetaData(NAME_EditorOnlyLocalization))
		{
			return true;
		}
	}

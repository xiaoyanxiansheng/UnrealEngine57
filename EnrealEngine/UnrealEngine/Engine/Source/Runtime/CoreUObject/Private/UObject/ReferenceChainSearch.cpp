		...
		struct FProcessor
		{
				...
				bool bAllowReferenceElimination)
			{
				if (Object)
				{
					...
					{
						This->HandleObjectReference(Context, ThreadIndex, Object, ReferencingObject, MemberId, Origin);
					}
				}
			}
	...
	struct FReferenceInfoSearch : public TReferenceSearchBase<FReferenceInfoSearch>
	{
			...
			int32 ThreadIndex, UObject* Object, UObject* ReferencingObject, UE::GC::FMemberId MemberId, UE::GC::EOrigin Origin)
		{
			...
			*RefInfo = FReferenceChainSearch::FObjectReferenceInfo(ObjectInfo);
			...
			{
				RefInfo->ReferencerName = Context.SchemaStack->ToString();
			...
			else
			{
				...
				{
					if (FGCObject::GGCObjectReferencer->GetReferencerName(Object, RefInfo->ReferencerName, true))
					{
					}
			...
			if (DirectReferences)
			{
				for (TPair<FVertex, FReferenceChainSearch::FObjectReferenceInfo>& TargetPair : SourcePair.Value)
				{
					...
					if (RefInfo)
					{
						...
						if (SourceVertex == GCObjReferencerVertex || RefInfo->ReferencerName.IsNone())
						{
						...
						TargetPair.Value = FReferenceChainSearch::FObjectReferenceInfo(TargetInfo, ReferenceType, RefInfo->ReferencerName.ToString());
					}
				}
			}
		}
		...
		for (int32 NodeIndex = RootIndex - 1; NodeIndex >= 0 && bPostCallbackContinue; --NodeIndex)
		{
			...
			if (ReferenceInfo && ReferenceInfo->Type == EReferenceType::Property)
			{
				...
				else
				{
					...
					static const FStringView ClassPropertyName = TEXTVIEW("Class");
					static const FStringView OuterPropertyName = TEXTVIEW("Outer");
...
static TAutoConsoleVariable CVarMaxReferenceChainsToPrint(
	TEXT("gc.MaxReferenceChainsToPrint"),
	100,
	TEXT("Sets the limit of how many reference chains ReferenceChainSearch utility will print"),
	ECVF_Default
);
...
int32 FReferenceChainSearch::PrintResults(TFunctionRef<bool(FCallbackParams& Params)> ReferenceCallback, bool bDumpAllChains /*= false*/, UObject* TargetObject /*= nullptr*/, ELogVerbosity::Type InVerbosityForPrint /*= ELogVerbosity::Log*/) const
{
	...
	const int32 MaxChainsToPrint = CVarMaxReferenceChainsToPrint.GetValueOnAnyThread();
	...
	if (NumPrintedChains == 0)
	{
		auto LogUnreachableObject = [InVerbosityForPrint, this](const FGCObjectInfo& ObjInfo) {
			...
			else if (ObjInfo.GetRefCount() != 0)
			{
#if !NO_LOGGING
				GWarn->CategorizedLogf(LogReferenceChain.GetCategoryName(), InVerbosityForPrint,
					TEXT("%s%s is not currently reachable but it does have %d refcount set."), *GetObjectFlags(ObjInfo), *ObjInfo.GetFullName(), ObjInfo.GetRefCount());
#endif
			}
			else
			{
...
{
	return ObjectInfo && (ObjectInfo->HasAnyFlags(GARBAGE_COLLECTION_KEEPFLAGS) || ObjectInfo->HasAnyInternalFlags(EInternalObjectFlags_RootFlags) || ObjectInfo->GetRefCount() != 0);
}
...
TArray<FString> FReferenceChainSearch::FindAndPrintStaleReferencesToObjects(TConstArrayView<UObject*> ObjectsToFindReferencesTo, EPrintStaleReferencesOptions Options)
{
	...
	{
		// If you change the format of this string, update the corresponding AutomationUtils/Matcher/WorldLeakEventMatcher.cs
		UE_LOG(LogLoad, Fatal, 
			TEXT("====Fatal World Leaks====") LINE_TERMINATOR
			TEXT("Logging first error, check logs for additional information") LINE_TERMINATOR 
			TEXT("%s")
			TEXT("====End Fatal World Leaks====") LINE_TERMINATOR,

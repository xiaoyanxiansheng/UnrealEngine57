...
EConvertFromTypeResult FSetProperty::ConvertFromType(const FPropertyTag& Tag, FStructuredArchive::FSlot Slot, uint8* Data, const UStruct* DefaultsStruct, const uint8* Defaults)
{
...
EPropertyVisitorControlFlow FSetProperty::Visit(FPropertyVisitorContext& Context, const TFunctionRef<EPropertyVisitorControlFlow(const FPropertyVisitorContext& /*Conmtext*/)> InFunc) const
{
	...
	Context.Path.Top().bContainsInnerProperties = SetHelper.Num() > 0;
	...
	if (RetVal == EPropertyVisitorControlFlow::StepInto)
	{
		...
		{
			// visit element
			FPropertyVisitorScope Scope(Context.Path, FPropertyVisitorInfo(ElementProp, It.GetLogicalIndex(), EPropertyVisitorInfoType::ContainerIndex));

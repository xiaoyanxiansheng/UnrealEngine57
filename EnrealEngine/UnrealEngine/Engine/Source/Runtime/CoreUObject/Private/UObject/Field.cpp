...
FRegisterFieldClass::FRegisterFieldClass(FFieldClass* InFieldClass, const TCHAR* InCppName)
{
	InFieldClass->Name = &InCppName[1];
	FFieldClass::GetAllFieldClasses().Add(InFieldClass);
	FFieldClass::GetNameToFieldClassMap().Add(InFieldClass->Name, InFieldClass);
}
#endif // UE_WITH_CONSTINIT_UOBJECT

FFieldClass::FFieldClass(const TCHAR* InCPPName, uint64 InId, EClassCastFlags InCastFlags, FFieldClass* InSuperClass, FField* (*ConstructFnPtr)(const FFieldVariant&, const FName&, EObjectFlags))
...
FFieldVariant FFieldVariant::GetOwnerVariant() const
{
	...
	{
		return ToFieldUnsafe()->GetOwnerVariant();
	}
}
...
{
	return !IsUObject() && ToFieldUnsafe() && ToFieldUnsafe()->IsA(InClass);
}
...
UClass* FFieldVariant::GetOwnerClass() const
{
	...
	{
		return ToFieldUnsafe()->GetOwnerClass();
	}
}
...
FString FFieldVariant::GetFullName() const
{
	...
	{
		return ToFieldUnsafe()->GetFullName();
	}
}
...
FString FFieldVariant::GetPathName() const
{
	...
	{
		return ToFieldUnsafe()->GetPathName();
	}
}
...
FString FFieldVariant::GetClassName() const
{
	...
	{
		return ToFieldUnsafe()->GetClass()->GetName();
	}
}
...
UPackage* FFieldVariant::GetOutermost() const
{
	...
	{
		return ToFieldUnsafe()->GetOutermost();
	}
}
...
bool FFieldVariant::HasMetaData(const FName& Key) const
{
	...
	{
		return ToFieldUnsafe()->HasMetaData(Key);
	}
}
#endif // WITH_METADATA
...
#if UE_WITH_CONSTINIT_UOBJECT
constinit TNoDestroy<FFieldClass> FField::ConstInitClass(
    NoDestroyConstEval,
    ConstEval,
    TEXT("FField"),
    FField::StaticClassCastFlagsPrivate(),
    FField::StaticClassCastFlags(),
    nullptr,
    (FFieldClass::FConstructFunction*)&FField::Construct // Explicit cast required to select overload for template argument
);
#endif // UE_WITH_CONSTINIT_UOBJECT
...
FString GetPathNameSafe(const FField* InField, const UObject* StopOuter /*= nullptr*/)
{
	if (InField)
	{
		return InField->GetPathName(StopOuter);
	}
	else
	{
		return TEXT("none");
...
TEST_CASE_NAMED(FFieldVariantTest, "System::CoreUObject::FFieldVariant", "[CoreUObject]")
{
	FFieldVariant NullObjectVariant((UObject*)nullptr);
	CHECK_MESSAGE("Null object coerces to false", !NullObjectVariant);
	FFieldVariant NullFieldVariant((FField*)nullptr);
	CHECK_MESSAGE("Null field coerces to false", !NullFieldVariant);

	// Fails, impact of changing behavior is unknown at this time
	// CHECK_EQUALS("Nulls equal to each other", NullObjectVariant, NullFieldVariant);
}
#endif

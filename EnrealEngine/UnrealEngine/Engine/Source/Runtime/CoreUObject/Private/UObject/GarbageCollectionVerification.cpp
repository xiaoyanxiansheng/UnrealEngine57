...
void VerifyNoUnreachableObjects(int32 NumUnreachable)
{
	...
	ParallelFor(TEXT("GC.VerifyNoUnreachableObjects"), NumThreads, 1, [&NumErrors, &VerifiedNumUnreachable, NumberOfObjectsPerThread, NumThreads, MaxNumberOfReachableObjects, NumUnreachable](int32 ThreadIndex)
	{
		...
		{
			const int32 ObjectIndexInGlobalArray = FirstObjectIndex + ObjectIndex;
			FUObjectItem& ObjectItem = GUObjectArray.GetObjectItemArrayUnsafe()[ObjectIndexInGlobalArray];
			if (ObjectItem.GetObject())
			{
				...
				if (!ObjectItem.HasAnyFlags(EInternalObjectFlags::Unreachable))
				{
					...
					{
						UE_LOG(LogGarbage, Warning, TEXT("Object [%d] %s%s is still marked as MaybeUnreachable after Reachability Analysis is complete."),
							ObjectIndexInGlobalArray,
							*FReferenceChainSearch::GetObjectFlags(FGCObjectInfo(Object)),
							*Object->GetFullName());
						NumErrors++;
					}
					if (!FGCFlags::IsReachable_ForGC(&ObjectItem) && !GUObjectArray.IsDisregardForGC(Object))
					{
						UE_LOG(LogGarbage, Warning, TEXT("Object [%d] %s%s is NOT marked as Unreachable and NOT marked as Reachable."),
							ObjectIndexInGlobalArray,
							*FReferenceChainSearch::GetObjectFlags(FGCObjectInfo(Object)),
							*Object->GetFullName());
						NumErrors++;
				...
				else
				{
					...
					{
						UE_LOG(LogGarbage, Warning, TEXT("Object [%d] %s%s is still marked as Reachable."),
							ObjectIndexInGlobalArray,
							*FReferenceChainSearch::GetObjectFlags(FGCObjectInfo(Object)),
							*Object->GetFullName());
						NumErrors++;

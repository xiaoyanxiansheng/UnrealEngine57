...
static constexpr uint32 InnerSchemaTypes =			ToBit(EMemberType::StructArray) | ToBit(EMemberType::StructSet) | ToBit(EMemberType::FreezableStructArray) | ToBit(EMemberType::Optional);
...
FName ToName(EMemberType Type)
{
	struct FMemberTypeNames
	{
		FMemberTypeNames()
		{
			...
			Names[(uint8)EMemberType::StructSet] =						"StructSet";
...
{
	FNameBuilder Buffer(InPropertyPath);
	return ConvertPathToProperties(ObjectClass, Buffer.ToView(), OutProperties);
}

bool FPropertyStack::ConvertPathToProperties(UClass* ObjectClass, FStringView PropertyNameOrPath, TArray<FProperty*>& OutProperties)
{
	constexpr FAsciiSet Delimiters(".");
	bool bFullPathConstructed = true;

	FStringView PropertyName = FAsciiSet::FindPrefixWithout(PropertyNameOrPath, Delimiters);
	if (PropertyName.Len() == PropertyNameOrPath.Len())
	{
		// 99% of the time we're be dealing with just a single property
		if (FProperty* FoundProperty = ObjectClass->FindPropertyByName(FName(PropertyNameOrPath)))
		{
	...
	else
	{
		...
		if (FProperty* FoundProperty = ObjectClass->FindPropertyByName(FName(PropertyName)))
		{
			FStringView Remaining = FAsciiSet::TrimPrefixWith(PropertyNameOrPath.RightChop(PropertyName.Len()), Delimiters);
			OutProperties.Add(FoundProperty);
			do
			{
				// Determine the next property name
				PropertyName = FAsciiSet::FindPrefixWithout(Remaining, Delimiters);
				Remaining = FAsciiSet::TrimPrefixWith(Remaining.RightChop(PropertyName.Len()), Delimiters);
				if (FStructProperty* StructProp = CastField<FStructProperty>(FoundProperty))
				{
					...
					FoundProperty = StructProp->Struct->FindPropertyByName(FName(PropertyName));
				}
				else
				{
					// In all other case (though in reality it should only be a TMap) find the inner property
					FoundProperty = CastField<FProperty>(FoundProperty->GetInnerFieldByName(FName(PropertyName)));
				}
			...
			} while (Remaining.Len() > 0 && bFullPathConstructed);
		}
		else
		{
			bFullPathConstructed = false;
...
struct FStructMemberStats
{
	...
	void VisitInstances(const uint8* Instance, Fn Visitor) const
	{
		...
		{ 
			...
			break;
			case EMemberType::StructSet:			VisitSet(*(const FScriptSet*)Container, Visitor);
			break;
...
private:
	...
	template<typename Fn>
	void VisitSet(const FScriptSet& Set, Fn Visitor) const
	{
		const uint8* Data = Private::GetSetData(Set);
		const uint32 Stride = Schema.GetStructStride();
		for (int32 Idx = 0, Num = Set.GetMaxIndex(); Idx < Num; ++Idx)
		{

...
namespace
{
	...
	static int32 GUseNewPropertySaving = 0;
	static FAutoConsoleVariableRef CVarUseNewPropertySaving(
		TEXT("ini.UseNewPropertySaving"),
		GUseNewPropertySaving,
		TEXT("Determines if UObject property saving should use the Config APIs that can set a property (including arrays) directly in a static layer."));
...
bool UObject::Rename(const TCHAR* InName, UObject* NewOuter, ERenameFlags Flags)
{
	...
	if (NewOuter && NewOuter != GetOuter())
	{
		...
		if (((Flags & REN_AllowPackageLinkerMismatch) == 0) && Linker != NewOuterLinker)
		{
			...
			{
				if (Linker && HasAnyFlags(RF_NeedLoad | RF_NeedPostLoad))
				{
					const bool bIsPreloading = HasAnyFlags(RF_NeedLoad);
					const TCHAR* PreloadingGuidance =
						TEXT("Ensure the target package has completed loading before renaming to correct this error. ") 
						TEXT("Renaming before an object has been serialized will leave the object uninitialized.");
					const TCHAR* PostloadingGuidance =
						TEXT("If this object is no longer valid / is being discarded, call FLinkerLoad::InvalidateExport(ObjectToRename) ")
						TEXT("before calling Rename, otherwise ensure this object has been postloaded by waiting for the target package's load to complete, or explicitly calling ConditionalPostload(ObjectToRename).");

					UE_LOG(LogObj, Error,
						TEXT("Rename is moving object '%s' to a new package (%s) or a new linker while the object is still being loaded (it hasn't been %s yet). ")
						TEXT("%s ")
						TEXT("Renaming a partially loaded object without first calling FLinkerLoad::InvalidateExport(ObjectToRename) ")
						TEXT("will result in a partially loaded package likely leading to crashes and/or corrupt data."),
						*GetFullName(),
						*NewOuter->GetPackage()->GetFullName(),
						(HasAnyFlags(RF_NeedLoad) ? TEXT("serialized") : TEXT("postloaded")),
						(bIsPreloading ? PreloadingGuidance : PostloadingGuidance)
					);
					FDebug::DumpStackTraceToLog(ELogVerbosity::Display);
				}

				SetLinker(nullptr, INDEX_NONE, true /*bShouldDetachExisting*/, true /*bSilenceLogs*/);
			}
		}
	}
	...
	if (InName)
	{
		...
		{
			ExistingObject = StaticFindObject(/*Class=*/ nullptr, NewOuter ? NewOuter : GetOuter(), InName, EFindObjectFlags::ExactClass);
		}
		else
		{
...

	{
		...
		if (HasAnyFlags(RF_Public))
		{
			...
			const bool bRedirectionAllowed = ((Flags & REN_DontCreateRedirectors) == 0) && GIsEditor && !IsRunningGame();
			auto IsGameObject =
				[this]() -> bool
				{
					return GetPackage()->HasAnyPackageFlags(PKG_PlayInEditor);
				};
			...
			bCreateRedirector = bRootPackage == false && bUniquePathChanged == true && bRedirectionAllowed == true && bIsCaseOnlyChange == false && IsGameObject() == false;
		}
	...
	if (bCreateRedirector)
	{
		...
		UObjectRedirector* Redirector = FindObject<UObjectRedirector>(OldOuter, *OldName.ToString(), EFindObjectFlags::ExactClass);
...
void UObject::PostEditChangeChainProperty(FPropertyChangedChainEvent& PropertyChangedEvent)
{
	...
	FCoreUObjectDelegates::OnPostObjectPropertyChanged.Broadcast(this, PropertyChangedEvent);
}
...
{
	// The check for EInternalObjectFlags::AsyncLoadingPhase1 is important here to make sure we go
	// through the Preload function to get rid of any potential race condition that might happen
	// with the parallel loader threads when going wide.
	if (HasAnyFlags(RF_NeedLoad) || HasAnyInternalFlags(EInternalObjectFlags::AsyncLoadingPhase1))
	{
...
void UObject::BuildSubobjectMapping(UObject* OtherObject, TMap<UObject*, UObject*>& ObjectMapping) const
{
	...
	// ForEachObjectWithOuter is taking a hash lock, and we can't load new objects inside a hash lock
	// so gather objects first, and process them after the hash lock is released.
	TArray<UObject*> ObjectsToProcess;
	ForEachObjectWithOuter(this, [&](UObject* InSubObject)
	{
		ObjectsToProcess.Add(InSubObject);
	}, false, RF_NoFlags, EInternalObjectFlags::Garbage);

	for (UObject* SubObject : ObjectsToProcess)
	{
		if (ObjectMapping.Contains(SubObject))
		{
			return;
		}

		FString NewSubObjectName = SubObject->GetName();

		UClass* OtherSubObjectClass = SubObject->GetClass();
#if WITH_EDITORONLY_DATA
		...
		ObjectMapping.Emplace(SubObject, OtherSubObject);

		if (OtherSubObject)
		{
			SubObject->BuildSubobjectMapping(OtherSubObject, ObjectMapping);
		}
	};
}
...
void UObject::ThreadedPostLoadAssetRegistryTags(FPostLoadAssetRegistryTagsContext& Context) const
{
	...
	// Before executing any postloads, check whether we will be able to complete.
	bool bHasAssetRegistrySearchableProperties = GetClass()->HasAssetRegistrySearchableProperties();
	UScriptStruct* SparseClassDataStruct = nullptr;
	const void* SparseClassData = nullptr;
	if (bHasAssetRegistrySearchableProperties)
	{
		SparseClassDataStruct = GetClass()->GetSparseClassDataStruct();
		if (SparseClassDataStruct)
		{
			SparseClassData = GetClass()->GetSparseClassData(
				Context.CanCreateObjects() ? EGetSparseClassDataMethod::ArchetypeIfNull
				: EGetSparseClassDataMethod::DeferIfNull);
			if (!SparseClassData)
			{
				Context.SetPostLoadCouldComplete(false);
				return;
			}
		}
	}
	...
	TArray<UE::ConfigAccessTracking::FConfigAccessData>* OutAccessedValues/*=nullptr*/)
{
	...
	checkf(ConfigClass->PropertyLink != nullptr
		...
		|| (UE_WITH_CONSTINIT_UOBJECT ? !ConfigClass->HasAnyClassFlags(CLASS_Constructed) : ConfigClass->PropertiesSize == 0)
		...
		TEXT("class %s has uninitialized properties. Accessed too early?"), *ConfigClass->GetPathName());
#endif
...
void UObject::SaveConfig(uint64 RequiredPropertyFlags, const TCHAR* InFilename, FConfigCacheIni* Config/*=GConfig*/, bool bAllowCopyToDefaultObject/*=true*/)
{
	FSaveConfigContext Context;
	Context.RequiredPropertyFlags = (EPropertyFlags)RequiredPropertyFlags;
	Context.Filename = InFilename;
	Context.ConfigSystem = Config;
	Context.bAllowCopyToDefaultObject = bAllowCopyToDefaultObject;
...
void UObject::SaveConfig(const FSaveConfigContext& Context)
{
	...
	const FString Filename = Context.Filename.Len() > 0 ? Context.Filename : GetConfigFilename(this);
	
	FConfigCacheIni* Config = Context.ConfigSystem;
	...
	// Determine whether the file we are writing to is a layer in the hierarchy, as opposed to the final Saved file
	const bool bIsLayerWrite = Filename == GetDefaultConfigFilename() || Filename == GetGlobalUserConfigFilename() || Context.bSaveToLayer;

	const bool bPerObject = UsesPerObjectConfig(this);
	FString Section = Context.OverrideConfigSectionName;
	...
	if (bPerObject == true)
	{
		...
		if (Context.OverrideConfigSectionName.IsEmpty())
		{
	...
	const bool bCopyValues = (Context.bAllowCopyToDefaultObject && this != CDO && Context.ConfigSystem == GConfig);

	for (FProperty* Property = GetClass()->PropertyLink; Property; Property = Property->PropertyLinkNext)
	{
		// we just check names, in case this property is a duplicated but same property as this object's class
		if (!Context.PropertyNames.IsEmpty() && !Context.PropertyNames.Contains(Property->GetFName()))
		{
			continue;
		}
		...
		if ((Property->PropertyFlags & Context.RequiredPropertyFlags) == Context.RequiredPropertyFlags)
		{
			...
			if (!bPerObject && Context.OverrideConfigSectionName.IsEmpty())
			{
			...
			// globalconfig properties should always use the owning class's config file
			// specifying a value for InFilename will override this behavior (as it does with normal properties)
			const FString& PropFileName = ((Property->PropertyFlags & CPF_GlobalConfig) && Context.Filename.Len() == 0) ? Property->GetOwnerClass()->GetConfigName() : Filename;
			...
			if (Array || SetProperty)
			{
				// write a property to a specific layer (with new path, for now the old way of writing to defaults is still below)
				if (Context.bSaveToLayer)
				{
					TArray<FString> ExportedValues;
					if (Array)
					{
						FScriptArrayHelper_InContainer ArrayHelper(Array, this);
						ExportedValues.AddDefaulted(ArrayHelper.Num());
						for (int32 i = 0; i < ArrayHelper.Num(); i++)
						{
							Array->Inner->ExportTextItem_Direct(ExportedValues[i], ArrayHelper.GetRawPtr(i), ArrayHelper.GetRawPtr(i), this, PortFlags);
						}
						Config->SetArrayInSectionOfStaticLayer(*Section, *Key, ExportedValues, false, Property->GetOwnerClass()->GetConfigName(), Filename, true);
						bProcessedProperty = true;
					}
					else if (SetProperty)
					{
						...
						if (SetHelper.Num() > 1)
						{
							ExportedValues.AddDefaulted(SetHelper.Num());
							int Index = 0;
							for (FScriptSetHelper::FIterator It = SetHelper.CreateIterator(); It; ++It, Index++)
							{
								SetProperty->ElementProp->ExportTextItem_Direct(ExportedValues[Index], SetHelper.GetElementPtr(It), SetHelper.GetElementPtr(It), this, PortFlags);
							}
							Config->SetArrayInSectionOfStaticLayer(*Section, *Key, ExportedValues, false, Property->GetOwnerClass()->GetConfigName(), Filename, true);
							bProcessedProperty = true;
						}
				...
				else
				{
					...
					// Layer ini's require the array syntax to be applied to the property name
					FString CompleteKey = FString::Printf(TEXT("%s%s"), bIsLayerWrite ? TEXT("+") : TEXT(""), *Key);
					if (Sec)
					{
					...
					if (!bPropDeprecated && (!bShouldCheckIfIdenticalBeforeAdding || !Property->Identical_InContainer(this, SuperClassDefaultObject)))
					{
						if (Array)
						{
							...
							if (ArrayHelper.Num() == 0 && bIsLayerWrite)
							{
			...
			if (!bProcessedProperty)
			{
				...
				for (int32 Index=0; Index<Property->ArrayDim; Index++)
				{
					...
					if (!bPropDeprecated && (!bShouldCheckIfIdenticalBeforeAdding || !Property->Identical_InContainer(this, SuperClassDefaultObject, Index)))
					{
						...
						if (Context.bSaveToLayer)
						{
							Config->SetInSectionOfStaticLayer(*Section, *Key, *Value, Property->GetOwnerClass()->GetConfigName(), Filename, true);
						}
						else
						{
					...
					else
					{
						if (Context.bSaveToLayer)
						{
							Config->ResetKeyInSectionOfStaticLayer(*Section, *Key, Property->GetOwnerClass()->GetConfigName(), Filename, true);
						}
						else
						{
	...
	if (Context.ConfigSystem == GConfig || bIsTemporaryBranch)
	{
...
{
	if (GUseNewPropertySaving > 0)
	{
		// run SaveConfig, which has some logic to for diffing against parent class, and now has logic to also
		// directly update a single config property, including arrays, via FConfigCacheIni functions
		FSaveConfigContext Context;
		Context.Filename = InConfigIniName;
		Context.bSaveToLayer = true;
		Context.PropertyNames = { InProperty->GetFName() };
		SaveConfig(Context);
		return;
	}
...
void StaticUObjectInit()
{
	...
	// Force GC verification on or off based on command line parameters.
	bool bCommandLineForcesVerifyGC = FParse::Param(FCommandLine::Get(), TEXT("VERIFYGC"));
	bool bCommandLineForcesNoVerifyGC = FParse::Param(FCommandLine::Get(), TEXT("NOVERIFYGC"));
	if (IConsoleVariable* CVarVerifyGCAssumptions = IConsoleManager::Get().FindConsoleVariable(TEXT("gc.VerifyAssumptions")))
	{
		if (bCommandLineForcesVerifyGC)
		{
			CVarVerifyGCAssumptions->Set(true, ECVF_SetByCommandline);
		}
		else if (bCommandLineForcesNoVerifyGC)
		{
			CVarVerifyGCAssumptions->Set(false, ECVF_SetByCommandline);
		}
	}
	if (IConsoleVariable* CVarVerifyGCAssumptionsOnFullPurge = IConsoleManager::Get().FindConsoleVariable(TEXT("gc.VerifyAssumptionsOnFullPurge")))
	{
		if (bCommandLineForcesVerifyGC)
		{
			CVarVerifyGCAssumptionsOnFullPurge->Set(true, ECVF_SetByCommandline);
		}
		else if (bCommandLineForcesNoVerifyGC)
		{
			CVarVerifyGCAssumptionsOnFullPurge->Set(false, ECVF_SetByCommandline);
		}
	}
	if (IConsoleVariable* CVarVerifyGCChance = IConsoleManager::Get().FindConsoleVariable(TEXT("gc.VerifyAssumptionsChance")))
	{
		if (bCommandLineForcesNoVerifyGC)
		{
			CVarVerifyGCChance->Set(0.0f, ECVF_SetByCommandline);
		}
	}
...
void StaticExit()
{
	...
	UE::RemoteObject::Private::ShutdownRemoteObjects();
...
{
	//TODO: This should test for RF_ArchetypeObject too
...
bool UObject::IsFullNameStableForNetworking() const
{
	...
	// Consider templates to always be stable
	return IsTemplate() || IsNameStableForNetworking();
}

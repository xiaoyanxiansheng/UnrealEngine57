...
void FOptionalProperty::InstanceSubobjects(void* Data, void const* DefaultData, TNotNull<UObject*> InOwner, struct FObjectInstancingGraph* InstanceGraph)
{
	if (Data
		&& IsSet(Data)
		&& (ValueProperty->ContainsInstancedObjectProperty() || InOwner->GetClass()->ShouldUseDynamicSubobjectInstancing()))
	{
		ValueProperty->InstanceSubobjects(Data, DefaultData ? GetValuePointerForReadIfSet(DefaultData) : nullptr, InOwner, InstanceGraph);
	}
}
...
EConvertFromTypeResult FOptionalProperty::ConvertFromType(const FPropertyTag& Tag, FStructuredArchive::FSlot Slot, uint8* ContainerData, const UStruct* DefaultsStruct, const uint8* DefaultsContainer)
{
...
EPropertyVisitorControlFlow FOptionalProperty::Visit(FPropertyVisitorContext& Context, const TFunctionRef<EPropertyVisitorControlFlow(const FPropertyVisitorContext& /*Context*/)> InFunc) const
{
	...
	Context.Path.Top().bContainsInnerProperties = IsSet(Context.Data.PropertyData);

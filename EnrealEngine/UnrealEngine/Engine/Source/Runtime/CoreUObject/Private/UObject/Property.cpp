...
bool FProperty::ShouldSerializeValue(FArchive& Ar) const
{
	...
	{
#if WITH_METADATA
		// we can't tell if we are loading an optional object at this point, so we allow any AllowedInOptional properties while loading
		// and then the later code can 
		bool bLoadingWithEditorData = FPlatformProperties::HasEditorOnlyData() && Ar.IsLoading();
		bool bAllowed = (bLoadingWithEditorData || Ar.IsSavingOptionalObject()) && HasMetaData("AllowedInOptional");
		if (bAllowed)
		{
			UE_LOG(LogInit, Display, TEXT("allowing serialize of EO property, even when filtering EO data in an Optional object - %s"), *GetFullName());
		}
#else
		bool bAllowed = false;
#endif
...
EConvertFromTypeResult FProperty::ConvertFromType(const FPropertyTag& Tag, FStructuredArchive::FSlot Slot, uint8* Data, const UStruct* DefaultsStruct, const uint8* Defaults)
{
	...
	if (*Str && !PropertyName.IsNone())
	{
		...
		if (Operation.IsSet())
		{
				...
				Operation.GetValue() != EOverriddenPropertyOperation::Remove)
			{
				...
				OverriddenProperties->ConditionallyRestoreOverriddenPropertyOperation(Operation.GetValue(), &Chain, /*Property*/nullptr);
			}
		}
		...
		else
		{
			...
			// Avoid shadowed reference values (not directly imported) - callers should advance
			// @todo - IDOs will need to capture the referenced instance to help avoid data loss
			if (Operation.IsSet() && Operation.GetValue() == EOverriddenPropertyOperation::SubObjectsShadowing)
			{
				return Str;
			}
						...
						case EOverriddenPropertyOperation::Remove:
						{
							...
							if (UObject* RemovedSubObject = InnerObjectProperty->GetObjectPropertyValue(TempValueStorage))
							{
								...
								if (Path)
								{
									...
									FArchiveSerializedPropertyChain Chain = Path->ToSerializedPropertyChain();
									if (FOverriddenPropertyNode* ArrayOverriddenPropertyNode = OverriddenProperties ? OverriddenProperties->RestoreOverriddenPropertyOperation(EOverriddenPropertyOperation::Modified, &Chain, /*Property*/nullptr) : nullptr)
									{
										// Rebuild the overridden info
										OverriddenProperties->RestoreSubObjectOperation(EOverriddenPropertyOperation::Remove, *ArrayOverriddenPropertyNode, RemovedSubObject);
									}
								}
							}
							...
							if (const FObjectPropertyBase* InnerObjectProperty =  ArrayProperty->Inner->HasAnyPropertyFlags(CPF_PersistentInstance) ? CastField<FObjectPropertyBase>(ArrayProperty->Inner) : nullptr)
							{
								...
								if (UObject* AddedSubObject = InnerObjectProperty->GetObjectPropertyValue(TempValueStorage))
								{
									...
									if (Path)
									{
										...
										{
											// Rebuild the overridden info
											OverriddenProperties->RestoreSubObjectOperation(EOverriddenPropertyOperation::Add, *ArrayOverriddenPropertyNode, AddedSubObject);
										}
									}
								}
					...
					switch(Operation.GetValue())
					{
						case EOverriddenPropertyOperation::Remove:
						{
							...
							if (Path)
							{
								...
								FArchiveSerializedPropertyChain Chain = Path->ToSerializedPropertyChain();
								if (FOverriddenPropertyNode* MapOverriddenPropertyNode = OverriddenProperties ? OverriddenProperties->RestoreOverriddenPropertyOperation(EOverriddenPropertyOperation::Modified, &Chain, /*Property*/nullptr) : nullptr)
								{
									// Rebuild the overridden info

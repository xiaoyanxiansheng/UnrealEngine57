...
FGCCSyncObject::FGCCSyncObject()
{
	...
	UE::Private::SetIsGCLockedFunction(IsGarbageCollectionLocked);
}
FGCCSyncObject::~FGCCSyncObject()
{
	FPlatformProcess::ReturnSynchEventToPool(GCUnlockedEvent);
	GCUnlockedEvent = nullptr;
	UE::Private::SetIsGCLockedFunction(nullptr);
}
...
static int32 GIncrementalBeginDestroyGranularity = 10;
static FAutoConsoleVariableRef CIncrementalBeginDestroyGranularity(
	TEXT("gc.IncrementalBeginDestroyGranularity"),
	GIncrementalBeginDestroyGranularity,
	TEXT("When using gc.IncrementalBeginDestroyEnabled, check the time limit after this number of destroys. A low number has more overhead but is more accurate."),
	ECVF_Default
);
...
namespace UE::GC::Private
{
	...
	static TConsumeAllMpmcQueue<int32> GDirtyRoots;
	static TSet<int32> GRoots;
	static UE::FMutex  GRootsMutex;
	...
	{
		// If this somehow race with a SetRootFlags or ClearRootFlags its not the end of the world
		// since another MarkRootAsDirty will be queued for later processing.
		...
		{
			// Disregard for GC objects should never be added to the roots list because they are meant to be entirely skipped by GC.
	...
	{
		// The order is not important since the calls are stateless
		GDirtyRoots.ConsumeAllLifo(
			[](int32 Index)
			{
				ProcessDirtyRootNoLock(Index);
			}
		);
	}
	...
	{
		if (GRootsMutex.TryLock())
		{
			// Process the whole list first to cooperatively help other threads when
			// we get the lock.
			ProcessDirtyRootsNoLock();

			// No need to queue anything since we have the lock, process it right away.
			ProcessDirtyRootNoLock(Index);
			GRootsMutex.Unlock();
		}
		else
		{
			// This will be eventually consistent as each thread able to take the lock
			// will update the value before using the GRoots content.
			GDirtyRoots.ProduceItem(Index);
		}
	}
}
...
void FUObjectItem::MarkRootAsDirty()
{
	UE::GC::Private::MarkRootAsDirty(GUObjectArray.ObjectToIndex(GetObject()));
}
...
bool FUObjectItem::SetRootFlags(EInternalObjectFlags FlagsToSet)
{
	...
	bool bIChangedIt = false;
	{
		int32 OldFlags;
		int32 OldRefCount;
		bIChangedIt = AtomicallySetFlag_ForGC(FlagsToSet, OldFlags, OldRefCount);
		if (bIChangedIt)
		{
			// Only dirty the root if there was no root flags before and no refcount
			if ((OldFlags & RootFlags) == 0 && OldRefCount == 0)
			{
				// RootFlags can only be added while GC is not running, otherwise GC might not see the change
				checkSlow(IsInGameThread() || IsInParallelGameThread() || IsGarbageCollectionLocked());
...
bool FUObjectItem::ClearRootFlags(EInternalObjectFlags FlagsToClear)
{
	...
	int32 OldFlags;
	int32 OldRefCount;
	bool bIChangedIt = AtomicallyClearFlag_ForGC(FlagsToClear, OldFlags, OldRefCount);
	if (bIChangedIt)
	{
		if ((OldFlags & RootFlags) != 0 && ((OldFlags & ~(int32)FlagsToClear) & RootFlags) == 0 && OldRefCount == 0)
		{
			MarkRootAsDirty();
		}
	}
...
void OnDisregardForGCSetDisabled(int32 NumObjects)
{
	...
	UE::TUniqueLock RootsLock(GRootsMutex);
	ProcessDirtyRootsNoLock();
...
private:
	FORCEINLINE_DEBUGGABLE void DrainSparseStructArrays(uint32 Num)
	{
		...
		for (FSparseStructArray Entry : MakeArrayView(UnvalidatedSparseStructArrays.Entries, Num))
		{
			if (Entry.Num)
			{
				FScriptSet& Array = *Entry.Array;
				FStructArray Slice = { Entry.Schema, Private::GetSetData(Array), Entry.Num, Entry.Stride };
...
struct TVerseDebugReachabilityVisitor : public Verse::FAbstractVisitor
{
	...
	virtual void VisitAuxNonNull(void* InAux, const TCHAR* ElementName) override
	{
		MarkStack.MarkAuxNonNull(InAux);
	}

	virtual bool IsMarked(Verse::VCell* InCell, const TCHAR*) override
	{
		return Verse::FHeap::IsMarked(InCell);
	}

	virtual void ReportNativeBytes(size_t Bytes) override
	{
		MarkStack.ReportNativeBytes(Bytes);
	}

	virtual void MarkNativeStructAsReachable(Verse::VNativeStruct* NativeStruct) override
	{
...
static bool bInFrankenGCStartStop = false; // This just tracks if we are in a StartVerseGC/StopVerseGC call pair
...
private:
	...
	void PerformReachabilityAnalysisPass(const EGCOptions Options)
	{
		...
		{
			// Add objects marked with the GC barrier to the initial set of objects for the next iteration of incremental reachability
...
bool IncrementalDestroyGarbage(bool bUseTimeLimit, double TimeLimit)
{
	...
	// amounts of time.
	const int32	TimeLimitEnforcementGranularityForDestroy = GIncrementalBeginDestroyGranularity;
	...
	{
		UE_LOG(LogGarbage, Verbose, TEXT("%.3f ms for incrementally purging unreachable objects (FinishDestroyed: %d, Destroyed: %d / %d)"),
			ElapsedTime * 1000,
			GObjCurrentPurgeObjectIndex,
	...
	{		
		...
		UE_LOG(LogGarbage, Log, TEXT("GC purged %i objects (%i -> %i) in %.3fms (%d iteration(s))"), PurgedObjectCountSinceLastMarkPhase,
			GObjectCountDuringLastMarkPhase.GetValue(),
			GObjectCountDuringLastMarkPhase.GetValue() - PurgedObjectCountSinceLastMarkPhase,
			GGCStats.DestroyGarbageTime.Total * 1000,
			GGCStats.DestroyGarbageTime.NumIterations);
...
void PreCollectGarbageImpl(EObjectFlags KeepFlags)
{
	...
	TRACE_BEGIN_REGION(TEXT("GarbageCollection"), TEXT("GarbageCollection"));
...
bool TryCollectGarbage(EObjectFlags KeepFlags, bool bPerformFullPurge)
{
	...
	if (AutoRTFM::IsTransactional())
	{
		...
		UE_LOG(LogGarbage, Log, TEXT("TryCollectGarbage: skipping garbage collection because an AutoRTFM transaction is active."));
		return false;
	}
...
void PurgeAllUObjectsOnExit()
{
	...
	for (FRawObjectIterator It; It; ++It)
	{
		...
		// Skip compiled-in objects which are stored in the data segment.
		if (Obj && !ObjItem->HasAnyFlags(EInternalObjectFlags::Native))
#else
		if (Obj)
#endif
		{
...
#if UE_WITH_CONSTINIT_UOBJECT
			// Skip compiled-in objects which are stored in the data segment.
			if (It->HasAnyFlags(EInternalObjectFlags::Native))
			{
				continue;
			}
#endif
...
void UObjectBase::MarkAsReachable() const
{
	...
	::MarkAsReachable</*bIsVerse*/ false>(this);
}
...
{
	::MarkAsReachable</*bIsVerse*/ true>(this);
}
...
bool FStructProperty::ContainsObjectReference(TArray<const FStructProperty*>& EncounteredStructProps, EPropertyObjectReferenceType InReferenceType /*= EPropertyObjectReferenceType::Strong*/) const
{
	...
	bool bValue = false;
#if UE_WITH_CONSTINIT_UOBJECT
	// Version that can iterate properties before full linking
	for (TFieldIterator<FProperty> It(Struct); It; ++It)
	{
		if (It->ContainsObjectReference(EncounteredStructProps, InReferenceType))
		{
			bValue = true;
			break;
		}
...
void FMapProperty::EmitReferenceInfo(UE::GC::FSchemaBuilder& Schema, int32 BaseOffset, TArray<const FStructProperty*>& EncounteredStructProps, UE::GC::FPropertyStack& DebugPath)
{
	...
	if (ContainsObjectReference(EncounteredStructProps, EPropertyObjectReferenceType::Strong))
	{
		...
		Schema.Add(UE::GC::DeclareMember(DebugPath, BaseOffset + GetOffset_ForGC(), EMemberType::StructSet, PairSchema.Build()));
	}
}
...
void FSetProperty::EmitReferenceInfo(UE::GC::FSchemaBuilder& Schema, int32 BaseOffset, TArray<const FStructProperty*>& EncounteredStructProps, UE::GC::FPropertyStack& DebugPath)
{
	...
	if (ContainsObjectReference(EncounteredStructProps, EPropertyObjectReferenceType::Strong))
	{
		...
		Schema.Add(UE::GC::DeclareMember(DebugPath, BaseOffset + GetOffset_ForGC(), EMemberType::StructSet, ElementSchema.Build()));
	}
}

...
COREUOBJECT_API uint8 GRegisterNative(int32 NativeBytecodeIndex, const FNativeFuncPtr& Func)
{
	...
	{
		if( NativeBytecodeIndex<0 || NativeBytecodeIndex > GNatives.Num() || GNatives[NativeBytecodeIndex] != &UObject::execUndefined) 
		{
			CA_SUPPRESS(6385)
...
static constinit TStaticArray<FNativeFuncPtr, CST_Max> GCasts(InPlace, &UObject::execUndefined);
...
DEFINE_FUNCTION(UObject::execLetBool)
{
	...
	*/
	...
	// Evaluate the r-value for this expression into Value
	// Note that a bool property can represent bitfields,
	// which is why a uint8 is needed here.
	...
	if (BoolAddr)
	{
		...
		{
			check(PropertyContainer != nullptr);
			BoolProperty->SetValue_InContainer(PropertyContainer, &NewBoolValue);
		}
		else
		{
			BoolProperty->SetPropertyValue(BoolAddr, NewBoolValue);
		}
	}
}

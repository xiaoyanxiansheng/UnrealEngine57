...
TMulticastDelegate<void(UClass* OldClass, UClass* NewClass, EHotReloadedClassFlags Flags)> FCoreUObjectDelegates::RegisterClassForHotReloadReinstancingDelegate;
...
FCoreUObjectDelegates::FOnPostObjectPropertyChanged FCoreUObjectDelegates::OnPostObjectPropertyChanged;
...
namespace
{
	 ...
	 */
	UObject* StaticFindObjectWithChangedLegacyPath(UClass* ObjectClass, UObject* ObjectPackage, FName ObjectName, EFindObjectFlags Flags, EInternalObjectFlags ExclusiveInternalFlags)
	{
			...
			ObjectPackage->IsA<UClass>()) // Only if outer is a class.
		{
			...
			bool bSubclassOfPathChangedClass = !bExactPathChangedClass && !(Flags & EFindObjectFlags::ExactClass)
			...
			MatchingObject = StaticFindObject(ObjectClass, ObjectPackage->GetOutermost(), *ObjectName.ToString(), Flags);
...
//
UObject* StaticFindObjectFast(UClass* ObjectClass, UObject* ObjectPackage, FName ObjectName, EFindObjectFlags Flags, EObjectFlags ExclusiveFlags, EInternalObjectFlags ExclusiveInternalFlags)
{
	...
	UObject* FoundObject = StaticFindObjectFastInternal(ObjectClass, ObjectPackage, ObjectName, Flags, ExclusiveFlags, ExclusiveInternalFlags);

	if (!FoundObject)
	{
		FoundObject = StaticFindObjectWithChangedLegacyPath(ObjectClass, ObjectPackage, ObjectName, Flags, ExclusiveInternalFlags);
	}

	return FoundObject;
}

UObject* StaticFindObjectFastSafe(UClass* ObjectClass, UObject* ObjectPackage, FName ObjectName, EFindObjectFlags Flags, EObjectFlags ExclusiveFlags, EInternalObjectFlags ExclusiveInternalFlags)
{
	...
	if (!UE::IsSavingPackage(nullptr) && !IsGarbageCollectingAndLockingUObjectHashTables())
	{
		// We don't want to return any objects that are currently being background loaded unless we're using FindObject during async loading.
		ExclusiveInternalFlags |= UE::GetAsyncLoadingInternalFlagsExclusion();;
		FoundObject = StaticFindObjectFastInternal(ObjectClass, ObjectPackage, ObjectName, Flags, ExclusiveFlags, ExclusiveInternalFlags);
		if (!FoundObject)
		{
			FoundObject = StaticFindObjectWithChangedLegacyPath(ObjectClass, ObjectPackage, ObjectName, Flags, ExclusiveInternalFlags);
		}
...
#if WITH_EDITOR
static UObject* LoadObjectWhenImportingT3D(UClass* ObjectClass, FStringView OrigInName)
{
	UObject* MatchingObject = nullptr;
	static bool s_bCurrentlyLoading = false;
	if (s_bCurrentlyLoading == false)
	{
		if (OrigInName.Contains(TEXT("."), ESearchCase::CaseSensitive) &&
			!OrigInName.Contains(TEXT("'"), ESearchCase::CaseSensitive) &&
			!OrigInName.Contains(TEXT(":"), ESearchCase::CaseSensitive))
		{
...
//
UObject* StaticFindObject( UClass* ObjectClass, UObject* InObjectPackage, FStringView OrigInName, EFindObjectFlags Flags )
{
	...
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	return StaticFindObjectFast(ObjectClass, ObjectPackage, ObjectName, Flags);
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
}
...
// Find an optional object.
//
UObject* StaticFindObject(UClass* Class, FTopLevelAssetPath ObjectPath, EFindObjectFlags Flags)
{
	...
	if (!ObjectPath.IsNull())
	{
		...
		if (Package)
		{
			Result = StaticFindObjectFast(Class, Package, ObjectPath.GetAssetName(), Flags);
		}
	}
	return Result;
}

UObject* StaticFindObjectSafe(UClass* Class, FTopLevelAssetPath ObjectPath, EFindObjectFlags Flags)
{
	if (!UE::IsSavingPackage(nullptr) && !IsGarbageCollectingAndLockingUObjectHashTables())
	{
		FGCScopeGuard GCGuard;
		return StaticFindObject(Class, ObjectPath, Flags);
	}
	else
	{
		return nullptr;
...
//
UObject* StaticFindObjectChecked( UClass* ObjectClass, UObject* ObjectParent, FStringView InName, EFindObjectFlags Flags )
{
	UObject* Result = StaticFindObject( ObjectClass, ObjectParent, InName, Flags );
#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
	if( !Result )
	{
		UE_LOG(LogUObjectGlobals, Fatal, TEXT("Failed to find object '%s %s.%.*s'"), *ObjectClass->GetName(), ObjectParent ? *ObjectParent->GetName() : TEXT("None"), InName.Len(), InName.GetData());
	}
#endif
	return Result;
}
...
//
UObject* StaticFindObjectSafe( UClass* ObjectClass, UObject* ObjectParent, FStringView InName, EFindObjectFlags Flags )
{
	if (!UE::IsSavingPackage(nullptr) && !IsGarbageCollectingAndLockingUObjectHashTables())
	{
		FGCScopeGuard GCGuard;
		return StaticFindObject( ObjectClass, ObjectParent, InName, Flags );
	}
	else
	{
		return nullptr;
...
bool StaticFindAllObjectsFast(TArray<UObject*>& OutFoundObjects, UClass* ObjectClass, FName ObjectName, EFindObjectFlags Flags, EObjectFlags ExclusiveFlags, EInternalObjectFlags ExclusiveInternalFlags)
{
	...
	// We don't want to return any objects that are currently being background loaded unless we're using FindObject during async loading.
	ExclusiveInternalFlags |= UE::GetAsyncLoadingInternalFlagsExclusion();
	return StaticFindAllObjectsFastInternal(OutFoundObjects, ObjectClass, ObjectName, Flags, ExclusiveFlags, ExclusiveInternalFlags);
}
...
bool StaticFindAllObjectsFastSafe(TArray<UObject*>& OutFoundObjects, UClass* ObjectClass, FName ObjectName, EFindObjectFlags Flags, EObjectFlags ExclusiveFlags, EInternalObjectFlags ExclusiveInternalFlags)
{
	...
	if (!UE::IsSavingPackage(nullptr) && !IsGarbageCollectingAndLockingUObjectHashTables())
	{
		// We don't want to return any objects that are currently being background loaded unless we're using FindObject during async loading.
		ExclusiveInternalFlags |= UE::GetAsyncLoadingInternalFlagsExclusion();
		bFoundObjects = StaticFindAllObjectsFastInternal(OutFoundObjects, ObjectClass, ObjectName, Flags, ExclusiveFlags, ExclusiveInternalFlags);
	}
...
bool StaticFindAllObjects(TArray<UObject*>& OutFoundObjects, UClass* ObjectClass, const TCHAR* OrigInName, EFindObjectFlags Flags)
{
	...
	return StaticFindAllObjectsFast(OutFoundObjects, ObjectClass, ObjectName, Flags);
}
...
bool StaticFindAllObjectsSafe(TArray<UObject*>& OutFoundObjects, UClass* ObjectClass, const TCHAR* OrigInName, EFindObjectFlags Flags)
{
	...
	if (!UE::IsSavingPackage(nullptr) && !IsGarbageCollectingAndLockingUObjectHashTables())
	{
		bFoundObjects = StaticFindAllObjects(OutFoundObjects, ObjectClass, OrigInName, Flags);
	}
	return bFoundObjects;
}

UObject* StaticFindFirstObject(UClass* Class, FStringView Name, EFindFirstObjectOptions Options /*= EFindFirstObjectOptions::None*/, ELogVerbosity::Type AmbiguousMessageVerbosity /*= ELogVerbosity::Warning*/, const TCHAR* InCurrentOperation /*= nullptr*/)
{
	UObject* Result = nullptr;
	FName ObjectName;
	if (UE::String::FindFirstChar(Name, TCHAR('\'')) == INDEX_NONE)
	{
	...
	EFindObjectFlags FindObjectFlags = !!(Options & EFindFirstObjectOptions::ExactClass) ? EFindObjectFlags::ExactClass : EFindObjectFlags::None;
	...
	{
		Result = StaticFindFirstObjectFastInternal(Class, ObjectName, FindObjectFlags);
	}
	else
	{
		TArray<UObject*> FoundObjects;
		if (StaticFindAllObjectsFast(FoundObjects, Class, ObjectName, FindObjectFlags))
		{
...
UObject* StaticFindFirstObjectSafe(UClass* Class, FStringView Name, EFindFirstObjectOptions Options /*= EFindFirstObjectOptions::None*/, ELogVerbosity::Type AmbiguousMessageVerbosity /*= ELogVerbosity::Warning*/, const TCHAR* InCurrentOperation /*= nullptr*/)
{
	UObject* Result = nullptr;
	if (!UE::IsSavingPackage(nullptr) && !IsGarbageCollectingAndLockingUObjectHashTables())
	{
...
void StaticTick( float DeltaTime, bool bUseFullTimeLimit, float AsyncLoadingTime )
{
	...
	int64 NameTableMemorySize = FName::GetNameTableMemorySize();
	...
	SET_MEMORY_STAT( STAT_NameTableMemorySize, NameTableMemorySize );
...
UObject* StaticLoadObjectInternal(UClass* ObjectClass, UObject* InOuter, FStringView InName, FStringView Filename, uint32 LoadFlags, UPackageMap* Sandbox, bool bAllowObjectReconciliation, const FLinkerInstancingContext* InstancingContext)
{
	...
	UE_TRACE_LOG_SCOPED_T(Cpu, LoadObject, CpuChannel)
		<< LoadObject.AssetPath(InName.GetData(), InName.Len());
#endif // CPUPROFILERTRACE_ENABLED
	SCOPED_NAMED_EVENT(StaticLoadObjectInternal, FColor::Red);

	FScopedLoadingState ScopedLoadingState = FScopedLoadingState(FString(InName));
	TStringBuilder<512> StrName;
	StrName = InName;
	UObject* Result = nullptr;
	const bool bContainsObjectName = UE::String::FindFirstChar(InName, TEXT('.')) != INDEX_NONE;
...
UObject* StaticLoadObject(UClass* ObjectClass, UObject* InOuter, FStringView InName, FStringView Filename, uint32 LoadFlags, UPackageMap* Sandbox, bool bAllowObjectReconciliation, const FLinkerInstancingContext* InstancingContext)
{
...
int32 LoadAssetAsync(FTopLevelAssetPath InAssetPath, FLoadAssetAsyncDelegate InCompletionDelegate, FLoadAssetAsyncOptionalParams InOptionalParams)
{
	...
	FLoadPackageAsyncOptionalParams PackageParams{
		.ProgressDelegate = InOptionalParams.ProgressDelegate,
	...
	PackageParams.CompletionDelegate = MakeUnique<FLoadPackageAsyncDelegate>(FLoadPackageAsyncDelegate::CreateLambda(
		[InAssetPath, LoadFlags, CompletionDelegate = MoveTemp(InCompletionDelegate)](const FName& LoadedPackageName, UPackage* LoadedPackage, EAsyncLoadingResult::Type Result) mutable
		{
			...
			{
				LoadedObject = StaticFindObjectFast(UObject::StaticClass(), LoadedPackage, InAssetPath.GetAssetName());
...
namespace NameReuse
{
	...
	FName MakeUniqueObjectNameReusingNumber(UObject* Parent, FName BaseName, EUniqueObjectNameOptions Options)
	{
		...
		if (!Parent || !!(Options & EUniqueObjectNameOptions::GloballyUnique) || BaseName == NamePackage || FPlatformProperties::HasEditorOnlyData() || !GFastPathUniqueNameGeneration)
		{
			return FName();
		}
...
#if WITH_EDITOR
void RecycleUniqueObjectSuffixAnnotation(UObject* Parent, UClass* ForClass)
{
	int32 MaxIndex = 0;
	FName BaseName = ForClass->GetFName();
	BaseName.SetNumber(0);

	ForEachObjectWithOuter(Parent, [&MaxIndex, BaseName](UObject* Object)
		{
			FName ObjectName = Object->GetFName();
			int32 Index = ObjectName.GetNumber();
			ObjectName.SetNumber(0);

			if (BaseName == ObjectName)
			{
				if (Index > MaxIndex)
				{
					MaxIndex = Index;
	...
	UpdateSuffixForNextNewObject(Parent, ForClass, [MaxIndex](int32& Index)
		{
			Index = MaxIndex;
		});
}

void ReleaseUniquelyNamedObject(UObject* Object)
{
	UObject* Outer = Object->GetOuter();
	UClass* Class = Object->GetClass();

	// Make the name available again for other objects
	Object->Rename(nullptr, GetTransientPackage());

	// Reset the suffix in FPerClassNumberSuffixAnnotation to the lowest we can 
	RecycleUniqueObjectSuffixAnnotation(Outer, Class);
}
#endif
...
#undef TRY_REUSE_NAMES
	...
	do
	{
		...
		do
		{
			...
			{
				int32 NextClassUnique = 0;
				UE_AUTORTFM_OPEN
				{
					NextClassUnique = FPlatformAtomics::InterlockedIncrement((volatile int32*)&Class->ClassUnique);
				};

				if (Parent == NULL)
				{
					//package names should default to "/Temp/Untitled" when their parent is NULL. Otherwise they are a group.
					TestName = FName(*FString::Printf(TEXT("/Temp/%s"), LexToString(NAME_Untitled)), NextClassUnique);
				}
				else
				{
					//package names should default to "Untitled"
					TestName = FName(NAME_Untitled, NextClassUnique);
				}
			}
			else
			{
				...
				UE_AUTORTFM_OPEN
				{
					if (Parent && !(Options & EUniqueObjectNameOptions::GloballyUnique))
					{
					...
					{
						NameNumber = FPlatformAtomics::InterlockedIncrement((volatile int32*)&Class->ClassUnique);
					}
				};
				TestName = FName(BaseName, NameNumber);
			}

			if (!!(Options & EUniqueObjectNameOptions::GloballyUnique))
			{
...
#if UE_WITH_REMOTE_OBJECT_HANDLE
		| EInternalObjectFlags::RemoteReference
#endif
		);
	...
	for( int32 ObjectIndex = 0;ObjectIndex < SerializedObjects.Num(); ObjectIndex++)
	{
		...
		if ( Parameters.DuplicationSeed.Find(OrigObject) == nullptr )
		{
			...
			if (UObject* DuplicatedObject = DupObjectInfo.DuplicatedObject.GetEvenIfUnreachable())
			{
				...
				// Now that subobjects are instanced, we can invoke the class initializer function on the instance. Note that this will get
				// deferred during object construction because subobject instancing was initially disabled through the instance graph above.
				DuplicatedObject->GetClass()->PostInitInstance(DuplicatedObject, &InstanceGraph);
			}
		}
	}
...
namespace
{
	FORCENOINLINE void TryGetRemoteObjectOverrides(FName InName, UObject* InOuter, int32& InOutSerialNumber, FRemoteObjectId& InOutRemoteId)
	{
		using namespace UE::RemoteObject::Serialization;

		FRemoteObjectConstructionOverridesStack& OverridesStack = FRemoteObjectConstructionOverridesStack::Get();
		if (!OverridesStack.IsEmpty())
		{
			// Try to get remote subobject overrides so that if the object that's about to be constructed is a subobject
			...
			if (const FRemoteObjectConstructionParams* ConstructionOverrides = OverridesStack.Find(InName, InOuter))
			{
...
void FObjectInitializer::PostConstructInit()
{
	...
	// This code disables the config access tracking for CDOs. It should also be disabled for Immutable CDOs. When RF_ImmutableDefaultObject is set, 
	// RF_ClassDefaultObject is also set so I only check for RF_ClassDefaultObject.
	const bool bIsAnyKindOfCDO = Obj->HasAnyFlags(RF_ClassDefaultObject);
	TOptional<UE::ConfigAccessTracking::FIgnoreScope> IgnoreConfigAccessTracking;
	if (bIsAnyKindOfCDO)
	{
		IgnoreConfigAccessTracking.Emplace();
	}
	...
	if (bUseAnnotationsForPropertyInitializedStateTracking)
	{
		...
		if (UClass* ClassClass = Class->GetClass(); ClassClass && ClassClass->IsChildOf<UVerseClass>())
		{
			...
			{
				AutoRTFM::EContextStatus Status = AutoRTFM::Close([&] { UE::FInitializedPropertyValueState(Obj).ActivateTracking(); });
	...
	// Only invoke the class initializer if subobject instancing is enabled. Since object properties may have been default-initialized to
	// reference archetypes before instancing, archetypes could end up being mutated by compiled bytecode if executed before fixup occurs.
	if (bAllowInstancing)
	{
...
namespace UECodeGen_Private
{
	...
	void ConstructFProperty(FFieldVariant Outer, const FPropertyParamsBase* const*& PropertyArray, int32& NumProperties)
	{
		...
		switch (PropBase->Flags & PropertyTypeMask)
		{
...
#if WITH_VERSE_BPVM
			case EPropertyGenFlags::VerseType:
			{
				NewProp = NewFProperty<FVerseClassProperty, FVerseClassPropertyParams>(Outer, *PropBase);
			}
			break;
#endif
			...
			{
#if WITH_VERSE_BPVM
				static_assert(std::is_same_v<FVerseDynamicProperty, FVerseValueProperty>);
...
#else
				static_assert(std::is_same_v<FVRestValueProperty, FVerseValueProperty>);
			...
			case EPropertyGenFlags::VerseType:
			{
				NewProp = NewFProperty<FVRestValueProperty, FVerseClassPropertyParams>(Outer, *PropBase);
			}
			break;
#endif
	...
	void ConstructUPackage(UPackage*& OutPackage, const FPackageParams& Params)
	{
		...
		UObject* FoundPackage = StaticFindObjectFast(UPackage::StaticClass(), nullptr, FName(UTF8_TO_TCHAR(Params.NameUTF8)), EFindObjectFlags::None);
		...
		for (UObject* (*const *SingletonFunc)() = Params.SingletonFuncArray, *(*const *SingletonFuncEnd)() = SingletonFunc + Params.NumSingletons; SingletonFunc != SingletonFuncEnd; ++SingletonFunc)
		{
			...
			if (Object->GetOuter() == NewPackage)
			{
				...
				NotifyRegistrationEvent(NewPackage->GetFName(), Object->GetFName(), ENotifyRegistrationType::NRT_NoExportObject, ENotifyRegistrationPhase::NRP_Finished, nullptr, false, Object);
			}
		}
#if WITH_RELOAD

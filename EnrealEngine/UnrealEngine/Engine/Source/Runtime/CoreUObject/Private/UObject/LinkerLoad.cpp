...
{
	if (FBulkData::HasFlags(BulkDataFlags, BULKDATA_PayloadInSeparateFile) == false)
	{
...
FLinkerLoad::ELinkerStatus FLinkerLoad::ProcessPackageSummary(TMap<TPair<FName, FPackageIndex>, FPackageIndex>* ObjectNameWithOuterToExportMap)
{
	...
	if (Status == LINKER_Loaded)
	{
		SCOPED_LOADTIMER(LinkerLoad_SerializeImportTypeHierarchies);
		Status = SerializeImportTypeHierarchies();
	}
#endif
...
, bHasSerializedImportTypeHierarchies(false)
#if WITH_EDITOR
...
FLinkerLoad::ELinkerStatus FLinkerLoad::SerializePackageFileSummary()
{
	...
	if (bHasSerializedPackageFileSummary == false)
	{
		...
		ImportTypeHierarchies.Empty(Summary.ImportTypeHierarchiesCount);
...
FLinkerLoad::ELinkerStatus FLinkerLoad::SerializeSoftObjectPathList()
{
	...
	FSoftObjectPathSerializationScope SerializationScope(NAME_None, NAME_None,
		ESoftObjectPathCollectType::PackageHeader, ESoftObjectPathSerializeType::AlwaysSerialize);
#endif // WITH_EDITOR
...
FLinkerLoad::ELinkerStatus FLinkerLoad::SerializeImportTypeHierarchies()
{
	DECLARE_SCOPE_CYCLE_COUNTER(TEXT("FLinkerLoad::SerializeImportTypeHierarchies"), STAT_LinkerLoad_SerializeImportTypeHierarchies, STATGROUP_LinkerLoad);

	if (!UE::Serialization::Private::FImportTypeHierarchy::IsSerializationEnabled())
	{
		return LINKER_Loaded;
	}

	// Skip serializing import types if the package does not include any or when dealing with a text format.
	if (bHasSerializedImportTypeHierarchies || (Summary.ImportTypeHierarchiesCount == 0) || (Summary.ImportTypeHierarchiesOffset == 0) || IsTextFormat())
	{
		return LINKER_Loaded;
	}

	Seek(Summary.ImportTypeHierarchiesOffset);

	FStructuredArchive::FStream Stream = StructuredArchiveRootRecord->EnterStream(TEXT("ImportTypeHierarchies"));
	Stream << ImportTypeHierarchies;

	// Return whether we finished this step and it's safe to start with the next.
	if (Stream.GetUnderlyingArchive().IsCriticalError())
	{
		return LINKER_Failed;
	}
	if (IsTimeLimitExceeded(TEXT("serializing import type hierarchies")))
	{
		return LINKER_TimedOut;
	}

	bHasSerializedImportTypeHierarchies = true;
	...
	if (OuterObject)
	{
		...
		else
		{
			...
			else
			{
				...
				ClassPackage = StaticFindObjectFast(UPackage::StaticClass(), nullptr, ClassImport.ClassPackage, EFindObjectFlags::ExactClass);
			}
			if (ClassPackage)
			{
				TheClass = (UClass*)StaticFindObjectFast(UClass::StaticClass(), ClassPackage, ImpExp(Export.ClassIndex).ObjectName, EFindObjectFlags::None);
			}
			else
			{
		...
		if (TheClass)
		{
			...
			Export.Object = StaticFindObjectFast(TheClass, OuterObject, Export.ObjectName, EFindObjectFlags::ExactClass);
...
UObject* FLinkerLoad::FindExistingImport(int32 ImportIndex)
{
	...
	if (Import.OuterIndex.IsNull())
	{
		...
		UPackage* Package = static_cast<UPackage*>(StaticFindObjectFast(UPackage::StaticClass(), nullptr, ObjectName, EFindObjectFlags::ExactClass));
	...
	if (OuterObject)
	{
		...
		else
		{
			...
			if (ClassPackage)
			{
				TheClass = FindObjectFast<UClass>(ClassPackage, Import.ClassName, EFindObjectFlags::None);
			}
		}
		...
		if (TheClass)
		{
			return StaticFindObjectFast(UClass::StaticClass(), OuterObject, Import.ObjectName, EFindObjectFlags::ExactClass);
		}
	}
	return nullptr;
}
	...
	if (Import.SourceLinker && Import.SourceIndex == INDEX_NONE && Import.XObject == NULL && !Import.OuterIndex.IsNull() && Import.ObjectName != NAME_ObjectRedirector)
	{
		...
		else
		{
			...
			else
			{
				...
				bool bSuppressLinkerError = IsSuppressableBlueprintImportError(ImportIndex);
				if (!bSuppressLinkerError)
				{
		...
		if (UObject* OuterObject = InOutImportMap[Outer.ToImport()].XObject)
		{
			...
			for (FPackageIndex Inner : Inners)
			{
				...
				if (!ObjectImport.XObject)
				{
					...
					if (UObject* InnerObject = StaticFindObjectFastInternal(Class, OuterObject, ObjectImport.ObjectName, EFindObjectFlags::None))
					{
...
UClass* FLinkerLoad::TryCreatePlaceholderClassImport(int32 ImportIndex)
{
	...
	if (UObject* ImportClassPackage = FindObjectFast<UPackage>(nullptr, Import.ClassPackage, EFindObjectFlags::ExactClass))
	{
		if (UClass* ImportClass = FindObjectFast<UClass>(ImportClassPackage, Import.ClassName, EFindObjectFlags::None))
		{
			FPackageIndex ImportPackageIndex = FPackageIndex::FromImport(ImportIndex);
			const UE::Serialization::Private::FImportTypeHierarchy* ImportTypeHierarchy = ImportTypeHierarchies.Find(ImportPackageIndex);

			if (UE::CanCreatePropertyBagPlaceholderTypeForImportClass(ImportClass, ImportTypeHierarchy))
			{
...
void FLinkerLoad::Preload( UObject* Object )
{
	...
	// This is important to take the lock before checking the RF_NeedLoad flag in case
	// Preload is called on the same objects from 2 different threads, one might see the RF_NeedLoad and remove it
	// before the other, making one thread return from Preload before the object is fully serialized.
	UE::TUniqueLock UniqueLock(Mutex);
	...
	if (Object->HasAnyFlags(RF_NeedLoad))
	{
		...
		{
#if USE_CIRCULAR_DEPENDENCY_LOAD_DEFERRING
			// We need to take the lock before changing the flags
			UE::TUniqueLock UniqueLockInner(Linker->Mutex);
			uint32 const DeferredLoadFlag = (LoadFlags & LOAD_DeferDependencyLoads);
...
bool FLinkerLoad::WillTextureBeLoaded( UClass* Class, int32 ExportIndex )
{
	...
	UObject* ExistingTexture = StaticFindObjectFastExplicit( Class, Export.ObjectName, PathName, EFindObjectFlags::None, RF_NoFlags );
	if ( ExistingTexture )
	{
		return false;
	}
...
UObject* FLinkerLoad::CreateExport( int32 Index )
{
	...
	if( !Export.Object && !FilterExport(Export) ) // for some acceptable position, it was not "not for"
	{
		...
		if( !Export.SuperIndex.IsNull() )
		{
			...
			if (!SuperStruct)
			{
				// Try creating a placeholder type for the super struct.
				if (Export.SuperIndex.IsImport())
				{
					SuperStruct = TryCreatePlaceholderClassImport(Export.SuperIndex.ToImport());
				}
				else
				{
					SuperStruct = TryCreatePlaceholderClassForExport(Export.SuperIndex.ToExport());
				}
			}
#endif
		...
		UObject* ActualObjectWithTheName = StaticFindObjectFastInternal(nullptr, ThisParent, Export.ObjectName, EFindObjectFlags::ExactClass);
		...
		{
			UObjectRedirector* Redirector = (UObjectRedirector*)StaticFindObjectFast(UObjectRedirector::StaticClass(), ThisParent, Export.ObjectName, EFindObjectFlags::ExactClass);
...
UObject* FLinkerLoad::CreateImport( int32 Index )
{
	...
	if (Import.XObject == nullptr && Import.ObjectName != NAME_None)
	{
		...
		if (!GIsEditor && !IsRunningCommandlet())
		{
			...
			if( UPackage* ClassPackage = FindObjectFast<UPackage>( nullptr, Import.ClassPackage, EFindObjectFlags::None ) )
			{
				if( UClass*	FindClass = FindObjectFast<UClass>( ClassPackage, Import.ClassName, EFindObjectFlags::None ) ) //
				{
		...
		else if (ImportsToVerifyOnCreate.Contains(Index))
		{
			...
			if (UPackage* ImportClassPackage = FindObjectFast<UPackage>(nullptr, InstancingContext.RemapPackage(Import.ClassPackage), EFindObjectFlags::None))
			{
				const UObject* FoundObject = FindObjectFast<UObject>(ImportClassPackage, Import.ClassName, EFindObjectFlags::None);
		...
		if (ResolvedObject && UE::FPropertyBagRepository::IsPropertyBagPlaceholderObject(ResolvedObject))
		{
			if (UE::FPropertyBagRepository::IsPropertyBagPlaceholderObjectFeatureEnabled(UE::FPropertyBagRepository::EPlaceholderObjectFeature::SerializeExportReferencesOnLoad))
			{
				...
				if (!ObjectProperty || !ResolvedObject->GetClass()->IsChildOf(ObjectProperty->PropertyClass))
				{
					...
					return FObjectPtr(FObjectHandle{ UE::CoreUObject::Private::MakePackedObjectRef(ResolvedObject).EncodedRef });
#else
		...
		for ( int32 ExportMapIdx = 0; ExportMapIdx < ExportMap.Num(); ExportMapIdx++ )
		{
			...
			if (UEVer() < VER_UE4_BLUEPRINT_GENERATED_CLASS_COMPONENT_TEMPLATES_PUBLIC)
			{
				if ((Export.ObjectFlags & RF_Public) == 0)
				{
					...
					{
						static const UClass* ActorComponentClass = FindObjectChecked<UClass>(nullptr, TEXT("/Script/Engine.ActorComponent"), EFindObjectFlags::ExactClass);
...
bool FLinkerLoad::SerializeBulkData(FBulkData& BulkData, const FBulkDataSerializationParams& Params)
{
	...
	else if (Meta.HasAnyFlags(BULKDATA_PayloadInSeparateFile))
	{

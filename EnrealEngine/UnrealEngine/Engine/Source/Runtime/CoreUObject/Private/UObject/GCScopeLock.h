	...
	void LockAsync()
	{
		if (!IsInGameThread())
		{
			...
			do
			{
				...
				{
					// We only need a read lock here since AsyncCounter is already thread-safe
					// with other threads that want to lock but we need to guard against
					// the GC trying to take the write lock.
					UE::TSharedLock SharedLock(SharedMutex);
					if (GCCounter.GetValue() == 0)
					{

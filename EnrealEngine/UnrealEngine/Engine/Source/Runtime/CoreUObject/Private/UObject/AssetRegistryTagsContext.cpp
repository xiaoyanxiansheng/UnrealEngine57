...
void FAssetRegistryTagsContext::RemoveTag(FName TagName)
{
	RemoveTagInternal(TagName, FStringView());
}
...
void FAssetRegistryTagsContext::RemoveCookTag(FName TagName)
{
	if (!WantsCookTags())
	{
		return;
	}
	RemoveTagInternal(TagName, UE::AssetRegistry::CookTagPrefix);
}
...
FName FAssetRegistryTagsContext::TransformTagName(FName TagName, FStringView PrefixToAdd, bool bCreateNameIfDoesNotExist) const
{
	...
	if (!PrefixToAdd.IsEmpty())
	{
		...
		TagName = FName(TagNameStr, bCreateNameIfDoesNotExist ? FNAME_Add : FNAME_Find);
	}
	return TagName;
}
...
{
	TagName = TransformTagName(TagName, PrefixToAdd, false);
	if (TagName.IsNone())
	{
		return nullptr;
	}
...
void FAssetRegistryTagsContext::RemoveTagInternal(FName TagName, FStringView PrefixToAdd)
{
	TagName = TransformTagName(TagName, PrefixToAdd, false);
	if (TagName.IsNone())
	{

	...
	, FieldMask(1)
{
	...
	if (Prop.SetBitFunc)
	{
		DetermineBitfieldOffsetAndMask(Offset, BitMask, Prop.SetBitFunc, Prop.SizeOfOuter);
		check(BitMask);
	}
...
void FBoolProperty::DetermineBitfieldOffsetAndMask(uint32& Offset, uint32& BitMask, void (*SetBit)(void* Obj), SIZE_T SizeOf)
{
...
void FBoolProperty::LinkInternal(FArchive& Ar)
{
	...
	if (IsNative())
	{
		// Calculate bit mask settings since we couldn't do it at compile time
		bool bIsNativeBool = IsNativeBool();
		uint32 Offset = 0;
		uint32 BitMask = 0;
		if (SetBitFunc)
		{
			// Outer's PropertiesSize was set to sizeof() in constinit constructor
			DetermineBitfieldOffsetAndMask(Offset, BitMask, SetBitFunc, GetOwnerChecked<UStruct>()->PropertiesSize);
			check(BitMask);
		}

		SetOffset_Internal(Offset);
		SetBoolSize(GetElementSize(), bIsNativeBool, BitMask);
	}
#endif
...
EConvertFromTypeResult FBoolProperty::ConvertFromType(const FPropertyTag& Tag, FStructuredArchive::FSlot Slot, uint8* Data, const UStruct* DefaultsStruct, const uint8* Defaults)
{

...
class UARGeoTrackingSupport : public UObject, public IModularFeature
{
	GENERATED_BODY()
...
public:
	...
	static UE_API class UARGeoTrackingSupport* GetGeoTrackingSupport();
	 ...
	 */
	UE_API virtual TSharedPtr<FCheckGeoTrackingAvailabilityAsyncTask, ESPMode::ThreadSafe> CheckGeoTrackingAvailability(FString& OutError);
	
	/**
	 * @return the async task to check Geo tracking availability at a specific location.
	 * Can return null if the feature is not supported.
	 */
	UE_API virtual TSharedPtr<FCheckGeoTrackingAvailabilityAsyncTask, ESPMode::ThreadSafe> CheckGeoTrackingAvailability(float Longitude, float Latitude, FString& OutError);
	
	/**
	 * @return the async task to convert a UE4 world position into a Geo location.
	 * Can return null if the feature is not supported.
	 */
	UE_API virtual TSharedPtr<FGetGeoLocationAsyncTask, ESPMode::ThreadSafe> GetGeoLocationAtWorldPosition(const FVector& WorldPosition, FString& OutError);
...
public:
	...
	static UE_API UCheckGeoTrackingAvailabilityAsyncTaskBlueprintProxy* CheckGeoTrackingAvailability(UObject* WorldContextObject);
	
	/**
	 * Check Geo tracking availability at a specific Geo location.
	 */
	UFUNCTION(BlueprintCallable, Category = "AR AugmentedReality|Geo Tracking", meta=(BlueprintInternalUseOnly="true", WorldContext = "WorldContextObject"))
	static UE_API UCheckGeoTrackingAvailabilityAsyncTaskBlueprintProxy* CheckGeoTrackingAvailabilityAtLocation(UObject* WorldContextObject, float Longitude, float Latitude);
	
private:
	// UBlueprintAsyncActionBase interface
	UE_API void Activate() override;
	UE_API void ReportSuccess() override;
	UE_API void ReportFailure() override;
	//~UBlueprintAsyncActionBase interface
...
public:
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "AR AugmentedReality|Geo Tracking", meta=(BlueprintInternalUseOnly="true", WorldContext = "WorldContextObject"))
	static UE_API UGetGeoLocationAsyncTaskBlueprintProxy* GetGeoLocationAtWorldPosition(UObject* WorldContextObject, const FVector& WorldPosition);

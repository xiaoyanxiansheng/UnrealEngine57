...
public:
	...
	static UE_API bool IsARSupported();
	...
	static UE_API void StartARSession(UARSessionConfig* SessionConfig);
	...
	static UE_API void PauseARSession();
	...
	static UE_API void StopARSession();
	...
	static UE_API FARSessionStatus GetARSessionStatus();
	...
	static UE_API UARSessionConfig* GetSessionConfig();
	...
	static UE_API bool ToggleARCapture(const bool bOnOff, const EARCaptureType CaptureType);

	/** Enable or disable Mixed Reality Capture camera. */
	UFUNCTION(BlueprintCallable, Category = "AR", meta = (Keywords = "ar all"))
	static UE_API void SetEnabledXRCamera(bool bOnOff);

	/** Change screen size of Mixed Reality Capture camera. */
	UFUNCTION(BlueprintCallable, Category = "AR", meta = (Keywords = "ar all"))
	static UE_API FIntPoint ResizeXRCamera(const FIntPoint& InSize);
	...
	static UE_API void SetAlignmentTransform( const FTransform& InAlignmentTransform );
	...
	static UE_API TArray<FARTraceResult> LineTraceTrackedObjects( const FVector2D ScreenCoord, bool bTestFeaturePoints = true, bool bTestGroundPlane = true, bool bTestPlaneExtents = true, bool bTestPlaneBoundaryPolygon = true );
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "AR AugmentedReality|Trace Result", meta = (AdvancedDisplay = "1", Keywords = "ar augmentedreality augmented reality tracking tracing linetrace"))
	static UE_API TArray<FARTraceResult> LineTraceTrackedObjects3D(const FVector Start, const FVector End, bool bTestFeaturePoints = true, bool bTestGroundPlane = true, bool bTestPlaneExtents = true, bool bTestPlaneBoundaryPolygon = true);
	...
	static UE_API EARTrackingQuality GetTrackingQuality();
	...
	static UE_API EARTrackingQualityReason GetTrackingQualityReason();
	...
	static UE_API TArray<UARTrackedGeometry*> GetAllGeometries();
	...
	static UE_API TArray<UARTrackedGeometry*> GetAllGeometriesByClass(TSubclassOf<UARTrackedGeometry> GeometryClass);
	...
	static UE_API UARTexture* GetARTexture(EARTextureType TextureType);
	...
	static UE_API bool IsSessionTypeSupported(EARSessionType SessionType);
	...
	static UE_API void DebugDrawTrackedGeometry( UARTrackedGeometry* TrackedGeometry, UObject* WorldContextObject, FLinearColor Color = FLinearColor(1.0f, 1.0f, 0.0f, 0.75f), float OutlineThickness=5.0f, float PersistForSeconds = 0.0f );
	...
	static UE_API void DebugDrawPin( UARPin* ARPin, UObject* WorldContextObject, FLinearColor Color = FLinearColor(1.0f, 1.0f, 0.0f, 0.75f), float Scale=5.0f, float PersistForSeconds = 0.0f );
	...
	static UE_API UARLightEstimate* GetCurrentLightEstimate();
	...
	static UE_API UARPin* PinComponent( USceneComponent* ComponentToPin, const FTransform& PinToWorldTransform, UARTrackedGeometry* TrackedGeometry = nullptr, const FName DebugName = NAME_None );
	...
	static UE_API UARPin* PinComponentToTraceResult( USceneComponent* ComponentToPin, const FARTraceResult& TraceResult, const FName DebugName = NAME_None );
	...
	static UE_API bool PinComponentToARPin(USceneComponent* ComponentToPin, UARPin* Pin);

	/** Given a pinned \c ComponentToUnpin, remove its attachment to the real world. */
	UFUNCTION(BlueprintCallable, Category = "AR AugmentedReality|ARPin", meta = (Keywords = "ar augmentedreality augmented reality tracking arpin tracked geometry pinning anchor"))
	static UE_API void UnpinComponent( USceneComponent* ComponentToUnpin );
	
	/** Remove a pin such that it no longer updates the associated component. */
	UFUNCTION(BlueprintCallable, Category = "AR AugmentedReality|ARPin", meta = (Keywords = "ar augmentedreality augmented reality tracking arpin tracked geometry pinning anchor"))
	static UE_API void RemovePin( UARPin* PinToRemove );
	...
	static UE_API TArray<UARPin*> GetAllPins();
	...
	static UE_API bool IsARPinLocalStoreSupported();
	 ...
	 */
	UFUNCTION(BlueprintPure, Category = "AR AugmentedReality|ARPin", meta = (Keywords = "ar augmentedreality augmented reality tracking arpin anchor LocalStore"))
	static UE_API bool IsARPinLocalStoreReady();
	...
	static UE_API TMap<FName, UARPin*> LoadARPinsFromLocalStore();
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "AR AugmentedReality|ARPin", meta = (Keywords = "ar augmentedreality augmented reality tracking arpin anchor LocalStore"))
	static UE_API bool SaveARPinToLocalStore(FName InSaveName, UARPin* InPin);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "AR AugmentedReality|ARPin", meta = (Keywords = "ar augmentedreality augmented reality tracking arpin anchor LocalStore"))
	static UE_API void RemoveARPinFromLocalStore(FName InSaveName);

	/**
	 * Remove all ARPins from the local store
	 */
	UFUNCTION(BlueprintCallable, Category = "AR AugmentedReality|ARPin", meta = (Keywords = "ar augmentedreality augmented reality tracking arpin anchor LocalStore"))
	static UE_API void RemoveAllARPinsFromLocalStore();
	...
	static UE_API bool AddManualEnvironmentCaptureProbe(FVector Location, FVector Extent);
	...
	static UE_API EARWorldMappingState GetWorldMappingStatus();
	...
	static UE_API TArray<FVector> GetPointCloud();
	...
	static UE_API TArray<FARVideoFormat> GetSupportedVideoFormats(EARSessionType SessionType);

	static UE_API TSharedPtr<FARSaveWorldAsyncTask, ESPMode::ThreadSafe> SaveWorld();
	static UE_API TSharedPtr<FARGetCandidateObjectAsyncTask, ESPMode::ThreadSafe> GetCandidateObject(FVector Location, FVector Extent);
	...
	static UE_API UARCandidateImage* AddRuntimeCandidateImage(UARSessionConfig* SessionConfig, UTexture2D* CandidateTexture, FString FriendlyName, float PhysicalWidth);
	...
	static UE_API bool IsSessionTrackingFeatureSupported(EARSessionType SessionType, EARSessionTrackingFeature SessionTrackingFeature);
	...
	static UE_API bool IsSceneReconstructionSupported(EARSessionType SessionType, EARSceneReconstruction SceneReconstructionMethod);
	...
	static UE_API TArray<FARPose2D> GetAllTracked2DPoses();
	...
	*/
	...
	static UE_API bool GetObjectClassificationAtLocation(const FVector& InWorldLocation, EARObjectClassification& OutClassification, FVector& OutClassificationLocation, float MaxLocationDiff = 10.f);
	...
	static UE_API void SetARWorldOriginLocationAndRotation(FVector OriginLocation, FRotator OriginRotation, bool bIsTransformInWorldSpace = true, bool bMaintainUpDirection = true);
	...
	static UE_API void SetARWorldScale(float InWorldScale);
	...
	static UE_API float GetARWorldScale();
	...
	static UE_API FTransform GetAlignmentTransform();
	...
	static UE_API bool AddTrackedPointWithName(const FTransform& WorldTransform, const FString& PointName, bool bDeletePointsWithSameName = true);
	...
	static UE_API TArray<UARTrackedPoint*> FindTrackedPointsByName(const FString& PointName);
	...
	static UE_API void CalculateClosestIntersection(const TArray<FVector>& StartPoints, const TArray<FVector>& EndPoints, FVector& ClosestIntersection);
	...
	UFUNCTION(BlueprintCallable, Category = Alignment)
	static UE_API void CalculateAlignmentTransform(const FTransform& TransformInFirstCoordinateSystem, const FTransform& TransformInSecondCoordinateSystem, FTransform& AlignmentTransform);
	...
	static UE_API int32 GetNumberOfTrackedFacesSupported();
	
	/** @return the intrinsics of the AR camera. */
	UFUNCTION(BlueprintCallable, Category = "AR AugmentedReality|Geo Tracking")
	static UE_API bool GetCameraIntrinsics(FARCameraIntrinsics& OutCameraIntrinsics);
...
public:
	static UE_API void RegisterAsARSystem(const TSharedRef<FARSupportInterface , ESPMode::ThreadSafe>& NewArSystem);
	
private:
	static UE_API const TWeakPtr<FARSupportInterface , ESPMode::ThreadSafe>& GetARSystem();
	static UE_API TWeakPtr<FARSupportInterface , ESPMode::ThreadSafe> RegisteredARSystem;
};


UCLASS(MinimalAPI, meta=(ScriptName="ARTraceResultLibrary"))
class UARTraceResultLibrary : public UBlueprintFunctionLibrary
{
	...
	static UE_API float GetDistanceFromCamera( const FARTraceResult& TraceResult );
	 ...
	 */
	UFUNCTION( BlueprintPure, Category = "AR AugmentedReality|Trace Result" )
	static UE_API FTransform GetLocalToTrackingTransform( const FARTraceResult& TraceResult );
	
	/** @return Get the transform of the trace result in Unreal World Space. */
	UFUNCTION( BlueprintPure, Category = "AR AugmentedReality|Trace Result" )
	static UE_API FTransform GetLocalToWorldTransform( const FARTraceResult& TraceResult );
	
	/** @return Get the transform of the trace result in the AR system's local space. */
	UFUNCTION( BlueprintPure, Category = "AR AugmentedReality|Trace Result" )
	static UE_API FTransform GetLocalTransform( const FARTraceResult& TraceResult );
	
	/** @return Get the real-world object (as observed by the Augmented Reality system) that was intersected by the line trace. */
	UFUNCTION( BlueprintPure, Category = "AR AugmentedReality|Trace Result" )
	static UE_API UARTrackedGeometry* GetTrackedGeometry( const FARTraceResult& TraceResult );
	
	/** @return  Get the type of the tracked object (if any) that effected this trace result. */
	UFUNCTION( BlueprintPure, Category = "AR AugmentedReality|Trace Result" )
	static UE_API EARLineTraceChannels GetTraceChannel( const FARTraceResult& TraceResult );
};

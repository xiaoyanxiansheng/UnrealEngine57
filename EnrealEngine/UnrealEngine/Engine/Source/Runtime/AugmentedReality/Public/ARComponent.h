...
public:
	...
	UE_API virtual void Remove(UARTrackedGeometry* TrackedGeometry);
...
protected:
	...
	UFUNCTION()
	UE_API virtual void OnRep_Payload();
...
public:
	...
	UE_API void ReceiveAdd(const FARPlaneUpdatePayload& Payload);
	...
	UE_API void ReceiveUpdate(const FARPlaneUpdatePayload& Payload);
	
	/** Overridable native event for when native representation is updated. */
	UE_API virtual void Update(UARTrackedGeometry* TrackedGeometry) override;
	
	UFUNCTION(BlueprintCallable, Category = "AR Gameplay | Debug", meta=(DevelopmentOnly))
	static UE_API void SetPlaneComponentDebugMode(EPlaneComponentDebugMode NewDebugMode);
	
	UFUNCTION(BlueprintCallable, Category = "AR Gameplay | Debug", meta=(DevelopmentOnly))
	static UE_API void SetObjectClassificationDebugColors(const TMap<EARObjectClassification, FLinearColor>& InColors);
	
	UFUNCTION(BlueprintPure, Category = "AR Gameplay | Debug", meta=(DevelopmentOnly))
	static UE_API const TMap<EARObjectClassification, FLinearColor>& GetObjectClassificationDebugColors();
	
protected:
	UFUNCTION(Reliable, Server, WithValidation)
	UE_API void ServerUpdatePayload(const FARPlaneUpdatePayload& NewPayload);
	
	UPROPERTY(BlueprintReadOnly, ReplicatedUsing=OnRep_Payload, Category = "AR Gameplay")
	FARPlaneUpdatePayload ReplicatedPayload;
	
	UE_API virtual void OnRep_Payload() override;
	UE_API virtual void UpdateVisualization_Implementation() override;
...
public:
	/** Event when native representation is first added, called on server and clients. */
	UFUNCTION(BlueprintImplementableEvent, meta = (DisplayName = "Add"))
	UE_API void ReceiveAdd(const FARPointUpdatePayload& Payload);
	
	/** Event when native representation is updated, called on server and clients. */
	UFUNCTION(BlueprintImplementableEvent, meta = (DisplayName = "Update"))
	UE_API void ReceiveUpdate(const FARPointUpdatePayload& Payload);
...
protected:
	UFUNCTION(Reliable, Server, WithValidation)
	UE_API void ServerUpdatePayload(const FARPointUpdatePayload& NewPayload);
...
public:
	...
	/** Event when native representation is first added, called on server and clients. */
	UFUNCTION(BlueprintImplementableEvent, meta = (DisplayName = "Add"))
	UE_API void ReceiveAdd(const FARQRCodeUpdatePayload& Payload);
	
	/** Event when native representation is updated, called on server and clients. */
	UFUNCTION(BlueprintImplementableEvent, meta = (DisplayName = "Update"))
	UE_API void ReceiveUpdate(const FARQRCodeUpdatePayload& Payload);
	
	/** Overridable native event for when native representation is updated. */
	UE_API virtual void Update(UARTrackedGeometry* TrackedGeometry) override;
	
	UFUNCTION(BlueprintCallable, Category = "AR Gameplay | Debug", meta=(DevelopmentOnly))
	static UE_API void SetQRCodeComponentDebugMode(EQRCodeComponentDebugMode NewDebugMode);
	
protected:
	UFUNCTION(Reliable, Server, WithValidation)
	UE_API void ServerUpdatePayload(const FARQRCodeUpdatePayload& NewPayload);
...
public:
	...
	/** Event when native representation is first added, called on server and clients. */
	UFUNCTION(BlueprintImplementableEvent, meta = (DisplayName = "Add"))
	UE_API void ReceiveAdd(const FARPoseUpdatePayload& Payload);
	
	/** Event when native representation is updated, called on server and clients. */
	UFUNCTION(BlueprintImplementableEvent, meta = (DisplayName = "Update"))
	UE_API void ReceiveUpdate(const FARPoseUpdatePayload& Payload);
	
	/** Overridable native event for when native representation is updated. */
	UE_API virtual void Update(UARTrackedGeometry* TrackedGeometry) override;
	
	UFUNCTION(BlueprintCallable, Category = "AR Gameplay | Debug", meta=(DevelopmentOnly))
	static UE_API void SetPoseComponentDebugMode(EPoseComponentDebugMode NewDebugMode);
	
protected:
	UFUNCTION(Reliable, Server, WithValidation)
	UE_API void ServerUpdatePayload(const FARPoseUpdatePayload& NewPayload);
...
public:
	/** Event when native representation is first added, called on server and clients. */
	UFUNCTION(BlueprintImplementableEvent, meta = (DisplayName = "Add"))
	UE_API void ReceiveAdd(const FAREnvironmentProbeUpdatePayload& Payload);
	
	/** Event when native representation is updated, called on server and clients. */
	UFUNCTION(BlueprintImplementableEvent, meta = (DisplayName = "Update"))
	UE_API void ReceiveUpdate(const FAREnvironmentProbeUpdatePayload& Payload);
...
protected:
	UFUNCTION(Reliable, Server, WithValidation)
	UE_API void ServerUpdatePayload(const FAREnvironmentProbeUpdatePayload& NewPayload);
...
public:
	/** Event when native representation is first added, called on server and clients. */
	UFUNCTION(BlueprintImplementableEvent, meta = (DisplayName = "Add"))
	UE_API void ReceiveAdd(const FARObjectUpdatePayload& Payload);
	
	/** Event when native representation is updated, called on server and clients. */
	UFUNCTION(BlueprintImplementableEvent, meta = (DisplayName = "Update"))
	UE_API void ReceiveUpdate(const FARObjectUpdatePayload& Payload);
...
protected:
	UFUNCTION(Reliable, Server, WithValidation)
	UE_API void ServerUpdatePayload(const FARObjectUpdatePayload& NewPayload);

...
class FArrangedChildren
{
	...
	inline void InsertWidget(EVisibility VisibilityOverride, const FArrangedWidget& InWidgetGeometry, int32 Index)
	{
		if (Accepts(VisibilityOverride))
		{
	...
	inline void Reserve(FArrangedWidgetArray::SizeType Number)
	{
	...
	inline void SetFilter(EVisibility InVisibility)
	{
		VisibilityFilter = InVisibility;
	}

	inline EVisibility GetFilter() const
	{
		return VisibilityFilter;
	}

	inline bool Accepts(EVisibility InVisibility) const
	{
		return EVisibility::DoesVisibilityPassFilter(InVisibility, VisibilityFilter);
	}

	inline bool Allows3DWidgets() const
	{
...
public:
	...
	inline FArrangedWidgetArray& GetInternalArray()
	{
		return Array;
	}

	inline const FArrangedWidgetArray& GetInternalArray() const
	{
		return Array;
	}
	...
	inline const FArrangedWidget& operator[]( int32 Index ) const
	{
		return Array[Index];
	}

	inline FArrangedWidget& operator[](int32 Index)
	{
		return Array[Index];
	}

	inline const FArrangedWidget& Last() const
	{
		return Array.Last();
	}

	inline FArrangedWidget& Last()
	{
		return Array.Last();
	}

	inline int32 FindItemIndex(const FArrangedWidget& ItemToFind) const
	{
		return Array.Find(ItemToFind);
	}

	template<typename PredicateType>
	inline int32 IndexOfByPredicate(const PredicateType& Pred) const
	{
		return Array.IndexOfByPredicate( Pred );
	}

	template<typename PredicateType>
	inline int32 FindLastByPredicate(const PredicateType& Pred) const
	{
	...
	inline void Append(const FArrangedChildren& Source)
	{

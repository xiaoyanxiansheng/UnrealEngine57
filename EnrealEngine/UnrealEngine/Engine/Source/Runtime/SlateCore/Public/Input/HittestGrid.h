...
{
	friend struct FNavigationMethod;
	friend struct FNavigationMethodOrthogonal;
	friend struct FNavigationRouting;
...
public:
	...
	const FVector2f& GetCellSize() const { return CellSize; }
	...
	SLATECORE_API void DisplayNavigation(int32 InLayer, const FGeometry& AllottedGeometry, FSlateWindowElementList& WindowElementList);
...
private:
	...
	FORCEINLINE FCell& CellAt(const int32 X, const int32 Y)
	{
	...
	/** Access a cell at coordinates X, Y. Coordinates are row and column indexes. */
	FORCEINLINE const FCell& CellAt( const int32 X, const int32 Y ) const
	{
		checkfSlow((Y*NumCells.X + X) < Cells.Num(), TEXT("HitTestGrid CellAt() failed: X= %d Y= %d NumCells.X= %d NumCells.Y= %d Cells.Num()= %d"), X, Y, NumCells.X, NumCells.Y, Cells.Num());
		return Cells[Y*NumCells.X + X];
	}
	...
	static constexpr bool IsCompatibleUserIndex(int32 RequestedUserIndex, int32 TestUserIndex)
	{
	...
	TInstancedStruct<FNavigationMethod> GetNavigationMethod(const FArrangedWidget& StartingWidget);
	TSharedPtr<SWidget> FindNextFocusableWidgetDefault(const FArrangedWidget& StartingWidget, const EUINavigation Direction, const FNavigationReply& NavigationReply, const FArrangedWidget& RuleWidget, int32 UserIndex);
	TSharedPtr<SWidget> FindNextFocusableWidgetExperimental(const FArrangedWidget& StartingWidget, const EUINavigation Direction, const FNavigationReply& NavigationReply, const FArrangedWidget& RuleWidget, int32 UserIndex);
...
private:
	...
	static const FVector2f CellSize;

#if WITH_SLATE_DEBUGGING
	/** Cache the state of the most recent FindNextFocusableWidget request*/
	TInstancedStruct<FNavigationMethod> LastNavigationMethod;	
#endif
};

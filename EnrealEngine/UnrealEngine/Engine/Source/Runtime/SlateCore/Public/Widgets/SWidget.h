...
struct TWidgetTypeTraits
{
	/** Returns true if a widget supports invalidation. **/
	static constexpr bool SupportsInvalidation() { return false; }
};
...
public:
	...
	inline bool IsParentValid() const { return ParentWidgetPtr.IsValid(); }
	inline TSharedPtr<SWidget> GetParentWidget() const { return ParentWidgetPtr.Pin(); }

	inline TSharedPtr<SWidget> Advanced_GetPaintParentWidget() const { return PersistentState.PaintParent.Pin(); }
...
public:
	...
	uint32 Debug_GetLastPrepassFrame() const 
	{ 
		return LastPrepassFrame; 
	}
private:
	...
	void Debug_UpdateLastPrepassFrame() 
	{ 
		LastPrepassFrame = GFrameNumber; 
	}
#endif // WITH_SLATE_DEBUGGING

public:

	inline TStatId GetStatID(bool bForDeferredUse = false) const
	{
#if STATS
...
public:
	 ...
	 */
	inline EVisibility GetVisibility() const { return VisibilityAttribute.Get(); }
	 ...
	 */
	inline bool IsVolatileIndirectly() const { return bInheritedVolatility; }
	...
	inline bool ShouldInvalidatePrepassDueToVolatility() { return bVolatilityAlwaysInvalidatesPrepass; }
...
#else
	inline bool IsChildWidgetCulled(const FSlateRect&, const FArrangedWidget&) const { return false; }
#endif
...
protected:
	 ...
	 */
	inline bool Advanced_InvalidateVolatility()
	{
...
public:
	...
	inline float GetRenderOpacity() const
	{
		return RenderOpacity;
	}

	/** @param InOpacity The opacity of the widget during rendering. */
	inline void SetRenderOpacity(float InRenderOpacity)
	{
	...
	inline void SetTag(FName InTag)
	{
		Tag = InTag;
	}

	/** @return the render transform of the widget. */
	inline const TOptional<FSlateRenderTransform>& GetRenderTransform() const
	{
		return RenderTransformAttribute.Get();
	}

	inline TOptional<FSlateRenderTransform> GetRenderTransformWithRespectToFlowDirection() const
	{
	...
	inline UE::Slate::FDeprecateVector2DResult GetRenderTransformPivotWithRespectToFlowDirection() const
	{
	...
	inline void SetRenderTransform(TAttribute<TOptional<FSlateRenderTransform>> InTransform)
	{
		RenderTransformAttribute.Assign(*this, MoveTemp(InTransform));
	}

	/** @return the pivot point of the render transform. */
	inline UE::Slate::FDeprecateVector2DResult GetRenderTransformPivot() const
	{
		FVector2f TransformPivot = UE::Slate::CastToVector2f(RenderTransformPivotAttribute.Get());
		return TransformPivot;
	}
	...
	inline void SetRenderTransformPivot(TAttribute<FVector2D> InTransformPivot)
	{
	...
	inline EWidgetClipping GetClipping() const
	{
		return Clipping;
	}
	...
	*/
	...
	inline EWidgetPixelSnapping GetPixelSnapping() const
	{
	 ...
	 */
	inline void SetCullingBoundsExtension(const FMargin& InCullingBoundsExtension)
	{
	...
	inline FMargin GetCullingBoundsExtension() const
	{
		return CullingBoundsExtension;
	}
...
public:
	...
	/** Returns true if this widget's class supports invalidation. */
	SLATECORE_API bool SupportsInvalidation() const;

	/**
	 * Returns true if this widget and all of its children support invalidation.
	 * This value is cached and reused unless bUseCachedValue is set to false.
	 */
	SLATECORE_API virtual bool SupportsInvalidationRecursive(bool bUseCachedValue = true) const;
private:
	/** Fast path to invalidate all parents when a descendant widget is not supporting invalidation.. */
	SLATECORE_API void SetSelfAndParentAsNotSupportingInvalidationRecursive();
;
	/** The cached value of the last SupportsInvalidationRecursive call. */
	mutable bool CachedSupportsInvalidationRecursive = false;
#endif
...
private:
	...
	// We need to keep this information for each widget as not all widgets have a class to store this information
	uint8 bSupportsInvalidation : 1 = false;
#endif
...
private: 
	...
	/** The last time this widget got prepassed. */
	uint32 LastPrepassFrame = 0;
...
inline FArrangedWidget FGeometry::MakeChild(const TSharedRef<SWidget>& ChildWidget, const UE::Slate::FDeprecateVector2DParameter& InLocalSize, const FSlateLayoutTransform& LayoutTransform) const
{
...
inline FArrangedWidget FGeometry::MakeChild(const TSharedRef<SWidget>& ChildWidget, const FLayoutGeometry& LayoutGeometry) const
{
	return MakeChild(ChildWidget, FVector2f(LayoutGeometry.GetSizeInLocalSpace()), LayoutGeometry.GetLocalToParentTransform());
}

inline FArrangedWidget FGeometry::MakeChild(const TSharedRef<SWidget>& ChildWidget, const UE::Slate::FDeprecateVector2DParameter& ChildOffset, const UE::Slate::FDeprecateVector2DParameter& InLocalSize, float ChildScale) const
{

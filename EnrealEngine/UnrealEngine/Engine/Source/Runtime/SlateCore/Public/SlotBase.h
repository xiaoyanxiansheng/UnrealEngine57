...
public:
	...
	inline void AttachWidget(TSharedRef<SWidget>&& InWidget)
	{
		DetatchParentFromContent();
		Widget = MoveTemp(InWidget);
		AfterContentOrOwnerAssigned();
	}
	inline void AttachWidget( const TSharedRef<SWidget>& InWidget )
	{
		DetatchParentFromContent();
		Widget = InWidget;
		AfterContentOrOwnerAssigned();
	 ...
	 */
	inline const TSharedRef<SWidget>& GetWidget() const
	{
		return Widget;
	}

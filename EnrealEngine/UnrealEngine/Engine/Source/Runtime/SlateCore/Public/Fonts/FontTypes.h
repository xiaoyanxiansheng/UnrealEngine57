...
public:
	...
	inline const FSlateFontInfo& GetFontInfo() const
	{
		return FontInfo;
	}
	...
	inline const FFontOutlineSettings& GetFontOutlineSettings() const
	{
...
struct FSlateFontKeyFuncs : BaseKeyFuncs<TPair<FSlateFontKey, ValueType>, FSlateFontKey, false>
{
	...
	inline static const FSlateFontKey& GetSetKey(ElementInitType Element)
	{
		return Element.Key;
	}

	inline static bool Matches(const FSlateFontKey& A, const FSlateFontKey& B)
	{
		return A.IsIdenticalToForCaching(B);
	}

	inline static uint32 GetKeyHash(const FSlateFontKey& Identifier)
	{

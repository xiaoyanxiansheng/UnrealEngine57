...
public:
	...
	inline bool operator==(const FShapedGlyphEntryKey& Other) const
	{
...
public:
	...
	inline bool operator==(const FSdfGlyphEntryKey& Other) const
	{
		return FontFace == Other.FontFace
			&& GlyphIndex == Other.GlyphIndex
...
{
public:
	inline FSdfGlyphTaskKey(const FSdfGlyphEntryKey& InSdfGlyphEntryKey, float InEmOuterSpread, float InEmInnerSpread);

	inline bool operator==(const FSdfGlyphTaskKey& Other) const
	{
...
private:
	...
	public:
		...
		inline int32 GetSourceTextStartIndex() const
		{
			return SourceTextRange.TextStart;
		}

		inline int32 GetSourceTextEndIndex() const
		{
			return SourceTextRange.TextStart + SourceTextRange.TextLen;
		}

		inline FSourceIndexToGlyphData* GetGlyphData(const int32 InSourceTextIndex)
		{
		...
		inline const FSourceIndexToGlyphData* GetGlyphData(const int32 InSourceTextIndex) const
		{
			const int32 InternalIndex = InSourceTextIndex - SourceTextRange.TextStart;
			return (GlyphDataArray.IsValidIndex(InternalIndex)) ? &GlyphDataArray[InternalIndex] : nullptr;
		}

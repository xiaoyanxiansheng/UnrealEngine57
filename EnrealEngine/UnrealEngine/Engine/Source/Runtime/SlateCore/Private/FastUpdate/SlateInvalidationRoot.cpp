...
void FSlateInvalidationRoot::ProcessPreUpdate()
{
	...
	// We need to process the list first as the code below might recreate the list and lose the CurrentInvalidateReason
	WidgetsNeedingPreUpdate->ForEachIndexes([this](FSlateInvalidationWidgetHeapElement& Element)
	{
		const FSlateInvalidationWidgetList::InvalidationWidgetType& InvalidationWidget = (*FastWidgetPathList)[Element.GetWidgetIndex()];
		const SWidget* WidgetPtr = InvalidationWidget.GetWidget();
		const EInvalidateWidgetReason CurrentInvalidateReason = InvalidationWidget.CurrentInvalidateReason;
		if (WidgetPtr && EnumHasAnyFlags(CurrentInvalidateReason, EInvalidateWidgetReason::ChildOrder) )
		{
			// If the children have been modified in any way, the cached value of SupportsInvalidationRecursive might have changed.
			// We call SupportsInvalidationRecursive to recompute the value and invalidate the parent's value if needed
			const bool bUseCachedValue = false;
			bool bNowSupportsInvalidation = WidgetPtr->SupportsInvalidationRecursive(bUseCachedValue);
		}
	});
#endif

...
bool SWidget::SupportsInvalidation() const
{
	return bSupportsInvalidation;
}

UE_TRACE_EVENT_BEGIN(Cpu, SWidget_SupportsInvalidationRecursive, NoSync)
	UE_TRACE_EVENT_FIELD(UE::Trace::WideString, WidgetClassName)
	UE_TRACE_EVENT_FIELD(UE::Trace::WideString, CreatedInLocation)
	UE_TRACE_EVENT_FIELD(bool, UseCachedValue)
	UE_TRACE_EVENT_FIELD(bool, OldCanBeCached)
UE_TRACE_EVENT_END()

bool SWidget::SupportsInvalidationRecursive(bool bUseCachedValue) const
{
	if (!SupportsInvalidation())
	{
		CachedSupportsInvalidationRecursive = false;
		return false;
	}

	// If the cached value was true, then all children supported invalidation and the widget would have been invalidated if needed.
	// If the cached value was false, at least one of the descendant widget did not support invalidation and we therefore need to recheck everytime
	if (CachedSupportsInvalidationRecursive && bUseCachedValue)
	{
		return true;
	}

#if UE_SLATE_TRACE_ENABLED
	UE_TRACE_LOG_SCOPED_T(Cpu, SWidget_SupportsInvalidationRecursive, SlateChannel)
		<< SWidget_SupportsInvalidationRecursive.WidgetClassName(*TypeOfWidget.ToString())
		<< SWidget_SupportsInvalidationRecursive.CreatedInLocation(*CreatedInLocation.ToString())
		<< SWidget_SupportsInvalidationRecursive.UseCachedValue(bUseCachedValue)
		<< SWidget_SupportsInvalidationRecursive.OldCanBeCached(CachedSupportsInvalidationRecursive);
#endif // UE_SLATE_TRACE_ENABLED

	const bool OldCachedValue = CachedSupportsInvalidationRecursive;
	CachedSupportsInvalidationRecursive = true;

	const FChildren* Children = const_cast<SWidget*>(this)->GetAllChildren();
	for (int32 ChildIndex = 0; ChildIndex < Children->Num(); ++ChildIndex)
	{
		const TSharedRef<const SWidget> ChildWidget = Children->GetChildAt(ChildIndex);
		if (ChildWidget->ParentWidgetPtr != nullptr && !ChildWidget->SupportsInvalidationRecursive())
		{
			CachedSupportsInvalidationRecursive = false;
			break;
		}
	...
	// If the value has changed, we invalidate the parent
	if (CachedSupportsInvalidationRecursive != OldCachedValue)
	{
		if (const TSharedPtr<SWidget> ParentWidget = GetParentWidget())
		{
			if (!CachedSupportsInvalidationRecursive)
			{
				// If we supported invalidation but we do not anymore, tell the parent they do not support invalidation anymore, no need for them to check
				ParentWidget->SetSelfAndParentAsNotSupportingInvalidationRecursive();
			}
			else
			{
				// If we now support invalidation, ask the parent to recompute.
				const bool bParentUseCachedValue = false;
				bool bNowSupportsInvalidation = ParentWidget->SupportsInvalidationRecursive(bParentUseCachedValue);
			}
		}
	}

	return CachedSupportsInvalidationRecursive;
}

void SWidget::SetSelfAndParentAsNotSupportingInvalidationRecursive()
{
	// if this widget was supporting invalidation and it does not anymore, it needs to tell its parent.
	// Otherwise, its parent already knew so no need to do anything.
	if (CachedSupportsInvalidationRecursive)
	{
		CachedSupportsInvalidationRecursive = false;
		if (const TSharedPtr<SWidget> ParentWidget = GetParentWidget())

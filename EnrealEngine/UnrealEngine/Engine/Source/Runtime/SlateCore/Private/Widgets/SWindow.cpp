...
{
	return bAcceptsInput && Type != EWindowType::CursorDecorator
		&& (Type != EWindowType::ToolTip || !FSlateApplicationBase::Get().IsWindowHousingInteractiveTooltip(SharedThis(this)));
}
...
int32 SWindow::PaintWindow( double CurrentTime, float DeltaTime, FSlateWindowElementList& OutDrawElements, const FWidgetStyle& InWidgetStyle, bool bParentEnabled )
{
	...
	{
		const FGeometry& WindowGeometry = GetWindowGeometryInWindow();
		HittestGrid->DisplayNavigation(INT_MAX, WindowGeometry, OutDrawElements);
	}
#endif

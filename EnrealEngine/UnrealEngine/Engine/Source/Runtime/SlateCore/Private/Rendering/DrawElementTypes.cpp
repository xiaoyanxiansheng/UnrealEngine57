...
{
	if (InTint.A < UE_KINDA_SMALL_NUMBER || ShouldCull(ElementList, PaintGeometry))
	{
		return true;
	}
...
{
	if (InTint.A < UE_KINDA_SMALL_NUMBER || InText.Len() == 0 || ShouldCull(ElementList, PaintGeometry))
	{
		return true;
	}
...
{
	bool bIsFullyTransparent = InTint.A < UE_KINDA_SMALL_NUMBER && (InBrush->OutlineSettings.Color.GetSpecifiedColor().A < UE_KINDA_SMALL_NUMBER || InBrush->OutlineSettings.bUseBrushTransparency);
...
void FSlateDrawElement::MakeGeometryOutline(FSlateWindowElementList& ElementList, uint32 InLayer, const FPaintGeometry& PaintGeometry, FLinearColor Tint, ESlateDrawEffect InDrawEffects, bool bAntialias, float Thickness)
{
	PaintGeometry.CommitTransformsIfUsingLegacyConstructor();

	const FVector2f LocalSize = FVector2f(PaintGeometry.GetLocalSize());
	TArray<FVector2f> OutlinePoints =
	{
		FVector2f::ZeroVector,
		FVector2f(LocalSize.X, 0.f),
		FVector2f(LocalSize.X, LocalSize.Y),
		FVector2f(0.f, LocalSize.Y),
		FVector2f::ZeroVector
	};

	MakeLines(ElementList, InLayer, PaintGeometry, MoveTemp(OutlinePoints), InDrawEffects, Tint, bAntialias, Thickness);
}
...
void FSlateDrawElement::MakeText( FSlateWindowElementList& ElementList, uint32 InLayer, const FPaintGeometry& PaintGeometry, const FString& InText, const int32 InStartIndex, const int32 InEndIndex, const FSlateFontInfo& InFontInfo, ESlateDrawEffect InDrawEffects, const FLinearColor& InTint )
{
	const int32 SourceTextLen = InText.Len();
	const int32 StartIndex = FMath::Min<int32>(InStartIndex, SourceTextLen);
	const int32 EndIndex = FMath::Min<int32>(InEndIndex, SourceTextLen);
	const int32 TextLength = (EndIndex > StartIndex) ? EndIndex - StartIndex : 0;
	if (TextLength > 0)
	{
		MakeText(ElementList, InLayer, PaintGeometry, InText.Mid(StartIndex, TextLength), InFontInfo, InDrawEffects, InTint);
	}
}
...
void FSlateDrawElement::MakeShapedText(FSlateWindowElementList& ElementList, uint32 InLayer, const FPaintGeometry& PaintGeometry, const FShapedGlyphSequenceRef& InShapedGlyphSequence, ESlateDrawEffect InDrawEffects, const FLinearColor& BaseTint, const FLinearColor& OutlineTint, FTextOverflowArgs TextOverflowArgs)
{
	...
	if ((BaseTint.A < UE_KINDA_SMALL_NUMBER && InShapedGlyphSequence->GetFontOutlineSettings().OutlineSize == 0) ||
		(BaseTint.A < UE_KINDA_SMALL_NUMBER && OutlineTint.A < UE_KINDA_SMALL_NUMBER))
	{
		return;
	}

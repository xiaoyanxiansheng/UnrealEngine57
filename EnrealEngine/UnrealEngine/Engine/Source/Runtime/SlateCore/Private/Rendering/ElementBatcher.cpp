...
void FSlateElementBatcher::AddDebugQuadElement(const FSlateBoxElement& DrawElement)
{
	...
	FSlateRenderBatch& RenderBatch = CreateRenderBatch(Layer, FShaderParams(), nullptr, ESlateDrawPrimitive::TriangleList, ESlateShader::Default, InDrawEffects, ESlateBatchDrawFlag::None, DrawElement);
	...
	RenderBatch.AddIndex(0);
	RenderBatch.AddIndex(1);
	RenderBatch.AddIndex(2);

	RenderBatch.AddIndex(2);
	RenderBatch.AddIndex(1);
	RenderBatch.AddIndex(3);
}
...
void FSlateElementBatcher::AddBoxElements(const FSlateDrawElementArray<ElementType>& DrawElements)
{
	auto AddBoxElementInternal = [&](FSlateRenderBatch& RenderBatch, const ElementType& DrawElement)
	{
		...
		if(Resource != nullptr && !Resource->Debug_IsDestroyed())
		{
...
struct FLineBuilder
{
	...
	void BuildLineGeometry(const TArray<FVector2f>& Points, const TArray<FColor>& PackedColors, const FColor& PackedTint, ESlateVertexRounding Rounding)
	{
		...
		for (int32 Point = 1; Point < LastPointIndex; ++Point)
		{
			...
			PointColor = (Point < PackedColors.Num()) ? PackedColors[Point] : PackedTint;
			Position = NextPosition;
			NextPosition = Points[Point + 1];
		...
		PointColor = (LastPointIndex < PackedColors.Num()) ? PackedColors[LastPointIndex] : PackedTint;
...
void FSlateElementBatcher::AddViewportElement( const FSlateViewportElement& DrawElement )
{
	...
	if (ViewportResource != nullptr && ViewportResource->Debug_IsDestroyed())
	{
		UE_LOG(LogSlate, Error, TEXT("Adding a viewport element with an already destroyed viewport resource."));
		return;
	}

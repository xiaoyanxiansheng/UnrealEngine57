...
bool GSlateNavigationDebugging = false;
FAutoConsoleVariableRef CVarNavigationDebugging(
	TEXT("Slate.NavigationDebugging"),
	GSlateNavigationDebugging,
	TEXT("Whether to show a visualization of navigation from current widget"));

#endif

bool GSlateUseSharedBreakIterator = false;
static FAutoConsoleVariableRef CVarUseSharedBreakIterator(
	TEXT("Slate.UseSharedBreakIterator"),
	GSlateUseSharedBreakIterator,
	TEXT("If true, reuse cached break iterators to reduce memory overhead")
);

...
static bool GEnableNavigationExperimental = false;
static FAutoConsoleVariableRef CVarEnableNavigationExperimental(
	TEXT("Slate.Navigation.Experimental"),
	GEnableNavigationExperimental,
	TEXT("If true, will use experimental navigation system"));
...
const FVector2f FHittestGrid::CellSize(128.0f, 128.0f);
...
{
	if (GEnableNavigationExperimental)
	{
		return FindNextFocusableWidgetExperimental(StartingWidget, Direction, NavigationReply, RuleWidget, UserIndex);
	}
	return FindNextFocusableWidgetDefault(StartingWidget, Direction, NavigationReply, RuleWidget, UserIndex);
}


// TODO: NavigationMethodOrthogonal class deprecates the following and can be eventually removed. 
//		UE::Slate::Private, FindFocusableWidget, FindNextFocusableWidgetDefault
...
TSharedPtr<SWidget> FHittestGrid::FindNextFocusableWidgetDefault(const FArrangedWidget& StartingWidget, const EUINavigation Direction, const FNavigationReply& NavigationReply, const FArrangedWidget& RuleWidget, int32 UserIndex)
{
...
TSharedPtr<SWidget> FHittestGrid::FindNextFocusableWidgetExperimental(const FArrangedWidget& StartingWidget, const EUINavigation Direction, const FNavigationReply& NavigationReply, const FArrangedWidget& RuleWidget, int32 UserIndex)
{
	// Ensure that the hit test grid is valid before proceeding
	if (NumCells.X < 1 || NumCells.Y < 1)
	{
	...
	TSharedPtr<SWidget> NextFocusedWidget;
	TInstancedStruct<FNavigationMethod> NavigationMethod = GetNavigationMethod(StartingWidget);
	if (NavigationMethod.IsValid())
	{
		FNavigationMethod* NavigationMethodPtr = NavigationMethod.GetMutablePtr();

		NavigationMethodPtr->Initialize(this, IntermediateResultsPtr);
		NextFocusedWidget = NavigationMethodPtr->FindNextFocusableWidget(StartingWidget, Direction, NavigationReply, RuleWidget, UserIndex);
	}
	else
	{
		// Fallback to existing behavior
		NextFocusedWidget = FindNextFocusableWidgetDefault(StartingWidget, Direction, NavigationReply, RuleWidget, UserIndex);
	}

	FNavigationRoutingParams RoutingParams(StartingWidget.Widget, NextFocusedWidget, this, NavigationReply, UserIndex);
	NextFocusedWidget = FNavigationRouting::RouteNavigationFocus(RoutingParams);

#if WITH_SLATE_DEBUGGING
	if (IntermediateResultsPtr)
	{
		FDebuggingFindNextFocusableWidgetArgs Args {StartingWidget, Direction, NavigationReply, RuleWidget, UserIndex, NextFocusedWidget, MoveTemp(*IntermediateResultsPtr)};
		OnFindNextFocusableWidgetExecuted.Broadcast(this, Args);
	}
	
	if (GSlateNavigationDebugging && NavigationMethod.IsValid())
	{
		// Duplicate, as this pointer may outlive the navigation method associated with raw instanced struct pointer
		LastNavigationMethod = NavigationMethod;
	}

	NavigationMethod.Reset();
...
FSlateRect FHittestGrid::GetWidgetRenderBoundingRect(const TSharedPtr<SWidget>& Widget) const
{
	...
	FGeometry WidgetGeo = Widget->GetPaintSpaceGeometry();
	WidgetGeo.AppendTransform(FSlateLayoutTransform(-GridWindowOrigin));
	return WidgetGeo.GetRenderBoundingRect();
}
...
TInstancedStruct<FNavigationMethod> FHittestGrid::GetNavigationMethod(const FArrangedWidget& StartingWidget)
{
	TInstancedStruct<FNavigationMethod> NavigationMethodStruct;
	TSharedPtr<SWidget> Parent = StartingWidget.Widget;
	while (!NavigationMethodStruct.IsValid() && Parent != nullptr)
	{
		TSharedPtr<FNavigationMetaData> NavigationMetaData = Parent->GetMetaData<FNavigationMetaData>();
		if (NavigationMetaData.IsValid())
		{
			NavigationMethodStruct = NavigationMetaData->GetNavigationMethodStruct();
		}
		Parent = Parent->GetParentWidget();
	}

	return NavigationMethodStruct;
}
...
void FHittestGrid::DisplayNavigation(int32 InLayer, const FGeometry& AllottedGeometry, FSlateWindowElementList& WindowElementList)
{
	if (GSlateNavigationDebugging && LastNavigationMethod.IsValid())
	{
		FNavigationMethod* NavigationMethodPtr = LastNavigationMethod.GetMutablePtr();
		NavigationMethodPtr->DrawDebug(InLayer, AllottedGeometry, WindowElementList);
	}
}

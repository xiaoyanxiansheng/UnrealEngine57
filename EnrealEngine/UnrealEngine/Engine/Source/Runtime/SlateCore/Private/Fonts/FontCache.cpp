...
const FCharacterEntry* FCharacterList::CacheCharacter(TCHAR Character)
{
	...
	// Fake shape the character
	{
		...
		if (FaceGlyphData.FaceAndMemory.IsValid() && !FaceGlyphData.FaceAndMemory->IsFaceLoading() && FaceGlyphData.FaceAndMemory->IsFaceValid())
		{
			...
			const bool bIsWhitespace = FTextChar::IsWhitespace(Character);
			const uint32 GlyphIndex = FT_Get_Char_Index(FaceGlyphData.FaceAndMemory->GetFace(), Character);
			const uint32 FontRenderSize = FreeTypeUtils::ComputeFontPixelSize(FontInfo.Size, FinalFontScale);
			int16 XAdvance = 0;
			{
				FT_Fixed CachedAdvanceData = 0;
				TSharedRef<FFreeTypeAdvanceCache> AdvanceCache = FontCache.FTCacheDirectory->GetAdvanceCache(FaceGlyphData.FaceAndMemory->GetFace(), GlyphFlags, FontRenderSize);
			...
			if (CharEntry.Valid)
			{
				...
				if (ShapedGlyphFontAtlasData.Valid)
				{
					...
					CharEntry.KerningCache = FontCache.FontRenderer->GetKerningCache(*FontDataPtr, FontRenderSize);

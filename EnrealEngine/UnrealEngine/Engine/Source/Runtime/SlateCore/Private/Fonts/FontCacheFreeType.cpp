...
FT_Fixed DeterminePpemAndEmScale(const uint16 InEmSize, const uint32 InRenderSize)
{
	return DetermineEmScale(InEmSize, ConvertPixelTo26Dot6<FT_F26Dot6>(InRenderSize));
}
...
FFreeTypeGlyphCache::FFreeTypeGlyphCache(FT_Face InFace, const int32 InLoadFlags, const uint32 InFontRenderSize)
	: Face(InFace)
	, LoadFlags(InLoadFlags)
	, FontRenderSize(InFontRenderSize)
	, GlyphDataMap()
{
}
...
FFreeTypeAdvanceCache::FFreeTypeAdvanceCache(FT_Face InFace, const int32 InLoadFlags, const uint32 InFontRenderSize)
	: Face(InFace)
	, LoadFlags(InLoadFlags)
	, FontRenderSize((0 != (InLoadFlags & FT_LOAD_NO_SCALE))
			   ? static_cast<uint32>(FreeTypeUtils::DeterminePpemAndEmScale(InFace->units_per_EM, InFontRenderSize))
			   : InFontRenderSize) // compute once the EmScale and keep in FontSize when caching unscaled advances
	, AdvanceMap()
{
...
FFreeTypeKerningCache::FFreeTypeKerningCache(FT_Face InFace, const int32 InKerningFlags, const uint32 InFontRenderSize)
	: Face(InFace)
...
TSharedRef<FFreeTypeGlyphCache> FFreeTypeCacheDirectory::GetGlyphCache(FT_Face InFace, const int32 InLoadFlags, const uint32 InFontRenderSize)
{
	const FFontKey Key(InFace, InLoadFlags, InFontRenderSize);
	TSharedPtr<FFreeTypeGlyphCache>& Entry = GlyphCacheMap.FindOrAdd(Key);
	if (!Entry)
	{
		Entry = MakeShared<FFreeTypeGlyphCache>(InFace, InLoadFlags, InFontRenderSize);
	}
	return Entry.ToSharedRef();
}

TSharedRef<FFreeTypeAdvanceCache> FFreeTypeCacheDirectory::GetAdvanceCache(FT_Face InFace, const int32 InLoadFlags, const uint32 InFontRenderSize)
{
	...
	if (!Entry)
	{
		Entry = MakeShared<FFreeTypeAdvanceCache>(InFace, InLoadFlags, InFontRenderSize);
	}
	return Entry.ToSharedRef();
}

TSharedPtr<FFreeTypeKerningCache> FFreeTypeCacheDirectory::GetKerningCache(FT_Face InFace, const int32 InKerningFlags, const uint32 InFontRenderSize)
{
	...
	if (InFace && FT_HAS_KERNING(InFace))
	{
		...
		{
			const FFontKey Key(InFace, InKerningFlags, InFontRenderSize);
			TSharedPtr<FFreeTypeKerningCache>& Entry = KerningCacheMap.FindOrAdd(Key);
			if (!Entry)
			{
				Entry = MakeShared<FFreeTypeKerningCache>(InFace, InKerningFlags, InFontRenderSize);
			}
			return Entry;
		}
		checkNoEntry();

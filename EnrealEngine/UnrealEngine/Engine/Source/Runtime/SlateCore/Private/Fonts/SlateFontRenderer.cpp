...
void AppendGlyphFlags(const FFreeTypeFace& InFace, const FFontData& InFontData, uint32& InOutGlyphFlags)
{
	...
	if (EnableFontAntiAliasing)
	{
		...
		default:						InOutGlyphFlags |= FT_LOAD_TARGET_NORMAL; break; //-V616
		}
	}
	else
	{
...
int8 FSlateFontRenderer::GetKerning(const FFontData& InFontData, const float InSize, TCHAR First, TCHAR Second, const float InScale) const
{
	...
	if (FontFace && FT_HAS_KERNING(FontFace))
	{
		...
		const uint32 FontRenderSize = FreeTypeUtils::ComputeFontPixelSize(InSize, InScale);
		TSharedPtr<FFreeTypeKerningCache> KerningCache = GetKerningCache(InFontData, FontRenderSize);
		if (KerningCache)
		{
...
TSharedPtr<FFreeTypeKerningCache> FSlateFontRenderer::GetKerningCache(const FFontData& InFontData, const uint32 FontRenderSize) const
{
#if WITH_FREETYPE
	FT_Face FontFace = GetFontFace(InFontData);
	return FTCacheDirectory->GetKerningCache(FontFace, FT_KERNING_DEFAULT, FontRenderSize);
#else
	return nullptr;
#endif // WITH_FREETYPE
}
...
bool FSlateFontRenderer::GetRenderDataInternal(const FFreeTypeFaceGlyphData& InFaceGlyphData, const float InScale, const FFontOutlineSettings& InOutlineSettings, FCharacterRenderData& OutRenderData) const
{
	...
	if((ScaledOutlineSize > 0.0f || OutlineFontRenderMethod == 1) && Slot->format == FT_GLYPH_FORMAT_OUTLINE)
	{
		...
		FT_Error Error = FT_Render_Glyph(Slot, bIsMonochromeRendered ? FT_RENDER_MODE_MONO : FT_RENDER_MODE_NORMAL);
		if (Error != 0)
		{
			UE_LOG(LogSlate, Warning, TEXT("FreeType error %x while trying to render glyph index %u from face %s %s (from outline path)"), Error, Slot->glyph_index, UTF8_TO_TCHAR(Face->family_name), UTF8_TO_TCHAR(Face->style_name));
			return false;
		}
	}
	else
	{
		// This path renders a standard font with no outline.
		FT_Error Error = FT_Render_Glyph(Slot, bIsMonochromeRendered ? FT_RENDER_MODE_MONO : FT_RENDER_MODE_NORMAL);
		if (Error != 0)
		{
			UE_LOG(LogSlate, Warning, TEXT("FreeType error %x while trying to render glyph index %u from face %s %s (from standard path)"), Error, Slot->glyph_index, UTF8_TO_TCHAR(Face->family_name), UTF8_TO_TCHAR(Face->style_name));
			return false;
		}

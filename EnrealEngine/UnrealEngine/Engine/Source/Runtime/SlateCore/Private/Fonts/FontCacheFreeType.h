...
/**
* Determine the rounded Ppem and then from it determine the EmScale, in one call.
*/
FT_Fixed DeterminePpemAndEmScale(const uint16 InEmSize, const float InFontSize, const float InFontScale);
...
public:
#if WITH_FREETYPE
	FFreeTypeGlyphCache(FT_Face InFace, const int32 InLoadFlags, const uint32 InFontRenderSize);
...
public:
	...
	FFreeTypeAdvanceCache(FT_Face InFace, const int32 InLoadFlags, const uint32 InFontRenderSize);
...
public:
#if WITH_FREETYPE
	FFreeTypeKerningCache(FT_Face InFace, const int32 InKerningFlags, const uint32 InFontRenderSize);
...
public:
	 ...
	 */
	TSharedRef<FFreeTypeGlyphCache> GetGlyphCache(FT_Face InFace, const int32 InLoadFlags, const uint32 InFontRenderSize);

	/**
	 * Retrieve the advance cache for a given set of font parameters.
	 * @return A reference to the font advance cache.
	 */
	TSharedRef<FFreeTypeAdvanceCache> GetAdvanceCache(FT_Face InFace, const int32 InLoadFlags, const uint32 InFontRenderSize);
	 ...
	 */
	TSharedPtr<FFreeTypeKerningCache> GetKerningCache(FT_Face InFace, const int32 InKerningFlags, const uint32 InFontRenderSize);
#endif // WITH_FREETYPE
...
private:
	...
	{
	public:
		FFontKey(FT_Face InFace, const int32 InFlags, const uint32 InFontRenderSize)
			: Face(InFace)
			, Flags(InFlags)

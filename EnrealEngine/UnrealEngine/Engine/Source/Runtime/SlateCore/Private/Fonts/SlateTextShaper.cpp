...
DECLARE_CYCLE_STAT(TEXT("HarfBuzz Text Shaping"), STAT_PerformHarfBuzzTextShaping, STATGROUP_Slate);
DECLARE_CYCLE_STAT(TEXT("Kerning Only Text Shaping"), STAT_PerformKerningOnlyTextShaping, STATGROUP_Slate);
DECLARE_CYCLE_STAT(TEXT("Split By FontFace"), STAT_SplitByFontFace, STATGROUP_Slate);
DECLARE_CYCLE_STAT(TEXT("Split By Script"), STAT_SplitByScript, STATGROUP_Slate);
DECLARE_CYCLE_STAT(TEXT("Text Sequences Processing"), STAT_TextSequencesProcessing, STATGROUP_Slate);
DECLARE_CYCLE_STAT(TEXT("HarfBuzz Shape"), STAT_HarfBuzzNativeShape, STATGROUP_Slate);
...
{
	return FTextChar::IsWhitespace(InCodepoint)
...
FSlateTextShaper::FSlateTextShaper(FFreeTypeCacheDirectory* InFTCacheDirectory, FCompositeFontCache* InCompositeFontCache, FSlateFontRenderer* InFontRenderer, FSlateFontCache* InFontCache)
	...
	, GraphemeBreakIterator(GSlateUseSharedBreakIterator ? FBreakIterator::GetCharacterBoundaryIterator() : FBreakIterator::CreateCharacterBoundaryIterator())
#if WITH_HARFBUZZ
	, HarfBuzzFontCache(FTCacheDirectory)
#endif // WITH_HARFBUZZ
{
...
{
#if WITH_HARFBUZZ
	HarfBuzzFontCache.Flush();
#endif
}

FShapedGlyphSequenceRef FSlateTextShaper::ShapeBidirectionalText(const TCHAR* InText, const int32 InTextStart, const int32 InTextLen, const FSlateFontInfo& InFontInfo, const float InFontScale, const TextBiDi::ETextDirection InBaseDirection, const ETextShapingMethod TextShapingMethod)
{
...
FShapedGlyphSequenceRef FSlateTextShaper::ShapeUnidirectionalText(const TCHAR* InText, const int32 InTextStart, const int32 InTextLen, const FSlateFontInfo& InFontInfo, const float InFontScale, const TextBiDi::ETextDirection InTextDirection, const ETextShapingMethod TextShapingMethod)
{
...
void FSlateTextShaper::PerformTextShaping(const TCHAR* InText, const int32 InTextStart, const int32 InTextLen, const FSlateFontInfo& InFontInfo, const float InFontScale, const TextBiDi::ETextDirection InTextDirection, const ETextShapingMethod TextShapingMethod, TArray<FShapedGlyphEntry>& OutGlyphsToRender)
{
...
{
	SCOPE_CYCLE_COUNTER(STAT_PerformKerningOnlyTextShaping);
	...
	{
		SCOPE_CYCLE_COUNTER(STAT_SplitByFontFace);
	...
	{
		SCOPE_CYCLE_COUNTER(STAT_TextSequencesProcessing);
		...
		for (const FKerningOnlyTextSequenceEntry& KerningOnlyTextSequenceEntry : KerningOnlyTextSequence)
		{
			...
			const uint32 FontRenderSize = FreeTypeUtils::ComputeFontPixelSize(InFontInfo.Size, FinalFontScale);
			...
			TSharedPtr<FFreeTypeKerningCache> KerningCache = FTCacheDirectory->GetKerningCache(KerningOnlyTextSequenceEntry.FaceAndMemory->GetFace(), FT_KERNING_DEFAULT, FontRenderSize);
			TSharedRef<FFreeTypeAdvanceCache> AdvanceCache = FTCacheDirectory->GetAdvanceCache(KerningOnlyTextSequenceEntry.FaceAndMemory->GetFace(), GlyphFlags, FontRenderSize);
...
void FSlateTextShaper::PerformHarfBuzzTextShaping(const TCHAR* InText, const int32 InTextStart, const int32 InTextLen, const FSlateFontInfo& InFontInfo, const float InFontScale, const TextBiDi::ETextDirection InTextDirection, TArray<FShapedGlyphEntry>& OutGlyphsToRender)
{
	SCOPE_CYCLE_COUNTER(STAT_PerformHarfBuzzTextShaping);
	...
	TArray<FHarfBuzzTextSequenceEntry, TInlineAllocator<12>> HarfBuzzTextSequence;
	...
	// Step 2) Split the font face sections by their script code
	{
		SCOPE_CYCLE_COUNTER(STAT_SplitByScript);
	...
	// Step 3) Now we use HarfBuzz to shape each font data sequence using its FreeType glyph
	{
		...
		const bool bUseLetterSpacing = !bBypassLetterSpacing && InFontInfo.LetterSpacing != 0;
		...
		for (const FHarfBuzzTextSequenceEntry& HarfBuzzTextSequenceEntry : HarfBuzzTextSequence)
		{
...
#if WITH_FREETYPE
			const bool bHasKerning = FT_HAS_KERNING(HarfBuzzTextSequenceEntry.FaceAndMemory->GetFace()) != 0 || bUseLetterSpacing;
#else  // WITH_FREETYPE
			const bool bHasKerning = false;
#endif // WITH_FREETYPE

			// Letter spacing should scale proportional to font size / 1000 (to roughly mimic Photoshop tracking)
			const int32 LetterSpacingScaledAsInt = bUseLetterSpacing ? InFontInfo.LetterSpacing * InFontInfo.Size / 1000 : 0;
			ensure(LetterSpacingScaledAsInt <= std::numeric_limits<int16>::max());
			const int16 LetterSpacingScaled = bUseLetterSpacing ? (int16)(LetterSpacingScaledAsInt) : 0;
			...
			hb_font_t* HarfBuzzFont = HarfBuzzFontCache.CreateFont(*HarfBuzzTextSequenceEntry.FaceAndMemory, GlyphFlags, InFontInfo, FinalFontScale);
			if (!HarfBuzzFont)
			{
				continue;
			}
			...
			for (const FHarfBuzzTextSequenceEntry::FSubSequenceEntry& HarfBuzzTextSubSequenceEntry : HarfBuzzTextSequenceEntry.SubSequence)
			{
				...
				{
					SCOPE_CYCLE_COUNTER(STAT_HarfBuzzNativeShape);
...
bool FSlateTextShaper::InsertSubstituteGlyphs(const TCHAR* InText, const int32 InCharIndex, const TSharedRef<FShapedGlyphFaceData>& InShapedGlyphFaceData, const TSharedRef<FFreeTypeAdvanceCache>& AdvanceCache, TArray<FShapedGlyphEntry>& OutGlyphsToRender, const int16 InLetterSpacingScaled)
{
	auto GetSpecifiedGlyphIndexAndAdvance = [&InShapedGlyphFaceData, &AdvanceCache](TCHAR Char, uint32& OutSpaceGlyphIndex, int16& OutSpaceXAdvance)
	{

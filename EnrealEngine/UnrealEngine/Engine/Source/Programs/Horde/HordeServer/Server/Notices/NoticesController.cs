	...
	public class NoticesController : ControllerBase
	{
		...
		public async Task<List<GetNoticeResponse>> GetNoticesAsync(CancellationToken cancellationToken)
		{
			...
			for (int i = 0; i < notices.Count; i++)
			{
				...
				{
					startTime = TimeZoneInfo.ConvertTimeBySystemTimeZoneId(notice.StartTime.Value, "UTC");
				}
				DateTime? finishTime = null;
				if (notice.FinishTime != null)
				{
					finishTime = TimeZoneInfo.ConvertTimeBySystemTimeZoneId(notice.FinishTime.Value, "UTC");
				}
				bool active = _clock.UtcNow >= (startTime ?? DateTime.MinValue) && _clock.UtcNow < (finishTime ?? DateTime.MaxValue);

				messages.Add(new GetNoticeResponse() { Id = notice.Id.ToString(), Active = active, StartTime = startTime, FinishTime = finishTime, Message = notice.Message, CreatedByUser = userInfo });
			}

...
`streamTags` | [StreamTag](#streamtag)`[]`<br>Tags for the stream
...
`condition` | `string`<br>Condition for whether the schedule should be enabled. When a condition is provided the result of the expression will be logically AND with the  property to decide if the schedule should be enabled.
...
---- | -----------
`name` | `string`<br>Name of the tag
`enabled` | `boolean`<br>Whether the tag should be enabled

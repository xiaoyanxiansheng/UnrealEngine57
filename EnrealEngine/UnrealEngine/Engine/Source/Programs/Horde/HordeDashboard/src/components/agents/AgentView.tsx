...
const AgentMenuBar: React.FC<{ agentView?: boolean }> = observer(({ agentView }) => {
...
const PoolEditorModal: React.FC = observer(() => {
...
const PoolSelectionModal: React.FC = observer(() => {
...
const AgentViewInner: React.FC<{ agentId?: string, poolId?: string, searchParams?: URLSearchParams, agentView?: boolean }> = observer(({ agentId, poolId, searchParams, agentView }) => {
   ...
   function sortAgents(a: AgentData, b: AgentData) {
      ...
      switch (sortedCol.key) {
         ...
         case 'status':
            let [leftAgentStatus, leftAgentDate] = getAgentStatus(left);
            let [rightAgentStatus, rightAgentDate] = getAgentStatus(right);

            // sort by date if agents have date data and the same status
            if(leftAgentDate && rightAgentDate && (leftAgentStatus == rightAgentStatus)) {
               return leftAgentDate.getTime() - rightAgentDate.getTime();
            }
            return (leftAgentStatus ?? "").localeCompare(rightAgentStatus ?? "");
         case 'software':
   ...
   function getAgentStatus(agent: AgentData): [string, Date | undefined] {
      let agentStatus: string = '';
      let dateInfo: Date | undefined = undefined;

      // get status from agent's leases
      if (agent.leases && agent.leases.length > 0) {
         agent.leases.forEach(lease => {
      ...
      // if there are no leases, get status from other state
      if (agentStatus.length === 0) {
         ...
         if (!agent.online) {
            agentStatus = `Offline - ${agent.lastShutdownReason}`
            dateInfo = new Date(agent.updateTime);
         }
         else if (agent.pendingShutdown) {
            agentStatus = `Pending Shutdown ${agent.lastShutdownReason}`;
         }
         else if (agent.pendingConform || agent.pendingFullConform) {
            if (agent.conformAttemptCount && agent.conformAttemptCount > 0) {
               agentStatus = `${agent.pendingFullConform ? "Full " : ""}Conform failed (${agent.conformAttemptCount} attempts)`;
               if (agent.nextConformTime) {
                  dateInfo = new Date(agent.nextConformTime);
               }
            }
            else {
               agentStatus = "Pending Conform";
   ...
   return (<Stack>
      ...
      <ConfirmationDialog
         ...
         confirmText={"Disable"}
         cancelText={"Cancel"}
         textBoxLabel={"Enter Disable Reason - Required"}
         isTextBoxSpawned={true}
         isTextRequired={true}

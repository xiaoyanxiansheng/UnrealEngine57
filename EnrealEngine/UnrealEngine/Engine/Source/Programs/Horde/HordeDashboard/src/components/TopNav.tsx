...
export const TopNav: React.FC<{ suppressServer?: boolean }> = observer(({ suppressServer }) => {
   ...
   const generateToolsMenu = () => {
      ...
      const plugins = getHordePlugins().filter(p => p.mount?.type === MountType.TopNav || p.mounts?.find(m => m.type === MountType.TopNav));

      // Add plugin topnav mount points
      plugins.forEach(p => {
         if (p.mount) {
            serviceItems.push({
         ...
         p.mounts?.forEach( m => {
            if (m.type !== MountType.TopNav) {
               return;
            }

            serviceItems.push({
               key: `admin_plugin_${m.text}`,
               text: m.text,
               link: m.route
            });      
         })
   ...
   const generateHelpMenu = () => {
      ...
      if(dashboard.helpEmail || dashboard.helpSlack){
         hordeItems.push({
   ...
   return (
      <div style={{ backgroundColor: hordeTheme.horde.topNavBackground }}>
         ...
         {showSupport && <SupportModal show={true} onClose={() => {setShowSupport(false) }}/>}

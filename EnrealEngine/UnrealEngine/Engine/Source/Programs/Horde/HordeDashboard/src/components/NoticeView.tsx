...
const NoticePanel: React.FC = observer(() => {
   ...
   const renderItem = (item: GetNoticeResponse, index?: number, column?: IColumn) => {
      ...
      if (column?.name === "Message") {
         ...
         if (item.createdByUser && !!dashboard.user?.dashboardFeatures?.showNoticeEditor) {
      ...
      <Stack style={{ paddingBottom: 12 }}>
         <Stack verticalAlign="center">
            <Stack horizontalAlign="end">
               { !!dashboard.user?.dashboardFeatures?.showNoticeEditor &&<PrimaryButton styles={{ root: { fontFamily: "Horde Open Sans SemiBold" } }} onClick={() => setState({ showEditor: true, editNotice: userNotice })}>Set Notice</PrimaryButton> }
            </Stack>

...
export type FilterStatus = "Running" | "Complete" | "Succeeded" | "Failed" | "Cancelled" | "Waiting";
...
export class JobHandler {
    ...
    async update() {
        ...
        try {
            ...
            let filter = "id,streamId,name,change,preflightChange,templateId,templateHash,graphHash,startedByUserInfo,abortedByUserInfo,createTime,state,arguments,updateTime,labels,defaultLabel,preflightDescription,cancellationReason";
    ...
    getFilteredJobs(status?: FilterStatus[], minCL?: number, maxCL?: number) {
        if ((!status || !status.length) && !minCL && !maxCL) {
            return this.jobs;
        }

        return this.jobs.filter(job => {
            if((minCL || maxCL) && !job.change) return false;
            
            let withinBounds = true;
            if((minCL && job.change! < minCL) || (maxCL && job.change! > maxCL)) withinBounds = false;
            if((minCL || maxCL) && !withinBounds) return false;
            ...
            return status.some(status => {
                ...
                if(status === "Cancelled" && !job.abortedByUserInfo) {
                    return false;
                }

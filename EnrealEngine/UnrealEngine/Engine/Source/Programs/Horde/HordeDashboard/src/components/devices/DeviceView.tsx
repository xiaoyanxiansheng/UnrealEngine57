...
import { DashboardPreference, DevicePoolType, GetDeviceResponse } from "../../backend/Api";
...
const SearchUpdate: React.FC = observer(() => {
...
const DevicePanel: React.FC = observer(() => {
   ...
   const successColorPref = dashboard.getPreference(DashboardPreference.ColorSuccess);
   const runningColorPref = dashboard.getPreference(DashboardPreference.ColorRunning);

   const StatusColors = new Map<DeviceStatus, string>([
      [DeviceStatus.Available, successColorPref ? successColorPref : "#52C705"],
      [DeviceStatus.Problem, errorColorPref ? errorColorPref : "#DE4522"],
      [DeviceStatus.Reserved, warningColorPref ? warningColorPref : "#00BCF2"],
      [DeviceStatus.Disabled, dashboard.darktheme ? "#D3D2D1" : "#F3F2F1"],
      [DeviceStatus.Maintenance, runningColorPref ? runningColorPref : "#0078D4"]
   ]);
   ...
   column.onRender = ((item: DeviceItem, index, column) => {
      ...
      let color = status === DeviceStatus.Disabled ? "#777777 !important" : isBright(backgroundColor) ? "#2D3F5F" : "#FFFFFF";
      const hoveredPressedStyle = { backgroundColor: backgroundColor, border: '0px', filter: isBright(backgroundColor) ? "brightness(0.9)" : "brightness(1.2)" }
      ...
      return <Stack verticalFill={true} verticalAlign="center" horizontal>
         <PrimaryButton 
            styles={{ root: { border: "0px", width: 84, height: 24, fontSize: 12, fontFamily: "Horde Open Sans SemiBold !important", selectors: { ".ms-Button-label": { color: color } }, backgroundColor: backgroundColor }, rootHovered: hoveredPressedStyle, rootPressed: hoveredPressedStyle }}
            text={text}
            onClick={(ev) => {
   ...
   column.onRender = ((item: DeviceItem, index, column) => {
      ...
      if (!automationTab) {
         ...
         if (user.id === dashboard.userId) {
            return <Stack verticalFill={true} verticalAlign="center">
               <PrimaryButton styles={{ root: { border: "0px", width: 90, height: 24, fontSize: 12, fontFamily: "Horde Open Sans SemiBold !important" } }} text="Check In"
                  onClick={async (ev) => {
                     ev.stopPropagation(); ev.preventDefault();
...
const CheckoutConfirmModal: React.FC<{ check: "in" | "out" | "error", devices: GetDeviceResponse[], onClose: () => void }> = ({ check, devices, onClose }) => {
   ...
   return <Modal isOpen={true} topOffsetFixed={true} styles={{ main: { padding: 8, width: 840, hasBeenOpened: false, top: "80px", position: "absolute" } }} className={hordeClasses.modal} onDismiss={() => { close() }}>

...
export const ChangeContextMenu: React.FC<{ target: ChangeContextMenuTarget, onDismiss?: () => void, job?: JobParameters, stepRef?: GetJobStepRefResponse, commit?: GetChangeSummaryResponse, rangeCL?: number }> = ({ target, onDismiss, job, stepRef, commit, rangeCL }) => {
   ...
   // Should only show cancel text if not in an incremental view where "commit" exists
   if(!commit && job.abortedByUserInfo && job.cancellationReason) { 
      menuItems.push({
         key: "cancelled",
         onRender: () => {
            return (
               <Stack style={{ padding: 18, paddingTop: job.preflightDescription ? 0 : 18, paddingBottom: 18, maxWidth: 800 }} horizontal tokens={{ childrenGap: 12 }}>
                  <Text variant="small" style={{ fontFamily: "Horde Open Sans Bold" }}>Cancellation Reason:</Text>
                  <Text variant="small" >{job.cancellationReason}</Text>
               </Stack>
            )
         }
      });
   ...
   if(job.cancellationReason || commit || job.preflightDescription){
      menuItems.push({
         key: "commit_divider",
...
export const ChangeButton: React.FC<{ job?: JobParameters, stepRef?: GetJobStepRefResponse, commit?: GetChangeSummaryResponse, hideAborted?: boolean, rangeCL?: number, pinned?: boolean, prefix?: string, buttonColor?: string }> = ({ job, stepRef, commit, hideAborted, rangeCL, pinned, prefix, buttonColor }) => {
   ...
   return (<Stack verticalAlign="center" verticalFill={true} horizontalAlign="start"> <div style={{ paddingBottom: "1px" }}>
      <Stack tokens={{ childrenGap: 4 }}>
         <span 
            ref={spanRef} 
            style={{ padding: "2px 6px 2px 6px", height: "15px", cursor: "pointer", color: "#FFFFFF", backgroundColor: buttonColor ? `${buttonColor}` : defaultBackgroundColor }} 
            className={job.startedByUserInfo ? "cl-callout-button-user" : "cl-callout-button"} 
            onClick={(ev) => { ev.stopPropagation(); ev.preventDefault(); setMenuShown(!menuShown) }}
         >
            {change}
         </span>
         {(!!showStatus) && 
            <span 
               ref={spanRef} 
               style={{ padding: "2px 6px 2px 6px", height: "16px", cursor: "pointer", userSelect: "none", fontFamily: "Horde Open Sans SemiBold", fontSize: "10px", backgroundColor: color, color: "rgb(255, 255, 255)" }} 
               className={job.startedByUserInfo ? "cl-callout-button-user" : "cl-callout-button"} 
               onClick={(ev) => { ev.stopPropagation(); ev.preventDefault(); setMenuShown(!menuShown) }}
            >
               {text}

...
const StreamViewInner: React.FC = observer(() => {
   ...
   const searchInURL: boolean = searchParams.has("search");
   const [findJobsShown, setFindJobsShown] = useState(searchInURL);
   ...
   // Respond to search param even when navigating between search queries within the same route
   // One consequence being pressing the browser's back button can sometimes close the search modal depending on search state
   useEffect(() => {
      setFindJobsShown(searchParams.has("search"));
   }, [location.search])
   ...
   return (
      <Stack className={hordeClasses.horde}>
         ...
         {findJobsShown && <JobSearchSimpleModal onClose={() => { setFindJobsShown(false) }} streamIdIn={stream.id} jobTab={currentTab} autoloadIn={searchInURL} />}

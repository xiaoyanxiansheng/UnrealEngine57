...
import { CompactPeoplePicker, IBasePickerSuggestionsProps, IInputProps, IPersonaProps, Label, Stack } from "@fluentui/react";
...
export const UserSelect: React.FC<{ handleSelection: (userId: string | undefined) => void, userIdHints?: string[], noResultsFoundText?: string, defaultUser?: GetUserResponse, autoFocus?: boolean, defaultId?: string }> = observer(({ handleSelection, userIdHints, noResultsFoundText, defaultUser, autoFocus, defaultId }) => {
   const [selectedItems, setSelectedItems] = useState<IPersonaProps[]>([]);

   const loadUser = (user: GetUserResponse) => {
      if(!user) return;
      ...
      const persona = {
         text: user.name,
         imageUrl: user.image24,
         optionalText: user.id,
      } as IPersonaProps;

      setSelectedItems([persona]);
   ...
   const loadUserFromId = async (userId: string) => {
      ...
      const users = await backend.getUsers({ids: [userId], includeAvatar: true});
      if(users.length) loadUser(users[0]);
   }
   ...
   useEffect(() => {
      if(defaultUser) {
         ...
         loadUserFromId(defaultId);
      } else {
         setSelectedItems([])
         handleSelection(undefined);
      }
   }, [defaultUser, defaultId]);

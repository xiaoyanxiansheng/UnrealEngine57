...
export class Dashboard {
    ...
    get hordeUser(): string | undefined {
        const claims = this.claims;
        const user = claims.filter(c => c.type.endsWith("/horde/user"));
        if (!user.length) {
            return undefined;
        }

        return user[0].value?.length ? user[0].value : undefined;
    }

    get hordeAdmin():boolean {        
        return !!this.claims?.find(c => c.type === "groups" && c.value === "app-horde-admins");
    }

...
const StatusBar = (stack: StatusBarStack[], width: number, height: number, basecolor?: string, style?: any): JSX.Element => {
...
const PoolList: React.FC = observer(() => {
   ...
   const filtered = handler.pools.filter(p => {
      ...
      // filter out anonymous pools from display
      if (!p.name) {
         return false;
      }

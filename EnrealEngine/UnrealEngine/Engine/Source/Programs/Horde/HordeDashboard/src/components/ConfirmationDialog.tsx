...
export const ConfirmationDialog: React.FC<{ isOpen: boolean; title: string; cancelText: string; confirmText: string; subText?: string; isTextBoxSpawned?: boolean; textBoxLabel?: string; isTextRequired?: boolean, checkBoxText?: string; onConfirm: (...args: any[]) => any; onCancel: (...args: any[]) => any }> = (({ isOpen, title, cancelText, confirmText, subText, isTextBoxSpawned, textBoxLabel, isTextRequired, checkBoxText, onConfirm, onCancel }) => {
   ...
   return (
      ...
      >
         ...
         <DialogFooter>
            <PrimaryButton disabled={isTextBoxSpawned && isTextRequired && !textFieldText?.length} onClick={() => { onConfirm(textFieldText,checkboxRef?.current?.checked ); setTextFieldText(undefined); }} text={confirmText} />

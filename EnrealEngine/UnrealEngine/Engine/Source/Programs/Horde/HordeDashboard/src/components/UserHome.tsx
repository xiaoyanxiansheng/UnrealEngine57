                  ...
                  return (
                     ...
                     >
                        <Link onClick={() => { projectStore.setActive(project.id); }} to={`/project/${project.id}`}>
                           <img src={logoSrc} alt={project.name + " logo button"} width={560 * .65} height={280 * .65} />
                        </Link>
                     </Stack>
                  );
               })
...
class UserJobsHandler {
   ...
   async update() {
      ...
      try {
         let filter = "id,streamId,name,change,preflightChange,templateId,templateHash,graphHash,startedByUserInfo,createTime,state,arguments,updateTime,labels,defaultLabel,batches,autoSubmit,autoSubmitChange,preflightDescription,abortedByUserInfo,cancellationReason";

...
import { ColorPicker, ComboBox, DefaultButton, DialogFooter, IColorPickerStyles, IComboBoxOption, Label, MessageBar, MessageBarType, Modal, Pivot, PivotItem, PrimaryButton, Spinner, SpinnerSize, Stack, Text, TextField, Toggle } from '@fluentui/react';
import { observer } from 'mobx-react-lite';
import React, { useState } from 'react';
import backend from '../backend';
import { AuthMethod, DashboardPreference, DownloadFormat, UpdateCurrentAccountRequest } from '../backend/Api';
...
const GeneralPanel: React.FC = observer(() => {
   ...
   const downloadOptions: IComboBoxOption[] = [
      { key: `key_download_option_zip`, text: "Zip", data: DownloadFormat.Zip },
      { key: `key_download_option_ugs`, text: "Unreal Game Sync", data: DownloadFormat.UGS },
      { key: `key_download_option_toolbox`, text: "Unreal Toolbox", data: DownloadFormat.UTOOLBOX }
   ];

   let defaultDownloadKey = "key_download_option_zip";
   if (dashboard.downloadFormat === DownloadFormat.UGS) {
      defaultDownloadKey = "key_download_option_ugs";
   } else if (dashboard.downloadFormat === DownloadFormat.UTOOLBOX) {
      defaultDownloadKey = "key_download_option_toolbox";
   }
      ...
      <Stack styles={{ root: { paddingTop: 18, paddingLeft: 12, paddingRight: 12, width: "100%" } }} >
         <Stack tokens={{ childrenGap: 12 }} style={{ height: 'calc(100vh - 200px)' }}>
            ...
            {pivotKey === "Settings" && <Stack style={{ paddingLeft: 12 }} tokens={{ childrenGap: 12 }}>
               ...
               <Stack style={{ paddingLeft: 8 }} tokens={{ childrenGap: 12 }}>
                  <ComboBox style={{ width: 218 }} label="Download Format" defaultSelectedKey={defaultDownloadKey} options={downloadOptions} autoComplete="off" onChange={(ev, option, index, value) => {
                     if (option) {
                        dashboard.setDownloadFormat(option?.data);
                     }                     
                   }} />
               </Stack>
               <Stack style={{ paddingLeft: 8 }} tokens={{ childrenGap: 12 }}>

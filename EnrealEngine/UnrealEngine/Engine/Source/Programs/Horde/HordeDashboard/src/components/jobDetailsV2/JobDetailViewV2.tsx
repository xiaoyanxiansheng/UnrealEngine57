...
import { ErrorView } from "horde/components/error/ErrorView.tsx";
...
const DetailsViewErrorView: React.FC<{ jobDetails: JobDetailsV2 }> = observer(({ jobDetails }) => {

   jobDetails.subscribe();

   if (jobDetails.jobError) {
      return <ErrorView title="Unable to load job" description="Verify job and stream exists and potentially your internet connection." reason={jobDetails.jobError} />;
   }
})


/** This is a root view and should not be made an observer other than for a specific observable, ie. not JobDetails updates as these are very coarse and cause issues */
const DetailsView: React.FC<{ jobDetails: JobDetailsV2 }> = ({ jobDetails }) => {
   ...
   return <Stack id="rail_job_overview">
      <DetailsViewErrorView jobDetails={jobDetails}/>

...
namespace HordeAgent.Leases
{
	...
	sealed class LeaseLoggerFactory : IAsyncDisposable
	{
		...
		/// Constructor
		/// </summary>
		public LeaseLoggerFactory(IOptionsMonitor<AgentSettings> settings, ILogger<LeaseLoggerFactory> logger)
		{
			_settings = settings;
			_logDir = DirectoryReference.Combine(settings.CurrentValue.LogsDir, "Leases");
			_logger = logger;
		...
		public ILoggerFactory CreateLoggerFactory(LeaseId leaseId)
		{
			return LoggerFactory.Create(builder =>
			{
				...
				if (_settings.CurrentValue.WriteStepOutputToLogger)
				{

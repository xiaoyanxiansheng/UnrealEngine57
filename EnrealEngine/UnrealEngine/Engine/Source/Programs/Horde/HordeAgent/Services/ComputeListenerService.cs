...
namespace HordeAgent.Services
{
	...
	class ComputeListenerService : IHostedService, IDisposable
	{
		...
		/// Constructor
		/// </summary>
		public ComputeListenerService(IOptionsMonitor<AgentSettings> settings, ILogger<ComputeListenerService> logger)
		{
			_settings = settings;
			_logger = logger;
		}
		...
		{
			if (_settings.CurrentValue.ComputePort != 0)
			{
				Start(IPAddress.Any, _settings.CurrentValue.ComputePort);
			}

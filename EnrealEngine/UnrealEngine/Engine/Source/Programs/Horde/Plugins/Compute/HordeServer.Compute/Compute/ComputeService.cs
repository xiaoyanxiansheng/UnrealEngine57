...
namespace HordeServer.Compute
{
	...
	/// <summary>
	/// Internal parameters for allocating a resource
	/// </summary>
	public class AllocateResourceParams
	{
		...
		/// <summary>
		/// User ID allocating the resource
		/// </summary>
		...
		/// <inheritdoc cref="AssignComputeResponse.Uba" />
		public bool? UseUbaCache { get; init; }

		/// <summary>
		/// Constructor
		/// </summary>
		public AllocateResourceParams(ClusterId clusterId, ClaimsPrincipal user, ComputeProtocol protocol, Requirements? requirements = null)
		{
			ClusterId = clusterId;
			User = user;
			Protocol = protocol;
	...
	public sealed class ComputeService : IHostedService, IAsyncDisposable
	{
		...
		public async Task<ComputeResource> TryAllocateResourceAsync(AllocateResourceParams arp, CancellationToken cancellationToken)
		{
			...
			span.SetAttribute("useUbaCache", arp.UseUbaCache ?? false);
			...
			Task<UbaComputeResource>? ubaCache = null;
			if (arp.UseUbaCache ?? false)
			{
				// Allocate UBA cache session key in the background, await the task later
				ubaCache = AllocateUbaCacheServerAsync(arp.User, clusterConfig, 120, cancellationToken);
			}
		...
		internal static bool VerifyUbaCache(ClaimsPrincipal user, ComputeClusterConfig clusterConfig)
		{
			if (clusterConfig.Uba?.CacheEndpoint == null || clusterConfig.Uba.CacheHttpEndpoint == null || clusterConfig.Uba.CacheHttpCrypto == null)
			{
				throw new ComputeServiceException($"No UBA cache server defined for compute cluster {clusterConfig.Id}") { ShowToUser = true };
			}
			
			bool hasReadAccess = clusterConfig.Authorize(ComputeAclAction.UbaCacheRead, user);
			bool hasWriteAccess = clusterConfig.Authorize(ComputeAclAction.UbaCacheWrite, user);
			bool hasAccess = hasReadAccess || hasWriteAccess;
			if (!hasAccess)
			{
				throw new ComputeServiceException($"User does not have UBA cache server access (check {ComputeAclAction.UbaCacheRead} or {ComputeAclAction.UbaCacheWrite})") { ShowToUser = true };
			}
		...
		/// <summary>
		/// Allocate a UBA cache server session
		/// </summary>
		/// <param name="user">Requesting user</param>
		/// <param name="clusterConfig">Compute cluster</param>
		/// <param name="ttl">Time-to-live in seconds for the session</param>
		/// <param name="cancellationToken">Cancellation token</param>
		/// <param name="httpClient">Optional HTTP client override for tests</param>
		/// <returns>Settings for a UBA cache server session</returns>
		public async Task<UbaComputeResource> AllocateUbaCacheServerAsync(
			ClaimsPrincipal user, ComputeClusterConfig clusterConfig, int ttl, CancellationToken cancellationToken, HttpClient? httpClient = null)
		{
			string? cacheEndpoint = clusterConfig.Uba?.CacheEndpoint;
			string? httpEndpoint = clusterConfig.Uba?.CacheHttpEndpoint;
			string? httpCrypto = clusterConfig.Uba?.CacheHttpCrypto;
			if (cacheEndpoint == null || httpEndpoint == null || httpCrypto == null)
			{
				throw new ComputeServiceException($"No UBA cache server defined for compute cluster {clusterConfig.Id}") { ShowToUser = true };
			}
			...
			try
			{
				...
				// Generate a session encryption key that we activate in the UBA cache server
				byte[] sessionKey = RandomNumberGenerator.GetBytes(16);
				string sessionKeyHex = Convert.ToHexString(sessionKey);
				byte[] cryptoKey = Convert.FromHexString(httpCrypto);
				if (cryptoKey.Length != 16)
				{
					throw new Exception($"Invalid crypto key length: {cryptoKey.Length}");
				}
				
				using UbaCrypto crypto = new(cryptoKey);
				string encryptedSessionKeyHex = Convert.ToHexString(crypto.Encrypt(sessionKey));
				string readOnlyStr = hasWriteAccess ? "false" : "true";
				Uri uri = new ($"http://{httpEndpoint}/addsession?crypto={encryptedSessionKeyHex},readonly={readOnlyStr},expiration={ttl}");
				HttpResponseMessage res = await (httpClient ?? _httpClient).GetAsync(uri, timeoutCts.Token);
				res.EnsureSuccessStatusCode();

				return new UbaComputeResource(cacheEndpoint, sessionKeyHex, hasWriteAccess);
			}
			catch (TaskCanceledException ex)
			{
				throw new ComputeServiceException("UBA cache server allocation timed out or was cancelled", ex);
			}
			catch (Exception ex)
			{
				throw new ComputeServiceException("Unexpected error during UBA cache server allocation", ex);
			}
		}

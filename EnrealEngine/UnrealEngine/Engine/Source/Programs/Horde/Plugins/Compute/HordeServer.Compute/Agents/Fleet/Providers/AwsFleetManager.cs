...
namespace HordeServer.Agents.Fleet.Providers
{
	...
	{
		/// <summary>
		/// EC2 tag prefix signaling an instance belong to agent pool, through pool name
		/// Deprecated in favor of <see cref="AwsTagPoolId" /> as pool ID is the stable reference to a pool. 
		/// </summary>
		public const string AwsTagPoolName = "Horde_Autoscale_Pool";
		
		/// <summary>
		/// EC2 tag prefix signaling an instance belong to agent pool, through pool ID
		/// </summary>
		public const string AwsTagPoolId = "Horde_Autoscale_PoolId";
		...
		public static async Task ShrinkPoolViaAgentShutdownRequestAsync(IAgentCollection agentCollection, IPool pool, IReadOnlyList<IAgent> agents, int count, CancellationToken cancellationToken)
		{
			...
			string awsTagPoolNameProperty = $"{AwsTagPropertyName}={AwsTagPoolName}:{pool.Name}";
			string awsTagPoolIdProperty = $"{AwsTagPropertyName}={AwsTagPoolId}:{pool.Id.ToString()}";

			// Sort the agents by number of active leases. It's better to shut down agents currently doing nothing.
			List<IAgent> filteredAgents = agents.OrderBy(x => x.Leases.Count).ToList();
			List<IAgent> agentsWithAwsTags = filteredAgents.Where(x => x.HasProperty(awsTagPoolNameProperty) || x.HasProperty(awsTagPoolIdProperty)).ToList();
		...
		{
			return (await GetInstancesInPoolAsync(_ec2, InstanceStateName.Stopped, pool, _tracer, cancellationToken)).Count;
		}
		
		/// <summary>
		/// Get instances that are part of given Horde agent pool
		/// </summary>
		/// <param name="ec2">EC2 client</param>
		/// <param name="instanceState">Optional instance state to filter on</param>
		/// <param name="pool">Pool to filter on</param>
		/// <param name="tracer">Tracer</param>
		/// <param name="cancellationToken">Cancellation token</param>
		/// <returns>A list of EC2 instances</returns>
		public static Task<List<Instance>> GetInstancesInPoolAsync(IAmazonEC2 ec2, InstanceStateName? instanceState, IPoolConfig pool, Tracer tracer, CancellationToken cancellationToken)
		{
			return GetInstancesInPoolAsync(ec2, instanceState, pool.Id.ToString(), pool.Name, tracer, cancellationToken);
		}
		...
		/// <param name="ec2">EC2 client</param>
		/// <param name="instanceState">Optional instance state to filter on</param>
		/// <param name="poolId">Pool ID to filter on</param>
		/// <param name="poolName">Pool ID to filter on</param>
		/// <param name="tracer">Tracer</param>
		/// <param name="cancellationToken">Cancellation token</param>
		/// <returns>A list of EC2 instances</returns>
		public static async Task<List<Instance>> GetInstancesInPoolAsync(IAmazonEC2 ec2, InstanceStateName? instanceState, string poolId, string poolName, Tracer tracer, CancellationToken cancellationToken)
		{
			using TelemetrySpan span = tracer.StartActiveSpan($"{nameof(AwsFleetManager)}.{nameof(GetInstancesInPoolAsync)}");
			
			List<Filter> poolIdFilters = [new Filter($"tag:{AwsTagPoolId}", [poolId])];
			List<Filter> poolNameFilters = [new Filter($"tag:{AwsTagPoolName}", [poolName])];
			if (instanceState != null)
			{
				poolIdFilters.Add(new Filter("instance-state-name", [instanceState.Value]));
				poolNameFilters.Add(new Filter("instance-state-name", [instanceState.Value]));
			}
			
			List<Instance> result = [];
			{
				// Get matching instances by pool *name*
				using TelemetrySpan ec2Span = tracer.StartActiveSpan($"DescribeInstances");
				DescribeInstancesRequest request = new() { Filters = poolNameFilters };
				DescribeInstancesResponse response = await ec2.DescribeInstancesAsync(request, cancellationToken);
				result.AddRange(response.Reservations.SelectMany(x => x.Instances));
				ec2Span.SetAttribute("res.statusCode", (int)response.HttpStatusCode);
				ec2Span.SetAttribute("res.numReservations", response.Reservations.Count);
			}
			{
				// Get matching instances by pool *ID*
				using TelemetrySpan ec2Span = tracer.StartActiveSpan($"DescribeInstances");
				DescribeInstancesRequest request = new() { Filters = poolIdFilters };
				DescribeInstancesResponse response = await ec2.DescribeInstancesAsync(request, cancellationToken);
				result.AddRange(response.Reservations.SelectMany(x => x.Instances));
				ec2Span.SetAttribute("res.statusCode", (int)response.HttpStatusCode);
				ec2Span.SetAttribute("res.numReservations", response.Reservations.Count);
			...
			return result.DistinctBy(x => x.InstanceId).ToList();
		}
	}
}

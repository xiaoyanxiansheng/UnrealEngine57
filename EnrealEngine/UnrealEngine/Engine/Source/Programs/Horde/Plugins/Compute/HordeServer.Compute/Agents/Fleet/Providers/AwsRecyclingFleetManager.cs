...
public sealed class AwsRecyclingFleetManager : IFleetManager
{
	...
	private async Task StopStuckPendingInstancesAsync(IPool pool, CancellationToken cancellationToken)
	{
		...
		List<Instance> stuckInstances = await AwsFleetManager.GetInstancesInPoolAsync(_ec2, InstanceStateName.Pending, pool, _tracer, cancellationToken);
		List<string> stuckInstanceIds = stuckInstances
	...
	private async Task<Dictionary<string, List<Instance>>> GetCandidateInstancesAsync(IPool pool, CancellationToken cancellationToken)
	{
		...
		List<Instance> instances = await AwsFleetManager.GetInstancesInPoolAsync(_ec2, InstanceStateName.Stopped, pool, _tracer, cancellationToken);

		// Group instance objects by availability zone
		List<IGrouping<string, Instance>> azInstancePairs = instances.GroupBy(x => x.Placement.AvailabilityZone).ToList();

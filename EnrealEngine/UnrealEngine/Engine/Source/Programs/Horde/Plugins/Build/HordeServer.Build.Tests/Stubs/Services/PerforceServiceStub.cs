...
namespace HordeServer.Tests.Stubs.Services
{
	class PerforceServiceStub : IPerforceService
	{
		...
		public Task<List<ICommit>> GetChangeDetailsAsync(StreamConfig stream, IReadOnlyList<int> changeNumbers, CancellationToken cancellationToken)
		{
			...
			{
				if(!Changes.ContainsKey(stream.Id) || !Changes[stream.Id].ContainsKey(changeNumber))
				{
					throw new PerforceException($"PerforceServiceStub is simulating a changelist not found exception {changeNumber}");
				}

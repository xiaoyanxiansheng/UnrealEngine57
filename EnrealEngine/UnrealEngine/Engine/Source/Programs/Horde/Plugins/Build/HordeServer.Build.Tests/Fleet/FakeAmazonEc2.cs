...
public class FakeAmazonEc2
{
	...
	public Instance AddInstance(
		InstanceState state,
		InstanceType type,
		string availabilityZone = AzUsEast1A,
		string? tagPoolId = null,
		string? tagPoolName = null,
		DateTime? launchTime = null)
	{
		...
		if (tagPoolId != null)
		{
			i.Tags.Add(new Tag(AwsFleetManager.AwsTagPoolId, tagPoolId));
		}
		if (tagPoolName != null)
		{
			i.Tags.Add(new Tag(AwsFleetManager.AwsTagPoolName, tagPoolName));
		}
	...
	private static bool MatchesFilter(Instance i, Filter f)
	{
		if (f.Name == "instance-state-name")
		{
			return i.State.Name == f.Values[0];
		}
		
		if (f.Name.StartsWith("tag:", StringComparison.Ordinal))
		{
			string key = f.Name["tag:".Length..];
			Tag? tag = i.Tags.FirstOrDefault(t => String.Equals(t.Key, key, StringComparison.Ordinal));
			if (tag == null)
			{
				return false;
			}
			
			return tag.Value == f.Values[0];
		}
	...
	{
		 List<Instance> filteredInstances = request.Filters.Count > 0
			? _instances.Values.Where(i => request.Filters.All(f => MatchesFilter(i, f))).ToList()
			: _instances.Values.ToList();

		return Task.FromResult(new DescribeInstancesResponse
		{
			Reservations = [new Reservation { Instances = filteredInstances.Select(CopyInstance).ToList() }],

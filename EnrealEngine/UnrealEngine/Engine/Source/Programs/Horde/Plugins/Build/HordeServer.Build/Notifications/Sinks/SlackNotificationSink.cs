...
namespace HordeServer.Notifications.Sinks
{
	...
	public sealed class SlackNotificationSink : IHostedService, INotificationSink, IAvatarService, IAsyncDisposable
	{
		...
		const string BlueBadge = ":large_blue_square:";
		...
		async Task CreateOrUpdateWorkflowThreadAsync(string triageChannel, IIssue issue, IIssueSpan span, IReadOnlyList<IIssueSpan> spans, WorkflowConfig workflow, CancellationToken cancellationToken)
		{
			...
			await UpdateReactionsAsync(triageChannel, issue, fixFailedSpan, suspects, eventId, threadId, cancellationToken);
		...
		private async Task UpdateReactionsAsync(string triageChannel, IIssue issue, IIssueSpan? fixFailedSpan, IReadOnlyList<IIssueSuspect> suspects, string eventId, SlackMessageId threadId, CancellationToken cancellationToken)
		{
			...
			if (issue.OwnerId == null && (suspects.Count == 0 || suspects.Count > 5))
			{
				reactions |= ReactionFlags.NoSuspects;
			}
			...
			if ((reactionState == null && reactions != ReactionFlags.None) || (reactionState != null && reactionState.Digest != reactionStateDigest))
			{
				...
				if ((reactions & ReactionFlags.NoSuspects) != 0)
				{
					await _slackClient.AddReactionAsync(threadId, "question", cancellationToken);
				}
				else
				{
					await _slackClient.RemoveReactionAsync(threadId, "question", cancellationToken);
				}
		...
		public async Task SendDeviceIssueReportAsync(DeviceIssueReport report, CancellationToken cancellationToken)
		{
			...
			if (report.PlatformReports.Count > 0)
			{
				foreach (DevicePlatformReport platform in report.PlatformReports)
				{
					...
					{
						string badge = device.CleaningTime != null ? BlueBadge : RedBadge;
						StringBuilder builder = new StringBuilder($"{badge} <{device.DevicePoolURL}|*{device.DeviceName}*> | {device.DeviceAddress} | {device.PoolName}");
						if (device.CleaningTime != null)
						{
							builder.Append($"\n> Cleaning for {(int) (device.CleaningTime.Value.TotalHours)} hours");
						}
						else
						{

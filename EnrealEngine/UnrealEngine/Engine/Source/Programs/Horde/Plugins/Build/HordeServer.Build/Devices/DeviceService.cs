...
namespace HordeServer.Devices
{
	...
	public sealed class DeviceService : IHostedService, IAsyncDisposable
	{
			...
			if (!cancellationToken.IsCancellationRequested)
			{
				...
				try
				{
					...
					if (expireNotifications != null && expireNotifications.Count > 0)
					{
						...
						{
							await NotifyDeviceServiceAsync(buildConfig, $"Device {expiredDevice.Item2.PlatformId.ToString().ToUpperInvariant()} / {expiredDevice.Item2.Name} checkout will expire in 24 hours.  Please visit {_serverInfo.DashboardUrl}devices to check in and back out if needed.", null, null, null, expiredDevice.Item1, cancellationToken);
						}
					}
		...
		/// <param name="tags">The metadata tags of the device</param>
		/// <returns></returns>
		public async Task<IDevice?> TryCreateDeviceAsync(DeviceId id, string name, DevicePlatformId platformId, DevicePoolId poolId, bool? enabled, string? address, string? modelId, UserId? userId = null, List<string>? tags = null)
		{
			return await _devices.TryAddDeviceAsync(id, name, platformId, poolId, enabled, address, modelId, userId, tags);
		}

		/// <summary>
		/// Update a device
		/// </summary>
		public async Task UpdateDeviceAsync(DeviceId deviceId, DevicePoolId? newPoolId = null, string? newName = null, string? newAddress = null, string? newModelId = null, string? newNotes = null, bool? newEnabled = null, bool? newProblem = null, bool? newMaintenance = null, UserId? modifiedByUserId = null, string? message = null, List<string>? newTags = null)
		{
			if (newProblem == true)
			{
			...
			await _devices.UpdateDeviceAsync(deviceId, newPoolId, newName, newAddress, newModelId, newNotes, newEnabled, newProblem, newMaintenance, modifiedByUserId, message, newTags);
		}

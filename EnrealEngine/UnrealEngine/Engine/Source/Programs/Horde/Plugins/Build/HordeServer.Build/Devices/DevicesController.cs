...
namespace HordeServer.Devices
{
	...
	public class DevicesController : ControllerBase
	{
		...
		public async Task<ActionResult<CreateDeviceResponse>> CreateDeviceAsync([FromBody] CreateDeviceRequest deviceRequest, CancellationToken cancellationToken = default)
		{
			...
			IDevice? device = await _deviceService.TryCreateDeviceAsync(DeviceId.Sanitize(name), name, platform.Id, pool.Id, deviceRequest.Enabled, address, modelId, internalUser?.Id, deviceRequest.Tags);
		...
		public async Task<ActionResult> UpdateDeviceAsync(string deviceId, [FromBody] UpdateDeviceRequest update, CancellationToken cancellationToken = default)
		{
			...
			await _deviceService.UpdateDeviceAsync(deviceIdValue, poolIdValue, name, address, modelIdValue, update.Notes, update.Enabled, update.Problem, update.Maintenance, internalUser?.Id, update.Message, update.Tags);
		...
		public async Task<ActionResult<CreateDeviceReservationResponse>> CreateDeviceReservationAsync([FromBody] CreateDeviceReservationRequest request)
		{
			...
			foreach (DeviceReservationRequest deviceRequest in request.Devices)
			{
				...
				HashSet<string>? duplicateTags = deviceRequest.Tags?.GetDuplicateEntries();
				if (duplicateTags != null && duplicateTags.Count > 0)
				{
					string duplicateList = String.Join(", ", duplicateTags);
					return BadRequest($"The following device tags were requested more than once: {duplicateList}");
				}

				requestedDevices.Add(new DeviceRequestData(platformIdValue, platformIdValue.ToString(), deviceRequest.IncludeModels, deviceRequest.ExcludeModels, tags: deviceRequest.Tags));
			}
			...
			response.Devices = new List<GetDeviceResponse>();
		...
		public async Task<ActionResult<GetLegacyReservationResponse>> CreateDeviceReservationV1Async([FromBody] LegacyCreateReservationRequest request)
		{
			...
			foreach (string deviceType in request.DeviceTypes)
			{
				...
				requestedDevices.Add(new DeviceRequestData(platform.Id, platformName, includeModels, excludeModels, request.DeviceName, request.Tags));
			}

			HashSet<string>? duplicateTags = request.Tags?.GetDuplicateEntries();
			if (duplicateTags != null && duplicateTags.Count > 0)
			{
				string duplicateList = String.Join(", ", duplicateTags);
				return BadRequest($"The following device tags were requested more than once: {duplicateList}");
		...
		public async Task<ActionResult<GetLegacyDeviceResponse>> GetDeviceV1Async(string deviceName)
		{
			...
			response.Tags = device.Tags ?? new List<string>();

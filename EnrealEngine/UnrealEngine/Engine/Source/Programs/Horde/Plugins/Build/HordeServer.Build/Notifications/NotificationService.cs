...
namespace HordeServer.Notifications
{
	...
	public sealed class NotificationService : IHostedService, INotificationService, IAsyncDisposable
	{
			...
			IClock clock)
		{
			...
			jobService.OnJobAborted += NotifyJobAborted;
		...
		public async ValueTask DisposeAsync()
		{
			...
			_jobService.OnJobAborted -= NotifyJobAborted;
		...
		/// <inheritdoc/>
		public void NotifyJobAborted(IJob job, IGraph graph)
		{
			// Enqueue job complete notifications if needed
			if (job.GetState() == JobState.Complete && job.AbortedByUserId is not null)
			{
				_logger.LogInformation("Queuing job notifications for {JobId}", job.Id);
		...
		private Task RecordJobCompleteMetricsAsync(IJob job, CancellationToken cancellationToken)
		{
			...
			{
				_logger.LogWarning(e, "Failed to Record job metric for {JobId}", job.Id);
			}
		...
		private async Task SendJobStepNotificationsAsync(IJob job, JobStepBatchId batchId, JobStepId stepId, CancellationToken cancellationToken)
		{
			...
			int warnings = job.Batches.Sum(x => x.Steps.Count(y => y.Outcome == JobStepOutcome.Warnings));
			bool firstFailure = step.Outcome == JobStepOutcome.Failure && failures == 1;
			bool firstWarning = step.Outcome == JobStepOutcome.Warnings && failures == 0 && warnings == 1;
			if (job.StartedByUserId.HasValue && !usersToNotify.Any(x => x.Id == job.StartedByUserId))
			{
				if (firstFailure || firstWarning)
				{					
					...
					{
						_logger.LogWarning("Author {AuthorUserId} is not in notify list, step outcome is {JobStepOutcome}, though unable to find in user collection", job.StartedByUserId, step.Outcome);
					}
				}
				else
				{
					_logger.LogInformation("Author {AuthorUserId} is not in notify list, step outcome is {JobStepOutcome}, not adding to list with failure count: {JobStepFailures} and warning count: {JobStepWarnings}...", job.StartedByUserId, step.Outcome, failures, warnings);
				}
			}
			...
			// Fetch our event data which is mandatory for a completed step, but optional for canceled steps
			ILog? log = null;
			List<ILogEventData> jobStepEventData = new List<ILogEventData>();
			if (step.LogId is not null)
			{
				log = await _logCollection.GetAsync(step.LogId.Value, cancellationToken);
				if (log is not null)
				{
			...
			// Treat a canceled job step separately from a completed step as we may have different logic for each
			if (step.State == JobStepState.Aborted)
			{
				if (filteredUsersToNotify.Count == 0)
				{
					_logger.LogInformation("No users to notify for step {JobId}:{BatchId}:{StepId}", job.Id, batchId, stepId);
				}
				else
				{
					EnqueueTasks((sink, ctx) => sink.NotifyJobStepAbortedAsync(filteredUsersToNotify, job, batch, step, node, jobStepEventData, ctx));
				}

				if (job.PreflightCommitId is null)
				{
					EnqueueTasks((sink, ctx) => sink.NotifyJobStepAbortedAsync(null, job, batch, step, node, jobStepEventData, ctx));
				}
			}
			else
			{
				...
				if (job.PreflightCommitId is null)
				{
					EnqueueTasks((sink, ctx) => sink.NotifyJobStepCompleteAsync(null, job, batch, step, node, jobStepEventData, ctx));
				}
			}

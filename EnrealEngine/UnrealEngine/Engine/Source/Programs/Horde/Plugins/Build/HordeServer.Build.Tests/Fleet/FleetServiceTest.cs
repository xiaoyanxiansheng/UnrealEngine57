	...
	public class FleetServiceTest : BuildTestSetup
	{
		...
		private FleetService GetFleetService(IFleetManager fleetManager, bool isDowntimeActive = false)
		{
			...
			FleetService service = new(
				AgentService, PoolCollection, new DowntimeServiceStub(isDowntimeActive),
				new StubFleetManagerFactory(fleetManager), Clock, ServiceProvider.GetRequiredService<IEnumerable<IPoolSizeStrategyFactory>>(), serverSettingsOpt, Tracer, loggerFactory.CreateLogger<FleetService>());

	...
	public sealed class IssueService : IHostedService, IAsyncDisposable
	{
		...
		async Task<HashSet<IssueEventGroupInternal>> GetEventGroupsForStepAsync(IJob job, IJobStepBatch batch, IJobStep step, INode node, IReadOnlyNodeAnnotations annotations, WorkflowConfig? workflow, CancellationToken cancellationToken)
		{
			...
			foreach (IssueEventInternal issueEvent in issueEvents)
			{
				...
				issueEvent.AuditLogger.LogDebug("Logging events from Job {JobId}, step {StepId}", job.Id, step.Id);
		...
		static void EmitIssueAuditLogs(ILogger auditLogger, List<IssueEventGroupInternal> eventGroups)
		{
			...
			if (uniqueEntries != null && uniqueEntries.Count > 0)
			{
				foreach (IssueAuditLogEntry e in uniqueEntries)
				{
					...
					{
						auditLogger.LogError(e.GetMessageTemplate(), args);
					}
					else if (e.GetLogLevel() == LogLevel.Warning)
					{
						auditLogger.LogWarning(e.GetMessageTemplate(), args);
					}
					else if (e.GetLogLevel() == LogLevel.Information)
					{
						auditLogger.LogInformation(e.GetMessageTemplate(), args);
					}
					else
					{
						auditLogger.LogDebug(e.GetMessageTemplate(), args);
					}
		...
		async Task<List<NewIssueSpanSuspectData>> FindSuspectsForSpanAsync(StreamConfig streamConfig, IIssueFingerprint fingerprint, CommitId lastSuccess, CommitId firstFailure, CancellationToken cancellationToken)
		{
			...
			if (!String.IsNullOrEmpty(fingerprint.ChangeFilter))
			{
				...
				List<ICommit> changes = await _commitService.GetCollection(streamConfig).FindAsync(lastSuccess, includeMinCommit: false, firstFailure, includeMaxCommit: true, MaxChanges, tags, cancellationToken: cancellationToken).ToListAsync(cancellationToken);
				...
				foreach (ICommit commit in changes)
				{
					...
					try
					{
						rank = await GetChangeScoreAsync(commit, filter, suspectFiles, suspectNotes, suspectSymbols, _tracer);
					}
					catch (NullReferenceException nullRefEx)
					{
						_logger.LogError("Exception ranking suspect change {Commit}: {Message} - {Callstack}", commit.Id, nullRefEx.Message, nullRefEx.StackTrace);

					}
					catch (EpicGames.Perforce.PerforceException perforceEx)
					{
						_logger.LogError("Exception ranking suspect change {Commit}: {Message} - {Callstack}", commit.Id, perforceEx.Message, perforceEx.StackTrace);

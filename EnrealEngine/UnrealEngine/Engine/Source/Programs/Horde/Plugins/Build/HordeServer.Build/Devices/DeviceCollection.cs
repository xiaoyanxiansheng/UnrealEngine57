...
namespace HordeServer.Devices
{
	...
	public class DeviceCollection : IDeviceCollection
	{
		...
		class DeviceDocument : IDevice
		{
			...
			public DeviceDocument(DeviceId id, DevicePlatformId platformId, DevicePoolId poolId, string name, bool enabled, string? address, string? modelId, UserId? userId, List<string>? tags)
			{
				Id = id;
		...
		/// <inheritdoc/>
		public async Task<IDevice?> TryAddDeviceAsync(DeviceId id, string name, DevicePlatformId platformId, DevicePoolId poolId, bool? enabled, string? address, string? modelId, UserId? userId, List<string>? tags = null)
		{
			DeviceDocument newDevice = new DeviceDocument(id, platformId, poolId, name, enabled ?? true, address, modelId, userId, tags);
		...
		/// <inheritdoc/>
		public async Task UpdateDeviceAsync(DeviceId deviceId, DevicePoolId? newPoolId, string? newName, string? newAddress, string? newModelId, string? newNotes, bool? newEnabled, bool? newProblem, bool? newMaintenance, UserId? modifiedByUserId = null, string? message = null, List<string>? newTags = null)
		{
			UpdateDefinitionBuilder<DeviceDocument> updateBuilder = Builders<DeviceDocument>.Update;
			List<UpdateDefinition<DeviceDocument>> updates = new List<UpdateDefinition<DeviceDocument>>();
			...
			if (newTags != null)
			{
				updates.Add(updateBuilder.Set(x => x.Tags, newTags));
			}
		...
		public async Task<(IDeviceReservation?, bool)> TryAddReservationAsync(DevicePoolId poolId, List<DeviceRequestData> request, int problemCooldown, string? hostname, string? reservationDetails, IJob? job, JobStepId? stepId, string? stepName, List<JobStepId>? stepIds)
		{
			...
			Comparison<DeviceDocument> rankByLastReservationTime = (a, b) =>
			{
			...
			poolDevices.Sort(rankByLastReservationTime);
			...
			foreach (DeviceRequestData data in request)
			{
				List<DeviceDocument> rankedDevices = new List<DeviceDocument>(poolDevices);
				
				if (data.Tags != null)
				{
					bool hasPreferences = data.Tags.PreferredTags != null && data.Tags.PreferredTags.Count != 0;
					bool hasUndesired = data.Tags.UndesiredTags != null && data.Tags.UndesiredTags.Count != 0;

					if (hasPreferences || hasUndesired)
					{
						// if there are preferred or undesired tags in the request, sort the devices by ranking preferred highest, undesired lowest.
						// if a device happens to have both a preferred and undesired tag, it's sort order will be ignored
						rankedDevices.Sort((a, b) =>
						{
							int aPreferred = hasPreferences && a.Tags?.Count > 0 ? data.Tags.PreferredTags!.Count(tag => a.Tags.Contains(tag, StringComparer.OrdinalIgnoreCase)) : 0;
							int aUndesired = hasUndesired && a.Tags?.Count > 0 ? data.Tags.UndesiredTags!.Count(tag => a.Tags.Contains(tag, StringComparer.OrdinalIgnoreCase)) : 0;
							
							int bPreferred = hasPreferences && b.Tags?.Count > 0 ? data.Tags.PreferredTags!.Count(tag => b.Tags.Contains(tag, StringComparer.OrdinalIgnoreCase)) : 0;
							int bUndesired = hasUndesired && b.Tags?.Count > 0 ? data.Tags.UndesiredTags!.Count(tag => b.Tags.Contains(tag, StringComparer.OrdinalIgnoreCase)) : 0;

							bool aIsPreferred = aPreferred > 0 && aUndesired == 0;
							bool bIsPreferred = bPreferred > 0 && bUndesired == 0;
							
							bool aIsUndesired = aUndesired > 0 && aPreferred == 0;
							bool bIsUndesired = bUndesired > 0 && bPreferred == 0;
							...
							if (aIsPreferred && bIsPreferred)
							{
								...
								{
									return rankByLastReservationTime(a, b);
								}
								return aPreferred > bPreferred ? -1 : 1;
							}

							// Sort undesired down
							if (aIsUndesired && !bIsUndesired)
							{
								return 1;
							}
							if (bIsUndesired && !aIsUndesired)
							{
								return -1;
							}
							if (aIsUndesired && bIsUndesired)
							{
								if (aUndesired == bUndesired)
								{
									return rankByLastReservationTime(a, b);
								}
								return aUndesired > bUndesired ? 1 : -1;
							}
				...
				DeviceDocument? device = rankedDevices.FirstOrDefault(a =>
				{
					...
					if (data.Tags != null)
					{
						if (data.Tags.RequiredTags != null && data.Tags.RequiredTags.Count > 0)
						{
							if (!data.Tags.RequiredTags.All(tag => a.Tags != null && a.Tags.Contains(tag, StringComparer.OrdinalIgnoreCase)))
							{
								return false;
							}
						}

						if (data.Tags.BlockedTags != null && data.Tags.BlockedTags.Count > 0)
						{
							if (data.Tags.BlockedTags.Any(tag => a.Tags != null && a.Tags.Contains(tag, StringComparer.OrdinalIgnoreCase)))
							{
								return false;
							}
						}

...
enum class EHashType : uint8 {
	...
	Blake3_160,	 // Blake3 hash, truncated to 160 bits, AKA Unreal Engine IoHash
...
private:
	static constexpr size_t STATE_SIZE = 1912;
	uint8 State[STATE_SIZE];
};

class alignas(8) FMD5Hasher
{
public:
	FMD5Hasher();
...
private:
	static constexpr size_t STATE_SIZE = 88;
	uint8					State[STATE_SIZE];
};
...
{
public:
	FStrongHasher(EStrongHashAlgorithmID InAlgorithm) : Algorithm(InAlgorithm) {}

	void		 Reset();
	void		 Update(const void* Data, uint64 Size);
	FGenericHash Finalize();

private:
	const EStrongHashAlgorithmID Algorithm;
	FBlake3Hasher				 Blake3;
	FMD5Hasher					 MD5;
};

...
namespace ChaosTest {
	...
	{
		for (int32 IndexUseLinear = 0; IndexUseLinear < 2; ++IndexUseLinear)
		{
			const bool bUseLinear = IndexUseLinear == 0;

			for (int32 IndexSimd = 0; IndexSimd < 2; IndexSimd++)
			{
				bool bUseSimd = IndexSimd == 0;
				const int32 NumSolverIterations = 20;
				const FReal Gravity = 1000.0;
				const FReal Dt = 0.01;
				const int32 NumSteps = 100;
				const int32 NumBodies = 2;
				const FReal Extension = 10;
				...
				FPBDJointConstraints& Constraints = bUseLinear ? Test.Evolution.GetJointCombinedConstraints().LinearConstraints : Test.Evolution.GetJointCombinedConstraints().NonLinearConstraints;

				Constraints.SetUseSimd(bUseSimd);

				if (!Constraints.GetSettings().bUsePositionBasedDrives)
				{
					Test.Evolution.SetNumVelocityIterations(NumSolverIterations);
				}
				...
				Test.JointSettings[0].bUseLinearSolver = bUseLinear;
				...
				for (int32 i = 0; i < NumSteps; ++i)
				{
					...
					const FReal ForceZ = -Constraints.GetConstraintLinearImpulse(0).Z / Dt;

...
namespace GeometryCollectionTest
{
	...
	FGeometryCollectionWrapper::FGeometryCollectionWrapper() 
		: WrapperBase(WrapperType::GeometryCollection)
	{
		FChaosUserData::Set<FCustomChaosPayload>(&UserData, reinterpret_cast<FCustomChaosPayload*>(this));
	}
	...
	WrapperBase* CommonInit(const TSharedPtr<FGeometryCollection>& RestCollection, const CreationParameters& Params)
	{
		...
		FGeometryCollectionWrapper* Wrapper = new FGeometryCollectionWrapper();
		...
		FSimulationParameters SimulationParams;
		{
			...
			SimulationParams.AllowPartialIslandSleep = true;
			SimulationParams.PositionSolverIterations = 8;
			SimulationParams.VelocitySolverIterations = 2;
			SimulationParams.ProjectionSolverIterations = 1;
			SimulationParams.UserData = reinterpret_cast<void*>(&Wrapper->UserData);
		...
		SimFilterData.Word3 = 0xFFFF; // collision candidate channels (also enables EPDF_ContactNotify)
		...
		Wrapper->RestCollection = RestCollection;
		Wrapper->DynamicCollection = DynamicCollection;
		Wrapper->PhysObject = PhysObject;
		return Wrapper;
	}
	...
	, Solver(nullptr)
	{
		Solver = Module->CreateSolver(nullptr, Parameters.AsyncDt, Parameters.ThreadingMode);	//until refactor is done, solver must be created after thread change
	...
	void FFramework::RemoveSimulationObject(WrapperBase* Object)
	{
		if (PhysicsObjects.Remove(Object) > 0)
		{
			if (FGeometryCollectionWrapper* GCW = Object->As<FGeometryCollectionWrapper>())
			{
				Solver->UnregisterObject(GCW->PhysObject);

...
namespace ChaosTest {
	...
	public:
		...
		virtual void Create()
		{
			...
			for (int32 JointIndex = 0; JointIndex < JointPositions.Num(); ++JointIndex)
			{
				...
				FPBDJointConstraintHandle*  Joint = AddJoint(ConstraintedParticleIds, JointPositions[JointIndex]);
				Joints.Add(Joint);
			}
		}
		...
		FPBDJointConstraintHandle* GetJoint(int32 Index)
		{
			return Joints[Index];
		}
	...
	// Change the linear/non-linear solver mode of a constraint after creation using the GT Proxy API.
	// Bug Fix: changing the solver type after creation would result in an attempt to remove the constraint
	// from the wrong Container on destruction, leading to an OOB array access, or destruction of the wrong
	// constraint and a later OOB access attempting to destroy that constraint.
	GTEST_TEST(JointTests, TestSetUseLinearJointSolver)
	{
		using namespace Chaos;
		...
		FChaosEngineInterface::SetUseLinearJointSolver_AssumesLocked(Constraint, false);

		// Tick physics thread to create the PT constraint
		Solver->AdvanceAndDispatch_External(/*Dt=*/1.0f);

		// Change the solver type for the joint
		FChaosEngineInterface::SetUseLinearJointSolver_AssumesLocked(Constraint, true);
		...
		// Tick physics thread
		// Bug Fix: This would crash in FPBDJointConstraints::RemoveConstraint
		Solver->AdvanceAndDispatch_External(/*Dt=*/1.0f);

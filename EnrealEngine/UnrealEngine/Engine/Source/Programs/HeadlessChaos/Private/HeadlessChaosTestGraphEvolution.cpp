...
namespace ChaosTest
{
	...
	TEST_P(GraphEvolutionTests, SparseArrayTests_TestSortFreeList)
	{
	...
	TEST_P(GraphEvolutionTests, TestConstraintGraph_TeleportSleeping)
	{
		...
		FGraphEvolutionTest Test(5, GetParam());
		Test.MakeChain();
		Test.Evolution.SetParticleObjectState(Test.ParticleHandles[0], EObjectStateType::Kinematic);
		...
		EXPECT_TRUE(Test.ParticleHandles[4]->IsInConstraintGraph());
		...
		EXPECT_TRUE(Test.ConstraintHandles[1]->IsInConstraintGraph());
		EXPECT_TRUE(Test.ConstraintHandles[2]->IsInConstraintGraph());
		EXPECT_TRUE(Test.ConstraintHandles[3]->IsInConstraintGraph());
		EXPECT_TRUE(Test.ConstraintHandles[0]->IsSleeping());
		EXPECT_TRUE(Test.ConstraintHandles[1]->IsSleeping());
		EXPECT_TRUE(Test.ConstraintHandles[2]->IsSleeping());
		...
		Test.Evolution.SetParticleObjectState(Test.ParticleHandles[4], EObjectStateType::Dynamic);
		Test.Evolution.SetParticleTransform(Test.ParticleHandles[4], FVec3(0, 3, 0), FRotation3::Identity, bIsTeleport, bWakeUp);
	...
	TEST_P(GraphEvolutionTests, TestConstraintGraph_SetVelocityOfSleeping)
	{
		...
		Test.Evolution.SetParticleDynamics(Test.ParticleHandles[2], Dynamics);
	...
	TEST_P(GraphEvolutionTests, TestConstraintGraph_WakeUpPropagation_AddToStackTop)
	{
		...
		if (Test.CVarPartialSleeping->GetBool())
		{
			...
			Dynamics.SetLinearImpulseVelocity(FVec3(50, 50, 0));
			Dynamics.SetAngularImpulseVelocity(FVec3(0, 0, 0));
			Test.Evolution.SetParticleDynamics(Test.ParticleHandles.Last(), Dynamics);
	...
	TEST_P(GraphEvolutionTests, TestConstraintGraph_WakeUpPropagation_AddToBlockWithoutSpacingAwake)
	{
		...
		Dynamics.SetLinearImpulseVelocity(FVec3(50, 50, 0));
		Dynamics.SetAngularImpulseVelocity(FVec3(0, 0, 0));
		Test.Evolution.SetParticleDynamics(Test.ParticleHandles[7], Dynamics);
	...
	TEST_P(GraphEvolutionTests, TestConstraintGraph_SleepCounterReset)
	{
		...
		{	
			...
			IConsoleVariable* CVarSleepCounterThreshold = IConsoleManager::Get().FindConsoleVariable(TEXT("p.Chaos.Solver.Sleep.Defaults.SleepCounterThreshold"), false);
			check(CVarSleepCounterThreshold && CVarSleepCounterThreshold->IsVariableInt());
			const int32 SleepCounterThreshold = CVarSleepCounterThreshold->GetInt();
			...
			for (FPBDRigidParticleHandle* Particle : Test.ParticleHandles)
			{
				Test.Evolution.SetParticleDynamics(Particle, Dynamics);
			}
	...
	// Test would fail since introducing Island->Flags.bHasSleepingNode.
	// When the entire island is already awake, we don't perform partial island waking (FPBDIslandManager::ProcessPartialIslandWake)
	// but just sync the island sleep state to all particles and constraints (FPBDIslandManager::PropagateIslandSleep).
	// Thus, the particle sleep counter is not reset if there is a wake event and the entire island is already awake
	// Since this is difficult to fix and there is no known practical issue, we leave this as is for now and disable the test. 
	TEST_P(GraphEvolutionTests, DISABLED_TestConstraintGraph_PreventSleepDuringWakeEvent)
	{
		// Partial island sleeping only
		if (GetParam())
		{
	...
	TEST_P(GraphEvolutionTests, TestConstraintGraph_PartialSleepAllowedParticleFlag)
	{
		if (GetParam())
		{
			FGraphEvolutionTest Test(4, GetParam());
			...
			// Explicitly put two particles asleep
			Test.Evolution.SetParticleObjectState(Test.ParticleHandles[0], EObjectStateType::Sleeping);
			Test.Evolution.SetParticleObjectState(Test.ParticleHandles[1], EObjectStateType::Sleeping);

			Test.Advance();

			// Partial sleeping should work as usual
			EXPECT_TRUE(FConstGenericParticleHandle(Test.ParticleHandles[0])->Sleeping());
			EXPECT_TRUE(FConstGenericParticleHandle(Test.ParticleHandles[1])->Sleeping());
			EXPECT_FALSE(FConstGenericParticleHandle(Test.ParticleHandles[2])->Sleeping());
			EXPECT_FALSE(FConstGenericParticleHandle(Test.ParticleHandles[3])->Sleeping());

			// Disallow partial sleeping for one of the sleeping particles
			Test.ParticleHandles[0]->SetPartialIslandSleepAllowed(false);

			Test.Advance();

			// All particles wake up
			EXPECT_FALSE(FConstGenericParticleHandle(Test.ParticleHandles[0])->Sleeping());
			EXPECT_FALSE(FConstGenericParticleHandle(Test.ParticleHandles[1])->Sleeping());
			EXPECT_FALSE(FConstGenericParticleHandle(Test.ParticleHandles[2])->Sleeping());
			EXPECT_FALSE(FConstGenericParticleHandle(Test.ParticleHandles[3])->Sleeping());
			...
			// Allow partial sleeping again of the particles
			Test.ParticleHandles[0]->SetPartialIslandSleepAllowed(true);
			...
			// Disallow partial sleeping for one of the awake particles
			Test.ParticleHandles[2]->SetPartialIslandSleepAllowed(false);
			...
			// All particles are asleep (using island sleeping)
			EXPECT_TRUE(FConstGenericParticleHandle(Test.ParticleHandles[0])->Sleeping());
			EXPECT_TRUE(FConstGenericParticleHandle(Test.ParticleHandles[1])->Sleeping());
			EXPECT_TRUE(FConstGenericParticleHandle(Test.ParticleHandles[2])->Sleeping());
			EXPECT_TRUE(FConstGenericParticleHandle(Test.ParticleHandles[3])->Sleeping());
	...
	TEST_P(GraphEvolutionTests, TestConstraintGraph_PartialSleepAllowedParticleFlag_2)
	{
		if (GetParam())
		{
			FGraphEvolutionTest Test(4, GetParam());
			...
			// Add another particle which does not allow partial sleep
			Test.ParticleHandles.Add(Test.Evolution.CreateDynamicParticles(1)[0]);
			Test.Evolution.EnableParticle(Test.ParticleHandles.Last());
			Test.ConstraintHandles.Add(Test.Constraints.AddConstraint({ Test.ParticleHandles.Last(1), Test.ParticleHandles.Last() }));
			Test.ParticleHandles.Last()->SetPartialIslandSleepAllowed(false);
			...
			// Disconnect the particle which does not allow partial sleep
			Test.ConstraintHandles.Last()->SetEnabled(false);
	...
	TEST_P(GraphEvolutionTests, TestConstraintGraph_MomentumPropagation)
	{
		const bool bEnablePartialSleep = true;
		const FRealSingle MomentumPropagation = GetParam() ? 0.5f : 0.025f;

		const FRealSingle MomentumPropagationDefault = CVarMomentumPropagation->GetFloat();
		CVarMomentumPropagation->Set(MomentumPropagation);

		FGraphEvolutionTest Test(5, bEnablePartialSleep);
		Test.MakeChain();
		Test.Evolution.SetParticleObjectState(Test.ParticleHandles[0], EObjectStateType::Kinematic);

		Test.AdvanceUntilSleeping();

		// Explicitly wake up one of the top particle
		Test.Evolution.SetParticleObjectState(Test.ParticleHandles[4], EObjectStateType::Dynamic);
		Test.Advance();

		// One the particle explicitly woken and the particle next to it are awake.
		// Momentum of the woken particle is zero, i.e. no momentum propagation.
		EXPECT_TRUE(FConstGenericParticleHandle(Test.ParticleHandles[0])->IsKinematic());
		EXPECT_TRUE(FConstGenericParticleHandle(Test.ParticleHandles[1])->Sleeping());
		EXPECT_TRUE(FConstGenericParticleHandle(Test.ParticleHandles[2])->Sleeping());
		EXPECT_FALSE(FConstGenericParticleHandle(Test.ParticleHandles[3])->Sleeping());
		...
		// Explicitly wake up one of the particles and add an impulse
		Test.Evolution.SetParticleObjectState(Test.ParticleHandles[4], EObjectStateType::Dynamic);
		FParticleDynamics Dynamics;
		Dynamics.SetAcceleration(FVec3(0, 0, 0));
		Dynamics.SetAngularAcceleration(FVec3(0, 0, 0));
		Dynamics.SetLinearImpulseVelocity(FVec3(0, 0.1, 0));
		Dynamics.SetAngularImpulseVelocity(FVec3(0, 0, 0));
		Test.Evolution.SetParticleDynamics(Test.ParticleHandles[4], Dynamics);

		Test.Advance();

		if (GetParam())
		{
			// All particles wake up, momentum is propagated through
			EXPECT_TRUE(FConstGenericParticleHandle(Test.ParticleHandles[0])->IsKinematic());
			EXPECT_FALSE(FConstGenericParticleHandle(Test.ParticleHandles[1])->Sleeping());
			EXPECT_FALSE(FConstGenericParticleHandle(Test.ParticleHandles[2])->Sleeping());
			EXPECT_FALSE(FConstGenericParticleHandle(Test.ParticleHandles[3])->Sleeping());
		...
		{
			// Only the first two particles wake up, momentum is too small to be propagated through
			EXPECT_TRUE(FConstGenericParticleHandle(Test.ParticleHandles[0])->IsKinematic());
			EXPECT_TRUE(FConstGenericParticleHandle(Test.ParticleHandles[1])->Sleeping());
			EXPECT_TRUE(FConstGenericParticleHandle(Test.ParticleHandles[2])->Sleeping());
			EXPECT_FALSE(FConstGenericParticleHandle(Test.ParticleHandles[3])->Sleeping());
	...
	TEST_P(GraphEvolutionTests, TestConstraintGraph_MinimumMotionlessRatio)
	{
		const bool bEnablePartialSleep = true;
		const FRealSingle MinMotionlessRatio = GetParam() ? 0.5f : 0.1f;

		const FRealSingle MinMotionlessRatioDefault = CVarMinMotionlessRatio->GetFloat();
		CVarMinMotionlessRatio->Set(MinMotionlessRatio);

		const IConsoleVariable* CVarSleepModeSwitchCounterThreshold = IConsoleManager::Get().FindConsoleVariable(TEXT("p.Chaos.Solver.Sleep.PartialIslandSleep.SleepModeSwitchCounterThreshold"), false);
		check(CVarSleepModeSwitchCounterThreshold && CVarSleepModeSwitchCounterThreshold->IsVariableInt());
		const int32 SleepModeSwitchCounterThreshold = CVarSleepModeSwitchCounterThreshold->GetInt();
		const int32 Steps = SleepModeSwitchCounterThreshold;

		FGraphEvolutionTest Test(6, bEnablePartialSleep);
		Test.MakeChain();
		Test.Evolution.SetParticleObjectState(Test.ParticleHandles[0], EObjectStateType::Kinematic);
		...
		// Explicitly wake up one of the top 3 particles and apply and impulse
		Test.Evolution.SetParticleObjectState(Test.ParticleHandles[3], EObjectStateType::Dynamic);
		Test.Evolution.SetParticleObjectState(Test.ParticleHandles[4], EObjectStateType::Dynamic);
		Test.Evolution.SetParticleObjectState(Test.ParticleHandles[5], EObjectStateType::Dynamic);
		Test.Evolution.SetParticleDynamics(Test.ParticleHandles[3], Dynamics);
		Test.Evolution.SetParticleDynamics(Test.ParticleHandles[4], Dynamics);
		Test.Evolution.SetParticleDynamics(Test.ParticleHandles[5], Dynamics);
		...
		// The particles explicitly woken and the particle next to it are awake.
		EXPECT_TRUE(FConstGenericParticleHandle(Test.ParticleHandles[0])->IsKinematic());
		EXPECT_TRUE(FConstGenericParticleHandle(Test.ParticleHandles[1])->Sleeping()); // sleeping
		EXPECT_FALSE(FConstGenericParticleHandle(Test.ParticleHandles[2])->Sleeping()); // awake, motionless
		EXPECT_FALSE(FConstGenericParticleHandle(Test.ParticleHandles[3])->Sleeping()); // awake, moving
		EXPECT_FALSE(FConstGenericParticleHandle(Test.ParticleHandles[4])->Sleeping()); // awake, moving
		EXPECT_FALSE(FConstGenericParticleHandle(Test.ParticleHandles[5])->Sleeping()); // awake, moving

		// Partial island sleep remains enabled because particles have not moved enough steps
		EXPECT_TRUE(Test.IslandManager->GetIsland(0)->IsPartialSleepEnabled());

		Test.Advance();
		
		if (MinMotionlessRatio == 0.5f)
		{
			// Partial island sleeping is internally disabled since 2/5 = 40% of dynamic particles are motionless.
			EXPECT_FALSE(Test.IslandManager->GetIsland(0)->IsPartialSleepEnabled());
		}
		else
		{
			// Partial island sleeping is internally enabled since 2/5 = 40% of dynamic particles are motionless.
			EXPECT_TRUE(Test.IslandManager->GetIsland(0)->IsPartialSleepEnabled());
		}
		...
		Test.CVarPartialSleeping->Set(false);
		Test.Advance();

		// All particles wake up when disabling partial island sleep explicitly (CVar).
		EXPECT_TRUE(FConstGenericParticleHandle(Test.ParticleHandles[0])->IsKinematic());
		EXPECT_FALSE(FConstGenericParticleHandle(Test.ParticleHandles[1])->Sleeping());
		EXPECT_FALSE(FConstGenericParticleHandle(Test.ParticleHandles[2])->Sleeping());
		EXPECT_FALSE(FConstGenericParticleHandle(Test.ParticleHandles[3])->Sleeping());
		...
		CVarMinMotionlessRatio->Set(MinMotionlessRatioDefault);
	}

	TEST_P(GraphEvolutionTests, TestConstraintGraph_MinimumMotionlessRatio_Extremes)
	{
		const bool bEnablePartialSleep = true;
		// MinMotionlessRatio == 0.0f => 0% of particles need to be motionless for partial island sleep to be active
		// MinMotionlessRatio == 1.0f => 100% of particles need to be motionless for partial island sleep to be active
		const FRealSingle MinMotionlessRatio = GetParam() ? 1.0f : 0.0f;
		...
		check(CVarSleepModeSwitchCounterThreshold && CVarSleepModeSwitchCounterThreshold->IsVariableInt());
		const int32 SleepModeSwitchCounterThreshold = CVarSleepModeSwitchCounterThreshold->GetInt();
		const int32 Steps = SleepModeSwitchCounterThreshold + 1;
		...
		Dynamics.SetAcceleration(FVec3(0, 0, 0));
		Dynamics.SetAngularAcceleration(FVec3(0, 0, 0));
		if (MinMotionlessRatio == 0.0f)
		{
			Dynamics.SetLinearImpulseVelocity(FVec3(0, 0.1, 0));
		}
		else // MinMotionlessRatio == 1.0f
		{
			Dynamics.SetLinearImpulseVelocity(FVec3(0, 0, 0));
		}
		Dynamics.SetAngularImpulseVelocity(FVec3(0, 0, 0));

		for (FPBDRigidParticleHandle* Particle : Test.ParticleHandles)
		{
			Test.Evolution.SetParticleObjectState(Particle, EObjectStateType::Dynamic);
			Test.Evolution.SetParticleDynamics(Particle, Dynamics);
		}
		...
		if (MinMotionlessRatio == 0.0f)
		{
			// Partial island sleeping remains enabled since all particles are in motion
			EXPECT_TRUE(Test.IslandManager->GetIsland(0)->IsPartialSleepEnabled());
		}
		else // MinMotionlessRatio == 1.0f
		{
			// Partial island sleeping remains enabled since all particles are in motionless
			EXPECT_TRUE(Test.IslandManager->GetIsland(0)->IsPartialSleepEnabled());
		}

...
namespace ChaosTest {
	...
	GTEST_TEST(EngineInterface, UpdatingAccelerationStructurePrePreFilterOnShapeFilterChange)
	{
		...
		for (const TUniquePtr<Chaos::FPerShapeData>& Shape : Particle.ShapesArray())
		{
			...
			const FCollisionFilterData QueryData = CollisionData.GetQueryData();
			PRAGMA_ENABLE_INTERNAL_WARNINGS
			EXPECT_EQ(QueryData.Word0, 0); // ensure query filter is defaulted to 0 (no query allowed at all)

			// Verify query is filtered out with default collision data on shape
			bool bFiltered = PrePreQueryFilterImp(QueryFilter, QueryData);
		...
		for (const TUniquePtr<Chaos::FPerShapeData>& Shape : Particle.ShapesArray())
		{
			...
			NewCollisionData.SetQueryData(NewParticleQueryFilter);
			Shape->SetCollisionData(NewCollisionData);

			// Filter with new data, ensuring we pass and are not filtered out.
			bool bFiltered = PrePreQueryFilterImp(QueryFilter, NewCollisionData.GetQueryData());

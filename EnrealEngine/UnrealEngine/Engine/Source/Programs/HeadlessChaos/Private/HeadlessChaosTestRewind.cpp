...
namespace ChaosTest {
	...
	void EnableRewindCaptureHelper(T_SOLVER& Solver, const int32 RewindHistorySize, const int32& Optimization)
	{
	...
	auto* CreateSolverHelper(int32 StepMode, int32 RewindHistorySize, int32 Optimization, FReal& OutSimDt)
	{
		...
		EnableRewindCaptureHelper(Solver, RewindHistorySize, Optimization);
	...
	GTEST_TEST(AllTraits, RewindTest_MovingGeomChange)
	{
		TRewindHelper::TestEmpty([](auto* Solver, FReal SimDt, int32 Optimization)
		{
			...
			for (int SimStep = 0; SimStep < LastSimStep - 1; ++SimStep)
			{
				...
				/** Grab the cached data from the beginning of the next frame. When the particle is set by GT data we cache the pre-GT state in the RewindData's PrePushData phase, 
				* the dirty GT data is separately cached as marshaled PushData and the result of the GT being applied will be cached in the next frames PrePushData. */
				const auto ParticleState = RewindData->GetPastStateAtFrame(*Proxy->GetHandle_LowLevel(), SimStep + 1, Chaos::FFrameAndPhase::PrePushData);
				...
				Helper->ProcessInputsFunc = [Proxy, RewindData = Solver->GetRewindData(), SimDt](int32 PhysicsStep, bool bResim)
					{
						...
						for (int32 Step = 0; Step < PhysicsStep; ++Step)
						{
							const FReal OldV = RewindData->GetPastStateAtFrame(*Proxy->GetHandle_LowLevel(), Step, Chaos::FFrameAndPhase::PrePushData).GetV()[2];
							const FReal Time = Step * SimDt;

							if (Time == 0)
							{
								// First frame is not cached since the proxy is marked as Uninitialized at that point
							}
							else if (Time <= 2)	//simple acceleration
							{
								EXPECT_NEAR(OldV, -Time, 1e-2);
							}
							else if (Time <= 3)	//simple acceleration after GT reset the velocity on step 2
							{
						...
						{
							{
								...
								FGeometryParticleState PrePushState = RewindData->GetPastStateAtFrame(*Proxy->GetHandle_LowLevel(), Step, Chaos::FFrameAndPhase::PrePushData);
								EXPECT_EQ(PrePushState.Acceleration()[2], 0);
								EXPECT_EQ(PrePushState.AngularAcceleration()[2], 0);
							}

							if (Optimization != 2) // Optimization 2 does not cache data in PostPushState so no need to check that phase
							{
								//After push (but before callback) only see GT force, and no torque
								FGeometryParticleState PostPushState = RewindData->GetPastStateAtFrame(*Proxy->GetHandle_LowLevel(), Step, Chaos::FFrameAndPhase::PostPushData);
							...
							{
							FGeometryParticleState State = RewindData->GetPastStateAtFrame(*Proxy->GetHandle_LowLevel(), Step, Chaos::FFrameAndPhase::PostCallbacks);
								if (Step == 3)
								{
	...
	GTEST_TEST(AllTraits, RewindTest_AddForce)
	{
		TRewindHelper::TestDynamicSphere([](auto* Solver, FReal SimDt, int32 Optimization, auto Proxy, auto Sphere)
		{
			...
			const int32 LastSimStep = LastGameStep / SimDt;
			FReal ExpectedX = 0;
			FReal ExpectedVelocity = 0;
			FReal PreviousForce = 0;
			...
			for (int Step = 0; Step < LastSimStep - 1; ++Step)
			{
				...
				ExpectedVelocity += PreviousForce * SimDt;
				ExpectedX += ExpectedVelocity * SimDt;

				EXPECT_EQ(ParticleState.GetV()[2], ExpectedVelocity);
				EXPECT_EQ(ParticleState.GetX()[2], ExpectedX);

				/** With Optimization 2 we only cache PrePushData where Acceleration is always 0. Calling GetPastStateAtFrame for PostPushData will return next frames PrePushData since it's the next available data */
				if (Optimization == 2)
				{
	...
	GTEST_TEST(AllTraits, RewindTest_IntermittentForce)
	{
		TRewindHelper::TestDynamicSphere([](auto* Solver, FReal SimDt, int32 Optimization, auto Proxy, auto Sphere)
		{
			...
			for (int Step = 0; Step < LastSimStep - 1; ++Step)
			{
				...
				const auto ParticleState = RewindData->GetPastStateAtFrame(*Proxy->GetHandle_LowLevel(), Step, (Optimization == 2 ? Chaos::FFrameAndPhase::PrePushData : Chaos::FFrameAndPhase::PostPushData));

				FReal ExpectedForce = 0;
				...
				/** With Optimization 2 we only cache PrePushData where Acceleration is always 0. */
				if (SimDt <= 1)
				{
					if (SimTime >= 3 && SimTime < 4)
					{
						ExpectedForce = 3;
						EXPECT_EQ(ParticleState.Acceleration()[2], (Optimization == 2) ? 0.0f : ExpectedForce);
					}
	...
	GTEST_TEST(AllTraits, RewindTest_IntermittentGeomChange)
	{
		TRewindHelper::TestDynamicSphere([](auto* Solver, FReal SimDt, int32 Optimization, auto Proxy, auto Sphere)
		{
			...
			for (int Step = 0; Step < LastSimStep - 1; ++Step)
			{
				/** Grab the cached data from the beginning of the next frame. When the particle is set by GT data we cache the pre-GT state in the RewindData's PrePushData phase,
				* the dirty GT data is separately cached as marshaled PushData and the result of the GT being applied will be cached in the next frames PrePushData. */
				const auto ParticleState = RewindData->GetPastStateAtFrame(*Proxy->GetHandle_LowLevel(), Step + 1, Chaos::FFrameAndPhase::PrePushData);
				if (SimDt <= 1)
				{
					const float SimTime = Step * SimDt;
					if (SimTime < 3)
	...
	GTEST_TEST(AllTraits, RewindTest_FallingObjectWithTeleport)
	{
		TRewindHelper::TestDynamicSphere([](auto* Solver, FReal SimDt, int32 Optimization, auto Proxy, auto Sphere)
		{
			...
			for (int Step = 0; Step < LastSimStep - 1; ++Step)
			{
				...
				if (Optimization == 2)
				{
					/* Teleport was made on GT and is cached in PushData when Optimization 2 is enabled so we will have the first RewindData cached on the next physics frame after the teleport
					* Track expected V and X from the next frame by adding SimDt to the teleport frame and then step the expected V and X once*/
					if (SimStart <= (5 + SimDt) && SimEnd > (5 + SimDt))
					{
						ExpectedVZ = 0;
						ExpectedXZ = 10;
	...
	GTEST_TEST(AllTraits, RewindTest_NumDirty)
	{
		for (int Optimization = 0; Optimization < 3; ++Optimization)
		{
			...
			EnableRewindCaptureHelper(Solver, 5, Optimization);
	...
	GTEST_TEST(AllTraits, RewindTest_ResimDesyncAfterMissingTeleport)
	{
		for (int Optimization = 0; Optimization < 3; ++Optimization)
		{
			...
			EnableRewindCaptureHelper(Solver, 7, Optimization);
	...
	GTEST_TEST(AllTraits, DISABLED_RewindTest_FullResimFallSeeCollisionCorrection)
	{
		for (int Optimization = 0; Optimization < 3; ++Optimization)
		{
			...
			EnableRewindCaptureHelper(Solver, 100, Optimization);
	...
	GTEST_TEST(AllTraits, DISABLED_RewindTest_SoftDesyncFromSameIslandThenBackToInSync_GeometryCollection_SingleFallingUnderGravity)
	{
		...
		for (int Optimization = 0; Optimization < 3; ++Optimization)
		{
			...
			FFramework UnitTest;
			EnableRewindCaptureHelper(UnitTest.Solver, 100, Optimization);

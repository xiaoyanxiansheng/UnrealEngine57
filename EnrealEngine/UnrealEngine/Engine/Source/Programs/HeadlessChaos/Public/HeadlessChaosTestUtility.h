...
#define EXPECT_VECTOR_FLOAT_EQ_ERR(A, B, E) EXPECT_FLOAT_EQ(A.X, B.X) << *E; EXPECT_FLOAT_EQ(A.Y, B.Y) << *E; EXPECT_FLOAT_EQ(A.Z, B.Z) << *E;
	...
	template <typename TParticle>
	void SetBoxInertiaTensor(TParticle& Particle, const FVec3& Dimensions, float Mass)
	{
		float ElementX = Mass * (Dimensions.Y * Dimensions.Y + Dimensions.Z * Dimensions.Z) / 12.0f;
		float ElementY = Mass * (Dimensions.X * Dimensions.X + Dimensions.Z * Dimensions.Z) / 12.0f;
		float ElementZ = Mass * (Dimensions.Y * Dimensions.Y + Dimensions.X * Dimensions.X) / 12.0f;
		float InvElementX = 1.f / ElementX;
		float InvElementY = 1.f / ElementY;
		float InvElementZ = 1.f / ElementX;
		Particle.SetI(TVec3<FRealSingle>(ElementX, ElementY, ElementZ));
		Particle.SetInvI(TVec3<FRealSingle>(InvElementX, InvElementY, InvElementZ));
	}

	template <typename TParticle>
	void SetSphereInertiaTensor(TParticle& Particle, float Radius, float Mass)
	{
		float Element = 2.0f * Mass * Radius * Radius / 5.0f;
		float InvElement = 1.f / Element;
		Particle.SetI(TVec3<FRealSingle>(Element));
		Particle.SetInvI(TVec3<FRealSingle>(InvElement));
	}

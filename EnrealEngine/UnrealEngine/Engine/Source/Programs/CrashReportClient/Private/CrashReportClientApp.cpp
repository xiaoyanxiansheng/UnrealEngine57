...
/** Default main window size with screenshot present */
const FVector2D InitialWindowDimensionsWithScreenshot(740, 1000);

/** Simple dialog window size with screenshot present */
const FVector2D InitialSimpleWindowDimensionsWithScreenshot(920, 700);
...
SubmitCrashReportResult RunWithUI(FPlatformErrorReport ErrorReport, bool bImplicitSend)
{
	...
	// If there's an optional screenshot attachment, ensure Slate loads it so that we can show it as part of the dialog.
	FCrashReportClientStyleOptions StyleOptions = {};
	FString CrashScreenshotPath = FPaths::Combine(ErrorReport.GetReportDirectory(), FGenericCrashContext::CrashReportOptionalAttachmentsDirName, TEXT("CrashCapture.jpg"));
	bool bHasScreenshot = FPaths::FileExists(CrashScreenshotPath);
	FVector2D CrashScreenshotSize{ 0, 0 };
	if (bHasScreenshot && FPrimaryCrashProperties::Get()->IsValid())
	{
		StyleOptions.OptionalCrashScreenshot.Filepath = CrashScreenshotPath;
		StyleOptions.OptionalCrashScreenshot.ImageSize = FVector2D(FPrimaryCrashProperties::Get()->CrashScreenshotWidth, FPrimaryCrashProperties::Get()->CrashScreenshotHeight);

		bHasScreenshot = !StyleOptions.OptionalCrashScreenshot.ImageSize.IsZero();
	}
	FCrashReportClientStyle::Initialize(StyleOptions);
	...
	FVector2D WindowDimensions;
	if (bImplicitSend)
	{
		WindowDimensions = bHasScreenshot ? InitialSimpleWindowDimensionsWithScreenshot : InitialSimpleWindowDimensions;
	}
	else
	{
		WindowDimensions = bHasScreenshot ? InitialWindowDimensionsWithScreenshot : InitialWindowDimensions;
	}
...
FPlatformErrorReport CollectErrorReport(FRecoveryService* RecoveryService, uint32 Pid, const FSharedCrashContext& SharedCrashContext, void* WritePipe, bool& bOutCrashPortableCallstackAvailable)
{
	...
	FPlatformCrashContext::InitializeFromContext(
		...
		SharedCrashContext.FilePathsNum > 0 ? &SharedCrashContext.DynamicData[SharedCrashContext.FilePathsOffset] : nullptr,
...
static bool LoadTempCrashContextFromFile(FSharedCrashContext& CrashContext, uint64 ProcessID)
{
	...
	// Enabled Plugins
	CrashContext.EnabledPluginsOffset = (uint32)DynamicDataBuilder.Len();
	if (const FXmlNode* EnabledPluginsNode = File.GetRootNode()->FindChildNode(TEXT("EnabledPlugins")))
	{
		const TArray<FXmlNode*>& EnabledPlugins = EnabledPluginsNode->GetChildrenNodes();
		CrashContext.EnabledPluginsNum = EnabledPlugins.Num();
		for (const FXmlNode* Node : EnabledPlugins)
		{
			DynamicDataBuilder.Append(Node->GetContent());
			DynamicDataBuilder.Append(CR_PAIR_DELIM);
		}
	}
	else
	{
		CrashContext.EnabledPluginsNum = 0;
	}
	
	// Engine Data
	CrashContext.EngineDataOffset = (uint32)DynamicDataBuilder.Len();
	if (const FXmlNode* EngineDataNode = File.GetRootNode()->FindChildNode(TEXT("EngineData")))
	{
		const TArray<FXmlNode*>& EngineDataEntries = EngineDataNode->GetChildrenNodes();
		CrashContext.EngineDataNum = EngineDataEntries.Num();
		for (const FXmlNode* Node : EngineDataEntries)
		{
			DynamicDataBuilder.Append(Node->GetTag());
	...
	{
		CrashContext.EngineDataNum = 0;
	}
	
	// Game Data
	CrashContext.GameDataOffset = (uint32)DynamicDataBuilder.Len();
	if (const FXmlNode* GameDataNode = File.GetRootNode()->FindChildNode(TEXT("GameData")))
	{
		const TArray<FXmlNode*>& GameDataEntries = GameDataNode->GetChildrenNodes();
		CrashContext.GameDataNum = GameDataEntries.Num();
		for (const FXmlNode* Node : GameDataEntries)
		{
			DynamicDataBuilder.Append(Node->GetTag());
			DynamicDataBuilder.Append(CR_PAIR_EQ);
			DynamicDataBuilder.Append(Node->GetContent());
	...
	{
		CrashContext.GameDataNum = 0;
	}
	
	// File Paths
	CrashContext.FilePathsOffset = (uint32)DynamicDataBuilder.Len();
	if (const FXmlNode* FilePathsNode = File.GetRootNode()->FindChildNode(TEXT("FilePaths")))
	{
		const TArray<FXmlNode*>& FilePaths = FilePathsNode->GetChildrenNodes();
		CrashContext.FilePathsNum = FilePaths.Num();
		for (const FXmlNode* Node : FilePaths)
		{
			DynamicDataBuilder.Append(Node->GetContent());
			DynamicDataBuilder.Append(CR_PAIR_DELIM);
		}
	...
	{
		CrashContext.FilePathsNum = 0;
	}

	FCString::Strncpy(CrashContext.DynamicData, DynamicDataBuilder.ToString(), CR_MAX_DYNAMIC_BUFFER_CHARS - 1);
	CrashContext.DynamicData[CR_MAX_DYNAMIC_BUFFER_CHARS - 1] = 0;

...
ACHIEVEMENTS_TEST_CASE("Unlock Title-Managed Achievements By Stats", EG_ACHIEVEMENTS_UNLOCKACHIEVEMENTS_TITLEMANAGED_TAG)
{
	...
	LoginPipeline
		...
		.EmplaceAsyncLambda([&AccountId, &AchievementId, &AchievementProgress](FAsyncLambdaResult Promise, const IOnlineServicesPtr& Services) 
			{ 
				CheckAchievementState_Fixture(Promise, Services, AccountId, AchievementId, AchievementProgress);
			})
		.EmplaceAsyncLambda([&AccountId, &StatNameKey, &StatValue](FAsyncLambdaResult Promise, const IOnlineServicesPtr& Services) 
			{
		...
		.EmplaceAsyncLambda([&AccountId, &AchievementId, &UpdatedAchievementProgress](const FAsyncLambdaResult& Promise, const IOnlineServicesPtr& Services) 
			{ 
...
ACHIEVEMENTS_TEST_CASE("Unlock Title-Managed Achievements By Stats With Multiple Conditions", EG_ACHIEVEMENTS_UNLOCKACHIEVEMENTS_TITLEMANAGED_TAG)
{
	...
	LoginPipeline
		.EmplaceStep<FQueryAchievementDefinitionsHelper>(MoveTemp(QueryDefinitionsHelperParams))
		.EmplaceAsyncLambda([&AccountId, &AchievementId, &AchievementProgress](const FAsyncLambdaResult& Promise, const IOnlineServicesPtr& Services) 
			{ 
				CheckAchievementState_Fixture(Promise, Services, AccountId, AchievementId, AchievementProgress);
			})
		.EmplaceAsyncLambda([&AccountId, &FirstStatNameKey, &FirstStatValue, &SecondStatNameKey, &SecondStatValue](const FAsyncLambdaResult& Promise, const IOnlineServicesPtr& Services) 
			{
		...
		.EmplaceAsyncLambda([&AccountId, &FirstStatNameKey, &ThirdStatValue, &SecondStatNameKey, &SecondStatValue]
			(const FAsyncLambdaResult& Promise, const IOnlineServicesPtr& Services) 
			{

		...
		Op.OnComplete([this, Promise](const OnlineResultType Result) mutable
		{
			...
			if (!bPass)
			{
				for (const OnlineResultType& ExpectedResult : ExpectedResults)
				{
					...
					if (Result.IsError())
					{
						UE::Online::FOnlineError Error = ExpectedResult.IsError() ? ExpectedResult.GetErrorValue() : UE::Online::FOnlineError(Errors::Unknown());
						CHECK(Result.GetErrorValue() == Error);
					}
				}
			}

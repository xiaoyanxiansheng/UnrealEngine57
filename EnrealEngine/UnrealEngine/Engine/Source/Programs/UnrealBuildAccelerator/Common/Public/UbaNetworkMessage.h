...
namespace uba
{
	enum MessagePriority : u8 { NormalPriority, HasPriority };

	struct NetworkMessage
	{
		NetworkMessage(NetworkClient& client, u8 serviceId, u8 messageType, BinaryWriter& sendWriter, MessagePriority priority = NormalPriority);
		NetworkMessage() {}
		~NetworkMessage();

		void Init(NetworkClient& client, u8 serviceId, u8 messageType, BinaryWriter& sendWriter, MessagePriority priority = NormalPriority);

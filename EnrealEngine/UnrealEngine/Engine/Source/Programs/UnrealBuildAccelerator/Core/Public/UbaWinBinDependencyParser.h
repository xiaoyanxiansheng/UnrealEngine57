...
#define True_GetFileSizeEx ::GetFileSizeEx
#endif
...
namespace uba
{
	...
	inline bool FindImportsInMem(const tchar* fileName, void* mem, u64 memSize, const Function<void(const tchar* import, bool isKnown, const char* const* importLoaderPaths)>& func)
	{
		...
		__try
		{
			...
			while(true)
			{
				if (u64((u8*)importDesc - (u8*)mem) + sizeof(IMAGE_IMPORT_DESCRIPTOR) > memSize)
					return false;
				if (importDesc->Name == NULL)
					break;
				char* name = (PCHAR)((DWORD_PTR)mem + GetOffset(importDesc->Name, pSech, hdrs));
				if (u64((u8*)name - (u8*)mem) + 2 > memSize)
					return false;
	...
	inline bool ParseBinary(const StringView& filePath, const StringView& originalPath, BinaryInfo& outInfo, const Function<void(const tchar* import, bool isKnown, const char* const* importLoaderPaths)>& func, StringBufferBase& outError)
	{
		...
		if (!True_GetFileSizeEx(fileHandle, &fileSize))
			return false;

		auto unmap = MakeGuard([&]() { True_UnmapViewOfFile(mem); });

		return FindImportsInMem(filePath.data, mem, fileSize.QuadPart, func);
	}
}

...
namespace UnsyncUI
{
	...
	public sealed class BuildModel : BaseModel
	{
		...
		public UInt64 CL => Template.CL == null ? 0 : (UInt64)Template.CL;
	...
	public sealed class ProjectModel : TabModel
	{
		...
		public bool BuildsViewFilter(object item)
		{
			...
			{
				if (!build.CL.ToString().Contains(term, StringComparison.InvariantCultureIgnoreCase)
		...
		private void CopyBuildCL()
		{
			if (selectedBuild != null)
			{
				CopyStringToClipboard(selectedBuild.CL.ToString());
			}
		}

...
void FInterchangeWorkerImpl::ProcessCommand(const TSharedPtr<UE::Interchange::ICommand> Command, const FString& ThreadName)
{
	...
	FJsonFBXLoadSourceCmd LoadSourceCommand;
	...
	{
		//Load file command
		if (LoadSourceCommand.GetTranslatorID().Equals(FJsonFBXLoadSourceCmd::TaskName))
		{
	...
	UE_LOG(LogInterchangeWorker, Verbose, TEXT("CompletedTask.Result : %d\n %s"), (int32)ProcessResult , *CompletedTask.JSonResult);
	int32 Index = 0;
	for (const FString& Message : CompletedTask.JSonMessages)
	{
		UE_LOG(LogInterchangeWorker, Verbose, TEXT("CompletedTask.Message #%d : %s"), ++Index, *Message);
	}
...
ETaskState FInterchangeWorkerImpl::LoadFbxFile(const FJsonFBXLoadSourceCmd& LoadSourceCommand, FString& OutJSonResult, TArray<FString>& OutJSonMessages)
{
	ETaskState ResultState = ETaskState::Unknown;
	FString SourceFilename = LoadSourceCommand.GetSourceFilename();
	UE_LOG(LogInterchangeWorker, Verbose, TEXT("LoadFbxFile : %s"), *SourceFilename);
	FbxParser.Reset();
	...
	UE_LOG(LogInterchangeWorker, Verbose, TEXT("ResultParser : %s"), *OutJSonResult);

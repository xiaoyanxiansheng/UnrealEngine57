...
public abstract class TextureBuildWorkerTarget : DerivedDataBuildWorkerTarget
{
	...
	static PluginInfo[] OodlePlugin = [null, null];
	...
	private void AddOodleModule(TargetInfo Target,bool bProjectOodle)
	{
		...
		int TypeIndex = (int)OodlePluginType;

		// We want to cache this and not re-parse plugin file over and over again
		lock (OodlePlugin)
		{
			OodlePlugin[TypeIndex] ??= new PluginInfo(OodleUPluginFile, OodlePluginType);
		}

		if (Plugins.IsPluginEnabledForTarget(OodlePlugin[TypeIndex], ProjectDesc, Target.Platform, Target.Configuration, TargetType.Program))
		{

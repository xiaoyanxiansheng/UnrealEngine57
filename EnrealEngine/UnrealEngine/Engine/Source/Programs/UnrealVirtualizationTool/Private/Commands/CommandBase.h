...
public:
	...
	/**
	 * Check to see if the provided project can be run with the current command.
	 * 
	 * @Project	The project to be validated
	 * @return	True if the project is allowed to be processed by the command and false if it is not.
	 */
	virtual bool IsProjectValidForCommand(const FProject& Project) const = 0;

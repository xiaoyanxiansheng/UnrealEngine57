...
bool FRehydrateCommand::ProcessProject(const FProject& Project, TUniquePtr<FCommandOutput>& Output)
{
	...
	{
		UE_LOG(LogVirtualizationTool, Display, TEXT("\t\t%d packages were hydrated"), Result.RehydratedPackages.Num());
		UE_LOG(LogVirtualizationTool, Display, TEXT("\t\tTotal hydration increase %s (%s -> %s)"),
	...
	{
		UE_LOG(LogVirtualizationTool, Display, TEXT("\t\tNo package files were hydrated"));
	}

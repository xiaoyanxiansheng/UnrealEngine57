...
bool FCommand::TryConnectToSourceControl(FStringView ClientSpecName)
{
	...
	{
		UE_LOG(LogVirtualizationTool, Log, TEXT("\t\tTrying to connect to source control with project settings..."));
	...
	{
		UE_LOG(LogVirtualizationTool, Log, TEXT("\t\tTrying to connect to source control with default settings..."));
	...
	if (SCCProvider->IsAvailable())
	{
		...
		{
			UE_LOG(LogVirtualizationTool, Log, TEXT("\t\tSuccessfully connected to server '%s'"), **Server);
		}

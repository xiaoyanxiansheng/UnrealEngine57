	...
	public class ManagedWorkspace
	{
		...
		*/
		...
		public async Task SyncAsync(IPerforceConnection perforce, string streamName, int changeNumber, IReadOnlyList<string> view, bool removeUntracked, bool fakeSync, FileReference? cacheFile, CancellationToken cancellationToken)
		{
			...
			if (changeNumber == -1)
			{
				_logger.LogInformation("Syncing to {StreamName} at latest (removeUntracked={RemoveUntracked})", streamName, removeUntracked);
			}
			else
			{
				_logger.LogInformation("Syncing to {StreamName} at CL {CL} (removeUntracked={RemoveUntracked})", streamName, changeNumber, removeUntracked);
			}
		...
		private async Task<ClientRecord> GetOrCreateClientAsync(IPerforceConnection perforceClient, string streamName, CancellationToken cancellationToken)
		{
			...
			using (ILoggerProgress status = _logger.BeginProgressScope("Updating client..."))
			{
				...
				if (!_options.UseHaveTable)
				{
					...
					await UpdateHaveTablePathAsync(perforceClient, $"//{clientName}/...#0", _tracer, cancellationToken);
				}
		...
		private async Task UpdateClientHaveTableAsync(IPerforceConnection perforceClient, int changeNumber, IReadOnlyList<string> view, CancellationToken cancellationToken)
		{
			...
			using (ILoggerProgress scope = _logger.BeginProgressScope("Updating have table..."))
			{
				...
				{
					await UpdateHaveTablePathAsync(perforceClient, $"//{perforceClient.Settings.ClientName}/...@{changeNumber}", _tracer, cancellationToken);
				}
				else
				{
					await UpdateHaveTablePathAsync(perforceClient, $"//{perforceClient.Settings.ClientName}/...#0", _tracer, cancellationToken);
				}
				...
				foreach (string filter in view)
				{
					...
					await UpdateHaveTablePathAsync(perforceClient, syncPath, _tracer, cancellationToken);
				}
		...
		/// <param name="cancellationToken">Cancellation token</param>
		/// <returns>Async task</returns>
		private static async Task UpdateHaveTablePathAsync(IPerforceConnection perforceClient, string syncPath, Tracer tracer, CancellationToken cancellationToken)
		{
			using TelemetrySpan span = tracer.StartActiveSpan($"{nameof(ManagedWorkspace)}.{nameof(UpdateHaveTablePathAsync)}");
			span.SetAttribute("horde.sync_path", syncPath);
		...
		private async Task<StreamSnapshotFromMemory> FindClientContentsAsync(IPerforceConnection perforceClient, int changeNumber, CancellationToken cancellationToken)
		{
			...
			using (ILoggerProgress scope = _logger.BeginProgressScope("Fetching metadata..."))
			{
				...
				scope.Progress = $"({timer.Elapsed.TotalSeconds:0.0}s {numFiles} files)";
			}

...
namespace EpicGames.Serialization
{
	...
	public abstract class CbWriterBase : ICbWriter
	{
		...
		/// <summary>
		/// Begin writing a uniform object field, an object which fields are all the same type
		/// </summary>
		/// <param name="name">Name of the field</param>
		/// <param name="elementType">The type of the uniform field</param>
		public void BeginUniformObject(CbFieldName name, CbFieldType elementType)
		{
			WriteFields();
			...
			Scope scope = EnterScope(CbFieldType.UniformObject, name);
		...
		void WriteFieldHeader(CbFieldType type, Utf8String name)
		{
			...
			else
			{
				...
				{
					if (elementType != (type | CbFieldType.HasFieldName))
					{
						throw new CbWriterException($"Mismatched type for uniform object - expected {elementType}, not {type}");
					}
					Memory<byte> buffer = Allocate(nameVarIntLength + name.Length);
	...
	public static class CbWriterExtensions
	{
		...
		/// <summary>
		/// Writes a named uuid/guid value
		/// </summary>
		/// <param name="writer">Writer for output data</param>
		/// <param name="name">Name of the field</param>
		/// <param name="value">Value to be written</param>
		public static void WriteUuid(this ICbWriter writer, CbFieldName name,  Guid value)
		{
			Span<byte> buffer = writer.WriteField(CbFieldType.Uuid, name, 16);
			value.TryWriteBytes(buffer, true, out _);
		}

		/// <summary>
		/// Writes a unnamed uuid/guid value
		/// </summary>
		/// <param name="writer">Writer for output data</param>
		/// <param name="value">The uuid value</param>
		public static void WriteUuidValue(this ICbWriter writer, Guid value)
		{
			Span<byte> buffer = writer.WriteField(CbFieldType.Uuid, default, 16);
			value.TryWriteBytes(buffer, true, out _);
		}

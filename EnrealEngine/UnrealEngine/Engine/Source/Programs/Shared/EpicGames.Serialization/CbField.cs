...
namespace EpicGames.Serialization
{
	...
	public class CbField : IEquatable<CbField>, IEnumerable<CbField>
	{
		...
		/// </summary>
		/// <returns>Value of the field</returns>
		public Guid AsUuid() => AsUuid(Guid.Empty);
...
#pragma warning restore CA1043 // Use Integral Or String Argument For Indexers
		...
		public static void WriteField(Utf8JsonWriter writer, CbField field, JsonSerializerOptions options)
		{
			switch (field.GetType())
			{
				...
				case CbFieldType.ObjectId:
					if (field.HasName())
					{
						writer.WriteString(field.Name.Span, field.AsObjectId().ToString());
					}
					else
					{
						writer.WriteStringValue(field.AsObjectId().ToString());
					}
					break;
				case CbFieldType.Uuid:
					if (field.HasName())
	...
	public class CbObjectJsonConverter : JsonConverter<CbObject>
	{
		...
		public override CbObject Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
		{
			...
			CbJsonReader.ReadJsonField(doc.RootElement, null, writer);
...
#pragma warning restore CA1043 // Use Integral Or String Argument For Indexers
		...
		public string ToJson()
		{
			...
			{
				ToJson(jsonWriter, JsonSerializerOptions.Default);
			}
		...
		public void ToJson(Utf8JsonWriter writer)
		{
			...
			{
				CbFieldJsonConverter.WriteField(writer, field, JsonSerializerOptions.Default);
			}
			writer.WriteEndObject();
		}
		...
		public void ToJson(Utf8JsonWriter writer, JsonSerializerOptions options)
		{
			...
			foreach (CbField field in _innerField)
			{
				CbFieldJsonConverter.WriteField(writer, field, options);
			}
			writer.WriteEndObject();
		}
...
#pragma warning disable CA1036 // we do not need the compare operators
	public readonly struct CbObjectId(uint a, uint b, uint c) : IEquatable<CbObjectId>, IComparer<CbObjectId>, IComparable<CbObjectId>
#pragma warning restore CA1036
	{
		...
		/// </summary>
		public int Compare(CbObjectId x, CbObjectId y)
		{
			...
			return x._c.CompareTo(y._c);
		}

		/// <summary>
		///  Compare two object ids by comparing their components
		/// </summary>
		public int CompareTo(CbObjectId other)
		{
	...
	/// </summary>
	public sealed class CbObjectIdReverseComparer : IComparer<CbObjectId>
	{
		/// <summary>
		/// Compare two cb objects returning the newst first
		/// </summary>
		public int Compare(CbObjectId x, CbObjectId y)
		{

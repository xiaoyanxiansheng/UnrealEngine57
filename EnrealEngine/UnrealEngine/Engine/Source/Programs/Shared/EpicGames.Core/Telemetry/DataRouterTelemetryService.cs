...
namespace EpicGames.Core.Telemetry
{
	...
	public class DataRouterTelemetryService : ITelemetryService<DataRouterEvent>
	{
		...
		public DataRouterTelemetryService(HttpClient inHttpClient, Uri inBaseAddress)
		{
			...
			Task<HttpResponseMessage> initializeConnectionFireAndForget = HeadRequestAsync();
		...
		/// <summary>
		/// Async function to issue a single async head request
		/// </summary>
		/// <returns>Http responce</returns>
		private async Task<HttpResponseMessage> HeadRequestAsync()
		{
			using HttpRequestMessage headRequest = new HttpRequestMessage(HttpMethod.Head, $"{BaseAddress}");
			return await Task.Run(async () => await HttpClient.SendAsync(headRequest));
		}
		...
		public virtual async Task FlushEventsAsync()
		{
			...
			DataRouterEvent? eventData;
			while (EventsQueue.TryDequeue(out eventData))
			{
				...
				posts[requestUri].Events.Add((DataRouterEvent)eventData);
			}
			...
			List<Task<HttpResponseMessage>> postRequests = [];
			foreach (KeyValuePair<string, DataRouterPost> postData in posts)
			{

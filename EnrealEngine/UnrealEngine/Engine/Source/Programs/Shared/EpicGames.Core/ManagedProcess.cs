...
#pragma warning disable IDE1006 // Naming Styles
		...
		[DllImport("kernel32.dll", SetLastError = true)]
		static extern bool QueryInformationJobObject(SafeFileHandle hJob, int JobObjectInformationClass, out JOBOBJECT_BASIC_ACCOUNTING_INFORMATION lpJobObjectInformation, int cbJobObjectInformationLength, out int lpReturnLength);

		[DllImport("kernel32.dll", SetLastError = true)]
		static extern bool QueryInformationJobObject(SafeFileHandle hJob, int JobObjectInformationClass, out JOBOBJECT_EXTENDED_LIMIT_INFORMATION lpJobObjectInformation, int cbJobObjectInformationLength, out int lpReturnLength);
...
#pragma warning restore IDE0049 // Naming Styles
		...
		public TimeSpan TotalProcessorTime =>
			SupportsJobObjects && JobHandle != null && QueryInformationJobObject(JobHandle, JobObjectBasicAccountingInformation, out JOBOBJECT_BASIC_ACCOUNTING_INFORMATION accountingInformation, Marshal.SizeOf(typeof(JOBOBJECT_BASIC_ACCOUNTING_INFORMATION)), out _)
				? new TimeSpan((long)accountingInformation.TotalUserTime + (long)accountingInformation.TotalKernelTime)
				: new TimeSpan();

		/// <summary>
		/// Returns the the peak memory for a process for this job.
		/// </summary>
		public long PeakProcessMemoryUsed =>
			SupportsJobObjects && JobHandle != null && QueryInformationJobObject(JobHandle, JobObjectExtendedLimitInformation, out JOBOBJECT_EXTENDED_LIMIT_INFORMATION accountingInformation, Marshal.SizeOf(typeof(JOBOBJECT_EXTENDED_LIMIT_INFORMATION)), out _)
				? (long)accountingInformation.PeakProcessMemoryUsed
				: 0;

		/// <summary>
		/// Returns the the peak memory for this job.
		/// </summary>
		public long PeakJobMemoryUsed =>
			SupportsJobObjects && JobHandle != null && QueryInformationJobObject(JobHandle, JobObjectExtendedLimitInformation, out JOBOBJECT_EXTENDED_LIMIT_INFORMATION accountingInformation, Marshal.SizeOf(typeof(JOBOBJECT_EXTENDED_LIMIT_INFORMATION)), out _)
				? (long)accountingInformation.PeakJobMemoryUsed
				: 0;
...
#pragma warning restore CA2213
		...
		/// </summary>
		public TimeSpan TotalProcessorTime => _accountingProcessGroup != null ? _accountingProcessGroup.TotalProcessorTime : new TimeSpan();

		/// <summary>
		/// Peak memory used
		/// </summary>
		public long PeakMemoryUsed => _accountingProcessGroup != null ? _accountingProcessGroup.PeakJobMemoryUsed : 0;

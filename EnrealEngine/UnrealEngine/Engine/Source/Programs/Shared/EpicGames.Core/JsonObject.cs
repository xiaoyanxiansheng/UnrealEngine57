...
namespace EpicGames.Core
{
	...
	public class JsonObject
	{
		...
		/// <param name="createCopy">raw object should be copied or not</param>
		private JsonObject(OrderedDictionary inRawObject, bool createCopy)
		{
			if (createCopy)
			{
				...
				case JsonValueKind.Array:
					{
						...
						object?[] elementArray = new object[len];
						int index = 0;
						foreach (JsonElement x in element.EnumerateArray())
						{
							elementArray[index++] = ParseElement(x);
						}
						return elementArray;
					}
		...
		static JsonDocument ParseUtf8File(string path)
		{
			using FileStream fs = new(path, FileMode.Open, FileAccess.Read, FileShare.Read, bufferSize: 1, FileOptions.SequentialScan);

			int len = checked((int)fs.Length);
			byte[] buf = len <= 128 * 1024 ? new byte[len] : ArrayPool<byte>.Shared.Rent(len);
			try
			{
				fs.ReadExactly(buf.AsSpan(0, len));
				int off = (len >= 3 && buf[0] == 0xEF && buf[1] == 0xBB && buf[2] == 0xBF) ? 3 : 0; // Skip UTF-8 BOM if present

				try
				{
					return JsonDocument.Parse(new ReadOnlyMemory<byte>(buf, off, len - off), new() { AllowTrailingCommas = true, CommentHandling = JsonCommentHandling.Skip });
				}
				catch (JsonException)
				{
					throw;
				...
				{
					throw new JsonException($"Unable to parse {path}: {ex.Message}", path, null, null, ex);
				}
			}
			finally
			{
		...
		{
			JsonDocument doc = ParseUtf8File(file.FullName);
			return new JsonObject(doc.RootElement);
		}
		...
		public static bool TryRead(FileReference fileName, [NotNullWhen(true)] out JsonObject? result)
		{
			...
			try
			{
				JsonDocument doc = ParseUtf8File(fileName.FullName);
				result = new JsonObject(doc.RootElement);
				return true;
			}
			catch (Exception)
			{
		...
		public bool TryGetObjectField(string fieldName, [NotNullWhen(true)] out JsonObject? result)
		{
			...
			{
				result = new JsonObject(orderedDictValue, false);
				return true;
			}
			else
			{
		...
		public bool TryGetObjectArrayField(string fieldName, [NotNullWhen(true)] out JsonObject[]? result)
		{
			...
			{
				result = enumValue.Select(x => new JsonObject((OrderedDictionary)x, false)).ToArray();
				return true;
			}
			else
			{

...
public abstract class BasePerforceFixtureTest : IDisposable
{
	...
	/// </summary>
	protected DirectoryReference TempDir { get; set; }
	...
	/// <summary>
	/// Unique ID to avoid tests clashing with each other
	/// </summary>
	protected string TestGuid { get; }
	...
	protected BasePerforceFixtureTest()
	{
		...
		TestGuid = Guid.NewGuid().ToString()[..8];
		_clientName = "test-fixture-" + TestGuid;
	...
	protected DirectoryReference CreateTempDir()
	{
		DirectoryReference dir = new (Path.Join(Path.GetTempPath(), "p4-" + _clientName));
	...
	{
		DeletePerforceDir(TempDir.FullName);
	}

	public static void DeletePerforceDir(string dirPath)
	{

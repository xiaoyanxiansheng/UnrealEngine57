...
namespace UnrealBuildBase
{
	...
	public class FileHasher(ILogger? logger = null)
	{
		private readonly struct CachedDigest(FileItem item, IoHash digest)
		{
			[JsonPropertyName("l")]
			public long Length { get; init; } = item.Length;
			[JsonPropertyName("w")]
			public long LastWriteTimeUtc { get; init; } = item.LastWriteTimeUtc.Ticks;
			[JsonPropertyName("d")]
			public IoHash Digest { get; init; } = digest;
		...
		// Dictionary containing cached digests, to prevent rehashing data if the file is unchanged

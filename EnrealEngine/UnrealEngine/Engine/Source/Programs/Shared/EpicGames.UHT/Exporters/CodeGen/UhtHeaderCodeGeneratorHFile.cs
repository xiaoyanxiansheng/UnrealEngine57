		...
		: UhtHeaderCodeGenerator
	{
		...
		public void Generate(IUhtExportFactory factory)
		{
			ref UhtCodeGenerator.HeaderInfo headerInfo = ref HeaderInfos[HeaderFile.HeaderFileTypeIndex];
			{
				...
				builder.Append("\r\n");
				if (Module.Module.MinimizeGeneratedIncludes)
				{
					builder.Append("#include \"UObject/ObjectMacros.h\"\r\n");
					builder.Append("#include \"Templates/NoDestroy.h\"\r\n");
				...
				if (headerInfo.NeedsVerseCoroutine)
				{
					builder.Append("#include \"VerseCoroutine.h\"\r\n");
				}
				...
				{
					UhtField[] sorted = [.. HeaderFile.References.ForwardDeclarations];
					Array.Sort(sorted, (x, y) =>
					{
					
						int compare;
						compare = compareUE.Compare(x.Namespace.FullSourceName, y.Namespace.FullSourceName);
						if (compare != 0)
						{
							return compare;
						}

						compare = compareUE.Compare(x.EngineClassName, y.EngineClassName);
						if (compare != 0)
						{
							return compare;
						}

						return compareUE.Compare(x.SourceName, y.SourceName);
					});

					for (int index = 0; index < sorted.Length;)
					{
						UhtNamespace namespaceObj = sorted[index].Namespace;
						namespaceObj.AppendMultipleLines(builder, (builder) =>
						{
							for (; index < sorted.Length && sorted[index].Namespace == namespaceObj; index++)
							{
								builder.AppendForwardDeclaration(sorted[index]).Append("\r\n");
							}
						});
					}
				}
				if (HeaderFile.References.ForwardDeclarationStrings.Count > 0)
				{
					string[] sorted = [.. HeaderFile.References.ForwardDeclarationStrings];
		...
		private static StringBuilder AppendRigVmStructInfoMethods(StringBuilder builder, UhtScriptStruct scriptStruct)
		{
					...
					if (methodInfo.IsPredicate)
					{
						...
						foreach (UhtRigVMParameter parameter in methodInfo.Parameters)
						{
							...
							builder.Append("\t\t\t\t*(").Append(baseType).Append("*) Branch.MemoryHandles[")
								.Append(parameterIndex++).Append("].GetOutputData() = ")
								.Append(parameter.Name).Append(";  \\\r\n");
							
						}
						builder.Append("\t\t\t\tBranch.Execute(*Context);  \\\r\n");
						builder.Append("\t\t\t\treturn *(").Append(methodInfo.ReturnType).Append("*)Branch.MemoryHandles[").Append(methodInfo.Parameters.Count).Append("].GetInputData();  \\\r\n");
						builder.Append("\t\t\t}  \\\r\n");
				...
				foreach (UhtRigVMMethodInfo methodInfo in scriptStruct.RigVMStructInfo.Methods)
				{
					...
					if (scriptStruct.RigVMStructInfo.Members.Count > 0)
					{
						...
						foreach (UhtRigVMParameter parameter in scriptStruct.RigVMStructInfo.Members)
						{
							...
							string getDataMethod = "GetInputData";
							if (parameter.IsLazy)
							{
							...
							{
								getDataMethod = "GetOutputData";
							}
							else if (!parameter.Input && !parameter.Output && !parameter.Singleton)
							{
								getDataMethod = "GetPrivateData";
							}
		...
		{
			builder.Append("struct Z_Construct_UScriptStruct_").Append(scriptStruct.SourceName).Append("_Statics;\r\n");
			...
			if (scriptStruct.ScriptStructFlags.HasAnyFlags(EStructFlags.Native))
			{
				...
				AppendForwardDeclarationWithNamespace(builder, scriptStruct);
				...
				if (scriptStruct.IsVerseField)
				{
					...
					builder
						...
						.AppendVerseScopeAndName(scriptStruct, UhtVerseNameMode.Default).Append("\")\r\n");
				}
		...
		private StringBuilder AppendEnum(StringBuilder builder, UhtEnum enumObj)
		{
			...
			{
				builder.Append("\r\n");
				enumObj.Namespace.AppendSingleLine(builder, (StringBuilder builder) =>
				{
				...
				builder.Append("template<> struct TIsUEnumClass<").Append(enumObj.Namespace.FullSourceName).Append(enumObj.CppType).Append("> { enum { Value = true }; };\r\n");
			}
			else if (enumObj.CppForm == UhtEnumCppForm.Regular && enumObj.UnderlyingType != UhtEnumUnderlyingType.Unspecified)
			{
				...
				enumObj.Namespace.AppendSingleLine(builder, (StringBuilder builder) =>
				{
					builder.Append("enum ").Append(enumObj.CppType).Append(" : ").Append(enumObj.UnderlyingType.ToString().ToLower()).Append(';');
				});
			}
			else if (enumObj.CppForm == UhtEnumCppForm.Namespaced && enumObj.UnderlyingType != UhtEnumUnderlyingType.Unspecified)
			{
				...
				builder.Append("\r\n");
				builder.Append("namespace ").Append(enumObj.Namespace.FullSourceName).Append(splitName[0]).Append(" { enum ").Append(splitName[1]);
				builder.Append(" : ").Append(enumObj.UnderlyingType.ToString().ToLower());
				builder.Append("; }\r\n");
			}
		...
		private StringBuilder AppendClass(StringBuilder builder, UhtClass classObj)
		{
			...
			if (Session.IsUsingCompiledInObjectFormat(UhtCompiledInObjectFormat.ConstInit))
			{
				...
				// Predeclare class object outside class body macro
				builder.Append(GetExternalDecl(classObj.ObjectTypeIndex, UhtSingletonType.ConstInit));
			}
			...
			{
				builder.Append($"struct {GetSingletonName(classObj, UhtSingletonType.Statics)};\r\n");
			}
			// For functions with custom thunks, make the functions _Statics struct will need to be a friend of the class
			// We need to forward declare it outside of the class body to properly handle classes in namespaces
			foreach (UhtType child in classObj.Children)
			{
				if (child is UhtFunction function && function.FunctionExportFlags.HasAllFlags(UhtFunctionExportFlags.CustomThunk))
				{
					builder.Append($"\tstruct {GetSingletonName(function, UhtSingletonType.Statics)}; \\\r\n");
				}
			}
			// Declaration of global function fetching UClass of type
			...
			AppendForwardDeclarationWithNamespace(builder, classObj);
		...
		private StringBuilder AppendNetValidateDeclaration(StringBuilder builder, UhtClass classObj, UhtFunction function)
		{
			if (function.FunctionFlags.HasAnyFlags(EFunctionFlags.NetValidate))
			{
				...
				if (!function.FunctionFlags.HasAnyFlags(EFunctionFlags.Static) && 
					!function.FunctionExportFlags.HasAnyFlags(UhtFunctionExportFlags.Final) &&
					!classObj.ClassExportFlags.HasAnyFlags(UhtClassExportFlags.Final))
				{
		...
		private StringBuilder AppendClassGeneratedBody(StringBuilder builder, UhtClass classObj, string api)
		{
			...
			if (classObj.Config.Length > 0 && (superClass == null || classObj.Config != superClass.Config))
			{
				builder.Append("\tstatic constexpr const TCHAR* StaticConfigName() {return TEXT(\"").Append(classObj.Config).Append("\");} \\\r\n \\\r\n");
			}
		...
		private StringBuilder AppendCommonGeneratedBody(StringBuilder builder, UhtClass classObj, string api)
		{
			...
			if (!classObj.ClassExportFlags.HasAnyFlags(UhtClassExportFlags.NoExport))
			{
				builder.Append($"\tfriend struct ::{info.RegisteredSingletonName}_Statics; \\\r\n");
			}
			
			// For functions with custom thunks, make the functions _Statics struct a friend so it can take the address
			// of the thunk even if it's private
			foreach (UhtType child in classObj.Children)
			{
				if (child is UhtFunction function && function.FunctionExportFlags.HasAllFlags(UhtFunctionExportFlags.CustomThunk))
				{
					builder.Append($"\tfriend struct ::{GetSingletonName(function, UhtSingletonType.Statics)}; \\\r\n");
				}
			}
			...
			builder
				...
				.Append(classObj)
				.Append(", ")
				.Append(superClass != null ? superClass.SourceName : classObj.SourceName) // UObject passes itself as super to the macro
			...
			if (castedClass)
			{
				builder
					.Append("CASTCLASS_")
					.Append(classObj.ClassCastFlags.ToString());
			}
			else
			{
			...
			{
				if (classObj.ClassWithin == Session.UPackage)
				{
					builder.Append("\tDECLARE_WITHIN_UPACKAGE() \\\r\n");
		...
		private static StringBuilder AppendForwardDeclarationWithNamespace(StringBuilder builder, UhtField field)
		{
			return field.Namespace.AppendSingleLine(builder, (builder) => builder.AppendForwardDeclaration(field));
		}

...
namespace EpicGames.UHT.Exporters.CodeGen
{
			...
			string? alternateFunctionName, string? extraParam, UhtFunctionExportFlags extraExportFlags, int tabs, string endl)
		{
			...
			if (isDeclaration)
			{
				...
				else
				{
					...
					if (outerClass != null && outerClass.ClassExportFlags.HasAnyFlags(UhtClassExportFlags.Final))
					{
						//
					}
					...
					else if (textType != UhtPropertyTextType.EventFunctionArgOrRetVal &&
						!function.FunctionFlags.HasAnyFlags(EFunctionFlags.Static) &&
						!exportFlags.HasAnyFlags(UhtFunctionExportFlags.Final))
					{
						isVirtual = true;
					}
	...
	internal static class UhtHaederCodeGeneratorStringBuilderExtensions
	{
		...
		public static StringBuilder AppendVerseNativeCallableTypeDef(this StringBuilder builder, UhtFunction function)
		{
			...
			{
				builder.AppendTokens(returnProperty.TypeTokens.FullDeclarationTokens);
			}

			builder.Append(isSuspends ? '>' : '(').AppendVerseFunctionArgs(function, false, true, property => builder.AppendTokens(property.TypeTokens.FullDeclarationTokens), () => builder.Append("FDecidesContext")).Append(")>");
			return builder;
		}

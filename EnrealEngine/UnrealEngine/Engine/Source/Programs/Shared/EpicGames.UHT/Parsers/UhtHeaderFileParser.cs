...
namespace EpicGames.UHT.Parsers
{
	...
	public class UhtHeaderFileParser : IUhtTokenPreprocessor
	{
		...
		/// <summary>
		/// Depth of the namespaces
		/// </summary>
		public int NamespaceDepth => _namespaces.Count;

		/// <summary>
		/// Stack of current #if states
		/// </summary>
		private readonly List<CompilerDirective> _compilerDirectives = [];

		/// <summary>
		/// Stack of current #if states saved as part of the preprocessor state
		/// </summary>
		private readonly List<CompilerDirective> _savedCompilerDirectives = [];
		...
		/// <summary>
		/// Current namespaces being parsed
		/// </summary>
		private readonly List<UhtNamespaceElement> _namespaces = [];

		/// <summary>
		/// Lookup cache for any namespaces defined in this header.
		/// </summary>
		private readonly Dictionary<string, UhtNamespace> _namespaceLookupCache = [];

		/// <summary>
		/// Parse the given header file
		/// </summary>
		/// <param name="headerFile">Header file to parse</param>
		/// <param name="globalNamespace">Global namespace</param>
		/// <returns>Parser</returns>
		public static UhtHeaderFileParser Parse(UhtHeaderFile headerFile, UhtNamespace globalNamespace)
		{
			UhtHeaderFileParser headerParser = new(headerFile, globalNamespace);
			...
			if (!headerParser.SpottedAutogeneratedHeaderInclude && headerParser.HeaderFile.Data.Length > 0)
			{
				...
				{
					switch (headerFile.Session.Config!.MissingGeneratedHeaderIncludeBehavior.GetBehavior(headerFile.Module)) 
					{
		...
		private UhtHeaderFileParser(UhtHeaderFile headerFile, UhtNamespace globalNamespace)
		{
			TokenReader = new UhtTokenBufferReader(headerFile, headerFile.Data.Memory);
			HeaderFile = headerFile;
			TokenReader.TokenPreprocessor = this;
			_namespaces.Add(new("", globalNamespace));
			_namespaceLookupCache.Add("", globalNamespace);
		}
		...
		/// <summary>
		/// Add a namespace to the token list
		/// </summary>
		/// <param name="token">Namespace being added</param>
		public void PushNamespace(UhtToken token)
		{
			_namespaces.Add(new(token.Value, null));
		}

		/// <summary>
		/// Remove the given number of namespaces
		/// </summary>
		/// <param name="count"></param>
		public void PopNamespaces(int count)
		{
			if (count > _namespaces.Count)
			{
				throw new UhtException(TokenReader, "Mismatch when looking for closing brace");
			}
			_namespaces.RemoveRange(_namespaces.Count - count, count);
		}
		...
		/// </summary>
		/// <returns></returns>
		public UhtNamespace GetNamespace()
		{
			if (_topScope == null)
			{
				throw new UhtIceException("Expected a top scope");
			}
			return _topScope.Session.GetNamespace(_namespaces, _namespaceLookupCache);
		}
		...
		public static bool ParseStatement(UhtParsingScope topScope, ref UhtToken token, bool logUnhandledKeywords)
		{
			...
			if (parseResult == UhtParseResult.Unhandled || parseResult == UhtParseResult.Invalid)
			{
				if (topScope.ScopeType is UhtClass classObj)
				{
					...
					topScope.SkipDeclaration(token);
					if (recorder.Stop())
					{

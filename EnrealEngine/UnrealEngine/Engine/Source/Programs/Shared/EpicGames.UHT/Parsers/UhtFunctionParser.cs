...
namespace EpicGames.UHT.Parsers
{
	...
	public static class UhtFunctionParser
	{
		...
		[UhtKeyword(Extends = UhtTableNames.VModule)]
		[SuppressMessage("CodeQuality", "IDE0051:Remove unused private members", Justification = "Attribute accessed method")]
		[SuppressMessage("Style", "IDE0060:Remove unused parameter", Justification = "Attribute accessed method")]
		...
		{
			if (topScope.HeaderParser.NamespaceDepth == 1 && UhtFunctionParser.IsValidateDelegateDeclaration(token))
			{
		...
		{
			UhtFunction function = new(parentScope.HeaderFile, parentScope.HeaderParser.GetNamespace(), parentScope.ScopeType, token.InputLine);
...

			{
...

				{
					...
					// Record the tokens so we can detect this function as a declaration later (i.e. RPC)
					{
						...
						UhtProperty? returnValueProperty = null;
						bool firstArgIsReturnValue = false;
						if (function.FunctionExportFlags.HasAnyFlags(UhtFunctionExportFlags.VerseCallable))
						{
							if (topScope.TokenReader.TryOptional("FVerseResult"))
							{
								firstArgIsReturnValue = true;
								function.FunctionExportFlags |= UhtFunctionExportFlags.VerseSuspends;
								funcNameToken = topScope.TokenReader.GetIdentifier();
							}
						}
						if (!firstArgIsReturnValue)
						{
							UhtPropertyParser.Parse(topScope, EPropertyFlags.None,
								function.GetPropertyParseOptions(true), UhtPropertyCategory.Return,
								(UhtParsingScope topScope, UhtProperty property, ref UhtToken nameToken, UhtLayoutMacroType layoutMacroType) =>
						...
						// If we are parsing a verse callable coroutine, then treat the first argument as the return value
						if (firstArgIsReturnValue && function.Children.Count > 1)
						{
							UhtProperty firstProperty = (UhtProperty)function.Children[0];
							function.Children.RemoveAt(0);
							if (!firstProperty.PropertyExportFlags.HasAnyFlags(UhtPropertyExportFlags.TVerseCall))
							{
								firstProperty.LogError("The first argument for a verse_callable coroutine must be TVerseCall<ReturnType>");
							}
							else if (firstProperty is not UhtVoidProperty)
							{
								firstProperty.PropertyFlags |= EPropertyFlags.Parm | EPropertyFlags.OutParm | EPropertyFlags.ReturnParm;
								returnValueProperty = firstProperty;
							}
						}

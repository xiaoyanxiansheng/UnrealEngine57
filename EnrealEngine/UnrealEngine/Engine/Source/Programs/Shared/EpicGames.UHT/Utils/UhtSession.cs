...
namespace EpicGames.UHT.Utils
{
	...
	public partial class UhtSession : IUhtMessageSite, IUhtMessageSession
	{
		...
		/// <summary>
		/// What format of code to generate for compiled-in UObject instances such as UClass instances
		/// </summary>
		public UhtCompiledInObjectFormat CompiledInObjectFormat => Manifest?.TargetSettings.CompiledInObjectFormat ?? UhtCompiledInObjectFormat.Default;

		/// <summary>
		/// Access to target-wide settings for code generation etc.
		/// </summary>
		public UHTTargetSettings? TargetSettings => Manifest?.TargetSettings;
		...
		/// <summary>
		/// After headers are parsed, returns the UPackage class.
		/// </summary>
		public UhtClass UPackage
		{
			get
			{
				if (_upackage == null)
				{
					throw new UhtIceException("UPackage was not defined.");
				}
				return _upackage;
			}
		}
		...
		/// After headers are parsed, returns the UClass class.
		/// </summary>
		public UhtClass UVerseClass
		{
			get
			{
				if (_uverseclass == null)
				{
					throw new UhtIceException("UVerseClass was not defined.");
				}
				return _uverseclass;
			}
		}
		...
		/// <summary>
		/// After headers are parsed, returns the UScriptStruct class.
		/// </summary>
		public UhtClass UScriptStruct
		{
			get
			{
				if (_uscriptstruct == null)
				{
					throw new UhtIceException("UScriptStruct was not defined.");
				}
				return _uscriptstruct;
			}
		}
		...
		/// After headers are parsed, returns the UScriptStruct class.
		/// </summary>
		public UhtClass UVerseStruct
		{
			get
			{
				if (_uversestruct == null)
				{
					throw new UhtIceException("UVerseStruct was not defined.");
				}
				return _uversestruct;
			}
		}

		/// <summary>
		/// After headers are parsed, returns the UFunction class.
		/// </summary>
		public UhtClass UFunction
		{
			get
			{
				if (_ufunction == null)
				{
					throw new UhtIceException("UFunction was not defined.");
				}
				return _ufunction;
			}
		}
		...
		/// After headers are parsed, returns the UFunction class.
		/// </summary>
		public UhtClass UVerseFunction
		{
			get
			{
				if (_uversefunction == null)
				{
					throw new UhtIceException("UVerseFunction was not defined.");
				}
				return _uversefunction;
			}
		}

		/// <summary>
		/// After headers are parsed, returns the UDelegateFunction class.
		/// </summary>
		public UhtClass UDelegateFunction
		{
			get
			{
				if (_udelegatefunction == null)
				{
					throw new UhtIceException("UDelegateFunction was not defined.");
				}
				return _udelegatefunction;
			}
		}

		/// <summary>
		/// After headers are parsed, returns the USparseDelegateFunction class.
		/// </summary>
		public UhtClass USparseDelegateFunction
		{
			get
			{
				if (_usparsedelegatefunction == null)
				{
					throw new UhtIceException("USparseDelegateFunction was not defined.");
				}
				return _usparsedelegatefunction;
			}
		}

		/// <summary>
		/// After headers are parsed, returns the UEnum class.
		/// </summary>
		public UhtClass UEnum
		{
			get
			{
				if (_uenum == null)
				{
					throw new UhtIceException("UEnum was not defined.");
				}
				return _uenum;
			}
		}
		...
		/// After headers are parsed, returns the UEnum class.
		/// </summary>
		public UhtClass UVerseEnum
		{
			get
			{
				if (_uverseenum == null)
				{
					throw new UhtIceException("UVerseEnum was not defined.");
				}
				return _uverseenum;
			}
		}
		...
		/// <summary>
		/// NoExportTypes.h header in CoreUObject module.
		/// </summary>
		public UhtHeaderFile? NoExportTypesHeader { get; private set; } = null;

		/// <summary>
		/// Number of new/modified files written to disk
		/// </summary>
		public int TotalFilesWritten { get; internal set; }

		private readonly List<UhtModule> _modules = [];
		private readonly List<UhtHeaderFile> _headerFiles = [];
		private readonly List<UhtHeaderFile> _sortedHeaderFiles = [];
		...
		private readonly List<UhtMessage> _messages = [];
		private Task? _messageTask = null;
		private UhtClass? _uobject = null;
		private UhtClass? _upackage = null;
		private UhtClass? _uclass = null;
		private UhtClass? _uverseclass = null;
		private UhtClass? _uscriptstruct = null;
		private UhtClass? _uversestruct = null;
		private UhtClass? _ufunction = null;
		private UhtClass? _uversefunction = null;
		private UhtClass? _udelegatefunction = null;
		private UhtClass? _usparsedelegatefunction = null;
		private UhtClass? _uenum = null;
		private UhtClass? _uverseenum = null;
		...
		private readonly Dictionary<string, EnumAndValue> _fullEnumValueLookup = [];
		private readonly Dictionary<string, UhtEnum> _shortEnumValueLookup = [];
		private JsonDocument? _projectJson = null;
		private readonly ConcurrentDictionary<string, UhtNamespace> _namespaces = [];
		...
		public bool IsUsingCompiledInObjectFormat(UhtCompiledInObjectFormat format) => (CompiledInObjectFormat & format) != UhtCompiledInObjectFormat.None;
		...
		/// </summary>
		public bool IsUsingMultipleCompiledInObjectFormats => Int32.PopCount((int)CompiledInObjectFormat) > 1;
		...
		public UhtType? FindType(UhtType? startingType, UhtFindOptions options, UhtTokenList identifiers, IUhtMessageSite? messageSite = null, int lineNumber = -1)
		{
			...
			// UHT uses a grossly primitive method of resolving namespaces.  We just look at the last two tokens and assume the type will match
			UhtTokenList lookupIdentifiers = identifiers;
			while (lookupIdentifiers.Next != null && lookupIdentifiers.Next.Next != null)
			{
				lookupIdentifiers = lookupIdentifiers.Next;
			}

			UhtType? type;
			if (lookupIdentifiers.Next != null)
			{
				type = FindTypeTwoNamesInternal(startingType, options, lookupIdentifiers.Token.Value.ToString(), lookupIdentifiers.Next.Token.Value.ToString());
			}
			else
			{
				type = FindTypeInternal(startingType, options, lookupIdentifiers.Token.Value.ToString());
			}
		...
		public UhtType? FindType(UhtType? startingType, UhtFindOptions options, UhtToken[] identifiers, IUhtMessageSite? messageSite = null, int lineNumber = -1)
		{
			...
			{
				type = FindTypeTwoNamesInternal(startingType, options, identifiers[^2].Value.ToString(), identifiers[^1].Value.ToString());
			}
			else
			{
				type = FindTypeInternal(startingType, options, identifiers[^1].Value.ToString());
			}

			if (type == null && messageSite != null)
			{
				string fullIdentifier = String.Join("::", identifiers.Select(x => x.Value));
				FindTypeError(messageSite, lineNumber, options, fullIdentifier);
			}
			return type;
		}
		...
		private UhtType? FindTypeTwoNamesInternal(UhtType? startingType, UhtFindOptions options, string firstName, string secondName)
		{
			...
			if (secondName.Length > 0)
			{
				if (options.HasAnyFlags(UhtFindOptions.DelegateFunction | UhtFindOptions.Enum))
				{
					...
					if (type == null)
					{
						return FindTypeInternal(startingType, options, secondName);
					}
					if (type is UhtEnum)
					{
						return type;
				...
				// Just try to match the second name normally
				return FindTypeInternal(startingType, options, secondName);
			}
		...
		/// </summary>
		/// <param name="namespaceElements">Elements of the namespace</param>
		/// <param name="headerNamespaceLookupCache">Header file local version of the namespace cache (to avoid locking)</param>
		/// <returns></returns>
		/// <exception cref="UhtIceException"></exception>
		public UhtNamespace GetNamespace(List<UhtNamespaceElement> namespaceElements, Dictionary<string, UhtNamespace> headerNamespaceLookupCache)
		{
			if (namespaceElements.Count == 0)
			{
				throw new UhtIceException("Namespace stack must have at least one element");
			}

			int index = namespaceElements.Count - 1;
			if (namespaceElements[index].Namespace != null)
			{
				return namespaceElements[index].Namespace!;
			}

			int firstUnresolved = namespaceElements.FindIndex(x => x.Namespace == null);
			StringBuilder builder = new();
			builder.Append(namespaceElements[firstUnresolved - 1].Namespace!.FullSourceName);
			foreach (UhtNamespaceElement namespaceElement in namespaceElements[firstUnresolved..])
			{
				builder.Append(namespaceElement.Name);
				builder.Append("::");
			}

			string fullName = builder.ToString();
			if (headerNamespaceLookupCache.TryGetValue(fullName, out UhtNamespace? namespaceObj))
			{
				return namespaceObj;
			}

			return GetNamespace(namespaceElements, index, builder.ToString(), headerNamespaceLookupCache);
		}

		private UhtNamespace GetNamespace(List<UhtNamespaceElement> namespaceElements, int index, string fullName, Dictionary<string, UhtNamespace> headerNamespaceLookupCache)
		{
			if (!_namespaces.TryGetValue(fullName, out UhtNamespace? namespaceObj))
			{
				if (namespaceElements[index - 1].Namespace == null)
				{
					GetNamespace(namespaceElements, index - 1, fullName.Substring(0, fullName.Length - 2 - namespaceElements[index].Name.Length), headerNamespaceLookupCache);
				}
				namespaceObj = _namespaces.GetOrAdd(fullName, (x) =>
				{
					return new(namespaceElements[index - 1].Namespace, namespaceElements[index].Name.ToString(), fullName, false);
				});
			}
			headerNamespaceLookupCache.TryAdd(fullName, namespaceObj);
			namespaceElements[index] = new(namespaceElements[index].Name, namespaceObj);
			return namespaceObj;
		}
		...
		private void StepPrepareHeaders()
		{
			...
			Try(ManifestFile.MessageSource, () =>
			{
				...
				{
					NoExportTypesHeader = noExportTypes;
					foreach (UhtHeaderFile headerFile in _headerFiles)
					{
		...
		private void StepParseHeaders()
		{
			...
			UhtNamespace globalNamespace = new UhtNamespace(null, "", "", true);
			_namespaces.TryAdd("", globalNamespace);
			ForEachHeader(headerFile =>
			{
				headerFile.Read();
				UhtHeaderFileParser.Parse(headerFile, globalNamespace);
			}, true);
		}
		...
		private void StepPopulateTypeTable()
		{
			Try(null, () =>
			{
				...
				_upackage = (UhtClass?)FindType(null, UhtFindOptions.SourceName | UhtFindOptions.Class, "UPackage");
				_uclass = (UhtClass?)FindType(null, UhtFindOptions.SourceName | UhtFindOptions.Class, "UClass");
				_uverseclass = (UhtClass?)FindType(null, UhtFindOptions.SourceName | UhtFindOptions.Class, "UVerseClass");
				_uscriptstruct = (UhtClass?)FindType(null, UhtFindOptions.SourceName | UhtFindOptions.Class, "UScriptStruct");
				_uversestruct = (UhtClass?)FindType(null, UhtFindOptions.SourceName | UhtFindOptions.Class, "UVerseStruct");
				_ufunction = (UhtClass?)FindType(null, UhtFindOptions.SourceName | UhtFindOptions.Class, "UFunction");
				_uversefunction = (UhtClass?)FindType(null, UhtFindOptions.SourceName | UhtFindOptions.Class, "UVerseFunction");
				_udelegatefunction = (UhtClass?)FindType(null, UhtFindOptions.SourceName | UhtFindOptions.Class, "UDelegateFunction");
				_usparsedelegatefunction = (UhtClass?)FindType(null, UhtFindOptions.SourceName | UhtFindOptions.Class, "USparseDelegateFunction");
				_uenum = (UhtClass?)FindType(null, UhtFindOptions.SourceName | UhtFindOptions.Class, "UEnum");
				_uverseenum = (UhtClass?)FindType(null, UhtFindOptions.SourceName | UhtFindOptions.Class, "UVerseEnum");

...
namespace EpicGames.UHT.Types
{
	...
	public abstract class UhtStruct : UhtField
	{
		...
		/// <inheritdoc/>
		public override string EngineClassName => IsVerseField ? "VerseStruct" : "Struct";
		...
		/// <summary>
		/// Super struct type for inheritance, i.e. functions do not inherit properties from their Super
		/// </summary>
		[JsonIgnore]
		public virtual UhtStruct? InheritanceSuperStruct => Super;

		/// <summary>
		/// After properties are fully resolved, points to the first property in this struct
		/// </summary>
		public UhtDefineScopeLink<UhtProperty>? FirstProperty { get; set; }

		/// <summary>
		/// For classes, struct type for sparse class data shared between all instances
		/// </summary>
		public UhtScriptStruct? SparseClassDataStruct { get; set; }
		...
		protected UhtStruct(UhtHeaderFile headerFile, UhtNamespace namespaceObj, UhtType outer, int lineNumber) : base(headerFile, namespaceObj, outer, lineNumber)
		{
		}
		...
		protected override void ResolveChildren(UhtResolvePhase phase)
		{
			...
			{
				// Build a linked list of each property of this struct/class/function
				using (UhtDefineScopeListBuilder<UhtProperty> propertyList = new(DefineScope))
				{
					foreach (UhtType type in Children)
					{
						if (type is UhtProperty property)
						{
							...
							property.NextProperty = propertyList.Add(property);
						}
					}
					FirstProperty = propertyList.Head;
				}
			}
		}
		...
		{
			if (superIdentifier.Length > 0)
			{
				Super = (UhtStruct?)FindType(findOptions | UhtFindOptions.SourceName | UhtFindOptions.NoSelf, superIdentifier);
				if (Super == null)
				{
					string fullIdentifier = String.Join("::", superIdentifier.Select(x => x.Value));
					throw new UhtException(this, $"Unable to find parent {EngineType.ShortLowercaseText()} type for '{SourceName}' named '{fullIdentifier}'");
				}
		...
		private void ValidateSparseClassData()
		{
			...
			{
				SparseClassDataStruct = Session.FindType(null, UhtFindOptions.EngineName | UhtFindOptions.ScriptStruct, sparseClassDataTypeName) as UhtScriptStruct;

				// make sure the sparse class data struct actually exists
				if (SparseClassDataStruct  == null)
				{
				...
				foreach (UhtProperty property in SparseClassDataStruct  .Properties)
				{
					if (property.PropertyFlags.HasAnyFlags(EPropertyFlags.BlueprintAssignable))
					{
						property.LogError($"Sparse class data types can not contain blueprint assignable delegates. Type '{SparseClassDataStruct  .EngineName}' Delegate '{property.SourceName}'");
					}
					...
					{
						property.LogError($"Sparse class data types must be VisibleDefaultsOnly or EditDefaultsOnly. Type '{SparseClassDataStruct  .EngineName}' Property '{property.SourceName}'");
					}
					...
					{
						property.LogError($"Sparse class data types must not be BlueprintReadWrite. Type '{SparseClassDataStruct  .EngineName}' Property '{property.SourceName}'");
					}
				}
				...
				if (superClass != null)
				{
					...
					if (superSparseClassDataTypes != null)
					{
						foreach (string superSparseClassDataTypeName in superSparseClassDataTypes)
						{
							...
							{
								if (!SparseClassDataStruct  .IsChildOf(superSparseScriptStruct))
								{
									this.LogError(
										$"Class '{SourceName}' is a child of '{superClass.SourceName}' but its sparse class data struct " +
										$"'{SparseClassDataStruct  .EngineName}', does not inherit from '{superSparseScriptStruct.EngineName}'.");
								}
							}
						}
					}
		...
		public override void CollectReferences(IUhtReferenceCollector collector)
		{
			...
			collector.AddCrossModuleReference(SparseClassDataStruct, UhtSingletonType.ConstInit);
		}
	}
}

	...
	public class UhtBoolProperty : UhtProperty
	{
		...
		public override StringBuilder AppendMemberDef(StringBuilder builder, IUhtPropertyMemberContext context, string name, string nameSuffix, string? offset, int tabs)
		{
			if (Outer == context.OuterStruct)
			{
				...
				builder.AppendTabs(tabs + 1).Append("((").Append(context.OuterStructNamespaceName).Append(context.OuterStructSourceName).Append("*)Obj)->").Append(SourceName).Append(" = 1;\r\n");
				builder.AppendTabs(tabs).Append("}\r\n");
			}
			...
			{
				builder
					.Append("sizeof(").Append(context.OuterStructNamespaceName).Append(context.OuterStructSourceName).Append("), ")
		...
		public override StringBuilder AppendConstInitMemberDef(StringBuilder builder, IUhtPropertyMemberContext context, string name, string nameSuffix, Action<StringBuilder>? outerFunc, string? offset, int tabs)
		{
			...
			AppendConstInitMemberDefStart(builder, context, name, nameSuffix, outerFunc, IsBitfield ? "0" : offset, tabs);
			builder.Append("sizeof(").Append(CppTypeText).Append("), ");
			builder.Append(IsNativeBool ? "true, " : "false, ");
			if (Outer == context.OuterStruct)
			{
				builder.Append('&').AppendNameDef(context, name, nameSuffix).Append("_SetBit, ");
			}
			else
			{

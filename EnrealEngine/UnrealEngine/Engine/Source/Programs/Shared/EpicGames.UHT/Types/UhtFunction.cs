...
namespace EpicGames.UHT.Types
{
	...
	public enum UhtFunctionExportFlags : int
	{
		...
		/// <summary>
		/// True if the function is an extension method
		/// </summary>
		VerseExtensionMethod = 1 << 24,
	}
	...
	public class UhtFunction : UhtStruct
	{
		...
		/// </summary>
		[JsonIgnore]
		public override UhtStruct? InheritanceSuperStruct => null;
		...
		/// <summary>
		/// After functions are fully resolved, points to the next function in the owning struct in each define scope.
		/// </summary>
		public UhtDefineScopeLink<UhtFunction>? NextFunction { get; set; }
			...
			get
			{
				switch (FunctionType)
				{
					case UhtFunctionType.Function:
						return UhtEngineType.Function;
					case UhtFunctionType.Delegate:
						return UhtEngineType.Delegate;
					case UhtFunctionType.SparseDelegate:
						return UhtEngineType.SparseDelegate;
					default:
		...
		public override string EngineClassName
		{
			get
			{
				...
				{
					case UhtFunctionType.Function:
						return IsVerseField ? "VerseFunction" : "Function";
					case UhtFunctionType.Delegate:
		...
		/// <inheritdoc/>
		public override string EngineLinkClassName => "Function";

		/// <inheritdoc/>
		public override UhtClass? EngineClass
		{
			get
			{
				switch (FunctionType)
				{
					case UhtFunctionType.Function:
						return IsVerseField ? Session.UVerseFunction : Session.UFunction;
					case UhtFunctionType.Delegate:
						return Session.UDelegateFunction;
					case UhtFunctionType.SparseDelegate:
						return Session.USparseDelegateFunction;
					default:
						throw new UhtIceException("Invalid function type");
				}
			}
		...
		public UhtFunction(UhtHeaderFile headerFile, UhtNamespace namespaceObj, UhtType outer, int lineNumber) : base(headerFile, namespaceObj, outer, lineNumber)
		{
		}
		...
		/// <inheritdoc/>
		public override void AppendVerseScopeAndName(StringBuilder builder, UhtVerseNameMode mode)
		{
			...
			{
				base.AppendVerseScopeAndName(builder, mode);
			}
			else
			{
			...
			if (!parameterProperties.IsEmpty)
			{
				...
				foreach (UhtType type in parameterProperties.Span)
				{
					if (type is UhtProperty property)
					{
						...
						if (property.DefaultValueTokens != null || property.PropertyExportFlags.HasAnyFlags(UhtPropertyExportFlags.VerseDefaultValue))
						{
							builder.Append(" = ...");
						}
						isFirst = false;
			...
			switch (FunctionType)
			{
				case UhtFunctionType.Function:
					...
					if (IsVerseField)
					{
						...
						if (GetVerseScopeAndName(UhtVerseNameMode.Default) == SourceName)
						{
		...
		public override void CollectReferences(IUhtReferenceCollector collector)
		{
			...
			{
				case UhtFunctionType.Function:
					collector.AddCrossModuleReference(Session.UFunction, UhtSingletonType.ConstInit);
					collector.AddCrossModuleReference(this, UhtSingletonType.ConstInit);
					collector.AddCrossModuleReference(Package, UhtSingletonType.ConstInit);
					break;
				case UhtFunctionType.Delegate:
					collector.AddCrossModuleReference(Session.UDelegateFunction, UhtSingletonType.ConstInit);
					break;
				case UhtFunctionType.SparseDelegate:
					collector.AddCrossModuleReference(Session.USparseDelegateFunction, UhtSingletonType.ConstInit);
					break;
				default:
					break;
			}

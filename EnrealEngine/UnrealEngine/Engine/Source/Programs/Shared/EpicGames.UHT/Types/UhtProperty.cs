...
namespace EpicGames.UHT.Types
{
	...
	/// </summary>
	public interface IUhtPropertyMemberContext : IUhtSingletonNameProvider, IUhtSessionSettingsProvider
	{
		/// <summary>
		...
		/// <summary>
		/// The outer/owning structure namespace name.  In some cases, this can differ from the OuterStruct.Namespace.FullName
		/// </summary>
		public string OuterStructNamespaceName { get; }
...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
	...
	public abstract class UhtProperty : UhtType
	{
		...
		{
			return builder.AppendMetaDataDefinition(this, context, context.NamePrefix, name, nameSuffix, context.MetaDataSuffix, tabs);
		}
		...
		/// For comound properties this must be overridden to declare the inner/key/value property.
		/// </summary>
		/// <param name="builder">Output builder</param>
		/// <param name="context">Current context</param>
		/// <param name="name">Name of the property.  This is needed in some cases where the name in the declarations doesn't match the property name.</param>
		...
		/// <param name="tabs">Number of tabs prefix the line with</param>
		/// <returns>Output builder</returns>
		public virtual StringBuilder AppendConstInitMemberDecl(StringBuilder builder, IUhtPropertyMemberContext context, string name, string nameSuffix, int tabs)
		{
			string? typePrefix = HasGetterOrSetter ? "TPropertyWithSetterAndGetter<" : null;
			string? typeSuffix = HasGetterOrSetter ? ">" : null;
			return builder.AppendTabs(tabs).Append($"static UE_CONSTINIT_UOBJECT_DECL TNoDestroy<{typePrefix}F{EngineClassName}{typeSuffix}> ").AppendNameDecl(context, name, nameSuffix).Append(";\r\n");
		}
		...
		/// <param name="tabs">Number of tabs prefix the line with</param>
		/// <returns>Output builder</returns>
		public abstract StringBuilder AppendConstInitMemberDef(StringBuilder builder, IUhtPropertyMemberContext context, string name, string nameSuffix, Action<StringBuilder>? outerFunc, string? offset, int tabs);
			...
			string paramsStructName, string paramsGenFlags, bool appendOffset = true)
		{
			...
			if (appendOffset)
			{
				...
				{
					builder.Append("STRUCT_OFFSET(").Append(context.OuterStructNamespaceName).Append(context.OuterStructSourceName).Append(", ").Append(SourceName).Append("), ");
				}
			}
			return builder;
		}
		...
		/// <param name="tabs">Number of tabs prefix the line with</param>
		/// <returns>Output builder</returns>
		public StringBuilder AppendConstInitMemberDefStart(StringBuilder builder, IUhtPropertyMemberContext context, string name, string nameSuffix, Action<StringBuilder>? outerFunc, string? offset, int tabs)
		{
			string? typePrefix = HasGetterOrSetter ? "TPropertyWithSetterAndGetter<" : null;
			string? typeSuffix = HasGetterOrSetter ? ">" : null;
			builder.AppendTabs(tabs)
			 	.Append($"UE_CONSTINIT_UOBJECT_DECL TNoDestroy<{typePrefix}F{EngineClassName}{typeSuffix}> ")
				.AppendNameDef(context, name, nameSuffix).Append("{NoDestroyConstEval, ");
			...
			builder.Append($"UE::CodeGen::ConstInit::FPropertyParams{{ .FieldClass = &F{EngineClassName}::ConstInitClass, .Owner = ");
			if (outerFunc is not null)
			{
				outerFunc(builder);
				builder.Append(", ");
			}
			else if (Outer is UhtObject obj)
			{
			...
			if (NextProperty is not null)
			{
				...
				if (NextProperty!.Next.Count > 1)
				{
					...
					builder.Append($"GetNextProperty_{EngineName}(), ");
				}
				else 
				{
					...
					UhtProperty? next = NextProperty!.Next[0];
					builder.AppendConstInitMemberPtr(next, context, next?.EngineName, "", 0, ", ");
				}
			}
			builder.Append(".NameUTF8 = UTF8TEXT(").AppendUTF8LiteralString(EngineName).Append("), ");
			if (RepNotifyName is not null)
			{
				builder.Append(".RepNotifyFuncUTF8 = UTF8TEXT(").AppendUTF8LiteralString(RepNotifyName).Append("), ");
			}
			builder.Append(".PropertyFlags = ").AppendFlags(PropertyFlags).Append(", ");
			builder.Append(".ObjectFlags = ").Append(ObjectFlags).Append(", ");

			builder.Append(".ArrayDim = ").AppendArrayDim(this, context).Append(", ");
			...
			{
				builder.Append($".Offset = STRUCT_OFFSET({context.OuterStructNamespaceName}{context.OuterStructSourceName}, {SourceName}), ");
			}
			...
			{
				builder.Append("IF_WITH_METADATA(.MetaData = MakeConstArrayView(").AppendNameDef(context, name, nameSuffix).Append($"{context.MetaDataSuffix}),)");
			}
			builder.Append("}, ");
			return builder;
		}
		...
		/// <param name="context">Context of the call</param>
		/// <returns>Output builder</returns>
		protected StringBuilder AppendConstInitMemberDefEnd(StringBuilder builder, IUhtPropertyMemberContext context)
		{
			builder
				.Append(" };")
		...
		/// <returns>Output builder</returns>
		protected static StringBuilder AppendMemberDefRef(StringBuilder builder, IUhtPropertyMemberContext context, UhtObject? obj, UhtSingletonType type, bool appendNull = false)
		{
			if (obj != null)
			{
		...
		/// <param name="tabs">Number of tabs prefix the line with</param>
		/// <returns>Output builder</returns>
		public virtual StringBuilder AppendConstInitMemberPtr(StringBuilder builder, IUhtPropertyMemberContext context, string name, string nameSuffix, int tabs)
		{
			builder.AppendTabs(tabs).Append('&').AppendNameDef(context, name, nameSuffix);
			return builder;
		}
		...
		public StringBuilder AppendGetNextProperty(StringBuilder builder, IUhtPropertyMemberContext context, int tabs)
		{
			if (NextProperty?.Next?.Count > 1)
			{
				...
				builder.AppendScopeLink(NextProperty, UhtDefineScopeNames.Standard, context, tabs + 1, "return ", ";\r\n");
		...
		public virtual void Validate(UhtStruct outerStruct, UhtProperty outermostProperty, UhtValidationOptions options)
		{
			...
			{
				this.LogError($"Property type '{outermostProperty.TypeTokens.ToString()}' isn't support as a Verse type");
			}
		...
		public override void CollectReferences(IUhtReferenceCollector collector)
		{
			...
			{
				collector.AddCrossModuleReference(outerObject, UhtSingletonType.ConstInit);
			}
		}
	...
	public static class UhtPropertyStringBuilderExtensions
	{
		...
		/// <param name="tabs">Number of tabs in the formatting</param>
		/// <returns>Destination builder</returns>
		public static StringBuilder AppendConstInitMemberDecl(this StringBuilder builder, UhtProperty property, IUhtPropertyMemberContext context, string name, string nameSuffix, int tabs)
		{
			return property.AppendConstInitMemberDecl(builder, context, name, nameSuffix, tabs);
		}
		...
		/// <param name="tabs">Number of tabs in the formatting</param>
		/// <returns>Destination builder</returns>
		public static StringBuilder AppendConstInitMemberDef(this StringBuilder builder, UhtProperty property, IUhtPropertyMemberContext context, string name, string nameSuffix, Action<StringBuilder>? outerFunc, string? offset, int tabs)
		{
			return property.AppendConstInitMemberDef(builder, context, name, nameSuffix, outerFunc, offset, tabs);
		}
		...
		/// <returns>Destination builder</returns>
		public static StringBuilder AppendConstInitMemberPtr(this StringBuilder builder, UhtProperty? property, IUhtPropertyMemberContext context, string? name, string nameSuffix, int tabs, string? post)
		{
			if (property is null)
			{
				return builder.Append("nullptr").Append(post);
			}
			return property.AppendConstInitMemberPtr(builder, context, name ?? property.EngineName, nameSuffix, tabs).Append(post);
		}
		...
		public static StringBuilder AppendArrayDim(this StringBuilder builder, UhtProperty property, IUhtPropertyMemberContext context)
		{
			...
			{
				builder.Append("CPP_ARRAY_DIM(").Append(property.SourceName).Append(", ").Append(context.OuterStructNamespaceName).Append(context.OuterStruct.SourceName).Append(')');
			}
			else
			{
				builder.Append('1');

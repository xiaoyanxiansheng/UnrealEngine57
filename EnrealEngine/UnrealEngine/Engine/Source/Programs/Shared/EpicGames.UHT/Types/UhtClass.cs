...
namespace EpicGames.UHT.Types
{
	...
	public enum UhtClassType
	{
		...
		/// <summary>
		/// Class is an implementation detail for a Verse module
		/// </summary>
		VModule,
	}
	...
	public class UhtClass : UhtStruct
	{
		...
		/// <summary>
		/// Class flags used to populate UMyClass::StaticClassFlags. 
		/// Note that these may be different to those in the instance field UClass::ClassFlags which
		/// does not respect removing class flags in a derived class.
		/// </summary>
		[JsonConverter(typeof(JsonStringEnumConverter))]
		...
		public override UhtEngineType EngineType
		{
			get
			{
				switch (ClassType)
				{
					...
					default:
						throw new UhtIceException($"UhtEngineType: Unknown class type {ClassType}");
				}
			}
		}

		/// <inheritdoc/>
		public override string EngineClassName => IsVerseField ? "VerseClass" : "Class";

		/// <inheritdoc/>
		public override string EngineLinkClassName => "Class";

		/// <inheritdoc/>
		public override UhtClass? EngineClass => IsVerseField ? Session.UVerseClass : Session.UClass;

		private string? _nativeFunctionCallName;

		/// <summary>
		/// The name used to scope native function calls into this type e.g. UClassName or IInterfaceName
		/// </summary>
		public string NativeFunctionCallName => _nativeFunctionCallName ??= ClassType switch
		{
			UhtClassType.Class or UhtClassType.NativeInterface or UhtClassType.VModule => SourceName,
			UhtClassType.Interface => $"I{SourceName[1..]}",
			var unknown => throw new UhtIceException($"NativeFunctionCallName: Unknown class type {unknown}"),
		};
		...
		protected override UhtSpecifierValidatorTable? SpecifierValidatorTable
		{
			get
			{
				switch (ClassType)
				{
					...
					case UhtClassType.VModule:
						return Session.GetSpecifierValidatorTable(UhtTableNames.VModule);
					default:
						throw new UhtIceException($"SpecifierValidatorTable: Unknown class type {ClassType}");
				}
			}
		}
		...
		public UhtClass(UhtHeaderFile headerFile, UhtNamespace namespaceObj, UhtType outer, int lineNumber) : base(headerFile, namespaceObj, outer, lineNumber)
		{
			ClassWithin = this;
		}
		...
		{
			if (this != Session.UObject) // Don't try to find UObjectBaseUtility
			{
			...
			switch (resolvePhase)
			{
				case UhtResolvePhase.Bases:
					...
					switch (ClassType)
					{
						...
						case UhtClassType.VModule:
							ClassWithin = superClass != null ? superClass.ClassWithin : Session.UObject;
							break;
		...
		protected override bool ResolveSelf(UhtResolvePhase phase)
		{
			...
			switch (phase)
			{
				...
				case UhtResolvePhase.Bases:
					...
					// Also for legacy UObject-derived property types, try replacing the U with F. e.g. UByteProperty -> FByteProperty
					if (Enum.TryParse<EClassCastFlags>(SourceName, false, out EClassCastFlags found) || Enum.TryParse($"F{SourceName[1..]}", false, out found))
					{
		...
		{
			List<UhtFunction> functions = new();
...
#pragma warning restore IDE0059 // Unnecessary assignment of a value
		...
		protected override UhtValidationOptions Validate(UhtValidationOptions options)
		{
			...
			else
			{
				...
				else if (ClassType == UhtClassType.Class && this != Session.UObject)
				{
					this.LogError($"UClass '{SourceName}' should derive from UObject or another UClass");
				}
			}
			...
			foreach (UhtType child in Children)
			{
				if (child is UhtProperty property)
				{
					...
					if (ClassFlags.HasAnyFlags(EClassFlags.Optional))
					{
						...
						{
							property.LogError("Cannot specify an editor only property inside an optional class (adding \"AllowedInOptional\" metadata will not remove this error, as this is a design issue).");
						}
						else if (!property.IsAllowedInOptionalClass(out string? propPath))
						{
							property.LogError($"Do not specify struct property ({propPath}) containing editor only properties inside an optional class. If it's needed, add \"AllowedInOptional\" metadata to the property.");
						}
					}
		...
		public override void CollectReferences(IUhtReferenceCollector collector)
		{
			...
			base.CollectReferences(collector);

			// Add myself as declarations and cross module references
			collector.AddDeclaration(this, UhtSingletonType.Unregistered);
			collector.AddDeclaration(this, UhtSingletonType.Registered);
			collector.AddCrossModuleReference(this, UhtSingletonType.Unregistered);
			collector.AddCrossModuleReference(this, UhtSingletonType.Registered);

			// Add the super class
			if (SuperClass != null)
			{
				collector.AddDeclaration(SuperClass, UhtSingletonType.Registered);
				collector.AddCrossModuleReference(SuperClass, UhtSingletonType.Registered);
			}
			...
			foreach (UhtStruct baseObj in Bases)
			{
				...
				{
					collector.AddCrossModuleReference(baseClass, UhtSingletonType.Unregistered);
				}
			}
			...
			{
				collector.AddCrossModuleReference(interfaceObj, UhtSingletonType.Unregistered);
			}

			// Add the package
			collector.AddDeclaration(Package, UhtSingletonType.Registered);
			collector.AddCrossModuleReference(Package, UhtSingletonType.Registered);

			// Pointers that are needed for constinit only
			collector.AddCrossModuleReference(Session.UClass, UhtSingletonType.ConstInit);
			collector.AddCrossModuleReference(ClassWithin, UhtSingletonType.ConstInit);
			// Add all parents for constinit initializion of bases array 
			for (UhtClass? super = SuperClass; super is not null; super = super.SuperClass)
			{
				collector.AddCrossModuleReference(super, UhtSingletonType.ConstInit);
			}
		...
		private void BindVerseInterfaces()
		{
			if (VerseInterfaceIdentifiers != null)
			{
				foreach (UhtToken[] identifier in VerseInterfaceIdentifiers)
				{
					...
					UhtClass? verseInterface = (UhtClass?)FindType(UhtFindOptions.Class | UhtFindOptions.SourceName | UhtFindOptions.NoSelf, identifier);
					...
					{
						string fullIdentifier = String.Join("::", identifier.Select(x => x.Value));
						this.LogError($"Verse interface {fullIdentifier} not found");
					}
				}
			}
		}

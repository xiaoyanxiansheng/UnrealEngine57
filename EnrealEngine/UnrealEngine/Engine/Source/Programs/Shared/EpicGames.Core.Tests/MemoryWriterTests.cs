...
namespace EpicGames.Core.Tests
{
	...
	public class MemoryWriterTests
	{
		...
		{
			class Allocation(MemoryWriterTests.TestAllocator allocator, Memory<byte> memory) : IMemoryOwner<byte>
			{
				TestAllocator? _allocator = allocator;
				...
				public void Dispose()
				{
					if (_allocator != null)
					{
						...
						Interlocked.Add(ref _allocator._allocatedSize, -memory.Length);
						_allocator = null;
					}
				}
			}

...
namespace EpicGames.Horde.Storage.Nodes
{
	...
	public sealed class LeafChunkedDataWriter : IDisposable
	{
		...
		{
			IoHash hash = IoHash.FromBlake3(_hasher);
			await FlushLeafNodeAsync(cancellationToken);
			LeafChunkedData leafChunkedData = new LeafChunkedData(hash, new List<ChunkedDataNodeRef>(_leafHandles));
	...
	{
		static readonly ChunkingOptions s_defaultChunkingOptions = new ChunkingOptions();

		/// <summary>
		/// Creates chunked data from a block of memory
		/// </summary>
		/// <param name="blobWriter">Writer for output blobs</param>
		/// <param name="data">Data to create from</param>
		/// <param name="cancellationToken">Cancellation token for the operation</param>
		public static Task<ChunkedData> CreateAsync(IBlobWriter blobWriter, ReadOnlyMemory<byte> data, CancellationToken cancellationToken = default)
			=> CreateAsync(blobWriter, data, s_defaultChunkingOptions, cancellationToken);
		...
		/// <param name="blobWriter">Writer for output blobs</param>
		/// <param name="data">Data to create from</param>
		/// <param name="chunkingOptions">Options controlling the chunking</param>
		/// <param name="cancellationToken">Cancellation token for the operation</param>
		public static async Task<ChunkedData> CreateAsync(IBlobWriter blobWriter, ReadOnlyMemory<byte> data, ChunkingOptions chunkingOptions, CancellationToken cancellationToken = default)
		{
			using ChunkedDataWriter chunkedWriter = new ChunkedDataWriter(blobWriter, chunkingOptions);
			await chunkedWriter.AppendAsync(data, cancellationToken);
			return await chunkedWriter.FlushAsync(cancellationToken);
		}
	}

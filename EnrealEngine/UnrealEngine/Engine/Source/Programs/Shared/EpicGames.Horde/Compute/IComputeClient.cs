...
namespace EpicGames.Horde.Compute
{
	...
	public interface IComputeClient
	{
		...
		/// <param name="requirements">Requirements for the agent</param>
		/// <param name="requestId">Optional ID identifying the request over multiple calls, such as retrying the same request</param>
		/// <param name="useUbaCache">Whether to allocate an Unreal Build Accelerator (UBA) cache server session for this compute request</param>
		/// <param name="connection">Optional preference of connection details</param>
		/// <param name="logger">Logger for output from this worker</param>
		/// <param name="cancellationToken">Cancellation token for the operation</param>
		public Task<IComputeLease?> TryAssignWorkerAsync(ClusterId? clusterId, Requirements? requirements, string? requestId, ConnectionMetadataRequest? connection, bool? useUbaCache, ILogger logger, CancellationToken cancellationToken = default);
		...
		/// <summary>
		/// Allocate a session for the UBA cache associated with the given cluster
		/// </summary>
		/// <param name="clusterId">Cluster to execute the request</param>
		/// <param name="cancellationToken">Cancellation token for the operation</param>
		public Task<UbaConfig> AllocateUbaCacheServerAsync(ClusterId clusterId, CancellationToken cancellationToken = default);
	}
	...
	public static class ComputeClientExtensions
	{
		...
		public static Task<IComputeLease?> TryAssignWorkerAsync(this IComputeClient computeClient, ClusterId clusterId, Requirements? requirements, string? requestId, ConnectionMode? connectionPreference, ILogger logger, CancellationToken cancellationToken = default)
		{
			...
			return computeClient.TryAssignWorkerAsync(clusterId, requirements, requestId, cmr, null, logger, cancellationToken);
		}
		...
		{
			return computeClient.TryAssignWorkerAsync(clusterId, requirements, null, null, logger, cancellationToken);
		}

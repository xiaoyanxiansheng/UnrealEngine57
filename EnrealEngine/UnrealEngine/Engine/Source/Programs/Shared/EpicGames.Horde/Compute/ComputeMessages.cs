...
namespace EpicGames.Horde.Compute
{
	...
	/// <summary>
	/// Request for allocating a UBA cache server/session
	/// </summary>
	public class AllocateUbaCacheServerRequest
	{
	}
	
	/// <summary>
	/// Configuration settings for Unreal Build Accelerator (UBA) for a compute task invocation
	/// Contains cache server connectivity and other UBA-related settings.
	/// </summary>
	public class UbaConfig(string cacheEndpoint, string cacheSessionKey, bool writeAccess)
	{
		/// <summary>
		/// Network endpoint (IP address and port) for connecting to the UBA cache server
		/// </summary>
		public string CacheEndpoint { get; } = cacheEndpoint;
		
		/// <summary>
		/// AES encryption key used for secure client-server communication with the UBA cache server
		/// </summary>
		public string CacheSessionKey { get; } = cacheSessionKey;
		
		/// <summary>
		/// Session has write access to cache
		/// </summary>
		public bool WriteAccess { get; } = writeAccess;
	}
	...
	public class AssignComputeRequest
	{
		...
		/// <summary>
		/// Whether to allocate an Unreal Build Accelerator (UBA) cache server session for this compute request.
		/// If requested, a UBA cache server must be defined for the compute cluster. Null is treated as false.
		/// </summary>
	...
	public class AssignComputeResponse
	{
		...
		/// <inheritdoc cref="UbaConfig" />
		public UbaConfig? Uba { get; set; }

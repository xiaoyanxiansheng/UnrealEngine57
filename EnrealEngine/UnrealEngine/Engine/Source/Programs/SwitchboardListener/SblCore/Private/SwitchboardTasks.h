...
{
	FSwitchboardKillTask(const FGuid& InTaskId, const FIPv4Endpoint& InEndpoint, const FGuid& InProgramID, float InSoftKillTimeout = -1.0f)
		: FSwitchboardTask{ ESwitchboardTaskType::Kill, InTaskId, InEndpoint}
		, ProgramID(InProgramID)
		, SoftKillTimeout(InSoftKillTimeout)
	{}

	// unique ID of process to kill
	FGuid ProgramID;

	/** Time after a soft termination signal before issuing a hard termination signal. */
	float SoftKillTimeout = 2.0f;
	...
	virtual uint32 GetEquivalenceHash() const override
	{
		// Ignore the SoftKillTimeout for this.  A new kill command of the same program with a different 
		// timeout should be considered a duplicate.

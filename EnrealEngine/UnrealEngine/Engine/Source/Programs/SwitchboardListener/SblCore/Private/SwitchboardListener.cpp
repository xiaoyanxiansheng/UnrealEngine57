...
struct FRunningProcess
{
	...
	void* StderrParentReadPipe = nullptr;
	void* StderrChildWritePipe = nullptr;
	...
	bool bCombinedStdoutStderr = true;
	bool bLockGpuClock = false;

	FTSSimpleMulticastDelegate OnProcessEnded;
	...
	bool CreatePipes()
	{
		...
		if (!bCombinedStdoutStderr)
		{
			if (!FPlatformProcess::CreatePipe(StderrParentReadPipe, StderrChildWritePipe))
			{
				UE_LOG(LogSwitchboard, Error, TEXT("Could not create pipe to read process stderr!"));
				return false;
			}
		}
	...
	void ClosePipes()
	{
		...
		FPlatformProcess::ClosePipe(StderrParentReadPipe, StderrChildWritePipe);
...
#if PLATFORM_WINDOWS
	// If we've made it this far, we were able to bind the listen endpoint successfully,
	// which precludes interfering with any other running instance.
	HarvestExistingFlipModeSessions();
#endif
...
bool FSwitchboardListener::Task_StartProcess(const FSwitchboardStartTask& InRunTask)
{
	...
	NewProcess->bCombinedStdoutStderr = InRunTask.bCombinedStdoutStderr;
	...
	NewProcess->Handle = FSwitchboardProcess::CreateProc(
		*InRunTask.Command, 
		*InRunTask.Arguments, 
		bLaunchDetached, 
		bLaunchHidden, 
		...
		InRunTask.bCombinedStdoutStderr ? NewProcess->StdoutChildWritePipe : NewProcess->StderrChildWritePipe
	);
...
bool FSwitchboardListener::Task_KillProcess(const FSwitchboardKillTask& KillTask)
{
	...
	MessageFuture.Future = Async(EAsyncExecution::ThreadPool, [this, KillTask, Process, FlipModeMonitor, UUID]() {
		...
		const float SoftKillTimeout = FMath::Max(0.0f, KillTask.SoftKillTimeout);
...
/**
 * Recursively collects the PIDs of all child processes of a given root PID.
 */
static void WindowsGatherChildPIDs(uint32 RootPID, TArray<uint32>& OutPIDs)
{
	...
	if (Process32First(SnapshotHandle, &ProcessEntry))
	{
		do
		{
			if (ProcessEntry.th32ParentProcessID == RootPID)
			{
				OutPIDs.Add(ProcessEntry.th32ProcessID);
				WindowsGatherChildPIDs(ProcessEntry.th32ProcessID, OutPIDs); // Recursion
			}
...
/** 
 * Soft-terminates a process by PID, excluding its process tree. 
 */
static bool WindowsSoftTerminateByPID(uint32 PID)
{
	const FString Params = FString::Printf(TEXT("/PID %u"), PID);

	FString StdOut;
	int32 ReturnCode = 0;

	FPlatformProcess::ExecProcess(TEXT("TASKKILL"), *Params, &ReturnCode, &StdOut, nullptr);
...
/**
 * Terminate a process tree by first sending individual soft sins to each PID,
 * waiting for up to SoftKillTimeout seconds, then forcefully terminating survivors.
 */
bool WindowsKillProcessNow(FRunningProcess* InProcess, float SoftKillTimeout)
{
	check(InProcess && InProcess->Handle.IsValid()); // We expect this was checked before calling this platform specific function.

	// Gather all PIDs to terminate, including the root.

	TArray<uint32> AllPIDs;
	AllPIDs.Add(InProcess->PID);

	WindowsGatherChildPIDs(InProcess->PID, AllPIDs);

	// Soft terminate each one

	for (uint32 PID : AllPIDs)
	{
		WindowsSoftTerminateByPID(PID);
	}

	// Wait for them to terminate, up to the given timeout

	const double DeadlineTime = FPlatformTime::Seconds() + SoftKillTimeout;

	while (FPlatformTime::Seconds() < DeadlineTime)
	{
		bool bAnyStillRunning = false;

		for (uint32 PID : AllPIDs)
		{
			const HANDLE ProcessHandle = ::OpenProcess(SYNCHRONIZE, false, PID);
	...
	// Hard‐terminate surviving PIDs

	for (uint32 PID : AllPIDs)
	{
		FProcHandle TermHandle = FPlatformProcess::OpenProcess(PID);

		if (TermHandle.IsValid())
		{
			if (FPlatformProcess::IsProcRunning(TermHandle))
			{
				FPlatformProcess::TerminateProc(TermHandle, false /* bKillTree */);
			}
			FPlatformProcess::CloseProc(TermHandle);
		}
	}
...
bool FSwitchboardListener::KillProcessNow(FRunningProcess* InProcess, float SoftKillTimeout)
{
	...
	return WindowsKillProcessNow(InProcess, SoftKillTimeout);

#else

	if (FPlatformProcess::IsProcRunning(InProcess->Handle))
	{
		FPlatformProcess::TerminateProc(InProcess->Handle, true /* bKillTree */);
	}
...
bool FSwitchboardListener::Task_RedeployListener(const FSwitchboardRedeployListenerTask& InRedeployListenerTask)
{
	...
	FProcHandle RedeployedProcHandle = FSwitchboardProcess::CreateProc(*TempNewListenerPath, *RedeployArgs, bLaunchDetached, bLaunchHidden, bLaunchReallyHidden, OutProcessId, PriorityModifier, WorkingDirectory, PipeWriteChild);
...
#if PLATFORM_WINDOWS
void FSwitchboardListener::HarvestExistingFlipModeSessions()
{
	RunningFlipModeSessions.Reserve(MaxFlipModeMonitorSessions);
	RunningFlipModeSessions.Reset();

	for (FFlipModeMonitorSessionId SessionId = 0; SessionId < MaxFlipModeMonitorSessions; ++SessionId)
	{
		bool bOutStoppedSession = false;
		if (!EnsureFlipModeSessionAvailable(SessionId, &bOutStoppedSession))
		{
			UE_LOGFMT(LogSwitchboard, Error, "Unable to stop existing flip mode session {SessionId}", SessionId);
			RunningFlipModeSessions.Add(true, 1);
		}
		else
		{
			if (bOutStoppedSession)
			{
				UE_LOGFMT(LogSwitchboard, Warning, "Stopped existing flip mode session {SessionId}", SessionId);
			}

			RunningFlipModeSessions.Add(false, 1);
		}
	}
}


bool FSwitchboardListener::EnsureFlipModeSessionAvailable(FFlipModeMonitorSessionId SessionId, bool* bOutStoppedSession)
{
	const FString SessionName = MakeFlipModeMonitorSessionName(SessionId);

	EVENT_TRACE_PROPERTIES TraceProps;
	FMemory::Memzero(TraceProps);
	TraceProps.Wnode.BufferSize = sizeof(TraceProps);
	TraceProps.Wnode.Flags = WNODE_FLAG_TRACED_GUID;

	if (bOutStoppedSession != nullptr)
	{
		*bOutStoppedSession = false;
	}

	const uint32 Result = ControlTrace(0, *SessionName, &TraceProps, EVENT_TRACE_CONTROL_STOP);
	if (Result == ERROR_SUCCESS || Result == ERROR_MORE_DATA)
	{
		if (bOutStoppedSession != nullptr)
		{
			*bOutStoppedSession = true;
		}
		return true;
	}
	...
	UE_LOGFMT(LogSwitchboard, Error, "Failed to stop flip mode session {SessionName} with result {Result}",
		SessionName, Result);
...
FSwitchboardListener::FFlipModeMonitorSessionId FSwitchboardListener::AllocFlipModeMonitorSessionId()
{
	check(IsInGameThread());

	const int32 AvailableId = RunningFlipModeSessions.FindAndSetFirstZeroBit();
	if (AvailableId < 0)
	{
		UE_LOGFMT(LogSwitchboard, Error, "Failed to allocate flip mode monitor session ID; too many sessions");
		return InvalidFlipModeSessionId;
	}

	UE_LOGFMT(LogSwitchboard, Verbose, "Allocated flip mode monitor session ID {ID}", AvailableId);

	return static_cast<FFlipModeMonitorSessionId>(AvailableId);
}


void FSwitchboardListener::FreeFlipModeMonitorSessionId(FFlipModeMonitorSessionId SessionId)
{
	check(IsInGameThread());

	if (!ensure(SessionId < RunningFlipModeSessions.Num() && SessionId < MaxFlipModeMonitorSessions))
	{
		UE_LOGFMT(LogSwitchboard, Error, "Attempted to free invalid flip mode monitor session ID {Id}", SessionId);
		return;
	}

	if (!ensure(RunningFlipModeSessions[SessionId]))
	{
		UE_LOGFMT(LogSwitchboard, Error, "Attempted to free unused flip mode monitor session ID {Id}", SessionId);
		return;
	}

	RunningFlipModeSessions[SessionId] = false;

	UE_LOGFMT(LogSwitchboard, Verbose, "Freed flip mode monitor session ID {ID}", SessionId);
}


FString FSwitchboardListener::MakeFlipModeMonitorSessionName(FFlipModeMonitorSessionId SessionId)
{
	return FString::Printf(TEXT("switchboard_%s_presentmon_%d"),
		 *ListenerEndpoint->ToString(), SessionId);
}
...
FRunningProcess* FSwitchboardListener::FindOrStartFlipModeMonitorForUUID(const FGuid& UUID)
{
	...
	const FFlipModeMonitorSessionId SessionId = AllocFlipModeMonitorSessionId();
	if (SessionId == InvalidFlipModeSessionId)
	{
		UE_LOGFMT(LogSwitchboard, Error, "Failed to allocate flip mode monitor session");
		return nullptr;
	}

	MonitorProcess->OnProcessEnded.AddLambda(
		[this, SessionId]()
		{
			if (EnsureFlipModeSessionAvailable(SessionId))
			{
				FreeFlipModeMonitorSessionId(SessionId);
			}
			else
			{
				UE_LOGFMT(LogSwitchboard, Error, "Unable to recover flip mode monitor session {SessionId}", SessionId);
			}
		}
	);
	...
	const FString Arguments = FString::Printf(
		TEXT("-session_name %s -output_stdout -terminate_on_proc_exit -stop_existing_session -process_id %d"), 
		*SessionName, Process->PID);

	MonitorProcess->Handle = FSwitchboardProcess::CreateProc(
		*MonitorProcess->Path,
		*Arguments,
		bLaunchDetached,
		bLaunchHidden,
...
static void FillOutFlipMode(FSyncStatus& SyncStatus, FRunningProcess* FlipModeMonitor)
{
	...
	// Get stdout.
	FUTF8ToTCHAR StdoutConv(reinterpret_cast<const ANSICHAR*>(FlipModeMonitor->Stdout.GetData()), FlipModeMonitor->Stdout.Num());
	const FString StdOut(StdoutConv.Length(), StdoutConv.Get());

	// Clear out the StdOut array.
	FlipModeMonitor->Stdout.Empty();
...
void FSwitchboardListener::HandleStdout(const TSharedPtr<FRunningProcess>& Process)
{
	...
	TArray<uint8> Stdout, Stderr;
	if (FPlatformProcess::ReadPipeToArray(Process->StdoutParentReadPipe, Stdout))
	{
		Process->Stdout.Append(Stdout);
	}

	if (!Process->bCombinedStdoutStderr)
	{
		if (FPlatformProcess::ReadPipeToArray(Process->StderrParentReadPipe, Stderr))
		{
			Process->Stderr.Append(Stderr);
		}
	}

	// If there was new output, update the clients
	if (Process->bUpdateClientsWithStdout && (Stdout.Num() || Stderr.Num()))
	{
		...
		if (Stdout.Num())
		{
			Packet.PartialStdoutB64 = FBase64::Encode(Stdout);
		}

		if (Stderr.Num())
		{
			Packet.PartialStderrB64 = FBase64::Encode(Stderr);
		}
	...
	for (auto Iter = Processes.CreateIterator(); Iter; ++Iter)
	{
		...
		if (Process->Handle.IsValid())
		{
			...
			if (!FPlatformProcess::IsProcRunning(Process->Handle))
			{
				...
				if (ReturnCode != 0)
				{
					if (Process->Stdout.Num() > 0)
					{
						FUTF8ToTCHAR StdoutConv(reinterpret_cast<const ANSICHAR*>(Process->Stdout.GetData()), Process->Stdout.Num());
						UE_LOG(LogSwitchboard, Display, TEXT("Stdout:\n%.*s"), StdoutConv.Length(), StdoutConv.Get());
					}

					if (Process->Stderr.Num() > 0)
					{
						FUTF8ToTCHAR StderrConv(reinterpret_cast<const ANSICHAR*>(Process->Stderr.GetData()), Process->Stderr.Num());
						UE_LOG(LogSwitchboard, Display, TEXT("Stderr:\n%.*s"), StderrConv.Length(), StderrConv.Get());
					}
				}
				...
				if (bNotifyThatProgramEnded)
				{
					...
					Packet.StdoutB64 = FBase64::Encode(Process->Stdout);
					Packet.StderrB64 = FBase64::Encode(Process->Stderr);
				...
				Process->OnProcessEnded.Broadcast();

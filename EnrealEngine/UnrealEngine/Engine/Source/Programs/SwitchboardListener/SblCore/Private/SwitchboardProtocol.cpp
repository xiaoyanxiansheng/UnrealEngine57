...
FCreateTaskResult CreateTaskFromCommand(const FString& InCommand, const FIPv4Endpoint& InEndpoint, bool bAuthenticated)
{
	...
	if (CommandName == FSwitchboardStartTask::CommandName)
	{
		...
		if (TSharedPtr<FJsonValue> CombinedStdoutStderrField = JsonData->TryGetField(TEXT("bCombinedStdoutStderr")))
		{
			CombinedStdoutStderrField->TryGetBool(Task->bCombinedStdoutStderr);
		}
	...
	else if (CommandName == FSwitchboardKillTask::CommandName)
	{
		...
		{
			// Extract optional soft kill timeout, use default if not provided
			TUniquePtr<FSwitchboardKillTask> KillTask = MakeUnique<FSwitchboardKillTask>(MessageID, InEndpoint, ProgramID);
			if (TSharedPtr<FJsonValue> TimeoutField = JsonData->TryGetField(TEXT("soft_kill_timeout")))
			{
				KillTask->SoftKillTimeout = TimeoutField->AsNumber();
			}

			Result.Status = ECreateTaskStatus::Success;
			Result.Task = MoveTemp(KillTask);
			return Result;
		}
	}

...
namespace UE::SwitchboardListener::Private
{
	...
	bool FPosixCredentialManager::SaveCredential(FStringView InCredentialName, FStringView InUser, FStringView InBlob)
	{
		...
		if (!ensure(TempFd != -1))
		{
			...
			UE_LOGFMT(LogSwitchboard, Error, "{Func}: mkstemp failed (errno: {errno}; path: '{TempFilePath}')",
		...
		if (!ensure(CloseResult == 0))
		{
			...
			UE_LOGFMT(LogSwitchboard, Warning, "{Func}: close failed (errno: {errno}; path: '{TempFilePath}')",
		...
		if (TotalBytesWritten < JsonString.Len())
		{
			...
			UE_LOGFMT(LogSwitchboard, Error,
				"{Func}: write failed (errno: {errno}; written {BytesWritten}/{JsonByteLen}; path: '{TempFilePath}')",
		...
		if (!ensure(RenameResult == 0))
		{
			ErrorCode = errno;
			PlatformFile.DeleteFile(UTF8_TO_TCHAR(TempFilePathU8.GetData()));
			UE_LOGFMT(LogSwitchboard, Warning, "{Func}: rename failed (errno: {errno}; path: '{TempFilePath}' -> '{DestFilePath}')",

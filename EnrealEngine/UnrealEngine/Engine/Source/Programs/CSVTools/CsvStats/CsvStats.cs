...
namespace CSVStats
{
	...
	public class DerivedStatException : Exception
	{
		public DerivedStatException(string message) : base(message) { }
	}

	public class DerivedStatMissingSourceException : DerivedStatException
	{
		public DerivedStatMissingSourceException(string message) : base(message) { }
	}
	...
	public class CsvStats
	{
		...
		string ReadNextToken(string expression, ref int index, Dictionary<string, StatSamples> statsOut, int tokenIndex)
		{
			...
			for ( int i=0; i<(int)BuiltinFunctionType.COUNT; i++)
			{
					...
					remaining.StartsWith(funcName+"("))
				{
					...
					if (funcType == BuiltinFunctionType.meta)
					{
						...
						if (val == null)
						{
							throw new DerivedStatMissingSourceException("Metadata not found: " + expression[oldIndex..index]);
						}
						if (!float.TryParse(val, out _))
						{
							throw new DerivedStatException("Failed to parse metadata : " + expression[oldIndex..index]);
						}
						return val.ToString();
					}
					else if (funcType == BuiltinFunctionType.eventcount)
					{
						...
						if (eventStr == null)
						{
							throw new DerivedStatException("Eventstr requires an arg: " + expression[oldIndex..index]);
						}
					...
					else if (funcType == BuiltinFunctionType.framedelta)
					{
						...
						if (srcStatName == null)
						{
							throw new DerivedStatException("FrameDelta requires an arg: " + expression[oldIndex..index]);
						}

						StatSamples srcStat = GetStat(srcStatName);
						if (srcStat == null)
						{
							throw new DerivedStatMissingSourceException("Source stat not found: " + expression[oldIndex..index]);
						}
			...
			if (remaining.StartsWith('\''))
			{
				...
				if (stat == null)
				{
					throw new DerivedStatMissingSourceException("Stat not found: " + statName);
				}
				string safeName = stat.Name.Replace("/", "__SLASH__");
				statsOut[safeName] = stat;
				return safeName;
		...
		public StatSamples MakeDerivedStatFromExpression(string expression, bool bVerboseErrors = false)
		{
			if (expression == "")
			{
				return null;
			...
			catch (DerivedStatException ex)
			{
				if (!(ex is DerivedStatMissingSourceException) || bVerboseErrors)
				{
		...
		public StatSamples AddDerivedStatFromExpression(string expression, bool bVerboseErrors = false)
		{
			StatSamples derivedStat = MakeDerivedStatFromExpression(expression, bVerboseErrors);
			if (derivedStat != null)
			{
				AddStat(derivedStat);
			}

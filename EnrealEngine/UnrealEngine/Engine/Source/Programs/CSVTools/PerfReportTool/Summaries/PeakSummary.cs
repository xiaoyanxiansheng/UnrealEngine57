	...
	class PeakSummary : Summary
	{
		...
		*/
		class PeakSummarySection
		{
			...
			{
				if (statNameFilterSet == null)
				{
					statNameFilterSet = csvStats.GetStatNamesMatchingStringList_Set(statNamesFilter);
				}
				return statNameFilterSet.Contains(statName);
			}
			...
			HashSet<string> statNameFilterSet;

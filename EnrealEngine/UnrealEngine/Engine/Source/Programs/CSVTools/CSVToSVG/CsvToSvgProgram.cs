...
namespace CSVTools
{
	...
	class Program : CSVStats.CommandLineTool
    {
		...
		static string formatString =
			...
			"		-view : open the graph in the default viewer\n" + 
			"";
		...
		void MakeGraph()
		{
			...
			try
			{
				...
				if ( GetBoolArg("view") )
				{
					ViewGraph(System.IO.Path.GetFullPath(svgFilename));
				}
		...
		void ViewGraph(string svgPath)
		{
			if (!OperatingSystem.IsWindows())
			{
				Console.WriteLine("View graph is not currently supported on this platform");
				return;
			}

...
public:
	...
	virtual EReturnCode Execute() override
	{
		...
		if (bHelp)
		{
			...
			UE_LOG(LogBuildPatchTool, Display, TEXT("  -PatchDescriptorPath=\"\" If specified, will write out metadata files specifying what parts of each patched file changed."));
			UE_LOG(LogBuildPatchTool, Display, TEXT("  -OnlyPatchDescriptors    If specified with PatchDescriptorPath, DiffManifests will only emit the files and not emit diff info."));
			UE_LOG(LogBuildPatchTool, Display, TEXT("  -RequireOptimizedDelta   If specified, fail if an optimized delta file doesn't exist."));
			UE_LOG(LogBuildPatchTool, Display, TEXT(""));
		...
		Configuration.bEmitInstallTime = !bSkipInstallTime;
...
#define PARSE_SWITCH(Switch) ParseSwitch(TEXT(#Switch "="), Switch, Switches)
		...
		bSkipInstallTime = ParseOption(TEXT("SkipInstallTime"), Switches);

...
namespace UnrealBuildTool.Modes
{
	...
	class PipInstallMode : ToolMode
	{
		...
		private int PipInstallProjectDependencies(TargetDescriptor TargetDescriptor, BuildConfiguration BuildConfiguration, ILogger Logger)
		{
			...
			PipEnv Pip = new(InstallDir, TargetDescriptor, Logger, ProgressWriter.bWriteMarkup);
	...
	class PipEnv
	{
		...
		public PipEnv(DirectoryReference InInstallDir, TargetDescriptor InTargetDesc, ILogger InLogger, bool UseProgressWriter = false)
		{
			if (InTargetDesc.Architectures.bIsMultiArch)
			{
				InLogger.LogWarning("PipInstall expects single-arch target(host): {arch}", InTargetDesc.Architectures.ToString());
			}
			
			Logger = InLogger;
			InstallDir = InInstallDir;
			TargetPlatform = InTargetDesc.Platform;
			TargetArch = InTargetDesc.Architectures.SingleArchitecture;
		...
		public void WritePluginsListing(UEBuildTarget Target, ILogger Logger, bool bAllPlugins = false)
		{
			...
			string ArchStr = MakeArchString(TargetArch);

			List<string> PluginsSitePackages = new List<string>();
			List<string> PluginsList = new List<string>();

			string[] SitePkgSubdirs = { 
				Path.Combine("Lib", Target.Platform.ToString(), ArchStr, "site-packages"),
				Path.Combine("Lib", Target.Platform.ToString(), "site-packages"),
				Path.Combine("Lib", "site-packages")
			};
			
			foreach (PluginInfo Plugin in CheckPlugins)
			{
				// Add all existing plugin site-packages subdirs to be written to pth file listing
				DirectoryReference PythonContentPath = DirectoryReference.Combine(Plugin.Directory, "Content", "Python");
				foreach (string CheckSubdir in SitePkgSubdirs)
				{
					DirectoryReference CheckPluginPath = DirectoryReference.Combine(PythonContentPath, CheckSubdir);
					if (DirectoryReference.Exists(CheckPluginPath))
					{
						PluginsSitePackages.Add(CheckPluginPath.ToString());
					}
				}
				...
				foreach (JsonObject PlatformReqs in PipEnv.CompatibleRequirements(PluginJson, TargetPlatform, TargetArch))
				{
		...
		public static IEnumerable<JsonObject> CompatibleRequirements(JsonObject PluginJson, UnrealTargetPlatform Platform, UnrealArch Arch)
		{
			...
			foreach (JsonObject PlatformReqs in RequirementsJson)
			{
				...
				PlatformReqs.TryGetStringField("Architecture", out string? ArchField);
				if (!CompatibleArch(ArchField, Arch))
				{
					continue;
				}
		...
		private static bool CompatibleArch(string? ArchField, UnrealArch Arch)
		{
			return (ArchField == null)
				|| String.Equals(ArchField, MakeArchString(Arch), StringComparison.InvariantCultureIgnoreCase)
				|| String.Equals(ArchField, "All", StringComparison.InvariantCultureIgnoreCase);
		}

		private static string MakeArchString(UnrealArch Arch)
		{
			...
			if (Arch == UnrealArch.Arm64 || Arch == UnrealArch.Arm64ec)
			{
				return "Arm64";
			}
		...
		private bool MergeRequirements(FileReference PluginsListingFile, out List<string>? MergedRequirements, out List<string>? ExtraIndexUrls)
		{
			...
			foreach (string PluginPath in PythonPlugins)
			{
				...
				foreach (JsonObject PlatformReqs in CompatibleRequirements(PluginJson, TargetPlatform, TargetArch))
				{

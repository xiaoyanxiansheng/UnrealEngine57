...
namespace UnrealBuildTool
{
	...
	///			become available.
	/// </summary>
	class ImmediateActionQueue : ActionLogger, IDisposable
	{
		...
		:	base(actions.Count(), progressWriterText, writeToolOutput, flushToolOutput, logger)
		{
			...
			if (readArtifacts)
			{
				Func<Task> runAction(LinkedAction action)
				{
					return new Func<Task>(async () =>
					{
						...
						if (result.Success)
						{
							ExecuteResults results = new(result.LogLines, result.ExitCode, TimeSpan.Zero, TimeSpan.Zero, 0, action, "[Cache]");
		...
		public void GetActionResultCounts(out int totalActions, out int succeededActions, out int failedActions, out int cacheHitActions, out int cacheMissActions)
		{
			...
			succeededActions = Results.Where(x => x?.ExitCode == 0).Count();
			failedActions = Results.Where(x => x != null && x.ExitCode != 0).Count();
		...
		void HandleException(LinkedAction action, Exception ex)
		{
			if (ex is AggregateException aggregateEx)
			{
				...
				{
					ExecuteResults results = new(new List<string>(), Int32.MaxValue, TimeSpan.Zero, TimeSpan.Zero, 0, action);
			...
			else
			{
				...
				ExecuteResults results = new(text, Int32.MaxValue, TimeSpan.Zero, TimeSpan.Zero, 0, action);
				OnActionCompleted(action, false, results);
			}
		}
		...
		private void LogActions()
		{
			for (; ; )
			{
				...
				foreach (int index in actionsToLog)
				{
					...
					LogAction(Actions[index].Action, result, Actions.Length);

					// Cancel all other pending tasks
					if (StopCompilationAfterErrors && (result == null || result.ExitCode != 0))
					{
						CancellationTokenSource.Cancel();
					}
				}

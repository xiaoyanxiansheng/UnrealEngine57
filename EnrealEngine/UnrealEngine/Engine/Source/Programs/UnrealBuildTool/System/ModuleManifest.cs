...
namespace UnrealBuildTool
{
	...
	public class ModuleManifest
	{
		...
		/// <summary>
		/// Map of dependencies for each merged library.
		/// </summary>
		public Dictionary<string, HashSet<string>> LibraryDependencies = new Dictionary<string, HashSet<string>>();
		...
		public void Write(TextWriter Writer)
		{
			using (JsonWriter OutputWriter = new JsonWriter(Writer, true))
			{
				...
				{
					OutputWriter.WriteObjectStart("LibraryDependencies");
					foreach (KeyValuePair<string, HashSet<string>> LibraryNameToDependenciesPair in LibraryDependencies.Where(x => x.Value.Any()).OrderBy(x => x.Key))
					{
						OutputWriter.WriteArrayStart(LibraryNameToDependenciesPair.Key);
						foreach (string DependencyName in LibraryNameToDependenciesPair.Value)
						{
							OutputWriter.WriteValue(DependencyName);
						}

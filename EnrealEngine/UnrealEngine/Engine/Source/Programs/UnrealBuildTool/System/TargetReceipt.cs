...
namespace UnrealBuildTool
{
	...
	public class TargetReceipt
	{
		...
		/// <summary>
		/// The build settings version used for this target.
		/// </summary>
		public BuildSettingsVersion BuildSettingsVersion;

		/// <summary>
		/// The build environment used for this target.
		/// </summary>
		public TargetBuildEnvironment TargetBuildEnvironment;
		...
		/// <param name="InBuildSettingsVersion">Build settings version of the target.</param>
		/// <param name="InTargetBuildEnvironment">Build environment used to produce the target.</param>
		public TargetReceipt(FileReference? InProjectFile, string InTargetName, TargetType InTargetType, UnrealTargetPlatform InPlatform, UnrealTargetConfiguration InConfiguration, BuildVersion InVersion, UnrealArchitectures InArchitectures, bool InIsTestTarget, BuildSettingsVersion InBuildSettingsVersion, TargetBuildEnvironment InTargetBuildEnvironment)
		{
			ProjectFile = InProjectFile;
			ProjectDir = DirectoryReference.FromFile(InProjectFile);
			TargetName = InTargetName;
			BuildSettingsVersion = InBuildSettingsVersion;
			TargetBuildEnvironment = InTargetBuildEnvironment;
			Platform = InPlatform;
		...
		public static TargetReceipt Read(FileReference Location, DirectoryReference EngineDir)
		{
			...
			BuildSettingsVersion BuildSettingsVersion;
			if(!RawObject.TryGetEnumField<BuildSettingsVersion>("BuildSettingsVersion", out BuildSettingsVersion))
			{
				// @todo this doesn't necessailry match how it was compiled - should this be an error case?
				BuildSettingsVersion = BuildSettingsVersion.Latest;
			}

			TargetBuildEnvironment TargetBuildEnvironment;
			if (!RawObject.TryGetEnumField<TargetBuildEnvironment>("TargetBuildEnvironment", out TargetBuildEnvironment))
			{
				// @todo this doesn't necessailry match how it was compiled - should this be an error case?
				TargetBuildEnvironment = TargetBuildEnvironment.Unique;
			}

			// Create the receipt
			TargetReceipt Receipt = new TargetReceipt(ProjectFile, TargetName, TargetType, Platform, Configuration, Version, Architectures, IsTestTarget, BuildSettingsVersion, TargetBuildEnvironment);
		...
		public void Write(FileReference Location, DirectoryReference EngineDir)
		{
			using (JsonWriter Writer = new JsonWriter(Location.FullName))
			{
				...
				Writer.WriteValue("BuildSettingsVersion", BuildSettingsVersion.ToString());
				Writer.WriteValue("TargetBuildEnvironment", TargetBuildEnvironment.ToString());

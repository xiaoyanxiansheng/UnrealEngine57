...
namespace UnrealBuildTool
{
	...
	public static class ConfigCache
	{
		...
		/// </summary>
		static ConcurrentDictionary<FileReference, Lazy<ConfigFile?>> LocationToConfigFile = [];

		/// <summary>
		/// Cache of config hierarchies by project
		/// </summary>
		static ConcurrentDictionary<ConfigHierarchyKey, Lazy<ConfigHierarchy>> HierarchyKeyToHierarchy = [];

		/// <summary>
		/// Cache of config fields by type
		/// </summary>
		static ConcurrentDictionary<Type, Lazy<List<ConfigMember>>> TypeToConfigMembers = [];
		...
		{
			ConfigFile = LocationToConfigFile.GetOrAdd(Location, static Location => new Lazy<ConfigFile?>(() =>
			{
			...
			string CustomConfig, CommandLineArguments CmdLineArgs, DirectoryReference? HotfixDir = null, TargetType? IncludePluginsForTargetType = null)
		{
			...
			return HierarchyKeyToHierarchy.GetOrAdd(Key, new Lazy<ConfigHierarchy>(() => InternalReadHierarchy(Logger, Type, ProjectDir, Platform, CustomConfig, HotfixDir, IncludePluginsForTargetType, OverrideStrings))).Value;
		}

		static ConfigHierarchy InternalReadHierarchy(ILogger Logger, ConfigHierarchyType Type, DirectoryReference? ProjectDir, UnrealTargetPlatform Platform, string CustomConfig
			, DirectoryReference? HotfixDir, TargetType? IncludePluginsForTargetType, List<string> OverrideStrings)
		{
			// Find all the input files
			List<ConfigFile> Files = new List<ConfigFile>();
			IEnumerable<FileReference> IniFileNames = ConfigHierarchy.EnumerateConfigFileLocations(Type, ProjectDir, Platform, CustomConfig, IncludePluginsForTargetType);
			List<FileReference> ReadFiles = [];
			foreach (FileReference IniFileName in IniFileNames)
			{
				ConfigFile? File;
			...
			return new ConfigHierarchy(Files, ReadFiles);
		}
		...
		{
			return TypeToConfigMembers.GetOrAdd(TargetObjectType, static TargetObjectType => new Lazy<List<ConfigMember>>(() =>
			{

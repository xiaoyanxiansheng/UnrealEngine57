...
namespace UnrealBuildTool
{
	...
	static class XmlConfig
	{
		...
		public static void ReadConfigFiles(FileReference? overrideCacheFile, DirectoryReference? projectRootDirectory, ILogger logger)
		{
			...
			{
				logger.LogDebug("No project directory provided for project scope BuildConfiguration.xml resolution.");
			}
			...
			else
			{
				...
				if (projectRootDirectory != null)
				{
					...
					{
						logger.LogDebug("No project scope config file at {ProjectRootConfigLocation}. Creating default.", projectRootConfigLocation.FullName);
				...
				// Write any configuration environment variables to a file so they can be loaded
				FileReference environmentXml = FileReference.Combine(Unreal.WritableEngineDirectory, "Intermediate", "Build", "UnrealBuildTool.Env.BuildConfiguration.xml");
				WriteEnvironmentXml(environmentXml, logger);
				temporaryInputFileLocations.Add(environmentXml);
				...
				if (s_values == null)
				{
					...
					{
						logger.LogDebug("Reading configuration file from: {Location}", inputFile.FullName);
				...
				{
					foreach (FileReference inputFile in s_values.InputFiles)
					{
						logger.LogDebug("Reading configuration file from cache ({CacheLocaltion}): {Location}", CacheFile.FullName, inputFile.FullName);
					}
				}
			}
		...
		/// <summary>
		/// Writes environment variables as a BuildConfiguration xml
		/// </summary>
		/// <param name="location">Path to write to</param>
		/// <param name="logger">Logger for output</param>
		/// <returns>True if the environment was written successfully</returns>
		static bool WriteEnvironmentXml(FileReference location, ILogger logger)
		{
			// Read the environment variables
			Dictionary<string, Dictionary<string, string>> contents = [];
			foreach (DictionaryEntry EnvironmentVariable in Environment.GetEnvironmentVariables())
			{
				string key = EnvironmentVariable.Key?.ToString() ?? String.Empty;
				string value = EnvironmentVariable.Value?.ToString() ?? String.Empty;
				if (!key.StartsWith("UnrealBuildTool_") || !key.Contains("__"))
				{
					continue;
				}
				string[] split = key.Substring("UnrealBuildTool_".Length).Split("__");
				string category = split.First();
				string field = split.Last();
				if (!contents.TryGetValue(category, out Dictionary<string, string>? fields))
				{
					fields = new();
					contents.Add(category, fields);
				}
			...
			using (XmlWriter writer = XmlWriter.Create(output, xmlSettings))
			{
				...
				XElement root = new XElement(ns + "Configuration");
				foreach (KeyValuePair<string, Dictionary<string, string>> CategoryItem in contents)
				{
					XElement categoryElement = new XElement(ns + CategoryItem.Key);
					foreach (KeyValuePair<string, string> FieldItem in CategoryItem.Value)
					{
						XElement fieldElement = new XElement(ns + FieldItem.Key);
						if (FieldItem.Value.Contains(';'))
						{
							foreach (string item in FieldItem.Value.Split(';', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries))
							{
								fieldElement.Add(new XElement(ns + "Item", item));
							}
						}
						else
						{
							fieldElement.Add(FieldItem.Value);
						}
						categoryElement.Add(fieldElement);
					}
					root.Add(categoryElement);
				}
				new XDocument(root).Save(writer);
			}
			Utils.WriteFileIfChanged(location, output.ToString(), logger);
			return true;
		}

	...
	public class RulesAssembly
	{
		...
		protected TargetRules? CreateTargetRulesInstance(string TypeName, TargetInfo TargetInfo, ILogger Logger, bool IsTestTarget = false, TargetRulesValidationOptions ValidationOptions = TargetRulesValidationOptions.ValidateTargetAndSDK)
		{
			...
			if (Rules.OptedInModulePlatforms != null)
			{
				...
				// This is really slow (disallow list can contain 20 items and there can be 1000 plugins with long paths.. so search space is huuuuge)
				// Use a bloom filter to minimize the expensive testing
				BloomFilter bloomFilter = new();
				bloomFilter.AddRange(DisallowedPlatformsAndGroups);

				DirectoryReference? ProjectDir = Rules.ProjectFile?.Directory;

				// look in all plugins' paths to see if any disallowed
				List<PluginInfo> DisallowedPluginsList = [];

				foreach (PluginSet set in EnumeratePlugins())
				{
					if (set.ChoiceVersion == null)
					{
						continue;
					}

					PluginInfo info = set.ChoiceVersion;

					bool mightContain = false;
					if (info.File.IsUnderDirectory(Unreal.EngineDirectory))
					{
						var span = info.File.FullName.AsSpan(Unreal.EngineDirectory.FullName.Length + 1);
						if (bloomFilter.MightContainSubString(span, Path.DirectorySeparatorChar))
							mightContain = true;
					}

					if (!mightContain && ProjectDir != null && info.File.IsUnderDirectory(ProjectDir))
					{
						var span = info.File.FullName.AsSpan(ProjectDir.FullName.Length + 1);
						if (bloomFilter.MightContainSubString(span, Path.DirectorySeparatorChar))
							mightContain = true;
					}
					...
					if (mightContain)
					{
						if (info.File.ContainsAnyNames(DisallowedPlatformsAndGroups, Unreal.EngineDirectory) ||
						(ProjectDir != null && info.File.ContainsAnyNames(DisallowedPlatformsAndGroups, ProjectDir)))
							DisallowedPluginsList.Add(info);
					}
				}

				DisallowedPluginsList.ForEach(x => Logger.LogDebug("Disallowing non-opted-in platform plugin {PluginFile}", x.File));
				// and, disable these plugins
				Rules.DisablePlugins.AddRange(DisallowedPluginsList.Select(x => x.Name));
			}

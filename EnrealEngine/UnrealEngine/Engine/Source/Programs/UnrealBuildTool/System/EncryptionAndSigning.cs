...
namespace UnrealBuildTool
{
	...
	public static class EncryptionAndSigning
	{
		...
		static ConcurrentDictionary<FileReference, Lazy<List<EncryptionKey>?>> s_encryptionKeyCache = [];
			...
			if (InProjectDirectory != null)
			{
				...
				if (GameIni != null)
				{
					...
					if (GameIni.GetString("ContentEncryption", "ProjectKeyChain", out Filename))
					{
						...
						List<EncryptionKey>? EncryptionKeysInFile = s_encryptionKeyCache.GetOrAdd(ProjectKeyChainFile, static ProjectKeyChainFile => new Lazy<List<EncryptionKey>?>(() =>
						{
						...
						if (EncryptionKeysInFile != null)
						{
							List<EncryptionKey> EncryptionKeys = new();
							if (Settings.SecondaryEncryptionKeys != null)
							{
								EncryptionKeys.AddRange(Settings.SecondaryEncryptionKeys);
							}

							EncryptionKeys.AddRange(EncryptionKeysInFile);

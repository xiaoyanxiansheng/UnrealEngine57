...
namespace UnrealBuildTool
{
	...
	public sealed class ConfigHierarchy : IDisposable
	{
		...
		/// <summary>
		/// Set of file references
		/// </summary>
		public IEnumerable<FileReference> FileReferences { get; }

		/// <summary>
		/// Cache of requested config sections
		/// </summary>
		readonly ConcurrentDictionary<string, Lazy<ConfigHierarchySection>> NameToSection = new(StringComparer.InvariantCultureIgnoreCase);

		/// <summary>
		/// Construct a config hierarchy from the given files
		/// </summary>
		/// <param name="Files">Set of files to include (in order)</param>
		/// <param name="FileReferences">Set of file paths, for tracking</param>
		public ConfigHierarchy(IEnumerable<ConfigFile> Files, IEnumerable<FileReference>? FileReferences = null)
		{
			this.Files = [.. Files];
		...
		/// </summary>
		/// <returns></returns>
		public HashSet<string> SectionNames => [.. Files.SelectMany(x => x.SectionNames)];
		...
		{
			return NameToSection.GetOrAdd(SectionName, new Lazy<ConfigHierarchySection>(() =>
			{
				...
				return new ConfigHierarchySection(RawSections);
			})).Value;
		}

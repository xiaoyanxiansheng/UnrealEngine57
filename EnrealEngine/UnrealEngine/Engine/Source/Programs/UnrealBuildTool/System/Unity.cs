	...
	class Unity
	{
			...
			int NumIncludedBytesPerUnitySource)
		{
			...
			foreach (FileCollection UnityFile in AllUnityFiles)
			{
				...
				HashSet<FileItem> InlinedSourceFilesInUnity = [];
				...
				foreach (FileItem SourceFile in UnityFile.Files)
				{
					...
					{
						InlinedSourceFilesInUnity.UnionWith(InlinedFiles);
					}
					InlinedSourceFilesInUnity.Add(SourceFile);
				}
				...
				CompileEnvironment.FileInlineSourceMap[UnitySourceFile] = InlinedSourceFilesInUnity;

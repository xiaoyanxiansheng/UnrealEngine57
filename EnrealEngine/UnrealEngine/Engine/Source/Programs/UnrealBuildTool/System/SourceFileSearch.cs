...
namespace UnrealBuildTool
{
	static class SourceFileSearch
	{
		...
		static void FindFilesInternal(DirectoryItem Directory, string[] ExcludedDirectorySuffixes, List<FileReference> FoundFiles, HashSet<DirectoryReference>? SearchedDirectories)
		{
			SearchedDirectories?.Add(Directory.Location);

			foreach (FileItem File in Directory.EnumerateFiles())
			{
				if (ShouldInclude(File.Location, DefaultExcludedFileSuffixes))
				{
		...
		static void FindFilesInternalRecursive(DirectoryItem Directory, string[] ExcludedDirectorySuffixes, List<FileReference> FoundFiles, HashSet<DirectoryReference>? SearchedDirectories)
		{
			...
			{
				if (ShouldInclude(SubDirectory.Location, ExcludedDirectorySuffixes))
				{

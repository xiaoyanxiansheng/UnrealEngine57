	...
	public class ConfigValueParser
	{
		...
		public static bool TryParseTyped(Type T, string Line, [NotNullWhen(true)] out object? Result)
		{
			try
			{
				...
				{
					if (TryParse(Line, out Guid GuidValue))
					{
						Result = GuidValue;
						return true;
					}

					throw new Exception($"Cannot parse Guid from {Line}");
				}
				...
				foreach (System.Reflection.FieldInfo Field in T.GetFields().Where(X => !X.Attributes.HasFlag(IgnoreFlags)))
				{
					...
					// NB. We allow uninitialized values in structs/classes - it may be expected & ini saving omits empty strings etc.
				}
				return true;
			}
			catch (Exception)

...
namespace UnrealBuildTool
{
	...
	public class PluginInfo
	{
		...
		/// </summary>
		public readonly List<FileReference> ChildFiles = new List<FileReference>();
	...
	public static class Plugins
	{
		...
		/// Cache of plugins
		/// </summary>
		static ConcurrentDictionary<string, PluginInfo> PluginInfoCache = [];

		/// <summary>
		/// Cache of plugins under each directory
		/// </summary>
		static ConcurrentDictionary<DirectoryReference, Lazy<List<PluginInfo>>> PluginDirectoryCache = [];
		...
		{
			if (PluginInfoCache.TryGetValue(PluginName, out PluginInfo? plugin))
			{
				return plugin;
			}
			return null;
		...
		{
			Dictionary<string, PluginSet> NameToPluginInfos = new Dictionary<string, PluginSet>(StringComparer.OrdinalIgnoreCase);
		...
		private static bool IsValidChildPluginSuffix(string Suffix)
		{
			...
			{
				if (Group.ToString().Equals(Suffix, StringComparison.OrdinalIgnoreCase))
				{
					return true;
				}
			}
			...
			{
				if (Platform.ToString().Equals(Suffix, StringComparison.OrdinalIgnoreCase))
				{
					return true;
				}
			}
		...
		private static void TryMergeWithParent(List<PluginInfo> Plugins, PluginInfo Child, FileReference Filename)
		{
			// find the parent
			PluginInfo? Parent = null;

			string FilenameWithoutExtension = Filename.GetFileNameWithoutAnyExtensions();
			int LastUnderscoreIndex = FilenameWithoutExtension.LastIndexOf('_');

			string ParentPluginName;
			if (LastUnderscoreIndex > 0 && LastUnderscoreIndex < FilenameWithoutExtension.Length - 1)
			{
				ParentPluginName = FilenameWithoutExtension.Substring(0, LastUnderscoreIndex);
				Parent = Plugins.FirstOrDefault(x => x.Name.Equals(ParentPluginName, StringComparison.OrdinalIgnoreCase) && x.LoadedFrom == Child.LoadedFrom);
			}
			else
			{
			...
			if (Parent == null)
			{
				throw new BuildException("Unable to find parent plugin {0} for platform extension plugin {1}. Make sure {0}.uplugin exists.", ParentPluginName, Filename);
			}

			// validate child plugin file name
			string PlatformName = FilenameWithoutExtension.Substring(LastUnderscoreIndex + 1);
			...
			// The reason we need a lock here is because there could be multiple targets using the same plugins
			// and ReadPluginsFromDirectory can be called in parallel from multiple targets causing ChildPlugins
			// to be populated from different threads.. not the same ChildPlugins but some from each thread.
			lock (Parent)
			{
				MergeWithParent(Parent, Child);
			}
		}
		private static void MergeWithParent(PluginInfo Parent, PluginInfo Child)
		{
				...
				else
				{
					foreach (ModuleDescriptor ChildModule in Child.Descriptor.Modules)
					{
						...
						if (ParentModule != null)
						{
							...
							static void MergePlatformArchitectureLists( Dictionary<UnrealTargetPlatform,List<UnrealArch>> Parent, Dictionary<UnrealTargetPlatform,List<UnrealArch>> Child)
							{
								foreach (KeyValuePair<UnrealTargetPlatform,List<UnrealArch>> Pair in Child)
								{
									if (!Parent.TryGetValue(Pair.Key, out List<UnrealArch>? ArchList))
									{
										Parent[Pair.Key] = Pair.Value;
									}
									else
									{
										foreach (UnrealArch Arch in Pair.Value)
										{
							...
							if (ChildModule.PlatformArchitectureAllowList != null)
							{
								if (ParentModule.PlatformArchitectureAllowList == null)
								{
									ParentModule.PlatformArchitectureAllowList = ChildModule.PlatformArchitectureAllowList;
								}
								else
								{
							...
							if (ChildModule.PlatformArchitectureDenyList != null)
							{
								if (ParentModule.PlatformArchitectureDenyList == null)
								{
									ParentModule.PlatformArchitectureDenyList = ChildModule.PlatformArchitectureDenyList;
								}
								else
								{
		...
		{
			DirectoryReference pluginDir = DirectoryReference.Combine(RootDirectory, Subdirectory);

			return PluginDirectoryCache.GetOrAdd(pluginDir, new Lazy<List<PluginInfo>>(() =>
			{
				...
				foreach (DirectoryReference Dir in RootDirectories)
				{
					...
					List<FileReference> PluginsInDir = PluginsBase.EnumeratePlugins(Dir);
					ConcurrentDictionary<FileReference, PluginInfo> CreatedInfos = [];
					Parallel2.ForEach(PluginsInDir, (PluginFileName) =>
					{
						CreatedInfos.TryAdd(PluginFileName, new PluginInfo(PluginFileName, Type));
					});

					foreach (FileReference PluginFileName in PluginsInDir)
					{
						PluginInfo Plugin = CreatedInfos[PluginFileName];
				...
				{
					TryMergeWithParent(AllParentPlugins, Pair.Key, Pair.Value);
				}

				// Add to cache. Note that we TryAdd because there is a chance the plugin is already added through another RootDirectory
				// (VersePrototypes.uproject has AdditionalPluginDirectory set to a subdir of a plugin folder so PluginDirectoryCache will not catch that)
				foreach (PluginInfo plugin in AllParentPlugins)
				{
					PluginInfoCache.TryAdd(plugin.Name, plugin);
				}
		...
		public static bool IsPluginEnabledForTarget(PluginInfo Plugin, ProjectDescriptor? Project, UnrealTargetPlatform Platform, UnrealTargetConfiguration Configuration, TargetType TargetType)
		{
			...
			if (Project != null && Project.Plugins != null)
			{
				...
				{
					if (String.Equals(PluginReference.Name, Plugin.Name, StringComparison.OrdinalIgnoreCase) && !PluginReference.bOptional)
					{
		...
		/// <param name="Architectures">The architectures being built</param>
		/// <returns>True if the plugin should be enabled for this project</returns>
		public static bool IsPluginCompiledForTarget(PluginInfo Plugin, ProjectDescriptor Project, UnrealTargetPlatform Platform, UnrealTargetConfiguration Configuration, TargetType TargetType, bool bRequiresCookedData, UnrealArchitectures? Architectures = null)
		{
			...
			if (IsPluginEnabledForTarget(Plugin, Project, Platform, Configuration, TargetType) && Plugin.Descriptor.Modules != null)
			{
				...
				{
					if (Module.IsCompiledInConfiguration(Platform, Configuration, "", TargetType, bBuildDeveloperTools, bRequiresCookedData, Architectures))
					{

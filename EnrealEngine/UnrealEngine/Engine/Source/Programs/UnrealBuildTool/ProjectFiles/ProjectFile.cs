...
namespace UnrealBuildTool
{
	abstract class ProjectFile
	{
		...
		/// </summary>
		Dictionary<FileReference, AliasedFile> AliasedFilesLookup = [];

		public Dictionary<FileReference, AliasedFile>.ValueCollection AliasedFiles => AliasedFilesLookup.Values;
		...
		{
			AliasedFilesLookup.TryAdd(File.Location, File);
		}
		...
		public void AddFileToProject(FileReference FilePath, DirectoryReference BaseFolder)
		{
			...
			if (AliasedFilesLookup.ContainsKey(FilePath))
			{
				return;
			}

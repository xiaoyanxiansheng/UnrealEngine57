...
namespace UnrealBuildTool
{
	...
	abstract class MSBuildProjectFile : ProjectFile
	{
			...
			: base(InitFilePath, BaseDir)
		{
			...
			_projectGuid = new Lazy<Guid>(() =>
			{
				...
				if (FileReference.Exists(ProjectFilePath))
				{
					try
					{
						projectGuid = InternalLoadGUIDFromExistingProject();
					}
					catch (Exception)
					{
						// Failed to find GUID, so just create a new one
						return VCProjectFileGenerator.MakeMd5Guid(PathNamespaceGuid, ProjectFilePath.FullName);
					}
				}
		...
		/// </summary>
		public Guid InternalLoadGUIDFromExistingProject()
		{
		...
		public override void LoadGUIDFromExistingProject()
		{
			if (!FileReference.Exists(ProjectFilePath))
			{
				throw new Exception("Can't find project file " + ProjectFilePath);
			}
		}
		...
		/// </summary>
		public Guid ProjectGUID => _projectGuid.Value;

		Lazy<Guid> _projectGuid;
	}
	...
	class VCProjectFile : MSBuildProjectFile
	{
		...
		public override MSBuildProjectContext? GetMatchingProjectContext(TargetType SolutionTarget, UnrealTargetConfiguration SolutionConfiguration, UnrealTargetPlatform SolutionPlatform, PlatformProjectGeneratorCollection PlatformProjectGenerators, UnrealArch? Architecture, ILogger Logger)
		{
			...
			bool IsProgramProject = ProjectTargets.Any() && ProjectTargets[0].TargetRules != null && ProjectTargets[0].TargetRules!.Type == TargetType.Program;
			{
				...
				else if (SolutionTarget == TargetType.Program)
				{
				...
				if (MatchingProjectTargets.Count == 1)
				{
					...
					else
					{
						...
						ProjectConfigAndTargetCombination? Combination = ProjectConfigAndTargetCombinations.FirstOrDefault(
							ProjectPlatform == UnrealTargetPlatform.Android && Architecture == null ?
								// Match first preferred Architecture if none is provided
								Combination =>
							...
							if (UEBuildPlatform.TryGetBuildPlatform(Platform, out BuildPlatform) && (BuildPlatform.HasRequiredSDKsInstalled() == SDKStatus.Valid))
							{
								foreach (ProjectTarget ProjectTarget in ProjectTargets.OfType<ProjectTarget>())
								{
									...
									Action<UnrealArch?> AddProjectAndTargetCombination = (UnrealArch? Arch) =>
									{
										...
										if (!bMakeProjectPerTarget && TargetConfigurationType != TargetType.Game && TargetConfigurationType != TargetType.Program)
										{
									...
									{
										IEnumerable<UnrealArch> Archs = Architectures.Architectures;
										...
										{
											// Order project Architectures first, as they are preferred
											Archs = Archs.Concat(BuildPlatform.ArchitectureConfig.AllSupportedArchitectures.Architectures.Except(Archs));
										}

										foreach (UnrealArch Arch in Archs)
										{
		...
		public override bool WriteProjectFile(List<UnrealTargetPlatform> InPlatforms, List<UnrealTargetConfiguration> InConfigurations, PlatformProjectGeneratorCollection PlatformProjectGenerators, ILogger Logger)
		{
			...
			StringBuilder SharedIncludeSearchPaths = new StringBuilder();
			{
				...
				Parallel2.ForEach(SourceFiles.Where(sf => sf.Reference.HasExtension(".cpp")), SourceFile =>
				{
				...
				Parallel2.ForEach(SourceDirToCount, Pair =>
				{
			...
			List<AliasedFile> LocalAliasedFiles = new(AliasedFiles);
			...
			ConcurrentDictionary<DirectoryReference, string> DirectoryToIncludeSearchPaths = new();
			{
				...
				LocalAliasedFiles.Sort((a, b) =>
				{
					return a.Location.CompareTo(b.Location);
				});

				Parallel2.ForEach(LocalAliasedFiles, LocalAliasedFile =>
				{
			...
			// Write each configuration
			{
				...
				Parallel2.For(0, ProjectConfigAndTargetCombinations.Count, Index =>
				{
		...
		private static bool EnsureFilterPathExists(string FilterRelativeSourceDirectory, StringBuilder VCFiltersFileContent, HashSet<string> FilterDirectories)
		{
			...
			if (!FilterDirectories.Contains(PathRemaining))
			{
				...
				{
					if (FilterDirectories.Add(LeadingDirectory))
					{
		...
		private void WriteConfiguration(string ProjectName, ProjectConfigAndTargetCombination Combination, StringBuilder VCProjectFileContent, PlatformProjectGeneratorCollection PlatformProjectGenerators, StringBuilder? VCUserFileContent, VisualStudioUserFileSettings? VCUserFileSettings)
		{
...

			{
				...
				else
				{
					...
					else if (TargetRulesObject.Type == TargetType.Editor && TargetRulesObject.ProjectFile != null && TargetRulesObject.LinkType == TargetLinkType.Modular && TargetRulesObject.BuildEnvironment == TargetBuildEnvironment.Unique)
					{
						VCProjectFileContent.AppendLine("    <LocalDebuggerCommandArguments>{0} -Project=\"{1}\"</LocalDebuggerCommandArguments>", TargetRulesObject.Name, NormalizeProjectPath(TargetRulesObject.ProjectFile));
					}

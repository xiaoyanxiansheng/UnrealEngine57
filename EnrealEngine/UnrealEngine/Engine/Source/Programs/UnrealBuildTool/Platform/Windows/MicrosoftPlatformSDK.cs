...
namespace UnrealBuildTool
{
	internal partial class MicrosoftPlatformSDK : UEBuildPlatformSDK
	{
		...
		public static bool TryGetToolChainDir(WindowsCompiler Compiler, string? CompilerVersion, UnrealArch Architecture, ILogger Logger, [NotNullWhen(true)] out VersionNumber? OutToolChainVersion, [NotNullWhen(true)] out DirectoryReference? OutToolChainDir, out DirectoryReference? OutRedistDir)
		{
			...
			else if (CompilerVersion.StartsWith('<') && VersionNumber.TryParse(CompilerVersion[1..], out VersionNumber? LessThanToolChainVersion))
			{
				ToolChain = SelectToolChain(ToolChains.Where(x => x.Version < LessThanToolChainVersion && x.Family < LessThanToolChainVersion), x => x, Architecture);
				if (ToolChain == null || !(ToolChain.Version < LessThanToolChainVersion || ToolChain.Family < LessThanToolChainVersion))
				{
					DumpToolChains(ToolChains, x => x.ThenByDescending(x => x.Version < LessThanToolChainVersion).ThenByDescending(x => x.Family < LessThanToolChainVersion), Architecture, Logger);
					throw new BuildException("Unable to find valid <{0} C++ toolchain for {1} {2}", LessThanToolChainVersion, Compiler, Architecture.ToString());
				}
			}
			...
			return CachedToolChainInstallations.GetOrAdd(Compiler, _ =>
			{
				...
				if (BuildHostPlatform.Current.Platform == UnrealTargetPlatform.Win64)
				{
					if (Compiler == WindowsCompiler.Clang)
					{
						...
						// Check for installations bundled with Visual Studio
						foreach (VisualStudioInstallation Installation in FindVisualStudioInstallations(WindowsCompiler.VisualStudio2022, Logger)
							.Concat(FindVisualStudioInstallations(WindowsCompiler.VisualStudio2026, Logger)))
						{
					...
					else if (Compiler == WindowsCompiler.ClangRTFM)
					{
						...
						DirectoryReference AutoRTFMBaseDir = DirectoryReference.Combine(Unreal.EngineDirectory, "Restricted", "NotForLicensees", "Binaries", "Win64", "AutoRTFM");
						foreach (DirectoryReference AutoRTFMDir in DirectoryReference.EnumerateDirectories(AutoRTFMBaseDir))
						{
					...
					else if (Compiler.IsMSVC())
					{
						...
						if (UEBuildPlatformSDK.TryGetHostPlatformAutoSDKDir(out PlatformDir))
						{
							...
							switch (Compiler)
							{
								...
								case WindowsCompiler.VisualStudio2026: VSDir = "VS2026"; break;
							}
			...
			if (BuildHostPlatform.Current.Platform == UnrealTargetPlatform.Win64)
			{
				try
				{
					...
					for (; ; )
					{
						...
						if (Version >= MicrosoftPlatformSDK.MinimumVisualStudio2026Version) // Treat any newer versions as 2026, until we have an explicit enum for them
						{
							Compiler = WindowsCompiler.VisualStudio2026;
						}
						else if (Version >= MicrosoftPlatformSDK.MinimumVisualStudio2022Version)
						{
...
#pragma warning restore CS0618 // Type or member is obsolete
					...
					Installations = [.. Installations.OrderBy(x => x.Compiler)
		...
		static void AddVisualCppToolChain(VersionNumber Version, UnrealArch HostArchitecture, WindowsCompilerChannel ReleaseChannel, DirectoryReference ToolChainDir, DirectoryReference? RedistDir, List<ToolChainInstallation> ToolChains, bool IsAutoSdk, ILogger Logger)
		{
			...
			else if (Version < MinimumVisualCppVersion)
			{

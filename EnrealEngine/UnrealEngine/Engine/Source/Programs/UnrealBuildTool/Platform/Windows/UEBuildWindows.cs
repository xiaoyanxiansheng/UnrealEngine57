...
namespace UnrealBuildTool
{
	...
	public enum WindowsCompiler
	{
		...
		/// <summary>
		/// Visual Studio 2026 (Visual C++ 19.0)
		/// </summary>
		VisualStudio2026,
	...
	public class WindowsTargetRules
	{
		...
		[CommandLine("-2026", Value = nameof(WindowsCompiler.VisualStudio2026))]
		...
		[CommandLine("-NoClangStandaloneDebug", Value = "false")]
		public bool bClangStandaloneDebug = true;
		...
		public bool bUpdatedCPPMacro { get; set; } = true;
		...
		[RequiresUniqueBuildEnvironment]
		[XmlConfigFile(Category = "WindowsPlatform")]
		[CommandLine("-NoStrictInline", Value = "false")]
		public bool bStrictInlineConformance { get; set; } = true;

		/// <summary>
		/// Enables preprocessor conformance (/Zc:preprocessor).
		/// </summary>
		[RequiresUniqueBuildEnvironment]
		[XmlConfigFile(Category = "WindowsPlatform")]
		[CommandLine("-NoStrictPreprocessor", Value = "false")]
		public bool bStrictPreprocessorConformance { get; set; } = true;

		/// <summary>
		/// Enables enum types conformance (/Zc:enumTypes).
		/// </summary>
		[RequiresUniqueBuildEnvironment]
		[XmlConfigFile(Category = "WindowsPlatform")]
		[CommandLine("-StrictEnumTypes")]
		public bool bStrictEnumTypesConformance { get; set; } = false;

		/// <summary>
		/// Enables enforcing standard C++ ODR violations (/Zc:checkGwOdr).
		/// </summary>
		[RequiresUniqueBuildEnvironment]
		[XmlConfigFile(Category = "WindowsPlatform")]
		[CommandLine("-StrictODR")]
		public bool bStrictODRViolationConformance { get; set; } = false;

		/// <summary>
		/// Volatile Metadata is enabled by default and improves x64 emulation on arm64, but may come at a small performance cost (/volatileMetadata-).
		/// </summary>
		[RequiresUniqueBuildEnvironment]
		[XmlConfigFile(Category = "WindowsPlatform")]
...
#pragma warning restore CS1591
	...
	class WindowsArchitectureConfig : UnrealArchitectureConfig
	{
		...
		{
			// for now x64 only uses architecture-less filenames.
			return Architectures.bIsMultiArch || !Architectures.Contains(UnrealArch.X64);
		}
	...
	class WindowsPlatform : UEBuildPlatform
	{
		...
		public override void ValidateTarget(TargetRules Target)
		{
			...
			// Intel compiler only supports x64
			if (Target.WindowsPlatform.Compiler.IsIntel() && !Target.Architecture.bIsX64)
			{
			...
			// for monolithic or merged module editor builds, add the PDBPAGESIZE option, (VS 16.11, VC toolchain 14.29.30133), but the pdb will be too large without this
			...
			if (!Target.WindowsPlatform.PdbPageSize.HasValue && (Target.LinkType == TargetLinkType.Monolithic || Target.bMergeModules) && Target.Type == TargetType.Editor)
			{
			...
			{
				// Monolithic or merged module targets always require increasing the pdb page size with Dynamic Debugging or linking may fail
				if ((Target.LinkType == TargetLinkType.Monolithic || Target.bMergeModules) && (Target.WindowsPlatform.PdbPageSize ?? 0) < 16384)
				{
					Target.WindowsPlatform.PdbPageSize = 16384;
				}
		...
		internal static WindowsCompiler GetDefaultCompiler(TargetRules? TargetRules, UnrealArch Architecture, ILogger Logger, bool bSkipWarning = false)
		{
			...
			if (PreferredCompilers.Any())
			{
				...
				if (Compiler != null)
				{
					...
					{
						Logger.Log(bSkipWarning || MicrosoftPlatformSDK.IgnoreToolchainErrors ? LogLevel.Debug : LogLevel.Warning, "{Target} {Platform} {Architecture} preferred compiler {Preferred} not available, compiler set to {Compiler}", TargetRules?.Name ?? "Default", TargetRules?.Platform ?? UnrealTargetPlatform.Win64, Architecture, PreferredCompilers.First(), Compiler);
					}
					else
					{
						Logger.LogDebug("{Target} {Platform} {Architecture} Compiler set to preferred compiler {Compiler}", TargetRules?.Name ?? "Default", TargetRules?.Platform ?? UnrealTargetPlatform.Win64, Architecture, Compiler);
					}
					return (WindowsCompiler)Compiler;
				}
				Logger.Log(bSkipWarning || MicrosoftPlatformSDK.IgnoreToolchainErrors ? LogLevel.Debug : LogLevel.Warning, "{Target} {Platform} {Architecture} No preferred compilers available [{Preferred}]", TargetRules?.Name ?? "Default", TargetRules?.Platform ?? UnrealTargetPlatform.Win64, Architecture, String.Join(',', PreferredCompilers));
			}
		...
		internal static WindowsCompiler GetDefaultToolchain(TargetRules? TargetRules, UnrealArch Architecture, ILogger Logger, bool bSkipWarning = false)
		{
			...
			if (ProjectFileGeneratorSettings.Format != null)
			{
				foreach (ProjectFileFormat Format in ProjectFileGeneratorSettings.ParseFormatList(ProjectFileGeneratorSettings.Format, Logger))
				{
					...
					else if (Format == ProjectFileFormat.VisualStudio2026)
					{
						return WindowsCompiler.VisualStudio2026;
					}
				}
			}
			...
			if (XmlConfig.TryGetValue(typeof(VCProjectFileSettings), "ProjectFileFormat", out ProjectFormatObject))
			{
				...
				else if (ProjectFormat == VCProjectFileFormat.VisualStudio2026)
				{
					return WindowsCompiler.VisualStudio2026;
				}
			}
			...
			if (ProjectFileGenerator.GetPreferredSourceCodeAccessor(TargetRules?.ProjectFile, out PreferredAccessor))
			{
				...
				else if (PreferredAccessor == ProjectFileFormat.VisualStudio2026)
				{
					return WindowsCompiler.VisualStudio2026;
				}
			}
			...
			else if (MicrosoftPlatformSDK.HasValidCompiler(WindowsCompiler.VisualStudio2026, Architecture, Logger))
			{
				return WindowsCompiler.VisualStudio2026;
			}
			...
			if (!bSkipWarning && !MicrosoftPlatformSDK.IgnoreToolchainErrors)
			{
				...
				else if (TryGetVSInstallDirs(WindowsCompiler.VisualStudio2026, Logger) != null)
				{
					Logger.LogWarning("Visual Studio 2026 is installed, but is out of date or missing a valid C++ toolchain (minimum version {MinVersion}, preferred version {PreferredVersion}). Please update Visual Studio 2026 to {MinimumVsVersionStr} or later and verify that the \"{Component}\" component is selected in the Visual Studio 2026 installation options.",
						MinimumMsvcVersionStr,
						PreferredMsvcVersionStr,
						MinimumVsVersionStr,
						PreferredComponentStr);
		...
		public override UEToolChain CreateToolChain(ReadOnlyTargetRules Target)
		{
			...
			else if (Target.bIWYU)
			{
				return new IWYUToolChain(Target, toolchain, Logger);
			}

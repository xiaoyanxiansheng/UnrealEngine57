...
namespace UnrealBuildTool
{
	...
	class RemoteMac
	{
		...
		private List<string> RsyncArgumentsForDownload(List<string> BaseArguments)
		{
			List<string> Arguments = new List<string>(BaseArguments);
			Arguments.Add(String.Format("--rsync-path=\"{0}/rsync\"", RemoteBaseDir));
			return Arguments;
		}

		private List<string> RsyncArgumentsForUpload(List<string> BaseArguments, string RemoteDirectory)
		{
			List<string> Arguments = new List<string>(BaseArguments);
			Arguments.Add(String.Format("--rsync-path=\"mkdir -p {0} && {1}/rsync\"", RemoteDirectory, RemoteBaseDir));
			return Arguments;
		}
		...
		{
			DirectoryReference Cwrsync = DirectoryReference.Combine(Unreal.EngineDirectory, "Extras", "ThirdPartyNotUE", "cwrsync");
			RsyncExe = FileReference.Combine(Cwrsync, "bin", "rsync.exe");
			SshExe = FileReference.Combine(Cwrsync, "bin", "ssh.exe");
			...
			string RemoteFile = GetRemotePath("rsync");

			List<string> Arguments = new List<string>(CommonRsyncArguments);
			// use MacOS default system rsync for the upload of the 2.6.9 rsync binary
			Arguments.Add(String.Format("--rsync-path=\"mkdir -p {0} && rsync\"", RemoteBaseDir));
			Arguments.Add(String.Format("\"{0}\"", GetLocalCygwinPath(FileReference.Combine(Cwrsync, "mac", "rsync"))));
			...
			// Remove quaratine extended attribute if remote machine set it.
			Execute(RemoteBaseDir, "if xattr -p com.apple.quarantine rsync > /dev/null 2>&1; then xattr -d com.apple.quarantine rsync; fi", Logger);
		}
		...
		List<string> GetRemoteArgumentsForTarget(TargetDescriptor TargetDesc, List<FileReference>? LocalManifestFiles)
		{
			...
			foreach (string LocalArgument in TargetDesc.AdditionalArguments)
			{
				...
				string Key = LocalArgument[..EqualsIdx];
				string Value = LocalArgument[(EqualsIdx + 1)..];
		...
		private FileReference GetLocalPath(string RemotePath)
		{
			foreach (RemoteMapping Mapping in Mappings)
			{
				...
				{
					return FileReference.Combine(Mapping.LocalDirectory, RemotePath[(Mapping.RemoteDirectory.Length + 1)..]);
				}
			}
		...
		private static string GetLocalCygwinPath(FileSystemReference InPath)
		{
			...
			return String.Format("/cygdrive/{0}{1}", InPath.FullName[..1], InPath.FullName[2..].Replace('\\', '/'));
		}
		...
		void UploadFile(FileReference LocalFile, ILogger Logger)
		{
			...
			List<string> Arguments = RsyncArgumentsForUpload(CommonRsyncArguments, RemoteDirectory);
		...
		{
			List<string> Arguments = RsyncArgumentsForUpload(BasicRsyncArguments, RemoteDirectory);
		...
		void DownloadFile(FileReference LocalFile, ILogger Logger)
		{
			...
			List<string> Arguments = RsyncArgumentsForDownload(CommonRsyncArguments);
		...
		private string ConvertRemotePathsToLocal(string Text)
		{
			...
			foreach (Match? Match in Matches)
			{
				...
				foreach (RemoteMapping Mapping in Mappings)
				{
					...
					{
						Path = Mapping.LocalDirectory + Path[Mapping.RemoteDirectory.Length..].Replace('/', '\\');
						break;
					}
				}

...
namespace UnrealBuildTool
{
	...
	abstract class UEBuildModule
	{
		...
		/// <summary>
		/// The name of the _API define for this module (for functions)
		/// </summary>
		protected readonly string ModuleApiDefine;

		/// <summary>
		/// The name of the _NON_ATTRIBUTED_API define for this module (for variables)
		/// </summary>
		protected readonly string ModuleNonAttributedDefine;
		...
		/// </summary>
		public HashSet<UEBuildModule>? ReferenceStackParentModules;
		...
		/// <summary>
		/// AutoRTFM function-map definition files
		/// </summary>
		public HashSet<FileItem> PublicAutoRTFMExternalMappingFiles = new();
		...
		public UEBuildModule(ModuleRules Rules, DirectoryReference IntermediateDirectory, DirectoryReference IntermediateDirectoryNoArch, ILogger Logger)
		{
			...
			ModuleNonAttributedDefine = Name.ToUpperInvariant() + "_NON_ATTRIBUTED_API";

			HashSet<string> PublicPreBuildLibraries = HashSetFromOptionalEnumerableStringParameter(Rules.PublicPreBuildLibraries);
			PublicDefinitions = [.. HashSetFromOptionalEnumerableStringParameter(Rules.PublicDefinitions).Select(TrimDefinition)];
			...
			PublicAutoRTFMExternalMappingFiles = [.. Rules.AutoRTFMExternalMappingFiles.Select(FileItem.GetItemByPath)];
		...
		{
			return InEnumerableStrings == null ? [] : [.. InEnumerableStrings.Select(x => ExpandPathVariables(x, null, null))];
		}
		...
		/// <summary>
		/// Add precompile build products from this module to the target
		/// </summary>
		/// <param name="ToolChain"></param>
		...
		/// <summary>
		/// Trims whitespace from a definition to match strict rules which does not allow space before and after '='.
		/// </summary>
		/// <param name="Def"></param>
		/// <returns>Trimmed string, or the current instance if unchanged</returns>
		private static string TrimDefinition(string Def)
		{
			int IndexOfAssign = Def.IndexOf('=');
			if (IndexOfAssign != -1)
			{
				ReadOnlySpan<char> DefSpan = Def.AsSpan();
				ReadOnlySpan<char> Name = DefSpan[..IndexOfAssign];
				ReadOnlySpan<char> NameTrim = Name.Trim();
				ReadOnlySpan<char> Value = DefSpan[(IndexOfAssign + 1)..];
				ReadOnlySpan<char> ValueTrim = Value.Trim();
				return Name.Length != NameTrim.Length || Value.Length != ValueTrim.Length
					? $"{NameTrim}={ValueTrim}"
					: Def;
			}
			return Def.Trim();
		...
		/// <summary>
		/// Add definitions from source to target. TrimDefinition is called in the constructor to match strict rules which does not allow space before and after '='.
		/// </summary>
		protected static void AddDefinitions(List<string> Target, HashSet<string> Source)
		{
			...
			)
		{
			...
			if (Rules.Type == ModuleRules.ModuleType.CPlusPlus)
			{
				...
				// We use a C-style attribute here because of where these macros are defined in UE. C++ attributes must come at the beginning of the function
				// definition, like `[[my_attr]] void Foo();`, but these macros are used like `void FOO_API Foo();`, and if we used the C++ attribute name to
				// disable AutoRTFM on this function the compiler would spit nails. But it'll happily consume the C-style attribute at that location and do
				// the right thing, so we use that instead.
				String AutoRTFMDisable = (Rules.Target.bUseAutoRTFMCompiler && Rules.bDisableAutoRTFMInstrumentation && Rules.bInjectAutoRTFMAttributeDisables) ?
					"__attribute__((autortfm(disable)))" : String.Empty;

				// Add a spacer if and only if we've actually used import/export above.
				String Spacer = ApiDefinition.Length == 0 ? String.Empty : " ";

				// Add the import or export declaration for the module
				String ModuleApiDefinitionString = $"{ModuleApiDefine}={ApiDefinition}{Spacer}{AutoRTFMDisable}";
				Definitions.Add(ModuleApiDefinitionString);

				String ModuleNonAttributedApiDefinitionString = $"{ModuleNonAttributedDefine}={ApiDefinition}";
				Definitions.Add(ModuleNonAttributedApiDefinitionString);
			}
			...
			AutoRTFMExternalMappingFiles.UnionWith(PublicAutoRTFMExternalMappingFiles);
		}
			...
			)
		{
			...
			{
				Module.AddModuleToCompileEnvironment(this, Binary, IncludePaths, SystemIncludePaths, ModuleInterfacePaths, Definitions, AdditionalFrameworks, AutoRTFMExternalMappingFiles, AdditionalPrerequisites, bWithLegacyPublicIncludePaths, bWithLegacyParentIncludePaths);
			}
		}
		...
		protected void RecursivelyCreateModules(CreateModuleDelegate CreateModule, string ReferenceChain, List<UEBuildModule> ReferenceStack, ILogger Logger)
		{
			...
			{
				if (!Rules.Plugin.Descriptor.IsModuleCompiledInConfiguration(Rules.Name, Rules.Target.Platform, Rules.Target.Configuration, Rules.Target.Name, Rules.Target.Type, Rules.Target.bBuildDeveloperTools, Rules.Target.bBuildRequiresCookedData, Rules.Target.Architectures, out string? InvalidReason))
				{
		...
		public virtual bool ValidateModule(ILogger logger)
		{
			...
			{
				if (!Rules.Plugin.Descriptor.IsModuleCompiledInConfiguration(Rules.Name, Rules.Target.Platform, Rules.Target.Configuration, Rules.Target.Name, Rules.Target.Type, Rules.Target.bBuildDeveloperTools, Rules.Target.bBuildRequiresCookedData, Rules.Target.Architectures, out string? _))
				{
					anyErrors = true;
				}
			}
		...
		bool ValidateCircularReferences(ILogger logger)
		{
			...
			if (allowed.Any())
			{
				logger.LogTrace(KnownLogEvents.Validation, "Allowed circular dependency for '{RulesFile}' detected:", RulesFile.MakeRelativeTo(Unreal.EngineDirectory));
				foreach (CircularReference circularReference in allowed)
				{
					logger.LogTrace(KnownLogEvents.Validation, "* {ReferenceStack} (allowed via {GuiltyModules})", String.Join(" -> ", circularReference.ReferenceStack), String.Join(", ", circularReference.GuiltyModules.Select(x => $"{x.Key} -> {x.Value}")));
				}
			}
			return errors.Any();
		}
		...
		public virtual void ExportJson(DirectoryReference? BinaryOutputDir, DirectoryReference? TargetOutputDir, JsonWriter Writer)
		{
			...
			ExportJsonStringArray(Writer, "AutoRTFMExternalMappingFiles", PublicAutoRTFMExternalMappingFiles.Select(x => x.FullName));

...
namespace UnrealBuildTool
{
	...
	abstract class UEBuildPlatform
	{
		...
		public virtual bool ValidateModuleIncludePaths(UEBuildModule Module, ReadOnlyTargetRules Target, IEnumerable<UEBuildModule> AllModules, ILogger Logger)
		{
			...
			else if (Module is UEBuildModuleCPP)
			{
				foreach (DirectoryReference Path in PublicIncludePaths)
				{
					...
					{
						// Warn on the root module directory being added, except for plugins as there are some plugins with a single module directly in the Source directory
						if (Path == Module.ModuleDirectory && (Module.Rules.Plugin == null || Module.ModuleDirectory.ParentDirectory != Module.Rules.Plugin.Directory))
						{
		...
		/// <summary>
		/// Struct used to add platform static shaders
		/// </summary>
		public struct StaticShader
		{
			/// <summary>
			/// Name of shader
			/// </summary>
		...
		/// <summary>
		/// Called by platforms to add platform specific static shaders
		/// </summary>
		/// <param name="Target">Target rules</param>
		/// <param name="Rules">Module rules</param>
		/// <param name="StaticShaders">Static shaders supported by current platform</param>
		public void AddPlatformStaticShaders(ReadOnlyTargetRules Target, ModuleRules Rules, IEnumerable<StaticShader> StaticShaders)
		{
			if (Target.bNeedsExtraShaderFormats || Target.Platform == Platform)
			{
				if (Rules.Name == "BuildSettings") // Put list of platforms in BuildSettings since this array will vary depending on what platforms machine has installed
				{
					List<string> content = [.. Rules.FilesToGenerate["RHIStaticShaderPlatformNames.gen.cpp"]];
					content.InsertRange(content.Count - 2, StaticShaders.Select(shader =>
						$"	{{ TEXT(\"{shader.Name}\"), TEXT(\"{Platform}\"), {shader.Index} }},"));
					Rules.FilesToGenerate["RHIStaticShaderPlatformNames.gen.cpp"] = [.. content];
				}
				else if (Rules.Name == "RHI") // Add one file per platform with EShaderPlatform entries
				{
					IEnumerable<string> content = [
						"#pragma once", "#include \"HAL/Platform.h\"", "enum EShaderPlatform : uint16;",
						.. StaticShaders.Select(shader => $"inline constexpr EShaderPlatform SP_{shader.Name} = EShaderPlatform({shader.Index});")
					];
					Rules.FilesToGenerate.Add($"{Platform}RHIShaderPlatform.h", content);
				}
			}
		}

...
namespace UnrealGameSync
{
	...
	public class PerforceChangeDetails
	{
		...
		public bool ContainsUEFN { get; }

		public bool ContainsUgsConfig { get; }

		public PerforceChangeDetails(DescribeRecord describeRecord, Func<string, bool>? isCodeFile = null, Func<string, bool>? isUEFNFile = null)
		{
			isCodeFile ??= IsCodeFile;
			isUEFNFile ??= IsUEFNFile;
		...
		public static bool IsUEFNFile(string depotFile)
		{
			return depotFile.Contains("/UEFNPROJECTS/", StringComparison.OrdinalIgnoreCase);
		}
	}
	...
	public static class Utility
	{
		...
		static Sha1Hash GetConfigHash(IPerforceConnection perforce, IEnumerable<string> syncPaths, IEnumerable<string> codeRules, IEnumerable<string> uefnRules)
		{
			...
			foreach (string uefnRule in uefnRules)
			{
				digest.AppendLine($"UEFN Rule: {uefnRule}");
			}
		...
		/// Gets the code filter from the project config file
		/// </summary>
		public static string[] GetUEFNFilter(ConfigFile projectConfigFile)
		{
			ConfigSection? projectConfigSection = projectConfigFile.FindSection("Perforce");
			return projectConfigSection?.GetValues("UEFNFilter", (string[]?)null) ?? Array.Empty<string>();
		}
		...
		public static async IAsyncEnumerable<ChangesRecord> EnumerateChanges(IPerforceConnection perforce, IEnumerable<string> syncPaths, int? minChangeNumber, int? maxChangeNumber, int? maxChanges, [EnumeratorCancellation] CancellationToken cancellationToken)
		{
			...
			Sha1Hash configHash = GetConfigHash(perforce, syncPaths, Array.Empty<string>(), Array.Empty<string>());
		...
		/// Creates or returns cached <see cref="PerforceChangeDetails"/> objects describing the requested chagnes
		/// </summary>
		public static IAsyncEnumerable<PerforceChangeDetails> EnumerateChangeDetails(IPerforceConnection perforce, int? minChangeNumber, int? maxChangeNumber, IEnumerable<string> syncPaths, IEnumerable<string> codeRules, IEnumerable<string> uefnRules, CancellationToken cancellationToken)
		{
			...
			return EnumerateChangeDetails(perforce, changeNumbers, codeRules, uefnRules, cancellationToken);
		}

		/// <summary>
		/// Creates or returns cached <see cref="PerforceChangeDetails"/> objects describing the requested chagnes
		/// </summary>
		public static async IAsyncEnumerable<PerforceChangeDetails> EnumerateChangeDetails(IPerforceConnection perforce, IAsyncEnumerable<int> changeNumbers, IEnumerable<string> codeRules, IEnumerable<string> uefnRules, [EnumeratorCancellation] CancellationToken cancellationToken)
		{
			...
			Func<string, bool>? isUefnFile = null;
			if (uefnRules.Any())
			{
				FileFilter filter = new FileFilter();
				foreach (string uefnRule in uefnRules)
				{
					filter.AddRule(uefnRule);
				}
				isUefnFile = filter.Matches;
			}

			// Get the hash of the configuration
			Sha1Hash hash = GetConfigHash(perforce, Enumerable.Empty<string>(), codeRules, uefnRules);
			...
			for (; ; )
			{
				...
				if (changeBatch.Count > 0)
				{
					...
					foreach (DescribeRecord describeRecordLoop in describeRecords.OrderByDescending(x => x.Number))
					{
						...
						PerforceChangeDetails details = new PerforceChangeDetails(describeRecord, isCodeFile, isUefnFile);
						...
						while (describeRecord.Files.Count >= currentMaxFiles && !details.ContainsCode)
						{
							...
							details = new PerforceChangeDetails(describeRecord, isCodeFile, isUefnFile);
						}
		...
		public static List<string> GetDepotConfigPaths(string enginePath, string projectPath, string branchClientPath)
		{
			...
			{
				// We currently assume that UEFN projects never have their UGS build folder in the same directory as the UEFN project
				string searchFolder = projectPath.Substring(0, projectPath.LastIndexOf('/'));

				while (searchFolder.EndsWith("uefnprojects", StringComparison.OrdinalIgnoreCase) == false && 
					searchFolder.Equals(branchClientPath, StringComparison.OrdinalIgnoreCase) == false)
				{
					int index = searchFolder.LastIndexOf('/');
					if (index <= 0)
					{
						break;
					}

					searchFolder = searchFolder.Substring(0, index);

					AddDepotConfigPaths_PlatformExtensions(searchFolder, "/Build", "UnrealGameSync.ini", searchPaths);
				}
			}
			else
			{

...
namespace UnrealGameSync
{
	...
	public class ConfigSection
	{
		...
		private readonly HashSet<string> _wildCardKeys = new HashSet<string>();
		...
		public void SetValue(string key, string value, bool fromWildCard = false)
		{
			if (value == null)
			{
				RemoveValue(key);
			...
			if (fromWildCard)
			{
				_wildCardKeys.Add(key);
			}
			else
			{
				_wildCardKeys.Remove(key);
			}
		}

		public bool HasWildCardValue(string key)
		{
			return _wildCardKeys.Contains(key);
		}
	...
	public class ConfigFile
	{
		...
		public void Parse(string[] lines, string? project = null)
		{
			ConfigSection? currentSection = null;
			bool wildCardSection = false;
				...
				if (!trimLine.StartsWith(";", StringComparison.Ordinal))
				{
					if (trimLine.StartsWith("[", StringComparison.Ordinal) && trimLine.EndsWith("]", StringComparison.Ordinal))
					{
						...
						if (String.IsNullOrEmpty(project) == false &&
							sectionName.StartsWith("//", StringComparison.Ordinal) && sectionName.Contains('*', StringComparison.Ordinal))
						{
							if (FileSystemName.MatchesSimpleExpression(sectionName, project))
							{
								sectionName = project;
					...
					else if (currentSection != null)
					{
						...
						if (equalsIdx != -1)
						{
							...
							{
								string key = trimLine.Substring(0, equalsIdx).TrimEnd();

								if (wildCardSection == false || // Non-wildcard settings always take priority
									currentSection.GetValue(key) == null || // If we don't have any value already set, allow a wildcard to be set
									currentSection.HasWildCardValue(key)) // If the current value was set from a wildcard, allow it to be overwritten
								{
									currentSection.SetValue(key, value, wildCardSection);
								}
							}
						}
					}

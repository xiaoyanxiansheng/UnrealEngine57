...
namespace UnrealGameSync
{
	...
	public class WorkspaceUpdateContext
	{
		...
		public static WorkspaceUpdateOptions GetOptionsFromConfig(GlobalSettings globalSettings, UserWorkspaceSettings workspaceSettings)
		{
			...
			if (workspaceSettings.Filter.LocalProjects ?? globalSettings.Filter.LocalProjects ?? false)
			{
				options |= WorkspaceUpdateOptions.SyncLocalProjects;
			}
		...
		public async Task<(WorkspaceUpdateResult, string)> ExecuteAsync(IPerforceSettings perforceSettings, ProjectInfo project, WorkspaceStateWrapper stateMgr, ILogger logger, CancellationToken cancellationToken)
		{
			...
			if (Context.Options.HasFlag(WorkspaceUpdateOptions.Sync) || Context.Options.HasFlag(WorkspaceUpdateOptions.SyncSingleChange))
			{
				using (TelemetryStopwatch syncTelemetryStopwatch = new TelemetryStopwatch("Workspace_Sync", project.TelemetryProjectIdentifier))
				{
					...
					List<string> relativeSyncPaths = GetRelativeSyncPaths(project, Context.Options, Context.SyncFilter);
...
#pragma warning restore CA5350 // Do Not Use Weak Cryptographic Algorithms
					...
					if (Context.Options.HasFlag(WorkspaceUpdateOptions.Sync) && !Context.Options.HasFlag(WorkspaceUpdateOptions.UpdateFilter))
					{
						...
						Context.ProjectConfigFile = await ReadProjectConfigFile(project.LocalRootPath, project.LocalFileName, project.ProjectIdentifier, logger);
						...
						if (codeChangeNumber == 0)
						{
							using (TelemetryStopwatch findingLastCodeChangeStopwatch = new TelemetryStopwatch("Workspace_Sync_FindingLastCodeChange", project.TelemetryProjectIdentifier))
							{
								...
								string[] uefnRules = Utility.GetUEFNFilter(Context.ProjectConfigFile);
								try
								{
									await foreach (PerforceChangeDetails details in Utility.EnumerateChangeDetails(perforce, minChangeNumber, maxChangeNumber: Context.ChangeNumber, syncPaths, codeRules, uefnRules, cancellationToken))
									{
								...
								catch (EpicGames.Perforce.PerforceException)
								{
									...
									await foreach (PerforceChangeDetails details in Utility.EnumerateChangeDetails(perforce, null, maxChangeNumber: Context.ChangeNumber, syncPaths, codeRules, uefnRules, cancellationToken))
									{
										if (details.ContainsCode)
										{
											codeChangeNumber = details.Number;
						...
						// Update the version files only when a non uefn project is selected
						if (!project.ClientFileName.EndsWith(".uefnproject", StringComparison.OrdinalIgnoreCase))
						{
			...
			if (Context.Options.HasFlag(WorkspaceUpdateOptions.SyncArchives) && !project.IsUEFNProject)
			{
				using (TelemetryStopwatch stopwatch = new TelemetryStopwatch("Workspace_SyncArchives", project.TelemetryProjectIdentifier))
				{
					...
					foreach ((string archiveType, IArchive? archive) in Context.ArchiveTypeToArchive)
					{
						...
						{
							removeOldBinaries = archive.RemoveOldBinaries.Value;
						}
		...
		public static List<string> GetSyncPaths(ProjectInfo project, WorkspaceUpdateOptions options, string[] syncFilter)
		{
			List<string> syncPaths = GetRelativeSyncPaths(project, options, syncFilter);
			return syncPaths.Select(x => project.ClientRootPath + x).ToList();
		}

		public static List<string> GetRelativeSyncPaths(ProjectInfo project, WorkspaceUpdateOptions options, IReadOnlyList<string>? syncFilter)
		{
			bool syncAllProjects = (options & WorkspaceUpdateOptions.SyncAllProjects) != 0;
			bool bSyncLocalProjects = (options & WorkspaceUpdateOptions.SyncLocalProjects) != 0;
			...
			// For UEFN projects we do not have any hardcoded sync paths
			else if (!project.ClientFileName.EndsWith(".uefnproject", StringComparison.OrdinalIgnoreCase))
			{
				...
				if (bSyncLocalProjects)
				{
					DirectoryInfo rootDir = project.LocalRootPath.ToDirectoryInfo();
					// We don't use EnumerateFiles because we don't want to visit subdirectories of projects, or other directories which we know cannot contain
					// projects but may be still around in the workspace (Intermediate, Content etc.).
					Queue<DirectoryInfo> traversalQueue = new Queue<DirectoryInfo>();
					traversalQueue.Enqueue(rootDir);
					bool bAddedTemplatesDir = false;
					while (traversalQueue.Count > 0)
					{
						DirectoryInfo currentDir = traversalQueue.Dequeue();
						bool bIsProjectDir = false;
						foreach (FileInfo file in currentDir.GetFiles())
						{
							if (file.Name.EndsWith(".uproject", System.StringComparison.OrdinalIgnoreCase))
							{
								bIsProjectDir = true;
								string relativeProjectPath = currentDir.FullName.Substring(project.LocalRootPath.FullName.Length).Replace('\\', '/');

								if (relativeProjectPath.StartsWith("/Templates", System.StringComparison.OrdinalIgnoreCase))
								{
									// Special handling for /Templates: this directory contains lots of projects and we don't want to add 40 or so individual
									// template directories to the sync filter. If at least one template is present, sync all of them.
									if (!bAddedTemplatesDir)
									{
										syncPaths.Add("/Templates/...");
										bAddedTemplatesDir = true;
									}
								}
								else
								{
									syncPaths.Add(relativeProjectPath + "/...");
								}
								break;
							}
						}
						if (bIsProjectDir)
						{
							// Projects cannot have sub-projects.
							continue;
						}
						foreach (DirectoryInfo subDir in currentDir.GetDirectories())
						{
							if (subDir.Name != "Engine" && subDir.Name != "Intermediate" && subDir.Name != "Saved" && subDir.Name != "Content" && subDir.Name != "Plugins")
							{
								traversalQueue.Enqueue(subDir);
							}
						}
					}
				}
		...
		public static async Task<ConfigFile> ReadProjectConfigFile(DirectoryReference localRootPath, FileReference selectedLocalFileName, string projectIdentifier, ILogger logger)
		{
			...
			foreach (FileInfo localConfigFile in localConfigFiles)
			{
				try
				{
					...
					projectConfig.Parse(lines, projectIdentifier);

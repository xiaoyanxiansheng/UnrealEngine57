...
{
	class TabControl : Control, ICustomThemeHandler
	{
		...
		bool _isHoveringCloseBtn = false;
		TabDragData? _dragState;
		Color _textColor = SystemColors.WindowText;
		Color _selectedColor = SystemColors.HotTrack;
		Color _borderColor = SystemColors.Window;
		Color _hoverColor = SystemColors.Window;
		Color _hoverStripeColor = Color.FromArgb(192, 192, 192);
		Color _closeColor = SystemColors.HotTrack;
		Color _closeColorHovered = Color.FromArgb(SystemColors.HotTrack.B, SystemColors.HotTrack.G, SystemColors.HotTrack.R);
		...
		protected override void OnMouseMove(MouseEventArgs e)
		{
			...
			if (_selectedTabIdx != -1)
			{
				bool isNowHoveringCloseBtn = _hoverTabIdx == _selectedTabIdx
								  && ((e.Location.X) > (_tabs[_hoverTabIdx]._minX + _tabs[_hoverTabIdx]._width - _tabs[_hoverTabIdx]._closeButtonWidth));
				if (_isHoveringCloseBtn != isNowHoveringCloseBtn)
				{
					_isHoveringCloseBtn = isNowHoveringCloseBtn;
					Invalidate();
				}
			}
		}
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			using (SolidBrush hoverBrush = new SolidBrush(_hoverColor))
			using (SolidBrush hoverStripeBrush = new SolidBrush(_hoverStripeColor))
			{
				for (int idx = 0; idx < _tabs.Count; idx++)
				{
					TabData tab = _tabs[idx];
					...
					{
						DrawBackground(e.Graphics, tab, hoverBrush, hoverStripeBrush, tab._highlight);
					}
					else
					{
						DrawBackground(e.Graphics, tab, backBrush, backBrush, tab._highlight);
					}
				}
			}
			...
			if (_selectedTabIdx != -1)
			{
				...
				Color tabBackColor = _hoverColor;

				using (SolidBrush backBrush = new SolidBrush(tabBackColor))
				using (SolidBrush selectedBrush = new SolidBrush(_selectedColor))
				{
					DrawBackground(e.Graphics, selectedTab, backBrush, selectedBrush, selectedTab._highlight);
				}
				...
				Color closeColor = _isHoveringCloseBtn ? _closeColorHovered : _closeColor;
				...
				using (SolidBrush backBrush = new SolidBrush(closeColor))
				{
					e.Graphics.FillEllipse(backBrush, closeButton);
				}

				using(Pen crossPen = new Pen(tabBackColor, 2.0f * dpiScaleX))
				{
				...
				{
					e.Graphics.DrawLine(borderPen, selectedTab._minX, 0, selectedTab._minX + selectedTab._width, 0);
					e.Graphics.DrawLine(borderPen, selectedTab._minX, 0, selectedTab._minX, ClientSize.Height - 2);
					e.Graphics.DrawLine(borderPen, selectedTab._minX + selectedTab._width, 0, selectedTab._minX + selectedTab._width, ClientSize.Height - 2);
				}
			}
			...
			using (Pen borderPen = new Pen(_borderColor))
			{
				...
				e.Graphics.DrawLine(borderPen, 0, ClientSize.Height - 1, ClientSize.Width, ClientSize.Height - 1);
			}
		}
		...
		void DrawText(Graphics graphics, TabData tab)
		{
			float dpiScaleX = graphics.DpiX / 96.0f;
			float dpiScaleY = graphics.DpiY / 96.0f;
			TextRenderer.DrawText(graphics, tab._name, Font, new Rectangle(tab._minX + (int)(TabPadding * dpiScaleX), 0, tab._textSize.Width, ClientSize.Height - (int)(3 * dpiScaleY)), _textColor, TextFormatFlags.NoPadding | TextFormatFlags.EndEllipsis | TextFormatFlags.VerticalCenter);
		}

		public void ApplyTheme(ApplicationThemeManager themeManager)
		{
			_textColor = themeManager.Theme.TextColor;
			_hoverColor = themeManager.Theme.InputBackgroundColor;
			_selectedColor = themeManager.Theme.LinkColor;
			_borderColor = themeManager.Theme.InputBorderColor;
			_closeColor = themeManager.Theme.LinkColor;
			_closeColorHovered = themeManager.Theme.LinkColorHover;

			float hoverWeight = 0.4f;
			_hoverStripeColor = Color.FromArgb(
				(int)Math.Round(_hoverColor.R * (1 - hoverWeight) + Color.Gray.R * hoverWeight),
				(int)Math.Round(_hoverColor.G * (1 - hoverWeight) + Color.Gray.G * hoverWeight),
				(int)Math.Round(_hoverColor.B * (1 - hoverWeight) + Color.Gray.B * hoverWeight));

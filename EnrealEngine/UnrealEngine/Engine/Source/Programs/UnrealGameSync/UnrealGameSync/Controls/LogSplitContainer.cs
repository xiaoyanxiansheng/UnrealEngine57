...
{
	public class LogSplitContainer : SplitContainer, ICustomThemeHandler
	{
		...
		Color _activeCaption = SystemColors.ActiveCaption;
		Color _inactiveCaption = SystemColors.InactiveCaption;
		Color _inactiveCaptionText = SystemColors.InactiveCaptionText;
		readonly InvertibleImage _logIcon = new InvertibleImage(Properties.Resources.Log, false);
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			using (Pen captionBorderPen = new Pen(_borderColor, 1.0f))
			{
			...
			if (_hoverOverClose)
			{
				using (Brush activeCaptionBrush = new SolidBrush(_activeCaption))
				{
					e.Graphics.FillRectangle(activeCaptionBrush, crossX - buttonSize / 2, crossY - buttonSize / 2, buttonSize, buttonSize);
				}
			}
			else if (_mouseDownOverClose)
			{
				using (Brush inactiveCaptionBrush = new SolidBrush(_inactiveCaption))
				{
					e.Graphics.FillRectangle(inactiveCaptionBrush, crossX - buttonSize / 2, crossY - buttonSize / 2, buttonSize - 2,buttonSize - 2);
				}
			}
			if (_hoverOverClose || _mouseDownOverClose)
			{
				using (Pen borderPen = new Pen(_inactiveCaptionText, 1.0f))
				{
			...
			{
				e.Graphics.DrawImage(_logIcon.ResolveImage() ?? Properties.Resources.Log, new Rectangle(crossX - (buttonSize / 2) - 1, crossY - (buttonSize / 2) - 1, buttonSize + 2, buttonSize + 2), new Rectangle(16, 0, 16, 16), GraphicsUnit.Pixel);
			}
			else
			{
				e.Graphics.DrawImage(_logIcon.ResolveImage() ?? Properties.Resources.Log, new Rectangle(crossX - (buttonSize / 2) - 1, crossY - (buttonSize / 2) - 1, buttonSize + 2, buttonSize + 2), new Rectangle(0, 0, 16, 16), GraphicsUnit.Pixel);
			}

			TextRenderer.DrawText(e.Graphics, Caption, CaptionFont, new Rectangle(2, captionMinY, ClientRectangle.Width - 20, captionMaxY - captionMinY), _textColor, TextFormatFlags.Left | TextFormatFlags.VerticalCenter);
		}
		...
		public void ApplyTheme(ApplicationThemeManager themeManager)
		{
			...
			BackColor = themeManager.Theme.InputBackgroundColor;
			ForeColor = themeManager.Theme.TextColor;

			_textColor = themeManager.Theme.TextColor;
			_borderColor = themeManager.Theme.InputBorderColor;
			_activeCaption = themeManager.Theme.ActiveCaption;
			_inactiveCaption = themeManager.Theme.InactiveCaption;
			_inactiveCaptionText = themeManager.Theme.InactiveCaptionText;
		}

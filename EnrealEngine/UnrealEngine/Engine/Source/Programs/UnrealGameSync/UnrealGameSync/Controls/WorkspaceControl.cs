...
#pragma warning restore CA2213
		...
		public WorkspaceControl(IWorkspaceControlOwner owner, DirectoryReference appDataFolder, string? apiUrl, OpenProjectInfo openProjectInfo, IServiceProvider serviceProvider, UserSettings settings)
		{
			...
			FilterContextMenu_Type_UEFN.Enabled = openProjectInfo.ProjectInfo.IsUEFNProject;
			FilterContextMenu_Type_UEFN.Visible = openProjectInfo.ProjectInfo.IsUEFNProject;

			s_syncActionsInactiveToolTip.ShowAlways = true;
			...
			if (projectConfigFile != null)
			{
				...
				if (cloudHost != null && cloudNamespace != null && ProjectInfo.StreamName != null)
				{
					...
					catch (Exception e)
					{
						...
						_logger.LogError(e, "Error determining available platforms for snapshot import from host {Host} and namespace {Namespace}", cloudHost, cloudNamespace);
					}
				}
				else
				{
			...
			if (ProjectInfo.LocalFileName.HasExtension(".uproject") || ProjectInfo.LocalFileName.HasExtension(".uefnproject"))
			{
				DirectoryReference baseDirectory = ProjectInfo.LocalFileName.HasExtension(".uefnproject") ?
					ProjectInfo.LocalFileName.Directory.ParentDirectory! : ProjectInfo.LocalFileName.Directory;

				FileReference logoFileName = FileReference.Combine(baseDirectory, "Build", "UnrealGameSync.png");
			...
			_perforceMonitor = new PerforceMonitor(
				perforceClientSettings,
				openProjectInfo.ProjectInfo,
				_workspace,
				openProjectInfo.LatestProjectConfigFile,
				openProjectInfo.ProjectInfo.CacheFolder,
				openProjectInfo.LocalConfigFiles,
				_serviceProvider);
		...
		void StartSync(int changeNumber, bool syncOnly, WorkspaceUpdateCallback? callback, bool skipKnownChangeLists)
		{
			...
			if (!syncOnly)
			{
				if (ProjectInfo.IsUEFNProject == false)
				{
		...
		void StartWorkspaceUpdate(int changeNumber, WorkspaceUpdateOptions options, WorkspaceUpdateCallback? callback, bool skipKnownChangeLists)
		{
			...
			if (options.HasFlag(WorkspaceUpdateOptions.SyncArchives) && !ProjectInfo.IsUEFNProject)
			{
		...
		bool ShouldShowChange(ChangesRecord change, string[] excludeChanges)
		{
			if (_projectSettings.FilterType != FilterType.None)
			{
				...
				if (_projectSettings.FilterType == FilterType.UEFN && !details.ContainsUEFN)
				{
					return false;
				}
			}
		...
		private bool CanSyncChange(int changeNumber, bool skipKnownChangeLists)
		{
			...
			// We do not support pcb's when a UEFN project is selected, so we can sync any changelist
			if (ProjectInfo.IsUEFNProject)
			{
				return true;
			}
		...
		private List<BadgeInfo> CreateTypeBadges(int changeNumber)
		{
			...
			if (_perforceMonitor.TryGetChangeDetails(changeNumber, out details))
			{
				...
				if (ProjectInfo.IsUEFNProject && details.ContainsUEFN)
				{
					Color badgeColor = ProgramApplicationContext.GetTheme().UEFNBadgeColor;
					badges.Add(new BadgeInfo("UEFN", "ChangeType", badgeColor));
				}
			}
			if (badges.Count == 0)
			{
...
#pragma warning restore CA2000
		...
		private void OptionsButton_Click(object sender, EventArgs e)
		{
			...
			if (ProjectInfo.IsUEFNProject)
			{
				...
				OptionsContextMenu_SyncPrecompiledBinaries.ToolTipText = "Precompiled binaries are not available for UEFN projects";
		...
		private static readonly ToolTip s_syncActionsInactiveToolTip = new ToolTip();
		private const string _syncActionsInactiveToolTipText = "After sync actions disabled for UEFN projects";
		...
		private void UpdateSyncActionCheckboxes()
		{
			...
			else if (ProjectInfo.IsUEFNProject)
			{
				// Make the checkboxes inactive
				AfterSyncingLabel.ForeColor = theme.DimmedText;
				GenerateAfterSyncCheckBox.ReadOnly = true;
				BuildAfterSyncCheckBox.ReadOnly = true;
				RunAfterSyncCheckBox.ReadOnly = true;
				...
				// Uncheck all options to avoid confusing users
				GenerateAfterSyncCheckBox.Checked = false;
				BuildAfterSyncCheckBox.Checked = false;
				RunAfterSyncCheckBox.Checked = false;
				OpenSolutionAfterSyncCheckBox.Checked = false;

				s_syncActionsInactiveToolTip.SetToolTip(AfterSyncingLabel, _syncActionsInactiveToolTipText);
				s_syncActionsInactiveToolTip.SetToolTip(GenerateAfterSyncCheckBox, _syncActionsInactiveToolTipText);
				s_syncActionsInactiveToolTip.SetToolTip(BuildAfterSyncCheckBox, _syncActionsInactiveToolTipText);
				s_syncActionsInactiveToolTip.SetToolTip(RunAfterSyncCheckBox, _syncActionsInactiveToolTipText);
				s_syncActionsInactiveToolTip.SetToolTip(OpenSolutionAfterSyncCheckBox, _syncActionsInactiveToolTipText);
			}
			else
			{
		...
		public void ScheduleTimerElapsed()
		{
			if (_settings.ScheduleEnabled)
			{
				...
				{
					WorkspaceUpdateOptions options = WorkspaceUpdateOptions.Sync | WorkspaceUpdateOptions.ScheduledBuild;

					if ( ProjectInfo.IsUEFNProject == false )
					{
						options |= WorkspaceUpdateOptions.SyncArchives | WorkspaceUpdateOptions.GenerateProjectFiles | WorkspaceUpdateOptions.Build;
					}
		...
		private void MoreToolsContextMenu_CleanWorkspace_Click(object sender, EventArgs e)
		{
			...
			WorkspaceUpdateOptions options = WorkspaceUpdateContext.GetOptionsFromConfig(_settings.Global, _workspaceSettings);
			List<string> syncPaths = WorkspaceUpdate.GetSyncPaths(_workspace.Project, options, combinedSyncFilter);
...
#pragma warning restore CA2000
		...
		private bool ShouldSyncPrecompiledEditor => !ProjectInfo.IsUEFNProject && _settings.Archives.Any(x => x.Enabled && x.Type == IArchiveChannel.EditorArchiveType) && GetArchiveChannels().Any(x => x.Type == "Editor");
...
#pragma warning restore CA2000
		...
		private void FilterButton_Click(object sender, EventArgs e)
		{
			...
			FilterContextMenu_Type_UEFN.Checked = _projectSettings.FilterType == FilterType.UEFN;
		...
		private void FilterContextMenu_Type_UEFN_Click(object sender, EventArgs e)
		{
			_projectSettings.FilterType = FilterType.UEFN;
			_projectSettings.Save(_logger);
		...
		private Color? GetNewTintColor()
		{
			...
			using FileStream fs = new FileStream(newConfigFile.FullName, FileMode.Open,FileAccess.Read, FileShare.Read);

...
namespace UnrealGameSync
{
	...
	class LinkStatusElement : StatusElement
	{
		...
		public override void Draw(Graphics graphics, StatusElementResources resources)
		{
			Color textColor = ProgramApplicationContext.GetTheme().LinkColor;
			if (MouseDown)
			{
				textColor = ProgramApplicationContext.GetTheme().LinkColorActive;
			}
			else if (MouseOver)
			{
				textColor = ProgramApplicationContext.GetTheme().LinkColorHover;
			}
	...
	class BadgeStatusElement : StatusElement
	{
		...
		public override void Draw(Graphics graphics, StatusElementResources resources)
		{
			...
			Color badgeColor = MouseOver ? _hoverBackgroundColor : _backgroundColor;
			...
			Color textColor = ApplicationTheme.IsColorLight(badgeColor) ? Color.Black : Color.White;
			graphics.SmoothingMode = prevSmoothingMode;
			TextRenderer.DrawText(graphics, _name, resources.BadgeFont, Bounds, textColor, TextFormatFlags.HorizontalCenter | TextFormatFlags.VerticalCenter | TextFormatFlags.SingleLine | TextFormatFlags.NoPrefix | TextFormatFlags.PreserveGraphicsClipping);
		}
	}
	...
	class StatusLine
	{
		...
		public void AddText(string inText, FontStyle inStyle = FontStyle.Regular)
		{
			...
			_elements.Add(new TextStatusElement(inText, textColor, inStyle));
			_modified = true;
		}
	...
	class StatusPanel : Panel, ICustomThemeHandler
	{
		const float LineSpacing = 1.35f;

		InvertibleImage _projectLogo = new InvertibleImage(Properties.Resources.DefaultProjectLogo, false);
		...
		{
			_projectLogo.Dispose();
			_projectLogo = new InvertibleImage(newProjectLogo, dispose);
		...
		public void Set(IEnumerable<StatusLine> newLines, StatusLine? newCaption, StatusLine? newAlert, Color? newTintColor)
		{
			...
			if (((_tintColor == null) != (newTintColor == null)) || (_tintColor != null && _tintColor.StandardColor != newTintColor))
			{
				Invalidate();
				_tintColor = newTintColor.HasValue ? new InvertibleColor(newTintColor.Value) : null;
			}
		...
		protected void LayoutElements(Graphics graphics)
		{
			...
			// Get the logo size
			Image drawProjectLogo = _projectLogo.ResolveImage() ?? Properties.Resources.DefaultProjectLogo;
		...
		protected override void OnPaintBackground(PaintEventArgs e)
		{
			...
			if (_tintColor != null)
			{
				...
				using (LinearGradientBrush backgroundBrush = new LinearGradientBrush(new Point(Width, 0), new Point(Width - tintSize, tintSize), _tintColor.ResolveColor(), BackColor))
				{
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			{
				e.Graphics.DrawImage(_projectLogo.ResolveImage() ?? Properties.Resources.DefaultProjectLogo, _projectLogoBounds);

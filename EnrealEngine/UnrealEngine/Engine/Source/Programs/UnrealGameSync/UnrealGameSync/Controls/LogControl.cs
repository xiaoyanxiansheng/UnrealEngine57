...
{
	public class LogControl : UserControl, ICustomThemeHandler
	{
		[Flags]
		...
		Color _warningColor = Color.FromArgb(128, 128, 0);
		Color _errorColor = Color.FromArgb(189, 54, 47);
		...
		public bool OpenFile(FileReference newLogFileName)
		{
			...
			{
				lines.Add(reader.ReadLine()!.TrimEnd('\r', '\n'));

				if (lines.Count >= batchSize)
				{
					AddLinesInternal(lines.Select(x => x + "\n").ToList());
					lines.Clear();
				}
			}
		...
		protected override void OnMouseUp(MouseEventArgs e)
		{
			...
			if (e.Button == MouseButtons.Right)
			{
				...
				ProgramApplicationContext.GetThemeManager()?.ApplyThemeRecursively(ContextMenuStrip);
			}
		}
		...
		public void ApplyTheme(ApplicationThemeManager themeManager)
		{
			...
			_warningColor= themeManager.Theme.WarningColor;
			_errorColor = themeManager.Theme.ErrorColor;
		}
	}

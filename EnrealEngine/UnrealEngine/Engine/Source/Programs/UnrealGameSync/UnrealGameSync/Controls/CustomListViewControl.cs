...
namespace UnrealGameSync
{
	...
	partial class CustomListViewControl : ListView, ICustomThemeHandler
	{
		...
		public void DrawNormalSubItem(DrawListViewSubItemEventArgs e)
		{
			if (e.SubItem != null)
			{
				DrawText(e.Graphics, e.SubItem.Bounds, Columns[e.ColumnIndex].TextAlign, _textColor, e.SubItem.Text);
			}
		}
		...
		public void DrawDefaultBackground(Graphics graphics, Rectangle bounds)
		{
			bounds.Inflate(0, 1);
			_ = this;
			using SolidBrush inputBGBrush = new SolidBrush(BackColor);
			graphics.FillRectangle(inputBGBrush, bounds);
		}
		...
		protected override void OnDrawColumnHeader(DrawListViewColumnHeaderEventArgs e)
		{
			...
			using (SolidBrush backBrush = new SolidBrush(theme.InputBackgroundColor))
			{
				e.Graphics.FillRectangle(backBrush, e.Bounds);
			}

			using (Pen backgroundPen = new Pen(theme.InputBackgroundColor))
			using (Pen linePen = new Pen(theme.BackgroundColor))
			{
				Rectangle r = e.Bounds;
				r.Width -= 1;
				r.Height -= 1;

				e.Graphics.DrawRectangle(backgroundPen, r);

				r.Width -= 1;
				r.Height -= 1;

				e.Graphics.DrawLine(linePen, r.X, r.Y, r.Right, r.Y);
				e.Graphics.DrawLine(linePen, r.X, r.Y, r.X, r.Bottom);

				e.Graphics.DrawLine(backgroundPen, r.X + 1, r.Bottom, r.Right, r.Bottom);
				e.Graphics.DrawLine(backgroundPen, r.Right, r.Y + 1, r.Right, r.Bottom);

				Rectangle newBounds = Rectangle.Inflate(e.Bounds, -TextRenderer.MeasureText(" ", e.Font).Width, 0);
				TextRenderer.DrawText(e.Graphics, e.Header?.Text, e.Font, newBounds, _textColor, TextFormatFlags.Left | TextFormatFlags.WordEllipsis | TextFormatFlags.VerticalCenter);
			}
		}
		...
		public void ApplyTheme(ApplicationThemeManager themeManager)
		{
			...
			ForeColor = themeManager.Theme.ForegroundColor;
		}
	}
}

...
namespace UnrealGameSync
{
	...
	{
		static ProgramApplicationContext? s_instance { get; set; }
		...
		readonly ApplicationThemeManager _themeManager;
...
#pragma warning restore CA2213
		...
		public ProgramApplicationContext(IPerforceSettings defaultPerforceSettings, UpdateMonitor updateMonitor, string? apiUrl, DirectoryReference dataFolder, EventWaitHandle activateEvent, bool restoreState, string? updateSpawn, string? projectFileName, bool preview, IServiceProvider serviceProvider, string? uri)
		{
			...
			// Create the theme manager before the settings so that the user's platform theme preference is detected first
			_themeManager = new ApplicationThemeManager();
			...
			((PerforceSettings)_defaultPerforceSettings).PreferNativeClient = _settings.SyncOptions.UseNativeClient ?? PerforceSyncOptions.DefaultUseNativeLibrary;
		...
		public static ApplicationThemeManager? GetThemeManager()
		{
			return s_instance?._themeManager;
		}

		public static ApplicationTheme GetTheme()
		{
			return s_instance?._settings.Theme ?? ApplicationTheme.ConstructDefaultTheme();
		}

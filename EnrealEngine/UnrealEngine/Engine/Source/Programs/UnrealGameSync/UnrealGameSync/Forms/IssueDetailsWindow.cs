...
{
	partial class IssueDetailsWindow : ThemedForm
	{
		class BuildGroup
		{
			...
			foreach (PerforceChangeRange range in _selectedStreamRanges)
			{
				if (range.Expanded)
				{
					if (range.Changes == null)
					{
						...
						{
							fetchingWidget.Line.AddText(range.ErrorMessage, textColor, FontStyle.Italic);
						}
						else
						{
							fetchingWidget.Line.AddText("Fetching changes, please wait...", textColor, FontStyle.Italic);
						}
...
#pragma warning restore CA2000
		...
		static void UpdateChangeTypeWidget(StatusLineListViewWidget typeWidget, PerforceChangeDetails? details)
		{
			...
			else
			{
				...
				{
					typeWidget.Line.AddBadge("Code", Theme.CodeBadgeColor, null);
				}
				if (details.ContainsContent)
				{
					typeWidget.Line.AddBadge("Content", Theme.ContentBadgeColor, null);
				}
			}
		}
		...
		private void BuildListView_DrawItem(object sender, DrawListViewItemEventArgs e)
		{
			...
			else if (e.Item.Tag is BuildGroup buildGroup)
			{
				...
				{
					backgroundColor = Theme.ErrorNotificationBGTint;
				}
				else if (buildGroup.Outcome == IssueBuildOutcome.Warning)
				{
					backgroundColor = Theme.WarningNotificationBGTint;
				}
				else if (buildGroup.Outcome == IssueBuildOutcome.Success)
				{
					backgroundColor = Theme.SuccessNotificationBGTint;
				}
				else
				{
					backgroundColor = Theme.StandardNotificationBGTint;
				}
			...
			if (args.Button == MouseButtons.Right)
			{
				...
				if (hitTest.Item != null && hitTest.Item.Tag != null)
				{
					...
					if (_contextMenuChange != null)
					{
						...
						ThemeManager?.ApplyThemeRecursively(BuildListContextMenu);
					}
				}
			}
		}

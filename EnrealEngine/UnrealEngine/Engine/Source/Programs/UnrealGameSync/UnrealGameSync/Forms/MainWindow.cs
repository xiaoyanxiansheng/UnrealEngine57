	...
	partial class MainWindow : ThemedForm, IWorkspaceControlOwner
	{
		...
		void PostAutomationRequest(AutomationRequest request)
		{
			try
			{
				...
				else if (request.Input.Type == AutomationRequestType.RunCustomTool)
				{
					AutomationRequestOutput output = RunCustomTool(request);
					request.SetOutput(new AutomationRequestOutput(AutomationRequestResult.Ok));
				}
				else
				{
		...
		AutomationRequestOutput RunCustomTool(AutomationRequest request)
		{
			using BinaryReader reader = new BinaryReader(new MemoryStream(request.Input.Data));
			string toolName = reader.ReadString();
			string command = reader.ReadString();
			...
			for (int existingTabIdx = 0; existingTabIdx < TabControl.GetTabCount(); existingTabIdx++)
			{
				...
				if (existingWorkspace != null)
				{
					// make sure the tool monitor has updated its state
					ToolUpdateMonitor.GetDataFromBackendAsync().Wait();

					if (ToolUpdateMonitor.GetTools().FirstOrDefault(t => String.Equals(t.Name, toolName, StringComparison.OrdinalIgnoreCase)) == null)
					{
						_logger.LogError("Unable to find tool {ToolName} when attempting to process uri request to it", toolName);
						return new AutomationRequestOutput(AutomationRequestResult.Error);
					}

					DirectoryReference? toolPath = ToolUpdateMonitor.GetToolPath(toolName);
					if (toolPath == null)
					{
						_logger.LogError("Unable to find path for {ToolName}", toolName);
						return new AutomationRequestOutput(AutomationRequestResult.Error);
					}

					// this is a async method but we do not wait for it as that blocks UGS
					_ = ToolUpdateMonitor.RunCommandAsync(toolName, command, CancellationToken.None);
					// we have started the tool, no need to check other workspaces for this command
					break; 
				}
			}

...
{
	partial class NewWorkspaceWindow : ThemedForm
	{
		...
		{
			public static async Task RunAsync(IPerforceConnection perforce, NewWorkspaceSettings settings, string owner, string hostName, ClientSubmitOptions clientSubmitOptions, CancellationToken cancellationToken)
			{
				...
				client.SubmitOptions = clientSubmitOptions;
		...
		private NewWorkspaceWindow(IPerforceSettings perforceSettings, string? forceStream, string? defaultStream, InfoRecord info, List<ClientsRecord> clients, IServiceProvider serviceProvider, UserSettings settings)
		{
			InitializeComponent();
		...
		public static bool ShowModal(IWin32Window owner, IPerforceSettings perforceSettings, string? forceStreamName, string currentWorkspaceName, UserSettings settings, IServiceProvider serviceProvider, [NotNullWhen(true)] out string? workspaceName)
		{
			...
			using NewWorkspaceWindow window = new NewWorkspaceWindow(perforceSettings, forceStreamName, task.Result.CurrentStream, task.Result.Info, task.Result.Clients, serviceProvider, settings);
		...
		private void OkBtn_Click(object sender, EventArgs e)
		{
			if (TryGetWorkspaceSettings(out _settings))
			{
				...
				ModalTask? result = PerforceModalTask.Execute(Owner, "Creating workspace", "Creating workspace, please wait...", _perforceSettings, (p, c) => NewWorkspaceTask.RunAsync(p, _settings, _info.UserName ?? "", _info.ClientHost ?? "", _userSettings.ClientSubmitOptions, c), logger);

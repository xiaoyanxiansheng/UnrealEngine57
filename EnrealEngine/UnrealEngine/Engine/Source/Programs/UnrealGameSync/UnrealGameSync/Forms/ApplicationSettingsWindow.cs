...
{
	partial class ApplicationSettingsWindow : ThemedForm
	{
		public enum Result
		{
			Cancel,
		...
		private ApplicationSettingsWindow(IPerforceSettings defaultPerforceSettings, string originalExecutableFileName, UserSettings settings, ToolUpdateMonitor toolUpdateMonitor, ILogger<ApplicationSettingsWindow> logger)
		{
			...
			foreach (KeyValuePair<string, string> themeIdNamePair in settings.ThemeList)
			{
				ThemeComboBox.Items.Add(themeIdNamePair);

				if (_settings.Theme?.ThemeId == themeIdNamePair.Key)
				{
					ThemeComboBox.SelectedItem = themeIdNamePair;
				}
			}
		...
		private void ApplySettings(Result result)
		{
			...
			string themeId = (ThemeComboBox.SelectedItem != null) ? ((KeyValuePair<string, string>)ThemeComboBox.SelectedItem).Key : "";
			bool didThemeChange = (_settings.Theme == null || _settings.Theme.ThemeId != themeId);
			...
			if (didThemeChange)
			{
				_settings.Theme = _settings.LoadTheme(themeId);
				_settings.Save(_logger);
			}

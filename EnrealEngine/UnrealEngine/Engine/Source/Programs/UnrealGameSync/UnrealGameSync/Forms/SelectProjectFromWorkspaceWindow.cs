...
{
	partial class SelectProjectFromWorkspaceWindow : ThemedForm
	{
		static class EnumerateWorkspaceProjectsTask
		{
			...
			static readonly string[] UEFNPatterns =
			{
				"....uproject",
				"....uprojectdirs",
				"....uefnproject"
			...
			public static async Task<List<string>> RunAsync(IPerforceConnection perforce, string clientName, bool allowUEFNProjects, CancellationToken cancellationToken)
			{
				...
				string[] patterns = allowUEFNProjects ? UEFNPatterns : Patterns;
		...
		public static bool ShowModal(IWin32Window owner, IPerforceSettings perforce, string workspaceName, string workspacePath, IServiceProvider serviceProvider, bool allowUEFNProjects, [NotNullWhen(true)] out string? newWorkspacePath)
		{
			...
			ModalTask<List<string>>? pathsTask = PerforceModalTask.Execute(owner, "Finding Projects", "Finding projects, please wait...", perforce, (p, c) => EnumerateWorkspaceProjectsTask.RunAsync(p, workspaceName, allowUEFNProjects, c), logger);

...
bool FLiveCodingManifest::Parse(FJsonObject& Object, FString& OutFailReason)
{
	...
	const TArray<TSharedPtr<FJsonValue>>* VfsArray;
	if (!Object.TryGetArrayField(TEXT("Vfs"), VfsArray))
	{
		OutFailReason = TEXT("missing Vfs field");
		return false;
	}

	for (const TSharedPtr<FJsonValue>& VfsValue : *VfsArray)
	{
		if (VfsValue->Type != EJson::Object)
		{
			OutFailReason = TEXT("invalid vfs object");
			return false;
		}

		const FJsonObject& VfsObject = *VfsValue->AsObject();

		FString VirtualPath;
		if (!VfsObject.TryGetStringField(TEXT("Virtual"), VirtualPath))
		{
			OutFailReason = TEXT("missing Vfs 'Virtual' field");
			return false;
		}

		FString LocalPath;
		if (!VfsObject.TryGetStringField(TEXT("Local"), LocalPath))
		{
			OutFailReason = TEXT("missing Vfs 'Local' field");
			return false;
		}

		VfsEntries.Emplace(VirtualPath, LocalPath);
	}

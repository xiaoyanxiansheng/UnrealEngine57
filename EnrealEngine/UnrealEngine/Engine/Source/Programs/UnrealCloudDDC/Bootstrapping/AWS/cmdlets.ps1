...
function Upload-AWSBootstrapperToS3Dev {
    ...
    aws s3 cp setup.zip s3://devtools-misc/dev-cloud-ddc-bootstrapper/aws/setup.zip --acl public-read --region eu-north-1
}
...
function Upload-AWSBootstrapperToS3Prod {
    ...
    Write-Host "Uploading to S3"
    aws s3 cp setup.zip s3://devtools-misc/cloud-ddc-bootstrapper/aws/setup.zip --acl public-read --region eu-north-1
}

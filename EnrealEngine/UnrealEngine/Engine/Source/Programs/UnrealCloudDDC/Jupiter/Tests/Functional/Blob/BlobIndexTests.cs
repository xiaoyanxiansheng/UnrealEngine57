...
namespace Jupiter.FunctionalTests.Storage
{
	public abstract class BlobIndexTests
	{
		...
		public async Task RunConsistencyCheckAsync()
		{
			...
			writer.BeginObject();
			writer.WriteString("stringField", nameof(RunConsistencyCheckAsync));
			writer.EndObject();
			...
			RefId putKey = RefId.FromName("newReferenceUploadObject2");
			...
			BlobIndexConsistencyCheckService? consistencyCheckService = _server!.Services.GetService<BlobIndexConsistencyCheckService>();
			Assert.IsNotNull(consistencyCheckService);

			// manually invoke the consistency check
			await consistencyCheckService.RunConsistencyCheckBlobReferencesAsync(CancellationToken.None);
		}

		...
		private async Task<bool> ReplicateOpAsync(NamespaceId ns, BucketId bucket, RefId key, BlobId objectToReplicate, CancellationToken cancellationToken)
		{
			...
			for (int i = 0; i < missingBlobs.Length; i++)
			{
				...
				blobReplicationTasks[i] = Task.Run(async () =>
				{
					...
					using IBufferedPayload payload = await _bufferedPayloadFactory.CreateFromStreamAsync(s, contentLength.Value, "ref-replicator", cancellationToken);

					await _blobService.PutObjectAsync(ns, payload, blobToReplicate, bucketHint: bucket, cancellationToken: cancellationToken);
				}, cancellationToken);
			}

	...
	public class MongoContentIdStore : MongoStore, IContentIdStore
	{
		...
		public async Task PutAsync(NamespaceId ns, ContentId contentId, BlobId[] blobIdentifiers, int contentWeight, CancellationToken cancellationToken)
		{
			...
			UpdateDefinition<MongoContentIdModelV0> update = Builders<MongoContentIdModelV0>.Update.AddToSet(m => m.ContentWeightToBlobsMap,
				new KeyValuePair<int, string[]>(contentWeight, blobIdentifiers.Select(id => id.ToString()).ToArray()));
		...
		public async IAsyncEnumerable<ContentIdMapping> GetContentIdMappingsAsync(NamespaceId ns, ContentId contentId, [EnumeratorCancellation] CancellationToken cancellationToken)
		{
			...
			IAsyncCursor<MongoContentIdModelV0> cursor = await collection.FindAsync(model => model.Ns == ns.ToString() && model.ContentId == contentId.ToString(), cancellationToken: cancellationToken);

			MongoContentIdModelV0 model = await cursor.FirstOrDefaultAsync(cancellationToken);

			foreach (KeyValuePair<int, string[]> entry in model.ContentWeightToBlobsMap)
			{
				yield return new ContentIdMapping(entry.Key, entry.Value.Select(s => new BlobId(s)).ToArray());
			}
		}
	}

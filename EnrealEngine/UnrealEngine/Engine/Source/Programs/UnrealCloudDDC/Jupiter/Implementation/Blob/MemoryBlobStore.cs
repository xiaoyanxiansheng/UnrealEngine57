...
namespace Jupiter.Implementation
{
	internal class MemoryBlobStore : IBlobStore
	{
		...
		{
			if (await GetBackend(targetNamespace).ExistsAsync(GetPath(blobId)))
			{
				// blob already exists, do not copy
				return;
			}

			BlobContents? contents = await GetBackend(ns).TryReadAsync(GetPath(blobId));
			if (contents == null)
			{
				throw new BlobNotFoundException(ns, blobId);
			}

			await PutObjectAsync(targetNamespace, contents.Stream, blobId);
		}

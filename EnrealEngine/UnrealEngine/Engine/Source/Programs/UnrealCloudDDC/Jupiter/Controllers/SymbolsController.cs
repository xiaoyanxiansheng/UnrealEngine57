...
namespace Jupiter.Controllers
{
	...
	public class SymbolsController : ControllerBase
	{
		...
		/// <param name="supportsRedirect">Set to true to indicate that you support getting redirected for this data</param>
			...
			[FromQuery] bool supportsRedirect = false)
		{
			...
			BlobContents referencedBlobContents = await _blobStore.GetObjectAsync(ns, BlobId.FromIoHash(payloadHash), storageLayers: null, supportsRedirectUri: supportsRedirect, bucketHint: bucket);
			...
			[FromRoute][Required] string moduleName)
		{
			...
			await _blobStore.PutObjectKnownHashAsync(ns, payload, attachmentHash, bucketName, bypassCache: bypassCache, cancellationToken: HttpContext.RequestAborted);

			(ContentId[], BlobId[]) missingHashes = await _refService.PutAsync(ns, bucketName, RefId.FromName($"{moduleName}.{pdbIdentifier}{pdbAge}.{filename}"), blobHeader, o, cancellationToken: HttpContext.RequestAborted);

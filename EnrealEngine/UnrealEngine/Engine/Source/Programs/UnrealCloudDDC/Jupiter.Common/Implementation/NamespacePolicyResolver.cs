...
namespace Jupiter.Common
{
	...
	public class NamespacePolicy
	{
		...
		/// <summary>
		/// Set to limit the amount of bytes used for filesystem storage - PerNamespaceGc needs to be set in the Filesystem settings for this to be applicable
		/// </summary>
		public ulong? MaxFilesystemStorageBytes { get; set; } = null;

		/// <summary>
		/// When enabled blobs that are written are not saved in the cache, only read blobs would be saved. Useful for write heavy operations were we frequently only read parts of the data.
		/// </summary>
		public bool? BypassCacheOnWrite { get; set; } = null;
		...
		public bool AllowOverwritesOfRefs { get; set; } = true;
	}

...
namespace Jupiter
{
	...
	public abstract class BaseStartup
	{
		...
		public void ConfigureServices(IServiceCollection services)
		{
			...
			string? traceScyllaRequestsString = Configuration["OTEL_TRACE_SCYLLA_REQUESTS"];
			_ = bool.TryParse(traceScyllaRequestsString, out bool traceScyllaRequests);
			...
			}).WithTracing(builder =>
			{
				...
				builder.AddSource("UnrealCloudDDC");

				if (traceScyllaRequests)
				{
					builder.AddSource("ScyllaDB");
				}
			}).WithMetrics(builder =>
			{
				builder
					.AddMeter("UnrealCloudDDC")
					.AddOtlpExporter()

...
namespace EpicGames.SmartlingLocalization
{
	...
	public abstract class SmartlingLocalizationProvider : LocalizationProvider
	{
		...
		public async override Task DownloadProjectFromLocalizationProvider(string ProjectName, ProjectImportExportInfo ProjectImportInfo)
		{
			...
			Watch.Stop();
			Logger.LogInformation($"Completed Smartling download for {ProjectName} project files in {Watch.ElapsedMilliseconds / 1000.0:F2} seconds.");
		}
		...
		private async Task<List<string>> DownloadLatestPOFile(string EpicLocale, string Platform, ProjectImportExportInfo ProjectImportInfo)
		{
			...
			while (true)
			{
				...
				catch (Exception Ex)
				{
					...
					{
						DownloadStopWatch.Stop();
						Logs.Add($"[FAILED] Downloading: '{ExportFile.FullName}' ({EpicLocale}) exhausted all retries in {DownloadStopWatch.ElapsedMilliseconds / 1000.0:F2} seconds. - {Ex}");
						return Logs;
					}
			...
			if (DownloadResponse.IsSuccessStatusCode)
			{
				...
				{
					Logger.LogInformation($"Culture directory for '{EpicLocale}' does not exist. Creating directory '{CultureDirectory.FullName}'");
					CultureDirectory.Create();
					CultureDirectory.Refresh();
				}
				// Do not initialize this to true. Further down we will do a check to see the previous state of the file and restore the read-only settings of the file.
				// Even if a file did not exist on disk yet and was downloaded by Smartling, fi this defaults to true, the file will be set to read-only
				// Newly downloaded files would be added by Perforce during commandlet execution. Files added by Perforce will NOT have their read-only status removed 
				// That only happens for perforce check outs. As such, adding a newly downloaded file that is read-only will cause commandlets that need to update the file to fail 
				bool ExportFileWasReadOnly = false;
				...
				// We refresh to update the state of the ifle after it's downloaded 
				ExportFile.Refresh();
				DownloadStopWatch.Stop();
				Logs.Add($"[SUCCESS] Downloading: '{SmartlingFileUri}' as '{ExportFile.FullName}' ({EpicLocale}) size {ExportFile.Length / 1024.0:F2}kb in {DownloadStopWatch.ElapsedMilliseconds / 1000.0:F2} seconds.");
			...
			else
			{
				...
				DownloadStopWatch.Stop();
				Logs.Add($"[FAILED] Downloading: '{ExportFile.FullName}' ({EpicLocale} in {DownloadStopWatch.ElapsedMilliseconds / 1000.0:F2} seconds. The file may need to be uploaded first.)");
		...
		public async override Task UploadProjectToLocalizationProvider(string ProjectName, ProjectImportExportInfo ProjectExportInfo)
		{
			...
			Watch.Stop();
			Logger.LogInformation($"Completed Smartling upload for {ProjectName} project files in {Watch.ElapsedMilliseconds / 1000.0:F2} seconds.");
		}

...
#pragma warning restore CA2254
	...
	public class BgScriptReader
	{
		...
		public static async Task<BgGraphDef?> ReadAsync(FileReference file, DirectoryReference rootDir, Dictionary<string, string> arguments, Dictionary<string, string?> defaultProperties, BgScriptSchema? schema, ILogger logger, string? singleNodeName = null)
		{
			...
			foreach (string argumentName in arguments.Keys)
			{
				if (!validArgumentNames.Contains(argumentName))
				{
					...
					logger.LogWarning(KnownLogEvents.AutomationTool_BuildGraphScript, "Unknown argument '{ArgumentName}' for '{Script}'", argumentName, file);
				}
			}
		...
		void AddCloudArtifactFromTask(BgScriptElement element, Dictionary<string, string> arguments)
		{
			...
			arguments.TryGetValue(nameof(CreateCloudArtifactTaskParameters.HordeArtifactID), out string? hordeArtifactID);
			...
			string? artifactName = String.IsNullOrEmpty(hordeArtifactID) ? name : hordeArtifactID;
		...
		async Task ReadDiagnosticAsync(BgScriptElement element, LogLevel level)
		{
			if (await EvaluateConditionAsync(element))
			{
				...
				bool reportOnExecution = ReadBooleanAttribute(element, "ReportOnExecution", false);
				string[] reportOnNodes = ReadListAttribute(element, "ReportOnNodes");

				BgDiagnosticDef diagnostic = new(element.Location.File.FullName, element.Location.LineNumber, level, message, reportOnExecution, reportOnNodes);

...
namespace AutomationScripts
{
	...
	public partial class Project : CommandUtils
	{
		...
		/// <summary>
		/// Returns the zen hostname and port connection information as a single string, e.g. "localhost:8558".
		/// When not using unixsockets, SocketHostName and HttpHostName are identical, but when using unixsockets
		/// they can be different. @see ZenRunContext.GetRobustHostNameAndPortStrings.
		/// </summary>
		private static void GetZenHostNameAndPort(DeploymentContext SC, out string SocketHostNameAndPort,
			out string HttpHostNameAndPort)
		{
			if (SC.PackageStoreData == null || SC.PackageStoreData.ZenServerStore == null)
			{
				SocketHostNameAndPort = $"localhost:{ZenRunContext.DefaultPort}";
				HttpHostNameAndPort = SocketHostNameAndPort;
			}
			else
			{
				GetZenHostNameAndPort(SC.PackageStoreData.ZenServerStore, out SocketHostNameAndPort,
					out HttpHostNameAndPort);
			}
		}
		/// <summary>
		/// @see GetZenHostNameAndPort(DeploymentContext SC,...)
		/// </summary>
		private static void GetZenHostNameAndPort(ZenServerStoreData ZenServer, out string SocketHostNameAndPort,
			out string HttpHostNameAndPort)
		{
			ZenRunContext.GetRobustHostNameAndPortStrings(ZenServer.HostName, ZenServer.HostPort,
				out SocketHostNameAndPort, out HttpHostNameAndPort);
		}

		private static List<ZenCookedFile> ReadZenCookedFilesFromZenServer(ProjectParams Params, DeploymentContext SC,
			bool bAutoLaunch, string PackageStoreFileArgName, string PackageStoreFileArgValue)
		{
			List<ZenCookedFile> ZenCookedFiles = new List<ZenCookedFile>();

			GetZenHostNameAndPort(SC, out string SocketHostNameAndPort, out string HttpHostNameAndPort);
			string ProjectId = SC.PackageStoreData.ZenServerStore.ProjectId;
			string OplogId = SC.PackageStoreData.ZenServerStore.OplogId;

			if (!ZenUtils.IsZenServerRunning(SocketHostNameAndPort))
			{
				if (!bAutoLaunch)
				{
					string ExceptionMessage = string.Format(
						"Failed sending oplog request to Zen at {0}: ZenServer is not running (or is not responding), and bAutoLaunch == false.",
						SocketHostNameAndPort);
					throw new AutomationException(ExceptionMessage);
				}

				string Arguments = String.Format("IoStore -StartZenServerForStage -SponsorProcessID={0} -{1}={2}",
					Environment.ProcessId, PackageStoreFileArgName, MakePathSafeToUseWithCommandLine(PackageStoreFileArgValue));
				...
				{
					string ExceptionMessage = string.Format(
						"Failed sending oplog request to Zen at {0}: ZenServer is not running (or is not responding), even after attempting to AutoLaunch.",
						SocketHostNameAndPort);
					throw new AutomationException(ExceptionMessage);
				}
			}
			...
			{
				HttpClient HttpClient = ZenUtils.CreateHttpClient(SocketHostNameAndPort);
				HttpClient.Timeout = TimeSpan.FromMinutes(20);

				string Uri = string.Format("http://{0}/prj/{1}/oplog/{2}/entries", HttpHostNameAndPort, ProjectId, OplogId);
				...
				{
					string ExceptionMessage = String.Format("Failed sending oplog request to Zen at {0} for oplog {1}.{2}: {3}.",
							SocketHostNameAndPort, ProjectId, OplogId, Ex.Message);
					throw new AutomationException(ExceptionMessage);
				}
			}
			if (!HttpGetResult.IsSuccessStatusCode)
			{
				string ExceptionMessage = String.Format("Failed reading oplog from Zen at {0} for oplog {1}.{2}, "
					+ "HttpClient.Send returned StatusCode {3}. Ensure that cooking was successful.",
					SocketHostNameAndPort, ProjectId, OplogId, HttpGetResult.StatusCode);
				throw new AutomationException(ExceptionMessage);
			}
			...
			foreach (CbField EntryField in OplogObject["entries"].AsArray())
			{
				...
				if (EntryField["key"].AsString() == "EndCook")
				{
					foreach (CbField PackageDataField in EntryField["files"].AsArray())
					{
						...
						if (RelativeFilename.EndsWith(".bin")
							|| RelativeFilename.EndsWith(".ushaderbytecode")
							|| RelativeFilename.EndsWith(".metallib")
							|| RelativeFilename.EndsWith(".metalmap")
							|| (RelativeFilename.EndsWith(".txt") && RelativeFilename.Contains("/Metadata/ChunkManifest/"))
							|| (RelativeFilename.EndsWith(".locmeta") && RelativeFilename.Contains("{project}/Content/Localization/") && RelativeFilename.Contains("locchunk"))
							|| (RelativeFilename.EndsWith(".locres") && RelativeFilename.Contains("{project}/Content/Localization/") && RelativeFilename.Contains("locchunk"))
							)
						{
		...
		private static void LoadPackageStoreData(ProjectParams Params, DeploymentContext SC)
		{
			...
			if (ProjectStoreFileInfo.Exists)
			{
				...
				if ((ParsedProjectStore != null) && (ParsedProjectStore.ZenServer != null))
				{
					...
					SC.PackageStoreData.ZenCookedFiles = ReadZenCookedFilesFromZenServer(Params, SC, true, "ProjectStore", ProjectStoreFile.FullName);
					return;
				}
			}
		...
		{
			GetZenHostNameAndPort(StoreData, out string SocketHostNameAndPort, out string HttpHostNameAndPort);
			using var Request = new HttpRequestMessage(HttpMethod.Get, string.Format("http://{0}/prj/{1}/oplog/{2}/{3}",
				HttpHostNameAndPort, StoreData.ProjectId, StoreData.OplogId, ChunkId));
		...
		private static List<FileReference> DiscoverCookedFiles(ProjectParams Params, DeploymentContext SC, bool bAlwaysFullyScanCookDir)
		{
			...
			if (bReadZenCookedFiles)
			{
				...
				GetZenHostNameAndPort(SC, out string SocketHostNameAndPort, out string HttpHostNameAndPort);
				HttpClient Client = ZenUtils.CreateHttpClient(SocketHostNameAndPort);
		...
		private static void StageLocalizationForCookedGameFeaturePlugins(DeploymentContext SC, List<string> CulturesToStage, List<KeyValuePair<StagedFileReference, FileReference>> StagedGameFeaturePlugins, List<FileReference> CookedFiles)
		{
			// build map of possible GFD -> Plugin
			Dictionary<FileReference, FileReference> GameFeatureDataToPlugin = new Dictionary<FileReference, FileReference>();
			foreach (KeyValuePair<StagedFileReference, FileReference> StagedPlugin in StagedGameFeaturePlugins)
			{
				string PluginNameWithExtension = StagedPlugin.Value.GetFileName();
				string PluginContentDirectory = Path.Combine(SC.PlatformCookDir.FullName, StagedPlugin.Key.ToString().TrimEnd(PluginNameWithExtension.ToCharArray()), "Content");
				FileReference PluginGFDAsset = new FileReference(Path.Combine(PluginContentDirectory, "GameFeatureData.uasset"));
				FileReference PluginGFDAssetWithPluginName = new FileReference(Path.Combine(PluginContentDirectory, $"{StagedPlugin.Value.GetFileNameWithoutAnyExtensions()}.uasset"));
				if (!GameFeatureDataToPlugin.ContainsKey(PluginGFDAsset))
				{
					GameFeatureDataToPlugin.Add(PluginGFDAsset, StagedPlugin.Value);
				}
				if (!GameFeatureDataToPlugin.ContainsKey(PluginGFDAssetWithPluginName))
				{
					GameFeatureDataToPlugin.Add(PluginGFDAssetWithPluginName, StagedPlugin.Value);
				}
			}
			foreach (FileReference CookedFile in CookedFiles)
			{
				// GFD was cooked. Stage this localized plugin data now.
				if (GameFeatureDataToPlugin.ContainsKey(CookedFile))
				{
					StageLocalizationDataForPlugin(SC, CulturesToStage, GameFeatureDataToPlugin[CookedFile]);
				}
			}
		}
		...
		public static void CreateStagingManifest(ProjectParams Params, DeploymentContext SC)
		{
			...
			DirectoryReference GameFeaturePluginDir = DirectoryReference.Combine(SC.ProjectRoot, "Plugins", "GameFeatures");
			...
			if (Params.HasDLCName)
			{
				...
				if (Params.StageOnlyCookedLocalizationForGameFeatures && CulturesToStage != null)
				{
					List<KeyValuePair<StagedFileReference, FileReference>> GameFeaturePluginsToCheckIfCookedDuringDLC = new List<KeyValuePair<StagedFileReference, FileReference>>();
					foreach (StageTarget Target in SC.StageTargets)
					{
						foreach (RuntimeDependency RuntimeDependency in Target.Receipt.RuntimeDependencies)
						{
							if (RuntimeDependency.Path.HasExtension(".uplugin") && RuntimeDependency.Path.IsUnderDirectory(GameFeaturePluginDir))
							{
								GameFeaturePluginsToCheckIfCookedDuringDLC.Add(new KeyValuePair<StagedFileReference, FileReference>(SC.GetStagedFileLocation(RuntimeDependency.Path), RuntimeDependency.Path));
								continue;
							}
						}
					}
					StageLocalizationForCookedGameFeaturePlugins(SC, CulturesToStage, GameFeaturePluginsToCheckIfCookedDuringDLC, CookedFiles);
				}
				...
				SC.CustomStagingHandlers.ForEach(H => H.GetFilesToStageForDLC(Params, SC));
			...
			else
			{
				if (!Params.IterateSharedBuildUsePrecompiledExe)
				{
					...
					SC.CustomStagingHandlers.ForEach(H => H.GetFilesToStage(Params, SC));
				...
				if (!Params.CookOnTheFly && !Params.SkipCookOnTheFly) // only stage the UFS files if we are not using cook on the fly
				{
					...
					if (!SC.StageTargetConfigurations.Any(Config => Config == UnrealTargetConfiguration.Shipping) && Params.StageTagMappingConfigFiles)
					{
						StageTagConfigFiles(SC, "Engine", SC.ProjectRoot.FullName);
						StageTagConfigFiles(SC, "Engine", SC.EngineRoot.FullName);
					}
					...
					foreach (KeyValuePair<StagedFileReference, FileReference> StagedPlugin in StagedPlugins)
					{
						...
						if (Info != null)
						{
							...
							// Stage Verse source files?
							if (bStageVerseSourceFiles && Info.Descriptor.bCanContainVerse)
							{
								if (Info.Descriptor.VersePath != null)
								{
									...
									if (DirectoryReference.Exists(PluginContentDirectory))
									{
										...
										List<FileReference> ContentFiles = SC.FindFilesToStage(PluginContentDirectory, StageFilesSearch.AllDirectories);
										SC.StageFiles(StagedFileType.NonUFS, ContentFiles.Where(x => x.HasExtension(".verse") || x.HasExtension(".vmodule")));
									}
								}
					...
					List<KeyValuePair<StagedFileReference, FileReference>> PluginsSkippedStagingLocalization = new List<KeyValuePair<StagedFileReference, FileReference>>();
					// Stage all plugin localization targets
					foreach (KeyValuePair<StagedFileReference, FileReference> StagedPlugin in StagedPlugins)
					{
						if (Params.StageOnlyCookedLocalizationForGameFeatures)
						{
							// Only stage localization for this plugin if it was actually cooked. This is checked later.
							if (StagedPlugin.Value.IsUnderDirectory(GameFeaturePluginDir))
							{
								PluginsSkippedStagingLocalization.Add(StagedPlugin);
								continue;
							}
						}
					...
					if (DirectoryReference.Exists(SC.PlatformCookDir))
					{
						...
						if (Params.StageOnlyCookedLocalizationForGameFeatures)
						{
							// now that we have the cooked files check which plugins should have their localization staged.
							StageLocalizationForCookedGameFeaturePlugins(SC, CulturesToStage, PluginsSkippedStagingLocalization, CookedFiles);
						}
					...
					else if (ShouldCreateIoStoreContainerFiles(Params, SC))
					{
						throw new AutomationException(String.Format("A package store manifest or project store is required when staging to IoStore. Expected to find cooked directory {0}. Ensure that cooking was successful.", SC.PlatformCookDir));
					}
					...
					if (ShouldCreateBinaryConfig(Params, SC) && !Params.SkipPak)
					{
						...
						{
							UnrealPakParams += $" -CustomConfig={SC.CustomConfig}";
						}
		...
		static void StageTagConfigFiles(DeploymentContext SC, string StagedBaseFolder, string BasePath)
		{
			List<string> AllRestrictedFolderNames = RestrictedFolder.GetNames().ToList();
			AllRestrictedFolderNames.AddRange(new List<string>() { "Platforms", "Restricted", SC.StageTargetPlatform.ToString() });

			List<string> AllPaths = new List<string>() { BasePath };

			GetPossibleRestrictedPaths(BasePath, AllRestrictedFolderNames, AllPaths);

			string TagsSettingsDirPath;
			foreach (var Dir in AllPaths)
			{
				TagsSettingsDirPath = Path.Combine(Dir, "Config", "Tests");
				if (DirectoryExists(TagsSettingsDirPath))
				{
					string Allowed = Path.Combine(Path.GetRelativePath(new DirectoryReference(BasePath).ParentDirectory.FullName, TagsSettingsDirPath), "Tags.ini");
					ConsoleUtils.WriteWarning("Allowing test tags config from restricted path " + Allowed);
					SC.ConfigFilesAllowList.Add(new StagedFileReference(Allowed));
					StageConfigFiles(SC, new DirectoryReference(TagsSettingsDirPath), null);
				}
			}
		}

		static void GetPossibleRestrictedPaths(string BasePath, List<string> RestrictedFolderNames, List<string> OutRestrictedFolders)
		{
			string PotentialPath;
			foreach (string RestrictedFolderName in RestrictedFolderNames)
			{
				PotentialPath = Path.Combine(BasePath, RestrictedFolderName);
		...
		static void StageConfigFiles(DeploymentContext SC, DirectoryReference ConfigDir, string PlatformExtensionName)
		{
			...
			ConcurrentBag<FileReference> ConfigsToStage = new ConcurrentBag<FileReference>();
			ConfigFiles.AsParallel().ForAll(ConfigFile =>
			{
			...
			foreach (FileReference ConfigFile in ConfigsToStage)
			{
		...
		private static List<PakFileRules> GetPakFileRules(ProjectParams Params, DeploymentContext SC)
		{
			...
			if (Params.CreateDefaultOnDemandPakRule)
			{
		...
		private static void CreatePakUsingStagingManifest(ProjectParams Params, DeploymentContext SC)
		{
			...
			ConcurrentDictionary<string, string> DefaultPak = new ConcurrentDictionary<string, string>(Environment.ProcessorCount, UnrealPakResponseFile.Count());
			ConcurrentDictionary<string, string> OptionalPak = new ConcurrentDictionary<string, string>(Environment.ProcessorCount, UnrealPakResponseFile.Count());
			ConcurrentDictionary<string, string> OnDemandPak = new ConcurrentDictionary<string, string>(Environment.ProcessorCount, UnrealPakResponseFile.Count());
			ConcurrentDictionary<string, string> OnDemandOptionalPak = new ConcurrentDictionary<string, string>(Environment.ProcessorCount, UnrealPakResponseFile.Count());
			...
			Parallel.ForEach(UnrealPakResponseFile, StagingFile =>
			{
				...
				PakFileRules? MatchingRule = ApplyPakFileRules(PakRulesList, StagingFile, null, null, out bExcludeFromPaks);
				...
				if (Params.GenerateOnDemandPakForNonChunkedBuild && MatchingRule.HasValue && MatchingRule.Value.bOnDemand)
				{
					if (Params.GenerateOptionalPakForNonChunkedBuild && OrigExt.Equals(OptionalBulkDataFileExtension))
					{
						OnDemandOptionalPak.TryAdd(StagingFile.Key, StagingFile.Value);
					}
					else
					{
						OnDemandPak.TryAdd(StagingFile.Key, StagingFile.Value);
					}
				}
				else if (Params.GenerateOptionalPakForNonChunkedBuild && OrigExt.Equals(OptionalBulkDataFileExtension))
				{
					OptionalPak.TryAdd(StagingFile.Key, StagingFile.Value);
				}
				else
				{
					DefaultPak.TryAdd(StagingFile.Key, StagingFile.Value);
				}
			});
			...
			PakInputs.Add(new CreatePakParams(SC.ShortProjectName, DefaultPak.ToDictionary(), Params.Compressed, Params.RehydrateAssets, Params.DLCOverrideEncryptionKeyGuid));
			if (!OptionalPak.IsEmpty)
			{
				string OptionalPakName = SC.ShortProjectName + "optional";
				PakInputs.Add(new CreatePakParams(OptionalPakName, OptionalPak.ToDictionary(), Params.Compressed, Params.RehydrateAssets, Params.DLCOverrideEncryptionKeyGuid));
			}
			if (!OnDemandPak.IsEmpty)
			{
				string OnDemandPakName = SC.ShortProjectName + "ondemand";
				PakInputs.Add(new CreatePakParams(OnDemandPakName, OnDemandPak.ToDictionary(), Params.Compressed, Params.RehydrateAssets, Params.DLCOverrideEncryptionKeyGuid, bOnDemand : true));
			}
			if (!OnDemandOptionalPak.IsEmpty)
			{
				string OnDemandOptionalPakName = SC.ShortProjectName + "ondemandoptional";
				PakInputs.Add(new CreatePakParams(OnDemandOptionalPakName, OnDemandOptionalPak.ToDictionary(), Params.Compressed, Params.RehydrateAssets, Params.DLCOverrideEncryptionKeyGuid, bOnDemand : true));
			}
		...
		private static void CreatePaks(ProjectParams Params, DeploymentContext SC, List<CreatePakParams> PakParamsList, EncryptionAndSigning.CryptoSettings CryptoSettings)
		{
			...
			foreach (CreatePakParams PakParams in PakParamsList)
			{
				...
				if (Params.HasDLCName)
				{
					string DLCPakName = null;
					if (Params.DLCSkipNamedPak)
					{
						DLCPakName = OutputFilename;
					}
					else
					{
						DLCPakName = Params.DLCFile.GetFileNameWithoutExtension() + OutputFilename;
					}
					if (Params.DLCOverrideStagedSubDir != null)
					{
						OutputRelativeLocation = StagedFileReference.Combine(SC.RelativeProjectRootForStage, Params.DLCOverrideStagedSubDir, "Content", "Paks", DLCPakName + ".pak");
					}
					else
					{
						...
						OutputRelativeLocation = StagedFileReference.Combine(SC.RelativeProjectRootForStage, PluginSubdirectory, "Content", "Paks", SC.FinalCookPlatform, DLCPakName + ".pak");
					}
				}
				else
				{
			...
			if (!string.IsNullOrWhiteSpace(Params.Upload))
			{
				...
				{
					int Port = ZenRunContext.DefaultPort;
			...
			for (int Idx = 0; Idx < PakParamsList.Count; Idx++)
			{
				...
				if (Params.HasCreateReleaseVersion)
				{
					...
					if (InternalUtils.SafeFileExists(Path.ChangeExtension(OutputLocation.FullName, ".sig")))
					{
						InternalUtils.SafeCopyFile(Path.ChangeExtension(OutputLocation.FullName, ".sig"), Path.ChangeExtension(ReleaseVersionPath, ".sig"));
					}
					if (bShouldCreateIoStoreContainerFiles)
					{
		...
		private static byte[] GetChunkFromPackageStore(DeploymentContext SC, string Filename)
		{
			if (SC.PackageStoreData != null && SC.PackageStoreData.ZenServerStore != null && SC.PackageStoreData.ZenCookedFiles != null)
			{
				...
				if (ZenFile != null)
				{
					...
					HttpClient HttpClient = ZenUtils.CreateHttpClient(SocketHostNameAndPort);
					using var Request = new HttpRequestMessage(HttpMethod.Get, string.Format("http://{0}/prj/{1}/oplog/{2}/{3}",
						HttpHostNameAndPort, SC.PackageStoreData.ZenServerStore.ProjectId, SC.PackageStoreData.ZenServerStore.OplogId, ZenFile.ChunkId.ToString()));
					Request.Headers.Add("Accept", "application/octet-stream");
					try
					{
						HttpGetResult = HttpClient.Send(Request);
					...
					{
						throw new AutomationException(String.Format("Failed sending chunk request to Zen at {0}. Ensure that the server is running.",
							SocketHostNameAndPort));
					}

					if (!HttpGetResult.IsSuccessStatusCode)
					{
						throw new AutomationException(String.Format("Failed reading chunk {0}.{1} from Zen. Ensure that cooking was successful.",
							SC.PackageStoreData.ZenServerStore.ProjectId, SC.PackageStoreData.ZenServerStore.OplogId));
					}
		...
		public static void ApplyStagingManifest(ProjectParams Params, DeploymentContext SC)
		{
			...
			SC.CustomStagingHandlers.ForEach(H => H.PostStagingFileCopy(Params, SC));
		}
		...
		public static List<DeploymentContext> CreateDeploymentContext(ProjectParams Params, bool InDedicatedServer, bool DoCleanStage = false)
		{
			...
			foreach (var StagePlatform in PlatformsToStage)
			{
				...
				SC.CustomStagingHandlers = CustomStagingHandler.GetCustomStagingHandlers(Params, SC);
		...
		private static void SetupCustomStageCopyHandler(ProjectParams Params, DeploymentContext SC)
		{
			...
			if (!string.IsNullOrEmpty(CustomStageCopyHandlerName))
			{
				...
				try
				{
					...
					SC.CustomStageCopyHandler.Init(ConfigCache.ReadHierarchy(ConfigHierarchyType.Engine, DirectoryReference.FromFile(Params.RawProjectPath), SC.StageTargetPlatform.IniPlatformType));
				}
				catch
				{

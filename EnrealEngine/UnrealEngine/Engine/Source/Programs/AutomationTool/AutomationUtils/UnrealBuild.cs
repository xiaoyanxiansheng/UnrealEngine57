...
namespace AutomationTool
{
	...
	public class UnrealBuild
	{
		...
		{
			List<FileReference> ManifestFiles = TargetToManifest != null ? new List<FileReference>(Targets.Count) : null;

			StringBuilder FullCommandLine = new StringBuilder();
			if (Targets.Count == 1)
			{
				FileReference ManifestFile = null;
				if (TargetToManifest != null)
				{
					ManifestFiles.Add(GetManifestFile(Targets[0], -1));
					ManifestFile = ManifestFiles[0];
				}
				FullCommandLine.Append(GetTargetArguments(Targets[0], ManifestFile));
			}
			else
			{
				...
				for (int Idx = 0; Idx < Targets.Count; Idx++)
				{
					...
					if (TargetToManifest != null)
					{
						...
						ManifestFile = ManifestFiles[Idx];
					}
					Arguments.Add("-Target=" + GetTargetArguments(Targets[Idx], ManifestFile));
				}

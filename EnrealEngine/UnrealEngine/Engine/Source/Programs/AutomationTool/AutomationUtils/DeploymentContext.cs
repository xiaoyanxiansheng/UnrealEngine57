...
public class DeploymentContext //: ProjectParams
{
	...
	/// <summary>
	/// Allows a platform to change how pak files are generated and how it is staged, packaged and deployed - for example, when packaging for a specific game store
	/// </summary>
	...
	/// </summary>
	public List<CustomStagingHandler> CustomStagingHandlers = [];
		...
		)
	{
		...
		// Only stage the Insights base settings with the program.
		if (InProgram)
		{
			IniSuffixAllowList.Add("unrealinsightssettings.ini");
		}
		else
		{
			IniSuffixDenyList.Add("unrealinsightssettings.ini");
		}

...
namespace Gauntlet
{
	...
	public static class UnrealHelpers
	{
		...
		public class ModuleConfig
		{
			...
			public ModuleConfig(string InModuleName, UnrealTargetRole InRole, string InFlavor = "")
			{
		...
		public static Dictionary<string, ModuleConfig> CustomModules = new();
		...
		/// <param name="InRole">Flavor of the role</param>
		public static void AddCustomModuleName(string InModuleName, UnrealTargetRole InRole, string InFlavor = "")
		{
			AddCustomModuleName(new ModuleConfig(InModuleName, InRole, InFlavor));
		}

		public static void AddCustomModuleName(ModuleConfig Config)
		{
			CustomModules.Add(Config.ModuleName, Config);
		}
		...
		static public ConfigInfo GetUnrealConfigFromFileName(string InProjectName, string InName)
		{
			...
			foreach (string ModuleName in CustomModules.Keys)
			{
				ProjectNameRegEx += string.Format("|{0}", ModuleName);
			}
			string RegExMatch = string.Format(@"^(?:.+[_-])?(({0}(Game|Client|Server|CookedEditor)){1})(?:[-_](.+?)[-_](Debug|DebugGame|Test|Shipping))?(?:[_-](.+))?$", ShortName, ProjectNameRegEx);
			...
			if (NameMatch.Success)
			{
				...
				Config.Flavor = NameMatch.Groups[6].ToString().ToLower();
				if (CustomModules.TryGetValue(ModuleName, out ModuleConfig ModuleConfig))
				{
					Config.RoleType = ModuleConfig.Role;
					Config.Flavor = ModuleConfig.Flavor;
				}

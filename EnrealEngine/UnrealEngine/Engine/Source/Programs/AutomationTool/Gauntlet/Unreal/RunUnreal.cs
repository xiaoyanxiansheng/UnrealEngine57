...
namespace Gauntlet
{
	...
	public class RunUnreal : BuildCommand
	{
		...
		public virtual ExitCode RunTests(UnrealTestOptions ContextOptions)
		{
			...
			// Tag the the job with metadata about the build 
			Horde.AddJobBuildMetaData(BuildInfo);
			...
			foreach (ArgumentWithParams PlatformWithParams in ContextOptions.PlatformList)
			{
				...
				foreach (UnrealTargetRole Type in Enum.GetValues(typeof(UnrealTargetRole)))
				{
					...
					string TagFilter = Globals.Params.ParseValue("TagFilter", string.Empty);
					...
					{
						Role.ExtraArgs += string.Format(" -ExecCmds=\"Automation SetTagFilter {0}\"", TagFilter);
					}
			...
			// Generate Horde summary for CIS test
		...
		IEnumerable<ITestNode> CreateTestList(UnrealTestContext Context, Params DefaultParams, ArgumentWithParams PlatformParams = null)
		{
			...
			foreach (var Test in Context.Options.TestList)
			{
				...
				Reservation.Tags DeviceTags = new Reservation.Tags();
				DeviceTags.AddTags(Globals.Params.ParseValues("RequiredDeviceTags", true), Reservation.Tags.Type.Required);
				DeviceTags.AddTags(Globals.Params.ParseValues("PreferredDeviceTags", true), Reservation.Tags.Type.Preferred);
				DeviceTags.AddTags(Globals.Params.ParseValues("UndesiredDeviceTags", true), Reservation.Tags.Type.Undesired);
				DeviceTags.AddTags(Globals.Params.ParseValues("BlockedDeviceTags", true), Reservation.Tags.Type.Blocked);

				TestContext.Constraint = new UnrealDeviceTargetConstraint(UnrealPlatform, PerfSpec, Model, DeviceName, DeviceTags);

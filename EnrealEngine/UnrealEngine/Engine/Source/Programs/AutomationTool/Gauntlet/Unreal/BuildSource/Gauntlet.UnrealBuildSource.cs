...
namespace Gauntlet
{
	public class UnrealBuildSource : IBuildSource
	{
		...
		IEnumerable<IBuild> GetMatchingBuilds(UnrealTargetRole InRole, UnrealTargetPlatform? InPlatform, UnrealTargetConfiguration InConfiguration, BuildFlags InFlags, string InFlavor="")
		{
			...
			IEnumerable<IBuild> MatchingBuilds = PlatformBuilds.Where((B) =>
			{
				if (B.CanSupportRole(InRole)
					...
					&& (B.Flavor.Equals(InFlavor, StringComparison.OrdinalIgnoreCase)))
				{
					return true;
				}
		...
		private string GenerateProcessedCommandLine(string InCommandLine)
		{
			...
			MatchCollection Matches = GauntletCommandLine.CommandlineOptionRegex.Matches(InCommandLine);
			...
			foreach (Match M in Matches)
			{
				...
				if (CommandlineTokens.ContainsKey(Name))
				{
					...
					{
						Log.Info(string.Format("Duplicate flag {0} found and ignored. Please fix this. ", Name));
					}
					...
					else
					{
						...
						{
							Log.Info(string.Format("Duplicate flag {0}={1} found and ignored. Please fix this. ", Name, Params));
						}
						else
						{
				...
				{
					CommandlineTokens.Add(Name, Params);
				}
			}
		...
		private string GetModuleOverlayExecutablePath(UnrealTargetRole role, UnrealTargetPlatform platform)
		{
			return Path.Join(
				...
				UnrealHelpers.CustomModules.FirstOrDefault(Module => Module.Value.Role == role).Key
			);
		}
		...
		virtual public string GetRelativeExecutablePath(UnrealTargetRole TargetRole, UnrealTargetPlatform TargetPlatform, UnrealTargetConfiguration TargetConfiguration)
		{
			...
			{
				bool HasCustomTarget = UnrealHelpers.CustomModules.Any(Module => Module.Value.Role == UnrealTargetRole.Editor);
				string EditorTarget = HasCustomTarget ? UnrealHelpers.CustomModules.FirstOrDefault(Module => Module.Value.Role == UnrealTargetRole.Editor).Key : string.Empty;

...
namespace Gauntlet
{
	...
	{
		private readonly string AndroidLlvmProfdataPath;
		string LocalOutputDirectory;
		string LocalProfDataFile;

		public AndroidPGOPlatform()
		{
			if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
			{
				AndroidLlvmProfdataPath = Path.Combine(Environment.ExpandEnvironmentVariables("%NDKROOT%"), @"toolchains\llvm\prebuilt\windows-x86_64\bin\llvm-profdata.exe");
			}
			else
			{
				AndroidLlvmProfdataPath = Path.Combine(Environment.ExpandEnvironmentVariables("%NDKROOT%"), "toolchains/llvm/prebuilt/darwin-x86_64/bin/llvm-profdata");
			}
		}

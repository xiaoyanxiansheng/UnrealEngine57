...
namespace Gauntlet
{
	...
	*/
	...
	namespace Utils
	{
		...
		public class Image
		{
			...
			public static bool ResizeImages(string InDirectory, uint MaxWidth)
			{
				...
				if (Files.Count() == 0)
				{
					Log.Verbose("Could not find files at {0} to resize", InDirectory);
					return false;
				}
			...
			public static bool ConvertImages(string InDirectory, string OutDirectory, string OutExtension, bool DeleteOriginals)
			{
				...
				if (Files.Count() == 0)
				{
					Log.Verbose("Could not find files at {0} to convert", InDirectory);
					return false;
				}
	...
	public static class FileUtils
	{
		...
		/// <summary>
		/// Convert file path to Unix-style path
		/// </summary>
		/// <param name="Path"></param>
		/// <returns></returns>
		static public string ConvertPathToUnix(string Path)
		{
			return Path.Replace("\\", "/");
		}
	}

...
namespace Gauntlet
{
	...
	/// <summary>
	/// Static helper class for querying platform specific target support
	/// </summary>
	public static class PlatformTargetSupport
	{
		public static IPlatformTargetSupport Get(UnrealTargetPlatform Platform)
		{
			if (PlatformSupportInstances == null || PlatformSupportInstances.Count == 0)
			{
				PlatformSupportInstances = Utils.InterfaceHelpers.FindImplementations<IPlatformTargetSupport>().ToList();
			}

			return PlatformSupportInstances
				.Where(x => x.Platform == Platform)
				.FirstOrDefault();
		}

		public static string GetHostMountedPath(UnrealTargetPlatform Platform, string Path) => Get(Platform).GetHostMountedPath(Path);
		public static bool IsHostMountingSupported(UnrealTargetPlatform Platform) => Get(Platform).IsHostMountingSupported();

		private static List<IPlatformTargetSupport> PlatformSupportInstances;
	}

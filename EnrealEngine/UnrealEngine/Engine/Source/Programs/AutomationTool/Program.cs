	...
	public partial class Program
	{
		...
		public static async Task<int> Main(string[] Arguments)
		{
			...
			try
			{
				...
				DirectoryReference.SetCurrentDirectory(Unreal.RootDirectory);

				// Ensure we can resolve any external assemblies as necessary.
				DirectoryReference AutomationToolLocation = new DirectoryReference(Path.GetDirectoryName(Assembly.GetEntryAssembly().GetOriginalLocation()));
				AssemblyUtils.InstallAssemblyResolver(AutomationToolLocation.FullName);
				AssemblyUtils.InstallRecursiveAssemblyResolver(AutomationToolLocation.FullName);
            ...
            {
				// If there was an error before a log writer was attached, do so now
				if (!Log.HasFileWriter() && ReturnCode != ExitCode.Success)
				{
					FileReference errorLogFile = FileReference.Combine(Unreal.WritableEngineDirectory, "Programs/AutomationTool/Saved/Logs/ErrorLog.txt");
					Log.AddFileWriterWithoutBackup("UnhandledFileLogListener", errorLogFile);
					Logger.LogError("AutomationTool encountered an error before a log file was set, see '{File}' for more details", errorLogFile);
				}
		...
		static async Task<ExitCode> MainProc()
		{
			...
			HashSet<FileReference> ScriptModuleAssemblyPaths = CompileScriptModule.InitializeScriptModules(
                    ...
                    {
						Logger.LogInformation("Building {Count} projects...", Count);
					},
					Log.Logger);
			...
			AssemblyUtils.InstallRecursiveAssemblyResolver(AssemblyPath.Directory.FullName);

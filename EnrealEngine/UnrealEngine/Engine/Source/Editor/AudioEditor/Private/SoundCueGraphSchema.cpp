...
#include UE_INLINE_GENERATED_CPP_BY_NAME(SoundCueGraphSchema)
...
void USoundCueGraphSchema::GetAssetsGraphHoverMessage(const TArray<FAssetData>& Assets, const UEdGraph* HoverGraph, FString& OutTooltipText, bool& OutOkIcon) const
{
	...
	for (const FAssetData& AssetData : Assets)
	{
		...
		// As soon as one of the items is a sound or dialogue wave, say we can drag it on... we actually eat only the sound waves.
		if (Cast<const USoundWave>(Asset) != nullptr || Cast<const UDialogueWave>(Asset) != nullptr)
		{
...
void USoundCueGraphSchema::DroppedAssetsOnGraph(const TArray<FAssetData>& Assets, const FVector2f& GraphPosition, UEdGraph* Graph) const
{
	...
	for (const FAssetData& AssetData : Assets)
	{
		UDialogueWave* DialogueWave = Cast<UDialogueWave>(AssetData.GetAsset());
		if (DialogueWave)
		{
			if (!AssetReferenceFilter.IsValid() || AssetReferenceFilter->PassesFilter(AssetData))
			{
...
TSharedPtr<IAssetReferenceFilter> USoundCueGraphSchema::MakeAssetReferenceFilter(const UEdGraph* Graph)
{
	if (const USoundCueGraph* SoundCueGraph = Cast<const USoundCueGraph>(Graph))
	{
		if (const USoundCue* SoundCue = SoundCueGraph->GetSoundCue())
		{
			if (GEditor)
			{
				FAssetReferenceFilterContext AssetReferenceFilterContext;
				AssetReferenceFilterContext.AddReferencingAsset(SoundCue);

...
void FSkeletalMeshEditor::SaveAsset_Execute()
{
	OnPreSaveAssetDelegate.Broadcast();
	ISkeletalMeshEditor::SaveAsset_Execute();
}

void FSkeletalMeshEditor::SaveAssetAs_Execute()
{
	OnPreSaveAssetAsDelegate.Broadcast();
	ISkeletalMeshEditor::SaveAssetAs_Execute();
}
...
void FSkeletalMeshEditor::RegisterTabSpawners(const TSharedRef<class FTabManager>& InTabManager)
{
	...
	SkeletalMeshEditorMenuCategory = InTabManager->AddLocalWorkspaceMenuCategory(LOCTEXT("WorkspaceMenu_SkeletalMeshEditor", "SkeletalMeshEditor"));
}
...
void FSkeletalMeshEditor::InitSkeletalMeshEditor(const EToolkitMode::Type Mode, const TSharedPtr<IToolkitHost>& InitToolkitHost, USkeletalMesh* InSkeletalMesh)
{
	...
	ModeUILayer = MakeShareable(new FSkeletalMeshEditorModeUILayer(PinnedToolkitHost.Get()));
	ModeUILayer->SetModeMenuCategory(SkeletalMeshEditorMenuCategory);
...
{
	ISkeletalMeshEditor::OnToolkitHostingStarted(Toolkit);
...
{
	ISkeletalMeshEditor::OnToolkitHostingFinished(Toolkit);
	
	ModeUILayer->OnToolkitHostingFinished(Toolkit);
}

void FSkeletalMeshEditor::PostInitAssetEditor()
{
	ISkeletalMeshEditor::PostInitAssetEditor();
...
{
	if (const USkeletalMesh* const Mesh = GetPersonaToolkit()->GetPreviewMesh())
	{
		MenuBuilder.BeginSection(TEXT("ApplyClothingMenu"), LOCTEXT("ApplyClothingMenuHeader", "Available Assets"));
		{
			for (UClothingAssetBase* const Asset : Mesh->GetMeshClothingAssets())
			{
				if (Asset)  // Asset could be null if the relevant plugin isn't loaded
				{
					FUIAction Action;
					Action.CanExecuteAction = FCanExecuteAction::CreateSP(this, &FSkeletalMeshEditor::CanApplyClothing, InLodIndex, InSectionIndex);

					if (const UClothingAssetCommon* const AssetCommon = Cast<UClothingAssetCommon>(Asset))
					{
						const int32 NumClothLods = AssetCommon->GetNumLods();
						for (int32 ClothLodIndex = 0; ClothLodIndex < NumClothLods; ++ClothLodIndex)
						{
							Action.ExecuteAction = FExecuteAction::CreateSP(this, &FSkeletalMeshEditor::OnApplyClothingAssetClicked, Asset, InLodIndex, InSectionIndex, ClothLodIndex);

							MenuBuilder.AddMenuEntry(
								FText::Format(LOCTEXT("ApplyClothingMenuItem", "{0} - LOD{1}"), FText::FromString(Asset->GetName()), FText::AsNumber(ClothLodIndex)),
								LOCTEXT("ApplyClothingMenuItem_ToolTip", "Apply this clothing data to the selected mesh LOD and section"),
								FSlateIcon(),
								Action);
						}
					}
					...
					{
						const int32 ClothLodIndex = InLodIndex;  // Use the same LOD as the cloth for all other types of clothing data
						Action.ExecuteAction = FExecuteAction::CreateSP(this, &FSkeletalMeshEditor::OnApplyClothingAssetClicked, Asset, InLodIndex, InSectionIndex, ClothLodIndex);
	...
	FScopedSuspendAlternateSkinWeightPreview ScopedSuspendAlternateSkinnWeightPreview(Mesh);
	{
		...
		if (InAsset)
		{
			...
			if (InAsset->BindToSkeletalMesh(Mesh, InLodIndex, InSectionIndex, InClothingLod))
			{
				...
				verify(Mesh->GetMeshClothingAssets().Find(InAsset, AssetIndex));
	...
	if (FAnimationViewportClient* AnimViewportClient = static_cast<FAnimationViewportClient*>(&ViewportClient))
	{
		AnimViewportClient->OnSetBoneSize.BindLambda([InSkeletalMesh=SkeletalMesh](float InBoneSize)
		{
			if (InSkeletalMesh)
			{
				...
				InSkeletalMesh->SetBoneDrawSize(InBoneSize);
			}
		});
		...
		AnimViewportClient->OnGetBoneSize.BindLambda([InSkeletalMesh=SkeletalMesh]() -> float
		{
			if (InSkeletalMesh)
			{
				return InSkeletalMesh->GetBoneDrawSize();
			}
...
FSimpleMulticastDelegate& FSkeletalMeshEditor::OnPreSaveAsset()
{
	return OnPreSaveAssetDelegate;
}

FSimpleMulticastDelegate& FSkeletalMeshEditor::OnPreSaveAssetAs()
{
	return OnPreSaveAssetAsDelegate;
}

TSharedPtr<FUICommandInfo> FSkeletalMeshEditor::GetResetBoneTransformsCommand()
{
	const FSkeletalMeshEditorCommands Commands = FSkeletalMeshEditorCommands::Get();
	return Commands.ResetBoneTransforms;
}

TSharedPtr<FUICommandInfo> FSkeletalMeshEditor::GetResetAllBonesTransformsCommand()
{
	const FSkeletalMeshEditorCommands Commands = FSkeletalMeshEditorCommands::Get();
	return Commands.ResetAllBonesTransforms;
}
...
TSharedPtr<ISkeletalMeshNotifier> FSkeletalMeshEditorBinding::GetNotifier()
{
	if (!Notifier)
	{
		Notifier = MakeShared<FSkeletalMeshEditorNotifier>(Editor.Pin().ToSharedRef());
	}
	return Notifier;	
}

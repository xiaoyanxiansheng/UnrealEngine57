...
public:
	...
	UE_API virtual FReply OnPreviewKeyDown( const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent ) override;
...
private:
	...
	UE_API FReply HandleKeyDown(const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent);

	/** Handler for when text in the editable text box changed */
	UE_API void HandleTextChanged(const FText& NewText);

	/** Handler for when text in the editable text box changed */
	UE_API void HandleTextCommitted(const FText& NewText, ETextCommit::Type CommitType);

	/** Called by SListView when the selection changes in the suggestion list */
	UE_API void OnSelectionChanged( TSharedPtr<FSuggestionListEntry> NewValue, ESelectInfo::Type SelectInfo );

	/** Makes the widget for a suggestion message in the list view */
	UE_API TSharedRef<ITableRow> MakeSuggestionListItemWidget(TSharedPtr<FSuggestionListEntry> Suggestion, const TSharedRef<STableViewBase>& OwnerTable);
	...
	UE_API TSharedPtr<FSuggestionListEntry> GetSelectedSuggestion() const;

	/** Default implementation of OnAssetSearchBoxSuggestionFilter, if no external implementation is provided */
	static UE_API void DefaultSuggestionFilterImpl(const FText& SearchText, TArray<FAssetSearchBoxSuggestion>& PossibleSuggestions, FText& SuggestionHighlightText);

	/** Default implementation of OnAssetSearchBoxSuggestionChosen, if no external implementation is provided */
	static UE_API FText DefaultSuggestionChosenImpl(const FText& SearchText, const FString& Suggestion);

...
namespace UE::Editor::Widgets
{
	...
	struct FFilterCategoryMenu
	{
		...
		UE_API FFilterCategoryMenu(const FName& InSectionExtensionHook, const FText& InSectionHeading);
	};
...
UCLASS(MinimalAPI)
class UAssetFilterBarContext : public UObject
{
	GENERATED_BODY()
	...
	void CreateFiltersMenuCategory(FToolMenuSection& Section, const TArray<TSharedPtr<FCustomClassFilterData>> CustomClassFilterDatas) const
	{
		for (int32 ClassIdx = 0; ClassIdx < CustomClassFilterDatas.Num(); ++ClassIdx)
		{
			...
			Section.AddMenuEntry(
				...
				FUIAction(
					FExecuteAction::CreateSP( const_cast< SAssetFilterBar<FilterType>* >(this), &SAssetFilterBar<FilterType>::FilterByType, CustomClassFilterData ),
					FCanExecuteAction(),
	...
	virtual void PopulateAddFilterMenu(UToolMenu* Menu, TSharedPtr<FFilterCategory> MenuExpansion, FOnFilterAssetType OnFilterAssetType)
	{
		...
		if (MenuExpansion)
		{
			...
			if (FFilterCategoryMenu* ExpandedCategory = CategoryToMenuMap.Find( MenuExpansion ))
			{
                ...
                Section.AddMenuEntry(
                	...
                	FUIAction(
                	FExecuteAction::CreateSP( this, &SAssetFilterBar<FilterType>::FilterByTypeCategory, MenuExpansion, ExpandedCategory->Classes ),
                	FCanExecuteAction(),
		...
		for (const TPair<TSharedPtr<FFilterCategory>, FFilterCategoryMenu>& CategoryMenuPair : CategoryToMenuMap)
		{
			Section.AddSubMenu(
				...
				FUIAction(
				FExecuteAction::CreateSP(this, &SAssetFilterBar<FilterType>::FilterByTypeCategory, CategoryMenuPair.Key, CategoryMenuPair.Value.Classes),
				FCanExecuteAction(),

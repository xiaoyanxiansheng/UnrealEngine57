...
protected:
	UE_API FReply OnDropped(const FGeometry& InGeometry, const FDragDropEvent& InDragDropEvent);
	UE_API virtual bool OnAllowDrop(TSharedPtr<FDragDropOperation> DragDropOperation) const override;
	UE_API virtual bool OnIsRecognized(TSharedPtr<FDragDropOperation> DragDropOperation) const override;
	UE_API virtual void OnDragLeave(const FDragDropEvent& DragDropEvent) override;

private:
	UE_API TArray<FAssetData> GetDroppedAssets(TSharedPtr<FDragDropOperation> DragDropOperation, bool& bOutRecognizedEvent) const;

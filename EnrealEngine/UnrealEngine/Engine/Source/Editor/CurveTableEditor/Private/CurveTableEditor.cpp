...
{
	const UCurveTable* CurveTable = GetCurveTable();

	// Synchronize the list views if the table has data in it
	if (TableView && ((CurveTable && CurveTable->HasRichCurves()) || AvailableColumns.Num() > 0))
	{

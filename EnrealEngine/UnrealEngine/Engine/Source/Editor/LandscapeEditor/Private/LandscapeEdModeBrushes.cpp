...
public:
	...
	virtual bool CanPaint(const TSet<ULandscapeComponent*>& InAffectedComponents, bool bInHasUnloadedComponents) const 
	{
		...
		if (!EdMode->CanEditSelectedTargetLayer())
		{
			return false;
		}
		...
		{
			const ALandscapeProxy* LandscapeProxy = Component ? Component->GetLandscapeProxy() : nullptr;
			if (Component == nullptr || LandscapeProxy == nullptr)
			{
				return false;
			}
	...
	virtual void Tick(FEditorViewportClient* ViewportClient, float DeltaTime) override
	{
		...
		// Store components from the previous tick
		TSet<ULandscapeComponent*> PreviousComponents;
		PreviousComponents.Reserve(BrushMaterialComponents.Num());
		Algo::TransformIf(BrushMaterialComponents, PreviousComponents, [](TWeakObjectPtr<ULandscapeComponent> Component) { return Component.IsValid(); }, [](TWeakObjectPtr<ULandscapeComponent> Component) { return Component.Get(); });
		...
		BrushMaterialComponents.Empty(NewComponents.Num());
		Algo::Transform(NewComponents, BrushMaterialComponents, [](ULandscapeComponent* Component) { return TWeakObjectPtr<ULandscapeComponent>(Component); });
	...
	virtual FLandscapeBrushData ApplyBrush(const TArray<FLandscapeToolInteractorPosition>& InInteractorPositions) override
	{
		...
		{
			FIntRect SpotExclusiveBounds;
			SpotExclusiveBounds.Min.X = FMath::FloorToInt32(InteractorPosition.Position.X - TotalRadius);
			SpotExclusiveBounds.Min.Y = FMath::FloorToInt32(InteractorPosition.Position.Y - TotalRadius);
			SpotExclusiveBounds.Max.X = FMath::CeilToInt32(InteractorPosition.Position.X + TotalRadius) + 1;
			SpotExclusiveBounds.Max.Y = FMath::CeilToInt32(InteractorPosition.Position.Y + TotalRadius) + 1;

			if (ExclusiveBounds.IsEmpty())
			{
				ExclusiveBounds = SpotExclusiveBounds;
			}
			else
			{
				ExclusiveBounds.Union(SpotExclusiveBounds);
			}
		}

		// Clamp to landscape bounds.  GetLandscapeExtent provides inclusive bounds.
		...
		// Clip using exclusive bounds so we can detect empty result.
		ExclusiveBounds.Clip(FIntRect(MinX, MinY, MaxX + 1, MaxY + 1));
		if (ExclusiveBounds.Area() <= 0)
		{
			// Brush is completely outside the landscape extent
			return FLandscapeBrushData();
		}

		// check if region to be modified overlaps with an unloaded component
		// TODO [chris.tchou] : The exclusive bounds here (InclusiveMax+1) shouldn't be necessary since the code looks like it expects inclusive values.  Without it, edges can be modified by the brush. Possibly a bug in the brush apply code.
		bool bHasUnloadedComponents = LandscapeInfo->HasUnloadedComponentsInRegion(ExclusiveBounds.Min.X, ExclusiveBounds.Min.Y, ExclusiveBounds.Max.X, ExclusiveBounds.Max.Y);
		...
		for (const FLandscapeToolInteractorPosition& InteractorPosition : InteractorPositions)
		{
			...
			SpotExclusiveBounds.Max.X = FMath::CeilToInt32(InteractorPosition.Position.X + TotalRadius) + 1;
			SpotExclusiveBounds.Max.Y = FMath::CeilToInt32(InteractorPosition.Position.Y + TotalRadius) + 1;
			SpotExclusiveBounds.Clip(ExclusiveBounds);

			for (int32 Y = SpotExclusiveBounds.Min.Y; Y < SpotExclusiveBounds.Max.Y; Y++)
			{
				float* Scanline = BrushData.GetDataPtr(FIntPoint(0, Y));

				for (int32 X = SpotExclusiveBounds.Min.X; X < SpotExclusiveBounds.Max.X; X++)
				{
...
public:
	...
	virtual void Tick(FEditorViewportClient* ViewportClient, float DeltaTime) override
	{
		TSet<TWeakObjectPtr<ULandscapeComponent>, TWeakObjectPtrSetKeyFuncs<TWeakObjectPtr<ULandscapeComponent>>> NewComponents;
		...
		if (!ViewportClient->IsMovingCamera() || EdMode->IsAdjustingBrush(ViewportClient))
		{
			...
			if (LandscapeInfo && LandscapeInfo->ComponentSizeQuads > 0)
			{
				...
				// Set brush material for components in new region
				for (TWeakObjectPtr<ULandscapeComponent> NewComponent : NewComponents)
				{
		...
		// Remove the material from any old components that are no longer in the region
		TSet<TWeakObjectPtr<ULandscapeComponent>, TWeakObjectPtrSetKeyFuncs<TWeakObjectPtr<ULandscapeComponent>>> RemovedComponents = BrushMaterialComponents.Difference(NewComponents);
		for (TWeakObjectPtr<ULandscapeComponent> RemovedComponent : RemovedComponents)
		{
			if (RemovedComponent.IsValid())
			{
				RemovedComponent->EditToolRenderData.ToolMaterial = nullptr;
				RemovedComponent->UpdateEditToolRenderData();
			}
	...
	virtual FLandscapeBrushData ApplyBrush(const TArray<FLandscapeToolInteractorPosition>& InteractorPositions) override
	{
		...
		else
		{
			...
			for (TWeakObjectPtr<ULandscapeComponent> Component : BrushMaterialComponents)
			{
...
public:
	...
	virtual void Tick(FEditorViewportClient* ViewportClient, float DeltaTime) override
	{
		if (GLandscapeEditRenderMode & ELandscapeEditRenderMode::Gizmo || GLandscapeEditRenderMode & ELandscapeEditRenderMode::Select)
		{
			...
			if (Gizmo && Gizmo->TargetLandscapeInfo.IsValid() && (Gizmo->TargetLandscapeInfo == EdMode->CurrentToolTarget.LandscapeInfo.Get()) && Gizmo->GizmoTexture && Gizmo->GetRootComponent())
			{
				ULandscapeInfo* LandscapeInfo = Gizmo->TargetLandscapeInfo.Get();
				if (LandscapeInfo && LandscapeInfo->GetLandscapeProxy())
				{
					...
					float Angle = static_cast<float>(-EdMode->UISettings->CurrentGizmoActor->GetActorRotation().Euler().Z * PI / 180.0f);
					FVector4 LandscapeLocation(EdMode->UISettings->CurrentGizmoActor->GetActorLocation().X, EdMode->UISettings->CurrentGizmoActor->GetActorLocation().Y, EdMode->UISettings->CurrentGizmoActor->GetActorLocation().Z, Angle);
					...
					// Convert NewComponents to TWeakObjectPtr TSet
					TSet<TWeakObjectPtr<ULandscapeComponent>, TWeakObjectPtrSetKeyFuncs<TWeakObjectPtr<ULandscapeComponent>>> NewComponentsWeakSet;
					NewComponentsWeakSet.Reserve(NewComponents.Num());
					Algo::Transform(NewComponents, NewComponentsWeakSet, [](ULandscapeComponent* Component) { return TWeakObjectPtr<ULandscapeComponent>(Component); });

					// Remove the material from any old components that are no longer in the region
					TSet<TWeakObjectPtr<ULandscapeComponent>, TWeakObjectPtrSetKeyFuncs<TWeakObjectPtr<ULandscapeComponent>>> RemovedComponents = BrushMaterialComponents.Difference(NewComponentsWeakSet);
					for (TWeakObjectPtr<ULandscapeComponent> RemovedComponent : RemovedComponents)
					{
						if (RemovedComponent.IsValid())
						{
					...
					BrushMaterialComponents = MoveTemp(NewComponentsWeakSet);
				}
			}
		}
	}
...
public:
	...
	virtual FLandscapeBrushData ApplyBrush(const TArray<FLandscapeToolInteractorPosition>& InteractorPositions) override
	{
		...
		FIntRect ExclusiveBounds;
		ExclusiveBounds.Min.X = FMath::FloorToInt(LastMousePosition.X - TotalRadius);
		ExclusiveBounds.Min.Y = FMath::FloorToInt(LastMousePosition.Y - TotalRadius);
		ExclusiveBounds.Max.X = FMath::CeilToInt(LastMousePosition.X + TotalRadius) + 1;
		ExclusiveBounds.Max.Y = FMath::CeilToInt(LastMousePosition.Y + TotalRadius) + 1;
...
public:
	...
	virtual FLandscapeBrushData ApplyBrush(const TArray<FLandscapeToolInteractorPosition>& InteractorPositions) override
	{
		...
		else
		{
			...
			FIntRect ExclusiveBounds;
			ExclusiveBounds.Min.X = FMath::FloorToInt(LastMousePosition.X - Radius);
			ExclusiveBounds.Min.Y = FMath::FloorToInt(LastMousePosition.Y - Radius);
			ExclusiveBounds.Max.X = FMath::CeilToInt(LastMousePosition.X + Radius) + 1;
			ExclusiveBounds.Max.Y = FMath::CeilToInt(LastMousePosition.Y + Radius) + 1;
			...
			FLandscapeBrushData BrushData(FIntRect(ExclusiveBounds.Min, ExclusiveBounds.Max));

...
void SLandscapeAssetThumbnail::Construct(const FArguments& InArgs, UObject* Asset, TSharedRef<FAssetThumbnailPool> ThumbnailPool, const FName& ClassThumbnailBrushOverride)
{
	...
	TSharedRef<SWidget> AssetThumbnailWidget = AssetThumbnail->MakeThumbnailWidget(AssetThumbnailConfig);

	// Disable the tooltip instead of showing a "None" tooltip for invalid assets
	if (Asset == nullptr)
	{
		AssetThumbnailWidget->SetToolTip(TSharedPtr<IToolTip>());
	}
...
FText SLandscapeEditor::GetErrorText() const
{
	...
	switch (EditState)
	{
		...
		case ELandscapeEditingState::BadFeatureLevel:
		{
			if (LandscapeEdMode->NewLandscapePreviewMode != ENewLandscapePreviewMode::None)
			{
				return LOCTEXT("IsFLError_create", "Can't create landscape with a feature level less than SM5!");
			}
			else
			{
				return LOCTEXT("IsFLError_edit", "Can't edit landscape with a feature level less than SM5!");
			}
		}

				...
				[
					...
					.OnSelectionChanged_Lambda([this](TSharedPtr<FString> InItem, ESelectInfo::Type)
					{
						if (InItem.IsValid())
						{
							...
							TArray<FString> SeparatedValues;
							CurrentGuessedDimension.ParseIntoArray(SeparatedValues, TEXT(" "));

							check(SeparatedValues.Num() == 3); // it should contain Number1 x Number2

							int32 FirstNumber = FCString::Atoi(*SeparatedValues[0]);
							int32 SecondNumber = FCString::Atoi(*SeparatedValues[2]);
...
FReply FLandscapeEditorDetailCustomization_CopyPaste::OnFitGizmoToSelectionButtonClicked()
{
	...
	if (LandscapeEdMode != nullptr)
	{
		ALandscapeGizmoActiveActor* Gizmo = LandscapeEdMode->UISettings->CurrentGizmoActor.Get();
		if (Gizmo && Gizmo->TargetLandscapeInfo.IsValid())
		{
...
FReply FLandscapeEditorDetailCustomization_CopyPaste::OnGizmoImportButtonClicked()
{
	...
	if (LandscapeEdMode != nullptr)
	{
		...
		if (Gizmo)
		{
			...
			Gizmo->Import(LandscapeEdMode->UISettings->GizmoImportSize.X, LandscapeEdMode->UISettings->GizmoImportSize.Y, (uint16*)Data.GetData(), LayerInfos, LayerDataPtrs.Num() ? LayerDataPtrs.GetData() : nullptr);
...
FReply FLandscapeEditorDetailCustomization_CopyPaste::OnGizmoExportButtonClicked()
{
	...
	{
		ALandscapeGizmoActiveActor* Gizmo = LandscapeEdMode->UISettings->CurrentGizmoActor.Get();
		if (Gizmo && Gizmo->TargetLandscapeInfo.IsValid() && Gizmo->SelectedData.Num())
		{
			int32 TargetIndex = -1;
			...
			for (int32 i = -1; i < Gizmo->LayerInfos.Num(); i++)
			{
				...
				else
				{
					...
					FName LayerName = Gizmo->LayerInfos[i]->GetLayerName();

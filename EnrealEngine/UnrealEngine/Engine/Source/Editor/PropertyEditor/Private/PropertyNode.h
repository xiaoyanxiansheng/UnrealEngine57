...
public:
	...
	*/
	 ...
	 */
	static void DuplicateArrayEntry(UObject* ModifiedObject, FProperty* NodeProperty, FScriptArrayHelper& ArrayHelper, int32 Index);
	...
	/**
	 * Broadcasts before an item in a container is being mutated, giving it a chance to back out.
	 * If an error occurs, it's sent by ChangeBlockedEvent.
	 */
	FCanPropertyItemChange& OnCanItemChangeEvent() { return CanItemChangeEvent; }

	/** Broadcasts before an item in a container is being mutated, giving it a chance to back out */
	FPropertyChangeBlocked& OnChangeBlockedEvent() { return ChangeBlockedEvent; }
...
protected:
	...
	/** Called before a container element is changed */
	FCanPropertyItemChange CanItemChangeEvent;

	/** Called when an attempted property change has been disallowed */
	FPropertyChangeBlocked ChangeBlockedEvent;

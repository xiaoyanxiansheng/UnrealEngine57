...
public:
	...
	/**
	 * Sets a delegate to call before an item in a container is being mutated
	 */
	void SetOnCanItemChange(const FCanPropertyItemChange& InDelegate);
	bool CanItemChange(EPropertyChangeType::Type Type, int32 Index);

	/**
	 * Sets a delegate to call when a property change has been disallowed
	 */
	FDelegateHandle SetOnChangeBlocked(const FPropertyChangeBlocked::FDelegate& InDelegate);

	/**
	 * Unregisters a delegate that's bound to OnChangeBlocked
	 */
	void UnregisterOnChangeBlocked(FDelegateHandle Handle);
...
public:
	...
	virtual void SetOnCanItemChange(const FCanPropertyItemChange& InDelegate) override;
	virtual FDelegateHandle SetOnChangeBlocked(const FPropertyChangeBlocked::FDelegate& InDelegate) override;
	virtual void UnregisterOnChangeBlocked(FDelegateHandle Handle) override;

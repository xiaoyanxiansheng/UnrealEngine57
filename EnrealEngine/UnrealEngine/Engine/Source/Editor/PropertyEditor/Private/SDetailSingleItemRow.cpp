	...
	if (bIsValidTreeNode)
	{
		if (Customization->IsValidCustomization())
		{
			...
			// Setup copy / paste actions
			{
				...
				if (WidgetRow.IsCopyToTextBound())
				{
					OnCopyToTextDelegate = WidgetRow.OnCopyToTextDelegate.Pin();					
				}
				else if (Category.IsValid())
				{
					OnCopyToTextDelegate = Category->OnCopyToText();
				}
				// If still not set, but this is a group, initialize
				else if (Group.IsValid())
				{
					if (TSharedPtr<FOnCopyToText> GroupOnCopyToTextDelegate = Group->OnCopyToText())
					{
						OnCopyToTextDelegate = GroupOnCopyToTextDelegate;
					}
				}

				bool bIncludeEditCondition = true;
				...
				else if (Category.IsValid())
				{
					...
					// When pasting an entire category, an edit condition property might come after the property that uses it.
					// Consider the case where the value on the clipboard enables the edit condition but that it may have been disabled prior to the paste.
					// Therefore, the edit condition should not be included when checking if a pasted property IsEditConst.
					bIncludeEditCondition = false;
				}
				// If still not set, but this is a group, initialize
				else if (Group.IsValid())
				{
				...
				// Note: We currently only animate the row on paste but if there is other custom behavior in OnPasteFromText we should
				// also bind to OnCopyToText to match it
				if (OnPasteFromTextDelegate.IsValid())
				{
					OnPasteFromTextDelegate->AddSP(this, &SDetailSingleItemRow::OnPasteFromText, bIncludeEditCondition);
				}
			}
...
bool SDetailSingleItemRow::CanPasteFromText(const FString& InTag, const FString& InText, const bool bIncludeEditCondition) const
{
	if (InText.IsEmpty())
	{
		return false;
	...
	// Allow paste if no property row can be found.
	{
		...
		if (PropertyRow.IsValid())
		{
			...
			{
				return !PropertyEditor->IsEditConst(bIncludeEditCondition);
			}
		}
	}
...
void SDetailSingleItemRow::OnPasteFromText(const FString& InTag, const FString& InText, const TOptional<FGuid>& InOperationId, const bool bIncludeEditCondition)
{
	if (PasteFromText(InTag, InText, bIncludeEditCondition))
	{
		TSharedPtr<FPropertyNode> PropertyNode = GetPropertyNode();
		if (PropertyNode.IsValid())
		{
...
bool SDetailSingleItemRow::PasteFromText(const FString& InTag, const FString& InText, const bool bIncludeEditCondition)
{
	if (!CanPasteFromText(InTag, InText, bIncludeEditCondition))
	{
		return false;
	}

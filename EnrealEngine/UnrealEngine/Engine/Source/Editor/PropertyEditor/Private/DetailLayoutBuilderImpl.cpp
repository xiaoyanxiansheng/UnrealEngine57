...
{
	for(FDetailTreeNode* Node : TickableNodes)
	{
		if (!TickableNodesToRemove.Contains(Node))
		{
...
{
	TGuardValue Guard(bIsIteratingTickableNodes, true);

	auto TickNode = [this, DeltaTime](FDetailTreeNode* Node)
	{
		if (!TickableNodesToRemove.Contains(Node))
		{
	...
	while (!TickableNodesToAdd.IsEmpty())
	{
		// Tick the new nodes, but keep them in a local set so we don't modify TickableNodesToAdd while ticking them
		TSet<FDetailTreeNode*> NodesToAdd = MoveTemp(TickableNodesToAdd);
		TickableNodesToAdd.Reset();

		for( FDetailTreeNode* Node : NodesToAdd)
		{
			TickNode(Node);
		}

		TickableNodes.Append(MoveTemp(NodesToAdd));
	}

	for( FDetailTreeNode* Node : TickableNodesToRemove )
	{
		TickableNodes.Remove(Node);
	}
	TickableNodesToRemove.Reset();
}

void FDetailLayoutBuilderImpl::AddTickableNode( FDetailTreeNode& TickableNode )
{
	if (bIsIteratingTickableNodes)
	{
		TickableNodesToAdd.Add(&TickableNode);
	}
	else
	{
...
void FDetailLayoutBuilderImpl::RemoveTickableNode( FDetailTreeNode& TickableNode )
{
	if (bIsIteratingTickableNodes)
	{
		TickableNodesToRemove.Add(&TickableNode);
	}
	else
	{

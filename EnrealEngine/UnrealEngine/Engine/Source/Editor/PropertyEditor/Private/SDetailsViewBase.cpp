	...
	, bRunningDeferredActions(false)
{
	...
	OnCultureChangedDelegate = FInternationalization::Get().OnCultureChanged().AddRaw(this, &SDetailsViewBase::RequestForceRefresh);
}
...
SDetailsViewBase::~SDetailsViewBase()
{
	...
	{
		FInternationalization::Get().OnCultureChanged().Remove(OnCultureChangedDelegate);
	}
}
...
void SDetailsViewBase::UpdatePropertyMapsWithItemExpansions()
{
	...
	for (TSharedPtr<FComplexPropertyNode>& RootPropertyNode : GetRootNodes())
	{
		...
		UStruct* BaseStruct = GetExpansionStruct(RootPropertyNode.ToSharedRef());
	...
	for (const FDetailLayoutData& LayoutData : DetailLayouts)
	{
		...
		for (TSharedPtr<FComplexPropertyNode>& ExternalRootPropertyNode : ExternalRootPropertyNodes)
		{
			...
			UStruct* BaseStruct = GetExpansionStruct(ExternalRootPropertyNode.ToSharedRef());
			FStringPrefixTree& PrefixTree = ExpansionStates.Add(BaseStruct);
			GetExpansionState(BaseStruct, PrefixTree);
		}
	}
...
UStruct* SDetailsViewBase::GetExpansionStruct(const TSharedRef<FPropertyNode>& StartNode)
{
	FComplexPropertyNode* ComplexParent = StartNode->FindComplexParent();
	UStruct* BaseStructure = ComplexParent->GetBaseStructure();

	FString FuncName;
	if (BaseStructure && BaseStructure->GetStringMetaDataHierarchical("GetExpansionStructFunc", &FuncName))
	{
		if (const FObjectPropertyNode* ObjectNode = ComplexParent->AsObjectNode())
		{
			const UClass* BaseClass = Cast<UClass>(BaseStructure);
			const int32 InstancesNum = ObjectNode->GetInstancesNum();

			if (InstancesNum > 0 && BaseClass)
			{
				if (UFunction* Function = BaseClass->FindFunctionByName(FName(FuncName)))
				{
					if (Function->HasAnyFunctionFlags(FUNC_Static))
					{
						...
						Params.Objects.Reserve(InstancesNum);
						for (int32 i = 0; i < InstancesNum; ++i)
						{
							if (UObject* Object = ObjectNode->GetInstanceAsUObject(i).Get())
							{
								Params.Objects.Add(Object);
							}
						}

						BaseStructure->ProcessEvent(Function, &Params);

						if (Params.Struct)
						{
							return const_cast<UStruct*>(Params.Struct);
						}
					}
				}
			}
...
void SDetailsViewBase::SaveExpandedItems(TSharedRef<FPropertyNode> StartNode)
{
	...
	UStruct* BestBaseStruct = GetExpansionStruct(StartNode);

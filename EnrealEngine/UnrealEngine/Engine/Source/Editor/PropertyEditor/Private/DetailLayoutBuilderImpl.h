...
private:
	...
	/** Nodes to add to being ticked, only used while iterating */
	TSet<FDetailTreeNode*> TickableNodesToAdd;
	/** Nodes to remove from being ticked, only used while iterating */
	TSet<FDetailTreeNode*> TickableNodesToRemove;
	...
	/** True if the layout is for an external root property node and not in the main set of objects the details panel is observing */
	bool bIsIteratingTickableNodes = false;

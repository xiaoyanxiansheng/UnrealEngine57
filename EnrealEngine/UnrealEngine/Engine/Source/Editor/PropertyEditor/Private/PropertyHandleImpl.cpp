            	...
            	if (CurObject)
            	{
            		...
            		if (Archetype)
            		{
            			...
            			for (int32 i = 0; i < PathNames.Num(); i++)
            			{
							...
							if (CurrObjValue && !CurrObjValue.IsDynamicContainerElement()) // initilization state tracking for container elements is not supported
							{
								if (ArchetypeValue && !ArchetypeValue.IsDynamicContainerElement()) // initilization state tracking for container elements is not supported
								{
...
void FPropertyValueImpl::SetOnCanItemChange(const FCanPropertyItemChange& InDelegate)
{
	if (PropertyNode.IsValid())
	{
		PropertyNode.Pin()->OnCanItemChangeEvent() = InDelegate;
	}
}

bool FPropertyValueImpl::CanItemChange(EPropertyChangeType::Type Type, int32 Index)
{
	if (const TSharedPtr<FPropertyNode> Node = PropertyNode.Pin())
	{
		const FCanPropertyItemChange& Event = Node->OnCanItemChangeEvent();
		if (Event.IsBound())
		{
			FCanPropertyItemChangeRetType Result = Event.Execute(Type, Index);
			if (Result.HasError())
			{
				const FPropertyChangeBlocked& ErrorEvent = Node->OnChangeBlockedEvent();
				ErrorEvent.Broadcast(Node->GetProperty(), Result.GetError());
				return false;
			}
...
FDelegateHandle FPropertyValueImpl::SetOnChangeBlocked(const FPropertyChangeBlocked::FDelegate& InDelegate)
{
	if (PropertyNode.IsValid())
	{
		return PropertyNode.Pin()->OnChangeBlockedEvent().Add(InDelegate);
	}

	return { };
}

void FPropertyValueImpl::UnregisterOnChangeBlocked(FDelegateHandle Handle)
{
	if (PropertyNode.IsValid())
	{
		PropertyNode.Pin()->OnChangeBlockedEvent().Remove(Handle);
	}
}
...
void FPropertyValueImpl::DuplicateChild( TSharedPtr<FPropertyNode> ChildNodeToDuplicate )
{
	...
	if( Addr )
	{
		...
		FScriptArrayHelper ArrayHelper(ArrayProperty, Addr);
		FPropertyNode::DuplicateArrayEntry(Obj, NodeProperty, ArrayHelper, Index);
...
FPropertyHandleItemAddResult FPropertyHandleArray::AddItem()
{
	...
	if( IsEditable() )
	{
		if (Implementation->CanItemChange(EPropertyChangeType::ArrayAdd, Index))
		{
...
FPropertyAccess::Result FPropertyHandleArray::EmptyArray()
{
	...
	if( IsEditable() )
	{
		if (Implementation->CanItemChange(EPropertyChangeType::ArrayClear, INDEX_NONE))
		{
...
FPropertyAccess::Result FPropertyHandleArray::DuplicateItem( int32 Index )
{
	...
	if( IsEditable() && Index < Implementation->GetNumChildren() )
	{
		if (Implementation->CanItemChange(EPropertyChangeType::Duplicate, Index))
		{
...
FPropertyAccess::Result FPropertyHandleArray::DeleteItem( int32 Index )
{
	...
	if( IsEditable() && Index < Implementation->GetNumChildren() )
	{
		if (Implementation->CanItemChange(EPropertyChangeType::ArrayRemove, Index))
		{
...
FPropertyAccess::Result FPropertyHandleArray::SwapItems(int32 FirstIndex, int32 SecondIndex)
{
	...
	{
		if (Implementation->CanItemChange(EPropertyChangeType::ArrayMove, FirstIndex) &&
			Implementation->CanItemChange(EPropertyChangeType::ArrayMove, SecondIndex))
		{
...
void FPropertyHandleArray::SetOnCanItemChange(const FCanPropertyItemChange& InDelegate)
{
	Implementation->SetOnCanItemChange(InDelegate);
}

FDelegateHandle FPropertyHandleArray::SetOnChangeBlocked(const FPropertyChangeBlocked::FDelegate& InDelegate)
{
	return Implementation->SetOnChangeBlocked(InDelegate);
}

void FPropertyHandleArray::UnregisterOnChangeBlocked(FDelegateHandle Handle)
{
	return Implementation->UnregisterOnChangeBlocked(Handle);
}
		...
		for (int32 ObjectIndex = 0; ObjectIndex < ObjectsToModify.Num(); ++ObjectIndex)
		{
			...
			if (ObjectToModify.BaseAddress)
			{
				...
				if (TextProperty && NewTextValue.IsInitializedFromString())
				{
					...
					UObject* Object = ObjectToModify.Object;

					auto DeterministicTextKeyGenerator = [PropertyPath, OuterPackage, Object, TextProperty]()
					{
						...
						// Build a unique and deterministic key from a combination of the outer package, object and text property info.
						// We can use both the Object or Outerpackage because in some cases (like for structs), we might not have an object but, if we do, we need to use that to create a unique key.
						FGuid KeyGuid(GetObjectKeyHash(Object != nullptr ? Object : OuterPackage), TextKeyUtil::HashString(PropertyPath), GetNameKeyHash(TextProperty->GetOwnerStruct()->GetFName()), GetNameKeyHash(TextProperty->GetFName()));

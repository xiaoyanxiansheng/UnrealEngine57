...
TSharedRef<FPropertySection> FClassSectionMapping::FindOrAddSection(FName SectionName, FText DisplayName, const int32 Order)
{
	TSharedPtr<FPropertySection> Section = FindSection(SectionName);
	if (!Section.IsValid())
	{
		Section = MakeShared<FPropertySection>(SectionName, DisplayName, Order);
...
TSharedRef<FPropertySection> FPropertyEditorModule::FindOrCreateSection(FName ClassName, FName SectionName, FText DisplayName, const int32 Order)
{
	...
	return ClassMapping->FindOrAddSection(SectionName, DisplayName, Order);
}
...
TSharedRef<class IStructureDetailsView> FPropertyEditorModule::CreateStructureDetailView(const FDetailsViewArgs& DetailsViewArgs, const FStructureDetailsViewArgs& StructureDetailsViewArgs, const FText& CustomName)
{
	...
	struct FStructureDetailsViewFilter
	{
		...
		static bool PassesFilter( const FPropertyAndParent& PropertyAndParent, const FStructureDetailsViewArgs InStructureDetailsViewArgs )
		{
			...
			if( ObjectProperty )
			{
				if (InStructureDetailsViewArgs.bShowClasses && ObjectProperty->PropertyClass && ObjectProperty->PropertyClass->IsChildOf(UScriptStruct::StaticClass()))
				{
					return true;
				}

...
namespace PropertyEditorHelpers
{
	...
	TSharedRef<SWidget> MakePropertyButton( const EPropertyButton::Type ButtonType, const TSharedRef< FPropertyEditor >& PropertyEditor )
	{
		...
		TAttribute<bool> IsAddEnabledAttribute = TAttribute<bool>::Create(
			...
			if (WeakPropertyNode.IsValid())
			{
				...
				{
					void* ValueAddress;
					FPropertyAccess::Result GetValResult = PropertyEditor->GetPropertyHandle()->GetValueData(ValueAddress);
	...
	*/
	...
	const UClass* FindOrLoadClass(const FString& ClassPath)
	{
		if (const UObject* Object = StaticLoadObject(nullptr, nullptr, ClassPath))
		{
			...
			{
				UE_LOG(LogPropertyNode, Error, TEXT("Found object '%s' instead of a class."), *Object->GetFullName());
			}
		}
		else
		{
			UE_LOG(LogPropertyNode, Error, TEXT("Unable to load class '%s'."), *ClassPath);
		}

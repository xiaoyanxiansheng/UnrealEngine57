...
void FItemPropertyNode::InitBeforeNodeFlags()
{
	...
	if (ObjectProperty)
	{
		...
		{
			const UClass* OwnerClass = ObjectProperty->GetOwnerClass();
			if (OwnerClass && OwnerClass->ShouldUseDynamicSubobjectInstancing())
			{
...
void FItemPropertyNode::InitChildNodes()
{
	...
	{
		FReadAddressList Addresses;
		if (GetReadAddress(!!HasNodeFlags(EPropertyNodeFlags::SingleSelectOnly), Addresses, false))
		{
			bool bAllOptionalsSet = true;
			for (int i = 0; i < Addresses.Num(); i++)
			{
				bAllOptionalsSet &= OptionalProperty->IsSet(Addresses.GetAddress(i));
				if (!bAllOptionalsSet)
				{
					break;
				}

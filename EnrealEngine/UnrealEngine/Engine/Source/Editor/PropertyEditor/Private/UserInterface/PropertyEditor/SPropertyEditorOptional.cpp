...
void SPropertyEditorOptional::Construct(const FArguments& InArgs, const TSharedRef<FPropertyEditor>& InPropertyEditor, TSharedRef<IPropertyUtilities> InPropertyUtilities)
{
	...
	TSharedRef<FPropertyNode> PropertyNode = InPropertyEditor->GetPropertyNode();
	...
	if (ValueProperty->IsA(FObjectPropertyBase::StaticClass()) || ValueProperty->IsA(FInterfaceProperty::StaticClass()))
	{
		ValueEditorWidget = SNew(SBox)
			.HAlign(HAlign_Left)
			.VAlign(VAlign_Center)
			.Padding(2.0f, 0.0f)
			[
				PropertyEditorHelpers::MakePropertyButton(EPropertyButton::OptionalPick, InPropertyEditor)
			];
	}
	else if (PropertyNode->GetSingleReadAddress(ValueAddress) == FPropertyAccess::MultipleValues)
	{	
		// Note: multi-select for `OptionalPick` types is handled in `MakePickOptionalButton`
		MultiselectOptions.Add(MakeShareable( new FText(LOCTEXT("OptionalMultiselect_Multiple", "Multiple States"))));
		...
		SAssignNew(ValueEditorWidget, SComboBox<TSharedPtr<FText>>)
...
void SPropertyEditorOptional::OnOptionChanged(TSharedPtr<FText> NewOption, ESelectInfo::Type)
{
	...
	if (NewOption->EqualTo(*MultiselectOptions[1]) /*"Set all to Value"*/)
	{
		...
		OptionalHandle->SetOptionalValue(nullptr, nullptr); // Set values to default-initialized
	}

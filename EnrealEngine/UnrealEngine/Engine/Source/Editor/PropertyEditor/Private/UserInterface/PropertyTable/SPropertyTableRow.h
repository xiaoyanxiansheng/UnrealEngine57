...
public:
	...
	FReply OnMouseButtonDown( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent ) override
	{
		...
		Row->GetTable()->SetCurrentCell( nullptr );
	...
	FReply OnMouseButtonUp( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent ) override
	{
		...
		if ( SelectionUnit == EPropertyTableSelectionUnit::Cell )
		{
			if ( LastClickedCell.IsValid() )
			{
				...
				{
					Table->SetLastClickedCell( nullptr );
					Table->SetCurrentCell( nullptr );
				}
				else
				{
					if ( CurrentCell.IsValid() && LastClickedCell == CurrentCell && !MouseEvent.IsControlDown())
					{
						// Don't let the table enter edit mode if the ctrl key is down
					...
					{
						if (SelectedCells.Contains(LastClickedCell.ToSharedRef()))
						{
							SelectedCells.Remove(LastClickedCell.ToSharedRef());
							Table->SetCurrentCell(nullptr);
						}
						else
						{

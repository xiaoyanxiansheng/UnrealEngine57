...
void FPropertyNode::InitNode(const FPropertyNodeInitParams& InitParams)
{
	...
	else
	{
		...
		UEnum* Enum = nullptr;
		if (const FByteProperty* ByteProperty = CastField<FByteProperty>(MyProperty))
		{
		...
		else if (const FEnumProperty* EnumProperty = CastField<FEnumProperty>(MyProperty))
		{
			Enum = EnumProperty->GetEnum();
		}

		if (Enum && FEnumValuePermissionList::Get().HasFiltering(Enum))
		{
			TSharedRef<FPropertyRestriction> Restriction = MakeShared<FPropertyRestriction>(FText::FromString(TEXT("Not available")));

			for (int32 Index = 0; Index < Enum->NumEnums(); ++Index)
			{
				const FString NameString = Enum->GetNameStringByIndex(Index);

				if (!FEnumValuePermissionList::Get().DoesEnumValuePassFilter(Enum, FName(NameString)))
				{
					Restriction->AddHiddenValue(NameString);
					Restriction->AddDisabledValue(NameString);
				}
			}

			AddRestriction(MoveTemp(Restriction));
		}
	}
	...
	if (ObjectNode != nullptr)
	{
		for (int32 ObjIndex = 0; ObjIndex < ObjectNode->GetNumObjects(); ++ObjIndex)
		{
			...
			if (Object != nullptr && Object->IsTemplate())
			{
				...
				{
					uint8* ArchetypeBaseOffset = nullptr;
					if (!bIsSparse)
					{
					...
					{
						if(uint8* ConditionedPropertyAddress = GetValueAddressFromObject(ArchetypeInstances[InstanceIndex]))
						{
							const int32 ConditionedPropertyOffset = GetProperty()->GetOffset_ForInternal();
							uint8* OwningStructStartAddress = ConditionedPropertyAddress - ConditionedPropertyOffset;
					...
					if(ArchetypeBaseOffset != nullptr)
					{
...
void FPropertyNode::DuplicateArrayEntry(UObject* ModifiedObject, FProperty* NodeProperty, FScriptArrayHelper& ArrayHelper, int32 Index)
{
	...
	bool bHasDynamicInstancedReferences = false;
	if (ModifiedObject && ModifiedObject->GetClass()->ShouldUseDynamicSubobjectInstancing())
	{
		bHasDynamicInstancedReferences = true;
	}

	auto IsDynamicInstancedReference = [bHasDynamicInstancedReferences, ModifiedObject](const UObject* ReferencedObject)
	{
		return bHasDynamicInstancedReferences && ReferencedObject->IsInOuter(ModifiedObject);
	};

	// If this is a property with instanced references within it the new entry will reference the old subobjects
	// Go through and duplicate the subobjects so that each new entry has unique instances
	FPropertyVisitorPath Path;
	Path.Push(FPropertyVisitorInfo(NodeProperty));
	FPropertyVisitorData Data(SrcAddress, nullptr);
	FPropertyVisitorContext VisitorContext(Path, Data, FPropertyVisitorContext::EScope::ObjectRefs);
	NodeProperty->Visit(VisitorContext, [&IsDynamicInstancedReference](const FPropertyVisitorContext& Context) -> EPropertyVisitorControlFlow
	{
		if (const FObjectPropertyBase* ObjectProperty = CastField<FObjectPropertyBase>(Context.Path.Top().Property))
		{
			UObject* ReferencedObject = ObjectProperty->GetObjectPropertyValue(Context.Data.PropertyData);
			if (ReferencedObject && (ObjectProperty->HasAnyPropertyFlags(CPF_InstancedReference) || IsDynamicInstancedReference(ReferencedObject)))
			{
				// Make a deep copy
				UObject* DuplicatedObject = DuplicateObject(ReferencedObject, ReferencedObject->GetOuter());
				ObjectProperty->SetObjectPropertyValue(Context.Data.PropertyData, DuplicatedObject);
			}
			
			// Continue on to the next property/item.
			return EPropertyVisitorControlFlow::StepOver;
		}

		// In the case of container/optional properties, this will recurse into any inner value(s).
	...
	for (UObject* InstanceToChange : AffectedInstances)
	{
		...
		if (ArrayProperty)
		{
			...
			case EPropertyArrayChangeType::Duplicate:
				DuplicateArrayEntry(InstanceToChange, NodeProperty, ArrayHelper, ArrayIndex);
				break;

...
void SDetailsView::Construct(const FArguments& InArgs, const FDetailsViewArgs& InDetailsViewArgs)
{
	...
	if (DetailsViewArgs.InitialSections.Num() > 0)
	{
		CurrentFilter.VisibleSections = DetailsViewArgs.InitialSections;
	}
	...
	FMenuBuilder DetailViewOptions(true, nullptr, InDetailsViewArgs.OptionsExtender);
	const bool bDisplayShowSectionHeadingText = InDetailsViewArgs.OptionsExtender.IsValid() && (InDetailsViewArgs.OptionsExtender->NumExtensions() > 0);
	DetailViewOptions.BeginSection("DetailsViewShowOptions", (bDisplayShowSectionHeadingText ? LOCTEXT("ShowSectionHeading", "Show") : FText::GetEmpty()));
	...
	DetailViewOptions.BeginSection("DetailsViewCategories");
...
void SDetailsView::RebuildSectionSelector()
{
	...
	const TMap<FName, SDetailsView::FSection> AllSections = GetAllSections();
	...
	struct FOrderedSection
	{
		FName Name;
		int32 Order = 0;
	};

	TArray<FOrderedSection> SortedKeys;
	SortedKeys.Reserve(AllSections.Num());

	for (const TPair<FName, SDetailsView::FSection>& SectionPair : AllSections)
	{
		SortedKeys.Emplace(FOrderedSection{ SectionPair.Key, SectionPair.Value.Order });
	}

	SortedKeys.Sort([](const FOrderedSection& A, const FOrderedSection& B)
		{
			static const FName General("General");
			static const FName All("All");

			// General first, All last, rest ordered then alphabetical
			if (A.Name.IsEqual(General) || B.Name.IsEqual(All))
			{
				return true;
			}
			if (A.Name.IsEqual(All) || B.Name.IsEqual(General))
			{
				return false;
			}
			if (A.Order != B.Order)
	...
	for (const FOrderedSection& Key : SortedKeys)
	{
		SectionSelectorBox->AddSlot()
		[
			CreateSection(Key.Name, AllSections[Key.Name].DisplayName)
		];
	}
	...
	for (const TSharedPtr<FComplexPropertyNode>& RootPropertyNode : RootPropertyNodes)
	{
		...
		if (const FDetailsViewConfig* ViewConfig = GetConstViewConfig())
		{
...
TMap<FName, SDetailsView::FSection> SDetailsView::GetAllSections() const
{
	static const FName PropertyEditor("PropertyEditor");
	FPropertyEditorModule& PropertyModule = FModuleManager::GetModuleChecked<FPropertyEditorModule>(PropertyEditor);

	TMap<FName, SDetailsView::FSection> AllSections;
	...
	for (const TSharedPtr<FComplexPropertyNode>& RootPropertyNode : RootPropertyNodes)
	{
		...
		for (const FName& Category : LayoutCategories)
		{
			...
			{
				AllSections.Add(Section->GetName(), SDetailsView::FSection{ Section->GetDisplayName(), Section->GetOrder() });
			}
		}
	}
...
bool SDetailsView::ResetToDefaultSection()
{
	if (RootPropertyNodes.IsEmpty())
	{
		return false;
	}

	bool bResetViewConfig = false;
	CurrentFilter.VisibleSections.Reset();
	for (const TSharedPtr<FComplexPropertyNode>& RootPropertyNode : RootPropertyNodes)
	{
		if (const UStruct* RootBaseStruct = RootPropertyNode->GetBaseStructure())
		{
			if (FDetailsViewConfig* MutableViewConfig = GetMutableViewConfig())
			{
				FDetailsSectionSelection& SectionSelection = MutableViewConfig->SelectedSections.FindOrAdd(RootBaseStruct->GetFName());
				SectionSelection.SectionNames = CurrentFilter.VisibleSections;
				SaveViewConfig();
				bResetViewConfig |= true;
			}
		}
	}

	return bResetViewConfig;
}

...
public:
	...
	void CacheValue()
	{
		...
		TSharedPtr<FPropertyNode> PropertyNodePtr = PropertyNode.Pin();
		if(FProperty* Property = PropertyNodePtr ? PropertyNodePtr->GetProperty() : nullptr)
		{
			...
			if(PropertyNodePtr->GetArrayIndex() == INDEX_NONE && Property->ArrayDim > 1 )
			{
				// Check static arrays
		...
		if (FProperty* Property = PropertyNodePtr ? PropertyNodePtr->GetProperty() : nullptr)
		{
			...
			if (PropertyValueAddresses.BaseAddress != nullptr && PropertyValueAddresses.Address != nullptr)
			{
				...
				if (OuterArrayProperty != NULL)
				{
					...
					if (PropertyNodePtr->GetArrayIndex() >= ArrayHelper.Num())
					{
				...
				else if (OuterSetProperty != NULL)
				{
					...
					bool bIsValidIndex = PropertyNodePtr->GetArrayIndex() >= 0 && PropertyNodePtr->GetArrayIndex() < SetHelper.Num();
					if (!bIsValidIndex)
					{
						bPropertyValid = false;
					}
				...
				else if (OuterMapProperty != NULL)
				{
					...
					bool bIsValidIndex = PropertyNodePtr->GetArrayIndex() >= 0 && PropertyNodePtr->GetArrayIndex() < MapHelper.Num();
					if (!bIsValidIndex)
					{
						bPropertyValid = false;
					}
...
private:
	...
	FPropertyValueAddresses GetPropertyValueAddresses()
	{
		...
		if (FProperty* Property = PropertyNodePtr ? PropertyNodePtr->GetProperty() : nullptr)
		{
			FPropertyNode* ParentNode = PropertyNodePtr->GetParentNode();
			...
			ValueAddresses.BaseAddress = (OuterArrayProp == NULL && OuterSetProp == NULL && OuterMapProp == NULL)
				? PropertyNodePtr->GetValueBaseAddressFromObject(OwnerObject)
				: (ParentNode ? ParentNode->GetValueBaseAddressFromObject(OwnerObject) : nullptr);

			ValueAddresses.Address = PropertyNodePtr->GetValueAddressFromObject(OwnerObject);

			return ValueAddresses;
		}

		return FPropertyValueAddresses();
	}

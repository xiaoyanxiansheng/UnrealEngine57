...
public:
	 ...
	 */
	UE_API virtual bool AreValuesEqual(const TWeakPtr<FDetailTreeNode>& TreeNodeA, const TWeakPtr<FDetailTreeNode>& TreeNodeB) const;
	UE_DEPRECATED(5.6, "Output parameters wasn't generic and is no longer used.")
	UE_API virtual bool AreValuesEqual(const TWeakPtr<FDetailTreeNode>& TreeNodeA, const TWeakPtr<FDetailTreeNode>& TreeNodeB, TArray<FPropertySoftPath>*) const final;
	 ...
	 * @param TreeNodeB node from the second user provided tree (guaranteed not to be null)
	 */
	UE_API virtual bool AreMatching(const TWeakPtr<FDetailTreeNode>& TreeNodeA, const TWeakPtr<FDetailTreeNode>& TreeNodeB) const;
	UE_DEPRECATED(5.6, "Output parameters wasn't generic and is no longer used.")
	UE_API virtual bool AreMatching(const TWeakPtr<FDetailTreeNode>& TreeNodeA, const TWeakPtr<FDetailTreeNode>& TreeNodeB, TArray<FPropertySoftPath>*) const final;
	 ...
	 */
	UE_API virtual void GetChildren(const TWeakPtr<FDetailTreeNode>& InParent, TArray<TWeakPtr<FDetailTreeNode>>& OutChildren) const;
	...
	*/
	UE_API virtual bool ShouldMatchByValue(const TWeakPtr<FDetailTreeNode>& TreeNode) const;
	...
	*/
	UE_API virtual bool ShouldInheritEqualFromChildren(const TWeakPtr<FDetailTreeNode>& TreeNodeA, const TWeakPtr<FDetailTreeNode>& TreeNodeB) const;
};
...
{
public:
	UE_API FAsyncDetailViewDiff(TSharedRef<IDetailsView> LeftView, TSharedRef<IDetailsView> RightView);
	...
	UE_API void GetPropertyDifferences(TArray<FSingleObjectDiffEntry>& OutDiffEntries) const;
	...
	UE_API TPair<int32, int32> ForEachRow(const TFunction<ETreeTraverseControl(const TUniquePtr<DiffNodeType>&, int32, int32)>& Method) const;

	// generates scroll data used by LinkableScrollbar
	UE_API TArray<FVector2f> GenerateScrollSyncRate() const;

private:
	static UE_API TAttribute<TArray<TWeakPtr<FDetailTreeNode>>> RootNodesAttribute(TWeakPtr<IDetailsView> DetailsView);

...
public:
	...
	*/
	 ...
	 */
	virtual void SetOnCanItemChange(const FCanPropertyItemChange& InDelegate) = 0;

	/**
	 * Sets a delegate to call when a property change has been disallowed
	 */
	virtual FDelegateHandle SetOnChangeBlocked(const FPropertyChangeBlocked::FDelegate& InDelegate) = 0;

	/**
	 * Unregisters a delegate that is bound to OnChangeBlocked
	 */
	virtual void UnregisterOnChangeBlocked(FDelegateHandle Handle) = 0;

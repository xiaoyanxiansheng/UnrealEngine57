...
public:
	...
	*/
	static UE_API TSharedRef<FMaterialItemView> Create(
	 ...
	 */
	UE_API virtual TSharedRef<SWidget> CreateNameContent();

	/**
	 * Create a Value Content for Material Item Widget
	 */
	UE_API virtual TSharedRef<SWidget> CreateValueContent(IDetailLayoutBuilder& InDetailBuilder, const TArray<FAssetData>& OwnerAssetDataArray, UActorComponent* InActorComponent);
	 ...
	 */
	UE_API void ReplaceMaterial( UMaterialInterface* NewMaterial, bool bReplaceAll = false );

	/**
	 * Called to get the visibility of the reset to base button
	 */
	UE_API bool GetResetToBaseVisibility() const;

	/**
	 * Called when reset to base is clicked
	 */
	UE_API void OnResetToBaseClicked();

private:
	UE_API FMaterialItemView(const FMaterialListItem& InMaterial, 
					FOnMaterialChanged& InOnMaterialChanged, 
	 ...
	 */
	UE_API void OnSetObject( const FAssetData& AssetData );

	/**
	 * @return Whether or not the textures menu is enabled
	 */
	UE_API bool IsTexturesMenuEnabled() const;

	/**
	 * Generate list of the textures for material selection list
	 */
	UE_API TSharedRef<SWidget> OnGetTexturesMenuForMaterial();

	/**
	 * Create button for browse to nanite override material
	 */
	UE_API TSharedRef<SWidget> MakeBrowseNaniteOverrideMaterialButton() const;

	/**
	 * @return The tool tip for the textures menuuuuu
	 */
	UE_API FText GetTexturesMenuToolTipText() const;

	/**
	 * On Get object path handler
	 */
	UE_API FString OnGetObjectPath() const;

	/**
	 * Finds the asset in the content browser
	 */
	UE_API void GoToAssetInContentBrowser( TWeakObjectPtr<UObject> Object );
	 ...
	 */
	UE_API struct FOnGenerateGlobalRowExtensionArgs GetGlobalRowExtensionArgs(IDetailLayoutBuilder& InDetailBuilder, UActorComponent* InCurrentComponent) const;
	 ...
	 */
	UE_API TSharedPtr<SWidget> GetGlobalRowExtensionWidget(IDetailLayoutBuilder& InDetailBuilder, UActorComponent* InCurrentComponent) const;

...
void FClothMeshPaintAdapter::SetSelectedClothingAsset(const FGuid& InAssetGuid, int32 InAssetLod, int32 InMaskIndex)
{
	...
	if(InAssetGuid.IsValid() && ReferencedSkeletalMesh)
	{
		...
		{
			if(const UClothingAssetCommon* const ConcreteAsset = Cast<const UClothingAssetCommon>(Asset))
			{
				if(ConcreteAsset->GetAssetGuid() == InAssetGuid)
				{
					if(ConcreteAsset->IsValidLod(InAssetLod))
					{
						const FClothLODDataCommon& LodData = ConcreteAsset->LodData[InAssetLod];
...
bool FClothMeshPaintAdapter::InitializeVertexData()
{
	...
	if (UClothingAssetCommon* const ConcreteAsset = Cast<UClothingAssetCommon>(SelectedAsset))
	{
		if(const UDebugSkelMeshComponent* const DebugComponent = Cast<UDebugSkelMeshComponent>(SkeletalMeshComponent))
		{
...
{
	if(const UClothingAssetCommon* const ConcreteAsset = Cast<UClothingAssetCommon>(SelectedAsset))
	{

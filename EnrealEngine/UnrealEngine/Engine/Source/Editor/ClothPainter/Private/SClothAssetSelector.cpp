...
public:
	...
	void OnCommitAssetName(const FText& InText, ETextCommit::Type CommitInfo)
	{
		if(Item.IsValid())
		{
			if(UClothingAssetBase* const Asset = Item->ClothingAsset.Get())
			{
				...
				if(Asset->GetName() != TrimText.ToString())
				{
					...
					if(StaticFindObject(UClothingAssetBase::StaticClass(), Asset->GetOuter(), *NewName.ToString()))
					{
						NewName = MakeUniqueObjectName(Asset->GetOuter(), UClothingAssetBase::StaticClass(), FName(*TrimText.ToString()));
					}
...
private:
	...
	void RebuildLODParameters()
	{
		...
		if(UClothingAssetCommon* const Asset = Cast<UClothingAssetCommon>(Item->ClothingAsset.Get()))
		{
	...
	void ExportAsset(UClass* ExportedType)
	{
		...
		if (const UClothingAssetBase* const ClothingAsset = Item->ClothingAsset.Get())
		{
	...
	bool CanRebuildLODParameters() const
	{
		...
		if(const UClothingAssetCommon* const Asset = Cast<UClothingAssetCommon>(Item->ClothingAsset.Get()))
		{
...
private:
	...
	void OnCopyMask(int32 InMaskIndex)
	{
		if (AssetSelectorPtr.IsValid() && Item.IsValid())
		{
			if (USkeletalMesh* CurrentMesh = Item->GetOwningMesh())
			{
				if (FPointWeightMap* Mask = Item->GetMask())
				{
					if (UClothingAssetCommon* const ClothingAsset = Cast<UClothingAssetCommon>(AssetSelectorPtr.Pin()->GetSelectedAsset().Get()))
					{
	...
	void BuildCopyMaskSubmenu(FMenuBuilder& Builder)
	{
		if (AssetSelectorPtr.IsValid() && Item.IsValid())
		{
			if (const UClothingAssetCommon* const ClothingAsset = Cast<const UClothingAssetCommon>(AssetSelectorPtr.Pin()->GetSelectedAsset().Get()))
			{
				const int32 LOD = AssetSelectorPtr.Pin()->GetSelectedLod();
				if (ClothingAsset->LodData.IsValidIndex(LOD))
				{
	...
	void BuildCopyVertexColorSubmenu(FMenuBuilder& Builder)
	{
		if (AssetSelectorPtr.IsValid())
		{
			...
			{
				const int32 LOD = AssetSelectorPtr.Pin()->GetSelectedLod();
				FPointWeightMap* const Mask = Item->GetMask();

				const TSharedRef<SWidget> Widget = SNew(SCopyVertexColorSettingsPanel, ClothingAsset, LOD, Mask);
...
void SClothAssetSelector::Construct(const FArguments& InArgs, USkeletalMesh* InMesh)
{
	...
	[
		...
		[
			...
			[
				...
				[
					...
					.ToolTipText(LOCTEXT("CopyClothingFromMeshText_TOOLTIP", "Create clothing data from another asset."))
				...
				[
					SNew(SComboButton)
					.Visibility_Lambda([this]() -> EVisibility { return CanSelectLod() ? EVisibility::Visible : EVisibility::Collapsed; })
		...
		[
			SNew(SExpandableArea)
			.Visibility_Lambda([this]() -> EVisibility { return CanAddNewMask() ? EVisibility::Visible : EVisibility::Collapsed; })
			.BorderImage(FAppStyle::Get().GetBrush("DetailsView.CategoryTop"))
			.Padding(FMargin(0.f, 2.0f))
			.HeaderContent()
			[
		...
		[
			SNew(SExpandableArea)
			.Visibility_Lambda([this]() -> EVisibility { return CanEditDeformer() ? EVisibility::Visible : EVisibility::Collapsed; })
			.BorderImage(FAppStyle::Get().GetBrush("DetailsView.CategoryTop"))
			.Padding(FMargin(0.f, 2.0f))
			.HeaderContent()
			[
				...
				[
					SNew(STextBlock)
					.Text(LOCTEXT("MeshProxyDeformer", "Mesh Proxy Deformer"))
					.TextStyle(FAppStyle::Get(), "DetailsView.CategoryTextStyle")
					.Font(FAppStyle::Get().GetFontStyle("PropertyWindow.BoldFont"))
				]
			]
			...
			[	
				...
				[
					...
					[
						...
						.ToolTipText(LOCTEXT("MultipleInfluencesToolTip", "When enabled, use multiple triangles from the simulation mesh to deform the render section, instead of a single one."))
					]
					...
					[
						SNew(STextBlock)
						.Font(FAppStyle::Get().GetFontStyle("PropertyWindow.NormalFont"))
						.Text(LOCTEXT("SearchRadius", "Search Radius"))
						.ToolTipText(LOCTEXT("SearchRadiusToolTip", "The search radius around the render section vertices to look for all the influencing simulation triangles."))
					]
					...
					[
						SNew(STextBlock)
						.Font(FAppStyle::Get().GetFontStyle("PropertyWindow.NormalFont"))
						.Text(LOCTEXT("SmoothTransition", "Smooth Transition"))
						.ToolTipText(LOCTEXT("SmoothTransitionToolTip", "Whether to enable smooth transitions between kinematic and dynamic simulation triangles deforming the render section."))
					]
				]
...
{
	const UClothingAssetCommon* const Asset = Cast<UClothingAssetCommon>(SelectedAsset.Get());
...
{
	UClothingAssetCommon* const Asset = Cast<UClothingAssetCommon>(SelectedAsset.Get());
	if (Asset && Asset->IsValidLod(SelectedLod))
	{
...
TWeakObjectPtr<UClothingAssetBase> SClothAssetSelector::GetSelectedAsset() const
{
...
void SClothAssetSelector::PostRedo(bool bSuccess)
{
	OnRefresh();
}
...
{
	if (const UObject* const SourceAsset = AssetData.GetAsset())
	{
		if (Mesh && Mesh != Cast<USkeletalMesh>(SourceAsset))
		{
			...
			for (const UClothingAssetFactoryBase* const ClothingAssetFactory : ClothingEditorModule.GetClothingAssetFactories())
			{
				if (SourceAsset->GetClass()->IsChildOf(ClothingAssetFactory->GetSupportedSourceAssetType()))
				{
					if (!Transaction.IsSet())
					{
						Transaction.Emplace(LOCTEXT("CreatingClothingAsset", "Creating clothing asset from another asset."));
						Mesh->Modify();
					}

					for (UClothingAssetBase* const NewAsset : ClothingAssetFactory->CreateFromSourceAsset(Mesh, SourceAsset))
					{
...
TSharedRef<SWidget> SClothAssetSelector::OnGenerateSkeletalMeshPickerForClothCopy()
{
	...
	AssetPickerConfig.OnShouldFilterAsset = FOnShouldFilterAsset::CreateLambda(
		[this, &ClothingEditorModule](const FAssetData& AssetData)
		{
			const FSoftClassPath SoftClassPath(AssetData.AssetClassPath.ToString());
			if (UClass* const ResolvedClass = SoftClassPath.ResolveClass())
			{
				for (const UClothingAssetFactoryBase* const ClothingAssetFactory : ClothingEditorModule.GetClothingAssetFactories())
				{
					if (ResolvedClass->IsChildOf(ClothingAssetFactory->GetSupportedSourceAssetType()))
					{
						return false;
					}
				}
	...
	AssetPickerConfig.SaveSettingsName = TEXT("ClothAssetSelectorAssetPicker");
	AssetPickerConfig.bShowPathInColumnView = true;
	AssetPickerConfig.bShowTypeInColumnView = true;
	AssetPickerConfig.OnExtendAddFilterMenu = FOnExtendAddFilterMenu::CreateLambda([](UToolMenu* ToolMenu)
		{
			ToolMenu->RemoveSection("FilterBarFilterBasicAsset");  // Remove common filters section
		});
	AssetPickerConfig.bAddFilterUI = true;
	AssetPickerConfig.Filter.bRecursiveClasses = true;
	for (const UClothingAssetFactoryBase* const ClothingAssetFactory : ClothingEditorModule.GetClothingAssetFactories())
	{
		if (const UClass* const SupportedSourceAssetType = ClothingAssetFactory->GetSupportedSourceAssetType())
		{
			AssetPickerConfig.Filter.ClassPaths.Emplace(SupportedSourceAssetType->GetClassPathName());
		}
	}
...
bool SClothAssetSelector::CanSelectLod() const
{
	return Cast<const UClothingAssetCommon>(SelectedAsset.Get()) != nullptr;
}

bool SClothAssetSelector::CanEditDeformer() const
{
	return Cast<const UClothingAssetCommon>(SelectedAsset.Get()) != nullptr;
}
...
TSharedRef<SWidget> SClothAssetSelector::OnGetLodMenu()
{
	...
	if(const UClothingAssetCommon* const CurrAsset = Cast<UClothingAssetCommon>(SelectedAsset.Get()))
	{
...
{
	if(InItem.IsValid() && InItem->ClothingAsset.Get())
	{
...
{
	if(UClothingAssetCommon* const Asset = Cast<UClothingAssetCommon>(SelectedAsset.Get()))
	{
...
{
	UClothingAssetBase* CurrSelectedAsset = nullptr;
...
void SClothAssetSelector::OnClothingLodSelected(int32 InNewLod)
{
	...
	if(const UClothingAssetCommon* const Asset = Cast<UClothingAssetCommon>(SelectedAsset.Get()))
	{
		SetSelectedLod(InNewLod);
...
void SClothAssetSelector::SetSelectedAsset(TWeakObjectPtr<UClothingAssetBase> InSelectedAsset)
{
	SelectedAsset = InSelectedAsset;

	RefreshMaskList();

	const UClothingAssetCommon* const NewAsset = Cast<const UClothingAssetCommon>(SelectedAsset.Get());
	if(NewAsset && NewAsset->GetNumLods() > 0)
	{
		SetSelectedLod(0);

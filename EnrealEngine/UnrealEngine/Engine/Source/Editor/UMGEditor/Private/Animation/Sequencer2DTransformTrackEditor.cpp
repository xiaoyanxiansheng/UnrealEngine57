...
#define LOCTEXT_NAMESPACE "2DTransformTrackEditor"
...
FText F2DTransformTrackEditor::GetDisplayName() const
{
	return LOCTEXT("2DTransformTrackEditor_DisplayName", "2D Transform");
}
...
void F2DTransformTrackEditor::ProcessKeyOperation(FFrameNumber InKeyTime, const UE::Sequencer::FKeyOperation& Operation, ISequencer& InSequencer, TArray<UE::Sequencer::FAddKeyResult> *OutResults)
{
...
void F2DTransformTrackEditor::ProcessKeyOperation(UObject* ObjectToKey, TArrayView<const UE::Sequencer::FKeySectionOperation> SectionsToKey, ISequencer& InSequencer, FFrameNumber KeyTime, TArray<UE::Sequencer::FAddKeyResult> *OutResults)
{
	using namespace UE::Sequencer;
	...
	for (const FKeySectionOperation& Operation : SectionsToKey)
	{
		...
		for (TSharedPtr<IKeyArea> KeyArea : Operation.KeyAreas)
		{
			...
			if (Channel)
			{
				...
				{
					FKeyHandle KeyHandle = AddKeyToChannel(static_cast<FMovieSceneFloatChannel*>(Channel), KeyTime, Value, InSequencer.GetKeyInterpolation());

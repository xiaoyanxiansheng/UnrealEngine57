...
TSharedRef<SWidget> FBlueprintWidgetCustomization::MakePropertyBindingWidget(TWeakPtr<FWidgetBlueprintEditor> InEditor, UFunction* SignatureFunction, TSharedRef<IPropertyHandle> InPropertyHandle, TWeakPtr<IDetailsView> InDetailsView, bool bInGeneratePureBindings, bool bAllowDetailsPanelLegacyBinding, EPropertyBindingMode Mode)
{
	...
	Args.OnCanUseCustomGenerateMenu = FOnHasAnyBindings::CreateLambda([InEditor, InPropertyHandle, Mode]()
		{
			UUMGEditorProjectSettings* UMGEditorProjectSettings = GetMutableDefault<UUMGEditorProjectSettings>();
			return UMGEditorProjectSettings->bUseNewBindingFromDetailsView &&
					((Mode == EPropertyBindingMode::Value) || !HasPropertyBindings(InEditor, InPropertyHandle));
		});
	...
	UUMGEditorProjectSettings* UMGEditorProjectSettings = GetMutableDefault<UUMGEditorProjectSettings>();
	Args.bAllowFunctionBindings = !UMGEditorProjectSettings->bUseNewBindingFromDetailsView;
	Args.bAllowPropertyBindings = !UMGEditorProjectSettings->bUseNewBindingFromDetailsView;

	if (Mode == EPropertyBindingMode::Extension)
	{
		Args.bLinkIconStyleCompactMode = UMGEditorProjectSettings->bUseCompactLinkButtonInExtension;
		Args.bCanDisplayLinkIcon = true;
	}
	else
	{
		Args.bLinkIconStyleCompactMode = false;
		Args.bCanDisplayLinkIcon = false;
	}

	Args.OnGenerateMenu = FOnGetContent::CreateLambda([InEditor, Objects, InPropertyHandle, ActiveExtensions, InDetailsView]() -> TSharedRef<SWidget>
	{
		UWidgetBlueprint* WidgetBlueprint = InEditor.Pin()->GetWidgetBlueprintObj();
		UWidget* Widget = Objects.Num() ? Cast<UWidget>(Objects[0]) : nullptr;
		...
		if (Widget && WidgetBlueprint)
		{
			for (const TSharedPtr<IPropertyBindingExtension>& Extension : ActiveExtensions)
			{
				TSharedRef<SWidget> GeneratedWidget = Extension->OnGenerateMenu(WidgetBlueprint, Widget, InPropertyHandle, InDetailsView);
				if (GeneratedWidget != SNullWidget::NullWidget)
				{
...
bool FBlueprintWidgetCustomization::HasPropertyBindings(TWeakPtr<FWidgetBlueprintEditor> InEditor, const TSharedRef<IPropertyHandle>& InPropertyHandle, bool bOnlyEditableValue)
{
	...
	for (const UObject* Object : Objects)
	{
		...
		for (const TSharedPtr<IPropertyBindingExtension>& BindingExtension : EditorModule.GetPropertyBindingExtensibilityManager()->GetExtensions())
		{
			if (BindingExtension->CanExtend(ThisBlueprint, Widget, InPropertyHandle))
			{
				...
				{
					if (bOnlyEditableValue)
					{
						return BindingExtension->ShouldAllowEditingValue(ThisBlueprint, Widget, InPropertyHandle);
					}
					else
					{
						return true;
...
void FBlueprintWidgetCustomization::CreateEventCustomization( IDetailLayoutBuilder& DetailLayout, FDelegateProperty* Property, UWidget* Widget )
{
	...
	PropertyRow.CustomWidget(bShowChildren)
		...
		.MaxDesiredWidth(250)
		[
			MakePropertyBindingWidget(Editor.Pin(), Property->SignatureFunction, DelegatePropertyHandle, DetailLayout.GetDetailsViewSharedPtr(), false, true)
		];
}
...
void FBlueprintWidgetCustomization::CustomizeAccessibilityProperty(IDetailLayoutBuilder& DetailLayout, const FName& BehaviorPropertyName, const FName& TextPropertyName)
{
	...
	TSharedRef<SWidget> BindingWidget = MakePropertyBindingWidget(Editor, AccessibleTextDelegateProperty->SignatureFunction, AccessibleTextPropertyHandle, DetailLayout.GetDetailsViewSharedPtr(), false, BlueprintObj->ArePropertyBindingsAllowed());

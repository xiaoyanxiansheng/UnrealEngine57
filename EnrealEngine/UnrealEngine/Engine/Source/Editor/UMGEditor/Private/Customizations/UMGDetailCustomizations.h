...
public:
	...
	enum class EPropertyBindingMode
	{
		Value,
		Extension
	};
	static TSharedRef<SWidget> MakePropertyBindingWidget(TWeakPtr<FWidgetBlueprintEditor> InEditor, UFunction* SignatureFunction, TSharedRef<IPropertyHandle> InDelegatePropertyHandle, TWeakPtr<IDetailsView> InDetailsView, bool bInGeneratePureBindings, bool bAllowDetailsPanelLegacyBinding, EPropertyBindingMode Mode = EPropertyBindingMode::Extension);

	/** 
	 * Whether the property (or its parent property, if this is an array/struct element) currently has bindings.
	 * When bOnlyCompilableBindingsFilter is set to true, only bindings which have there Compile flag enable will be considered.
	 */
	static bool HasPropertyBindings(TWeakPtr<FWidgetBlueprintEditor> InEditor, const TSharedRef<IPropertyHandle>& InPropertyHandle, bool bOnlyEditableValue = false);

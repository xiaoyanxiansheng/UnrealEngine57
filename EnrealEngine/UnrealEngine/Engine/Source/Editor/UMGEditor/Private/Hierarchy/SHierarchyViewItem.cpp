	...
	STableRow< TSharedPtr<FHierarchyModel> >::Construct(
		...
		[
			...
			[
				...
				.VAlign(VAlign_Center)
				.AddMetaData<FTagMetaData>(FTagMetaData(TEXT("ToggleLockedInDesigner")))
				[
					SNew(SBox)
...
void SHierarchyViewItem::ToggleLockedInDesigner(const bool bRecursive)
{
	if (Model.IsValid())
	{
...
FReply SHierarchyViewItem::OnToggleLockedInDesigner()
{
	if ( Model.IsValid() )
	{
		...
		ToggleLockedInDesigner(bRecursive);
	}

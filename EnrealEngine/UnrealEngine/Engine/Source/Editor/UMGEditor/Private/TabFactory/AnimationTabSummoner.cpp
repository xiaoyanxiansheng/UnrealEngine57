...
bool VerifyAnimationRename( FWidgetBlueprintEditor& BlueprintEditor, UWidgetAnimation* Animation, FString NewAnimationName, FText& OutErrorMessage )
{
	...
	if (Blueprint && FindObject<UWidgetAnimation>( Blueprint, *NewAnimationName, EFindObjectFlags::ExactClass ) )
	{
...
private:
	...
	void RefreshCurrentAnimationSelection()
	{		
		if(const UWidgetAnimation* CurrentAnim = BlueprintEditor.Pin()->RefreshCurrentAnimation())
		{
			const TSharedPtr<FWidgetAnimationListItem>* FoundListItemPtr = Animations.FindByPredicate([&](const TSharedPtr<FWidgetAnimationListItem>& ListItem) { return ListItem->Animation == CurrentAnim; });
	...
	void OnUpdatedAnimationList()
	{
		...
		RefreshCurrentAnimationSelection();

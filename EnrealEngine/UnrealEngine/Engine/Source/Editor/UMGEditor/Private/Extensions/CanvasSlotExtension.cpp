...
{
	if (Event.GetEffectingButton() == EKeys::LeftMouseButton && AnchorWidgets[(int32)AnchorType].IsValid())
	{ 
...
{
	if (AnchorWidgets[(int32)AnchorType].IsValid() && AnchorWidgets[(int32)AnchorType]->HasMouseCapture() && Event.GetEffectingButton() == EKeys::LeftMouseButton)
	{
		EndTransaction();

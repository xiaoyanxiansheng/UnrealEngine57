	...
	[
		...
		[
			...
			[
				...
				.ToolTipText(LOCTEXT("ScreenSize_ToolTip", "Change the size of the designer canvas to the screen resolution."))
		...
		[
			...
			[
				...
				.ToolTipText(LOCTEXT("ScreenFill_ToolTip", "Adjust how the widget content fills the selected Screen Size."))
				.TextStyle(&ToolBarStyle.LabelStyle)
				.ColorAndOpacity(FSlateColor::UseForeground())
			]
		]
...
{
	if (TSharedPtr<SWidget> CachedPreviewWidget = InPreviewWidget->GetCachedWrappedWidget())
	{
...
void SDesignerView::BroadcastDesignerChanged()
{
	...
	UUserWidget* LatestPreviewWidget = WidgetBlueprintEditor->GetPreview();
	...
	if ( LatestPreviewWidget )
	{
		...
		WidgetBlueprintEditor->OnWidgetPreviewReady.Broadcast();
	}
}
...
void SDesignerView::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{
	...
	if ( PreviewWidget )
	{
		if (TSharedPtr<SWidget> CachedWidget = PreviewWidget->GetCachedWrappedWidget())
		{
...
FReply SDesignerView::OnDragDetected(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	if (SelectedWidgets.Num() > 0)
	{
		...
		{		
			if (UWidget* WidgetPreview = SelectedWidget.GetPreview())
			{
				if (TSharedPtr<SWidget> CachedWidget = WidgetPreview->GetCachedWrappedWidget())
				{
				...
				if (ensure(Widget))
				{
					...
					if (Slot != nullptr)
					{
						...
						if (UCanvasPanelSlot* CanvasSlot = Cast<UCanvasPanelSlot>(Slot))
						{
							if (bIsPreview)
							{
								if (TSharedPtr<SWidget> CachedWidget = Widget->GetCachedWrappedWidget())
								{

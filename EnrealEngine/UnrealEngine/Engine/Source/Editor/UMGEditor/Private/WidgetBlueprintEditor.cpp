...
{
	if (!bAppendOrToggle && SelectedWidgets.Array() == Widgets.Array())
	{
		return;
	}
...
void FWidgetBlueprintEditor::MigrateFromChain(const FPropertyChangedEvent* PropertyChangedEvent, FEditPropertyChain* PropertyThatChanged, bool bIsModify)
{
	...
	if (const UUserWidget* PreviewUserWidget = GetPreview())
	{
		...
		// The root widget is only ever in the SelectedObjects array, the root widget is a copy of the CDO instanced for preview purposes.
		...
		for (TWeakObjectPtr<UObject> SelectedObjectRef : SelectedObjects)
		{
			UObject* SelectedObjectPtr = SelectedObjectRef.Get();
			ensure(SelectedObjectPtr);
			if (!SelectedObjectPtr)
			{
				continue;
			}

			// Ignore selected objects that aren't the root widget
			if (!PreviewUserWidget->IsA(SelectedObjectPtr->GetClass()))
			{
				continue;
			}

			// dealing with root widget here
			if (UObject* WidgetCDO = SelectedObjectPtr->GetClass()->GetDefaultObject(true))
			{
				...
				if (SelectedObjectPtr->IsA(OwnerClass) && WidgetCDO->IsA(OwnerClass) && PropertyThatChanged)
				{
					if (PropertyChangedEvent == nullptr)
					{
					...
					else
					{
						...
						// able to cheat like we do still having a pristine CDO copy. The details panel has to do shenanigans where it copies
						...
						PropagateDefaultPropertyChange(*PropertyChangedEvent, *PropertyThatChanged, WidgetCDO, SelectedObjectRef.Get());
					}
				
					MigratePropertyValue(SelectedObjectRef.Get(), WidgetCDO, PropertyChainNode, PropertyChainNode->GetValue(), bIsModify);
...
void FWidgetBlueprintEditor::OnBuildCustomContextMenuForGuid(FMenuBuilder& MenuBuilder, FGuid ObjectBinding)
{
	if (CurrentAnimation.IsValid())
	{
		...
		if (ValidSelectedWidgets.Num() > 0)
		{
			...
			if (CanAddDynamicPossessionMenu())
			{
				MenuBuilder.AddSubMenu(
...
bool FWidgetBlueprintEditor::CanAddDynamicPossessionMenu()
{
	TSharedPtr<ISequencer>& ActiveSequencer = GetSequencer();
	...
	FMovieSceneSequenceEditor* SequenceEditor = SequencerModule.FindSequenceEditor(WidgetAnimation->GetClass());
	return SequenceEditor && SequenceEditor->SupportsDirectorBlueprint(WidgetAnimation);
}
...
void FWidgetBlueprintEditor::OnMovieSceneBindingsPasted(const TArray<FMovieSceneBinding>& BindingsPasted)
{
	...
	for (const FMovieSceneBinding& BindingPasted : BindingsPasted)
	{
		...
		FText BindingPastedName = AnimationSequence->GetMovieScene()->GetObjectDisplayName(BindingPasted.GetObjectGuid());
		if (Possessable && !Possessable->GetParent().IsValid())
		{
			for (FObjectAndDisplayName& BindableObject : BindableObjects)
			{
				if (BindableObject.DisplayName.EqualTo(BindingPastedName))
				{
		...
		if (Possessable && Possessable->GetParent().IsValid())
		{
			for (FObjectAndDisplayName& BindableObject : BindableObjects)
			{
				...
				if (PanelSlot && PanelSlot->Content)
				{
					...
					if (BindableObject.Object->GetFName() == BindingPastedName.ToString())
					{

...
#include UE_INLINE_GENERATED_CPP_BY_NAME(WidgetBlueprint)
...
void UWidgetBlueprint::NotifyGraphRenamed(class UEdGraph* Graph, FName OldName, FName NewName)
{
	...
	const TObjectPtr<UBlueprintExtension>* UIComponentExtensionPtr = GetExtensions().FindByPredicate([](const UBlueprintExtension* Other) {
		return Other && Other->GetClass() == UUIComponentWidgetBlueprintExtension::StaticClass(); 
	});

	UUIComponentWidgetBlueprintExtension* UIComponentExtension = nullptr;
	if (UIComponentExtensionPtr)
	{
		UIComponentExtension = CastChecked<UUIComponentWidgetBlueprintExtension>(*UIComponentExtensionPtr);
	}

	// Update any explicit widget bindings.
	WidgetTree->ForEachWidget([Graph, OldName, NewName, UIComponentExtension](UWidget* Widget) {
		...
		if (UIComponentExtension != nullptr)
		{
			for (UUIComponent* Component : UIComponentExtension->GetComponentsFor(Widget))
			{
				Component->SetFlags(RF_Transactional);
				Component->Modify();
				Component->GraphRenamed(Graph, OldName, NewName);
			}
		}
	});
}
...
bool UWidgetBlueprint::FindDiffs(const UBlueprint* OtherBlueprint, FDiffResults& Results) const
{
	...
	for (const TObjectPtr<UBlueprintExtension>& Extension : GetExtensions())
	{
		if (Extension->IsA<UWidgetBlueprintExtension>())
		{
			if (UWidgetBlueprintExtension* WidgetExtension = StaticCast<UWidgetBlueprintExtension*>(Extension.Get()))
			{
				WidgetExtension->FindDiffs(this, OtherWidgetBP, Results);
			}
		}
	}

...
public:
	UE_API UWidgetEditingProjectSettings();
...
public:
	...
	/** Use the new version of the binding in the details view. */
	bool bUseNewBindingFromDetailsView = false;

	/** Use compact link button (just display the link) or complete one (with info about the current binding in the extension part of the detail view. */
	bool bUseCompactLinkButtonInExtension = false;
	...
	UE_API bool CompilerOption_AllowBlueprintTick(const class UWidgetBlueprint* WidgetBlueprint) const;
	UE_API bool CompilerOption_AllowBlueprintPaint(const class UWidgetBlueprint* WidgetBlueprint) const;
	UE_API EPropertyBindingPermissionLevel CompilerOption_PropertyBindingRule(const class UWidgetBlueprint* WidgetBlueprint) const;
	UE_API TArray<UWidgetCompilerRule*> CompilerOption_Rules(const class UWidgetBlueprint* WidgetBlueprint) const;

	/** Get the permission list that controls which categories are exposed in config palette filtering */
	UE_API FNamePermissionList& GetAllowedPaletteCategories();
	UE_API const FNamePermissionList& GetAllowedPaletteCategories() const;

	/** Get the permission list that controls which widgets are exposed in config palette filtering */
	UE_API FPathPermissionList& GetAllowedPaletteWidgets();
	UE_API const FPathPermissionList& GetAllowedPaletteWidgets() const;
...
private:
	...
	UE_API void GetCompilerOptionsForWidget(const class UWidgetBlueprint* WidgetBlueprint, TFunctionRef<bool(const FWidgetCompilerOptions&)> Operator) const;

protected:
	UE_API virtual void PerformUpgradeStepForVersion(int32 ForVersion);

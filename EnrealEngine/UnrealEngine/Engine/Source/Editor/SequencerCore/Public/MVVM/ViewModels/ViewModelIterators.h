...
struct TPopulatableIteratorMixin
{
	...
	/**
	 * Populate the specified container with the remaining items in this iterator.
	 * ContainerType must have a .Add method that accepts the enumerated type.
	 * The state of this iterator remains unchanged.
	 * 
	 * Example usage:
	 * 
	 * TSet<FViewModelPtr> Set;
	 * It.Populate(Set);
	 */
	template<typename ContainerType>
	void Populate(ContainerType& OutContainer) const
	{
		for (const EnumeratedType& Item : static_cast<const DerivedType&>(*this))
		{
	...
	/**
	 * Create a new container of the templated type and populate it with the remaining items in this iterator.
	 * ContainerType must have a .Add method that accepts the enumerated type.
	 * The state of this iterator remains unchanged.
	 * 
	 * Example usage:
	 * 
	 * TSet<TViewModelPtr<FTrackModel>> Tracks = ViewModel->GetDescendentsOfType<FTrackModel>().Populate<TSet>();
	 */
	template<template<typename T, typename ...Rest> class ContainerType>
	ContainerType<EnumeratedType> Populate() const
	{
		ContainerType<EnumeratedType> Temp;
		Populate(Temp);
		return Temp;
	}
	 ...
	 * The state of this iterator remains unchanged
	 */
	void ToArray(TArray<EnumeratedType>& OutArray) const
	{
		Populate(OutArray);
	}
	...
	EnumeratedType FindBy(const ValueType& Value, ProjectionType Proj) const
	{
		for (const EnumeratedType& Item : static_cast<const DerivedType&>(*this))
		{
 ...
 */
struct FViewModelSubListIterator : TPopulatableIteratorMixin<FViewModelSubListIterator, FViewModelPtr>
{
 ...
 */
struct FViewModelListIterator : TPopulatableIteratorMixin<FViewModelListIterator, FViewModelPtr>
{
...
struct TTypedIteratorBase : public IteratorType, public TPopulatableIteratorMixin<TTypedIteratorBase<T, IteratorType>, TViewModelPtr<T>>
{
	...
	using TPopulatableIteratorMixin<TTypedIteratorBase<T, IteratorType>, TViewModelPtr<T>>::ToArray;
	using TPopulatableIteratorMixin<TTypedIteratorBase<T, IteratorType>, TViewModelPtr<T>>::Populate;
	using TPopulatableIteratorMixin<TTypedIteratorBase<T, IteratorType>, TViewModelPtr<T>>::FindBy;

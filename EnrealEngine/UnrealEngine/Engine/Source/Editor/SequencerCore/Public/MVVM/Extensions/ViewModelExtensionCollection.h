 ...
 */
struct FViewModelExtensionCollection : FNoncopyable
{
private:

	UE_API FViewModelExtensionCollection(FViewModelTypeID InExtensionType);
	UE_API explicit FViewModelExtensionCollection(FViewModelTypeID InExtensionType, TWeakPtr<FViewModel> InWeakModel, int32 InDesiredRecursionDepth = -1);

	UE_API virtual ~FViewModelExtensionCollection();
...
protected:
	...
	UE_API void Reinitialize(TWeakPtr<FViewModel> InWeakModel, int32 InDesiredRecursionDepth = -1);
	UE_API void Destroy();
...
private:
	...
	UE_API void Update() const;

	UE_API void OnHierarchyUpdated();

	UE_API void ConditionalUpdate() const;

	UE_API void DestroyImpl();

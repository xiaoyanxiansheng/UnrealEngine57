...
{
	UE_API ~FViewModelListLink();

	/** Detach only this link from its list */
	UE_API void Unlink();

	/** Find ths last valid link in the list */
	UE_API TSharedPtr<FViewModel> FindLastLink() const;
...
private:
	...
	UE_API void DetectLinkListCycle();

	/** Throws an assert if a cycle is detected */
	static UE_API void DetectLinkListCycle(TSharedPtr<FViewModel> StartAt);

	/** Unlinks the model from its siblings and insert it after the given chain link */
	static UE_API void LinkModelTo(TSharedPtr<FViewModel> Model, TSharedPtr<FViewModelListLink> ToLink);
...
struct FViewModelListHead
{
	...
	UE_API FViewModelPtr GetHead() const;

	/** Finds the last model of this list */
	UE_API FViewModelPtr GetTail() const;

	/** Iterate the children held within this sub list */
	UE_API FViewModelSubListIterator Iterate() const;
	 ...
	 */
	UE_API TSharedPtr<FViewModel> ReliquishList();
...
struct FScopedViewModelListHead
{
	...
	UE_API FScopedViewModelListHead(const TSharedPtr<FViewModel>& InModel, EViewModelListType InType);
	/** Destructor that removes the list to the model */
	UE_API ~FScopedViewModelListHead();
	 ...
	 */
	UE_API FViewModelChildren GetChildren();
...
struct FViewModelChildren
{
	 ...
	 */
	UE_API EViewModelListType GetType() const;
	 ...
	 */
	UE_API void AddChild(const TSharedPtr<FViewModel>& Child);
	 ...
	 */
	UE_API void InsertChild(const TSharedPtr<FViewModel>& Child, const TSharedPtr<FViewModel>& PreviousSibling);

	/**
	 * Move all these children into the specified destination list, changing parent pointers if necessary
	 */
	UE_API void MoveChildrenTo(const FViewModelChildren& OutDestination);
...
public:
	...
	UE_API FViewModelSubListIterator IterateSubList() const;
...
private:
	...
	UE_API FViewModelChildren(const TSharedPtr<FViewModel>& InOwner, FViewModelListHead* InListHead);
...
public:
	...
	UE_API explicit FViewModelHierarchyOperation(const TSharedPtr<FSharedViewModelData>& InSharedData);
	UE_API ~FViewModelHierarchyOperation();

	/**
	 * Called before a change is made to the specified model's parent or previous sibling
	 */
	UE_API void PreHierarchicalChange(const TSharedPtr<FViewModel>& InChangedNode);

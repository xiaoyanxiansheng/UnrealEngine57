...
public:
	 ...
	 */
	static UE_API TSharedRef<FOutlinerViewModelDragDropOp> New(TArray<TWeakViewModelPtr<IOutlinerExtension>>&& InDraggedViewModels, FText InDefaultText, const FSlateBrush* InDefaultIcon);
...
public:
	 ...
	 */
	UE_API void ResetToDefaultToolTip();

	/**
	 * Gets the nodes which are currently being dragged.
	 */
	UE_API TArrayView<const TWeakViewModelPtr<IOutlinerExtension>> GetDraggedViewModels() const;
	 ...
	 */
	UE_API bool ValidateParentChildDrop(const FViewModel& ProspectiveItem);

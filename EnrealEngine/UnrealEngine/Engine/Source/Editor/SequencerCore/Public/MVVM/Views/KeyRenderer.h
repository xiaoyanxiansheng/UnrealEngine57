...
struct FKeyRenderer
{
	 ...
	 */
	UE_API void Initialize(const FViewModelPtr& InViewModel);

	/**
	 * Update any invalidated data required for drawing keys
	 */
	UE_API void Update(const FKeyBatchParameters& Params, const FGeometry& AllottedGeometry) const;

	/**
	 * Draw this batch's curve
	 */
	UE_API int32 DrawCurve(const FKeyBatchParameters& Params, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, const FKeyRendererPaintArgs& Args, int32 LayerId) const;

	/**
	 * Draw this batch
	 */
	UE_API int32 Draw(const FKeyBatchParameters& Params, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, const FKeyRendererPaintArgs& Args, int32 LayerId) const;

	/**
	 * Retrieve keys at the specified position
	 */
	UE_API void HitTestKeys(const FFrameTime& Time, TArray<FKeysForModel>& OutAllKeys) const;

	/**
	 * Retrieve keys at the specified position
	 */
	UE_API bool HitTestKeyBar(const FFrameTime& Time, FKeyBar& OutKeyBar) const;

	/**
	 * Returns true if this renderer has a curve, false otherwise
	 */
	UE_API bool HasCurve() const;
...
private:
	 ...
	 */
	UE_API void CacheKeyExtensions(const FKeyBatchParameters& Params) const;

	/**
	 * Attempt to update data that is not dependent upon the current view
	 * @return EViewDependentCacheFlags::DataChanged if view-indipendent data was updated, EViewDependentCacheFlags::None otherwise
	 */
	UE_API EViewDependentCacheFlags UpdateViewIndependentData() const;


	/**
	 * Ensure that view-dependent data (such as which keys need drawing and how) is up to date
	 */
	UE_API void UpdateViewDependentData(const FKeyBatchParameters& Params, const FGeometry& AllottedGeometry) const;

...
struct FSortingKey
{
	...
	static UE_API bool ComparePriorityFirst(const FSortingKey& A, const FSortingKey& B);
	/** Compare custom orders (if both set), priorities, and identifiers */
	static UE_API bool CompareCustomOrderFirst(const FSortingKey& A, const FSortingKey& B);
...
public:
	...
	UE_SEQUENCER_DECLARE_VIEW_MODEL_TYPE_ID_API(UE_API, ISortableExtension)
	...
	static UE_API void SortChildren(TSharedPtr<FViewModel> ParentModel, ESortingMode SortingMode);
	/** Utility function for sorting children based on their sorting key */
	static UE_API void SortChildren(FViewModelChildren& Children, ESortingMode SortingMode);
};

...
void SCompoundTrackLaneView::Construct(const FArguments& InArgs, TSharedPtr<ITrackLaneWidgetSpace> InTrackLaneWidgetSpace)
{
	SetClipping(EWidgetClipping::ClipToBounds);

	WeakTrackLaneWidgetSpace = InTrackLaneWidgetSpace;
}
...
{
	TSharedPtr<ITrackLaneWidgetSpace> WidgetSpace = WeakTrackLaneWidgetSpace.Pin();
	if (!WidgetSpace)
	{
		return;
	}
	...
	for (int32 WidgetIndex = 0; WidgetIndex < Children.Num(); ++WidgetIndex)
	{
		...
		if (ArrangedChildren.Accepts(WidgetVisibility))
		{
			...
			FTrackLaneScreenAlignment ScreenAlignment = Interface->GetAlignment(*WidgetSpace, LaneGeometry);

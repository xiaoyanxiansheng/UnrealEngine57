...
void FTrackAreaViewModel::OnConstruct()
{
	UpdateViewSpaces();
}
...
void FTrackAreaViewModel::UpdateViewSpaces()
{
	ViewSpaces.Empty();

	FViewModelPtr RootModel = GetEditor()->GetRootModel();
	FSimpleMulticastDelegate& Event = RootModel->GetSharedData()->SubscribeToHierarchyChanged(RootModel);
	if (!Event.IsBoundToObject(this))
	{
		Event.AddSP(this, &FTrackAreaViewModel::UpdateViewSpaces);
	}

	for (TViewModelPtr<ITrackAreaViewSpaceProviderExtension> ViewSpace : RootModel->GetChildrenOfType<ITrackAreaViewSpaceProviderExtension>())
	{
		ViewSpace->UpdateViewSpaces(*this);
	}
}
...
void FTrackAreaViewModel::AddViewSpace(const FGuid& InID, TSharedPtr<FTrackAreaViewSpace> ViewSpace)
{
	TSharedPtr<FTrackAreaViewSpace> Existing = ViewSpaces.FindRef(InID);
	if (Existing != ViewSpace)
	{
		ViewSpaces.Add(InID, ViewSpace);
		OnViewSpaceAdded.Broadcast(InID, ViewSpace);
	}
}

void FTrackAreaViewModel::RemoveViewSpace(const FGuid& InID)
{
	TSharedPtr<FTrackAreaViewSpace> Existing = ViewSpaces.FindRef(InID);
	if (Existing)
	{
		ViewSpaces.Remove(InID);
		OnViewSpaceRemoved.Broadcast(InID, Existing);
	}
}

TSharedPtr<FTrackAreaViewSpace> FTrackAreaViewModel::FindViewSpace(const FGuid& InID) const
{
	return ViewSpaces.FindRef(InID);
}

...
void SSchematicGraphPanel::RemoveNode(const FSchematicGraphNode* InNodeToRemove)
{
	...
	for (int32 Iter = Children.Num()-1; Iter >= 0; --Iter)
	{
...
static bool IsInMinimizedOrBackgrounded(const TSharedPtr<SWidget>& Widget)
{
	if (!Widget.IsValid())
	{
		return true;
...
{
	if (IsInMinimizedOrBackgrounded(AsShared()))
	{
		return;
	}

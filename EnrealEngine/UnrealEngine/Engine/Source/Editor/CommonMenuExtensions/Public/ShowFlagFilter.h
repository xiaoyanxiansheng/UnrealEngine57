...
public:
	...
	UE_API explicit FShowFlagFilter(EDefaultMode InDefaultMode);
	...
	UE_API FShowFlagFilter& IncludeFlag(FEngineShowFlags::EShowFlag Flag);
	UE_API FShowFlagFilter& ExcludeFlag(FEngineShowFlags::EShowFlag Flag);
	UE_API FShowFlagFilter& IncludeGroup(EShowFlagGroup Group);
	UE_API FShowFlagFilter& ExcludeGroup(EShowFlagGroup Group);

	UE_API const FGroupedShowFlagIndices& GetFilteredIndices(bool bForceRebuild = false) const;

private:
	UE_API bool ShouldIncludeFlag(const FShowFlagData& Flag) const;
	UE_API void RebuildIndexArray() const;

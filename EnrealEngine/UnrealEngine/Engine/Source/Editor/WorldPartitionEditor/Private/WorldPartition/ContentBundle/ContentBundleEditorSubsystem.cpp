...
#include UE_INLINE_GENERATED_CPP_BY_NAME(ContentBundleEditorSubsystem)
...
void UContentBundleEditorSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	LevelExternalActorsPathsProviderDelegateHandle = ULevel::RegisterLevelExternalActorsPathsProvider(ULevel::FLevelExternalActorsPathsProviderDelegate::CreateUObject(this, &UContentBundleEditorSubsystem::OnGetLevelExternalActorsPaths));
...
void UContentBundleEditorSubsystem::OnGetLevelExternalActorsPaths(const FString& InLevelPackageName, const FString& InPackageShortName, TArray<FString>& OutExternalActorsPaths)
{
	ContentBundlePaths::ForEachContentBundleLevelPackagePath(InLevelPackageName, [&OutExternalActorsPaths](const FString& Path)
	{
		const FString ExternalActorPath = ULevel::GetExternalActorsPath(Path);
		OutExternalActorsPaths.Add(ExternalActorPath);
	});
}

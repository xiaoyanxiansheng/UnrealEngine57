	...
	for (const ULocalizationTarget* Target : Targets)
	{
		...
		TArray<FString> SoundWavePathsToTest;
		{
			...
			for (const FString& Culture : CulturesToTest)
			{
				...
				if (const FString VerseIdentifier = FCulture::CultureNameToVerseIdentifier(Culture);
					VerseIdentifier != Culture)
				{
					DialogueWavePathsToTest.Add(RootAssetPath / TEXT("L10N") / VerseIdentifier);
					SoundWavePathsToTest.Add(RootAssetPath / TEXT("L10N") / VerseIdentifier / Target->Settings.ImportDialogueSettings.ImportedDialogueFolder);
				}
			}
		}

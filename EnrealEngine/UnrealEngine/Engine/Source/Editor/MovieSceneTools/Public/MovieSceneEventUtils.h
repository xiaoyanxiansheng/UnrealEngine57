...
struct FMovieSceneEventUtils
{
	 ...
	 */
	static UE_API FMovieSceneDirectorBlueprintEndpointDefinition GenerateEventDefinition(UMovieSceneTrack* Track);

	/**
	 * Generate event endpoint definition for the given object binding
	 */
	static UE_API FMovieSceneDirectorBlueprintEndpointDefinition GenerateEventDefinition(UMovieScene* MovieScene, const FGuid& ObjectBindingID);
	 ...
	 */
	static UE_API UK2Node_CustomEvent* BindNewUserFacingEvent(FMovieSceneEvent* EntryPoint, UMovieSceneEventSectionBase* EventSection, UBlueprint* Blueprint);
	 ...
	 */
	static UE_API UK2Node_FunctionEntry* GenerateEntryPoint(FMovieSceneEvent* EntryPoint, FKismetCompilerContext* Compiler, UEdGraphNode* Endpoint);
	 ...
	 */
	static UE_API void BindEventSectionToBlueprint(UMovieSceneEventSectionBase* EventSection, UBlueprint* DirectorBP);
	 ...
	 */
	static UE_API void RemoveEndpointsForEventSection(UMovieSceneEventSectionBase* EventSection, UBlueprint* DirectorBP);

	/**
	 * Clear invalid endpoints (end points to that point to a blueprint in a different movie scene, possibly from copy/paste)
	 *
	 * @param EventSection           (Required, non-null) The event section to search for invalid event endpoints from
	 */
	static UE_API void ClearInvalidEndpointsForEventSection(UMovieSceneEventSectionBase* EventSection);
	 ...
	 * @param DirectorBP             (Required, non-null) The owning blueprint from which to remove the event section's entry points
	 */
	static UE_API void RemoveUnusedCustomEvents(const TArray<TWeakObjectPtr<UMovieSceneEventSectionBase>>& EventSections, UBlueprint* DirectorBP);
	 ...
	 */
	static UE_API UK2Node* FindEndpoint(FMovieSceneEvent* EntryPoint, UMovieSceneEventSectionBase* EventSection, UBlueprint* OwnerBlueprint);
	 ...
	 */
	static UE_API void SetEndpoint(FMovieSceneEvent* EntryPoint, UMovieSceneEventSectionBase* EventSection, UK2Node* InNewEndpoint, UEdGraphPin* BoundObjectPin);
};

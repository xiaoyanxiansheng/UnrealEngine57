...
*/
class FMovieSceneTranslatorContext : public TSharedFromThis<FMovieSceneTranslatorContext>
{
public:
	/** Constructor */
	...
	/** Add message. */
	UE_API void AddMessage(EMessageSeverity::Type MessageSeverity, FText ErrorMessage);

	/** Reset all messages. */
	UE_API void ClearMessages();

	/** Returns true if specified type of message */
	UE_API bool ContainsMessageType(EMessageSeverity::Type InSeverity) const;

	/** Get error messages. */
	UE_API const TArray<TSharedRef<FTokenizedMessage>>& GetMessages() const;
...
public:
	/** Constructor */
	UE_API FMovieSceneExportData(const UMovieScene* InMovieScene, FFrameRate InFrameRate, uint32 InResX, uint32 InResY, int32 InHandleFrames, FString InSaveFilename, TSharedPtr<FMovieSceneTranslatorContext> InContext, FString InMovieExtension);
	/** Default constructor, necessary for shared ref - should not be used */
	UE_API FMovieSceneExportData();
	/** Destructor */
	UE_API ~FMovieSceneExportData();
	...
	UE_API FString GetFilenamePath() const;
	/** Gets the shot movie extension */
	UE_API FString GetMovieExtension() const;

	/** Gets export frame rate */
	UE_API FFrameRate GetFrameRate() const;
	/** Gets x resolution */
	UE_API uint32 GetResX() const;
	/** Gets y resolution */
	UE_API uint32 GetResY() const;
	/** Returns true if frame rate is a non-integral frame rate */
	UE_API bool GetFrameRateIsNTSC() const;
	/** Returns the nearest integral frame rate */
	UE_API uint32 GetNearestWholeFrameRate() const;

	/** Gets the frame handle */
	UE_API int32 GetHandleFrames() const;
	/** Gets default audio sample rate */
	UE_API int32 GetDefaultAudioSampleRate() const;
	/** Gets default audio depth */
	UE_API int32 GetDefaultAudioDepth() const;

	/** True when the export data was successfully constructed. */
	UE_API bool IsExportDataValid() const;

	/** Find audio sections */
	UE_API bool FindAudioSections(const FString& InSoundPathName, TArray<TSharedPtr<FMovieSceneExportAudioSectionData>>& OutFoundSections) const;

private:

	/** Entry point for creating the intermediate data to use when exporting. */
	UE_API bool ConstructData(const UMovieScene* InMovieScene);
	/** Loads intermediate movie scene data from Sequencer. */
	UE_API bool ConstructMovieSceneData(const UMovieScene* InMovieScene);
	/** Loads intermediate cinematic data from Sequencer. */
	UE_API bool ConstructCinematicData(const UMovieScene* InMovieScene, const UMovieSceneCinematicShotTrack* InCinematicTrack);
	/** Loads intermediate cinematic data from Sequencer. */
	UE_API bool ConstructCinematicTrackData(const UMovieScene* InMovieScene, TSharedPtr<FMovieSceneExportCinematicData> InCinematicData, int32 InRowIndex);
	/** Loads intermediate audio data from Sequencer. */
	UE_API bool ConstructAudioData(const UMovieScene* InMovieScene, const UMovieSceneAudioTrack* InAudioTrack, TMap<int32, TSharedPtr<FMovieSceneExportAudioData>>& InAudioDataMap);
	/** Loads intermediate audio data from Sequencer. */
	UE_API bool ConstructAudioData(const UMovieScene* InMovieScene, TSharedPtr<FMovieSceneExportAudioData> InAudioData, int32 InRowIndex);
	/** Loads intermediate cinematic section data from Sequencer. */
	UE_API bool ConstructCinematicSectionData(const UMovieScene* InMovieScene, TSharedPtr<FMovieSceneExportCinematicData> InCinematicData, const UMovieSceneCinematicShotSection* InCinematicSection);
	/** Loads intermediate audio section data from Sequencer. */
	UE_API bool ConstructAudioSectionData(const UMovieScene* InMovieScene, TSharedPtr<FMovieSceneExportAudioData> InAudioData, const UMovieSceneAudioSection* InAudioSection);
	/** Loads intermediate common section data from Sequencer. */
	UE_API bool ConstructSectionData(const UMovieScene* InMovieScene, TSharedPtr<FMovieSceneExportSectionData> InSectionData, const UMovieSceneSection* InSection, EMovieSceneTranslatorSectionType InSectionType, const FString& InSectionDisplayName);
...
class FMovieSceneImporter : public FMovieSceneTranslator
{
public:
...
class FMovieSceneExporter : public FMovieSceneTranslator
{
public:

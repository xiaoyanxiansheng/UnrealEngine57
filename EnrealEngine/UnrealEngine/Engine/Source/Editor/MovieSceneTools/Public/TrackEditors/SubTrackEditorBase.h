 ...
 */
class FSubSectionPainterUtil
{
public:
    /** Paints the sub-section, mostly by painting loop boundaries, if appropriate */
    static UE_API FSubSectionPainterResult PaintSection(TSharedPtr<const ISequencer> Sequencer, const UMovieSceneSubSection& SectionObject, FSequencerSectionPainter& InPainter, FSubSectionPainterParams Params);

private:
    static UE_API void PaintSectionBounds(TSharedPtr<const ISequencer> Sequencer, const UMovieSceneSubSection& SectionObject, const UMovieSceneSequence& InnerSequence, FSequencerSectionPainter& InPainter, ESlateDrawEffect DrawEffects);
};
...
{
public:
    UE_API FSubSectionEditorUtil(UMovieSceneSubSection& InSection);
    UE_API ~FSubSectionEditorUtil();

    /** Starts a resize operation */
    UE_API void BeginResizeSection();
    /** Resizes the section, returns the new resize time */
    UE_API FFrameNumber ResizeSection(ESequencerSectionResizeMode ResizeMode, FFrameNumber ResizeTime);

    /** Starts a new slip operation */
    UE_API void BeginSlipSection();
    /** Slips the section, returns the new slip time */
    UE_API FFrameNumber SlipSection(FFrameNumber SlipTime);

    /** Starts a new dilate operation */
    UE_API void BeginDilateSection();
    /** Dilates the section */
    UE_API void DilateSection(const TRange<FFrameNumber>& NewRange, float DilationFactor);
...
public:
	 ...
	 */
	static UE_API bool CanAddSubSequence(const UMovieSceneSequence* CurrentSequence, const UMovieSceneSequence& SubSequence);

	static UE_API UMovieSceneMetaData* FindOrAddMetaData(UMovieSceneSequence* Sequence);

	static UE_API FText GetMetaDataText(const UMovieSceneSequence* Sequence);
};

...
struct FMovieSceneChannelValueSetter
{
	...
	/** Templated construction function that can add a key (and potentially also set a default) for the specified channel and value */
	static FMovieSceneChannelValueSetter Create(UE::MovieScene::FUnpackedChannelValue&& InValue, bool bAddKey)
	{
		FMovieSceneChannelValueSetter NewValue;
		NewValue.Impl = FUnpackedChannelValueSetter(MoveTemp(InValue), bAddKey);
		return MoveTemp(NewValue);
	}
...
private:
	...
	struct FUnpackedChannelValueSetter : IImpl
	{
		UE::MovieScene::FUnpackedChannelValue ChannelValue;
		bool bAddKey;

		FUnpackedChannelValueSetter(UE::MovieScene::FUnpackedChannelValue&& InValue, bool bInAddKey)
			: ChannelValue(MoveTemp(InValue)), bAddKey(bInAddKey)
		{}
		...
		virtual bool Apply(UMovieSceneSection* Section, FMovieSceneChannelProxy& Proxy, FFrameNumber InTime, EMovieSceneKeyInterpolation InterpolationMode, bool bKeyEvenIfUnchanged, bool bKeyEvenIfEmpty) const override
		{
			...
			FMovieSceneChannel* Channel = ChannelValue->RetrieveChannel(Proxy);
			if (bAddKey && Channel)
			{
				bool bShouldKeyChannel = bKeyEvenIfUnchanged;
				if (!bShouldKeyChannel)
				{
					bShouldKeyChannel = !ChannelValue->AlreadyExistsAtTime(Channel, InTime);
				}
				...
				if (bShouldKeyChannel)
				{
					if (Channel->GetNumKeys() != 0 || bKeyEvenIfEmpty)
					{
						if (Section->TryModify())
						{
							ChannelValue->AddKey(Channel, InTime, InterpolationMode);
							bKeyCreated = true;
						}
					}
				}
		...
		virtual void ApplyDefault(UMovieSceneSection* Section, FMovieSceneChannelProxy& Proxy, EKeyFrameTrackEditorSetDefault SetDefault) const override
		{
			...
			const bool bShouldSetDefault = SetDefault == EKeyFrameTrackEditorSetDefault::SetDefault || (SetDefault == EKeyFrameTrackEditorSetDefault::SetDefaultOnAddKeys && bAddKey);
			FMovieSceneChannel* Channel = ChannelValue->RetrieveChannel(Proxy);
			if (bShouldSetDefault && Channel && Channel->GetNumKeys() == 0 && !ChannelValue->IsAlreadyDefault(Channel))
			{
				if (Section->TryModify())
				{
					ChannelValue->SetDefault(Channel);
				}
			}
		}
		...
		virtual bool ModifyByCurrentAndWeight(FMovieSceneChannelProxy& Proxy, FFrameNumber InTime, void* VCurrentValue, float Weight) override
		{
			ensure(!"This should never be called any more");
			return false;
		}
	};
...
protected:
	...
	UE_DEPRECATED(5.7, "This function is no longer called. Please ensure that generated keys populated through GenerateKeysCallback have already been compensated.")

...
namespace
{
	...
	TArray<IndexRange> GetIndexRanges(const FMovieSceneConstraintChannel* Channel)
	{
		...
		const bool bIsActiveByDefault = Channel->GetDefault() && *Channel->GetDefault();
		if (Values.Num() == 0 && bIsActiveByDefault)
		{
			Ranges.Emplace(INDEX_NONE);
			return Ranges;
		}
		...
		if (Values.Num() == 1)
		{
			...
			{
				if (bIsActiveByDefault)
				{
					Ranges.Emplace(INDEX_NONE, 0);
				}
			}
			return Ranges;
		}
		...
		if (bIsActiveByDefault)
		{
			const int32 FirstInactive = Values.Find(false);
			if (FirstInactive != INDEX_NONE)
			{
				Ranges.Emplace(INDEX_NONE, FirstInactive);
				Offset = FirstInactive+1;
			}
		}
	...
	const UMovieSceneSection* Owner)
{
	...
	auto AddNewRange = [&Ranges, LabelStr = LabelBuilder.Label.ToString()]() -> FKeyBarCurveModel::FBarRange&
	{
		FKeyBarCurveModel::FBarRange& BarRange = Ranges.Emplace_GetRef();
		
		BarRange.Name = FName(*LabelStr);
	...
	// infinite range
	if (IndexRanges[0].HasLowerBound() && IndexRanges[0].GetLowerBoundValue() == INDEX_NONE &&
		IndexRanges[0].HasUpperBound() && IndexRanges[0].GetUpperBoundValue() == INDEX_NONE)
	{
		FKeyBarCurveModel::FBarRange& BarRange = AddNewRange();
		BarRange.bRangeIsInfinite = true;
	}
	else
	{
		...
		for (const IndexRange& ActiveRange : IndexRanges)
		{
			...
			if (ActiveRange.HasLowerBound() && ActiveRange.GetLowerBoundValue() > INDEX_NONE)
			{
			...
			if (ActiveRange.HasUpperBound() && ActiveRange.GetUpperBoundValue() > INDEX_NONE)
			{
	...
	int32 LayerId)
{
	...
	for (int32 Index = 0; Index < Ranges.Num(); ++Index)
	{
		...
		// draw bar
		double LowerSeconds = InputMin;
		double UpperSeconds = InputMax;
		
		if (Range.Range.HasLowerBound())
		{
			LowerSeconds = Range.Range.GetLowerBoundValue();
		}

		if (Range.Range.HasUpperBound())
		{
			UpperSeconds = Range.Range.GetUpperBoundValue();
			if (Range.Range.HasLowerBound() && UpperSeconds == Range.Range.GetLowerBoundValue())
			{
...
TUniquePtr<FCurveModel> CreateCurveEditorModel(const TMovieSceneChannelHandle<FMovieSceneConstraintChannel>& ChannelHandle, const UE::Sequencer::FCreateCurveEditorModelParams& Params)
{
	if (FMovieSceneConstraintChannel* Channel = ChannelHandle.Get())
	{
		...
		if (Settings && Settings->GetShowBars())
		{

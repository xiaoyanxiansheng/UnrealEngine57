	...
	for (auto It = CurveInfoByID.CreateConstIterator(); It; ++It)
	{
		...
		if (KeyBarCurveModel)
		{
			...
			for (int32 Index = 0;Index < Ranges.Num(); ++Index)
			{
				...
				double LowerSeconds = Range.Range.HasLowerBound() ? Range.Range.GetLowerBoundValue() : InputMin;
				double UpperSeconds = Range.Range.HasUpperBound() ? Range.Range.GetUpperBoundValue() : InputMax;

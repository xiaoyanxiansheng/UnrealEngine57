...
{
static FString LexToString(TArrayView<const FFrameNumber> Times)
{
	return FString::JoinBy(Times, TEXT(", "), [](const FFrameNumber& FrameNumber)
	{
		return LexToString (FrameNumber);
	});
}
}
...
void FDoubleChannelCurveModel::GetValueRange(double InMinTime, double InMaxTime, double& MinValue, double& MaxValue) const
{
	...
	if (Channel && Section)
	{
		...
		const FFrameTime StartTime = InMinTime * TickResolution;
		const FFrameTime EndTime = InMaxTime * TickResolution;

		// DP 5.7: This exists to debug issue UE-326363. We think that the Times array may be out of order (broken invariant). Remove once the issue is fixed.
		using namespace UE::MovieSceneTools;
		UE_CLOG(StartTime > EndTime, LogMovieScene, Warning, TEXT("StartTime > EndTime: InMinTime: %f, InMaxTime: %f, TickResolution: %s, StartTime (%s), EndTime (%s): Array is %s"),
			InMinTime, InMaxTime, *TickResolution.ToPrettyText().ToString(), *LexToString(StartTime), *LexToString(EndTime), *LexToString(Times)
			);
		
		UE::MovieScene::Interpolation::FInterpolationExtents Extents =
			Channel->ComputeExtents(StartTime, EndTime);
...
void FDoubleChannelCurveModel::CreateKeyProxies(TWeakPtr<FCurveEditor> InWeakCurveEditor, FCurveModelID InCurveModelID, TArrayView<const FKeyHandle> InKeyHandles, TArrayView<UObject*> OutObjects)
{

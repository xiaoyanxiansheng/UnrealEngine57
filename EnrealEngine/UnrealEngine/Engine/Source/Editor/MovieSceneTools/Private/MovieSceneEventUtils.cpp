...
void FMovieSceneEventUtils::ClearInvalidEndpointsForEventSection(UMovieSceneEventSectionBase* EventSection)
{
	check(EventSection);
	...
	if (Sequence)
	{
		TArray<TTuple<FMovieSceneEvent*, UObject*, UMovieSceneSequence*>> InvalidEntryPoints;

		for (FMovieSceneEvent& EntryPoint : EventSection->GetAllEntryPoints())
		{
			TWeakObjectPtr<UObject> EndPoint = EntryPoint.WeakEndpoint;
			if (!EndPoint.IsValid())
			{
				continue;
			}

			UBlueprint* FunctionBlueprint = EndPoint->GetTypedOuter<UBlueprint>();
			if (!FunctionBlueprint)
			{
				continue;
			}

			UMovieSceneSequence* FunctionBlueprintSequence = Cast<UMovieSceneSequence>(FunctionBlueprint->GetOuter());
			if (FunctionBlueprintSequence != nullptr && FunctionBlueprintSequence != Sequence)
			{
				InvalidEntryPoints.Emplace(&EntryPoint, EndPoint.Get(), FunctionBlueprintSequence);
			}
		}

		if (InvalidEntryPoints.Num())
		{
			FScopedTransaction ClearInvalidEndpoints(LOCTEXT("ClearInvalidEndpoint", "Clear Invalid Endpoints"));

			for (TTuple<FMovieSceneEvent*, UObject*, UMovieSceneSequence*> InvalidEntryPoint : InvalidEntryPoints)
			{
				FMovieSceneEvent* EntryPoint = InvalidEntryPoint.Get<0>();
				UObject* EndPoint = InvalidEntryPoint.Get<1>();
				UMovieSceneSequence* FunctionBlueprintSequence = InvalidEntryPoint.Get<2>();

				FNotificationInfo Info(FText::Format(LOCTEXT("ClearInvalidEndpointNotify", "Clearing event {0} from {1} because it refers to a different sequence {2}"), FText::FromString(GetNameSafe(EndPoint)), FText::FromString(GetNameSafe(Sequence)), FText::FromString(GetNameSafe(FunctionBlueprintSequence))));
				Info.ExpireDuration = 3.f;
				FSlateNotificationManager::Get().AddNotification(Info);

				UE_LOG(LogMovieScene, Display, TEXT("Clearing event %s from %s because it refers to a different sequence %s"), *GetNameSafe(EndPoint), *GetNameSafe(Sequence), *GetNameSafe(FunctionBlueprintSequence));

				// Clear this entry point
				*EntryPoint = FMovieSceneEvent();
			}
		}
	}
}

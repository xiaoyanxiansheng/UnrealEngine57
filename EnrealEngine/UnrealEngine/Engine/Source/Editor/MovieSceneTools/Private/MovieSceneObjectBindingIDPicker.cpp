...
void FMovieSceneObjectBindingIDPicker::OnGetMenuContent(FMenuBuilder& MenuBuilder, TSharedPtr<FSequenceBindingNode> Node)
{
	...
	if (Node->BindingID.Guid.IsValid() && IsNodeAllowed(Node))
	{
		bHadAnyEntries = true;
...
bool FMovieSceneObjectBindingIDPicker::IsNodeAllowed(const TSharedPtr<FSequenceBindingNode>& Node) const
{
	return IsClassAllowed(Node);
}

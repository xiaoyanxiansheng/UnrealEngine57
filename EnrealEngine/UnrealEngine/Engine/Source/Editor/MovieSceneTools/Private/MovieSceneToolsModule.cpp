...
#include "TrackEditors/PropertyTrackEditors/TextPropertyTrackEditor.h"
...
void FMovieSceneToolsModule::StartupModule()
{
	...
	if (GIsEditor)
	{
		...
		RegisterPropertyType(MakeShared<TRegisteredProperty<UMovieSceneBoolTrack, UMovieSceneBoolSection>>([](UMovieSceneSection* SectionObject, UMovieSceneTrack* Track, const FGuid& ObjectBinding, TWeakPtr<ISequencer> WeakSequencer){
			return TSharedPtr<ISequencerSection>(MakeShared<FBoolPropertySection>(*SectionObject));
		}));
		RegisterPropertyType(MakeShared<TRegisteredProperty<UMovieSceneTransformTrack, UMovieScene3DTransformSection>>([](UMovieSceneSection* SectionObject, UMovieSceneTrack* Track, const FGuid& ObjectBinding, TWeakPtr<ISequencer> WeakSequencer){
			return TSharedPtr<ISequencerSection>(MakeShared<FTransformSection>(*SectionObject, WeakSequencer));
		}));
		RegisterPropertyType(MakeShared<TRegisteredProperty<UMovieSceneColorTrack, UMovieSceneColorSection>>([](UMovieSceneSection* SectionObject, UMovieSceneTrack* Track, const FGuid& ObjectBinding, TWeakPtr<ISequencer> WeakSequencer){
			return TSharedPtr<ISequencerSection>(MakeShared<FColorPropertySection>(*SectionObject, ObjectBinding, WeakSequencer));
		}));
		...
		VisibilityPropertyTrackCreateEditorHandle = SequencerModule.RegisterTrackEditor(FOnCreateTrackEditor::CreateStatic(FVisibilityPropertyTrackEditor::CreateTrackEditor));
		...
		RegisteredPropertyTrackCreateEditorHandle = SequencerModule.RegisterTrackEditor(FOnCreateTrackEditor::CreateStatic(&FRegisteredPropertyTrackEditor::CreateTrackEditor));
...
void FMovieSceneToolsModule::ShutdownModule()
{
	...
	SequencerModule.UnRegisterTrackEditor( RegisteredPropertyTrackCreateEditorHandle );
...
void FMovieSceneToolsModule::PostDuplicateEventSection(UMovieSceneEventSectionBase* Section)
{
	...
	FMovieSceneEventUtils::ClearInvalidEndpointsForEventSection(Section);
...
FMovieSceneToolsAnimationBakingWrapper::FMovieSceneToolsAnimationBakingWrapper(UMovieScene* InMovieScene)
	: MovieScene(InMovieScene)
	, BakeHelpers(FMovieSceneToolsModule::Get().GetAnimationBakeHelpers())
{
	ensure(InMovieScene);
}

void FMovieSceneToolsAnimationBakingWrapper::Start() const
{
	if (MovieScene)
	{
...
void FMovieSceneToolsAnimationBakingWrapper::PreEvaluate(const FFrameNumber& Frame) const
{
	if (MovieScene)
	{
		for (IMovieSceneToolsAnimationBakeHelper* BakeHelper : BakeHelpers)
		{
			if (BakeHelper)
			{
				BakeHelper->PreEvaluation(MovieScene, Frame);
			}
		}
	}
}

void FMovieSceneToolsAnimationBakingWrapper::PostEvaluate(const FFrameNumber& Frame) const
{
	if (MovieScene)
	{
		for (IMovieSceneToolsAnimationBakeHelper* BakeHelper : BakeHelpers)
		{
			if (BakeHelper)
			{
				BakeHelper->PostEvaluation(MovieScene, Frame);
			}
		}
	}
}

void FMovieSceneToolsAnimationBakingWrapper::Stop() const
{
	if (MovieScene)
	{
		for (IMovieSceneToolsAnimationBakeHelper* BakeHelper : BakeHelpers)

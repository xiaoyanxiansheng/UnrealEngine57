...
FKeyPropertyResult F3DAttachTrackEditor::AddKeyInternal( FFrameNumber KeyTime, const TArray<TWeakObjectPtr<UObject>> Objects, const FName SocketName, const FName ComponentName, FActorPickerID ActorPickerID)
{
	...
	for (int32 ObjectIndex = 0; ObjectIndex < Objects.Num(); ++ObjectIndex)
	{
		...
		FFrameNumber AttachEndTime = PlaybackRange.GetUpperBoundValue();
		...
		if (PreserveType == ETransformPreserveType::None)
		{
			...
			UMovieSceneSection* ExistingSection = MovieSceneHelpers::FindSectionAtTime(Track->GetAllSections(), KeyTime);
			UMovieSceneSection* NextSection = MovieSceneHelpers::FindNextSection(Track->GetAllSections(), KeyTime);

			// If there's an existing section, clamp it to the start time of the new section
			if (ExistingSection)
			{
				ExistingSection->Modify();
				ExistingSection->SetEndFrame(TRangeBound<FFrameNumber>::Exclusive(KeyTime));
			}
			...
			// If there isn't an existing section, make the start frame of the new section infinite
			if (!ExistingSection)
			{
				NewSection->SectionRange.Value.SetLowerBound(TRangeBound<FFrameNumber>::Open());
			}

			// If there's a next section, make the end frame of the new section the start frame of the next section
			if (NextSection)
			{
				NewSection->SectionRange.Value.SetUpperBound(NextSection->GetInclusiveStartFrame());
			}
			// Otherwise, make the end frame of the new section infinite
			else
			{
		...
		if (IntersectingSection)
		{
			...
			FFrameNumber IntersectionSectionStart = IntersectingSection.GetValue()->HasStartFrame() ? IntersectingSection.GetValue()->GetInclusiveStartFrame() : PlaybackRange.GetLowerBoundValue();
			FFrameNumber IntersectionSectionEnd = IntersectingSection.GetValue()->HasEndFrame() ? IntersectingSection.GetValue()->GetExclusiveEndFrame() : PlaybackRange.GetUpperBoundValue();

			TRange<FFrameNumber> RevertRange = TRange<FFrameNumber>(KeyTime, FMath::Min(AttachEndTime, IntersectionSectionEnd));

			// If the intersecting section starts at the same time as the new section, remove it
			if (IntersectionSectionStart == KeyTime)
			{
			...
			else
			{
				...
				IntersectingSection.GetValue()->SetEndFrame(TRangeBound<FFrameNumber>::Exclusive(KeyTime));
			}

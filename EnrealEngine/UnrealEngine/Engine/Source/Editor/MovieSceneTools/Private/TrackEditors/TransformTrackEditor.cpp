...
void F3DTransformTrackEditor::OnRelease()
{
	...
	{
		FConstraintsManagerController::OnSceneComponentConstrained().Remove(OnSceneComponentConstrainedHandle);
...
void F3DTransformTrackEditor::OnPreTransformChanged( UObject& InObject )
{
	...
	if(Actor && !Actor->IsEditorOnly())
	{
		...
		if( SceneComponentThatChanged )
		{
			...
			{
				// switch to constraint space if needed so that the optional LastTransform passed to F3DTransformTrackEditor::GetTransformKeys
				// is expressed in the right space to compare values
				const FTransform Relative(Transform.Rotation, Transform.Translation, Transform.Scale);
				FTransform ConstraintRelative(Relative);
				UE::TransformConstraintUtil::UpdateTransformBasedOnConstraint(ConstraintRelative, SceneComponentThatChanged);
				if (!ConstraintRelative.Equals(Relative, UE_SMALL_NUMBER))
				{
					Transform.Translation = ConstraintRelative.GetTranslation();
					Transform.Rotation = ConstraintRelative.Rotator();
					Transform.Scale = ConstraintRelative.GetScale3D();
				}
			}
...
{
	FProperty* PropertyAboutToChange = InPropertyChain.GetActiveMemberNode() ? InPropertyChain.GetActiveMemberNode()->GetValue() : nullptr;
...
void F3DTransformTrackEditor::GetTransformKeys( const TOptional<FTransformData>& LastTransform, const FTransformData& CurrentTransform, EMovieSceneTransformChannel ChannelsToKey, UObject* Object, UMovieSceneSection* Section, FGeneratedTrackKeys& OutGeneratedKeys)
{
	...
	bool bHasNeutralRotationSpace = IsValid(AttachParent) || (GetTransformOrigin().GetRotation() == FQuat::Identity && GetRootMotionOffset(Object).Equals(FTransform::Identity));
...
FTransformData F3DTransformTrackEditor::RecomposeTransform(const FTransformData& InTransformData, UObject* AnimatedObject, UMovieSceneSection* Section)
{
	...
	if (ImportedEntityIDs.Num())
	{
		...
		if (System)
		{
			...
			CurrentTransformChannels[0] = CurrentValue.GetTranslation()[0];
			CurrentTransformChannels[1] = CurrentValue.GetTranslation()[1];
			CurrentTransformChannels[2] = CurrentValue.GetTranslation()[2];
			CurrentTransformChannels[3] = CurrentValue.GetRotation().Roll;
			CurrentTransformChannels[4] = CurrentValue.GetRotation().Pitch;
			CurrentTransformChannels[5] = CurrentValue.GetRotation().Yaw;
			CurrentTransformChannels[6] = CurrentValue.GetScale()[0];
			CurrentTransformChannels[7] = CurrentValue.GetScale()[1];
			CurrentTransformChannels[8] = CurrentValue.GetScale()[2];
...
void F3DTransformTrackEditor::HandleOnConstraintAdded(IMovieSceneConstrainedSection* InSection, FMovieSceneConstraintChannel* InConstraintChannel)
{
	...
	if (!FConstraintsManagerController::OnSceneComponentConstrained().IsBound())
	{
		OnSceneComponentConstrainedHandle =
			FConstraintsManagerController::OnSceneComponentConstrained().AddLambda([](USceneComponent* InSceneComponent)
				{
...
void F3DTransformTrackEditor::HandleConstraintRemoved(IMovieSceneConstrainedSection* InSection) 
{
	...
	if (!Section->OnConstraintRemovedHandle.IsValid())
	{
		Section->OnConstraintRemovedHandle =
			FConstraintsManagerController::GetNotifyDelegate().AddLambda([WeakSection = MakeWeakObjectPtr(Section), this](EConstraintsManagerNotifyType InNotifyType, UObject *InObject)
				{
					switch (InNotifyType)
					{
						...
						case EConstraintsManagerNotifyType::ConstraintRemovedWithCompensation:
							{
								...
								UMovieScene3DTransformSection* Section = WeakSection.IsValid() ? WeakSection.Get() : nullptr;
						...
						case EConstraintsManagerNotifyType::ManagerUpdated:
							if (UMovieScene3DTransformSection* Section = WeakSection.IsValid() ? WeakSection.Get() : nullptr)
							{

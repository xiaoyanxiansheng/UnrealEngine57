...
void FStaticMeshEditorViewportClient::PerspectiveCameraMoved()
{
	...
	StaticMesh->SetEditorCameraPosition(FAssetEditorOrbitCameraPosition(
		OrbitPoint,
		OrbitZoom,
		GetViewRotation()
		));
...
void FStaticMeshEditorViewportClient::SetPreviewMesh(UStaticMesh* InStaticMesh, UStaticMeshComponent* InStaticMeshComponent, bool bResetCamera)
{
	...
	if (bResetCamera)
	{
		...
		const USceneThumbnailInfo* const AssetThumbnailInfo = Cast<USceneThumbnailInfo>(StaticMesh->GetThumbnailInfo());
		...
		if (!AssetThumbnailInfo && StaticMesh->GetEditorCameraPosition().bIsSet)
		{
			// The static mesh editor saves the camera position in terms of an orbit camera, so ensure 
			...
			SetViewRotation(StaticMesh->GetEditorCameraPosition().CamOrbitRotation);
			SetViewLocation(StaticMesh->GetEditorCameraPosition().CamOrbitPoint + StaticMesh->GetEditorCameraPosition().CamOrbitZoom);
			SetLookAtLocation(StaticMesh->GetEditorCameraPosition().CamOrbitPoint);

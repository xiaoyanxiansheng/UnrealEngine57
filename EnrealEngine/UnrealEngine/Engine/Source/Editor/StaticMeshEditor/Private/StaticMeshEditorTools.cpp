...
public:
	...
	: StaticMeshEditor(InStaticMeshEditor)
	{
		...
		check(StaticMesh);
		LayoutImpl->UpdateSettings(StaticMesh->GetNaniteSettings());
	}
...
void FStaticMeshDetails::CustomizeDetails( class IDetailLayoutBuilder& DetailBuilder )
{
	...
	TSharedRef<IPropertyHandle> ImportSettings = DetailBuilder.GetProperty(UStaticMesh::GetAssetImportDataMemberName());
	if (!StaticMeshEditor.GetStaticMesh() || 
		!StaticMeshEditor.GetStaticMesh()->GetAssetImportData() ||
		!StaticMeshEditor.GetStaticMesh()->GetAssetImportData()->IsA<UFbxStaticMeshImportData>())
	{
	...
	TSharedRef<IPropertyHandle> NaniteSettingsProperty = DetailBuilder.GetProperty(UStaticMesh::GetNaniteSettingsMemberName());
	...
	TSharedPtr<IPropertyHandle> SupportRayTracingProperty = DetailBuilder.GetProperty(UStaticMesh::GetSupportRayTracingMemberName());

	// Hide the existing RayTracingProxySettings property so we can use the customization instead
	TSharedRef<IPropertyHandle> RayTracingProxySettingsProperty = DetailBuilder.GetProperty(UStaticMesh::GetRayTracingProxySettingsMemberName());
...
void FLevelOfDetailSettingsLayout::AddToDetailsPanel( IDetailLayoutBuilder& DetailBuilder )
{
	...
	int32 LODGroupIndex = LODGroupNames.Find(StaticMesh->GetLODGroup());
	check(LODGroupIndex == INDEX_NONE || LODGroupIndex < LODGroupOptions.Num());

	IDetailPropertyRow& LODGroupRow = LODSettingsCategory.AddProperty(UStaticMesh::GetLODGroupMemberName());
...
bool FLevelOfDetailSettingsLayout::AddLODScreenSizePlatformOverride(FName PlatformGroupName, int32 LODIndex)
{
	...
	{
		if(!StaticMesh->GetAutoComputeLODScreenSize() && StaticMesh->IsSourceModelValid(LODIndex))
		{
			StaticMesh->Modify();
...
void FLevelOfDetailSettingsLayout::OnLODGroupChanged(TSharedPtr<FString> NewValue, ESelectInfo::Type SelectInfo)
{
	...
	if (StaticMesh->GetLODGroup() != NewGroup)
	{
		if (NewGroup != NAME_None)
		{
			...
			else
			{
				...
				int32 Index = LODGroupNames.Find(StaticMesh->GetLODGroup());
...
bool FLevelOfDetailSettingsLayout::IsAutoLODEnabled() const
{
	UStaticMesh* StaticMesh = StaticMeshEditor.GetStaticMesh();
	check(StaticMesh);
	return StaticMesh->GetAutoComputeLODScreenSize();
}
...
void FLevelOfDetailSettingsLayout::OnAutoLODChanged(ECheckBoxState NewState)
{
	...
	check(StaticMesh);
	StaticMesh->Modify();
	StaticMesh->SetAutoComputeLODScreenSize((NewState == ECheckBoxState::Checked) ? true : false);
	if (!StaticMesh->GetAutoComputeLODScreenSize())
	{
...
void FLevelOfDetailSettingsLayout::OnImportLOD(TSharedPtr<FString> NewValue, ESelectInfo::Type SelectInfo)
{
	...
	if( LODNames.Find(NewValue, LODIndex) && LODIndex > 0 )
	{
		...
		if (StaticMesh->GetLODGroup() != NAME_None && StaticMesh->IsSourceModelValid(LODIndex))
		{
			...
			const FStaticMeshLODGroup& LODGroup = LODSettings.GetLODGroup(StaticMesh->GetLODGroup());
			if (LODIndex < LODGroup.GetDefaultNumLODs())
			{
				...
				EAppReturnType::Type ReturnResult = FMessageDialog::Open(EAppMsgType::OkCancel, EAppReturnType::Ok, 
					FText::Format(LOCTEXT("LODImport_LODGroupVersusCustomLODConflict",
										  ...
										  FText::FromName(StaticMesh->GetLODGroup())));
				...
				UFbxStaticMeshImportData* ImportData = Cast<UFbxStaticMeshImportData>(StaticMesh->GetAssetImportData());
...
static bool UpdateStaticMeshNumStreamedLODsHelper(UStaticMesh* StaticMesh, int32 NewValue, FName Platform)
{
	...
	StaticMesh->Modify();
	{
		...
		if (Platform == NAME_None)
		{
			bWasDifferent = StaticMesh->GetNumStreamedLODs().Default != NewValue;
			StaticMesh->GetNumStreamedLODs().Default = NewValue;
		}
		else
		{
			int32* ValuePtr = StaticMesh->GetNumStreamedLODs().PerPlatform.Find(Platform);
			if (ValuePtr != nullptr)
			{
...
int32 FLevelOfDetailSettingsLayout::GetNumStreamedLODs(FName Platform) const
{
	...
	int32* ValuePtr = (Platform == NAME_None) ? nullptr : StaticMesh->GetNumStreamedLODs().PerPlatform.Find(Platform);
	return (ValuePtr != nullptr) ? *ValuePtr : StaticMesh->GetNumStreamedLODs().Default;
}
...
bool FLevelOfDetailSettingsLayout::AddNumStreamedLODsPlatformOverride(FName PlatformGroupName)
{
	...
	check(StaticMesh);
	StaticMesh->Modify();
	if (StaticMesh->GetNumStreamedLODs().PerPlatform.Find(PlatformGroupName) == nullptr)
	{
		int32 Value = StaticMesh->GetNumStreamedLODs().Default;
		StaticMesh->GetNumStreamedLODs().PerPlatform.Add(PlatformGroupName, Value);
...
bool FLevelOfDetailSettingsLayout::RemoveNumStreamedLODsPlatformOverride(FName PlatformGroupName)
{
	...
	check(StaticMesh);
	StaticMesh->Modify();
	if (StaticMesh->GetNumStreamedLODs().PerPlatform.Remove(PlatformGroupName) != 0)
	{
		OnNumStreamedLODsChanged(StaticMesh->GetNumStreamedLODs().Default, PlatformGroupName);
		return true;
	}
	return false;
}
...
TArray<FName> FLevelOfDetailSettingsLayout::GetNumStreamedLODsPlatformOverrideNames() const
{
	...
	check(StaticMesh);
	TArray<FName> KeyArray;
	StaticMesh->GetNumStreamedLODs().PerPlatform.GenerateKeyArray(KeyArray);
	KeyArray.Sort(FNameLexicalLess());
	return KeyArray;
}
...
bool FRayTracingProxySettingsLayout::IsApplyNeeded() const
{
	UStaticMesh* StaticMesh = StaticMeshEditor.GetStaticMesh();
	check(StaticMesh);
	return StaticMesh->GetRayTracingProxySettings() != RayTracingProxySettings;
}
...
void FRayTracingProxySettingsLayout::ApplyChanges()
{
	...
	checkf(!ShouldGenerateRayTracingProxiesByDefault() || StaticMesh->GetRayTracingProxySettings().bEnabled == RayTracingProxySettings.bEnabled,
		TEXT("RayTracingProxySettings.bEnabled shouldn't be modified when ray tracing proxies are enabled by default."));
...
{
	const UStaticMesh* StaticMesh = StaticMeshEditor.GetStaticMesh();
	return StaticMesh && StaticMesh->bSupportRayTracing && StaticMesh->GetNaniteSettings().bEnabled;
}

...
void FStaticMeshEditor::DuplicateSelectedPrims(const FVector* InOffset)
{
	if (SelectedPrims.Num() > 0)
	{
		...
		StaticMesh->SetCustomizedCollision(true);	//mark the static mesh for collision customization
	}
}
...
{
	if ( ensure(StaticMesh->GetAssetImportData()) )
	{
		const FString SourceFilePath = StaticMesh->GetAssetImportData()->GetFirstFilename();
...
void FStaticMeshEditor::NotifyPostChange( const FPropertyChangedEvent& PropertyChangedEvent, FProperty* PropertyThatChanged )
{
	if(StaticMesh && StaticMesh->GetBodySetup())
	{
		...
		if (UStaticMesh::GetLODGroupMemberName() == PropertyChangedEvent.GetPropertyName())
		{
			RefreshTool();
		}

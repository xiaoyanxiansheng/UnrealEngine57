...
void FTurnkeyEditorSupport::LaunchRunningMap(const FString& DeviceId, const FString& DeviceName, const FString& ProjectPath, bool bUseTurnkey, bool bOnSimulator, FStringView Architecture)
{
#if WITH_EDITOR
	...
	if (FTargetDeviceId::Parse(DeviceId, TargetDeviceId))
	{
		...
		if (FModuleManager::LoadModuleChecked<IProjectTargetPlatformEditorModule>("ProjectTargetPlatformEditor").ShowUnsupportedTargetWarning(*TargetDeviceId.GetPlatformName()))
		{
			...
			DeviceInfo.Architecture = Architecture;

			FRequestPlaySessionParams SessionParams;
			SessionParams.SessionDestination = EPlaySessionDestinationType::Launcher;
			SessionParams.LauncherTargetDevice = MoveTemp(DeviceInfo);

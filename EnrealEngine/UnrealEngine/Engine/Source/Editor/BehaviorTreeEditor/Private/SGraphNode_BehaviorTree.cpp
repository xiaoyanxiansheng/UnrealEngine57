...
void SGraphNode_BehaviorTree::GetOverlayBrushes(bool bSelected, const FVector2f& WidgetSize, TArray<FOverlayBrushInfo>& Brushes) const
{
	UBehaviorTreeGraphNode* BTNode = Cast<UBehaviorTreeGraphNode>(GraphNode);
	if (BTNode == NULL)
	{
		...
		if (TriggerOffsets.Num())
		{
			...
			if (IPOverlayInfo.Brush)
			{
				for (int32 i = 0; i < TriggerOffsets.Num(); i++)
				{
					...
					IPOverlayInfo.AnimationEnvelope = FVector2f(10.f, 0.f);
					Brushes.Add(IPOverlayInfo);
				}
			}
		}
...
TArray<FOverlayWidgetInfo> SGraphNode_BehaviorTree::GetOverlayWidgets(bool bSelected, const FVector2f& WidgetSize) const
{
...
void SGraphNode_BehaviorTree::MoveTo(const FVector2f& NewPosition, FNodeSet& NodeFilter, bool bMarkDirty)
{

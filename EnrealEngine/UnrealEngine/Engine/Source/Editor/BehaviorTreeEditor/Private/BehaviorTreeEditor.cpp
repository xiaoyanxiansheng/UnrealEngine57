...
void FBehaviorTreeEditor::BindDebuggerToolbarCommands()
{
	...
	ToolkitCommands->MapAction(
		Commands.OpenParentTree,
		FExecuteAction::CreateSP(DebuggerOb, &FBehaviorTreeDebugger::OpenParentTree),
		FCanExecuteAction::CreateSP(DebuggerOb, &FBehaviorTreeDebugger::CanOpenParentTree));

	ToolkitCommands->MapAction(
		Commands.OpenSubtree,
		FExecuteAction::CreateSP(DebuggerOb, &FBehaviorTreeDebugger::OpenSubtree),
		FCanExecuteAction::CreateSP(DebuggerOb, &FBehaviorTreeDebugger::CanOpenSubtree));
...
void FBehaviorTreeEditor::OnNodeDoubleClicked(class UEdGraphNode* Node)
{
	...
	{
		if (const UBTNode* NodeInstance = Cast<UBTNode>(MyNode->NodeInstance))
		{
			const TOptional<FBehaviorTreeNodeDebugContext> DebugContext = Debugger ? Debugger->GetDebugContextForNode(NodeInstance) : TOptional<FBehaviorTreeNodeDebugContext>{};
			if (const UObject* AssetToOpen = NodeInstance->GetAssociatedAsset(DebugContext))
			{
				GEditor->GetEditorSubsystem<UAssetEditorSubsystem>()->OpenEditorForAsset(AssetToOpen);
				if (const UBehaviorTree* OpenedBehaviorTree = Cast<UBehaviorTree>(AssetToOpen))
				{
					if (IBehaviorTreeEditor* ChildNodeEditor = static_cast<IBehaviorTreeEditor*>(GEditor->GetEditorSubsystem<UAssetEditorSubsystem>()->FindEditorForAsset(const_cast<UBehaviorTree*>(OpenedBehaviorTree), true)))
					{
						ChildNodeEditor->InitializeDebuggerState(Debugger.Get());
					}
				}

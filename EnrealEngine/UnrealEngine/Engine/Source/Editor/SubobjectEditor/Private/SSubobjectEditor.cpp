...
static const FName SubobjectEditor_ClearFocusOnDuplicate("SubobjectEditor_ClearFocusOnDuplicate");
...
{
	FSubobjectData* Data = GetDataSource();
	return Data ? Data->IsComponent() : false;
}
...
TSharedRef<SWidget> SSubobject_RowWidget::GenerateWidgetForColumn(const FName& ColumnName)
{
	...
	if (ColumnName == SubobjectTree_ColumnName_ComponentClass)
	{
		...
		InlineWidget->AddMetadata<FTagMetaData>(MakeShared<FTagMetaData>(SubobjectEditor_ClearFocusOnDuplicate));
...
const FSlateBrush* SSubobject_RowWidget::GetIconBrush() const
{
	...
	if (FSubobjectEditorTreeNodePtrType NodePtr = GetSubobjectPtr())
	{
		...
		{
			TSharedPtr<SSubobjectEditor> SubobjectEditorPinned = SubobjectEditor.Pin();
			const ISCSEditorUICustomization* UICustomization = SubobjectEditorPinned ? SubobjectEditorPinned->GetUICustomization() : nullptr;
			if (UICustomization)
			{
				const FSlateBrush* SubobjectIcon = UICustomization->GetIconBrush(*Data);
				if (SubobjectIcon)
				{
...
TSharedRef<SWidget> SSubobject_RowWidget::GetInheritedLinkWidget()
{
	...
	if (TSharedPtr<SSubobjectEditor> PinnedSubobjectEditor = SubobjectEditor.Pin())
	{
		if (const ISCSEditorUICustomization* UICustomization = PinnedSubobjectEditor->GetUICustomization())
		{
			if (TSharedPtr<SWidget> ControlsWidget = UICustomization->GetControlsWidget(*Data))
			{
...
void SSubobjectEditor::UpdateTree(bool bRegenerateTreeNodes /* = true */)
{
	...
	if (bRegenerateTreeNodes)
	{
		...
		if (UObject* Context = GetObjectContext())
		{
			...
			if (UICustomization)
			{
				UICustomization->SortSubobjectData(SubobjectData);
			}
...
const ISCSEditorUICustomization* SSubobjectEditor::GetUICustomization() const
{
	return UICustomization.Get();
}
...
bool SSubobjectEditor::ShouldClearFocusOnDuplicate(TSharedPtr<SWidget> InCurrentlyFocusedWidget)
{
	TSharedPtr<SWidget> CurrentWidget = InCurrentlyFocusedWidget;
	while (CurrentWidget)
	{
		for (const TSharedRef<FTagMetaData>& TagMetaData : CurrentWidget->GetAllMetaData<FTagMetaData>())
		{
			if (TagMetaData->Tag == SubobjectEditor_ClearFocusOnDuplicate)
			{
				return true;
			}
		}
...
FSubobjectDataHandle SSubobjectEditor::GetObjectContextHandle() const
{
	if (RootNodes.Num() > 0)
	{
		if (RootNodes[0]->GetObject() == GetObjectContext())
		{

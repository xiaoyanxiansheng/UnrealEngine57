...
void SSubobjectBlueprintEditor::Construct(const FArguments& InArgs)
{
	...
	UICustomization = InArgs._SCSEditorUICustomization;
...
{
	const TArray<FSubobjectDataHandle> SelectedNodes = GetSelectedHandles();
	...
	bool ClearedFocus = false;

	// Check if the focus is currently in some sort of textbox
	if (const TSharedPtr<SWidget> FocusedWidget = FSlateApplication::Get().GetKeyboardFocusedWidget())
	{
		if (ShouldClearFocusOnDuplicate(FocusedWidget))
		{
	...
	// If nothing was duplicated, no further action is needed
	if (DuplicatedHandles.Num() <= 0)
	{
		return;
	}
	...
	FSubobjectEditorTreeNodePtrType FirstNode = nullptr;

	// Select all duplicated subobjects
	for (const FSubobjectDataHandle& DataHandle : DuplicatedHandles)
	{
		if (const FSubobjectEditorTreeNodePtrType NewNode = FindSlateNodeForHandle(DataHandle))
		{
			if (!FirstNode)
			{
				FirstNode = NewNode;
	...
	// If only one subobject was duplicated, start a rename operation on it
	if (DuplicatedHandles.Num() == 1)
	{
		OnRenameComponent(MoveTemp(Transaction));
	}
	// Otherwise, scroll the first of the duplicated nodes into view in the tree widget
	else if (FirstNode)
	{
		TreeWidget->RequestScrollIntoView(FirstNode);
		if (ClearedFocus)
		{
			// If focus had to be cleared, put it back on the tree widget so further interactions can be made with the new selection
			FSlateApplication::Get().SetKeyboardFocus(TreeWidget, EFocusCause::SetDirectly);
		}
	}
}
...
void SSubobjectBlueprintEditor::PopulateContextMenuImpl(UToolMenu* InMenu, TArray<FSubobjectEditorTreeNodePtrType>& InSelectedItems, bool bIsChildActorSubtreeNodeSelected)
{
	...
	if (IsEditingAllowed())
	{
		...
		if (SelectionClasses.Num())
		{
			...
			UBlueprint* Blueprint = GetBlueprint();
			if (FBlueprintEditorUtils::CanClassGenerateEvents(SelectedClass) && Blueprint != nullptr)
			{
				BlueprintSCSSection.AddSubMenu(
					"AddEventSubMenu",
					...
					FNewMenuDelegate::CreateStatic(&SSubobjectBlueprintEditor::BuildMenuEventsSection,
						Blueprint,
						SelectedClass,
						FCanExecuteAction::CreateSP(this, &SSubobjectEditor::IsEditingAllowed),

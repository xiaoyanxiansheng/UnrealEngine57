	...
	bAddedViewportWorldInteractionExtension( false )
{
	...
	FEditorDelegates::OnEditorPreExit.AddRaw(this, &FVREditorModeManager::HandleEditorPreExit);
}
...
void FVREditorModeManager::HandleEditorPreExit()
{
	if (IsVREditorActive())
	{
		const bool bEnable_false = false;
		const bool bForceWithoutHMD_false = false;
		EnableVREditor(bEnable_false, bForceWithoutHMD_false);
	}
}

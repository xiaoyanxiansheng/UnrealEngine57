...
class UE_DEPRECATED(5.7, "VR Editor mode is deprecated; use the XR Creative Framework plugin instead.") AVREditorTeleporter : public AActor
{
	GENERATED_BODY()
...
private:
	...
	UFUNCTION( BlueprintCallable, Category = "Teleporter" )
	UE_API void StartAiming( class UViewportInteractor* Interactor );

	/** Cancel teleport aiming mode without doing the teleport */
	UFUNCTION( BlueprintCallable, Category = "Teleporter" )
	UE_API void StopAiming();

	/** Do and finalize teleport.  	*/
	UFUNCTION( BlueprintCallable, Category = "Teleporter" )
	UE_API void DoTeleport();

	/** Get the actor we're currently trying to teleport with.
	* Valid during aiming and teleporting.
	*/
	UFUNCTION(BlueprintCallable, Category = "Teleporter")
	UE_API UViewportInteractor* GetInteractorTryingTeleport() const;

	/** Called when the user presses a button on their motion controller device */
	UE_API void OnPreviewInputAction(class FEditorViewportClient& ViewportClient, UViewportInteractor* Interactor,
	...
	UE_API void Teleport(const float DeltaTime);

	/** Updating aiming with teleport to end of laser and pulling and pushing */
	UE_API void UpdateTeleportAim(const float DeltaTime);

	/** Helper function to push and pull the teleporter along the laser */
	UE_API FVector UpdatePushPullTeleporter(class UVREditorInteractor* VREditorInteractor, const FVector& LaserPointerStart, const FVector& LaserPointerEnd, const bool bEnablePushPull = true);
	...
	UE_API void UpdateVisuals(const FVector& NewLocation);

	/** If we want to start showing or hiding the meshes */
	UE_API void Show(const bool bShow);

	UE_API void UpdateFadingState(const float DeltaTime);

	/** Calculated the scale for animation */
	UE_API float CalculateAnimatedScaleFactor() const;

...
public:
	...
	static KISMET_API FText RightRevision;
	static KISMET_API TSharedRef<SWidget> GenerateSimpleDiffWidget(FText DiffText);
	static KISMET_API TSharedRef<SWidget> GenerateObjectDiffWidget(FSingleObjectDiffEntry DiffEntry, FText ObjectName);

	// to support comment posting, set this to the tree view that contains the comments.
	KISMET_API void EnableComments(TWeakPtr<STreeView<TSharedPtr<FBlueprintDifferenceTreeEntry>>> TreeView, const UObject* OldObject, const UObject* NewObject);
	KISMET_API void EnableComments(TWeakPtr<STreeView<TSharedPtr<FBlueprintDifferenceTreeEntry>>> TreeView, const TArray<const UObject*>& Objects);
...
protected:
	...
	KISMET_API void GenerateCategoryCommentTreeEntries(TArray< TSharedPtr<FBlueprintDifferenceTreeEntry> >& , TArray< TSharedPtr<FBlueprintDifferenceTreeEntry> >& OutRealDifferences, FString CategoryKey);
...
{
public:
	KISMET_API FDetailsDiffControl(const UObject* InOldObject, const UObject* InNewObject, FOnDiffEntryFocused InSelectionCallback, bool bPopulateOutTreeEntries);
	KISMET_API ~FDetailsDiffControl();

	KISMET_API virtual void Tick() override;
	
	KISMET_API virtual void GenerateTreeEntries(TArray< TSharedPtr<FBlueprintDifferenceTreeEntry> >& OutTreeEntries, TArray< TSharedPtr<FBlueprintDifferenceTreeEntry> >& OutRealDifferences) override;
	KISMET_API void GenerateTreeEntriesWithoutComments(TArray< TSharedPtr<FBlueprintDifferenceTreeEntry> >& OutTreeEntries, TArray< TSharedPtr<FBlueprintDifferenceTreeEntry> >& OutRealDifferences);

	KISMET_API TSharedRef<IDetailsView> InsertObject(const UObject* Object, bool bScrollbarOnLeft = false, int32 Index = INDEX_NONE);
	KISMET_API TSharedRef<IDetailsView> GetDetailsWidget(const UObject* Object) const;
	KISMET_API TSharedPtr<IDetailsView> TryGetDetailsWidget(const UObject* Object) const;
	KISMET_API TSharedPtr<FAsyncDetailViewDiff> GetDifferencesWithLeft(const UObject* Object) const;
	KISMET_API TSharedPtr<FAsyncDetailViewDiff> GetDifferencesWithRight(const UObject* Object) const;
	KISMET_API int32 IndexOfObject(const UObject* Object) const;

	// to support comment posting, set this to the tree view that contains the comments.
	KISMET_API virtual void EnableComments(TWeakPtr<STreeView<TSharedPtr<FBlueprintDifferenceTreeEntry>>> TreeView) override;
...
protected:
	KISMET_API virtual void OnSelectDiffEntry(FPropertySoftPath PropertyName);
	KISMET_API TAttribute<TArray<FVector2f>> GetLinkedScrollRateAttribute(const TSharedRef<IDetailsView>& OldDetailsView, const TSharedRef<IDetailsView>& NewDetailsView);
	...
	KISMET_API TArray<FVector2f> GetLinkedScrollRate(TSharedRef<IDetailsView> LeftDetailsView, TSharedRef<IDetailsView> RightDetailsView) const;
private:
	KISMET_API void HandlePropertiesChanged();
...
struct FBlueprintTypeDiffControl : public TSharedFromThis<FBlueprintTypeDiffControl>, public IDiffControl
{
	...
	struct FDiffCustomObject
	{
		FDiffSingleResult SourceResult;
		TSharedPtr<IDiffCustomObject> DiffWidget;
		TArray<TSharedPtr<FBlueprintDifferenceTreeEntry>> Diffs;
		FDiffCustomObject(const FDiffSingleResult& InSourceResult, TSharedPtr<IDiffCustomObject> DiffWidget)
			: SourceResult(InSourceResult)
...
private:
	...
	/** Handle selecting a custom object diff */
	void OnSelectDiffCustomObject(TSharedPtr<IDiffCustomProperty> CustomProperty, TSharedPtr<FDiffCustomObject> DiffCustomObject);

	/** List of objects with differences */
	TArray<TSharedPtr<FSubObjectDiff>> SubObjectDiffs;
	
	/** List of custom object differences */
	TArray<TSharedPtr<FDiffCustomObject>> DiffCustomObjects;

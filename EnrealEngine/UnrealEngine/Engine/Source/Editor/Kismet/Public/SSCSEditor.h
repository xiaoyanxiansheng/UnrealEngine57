...
public:
	...
	UE_API TSharedRef<ITableRow> MakeTableRowWidget(FSCSEditorTreeNodePtrType InNodePtr, const TSharedRef<STableViewBase>& OwnerTable);

	/** Used by tree control - get children for a specified node */
	UE_API void OnGetChildrenForTree(FSCSEditorTreeNodePtrType InNodePtr, TArray<FSCSEditorTreeNodePtrType>& OutChildren);

	/** Returns true if editing is allowed */
	UE_API bool IsEditingAllowed() const;

	/** gets the actor root */
	UE_API FSCSEditorActorNodePtrType GetActorNode() const;
	 ...
	 */
	UE_API UActorComponent* AddNewComponent(UClass* NewComponentClass, UObject* Asset, const FAddNewComponentParams Params = FAddNewComponentParams());
	...
	UE_API void AddNewNode(FAddedNodeDetails& OutNodeDetails, TUniquePtr<FScopedTransaction> OngoingCreateTransaction, USCS_Node* NewNode, UObject* Asset, bool bMarkBlueprintModified, bool bSetFocusToNewItem = true);
	...
	UE_API UActorComponent* AddNewNode(TUniquePtr<FScopedTransaction> OngoingCreateTransaction, USCS_Node* NewNode, UObject* Asset, bool bMarkBlueprintModified, bool bSetFocusToNewItem = true);
	...
	/** Adds a new component instance node to the component Table
		...
		@param Asset (In) Optional asset to assign to the component
		@param bSetFocusToNewItem (In) Select the new item and activate the inline rename widget (default is true) */
	UE_API void AddNewNodeForInstancedComponent(TUniquePtr<FScopedTransaction> OngoingCreateTransaction, UActorComponent* NewInstanceComponent, FSCSEditorTreeNodePtrType InParentNodePtr, UObject* Asset, bool bSetFocusToNewItem = true);
	...
	UE_API void OnDuplicateComponent();

	/** Removes existing selected component nodes from the SCS */
	UE_API void OnDeleteNodes();
	...
	UE_API void RemoveComponentNode(FSCSEditorTreeNodePtrType InNodePtr);

	/** Called when selection in the tree changes */
	UE_API void OnTreeSelectionChanged(FSCSEditorTreeNodePtrType InSelectedNodePtr, ESelectInfo::Type SelectInfo);

	/** Called when the Actor is selected. */
	UE_API void OnActorSelected(const ECheckBoxState NewCheckedState);

	/** Called to determine if actor is selected. */
	UE_API ECheckBoxState OnIsActorSelected() const;

	/** Update any associated selection (e.g. details view) from the passed in nodes */
	UE_API void UpdateSelectionFromNodes(const TArray<FSCSEditorTreeNodePtrType> &SelectedNodes );
	...
	UE_API TArray<FSCSEditorTreeNodePtrType> GetSelectedNodes() const;
	 ...
	 */
	static UE_API void BuildMenuEventsSection( FMenuBuilder& Menu, UBlueprint* Blueprint, UClass* SelectedClass, FCanExecuteAction CanExecuteActionDelegate, FGetSelectedObjectsDelegate GetSelectedObjectsDelegate );
	 ...
	 */
	UE_API FSCSEditorTreeNodePtrType GetNodeFromActorComponent(const UActorComponent* ActorComponent, bool bIncludeAttachedComponents = true) const;

	/** Select the root of the tree */
	UE_API void SelectRoot();

	/** Select the given tree node */
	UE_API void SelectNode(FSCSEditorTreeNodePtrType InNodeToSelect, bool IsCntrlDown);
	 ...
	 */
	UE_API void SetNodeExpansionState(FSCSEditorTreeNodePtrType InNodeToChange, const bool bIsExpanded);
	 ...
	 */
	UE_API void HighlightTreeNode( FName TreeNodeName, const class FPropertyPath& Property );
	/**
	 * Highlight a tree node and, optionally, a property with in it
	 *
	 ...
	 * @param Property	The name of the property to be highlighted in the details view
	 */
	UE_API void HighlightTreeNode( const USCS_Node* Node, FName Property );
	 ...
	 */
	static UE_API void SaveSCSCurrentState( USimpleConstructionScript* SCSObj );
	 ...
	 */
	static UE_API void SaveSCSNode(USCS_Node* Node);

	/** Is this node still used by the Simple Construction Script */
	UE_API bool IsNodeInSimpleConstructionScript(USCS_Node* Node) const;
	...
	UE_API TArray<UObject*> GetSelectedEditableObjects() const;

	/** Provides access to the Blueprint context that's being edited */
	UE_API class UBlueprint* GetBlueprint() const;
	...
	UE_API void SetItemExpansionRecursive(FSCSEditorTreeNodePtrType Model, bool bInExpansionState);
	...
	UE_API TSharedPtr<SWidget> GetToolButtonsBox();
...
protected:
	UE_API FSCSEditorTreeNodePtrType FindOrCreateParentForExistingComponent(UActorComponent* InActorComponent, FSCSEditorActorNodePtrType ActorRootNode);
	UE_API FSCSEditorTreeNodePtrType FindParentForNewComponent(UActorComponent* NewComponent) const;
	UE_API FSCSEditorTreeNodePtrType FindParentForNewNode(USCS_Node* NewNode) const;

	/** Add a component from the selection in the combo box */
	UE_API UActorComponent* PerformComboAddClass(TSubclassOf<UActorComponent> ComponentClass, EComponentCreateAction::Type ComponentCreateAction, UObject* AssetOverride);
	...
	UE_API void OnLevelComponentRequestRename(const UActorComponent* InComponent);
	 ...
	 */
	UE_API void OnRenameComponent(TUniquePtr<FScopedTransaction> OngoingCreateTransaction);
	...
	UE_API void ReplaceComponentReferencesInTree(FSCSEditorActorNodePtrType InActorNode, const TMap<UObject*, UObject*>& OldToNewInstanceMap, bool& OutHasChanges);

	/** Update component pointers held by tree nodes if components have been replaced following construction script execution */
	UE_API void ReplaceComponentReferencesInTree(const TArray<FSCSEditorTreeNodePtrType>& Nodes, const TMap<UObject*, UObject*>& OldToNewInstanceMap, bool& OutHasChanges);
	 ...
	 */
	static UE_API void CreateEventsForSelection(UBlueprint* Blueprint, FName EventName, FGetSelectedObjectsDelegate GetSelectedObjectsDelegate);
	 ...
	 */
	static UE_API void ConstructEvent(UBlueprint* Blueprint, const FName EventName, const FComponentEventConstructionData EventData);
	 ...
	 * @param EventData						the event data structure describing the node
	 */
	static UE_API void ViewEvent(UBlueprint* Blueprint, const FName EventName, const FComponentEventConstructionData EventData);

	/** Helper method to add a tree node for the given SCS node */
	UE_API FSCSEditorTreeNodePtrType AddTreeNode(USCS_Node* InSCSNode, FSCSEditorTreeNodePtrType InParentNodePtr, const bool bIsInheritedSCS);

	/** Helper method to add a tree node for the given actor component */
	UE_API FSCSEditorTreeNodePtrType AddTreeNodeFromComponent(UActorComponent* InSceneComponent, FSCSEditorTreeNodePtrType InParentTreeNode = FSCSEditorTreeNodePtrType());
	
	/** Helper method to add a tree node for the given node's child actor node, if present */
	UE_API FSCSEditorTreeNodePtrType AddTreeNodeFromChildActor(FSCSEditorTreeNodePtrType InNodePtr);
	
	/** Helper method to recursively find a tree node for the given SCS node starting at the given tree node */
	UE_API FSCSEditorTreeNodePtrType FindTreeNode(const USCS_Node* InSCSNode, FSCSEditorTreeNodePtrType InStartNodePtr = FSCSEditorTreeNodePtrType()) const;

	/** Helper method to recursively find a tree node for the given scene component starting at the given tree node */
	UE_API FSCSEditorTreeNodePtrType FindTreeNode(const UActorComponent* InComponent, FSCSEditorTreeNodePtrType InStartNodePtr = FSCSEditorTreeNodePtrType()) const;

	/** Helper method to recursively find a tree node for the given variable or instance name starting at the given tree node */
	UE_API FSCSEditorTreeNodePtrType FindTreeNode(const FName& InVariableOrInstanceName, FSCSEditorTreeNodePtrType InStartNodePtr = FSCSEditorTreeNodePtrType()) const;

	/** Callback when a component item is scrolled into view */
	UE_API void OnItemScrolledIntoView( FSCSEditorTreeNodePtrType InItem, const TSharedPtr<ITableRow>& InWidget);

	/** Callback when a component item is double clicked. */
	UE_API void HandleItemDoubleClicked(FSCSEditorTreeNodePtrType InItem);

	/** Recursively visits the given node + its children and invokes the given function for each. */
	UE_API void DepthFirstTraversal(const FSCSEditorTreeNodePtrType& InNodePtr, TSet<FSCSEditorTreeNodePtrType>& OutVisitedNodes, const TFunctionRef<void(const FSCSEditorTreeNodePtrType&)> InFunction) const;

	/** Returns the set of expandable nodes that are currently collapsed in the UI */
	UE_API void GetCollapsedNodes(const FSCSEditorTreeNodePtrType& InNodePtr, TSet<FSCSEditorTreeNodePtrType>& OutCollapsedNodes) const;
	...
	UE_API EVisibility GetPromoteToBlueprintButtonVisibility() const;

	/** @return The visibility of the Edit Blueprint button (only visible with an actor instance that is created from a blueprint)*/
	UE_API EVisibility GetEditBlueprintButtonVisibility() const;

	/** @return The visibility of the Add Component combo button */
	UE_API EVisibility GetComponentClassComboButtonVisibility() const;
	...
	UE_API const TArray<FSCSEditorTreeNodePtrType>& GetRootNodes() const;
	 ...
	 */
	UE_API UClass* CreateNewCPPComponent(TSubclassOf<UActorComponent> ComponentClass);
	 ...
	 * @return The new class that was created
	 */
	UE_API UClass* CreateNewBPComponent(TSubclassOf<UActorComponent> ComponentClass);
	 ...
	 */
	UE_API bool RefreshFilteredState(FSCSEditorTreeNodePtrType TreeNode, bool bRecursive);

	/** Helper method to construct the subtree for the given actor (root) node. */
	UE_API void BuildSubTreeForActorNode(FSCSEditorActorNodePtrType InActorNode);
...
public:
	...
	UE_API class AActor* GetActorContext() const;

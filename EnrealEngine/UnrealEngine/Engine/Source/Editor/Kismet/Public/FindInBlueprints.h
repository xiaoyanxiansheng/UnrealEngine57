...
public:
	...
	UE_API void Construct(const FArguments& InArgs, TSharedPtr<class FBlueprintEditor> InBlueprintEditor = nullptr);
	UE_API ~SFindInBlueprints();
	 ...
	 */
	UE_API void MakeSearchQuery(FString InSearchString, bool bInIsFindWithinBlueprint, const FStreamSearchOptions& InSearchOptions = FStreamSearchOptions(), FOnSearchComplete InOnSearchComplete = FOnSearchComplete());

	/** Called when caching Blueprints is started */
	UE_API void OnCacheStarted(EFiBCacheOpType InOpType, EFiBCacheOpFlags InOpFlags);
	
	/** Called when caching Blueprints is complete */
	UE_API void OnCacheComplete(EFiBCacheOpType InOpType, EFiBCacheOpFlags InOpFlags);
	...
	UE_API bool IsSearchInProgress() const;
	...
	UE_API void ClearResults();

private:
	/** Processes results of the ongoing async stream search */
	UE_API EActiveTimerReturnType UpdateSearchResults( double InCurrentTime, float InDeltaTime );
	...
	UE_API void OnGetChildren( FSearchResult InItem, TArray< FSearchResult >& OutChildren );

	/* Called when user double clicks on a new result */
	UE_API void OnTreeSelectionDoubleClicked( FSearchResult Item );

	/* Called when a new row is being generated */
	UE_API TSharedRef<ITableRow> OnGenerateRow(FSearchResult InItem, const TSharedRef<STableViewBase>& OwnerTable);
	
	/** Launches a thread for streaming more content into the results widget */
	UE_API void LaunchStreamThread(const FString& InSearchValue, const FStreamSearchOptions& InSearchOptions = FStreamSearchOptions(), FOnSearchComplete InOnSearchComplete = FOnSearchComplete());

	/** Returns the percent complete on the search for the progress bar */
	UE_API TOptional<float> GetPercentCompleteSearch() const;

	/** Returns the search bar visiblity for the given widget */
	UE_API EVisibility GetSearchBarWidgetVisiblity(EFiBSearchBarWidget InSearchBarWidget) const;

	/** Adds the "cache" bar at the bottom of the Find-in-Blueprints widget, to notify the user that the search is incomplete */
	UE_API void ConditionallyAddCacheBar();

	/** Callback to remove the "cache" bar when a button is pressed */
	UE_API FReply OnRemoveCacheBar();

	/** Callback to return the cache bar's display text, informing the user of the situation */
	UE_API FText GetCacheBarStatusText() const;

	/** Callback to return the current asset name during a cache operation */
	UE_API FText GetCacheBarCurrentAssetName() const;

	/** Whether user is allowed to initiate loading and indexing all blueprints with out-of-date metadata */
	UE_API bool CanCacheAllUnindexedBlueprints() const;

	/** Callback to cache all unindexed Blueprints */
	UE_API FReply OnCacheAllUnindexedBlueprints();

	/** Callback to export a list of all blueprints that need reindexing */
	UE_API FReply OnExportUnindexedAssetList();

	/** Callback to cache all Blueprints according to the given options */
	UE_API FReply OnCacheAllBlueprints(const FFindInBlueprintCachingOptions& InOptions);

	/** Callback to cancel the caching process */
	UE_API FReply OnCancelCacheAll();
	...
	UE_API TOptional<float> GetPercentCompleteCache() const;
	...
	UE_API EVisibility GetCacheBarVisibility() const;

	/** Returns the cache bar visibility for the given widget */
	UE_API EVisibility GetCacheBarWidgetVisibility(EFiBCacheBarWidget InCacheBarWidget) const;
	...
	UE_API const FSlateBrush* GetCacheBarImage() const;
	...
	UE_API void SelectAllItemsHelper(FSearchResult InItemToSelect);

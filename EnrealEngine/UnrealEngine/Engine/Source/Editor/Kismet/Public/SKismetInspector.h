...
class SKismetInspector : public SCompoundWidget, public FGCObject
{
public:
	...
	UE_API void EnableComponentDetailsCustomization(bool bEnable);
	...
	UE_API bool IsSelected(UObject* Object) const;
...
protected:
	/** Update the inspector window to show information on the supplied objects */
	UE_API void UpdateFromObjects(const TArray<UObject*>& PropertyObjects, struct FKismetSelectionInfo& SelectionInfo, const FShowDetailsOptions& Options);
...
protected:
	...
	UE_API EVisibility GetInheritedBlueprintComponentWarningVisibility() const;

	/** Opens the parent blueprint when the hyperlink in the warning is clicked */
	UE_API void OnInheritedBlueprintComponentWarningHyperlinkClicked(const FSlateHyperlinkRun::FMetadata& Metadata);
	 ...
	 */
	UE_API TSharedRef<SWidget> MakeContextualEditingWidget(struct FKismetSelectionInfo& SelectionInfo, const FShowDetailsOptions& Options);
	...
	UE_API void ImportNamespacesForPropertyValue(const FProperty* InProperty) const;
};

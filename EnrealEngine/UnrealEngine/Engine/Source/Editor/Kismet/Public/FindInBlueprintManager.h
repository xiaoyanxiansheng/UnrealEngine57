 ...
 */
struct FFindInBlueprintSearchTags
{
	/** Properties tag, for Blueprint variables */
	static UE_API const FText FiB_Properties;

	/** Components tags */
	static UE_API const FText FiB_Components;
	static UE_API const FText FiB_IsSCSComponent;
	/** End Components tags */

	/** Nodes tag */
	static UE_API const FText FiB_Nodes;

	/** Schema Name tag, to identify the schema that a graph uses */
	static UE_API const FText FiB_SchemaName;
	/** Uber graphs tag */
	static UE_API const FText FiB_UberGraphs;
	/** Function graph tag */
	static UE_API const FText FiB_Functions;
	/** Macro graph tag */
	static UE_API const FText FiB_Macros;
	/** Sub graph tag, for any sub-graphs in a Blueprint */
	static UE_API const FText FiB_SubGraphs;
	/** Extension graph tag, for any graphs in a Blueprint Extension */
	static UE_API const FText FiB_ExtensionGraphs;
	/** Blueprint extension tag. */
	static UE_API const FText FiB_Extensions;

	/** Name tag */
	static UE_API const FText FiB_Name;
	/** Native Name tag */
	static UE_API const FText FiB_NativeName;
	/** Class Name tag */
	static UE_API const FText FiB_ClassName;
	/** NodeGuid tag */
	static UE_API const FText FiB_NodeGuid;
	/** Default value */
	static UE_API const FText FiB_DefaultValue;
	/** Tooltip tag */
	static UE_API const FText FiB_Tooltip;
	/** Description tag */
	static UE_API const FText FiB_Description;
	/** Comment tag */
	static UE_API const FText FiB_Comment;
	/** Path tag */
	static UE_API const FText FiB_Path;
	/** Parent Class tag */
	static UE_API const FText FiB_ParentClass;
	/** Interfaces tag */
	static UE_API const FText FiB_Interfaces;
	/** Class that originally defined the function tag */
	static UE_API const FText FiB_FuncOriginClass;

	/** Pin type tags */

	/** Pins tag */
	static UE_API const FText FiB_Pins;
	...
	/** Pin IsReference tag */
	static UE_API const FText FiB_IsReference;
	/** Glyph icon tag */
	...
	static UE_API const FText FiBMetaDataTag;
...
struct FFiBMD
{
	static UE_API const FString FiBSearchableMD;
	static UE_API const FString FiBSearchableShallowMD;
	static UE_API const FString FiBSearchableExplicitMD;
	static UE_API const FString FiBSearchableHiddenExplicitMD;
	static UE_API const FString FiBSearchableFormatVersionMD;
};
...
public:
	...
	/* Create a root */
	UE_API explicit FFindInBlueprintsResult(const FText& InDisplayText);
	...
	*/
	...
	UE_API virtual UObject* GetObject(UBlueprint* InBlueprint) const;
...
public:
	/** Constructor */
	UE_API FStreamSearch(const FString& InSearchValue, const FStreamSearchOptions& InSearchOptions = FStreamSearchOptions());
	 ...
	 */
	UE_API void GetFilteredItems(TArray<TSharedPtr<class FFindInBlueprintsResult>>& OutItemsFound);

	/** Helper function to query the percent complete this search is */
	UE_API float GetPercentComplete() const;
	...
	UE_API void GetFilteredImaginaryResults(TArray<FImaginaryFiBDataSharedPtr>& OutFilteredImaginaryResults);
...
{
public:
	static UE_API FFindInBlueprintSearchManager* Instance;
	static UE_API FFindInBlueprintSearchManager& Get();

	UE_API FFindInBlueprintSearchManager();
	UE_API ~FFindInBlueprintSearchManager();
	 ...
	 */
	UE_API void ApplySearchDataToDatabase(FSearchData InSearchData, bool bAllowNewEntry = false);
	 ...
	 */
	UE_API FSearchData GetSearchDataForAssetPath(const FSoftObjectPath& InAssetPath);
	 ...
	 */
	UE_API void AddOrUpdateBlueprintSearchMetadata(UBlueprint* InBlueprint, EAddOrUpdateBlueprintSearchMetadataFlags InFlags = EAddOrUpdateBlueprintSearchMetadataFlags::None, EFiBVersion InVersion = EFiBVersion::FIB_VER_NONE);
	 ...
	 */
	UE_API void BeginSearchQuery(const class FStreamSearch* InSearchOriginator);
	 ...
	 */
	UE_API bool ContinueSearchQuery(const class FStreamSearch* InSearchOriginator, FSearchData& OutSearchData);
	 ...
	 */
	UE_API void EnsureSearchQueryEnds(const class FStreamSearch* InSearchOriginator);
	 ...
	 */
	UE_API float GetPercentComplete(const class FStreamSearch* InSearchOriginator) const;
	 ...
	 */
	UE_API FSearchData QuerySingleBlueprint(UBlueprint* InBlueprint, bool bInRebuildSearchData);
	 ...
	 */
	UE_API void CacheAllAssets(TWeakPtr< class SFindInBlueprints > InSourceWidget, const FFindInBlueprintCachingOptions& InCachingOptions);

	/**
	 * Exports a list of all unindexed assets to Saved/FindInBlueprints_OutdatedAssetList.txt
	 */
	UE_API void ExportOutdatedAssetList();
	 ...
	 */
	UE_API void OnCacheAllUnindexedAssets(bool bInSourceControlActive, bool bInCheckoutAndSave);

	/** Stops the caching process where it currently is at, the rest can be continued later */
	UE_API void CancelCacheAll(SFindInBlueprints* InFindInBlueprintWidget);

	/** Returns the current index in the caching */
	UE_API int32 GetCurrentCacheIndex() const;

	/** Returns the path of the current Blueprint being cached */
	UE_API FSoftObjectPath GetCurrentCacheBlueprintPath() const;

	/** Returns the progress complete on the caching */
	UE_API float GetCacheProgress() const;
	...
	UE_API void StartedCachingBlueprints(EFiBCacheOpType InCacheOpType, EFiBCacheOpFlags InCacheOpFlags);
	 ...
	 */
	UE_API void FinishedCachingBlueprints(EFiBCacheOpType InCacheOpType, EFiBCacheOpFlags InCacheOpFlags, int32 InNumberCached, TSet<FSoftObjectPath>& InFailedToCacheList);

	/** Returns TRUE if Blueprints are being cached. */
	UE_API bool IsCacheInProgress() const;

	/** Returns TRUE if unindexed Blueprints are being cached (since this can block the UI) */
	UE_API bool IsUnindexedCacheInProgress() const;

	/** Returns TRUE if we're still inside the initial asset discovery and registration stage */
	UE_API bool IsAssetDiscoveryInProgress() const;

	/** Returns TRUE if there are one or more active asynchronous search queries */
	UE_API bool IsAsyncSearchQueryInProgress() const;
	...
	UE_API TSharedPtr<SFindInBlueprints> GetGlobalFindResults();

	/** Enable or disable the global find results tab feature */
	UE_API void EnableGlobalFindResults(bool bEnable);

	/** Close any orphaned global find results tabs for a particular tab manager */
	UE_API void CloseOrphanedGlobalFindResultsTabs(TSharedPtr<class FTabManager> TabManager);
...
private:
	...
	UE_API void PauseFindInBlueprintSearch();

	/** Callback hook during post-garbage collection, saves the cache to disk and unpauses all processing searches */
	UE_API void UnpauseFindInBlueprintSearch();

	/** Callback hook from the Asset Registry when an asset is added */
	UE_API void OnAssetAdded(const struct FAssetData& InAssetData);

	/** Callback hook from the Asset Registry, marks the asset for deletion from the cache */
	UE_API void OnAssetRemoved(const struct FAssetData& InAssetData);

	/** Callback hook from the Asset Registry, marks the asset for deletion from the cache */
	UE_API void OnAssetRenamed(const struct FAssetData& InAssetData, const FString& InOldName);
	...
	UE_API void OnAssetLoaded(class UObject* InAsset);

	/** Callback from Kismet when a Blueprint is unloaded */
	UE_API void OnBlueprintUnloaded(class UBlueprint* InBlueprint);
	...
	UE_API FSearchData GetSearchDataForIndex(int32 CacheIndex);
	 ...
	 */
	UE_API int32 AddSearchDataToDatabase(FSearchData InSearchData);

	/** Removes a Blueprint from being managed by the FiB system by passing in the UBlueprint's path */
	UE_API void RemoveBlueprintByPath(const FSoftObjectPath& InPath);

	/** Adds a new search database entry for unloaded asset data */
	UE_API void AddUnloadedBlueprintSearchMetadata(const FAssetData& InAssetData);

	/** Begins the process of extracting FiB data from an unloaded asset */
	UE_API void ExtractUnloadedFiBData(const FAssetData& InAssetData, FString* InFiBData, FName InKeyForFiBData, EFiBVersion InFiBDataVersion);

	/** Determines the global find results tab label */
	UE_API FText GetGlobalFindResultsTabLabel(int32 TabIdx);

	/** Handler for a request to spawn a new global find results tab */
	UE_API TSharedRef<SDockTab> SpawnGlobalFindResultsTab(const FSpawnTabArgs& SpawnTabArgs, int32 TabIdx);

	/** Creates and opens a new global find results tab */
	UE_API TSharedPtr<SFindInBlueprints> OpenGlobalFindResultsTab();
...
protected:
	...
	UE_API FActiveSearchQueryPtr FindSearchQuery(const class FStreamSearch* InSearchOriginator) const;

	/** Returns the next pending search data for the given query and advances the index to the next entry */
	UE_API FSearchData GetNextSearchDataForQuery(const FStreamSearch* InSearchOriginator, FActiveSearchQueryPtr InSearchQueryPtr, bool bCheckDeferredList);

	/** If searches are paused, blocks the calling thread until searching is resumed */
	UE_API void BlockSearchQueryIfPaused();

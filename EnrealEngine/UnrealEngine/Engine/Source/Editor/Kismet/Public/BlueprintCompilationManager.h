...
struct FBlueprintCompilationManager
{
	 ...
	 */
	static UE_API void FlushCompilationQueueAndReinstance(const bool bFindAndReplaceCDOReferences = false);
	 ...
	 */
	static UE_API void NotifyBlueprintLoaded(UBlueprint* BPLoaded);
	...
	static UE_API bool IsGeneratedClassLayoutReady();
	 ...
	 * value from there
	 */
	static UE_API bool GetDefaultValue(const UClass* ForClass, const FProperty* Property, FString& OutDefaultValueAsString);
	 ...
	 */
	static UE_API void ReparentHierarchies(const TMap<UClass*, UClass*>& OldClassToNewClass);
...
private:
	UE_API FBlueprintCompilationManager();
};

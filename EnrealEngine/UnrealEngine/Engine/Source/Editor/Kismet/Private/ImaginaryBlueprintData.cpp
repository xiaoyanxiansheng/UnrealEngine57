...
void FImaginaryFiBData::ParseJsonValue(FText InKey, FText InDisplayKey, TSharedPtr< FJsonValue > InJsonValue, TArray<FSearchableValueInfo>& OutParsedValues, bool bIsInArray/*=false*/, ESearchableValueStatus InSearchabilityOverride/* = ESearchableValueStatus::Searchable*/)
{
	...
	{
		// Create a FCategorySectionHelper for Json objects, which are Json representations of structs, instanced objects, array elements.
		// bIsTagAndValueCategory represents whether a match with the category's contents should return its parent as a search result.
		// Let the following category return this as search result when:
		// - The following category represents an array element, return this (the array)
		// - The following category represents the default value for a struct or object, return this (the default value)
		// - The following category represents a nested element for one of the above
		const bool bIsTagAndValueCategory = bIsInArray || IsTagAndValueCategory() || InDisplayKey.EqualTo(FFindInBlueprintSearchTags::FiB_DefaultValue);
		TSharedPtr< FCategorySectionHelper, ESPMode::ThreadSafe > SubObjectCategory = MakeShareable(new FCategorySectionHelper(AsShared(), InJsonValue->AsObject(), LookupTablePtr, InDisplayKey, bIsTagAndValueCategory));
...
FString FCategorySectionHelper::GetCategoryPath() const
{
	const FCategorySectionHelper* OuterCategory = this;
	FString Path;
	do
	{
		// Prepend category
		Path = FString::Printf(TEXT("%s.%s"), *OuterCategory->GetCategoryFunctionName(), *Path);

		// Resolve outer category, or null if the outer isn't a category
		OuterCategory = (OuterCategory->GetOuter().Pin().IsValid() && OuterCategory->GetOuter().Pin()->IsCategory())
			? (FCategorySectionHelper*)(OuterCategory->GetOuter().Pin().Get())
			: nullptr;
	} while (OuterCategory);

	// Remove redundant . at the end
	return Path.LeftChop(1);
}

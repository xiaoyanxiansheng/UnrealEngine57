...
#include "IBlueprintSelectionNotifier.h"
...
void FBlueprintEditor::OnSelectionUpdated(const TArray<TSharedPtr<FSubobjectEditorTreeNode>>& SelectedNodes)
{
	...
	// Get all registered Blueprint Selection Notifiers
	TArray<IBlueprintSelectionNotifier*> Notifiers = IModularFeatures::Get().GetModularFeatureImplementations<IBlueprintSelectionNotifier>(
		IBlueprintSelectionNotifier::GetModularFeatureName());

	// Notify each registered implementation
	TArray<FTypedElementHandle> SelectedElements;
	for (const FSubobjectEditorTreeNodePtrType& SelectedNode : SelectedNodes)
	{
		if (const FSubobjectData* const Data = SelectedNode->GetDataSource())
		{
			if (const UActorComponent* Component = Data->FindComponentInstanceInActor(GetPreviewActor()))
			{
				// Convert component to TypedElementHandle
				FTypedElementHandle ElementHandle = UEngineElementsLibrary::AcquireEditorComponentElementHandle(Component);
				if (ElementHandle.IsSet())
				{
					SelectedElements.Add(ElementHandle);
				}
			}
		}
	}

	for (IBlueprintSelectionNotifier* Notifier : Notifiers)
	{
		if (Notifier)
		{
			Notifier->OnBlueprintSelectionUpdated(this, Blueprint, SelectedElements);
		}
	}
}
	...
	if(!GraphEditorCommands.IsValid())
	{
		GraphEditorCommands = MakeShareable( new FUICommandList );
		{
			...
			GraphEditorCommands->MapAction(FGraphEditorCommands::Get().OpenInNewTab,
				FExecuteAction::CreateSP(this, &FBlueprintEditor::OnOpenSelectedNodesInNewTab),
				FCanExecuteAction::CreateSP(this, &FBlueprintEditor::CanOpenSelectedNodesInNewTab),
				FIsActionButtonVisible::CreateSP(this, &FBlueprintEditor::CanOpenSelectedNodesInNewTab)
			);
...
void FBlueprintEditor::InitBlueprintEditor(
	...
	TSharedPtr<ISCSEditorUICustomization> InSCSEditorUICustomization)
{
...
void FBlueprintEditor::CreateSubobjectEditors()
{
	...
	SubobjectEditor = SAssignNew(SubobjectEditor, SSubobjectBlueprintEditor)
		...
		.SCSEditorUICustomization(SCSEditorUICustomization);
...
void FBlueprintEditor::OnOpenSelectedNodesInNewTab()
{
	const FGraphPanelSelectionSet SelectedNodes = GetSelectedNodes();
	for (FGraphPanelSelectionSet::TConstIterator NodeIt(SelectedNodes); NodeIt; ++NodeIt)
	{
		if (const UK2Node_Composite* CompositeNode = Cast<UK2Node_Composite>(*NodeIt))
		{
		...
		else if (const UK2Node_MacroInstance* SelectedMacroInstanceNode = Cast<UK2Node_MacroInstance>(*NodeIt))
		{
			if (SelectedMacroInstanceNode->GetMacroGraph())
			{
				OpenDocument(SelectedMacroInstanceNode->GetMacroGraph(), FDocumentTracker::ForceOpenNewDocument);
			}
		}
		else if (const UK2Node_CallFunction* SelectedCallFunctionNode = Cast<UK2Node_CallFunction>(*NodeIt))
		{
			const UEdGraphNode* ResultEventNode = nullptr;
			const UEdGraph* FunctionGraph = SelectedCallFunctionNode->GetFunctionGraph(ResultEventNode);

			if (FunctionGraph && !ResultEventNode)
			{
				OpenDocument(FunctionGraph, FDocumentTracker::ForceOpenNewDocument);
			}
		}
	}
}

bool FBlueprintEditor::CanOpenSelectedNodesInNewTab() const
{
	const FGraphPanelSelectionSet SelectedNodes = GetSelectedNodes();
	for (FGraphPanelSelectionSet::TConstIterator NodeIt(SelectedNodes); NodeIt; ++NodeIt)
	{
		...
		else if (const UK2Node_CallFunction* SelectedCallFunctionNode = Cast<UK2Node_CallFunction>(*NodeIt))
		{
			...
			// Only allow opening a new tab if the call function isn't sourced by a custom event call,
			// because in that case it just opens the current graph in a new tab which can be confusing and doesn't add any benefit to the user 
			return FunctionGraph != nullptr && ResultEventNode == nullptr;
		}
	}
...
void FBlueprintEditor::ConvertFunctionToEvent(UK2Node_FunctionEntry* SelectedCallFunctionNode)
{
	...
	if (Func && EventGraph && FunctionGraph)
	{
		...
		TMap<FString, TSet<UEdGraphPin*>> EntryNodePinConnections;
		FEdGraphUtilities::GetPinConnectionMap(SelectedCallFunctionNode, EntryNodePinConnections);
		...
		TArray<UK2Node_Variable*> AllVarNodes;
		TArray<UK2Node_Variable*> RemainingVarNodes;
		FunctionGraph->GetNodesOfClass<UK2Node_Variable>(AllVarNodes);

		// Parameter getter nodes aren't supported in the event graph, so they'll be destroyed.
		// In order to preserve their connections, we'll add their linked pins to the entry node's pin connection map.
		for (UK2Node_Variable* VarNode : AllVarNodes)
		{
			check(VarNode);
			const FProperty* Property = VarNode->GetPropertyForVariable();
			const bool bIsInputParameter =
				Property &&
				Property->HasAnyPropertyFlags(CPF_Parm) &&
				!Property->HasAnyPropertyFlags(CPF_ReturnParm)
			;

			if (bIsInputParameter)
			{
				TSet<UEdGraphPin*>* ConnectionsEntry = EntryNodePinConnections.Find(Property->GetName());
				if (ensure(ConnectionsEntry))
				{
					UEdGraphPin* ValuePin = VarNode->GetValuePin();
					if (ensure(ValuePin))
					{
						for (UEdGraphPin* ConnectedPin : ValuePin->LinkedTo)
						{
							if (ensure(ConnectedPin))
							{
								ConnectionsEntry->Add(ConnectedPin);
							}
						}
					}
				}
		...
		if (SelectedCallFunctionNode->LocalVariables.Num() > 0)
		{
			...
			for (const FBPVariableDescription& LocalVar : SelectedCallFunctionNode->LocalVariables)
			{
				...
				if (FBlueprintEditorUtils::AddMemberVariable(NodeBP, NewVarName, LocalVar.VarType, LocalVar.DefaultValue))
				{
					...
					for (UK2Node_Variable* VarNode : RemainingVarNodes)
					{
		...
		FEdGraphUtilities::ReconnectPinMap(NewEventNode, EntryNodePinConnections);

		// Copy over parameter default values to the new event node
...
{	
	if (SelectedEventNode)
	{
		...
		if (FunctionSig && SourceGraph)
		{
			...
			UObject* ExistingObject = StaticFindObject(/*Class=*/ nullptr, NodeBP, *OriginalEventNameString, EFindObjectFlags::ExactClass);
			if (ExistingObject)
			{
...
void FBlueprintEditor::SetSubobjectEditorUICustomization(TSharedPtr<ISCSEditorUICustomization> InSCSEditorUICustomization)
{
	SCSEditorUICustomization = InSCSEditorUICustomization;
...
AActor* FBlueprintEditor::GetPreviewActor() const
{
	...
	{
		// On editor shutdown the PreviewScene might uninitialize before the BlueprintEditor.
		// The PreviewScene nulls its world and starts the gc, which would theoretically collect the PreviewActor too.
		// In practice however, the actor might be kept alive through references (e.g. on Linux, see UE-296301).
		// Thus, we test manually for the state of the PreviewScene, to not return an invalid actor.
		if (AActor* Actor = PreviewActorPtr.Get())
		{
			UE_LOG(LogBlueprint, Warning, TEXT("Expected preview actor '%s' to be garbage collected, but it was not:"), *Actor->GetName());
			GEngine->Exec(Actor->GetWorld(), *FString::Printf(TEXT("obj refs name=\"%s\""), *Actor->GetPathName()));
		}

...
void FSCSEditorViewportClient::ProcessClick(class FSceneView& View, class HHitProxy* HitProxy, FKey Key, EInputEvent Event, uint32 HitX, uint32 HitY)
{
	...
	if (!HitProxy)
	{
		...
		{
			if (const TSharedPtr<SSubobjectEditor> SubobjectEditor = BlueprintEditor->GetSubobjectEditor())
			{
				// If the user clicked on empty space, clear the selection
	...
	if(bIsManipulating && CurrentAxis != EAxisList::None)
	{
		...
		if (PreviewActor && BlueprintEditor.IsValid())
		{
			...
			if(SelectedNodes.Num() > 0)
			{
				...
				{
					FSnappingUtils::AdjustPercentageBasedScale(ModifiedScale);
				}
				else
				{
...
const FEditorViewportCameraSpeedSettings& FSCSEditorViewportClient::GetCameraSpeedSettings() const
{
	return GetDefault<UEditorPerProjectUserSettings>()->SCSViewportCameraSpeedSettings;
}

void FSCSEditorViewportClient::SetCameraSpeedSettings(const FEditorViewportCameraSpeedSettings& NewCameraSpeedSettings)
{
	GetMutableDefault<UEditorPerProjectUserSettings>()->SCSViewportCameraSpeedSettings = NewCameraSpeedSettings;
}

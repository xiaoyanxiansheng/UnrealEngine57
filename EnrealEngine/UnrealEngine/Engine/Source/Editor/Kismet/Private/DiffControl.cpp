...
void FBlueprintTypeDiffControl::GenerateTreeEntries(TArray< TSharedPtr<FBlueprintDifferenceTreeEntry> >& OutTreeEntries, TArray< TSharedPtr<FBlueprintDifferenceTreeEntry> >& OutRealDifferences)
{
	...
	// Now add customobject diffs, one category per object
	for (const TSharedPtr<FDiffCustomObject>& DiffCustomObject : DiffCustomObjects)
	{
		Children.Reset();

		Children.Append(DiffCustomObject->Diffs);
		OutRealDifferences.Append(DiffCustomObject->Diffs);

		TSharedPtr<FBlueprintDifferenceTreeEntry> CustomObjectEntry = FBlueprintDifferenceTreeEntry::CreateCategoryEntry(
			DiffCustomObject->SourceResult.DisplayString,
			DiffCustomObject->SourceResult.ToolTip,
			FOnDiffEntryFocused::CreateSP(AsShared(), &FBlueprintTypeDiffControl::OnSelectDiffCustomObject, TSharedPtr<IDiffCustomProperty>(), DiffCustomObject),
			Children,
			true);
		OutTreeEntries.Push(CustomObjectEntry);
	}
}
...
void FBlueprintTypeDiffControl::BuildDiffSourceArray()
{
	...
	if (BlueprintNew->FindDiffs(BlueprintOld, BlueprintDiffs))
	{
		...
		for (const FDiffSingleResult& CurrentDiff : BlueprintDiffResults)
		{
			...
			else if (CurrentDiff.Diff == EDiffType::CUSTOM_OBJECT)
			{
				// If no custom object, do not add to DiffListSource
				if (!CurrentDiff.CustomObject.IsValid())
				{
					continue;
				}
				
				// Invert order, we want old then new
				TSharedPtr<FDiffCustomObject> DiffCustomObject = MakeShared<FDiffCustomObject>(CurrentDiff, CurrentDiff.CustomObject);

				// Gather associated custom property diffs for the object
				for (const FDiffSingleResult& OtherDiff : BlueprintDiffResults)
				{
					// If the property diff is not associated with the custom object diff, ignore
					if (OtherDiff.Object1 != CurrentDiff.Object1 || OtherDiff.Object2 != CurrentDiff.Object2)
					{
						continue;
					}

					if (OtherDiff.CustomProperty.IsValid())
					{
						FGenerateDiffEntryWidget CreateEntryWidget = FGenerateDiffEntryWidget::CreateSPLambda(this, [this](const FDiffSingleResult& Diff) {
							return SNew(STextBlock)
							.Text(Diff.DisplayString)
							.ToolTipText(Diff.ToolTip)
							.ColorAndOpacity(Diff.GetDisplayColor());
						}, OtherDiff);

						TSharedPtr<FBlueprintDifferenceTreeEntry> Entry = MakeShared<FBlueprintDifferenceTreeEntry>(
							FOnDiffEntryFocused::CreateSP(AsShared(), &FBlueprintTypeDiffControl::OnSelectDiffCustomObject, OtherDiff.CustomProperty, DiffCustomObject),
							CreateEntryWidget);
						DiffCustomObject->Diffs.Push(Entry);
					}
				}

				if(!DiffCustomObject->Diffs.IsEmpty())
				{
					DiffCustomObjects.Add(DiffCustomObject);
				}
			}
			else
			{
...
void FBlueprintTypeDiffControl::OnSelectDiffCustomObject(TSharedPtr<IDiffCustomProperty> CustomProperty, TSharedPtr<FDiffCustomObject> DiffCustomObject)
{
	// This allows the owning control to focus the correct tab (or do whatever else it likes):
	SelectionCallback.ExecuteIfBound();

	if (DiffCustomObject.IsValid())
	{
		if (TSharedPtr<IDiffCustomObject> DiffWidget = DiffCustomObject->DiffWidget)
		{
			if (CustomProperty.IsValid())
			{
				CustomProperty->HighlightProperty(DiffWidget);
			}

			if (DiffWidget->GetOldWidget().IsValid())
			{
				OldDetailsBox->SetContent(DiffWidget->GetOldWidget().ToSharedRef());
			}
			if (DiffWidget->GetNewWidget().IsValid())
			{
				NewDetailsBox->SetContent(DiffWidget->GetNewWidget().ToSharedRef());
			}
		}
		else
		{
			OldDetailsBox->SetContent(SNullWidget::NullWidget);
			NewDetailsBox->SetContent(SNullWidget::NullWidget);
		}
	}
}

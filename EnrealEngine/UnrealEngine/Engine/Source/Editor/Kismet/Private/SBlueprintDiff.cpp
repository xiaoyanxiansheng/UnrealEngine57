...
void SBlueprintDiff::GenerateDifferencesList()
{
	...
	// match the graphs in the old blueprint with the graphs in the new blueprint so they can be diffed
	struct FGraphPair
	{
		TObjectPtr<UEdGraph> GraphOld;
		TObjectPtr<UEdGraph> GraphNew;
	};
	TMap<FString, FGraphPair> PathToGraphs;
	for (UEdGraph* Graph : GraphsOld)
	{
		FString Path;
		Graph->GetPathName(PanelOld.Blueprint, Path);
		PathToGraphs.FindOrAdd(Path).GraphOld = Graph;
	}
	for (UEdGraph* Graph : GraphsNew)
	{
		FString Path;
		Graph->GetPathName(PanelNew.Blueprint, Path);
		PathToGraphs.FindOrAdd(Path).GraphNew = Graph;
	}

	for (const auto& [Path, GraphPair] : PathToGraphs)
	{
		if (GraphPair.GraphOld ? IsGraphDiffNeeded(GraphPair.GraphOld) : IsGraphDiffNeeded(GraphPair.GraphNew))
		{
			CreateGraphEntry(GraphPair.GraphOld,GraphPair.GraphNew);
		}
	}

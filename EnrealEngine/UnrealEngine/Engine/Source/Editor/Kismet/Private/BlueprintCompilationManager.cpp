...
struct FBlueprintCompilationManagerImpl : public FGCObject
{
	...
	void DiscardFunctionEntryCaches(const TArray<FCompilerData>& CurrentlyCompilingBPs);
...
void FBlueprintCompilationManagerImpl::CompileSynchronouslyImpl(const FBPCompileRequest& Request)
{
	...
	if (CompiledBlueprintsToSave.Num() > 0 && !bRegenerateSkeletonOnly)
	{
		if (!bSkipSaving)
		{
			...
			{
				// The status of the blueprint might have changed during PostCDOCompiled
...
void FBlueprintCompilationManagerImpl::FlushCompilationQueueImpl(bool bSuppressBroadcastCompiled, TArray<UBlueprint*>* BlueprintsCompiled, TArray<UBlueprint*>* BlueprintsCompiledOrSkeletonCompiled, FUObjectSerializeContext* InLoadContext, TMap<UClass*, TMap<UObject*, UObject*>>* OldToNewTemplates /* = nullptr*/)
{
	...
	{ // begin TimeCompiling scope 
		...
		*/
		...
		// STAGE VII: safely throw away old skeleton CDOs and function entry caches::
		{
			using namespace UE::Kismet::BlueprintCompilationManager;

			DiscardFunctionEntryCaches(CurrentlyCompilingBPs);
		...
		for (FCompilerData& CompilerData : CurrentlyCompilingBPs)
		{
			...
			if(!CompilerData.ShouldCompileClassFunctions())
			{
						...
						BPGC->GetDefaultObject(false)->GetClass() != BPGC) )
				{
					...
					// Note: This will clear flags that are otherwise set at Link() time.
...
void FBlueprintCompilationManagerImpl::FlushReinstancingQueueImpl(bool bFindAndReplaceCDOReferences, TMap<UClass*, TMap<UObject*, UObject*>>* OldToNewTemplates /* = nullptr*/)
{
	...
	int NumClassesToReinstance = ClassesToReinstance.Num();
	{
		...
		if (IsAsyncLoading() && (!IsAsyncLoadingMultithreaded() || !IsInActualLoadingThread()))
		{
...
void FBlueprintCompilationManagerImpl::DiscardFunctionEntryCaches(const TArray<FCompilerData>& CurrentlyCompilingBPs)
{
	for (const FCompilerData& CompilerData : CurrentlyCompilingBPs)
	{
		UBlueprint* BP = CompilerData.BP;
		...
		for (UEdGraph* Graph : AllGraphs)
		{
			...
			{
				EntryNode->ClearCachedBlueprintData(BP);
			}
			EntryNodes.Reset(); // typically 1 or 0 so just avoiding repeated small allocations
		}
	}
}
	...
	for (const FReinstancingJob* ReinstancingJobPtr : ReinstancersPtr)
	{
		...
		if (OldClass)
		{
			...
			if (OldCDO && ReinstancingJob.Reinstancer.IsValid())
			{
				...
				FOverridableManager::Get().RestoreOverrideState(OldCDO, NewCDO);
	...
	for (FReinstancingJob* ReinstancingJobPtr : ReinstancersPtr)
	{
		...
		if(OldClass)
		{
			...
			for(const FReinstancingJob::FArchetypeInfo& OldArchetypeInfo : ReinstancingJob.OldArchetypeObjects)
			{
				...
				FObjectInstancingGraph PrePopulatedInstancingGraph;
				const bool bUsePrePopulatedInstancingGraph = FBlueprintCompileReinstancer::PrePopulateInstancingGraphForArchetype(PrePopulatedInstancingGraph, OldInstance, Template, &OldArchetypeToNewArchetype);
				if (bUsePrePopulatedInstancingGraph)
				{
					// Make sure the template object as all it references path up before using to instance another archetype
					FArchiveReplaceObjectRef<UObject> ReplaceInCDOAr(Template, OldArchetypeToNewArchetype);
				}
				...
				UObject* NewArchetype = NewObject<UObject>(OriginalOuter, NewClass, OriginalName, OriginalFlags & FlagMask, Template, false, bUsePrePopulatedInstancingGraph ? &PrePopulatedInstancingGraph : nullptr);
				...
				if(NewArchetype)
				{
					...
					FOverridableManager::Get().RestoreOverrideState(OldInstance, NewArchetype);
					...
					for (const TPair<UObject*, UObject*>& Pair : OldToNewInstanceMap)
					{
						...
						{
							OldArchetypeToNewArchetype.Add(Pair.Key, Pair.Value);
						}
					}
...
void FBlueprintCompilationManager::FlushCompilationQueueAndReinstance(const bool bFindAndReplaceCDOReferences /* = false*/)
{
	if(BPCMImpl)
	{
		TMap<UClass*, TMap<UObject*, UObject*>> OldToNewTemplates;
		BPCMImpl->FlushCompilationQueueImpl(false, nullptr, nullptr, nullptr, bFindAndReplaceCDOReferences ? &OldToNewTemplates : nullptr);
		BPCMImpl->FlushReinstancingQueueImpl(bFindAndReplaceCDOReferences, bFindAndReplaceCDOReferences ? &OldToNewTemplates : nullptr);

...
void SGraphTitleBarAddNewBookmark::Construct(const FArguments& InArgs)
{
	...
	[
		SNew(SImage)
		.Image(this, &SGraphTitleBarAddNewBookmark::GetAddButtonIcon)
...
const FSlateBrush* SGraphTitleBarAddNewBookmark::GetAddButtonIcon() const
{
	TSharedPtr<FBlueprintEditor> EditorContext = EditorContextPtr.Pin();
	if (EditorContext.IsValid())
	{
		...
		{
			return FAppStyle::Get().GetBrush("GraphEditor.Bookmark"); // Filled bookmark icon
		}
	}
	return FAppStyle::Get().GetBrush("GraphEditor.Bookmark.Outline"); // Hollow bookmark icon
}

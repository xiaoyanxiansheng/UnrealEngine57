...
#include UE_INLINE_GENERATED_CPP_BY_NAME(FindInBlueprintManager)
...
public:
		...
		:Controller(InController)
	{
		...
		// Make sure engine shut down is properly handled. Modules involved in the async search will be destroyed after engine pre exit.
		FCoreDelegates::OnEnginePreExit.AddRaw(this, &FAsyncSearchIndexTaskRunnable::OnEnginePreExit);
	}

	virtual ~FAsyncSearchIndexTaskRunnable()
	{
		bDestructing = true;
		EnsureCompletion();
	...
	virtual uint32 Run() override
	{
			...
			Controller->IsWorkPending())
		{
			...
			ParallelFor(AssetPathsToIndex.Num(), [&AssetPathsToIndex, bEnableFullIndexingPass, ParallelForFlags, this](int32 ArrayIdx)
			{
...
private:
	/** Called before engine exits. Useful to terminate async work before involved engine modules are being destroyed. */
	...
	/** True when this object is being destructed, useful to terminate intermediate work instead of blocking until completion is ensured */
	TAtomic<bool> bDestructing = false;
};

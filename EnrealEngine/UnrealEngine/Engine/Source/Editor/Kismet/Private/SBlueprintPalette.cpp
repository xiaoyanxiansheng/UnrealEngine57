	...
	case EEdGraphSchemaAction_K2Graph::Interface:
		{
			...
			if (const UAnimBlueprint* AnimBlueprint = Cast<UAnimBlueprint>(BlueprintIn))
			{
				IAnimClassInterface* AnimClassInterface = IAnimClassInterface::GetFromClass(AnimBlueprint->GetAnimBlueprintGeneratedClass());
				if (const FAnimBlueprintFunction* Function = IAnimClassInterface::FindAnimBlueprintFunction(AnimClassInterface, ActionIn->FuncName))
				{
					if (Function->bImplemented)
					{
						ToolTipOut = FText::Format(LOCTEXT("FunctionFromInterfaceImplemented_Tooltip", "Function (from Interface '{InterfaceName}') with implementation"), Args);
						ColorOut = FLinearColor(0.0f, 1.0f, 1.0f);
					}
				}
			}
		}

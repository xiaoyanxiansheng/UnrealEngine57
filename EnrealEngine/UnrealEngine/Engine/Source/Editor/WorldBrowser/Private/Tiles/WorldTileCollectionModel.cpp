...
void FWorldTileCollectionModel::UnloadLevels(const FLevelModelList& InLevelList)
{
	...
	if (bHasDirtyLevels)
	{
		...
		struct FHierarchyCollector : public FLevelModelVisitor
		{
			...
			FHierarchyCollector(FLevelModelList& LevelToUnload) : DirtyHierarchy(LevelToUnload) {}
		...
		FHierarchyCollector HierarchyCollector(LevelsToUnload);
		...
		for (TSharedPtr<FLevelModel> LevelModel : InLevelList)
		{
			...
			if (Level && Level->GetOutermost()->IsDirty())
			{
				LevelModel->Accept(HierarchyCollector);
			}
		}
	}
...
void FWorldTileCollectionModel::OnLevelsCollectionChanged()
{
	...
	UWorldComposition* WorldComposition = ThisWorld ? ThisWorld->WorldComposition.Get() : nullptr;
	if (WorldComposition)
	{
...
static void SetupLandscapeImportLayers(const FTiledLandscapeImportSettings& InImportSettings, const FString& ContentPath, int32 TileIndex, TArray<FLandscapeImportLayerInfo>& OutLayerInfo)
{
	for (const auto& LayerSettings : InImportSettings.LandscapeLayerSettingsList)
	{
		...
		LayerImportInfo.LayerInfo->SetBlendMethod(LayerSettings.bNoBlendWeight ? ELandscapeTargetLayerBlendMethod::None : ELandscapeTargetLayerBlendMethod::FinalWeightBlending, /*bInModify = */true);
	...
	for (auto TileModel : TargetLandscapeTiles)
	{
		...
		else // Weightmap
		{
			for (const TPair<FName, FLandscapeTargetLayerSettings>& Pair : Landscape->GetTargetLayers())
			{
				...
				if (LayerSettings.LayerInfoObj && (LayerSettings.LayerInfoObj->GetLayerName() == TargetLayer || TargetLayer == NAME_None))
				{
					if (!LayerSettings.ReimportLayerFilePath.IsEmpty())
					{
						TArray<uint8> RawData;
						ReadWeightmapFile(RawData, *LayerSettings.ReimportLayerFilePath, LayerSettings.LayerInfoObj->GetLayerName(), NumSamplesX, NumSamplesY);
						FScopedSetLandscapeEditingLayer Scope(Landscape->GetLandscapeActor(), Landscape->ReimportDestinationLayerGuid);

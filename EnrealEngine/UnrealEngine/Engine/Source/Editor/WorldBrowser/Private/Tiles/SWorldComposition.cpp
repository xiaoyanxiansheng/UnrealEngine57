...
public:
	...
	virtual void OnArrangeChildren( const FGeometry& AllottedGeometry, FArrangedChildren& ArrangedChildren ) const override
	{
		for (int32 ChildIndex=0; ChildIndex < VisibleChildren.Num(); ++ChildIndex)
		{
			...
			{
				FVector2f ChildPos = Child->GetPosition2f();
	...
	virtual int32 OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled ) const override
	{
		...
		if (!WorldModel->IsSimulating())
		{
			...
			FVector2f PaintSize = FVector2f(ScreenSpaceSize, ScreenSpaceSize);
			FVector2f PaintPosition = GraphCoordToPanelCoord(FVector2f::ZeroVector) - (PaintSize*0.5f);
			...
			FSlateRenderTransform SlateRenderTransform(Scale, UE::Slate::CastToVector2f(AllottedGeometry.GetAccumulatedRenderTransform().GetTranslation()) + PaintPosition);
		...
		// Draw observer location
		{
			...
			{
				FVector2f ObserverPositionScreen = GraphCoordToPanelCoord(FVector2f(UE_REAL_TO_FLOAT(ObserverPosition.X), UE_REAL_TO_FLOAT(ObserverPosition.Y)));
			...
			{
				FVector2f PlayerPositionScreen = GraphCoordToPanelCoord(FVector2f(UE_REAL_TO_FLOAT(PlayerPosition.X), UE_REAL_TO_FLOAT(PlayerPosition.Y)));
				const FSlateBrush* CameraImage = FAppStyle::GetBrush(TEXT("WorldBrowser.SimulationViewPosition"));
...
protected:
	...
	{
		FVector2f ScreenWorldOrigin = GraphCoordToPanelCoord(FVector2f::ZeroVector);
		FSlateRect ScreenRect(FVector2f::ZeroVector, AllottedGeometry.GetLocalSize());
			...
			ScreenWorldOrigin.X < ScreenRect.Right)
		{
			...
			LinePoints.Add(FVector2f(ScreenWorldOrigin.X, ScreenRect.Top));
			LinePoints.Add(FVector2f(ScreenWorldOrigin.X, ScreenRect.Bottom));
		...
		{
			TArray<FVector2f> LinePoints;
			LinePoints.Add(FVector2f(ScreenRect.Left, ScreenWorldOrigin.Y));
			LinePoints.Add(FVector2f(ScreenRect.Right, ScreenWorldOrigin.Y));
	...
	uint32 PaintScaleRuler(const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, uint32 LayerId) const
	{
		...
		TArray<FVector2f> LinePoints;
		LinePoints.Add(FVector2f::ZeroVector);
		LinePoints.Add(FVector2f::ZeroVector + FVector2f(ScaleRulerLength, 0.f));
	
		FSlateDrawElement::MakeLines( 
			OutDrawElements,
			LayerId,
			AllottedGeometry.ToOffsetPaintGeometry(FVector2f(10.0f, 40.0f)),
			LinePoints,
			ESlateDrawEffect::None,
			FColor(200, 200, 200));
		...
		FSlateDrawElement::MakeText(
			OutDrawElements,
			LayerId,
			AllottedGeometry.ToOffsetPaintGeometry(FVector2f(10.0f, 27.0f)),
			RulerText,
	...
	/**  SNodePanel interface */
	virtual void OnBeginNodeInteraction(const TSharedRef<SNode>& InNodeToDrag, const FVector2f& GrabOffset) override
	{
		bHasNodeInteraction = true;
	...
	void OnUpdateSelection()
	{
		...
		if (SelectionManager.AreAnyNodesSelected())
		{
			...
			if (GetBoundsForNodes(true, MinCorner, MaxCorner, 0.f))
			{
				...
				FSlateRect PanelRect = FSlateRect(FVector2f::ZeroVector, CachedGeometry.GetLocalSize());
				bool bIsVisible = FSlateRect::DoRectanglesIntersect(PanelRect, SelectionRect);
				if (!bIsVisible)
				{
					FVector2f TargetPosition = MaxCorner/2.f + MinCorner/2.f;
					FitToSelection_Executed();
				}
			}
		}
	...
	void MoveSelectedNodes(const TSharedPtr<SNode>& InNodeToDrag, FVector2f NewPosition)
	{
		...
		if (ItemDragged->IsItemEditable() && !WorldModel->IsLockTilesLocationEnabled())
		{
			...
			FVector2f StartPosition = ItemDragged->GetPosition2f() - UE::Slate::CastToVector2f(ItemDragged->GetLevelModel()->GetLevelTranslationDelta());
			FVector2f AbsoluteDelta = NewPosition - StartPosition;

			WorldModel->UpdateTranslationDelta(WorldModel->GetSelectedLevels(), FVector2D(AbsoluteDelta), bBoundsSnapping, SnappingDistanceWorld);
		}
	}

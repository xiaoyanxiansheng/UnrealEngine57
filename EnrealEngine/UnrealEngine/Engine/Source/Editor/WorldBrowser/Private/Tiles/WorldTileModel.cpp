...
void FWorldTileModel::UpdateLandscapeSectionsOffset(FIntPoint LevelOffset)
{
	...
	if (LandscapeProxy)
	{
		...
		LandscapeProxy->SetSectionBase(QuadsSpaceOffset + LandscapeProxy->GetSectionBase());
	}
}
...
bool FWorldTileModel::CreateAdjacentLandscapeProxy(ALandscapeProxy* SourceLandscape, FWorldTileModel::EWorldDirections InWhere)
{
	...
	FIntPoint LandscapeSectionOffset = FIntPoint((SourceLandscape->GetSectionBase().X+SourceLandscapeRect.Min.X)*SourceLandscapeScale.X,
												 (SourceLandscape->GetSectionBase().Y+SourceLandscapeRect.Min.Y)*SourceLandscapeScale.Y);
...
ALandscapeProxy* FWorldTileModel::ImportLandscapeTile(const FLandscapeImportSettings& Settings)
{
	...
	{
		if (ImportLayerInfo.LayerInfo != nullptr)
		{
			LandscapeProxy->AddTargetLayer(ImportLayerInfo.LayerInfo->GetLayerName(), FLandscapeTargetLayerSettings(ImportLayerInfo.LayerInfo, ImportLayerInfo.SourceFilePath));
		}
	}

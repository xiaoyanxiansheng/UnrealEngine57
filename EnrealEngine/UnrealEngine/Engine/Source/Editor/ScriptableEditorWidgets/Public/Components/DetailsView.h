 ...
 */
UCLASS(MinimalAPI)
class UDetailsView : public UPropertyViewBase, public FNotifyHook
{
	GENERATED_BODY()
...
protected:
	UE_API void ToggleShowingOnlyAllowedProperties();
	UE_API virtual bool IsRowVisibilityFiltered() const;
	UE_API virtual bool GetIsPropertyVisible(const FPropertyAndParent& PropertyAndParent) const;
	UE_API virtual bool GetIsRowVisible(FName InRowName, FName InParentName) const;

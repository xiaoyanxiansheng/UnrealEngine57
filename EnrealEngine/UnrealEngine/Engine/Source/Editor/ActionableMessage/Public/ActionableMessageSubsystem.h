 ...
 */
UCLASS(MinimalAPI)
class UActionableMessageSubsystem : public UWorldSubsystem
{
	GENERATED_BODY()
...
public:
	 ...
	 */
	UE_API void SetActionableMessage(FName InProvider, const FActionableMessage& InActionableMessage);

	/**
	 * Removes an entry from the current actionable messages. 
	 * @param InProvider The name of the entry to remove.
	 */
	UE_API void ClearActionableMessage(FName InProvider);

			...
			[
				...
				[
					...
					.Size_Lambda([this]()
					{
						...
						if (IsExpanded() && ActionableMessageList.IsValid() && TextBlock.IsValid())
						{
				...
				[
					SNew(SImage)
					.Image_Lambda([this]() { return IsExpanded() ? FAppStyle::GetBrush("ContentBrowser.SortDown") : FAppStyle::GetBrush("Symbols.LeftArrow"); })
...
EVisibility SActionableMessageViewportWidget::GetVisibility()
{
	if (UWorld* World = GEditor->GetEditorWorldContext().World())
	{
		...
		TWeakObjectPtr<const UObject> DataSourceID(ActionableMessageSubsystem);  // detect if the data source has changed, probably from loading a new level
		const uint32 SubsystemStateID = ActionableMessageSubsystem->GetStateID();

		if (CachedStateID == SubsystemStateID && CachedDataSourceID == DataSourceID)
		{
		...
		for (TSharedPtr<FActionableMessage> ActionableMessage : ActionableMessages)
		{
			if (ActionableMessage->bForceExpand)
			{
				bForceExpand = true;
				break;
			}

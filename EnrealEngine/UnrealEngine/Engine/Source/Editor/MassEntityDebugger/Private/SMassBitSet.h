...
public:
	...
	void Construct(const SMassBitSet::FArguments& InArgs, TSharedRef<FMassDebuggerModel> DebuggerModel, const FString& Label, const TBitSet& BitSet)
	{
		Construct(InArgs, DebuggerModel, Label, MakeArrayView(&BitSet, 1));
	}

	void Construct(const SMassBitSet::FArguments& InArgs, TSharedRef<FMassDebuggerModel> DebuggerModel, const FString& Label, TConstArrayView<TBitSet> BitSets, TConstArrayView<const FSlateBrush*> InBrushes = TConstArrayView<const FSlateBrush*>())
	{		
		...
		for (int i = 0; i < BitSets.Num(); ++i)
		{
			...
			AddBitSet(InArgs, ButtonBox, BitSets[i], Brush, DebuggerModel);
		}
		...
		for (const FName& Name : TypeNames)
		{
			...
			[
				...
				[
					...
					{
						DebuggerModel->SelectFragment(Name);
...
public:
	...
	void Construct(const SMassBitSetDiff::FArguments& InArgs, const FString& Label, const TBitSet& BaseBitSet, const TBitSet& BitSet
		, const EMassBitSetDiffPrune Prune, TSharedRef<FMassDebuggerModel> DebuggerModel)
	{
		...
		AddBitSet(InArgs, ButtonBox, BaseBitSet, BitSet, Brush, Prune, DebuggerModel);
		...
		[
			...
			[
				...
				[
					SNew(SButton)
					.OnClicked_Lambda([DebuggerModel, Label]()
					{
						DebuggerModel->SelectFragment(FName(*Label));
						return FReply::Handled();
					})
					[
						SNew(STextBlock)
...
protected:
	void AddBitSet(const SMassBitSetDiff::FArguments& InArgs, TSharedRef<SWrapBox>& ButtonBox, const TBitSet& BaseBitSet, const TBitSet& BitSet
		, const FSlateBrush* Brush, const EMassBitSetDiffPrune Prune, TSharedRef<FMassDebuggerModel> DebuggerModel)
	{
#if WITH_MASSENTITY_DEBUG

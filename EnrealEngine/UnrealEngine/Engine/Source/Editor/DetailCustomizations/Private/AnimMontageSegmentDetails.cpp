					...
					{						
						if (PlayLengthHandle.IsValid())
						{
							...
							{									
								...
								{
									const float NewPlayLength = static_cast<float>(DataModel->GetPlayLength());
...
void SAnimationSegmentViewport::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{
	...
	if (Component != nullptr)
	{
		...
		if (Component->PreviewInstance && (Component->PreviewInstance->GetCurrentTime() > End || Component->PreviewInstance->GetCurrentTime() < Start))
		{
...
{ 
	if (PreviewComponent != NULL)
	{
		...
		{
			return FMath::Max(static_cast<float>(PreviewComponent->GetAnimInstance()->LifeTimer - 30.0), 0.0f);
		}
	}

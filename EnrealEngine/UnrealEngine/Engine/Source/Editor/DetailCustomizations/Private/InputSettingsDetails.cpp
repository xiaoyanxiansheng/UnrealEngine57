...
namespace UE
{
	static void HandleInputDeprecationStatusChanged(UInputSettings* InputSettings, ECheckBoxState NewState, const FName MappingName)
	{
		if (!InputSettings)
		{
			return;
		...
		const FScopedTransaction Transaction(LOCTEXT("DeprecateActionMappingGroup_Transaction", "Deprecate action mapping group"));
		
		InputSettings->Modify();

		const bool bShouldBeDeprecated = NewState == ECheckBoxState::Checked;
		InputSettings->SetActionMappingDeprecated(MappingName, bShouldBeDeprecated);

		// Make sure to save out the default config file changes
		InputSettings->SaveKeyMappings();
		InputSettings->TryUpdateDefaultConfigFile();
	}

	static ECheckBoxState GetInputDeprecationCheckBoxState(UInputSettings* InputSettings, const FName MappingName)
	{
		if (InputSettings)
		{
			const bool bIsDeprecated = InputSettings->IsActionMappingDeprecated(MappingName);
			return bIsDeprecated ? ECheckBoxState::Checked : ECheckBoxState::Unchecked;
		}
...
FName InputSettingsDetails::FMappingSet::GetActionMappingName() const
{
	// Get the name of the mapping to deprecate
	FName MappingName = NAME_None;
	if (Mappings.Num() > 0)
	{
		Mappings[0]->GetChildHandle(GET_MEMBER_NAME_CHECKED(FInputActionKeyMapping, ActionName))->GetValue(MappingName);
	}
	
	return MappingName;
}

FName InputSettingsDetails::FMappingSet::GetAxisMappingName() const
{
	FName MappingName = NAME_None;
	if (Mappings.Num() > 0)
	{
		Mappings[0]->GetChildHandle(GET_MEMBER_NAME_CHECKED(FInputAxisKeyMapping, AxisName))->GetValue(MappingName);
	}
	return MappingName;
}
...
void FActionMappingsNodeBuilder::GenerateChildContent( IDetailChildrenBuilder& ChildrenBuilder )
{
	...
	for (int32 Index = 0; Index < GroupedMappings.Num(); ++Index)
	{
		...
		[
			...
			[
				SNew(STextBlock)
				.Text(LOCTEXT("DeprecationLabel_Checkbox", "Is Deprecated"))
			]
			+SHorizontalBox::Slot()
			.Padding(InputSettingsDetails::InputConstants::PropertyPadding)
			.HAlign(HAlign_Center)
			...
			[
				SNew(SCheckBox)
				.IsChecked(this, &FActionMappingsNodeBuilder::GetActionMappingCheckBoxState, MappingSet.GetActionMappingName())
				.OnCheckStateChanged(this, &FActionMappingsNodeBuilder::OnDeprecationCheckboxChanged, MappingSet.GetActionMappingName())
				.ToolTipText(LOCTEXT("DeprecateActionGroup", "If true, then this action mapping will be considered deprecated."))
			]
		];
...
void FActionMappingsNodeBuilder::RemoveActionMappingGroupButton_OnClick(const InputSettingsDetails::FMappingSet MappingSet)
{
	...
	// Since this axis mapping has been removed, also remove it from the deprecation array
	UE::HandleInputDeprecationStatusChanged(GetInputSettings(), ECheckBoxState::Unchecked, MappingSet.GetActionMappingName());
...
void FActionMappingsNodeBuilder::OnDeprecationCheckboxChanged(ECheckBoxState NewState, const FName MappingName)
{
	UE::HandleInputDeprecationStatusChanged(GetInputSettings(), NewState, MappingName);
}

ECheckBoxState FActionMappingsNodeBuilder::GetActionMappingCheckBoxState(const FName MappingName) const
{
	return UE::GetInputDeprecationCheckBoxState(GetInputSettings(), MappingName);
}

UInputSettings* FActionMappingsNodeBuilder::GetInputSettings() const
{
	TArray<UObject*> OuterObjects;
	ActionMappingsPropertyHandle->GetOuterObjects(OuterObjects);
	...
	if (OuterObjects.Num() == 1)
	{
		return CastChecked<UInputSettings>(OuterObjects[0]);
	}
	return nullptr;
}
...
void FAxisMappingsNodeBuilder::GenerateChildContent( IDetailChildrenBuilder& ChildrenBuilder )
{
	...
	for (int32 Index = 0; Index < GroupedMappings.Num(); ++Index)
	{
		...
		[
			...
			[
				SNew(SCheckBox)
				.IsChecked(this, &FAxisMappingsNodeBuilder::GetAxisMappingCheckBoxState, MappingSet.GetAxisMappingName())
				.OnCheckStateChanged(this, &FAxisMappingsNodeBuilder::OnDeprecationCheckboxChanged, MappingSet.GetAxisMappingName())
				.ToolTipText(LOCTEXT("DeprecateAxisGroup", "If true, then this axis mapping will be considered deprecated."))
			]
		];
...
void FAxisMappingsNodeBuilder::RemoveAxisMappingGroupButton_OnClick(const InputSettingsDetails::FMappingSet MappingSet)
{
	...
	// Since this axis mapping has been removed, also remove it from the deprecation array
	UE::HandleInputDeprecationStatusChanged(GetInputSettings(), ECheckBoxState::Unchecked, MappingSet.GetAxisMappingName());
...
void FAxisMappingsNodeBuilder::OnDeprecationCheckboxChanged(ECheckBoxState NewState, const FName MappingName)
{
	UE::HandleInputDeprecationStatusChanged(GetInputSettings(), NewState, MappingName);
}

ECheckBoxState FAxisMappingsNodeBuilder::GetAxisMappingCheckBoxState(const FName MappingName) const
{
	return UE::GetInputDeprecationCheckBoxState(GetInputSettings(), MappingName);
}

UInputSettings* FAxisMappingsNodeBuilder::GetInputSettings() const
{
	TArray<UObject*> OuterObjects;
	AxisMappingsPropertyHandle->GetOuterObjects(OuterObjects);

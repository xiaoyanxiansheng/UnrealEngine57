...
void FSlateFontInfoStructCustomization::OnFontChanged(const FAssetData& InAssetData)
{
	...
	const FCompositeFont* CompositeFont = FontAsset ? FontAsset->GetCompositeFont() : nullptr;
	const FName FirstFontName = (CompositeFont && CompositeFont->DefaultTypeface.Fonts.Num()) ? CompositeFont->DefaultTypeface.Fonts[0].Name : NAME_None;
...
void FSlateFontInfoStructCustomization::OnFontEntryComboOpening()
{
	...
	if(SlateFontInfoStructs.Num() > 0)
	{
		...
		if (const FCompositeFont* CompositeFont = FontObject->GetCompositeFont())
		{
...
FName FSlateFontInfoStructCustomization::GetActiveFontEntry() const
{
	...
	if(SlateFontInfoStructs.Num() > 0)
	{
		...
		if(FontObject)
		{
			if (const FCompositeFont* CompositeFont = FontObject->GetCompositeFont())
			{
				return (FirstSlateFontInfo->TypefaceFontName.IsNone() && CompositeFont->DefaultTypeface.Fonts.Num())
					? CompositeFont->DefaultTypeface.Fonts[0].Name

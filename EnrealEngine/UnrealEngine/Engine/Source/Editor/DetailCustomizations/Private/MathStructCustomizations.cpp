...
{
	// Grab children if CustomizeHeader was never run (e.g. vector mask params in the material editor)
	if (SortedChildHandles.Num() == 0)
	{
		GetSortedChildren(StructPropertyHandle, SortedChildHandles);
	}
...
void FMathStructCustomization::ExtractNumericMetadata(TSharedRef<IPropertyHandle>& PropertyHandle, FNumericMetadata<NumericType>& MetadataOut)
{
	...
	const FString& LinearDeltaSensitivityString = PropertyHandle->GetMetaData(TEXT("LinearDeltaSensitivity"));
	...
	bool bAllowSpin = !PropertyHandle->GetBoolMetaData(TEXT("NoSpinBox"));
	...
	if (FStructProperty* StructProp = CastField<FStructProperty>(PropertyHandle->GetProperty()))
	{
		if (StructProp->Struct == TBaseStructure<FRotator>::Get())
		{

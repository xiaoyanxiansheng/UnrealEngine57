...
void FComponentTransformDetails::OnCopy( ETransformField::Type TransformField )
{
	...
	CopyToText(CopyStr, TransformField);
...
void FComponentTransformDetails::OnCopyToText(TSharedPtr<IPropertyHandle> PropertyHandle, FString& Text, const TOptional<FGuid>& InOperationId,
	ETransformField::Type InTransformField)
{
	const FString PropertyPath = UE::PropertyEditor::GetPropertyPath(PropertyHandle);

	// ensure that if tag is specified, that it matches the subscriber
	if (!PropertyPath.Equals(UE::DetailsCustomizations::Internal::TransformFieldToPropertyNameString[InTransformField]))
	{
		return;
	}

	CopyToText(Text, InTransformField);
}

void FComponentTransformDetails::CopyToText(FString& CopyStr, ETransformField::Type InTransformField)
{
...
void FComponentTransformDetails::GenerateChildContent( IDetailChildrenBuilder& ChildrenBuilder )
{
	...
	if(!bHideLocationField)
	{
		...
		ParentCategory.OnCopyToText()->AddSP(this, &FComponentTransformDetails::OnCopyToText, ETransformField::Location);
	...
	if(!bHideRotationField)
	{
		...
		ParentCategory.OnCopyToText()->AddSP(this, &FComponentTransformDetails::OnCopyToText, ETransformField::Rotation);
	...
	if(!bHideScaleField)
	{
		...
		ParentCategory.OnCopyToText()->AddSP(this, &FComponentTransformDetails::OnCopyToText, ETransformField::Scale);

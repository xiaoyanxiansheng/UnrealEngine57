...
void FDateTimeStructCustomization::CustomizeHeader(TSharedRef<IPropertyHandle> StructPropertyHandle, class FDetailWidgetRow& HeaderRow, IPropertyTypeCustomizationUtils& StructCustomizationUtils)
{
	...
	const FText DateTimeTooltipText = LOCTEXT("Tooltip", "A date-time string in the following format: YYYY.MM.DD-hh.mm.ss +hhmm \n\nThe time can be entered as UTC (timezone-agnostic) or with a timezone specific UTC offset (+hhmm). The value is displayed here with the local timezone's UTC offset.");
	...
	HeaderRow
		...
		[
			SNew(SEditableTextBox)
				...
				.ToolTipText(DateTimeTooltipText)

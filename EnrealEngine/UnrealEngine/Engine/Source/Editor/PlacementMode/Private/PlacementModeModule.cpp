	...
	if (FSlateApplication::IsInitialized())
	{
		...
		auto GenerateQuickCreateSection = [this](const FName& SectionName, UToolMenu* InMenu, int MaxItems = 20)
		{
			...
			for (auto& Pair : Category->Items)
			{
				...
				// Use NativeName instead of AssetName to allow for multiple item to be registered otherwise can't have different item sharing same FAssetData
				FToolMenuEntry& NewEntry = Section.AddEntry(
					FToolMenuEntry::InitWidget(FName(Item->NativeName), SNew(SPlacementAssetMenuEntry, Item), FText(), true, true)
				);

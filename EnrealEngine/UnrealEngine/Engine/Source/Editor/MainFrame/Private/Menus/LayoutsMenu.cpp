	...
	const bool bCleanLayoutNameAndDescriptionFieldsIfNoSameValues, const bool bShouldAskBeforeCleaningLayoutNameAndDescriptionFields, const bool bShowSaveToast)
{
	...
	else
	{
		...
		{
			// Update fields
			FLayoutSaveRestore::SaveSectionToConfig(GEditorLayoutIni, "LayoutName", TEXT(""));
			FLayoutSaveRestore::SaveSectionToConfig(GEditorLayoutIni, "LayoutDescription", TEXT(""));
	...
	const bool bShouldAskBeforeCleaningLayoutNameAndDescriptionFields)
{
	...
	if (DesktopPlatform)
	{
		...
		if (bMustBeSavedInDefaultDirectory)
		{
			...
			LayoutNames.Emplace(FLayoutSaveRestore::LoadSectionFromConfig(GEditorLayoutIni, "LayoutName").ToString());
			TArray<FName> LayoutDescriptions;
			LayoutDescriptions.Emplace(FLayoutSaveRestore::LoadSectionFromConfig(GEditorLayoutIni, "LayoutDescription").ToString());
			...
			const FText NewLayoutName = LOCTEXT("NewLayoutName", "My New Layout");
			LayoutNames[0] = FName(NewLayoutName.ToString());

			const FText NewLayoutDescription = LOCTEXT("NewLayoutDescription", "Tooltip for My New Layout");
			LayoutDescriptions[0] = FName(NewLayoutDescription.ToString());
			...
			if (bWereFilesSelected && LayoutNames.Num() > 0 && LayoutDescriptions.Num() > 0 && (LayoutNames[0].ToString().Len() > 0 || LayoutDescriptions[0].ToString().Len() > 0))
			{
				...
				{
					const FString LayoutName = LayoutNames[Index].ToString();
					const FString LayoutDescription = LayoutDescriptions[Index].ToString();
					// Update fields
					FLayoutSaveRestore::SaveSectionToConfig(GEditorLayoutIni, "LayoutName", LayoutName);
					FLayoutSaveRestore::SaveSectionToConfig(GEditorLayoutIni, "LayoutDescription", LayoutDescription);
					// Flush file
					const bool bRead = true;
					GConfig->Flush(bRead, GEditorLayoutIni);
				}
...
void FLayoutsMenuLoad::LoadLayout(const FString& InLayoutPath, bool bInSkipLayoutReload)
{
	...
	if (SucessfullySaved && !bInSkipLayoutReload)
	{
...
void FLayoutsMenuSave::OverrideLayout(const int32 InLayoutIndex, const FLayoutsMenu::ELayoutsType InLayoutsType)
{
	...
	// Loading the layout which is about to be overriden, so we can store its proper name.
	// The code which overrides the current layout ends up using the Display Name and Description of the currently loaded layout.
	// That's not always the desired one. Making sure of it by loading it.
	constexpr bool bSkipReload = true;
	FLayoutsMenuLoad::LoadLayout(DesiredLayoutFullPath, bSkipReload);
...
void FLayoutsMenuRemove::RemoveLayout(const int32 InLayoutIndex, const FLayoutsMenu::ELayoutsType InLayoutsType)
{
	...
	// Remove JSON layout as well
	const FString TargetJsonFilepath = GetLayoutJsonFileName(LayoutIniFileNames[InLayoutIndex]);
	if (FLayoutSaveRestore::IsValidConfig(TargetJsonFilepath))
	{
		FPlatformFileManager::Get().GetPlatformFile().DeleteFile(*TargetJsonFilepath);
	}
}
...
void FLayoutsMenuRemove::RemoveUserLayouts()
{
	...
	if (NumberUserLayoutFiles > 0)
	{
		...
		for (const FString& UserLayoutIniFileName : UserLayoutIniFileNames)
		{
			...
			// Remove JSON layout as well
			const FString TargetJsonFilepath = GetLayoutJsonFileName(UserLayoutIniFileName);
			if (FLayoutSaveRestore::IsValidConfig(TargetJsonFilepath))
			{
				FPlatformFileManager::Get().GetPlatformFile().DeleteFile(*TargetJsonFilepath);
			}

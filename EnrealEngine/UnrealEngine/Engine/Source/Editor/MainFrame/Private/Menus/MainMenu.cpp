...
void FMainMenu::RegisterEditMenu()
{
	UToolMenu* EditMenu = UToolMenus::Get()->RegisterMenu("MainFrame.MainMenu.Edit");
	{
		...
		// IUndoHistoryEditorModule registers the nomad tab spawner. Ensure it's registered before SLevelEditor::RestoreContentArea restores the layout.
		IUndoHistoryEditorModule::Get();
	}
...
void FMainMenu::RegisterFileProjectMenu()
{
	...
	if (GetDefault<UEditorStyleSettings>()->bShowProjectMenus)
	{
		...
		Section.AddMenuEntry(FMainFrameCommands::Get().OpenCurrentProjectDirectory);
...
void FMainMenu::RegisterToolsMenu()
{
		...
		FToolMenuSection& ExperimentalSection = Menu->AddSection("ExperimentalTabSpawners", LOCTEXT("ExperimentalTabSpawnersHeading", "Experimental"));
		{
			...
			if (bTranslationPicker)
			{
				ExperimentalSection.AddMenuEntry(
					...
					FSlateIcon(FAppStyle::GetAppStyleSetName(), "TranslationPicker.MenuIcon"),
	...
	SourceControlSection.AddDynamicEntry("SubmitTab", FNewToolMenuSectionDelegate::CreateLambda([](FToolMenuSection& InSection)
	{
		...
		if (SourceControlModule.IsEnabled() && !SourceControlModule.GetProvider().UsesChangelists() &&
	...
	SourceControlSection.AddDynamicEntry("SnapshotHistoryTab", FNewToolMenuSectionDelegate::CreateLambda([](FToolMenuSection& InSection)
	{
		...
		if (SourceControlModule.GetProvider().GetName() == SourceControlProviders::GetUrcProviderName())
		{
			InSection.AddMenuEntry(

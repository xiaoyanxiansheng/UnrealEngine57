...
public:
	...
	static FVector2D GetProjectBrowserWindowSize() { return FVector2D(1206, 822); }

	// Spawns the home screen tab
	static TSharedRef<SDockTab> SpawnHomeScreenWindow(const FSpawnTabArgs& InArgs);

	// Create the home screen widget
	static TSharedRef<SWidget> CreateHomeScreenWidget(const TSharedPtr<SDockTab>& InParentTab);
...
private:
	...
	// Register HomeScreen file window menu
	static void RegisterHomeScreenFileMenu();

	// Register HomeScreen edit window menu
	static void RegisterHomeScreenEditMenu();

	// Register HomeScreen help window menu
	static void RegisterHomeScreenHelpMenu();

	// Register the Window Menu for the HomeScreen tab
    static FDelayedAutoRegisterHelper HomeScreenWindowMenuRegistration;
private:
	...
	static TStrongObjectPtr<UHomeScreenWeb> WebObject;

	// Web browser window instance
	static TSharedPtr<IWebBrowserWindow> WebBrowserWindow;

	// Web browser widget instance
	static TSharedPtr<SWebBrowser> WebBrowser;

	// Holds the tab manager that manages the HomeScreen tab.
    static TSharedPtr<FTabManager> HomeScreenTabManager;

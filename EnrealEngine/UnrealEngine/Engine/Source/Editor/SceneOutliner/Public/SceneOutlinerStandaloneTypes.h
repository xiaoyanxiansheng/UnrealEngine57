...
private:
	...
	alignas(8) mutable uint8 Data[MaxSize];
};
...
private:
	static UE_API uint32 NextUniqueID;
	uint32 ID;
...
struct FSceneOutlinerCommonLabelData
{
	...
	static UE_API const FLinearColor DarkColor;

	UE_API TOptional<FLinearColor> GetForegroundColor(const ISceneOutlinerTreeItem& TreeItem) const;

	UE_API bool CanExecuteRenameRequest(const ISceneOutlinerTreeItem& Item) const;
};

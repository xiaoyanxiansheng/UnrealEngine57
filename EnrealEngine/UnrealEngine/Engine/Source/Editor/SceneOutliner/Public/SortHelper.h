...
struct FSceneOutlinerSortHelper
{
	...
	void Sort(TArray<FSceneOutlinerTreeItemPtr>& Array)
	{
		...
		for (int32 Index = 0; Index < End; ++Index)
		{
			...
			const int32 DefaultSortID = Element.IsValid() ? GetTypeHash(Element->GetID()) : 0;
			SortData.Emplace(Index, MoveTemp(PrimaryKey), MoveTemp(SecondaryKey), DefaultSortID);
		}
		...
		SortData.Sort([&](const FSortPayload& One, const FSortPayload& Two){
			...
			if (SecondarySortMode == EColumnSortMode::Ascending && One.SecondaryKey != Two.SecondaryKey)
				return One.SecondaryKey < Two.SecondaryKey;
			else if (SecondarySortMode == EColumnSortMode::Descending && One.SecondaryKey != Two.SecondaryKey)
				return One.SecondaryKey > Two.SecondaryKey;

			return One.DefaultSortID < Two.DefaultSortID;
		});
...
private:
	...
	struct FSortPayload
	{
		...
		FSortPayload(int32 InOriginalIndex, PrimaryKeyType&& InPrimaryKey, SecondaryKeyType&& InSecondaryKey, int32 InDefaultSortID)
			: OriginalIndex(InOriginalIndex), PrimaryKey(MoveTemp(InPrimaryKey)), SecondaryKey(MoveTemp(InSecondaryKey)), DefaultSortID(InDefaultSortID)
		{}
		...
		FSortPayload& operator=(FSortPayload&& rhs)
		{
			...
			DefaultSortID = rhs.DefaultSortID;
			return *this;
		}

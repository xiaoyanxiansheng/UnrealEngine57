...
TMap<FName, FWorldTreeItem::FContextMenuProviderFunc> FWorldTreeItem::ContextMenuProviders;
...
void FWorldTreeItem::GenerateContextMenu(UToolMenu* Menu, SSceneOutliner& Outliner)
{
	...
	if (WorldPtr)
	{
		// Invoke context menu providers
		for (TPair<FName, FContextMenuProviderFunc> ContextMenuProvider : ContextMenuProviders)
		{
			ContextMenuProvider.Value(WorldPtr, Menu);
		}
	}
	...
	ULevel* Level = WorldPtr ? WorldPtr->PersistentLevel.Get() : nullptr;
...
void FWorldTreeItem::RegisterContextMenuProvider(FName InProviderName, const FContextMenuProviderFunc& InProviderFunc)
{
	check(!ContextMenuProviders.Contains(InProviderName));
	ContextMenuProviders.Add(InProviderName, InProviderFunc);
}

void FWorldTreeItem::UnregisterContextMenuProvider(FName InProviderName)
{
	verify(ContextMenuProviders.Remove(InProviderName));
}

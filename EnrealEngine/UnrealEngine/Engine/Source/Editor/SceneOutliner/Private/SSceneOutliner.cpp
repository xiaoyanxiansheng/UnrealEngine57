...
void SSceneOutliner::Construct(const FArguments& InArgs, const FSceneOutlinerInitializationOptions& InInitOptions)
{
	...
	PrimaryColumnName = InInitOptions.PrimaryColumnName;
	
	SortByColumn = PrimaryColumnName;
...
FName SSceneOutliner::GetPrimaryColumnName() const
{
...
{	
	...
	if (bSortDirty)
	{
		...
		// If the column is doing any work to prepare the sort, make sure it is ready before actually sorting the items
		if (TSharedPtr<ISceneOutlinerColumn> Column = Columns.FindRef(SortByColumn))
		{
			bExecuteSort &= Column->IsSortReady();
		}
...
void SSceneOutliner::RequestSort()
{
	...
	if (TSharedPtr<ISceneOutlinerColumn> Column = Columns.FindRef(SortByColumn))
	{
		Column->OnSortRequested(EColumnSortPriority::Primary, SortMode);
	}
}

void SSceneOutliner::SortItems(TArray<FSceneOutlinerTreeItemPtr>& Items) const
{
	TRACE_CPUPROFILER_EVENT_SCOPE(SSceneOutliner::SortItems);

...
private:
	...
	bool IsInActorEditorContext() const
	{
		...
		if(Folder->IsValid())
		{
			return SceneOutliner::FSceneOutlinerHelpers::IsFolderCurrent(Folder->GetFolder(), Folder->World.Get());
		}
	...
	bool OnVerifyItemLabelChanged(const FText& InLabel, FText& OutErrorMessage)
	{
		...
		return SceneOutliner::FSceneOutlinerHelpers::ValidateFolderName(TreeItem->GetFolder(), TreeItem->World.Get(), InLabel, OutErrorMessage);
	}
	...
	void OnLabelCommitted(const FText& InLabel, ETextCommit::Type InCommitInfo)
	{
		...
		if (TreeItem.IsValid())
		{
			...
			{
				// Rename the item
				SceneOutliner::FSceneOutlinerHelpers::RenameFolder(TreeItem->GetFolder(), TrimmedLabel, TreeItem->World.Get());

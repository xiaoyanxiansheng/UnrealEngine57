...
public:
	...
	public:
		FString Name;
		TArray<TWeakObjectPtr<UShaderCookerStats>> Stats;
		bool bInitialized;
	...
	const TArray<TWeakObjectPtr<UShaderCookerStats>>& GetShaderCookerStats(uint32 Index)
	{
		check(Index < (uint32)StatSets.Num());
		FShaderCookerStatsSet& Set = StatSets[Index];
		if(!Set.bInitialized)
		{
			Initialize(Index);
		}
		else if (!ensureMsgf(Set.Stats.IsEmpty() || Set.Stats[0].IsValid(), TEXT("Shader cook statistics set got stale. This should not happen and indicates an issue with refreshing FShaderCookerStatsPage!")))
		{
			Set.Stats.Empty();
			Initialize(Index);
		}
...
void FShaderCookerStatsPage::Generate( TArray< TWeakObjectPtr<UObject> >& OutObjects ) const
{
	...
	{
		const TArray<TWeakObjectPtr<UShaderCookerStats>>& CookStats = Stats.GetShaderCookerStats(SelectedPlatform);
		for(const TWeakObjectPtr<UShaderCookerStats>& Stat : CookStats)
		{
			if (Stat.IsValid())
			{

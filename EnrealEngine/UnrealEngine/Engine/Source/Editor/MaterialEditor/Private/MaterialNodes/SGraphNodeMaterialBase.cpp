...
{
	this->TitleBorderMargin = FMargin(10.f, 5.f, 10.f, 3.f);
	...
	if (Substrate::IsSubstrateEnabled() && MaterialNode)
	{
		...
		if (Guids.Num() > 0)
		{
			...
			{		
				...
				{
					if (const FMaterialResource* MaterialResource = MaterialForStats->GetMaterialResource(GMaxRHIShaderPlatform))
					{
...
TSharedRef<SWidget> SGraphNodeMaterialBase::CreateTitleRightWidget()
{
...
TSharedRef<SWidget> SGraphNodeMaterialBase::CreatePreviewWidget()
{
	...
	if (MaterialNode->MaterialExpression->ShouldShowPreview())
	{
...
{
	return MaterialNode->MaterialExpression->ShouldShowPreview() ? EVisibility::Visible : EVisibility::Collapsed;
}

...
FText FMaterialEditorUtilities::GetOriginalObjectName(const class UEdGraph* Graph)
{
	...
	// When diffing Material, there might not be any MaterialEditor opened for the Material
	// So we have to get the MaterialGraph's name another way
	if (UMaterial* GraphMaterial = Cast<UMaterial>(Graph->GetOuter()))
	{
		return FText::FromString(GraphMaterial->GetName());
	}
	
	return FText::GetEmpty();
}

TSharedPtr<IAssetReferenceFilter> FMaterialEditorUtilities::MakeMaterialAssetReferenceFilter(const UEdGraph* Graph)
{
	if (TSharedPtr<IMaterialEditor> MaterialEditor = GetIMaterialEditorForObject(Graph))
	{
		return MaterialEditor->MakeAssetReferenceFilter();
	}
	...
	TArray<FGetVisibleMaterialParametersFunctionState*>& FunctionStack)
{
	...
	UMaterialExpressionCollectionParameter* ParameterCollectionParam = Cast<UMaterialExpressionCollectionParameter>(MaterialExpressionKey.Expression);
	...
	else if (ParameterCollectionParam)
	{
		ParameterInfo.Name = ParameterCollectionParam->ParameterName;
	}
		
	if (Param || TexParam || TextureCollectionParam || FontParam || RuntimeVirtualTexParam || SparseVolumeTexParam || ParameterCollectionParam)
	{
...
void FMaterialEditorUtilities::AddMaterialExpressionCategory(FGraphActionMenuBuilder& ActionMenuBuilder, FText CategoryName, TArray<struct FMaterialExpression>* MaterialExpressions, const UObject* MaterialOrFunction)
{
	...
	{
		FromPinType = UMaterialGraphSchema::GetMaterialIOValueType(ActionMenuBuilder.FromPin);
	}
...
{
	return HasCompatibleConnection(ExpressionClass, static_cast<EMaterialValueType>(TestType), TestDirection, bMaterialFunction);
}

bool FMaterialEditorUtilities::HasCompatibleConnection(UClass* ExpressionClass, EMaterialValueType TestType, EEdGraphPinDirection TestDirection, bool bMaterialFunction)
{
	if (TestType != 0)
	{
...
void FMaterialEditorUtilities::BuildTextureStreamingData(UMaterialInterface* UpdatedMaterial)
{
	...
	if (UpdatedMaterial)
	{
		...
		if (!RootMaterial || !RootMaterial->GetPackage()->HasAnyPackageFlags(PKG_Cooked))
		{
			...
			{
				FMaterialUtilities::FExportErrorManager ExportErrors(ShaderPlatform);
				for (UMaterialInterface* MaterialInterface : Materials)
				{
					FMaterialUtilities::ExportMaterialUVDensities(MaterialInterface, QualityLevel, ShaderPlatform, ExportErrors);
				}

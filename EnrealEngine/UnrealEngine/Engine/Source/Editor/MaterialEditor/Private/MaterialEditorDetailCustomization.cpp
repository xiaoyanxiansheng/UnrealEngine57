...
void FMaterialExpressionParameterDetails::CustomizeDetails( IDetailLayoutBuilder& DetailLayout )
{
	...
	for (const auto& WeakObjectPtr : Objects)
	{
		...
		if (ScalarParameter)
		{
			...
			TSharedPtr<IPropertyHandle> DefaultValueHandle = DetailLayout.GetProperty("DefaultValue", UMaterialExpressionScalarParameter::StaticClass());
			DefaultValueHandles.Emplace(DefaultValueHandle);

			if (DefaultValueHandle.IsValid() && DefaultValueHandle->IsValidHandle())
			{
				if (!ScalarParameter->bUseCustomPrimitiveData && ScalarParameter->ControlType == EMaterialScalarParameterControlType::Enumeration && !ScalarParameter->Enumeration.IsNull())
				{
					// Display enumeration widget for DefaultValue
					IDetailPropertyRow& PropertyRow = *DetailLayout.EditDefaultProperty(DefaultValueHandle);
					...
					.ValueContent()
					[
						FMaterialPropertyHelpers::CreateScalarEnumerationParameterValueWidget(ScalarParameter->Enumeration, DefaultValueHandle)
					];
				}
			}
			...
			// Some properties require a layout rebuild on edit.
			auto ForceRefresh = FSimpleDelegate::CreateLambda([&DetailLayout]() { DetailLayout.ForceRefreshDetails(); });

			TSharedPtr<IPropertyHandle> ControlTypeHandle = DetailLayout.GetProperty("ControlType", UMaterialExpressionScalarParameter::StaticClass());
			if (ControlTypeHandle.IsValid() && ControlTypeHandle->IsValidHandle())
			{
				ControlTypeHandle->SetOnPropertyValueChanged(ForceRefresh);
			}

			TSharedPtr<IPropertyHandle> EnumerationHandle = DetailLayout.GetProperty("Enumeration", UMaterialExpressionScalarParameter::StaticClass());
			if (EnumerationHandle.IsValid() && EnumerationHandle->IsValidHandle())
			{
				EnumerationHandle->SetOnPropertyValueChanged(ForceRefresh);
			}

			TSharedPtr<IPropertyHandle> UseCustomPrimitiveDataPropertyHandle = DetailLayout.GetProperty("bUseCustomPrimitiveData", UMaterialExpressionScalarParameter::StaticClass());
			if (UseCustomPrimitiveDataPropertyHandle.IsValid() && UseCustomPrimitiveDataPropertyHandle->IsValidHandle())
			{
				UseCustomPrimitiveDataPropertyHandle->SetOnPropertyValueChanged(ForceRefresh);
			}
			...
			if (ScalarParameter->IsUsedAsAtlasPosition())
			{
				...
				if (CurveHandle.IsValid() && CurveHandle->IsValidHandle())
				{
					...
					AtlasCategory.AddCustomRow(CurveHandle->GetPropertyDisplayName())
						...
						{
							AtlasHandle->SetOnPropertyValueChanged(ForceRefresh);
						}

				}

				DetailLayout.HideCategory(NAME_UI);
				DetailLayout.HideCategory(TEXT("CustomPrimitiveData"));
			}

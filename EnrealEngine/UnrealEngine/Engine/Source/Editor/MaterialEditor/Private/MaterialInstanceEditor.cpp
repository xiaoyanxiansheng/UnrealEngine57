...
static TAutoConsoleVariable<bool> CVarEnableTextureValidationInMaterialInstanceEditor(
	TEXT("r.MaterialEditor.EnableTextureValidation"),
	true,
	TEXT("Enables expensive texture validation in the material instance editor.  On by default.  Disable this to improve performance when changing texture paramters on large materials."),
	ECVF_Default
);
...
TSharedPtr<IAssetReferenceFilter> FMaterialInstanceEditor::MakeAssetReferenceFilter() const
{
	if (GEditor)
	{
...
void FMaterialInstanceEditor::NotifyPostChange( const FPropertyChangedEvent& PropertyChangedEvent, FProperty* PropertyThatChanged)
{
	...
	if (PropertyClass && 
	   (PropertyClass->GetName() == TEXT("DEditorTextureParameterValue") || 
		PropertyClass->GetName() == TEXT("DEditorRuntimeVirtualTextureParameterValue") ||
		PropertyClass->GetName() == TEXT("DEditorSparseVolumeTextureParameterValue")))
	{
		RefreshOnScreenTextureWarnings();
	}
...
void FMaterialInstanceEditor::DrawMessages( FViewport* Viewport, FCanvas* Canvas )
{
	...
	{
		const FMaterialResource* MaterialResource = MaterialEditorInstance->SourceInstance->GetMaterialResource(GMaxRHIShaderPlatform);
...
void FMaterialInstanceEditor::RefreshOnScreenTextureWarnings()
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FMaterialInstanceEditor::RefreshOnScreenTextureWarnings)

	OnScreenMessages.Reset();

	const bool bValidateMaterials = CVarEnableTextureValidationInMaterialInstanceEditor.GetValueOnAnyThread();

	if (bValidateMaterials && MaterialEditorInstance->SourceInstance)
	{
		UMaterial* BaseMaterial = MaterialEditorInstance->SourceInstance->GetMaterial();
		if (BaseMaterial)
		{

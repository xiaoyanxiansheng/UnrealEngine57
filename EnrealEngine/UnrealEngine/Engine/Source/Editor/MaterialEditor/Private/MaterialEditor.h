...
protected:
	/**
	 * Helper struct used to validate the state of the material after the material editor
	 * performs an operation, searching for any invalid state.
	 */
	struct FScopedMaterialChecker
	{
		UE_NONCOPYABLE(FScopedMaterialChecker);

		/** The material editor performing the operation */
		FMaterialEditor* Editor;

		/** Name of the operation being performed, logged when an issue is reported */
		const char* DebugEditName;
		
		FScopedMaterialChecker(FMaterialEditor* Editor, const char* DebugEditName);
		~FScopedMaterialChecker();
	};
...
protected:
	...
	/** Opens collapsed nodes in a new tab */
	void OnOpenSelectedNodesInNewTab();
	/** Check if selected nodes can be opened in a new tab*/
	bool CanOpenSelectedNodesInNewTab() const;
...
private:
	...
	*/
	...
	void SetNumericParameterDefaultOnDependentMaterials(EMaterialParameterType Type, FName ParameterName, const UE::Shader::FValue& Value);
	void ClearNumericParameterDefaultOnDependentMaterials(EMaterialParameterType Type, FName ParameterName);
	...
	/**
	* Fixes up the samplers for all broken texture sample expressions.
	*/
	void FixupMismatchedSamplerExpressions();

	...
	for (auto It(Graph->Nodes.CreateConstIterator()); It; ++It)
	{
		...
		if (!bNodeMatchesSearch)
		{
			...
			if (UMaterialGraphNode* MatNode = Cast<UMaterialGraphNode>(Node))
			{
				...
				{
					if (!MatNode->MaterialExpression->DeepSearchQuery(*Tokens[Index], NodeResult))
					{
	...
	return SNew(STableRow< TSharedPtr<FFindInMaterialResult> >, OwnerTable)
		[
			...
			+ SHorizontalBox::Slot()
				...
				[
					SNew(STextBlock)
					.Text(InItem->bEmbeddedViaNode ? FText::Format(LOCTEXT("MaterialResultSearchValueEmbeddedFmt", "Is found inside of {0}"), FText::FromString(InItem->Value)) : FText::FromString(InItem->Value))

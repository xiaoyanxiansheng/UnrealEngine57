...
// PROPERTY ----------------------------------------------
	if (StackParameterData->StackDataType == EStackDataType::Property)
	{
		...
		// Handle our own EditCondition, force Visibility to "Visible" since WrapperWidget controls our actual visibility
		FMaterialParameterPropertyRowOverrides PropertyRowOverrides;
		PropertyRowOverrides.EditConditionValue = TAttribute<bool>::Create(TAttribute<bool>::FGetter::CreateSP(Tree, &SMaterialLayersFunctionsInstanceTree::IsOverriddenExpression, StackParameterData->Parameter, LayerStateIndex));
		PropertyRowOverrides.OnEditConditionValueChanged = FOnBooleanValueChanged::CreateSP(this, &SMaterialLayersFunctionsInstanceTreeItem::OnOverrideParameter, StackParameterData->Parameter);
		PropertyRowOverrides.Visibility = TAttribute<EVisibility>(EVisibility::Visible);
		...
		FMaterialPropertyHelpers::ConfigurePropertyRowForParameter(Row, StackParameterData->Parameter, PropertyHandle, MaterialEditorInstance, PropertyRowOverrides);
		...
		ResetWidget = SNew(SResetToDefaultPropertyEditor, PropertyHandle)
			.IsEnabled(PropertyRowOverrides.EditConditionValue)
			.CustomResetToDefault(FMaterialPropertyHelpers::CreateResetToDefaultOverride(StackParameterData->Parameter, MaterialEditorInstance));

		PropertyHandle->SetOnPropertyValueChanged(FSimpleDelegate::CreateSP(Tree, &SMaterialLayersFunctionsInstanceTree::UpdateThumbnailMaterial, StackParameterData->ParameterInfo.Association, StackParameterData->ParameterInfo.Index, false));
		PropertyHandle->SetOnChildPropertyValueChanged(FSimpleDelegate::CreateSP(Tree, &SMaterialLayersFunctionsInstanceTree::UpdateThumbnailMaterial, StackParameterData->ParameterInfo.Association, StackParameterData->ParameterInfo.Index, false));
...
void SMaterialLayersFunctionsInstanceTree::Construct(const FArguments& InArgs)
{
	...
	NotifyHook = InArgs._InNotifyHook;
	CreateGroupsWidget();
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(SMaterialLayersFunctionsInstanceWrapper::Refresh)

	LayerParameter.Reset();

	UMaterialInterface* Material = MaterialEditorInstance->GetMaterialInterface();
	UMaterialInterface* ParentMaterial = MaterialEditorInstance->GetParentMaterialInterface();

	FMaterialLayersFunctions Layers;
	const bool bHasLayers = ParentMaterial ? ParentMaterial->GetMaterialLayers(Layers) : Material->GetMaterialLayers(Layers);
...
void SMaterialLayersFunctionsInstanceWrapper::Construct(const FArguments& InArgs)
{
	NestedTree = SNew(SMaterialLayersFunctionsInstanceTree)
		...
		.InNotifyHook(InArgs._InNotifyHook);
					...
					[
						...
						[
							...
							[
								...
								.HAlign(ValueAlignment)
								.VAlign(VAlign_Center)
								.Padding(FMargin(12.0f, 0.0f, 2.0f, 0.0f))	// Match DetailWidgetConstants::RightRowPadding
								[
									RightSideWidget
								]
							]

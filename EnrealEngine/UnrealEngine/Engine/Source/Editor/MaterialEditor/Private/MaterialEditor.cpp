...
bool FMatExpressionPreview::ShouldCache(const FShaderType* ShaderType, const FVertexFactoryType* VertexFactoryType) const
{
			...
			if (FocusedGraphEdPtr.IsValid())
			{
				...
				else if (MaterialFunction->GetMaterialFunctionUsage() == EMaterialFunctionUsage::MaterialLayerBlend)
				{
					...
					if (InputTop)
					{
						...
						BaseAttributesInput->InputType = FunctionInput_MaterialAttributes;
						BaseAttributesInput->InputName = TopMaterialBlendInputName;
						BaseAttributesInput->bUsePreviewValueAsDefault = true;
						BaseAttributesInput->BlendInputRelevance = EBlendInputRelevance::Top;
					}
					...
					if (InputBottom)
					{
						...
						BaseAttributesInput->InputType = FunctionInput_MaterialAttributes;
						BaseAttributesInput->InputName = BottomMaterialBlendInputName;
						BaseAttributesInput->bUsePreviewValueAsDefault = true;
						BaseAttributesInput->BlendInputRelevance = EBlendInputRelevance::Bottom;
					}
					if (GetDefault<UEditorExperimentalSettings>()->bExampleLayersAndBlends)
					{
...
FMaterialEditor::~FMaterialEditor()
{
	...
	{
		ClearNumericParameterDefaultOnDependentMaterials(It.Key, It.Value);
	}
...
void FMaterialEditor::RegisterToolBar()
{
	...
	if (!UToolMenus::Get()->IsMenuRegistered(MenuName))
	{
			...
			FNewToolMenuDelegate::CreateLambda([](UToolMenu* InSubMenu)
				{
					...
					Section.AddEntry(FToolMenuEntry::InitMenuEntry(FMaterialEditorCommands::Get().FixupMismatchedSamplers));
				}),
		...
		SMaterialRevisionMenu::MakeDiffMenu(ToolBar);
	}
}
...
void FMaterialEditor::DrawMessages( FViewport* InViewport, FCanvas* Canvas )
{
	...
	{
		Canvas->PushAbsoluteTransform( FTranslationMatrix(FVector::ZeroVector ) );
...
void FMaterialEditor::UpdatePreviewMaterial( bool bForce )
{
	...
	{
		MaterialStatsManager->SetMaterial(ExpressionPreviewMaterial, bStatsFromPreviewMaterial ? DerivedMaterialInstances : OriginalDerivedMaterialInstances);
	}
	else
	{
		MaterialStatsManager->SetMaterial(bStatsFromPreviewMaterial ? Material : OriginalMaterial, bStatsFromPreviewMaterial ? DerivedMaterialInstances : OriginalDerivedMaterialInstances);
...
bool FMaterialEditor::UpdateOriginalMaterial()
{
	...
	EShaderPlatform EditorPreviewPlatform = GEditor->PreviewPlatform.GetShaderPlatform();
	EditorPreviewPlatform = EditorPreviewPlatform == GMaxRHIShaderPlatform ? EShaderPlatform::SP_NumPlatforms : EditorPreviewPlatform;
	EShaderPlatform ShaderPlatformsToTest[] = {GMaxRHIShaderPlatform, EditorPreviewPlatform};
	...
	for (EShaderPlatform ShaderPlatform : ShaderPlatformsToTest)
	{
		...
		{
			Errors.Push(FText::Format(NSLOCTEXT("UnrealEd", "Warning_CompileErrorsInMaterial_ListEntryFeatureLevel", "- At Shader Platform {0}."), FText::FromString(*FDataDrivenShaderPlatformInfo::GetName(ShaderPlatform).ToString())));
...
void FMaterialEditor::UpdateMaterialinfoList_Old()
{
	...
	for (int32 i = 0; i < NumFeatureLevels; ++i)
	{
		...
		const FMaterialResource* MaterialResource = MaterialForStats->GetMaterialResource(GetFeatureLevelShaderPlatform_Checked(FeatureLevel));
		...
		if (CompileErrors.Num() == 0 && (!MaterialFunction || MaterialFunction->GetMaterialFunctionUsage() != EMaterialFunctionUsage::Default))
		{
			...
			const FMaterialResource* EmptyMaterialResource = EmptyMaterial ? EmptyMaterial->GetMaterialResource(GetFeatureLevelShaderPlatform_Checked(FeatureLevel)) : nullptr;
...
void FMaterialEditor::UpdateMaterialInfoList()
{
	...
	if (MaterialFunction && ExpressionPreviewMaterial)
	{
		...
		CompileErrors = ExpressionPreviewMaterial->GetMaterialResource(GMaxRHIShaderPlatform)->GetCompileErrors();
...
{
	const FMaterialResource* ErrorMaterialResource = PreviewExpression ? ExpressionPreviewMaterial->GetMaterialResource(GMaxRHIShaderPlatform) : Material->GetMaterialResource(GMaxRHIShaderPlatform);
...
void FMaterialEditor::BindCommands()
{
	...
	ToolkitCommands->MapAction(
		Commands.FixupMismatchedSamplers,
		FExecuteAction::CreateSP(this, &FMaterialEditor::FixupMismatchedSamplerExpressions),
		FCanExecuteAction() );
...
void FMaterialEditor::OnPromoteObjects()
{
	...
	if (SelectedNodes.Num() > 0)
	{
		...
		FScopedMaterialChecker Checker{ this, __FUNCTION__ };
		ModifyMaterial();
...
void GatherMaterialsToOverride(const TObjectPtr<UMaterialFunction>& MaterialFunction, const TObjectPtr<UMaterial>& Material, TObjectPtr<UMaterial> OriginalMaterial, TArray<UMaterial*>& OutMaterialsToOverride)
{
	if (MaterialFunction)
	{
		...
		for (TObjectIterator<UMaterial> It(/*AdditionalExclusionFlags = */RF_ClassDefaultObject, /*bIncludeDerivedClasses = */true, /*InInternalExclusionFlags = */EInternalObjectFlags::Garbage); It; ++It)
		{
			...
			if (CurrentMaterial != Material)
			{
				...
				if (bUpdate)
				{
					OutMaterialsToOverride.Add(CurrentMaterial);
				}
			}
		}
	}
	else
	{
		...
		OutMaterialsToOverride.Add(Material);
		// The original Material we are editing.
		OutMaterialsToOverride.Add(OriginalMaterial);
	}
}

void FMaterialEditor::SetNumericParameterDefaultOnDependentMaterials(EMaterialParameterType Type, FName ParameterName, const UE::Shader::FValue& Value)
{
	TArray<UMaterial*> MaterialsToOverride;
	GatherMaterialsToOverride(MaterialFunction, Material, OriginalMaterial, MaterialsToOverride);

	EShaderPlatform ShaderPlatform = GetFeatureLevelShaderPlatform_Checked(GEditor->GetEditorWorldContext().World()->GetFeatureLevel());
	for (int32 MaterialIndex = 0; MaterialIndex < MaterialsToOverride.Num(); MaterialIndex++)
	{
		...
		CurrentMaterial->SetNumericParameterDefault(Type, ParameterName, Value, ShaderPlatform);
	}
	...
	for (TObjectIterator<UMaterialInstance> It(/*AdditionalExclusionFlags = */RF_ClassDefaultObject, /*bIncludeDerivedClasses = */true, /*InInternalExclusionFlags = */EInternalObjectFlags::Garbage); It; ++It)
	{
		...
		if (CurrentMaterialInstance->bHasStaticPermutationResource)
		{
			...
			{
				CurrentMaterialInstance->SetNumericParameterDefault(Type, ParameterName, Value, ShaderPlatform);
			}
		}
	}
}

void FMaterialEditor::ClearNumericParameterDefaultOnDependentMaterials(EMaterialParameterType Type, FName ParameterName)
{
	TArray<UMaterial*> MaterialsToOverride;
	GatherMaterialsToOverride(MaterialFunction, Material, OriginalMaterial, MaterialsToOverride);
	...
	{
		UMaterial* CurrentMaterial = MaterialsToOverride[MaterialIndex];
		CurrentMaterial->ClearNumericParameterDefault(Type, ParameterName, ShaderPlatform);
	}
	...
	for (TObjectIterator<UMaterialInstance> It(/*AdditionalExclusionFlags = */RF_ClassDefaultObject, /*bIncludeDerivedClasses = */true, /*InInternalExclusionFlags = */EInternalObjectFlags::Garbage); It; ++It)
	{
		...
		if (CurrentMaterialInstance->bHasStaticPermutationResource)
		{
			...
			if (MaterialsToOverride.Contains(BaseMaterial))
			{
				CurrentMaterialInstance->ClearNumericParameterDefault(Type, ParameterName, ShaderPlatform);
			}
		}
	}
}
...
void FMaterialEditor::OnNumericParameterDefaultChanged(class UMaterialExpression* Expression, EMaterialParameterType Type, FName ParameterName, const UE::Shader::FValue& Value)
{
	...
	{
		SetNumericParameterDefaultOnDependentMaterials(Type, ParameterName, Value);
...
UMaterialExpressionComment* FMaterialEditor::CreateNewMaterialExpressionComment(const FVector2D& NodePos, const UEdGraph* Graph)
{
	...
	UMaterialExpressionComment* NewComment = NULL;
	{
		...
		NewComment->CommentColor = GetDefault<UGraphEditorSettings>()->DefaultCommentNodeTitleColor;
...
TSharedPtr<IAssetReferenceFilter> FMaterialEditor::MakeAssetReferenceFilter() const
{
	if (GEditor)
	{
...
void FMaterialEditor::CreateDerivedMaterialInstancesPreviews()
{
	...
	{
		//UE-296291, currently unable to track down why the editor occasionally crashes from OriginalMaterial object being a nullptr at this point.
		//Our earliest report we are aware of is around UE5.5.3 but there's no obvious criteria as to why, and currently unable to repro deterministically, so this debug path is temporary to try and pull more info via the CrashReporter.
		//If you see this message and know a method to reproduce please let Epic know!
		const FString MissingEntry = "NULL";
		UE_LOG(LogMaterialEditor, Warning, TEXT("MaterialEditor missing valid OriginalMaterial."));
		UE_LOG(LogMaterialEditor, Warning, TEXT("OriginalMaterialObject = %s"), *(OriginalMaterialObject ? OriginalMaterialObject->GetPathName() : MissingEntry));
		UE_LOG(LogMaterialEditor, Warning, TEXT("Material = %s"), *(Material ? Material->GetPathName() : MissingEntry));
		UE_LOG(LogMaterialEditor, Warning, TEXT("ExpressionPreviewMaterial = %s"), *(ExpressionPreviewMaterial ? ExpressionPreviewMaterial->GetPathName() : MissingEntry));
		UE_LOG(LogMaterialEditor, Warning, TEXT("PreviewExpression = %s"), *(PreviewExpression ? PreviewExpression->GetPathName() : MissingEntry));
		UE_LOG(LogMaterialEditor, Warning, TEXT("MaterialFunction = %s"), *(MaterialFunction ? MaterialFunction->GetPathName() : MissingEntry));
		UE_LOG(LogMaterialEditor, Warning, TEXT("bMaterialDirty = %s"), *LexToString(bMaterialDirty));
		UE_LOG(LogMaterialEditor, Warning, TEXT("bStatsFromPreviewMaterial = %s"), *LexToString(bStatsFromPreviewMaterial));
		UE_LOG(LogMaterialEditor, Warning, TEXT("bDestructing = %s"), *LexToString(bDestructing));

		UE_LOG(LogMaterialEditor, Fatal, TEXT("OriginalMaterial is invalid in the material editor, with no deterministic repro of the problem. This message is to highlight the crash/callstack via the crash reporter with further Log info for Epic to investigate."));
	}
...
void FMaterialEditor::OnOpenSelectedNodesInNewTab()
{
	const FGraphPanelSelectionSet SelectedNodes = GetSelectedNodes();
	for (FGraphPanelSelectionSet::TConstIterator NodeIt(SelectedNodes); NodeIt; ++NodeIt)
	{
		if (const UMaterialGraphNode_Composite* CompositeNode = Cast<UMaterialGraphNode_Composite>(*NodeIt))
		{
			if (CompositeNode->BoundGraph)
			{
				OpenDocument(CompositeNode->BoundGraph, FDocumentTracker::ForceOpenNewDocument);
			}
		}
	}
}

bool FMaterialEditor::CanOpenSelectedNodesInNewTab() const
{
	const FGraphPanelSelectionSet SelectedNodes = GetSelectedNodes();
	for (FGraphPanelSelectionSet::TConstIterator NodeIt(SelectedNodes); NodeIt; ++NodeIt)
	{
...
void FMaterialEditor::PostUndo(bool bSuccess)
{
	if (bSuccess)
	{	
		checkf(Material, TEXT("Unknown material during material Undo operation"));
		FScopedMaterialChecker Checker{ this, __FUNCTION__ };
		...
		if (TSharedPtr<SGraphEditor> FocusedGraphEd = FocusedGraphEdPtr.Pin())
		{
			checkf(FocusedGraphEd->GetCurrentGraph(), TEXT("Null FocusedGraphEd->GetCurrentGraph() during Undo operation in material '%s'"), *Material->GetName());

			// @TODO Find a more coherent check for this, rather than rely on sage knowledge that the schema won't exist.
			// If our previous transaction created the current graph, it won't have a schema after undo.
			if (!FocusedGraphEd->GetCurrentGraph()->GetSchema())
			{
				checkf(DocumentManager, TEXT("Null DocumentManager during Undo operation in material '%s'"), *Material->GetName());
...
void FMaterialEditor::RefreshExpressionPreviews(bool bForceRefreshAll /*= false*/)
{
	...
	// @todo-massimo.tristano
	// We have a bug causing nulls to end up in the expressions array, but we are not yet aware of
	// a repro to track down what actually causes it. This temporary code not only sanitizes any
	// null expression in the array (that would crash the editor) but also allows us to know when
	// null expressions are found.
	if (Material->GetEditorOnlyData()->ExpressionCollection.Expressions.Remove(nullptr) > 0)
	{
		UE_LOG(LogMaterialEditor, Warning, TEXT("Null expressions found in expressions list during expression previews refresh in material '%s'."), *GetEditingObjects()[0]->GetName());
	}
...
void FMaterialEditor::RefreshExpressionPreview(UMaterialExpression* MaterialExpression, bool bRecompile)
{
	...
	if ( (MaterialExpression->bRealtimePreview || MaterialExpression->bNeedToUpdatePreview) && !MaterialExpression->bCollapsed )
	{
		for( int32 PreviewIndex = 0 ; PreviewIndex < ExpressionPreviews.Num() ; ++PreviewIndex )
		{
			...
			if (ensure(ExpressionPreview) && ExpressionPreview->GetExpression() == MaterialExpression)
			{
...
{
	bNewlyCreated = false;
	if (MaterialExpression->ShouldShowPreview() && !MaterialExpression->IsA<UMaterialExpressionCustomOutput>())
	{
		...
		if (!Preview && MaterialExpression->Material->GetExpressions().Contains(MaterialExpression))
		{
			...
			Preview->CacheShaders(EMaterialShaderPrecompileMode::None);
		}
	...
	if (!GraphEditorCommands)
{
	GraphEditorCommands = MakeShareable( new FUICommandList );
		...
		GraphEditorCommands->MapAction(FGraphEditorCommands::Get().OpenInNewTab,
			FExecuteAction::CreateSP(this, &FMaterialEditor::OnOpenSelectedNodesInNewTab),
			FCanExecuteAction::CreateSP(this, &FMaterialEditor::CanOpenSelectedNodesInNewTab),
			FIsActionButtonVisible::CreateSP(this, &FMaterialEditor::CanOpenSelectedNodesInNewTab)
		);
...
FGraphAppearanceInfo FMaterialEditor::GetGraphAppearance() const
{
	...
	if (Substrate::IsSubstrateEnabled())
	{
		...
		const FMaterialResource* MaterialResource = MaterialForStats->GetMaterialResource(GMaxRHIShaderPlatform);
		if (MaterialResource)
		{
...
void FMaterialEditor::FixupMismatchedSamplerExpressions()
{
	// Find all expressions that need to be fixed.
	TArray<UMaterialExpressionTextureBase*> ExpressionsToFix;
	for (UMaterialExpression* MaterialExpression : Material->GetExpressions())
	{
		if (UMaterialExpressionTextureBase* TextureBase = Cast<UMaterialExpressionTextureBase>(MaterialExpression))
		{
			FString SamplerTypeError;
			if (!UMaterialExpressionTextureBase::VerifySamplerType(GEditor->PreviewPlatform.GetShaderPlatform(), nullptr, TextureBase->Texture, TextureBase->SamplerType, SamplerTypeError))
			{
				ExpressionsToFix.Add(TextureBase);
			}
		}
	}

	if (ExpressionsToFix.Num() > 0)
	{
		if (EAppReturnType::Yes != FMessageDialog::Open(EAppMsgType::YesNo,
														FText::Format(
															NSLOCTEXT("UnrealEd", "Prompt_MaterialEditorFixupTextureSamplers", "There are \"{0}\" texture sampler(s) with the incorrect sampler picked for their texture.\nWould you like to automatically fix them up?"),
															ExpressionsToFix.Num())))
		{
			return;
		}

		const FScopedTransaction Transaction(NSLOCTEXT("UnrealEd", "MaterialEditorFixupSamplerExpressions", "Material Editor: Fixup Sampler Expressions"));
			
		{
			FScopedMaterialChecker Checker{ this, __FUNCTION__ };
			ModifyMaterial();

			for (UMaterialExpressionTextureBase* TextureBase : ExpressionsToFix)
			{
				TextureBase->Modify();
				TextureBase->AutoSetSampleType();
			}
...
FMaterialEditor::FScopedMaterialChecker::FScopedMaterialChecker(FMaterialEditor* InEditor, const char* InDebugEditName)
	: Editor{ InEditor }
	, DebugEditName{ InDebugEditName }
{
}

FMaterialEditor::FScopedMaterialChecker::~FScopedMaterialChecker()
{
	if (Editor->Material->GetEditorOnlyData()->ExpressionCollection.Expressions.Remove(nullptr) > 0)
	{
		UE_LOG(LogMaterialEditor, Warning, 
			TEXT("Material Editor internal error after operation '%hs': null expressions found in material '%s' expressions list."), 
			DebugEditName, *Editor->Material->GetName());
	}
}

...
TArray<UTexture*> UMaterialEditingLibrary::GetUsedTextures(UMaterial* Material)
{
	...
	Material->GetUsedTextures(OutTextures, GetCurrentMaterialQualityLevelChecked());
	return OutTextures;
}
...
static bool RenameInternal(AssetType* Asset, const FName OldGroupName, const FName NewGroupName)
{
	if (!Asset)
	{
		UE_LOG(LogMaterialEditingLibrary, Warning, TEXT("Invalid asset"));
		return false;
	}

	if (Asset->HasAnyFlags(RF_ClassDefaultObject))
	{
		UE_LOG(LogMaterialEditingLibrary, Warning, TEXT("Can't rename groups on a Class Default Object."));
		return false;
	}

	if (OldGroupName == NewGroupName)
	{
		UE_LOG(LogMaterialEditingLibrary, Log, TEXT("Old and new group names are identical: %s. Nothing to do."), *NewGroupName.ToString());
		return false;
	}
			...
			if (FNameProperty* NameProp = CastField<FNameProperty>(Prop))
			{
				...
				if (CurrentGroupName && *CurrentGroupName == OldGroupName)
				{
					*CurrentGroupName = NewGroupName;
					UE_LOG(LogMaterialEditingLibrary, Verbose, TEXT("Moved %s to %s."), *Expression->GetClass()->GetName(), *NewGroupName.ToString());
					NumModified++;
				}
			}
		}
	...
	if (NumModified > 0)
	{
		UE_LOG(LogMaterialEditingLibrary, Log, TEXT("Moved %d parameters from %s into group %s"), NumModified, *OldGroupName.ToString(), *NewGroupName.ToString());
	}
	else
	{
		UE_LOG(LogMaterialEditingLibrary, Log, TEXT("No parameters found with the group name: %s"), *OldGroupName.ToString());
	}

...
void FStatsGridRow_NumVirtualTextureStacks::AddPlatform(TSharedPtr<FMaterialStats> StatsManager, const TSharedPtr<FShaderPlatformSettings> Platform, const EMaterialQualityLevel::Type QualityLevel, const int32 InstanceIndex)
{
	// cell that will enumerate the total number of texture samples in this material
	const FString CellContent = FMaterialStatsUtils::MaterialQualityToShortString(QualityLevel);
	TSharedPtr<FGridCell_ShaderValue> Cell = MakeShareable(new FGridCell_ShaderValue(StatsManager, EShaderInfoType::VirtualTextureStackCount, ERepresentativeShader::Num, QualityLevel, Platform->GetPlatformShaderType(), InstanceIndex));
...
{
	// reset all rows to rebuild columns
	BuildStaticRows();
	OnShaderChanged();
}
...
FString FGridCell_ShaderValue::InternalGetContent(bool bLongContent)
{
	...
	switch (InfoType)
	{
		...
		case EShaderInfoType::VirtualTextureStackCount:
			return bLongContent ? InstanceData.ShaderStatsInfo.VirtualTextureStackCount.StrDescriptionLong : InstanceData.ShaderStatsInfo.VirtualTextureStackCount.StrDescription;

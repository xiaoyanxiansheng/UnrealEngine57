...
public:
	SLATE_BEGIN_ARGS( SMaterialEditor3DPreviewViewport )
		: _PreviewMaterial(nullptr){}
		SLATE_ARGUMENT(TWeakPtr<IMaterialEditor>, MaterialEditor)
		SLATE_ARGUMENT(UMaterialInterface*, PreviewMaterial)
	SLATE_END_ARGS()
...
private:
	/** If we have a MaterialEditor, it will look up the preview material from that. Otherwise it will fallback to previewing this material */
	UMaterialInterface* GetPreviewingMaterialInterface();
	...
	/** The material we're previewing if we're not hosted by a material editor. Used for diffs. */
	TWeakObjectPtr<UMaterialInterface> PreviewMaterialPtr;

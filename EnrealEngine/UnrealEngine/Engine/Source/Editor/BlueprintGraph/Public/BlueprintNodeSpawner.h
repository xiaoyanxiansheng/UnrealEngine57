...
class UBlueprintNodeSpawner : public UObject, public IBlueprintNodeBinder
{
...
public:
	 ...
	 */
	static UE_API UBlueprintNodeSpawner* Create(TSubclassOf<UEdGraphNode> const NodeClass, UObject* Outer = nullptr, FCustomizeNodeDelegate PostSpawnDelegate = FCustomizeNodeDelegate());
...
public:
	 ...
	 */
	UE_API FBlueprintActionUiSpec const& PrimeDefaultUiSpec(UEdGraph* TargetGraph = nullptr) const;
	 ...
	 */
	UE_API virtual FBlueprintNodeSignature GetSpawnerSignature() const;
	 ...
	 */
	UE_API virtual FBlueprintActionUiSpec GetUiSpec(FBlueprintActionContext const& Context, FBindingSet const& Bindings) const;
	 ...
	 */
	UE_API virtual UEdGraphNode* Invoke(UEdGraph* ParentGraph, FBindingSet const& Bindings, FVector2D const Location) const;
	 ...
	 */
	UE_API UEdGraphNode* GetCachedTemplateNode() const;
	 ...
	 */
	UE_API UEdGraphNode* GetTemplateNode(UEdGraph* TargetGraph = nullptr, FBindingSet const& Bindings = FBindingSet()) const;
	 ...
	 */
	UE_API void ClearCachedTemplateNode() const;

	/**
	 * Determine if the node of this spawner's template's type should be filtered in the actions menu
	 */
	UE_API virtual bool IsTemplateNodeFilteredOut(FBlueprintActionFilter const& Filter) const;
...
private:
	 ...
	 * @return Null if it failed to spawn a node (if NodeClass is null), otherwise a newly spawned node.
	 */
	UE_API UEdGraphNode* SpawnEdGraphNode(TSubclassOf<UEdGraphNode> NodeClass, UEdGraph* ParentGraph, const FBindingSet& Bindings, FVector2D Location, FCustomizeNodeDelegate PostSpawnDelegate) const;
};

...
public:
	...
	/** If true we'll automatically search for references to a variable when its type is changed */
	UPROPERTY(EditAnywhere, config, Category = FindInBlueprints)
	bool bSearchForReferencesWhenVariableTypeChanged;
	 ...
	 * (This setting exists both as an editor preference and project setting, and will be allowed if listed in either place) 
	 */
	UPROPERTY(EditAnywhere, config, Category=Play, meta=(AllowAbstract, ShowObjectRootClass))
	...
	UE_API void RegisterIsClassAllowedDelegate(const FName OwnerName, FOnIsClassAllowed Delegate);
	UE_API void UnregisterIsClassAllowedDelegate(const FName OwnerName);
	UE_API bool IsClassAllowed(const UClass* InClass) const;
	bool HasClassFiltering() const { return IsClassAllowedDelegates.Num() > 0; }

	UE_API void RegisterIsClassPathAllowedDelegate(const FName OwnerName, FOnIsClassPathAllowed Delegate);
	UE_API void UnregisterIsClassPathAllowedDelegate(const FName OwnerName);
	UE_API bool IsClassPathAllowed(const FTopLevelAssetPath& InClassPath) const;
	bool HasClassPathFiltering() const { return IsClassPathAllowedDelegates.Num() > 0; }

	UE_API void RegisterIsClassAllowedOnPinDelegate(const FName OwnerName, FOnIsClassAllowed Delegate);
	UE_API void UnregisterIsClassAllowedOnPinDelegate(const FName OwnerName);
	UE_API bool IsClassAllowedOnPin(const UClass* InClass) const;
	bool HasClassOnPinFiltering() const { return IsClassAllowedOnPinDelegates.Num() > 0; }

	UE_API void RegisterIsClassPathAllowedOnPinDelegate(const FName OwnerName, FOnIsClassPathAllowed Delegate);
	UE_API void UnregisterIsClassPathAllowedOnPinDelegate(const FName OwnerName);
	UE_API bool IsClassPathAllowedOnPin(const FTopLevelAssetPath& InClassPath) const;
	...
	UE_API void RegisterIsFunctionAllowedForAssetDelegate(const FName OwnerName, FOnIsFunctionAllowedForAsset Delegate);
	UE_API void UnregisterIsFunctionAllowedForAssetDelegate(const FName OwnerName);

	UE_API bool IsFunctionAllowed(const UBlueprint* InBlueprint, const FName InFunctionName) const;

	UE_API bool IsFunctionAllowedForAsset(const UBlueprint* InBlueprint, const FName InFunctionName) const;

	static UE_API bool IsFunctionAllowedForPermissions(const UBlueprint* InBlueprint, const FName FunctionName, const FPathPermissionList& InFunctionPermissions);
...
protected:
	...
	UE_API virtual void PostInitProperties();
	UE_API virtual void PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent) override;
	//~ End UObject Interface
	
	UE_API void OnAssetRenamed(FAssetData const& AssetInfo, const FString& InOldName);

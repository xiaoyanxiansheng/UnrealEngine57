...
{
	/** */
	UE_API FBlueprintActionInfo(UObject const* ActionOwner, UBlueprintNodeSpawner const* Action);

	/** */
	UE_API FBlueprintActionInfo(FBlueprintActionInfo const& Rhs, IBlueprintNodeBinder::FBindingSet const& Bindings);
	 ...
	 */
	UE_API UObject const* GetActionOwner() const;
	 ...
	 */
	UE_API IBlueprintNodeBinder::FBindingSet const& GetBindings() const;
	 ...
	 */
	UE_API UClass const* GetOwnerClass();
	 ...
	 */
	UE_API UClass const* GetNodeClass();
	 ...
	 */
	UE_API FFieldVariant GetAssociatedMemberField();
	 ...
	 */
	UE_API FProperty const* GetAssociatedProperty();
	 ...
	 */
	UE_API UFunction const* GetAssociatedFunction();
...
struct FActionFilterTest : TSharedFromThis<FActionFilterTest>
{
	static UE_API TAutoConsoleVariable<bool> CVarEnableCaching;
	...
	UE_API FActionFilterTest(const FRejectionDelegate Delegate, FString Name, EActionFilterTestFlags Flags = EActionFilterTestFlags::Default);
	...
	UE_API bool Call(FBlueprintActionFilter const& Filter, FBlueprintActionInfo& ActionInfo);
	UE_API bool operator==(const FActionFilterTest& other) const;
...
public:
	...
	UE_API FBlueprintActionFilter(const EFlags InFlags = BPFILTER_NoFlags);

	UE_DEPRECATED(5.1, "Please use the version that takes the EFlags type as input")
	UE_API FBlueprintActionFilter(uint32 const Flags);
	...
	static UE_API void AddUnique(TArray<FTargetClassFilterData>& ToArray, UClass* TargetClass);

	/** Helper to add a class to the TargetClasses, fills out FTargetClassFilterData */
	static UE_API void Add(TArray<FTargetClassFilterData>& ToArray, UClass* TargetClass);
	 ...
	 */
	UE_API void AddRejectionTest(FRejectionTestDelegate RejectionTestDelegate);
	 ...
	 */
	UE_API void AddRejectionTest(TSharedRef<FActionFilterTest> RejectionTest);
	 ...
	 */
	UE_API bool IsFiltered(FBlueprintActionInfo& BlueprintAction);
	 ...
	 * @return This.
	 */
	UE_API FBlueprintActionFilter const& operator|=(FBlueprintActionFilter const& Rhs);
	...
	static UE_API bool IsFilterTestTraceLoggingEnabled();
	static UE_API bool IsFilterTestStatsLoggingEnabled();
	UE_API TArray<FString> GetFilterTestProfile();
...
private:
	 ...
	 */
	UE_API bool IsFilteredByThis(FBlueprintActionInfo& BlueprintAction) const;
	...
	UE_API FFilterTestProfileRecord* BeginFilterTestProfileEvent(int32 FilterTestIndex, uint64 FilterTestFuncPtr) const;
	UE_API void EndFilterTestProfileEvent(FFilterTestProfileRecord* InEvent, bool bWasFiltered, double StartTime) const;
#endif	// ENABLE_BLUEPRINT_ACTION_FILTER_PROFILING
};

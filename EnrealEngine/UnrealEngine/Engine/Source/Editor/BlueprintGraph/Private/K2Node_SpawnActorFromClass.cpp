...
#include UE_INLINE_GENERATED_CPP_BY_NAME(K2Node_SpawnActorFromClass)
	...
	for (UEdGraphPin* Pin : OldPins)
	{
		if (Pin->PinName == FK2Node_SpawnActorFromClassHelper::NoCollisionFailPinName || Pin->PinName == FK2Node_SpawnActorFromClassHelper::SpawnEvenIfCollidingPinName)
		{
			...
			if (Pin->LinkedTo.Num() == 0)
			{
				...
				{
					UEnum const* const MethodEnum = FindObjectChecked<UEnum>(nullptr, TEXT("/Script/Engine.ESpawnActorCollisionHandlingMethod"), EFindObjectFlags::ExactClass);
			...
			else
			{
				...
				UEnum* const MethodEnum = FindObjectChecked<UEnum>(nullptr, TEXT("/Script/Engine.ESpawnActorCollisionHandlingMethod"), EFindObjectFlags::ExactClass);

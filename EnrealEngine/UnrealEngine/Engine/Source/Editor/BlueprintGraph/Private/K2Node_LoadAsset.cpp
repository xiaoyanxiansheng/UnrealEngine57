...
#include UE_INLINE_GENERATED_CPP_BY_NAME(K2Node_LoadAsset)
...
void UK2Node_LoadAsset::ExpandNode(class FKismetCompilerContext& CompilerContext, UEdGraph* SourceGraph)
{
	...
	if (AssetPin)
	{
		PinContainerType = AssetPin->PinType.ContainerType;
	}
	UK2Node_TemporaryVariable* TempVarOutput = CompilerContext.SpawnInternalVariable(this, GetOutputCategory(), NAME_None, UObject::StaticClass(), PinContainerType);
	...
	OnLoadEventNode->AllocateDefaultPins();
	{
		...
		UFunction* OnLoadedSignature = OnLoadDelegateProperty ? OnLoadDelegateProperty->SignatureFunction.Get() : nullptr;
...
// UK2Node_LoadAssets
void UK2Node_LoadAssets::AllocateDefaultPins()
{
	CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Exec, UEdGraphSchema_K2::PN_Execute);
	// The immediate continue pin
	CreatePin(EGPD_Output, UEdGraphSchema_K2::PC_Exec, UEdGraphSchema_K2::PN_Then);
	// The delayed completed pin, this used to be called Then
	CreatePin(EGPD_Output, UEdGraphSchema_K2::PC_Exec, UEdGraphSchema_K2::PN_Completed);
	FCreatePinParams ArrayParams;
	ArrayParams.ContainerType = EPinContainerType::Array;
	CreatePin(EGPD_Input, GetInputCategory(), UObject::StaticClass(), GetInputPinName(), ArrayParams);
	CreatePin(EGPD_Output, GetOutputCategory(), UObject::StaticClass(), GetOutputPinName(), ArrayParams);
}

FText UK2Node_LoadAssets::GetTooltipText() const
{
	return FText(LOCTEXT("UK2Node_LoadAssetsGetTooltipText", "Asynchronously loads a list Soft Object References and returns the objects of the correct type if the load succeeds"));
}

FText UK2Node_LoadAssets::GetNodeTitle(ENodeTitleType::Type TitleType) const
{
	return FText(LOCTEXT("UK2Node_LoadAssetsGetNodeTitle", "Async Load Assets"));
}

FName UK2Node_LoadAssets::NativeFunctionName() const
{
	return GET_FUNCTION_NAME_CHECKED(UKismetSystemLibrary, LoadAssets);
}

const FName& UK2Node_LoadAssets::GetInputPinName() const
{
	static const FName InputAssetPinName("Assets");
	return InputAssetPinName;
}

const FName& UK2Node_LoadAssets::GetOutputPinName() const
{
	static const FName OutputObjectPinName("Objects");
	return OutputObjectPinName;
}

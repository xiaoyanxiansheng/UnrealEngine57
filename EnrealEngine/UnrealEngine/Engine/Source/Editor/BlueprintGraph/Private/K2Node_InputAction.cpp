...
#include UE_INLINE_GENERATED_CPP_BY_NAME(K2Node_InputAction)
...
bool UK2Node_InputAction::IsDeprecated() const
{
	const UInputSettings* Settings = GetDefault<UInputSettings>();
	return Super::IsDeprecated() || (Settings->bEnabledLegacyMappingDeprecationWarnings && Settings->IsActionMappingDeprecated(InputActionName));
}

bool UK2Node_InputAction::HasDeprecatedReference() const
{
	...
	return GetDefault<UInputSettings>()->IsActionMappingDeprecated(InputActionName);	
}

FEdGraphNodeDeprecationResponse UK2Node_InputAction::GetDeprecationResponse(EEdGraphNodeDeprecationType DeprecationType) const
{
	...
	if (DeprecationType == EEdGraphNodeDeprecationType::NodeHasDeprecatedReference)
	{
		...
		FFormatNamedArguments Args;
		Args.Add(TEXT("ActionName"), FText::FromName(InputActionName));
		Response.MessageText = FText::Format(LOCTEXT("ActionMappingDeprecationWarning", "The Action Mapping '{ActionName}' has been deprecated."), Args);
	}
...
void UK2Node_InputAction::GetMenuActions(FBlueprintActionDatabaseRegistrar& ActionRegistrar) const
{
	...
	if (ActionRegistrar.IsOpenForRegistration(ActionKey))
	{
		...
		for (const FName& ActionName : ActionNames)
		{
			...
			if (Settings->bEnabledLegacyMappingDeprecationWarnings && Settings->IsActionMappingDeprecated(ActionName))
			{
				continue;
			}

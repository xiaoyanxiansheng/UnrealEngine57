...
static void BlueprintActionDatabaseImpl::AddClassFunctionActions(UClass const* const Class, FActionList& ActionListOut)
{
	...
	for (TFieldIterator<UFunction> FunctionIt(Class, EFieldIteratorFlags::ExcludeSuper); FunctionIt; ++FunctionIt)
	{
		...
		if (UEdGraphSchema_K2::CanUserKismetCallFunction(Function))
		{
			...
			// If an interface class has specified "CannotGenerateMessageNodes=true" in its metadata, then don't create any message nodes for
			// this interface class.
			if (FKismetEditorUtilities::IsClassABlueprintInterface(Class) && !Class->GetBoolMetaData(TEXT("CannotGenerateMessageNodes")))
			{

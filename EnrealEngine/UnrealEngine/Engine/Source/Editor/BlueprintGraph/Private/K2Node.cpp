...
#include UE_INLINE_GENERATED_CPP_BY_NAME(K2Node)
...
void UK2Node::ReconstructNode()
{
	...
	// Make sure we have a valid Blueprint owner, so we don't crash the editor attempting to access it below.
	if (!ensureMsgf(HasValidBlueprint(), TEXT("UK2Node::ReconstructNode(): Attempting to reconstruct [%s] without a valid Blueprint owner (this is unexpected)."), *GetFullName()))
	{
		return;
	}
				...
				if (Schema->ConvertPropertyToPinType(InnerProperty, /*out*/ PinType))
				{
					...
					for (int32 Index = 0; Index < ArrayHelper.Num(); ++Index)
					{
						...
						if (PropertyEntry.bShowPin)
						{
							...
							if (UEdGraphSchema_K2::IsPropertyDeprecated(OuterProperty))
							{
								NewPin->bDeprecated = true;
								NewPin->DeprecationMessage = OuterProperty->GetMetaData(FBlueprintMetadata::MD_DeprecationMessage);
							}

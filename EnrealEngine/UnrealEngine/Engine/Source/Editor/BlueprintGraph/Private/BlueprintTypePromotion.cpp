...
bool FTypePromotion::IsValidPromotion(const FEdGraphPinType& A, const FEdGraphPinType& B)
{
	...
	if (A.PinCategory == UEdGraphSchema_K2::PC_Struct || B.PinCategory == UEdGraphSchema_K2::PC_Struct)
	{
		...
		const TOptional<UEdGraphSchema_K2::FSearchForAutocastFunctionResults> Res = K2Schema->SearchForAutocastFunction(A, B);
		
		const UFunction* AutoFunc =
			Res.IsSet() ?
			Res->FunctionOwner->FindFunctionByName(Res->TargetFunction) :
			nullptr;

		// We can only use this conversion function for promotion if it is not marked to ignore it
		return AutoFunc != nullptr && !AutoFunc->HasMetaData(FBlueprintMetadata::MD_IgnoreTypePromotion);
	}
	else
	{

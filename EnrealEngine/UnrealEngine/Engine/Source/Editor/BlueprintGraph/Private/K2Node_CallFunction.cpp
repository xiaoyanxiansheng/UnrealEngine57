...
#include UE_INLINE_GENERATED_CPP_BY_NAME(K2Node_CallFunction)
...
void UK2Node_CallFunction::GeneratePinTooltipFromFunction(UEdGraphPin& Pin, const UFunction* Function)
{
	...
	const FString ParamTooltip = bReturnPin ? ObjectTools::GetDefaultTooltipForFunctionReturn(Function) : ObjectTools::GetDefaultTooltipForFunctionParam(Function, Pin.PinName.ToString());
	Pin.PinToolTip += ParamTooltip;
...
void UK2Node_CallFunction::ValidateNodeDuringCompilation(class FCompilerResultsLog& MessageLog) const
{
	...
	if (Function)
	{
			...
			(!Function->HasMetaData(FBlueprintMetadata::MD_CallableWithoutWorldContext)))
		{
			...
			// go through all of the pins and verify if we have a connected world context or not.
			const FName ContextParam = FName(*Function->GetMetaData(FBlueprintMetadata::MD_WorldContext));
			bool bConnectedWorldContext = false;
			for (const UEdGraphPin* Pin : Pins)
			{
				if (Pin->PinName.IsEqual(ContextParam) && Pin->LinkedTo.Num() > 0)
				{
					bConnectedWorldContext = true;
					break;
				}
			}

			if (!bConnectedWorldContext && ParentClass && !FBlueprintEditorUtils::ImplementsGetWorld(Blueprint) && !ParentClass->HasMetaDataHierarchical(FBlueprintMetadata::MD_ShowWorldContextPin))
			{
				MessageLog.Warning(*LOCTEXT("FunctionUnsafeInContext", "Function '@@' is unsafe to call from blueprints of class '@@' without connecting a World Context Object.").ToString(), this, ParentClass);
			}
		}
...
void UK2Node_CallFunction::ExpandNode(class FKismetCompilerContext& CompilerContext, UEdGraph* SourceGraph)
{
	...
	if (SourceGraph && Schema->IsStaticFunctionGraph(SourceGraph) && Function)
	{
		...
		else if (UEdGraphPin* BetterSelfPin = EntryPoints[0]->GetAutoWorldContextPin())
		{
			...
			// Attempt to connect, even if it's optional because of MD_CallableWithoutWorldContext
			if (!WorldContextMetaValue.IsEmpty())
			{

...
#include UE_INLINE_GENERATED_CPP_BY_NAME(EdGraphSchema_K2)
...
bool UEdGraphSchema_K2::CanFunctionBeUsedInGraph(const UClass* InClass, const UFunction* InFunction, const UEdGraph* InDestGraph, uint32 InAllowedFunctionTypes, bool bInCalledForEach, FText* OutReason) const
{
	if (CanUserKismetCallFunction(InFunction))
	{
		...
		// We no longer block functions that require world contexts as it will add the pin when needed
...
bool UEdGraphSchema_K2::IsPropertyDeprecated(const FProperty* Property)
{
	Property = FBlueprintEditorUtils::GetMostUpToDateProperty(Property);
	if (Property)
	{
		const bool bMeta = Property->HasMetaData(FBlueprintMetadata::MD_DeprecatedProperty);
		const bool bFlag = Property->HasAllPropertyFlags(CPF_Deprecated);
		return bMeta || bFlag;
	}
	return false;
}
		...
		if (StructType && !bDisableSplit)
		{
			if (InGraphPin->Direction == EGPD_Input)
			{
				...
				if (!bCanSplit)
				{
					...
					UFunction* Function = FindObject<UFunction>(nullptr, *MetaData, EFindObjectFlags::ExactClass);
...
void UEdGraphSchema_K2::GetContextMenuActions(UToolMenu* Menu, UGraphNodeContextMenuContext* Context) const
{
	...
	else if (InGraphNode != NULL)
	{
		...
		else
		{
			...
			{
				FToolMenuSection& Section = Menu->AddSection("EdGraphSchemaNodeNavigation", LOCTEXT("NodeNavigationMenuHeader", "Navigation"));
				// Opens a function, macro or collapsed graph in a new tab
				Section.AddMenuEntry(FGraphEditorCommands::Get().OpenInNewTab);
			}
...
void UEdGraphSchema_K2::ConstructBasicPinTooltip(const UEdGraphPin& Pin, const FText& PinDescription, FString& TooltipOut) const
{
	...
	else
	{
		...
		if (Pin.bDeprecated && !Pin.DeprecationMessage.IsEmpty())
		{
			Args.Add(TEXT("DeprecationWarning"), FText::FromString(Pin.DeprecationMessage));
			Args.Add(TEXT("LineFeed1"), FText::FromString(TEXT("\n")));
		}
		else if (Pin.bDeprecated)
		{
			Args.Add(TEXT("DeprecationWarning"), LOCTEXT("Deprecated", "Deprecated"));
			Args.Add(TEXT("LineFeed1"), FText::FromString(TEXT("\n")));
		}
		else
		{
			Args.Add(TEXT("DeprecationWarning"), FText::GetEmpty());
		...
		if (!PinDescription.IsEmpty())
		{
			...
			Args.Add(TEXT("LineFeed3"), FText::FromString(TEXT("\n\n")));
		}
		else
		{
			Args.Add(TEXT("Description"), FText::GetEmpty());
			Args.Add(TEXT("LineFeed3"), FText::GetEmpty());
		}
	
		TooltipOut = FText::Format(LOCTEXT("PinTooltip", "{DeprecationWarning}{LineFeed1}{DisplayName}{LineFeed2}{PinType}{LineFeed3}{Description}"), Args).ToString(); 
	}
}
...
{
	UBlueprint* Blueprint = FBlueprintEditorUtils::FindBlueprintForGraph(Graph);
	TSharedPtr<IAssetReferenceFilter> AssetReferenceFilter = MakeAssetReferenceFilter(Blueprint);
	...
	if (FBlueprintEditorUtils::IsEventGraph(Graph))
	{
		...
		for (const FAssetData& AssetData : Assets)
		{
			...
			{
				if (!AssetReferenceFilter || AssetReferenceFilter->PassesFilter(AssetData))
				{
	...
	if ((Blueprint != nullptr) && FBlueprintEditorUtils::IsActorBased(Blueprint))
	{
		...
		for(int32 AssetIdx=0; AssetIdx < Assets.Num(); AssetIdx++)
		{
			...
			{
				if (AssetReferenceFilter.IsValid() && !AssetReferenceFilter->PassesFilter(Assets[AssetIdx]))
				{
					continue;
				}
...
void UEdGraphSchema_K2::DroppedAssetsOnPin(const TArray<FAssetData>& Assets, const FVector2f& GraphPosition, UEdGraphPin* Pin) const
{
	...
	if ((Pin->PinType.PinCategory == PC_Object) || (Pin->PinType.PinCategory == PC_Interface))
	{
		...
		if(PinClass != nullptr)
		{
			...
			{
				TSharedPtr<IAssetReferenceFilter> AssetReferenceFilter = MakeAssetReferenceFilter(FBlueprintEditorUtils::FindBlueprintForNode(Pin->GetOwningNode()));
				if (!AssetReferenceFilter || AssetReferenceFilter->PassesFilter(AssetData))
				{
					UObject* Asset = AssetData.GetAsset(); //-V758
					if (Asset != nullptr)
					{
...
{ 
	...
	if ((HoverPin->PinType.PinCategory == PC_Object) || (HoverPin->PinType.PinCategory == PC_Interface))
	{
		...
		if(PinClass != nullptr)
		{
			...
			if(AssetData.IsValid())
			{
				TSharedPtr<IAssetReferenceFilter> AssetReferenceFilter = MakeAssetReferenceFilter(FBlueprintEditorUtils::FindBlueprintForNode(HoverPin->GetOwningNode()));
				FText FailureReason;
				if (AssetReferenceFilter.IsValid() && !AssetReferenceFilter->PassesFilter(AssetData, &FailureReason))
				{
				...
				{
					OutOkIcon = true;
					OutTooltipText = FText::FormatOrdered(LOCTEXT("SupportedAssetTypeForPinDragDrop", "Assign {0} to this pin"), FText::FromName(AssetData.AssetName)).ToString();
				}
			}
			else
			{
				OutTooltipText = LOCTEXT("UnsupportedAssetTypeForPinDragDrop", "Not compatible with this pin").ToString();
			}
		}
	}
}
	...
	if ((Blueprint != nullptr) && FBlueprintEditorUtils::IsActorBased(Blueprint))
	{
		...
		for (const FAssetData& AssetData : Assets)
		{
			...
			if (const FAssetBlueprintGraphActions* GraphActions = Module.GetAssetBlueprintGraphActions(AssetData.GetClass()))
			{
				...
				{
					OutTooltipText = GraphActions->GetGraphHoverMessage(AssetData, HoverGraph).ToString();
					OutOkIcon = true;
					return;
				}
				else
...
UK2Node* UEdGraphSchema_K2::CreateSplitPinNode(UEdGraphPin* Pin, const FCreateSplitPinNodeParams& Params) const
{
	...
	if (Pin->Direction == EGPD_Input)
	{
		...
		{
			const FString& MetaData = StructType->GetMetaData(FBlueprintMetadata::MD_NativeMakeFunction);
			const UFunction* Function = FindObject<UFunction>(nullptr, *MetaData, EFindObjectFlags::ExactClass);
...
void UEdGraphSchema_K2::SplitPin(UEdGraphPin* Pin, const bool bNotify) const
{
	...
	for (UEdGraphPin* ProtoPin : ProtoExpandNode->Pins)
	{
		if (ProtoPin->Direction == Pin->Direction && !ProtoPin->bHidden)
		{
			...
			SubPin->bDeprecated = ProtoPin->bDeprecated;
			SubPin->DeprecationMessage = ProtoPin->DeprecationMessage;
...
void UEdGraphSchema_K2::HandleParameterDefaultValueChanged(UK2Node* InTargetNode) const
{
	if (UK2Node_EditablePinBase* EditablePinNode = Cast<UK2Node_EditablePinBase>(InTargetNode))
	{
		...
		// Make sure we have a valid Blueprint owner before proceeding, so we don't crash the editor attempting to modify it below.
		UBlueprint* TargetBlueprint = FBlueprintEditorUtils::FindBlueprintForNode(EditablePinNode);
		if (!ensureMsgf(TargetBlueprint, TEXT("UEdGraphSchema_K2::HandleParameterDefaultValueChanged(): Failed to find a Blueprint for target node: %s"), *GetPathNameSafe(EditablePinNode)))
		{
			return;
		}

		// Make sure we have a valid outer graph object before proceeding, so we don't crash the editor attempting to modify it below.
		UEdGraph* EditablePinNodeGraph = EditablePinNode->GetGraph();
		if (!ensureMsgf(EditablePinNodeGraph, TEXT("UEdGraphSchema_K2::HandleParameterDefaultValueChanged(): Failed to find a valid graph for target node: %s"), *GetPathNameSafe(EditablePinNode)))
		{
			return;
		}

		FParamsChangedHelper ParamsChangedHelper;
		ParamsChangedHelper.ModifiedBlueprints.Add(TargetBlueprint);
		FBlueprintEditorUtils::MarkBlueprintAsStructurallyModified(TargetBlueprint);

		ParamsChangedHelper.Broadcast(TargetBlueprint, EditablePinNode, EditablePinNodeGraph);
...
TSharedPtr<IAssetReferenceFilter> UEdGraphSchema_K2::MakeAssetReferenceFilter(const UBlueprint* Blueprint)
{
	if (GEditor && Blueprint)
	{
		FAssetReferenceFilterContext AssetReferenceFilterContext;
		AssetReferenceFilterContext.AddReferencingAsset(Blueprint);

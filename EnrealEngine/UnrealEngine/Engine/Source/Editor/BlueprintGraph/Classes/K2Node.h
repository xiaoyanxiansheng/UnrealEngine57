...
public:
	...
	UE_API virtual void GetRecordDefaults(FProperty* TestProperty, FOptionalPinFromProperty& Record) const;

	/** Can this property be managed as an optional pin (with the ability to be shown or hidden) */
	UE_API virtual bool CanTreatPropertyAsOptional(FProperty* TestProperty) const;
	 ...
	 */
	UE_API void RebuildPropertyList(TArray<FOptionalPinFromProperty>& Properties, UStruct* SourceStruct);
	 ...
	 */
	UE_API void CreateVisiblePins(TArray<FOptionalPinFromProperty>& Properties, UStruct* SourceStruct, EEdGraphPinDirection Direction, class UK2Node* TargetNode, uint8* StructBasePtr = nullptr, uint8* DefaultsPtr = nullptr);
	...
	static UE_API void CacheShownPins(const TArray<FOptionalPinFromProperty>& OptionalPins, TArray<FName>& OldShownPins);

	/** Helper function to make consistent behavior between nodes that use optional pins */
	static UE_API void EvaluateOldShownPins(const TArray<FOptionalPinFromProperty>& OptionalPins, TArray<FName>& OldShownPins, UK2Node* Node);
...
protected:
	...
	UE_API void RebuildProperty(FProperty* TestProperty, FName CategoryName, TArray<FOptionalPinFromProperty>& Properties, UStruct* SourceStruct, TMap<FName, FOldOptionalPinSettings>& OldSettings);
};
...
public:
	...
	virtual void ClearCachedBlueprintData(UBlueprint* Blueprint) {}
	...
	virtual UClass* GetDynamicBindingClass() const { return NULL; }

	// Puts information about this node into the dynamic binding object
	virtual void RegisterDynamicBinding(UDynamicBlueprintBinding* BindingObject) const { };
	...
	virtual int32 GetNodeRefreshPriority() const { return EBaseNodeRefreshPriority::Normal; }

	BLUEPRINTGRAPH_API bool DoesWildcardPinAcceptContainer(const UEdGraphPin* Pin);
	virtual bool DoesInputWildcardPinAcceptArray(const UEdGraphPin* Pin) const { return true; }
	virtual bool DoesOutputWildcardPinAcceptContainer(const UEdGraphPin* Pin) const { return true; }
...
protected:
	...
	void PreloadObject(UObject* ReferencedObject)
	{
		if (ReferencedObject)
		{
			ReferencedObject->ConditionalPreload();
		}
	}

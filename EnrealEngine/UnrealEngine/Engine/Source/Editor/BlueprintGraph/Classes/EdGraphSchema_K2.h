...
public:
	...
	static UE_API const FName MD_AllowableBlueprintVariableType;

	// If true, this class, struct, or enum is not valid for use as a variable in a blueprint
	static UE_API const FName MD_NotAllowableBlueprintVariableType;

	// Class:
	// [ClassMetadata] If present, the component class can be spawned by a blueprint
	static UE_API const FName MD_BlueprintSpawnableComponent;

	/** If true, the class will be usable as a base for blueprints */
	static UE_API const FName MD_IsBlueprintBase;
	...
	static UE_API const FName MD_RestrictedToClasses;
	...
	static UE_API const FName MD_ChildCanTick;
	...
	static UE_API const FName MD_WorldContext;
	...
	static UE_API const FName MD_CallableWithoutWorldContext;

	/** For functions that should be compiled in development mode only */
	static UE_API const FName MD_DevelopmentOnly;
	...
	static UE_API const FName MD_AutoCreateRefTerm;

	UE_DEPRECATED(5.6, "Use MD_HidePinAssetPicker instead.")
	static UE_API const FName MD_HideAssetPicker;
...
struct FAllowedMutableContainerFunction
{
	...
	UPROPERTY()
	FString ContainerParameterName;

	friend uint32 GetTypeHash(const FAllowedMutableContainerFunction& Input)
	{
		return GetTypeHash(Input.Function);
	}

	bool operator==(const FAllowedMutableContainerFunction& RHS) const
	{
...
class UEdGraphSchema_K2 : public UEdGraphSchema
{
	...
	static UE_API const FName PC_Exec;
	static UE_API const FName PC_Boolean;
	static UE_API const FName PC_Byte;
	static UE_API const FName PC_Class;    // SubCategoryObject is the MetaClass of the Class passed thru this pin, or SubCategory can be 'self'. The DefaultValue string should always be empty, use DefaultObject.
	static UE_API const FName PC_SoftClass;
	static UE_API const FName PC_Int;
	static UE_API const FName PC_Int64;
	static UE_API const FName PC_Float;
	static UE_API const FName PC_Double;
	static UE_API const FName PC_Real;
	static UE_API const FName PC_Name;
	static UE_API const FName PC_Delegate;    // SubCategoryObject is the UFunction of the delegate signature
	static UE_API const FName PC_MCDelegate;  // SubCategoryObject is the UFunction of the delegate signature
	static UE_API const FName PC_Object;    // SubCategoryObject is the Class of the object passed thru this pin, or SubCategory can be 'self'. The DefaultValue string should always be empty, use DefaultObject.
	static UE_API const FName PC_Interface;	// SubCategoryObject is the Class of the object passed thru this pin.
	static UE_API const FName PC_SoftObject;		// SubCategoryObject is the Class of the AssetPtr passed thru this pin.
	static UE_API const FName PC_String;
	static UE_API const FName PC_Text;
	static UE_API const FName PC_Struct;    // SubCategoryObject is the ScriptStruct of the struct passed thru this pin, 'self' is not a valid SubCategory. DefaultObject should always be empty, the DefaultValue string may be used for supported structs.
	static UE_API const FName PC_Wildcard;    // Special matching rules are imposed by the node itself
	static UE_API const FName PC_Enum;    // SubCategoryObject is the UEnum object passed thru this pin
	static UE_API const FName PC_FieldPath;		// SubCategoryObject is the Class of the property passed thru this pin.

	// Common PinType.PinSubCategory values
	static UE_API const FName PSC_Self;    // Category=PC_Object or PC_Class, indicates the class being compiled

	static UE_API const FName PSC_Index;	// Category=PC_Wildcard, indicates the wildcard will only accept Int, Bool, Byte and Enum pins (used when a pin represents indexing a list)
	static UE_API const FName PSC_Bitmask;	// Category=PC_Byte or PC_Int, indicates that the pin represents a bitmask field. SubCategoryObject is either NULL or the UEnum object to which the bitmap is linked for bitflag name specification.

	// Pin names that have special meaning and required types in some contexts (depending on the node type)
	static UE_API const FName PN_Execute;    // Category=PC_Exec, singleton, input
	static UE_API const FName PN_Then;    // Category=PC_Exec, singleton, output
	static UE_API const FName PN_Completed;    // Category=PC_Exec, singleton, output
	static UE_API const FName PN_DelegateEntry;    // Category=PC_Exec, singleton, output; entry point for a dynamically bound delegate
	static UE_API const FName PN_EntryPoint;	// entry point to the ubergraph
	static UE_API const FName PN_Self;    // Category=PC_Object, singleton, input
	static UE_API const FName PN_Else;    // Category=PC_Exec, singleton, output
	static UE_API const FName PN_Loop;    // Category=PC_Exec, singleton, output
	static UE_API const FName PN_After;    // Category=PC_Exec, singleton, output
	static UE_API const FName PN_ReturnValue;		// Category=PC_Object, singleton, output
	static UE_API const FName PN_ObjectToCast;    // Category=PC_Object, singleton, input
	static UE_API const FName PN_Condition;    // Category=PC_Boolean, singleton, input
	static UE_API const FName PN_Start;    // Category=PC_Int, singleton, input
	static UE_API const FName PN_Stop;    // Category=PC_Int, singleton, input
	static UE_API const FName PN_Index;    // Category=PC_Int, singleton, output
	static UE_API const FName PN_Item;    // Category=PC_Int, singleton, output
	static UE_API const FName PN_CastSucceeded;    // Category=PC_Exec, singleton, output
	static UE_API const FName PN_CastFailed;    // Category=PC_Exec, singleton, output
	static UE_API const FString PN_CastedValuePrefix;    // Category=PC_Object, singleton, output; actual pin name varies depending on the type to be casted to, this is just a prefix

	// construction script function names
	static UE_API const FName FN_UserConstructionScript;
	static UE_API const FName FN_ExecuteUbergraphBase;
	...
	// graph names
	static UE_API const FName GN_EventGraph;
	static UE_API const FName GN_AnimGraph;

	// variable names
	static UE_API const FText VR_DefaultCategory;

	// action grouping values
	static UE_API const int32 AG_LevelReference;
	...
	static UE_API bool bGeneratingDocumentation;
	...
	static UE_API const FName AllObjectTypes;

	UPROPERTY(globalconfig)
	TArray<FBlueprintCallableFunctionRedirect> EditoronlyBPFunctionRedirects;

	UPROPERTY(globalconfig)
	bool bEnableMutableContainerWarnings;

	UPROPERTY(globalconfig)
	TSet<FAllowedMutableContainerFunction> AllowedMutableContainerFunctions;
...
public:
	...
	public:
		UE_API const FEdGraphPinType& GetPinType(bool bForceLoadedSubCategoryObject);
		...
		UE_API FPinTypeTreeInfo(const FText& InFriendlyName, const FName CategoryName, const UEdGraphSchema_K2* Schema, const FText& InTooltip, bool bInReadOnly = false);
		UE_API FPinTypeTreeInfo(const FName CategoryName, UObject* SubCategoryObject, const FText& InTooltip, bool bInReadOnly = false, uint8 InPossibleObjectReferenceTypes = 0);
		UE_API FPinTypeTreeInfo(const FText& InFriendlyName, const FName CategoryName, const FAssetData& AssetData, const FText& InTooltip, bool bInReadOnly = false, uint8 InPossibleObjectReferenceTypes = 0);
		UE_API FPinTypeTreeInfo(TSharedPtr<FPinTypeTreeInfo> InInfo);
		
		/** Returns a succinct menu description of this type */
		UE_API const FText& GetDescription() const;
		...
		UE_API const FAssetData& GetCachedAssetData() const;
	...
	private:
		...
		UE_API FText GenerateDescription();
	};

public:
	UE_API void SelectAllNodesInDirection(TEnumAsByte<enum EEdGraphPinDirection> InDirection, UEdGraph* Graph, UEdGraphPin* InGraphPin);
	...
	UE_API virtual bool TryCreateConnection(UEdGraphPin* A, UEdGraphPin* B) const override;
	UE_API virtual bool CreateAutomaticConversionNodeAndConnections(UEdGraphPin* A, UEdGraphPin* B) const override;
	UE_API virtual bool CreatePromotedConnection(UEdGraphPin* A, UEdGraphPin* B) const override;
	UE_API virtual FString IsPinDefaultValid(const UEdGraphPin* Pin, const FString& NewDefaultValue, TObjectPtr<UObject> NewDefaultObject, const FText& InNewDefaultText) const override;
	UE_API virtual bool DoesSupportPinWatching() const	override;
	...
	UE_API virtual void TrySetDefaultValue(UEdGraphPin& Pin, const FString& NewDefaultValue, bool bMarkAsModified = true) const override;
	UE_API virtual void TrySetDefaultObject(UEdGraphPin& Pin, UObject* NewDefaultObject, bool bMarkAsModified = true) const override;
	UE_API virtual void TrySetDefaultText(UEdGraphPin& InPin, const FText& InNewDefaultText, bool bMarkAsModified = true) const override;
	UE_API virtual bool DoesDefaultValueMatchAutogenerated(const UEdGraphPin& InPin) const override;
	UE_API virtual void ResetPinToAutogeneratedDefaultValue(UEdGraphPin* Pin, bool bCallModifyCallbacks = true) const override;
	UE_API virtual bool ShouldHidePinDefaultValue(UEdGraphPin* Pin) const override;
	UE_API virtual bool ShouldShowAssetPickerForPin(UEdGraphPin* Pin) const override;
	UE_API virtual FLinearColor GetPinTypeColor(const FEdGraphPinType& PinType) const override;
	UE_API virtual FLinearColor GetSecondaryPinTypeColor(const FEdGraphPinType& PinType) const override;
	UE_API virtual FText GetPinDisplayName(const UEdGraphPin* Pin) const override;
	UE_API virtual void ConstructBasicPinTooltip(const UEdGraphPin& Pin, const FText& PinDescription, FString& TooltipOut) const override;
	UE_API virtual EGraphType GetGraphType(const UEdGraph* TestEdGraph) const override;
	UE_API virtual bool IsTitleBarPin(const UEdGraphPin& Pin) const override;
	...
	UE_API virtual void ReconstructNode(UEdGraphNode& TargetNode, bool bIsBatchRequest=false) const override;
	UE_API virtual bool CanEncapuslateNode(UEdGraphNode const& TestNode) const override;
	UE_API virtual void HandleGraphBeingDeleted(UEdGraph& GraphBeingRemoved) const override;
	...
	UE_API virtual void DroppedAssetsOnNode(const TArray<FAssetData>& Assets, const FVector2f& GraphPosition, UEdGraphNode* Node) const override;
	UE_API virtual void DroppedAssetsOnPin(const TArray<FAssetData>& Assets, const FVector2f& GraphPosition, UEdGraphPin* Pin) const override;
	UE_API virtual void GetAssetsPinHoverMessage(const TArray<FAssetData>& Assets, const UEdGraphPin* HoverPin, FString& OutTooltipText, bool& OutOkIcon) const override;
	...
	UE_API virtual bool CanDuplicateGraph(UEdGraph* InSourceGraph) const override;
	UE_API virtual UEdGraph* DuplicateGraph(UEdGraph* GraphToDuplicate) const override;
	UE_API virtual UEdGraphNode* CreateSubstituteNode(UEdGraphNode* Node, const UEdGraph* Graph, FObjectInstancingGraph* InstanceGraph, TSet<FName>& InOutExtraNames) const override;
	...
	UE_API virtual bool FadeNodeWhenDraggingOffPin(const UEdGraphNode* Node, const UEdGraphPin* Pin) const override;
	UE_API virtual void BackwardCompatibilityNodeConversion(UEdGraph* Graph, bool bOnlySafeChanges) const override;
	virtual bool ShouldAlwaysPurgeOnModification() const override { return false; }
	UE_API virtual void SplitPin(UEdGraphPin* Pin, bool bNotify = true) const override;
	UE_API virtual void RecombinePin(UEdGraphPin* Pin) const override;
	...
	UE_API virtual bool SafeDeleteNodeFromGraph(UEdGraph* Graph, UEdGraphNode* NodeToDelete) const override;
	virtual bool CanVariableBeDropped(UEdGraph* InGraph, FProperty* InVariableToDrop) const override { return true; }
	UE_API virtual bool CanShowDataTooltipForPin(const UEdGraphPin& Pin) const override;

#if WITH_EDITORONLY_DATA
	UE_API virtual float GetActionFilteredWeight(const FEdGraphSchemaAction& InCurrentAction, const TArray<FString>& InFilterTerms, const TArray<FString>& InSanitizedFilterTerms, const TArray<UEdGraphPin*>& DraggedFromPins) const override;
	...
	UE_API virtual float GetActionFilteredWeight(const FGraphActionListBuilderBase::ActionGroup& InCurrentAction, const TArray<FString>& InFilterTerms, const TArray<FString>& InSanitizedFilterTerms, const TArray<UEdGraphPin*>& DraggedFromPins) const override;
	...
	*/
	static UE_API void ConfigureVarNode(class UK2Node_Variable* InVarNode, FName InVariableName, UStruct* InVariableSource, UBlueprint* InTargetBlueprint);
	...
	*/
	UE_API virtual class UK2Node_VariableGet* SpawnVariableGetNode(const FVector2D GraphPosition, class UEdGraph* ParentGraph, FName VariableName, UStruct* Source) const;
	...
	* @return	A pointer to the newly spawned node
	*/
	UE_API virtual class UK2Node_VariableSet* SpawnVariableSetNode(const FVector2D GraphPosition, class UEdGraph* ParentGraph, FName VariableName, UStruct* Source) const;

	// Returns whether the supplied Pin is a splittable struct
	UE_API bool PinHasSplittableStructType(const UEdGraphPin* InGraphPin) const;

	/** Returns true if the pin has a value field that can be edited inline */
	UE_API bool PinDefaultValueIsEditable(const UEdGraphPin& InGraphPin) const;

	/** Returns true if the pin has a custom default string format and it is not safe to use ExportText */
	UE_API bool PinHasCustomDefaultFormat(const UEdGraphPin& InGraphPin) const;
	...
	UE_API UK2Node* CreateSplitPinNode(UEdGraphPin* Pin, const FCreateSplitPinNodeParams& Params) const;
	...
	UE_API virtual void GetPinDefaultValuesFromString(const FEdGraphPinType& PinType, UObject* OwningObject, const FString& NewValue, FString& UseDefaultValue, TObjectPtr<UObject>& UseDefaultObject, FText& UseDefaultText, bool bPreserveTextIdentity = true) const;

	/** Do validation, that doesn't require a knowledge about actual pin */
	UE_API virtual bool DefaultValueSimpleValidation(const FEdGraphPinType& PinType, const FName PinName, const FString& NewDefaultValue, TObjectPtr<UObject> NewDefaultObject, const FText& InText, FString* OutMsg = nullptr) const;

	/** Returns true if the owning node is a function with AutoCreateRefTerm meta data */
	static UE_API bool IsAutoCreateRefTerm(const UEdGraphPin* Pin);

	/** See if a class has any members that are accessible by a blueprint */
	UE_API bool ClassHasBlueprintAccessibleMembers(const UClass* InClass) const;
	 ...
	 */
	static UE_API bool IsConstructionScript(const UEdGraph* TestEdGraph);
	 ...
	 */
	UE_API bool IsCompositeGraph(const UEdGraph* TestEdGraph) const;
	 ...
	 */
	UE_API bool IsConstFunctionGraph(const UEdGraph* TestEdGraph, bool* bOutIsEnforcingConstCorrectness = nullptr) const;
	 ...
	 * @return	true if this is a const function graph
	 */
	UE_API bool IsStaticFunctionGraph(const UEdGraph* TestEdGraph) const;
	 ...
	 */
	UE_API virtual bool IsSelfPin(const UEdGraphPin& Pin) const override;
	...
	UE_API virtual bool IsDelegateCategory(const FName Category) const override;
	...
	UE_API bool CanPromotePinToVariable (const UEdGraphPin& Pin, bool bInToMemberVariable) const;

	/** Can Pin be split in to its component elements */
	UE_API bool CanSplitStructPin(const UEdGraphPin& Pin) const;

	/** Can Pin be recombined back to its original form */
	UE_API bool CanRecombineStructPin(const UEdGraphPin& Pin) const;
	 ...
	 */
	static UE_API bool GetPropertyCategoryInfo(const FProperty* TestProperty, FName& OutCategory, FName& OutSubCategory, UObject*& OutSubCategoryObject, bool& bOutIsWeakPointer);
	 ...
	 * @return	true on success, false if the property is unsupported or invalid.
	 */
	UE_API bool ConvertPropertyToPinType(const FProperty* Property, /*out*/ FEdGraphPinType& TypeOut) const;
	...
	static UE_API bool HasWildcardParams(const UFunction* Function);
	...
	static UE_API bool IsWildcardProperty(const FProperty* ParamProperty);
	 ...
	 */
	static UE_API UFunction* GetCallableParentFunction(UFunction* Function);
	...
	UE_API bool IsActorValidForLevelScriptRefs(const AActor* TestActor, const UBlueprint* Blueprint) const;
	 ...
	 */
	UE_API void AddSelectedReplaceableNodes(FToolMenuSection& Section, UBlueprint* Blueprint, const UEdGraphNode* InGraphNode) const;
	 ...
	 */
	UE_API void ReplaceSelectedNode(UEdGraphNode* SourceNode, AActor* TargetActor);

	/** Returns whether a function is marked 'override' and doesn't have any out parameters */
	static UE_API bool FunctionCanBePlacedAsEvent(const UFunction* InFunction);

	/** Can this function be called by kismet delegate */
	static UE_API bool FunctionCanBeUsedInDelegate(const UFunction* InFunction);

	/** Can this function be called by kismet code */
	static UE_API bool CanUserKismetCallFunction(const UFunction* Function);

	/** Returns if function has output parameter(s) */
	static UE_API bool HasFunctionAnyOutputParameter(const UFunction* Function);
	...
	static UE_API bool CanUserKismetAccessVariable(const FProperty* Property, const UClass* InClass, EDelegateFilterMode FilterMode);

	/** Can this function be overridden by kismet (either placed as event or new function graph created) */
	static UE_API bool CanKismetOverrideFunction(const UFunction* Function);

	/** returns friendly signature name if possible or Removes any mangling to get the unmangled signature name of the function */
	static UE_API FText GetFriendlySignatureName(const UFunction* Function);

	/** Returns true if this enum is safe to be used as a variable in blueprints */
	static UE_API bool IsAllowableBlueprintVariableType(const UEnum* InEnum);
	 ...
	 */
	static UE_API bool IsAllowableBlueprintVariableType(const UClass* InClass, bool bAssumeBlueprintType = false);
	 ...
	 */
	static UE_API bool IsAllowableBlueprintVariableType(const UScriptStruct *InStruct, bool bForInternalUse = false);

	static UE_API bool IsPropertyExposedOnSpawn(const FProperty* Property);

	/** returns whether the property is deprecated. */
	static UE_API bool IsPropertyDeprecated(const FProperty* Property);
	 ...
	 */
	static UE_API void GetAutoEmitTermParameters(const UFunction* Function, TArray<FString>& AutoEmitParameterNames);
	 ...
	 */
	UE_API bool FunctionHasParamOfType(const UFunction* InFunction, UEdGraph const* InGraph, const FEdGraphPinType& DesiredPinType, bool bWantOutput) const;
	 ...
	 */
	UE_API void AddExtraFunctionFlags(const UEdGraph* CurrentGraph, int32 ExtraFlags) const;
	 ...
	 */
	UE_API void MarkFunctionEntryAsEditable(const UEdGraph* CurrentGraph, bool bNewEditable) const;
	 ...
	 */
	UE_API virtual void CreateMacroGraphTerminators(UEdGraph& Graph, UClass* Class) const;
	 ...
	 * @param	ContextClass	If specified, the graph terminators will use this class to search for context for signatures (i.e. interface functions)
	 */
	UE_API virtual void CreateFunctionGraphTerminators(UEdGraph& Graph, UClass* Class) const;
	 ...
	 */
	UE_API virtual void CreateFunctionGraphTerminators(UEdGraph& Graph, const UFunction* FunctionSignature) const;
	 ...
	 */
	static UE_API FText TypeToText(const FProperty* const Property);
	...
	*/
	static UE_API FText TerminalTypeToText(const FName Category, const FName SubCategory, UObject* SubCategoryObject, bool bIsWeakPtr);
	 ...
	 * @return	The converted type text.
	 */
	static UE_API FText TypeToText(const FEdGraphPinType& Type);
	 ...
	 */
	static UE_API FText GetCategoryText(const FName Category, const bool bForMenu = false);
	 ...
	 * @return	The text to display for the category.
	 */
	static UE_API FText GetCategoryText(FName Category, FName SubCategory, bool bForMenu = false);
	 ...
	 */
	UE_API void GetVariableTypeTree(TArray< TSharedPtr<FPinTypeTreeInfo> >& TypeTree, ETypeTreeFilter TypeTreeFilter = ETypeTreeFilter::None) const;
	 ...
	 */
	UE_API bool DoesTypeHaveSubtypes( const FName Category ) const;
	 ...
	 */
	UE_API virtual bool ArePinsCompatible(const UEdGraphPin* PinA, const UEdGraphPin* PinB, const UClass* CallingContext = NULL, bool bIgnoreArray = false) const override;
	 ...
	 */
	UE_API virtual const FPinConnectionResponse DetermineConnectionResponseOfCompatibleTypedPins(const UEdGraphPin* PinA, const UEdGraphPin* PinB, const UEdGraphPin* InputPin, const UEdGraphPin* OutputPin) const;
	 ...
	 */
	UE_API virtual bool ArePinTypesCompatible(const FEdGraphPinType& Output, const FEdGraphPinType& Input, const UClass* CallingContext = NULL, bool bIgnoreArray = false) const;
	 ...
	 * @return	true if the pin types and directions are compatible.
	 */
	UE_API virtual bool ArePinTypesEquivalent(const FEdGraphPinType& PinA, const FEdGraphPinType& PinB) const;
	...
	UE_API virtual void SetPinAutogeneratedDefaultValue(UEdGraphPin* Pin, const FString& NewValue) const;
	...
	UE_API virtual void SetPinAutogeneratedDefaultValueBasedOnType(UEdGraphPin* Pin) const;
	...
	UE_API virtual void SetPinDefaultValueAtConstruction(UEdGraphPin* Pin, const FString& DefaultValueString) const;

	/** Call to let blueprint and UI know that parameters have changed for a function/macro/etc */
	UE_API virtual void HandleParameterDefaultValueChanged(UK2Node* TargetNode) const;

	/** Given a function and property, return the default value */
	static UE_API bool FindFunctionParameterDefaultValue(const UFunction* Function, const FProperty* Param, FString& OutString);

	/** Utility that makes sure existing connections are valid, breaking any that are now illegal. */
	static UE_API void ValidateExistingConnections(UEdGraphPin* Pin);

	/** Find a 'set value by name' function for the specified pin, if it exists */
	static UE_API UFunction* FindSetVariableByNameFunction(const FEdGraphPinType& PinType);

	UE_DEPRECATED(5.2, "Use the FSearchForAutocastFunctionResults variant.")
	UE_API virtual bool SearchForAutocastFunction(const FEdGraphPinType& OutputPinType, const FEdGraphPinType& InputPinType, /*out*/ FName& TargetFunction, /*out*/ UClass*& FunctionOwner) const;
	...
	[[nodiscard]] UE_API virtual TOptional<FSearchForAutocastFunctionResults> SearchForAutocastFunction(const FEdGraphPinType& OutputPinType, const FEdGraphPinType& InputPinType) const;

	UE_DEPRECATED(5.2, "Use the FFindSpecializedConversionNodeResults variant.")
	UE_API virtual bool FindSpecializedConversionNode(const UEdGraphPin* OutputPin, const UEdGraphPin* InputPin, bool bCreateNode, /*out*/ class UK2Node*& TargetNode) const;
	
	UE_DEPRECATED(5.2, "Use the FFindSpecializedConversionNodeResults variant.")
	UE_API virtual bool FindSpecializedConversionNode(const FEdGraphPinType& OutputPinType, const UEdGraphPin* InputPinType, bool bCreateNode, /*out*/ class UK2Node*& TargetNode) const;
	...
	[[nodiscard]] UE_API virtual TOptional<FFindSpecializedConversionNodeResults> FindSpecializedConversionNode(const FEdGraphPinType& OutputPinType, const UEdGraphPin& InputPin, bool bCreateNode) const;

	/** Create menu for variable get/set nodes which refer to a variable which does not exist. */
	UE_API void GetNonExistentVariableMenu(FToolMenuSection& Section, const UEdGraphNode* InGraphNode, UBlueprint* OwnerBlueprint) const;
	 ...
	 */
	UE_API void GetReplaceVariableMenu(FToolMenuSection& Section, const UEdGraphNode* InGraphNode, UBlueprint* InOwnerBlueprint, bool bInReplaceExistingVariable = false) const;

	// Calculates an average position between the nodes owning the two specified pins
	static UE_API FVector2D CalculateAveragePositionBetweenNodes(UEdGraphPin* InputPin, UEdGraphPin* OutputPin);
	...
	UE_API void AutowireConversionNode(UEdGraphPin* InputPin, UEdGraphPin* OutputPin, UEdGraphNode* ConversionNode) const;
	 ...
	 */
	UE_API bool DoesGraphSupportImpureFunctions(const UEdGraph* InGraph) const;
	 ...
	 */
	UE_API bool IsGraphMarkedThreadSafe(const UEdGraph* InGraph) const;
	 ...
	 */
	UE_API bool CanFunctionBeUsedInGraph(const UClass* InClass, const UFunction* InFunction, const UEdGraph* InDestGraph, uint32 InFunctionTypes, bool bInCalledForEach, FText* OutReason = nullptr) const;
	 ...
	 */
	UE_API bool CollapseGatewayNode(UK2Node* InNode, UEdGraphNode* InEntryNode, UEdGraphNode* InResultNode, class FKismetCompilerContext* CompilerContext = nullptr, TSet<UEdGraphNode*>* OutExpandedNodes = nullptr) const;
	 ...
	 */
	UE_API void CombineTwoPinNetsAndRemoveOldPins(UEdGraphPin* InPinA, UEdGraphPin* InPinB) const;

	/**
	 * Make links from all data pins from InOutputNode output to InInputNode input.
	 */
	UE_API void LinkDataPinFromOutputToInput(UEdGraphNode* InOutputNode, UEdGraphNode* InInputNode) const;
	...
	UE_API bool ReplaceOldNodeWithNew(UEdGraphNode* OldNode, UEdGraphNode* NewNode, const TMap<FName, FName>& OldPinToNewPinMap) const;

	/** Convert a deprecated node into a function call node, called from per-node ConvertDeprecatedNode */
	UE_API UK2Node* ConvertDeprecatedNodeToFunctionCall(UK2Node* OldNode, UFunction* NewFunction, TMap<FName, FName>& OldPinToNewPinMap, UEdGraph* Graph) const;
	...
	static UE_API void OnCreateNonExistentVariable(class UK2Node_Variable* Variable, UBlueprint* OwnerBlueprint);

	/** Create the local variable that the broken node refers to */
	static UE_API void OnCreateNonExistentLocalVariable(class UK2Node_Variable* Variable, UBlueprint* OwnerBlueprint);

	/** Replace the variable that a variable node refers to when the variable it refers to does not exist */
	static UE_API void OnReplaceVariableForVariableNode(class UK2Node_Variable* Variable, UBlueprint* OwnerBlueprint, FName VariableName, bool bIsSelfMember);

	/** Create sub menu that shows all possible variables that can be used to replace the existing variable reference */
	static UE_API void GetReplaceVariableMenu(UToolMenu* Menu, class UK2Node_Variable* Variable, UBlueprint* OwnerBlueprint, bool bReplaceExistingVariable = false);

	/** Function called when the owning module is shut down */ 
	static UE_API void Shutdown();
private:

	static TSharedPtr<IAssetReferenceFilter> MakeAssetReferenceFilter(const UBlueprint* Blueprint);
	 ...
	 */
	UE_API bool DoesFunctionHaveOutParameters( const UFunction* Function ) const;
	
	UE_API bool ShouldShowPanelContextMenuForIncompatibleConnectionsImpl() const override;

	static UE_API const UScriptStruct* VectorStruct;
	static UE_API const UScriptStruct* Vector3fStruct;
	static UE_API const UScriptStruct* RotatorStruct;
	static UE_API const UScriptStruct* TransformStruct;
	static UE_API const UScriptStruct* LinearColorStruct;
	static UE_API const UScriptStruct* ColorStruct;
};

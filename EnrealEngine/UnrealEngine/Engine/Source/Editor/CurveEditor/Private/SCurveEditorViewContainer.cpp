...
bool SCurveEditorViewContainer::IsScrubTimeKeyEvent(const FKeyEvent& InKeyEvent)
{
	...
	for (uint32 i = 0; i < static_cast<uint8>(EMultipleKeyBindingIndex::NumChords); ++i)
	{
		...
		const bool bIsMovingTimeSlider = Chord.IsValidChord() && InKeyEvent.GetKey() == Chord.Key &&
			(Chord.NeedsAlt() ? InKeyEvent.IsAltDown() : !InKeyEvent.IsAltDown()) &&
			(Chord.NeedsShift() ? InKeyEvent.IsShiftDown() : !InKeyEvent.IsShiftDown()) &&
			(Chord.NeedsControl() ? InKeyEvent.IsControlDown() : !InKeyEvent.IsControlDown());
...
void SCurveEditorViewContainer::BroadcastOnCurveHasChangedExternally(const FCurveModelID& InCurveModelId) const
{
	OnCurveHasChangedExternallyDelegate.Broadcast(InCurveModelId);
}
...
FReply SCurveEditorViewContainer::OnMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	if (MouseEvent.GetEffectingButton() == EKeys::LeftMouseButton)
	{
		...
		const FVector2D InitialPosition = DragOperation->GetInitialPosition();
		DragOperation->DragImpl->MouseButtonDown(InitialPosition, MyGeometry, MouseEvent);
		return FReply::Handled();
	}
...
FReply SCurveEditorViewContainer::OnMouseButtonUp(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	if (DragOperation.IsSet())
	{
		DragOperation->DragImpl->MouseButtonUp(MyGeometry, MouseEvent);
	}
	if (DragOperation.IsSet() && DragOperation->IsDragging())
	{
		FVector2D InitialPosition = DragOperation->GetInitialPosition();
	...
	if (!Reply.IsEventHandled() && MouseEvent.GetEffectingButton() == EKeys::LeftMouseButton && bCaughtMouseDown)
	{
		...
		{
			const UE::CurveEditor::FScopedSelectionChange Transaction(CurveEditor, LOCTEXT("ClearSelection", "Clear selection"));
...
void SCurveEditorViewContainer::OnMouseCaptureLost(const FCaptureLostEvent& CaptureLostEvent)
{
#if WITH_EDITOR
	if (!DragOperation.IsSet())
	{
...
void SCurveEditorViewContainer::AddView(TSharedRef<SCurveEditorView> ViewToAdd)
{
	...
	ViewToAdd->OnCurveHasChangedExternally().AddRaw(this, &SCurveEditorViewContainer::BroadcastOnCurveHasChangedExternally);
}

void SCurveEditorViewContainer::Clear()
{
	for (const TSharedPtr<SCurveEditorView>& View : Views)
	{
		View->OnCurveHasChangedExternally().RemoveAll(this);
	}

...
TArray<FCurveEditorTreeItemID> FCurveEditorTree::GetCachedExpandedItems() const
{
	...
	const int32 NumCachedExpandedItems = CachedExpandedItems.Num();
	if (NumCachedExpandedItems > 0)
	{
		ExpandedItems.Reserve(NumCachedExpandedItems);
		for (const TPair<FCurveEditorTreeItemID, FCurveEditorTreeItem>& Item : Items)
		{
			...
			if (PathName.IsSet())
			{
				...
				if (CachedExpandedItems.Contains(Hash))
				{
					...
					// early exit
					if (ExpandedItems.Num() == NumCachedExpandedItems)
					{
...
void FCurveEditorTree::SetItemExpansion(FCurveEditorTreeItemID InTreeItemID, bool bInExpansion)
{
	...
	{
		const TOptional<FString>& OptString = TreeItem->GetUniquePathName();
		if (OptString.IsSet())
		{

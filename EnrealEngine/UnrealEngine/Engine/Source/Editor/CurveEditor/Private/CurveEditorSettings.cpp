...
#include UE_INLINE_GENERATED_CPP_BY_NAME(CurveEditorSettings)

UCurveEditorSettings::UCurveEditorSettings()
{
	PanningMouseButton = ECurveEditorPanningMouseButton::RightOrAltMiddle;
...
bool UCurveEditorSettings::AllowMousePan(const FPointerEvent& MouseEvent) const
{
	if (PanningMouseButton == ECurveEditorPanningMouseButton::AltMiddle)
	{
		return MouseEvent.IsMouseButtonDown(EKeys::MiddleMouseButton) && MouseEvent.IsAltDown();
	}
	else if (PanningMouseButton == ECurveEditorPanningMouseButton::Right)
	{
		return MouseEvent.IsMouseButtonDown(EKeys::RightMouseButton) && !MouseEvent.IsAltDown();
	}
	else if (PanningMouseButton == ECurveEditorPanningMouseButton::RightOrAltMiddle)
	{
		if ((MouseEvent.IsMouseButtonDown(EKeys::MiddleMouseButton) && MouseEvent.IsAltDown()) ||
			(MouseEvent.IsMouseButtonDown(EKeys::RightMouseButton) && !MouseEvent.IsAltDown()))
		{
			return true;
		}
		return false;
	...
	// Default to right mouse button
	return MouseEvent.GetEffectingButton() == EKeys::RightMouseButton;
}

bool UCurveEditorSettings::AllowMouseEdit(const FPointerEvent& MouseEvent) const
{
	// If panning with alt-middle, middle allows mouse edits
	if (PanningMouseButton == ECurveEditorPanningMouseButton::AltMiddle)
	{
		return MouseEvent.IsMouseButtonDown(EKeys::MiddleMouseButton) && !MouseEvent.IsAltDown();
	}
	// If panning with right, middle allows mouse edits
	else if (PanningMouseButton == ECurveEditorPanningMouseButton::Right)
	{
		return MouseEvent.IsMouseButtonDown(EKeys::MiddleMouseButton);
	}
	else if (PanningMouseButton == ECurveEditorPanningMouseButton::RightOrAltMiddle)
	{
		if (MouseEvent.IsMouseButtonDown(EKeys::MiddleMouseButton) && !MouseEvent.IsAltDown())
	...
	// Default to middle mouse button
...
bool UCurveEditorSettings::GetScrubTimeStartFromCursor() const
{
	return bScrubTimeStartFromCursor;
}

void UCurveEditorSettings::SetScrubTimeStartFromCursor(bool bInValue)
{

...
FReply SCurveEditorFilterPanel::OnApplyClicked()
{
	...
	if (CurveEditor && DetailsView->GetSelectedObjects().Num() > 0)
	{
		const FText TransactionText = FText::Format(
			NSLOCTEXT("CurveEditorFilterApply", "Filter Curves", "Filtered {0}|plural(one=Curve, other=Curves)"),
			CurveEditor->GetSelection().Count()
			);
		const FScopedTransaction Transaction(TransactionText);
		
		UCurveEditorFilterBase* Filter = Cast<UCurveEditorFilterBase>(DetailsView->GetSelectedObjects()[0]);
		UE::CurveEditor::FilterUtils::ApplyFilter(CurveEditor.ToSharedRef(), *Filter);
	}

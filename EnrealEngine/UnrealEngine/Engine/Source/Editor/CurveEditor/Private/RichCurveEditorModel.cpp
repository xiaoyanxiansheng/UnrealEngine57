...
void FRichCurveEditorModel::SetCurveAttributes(const FCurveAttributes& InCurveAttributes)
{
 ...
 void FRichCurveEditorModel::CreateKeyProxies(TWeakPtr<FCurveEditor> InWeakCurveEditor, FCurveModelID InCurveModelID, TArrayView<const FKeyHandle> InKeyHandles, TArrayView<UObject*> OutObjects)
 {
	 if (UObject* Owner = WeakOwner.Get())
	 {
		 if (IsValid())
		 {
			 for (int32 Index = 0; Index < InKeyHandles.Num(); ++Index)
			 {
				 ...
				 NewProxy->Initialize(InWeakCurveEditor, InCurveModelID, InKeyHandles[Index], WeakOwner);

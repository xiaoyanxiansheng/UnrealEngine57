...
public:
	 ...
	 */
	UE_API FCurveEditorScreenSpace GetCurveSpace(FCurveModelID CurveID) const;
	...
	UE_API FTransform2d GetViewToCurveTransform(FCurveModelID CurveID) const;

	/**
	 * Retrieve the horizontal screen-space information for the specified axis
	 */
	UE_API FCurveEditorScreenSpaceH GetHorizontalAxisSpace(FCurveEditorViewAxisID ID) const;

	/**
	 * Retrieve the vertical screen-space information for the specified axis
	 */
	UE_API FCurveEditorScreenSpaceV GetVerticalAxisSpace(FCurveEditorViewAxisID ID) const;

	/**
	 * Retrueve the axis ID assigned to the specified curve and orientation
	 */
	UE_API FCurveEditorViewAxisID GetAxisForCurve(FCurveModelID CurveID, ECurveEditorAxisOrientation AxisOrientation) const;

	/**
	 * Retrueve the axis associated with the specified ID and orientation
	 */
	UE_API TSharedPtr<FCurveEditorAxis> GetAxis(FCurveEditorViewAxisID ID, ECurveEditorAxisOrientation AxisOrientation) const;
	 ...
	 */
	UE_API void GetInputBounds(double& OutInputMin, double& OutInputMax) const;
	 ...
	 */
	UE_API void SetOutputBounds(double InOutputMin, double InOutputMax, FCurveEditorViewAxisID AxisID = FCurveEditorViewAxisID());

	/**
	 * Set this view's input bounds
	 */
	UE_API void SetInputBounds(double InInputMin, double InInputMax, FCurveEditorViewAxisID AxisID = FCurveEditorViewAxisID());
	 ...
	 */
	UE_API void Zoom(const FVector2D& Amount);
	 ...
	 */
	UE_API void ZoomAround(const FVector2D& Amount, double InputOrigin, double OutputOrigin);
	...
	UE_API virtual void CheckCacheAndInvalidateIfNeeded();
	...
	UE_API virtual void FrameVertical(double InOutputMin, double InOutputMax, FCurveEditorViewAxisID AxisID = FCurveEditorViewAxisID());

	/** Frame the view horizontall by the input and output bounds, peformaing any custom clipping as needed */
	UE_API virtual void FrameHorizontal(double InInputMin, double InInputMax, FCurveEditorViewAxisID AxisID = FCurveEditorViewAxisID());
	...
	/** Invoked when we detect that a curve needs to be redrawn, i.e. when FCurveModel::HasChangedAndResetTest returns true. */
...
public:
	...
	UE_API void RefreshRetainer();

	/** Update the custom axes if necessary */
	UE_API void UpdateCustomAxes();

protected:

	/** Inits this view with the curve editor and sets up required callbacks. */
	UE_API void InitCurveEditorReference(const TSharedRef<FCurveEditor>& InCurveEditor);
	...
	UE_API void GetCurveDrawParams(TArray<FCurveDrawParams>& OutDrawParams);
	...
	UE_API void GetCurveDrawParam(TSharedPtr<FCurveEditor>& CurveEditor, const FCurveModelID& ModelID, FCurveModel* CurveModel, FCurveDrawParams& OutDrawParam) const;

	/** Update all the curve view transforms from curve models */
	UE_API void UpdateCurveViewTransformsFromModels();
...
private:
	...
	UE_API void AddCurve(FCurveModelID CurveID);

	/** Add a curve from this view */
	UE_API void RemoveCurve(FCurveModelID CurveID);

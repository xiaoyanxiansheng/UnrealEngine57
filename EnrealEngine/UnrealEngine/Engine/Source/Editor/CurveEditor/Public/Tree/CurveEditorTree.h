...
{
	UE_API explicit FScopedCurveEditorTreeEventGuard(FCurveEditorTree* InTree);
	UE_API ~FScopedCurveEditorTreeEventGuard();

	UE_API FScopedCurveEditorTreeEventGuard(FScopedCurveEditorTreeEventGuard&& RHS);
	UE_API FScopedCurveEditorTreeEventGuard& operator=(FScopedCurveEditorTreeEventGuard&& RHS);
...
struct FCurveEditorTreeItem
{
	...
	*/
	 ...
	 */
	UE_API TArrayView<const FCurveModelID> GetOrCreateCurves(FCurveEditor* CurveEditor);

	/**
	 * Destroy any previously constructed curve models that this tree item owns
	 */
	UE_API void DestroyCurves(FCurveEditor* CurveEditor);

	/**
	 * Destroy any previously constructed unpinned curve models that this tree item owns
	 */
	UE_API void DestroyUnpinnedCurves(FCurveEditor* CurveEditor);
...
public:
	...
	UE_API FCurveEditorTree();

	/**
	 * Retrieve an item from its ID, assuming it is definitely valid
	 */
	UE_API FCurveEditorTreeItem& GetItem(FCurveEditorTreeItemID ItemID);

	/**
	 * Retrieve an item from its ID, assuming it is definitely valid
	 */
	UE_API const FCurveEditorTreeItem& GetItem(FCurveEditorTreeItemID ItemID) const;

	/**
	 * Retrieve an item from its ID or nullptr if the ID is not valid
	 */
	UE_API FCurveEditorTreeItem* FindItem(FCurveEditorTreeItemID ItemID);

	/**
	 * Retrieve an item from its ID or nullptr if the ID is not valid
	 */
	UE_API const FCurveEditorTreeItem* FindItem(FCurveEditorTreeItemID ItemID) const;

	/** 
	 * Retrieve this curve editor's root items irrespective of filter state
	 */
	UE_API const TArray<FCurveEditorTreeItemID>& GetRootItems() const;

	/** 
	 * Retrieve all the items stored in this tree irrespective of filter state
	 */
	UE_API const TMap<FCurveEditorTreeItemID, FCurveEditorTreeItem>& GetAllItems() const;
	 ...
	 */
	UE_API FCurveEditorTreeItem* AddItem(FCurveEditorTreeItemID ParentID);
	 ...
	 */
	UE_API void RemoveItem(FCurveEditorTreeItemID ItemID, FCurveEditor* CurveEditor);

	/**
	 * Run all the filters on this tree, updating filter state for all tree items
	 */
	UE_API void RunFilters();
	 ...
	 */
	UE_API void AddFilter(TWeakPtr<FCurveEditorTreeFilter> NewFilter);
	 ...
	 */
	UE_API void RemoveFilter(TWeakPtr<FCurveEditorTreeFilter> FilterToRemove);

	/**
	 * Direct access to all current filters (potentially including expired ones)
	 */
	UE_API TArrayView<const TWeakPtr<FCurveEditorTreeFilter>> GetFilters() const;
	 ...
	 */
	UE_API void SetDirectSelection(TArray<FCurveEditorTreeItemID>&& TreeItems, FCurveEditor* InCurveEditor);

	/**
	 * Removes tree items from the current selection.
	 */
	UE_API void RemoveFromSelection(TArrayView<const FCurveEditorTreeItemID> TreeItems, FCurveEditor* InCurveEditor);
	 ...
	 */
	UE_API const TMap<FCurveEditorTreeItemID, ECurveEditorTreeSelectionState>& GetSelection() const;

	/**
	 * Check a specific tree item's selection state
	 */
	UE_API ECurveEditorTreeSelectionState GetSelectionState(FCurveEditorTreeItemID InTreeItemID) const;
	 ...
	 */
	UE_API const FCurveEditorFilterStates& GetFilterStates() const;

	/**
	 * Check a specific tree item's filter state
	 */
	UE_API ECurveEditorTreeFilterState GetFilterState(FCurveEditorTreeItemID InTreeItemID) const;
	...
	*/
	UE_API TArray<FCurveEditorTreeItemID> GetCachedExpandedItems() const;
	
	/*
	* Set Item expansion state
	*/
	UE_API void SetItemExpansion(FCurveEditorTreeItemID InTreeItemID, bool bInExpansion);
	...
	*/
	UE_API void RecreateModelsFromExistingSelection(FCurveEditor* CurveEditor);

private:

	// Recursively removes children without removing them from the parent (assuming the parent is also being removed)
	UE_API void RemoveChildrenRecursive(TArray<FCurveEditorTreeItemID>&& Children, FCurveEditor* CurveEditor);
	 ...
	 */
	UE_API ECurveEditorTreeFilterState PerformFilterPass(TArrayView<const FCurveEditorTreeFilter* const> FilterPtrs, TArrayView<const FCurveEditorTreeItemID> ItemsToFilter, ECurveEditorTreeFilterState InheritedState);

	/** 
	 * Recursively sorts the tree item ids using the sort predicate.
	 */
	UE_API void SortTreeItems(FSortedCurveEditorTreeItems& TreeItemIDsToSort);

...
void FComponentVisualizersModule::MuteComponentVisualizer(FName ComponentClassName)
{
	if (!GUnrealEd || !RegisteredComponentClassNames.Contains(ComponentClassName) || MutedComponentVisualizers.Contains(ComponentClassName))
	{
		return;
	}
	
	if (const TSharedPtr<FComponentVisualizer> Visualizer = GUnrealEd->FindComponentVisualizer(ComponentClassName))
	{
		// Unregister the visualizer.
		GUnrealEd->UnregisterComponentVisualizer(ComponentClassName);
		RegisteredComponentClassNames.Remove(ComponentClassName);

		// Cache the visualizer.
		MutedComponentVisualizers.Add(ComponentClassName, Visualizer);
	}
}

void FComponentVisualizersModule::UnmuteComponentVisualizer(FName ComponentClassName)
{
	if (!GUnrealEd || RegisteredComponentClassNames.Contains(ComponentClassName) || !MutedComponentVisualizers.Contains(ComponentClassName))
	{
		return;
	...
	if (const TSharedPtr<FComponentVisualizer>* VisualizerPtr = MutedComponentVisualizers.Find(ComponentClassName))
	{
		const TSharedPtr<FComponentVisualizer> Visualizer = *VisualizerPtr;

		// Register the visualizer, but don't call OnRegister (already called previously).
		GUnrealEd->RegisterComponentVisualizer(ComponentClassName, Visualizer);
		RegisteredComponentClassNames.Add(ComponentClassName);

		// Uncache the visualizer.
		MutedComponentVisualizers.Remove(ComponentClassName);
	}
}
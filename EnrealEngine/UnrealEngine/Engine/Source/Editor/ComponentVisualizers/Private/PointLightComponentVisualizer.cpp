...
namespace TextureLightProfileVisualizerImpl
{
	...
	float FilterLightProfile(const FTextureLightProfileData& TextureLightProfileData, const float X, const float Y)
	{
		...
		float UnNormalizedX = FMath::Clamp(X * static_cast<float>(SizeX), 0.0f, static_cast<float>(SizeX - 1));
		float UnNormalizedY = FMath::Clamp(Y * static_cast<float>(SizeY), 0.0f,static_cast<float>(SizeY - 1));
		...
		float XFraction = UnNormalizedX - static_cast<float>(X0);
		float YFraction = UnNormalizedY - static_cast<float>(Y0);
...
bool FTextureLightProfileVisualizer::UpdateIntensitiesCache(UTextureLightProfile* TextureLightProfile, const FTransform& LightTM)
{
	...
	FTextureLightProfileData TextureLightProfileData{ MipData, (int32)TextureLightProfile->Source.GetSizeX(), (int32)TextureLightProfile->Source.GetSizeY(), (int32)TextureLightProfile->Source.GetBytesPerPixel() };

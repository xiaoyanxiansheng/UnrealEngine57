...
public:
	...
	UE_DEPRECATED(5.6, "Viewport sizes are represented in float, please update your callsites to use GetActiveViewportWidgetSize and use FVector2f for slate sizes.")
	virtual FVector2D GetActiveViewportSize() final
	{
		return FVector2D(GetActiveViewportWidgetSize());
	}

	/** Return the size of the active viewport, accounting for DPI Scale */
	virtual FVector2f GetActiveViewportWidgetSize() const
	{

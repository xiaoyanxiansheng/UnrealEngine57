...
public:
	...
	UE_API bool BelongsToTab(TSharedRef<SDockTab> InParentTab) const;
	...
	*/
	UE_API bool IsViewportConfigurationSet(const FName& ConfigurationName) const;
	...
	UE_API const TMap< FName, TSharedPtr< IEditorViewportLayoutEntity > >* GetViewports() const;

	using ViewportActionFunction = TFunction<void(FName Name, TSharedPtr<IEditorViewportLayoutEntity>)>;
	UE_API void PerformActionOnViewports(ViewportActionFunction& TFuncPtr);

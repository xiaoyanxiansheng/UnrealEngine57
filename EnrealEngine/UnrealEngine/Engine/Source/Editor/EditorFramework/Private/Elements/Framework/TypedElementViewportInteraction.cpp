...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TypedElementViewportInteraction)
...
bool FTypedElementViewportInteractionCustomization::GetScaleForGizmo(const TTypedElement<ITypedElementWorldInterface>& InElementWorldHandle, FVector& OutScale)
{
	FTransform WorldTransform;
	if (InElementWorldHandle.GetWorldTransform(WorldTransform))
	{
...
bool UTypedElementViewportInteraction::GetScaleForGizmo(FTypedElementListConstRef InElements, FVector& OutScale)
{
	bool bHasAnyScale = false;
	FVector CombinedScale(ForceInitToZero);
	InElements->ForEachElementHandle([this, &CombinedScale, &bHasAnyScale](const FTypedElementHandle& InElementToMove)
	{
		FTypedElementViewportInteractionElement ViewportInteractionElement = ResolveViewportInteractionElement(InElementToMove);
		if (ViewportInteractionElement)
		{
			FVector ElementScale;
			if (ViewportInteractionElement.GetScaleForGizmo(ElementScale))
			{
				if (!bHasAnyScale)
				{
					CombinedScale = ElementScale;
				}
				else
				{
					CombinedScale = FVector::Max(CombinedScale, ElementScale);
				}

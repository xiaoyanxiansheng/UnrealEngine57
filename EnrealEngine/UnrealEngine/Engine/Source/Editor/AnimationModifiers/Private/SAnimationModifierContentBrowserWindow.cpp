...
FReply SRemoveAnimationModifierContentBrowserWindow::OnApply()
{
	...
	for (UAnimSequence* AnimationSequence : AnimSequences)
	{
		if (UAnimationModifiersAssetUserData* UserData = AnimationSequence->GetAssetUserData<UAnimationModifiersAssetUserData>())
		{
			const TArray<UAnimationModifier*>& AnimModifierInstancesInUserData = UserData->GetAnimationModifierInstances();
			for (const UAnimationModifier* Modifier : Modifiers)
			{
				checkf(Modifier, TEXT("Invalid selected modifier"));

				UAnimationModifier* const* ModifierInstanceInUserData = AnimModifierInstancesInUserData.FindByPredicate([Modifier](const UAnimationModifier* TestModifier)
				{
					return Modifier->GetClass() == TestModifier->GetClass();
				});

				if (ModifierInstanceInUserData == nullptr)
				{
					continue;
				}
				...
				if (bShouldRevert)
				{
					(*ModifierInstanceInUserData)->RevertFromAnimationSequence(AnimationSequence);

					// Revert can not fail, thus we can always mark reverted
					if ((*ModifierInstanceInUserData)->HasLegacyPreviousAppliedModifierOnSkeleton())
					{
						checkf(AnimationSequence->GetSkeleton() != nullptr, TEXT("Invalid skeleton for anim sequence"));
						(*ModifierInstanceInUserData)->RemoveLegacyPreviousAppliedModifierOnSkeleton(AnimationSequence->GetSkeleton());
					}
				}

...
#define UE_API PINNEDCOMMANDLIST_API
...
{
public:
	UE_API FUICommandList_Pinnable();
	...
	UE_API virtual bool ExecuteAction(const TSharedRef< const FUICommandInfo > InUICommandInfo) const override;
	UE_API virtual void MapAction( const TSharedPtr< const FUICommandInfo > InUICommandInfo, const FUIAction& InUIAction ) override;

	/** Mark that a custom widget was interacted with */
	UE_API void WidgetInteraction(FName InCustomWidgetIdentifier) const;
	 ...
	 */
	UE_API int32 GetMappedCommandIndex( FName InCommandName ) const;

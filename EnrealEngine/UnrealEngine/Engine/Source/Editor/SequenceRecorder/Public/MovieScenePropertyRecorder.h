...
private:
	 ...
	 * on previous values
	 */
	UE_API bool ShouldAddNewKey(const int64& InNewValue) const;

	/** Helper function, specialized by type, used to add an appropriate section to the movie scene */
	UE_API class UMovieSceneSection* AddSection(UObject* InObjectToRecord, class UMovieScene* InMovieScene, const FGuid& InGuid, float InTime);

	/** Helper function, specialized by type, used to add keys to the movie scene section at Finalize() time */
	UE_API void AddKeyToSection(UMovieSceneSection* InSection, const FPropertyKey<int64>& InKey);

	/** Helper function, specialized by type, used to reduce keys */
	UE_API void ReduceKeys(UMovieSceneSection* InSection);

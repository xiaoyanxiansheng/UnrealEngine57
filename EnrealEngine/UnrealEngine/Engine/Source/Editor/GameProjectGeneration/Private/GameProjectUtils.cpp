...
namespace
{
	...
	/** Get the configuration values for enabling Substrate by default. */
	void AddSubstrateConfigValues(const FProjectInformation& InProjectInfo, TArray<FTemplateConfigValue>& ConfigValues)
	{
		ConfigValues.Emplace(TEXT("DefaultEngine.ini"),
			TEXT("/Script/Engine.RendererSettings"),
			TEXT("r.Substrate"),
...
void GameProjectUtils::OpenAddToProjectDialog(const FAddToProjectConfig& Config, EClassDomain InDomain)
{
	...
	TSharedRef<SCustomDialog> AddCodeWindow =
		SNew(SCustomDialog)
		.Title( WindowTitle )
	...
	AddCodeWindow->SetOnCancelHotkeyPressed(FExecuteAction::CreateSP(NewClassDialog, &SNewClassDialog::CancelClicked));
	AddCodeWindow->SetOnConfirmHotkeyPressed(FExecuteAction::CreateSP(NewClassDialog, &SNewClassDialog::OnConfirmHotkeyPressed));
	AddCodeWindow->SetWidgetToFocusOnActivate(NewClassDialog->GetWidgetToFocusOnActivate());
...
TOptional<FGuid> GameProjectUtils::CreateProjectFromTemplate(const FProjectInformation& InProjectInfo, FText& OutFailReason, FText& OutFailLog, TArray<FString>* OutCreatedFiles)
{
	...
	AddSubstrateConfigValues(InProjectInfo, ConfigValuesToSet);
...
{
	return TEXT("BuildSettingsVersion.V6");
}

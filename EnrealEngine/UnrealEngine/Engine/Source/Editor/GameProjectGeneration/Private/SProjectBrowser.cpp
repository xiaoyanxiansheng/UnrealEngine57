				...
				[
					...
					[
						...
						[
							...
							[
								SNew(SImage)
								.Image(ProjectItem->ProjectThumbnail ? ProjectItem->ProjectThumbnail.Get() : FAppStyle::Get().GetBrush("UnrealDefaultThumbnailRounded"))
...
void SProjectBrowser::Construct( const FArguments& InArgs )
{
	...
	SAssignNew(ProjectTileView, STileView<TSharedPtr<FProjectItem>>)
		...
		.AllowOverscroll(InArgs._AllowScrolling ? EAllowOverscroll::Yes : EAllowOverscroll::No)
		.ConsumeMouseWheel(InArgs._AllowScrolling ? EConsumeMouseWheel::Always : EConsumeMouseWheel::Never)
		.ScrollbarVisibility(InArgs._AllowScrolling ? EVisibility::Visible : EVisibility::Collapsed)
		.WheelScrollMultiplier(InArgs._AllowScrolling ? GetGlobalScrollAmount() : 0.f)
	...
	[
		...
		[
			SNew(SHorizontalBox)
			.Visibility(InArgs._ShowHeaderBar ? EVisibility::Visible : EVisibility::Collapsed)

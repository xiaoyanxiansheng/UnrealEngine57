	...
	[
		...
		[
			...
			[
				...
				.ShowHistoryButtons(ClassDomain == EClassDomain::Native || !ParentClassInfo.IsSet())
...
TSharedPtr<SWidget> SNewClassDialog::GetWidgetToFocusOnActivate()
{
	return MainWizard->GetWidgetToFocusOnActivate();
}
...
void SNewClassDialog::OnConfirmHotkeyPressed()
{
	// Advance to the name page
	constexpr int32 NamePageIdx = 1;
	if (MainWizard->CanShowPage(NamePageIdx))
	{
		MainWizard->ShowPage(NamePageIdx);
	}

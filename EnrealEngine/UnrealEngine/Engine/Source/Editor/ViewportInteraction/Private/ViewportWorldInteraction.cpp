...
#include UE_INLINE_GENERATED_CPP_BY_NAME(ViewportWorldInteraction)
		...
		if ( !bWasHandled )
		{
			...
			if ( HitHoverResult.HitObjectHandle.IsValid() )
			{
				...
				if ( HoveredActorComponent && IsInteractableComponent( HoveredActorComponent ) )
				{
					...
					{
						// This is a "manual" interface Cast<> to avoid emitting a spurious deprecation warning.
						// This code will all be deleted in UE 5.9.
						if (Actor->GetClass()->ImplementsInterface(UViewportInteractableInterface::StaticClass()))
						{
							IViewportInteractableInterface* Interactable =
								(IViewportInteractableInterface*)Actor->GetInterfaceAddress(UViewportInteractableInterface::StaticClass());
				...
				&& !IsOtherInteractorHoveringOverComponent( Interactor, NewHoveredActorComponent ) )
			{
				...
				if ( HoveredActor )
				{
					// This is a "manual" interface Cast<> to avoid emitting a spurious deprecation warning.
					// This code will all be deleted in UE 5.9.
					if (HoveredActor->GetClass()->ImplementsInterface(UViewportInteractableInterface::StaticClass()))
					{
						IViewportInteractableInterface* Interactable =
							(IViewportInteractableInterface*)HoveredActor->GetInterfaceAddress(UViewportInteractableInterface::StaticClass());

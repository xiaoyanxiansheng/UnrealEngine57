...
private:
	...
	**/
	...
	{
		if ((MessageType == EMessageSeverity::Error || MessageType == EMessageSeverity::Warning))
		{
...
public:
	...
	**/
	static void ProcessAndHandleCookErrorOutput(FString UATOutput)
	{
		...
		else if (UATOutput.Split(TEXT("Warning: "), &LhsUATOutputMsg, &ParsedCookIssue))
		{
			SyncMessageWithMessageLog(ParsedCookIssue, EMessageSeverity::Warning);
		}
		else if (UATOutput.Split(TEXT("Error: "), &LhsUATOutputMsg, &ParsedCookIssue))
		{
			SyncMessageWithMessageLog(ParsedCookIssue, EMessageSeverity::Error);
		}
	}
	...
	**/
	static void ClearAssetErrors() { bHasAssetErrors = false; }
...
public:
	...
	static ELogVerbosity::Type GetVerbosityFromStructuredLogString(FStringView Level)
	{
		if (Level == TEXT("Information"))
		{
			return ELogVerbosity::Log; // could be Display
		}
		else if (Level == TEXT("Debug"))
		{
			return ELogVerbosity::Verbose; // could be VeryVerbose
		}
		...
		else if (Level == TEXT("Critical"))
		{
			return ELogVerbosity::Error; // don't report external criticals as Fatal because they will then crash our process
		}
		else
		{
	...
	static void HandleUatProcessOutput(FString Output, TWeakPtr<SNotificationItem> NotificationItemPtr, FText PlatformDisplayName, FText TaskName)
	{
		...
		bool bStructuredMessage = false;

		// attempt to parse structured logging
		TSharedRef< TJsonReader<> > JsonReader = FJsonStringReader::Create(Output);
		TSharedPtr<FJsonObject> JsonData;
		if (FJsonSerializer::Deserialize(JsonReader, JsonData))
		{
			TSharedPtr<FJsonValue> LevelField = JsonData->TryGetField(TEXT("level"));
			TSharedPtr<FJsonValue> MessageField = JsonData->TryGetField(TEXT("message"));

			FString LevelString;
			FString MessageString;

			if (LevelField && LevelField->TryGetString(LevelString)
				&& MessageField && MessageField->TryGetString(Message))
			{
				Verbosity = GetVerbosityFromStructuredLogString(LevelString);
				bStructuredMessage = true;
			}

			// TODO add support for "format" field
		}
		...
		if (bDisplayLog)
		{
			FMsg::Logf(__FILE__, __LINE__,
				UATHelper.GetCategoryName(),
				Verbosity,
				TEXT("%s (%s): %s"), *TaskName.ToString(), *PlatformDisplayName.ToString(), *Message);
		}

		// deal with non-structured packaging/cooking warnings and errors
		if (!bStructuredMessage && (TaskName.EqualTo(LOCTEXT("PackagingTaskName", "Packaging")) || TaskName.EqualTo(LOCTEXT("CookingTaskName", "Cooking"))))
		{

...
#define UE_API MOVIESCENECAPTUREDIALOG_API
...
struct FMovieSceneCaptureBase : TSharedFromThis<FMovieSceneCaptureBase>
{
public:
	...
	UE_API virtual void OnCaptureFinished(bool bSuccess);
...
struct FInEditorCapture : FMovieSceneCaptureBase
{
	...
	UE_API virtual void OnCaptureStarted() override;
	UE_API virtual void Start() override;
	UE_API virtual void Cancel() override;
	UE_API virtual FCaptureState GetCaptureState() const override;
	// ~FMovieSceneCaptureBase interface

private:
	/** Overrides the Level Editor Play settings to specifically disable some things (such as audio playback) */
	UE_API void OverridePlaySettings(class ULevelEditorPlaySettings* PlayInEditorSettings);
	/** Called when the PIE Viewport is created. */
	UE_API void OnPIEViewportStarted();
	...
	UE_API void OnLevelSequenceFinished();
...
struct FNewProcessCapture : FMovieSceneCaptureBase
{

...
#include UE_INLINE_GENERATED_CPP_BY_NAME(SubobjectData)

#define LOCTEXT_NAMESPACE "SubobjectData"

PRAGMA_DISABLE_DEPRECATION_WARNINGS
#define REDIRECT_TO_ADAPTER(FunctionName, ...) \
	if (const FSubobjectDataAdapterBase* Adapter = USubobjectDataSubsystem::Get()->FindDataAdapter(*this))				\
	{																													\
		return Adapter->FunctionName(*this, ##__VA_ARGS__);																\
	}
...
{
	Initialize(ContextObject, InParentHandle, bInIsInheritedSCS);
}

void FSubobjectData::Initialize(UObject* ContextObject, const FSubobjectDataHandle& InParentHandle, const bool bInIsInheritedSCS)
{
	WeakObjectPtr = ContextObject;
	ParentObjectHandle = InParentHandle;
	SCSNodePtr = nullptr;
	
	FSubobjectDataSubsystemContextData_SingleUObjectContextObject SingleUObject(ContextObject);
	ContextData.InitializeAsScriptStruct(FSubobjectDataSubsystemContextData_SingleUObjectContextObject::StaticStruct(), reinterpret_cast<const uint8*>(&SingleUObject));
	
	// Check for a BP added CAC
	if (USCS_Node* SCS = Cast<USCS_Node>(ContextObject))
	{
		if (SCS->ComponentTemplate && SCS->ComponentTemplate->IsA<UChildActorComponent>())
		{
			bIsChildActor = true;
		}
	}
...
void FSubobjectData::Initialize(TInstancedStruct<FSubobjectDataSubsystemContextDataBase> InContextData, const FSubobjectDataHandle& ParentHandle)
{
	...
	ContextData = MoveTemp(InContextData);
	ParentObjectHandle = ParentHandle;
}

bool FSubobjectData::CanEdit() const
{
	REDIRECT_TO_ADAPTER(CanEdit);
...
{
	REDIRECT_TO_ADAPTER(CanDuplicate);
...
{
	REDIRECT_TO_ADAPTER(CanReparent);
...
{
	REDIRECT_TO_ADAPTER(GetObjectForBlueprint, Blueprint);
...
{
	REDIRECT_TO_ADAPTER(IsInstancedComponent);
...
{
	REDIRECT_TO_ADAPTER(FindMutableComponentInstanceInActor, InActor);
...
{
	REDIRECT_TO_ADAPTER(GetBlueprint);
...
{
	REDIRECT_TO_ADAPTER(GetBlueprintBeingEdited);
...
{
	REDIRECT_TO_ADAPTER(GetDisplayString, bShowNativeComponentNames);
...
{
	REDIRECT_TO_ADAPTER(GetDragDropDisplayText);
...
{
	REDIRECT_TO_ADAPTER(GetDisplayNameContextModifiers, bShowNativeComponentNames);
...
{
	REDIRECT_TO_ADAPTER(GetDisplayName);
	
	if (bIsChildActor)
	{
		if (const UChildActorComponent* CAC = GetObject<UChildActorComponent>())
		{
...
{
	REDIRECT_TO_ADAPTER(GetVariableName);
...
{
	REDIRECT_TO_ADAPTER(GetSocketName);
...
{
	REDIRECT_TO_ADAPTER(GetSocketFName);
...
{
	REDIRECT_TO_ADAPTER(HasValidSocket);
...
{
	REDIRECT_TO_ADAPTER(SetSocketName, InNewName);
...
{
	REDIRECT_TO_ADAPTER(SetupAttachment, SocketName, AttachParentHandle);
...
{
	REDIRECT_TO_ADAPTER(FindChildByObject, ContextObject)
...
{
	REDIRECT_TO_ADAPTER(GetAssetName)
...
{
	REDIRECT_TO_ADAPTER(GetAssetPath)
...
{
	REDIRECT_TO_ADAPTER(IsAssetVisible)
...
{
	REDIRECT_TO_ADAPTER(GetMobilityToolTipText)
...
{
	REDIRECT_TO_ADAPTER(GetComponentEditorOnlyTooltipText)
...
{
	REDIRECT_TO_ADAPTER(GetIntroducedInToolTipText)
...
{
	REDIRECT_TO_ADAPTER(GetActorDisplayText)
...
const UObject* FSubobjectData::GetObject(bool bEvenIfPendingKill) const
{
	return WeakObjectPtr.Get(bEvenIfPendingKill);
}

const TInstancedStruct<FSubobjectDataSubsystemContextDataBase>& FSubobjectData::GetContextData() const
{
	return ContextData;
}
...
{
	REDIRECT_TO_ADAPTER(IsInstancedActor)
...
{
	REDIRECT_TO_ADAPTER(IsNativeComponent)
...
{
	REDIRECT_TO_ADAPTER(IsBlueprintInheritedComponent)
...
{
	REDIRECT_TO_ADAPTER(IsInheritedComponent)
...
{
	REDIRECT_TO_ADAPTER(IsSceneComponent)
...
{
	REDIRECT_TO_ADAPTER(IsRootComponent)
...
{
	REDIRECT_TO_ADAPTER(IsDefaultSceneRoot)
...
{
	REDIRECT_TO_ADAPTER(SceneRootHasDefaultName)
...
{
	REDIRECT_TO_ADAPTER(IsComponent)
...
{
	REDIRECT_TO_ADAPTER(IsChildActor)
...
{
	REDIRECT_TO_ADAPTER(IsChildActorSubtreeObject)
...
{
	REDIRECT_TO_ADAPTER(IsRootActor)
...
{
	REDIRECT_TO_ADAPTER(IsInstancedInheritedComponent)
...
{
	REDIRECT_TO_ADAPTER(IsAttachedTo, InHandle)

...
#include UE_INLINE_GENERATED_CPP_BY_NAME(SubobjectDataSubsystem)
...
FSubobjectDataAdapterHandle USubobjectDataSubsystem::RegisterSubobjectDataAdapter(
	FSubobjectDataAdapterFilter InSubobjectDataAdapterFilter,
	TUniquePtr<FSubobjectDataAdapterBase> InDataAdapter)
{
	FSubobjectDataAdapterHandle Handle;
	// Pre-increment so that invalid handles are 0
	Handle.Opaque = ++DataAdapterHandleIndex;

	DataAdapters.Emplace(FSubobjectDataAdapterEntry
		{
			.Adapter = MoveTemp(InDataAdapter),
			.Handle = Handle,
			.Filter = MoveTemp(InSubobjectDataAdapterFilter)
		});
	return Handle;
}

void USubobjectDataSubsystem::UnregisterSubobjectDataAdapter(FSubobjectDataAdapterHandle Handle)
{
	DataAdapters.RemoveAll([Handle](const FSubobjectDataAdapterEntry& Entry)
		{
			return Entry.Handle.Opaque == Handle.Opaque;
		});
}

const FSubobjectDataAdapterBase* USubobjectDataSubsystem::FindDataAdapter(const FSubobjectData& SubobjectData) const
{
	for (const FSubobjectDataAdapterEntry& Entry : DataAdapters)
	{
		if (Entry.Filter(SubobjectData))
		{
...
FSubobjectDataSubsystemAdapterHandle USubobjectDataSubsystem::RegisterSubsystemAdapter(
	FSubobjectSubsystemAdapterFilter InFilter,
	TUniquePtr<FSubobjectDataSubsystemAdapterBase> InAdapter)
{
	FSubobjectDataSubsystemAdapterHandle Handle;
	// Pre-increment so that invalid handles are 0
	Handle.Opaque = ++SubsystemAdapterHandleIndex;
	
	SubsystemAdapters.Emplace(FSubsystemAdapterEntry
		{
...
void USubobjectDataSubsystem::UnregisterSubsystemAdapter(FSubobjectDataSubsystemAdapterHandle Handle)
{
	SubsystemAdapters.RemoveAll([Handle](const FSubsystemAdapterEntry& Entry)
	{
		return Entry.Handle.Opaque == Handle.Opaque;
	});
}

const FSubobjectDataSubsystemAdapterBase* USubobjectDataSubsystem::FindSubsystemAdapter(
	const TInstancedStruct<FSubobjectDataSubsystemContextDataBase>& Struct) const
{
	for (const FSubsystemAdapterEntry& Entry : SubsystemAdapters)
	{
		if (Entry.Filter(Struct))
		{
			return Entry.Adapter.Get();
		}
	}
...
void USubobjectDataSubsystem::GatherSubobjectData(UObject* Context, TArray<FSubobjectDataHandle>& OutArray)
{
	...
	FSubobjectDataSubsystemContextData_SingleUObjectContextObject ContextDataAdapter_OnStack(Context);
	TConstStructView<FSubobjectDataSubsystemContextDataBase> StructView(ContextDataAdapter_OnStack);
	TInstancedStruct<FSubobjectDataSubsystemContextDataBase> ContextDataAdapter(StructView);
	
	if (const FSubobjectDataSubsystemAdapterBase* Adapter = FindSubsystemAdapter(ContextDataAdapter))
	{
		return Adapter->GatherSubobjectData(
			ContextDataAdapter,
			OutArray,
			[this]() { return this->CreateSubobjectData(); });
	}
	...
	if (!bIsInstanced)
	{
		...
		// from the constructor/ObjectInitializer
			...
			if (ParentSCS)
			{
				...
				for (int32 NodeIndex = 0; NodeIndex < SCS_RootNodes.Num(); ++NodeIndex)
				{
					...
					else
					{
						...
						if (RootComponentHandle.IsValid() && SCS_Node->ComponentTemplate && SCS_Node->ComponentTemplate->IsA<USceneComponent>())
						{
...
int32 USubobjectDataSubsystem::DeleteSubobjects(const FSubobjectDataHandle& ContextHandle, const TArray<FSubobjectDataHandle>& SubobjectsToDelete, FSubobjectDataHandle& OutComponentToSelect, UBlueprint* BPContext/* = nullptr*/, bool bForce /* = false */)
{
	...
	const FSubobjectData* SubobjectData = ContextHandle.GetData();
	if (!SubobjectData)
	{
		return NumDeletedSubobjects;
	}
	
	if (const FSubobjectDataSubsystemAdapterBase* Adapter = FindSubsystemAdapter(SubobjectData->GetContextData()))
	{
		NumDeletedSubobjects = Adapter->DeleteSubobjects(
			ContextHandle,
			SubobjectsToDelete);
	...
	UObject* ContextObj = SubobjectData->GetMutableObject();
	check(ContextObj);
...
bool USubobjectDataSubsystem::CanCopySubobjects(const TArray<FSubobjectDataHandle>& Handles) const
{
	...
	for (const FSubobjectDataHandle& Handle : Handles)
    {
    	if(FSubobjectData* Data = Handle.GetData())
    	{
			...
			const FSubobjectDataHandle& ActorRootHandle = GetActorRootHandle(Handle);
			if (const FSubobjectData* ActorData = ActorRootHandle.GetData())
			{
				ComponentTemplate = Data->FindMutableComponentInstanceInActor(ActorData->GetObject<AActor>());
			}
			
			if (!ComponentTemplate)
			{
				ComponentTemplate = Data->GetMutableComponentTemplate();
			}
...
FSubobjectDataHandle USubobjectDataSubsystem::CreateSubobjectData()
{
	TSharedPtr<FSubobjectData> SharedPtr = MakeShared<FSubobjectData>();
...
{
	TSharedPtr<FSubobjectData> SharedPtr = TSharedPtr<FSubobjectData>(new FSubobjectData());
	SharedPtr->Initialize(Context, ParentHandle, bIsInheritedSCS);

...
void FFontEditorViewportClient::Draw(FViewport* Viewport, FCanvas* Canvas)
{
	...
	else
	{
		...
		if (Font->FontCacheType == EFontCacheType::Runtime)
		{
			...
			TArrayView<const FTypefaceEntry> TypefaceEntries;
			if (const FCompositeFont* CompositeFont = Font->GetCompositeFont())
			{
				TypefaceEntries = CompositeFont->DefaultTypeface.Fonts;
			}

			// Draw and measure each name so we can work out where to start drawing the preview text column
			for (const FTypefaceEntry& TypefaceEntry : TypefaceEntries)
			{

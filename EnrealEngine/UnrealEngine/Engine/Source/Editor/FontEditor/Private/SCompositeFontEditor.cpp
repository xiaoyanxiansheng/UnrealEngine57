...
FCompositeFont* SCompositeFontEditor::GetCompositeFont() const
{
	...
	return (FontObject) ? &FontObject->GetMutableInternalCompositeFont() : nullptr;
}

FTypeface* SCompositeFontEditor::GetDefaultTypeface() const
{
	UFont* const FontObject = GetFontObject();
	return (FontObject) ? &FontObject->GetMutableInternalCompositeFont().DefaultTypeface : nullptr;
}
...
{
	UFont* const FontObject = GetFontObject();
	return (FontObject) ? &FontObject->GetMutableInternalCompositeFont().FallbackTypeface.Typeface : nullptr;
}
...
{
	UFont* const FontObject = GetFontObject();
	return (FontObject) ? &FontObject->GetMutableInternalCompositeFont().FallbackTypeface : nullptr;
}
...
void SCharacterRangeEditor::Construct(const FArguments& InArgs)
{
	...
	TSharedPtr<FUnicodeBlockRange> CurrentRangeSelectionItem;
	{
		...
		for(const FUnicodeBlockRange& UnicodeBlockRange : UnicodeBlockRanges)
		{
			...
			if(CurrentRangeSelection.IsSet() && CurrentRangeSelection->GetRange() == UnicodeBlockRange.GetRange())
			{
...
void SCharacterRangeEditor::CacheCurrentRangeSelection()
{
	...
	if(CharacterRangePtr)
	{
		for(const FUnicodeBlockRange& UnicodeBlockRange : UnicodeBlockRanges)
		{
			if(UnicodeBlockRange.GetRange() == *CharacterRangePtr)
			{
...
{
	return CurrentRangeSelection.IsSet() ? CurrentRangeSelection->GetDisplayName() : LOCTEXT("UnicodeBlock_CustomSelection", "Custom");
}
...
void SCharacterRangeEditor::OnRangeSelectionComboOpening()
{
	...
	{
		return One->GetDisplayName().CompareTo(Two->GetDisplayName()) < 0;
	});
...
void SCharacterRangeEditor::OnRangeSelectionChanged(TSharedPtr<FUnicodeBlockRange> InNewRangeSelection, ESelectInfo::Type)
{
	if(InNewRangeSelection.IsValid())
	{
		...
		if(CharacterRangePtr)
		{
			...
			*CharacterRangePtr = InNewRangeSelection->GetRange();
...
TSharedRef<SWidget> SCharacterRangeEditor::MakeRangeSelectionWidget(TSharedPtr<FUnicodeBlockRange> InRangeSelection)
{
	return SNew(STextBlock)
		.Text(InRangeSelection->GetDisplayName())
		.ToolTipText(FText::Format(LOCTEXT("RangeSelectionTooltipFmt", "{0} ({1})"), InRangeSelection->GetDisplayName(), FText::AsCultureInvariant(FString::Printf(TEXT("0x%04x - 0x%04x"), InRangeSelection->RangeLower, InRangeSelection->RangeUpper))));
}

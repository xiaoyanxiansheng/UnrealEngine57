...
class FEditorGizmoSelectionBuilderHelper
{
public:
	/**
	 * Utility method that creates a transform proxy based on the current selection.
	 */
	static UE_API UTransformProxy* CreateTransformProxyForSelection(const FToolBuilderState& SceneState);
};

UINTERFACE(MinimalAPI)
class UEditorInteractiveGizmoSelectionBuilder : public UInterface
{
	GENERATED_BODY()
};

...
protected:
	...
	UE_API UEditorInteractiveGizmoManager();
	...
	UE_API virtual void InitializeWithEditorModeManager(IToolsContextQueriesAPI* QueriesAPI, IToolsContextTransactionsAPI* TransactionsAPI, UInputRouter* InputRouter, FEditorModeTools* InEditorModeManager);
...
public:
	 ...
	 */
	UE_API void RegisterEditorGizmoType(EEditorGizmoCategory InGizmoCategory, UInteractiveGizmoBuilder* InGizmoBuilder);
	...
	*/
	UE_API void DeregisterEditorGizmoType(EEditorGizmoCategory InGizmoCategory, UInteractiveGizmoBuilder* InGizmoBuilder);
	 ...
	 */
	UE_API void GetQualifiedEditorGizmoBuilders(EEditorGizmoCategory InGizmoCategory, const FToolBuilderState& InToolBuilderState, TArray<UInteractiveGizmoBuilder*>& InFoundBuilders);
	 ...
	 */
	UE_API virtual bool DestroyEditorGizmo(UInteractiveGizmo* Gizmo);

	/**
	 * Shutdown and remove all active auto gizmos
	 */
	UE_API virtual void DestroyAllEditorGizmos();

	/** Try to activate a new Gizmo instance (UInteractiveGizmoManager override) */	
	UE_API virtual UInteractiveGizmo* CreateGizmo(
	...
	static UE_API FOnUsesNewTRSGizmosChanged& OnUsesNewTRSGizmosChangedDelegate();

	/**
	 * Notifies FGizmosParameters changes across bound transform gizmos
	 */
	static UE_API void SetGizmosParameters(const FGizmosParameters& InParameters);
    ...
    static UE_API FOnGizmosParametersChanged& OnGizmosParametersChangedDelegate();

	/**
	 * Returns the default gizmos parameters if set
	 */
    static UE_API const TOptional<FGizmosParameters>& GetDefaultGizmosParameters();

	/**
	 * Returns true if the Explicit mode is enabled;
	 */
	static UE_API bool IsExplicitModeEnabled();
...
protected:
	 ...
	 */
	UE_API virtual bool GetShowEditorGizmos();

	/**
	 * Returns true if gizmos should be visible based on the current view's engine show flag.
	 * @todo move this to a gizmo context object
	 */
	UE_API virtual bool GetShowEditorGizmosForView(IToolsContextRenderAPI* RenderAPI);

	/**
	 * Updates active selection gizmos when show selection state changes
	 */
	UE_API void UpdateActiveEditorGizmos();
...
private:
	...
	UE_API UTransformGizmo* FindDefaultTransformGizmo() const;

	UE_API void OnEditorModeChanged(const FName& InModeID, bool bInIsEnteringMode);
};

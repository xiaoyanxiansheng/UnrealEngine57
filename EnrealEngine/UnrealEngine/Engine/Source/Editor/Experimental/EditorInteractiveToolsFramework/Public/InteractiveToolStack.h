...
public:
	 ...
	 */
	static UE_API void RegisterToolStack(UEdMode* EdMode, UEditorInteractiveToolsContext* UseToolsContext, TSharedPtr<FUICommandInfo> UICommand, FString ToolStackIdentifier);
	 ...
	 */
	static UE_API void AddToolToStack(UEdMode* EdMode, UEditorInteractiveToolsContext* UseToolsContext, TSharedPtr<FUICommandInfo> UICommand, FString ToolStackIdentifier, FString ToolIdentifier, UInteractiveToolBuilder* Builder);

public:

	/** Add the tool to our internal tool array */
	UE_API int32 AddTool(const FString& InToolName, const TSharedPtr<FUICommandInfo>& InUICommand);

	/** Signal that a tool was activated increasing its priority in the toolstack for future chord inputs */
	UE_API void NotifyToolActivated(const FString& InToolName);

	/** Get Tool that should be activated for the given input chord */
	UE_API const FString GetLastActiveToolByChord(const FInputChord& InInputChord) const;

	/** Get most recently activated tool within this stack */
	UE_API const FString GetLastActiveTool() const;

	/** Get most recently activated tool's icon within this stack */
	UE_API const FSlateIcon& GetLastActiveToolIcon() const;

	/** Get most recently activated tool's label within this stack */
	UE_API const FText& GetLastActiveToolLabel() const;

	/** Get most recently activated tool's description within this stack */
	UE_API const FText& GetLastActiveToolDescription() const;

	/** Get the original tool stack */
	UE_API const TArray<TPair<FString, TSharedPtr<FUICommandInfo>>>& GetStack() const;

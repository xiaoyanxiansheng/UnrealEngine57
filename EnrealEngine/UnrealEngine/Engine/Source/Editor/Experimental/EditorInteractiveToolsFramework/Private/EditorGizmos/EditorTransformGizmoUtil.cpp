...
void UEditorTransformGizmoContextObject::UpdateGizmo(const TArray<FEditorViewportClient*>& InViewportClients) const
{
	...
	// swap default mode if not dealing with the level editor mode manager
	const bool bIsLevelViewportClient = [&ViewportClients]()
	{
		if (GEditor)
		{
			return GEditor->GetLevelViewportClients().ContainsByPredicate([&ViewportClients](const FEditorViewportClient* LevelViewportClient)
			{
				return ViewportClients.Contains(LevelViewportClient);
			});
		}
		return false;
	}();
	
	const bool Swapped = !bIsLevelViewportClient ? SwapDefaultMode(FBuiltinEditorModes::EM_Default, FAssetEdModes::EM_AssetDefault) : false;
...
void UEditorTransformGizmoContextObject::InitializeViewportsBinding()
{
	if (GEditor)
	{
		...
		auto OnLevelViewportClientsChanged = [this]()
		{
			if (!ModeTools)
			{
				return;
			...
			const bool bIsLevelViewportClient =
			GEditor->GetLevelViewportClients().ContainsByPredicate([this](const FLevelEditorViewportClient* ViewportClient)
			{
				return ViewportClient && ViewportClient->GetModeTools() == ModeTools;
			});

			if (bIsLevelViewportClient)
			{
				// ensure level default mode
				ModeTools->RemoveDefaultMode(FAssetEdModes::EM_AssetDefault);
				ModeTools->AddDefaultMode(FBuiltinEditorModes::EM_Default);

				if (ModeTools->IsModeActive(FAssetEdModes::EM_AssetDefault))
				{
					ModeTools->DeactivateMode(FAssetEdModes::EM_AssetDefault);
					ModeTools->ActivateMode(FBuiltinEditorModes::EM_Default);
				}
			}
		};

		if (!LevelViewportClientsChangedHandle.IsValid())
		{
			LevelViewportClientsChangedHandle = GEditor->OnLevelViewportClientListChanged().AddLambda(OnLevelViewportClientsChanged);
		}
...
void UEditorTransformGizmoContextObject::RemoveViewportsBinding()
{
	...
	if (LevelViewportClientsChangedHandle.IsValid())
	{
		if (ensure(GEditor))
		{
			GEditor->OnLevelViewportClientListChanged().Remove(LevelViewportClientsChangedHandle);
		}
		LevelViewportClientsChangedHandle.Reset();
	}
}

...
void FEditor2DScrollBehaviorTarget::OnClickPress(const FInputDeviceRay& PressPos)
{
	if (ViewportClient->IsOrtho())
	{
		DragStart = FVector3d(PressPos.ScreenPosition.X, PressPos.ScreenPosition.Y, 0.0);
...
{
	if (ViewportClient->IsPerspective() || ViewportClient->GetViewportType() == LVT_OrthoFreelook)
	{
	...
	{
		const float UnitsPerPixel = ViewportClient->GetOrthoUnitsPerPixel(ViewportClient->Viewport) * ViewportClient->GetDPIScale();
		const double DX = DragPos.ScreenPosition.X - DragStart.X;
		const double DY = DragPos.ScreenPosition.Y - DragStart.Y;

		FVector CameraDisplacement(0.0, 0.0, 0.0);
		switch (ViewportClient->GetViewportType())
		{
		case LVT_OrthoXY:
			CameraDisplacement = FVector(-DX * UnitsPerPixel, -DY * UnitsPerPixel, 0.0);
			break;
		case LVT_OrthoXZ:
			CameraDisplacement = FVector(-DX * UnitsPerPixel, 0.0, DY * UnitsPerPixel);
			break;
		case LVT_OrthoYZ:
			CameraDisplacement = FVector(0.0, -DX * UnitsPerPixel, DY * UnitsPerPixel);
			break;
		...
		case LVT_OrthoNegativeYZ:
			...
			break;
		default:
			checkf(false, TEXT("Unexpected viewport type in FEditor2DScrollBehaviorTarget"));
			break;
		}

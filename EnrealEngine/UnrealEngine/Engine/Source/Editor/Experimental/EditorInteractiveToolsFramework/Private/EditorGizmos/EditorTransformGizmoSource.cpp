...
#include UE_INLINE_GENERATED_CPP_BY_NAME(EditorTransformGizmoSource)
...
bool UEditorTransformGizmoSource::GetVisible(const EViewportContext InViewportContext) const
{
	if (const FEditorViewportClient* ViewportClient = GetViewportClient(InViewportContext)) 
	{
		...
		const bool bEngineShowFlagsModeWidget = ViewportClient->EngineShowFlags.ModeWidgets;
		...
		{
			// Always show for hover even in game mode
			constexpr bool bCanHoverInGameMode = true;
			return bCanHoverInGameMode;
		}

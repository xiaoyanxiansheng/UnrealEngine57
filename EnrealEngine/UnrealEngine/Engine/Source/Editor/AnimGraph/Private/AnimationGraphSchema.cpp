...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AnimationGraphSchema)
...
void UAnimationGraphSchema::DroppedAssetsOnGraph( const TArray<FAssetData>& Assets, const FVector2f& GraphPosition, UEdGraph* Graph ) const 
{
	if (Graph != nullptr)
	{
		if (UAnimationAsset* AnimationAsset = FAssetData::GetFirstAsset<UAnimationAsset>(Assets))
		{
			...
			if (!AssetReferenceFilter.IsValid() || AssetReferenceFilter->PassesFilter(FAssetData(AnimationAsset)))
			{
		...
		{
			const TSharedPtr<IAssetReferenceFilter> AssetReferenceFilter = MakeAssetReferenceFilter(Graph);
			if (!AssetReferenceFilter.IsValid() || AssetReferenceFilter->PassesFilter(FAssetData(PhysicsAsset)))
			{
...
void UAnimationGraphSchema::DroppedAssetsOnNode(const TArray<FAssetData>& Assets, const FVector2f& GraphPosition, UEdGraphNode* Node) const
{
	...
	if ((Asset != nullptr) && (K2Node!= nullptr))
	{
		const TSharedPtr<IAssetReferenceFilter> AssetReferenceFilter = MakeAssetReferenceFilter(K2Node->GetGraph());
		if (!AssetReferenceFilter.IsValid() || AssetReferenceFilter->PassesFilter(FAssetData(Asset)))
		{
...
{
	UAnimationAsset* Asset = FAssetData::GetFirstAsset<UAnimationAsset>(Assets);
	if ((Asset != nullptr) && (Pin != nullptr))
	{
		const TSharedPtr<IAssetReferenceFilter> AssetReferenceFilter = MakeAssetReferenceFilter(Pin->GetOwningNode()->GetGraph());
		if (!AssetReferenceFilter.IsValid() || AssetReferenceFilter->PassesFilter(FAssetData(Asset)))
		{
...
{ 
	UAnimationAsset* Asset = FAssetData::GetFirstAsset<UAnimationAsset>(Assets);
	if ((Asset == nullptr) || (HoverNode == nullptr) || !HoverNode->IsA(UAnimGraphNode_Base::StaticClass()))
	{
	...
	{
		const TSharedPtr<IAssetReferenceFilter> AssetReferenceFilter = MakeAssetReferenceFilter(HoverNode->GetGraph());
		FText FailureReason;
		if (AssetReferenceFilter.IsValid() && !AssetReferenceFilter->PassesFilter(FAssetData(Asset), &FailureReason))
		{
			OutOkIcon = false;
...
{ 
	UAnimationAsset* Asset = FAssetData::GetFirstAsset<UAnimationAsset>(Assets);
	if ((Asset == nullptr) || (HoverPin == nullptr))
	{
		OutTooltipText.Reset();
		OutOkIcon = false;
		return;
	...
	if (bSkelMatch && bTypeMatch && bDirectionMatch)
	{
		const TSharedPtr<IAssetReferenceFilter> AssetReferenceFilter = MakeAssetReferenceFilter(HoverPin->GetOwningNode()->GetGraph());
		FText FailureReason;
		if (AssetReferenceFilter.IsValid() && !AssetReferenceFilter->PassesFilter(FAssetData(Asset), &FailureReason))
		{
			OutOkIcon = false;
...
void UAnimationGraphSchema::GetAssetsGraphHoverMessage(const TArray<FAssetData>& Assets, const UEdGraph* HoverGraph, FString& OutTooltipText, bool& OutOkIcon) const
{
	if (UAnimationAsset* AnimationAsset = FAssetData::GetFirstAsset<UAnimationAsset>(Assets))
	{
		...
		else if(UAnimMontage* Montage = Cast<UAnimMontage>(AnimationAsset))
		{
			OutOkIcon = false;
		...
		else
		{
			...
			FText FailureReason;
			if (AssetReferenceFilter.IsValid() && !AssetReferenceFilter->PassesFilter(FAssetData(AnimationAsset), &FailureReason))
			{
				OutOkIcon = false;
				OutTooltipText = FailureReason.ToString();
			}
	...
	else if(UPhysicsAsset* PhysicsAsset = FAssetData::GetFirstAsset<UPhysicsAsset>(Assets))
	{
		const TSharedPtr<IAssetReferenceFilter> AssetReferenceFilter = MakeAssetReferenceFilter(HoverGraph);
		FText FailureReason;
		if (AssetReferenceFilter.IsValid() && !AssetReferenceFilter->PassesFilter(FAssetData(PhysicsAsset), &FailureReason))
		{
			OutOkIcon = false;
			OutTooltipText = FailureReason.ToString();
		}
...
TSharedPtr<IAssetReferenceFilter> UAnimationGraphSchema::MakeAssetReferenceFilter(const UEdGraph* Graph)
{

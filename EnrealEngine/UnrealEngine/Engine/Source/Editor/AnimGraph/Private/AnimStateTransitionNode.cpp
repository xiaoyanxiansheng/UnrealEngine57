...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AnimStateTransitionNode)
	...
	: Super(ObjectInitializer)
{
	...
	bAllowInertializationForSelfTransitions = false;	// Defaults to false to preserve existing behavior
}
...
void UAnimStateTransitionNode::PostPlacedNewNode()
{
	...
	// Only newly placed nodes fall back to inertialization for self transitions
	bAllowInertializationForSelfTransitions = true;
}
...
void UAnimStateTransitionNode::RelinkHead(UAnimStateNodeBase* NewTargetState)
{
	...
	TargetStateBeforeRelinking->GetInputPin()->BreakLinkTo(GetOutputPin());

	// Add the new incoming transition to the new target state
	NewTargetState->GetInputPin()->Modify();
	NewTargetState->GetInputPin()->MakeLinkTo(GetOutputPin());
...
void UAnimStateTransitionNode::RelinkTail(UAnimStateNodeBase* NewSourceState)
{
	UAnimStateNodeBase* SourceStateBeforeRelinking = GetPreviousState();

	// Remove the outgoing transition from the previous source state
	SourceStateBeforeRelinking->GetOutputPin()->Modify();
	SourceStateBeforeRelinking->GetOutputPin()->BreakLinkTo(GetInputPin());

	// Add the new outgoing transition to the new source state
	NewSourceState->GetOutputPin()->Modify();
	NewSourceState->GetOutputPin()->MakeLinkTo(GetInputPin());

	// Relink the source state of the transition node
	Pins[0]->Modify();
	Pins[0]->BreakLinkTo(SourceStateBeforeRelinking->GetOutputPin());
	Pins[0]->MakeLinkTo(NewSourceState->GetOutputPin());
}

void UAnimStateTransitionNode::Reverse()
{
	UAnimStateNodeBase* OldSourceState = GetPreviousState();
	UAnimStateNodeBase* OldTargetState = GetNextState();
	UAnimStateNodeBase* NewTargetState = OldSourceState;
	UAnimStateNodeBase* NewSourceState = OldTargetState;

	// Remove the incoming transition from the previous target state
	OldSourceState->GetOutputPin()->Modify();
	OldSourceState->GetOutputPin()->BreakLinkTo(OldTargetState->GetInputPin());

	// Add the new incoming transition to the new target state
	NewSourceState->GetOutputPin()->Modify();
	NewSourceState->GetOutputPin()->MakeLinkTo(NewTargetState->GetInputPin());

	// Relink the target state of the transition node
	Pins[0]->Modify();
	Pins[0]->BreakLinkTo(OldSourceState->GetOutputPin());
	Pins[0]->MakeLinkTo(NewSourceState->GetOutputPin());

	Pins[1]->Modify();
	Pins[1]->BreakLinkTo(OldTargetState->GetInputPin());
	Pins[1]->MakeLinkTo(NewTargetState->GetInputPin());
}
...
{
	if (SourcePin->Direction == EGPD_Input && OldTargetPin->Direction == EGPD_Output)
	{
		Swap(SourcePin, OldTargetPin);
	}
	...
	for (int i = TransitionNodeCandidates.Num() - 1; i >= 0; i--)
	{
		...
		UEdGraphNode* OldTargetNode = OldTargetPin->GetOwningNode();
...
void UAnimStateTransitionNode::GetNodeContextMenuActions(UToolMenu* Menu, UGraphNodeContextMenuContext* Context) const
{
	// Add transition actions
	{
		FToolMenuSection& Section = Menu->AddSection("AnimTransitionNode", LOCTEXT("AnimTransitionNodeHeading", "Transition"));
		Section.AddMenuEntry(FAnimGraphCommands::Get().ReverseTransition);
	}
}

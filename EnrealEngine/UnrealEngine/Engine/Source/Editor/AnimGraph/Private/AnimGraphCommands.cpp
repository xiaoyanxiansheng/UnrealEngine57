...
UE_DEFINE_TCOMMANDS(FAnimGraphCommands)
...
void FAnimGraphCommands::RegisterCommands()
{
	...
	UI_COMMAND( CreateSelfTransition, "Create Self Transition", "Creates an alias for this state and adds a 'self' transition from the alias to this state", EUserInterfaceActionType::Button, FInputChord() )
	UI_COMMAND( SetActiveState, "Set Active State", "Sets the currently-selected state as active", EUserInterfaceActionType::Button, FInputChord() )
	UI_COMMAND( ReverseTransition, "Reverse Transition", "Reverses this transition", EUserInterfaceActionType::Button, FInputChord() )
}

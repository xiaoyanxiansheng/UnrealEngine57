...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AnimGraphNode_LookAt)
...
void UAnimGraphNode_LookAt::CopyNodeDataToPreviewNode(FAnimNode_Base* InPreviewNode)
{
	FAnimNode_LookAt* LookAt = static_cast<FAnimNode_LookAt*>(InPreviewNode);

	// copies Pin values from the internal node to get data which are not compiled yet
	LookAt->LookAtLocation = Node.LookAtLocation;
}

void UAnimGraphNode_LookAt::CopyPinDefaultsToNodeData(UEdGraphPin* InPin)
{
	if (InPin->GetName() == GET_MEMBER_NAME_STRING_CHECKED(FAnimNode_LookAt, LookAtLocation))
	{
		GetDefaultValue(GET_MEMBER_NAME_STRING_CHECKED(FAnimNode_LookAt, LookAtLocation), Node.LookAtLocation);
	}
}

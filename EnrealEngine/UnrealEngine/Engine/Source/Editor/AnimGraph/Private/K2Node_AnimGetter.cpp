...
#include UE_INLINE_GENERATED_CPP_BY_NAME(K2Node_AnimGetter)
...
void UK2Node_AnimGetter::GetMenuActions(FBlueprintActionDatabaseRegistrar& ActionRegistrar) const
{
	...
	if(AnimBlueprint && ActionRegistrar.IsOpenForRegistration(AnimBlueprint))
	{
		...
		if(BPClass)
		{
			...
			FBlueprintEditorUtils::GetAllNodesOfClass(AnimBlueprint, AliasNodes);
			...
			for(UFunction* Getter : AnimGetters)
			{
				...
				else if(GetterRequiresParameter(Getter, TEXT("MachineIndex")))
				{
					if(GetterRequiresParameter(Getter, TEXT("StateIndex")))
					{
						...
						for (UAnimStateAliasNode* AliasNode : AliasNodes)
						{
							// Get the state machine node from the outer chain
							UAnimationStateMachineGraph* Graph = Cast<UAnimationStateMachineGraph>(AliasNode->GetOuter());
							if (Graph)
							{
								if (UAnimGraphNode_StateMachine* MachineNode = Cast<UAnimGraphNode_StateMachine>(Graph->GetOuter()))
								{
							...
							Params.SourceStateNode = AliasNode;
							UpdateCachedTitle(Params);

							UBlueprintNodeSpawner* Spawner = UBlueprintNodeSpawner::Create(UK2Node_AnimGetter::StaticClass(), /*AliasNode->GetGraph()*/nullptr, UBlueprintNodeSpawner::FCustomizeNodeDelegate::CreateUObject(const_cast<UK2Node_AnimGetter*>(this), &UK2Node_AnimGetter::PostSpawnNodeSetup, Params));
							Spawner->DynamicUiSignatureGetter = UBlueprintNodeSpawner::FUiSpecOverrideDelegate::CreateStatic(UiSpecOverride, Params.CachedTitle);
							ActionRegistrar.AddBlueprintAction(AnimBlueprint, Spawner);
						}
					}
					...
					if(SourceNode)
					{
						if(UAnimStateTransitionNode* TransitionNode = Cast<UAnimStateTransitionNode>(CurrGraph->GetOuter()))
						{
							if(SourceStateNode)
							{
								...
								else if (UAnimStateAliasNode* PreviousAliasNode = Cast<UAnimStateAliasNode>(TransitionNode->GetPreviousState()))
								{
									// Only allow actions using states that are referencing the previous alias
									if (SourceStateNode == PreviousAliasNode)
									{
										return false;
									}
								}
...
{
	// Make sure we have a valid Blueprint owner before proceeding. Otherwise we will assert trying to resolve the target function below.
	if (!ensureMsgf(HasValidBlueprint(), TEXT("UK2Node_AnimGetter::UpdateCachedTitle(): Attempting to update cached title for [%s] without a valid Blueprint owner (this is unexpected)."), *GetFullName()))
	{
		return;
	}
	...
	{
		CachedTitle = GenerateTitle(Function, SourceStateNode, SourceNode);
	}
}

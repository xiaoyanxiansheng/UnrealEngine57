...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AnimationStateMachineSchema)
...
bool UAnimationStateMachineSchema::TryRelinkConnectionTarget(UEdGraphPin* SourcePin, UEdGraphPin* OldTargetPin, UEdGraphPin* NewTargetPin, const TArray<UEdGraphNode*>& InSelectedGraphNodes) const
{
	...
	bool bRelinkHead = SourcePin->Direction == EGPD_Output;
	...
	else
	{
		for (UAnimStateTransitionNode* TransitionNode : TransitionNodes)
		{
			TransitionNode->RelinkTail(NewTargetState);
		}
	}
...
void UAnimationStateMachineSchema::DroppedAssetsOnGraph(const TArray<FAssetData>& Assets, const FVector2f& GraphPosition, UEdGraph* Graph) const
{
	...
	if(Asset != nullptr && GetNodeClassForAsset(Asset->GetClass()))
	{
...
void UAnimationStateMachineSchema::DroppedAssetsOnNode(const TArray<FAssetData>& Assets, const FVector2f& GraphPosition, UEdGraphNode* Node) const
{
	...
	if(Asset != nullptr && StateNodeUnderCursor != nullptr)
	{
		const TSharedPtr<IAssetReferenceFilter> AssetReferenceFilter = MakeAssetReferenceFilter(StateNodeUnderCursor->GetGraph());
		if (!AssetReferenceFilter.IsValid() || AssetReferenceFilter->PassesFilter(FAssetData(Asset)))
		{
...
void UAnimationStateMachineSchema::GetAssetsGraphHoverMessage(const TArray<FAssetData>& Assets, const UEdGraph* HoverGraph, FString& OutTooltipText, bool& OutOkIcon) const
{
	UAnimationAsset* Asset = FAssetData::GetFirstAsset<UAnimationAsset>(Assets);
	if ((Asset == nullptr) || (HoverGraph == nullptr))
	{
	...
	if (GetNodeClassForAsset(Asset->GetClass()) != nullptr)
	{
...
{ 
	UAnimationAsset* Asset = FAssetData::GetFirstAsset<UAnimationAsset>(Assets);
	if ((Asset == nullptr) || (HoverNode == nullptr))
	{
		OutTooltipText.Reset();
		OutOkIcon = false;
		return;
	...
	if (StateNodeUnderCursor != nullptr)
	{
		...
		{
			OutOkIcon = true;
			OutTooltipText = FText::FormatOrdered(LOCTEXT("DragDropAssetMessage", "Change node to play {0}"), FText::FromName(Asset->GetFName())).ToString();
		}
	}
	else
	{
...
TSharedPtr<IAssetReferenceFilter> UAnimationStateMachineSchema::MakeAssetReferenceFilter(const UEdGraph* Graph)
{

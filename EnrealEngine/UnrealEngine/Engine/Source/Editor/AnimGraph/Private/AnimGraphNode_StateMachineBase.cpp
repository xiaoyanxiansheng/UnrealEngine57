...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AnimGraphNode_StateMachineBase)
...
void UAnimGraphNode_StateMachineBase::OnProcessDuringCompilation(IAnimBlueprintCompilationContext& InCompilationContext, IAnimBlueprintGeneratedClassCompiledData& OutCompiledData)
{
	...
	for (auto StateNodeIt = Oven.StateIndexTable.CreateIterator(); StateNodeIt; ++StateNodeIt)
	{
		...
		// Clone any alias transitions. Alias nodes share a single transition graph for all alias states. But we need a clone per alias state to allow
		// for each alias transition graph to have correct hardcoded state index pin values. Note: Alias's are simply decompiled into multiple connections.
		TMap<UAnimStateTransitionNode*, UEdGraph*> AliasTransitionGraphClones;

		for (const int32 AliasIndex : StateAliasIndices)
		{
			// Operate on a separate list to avoid re-processing nodes in the final list.
			TArray<UAnimStateTransitionNode*> AliasTransitionList;

			// Let the final transition list do the sort.
			Oven.StateAliasNodes[AliasIndex]->GetTransitionList(AliasTransitionList, /*bWantSortedList=*/ false);

			for (UAnimStateTransitionNode* AliasTransitionNode : AliasTransitionList)
			{
				if (UEdGraph* AliasTransitionGraph = AliasTransitionNode->BoundGraph)
				{
					UEdGraph* AliasTransitionGraphClone = FEdGraphUtilities::CloneGraph(AliasTransitionGraph, InCompilationContext.GetConsolidatedEventGraph(), &InCompilationContext.GetMessageLog(), true);

					// Note: We do this here instead of in `UAnimBlueprintExtension_StateMachine::AutoWireAnimGetter` since we have the context of correct state index.
					for (UEdGraphNode* Node : AliasTransitionGraphClone->Nodes)
					{
						if (UK2Node_AnimGetter* GetterNode = Cast<UK2Node_AnimGetter>(Node))
						{
							for (UEdGraphPin* Pin : GetterNode->Pins)
							{
								if (Pin->PinName == TEXT("StateIndex"))
								{
									Pin->DefaultValue = FString::FromInt(StateIndex);
								}
							}
						}
					}

					AliasTransitionGraphClones.Add(AliasTransitionNode, MoveTemp(AliasTransitionGraphClone));
				}
			}

			TransitionList.Append(MoveTemp(AliasTransitionList));
		}
		...
		for (auto TransitionIt = TransitionList.CreateIterator(); TransitionIt; ++TransitionIt)
		{
			...
			BakedTransition.MinTimeBeforeReentry = TransitionNode->MinTimeBeforeReentry;
			BakedTransition.LogicType = TransitionNode->LogicType;
			BakedTransition.bAllowInertializationForSelfTransitions = TransitionNode->bAllowInertializationForSelfTransitions;
			...
			if (UEdGraph** AliasTransitionGraphClone = AliasTransitionGraphClones.Find(TransitionNode))
			{
				TransitionGraph = *AliasTransitionGraphClone;
			}

			if (UAnimGraphNode_TransitionResult* TransitionResultNode = CastChecked<UAnimationTransitionGraph>(TransitionGraph)->GetResultNode())
			{
				...
				{
					Rule.CanTakeDelegateIndex = Extension->ExpandGraphAndProcessNodes(TransitionGraph, TransitionResultNode, InCompilationContext, OutCompiledData, TransitionNode);
				...
				{
					if (UAnimationTransitionGraph* TransGraph = Cast<UAnimationTransitionGraph>(TransitionGraph))
					{

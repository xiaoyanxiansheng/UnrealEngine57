...
public:	
	...
	static UE_API const FName ContextId_Automatic;

	// Can safely be executed on worker threads
	static UE_API const FName ContextId_UnBatched_ThreadSafe;

	// Executed batched on the game thread before the event graph is run
	static UE_API const FName ContextId_Batched_WorkerThreadPreEventGraph;

	// Executed batched on the game thread after the event graph is run
	static UE_API const FName ContextId_Batched_WorkerThreadPostEventGraph;

	// Executed batched on the game thread before the event graph is run
	static UE_API const FName ContextId_Batched_GameThreadPreEventGraph;

	// Executed batched on the game thread after the event graph is run
	static UE_API const FName ContextId_Batched_GameThreadPostEventGraph;
...
public:
	...
	UE_API FPropertyAccessHandle AddCopy(TArrayView<FString> InSourcePath, TArrayView<FString> InDestPath, const FName& InContextId, UObject* InObject = nullptr);
	...
	UE_API FPropertyAccessHandle AddAccess(TArrayView<FString> InPath, UObject* InObject = nullptr);
	...
	UE_API FCompiledPropertyAccessHandle GetCompiledHandle(FPropertyAccessHandle InHandle) const;

	// Get the access type for the specified handle
	UE_API EPropertyAccessCopyType GetCompiledHandleAccessType(FPropertyAccessHandle InHandle) const;

	// Expands a property access path to a pure chain of BP nodes
	UE_API void ExpandPropertyAccess(FKismetCompilerContext& InCompilerContext, TArrayView<FString> InSourcePath, UEdGraph* InParentGraph, UEdGraphPin* InTargetPin) const;

	// Maps a compiled handle back to a human-readable context 
	static UE_API FText GetCompiledHandleContext(FCompiledPropertyAccessHandle InHandle);
	
	// Maps a compiled handle back to a human-readable context description
	static UE_API FText GetCompiledHandleContextDesc(FCompiledPropertyAccessHandle InHandle);

	// Whether a context name requires an auto-generated variable to be cached for an access
	static UE_API bool ContextRequiresCachedVariable(FName InName);
	
private:
	// UAnimBlueprintExtension interface
	UE_API virtual void HandleStartCompilingClass(const UClass* InClass, IAnimBlueprintCompilationBracketContext& InCompilationContext, IAnimBlueprintGeneratedClassCompiledData& OutCompiledData) override;
	UE_API virtual void HandleFinishCompilingClass(const UClass* InClass, IAnimBlueprintCompilationBracketContext& InCompilationContext, IAnimBlueprintGeneratedClassCompiledData& OutCompiledData) override;

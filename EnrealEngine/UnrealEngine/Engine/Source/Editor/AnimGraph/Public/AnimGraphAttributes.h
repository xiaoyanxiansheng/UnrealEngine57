...
public:
	...
	UE_API void Register(const FAnimGraphAttributeDesc& InDesc);
	...
	UE_API const FAnimGraphAttributeDesc* FindAttributeDesc(FName InName) const;

	// Iterate over all attributes that are registered
	// @param	InFunction	FUnction receiving the various attribute descriptors
	UE_API void ForEachAttribute(TFunctionRef<void(const FAnimGraphAttributeDesc&)> InFunction) const;

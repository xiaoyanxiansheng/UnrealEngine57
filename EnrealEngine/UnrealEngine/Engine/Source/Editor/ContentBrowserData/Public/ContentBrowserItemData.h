 ...
 */
class FContentBrowserItemData final
{
public:
	/**
	 * Default constructor.
	 ...
	 */
	UE_API FContentBrowserItemData(
	 ...
	 * Comparison support.
	 */
	UE_API bool operator==(const FContentBrowserItemData& InOther) const;
	UE_API bool operator!=(const FContentBrowserItemData& InOther) const;
	 ...
	 */
	UE_API UContentBrowserDataSource* GetOwnerDataSource() const;
	 ...
	 */
	UE_API TSharedPtr<const IContentBrowserItemDataPayload> GetPayload() const;
...
public:
	 ...
	 */
	UE_API explicit FContentBrowserItemDataAttributeValue(const TCHAR* InStr);
	UE_API explicit FContentBrowserItemDataAttributeValue(const FString& InStr);
	UE_API explicit FContentBrowserItemDataAttributeValue(FString&& InStr);

	/**
	 * Construct this attribute value from the given name.
	 */
	UE_API explicit FContentBrowserItemDataAttributeValue(const FName InName);

	/**
	 * Construct this attribute value from the given text.
	 */
	UE_API explicit FContentBrowserItemDataAttributeValue(FText InText);
	 ...
	 */
	UE_API EContentBrowserItemDataAttributeValueType GetValueType() const;
	...
	UE_API void SetValue(const TCHAR* InStr);
	UE_API void SetValue(const FString& InStr);
	UE_API void SetValue(FString&& InStr);
	UE_API void SetValue(const FName InName);
	UE_API void SetValue(FText InText);
	 ...
	 */
	UE_API FStringView GetValueStringView(FStringBuilderBase& ScratchBuffer) const;

	/**
	 * Get the internal string value.
	 * @note Only valid when ValueType == String.
	 */
	UE_API const FString& GetValueString() const;

	/**
	 * Get the internal name value.
	 * @note Only valid when ValueType == Name.
	 */
	UE_API FName GetValueName() const;

	/**
	 * Get the internal text value.
	 * @note Only valid when ValueType == Name.
	 */
	UE_API FText GetValueText() const;

	/**
	 * Get the meta-data associated with this attribute value.
	 */
	UE_API const FContentBrowserItemDataAttributeMetaData& GetMetaData() const;

	/**
	 * Set the meta-data associated with this attribute value.
	 */
	UE_API void SetMetaData(const FContentBrowserItemDataAttributeMetaData& InMetaData);
	UE_API void SetMetaData(FContentBrowserItemDataAttributeMetaData&& InMetaData);
...
public:
	 ...
	 */
	UE_API FContentBrowserItemDataTemporaryContext(FContentBrowserItemData&& InItemData, FOnValidateItem InOnValidateItem, FOnFinalizeItem InOnFinalizeItem);
	 ...
	 */
	UE_API const FContentBrowserItemData& GetItemData() const;
	 ...
	 */
	UE_API FContentBrowserItemData FinalizeItem(const FString& InProposedName, FText* OutErrorMsg = nullptr) const;
...
{
public:
	UE_API explicit FContentBrowserMinimalItemData(const FContentBrowserItemData& InItemData);
	UE_API FContentBrowserMinimalItemData(EContentBrowserItemFlags InItemType, FName InVirtualPath, const UContentBrowserDataSource* InSource);
...
public:
	 ...
	 */
	UE_API explicit FContentBrowserItemDataKey(const FContentBrowserItemData& InItemData);
	
	/**
	 * Construct this key from the given item type and virtual path.
	 */
	UE_API FContentBrowserItemDataKey(EContentBrowserItemFlags InItemType, FName InVirtualPath);
...
public:
	 ...
	 */
	static UE_API FContentBrowserItemDataUpdate MakeItemAddedUpdate(FContentBrowserItemData InItemData);

	/**
	 * Create an item update for when an existing item is updated.
	 */
	static UE_API FContentBrowserItemDataUpdate MakeItemModifiedUpdate(FContentBrowserItemData InItemData);

	/**
	 * Create an item update for when an existing item is moved (or renamed), including its previous virtual path.
	 */
	static UE_API FContentBrowserItemDataUpdate MakeItemMovedUpdate(FContentBrowserItemData InItemData, FName InPreviousVirtualPath);

	/**
	 * Create an item update for when an item is deleted.
	 */
	static UE_API FContentBrowserItemDataUpdate MakeItemRemovedUpdate(FContentBrowserItemData InItemData);

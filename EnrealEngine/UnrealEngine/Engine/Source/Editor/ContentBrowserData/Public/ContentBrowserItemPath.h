...
struct FContentBrowserItemPath
{
	...
	UE_API FContentBrowserItemPath();
	...
	UE_API FContentBrowserItemPath(const FStringView InPath, const EContentBrowserPathType InPathType);
	UE_API FContentBrowserItemPath(const TCHAR* InPath, const EContentBrowserPathType InPathType);
	UE_API FContentBrowserItemPath(const FName InPath, const EContentBrowserPathType InPathType);
	 ...
	 */
	UE_API void SetPathFromString(const FStringView InPath, const EContentBrowserPathType InPathType);

	/**
	 * Set the path being stored
	 */
	UE_API void SetPathFromName(const FName InPath, const EContentBrowserPathType InPathType);

	/**
	 * Returns virtual path as FName (eg, "/All/Plugins/PluginA/MyFile").
	 */
	UE_API FName GetVirtualPathName() const;

	/**
	 * Returns internal path if there is one (eg,. "/PluginA/MyFile").
	 */
	UE_API FName GetInternalPathName() const;

	/**
	 * Returns virtual path as newly allocated FString (eg, "/All/Plugins/PluginA/MyFile").
	 */
	UE_API FString GetVirtualPathString() const;

	/**
	 * Returns internal path as newly allocated FString if there is one or empty FString (eg,. "/PluginA/MyFile").
	 */
	UE_API FString GetInternalPathString() const;

	/**
	 * Returns true if there is an internal path
	 */
	UE_API bool HasInternalPath() const;

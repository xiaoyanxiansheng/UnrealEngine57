...
public:
	 ...
	 * Constructor
	 */
	UE_API FLevelViewportLayout();

	/**
	 * Destructor
	 */
	UE_API virtual ~FLevelViewportLayout();
	 ...
	 */
	UE_API virtual TSharedRef<SWidget> BuildViewportLayout(TSharedPtr<SDockTab> InParentDockTab, TSharedPtr<FEditorViewportTabContent> InParentTab, const FString& LayoutString) override;

	UE_API virtual TSharedRef<SWidget> FactoryViewport(FName InTypeName, const FAssetEditorViewportConstructionArgs& ConstructionArgs) override;
	 ...
	 */
	UE_API void RequestMaximizeViewport( FName ViewportToMaximize, const bool bWantMaximize, const bool bWantImmersive, const bool bAllowAnimation = true );
	 ...
	 */
	UE_API bool IsLevelViewportVisible( FName InViewport ) const;
	...
	*/
	 ...
	 */
	UE_API bool IsViewportMaximized( FName InViewport ) const;
	 ...
	 */
	UE_API bool IsViewportImmersive( FName InViewport ) const;
	...
	UE_API virtual void LoadConfig(const FString& LayoutString) override;
	UE_API virtual void SaveConfig(const FString& LayoutString) const override;
...
protected:
	 ...
	 * @param	bAllowAnimation		True if an animated transition should be used
	 */
	UE_API void MaximizeViewport( FName ViewportToMaximize, const bool bWantMaximize, const bool bWantImmersive, const bool bAllowAnimation );
	 ...
	 */
	UE_API virtual EVisibility OnGetNonMaximizedVisibility() const override;
	 ...
	 */
	UE_API FVector2D GetMaximizedViewportPositionOnCanvas() const;
...
protected:
	...
	UE_API void TakeHighResScreenShot();

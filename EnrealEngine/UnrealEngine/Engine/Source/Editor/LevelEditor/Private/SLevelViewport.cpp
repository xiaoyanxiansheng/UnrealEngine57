...
void SLevelViewport::ConstructViewportOverlayContent()
{
	...
	// Add widget to show viewport warnings from the ActionableMessageSubsystem.
	{
		// clang-format off
		ViewportOverlay->AddSlot(SlotIndex)
		.Padding(FMargin(0.f, 8.f, 10.f, 0.f))
		.VAlign(VAlign_Top)
		.HAlign(HAlign_Right)
		[
			SNew(SBorder)
			...
			[
				...
				.Visibility_Lambda(
					[this]()
					{
						if (GetToolBarVisibility() != EVisibility::Visible)
						{
	...
	auto GetCombinedVisibility = [this]()
	{
		if (GetCurrentScreenPercentageVisibility() == EVisibility::Collapsed &&
			GetCurrentShaderPlatformPreviewTextVisibility() == EVisibility::Collapsed &&
...
bool SLevelViewport::HandleDragObjects(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent)
{
	...
	else if (Operation->IsOfType<FExternalDragOperation>() && StaticCastSharedPtr<FExternalDragOperation>(Operation)->HasText())
	{
		bValidDrag = true;
	}
	else
...
FName SLevelViewport::RegisterViewportToolbar() const
{
	...
	if (!UToolMenus::Get()->IsMenuRegistered(ViewportToolbarName))
	{
		...
		// Sections added for compatibility with old toolbar transform menu extensions
		{
			...
			// TODO: Remove these once the deprecation period is complete
...
TSharedPtr<SWidget> SLevelViewport::BuildViewportToolbar()
{
	FToolMenuContext ViewportToolbarContext;
	{
		...
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		LevelViewportToolBar = SNew(SLevelViewportToolBar);
	...
	if (!UToolMenus::Get()->IsMenuRegistered(ToolbarName))
	{
		...
		
		{
			...
			LeftSection.AddEntry(FToolMenuEntry::InitDynamicEntry(
				...
				{
					TSharedPtr<SLevelViewport> Viewport = ULevelViewportContext::GetLevelViewport(Section);
					if (!Viewport)
					{
						return;
					}
					...
					Section.AddEntry(FToolMenuEntry::InitWidget(
						...
						.Visibility_Lambda([WeakViewport = Viewport.ToWeakPtr()]
						{
...
void SLevelViewport::OnElementSelectionChanged(const UTypedElementSelectionSet* SelectionSet, bool bForceRefresh)
{
	...
	DepthBar->ClearDepthIndicators();
	FTypedElementListConstRef ElementList = SelectionSet->GetElementList();
	for (int32 Index = 0; Index < ElementList->Num(); ++Index)
	{
		FTypedElementHandle Handle = ElementList->GetElementHandleAt(Index);
		
		if (TTypedElement<ITypedElementWorldInterface> WorldElement = ElementList->GetElement<ITypedElementWorldInterface>(Handle))
		{
			TAttribute<FBoxSphereBounds> Bounds = TAttribute<FBoxSphereBounds>::CreateLambda([SelectionSet, Handle]
			{
				FBoxSphereBounds Bounds(ForceInit);
				if (TTypedElement<ITypedElementWorldInterface> WorldElement = SelectionSet->GetElementList()->GetElement<ITypedElementWorldInterface>(Handle))
				{
					WorldElement.GetBounds(Bounds);
				}
			...
			if (TTypedElement<ITypedElementObjectInterface> ObjectElement = ElementList->GetElement<ITypedElementObjectInterface>(Handle))
			{
				if (AActor* Actor = ObjectElement.GetObjectAs<AActor>())
				{
					DisplayName.BindWeakLambda(Actor, [Actor]
					{
			...
			DepthBar->AddDepthIndicatorSlot()
			.Bounds(Bounds)
			.HAlign(HAlign_Center)
			[
				SNew(SImage)
				.Image(FAppStyle::Get().GetBrush("EditorViewport.DepthBar.SelectionHighlight"))
				.ToolTipText(DisplayName)
				.DesiredSizeOverride(FVector2d(8, 4))
			];
		}
	}
}

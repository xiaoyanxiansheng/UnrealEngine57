...
#define LOCTEXT_NAMESPACE "LevelEditorToolBar"
		...
		FText FriendlyNamePreviewOff = FDataDrivenShaderPlatformInfo::GetFriendlyName(GMaxRHIShaderPlatform);
		FText FriendlyNamePreviewOn = GEditor->PreviewPlatform.GetFriendlyName();
		{
			const FText& RenderingAsPlatformName = GEditor->PreviewPlatform.bPreviewFeatureLevelActive ? FriendlyNamePreviewOn : FriendlyNamePreviewOff;
			const FText& SwitchToPlatformName = GEditor->PreviewPlatform.bPreviewFeatureLevelActive ? FriendlyNamePreviewOff : FriendlyNamePreviewOn;
			if (PreviewShaderPlatform == GMaxRHIShaderPlatform)
			{
				return FText::Format(LOCTEXT("PreviewModeViewingAs", "Viewing {0}."), RenderingAsPlatformName);
			}
			else if (GEditor->IsFeatureLevelPreviewActive())
			{

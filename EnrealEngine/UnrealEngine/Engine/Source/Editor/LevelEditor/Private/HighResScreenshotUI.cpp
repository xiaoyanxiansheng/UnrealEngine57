...
FReply SHighResScreenshotDialog::OnCaptureClicked()
{
	if (!GIsHighResScreenshot)
	{
		...
		if (ConfigViewport.IsValid())
		{
			GScreenshotResolutionX = uint32(static_cast<float>(ConfigViewport->GetRenderTargetTextureSizeXY().X) * Config.ResolutionMultiplier);
			GScreenshotResolutionY = uint32(static_cast<float>(ConfigViewport->GetRenderTargetTextureSizeXY().Y) * Config.ResolutionMultiplier);
			...
			if (ScaledCaptureRegion.Area() > 0)
			{
				...
				ScaledCaptureRegion *= FMath::TruncToInt32(Config.ResolutionMultiplier);
			}

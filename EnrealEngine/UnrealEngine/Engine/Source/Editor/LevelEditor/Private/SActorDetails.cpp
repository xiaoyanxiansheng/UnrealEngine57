...
void SActorDetails::Construct(const FArguments& InArgs, UTypedElementSelectionSet* InSelectionSet, const FName TabIdentifier, TSharedPtr<FUICommandList> InCommandList, TSharedPtr<FTabManager> InTabManager)
{
	...
	SubobjectEditor = SNew(SSubobjectInstanceEditor)
		...
		.SCSEditorUICustomization(InArgs._SCSEditorUICustomization);
...
void SActorDetails::RefreshSubobjectTreeElements(TArrayView<const FSubobjectEditorTreeNodePtrType> InSelectedNodes, const bool bForceRefresh, const bool bOverrideLock)
{
	...
	TArray<FTypedElementHandle> Components;
				...
				if(const FSubobjectData* Data = SelectedNode->GetDataSource())
				{
					...
					if (Data->IsComponent())
					{
						if (const UActorComponent* Component = Data->FindComponentInstanceInActor(Actor))
						{
							...
							{
								Components.Add(ComponentElementHandle);
							}
						}
					}
				}
	...
	else
	{
		...
		const ICoreProvider* CoreProvider = GetDataStorageFeature<ICoreProvider>(StorageFeatureName);
		for (const FSubobjectEditorTreeNodePtrType& SelectedNode : InSelectedNodes)
		{
			if (SelectedNode)
			{
				if(const FSubobjectData* Data = SelectedNode->GetDataSource())
				{
					if(Data->IsRootActor()) // Not an actor, using IsRootActor to mean IsRootNode
					{
						// If the root node is selected then we ignore the component selection
						Components.Reset();
						break;
					}
					...
					if (Data->IsComponent())
					{
						const FSubobjectDataSubsystemContextData_TedsRow* TedsRowAdapter = Data->GetContextData().GetPtr<FSubobjectDataSubsystemContextData_TedsRow>();
						if (TedsRowAdapter)
						{
							UE::Editor::DataStorage::RowHandle ComponentRowHandle = TedsRowAdapter->RowHandle;
							if (const FTedsTypedElementColumn* TypedElementHandleColumn = CoreProvider->GetColumn<FTedsTypedElementColumn>(ComponentRowHandle))
							{
								FTypedElementHandle ComponentElementHandle = TypedElementHandleColumn->Handle;
								Components.Add(TypedElementHandleColumn->Handle);
							}
						}
					}
				}
	...
	{
		UTypedElementRegistry* Registry = UTypedElementRegistry::GetInstance();
		for (const FTypedElementHandle& ComponentElementHandle : Components)
		{
...
void SActorDetails::OnSubobjectEditorTreeViewSelectionChanged(const TArray<FSubobjectEditorTreeNodePtrType>& SelectedNodes)
{
	...
	if (AActor* ActorContext = GetActorContext())
	{
	...
	else if (UObject* ObjectContext = GetObjectContext())
	{
		RefreshSubobjectTreeElements(SelectedNodes, /*bForceRefresh*/false, DetailsView->IsLocked());
	}
}
...
EVisibility SActorDetails::GetComponentEditorVisibility() const
{
	...
	return GetObjectContext() && !bHideEditorFromDetailsView  ? EVisibility::Visible : EVisibility::Collapsed;
}

EVisibility SActorDetails::GetComponentEditorButtonsVisibility() const
{
	return GetObjectContext() ? EVisibility::Visible : EVisibility::Collapsed;
}

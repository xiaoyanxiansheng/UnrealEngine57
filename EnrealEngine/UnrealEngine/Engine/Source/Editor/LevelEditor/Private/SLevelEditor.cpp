	...
	, CachedViewportContextMenuTitle(FLevelEditorContextMenu::GetContextMenuTitle(ELevelEditorMenuContext::MainMenu, nullptr))
{
	...
	// This is used while working with both legacy and ITF interactions. Once ITF is shipped as default, it shouldn't be needed anymore
	UE::Editor::ViewportInteractions::OnEditorViewportInteractionsActivated().AddRaw(
		this, &SLevelEditor::RegisterViewportInteractions
	);

	UEditorInteractiveGizmoManager::OnUsesNewTRSGizmosChangedDelegate().AddLambda([this](bool bInUsesExperimentalGizmos)
	{
		RegisterViewportInteractions();
	});
}
...
SLevelEditor::~SLevelEditor()
{
	...
	// This will be likely removed once ITF interactions become the default, and legacy ones are removed/deactivated
...
void SLevelEditor::RegisterViewportInteractions() const
{
	if (UModeManagerInteractiveToolsContext* const ModeManagerInteractiveToolsContext =
			SLevelEditor::GetEditorModeManager().GetInteractiveToolsContext())
	{
		if (UViewportInteractionsBehaviorSource* InputToolsSource =
				ModeManagerInteractiveToolsContext->GetViewportInteractionsBehaviorSource())
		{
			InputToolsSource->Reset();

			UE::Editor::ViewportInteractions::AddDefaultCameraMovementInteractions(InputToolsSource);

			// make sure to register all BehaviorSources
			InputToolsSource->RegisterBehaviorSources();
		}
	}
}
...
TSharedRef<SWidget> SLevelEditor::RestoreContentArea( const TSharedRef<SDockTab>& OwnerTab, const TSharedRef<SWindow>& OwnerWindow )
{
	...
	TSharedRef<SWidget> LevelEditorContent = SNew(SVerticalBox)
		+ SVerticalBox::Slot()
	...
	if (TSharedPtr<SWidget> PanelDrawer = LevelEditorTabManager->RestorePanelDrawer(LevelEditorContent, OwnerWindow))
	{
		LevelEditorContent = PanelDrawer.ToSharedRef();
	}

	return LevelEditorContent;
}
...
TSharedRef<SWidget> SLevelEditor::CreateActorDetails( const FName TabIdentifier )
{
	TSharedRef<SActorDetails> ActorDetails = SNew( SActorDetails, GetMutableElementSelectionSet(), TabIdentifier, LevelEditorCommands, GetTabManager())
		.SCSEditorUICustomization(ActorDetailsSCSEditorUICustomization);

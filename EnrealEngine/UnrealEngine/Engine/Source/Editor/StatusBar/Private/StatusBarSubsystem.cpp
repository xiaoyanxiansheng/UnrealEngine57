...
#include UE_INLINE_GENERATED_CPP_BY_NAME(StatusBarSubsystem)
...
namespace UE
{
	namespace StatusBarSubsystem
	{
		namespace Private
		{
			...
			TArray<UStatusBarSubsystem::FTabIdAndButtonLabel> CreatePanelDrawerSummonsData(const TSharedRef<SDockTab>& InParentDockTab, UStatusBarSubsystem::FRegisterPanelDrawerSummonDelegate& InPanelDrawerSummonsDelegate)
			{
				// Not great but that will do it as quick toggle to turn off the feature in 5.7 if needed
				if (TConsoleVariableData<bool>* EnablePanelDrawer = IConsoleManager::Get().FindTConsoleVariableDataBool(TEXT("EnablePanelDrawer")))
				{
					if (!EnablePanelDrawer->GetValueOnGameThread())
					{
						return {};
					}
				}

				TSharedPtr<FTabManager> TabManager = InParentDockTab->GetTabManagerPtr();
				if (InParentDockTab->GetTabRole() == ETabRole::MajorTab)
				{
					TabManager = FGlobalTabmanager::Get()->GetTabManagerForMajorTab(InParentDockTab);
				}

				if (!TabManager || !TabManager->HasPanelDrawer(InParentDockTab->GetParentWindow()))
				{
					return {};
				}

				TArray<UStatusBarSubsystem::FTabIdAndButtonLabel> PanelDrawerSummons;
				InPanelDrawerSummonsDelegate.Broadcast(PanelDrawerSummons, InParentDockTab);
				return PanelDrawerSummons;
			}
		}
	}
}
...
TSharedRef<SWidget> UStatusBarSubsystem::MakeStatusBarWidget(FName StatusBarName, const TSharedRef<SDockTab>& InParentTab, UE::Editor::Toolbars::ECreateStatusBarOptions CreateStatusBarOptions)
{
	...
	TArray<UStatusBarSubsystem::FTabIdAndButtonLabel> PanelDrawerSummons = UE::StatusBarSubsystem::Private::CreatePanelDrawerSummonsData(InParentTab, RegisteredPanelDrawerSummons);

	TSharedRef<SStatusBar> StatusBar =
		SNew(SStatusBar, StatusBarName, InParentTab, MoveTemp(PanelDrawerSummons))
...
FDelegateHandle UStatusBarSubsystem::RegisterPanelDrawerSummon(FRegisterPanelDrawerSummonDelegate::FDelegate&& InRegistrationCallback)
{
	return RegisteredPanelDrawerSummons.Add(MoveTemp(InRegistrationCallback));
}
void UStatusBarSubsystem::UnregisterPanelDrawerSummon(FDelegateHandle InHandle)
{
	RegisteredPanelDrawerSummons.Remove(InHandle);
}

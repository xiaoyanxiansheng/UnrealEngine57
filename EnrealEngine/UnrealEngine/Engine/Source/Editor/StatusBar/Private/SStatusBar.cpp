...
TUniquePtr<FSlateRoundedBoxBrush> SStatusBar::SummonPanelDrawerRoundedBox;
...
void SStatusBar::Construct(const FArguments& InArgs, FName InStatusBarName, const TSharedRef<SDockTab> InParentTab, TArray<UStatusBarSubsystem::FTabIdAndButtonLabel>&& PanelDrawerSummons)
{
	...
	TSharedPtr<SHorizontalBox> MainContentBox;
	TSharedPtr<SWidget> SummonPanelDrawerWidget = MakeSummonPanelDrawer(MoveTemp(PanelDrawerSummons));
	...
	[
		...
		[	
			...
			+SHorizontalBox::Slot()
			[
				SAssignNew(MainContentBox, SHorizontalBox)
				+SHorizontalBox::Slot()
				.FillWidth(1.0f)
				.Padding(1.0f, 0.0f)
				[
	...
	if (SummonPanelDrawerWidget)
	{
		MainContentBox->AddSlot()
			...
			.AutoWidth()
			[
				SummonPanelDrawerWidget.ToSharedRef()
			];
	}
}
...
TSharedPtr<SWidget> SStatusBar::MakeSummonPanelDrawer(TArray<UStatusBarSubsystem::FTabIdAndButtonLabel>&& InTabIdAndLabels)
{
	if (InTabIdAndLabels.IsEmpty())
	{
		return {};
	}
	TSharedPtr<SDockTab> ParentTabPtr = ParentTab.Pin();
	TSharedPtr<FTabManager> TabManager = FGlobalTabmanager::Get()->GetTabManagerForMajorTab(ParentTabPtr);

	if (!TabManager || !TabManager->HasPanelDrawer(ParentTabPtr->GetParentWindow()))
	{
		return {};
	}

	TabManager->RegisterOnPanelDrawerStateChanges(
		FTabManager::FOnPanelDrawerStateChanged::FDelegate::CreateSP(this, &SStatusBar::UpdateSummonPanelDrawerArrowRotation)
		);


	// Todo add its own style when more mature and less a proof of concept
	const float StatusBarHalfHeight = FAppStyle::Get().GetFloat("StatusBar.Height") / 2;
	if (!SummonPanelDrawerRoundedBox)
	{
		FVector4 Radius(StatusBarHalfHeight, 0.f, 0.f, StatusBarHalfHeight);
		SummonPanelDrawerRoundedBox = MakeUnique<FSlateRoundedBoxBrush>(FStyleColors::Panel, Radius);
	}

	const bool bOneEntry = InTabIdAndLabels.Num() == 1;
	...
	if (bOneEntry)
	{
		HorizontalBox= SNew(SHorizontalBox)
			...
			[
				SNew(SBorder)
				.BorderImage(SummonPanelDrawerRoundedBox.Get())
				.VAlign(VAlign_Center)
				[
					SNew(SButton)
					.ButtonStyle(&FAppStyle::Get().GetWidgetStyle<FButtonStyle>("StatusBar.StatusBarButton"))
					.OnClicked(this, &SStatusBar::ToggleTabInPanelDrawer, InTabIdAndLabels[0].Key)
					[
						SNew(SHorizontalBox)
						+ SHorizontalBox::Slot()
						.Padding(FMargin(0.0f, 2.f))
						.VAlign(VAlign_Center)
						.AutoWidth()
						[
							SAssignNew(PanelDrawerArrow, SImage)
								.ColorAndOpacity(FSlateColor::UseForeground())
	...
	else
	{
		...
		HorizontalBox = SNew(SHorizontalBox)
			...
			[
				...
				[
					SNew(SButton)
					.ButtonStyle(&FAppStyle::Get().GetWidgetStyle<FButtonStyle>("StatusBar.StatusBarButton"))
					.OnClicked(this, &SStatusBar::ClosePanelDrawer)
					[
		...
		for (int32 Index = 0; Index < InTabIdAndLabels.Num(); ++Index)
		{
			HorizontalBox->AddSlot()
				...
				[
					...
					[
						SNew(SButton)
						.ButtonStyle(&FAppStyle::Get().GetWidgetStyle<FButtonStyle>("StatusBar.StatusBarButton"))
						.OnClicked(this, &SStatusBar::ToggleTabInPanelDrawer, InTabIdAndLabels[Index].Key)
						[
							SNew(STextBlock)
							.TextStyle(&FAppStyle::Get().GetWidgetStyle<FTextBlockStyle>("NormalText"))
							.Text(InTabIdAndLabels[Index].Value)
						]
					]
				];
		}
	...
	UpdateSummonPanelDrawerArrowRotation(*TabManager.Get());
...
void SStatusBar::UpdateSummonPanelDrawerArrowRotation(const FTabManager& TabManager)
{
	// Todo add its own style when more mature and less a proof of concept
	TSharedPtr<SDockTab> ParentTabPtr = ParentTab.Pin();
	if (TabManager.IsPanelDrawerOpen(ParentTabPtr->GetParentWindow()))
	{
		PanelDrawerArrow->SetImage(FAppStyle::Get().GetBrush(TEXT("Icons.ChevronRight")));
	}
	else
	{
		PanelDrawerArrow->SetImage(FAppStyle::Get().GetBrush(TEXT("Icons.ChevronLeft")));
	}
}

FReply SStatusBar::ToggleTabInPanelDrawer(FName TabId)
{
	TSharedPtr<SDockTab> ParentTabPtr = ParentTab.Pin();
	if (TSharedPtr<FTabManager> TabManager = FGlobalTabmanager::Get()->GetTabManagerForMajorTab(ParentTabPtr))
	{
		constexpr bool bForceInPanelDrawer = true;
		TabManager->TryToggleTabInPanelDrawer(TabId, ParentTabPtr->GetParentWindow(), bForceInPanelDrawer);
	}

	return FReply::Handled();
}

FReply SStatusBar::ClosePanelDrawer()
{
	TSharedPtr<SDockTab> ParentTabPtr = ParentTab.Pin();
	if (TSharedPtr<FTabManager> TabManager = FGlobalTabmanager::Get()->GetTabManagerForMajorTab(ParentTabPtr))
	{
		TabManager->ClosePanelDrawer(ParentTabPtr->GetParentWindow());
	}

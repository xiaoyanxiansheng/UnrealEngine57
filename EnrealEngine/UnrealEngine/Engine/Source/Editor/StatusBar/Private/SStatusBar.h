...
public:
	...
	void Construct(const FArguments& InArgs, FName InStatusBarName, const TSharedRef<SDockTab> InParentTab, TArray<UStatusBarSubsystem::FTabIdAndButtonLabel>&& PanelDrawerSummons);
...
private:
	...
	TSharedPtr<SWidget> MakeSummonPanelDrawer(TArray<UStatusBarSubsystem::FTabIdAndButtonLabel>&& InTabIdAndLabels);
	...
	void UpdateSummonPanelDrawerArrowRotation(const FTabManager& TabManager);
	FReply ToggleTabInPanelDrawer(FName TabId);
	FReply ClosePanelDrawer();
...
private:
	...
	TSharedPtr<SImage> PanelDrawerArrow;
	static TUniquePtr<FSlateRoundedBoxBrush> SummonPanelDrawerRoundedBox;

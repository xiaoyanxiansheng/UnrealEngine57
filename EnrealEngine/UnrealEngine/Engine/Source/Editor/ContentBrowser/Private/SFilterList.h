...
public:
	...
	/** Deletes the custom text filter with the matching label */
	void DeleteCustomTextFilterByLabel(const FText& FilterLabel);

	/** Modifies the custom text filter with the matching label */
	void ModifyCustomTextFilterByLabel(const FCustomTextFilterData& NewFilterData, const FText& FilterLabel);

	/** Whether the filters are being applied to paths recursively */
	bool IsFilteringPathsRecursively() const;

	/** Sets whether the filters are being applied to paths recursively */
	void SetFilterPathsRecursively(const bool bInFilterRecursively);

	/** Toggle whether the filters are being applied to paths recursively */
	void ToggleFilterPathsRecursively();

	/** Returns a pointer to the active filter container widget (Horizontal or Vertical), nullptr if neither is active */
...
protected:
	...
	virtual void SaveSettingsInternal(FFilterBarSettings* InSettingsToSave) override;
...
private:
	...
	virtual void PopulateAddFilterMenu(UToolMenu* Menu, TSharedPtr<FFilterCategory> MenuExpansion, FOnFilterAssetType OnFilterAssetType) override;
...
private:
	...
	/** Whether this FilterList applies to paths recursively */
	bool bFilterPathsRecursively = true;

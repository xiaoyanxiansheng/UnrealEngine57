		...
		// Common operations section //
		{
...

			{
				...
				if(SelectedFolders.Num() == 1)
				{
					...
					{
						NewAssetToolTip = FText::Format(LOCTEXT("NewAssetTooltip_CreateIn", "Create a new item in {0}."), IContentBrowserDataModule::Get().GetSubsystem()->ConvertVirtualPathToDisplay(SelectedFolders[0]));
					}
				}
				else
				{
			...
			if (!Context->GetSelectedPackagePaths().IsEmpty())
			{
				Section.AddMenuEntry(
					"CopyPath",

...
{
	// First see if this item has a custom implementation
	if (TSharedPtr<FTreeItem> TreeItemPin = TreeItem.Pin())
	{
		FContentBrowserItemDataAttributeValue FolderAttributeValue = TreeItemPin->GetItem().GetItemAttribute(IsItemExpanded.Get() ? ContentBrowserItemAttributes::ItemAssetTreeFolderOpenBrush : ContentBrowserItemAttributes::ItemAssetTreeFolderClosedBrush);
		if (FolderAttributeValue.IsValid())
		{
			FName BrushName = FolderAttributeValue.GetValue<FName>();

			const FSlateStyleSet& ContentBrowserStyle = UE::ContentBrowser::Private::FContentBrowserStyle::Get();
			return ContentBrowserStyle.GetBrush(BrushName);
		}
	}

	// If we made it here, nothing overrode it, so use the default brushes.
...
FText SAssetTreeItem::GetToolTipText() const
{
	if (TSharedPtr<FTreeItem> TreeItemPin = TreeItem.Pin())
	{
		...
		FText PathText = IContentBrowserDataModule::Get().GetSubsystem()->ConvertVirtualPathToDisplay(Item);
		if (Item.IsInPlugin())
		{

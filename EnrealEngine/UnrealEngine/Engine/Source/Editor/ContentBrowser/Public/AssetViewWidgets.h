...
public:
	...
	static TSharedRef<SWidget> CreateTileItemContents(SAssetTileItem* const InTileItem, const TSharedRef<SWidget>& InThumbnail, FName& OutItemShadowBorder, const FMargin& InFolderOverlayPadding = FMargin(0.f), bool bShowAssetAccessSpecifier = false);

private:
	template <typename T>
	static TSharedRef<SWidget> CreateListTileItemContents(T* const InTileOrListItem, const TSharedRef<SWidget>& InThumbnail, FName& OutItemShadowBorder, const FMargin& InFolderOverlayPadding, bool bShowAssetAccessSpecifier);
};
...
public:
	...
	SLATE_ARGUMENT(TSharedPtr<FAssetViewItem>, AssetItem)
		...
		/** If true, display the item's EAssetAccessSpecifier */
...
protected:
	...
	/** Returns the SBox that will hold the AssetAccessSpecifierIcon */
	TSharedRef<SWidget> GetAssetAccessSpecifierIconBox() const;

	/** Renders the appropriate AssetAccessSpecifierIcon based on the EAssetAccessSpecifier of the asset */
	void UpdateAssetAccessSpecifierIcon();

	/** Gets the AssetAccessSpecifier as FText. */
	FText GetAssetAccessSpecifierText() const;
...
protected:
	...
	/** Whether to show the icon / text for the item's EAssetAccessSpecifier */
	bool bShowAssetAccessSpecifier = false;

	/** Widget holding the EAssetAccessSpecifier as an icon */
	TSharedPtr<SBox> AssetAccessSpecifierIconBox;

	/** Widget rendering the EAssetAccessSpecifier as text */
	TSharedPtr<STextBlock> AssetAccessSpecifierText;
};

...
public:
	...
	/** Delegates to call to provide the value for a ContentBrowserItemData's attribute */
	PRAGMA_DISABLE_EXPERIMENTAL_WARNINGS
	UE_EXPERIMENTAL(5.7, "GetAllItemDataAttributeDelegates() is experimental while a newer system is in active development.")
	virtual TArray<FGetItemDataAttribute>& GetAllItemDataAttributeDelegates() { return ItemDataAttributeDelegates; }
	...
	struct FWizard
	{
		FName WizardIdentifier;
		FText DisplayName;
		FText Description;
		FSlateIcon Icon;
		FAdvancedAssetCategory CategoryPath;
		FSimpleDelegate OnOpen;
	};

	CONTENTBROWSER_API void RegisterWizard(const FWizard& InWizard);
	CONTENTBROWSER_API void UnregisterWizard(const FName WizardIdentifier);
	CONTENTBROWSER_API const TArray<FWizard>& GetWizards() const;

private:
	TArray<FWizard> Wizards;
...
private:
	...
	/** All delegates that return values for ContentItemData attributes */
	PRAGMA_DISABLE_EXPERIMENTAL_WARNINGS
	TArray<FGetItemDataAttribute> ItemDataAttributeDelegates;
	PRAGMA_ENABLE_EXPERIMENTAL_WARNINGS

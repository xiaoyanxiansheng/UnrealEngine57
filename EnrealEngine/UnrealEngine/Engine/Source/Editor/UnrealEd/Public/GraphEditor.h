...
DECLARE_DELEGATE_ThreeParams(FOnGraphEditorTickDelegate, const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime);
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	SLATE_BEGIN_ARGS(SGraphEditor)
		...
		SLATE_EVENT(FOnGraphEditorTickDelegate, OnTick);
	...
	/** Sets the on tick delegate for the graph editor */
	virtual void SetOnTick(FOnGraphEditorTickDelegate InOnTickDelegate)
	{
		if (Implementation.IsValid())
		{
			Implementation->SetOnTick(InOnTickDelegate);
		}
	}

...
public:
	...
	virtual EAssetOpenMethod GetOpenMethod() const override { return OpenMethod; }
	...
	virtual TSharedPtr<SWidget> CreateMenuBar(const TSharedPtr<FTabManager>& InTabManager, const FName InMenuName, FToolMenuContext& InToolMenuContext) { return nullptr; }
	...
	UNREALED_API virtual FReply OnViewportDrop(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent);
...
protected:
	...
	virtual void SetupReadOnlyMenuProfiles(FReadOnlyAssetEditorCustomization& OutReadOnlyCustomization) { }

	/** Get the name of the profile registered with UToolMenus for read only menu customizations */
	virtual FName GetReadOnlyMenuProfileName() { return ReadOnlyMenuProfileName; }

...
public:
	...
	// Function to be hooked up to the Cancel hotkey (Escape) for the dialog window
	void OnCancelHotkeyPressed();

	// Function to be hooked up to the Confirm hotkey (Enter) for the dialog window
	void OnConfirmHotkeyPressed();

	// Function to get the appropriate widget to focus when the parent window is activated
...
private:
	...
	TSharedPtr<SWidget> ForceDeleteButton;
	TSharedPtr<SWidget> DeleteButton;

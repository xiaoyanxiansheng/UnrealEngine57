...
public:
	...
	UNREALED_API void PushTabFactories(FWorkflowAllowedTabSet& FactorySetToPush) const;
...
protected:
	...
	// Remove an application mode.
	// @return true if the mode was successfully removed.
	// @note Has no effect if you call this function with the current mode.
	UNREALED_API virtual bool RemoveApplicationMode(FName ModeName);

	// Remove all application modes that were added through AddApplicationMode.
	UNREALED_API virtual void RemoveAllApplicationModes();

	// Get the list of registered application modes. Use AddApplicationMode to add modes.
	UNREALED_API virtual const TMap< FName, TSharedPtr<FApplicationMode> >& GetApplicationModeList() const;
protected:

...
protected:
	...
	/**
	 * Helper function to warn user that we are about to search for a reference, which can incur an expensive
	 * indexing operation
	 *
	 * @param Reason		The reason the search would be requested, logged if search is suppressed
	 * @param SearchString	The search string that will be used if the user wants to search, logged if search is suppressed
	 * @return				TRUE if the user wants to perform the search operation
	 */
	static UNREALED_API bool VerifyUserWantsToSearch(FText Reason, const FString& SearchString);
...
public:
	...
	/**
	 * Returns true if this BP needs to show world context pins, because it does not implement GetWorld or has ShowWorldContextPin set
	 */
	static UNREALED_API bool ShouldShowWorldContextPin(const UBlueprint* BP);

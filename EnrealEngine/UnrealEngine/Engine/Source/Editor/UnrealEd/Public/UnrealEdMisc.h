...
private:
	/** Delegates to support multi-select of message tokens.  Selections from calls to OnMessageTokenActivated or OnActorTokenActivated are batched and selected as a group. */
	UNREALED_API void OnMultiSelectActorFromMessageTokenBegin();
	UNREALED_API void OnMultiSelectActorFromMessageTokenEnd();
	...
	bool bActorMultiSelect = false;
	TArray<AActor*> ActorsSelected;
	TArray<FGuid> UnloadedActorsSelected;
};

...
public:
	...
	/** Check to see if an actor selection is exclusively disallowed by a mode -- no side effects */
	virtual bool IsSelectionDisallowed(AActor* InActor, bool bInSelection) const { return false; }

	/** Check to see if an actor edit is exclusively disallowed by a mode */
	virtual bool IsEditingDisallowed(AActor* InActor) const { return false; }

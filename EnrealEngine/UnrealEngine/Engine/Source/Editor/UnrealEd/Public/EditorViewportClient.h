...
UE_DECLARE_TCOMMANDS(class FViewportNavigationCommands, UNREALED_API)
...
{
public:
	UNREALED_API FViewportCursorLocation( const FSceneView* View, FEditorViewportClient* InViewportClient, int32 X, int32 Y, const bool bInClampToBounds = true );
...
public:
	...
	/** @return The forward vector of the camera view */
	UNREALED_API FVector GetForwardVector() const;
	...
	UNREALED_API virtual TOptional<TSharedRef<SWidget>> MapCursor(FViewport* Viewport, const FCursorReply& CursorReply) override;
...
public:
	...
	/**
	 * Override to specify what should happen when a context menu is requested
	 * @param HitProxyElement the clicked element
	 */
	UNREALED_API virtual void SummonContextMenu(const FTypedElementHandle& HitProxyElement = FTypedElementHandle()) {}
	...
	*/
	...
	virtual bool DropObjectsAtCoordinates(int32 MouseX, int32 MouseY, const TArray<UObject*>& DroppedObjects, TArray<AActor*>& OutNewActors,
...
public:
	...
	/**
	 * Sets the viewport type to Geometry Inspection Zebra mode, horizontal or vertical type
	 * 
	 * @param	bHorizontalZebraMode		Controls if this is horizontal or vertical stripe type
	 */
	UNREALED_API virtual void SetZebraViewMode(bool bHorizontalZebraMode);
	
	/**
	 * Sets Geometry Inspection Zebra mode stripe count
	 * 
	 * @param	InStripeCount				Number of stripes Zebra mode should have
	 */
	UNREALED_API virtual void SetZebraViewModeStripeCount(uint8 InStripeCount);
	...
	/** @return True if ViewMode is Geometry Inspection Zebra in specified orientation */
	UNREALED_API bool IsZebraViewModeEnabled(bool bHorizontal) const;

	/** @return Configured number of stripes in Geometry Inspection Zebra view mode */
	UNREALED_API static uint8 GetZebraViewModeStripeCount();
	...
	*/
	...
	/**
	 * Gets whether focusing on a box should clip to selection.
	 */
	bool GetClipToSelectionOnFocus() const { return bClipToSelectionOnFocus; }

	/**
	 * Sets whether focusing on a box should clip to selection.
	 */
	void SetClipToSelectionOnFocus(bool bInClipToSelection);
	...
	/** Gets the settings that control camera speed for this viewport */
	UNREALED_API virtual const FEditorViewportCameraSpeedSettings& GetCameraSpeedSettings() const;
	
	/** Sets the camera speed settings for this viewport */
	UNREALED_API virtual void SetCameraSpeedSettings(const FEditorViewportCameraSpeedSettings& InCameraSpeedSettings);
	
	/** Get the camera speed for this viewport based on the specified speed setting */
	UE_DEPRECATED(5.7, "Integer-based camera speed settings are deprecated.")
	UNREALED_API float GetCameraSpeed(int32 SpeedSetting) const;

	/** Set the speed setting for the camera in this viewport */
	UE_DEPRECATED(5.7, "Integer-based camera speed settings are deprecated.")
	UNREALED_API virtual void SetCameraSpeedSetting(int32 SpeedSetting) final;

	/** Get the camera speed setting for this viewport */
	UE_DEPRECATED(5.7, "Integer-based camera speed settings are deprecated.")
	UNREALED_API virtual int32 GetCameraSpeedSetting() const final;

	/** Get the camera speed scalar for this viewport */
	UE_DEPRECATED(5.7, "Please use GetCameraSpeed() directly")
	UNREALED_API virtual float GetCameraSpeedScalar() const final;

	UE_DEPRECATED(5.7, "Please use SetCameraSpeedSettings()")
	/** Set the camera speed scalar for this viewport */
	UNREALED_API virtual void SetCameraSpeedScalar(float SpeedScalar) final;
...
protected:
	...
	/** The default camera speed settings for the viewport */
	FEditorViewportCameraSpeedSettings CameraSpeedSettings;

	/** Camera speed setting */
	UE_DEPRECATED(5.7, "Use the new GetCameraSpeed() and SetCameraSpeed() functions.")
...
public:
	...
	/** Accessor for whether orthographic control should be enabled. */
	bool SupportsManualOrthographicDepthControl() const; 

	/** Get the orthographic near plane override for this viewport. */
	UNREALED_API TOptional<double> GetOrthographicNearPlaneOverride() const;
	
	/** Set the orthographic near plane override for this viewport. */
	UNREALED_API void SetOrthographicNearPlaneOverride(const TOptional<double>& InValue);
	
	/** Get the orthographic far plane override for this viewport. */
	UNREALED_API TOptional<double> GetOrthographicFarPlaneOverride() const;
	
	/** Set the orthographic far plane override for this viewport. */
	UNREALED_API void SetOrthographicFarPlaneOverride(const TOptional<double>& InValue);
	
	/** Gets the bounds of the world being represented. */
	UNREALED_API FBox GetWorldBounds() const;
	...
	*/
	...
	FViewportClientNavigationHelper* GetViewportNavigationHelper() const
	{
		return ViewportNavigationHelper;
	}
...
public:
	UE_DEPRECATED(5.7, "Camera speed range is now controlled with FEditorViewportClient::SetCameraSpeedRange()")
	...
	TSharedPtr<UE::RenderViewportFeedback::FReceiver, ESPMode::ThreadSafe> RenderViewportFeedback = nullptr;
...
protected:
	...
	/**
	 * Can be used to add location, rotation, and FOV deltas to the viewport camera.
	 * Should be used by ITF-based Viewport Interactions to control the camera.
	 */
	FViewportClientNavigationHelper* ViewportNavigationHelper;
	...
	UE_DEPRECATED(5.7, "Please use GetCameraSpeed() and SetCameraSpeed() instead.")
...
private:
	...
	void OnPostLoadMap(uint32 MapChangeFlags);
...
private:
	...
	/** The location of the near clipping plane in orthographic view. */
	TOptional<double> OrthographicNearPlane;
	
	/** The location of the far clipping plane in orthographic view. */
	TOptional<double> OrthographicFarPlane;
	
	/** Whether focusing on a box should clip the near plane to that selection. */
	bool bClipToSelectionOnFocus = false;
	
	/** The cached bounds of the world. */

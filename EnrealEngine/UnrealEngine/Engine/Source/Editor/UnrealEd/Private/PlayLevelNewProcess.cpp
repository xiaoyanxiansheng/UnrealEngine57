...
void UEditorEngine::LaunchNewProcess(const FRequestPlaySessionParams& InParams, const int32 InInstanceNum, EPlayNetMode NetMode, bool bIsDedicatedServer)
{
	...
	// If we are not launching a mobile preview, them we need to check the editor/project settings for using mouse touch as well.
	else
	{
#if WITH_EDITOR
		const bool bUseMouseForTouch = GetDefault<ULevelEditorPlaySettings>()->UseMouseForTouch || GetDefault<UInputSettings>()->bUseMouseForTouch;
#else
		const bool bUseMouseForTouch = GetDefault<UInputSettings>()->bUseMouseForTouch;
#endif

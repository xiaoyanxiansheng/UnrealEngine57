...
void FEditorBuildUtils::TriggerHierarchicalLODBuilder(UWorld* InWorld)
{
	if (InWorld->IsPartitionedWorld())
	{
		TSubclassOf<UWorldPartitionBuilder> WorldPartitionHLODsBuilder = FindObjectChecked<UClass>(nullptr, TEXT("/Script/UnrealEd.WorldPartitionHLODsBuilder"), EFindObjectFlags::ExactClass);
...
void FEditorBuildUtils::TriggerMinimapBuilder(UWorld* InWorld)
{
	if (InWorld->IsPartitionedWorld())
	{
		TSubclassOf<UWorldPartitionBuilder> WorldPartitionMiniMapBuilder = FindObjectChecked<UClass>(nullptr, TEXT("/Script/UnrealEd.WorldPartitionMiniMapBuilder"), EFindObjectFlags::ExactClass);
...
void FEditorBuildUtils::TriggerLandscapeSplineMeshesBuilder(UWorld* InWorld)
{
	if (InWorld->IsPartitionedWorld())
	{
		TSubclassOf<UWorldPartitionBuilder> WorldPartitionLandscapeSplineMeshesBuilder = FindObjectChecked<UClass>(nullptr, TEXT("/Script/UnrealEd.WorldPartitionLandscapeSplineMeshesBuilder"), EFindObjectFlags::ExactClass);
...
void FEditorBuildUtils::UpdateTextureStreamingMaterialBindings(UWorld* InWorld)
{
	...
	if (GetUsedMaterialsInWorld(InWorld, Materials, nullptr))
	{
		...
		for (UMaterialInterface* MaterialInterface : Materials)
		{
			...
			MaterialInterface->GetUsedTexturesAndIndices(UsedTextures, UsedIndices, QualityLevel, ShaderPlatform);
	...
	if (CompileDebugViewModeShaders(DVSM_OutputMaterialTextureScales, QualityLevel, FeatureLevel, Materials, &SlowTask))
	{
		...
		for (UMaterialInterface* MaterialInterface : Materials)
		{
			...
			if (MaterialInterface)
			{
				...
				FMaterialResource* Resource = Package ? MaterialInterface->GetMaterialResource(GMaxRHIShaderPlatform) : nullptr;

				TArray<FMaterialTextureInfo> PreviousData = MaterialInterface->GetTextureStreamingData();
				if (FMaterialUtilities::ExportMaterialUVDensities(MaterialInterface, Resource ? Resource->GetQualityLevel() : QualityLevel, ShaderPlatform, ExportErrors))
				{

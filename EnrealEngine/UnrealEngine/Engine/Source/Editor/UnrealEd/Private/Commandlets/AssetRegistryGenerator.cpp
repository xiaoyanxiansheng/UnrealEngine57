	...
	UE::Cook::FCookSandbox& InSandboxFile)
{
	...
	static bool bIsDLCCook = []()
		{
			FString DLCNameReturn;
			FParse::Value(FCommandLine::Get(), TEXT("DLCNAME="), DLCNameReturn);
			return !DLCNameReturn.IsEmpty();
		}();
	...
	for (int32 PakchunkIndex = 0; PakchunkIndex < FinalChunkManifests.Num() && bSucceeded; ++PakchunkIndex)
	{
		...
		{
			// During a chunk'd DLC cook ignore DefaultPakFile Rules. Those are handled during main cooks.
			if (bIsDLCCook)
			{
				continue;
			}
	...
	const TSet<FName>& StartupPackages)
{
	...
	else
	{
		...
		// Handle encrypted packages. Encrypted packages specified by GetEncryptedChunkIDsForPackage
		// use those specific chunks and nothing else.
		TArray<int32> EncryptedPackageChunkIDs = AssetManager.GetEncryptedChunkIDsForPackage(PackageFName).Array();
		if (EncryptedPackageChunkIDs.Num() != 0)
		{
			TargetChunks.Append(EncryptedPackageChunkIDs);
		}
		else
		{
			// Handle propagation of chunk assignments of Generated packages.
			// Generated packages by default use the chunks assigned to their Generator (unless encrypted, see above).
			// But generated packages for which the generator said it does not want chunk assignment from the generator
			// get their chunk assignment separately from the AssetManager.
			FName PackageNameThatDefinesChunks = PackageFName;
			UE::AssetRegistry::EDependencyProperty DependencyPropertyOfGenerator;
			FName GeneratorName = GetGeneratorPackage(PackageFName, this->State, DependencyPropertyOfGenerator);
			if (!GeneratorName.IsNone())
			{
				// CookGenerationHelper sets the Game flag on the dependency property if and only if it wants
				// the generated package to receive chunks from the generator,
				// see FGenerationHelper::FinishGeneratorPlatformSave.
				if (EnumHasAnyFlags(DependencyPropertyOfGenerator, UE::AssetRegistry::EDependencyProperty::Game))
				{
...
TMap<int32, FString> FAssetRegistryGenerator::GetPakChunkIdToStringOverrideMap()
{
	...
	for (const TPair<int32, FString>& PakChunkToName : ChunkIdStringOverride)
	{
		...
		for (TCHAR Char : PakChunkName.GetCharArray())
		{
			...
			else if (!ValidCharacters.Contains(Char))
			{
	...
	const FAssetRegistryState& PreviousState, FAssetRegistryDifference& OutDifference)
{
	...
	for (const TPair<FName, const FAssetPackageData*>& PackagePair : PreviousAssetPackageDataMap)
	{
		...
		if (!CurrentPackageData)
		{
			...
			UE::AssetRegistry::EDependencyProperty UnusedGeneratorProperty;
			FName GeneratorName = GetGeneratorPackage(PackageName, PreviousState, UnusedGeneratorProperty);
			if (!GeneratorName.IsNone())
			{
...
FName FAssetRegistryGenerator::GetGeneratorPackage(FName PackageName, const FAssetRegistryState& InState,
	UE::AssetRegistry::EDependencyProperty& OutDependencyPropertyOfGenerator)
{
	OutDependencyPropertyOfGenerator = UE::AssetRegistry::EDependencyProperty::None;
	bool bGenerated = false;
	...
	if (Referencers.Num() != 1)
	{
		return NAME_None;
	}
	FAssetDependency& Dependency = Referencers[0];
	OutDependencyPropertyOfGenerator = Dependency.Properties;
	return Dependency.AssetId.PackageName;
}
	...
	bool bGenerateStreamingInstallManifest, const TSet<FName>& StartupPackages)
{
	...
	// Prune our asset registry to cooked + dev only list
	{
		...
		State.PruneAssetData(AllPackages, TSet<FName>(), TSet<int32>(), DevelopmentSaveOptions);
	}
	...
	bool bForceNoFilter, uint64& OutDevelopmentAssetRegistryHash)
{
	...
	if (DevelopmentSaveOptions.bSerializeAssetRegistry)
	{
		...
		if (bGenerateChunks || FParse::Param(FCommandLine::Get(), TEXT("ForceGenerateAllChunksInfo")))
		{
...
bool FAssetRegistryGenerator::GenerateAssetChunkInformationCSV(const FString& OutputPath, bool bWriteIndividualFiles)
{
	...
	// Get the list of assets sorted by packagename, so the CSV is consistent between cooks.
	TArray<FAssetData*> AssetDataList = State.GetMutableAssetsSortedByObjectPath();
...
{
	static FString DLCName = []()
		{
			FString DLCNameReturn;
			FParse::Value(FCommandLine::Get(), TEXT("DLCNAME="), DLCNameReturn);
			return DLCNameReturn;
		}();
	if (DLCName.IsEmpty())
	{
	...
	{
		return InSandboxFile.GetSandboxDirectory(Platform) / FApp::GetProjectName() / TEXT("Plugins") / DLCName / TEXT("Metadata") / TEXT("ChunkManifest");
	}
}

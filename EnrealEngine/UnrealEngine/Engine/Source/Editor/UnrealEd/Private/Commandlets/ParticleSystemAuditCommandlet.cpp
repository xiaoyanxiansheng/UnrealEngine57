...
#include UE_INLINE_GENERATED_CPP_BY_NAME(ParticleSystemAuditCommandlet)
...
bool UParticleSystemAuditCommandlet::ProcessParticleSystems()
{
	...
	for (const FAssetData& AssetIt : AssetList)
	{
		...
		if (PSys != NULL)
		{
			...
			if (PSys->SystemUpdateMode == EParticleSystemUpdateMode::EPSUM_FixedTime)
			{
				ParticleSystemsWithFixedDeltaTime.Add(PSys->GetPathName());
			}
...
void UParticleSystemAuditCommandlet::DumpResults()
{
	...
	DumpSimplePSysSet(ParticleSystemsWithFixedDeltaTime, TEXT("PSysWithFixedDeltaTime"), TEXT("Name"));
}

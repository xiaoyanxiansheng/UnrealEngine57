...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AudioMixerCommandlet)
...
static void PlayOneShotSound(FAudioDevice* InAudioDevice, const TArray<USoundWave*>& InSounds)
{
	...
	const FString* EventLogID = &Audio::Trace::EventLog::ID::PlayRequestOneShot;
#else
	const FString* EventLogID = nullptr;
#endif // UE_AUDIO_PROFILERTRACE_ENABLED

	// Add it to the audio device
	InAudioDevice->AddNewActiveSound(NewActiveSound, nullptr, EventLogID);
}

...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AssetRegUtilCommandlet)
...
{
	TSet<FName> OldOrderFileLineFullSet;
	TSet<FName> NewOrderFileLineSet;
	if (!LoadOrderFiles(OldOrderFilePath, OldOrderFileLineFullSet) ||
	...
	// Remove deleted files from old order file lines, don't use .Intersect so we keep the original order
	for (const FName& OldOrderFileIter : OldOrderFileLineFullSet)
	{
		if (NewOrderFileLineSet.Contains(OldOrderFileIter))
		{
			OldOrderFileLineSet.Add(OldOrderFileIter);
		}
	}

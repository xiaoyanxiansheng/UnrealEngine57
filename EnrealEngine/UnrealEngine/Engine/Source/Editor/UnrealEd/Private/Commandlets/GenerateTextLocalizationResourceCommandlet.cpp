...
#include UE_INLINE_GENERATED_CPP_BY_NAME(GenerateTextLocalizationResourceCommandlet)
...
int32 UGenerateTextLocalizationResourceCommandlet::Main(const FString& Params)
{
	...
	// Generate the LocMeta file for all cultures
	{
		...
		{
			UE_LOG(LogGenerateTextLocalizationResourceCommandlet, Log, TEXT("Generated file %s"), *TextLocalizationMetaDataResourcePath);
		}
		else
		{
	...
	TSharedPtr<ILocFileNotifies> LocFileNotifies = LocTextHelper.GetLocFileNotifies();
	...
	ParallelFor(TEXT("GenerateTextLocalizationResourceCommandlet"), CulturesToGenerate.Num(), 1, [&](int32 Index)
	{
		const FString& CultureName = CulturesToGenerate[Index];
		...
		auto GenerateSingleLocRes = [this, &DestinationPath, &CultureName, &ResourceName](const FTextLocalizationResource& InLocRes, const FName InPlatformName) -> bool
		{
			...
			if (bLocResFileSaved)
			{
				UE_LOG(LogGenerateTextLocalizationResourceCommandlet, Log, TEXT("Generated file %s"), *TextLocalizationResourcePath);
			}
			else
			{
		...
		if (!GenerateSingleLocRes(PlatformAgnosticLocRes, FName()))
		{
			bSuccess = false;
		}
		for (const auto& PerPlatformLocResPair : PerPlatformLocRes)
		{
			if (!GenerateSingleLocRes(*PerPlatformLocResPair.Value, PerPlatformLocResPair.Key))
			{
				bSuccess = false;
			}
		}

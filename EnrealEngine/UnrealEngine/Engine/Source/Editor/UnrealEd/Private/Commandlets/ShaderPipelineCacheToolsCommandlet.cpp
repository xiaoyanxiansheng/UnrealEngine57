...
#include UE_INLINE_GENERATED_CPP_BY_NAME(ShaderPipelineCacheToolsCommandlet)
...
static void LoadStableShaderKeysMultiple(FStableShaderSet& StableMap, TArrayView<const FStringView> FileNames)
{
	...
	{
		// Since stable keys are saved from a TSet, we assume that a single array does not have non-unique members, so add the largest one first in the most optimal manner
		...
		for (const TArray<FStableShaderKeyAndValue>& StableArray : StableArrays)
		{
			...
			{
				bFirst = false;
				for (const FStableShaderKeyAndValue& Shader : StableArray)
				{
					StableMap.Add(MakeUnique<FStableShaderKeyAndValue>(Shader));
				}
			}
			else
			{
				for (const FStableShaderKeyAndValue& Shader : StableArray)
				{
					// Double lookup to avoid MakeUnique for duplicates
					if (!StableMap.ContainsByHash(GetTypeHash(Shader), Shader))
					{
						StableMap.Add(MakeUnique<FStableShaderKeyAndValue>(Shader));
					}
				}
...
int32 DumpPSOSC(FString& Token, const FString& StableKeyFileDir)
{
	...
	if (StableKeyFileDir.Len())
	{
		...
		for (const TUniquePtr<FStableShaderKeyAndValue>& ShaderKeyPtr : StableMap)
		{
			InverseMap.FindOrAdd(ShaderKeyPtr->OutputHash).Add(ShaderKeyPtr->ToString());
		}
	}
...
static void StableShadersSerializationSelfTest(const FStableShaderSet& StableMap)
{
	TAnsiStringBuilder<384> TestString;
	for (const TUniquePtr<FStableShaderKeyAndValue>& ShaderKeyPtr : StableMap)
	{
		TestString.Reset();
		check(ShaderKeyPtr->OutputHash != FSHAHash());
		ShaderKeyPtr->AppendString(TestString);
		FStableShaderKeyAndValue TestItem;
		TestItem.ParseFromString(UTF8_TO_TCHAR(TestString.ToString()));
		check(*ShaderKeyPtr == TestItem);
		check(GetTypeHash(*ShaderKeyPtr) == GetTypeHash(TestItem));
		check(ShaderKeyPtr->OutputHash == TestItem.OutputHash);
	}
}
...
int32 DumpStableKeysFile(const FString& Token, const TMap<FString, FString>& ParamVals)
{
	...
	for (const TUniquePtr<FStableShaderKeyAndValue>& ShaderKeyPtr : StableMap)
	{
		if (!OutputHashesToSearch.IsEmpty() && !OutputHashesToSearch.Contains(ShaderKeyPtr->OutputHash.ToString().ToLower()))
		{
			continue;
		}
		...
		{
			const FString* Found = Algo::FindByPredicate(NamesToSearch, [ObjName = ShaderKeyPtr->ClassNameAndObjectPath.ToString().ToLower()](const FString& PartialName) { return ObjName.Contains(PartialName); });
			if (!Found)
			{
				continue;
			}
		...
		UE_LOG(LogShaderPipelineCacheTools, Display, TEXT("    %s"), *ShaderKeyPtr->ToString());
	}
	return 0;
}
...
int32 CheckStableKeyAliasing(const TArray<FString>& Tokens)
{
	...
	TArray<FStableShaderSet> StableMaps;
	...
	TArray<TMultiMap<FSHAHash, const FStableShaderKeyAndValue*>> InverseStableMaps;
	InverseStableMaps.AddDefaulted(StableMaps.Num());

	for (int32 IdxFile = 0; IdxFile < Tokens.Num(); ++IdxFile)
	{
		for (const TUniquePtr<FStableShaderKeyAndValue>& ShaderKeyPtr : StableMaps[IdxFile])
		{
			InverseStableMaps[IdxFile].Add(ShaderKeyPtr->OutputHash, ShaderKeyPtr.Get());
		}
	}

	auto LogAllInFile = [](const FString& FileName, const TMultiMap<FSHAHash, const FStableShaderKeyAndValue*>& InverseMap, const FSHAHash& Hash)
	{
		TArray<const FStableShaderKeyAndValue*> Values;
...
static TSet<FPipelineCacheFileFormatPSO> ParseStableCSV(const FString& FileName, const TArray<FString>& CSVLines, const FStableShaderSet& StableMap, FName& TargetPlatform, int32& PSOsRejected, int32& PSOsMerged)
{
	TSet<FPipelineCacheFileFormatPSO> PSOs;
	...
	ReadStableCSV(CSVLines, [&FileName, &StableMap, &TargetPlatform, &PSOs, &LineIndex, &bParsed, &PSOsRejected, &PSOsMerged](FStringView Line)
	{
		...
		for (int32 PartIndex = 2; PartIndex < Parts.Num(); ++PartIndex)
		{
			...
			if (const TUniquePtr<FStableShaderKeyAndValue>* ShaderKeyPtr = StableMap.Find(Shader))
			{
				Match = (*ShaderKeyPtr)->OutputHash;
				check(Match != FSHAHash());

				TargetPlatform = (*ShaderKeyPtr)->TargetPlatform;
			}
			else
			{
...
void FilterInvalidPSOs(TSet<FPipelineCacheFileFormatPSO>& InOutPSOs, const FStableShaderSet& StableMap)
{
	...
	for (const TUniquePtr<FStableShaderKeyAndValue>& ShaderKeyPtr : StableMap)
	{
		InverseMap.FindOrAdd(ShaderKeyPtr->OutputHash).Add(*ShaderKeyPtr);
	}
...
void AddComputePSOs(TSet<FPipelineCacheFileFormatPSO>& OutPSOs, const FStableShaderSet& StableShaderMap)
{
	...
	for (const TUniquePtr<FStableShaderKeyAndValue>& ShaderKeyPtr : StableShaderMap)
	{
		if (ShaderKeyPtr->TargetFrequency == NAME_SF_Compute)
		{
			// add a new Compute PSO
			// Check if we are only allowed to add Niagara PSOs
			if (GShaderPipelineCacheTools_ComputePSOInclusionMode == 2 &&ShaderKeyPtr->ShaderType != NAME_NiagaraShader)
			{
				continue;
			}
			...
			NewPso.ComputeDesc.ComputeShader = ShaderKeyPtr->OutputHash;
...
FName GetTargetPlatformFromStableShaderKeys(const FStableShaderSet& StableShaderMap)
{
	if (const TUniquePtr<FStableShaderKeyAndValue>* ShaderKeyPtr = StableShaderMap.FindArbitraryElement())
	{
		return (*ShaderKeyPtr)->TargetPlatform;
	}
...
int32 SaveBinaryPipelineCacheFile(const FString& OutputFilename, const EShaderPlatform ShaderPlatform, const FString& ShaderFormat, const FString& ChunkInfoFilesPath, const FString& AssociatedShaderLibraryName, const FString& TargetPlatformName, const TSet<FPipelineCacheFileFormatPSO>& PSOs, const FStableShaderSet& StableMap)
{
	...
	else
	{
		...
		FGraphEventRef StableMapConvTask = FFunctionGraphTask::CreateAndDispatchWhenReady([&StableNameMap, &StableMap]
			{
				for (const TUniquePtr<FStableShaderKeyAndValue>& ShaderKeyPtr : StableMap)	// could be parallelized (skip first N*ThreadIdx iterations on each thread?)
				{
					FName PackageName(*ShaderKeyPtr->ClassNameAndObjectPath.ToStringPathOnly());
					StableNameMap.Add(PackageName, ShaderKeyPtr->OutputHash);
				}
			}, TStatId());
...
int32 BuildPSOSC(const TArray<FString>& Tokens, const TMap<FString, FString>& ParamVals)
{
	...
	FGraphEventRef StableMapTask = FFunctionGraphTask::CreateAndDispatchWhenReady([&StableShaderFiles, &StableMap]
	{
		...
		if (UE_LOG_ACTIVE(LogShaderPipelineCacheTools, Verbose))
		{
			...
			for (const TUniquePtr<FStableShaderKeyAndValue>& ShaderKeyPtr : StableMap)
			{
				UE_LOG(LogShaderPipelineCacheTools, Verbose, TEXT("    %s"), *ShaderKeyPtr->ToString());
			}

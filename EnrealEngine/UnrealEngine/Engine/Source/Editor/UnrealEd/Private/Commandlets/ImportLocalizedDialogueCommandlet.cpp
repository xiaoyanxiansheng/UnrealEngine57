...
#include UE_INLINE_GENERATED_CPP_BY_NAME(ImportLocalizedDialogueCommandlet)
...
int32 UImportLocalizedDialogueCommandlet::Main(const FString& Params)
{
	...
	// Paths to scan for existing localized dialogue and sound waves
	...
	{
		FString PreferredCultureIdentifier = CultureName;
		if (FCulture::GetPreferredOnDiskIdentifierType() == ECultureIdentifierType::Verse)
		{
			PreferredCultureIdentifier = FCulture::CultureNameToVerseIdentifier(CultureName);
		}
		...
		CultureImportInfo.LocalizedRootContentPath = RootContentDir / TEXT("L10N") / PreferredCultureIdentifier;
		CultureImportInfo.LocalizedRootPackagePath = RootAssetPath / TEXT("L10N") / PreferredCultureIdentifier;
		...
		LocalizedDialogueWavePathsToSearch.Add(*(RootAssetPath / TEXT("L10N") / CultureName));
		LocalizedSoundWavePathsToSearch.Add(*(RootAssetPath / TEXT("L10N") / CultureName / ImportedDialogueFolder));
		if (const FString VerseIdentifier = FCulture::CultureNameToVerseIdentifier(CultureName);
			VerseIdentifier != CultureName)
		{
			LocalizedDialogueWavePathsToSearch.Add(*(RootAssetPath / TEXT("L10N") / VerseIdentifier));
			LocalizedSoundWavePathsToSearch.Add(*(RootAssetPath / TEXT("L10N") / VerseIdentifier / ImportedDialogueFolder));
		}
	}
	...
	for (const FAssetData& AssetData : AssetDataArrayForDialogueWaves)
	{
		...
		{
			UE_LOG(LogImportLocalizedDialogueCommandlet, Error, TEXT("Asset registry found asset '%s', but it's not actually a dialogue wave."), *AssetData.GetSoftObjectPath().ToString());
			continue;
		}
		...
		if (!DialogueWave)
		{
			UE_LOG(LogImportLocalizedDialogueCommandlet, Error, TEXT("Asset registry found asset '%s', but the dialogue wave could not be loaded."), *AssetData.GetSoftObjectPath().ToString());
			continue;
		}
	...
	for (const FAssetData& LocalizedAssetData : LocalizedAssetsToPotentiallyDelete)
	{
		...
		if (!LocalizedAsset)
		{
			UE_LOG(LogImportLocalizedDialogueCommandlet, Error, TEXT("Asset registry found localized asset '%s' to delete, but the asset could not be loaded."), *LocalizedAssetData.GetSoftObjectPath().ToString());
			continue;
		}

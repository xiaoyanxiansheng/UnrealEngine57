...
#include UE_INLINE_GENERATED_CPP_BY_NAME(WorldPartitionConvertCommandlet)
...
void UWorldPartitionConvertCommandlet::ClearHierarchicalLODSetup(ULevel* Level)
{
...
bool UWorldPartitionConvertCommandlet::RenameWorldPackageWithSuffix(UWorld* World)
{
	...
	// First, rename the world package so code relying on it when renaming the world can work properly.
	...
	// Make sure the world object has the same name as its package.
...
int32 UWorldPartitionConvertCommandlet::Main(const FString& Params)
{
	...
	if (!bReportOnly)
	{
		...
		EAssetsWithOuterForPathsFlags OuterForPathsFlags = (EAssetsWithOuterForPathsFlags::RecursivePaths | EAssetsWithOuterForPathsFlags::IncludeOnlyOnDiskAsset | EAssetsWithOuterForPathsFlags::ScanPaths);
		if (TArray<FAssetData> EntityAssets = UAssetRegistryHelpers::GetAssetsWithOuterForPaths({ *OldLevelName }, *OldLevelName, OuterForPathsFlags); !EntityAssets.IsEmpty())
		{
			for (const FAssetData& EntityAsset : EntityAssets)
			{
				FString EntityAssetPath;
				if (FPackageName::TryConvertLongPackageNameToFilename(EntityAsset.PackageName.ToString(), EntityAssetPath, FPackageName::GetAssetPackageExtension()))
				{
					FilesToDelete.Add(EntityAssetPath);
				}
				else
				{
					UE_LOG(LogWorldPartitionConvertCommandlet, Error, TEXT("Failed to delete previous conversion package %s"), *EntityAsset.PackageName.ToString());
					return 1;
				}
			}
		}

		if (!PackageHelper.Delete(FilesToDelete))
		{
	...
	auto PrepareLevelActors = [this, PartitionFoliage, MainWorld, ConvertActorLayersToDataLayers](ULevel* Level, TArray<AActor*>& Actors, bool bMainLevel) -> bool
	{
		...
		ClearHierarchicalLODSetup(Level);
	...
	for(ULevel* SubLevel : SubLevelsToConvert)
	{
		...
		if (LevelHasLevelScriptBlueprint(SubLevel))
		{
			...
			LevelInstanceActor->SetDesiredRuntimeBehavior(ELevelInstanceRuntimeBehavior::LevelStreaming);
	...
	if (!bReportOnly)
	{
		...
		for(AActor* Actor: ActorList)
		{
			...
			ForEachObjectWithOuter(Actor, [this](UObject* Object)
			{
				if (Object->IsPackageExternal())
				{
					PackagesToSave.Add(Object->GetPackage());
				}
			});

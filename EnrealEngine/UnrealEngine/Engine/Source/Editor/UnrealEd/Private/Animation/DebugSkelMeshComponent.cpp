...
#include "ClothingSimulationInstance.h"  // UE_DEPRECATED(5.7, "Use GetClothingSimulationInstances instead.")
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(DebugSkelMeshComponent)
...
FPrimitiveSceneProxy* UDebugSkelMeshComponent::CreateSceneProxy()
{
		...
		MeshObject)
	{
		...
		{
			Result = ::new FDebugSkelMeshSceneProxyNanite(*MeshObject->GetNaniteMaterials(), this, SkelMeshRenderData, WireframeMeshOverlayColor);
		}
		else
		{
...
{
	return !IsDisplayingNaniteFallback() && USkinnedMeshComponent::ShouldNaniteSkin();
}
...
void UDebugSkelMeshComponent::SendRenderDynamicData_Concurrent()
{
	...
	if (SceneProxy)
	{
		...
		const bool bNaniteProxy = MeshObject && MeshObject->IsNaniteMesh();
		FDebugSkelMeshSceneProxyCommon* TargetProxy;
		if (bNaniteProxy)
		{
			TargetProxy = static_cast<FDebugSkelMeshSceneProxyNanite*>(SceneProxy);
		}
		else
		{
			TargetProxy = static_cast<FDebugSkelMeshSceneProxy*>(SceneProxy);
		}
		
		ENQUEUE_RENDER_COMMAND(DebugSkelMeshObjectUpdateDataCommand)(
			[NewDynamicData, TargetProxy, bNaniteProxy](FRHICommandListImmediate& RHICommandList)
		{
...
void UDebugSkelMeshComponent::RefreshSelectedClothingSkinnedPositions()
{
	...
	{
		const TObjectPtr<UClothingAssetBase>* const Asset = GetSkeletalMeshAsset()->GetMeshClothingAssets().FindByPredicate(
			[this](const TObjectPtr<UClothingAssetBase>& Item) -> bool
			{
				return Item && Item->GetAssetGuid() == SelectedClothingGuidForPainting;
			});

		if (const UClothingAssetCommon* const ConcreteAsset = Cast<const UClothingAssetCommon>(Asset ? *Asset : nullptr))
		{
			if (ConcreteAsset->LodData.IsValidIndex(SelectedClothingLodForPainting))
			{
				TArray<FMatrix44f> RefToLocals;
				GetCurrentRefToLocalMatrices(RefToLocals, 0);  // Pass LOD0 to collect all bones

				const FClothLODDataCommon& LodData = ConcreteAsset->LodData[SelectedClothingLodForPainting];

				ClothingMeshUtils::SkinPhysicsMesh(
					ConcreteAsset->UsedBoneIndices,
					LodData.PhysicalMeshData,
					FTransform::Identity,
					RefToLocals.GetData(),
					RefToLocals.Num(),
...
{
	if (GetClothingSimulationInstances().Num())
	{
		// This function seems unused, and therefore a const_cast is probably a better idea than compromising the const version of GetClothingSimulation()
		if (IClothingSimulationInterface* const ClothingSimulation =
			const_cast<IClothingSimulationInterface*>(GetClothingSimulationInstances()[0].GetClothingSimulation()))
		{
...
/***************************************************
 * FDebugSkelMeshSceneProxyCommon
 ***************************************************/
void FDebugSkelMeshSceneProxyCommon::GetDynamicMeshElements(
	...
	FMeshElementCollector& Collector) const
{
	if (MeshObject && DynamicData && (DynamicData->bDrawNormals || DynamicData->bDrawTangents || DynamicData->bDrawBinormals))
	{
		for(int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
		{
			...
			{
				MeshObject->DrawVertexElements(Collector.GetPDI(ViewIndex), SceneProxy->GetLocalToWorld(), DynamicData->bDrawNormals, DynamicData->bDrawTangents, DynamicData->bDrawBinormals);
			}
		}
	}
			...
			if (Views.Num())
			{
				...
				if (MatProxySurface && MatProxyWireframe)
				{
					...
					for (int32 ViewIndex = 0; ViewIndex < NumViews; ++ViewIndex)
					{
						...
						MeshBuilderSurface.GetMesh(SceneProxy->GetLocalToWorld(), MatProxySurface, SDPG_Foreground, false, false, ViewIndex, Collector);
						MeshBuilderWireframe.GetMesh(SceneProxy->GetLocalToWorld(), MatProxyWireframe, SDPG_Foreground, false, false, ViewIndex, Collector);
					}
				}
			}
		}
 ...
 ***************************************************/
FDebugSkelMeshSceneProxy::FDebugSkelMeshSceneProxy(const UDebugSkelMeshComponent* InComponent, FSkeletalMeshRenderData* InSkelMeshRenderData, FLinearColor InWireframeOverlayColor)
	: FSkeletalMeshSceneProxy(InComponent, InSkelMeshRenderData)
{
	...
	{
		MaterialRelevance |= GEngine->ClothPaintMaterial->GetRelevance_Concurrent(GetScene().GetShaderPlatform());
	}
}
...
void FDebugSkelMeshSceneProxy::GetDynamicMeshElements(const TArray<const FSceneView*>& Views, const FSceneViewFamily& ViewFamily, uint32 VisibilityMap, FMeshElementCollector& Collector) const
{
	...
	{
		GetMeshElementsConditionallySelectable(Views, ViewFamily, IsSelectable(), VisibilityMap, Collector);
	}
	FDebugSkelMeshSceneProxyCommon::GetDynamicMeshElements(this, GetMeshObject(), Views, ViewFamily, VisibilityMap, Collector);
}

/***************************************************
 * FDebugSkelMeshSceneProxyNanite
 ***************************************************/
FDebugSkelMeshSceneProxyNanite::FDebugSkelMeshSceneProxyNanite(const Nanite::FMaterialAudit& Materials, const UDebugSkelMeshComponent* InComponent, FSkeletalMeshRenderData* InSkelMeshRenderData, FLinearColor InWireframeOverlayColor)
	: Nanite::FSkinnedSceneProxy(Materials, InComponent, InSkelMeshRenderData)
{
	SetWireframeColor(InWireframeOverlayColor);
}

SIZE_T FDebugSkelMeshSceneProxyNanite::GetTypeHash() const
{
	static size_t UniquePointer;
	return reinterpret_cast<size_t>(&UniquePointer);
}

void FDebugSkelMeshSceneProxyNanite::GetDynamicMeshElements(const TArray<const FSceneView*>& Views, const FSceneViewFamily& ViewFamily, uint32 VisibilityMap, FMeshElementCollector& Collector) const
{
	if (!DynamicData || DynamicData->bDrawMesh)
	{

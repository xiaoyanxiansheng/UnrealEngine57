...
FEditorDomain::FEditorDomain(EEditorDomainEnabled EnableLevel)
{
	...
	// Load / Save from EditorDomain before AssetRegistry has finished its gather can cause performance problems due to
	// blocking on AssetRegistry, and can also cause deadlocks with the AssetRegistry's gather thread, UE-281930.
	// Disable it until we have fixed those problems.
	bEnabledBeforeAssetRegistryGather = false;
	if (bExternalSave)
	{
	...
	if (AssetRegistry->IsLoadingAssets() || !AssetRegistry->IsSearchAllAssets())
	{
		AssetRegistry->OnFilesLoaded().AddRaw(this, &FEditorDomain::OnAssetRegistryInitialScanComplete);
	}
	else
	{
		bAssetRegistryInitialScanComplete.store(true, std::memory_order_relaxed);
	}
...
{
	// We have a pointer to AssetRegistry, but it might be dangling due to the AssetRegistry module already shutting down.
	// Refetch it from the singleton pointer, and handle the case that it is now null.
	AssetRegistry = IAssetRegistry::Get();
	...
	EPackageSegment PackageSegment, FPackagePath* OutUpdatedPath)
{
	...
	if (PackageSegment != EPackageSegment::Header
		|| (!bEnabledBeforeAssetRegistryGather && !IsAssetRegistryInitialScanComplete()))
	{
...
void FEditorDomain::OnEndLoadPackage(const FEndLoadPackageContext& Context)
{
	...
	if (!bEnabledBeforeAssetRegistryGather && !IsAssetRegistryInitialScanComplete())
	{
		return;
	}
...
void FEditorDomain::OnAssetRegistryInitialScanComplete()
{
	bAssetRegistryInitialScanComplete.store(true, std::memory_order_relaxed);
}

bool FEditorDomain::IsAssetRegistryInitialScanComplete() const
{
	return bAssetRegistryInitialScanComplete.load(std::memory_order_relaxed);
}

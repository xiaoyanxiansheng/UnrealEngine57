...
private:
	...
	*/
	...
	/** Callback for AssetRegistry's InitialScanComplete, to enable save/load if we disable before initial scan. */
	void OnAssetRegistryInitialScanComplete();
	/** Report whether we have set our flag from OnAssetRegistryInitialScanComplete. */
	bool IsAssetRegistryInitialScanComplete() const;
	...
	/** Marker for whether AssetRegistry initial scan has finished. */
	std::atomic<bool> bAssetRegistryInitialScanComplete{ false };
	/** Configuration setting for whether save/load is allowed before the AssetRegistry initial scan has finished. */
	bool bEnabledBeforeAssetRegistryGather = true;
	/**

...
FBulkDataRegistryImpl::TryRegister(UPackage* Owner, const UE::Serialization::FEditorBulkData& BulkData)
{
	...
	{
		AddPendingPackageBulkData(PackageName, MoveTemp(PendingPackageBulk), bAllowedToReadWritePayloadIdFromCache);

		// If The PendingPackage found that it is not allowed to use cache operations, then go back into the BulkData
		// in the registry and mark that it is not allowed to write its payload ids to the cache. PayloadIds are not
		// allowed to be written unless we read them first and find they do not exist, and the only place we read them
		// is from the PendingPackage.
		if (!bAllowedToReadWritePayloadIdFromCache)
		{
			FWriteScopeLock RegistryScopeLock(RegistryLock);
			if (bActive)
			{
				UE::BulkDataRegistry::Private::FRegisteredBulk* RegisteredBulk
					= Registry.Find(BulkData.GetIdentifier());
				if (RegisteredBulk)
				{
					RegisteredBulk->bAllowedToWritePayloadIdToCache = false;
				}
...
void FBulkDataRegistryImpl::AddPendingPackageBulkData(FName PackageName, UE::Serialization::FEditorBulkData&& BulkData,
	bool& bOutCacheOperationsAllowed)
{
	FScopeLock PendingPackageScopeLock(&PendingPackageLock);
	...
	{
		bOutCacheOperationsAllowed = false;
		return;
	}
	bOutCacheOperationsAllowed = PendingPackage->AreCacheOperationsAllowed();
	...
	, Owner(InOwner)
{
	...
	// The BulkDataList for the package is keyed by the EditorDomain key for the package, which requires
	// data from the AssetRegistry and will block on the AssetRegistry's gather of the package. That blocking makes editor
	// startup too slow, and we want to skip it.
	// Therefore, for packages that load while the AssetRegistry is still scanning, we skip reading their cache value,
	// and never read/write their cache. Building DDC jobs with these packages, if they have any PlaceHolderPayloadIds,
	// will therefore always need to recalculate the real PayloadId by loading the bulk data before the DDC build starts.
	// TODO: Change this to be more nuanced, by waiting for the AssetRegistry to report the package is loaded and then
	// sending off this GetBulkDataList request, and finish off the rest of the package.
	...
	{
		bCacheOperationsAllowed = false;
		OnBulkDataListResults(FSharedBuffer());
	}
}
...
void FPendingPackage::OnEndLoad(bool& bOutShouldRemove, bool& bOutShouldWriteCache)
{
	...
	{
		bOutShouldWriteCache = bCacheOperationsAllowed;
...
void FPendingPackage::WriteCache()
{
	...
	if (!bCacheOperationsAllowed)
	{
		// bCacheOperationsAllowed is set to false for packages that are loaded to early to read from the cache.
		// It might still be too early to write to the cache, but also, we're not allowed to write without
		// first checking whether the value already exists by reading.
		return;
	}

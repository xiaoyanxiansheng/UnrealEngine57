...
static const FBlake3Hash* GetClassDigestInternal(FTopLevelAssetPath ClassPath, FClassDigestMap& ClassDigests)
{
	// Caller has read-locked ClassDigests.Lock.

	const FClassDigestData* ExistingData = ClassDigests.Map.Find(ClassPath);
	if (!ExistingData)
	{
		if (!IsInGameThread() || GIsSavingPackage)
		{
			return nullptr;
		}
	...
	const FClassDigestData* NativeData = ExistingData;
	if (ExistingData->ClosestNative != ClassPath)
	{
		NativeData = ClassDigests.Map.Find(ExistingData->ClosestNative);
		checkf(NativeData, TEXT("Classes are only stored in a ClosestNative field if they exist"));
	...
	return &NativeData->InclusiveSchemaHash;
}

bool TryAppendClassDigests(const TArrayView<UE::Cook::FCookDependency>& Dependencies, FBlake3& Writer,
	UE::Cook::FCookDependencyContext& Context, bool bUpdateValues)
{
	FClassDigestMap& ClassDigests = GetClassDigests();
	FReadScopeLock ClassDigestsScopeLock(ClassDigests.Lock);
	...
	{
		FStringView ClassPathStr = ClassDependency.GetClassPath();
		FTopLevelAssetPath ClassPath(ClassPathStr);
		if (!ClassPath.IsValid())
		{
			Context.LogError(FString::Printf(TEXT("ClassDependency failed: %.*s is not a valid TopLevelAssetPath."),
				ClassPathStr.Len(), ClassPathStr.GetData()));
			continue;
		}

		const FBlake3Hash* Hash = GetClassDigestInternal(ClassPath, ClassDigests);
		if (!Hash)
		{
			Context.LogError(FString::Printf(TEXT("%s is not a valid class."), *ClassPath.ToString()));
			return false;
		}

		if (bUpdateValues)
		{
			ClassDependency.SetValue(*Hash);
		}
		Writer.Update(&Hash->GetBytes(), sizeof(Hash->GetBytes()));
	}

	return true;
}

TOptional<FBlake3Hash> TryGetClassDigest(FTopLevelAssetPath ClassPath)
{
	if (!ClassPath.IsValid())
	{
		return TOptional<FBlake3Hash>();
	}
	...
	const FBlake3Hash* Hash = GetClassDigestInternal(ClassPath, ClassDigests);
	return Hash ? TOptional<FBlake3Hash>(*Hash) : TOptional<FBlake3Hash>();
}
...
class FEditorDomainPackageWriter final : public TPackageWriterToSharedBuffer<FBasePackageWriter>
{
public:
	// IPackageWriter

...
bool UUnrealEdEngine::Exec_Actor( UWorld* InWorld, const TCHAR* Str, FOutputDevice& Ar )
{
	...
	else if( FParse::Command(&Str,TEXT("HIDE")) )
	{
		if( FParse::Command(&Str,TEXT("SELECTED")) ) // ACTOR HIDE SELECTED
		{
			...
			{
				if (FParse::Command(&Str, TEXT("HIERARCHY")))
				{
					const FScopedTransaction Transaction( NSLOCTEXT("UnrealEd", "HideSelectedHierarchyAtStartup", "Hide Selected Hierarchy at Editor Startup") );
					edactHideSelectedHierarchyStartup( InWorld );
					return true;
				}
				else
				{
			...
			else if (FParse::Command(&Str, TEXT("HIERARCHY")))
			{
				const FScopedTransaction Transaction( NSLOCTEXT("UnrealEd", "HideSelectedHierarchy", "Hide Selected Hierarchy") );
				edactHideSelectedHierarchy( InWorld );
	...
	else if( FParse::Command(&Str,TEXT("UNHIDE")) ) 
	{
		...
		else if( FParse::Command(&Str,TEXT("SELECTED")) )	// ACTOR UNHIDE SELECTED
		{
			if ( FParse::Command(&Str,TEXT("STARTUP")) ) // ACTOR UNHIDE SELECTED STARTUP
			{
				if (FParse::Command(&Str, TEXT("HIERARCHY")))
				{
					const FScopedTransaction Transaction( NSLOCTEXT("UnrealEd", "ShowSelectedHierarchyAtStartup", "Show Selected Hierarchy at Editor Startup") );
					edactUnHideSelectedHierarchyStartup( InWorld );
					return true;
				}
				else
				{
			...
			{
				const FScopedTransaction Transaction( NSLOCTEXT("UnrealEd", "UnhideHierarchySelected", "Unhide Selected Hierarchy") );
				edactUnhideSelectedHierarchy( InWorld );
				return true;
			}
			else
			{
	...
	else if ( FParse::Command(&Str, TEXT("TOGGLE SELECTED VISIBILITY")) )
	{
		if (FParse::Command(&Str, TEXT("HIERARCHY")))
		{
			const FScopedTransaction Transaction( NSLOCTEXT("UnrealEd", "ToggleSelectedHierarchyVisibility", "Toggle Selected Hierarchy Visibility") );
			edactToggleSelectedHierarchyVisibility( InWorld );
			return true;
		}
		else
		{
			const FScopedTransaction Transaction( NSLOCTEXT("UnrealEd", "ToggleSelectedVisibility", "Toggle Selected Visibility") );
			edactToggleSelectedVisibility( InWorld );
			return true;
		}
	}
...
bool UUnrealEdEngine::Exec_Mode( const TCHAR* Str, FOutputDevice& Ar )
{
	...
	if(GCurrentLevelEditingViewportClient)
	{
		float NewCameraSpeed = 1.0f;
		if ( FParse::Value( Str, TEXT("SPEED="), NewCameraSpeed ) )
		{
			GetMutableDefault<ULevelEditorViewportSettings>()->CameraSpeedSettings.SetCurrentSpeed(NewCameraSpeed); 
		
			GetMutableDefault<ULevelEditorViewportSettings>()->CameraSpeed_DEPRECATED = NewCameraSpeed;
		}
	}

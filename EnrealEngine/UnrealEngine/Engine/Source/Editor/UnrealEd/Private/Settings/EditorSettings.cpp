...
#include UE_INLINE_GENERATED_CPP_BY_NAME(EditorSettings)
...
void UEditorSettings::PostEditChangeProperty( struct FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	FName DeprecatedName = GET_MEMBER_NAME_CHECKED(UEditorSettings, bLoadTheMostRecentlyLoadedProjectAtStartup);
	if (PropertyName == DeprecatedName)
	{
		...
		// Broadcast that this setting changed
		OnMostRecentProjectSettingChanged().Broadcast(bLoadTheMostRecentlyLoadedProjectAtStartup);
	}

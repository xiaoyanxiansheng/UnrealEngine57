...
#include "PackageTools_Private.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PackageTools)
...
FOnUnloadPackagesDelegate OnUnloadPackagesDelegate;
}	 // namespace PackageTools_Private
}	 // namespace UE::UnrealEd
	...
	: Super(ObjectInitializer)
{
	 ...
	 * @param	InLoadFlags	Flags controlling loading behavior, from the ELoadFlags enum
	 *
	 * @return	The loaded package (or NULL if something went wrong.)
	 */
	UPackage* UPackageTools::LoadPackage( FString InFilename, uint32 InLoadFlags )
	{
		...
		UPackage* Package = ::LoadPackage( NULL, *InFilename, InLoadFlags );
	...
	bool UPackageTools::UnloadPackages(FUnloadPackageParams& Params)
	{
		...
		if (PackagesToUnload.Num() > 0 && GEditor)
		{
			...
			// Before we clean up the assets in the packages that are about to be unloaded, we give a chance for any registered observers
			// to clean up any state that they need to for the assets in those packages.
			UE::UnrealEd::PackageTools_Private::OnUnloadPackagesDelegate.Broadcast(PackagesToUnload);
			...
			for (UPackage* PackageBeingUnloaded : PackagesToUnload)
			{
				...
				FCoreDelegates::CleanupUnloadingObjects.Broadcast(ObjectsInPackage);
				for (UObject* Obj : ObjectsInPackage)
				{
					// Asset manager can hold hard references to this object and prevent GC
					if (UE::UnrealEd::PackageTools_Private::bUnloadPackagesUnloadsPrimaryAssets)
					{
	...
	bool UPackageTools::ReloadPackages( const TArray<UPackage*>& TopLevelPackages, FText& OutErrorMessage, const EReloadPackagesInteractionMode InteractionMode )
	{
		...
		{
			FScopedSlowTask UnloadingWorldSlowTask(1.f, NSLOCTEXT("UnrealEd", "UnloadingWorld", "Unloading World"));
			UnloadingWorldSlowTask.ForceRefresh();
		...
		{
			FScopedSlowTask ReloadingWorldSlowTask(1.f, NSLOCTEXT("UnrealEd", "ReloadingWorld", "Reloading World"));
			ReloadingWorldSlowTask.ForceRefresh();
		...
		else
		{
			...
			CurrentWorldPtr->InvalidateModelGeometry(nullptr);
	...
	void UPackageTools::CheckOutRootPackages( const TArray<UPackage*>& Packages )
	{
		if (ISourceControlModule::Get().IsEnabled())
		{
			...
			if (FailedPackageNames.Num() > 0)
			{
				FMessageDialog::Open(EAppMsgType::Ok, NSLOCTEXT("UnrealEd", "Error_PackageStatusChanged", "Some package(s) can't be checked out - status has changed!"));
			}

			if (TouchedPackageNames.Num() > 0)
			{
	...
	UPackage* UPackageTools::FindOrCreatePackageForAssetType(const FName LongPackageName, UClass* AssetClass)
	{
		if (AssetClass)
		{
			...
			if (!bShouldGenerateUniquePackageName)
			{
				...
				Package = LoadPackage(PackageNameAsString, LOAD_NoWarn);

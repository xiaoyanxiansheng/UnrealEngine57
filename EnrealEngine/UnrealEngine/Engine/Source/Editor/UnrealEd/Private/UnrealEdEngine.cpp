...
#include UE_INLINE_GENERATED_CPP_BY_NAME(UnrealEdEngine)
...
void UUnrealEdEngine::Init(IEngineLoop* InEngineLoop)
{
	...
	if (!IsRunningCommandlet())
	{
		...
		{
			FMessageLog EditorLog("PackagingResults");
			EditorLog.Warning(NSLOCTEXT("UnrealEd", "DisableCookingInEditor=true is required",
				"bDisableCookInEditor=false is deprecated in 5.7; cooks launched from editor must be configured to call an external commandlet. "
				"See comment in EditorExperimentalSettings.h. "
				"Remove your config setting for EditorPerProjectUserSettings:[/Script/UnrealEd.EditorExperimentalSettings]:bDisableCookInEditor. "
				"Support will be removed entirely in a future release and the config setting will no longer be used."));

	...
	, public FGCObject
{
	...
	{ 
		// Only necessary to tick when in editor. However,
		// avoid ticking until the AssetRegistry is done gathering otherwise 
		// we can introduce excessive amounts of contention and cache updates when ticking.
		return GUnrealEd != nullptr && !AssetRegistryModule.Get().IsGathering();
	}
...
public:
	...
	TSAN_ATOMIC(bool) bNeedsRefreshNextTick;
};
	...
	, bNeedsRefreshNextTick(false)
{
	...
	AssetRegistry.OnKnownGathersComplete().AddLambda([](FComponentTypeRegistryData* Data) { Data->Invalidate(); }, this);
...
void FComponentTypeRegistryData::ForceRefreshComponentList()
{
	if (!IsTickable())
	{
		// If we aren't tickable, don't refresh for the same reasons why we don't tick. This early out is 
		// necessary should ForceRefreshComponentList be called explicitly rather than Invalidate().
		return;
	}
...
void FComponentTypeRegistryData::Tick(float)
{
	...
	if (!bRequiresRefresh && PendingAssetData.Num() != 0)
	{

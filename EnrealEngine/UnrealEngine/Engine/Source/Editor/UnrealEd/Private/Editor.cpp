...
FEditorDelegates::FOnWorldGenerationProcess				FEditorDelegates::OnPreWorldGenerationProcess;
FEditorDelegates::FOnWorldGenerationProcess				FEditorDelegates::OnPostWorldGenerationProcess;
...
void FReimportManager::UpdateReimportPath(UObject* Obj, const FString& Filename, int32 SourceFileIndex)
{
	if (Obj)
	{
		...
		{
			//In case no Handler was found with the SourcefileName acquisition,
			// we then try to set the reimportpath 1 by 1 and break out as soon as the the SetReimportPaths then provided a CanReimport state.
			...
			if (SourceFileIndex == INDEX_NONE)
			{
				...
				{
					OutFileNames.Empty();
					Handlers[HandlerIndex]->SetReimportPaths(Obj, FileNames);
					if (Handlers[HandlerIndex]->CanReimport(Obj, OutFileNames))
					{
						break;
					}
				}
			...
			else
			{
				...
				{
					FileNames.Empty();
					Handlers[HandlerIndex]->SetReimportPaths(Obj, Filename, SourceFileIndex);
					if (Handlers[HandlerIndex]->CanReimport(Obj, FileNames))
					{
						break;
					}
				}
...
UE::Interchange::FAssetImportResultRef FReimportManager::ReimportAsync(UObject* Obj, bool bAskForNewFileIfMissing, bool bShowNotification, FString PreferredReimportFile, FReimportHandler* SpecifiedReimportHandler, int32 SourceFileIndex, bool bForceNewFile /*= false*/, bool bAutomated /*= false*/, bool bInForceShowDialog /*= false*/)
{
	...
	if ( Obj )
	{
		...
		auto TryToSetCanReimportHandler = [&]()
			{
				for (int32 HandlerIndex = 0; HandlerIndex < Handlers.Num(); ++HandlerIndex)
				{
					SourceFilenames.Empty();
		...
		{
			TryToSetCanReimportHandler();
		}

		bool bHasSourceFile = false;
		if (CanReimportHandler == nullptr)
		{
			if (const UAssetDefinition* AssetDefinition = UAssetDefinitionRegistry::Get()->GetAssetDefinitionForAsset(Obj))
			{
				...
				AssetDefinition->GetSourceFiles(GetSourceFilesArgs, [&bHasSourceFile](const FAssetSourceFilesResult& AssetImportInfo)
					{
						bHasSourceFile = true;
			...
			if (!bHasSourceFile)
			{
				...
				//If Source file was not found, thenw e force a new file selection here so we can re-check the CanReimportHandler
				for (int32 FileIndex = 0; FileIndex < SourceFilenames.Num(); ++FileIndex)
				{
					GetNewReimportPath(Obj, SourceFilenames, FileIndex);
				}

				UpdateReimportPath(Obj, SourceFilenames[RealSourceFileIndex], RealSourceFileIndex);

				//Also setting the bForceNewFile to false as we already did force it.
				bForceNewFile = false;
				...
				if (bUseInterchangeFramework && CanReimportHandler->IsInterchangeFactory())
				{
					...
					if ( CanReimportHandler->CanReimport(Obj, SourceFilenames) )
					{
						...
						if (InterchangeManager.CanTranslateSourceData(ScopedSourceData.GetSourceData()))
						{
							...
							ImportAssetParameters.bIsAutomated = bAutomated || GIsAutomationTesting || FApp::IsUnattended() || IsRunningCommandlet() || GIsRunningUnattendedScript;
...
void FReimportManager::ValidateAllSourceFileAndReimport(TArray<UObject*> &ToImportObjects, bool bShowNotification, int32 SourceFileIndex, bool bForceNewFile /*= false*/, bool bAutomated /*= false*/)
{
	...
	for (UObject *Asset : ToImportObjects)
	{
		...
		{
			if (const UAssetDefinition* AssetDefinition = UAssetDefinitionRegistry::Get()->GetAssetDefinitionForAsset(Asset))
			{
...
void FReimportManager::GetNewReimportPath(UObject* Obj, TArray<FString>& InOutFilenames, int32 SourceFileIndex /*= INDEX_NONE*/)
{
	...
	if (UInterchangeManager::IsInterchangeImportEnabled())
	{
		...
		{
			//In case the asset was not imported or loose the sources file name, we want to be able to retrieve the possible formats.
			if (const UClass* FactoryClass = UInterchangeManager::GetInterchangeManager().GetRegisteredFactoryClass(Obj->GetClass()))
			{
				UInterchangeFactoryBase* FactoryBase = FactoryClass->GetDefaultObject<UInterchangeFactoryBase>();
				EInterchangeTranslatorAssetType TranslatorAssetType = EInterchangeTranslatorAssetType::None;
				switch (FactoryBase->GetFactoryAssetType())
				{
					case EInterchangeFactoryAssetType::Animations:
						TranslatorAssetType = EInterchangeTranslatorAssetType::Animations;
						break;
					case EInterchangeFactoryAssetType::Materials:
						TranslatorAssetType = EInterchangeTranslatorAssetType::Materials;
						break;
					case EInterchangeFactoryAssetType::Meshes:
					case EInterchangeFactoryAssetType::Physics:
						TranslatorAssetType = EInterchangeTranslatorAssetType::Meshes;
						break;
					case EInterchangeFactoryAssetType::Textures:
						TranslatorAssetType = EInterchangeTranslatorAssetType::Textures;
						break;
					case EInterchangeFactoryAssetType::Grooms:
						TranslatorAssetType = EInterchangeTranslatorAssetType::Grooms;
						break;
				};
				TranslatorFormats = UInterchangeManager::GetInterchangeManager().GetSupportedAssetTypeFormats(TranslatorAssetType, EInterchangeTranslatorType::Assets, false /*bStrictMatchTranslatorType*/);
			}
		}
			...
			if( GEditor->ObjectsThatExistInEditorWorld.Get( SimWorldActor ) )
			{
				...
				for( auto LevelIt( EditorWorld->GetLevelIterator() ); LevelIt; ++LevelIt )
				{
					...
					if( Level->GetFName() == SimWorldActor->GetLevel()->GetFName() )
					{
						...
						AActor* EditorWorldActor = FindObject<AActor>( Level, *SimWorldActor->GetFName().ToString(), EFindObjectFlags::None ); // Don't match class exactly, because we support all classes derived from Actor as well!
						if( EditorWorldActor )
						{
		...
		if( !bIsSimActor && GEditor && GEditor->EditorWorld != NULL )
		{
			...
			if (PlayWorld != nullptr)
			{
				for (auto LevelIt(PlayWorld->GetLevelIterator()); LevelIt; ++LevelIt)
				{
					...
					{
						// Find our counterpart actor
						AActor* SimWorldActor = FindObject<AActor>(Level, *EditorWorldActor->GetFName().ToString(), EFindObjectFlags::None); // Don't match class exactly, because we support all classes derived from Actor as well!

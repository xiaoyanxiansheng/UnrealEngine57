...
#include UE_INLINE_GENERATED_CPP_BY_NAME(FileHelpers)
...
/** A special output device that associates output to the current package being saved and stores said messages to InMessages. */
class FSaveErrorOutputDevice : public FOutputDevice
{
public:
	explicit FSaveErrorOutputDevice(TArray<TSharedRef<FTokenizedMessage>>& InMessages)
		: Messages(InMessages)
	{
	}

	~FSaveErrorOutputDevice()
	{
		checkf(CurrentPackageBeingSaved == nullptr, TEXT("Expected SetCurrentPackageBeingSaved(nullptr) call to clear CurrentPackageBeingSaved after it is saved."));
	}
	...
	virtual void Serialize( const TCHAR* InData, ELogVerbosity::Type Verbosity, const class FName& Category ) override
	{
		if ( Verbosity == ELogVerbosity::Error || Verbosity == ELogVerbosity::Warning )
		{
			...
			Messages.Add(FTokenizedMessage::Create(Severity, FText::FromString(InData)));
		}
		else
		{
			GLog->Serialize(InData, Verbosity, Category);
		}
	}

	virtual void SerializeRecord(const UE::FLogRecord& Record) override
	{
		if (Record.GetVerbosity() == ELogVerbosity::Error || Record.GetVerbosity() == ELogVerbosity::Warning)
		{
	...
	void SetCurrentPackageBeingSaved(const UPackage* Package)
	{
		// FOutputDevice only allows for non-tokenized messages, so add an additional message with a token to link to the package.
		if (CurrentPackageBeingSaved != nullptr && CurrentPackageFirstMessageIndex < Messages.Num())
		{
			int32 Errors = 0;
			int32 Warnings = 0;
			for (int32 Index = CurrentPackageFirstMessageIndex; Index < Messages.Num(); ++Index)
			{
				switch (Messages[Index]->GetSeverity())
				{
			...
			if (Errors > 0 || Warnings > 0)
			{
				...
				Message->AddToken(FAssetNameToken::Create(CurrentPackageBeingSaved->GetName(), FText::FromString(IAssetTools::Get().GetUserFacingLongPackageName(*CurrentPackageBeingSaved))));

				if (Errors > 0 && Warnings > 0)
				{
					Message->AddText(FText::Format(
						LOCTEXT("ErrorsAndWarningsSavingAsset", "{0} {0}|plural(one=error,other=errors), {1} {1}|plural(one=warning,other=warnings) saving asset."),
				...
				{
					Message->AddText(FText::Format(
						LOCTEXT("ErrorsSavingAsset", "{0} {0}|plural(one=error,other=errors) saving asset."),
						FText::AsNumber(Errors)));
				}
				else if (ensure(Warnings > 0))
				{
					Message->AddText(FText::Format(
						LOCTEXT("WarningsSavingAsset", "{0} {0}|plural(one=warning,other=warnings) saving asset."),
						FText::AsNumber(Warnings)));
				}

				Messages.Insert(MoveTemp(Message), CurrentPackageFirstMessageIndex);
			}
		}

		CurrentPackageBeingSaved = Package;
		CurrentPackageFirstMessageIndex = Messages.Num();
	}

private:
	// Holds the errors for the message log.
	TArray<TSharedRef<FTokenizedMessage>>& Messages;
	const UPackage* CurrentPackageBeingSaved = nullptr;
	int32 CurrentPackageFirstMessageIndex = 0;
};

class IScopedSaveOutput
{
public:
	IScopedSaveOutput() = default;
	IScopedSaveOutput(const IScopedSaveOutput&) = delete;
	virtual ~IScopedSaveOutput() = default;

	IScopedSaveOutput& operator=(const IScopedSaveOutput&) = delete;

	virtual FOutputDevice& GetOutputDevice() = 0;

	virtual void SetCurrentPackageBeingSaved(const UPackage* Package) = 0;

	virtual void AddFailedPackage(const UPackage* InFailedPackage) = 0;

	virtual void AddWritablePackage(const UPackage* InWritablePackage) = 0;
};

// An implementation of IScopedSaveOutput that just writes to GLog.
class FScopedAutosaveOutput final : public IScopedSaveOutput
{
public:
	FScopedAutosaveOutput()
		: OutputDevice(*GLog)
	{
	}

	FOutputDevice& GetOutputDevice() override
	{
		return OutputDevice;
	}

	void SetCurrentPackageBeingSaved(const UPackage* Package) override
	{
	}

	void AddFailedPackage(const UPackage* InFailedPackage) override
	{
	}

	void AddWritablePackage(const UPackage* InWritablePackage) override
	{
	}
...
// An implementation of IScopedSaveOutput that coalesces log messages to a single message log page,
// and shows a dialog warning about and packages that weren't saved or were made writable (if bInUseDialog is true).
class FScopedSaveOutput final : public IScopedSaveOutput
{
public:
	explicit FScopedSaveOutput(bool bInUseDialog)
		: bUseDialog(bInUseDialog)
		, Messages()
	...
	~FScopedSaveOutput()
	{
		...
		if (!WritablePackages.IsEmpty() && !FUncontrolledChangelistsModule::Get().IsEnabled())
		{
			TArray<FString> LongPackageNames = IAssetTools::Get().GetUserFacingLongPackageNames(WritablePackages);
			LongPackageNames.Sort();
			LongPackageNames.SetNum(Algo::Unique(LongPackageNames));

			const FText WritableFileWarning = FText::Format(NSLOCTEXT("UnrealEd", "Warning_WritablePackagesNotCheckedOut", "The following assets are writable on disk but not checked out from revision control:\n{0}"),
				FText::FromString(FString::Join(LongPackageNames, TEXT("\n"))));
		...
		if (!FailedPackages.IsEmpty() && bUseDialog)
		{
			TArray<FString> LongPackageNames = IAssetTools::Get().GetUserFacingLongPackageNames(FailedPackages);
			LongPackageNames.Sort();
			LongPackageNames.SetNum(Algo::Unique(LongPackageNames));

			FFormatNamedArguments Arguments;
			Arguments.Add(TEXT("Packages"), FText::FromString(FString::Join(LongPackageNames, TEXT("\n"))));
			FText MessageFormatting = NSLOCTEXT("FileHelper", "FailedSavePromptMessageFormatting", "The following assets failed to save correctly:\n{Packages}");
	...
	void SetCurrentPackageBeingSaved(const UPackage* Package) override
	{
		OutputDevice.SetCurrentPackageBeingSaved(Package);
	}

	void AddFailedPackage(const UPackage* InFailedPackage) override
	{
		FailedPackages.Add(InFailedPackage);
	}

	void AddWritablePackage(const UPackage* InWritablePackage) override
	{
		WritablePackages.Add(InWritablePackage);
	}

private:
	const bool bUseDialog;
	TArray<TSharedRef<FTokenizedMessage>> Messages;
	FSaveErrorOutputDevice OutputDevice;
	TArray<const UPackage*> FailedPackages;
	TArray<const UPackage*> WritablePackages;
};
...
{
	if (ISourceControlModule::Get().GetProvider().GetName() == SourceControlProviders::GetUrcProviderName())
	{
...
static bool InternalCheckoutAndSavePackages(const TArray<UPackage*>& PackagesToSave, IScopedSaveOutput& SaveOutput);
...
// @param	SaveOutput				Object used to consolidate save output to present to the user
// @returns true if all the save operations completed successfully
static bool SaveExternalPackages(UPackage* InPackage, TArray<UPackage*>& InOutPackagesToSave, bool bInNewlyCreated, bool bInAutosaving, IScopedSaveOutput& SaveOutput)
{
	bool bSuccess = true;
	...
	if (!bInAutosaving && (!FEditorFileUtils::ShouldSkipExternalObjectSave() || bInNewlyCreated))
	{
		...
		{
			TArray<UPackage*> Packages = InternalGetValidPackages(InOutPackagesToSave, /*bCheckDirty=*/ !bInNewlyCreated);
			if (!InternalCheckoutAndSavePackages(Packages, SaveOutput))
			{
				bSuccess = false;
			}
		}
...
TArray<UPackage*> InternalGetMapDataPackages(UWorld& WorldToSave)
{
	TArray<UPackage*> MapDataPackages;
		...
		&& !WorldPackage->HasAnyFlags(RF_Transient))
	{
		...
		if (Level->MapBuildData)
		{
			...
			{
				MapDataPackages.Add(BuiltDataPackage);
			}
		}
	}

	return MapDataPackages;
}
					   ...
					   IScopedSaveOutput& SaveOutput)
{
	...
	else
	{
		...
		if (bSuccess)
		{
			...
			SaveOutput.SetCurrentPackageBeingSaved(Package);
			ON_SCOPE_EXIT
			{
				SaveOutput.SetCurrentPackageBeingSaved(nullptr);
			};
		...
		if (bSuccess)
		{
			bSuccess = SaveExternalPackages(Package, PackagesToSave, bNewlyCreated, bAutosaving, SaveOutput);
		}
		...
		{
			TArray<UPackage*> MapDataPackages = InternalGetMapDataPackages(*SaveWorld);
			TArray<UPackage*> Packages = InternalGetValidPackages(MapDataPackages, bCheckDirty);
			if (!InternalCheckoutAndSavePackages(Packages, SaveOutput))
			{
				bSuccess = false;
			}
...
// @param	SaveOutput				Object used to consolidate save output to present to the user
// @returns true if all the save operations completed successfully
static bool SaveAsset(UPackage* InPackage, const FString& PackageName, const FString& FinalPackageSavePath, IScopedSaveOutput& SaveOutput)
{
	TArray<UPackage*> PackagesToSave;
	if (!SaveExternalPackages(InPackage, PackagesToSave, InPackage->HasAnyPackageFlags(PKG_NewlyCreated), false, SaveOutput))
	{
		return false;
	}

	SaveOutput.SetCurrentPackageBeingSaved(InPackage);
	ON_SCOPE_EXIT
	{
		SaveOutput.SetCurrentPackageBeingSaved(nullptr);
	};

	return GEngine->Exec(nullptr, *FString::Printf( TEXT("OBJ SAVEPACKAGE PACKAGE=\"%s\" FILE=\"%s\" SILENT=true"), *PackageName, *FinalPackageSavePath), SaveOutput.GetOutputDevice());
}
	...
	if (CheckOutResult != ECommandResult::Cancelled)
	{
		...
		if (IntermediateResult == ECommandResult::Succeeded)
		{
			...
			if (PkgsToRevert.Num() > 0)
			{
				...
				if (IntermediateResult == ECommandResult::Succeeded)
				{
					// Refetch states that have may have been updated by FRevert
					SourceControlStates.Empty();
					IntermediateResult = SourceControlProvider.GetState(PkgsToCheckOut, SourceControlStates, EStateCacheUsage::Use);
				}
			}
...
ECommandResult::Type FEditorFileUtils::CheckoutPackages(const TArray<FString>& PkgsToCheckOut, TArray<FString>* OutPackagesCheckedOut, const bool bErrorIfAlreadyCheckedOut)
{
	...
	// Source control may have been enabled in the package checkout dialog.
	// Ensure the status is up to date
	if(PkgsToCheckOutFilenames.Num() > 0)
	{
		CheckOutResult = SourceControlProvider.Execute(ISourceControlOperation::Create<FUpdateStatus>(), PkgsToCheckOutFilenames);
	}

	// If any file was marked for delete, revert it now so it can be checked out below
	if(CheckOutResult != ECommandResult::Cancelled)
	{
		TArray<FString> PkgsToRevertFilenames;
		PkgsToRevertFilenames.Reserve(PkgsToCheckOut.Num());
	
		for (const FString& PackageFilename : PkgsToCheckOutFilenames)
		{
			...
			{
				PkgsToRevertFilenames.Add(PackageFilename);
			}
		}
		
		if (PkgsToRevertFilenames.Num() > 0)
		{
			CheckOutResult = SourceControlProvider.Execute(ISourceControlOperation::Create<FRevert>(), PkgsToRevertFilenames);
		}
	}
...
bool FEditorFileUtils::SaveMap(UWorld* InWorld, const FString& Filename )
{
	...
	constexpr bool bUseDialog = true;
	FScopedSaveOutput SaveOutput(bUseDialog);
...
EAutosaveContentPackagesResult::Type FEditorFileUtils::AutosaveMapEx(const FString& AbsoluteAutosaveDir, const int32 AutosaveIndex, const bool bForceIfNotInList, const TSet< TWeakObjectPtr<UPackage>, TWeakObjectPtrSetKeyFuncs<TWeakObjectPtr<UPackage>> >& DirtyPackagesForAutoSave)
{
	...
	FScopedAutosaveOutput SaveOutput;
 ...
 */
static InternalSavePackageResult InternalSavePackage(UPackage* PackageToSave, bool bUseDialog, IScopedSaveOutput& SaveOutput)
{
	...
	bool bPackageLocallyWritable = true;

	ON_SCOPE_EXIT
	{
		if (ReturnCode == InternalSavePackageResult::Continue || ReturnCode == InternalSavePackageResult::Error)
		{
			SaveOutput.AddFailedPackage(PackageToSave);
		}
		if (bPackageLocallyWritable)
		{
			SaveOutput.AddWritablePackage(PackageToSave);
		}
	};
	...
	while( bAttemptSave )
	{
		...
		if (bIsMapPackage && !bSavingRedirectorToWorld)
		{
			...
			SaveOutput.GetOutputDevice().Log("LogFileHelpers", ELogVerbosity::Log, FString::Printf(TEXT("Saving Map: %s"), *PackageName));
			FString FinalFilename;
			bWasSuccessful = SaveWorld(AssociatedWorld, &FinalPackageSavePath,
				nullptr, nullptr,
				true, false,
				FinalFilename,
				false, false,
		...
		else
		{
			...
			SaveOutput.GetOutputDevice().Log("LogFileHelpers", ELogVerbosity::Log, FString::Printf(TEXT("Saving Package: %s"), *PackageName));
		...
		if (!FPackageName::IsTempPackage(PackageName))
		{
			...
			if (ISourceControlModule::Get().IsEnabled())
			{
				...
				{
					bPackageLocallyWritable = true;
				}
			}
			else
			{
				// If we are in offline mode, automatically add the modified package to an Uncontrolled Changelist
				bPackageLocallyWritable = UncontrolledChangelistsModule.IsEnabled() && (!IFileManager::Get().IsReadOnly(*FinalPackageSavePath));
			}

			if (bWasSuccessful && bPackageLocallyWritable)
			{
...
static void InternalNotifyNoPackagesSaved()
{
 ...
 * @param SaveOutput					Object used to consolidate save output to present to the user.
 */
static bool InternalSavePackagesFast(const TArray<UPackage*>& PackagesToSave, bool bUseDialog, IScopedSaveOutput& SaveOutput)
{
	...
	{
		const InternalSavePackageResult SaveStatus = InternalSavePackage(Package, bUseDialog, SaveOutput);
...
static bool InternalSavePackages(const TArray<UPackage*>& PackagesToSave, bool bPromptUserToSave, bool bFastSave, bool bCanBeDeclined, bool bCheckDirty)
{
	...
	else
	{
		...
		bReturnCode = InternalSavePackagesFast(PackagesToSave, bUseDialog, SaveOutput);
	}
	return bReturnCode;
}
...
bool FEditorFileUtils::SaveMapDataPackages(UWorld* WorldToSave, bool bCheckDirty)
{
	...
	if (WorldToSave == nullptr)
	{
		return false;
	}

	TArray<UPackage*> PackagesToSave = InternalGetMapDataPackages(*WorldToSave);
 ...
 */
FEditorFileUtils::EPromptReturnCode InternalPromptForCheckoutAndSave(const TArray<UPackage*>& FinalSaveList, bool bUseDialog, IScopedSaveOutput& SaveOutput, TArray<UPackage*>& OutFailedPackages)
{
	...
	if ( PackagesToSave.Num() > 0 && ReturnResponse == PR_Success )
	{
		...
		if(InOutParams.bAlreadyCheckedOut || (bUserResponse && (PackagesCheckedOutOrMadeWritable.Num() > 0 || PackagesNotNeedingCheckout.Num() > 0)) )
		{
...

			{
				constexpr bool bUseDialog = true;
				FScopedSaveOutput SaveOutput(bUseDialog);
				ReturnResponse = InternalPromptForCheckoutAndSave(FinalSaveList, bUseDialog, SaveOutput, FailedPackages);
			}
		...
		if (PackagesToRevert.Num() > 0)
		{
			...
			InternalPromptForCheckoutAndSave(PackagesToRevert, bUseDialog, SaveOutput, PackagesSaveFailed);
...
bool FEditorFileUtils::IsMapPackageAsset(const FString& ObjectPath, FString& MapFilePath)
{
	...
	if ( PackageName.Len() > 0 )
	{
		...
		if ( FPackageName::DoesPackageExist(PackageName, &PackagePath) )
		{
			...
			if (AssetRegistry.GetAssetsByPackageName(*ObjectPath, Assets))
			{
				for (const FAssetData& Asset : Assets)
				{
					if (Asset.HasAnyPackageFlags(PKG_ContainsMapData))
					{
						MapFilePath = PackagePath;
						return true;
					}
...
static bool InternalCheckoutAndSavePackages(const TArray<UPackage*>& PackagesToSave, IScopedSaveOutput& SaveOutput)
{
	...
	if (PackagesToSave.Num() > 0)
	{
		...
		constexpr bool bUseDialog = false;
		TArray<UPackage*> FailedPackages;
		FEditorFileUtils::EPromptReturnCode ReturnResponse = InternalPromptForCheckoutAndSave(PackagesToSave, bUseDialog, SaveOutput, FailedPackages);
...
static bool InternalCheckoutAndSavePackages(const TArray<UPackage*>& PackagesToSave)
{
	bool bResult = true;
	if (PackagesToSave.Num() > 0)
	{
		constexpr bool bUseDialog = false;
		FScopedSaveOutput SaveOutput(bUseDialog);
		bResult = InternalCheckoutAndSavePackages(PackagesToSave, SaveOutput);
	}
	else
	{
...
static bool InternalCheckoutAndSavePackagesWithDialog(const TArray<UPackage*>& PackagesToSave)
{
	bool bResult = true;
	if (PackagesToSave.Num() > 0)
	{
...
void UEditorLoadingAndSavingUtils::FullyLoadPackages(const TArray<UPackage*>& Packages)
{
	...
	if (PendingPackageRequestIds.Num() > 0)
	{
		FlushAsyncLoading(PendingPackageRequestIds);
	}
	ResetLoaders((const TArray<UObject*>&)Packages);
}

void UEditorLoadingAndSavingUtils::FullyLoadAssets(const TArray<UObject*>& Assets)
{
	TArray<UPackage*> Packages;
	Algo::Transform(Assets, Packages, &UObject::GetPackage);
	FullyLoadPackages(Packages);
}

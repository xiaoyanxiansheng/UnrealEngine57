...
namespace SnappingUtilsLocals
{
	static bool bAlwaysSnapPercentageBasedScaleToGrid = true;
	static FAutoConsoleVariableRef CVarAlwaysSnapPercentageBasedScaleToGrid(
		TEXT("Editor.AlwaysSnapPercentageBasedScale"),
		bAlwaysSnapPercentageBasedScaleToGrid,
		TEXT("When true (default), grid snapping is always used when scaling an object with Percentage based scaling, regardless of snap state."));
}
...
public:
     ...
     *
     * @param DragDelta			The current world space drag amount
     * @param InWorld			The world the user is dragging in
     * @return the location delta to the nearest actor, if any
     */
    TOptional<FVector> GetDeltaToNearestActor( FVector& DragDelta, const UWorld* InWorld );
...
{
	const TOptional<FVector> DeltaToNearestActor = GetDeltaToNearestActor(Drag, ViewportClient->GetWorld());
	const bool bWasSnapped = DeltaToNearestActor.IsSet();
	if (bWasSnapped)
	{
		ViewportClient->ApplyDeltaToActors(DeltaToNearestActor.GetValue(), FRotator::ZeroRotator, FVector::ZeroVector);
	}

	return bWasSnapped;
}

TOptional<FVector> FEditorViewportSnapping::GetDeltaToNearestActor(FVector& Drag, const UWorld* InWorld)
{
	TOptional<FVector> DeltaToNearestActor;
	...
	{		
		...
		if ( Selection->Num() > 0 )
		{
			...
			for (FActorIterator It(InWorld); It; ++It)	// Actor iterator :( [Note: Also, can't use BoxOverlapMulti to as the pivot may lie outside the bounds of the actor!]
			{
			...
			{				
				...
				if ( BestSqrdDist < FMath::Square( Dist )  )
				{
					...
					if ( !Tools.SnappedActor || !Tools.CachedLocation.Equals( BestPoint ) )
					{
						...
						DeltaToNearestActor = PivotDelta;
	...
	return DeltaToNearestActor;	// Returns the location delta to the nearest actor, if snapped
}
...
bool FEditorViewportSnapping::SnapToBSPVertex(FVector& Location, FVector GridBase, FRotator& Rotation)
{
	...
	if( IsSnapToVertexEnabled() )
	{
		...
		FVector3f	DestPoint;
		int32 Temp;
		if( GWorld && GWorld->GetModel() && GWorld->GetModel()->FindNearestVertex(SrcPoint, DestPoint, GetDefault<ULevelEditorViewportSettings>()->SnapDistance, Temp ) >= 0.0)
		{
...
TOptional<FVector> FSnappingUtils::GetDeltaToNearestActor(FVector& DragDelta, const UWorld* InWorld)
{
	return EditorViewportSnapper.IsValid() ? EditorViewportSnapper->GetDeltaToNearestActor(DragDelta, InWorld) : TOptional<FVector>();
}
...
void FSnappingUtils::AdjustPercentageBasedScale(FVector& InOutScale)
{
	// If we are scaling, we need to change the scaling factor a bit to properly align to grid
	if (!InOutScale.IsNearlyZero() && GEditor->UsePercentageBasedScaling())
	{
		// In both cases, the scale is assumed to be in the 0-100 range and is adjusted to 0-1
		if (SnappingUtilsLocals::bAlwaysSnapPercentageBasedScaleToGrid)
		{
			InOutScale *= ((GEditor->GetScaleGridSize() / 100.0f) / GEditor->GetGridSize());
		}
		else
		{
			InOutScale /= 100.0f;

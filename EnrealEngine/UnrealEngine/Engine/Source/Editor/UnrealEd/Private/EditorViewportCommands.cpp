...
UE_DEFINE_TCOMMANDS(FEditorViewportCommands)
...
void FEditorViewportCommands::RegisterCommands()
{
	...
	UI_COMMAND( GeometryInspectionClayMode, "Geometry Inspection Clay Mode", "Displays using clay material to evaluate surface curvature and smoothness.", EUserInterfaceActionType::RadioButton, FInputChord());
	UI_COMMAND( GeometryInspectionZebraHorizontalMode, "Horizontal", "Displays simulated light reflections using stripes to evaluate surface curvature and smoothness.", EUserInterfaceActionType::RadioButton, FInputChord());
	UI_COMMAND( GeometryInspectionZebraVerticalMode, "Vertical", "Displays simulated light reflections using stripes to evaluate surface curvature and smoothness.", EUserInterfaceActionType::RadioButton, FInputChord());
	UI_COMMAND( GeometryInspectionFrontBackFaceMode, "Geometry Inspection Front/Back Face Mode", "Highlights front-facing and back-facing polygons differently to evaluate face normals.", EUserInterfaceActionType::RadioButton, FInputChord());
	UI_COMMAND( GeometryInspectionRandomColorMode, "Geometry Inspection Random Color Mode", "Assigns random colors to individual elements or components to improve visual distinction between overlapping or adjacent parts.", EUserInterfaceActionType::RadioButton, FInputChord());
	UI_COMMAND( GeometryInspectionShowWireframe, "Use Wireframe", "Use Wireframe in Geometry Inspection Modes.", EUserInterfaceActionType::Check, FInputChord());
	...
	UI_COMMAND( FocusAndClipViewportToSelection, "Frame & Clip To Selected", "Centers the viewport on the selected actors and clips the near view plane to them", EUserInterfaceActionType::Button, FInputChord(EKeys::F, EModifierKey::Control | EModifierKey::Alt) );
	...
	UI_COMMAND( UseOrthographicClippingPlanes, "Ortho Clipping Planes", "Whether or not the depth of the orthographic view should be clipped.", EUserInterfaceActionType::Check, FInputChord() );
	UI_COMMAND( DismissOrthographicClippingPlanes, "Dismiss Ortho Clipping Planes", "Resets the orthographic clipping planes and hides the visualization of viewport depth.", EUserInterfaceActionType::Button, FInputChord() );
...
{
	check(MaterialInterface);
	const FMaterialResource* Material = MaterialInterface->GetMaterialResource(GetFeatureLevelShaderPlatform_Checked(FeatureLevel));
	if (Material)
	{

...
void FViewportConfigOptions::SetToDefault()
{
	...
	ViewFOV = 53.43f;
	CameraSpeedSettings = FEditorViewportCameraSpeedSettings();
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	CameraSpeedSetting_DEPRECATED = 4;
	CameraSpeedScalar_DEPRECATED = 1.0f;
...
void UPersonaOptions::SetCameraSpeedSettings(FName InContext, const FEditorViewportCameraSpeedSettings& SpeedSettings, int32 InViewportIndex)
{
	check(InViewportIndex >= 0 && InViewportIndex < 4);

	FAssetEditorOptions& Options = GetAssetEditorOptions(InContext);
	Options.ViewportConfigs[InViewportIndex].CameraSpeedSettings = SpeedSettings;
	SaveConfig();
}
...
void UPersonaOptions::SetCameraSpeed(FName InContext, int32 InCameraSpeed, int32 InViewportIndex)
{
	...
	FAssetEditorOptions& Options = GetAssetEditorOptions(InContext);
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	Options.ViewportConfigs[InViewportIndex].CameraSpeedSetting_DEPRECATED = InCameraSpeed;
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	SaveConfig();
}
...
void UPersonaOptions::SetCameraSpeedScalar(FName InContext, float InCameraSpeedScalar, int32 InViewportIndex)
{
	...
	FAssetEditorOptions& Options = GetAssetEditorOptions(InContext);
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	Options.ViewportConfigs[InViewportIndex].CameraSpeedScalar_DEPRECATED = InCameraSpeedScalar;
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	SaveConfig();
}

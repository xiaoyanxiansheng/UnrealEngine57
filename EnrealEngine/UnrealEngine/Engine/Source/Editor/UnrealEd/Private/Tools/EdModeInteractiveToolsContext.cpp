...
void UModeManagerInteractiveToolsContext::DrawHUD(FViewportClient* ViewportClient, FViewport* Viewport, const FSceneView* View, FCanvas* Canvas)
{
	...
	{
		DragToolsBehaviorSource->RenderTools(ViewportClient, View, Canvas);
	}
}
...
void UModeManagerInteractiveToolsContext::Initialize(IToolsContextQueriesAPI* QueriesAPIIn, IToolsContextTransactionsAPI* TransactionsAPIIn)
{
	...
	// Delegates used to handle DragTools.EnableITFTools CVAR changes
	...
	UE::Editor::ViewportInteractions::OnEditorViewportInteractionsActivated().AddUObject(this, &UModeManagerInteractiveToolsContext::RegisterViewportInteractions);
	UE::Editor::ViewportInteractions::OnEditorViewportInteractionsDeactivated().AddUObject(this, &UModeManagerInteractiveToolsContext::UnregisterViewportInteractions);
}
...
{
	if (bInBypassAltModifier
		&& (UE::Editor::ViewportInteractions::UseEditorViewportInteractions() || UE::Editor::DragTools::UseEditorDragTools()))
	{
		return;
	}
...
{
	if (!ViewportInteractionsBehaviorSource && UsesViewportInteractions() && UE::Editor::ViewportInteractions::UseEditorViewportInteractions())
	{
...
void UModeManagerInteractiveToolsContext::RegisterDragTools()
{
	...
	if (!DragToolsBehaviorSource)
	{
		...
		
		{
        	...
        	UToolMenu* TransformMenu = UToolMenus::Get()->ExtendMenu("LevelEditor.ViewportToolbar.Transform");
			FToolMenuSection& SelectionSection = TransformMenu->FindOrAddSection("Selection");
			
			FToolMenuEntry& MarqueeMode = SelectionSection.AddSubMenu(
				"StrictBoxSelect", // Overwrite the other setting
				LOCTEXT("MarqueeSelectionLabel", "Marquee Selection"),
				LOCTEXT("MarqueeSelectionTooltip", "Define how objects are selected by marquee selection"),
				FNewToolMenuDelegate::CreateLambda([](UToolMenu* Menu)
				{
					FProperty* Property = ULevelEditorViewportSettings::StaticClass()->FindPropertyByName(GET_MEMBER_NAME_CHECKED(ULevelEditorViewportSettings, MarqueeSelectionMode));
					if (!Property || !Property->IsA<FEnumProperty>())
					{
						return;
					}
					
					FEnumProperty* EnumProperty = StaticCast<FEnumProperty*>(Property);
					...
					TWeakObjectPtr<UEnum> WeakEnum = Enum;
					
					FToolMenuSection& Section = Menu->AddSection("Mode", LOCTEXT("MarqueeModeSection", "Mode"));
					
					const int32 MaxIndex = Enum->NumEnums() - (Enum->ContainsExistingMax() ? 1 : 0);
					for (int32 Index = 0; Index < MaxIndex; Index++)
					{
						Section.AddMenuEntry(
							...
							FSlateIcon(),
							FUIAction(
								FExecuteAction::CreateLambda([WeakEnum, Index]
								{
									if (UEnum* Enum = WeakEnum.Get())
									{
										ULevelEditorViewportSettings* Settings = GetMutableDefault<ULevelEditorViewportSettings>();
										Settings->MarqueeSelectionMode = static_cast<EMarqueeSelectionMode>(Enum->GetValueByIndex(Index));
										Settings->SaveConfig();
									}
								}),
								FCanExecuteAction(),
								FIsActionChecked::CreateLambda([WeakEnum, Index]
								{
									if (UEnum* Enum = WeakEnum.Get())
									{
										const ULevelEditorViewportSettings* Settings = GetDefault<ULevelEditorViewportSettings>();
										return Settings->MarqueeSelectionMode == static_cast<EMarqueeSelectionMode>(Enum->GetValueByIndex(Index));
									}
									return false;
								})
							),
			...
			MarqueeMode.Icon = FSlateIcon(FAppStyle::GetAppStyleSetName(), "LevelEditor.StrictBoxSelect");
			MarqueeMode.InsertPosition = FToolMenuInsert("TransparentBoxSelect", EToolMenuInsertType::Before);
		}
	}
}
...
void UModeManagerInteractiveToolsContext::RegisterViewportInteractions()
{
	...
	// This Mode Manager does not require Input Tools
...
void UModeManagerInteractiveToolsContext::OnEditorModeChanged(const FEditorModeID& InModeID, bool bInIsEnteringMode)
{
	...
	if (UE::Editor::ViewportInteractions::UseEditorViewportInteractions())
	{
		if (bInIsEnteringMode)
		{
			RegisterViewportInteractions();

...
#include UE_INLINE_GENERATED_CPP_BY_NAME(LegacyEdModeWidgetHelpers)
...
const FName FLegacyEdModeWidgetHelper::MD_WidgetLocalSpace(TEXT("LocalSpaceEditWidget"));
...
struct HPropertyWidgetProxy : public HHitProxy
{
	...
	/** This property is in local space */
	bool	bLocalSpace;

	HPropertyWidgetProxy(FString InPropertyName, int32 InPropertyIndex, bool bInPropertyIsTransform, bool bInLocalSpace = true)
...
static bool IsLocalSpace(const FProperty* InProp)
{
	return !InProp->HasMetaData(FEdMode::MD_WidgetLocalSpace)
		|| InProp->GetMetaData(FEdMode::MD_WidgetLocalSpace).IsEmpty()
		|| InProp->GetBoolMetaData(FEdMode::MD_WidgetLocalSpace);
}
...
struct FPropertyWidgetInfoChainElement
{
	...
	static FLegacyEdModeWidgetHelper::FPropertyWidgetInfo CreateWidgetInfo(const TArray<FPropertyWidgetInfoChainElement>& Chain, bool bIsTransform, bool bIsLocalSpace, FProperty* CurrentProp, int32 Index = INDEX_NONE)
	{
		check(CurrentProp);
		...
		WidgetInfo.bLocalSpace = bIsLocalSpace;
		...
		for (TFieldIterator<FProperty> PropertyIt(InStruct, EFieldIteratorFlags::IncludeSuper); PropertyIt; ++PropertyIt)
		{
			...
			if (EdMode.ShouldCreateWidgetForProperty(CurrentProp))
			{
				if (FArrayProperty* ArrayProp = CastField<FArrayProperty>(CurrentProp))
				{
					...
					{
						OutInfos.Add(FPropertyWidgetInfoChainElement::CreateWidgetInfo(Chain, IsTransformProperty(ArrayProp->Inner), IsLocalSpace(ArrayProp->Inner), CurrentProp, Index));
					}
				}
				else
				{
					OutInfos.Add(FPropertyWidgetInfoChainElement::CreateWidgetInfo(Chain, IsTransformProperty(CurrentProp), IsLocalSpace(CurrentProp), CurrentProp));
				}
			}
...
FVector FLegacyEdModeWidgetHelper::GetWidgetLocation() const
{
	if (UsesPropertyWidgets())
	{
		...
		if (BestSelectedItem)
		{
			...
			{
				FVector PropertyPosition = FVector::ZeroVector;

				if (bEditedPropertyIsTransform)
				{
					FTransform PropertyTransform = GetPropertyValueByName<FTransform>(BestSelectedItem, EditedPropertyName, EditedPropertyIndex);
					PropertyPosition = PropertyTransform.GetLocation();
				}
				else
				{
					PropertyPosition = GetPropertyValueByName<FVector>(BestSelectedItem, EditedPropertyName, EditedPropertyIndex);
				}
				...
				const FVector WorldPos = DisplayWidgetToWorld.TransformPosition(PropertyPosition);
				return WorldPos;
			}
		}
	}
	...
	if (UsesPropertyWidgets())
	{
		...
		if ((BestSelectedItem != nullptr) && (InViewportClient->GetCurrentWidgetAxis() != EAxisList::None))
		{
			...
			if (!EditedPropertyName.IsEmpty())
			{
				...
				if (bEditedPropertyIsTransform)
				{
					PropertyTransform = GetPropertyValueByName<FTransform>(BestSelectedItem, EditedPropertyName, EditedPropertyIndex);

				}
				else
				{
					FVector PropertyTranslation = GetPropertyValueByName<FVector>(BestSelectedItem, EditedPropertyName, EditedPropertyIndex);
					PropertyTransform = FTransform(PropertyTranslation);
				}

				// Convert local transform into world space
				if (bLocalSpace)
				{
					PropertyTransform = PropertyTransform * DisplayWidgetToWorld;
				}

				// Apply delta in world space
				PropertyTransform.SetTranslation(PropertyTransform.GetTranslation() + InDrag);
				PropertyTransform.SetRotation(InRot.Quaternion() * PropertyTransform.GetRotation());
				// Apply delta scale
				PropertyTransform.SetScale3D(PropertyTransform.GetScale3D() + InScale);

				// Convert world transform back into local space
				if (bLocalSpace)
				{
					PropertyTransform = PropertyTransform.GetRelativeTransform(DisplayWidgetToWorld);
				}
				BestSelectedItem->PreEditChange(nullptr);
				// Property that we actually change
				FProperty* SetProperty = nullptr;
				if (bEditedPropertyIsTransform)
				{
					SetPropertyValueByName<FTransform>(BestSelectedItem, EditedPropertyName, EditedPropertyIndex, PropertyTransform, SetProperty);
				}
				else
				{
					SetPropertyValueByName<FVector>(BestSelectedItem, EditedPropertyName, EditedPropertyIndex, PropertyTransform.GetLocation(), SetProperty);
				}
...
bool FLegacyEdModeWidgetHelper::HandleClick(FEditorViewportClient* InViewportClient, HHitProxy* HitProxy, const FViewportClick& Click)
{
	if (UsesPropertyWidgets() && (HitProxy != nullptr))
	{
		if (HitProxy->IsA(HPropertyWidgetProxy::StaticGetType()))
		{
			...
			bLocalSpace = PropertyProxy->bLocalSpace;
			return true;
		}
	...
	if (UsesPropertyWidgets())
	{
		...
		if (BestSelectedItem != nullptr)
		{
			...
			for (const FPropertyWidgetInfo& WidgetInfo : WidgetInfos)
			{
				...
				FTransform WorldWidgetTransform = LocalWidgetTransform;
				if (WidgetInfo.bLocalSpace)
				{
					WorldWidgetTransform = LocalWidgetTransform * DisplayWidgetToWorld;
				}
				...
				if (bHitTesting) PDI->SetHitProxy(new HPropertyWidgetProxy(WidgetInfo.PropertyName, WidgetInfo.PropertyIndex, WidgetInfo.bIsTransform, WidgetInfo.bLocalSpace));

...
struct FGarbageEliminationScopeGuard
{
	bool PreviousValue;

	FGarbageEliminationScopeGuard(bool bNewValue)
		: PreviousValue(UObjectBaseUtility::IsGarbageEliminationEnabled())
	{
		UObjectBaseUtility::SetGarbageEliminationEnabled(bNewValue);
	}

	~FGarbageEliminationScopeGuard()
	{
		UObjectBaseUtility::SetGarbageEliminationEnabled(PreviousValue);
	}
};
...
bool UUnrealEdEngine::DeleteComponents(const TArray<UActorComponent*>& InComponentsToDelete, UTypedElementSelectionSet* InSelectionSet, const bool bVerifyDeletionCanHappen)
{
	...
	if (NumDeletedComponents > 0)
	{
		...
		{
			FGarbageEliminationScopeGuard Guard(true);
...
void UUnrealEdEngine::GetChildHierarchyOfSelectedActor(TArray<AActor*>& OutActorArray, AActor* ParentActor, bool bExcludeSelectedChildren, const TOptional<bool>& bIsFindingHidden)
{
	// If the ParentActor is a builder brush or doesn't match the find flag (if it is set), then don't
	// add it to the array or continue searching the hierarchy
	if ( !FActorEditorUtils::IsABuilderBrush(ParentActor) )
	{
		// If the actor doesn't match the bIsFindingHidden, we still want to search its children in case they match
		if( !bIsFindingHidden.IsSet() || ParentActor->IsHiddenEd() == bIsFindingHidden.GetValue() )
		{
			OutActorArray.Add( ParentActor );
		}

		TArray<AActor*> ActorChildren;
		ParentActor->GetAttachedActors(ActorChildren, false, false);
		for (AActor* Child : ActorChildren)
		{
			if (!bExcludeSelectedChildren || !GEditor->GetSelectedActors()->IsSelected(Child))
			{
				GetChildHierarchyOfSelectedActor(OutActorArray, Child, bExcludeSelectedChildren, bIsFindingHidden);
			}
		}
	}
}

void UUnrealEdEngine::Internal_HideSelected( UWorld* InWorld, bool bApplyToChildHierarchy )
{
	...
	for ( FSelectionIterator It( GetSelectedActorIterator() ) ; It ; ++It )
	{
		...
		if ( bApplyToChildHierarchy )
		{
			GetChildHierarchyOfSelectedActor(ActorsToHide, Actor, true, false);
		}
		else if ( !FActorEditorUtils::IsABuilderBrush(Actor) && !Actor->IsHiddenEd() )
		{
...
{
	Internal_HideSelected(InWorld, false);
}

void UUnrealEdEngine::edactHideSelectedHierarchy( UWorld* InWorld )
{
	Internal_HideSelected(InWorld, true);
}
...
void UUnrealEdEngine::Internal_ToggleSelectedVisibility(UWorld* InWorld, bool bApplyToChildHierarchy)
{
	// Assemble a list of actors to operate on.
	...
	for ( FSelectionIterator It( GetSelectedActorIterator() ) ; It ; ++It )
	{
		...
		if (!FActorEditorUtils::IsABuilderBrush(Actor))
		{
			if ( bApplyToChildHierarchy )
			{
				GetChildHierarchyOfSelectedActor(ActorsToConsider, Actor, true);
			}
			else
			{
				ActorsToConsider.Add( Actor );
			}
			bTargetVisibility = bTargetVisibility && Actor->IsHiddenEd();
		}
	}
	
	// Check BSP to confirm their visibility
	if ( InWorld )
	{
		for ( TArray<ULevel*>::TConstIterator LevelIterator = InWorld->GetLevels().CreateConstIterator(); LevelIterator; ++LevelIterator )
		{
			...
			for ( TArray<FBspSurf>::TIterator SurfaceIterator( CurLevelModel.Surfs ); SurfaceIterator; ++SurfaceIterator )
			{
				...
				{
					bTargetVisibility = bTargetVisibility && CurSurface.IsHiddenEd();
				}
			}
		}
	}
	
	// Apply the target visibility to the target actors
	if ( ActorsToConsider.Num() > 0 )
	{
		for( int32 ActorIndex = 0 ; ActorIndex < ActorsToConsider.Num() ; ++ActorIndex )
		{
			AActor* Actor = ActorsToConsider[ ActorIndex ];
			if (Actor->IsHiddenEd() != !bTargetVisibility)
			{
				SetActorVisibility(Actor, bTargetVisibility);
			}
		}
	}
	
	// Apply the target visibility to BSP
	if ( InWorld )
	{
		for ( TArray<ULevel*>::TConstIterator LevelIterator = InWorld->GetLevels().CreateConstIterator(); LevelIterator; ++LevelIterator )
		{
			...
			for ( TArray<FBspSurf>::TIterator SurfaceIterator( CurLevelModel.Surfs ); SurfaceIterator; ++SurfaceIterator )
			{
				...
				{
					CurLevelModel.ModifySurf( SurfaceIterator.GetIndex(), false );
					CurSurface.bHiddenEdTemporary = !bTargetVisibility;
				}
			}
		}
	}
...
void UUnrealEdEngine::edactToggleSelectedVisibility(UWorld* InWorld)
{
	Internal_ToggleSelectedVisibility(InWorld, false);
}

void UUnrealEdEngine::edactToggleSelectedHierarchyVisibility(UWorld* InWorld)
{
	Internal_ToggleSelectedVisibility(InWorld, true);
}
...
void UUnrealEdEngine::Internal_HideSelectedStartup( UWorld* InWorld, bool bApplyToChildHierarchy )
{
	// Fires ULevel::LevelDirtiedEvent when falling out of scope.
	FScopedLevelDirtied LevelDirtyCallback;

	TArray<AActor*> ActorsToHideAtStartup;
	for ( FSelectionIterator It( GetSelectedActorIterator() ) ; It ; ++It )
	{
		AActor* Actor = static_cast<AActor*>( *It );
		checkSlow( Actor->IsA(AActor::StaticClass()) );
		...
		if ( bApplyToChildHierarchy )
		{
			GetChildHierarchyOfSelectedActor(ActorsToHideAtStartup, Actor, true);
		}
		else if ( !FActorEditorUtils::IsABuilderBrush(Actor) )
		{
			ActorsToHideAtStartup.Add( Actor );
		}
	}

	for (AActor* Actor : ActorsToHideAtStartup)
	{
		// Set the actor to hide at editor startup, if it's not already set that way
		if ( !Actor->IsHiddenEdAtStartup() )
		{
			Actor->Modify();
...
{
	Internal_HideSelectedStartup(InWorld, false);
}

void UUnrealEdEngine::edactHideSelectedHierarchyStartup( UWorld* InWorld )
{
	Internal_HideSelectedStartup(InWorld, true);
}
...
void UUnrealEdEngine::Internal_UnHideSelectedStartup( UWorld* InWorld, bool bApplyToChildHierarchy )
{
	// Fires ULevel::LevelDirtiedEvent when falling out of scope.
	FScopedLevelDirtied LevelDirtyCallback;

	TArray<AActor*> ActorsToShowAtStartup;
	for ( FSelectionIterator It( GetSelectedActorIterator() ) ; It ; ++It )
	{
		AActor* Actor = static_cast<AActor*>( *It );
		checkSlow( Actor->IsA(AActor::StaticClass()) );
		...
		if( bApplyToChildHierarchy )
		{
			GetChildHierarchyOfSelectedActor(ActorsToShowAtStartup, Actor, true);
		}
		else if ( !FActorEditorUtils::IsABuilderBrush(Actor) )
		{
			ActorsToShowAtStartup.Add( Actor );
		}
	}

	for (AActor* Actor : ActorsToShowAtStartup)
	{
		// Set the actor to show at editor startup, if it's not already set that way
		if ( Actor->IsHiddenEdAtStartup() )
		{
			Actor->Modify();
			Actor->bHiddenEd = false;
...
{
	Internal_UnHideSelectedStartup(InWorld, false);
}

void UUnrealEdEngine::edactUnHideSelectedHierarchyStartup( UWorld* InWorld )
{
	Internal_UnHideSelectedStartup(InWorld, true);
}

void UUnrealEdEngine::Internal_UnhideSelected(UWorld* InWorld, bool bApplyToChildHierarchy)
{
	// Assemble a list of actors to show.
	...
	for ( FSelectionIterator It( GetSelectedActorIterator() ) ; It ; ++It )
	{
		...
		if( bApplyToChildHierarchy )
		{
			GetChildHierarchyOfSelectedActor(ActorsToShow, Actor, true, true);
		}
		else if ( !FActorEditorUtils::IsABuilderBrush(Actor) && Actor->IsHiddenEd() )
		{
...
{
	Internal_UnhideSelected(InWorld, false);
}

void UUnrealEdEngine::edactUnhideSelectedHierarchy( UWorld* InWorld )
{
	Internal_UnhideSelected(InWorld, true);
}

...
namespace ObjectTools
{
	...
	FConsolidationResults ConsolidateObjects(TArrayView<FReplaceRequest> Requests, TSet<UObject*>& ObjectsToConsolidateWithin, TSet<UObject*>& ObjectsToNotConsolidateWithin, bool bShouldDeleteAfterConsolidate, bool bWarnAboutRootSet)
	{
		...
		if ( CriticalFailureObjects.Num() > 0 )
		{
			...
			for (const FString& UserFacingPath : IAssetTools::Get().GetUserFacingLongPackageNames(DirtiedPackages))
			{
				DirtiedPackageNames += UserFacingPath + TEXT("\n");
			}
	...
	void ShowReferencedObjs( UObject* Object, ICollectionContainer* CollectionContainer, FName CollectionName, ECollectionShareType::Type ShareType )
	{
		if( Object )
		{
			...
			TSet<UObject*> ReferencedObjects;
			{
				...
				{
					UPackage* PackageToIgnore = FindObject<UPackage>(NULL,*(IgnorePackageNames[PackageNameItr]),EFindObjectFlags::ExactClass);
	...
	int32 DeleteObjects( const TArray< UObject* >& InObjectsToDelete, bool bShowConfirmation, EAllowCancelDuringDelete AllowCancelDuringDelete )
	{
		...
		if ( bShowConfirmation )
		{
			...
			TSharedRef<SCustomDialog> DeleteAssetsWindow = SNew(SCustomDialog)
			...
			DeleteAssetsWindow->SetOnCancelHotkeyPressed(FExecuteAction::CreateSP(DeleteDialog, &SDeleteAssetsDialog::OnCancelHotkeyPressed));
			DeleteAssetsWindow->SetOnConfirmHotkeyPressed(FExecuteAction::CreateSP(DeleteDialog, &SDeleteAssetsDialog::OnConfirmHotkeyPressed));
			DeleteAssetsWindow->SetGetWidgetToFocusOnActivate(FGetFocusWidget::CreateSP(DeleteDialog, &SDeleteAssetsDialog::GetWidgetToFocusOnActivate));
				...
				for(TWeakObjectPtr<UObject>& Object : ObjectsToDelete)
				{
					if(Object.IsValid())
					{
						...
						if (BlueprintObject)
						{
							...
							// Also include the skeleton class, in case it is referenced
							if (BlueprintObject->SkeletonGeneratedClass)
							{
								ObjectsToReplace.AddUnique(BlueprintObject->SkeletonGeneratedClass);
							}
	...
	bool RenameSingleObject(UObject* Object, FPackageGroupName& PGN, TSet<UPackage*>& InOutPackagesUserRefusedToFullyLoad, FText& InOutErrorMessage, const TMap< UObject*, FString >* ObjectToLanguageExtMap, bool bLeaveRedirector)
	{
		...
		if ( !bMoveFailed )
		{
			...
			if (OldPackage)
			{
				...
				// Allow external systems to deal with metadata
				UPackage::OnMetaDataTransferRequestedDelegate.Broadcast(OldPackage->GetMetaData(), NewPackage->GetMetaData(), OldPackage, NewPackage);
			}
		...
		else
		{
			if(bMoveRedirectorFailed)
			{
				ErrorMessage += FText::Format( NSLOCTEXT("UnrealEd", "Error_CouldntRenameObjectRedirectorF", "Couldn't rename '{0}' object because there is an object redirector of the same name, please fixup redirect from editor by enabling Show Redirects in content browser.\n"),
					FText::FromString(IAssetTools::Get().GetUserFacingFullName(*Object)) ).ToString();
			}
			else
			{
	...
	bool IsAssetValidForPlacing(UWorld* InWorld, const FString& ObjectPath)
	{
		...
		if ( bResult )
		{
			...
			if ( !bResult )
			{
				...
				{
					if ( UPackage* AssetPackage = FindObjectSafe<UPackage>(nullptr, *AssetPackageName, EFindObjectFlags::ExactClass) )
					{
						...
						for ( int32 WorldIndex = 0; WorldIndex < CurrentMapWorlds.Num(); WorldIndex++ )
						{
							...
							if ( World && World->GetOutermost() == AssetPackage )
							{
								bResult = true;
								break;
							}
	...
	FString GetDefaultTooltipForFunction(const UFunction* Function, bool bIncludeTarget)
	{
		if (!ensure(Function))
		{
			return FString();
		...
		if (!Tooltip.IsEmpty())
		{
			...
			if (bIncludeTarget)
			{
				...
				Tooltip = FText::Format(LOCTEXT("CallFunction_Tooltip", "{Tooltip}\n\nTarget is {TargetName}"), Args).ToString();
			}
		}
		else if (bIncludeTarget)
		{
			Tooltip = GetUserFacingFunctionName(Function).ToString();
		}
		
		return Tooltip;
	}

	FString GetDefaultTooltipForFunctionParamOrReturn(const UFunction* Function, const FString& ParamName, const bool bReturnValue)
	{
		if (!ensure(Function))
		{
			return FString();
		...
		const FString TagStr = bReturnValue ? TEXT("@return") : TEXT("@param");
		
		// grab the function's comment block for us to parse
		...
		do 
		{
			...
			if (bReturnValue && CurStrPos < FullToolTipLen && FunctionToolTipText[CurStrPos] == TEXT('s'))
			{
				++CurStrPos;
			}
			...
			{
				// we found a match, so there's no need to continue
				return ParamDesc;
			}
	...
	FString GetDefaultTooltipForFunctionParam(const UFunction* Function, const FString& ParamName)
	{
		return GetDefaultTooltipForFunctionParamOrReturn(Function, ParamName, /*bReturnValue*/false);
	}
	
	FString GetDefaultTooltipForFunctionReturn(const UFunction* Function)
	{
		return GetDefaultTooltipForFunctionParamOrReturn(Function, FString(), /*bReturnValue*/true);
	}
}

...
SIZE_T FTransaction::DataSize() const
{
	...
	for( int32 i=0; i<Records.Num(); i++ )
	{
		const FObjectRecord& Record = Records[i];
		const SIZE_T CustomChangeSize = Record.CustomChange ? Record.CustomChange->GetSize() : 0;
		Result += Record.SerializedObject.SerializedData.Num() + CustomChangeSize;
	}
	return Result;
}

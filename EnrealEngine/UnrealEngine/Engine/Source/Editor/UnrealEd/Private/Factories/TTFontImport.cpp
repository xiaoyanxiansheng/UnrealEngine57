...
namespace
{
	...
	int32 RoundUpToMultipleOf4(int32 X)
	{
		return (X + 3) & ~3;
	}
}
	...
	FFeedbackContext*	Warn )
{
	...
	ImportOptions->Data.TexturePageWidth = RoundUpToMultipleOf4(ImportOptions->Data.TexturePageWidth);
	ImportOptions->Data.TexturePageMaxHeight = RoundUpToMultipleOf4(ImportOptions->Data.TexturePageMaxHeight);
...
void FTextureAlphaToDistanceField::Generate(int32 ScaleFactor, int32 ScanRadius)
{
	...
	// We want to run the distance field computation as a maximum of 16 tasks for a speed boost on multi-core machines.
	const int32 NumTasks = FMath::Min(DstSizeY, 16); //ensure we don't split the work so much that we end up with a null DstStripHeight... and an infinite loop.
	FThreadSafeCounter BuildTasksCounter;
	int32 DstStripHeight = DstSizeY / NumTasks;
	...
	{
		DstStripHeight = FMath::Min(DstStripHeight, DstSizeY - y); //Don't go out of bound.
		...
		(new FAutoDeleteAsyncTask<FBuildDistanceFieldTask>(
			...
			DstStripHeight,
			ScanRadius,
			ScaleFactor))->StartBackgroundTask();
	}

	while( BuildTasksCounter.GetValue() > 0 )
	{
		// Waiting for Distance Field to finish generating.
		GWarn->UpdateProgress(TotalDistFieldWork - BuildTasksCounter.GetValue(),TotalDistFieldWork);
...
UTexture2D* UTrueTypeFontFactory::CreateTextureFromDC( UFont* Font, HDC dc, const int32 BitmapWidth, const int32 BitmapHeight, int32 TextureNum )
{
	...
	const TArray< float >& ResHeights )
{
	...
	for( int32 Page = 0; Page < NumResolutions; ++Page )
	{
		...
		const int32 ActualDistanceFieldScaleFactor = ImportOptions->Data.bUseDistanceFieldAlpha ? ImportOptions->Data.DistanceFieldScaleFactor : 1;
		const int32 BitmapWidth = ImportOptions->Data.TexturePageWidth * ActualDistanceFieldScaleFactor;
		const int32 BitmapHeight = ImportOptions->Data.TexturePageMaxHeight * ActualDistanceFieldScaleFactor;
		const int32 BitmapPaddingX = ImportOptions->Data.XPadding * ActualDistanceFieldScaleFactor;
		const int32 BitmapPaddingY = ImportOptions->Data.YPadding * ActualDistanceFieldScaleFactor;
		...
		for( int32 CurCharIndex = 0; CurCharIndex < CharsPerPage; ++CurCharIndex )
		{
			...
			const int32 RequiredWidth = CharWidth + BitmapPaddingX * 2;
			const int32 RequiredHeight = CharHeight + BitmapPaddingY * 2;
			const bool bisBitmapTooNarrow = RequiredWidth > BitmapWidth;
			const bool bisBitmapTooShort = RequiredHeight > BitmapHeight;
			if (bisBitmapTooNarrow || bisBitmapTooShort)
			{
				UE_LOG(LogTTFontImport, Error, TEXT("At the specified font size, at least one font glyph would be larger than the maximum texture size you specified."));
				if (bisBitmapTooNarrow)
				{
					UE_LOG(LogTTFontImport, Error, TEXT("You should increase TexturePageWidth to at least %d."), RequiredWidth / ActualDistanceFieldScaleFactor);
				}
				else
				{
					UE_LOG(LogTTFontImport, Error, TEXT("You should increase TexturePageMaxHeight to at least %d."), RequiredHeight / ActualDistanceFieldScaleFactor);
				}
			...
			if( (CharWidth + X + BitmapPaddingX) > BitmapWidth)
			{
				X=BitmapPaddingX;
			...
			// new page
			if( (Y + RowHeight + BitmapPaddingY) > BitmapHeight )
			{
				const int32 RoundedHeight = RoundUpToMultipleOf4(Y + OldRowHeight);
				check(RoundedHeight <= BitmapHeight);
				Font->Textures.Add( CreateTextureFromDC( Font, DCHandle, BitmapWidth, RoundedHeight, CurrentTexture ) );
				CurrentTexture++;
		...
		// save final page
		const int32 RoundedHeight = RoundUpToMultipleOf4(Y + RowHeight);
		check(RoundedHeight <= BitmapHeight);
		Font->Textures.Add( CreateTextureFromDC( Font, DCHandle, BitmapWidth, RoundedHeight, CurrentTexture ) );
		CurrentTexture++;
...
UTexture2D* UTrueTypeFontFactory::CreateTextureFromBitmap( UFont* Font, uint8* BitmapData, const int32 BitmapWidth, const int32 BitmapHeight, int32 TextureNum )
{
	FString TextureString = FString::Printf( TEXT("%s_Page"), *Font->GetName() );
	if( TextureNum < 26 )
	{
	...
	const TArray< float >& ResHeights )
{
	...
	if (BitmapDataPtr == nullptr)
	{
		UE_LOG(LogTTFontImport, Error, TEXT("Unable to allocate bitmap buffer for font texture. Please verify the texture page size and distance field scale factor settings."));
	...
	for( int32 Page = 0; Page < NumResolutions; ++Page )
	{
		...
		for( int32 CurCharIndex = 0; CurCharIndex < CharsPerPage; ++CurCharIndex )
		{
			...
			if( (Y + RowHeight + BitmapPaddingY) > BitmapHeight )
			{
				const int32 RoundedHeight = RoundUpToMultipleOf4(Y + OldRowHeight);
				check(RoundedHeight <= BitmapHeight);
				Font->Textures.Add( CreateTextureFromBitmap( Font, BitmapDataPtr, BitmapWidth, RoundedHeight, CurrentTexture ) );
				CurrentTexture++;

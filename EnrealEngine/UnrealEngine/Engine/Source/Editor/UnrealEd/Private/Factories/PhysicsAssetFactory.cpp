...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PhysicsAssetFactory)
...
UObject* UPhysicsAssetFactory::CreatePhysicsAssetFromMesh(FName InAssetName, UObject* InParent, USkeletalMesh* SkelMesh, bool bSetToMesh)
{
	...
	if( bWasOkClicked )
	{
		...
		if(NewAsset)
		{
			...
			//Reset any property that should not be reconduct next time the dialog is open
			//	- the LodIndex must be reset to zero
			if (UPhysicsAssetGenerationSettings* MutablePhysicsAssetGenerationSettings = GetMutableDefault<UPhysicsAssetGenerationSettings>())
			{
				MutablePhysicsAssetGenerationSettings->CreateParams.LodIndex = 0;
				MutablePhysicsAssetGenerationSettings->SaveConfig();
			}
			if(bSuccess)
			{

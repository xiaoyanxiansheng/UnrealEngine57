...
UObject* UFontFileImportFactory::FactoryCreateBinary(UClass* InClass, UObject* InParent, FName InName, EObjectFlags InFlags, UObject* InContext, const TCHAR* InType, const uint8*& InBuffer, const uint8* InBufferEnd, FFeedbackContext* InWarn)
{
	...
	if (FontFace && bCreateFontAsset)
	{
		...
		if (Font)
		{
			...
			FTypefaceEntry& DefaultTypefaceEntry = Font->GetMutableInternalCompositeFont().DefaultTypeface.Fonts.AddDefaulted_GetRef();
...
bool UReimportFbxStaticMeshFactory::CanReimport( UObject* Obj, TArray<FString>& OutFilenames )
{
	...
	if(Mesh)
	{
		...
		{
			UFbxAssetImportData *FbxAssetImportData = Cast<UFbxAssetImportData>(Mesh->GetAssetImportData());
			...
			FString FileExtension = FPaths::GetExtension(Mesh->GetAssetImportData()->GetFirstFilename());
			...
			OutFilenames.Add(Mesh->GetAssetImportData()->GetFirstFilename());
		}
		else
		{
...
EReimportResult::Type UReimportFbxStaticMeshFactory::Reimport( UObject* Obj )
{
	...
	UFbxStaticMeshImportData* ImportData = Cast<UFbxStaticMeshImportData>(Mesh->GetAssetImportData());
	...
	else
	{
		...
		if (Mesh->GetCustomizedCollision())
		{
		...
		if ( bOperationCanceled && Mesh->GetCustomizedCollision())
		{
	...
	ImportOptions->StaticMeshLODGroup = Mesh->GetLODGroup();
	...
	if( !bOperationCanceled && ensure(ImportData) )
	{
		...
		if ( FFbxImporter->ImportFromFile( *Filename, FPaths::GetExtension( Filename ), true ) )
		{
			...
			if (FFbxImporter->ReimportStaticMesh(Mesh, ImportData))
			{
				...
				Mesh->GetAssetImportData()->Update(Filename);

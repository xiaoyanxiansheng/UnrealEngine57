...
void FSourceFileDatabase::UpdateIfNeeded()
{
	...
	auto AppendConvertedPaths = [](TArray<FString>& OutDirs, TArrayView<const FString> BaseDirs, FPaths::EPathConversion Method, const TCHAR* ExtraData = nullptr)
	{
		OutDirs.Reserve(OutDirs.Num() + BaseDirs.Num());
		for (const FString& BaseDir : BaseDirs)
		{
			OutDirs.Add(FPaths::ConvertPath(BaseDir, Method, ExtraData));
		}
	};

	auto AppendEnginePaths = [&AppendConvertedPaths](TArray<FString>& OutDirs, TArrayView<const FString> BaseDirs)
	{
		OutDirs.Append(BaseDirs);

		for (const FString& PlatformName : FDataDrivenPlatformInfoRegistry::GetPlatformDirectoryNames(false))
		{
			AppendConvertedPaths(OutDirs, BaseDirs, FPaths::EPathConversion::Engine_PlatformExtension, *PlatformName);
		}

		if (FEngineBuildSettings::IsInternalBuild())
		{
			AppendConvertedPaths(OutDirs, BaseDirs, FPaths::EPathConversion::Engine_NoRedist);
			AppendConvertedPaths(OutDirs, BaseDirs, FPaths::EPathConversion::Engine_NotForLicensees);
		}
	};

	auto AppendProjectPaths = [&AppendConvertedPaths](TArray<FString>& OutDirs, TArrayView<const FString> BaseDirs)
	{
		OutDirs.Append(BaseDirs);

		for (const FString& PlatformName : FDataDrivenPlatformInfoRegistry::GetPlatformDirectoryNames(false))
		{
			AppendConvertedPaths(OutDirs, BaseDirs, FPaths::EPathConversion::Project_PlatformExtension, *PlatformName);
		}

		if (FEngineBuildSettings::IsInternalBuild())
		{
			AppendConvertedPaths(OutDirs, BaseDirs, FPaths::EPathConversion::Project_NoRedist);
			AppendConvertedPaths(OutDirs, BaseDirs, FPaths::EPathConversion::Project_NotForLicensees);
		}
	};

	// Find all the non-plugin modules
	{
		TArray<FString> ModuleSearchDirs;

		// Build the list of Engine directories to search for modules
		{
			const FString EngineModuleSearchDirs[] = {
				FPaths::Combine(FPaths::EngineDir(), TEXT("Source"), TEXT("Developer")),
				FPaths::Combine(FPaths::EngineDir(), TEXT("Source"), TEXT("Editor")),
				FPaths::Combine(FPaths::EngineDir(), TEXT("Source"), TEXT("Runtime")),
			};
			AppendEnginePaths(ModuleSearchDirs, EngineModuleSearchDirs);
		}

		// Build the list of Project directories to search for modules
		if (FApp::HasProjectName())
		{
			const FString ProjectModuleSearchDirs[] = {
				FPaths::Combine(FPaths::ProjectDir(), TEXT("Source")),
			};
			AppendProjectPaths(ModuleSearchDirs, ProjectModuleSearchDirs);
		}

		// Find all the build rules within the module search directories
		for (const FString& ModuleSearchDir : ModuleSearchDirs)
		{
			FindRootFilesRecursive(ModuleNames, ModuleSearchDir, TEXT("*.Build.cs"));
		}
	}
	...
	// Find list of disallowed header names in native (non-plugin) directories
	{
		...
		{
			IFileManager::Get().FindFilesRecursive(HeaderFiles, *FPaths::Combine(FPaths::GetPath(ModuleName), TEXT("Classes")), TEXT("*.h"), true, false, false);
			IFileManager::Get().FindFilesRecursive(HeaderFiles, *FPaths::Combine(FPaths::GetPath(ModuleName), TEXT("Public")), TEXT("*.h"), true, false, false);
		}
	...
	// Find all the plugin modules
	{
		TArray<FString> PluginSearchDirs;

		// Build the list of Engine directories to search for plugins
		{
			const FString EnginePluginSearchDirs[] = {
				FPaths::Combine(FPaths::EngineDir(), TEXT("Plugins")),
			};
			AppendEnginePaths(PluginSearchDirs, EnginePluginSearchDirs);
		}

		// Build the list of Project directories to search for plugins
		if (FApp::HasProjectName())
		{
			const FString ProjectPluginSearchDirs[] = {
				FPaths::Combine(FPaths::ProjectDir(), TEXT("Plugins")),
			};
			AppendProjectPaths(PluginSearchDirs, ProjectPluginSearchDirs);
		}

		// Find all the plugin directories
		TArray<FString> PluginNames;
		for (const FString& PluginSearchDir : PluginSearchDirs)
		{
			FindRootFilesRecursive(PluginNames, PluginSearchDir, TEXT("*uplugin"));
		}

		// Add all the files within plugin directories
		for (const FString& PluginName : PluginNames)
		{
			FindRootFilesRecursive(ModuleNames, FPaths::Combine(FPaths::GetPath(PluginName), TEXT("Source")), TEXT("*.Build.cs"));
		}
	}
}
...
{
#if PLATFORM_WINDOWS
	TStringBuilder<512> Params;	

	// Try and load suggested components from the Windows SDK configuration file
	const FString WindowsSDKFile = FPaths::RootDir() / TEXT("Engine/Config/Windows/Windows_SDK.json");
	if (FPaths::FileExists(WindowsSDKFile))
	{
		FString FileContents;
		if (FFileHelper::LoadFileToString(FileContents, *WindowsSDKFile))
		{
			TSharedRef<TJsonReader<>> JsonReader = TJsonReaderFactory<>::Create(FileContents);
			TSharedPtr<FJsonObject> RootObject;
			if (FJsonSerializer::Deserialize(JsonReader, RootObject) && RootObject.IsValid())
			{
				const TArray<TSharedPtr<FJsonValue>> SuggestedComponentsArray = RootObject->GetArrayField(TEXT("VisualStudioSuggestedComponents"));
				for (const TSharedPtr<FJsonValue>& SuggestedComponents : SuggestedComponentsArray)
				{
					FString Value = SuggestedComponents->AsString();
					if (!Value.IsEmpty())
					{
						Params.Appendf(TEXT("--add \"%s\" "), *Value);
					}
				}
			}
		}
	...
	if (Params.Len() == 0)
	{
		UE_LOG(LogSelectionDetails, Warning, TEXT("LaunchIDEInstaller: Failed to load suggested Visual Studio components from [%s]"), *WindowsSDKFile);
	}

	Params.Append(TEXT("--productId \"Microsoft.VisualStudio.Product.Community\" --campaign \"EpicGames_UE5\""));
	FPlatformProcess::ExecElevatedProcess(*Filepath, Params.GetData(), nullptr);
#endif
}

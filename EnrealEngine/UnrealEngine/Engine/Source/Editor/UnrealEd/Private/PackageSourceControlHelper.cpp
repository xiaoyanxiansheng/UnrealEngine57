...
bool FPackageSourceControlHelper::Checkout(const TArray<FString>& PackageNames, bool bErrorsAsWarnings) const
{
	...
	if (bUseSourceControl)
	{
		...
		for(FSourceControlStateRef& SourceControlState : SourceControlStates)
		{
			...
			{
				// Package needs to be checked out but if it is currently opened for delete, it needs to be reverted first.
				if (SourceControlState->IsDeleted())
				{
					PackagesToRevert.Add(PackageFilename);
				}
	...
	else if (bUseSourceControl)
	{
		if (!PackagesToRevert.IsEmpty())
		{
			...
			if (GetSourceControlProvider().Execute(RevertOperation, PackagesToRevert) != ECommandResult::Succeeded)
			{
				FPackageSourceControlHelperLog::Error(bErrorsAsWarnings, TEXT("Error reverting opened for delete file packages prior to checking out"));
				return false;
			}
		}

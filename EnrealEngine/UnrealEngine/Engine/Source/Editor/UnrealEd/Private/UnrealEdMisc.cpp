...
void FUnrealEdMisc::EditorAnalyticsHeartbeat()
{
	...
	const bool bInVRMode = IVREditorModule::Get().GetVRModeBase() != nullptr;
...
void FUnrealEdMisc::PreSaveWorld(UWorld* World, FObjectPreSaveContext ObjectSaveContext)
{
	...
	const bool bAutosaveOrPIE = ObjectSaveContext.IsFromAutoSave();
...
FText FUnrealEdMisc::OnGetAssetDataDisplayName(const FAssetData& InObject, const bool bFullPath)
{
	...
	// Verse paths only exist for top level assets, so show the same value regardless of bFullPath;
	// we will fall through if we have a subobject.
	if (IAssetTools::Get().ShowingContentVersePath())
	{
		UE::Core::FVersePath VersePath = InObject.GetVersePath();
	...
	if (InObject != nullptr)
	{
		...
		else if (InObject != nullptr)
		{
			if (bFullPath)
			{
				...
				if (IAssetTools::Get().ShowingContentVersePath())
				{
					VersePath = InObject->GetVersePath();
				}

				if (VersePath.IsValid())
				{
					Name = FText::FromString(MoveTemp(VersePath).ToString());
				}
				else
				{
					Name = FText::FromString(InObject->GetPathName());
				}
			}
			else
			{
				Name = FText::FromString(InObject->GetName());
			}
		}
	}

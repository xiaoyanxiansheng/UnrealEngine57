	...
	)
{
	...
	while (FParse::LineExtended(&SourceText, StrLine, LinesConsumed, FParse::ELineExtendedFlags::OldExactMode))
	{
		...
		else if( GetBEGIN(&Str,TEXT("Object")))
		{
			...
			// Using override semantics means we do not import (shadowed) subobject references, as they are not explicitly overridden.
			// The legacy path will always use exported subobjects and replace references to default subobjects after moving them aside.
			const bool bUseOverrideSemantics = bOverrideT3DSupported && FOverridableSerializationLogic::HasCapabilities(FOverridableSerializationLogic::ECapabilities::SubObjectsShadowSerialization);
			...
			{
				// When using override semantics, during the initial definition phase (the 'else' case below), the exported
				// subobject may have been found to not be type-compatible with the default subobject that was instanced for
				// the outer object at construction time (e.g. due to schema changes after exporting). In that case, we will
				// have created a unique instance to serve as the destination for the redefinition here. This means that any
				// exported instance data will not be imported to the instanced default subobject, and its data will be lost.
				//
				// Note: Referencing properties are also NOT imported in this case unless explicitly overridden at export time,
				// so outside of that scenario, inner references (by design) are not remapped to the unique instance on import.
				//
				// @todo - IDO support will be needed to capture the unique instance and preserve its imported instance data.
				if (bUseOverrideSemantics
					&& FOverridableManager::Get().IsEnabled(BaseTemplate))
				{
					FSoftObjectPath ExportedObjectPath(ExportedObjectFullName);
					checkf(ObjectRemapper, TEXT("Object remapper expected to be non-NULL when importing with override semantics."));
					if (!ensureMsgf(bExportPath, TEXT("Subobject redefinition is missing the object's export path while importing it.")))
					{
						// Fall back to the resolved subobject, however its path can differ from the exported path that it was mapped
						// to if e.g. there was a name collision within the outer scope, so it's not as reliable as the exported path.
						ExportedObjectPath = FSoftObjectPath(BaseTemplate);
					}

					UObject* const* RemappedObject = ObjectRemapper->Find(ExportedObjectPath);
					if (RemappedObject)
					{
						BaseTemplate = *RemappedObject;
					}
				}
			...
			else 
			{
				...
				if (ComponentTemplate)
				{
					...
					{
						if (bUseOverrideSemantics
							&& FOverridableManager::Get().IsEnabled(ComponentTemplate)
							&& !ComponentTemplate->GetClass()->IsChildOf(TemplateClass))
						{
							// If we found an existing instanced subobject above that is not type-compatible,
							// create a new object to serve as the destination for importing instance data.
							// Otherwise, we allow instance data to be imported into the existing subobject.
							// 
							// This differs from the non-override path, which moves the instanced subobject
							// aside and constructs a new type-compatible subobject in its place (see below).
							// That path will remain in place for legacy support and backwards-compatibility.
							// 
							// When exporting override T3D, all reference property values will be "shadowed" on
							// import if they were not otherwise explicitly overridden at export time. Shadowed
							// reference values are skipped at import time, so it means we intentionally sidestep
							// the reference replacement pass below, and instead defer replacement to occur only
							// when we import object properties and find an explicitly-overridden reference value.
							// In that case the override will be resolved through the export path remapping table.
							// 
							// This effectively matches what happens on level load when a serialized export blocks
							// deferred subobject instancing due to e.g. a schema change that occurs while unloaded.
							// In that case archetype lookup will fail due to type incompatibility, and the exported
							// subobject is moved aside to allow a default instance of the updated type to be created.
							//
							// @todo - IDO support will be needed to capture the new instance that will result
							// in order to avoid data loss. If the new instance is not otherwise referenced by
							// an explicitly-overridden property at import time, then it will become unreachable.
							TemplateName = MakeUniqueObjectName(SubobjectOuter, TemplateClass, TemplateName);
						}
						else
						{
						...
						// Forces a new instance to be created below
				...
				// Make sure we have an up-to-date mapping in the object graph. This may not always be the case after defining an object.
				//
				// For example, if it was exported as an instance-added subobject with no non-default archetype, UE will bypass the subobject
				// instancing pass at construction time, and so the object graph will not include a mapping for it. This can cause the explicit
				// instancing pass that gets run after importing properties to assert, for example, if this object has its own default subobjects.
				//
				// Notes:
				//	- It is ok if the archetype is NULL. Including it avoids a redundant GetArchetype() call when it has already been set.
				//	- It is safe to assume that we're always inside a root object scope here, because we're importing a Begin Object block.
...
const TCHAR* ImportObjectProperties( FImportObjectParams& InParams )
{
	...
	// Use TOptional to avoid heap allocations and only pay for construction/destruction if Emplace is called.
	TOptional<TMap<FSoftObjectPath, UObject*>> ExportedPathToObjectCreated;
	if (bShouldUseDedicatedInstancingGraph)
	{
		InstanceGraph.SetDestinationRoot(InParams.SubobjectOuter);

		// When using override T3D, always map exported subobject paths so they can be resolved at redefinition time
		if (!InParams.ObjectRemapper)
		{
			InParams.ObjectRemapper = &ExportedPathToObjectCreated.Emplace();
		}
	}
	...
	if (ExportedPathToObjectCreated.IsSet())
	{
		// Remove dangling reference to ExportedPathToObjectCreated on the stack
		InParams.ObjectRemapper = nullptr;
	}

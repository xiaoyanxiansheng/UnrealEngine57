...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PanelExtensionSubsystem)

DECLARE_LOG_CATEGORY_CLASS(LogPanelExtensions, Log, All);
...
{
	if (const FText* DeprecationMessage = DeprecatedExtensions.Find(ExtensionPanelID))
	{
		UE_LOG(LogPanelExtensions, Warning, TEXT("Extension panel id \"%s\" has been deprecated. %s"), *ExtensionPanelID.ToString(), *DeprecationMessage->ToString());
	}
...
void UPanelExtensionSubsystem::MarkDeprecatedExtension(FName ExtensionID, const FText& Message)
{
	DeprecatedExtensions.Add(ExtensionID, Message);
	
	if (const TArray<FPanelExtensionFactory>* Factories = ExtensionPointMap.Find(ExtensionID))
	{
		if (Factories->Num())
		{
			UE_LOG(LogPanelExtensions, Warning, TEXT("Extension panel id \"%s\" has been deprecated. %s"), *ExtensionID.ToString(), *Message.ToString());
		}
	}
}

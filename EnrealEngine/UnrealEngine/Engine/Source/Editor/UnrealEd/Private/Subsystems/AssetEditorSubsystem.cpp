...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AssetEditorSubsystem)
...
void UAssetEditorSubsystem::InitializeRecentAssets()
{
	...
	if(RecentAssetsList->GetNumItems() != RecentAssetEditors.Num())
	{
		...
		UE_LOG(LogAssetEditorSubsystem, Display, TEXT("Something went wrong while loading recent assets! Num Recent Assets  = %d, Num Recent Asset Editors = %d"), RecentAssetsList->GetNumItems(), RecentAssetEditors.Num());
	}
...
bool UAssetEditorSubsystem::CloseAllAssetEditors()
{
	...
	TArray<IAssetEditorInstance*> OpenedEditorInstances;
	OpenedEditors.GetKeys(OpenedEditorInstances);
	for (IAssetEditorInstance* Editor : OpenedEditorInstances) // operate on copy since closing will remove from map
	{
		if (Editor != nullptr)
		{

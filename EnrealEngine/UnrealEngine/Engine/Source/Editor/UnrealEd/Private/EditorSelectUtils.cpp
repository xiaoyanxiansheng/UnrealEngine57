		...
		for (int32 CompIdx = 0; CompIdx < Components.Num(); CompIdx++)
		{
			...
			if (Comp.IsValid() && Comp->IsRegistered())
			{
				...
				if (Visualizer.IsValid())
				{
					...
					ComponentVisualizerForSelection.IsEnabledDelegate.Emplace(
						[bIsSelectedComponent = (Comp == SelectedComponent),
						 WeakViz = TWeakPtr<FComponentVisualizer>(Visualizer),
						 CompPath = CachedComponentVisualizer.ComponentPropertyPath]()
						{
							if (bIsSelectedComponent == true)
							{
								return true;
							}

							if (GetDefault<UEditorPerProjectUserSettings>()->bShowSelectionSubcomponents == false)
							{
								return false;
							}
							...
							{
								bShouldShowWithSelection = PinnedViz->ShouldShowForSelectedSubcomponents(CompPath.GetComponent());
							}

...
void SCreateAnimationAssetDlg::Construct(const FArguments& InArgs)
{
	...
	const bool bPrioritizeLastPathUsed = InArgs._PrioritizeLastPathUsed;
	
	if (AssetPath.IsEmpty() || (bPrioritizeLastPathUsed && !LastUsedAssetPath.IsEmpty()))
	{
		AssetPath = LastUsedAssetPath;

		// If the AssetPath is still empty, fall back to the Game dir

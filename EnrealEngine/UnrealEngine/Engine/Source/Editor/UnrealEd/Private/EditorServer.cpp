	...
	if( !GIsTransacting )
	{
		...
		for (FSelectionIterator It(GetSelectedActorIterator()); It; ++It)
		{
			...
			else
			{
				...
				if (ContainsObjectOfClass(AttachedActors, ABrush::StaticClass(), EFindObjectFlags::ExactClass, &AttachedBrushes))
				{
...
{
	// Temporary solution to avoid triggering the BPVM runaway detection when loading large maps with Verse objects.
	FBlueprintCoreDelegates::SetScriptMaximumLoopIterations(MAX_int32);
	ON_SCOPE_EXIT { FBlueprintCoreDelegates::SetScriptMaximumLoopIterations(GEngine->MaximumLoopIterationCount); };
...
ULevel*  UEditorEngine::CreateTransLevelMoveBuffer( UWorld* InWorld )
{
	...
	BufferDefaultBrush->Brush = CastChecked<UModel>(StaticFindObject(UModel::StaticClass(), BufferLevel->OwningWorld->GetOuter(), TEXT("Brush"), EFindObjectFlags::ExactClass), ECastCheckedType::NullAllowed);
			...
			if (RefdPackage != NULL)
			{
				...
				if (Linker)
				{
					for (int32 ExportIdx = 0; ExportIdx < Linker->ExportMap.Num(); ExportIdx++)
					{
						...
						UClass* CheckClass = FindObject<UClass>(CheckClassPathName, EFindObjectFlags::ExactClass);
						if (
							(CheckClass != NULL) &&

...
#include UE_INLINE_GENERATED_CPP_BY_NAME(ThumbnailHelpers)
...
FParticleSystemThumbnailScene::~FParticleSystemThumbnailScene()
{
	FFXSystemInterface::MarkPendingKill( ThumbnailFXSystem );
}
...
{
	return InMaterial->GetMaterialResource(GMaxRHIShaderPlatform) != nullptr ? InMaterial->GetMaterialResource(GMaxRHIShaderPlatform)->IsTranslucencyAfterDOFEnabled() : false;
}
...
void FStaticMeshThumbnailScene::GetViewMatrixParameters(const float InFOVDegrees, FVector& OutOrigin, float& OutOrbitPitch, float& OutOrbitYaw, float& OutOrbitZoom) const
{
	...
	USceneThumbnailInfo* ThumbnailInfo = Cast<USceneThumbnailInfo>(PreviewActor->GetStaticMeshComponent()->GetStaticMesh()->GetThumbnailInfo());
	if ( ThumbnailInfo )
	{
		if ( TargetDistance + ThumbnailInfo->OrbitZoom < 0 )
		{
...
void FClassActorThumbnailScene::SpawnPreviewActor(UClass* InClass)
{
	...
	if (InClass && !InClass->HasAnyClassFlags(CLASS_Deprecated | CLASS_Abstract))
	{
		...
		if (PreviewActor.IsValid())
		{
			...
			const FVector TargetLocation(-Bounds.Origin + FVector(0, 0, BoundsZOffset));

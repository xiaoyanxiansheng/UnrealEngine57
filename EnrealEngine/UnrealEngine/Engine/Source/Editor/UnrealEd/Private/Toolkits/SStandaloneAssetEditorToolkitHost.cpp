...
void SStandaloneAssetEditorToolkitHost::RestoreFromLayout( const TSharedRef<FTabManager::FLayout>& NewLayout )
{
	...
	// Let the manager handle some logic on the Areas before destroying
	...
	TSharedRef<SWidget> AssetEditorContent = SNew(SVerticalBox)
		+ SVerticalBox::Slot()
	...
	/**
	 * The ordering is important here, otherwise the status bar won't have a summon tab in PanelDrawer widget.
	 * The layout and panel drawer area must be restored before creating the status bar.
	 */
	if (TSharedPtr<SWidget> PanelDrawer = MyTabManager->RestorePanelDrawer(AssetEditorContent, ParentWindow.ToSharedRef()))
	{
		AssetEditorContent = PanelDrawer.ToSharedRef();
	}

...
UE_DEFINE_TCOMMANDS(FGlobalEditorCommonCommands)
...
void FGlobalEditorCommonCommands::RegisterCommands()
{
	...
	UI_COMMAND(OpenDocumentation, "Open Documentation...", "Opens documentation for this tool", EUserInterfaceActionType::Button, FInputChord(EModifierKey::Shift, EKeys::F1));
	...
	UI_COMMAND(OpenFindInAllBlueprints, "Open Find in all Blueprints", "Opens the Find in all Blueprints dialogue", EUserInterfaceActionType::Button, FInputChord());
}
...
void FGlobalEditorCommonCommands::MapActions(TSharedRef<FUICommandList>& ToolkitCommands)
{
	...
	ToolkitCommands->MapAction(
		Get().OpenFindInAllBlueprints,
		FExecuteAction::CreateStatic(&FGlobalEditorCommonCommands::OnOpenFindInAllBlueprints));
}
...
void FGlobalEditorCommonCommands::OnOpenFindInAllBlueprints()
{
	if (const TSharedPtr<SFindInBlueprints> Dialogue = FFindInBlueprintSearchManager::Get().GetGlobalFindResults())
	{
		Dialogue->FocusForUse(false);
	}
}

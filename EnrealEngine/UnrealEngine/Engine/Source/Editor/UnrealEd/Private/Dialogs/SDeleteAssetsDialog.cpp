	...
	[
		...
		[
			...
			[
				...
				.Padding( 6, 0 )
				[
					SAssignNew( ForceDeleteButton, SBorder )
					.BorderImage( FAppStyle::GetBrush( "NoBorder" ) )
...
void SDeleteAssetsDialog::OnCancelHotkeyPressed()
{
	Cancel();
}

void SDeleteAssetsDialog::OnConfirmHotkeyPressed()
{
	// If we're already in the process of deleting, we don't want to do anything
	if (DeleteModel->GetState() != FAssetDeleteModel::Finished)
	{
		return;
	}

	// If Force Delete is available, trigger that
	if (CanForceDelete())
	{
		ForceDelete();
	}
	// Otherwise trigger the normal Delete
	else
	{
		Delete();
	}
...
TSharedPtr<SWidget> SDeleteAssetsDialog::GetWidgetToFocusOnActivate()
{
	return CanForceDelete() ? ForceDeleteButton : DeleteButton;
}

...
#define ADD_SLOT_PRIMARY(Button)\
		ButtonBox->AddSlot(SlotIndex++,0)\
		[\
			SAssignNew( PrimaryButton, SButton )\
			.VAlign(VAlign_Center)\
			.HAlign(HAlign_Center)\
		...
		// Route user focus to the default dialog button. 
		if (ensure(PrimaryButton.IsValid()))
		{
			// Dialog should have a primary slot, allowing users to navigate options via esc, space, enter, and tab keys
			ParentWindow->SetWidgetToFocusOnActivate(PrimaryButton);
		}
		else
		{
			// Fallback to entire dialog. This will only support esc key
...
#undef ADD_SLOT_PRIMARY
	...
	{
		//see if we pressed the Escape to cancel choice
...
public:
	...
	TSharedRef<SHorizontalBox> ConstructConditionalInternals( const FArguments& InArgs ) 
	{
		...
		// Route user focus to the default dialog button. 
		if (ensure(MyWindow.Pin()))
		{
			MyWindow.Pin()->SetWidgetToFocusOnActivate(PrimaryButton);
		}

...
void OutputIncrementalCookStats()
{
	...
	if (NumPackagesIncrementallyModifiedByClassSum > 0)
	{
		...
		UE_LOG(LogCookStats, Display, TEXT("Incrementally Modified Counts for Each Class:"));
		UE_LOG(LogCookStats, Display, TEXT("\tTotal, %u"), NumPackagesIncrementallyModifiedByClassSum);
		for (const TPair<FTopLevelAssetPath, uint32>& Pair : ClassAndNums)
		{
			UE_LOG(LogCookStats, Display, TEXT("\t%s, %u"),

...
public:
	...
	FName Get(FPackageId PackageId) const
	{
		FScopeLock _(&CriticalSection);
	...
	{
		FScopeLock _(&CriticalSection);
		return PackageIdToName.Num();
	}

	/**
	 * Iterate over all entries in the registry
	 */
	void ForEachPackage(TFunction<void(const FPackageId&, const FName&)> Visitor) const
	{
		FScopeLock _(&CriticalSection);
		for (const TPair<FPackageId, FName>& Entry : PackageIdToName)
		{
...
public:
		...
		, ServiceId(FExternalCookOnTheFlyServer::GenerateServiceId())
	{
		...
		ConnectionServer->OnRequest(ECookOnTheFlyMessage::GetAllOnDiskPackages).BindRaw(this, &FIoStoreCookOnTheFlyRequestManager::HandleClientRequest);
...
private:
	...
	bool HandleClientRequest(UE::Cook::ICookOnTheFlyClientConnection& Connection, const UE::Cook::FCookOnTheFlyRequest& Request)
	{
		...
		{ 
			...
			case UE::Cook::ECookOnTheFlyMessage::RecookPackages:
				...
				break;
			case UE::Cook::ECookOnTheFlyMessage::GetAllOnDiskPackages:
				bRequestOk = HandleGetAllOnDiskPackagesRequest(Connection.GetPlatformName(), Request, Response);
				break;
			default:
				return false;
		}
	...
	bool HandleGetAllOnDiskPackagesRequest(const FName& PlatformName, const UE::Cook::FCookOnTheFlyRequest& Request, UE::Cook::FCookOnTheFlyResponse& Response)
	{
		using namespace UE::ZenCookOnTheFly::Messaging;

		TRACE_CPUPROFILER_EVENT_SCOPE(CookOnTheFly::HandleGetAllOnDiskPackagesRequest);

		if (PlatformName.IsNone())		// only accept this request from non-editor clients
		{
			UE_LOG(LogCookOnTheFly, Warning, TEXT("GetAllOnDiskPackages from editor client"));
			Response.SetStatus(UE::Cook::ECookOnTheFlyMessageStatus::Error);
			return true;
		}
		else
		{
			...
			// After this point the packages have been enumerated into FPlatformContext::PackageRegistry
			check(bCookerGlobalsInitialized);		

			FGetAllOnDiskPackagesRequest AllOnDiskPackages;
			AllOnDiskPackages.PackageIds.Reserve(PackageRegistry.Num());
			{
				FScopeLock __(&Context.GetLock());
				PackageRegistry.ForEachPackage([&AllOnDiskPackages](const FPackageId& ID, const FName& Name)
					{
						AllOnDiskPackages.PackageIds.Emplace(ID);
					});
			}

			Response.SetBodyTo(MoveTemp(AllOnDiskPackages));
		}

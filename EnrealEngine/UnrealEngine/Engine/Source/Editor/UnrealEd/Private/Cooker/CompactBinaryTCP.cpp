	...
	if (BytesSent < WriteSize)
	{
		...
		if (!bResult)
		{
			if (Socket->GetConnectionState() != SCS_ConnectionError &&
				(Error == SE_EWOULDBLOCK || Error == SE_TRY_AGAIN))
			{
				// Not an error, this can occur if the process on the other end of the socket is not
				// reading fast enough (breakpoint or a busy tick) and the socket's buffers have filled up.
				return EConnectionStatus::Incomplete;
			}

			UE_LOG(LogSockets, Log, TEXT("FCompactBinaryTCPImpl::PollSendBytes: Socket->Send(WriteSize==%d) failed with error %d and BytesSent == %d."),
				WriteSize, (int32)Error, BytesSent);
			...
			// TODO: Decide which errors are fatal even if the socket is still connected. We have seen one case
			// where the socket send fails forever, but we were not able to see what error it was returning.
		}

...
public:
	...
	void ReportFileTransfer(FStringView TempFileName, FStringView TargetFileName);
...
private:
	...
	void LogConnected();
	/**
	 * Helper for Tick. Gather messages from pending package results, queued messages, and any other sources, and add
	 * them to the SendBuffer.
	 */
	void SendPendingMessages();
	/** Helper for Tick, pump the messages that have been added to the buffer to send to the Server. */
	void PumpSendBuffer();
	...
	void SendMessage(UE::CompactBinaryTCP::FMarshalledMessage&& Message);
	/**
	 * Queue a message for sending during the next SendPendingMessages. @see QueuedMessages for how it is ordered
	 * with other messages.
	 */
	void QueueMessage(UE::CompactBinaryTCP::FMarshalledMessage&& Message);
...
private:
	...
	/**
	 * QueuedMessages are added to the SendBuffer during Tick, after PendingResults, GeneratorEvents, and
	 * FileTransfers. QueuedMessages are sent in the order in which they are queued, but they are unordered
	 * with respect to PendingResults/GeneratorEvents/FileTransfers. A QueuedMessage can be sent after a
	 * PendingResult/GeneratorEvent/FileTransfer even if it was queued before it.
	 */
	TArray<UE::CompactBinaryTCP::FMarshalledMessage> QueuedMessages;

	// Variables Read/Write only within FileTransferLock
	FCriticalSection FileTransferLock;
	TArray<FFileTransferMessage> PendingFileTransfers;
};

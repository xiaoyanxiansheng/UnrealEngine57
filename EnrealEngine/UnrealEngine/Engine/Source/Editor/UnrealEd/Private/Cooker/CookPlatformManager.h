...
namespace UE::Cook
{
	...
	*/
	...
	public:
		...
		/**
		 * Calls the Callback with each platform in GetSessionPlatforms. This is the accessor that must be used when
		 * trying to read the SessionPlatforms from an async thread in an FPlatformManager::FReadScopeLock.
		 */
		template <typename CallbackType>
		void EnumerateSessionPlatforms(CallbackType&& Callback) const;
	...
	private:
		...
		TMap<FPlatformId, FPlatformData*> PlatformDatas;
...
void RemapMapKeys(TMap<const ITargetPlatform*, Value>& Map, const TMap<ITargetPlatform*,
	ITargetPlatform*>& Remap)
{
	TMap<const ITargetPlatform*, Value> NewMap;
...
template <typename CallbackType>
void FPlatformManager::EnumerateSessionPlatforms(CallbackType&& Callback) const
{

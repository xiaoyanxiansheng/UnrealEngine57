...
void FLevelEditorViewportClient::LostFocus(FViewport* InViewport)
{
	if (ModeTools)
	{
		if (const FEditorViewportClient* FocusedViewportClient = ModeTools->GetFocusedViewportClient())
		{
			// Make sure we are calling LostFocus on currently focused viewport
			// We don't need to call LostFocus multiple times on the same viewport if it's already un-focused.
			if (InViewport == FocusedViewportClient->Viewport)
            {
...
void FLevelEditorViewportClient::ProcessClick(FSceneView& View, HHitProxy* HitProxy, FKey Key, EInputEvent Event, uint32 HitX, uint32 HitY)
{
	...
	if (Click.GetKey() == EKeys::MiddleMouseButton && !Click.IsAltDown() && !Click.IsShiftDown())
	{
		...
		{
			if (TSharedPtr<ILevelEditor> ParentLevelEditorPinned = ParentLevelEditor.Pin())
			{
				ParentLevelEditorPinned->SummonLevelViewportViewOptionMenu(LVT_Perspective);
			}
		}
	...
	if (!ModeTools->HandleClick(this, HitProxy,Click))
	{
		...
		if (HitProxy == NULL)
		{
			UE::Editor::ViewportSelectionUtilities::ClickBackdrop(this,Click);
		}
		else if (HitProxy->IsA(HWidgetAxis::StaticGetType()))
		{
		...
		else if (HitElement && UE::Editor::ViewportSelectionUtilities::ClickElement(this, HitElement, Click))
		{
		...
		else if (HitProxy->IsA(HActor::StaticGetType()))
		{
			...
			if (ConsideredActor) // It is possible to be clicking something during level transition if you spam click, and it might not be valid by this point
			{
				...
				if (bSelectComponent)
				{
					bComponentSelected = UE::Editor::ViewportSelectionUtilities::ClickComponent(this, ActorHitProxy, Click);
				}
				
				if (!bComponentSelected)
				{
					UE::Editor::ViewportSelectionUtilities::ClickActor(this, ConsideredActor, Click, true);
				}
		...
		{
			UE::Editor::ViewportSelectionUtilities::ClickActor(this, ((HInstancedStaticMeshInstance*)HitProxy)->Component->GetOwner(), Click, true);
		}
		else if (HitProxy->IsA(HBSPBrushVert::StaticGetType()) && ((HBSPBrushVert*)HitProxy)->Brush.IsValid())
		{
			...
			UE::Editor::ViewportSelectionUtilities::ClickBrushVertex(this,((HBSPBrushVert*)HitProxy)->Brush.Get(),&Vertex,Click);
		}
		else if (HitProxy->IsA(HStaticMeshVert::StaticGetType()))
		{
			UE::Editor::ViewportSelectionUtilities::ClickStaticMeshVertex(this,((HStaticMeshVert*)HitProxy)->Actor,((HStaticMeshVert*)HitProxy)->Vertex,Click);
		}
		else if (BrushSubsystem && BrushSubsystem->ProcessClickOnBrushGeometry(static_cast<FEditorViewportClient*>(this), HitProxy, Click))
		{
		...
		else if (HitProxy->IsA(HModel::StaticGetType()))
		{
			...
			{
				UE::Editor::ViewportSelectionUtilities::ClickSurface(this,ModelHit->GetModel(),SurfaceIndex,Click);
			}
		}
		else if (HitProxy->IsA(HLevelSocketProxy::StaticGetType()))
		{
			UE::Editor::ViewportSelectionUtilities::ClickLevelSocket(this, HitProxy, Click);
		}
	}
}

void FLevelEditorViewportClient::SummonContextMenu(const FTypedElementHandle& HitProxyElement)
{
	if (TSharedPtr<ILevelEditor> ParentLevelEditorPinned = ParentLevelEditor.Pin())
	{
		ParentLevelEditorPinned->SummonLevelViewportContextMenu(HitProxyElement);
	}
}
...
bool FLevelEditorViewportClient::InputKey(const FInputKeyEventArgs& InEventArgs)
{
	...
	{
		// This Viewport Client received a click press, let's retrieve the currently focused viewport client.
		// If that's not This viewport client, let's make sure This is focused, instead of the currently focused one.
		FEditorViewportClient* CurrentlyFocusedViewport = ModeTools->GetFocusedViewportClient();
		if (CurrentlyFocusedViewport != this)
		{
			if (CurrentlyFocusedViewport)
			{
				// Call LostFocus on the Viewport Client losing focus
				LostFocus(CurrentlyFocusedViewport->Viewport);
			}

			// Focusing on This viewport
			ReceivedFocus(Viewport);
		}
...
void FLevelEditorViewportClient::ApplyDeltaToSelectedElements(const FTransform& InDeltaTransform)
{
...

	{
		...
		FSnappingUtils::AdjustPercentageBasedScale(AdjustedScale);
...
const FEditorViewportCameraSpeedSettings& FLevelEditorViewportClient::GetCameraSpeedSettings() const
{
	return GetDefault<ULevelEditorViewportSettings>()->CameraSpeedSettings;
}

void FLevelEditorViewportClient::SetCameraSpeedSettings(const FEditorViewportCameraSpeedSettings& InCameraSpeedSettings)
{
	GetMutableDefault<ULevelEditorViewportSettings>()->CameraSpeedSettings = InCameraSpeedSettings;
}

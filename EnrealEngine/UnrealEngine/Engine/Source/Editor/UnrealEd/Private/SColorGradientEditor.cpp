...
FReply SColorGradientEditor::OnMouseButtonUp( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
	...
	if( IsEditingEnabled.Get() == true )
	{
		if( MouseEvent.GetEffectingButton() == EKeys::LeftMouseButton )
		{
			...
			DistanceDragged = 0;
			bDraggingStop = false;
			// during interactive drag we called OnCurveChanged with EPropertyChangeType::Interactive
			//	now the drag is over, do OnCurveChanged with EPropertyChangeType::ValueSet
...
void SColorGradientEditor::MoveStop( FGradientStopMark& Mark, float NewTime )
{
	...
	EPropertyChangeType::Type ChangeType = bDraggingStop ? EPropertyChangeType::Interactive : EPropertyChangeType::ValueSet;
	// we want to be able to pass ChangeType as an arg to OnCurveChanged for interactive drags, but OnCurveChanged doesn't take that
	// there are lots of implementations of OnCurveChanged so changing the function now is not great
	//	instead set a bool on the curve to indicate the next OnCurveChanged type
	CurveOwner->SetOnCurveChangedIsInteractive(bDraggingStop);
	CurveOwner->OnCurveChanged(CurveOwner->GetCurves());
	CurveOwner->SetOnCurveChangedIsInteractive(false);
}

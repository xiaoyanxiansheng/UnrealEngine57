...
TSharedPtr<FExistingStaticMeshData> StaticMeshImportUtils::SaveExistingStaticMeshData(UStaticMesh* ExistingMesh, UnFbx::FBXImportOptions* ImportOptions, int32 LodIndex)
{
	...
	ExistingMeshDataPtr->ImportVersion = ExistingMesh->GetImportVersion();
	ExistingMeshDataPtr->UseMaterialNameSlotWorkflow = InternalImportUtils::IsUsingMaterialSlotNameWorkflow(ExistingMesh->GetAssetImportData());
	...
	ExistingMeshDataPtr->ExistingNaniteSettings = ExistingMesh->GetNaniteSettings();
	...
	ExistingMeshDataPtr->ExistingCustomizedCollision = ExistingMesh->GetCustomizedCollision();
	ExistingMeshDataPtr->bAutoComputeLODScreenSize = ExistingMesh->GetAutoComputeLODScreenSize();
	...
	ExistingMeshDataPtr->ExistingImportData = ExistingMesh->GetAssetImportData();
	ExistingMeshDataPtr->ExistingThumbnailInfo = ExistingMesh->GetThumbnailInfo();
	...
	ExistingMeshDataPtr->LODGroup = ExistingMesh->GetLODGroup();
	...
	UFbxStaticMeshImportData* ImportData = Cast<UFbxStaticMeshImportData>(ExistingMesh->GetAssetImportData());
...
void StaticMeshImportUtils::RestoreExistingMeshSettings(const FExistingStaticMeshData* ExistingMesh, UStaticMesh* NewMesh, int32 LODIndex)
{
	...
	NewMesh->SetLODGroup(ExistingMesh->LODGroup);
	...
	if (LODIndex == INDEX_NONE)
	{
		...
		if (NewMesh->GetLODGroup() != NAME_None)
		{
			ITargetPlatform* CurrentPlatform = GetTargetPlatformManagerRef().GetRunningTargetPlatform();
			check(CurrentPlatform);
			const FStaticMeshLODGroup& LODGroup = CurrentPlatform->GetStaticMeshLODSettings().GetLODGroup(NewMesh->GetLODGroup());
	...
	if(ExistingMesh->ImportVersion != EImportStaticMeshVersion::LastVersion)
	{
		...
		NewMesh->AddMaterialRemapIndex(FMaterialRemapIndex(MaterialMapKey, ImportRemapMaterial));
	}
...
void StaticMeshImportUtils::UpdateSomeLodsImportMeshData(UStaticMesh* NewMesh, TArray<int32> *ReimportLodList)
{
	...
	UFbxStaticMeshImportData* ImportData = Cast<UFbxStaticMeshImportData>(NewMesh->GetAssetImportData());
...
void StaticMeshImportUtils::RestoreExistingMeshData(const TSharedPtr<const FExistingStaticMeshData>& ExistingMeshDataPtr, UStaticMesh* NewMesh, int32 LodLevel, bool bCanShowDialog, bool bForceConflictingMaterialReset)
{
	...
	NewMesh->SetCustomizedCollision(ExistingMeshDataPtr->ExistingCustomizedCollision);
	NewMesh->SetAutoComputeLODScreenSize(ExistingMeshDataPtr->bAutoComputeLODScreenSize);
	...
	if (ExistingMeshDataPtr->ExistingImportData.IsValid())
	{
		...
		NewMesh->SetAssetImportData(ExistingMeshDataPtr->ExistingImportData.Get());

		ImportData = Cast<UFbxStaticMeshImportData>(NewMesh->GetAssetImportData());
	...
	NewMesh->SetThumbnailInfo(ExistingMeshDataPtr->ExistingThumbnailInfo.Get());
	...
	NewMesh->SetLODGroup_Unsafe(ExistingMeshDataPtr->LODGroup);
	...
	NewMesh->SetNaniteSettings(ExistingMeshDataPtr->ExistingNaniteSettings);

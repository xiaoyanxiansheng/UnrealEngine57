...
#include UE_INLINE_GENERATED_CPP_BY_NAME(MaterialGraphSchema)
...
UEdGraphNode* FMaterialGraphSchemaAction_NewNode::PerformAction(class UEdGraph* ParentGraph, UEdGraphPin* FromPin, const FVector2f& Location, bool bSelectNewNode/* = true*/)
{
	...
	if (NewExpression)
	{
		if (MaterialExpressionClass == UMaterialExpressionFunctionInput::StaticClass() && FromPin)
		{
			...
			SetFunctionInputType(CastChecked<UMaterialExpressionFunctionInput>(NewExpression), UMaterialGraphSchema::GetMaterialIOValueType(FromPin));
		}
...
{
	return SetFunctionInputType(FunctionInput, static_cast<EMaterialValueType>(MaterialValueType));
}

void FMaterialGraphSchemaAction_NewNode::SetFunctionInputType(UMaterialExpressionFunctionInput* FunctionInput, EMaterialValueType MaterialValueType) const
{
...
{
	EMaterialValueType InputType = GetMaterialIOValueType(InputPin);
	EMaterialValueType OutputType = GetMaterialIOValueType(OutputPin);
...
{
	return static_cast<uint32>(GetMaterialIOValueType(MaterialPin));
}

EMaterialValueType UMaterialGraphSchema::GetMaterialIOValueType(const UEdGraphPin* MaterialPin)
{
...
const FPinConnectionResponse UMaterialGraphSchema::CanCreateConnection(const UEdGraphPin* A, const UEdGraphPin* B) const
{
	...
	{
		const EMaterialValueType InputType = GetMaterialIOValueType(InputPin);
	...
	{
		const EMaterialValueType OutputType = GetMaterialIOValueType(InputPin);
...
FLinearColor GetColorForConnectionType(const UGraphEditorSettings* Settings, UE::Shader::EValueType ConnectionType)
{
	...
	else
	{
		...
		{
			// Tentatively use these two color pin settings for LWC inputs to make them stand out better in the graph, since LWC math is expensive.
			// We could in the future add unique color pin settings for these.
			if (TypeDesc.NumComponents == 1)
			{
				return Settings->SoftObjectPinTypeColor;	// Default cyan
			}
			else
			{
				return Settings->SoftClassPinTypeColor;		// Default pink
			}
		}
...
void UMaterialGraphSchema::DroppedAssetsOnGraph(const TArray<struct FAssetData>& Assets, const FVector2f& GraphPosition, UEdGraph* Graph) const
{
	...
	TSharedPtr<IAssetReferenceFilter> AssetReferenceFilter = FMaterialEditorUtilities::MakeMaterialAssetReferenceFilter(Graph);
...
void UMaterialGraphSchema::GetMaterialFunctionActions(FGraphActionMenuBuilder& ActionMenuBuilder) const
{
	...
	EMaterialValueType FromPinType{};
	if (ActionMenuBuilder.FromPin)
	{
		FromPinType = GetMaterialIOValueType(ActionMenuBuilder.FromPin);
	}
...
bool UMaterialGraphSchema::HasCompatibleConnection(const FAssetData& FunctionAssetData, EMaterialValueType TestType, EEdGraphPinDirection TestDirection) const
{
	if (TestType != 0)
	{
		auto CombinedInputTypes = static_cast<EMaterialValueType>(FunctionAssetData.GetTagValueRef<uint64>(GET_MEMBER_NAME_CHECKED(UMaterialFunctionInterface, CombinedInputTypes)));
		auto CombinedOutputTypes = static_cast<EMaterialValueType>(FunctionAssetData.GetTagValueRef<uint64>(GET_MEMBER_NAME_CHECKED(UMaterialFunctionInterface, CombinedOutputTypes)));
		...
		if (CombinedOutputTypes == 0)
		{
			...
			{
				CombinedInputTypes = static_cast<EMaterialValueType>(MaterialFunction->CombinedInputTypes);
				CombinedOutputTypes = static_cast<EMaterialValueType>(MaterialFunction->CombinedOutputTypes);
			}
		}
...
void UMaterialGraphSchema::GetAssetsGraphHoverMessage(const TArray<FAssetData>& Assets, const UEdGraph* HoverGraph, FString& OutTooltipText, bool& OutOkIcon) const
{
	...
	TSharedPtr<IAssetReferenceFilter> AssetReferenceFilter = FMaterialEditorUtilities::MakeMaterialAssetReferenceFilter(HoverGraph);
	for (const FAssetData& AssetData : Assets)
	{
		UObject* Asset = AssetData.GetAsset();
		UClass* MaterialExpressionClass = Cast<UClass>(Asset);
		...
		if ((MaterialExpressionClass != nullptr && MaterialExpressionClass->IsChildOf(UMaterialExpression::StaticClass())) ||
			Func != nullptr ||
			Tex != nullptr ||

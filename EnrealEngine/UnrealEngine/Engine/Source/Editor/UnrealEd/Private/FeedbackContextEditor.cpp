...
void FFeedbackContextEditor::StartSlowTask( const FText& Task, bool bShowCancelButton )
{
	...
	if (GEditor)
	{
		// Don't show a dialog in the middle of a drag/drop operation, as it will cause reentrancy into user
		// drag operations:
		if(FSlateApplication::Get().IsDragDropping())
		{
			UE_LOG(LogSlate, Log, TEXT("Slow task dialog suppressed because a drag operation is in progress"));
			return;
		}
		...
		// Attempt to parent the slow task window to the current active window
		// This is more likely to show the slow task window where it's relevant and the user is already looking, then return focus appropriately when destroyed
		if (!ParentWindow.IsValid())
		{
			ParentWindow = FSlateApplicationBase::Get().GetActiveTopLevelRegularWindow();
		}

		// If we still don't have a valid window, attempt to parent the slow task window to the slate main frame

...
bool UAnimSequenceExporterFBX::ExportBinary( UObject* Object, const TCHAR* Type, FArchive& Ar, FFeedbackContext* Warn, int32 FileIndex, uint32 PortFlags )
{
	...
	if (AnimSkeleton)
	{
		// first try to get already set preview mesh
		PreviewMesh = AnimSkeleton->GetAssetPreviewMesh(AnimSequence);
		if (!PreviewMesh)
		{
			// find compatible one (as used in persona toolkit)
			PreviewMesh = AnimSkeleton->FindCompatibleMesh();
		}
	}

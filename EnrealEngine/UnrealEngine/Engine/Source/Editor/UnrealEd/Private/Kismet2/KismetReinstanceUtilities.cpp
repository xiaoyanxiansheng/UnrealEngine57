...
void FBlueprintCompileReinstancer::ReplaceObjectHelper(UObject*& OldObject, UClass* OldClass, UObject*& NewUObject, UClass* NewClass, TMap<UObject*, UObject*>& OldToNewInstanceMap, const TMap<UClass*, UClass*>& OldToNewClassMap, TMap<UObject*, FName>& OldToNewNameMap, int32 OldObjIndex, TArray<UObject*>& ObjectsToReplace, TArray<UObject*>& PotentialEditorsForRefreshing, TSet<AActor*>& OwnersToRerunConstructionScript, TFunctionRef<TArray<TObjectPtr<USceneComponent>>&(USceneComponent*)> GetAttachChildrenArray, bool bIsComponent, bool bArchetypesAreUpToDate)
{
						...
						
	{
		...
		const bool bUsePrePopulatedInstancingGraph = PrePopulateInstancingGraphForArchetype(PrePopulatedInstancingGraph, OldObject, NewArchetype, &OldToNewInstanceMap);
		
		FMakeClassSpawnableOnScope TemporarilySpawnable(NewClass);
		NewUObject = NewObject<UObject>(DestinationOuter, NewClass, NewName, RF_NoFlags, NewArchetype, false, bUsePrePopulatedInstancingGraph ? &PrePopulatedInstancingGraph : nullptr, OldExternalPackage);
	}
		...
		for (TPair<UClass*, UClass*> OldToNewClass : InOldToNewClassMap)
		{
			...
			check(OldClass != NewClass || IsReloadActive());
			{
				...
				for (int32 OldObjIndex = 0; OldObjIndex < ObjectsToReplace.Num(); ++OldObjIndex)
				{
					...
					if (!bIsValid)
					{
						// the object is invalid, we cannot replace it but we can ensure any external references
						// to it are nulled out, this is necessary because we've given it a new (invalid) REINST
						// class.
						if(!OldToNewInstanceMap.Contains(OldObject))
						{
							OldToNewInstanceMap.Add(OldObject, nullptr);
						}
						continue;
					}
					else if (!bIsComponent && !bIsChildActorTemplate &&
...
bool FBlueprintCompileReinstancer::PrePopulateInstancingGraphForArchetype(FObjectInstancingGraph& InstancingGraph, TNotNull<UObject*> OldInstance, UObject* InArchetype, TMap<UObject*, UObject*>* OldToNewInstanceMap)
{
	...
	auto GatherMatchingSubObjects = [&InstancingGraph, OldInstance, OldToNewInstanceMap](UObject* Instance, UObject* Template, auto GatherMatchingSubObjectsRecurse) -> void
	{
		InstancingGraph.AddNewObject(Instance, Template);

		ForEachObjectWithOuter(Instance, [OldInstance, Template, GatherMatchingSubObjectsRecurse, OldToNewInstanceMap](UObject* SubObject)
		{
			...
			{
				// Handle the fact that subobject will be re-instantiated later by pointing its archetype to the new one.
				if (UObject** NewSubObjectArchetype = OldToNewInstanceMap ? OldToNewInstanceMap->Find(SubObjectArchetype) : nullptr)
				{
					SubObjectArchetype = *NewSubObjectArchetype;
				}
				else
				{
					UE_LOG(LogClass, VeryVerbose, TEXT("Missmatched while trying to rebuild instancing graph for '%s', expecting the outer of archetype %s to be %s but was %s"), 

...
UE_DEFINE_TCOMMANDS(FPlayWorldCommands)
...
void FPlayWorldCommands::BuildToolbar(FToolMenuSection& InSection)
{
		...
		// play in view port
		{
			...
			if (FPlayWorldCommands::Get().Simulate)
			{
				Section.AddMenuEntry(
					...
					FSlateIcon(FAppStyle::GetAppStyleSetName(), "PlayWorld.PlaySimulate"),
...
void FInternalPlayWorldCommandCallbacks::PlayInNewProcess_Clicked(EPlayModeType PlayModeType)
{
	check(PlayModeType == PlayMode_InNewProcess 
		|| PlayModeType == PlayMode_InMobilePreview
		|| PlayModeType == PlayMode_InTargetedMobilePreview);

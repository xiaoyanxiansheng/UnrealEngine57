...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FBlueprintEditorUtils::PreloadMembers);
	...
	for( TArray<UObject*>::TIterator it(BPMemberReferences); it; ++it )
	{
		...
		CurrentObject->ConditionalPreload();
...
struct FRegenerationHelper
{
	...
	{
		bool bChanged = false;
		if (Struct->HasAnyFlags(RF_NeedLoad) || Struct->HasAnyInternalFlags(EInternalObjectFlags::AsyncLoadingPhase1))
		{
			if (FLinkerLoad* Linker = Struct->GetLinker())
			{
				UE::TUniqueLock LinkerLock(Linker->Mutex);
				bChanged = Struct->HasAnyFlags(RF_NeedLoad);
...
bool FBlueprintEditorUtils::IsVariableCreatedByBlueprint(UBlueprint* InBlueprint, FProperty* InVariableProperty)
{
	...
	if (InBlueprint != nullptr && InVariableProperty != nullptr)
	{
...
{
	if(InName != NAME_None && Blueprint != nullptr)
	{
		...
		for (TMap<FName, FString>::TConstIterator It(*MetaData); It; ++It)
		{
			...
			else if (Key == FBlueprintMetadata::MD_WorldContext)
			{
				...
				if(!K2Schema->IsStaticFunctionGraph(Graph))
				{
					...
					if (!ShouldShowWorldContextPin(FindBlueprintForGraph(Graph)))
					{
						HiddenPins.Add(*It.Value());
					}
				}
...
bool FBlueprintEditorUtils::VerifyUserWantsToSearch(FText Reason, const FString& SearchString)
{
	const UBlueprintEditorSettings* LocalSettings = GetDefault<UBlueprintEditorSettings>();
	// if the user has requested search operations on variable type changes then trigger the 
	// search operation
	 if(LocalSettings->bSearchForReferencesWhenVariableTypeChanged)
	 {
		return true;
	 }

	// else just log the search string so the user can search themself if they care to:
	FText LocalizedMessage = FText::Format( 
		LOCTEXT( "SearchSkipped", "{0} - search skipped as requested by editor settings, consider searching \"{1}\""),
		Reason, FText::FromString(SearchString));
	UE_LOG(LogBlueprint, Warning, TEXT("%s"), *LocalizedMessage.ToString());
	...
	if (VariableName != NAME_None)
	{
		...
		if (VarIndex != INDEX_NONE)
		{
			...
			if (Variable.VarType != NewPinType)
			{
				...
				FText OperationIdentifier = LOCTEXT("ChangeVariableType", "Change Variable Type");
				const FScopedTransaction Transaction( OperationIdentifier );
				Blueprint->Modify();
					...
					if(bBreakingVariableConnections)
					{
						...
						if (FoundAssetEditor.IsValid())
						{
							...
							if (FirstVariableNode)
							{
								...
								const FString SearchString = FirstVariableNode->GetFindReferenceSearchString(Flags);
								// At scale indexing is slow, allow the user to disable it:
								if(VerifyUserWantsToSearch(OperationIdentifier, SearchString))
								{
									constexpr bool bSetFindWithinBlueprint = false;
									constexpr bool bSelectFirstResult = false;
									BlueprintEditor->SummonSearchUI(bSetFindWithinBlueprint, SearchString, bSelectFirstResult);
								}
							}
						}
					}
	...
	if (InVariableName != NAME_None)
	{
		...
		if(VariablePtr)
		{
			...
			if (Variable.VarName == InVariableName && Variable.VarType != NewPinType)
			{
				...
				FText OperationIdentifier = LOCTEXT("ChangeLocalVariableType", "Change Local Variable Type");
				const FScopedTransaction Transaction( OperationIdentifier );
				InBlueprint->Modify();
				FunctionEntry->Modify();
				...
				if (FoundAssetEditor.IsValid() && VariableNodes.Num())
				{
					...
					{
						constexpr EGetFindReferenceSearchStringFlags Flags = EGetFindReferenceSearchStringFlags::UseSearchSyntax;
						const FString SearchString = VariableNodes[0]->GetFindReferenceSearchString(Flags);
						// At scale indexing is slow, allow the user to disable it:
						if(VerifyUserWantsToSearch(OperationIdentifier, SearchString))
						{
...
static void ConformInterfaceByName(UBlueprint* Blueprint, FBPInterfaceDescription& CurrentInterfaceDesc, int32 InterfaceIndex, TArray<UK2Node_Event*>& ImplementedEvents, const TArray<FName>& VariableNamesUsedInBlueprint)
{
	...
	if (CurrentInterfaceDesc.Interface)
	{
		...
		for (UK2Node_Event* EventNode : ImplementedEvents)
		{
			...
			UFunction* InterfaceFunction = EventNode->EventReference.ResolveMember<UFunction>(FBlueprintEditorUtils::GetMostUpToDateClass(CurrentInterfaceDesc.Interface));
...
bool FBlueprintEditorUtils::RenameTimeline(UBlueprint* Blueprint, const FName OldNameRef, const FName NewName)
{
	...
	if ((!NewTemplate && bUniqueNameForTemplate) || NewTemplate == Template)
	{
		if (Template)
		{
			...
			UObject* ExistingObject = StaticFindObject(nullptr, Template->GetOuter(), *NewTemplateName, EFindObjectFlags::ExactClass);
...
bool FBlueprintEditorUtils::ShouldShowWorldContextPin(const UBlueprint* BP)
{
	const UClass* ParentClass = BP ? *BP->ParentClass : nullptr;

	if (ParentClass && ParentClass->HasMetaDataHierarchical(FBlueprintMetadata::MD_ShowWorldContextPin) != nullptr)
	{
		// Always show even if not needed
		return true;
	}

	if (BP && BP->BlueprintType == BPTYPE_FunctionLibrary)
	{
		// NOTE: This was the old behavior, setting it to false would hide the pin which could break existing blueprints
		return true;
	}

	return !ImplementsGetWorld(BP);
}

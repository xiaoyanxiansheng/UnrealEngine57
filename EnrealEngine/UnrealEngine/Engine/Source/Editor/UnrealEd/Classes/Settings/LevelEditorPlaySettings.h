...
enum EPlayModeType : int
{
	...
	PlayMode_InVulkanPreview UE_DEPRECATED(5.7, "PlayMode_InVulkanPreview is no longer supported, please use PlayMode_InMobilePreview instead"),
...
protected:
	...
	UPROPERTY(config, EditAnywhere, Category=PlayInEditor, meta = (InlineEditConditionToggle))
	bool bSimulateDeviceMappingPolicy = false;
	
	/**
	 * If enabled, then this simulated device mapping policy will be used instead of the one set in the Project Settings.
	 ...
	 */
	UPROPERTY(config, EditAnywhere, Category=PlayInEditor, meta=(EditCondition=bSimulateDeviceMappingPolicy))
	EInputDeviceMappingPolicy SimulatedDeviceMappingPolicy = EInputDeviceMappingPolicy::UseManagedPlatformLogin;

public:

	bool IsSimulateDeviceMappingPolicy() const { return bSimulateDeviceMappingPolicy; }

	EInputDeviceMappingPolicy GetSimulatedDeviceMappingPolicy() const { return SimulatedDeviceMappingPolicy; }

	TMulticastDelegate<void()> SimulatedDeviceMappingPolicyPostEditChange;

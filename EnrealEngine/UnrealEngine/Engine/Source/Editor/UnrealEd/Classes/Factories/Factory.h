...
/**
 * Determines which workflows are available for a factory.
 * All factories *should* support the default workflow!
 */
UENUM(meta = (Bitflags, UseEnumValuesAsMaskValuesInEditor = "true"))
enum class EFactoryCreateWorkflow : uint8
{
	/**
	 * Synchronous asset creation (potentially with a modal dialog).
	 */
	Default = 1 << 0,

	/**
	 * Asynchronous asset creation (usually with a dialog).
	 * If supported, the UFactory::ConfigureProperties will not be called, but UFactory::ConfigurePropertiesAsync.
	 */
	Asynchronous = 1 << 1
};
ENUM_CLASS_FLAGS(EFactoryCreateWorkflow);

DECLARE_DELEGATE_OneParam(FFactoryConfigurePropertiesComplete, UFactory* /* this */);
DECLARE_DELEGATE_OneParam(FFactoryConfigurePropertiesCancelled, UFactory* /* this */);
...
public:
	...
	/**
	 * Called instead of ConfigureProperties() when an asset is being created asynchronously.
	 * It is not the responsibility of the caller of this function to stop the factory from being GC'd. It is the
	 * responsibility of the implementor.
	 * @param OnComplete  Called when the asynchronous process completes successfully.
	 * @param OnCancelled Called when the asynchronous process was cancelled.
	 * @return true if the async process was successfully started.
	 */
	virtual bool ConfigurePropertiesAsync(FFactoryConfigurePropertiesComplete OnComplete, FFactoryConfigurePropertiesCancelled OnCancelled)
	{
		OnCancelled.Execute(this);
		return false;
	}
	...
	/**
	 * @return the supported factory workflows.
	 */
	virtual EFactoryCreateWorkflow GetSupportedWorkflows() const
	{
		return static_cast<EFactoryCreateWorkflow>(SupportedWorkflows);
	}
public:
...
public:
	...
	UPROPERTY(BlueprintReadWrite, Category=Misc, meta = (Bitmask))
	uint8 SupportedWorkflows = static_cast<uint8>(EFactoryCreateWorkflow::Default);

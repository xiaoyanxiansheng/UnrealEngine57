...
struct FMaterialGraphSchemaAction_NewNode : public FEdGraphSchemaAction
{
	 ...
	 */
	UE_DEPRECATED(5.8, "Use SetFunctionInputType(..., EMaterialValueType) overload instead")
	...
	UNREALED_API void SetFunctionInputType(class UMaterialExpressionFunctionInput* FunctionInput, EMaterialValueType MaterialValueType) const;
};
...
class UMaterialGraphSchema : public UEdGraphSchema
{
	...
	UE_DEPRECATED(5.8, "Use GetMaterialIOValueType instead")
	UNREALED_API static uint32 GetMaterialValueType(const UEdGraphPin* MaterialPin);
	UNREALED_API static EMaterialValueType GetMaterialIOValueType(const UEdGraphPin* MaterialPin);
...
private:
	...
	*/
	bool HasCompatibleConnection(const FAssetData& FunctionAssetData, EMaterialValueType TestType, EEdGraphPinDirection TestDirection) const;

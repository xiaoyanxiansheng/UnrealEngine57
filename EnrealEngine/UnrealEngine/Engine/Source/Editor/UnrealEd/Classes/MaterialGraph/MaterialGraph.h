...
struct FMaterialInputInfo
{
	...
	bool IsVisiblePin(const UMaterial* Material, bool bIgnoreMaterialAttributes = false) const
	{
		...
		const bool bSubstrateHiddenMaterialConversion = Substrate::IsHiddenMaterialAssetConversionEnabled();
		const bool bFrontMaterialConnected = Material->HasSubstrateFrontMaterialConnected();
		const bool bShowAsSubstrate = bSubstrateEnabled && (!bSubstrateHiddenMaterialConversion || bFrontMaterialConnected);

		if (!bSubstrateEnabled && Property == MP_FrontMaterial)
		{
			return false;
		}
		...
		{
			if (bSubstrateEnabled && bSubstrateHiddenMaterialConversion && Property == MP_FrontMaterial)
			{
		...
		else
		{
			...
			if (bShowAsSubstrate)
			{
				// The only properties visible when Substrate is enable and FrontMaterial pin is plugged.
				return Property == MP_Opacity

...
/**
 * Data and interface common to both Nanite and non-Nanite scene proxies
 */
class FDebugSkelMeshSceneProxyCommon
{
public:
	void GetDynamicMeshElements(const FPrimitiveSceneProxy* SceneProxy, const FSkeletalMeshObject* MeshObject, const TArray<const FSceneView*>& Views, const FSceneViewFamily& ViewFamily, uint32 VisibilityMap, FMeshElementCollector& Collector) const;

	FDebugSkelMeshDynamicData* DynamicData = nullptr;
};

/**
* A skeletal mesh component scene proxy with additional debugging options.
*/
class FDebugSkelMeshSceneProxy : public FSkeletalMeshSceneProxy, public FDebugSkelMeshSceneProxyCommon
{
public:
...
class FDebugSkelMeshSceneProxyNanite : public Nanite::FSkinnedSceneProxy, public FDebugSkelMeshSceneProxyCommon
{
public:
	FDebugSkelMeshSceneProxyNanite(const Nanite::FMaterialAudit& Materials, const UDebugSkelMeshComponent* InComponent, FSkeletalMeshRenderData* InSkelMeshRenderData, FLinearColor InWireframeOverlayColor = FLinearColor::White);
	...
	SIZE_T GetAllocatedSize() const
	{
		return Nanite::FSkinnedSceneProxy::GetAllocatedSize();
	}
...
class UDebugSkelMeshComponent : public USkeletalMeshComponent
{
	...
	*/
	...
	void SetVisualizeRootMotionMode(EVisualizeRootMotionMode Mode) { VisualizeRootMotionMode = Mode; };
	bool IsVisualizeRootMotionMode(EVisualizeRootMotionMode Mode) const { return VisualizeRootMotionMode == Mode; };
	EVisualizeRootMotionMode GetVisualizeRootMotionMode() const { return VisualizeRootMotionMode; };
	...
	void SetShowNotificationVisualizations(const bool bShow) { bShowNotificationVisualizations = bShow; }
	bool IsNotificationVisualizationsEnabled() const { return bShowNotificationVisualizations; }

	bool IsRootMotionVisualizationsEnabled() const { return VisualizeRootMotionMode != EVisualizeRootMotionMode::None; }

	/** Sets flags whether we AssetUserData visualizations should be drawn in the viewport. */
	void SetShowAssetUserDataVisualizations(const bool bShow) { bShowAssetUserDataVisualizations = bShow; }
	bool IsAssetUserDataVisualizationsEnabled() const { return bShowAssetUserDataVisualizations; }
...
public:
	...
	*/
	...
	/** Whether this component renders the Nanite fallback for previewing */
	bool IsDisplayingNaniteFallback() const
	{
		return bDisplayNaniteFallback;
	}

	/** Sets whether this component should render the Nanite fallback for previewing */
	void SetDisplayNaniteFallback(bool bValue)
	{
		if (bValue != bDisplayNaniteFallback)
		{
			bDisplayNaniteFallback = bValue;
			MarkRenderStateDirty();
		}
	}

private:
	bool bDisplayNaniteFallback = false;
};

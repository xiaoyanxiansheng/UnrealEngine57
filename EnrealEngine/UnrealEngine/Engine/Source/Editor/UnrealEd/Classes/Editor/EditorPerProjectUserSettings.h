...
class UEditorPerProjectUserSettings : public UObject
{
	...
	/** Camera Speed settings for the SCS viewport */
	UPROPERTY(config)
	FEditorViewportCameraSpeedSettings SCSViewportCameraSpeedSettings;

	/** How fast the SCS viewport camera moves */
	UPROPERTY(config, meta=(UIMin = "1", UIMax = "8", ClampMin="1", ClampMax="8"))
	int32 SCSViewportCameraSpeed_DEPRECATED;
...
public:
	...
	UPROPERTY(config)
	float PreviewConstrainedAspectRatio;

	/** Safe Zones of the preview platform */
	UPROPERTY(config)
	FVector4f PreviewSafeZones;

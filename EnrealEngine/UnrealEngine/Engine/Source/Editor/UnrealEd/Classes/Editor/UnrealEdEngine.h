...
public:
	...
	*/
	...
	/**
	 * Hide selected actors, their child hierarchy, and BSP models by marking their bHiddenEdTemporary flags true.
	 * Will not modify/dirty actors/BSP.
	 */
	UNREALED_API virtual void edactHideSelectedHierarchy( UWorld* InWorld );

	/**
	 * Toggles visibility of selected actors. If any selected actors are visible, they will be made invisible.
	 * If all selected actors are invisible, the selection will be made visible. Will not modify/dirty actors/BSP.
	 */
	UNREALED_API virtual void edactToggleSelectedVisibility( UWorld* InWorld );

	/**
	 * Toggles visibility of selected actors and their child hierarchy. If any selected actors are visible, they will be 
	 * made invisible. If all selected actors are invisible, the selection will be made visible. Will not modify/dirty
	 * actors/BSP.
	 */
	UNREALED_API virtual void edactToggleSelectedHierarchyVisibility( UWorld* InWorld );
	...
	/**
	 * Mark all selected actors, their child hierarchy, and BSP models to be hidden upon editor startup, by setting
	 * their bHiddenEd flag to true, if it is not already. This directly modifies/dirties the relevant actors/BSP.
	 */
	UNREALED_API virtual void edactHideSelectedHierarchyStartup( UWorld* InWorld );
	 ...
	 * is not already. This directly modifies/dirties the relevant actors/BSP.
	 */
	UNREALED_API virtual void edactUnHideSelectedStartup( UWorld* InWorld );

	/**
	 * Mark all selected actors, their child hierarchy, and BSP models to be shown upon editor startup, by setting their bHiddenEd
	 * flag to false, if it is not already. This directly modifies/dirties the relevant actors/BSP.
	 */
	UNREALED_API virtual void edactUnHideSelectedHierarchyStartup( UWorld* InWorld );
	...
	/**
	 * Show selected actors, their child hierarchy, and BSP models by marking their bHiddenEdTemporary flags false. Will
	 * not modify/dirty actors/BSP.
	 */
	UNREALED_API virtual void edactUnhideSelectedHierarchy( UWorld* InWorld );
...
private:
	/** 
	 * Helper function to recursively add the children of a given actor to the OutActorArray. This is used for Hide/Show Selected functions to
	 * hide/show their child hierarchy. Set bExcludeSelectedChildren to true when calling this function in a loop of all selected actors, we
	 * want the helper function to not travel down pre-selected children so there won't be duplicate entries and unnecessary complexity.
	 */
	static void GetChildHierarchyOfSelectedActor(TArray<AActor*>& OutActorArray, AActor* ParentActor, const bool bExcludeSelectedChildren = false, const TOptional<bool>& bIsFindingHidden = TOptional<bool>());
	
	/**
	 * Internal function to hide selected Actors, allows bApplyToChildHierarchy to define whether we also want to hide the
	 * child hierarchies of the selected actors.
	 */
	void Internal_HideSelected(UWorld* InWorld, bool bApplyToChildHierarchy);
	
	/**
	 * Internal function to unhide selected Actors, allows bApplyToChildHierarchy to define whether we also want to unhide
	 * the child hierarchies of the selected actors.
	 */
	void Internal_UnhideSelected(UWorld* InWorld, bool bApplyToChildHierarchy);
	
	/**
	 * Internal function to toggle the visibility of selected Actors, allows bApplyToChildHierarchy to define whether we also
	 * want to toggle the child hierarchies of the selected actors.
	 */
	void Internal_ToggleSelectedVisibility(UWorld* InWorld, bool bApplyToChildHierarchy);
	
	/**
	 * Internal function to hide selected Actors on startup, allows bApplyToChildHierarchy to define whether we also want to
	 * hide the current child hierarchies of the selected actors.
	 */
	void Internal_HideSelectedStartup(UWorld* InWorld, bool bApplyToChildHierarchy);
	
	/**
	 * Internal function to unhide selected Actors on startup, allows bApplyToChildHierarchy to define whether we also want
	 * to unhide the current child hierarchies of the selected actors.
	 */
	void Internal_UnHideSelectedStartup(UWorld* InWorld, bool bApplyToChildHierarchy);

...
{
	const TSharedPtr<FSequencer> Sequencer = StaticCastSharedPtr<FSequencer>(GetEditor()->GetSequencer());
	const bool bIsReadOnly = Sequencer->GetRootMovieSceneSequence()->GetMovieScene()->IsReadOnly();
	
	MenuBuilder.BeginSection("ObjectBindings"); // Create the section to allow extension
	if (!bIsReadOnly)
	{
	...
	MenuBuilder.BeginSection("AddTracks"); // Create the section to allow extension
	if (!bIsReadOnly)
	{
...
bool FFolderModel::IsMuted() const
{
	UMovieSceneFolder* Folder = WeakFolder.Get();
	if (!Folder)
	{
	...
	if (UMovieSceneMuteSoloDecoration* MuteSoloDecoration = Folder->FindDecoration<UMovieSceneMuteSoloDecoration>())
	{
...
void FFolderModel::SetIsMuted(bool bIsMuted)
{
	UMovieSceneFolder* Folder = WeakFolder.Get();
	if (!Folder)
	{
	...
	Folder->Modify(bAlwaysMarkDirty);

	if (UMovieSceneMuteSoloDecoration* MuteSoloDecoration = Folder->GetOrCreateDecoration<UMovieSceneMuteSoloDecoration>())
	{
		MuteSoloDecoration->SetMuted(bIsMuted);
	}
}

bool FFolderModel::IsSolo() const
{
	UMovieSceneFolder* Folder = WeakFolder.Get();
	if (!Folder)
	{
...
void FFolderModel::SetIsSoloed(bool bIsSoloed)
{
	UMovieSceneFolder* Folder = WeakFolder.Get();
	if (!Folder)
	{

...
void FSectionModel::InitializeSection(TSharedPtr<ISequencerSection> InSectionInterface)
{
	SectionInterface = InSectionInterface;
}

void FSectionModel::InitializeObject(TWeakObjectPtr<> InWeakObject)
{
	UMovieSceneSection* Section = CastChecked<UMovieSceneSection>(InWeakObject.Get());

	WeakSection = Section;

	Section->EventHandlers.Link(this);
	Section->UMovieSceneSignedObject::EventHandlers.Link(this);

	UpdateCachedData();
}

UObject* FSectionModel::GetObject() const
{
...
FTrackLaneVirtualAlignment FSectionModel::ArrangeVirtualTrackLaneView() const
{
	...
	TSharedPtr<const IViewSpaceClientExtension> ViewSpaceClient = CastThisShared<const IViewSpaceClientExtension>();

	FGuid ViewSpaceID = ViewSpaceClient ? ViewSpaceClient->GetViewSpaceID() : FGuid();
	return FTrackLaneVirtualAlignment::Proportional(SectionRange, 1.f, VAlign_Center, ViewSpaceID);
}

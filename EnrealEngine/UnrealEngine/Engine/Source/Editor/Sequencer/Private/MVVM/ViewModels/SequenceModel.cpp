...
FSequenceModel::FSequenceModel(TWeakPtr<FSequencerEditorViewModel> InEditorViewModel)
	...
	, DecorationModelList(GetDecorationModelListType())
{
	WeakEditor = InEditorViewModel;

	RegisterChildList(&RootOutlinerItems);
	RegisterChildList(&DecorationModelList);

	TSharedPtr<FSharedViewModelData> NewSharedData = MakeShared<FEditorSharedViewModelData>(WeakEditor.Pin().ToSharedRef());
	SetSharedDataFromConstructor(NewSharedData);
}

EViewModelListType FSequenceModel::GetDecorationModelListType()
{
	static EViewModelListType DecorationModelList = RegisterCustomModelListType();
	return DecorationModelList;
}

void FSequenceModel::InitializeExtensions()
{
	TSharedPtr<FSharedViewModelData> ThisSharedData = GetSharedData();

	// Re-generate hierarchical caches when the sequence changes
	ThisSharedData->AddDynamicExtension(FOutlinerCacheExtension::ID);
	ThisSharedData->AddDynamicExtension(FDeactiveStateCacheExtension::ID);
	ThisSharedData->AddDynamicExtension(FMuteStateCacheExtension::ID);
	ThisSharedData->AddDynamicExtension(FSoloStateCacheExtension::ID);
	ThisSharedData->AddDynamicExtension(FLockStateCacheExtension::ID);
	ThisSharedData->AddDynamicExtension(FConditionStateCacheExtension::ID);

	// Add our hierarchical cache processor
	TSharedPtr<FOutlinerCacheExtension> OutlinerCache = ThisSharedData->CastThisSharedChecked<FOutlinerCacheExtension>();
	OutlinerCache->Initialize(AsShared());

	// Make sure the hierarchical cache is updated when our hierarchy changes
	FSimpleMulticastDelegate& HierarchyChanged = ThisSharedData->SubscribeToHierarchyChanged(AsShared());
...
void FSequenceModel::OnDecorationAdded(UObject* AddedDecoration)
{
	static ISequencerCoreModule& SequencerCore = FModuleManager::Get().LoadModuleChecked<ISequencerCoreModule>("SequencerCore");

	TSharedPtr<FViewModel> NewModel = SequencerCore.FactoryNewModel(AddedDecoration);
	if (NewModel && NewModel->IsA<IObjectModelExtension>())
	{
		GetChildrenForList(&DecorationModelList).AddChild(NewModel);
	}
}

void FSequenceModel::OnDecorationRemoved(UObject* RemovedDecoration)
{
	for (TViewModelPtr<IObjectModelExtension> Object : GetChildrenForList(&DecorationModelList).IterateSubList<IObjectModelExtension>())
	{
		if (Object->GetObject() == RemovedDecoration)
		{
			Object.AsModel()->RemoveFromParent();
			break;
		}
	}
}
...
void FSequenceModel::SetSequence(UMovieSceneSequence* InSequence, FMovieSceneSequenceID InSequenceID)
{
	if (WeakSequence.Get() != InSequence || SequenceID != InSequenceID)
	{
		...
		MovieSceneDataEventHandler.Unlink();
		...
		if (InSequence)
		{
			...
			InSequence->GetMovieScene()->EventHandlers.Link(MovieSceneDataEventHandler, this);
		}

		ReinitializeExtensions();

		GetChildrenForList(&DecorationModelList).Empty();
		if (InSequence)
		{
			for (UObject* Decoration : InSequence->GetMovieScene()->GetDecorations())
			{

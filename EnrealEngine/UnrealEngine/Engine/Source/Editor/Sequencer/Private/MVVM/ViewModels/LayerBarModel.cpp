...
FTrackLaneVirtualAlignment FLayerBarModel::ArrangeVirtualTrackLaneView() const
{
	...
	FTrackLaneVirtualAlignment Result = FTrackLaneVirtualAlignment::Fixed(Range, 10.f, VAlign_Top);
	...
	if (TrackArea && OutlinerItem)
	{
		...
		Result.VerticalAlignment.VSizeParam = OutlinerItem->GetOutlinerSizing().GetTotalHeight();
		Result.VerticalAlignment.VPadding   = Parameters.TrackLanePadding.Top + Parameters.TrackLanePadding.Bottom;
	}

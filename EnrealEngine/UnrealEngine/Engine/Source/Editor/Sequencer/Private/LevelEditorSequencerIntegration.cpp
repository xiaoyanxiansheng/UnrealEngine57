...
#include UE_INLINE_GENERATED_CPP_BY_NAME(LevelEditorSequencerIntegration)
...
static TAutoConsoleVariable<bool> CVarEnableHitProxyInvalidationDuringEvaluation(
	TEXT("LevelSequence.EnableHitProxyInvalidationDuringEvaluation"),
	false,
	TEXT("Whether Sequencer evaluation should cause the viewport's hit proxies to be invalidated. If enabled, this causes FPS drops while moving the mouse in the viewport while playing back Sequencer.")
	);
...
void FLevelEditorSequencerIntegration::OnSequencerEvaluated()
{
	...
	for (FEditorViewportClient* LevelVC : GEditor->GetLevelViewportClients())
	{
		if (LevelVC)
		{
			...
			// Summary: When Sequencer evaluates moving the mouse in the viewport caused a drop in FPS.
			// This fix is for 5.7, related to UE-320241.
			// This CVar exists only for safety, since this change is coming in late to 5.7. In 5.8, it should be removed (replace "bInvalidateHitProxies" with "false").
			// The cause WAS that every frame, hit proxies were invalidated due to the below FEditorViewportClient::Invalidate call.
			// Invalidating hit proxies is expensive because it causes the game thread to call FlushRenderingCommands (FViewport::GetRawHitProxyData).
			// After evaluation, it should be enough just to redraw the viewport (the rendered content) and skip the hit proxies. 
			// Hit proxies are still invalidated automatically by FSlateApplication when it processes a mouse event.
			const bool bInvalidateHitProxies = CVarEnableHitProxyInvalidationDuringEvaluation.GetValueOnAnyThread();
			LevelVC->Invalidate(true, bInvalidateHitProxies);
		}
	}
...
void FLevelEditorSequencerIntegration::BindDetailHandler(const FLevelEditorSequencerIntegrationOptions& Options)
{
	...
	// run the handler for any open details panels.
	ActivateDetailHandler();

...
void FResizeSection::OnDrag(const FPointerEvent& MouseEvent, FVector2D LocalMousePos, const UE::Sequencer::FVirtualTrackArea& VirtualTrackArea)
{
	...
	// Snapping
	const bool bSnapEnabled = (Settings->GetIsSnapEnabled() || MouseEvent.IsShiftDown());
	const bool bSnapToInterval = Settings->GetForceWholeFrames();
	const bool bSnapToLikeTypes = bSnapEnabled && Settings->GetSnapSectionTimesToElements();
	
	SnapField.GetValue().SetSnapToInterval(bSnapToInterval);
	SnapField.GetValue().SetSnapToLikeTypes(bSnapToLikeTypes);

	if (bSnapEnabled || bSnapToInterval)
	{
...
void FMoveKeysAndSections::OnDrag(const FPointerEvent& MouseEvent, FVector2D LocalMousePos, const UE::Sequencer::FVirtualTrackArea& VirtualTrackArea)
{
	...
	const bool bSnapToLikeTypes = bSnapEnabled && ((KeysAsArray.Num() > 0 && Settings->GetSnapKeyTimesToElements()) || (Sections.Num() > 0 && Settings->GetSnapSectionTimesToElements()));
	...
	HandleKeyMovement(MouseEvent, MaxDeltaX, MouseDeltaTime);
...
void FMoveKeysAndSections::OnEndDrag(const FPointerEvent& MouseEvent, FVector2D LocalMousePos, const UE::Sequencer::FVirtualTrackArea& VirtualTrackArea)
{
	...
	for (const FSequencerSelectedKey& Key : KeysAsArray)
	{
		KeyHandles.Add(Key.KeyHandle);
	}
	const FKeySelection& KeySelection = Sequencer.GetViewModel()->GetSelection()->KeySelection;
	SequencerHelpers::RemoveDuplicateKeys(KeySelection, MakeArrayView(KeyHandles));
...
void FMoveKeysAndSections::HandleKeyMovement(const FPointerEvent& MouseEvent, TOptional<FFrameNumber> MaxDeltaX, FFrameNumber DesiredDeltaX)
{
	...
	const bool bSnapPlayTimeToDraggedKey = 
		((Settings->GetIsSnapEnabled() && Settings->GetSnapPlayTimeToDraggedKey()) || (MouseEvent.IsShiftDown() && MouseEvent.IsMouseButtonDown(EKeys::LeftMouseButton))) &&
		CurrentKeyTimes.Num();
	if (bSnapPlayTimeToDraggedKey)
	{

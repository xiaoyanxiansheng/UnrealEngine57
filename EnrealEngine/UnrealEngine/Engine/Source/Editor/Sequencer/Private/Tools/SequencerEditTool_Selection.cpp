	...
	: ISequencerEntityVisitor
{
	...
	virtual void VisitKeys(const UE::Sequencer::TViewModelPtr<UE::Sequencer::FChannelModel>& Channel, const TRange<FFrameNumber>& VisitRangeFrames) const override
	{
		...
		for (int32 Index = 0; Index < KeyHandlesScratch.Num(); ++Index)
		{
			...
			const bool bPrioritizeLayerBars = ModifierKeys.IsControlDown() && ModifierKeys.IsAltDown();
			const bool bPrioritizeKeys = !bPrioritizeLayerBars && !ModifierKeys.IsControlDown();
			const bool bKeyIsSelected = ExistingSelection.Contains(KeyHandlesScratch[Index]);
				...
				(!bKeyIsSelected && SetStateTo == ESelectionPreviewState::Selected)))
			{
				...
				else if (bPrioritizeLayerBars)
				{
					SelectionPreview.EmptyDefinedKeyStates();
				}
			}
	...
	virtual void VisitDataModel(UE::Sequencer::FViewModel* DataModel) const override
	{
		...
		const bool bPrioritizeLayerBars = ModifierKeys.IsControlDown() && ModifierKeys.IsAltDown();
		const bool bPrioritizeKeys = !bPrioritizeLayerBars && !ModifierKeys.IsControlDown();
		const bool bKeyStateCheck = bPrioritizeKeys ? SelectionPreview.GetDefinedKeyStates().Num() == 0 : true;
...
public:
	...
	FScrubTimeDragOperation(FSequencer& InSequencer, UE::Sequencer::STrackAreaView& InTrackArea)
		: Sequencer(InSequencer), NextPlayerStatus(EMovieScenePlayerStatus::Stopped)
	{}
...
public:
	...
	virtual void OnBeginDrag(const FPointerEvent& MouseEvent, FVector2D LocalMousePos, const UE::Sequencer::FVirtualTrackArea& VirtualTrackArea) override
	{
		...
		NextPlayerStatus = (Sequencer.GetPlaybackStatus() == EMovieScenePlayerStatus::Playing) ? EMovieScenePlayerStatus::Playing :
	...
	virtual void OnEndDrag(const FPointerEvent& MouseEvent, FVector2D LocalMousePos, const UE::Sequencer::FVirtualTrackArea& VirtualTrackArea) override
	{
		...
		Sequencer.SetPlaybackStatus(NextPlayerStatus);
	}
...
private:
	...
	EMovieScenePlayerStatus::Type NextPlayerStatus; //reset on end drag, make sure to keep playing if it was
};
...
static FFrameTime ComputeFrameTimeFromMouse(const FSequencer& Sequencer, const FGeometry& Geometry, const FVector2D& ScreenSpacePosition, const FSequencerTimeSliderController::FScrubRangeToScreen& RangeToScreen, bool  bCheckSnapping)
{
	...
	if (bCheckSnapping && Sequencer.GetSequencerSettings()->GetForceWholeFrames())
	{
		FFrameNumber        SnappedFrameNumber = (MouseValue * Sequencer.GetFocusedDisplayRate()).FloorToFrame();
		FQualifiedFrameTime RoundedPlayFrame = FQualifiedFrameTime(SnappedFrameNumber, Sequencer.GetFocusedDisplayRate());
		return RoundedPlayFrame.ConvertTo(Sequencer.GetFocusedTickResolution());
	}
	else
	{
		return MouseValue * Sequencer.GetFocusedTickResolution();
	}
}
...
FReply FSequencerEditTool_Selection::OnMouseButtonDown(SWidget& OwnerWidget, const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	{
		if (bIsScrubbingTime && Sequencer.GetSequencerSettings()->GetScrubTimeStartFromCursor())
		{

			FSequencerTimeSliderController::FScrubRangeToScreen RangeToScreen = FSequencerTimeSliderController::FScrubRangeToScreen(Sequencer.GetViewRange(), MyGeometry.Size);
			FFrameTime FrameTime = ComputeFrameTimeFromMouse(Sequencer, MyGeometry, MouseEvent.GetScreenSpacePosition(), RangeToScreen, true);

			if (Sequencer.GetPlaybackStatus() == EMovieScenePlayerStatus::Playing)
			{
				Sequencer.SetPlaybackStatus(EMovieScenePlayerStatus::Paused);
				Sequencer.SetLocalTimeLooped(FrameTime);

...
{
	FGuid ObjectBindingID = GetSequencer()->GetHandleToObject(Object, false);
	if (ObjectBindingID.IsValid() || !bCreateHandleIfMissing)
	{
		return FFindOrCreateHandleResult{ ObjectBindingID, false };
	}

	FFindOrCreateHandleResult Result;
	Result.Handle      = GetSequencer()->GetHandleToObject(Object, true, CreatedFolderName);
	Result.bWasCreated = Result.Handle.IsValid();
	return Result;
}

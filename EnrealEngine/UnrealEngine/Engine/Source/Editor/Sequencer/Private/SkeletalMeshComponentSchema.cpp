...
void FSkeletalMeshComponentSchema::BindAnimationInstance(TWeakPtr<ISequencer> WeakSequencer, TArray<USkeletalMeshComponent*> Components, EAnimInstanceLocatorFragmentType Type) const
{
	...
	for (const TPair<FGuid, USkeletalMeshComponent*>& Pair : AllComponentGuids)
	{
		...
		FGuid PossessableGuid = MovieScene->AddPossessable(NameString, AnimClass ? AnimClass : UAnimInstance::StaticClass());
		BindingReferences->AddBinding(PossessableGuid, CopyTemp(AnimInstanceLocator));

		FMovieScenePossessable* NewPossessable = MovieScene->FindPossessable(PossessableGuid);

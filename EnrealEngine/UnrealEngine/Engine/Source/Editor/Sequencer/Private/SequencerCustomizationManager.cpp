...
static TAutoConsoleVariable<bool> CVarExactClassCustomizationsOnly(
	TEXT("Sequencer.ExactClassCustomizationsOnly"),
	false,
	TEXT("Enable/disable sequencer customizations with exact class matches only"));
...
void FSequencerCustomizationManager::GetSequencerCustomizations(UMovieSceneSequence* FocusedSequence, TArray<TUniquePtr<ISequencerCustomization>>& OutCustomizations)
{
    ...
    UClass* SequenceClass = FocusedSequence->GetClass();

	const FCustomizationRegistryEntry* FoundEntry = nullptr;

	if (CVarExactClassCustomizationsOnly->GetBool())
	{
		FoundEntry = CustomizationRegistryEntries.FindByPredicate(
	...
	{
		UClass* CurrentClass = SequenceClass;
		while (CurrentClass != UObject::StaticClass())
		{
			FoundEntry = CustomizationRegistryEntries.FindByPredicate(
				[=](const FCustomizationRegistryEntry& Entry)
				{
					return Entry.SequenceClass == CurrentClass;
				});

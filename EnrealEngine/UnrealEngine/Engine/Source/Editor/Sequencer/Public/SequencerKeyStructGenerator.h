...
public:
	...
	UE_API UMovieSceneKeyStructType(const FObjectInitializer& ObjInit);

	UE_API void InitializeStruct(void* InDest, int32 ArrayDim) const override;
	UE_API void DestroyStruct(void* Dest, int32 ArrayDim) const override;
 ...
 */
class FSequencerKeyStructGenerator : FGCObject
{
public:
	 ...
	 */
	static UE_API UMovieSceneKeyStructType* AllocateNewKeyStruct(UScriptStruct* ChannelType);


	/**
	 * Finalize the specified struct type by prepending a time property and linking the struct
	 */
	static UE_API void FinalizeNewKeyStruct(UMovieSceneKeyStructType* InStruct);


	/**
	 * Helper function to locate an array property with the specified meta-data tag
	 */
	static UE_API FArrayProperty* FindArrayPropertyWithTag(UScriptStruct* ChannelStruct, FName MetaDataTag);
...
public:
	 ...
	 */
	UE_API void AddGeneratedStruct(FName InstancedStructName, UMovieSceneKeyStructType* Struct);
	 ...
	 */
	UE_API UMovieSceneKeyStructType* FindGeneratedStruct(FName InstancedStructName);
	 ...
	 */
	UE_API UMovieSceneKeyStructType* DefaultInstanceGeneratedStruct(UScriptStruct* ChannelType);
...
private:
	 ...
	 */
	UE_API TSharedPtr<FStructOnScope> CreateInitialStructInstance(const void* SourceChannel, UMovieSceneKeyStructType* GeneratedStructType, int32 InitialKeyIndex);

...
{
public:
	UE_API FSequencerCustomizationManager();

	/**
	 * Registers a sequencer customization against a specific type of sequence.
	 */
	UE_API void RegisterInstancedSequencerCustomization(const UClass* SequenceClass, FOnGetSequencerCustomizationInstance GetCustomizationDelegate);
	/**
	 * Unregisters a sequencer customization that was previously registered against a specific type of sequence.
	 */
	UE_API void UnregisterInstancedSequencerCustomization(const UClass* SequenceClass);

	/**
	 * Gets the sequencer customizations for the given sequence.
	 */
	UE_API void GetSequencerCustomizations(UMovieSceneSequence* FocusedSequence, TArray<TUniquePtr<ISequencerCustomization>>& OutCustomizations);

	/**
	 * Determines whether the new edited sequence requires changing the customizations.
	 */
	UE_API bool NeedsCustomizationChange(const UMovieSceneSequence* OldFocusedSequence, const UMovieSceneSequence* NewFocusedSequence) const;

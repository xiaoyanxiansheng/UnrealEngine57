...
public:
	...
	/** The display name text to use in the UX. Currently only used for sorting. */
	 ...
	 */
	virtual void ProcessKeyOperation(FFrameNumber InKeyTime, const UE::Sequencer::FKeyOperation& Operation, ISequencer& InSequencer, TArray<UE::Sequencer::FAddKeyResult>* OutResults = nullptr) { Operation.ApplyDefault(InKeyTime, InSequencer, OutResults); }
	...
	*/
	...
	/**
	 * Builds up the sequencer's "Add Track" menu. for menu items pinned to the top
	 *
	 * @param MenuBuilder The menu builder to change.
	 */
	virtual void BuildPinnedAddTrackMenu(FMenuBuilder& MenuBuilder) = 0;
	...
	/**
	 * Builds a widget for the add column for outliner nodes which represent tracks which are edited by this editor.
	...
	/**
	 * Builds a widget for the edit column for outliner nodes which represent tracks which are edited by this editor.
	 * @param ObjectBinding The object binding associated with the track being edited by this editor.
	 * @param Track The track being edited by this editor.
	 * @param Params Parameter struct containing data relevant to the edit widget
	 * @returns The the widget to display in the outliner, or an empty shared ptr if not widget is to be displayed.
	 */
	virtual TSharedPtr<SWidget> BuildOutlinerEditColumnWidget(const FGuid& ObjectBinding, UMovieSceneTrack* Track, const FBuildEditWidgetParams& Params) = 0;

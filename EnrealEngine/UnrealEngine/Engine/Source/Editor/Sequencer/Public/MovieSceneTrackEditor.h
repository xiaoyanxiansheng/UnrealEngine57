...
public:
	...
	/** Constructor */
	UE_API FMovieSceneTrackEditor(TSharedRef<ISequencer> InSequencer);

	/** Destructor */
	UE_API virtual ~FMovieSceneTrackEditor();

public:

	/** @return The current movie scene */
	UE_API UMovieSceneSequence* GetMovieSceneSequence() const;

	/**
	 * @return The current local time at which we should add a key
	 */
	UE_API FFrameNumber GetTimeForKey();
	 ...
	 * one after the other.
	 */
	static UE_API void BeginKeying(FFrameNumber InFrameNumber);
	static UE_API void EndKeying();

	UE_API void UpdatePlaybackRange();

	UE_API void AnimatablePropertyChanged( FOnKeyProperty OnKeyProperty );
	...
	UE_API FFindOrCreateHandleResult FindOrCreateHandleToObject( UObject* Object, bool bCreateHandleIfMissing = true, const FName& CreatedFolderName = NAME_None );
	...
	UE_API FFindOrCreateTrackResult FindOrCreateTrackForObject( const FGuid& ObjectHandle, TSubclassOf<UMovieSceneTrack> TrackClass, FName PropertyName = NAME_None, bool bCreateTrackIfMissing = true );
	...
	FFindOrCreateRootTrackResult<TrackClass> FindOrCreateRootTrack()
	{
		...
		Result.Track = MovieScene->template FindTrack<TrackClass>();
		bTrackExisted = Result.Track != nullptr;

		if (Result.Track == nullptr)
		{
			Result.Track = MovieScene->template AddTrack<TrackClass>();
		}
...
public:
	...
	UE_API virtual void BindCommands(TSharedRef<FUICommandList> SequencerCommandBindings) override;
	UE_API virtual void BuildPinnedAddTrackMenu(FMenuBuilder& MenuBuilder) override;
	UE_API virtual void BuildAddTrackMenu(FMenuBuilder& MenuBuilder) override;
	UE_API virtual void BuildObjectBindingColumnWidgets(TFunctionRef<TSharedRef<SHorizontalBox>()> GetEditBox, const UE::Sequencer::TViewModelPtr<UE::Sequencer::FObjectBindingModel>& ObjectBinding, const UE::Sequencer::FCreateOutlinerViewParams& InParams, const FName& InColumnName) override;
	...
	UE_API virtual void BuildTrackSidebarMenu(FMenuBuilder& MenuBuilder, UMovieSceneTrack* Track) override;
...
protected:
	 ...
	 */
	UE_API UMovieScene* GetFocusedMovieScene() const;

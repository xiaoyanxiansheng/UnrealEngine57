...
public:
	 ...
	 */
	UE_API IKeyArea(TWeakPtr<ISequencerSection> InSection, FMovieSceneChannelHandle InChannel, const FCurveModelID& InCurveId);
	 ...
	 * @param InChannel Handle to the channel this key area represents
	 */
	UE_API void Reinitialize(TWeakPtr<ISequencerSection> InSection, FMovieSceneChannelHandle InChannel);
...
public:
	 ...
	 */
	UE_API ISequencerChannelInterface* FindChannelEditorInterface() const;
	 ...
	 */
	UE_API FMovieSceneChannel* ResolveChannel() const;
	 ...
	 */
	UE_API void SetName(FName InName);
	 ...
	 */
	UE_API UMovieSceneSection* GetOwningSection() const;
	 ...
	 */
	UE_API bool ShouldShowCurve() const;
...
public:
	 ...
	 */
	UE_API FKeyHandle AddOrUpdateKey(FFrameNumber Time, const FGuid& ObjectBindingID, ISequencer& InSequencer);
	 ...
	 */
	UE_API void DeleteKeys(TArrayView<const FKeyHandle> InHandles, FFrameNumber InTime);
	 ...
	 */
	UE_API FKeyHandle DuplicateKey(FKeyHandle InKeyHandle) const;
	 ...
	 */
	UE_API void GetKeyTimes(TArrayView<const FKeyHandle> InKeyHandles, TArrayView<FFrameNumber> OutTimes) const;
	 ...
	 * @param WithinRange      (Optional) A predicate range to search within
	 */
	UE_API void GetKeyInfo(TArray<FKeyHandle>* OutHandles, TArray<FFrameNumber>* OutTimes, const TRange<FFrameNumber>& WithinRange = TRange<FFrameNumber>::All()) const;
	 ...
	 */
	UE_API void SetKeyTimes(TArrayView<const FKeyHandle> InKeyHandles, TArrayView<const FFrameNumber> InKeyTimes) const;
...
public:
	 ...
	 */
	UE_API void DrawKeys(TArrayView<const FKeyHandle> InKeyHandles, TArrayView<FKeyDrawParams> OutKeyDrawParams);
	 ...
	 */
	UE_API void CopyKeys(FMovieSceneClipboardBuilder& ClipboardBuilder, TArrayView<const FKeyHandle> KeyMask) const;
	 ...
	 */
	UE_API int32 DrawExtra(const FSequencerChannelPaintArgs& PaintArgs, int32 LayerId) const;
	 ...
	 */
	UE_API TArray<FKeyHandle> PasteKeys(const FMovieSceneClipboardKeyTrack& KeyTrack, const FMovieSceneClipboardEnvironment& SrcEnvironment, const FSequencerPasteEnvironment& DstEnvironment);
	 ...
	 */
	UE_API TUniquePtr<FCurveModel> CreateCurveEditorModel(TSharedRef<ISequencer> InSequencer) const;

	/**
	 * Retrieve the interpolation mode for this key area's channel for the specified time
	 */
	UE_API EMovieSceneKeyInterpolation GetInterpolationMode(FFrameNumber InTime, EMovieSceneKeyInterpolation DefaultMode) const;
...
public:
	 ...
	 */
	UE_API TSharedPtr<FStructOnScope> GetKeyStruct(FKeyHandle KeyHandle) const;

	/**
	 * Check whether this key area can create an editor on the sequencer node tree
	 */
	UE_API bool CanCreateKeyEditor() const;
	 ...
	 */
	UE_API TSharedRef<SWidget> CreateKeyEditor(TWeakPtr<ISequencer> Sequencer, const FGuid& ObjectBindingID);

...
{
	return GetDivider(InputMinX, InputMaxX, UE::Slate::CastToVector2f(WidgetSize), SequenceLength, NumFrames);
}

/** This function is used by a few random widgets and is mostly arbitrary. It could be moved anywhere. */
int32 SScrubWidget::GetDivider(float InputMinX, float InputMaxX, FVector2f WidgetSize, float SequenceLength, int32 NumFrames)
{

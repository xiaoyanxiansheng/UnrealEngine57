...
void SPinTypeSelector::Construct(const FArguments& InArgs, FGetPinTypeTree GetPinTypeTreeFunc)
{
	...
	[
		...
		// Don't show the read-only widget when in Compact mode
		.WidgetIndex_Lambda([this](){return ReadOnly.Get() && SelectorType != ESelectorType::Compact ? 1 : 0; })
		+ SWidgetSwitcher::Slot() // editable version
		// Apply additional padding to Partial and Full selector type to prevent padding jumps
		.Padding(SelectorType == ESelectorType::Partial || SelectorType == ESelectorType::Full ? FMargin(-6.0f, 0.0f,0.0f,0.0f) : FMargin(0))
		[
			Widget.ToSharedRef()
		]

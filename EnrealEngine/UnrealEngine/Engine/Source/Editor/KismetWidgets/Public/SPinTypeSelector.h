...
{
public:
	static UE_API TSharedRef<SWidget> ConstructPinTypeImage(const FSlateBrush* PrimaryIcon, const FSlateColor& PrimaryColor, const FSlateBrush* SecondaryIcon, const FSlateColor& SecondaryColor, TSharedPtr<SToolTip> InToolTip);
	static UE_API TSharedRef<SWidget> ConstructPinTypeImage(TAttribute<const FSlateBrush*> PrimaryIcon, TAttribute<FSlateColor> PrimaryColor, TAttribute<const FSlateBrush*> SecondaryIcon, TAttribute<FSlateColor> SecondaryColor );
	static UE_API TSharedRef<SWidget> ConstructPinTypeImage(UEdGraphPin* Pin);
...
protected:
	...
	UE_API ECheckBoxState IsArrayChecked() const;
	UE_API void OnArrayCheckStateChanged(ECheckBoxState NewState);

	/** Toggles the variable type as an array */
	UE_API void OnArrayStateToggled();

	/** Updates the variable container type: */
	UE_API void OnContainerTypeSelectionChanged(EPinContainerType PinContainerType);
	...
	UE_API virtual TSharedRef<ITableRow> GenerateTypeTreeRow(FPinTypeTreeItem InItem, const TSharedRef<STableViewBase>& OwnerTree, bool bForSecondaryType);
	UE_API void OnTypeSelectionChanged(FPinTypeTreeItem Selection, ESelectInfo::Type SelectInfo, bool bForSecondaryType);
	UE_API void GetTypeChildren(FPinTypeTreeItem InItem, TArray<FPinTypeTreeItem>& OutChildren);

	/** Listview support functions for sub-menu */
	UE_API TSharedRef<ITableRow> GenerateObjectReferenceTreeRow(FObjectReferenceListItem InItem, const TSharedRef<STableViewBase>& OwnerTree);
	UE_API void OnObjectReferenceSelectionChanged(FObjectReferenceListItem InItem, ESelectInfo::Type SelectInfo, bool bForSecondaryType);

	/** Reference to the menu content that's displayed when the type button is clicked on */
	TSharedPtr<SMenuOwner> MenuContent;
	UE_API virtual TSharedRef<SWidget>	GetMenuContent(bool bForSecondaryType);
	UE_API TSharedRef<SWidget> GetPinContainerTypeMenuContent();

	/** Type searching support */
	FText SearchText;
	UE_API void OnFilterTextChanged(const FText& NewText);
	UE_API void OnFilterTextCommitted(const FText& NewText, ETextCommit::Type CommitInfo);

	/** Helper to generate the filtered list of types, based on the supported types of the schema */
	UE_API void FilterUnsupportedTypes(TArray<FPinTypeTreeItem>& UnfilteredList);

	/** Helper to generate the filtered list of types, based on the search string matching */
	UE_API bool GetChildrenMatchingSearch(const FText& SearchText, const TArray<FPinTypeTreeItem>& UnfilteredList, TArray<FPinTypeTreeItem>& OutFilteredList, FTopLevenshteinResult<FPinTypeTreeItem>& OutTopLevenshteinResult);

	/** Callback to get the tooltip text for the pin type combo box */
	UE_API FText GetToolTipForComboBoxType() const;

	/** Callback to get the tooltip text for the secondary pin type combo box */
	UE_API FText GetToolTipForComboBoxSecondaryType() const;

	/** Callback to get the tooltip for the array button widget */
	UE_API FText GetToolTipForArrayWidget() const;

	/** Callback to get the tooltip for the container type dropdown widget */
	UE_API FText GetToolTipForContainerWidget() const;

	/** Callback to get the display text for the total pin type item count */
	UE_API FText GetPinTypeItemCountText() const;
	 ...
	 */
	UE_API TSharedRef<SWidget> CreateObjectReferenceWidget(FPinTypeTreeItem InItem, FEdGraphPinType& InPinType, const FSlateBrush* InIconBrush, FText InSimpleTooltip) const;

	/** Gets the allowable object types for an tree item, used for building the sub-menu */
	UE_API TSharedRef< SWidget > GetAllowedObjectTypes(FPinTypeTreeItem InItem, bool bForSecondaryType);
	...
	UE_API void GenerateAllowedObjectTypesList(TArray<FObjectReferenceListItem>& OutList, FPinTypeTreeItem InItem, bool bForSecondaryType) const;
	 ...
	 */
	UE_API void OnSelectPinType(FPinTypeTreeItem InItem, FName InPinCategory, bool bForSecondaryType);

	/** Called whenever the custom filter options are changed. */
	UE_API void OnCustomFilterChanged();
};

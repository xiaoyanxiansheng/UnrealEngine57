...
public:
	...
	UE_API ~SPersonaToolbox();

	UE_API void Construct(const FArguments& InArgs, const TSharedRef<FPersonaAssetEditorToolkit>& InOwningEditor);

	// Called to attach a toolkit to this toolbox widget (e.g. when the mode changes).
	UE_API void AttachToolkit(const TSharedRef<IToolkit>& InToolkit);

	// Called to detach a toolkit from this toolbox widget (e.g. when the mode changes).
	UE_API void DetachToolkit(const TSharedRef<IToolkit>& InToolkit);

	// Sets the owning tab so that we can update the tab icon based on what the toolkit provides.
	UE_API void SetOwningTab(const TSharedRef<SDockTab>& InOwningTab);

private:	
	UE_API void UpdateInlineContent(const TSharedPtr<IToolkit>& Toolkit, TSharedPtr<SWidget> InlineContent);

	UE_API void UpdatePalette(const TWeakPtr<FModeToolkit>& InWeakToolkit) const;

	// Whether to show the inline content or not.
	UE_API EVisibility GetInlineContentHolderVisibility() const;

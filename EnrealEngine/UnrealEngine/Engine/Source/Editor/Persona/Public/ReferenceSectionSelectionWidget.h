...
private: 
	...
	UE_API TSharedRef<SWidget> CreateSectionListWidgetMenu();
	UE_API TSharedRef<ITableRow> MakeIntegerDisplayWidget(const TSharedPtr<int32> SectionIndex, const TSharedRef<STableViewBase>& OwnerTable) const;

	TSharedPtr<SListView<TSharedPtr<int32>>> SectionListView;
	// Called when the user selects a bone name
	UE_API void OnSelectionChanged(TSharedPtr<int32> NewSectionIndex, ESelectInfo::Type SelectInfo);

	UE_API const TArray<TSharedPtr<int32>>* GetSections() const;

	UE_API FText GetCurrentSectionIndex() const;

...
private:
	...
	UE_API void Initialize(TSharedRef<SAnimAssetFindReplace> InWidget);
...
protected:
	...
	UE_API void RequestRefreshUI();

	/** Requests cached data and search results */
	UE_API void RequestRefreshCachedData();

	/** Requests search results to be refreshed */
	UE_API void RequestRefreshSearchResults();
...
public:
	...
	UE_API void SetFindString(const FString& InString);
	...
	UE_API void SetReplaceString(const FString& InString);
	...
	UE_API void SetSkeletonFilter(const FAssetData& InSkeletonFilter);
	...
	UE_API void SetFindWholeWord(bool bInFindWholeWord);
	...
	UE_API void SetSearchCase(ESearchCase::Type InSearchCase);
...
protected:
	...
	UE_API virtual void ExtendToolbar(FToolMenuSection& InSection) override;
	UE_API virtual TSharedRef<SWidget> MakeFindReplaceWidget() override;
	UE_API virtual void FocusInitialWidget() override;
	UE_API virtual bool CanCurrentlyReplace() const override;
	UE_API virtual bool CanCurrentlyRemove() const override;
	UE_API virtual void RefreshCachedData() override;

protected:
	/** Check if the supplied string matches the find parameters */
	UE_API bool NameMatches(FStringView InNameStringView) const;

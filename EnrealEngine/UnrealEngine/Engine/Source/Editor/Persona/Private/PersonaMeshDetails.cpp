...
static const TCHAR* GSuffixDefaultMaterial = TEXT(" (Default)");

static bool GEnableSkeletalMeshMaterialSlotOverlayMaterialUI = false;
static FAutoConsoleVariableRef CVarEnableSkeletalMeshMaterialSlotOverlayMaterialUI(
...
namespace PersonaMeshDetailsConstants
{
	...
	constexpr static int32 LodSliderExtension = 5;
}
...
{
	static bool bAutoMeshReductionAvailable = FModuleManager::Get().LoadModuleChecked<IMeshReductionManagerModule>("MeshReductionInterface").GetSkeletalMeshReductionInterface() != nullptr;
...
private:
	...
	FText GetReimportButtonToolTipText() const
	{
		...
		const FSkeletalMeshLODInfo& LODInfo = *(SkelMesh->GetLODInfo(LODIndex));
		const FString Filename = FPaths::GetCleanFilename(LODInfo.SourceImportFilename);
...
void FSkeletalMeshReductionSettingsLayout::GenerateChildContent(IDetailChildrenBuilder& ChildrenBuilder)
{
	...
	{
		EnumReductionMethod = FindObject<UEnum>(nullptr, TEXT("/Script/Engine.SkeletalMeshOptimizationType"), EFindObjectFlags::ExactClass);
	}
	if (EnumImportance == nullptr)
	{
		EnumImportance = FindObject<UEnum>(nullptr, TEXT("/Script/Engine.SkeletalMeshOptimizationImportance"), EFindObjectFlags::ExactClass);
	}
	if (EnumTerminationCriterion == nullptr)
	{
		EnumTerminationCriterion = FindObject<UEnum>(nullptr, TEXT("/Script/Engine.SkeletalMeshTerminationCriterion"), EFindObjectFlags::ExactClass);
	}
...
void FPersonaMeshDetails::OnAttributePreChangePreventPostEditChange(const int32 LODIndex, const FName LODInfoPropertyName)
{
	...
	ChangeScopes.Push(MakeUnique<FScopedSkeletalMeshPostEditChange>(Mesh, bCallPostEditChange, bReregisterComponents));
}

void FPersonaMeshDetails::OnAttributeChangedPreventPostEditChange(const int32 LODIndex, const FName LODInfoPropertyName, const bool bForceComponentRefresh)
{
	USkeletalMesh* Mesh = GetPersonaToolkit()->GetMesh();
...
void FPersonaMeshDetails::PreventAttributePostEditChange(TSharedPtr<IPropertyHandle> AttributeHandle, const int32 LODIndex, const FName PropertyName, const bool bForceComponentRefresh)
{
...
void FPersonaMeshDetails::CustomizeLODInfoSetingsDetails(IDetailLayoutBuilder& DetailLayout, ULODInfoUILayout* LODInfoUILayout, TSharedRef<IPropertyHandle> LODInfoProperty, IDetailCategoryBuilder& LODCategory)
{
	...
	constexpr bool bForceComponentRefreshTrue = true;
	constexpr bool bForceComponentRefreshFalse = false;
	...
	IDetailPropertyRow& ScreenSizeRow = LODInfoGroup.AddPropertyRow(ScreenSizeHandle->AsShared()).EditCondition(EnabledAttrib, nullptr);
...
void FPersonaMeshDetails::AddLODLevelCategories(IDetailLayoutBuilder& DetailLayout)
{
	...
	if (SkelMesh)
	{
		...
		for (int32 LODIndex = 0; LODIndex < SkelMeshLODCount; ++LODIndex)
		{
			...
			TSharedRef<IPropertyHandle> LODInfoProperty = DetailLayout.GetProperty(FName("SourceModels"), USkeletalMesh::StaticClass());
...
void FPersonaMeshDetails::CustomizeSkinWeightProfiles(IDetailLayoutBuilder& DetailLayout)
{
	...
	DetailLayout.EditCategory("SkinWeights", LOCTEXT("SkinWeightsCategory", "Skin Weights"));
...
void FPersonaMeshDetails::CustomizeLODSettingsCategories(IDetailLayoutBuilder& DetailLayout)
{
	...
	MinLODRow.EditCondition(IsPerPlatformMinLodEnabled, nullptr);
	...
	LODSettingsCategory.AddCustomRow(LOCTEXT("QualityLevelMinLOD", "Quality Level Min LOD"))
		...
		.EditCondition(IsQualityLevelLodEnabled, nullptr)
		.NameContent()
		[
			SNew(SHorizontalBox)
...
FReply FPersonaMeshDetails::ApplyLODChanges(int32 LODIndex)
{
	...
	FScopedSuspendAlternateSkinWeightPreview ScopedSuspendAlternateSkinnWeightPreview(SkelMesh);
	{
		...
		(void)SkelMesh->MarkPackageDirty();
	}
...
FReply FPersonaMeshDetails::RegenerateLOD(int32 LODIndex)
{
	...
	if (SkelMesh->IsValidLODIndex(LODIndex))
	{
		const FSkeletalMeshLODInfo& CurrentLODInfo = *(SkelMesh->GetLODInfo(LODIndex));
		const bool bIsReductionActive = SkelMesh->IsReductionActive(LODIndex);
		if (CurrentLODInfo.bHasBeenSimplified == false && (LODIndex > 0 || bIsReductionActive))
		{
			...
			{
				//Ask user a special permission when the base LOD can be reduced 
...
void FPersonaMeshDetails::CustomizeDetails( IDetailLayoutBuilder& DetailLayout )
{
	...
	DetailLayout.EditCategory("Mesh");
			...
			if (SkelMeshMaterials.IsValidIndex(MaterialIndex))
			{
				...
				for (const FSkeletalMaterial &SkeletalMaterial : SkelMeshMaterials)
				{
					if (MaterialIndex != CurrentIterMaterialIndex)
					{
						...
						{
							FString BuildDefaultName = SkeletalMaterial.MaterialSlotName.ToString() + GSuffixDefaultMaterial;
...
void FPersonaMeshDetails::OnSectionChanged(int32 LODIndex, int32 SectionIndex, int32 NewMaterialSlotIndex, FName NewMaterialSlotName)
{
	...
	if(Mesh)
	{
		...
		NewMaterialSlotNameString.RemoveFromEnd(GSuffixDefaultMaterial, ESearchCase::CaseSensitive);
...
void FPersonaMeshDetails::OnGenerateElementForClothingAsset( TSharedRef<IPropertyHandle> StructProperty, int32 ElementIndex, IDetailChildrenBuilder& ChildrenBuilder, IDetailLayoutBuilder* DetailLayout )
{
	...
	// Properties are now inside the clothing data, so we just add a new inspector and handle everything through that
...
TSharedRef<SUniformGridPanel> FPersonaMeshDetails::MakeClothingDetailsWidget(int32 AssetIndex) const
{
	...
	if (UClothingAssetCommon* const ClothingAsset = Cast<UClothingAssetCommon>(SkelMesh->GetMeshClothingAssets()[AssetIndex]))
	{
		const FSlateFontInfo DetailFontInfo = IDetailLayoutBuilder::GetDetailFont();
		...
		for (int32 LODIndex = 0; LODIndex < NumLODs; ++LODIndex)
		{
			...
			const FClothLODDataCommon& LodData = ClothingAsset->LodData[LODIndex];
...
void FPersonaMeshDetails::UpdateClothingEntries()
{
	...
	{
		if (UClothingAssetBase* const ClothingAsset = Mesh->GetMeshClothingAssets()[Idx])
		{
			if (UClothingAssetCommon* const ClothingAssetCommon = Cast<UClothingAssetCommon>(ClothingAsset))
			{
				const int32 NumAssetLods = ClothingAssetCommon->GetNumLods();
			...
			else
			{
				...
				NewEntry->Asset = ClothingAsset;
				NewEntry->AssetIndex = Idx;
				NewEntry->AssetLodIndex = INDEX_NONE;
				NewClothingAssetEntries.Add(NewEntry);
			}
		}
	}
...
{
	const UClothingAssetBase* const Asset = InEntry->Asset.Get();

	FText EntryText;
	if(Asset)
	{
		EntryText = (InEntry->AssetLodIndex != INDEX_NONE) ?
			FText::Format(LOCTEXT("ClothingAssetEntry_Name", "{0} - LOD{1}"), FText::FromString(Asset->GetName()), FText::AsNumber(InEntry->AssetLodIndex)) : 
...
{
	const USkeletalMesh* const Mesh = GetPersonaToolkit()->GetMesh();

	if (Mesh && !Mesh->IsCompiling())
	{
		if (const UClothingAssetBase* const ClothingAsset = Cast<const UClothingAssetBase>(Mesh->GetSectionClothingAsset(InLodIdx, InSectionIdx)))
		{
			if (const UClothingAssetCommon* const ClothingAssetCommon = Cast<const UClothingAssetCommon>(Mesh->GetSectionClothingAsset(InLodIdx, InSectionIdx)))
			{
				if (ClothingAssetCommon->LodMap.IsValidIndex(InLodIdx))
				{
					const int32 ClothingLOD = ClothingAssetCommon->LodMap[InLodIdx];
			...
			{
				// Non ClothingAssetCommon clothing data cannot have a different LOD index to the section's LOD index, so just show the name
				return FText::FromString(ClothingAsset->GetName());
			}
		}
	}
...
void FPersonaMeshDetails::OnClothingSelectionChanged(TSharedPtr<FClothingEntry> InNewEntry, ESelectInfo::Type InSelectType, int32 BoxIndex, int32 InLodIdx, int32 InSectionIdx)
{
	...
	FScopedSuspendAlternateSkinWeightPreview ScopedSuspendAlternateSkinnWeightPreview(Mesh);
	{
		...
		if (UClothingAssetBase* const ClothingAsset = InNewEntry->Asset.Get())
		{
			...
			const int32 AssetLodIndex = InNewEntry->AssetLodIndex != INDEX_NONE ? InNewEntry->AssetLodIndex : InLodIdx;  //  Use the mesh LOD index if the asset type doesn't allow independent LOD setting
			if (!ClothingAsset->BindToSkeletalMesh(Mesh, InLodIdx, InSectionIdx, AssetLodIndex))
			{
			...
			else
			{
				...
				verify(Mesh->GetMeshClothingAssets().Find(ClothingAsset, AssetIndex));
				...
				OriginalSectionData.ClothingData.AssetLodIndex = AssetLodIndex;
			}
		}
		else if (Mesh)
		{

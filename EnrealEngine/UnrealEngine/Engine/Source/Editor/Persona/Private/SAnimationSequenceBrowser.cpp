...
/** A filter that displays animations that use an attribute */
class FFrontendFilter_Attributes : public FFrontendFilter
{
public:
	FFrontendFilter_Attributes(TSharedPtr<FFrontendFilterCategory> InCategory, TSharedPtr<IEditableSkeleton> InEditableSkeleton)
		: FFrontendFilter(InCategory)
		, EditableSkeleton(InEditableSkeleton)
	{}
	...
	virtual FString GetName() const override
	{
		return TEXT("AttributeAssets");
	}

	virtual FText GetDisplayName() const override
	{
		return AttributeName.IsEmpty()
			? LOCTEXT("FFrontendFilter_AttributeAssetsEmpty", "Uses Attributes")
			: FText::Format(LOCTEXT("FFrontendFilter_AttributeAssets", "Uses Attribute: {0}"), FText::FromString(AttributeName));
	}

	virtual FText GetToolTipText() const override
	{
		return AttributeName.IsEmpty()
			? LOCTEXT("FFrontendFilter_AttributeAssetsEmptyTooltip", "Show assets that contains a (specified) attribute")
			: FText::Format(LOCTEXT("FFrontendFilter_AttributeAssetsTooltip", "Show assets that use attribute '{0}'"), FText::FromString(AttributeName));
	}

	virtual void ModifyContextMenu(FMenuBuilder& MenuBuilder) override
	{
		MenuBuilder.BeginSection(TEXT("AttributeSection"), LOCTEXT("AttributeSectionHeading", "Choose Attribute"));
		
		MenuBuilder.AddMenuEntry(LOCTEXT("AllAttributeFilterName", "Any attribute"),
			LOCTEXT("AllAttributeFilterName_ToolTip", "Consider all animation attributes for filtering."),
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateLambda([this]()
				{
				...
				FGetActionCheckState::CreateLambda([this]()
				{
					return AttributeName.IsEmpty() ? ECheckBoxState::Checked : ECheckBoxState::Unchecked;
				})
			), 
			NAME_None,
			EUserInterfaceActionType::ToggleButton
		...
		TSharedRef<SWidget> Widget = 
			...
			.WidthOverride(200.0f)
			[
				SNew(SAnimAttributePicker, &EditableSkeleton->GetSkeleton())
				.bEnableMultiSelect(false)
				.OnAttributesPicked_Lambda([this](const TConstArrayView<FAnimationAttributeIdentifier> InAttributes)
				{
					check(InAttributes.Num() == 1);
					FSlateApplication::Get().DismissAllMenus();
					AttributeName = InAttributes[0].GetName().ToString();
					OnChanged().Broadcast();
				})
			];
	...
	virtual FLinearColor GetColor() const override
	{
		return FLinearColor(0.6f, 0.0f, 0.2f, 1.0f);
	}
	...
	virtual bool PassesFilter(FAssetFilterType InItem) const override
	{
		...
		if (InItem.Legacy_TryGetAssetData(ItemAssetData))
		{
			const FString TagValue = ItemAssetData.GetTagValueRef<FString>(USkeleton::AttributeTag);
			if (!TagValue.IsEmpty())
			{
				...
				if (AttributeName.IsEmpty() && !AttributesIdentifierArray.AttributeIdentifiers.IsEmpty())
				{
					return true;
				}
				...
				{
					if (AttrId.GetName().ToString() == AttributeName)
					{
						return true;
					}
				}
...
public:
	/** Attribute name string to use when filtering */
	FString AttributeName;

	/** Editable skeleton used to access available attributes */
...
void SAnimationSequenceBrowser::Construct(const FArguments& InArgs, const TSharedRef<IPersonaToolkit>& InPersonaToolkit)
{
	...
	Config.ExtraFrontendFilters.Add(MakeShareable(new FFrontendFilter_Attributes(AnimCategory, InPersonaToolkit->GetEditableSkeleton())));

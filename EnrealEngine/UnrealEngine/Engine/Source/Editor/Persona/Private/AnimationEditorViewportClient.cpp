	...
	, bResumeAfterTracking(false)
{
	...
	CameraSpeedSettings = ConfigOption->GetAssetEditorOptions(InAssetEditorToolkit->GetEditorName()).ViewportConfigs[ViewportIndex].CameraSpeedSettings;
...
FText FAnimationViewportClient::GetDisplayInfo(bool bDisplayAllInfo) const
{
	...
	const TArray<FClothingSimulationInstance>& ClothingSimulationInstances = PreviewMeshComponent->GetClothingSimulationInstances();
	for (int32 Index = 0; Index < ClothingSimulationInstances.Num(); ++Index)
	{
		const FClothingSimulationInstance& ClothingSimulationInstance = ClothingSimulationInstances[Index];

		if (const IClothingSimulationInterface* const ClothingSimulation = ClothingSimulationInstance.GetClothingSimulation())
		{
			// Cloth stats
			if (const int32 NumActiveCloths = ClothingSimulation->GetNumCloths())
			{
				TextValue = ConcatenateLine(TextValue, FText::Format(LOCTEXT("NumActiveCloths", "{0}>Active Cloths: {1}"), Index, NumActiveCloths));
			}
			if (const int32 NumKinematicParticles = ClothingSimulation->GetNumKinematicParticles())
			{
				TextValue = ConcatenateLine(TextValue, FText::Format(LOCTEXT("NumKinematicParticles", "{0}>Kinematic Particles: {1}"), Index, NumKinematicParticles));
			}
			if (const int32 NumDynamicParticles = ClothingSimulation->GetNumDynamicParticles())
			{
				TextValue = ConcatenateLine(TextValue, FText::Format(LOCTEXT("NumDynamicParticles", "#{0}>Dynamic Particles: {1}"), Index, NumDynamicParticles));
			}
			if (const int32 NumIterations = ClothingSimulation->GetNumIterations())
			{
				TextValue = ConcatenateLine(TextValue, FText::Format(LOCTEXT("NumIterations", "{0}:>Iterations: {1}"), Index, NumIterations));
			}
			if (const float SimulationTime = ClothingSimulation->GetSimulationTime())
			{
				...
				TextValue = ConcatenateLine(TextValue, FText::Format(LOCTEXT("SimulationTime", "{0}>Simulation Time: {1}ms"), Index, FText::AsNumber(SimulationTime, &NumberFormatOptions)));
			}
			if (ClothingSimulation->IsTeleported())
			{
				TextValue = ConcatenateLine(TextValue, FText::Format(LOCTEXT("IsTeleported", "{0}>Simulation Teleport Activated"), Index));
			}
		}
	}
...
void FAnimationViewportClient::SetCameraSpeedSettings(const FEditorViewportCameraSpeedSettings& InCameraSpeedSettings)
{
	FEditorViewportClient::SetCameraSpeedSettings(InCameraSpeedSettings);
	ConfigOption->SetCameraSpeedSettings(GetAssetEditorToolkit()->GetEditorName(), GetCameraSpeedSettings(), ViewportIndex);
}

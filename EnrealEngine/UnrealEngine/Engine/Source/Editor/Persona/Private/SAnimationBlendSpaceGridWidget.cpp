...
void SBlendSpaceGridWidget::PaintAnimationNames(const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32& DrawLayerId) const
{
	if(const UBlendSpace* BlendSpace = BlendSpaceBase.Get())
	{
		...
		for (int32 SampleIndex = 0; SampleIndex < Samples.Num(); ++SampleIndex)
		{
			...
			const FText Name = !Sample.bUseSingleFrameForBlending ? 
				FText::Format(LOCTEXT("SampleNameFormat", "{0} ({1})"), GetSampleName(Sample, SampleIndex), FText::AsNumber(SampleIndex)) : 
				FText::Format(LOCTEXT("FrameIndexSampleNameFormat", "{0} ({1}) (Frame {2})"), GetSampleName(Sample, SampleIndex), FText::AsNumber(SampleIndex), FText::AsNumber(Sample.FrameIndexToSample));
			...
			switch (DragState)
			{
				...
				{		
					...
					else if (HighlightedSampleIndex != INDEX_NONE && BlendSpace->IsValidBlendSampleIndex(HighlightedSampleIndex))
					{
						...
						ToolTipText = !BlendSample.bUseSingleFrameForBlending ? 
							FText::Format(LOCTEXT("SampleNameFormat", "{0} ({1})"), GetSampleName(BlendSample, HighlightedSampleIndex), FText::AsNumber(HighlightedSampleIndex)) : 
							FText::Format(LOCTEXT("FrameIndexSampleNameFormat", "{0} ({1}) (Frame {2})"), GetSampleName(BlendSample, HighlightedSampleIndex), FText::AsNumber(HighlightedSampleIndex), FText::AsNumber(BlendSample.FrameIndexToSample));
					}
					else if(TargetPosition.IsSet())
					{
						ToolTipText = PreviewValue;
				...
				case EDragState::DragSample:
				{
					if (BlendSpace->IsValidBlendSampleIndex(DraggedSampleIndex))
					{
						...
						ToolTipText = !BlendSample.bUseSingleFrameForBlending ? 
							FText::Format(LOCTEXT("SampleNameFormat", "{0} ({1})"), GetSampleName(BlendSample, DraggedSampleIndex), FText::AsNumber(DraggedSampleIndex)) : 
							FText::Format(LOCTEXT("FrameIndexSampleNameFormat", "{0} ({1}) (Frame {2})"), GetSampleName(BlendSample, DraggedSampleIndex), FText::AsNumber(DraggedSampleIndex), FText::AsNumber(BlendSample.FrameIndexToSample));
					}
			...
			{				
				for (const FBlendSampleData& SampleData : PreviewedSamples)
				{
					if(BlendSpace->IsValidBlendSampleIndex(SampleData.SampleDataIndex))
					{
						...
						const FText SampleNameWithWeight = !BlendSample.bUseSingleFrameForBlending ? 
							FText::Format(LOCTEXT("SampleFormat", "{0} ({1}): {2}"), GetSampleName(BlendSample, SampleData.SampleDataIndex), FText::AsNumber(SampleData.SampleDataIndex),FText::AsNumber(SampleData.TotalWeight)) : 
							FText::Format(LOCTEXT("FrameIndexSampleFormat", "{0} ({1}) (Frame {2}): {3}"), GetSampleName(BlendSample, SampleData.SampleDataIndex), FText::AsNumber(SampleData.SampleDataIndex), FText::AsNumber(BlendSample.FrameIndexToSample), FText::AsNumber(SampleData.TotalWeight));
						TextBuilder.AppendLine(SampleNameWithWeight);
					}
				}
			}
...
FText SBlendSpaceGridWidget::GetSampleErrorMessage(const FBlendSample &BlendSample) const
{
	...
	else if (const UBlendSpace* BlendSpaceBaseObject = BlendSpaceBase.Get())
	{
		if (!BlendSpaceBaseObject->IsSingleFrameBlendingIndexInBounds(BlendSample))
		{
			static const FText SamplingIndexOutOfBounds = LOCTEXT("SingleFrameBlendingIndexOutOfBounds", "Single frame blending index is out of bounds.");
			return SamplingIndexOutOfBounds;
		}
	}

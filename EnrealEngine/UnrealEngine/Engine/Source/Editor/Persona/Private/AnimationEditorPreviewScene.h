...
public:
	...
	virtual void UnregisterOnAnimChanged(FDelegateUserObject Thing) override
	{
	...
	virtual void UnregisterOnPreviewMeshChanged(FDelegateUserObject Thing) override
	{
	...
	virtual void UnregisterOnLODChanged(FDelegateUserObject Thing) override
	{
	...
	virtual void UnregisterOnMorphTargetsChanged(FDelegateUserObject Thing) override
	{
	...
	virtual void UnregisterOnInvalidateViews(FDelegateUserObject Thing) override
	{
	...
	virtual void UnregisterOnFocusViews(FDelegateUserObject Thing) override
	{
	...
	virtual void UnregisterOnMeshClick(FDelegateUserObject Thing) override
	{
		OnMeshClick.RemoveAll(Thing);
	}

	UE_DEPRECATED(5.6, "Use RegisterOnSelectedBonesChanged delegate instead.")	
	virtual FDelegateHandle RegisterOnSelectedBoneChanged(const FOnSelectedBoneChanged& Delegate) override
	{
		return OnSelectedBoneChanged_DEPRECATED.Add(Delegate);
	}
	
	UE_DEPRECATED(5.6, "Use UnregisterOnSelectedBonesChanged delegate instead.")
	virtual void UnregisterOnSelectedBoneChanged(FDelegateHandle InHandle) override
	{
		OnSelectedBoneChanged_DEPRECATED.Remove(InHandle);
	}
	
	virtual FDelegateHandle RegisterOnSelectedBonesChanged(const FOnSelectedBonesChanged& InDelegate) override
	{
		return OnSelectedBonesChanged.Add(InDelegate);
	}

	virtual void UnregisterOnSelectedBonesChanged(FDelegateHandle InHandle) override
	{
		OnSelectedBonesChanged.Remove(InHandle);
	}

...
void FBlendSampleDetails::CustomizeDetails(class IDetailLayoutBuilder& DetailBuilder)
{
	...
	if(BlendspaceToUse->IsAsset())
	{
		...
		TSharedPtr<IPropertyHandle> UseSingleFrameBlendingProperty = DetailBuilder.GetProperty(
			GET_MEMBER_NAME_CHECKED(FBlendSample, FrameIndexToSample), FBlendSample::StaticStruct());
			 
		IDetailPropertyRow& UseSingleFrameBlendingPropertyRow = CategoryBuilder.AddProperty(UseSingleFrameBlendingProperty);
		FBlendSampleDetails::GenerateSingleFrameBlendingWidget(UseSingleFrameBlendingPropertyRow, UseSingleFrameBlendingProperty, AnimationProperty);
...
void FBlendSampleDetails::GenerateSingleFrameBlendingWidget(IDetailPropertyRow& PropertyRow, TSharedPtr<IPropertyHandle> SingleFrameIndexProperty, TSharedPtr<IPropertyHandle> AnimationProperty)
{
	auto ValueChangedLambda = [SingleFrameIndexProperty](uint32 NewValue, bool bIsInteractive)
	{
		if (SingleFrameIndexProperty.IsValid())
		{			
			SingleFrameIndexProperty->SetValue(NewValue, bIsInteractive ? EPropertyValueSetFlags::InteractiveChange : EPropertyValueSetFlags::DefaultFlags);
		}
	};
	
	auto MaxValueLambda = [AnimationProperty]() -> TOptional<uint32>
	{
		if (AnimationProperty.IsValid())
		{
			FAssetData AssetData;
			AnimationProperty->GetValue(AssetData);

			if (UAnimSequence* BlendSampleAnimation = Cast<UAnimSequence>(AssetData.GetAsset()))
			{
				return BlendSampleAnimation->GetNumberOfSampledKeys() - 1;
			}
		}
	...
	[
		...
		.Font(FAppStyle::GetFontStyle("CurveEd.InfoFont"))
		.Value_Lambda([SingleFrameIndexProperty]() -> TOptional<uint32>
		{
			if (SingleFrameIndexProperty.IsValid())
			{
				uint32 Value = 0;
				if (SingleFrameIndexProperty->GetValue(Value) == FPropertyAccess::Success)
				{
					return Value;
				}
			}
		...
		.OnValueCommitted_Lambda([ValueChangedLambda](uint32 NewValue, ETextCommit::Type CommitType)
		{
			ValueChangedLambda(NewValue, false);
		})
		.OnValueChanged_Lambda([ValueChangedLambda](uint32 NewValue)
		{
			ValueChangedLambda(NewValue, true);
		})
		.LabelVAlign(VAlign_Center)
		.AllowSpin(true)
		.MinValue(0)
		.MaxValue_Lambda(MaxValueLambda)
		.MinSliderValue(0)
		.MaxSliderValue_Lambda(MaxValueLambda)
	];
}

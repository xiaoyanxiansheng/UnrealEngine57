...
void FBlendSpaceDetails::CustomizeDetails(class IDetailLayoutBuilder& DetailBuilder)
{
	...
	if (BlendSpacePtr)
	{
		...
		if (BlendSpace->IsAsset())
		{
			...
			TWeakObjectPtr<const UBlendSpace> WeakBlendspace = BlendSpace;
			auto ContainsSingleFrameBlendSampleVisibility = [WeakBlendspace](bool bExpectedValueForVisibility) -> EVisibility
			{
				if (const UBlendSpace* PinnedBlendspace = WeakBlendspace.Get())
				{
					const bool bValue = PinnedBlendspace->GetBlendSamples().ContainsByPredicate([](const FBlendSample& BlendSample) { return BlendSample.bUseSingleFrameForBlending; } );
					return bExpectedValueForVisibility == bValue ? EVisibility::Visible : EVisibility::Collapsed;
				}
			...
			for (int32 AxisIndex = 0; AxisIndex < 3; ++AxisIndex)
			{
				...
				if (AxisIndex < HideIndex)
				{
					...
					.Visibility(TAttribute<EVisibility>::CreateLambda(ContainsSingleFrameBlendSampleVisibility, false));
				}
				else
				{
			...
			const FText WarningText = LOCTEXT("AnalysisAndSingleFrameIncompatibilityWarning", "Analysis feature is not compatible with single frame blending. "
																					  "\n\nPlease disable single frame blending in each blend sample to use analysis.");
			FDetailWidgetRow& IncompatibilityWarningRow = DetailCategoryBuilder.AddCustomRow(WarningText)
				.WholeRowContent()
				[
				...
				.Visibility(TAttribute<EVisibility>::CreateLambda(ContainsSingleFrameBlendSampleVisibility, true));
		}
		...
		//==============================================================================================================
		{
			...
			for (uint32 SampleIndex = 0; SampleIndex < NumBlendSampleEntries; ++SampleIndex)
			{
				...
				TSharedPtr<IPropertyHandle> UseSingleFrameBlendingProperty = BlendSampleProperty->GetChildHandle(GET_MEMBER_NAME_CHECKED(FBlendSample, FrameIndexToSample));
				...
				if(BlendSpace->IsAsset())
				{
					...
					IDetailPropertyRow& UseSingleFrameBlendingPropertyRow = Group.AddPropertyRow(UseSingleFrameBlendingProperty.ToSharedRef());
					FBlendSampleDetails::GenerateSingleFrameBlendingWidget(UseSingleFrameBlendingPropertyRow, UseSingleFrameBlendingProperty.ToSharedRef(), AnimationProperty.ToSharedRef());

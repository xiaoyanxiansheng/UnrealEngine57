...
public:
	...
	void OnSetSkinWeightProfile(FName InProfileName, ESkinWeightProfileLayer InLayer);
	bool IsSkinWeightProfileSelected(FName InProfileName, ESkinWeightProfileLayer InLayer) const;
	TArray<FName> GetSelectedProfileNames() const;
...
private:
	...
	/** Called to toggle showing the Nanite fallback of the current preview mesh */
	void ToggleShowNaniteFallback();
	bool CanShowNaniteFallback() const;
	bool IsShowNaniteFallbackEnabled() const;

...
bool FDisplayedAnimCurveInfo::GetActiveFlag(const TSharedPtr<SAnimCurveViewer>& InAnimCurveViewer, bool bInMorphTarget) const
{
	...
	if (AnimInstance)
	{
		auto FindMorphOrMatCurveOnAnimInstance = [AnimInstance](const FName& CurveName, bool bIsMorphTarget) {
			const TMap<FName, float>& CurveList = AnimInstance->GetAnimationCurveList(bIsMorphTarget ? EAnimCurveType::MorphTargetCurve : EAnimCurveType::MaterialCurve);
			return CurveList.Find(CurveName) != nullptr;
		};

		// Find if we want to use a pose watch
		if (InAnimCurveViewer->PoseWatchPicker.IsValid())
		{
			...
			{
				// We are just displaying the output curves, so find if the curve exists on the anim instance
				return FindMorphOrMatCurveOnAnimInstance(CurveName, bInMorphTarget);
			}
		}
		else
		{
			// We aren't using pose watch so check on the anim instance and the skeleton
			if (FindMorphOrMatCurveOnAnimInstance(CurveName, bInMorphTarget))
			{
				return true;
			}

			if (TSharedPtr<IEditableSkeleton> EditableSkeleton = InAnimCurveViewer->EditableSkeletonPtr.Pin())
			{
				if (const FCurveMetaData* CurveMetaData = EditableSkeleton->GetSkeleton().GetCurveMetaData(CurveName))
				{
					return bInMorphTarget ? CurveMetaData->Type.bMorphtarget : CurveMetaData->Type.bMaterial;
				}
			}
		}
	}
...
bool SAnimCurveListRow::GetActiveWeight(float& OutWeight) const
{
	...
	if (AnimCurveViewer.IsValid())
	{
		...
		if(AnimInstance)
		{
			// If we're dealing with a pose watch, get the data from that
			if (AnimCurveViewer->PoseWatchPicker.IsValid())
			{
				// Find if we want to use a pose watch
				if (UPoseWatchPoseElement* PoseWatchPoseElement = AnimCurveViewer->PoseWatchPicker->GetCurrentPoseWatch())
				{
					...
					// didn't find the curve on the pose watch element
					return false;
				}
			}

			// We aren't dealing with a pose watch so go directly to the anim instance curves
...
void SAnimCurveViewer::CreateAnimCurveList( const FString& SearchText, bool bInFullRefresh )
{
	...
	if(AnimInstance)
	{
		...
		// If there isn't an active pose watch, grab the curve list directly from the anim instance
		if (!bIsPoseWatchActive)
		{
			if (EnumHasAnyFlags(CurrentCurveFlag, EAnimCurveViewerFilterFlags::MorphTarget))
			{
	...
	for (const TPair<FName, TSharedPtr<FDisplayedAnimCurveInfo>>& CurveNameValuePair : AllSeenAnimCurvesMap)
	{
		...
		// Look at whether we need to filter out any curves that aren't morphs or materials
		if (bAddToList)
		{
			if ((EnumHasAnyFlags(CurrentCurveFlag, EAnimCurveViewerFilterFlags::MorphTarget) && !CurveNameValuePair.Value->bMorphTarget)
				|| (EnumHasAnyFlags(CurrentCurveFlag, EAnimCurveViewerFilterFlags::Material) && !CurveNameValuePair.Value->bMaterial))
			{
				bAddToList = false;
			}
		}

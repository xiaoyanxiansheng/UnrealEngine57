...
private:
	...
	void OnCropAnimSequence(bool bFromStart, FFrameNumber StartFrame);

	void OnAppendAnimSequence(bool bFromStart, int32 NumOfFrames);

	void OnInsertAnimSequence(bool bBefore, int32 CurrentFrame, int32 NumFrames);

	void OnReZeroAnimSequence(int32 FrameIndex);

	void OnPromptUserForNumberOfFrames(FWidgetPath WidgetPath, TFunction<void(const int32 NumFrames)> Callback);

...
void SAnimTimelineTransportControls::Construct(const FArguments& InArgs, const TSharedRef<IPersonaPreviewScene>& InPreviewScene, UAnimSequenceBase* InAnimSequenceBase)
{
	...
	// Add Playback Speed widget - we need to retrieve the viewport tab and the viewport it hosts
	if (TSharedPtr<FAnimationEditorPreviewScene> PreviewScene =
			StaticCastSharedPtr<FAnimationEditorPreviewScene>(WeakPreviewScene.Pin()))
	{
		const FName TimelinePlaybackMenu = "AnimationEditor.Timeline.PlaybackMenu";
		if (!UToolMenus::Get()->IsMenuRegistered(TimelinePlaybackMenu))
		{
			if (UToolMenu* Menu = UToolMenus::Get()->RegisterMenu(
					TimelinePlaybackMenu, NAME_None, EMultiBoxType::SlimHorizontalToolBar, false
				))
			{
		...
		{
			MenuContext.AppendCommandList(PreviewScene->GetCommandList());
			UAnimViewportContext* const ContextObject = NewObject<UAnimViewportContext>();
			ContextObject->PersonaPreviewScene = PreviewScene;
		...
		.FillWidth(0.4)
		[
			UToolMenus::Get()->GenerateWidget(TimelinePlaybackMenu, MenuContext)
		];
		// clang-format on
	}
}

...
/** Returns true if world reloads are allowed. */
bool AreWorldReloadsAllowed()
{
	if (IConsoleVariable* CVar = IConsoleManager::Get().FindConsoleVariable(TEXT("SourceControl.EnableWorldReload")); ensure(CVar))
	{
		return CVar->GetBool();
	}
	else
...
FSourceControlChangelistStatePtr SSourceControlChangelistsWidget::GetCurrentChangelistState() const
{
...
void SSourceControlChangelistsWidget::SetSelectedFiles(const TArray<FString>& Filenames)
{
	...
	auto FindChangelist = [this](const FString& Filename) -> TSharedPtr<IChangelistTreeItem>
	{
		for (const TSharedPtr<IChangelistTreeItem>& Item : ChangelistTreeNodes)
		{
			for (const TSharedPtr<IChangelistTreeItem>& ChildItem : Item->GetChildren())
			{
				...
				{
					const FString& ChildFilename = StaticCastSharedPtr<FFileTreeItem>(ChildItem)->GetFullPathname();
		...
		for (const TSharedPtr<IChangelistTreeItem>& Item : UncontrolledChangelistTreeNodes)
		{
			for (const TSharedPtr<IChangelistTreeItem>& ChildItem : Item->GetChildren())
			{
				if (ChildItem->GetTreeItemType() == IChangelistTreeItem::File || ChildItem->GetTreeItemType() == IChangelistTreeItem::OfflineFile)
				{
					const FString& ChildFilename = StaticCastSharedPtr<IFileViewTreeItem>(ChildItem)->GetFullPathname();
					if (ChildFilename.Compare(Filename, ESearchCase::IgnoreCase) == 0)
					{
						return Item;
					}
	...
	if (FoundChangelistTreeItem)
	{
		...
		for (const TSharedPtr<IChangelistTreeItem>& ChildItem : FoundChangelistTreeItem->GetChildren())
		{
			if (ChildItem->GetTreeItemType() == IChangelistTreeItem::File || ChildItem->GetTreeItemType() == IChangelistTreeItem::OfflineFile)
			{
				const FString ChildFilename = StaticCastSharedPtr<IFileViewTreeItem>(ChildItem)->GetFullPathname().ToLower();
	...
	for (const TSharedPtr<IChangelistTreeItem>& Item : GetActiveFileListView().GetSelectedItems())
	{
		...
		else if (Item->GetTreeItemType() == IChangelistTreeItem::OfflineFile)
		{
			if (TSharedPtr<IChangelistTreeItem> Parent = Item->GetParent())
			{
				...
				{
					const FString& Filename = StaticCastSharedPtr<FOfflineFileTreeItem>(Item)->GetFullPathname();
					OutUncontrolledFiles.Add(Filename);
				}
...
bool SSourceControlChangelistsWidget::CanDeleteChangelist() const
{
	return CanDeleteChangelist(/*OutFailureMessage*/nullptr);
}

bool SSourceControlChangelistsWidget::CanDeleteChangelist(FText* OutFailureMessage) const
{
	FSourceControlChangelistStatePtr ChangelistState = GetCurrentChangelistState();
...
void SSourceControlChangelistsWidget::OnRevert()
{
	...
	if (TSharedPtr<ISourceControlChangelist> SelectedChangelist = GetChangelistFromSelection())
	{
		...
		if (!SelectedControlledFiles.IsEmpty())
		{
			...
			{
				const bool bRevertAll = false;
				const bool bReloadWorld = AreWorldReloadsAllowed();
			
				if ((PackageFiles.Num() == 0 || SourceControlHelpers::RevertAndReloadPackages(PackageFiles, bRevertAll, bReloadWorld)) && (NonPackageFiles.Num() == 0 || SourceControlHelpers::RevertFiles(NonPackageFiles)))
				{
...
{
	FSourceControlChangelistPtr Changelist = GetChangelistFromSelection();
	TArray<FString> Files = GetSelectedShelvedFiles();

	const FText TextMessage = FText::Format(LOCTEXT("Unshelving_Files", "Unshelving {0}|plural(one=file,other=files)..."), Files.Num());
	const FText TextSucceeded = FText::Format(LOCTEXT("Unshelve_Files_Succeeded", "The selected {0}|plural(one=file,other=files) were unshelved."), Files.Num());
	const FText TextFailed = FText::Format(LOCTEXT("Unshelve_Files_Failed", "Failed to unshelved the selected {0}|plural(one=file,other=files)."), Files.Num());

	int32 NumPackageFiles = Algo::CountIf(Files, [](const FString& File) -> bool { return FPackageName::IsPackageFilename(File); });
	if (NumPackageFiles > 0) // Use ApplyOperationAndReloadPackages to deal with editor file locks.
	{
		SSourceControlCommon::ExecuteChangelistOperationWithSlowTaskWrapper(TextMessage,
			[TextSucceeded, TextFailed, Changelist, Files]()
			{
				auto UnshelveOperation =
					[Changelist, Files](const TArray<FString>& InFilenames) -> bool
					{
						return ISourceControlModule::Get().GetProvider().Execute(ISourceControlOperation::Create<FUnshelve>(), Changelist, Files) == ECommandResult::Succeeded;
					};

				const bool bReloadWorld = AreWorldReloadsAllowed();
				const bool bInteractive = false;

				if (SourceControlHelpers::ApplyOperationAndReloadPackages(Files, UnshelveOperation, bReloadWorld, bInteractive))
				{
					SSourceControlCommon::DisplaySourceControlOperationNotification(TextSucceeded, SNotificationItem::CS_Success);
				}
				else
				{
					SSourceControlCommon::DisplaySourceControlOperationNotification(TextFailed, SNotificationItem::CS_Fail);
				}
			}
		);
	}
	else
	{
		...
		Execute(TextMessage, UnshelveOperation, Changelist, Files, EConcurrency::Synchronous, FSourceControlOperationComplete::CreateLambda(
			[TextSucceeded, TextFailed](const TSharedRef<ISourceControlOperation>& Operation, ECommandResult::Type InResult)
			{
				if (InResult == ECommandResult::Succeeded)
				{
					SSourceControlCommon::DisplaySourceControlOperationNotification(TextSucceeded, SNotificationItem::CS_Success);
...
FText SSourceControlChangelistsWidget::GetCanSubmitChangelistToolTip() const
{
	FText Error;
	if (!CanSubmitChangelist(&Error))
	{
		return Error;
	}
	return LOCTEXT("SourceControl_SubmitChangeslit_Tooltip", "Submits a changelist");
}

bool SSourceControlChangelistsWidget::CanSubmitChangelist() const
{
	return CanSubmitChangelist(/*OutFailureMessage*/nullptr);
}

bool SSourceControlChangelistsWidget::CanSubmitChangelist(FText* OutFailureMessage) const
{
	...
	else if (ISourceControlWindowsModule::Get().CanSubmitOverrideDelegate.IsBound())
	{
		// save the changelist description from the widget to perforce
		const FString Identifier = Changelist->GetChangelist()->GetIdentifier();

		SSubmitOverrideParameters SubmitOverrideParameters;
		SubmitOverrideParameters.Description = Changelist->GetDescriptionText().ToString();
		SubmitOverrideParameters.ToSubmit.SetSubtype<FString>(Identifier);

		FSubmitOverrideReply SubmitOverrideReply = ISourceControlWindowsModule::Get().CanSubmitOverrideDelegate.Execute(SubmitOverrideParameters, OutFailureMessage);
	...
	if (Changelist->GetShelvedFilesStatesNum() > 0)
	{
		if (OutFailureMessage)
		{
...
void SSourceControlChangelistsWidget::OnValidateChangelist()
{
	...
	Info.Hyperlink = FSimpleDelegate::CreateLambda([]() { FMessageLog("AssetCheck").Open(EMessageSeverity::Info, true); });

	TSharedPtr<SNotificationItem> Notification = FSlateNotificationManager::Get().AddNotification(Info);
	Notification->SetCompletionState(bValidationResult ? SNotificationItem::CS_Success : SNotificationItem::CS_Fail);
}

bool SSourceControlChangelistsWidget::CanValidateChangelist() const
{
	FSourceControlChangelistStatePtr Changelist = GetCurrentChangelistState();
...
TSharedPtr<SWidget> SSourceControlChangelistsWidget::OnOpenContextMenu()
{
	...
	if (bHasSelectedUnsavedAsset)
	{
		...
		Section.AddMenuEntry(
			...
			FUIAction(FExecuteAction::CreateLambda([this]
			{
				...
				TArray<FString> PackageNamesToCheckout;
				for (const FString& Filename : SelectedFiles)
				{
					FString PackageName = UPackageTools::FilenameToPackageName(Filename);
					PackageNames.Add(PackageName);

					// If source control is disabled and the file does not exist then don't try to checkout.
					if (PackageHelper.UseSourceControl() || IPlatformFile::GetPlatformPhysical().FileExists(*Filename))
					{
						PackageNamesToCheckout.Add(PackageName);
					}
				}
				
				if (!PackageHelper.Checkout(PackageNamesToCheckout))
				{
					return;
				}
	...
	if (bHasSelectedChangelist)
	{
		Section.AddMenuEntry(
			...
			{ this, &SSourceControlChangelistsWidget::GetCanSubmitChangelistToolTip },
			FSlateIcon(),
			FUIAction(
	...
	if (InMouseEvent.IsMouseButtonDown(EKeys::LeftMouseButton) && !UnsavedAssetsFileListView->GetSelectedItems().IsEmpty())
	{
		...
		for (const FChangelistTreeItemPtr& InTreeItem : UnsavedAssetsFileListView->GetSelectedItems())
		{
			if (InTreeItem->GetTreeItemType() == IChangelistTreeItem::OfflineFile)
			{
				...
				Operation->OfflineFiles.Emplace(FileTreeItem->GetFullPathname());
			}
		}
...
{
	if (Item->GetTreeItemType() == IChangelistTreeItem::File || Item->GetTreeItemType() == IChangelistTreeItem::OfflineFile)
	{
		const FString& Filename = StaticCastSharedPtr<IFileViewTreeItem>(Item)->GetFullPathname();

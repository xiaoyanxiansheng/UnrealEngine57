...
void FAnimationBlueprintEditor::OnCreateGraphEditorCommands(TSharedPtr<FUICommandList> GraphEditorCommandsList)
{
	...
	GraphEditorCommandsList->MapAction( FAnimGraphCommands::Get().CreateSelfTransition,
		FExecuteAction::CreateSP( this, &FAnimationBlueprintEditor::OnCreateSelfTransition )
		);

	GraphEditorCommandsList->MapAction( FAnimGraphCommands::Get().SetActiveState,
		FUIAction(
			FExecuteAction::CreateSP( this, &FAnimationBlueprintEditor::OnSetActiveState ),
			FCanExecuteAction::CreateSP( this, &FAnimationBlueprintEditor::CanSetActiveState )
			)
		);

	GraphEditorCommandsList->MapAction( FAnimGraphCommands::Get().ReverseTransition,
		FExecuteAction::CreateSP( this, &FAnimationBlueprintEditor::OnReverseTransition )
		);
}
...
void FAnimationBlueprintEditor::OnCreateSelfTransition()
{
	FGraphPanelSelectionSet SelectedNodes = GetSelectedNodes();
	if (SelectedNodes.Num() == 0)
	{
	...
	const FScopedTransaction Transaction(LOCTEXT("CreateSelfTransition", "Create Self Transition"));

	for (UObject* Node : SelectedNodes)
	{
		if (UAnimStateNode* StateNode = Cast<UAnimStateNode>(Node))
		{
			UEdGraph* TargetGraph = StateNode->GetGraph();
			TargetGraph->Modify();

			FGraphNodeCreator<UAnimStateAliasNode> AliasNodeCreator(*TargetGraph);
			UAnimStateAliasNode* AliasNode = AliasNodeCreator.CreateNode();
			TSharedPtr<INameValidatorInterface> NameValidator = AliasNode->MakeNameValidator();
			check(NameValidator.IsValid());
			FString NewName(TEXT("Self"));
			NameValidator->FindValidString(NewName);
			AliasNode->StateAliasName = NewName;
			AliasNodeCreator.Finalize();

			AliasNode->SetNodePosX(StateNode->GetNodePosX() + 200);
			AliasNode->SetNodePosY(StateNode->GetNodePosY() - 100);
			AliasNode->GetAliasedStates().Add(StateNode);
			
			FGraphNodeCreator<UAnimStateTransitionNode> TransitionNodeCreator(*TargetGraph);
			UAnimStateTransitionNode* TransitionNode = TransitionNodeCreator.CreateNode();
			TransitionNodeCreator.Finalize();

			// Self-transitions should always fall back to inertialization
			TransitionNode->bAllowInertializationForSelfTransitions = true;
			// Self transitions should always have a delay so they dont re-enter each frame (footgun prevention)
			TransitionNode->MinTimeBeforeReentry = 0.0f;
			TransitionNode->CreateConnections(AliasNode, StateNode);
		}
	}
}

FAnimNode_StateMachine* FAnimationBlueprintEditor::GetSelectedStateInstance(int32& OutSelectedState) const
{
	FGraphPanelSelectionSet SelectedNodes = GetSelectedNodes();
	if (SelectedNodes.Num() != 1)
	{
	...
	UAnimStateNode* StateNode = Cast<UAnimStateNode>(*SelectedNodes.begin());
	if (StateNode == nullptr)
	{
		return nullptr;
	}

	UAnimBlueprint* AnimBlueprint = Cast<UAnimBlueprint>(FBlueprintEditorUtils::FindBlueprintForNode(StateNode));
	...
	if ((ActiveObject == nullptr) || (Class == nullptr) || Class->GetAnimNodeProperties().Num() == 0)
	{
		return nullptr;
	}

	FStateMachineDebugData* DebugInfo = Class->GetAnimBlueprintDebugData().StateMachineDebugData.Find(StateNode->GetGraph());
	if (DebugInfo == nullptr)
	{
		return nullptr;
	}

	int32* StateIndexPtr = DebugInfo->NodeToStateIndex.Find(StateNode);
	if(StateIndexPtr == nullptr)
	{
		return nullptr;
	}

	if (!Class->BakedStateMachines.IsValidIndex(DebugInfo->MachineIndex))
	{
		return nullptr;
	}

	OutSelectedState = *StateIndexPtr;
	return const_cast<FAnimNode_StateMachine*>(ActiveObject->GetStateMachineInstanceFromName(Class->BakedStateMachines[DebugInfo->MachineIndex].MachineName));
}

bool FAnimationBlueprintEditor::CanSetActiveState() const
{
	int32 StateIndex = 0;
	return GetSelectedStateInstance(StateIndex) != nullptr;
}

void FAnimationBlueprintEditor::OnSetActiveState()
{
	int32 StateIndex = 0;
	FAnimNode_StateMachine* StateMachineInstance = GetSelectedStateInstance(StateIndex);
	if (StateMachineInstance == nullptr)
	{
		return;
	}

	StateMachineInstance->DebugForceState(StateIndex);
}

void FAnimationBlueprintEditor::OnReverseTransition()
{
	FGraphPanelSelectionSet SelectedNodes = GetSelectedNodes();
	if (SelectedNodes.Num() == 0)
	{
	...
	const FScopedTransaction Transaction(LOCTEXT("ReverseTransition", "Reverse Transition"));

	for (UObject* Node : SelectedNodes)
	{
		if (UAnimStateTransitionNode* TransitionNode = Cast<UAnimStateTransitionNode>(Node))
		{
			TransitionNode->Reverse();
		}
	}
}

...
#include UE_INLINE_GENERATED_CPP_BY_NAME(EditorUtilityWidgetBlueprint)
...
TSharedRef<SDockTab> UEditorUtilityWidgetBlueprint::SpawnEditorUITab(const FSpawnTabArgs& SpawnTabArgs)
{
	if(bSpawnAsNomadTab)
	{
		...
		OnCompiled().Remove(OnBlueprintCompileHandle);
		OnBlueprintCompileHandle = OnCompiled().AddUObject(this, &UEditorUtilityWidgetBlueprint::RegenerateCreatedTab);

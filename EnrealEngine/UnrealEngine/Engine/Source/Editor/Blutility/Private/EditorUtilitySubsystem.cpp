...
#include UE_INLINE_GENERATED_CPP_BY_NAME(EditorUtilitySubsystem)
...
void UEditorUtilitySubsystem::HandleStartupAssets()
{
	...
	// Handle the current set of assets
	{
		...
		{
			const FString& AssetPath = FPackageName::LongPackageNameToFilename(StartupAsset.PackagePath.ToString());
			if (FPaths::IsUnderDirectory(AssetPath, FPaths::GameDevelopersDir()))
			{
				if (!FPaths::IsUnderDirectory(AssetPath, FPaths::GameUserDeveloperDir()))
				{
					continue;
				}
			}

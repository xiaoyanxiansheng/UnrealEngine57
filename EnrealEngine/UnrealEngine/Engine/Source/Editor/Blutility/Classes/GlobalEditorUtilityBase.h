...
class UDEPRECATED_GlobalEditorUtilityBase : public UObject
{
	...
	UE_API TArray<AActor*> GetSelectionSet();
	...
	UE_API void OnDefaultActionClicked();

	////////////////////////////

	// Calls OnEachSelectedActor for each selected actor
	UFUNCTION(BlueprintCallable, Category="Development|Editor")
	UE_API void ForEachSelectedActor();
	...
	UFUNCTION(BlueprintCallable, Category="Development|Editor")
	UE_API void ForEachSelectedAsset();
	...
	UFUNCTION(BlueprintCallable, Category="Development|Editor")
	UE_API TArray<UObject*> GetSelectedAssets();

	///////////////////////////

	UFUNCTION(BlueprintPure, Category="Development|Editor")
	UE_API UEditorPerProjectUserSettings* GetEditorUserSettings();

	// Remove all actors from the selection set
	UFUNCTION(BlueprintCallable, Category="Development|Editor")
	UE_API void ClearActorSelectionSet();

	// Selects nothing in the editor (another way to clear the selection)
	UFUNCTION(BlueprintCallable, Category = "Development|Editor")
	UE_API void SelectNothing();

	// Set the selection state for the selected actor
	UFUNCTION(BlueprintCallable, Category="Development|Editor")
	UE_API void SetActorSelectionState(AActor* Actor, bool bShouldBeSelected);

	UFUNCTION(BlueprintCallable, Category="Development|Editor")
	UE_API void GetSelectionBounds(FVector& Origin, FVector& BoxExtent, float& SphereRadius);

	// Renames an asset (cannot move folders)
	UFUNCTION(BlueprintCallable, Category = "Development|Editor")
	UE_API void RenameAsset(UObject* Asset, const FString& NewName);
	...
	UE_API void PostExecutionCleanup();
};

...
#include UE_INLINE_GENERATED_CPP_BY_NAME(FeaturePackContentSource)
	...
	{		
		...
		else
		{
			...
			if (FPlatformFileManager::Get().GetPlatformFile().FileExists(*FullPath))
			{
				...
				{
					UE_LOG(LogFeaturePack, Warning, TEXT("Unable to install feature pack: '%s'"), *FullPath);
				}
			}
			else
			{
				UE_LOG(LogFeaturePack, Warning, TEXT("Unable to find feature pack: '%s'"), *FullPath);
			}
		}

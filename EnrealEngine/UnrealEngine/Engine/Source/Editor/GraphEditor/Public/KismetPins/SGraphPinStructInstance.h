...
protected:
	...
	UE_API virtual void ParseDefaultValueData();

	/** Call to push changes from edit instance to pin */
	UE_API virtual void SaveDefaultValueData();

	/** Refreshes cached description and edit data after an edit change */
	UE_API virtual void RefreshCachedData();

	/** Creates widget used to edit the struct instance */
	UE_API virtual TSharedRef<SWidget> GetEditContent();

	/** Creates widget for displaying preview on the pin */
	UE_API virtual TSharedRef<SWidget> GetDescriptionContent();

	/** Slate accessor to shows cached description value */
	UE_API virtual FText GetCachedDescriptionText() const;

	/** Returns the base instance inside EditStruct, if null this is assumed to be a subclass that overrides other functions */
	UE_API virtual FPinStructEditWrapper* GetEditWrapper() const;

	/** Called when struct is modified by the details view */
	UE_API virtual void PropertyValueChanged(const FPropertyChangedEvent& PropertyChangedEvent);

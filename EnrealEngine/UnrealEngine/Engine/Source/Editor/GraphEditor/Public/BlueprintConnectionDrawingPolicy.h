...
protected:
	...
	UE_API virtual bool TreatWireAsExecutionPin(UEdGraphPin* InputPin, UEdGraphPin* OutputPin) const;

	// Determines the target Blueprint for querying the active debugging context.
	UE_API UBlueprint* GetTargetBlueprint() const;

	// Determines if the given Blueprint context has a valid object for debugging.
	UE_API bool CanBuildRoadmap(UBlueprint* TargetBP) const;

public:
	UE_API FKismetConnectionDrawingPolicy(int32 InBackLayerID, int32 InFrontLayerID, float ZoomFactor, const FSlateRect& InClippingRect, FSlateWindowElementList& InDrawElements, UEdGraph* InGraphObj);

	UE_API virtual void BuildExecutionRoadmap();

	UE_API virtual bool CanBuildRoadmap() const;

	UE_API void CalculateEnvelopeAlphas(double ExecutionTime, /*out*/ float& AttackAlpha, /*out*/ float& SustainAlpha) const;
	...
	UE_API virtual void DetermineStyleOfExecWire(float& Thickness, FLinearColor& WireColor, bool& bDrawBubbles, const FTimePair& Times);
...
protected: 
	...
	*/
	UE_API bool AreNodesGraphicallySequential(UEdGraphNode* InputNode, UEdGraphNode* OutputNode) const;
	 ...
	 */
	UE_API FTimePair const* BackTraceExecPath(UEdGraphPin const* const OutputPin, FExecPairingMap const* const NodeExecutionList);

	UE_API bool ShouldChangeTangentForKnot(class UK2Node_Knot* Knot);
	UE_API bool GetAverageConnectedPosition(class UK2Node_Knot* Knot, EEdGraphPinDirection Direction, FVector2f& OutPos) const;
	UE_API bool FindPinCenter(UEdGraphPin* Pin, FVector2f& OutCenter) const;
	static UE_API UObject* GetObjectBeingDebugged(UBlueprint* TargetBP);
};

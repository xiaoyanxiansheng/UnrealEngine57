...
public:
	...
	UE_API FText GetHeadTitle() const;

	/** Get the size of this title the last time it was drawn */
	UE_API UE::Slate::FDeprecateVector2DResult GetTitleSize() const;
...
public:
	...
	UE_API virtual void OnToolTipClosing() override;
	...
	UE_API virtual bool IsNodeEditable() const;

	/** Set event when node is double clicked */
	UE_API void SetDoubleClickEvent(FSingleNodeEvent InDoubleClickEvent);

	/** @param OwnerPanel  The GraphPanel that this node belongs to */
	UE_API virtual void SetOwner( const TSharedRef<SGraphPanel>& OwnerPanel );

	/** @return the editable title for a node */
	UE_API FString GetEditableNodeTitle() const;

	/** @return the editable title for a node */
	UE_API FText GetEditableNodeTitleAsText() const;

	/** @return the tint for the node's title image */
	UE_API virtual FSlateColor GetNodeTitleColor() const;

	/** @return the tint for the node's comment */
	UE_API FSlateColor GetNodeCommentColor() const;

	/** @return the tint for the node's main body */
	UE_API virtual FSlateColor GetNodeBodyColor() const;

	/** @return the image brush to use for the node */
	UE_API virtual const FSlateBrush * GetNodeBodyBrush() const;

	/** @return the tint for the node's title icon */
	UE_API virtual FSlateColor GetNodeTitleIconColor() const;

	/** @return the tint for the node's title text */
	UE_API virtual FLinearColor GetNodeTitleTextColor() const;

	/** @return the tooltip to display when over the node */
	UE_API virtual FText GetNodeTooltip() const;
	
	/** @return the node being observed by this widget*/
	UE_API UEdGraphNode* GetNodeObj() const;

	/** @return the node under the mouse (either this node or one of its children) */
	UE_API virtual TSharedRef<SGraphNode> GetNodeUnderMouse(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent);

	UE_API TSharedPtr<SGraphPanel> GetOwnerPanel() const;

	/**
	 * Update this GraphNode to match the data that it is observing
	 */
	UE_API virtual void UpdateGraphNode();
	
	/** Create the widgets for pins on the node */
	UE_API virtual void CreatePinWidgets();

	/** Create a single pin widget */
	UE_API virtual void CreateStandardPinWidget(UEdGraphPin* Pin);
	 ...
	 */
	UE_API void GetPins( TSet< TSharedRef<SWidget> >& AllPins ) const;
	UE_API void GetPins( TArray< TSharedRef<SWidget> >& AllPins ) const;
	 ...
	 */
	UE_API virtual TSharedPtr<SGraphPin> GetHoveredPin( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent ) const;

	UE_API TSharedPtr<SGraphPin> FindWidgetForPin( UEdGraphPin* ThePin ) const;

	UE_API void PlaySpawnEffect();
	...
	UE_API UE::Slate::FDeprecateVector2DResult NodeCoordToGraphCoord( const UE::Slate::FDeprecateVector2DParameter& PanelSpaceCoordinate ) const;

	UE_API UE::Slate::FDeprecateVector2DResult GetContentScale() const;
	UE_API FLinearColor GetColorAndOpacity() const;
	UE_API FLinearColor GetPinLabelColorAndOpacity() const;	

	/** Set event when text is committed on the node */
	UE_API void SetVerifyTextCommitEvent(FOnNodeVerifyTextCommit InOnVerifyTextCommit);
	/** Set event when text is committed on the node */
	UE_API void SetTextCommittedEvent(FOnNodeTextCommitted InDelegate);
	/** Set event when the user generates a warning tooltip because a connection was invalid */
	UE_API void SetDisallowedPinConnectionEvent(SGraphEditor::FOnDisallowedPinConnection InOnDisallowedPinConnection);
	/** called to replace this nodes comment text */
	UE_API virtual void OnCommentTextCommitted(const FText& NewComment, ETextCommit::Type CommitInfo);
	/** called when the node's comment bubble is toggled */
	UE_API virtual void OnCommentBubbleToggled(bool bInCommentBubbleVisible);
	...
	UE_API void RequestRename();

	/** Sets node into rename state if supported */
	UE_API void ApplyRename();

	/** Cancels the rename editing mode */
	UE_API void CancelRename();

	/** return rect of the title area */
	UE_API virtual FSlateRect GetTitleRect() const;

	/** Called from drag drop code when a disallowed connection is hovered */
	UE_API void NotifyDisallowedPinConnection(const class UEdGraphPin* PinA, const class UEdGraphPin* PinB) const;
	...
	UE_API void InvalidateGraphData();
	...
	// snapping:
	TArray<FArrangedWidget> GetArrangedSnappedChildNodes(const FGeometry& Geometry);
	TSharedPtr<SGraphNode> GetSnappedChildForPosition(const FGeometry& Geometry, const FVector2f& Position, FVector2f& OutPositionInNode);
	FGeometry GetNodeGeometry(const FGeometry& Geometry) const;
	void DrawChildrenNodeShadow( const FGraphPanelSelectionSet& SelectionSet, FSlateWindowElementList& OutDrawElements, int32 NodeShadowsLayerId, const FGeometry& Geometry, FLinearColor Color);
	/** Returns the set of pins that should have their connections drawn, including nested pins */
	void GetPinsToDraw( TSet< TSharedRef<SWidget> >& AllPins ) const;
	/** Returns all of the pins, including those nested within this node */
	void GetPinsIncludingSnappedChildren( TSet< TSharedRef<SWidget> >& AllPins ) const;
	void ForEachNodeIncludingChildren(TFunctionRef<void(const TSharedRef<SGraphNode>&)> Ftor);
	void ForEachPinOnSnappedChild(TFunctionRef<void(const TSharedRef<SGraphPin>&)> Ftor) const;
	void ForEachNodeSnappedToControlFlowPin(TFunctionRef<void(const TSharedRef<SGraphNode>&)> Ftor) const;
	void ForEachNodeSnappedToDataPin(TFunctionRef<void(const TSharedRef<SGraphNode>&)> Ftor) const;
protected:
	UE_API SGraphNode();

	UE_API void PositionThisNodeBetweenOtherNodes(const TMap< UObject*, TSharedRef<SNode> >& NodeToWidgetLookup, UEdGraphNode* PreviousNode, UEdGraphNode* NextNode, float HeightAboveWire) const;
	UE_API void PositionThisNodeBetweenOtherNodes(const TMap< UObject*, TSharedRef<SNode> >& NodeToWidgetLookup, TSet<UEdGraphNode*>& PreviousNodes, TSet<UEdGraphNode*>& NextNodes, float HeightAboveWire) const;
	UE_API void PositionThisNodeBetweenOtherNodes(const FVector2f& PrevPos, const FVector2f& NextPos, float HeightAboveWire) const;

	/**
	 * Check whether drag and drop functionality is permitted on the given node 
	 */
	static UE_API bool CanAllowInteractionUsingDragDropOp( const UEdGraphNode* GraphNodePtr, const TSharedPtr<FActorDragDropOp>& DragDropOp );
	 ...
	 */
	UE_API FText GetErrorMsgToolTip() const;
	 ...
	 */
	UE_API virtual void AddPin( const TSharedRef<SGraphPin>& PinToAdd );

	/** Hook that allows derived classes to supply their own SGraphPin derivatives for any pin. */
	UE_API virtual TSharedPtr<SGraphPin> CreatePinWidget(UEdGraphPin* Pin) const;
	...
	UE_API virtual bool IsNameReadOnly () const;

	/** Called when text is being committed to check for validity */
	UE_API bool OnVerifyNameTextChanged ( const FText& InText, FText& OutErrorMessage );

	/* Called when text is committed on the node */
	UE_API void OnNameTextCommited ( const FText& InText, ETextCommit::Type CommitInfo ) ;
	...
	UE_API EVisibility VisualWarningVisibility() const;

	/** Function to get a visual warning description string(visual warnings does not break the build) */
	UE_API FText GetVisualWarningMsgToolTip() const;

	/* Helper function to set the error color for the node */
	UE_API FSlateColor GetVisualWarningColor() const;
	...
	UE_API virtual void UpdateErrorInfo();

	/** Set-up the error reporting widget for the node */
	UE_API void SetupErrorReporting();

	// Should we use low-detail node titles?
	UE_API virtual bool UseLowDetailNodeTitles() const;
...
protected:
	...
	UE_API virtual void CreateAdvancedViewArrow(TSharedPtr<SVerticalBox> MainBox);

	/** Returns visibility of AdvancedViewButton */
	UE_API virtual EVisibility AdvancedViewArrowVisibility() const;

	/** Show/hide advanced view */
	UE_API virtual void OnAdvancedViewChanged( const ECheckBoxState NewCheckedState );

	/** hidden == unchecked, shown == checked */
	UE_API virtual ECheckBoxState IsAdvancedViewChecked() const;

	/** Up when shown, down when hidden */
	UE_API virtual const FSlateBrush* GetAdvancedViewArrow() const;

	/** Checks if the node is the only node selected */
	UE_API bool IsSelectedExclusively() const;

	virtual void SetDefaultTitleAreaWidget(TSharedRef<SOverlay> DefaultTitleAreaWidget) {}
	UE_API virtual TSharedRef<SWidget> CreateTitleWidget(TSharedPtr<SNodeTitle> NodeTitle);
	
	/** Optionally create a widget to dock to the right in the title bar of the node. */
	UE_API virtual TSharedRef<SWidget> CreateTitleRightWidget();
	
	/** Create the inner node content area, including the left/right pin boxes */
	UE_API virtual TSharedRef<SWidget> CreateNodeContentArea();
	...
	UE_API TSharedRef<SWidget> AddPinButtonContent(FText PinText, FText PinTooltipText, bool bRightSide = true, FString DocumentationExcerpt = FString(), TSharedPtr<SToolTip> CustomTooltip = NULL);
	...
	UE_API virtual void PopulateMetaTag(class FGraphNodeMetaData* TagMeta) const;

	/** Returns TRUE if the input pin should be hidden from view */
	UE_API bool ShouldPinBeHidden(const UEdGraphPin* InPin) const;

	/** Returns the widget to use for the enabled state of the node */
	UE_API virtual TSharedPtr<SWidget> GetEnabledStateWidget() const;

	/** Returns the widget to use for the corner icon of the node */
	UE_API virtual TSharedRef<SWidget> CreateIconWidget();
...
private:
	...
	friend struct FNodeSnappingManager; // want access to ControlFlowSequence, can reconsider design if snapping is successful
	TSharedPtr<SVerticalBox> ControlFlowSequence; // ref?
	TWeakPtr<SGraphNode> SnapParent; // dag so technically we can keep an owning reference.. but do we trust ourselves?
};

...
public:
	...
	struct FNodeMatch
	{
		 ...
		 */
		UE_API bool Diff(const FNodeDiffContext& DiffContext, TArray<FDiffSingleResult>* DiffsResultsOut = nullptr) const;
		UE_API bool Diff(const FNodeDiffContext& DiffContext, FDiffResults& DiffsOut) const;
	 ...
	 */
	static UE_API FNodeMatch FindNodeMatch(UEdGraph* OldGraph, UEdGraphNode* NewNode, TArray<FNodeMatch> const& PriorMatches);
	 ...
	 */
	static UE_API bool IsNodeMatch(UEdGraphNode* Node1, UEdGraphNode* Node2, bool bExactOnly = false, TArray<FGraphDiffControl::FNodeMatch> const* Exclusions = nullptr);
	 ...
	 */
	static UE_API bool DiffGraphs(UEdGraph* const OldGraph, UEdGraph* const NewGraph, TArray<FDiffSingleResult>& DiffsOut);

	/** Computes an object path for a specific graph, relative to the blueprint/asset root */
	static UE_API FString GetGraphPath(UEdGraph* Graph);
};

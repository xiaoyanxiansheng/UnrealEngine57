...
public:
	...
	UE_API void Construct(const FArguments& InArgs, FCreateWidgetForActionData* const InCreateData);
...
protected:
	...
	UE_API TSharedRef<SWidget> CreateIconWidget( const FText& IconToolTip, const FSlateBrush* IconBrush, const FSlateColor& IconColor, const FString& DocLink, const FString& DocExcerpt, const FSlateBrush* SecondaryIconBrush, const FSlateColor& SecondaryColor );
	/** Create an icon type widget */
	UE_API TSharedRef<SWidget> CreateIconWidget(const FText& IconToolTip, const FSlateBrush* IconBrush, const FSlateColor& IconColor);
	...
	UE_API virtual TSharedRef<SWidget> CreateTextSlotWidget( FCreateWidgetForActionData* const InCreateData, TAttribute<bool> bIsReadOnly );

	/** Callback when rename text is being verified on text changed */
	UE_API virtual bool OnNameTextVerifyChanged(const FText& InNewText, FText& OutErrorMessage);

	/** Callback when rename text is committed */
	UE_API virtual void OnNameTextCommitted(const FText& NewText, ETextCommit::Type InTextCommit);

	/** Callback to get the display text */ 
	UE_API virtual FText GetDisplayText() const;

	/** Callback to get the tooltip */
	UE_API virtual FText GetItemTooltip() const;
};
...
protected:
	UE_API virtual TSharedRef<SWidget> OnCreateWidgetForAction(FCreateWidgetForActionData* const InCreateData);

	UE_API virtual FReply OnActionDragged(const TArray< TSharedPtr<FEdGraphSchemaAction> >& InActions, const FPointerEvent& MouseEvent);

	UE_API virtual void RefreshActionsList(bool bPreserveExpansion);

...
UE_DECLARE_TCOMMANDS(class FGraphEditorCommandsImpl, GRAPHEDITOR_API)
...
public:
	...
	TSharedPtr< FUICommandInfo > GetFindReferences() const { return FindReferences; }
	...
	// Opens collapsed graphs, functions or macros in a new tab
...
class FGraphEditorCommands
{
public:
	static UE_API void Register();

	static UE_API const FGraphEditorCommandsImpl& Get();
	
	/** Build "Find References" submenu when a context allows for it */
	static UE_API void BuildFindReferencesMenu(FMenuBuilder& MenuBuilder);

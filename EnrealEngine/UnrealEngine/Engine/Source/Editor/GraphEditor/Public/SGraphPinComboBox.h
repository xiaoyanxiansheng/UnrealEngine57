...
public:
	...
	UE_API void RemoveItemByIndex(int32 InIndexToRemove);
	
private:

	//Function to set the newly selected item
	UE_API void OnSelectionChangedInternal( TSharedPtr<int32> NewSelection, ESelectInfo::Type SelectInfo );
	...
	UE_API TSharedRef<ITableRow> OnGenerateComboWidget( TSharedPtr<int32> InComboIndex, const TSharedRef<STableViewBase>& OwnerTable );
	
	/** Returns the target width of the enum selection menu. */
	UE_API FOptionalSize GetMenuDesiredWidth() const;

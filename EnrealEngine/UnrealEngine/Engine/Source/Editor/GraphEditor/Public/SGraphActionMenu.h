...
class SDefaultGraphActionWidget : public SCompoundWidget
{
	...
	UE_API void Construct(const FArguments& InArgs, const FCreateWidgetForActionData* InCreateData);
...
class SGraphActionMenu : public SCompoundWidget, public FGCObject
{
public:
	...
	UE_API void Construct( const FArguments& InArgs, bool bIsReadOnly = true );
	...
	UE_API void RefreshAllActions(bool bPreserveExpansion, bool bHandleOnSelectionEvent = true);

	/** Returns a map of all top level sections and their current expansion state. */
	UE_API void GetSectionExpansion(TMap<int32, bool>& SectionExpansion) const;

	/** Sets the sections to be expanded of all top level sections. */
	UE_API void SetSectionExpansion(const TMap<int32, bool>& SectionExpansion);
...
public:
	...
	UE_API void GetSelectedActions(TArray< TSharedPtr<FEdGraphSchemaAction> >& OutSelectedActions) const;
	...
	UE_API FString GetSelectedCategoryName() const;
	
	/** Get category child actions that is currently selected */
	UE_API void GetSelectedCategorySubActions(TArray<TSharedPtr<FEdGraphSchemaAction>>& OutActions) const;

	/** Get category child actions for the passed in action */
	UE_API void GetCategorySubActions(TWeakPtr<FGraphActionNode> InAction, TArray<TSharedPtr<FEdGraphSchemaAction>>& OutActions) const;
	 ...
	 */
	UE_API bool SelectItemByName(const FName& ItemName, ESelectInfo::Type SelectInfo = ESelectInfo::Direct, int32 SectionId = INDEX_NONE, bool bIsCategory = false );
...
protected:
	...
	UE_API bool TryToSpawnActiveSuggestion();

	/** Returns true if the tree should be autoexpanded */
	UE_API bool ShouldExpandNodes() const;

	/** Checks if the passed in node is safe for renaming */
	UE_API bool CanRenameNode(TWeakPtr<FGraphActionNode> InNode) const;
	...
	UE_API void OnFilterTextCommitted(const FText& InText, ETextCommit::Type CommitInfo);
	/** Get children  */
	UE_API void OnGetChildrenForCategory( TSharedPtr<FGraphActionNode> InItem, TArray< TSharedPtr<FGraphActionNode> >& OutChildren );
	/** Create widget for the supplied node */
	UE_API TSharedRef<ITableRow> MakeWidget( TSharedPtr<FGraphActionNode> InItem, const TSharedRef<STableViewBase>& OwnerTable, bool bIsReadOnly );
	 ...
	 */
	UE_API void OnItemSelected( TSharedPtr< FGraphActionNode > InSelectedItem, ESelectInfo::Type SelectInfo );
	 ...
	 */
	UE_API bool HandleSelection( TSharedPtr< FGraphActionNode > &InSelectedItem, ESelectInfo::Type InSelectionType );

	/** Called when tree item is double clicked */
	UE_API void OnItemDoubleClicked( TSharedPtr< FGraphActionNode > InClickedItem );
	/** Called when tree item dragged */
	UE_API FReply OnItemDragDetected( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent );
	/** Callback when rename text is committed */
	UE_API void OnNameTextCommitted(const FText& NewText, ETextCommit::Type InTextCommit, TWeakPtr< FGraphActionNode > InAction);
	/** Handler for when an item has scrolled into view after having been requested to do so */
	UE_API void OnItemScrolledIntoView( TSharedPtr<FGraphActionNode> InActionNode, const TSharedPtr<ITableRow>& InWidget );
	/** Callback for expanding tree items recursively */
	UE_API void OnSetExpansionRecursive(TSharedPtr<FGraphActionNode> InTreeNode, bool bInIsItemExpanded);
	/** Helper function for adding and scoring actions from our builder */
	UE_API void ScoreAndAddActions(int32 StartingIndex = INDEX_NONE);

	/** Helper functions for keyboard interaction */
	UE_API void SelectPreviousAction(int32 Num = 1);
	UE_API void SelectNextAction(int32 Num = 1);
	UE_API void SelectFirstAction();
	UE_API void SelectLastAction();
	/** General helper functions for accessing the filtered tree */
	UE_API TSharedPtr<FGraphActionNode> GetFirstAction();
	UE_API const TArray< TSharedPtr<FGraphActionNode> >& GetFilteredActionNodes(int32* OutSelectedIndex = nullptr);

...
#include UE_INLINE_GENERATED_CPP_BY_NAME(SNodePanel)
...
FNodeSnappingManager* SNodePanel::GetSnappingManager()
{
	return NodeSnappingManager.Get();
}

const FNodeSnappingManager* SNodePanel::GetSnappingManager() const
{
	return const_cast<SNodePanel*>(this)->GetSnappingManager();
}
...
FReply SNodePanel::OnMouseButtonDown( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
	...
	else if ( bIsLeftMouseButtonEffecting )
	{
		...
		{
			if(SnapManager->BeginDragWithSnap(MyGeometry, MouseEvent))
			{
...
void SNodePanel::FindNodesAffectedByMarquee( FGraphPanelSelectionSet& OutAffectedNodes ) const
{
	...
	const auto FindNodeAffectedByMarquee = [&OutAffectedNodes, &MarqueeSlateRect = std::as_const(MarqueeSlateRect)](const TSharedRef<SNode>& SomeNodeWidget)
	{
	...
	if(const FNodeSnappingManager* SnapManager = GetSnappingManager())
	{
		SnapManager->ForEachNode([&FindNodeAffectedByMarquee](const TSharedRef<SNode>& Node)
		{
			FindNodeAffectedByMarquee(Node);
		});
		return;
	}
	...
	{
		FindNodeAffectedByMarquee(Children[NodeIndex]);
	}	
}
...
void SNodePanel::FinalizeNodeMovements()
{
	if(FNodeSnappingManager* SnapManager = GetSnappingManager())
	{
		if(SnapManager->FinalizeNodeMovements())
		{
			return;
		}
	}

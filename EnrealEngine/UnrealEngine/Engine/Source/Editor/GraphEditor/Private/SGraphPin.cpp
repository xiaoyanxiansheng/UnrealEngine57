...
void SGraphPin::Construct(const FArguments& InArgs, UEdGraphPin* InPin)
{
	...
	TSharedPtr<SWidget> PinDeprecationIndicator;
	if (InPin->bDeprecated)
	{
		static const FName DeprecationWarning("Icons.WarningWithColor");
		PinDeprecationIndicator = SNew(SImage)
			.Image(FAppStyle::GetBrush(DeprecationWarning))
			.ToolTipText(FText::FromString(InPin->DeprecationMessage));
	}
	...
	if (!bIsInput)
	{
		...
		if (PinDeprecationIndicator)
		{
			LabelAndValue->AddSlot()
				.VAlign(VAlign_Center)
				[
					PinDeprecationIndicator.ToSharedRef()
				];
		}
	...
	bIsSnappingEnabled = SGraphPanel::IsSnappingEnabled();
}
...
FSlateColor SGraphPin::GetPinColor() const
{
	...
	if (GraphPin && !GraphPin->IsPendingKill())
	{
		...
		if (const UEdGraphSchema* Schema = GraphPin->GetSchema())
		{
			...
			{
				return Schema->GetPinColor(GraphPin) * FLinearColor(1.0f, 1.0f, 1.0f, 0.5f);
			}

			return Schema->GetPinColor(GraphPin) * PinColorModifier;
		}
	}
...
bool SGraphPin::SnapSGraphNode(TSharedRef<SGraphNode> Node)
{
	if(!SnappedDataNodeContainer)
	{
		const bool bIsInput = (GetDirection() == EGPD_Input);
		LabelAndValue->AddSlot()
			.Padding(bIsInput ? FMargin(5.f, 0.0f, 0.0f, 0.0f) : FMargin(0.0f, 0.0f, 5.f, 0.0f))
			.VAlign(VAlign_Center)
			[
				SAssignNew(SnappedDataNodeContainer, SBox)
				.Padding(0.0f)
			];
	}

	SnappedDataNodeContainer->SetContent(Node);
	SnappedNode = Node;
	return true;
}

void SGraphPin::ClearSnappedSGraphNode()
{
	if(SnappedDataNodeContainer) // control flow snaps won't have a valud widget
	{
		SnappedDataNodeContainer->SetContent(SNullWidget::NullWidget);
	}
	SnappedNode.Reset();
}

bool SGraphPin::IsSnapped() const
{
	if(!bIsSnappingEnabled)
	{
		return false;
	}
	...
	return GraphPin->IsSnapped();
}

TSharedPtr<SGraphNode> SGraphPin::GetSnappedChildForPosition(const FGeometry& MyGeometry, const FVector2f& Position, FVector2f& OutPositionInNode) const
{
	TSet< TSharedRef<SWidget> > ValueWidgets;
	TSharedPtr<SGraphNode> SnappedNodePinned = SnappedNode.Pin();
	if(!SnappedNodePinned)
	{
		return nullptr;
	}
	ValueWidgets.Add(SnappedNodePinned.ToSharedRef());
	TMap<TSharedRef<SWidget>, FArrangedWidget> Result;
	FindChildGeometries(MyGeometry, ValueWidgets, Result);
	if ( Result.Num() > 0 )
	{
		...
		{
			TSharedPtr<SGraphNode> HoveredNode = StaticCastSharedRef<SGraphNode>(
				ArrangedPins[HoveredPinIndex].Widget);
			OutPositionInNode = ArrangedPins[HoveredPinIndex].Geometry.AbsoluteToLocal(Position);
			return HoveredNode->GetSnappedChildForPosition(ArrangedPins[HoveredPinIndex].Geometry, Position, OutPositionInNode);
		}
	}
	return nullptr;
}

TOptional<FArrangedWidget> SGraphPin::GetArrangedSnappedChildNode(const FGeometry& MyGeometry) const
{
	TSharedPtr<SGraphNode> SnappedNodePinned = SnappedNode.Pin();
	if(!SnappedNodePinned)
	{
	...
	check(Result.Num() == 1);
	FArrangedChildren ArrangedChildNodes(EVisibility::Visible);
	Result.GenerateValueArray( ArrangedChildNodes.GetInternalArray() );
	return ArrangedChildNodes[0];
}

TSharedPtr<SGraphNode> SGraphPin::GetSnappedNode() const
{
	return SnappedNode.Pin();
}

...
{
	ValidEnumValues = InArgs._ValidEnumValues;
...
void SGraphPinEnum::GenerateComboBoxIndexes( TArray< TSharedPtr<int32> >& OutComboBoxIndexes )
{
	...
	if (EnumPtr)
	{
		...
		{
			// Ignore hidden enum values
			if( !EnumPtr->HasMetaData(TEXT("Hidden"), EnumIndex ) && (ValidEnumValues.IsEmpty() || ValidEnumValues.Contains(EnumPtr->GetNameByIndex(EnumIndex))))
			{

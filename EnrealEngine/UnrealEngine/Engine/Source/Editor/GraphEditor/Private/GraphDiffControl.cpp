...
static bool CheckGraphNodePin(UEdGraphPin* OldLinkedPin, UEdGraphPin* NewLinkedPin)
{
	if (!OldLinkedPin || !NewLinkedPin)
	{
		return false;
	}

	for (int32 i = 0; i < OldLinkedPin->LinkedTo.Num(); ++i)
	{
		UEdGraphPin* OldLinkedToPin = OldLinkedPin->LinkedTo[i];
		UEdGraphPin* NewLinkedToPin = nullptr;
		if(NewLinkedPin && OldLinkedToPin)
		{
			NewLinkedToPin = NewLinkedPin->LinkedTo.IsValidIndex(i) ? FindOtherLink(NewLinkedPin->LinkedTo, i, OldLinkedToPin) : nullptr;
		}

		if (!OldLinkedToPin || !NewLinkedToPin)
		{
			continue;
		}

		UEdGraphNode* OldLinkedToNode = OldLinkedToPin->GetOwningNode();
		UEdGraphNode* NewLinkedToNode = NewLinkedToPin->GetOwningNode();

		if (OldLinkedToNode && NewLinkedToNode && !FGraphDiffControl::IsNodeMatch(OldLinkedToNode, NewLinkedToNode))
		{
			return false;
		}
	}
...
{
	const UPackage* const TransientPackage = GetTransientPackage();
	...
	for(int32 i = 0;i<Size;++i)
	{
		...
		// ignore PinId for generated graphs, like the material graphs - instead do a more expensive comparison
		// of the pin's node to detect changed links:
		else if (OldNode->GetPackage() == TransientPackage && NewNode->GetPackage() == TransientPackage
			&& CheckGraphNodePin(OldLinkedPin, NewLinkedPin))
		{
			continue;
		}

...
FVector2f SGraphNode::GetDesiredSizeForMarquee2f() const
{
	return ControlFlowSequence ? UE::Slate::CastToVector2f(ControlFlowSequence->GetChildren()->GetChildAt(0)->GetCachedGeometry().GetLocalSize())
		: SNodePanel::SNode::GetDesiredSizeForMarquee2f();
}
	...
	this->GetOrAddSlot( ENodeZone::Center )
		...
		[
			...
			[
				...
				[
					SNew(SImage)
					.Image(this, &SGraphNode::GetNodeBodyBrush)
...
TSharedPtr<SWidget> SGraphNode::GetEnabledStateWidget() const
{
...
void SGraphNode::CancelRename()
{
	if (InlineEditableText.IsValid())
	{
		if(InlineEditableText->IsInEditMode())
		{
			InlineEditableText->ExitEditingMode();
		}
	}
}
...
TSharedRef<SWidget> SGraphNode::CreateIconWidget()
{
	// Get node icon
...
TArray<FArrangedWidget> SGraphNode::GetArrangedSnappedChildNodes(const FGeometry& Geometry)
{
	TArray<FArrangedWidget> FinalResult;
	TSet< TSharedRef<SWidget> > MyPins;
	{
		GetPins( MyPins );
	}
	TMap<TSharedRef<SWidget>, FArrangedWidget> Result;
	FindChildGeometries(Geometry, MyPins, Result);

	// walk each pin looking for a snapped child node, if found add it to final result:
	for(const TPair<TSharedRef<SWidget>, FArrangedWidget>& ArrangedPin : Result)
	{
		TSharedRef<SGraphPin> AsPin = StaticCastSharedRef<SGraphPin>(ArrangedPin.Key);
		TOptional<FArrangedWidget> ChildNode = AsPin->GetArrangedSnappedChildNode(ArrangedPin.Value.Geometry);
		if(ChildNode.IsSet())
		{
			FinalResult.Emplace(MoveTemp(ChildNode.GetValue()));
		}
	}
...
TSharedPtr<SGraphNode> SGraphNode::GetSnappedChildForPosition(const FGeometry& Geometry, const FVector2f& Position, FVector2f& OutPositionInNode )
{
	// check for control flow sequence child:
	if(ControlFlowSequence)
	{
		FChildren* ControlFlowChildren = ControlFlowSequence->GetChildren();
		if(ControlFlowChildren->Num() > 1)
		{
			TSet< TSharedRef<SWidget> > SubNodes;
			for(int32 I = 1; I < ControlFlowChildren->Num(); ++I)
			{
				SubNodes.Add( ControlFlowChildren->GetChildAt(I) );
			}
			TMap<TSharedRef<SWidget>, FArrangedWidget> Result;
			FindChildGeometries(Geometry, SubNodes, Result);
		
			for( const TPair<TSharedRef<SWidget>, FArrangedWidget>& SubNode : Result)
			{
				if(SubNode.Value.Geometry.IsUnderLocation( Position ))
				{
					return StaticCastSharedRef<SGraphNode>(SubNode.Key)->GetSnappedChildForPosition(SubNode.Value.Geometry, Position, OutPositionInNode);
				}
			}
		}
	}
	...
	if ( Result.Num() > 0 )
	{
		FArrangedChildren ArrangedPins(EVisibility::Visible);
		Result.GenerateValueArray( ArrangedPins.GetInternalArray() );
		int32 HoveredPinIndex = SWidget::FindChildUnderPosition( ArrangedPins, Position );
		if ( HoveredPinIndex != INDEX_NONE )
		{
			TSharedRef<SGraphPin> HoveredPin = StaticCastSharedRef<SGraphPin>(
				ArrangedPins[HoveredPinIndex].Widget);
			// if the hovered pin has a node then return that:
			TSharedPtr<SGraphNode> NodeSnappedToPin = HoveredPin->GetSnappedChildForPosition(ArrangedPins[HoveredPinIndex].Geometry, Position, OutPositionInNode);
			if(NodeSnappedToPin)
			{
				return NodeSnappedToPin;
			}
		}
	}
	OutPositionInNode = Geometry.AbsoluteToLocal(Position);
	return StaticCastSharedRef<SGraphNode>(AsShared());
}

FGeometry SGraphNode::GetNodeGeometry(const FGeometry& Geometry) const
{
	// if we have a snapped execution child we don't want to include it in our displayed geometry (e.g. dropshadow, selection outline):
	if(ControlFlowSequence )
	{
		TMap<TSharedRef<SWidget>, FArrangedWidget> Result;
		FindChildGeometries(Geometry, {ControlFlowSequence->GetChildren()->GetChildAt(0)}, Result); // this returns the geometry of the node itself, exc. its control flow children
		
		if ( Result.Num() > 0 )
		{
			TMap<TSharedRef<SWidget>, FArrangedWidget>::TIterator Begin(Result);
			return Begin->Value.Geometry;
		}
	}
	return Geometry;
}

void SGraphNode::DrawChildrenNodeShadow( const FGraphPanelSelectionSet& SelectionSet, FSlateWindowElementList& OutDrawElements, int32 NodeShadowsLayerId, const FGeometry& Geometry, FLinearColor Color)
{
	const FVector2f NodeShadowSize = UE::Slate::CastToVector2f(GetDefault<UGraphEditorSettings>()->GetShadowDeltaSize());
	const FSlateBrush* ShadowBrush = GetShadowBrush(SelectionSet.Contains(GraphNode));
	FGeometry ChildGeometry = GetNodeGeometry(Geometry);
	FSlateDrawElement::MakeBox(
		OutDrawElements,
		NodeShadowsLayerId,
		ChildGeometry.ToInflatedPaintGeometry(NodeShadowSize),
		ShadowBrush,
		ESlateDrawEffect::None,
		Color
	);

	TArray<FArrangedWidget> AllDataPinChildren = GetArrangedSnappedChildNodes(Geometry);
	for(const FArrangedWidget& DataPinNode : AllDataPinChildren)
	{
		// bugbugbug: this draws the node shadow behind our parent node, probably need to move it into onpaint:
		StaticCastSharedRef<SGraphNode>(DataPinNode.Widget)->DrawChildrenNodeShadow(SelectionSet, OutDrawElements, NodeShadowsLayerId, DataPinNode.Geometry, Color);
	}
	...
	if ( Result.Num() > 0 )
	{
		for( const TPair<TSharedRef<SWidget>, FArrangedWidget>& SubNode : Result)
		{
			TSharedRef<SGraphNode> ChildNode = StaticCastSharedRef<SGraphNode>(SubNode.Key);
			ChildNode->DrawChildrenNodeShadow(SelectionSet, OutDrawElements, NodeShadowsLayerId, SubNode.Value.Geometry, Color);
		}
	}
}

void SGraphNode::GetPinsIncludingSnappedChildren( TSet< TSharedRef<SWidget> >& AllPins ) const
{
	ForEachPinOnSnappedChild([&AllPins](const TSharedRef<SGraphPin>& GraphPin) {
		AllPins.Add(GraphPin);
	});
}

void SGraphNode::GetPinsToDraw( TSet< TSharedRef<SWidget> >& AllPins ) const
{
	ForEachPinOnSnappedChild([&AllPins](const TSharedRef<SGraphPin>& GraphPin) {
		if(!GraphPin->IsSnapped())
		{
			AllPins.Add(GraphPin);
		}
	});
...
void SGraphNode::ForEachNodeIncludingChildren(TFunctionRef<void(const TSharedRef<SGraphNode>&)> Ftor)
{
	Ftor(StaticCastSharedRef<SGraphNode>(AsShared()));
	ForEachNodeSnappedToControlFlowPin(	Ftor );
	ForEachNodeSnappedToDataPin(Ftor);
}

void SGraphNode::ForEachPinOnSnappedChild(TFunctionRef<void(const TSharedRef<SGraphPin>&)> Ftor) const
{
	for( TSharedRef<SGraphPin> InputPin : InputPins )
	{
		Ftor(InputPin);
		TSharedPtr<SGraphNode> SnappedNode = InputPin->GetSnappedNode();
		if(SnappedNode)
		{
			SnappedNode->ForEachPinOnSnappedChild(Ftor);
		}
	}
	
	for( TSharedRef<SGraphPin> OutputPin : OutputPins )
	{
		Ftor(OutputPin);
		TSharedPtr<SGraphNode> SnappedNode = OutputPin->GetSnappedNode();
		if(SnappedNode)
		{
			SnappedNode->ForEachPinOnSnappedChild(Ftor);
		}
	...
	// recurse into attached control flow pins:
	ForEachNodeSnappedToControlFlowPin([Ftor](const TSharedRef<SGraphNode>& AttachedNode) {
		AttachedNode->ForEachPinOnSnappedChild(Ftor);
	});
}

void SGraphNode::ForEachNodeSnappedToControlFlowPin(TFunctionRef<void(const TSharedRef<SGraphNode>&)> Ftor) const
{
	if(!ControlFlowSequence)
	{
		return;
	...
	for(int32 I = 1; I < ControlFlowChildren->Num(); ++I)
	{
		StaticCastSharedRef<SGraphNode>(ControlFlowChildren->GetChildAt(I))->ForEachNodeIncludingChildren(Ftor);
	}
}

void SGraphNode::ForEachNodeSnappedToDataPin(TFunctionRef<void(const TSharedRef<SGraphNode>&)> Ftor) const
{
	for( TSharedRef<SGraphPin> InputPin : InputPins )
	{
		TSharedPtr<SGraphNode> SnappedNode = InputPin->GetSnappedNode();
		if(SnappedNode)
		{
			SnappedNode->ForEachNodeIncludingChildren(Ftor);
		}
	}

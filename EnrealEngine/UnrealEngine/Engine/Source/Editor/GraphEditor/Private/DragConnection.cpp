		...
		for (UEdGraphPin* StartingPinObj : ValidSourcePins)
		{
			if (TargetPinObj != StartingPinObj)
			{
				...
				case EDragMode::CreateConnection:
					Response = Schema->CanCreateConnection( StartingPinObj, TargetPinObj );
					if( Response.Response == CONNECT_RESPONSE_DISALLOW && 
						Schema->ShouldShowPanelContextMenuForIncompatibleConnections())
					{
						// override the response, these pins aren't compatible but in this context
						// we can show the panel context menu listing compatible nodes:
						Response = FPinConnectionResponse(
							CONNECT_RESPONSE_MAKE_WITH_CONVERSION_NODE, 
							FString::Printf(
								TEXT("Search for Nodes matching %s (%s) and %s (%s)"), 
								*UEdGraphSchema_K2::TypeToText(StartingPinObj->PinType).ToString(), 
								LexToString(StartingPinObj->Direction), 
								*UEdGraphSchema_K2::TypeToText(TargetPinObj->PinType).ToString(), 
								LexToString(TargetPinObj->Direction)));
					}
					break;
				case EDragMode::RelinkConnection:
					Response = Schema->CanRelinkConnectionToPin(StartingPinObj, TargetPinObj);
					break;
				}
...
FReply FDragConnection::DroppedOnPin(const FVector2f& ScreenPosition, const FVector2f& GraphPosition)
{
	bool bError = false;
	...
	case EDragMode::CreateConnection:
	{
		...
		for (const FEdGraphPinHandle& PinA : ValidSourcePinHandles)
		{
			if ((PinA.GetPin() != NULL) && (PinB.GetPin() != NULL))
			{
				...
				if (Schema->TryCreateConnection(PinA.GetPin(), PinB.GetPin()))
				{
	...
	case EDragMode::RelinkConnection:
	{
		...
		if (SourceGraphPin && TargetGraphPin && HoveredGraphPin)
		{
			...
			if (Graph)
			{
				...
				bError = !Schema->TryRelinkConnectionTarget(SourceGraphPinHandle.GetPin(), TargetGraphPin, HoveredGraphPinHandle.GetPin(), SelectedGraphNodes);				if (!bError)
				{
	...
	if( Schema && Schema->ShouldShowPanelContextMenuForIncompatibleConnections() && 
		NodeList.IsEmpty())
	{
		// no connections made, lets show a list of compatible nodes if any:	
		TArray<UEdGraphPin*> PinObjects;
		ValidateGraphPinList(/*out*/ PinObjects); // this gathers source pins for the drag operation
		if(UEdGraphPin* CurrentHoveredPin = GetHoveredPin())
		{
			if(PinObjects.Contains(CurrentHoveredPin))
			{
				PinObjects.Reset(); // user dropped a pin on itself, ignore
			}
			else
			{
				PinObjects.Add(CurrentHoveredPin);
			}
		}
...
FReply FDragConnection::DroppedOnNode(const FVector2f& ScreenPosition, const FVector2f& GraphPosition)
{
...
FReply FDragConnection::DroppedOnPanel( const TSharedRef< SWidget >& Panel, const FVector2f& ScreenPosition, const FVector2f& GraphPosition, UEdGraph& Graph)
{
	if (DragMode != EDragMode::CreateConnection)
	{
		return FReply::Unhandled();

...
void SGraphEditorImpl::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{
	...
	{
		OnTickDelegate.Execute(AllottedGeometry, InCurrentTime, InDeltaTime);
	}
}
					...
					if (const UEdGraphSchema* GraphSchema = ContextObject->Graph->GetSchema())
					{
						if (ContextObject->Node->IsA<UMaterialGraphNode_Root>() && !ContextObject->Pin)
						{
							...
							TSharedRef<SButton> SubstrateConvertButton = SNew(SButton)
								.Text(LOCTEXT("SubstrateConvertButtonLabel", "\u26A0 Convert To Substrate \u26A0"))
...
void SGraphEditorImpl::SetOnTick(FOnGraphEditorTickDelegate InOnTickDelegate)
{
	OnTickDelegate = InOnTickDelegate;
}

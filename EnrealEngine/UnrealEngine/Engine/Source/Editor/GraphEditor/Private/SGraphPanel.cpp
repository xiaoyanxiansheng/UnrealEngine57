...
void SGraphPanel::Construct( const SGraphPanel::FArguments& InArgs )
{
	...
	if(FNodeSnappingManager::IsNodeSnappingEnabled())
	{
		NodeSnappingManager = MakePimpl<FNodeSnappingManager>(StaticCastSharedRef<SGraphPanel>(AsShared()));
	}
}
...
int32 SGraphPanel::OnPaint( const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled ) const
{
	...
	// Draw the child nodes
	{
		...
		for (int32 ChildIndex = 0; ChildIndex < ArrangedChildren.Num(); ++ChildIndex)
		{
			...
			if (bNodeIsVisible)
			{
				...
				{
					// draw drop shadow on connected pins:
					ChildNode->DrawChildrenNodeShadow(
						*SelectionToVisualize,
	...
	if (Children.Num() > 0 )
	{
		...
		for (int32 ChildIndex = 0; ChildIndex < Children.Num(); ++ChildIndex)
		{
			...
			{
				// if node snapping is enabled only unspapped pins will have their connections 
				// drawn, else this will be all visible pins:
				ChildNode->GetPinsToDraw(VisiblePins);
			}
		}
...
void SGraphPanel::GetAllPins(TSet< TSharedRef<SWidget> >& AllPins)
{
	...
	{
		TSharedRef<SGraphNode> ChildNode = StaticCastSharedRef<SGraphNode>(Children[ChildIndex]);
		ChildNode->GetPinsIncludingSnappedChildren(AllPins);
	}
}
...
TSharedPtr<SWidget> SGraphPanel::OnSummonContextMenu(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	if(!EdNodeUnderMouse && !PinUnderCursor)
	{
		// if there's a wire under the mouse, use its begin and end pins as the source pins:
		UEdGraphPin* Pin1;
		UEdGraphPin* Pin2;
		if (PreviousFrameSplineOverlap.GetPins(*this, Pin1, Pin2))
		{
	...
	return SummonContextMenu(MouseEvent.GetScreenSpacePosition(), NodeAddPosition, EdNodeUnderMouse, PinUnderCursor, SourcePins);
}
...
FReply SGraphPanel::OnMouseMove(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	if(NodeSnappingManager && NodeSnappingManager->IsDragging())
	{
		NodeSnappingManager->UpdateDrag(MyGeometry, MouseEvent);
		return FReply::Handled();
	}
...
TSharedPtr<SGraphNode> SGraphPanel::GetGraphNodeUnderMouse(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	FArrangedGraphNode ArrangedWidget = {};
	if (FNodeSnappingManager* SnapManager = GetSnappingManager())
	{
		ArrangedWidget = SnapManager->GetGraphNodeUnderMouse(MyGeometry, MouseEvent);
	}
	else
	{
		...
		if (HoveredNodeIndex != INDEX_NONE)
		{
			...
			ArrangedWidget.Geometry = HoveredNode.Geometry;
			ArrangedWidget.Widget = StaticCastSharedRef<SGraphNode>(HoveredNode.Widget);
		}
	}
	
	if (ArrangedWidget.Widget)
	{
		...
		TSharedPtr<SGraphNode> GraphSubNode = ArrangedWidget.Widget->GetNodeUnderMouse(ArrangedWidget.Geometry, MouseEvent);
		ArrangedWidget.Widget = GraphSubNode.IsValid() ? GraphSubNode.ToSharedRef() : ArrangedWidget.Widget;
		if (!SelectionManager.IsNodeSelected(ArrangedWidget.Widget->GetObjectBeingDisplayed()))
		{
			SelectionManager.SelectSingleNode(ArrangedWidget.Widget->GetObjectBeingDisplayed());
		}
	}

	return ArrangedWidget.Widget;
}
...
{
	if(NodeSnappingManager)
	{
		NodeSnappingManager->RemoveNode(Node);
	}
...
void SGraphPanel::Update()
{
	...
	if(NodeSnappingManager)
	{
		NodeSnappingManager->HandleGraphChanged();
	}
...
bool SGraphPanel::IsSnappingEnabled()
{
	return FNodeSnappingManager::IsNodeSnappingEnabled();
}

...
#include UE_INLINE_GENERATED_CPP_BY_NAME(ClothingAssetFactory)
...
const UClass* UClothingAssetFactory::GetSupportedSourceAssetType() const
{
	return USkeletalMesh::StaticClass();
}

TArray<UClothingAssetBase*> UClothingAssetFactory::CreateFromSourceAsset(USkeletalMesh* TargetMesh, const UObject* SourceAsset) const
{
	TArray<UClothingAssetBase*> ClothingAssets;
	if (const USkeletalMesh* const SourceSkeletalMesh = Cast<USkeletalMesh>(SourceAsset))
	{
		for (const UClothingAssetBase* const ClothingAsset : SourceSkeletalMesh->GetMeshClothingAssets())
		{
PRAGMA_DISABLE_DEPRECATION_WARNINGS  // UE_DEPRECATED(5.7, ""): CreateFromExistingCloth will be moved out of UClothingAssetFactoryBase to this class
			if (UClothingAssetBase* const NewAsset =
				const_cast<UClothingAssetFactory*>(this)->CreateFromExistingCloth(
					TargetMesh,
					const_cast<USkeletalMesh*>(SourceSkeletalMesh),
					const_cast<UClothingAssetBase*>(ClothingAsset)))
{
				ClothingAssets.Emplace(NewAsset);
			}
...
UClothingAssetBase* UClothingAssetFactory::CreateFromExistingCloth(USkeletalMesh* TargetMesh, const USkeletalMesh* /*SourceMesh*/, const UClothingAssetBase* SourceAsset) const
{
	const UClothingAssetCommon* const SourceClothingAsset = Cast<const UClothingAssetCommon>(SourceAsset);
	...
	const FName NewAssetName = MakeUniqueObjectName(TargetMesh, UClothingAssetCommon::StaticClass(), SourceClothingAsset->GetFName());
	UClothingAssetCommon* const NewAsset = DuplicateObject<UClothingAssetCommon>(SourceClothingAsset, TargetMesh, NewAssetName);
...
UClothingAssetBase* UClothingAssetFactory::ImportLodToClothing(USkeletalMesh* TargetMesh, FSkeletalMeshClothBuildParams& Params)
{
	...
	if(UClothingAssetBase* TargetClothing = Params.TargetAsset.Get())
	{
		...
		if(TargetMesh->GetMeshClothingAssets().Find(TargetClothing, MeshAssetIndex))
		{
			...
			if(UClothingAssetCommon* const ConcreteTarget = Cast<UClothingAssetCommon>(TargetClothing))
			{

...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AnimationBlueprintLibrary)
...
UAnimNotify* UAnimationBlueprintLibrary::AddAnimationNotifyEventFromSource(UAnimSequenceBase* AnimationSequenceBase, float StartTime, FAnimNotifyEvent NotifyEvent, FName NotifyTrackName)
{
	UAnimNotify* ResNotify = nullptr;
	
	if (AnimationSequenceBase)
	{
		TObjectPtr<UAnimNotify> Notify = NotifyEvent.Notify;
		const bool bValidNotify = Notify != nullptr;
		const bool bValidNotifyState = NotifyEvent.NotifyStateClass != nullptr;
		const bool bValidOuter = bValidNotify && Notify->GetOuter() == AnimationSequenceBase;
		const bool bIsValidTrackName = IsValidAnimNotifyTrackName(AnimationSequenceBase, NotifyTrackName);
		const bool bIsValidTime = IsValidTimeInternal(AnimationSequenceBase, StartTime);
		
		if (bValidNotify && bIsValidTrackName && bIsValidTime && !bValidNotifyState)
		{
			// Our actual new event we will copy.
			FAnimNotifyEvent& NewEvent = AnimationSequenceBase->Notifies.AddDefaulted_GetRef();

			// Copy over only blueprint exposed types (to support for adding info such as trigger weight threshold, filtering type, etc).
			const UScriptStruct* ScriptStruct = FAnimNotifyEvent::StaticStruct();
			for (TFieldIterator<FProperty> PropIt(ScriptStruct); PropIt; ++PropIt)
			{
				...
				{
					Property->CopyCompleteValue_InContainer(&NewEvent, &NotifyEvent);
				}
			}

			// Duplicate notify since its outer is not the target animation sequence.
			if (!bValidOuter)
			{
				UAnimNotify* DuplicatedNotify = static_cast<UAnimNotify*>(StaticDuplicateObject(Notify, AnimationSequenceBase, NAME_None, RF_AllFlags, nullptr, EDuplicateMode::Normal, ~EInternalObjectFlags::RootSet));
				check(DuplicatedNotify && DuplicatedNotify->CanBePlaced(AnimationSequenceBase));
				
				NewEvent.Notify = DuplicatedNotify;
			}
			else
			{
				NewEvent.Notify = Notify;
			...
			// Now we have our final notify instance.
			ResNotify = NewEvent.Notify;
			
			// Update info on new notify.
			NewEvent.NotifyName = NAME_None;
			NewEvent.Link(AnimationSequenceBase, StartTime);
			NewEvent.TriggerTimeOffset = GetTriggerTimeOffsetForType(AnimationSequenceBase->CalculateOffsetForNotify(StartTime));
			NewEvent.TrackIndex = GetTrackIndexForAnimationNotifyTrackName(AnimationSequenceBase, NotifyTrackName);
			...
			// Refresh all cached data
			AnimationSequenceBase->MarkPackageDirty();
			AnimationSequenceBase->RefreshCacheData();
		}
		else
		{
			...
			{
				UE_LOG(LogAnimationBlueprintLibrary, Warning, TEXT("Adding Animation Notify States is not supported yet."))
			}
...
int32 UAnimationBlueprintLibrary::GetNumCurveMetaData(const USkeleton* Skeleton)
{
	if (Skeleton)
	{
	...
	{
		UE_LOG(LogAnimationBlueprintLibrary, Warning, TEXT("Invalid Skeleton ptr for GetNumCurveMetaData"));
	}

	return 0;
}

bool UAnimationBlueprintLibrary::AddCurveMetaData(USkeleton* Skeleton, FName CurveName, bool bTransact /*= true*/)
{
	if (Skeleton)
	{
		return Skeleton->AddCurveMetaData(CurveName, bTransact);
	}
	else
	{
		UE_LOG(LogAnimationBlueprintLibrary, Warning, TEXT("Invalid Skeleton ptr for AddCurveMetaData"));
	}

	return false;
}

void UAnimationBlueprintLibrary::GetCurveMetaDataNames(const USkeleton* Skeleton, TArray<FName>& OutNames)
{
	if (Skeleton)
	{
		Skeleton->GetCurveMetaDataNames(OutNames);
	}
	else
	{
		UE_LOG(LogAnimationBlueprintLibrary, Warning, TEXT("Invalid Skeleton ptr for GetCurveMetaDataNames"));
	}
}

bool UAnimationBlueprintLibrary::RenameCurveMetaData(USkeleton* Skeleton, FName OldName, FName NewName)
{
	if (Skeleton)
	{
		return Skeleton->RenameCurveMetaData(OldName, NewName);
	}
	else
	{
		UE_LOG(LogAnimationBlueprintLibrary, Warning, TEXT("Invalid Skeleton ptr for RenameCurveMetaData"));
	}

	return false;
}

bool UAnimationBlueprintLibrary::RemoveCurveMetaData(USkeleton* Skeleton, FName CurveName)
{
	if (Skeleton)
	{
		return Skeleton->RemoveCurveMetaData(CurveName);
	}
	else
	{
		UE_LOG(LogAnimationBlueprintLibrary, Warning, TEXT("Invalid Skeleton ptr for RemoveCurveMetaData"));
	}

	return false;
}

bool UAnimationBlueprintLibrary::GetCurveMetaDataMaterial(const USkeleton* Skeleton, FName CurveName)
{
	if (Skeleton)
	{
	...
	{
		UE_LOG(LogAnimationBlueprintLibrary, Warning, TEXT("Invalid Skeleton ptr for GetCurveMetaDataMaterial"));
	}

	return false;
}

bool UAnimationBlueprintLibrary::GetCurveMetaDataMorphTarget(const USkeleton* Skeleton, FName CurveName)
{
	if (Skeleton)
	{
	...
	{
		UE_LOG(LogAnimationBlueprintLibrary, Warning, TEXT("Invalid Skeleton ptr for GetCurveMetaDataMorphTarget"));
	}

	return false;
}

void UAnimationBlueprintLibrary::SetCurveMetaDataMaterial(USkeleton* Skeleton, FName CurveName, bool bOverrideMaterial)
{
	if (Skeleton)
	{
		Skeleton->SetCurveMetaDataMaterial(CurveName, bOverrideMaterial);
	}
	else
	{
		UE_LOG(LogAnimationBlueprintLibrary, Warning, TEXT("Invalid Skeleton ptr for SetCurveMetaDataMaterial"));
	}
}


void UAnimationBlueprintLibrary::SetCurveMetaDataMorphTarget(USkeleton* Skeleton, FName CurveName, bool bOverrideMorphTarget)
{
	if (Skeleton)
	{
		Skeleton->SetCurveMetaDataMorphTarget(CurveName, bOverrideMorphTarget);
	}
	else
	{
		UE_LOG(LogAnimationBlueprintLibrary, Warning, TEXT("Invalid Skeleton ptr for SetCurveMetaDataMorphTarget"));
	}
}

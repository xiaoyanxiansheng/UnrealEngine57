...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AnimPose)
...
void FAnimPose::SetPose(USkeletalMeshComponent* Component)
{
	...
	{		
		...
		for (const FBoneIndexType BoneIndex : ContextBoneContainer.GetBoneIndicesArray())
		{
			...
			if (SkeletonBoneIndex != INDEX_NONE)
			{
				const int32 LocalBoneIndex = BoneIndices.IndexOfByKey(SkeletonBoneIndex);
				if (LocalBoneIndex != INDEX_NONE)
				{
					LocalSpacePoses[LocalBoneIndex] = BoneSpaceTransforms[BoneIndex];
				}
				else
				{
					UE_LOG(LogAnimationPoseScripting, Warning, TEXT("Failed to find local Bone Index for Skeleton Bone Index %i [%s]"), SkeletonBoneIndex, *ContextBoneContainer.GetReferenceSkeleton().GetBoneName(SkeletonBoneIndex).ToString());
				}
			}
			else
			{
				UE_LOG(LogAnimationPoseScripting, Warning, TEXT("Failed to find Skeleton index for Compact Pose Bone Index %i"), BoneIndex);
			}
		}
	...
	if (IsInitialized())
	{
		...
		for (const FCompactPoseBoneIndex BoneIndex : CompactPose.ForEachBoneIndex())
		{
			...
			if (SkeletonBoneIndex != INDEX_NONE)
			{
				...
				if (LocalBoneIndex != INDEX_NONE)
				{
					LocalSpacePoses[LocalBoneIndex] = CompactPose[BoneIndex];
				}
				else
				{
					UE_LOG(LogAnimationPoseScripting, Warning, TEXT("Failed to find local Bone Index for Skeleton Bone Index %i [%s]"), SkeletonBoneIndex, *ContextBoneContainer.GetReferenceSkeleton().GetBoneName(SkeletonBoneIndex).ToString());
			...
			{
				UE_LOG(LogAnimationPoseScripting, Warning, TEXT("Failed to find Skeleton index for Compact Pose Bone Index %i"), BoneIndex.GetInt());
			}
		}

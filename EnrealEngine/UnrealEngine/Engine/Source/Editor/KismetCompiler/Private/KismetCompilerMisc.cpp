...
static bool DoesTypeNotMatchProperty(UEdGraphPin* SourcePin, const FEdGraphPinType& OwningType, const FEdGraphTerminalType& TerminalType, FProperty* TestProperty, FCompilerResultsLog& MessageLog, UClass* SelfClass)
{
	...
	else if ((PinCategory == UEdGraphSchema_K2::PC_Object) || (PinCategory == UEdGraphSchema_K2::PC_Interface) || (PinCategory == UEdGraphSchema_K2::PC_SoftObject))
	{
		...
		else
		{
			...
			{
				const UClass* OutputClass = (Direction == EGPD_Output) ? ObjectType : ObjProperty->PropertyClass.Get();
				const UClass* InputClass  = (Direction == EGPD_Output) ? ObjProperty->PropertyClass.Get() : ObjectType;
...
{
	static const FName ProblemBPClassName(TEXT("QuestUpdatesLog_C"));
	...
	bool bDoTempValidate = false;
	int32 NumAssignments = 0;
	if(CompilerContext.NewClass && CompilerContext.NewClass->GetFName() == ProblemBPClassName)
	{
		bDoTempValidate = true;
	}
	...
	if(bDoTempValidate)
	{
		ensureMsgf(
			NumAssignments == 4, 
				TEXT("Incorrect link set detected, %d %s %s %d"), 
				NumAssignments, 
				*ForClass->GetName(), 
				ForClass->bLayoutChanging ? TEXT("layout changing") : TEXT("layout NOT changing"),
				ForClass->PropertiesSize);
	}
...
FFieldVariant FKismetCompilerUtilities::CheckPropertyNameOnScope(UStruct* Scope, const FName& PropertyName)
{
	...
	if (UObject* ExistingObject = FindObject<UObject>(Scope, *NameStr, EFindObjectFlags::None))
	{
...
ConvertibleSignatureMatchResult FKismetCompilerUtilities::DoSignaturesHaveConvertibleFloatTypes(const UFunction* A, const UFunction* B)
{
	...
	if (!A->IsSignatureCompatibleWith(B))
	{
		...
		while (PropAIt && (PropAIt->PropertyFlags & CPF_Parm))
		{
			if (PropBIt && (PropBIt->PropertyFlags & CPF_Parm))
			{
		...
		return (PropBIt && PropBIt->PropertyFlags & CPF_Parm) ? ConvertibleSignatureMatchResult::Different : ConvertibleSignatureMatchResult::HasConvertibleFloatParams;
	}
...
void FKismetFunctionContext::FinalSortLinearExecList()
{
	...
	while (UnsortedExecutionSet.Num())
	{
		...
		for (auto Iter = NodesToStartNextChain.CreateIterator(); !NextNode && Iter; ++Iter)
		{
			if (UnsortedExecutionSet.Remove(*Iter))
			{
				NextNode = *Iter;
			}
			Iter.RemoveCurrent();

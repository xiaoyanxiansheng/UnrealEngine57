...
#if (MI_INTPTR_SIZE >= 8)
// BEGIN EPIC MOD - Always use VirtualAlloc2 by default to avoid random OOM caused by mi_os_get_aligned_hint returning
// potentially already used address space.
  ...
  // fall through if VirtualAlloc2 is unavailable or we don't need to align...
...
// END EPIC MOD - Always use VirtualAlloc2 by default
}

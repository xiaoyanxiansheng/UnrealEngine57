...
static gulong (*g_sdl_signal_connect_data)(gpointer instance, const gchar *detailed_signal, GCallback c_handler, gpointer data, GClosureNotify destroy_data, GConnectFlags connect_flags);
static void (*g_sdl_object_unref)(gpointer object);
static gchar *(*g_sdl_mkdtemp)(gchar *template);
gpointer (*g_sdl_object_ref_sink)(gpointer object);
gpointer (*g_sdl_object_ref)(gpointer object);
#else
...
/* EG BEGIN */
#ifdef SDL_WITH_EPIC_EXTENSIONS
#define g_sdl_signal_connect(instance, detailed_signal, c_handler, data) \
    g_sdl_signal_connect_data ((instance), (detailed_signal), (c_handler), (data), NULL, (GConnectFlags) 0)
#else
...
static bool init_gtk(void)
{
    ...
    /* EG BEGIN */
#ifdef SDL_WITH_EPIC_EXTENSIONS
    g_sdl_mkdtemp = dlsym(libgdk, "g_mkdtemp");
    g_sdl_signal_connect_data = dlsym(libgdk, "g_signal_connect_data");
    g_sdl_object_unref = dlsym(libgdk, "g_object_unref");
    g_sdl_object_ref_sink = dlsym(libgdk, "g_object_ref_sink");
    g_sdl_object_ref = dlsym(libgdk, "g_object_ref");
#else
...
#ifdef SDL_WITH_EPIC_EXTENSIONS
        !g_sdl_mkdtemp ||
        !g_sdl_object_ref_sink ||
        !g_sdl_object_ref ||
        !g_sdl_signal_connect_data ||
        !g_sdl_object_unref ||
#else        
        !g_mkdtemp ||
        !g_object_ref_sink ||
...
/* EG BEGIN */
#ifdef SDL_WITH_EPIC_EXTENSIONS    
    if (!g_sdl_mkdtemp(tray->icon_dir)) {
...
/* EG BEGIN */
#ifdef SDL_WITH_EPIC_EXTENSIONS
    tray->menu_cached = (GtkMenuShell *) g_sdl_object_ref_sink(gtk_menu_new());
#else
...
/* EG BEGIN */
#ifdef SDL_WITH_EPIC_EXTENSIONS
    tray->menu->menu = g_sdl_object_ref(tray->menu_cached);
#else
...
/* EG BEGIN */
#ifdef SDL_WITH_EPIC_EXTENSIONS
    g_sdl_signal_connect(entry->item, "activate", G_CALLBACK(call_callback), entry);
#else
...
#ifdef SDL_WITH_EPIC_EXTENSIONS
    if (tray->menu_cached) {
        g_sdl_object_unref(tray->menu_cached);
    }
#else
    if (tray->menu_cached) {
...
#ifdef SDL_WITH_EPIC_EXTENSIONS
    if (tray->indicator) {
        g_sdl_object_unref(tray->indicator);
    }
#else
    if (tray->indicator) {

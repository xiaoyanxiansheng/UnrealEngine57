...
static void SDLCALL Pulseaudio_AudioDeviceDisconnected_Deferred(void *userdata)
{
    SDL_AudioDeviceDisconnected(SDL_FindPhysicalAudioDeviceByCallback(FindAudioDeviceByIndex, userdata));
}
...
/* EG BEGIN */
#ifdef SDL_WITH_EPIC_EXTENSIONS
            // Due to deadlock issues, do this on the main thread in the event loop
            SDL_RunOnMainThread(Pulseaudio_AudioDeviceDisconnected_Deferred,
                    (void*)(uintptr_t)idx,
                    false);
#else            

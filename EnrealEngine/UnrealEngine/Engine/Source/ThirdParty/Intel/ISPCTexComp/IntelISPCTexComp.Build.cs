...
public class IntelISPCTexComp : ModuleRules
{
	public IntelISPCTexComp(ReadOnlyTargetRules Target) : base(Target)
	{
		...
		{
			string LibArchPath = (Target.Architecture == UnrealArch.Arm64) ? "arm64/" : "x64/";
			string DLLArchPrefix = (Target.Architecture == UnrealArch.Arm64) ? "WinArm64-" : "Win64-";

            string configName = bUseDebugBuild ? "Debug" : "Release";
    		string LibraryPath = SourcePath + "ISPC Texture Compressor/" + LibArchPath + configName + "/";
            string DLLFolder = BinaryFolder + DLLArchPrefix + configName;

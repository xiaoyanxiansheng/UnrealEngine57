...
rem Specify architecture-specific parameters
rem  ...Clang is required to build the sse2neon.h emulation code. Can revist this if newer versions of Embree support neon/arm64 natively
set CMAKE_ARCH_PARAMS=
if /I "%ARCH_NAME%"=="arm64" (
    set CMAKE_ARCH_PARAMS=%CMAKE_ARCH_PARAMS%^
    -DEMBREE_ARM=ON^
    -DCMAKE_C_COMPILER_ID="clang"^
    -DCMAKE_CXX_COMPILER_ID="clang++"^
    -DCMAKE_CXX_STANDARD_LIBRARIES="advapi32.lib kernel32.lib"^
    -T ClangCL	
)

...
// XXX(Epic Games):
// On some platforms such as Linux, we will typically build the engine
// with RTTI disabled, so we allow OpenVDB to be used with RTTI disabled
// by avoiding the use of typeid.
...
namespace math {
// XXX(Epic Games): Instantiations of cwiseAdd() were missing for
// Vec2 and Vec4 of math::half.
template<>
inline auto cwiseAdd(const math::Vec2<math::half>& v, const float s)
{
    math::Vec2<math::half> out;
    const math::half* ip = v.asPointer();
    math::half* op = out.asPointer();
    for (unsigned i = 0u; i < 2u; ++i, ++op, ++ip) {
...
inline auto cwiseAdd(const math::Vec3<math::half>& v, const float s)
{
    ...
    const math::half* ip = v.asPointer();
    math::half* op = out.asPointer();
    for (unsigned i = 0u; i < 3u; ++i, ++op, ++ip) {
        OPENVDB_NO_TYPE_CONVERSION_WARNING_BEGIN
        *op = *ip + s;
        OPENVDB_NO_TYPE_CONVERSION_WARNING_END
    }
...
template<>
inline auto cwiseAdd(const math::Vec4<math::half>& v, const float s)
{
    math::Vec4<math::half> out;
    const math::half* ip = v.asPointer();
    math::half* op = out.asPointer();
    for (unsigned i = 0u; i < 4u; ++i, ++op, ++ip) {
        OPENVDB_NO_TYPE_CONVERSION_WARNING_BEGIN
        *op = *ip + s;
        OPENVDB_NO_TYPE_CONVERSION_WARNING_END
    }

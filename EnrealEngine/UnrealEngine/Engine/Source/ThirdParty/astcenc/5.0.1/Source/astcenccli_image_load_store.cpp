...
static bool store_dds_uncompressed_image(
	...
	// Collect image data to write
	uint8_t* dds_data8 = nullptr;
	uint16_t* dds_data16 = nullptr;

	if (bitness == 8)
	{
		dds_data8 = new uint8_t[dim_x * dim_y * dim_z * image_components];
		...
		for (unsigned int z = 0; z < dim_z; z++)
		{
			...
			{
				int ym = y_flip ? dim_y - y - 1 : y;
				uint8_t* src_row = data8 + 4 * dim_x * ym;
				uint8_t* dest_row = dds_data8 + (z * dim_y + y) * dim_x * image_components;
				...
				case 1:		// single-component, treated as Luminance
					for (unsigned int x = 0; x < dim_x; x++)
					{
						dest_row[x] = src_row[4 * x    ];
					}
					break;
				case 2:		// two-component, treated as Luminance-Alpha
					for (unsigned int x = 0; x < dim_x; x++)
					{
						dest_row[2 * x    ] = src_row[4 * x    ];
						dest_row[2 * x + 1] = src_row[4 * x + 3];
					}
					break;
				case 3:		// three-component, treated as RGB
					for (unsigned int x = 0; x < dim_x; x++)
					{
						dest_row[3 * x    ] = src_row[4 * x    ];
						dest_row[3 * x + 1] = src_row[4 * x + 1];
						dest_row[3 * x + 2] = src_row[4 * x + 2];
					}
					break;
				case 4:		// four-component, treated as RGBA
					for (unsigned int x = 0; x < dim_x; x++)
					{
						dest_row[4 * x    ] = src_row[4 * x    ];
						dest_row[4 * x + 1] = src_row[4 * x + 1];
						dest_row[4 * x + 2] = src_row[4 * x + 2];
						dest_row[4 * x + 3] = src_row[4 * x + 3];
					}
					break;
				}
			}
	...
	{
		dds_data16 = new uint16_t[dim_x * dim_y * dim_z * image_components];
		...
		for (unsigned int z = 0; z < dim_z; z++)
		{
			...
			{
				int ym = y_flip ? dim_y - y - 1: y;
				uint16_t* src_row = data16 + 4 * dim_x * ym;
				uint16_t* dest_row = dds_data16 + (y * dim_y + y) * dim_x * image_components;
	...
	{
		void *dataptr = (bitness == 16) ?
			reinterpret_cast<void*>(dds_data16) :
			reinterpret_cast<void*>(dds_data8);

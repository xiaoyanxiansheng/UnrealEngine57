...
static int edit_astcenc_config(
	...
	while (argidx < argc)
	{
		...
		else if (!strcmp(argv[argidx], "-rdo-lambda"))
		{
			...
			config.lz_rdo_lambda = static_cast<float>(atof(argv[argidx - 1]));
		}
		else if (!strcmp(argv[argidx], "-rdo-endpoints"))
		{
			argidx += 2;
			if (argidx > argc)
			{
				print_error("ERROR: -rdo-endpoints switch with no argument\n");
				return 1;
			}

			config.tune_lz_rdo_endpoint_history_size = atoi(argv[argidx - 1]);
		}
		else if (!strcmp(argv[argidx], "-rdo-weights"))
		{
			argidx += 2;
			if (argidx > argc)
			{
				print_error("ERROR: -rdo-weights switch with no argument\n");
				return 1;
			}

			config.tune_lz_rdo_weight_history_size = atoi(argv[argidx - 1]);
		}
		else if (!strcmp(argv[argidx], "-rdo-passes"))
		{
			argidx += 2;
			if (argidx > argc)
			{
				print_error("ERROR: -rdo-passes switch with no argument\n");
				return 1;
			}

			config.tune_lz_rdo_num_passes = atoi(argv[argidx - 1]);
		}
...
static void print_astcenc_config(
	...
	if (!cli_config.silentmode)
	{
		...
		if ((config.flags & ASTCENC_FLG_USE_LZ_RDO))
		{
			printf("    LZ-RDO endpoint history:    %u elements\n", config.tune_lz_rdo_endpoint_history_size);
			printf("    LZ-RDO weight history:      %u elements\n", config.tune_lz_rdo_weight_history_size);
			printf("    LZ-RDO num passes:          %u\n", config.tune_lz_rdo_num_passes);
		}

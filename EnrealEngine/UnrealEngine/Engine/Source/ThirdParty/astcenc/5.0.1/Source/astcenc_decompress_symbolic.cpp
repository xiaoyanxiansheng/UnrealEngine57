 ...
 */
static inline vint4 lerp_color_int(
	vmask4 u8_mask,
	vint4 color0,
	vint4 color1,
	vint4 weights
) {
	// This is bitwise identical to the spec formula
	//   (color0 * (64 - weights) + color1 * weights + 32) >> 6,
	// but takes fewer operations.
	vint4 diff = color1 - color0;
	vint4 color = color0 + asr<6>(diff * weights + vint4(32));
...
/* Internal function that implements 1-plane 1-partition difference. */
template<bool enable_weights>
static float compute_symbolic_block_difference_1plane_1partition_impl(
	const astcenc_config& config,
	const block_size_descriptor& bsd,
	const symbolic_compressed_block& scb,
	const image_block& blk,
	...
	vint4 epdiff = ep1 - ep0;

	unsigned int texel_count = bsd.texel_count;
	for (unsigned int i = 0; i < texel_count; i += ASTCENC_SIMD_WIDTH)
	{
		// This is bitwise identical to the spec formula
		//   (ep0 * (64 - weight1) + ep1 * weight1 + 32) >> 6
		// but takes fewer operations.
		vint weight1 = vint::loada(plane1_weights + i);
		vint colori_r = vint(ep0.lane<0>()) + asr<6>(vint(epdiff.lane<0>()) * weight1 + vint(32));
		vint colori_g = vint(ep0.lane<1>()) + asr<6>(vint(epdiff.lane<1>()) * weight1 + vint(32));
		vint colori_b = vint(ep0.lane<2>()) + asr<6>(vint(epdiff.lane<2>()) * weight1 + vint(32));
		vint colori_a = vint(ep0.lane<3>()) + asr<6>(vint(epdiff.lane<3>()) * weight1 + vint(32));
		...
		if (enable_weights)
		{
			metric = metric * vfloat(weights + i);
		}
...
float compute_symbolic_block_difference_1plane_1partition(
	...
	return compute_symbolic_block_difference_1plane_1partition_impl<false>(
		config,
		bsd,
		scb,
		blk,
...
float compute_symbolic_block_difference_1plane_1partition_weighted(
	...
	return compute_symbolic_block_difference_1plane_1partition_impl<true>(
		config,
		bsd,
		scb,
		blk,

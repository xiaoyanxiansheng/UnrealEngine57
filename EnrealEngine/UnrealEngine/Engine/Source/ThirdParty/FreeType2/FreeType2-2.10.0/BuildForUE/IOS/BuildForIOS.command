...
FREETYPE_IOS_VERSION=FreeType2-2.10.0
...
do
  "${ENGINE_ROOT}/Build/BatchFiles/RunUAT.command" BuildCMakeLib -TargetPlatform=IOS -TargetArchitecture=${ARCH} -TargetLib=FreeType2 -TargetLibVersion=${FREETYPE_IOS_VERSION} -TargetConfigs=Debug+Release -LibOutputPath=lib -CMakeGenerator=Makefile -CMakeAdditionalArguments="${CMAKE_ADDITIONAL_ARGUMENTS}" -MakeTarget=${MAKE_TARGET} -SkipCreateChangelist
done

pushd "${ENGINE_ROOT}/Source/ThirdParty/FreeType2/${FREETYPE_IOS_VERSION}/lib/IOS"
...
lipo -create iossimulator/Debug/libfreetyped.a x86_64/Debug/libfreetyped.a -output Simulator/Debug/libfreetyped.a 
lipo -create iossimulator/Release/libfreetype.a x86_64/Release/libfreetype.a -output Simulator/Release/libfreetype.a 

...
# We use the version of zlib found in the TVOS SDK 
...
PATH_TO_PNG_LIB=${PATH_TO_LIBPNG}/lib/TVOS/Device/libpng.a

TOOLCHAIN_FILE=${ENGINE_ROOT}/Source/ThirdParty/CMake/PlatformScripts/TVOS/TVOS.cmake
FREETYPE_TVOS_VERSION=FreeType2-2.10.0
PATH_TO_FREETYPE=${THIRD_PARTY_DIRECTORY}/FreeType2/${FREETYPE_TVOS_VERSION}
...
do
  "${ENGINE_ROOT}/Build/BatchFiles/RunUAT.command" BuildCMakeLib -TargetPlatform=TVOS -TargetArchitecture=${ARCH} -TargetLib=FreeType2 -TargetLibVersion=${FREETYPE_TVOS_VERSION} -TargetConfigs=Debug+Release -LibOutputPath=lib -CMakeGenerator=Makefile -CMakeAdditionalArguments="${CMAKE_ADDITIONAL_ARGUMENTS}" -MakeTarget=${MAKE_TARGET} -SkipCreateChangelist
done

pushd "${ENGINE_ROOT}/Source/ThirdParty/FreeType2/${FREETYPE_TVOS_VERSION}/lib/TVOS"
...
lipo -create tvossimulator/Debug/libfreetyped.a x86_64/Debug/libfreetyped.a -output Simulator/Debug/libfreetyped.a 
lipo -create tvossimulator/Release/libfreetype.a x86_64/Release/libfreetype.a -output Simulator/Release/libfreetype.a 

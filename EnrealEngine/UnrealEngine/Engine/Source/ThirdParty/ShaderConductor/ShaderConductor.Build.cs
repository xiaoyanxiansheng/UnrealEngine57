...
public class ShaderConductor : ModuleRules
{
	public ShaderConductor(ReadOnlyTargetRules Target) : base(Target)
	{
        ...
        {
			string SCArchDir = Target.Architecture == UnrealArch.Arm64 ? "WinArm64" : "Win64";
			string SCBinariesDir = Path.Combine(Target.UEThirdPartyBinariesDirectory, "ShaderConductor", SCArchDir);
			AddDependency(SCBinariesDir, "dxcompiler.dll");
			AddDependency(SCBinariesDir, "ShaderConductor.dll", bAddPDB: true);
			
			string SCLibPath = Path.Combine(Target.UEThirdPartySourceDirectory, "ShaderConductor", "ShaderConductor", "lib", SCArchDir, "ShaderConductor.lib");

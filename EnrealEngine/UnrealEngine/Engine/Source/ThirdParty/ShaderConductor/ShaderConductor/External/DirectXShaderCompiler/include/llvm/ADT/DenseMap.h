...
public:
  ...
  void resize(size_type NumEntries) { reserve(NumEntries); }
  ...
  iterator find(KeyT Val) {
    if (BucketT *Bucket = doFind(Val))
      return makeIterator(
          Bucket, getBucketsEnd(),
          *this, true);
    return end();
  }
  ...
  /// entry exists.
  const ValueT &at(KeyT Val) const {

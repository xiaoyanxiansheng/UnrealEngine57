...
#define MSL_RAY_QUERY_IS_OP2(op, msl_op) MSL_RAY_QUERY_OP_INNER2(op, .is, msl_op)
	...
	case OpRayQueryTerminateKHR:
		...
		break;
	// UE Change Begin: Support MetalRT
	case OpRayQueryGetIntersectionInstanceShaderBindingTableRecordOffsetKHR:
		if (MSL_RAY_QUERY_IS_CANDIDATE)
			emit_op(ops[0], ops[1],
			        join(raytracing_instance_descriptor_var_name, "[", to_expression(ops[2]),
			             ".get_candidate_instance_id()", "].intersectionFunctionTableOffset"),
			        false);
		else
			emit_op(ops[0], ops[1],
			        join(raytracing_instance_descriptor_var_name, "[", to_expression(ops[2]),
			             ".get_committed_instance_id()", "].intersectionFunctionTableOffset"),
			        false);
		break;
	// UE Change End: Support MetalRT

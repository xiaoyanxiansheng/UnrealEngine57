 ...
 * @param[in] flow A network flow descriptor
 * @return Status code as defined by @ref rmx_status
 * @remark Detaching a network flow means that no more packets that match this
 *         flow will be received. This operation does not affect packets
 *         already received by the network card.
 * @see rmx_input_flow
 */
__export
 ...
 * @note    This call always returns packets in the arrival order.
 * @note    The location of a packet in the receive buffer depends on the
 *          configured input stream options: if no ordering option is enabled
 *          then it will be placed into the circular buffer in the arrival
 *          order, if #RMX_INPUT_STREAM_RTP_SEQN_PLACEMENT_ORDER or
 *          #RMX_INPUT_STREAM_RTP_EXT_SEQN_PLACEMENT_ORDER is enabled then it
 *          will be placed according to its sequence number.
 ...
 *  @note This parameter is valid only if input stream was created with
 *        #RMX_INPUT_STREAM_RTP_SEQN_PLACEMENT_ORDER or with
 *        #RMX_INPUT_STREAM_RTP_EXT_SEQN_PLACEMENT_ORDER input option enabled.
 *  @see rmx_input_completion
 *  @see rmx_input_enable_stream_option
 */
__RMX_INLINE

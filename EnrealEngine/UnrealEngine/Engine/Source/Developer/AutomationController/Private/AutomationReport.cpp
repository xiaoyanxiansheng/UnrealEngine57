	...
	, TestInfo( InTestInfo )
{
	...
	// Get exclude test info from config for every supported platform. We want to avoid querying when TestInfo is empty
	// (e.g. this is the empty root node in FAutomationControllerManager and we don't actually need to load config yet).
	// This saves ~340ms when opening the editor.
	const FString& TestPath = TestInfo.GetFullTestPath();
	if (!TestPath.IsEmpty())
	{
		if (auto Entry = UAutomationTestExcludelist::Get()->GetExcludeTestEntry(TestPath))
		{
			ExcludeTestInfo = *Entry;
			ExcludeTestInfo.SetPropagation(TestPath);
			bNeedToSkip = true;
		}
	}
}
...
{
	return FAutomationTestFramework::Get().GetTagsForAutomationTest(this->GetFullTestPath());
}

TArray<FString> FAutomationReport::GetTagsArray() const
{
	return FAutomationTestFramework::Get().GetTagsForAutomationTestAsArray(this->GetFullTestPath());
}

...
namespace coff
{
	ObjFile* OpenObj(const wchar_t* filename)
	{
		...
		filename = Filesystem::Devirtualize(filename, temp);
		// END EPIC MOD

		ObjFile* objFile = LC_NEW(&g_objFileAllocator, ObjFile);
		objFile->filename = string::ToUtf8String(filename);
		objFile->memoryFile = Filesystem::OpenMemoryMappedFile(filename, Filesystem::OpenMode::READ, true);
	...
	static RawCoffType* ReadRaw(ObjFile* file, uint32_t uniqueId)
	{
		...
		rawCoff->size = Filesystem::GetMemoryMappedSize(file->memoryFile);

	...
	FLumenCardBuildDebugData& DebugData)
{
	...
	for (int32 RayIndex = 0; RayIndex < RayDirectionsOverHemisphere.Num(); ++RayIndex)
	{
		...
		const FMatrix44f SurfaceBasis = GetTangentBasisFrisvad(SurfelSample.Normal);
...
int64 FMeshUtilities::MemoryForGenerateMeshCardRepresentation(uint64_t IndexCount)
{
	// 1.5 was a derived from insights + LLM captures across a few different projects.
	return 1.5f * MeshRepresentation::MemoryEstimateForEmbreeScene(IndexCount); 
}

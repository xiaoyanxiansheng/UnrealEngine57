...
static TArray<LibIMobileDevice> GetLocalMacDevices()
{
	TArray<LibIMobileDevice> ToReturn;
	...
	ToAdd.DeviceInterface = DeviceConnectionInterface::NoValue;
...
private:
	...
	void QueryDevices()
	{
		...
		if (bEnableSimulatorSupport)
		{
			...
			HasOtherDevices |= SimulatorDevices.Num() > 0;
		}

		FString LibimobileDeviceId = GetLibImobileDeviceExe("idevice_id");
		if (LibimobileDeviceId.Len() == 0)
		{
			UE_LOG(LogIOSDeviceHelper, Warning, TEXT("idevice_id (iOS device detection) executable missing. Turning off iOS/tvOS device detection."));
			HasDevices = false;
			Enable(false);
			if (!HasOtherDevices)
			{
		...
		if (HasDevices)
		{
			ParsedDevices.Append(GetLibIMobileDevices());
		}

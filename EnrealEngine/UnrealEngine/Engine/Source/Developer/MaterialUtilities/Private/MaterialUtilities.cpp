...
#include UE_INLINE_GENERATED_CPP_BY_NAME(MaterialUtilities)
...
public:
		...
		, PropertyToCompile(InPropertyToCompile)
	{
		...
		FMaterialResource* Resource = InMaterialInterface->GetMaterialResource(GMaxRHIShaderPlatform);
...

		{
			...
			GetDependentShaderAndVFTypes(ResourceId.LayoutParams, ShaderTypes, ShaderPipelineTypes, VFTypes);
...
FIntPoint FMaterialUtilities::FindMaxTextureSize(UMaterialInterface* InMaterialInterface, FIntPoint MinimumSize)
{
	...
	InMaterialInterface->GetUsedTextures(MaterialTextures, GetCurrentMaterialQualityLevelChecked(), GetMaxShaderPlatformChecked());
...
bool FMaterialUtilities::ExportBaseColor(ULandscapeComponent* LandscapeComponent, int32 TextureSize, TArray<FColor>& OutSamples)
{
	...
	FIntPoint ComponentOrigin = LandscapeComponent->GetSectionBase() - LandscapeProxy->GetSectionBase();
...
void FMaterialUtilities::FExportErrorManager::Register(const UMaterialInterface* Material, FName TextureName, int32 RegisterIndex, EErrorType ErrorType)
{
	...
	FError Error;
	Error.Material = Material->GetMaterialResource(ShaderPlatform);
...
{
	return ExportMaterialUVDensities(InMaterial, QualityLevel, GetFeatureLevelShaderPlatform_Checked(FeatureLevel), OutErrors);
}

bool FMaterialUtilities::ExportMaterialUVDensities(UMaterialInterface* InMaterial, EMaterialQualityLevel::Type QualityLevel, EShaderPlatform InShaderPlatform, FExportErrorManager& OutErrors)
{
	check(InMaterial);
	...
	InMaterial->GetUsedTexturesAndIndices(Textures, Indices, QualityLevel, InShaderPlatform);

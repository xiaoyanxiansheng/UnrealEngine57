...
#define UE_API MATERIALUTILITIES_API
...
struct FExportMaterialProxyCache
{
	...
	UE_API FExportMaterialProxyCache();
	UE_API ~FExportMaterialProxyCache();
...
public:
	...
	*/
	static UE_API FIntPoint FindMaxTextureSize(UMaterialInterface* InMaterialInterface, FIntPoint MinimumSize = FIntPoint(1, 1));
	...
	static UE_API bool SupportsExport(EBlendMode InBlendMode, EMaterialProperty InMaterialProperty);
	static UE_API bool SupportsExport(bool bIsOpaque, EMaterialProperty InMaterialProperty);
	 ...
	 */
	static UE_API bool ExportLandscapeMaterial(const ALandscapeProxy* InLandscape, FFlattenMaterial& OutFlattenMaterial);
	 ...
	 * @return						Whether operation was successful
	 */
	static UE_API bool ExportLandscapeMaterial(const ALandscapeProxy* InLandscape, const TSet<FPrimitiveComponentId>& HiddenPrimitives, FFlattenMaterial& OutFlattenMaterial);
	 ...
	 */
	static UE_API UTexture2D* CreateTexture(UPackage* Outer, const FString& AssetLongName, FIntPoint Size, const TArray<FColor>& Samples, TextureCompressionSettings CompressionSettings, TextureGroup LODGroup, EObjectFlags Flags, bool bSRGB, const FGuid& SourceGuidHash = FGuid());
	 ...
	 * @return						The new texture.
	 */
	static UE_API UTexture2D* CreateTexture(UPackage* Outer, const FString& AssetLongName, FIntPoint Size, const TArray<FColor>& Samples, const FCreateTexture2DParameters& CreateParams, EObjectFlags Flags);
	 ...
	 */
	static UE_API UMaterial* CreateMaterial(const FFlattenMaterial& InFlattenMaterial, UPackage* InOuter, const FString& BaseName, EObjectFlags Flags, const struct FMaterialProxySettings& MaterialProxySettings, TArray<UObject*>& OutGeneratedAssets, const TextureGroup& InTextureGroup = TEXTUREGROUP_World);
	...
	*/
	...
	static UE_API UMaterialInstanceConstant* CreateInstancedMaterial(UMaterialInterface* BaseMaterial, UPackage* InOuter, const FString& BaseName, EObjectFlags Flags);
	...
	* @return						Whether operation was successful
	*/
	static UE_API bool ExportBaseColor(ULandscapeComponent* LandscapeComponent, int32 TextureSize, TArray<FColor>& OutSamples);
	...
	*/
	static UE_API FFlattenMaterial CreateFlattenMaterialWithSettings(const FMaterialProxySettings& InMaterialLODSettings);
	...
	*/
	static UE_API void AnalyzeMaterial(class UMaterialInterface* InMaterial, const struct FMaterialProxySettings& InMaterialSettings, int32& OutNumTexCoords, bool& OutRequiresVertexData);

	static UE_API void AnalyzeMaterial(class UMaterialInterface* InMaterial, const TArray<EMaterialProperty>& Properties, int32& OutNumTexCoords, bool& OutRequiresVertexData);
	...
	*/
	static UE_API void OptimizeFlattenMaterial(FFlattenMaterial& InFlattenMaterial);
	...
	* @param InFlattenMaterial				Flatten material to optimize
	*/
	static UE_API void ResizeFlattenMaterial(FFlattenMaterial& InFlattenMaterial, const struct FMeshProxySettings& InMeshProxySettings);

	/** Tries to optimize the sample array (will set to const value if all samples are equal) */
	static UE_API void OptimizeSampleArray(TArray<FColor>& InSamples, FIntPoint& InSampleSize);
	...
	*/
	...
	public:
		...
		FExportErrorManager(EShaderPlatform InShaderPlatform) : ShaderPlatform(InShaderPlatform) {}
		...
		*/
		UE_API void Register(const UMaterialInterface* Material, FName TextureName, int32 RegisterIndex, EErrorType ErrorType);

		/**
		* Output all errors registered.
		*/
		UE_API void OutputToLog();
	...
	* @return					Whether operation was successful
	*/
	UE_DEPRECATED(5.7, "Please use ExportMaterialUVDensities with EShaderPlatform argument and not ERHIFeatureLevel::Type")
	static UE_API bool ExportMaterialUVDensities(UMaterialInterface* InMaterial, EMaterialQualityLevel::Type QualityLevel, ERHIFeatureLevel::Type FeatureLevel, FExportErrorManager& OutErrors);
	static UE_API bool ExportMaterialUVDensities(UMaterialInterface* InMaterial, EMaterialQualityLevel::Type QualityLevel, EShaderPlatform InShaderPlatform, FExportErrorManager& OutErrors);

	/** Calculates an importance value for the given set of materials according to the texture sampler usage */
	static UE_API void DetermineMaterialImportance(const TArray<UMaterialInterface*>& InMaterials, TArray<float>& OutImportance);
	/** Generates a set of texture boxes according to the given weights and final atlas texture size */
	static UE_API void GeneratedBinnedTextureSquares(const FVector2D DestinationSize, TArray<float>& InTexureWeights, TArray<FBox2D>& OutGeneratedBoxes);

	/** Creates a proxy material and the required texture assets */
	static UE_API UMaterialInterface* CreateProxyMaterialAndTextures(UPackage* OuterPackage, const FString& AssetName, const FBakeOutput& BakeOutput, const FMeshData& MeshData, const FMaterialData& MaterialData, UMaterialOptions* Options);

	/** Creates a proxy material and the required texture assets */
	static UE_API UMaterialInterface* CreateProxyMaterialAndTextures(const FString& PackageName, const FString& AssetName, const FBakeOutput& BakeOutput, const FMeshData& MeshData, const FMaterialData& MaterialData, UMaterialOptions* Options);

	/** Creates a flatten material instance and the required texture assets */
	static UE_API UMaterialInstanceConstant* CreateFlattenMaterialInstance(UPackage* InOuter, const FMaterialProxySettings& InMaterialProxySettings, UMaterialInterface* InBaseMaterial, const FFlattenMaterial& FlattenMaterial, const FString& AssetBasePath, const FString& AssetBaseName, TArray<UObject*>& OutAssetsToSync, FMaterialUpdateContext* MaterialUpdateContext = nullptr);
	...
	*/
	static UE_API float ComputeRequiredTexelDensityFromScreenSize(const float InScreenSize, float InWorldSpaceRadius);
	...
	* @return					The required texel density per METER
	*/
	static UE_API float ComputeRequiredTexelDensityFromDrawDistance(const float InDrawDistance, float InWorldSpaceRadius);
	...
	*/
	static UE_API int32 GetTextureSizeFromTargetTexelDensity(const FMeshDescription& InMesh, float InTargetTexelDensity);
	...
	* @return The texture size needed to achieve the required texel density.
	*/
	static UE_API int32 GetTextureSizeFromTargetTexelDensity(const UE::Geometry::FDynamicMesh3& Mesh, float TargetTexelDensity);
	...
	* @return The texture size needed to achieve the required texel density.
	*/
	static UE_API int32 GetTextureSizeFromTargetTexelDensity(double InMesh3DArea, double InMeshUVArea, double InTargetTexelDensity);

	/** Validate that the provided material has all the required parameters needed to be considered a flattening material */
	static UE_API bool IsValidFlattenMaterial(const UMaterialInterface* InBaseMaterial);

	/** Get the name of the Texture parameter associated with a given flatten material property */
	static UE_API FString GetFlattenMaterialTextureName(EFlattenMaterialProperties InProperty, UMaterialInterface* InBaseMaterial);
...
private:
	...
	*/
	static UE_API UTextureRenderTarget2D* CreateRenderTarget(bool bInForceLinearGamma, bool bNormalMap, EPixelFormat InPixelFormat, FIntPoint& InTargetSize );
	
	/** Clears the pool with available render targets */
	static UE_API void ClearRenderTargetPool();	

	/** Call back for garbage collector, cleans up the RenderTargetPool if CurrentlyRendering is set to false */
	UE_API void OnPreGarbageCollect();
private:
	/** Flag to indicate whether or not a texture is currently being rendered out */
	static UE_API bool CurrentlyRendering;
	/** Pool of available render targets, cached for re-using on consecutive property rendering */
	static UE_API TArray<UTextureRenderTarget2D*> RenderTargetPool;
};

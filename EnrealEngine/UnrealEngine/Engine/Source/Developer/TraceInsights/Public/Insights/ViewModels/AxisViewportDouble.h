...
public:
	...
	UE_API bool SetSize(const float InSize);
	...
	UE_API bool SetScale(const double NewScale);
	UE_API bool ZoomWithFixedOffset(const double NewScale, const float Offset);
	UE_API bool RelativeZoomWithFixedOffset(const float Delta, const float Offset);

	UE_API void GetScrollLimits(float& OutMinPos, float& OutMaxPos);
	UE_API bool EnforceScrollLimits(const float InMinPos, const float InMaxPos, const float InterpolationFactor);
	UE_API bool UpdatePosWithinLimits();

	UE_API void OnUserScrolled(TSharedPtr<SScrollBar> ScrollBar, float ScrollOffset);

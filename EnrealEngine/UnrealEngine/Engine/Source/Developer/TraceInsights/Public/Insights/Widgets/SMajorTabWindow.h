...
public:
	/** Default constructor. */
	UE_API SMajorTabWindow(const FName& InMajorTabId);

	/** Virtual destructor. */
	UE_API virtual ~SMajorTabWindow();
	...
	UE_API void Construct(const FArguments& InArgs, const TSharedRef<SDockTab>& ConstructUnderMajorTab, const TSharedPtr<SWindow>& ConstructUnderWindow);

	UE_API void ShowTab(const FName& TabId);
	UE_API void HideTab(const FName& TabId);
...
protected:
	UE_API virtual const TCHAR* GetAnalyticsEventName() const;
	UE_API virtual TSharedRef<FWorkspaceItem> CreateWorkspaceMenuGroup();
	UE_API virtual void RegisterTabSpawners();
	UE_API virtual TSharedRef<FTabManager::FLayout> CreateDefaultTabLayout() const;
	UE_API virtual TSharedRef<SWidget> CreateToolbar(TSharedPtr<FExtender> Extender);

	void SetCommandList(const TSharedPtr<FUICommandList> InCommandList) { CommandList = InCommandList; }

	UE_API void AddOpenTab(const TSharedRef<SDockTab>& DockTab);
	UE_API void RemoveOpenTab(const TSharedRef<SDockTab>& DockTab);
...
private:
	 ...
	 */
	static UE_API void FillMenu(FMenuBuilder& MenuBuilder, const TSharedPtr<FTabManager> TabManager);

	/** Returns true if the current analysis session is valid. */
	UE_API bool IsValidSession() const;

	/** Updates the amount of time the window has been active. */
	UE_API EActiveTimerReturnType UpdateActiveDuration(double InCurrentTime, float InDeltaTime);

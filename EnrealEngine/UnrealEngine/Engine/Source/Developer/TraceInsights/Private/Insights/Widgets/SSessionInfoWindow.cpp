...
TSharedRef<SDockTab> SSessionInfoWindow::SpawnTab_SessionInfo(const FSpawnTabArgs& Args)
{
	...
	AddInfoLine(VerticalBox, LOCTEXT("Uri_HeaderText",				"URI"),					&SSessionInfoWindow::GetUriText,			&SSessionInfoWindow::IsVisibleUriText);
	...
	AddInfoLine(VerticalBox, LOCTEXT("StreamType_HeaderText",		"Stream Type"),			&SSessionInfoWindow::GetStreamTypeText,		&SSessionInfoWindow::IsAlwaysVisible);
...
void SSessionInfoWindow::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	...
	if (Session != AnalysisSession)
	{
		...
		ETraceStreamType TraceStreamType = FInsightsManager::Get()->GetTraceStreamType();

		switch (TraceStreamType)
		{
			case ETraceStreamType::TraceFile:
				StreamTypeText = LOCTEXT("TraceStreamType_TraceFile", "Trace File");
				break;

			case ETraceStreamType::TraceStore:
				StreamTypeText = FText::Format(LOCTEXT("TraceStreamType_TraceStore_Fmt", "Trace Store {0} -- TraceId={1}"),
					FText::FromString(FString::Printf(TEXT("%s:%u"), *FInsightsManager::Get()->GetLastStoreHost(), FInsightsManager::Get()->GetLastStorePort())),
					FText::FromString(FString::Printf(TEXT("0x%X"), Session->GetTraceId())));
				break;

			case ETraceStreamType::DirectTrace:
				StreamTypeText = FText::Format(LOCTEXT("TraceStreamType_DirectTrace_Fmt", "Direct Trace -- Port={0}"),
					FText::AsNumber(uint32(FInsightsManager::Get()->GetDirectTracePort()), &FNumberFormattingOptions::DefaultNoGrouping()));
				break;

			case ETraceStreamType::Custom:
				StreamTypeText = LOCTEXT("TraceStreamType_Custom", "Custom Stream");
				break;

			default:
				StreamTypeText = LOCTEXT("TraceStreamType_Unknown", "Unknown");
				break;
		}
		...
		// We can quickly get the session name and uri.
		{
			...
			if (TraceStreamType == ETraceStreamType::TraceFile ||
				TraceStreamType == ETraceStreamType::TraceStore)
			{
			...
			{
				SessionNameText = FText::FromString(Session->GetName());
			}
		}

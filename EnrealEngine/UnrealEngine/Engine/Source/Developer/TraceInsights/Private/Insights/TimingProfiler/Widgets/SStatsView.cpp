...
public:
	...
	virtual void RegisterCommands() override
	{
		UI_COMMAND(Command_GenerateNewColor,
			"Generate New Color",
			"Generates a new random color for the selected counters.",
			EUserInterfaceActionType::Button,
			FInputChord());

		UI_COMMAND(Command_EditColor,
			"Edit Color...",
			"Changes color for the selected counters.",
			EUserInterfaceActionType::Button,
			FInputChord());
...
void SStatsView::InitCommandList()
{
	...
	CommandList->MapAction(FStatsViewCommands::Get().Command_GenerateNewColor, FExecuteAction::CreateSP(this, &SStatsView::ContextMenu_GenerateNewColor_Execute), FCanExecuteAction::CreateSP(this, &SStatsView::ContextMenu_GenerateNewColor_CanExecute));
	CommandList->MapAction(FStatsViewCommands::Get().Command_EditColor, FExecuteAction::CreateSP(this, &SStatsView::ContextMenu_EditColor_Execute), FCanExecuteAction::CreateSP(this, &SStatsView::ContextMenu_EditColor_CanExecute));
...
TSharedPtr<SWidget> SStatsView::TreeView_GetMenuContent()
{
	...
	MenuBuilder.BeginSection("CounterOptions", LOCTEXT("ContextMenu_Section_CounterOptions", "Counter Options"));
	{
		...
		MenuBuilder.AddMenuEntry
		(
			FStatsViewCommands::Get().Command_GenerateNewColor,
			NAME_None,
			TAttribute<FText>(),
			TAttribute<FText>(),
		...
		MenuBuilder.AddMenuEntry
		(
			FStatsViewCommands::Get().Command_EditColor,
			NAME_None,
			TAttribute<FText>(),
			TAttribute<FText>(),
...
bool SStatsView::ContextMenu_GenerateNewColor_CanExecute() const
{
	return AreSelectedCounterNodesValid();
}

////////////////////////////////////////////////////////////////////////////////////////////////////

void SStatsView::ContextMenu_GenerateNewColor_Execute() const
{
	const TArray<FStatsNodePtr> SelectedNodes = TreeView->GetSelectedItems();
	for (const FStatsNodePtr& SelectedNode : SelectedNodes)
	{
		if (SelectedNode->Is<FStatsNode>())
		{
			FStatsNode& StatsNode = SelectedNode->As<FStatsNode>();
			SetRandomColorToNode(StatsNode);
		}
	}
};
...
void SStatsView::SetColorToNode(FStatsNode& StatsNode, FLinearColor CustomColor) const
{
	StatsNode.SetColor(CustomColor);

	const uint32 CounterId = StatsNode.GetCounterId();
	const FLinearColor BorderColor(CustomColor.R + 0.4f, CustomColor.G + 0.4f, CustomColor.B + 0.4f, 1.0f);

	TSharedPtr<FTimingGraphSeries> CounterSeries = GetTimingViewMainGraphTrack()->GetStatsCounterSeries(CounterId);
	if (CounterSeries.IsValid())
	{
		CounterSeries->SetColor(CustomColor, BorderColor);
	}
}

////////////////////////////////////////////////////////////////////////////////////////////////////

void SStatsView::SetRandomColorToNode(FStatsNode& StatsNode) const
{
...
bool SStatsView::ContextMenu_EditColor_CanExecute() const
{
	return AreSelectedCounterNodesValid();
};
...
void SStatsView::ContextMenu_EditColor_Execute()
{
	...
	for (const FStatsNodePtr& SelectedNode : SelectedNodes)
	{
		...
		{
			FStatsNode& StatsNode = SelectedNode->As<FStatsNode>();
			EditableColorValue = StatsNode.GetColor();
			break;
		}
	}
	...
	FColorPickerArgs PickerArgs;
	{
		...
		PickerArgs.OnColorCommitted = FOnLinearColorValueChanged::CreateSP(this, &SStatsView::SetEditableColor);
		PickerArgs.OnColorPickerCancelled = FOnColorPickerCancelled::CreateSP(this, &SStatsView::ColorPickerCancelled);
...
FLinearColor SStatsView::GetEditableColor() const
{
...
void SStatsView::SetEditableColor(FLinearColor NewColor)
{
	...
	for (const FStatsNodePtr& SelectedNode : SelectedNodes)
	{
		...
		{
			FStatsNode& StatsNode = SelectedNode->As<FStatsNode>();
			SetColorToNode(StatsNode, EditableColorValue);
		}
	}
}
...
void SStatsView::ColorPickerCancelled(FLinearColor OriginalColor)
{
...
bool SStatsView::AreSelectedCounterNodesValid() const
{
	const TArray<FStatsNodePtr> SelectedNodes = TreeView->GetSelectedItems();
	for (const FStatsNodePtr& SelectedNode : SelectedNodes)
	{

...
STimingView::~STimingView()
{
	...
	SelectedEvent.Reset();
	CustomFilterConfigurator.Reset();
...
void STimingView::Construct(const FArguments& InArgs, FName InViewName)
{
	...
	for (Timing::ITimingViewExtender* Extender : GetExtenders())
	{
		Extender->ExtendMenuToolbars(*this, LeftToolbar, RightToolbar);
	}
...
TSharedPtr<FFilterConfigurator> STimingView::MakeCustomFilterConfigurator(uint32 TimerId, double IntervalStart, double IntervalEnd)
{
	if (CustomFilterConfigurator.IsValid())
	{
		CustomFilterConfigurator.Reset();
	}

	CustomFilterConfigurator = MakeShared<FFilterConfigurator>();

	CustomFilterConfigurator->Add(MakeShared<FFilter>(
	...
	FFilterConfiguratorNodePtr StartTimeNode = MakeShared<FFilterConfiguratorNode>(TEXT(""), false);
	StartTimeNode->SetSelectedFilter(CustomFilterConfigurator->GetRootNode()->GetAvailableFilters()->GetData()[0]);
	StartTimeNode->SetTextBoxValue(FString::SanitizeFloat(IntervalStart));
	StartTimeNode->SetSelectedFilterOperator(MakeShared<FFilterOperator<double>>(EFilterOperator::Gte, TEXT(""), std::greater_equal<>()));

	FFilterConfiguratorNodePtr EndTimeNode = MakeShared<FFilterConfiguratorNode>(TEXT(""), false);
	EndTimeNode->SetSelectedFilter(CustomFilterConfigurator->GetRootNode()->GetAvailableFilters()->GetData()[1]);
	EndTimeNode->SetTextBoxValue(FString::SanitizeFloat(IntervalEnd));

	EndTimeNode->SetSelectedFilterOperator(MakeShared<FFilterOperator<double>>(EFilterOperator::Lte, TEXT(""), std::less_equal<>()));
	FFilterConfiguratorNodePtr TimerIdNode = MakeShared<FFilterConfiguratorNode>(TEXT(""), false);
	TimerIdNode->SetSelectedFilter(CustomFilterConfigurator->GetRootNode()->GetAvailableFilters()->GetData()[2]);
	TimerIdNode->SetTextBoxValue(FString::FromInt(TimerId));

	CustomFilterConfigurator->GetRootNode()->AddChildAndSetParent(StartTimeNode);
	CustomFilterConfigurator->GetRootNode()->AddChildAndSetParent(EndTimeNode);
	CustomFilterConfigurator->GetRootNode()->AddChildAndSetParent(TimerIdNode);
	CustomFilterConfigurator->GetRootNode()->ProcessFilter();

	return CustomFilterConfigurator;
}
...
void STimingView::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	...
	Timing::FTimingViewExtenderTickParams TickParams(*this, Session.Get());
	TickParams.DeltaTime = InDeltaTime;
	TickParams.CurrentTime = InCurrentTime;
...
void STimingView::ShowContextMenu(const FPointerEvent& MouseEvent)
{
	...
	MenuBuilder.BeginSection(TEXT("Misc"));
	{
		...
		if (HoveredEvent)
		{
			...
			MenuBuilder.AddMenuEntry(
				LOCTEXT("ContextMenu_Copy", "Copy Event To Clipboard"),
				FText(),
...
void STimingView::QuickFind_Execute()
{
	...
	if (!QuickFindVm.IsValid())
	{
		...
		TSharedRef<FFilterWithSuggestions> RegionNameFilter = MakeShared<FFilterWithSuggestions>(
			static_cast<int32>(EFilterField::RegionName),
			LOCTEXT("RegionName", "Region Name"),
			LOCTEXT("RegionName", "Region Name"),
			EFilterDataType::String,
			nullptr,
			FFilterService::Get()->GetStringOperators());
		RegionNameFilter->SetCallback([this](const FString& Text, TArray<FString>& OutSuggestions)
		{
			this->PopulateRegionNameSuggestionList(Text, OutSuggestions);
		});
		NewFilterConfigurator->Add(RegionNameFilter);

		TSharedRef<FFilterWithSuggestions> RegionCategoryFilter = MakeShared<FFilterWithSuggestions>(
			static_cast<int32>(EFilterField::RegionCategory),
			LOCTEXT("RegionCategory", "Region Category"),
			LOCTEXT("RegionCategory", "Region Category"),
			EFilterDataType::String,
			nullptr,
			FFilterService::Get()->GetStringOperators());
		RegionCategoryFilter->SetCallback([this](const FString& Text, TArray<FString>& OutSuggestions)
		{
			this->PopulateRegionCategorySuggestionList(Text, OutSuggestions);
		});
		NewFilterConfigurator->Add(RegionCategoryFilter);
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
		...
		constexpr bool bUseCustomFilter = false;
		QuickFindVm->GetOnFindFirstEvent().AddSP(this, &STimingView::FindFirstEvent, bUseCustomFilter);
		QuickFindVm->GetOnFindPreviousEvent().AddSP(this, &STimingView::FindPrevEvent, bUseCustomFilter);
		QuickFindVm->GetOnFindNextEvent().AddSP(this, &STimingView::FindNextEvent, bUseCustomFilter);
		QuickFindVm->GetOnFindLastEvent().AddSP(this, &STimingView::FindLastEvent, bUseCustomFilter);
		QuickFindVm->GetOnFindMaxEvent().AddSP(this, &STimingView::FindMaxEvent);
		QuickFindVm->GetOnFindMinEvent().AddSP(this, &STimingView::FindMinEvent);
...
void STimingView::FindFirstEvent(bool bUseCustomFilter)
{
	if (SelectedEvent.IsValid())
	{
		SelectedEvent.Reset();
	...
	FindNextEvent(bUseCustomFilter);
}

////////////////////////////////////////////////////////////////////////////////////////////////////

void STimingView::FindPrevEvent(bool bUseCustomFilter)
{
	...
	TSharedPtr<FFilterConfigurator> Filter = bUseCustomFilter ? CustomFilterConfigurator : QuickFindVm->GetFilterConfigurator();
	...
	if (!PriorityTrack.IsValid() && Filter.IsValid())
	{
		PriorityTrack = FindPriorityTrack(*Filter->GetRootNode());
	}

	EnumerateFilteredTracks(Filter, PriorityTrack,
		[&Params, &BestMatchEvent]
		(TSharedPtr<const FBaseTimingTrack> Track)
		{
	...
	PostFindEvent(BestMatchEvent);
}

////////////////////////////////////////////////////////////////////////////////////////////////////

void STimingView::FindNextEvent(bool bUseCustomFilter)
{
	TSharedPtr<const ITimingEvent> BestMatchEvent;
...
void STimingView::FindLastEvent(bool bUseCustomFilter)
{
	if (SelectedEvent.IsValid())
	{
		SelectedEvent.Reset();
	...
	FindPrevEvent(bUseCustomFilter);
}

////////////////////////////////////////////////////////////////////////////////////////////////////

void STimingView::FindMaxEvent()
{
	if (SelectedEvent.IsValid())
	{
		SelectedEvent.Reset();
	...
	TSharedPtr<FFilterConfigurator> Filter = QuickFindVm->GetFilterConfigurator();

	auto EventFilter = [&CurrentDuration](double EventStartTime, double EventEndTime, uint32 EventDepth)
	{
		const double Duration = EventEndTime - EventStartTime;
		return Duration > CurrentDuration;
	};

	auto EventMatched = [&CurrentDuration](double EventStartTime, double EventEndTime, uint32 EventDepth)
	{
		const double Duration = EventEndTime - EventStartTime;
		CurrentDuration = Duration;
	};

	FTimingEventSearchParameters Params(0.0, std::numeric_limits<double>::max(), ETimingEventSearchFlags::SearchAll, EventFilter, EventMatched);
	Params.FilterExecutor = Filter;
	...
	EnumerateFilteredTracks(Filter, PriorityTrack,
		...
		(TSharedPtr<const FBaseTimingTrack> Track)
		{
			...
			if (FoundEvent.IsValid())
			{
				if (!BestMatchEvent.IsValid() || BestMatchEvent->GetDuration() < FoundEvent->GetDuration())
				{
					BestMatchEvent = FoundEvent;
				}
			}
...
void STimingView::FindMinEvent()
{
	...
	TSharedPtr<const ITimingEvent> BestMatchEvent;
	double CurrentDuration = std::numeric_limits<double>::max();
...
void STimingView::PostFindEvent(TSharedPtr<const ITimingEvent> BestMatchEvent)
{
	if (BestMatchEvent)
	{
...
TSharedPtr<const FBaseTimingTrack> STimingView::FindPriorityTrack(FFilterConfiguratorNode& Node)
{
	for (FBaseTreeNodePtr Child : Node.GetChildren())
	{
		check(Child.IsValid() && Child->Is<FFilterConfiguratorNode>());
		FFilterConfiguratorNode& ChildNode = Child->As<FFilterConfiguratorNode>();
		TSharedPtr<FFilter> Filter = ChildNode.GetSelectedFilter();
		if (Filter.IsValid())
		{
			if (Filter->GetKey() == static_cast<int32>(EFilterField::RegionName) ||
				Filter->GetKey() == static_cast<int32>(EFilterField::RegionCategory))
			{
				for (auto& Entry : AllTracks)
				{
					if (Entry.Value->IsVisible() &&
						Entry.Value->Is<FTimingRegionsTrack>())
					{
						return Entry.Value;
					}
				}
		...
		TSharedPtr<const FBaseTimingTrack> Result = FindPriorityTrack(ChildNode);
		if (Result.IsValid())
		{
			return Result;
		}
...
void STimingView::PopulateRegionNameSuggestionList(const FString& Text, TArray<FString>& OutSuggestions)
{
	TSharedPtr<const TraceServices::IAnalysisSession> Session = FInsightsManager::Get()->GetSession();
	if (Session.IsValid())
	{
		...
		RegionProvider.EnumerateTimers(
			[&Text, &OutSuggestions]
			(const TraceServices::FRegionTimer& Timer) -> bool
			{
				if (Timer.Name && (FCString::Stristr(Timer.Name, *Text) || Text.IsEmpty()))
				{
					OutSuggestions.Add(Timer.Name);
				}
				return true;
...
void STimingView::PopulateRegionCategorySuggestionList(const FString& Text, TArray<FString>& OutSuggestions)
{
	TSharedPtr<const TraceServices::IAnalysisSession> Session = FInsightsManager::Get()->GetSession();
	if (Session.IsValid())
	{
		...
		RegionProvider.EnumerateCategories(
			[&Text, &OutSuggestions]
			(const TraceServices::FRegionCategory& Category) -> bool
			{
				if (Category.Name && (FCString::Stristr(Category.Name, *Text) || Text.IsEmpty()))
				{
					OutSuggestions.Add(Category.Name);
				}
				return true;
			});
	}
...
void STimingView::EnumerateFilteredTracks(TSharedPtr<UE::Insights::FFilterConfigurator> InFilterConfigurator, TSharedPtr<const FBaseTimingTrack> PriorityTrack, EnumerateFilteredTracksCallback Callback)
{
	...
	for (auto& Entry : AllTracks)
	{
		if (Entry.Value->GetId() != SkipId && Entry.Value->IsVisible())
		{
...
void STimingView::SelectEventInstance(uint32 TimerId, ESelectEventType Type, bool bUseSelection)
{
	...
	double IntervalEnd = std::numeric_limits<double>::max();
	...
	// Create a temporary filter object using the TimerId and search range as filter criteria
	MakeCustomFilterConfigurator(TimerId, IntervalStart, IntervalEnd);

	switch (Type)
	{
	case ESelectEventType::First:
		FindFirstEvent(true);
		TimingEvent = SelectedEvent;
		break;
	case ESelectEventType::Previous:
		FindPrevEvent(true);
		TimingEvent = SelectedEvent;
		break;
	case ESelectEventType::Next:
		FindNextEvent(true);
		TimingEvent = SelectedEvent;
		break;
	case ESelectEventType::Last:
		FindLastEvent(true);
		TimingEvent = SelectedEvent;
		break;
	case ESelectEventType::Min:

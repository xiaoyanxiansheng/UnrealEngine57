...
void FMemoryTagList::UpdateInternal()
{
	...
	uint32 AddedOrUpdatedTags = 0;

	MemoryProvider->EnumerateTags([this, &AddedOrUpdatedTags](const TraceServices::FMemoryTagInfo& TraceTag)
	{
		...
		while (TrackerFlags != 0)
		{
			if (TrackerFlags & 1)
			{
				...
				if (!TagPtr)
				{
					...
					Tag.TrackerId = static_cast<FMemoryTrackerId>(TrackerId);
					Tag.TagSetId = static_cast<FMemoryTagSetId>(TraceTag.TagSetId);
					Tag.SetColorAuto();

...
TSharedPtr<SToolTip> STimersViewTooltip::GetRowTooltip(const TSharedPtr<FTimerNode> TimerNodePtr)
{
	...
	if (true)
	{
		static constexpr TCHAR FormatString[] = TEXT("%.*fs (%s)");
		const FText TotalInclusiveTimeText = FText::FromString(FString::Printf(FormatString, NumDigits, Stats.TotalInclusiveTime, *FormatTimeAuto(Stats.TotalInclusiveTime, 2)));
		const FText TotalExclusiveTimeText = FText::FromString(FString::Printf(FormatString, NumDigits, Stats.TotalExclusiveTime, *FormatTimeAuto(Stats.TotalExclusiveTime, 2)));

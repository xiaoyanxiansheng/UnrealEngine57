...
public:
	...
	virtual void RegisterCommands() override
	{
		...
		UI_COMMAND(Command_ToggleTimeFormat,
			"Toggle Time Format",
			"Toggles between Relative and Real Time Formats.",
			EUserInterfaceActionType::Button,
			FInputChord());
	...
	TSharedPtr<FUICommandInfo> Command_ToggleTimeFormat;
...
public:
	virtual TSharedRef<SWidget> GenerateWidgetForColumn(const FName& ColumnName) override
	{
		...
		else if (ColumnName == LogViewColumns::SessionTimeColumnName)
		{
			return SNew(SBox)
				.Padding(FMargin(4.0, 0.0))
				[
		...
		else if (ColumnName == LogViewColumns::RealTimeColumnName)
		{
			return SNew(SBox)
				.Padding(FMargin(4.0, 0.0))
				[
	...
	FText GetTime() const
	{
		...
		if (ParentWidgetPin.IsValid() && LogMessagePin.IsValid())
		{
			...
			return ParentWidgetPin->IsUsingRelativeTime() ? CacheEntry.GetTimeAsText() : CacheEntry.GetAbsoluteDateTime(Session->GetBaseDateTime());
		}
		else
		{
			return FText();
...
SLogView::SLogView()
	: Filter(*TimingProfiler::FTimingProfilerManager::Get()->GetLogFilter())
			...
			[
				...
				[
					...
					(
						...
						+ SHeaderRow::Column(LogViewColumns::SessionTimeColumnName)
						.ManualWidth(100.0f)
						.DefaultLabel(GetTimeFormatColumnName())
...
void SLogView::InitCommandList()
{
	...
	CommandList->MapAction(FLogViewCommands::Get().Command_ToggleTimeFormat, FExecuteAction::CreateSP(this, &SLogView::ToggleTimeFormat), FCanExecuteAction::CreateSP(this, &SLogView::CanToggleTimeFormat));
...
TSharedPtr<SWidget> SLogView::ListView_GetContextMenu()
{
	...
	MenuBuilder.BeginSection("LogViewContextMenu");
	{
		...
		MenuBuilder.AddMenuEntry
		(
			FLogViewCommands::Get().Command_ToggleTimeFormat,
			NAME_None,
			bUseRelativeTime ?
			LOCTEXT("SwitchToRealTime", "Switch to Real Time") :
			LOCTEXT("SwitchToSessionTime", "Switch To Session Time"),
			bUseRelativeTime ?
			LOCTEXT("ToggleTimeFormatRealTime_Tooltip", "Changes the Time column to reflect real (UTC) timestamps.") :
			LOCTEXT("ToggleTimeFormatSessionTime_Tooltip", "Changes the Time column to reflect session relative timestamps."),
			FSlateIcon(FInsightsStyle::GetStyleSetName(), "Icons.TimersView")
		);
...
bool SLogView::CanToggleTimeFormat() const
{
	return true;
}
...
void SLogView::ToggleTimeFormat()
{
	bUseRelativeTime = !bUseRelativeTime;
	TSharedPtr<SHeaderRow> Row = ListView->GetHeaderRow();
	
	Row->RemoveColumn(Row->GetColumns()[1].ColumnId);
	Row->InsertColumn(
		SHeaderRow::Column(bUseRelativeTime ? LogViewColumns::SessionTimeColumnName : LogViewColumns::RealTimeColumnName)
		.ManualWidth(bUseRelativeTime ? 100.0f : 155.0f)
		.DefaultLabel(GetTimeFormatColumnName())
		, 1);
}
...
FText SLogView::GetTimeFormatColumnName() const
{
	return bUseRelativeTime ? LOCTEXT("SessionTimeColumn", "Session Time") : LOCTEXT("RealTimeColumn", "Real Time");
}
...
void SLogView::AppendFormatMessageDetailed(const FLogMessageRecord& Log, TStringBuilderBase<TCHAR>& InOutStringBuilder) const
{
	...
	InOutStringBuilder.Append(bUseRelativeTime ? FormatTimeHMS(Log.GetTime(), FTimeValue::Microsecond) : Log.GetAbsoluteDateTime(Session->GetBaseDateTime()).ToString());
	InOutStringBuilder.Append(TEXT("\nVerbosity="));

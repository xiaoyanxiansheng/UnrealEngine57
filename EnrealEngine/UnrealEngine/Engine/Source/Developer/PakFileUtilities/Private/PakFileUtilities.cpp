...
bool ListFilesInPak(const TCHAR * InPakFilename, int64 SizeFilter, bool bIncludeDeleted, const FString& CSVFilename, bool bExtractToMountPoint, const FKeyChain& InKeyChain, bool bAppendFile, bool bIncludeFooter);
...
FArchive* CreatePakWriter(const TCHAR* Filename, const FKeyChain& InKeyChain, bool bSign)
{
	...
	{
		UE_LOG(LogPakFile, Error, TEXT("Failed to create pak at %s. File already exists at that location."), Filename);
		return nullptr;
	}
...
bool ListFilesInPak(const TCHAR * InPakFilename, int64 SizeFilter, bool bIncludeDeleted, const FString& CSVFilename, bool bExtractToMountPoint, const FKeyChain& InKeyChain, bool bAppendFile, bool bIncludeFooter)
{
	...
	if (PakFile.IsValid())
	{
		...
		if (CSVFilename.Len() > 0)
		{
			...
			if (bIncludeFooter)
			{
				// If desired, emit a pseudoentry for our metadata. This is all stored at the end of the file
				// so rather than ask the pak file anything we just assign it to everything past the end of the 
				// records.
				int64 RecordEndOffset = 0;
				if (Records.Num())
				{
					const FPakEntry& LastEntry = Records[Records.Num()-1].Value;
					RecordEndOffset = LastEntry.Offset + LastEntry.Size;
				}
				OutputArchive->Logf(
					TEXT("<Footer>, %lld, %lld, 0, false, false, 0"),
					RecordEndOffset, PakFile.TotalSize() - RecordEndOffset);
			}
...
void RemoveIdenticalFiles( TArray<FPakInputPair>& FilesToPak, const FString& SourceDirectory, const TMap<FString, FFileInfo>& FileHashes, const FPatchSeekOptParams& SeekOptParams, const FString ChangedFilesOutputFilename)
{
	...
	ParallelFor(FilesToPak.Num(), [&FilesToPak, &SourceFileToIndex, &UAssetToUexpMapping](int32 Idx)
		{
			const auto& NewFile = FilesToPak[Idx];
			FString Ext(FPaths::GetExtension(FilesToPak[Idx].Source));
			if (Ext.Equals("uasset", ESearchCase::IgnoreCase) || Ext.Equals("umap", ESearchCase::IgnoreCase))
			{
				...
				if (UexpIndexPtr)
				{
					UAssetToUexpMapping[*UexpIndexPtr] = Idx;
					UAssetToUexpMapping[Idx] = *UexpIndexPtr;
				}
			}
		});
	...
	TArray<FString> ChangedFilesArchiveArray;
	ChangedFilesArchiveArray.SetNum(FilesToPak.Num());
    // Mark files to remove if they're unchanged
	ParallelFor(FilesToPak.Num(), [SourceDirectory, ChangedFilesArchive, &FilesToPak, &FileHashes, &FileSizes, &IncludeFilesMask, &ChangedFilesArchiveArray](int32 Idx)
		{
			const auto& NewFile = FilesToPak[Idx];
			...
			bool bFileIsIdentical = FileIsIdentical(SourceFilename, DestFilename, FoundFileHash, &FileSizes[Idx]);

			if (ChangedFilesArchive && !bFileIsIdentical)
			{
				ChangedFilesArchiveArray[Idx] = DestFilename;
			}
			...
			if (!bForceInclude && bFileIsIdentical)
			{
				...
				IncludeFilesMask[Idx] = false;
			}
		});
	if (ChangedFilesArchive)
	{
		for (const FString& Log : ChangedFilesArchiveArray)
		{
			if (!Log.IsEmpty())
			{
...
#if GUARANTEE_UASSET_AND_UEXP_IN_SAME_PAK
	ParallelFor(FilesToPak.Num(), [&FilesToPak, &UAssetToUexpMapping, &IncludeFilesMask](int32 Idx)
		{
			int32 CounterpartFileIndex = UAssetToUexpMapping[Idx];
			if (CounterpartFileIndex != -1)
			{
				if (IncludeFilesMask[Idx] != IncludeFilesMask[CounterpartFileIndex])
				{
					UE_LOG(LogPakFile, Display, TEXT("One of %s and %s is different from source, so both will be included in patch"), *FilesToPak[Idx].Source, *FilesToPak[CounterpartFileIndex].Source);
					IncludeFilesMask[Idx] = true;
					IncludeFilesMask[CounterpartFileIndex] = true;
				}
			}
		});
...
bool AtLeastOnePakFileExists(const FString& PakFilename)
{
	...
	else
	{
		// If file doesn't exist try using it as a search string, it may contain wild cards
		TArray<FString> PakFileList;
		FString PakFileDirectory = FPaths::GetPath(PakFilename);
		IFileManager::Get().FindFiles(PakFileList, *PakFileDirectory, *FPaths::GetCleanFilename(PakFilename));

		return PakFileList.Num() > 0;
	}
}
...
FString GetPakPath(const TCHAR* SpecifiedPath, bool bIsForCreation)
{
	...
	if (!bIsForCreation && !AtLeastOnePakFileExists(PakFilename))
	{
		PakFilename = FPaths::LaunchDir() + SpecifiedPath;

		if (!AtLeastOnePakFileExists(PakFilename))
		{
...
bool ExecuteUnrealPak(const TCHAR* CmdLine)
{
	...
	if (FParse::Param(CmdLine, TEXT("List")))
	{
		...
		// Handle if we're trying to list an entire directory or anything with wildcards.
		// This gets a little weird because GetPakPath does a bunch of normalization, and this
		// is trying to somewhat match behavior in IostoreUtilities.cpp.
		FString RawFileName = NonOptionArguments[0];
		TArray<FString> FilesToList;
		if (IFileManager::Get().DirectoryExists(*RawFileName))
		{
			// List the whole directory.
			FString Directory = RawFileName;
			FPaths::NormalizeDirectoryName(Directory);

			TArray<FString> FoundPaks;
			IFileManager::Get().FindFiles(FoundPaks, *(Directory / TEXT("*.pak")), true, false);

			for (const FString& Filename : FoundPaks)
			{
				FilesToList.Emplace(Directory / Filename);
			}
		}
		else if (RawFileName.Contains("*"))
		{
			// Technically GetPakPath handles wildcards, but it also only returns a single filename (?),
			// so we now assume if they include a wildcard they want multiple listings.
			FString Directory = FPaths::GetPath(RawFileName);
			FPaths::NormalizeDirectoryName(Directory);

			TArray<FString> FoundPaks;
			IFileManager::Get().FindFiles(FoundPaks, *RawFileName, true, false);
		...
		{
			// Otherwise, use the old system so we don't break anything.
			FilesToList.Emplace(GetPakPath(*NonOptionArguments[0], false));
		}
		
		FString CSVFilename;
		FParse::Value(CmdLine, TEXT("csv="), CSVFilename);

		const bool bOutputCSV = !CSVFilename.IsEmpty();

		// If CSV is a path, then dump multiple CSVs. If it's a single CSV, then append.
		bool bAppendCSV = false;
		if (bOutputCSV)
		{
			if (FilesToList.Num() > 1)
			{
				bAppendCSV = !IFileManager::Get().DirectoryExists(*CSVFilename);
			}
			else
			{
				// If it's a single file, then if it's a directory use the pak filename as a base
				if (IFileManager::Get().DirectoryExists(*CSVFilename))
				{
					CSVFilename = CSVFilename / FPaths::GetCleanFilename(FilesToList[0]) + TEXT(".csv");
				}
			}
		}

		bool bExtractToMountPoint = FParse::Param(CmdLine, TEXT("ExtractToMountPoint"));
		bool bIncludeFooter = FParse::Param(CmdLine, TEXT("IncludeFooters"));

		bool bAllSucceeded = true;
		for (const FString& PakFilename : FilesToList)
		{
			FString ThisPaksCSV = CSVFilename;
			if (bOutputCSV && !bAppendCSV && FilesToList.Num() > 1)
			{
				// It's a directory.
				ThisPaksCSV = CSVFilename / FPaths::GetCleanFilename(PakFilename) + TEXT(".csv");
			}

			if (!ListFilesInPak(*PakFilename, SizeFilter, !bExcludeDeleted, *ThisPaksCSV, bExtractToMountPoint, KeyChain, bAppendCSV, bIncludeFooter))
			{
				bAllSucceeded = false;
			}
		}

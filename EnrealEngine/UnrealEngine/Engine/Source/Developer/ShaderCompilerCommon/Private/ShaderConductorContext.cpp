...
namespace CrossCompiler
{
	...
	void FShaderConductorOptions::ApplyCommonCompilerFlags(const FShaderCompilerFlags& InCompilerFlags)
	{
		// Treat all warnings as errors
		bWarningsAsErrors = InCompilerFlags.Contains(CFLAG_WarningsAsErrors);

		// Include shader source code in the shader debug info. This allows source code correlation in shader profiling tools.
		bEnableDebugInfo = InCompilerFlags.Contains(CFLAG_ForceSpirvDebugInfo);

		// Enable HLSL 2021 if specified
		if (InCompilerFlags.Contains(CFLAG_HLSL2021))
		{
			HlslVersion = 2021;
		}
	}
	...
	{
		FAnsiStringView ErrorString(static_cast<const ANSICHAR*>(ErrorBlob.Data()), ErrorBlob.Size());

		// Forward parsed array of lines to primary conversion function
		FShaderConductorContext::ConvertCompileErrors(ErrorString, OutErrors);
	}
	...
	void FShaderConductorContext::ConvertCompileErrors(FAnsiStringView ErrorsString, TArray<FShaderCompilerError>& OutErrors)
	{
		auto ScanPastDigits = [](const ANSICHAR*& Ptr)
			{
				while (FChar::IsDigit(*(++Ptr))) {}
			};

		// if the null terminator character is included in the string view, remove it before parsing so whitespace trimming works correctly
		if (ErrorsString.EndsWith(0))
		{
			ErrorsString.LeftChopInline(1);
		}

		const ANSICHAR* ErrorBlobAnsiStart = ErrorsString.GetData();
		const ANSICHAR* ErrorBlobAnsiCur = ErrorBlobAnsiStart;
		const ANSICHAR* ErrorBlobAnsiEnd = ErrorBlobAnsiCur + ErrorsString.Len();
		const ANSICHAR* LastLineStart = ErrorBlobAnsiStart;
		const ANSICHAR* CurErrorStart = nullptr;
		int32 ColonCount = 0;

		// since DXC doesn't give us a good delimiter for error messages, we look for a line containing 3 colons with digits in between (which will be the case
		// for a file/line marker, i.e. /My/File/Shader.ush:269:29:) and use the start of that line as the start of a diagnostic message (and the end of the previous)
		while (ErrorBlobAnsiCur < ErrorBlobAnsiEnd)
		{
			if (*ErrorBlobAnsiCur == '\n')
			{
				LastLineStart = ErrorBlobAnsiCur + 1;
			}
			else if (*ErrorBlobAnsiCur == ':')
			{
				ScanPastDigits(ErrorBlobAnsiCur);
				if (*ErrorBlobAnsiCur++ == ':')
				{
					ScanPastDigits(ErrorBlobAnsiCur);
					if (*ErrorBlobAnsiCur++ == ':')
					{
						if (CurErrorStart != nullptr)
						{
							OutErrors.Add(FString(FAnsiStringView(CurErrorStart, LastLineStart - CurErrorStart).TrimStartAndEnd()));
						}
						CurErrorStart = LastLineStart;
					}
				}
			}
			ErrorBlobAnsiCur++;
		}
		// after the loop above ErrorBlobAnsiCur should be pointing to the end of the error blob, and CurErrorStart should point to the
		// start of the last error, so we need to add one more error containing the characters between the two
		if (CurErrorStart != nullptr)
		{
			OutErrors.Add(FString(FAnsiStringView(CurErrorStart, ErrorBlobAnsiEnd - CurErrorStart).TrimStartAndEnd()));
		}
	}

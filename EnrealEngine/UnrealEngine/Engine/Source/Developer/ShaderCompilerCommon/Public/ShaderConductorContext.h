...
namespace CrossCompiler
{
	...
	{
		FShaderConductorOptions() = default;

		/** Initializes the options with a call to ApplyCommonCompilerFlags(). */
		FORCEINLINE FShaderConductorOptions(const FShaderCompilerFlags& InCompilerFlags)
		{
			ApplyCommonCompilerFlags(InCompilerFlags);
		}
		...
		/**
		 * Applies common compiler flags to this options struct: WarningsAsErrors, ForceSpirvDebugInfo, and HLSL2021.
		 * Other flags must be individually applied depending on the shader backend.
		 */
		UE_API void ApplyCommonCompilerFlags(const FShaderCompilerFlags& InCompilerFlags);
	};
	...
	public:
		/** Convert raw ANSI string buffer containing DXC errors into array of <FShaderCompilerError>. */
		static UE_API void ConvertCompileErrors(FAnsiStringView ErrorsString, TArray<FShaderCompilerError>& OutErrors);

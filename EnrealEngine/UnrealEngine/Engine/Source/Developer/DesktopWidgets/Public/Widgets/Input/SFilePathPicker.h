...
private:
	...
	UE_API FReply HandleBrowseButtonClicked( );

	/** Callback for getting the text in the path text box. */
	UE_API FText HandleTextBoxText( ) const;

	/** Callback for committing the text in the path text box. */
	UE_API void HandleTextBoxTextCommitted( const FText& NewText, ETextCommit::Type /*CommitInfo*/ );

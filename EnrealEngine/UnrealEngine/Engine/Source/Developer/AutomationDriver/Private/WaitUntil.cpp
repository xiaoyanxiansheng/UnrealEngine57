...
FDriverWaitDelegate Until::ElementIsFocusedByKeyboard(const TSharedRef<IElementLocator, ESPMode::ThreadSafe>& ElementLocator,
	FWaitTimeout Timeout)
{
	return Until::ElementIsFocusedByKeyboard(ElementLocator, FWaitInterval::InSeconds(1.0), Timeout);
}

FDriverWaitDelegate Until::ElementIsFocusedByKeyboard(const TSharedRef<IElementLocator, ESPMode::ThreadSafe>& ElementLocator,
	FWaitInterval Interval, FWaitTimeout Timeout)
{
	return Until::Condition([ElementLocator]()
	{
		check(IsInGameThread());
		...
		for (const TSharedRef<IApplicationElement>& Element : Elements)
		{
...
FDriverWaitDelegate Until::ElementIsFocusedByUser(uint32 UserIndex,
	const TSharedRef<IElementLocator, ESPMode::ThreadSafe>& ElementLocator, FWaitTimeout Timeout)
{
	return Until::ElementIsFocusedByUser(UserIndex, ElementLocator, FWaitInterval::InSeconds(1.0), Timeout);
}

FDriverWaitDelegate Until::ElementIsFocusedByUser(uint32 UserIndex,
	const TSharedRef<IElementLocator, ESPMode::ThreadSafe>& ElementLocator, FWaitInterval Interval, FWaitTimeout Timeout)
{
	return Until::Condition([ElementLocator, UserIndex]()
	{
		check(IsInGameThread());
		...
		for (const TSharedRef<IApplicationElement>& Element : Elements)
		{
			if (Element->IsFocused(UserIndex))
			{
				return true;
			}
		}

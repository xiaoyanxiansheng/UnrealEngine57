...
void AScreenshotFunctionalTest::RequestScreenshot()
{
	...
	if(IsMobilePlatform(GShaderPlatformForFeatureLevel[GMaxRHIFeatureLevel]) && !GEngine->IsStereoscopic3D())
	{
		...
		FIntPoint Size = UAutomationBlueprintFunctionLibrary::GetAutomationScreenshotSize(ScreenshotOptions);
	...
	else
	{
		...
		FIntRect ViewRect = FIntRect(); // Passing an empty ViewRect defaults to including to the entire render target in the screenshot
		...
		if (ScreenshotOptions.bAllowEmulatingSplitscreen)
		{
			...
			const FIntPoint Size = UAutomationBlueprintFunctionLibrary::GetAutomationScreenshotSize(ScreenshotOptions);	
			...
			switch (EmulateSplitscreenMode)
			{
				case 0:
					break;
				case 1:	// Horizontal
					ViewRect = FIntRect(Size.X, 0, Size.X * 2, Size.Y);
					break;
				case 2:	// Vertical
					ViewRect = FIntRect(0, Size.Y, Size.X, Size.Y * 2);
					break;
				case 3:	// Diagonal
					ViewRect = FIntRect(Size, Size * 2);
					break;
				default:
					// Already hit ensure in AScreenshotFunctionalTestBase::PrepareForScreenshot()
					break;
			}

			checkf(ViewRect.Size() == Size || ViewRect.IsEmpty(), TEXT("Screenshot view rect should either match the expected automation screenshot size, or be empty (which requests a screenshot of the entire render target)."));
		}
		
		FScreenshotRequest::RequestScreenshot(TEXT(""), /*bInShowUI*/ false, /*bAddUniqueSuffix*/ true, /*bHdrScreenshot*/ false, ViewRect);
	}
}

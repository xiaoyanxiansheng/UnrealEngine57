...
void AFunctionalTest::EditTags()
{
	...
	if (Settings)
	{
		...
		Settings->InitialSelectedTestsNames.Add(GetBeautifiedName());
		Settings->bOpenTagEditorWhenInitialSelectedTestsNames = true;
	}
...
void AFunctionalTest::PostLoad()
{
	Super::PostLoad();

	SetFlags(RF_Transactional);

	PreviousTestTags = TestTags;
}
...
void AFunctionalTest::PostEditChangeProperty( struct FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyChangedEvent.Property != NULL)
	{
		if (FObjectEditorUtils::GetCategoryFName(PropertyChangedEvent.Property) == NAME_FunctionalTesting)
		{
			...
			else if (PropertyChangedEvent.Property->GetFName() == NAME_TestTags && !TestTags.IsEmpty())
			{
				bool bContinueEdit = true;
				// this property will be deprected
				if (EAppReturnType::Yes == FMessageDialog::Open(EAppMsgType::YesNo, FText::FromString(TEXT("Editing asset tags are being deprecated and \"Edit Test Tags\" should be used instead. Continue?"))))
				{
					PreviousTestTags = TestTags;
				}
				else
				{
					bContinueEdit = false;
					TestTags = PreviousTestTags;
				}

				if (bContinueEdit)
				{
					const FString FullTestName = GetBeautifiedName();
...
FString AFunctionalTest::GetBeautifiedName()
{

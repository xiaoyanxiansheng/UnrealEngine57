...
TAttribute<bool> UToolMenuEntryScript::CreateShowInToolbarTopLevelAttribute(FToolMenuContext& Context)
{
	static const FName FunctionName = GET_FUNCTION_NAME_CHECKED(UToolMenuEntryScript, ShowInToolbarTopLevel);
	if (GetClass()->IsFunctionImplementedInScript(FunctionName))
	{
		TWeakObjectPtr<UToolMenuEntryScript> WeakScriptObject(this);
		TAttribute<bool>::FGetter Getter;
		Getter.BindLambda(
			[WeakScriptObject, Context]()
			{
				UToolMenuEntryScript* Object = GetIfCanSafelyRouteCall(WeakScriptObject);
				return Object ? Object->ShowInToolbarTopLevel(Context) : false;
			}
		);
...
{
	bHasRegisteredMenuEntry = UToolMenus::AddMenuEntryObject(this);
}

void UToolMenuEntryScript::UnregisterMenuEntry()
{
	if (bHasRegisteredMenuEntry)
	{
		UToolMenus::RemoveMenuEntryObject(this);
		bHasRegisteredMenuEntry = false;
	}
}

...
#include "SAutomationTestTagEditor.h"
#include "SAutomationTestTagFilter.h"
...
public:
	...
	virtual void RegisterCommands() override
	{
		...
		UI_COMMAND( EditTags, "Edit Tags for Selection", "Edit Tags for Selection", EUserInterfaceActionType::Button, FInputChord());
...
public:
	...
	TSharedPtr<FUICommandInfo> EditTags;
...
void SAutomationWindow::Construct( const FArguments& InArgs, const IAutomationControllerManagerRef& InAutomationController, const TSharedRef<ISessionManager>& InSessionManager )
{
	...
	AutomationTagFilter = MakeShareable(new FAutomationGroupFilter());
	...
	AutomationFilters->Add( AutomationTagFilter );
	...
	TestTable = SNew(STreeView< TSharedPtr< IAutomationReport > >)
		...
		(
			...
			+ SHeaderRow::Column(AutomationTestWindowConstants::Tags)
			.FillWidth(0.20f)
...
void SAutomationWindow::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	SCompoundWidget::Tick(AllottedGeometry, InCurrentTime, InDeltaTime);

	UAutomationControllerSettings* Settings = UAutomationControllerSettings::StaticClass()->GetDefaultObject<UAutomationControllerSettings>();
	if (AutomationController->GetFilteredReports().Num() > 0 && Settings && Settings->bOpenTagEditorWhenInitialSelectedTestsNames)
	{
		Settings->bOpenTagEditorWhenInitialSelectedTestsNames = false;

		AutomationController->SetEnabledTests(Settings->InitialSelectedTestsNames);

		SAutomationTestTagEditor::ShowInWindow(this->AsShared(), AutomationController->GetEnabledReports(), FOnTestTagsSavedEvent());

		AutomationController->SetEnabledTests(TArray<FString>());
		Settings->InitialSelectedTestsNames.Empty();
	}
}
...
void SAutomationWindow::CreateCommands()
{
	...
	ActionList.MapAction( Commands.EditTags,
		FExecuteAction::CreateRaw(this, &SAutomationWindow::HandleEditTagsButtonClicked ),
		FCanExecuteAction::CreateRaw(this, &SAutomationWindow::IsEditTagsButtonEnabled )
		);
...
TSharedRef< SWidget > SAutomationWindow::MakeAutomationWindowToolBar( const TSharedRef<FUICommandList>& InCommandList )
{
	struct Local
	{
		static void FillToolbar(FToolBarBuilder& ToolbarBuilder, TSharedRef<SWidget> RunTests, TSharedRef<SWidget> DeviceGroups, TSharedRef<SWidget> PresetBox, TSharedRef<SWidget> TagFilter, TWeakPtr<class SAutomationWindow> InAutomationWindow)
		{
			...
			ToolbarBuilder.EndSection();
			ToolbarBuilder.BeginSection("Tags");
			{
				ToolbarBuilder.AddWidget(SNew(SSpacer));
				ToolbarBuilder.AddToolBarButton(FAutomationWindowCommands::Get().EditTags);
				ToolbarBuilder.AddWidget(SNew(SSpacer));
				ToolbarBuilder.AddWidget( TagFilter );
				ToolbarBuilder.AddWidget(SNew(SSpacer));
			}
			ToolbarBuilder.EndSection();
		}
	...
	TagFilter = SNew(SAutomationTestTagFilter, AutomationTagFilter);
	TagFilter->SetOnTestFilterAppliedEvent(FOnTestFilterAppliedEvent::CreateLambda([this]()
		{
	...
	Local::FillToolbar(ToolbarBuilder, RunTests, DeviceGroups, TestPresets, TagFilter.ToSharedRef(), AutomationWindow);
...
void SAutomationWindow::HandleEditTagsButtonClicked() const
{
#if WITH_EDITOR
	SAutomationTestTagEditor::ShowInWindow(this->AsShared(), AutomationController->GetEnabledReports(),
		FOnTestTagsSavedEvent::CreateLambda([this]()
			{
...
bool SAutomationWindow::IsEditTagsButtonEnabled() const
{
	return AutomationController->GetEnabledReports().Num() > 0;
}

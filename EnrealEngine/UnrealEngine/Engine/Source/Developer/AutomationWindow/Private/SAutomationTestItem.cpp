...
TSharedRef<SWidget> SAutomationTestItem::GenerateWidgetForColumn( const FName& ColumnName )
{
	...
	else if (ColumnName == AutomationTestWindowConstants::Tags)
	{
		return SNew(SHorizontalBox)
			+ SHorizontalBox::Slot()
			.HAlign(HAlign_Left)
			...
			[
				SNew(SSimpleButton)
					.Icon(FAutomationWindowStyle::Get().GetBrush("Icons.Edit"))
					.Visibility(this, &SAutomationTestItem::CanEditTags_GetVisibility)
					.ToolTipText(LOCTEXT("EditTestTags", "Edit Tags"))
					.OnClicked(FOnClicked::CreateSP(this, &SAutomationTestItem::OnEditTestTagsClicked))
			]
			+ SHorizontalBox::Slot()
			.Expose(TagsSlot)
			.HAlign(HAlign_Left)
			...
			[
				SNew(STextBlock)
					.Margin(FMargin(2, 0, 0, 0))
					.OverflowPolicy(ETextOverflowPolicy::Ellipsis)
					.Text(FText::FromString(TestStatus->GetTags()))
					.ToolTipText(FText::FromString(TestStatus->GetTags()))
			];
	}
...
EVisibility SAutomationTestItem::CanEditTags_GetVisibility() const
{
	return (WITH_EDITOR && !TestStatus->IsParent()) ? EVisibility::Visible : EVisibility::Collapsed;
}
...
FReply SAutomationTestItem::OnEditTestTagsClicked()
{
#if WITH_EDITOR
	TArray<IAutomationReportPtr> ReportList{ TestStatus };
	SAutomationTestTagEditor::ShowInWindow(this->AsShared(), ReportList,
		FOnTestTagsSavedEvent::CreateLambda([this]()
		{
			FChildren* ContainerWidgetChildren = TagsSlot->GetContainerWidget().GetChildren();
			if (ContainerWidgetChildren && ContainerWidgetChildren->Num() > 1)
			{
				STextBlock& TagsText = static_cast<STextBlock&>(ContainerWidgetChildren->GetChildAt(1).Get());
				FString UpdatedTags = TestStatus->GetTags();
				TagsText.SetText(FText::FromString(UpdatedTags));
				TagsText.SetToolTipText(FText::FromString(UpdatedTags));
			}
		}));
#endif
	return FReply::Handled();

...
namespace FStructSharedStructTest
{
	...
	struct FTest_SharedStructDestruction : FAITestBase
	{
		virtual bool InstantTest() override
		{
			int32 NumConstructedStructBefore = FTestStructNonTrivialDestructor::NumConstructed;
			{
				FTestStructNonTrivialDestructor Struct;	
				AITEST_EQUAL("Number of constructed FTestStructNonTrivialDestructor should have increased by 1 after local struct ctor", FTestStructNonTrivialDestructor::NumConstructed, NumConstructedStructBefore+1);
			}
			AITEST_EQUAL("Number of constructed FTestStructNonTrivialDestructor should be decreased by 1 after local struct dtor", FTestStructNonTrivialDestructor::NumConstructed, NumConstructedStructBefore);

			NumConstructedStructBefore = FTestStructNonTrivialDestructor::NumConstructed;
			{
				FTestStructNonTrivialDestructor Struct;
				FSharedStruct SharedStruct = FSharedStruct::Make(Struct);
				AITEST_EQUAL("Number of constructed FTestStructNonTrivialDestructor should have increased by 2 after local struct and shared struct ctors", FTestStructNonTrivialDestructor::NumConstructed, NumConstructedStructBefore+2);
			}
			AITEST_EQUAL("Number of constructed FTestStructNonTrivialDestructor should be decreased by 2 after local struct and shared struct dtors", FTestStructNonTrivialDestructor::NumConstructed, NumConstructedStructBefore);

			return true;
		}
	};

	IMPLEMENT_AI_INSTANT_TEST(FTest_SharedStructDestruction, "System.StructUtils.SharedStruct.Destruction");
}

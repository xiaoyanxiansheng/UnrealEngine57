...
public:
	...
	void sectionStarting(const Catch::SectionInfo& SectionInfo) final
	{
		if (ITestRunner* TestRunner = ITestRunner::Get())
		{
			if (SectionInfo.name != CurrentTest)
			{
				UE_CLOG(TestRunner->IsDebugMode(), LogLowLevelTests, Display,
					TEXT("running section \"%hs\" (line %" SIZE_T_FMT ")"),
					SectionInfo.name.c_str(),
					SIZE_T(SectionInfo.lineInfo.line));
			}
		}
	}

...
/* Return true of the Reference screenshot is compatible with the incoming one */
static bool IsCompatible(const FAutomationScreenshotMetadata& Reference, const FAutomationScreenshotMetadata& Incoming)
{
	// If Substrate is disabled and the screenshot was approved with Substrate enabled, mark it as incompatible as it might contains result specific to Substrate.
	return Substrate::IsSubstrateEnabled() ? true : !Reference.bIsSubstrate;
}
		...
		if ( ExistingMetadata.IsSet() )
		{
			...
			{	
				...
				if ( FFileHelper::LoadFileToString(Json, *ApprovedMetadataFile) )
				{
					...
					if ( FJsonObjectConverter::JsonObjectStringToUStruct(Json, &Metadata, 0, 0) )
					{
						// If there is only one reference screenshot, use it.
						const bool bCompatibleScreenShot = ApprovedDeviceShots.Num() == 1 || IsCompatible(Metadata, ExistingMetadata.GetValue());
						if (!bCompatibleScreenShot)
						{
							continue;
						}

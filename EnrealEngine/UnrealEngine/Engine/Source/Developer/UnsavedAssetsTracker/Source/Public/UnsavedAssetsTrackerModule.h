...
#define UE_API UNSAVEDASSETSTRACKER_API
...
public:
	...
	UE_API int32 GetUnsavedAssetNum() const;

	/** Returns the list of unsaved assets. */
	UE_API TArray<FString> GetUnsavedAssets() const;

	/** Check if the input asset is unsaved. */
	UE_API bool IsAssetUnsaved(const FString& FileAbsPathname) const;

	/** Construct the widget for the Editor status bar.*/
	UE_API TSharedRef<SWidget> MakeUnsavedAssetsStatusBarWidget();

	/** Displays a dialog prompting the user to save unsaved packages. */
	UE_API bool PromptToSavePackages();

...
void FUnsavedAssetsAutoCheckout::OnAsyncCheckout(const FString& AbsoluteAssetFilepath)
{
	...
	// Check if SourceControl has already checked out this file for us.
	// If it's checked out by someone else, we want this to proceed, so the operation can be auto-undo'd.
	FSourceControlStatePtr State = SourceControlModule.GetProvider().GetState(AbsoluteAssetFilepath, EStateCacheUsage::Use);
	if (State.IsValid() && State->IsCheckedOut())
	{
		return;
	}
	// Check if this is an added file, in which case it cannot be checked out or does not need to be checked out.
	if (State.IsValid() && State->IsAdded())
	{
		return;
	}

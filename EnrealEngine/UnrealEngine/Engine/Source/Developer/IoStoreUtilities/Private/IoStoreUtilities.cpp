...
public:
	...
	void CreateOutputFile(const TCHAR* OutputFilename)
	{
		...
		if (OutputArchive.IsValid())
		{
			OutputArchive->Logf(TEXT("OrderInContainer, ChunkId, PackageId, PackageName, Filename, ContainerName, Offset, OffsetOnDisk, Size, CompressedSize, Hash, ChunkType, ClassType, PakChunk, Platform, InstallType, DeliveryType"));
		}
	}
	...
	void AddChunk(const FString& ContainerName, int32 Index, const FIoStoreTocChunkInfo& Info, FPackageId PackageId, const FString& PackageName, const FString& ClassType)
	{
		if (OutputArchive.IsValid())
		{
			...
			OutputArchive->Logf(TEXT("%d, %s, 0x%s, %s, %s, %s, %lld, %lld, %lld, %lld, 0x%s, %s, %s, %s, %s, %s, %s"),
				Index,
				*LexToString(Info.Id),
...
int32 CreateTarget(const FIoStoreArguments& Arguments, const FIoStoreWriterSettings& GeneralIoWriterSettings)
{
	...
	UE_LOG(LogIoStore, Display, TEXT("Input:  %s UExp"),   *FormatMemoryString(UExpSize));
	UE_LOG(LogIoStore, Display, TEXT("Input:  %s UAsset"), *FormatMemoryString(UAssetSize));
	UE_LOG(LogIoStore, Display, TEXT("Input:  %s UBulk"),  *FormatMemoryString(UBulkSize));
	UE_LOG(LogIoStore, Display, TEXT("Input:  %s for %d Global shaders"), *FormatMemoryString(GlobalShaderSize), GlobalShaderCount);
	UE_LOG(LogIoStore, Display, TEXT("Input:  %s for %d Shared shaders"), *FormatMemoryString(SharedShaderSize), SharedShaderCount);
	UE_LOG(LogIoStore, Display, TEXT("Input:  %s for %d Unique shaders"), *FormatMemoryString(UniqueShaderSize), UniqueShaderCount);
	UE_LOG(LogIoStore, Display, TEXT("Input:  %s for %d Inline shaders"), *FormatMemoryString(InlineShaderSize), InlineShaderCount);
	UE_LOG(LogIoStore, Display, TEXT(""));
	...
	if (bIsLegacyStage)
	{
		UE_LOG(LogIoStore, Display, TEXT("Output: %s HeaderData"), *FormatMemoryString(HeaderSize));
	}

	UE_LOG(LogIoStore, Display, TEXT("Output: %s InitialLoadData"), *FormatMemoryString(InitialLoadSize));
	...
	if (uint32 ErrorCount = IoStoreWriterContext->GetErrors())
	{
		...
		UE_LOG(LogIoStore, Display, TEXT("Failure - %d error(s)"), ErrorCount);
		return -2;
	}
	UE_LOG(LogIoStore, Display, TEXT("Success"));
	return 0;
}
...
static void FindAndExtractPackages(
...
bool ParseContainerGenerationArguments(FIoStoreArguments& Arguments, FIoStoreWriterSettings& WriterSettings)
{
	...
	{
		UE_LOG(LogIoStore, Display, TEXT("Using max partition size override %s -> '%lld'"), *MaxPartitionSizeOverrideIter.Key, MaxPartitionSizeOverrideIter.Value);
	}
...
int32 CreateIoStoreContainerFiles(const TCHAR* CmdLine)
{
	...
	else if (FParse::Value(FCommandLine::Get(), TEXT("FindAndExtract="), ArgumentValue))
	{
		...
		{
			UE_LOG(LogIoStore, Error, TEXT("-PackageFilter=<wildcard> is required, use * for all packages"));
			return -1;
		}
		...
		if (OutPath.Len() == 0)
		{
			UE_LOG(LogIoStore, Error, TEXT("-OutPath=/path/to/out is required"));
			return -1;
		}

		FindAndExtractPackages(Arguments.KeyChain, ContainerPathOrWildcard, OutPath, PackageFilter);
	}
	...
	else
	{
		...
		UE_LOG(LogIoStore, Display, TEXT(" -Describe=</path/to/global.utoc> [-PackageFilter=<PackageNameWildcard>] [-DumpToFile=<describe.txt>] [-CryptoKeys=</path/to/crypto.json>]"));
		UE_LOG(LogIoStore, Display, TEXT(" -FindAndExtract=</path/to/[container.utoc|*.utoc]> -PackageFilter=<PackageNameWildcard> -OutPath=</path/to/output> [-CryptoKeys=</path/to/crypto.json>]"));
		return -1;
	}

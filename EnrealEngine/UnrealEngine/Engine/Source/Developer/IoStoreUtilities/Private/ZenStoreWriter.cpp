...
void FZenStoreWriter::WriteAdditionalFile(const FAdditionalFileInfo& Info, const FIoBuffer& FileData)
{
	...
	FileEntry.CompressedPayload = Async(EAsyncExecution::TaskGraph,
...
void FZenStoreWriter::Initialize(const FCookInfo& Info)
{
	...
	if (!bInitialized)
	{
		...
		IsRunningLocally = ZenServiceInstance.IsServiceRunningLocally();
		ServiceHostName = ZenServiceInstance.GetEndpoint().GetName();
		ServicePort = ZenServiceInstance.GetEndpoint().GetPort();
#endif

		ProjectStoreWriter.Write(ServiceHostName, ServicePort, IsRunningLocally, *ProjectId, *OplogId, TargetPlatformFName);
...
void FZenStoreWriter::UpdatePackageModifiedStatus(FUpdatePackageModifiedStatusContext& Context)
{
...

	{
		...
		if (!Index)
		{
			...
			{
				UE_LOG(LogZenStoreWriter, Verbose, TEXT("UpdatePackageModifiedStatus called with package %s that is not in the oplog."),

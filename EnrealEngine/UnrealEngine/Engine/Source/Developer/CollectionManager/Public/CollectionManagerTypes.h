...
{
	FCollectionRef(const TSharedPtr<ICollectionContainer>& InContainer, const FCollectionNameType& InCollection)
	...
	FCollectionRef(const TSharedPtr<ICollectionContainer>& InContainer, FName InName, ECollectionShareType::Type InType)
		: Container(InContainer)
		, Name(InName)
		, Type(InType)
	{
	...
	bool operator==(const FCollectionRef& Other) const
	{
		return Container == Other.Container && Name == Other.Name && Type == Other.Type;
	}
	...
	friend inline uint32 GetTypeHash(const FCollectionRef& Key)
	{
		uint32 Hash = 0;
		Hash = HashCombine(Hash, GetTypeHash(Key.Container));
		Hash = HashCombine(Hash, GetTypeHash(Key.Name));
		Hash = HashCombine(Hash, GetTypeHash(Key.Type));
		return Hash;
	}
	...
	bool IsValid() const
	{
		return Container.IsValid() && Name != NAME_None && Type < ECollectionShareType::CST_All;
	}

	TSharedPtr<ICollectionContainer> Container;
	FName Name;
	ECollectionShareType::Type Type;
};

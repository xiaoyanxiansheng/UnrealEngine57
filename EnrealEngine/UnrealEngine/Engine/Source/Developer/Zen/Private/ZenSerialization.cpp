		...
		if (Entry.Flags & CbAttachmentEntry::kIsLocalRef)
		{
			...
			if (Path.StartsWith(HandlePrefix))
			{
				...
				if (HandleNum != 0)
				{
					...
					if (!OK)
					{
						UE_LOG(LogZenSerialization, Warning, TEXT("Unable to read local file via handle '%" UINT64_FMT "', treating it as a missing attachment"), HandleNum);
						++Index;
						continue;
					}
					...
					while (Offset < FileSize.QuadPart)
					{
						...
						if (!Success)
						{
							...
							UE_LOG(LogZenSerialization, Warning, TEXT("Unable to read local file via handle '%" UINT64_FMT "', error: %u, treating it as a missing attachment"), HandleNum, LastError);
							break;
						}
						Offset += Length;
					}

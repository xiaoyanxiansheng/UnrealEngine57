...
namespace CrossCompiler
{
	...
	{
	public:
		FHlslParser(FLinearAllocator* InAllocator, FCompilerMessages& InCompilerMessages, TConstArrayView<UE::HlslParser::FScopedDeclarations> InScopedDeclarations);
...
Done:
	...
	// Prints the specified number of tokens, starting from the token the scanner is currently pointing to, into a string.
	static FString PrintDiagnosticTokenStream(const FHlslScanner& Scanner, int32 Count)
	{
		const FHlslToken* Token = nullptr;
		FString OutString;
		for (int32 TokenIndex = 0; TokenIndex < Count && (Token = Scanner.PeekToken(TokenIndex)) != nullptr; ++TokenIndex)
		{
			if (TokenIndex > 0)
			{
				OutString += TCHAR(' ');
			}
			OutString += Token->String;
		}
		return OutString;
	}
	...
	EParseResult TryTranslationUnit(FHlslParser& Parser, FLinearAllocator* Allocator, AST::FNode** OutNode)
	{
		...
		constexpr int32 kTokenLookAheadForError = 10;
		Parser.Scanner.SourceError(FString::Printf(TEXT("Unable to match rule! [Token stream: \"%s\"]"), *PrintDiagnosticTokenStream(Parser.Scanner, kTokenLookAheadForError)));
		return ParseResultError();
	}
	...
	FHlslParser::FHlslParser(FLinearAllocator* InAllocator, FCompilerMessages& InCompilerMessages, TConstArrayView<UE::HlslParser::FScopedDeclarations> InScopedDeclarations) :
		...
		Allocator(InAllocator)
	{
		...
		for (const UE::HlslParser::FScopedDeclarations& ScopedDeclarations : InScopedDeclarations)
		{
	...
	namespace Parser
	{
		bool Parse(const FString& Input, const FString& Filename, FCompilerMessages& OutCompilerMessages, TConstArrayView<UE::HlslParser::FScopedDeclarations> InScopedDeclarations, TCallback* Callback, void* CallbackData)
		{
		...
		bool Parse(const FString& Input, const FString& Filename, FCompilerMessages& OutCompilerMessages, TConstArrayView<UE::HlslParser::FScopedDeclarations> InScopedDeclarations, TFunction< void(CrossCompiler::FLinearAllocator* Allocator, CrossCompiler::TLinearArray<CrossCompiler::AST::FNode*>& ASTNodes)> Function)
		{
			CrossCompiler::FNodeContainer Container;
		...
		bool Parse(const FString& Input, const FString& Filename, FCompilerMessages& OutCompilerMessages, TConstArrayView<UE::HlslParser::FScopedDeclarations> InScopedDeclarations, CrossCompiler::FNodeContainer& OutContainer)
		{

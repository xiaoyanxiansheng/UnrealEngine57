...
#include "Developer/HlslParser/Private/HlslParser.h"
#include "Developer/HlslParser/Private/HlslAST.h"
	...
	EParseResult ComputeExpr(FHlslScanner& Scanner, int32 MinPrec, /*FInfo& Info,*/ FSymbolScope* SymbolScope, int32 ExpressionFlags, FLinearAllocator* Allocator, AST::FExpression** OutExpression, AST::FExpression** OutTernaryExpression)
	{
		...
		*/
		...
		do
		{
			...
			const int32 Precedence = GetPrecedence(Token);
			...
			Scanner.Advance();
			const int32 NextMinPrec = IsRightAssociative(Token) ? Precedence : Precedence + 1;
			...
			check(RHSExpression);
			const AST::EOperators BinaryOperator = AST::TokenToASTOperator(Token->Token);

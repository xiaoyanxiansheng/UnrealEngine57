...
void SReflectorToolTipWidget::Construct( const FArguments& InArgs )
{
			...
			.FillColumn(1, 1.0f);
	{
		...
		if (bIsInsideInvalidationRoot)
		{
			...
			BuildLabelAndValue(LOCTEXT("SupportsInvalidation", "Supports Invalidation"), { this, &SReflectorToolTipWidget::GetSupportsInvalidation });
			BuildLabelAndValue(LOCTEXT("SupportsInvalidationRecursive", "Recursively Supports Invalidation"), { this, &SReflectorToolTipWidget::GetSupportsInvalidationRecursive });
		}

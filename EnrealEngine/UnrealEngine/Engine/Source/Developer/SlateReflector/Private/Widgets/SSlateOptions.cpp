...
void SSlateOptions::Construct( const FArguments& InArgs )
{
	struct Local
	{
		static void AddMenuEntry(FMenuBuilder& MenuBuilder, const FSlateIcon& Icon, const FText& Label, const TCHAR* ConsoleVariable, bool bCanEdit = true)
		{
			...
			if (CVar)
			{
				...
				MenuBuilder.AddMenuEntry(
					...
					Icon,
					FUIAction(
						FExecuteAction::CreateLambda([CVar]()
						{
							// If the variable was changed in the command line, we might not be able to change it anymore from the UI so we remove the flag
							CVar->ClearFlags(EConsoleVariableFlags::ECVF_SetByCommandline);
							CVar->Set(!CVar->GetBool(), EConsoleVariableFlags::ECVF_SetByCode);
						}),
		...
		static void AddSubMenuEntry(FMenuBuilder& MenuBuilder, const FSlateIcon& Icon, const FText& Label, const TCHAR* ConsoleVariable, const FNewMenuDelegate& InSubMenu, bool bCanEdit = true)
		{
			IConsoleVariable* CVar = IConsoleManager::Get().FindConsoleVariable(ConsoleVariable);
			if (CVar)
			{
	...
	{
		static void FillPaintSubmenu(FMenuBuilder& MenuBuilder)
		{
			FSlateIcon Icon(FWidgetReflectorStyle::GetStyleSetName(), "Icon.Empty");
			AddMenuEntry(MenuBuilder, Icon, LOCTEXT("Paint_OnlyGameWindow", "Only Game Window"), TEXT("SlateDebugger.Paint.OnlyGameWindow"));
			AddMenuEntry(MenuBuilder, Icon, LOCTEXT("Paint_ToggleWidgetNameList", "Toggle Widget Name List"), TEXT("SlateDebugger.Paint.EnableWidgetNameList"));
			AddMenuEntry(MenuBuilder, Icon, LOCTEXT("Paint_Draw Border", "Draw Border"), TEXT("SlateDebugger.Paint.DrawBorder"));
			AddMenuEntry(MenuBuilder, Icon, LOCTEXT("Paint_Draw Fill", "Draw Fill"), TEXT("SlateDebugger.Paint.DrawFill"));
		}
		static void FillPrepassSubmenu(FMenuBuilder& MenuBuilder)
		{
			FSlateIcon Icon(FWidgetReflectorStyle::GetStyleSetName(), "Icon.Empty");
			AddMenuEntry(MenuBuilder, Icon, LOCTEXT("Prepass_OnlyGameWindow", "Only Game Window"), TEXT("SlateDebugger.Prepass.OnlyGameWindow"));
			AddMenuEntry(MenuBuilder, Icon, LOCTEXT("Prepass_ToggleWidgetNameList", "Toggle Widget Name List"), TEXT("SlateDebugger.Prepass.EnableWidgetNameList"));
			AddMenuEntry(MenuBuilder, Icon, LOCTEXT("Prepass_Draw Border", "Draw Border"), TEXT("SlateDebugger.Prepass.DrawBorder"));
			AddMenuEntry(MenuBuilder, Icon, LOCTEXT("Prepass_Draw Fill", "Draw Fill"), TEXT("SlateDebugger.Prepass.DrawFill"));
		}
		...
		static TSharedRef<SWidget> FillToolbar()
		{
			...
			AddSubMenuEntry(MenuBuilder, Icon, LOCTEXT("PaintDebugging", "Show Paint"), TEXT("SlateDebugger.Paint.Enable"), FNewMenuDelegate::CreateStatic(&FillPaintSubmenu));
			AddSubMenuEntry(MenuBuilder, Icon, LOCTEXT("PrepassDebugging", "Show Prepass"), TEXT("SlateDebugger.Prepass.Enable"), FNewMenuDelegate::CreateStatic(&FillPrepassSubmenu));

...
#if WITH_EDITOR
	FEditorDelegates::EndPIE.AddSP(this, &SWidgetReflector::HandleEndPIE);
	FCoreUObjectDelegates::OnObjectTransacted.AddRaw(this, &SWidgetReflector::HandleObjectTransacted);
#endif //WITH_EDITOR
}
...
#if WITH_EDITOR
void SWidgetReflector::HandleEndPIE(const bool bIsSimulating)
{
	if (PropertyViewPtr.IsValid())
	{
		PropertyViewPtr->SetObject(nullptr);
	}

	if (bTransactedPIEObject)
	{
		GEditor->ResetTransaction(LOCTEXT("WidgetReflectorDuringPIE", "PIE Objects may be in the transaction buffer that need to be destroyed"));
		bTransactedPIEObject = false;
	}
}

void SWidgetReflector::HandleObjectTransacted(UObject* InObject, const FTransactionObjectEvent& InTransactionEvent)
{
	// If the transacted object was recorded by the widget reflector.
	if ((GEditor != nullptr && GEditor->PlayWorld != nullptr) || GIsPlayInEditorWorld)
	{
		if (PropertyViewPtr && PropertyViewPtr->GetSelectedObjects().Contains(InObject))
		{
			bTransactedPIEObject = true;
		}
	}
}
#endif //WITH_EDITOR

...
void FMeshSimplifier::ShrinkVoxelTriangles( float ShrinkAmount, TBitArray<>& InVoxelTriangles )
{
	for (TConstSetBitIterator<> BitIt( InVoxelTriangles ); BitIt; ++BitIt)
	{
		const uint32 TriIndex = BitIt.GetIndex();
		...
		for (uint32 k = 0; k < 3; k++)
		{
			const uint32 VertIndex = Indexes[ TriIndex * 3 + k ];
			Center += GetPosition( VertIndex );
		}

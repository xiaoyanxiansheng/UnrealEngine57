...
FText SSourceControlControls::GetSourceControlSyncStatusToolTipText()
{
	...
	{
		return LOCTEXT("SyncLatestButtonNotAtHeadTooltipTextConflictUpcoming", "Some of your local changes conflict with later snapshots on the project. Sync Latest or Check-in Changes to review and address these issues.");
	}
	if (HasSourceControlChangesToSync())
	{
...
FText SSourceControlControls::GetSourceControlCheckInStatusToolTipText()
{
	...
	if (GetNumConflictsUpcoming() > 0)
	{
		return LOCTEXT("CheckInButtonChangesTooltipTextConflictUpcoming", "Some of your local changes conflict with later snapshots on the project. Sync Latest or Check-in Changes to review and address these issues.");
	}
	if (HasSourceControlChangesToCheckIn())
	{

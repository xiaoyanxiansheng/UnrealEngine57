...
void ISourceControlProvider::RegisterStateBranches(const TArray<FString>& BranchNames, const TArray<FString>& ContentRoots)
{
	if (ContentRoots.Num() > 0)
	{
		ensureMsgf(ContentRoots.Num() >= 1, TEXT("This provider does not support multiple content roots, only using the first provided root (%s)"), *ContentRoots[0]);
		RegisterStateBranches(BranchNames, ContentRoots[0]);
	}
}

...
void FSourceControlModule::Tick()
{
	if (CurrentSourceControlProvider != nullptr)
	{
		...
		if (TemporarilyDisabledCount == 0)
		{
			...
			{
				// get batch size
				const int32 MaxStatusDispatchesPerRequest = GetSourceControlFileStatusMonitor().GetMaxFilePerRequestPolicy();

				// grab a batch of files
				TArray<FString> FilesToDispatch;
				FilesToDispatch.Reserve(MaxStatusDispatchesPerRequest);
				for (const FString& Filename : PendingStatusUpdateFiles)
				{
					if(FilesToDispatch.Num() >= MaxStatusDispatchesPerRequest)
					{
						break;
					}
...
void FSourceControlModule::RegisterPreSubmitChangelistValidation(const FSourceControlPreSubmitChangelistValidationDelegate& PreSubmitChangelistValidationDelegate)
{
	OnSourceControlPreSubmitChangelistValidation = PreSubmitChangelistValidationDelegate;
}

void FSourceControlModule::UnregisterPreSubmitChangelistValidation()
{
	OnSourceControlPreSubmitChangelistValidation = FSourceControlPreSubmitChangelistValidationDelegate();
}

FSourceControlPreSubmitChangelistValidationDelegate FSourceControlModule::GetRegisteredPreSubmitChangelistValidation()
{
	return OnSourceControlPreSubmitChangelistValidation;
}

void FSourceControlModule::RegisterPreSubmitFilesValidation(const FSourceControlPreSubmitFilesValidationDelegate& PreSubmitFilesValidationDelegate)
{
	OnSourceControlPreSubmitFilesValidation = PreSubmitFilesValidationDelegate;
}

void FSourceControlModule::UnregisterPreSubmitFilesValidation()
{
	OnSourceControlPreSubmitFilesValidation = FSourceControlPreSubmitFilesValidationDelegate();
}

FSourceControlPreSubmitFilesValidationDelegate FSourceControlModule::GetRegisteredPreSubmitFilesValidation()
{
	return OnSourceControlPreSubmitFilesValidation;
}

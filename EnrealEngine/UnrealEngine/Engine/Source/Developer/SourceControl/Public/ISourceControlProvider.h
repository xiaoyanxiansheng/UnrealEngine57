...
public:
	...
	* Register branches to query for state in addition to the current branch
	* @param	BranchNames			Names of the branches to query
	* @param	ContentRoots			Paths to the content roots for branch mapping
	*/
	SOURCECONTROL_API virtual void RegisterStateBranches( const TArray<FString>& BranchNames, const TArray<FString>& ContentRoots );

	/**
	* Gets the state index of the specified branch, higher index branches are generally closer to releases
	* @param	BranchName			Name of the branch to query
	* @return	the index of the specified branch
	*/
	virtual int32 GetStateBranchIndex(const FString& BranchName) const = 0;

	/**
	* Gets the name of the state branch at the given index, higher index branches are generally closer to releases
	* @param	BranchIndex			Index of the branch to retrieve
	* @param	OutBrancheName[out] Name of the branch, if successful
	* @return true if the branch name could be retrieved, false otherwise.
	*/
	virtual bool GetStateBranchAtIndex(int32 BranchIndex, FString& OutBranchName) const = 0;

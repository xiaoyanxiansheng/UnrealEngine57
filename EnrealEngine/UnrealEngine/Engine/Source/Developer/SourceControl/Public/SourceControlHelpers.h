...
public:
	...
	void SetFromStatus(const FSourceControlStatePtr& InStatus)
	{
		if (InStatus)
		{
			...
			//Go through the history to find the most recent user
			const int32 HistorySize = InStatus->GetHistorySize();
			for ( int32 Index = 0; Index < HistorySize; Index++ )
			{
				if ( const ISourceControlRevision* Revision = InStatus->GetHistoryItem( Index ).Get() )
				{
					const FString User = Revision->GetUserName();
					if ( !User.IsEmpty() )
					{
						PreviousUser = User;
						break;
	...
	/** The username of the previous user to modify this file */
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Editor Scripting | Editor Revision Control Helpers", meta = (Keywords = "Source Control"))
	FString PreviousUser;
};  // FSourceControlState

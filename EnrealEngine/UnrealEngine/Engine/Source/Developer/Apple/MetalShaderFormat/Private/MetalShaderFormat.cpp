...
extern bool FinalizeLibrary_Metal(class FName const& Format, class FString const& ArchivePath, class FString const& LibraryPath, TSet<uint64> const& Shaders, class FString const& DebugOutputDir, bool bStripLibs);
...
public:
		...
		TArray<FString>* OutputFiles) const override final
	{
		...
		// Bindless needs a lower limit of shaders per library as the packing process takes a significant amount of RAM
		...
		if(FDataDrivenShaderPlatformInfo::GetSupportsBindless(Platform)
		   ...
		   && ShouldGenerateShaderSymbolsInfo(ShaderFormatName))
		{
			...
			FGraphEventRef CompletionFence = FFunctionGraphTask::CreateAndDispatchWhenReady([ShaderFormatName, ArchivePath, LibraryPath, PartialShaders, DebugOutputDir, &CompiledLibraries, Platform]()
				{
					...
					bool bStripLibrary = FDataDrivenShaderPlatformInfo::GetIsMobile(Platform) && CVarSymbols->GetInt() == 0 && CVarExtraData->GetInt() == 0;

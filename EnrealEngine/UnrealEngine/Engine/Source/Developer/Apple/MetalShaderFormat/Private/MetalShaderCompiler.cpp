...
#if UE_METAL_USE_METAL_SHADER_CONVERTER
	bUseMetalShaderConverter = bBindlessEnabled;
#endif
...
bool FinalizeLibrary_Metal(FName const& Format, FString const& WorkingDir, FString const& LibraryPath, TSet<uint64> const& Shaders, class FString const& DebugOutputDir, bool bStripLibs)
{
	bool bOK = false;
    ...
    bool bCompiledWithMetalShaderConverter = FDataDrivenShaderPlatformInfo::GetSupportsBindless(Platform) &&
											 UE::ShaderCompiler::GetBindlessConfiguration(Platform) != ERHIBindlessConfiguration::Disabled;
	...
	if (bCompilerAvailable)
	{
        ...
        if (bCompiledWithMetalShaderConverter)
        {
            ...
            TArray<FString> MetalPackArgsBatches;
            FString MetalPackArgs;
            ...
            for (auto Shader : Shaders)
            {
				...
				MetalPackArgs += FString::Printf(TEXT("%s "), *SourceFilePath);
                CurPackArgsArgc++;

                if (CurPackArgsArgc > NumArgcPerBatch)
                {
					MetalPackArgsBatches.Add(MetalPackArgs);
					MetalPackArgs.Empty(); // TODO: Might switch to SetNum(0); as Empty() implicitly reallocs (IIRC)
                    CurPackArgsArgc = 0;
                }
            }

            // Add pending batch to the list.
            if (MetalPackArgs.Len() > 0)
            {
				MetalPackArgsBatches.Add(MetalPackArgs);
            }
...

            {
				...
				{
					UE_LOG(LogShaders, Error, TEXT("Archiving failed: no valid input for metal-pack."));
					return false;
				}
				
				// MetalPack each batch.
				FString BatchMergeArgs;
				for (int32 BatchIdx = 0; BatchIdx < MetalPackArgsBatches.Num(); BatchIdx++)
				{
					FString BatchOutputFile = FString::Printf(TEXT("MetalPackBatch_%d_"), BatchIdx);
                    ...
                    UE_LOG(LogMetalShaderCompiler, Display, TEXT("[%d/%d] %s"), (BatchIdx + 1), MetalPackArgsBatches.Num(), *BatchOutputPath);

					FString MetalPackParams = FString::Printf(TEXT("-pack-metallibs internal -pack-descriptors internal -pack-reflections internal %s -o \"%s\""), *MetalPackArgsBatches[BatchIdx], *BatchOutputPath);
					ReturnCode = 0;
					Results.Empty();
					Errors.Empty();

					bool bSuccess = Toolchain->ExecMetalPack(SDK, *MetalPackParams, &ReturnCode, &Results, &Errors);

					// handle compile error
					if (!bSuccess || ReturnCode != 0)
					{
						UE_LOG(LogShaders, Error, TEXT("Archiving failed: metal-pack failed with code %d: %s %s"), ReturnCode, *Results, *Errors);
					}
				}
				...
				FString MetalPackParams = FString::Printf(TEXT("-pack-metallibs internal -pack-descriptors internal -pack-reflections internal %s -o \"%s\""), *BatchMergeArgs, *LocalMetalLibPath);
				ReturnCode = 0;
				Results.Empty();
				Errors.Empty();
...
#endif
		{
			...
			{
				// Archive build phase - like unix ar, build metal archive from all the object files
				UE_LOG(LogMetalShaderCompiler, Display, TEXT("Stripping lib %s Size: %d"), *LibraryPath, IFileManager::Get().FileSize(*LibraryPath));
				
                FString MetalStripParams = FString::Printf(TEXT("-S -T -o=\"%s\" \"%s\""), *LibraryPath, *LibraryPath);
				
				bSuccess = Toolchain->ExecMetalStrip(SDK, *MetalStripParams, &ReturnCode, &Results, &Errors);
				
				if (!bSuccess || ReturnCode != 0)
				{
					UE_LOG(LogShaders, Error, TEXT("Stripping failed: metallib failed with code %d: %s %s"), ReturnCode, *Results, *Errors);
					return false;
				}
				
				UE_LOG(LogMetalShaderCompiler, Display, TEXT("Stripped size of %s Size: %d"), *LibraryPath, IFileManager::Get().FileSize(*LibraryPath));
				...
				if (!bOK)
				{
					UE_LOG(LogShaders, Error, TEXT("Stripping failed: failed to copy to local destination: %s"), *LibraryPath);
					return bOK;
				}
			}
		}

...
struct FDirectoryWatcherTestPayload
{
	...
	void OnDirectoryChanged(const TArray<FFileChangeData>& InFileChanges)
	{
		...
		{
			FString RelativeFilename = *FPaths::ConvertRelativePathToFull(Change.Filename) + FPaths::ConvertRelativePathToFull(WorkingDir).Len();
...
{
	FString GetWorkingDirAsRelativePath()
	{
		return FPaths::AutomationTransientDir() / TEXT("DirectoryWatcher") / TEXT("");
	}

	FString GetWorkingDir()
	{
		return FPaths::ConvertRelativePathToFull(GetWorkingDirAsRelativePath()) / TEXT("");
	}
}
...
// Test dual directory watchers for the same directory using both relative and absolute paths.
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FDirectoryWatcherCreateDualWatchersTest, "System.Plugins.Directory Watcher.Create Dual Watchers", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
bool FDirectoryWatcherCreateDualWatchersTest::RunTest(const FString& Parameters)
{
	static const TCHAR* Filename = TEXT("dual.tmp");

	const FString WorkingDirRelativePath = DirectoryWatcherTests::GetWorkingDirAsRelativePath();
	const FString WorkingDirFullPath = DirectoryWatcherTests::GetWorkingDir();
	const FString FileFullPath = WorkingDirFullPath / Filename;

	// Start watching the directory using relative path
	TSharedPtr<FDirectoryWatcherTestPayload> WatchRelativePath = MakeShareable(new FDirectoryWatcherTestPayload(WorkingDirRelativePath));

	// Start watching the directory using full path
	TSharedPtr<FDirectoryWatcherTestPayload> WatchFullPath = MakeShareable(new FDirectoryWatcherTestPayload(WorkingDirFullPath));

	// Give the stream time to start up before doing the test
	AddCommand(new FDelayedFunctionLatentCommand([this, FileFullPath, WatchRelativePath, WatchFullPath] {

		// Create a file and check that it got reported as created by both watchers
		FFileHelper::SaveStringToFile(TEXT(""), *FileFullPath);

		AddCommand(new FDelayedFunctionLatentCommand([this, WatchRelativePath, WatchFullPath] {

			FFileChangeData::EFileChangeAction* Action = WatchRelativePath->ReportedChanges.Find(Filename);

			if (!Action || *Action != FFileChangeData::FCA_Added)
			{
				UE_LOG(LogDirectoryWatcherTests, Error, TEXT("New file '%s' was not correctly reported as being added by relative path watcher."), Filename);
			}

			Action = WatchFullPath->ReportedChanges.Find(Filename);

			if (!Action || *Action != FFileChangeData::FCA_Added)
			{
				UE_LOG(LogDirectoryWatcherTests, Error, TEXT("New file '%s' was not correctly reported as being added by full path watcher."), Filename);
			}

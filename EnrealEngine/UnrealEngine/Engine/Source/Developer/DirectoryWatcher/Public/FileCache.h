...
#define UE_API DIRECTORYWATCHER_API
...
public:
	...
	UE_API FFileCache(const FFileCacheConfig& InConfig);
	...
	UE_API bool HasStartedUp() const;
	...
	UE_API bool MoveDetectionInitialized() const;
	 ...
	 */
	UE_API const FFileData* FindFileData(FImmutableString InFilename) const;

	/** Tick this FileCache */
	UE_API void Tick();

	/** Write out the cached file, if we have any changes to write */
	UE_API void WriteCache();
	 ...
	 */
	UE_API void CompleteTransaction(FUpdateCacheTransaction&& Transaction);

	/** Report an external change to the manager, such that a subsequent equal change reported by the os be ignored */
	UE_API void IgnoreNewFile(const FString& Filename);
	UE_API void IgnoreFileModification(const FString& Filename);
	UE_API void IgnoreMovedFile(const FString& SrcFilename, const FString& DstFilename);
	UE_API void IgnoreDeletedFile(const FString& Filename);
	...
	*/
	UE_API void IterateOutstandingChanges(TFunctionRef<bool(const FUpdateCacheTransaction&, const FDateTime&)> InPredicate) const;
	...
	UE_API TArray<FUpdateCacheTransaction> FilterOutstandingChanges(TFunctionRef<bool(const FUpdateCacheTransaction&, const FDateTime&)> InPredicate);
	UE_API TArray<FUpdateCacheTransaction> GetOutstandingChanges();

private:

	/** Called when the directory we are monitoring has been changed in some way */
	UE_API void OnDirectoryChanged(const TArray<FFileChangeData>& FileChanges);
	 ...
	 */
	UE_API void DiffDirtyFiles(TMap<FImmutableString, FFileData>& InDirtyFiles, TArray<FUpdateCacheTransaction>& OutTransactions, const FDirectoryState* InFileSystemState = nullptr) const;

	/** Detect a rename for the specified file */
	UE_API void DetectRename();

	/** Get the absolute path from a transaction filename */
	UE_API FString GetAbsolutePath(const FString& InTransactionPath) const;

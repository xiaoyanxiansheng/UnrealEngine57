...
bool FVisualLoggerFilters::ShouldDisplayLine(const FVisualLogLine& Line, const bool bSearchInsideLogs) const
{
	bool bShowLine = ShouldDisplayCategory(Line.Category, Line.Verbosity);
	if (bShowLine && bSearchInsideLogs)
	{
		const FString SearchString = GetSearchString();
		if (SearchString.Len() > 0
			&& Line.Line.Find(SearchString) == INDEX_NONE
			&& Line.Category.ToString().Find(SearchString) == INDEX_NONE)
		{
			bShowLine = false;
		}
	}
...
bool FVisualLoggerFilters::ShouldDisplayCategory(const FName Name, const ELogVerbosity::Type Verbosity) const
{
...
bool FVisualLoggerFilters::ShouldDisplayCategoryByString(const FStringView String, const ELogVerbosity::Type Verbosity) const
{
	const ULogVisualizerSettings* Settings = ULogVisualizerSettings::StaticClass()->GetDefaultObject<ULogVisualizerSettings>();
	for (const FCategoryFilter& Filter : Categories)
	{
		if (Filter.CategoryName.Equals(String.GetData(), ESearchCase::IgnoreCase))
		{
			if (Filter.Enabled)
			{
				const bool bFineWithSearchString = Settings->bSearchInsideLogs == true || (SearchBoxFilter.Len() == 0 || Filter.CategoryName.Find(SearchBoxFilter) != INDEX_NONE);
...
void FVisualLoggerFilters::DeactivateAllButThis(const FString& InName)
{
	...
	{
		Filter.Enabled = Filter.CategoryName == InName;
	}
}

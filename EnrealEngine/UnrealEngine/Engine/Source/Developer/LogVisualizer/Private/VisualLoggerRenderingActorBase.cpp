...
public:
		...
		: FDebugRenderSceneProxy(InComponent)
	{
		...
		FarClippingDistance = InComponent->GetFarClippingDistance();
	}
...
UVisualLoggerRenderingComponent::UVisualLoggerRenderingComponent(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void UVisualLoggerRenderingComponent::SetFarClippingDistance(double Distance)
{
	if (FarClippingDistance != Distance)
	{
	...
	RenderingActor->IterateDebugShapes([VLogSceneProxy] (const AVisualLoggerRenderingActorBase::FTimelineDebugShapes& Shapes)
		{
			...
			VLogSceneProxy->Circles.Append(Shapes.Circles);
			...
			VLogSceneProxy->CoordinateSystems.Append(Shapes.CoordinateSystems);
		});
...
void AVisualLoggerRenderingActorBase::SetFarClippingDistance(double Distance)
{
	if (RenderingComponent != nullptr)
	{
		RenderingComponent->SetFarClippingDistance(Distance);
	}
}

double AVisualLoggerRenderingActorBase::GetFarClippingDistance() const
{
	return (RenderingComponent != nullptr) ? RenderingComponent->GetFarClippingDistance() : 0.0;
}
...
void AVisualLoggerRenderingActorBase::GetDebugShapes(const FVisualLogEntry& InEntry, bool bAddEntryLocationPointer, AVisualLoggerRenderingActorBase::FTimelineDebugShapes& DebugShapes)
{
	...
	for (int32 ElementIndex = 0; ElementIndex < ElementsCount; ++ElementIndex, ++ElementToDraw)
	{
		...
		const FDebugRenderSceneProxy::EDrawType DrawTypeOverride = (
			...
			|| (ElementType == EVisualLoggerShapeElement::WireCylinder)
			|| (ElementType == EVisualLoggerShapeElement::WireCircle)) ? FDebugRenderSceneProxy::EDrawType::WireMesh : FDebugRenderSceneProxy::EDrawType::Invalid;
		...
		case EVisualLoggerShapeElement::Mesh:
		{
			...
			{
				const FVector& CurrentPoint = ElementToDraw->Points[VIdx];
				TestMesh.Vertices.Add(FVector3f(CurrentPoint));
				TestMesh.Box += CurrentPoint;
			}
		...
		case EVisualLoggerShapeElement::WireBox:
		{
			...
			for (int32 Index = 0; Index + 1 < NumPoints; Index += 2, BoxExtent += 2)
			{
				...
				DebugShapes.Boxes.Add(FDebugRenderSceneProxy::FDebugBox(Box, Color, Transform, DrawTypeOverride, Thickness));
		...
		case EVisualLoggerShapeElement::WireCone:
		{
			...
			for (int32 Index = 0; Index + 2 < ElementToDraw->Points.Num(); Index += 3)
			{
				...
				DebugShapes.Cones.Add(FDebugRenderSceneProxy::FCone(FScaleMatrix(FVector(Length)) * FMatrix(Direction, YAxis, ZAxis, Origin),
					static_cast<float>(Angles.Y), static_cast<float>(Angles.Z), Color, DrawTypeOverride, Thickness));
		...
		case EVisualLoggerShapeElement::WireCylinder:
		{
			...
			for (int32 Index = 0; Index + 2 < ElementToDraw->Points.Num(); Index += 3)
			{
				...
				DebugShapes.Cylinders.Add(FDebugRenderSceneProxy::FWireCylinder(Center
					...
					, HalfHeight, Color, DrawTypeOverride, Thickness)); // Base parameter is the center of the cylinder
				if (bDrawLabel)
				{
		...
		case EVisualLoggerShapeElement::WireCapsule:
		{
			...
			for (int32 Index = 0; Index + 2 < ElementToDraw->Points.Num(); Index += 3)
			{
				...
				DebugShapes.Capsules.Add(FDebugRenderSceneProxy::FCapsule(Base, Radius, XAxis, YAxis, ZAxis, HalfHeight, Color, DrawTypeOverride, Thickness));
				if (bDrawLabel)
				{
		...
		case EVisualLoggerShapeElement::Arrow:
		{
			...
			const FVector* Location = ElementToDraw->Points.GetData();
			const int32 NumPoints = ElementToDraw->Points.Num();
			const float Mag = float(ElementToDraw->Mag);

			for (int32 Index = 0; Index + 1 < NumPoints; Index += 2, Location += 2)
			{
				DebugShapes.Arrows.Add(FDebugRenderSceneProxy::FArrowLine(*Location, *(Location + 1), Color, Mag));
		...
		case EVisualLoggerShapeElement::WireCircle:
		{
			const float Thickness = float(ElementToDraw->Thickness);
			const bool bDrawLabel = ElementToDraw->Description.IsEmpty() == false;
			const int32 NumPoints = ElementToDraw->Points.Num();
			for (int32 Index = 0; Index + 2 < NumPoints; Index += 3)
			{
				...
				DebugShapes.Circles.Add(FDebugRenderSceneProxy::FCircle(Center, UpAxis.GetSafeNormal(), Radius, Color, DrawTypeOverride, Thickness)); 
		...
		case EVisualLoggerShapeElement::CoordinateSystem:
		{
			...
			for (int32 Index = 0; Index + 2 < NumPoints; Index += 3)
			{
				...
				DebugShapes.CoordinateSystems.Add(FDebugRenderSceneProxy::FCoordinateSystem(AxisLoc, AxisRot, Scale, Color, Thickness));

				if (bDrawLabel)
				{
					const FString PrintString = NumPoints == 3 ? ElementToDraw->Description : FString::Printf(TEXT("%s_%d"), *ElementToDraw->Description, Index / 3);
					DebugShapes.Texts.Add(FDebugRenderSceneProxy::FText3d(PrintString, AxisLoc, Color.WithAlpha(255)));
				}
			}
		}
			break;

...
void FSourceControlCheckInPrompter::OnSourceControlOperationComplete(const FSourceControlOperationRef& InOperation, ECommandResult::Type InResult)
{
	...
	if (InOperation->GetName() == TEXT("GetSubmittedChangelists"))
	{
		...
		{
			// If operation failed, end flow and mark as delayed so we try again in the next cycle
			bPromptDelayed = true;
			TimeGetSubmittedChangelistsExecuted.Remove(PromptFlowMapName);
			PromptFlowMapName.Empty();
		}
	}
}
...
void FSourceControlCheckInPrompter::CheckPrompt()
{
	...
	if (PromptFlowMapName.IsEmpty() && !EditorMapName.IsEmpty())
	{
		...
		// If the user has not been active in this world for a sufficient amount of time,
		// the prompt is delayed until the required interval has passed.
		bPromptDelayed = (FDateTime::UtcNow() - IntervalSessionLength < ProjectActivationTime);
		...
		// Otherwise, validate we can get latest changes and we actually have local changes to check in
		bool bIsPromptAllowed = IsPromptAllowed();
		bool bIsGetSubmittedChangelistsAllowed = IsGetSubmittedChangelistsAllowed();
		bool bPromptShow = bIsPromptAllowed && bIsGetSubmittedChangelistsAllowed;
		if (bPromptShow)
		{
			OnStartPrompt();
		}

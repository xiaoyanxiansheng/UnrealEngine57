...
bool FHordeHttpClient::Login(bool bUnattended, FFeedbackContext* Warn)
{
	...
	if (DesktopPlatform != nullptr)
	{
		...
		EDesktopLoginInteractionLevel InteractionLevel = bUnattended ? EDesktopLoginInteractionLevel::TimeLimited : EDesktopLoginInteractionLevel::Interactive;
		if (DesktopPlatform->GetHordeAccessToken(ServerUrl, InteractionLevel, Warn, NewToken, ExpiresAt, bWasInteractive))
		{
			Token = NewToken;
			return true;
		}
...
bool FHordeHttpClient::LoginWithOidc(const TCHAR* Profile, bool bUnattended, FFeedbackContext* Warn)
{
	...
	if (DesktopPlatform != nullptr)
	{
		...
		EDesktopLoginInteractionLevel InteractionLevel = bUnattended ? EDesktopLoginInteractionLevel::TimeLimited : EDesktopLoginInteractionLevel::Interactive;
		if (DesktopPlatform->GetOidcAccessToken(FPaths::RootDir(), FString(), Profile, InteractionLevel, Warn, NewToken, ExpiresAt, bWasInteractive))
		{
			Token = NewToken;
			return true;
		}

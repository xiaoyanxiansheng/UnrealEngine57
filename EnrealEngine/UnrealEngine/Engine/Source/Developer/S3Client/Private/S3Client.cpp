...
private:
	...
	static size_t SeekBodyCallback(void* UserData, curl_off_t Offset, int Origin);
...
size_t FS3Client::FS3Request::SeekBodyCallback(void* UserData, curl_off_t Offset, int Origin)
{
	auto& Request = *static_cast<FS3Client::FS3Request*>(UserData);

	uint64 Size = Request.RequestBody.GetSize();
	uint64 Position = Request.BytesSent;

	switch (Origin)
	{
	case SEEK_SET:	Position = Offset;				break;
	case SEEK_CUR:	Position = Position + Offset;	break;

...
void FLocTextHelper::SortManifest()
{
	checkf(Manifest.IsValid(), TEXT("Attempted to enumerate source texts, but no manifest has been loaded!"));
	FJsonInternationalizationManifestSerializer::SortManifest(Manifest.ToSharedRef());
}
...
void FLocTextHelper::TrimArchive(const FString& InCulture)
{
	...
	Archives[InCulture] = TrimmedArchive;
}
...
void FLocTextHelper::EnumerateSourceTextsParallel(const FEnumerateSourceTextsFuncPtr& InCallback, const bool InCheckDependencies) const
{
	checkf(Manifest.IsValid(), TEXT("Attempted to enumerate source texts, but no manifest has been loaded!"));

	std::atomic<bool> bBreak = false;

	TArray<TPair<FString, TSharedRef<FManifestEntry>>> ManifestEntries = Manifest->GetEntriesBySourceTextAsArray();
	ParallelFor(ManifestEntries.Num(), [this, &ManifestEntries, &bBreak, &InCallback, &InCheckDependencies](int32 Index)
		{
			if (bBreak.load(std::memory_order_acquire))
			{
				return;
			}

			const TSharedRef<FManifestEntry> ManifestEntry = ManifestEntries[Index].Value;
			...
			if (bShouldEnumerate && !InCallback(ManifestEntry))
			{
				bBreak.store(true, std::memory_order_release);
			}
		});
}

...
bool PortableObjectPipeline::ImportAll(FLocTextHelper& InLocTextHelper, const FString& InPOCultureRootPath, const FString& InPOFilename, const ELocalizedTextCollapseMode InTextCollapseMode, const EPortableObjectFormat InPOFormat, const bool bUseCultureDirectory)
{
	...
	const TArray<FString> Cultures = InLocTextHelper.GetAllCultures(bSingleCultureMode);
	if (bSingleCultureMode && (Cultures.Num() != 1 || InLocTextHelper.ShouldSplitPlatformData()))
	{
	...
	ParallelFor(TEXT("PortableObjectPipeline::ImportAll"), Cultures.Num(), 1, [&](int32 Index)
	{
		const FString& CultureName = Cultures[Index];
		...
		if (!ImportSinglePortableObject(FName()))
		{
			bSuccess = false;
		}
		for (const auto& PerPlatformManifestPair : PerPlatformManifests)
		{
			if (!ImportSinglePortableObject(PerPlatformManifestPair.Key))
			{
				bSuccess = false;
			}
		}
...
bool PortableObjectPipeline::ExportAll(FLocTextHelper& InLocTextHelper, const FString& InPOCultureRootPath, const FString& InPOFilename, const ELocalizedTextCollapseMode InTextCollapseMode, const EPortableObjectFormat InPOFormat, const bool bShouldPersistComments, const bool bUseCultureDirectory)
{
	...
	// Process the desired cultures
	std::atomic<bool> bSuccess = true;
	ParallelFor(TEXT("PortableObjectPipeline::ExportAll"), Cultures.Num(), 1, [&](int32 Index)
	{
		const FString& CultureName = Cultures[Index];
		...
		if (!ExportSinglePortableObject(PlatformAgnosticManifest.ToSharedRef(), FName()))
		{
			bSuccess = false;
		}
		for (const auto& PerPlatformManifestPair : PerPlatformManifests)
		{
			if (!ExportSinglePortableObject(PerPlatformManifestPair.Value, PerPlatformManifestPair.Key))
			{
				bSuccess = false;
			}
		}

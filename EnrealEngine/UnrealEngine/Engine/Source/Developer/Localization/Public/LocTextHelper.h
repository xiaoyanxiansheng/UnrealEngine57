...
public:
	/** Virtual destructor */
	virtual ~ILocFileNotifies() = default;

	/** Start a block of parallel tasks */
	virtual void BeginParallelTasks() = 0;

	/** Stop a block of parallel tasks */
	virtual void EndParallelTasks() = 0;
...
public:
	...
	/**
	 * Sort the currently loaded manifest into a deterministic order (as if it had been serialized and then deserialized from JSON).
	 */
	UE_API void SortManifest();
	...
	/**
	 * Enumerate in parallel all the source texts in the manifest, optionally skipping those entries from a dependent manifest.
	 * InCallback shall be called from multiple threads so it is the calllers responsibility to guarantee thread safety inside
	 * the callback's scope.
	 *
	 * @param InCallback			Function to call for each source text. Returns true to continue enumeration.
	 * @param InCheckDependencies	True to skip entries that can be found in a dependent manifest, false to include them.
	 */
	UE_API void EnumerateSourceTextsParallel(const FEnumerateSourceTextsFuncPtr& InCallback, const bool InCheckDependencies) const;

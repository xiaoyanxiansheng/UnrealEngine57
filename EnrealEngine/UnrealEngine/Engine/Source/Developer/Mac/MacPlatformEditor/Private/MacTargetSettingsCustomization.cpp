...
#include "MacTargetSettingsCustomization.h"
...
TSharedRef<IDetailCustomization> FMacTargetSettingsCustomization::MakeInstance()
{
	return MakeShareable(new FMacTargetSettingsCustomization);
}
...
void FMacTargetSettingsCustomization::CustomizeDetails( IDetailLayoutBuilder& DetailBuilder )
{
	FSimpleDelegate OnUpdateShaderStandardWarning = FSimpleDelegate::CreateSP(this, &FMacTargetSettingsCustomization::UpdateShaderStandardWarning);
	...
	// Handle max. shader version a little specially.
	{
		...
		[
			...
			[
				SNew(SComboButton)
				.OnGetMenuContent(this, &FMacTargetSettingsCustomization::OnGetShaderVersionContent)
				.ContentPadding(FMargin( 2.0f, 2.0f ))
				.ButtonContent()
				[
					SNew(STextBlock)
					.Text(this, &FMacTargetSettingsCustomization::GetShaderVersionDesc)
		...
		RenderCategory.AddCustomRow(LOCTEXT("UseBindlessWithSM5", "Use Bindless With SM5 (Experimental)"), false)
		.RowTag("UseBindlessWithSM5")
		.NameContent()
		[
			SNew(STextBlock)
			.Font(IDetailLayoutBuilder::GetDetailFont())
			.Text(LOCTEXT("UseBindlessWithSM5", "Use Bindless With SM5 (Experimental)"))
		]
		.ValueContent()
		[
			SNew(SCheckBox)
			.IsChecked(this, &FMacTargetSettingsCustomization::ShouldUseBindlessWithSM5)
			.OnCheckStateChanged(this, &FMacTargetSettingsCustomization::OnUseBindlessWithSM5Changed)
		];
	...
	[
		...
		[
			...
			.OnGetPickerPath(FOnGetPickerPath::CreateSP(this, &FMacTargetSettingsCustomization::GetPickerPath))
			.OnPostExternalImageCopy(FOnPostExternalImageCopy::CreateSP(this, &FMacTargetSettingsCustomization::HandlePostExternalIconCopy))
		]
	];
	...
	[
		...
		[
			...
			.FileDescription(GameSplashDesc)
			.OnPreExternalImageCopy(FOnPreExternalImageCopy::CreateSP(this, &FMacTargetSettingsCustomization::HandlePreExternalIconCopy))
			.OnGetPickerPath(FOnGetPickerPath::CreateSP(this, &FMacTargetSettingsCustomization::GetPickerPath))
			.OnPostExternalImageCopy(FOnPostExternalImageCopy::CreateSP(this, &FMacTargetSettingsCustomization::HandlePostExternalIconCopy))
		]
	];
...
bool FMacTargetSettingsCustomization::HandlePreExternalIconCopy(const FString& InChosenImage)
{
	return true;
}


FString FMacTargetSettingsCustomization::GetPickerPath()
{
	return FEditorDirectories::Get().GetLastDirectory(ELastDirectory::GENERIC_OPEN);
}


bool FMacTargetSettingsCustomization::HandlePostExternalIconCopy(const FString& InChosenImage)
{
...
TSharedRef<SWidget> FMacTargetSettingsCustomization::OnGetShaderVersionContent()
{
	FMenuBuilder MenuBuilder(true, NULL);
	
	UEnum* Enum = FindObjectChecked<UEnum>(nullptr, TEXT("/Script/MacTargetPlatformSettings.EMacMetalShaderStandard"), EFindObjectFlags::ExactClass);
	...
	for (int32 i = 0; i < Enum->GetMaxEnumValue(); i++)
	{
		...
		{
			FUIAction ItemAction(FExecuteAction::CreateSP(this, &FMacTargetSettingsCustomization::SetShaderStandard, i));
...
FText FMacTargetSettingsCustomization::GetShaderVersionDesc() const
{
	int32 EnumValue;
...
void FMacTargetSettingsCustomization::SetShaderStandard(int32 Value)
{
...
void FMacTargetSettingsCustomization::UpdateShaderStandardWarning()
{
	// Update the UI
	uint8 EnumValue = 0;
...
constexpr TCHAR SM5SPSection[] = TEXT("ShaderPlatformConfig METAL_SM5");
constexpr TCHAR BindlessConfigurationKey[] = TEXT("BindlessConfiguration");

ECheckBoxState FMacTargetSettingsCustomization::ShouldUseBindlessWithSM5() const
{
	FString OutSetting;
	if (GConfig->GetString(SM5SPSection, BindlessConfigurationKey, OutSetting, GEngineIni))
	{
		return OutSetting.Equals(TEXT("Disabled"), ESearchCase::IgnoreCase) ? ECheckBoxState::Unchecked : ECheckBoxState::Checked;
	}
	else
	{
...
void FMacTargetSettingsCustomization::OnUseBindlessWithSM5Changed(ECheckBoxState NewState)
{
	static const FString ClassConfigName = UMacTargetSettings::StaticClass()->ClassConfigName.ToString();
	static const FString DefaultConfigLayerFilename = UMacTargetSettings::StaticClass()->GetDefaultConfigFilename();

	if (!SettingsHelpers::CheckOutOrAddFile(DefaultConfigLayerFilename))
	{
		SettingsHelpers::MakeWritable(DefaultConfigLayerFilename);
	}

	// CF UObject::UpdateSingleSectionOfConfigFile implementation
	GConfig->Flush(false, *ClassConfigName);
	
	// Because BindlessConfiguration is not a UPROPERTY with config keyboard we have to do it this way.
	FConfigFile Temp;
	Temp.SetString(SM5SPSection, BindlessConfigurationKey, NewState == ECheckBoxState::Unchecked ? TEXT("Disabled") : TEXT("All"));
	Temp.UpdateSinglePropertyInSection(*DefaultConfigLayerFilename, BindlessConfigurationKey, SM5SPSection);
	
	// CF UObject::UpdateSingleSectionOfConfigFile implementation
	FConfigContext Context = FConfigContext::ForceReloadIntoGConfig();
	Context.bWriteDestIni = false;
	Context.Load(*ClassConfigName);

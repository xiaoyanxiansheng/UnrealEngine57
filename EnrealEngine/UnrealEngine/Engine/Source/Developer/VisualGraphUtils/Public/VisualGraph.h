...
class FVisualGraphSubGraph : public FVisualGraphElement
{
public:
...
public:
	...
	UE_API FVisualGraph(const FName& InName, const FName& InDisplayName = NAME_None);
	...
	UE_API int32 FindNode(const FName& InName) const;
	UE_API int32 FindEdge(const FName& InName) const;
	UE_API int32 FindSubGraph(const FName& InName) const;

	UE_API bool AddNodeToSubGraph(int32 InNodeIndex, int32 InSubGraphIndex);
	UE_API bool RemoveNodeFromSubGraph(int32 InNodeIndex);

	UE_API void TransitiveReduction(TFunction<bool(FVisualGraphEdge&)> KeepEdgeFunction);
...
protected:
	...
	static UE_API bool IsNameAvailable(const FName& InName, const TMap<FName, int32>& InMap);
	static UE_API FName GetUniqueName(const FName& InName, const TMap<FName, int32>& InMap);

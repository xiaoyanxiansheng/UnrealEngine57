...
public:
	...
	FLauncherUATTask( const FString& InCommandLine, const FString& InName, const FString& InDesc, void* InReadPipe, void* InWritePipe, const FString& InEditorExe, FProcHandle& InProcessHandle, ILauncherWorker* InWorker, const FString& InCommandEnd, const FString& InBuildCookRunPreCommand, const FString& InBuildCookRunPostCommand)
		...
		, PostCommand(InBuildCookRunPostCommand)
	{
...
protected:
	...
	virtual bool PerformTask( FLauncherTaskChainState& ChainState ) override
	{
		...
		// add any post command line
		if (!PostCommand.IsEmpty())
		{
			UATCommandLine += TEXT(" ");
			UATCommandLine += PostCommand;
		}
...
private:
	...
	// a UAT command to run after BuildCookRun (in the same insance as the BuildCookRun)
	FString PostCommand;

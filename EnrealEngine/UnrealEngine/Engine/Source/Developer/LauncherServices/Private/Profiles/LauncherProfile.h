...
public:
	...
	*/
	...
	virtual void AddCustomError( const FString& UniqueId, const FText& Text ) override
	{
		CustomErrors.Add(UniqueId, Text);
	}

	virtual bool HasCustomError( const FString& UniqueId ) const override
	{
		return CustomErrors.Contains(UniqueId);
	}

	virtual FText GetCustomErrorText( const FString& UniqueId ) const override
	{
		const FText* TextPtr = CustomErrors.Find(UniqueId);
		return TextPtr ? (*TextPtr) : FText::GetEmpty();
	}

	virtual TArray<FString> GetAllCustomErrors() const override
	{
		TArray<FString> Keys;
		CustomErrors.GetKeys(Keys);
		return MoveTemp(Keys);
	}

	virtual void AddCustomWarning( const FString& UniqueId, const FText& Text ) override
	{
		CustomWarnings.Add(UniqueId, Text);
	}

	virtual bool HasCustomWarning( const FString& UniqueId ) const override
	{
		return CustomWarnings.Contains(UniqueId);
	}

	virtual FText GetCustomWarningText( const FString& UniqueId ) const override
	{
		const FText* TextPtr = CustomWarnings.Find(UniqueId);
		return TextPtr ? (*TextPtr) : FText::GetEmpty();
	}

	virtual TArray<FString> GetAllCustomWarnings() const override
	{
		TArray<FString> Keys;
		CustomWarnings.GetKeys(Keys);
		return MoveTemp(Keys);
	}

	virtual void RefreshCustomWarningsAndErrors() override
	{
	...
	{
		return (ValidationErrors.Num() == 0 && CustomErrors.Num() == 0);
	}
	...
	virtual bool Serialize( FArchive& Archive ) override
	{
		...
		if (Version >= LAUNCHERSERVICES_AUTOMATEDTESTS)
		{
			...
			for( int32 TestIndex = 0; TestIndex < NumAutomatedTests; TestIndex++)
			{
				if (Archive.IsLoading())
				{
					AutomatedTests.Add(MakeShared<FLauncherProfileAutomatedTest>(Archive));
				}
				else
				{
	...
	virtual void Save(TJsonWriter<>& Writer) override
	{
		...
		// serialize the automated tests
		Writer.WriteArrayStart("AutomatedTests");
		for ( ILauncherProfileAutomatedTestRef AutomatedTest : AutomatedTests)
		{
			AutomatedTest->Save(Writer);
		}
		Writer.WriteArrayEnd();
	...
	virtual bool Load(const FJsonObject& Object) override
	{
		...
		if (Version >= LAUNCHERSERVICES_AUTOMATEDTESTS)
		{
			...
			if (Object.TryGetArrayField(TEXT("AutomatedTests"), Values))
			{
				...
				{
					AutomatedTests.Add(MakeShared<FLauncherProfileAutomatedTest>(*Value->AsObject().Get()));
				}
			}
		...
		if (Version >= LAUNCHERSERVICES_FASTITERATE)
		{
...
/*		if (GetTargetPlatformManager()->GetRunningTargetPlatform() != NULL)
	...
	{
		const ILauncherProfileAutomatedTestRef* TestPtr = AutomatedTests.FindByPredicate([InternalName]( const ILauncherProfileAutomatedTestRef& Test )
		{
			return FCString::Stricmp( InternalName, Test->GetInternalName()) == 0;
		});

		return TestPtr ? TestPtr->ToSharedPtr() : nullptr;
	}

	virtual ILauncherProfileAutomatedTestRef FindOrAddAutomatedTest( const TCHAR* InternalName ) override
	{
		ILauncherProfileAutomatedTestPtr AutomatedTest = GetAutomatedTest(InternalName);
		if (AutomatedTest == nullptr)
		{
			AutomatedTest = MakeShared<FLauncherProfileAutomatedTest>(InternalName);
	...
	virtual void RemoveAutomatedTest( const TCHAR* InternalName ) override
	{
		...
		if (AutomatedTest != nullptr)
		{
			AutomatedTests.Remove(AutomatedTest.ToSharedRef());
		}
	}
...
protected:
	...
	void Validate( )
	{
		...
		if ((DeploymentMode == ELauncherProfileDeploymentModes::CopyToDevice) && ((CookMode != ELauncherProfileCookModes::ByTheBook)&&(CookMode!=ELauncherProfileCookModes::ByTheBookInEditor)) && !IsUsingZenPakStreaming() && !IsImportingZenSnapshot())
		{
		...
		RefreshCustomWarningsAndErrors();
	}

 ...
 *
 * The first parameter is the automated test, the second is the profile that is being launched, the third parameter is the command line to modify.
 */
DECLARE_MULTICAST_DELEGATE_ThreeParams(FOnLauncherProfilePostProcessAutomatedTestCommandLine, const ILauncherProfileAutomatedTestRef&, const ILauncherProfileRef&, FString& );
...
public:
	...
	/**
	 * Returns a delegate that is invoked to post-process the command line before an automated test is launched
	 * 
	 * @return The delegate.
	 */
	virtual FOnLauncherProfilePostProcessAutomatedTestCommandLine& OnPostProcessAutomatedTestCommandLine( ) = 0;

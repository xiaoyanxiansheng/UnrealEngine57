...
struct FPhysAssetCreateParams
{
	...
	/** The LOD index use to create the physics asset.*/
	UPROPERTY(EditAnywhere, Category = "Body Creation")
	int32								LodIndex;
...
namespace FPhysicsAssetUtils
{
	...
	*/
	...
	/**
	 * Compute a covariance matrix describing the distribution of points in the provided set
	 * 
	 * @param PointView An array view of a vector type to calculate the covariance matrix for
	...
	PHYSICSUTILITIES_API FMatrix ComputeCovarianceMatrix(const FBoneVertInfo& VertInfo);

	extern template FMatrix ComputeViewCovarianceMatrix(TArrayView<const UE::Math::TVector<float>, int> PointView);
	extern template FMatrix ComputeViewCovarianceMatrix(TArrayView<const UE::Math::TVector<double>, int> PointView);

	/**
	 * Compute the dominant eigenvector of a provided matrix
	 * 
	 * @param InMatrix Matrix input
	 * @return Domininant eigenvector of InMatrix
	 */
	PHYSICSUTILITIES_API FVector ComputeEigenVector(const FMatrix& InMatrix);
};
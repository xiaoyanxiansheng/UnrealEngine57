...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PhysicsAssetUtils)
...
bool CreateFromSkeletalMeshInternal(UPhysicsAsset* PhysicsAsset, USkeletalMesh* SkelMesh, const FPhysAssetCreateParams& Params, const FSkinnedBoneTriangleCache& TriangleCache, bool bShowProgress)
{
	...
	FMeshUtilitiesEngine::CalcBoneVertInfos(SkelMesh, Infos, (Params.VertWeight == EVW_DominantWeight), Params.LodIndex);
	check(Infos.Num() == SkelMesh->GetRefSkeleton().GetRawBoneNum());
...
{
	return ComputeViewCovarianceMatrix(MakeArrayView(VertInfo.Positions));
}
...
// Explicitly instantiate and export covariance matrix calc
template PHYSICSUTILITIES_API FMatrix ComputeViewCovarianceMatrix(TArrayView<const UE::Math::TVector<float>, int> PointView);
template PHYSICSUTILITIES_API FMatrix ComputeViewCovarianceMatrix(TArrayView<const UE::Math::TVector<double>, int> PointView);

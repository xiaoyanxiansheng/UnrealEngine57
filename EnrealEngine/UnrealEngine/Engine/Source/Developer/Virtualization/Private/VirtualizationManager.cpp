...
void FVirtualizationManager::ApplySettingsFromConfigFiles(const FConfigFile& ConfigFile)
{
	...
	// Deprecated
	{
		...
		{
			UE_LOG(LogVirtualization, Error, TEXT("\tFilterEngineContent is now deprecated (engine content is never virtualized) please update before compatibility is removed in UE 5.9"));
		}
		...
		{
			UE_LOG(LogVirtualization, Error, TEXT("\tFilterEnginePluginContent is now deprecated (engine content is never virtualized) please update before compatibility is removed in UE 5.9"));
		}
	}
	...
	// Check for any legacy settings and print them out (easier to do this in one block rather than one and time)
	{
		...
		{
			UE_LOG(LogVirtualization, Error, TEXT("\tFound %d legacy ini file settings under [Core.ContentVirtualization] that should be moved to [Core.VirtualizationModule], please update before compatibility is removed in UE 5.9"), LegacyEntries.Num());
			for (const FString& LegacyEntry : LegacyEntries)
			{
				UE_LOG(LogVirtualization, Error, TEXT("\t\t%s"), *LegacyEntry);
			}
		}
	}
}

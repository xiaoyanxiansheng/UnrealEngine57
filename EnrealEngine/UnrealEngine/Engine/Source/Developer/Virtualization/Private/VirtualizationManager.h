 ...
 * AllowBackendReconnection [bool]:				When true backends that have previously failed to connect to their services can retry to connect
 *												each time they are accessed. When false the legacy behavior is invoked and backends may only attempt
 *												to connect a single time and after that will be unusable. [Default=true]
...
private:
	...
	/** When true backends will be able to try to reconnect to their service's if previous connection attempts failed */
	bool bAllowBackendReconnection;

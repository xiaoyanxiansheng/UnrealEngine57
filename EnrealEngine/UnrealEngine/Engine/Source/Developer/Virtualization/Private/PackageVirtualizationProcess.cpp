...
void VirtualizePackages(TConstArrayView<FString> PackagePaths, EVirtualizationOptions Options, FVirtualizationResult& OutResultInfo)
{
	...
	
	{
		...
		OutResultInfo.PreOperationSize = 0;
		OutResultInfo.PostOperationSize = 0;
		...
		for (const FPackageReplacement& PackageInfo : PackagesToReplace)
		{
			...
			if (IFileManager::Get().Move(*OriginalPackagePath, *NewPackagePath))
			{
				...
				OutResultInfo.PreOperationSize += OriginalSize;
				OutResultInfo.PostOperationSize += ReducedSize;
		...
		if (OutResultInfo.PreOperationSize != OutResultInfo.PostOperationSize)
		{
			UE_LOG(LogVirtualization, Display, TEXT("Total Reduction %s (%s -> %s)"),
				*FText::AsMemory(OutResultInfo.PreOperationSize - OutResultInfo.PostOperationSize).ToString(),
				*FText::AsMemory(OutResultInfo.PreOperationSize).ToString(),
				*FText::AsMemory(OutResultInfo.PostOperationSize).ToString());
		}

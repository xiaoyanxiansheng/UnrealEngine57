...
public:
	...
	UE_DEPRECATED(5.6, "Call IVirtualizationBackend::TryConnect instead")
	 ...
	 * 
	 * @praram bAllowReconnect	When true the backend will attempt to connect to it's services even
	 *							if it has previously failed to do so. When false the backend will
	 *							only ever attempt to connect a single time with that result being
	 *							final.
	 * 
	 * @return The current connection status of the backend. @see EConnectionStatus
	 */
	EConnectionStatus TryConnect(bool bAllowReconnect)
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(IVirtualizationBackend::TryConnect);

		UE::TUniqueLock _(ConnectionMutex);

		if ((bAllowReconnect && ConnectionStatus != EConnectionStatus::Connected) || (!bAllowReconnect && ConnectionStatus == EConnectionStatus::None))
		{
...
private:
	...
	/** Protects threaded access to 'ConnectionStatus' */
	mutable UE::FMutex ConnectionMutex;
	...
	/** The operations that have been disabled for debugging purposes */

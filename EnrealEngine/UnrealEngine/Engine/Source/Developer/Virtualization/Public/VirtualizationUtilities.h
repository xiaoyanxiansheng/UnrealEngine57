...
#define UE_API VIRTUALIZATION_API
 ...
 */
UE_API void PayloadIdToPath(const FIoHash& Id, FStringBuilderBase& OutPath);
 ...
 */
UE_API FString PayloadIdToPath(const FIoHash& Id);
 ...
 */
UE_API void GetFormattedSystemError(FStringBuilderBase& SystemErrorMessage);
 ...
 */
UE_API ETrailerFailedReason FindTrailerFailedReason(const FPackagePath& PackagePath);
 ...
 */
UE_API bool ExpandEnvironmentVariables(FStringView InputPath, FStringBuilderBase& OutExpandedPath);

/** 
 * Returns true if the process is interactive and we should display error dialogs to
 * the user.
 * */
UE_API bool IsProcessInteractive();

/**
 * Find the project file for the project that contains the provided package file. Note that this only intended to work for package files as it makes
 * several assumptions about the path conventions in order to speed up the search.
 * 
 * @PackagePath			Path of a package file to start searching from. The file must be stored under a /content/ directory to be considered valid.
 * @ProjectExtension	The extension that the project file may have.
 * @OutProjectFilePath	The project path will be written to this string if found.
 * @OutPluginFilePath	If the package is stored in a plugin then the path to it's '.uplugin' file will be written to this string.
 * 
 * @return				True if a project file is found otherwise it will return false.
 */
UE_API bool TryFindProject(const FString& PackagePath, const FString& ProjectExtension, FString& OutProjectFilePath, FString& OutPluginFilePath);
 ...
 *
 * @PackagePath			Path of a package file to start searching from. The file must be stored under a /content/ directory to be considered valid.
 * @ProjectExtensions	An array of potential extensions that the project file may have.
 * @OutProjectFilePath	The project path will be written to this string if found.
 * @OutPluginFilePath	If the package is stored in a plugin then the path to it's '.uplugin' file will be written to this string.
 *
 * @return				True if a project file is found otherwise it will return false.
 */
UE_API bool TryFindProject(const FString& PackagePath, TConstArrayView<FString> ProjectExtensions, FString& OutProjectFilePath, FString& OutPluginFilePath);

...
public:
	...
	virtual bool CreateAssetAsync(const FString& AssetName, const FString& PackagePath, UClass* AssetClass, UFactory* Factory, FAssetCreateComplete OnComplete = FAssetCreateComplete(), FAssetCreateCancelled OnCancelled = FAssetCreateCancelled(), FName CallingContext = NAME_None) override;
	virtual void CreateAssetsFrom(TConstArrayView<UObject*> SourceObjects, UClass* CreateAssetType, const FString& DefaultSuffix, TFunctionRef<UFactory*(UObject*)> FactoryConstructor, FName CallingContext) override;
	virtual UObject* CreateAssetWithDialog(UClass* AssetClass, UFactory* Factory, FName CallingContext = NAME_None) override;
	virtual UObject* CreateAssetWithDialog(const FString& AssetName, const FString& PackagePath, UClass* AssetClass, UFactory* Factory, FName CallingContext, const bool bCallConfigureProperties = true) override;
	virtual bool CreateAssetWithDialogAsync(const FString& AssetName, const FString& PackagePath, UClass* AssetClass, UFactory* Factory, FAssetCreateComplete OnComplete, FAssetCreateCancelled OnCancelled, FName CallingContext) override;
	virtual bool CreateAssetWithDialogAsync(UClass* AssetClass, UFactory* Factory, FAssetCreateComplete OnComplete = FAssetCreateComplete(), FAssetCreateCancelled OnCancelled = FAssetCreateCancelled(), FName CallingContext = NAME_None) override;
	...
	virtual const FString& GetUserFacingPluginName(const IPlugin& Plugin) const override;
	virtual FString GetUserFacingPluginName(FStringView PluginName) const override;
	virtual FString GetUserFacingLongPackagePath(FStringView LongPackagePath) const override;
	virtual FString GetUserFacingLongPackageName(FName PackageName) const override;
	virtual TArray<FString> GetUserFacingLongPackageNames(TConstArrayView<FName> PackageNames) const override;
	virtual FString GetUserFacingObjectPath(const FSoftObjectPath& ObjectPath) const override;
	virtual TArray<FString> GetUserFacingObjectPaths(TConstArrayView<FSoftObjectPath> ObjectPaths) const override;
	virtual FString GetUserFacingLongPackageName(const UPackage& Package) const override;
	virtual TArray<FString> GetUserFacingLongPackageNames(TConstArrayView<const UPackage*> Packages) const override;
	virtual FString GetUserFacingFullName(const UObject& Object) const override;
	virtual FString GetUserFacingPathName(const UObject& Object) const override;
	virtual FString GetUserFacingFullName(const FAssetData& AssetData) const override;
	virtual FString GetUserFacingPathName(const FAssetData& AssetData) const override;
...
private:
	...
	/** Called when async asset creation is completed successfully. */
	void CreateAssetAsync_OnComplete(UFactory* Factory, TStrongObjectPtr<UPackage> Package, UClass* AssetClass, FString AssetName, EObjectFlags Flags,
		FName CallingContext, FString PackageName, FAssetCreateComplete OnComplete, FAssetCreateCancelled OnCancelled);

	/** Called when async asset creation with a file location picker dialog is completed successfully. */
	void CreateAssetWithDialogAsync_OnComplete(UFactory* Factory, UClass* AssetClass, FName CallingContext, FString SaveObjectPath,

...
public:
	...
	void Construct(const FArguments& InArgs, const TArray<FRedirectorRefs>& RedirectorRefsList, bool InCanDelete)
	{
		...
		const UAssetToolsImpl& AssetTools = UAssetToolsImpl::Get();
		...
		for (const FRedirectorRefs& Redirector : RedirectorRefsList)
		{
			TSharedPtr<FFixupReportItem>& NewItem = TreeItems.Emplace_GetRef(MakeShared<FFixupReportItem>(
				...
				FText::FromString(Redirector.Redirector ? AssetTools.GetUserFacingPathName(*Redirector.Redirector) : AssetTools.GetUserFacingLongPackageName(Redirector.RedirectorPackageName))));
			{
				TArray<FString> LockedReferencers = AssetTools.GetUserFacingLongPackageNames(Redirector.LockedReferencerPackageNames);
				LockedReferencers.Sort();
				for (FString& LockedReferencer : LockedReferencers)
				{
					NewItem->Children.Emplace(MakeShared<FFixupReportItem>(
						EFixupReportItemType::LockedReferencer,
						FText::FromString(MoveTemp(LockedReferencer))));
				}
			}
			{
				TArray<FString> FailedReferencers = AssetTools.GetUserFacingLongPackageNames(Redirector.FailedReferencerPackageNames);
				FailedReferencers.Sort();
				for (FString& FailedReferencer : FailedReferencers)
				{
					NewItem->Children.Emplace(MakeShared<FFixupReportItem>(
						EFixupReportItemType::FailedReferencer,
						FText::FromString(MoveTemp(FailedReferencer))));
				}
			}
		...
		TreeItems.Sort([](const TSharedPtr<FFixupReportItem>& A, const TSharedPtr<FFixupReportItem>& B)
		{
			// Sort nodes with problems first.
			const bool bAHasProblems = A->Type != EFixupReportItemType::UnreferencedRedirector;
			const bool bBHasProblems = B->Type != EFixupReportItemType::UnreferencedRedirector;
			if (bAHasProblems != bBHasProblems)
			{
				return bAHasProblems;
			}
...
private:
	...
	FText GetDeleteExplanationText() const
	{
		return LOCTEXT("RedirectorDeleteExplanation", 
			"Delete all redirectors which are no longer referenced locally.\n\n"
	...
	FText GetKeepExplanationText() const
	{
		return LOCTEXT("RedirectorKeepExplanation", 
			"Keep all redirectors, both referenced and unreferenced.\n\n"
...
void FAssetFixUpRedirectors::ExecuteFixUp(TArray<TWeakObjectPtr<UObjectRedirector>> Objects, const bool bCheckoutDialogPrompt, ERedirectFixupMode FixupMode) const
{
	...
	if (bMayDeleteRedirectors && ISourceControlModule::Get().IsEnabled())
	{
		...
		{
			if (SourceControlProvider.Login() != ECommandResult::Succeeded)
			{
				// We have failed to connect to source control even though it is enabled. This is critical and we can not continue
		...
		if (SourceControlProvider.Execute(ISourceControlOperation::Create<FUpdateStatus>(), PackagesToAddToSCCUpdate) != ECommandResult::Succeeded)
		{
			// We have failed to update source control even though it is enabled. This is critical and we can not continue
			FMessageDialog::Open( EAppMsgType::Ok, NSLOCTEXT("UnrealEd", "SourceControl_UpdateFailed", "Revision Control update failed. Please check your connection and try again.") );
			return;
		}
	}

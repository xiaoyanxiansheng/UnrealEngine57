...
namespace UE::AssetTools::Private
{
	...
	public:
		...
		static void SaveInstancedPackagesIntoDestination(FPackageMigrationContext& PackageMigrationContext, FPackageMigrationImplContext& MigrationImplContext)
		{
			...
			for (const FPackageMigrationContext::FMigrationPackageData& MigrationPackageData : PackageMigrationContext.MigrationPackagesData)
			{
				...
				if (MigrationPackageData.bNeedToBeSaveMigrated)
				{
					...
					{
						PackageMigrationContext.AddErrorMigrationMessage(FText::Format(LOCTEXT("MigratePackages_SaveFailedNoAsset", "Couldn't migrate package ({0}) because the package didn't contain an asset. Destination File ({1})")
							, FText::FromString(MigrationPackageData.GetOriginalPackageName())
							, FText::FromString(MigrationPackageData.GetDestinationFilename())
						));
					}
		...
		static void CleanInstancedPackages(const TArray<FName>& PackagesToClean, FPackageMigrationContext& PackageMigrationContext)
		{
			...
			{
				if (UPackage* Package = FindObjectFast<UPackage>(nullptr, PackageName, EFindObjectFlags::ExactClass))
				{
...
UObject* UAssetToolsImpl::CreateAsset(const FString& AssetName, const FString& PackagePath, UClass* AssetClass, UFactory* Factory, FName CallingContext)
{
	...
	// This is the class default and this should not fail for all current factories.
	if (Factory && !EnumHasAnyFlags(Factory->GetSupportedWorkflows(), EFactoryCreateWorkflow::Default))
	{
...
bool UAssetToolsImpl::CreateAssetAsync(const FString& AssetName, const FString& PackagePath, UClass* AssetClass, UFactory* Factory,
	FAssetCreateComplete OnComplete, FAssetCreateCancelled OnCancelled, FName CallingContext)
{
	// Verify the factory class
	if (!ensure(AssetClass && Factory))
	{
		FMessageDialog::Open(EAppMsgType::Ok, LOCTEXT("MustSupplyClassOrFactory", "The new asset wasn't created due to a problem finding the appropriate factory or class for the new asset."));
		OnCancelled.ExecuteIfBound(Factory);
		return false;
	}

	if (!EnumHasAnyFlags(Factory->GetSupportedWorkflows(), EFactoryCreateWorkflow::Asynchronous))
	{
		FMessageDialog::Open(EAppMsgType::Ok, UE::Private::GetInvalidFactoryMessage());
		OnCancelled.ExecuteIfBound(Factory);
		return false;
	...
	// Verify the factory supports the asset class
	if (((Factory->SupportedClass != nullptr && !ensure(AssetClass->IsChildOf(Factory->GetSupportedClass()))) ||
			(Factory->SupportedClass == nullptr && !ensure(Factory->DoesSupportClass(AssetClass)))))
	{
		FMessageDialog::Open(EAppMsgType::Ok, UE::Private::GetInvalidFactoryMessage());
		OnCancelled.ExecuteIfBound(Factory);
	...
	TStrongObjectPtr<UPackage> DontGCPackage(Pkg);

	return Factory->ConfigurePropertiesAsync(
		FFactoryConfigurePropertiesComplete::CreateUObject(this, &UAssetToolsImpl::CreateAssetAsync_OnComplete, DontGCPackage, ClassToUse, 
			AssetName, Flags, CallingContext, PackageName, OnComplete, OnCancelled),
		FFactoryConfigurePropertiesCancelled()
	);
}
...
void UAssetToolsImpl::CreateAssetAsync_OnComplete(UFactory* Factory, TStrongObjectPtr<UPackage> Package, UClass* AssetClass, FString AssetName,
	EObjectFlags Flags, FName CallingContext, FString PackageName, FAssetCreateComplete OnComplete, FAssetCreateCancelled OnCancelled)
{
	UPackage* Pkg = Package.Get();
	UObject* NewObj = Factory->FactoryCreateNew(AssetClass, Pkg, FName(*AssetName), Flags, nullptr, GWarn, CallingContext);
	...
	if (NewObj)
	{
		...
		OnComplete.ExecuteIfBound(Factory, NewObj);
	}
	else
	{
		OnCancelled.ExecuteIfBound(Factory);
...
bool UAssetToolsImpl::CreateAssetWithDialogAsync(UClass* AssetClass, UFactory* Factory, FAssetCreateComplete OnComplete,
	FAssetCreateCancelled OnCancelled, FName CallingContext)
{
	if (!Factory)
	{
		OnCancelled.ExecuteIfBound(Factory);
	...
	return CreateAssetWithDialogAsync(AssetName, AssetPath, AssetClass, Factory, OnComplete, OnCancelled, CallingContext);
}

bool UAssetToolsImpl::CreateAssetWithDialogAsync(const FString& AssetName, const FString& PackagePath, UClass* AssetClass, UFactory* Factory,
	FAssetCreateComplete OnComplete, FAssetCreateCancelled OnCancelled, FName CallingContext)
{
	if (!Factory)
	{
	...
	return Factory->ConfigurePropertiesAsync(
		FFactoryConfigurePropertiesComplete::CreateUObject(this, &UAssetToolsImpl::CreateAssetWithDialogAsync_OnComplete,
			AssetClass, CallingContext, SaveObjectPath, OnComplete, OnCancelled),
		FFactoryConfigurePropertiesCancelled()
	);
}

void UAssetToolsImpl::CreateAssetWithDialogAsync_OnComplete(UFactory* Factory, UClass* AssetClass, FName CallingContext, FString SaveObjectPath,
	FAssetCreateComplete OnComplete, FAssetCreateCancelled OnCancelled)
{
	const FString SavePackageName = FPackageName::ObjectPathToPackageName(SaveObjectPath);
	const FString SavePackagePath = FPaths::GetPath(SavePackageName);
	const FString SaveAssetName = FPaths::GetBaseFilename(SavePackageName);
	...
	if (UObject* NewObj = CreateAsset(SaveAssetName, SavePackagePath, AssetClass, Factory, CallingContext))
	{
		OnComplete.ExecuteIfBound(Factory, NewObj);
	}
	else
	...
	EMessageSeverity::Type NotificationSeverityFilter) const
{
	...
	if (ValidateFlattenedAdvancedCopyDestinations(SourceAndDestPackages))
	{
		...
		if (UE::AssetTools::Private::bEnableHeaderPatching)
		{
			...
			FAssetHeaderPatcher::FPatchAsyncParams PatchParams;
			PatchParams.OutNumFilesToPatch = &NumFilesToPatch;
			PatchParams.OutNumFilesPatched = &PatchAssetsCompletedCount;
			PatchParams.TaskPriority = UE::Tasks::ETaskPriority::BackgroundHigh;

			UE::Tasks::FTask PatchAssetsCompletionTask = Patcher.PatchAsync(PatchParams);
...
TArray<UObject*> UAssetToolsImpl::ImportAssetsInternal(const TArray<FString>& Files, const FString& RootDestinationPath, TArray<TPair<FString, FString>> *FilesAndDestinationsPtr, const FAssetImportParams& Params) const
{
	...
	bool bShowSlowTask = !bUseInterchangeFramework;
	...
	if(bUseInterchangeFramework)
	{
		...
		{
			//Do not show the slow task if we are only using the Interchange Manager for imports
			bShowSlowTask = false;
		}
	}

	// Create a slow task to document progress
	FScopedSlowTask SlowTask(static_cast<float>(ValidFiles.Num()), LOCTEXT("ImportSlowTask", "Importing"), bShowSlowTask);

	if (bShowSlowTask && ValidFiles.Num() > 1)
	{
				...
				TFunction<void(UE::Interchange::FImportResult&)> AppendAndBroadcastImportResultIfNeeded =
					...
					[bSceneImport = Params.bSceneImport, ImportStatus, AppendImportResult, bSyncToBrowser](UE::Interchange::FImportResult& Result)
					{
						...
						if (--ImportStatus->ImportCount == 0)
						{
							...
							const IConsoleVariable* Var = IConsoleManager::Get().FindConsoleVariable(TEXT("Interchange.FeatureFlags.Import.SyncToBrowser")); // Declared in InterchangeEditorModule.cpp

							// If the CVar is found, it overrides the passed value.
							const bool bResolvedSyncToBrowser = Var ? Var->GetBool() : bSyncToBrowser;
							
							if (bResolvedSyncToBrowser)
							{
...
void UAssetToolsImpl::InitAdvancedCopyFromCopyParams(FAdvancedCopyParams CopyParams) const
{
	...
	else
	{
		...
		SAdvancedCopyReportDialog::OpenPackageReportDialog(CopyParams, CopyParams.GetDropLocationForAdvancedCopy(), CompleteDestinationMap, CompleteDependencyMap, OnReportConfirmed);
	}
}
...
TArray<UFactory*> UAssetToolsImpl::GetNewAssetFactories() const
{
	...
	{
		UClass* Class = *It;
		if (!Class->IsChildOf(UFactory::StaticClass()) || Class->HasAnyClassFlags(CLASS_Abstract) || Class->HasAnyClassFlags(CLASS_Deprecated))
		{
			continue;
		}
...
TArray<UObject*> UAssetToolsImpl::ImportAssetsWithDialogImplementation(const FString& DestinationPath, bool bAllowAsyncImport)
{
	...
	if (bOpened)
	{
		if (OpenFilenames.Num() > 0)
		{
			...
			// A filter may have been chosen but most likely for convenience, let ImportAssets decipher that.
			if (!UInterchangeManager::IsInterchangeImportEnabled() && FilterIndex > 0)
			{
...
const FString& UAssetToolsImpl::GetUserFacingPluginName(const IPlugin& Plugin) const
{
	if (!Plugin.GetVersePath().IsEmpty() && ShowingContentVersePath())
	{
		return Plugin.GetVersePath();
	}

	return Plugin.GetFriendlyName();
}

FString UAssetToolsImpl::GetUserFacingPluginName(FStringView PluginName) const
{
	if (TSharedPtr<IPlugin> Plugin = IPluginManager::Get().FindPlugin(PluginName))
	{
		return GetUserFacingPluginName(*Plugin);
	}

	return FString(PluginName);
}

FString UAssetToolsImpl::GetUserFacingLongPackagePath(FStringView LongPackagePath) const
{
	if (!LongPackagePath.IsEmpty() && ShowingContentVersePath())
	{
		UE::Core::FVersePath VersePath = FPackageName::LongPackagePathToVersePath(LongPackagePath);
		if (VersePath.IsValid())
		{
	...
	return FString(LongPackagePath);
}

FString UAssetToolsImpl::GetUserFacingLongPackageName(FName PackageName) const
{
	if (!PackageName.IsNone() && ShowingContentVersePath())
	{
		TMap<FName, FAssetData> Assets;
		UE::AssetRegistry::GetAssetForPackages(TConstArrayView<FName>(&PackageName, 1), Assets);

		if (const FAssetData* AssetData = Assets.Find(PackageName))
		{
...
TArray<FString> UAssetToolsImpl::GetUserFacingLongPackageNames(TConstArrayView<FName> PackageNames) const
{
	...
	TArray<FString> UserFacingPaths;
	UserFacingPaths.Reserve(PackageNames.Num());
		...
		for (const FName& PackageName : PackageNames)
		{
			if (!PackageName.IsNone())
			{
				if (const FAssetData* AssetData = Assets.Find(PackageName))
				{
					...
					if (VersePath.IsValid())
					{
						UserFacingPaths.Add(MoveTemp(VersePath).ToString());
						continue;
					}
				}
			}

			UserFacingPaths.Add(PackageName.ToString());
		}
	}
	else
	{
	...
	return UserFacingPaths;
}

FString UAssetToolsImpl::GetUserFacingObjectPath(const FSoftObjectPath& ObjectPath) const
{
	// If the object path is just a package name, get the path for the package.
	if (ObjectPath.IsAsset() && ObjectPath.GetAssetFName().IsNone())
	{
		return GetUserFacingLongPackageName(ObjectPath.GetLongPackageFName());
	}
	...
	if (ShowingContentVersePath())
	{
		FAssetRegistryModule& AssetRegistryModule = FModuleManager::LoadModuleChecked<FAssetRegistryModule>(TEXT("AssetRegistry"));
		FAssetData AssetData;
		if (AssetRegistryModule.Get().TryGetAssetByObjectPath(ObjectPath, AssetData) == UE::AssetRegistry::EExists::Exists)
		{
			UE::Core::FVersePath VersePath = AssetData.GetVersePath();
			if (VersePath.IsValid())
			{
...
TArray<FString> UAssetToolsImpl::GetUserFacingObjectPaths(TConstArrayView<FSoftObjectPath> ObjectPaths) const
{
	...
	TArray<FString> UserFacingPaths;
	UserFacingPaths.Reserve(ObjectPaths.Num());

	if (ShowingContentVersePath())
	{
		FAssetRegistryModule& AssetRegistryModule = FModuleManager::LoadModuleChecked<FAssetRegistryModule>(TEXT("AssetRegistry"));

		// Get the primary asset for any object paths that are just package names.
		TMap<FName, FAssetData> Assets;
		{
			...
			for (const FSoftObjectPath& ObjectPath : ObjectPaths)
			{
				if (ObjectPath.IsAsset() && ObjectPath.GetAssetFName().IsNone())
				{
					PackageNames.Add(ObjectPath.GetLongPackageFName());
				}
			}
		...
		for (const FSoftObjectPath& ObjectPath : ObjectPaths)
		{
			...
			// If the object path is just a package name, get the Verse path for the primary asset.
			if (ObjectPath.IsAsset() && ObjectPath.GetAssetFName().IsNone())
			{
				if (const FAssetData* AssetData = Assets.Find(ObjectPath.GetLongPackageFName()))
				{
			...
			{
				UserFacingPaths.Add(ObjectPath.ToString());
			}
		}
	}
	else

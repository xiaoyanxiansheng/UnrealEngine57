...
bool FAssetRenameManager::FixReferencesAndRename(const TArray<FAssetRenameData>& AssetsAndNames, bool bAutoCheckout, bool bWithDialog, FScopedSlowTask& RenamingSlowTask) const
{
	...
	if (bSoftReferencesOnly || UpdatePackageStatus(AssetsToRename))
	{
		...
		if (bUserAcceptedCheckout || bSoftReferencesOnly)
		{
			...
			// Make public any private asset that will be referenced from another plugin after the rename.
...
bool FAssetRenameManager::UpdatePackageStatus(TArray<FAssetRenameDataWithReferencers>& AssetsToRename) const
{
	...
	if (ISourceControlModule::Get().IsEnabled())
	{
		...
		// Update the source control server availability if necessary to make sure we can do the rename operation
...
void FAssetRenameManager::SetupPublicAssets(TArray<FAssetRenameDataWithReferencers>& AssetsToRename) const
{
	...
	for (FAssetRenameDataWithReferencers& RenameData : AssetsToRename)
	{
		...
		const FName OldPackageName = RenameData.OldObjectPath.GetLongPackageFName();
		const FName NewPackageName = RenameData.NewObjectPath.GetLongPackageFName();
		const FNameBuilder OldPackageNameBuilder(OldPackageName);
		const FNameBuilder NewPackageNameBuilder(NewPackageName);
		const FStringView OldPackageMountPoint = FPathViews::GetMountPointNameFromPath(OldPackageNameBuilder);
		const FStringView NewPackageMountPoint = FPathViews::GetMountPointNameFromPath(NewPackageNameBuilder);
					...
					if (AssetRegistry.GetAssetsByPackageName(DependencyPackageName, DependencyAssetDatas) && !DependencyAssetDatas.IsEmpty())
					{
						if (DependencyAssetDatas[0].GetAssetAccessSpecifier() == EAssetAccessSpecifier::Private)
						{
							...
							RenameData.FailureReason = FText::Format(
								...
								FText::FromString(AssetTools.GetUserFacingPluginName(NewPackageMountPoint)),
								FText::FromString(AssetTools.GetUserFacingLongPackageName(DependencyPackageName)));
							break;
						}
					}
				}
		...
		// If the asset isn't private, it can be referenced externally so we're good
		if (Package->GetAssetAccessSpecifier() != EAssetAccessSpecifier::Private)
		{
			continue;
		}
		...
		FName ReferencingAssetInDifferentMountPoint;
		if (RenameData.bCreateRedirector)
		{
			ReferencingAssetInDifferentMountPoint = OldPackageName;
		}
		if (ReferencingAssetInDifferentMountPoint.IsNone())
		{
			for (FName It : RenameData.NotRenamedReferencingPackageNames)
			{
				...
				{
					ReferencingAssetInDifferentMountPoint = It;
					break;
				}
			}
		}
		if (ReferencingAssetInDifferentMountPoint.IsNone())
		{
			for (const TPair<FName, FName>& It : RenameData.RenamedReferencingPackageNames)
			{
				...
				FName ReferencingPackageName;
				if (ReferencingRenameData.bRenameFailed || ReferencingRenameData.bOnlyFixSoftReferences)
				{
					ReferencingPackageName = OldReferencingPackageName;
				}
				else
				{
					ReferencingPackageName = NewReferencingPackageName;
				}

				const FNameBuilder ReferencingPackageNameBuilder(ReferencingPackageName);
				const FStringView ReferencingPackagMountPoint = FPathViews::GetMountPointNameFromPath(ReferencingPackageNameBuilder);
			...
			else
			{
				...
				if (RenameData.bCreateRedirector)
				{
					RenameData.FailureReason = FText::Format(
						...
						FText::FromStringView(AssetTools.GetUserFacingPluginName(NewPackageMountPoint)));
				}
				else
				{
					RenameData.FailureReason = FText::Format(
						...
						FText::FromStringView(AssetTools.GetUserFacingPluginName(NewPackageMountPoint)),
						FText::FromString(AssetTools.GetUserFacingLongPackageName(ReferencingAssetInDifferentMountPoint)));
				}
			}
		}
	}
	...
	for (const FAssetRenameDataWithReferencers& RenameData : AssetsToRename)
	{
		if (RenameData.bRenameFailed)
		{
			...
			if (Asset)
			{
				...
				{
					AssetName = MoveTemp(VersePath).AsText();
				}
				else
				{
					AssetName = FText::FromName(Asset->GetOutermost()->GetFName());
				}

				FFormatNamedArguments Args;
				Args.Add(TEXT("FailureReason"), RenameData.FailureReason);
				Args.Add(TEXT("AssetName"), MoveTemp(AssetName));

...
TSharedPtr<SWidget> FFilterService::CreateFilterConfiguratorWidget(TSharedPtr<FFilterConfigurator> FilterConfiguratorViewModel, TWeakPtr<SWidget> ParentWidget)
{
	SAssignNew(PendingWidget, SAdvancedFilter, FilterConfiguratorViewModel);
	PendingParentWidget = ParentWidget;
...
TSharedRef<SDockTab> FFilterService::SpawnTab(const FSpawnTabArgs& Args)
{
	...
	TSharedPtr<SWidget> ParentWidget = PendingParentWidget.Pin();
	const TSharedPtr<SWindow>& OwnerWindow = Args.GetOwnerWindow();
	if (OwnerWindow.IsValid() && ParentWidget.IsValid() && OwnerWindow != FSlateApplication::Get().FindWidgetWindow(ParentWidget.ToSharedRef()))
	{

...
FTreeNodeGrouping::FTreeNodeGrouping()
...
void FTreeNodeGrouping::GroupNodes(const TArray<FTableTreeNodePtr>& Nodes, FTableTreeNode& ParentGroup, TWeakPtr<FTable> InParentTable, IAsyncOperationProgress& InAsyncOperationProgress) const
{
	...
	for (FTableTreeNodePtr NodePtr : Nodes)
	{
		...
		FTableTreeNodePtr* GroupPtrPtr = GroupMap.Find(GroupInfo.GetName());
		if (!GroupPtrPtr)
		{
			if (GroupInfo.IsCustom())
			{
				const FSlateBrush* GroupIcon = GroupInfo.GetIcon() != nullptr ? GroupInfo.GetIcon() : FBaseTreeNode::GetDefaultIcon(true);
				GroupPtr = MakeShared<FCustomTableTreeNode>(GroupInfo.GetName(), InParentTable, GroupIcon, GroupInfo.GetIconColor(), GroupInfo.GetColor());
			}
			else
			{
				GroupPtr = MakeShared<FTableTreeNode>(GroupInfo.GetName(), InParentTable);
			}
			GroupPtr->SetExpansion(GroupInfo.IsExpanded());
			ParentGroup.AddChildAndSetParent(GroupPtr);
			GroupMap.Add(GroupInfo.GetName(), GroupPtr);
		}
		else
		{
...
FLinearColor FTreeNodeGrouping::MakeDistinctColor(const FName& InName)
{
	uint32 Seed = GetTypeHash(InName.ToString());
	return FLinearColor(static_cast<float>(Seed % 3600) / 10.0f, 0.7f, 1.0f, 1.0f).HSVToLinearRGB();
}
...
void FTreeNodeGroupingFlat::GroupNodes(const TArray<FTableTreeNodePtr>& Nodes, FTableTreeNode& ParentGroup, TWeakPtr<FTable> InParentTable, IAsyncOperationProgress& InAsyncOperationProgress) const
{
	...
	FTableTreeNodePtr GroupPtr = MakeShared<FCustomTableTreeNode>(FName(TEXT("All")), InParentTable, GetColor());
...
{
	SetColor(FTreeNodeGrouping::MakeDistinctColor(InColumnRef->GetId()));
}
...
FTreeNodeGroupInfo FTreeNodeGroupingByUniqueValue::GetGroupForNode(const FBaseTreeNodePtr InNode) const
{
	...
	return FTreeNodeGroupInfo(FName(GroupName, 0), false, GetColor());
}
			...
			else
			{
				...
				while (true)
				{
					if (*End == TEXT('/') || *End == TEXT('\\') || *End == TEXT('\0'))
					{
						...
						{
							FTableTreeNodePtr GroupPtr = MakeShared<FCustomTableTreeNode>(DirName, InParentTable, GetColor());
							GroupPtr->SetExpansion(false);

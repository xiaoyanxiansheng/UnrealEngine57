...
{
	INSIGHTS_DECLARE_RTTI_BASE(FBaseTreeNode, UE_API)
...
public:
	 ...
	 */
	UE_API virtual const FText GetDisplayName() const;
	 ...
	 */
	UE_API virtual const FText GetExtraDisplayName() const;

	/**
	 * @returns true if the node has an extra display name (a name suffix to display in tree view).
	 */
	UE_API virtual bool HasExtraDisplayName() const;
	 ...
	 */
	static UE_API const FSlateBrush* GetDefaultIcon(bool bIsGroupNode);
	 ...
	 */
	static UE_API FLinearColor GetDefaultIconColor(bool bIsGroupNode);
	 ...
	 * @returns the default color tint for a group or leaf node.
	 */
	static UE_API FLinearColor GetDefaultColor(bool bIsGroupNode);
	...
	UE_API void SortChildren(const ITableCellValueSorter& Sorter, ESortMode SortMode);
	...
	UE_API void SortFilteredChildren(const ITableCellValueSorter& Sorter, ESortMode SortMode);
...
private:
	...
	static UE_API FGroupNodeData DefaultGroupData;
};

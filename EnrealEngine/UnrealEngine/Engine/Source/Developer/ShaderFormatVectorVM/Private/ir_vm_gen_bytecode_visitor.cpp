...
struct op_external_func : public op_base
{
	...
	virtual void add_to_bytecode(TArray<uint8>& bytecode)
	{
		...
		check(function_index < std::numeric_limits<uint16>::max());
		emit_short(function_index, bytecode);
	...
	virtual bool finalize(_mesa_glsl_parse_state* parse_state, FVectorVMCompilationOutput&CompilationOutput, TArray<uint32>& temp_registers, unsigned op_idx)
	{
		...
		// we're also reserving one slot for -1 as an invalid index
		constexpr unsigned int MaxExternalFunctionCount = std::numeric_limits<uint16>::max() - 1;
		if (function_index >= MaxExternalFunctionCount)
		{
			_mesa_glsl_error(parse_state, "Exceeded maximum number (%d) of available external function calls.", MaxExternalFunctionCount);
			return false;
		}

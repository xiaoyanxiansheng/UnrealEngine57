...
FReply SSettingsEditor::HandleExportButtonClicked()
{
		...
		OutSaveFiles))
	{
		...
		{
			ShowNotification(LOCTEXT("ExportSettingsExistingFileFailure", "Export settings failed. Cannot use a file that is already part of the config system."), SNotificationItem::CS_Fail);
		...
		if (FPaths::DirectoryExists(TempDestinationFolder))
		{
			...
			{
				ShowNotification(LOCTEXT("ExportSettingsExistingTempFolderFailure", "Export settings failed. TempExport folder already exists under project Saved folder."), SNotificationItem::CS_Fail);
				return FReply::Handled();
			}
		}
		...
		for (const ISettingsCategoryPtr& Category : Categories)
		{
			...
			for (const ISettingsSectionPtr& Section : Sections)
			{
				...
				TempBranch.InMemoryFile.Combine(TempDestinationFilePath);

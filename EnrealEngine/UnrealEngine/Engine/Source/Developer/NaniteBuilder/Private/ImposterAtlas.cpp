...
void FImposterAtlas::Rasterize( const FIntPoint& TilePos, const FCluster& Cluster, uint32 ClusterIndex )
{
	...
	Positions.SetNum( Cluster.Verts.Num(), EAllowShrinking::No );

	for( uint32 VertIndex = 0; VertIndex < Cluster.Verts.Num(); VertIndex++ )
	{
		FVector3f Position = Cluster.Verts.GetPosition( VertIndex );

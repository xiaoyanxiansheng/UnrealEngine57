	...
	ParallelFor(TEXT("Nanite.BuildRayTracingData.PF"), Clusters.Num(), 1, [&](uint32 ClusterIndex)
		{
			...
			for (uint32 TriIndex = 0; TriIndex < Cluster.NumTris; TriIndex++)
			{
				...
				const FVector3f& v0 = Cluster.Verts.GetPosition(Cluster.Indexes[TriIndex * 3 + 0]);
				const FVector3f& v1 = Cluster.Verts.GetPosition(Cluster.Indexes[TriIndex * 3 + 1]);
				const FVector3f& v2 = Cluster.Verts.GetPosition(Cluster.Indexes[TriIndex * 3 + 2]);

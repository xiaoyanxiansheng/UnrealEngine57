...
static inline void AdjustColorsNew(FLinearColor* Colors, int64 Count, const FTextureBuildSettings& InBuildSettings)
{
	...
	for (int64 i=0; i<Count; i++)
	{
		...
		if (!bHDRSource)
		{
			...
			// A will be clamped later when used
		}
		else
		{			
		...
		// Convert to HSV
		const FLinearColor OriginalHSVColor = OriginalColor.LinearRGBToHSV();
		float PixelHue = OriginalHSVColor.R;
		float PixelSaturation = OriginalHSVColor.G;
		float PixelValue = OriginalHSVColor.B;
		...
		// Convert back to a linear color
		FLinearColor HSVColor(PixelHue,PixelSaturation,PixelValue);
		// HSVToLinearRGB does the clamping/fmod for you
		FLinearColor LinearColor = HSVColor.HSVToLinearRGB();

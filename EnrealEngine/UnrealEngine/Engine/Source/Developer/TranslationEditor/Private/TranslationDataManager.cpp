...
bool FTranslationDataManager::WriteTranslationData(bool bForceWrite /*= false*/)
{
	...
	if (ArchivePtr.IsValid())
	{
		...
		{
			FString FinalArchiveJson;
			FJsonInternationalizationArchiveSerializer::SerializeArchiveToString(Archive, FinalArchiveJson);

			bSuccess = WriteJSONToTextFile(FinalArchiveJson, OpenedArchiveFilePath);
		}
	}
...
bool FTranslationDataManager::WriteJSONToTextFile(const FString& Output, const FString& Filename)
{
	...
	if ( !Filename.IsEmpty() )
	{
		...
		if( CheckoutAndSaveWasSuccessful )
		{
			...
			if (!FFileHelper::SaveStringToFile(Output, *Filename, FFileHelper::EEncodingOptions::ForceUnicode))
			{

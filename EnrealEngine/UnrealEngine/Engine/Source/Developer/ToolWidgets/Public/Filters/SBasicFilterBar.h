...
private:
	...
	public:
		...
		void SetOnFilterAltClicked(const FOnClicked& NewFilterAltClicked)
		{
			OnFilterAltClicked = NewFilterAltClicked;
		}
		...
		virtual FReply OnMouseButtonUp( const FGeometry& InMyGeometry, const FPointerEvent& InMouseEvent ) override
		{
			...
			bIsPressed = false;
			return EventReply.ReleaseMouseCapture();
		}
...
protected:
	...
	void CreateCustomTextFilterWindow(const FCustomTextFilterData& CustomTextFilterData, TSharedPtr<ICustomTextFilter<FilterType>> InFilter)
	{
		...
		TSharedPtr<SCustomTextFilterDialog> CustomTextFilterDialog =
			...
			.InEnableColorEditing(FilterPillStyle != EFilterPillStyle::Basic)
	...
	void CreateTextFiltersMenu(UToolMenu* InMenu)
	{
		...
		for (const TSharedRef<ICustomTextFilter<FilterType>>& Filter : CustomTextFilters)
		{
			...
			TSharedPtr<SButton> CustomTextFilterWidget = 
				SNew(SButton)
				...
				[
					...
					// A text block showing the filter name
					+ SHorizontalBox::Slot()
					.FillWidth(1.0)
					.Padding(4, 0, 0, 0)
					.VAlign(VAlign_Center)

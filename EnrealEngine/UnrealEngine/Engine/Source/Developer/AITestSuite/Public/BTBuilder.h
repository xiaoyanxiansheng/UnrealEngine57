...
struct FBTBuilder
{
	...
	static T& WithDecorator(UBTCompositeNode& ParentNode, UClass* DecoratorClass = T::StaticClass())
	{
		...
		// Push additional memory validation decorator for extra coverage
		ParentNode.Children.Last().Decorators.Add(NewObject<UTestBTDecorator_NodeMemoryValidator>(ParentNode.GetTreeAsset()));
	...
	static void WithDecoratorLoopUntil(UBTCompositeNode& ParentNode, EBTNodeResult::Type RequiredResult)
	{
		UTestBTDecorator_LoopUntil& LoopUntilDecorator = WithDecorator<UTestBTDecorator_LoopUntil>(ParentNode);
		LoopUntilDecorator.SetRequiredResult(RequiredResult);
	}

...
# $1 parameter should be the profile file
updateProfileFile()
{
	if [ ! -f "${1}" ]; then
		touch "${1}"
...
# $1 parameter should be the profile contents to remove
# $2 parameter should be the profile file
undoProfileFile()
{
	if [ ! -f "${2}" ]; then
		return 0
...
END {
	...
	if (i != 0)
	{
		data = substr(data, 1, i - 1) substr(data, i + length(text))
		printf "%s", data > out
		exit close(out)
	}
	...
	if awk -v text="${1}" -v out="${2}" "${code}" "${2}"; then
		echo "Removed UE Android developer environment variables from ${2}"

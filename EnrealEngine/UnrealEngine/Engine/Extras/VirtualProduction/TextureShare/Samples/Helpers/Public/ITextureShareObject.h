...
public:
	/** Return resource request based on input texture
	* @param InResourceDesc - resource descriptor
	* @param InTexture      - The desired size and format will be obtained from there
	* @param InTextureGamma - Desired gamma on this side. Default -1 use gamma from UE
	*/
	static FTextureShareCoreResourceRequest GetResourceRequest(
		const FTextureShareCoreResourceDesc& InResourceDesc,
		const ITextureShareImage& InTexture,
		const float InTextureGamma = -1.0f);

...
{
	bool bEndFrameResult = true;

	if (TextureShareSDKObject->IsFrameSyncActive_RenderThread())
	{
		// Flush deffered sync steps
		if (!DefferedResourceSync_RenderThread(ETextureShareSyncStep::FrameProxyFlush))
		{
			bEndFrameResult = false;
		}
	}

	// Always call EndFrameSync_RenderThread().
	if (!TextureShareSDKObject->EndFrameSync_RenderThread())
	{
		bEndFrameResult = false;
	}

	return bEndFrameResult;
}
...
bool FTextureShareObject::DefferedResourceSync_RenderThread(const ETextureShareSyncStep InSyncStep)
{
...
bool FTextureShareObject::ResourceSync_RenderThread(const FTextureShareCoreResourceDesc& InResourceDesc)
{
	...
	if (!DefferedResourceSync_RenderThread(InSyncStep))
	{
		return false;
	}

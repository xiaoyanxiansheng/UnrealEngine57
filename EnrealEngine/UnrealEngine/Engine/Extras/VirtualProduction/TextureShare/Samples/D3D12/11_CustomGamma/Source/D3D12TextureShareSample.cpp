...
FD3D12TextureShareSample::FD3D12TextureShareSample(ID3D12Resource* InBackBufferTexture)
{
	...
	if(TextureShareObject = ITextureShareObject::CreateInstance(TextureShareSample::ObjectDesc))
	{
		...
		{
			FTextureShareCoreResourceRequest SendBackbufferRequest = ITextureShareObject::GetResourceRequest(
				TextureShareSample::Send::Backbuffer::Desc,
				FTextureShareImageD3D12(InBackBufferTexture),
				TextureShareSample::DisplayGamma);

			TextureShareObject->GetData().ResourceRequests.Add(SendBackbufferRequest);
		}
	}
}

...
class UnrealActorCreator {
  ...
  void createLightcard([ActorCreationSettings settings = ActorCreationSettings.defaults]) async {
    ...
    createStageActor(className: lightCardClassName, settings: settings);
  ...
  void createChromakeyCard([ActorCreationSettings settings = ActorCreationSettings.defaults]) {
    ...
    createStageActor(className: chromakeyCardClassName, settings: settings);
  ...
  /// Send a message to the engine requesting that a new CCR be created.
  void createColorCorrectionRegion([ActorCreationSettings settings = ActorCreationSettings.defaults]) async {
    if (settings.name == null) {
      settings = settings.copyWith(name: settings.name ?? _localizations.newActorNameColorCorrectRegion);
    }

    createStageActor(
      // colorCorrectRegionClassName (ColorCorrectRegion) is an abstract class, so we need to use the placeable class
      // name here instead
      className: '/Script/ColorCorrectRegions.ColorCorrectionRegion',
      settings: settings.copyWith(
        mapPosition: settings.mapPosition ?? Offset(0.5, 0.5),
      ),
    );
    ...
    _recentActorSettings.addRecentlyPlacedActor(RecentlyPlacedActorData(
      type: RecentlyPlacedActorType.colorCorrectRegion,
    ));
  }

  /// Send a message to the engine requesting that a new stage actor be created.
  void createStageActor({

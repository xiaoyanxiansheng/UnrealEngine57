...
def _get_prompt(system, line_iter, reply):
    ...
    if title := (os.getenv("FLOW_TITLE") or getattr(context, "TITLE", None)):
        ...
        print(f"\x1b]0;{title} - ushell\x07", file=sys.stderr)

    print("\x1b[0 q", file=sys.stderr) # cursor reset
...
class Complete(flow.cmd.Cmd):
    ...
    command = flow.cmd.Arg("", "The initiating command")
    words   = flow.cmd.Arg([str], "Follow on words to parse and print completion for")
    ...
    def main(self):
        ...
        words = tuple()
        if command := self.args.command:
            words = (command, *self.args.words)

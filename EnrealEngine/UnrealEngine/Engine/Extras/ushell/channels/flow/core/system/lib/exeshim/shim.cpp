...
Vt100Scope::Vt100Scope()
{
    ...
    if (!SetConsoleMode(handle, _con_mode))
        return;
...
Vt100Scope::~Vt100Scope()
{
    ...
    const wchar_t restore[] =
        L"\x1b[0 q"             // restore cursor to user's preference
        L"\x1b" L"1049l"        // switch back to main buffer
        L"\x1b]9;4;0;50\x07"    // disables progress bar
        ;
    WriteFile(_stdout_handle, restore, sizeof(restore) - sizeof(restore[0]), &unused, nullptr);

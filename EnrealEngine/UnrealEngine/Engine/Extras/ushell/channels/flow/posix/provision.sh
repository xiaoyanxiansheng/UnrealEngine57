...
function _bin_select()
{
    ...
    if command -v uv; then
        for candidate in `uv python find --no-project`; do
            if ! _check_bin $candidate; then
                continue
            fi
...
if [ -z "$py_bin" ]; then
    _error "No suitable binary found that is $py_ver (or better)"
    echo ""

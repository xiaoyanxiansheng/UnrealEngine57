...
class Workspace(flow.cmd.Cmd):
    ...
    def main(self):
        ...
        if not self.args.dryrun:
            print("Creating...", end="")
            P4(u=username).client(i=True).run(input_data=client_spec)
            print(" done")
        ...
        if not self.args.dryrun:
            ...
            with p4config_path.open("wt") as out:
                ...
                print("P4USER=", username, sep="", file=out)
                if port := p4utils.get_p4_set("P4PORT"):
                    print("P4PORT=", port, sep="", file=out)

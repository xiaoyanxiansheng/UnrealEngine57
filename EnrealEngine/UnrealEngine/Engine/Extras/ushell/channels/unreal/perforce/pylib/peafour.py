...
class _P4Result(object):
    ...
    def __contains__(self, key):
        key = key.encode()
        return (key in self._result) or (key + b"0" in self._result)
...
class _P4Command(object):
    ...
    def _iter(self, input_data=None, on_error=True, on_info=None, on_text=None):
        ...
        if stdin:
            stdin_thread = threading.Thread(target=_stdin_writethread)
        ...
        def _stdout_readthread():
            try:
                while proc.poll() is None:
                    try:
                        result = marshal.load(proc.stdout)
                        q.put(result)
                    except (EOFError, ValueError):
                        # the sub-process can close the stdout file before the
                        # process has terminated, which will raise a ValueError
                        break
            finally:
                q.put(_EOF)

        stdout_thread = threading.Thread(target=_stdout_readthread)

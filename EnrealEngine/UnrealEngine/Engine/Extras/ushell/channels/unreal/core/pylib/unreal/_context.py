    ...
    def _get_provisions(self) -> dict[str, Path]:
        ...
        for platform_dir in (x for x in candidate.glob("*") if x.is_dir()):
            ...
            for item in extras_dir.glob("platform_*.py"):
                provisions[item.stem[9:].lower()] = item
...
class Context(object):
    ...
    def __init__(self, primary_dir:str):
        ...
        if sys.platform == "darwin":
            # On Mac, we want to use the case-correct path, and the fcntl.F_GETPATH
            # operation is the most direct way to get that. Note that if we encounter
            # an exception, the path will be left in its original form
            try:
                import fcntl
        ...
        if primary_dir.is_dir():
            ...
            while not iter_dir.samefile(iter_dir.anchor):
                ...
                if (iter_dir / "Config").is_dir():
                    return self._initialize_noproject(iter_dir)

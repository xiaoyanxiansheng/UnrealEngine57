...
class _PlatformBase(unreal.Platform):
    ...
    def _get_version_json(self, **kwargs):
        ret = {
            "sdk" : super()._get_version_json()
        }

        toolchains = (
            ("vc",      "PreferredVisualCppVersions"),
            ("clang",   "PreferredClangVersions"),
        )
        for name, key in toolchains:
            tool_ver = super()._get_version_json(key=key, use_autosdk=False)
            if not tool_ver:                continue
            if isinstance(tool_ver, list):  tool_ver = max(x for x in tool_ver)
            if "-" in tool_ver:             tool_ver = tool_ver.split("-")[0]
            ret[name] = tool_ver

...
class _Cook(unrealcmd.MultiPlatformCmd):
    ...
    def _cook(self, cook_form, exec_context=None):
        ...
        if self.args.build:
            ...
            proc = sp.run(("_build", "editor", variant, "--nosummary"))
            if proc.returncode:
        ...
        forcerecook_alreadypresent = any(True for x in self.args.cookargs \
            if x == "-forcerecook" or x.startswith("-forcerecook="))
        ...
        commandlet_args = (
            ...
            "-unversioned" if not self.args.versioned else None,
            "-stdout",
            "-cookonthefly" if self.args.onthefly else None,
            "-noxgeshadercompile" if self.args.noxge else None,
            "-forcerecook=false" if self.args.iterate and not forcerecook_alreadypresent else None,

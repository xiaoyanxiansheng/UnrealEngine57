...
class Platform(object):
    ...
    def _get_version_json(self, *, key="MainVersion", use_autosdk=True):
        ...
        def read_sdk_json(path):
            ...
            if version := sdk_info.get(key, None):
                return version
    ...
    def get_version(self, *, detailed=False):
        ...
        if isinstance(version, dict) and version and not detailed:
            version = next(x for x in version.values())
    ...
    def get_cook_flavor(self, target = None):
        if hasattr(self, "_get_cook_flavor"):
            return self._get_cook_flavor(target)

...
MASSAIBEHAVIOR_API DECLARE_LOG_CATEGORY_EXTERN(LogMassBehavior, Warning, All);
 ...
 */
#if WITH_MASSGAMEPLAY_DEBUG
#define MASSBEHAVIOR_LOG(Verbosity, Format, ...) UE_VLOG_UELOG(static_cast<const FMassStateTreeExecutionContext&>(Context).GetOwner(), LogMassBehavior, Verbosity, \
	TEXT("Entity [%s][%s] ") Format, *static_cast<const FMassStateTreeExecutionContext&>(Context).GetEntity().DebugGetDescription(), *StaticStruct()->GetName(), ##__VA_ARGS__)
#define MASSBEHAVIOR_CLOG(Condition, Verbosity, Format, ...) UE_CVLOG_UELOG((Condition), static_cast<const FMassStateTreeExecutionContext&>(Context).GetOwner(), LogMassBehavior, Verbosity, \
	TEXT("Entity [%s][%s] ") Format, *static_cast<const FMassStateTreeExecutionContext&>(Context).GetEntity().DebugGetDescription(), *StaticStruct()->GetName(), ##__VA_ARGS__)
#else

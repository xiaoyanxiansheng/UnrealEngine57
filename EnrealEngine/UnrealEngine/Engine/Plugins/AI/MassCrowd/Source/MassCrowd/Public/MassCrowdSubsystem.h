...
public:
	...
	MASSCROWD_API void RebuildLaneData();
#endif

	/** @return true if the Crowd subsystem has lane data for specified graph. */
	MASSCROWD_API bool HasCrowdDataForZoneGraph(const FZoneGraphDataHandle DataHandle) const;
	 ...
	 */
	MASSCROWD_API const FRegisteredCrowdLaneData* GetCrowdData(const FZoneGraphDataHandle DataHandle) const;
	 ...
	 */
	MASSCROWD_API TOptional<FZoneGraphCrowdLaneData> GetCrowdLaneData(const FZoneGraphLaneHandle LaneHandle) const;
	 ...
	 */
	MASSCROWD_API const FCrowdTrackingLaneData* GetCrowdTrackingLaneData(const FZoneGraphLaneHandle LaneHandle) const;
	 ...
	 */
	MASSCROWD_API const FCrowdBranchingLaneData* GetCrowdBranchingLaneData(const FZoneGraphLaneHandle LaneHandle) const;
	 ...
	 * @return Runtime data associated to the lane (nullptr if provided handle is invalid or no data is associated to that lane)
	 */
	MASSCROWD_API const FCrowdWaitAreaData* GetCrowdWaitingAreaData(const FZoneGraphLaneHandle LaneHandle) const;
	 ...
	 */
	MASSCROWD_API ECrowdLaneState GetLaneState(const FZoneGraphLaneHandle LaneHandle) const;
	 ...
	 */
	MASSCROWD_API bool SetLaneState(const FZoneGraphLaneHandle LaneHandle, ECrowdLaneState NewState);
	 ...
	 */
	MASSCROWD_API int32 AcquireWaitingSlot(const FMassEntityHandle Entity, const FVector& EntityPosition, const FZoneGraphLaneHandle LaneHandle,
	 ...
	 */
	MASSCROWD_API void ReleaseWaitingSlot(const FMassEntityHandle Entity, const FZoneGraphLaneHandle LaneHandle, const int32 SlotIndex);
	 ...
	 */
	MASSCROWD_API void OnEntityLaneChanged(const FMassEntityHandle Entity, const FZoneGraphLaneHandle PreviousLaneHandle, const FZoneGraphLaneHandle CurrentLaneHandle);

	/** Returns the weight for lane selection that is associated to the given lane based on its density tag. */
	MASSCROWD_API float GetDensityWeight(const FZoneGraphLaneHandle LaneHandle, const FZoneGraphTagMask LaneTagMask) const;
...
protected:
	...
	MASSCROWD_API void UpdateDensityMask();

	// USubsystem BEGIN
	MASSCROWD_API virtual void Initialize(FSubsystemCollectionBase& Collection) override;
	MASSCROWD_API virtual void PostInitialize() override;
	MASSCROWD_API virtual void Deinitialize() override;
	// USubsystem END

	MASSCROWD_API void PostZoneGraphDataAdded(const AZoneGraphData* ZoneGraphData);
	MASSCROWD_API void PreZoneGraphDataRemoved(const AZoneGraphData* ZoneGraphData);
	 ...
	 */
	MASSCROWD_API FZoneGraphCrowdLaneData* GetMutableCrowdLaneData(const FZoneGraphLaneHandle LaneHandle);
	 ...
	 */
	MASSCROWD_API void BuildLaneData(FRegisteredCrowdLaneData& LaneData, const FZoneGraphStorage& Storage);
	 ...
	 */
	MASSCROWD_API void OnEnterTrackedLane(const FMassEntityHandle Entity, const int32 LaneIndex, FCrowdTrackingLaneData& TrackingData);
	 ...
	 * @param TrackingData Runtime state associated to the lane
	 */
	MASSCROWD_API void OnExitTrackedLane(const FMassEntityHandle Entity, const int32 LaneIndex, FCrowdTrackingLaneData& TrackingData);
	 ...
	 */
	MASSCROWD_API FCrowdTrackingLaneData& CreateTrackingData(const int32 LaneIndex, const FZoneGraphStorage& ZoneGraphStorage);
	 ...
	 */
	MASSCROWD_API FCrowdBranchingLaneData& CreateBranchingData(const int32 LaneIndex, const FZoneGraphStorage& ZoneGraphStorage);

	MASSCROWD_API void CreateWaitSlots(const int32 CrossingLaneIndex, FCrowdWaitAreaData& WaitArea, const FZoneGraphStorage& ZoneGraphStorage);

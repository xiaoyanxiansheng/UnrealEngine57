...
struct THTNWorldStateOperation
{
	...
	inline bool IsRHSAbsolute() const { return KeyRightHand == FHTNPolicy::InvalidWSKey; }
...
struct HTNPLANNER_API FHTNDomain : public TSharedFromThis<FHTNDomain>
{
	...
	inline bool IsPrimitiveTask(const FHTNPolicy::FTaskID TaskID) const
	{
		return TaskID < FirstCompositeTaskID;
	}
	inline bool IsCompositeTask(const FHTNPolicy::FTaskID TaskID) const
	{
...
{	
	...
	inline bool CheckConditions(const FHTNCondition* Conditions, const int32 ConditionsCount) const
	{
	...
	inline void ApplyEffects(const FHTNEffect* Effects, const int32 EffectsCount)
	{

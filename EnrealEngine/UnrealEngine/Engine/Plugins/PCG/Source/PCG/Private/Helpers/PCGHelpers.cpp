...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PCGHelpers)
...
namespace PCGHelpers
{
	...
	FBox GetGridBounds(const AActor* Actor, const UPCGComponent* Component)
	{
		...
		else if (const ALandscapeProxy* LandscapeActor = Cast<const ALandscapeProxy>(Actor))
		{
	...
	FBox GetLandscapeBounds(const ALandscapeProxy* InLandscape)
	{
		...
		{
			return InLandscape->GetProxyBounds();
		}
	}
	...
	bool IsInWorldPartitionBuilderCommandletContext()
	{
		static const UClass* WorldPartitionBuilderCommandletClass = FindObject<UClass>(nullptr, TEXT("/Script/UnrealEd.WorldPartitionBuilderCommandlet"), EFindObjectFlags::ExactClass);
		check(WorldPartitionBuilderCommandletClass);
		const UClass* RunningCommandletClass = GetRunningCommandletClass();
		return RunningCommandletClass && RunningCommandletClass->IsChildOf(WorldPartitionBuilderCommandletClass);
	}
#endif
	...
	{
		return InExecutionSource ? InExecutionSource->GetExecutionState().GetGenerationGridSize() : PCGHiGenGrid::UninitializedGridSize();
	}

	bool IsRuntimeGeneration(const IPCGGraphExecutionSource* InExecutionSource)
	{
		return InExecutionSource && InExecutionSource->GetExecutionState().IsManagedByRuntimeGenSystem();
	}
	...
	bool IsServer(const UWorld* InWorld)
	{
		return IsDedicatedServer(InWorld) || IsListenServer(InWorld);
	}
	
	bool IsDedicatedServer(const UWorld* InWorld)
	{
		if (InWorld)
		{
	...
	bool IsListenServer(const UWorld* InWorld)
	{
		if (InWorld)
		{
			const ENetMode NetMode = InWorld->GetNetMode();
	...
	int32 GetNumberOfElements(const UPCGData* InData)
	{
		if (!InData->IsSupportedMetadataDomainID(PCGMetadataDomainID::Elements))
		{
			return 0;
		}
		
		FPCGAttributePropertyInputSelector ElementSelector;
		ElementSelector.SetAttributeName(NAME_None);
		InData->SetDomainFromDomainID(PCGMetadataDomainID::Elements, ElementSelector);
		TUniquePtr<const IPCGAttributeAccessorKeys> ElementKeys = PCGAttributeAccessorHelpers::CreateConstKeys(InData, ElementSelector);
		return ElementKeys ? ElementKeys->GetNum() : 0;
	}

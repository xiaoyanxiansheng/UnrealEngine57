...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PCGBooleanSelect)
...
FPCGDataTypeIdentifier UPCGBooleanSelectSettings::GetCurrentPinTypesID(const UPCGPin* Pin) const
{
	...
	if (Pin->IsOutputPin())
	{
		const FPCGDataTypeIdentifier InputTypeUnion = GetTypeUnionIDOfAllIncidentEdges({PCGBooleanSelectConstants::InputLabelA, PCGBooleanSelectConstants::InputLabelB});

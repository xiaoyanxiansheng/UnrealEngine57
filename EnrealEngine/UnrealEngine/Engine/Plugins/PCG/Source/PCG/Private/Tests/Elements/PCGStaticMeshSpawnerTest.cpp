...
/** Finds the output UPCGBasePointData and builds a mapping from world-space position to each Point Index */
void GetOutputDataAndBuildPositionMap(FPCGTestBaseClass* Test, const TArray<FPCGTaggedData>& OutputData, TMap<FVector, int32>& OutLocationToPoint, const UPCGBasePointData*& OutPointData)
{
	check(Test);
	...
	if (Test->TestEqual("Valid number of data elements in output", OutputData.Num(), 2))
	{
		...
		OutPointData = Cast<UPCGBasePointData>(OutputData[1].Data);
	}
	...
	if (Test->TestNotNull("Valid output data", OutPointData))
	{
		...
		{
			OutLocationToPoint.Emplace(TransformRange[Index].GetLocation(), Index);
		}
	}
}
...
void ValidateOutPointData(
	...
	const TMap<FVector, int32>& LocationToPointIndex, 
	const FName& OutputAttributeName, 
	const UPCGBasePointData* OutPointData) 
{
	check(Test);
	check(ISMC);
	...
	for (int InstanceIndex = 0; InstanceIndex < ISMC->GetInstanceCount(); ++InstanceIndex)
	{
		...
		const int32* OutPointIndex = LocationToPointIndex.Find(Transform.GetLocation());

		if (Test->TestNotNull("Output point index exists", OutPointIndex) && Test->TestNotEqual("Ouput point index is valid", *OutPointIndex, (int32)INDEX_NONE))
		{
			const PCGMetadataValueKey ValueKey = OutAttribute->GetValueKey(MetadataEntryRange[*OutPointIndex]);
	...
	bool bValidateOutput=false)
{
	...
	TMap<FVector, int32> LocationToPointIndex;
	const UPCGBasePointData* OutPointData = nullptr;

	if (bValidateOutput)
	{
		GetOutputDataAndBuildPositionMap(Test, Context->OutputData.TaggedData, LocationToPointIndex, OutPointData);
	}
	...
	for (UInstancedStaticMeshComponent* ISMC : ISMCs)
	{
		...
		if (Test->TestNotNull("Validate instanced mesh exists in MeshEntries", Entry))
		{
			...
			if (bValidateOutput)
			{
				ValidateOutPointData(Test, ISMC, LocationToPointIndex, Settings->OutAttributeName, OutPointData);
			}
		}
	}
...
bool FPCGStaticMeshSpawnerByAttributeTest::RunTest(const FString& Parameters)
{
	...
	for (int i = 0; i < PointCount; i++)
	{
		...
		UPCGMetadataAccessorHelpers::SetStringAttributeByMetadataKey(MetadataEntryRange[i], PointData->Metadata, AttributeName, ObjectPath.ToString());
	}
...
bool FPCGStaticMeshSpawnerWeightedByCategoryTest::RunTest(const FString& Parameters)
{
	...
	for (int i = 0; i < PointCount; i++)
	{
		...
		UPCGMetadataAccessorHelpers::SetStringAttributeByMetadataKey(MetadataEntryRange[i], PointData->Metadata, AttributeName, AttributeValue);
	}

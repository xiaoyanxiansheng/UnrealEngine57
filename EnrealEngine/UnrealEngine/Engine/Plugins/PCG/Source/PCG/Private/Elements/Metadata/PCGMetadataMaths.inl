...
namespace PCGMetadataMaths
{
	...
	////////////////////
	template <typename T>
	inline T AddModulo(const T& Value1, const T& Value2, const T& Value3)
	{
		// If Result is negative, add Value3. (i.e. A=0, B=-1, C=5 Result=-1, returns 4.
		const T Result = Modulo(Value1 + Value2, Value3);
		return Result >= 0 ? Result : (Result + Value3);
	}

	template<>
	inline FVector2D AddModulo(const FVector2D& Value1, const FVector2D& Value2, const FVector2D& Value3)
	{
		return FVector2D(
			AddModulo(Value1.X, Value2.X, Value3.X),
			AddModulo(Value1.Y, Value2.Y, Value3.X));
	}

	template<>
	inline FVector AddModulo(const FVector& Value1, const FVector& Value2, const FVector& Value3)
	{
		return FVector(
			AddModulo(Value1.X, Value2.X, Value3.X),
			AddModulo(Value1.Y, Value2.Y, Value3.Y),
			AddModulo(Value1.Z, Value2.Z, Value3.Z));
	}

	template<>
	inline FVector4 AddModulo(const FVector4& Value1, const FVector4& Value2, const FVector4& Value3)
	{
		return FVector4(
			AddModulo(Value1.X, Value2.X, Value3.X),
			AddModulo(Value1.Y, Value2.Y, Value3.Y),
			AddModulo(Value1.Z, Value2.Z, Value3.Z),
			AddModulo(Value1.W, Value2.W, Value3.W));
	}
}

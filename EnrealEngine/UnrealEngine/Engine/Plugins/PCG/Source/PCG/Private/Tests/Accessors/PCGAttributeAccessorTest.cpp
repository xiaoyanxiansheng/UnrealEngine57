...
namespace FPCGAttributeAccessorTestHelpers
{
	...
	{
		AttributeData()
			: PointData(PCGTestsCommon::CreateEmptyBasePointData())
...
{
	FPCGAttributeAccessorTestHelpers::AttributeData AttributeData;
	FPCGAttributeAccessorKeysEntries Keys(const_cast<FPCGMetadataDomain*>(AttributeData.FloatAttribute->GetMetadataDomain()));
...
bool FPCGAttributeAccessorSingleGetPointsTest::RunTest(const FString& Parameters)
{
	...
	const TConstPCGValueRange<int64> MetadataEntryRange = AttributeData.PointData->GetConstMetadataEntryValueRange();
	TUniquePtr<const IPCGAttributeAccessorKeys> Keys = PCGAttributeAccessorHelpers::CreateConstKeys(AttributeData.PointData, FPCGAttributePropertySelector());

	if (!TestEqual("Right number of keys", Keys->GetNum(), AttributeData.PointData->GetNumPoints()))
	{
		return false;
	}
	...
	for (int32 i = 0; i < 4; ++i)
	{
		if (!TestTrue(FString::Printf(TEXT("Get Float value %d"), i), AttributeData.FloatAccessor->Get(*AttributeData.TempFloats, i, *Keys))
			|| !TestEqual(FString::Printf(TEXT("Float value %d"), i), *AttributeData.TempFloats, AttributeData.FloatAttribute->GetValueFromItemKey(MetadataEntryRange[i])))
		{
			return false;
		}
	}
...
bool FPCGAttributeAccessorSingleSetPointsTest::RunTest(const FString& Parameters)
{
	...
	const TConstPCGValueRange<int64> MetadataEntryRange = AttributeData.PointData->GetConstMetadataEntryValueRange();
	TUniquePtr<IPCGAttributeAccessorKeys> Keys = PCGAttributeAccessorHelpers::CreateKeys(AttributeData.PointData, FPCGAttributePropertySelector());
	...
	for (int32 i = 0; i < 4; ++i)
	{
		if (!TestTrue(FString::Printf(TEXT("Set Float value %d"), i), AttributeData.FloatAccessor->Set(TempFloats[i], i, *Keys))
			|| !TestEqual(FString::Printf(TEXT("Float value %d"), i), AttributeData.FloatAttribute->GetValueFromItemKey(MetadataEntryRange[i]), TempFloats[i]))
		{
			return false;
		}
	}
	...
	if (!TestEqual("Point 0 should have entry 2", MetadataEntryRange[0], PCGMetadataEntryKey(2))
		|| !TestEqual("Point 1 should have entry 1", MetadataEntryRange[1], PCGMetadataEntryKey(1))
		|| !TestEqual("Point 2 should have entry 0", MetadataEntryRange[2], PCGFirstEntryKey))
	{
		return false;
	}
	...
	if (!TestEqual("Last point should have a valid entry", MetadataEntryRange[3], PCGMetadataEntryKey(3)))
	{
		return false;
	}
...
bool FPCGAttributeAccessorSetRangePointsTest::RunTest(const FString& Parameters)
{
	...
	if (!TestTrue("Set Float attribute full range", AttributeData.FloatAccessor->SetRange(FPCGAttributeAccessorTestHelpers::MakeConstArrayView(TempFloats), Index, *Keys))
		|| !TestEqual("Float value 0", AttributeData.FloatAttribute->GetValueFromItemKey(MetadataEntryRange[0]), TempFloats[0])
		|| !TestEqual("Float value 1", AttributeData.FloatAttribute->GetValueFromItemKey(MetadataEntryRange[1]), TempFloats[1])
		|| !TestEqual("Float value 2", AttributeData.FloatAttribute->GetValueFromItemKey(MetadataEntryRange[2]), TempFloats[2])
		|| !TestEqual("Float value 3", AttributeData.FloatAttribute->GetValueFromItemKey(MetadataEntryRange[3]), TempFloats[3]))
	{
		return false;
	}

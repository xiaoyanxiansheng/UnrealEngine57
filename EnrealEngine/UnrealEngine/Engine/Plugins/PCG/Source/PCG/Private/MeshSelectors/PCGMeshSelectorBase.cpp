...
bool UPCGMeshSelectorBase::SelectMeshInstances(
...
{
	// This method should be overwritten by the subclasses but in case that subclass hasn't yet been updated try and call the old signature
	const UPCGPointData* TypedInPointData = Cast<UPCGPointData>(InPointData);
	UPCGPointData* TypedOutPointData = Cast<UPCGPointData>(OutPointData);

	const FText Message = FText::Format(LOCTEXT("SelectMeshInstancesNotImplemented", "MeshSelector class '{0}' needs to implement SelectMeshInstances"), FText::FromString(GetClass()->GetName()));
	if (TypedInPointData && TypedOutPointData)
	{
		PCGE_LOG_C(Warning, GraphAndLog, &Context, Message);
PRAGMA_DISABLE_DEPRECATION_WARNINGS
		return SelectInstances(Context, Settings, TypedInPointData, OutMeshInstances, TypedOutPointData);
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	}
	else
	{
		PCGE_LOG_C(Error, GraphAndLog, &Context, Message);
		return false;
	}
}

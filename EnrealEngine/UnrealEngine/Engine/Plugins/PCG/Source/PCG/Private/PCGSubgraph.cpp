...
{
	return IsSubgraphOverridden();
}

bool UPCGSubgraphSettings::IsSubgraphOverridden() const
{
...
void FPCGSubgraphContext::InitializeUserParametersStruct()
{
	...
	if (!OverridableParams.IsEmpty() && UserParameters)
	{
		// In the case where we have a subgraph override, we'll need to make sure the override properties are also taken into account when we apply the overrides
		// and copy the overriden values.
		const UPCGGraphInstance* OriginalGraphInstance = Cast<UPCGGraphInstance>(Settings->GetOriginalSubgraphInterface());
		if (OriginalGraphInstance && Graph != OriginalGraphInstance && !OriginalGraphInstance->ParametersOverrides.PropertiesIDsOverridden.IsEmpty())
		{
			// Copy the subgraph override parameters first and copy to the subgraph override parameters, copying over compatible parameters
			FInstancedPropertyBag TempGraphInstanceParametersOverride = *UserParameters; // Full copy - Also set the layout of the property bag
			TArray<FName> PropertyNames;
			PropertyNames.Reserve(OriginalGraphInstance->ParametersOverrides.PropertiesIDsOverridden.Num());
			const UPropertyBag* OriginalBag = OriginalGraphInstance->ParametersOverrides.Parameters.GetPropertyBagStruct();
			if (ensure(OriginalBag))
			{
				Algo::Transform(OriginalGraphInstance->ParametersOverrides.PropertiesIDsOverridden, PropertyNames,
				[OriginalBag](const FGuid& InId) -> FName
					{
						// If the property is overridden the desc must exist.
						const FPropertyBagPropertyDesc* Desc = OriginalBag->FindPropertyDescByID(InId);
						return ensure(Desc) ? Desc->Name : NAME_None;
					});

				TempGraphInstanceParametersOverride.CopyMatchingValuesByName(OriginalGraphInstance->ParametersOverrides.Parameters, MakeConstArrayView(PropertyNames)); // Subset copy
				GraphInstanceParametersOverride = TempGraphInstanceParametersOverride.Detach(); // Move
			}

			return;
		}

		// Otherwise look for any overridden param

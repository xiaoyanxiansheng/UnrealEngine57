...
IMPLEMENT_CUSTOM_SIMPLE_AUTOMATION_TEST(FPCGMetadataMathsOp_AddModulo, FPCGMetadataMathsOpTest, "Plugins.PCG.Metadata.MathsOp.AddModulo", PCGTestsCommon::TestFlags)

bool FPCGMetadataMathsOp_AddModulo::RunTest(const FString& Parameters)
{
	bool bSuccess = true;

	using EPCGMetadataMathsOperation::AddModulo;
	
	// ({0, 1, 2} + 1) % 3 -> {1, 2, 0}
	bSuccess &= ExecuteTest(TestParams(AddModulo, GenerateParamData<int32>({{0, 1, 2}, {1}, {3}}), TArray({0, 0, 0}), TArray({1, 2, 0})));
	
	// ({0, 1, 2} - 1) % 3 -> {2, 0, 1}
	bSuccess &= ExecuteTest(TestParams(AddModulo, GenerateParamData<int64>({{0ll, 1ll, 2ll}, {-1ll}, {3ll}}), TArray({0ll, 0ll, 0ll}), TArray({2ll, 0ll, 1ll})));

...
TArray<FPCGGraphTask> FPCGGraphCompiler::CompileGraph(UPCGGraph* InGraph, FPCGTaskId& NextId, FPCGStackContext& InOutStackContext)
{
	...
	while (NodeQueue.Num() > 0)
	{
		...
		UPCGGraph* Subgraph = SubgraphNode ? SubgraphNode->GetSubgraph().Get() : nullptr;
		...
		if(Subgraph && !bIsDynamic && bIsNotDisabled && !bIsRecursiveGraph)
		{
			...
			for (const UPCGPin* InputPin : Node->InputPins)
			{
				...
				const bool bIsDatalessPin = InputPin->Properties.IsDatalessPin();
				...
				for (const UPCGEdge* InboundEdge : InputPin->Edges)
				{
					if (InboundEdge->IsValid())
					{
						...
						else
						{
							...
							InputNodeTask->Inputs.Emplace(IdMapping[InboundEdge->InputPin->Node], InboundEdge->InputPin->Properties, InboundEdge->OutputPin->Properties, /*bInProvideData=*/!bIsDatalessPin);
						}
					}
					else
					{
		...
		else
		{
			...
			for (const UPCGPin* InputPin : Node->InputPins)
			{
				...
				for (const UPCGEdge* InboundEdge : InputPin->Edges)
				{
					...
					{
						Task.Inputs.Emplace(*InboundId, InboundEdge->InputPin->Properties, InputPin->Properties, !bIsDatalessPin);
					}
					else
					{
	...
	for (FPCGTaskId TaskId = 0; TaskId < NumCompiledTasksBefore; ++TaskId)
	{
		...
		for (int32 InputIndex = 0; InputIndex < InOutCompiledTasks[TaskId].Inputs.Num(); ++InputIndex)
		{
			...
			if (InputGraphGenerationGrid != EPCGHiGenGrid::Uninitialized && InputGraphGenerationGrid > GraphGenerationGrid)
			{
				...
				{
					UpstreamPinProps.AllowedTypes = UpstreamSettings->GetCurrentPinTypesID(UpstreamPin);
				}
...
void FPCGGraphCompiler::PostCullStackCleanup(TArray<FPCGGraphTask>& InCompiledTasks, FPCGStackContext& InOutStackContext)
{
	...
	if (ActiveStackIDs.Num() < InOutStackContext.GetNumStacks())
	{
		...
		for (FPCGGraphTask& Task : InCompiledTasks)
		{
			if (ensure(RemovedCounts.IsValidIndex(Task.StackIndex)))
			{
...
void FPCGGraphCompiler::CalculateDynamicActivePinDependencies(FPCGTaskId InTaskId, TArray<FPCGGraphTask>& InOutCompiledTasks)
{
	...
	auto LogTooManyOutputPinsWarning = [](const UPCGNode* InUpstreamNode)
	{
		UE_LOG(LogPCG, Warning, TEXT("Number of output pins (%d) on node %s exceeded maximum limit (%d) for dynamic node culling. "
			"Dynamic culling will not function for this node's output pins."),
			InUpstreamNode->GetOutputPins().Num(), *InUpstreamNode->GetName(), PCGPinIdHelpers::MaxOutputPins);
	};
	...
	for (const FPCGGraphTaskInput& Input : InOutCompiledTasks[InTaskId].Inputs)
	{
		...
		if (UpstreamNode->GetOutputPins().Num() > PCGPinIdHelpers::MaxOutputPins)
		{
			LogTooManyOutputPinsWarning(UpstreamNode);
			continue;
		}

...
struct FPCGCacheEntryKey
{
	FPCGCacheEntryKey(const IPCGElement* InElement, const FPCGCrc& InCrc)
		: Element(InElement)
		, CrcHash(InCrc.GetValue())
	{
		check(Element);
		ensure(InCrc.IsValid());

		Hash = GetTypeHash(Element);
		Hash = HashCombine(Hash, CrcHash);
	}

	bool operator==(const FPCGCacheEntryKey& Other) const
	{
		return Element == Other.Element && CrcHash == Other.CrcHash;
	}
...
{
public:
	FPCGGraphCache(bool bGameWorld);
	~FPCGGraphCache();

	bool IsEnabled() const;

	int32 GetMemoryBudgetMB() const;

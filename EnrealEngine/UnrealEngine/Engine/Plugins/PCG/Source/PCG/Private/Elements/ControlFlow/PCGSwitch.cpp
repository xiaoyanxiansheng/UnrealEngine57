...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PCGSwitch)
...
void UPCGSwitchSettings::UpdateSettings()
{
	CacheEnumValueToPinLabelsMapping();
	CachePinLabels();
}
...
void UPCGSwitchSettings::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyChangedEvent.GetMemberPropertyName() == GET_MEMBER_NAME_CHECKED(UPCGSwitchSettings, SelectionMode) ||
		PropertyChangedEvent.GetMemberPropertyName() == GET_MEMBER_NAME_CHECKED(UPCGSwitchSettings, IntOptions) ||
...
int UPCGSwitchSettings::GetSelectedOutputPinIndex() const
{
	...
	else if (SelectionMode == EPCGControlFlowSelectionMode::Enum)
	{
		...
		const int FoundIndex = PinLabel ? CachedPinLabels.IndexOfByKey(*PinLabel) : INDEX_NONE;
		return FoundIndex != INDEX_NONE ? FoundIndex : EnumValueToPinLabelMapping.Num();
	}
...
bool UPCGSwitchSettings::GetSelectedPinLabel(FName& OutSelectedPinLabel) const
{
	...
	if ((SelectionMode == EPCGControlFlowSelectionMode::Integer && IsValuePresent(IntegerSelection))
		|| (SelectionMode == EPCGControlFlowSelectionMode::String && IsValuePresent(StringSelection))
		|| (SelectionMode == EPCGControlFlowSelectionMode::Enum && IsValuePresent(EnumSelection.Value)))
	{
		const int32 Index = GetSelectedOutputPinIndex();

		if (!CachedPinLabels.IsValidIndex(Index))
		{
			return false;
		}
...
void UPCGSwitchSettings::CacheEnumValueToPinLabelsMapping() const
{
...
bool UPCGSwitchSettings::IsPinUsedByNodeExecution(const UPCGPin* InPin) const
{
	...
	if (!CachedPinLabels.IsValidIndex(SelectedPinIndex))
	{
		ensure(false);
		return true;
	}

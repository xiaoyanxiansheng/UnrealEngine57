...
namespace PCGDeterminismTests
{
...
#if WITH_EDITOR
	void RunDeterminismTest(const UPCGGraph* InPCGGraph, const UPCGComponent* InPCGComponent, FDeterminismTestResult& OutResult)
	{
		RunDeterminismTest(InPCGGraph, static_cast<const IPCGGraphExecutionSource*>(InPCGComponent), OutResult);
	}
	
	void RunDeterminismTest(const UPCGGraph* InPCGGraph, const IPCGGraphExecutionSource* InSource, FDeterminismTestResult& OutResult)
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(PCGDeterminismTests::RunDeterminismTest::Graph);

		// @todo_pcg: Support Determinism test on all sources.  
		const UPCGComponent* InPCGComponent = Cast<const UPCGComponent>(InSource);
		...
		check(InputPCGData);
		OutResult.DataTypesTested |= InputPCGData->GetDataTypeId();
	...
	bool RunBasicSelfTest(const FNodeAndOptions& NodeAndOptions)
	{
		...
		const TArray<TArray<FPCGDataTypeIdentifier>>& BaseOptionsByPin = NodeAndOptions.BaseOptionsByPin;
		...
		for (int32 PermutationIndex = 0; PermutationIndex < NumPermutations; ++PermutationIndex)
		{
			...
			{
				FPCGDataTypeIdentifier DataType = GetPermutation(PermutationIndex, PinIndex, BaseOptionsByPin);
	...
	EDeterminismLevel GetHighestDeterminismLevelSimpleShuffleInput(const FNodeAndOptions& NodeAndOptions, int32 NumInputsPerPin, EDeterminismLevel MaxLevel)
	{
		...
		for (int32 PinIndex = 0; PinIndex < BaseOptionsByPin.Num(); ++PinIndex)
		{
			for (int32 I = 0; I < NumInputs; ++I)
			{
				FPCGDataTypeIdentifier DataType = BaseOptionsByPin[PinIndex][I];
				AddRandomizedInputData(FirstTestData, DataType);
				AddRandomizedInputData(SecondTestData, DataType);
			}
		}
	...
	void AddRandomizedInputData(PCGTestsCommon::FTestData& TestData, FPCGDataTypeIdentifier DataType, const FName& PinName)
	{
		if (DataType == EPCGDataType::Point)
		{
			AddRandomizedMultiplePointInputData(TestData, Defaults::NumTestPointsToGenerate, PinName);
			return;
		}
		else if (DataType == EPCGDataType::Volume)
		{
			AddRandomizedVolumeInputData(TestData, PinName);
			return;
		}
		else if (DataType == EPCGDataType::PolyLine)
		{
			AddRandomizedPolyLineInputData(TestData, Defaults::NumTestPolyLinePointsToGenerate, PinName);
			return;
		}
		else if (DataType == EPCGDataType::Primitive)
		{
			AddRandomizedPrimitiveInputData(TestData, PinName);
			return;
		}
		else if (DataType == EPCGDataType::Surface)
		{
			AddRandomizedSurfaceInputData(TestData, PinName);
			return;
		}
		else if (DataType == EPCGDataType::Landscape)
		{
			AddRandomizedLandscapeInputData(TestData, PinName);
			return;
		}
		// Just return to represent "empty" data
		else if (!DataType.IsValid())
		{
			return;
		}
		else
		{
			UE_LOG(LogPCG, Error, TEXT("Specific type required to add to test data"));
		}
	}
	...
	bool DataCollectionsAreIdentical(const FPCGDataCollection& FirstCollection, const FPCGDataCollection& SecondCollection)
	{
		...
		{
			FPCGDataTypeIdentifier FirstDataType = FirstCollection.TaggedData[I].Data->GetDataTypeId();
			FPCGDataTypeIdentifier SecondDataType = SecondCollection.TaggedData[I].Data->GetDataTypeId();
		...
		TArray<int32>& OutIndexOffsets)
	{
		...
		for(int32 FirstIndex = 0; FirstIndex < FirstCollectionComparableIndices.Num(); ++FirstIndex)
		{
			...
			for(int32 SecondIndex = 0; SecondIndex < SecondCollectionComparableIndices.Num(); ++SecondIndex)
			{
				...
				FPCGDataTypeIdentifier FirstDataType = FirstCollection.TaggedData[I].Data->GetDataTypeId();
				FPCGDataTypeIdentifier SecondDataType = SecondCollection.TaggedData[J].Data->GetDataTypeId();

				// Only compare if they are the same type and same pin label
				if (FirstDataType == SecondDataType && FirstCollection.TaggedData[I].Pin == SecondCollection.TaggedData[J].Pin)
				{
	...
	bool InternalDataMatches(const UPCGData* FirstData, const UPCGData* SecondData, TArray<int32>& OutIndexOffsets)
	{
		...
		check(FirstData->GetDataTypeId() == SecondData->GetDataTypeId());

		// Filter output differences
		auto CompareFunction = GetDataCompareFunction(FirstData->GetDataTypeId(), EDeterminismLevel::OrderConsistent);
	...
	bool CompareProperty(const UPCGBasePointData* FirstPointData, const UPCGBasePointData* SecondPointData)
	{
		const typename TPCGPointNativeProperty<Property>::ConstValueRange FirstRange = FirstPointData->GetConstValueRange<Property>();
		const typename TPCGPointNativeProperty<Property>::ConstValueRange SecondRange = SecondPointData->GetConstValueRange<Property>();
		check(FirstRange.Num() == SecondRange.Num());

		for (int32 Index = 0; Index < FirstRange.Num(); ++Index)
		{
			if (!PCG::Private::MetadataTraits<typename TPCGPointNativeProperty<Property>::Type>::Equal(FirstRange[Index], SecondRange[Index]))
			{
				return false;
			}
		}
	...
	bool PointDataIsIdentical(const UPCGData* FirstData, const UPCGData* SecondData)
	{
		...
		if (!CompareProperty<EPCGPointNativeProperties::Steepness>(FirstPointData, SecondPointData))
		{
			return false;
		}

		if (!CompareProperty<EPCGPointNativeProperties::Density>(FirstPointData, SecondPointData))
		{
			return false;
		}
		
		if (!CompareProperty<EPCGPointNativeProperties::BoundsMin>(FirstPointData, SecondPointData))
		{
			return false;
		}
		
		if (!CompareProperty<EPCGPointNativeProperties::BoundsMax>(FirstPointData, SecondPointData))
		{
			return false;
		}
		
		if (!CompareProperty<EPCGPointNativeProperties::Color>(FirstPointData, SecondPointData))
		{
			return false;
		}
		
		if (!CompareProperty<EPCGPointNativeProperties::Transform>(FirstPointData, SecondPointData))
		{
			return false;
		}
	...
	bool DataTypeIsComparable(FPCGDataTypeIdentifier DataType)
	{
		...
		UE_LOG(LogPCG, Warning, TEXT("Unknown data comparison type: %s"), *DataType.ToString());
		return false;
	}

	bool DataIsComparable(const UPCGData* Data)
	{
		return Data && DataTypeIsComparable(Data->GetDataTypeId());
	}
	...
	TArray<FPCGDataTypeIdentifier> FilterTestableDataTypes(FPCGDataTypeIdentifier AllowedTypes, int32 NumMultipleInputs)
	{
		TArray<FPCGDataTypeIdentifier> RemainingDataTypes;
		if (!(AllowedTypes & EPCGDataType::Spatial))
		{
		...
		for (const FPCGDataTypeIdentifier& DataType : TestableDataTypes)
		{
			if (!DataType.IsValid() || !!(AllowedTypes & DataType))
			{
	...
	void RetrieveBaseOptionsPerPin(TArray<TArray<FPCGDataTypeIdentifier>>& BaseOptionsArray,
		const TArray<TObjectPtr<UPCGPin>>& InputPins,
		FPCGDataTypeIdentifier& OutDataTypesTested,
		int32 NumMultipleInputs)
	{
		...
		{
			FPCGDataTypeIdentifier AllowedDataTypes = InputPins[I]->Properties.AllowedTypes;
			if (!DataTypeIsComparable(AllowedDataTypes))
			{
				continue;
			}

			TArray<FPCGDataTypeIdentifier> TestableDataTypesForPin = FilterTestableDataTypes(AllowedDataTypes, NumMultipleInputs);
	...
	int32 GetNumPermutations(const TArray<TArray<FPCGDataTypeIdentifier>>& BaseOptionsArray)
	{
	...
	FPCGDataTypeIdentifier GetPermutation(int32 PermutationIteration, int32 PinIndex, const TArray<TArray<FPCGDataTypeIdentifier>>& BaseOptionsPerPin)
	{
		check(PermutationIteration >= 0);
		check(BaseOptionsPerPin.IsValidIndex(PinIndex));
	...
	TFunction<bool(const UPCGData*, const UPCGData*)> GetDataCompareFunction(FPCGDataTypeIdentifier DataType, EDeterminismLevel DeterminismLevel)
	{
	...
	TFunction<bool(const UPCGData*, const UPCGData*)> GetDataCollectionCompareFunction(FPCGDataTypeIdentifier DataType)
	{

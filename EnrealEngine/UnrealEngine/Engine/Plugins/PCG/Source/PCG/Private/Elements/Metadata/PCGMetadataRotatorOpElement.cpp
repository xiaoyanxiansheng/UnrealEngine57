...
namespace PCGMetadataRotatorSettings
{
	...
	inline constexpr bool IsTransformOp(EPCGMetadataRotatorOperation Operation)
	{
...
FName UPCGMetadataRotatorSettings::GetInputPinLabel(uint32 Index) const
{
	...
	{
	case 0:
		if (!PCGMetadataRotatorSettings::IsUnaryOp(Operation) && !PCGMetadataRotatorSettings::IsTransformOp(Operation))
		{
	...
	case 1:
		return PCGMetadataRotatorSettings::IsTransformOp(Operation) ? PCGMetadataSettingsBaseConstants::TransformLabel : PCGMetadataSettingsBaseConstants::DoubleInputSecondLabel;
	case 2:
...
bool UPCGMetadataRotatorSettings::IsSupportedInputType(uint16 TypeId, uint32 InputIndex, bool& bHasSpecialRequirement) const
{
	...
	else if (InputIndex == 1 && PCGMetadataRotatorSettings::IsTransformOp(Operation))
	{
		bHasSpecialRequirement = true;
...
{
	return FPCGPreConfiguredSettingsInfo::PopulateFromEnum<EPCGMetadataRotatorOperation>({ EPCGMetadataRotatorOperation::RotatorOp, EPCGMetadataRotatorOperation::TransformOp });
}
#endif // WITH_EDITOR
...
bool UPCGMetadataRotatorSettings::IsPinDefaultValueMetadataTypeValid(FName PinLabel, const EPCGMetadataTypes DataType) const
{
	return DataType == EPCGMetadataTypes::Rotator
		|| DataType == EPCGMetadataTypes::Vector
		|| DataType == EPCGMetadataTypes::Double;
}

EPCGMetadataTypes UPCGMetadataRotatorSettings::GetPinInitialDefaultValueType(const FName PinLabel) const
{
	if (PinLabel == PCGPinConstants::DefaultInputLabel
		|| PinLabel == PCGMetadataSettingsBaseConstants::DoubleInputFirstLabel
		|| PinLabel == PCGMetadataSettingsBaseConstants::DoubleInputSecondLabel)
	{
		return EPCGMetadataTypes::Rotator;
	}
	else if (PinLabel == PCGMetadataSettingsBaseConstants::TransformLabel)
	{
		return EPCGMetadataTypes::Transform;
	}
	else if (PinLabel == PCGMetadataSettingsBaseConstants::LerpRatioLabel)
	{
...
bool FPCGMetadataRotatorElement::DoOperation(PCGMetadataOps::FOperationData& OperationData) const
{
	...
	auto RotatorFunc = [this, &OperationData, Operation = Settings->Operation]<typename AttributeType>(AttributeType) -> bool
	{
		...
		{
			if (PCGMetadataRotatorSettings::IsTransformOp(Operation))
			{

...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PCGSubdivisionBase)
...
bool FPCGSubdivisionBaseElement::MatchAndSetAttributes(const TArray<FPCGTaggedData>& InputData, TArray<FPCGTaggedData>& OutputData, const UPCGParamData* InModuleInfoParamData, const UPCGSubdivisionBaseSettings* InSettings, const FPCGMetadataDomainID& InTargetDomain) const
{
	...
	for (FPCGTaggedData& TaggedData : OutputData)
	{
		...
		FPCGMetadataDomain* OutputMetadataDomain = OutputMetadata->GetMetadataDomain(InTargetDomain);

		if (!OutputMetadataDomain)
		{
			continue;
		}

		const bool bSupportMultipleValues = OutputMetadataDomain->SupportsMultiEntries();

		// Look for the Symbol Attribute in the output metadata to query its value.
		const FPCGMetadataAttribute<FName>* OutSymbolAttribute = OutputMetadataDomain->GetConstTypedAttribute<FName>(InSettings->SymbolAttributeName);
		if (!OutSymbolAttribute)
		{
			continue;
		}
		...
		if (UPCGBasePointData* OutPointData = Cast<UPCGBasePointData>(OutData))
		{
			...
			for (const FName AttributeName : AttributeNames)
			{
				...
				if (OutputMetadataDomain->HasAttribute(AttributeName))
				{
					OutputMetadataDomain->DeleteAttribute(AttributeName);
				}

				InAttributes.Add(InputMetadata->GetConstAttribute(AttributeName));
				OutAttributes.Add(OutputMetadataDomain->CopyAttribute(InAttributes.Last(), AttributeName, /*bKeepParent=*/false, /*bCopyEntries=*/false, /*bCopyValues=*/bSupportMultipleValues));
			}

			// @todo_pcg: potential optimization GetValuesFromItemKeys / SetValuesFromValueKeys. Would need support for TPCGValueRange on those.
			if (bSupportMultipleValues)
			{
			...
			{
				const FName Symbol = OutSymbolAttribute->GetValueFromItemKey(PCGFirstEntryKey);
				...
				if (OutputMetadataDomain->GetItemCountForChild() == 0)
				{
					OutputMetadataDomain->AddEntry();
				}

				for (int32 i = 0; i < InAttributes.Num(); ++i)
				{
					OutAttributes[i]->SetValue(PCGFirstEntryKey, InAttributes[i], InputEntryKey);
				}
			}
		}
		else
		{
			...
			auto AddAttribute = [InputMetadata, OutputMetadataDomain, InputEntryKey]<typename T>(T Dummy, const FName AttributeName)
			{
				...
				OutputMetadataDomain->CreateAttribute<T>(AttributeName, InAttribute->GetValueFromItemKey(InputEntryKey), InAttribute->AllowsInterpolation(), /*bOverrideParent=*/true);
			};
			...
			for (int32 i = 0; i < AttributeNames.Num(); ++i)
			{
				...
				if (OutputMetadataDomain->HasAttribute(AttributeNames[i]))
				{
					continue;
				}

...
{
	return FPCGPreConfiguredSettingsInfo::PopulateFromEnum<EPCGMetadataTransformOperation>();
}
#endif // WITH_EDITOR
...
bool UPCGMetadataTransformSettings::IsPinDefaultValueEnabled(const FName PinLabel) const
{
	// @todo_pcg: Add transforms when they are supported.
	return PinLabel == PCGMetadataSettingsBaseConstants::LerpRatioLabel && Super::IsPinDefaultValueEnabled(PinLabel);
}

EPCGMetadataTypes UPCGMetadataTransformSettings::GetPinDefaultValueType(const FName PinLabel) const
{
	return GetPinInitialDefaultValueType(PinLabel);
}

EPCGMetadataTypes UPCGMetadataTransformSettings::GetPinInitialDefaultValueType(const FName PinLabel) const
{
	if (PinLabel == PCGMetadataSettingsBaseConstants::LerpRatioLabel)
	{

...
namespace PCGCreatePointsSphere
{
	...
	struct FCommonParams
	{
		...
		int32 Seed = 0;
		int32 PointLimit = INT_MAX;
		FPCGContext* Context = nullptr;
	};

	bool GenerationWouldOverflow(const int32 PointCount, const int32 PointLimit, const FPCGContext* InContext = nullptr)
	{
		if (PointCount > PointLimit)
		{
			PCGLog::LogErrorOnGraph(LOCTEXT("PointOverflow", "The number of points to be produced exceeds the point limit. See 'Point Limit' under Advanced Settings."), InContext);
			return true;
		}
		else if (PCGFeatureSwitches::CVarCheckSamplerMemory.GetValueOnAnyThread() && PCGFeatureSwitches::Helpers::GetAvailableMemoryForSamplers() < sizeof(FPCGPoint) * PointCount)
		{
			PCGLog::LogErrorOnGraph(LOCTEXT("MemoryOverflow", "The number of iterations produced is larger than available memory."), InContext);
			return true;
		}
		else
		{
	...
	void GenerateStandardSpherePointLocations(const FCommonParams& Params, const double InTheta, const double InPhi, const double InJitter, TArray<FVector>& OutLocations)
	{
		...
		// Clamp to match the UI input and guard against divide-by-zero.
		const double ThetaPerSegment = FMath::Clamp(InTheta, 0.001, 90.0);
		const double PhiPerSegment = FMath::Clamp(InPhi, 0.001, 180.0);

		const int32 LatSegCount = 1 + FMath::FloorToInt32(FMath::Abs(LatEndAngle - LatStartAngle) / ThetaPerSegment);
		const int32 LonSegCount = FMath::FloorToInt32(FMath::Abs(LonEndAngle - LonStartAngle) / PhiPerSegment);
		...
		if (GenerationWouldOverflow(PointCount, Params.PointLimit, Params.Context))
		{
			return;
		}
		...
		for (int LatIndex = 0; LatIndex < LatSegCount; ++LatIndex)
		{
			...
			const double Theta = FMath::DegreesToRadians(LatStartAngle - 90 + ThetaPerSegment * LatIndex);
			...
			{
				double Phi = FMath::DegreesToRadians(LonStartAngle + PhiPerSegment * LonIndex);

				if (bApplyJitter)
				{
					Phi += FMath::DegreesToRadians(PhiPerSegment) * RandomSource.FRandRange(-Jitter, Jitter);
					const double JitteredTheta = Theta + FMath::DegreesToRadians(ThetaPerSegment) * RandomSource.FRandRange(-Jitter, Jitter);
	...
	void GenerateRandomSamplingPointLocations(const FCommonParams& Params, const int32 SampleCount, TArray<FVector>& OutLocations)
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(FPCGCreatePointsSphereElement::RandomSampling);
		OutLocations.Empty();

		if (GenerationWouldOverflow(SampleCount, Params.PointLimit, Params.Context))
		{
			return;
		}
	...
	void GeneratePoissonSamplingPointLocations(const FCommonParams& Params, const double Distance, const int32 SampleCount, const int32 MaxAttempts, TArray<FVector>& OutLocations)
	{

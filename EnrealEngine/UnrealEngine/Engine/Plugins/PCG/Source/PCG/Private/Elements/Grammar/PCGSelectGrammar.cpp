...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PCGSelectGrammar)
...
bool FPCGSelectGrammarElement::ExecuteInternal(FPCGContext* InContext) const
{
	...
	for (const FPCGTaggedData& PointInput : PointInputs)
	{
		...
		else
		{
			...
			TUniquePtr<const IPCGAttributeAccessorKeys> ComparedValuesKeys = PCGAttributeAccessorHelpers::CreateConstKeys(InputPointData, ComparedValueSelector);

			if (!ComparedValuesAccessor || !ComparedValuesKeys)
			{
			...
			TArray<const IPCGAttributeAccessorKeys*> Keys = {InputKeys.Get(), ComparedValuesKeys.Get()};
			PCGMetadataElementCommon::ApplyOnMultiAccessorsRange<FName, double>(Keys, Accessors, ProcessWithCriteria);
		}
	}

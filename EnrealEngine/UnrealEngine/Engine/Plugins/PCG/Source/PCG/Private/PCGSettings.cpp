...
PCG_DEFINE_TYPE_INFO(FPCGDataTypeInfoSettings, UPCGSettings)
...
public:
	static FPCGCrc PCGCrc(const UPCGSettings* PCGSettings)
	{
		...
		FPCGSettingsObjectCrc Ar(PCGSettings);
	...
	{
		// @todo_pcg: convert FPCGSettingsObjectCrc to use the new hashing framework (FPCGSettingsHashContext) which skips transient properties by default.
		if (InProperty && InProperty->HasAnyPropertyFlags(CPF_Transient))
		{
			return true;
		}

		static FPCGSettingsHashPolicy Policy;
		return !Policy.ShouldHashProperty(PCGSettings, InProperty);
	}
#endif // WITH_EDITOR

private: 
	FPCGSettingsObjectCrc(const UPCGSettings* InPCGSettings)
		: PCGSettings(InPCGSettings)
	{
	}

	const UPCGSettings* PCGSettings = nullptr;
};
...
void UPCGSettings::PostEditUndo()
{
	...
	InitializeOverriddenSettingsPropertyBag();
...
FInstancedPropertyBag UPCGSettings::CreateEmptyPropertyBagInstance() const
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UPCGSettings::CreateEmptyPropertyBagInstance);

	FInstancedPropertyBag Instance;
	Instance.InitializeFromBagStruct(OverriddenSettingsPropertyBag);
...
void UPCGSettings::FillOverridableParamsPins(TArray<FPCGPinProperties>& OutPins) const
{
	...
	TMap<FName, FPCGDataTypeIdentifier> InputPinsLabelsAndTypes;
	...
	const FPCGDataTypeIdentifier* PinType = InputPinsLabelsAndTypes.Find(PCGPinConstants::DefaultParamsLabel);
	if (PinType)
	{
	...
	else
	{
		...
		ParamPin.AllowedTypes.CustomSubtype = static_cast<int32>(EPCGMetadataTypes::Unknown);
	...
	for (const FPCGSettingsOverridableParam& OverridableParam : OverridableParams())
	{
		...
		ParamPin.SetOverrideOrUserParamPin();
#if WITH_EDITOR
		ParamPin.AllowedTypes.CustomSubtype = static_cast<int32>(OverridableParam.UnderlyingType);
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	return GetTypeUnionIDOfIncidentEdges(PinLabel);
PRAGMA_ENABLE_DEPRECATION_WARNINGS
}

FPCGDataTypeIdentifier UPCGSettings::GetTypeUnionIDOfIncidentEdges(const FName& PinLabel) const
{
	return GetTypeUnionIDOfAllIncidentEdges({PinLabel});
}

FPCGDataTypeIdentifier UPCGSettings::GetTypeUnionIDOfAllIncidentEdges(TConstArrayView<FName> PinLabels) const
{
	TArray<FPCGDataTypeIdentifier, TInlineAllocator<32>> AllTypes;
	PCGSettingsHelpers::FPinTypeScopeHelper PinTypeHelper;
	
	for (const FName PinLabel : PinLabels)
	{
		...
		if (Pin && Pin->EdgeCount() > 0)
		{
			for (const UPCGEdge* Edge : Pin->Edges)
			{
				...
				if (OtherOutputPin)
				{
					AllTypes.AddUnique(PinTypeHelper.GetCurrentPinTypeID(OtherOutputPin));
				}
			}
		}
	}

	return FPCGModule::GetConstDataTypeRegistry().GetIdentifiersUnion(AllTypes);
}
...
bool UPCGSettings::IsPinUsedByNodeExecution(const UPCGPin* InPin) const
{
#if WITH_EDITOR
	if (InPin && ShouldExecuteOnGPU())
	{
		auto MatchesPinLabel = [PinLabel = InPin->Properties.Label](const FPCGSettingsOverridableParam& OverridableParam) { return OverridableParam.Label == PinLabel; };

		const FPCGSettingsOverridableParam* Param = OverridableParams().FindByPredicate(MatchesPinLabel);

		// Pin is not in use if we are in GPU execution mode and the pin is an OverridableParam that doesn't support GPU overrides.
		// Generic overrides pin is also not supported.
		if ((Param && !Param->bSupportsGPU) || InPin->Properties.Label == PCGPinConstants::DefaultParamsLabel)
		{
			return false;
		}
	}
...
{
	return EPCGDataType::DeprecationSentinel;
}
PRAGMA_ENABLE_DEPRECATION_WARNINGS

FPCGDataTypeIdentifier UPCGSettings::GetCurrentPinTypesID(const UPCGPin* InPin) const
{
	// To support old code, we call the legacy version. If we have anything different from the Sentinel, it means the function is overridden
	// in a child class and we want to use that result to avoid any issue, even if we have a loss of info. We'll show a message to indicate
	// the child class to have its code updated
	// All PCG vanilla nodes are updated, and none is implementing `GetCurrentPinTypes` anymore.
	// @todo_pcg: To be removed when GetCurrentPinTypes is removed, in 2 versions
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	const EPCGDataType LegacyResult = GetCurrentPinTypes(InPin);
	if (LegacyResult != EPCGDataType::DeprecationSentinel)
	{
		UE_LOG(LogPCG, Warning, TEXT("Class '%s' implements `GetCurrentPinTypes` and not `GetCurrentPinTypesID`. Needs to be updated."), *GetClass()->GetName());
		return LegacyResult;
	}
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	if (HasDynamicPins() && InPin->IsOutputPin())
	{
		...
		if (FirstPin != nullptr && !FirstPin->Properties.IsAdvancedPin() && !FirstPin->Properties.IsDatalessPin())
		{
			const FPCGDataTypeIdentifier InputTypeUnion = GetTypeUnionIDOfIncidentEdges(FirstPin->Properties.Label);
			return InputTypeUnion.IsValid() ? InputTypeUnion : InPin->Properties.AllowedTypes;
		}
	}
...
TArray<FPCGSettingsOverridableParam> UPCGSettings::GatherOverridableParams() const
{
	...
	Config.IncludeMetadataValues.Add(PCGObjectMetadata::OverridableCPUAndGPU);
	Config.IncludeMetadataValues.Add(PCGObjectMetadata::OverridableCPUAndGPUWithReadback);
...
void UPCGSettings::InitializeOverriddenSettingsPropertyBag()
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UPCGSettings::InitializeOverriddenSettingsPropertyBag);
	...
	{
		// Only include properties declared in classes *after* the abstract base class UPCGSettings (almost all properties in UPCGSettings are not interesting for override).
		// @todo_pcg: We should probably allow some UPCGSettings properties (e.g. Seed).
		if (It->GetOwnerClass()->IsChildOf(BaseClass) && It->GetOwnerClass() != BaseClass)
		{
			Descriptors.Emplace(It->GetFName(), *It);
		}
	}

	OverriddenSettingsPropertyBag = UPropertyBag::GetOrCreateFromDescs(Descriptors);
}

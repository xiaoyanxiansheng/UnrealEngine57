...
void UPCGMetadataPartitionSettings::CreateKernels(FPCGGPUCompilationContext& InOutContext, UObject* InObjectOuter, TArray<UPCGComputeKernel*>& OutKernels, TArray<FPCGKernelEdge>& OutEdges) const
{
	...
	UPCGMetadataPartitionKernel* PartitionKernel = PCGKernelHelpers::CreateKernel<UPCGMetadataPartitionKernel>(InOutContext, CreateParams, OutKernels, OutEdges);
	...
	CountKernel->SetAttributeName(PartitionAttributeSelectors[0].GetAttributeName());
	CountKernel->SetEmitPerDataCounts(true);

	// Wire count kernel to copy partition kernel.
	OutEdges.Emplace(FPCGPinReference(CountKernel, PCGPinConstants::DefaultOutputLabel), FPCGPinReference(PartitionKernel, PCGMetadataPartitionConstants::ElementCountsPinLabel));
}

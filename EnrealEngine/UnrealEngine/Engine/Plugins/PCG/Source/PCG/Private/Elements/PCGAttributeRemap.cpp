...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PCGAttributeRemap)

#define LOCTEXT_NAMESPACE "PCGAttributeRemapElement"

namespace PCGAttributeRemapElement
{
	constexpr int32 DensityRemapPreconfiguredIndex = 0;
	constexpr int32 CurveRemapPreconfiguredIndex = 1;
	...
	void Remap(T& InOutValue, const FPCGAttributeRemapParams& InParams)
	{
		if (InParams.Mode == EPCGAttributeRemapMode::Ranges)
		{
		...
		else if (InParams.Mode == EPCGAttributeRemapMode::Curve && ensure(InParams.Curve))
		{
			InOutValue = static_cast<T>(InParams.Curve->Eval(static_cast<float>(InOutValue)));
		}
	}
...
FText UPCGAttributeRemapSettings::GetNodeTooltipText() const
{
	return LOCTEXT("NodeTooltip", "Remap an attribute using either a range or a curve.");
}

TArray<FPCGPreConfiguredSettingsInfo> UPCGAttributeRemapSettings::GetPreconfiguredInfo() const
{
	return {
		FPCGPreConfiguredSettingsInfo{PCGAttributeRemapElement::DensityRemapPreconfiguredIndex, LOCTEXT("DensityNodeTitle", "Density Remap")},
		FPCGPreConfiguredSettingsInfo{PCGAttributeRemapElement::CurveRemapPreconfiguredIndex, LOCTEXT("CurveNodeTitle", "Attribute Curve Remap")},
	};
}
#endif // WITH_EDITOR
...
FString UPCGAttributeRemapSettings::GetAdditionalTitleInformation() const
{
	...
	Builder += (Mode == EPCGAttributeRemapMode::Curve ? LOCTEXT("CurveSubtitle", "Curve") : LOCTEXT("RangesSubtitle", "Ranges")).ToString();

	if (!IsPropertyOverriddenByPin(GET_MEMBER_NAME_CHECKED(UPCGAttributeRemapSettings, InputSource)) && !IsPropertyOverriddenByPin(GET_MEMBER_NAME_CHECKED(UPCGAttributeRemapSettings, OutputTarget)))
	{
		if (OutputTarget.GetAttributeName() != PCGMetadataAttributeConstants::SourceAttributeName)
		{
			Builder += FString::Printf(TEXT(" %s -> %s"), *InputSource.ToString(), *OutputTarget.ToString());
		}
		else
		{
			Builder += TEXT(" ");
			Builder += InputSource.ToString();
		}
	}
...
void UPCGAttributeRemapSettings::ApplyPreconfiguredSettings(const FPCGPreConfiguredSettingsInfo& PreconfigureInfo)
{
	...
	{
	case PCGAttributeRemapElement::DensityRemapPreconfiguredIndex:
		InputSource.SetPointProperty(EPCGPointProperties::Density);
		bClampToUnitRange = true;
		break;
	case PCGAttributeRemapElement::CurveRemapPreconfiguredIndex:
		Mode = EPCGAttributeRemapMode::Curve;
		break;
	default:
		break;
	}
...
bool FPCGAttributeRemapElement::DoOperation(PCGMetadataOps::FOperationData& OperationData) const
{
	...
	if (Params.Mode == EPCGAttributeRemapMode::Ranges)
	{
	...
	else if (Params.Mode == EPCGAttributeRemapMode::Curve)
	{
		Params.Curve = Settings->Curve.GetRichCurveConst();
		if (!Params.Curve)
		{
			PCGLog::LogErrorOnGraph(LOCTEXT("InvalidCurve", "Curve is invalid"), OperationData.Context);
			return true;
		}
	}
	else
	{
		PCGLog::LogErrorOnGraph(LOCTEXT("InvalidMode", "Invalid mode"), OperationData.Context);
		return true;
	}

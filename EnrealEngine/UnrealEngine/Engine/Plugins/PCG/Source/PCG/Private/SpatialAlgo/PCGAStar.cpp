...
namespace Helpers
{
	...
	TTuple<const FPCGPoint*, double> GetNearestGoalToLocation(const FVector& Location, const TConstArrayView<FPCGPoint>& GoalPoints)
	{
...
ESearchResult ExecuteSearchIteration(const FSearchSettings& SearchSettings, FSearchState& SearchState, const TConstArrayView<FPCGPoint> GoalPoints, TArray<FPCGPoint>& OutPath)
{
	...
	const TArray<FPCGPoint>& Points = SearchState.OriginatingPointData->GetPoints();
	...
	UPCGOctreeQueries::ForEachPointInsideSphere(
		...
		[&Points, &PerPointProcessing](const UPCGBasePointData* BasePointData, int32 PointIndex, const double DistanceToPointSquared)
		{
			const FPCGPoint* PCGPoint = &Points[PointIndex];
			PerPointProcessing(PCGPoint, DistanceToPointSquared);
		});

...
#if WITH_EDITOR
  bool UPCGMetadataBreakTransformSettings::CanEditChange(const FProperty* InProperty) const
{
	if (!InProperty || !UPCGSettings::CanEditChange(InProperty))
	{
		return false;
	}

	if (InProperty->GetFName() == GET_MEMBER_NAME_CHECKED(UPCGMetadataBreakTransformSettings, InputSource))
	{
		if (const UPCGNode* Node = Cast<UPCGNode>(GetOuter()))
		{
			return Node->IsInputPinConnected(PCGPinConstants::DefaultInputLabel);
		}
	}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FPCGMetadataBreakTransformElement::Execute);

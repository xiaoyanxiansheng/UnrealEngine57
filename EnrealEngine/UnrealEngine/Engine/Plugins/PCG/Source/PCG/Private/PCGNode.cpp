...
void UPCGNode::ApplyDeprecationBeforeUpdatePins()
{
	if (UPCGSettings* Settings = GetSettings())
	{
		...
		// Give the chance to the settings to rename the pins
		Settings->ApplyDeprecationBeforeUpdatePins(this, InputPins, OutputPins);

		// Then make sure the pin properties are up to date on their types. We won't break edges here, it will be done in the update pins.
		// But all the nodes need to have their pins types up-to-date to know if an edge should be broken or not. Otherwise, the first node gets
		// its pin updated, check the other side of the edge, finds that the (old) type is not matching and break the edge, even if the new type would have been
		// compatible.
		auto UpdatePinTypes = [](const TArray<FPCGPinProperties>& PinProperties, UPCGPin* Pin)
		{
			if (const FPCGPinProperties* MatchingProperties = PinProperties.FindByPredicate([Pin](const FPCGPinProperties& Prop) { return Prop.Label == Pin->Properties.Label; }))
			{
				if (!(Pin->Properties == *MatchingProperties))
				{
					...
					Pin->bPropertiesHaveChangedOnDeprecation = true;
				}
			}
		};
		...
		for (UPCGPin* InputPin : InputPins)
		{
			UpdatePinTypes(Settings->AllInputPinProperties(), InputPin);
		}

		for (UPCGPin* OutputPin : OutputPins)
		{
			UpdatePinTypes(Settings->AllOutputPinProperties(), OutputPin);
		}
...
bool UPCGNode::SetNodeTitle(FName NewTitle, bool bApplySanitization)
{	
	...
	if (bApplySanitization)
	{
		// Ask the graph and the settings to do sanitization if needed.
		if (const UPCGGraph* Graph = GetGraph())
		{
			Graph->SanitizeNodeTitle(NewTitle, this);
		}

		if (const UPCGSettings* Settings = GetSettings())
		{
			Settings->SanitizeNodeTitle(NewTitle);
		}
	}

	// Re-check if after the sanitization the name is the same or not.
	if (NewTitle == NodeTitle)
	{
		return false;
	}
...
const UPCGPin* UPCGNode::GetPassThroughInputPin() const
{
	...
	if (const TObjectPtr<UPCGPin>* FirstPassThroughPin = Algo::FindByPredicate(AllInputPins, FindFirstPassThrough))
	{
		...
		const FPCGDataTypeIdentifier InputType = FirstPassThroughPin->Get()->GetCurrentTypesID();
		const FPCGDataTypeIdentifier OutputType = PassThroughOutput->GetCurrentTypesID();
	...
	auto UpdatePins = [this, &PinAllocator, &TouchedNodes](TArray<UPCGPin*>& Pins, const TArray<FPCGPinProperties>& PinProperties)
	{
		...
		for (UPCGPin* Pin : Pins)
		{
			if (const FPCGPinProperties* MatchingProperties = PinProperties.FindByPredicate([Pin](const FPCGPinProperties& Prop) { return Prop.Label == Pin->Properties.Label; }))
			{
				...
				// Make sure that if the pin type changed in the ApplyDeprecationBeforeUpdatePins to break incompatible edges.
				if (Pin->bPropertiesHaveChangedOnDeprecation)
				{
					bAppliedEdgeChanges |= Pin->BreakAllIncompatibleEdges(&TouchedNodes);
					Pin->bPropertiesHaveChangedOnDeprecation = false;
				}
			}
			else
			{

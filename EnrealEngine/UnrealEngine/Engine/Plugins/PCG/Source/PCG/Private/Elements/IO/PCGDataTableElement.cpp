...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PCGDataTableElement)
...
FPCGDataTypeIdentifier UPCGLoadDataTableSettings::GetCurrentPinTypesID(const UPCGPin* InPin) const
{
	check(InPin);
	...
	FPCGDataTypeIdentifier Result = UPCGDataTypeIdentifierHelpers::GetIdentifierFromLegacyType(OutputType);
	return Result.IsValid() ? Result : Super::GetCurrentPinTypesID(InPin); 
}

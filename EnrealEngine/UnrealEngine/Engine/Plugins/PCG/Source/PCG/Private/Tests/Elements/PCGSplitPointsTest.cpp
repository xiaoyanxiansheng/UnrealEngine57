...
protected:
	...
	bool GenerateTestDataRunAndValidate(const FTestParameters& Parameters)
	{
		...
		Inputs.Data = PCGTestsCommon::CreateBasePointData();
		...
		const UPCGBasePointData* OutPointDataA = Cast<UPCGBasePointData>(Outputs[0].Data);

		UTEST_NOT_NULL("OutputA point data", OutPointDataA);

		UTEST_EQUAL("OutputA point count", OutPointDataA->GetNumPoints(), 1);

		const FConstPCGPointValueRanges RangesA(OutPointDataA);
		UTEST_EQUAL("OutputA Bounds Min", RangesA.BoundsMinRange[0], Parameters.ExpectedABoundsMin);
		UTEST_EQUAL("OutputA Bounds Max", RangesA.BoundsMaxRange[0], Parameters.ExpectedABoundsMax);

		// checking the PointB portion
		const UPCGBasePointData* OutPointDataB = Cast<UPCGBasePointData>(Outputs[1].Data);

		UTEST_NOT_NULL("OutputB point data", OutPointDataB);

		UTEST_EQUAL("OutputB point count", OutPointDataB->GetNumPoints(), 1);

		const FConstPCGPointValueRanges RangesB(OutPointDataB);
		UTEST_EQUAL("OutputB Bounds Min", RangesB.BoundsMinRange[0], Parameters.ExpectedBBoundsMin);
		UTEST_EQUAL("OutputB Bounds Max", RangesB.BoundsMaxRange[0], Parameters.ExpectedBBoundsMax);

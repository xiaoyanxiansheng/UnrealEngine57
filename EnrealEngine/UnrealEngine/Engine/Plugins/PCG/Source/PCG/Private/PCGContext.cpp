			...
			if (bHasParamConnected)
			{
				...
				if (NodeSettings->HasAnyOverridableHardReferences())
				{
					for (const FPCGSettingsOverridableParam& Param : OverridableParams)
					{
						...
						if (PCGAttributeAccessorHelpers::GetOverrideParamValue(*AttributeAccessor, ObjectPath))
						{
...
void FPCGContext::OverrideSettings()
{
	...
	for (const FPCGSettingsOverridableParam& Param : OriginalSettings->OverridableParams())
	{
		...
		const bool bParamOverridden = PCGMetadataAttribute::CallbackWithRightType(PropertyAccessor->GetUnderlyingType(), [this, &AttributeAccessor, &PropertyAccessor, &Param, &AttributeName, Container](auto Dummy) -> bool
		{
			...
			PropertyType Value{};
			if (!PCGAttributeAccessorHelpers::GetOverrideParamValue(*AttributeAccessor, Value))
			{
				PCGE_LOG_C(Warning, GraphAndLog, this, FText::Format(LOCTEXT("ConversionFailed", "Parameter '{0}' ({1}) cannot be converted from attribute '{2}' ({3})"),
					FText::FromName(Param.Label), 
					PCG::Private::GetTypeNameText(PCG::Private::MetadataTypes<PropertyType>::Id),
					FText::FromName(AttributeName),
					PCG::Private::GetTypeNameText(AttributeAccessor->GetUnderlyingType())));
...
bool FPCGContext::IsValueOverriden(const TArrayView<const FName>& PropertyNameChain) const
{
	...
	const FPCGSettingsOverridableParam* const* OverrideParam = OverriddenParams.FindByPredicate([&PropertyNameChain](const FPCGSettingsOverridableParam* ParamToCheck)
	{
		return ParamToCheck && ParamToCheck->PropertiesNames == PropertyNameChain;
	});

	return OverrideParam != nullptr;
}

bool FPCGContext::IsValueOverriden(const FName PropertyName) const
{
	const FPCGSettingsOverridableParam* const* OverriddenParam = OverriddenParams.FindByPredicate([PropertyName](const FPCGSettingsOverridableParam* ParamToCheck)
	{
...
bool FPCGContext::IsExecutingGraphInspected() const
{
	const FPCGStack* StackPtr = GetStack();
	const UPCGGraph* Graph = StackPtr ? StackPtr->GetGraphForCurrentFrame() : nullptr;

	return Graph && Graph->IsInspecting() && Graph->GetInspectedStack() == *StackPtr;
}
#endif // WITH_EDITOR

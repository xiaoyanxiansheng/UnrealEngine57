...
protected:
	...
	bool RunTestInternal(TArrayView<const InType> InData, TArrayView<const OutType> ExpectedOutData)
	{
		...
		if constexpr (std::is_same_v<PCGDataType, UPCGPointData> || std::is_same_v<PCGDataType, UPCGPointArrayData>)
		{
			InputData->SetNumPoints(InData.Num()-1);
			MetadataEntryRange = InputData->GetMetadataEntryValueRange();
		}
		...
		for (int32 i = 1; i < InData.Num(); ++i)
		{
			...
			if constexpr (std::is_same_v<PCGDataType, UPCGPointData> || std::is_same_v<PCGDataType, UPCGPointArrayData>)
			{
				MetadataEntryRange[WriteIndex] = EntryKey;
			}
			++WriteIndex;
		}
		...
		if constexpr (std::is_same_v<PCGDataType, UPCGPointData> || std::is_same_v<PCGDataType, UPCGPointArrayData>)
		{
			UTEST_EQUAL("Output has the same number of points", InputData->GetNumPoints(), OutputData->GetNumPoints());
		}
		...
		TConstPCGValueRange<int64> OutMetadataEntryRange;
		if constexpr (std::is_same_v<PCGDataType, UPCGPointData> || std::is_same_v<PCGDataType, UPCGPointArrayData>)
		{
			OutMetadataEntryRange = OutputData->GetConstMetadataEntryValueRange();
		}
		...
		for (int32 i = 1; i < ExpectedOutData.Num(); ++i)
		{
			...
			if constexpr (std::is_same_v<PCGDataType, UPCGPointData> || std::is_same_v<PCGDataType, UPCGPointArrayData>)
			{
				EntryKey = OutMetadataEntryRange[i-1];
			}
...
bool FPCGAttributeCast_PointsIntToDouble::RunTest(const FString& Parameters)
{
	...
	{
		return RunTestInternal<int, double, UPCGPointArrayData>({ 5, 1, 2 }, { 5.0, 1.0, 2.0 });
	}
	else
	{
...
bool FPCGAttributeCast_PointsIntToDoubleToDensity::RunTest(const FString& Parameters)
{
	...
	UPCGBasePointData* InputPoints = PCGTestsCommon::CreateEmptyBasePointData();
	FPCGMetadataAttribute<int32>* Attribute = InputPoints->Metadata->CreateAttribute<int32>(InputAttributeName, 5, true, false);

	InputPoints->SetNumPoints(2);
	TPCGValueRange<int64> MetadataEntryRange = InputPoints->GetMetadataEntryValueRange();

	InputPoints->Metadata->InitializeOnSet(MetadataEntryRange[0]);
	InputPoints->Metadata->InitializeOnSet(MetadataEntryRange[1]);
	Attribute->SetValue(MetadataEntryRange[0], 1);
	Attribute->SetValue(MetadataEntryRange[1], 2);
	...
	const UPCGBasePointData* OutputPoints = Cast<const UPCGBasePointData>(Context->OutputData.TaggedData[0].Data);

	UTEST_NOT_NULL("Output is a point data", OutputPoints);
	UTEST_EQUAL("Output has 2 points", OutputPoints->GetNumPoints(), 2);

	const TConstPCGValueRange<float> DensityRange = OutputPoints->GetConstDensityValueRange();
	UTEST_EQUAL("First point has the right value", DensityRange[0], 1.0f);
	UTEST_EQUAL("Second point has the right value", DensityRange[1], 2.0f);

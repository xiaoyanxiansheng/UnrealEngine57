...
namespace PCGGeneratedResourcesLogging
{
	...
	void LogCleanupLocal(const UPCGComponent* InComponent, bool bRemoveComponents)
	{
#if WITH_EDITOR
		if (!LogEnabled())
		{
		...
		UE_LOG(LogPCG, Log, TEXT("[PCGMANAGEDRESOURCES] [%s/%s] UPCGComponent::CleanupLocal, bRemoveComponents: %d"),
			(InComponent && InComponent->GetOwner()) ? *InComponent->GetOwner()->GetName() : TEXT("MISSINGCOMPONENT"),
			(InComponent && InComponent->GetGraph()) ? *InComponent->GetGraph()->GetName() : TEXT("MISSINGGRAPH"),
			bRemoveComponents);
#endif

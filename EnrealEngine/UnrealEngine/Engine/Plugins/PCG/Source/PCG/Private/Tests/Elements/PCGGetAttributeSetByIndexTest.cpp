...
bool FPCGGetAttributeSetByIndexTest_Basic::RunTest(const FString& Parameters)
{
	...
	UTEST_EQUAL("Double attribute first entry", OutDoubleAttr->GetValueFromItemKey(PCGFirstEntryKey), 2.0);
	UTEST_EQUAL("Double attribute default value was not changed", OutDoubleAttr->GetValueFromItemKey(PCGInvalidEntryKey), 0.0);

	UTEST_EQUAL("Int attribute first entry", OutIntAttr->GetValueFromItemKey(PCGFirstEntryKey), 3);
	UTEST_EQUAL("Int attribute default value was not changed", OutIntAttr->GetValueFromItemKey(PCGInvalidEntryKey), 0);

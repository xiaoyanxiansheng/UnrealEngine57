...
bool FPCGCollapseSubgraphCombinationOfCollapses::RunTest(const FString& Parameters)
{
	...
	auto ExecuteGraph = [&TestData](UPCGGraph* GraphToExecute, double FirstConstant, double SecondConstant) -> TArray<FTransform>
	{
		TSharedPtr<FPCGGraphExecutor> GraphExecutor = MakeShared<FPCGGraphExecutor>();
		...
		const FPCGTaskId FinalTaskId = GraphExecutor->ScheduleGeneric([&OutputTransforms, &GraphExecutor, TaskId, &bDone]() -> bool
		{
			...
			{
				if (const UPCGBasePointData* PointData = Cast<UPCGBasePointData>(OutputData.TaggedData[0].Data))
				{
					OutputTransforms = PointData->GetTransformsCopy();
				}
			}
	...
	auto CompareTransforms = [this](const TArray<FTransform>& TransformsA, const TArray<FTransform>& TransformsB, const FString& Name) -> bool
	{
		UTEST_EQUAL(*FString::Printf(TEXT("%s: Same number of points"), *Name), TransformsB.Num(), TransformsA.Num());
		for (int i = 0; i < TransformsA.Num(); ++i)
		{
			UTEST_EQUAL(*FString::Printf(TEXT("%s: Same position"), *Name), TransformsB[i].GetLocation(), TransformsA[i].GetLocation());
		}
	...
	const TArray<FTransform> FirstRunPoints = ExecuteGraph(UnitTestGraph, FirstConstantValue, SecondConstantValue);
	const TArray<FTransform> SecondRunPoints = ExecuteGraph(UnitTestGraph, FirstConstantValue2, SecondConstantValue);
	const TArray<FTransform> ThirdRunPoints = ExecuteGraph(UnitTestGraph, FirstConstantValue, SecondConstantValue2);
			...
			for (int k = j + 1; k < NodesNum && bSuccess; ++k)
			{
				for (int l = k + 1; l < NodesNum && bSuccess; ++l)
				{
					...
					if (UPCGGraph* Subgraph = FPCGSubgraphHelpers::CollapseIntoSubgraphWithReason(OriginalGraph, NodesToCollapse, {}, OutFailReason))
					{
						...
						const TArray<FTransform> FirstRunCollapsedPoints = ExecuteGraph(OriginalGraph, FirstConstantValue, SecondConstantValue);
						const TArray<FTransform> SecondRunCollapsedPoints = ExecuteGraph(OriginalGraph, FirstConstantValue2, SecondConstantValue);
						const TArray<FTransform> ThirdRunCollapsedPoints = ExecuteGraph(OriginalGraph, FirstConstantValue, SecondConstantValue2);

						bool bLocalSuccess = CompareTransforms(FirstRunPoints, FirstRunCollapsedPoints, Name + TEXT(" First run"));
						bLocalSuccess &= CompareTransforms(SecondRunPoints, SecondRunCollapsedPoints, Name + TEXT(" Second run"));
						bLocalSuccess &= CompareTransforms(ThirdRunPoints, ThirdRunCollapsedPoints, Name + TEXT(" Third run"));

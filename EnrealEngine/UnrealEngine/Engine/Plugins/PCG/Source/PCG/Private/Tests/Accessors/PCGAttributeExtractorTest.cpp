...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PCGAttributeExtractorTest)
...
IMPLEMENT_CUSTOM_SIMPLE_AUTOMATION_TEST(FPCGAttributeStringPropertyExtractorTest, FPCGTestBaseClass, "Plugins.PCG.Accessor.Property.StringExtractor", PCGTestsCommon::TestFlags)
...
bool FPCGAttributeVectorPropertyExtractorTest::RunTest(const FString& Parameters)
{
	...
	auto Verify = [this, &Struct](const auto& ExpectedValue, const FName PropertyName, const FName ExtractorName) -> bool
	{
		...
		UTEST_EQUAL(TEXT("Extractor accessor type"), PCG::Private::MetadataTypes<PropertyType>::Id, ExtractorAccessor->GetUnderlyingType());
...
bool FPCGAttributeRotatorPropertyExtractorTest::RunTest(const FString & Parameters)
{
	...
	auto Verify = [this, &Struct](auto ExpectedValue, const FName PropertyName, const FName ExtractorName) -> bool
	{
		...
		UTEST_TRUE(TEXT("Valid extractor accessor"), (bSuccess && ExtractorAccessor.IsValid()));
		UTEST_TRUE(TEXT("Invalid accessor (ownership moved)"), !Accessor.IsValid());
		UTEST_EQUAL(TEXT("Extractor accessor type"), PCG::Private::MetadataTypes<ExpectedType>::Id, ExtractorAccessor->GetUnderlyingType());
...
bool FPCGAttributeStringPropertyExtractorTest::RunTest(const FString & Parameters)
{
	UPCGAttributeExtractorTestObject* Object = NewObject<UPCGAttributeExtractorTestObject>();
	FPCGMetadataTypesConstantStruct Struct = FPCGAttributeExtractorTestHelpers::CreateStruct(Object);

	auto Verify = [this, &Struct]<typename T>(const T& ExpectedValue, const FName PropertyName, const TArray<FName>& ExtractorNames) -> bool
	{
		using PropertyType = std::decay_t<T>;
		...
		UTEST_EQUAL("Extractor accessor type", PCG::Private::MetadataTypes<PropertyType>::Id, ExtractorAccessor->GetUnderlyingType());
	...
	// "Foo" = 3 characters
	bTestPassed &= Verify(3, PCG_STRUCT_NAME_CHECK(StringValue), {PCGAttributeExtractorConstants::StringLength});
	// "Bar" = 3 characters
	bTestPassed &= Verify(3, PCG_STRUCT_NAME_CHECK(NameValue), {PCGAttributeExtractorConstants::StringLength});
	auto String = FSoftObjectPath(Object).ToString();
	// "/Engine/Transient.PCGAttributeExtractorTestObject_1" = 51 characters
	bTestPassed &= Verify(51, PCG_STRUCT_NAME_CHECK(SoftObjectPathValue), {PCGAttributeExtractorConstants::StringLength});
	// "/Script/PCG.PCGMetadata" = 23 characters
	bTestPassed &= Verify(23, PCG_STRUCT_NAME_CHECK(SoftClassPathValue), {PCGAttributeExtractorConstants::StringLength});

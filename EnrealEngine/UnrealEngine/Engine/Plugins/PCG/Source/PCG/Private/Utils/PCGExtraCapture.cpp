...
{
	if (!InScopedCall.Context || !InScopedCall.Context->GetStack())
	{
		return;
	}
...
PCGUtils::FCallTreeInfo PCGUtils::FExtraCapture::CalculateCallTreeInfo(const IPCGGraphExecutionSource* ExecutionSource, const FPCGStack& RootStack) const
{
	...
	TMap<TObjectKey<const UPCGNode>, TSet<FPCGGraphExecutionInspection::FNodeExecutedNotificationData>> NodeToStacksInWhichNodeExecuted = ExecutionSource->GetExecutionState().GetInspection().GetExecutedNodeStacks();
	...
	auto GetNodePath = [&NodePath, &NodePathLoop, &RootStack](const FPCGStack& Stack)
	{
		...
		for(int StackFrameIndex = RootStack.GetStackFrames().Num(); StackFrameIndex < StackFrames.Num(); ++StackFrameIndex)
		{
			...
			if (const UPCGNode* Node = StackFrame.GetObject_GameThread<UPCGNode>())
			{
				NodePath.Add(Node);
				NodePathLoop.Add(-1);
			}
	...
	for (const auto& NodeToStacks : NodeToStacksInWhichNodeExecuted)
	{
		...
		const TSet<FPCGGraphExecutionInspection::FNodeExecutedNotificationData>& NodeExecutions = NodeToStacks.Value;

		for(const FPCGGraphExecutionInspection::FNodeExecutedNotificationData& NodeExecution : NodeExecutions)
		{
...
PCGUtils::FScopedCall::FScopedCall(const IPCGElement& InOwner, FPCGContext* InContext, FScopedCallOutputDevice& InOutputDevice)
	: Owner(InOwner)
	, Context(InContext)
	...
	, OutputDevice(InOutputDevice)
{
	...
	check(!OutputDevice.ScopedCall);
	OutputDevice.ScopedCall = this;
}
...
PCGUtils::FScopedCall::~FScopedCall()
{
	...
	{
		Context->ExecutionSource->GetExecutionState().GetExtraCapture().Update(*this);
	}
	OutputDevice.ScopedCall = nullptr;
}

PCGUtils::FScopedCallOutputDevice::FScopedCallOutputDevice()
...
void PCGUtils::FScopedCallOutputDevice::Serialize(const TCHAR* V, ELogVerbosity::Type Verbosity, const class FName& Category)
{
	...
	if (ScopedCall)
	{
		ScopedCall->CapturedMessages.Add(PCGUtils::FCapturedMessage{ MessageCounter++, Category, V, Verbosity });
	}
}

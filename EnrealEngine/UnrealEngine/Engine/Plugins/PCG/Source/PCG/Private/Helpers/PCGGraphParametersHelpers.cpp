...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PCGGraphParametersHelpers)
...
namespace PCGGraphParameter::Helpers
{
	...
	void LogGraphParamNamingErrors(const FName IntendedName, const EPropertyBagAlterationResult Result, const FPCGContext* InContext)
	{
		switch (Result)
		{
			case EPropertyBagAlterationResult::PropertyNameEmpty:
				PCGLog::LogErrorOnGraph(LOCTEXT("EmptyPropertyName", "Could not add graph parameter with empty name."), InContext);
				return;
			case EPropertyBagAlterationResult::PropertyNameInvalidCharacters:
				PCGLog::LogErrorOnGraph(FText::Format(LOCTEXT("InvalidCharacters", "Could not add graph parameter '{0}'. It contains invalid characters."), FText::FromName(IntendedName)), InContext);
				return;
			case EPropertyBagAlterationResult::TargetPropertyAlreadyExists:
				PCGLog::LogErrorOnGraph(FText::Format(LOCTEXT("AlreadyExists", "Could not add graph parameter '{0}'. It already exists."), FText::FromName(IntendedName)), InContext);
				return;
			case EPropertyBagAlterationResult::InternalError:
				PCGLog::LogErrorOnGraph(FText::Format(LOCTEXT("InternalError", "Could not add graph parameter '{0}' due to internal problems with the property bag."), FText::FromName(IntendedName)), InContext);
				return;

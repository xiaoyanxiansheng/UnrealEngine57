...
namespace PCGBreakTransformTest
{
	...
	void GenerateSpatialData(PCGTestsCommon::FTestData& TestData)
	{
		...
		{
			UPCGMetadataAccessorHelpers::SetTransformAttributeByMetadataKey(MetadataEntryRange[I], PointData->Metadata, TransformAttribute, RandomTransform(RandomSource));
			UPCGMetadataAccessorHelpers::SetFloatAttributeByMetadataKey(MetadataEntryRange[I], PointData->Metadata, InvalidAttribute, RandomSource.FRand());
		}
	}

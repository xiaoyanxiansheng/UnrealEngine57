...
namespace PCGSubgraphHelpersExtra
{
	...
	*/
	...
	FName FCollapsingInformation::CreateNewCustomPin(UPCGGraph* InGraph, const UPCGPin* InPinToClone, bool bIsInput)
	{
		...
		{
			const FPCGDataTypeIdentifier FirstPinTypeUnion = PinToCloneSettings->GetTypeUnionIDOfIncidentEdges(InPinToClone->Properties.Label);
			if (FirstPinTypeUnion.IsValid())
			{
		...
		{
			NewProperties.AllowedTypes = PinToCloneSettings->GetCurrentPinTypesID(InPinToClone);
		}
...
UPCGNode* FPCGSubgraphHelpers::SpawnNodeAndConnect(UPCGGraph* InGraph, UPCGPin* InUpstreamPin, UPCGPin* InDownstreamPin, TSubclassOf<UPCGSettings> InSettingsClass, const TFunction<bool(UPCGSettings*)>& PostSettingsCreation)
{
	check(InGraph && InUpstreamPin && InDownstreamPin && InSettingsClass);
	UPCGSettings* Settings;
	UPCGNode* NewNode = InGraph->AddNodeOfType(InSettingsClass, Settings);
	
	if (NewNode)
	{
		if (PostSettingsCreation.IsSet() && PostSettingsCreation(Settings))
		{
			NewNode->UpdateAfterSettingsChangeDuringCreation();	
		}
		
		UPCGNode* UpstreamNode = InUpstreamPin->Node;
		UPCGNode* DownstreamNode = InDownstreamPin->Node;
		check(UpstreamNode && DownstreamNode);

#if WITH_EDITOR
		// Put the new node between A & B but make it stay within a radius of B to keep things tidy.
		NewNode->PositionX = (UpstreamNode->PositionX + DownstreamNode->PositionX) / 2;
		NewNode->PositionY = (UpstreamNode->PositionY + DownstreamNode->PositionY) / 2;

		// A hand tweaked distance that keeps it reasonably close.
		constexpr float MaxDistFromB = 200;
		const FVector2D OffsetFromB(NewNode->PositionX - DownstreamNode->PositionX, NewNode->PositionY - DownstreamNode->PositionY);
		const float Dist = OffsetFromB.Length();
		if (Dist > MaxDistFromB)
		{
			const float Scale = MaxDistFromB / Dist;
			NewNode->PositionX = DownstreamNode->PositionX + Scale * OffsetFromB.X;
			NewNode->PositionY = DownstreamNode->PositionY + Scale * OffsetFromB.Y;
		}
#endif // WITH_EDITOR

		// Connect by default the first in and out pins of the new node.
		if (ensure(!NewNode->GetInputPins().IsEmpty()))
		{
			check(NewNode->GetInputPins()[0]);
			const FName NewNodeInputPinLabel = NewNode->GetInputPins()[0]->Properties.Label;
			UpstreamNode->AddEdgeTo(InUpstreamPin->Properties.Label, NewNode, NewNodeInputPinLabel);
		}

		if (ensure(!NewNode->GetOutputPins().IsEmpty()))
		{
			check(NewNode->GetOutputPins()[0]);
			const FName NewNodeOutputPinLabel = NewNode->GetOutputPins()[0]->Properties.Label;
			NewNode->AddEdgeTo(NewNodeOutputPinLabel, DownstreamNode, InDownstreamPin->Properties.Label);
		}
	}

...
{
	template <typename T>
	bool ApplyCompare(const T& Input1, const T& Input2, EPCGMetadataCompareOperation Operation, double Tolerance = UE_DOUBLE_SMALL_NUMBER)
	{
		if (Operation == EPCGMetadataCompareOperation::Equal)
		{
			if constexpr (PCG::Private::MetadataTraits<T>::IsFloatingPoint)
			{
				return PCG::Private::MetadataTraits<T>::Equal(Input1, Input2, Tolerance);
			}
			else
			{
...
{
	return FPCGPreConfiguredSettingsInfo::PopulateFromEnum<EPCGMetadataCompareOperation>();
}
#endif // WITH_EDITOR
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FPCGMetadataCompareElement::Execute);

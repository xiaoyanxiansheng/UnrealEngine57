...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PCGSkinnedMeshInstanceDataPackerBase)

void UPCGSkinnedMeshInstanceDataPackerBase::PackInstances_Implementation(FPCGContext& Context, const UPCGSpatialData* InSpatialData, const FPCGSkinnedMeshInstanceList& InstanceList, FPCGSkinnedMeshPackedCustomData& OutPackedCustomData) const
{
	PCGE_LOG_C(Error, GraphAndLog, &Context, NSLOCTEXT("PCGSkinnedMeshInstanceDataPackerBase", "SkinnedMeshInstanceDataPackerBaseFailed", "Unable to execute SkinnedMeshInstanceDataPacker pure virtual base function, override the PackInstances function or use a default implementation."));
}

bool UPCGSkinnedMeshInstanceDataPackerBase::AddTypeToPacking(int TypeId, FPCGSkinnedMeshPackedCustomData& OutPackedCustomData) const
{
	switch (TypeId)
	{
...
void UPCGSkinnedMeshInstanceDataPackerBase::PackCustomDataFromAttributes(const FPCGSkinnedMeshInstanceList& InstanceList, const UPCGMetadata* Metadata, const TArray<FName>& AttributeNames, FPCGSkinnedMeshPackedCustomData& OutPackedCustomData) const
{
	if (!Metadata)
	{
...
void UPCGSkinnedMeshInstanceDataPackerBase::PackCustomDataFromAttributes(const FPCGSkinnedMeshInstanceList& InstanceList, const TArray<const FPCGMetadataAttributeBase*>& Attributes, FPCGSkinnedMeshPackedCustomData& OutPackedCustomData) const
{
	const UPCGPointData* PointData = InstanceList.PointData.Get();
	for (const int32 Index : InstanceList.InstancePointIndices)
	{
...
void UPCGSkinnedMeshInstanceDataPackerBase::PackCustomDataFromAccessors(const FPCGSkinnedMeshInstanceList& InstanceList, TArray<TUniquePtr<const IPCGAttributeAccessor>> Accessors, TArray<TUniquePtr<const IPCGAttributeAccessorKeys>> AccessorKeys, FPCGSkinnedMeshPackedCustomData& OutPackedCustomData) const
{
	const int32 TotalNumCustomDataFloats = OutPackedCustomData.NumCustomDataFloats * InstanceList.Instances.Num();

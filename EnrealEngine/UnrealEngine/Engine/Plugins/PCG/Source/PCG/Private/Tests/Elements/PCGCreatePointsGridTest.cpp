...
bool FPCGCreatePointsGridTest_CellCenter::RunTest(const FString& Parameters)
{
	...
	const TConstPCGValueRange<FTransform> Transforms = OutPointData->GetConstTransformValueRange();

	UTEST_EQUAL("Output point count", Transforms.Num(), 100);

	UTEST_EQUAL("Point 0 has the correct transform,", Transforms[0], FTransform(FRotator::ZeroRotator, FVector(-450.0, -450.0, 0), FVector::One()));
	UTEST_EQUAL("Point 23 has the correct transform,", Transforms[23], FTransform(FRotator::ZeroRotator, FVector(-150.0, -250.0, 0), FVector::One()));
	UTEST_EQUAL("Point 65 has the correct transform,", Transforms[65], FTransform(FRotator::ZeroRotator, FVector(50.0, 150.0, 0), FVector::One()));
	UTEST_EQUAL("Point 92 has the correct transform,", Transforms[92], FTransform(FRotator::ZeroRotator, FVector(-250.0, 450.0, 0), FVector::One()));
...
bool FPCGCreatePointsGridTest_CellCorner::RunTest(const FString& Parameters)
{
	...
	UTEST_EQUAL("Output point count", Transforms.Num(), 242);

	UTEST_EQUAL("Point 0 has the correct transform,", Transforms[0], FTransform(FRotator::ZeroRotator, FVector(-500.0, -500.0, -50.0), FVector::One()));
	UTEST_EQUAL("Point 23 has the correct transform,", Transforms[23], FTransform(FRotator::ZeroRotator, FVector(-400.0, -300.0, -50.0), FVector::One()));
	UTEST_EQUAL("Point 65 has the correct transform,", Transforms[65], FTransform(FRotator::ZeroRotator, FVector(500.0, 0.0, -50.0), FVector::One()));
	UTEST_EQUAL("Point 92 has the correct transform,", Transforms[92], FTransform(FRotator::ZeroRotator, FVector(-100.0, 300.0, -50.0), FVector::One()));
	UTEST_EQUAL("Point 145 has the correct transform,", Transforms[145], FTransform(FRotator::ZeroRotator, FVector(-300.0, -300.0, 50.0), FVector::One()));
...
bool FPCGCreatePointsGridTest_XYCenter::RunTest(const FString& Parameters)
{
	...
	UTEST_EQUAL("Output point count", Transforms.Num(), 4);

	UTEST_EQUAL("Point 0 has the correct transform,", Transforms[0], FTransform(FRotator::ZeroRotator, FVector(-50.0, -50.0, 0.0), FVector::One()));
	UTEST_EQUAL("Point 1 has the correct transform,", Transforms[1], FTransform(FRotator::ZeroRotator, FVector(50.0, -50.0, 0.0), FVector::One()));
	UTEST_EQUAL("Point 2 has the correct transform,", Transforms[2], FTransform(FRotator::ZeroRotator, FVector(-50.0, 50.0, 0.0), FVector::One()));
	UTEST_EQUAL("Point 3 has the correct transform,", Transforms[3], FTransform(FRotator::ZeroRotator, FVector(50.0, 50.0, 0.0), FVector::One()));
...
bool FPCGCreatePointsGridTest_XYCorner::RunTest(const FString& Parameters)
{
	...
	UTEST_EQUAL("Output point count", Transforms.Num(), 9);

	UTEST_EQUAL("Point 1 has the correct transform,", Transforms[1], FTransform(FRotator::ZeroRotator, FVector(0.0, -100.0, 0.0), FVector::One()));
	UTEST_EQUAL("Point 3 has the correct transform,", Transforms[3], FTransform(FRotator::ZeroRotator, FVector(-100.0, 0.0, 0.0), FVector::One()));
	UTEST_EQUAL("Point 5 has the correct transform,", Transforms[5], FTransform(FRotator::ZeroRotator, FVector(100.0, 0.0, 0.0), FVector::One()));
	UTEST_EQUAL("Point 7 has the correct transform,", Transforms[7], FTransform(FRotator::ZeroRotator, FVector(0.0, 100.0, 0.0), FVector::One()));
...
bool FPCGCreatePointsGridTest_XZCenter::RunTest(const FString& Parameters)
{
	...
	UTEST_EQUAL("Point 0 has the correct transform,", Transforms[0], FTransform(FRotator::ZeroRotator, FVector(-50.0, 0.0, -50.0), FVector::One()));
	UTEST_EQUAL("Point 1 has the correct transform,", Transforms[1], FTransform(FRotator::ZeroRotator, FVector(50.0, 0.0, -50.0), FVector::One()));
	UTEST_EQUAL("Point 2 has the correct transform,", Transforms[2], FTransform(FRotator::ZeroRotator, FVector(-50.0, 0.0, 50.0), FVector::One()));
	UTEST_EQUAL("Point 3 has the correct transform,", Transforms[3], FTransform(FRotator::ZeroRotator, FVector(50.0, 0.0, 50.0), FVector::One()));
...
bool FPCGCreatePointsGridTest_XZCorner::RunTest(const FString& Parameters)
{
	...
	UTEST_EQUAL("Point 1 has the correct transform,", Transforms[1], FTransform(FRotator::ZeroRotator, FVector(0.0, 0.0, -100.0), FVector::One()));
	UTEST_EQUAL("Point 3 has the correct transform,", Transforms[3], FTransform(FRotator::ZeroRotator, FVector(-100.0, 0.0, 0.0), FVector::One()));
	UTEST_EQUAL("Point 5 has the correct transform,", Transforms[5], FTransform(FRotator::ZeroRotator, FVector(100.0, 0.0, 0.0), FVector::One()));
	UTEST_EQUAL("Point 7 has the correct transform,", Transforms[7], FTransform(FRotator::ZeroRotator, FVector(0.0, 0.0, 100.0), FVector::One()));
...
bool FPCGCreatePointsGridTest_YZCenter::RunTest(const FString& Parameters)
{
	...
	UTEST_EQUAL("Point 0 has the correct transform,", Transforms[0], FTransform(FRotator::ZeroRotator, FVector(0.0, -50.0, -50.0), FVector::One()));
	UTEST_EQUAL("Point 1 has the correct transform,", Transforms[1], FTransform(FRotator::ZeroRotator, FVector(0.0, 50.0, -50.0), FVector::One()));
	UTEST_EQUAL("Point 2 has the correct transform,", Transforms[2], FTransform(FRotator::ZeroRotator, FVector(0.0, -50.0, 50.0), FVector::One()));
	UTEST_EQUAL("Point 3 has the correct transform,", Transforms[3], FTransform(FRotator::ZeroRotator, FVector(0.0, 50.0, 50.0), FVector::One()));
...
bool FPCGCreatePointsGridTest_YZCorner::RunTest(const FString& Parameters)
{
	...
	UTEST_EQUAL("Point 1 has the correct transform,", Transforms[1], FTransform(FRotator::ZeroRotator, FVector(0.0, 0.0, -100.0), FVector::One()));
	UTEST_EQUAL("Point 3 has the correct transform,", Transforms[3], FTransform(FRotator::ZeroRotator, FVector(0.0, -100.0, 0.0), FVector::One()));
	UTEST_EQUAL("Point 5 has the correct transform,", Transforms[5], FTransform(FRotator::ZeroRotator, FVector(0.0, 100.0, 0.0), FVector::One()));
	UTEST_EQUAL("Point 7 has the correct transform,", Transforms[7], FTransform(FRotator::ZeroRotator, FVector(0.0, 0.0, 100.0), FVector::One()));
...
bool FPCGCreatePointsGridTest_OversizedCell::RunTest(const FString& Parameters)
{
	...
	UTEST_EQUAL("Output point count", Transforms.Num(), 1);

	UTEST_EQUAL("Point 0 has the correct transform,", Transforms[0], FTransform(FRotator::ZeroRotator, FVector(0.0, 0.0, 0.0), FVector::One()));
...
bool FPCGCreatePointsGridTest_LocalCenter::RunTest(const FString& Parameters)
{
	...
	{
		UTEST_TRUE(FString::Format(TEXT("Point {0}, {1} is within the volume"), { i }), TestBounds.IsInsideOrOn(Transforms[i].GetLocation()));
	}

	UTEST_EQUAL("Point 0 has the correct transform,", Transforms[0], FTransform(FRotator::ZeroRotator, FVector(50.0, 50.0, 0.0), FVector::One()));
	UTEST_EQUAL("Point 1 has the correct transform,", Transforms[1], FTransform(FRotator::ZeroRotator, FVector(150.0, 50.0, 0.0), FVector::One()));
	UTEST_EQUAL("Point 2 has the correct transform,", Transforms[2], FTransform(FRotator::ZeroRotator, FVector(50.0, 150.0, 0.0), FVector::One()));
	UTEST_EQUAL("Point 3 has the correct transform,", Transforms[3], FTransform(FRotator::ZeroRotator, FVector(150.0, 150.0, 0.0), FVector::One()));
...
bool FPCGCreatePointsGridTest_LocalCorner::RunTest(const FString& Parameters)
{
	...
	UTEST_EQUAL("Output point count", Transforms.Num(), 18);

	// Check to see if the position of the points are within the bounds of the volume
	for (int i = 0; i < Transforms.Num(); ++i)
	{
		UTEST_TRUE(FString::Format(TEXT("Point {0} is within the volume"), { i }), TestBounds.IsInsideOrOn(Transforms[i].GetLocation()));
	}

	UTEST_EQUAL("Point 1 has the correct transform,", Transforms[1], FTransform(FRotator::ZeroRotator, FVector(100.0, 0.0, -50.0), FVector::One()));
	UTEST_EQUAL("Point 3 has the correct transform,", Transforms[3], FTransform(FRotator::ZeroRotator, FVector(0.0, 100.0, -50.0), FVector::One()));
	UTEST_EQUAL("Point 5 has the correct transform,", Transforms[5], FTransform(FRotator::ZeroRotator, FVector(200.0, 100.0, -50.0), FVector::One()));
	UTEST_EQUAL("Point 11 has the correct transform,", Transforms[11], FTransform(FRotator::ZeroRotator, FVector(200.0, 0.0, 50.0), FVector::One()));

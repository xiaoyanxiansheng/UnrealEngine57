...
namespace PCGAttributeExtractor
{
	...
	TUniquePtr<IPCGAttributeAccessor> CreateStringExtractor(TUniquePtr<IPCGAttributeAccessor> InAccessor, const FName Name, bool& bOutSuccess)
	{
		bOutSuccess = true;

		if (Name == PCGAttributeExtractorConstants::StringLength)
		{
			if constexpr (std::is_same_v<StringType, FString>)
			{
				return MakeUnique<FPCGChainAccessor<int32, FString>>(MoveTemp(InAccessor),
					[](const FString& Value) -> int32 { return Value.Len(); });
			}
			else
			{
				return MakeUnique<FPCGChainAccessor<int32, StringType>>(MoveTemp(InAccessor),
					[](const StringType& Value) -> int32 { return Value.ToString().Len(); });
			}
		}
	...
	template TUniquePtr<IPCGAttributeAccessor> CreateStringExtractor<FString>(TUniquePtr<IPCGAttributeAccessor> InAccessor, FName Name, bool& bOutSuccess);
	template TUniquePtr<IPCGAttributeAccessor> CreateStringExtractor<FName>(TUniquePtr<IPCGAttributeAccessor> InAccessor, FName Name, bool& bOutSuccess);
	template TUniquePtr<IPCGAttributeAccessor> CreateStringExtractor<FSoftObjectPath>(TUniquePtr<IPCGAttributeAccessor> InAccessor, FName Name, bool& bOutSuccess);
	template TUniquePtr<IPCGAttributeAccessor> CreateStringExtractor<FSoftClassPath>(TUniquePtr<IPCGAttributeAccessor> InAccessor, FName Name, bool& bOutSuccess);
}

...
public:
	...
	/** Since we use the Display name as the output pins for enums, this can be different in non-editor build. We need to keep a mapping between the enum value and its associated pin name. */
	UPROPERTY()
	mutable TMap<int64, FName> EnumValueToPinLabelMapping;

	/** Cached pin labels for use during the selection process. */
	UPROPERTY(Transient)
	TArray<FName> CachedPinLabels;

	/** To be called after the settings are modified directly in native code. */
	PCG_API void UpdateSettings(); 
...
protected:
	...
	void CacheEnumValueToPinLabelsMapping() const;
};

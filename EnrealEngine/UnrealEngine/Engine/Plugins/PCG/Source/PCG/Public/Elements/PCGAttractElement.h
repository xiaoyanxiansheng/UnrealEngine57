...
public:
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Settings, meta = (ClampMin = "-1.0", ClampMax = "1.0", EditCondition = "!bUseSourceWeight && !bUseTargetWeight", PCG_Overridable))
	double Weight = 1.0;
...
namespace PCGAttractElement
{
	struct FAttractState
	{
		...
		FAttractState() = default;
		~FAttractState() = default;
		PRAGMA_ENABLE_DEPRECATION_WARNINGS

		const UPCGBasePointData* TargetData = nullptr;
		const UPCGBasePointData* SourceData = nullptr;
		UPCGBasePointData* OutputData = nullptr;

		UE_DEPRECATED(5.6, "Use TargetData instead")
		const UPCGPointData* TargetPointData = nullptr;
		UE_DEPRECATED(5.6, "Use SourceData instead")

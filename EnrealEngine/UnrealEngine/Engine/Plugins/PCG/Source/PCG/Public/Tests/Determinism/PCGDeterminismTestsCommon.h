...
struct FDeterminismTestResult
{
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Determinism)
	FPCGDataTypeIdentifier DataTypesTested = EPCGDataType::None;
...
namespace PCGDeterminismTests
{
	...
	struct FNodeAndOptions
	{
		...
		TArray<TArray<FPCGDataTypeIdentifier>> BaseOptionsByPin;
	};
	...
	PCG_API void RunDeterminismTest(const UPCGGraph* InPCGGraph, const IPCGGraphExecutionSource* InSource, FDeterminismTestResult& OutResult);
#endif
	...
	void AddRandomizedInputData(PCGTestsCommon::FTestData& TestData, FPCGDataTypeIdentifier DataType, const FName& PinName = Defaults::TestPinName);
	...
	TArray<FPCGDataTypeIdentifier> FilterTestableDataTypes(FPCGDataTypeIdentifier AllowedDataTypes, int32 NumMultipleInputs = 1);
	/** Given input pins, updates an array with a permutation base of options */
	void RetrieveBaseOptionsPerPin(TArray<TArray<FPCGDataTypeIdentifier>>& InBaseOptionsArray,

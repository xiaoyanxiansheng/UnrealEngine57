...
namespace PCGHelpers
{
	...
	/** Returns true if it's running a World Partition builder commandlet. */
	PCG_API bool IsInWorldPartitionBuilderCommandletContext();
#endif
	...
	*/
	...
	PCG_API bool IsServer(const UWorld* InWorld);
	PCG_API bool IsDedicatedServer(const UWorld* InWorld);
	PCG_API bool IsListenServer(const UWorld* InWorld);
	...
	// Create keys for a hypothetical attribute in the 'Elements' domain, get its count, and create a const value accessor for retrieving it
	// Only available if the data supports the Elements domain 
	PCG_API int32 GetNumberOfElements(const UPCGData* InData);

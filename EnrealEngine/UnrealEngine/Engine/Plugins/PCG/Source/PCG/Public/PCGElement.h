...
enum class EPCGElementExecutionLoopMode : uint8
{
	...
	MatchingPrimaryPins,
	/** Loops on matching indices on required pin(s), but the second+ pin can have a single entry. E.g. 1:1, N:N or N:1 */
...
private:
	...
	/** Poll GPU overridden attributes for readback. Replaces the GPU data with a CPU param data upon successful readback. Returns false until readback is complete. */
	bool ReadbackGPUDataForOverrides(FPCGContext* Context) const;
};

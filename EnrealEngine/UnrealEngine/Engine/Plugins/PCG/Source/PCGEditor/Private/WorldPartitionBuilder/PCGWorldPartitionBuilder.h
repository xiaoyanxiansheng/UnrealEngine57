...
struct FPCGWorldPartitionBuilderArgs
{
	...
	/** Experimental feature that will try and revert PCG Component Actors which haven't changed from their previous generation. */
	bool bRevertUnchangedActors = false;

	/** Outputs that can change on every generation but do not have an impact on final result should be listed here so they can be ignored when comparing results for revert. */
...
private:
	...
	/** Revertable Actors */
	TMap<TSoftObjectPtr<AActor>, FPCGCrc> RevertableActors;

	/** Reverted Actors : when using the builder in per cell iteration, it is possible that one cell reverts an actor because it doesn't modify it but the next one will so keep track for stats */
	TSet<TSoftObjectPtr<AActor>> RevertedActors;

	/** Final stats */
	int32 NumNewPackages = 0;
	int32 NumModifiedPackages = 0;
	int32 NumDeletedPackages = 0;
	int32 NumRevertedPackages = 0;

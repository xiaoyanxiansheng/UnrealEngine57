...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PCGEditorSettings)
	...
	: Super(ObjectInitializer)
{
	...
	Polygon2DDataPinColor = FLinearColor(0.055f, 0.65f, 0.515f);
...
FLinearColor UPCGEditorSettings::GetColor(UPCGSettings* Settings) const
{
	...
	switch (Settings->GetType())
	{
		...
		{
			// For graph parameters, use the output pin
			const TArray<FPCGPinProperties> Properties = Settings->OutputPinProperties();
			return !Properties.IsEmpty() ? FPCGModule::GetConstDataTypeRegistry().GetPinColor(Properties[0].AllowedTypes) : GraphParametersNodeColor;
		}

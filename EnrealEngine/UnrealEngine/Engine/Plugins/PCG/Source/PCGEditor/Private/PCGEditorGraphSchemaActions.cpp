...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PCGEditorGraphSchemaActions)

#define LOCTEXT_NAMESPACE "PCGEditorGraphSchemaActions"

FText FPCGEditorGraphSchemaActionBase::GetMenuDescriptionOverride() const
{
	return GetMenuDescription();
}

FSlateBrush const* FPCGEditorGraphSchemaActionBase::GetPaletteIcon() const
{
	return PaletteIcon.Get();
}

void FPCGEditorGraphSchemaActionBase::SetPaletteIcon(TSharedPtr<FSlateBrush>& InPaletteIcon)
{
	PaletteIcon = InPaletteIcon;
}
...
FSlateBrush const* FPCGEditorGraphSchemaAction_NewNativeElement::GetPaletteIcon() const
{
	...
	if (!PaletteIcon && PreconfiguredInfo.ActionIconDescriptor)
	{
		...
		// Brush is expected to be set and have a valid resource.
		if (BaseBrush && ensure(BaseBrush->IsSet() && BaseBrush->GetResourceName() != NAME_None))
		{
...
void FPCGEditorGraphSchemaAction_NewGetParameterElement::PostCreation(UPCGNode* NewNode)
{
	...
	NewNode->SetNodeTitle(Settings->PropertyName, /*bApplySanitization=*/false);

	// We need to set the settings to update the pins.
	NewNode->SetSettingsInterface(Settings);
}

FText FPCGEditorGraphSchemaAction_NewSettingsElement::GetMenuDescriptionOverride() const
{
	switch (Behavior)
	{
		case EPCGEditorNewSettingsBehavior::ForceCopy:
			return FText::Format(LOCTEXT("AssetMenuDescriptionCopyQualifierFormat", "{0} (copy)"), GetMenuDescription());
		case EPCGEditorNewSettingsBehavior::ForceInstance:
			return FText::Format(LOCTEXT("AssetMenuDescriptionInstanceQualifierFormat", "{0} (instance)"), GetMenuDescription());
		case EPCGEditorNewSettingsBehavior::Normal:
			break;
	}

	return Super::GetMenuDescriptionOverride();
}
...
FSlateBrush const* FPCGEditorGraphSchemaAction_NewSettingsElement::GetPaletteIcon() const
{
	// @todo_pcg: Enable when icons exist for these types.
	// switch (Behavior)
	// {
	// 	case EPCGEditorNewSettingsBehavior::ForceCopy:
...
UEdGraphNode* FPCGEditorGraphSchemaAction_NewBlueprintElement::PerformAction(UEdGraph* ParentGraph, UEdGraphPin* FromPin, const FVector2f& Location, bool bSelectNewNode)
{
	...
	DefaultBlueprintSettings->SetBlueprintElementType(BlueprintClass, ElementInstance);
...
FSlateBrush const* FPCGEditorGraphSchemaAction_NewBlueprintElement::GetPaletteIcon() const
{
	return FAppStyle::GetBrush("Icons.Blueprint");
}
...
UEdGraphNode* FPCGEditorGraphSchemaAction_NewNamedRerouteUsage::PerformAction(class UEdGraph* ParentGraph, UEdGraphPin* FromPin, const FVector2f& Location, bool bSelectNewNode)
{
	...
	NewPCGNode->SetNodeTitle(DeclarationNode->NodeTitle, /*bApplySanitization=*/false);

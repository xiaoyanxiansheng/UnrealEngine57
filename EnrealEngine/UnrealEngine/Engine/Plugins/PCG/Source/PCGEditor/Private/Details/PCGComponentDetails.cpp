...
void FPCGComponentDetails::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	if (Algo::NoneOf(ObjectsBeingCustomized, [](const TWeakObjectPtr<UObject>& Object) { return !Object.IsValid() || Object.Get()->IsA<UPCGComponent>(); }))
	{
		...
		Objects.Reserve(SelectedComponents.Num());
		Algo::Transform(SelectedComponents, Objects, [](const TWeakObjectPtr<UPCGComponent>& Component) -> UObject*
		{
			return Component.IsValid() ? CastChecked<UObject>(Component.Get()->GetGraphInstance(), ECastCheckedType::NullAllowed) : nullptr;
		});
	...
	NewRow.ValueContent()
		...
		[
			...
			[
				...
				.Visibility(this, &FPCGComponentDetails::ClearPCGLinkButtonVisible)
				[
					SNew(STextBlock)
					.Font(IDetailLayoutBuilder::GetDetailFont())
...
EVisibility FPCGComponentDetails::ClearPCGLinkButtonVisible() const
{
	for (const TWeakObjectPtr<UPCGComponent>& Component : SelectedComponents)
	{
		if (Component.IsValid() && !Component->IsManagedByRuntimeGenSystem())
...
FReply FPCGComponentDetails::OnGenerateClicked()
{
	...
	for (TWeakObjectPtr<UPCGComponent>& SelectedComponentPtr : SelectedComponents)
	{
		UPCGComponent* PCGComponent = SelectedComponentPtr.Get();

		if (IsValid(PCGComponent) && !PCGComponent->IsGenerating() && !PCGComponent->IsManagedByRuntimeGenSystem())
		{
			// Flush cache if needed
			if (bIsControlDown && !bFlushedCache)
			{
				if (UPCGSubsystem* PCGSubsystem = PCGComponent->GetSubsystem())
				{
			...
			PCGComponent->Generate(/*bForce=*/bIsControlDown);
		}
	}

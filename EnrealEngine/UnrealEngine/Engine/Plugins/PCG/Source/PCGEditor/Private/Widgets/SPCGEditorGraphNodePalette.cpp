...
void SPCGEditorGraphNodePalette::CollectAllActions(FGraphActionListBuilderBase& OutAllActions)
{
	...
	FPCGEditor* Editor = PCGEditor.Pin().Get();
	check(Editor);
	FPCGActionsFilter ActionFilter(Editor->GetPCGEditorGraph(), ElementType & PCGSchema->GetElementTypeFiltering(), PCGSchema->GetGraphEditorCustomization(Editor->GetPCGEditorGraph()));
	
	PCGSchema->GetPaletteActions(ActionMenuBuilder, ActionFilter);

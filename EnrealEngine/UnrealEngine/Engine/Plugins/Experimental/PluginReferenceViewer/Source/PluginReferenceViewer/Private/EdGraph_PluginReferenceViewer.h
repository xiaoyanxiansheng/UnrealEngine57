...
{
	explicit FPluginDependsNode(const FPluginIdentifier& InIdentifier, bool InMissing)
	...
	TArray<FPluginDependsNode*> Dependencies; // Raw pointer to another node in CachedDependsNodes, same lifetime
	TArray<FPluginDependsNode*> Referencers; // Raw pointer to another node in CachedDependsNodes, same lifetime
	bool bMissing = false;
};
...
private:
	...
	// Mapping of plugins which were seen when traversing the dependency graph of active plugins, whether or not they exist in the plugin manager 
	TMap<FPluginIdentifier, TUniquePtr<FPluginDependsNode>> CachedDependsNodes;
	// Mapping of plugins which actually exist - may have fewer keys than CachedDependsNodes
	TMap<FPluginIdentifier, const TSharedPtr<IPlugin>> PluginMap;
	TMap<FPluginIdentifier, FPluginStats> StatsMap;

	// Current set of referencers of the graph root plugin(s) to be displayed, expanded to configured depth
	TMap<FPluginIdentifier, FPluginReferenceNodeInfo> ReferencerNodeInfos;
	// Current set of dependencies of the graph root plugin(s) to be displayed, expanded to configured depth 

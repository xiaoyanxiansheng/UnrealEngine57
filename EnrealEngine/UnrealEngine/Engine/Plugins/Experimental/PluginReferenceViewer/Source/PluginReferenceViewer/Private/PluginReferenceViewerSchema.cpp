...
{
	if (TSharedPtr<IPlugin> Plugin = GetDroppedPlugin(Assets))
	{
		FPluginReferenceViewerModule::Get().OpenPluginReferenceViewerUI(MoveTemp(Plugin).ToSharedRef());
	}
}
...
void UPluginReferenceViewerSchema::GetAssetsGraphHoverMessage(const TArray<FAssetData>& Assets, const UEdGraph* HoverGraph, FString& OutTooltipText, bool& OutOkIcon) const
{
	if (TSharedPtr<IPlugin> Plugin = GetDroppedPlugin(Assets))
	{
		OutTooltipText = FText::FormatOrdered(NSLOCTEXT("PluginReferenceViewerSchema", "RecenterGraphOnPlugin", "Re-center graph on plugin {0}."), FText::FromString(Plugin->GetFriendlyName())).ToString();
		OutOkIcon = true;
	}
	else
	{
		OutTooltipText = NSLOCTEXT("PluginReferenceViewerSchema", "NoPlugin", "Asset does not belong to a plugin.").ToString();
		OutOkIcon = false;
	}
}

TSharedPtr<IPlugin> UPluginReferenceViewerSchema::GetDroppedPlugin(const TArray<FAssetData>& Assets)
{
	for (const FAssetData& AssetData : Assets)
	{
		const FNameBuilder PackageName(AssetData.PackageName);
		FStringView PluginName = FPathViews::GetMountPointNameFromPath(PackageName);

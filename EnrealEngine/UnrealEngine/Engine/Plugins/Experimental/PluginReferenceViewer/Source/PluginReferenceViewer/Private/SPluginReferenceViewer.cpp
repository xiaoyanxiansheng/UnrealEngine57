...
void SPluginReferenceViewer::OnShowPluginInContentBrowser()
{
	TSharedPtr<const IPlugin> Plugin;
	...
	if (Plugin != nullptr)
	{
		...
		FStringView MountedPathView(MountedPath);
		
		// Remove any trailing slashes
		if (MountedPathView.EndsWith('/'))
		{
			MountedPathView.LeftChopInline(1);
		}

		FName VirtualPath;
		ContentBrowserData->ConvertInternalPathToVirtual(MountedPathView, VirtualPath);

		if (!VirtualPath.IsNone())
		{
			FContentBrowserItem Item = ContentBrowserData->GetItemAtPath(VirtualPath, EContentBrowserItemTypeFilter::IncludeAll);
			if (Item.IsValid())
			{
				TArray<FContentBrowserItem> Items = { Item };
				IContentBrowserSingleton& ContentBrowser = FModuleManager::Get().GetModuleChecked<FContentBrowserModule>("ContentBrowser").Get();
				ContentBrowser.SyncBrowserToItems(Items);
			}	
		}
	}
}
...
FText SPluginReferenceViewer::GetPluginStatsText() const
{
	...
	const UEdGraphNode_PluginReference* PluginReferenceNode = Cast<const UEdGraphNode_PluginReference>(SelectedNode);

	if (PluginReferenceNode == nullptr)
	{
		return FText::GetEmpty();
	}

	const FPluginStats & PluginStats = GraphObj->GetPluginStats(PluginReferenceNode->GetIdentifier());

	return FText::Format(LOCTEXT("PluginInfoText", "Name: {0}\nContent Size: {1}\nTotal Content Size With Dependencies: {2}\nDependencies: {3}\nReferencers: {4}"), 
		{ FText::FromString(PluginReferenceNode->GetIdentifier().ToString()), GetSizeInBytesAsBiggerUnits(PluginStats.Size), GetSizeInBytesAsBiggerUnits(PluginStats.SizeWithDependencies), PluginStats.Dependencies, PluginStats.Referencers });
}
...
void SPluginReferenceViewer::RegisterActions()
{
	...
	PluginReferenceViewerActions->MapAction(
		FPluginReferenceViewerCommands::Get().ShowInContentBrowser,
		FExecuteAction::CreateSP(this, &SPluginReferenceViewer::OnShowPluginInContentBrowser),
		FCanExecuteAction::CreateSP(this, &SPluginReferenceViewer::HasAtLeastOneRealNodeSelected));

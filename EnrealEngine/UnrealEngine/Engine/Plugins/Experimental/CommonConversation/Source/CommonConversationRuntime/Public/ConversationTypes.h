...
public:
	...
	UE_API const FConversationParticipantEntry* GetParticipant(FGameplayTag ParticipantID) const;

	UE_API UConversationParticipantComponent* GetParticipantComponent(FGameplayTag ParticipantID) const;

	UE_API bool Contains(AActor* PotentialParticipant) const;
...
public:
	static UE_API const FConversationChoiceReference Empty;
...
public:
	static UE_API const FAdvanceConversationRequest Any;
...
public:
	...
	UE_API FAdvanceConversationRequest ToAdvanceConversationRequest(const TArray<FConversationNodeParameterPair>& InUserParameters = TArray<FConversationNodeParameterPair>()) const;
...
{
	UE_API void AddChoice(const FConversationContext& InContext, FClientConversationOptionEntry&& InChoice);

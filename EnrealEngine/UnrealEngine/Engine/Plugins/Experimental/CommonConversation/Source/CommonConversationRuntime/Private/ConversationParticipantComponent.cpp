...
void UConversationParticipantComponent::GetOtherParticipantActors(TArray<AActor*>& OutParticipantActors) const
{
	const TArray<FConversationParticipantEntry>* ConversationParticipantsPtr = nullptr;

	if (GetOwner()->GetLocalRole() == ROLE_Authority)
	{
		if (Auth_CurrentConversation)
		{
			ConversationParticipantsPtr = &Auth_CurrentConversation->GetParticipantList();
		}
	}
	else
	{
		ConversationParticipantsPtr = &LastMessage.Participants.List;
	}

	if (ConversationParticipantsPtr == nullptr)
	{
		return;
	}

	for (const FConversationParticipantEntry& ConversationParticipantEntry : *ConversationParticipantsPtr)
	{
		if (ConversationParticipantEntry.GetParticipantComponent() == this)
		{
			// Ignore self
			continue;
		}

		if (!IsValid(ConversationParticipantEntry.Actor))
		{
			continue;
		}

		OutParticipantActors.Add(ConversationParticipantEntry.Actor);
	}
}

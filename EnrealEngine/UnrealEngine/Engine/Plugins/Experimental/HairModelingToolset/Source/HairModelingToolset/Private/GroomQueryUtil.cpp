	...
	FMeshCardStripSet* CardsOut)
{
	...
	for (int32 GroupIdx = 0; GroupIdx < NumHairGroups; ++GroupIdx)
	{
		...
		const uint32 CardUVIndex = FHairCardsGeometry::FUVChannel::GetCardUVIndex();
		...
		for (int32 k = 0; k < NumVerts; ++k)
		{
			...
			int32 NewUVID = UVs->AppendElement( FVector2f(CardGeo.UVs[CardUVIndex].UV[k].X, CardGeo.UVs[CardUVIndex].UV[k].Y) );
			MapU[k] = NewUVID;
		}

...
public:
	...
	static UE_API OSCPacketType GetType(const void* Data, int32 DataLength);

	/* Helper to construct a packet (and in the case of a bundle, any sub-packets) from the specified buffer */
	static UE_API TSharedPtr<FBackChannelOSCPacket> CreateFromBuffer(const void* Data, int32 DataLength);
};

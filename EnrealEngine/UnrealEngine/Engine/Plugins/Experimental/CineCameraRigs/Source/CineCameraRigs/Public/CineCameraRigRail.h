...
public:
	UE_API ACineCameraRigRail(const FObjectInitializer& ObjectInitializer);
	...
	UE_API void SetSplineMeshMaterial(UMaterialInterface* InMaterial);

	/* Set texture used in the spline mesh material */
	UFUNCTION(BlueprintSetter)
	UE_API void SetSplineMeshTexture(UTexture2D* InTexture);

	/* Calculate internal velocity at the given position */
	UFUNCTION(BlueprintCallable, Category = "CineCameraRigRail")
	UE_API FVector GetVelocityAtPosition(const float InPosition, const float delta = 0.001) const;

	/* Set drive mode*/
	UFUNCTION(BlueprintSetter)
	UE_API void SetDriveMode(ECineCameraRigRailDriveMode InMode);

	/* Enable display speed heatmap*/
	UFUNCTION(BlueprintSetter)
	UE_API void SetDisplaySpeedHeatmap(bool bEnable);

	/* Returns true if the rig rail is driven by Sequencer */
	UFUNCTION()
	UE_API bool IsSequencerDriven();

	/* Set AbsolutePositionOnRail value*/
	UFUNCTION(BlueprintSetter)
	UE_API void SetAbsolutePositionOnRail(float Value);

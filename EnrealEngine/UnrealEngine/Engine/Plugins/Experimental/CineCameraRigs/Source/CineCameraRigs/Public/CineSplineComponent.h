...
public:
	UE_API UCineSplineComponent(const FObjectInitializer& ObjectInitializer);
	...
	*/
	...
	UE_API void ApplyComponentInstanceData(struct FCineSplineInstanceData* ComponentInstanceData, const bool bPostUCS);
	...
	UE_API void SetFocalLengthAtSplinePoint(const int32 PointIndex, const float Value );

	/** Set aperture metadata at a given spline point*/
	UFUNCTION(BlueprintCallable, Category = "CineSpline")
	UE_API void SetApertureAtSplinePoint(const int32 PointIndex, const float value );

	/** Set focus distance metadata at a given spline point*/
	UFUNCTION(BlueprintCallable, Category = "CineSpline")
	UE_API void SetFocusDistanceAtSplinePoint(const int32 PointIndex, const float value);

	/** Set custom position metadata at a given spline point*/
	UFUNCTION(BlueprintCallable, Category = "CineSpline")
	UE_API void SetAbsolutePositionAtSplinePoint(const int32 PointIndex, const float value);

	/* Set camera rotation metadata at a given spline point*/
	UFUNCTION(BlueprintCallable, Category = "CineSpline")
	UE_API void SetPointRotationAtSplinePoint(const int32 PointIndex, const FQuat value);
	...
	UE_API bool FindSplineDataAtPosition(const float InPosition, int32& OutIndex, const float Tolerance=0.001f) const;

	/* Update spline point data at the given spline point*/
	UFUNCTION(BlueprintCallable, Category = "CineSpline")
	UE_API void UpdateSplineDataAtIndex(const int InIndex, const FCineSplinePointData& InPointData );

	/* Add a new spline point data at the given custom position*/
	UFUNCTION(BlueprintCallable, Category = "CineSpline")
	UE_API void AddSplineDataAtPosition(const float InPosition, const FCineSplinePointData& InPointData);

	/* Get a spline point data at the given custom position*/
	UFUNCTION(BlueprintCallable, Category = "CineSpline")
	UE_API FCineSplinePointData GetSplineDataAtPosition(const float InPosition) const;

	/* Get input key value from custom position */
	UFUNCTION(BlueprintCallable, Category = "CineSpline")
	UE_API float GetInputKeyAtPosition(const float InPosition) const;

	/* Get custom position value at spline input key */
	UFUNCTION(BlueprintCallable, Category = "CineSpline")
	UE_API float GetPositionAtInputKey(const float InKey) const;

	/* Get camera rotation metadata property value along the spline at spline point */
	UFUNCTION(BlueprintCallable, Category = "CineSpline")
	UE_API FQuat GetPointRotationAtSplinePoint(int32 Index) const;

	/* Get camera rotation metadata value along the spline at spline input key*/
	UFUNCTION(BlueprintCallable, Category = "CineSpline")
	UE_API FQuat GetPointRotationAtSplineInputKey(float InKey) const;
	...
	UE_API virtual void UpdateSpline() override;

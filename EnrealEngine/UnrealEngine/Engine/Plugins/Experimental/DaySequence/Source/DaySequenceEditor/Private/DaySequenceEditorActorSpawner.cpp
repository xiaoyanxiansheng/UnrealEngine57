...
#define LOCTEXT_NAMESPACE "DaySequenceEditorActorSpawner"

TSharedRef<IMovieSceneObjectSpawner> FDaySequenceEditorActorSpawner::CreateObjectSpawner()
{
	return MakeShareable(new FDaySequenceEditorActorSpawner);
}

#if WITH_EDITOR

namespace DaySequenceEditorActorSpawnerLocals
{
	void PlaceActorInFrontOfCamera(AActor* ActorCDO)
	{
...
TValueOrError<FNewSpawnable, FText> FDaySequenceEditorActorSpawner::CreateNewSpawnableType(UObject& SourceObject, UMovieScene& OwnerMovieScene, UActorFactory* ActorFactory)
{
...
bool FDaySequenceEditorActorSpawner::CanSetupDefaultsForSpawnable(UObject* SpawnedObject) const
{
	...
	return FDaySequenceActorSpawner::CanSetupDefaultsForSpawnable(SpawnedObject);
}

void FDaySequenceEditorActorSpawner::SetupDefaultsForSpawnable(UObject* SpawnedObject, const FGuid& Guid, const TOptional<FTransformData>& TransformData, TSharedRef<ISequencer> Sequencer, USequencerSettings* Settings)
{
	...
	if (SpawnedActor)
	{
		...
		{
			DaySequenceEditorActorSpawnerLocals::PlaceActorInFrontOfCamera(SpawnedActor);
		}

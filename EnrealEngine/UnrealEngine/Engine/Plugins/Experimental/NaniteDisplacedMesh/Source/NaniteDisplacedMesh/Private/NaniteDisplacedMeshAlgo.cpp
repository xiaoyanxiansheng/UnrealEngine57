...
inline T FallOff1D(const T Value, const T FallOffDist)
{
	...
	{
		return FMath::SmoothStep(T(0.), FallOffDist, Value);
	}
	else if (Value > T(1) - FallOffDist)
	{
		return FMath::SmoothStep(T(0.), FallOffDist, (T(1) - Value));
	}
	...
	EDisplaceNaniteMeshOptions::Type Options )
{
	...
	{
		if (UV.X < DisplacementIndex || UV.X > DisplacementIndex + 1 || UV.Y < 0.0f || UV.Y > 1.0f)
		{
			return FVector3f::ZeroVector;
		}

		DisplacementWeight = FallOff1D(fmodf(UV[0], 1.f), 0.2f) * FallOff1D(fmodf(UV[1], 1.f), 0.2f);
	}
	...
	return Normal * (Displacement * DisplacementWeight);
}
...
)
{
	...
	Nanite::AdaptiveTessellation::TessellateAdaptive( LerpVerts, Indexes, MaterialIndexes, TargetError, TargetError, true,
		[&](const FVector3f& Barycentrics,
			const FLerpVert& Vert0,
			const FLerpVert& Vert1,
			const FLerpVert& Vert2,

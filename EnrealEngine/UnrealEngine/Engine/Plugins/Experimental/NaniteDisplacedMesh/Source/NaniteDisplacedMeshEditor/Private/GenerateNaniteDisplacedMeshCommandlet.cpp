...
int32 UGenerateNaniteDisplacedMeshCommandlet::Main(const FString& CmdLineParams)
{
	...
	WritableFoldersPermission = IAssetTools::Get().GetWritableFolderPermissionList();
	if (!WritableFoldersPermission->HasFiltering())
	{
		WritableFoldersPermission.Reset();
	}
	...
	const FString MapsPackageFilter = Params.FindRef(TEXT("Map"));
	bSaveModifiedActor = Switches.Contains(TEXT("GNDMSaveModifiedActors"));
	...
	if (!MapsPackageFilter.IsEmpty())
	{
		TArray<FStringView> MapPackages;
		UE::String::ParseTokens(FStringView(MapsPackageFilter), TEXT('+'), MapPackages, UE::String::EParseTokensOptions::SkipEmpty | UE::String::EParseTokensOptions::Trim);

		Filter.PackageNames.Reserve(MapPackages.Num());
		for (const FStringView& MapPackage : MapPackages)
		{
			Filter.PackageNames.Add(FName(MapPackage));
		}
	}
	...
	if (LinkedPackageFolders.Num() > 0)
	{
		...
		TArray<FString> OtherAddedPackagesAsArray = AddedOtherPackages.Array();
		if (OtherAddedPackagesAsArray.Num())
		{
			// Might be redundant, but not sure if we can rely on the auto check out on save
			SourceControlHelper.AddToSourceControl(OtherAddedPackagesAsArray);
			UE_LOG(LogNaniteDisplacedMesh, Display, TEXT("Modified %d Actor(s) or Level(s):\n\t%s"), OtherAddedPackagesAsArray.Num(), *FString::Join(OtherAddedPackagesAsArray, TEXT("\n\t")));
		}
...
UNaniteDisplacedMesh* UGenerateNaniteDisplacedMeshCommandlet::OnLinkDisplacedMesh(UNaniteDisplacedMesh* ExistingDisplacedMesh, FValidatedNaniteDisplacedMeshParams&& InParameters, const FNaniteDisplacedMeshLinkParameters& InLinkParameters)
{
	...
	if (NaniteDisplacedMesh != nullptr)
	{
		...
		if (bCreatedNewAsset)
		{
			...
			PackagesToSave.Add(NaniteDisplacedMesh->GetPackage());
		}
	}
...
void UGenerateNaniteDisplacedMeshCommandlet::LoadLevel(const FAssetData& AssetData)
{
	...
	if (const ULevel* PersistantLevel = World->PersistentLevel)
	{
		...
		{
			if (bSaveModifiedActor)
			{
				if (UPackage* LevelPackage = PersistantLevel->GetPackage())
				{
					if (LevelPackage->IsDirty())
					{
						PackagesToSave.Add(LevelPackage);
						AddedOtherPackages.Add(LevelPackage->GetPathName());
					}
				}
			}
...
void UGenerateNaniteDisplacedMeshCommandlet::SavePackages()
{
	TArray<UPackage*> PackagesToSaveAsArray = PackagesToSave.Array();
	PackagesToSave.Reset();
	UEditorLoadingAndSavingUtils::SavePackages(PackagesToSaveAsArray, /*bOnlyDirty=*/ false);
}

void UGenerateNaniteDisplacedMeshCommandlet::FreeMemoryIfNeeded()
{
	if (ShouldFreeMemory())
	{
		// Save package that need to be saved. Some then might be collected by the garbage collector
		SavePackages();
		CollectGarbage(RF_NoFlags);
	}
}

bool UGenerateNaniteDisplacedMeshCommandlet::ShouldFreeMemory() const
{
	const FPlatformMemoryStats MemoryStats = FPlatformMemory::GetStats();

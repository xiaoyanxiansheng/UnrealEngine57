...
{
	FScopedTransaction Transaction(LOCTEXT("ChaosCachingEditorPlugin_CreateCacheManager_Transaction", "Create a Chaos Cache Manager From Selection"));
	...
	for(AActor* Actor : Actors)
	{
		...
		for(UActorComponent* Component : ComponentArray)
		{
			...
			if(UPrimitiveComponent* PrimitiveComp = Cast<UPrimitiveComponent>(Component))
			{
				...
				if (PrimitiveComp->HasAnyFlags(RF_Transient))
				{
					continue;
				}

...
struct FObservedComponent
{
	...
	~FObservedComponent()
	{
		OnDestroying.ExecuteIfBound(*this);
	}
...
private:
	...
	DECLARE_DELEGATE_OneParam(FOnDestroying, const FObservedComponent&);
...
public:
	...
	/**
	* When on, each time the cache starts recording a new asset will be created as an increment of the previous one
	*/
	UPROPERTY(EditAnywhere, Category = "Caching", meta = (EditCondition = "CacheCollection != nullptr"))
	bool bCreateNewCacheCollectionAssetOnRecord = false;
	...
	*/
	...
	// if true, cache will start automatically on begin play ( playing or recording )
...
#if WITH_EDITOR
	CHAOSCACHING_API virtual void PreEditChange(class FProperty* PropertyAboutToChange) override;
	CHAOSCACHING_API virtual void PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent) override;
#endif
	/** end UObject interface */

	CHAOSCACHING_API void SetCacheMode(ECacheMode NewCacheMode);

	UFUNCTION(BlueprintCallable, Category = "Caching")
	CHAOSCACHING_API bool IsRecording() const;
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "Caching")
	void Start(float InStartTime = 0);
	...
	*/
	...
	UFUNCTION(BlueprintCallable, Category = "Caching", meta = (DisplayName = "UpdateOrAddObservedComponent"))
	...
	UFUNCTION(BlueprintCallable, Category = "Caching")
	CHAOSCACHING_API void ClearObservedComponents(bool bDestroyComponents = true);
...
protected:
	...
	CHAOSCACHING_API void Destroyed() override;
	/** End AActor interface */
...
private:
	...
	*/
	...
	/**
	* this creates a new cache collection asset based on the existing one and set it as the current 
	* this also find the editor counterpart actor and update the cache collection accordingly
	*/
	void CreateAndSetNewCacheAsset();

	void OnObservedComponentDestroyed(const FObservedComponent& Component);
	void RegisterObservedComponentsNotifiers();
	void UnregisterObservedComponentsNotifiers();
	void EnsureObservedComponentsNotifierRegistration();

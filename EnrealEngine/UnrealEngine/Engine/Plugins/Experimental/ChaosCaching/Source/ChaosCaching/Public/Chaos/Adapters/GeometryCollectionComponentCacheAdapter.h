...
namespace Chaos
{
	// forward declaration to avoid adding EventManager.h
	...
	public:
		...
		virtual void				   FinalizeComponent(UPrimitiveComponent* InComponent, UChaosCache* InCache) override;
	...
	protected:
		...
		void RegisterEventHandler(FGeometryCollectionPhysicsProxy* Proxy);
		void UnregisterEventHandler(FGeometryCollectionPhysicsProxy* Proxy);
	...
	private:
		...
		/** 
		* List of the solvers we are currently listening for events for recording
		* we need to track them because each of them owns a seperate unique event manager 
		*/
		using FSolverEventPair = TPair<Chaos::FPhysicsSolver*, Chaos::EEventType>;
		TSet<FSolverEventPair> SolversListenedToForEvents;

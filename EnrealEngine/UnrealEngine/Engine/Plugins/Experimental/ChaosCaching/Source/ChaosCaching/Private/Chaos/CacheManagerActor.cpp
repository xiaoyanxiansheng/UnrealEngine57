...
void AChaosCacheManager::PreEditChange(class FProperty* PropertyAboutToChange)
{
	Super::PreEditChange(PropertyAboutToChange);

	if (PropertyAboutToChange && PropertyAboutToChange->GetFName() == GET_MEMBER_NAME_CHECKED(AChaosCacheManager, ObservedComponents))
	{
		EnsureObservedComponentsNotifierRegistration();
	}
}
...
void AChaosCacheManager::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(AChaosCacheManager, CacheCollection))
	{
		SetCacheCollection(CacheCollection);
	}
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(AChaosCacheManager, ObservedComponents))
	{
		EnsureObservedComponentsNotifierRegistration();
	}
...
void AChaosCacheManager::Destroyed()
{
	ClearObservedComponents(/*bDestroyCOmponents*/false);

	Super::Destroyed();
}

void AChaosCacheManager::OnObservedComponentDestroyed(const FObservedComponent& ObservedComponent)
{
...
void AChaosCacheManager::RegisterObservedComponentsNotifiers()
{
	for (FObservedComponent& ObservedComponent : ObservedComponents)
	{
		ObservedComponent.OnDestroying.BindUObject(this, &AChaosCacheManager::OnObservedComponentDestroyed);
	}

}

void AChaosCacheManager::UnregisterObservedComponentsNotifiers()
{
	for (FObservedComponent& ObservedComponent : ObservedComponents)
	{
		ObservedComponent.OnDestroying.Unbind();
	}
}

void AChaosCacheManager::EnsureObservedComponentsNotifierRegistration()
{
	UnregisterObservedComponentsNotifiers();
	RegisterObservedComponentsNotifiers();
}
...
void AChaosCacheManager::SetCacheMode(ECacheMode NewCacheMode)
{
	...
	SetStartTime(IsRecording() ? 0 : StartTime);
}

bool AChaosCacheManager::IsRecording() const
{
	return (CacheMode == ECacheMode::Record);
}
...
void AChaosCacheManager::Start(float InStartTime)
{
	...
	if (IsRecording() && bCreateNewCacheCollectionAssetOnRecord)
	{
		CreateAndSetNewCacheAsset();
	}

	if (!IsRecording())
	{
		StartTime = InStartTime; // todo(ccaillaud) - is that needed in record ?
		OnStartFrameChanged(InStartTime);
	}
	BeginEvaluate();
}
...
#if WITH_EDITOR
	// if the cache collection was changed during PIE ( automatic asset creation upon recording for example ) 
	// make sure we update the editor actor with this new collection so it canb be replayed properly in non-PIE mode
	if (AActor* EditorWorldActor = EditorUtilities::GetEditorWorldCounterpartActor(this))
	{
		if (AChaosCacheManager* EditCachemanagerActor = Cast<AChaosCacheManager>(EditorWorldActor))
		{
			if (EditCachemanagerActor->CacheCollection != CacheCollection)
			{
				EditCachemanagerActor->SetCacheCollection(CacheCollection);
			}
		}
	}
#endif
...
void AChaosCacheManager::EndEvaluate()
{
	...
	{
		if (Chaos::FComponentCacheAdapter* Adapter = ActiveAdapters[CompIndex])
		{
			FObservedComponent& Observed = ObservedComponents[CompIndex];
			Adapter->FinalizeComponent(Observed.GetComponent(this), Observed.GetChaosCache());
		}
	}
...
void AChaosCacheManager::SetCacheCollection(UChaosCacheCollection* InCacheCollection)
{
	...
	// Reset cache collection
	for (FObservedComponent& Observed : ObservedComponents)
	{
		if (Observed.Cache && Observed.Cache->GetOuter() != CacheCollection)
		{
			Observed.Cache = nullptr;
		}
	}

	// reupdate the pose based on the new cache 
	SetStartTime(0);
}

void AChaosCacheManager::CreateAndSetNewCacheAsset()
{
#if WITH_EDITOR
	if (CacheCollection)
	{
		...
		static const FString EmptyDefaultSuffix;
		AssetToolsModule.Get().CreateUniqueAssetName(CacheCollection->GetOutermost()->GetName(), EmptyDefaultSuffix, /*out*/ NewPackageName, /*out*/ NewAssetName);
		...
		UChaosCacheCollection* NewCacheCollection = NewObject<UChaosCacheCollection>(NewPackage, *NewAssetName, Flags);

		NewCacheCollection->MarkPackageDirty();
		FAssetRegistryModule::AssetCreated(NewCacheCollection);

		//UObject* NewObject = AssetToolsModule.Get().CreateAsset(NewAssetName, NewPackagePath, UChaosCacheCollection::StaticClass(), /* Factory */ nullptr);
		CacheCollection = NewCacheCollection;
	}
#else
	UE_LOG(LogChaosCache, Error, TEXT("Cache Manager [%s] - Creating new asset upon recording is not supported in non-editor builds"), *GetName());
#endif
}
...
void AChaosCacheManager::FindOrAddObservedComponent(UPrimitiveComponent* InComponent, const FName& CacheName, const bool bTransferSimulation)
{
	...
	{
		// we do not want to record transient component as they may not exists when we start recording 
		...
		if(FObservedComponent* ObservedComponent = FoundComponent ? FoundComponent : &AddNewObservedComponent(InComponent))
		{
			...
			if (UGeometryCollectionComponent* GeomComponent = Cast<UGeometryCollectionComponent>(InComponent))
			{
				ObservedComponent->bHasNotifyBreaks = GeomComponent->bNotifyBreaks;
			}
...
{
	EnsureObservedComponentsNotifierRegistration();
	// make sure we listen to those component being removed
...
void AChaosCacheManager::ClearObservedComponents(bool bDestroyComponents)
{
	...
	{
		// we are destroying, no need to restore state
		UnregisterObservedComponentsNotifiers();
		for (FObservedComponent& ObservedComponent : ObservedComponents)
		{

...
struct FCacheEventTrack
{
	...
	struct FHandle
	{
		...
		template<typename T>
		UE_DEPRECATED(5.6, "Non-const access is deprecated. Use GetConst instead")
		T* Get() const
		{
			return IsAlive() ? const_cast<FCacheEventTrack*>(Track)->GetEvent<T>(Index) : nullptr;
		}
	...
	template<typename T>
	const T* GetEvent(int32 Index) const
	{
		if (EventData.IsValidIndex(Index) && Struct)
		{
			check(T::StaticStruct() == Struct);
			return reinterpret_cast<const T*>(EventData[Index]);
		}
	...
	FHandle GetEventHandle(int32 Index) const;

...
namespace Chaos
{
			...
			for (const FCacheEventHandle& Handle : *TrailingEvents)
			{
				if (const FTrailingEvent* Event = Handle.GetConst<FTrailingEvent>())
				{
					if (Chaos::FPBDRigidClusteredParticleHandle* Particle = Proxy->GetParticle_Internal(Event->Index))
					{
						...
						CachedTrail.Mass = Particle->M();
						CachedTrail.TransformGroupIndex = INDEX_NONE; // todo(ccaillaud) we should probably record that info in the cache : but we need to be careful that this value may change with the GC
	...
	void FGeometryCollectionCacheAdapter::SetRestState(UPrimitiveComponent* InComponent, UChaosCache* InCache, const FTransform& InRootTransform, Chaos::FReal InTime) const
	{
		...
		if (UGeometryCollectionComponent* Comp = CastChecked<UGeometryCollectionComponent>(InComponent))
		{
			...
			if (const UGeometryCollection* RestCollection = Comp->GetRestCollection())
			{
				...
				const int32 RootIndex = Comp->GetRootIndex();
				bool bIsRootBroken = false;
				for (int32 ReleaseIdx : ReleaseIndices)
				{
					const int32 ClusterIdx = Parent[ReleaseIdx];
					bIsRootBroken = (ClusterIdx == RootIndex);
				...
				if (bIsRootBroken != Comp->IsForceBrokenForCustomRenderer())
				{
					// this is the root bone broken we need to disable the root proxy
					Comp->ForceBrokenForCustomRenderer(bIsRootBroken);
				}
	...
	void FGeometryCollectionCacheAdapter::RegisterEventHandler(FGeometryCollectionPhysicsProxy* Proxy)
	{
		if (!Proxy)
		{
			return;
		...
		if (Chaos::FPhysicsSolver* Solver = Proxy->GetSolver<Chaos::FPhysicsSolver>())
		{
			const FSimulationParameters& SimulationParameters = Proxy->GetSimParameters();

			// this method ensure that only the necessary handlers are registered by solver and for the need of the specific proxies we are tracking
			if (Chaos::FEventManager* EventManager = Solver->GetEventManager())
			{
				if (SimulationParameters.bGenerateCollisionData || SimulationParameters.bGenerateGlobalCollisionData)
				{
					const FSolverEventPair SolverCollisionEventPair{ Solver,Chaos::EEventType::Collision };
					if (!SolversListenedToForEvents.Contains(SolverCollisionEventPair))
					{
						EventManager->RegisterHandler<Chaos::FCollisionEventData>(Chaos::EEventType::Collision, this, &FGeometryCollectionCacheAdapter::HandleCollisionEvents);
						SolversListenedToForEvents.Add(SolverCollisionEventPair);
					}
				}
				if (SimulationParameters.bGenerateBreakingData || SimulationParameters.bGenerateGlobalBreakingData)
				{
					const FSolverEventPair SolverBreakingEventPair{ Solver,Chaos::EEventType::Breaking };
					if (!SolversListenedToForEvents.Contains(SolverBreakingEventPair))
					{
						EventManager->RegisterHandler<Chaos::FBreakingEventData>(Chaos::EEventType::Breaking, this, &FGeometryCollectionCacheAdapter::HandleBreakingEvents);
						SolversListenedToForEvents.Add(SolverBreakingEventPair);
					}
				}
				if (SimulationParameters.bGenerateTrailingData)
				{
					const FSolverEventPair SolverTrailingEventPair{ Solver,Chaos::EEventType::Trailing};
					if (!SolversListenedToForEvents.Contains(SolverTrailingEventPair))
					{
						EventManager->RegisterHandler<Chaos::FTrailingEventData>(Chaos::EEventType::Trailing, this, &FGeometryCollectionCacheAdapter::HandleTrailingEvents);
						SolversListenedToForEvents.Add(SolverTrailingEventPair);
					}
				}
			}
		}
	...
	void FGeometryCollectionCacheAdapter::UnregisterEventHandler(FGeometryCollectionPhysicsProxy* Proxy)
	{
		if (!Proxy)
		{
			return;
		...
		if (Chaos::FPhysicsSolver* Solver = Proxy->GetSolver<Chaos::FPhysicsSolver>())
		{
			// find all proxies for this Solver and check what they need to listen to after removal of the current proxy
			bool bListenToCollisionEvents = false;
			bool bListenToBreakingEvents = false;
			bool bListenToTrailingEvents = false;
			for (const TPair<IPhysicsProxyBase*, FCachedEventData>& CacheDataPair : CachedData)
			{
				const IPhysicsProxyBase* OtherProxy = CacheDataPair.Key;
				const FGeometryCollectionPhysicsProxy* OtherGCProxy =
					OtherProxy->GetType() == FGeometryCollectionPhysicsProxy::ConcreteType()
					? static_cast<const FGeometryCollectionPhysicsProxy*>(OtherProxy)
					: nullptr;

				// exclude the proxy we are about to remove 
				if (Proxy != OtherGCProxy && OtherGCProxy)
				{
					const FSimulationParameters& OtherSimParams = OtherGCProxy->GetSimParameters();
					if (OtherProxy->GetSolver<Chaos::FPhysicsSolver>() == Solver)
					{
						bListenToCollisionEvents |= (OtherSimParams.bGenerateCollisionData || OtherSimParams.bGenerateGlobalCollisionData);
						bListenToBreakingEvents |= (OtherSimParams.bGenerateBreakingData || OtherSimParams.bGenerateGlobalBreakingData);
						bListenToTrailingEvents |= (OtherSimParams.bGenerateTrailingData);
					}
				}
			}

			// unregister the handlers that we do not need anymore
			if (Chaos::FEventManager* EventManager = Solver->GetEventManager())
			{
				if (!bListenToCollisionEvents)
				{
					const FSolverEventPair SolverCollisionEventPair{ Solver,Chaos::EEventType::Collision };
					EventManager->UnregisterHandler(Chaos::EEventType::Collision, this);
					SolversListenedToForEvents.Remove(SolverCollisionEventPair);
				}
				if (!bListenToBreakingEvents)
				{
					const FSolverEventPair SolverBreakingEventPair{ Solver,Chaos::EEventType::Breaking };
					EventManager->UnregisterHandler(Chaos::EEventType::Breaking, this);
					SolversListenedToForEvents.Remove(SolverBreakingEventPair);
				}
				if (!bListenToTrailingEvents)
				{
					const FSolverEventPair SolverTrailingEventPair{ Solver,Chaos::EEventType::Trailing };
					EventManager->UnregisterHandler(Chaos::EEventType::Trailing, this);
					SolversListenedToForEvents.Remove(SolverTrailingEventPair);
				}
			}
		}
	}
	...
	bool FGeometryCollectionCacheAdapter::InitializeForRecord(UPrimitiveComponent* InComponent, UChaosCache* InCache)
	{
		...
		UGeometryCollectionComponent*    Comp     = CastChecked<UGeometryCollectionComponent>(InComponent);
		FGeometryCollectionPhysicsProxy* Proxy = Comp ? Comp->GetPhysicsProxy() : nullptr;
		...
		if (Comp)
		{
			// make sure we have the root proxy enabled if any
			Comp->ForceBrokenForCustomRenderer(false);
		}
		...
		RegisterEventHandler(Proxy);

		// we only need to clear the data for the first proxy we register
		if (CachedData.Num() == 0)
		{
	...
	void FGeometryCollectionCacheAdapter::FinalizeComponent(UPrimitiveComponent* InComponent, UChaosCache* InCache)
	{
		EnsureIsInGameThreadContext();

		// make sure we remove the proxy from the list to avoid accumulating them record after record
		if (UGeometryCollectionComponent* Comp = CastChecked<UGeometryCollectionComponent>(InComponent))
		{
			if (FGeometryCollectionPhysicsProxy* Proxy = Comp->GetPhysicsProxy())
			{
				UnregisterEventHandler(Proxy);
				CachedData.Remove(Proxy);
			}
		}
	}

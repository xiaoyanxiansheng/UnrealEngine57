 ...
 * the RigidBodyWithControl node, and Control Rig physics.
 */
...
{
	// Target is in world space. For a control rig, this is the space of the control rig, rather
	// than the "real" world.
	World,
	// If a skeleton/bone is being used, the target is in the space of that component. If not then
	// it is in the space of the owning component (e.g. PhysicsControl, or ControlRig etc).
	Component,
	// If a skeleton/bone is being used, the target is specified relative to the bone that controls
	// the (kinematic) body, with the offset applied in the space of that body. If not, then this is
	// the same as World.
	OffsetInBoneSpace,
	// If a skeleton/bone is being used, the target is specified relative to the bone that controls
	// the (kinematic) body, with the offset applied in world space. If not, then this is
	// the same as World.
	OffsetInWorldSpace,
	// If a skeleton/bone is being used, the target is specified relative to the bone that controls
	// the (kinematic) body, with the offset applied in component space. If not, then this is
...
struct FPhysicsControlModifierSparseData
{
	...
	FPhysicsControlModifierSparseData(
		...
		const EPhysicsControlKinematicTargetSpace InKinematicTargetSpace = EPhysicsControlKinematicTargetSpace::OffsetInBoneSpace,
		...
		, KinematicTargetSpace(InKinematicTargetSpace)
		...
		, bEnableKinematicTargetSpace(true)
	...
	// The space in which kinematic targets should operate
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = PhysicsControl, meta = (editcondition = "bEnableKinematicTargetSpace"))
	...
	UPROPERTY(EditAnywhere, Category = PhysicsControl, meta = (InlineEditConditionToggle))
	uint8 bEnableKinematicTargetSpace : 1;
...
struct FPhysicsControlModifierData
{
	...
	friend UE_API FArchive& operator <<(FArchive& Ar, FPhysicsControlModifierData& Data);
...
struct FPhysicsControlSparseData
{
	...
	/**
	 * Multiplier on the linear velocity, which gets applied to the damping. A value of 1 means the animation target
...
struct FPhysicsControlData
{
	...
	friend UE_API FArchive& operator<<(FArchive& Ar, FPhysicsControlData& Data);

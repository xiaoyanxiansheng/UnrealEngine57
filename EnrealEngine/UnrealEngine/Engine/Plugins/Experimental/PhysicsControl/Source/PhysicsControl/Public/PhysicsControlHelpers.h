...
inline FVector CalculateAngularVelocity(const FQuat& PrevQ, const FQuat& CurrentQ, float Dt)
{
	const FQuat DeltaQ = (CurrentQ * PrevQ.Inverse()).GetShortestArcWith(FQuat::Identity);
	return DeltaQ.ToRotationVector() / Dt;
}

inline FVector CalculateLinearVelocity(const FVector& PrevP, const FVector& CurrentP, float Dt)
{

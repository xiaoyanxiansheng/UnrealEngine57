...
AActor* UWaveFunctionCollapseSubsystem::SpawnActorFromTiles(const TArray<FWaveFunctionCollapseTile>& Tiles)
{
	...
	for (int32 index = 0; index < Tiles.Num(); index++)
	{
		...
		if (LoadedObject)
		{
			...
			{
				const TSubclassOf<UObject> GeneratedClass = LoadedBlueprint->GeneratedClass;

				// if BP is placeable
				if (!(GeneratedClass->GetClass()->HasAnyClassFlags(CLASS_NotPlaceable | CLASS_Deprecated | CLASS_Abstract))
					&& GeneratedClass->GetClass()->IsChildOf(AActor::StaticClass()))
				{

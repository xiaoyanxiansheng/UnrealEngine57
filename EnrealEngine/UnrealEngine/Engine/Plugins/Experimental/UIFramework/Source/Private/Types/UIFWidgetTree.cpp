	...
	for (int32 Index : AddedIndices)
	{
		...
		if (Entry.ParentId.IsValid() && Entry.ChildId.IsValid())
		{
			...
			if (!Entry.ParentId.IsRoot())
			{
 			...
 			LocalOnWidgetAdded.Broadcast(Entry.Child);
		}
	}
}
...
bool FUIFrameworkWidgetTree::AuthorityRemoveChildRecursiveInternal(UUIFrameworkWidget* Widget)
{
	...
	int32 PreviousEntryIndex = INDEX_NONE;
	if (AuthorityIndexByWidgetMap.RemoveAndCopyValue(WidgetKey, PreviousEntryIndex))
	{
		check(Entries.IsValidIndex(PreviousEntryIndex));

		WidgetByIdMap.Remove(Widget->GetWidgetId());

		if (ensure(ReplicatedOwner) && ReplicatedOwner->IsUsingRegisteredSubObjectList())
		{
			ReplicatedOwner->DestroyReplicatedSubObjectOnRemotePeers(Widget);
		}

		Entries.RemoveAtSwap(PreviousEntryIndex);

		// Fix up the swap item
		if (Entries.IsValidIndex(PreviousEntryIndex))
		{
			if (int32* FixUpChildIndexPtr = AuthorityIndexByWidgetMap.Find(FObjectKey(Entries[PreviousEntryIndex].Child)))
			{
				*FixUpChildIndexPtr = PreviousEntryIndex;
			}
		}
...
void FUIFrameworkWidgetTree::AuthorityRemoveAllWidgetsFromActorChannel()
{
	if (ensure(ReplicatedOwner) && ReplicatedOwner->IsUsingRegisteredSubObjectList())
	{
		for (const FUIFrameworkWidgetTreeEntry& Entry : Entries)
		{
			...
			{
				ReplicatedOwner->DestroyReplicatedSubObjectOnRemotePeers(Entry.Child);
			}
		}
	}
}
...
{
	return Entries.FindByPredicate([ReplicationId](const FUIFrameworkWidgetTreeEntry& Entry)
	{
		return Entry.ReplicationID == ReplicationId;
	});
}
...
const FUIFrameworkWidgetTreeEntry* FUIFrameworkWidgetTree::FindRootEntryById(FUIFrameworkWidgetId WidgetId) const
{
	FUIFrameworkWidgetId NextWidgetId = WidgetId;
	while (NextWidgetId.IsValid())
	{
		const FUIFrameworkWidgetTreeEntry* Entry = Entries.FindByPredicate([NextWidgetId](const FUIFrameworkWidgetTreeEntry& Entry)
			{
				return Entry.ChildId == NextWidgetId;
			});

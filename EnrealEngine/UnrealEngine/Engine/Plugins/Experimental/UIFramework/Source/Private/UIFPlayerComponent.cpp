...
void FUIFrameworkGameLayerSlotList::PreReplicatedRemove(const TArrayView<int32>& ChangedIndices, int32 FinalSize)
{
	...
	for (int32 Index : ChangedIndices)
	{
		...
		{
			if (UUIFrameworkPresenter* Presenter = Owner->GetOrCreatePresenter())
			{
				Presenter->RemoveFromViewport(Slot.GetWidgetId());
			}
		}
	}
}
...
void UUIFrameworkPlayerComponent::UninitializeComponent()
{
	...
	else
	{
		...
		FTSTicker::RemoveTicker(PendingWidgetConstructedTickerHandle);
	}
...
UUIFrameworkPresenter* UUIFrameworkPlayerComponent::GetOrCreatePresenter()
{
	if (Presenter == nullptr)
	{
		Presenter = NewObject<UUIFrameworkPresenter>(this, FUIFrameworkModule::GetPresenterClass());
	}
...
void UUIFrameworkPlayerComponent::LocalAddChild(FUIFrameworkWidgetId WidgetId)
{
	...
	if (FUIFrameworkGameLayerSlot* LayerSlot = RootList.FindEntry(WidgetId))
	{
		if (Widget)
		{
			...
			if (ensure(UMGWidget))
			{
				...
				LayerSlot->LocalAquireWidget();
				if (UUIFrameworkPresenter* PresenterPtr = GetOrCreatePresenter())
				{
					PresenterPtr->AddToViewport(UMGWidget, *LayerSlot);
				}
			}
		}
		else
	...
	if (UUIFrameworkWidget* Widget = GetWidgetTree().FindWidgetById(CurrentWidgetToFocus))
	{
		if (UWidget* UMGWidget = Widget->LocalGetUMGWidget())
		{
			if (UMGWidget->IsConstructed())
			{
				...
				if (RootEntry)
				{
					if (FUIFrameworkGameLayerSlot* LayerSlot = RootList.FindEntry(RootEntry->ChildId))
					{
						if (LayerSlot->InputMode == EUIFrameworkInputMode::UI)
						{
							if (UUIFrameworkPresenter* PresenterPtr = GetOrCreatePresenter())
							{
								PresenterPtr->FocusWidget(UMGWidget);
							}
						}
					}
				}

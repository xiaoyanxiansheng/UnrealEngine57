...
void UUIFrameworkTextBase::SetTextSize(float InTextSize)
{
	if (TextSize != InTextSize)
	{
		TextSize = InTextSize;
		MARK_PROPERTY_DIRTY_FROM_NAME(ThisClass, TextSize, this);
		ForceNetUpdate();
	}
}
...
void UUIFrameworkTextBase::OnRep_TextSize()
{
	if (LocalGetUMGWidget())
	{
		SetTextSizeToWidget(TextSize);
...
void UUIFrameworkTextBlock::SetTextSizeToWidget(float InValue)
{
	UTextBlock* TextBlock = CastChecked<UTextBlock>(LocalGetUMGWidget());
	FSlateFontInfo CurrentFontInfo = TextBlock->GetFont();
	CurrentFontInfo.Size = InValue;
	TextBlock->SetFont(CurrentFontInfo);
}

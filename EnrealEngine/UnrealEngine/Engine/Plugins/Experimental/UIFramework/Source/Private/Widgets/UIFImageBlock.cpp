	...
	else
	{
		...
		if (UMaterialInterface* MaterialInterface = Cast<UMaterialInterface>(ResourceObject))
		{
			if (UMaterial* BaseMaterial = MaterialInterface->GetBaseMaterial())
			{
				...
				{
					UE_LOG(LogUIFramework, Error, TEXT("Can't use material %s for material_block %s. The material domain is not set to \"User Interface\""), *BaseMaterial->GetName(), *GetName());
				}
			}
		}

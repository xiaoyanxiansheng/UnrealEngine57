...
FAVResult TVideoEncoderVT<TResource>::SendFrame(TSharedPtr<FVideoResourceMetal> const &Resource, uint32 Timestamp, bool bForceKeyframe)
{
    if (IsOpen())
    {
        ...
        if (Resource.IsValid())
        {
			...
			int64 PixelType = 0;
			switch(Resource->GetDescriptor().Format)
			{
			...
			CVReturn Result = CVPixelBufferCreate(kCFAllocatorDefault, Resource->GetDescriptor().Width, Resource->GetDescriptor().Height, PixelType, SourceAttributes, &PixelBuffer);
			...
			Resource->GetRaw()->getBytes(reinterpret_cast<uint8*>(CVPixelBufferGetBaseAddressOfPlane(PixelBuffer, 0)), CVPixelBufferGetBytesPerRow(PixelBuffer), MTL::Region(0, 0, Resource->GetDescriptor().Width, Resource->GetDescriptor().Height), 0);
			CVPixelBufferUnlockBaseAddress(PixelBuffer, 0);
			
            OSStatus Status = VTCompressionSessionEncodeFrame(Encoder, PixelBuffer, PresentationTime, kCMTimeInvalid, FrameProperties, (void *)Params.Release(), NULL);

            CONDITIONAL_RELEASE(FrameProperties);
			
			CONDITIONAL_RELEASE(PixelBuffer);

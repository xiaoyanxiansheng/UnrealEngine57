...
CMVideoFormatDescriptionRef NaluRewriter::CreateVP9VideoFormatDescription(const uint8* rtp_buffer, size_t rtp_buffer_size)
{
	...
	// Early exit if size is not set because this means a valid VP9VideoFormatDescription cannot be constructed.
	if (Header.frame_width_minus_1 <= 0)
	{
		return nullptr;
	}
	...
	uint8 BitDepthChromaAndRange = (0xF & static_cast<uint8>(Header.bit_depth.Value)) << 4 | (0x7 & ChromaSubSampling) << 1 | (0x1 & static_cast<uint8>(Header.color_range.Value));

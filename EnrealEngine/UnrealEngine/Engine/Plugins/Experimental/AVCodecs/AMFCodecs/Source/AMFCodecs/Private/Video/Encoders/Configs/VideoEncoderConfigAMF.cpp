...
DLLEXPORT FAVResult FAVExtension::TransformConfig(FVideoEncoderConfigAMF& OutConfig, FVideoEncoderConfig const& InConfig)
{
	...
	// Pixel streaming uses Nvidia convention of using 0xffffffff for infinite p-frames.
	// AMD uses 0. See AMF_VIDEO_ENCODER_IDR_PERIOD in https://github.com/GPUOpen-LibrariesAndSDKs/AMF/blob/master/amf/doc/AMF_Video_Encode_API.md
	uint32 KeyframeInterval = InConfig.KeyframeInterval == 0xffffffff ? 0 : InConfig.KeyframeInterval;
	OutConfig.SetProperty(AMF_VIDEO_ENCODER_IDR_PERIOD, KeyframeInterval);
...
DLLEXPORT FAVResult FAVExtension::TransformConfig(FVideoEncoderConfig& OutConfig, FVideoEncoderConfigAMF const& InConfig)
{
	...
	OutConfig.KeyframeInterval = OutConfig.KeyframeInterval > 0 ? OutConfig.KeyframeInterval : 0xffffffff;

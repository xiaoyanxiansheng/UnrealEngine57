...
public:
	 ...
	 */
	inline FVideoDescriptor const& GetRawDescriptor() const { return Descriptor.RawDescriptor ? *Descriptor.RawDescriptor : Descriptor; }

	/**
	 * @return Get the descriptor of our video data.
	 */
	inline FVideoDescriptor const& GetDescriptor() const { return Descriptor; }

	/**
	 * @return Get the format of our data.
	 */
	inline EVideoFormat GetFormat() const { return Descriptor.Format; }

	/**
	 * @return Get the width of our data.
	 */
	inline uint32 GetWidth() const { return Descriptor.Width; }

	/**
	 * @return Get the height of our data.
	 */
	inline uint32 GetHeight() const { return Descriptor.Height; }

	/**
	 * @return Get the Size in bytes of our Resource.
	 */
	inline uint32 GetSize() const { return Descriptor.GetSizeInBytes(); }

	/**
	 * 
	 */
	inline void SetUsing(bool bUsing) { bIsInUse = bUsing; }
	...
	UE_API FVideoResource(TSharedRef<FAVDevice> const& Device, FAVLayout const& Layout, FVideoDescriptor const& Descriptor);
...
{
public:
	inline TSharedPtr<TContext> GetContext() const { return GetDevice()->template GetContext<TContext>(); }

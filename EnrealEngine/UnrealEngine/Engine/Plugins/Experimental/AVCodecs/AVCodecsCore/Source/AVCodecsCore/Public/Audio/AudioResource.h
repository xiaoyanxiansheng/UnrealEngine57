 ...
 */
class FAudioResource : public FAVResource
{
private:
	/**
...
public:
	 ...
	 */
	inline FAudioDescriptor const& GetDescriptor() const { return Descriptor; }
	
	/**
	 * @return Get the number of samples in our data in device memory.
	 */
	inline float GetNumSamples() const { return Descriptor.NumSamples; }

	/**
	 * @return Get the duration of our data in device memory.
	 */
	inline float GetSampleDuration() const { return Descriptor.SampleDuration; }

	UE_API FAudioResource(TSharedRef<FAVDevice> const& Device, FAVLayout const& Layout, FAudioDescriptor const& Descriptor);

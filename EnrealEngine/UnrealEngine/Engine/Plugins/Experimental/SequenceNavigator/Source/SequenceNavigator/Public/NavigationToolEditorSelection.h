...
enum class ENavigationToolSelectionSource
{
...
public:
	FNavigationToolEditorSelection(FEditorModeTools& InModeTools, UObject* InChangedSelection = nullptr)
	...
	template<typename InObjectType = UObject, ENavigationToolSelectionSource InSource = ENavigationToolSelectionSource::All
	...
	TArray<InObjectReturnType*> GetSelectedObjects() const
	{
		...
		if constexpr (InSource == ENavigationToolSelectionSource::Single)
		{
			OutSelectedObjects = GetSelectedObjects<InObjectReturnType*>(GetSelection<InObjectType>());
		}
		else
		{
			static_assert(InSource == ENavigationToolSelectionSource::All);
...
private:
	...
	/** The Selection that changed that triggered this FNavigationToolEditorSelection to be instanced. Can be null if nothing changed. */

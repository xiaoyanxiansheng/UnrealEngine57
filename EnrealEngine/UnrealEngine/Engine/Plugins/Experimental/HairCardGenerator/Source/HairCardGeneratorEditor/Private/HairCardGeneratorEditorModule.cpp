...
bool FHairCardGeneratorUtils::BuildGenerationSettings(const bool bQuerySettings, UGroomAsset* NewGroomAsset, FHairGroupsCardsSourceDescription& CardsDesc,
	TObjectPtr<UHairCardGeneratorPluginSettings>& GenerationSettings, uint8& AllGenFlags, uint8& PipelineGenFlags, const FCardsGenerationAdvancedOptions& AdvancedOptions)
{
	// Find the HairCardGenController in Python (or wherever it may be implemented -- could be moved to C++/Blueprints/etc.)
	...
	{
		GenerationSettings->bGenerateGeometryForAllGroups = AdvancedOptions.bGenerateGeometryForAllGroups;
		GenerationSettings->bReduceCardsFromPreviousLOD = AdvancedOptions.bReduceCardsFromPreviousLOD;
		GenerationSettings->RandomSeed = AdvancedOptions.RandomSeed;
		GenerationSettings->bUseReservedSpaceFromPreviousLOD = AdvancedOptions.bUseReservedSpaceFromPreviousLOD;
		GenerationSettings->AtlasSize = static_cast<EHairCardAtlasSize>(AdvancedOptions.AtlasSize);
		GenerationSettings->ReserveTextureSpaceLOD = AdvancedOptions.ReserveTextureSpaceLOD;
		GenerationSettings->bUseGroomAssetStrandWidth = AdvancedOptions.bUseGroomAssetStrandWidth;
	}
...
void FHairCardGeneratorUtils::BuildCardsGroups(TObjectPtr<const UGroomAsset> GroomAsset, TArray<FName>& GroupNames, TArray<int32>& GroupCount)
{
	HairCardSettings_Helpers::GetStrandCardGroupInfo(GroomAsset, GroupNames, GroupCount);
}
...
bool FHairCardGeneratorEditorModule::GenerateHairCardsForLOD(UGroomAsset* NewGroomAsset, FHairGroupsCardsSourceDescription& CardsDesc)
{
	...
	if(!FHairCardGeneratorUtils::BuildGenerationSettings(true, NewGroomAsset, CardsDesc, GenerationSettings, AllGenFlags, PipelineGenFlags, FCardsGenerationAdvancedOptions()))
	{
		return false;
	}

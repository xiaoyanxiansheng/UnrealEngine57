...
void HairCardSettings_Helpers::GetStrandCardGroupInfo(TObjectPtr<const UGroomAsset> GroomAsset, TArray<FName>& OutGrpIds, TArray<int32>& OutGrpCount)
{
    TMap<FName,int32> IdNames;
    ...
    if ( TraverseStrands.First().HasCardGroupName() )
    {
		...
		{
			int32* NumStrands = IdNames.Find(StrandAttribs.GetCardGroupName());
			if(!NumStrands)
			{
				IdNames.Add(StrandAttribs.GetCardGroupName(), 1);
			}
			else
			{
				(*NumStrands) += 1;
	...
	IdNames.GenerateKeyArray(OutGrpIds);
	IdNames.GenerateValueArray(OutGrpCount);
}
...
void UHairCardGeneratorPluginSettings::ResetFromSourceDescription(const FHairGroupsCardsSourceDescription& SourceDesc)
{
	...
	TArray<int32> CardGroupCount;
	HairCardSettings_Helpers::GetStrandCardGroupInfo(GroomAsset, CardGroupIds, CardGroupCount);

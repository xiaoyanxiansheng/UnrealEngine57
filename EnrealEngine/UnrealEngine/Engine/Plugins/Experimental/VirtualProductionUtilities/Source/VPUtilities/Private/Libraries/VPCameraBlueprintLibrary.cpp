...
ACameraRig_Rail* UVPCameraBlueprintLibrary::SpawnDollyTrackFromPoints(UObject* WorldContextObject, const TArray<FTransform>& Points, ESplinePointType::Type InterpType)
{
	...
	// Note: We previously called AutoSetTangents on the reparameterization table here.
	// With the recent refactor deprecating SplineCurves, there was no way to replace this code.
	// It did have the effect of setting tangents, but these tangents were not actually
	// used because the reparam points were all CIM_Linear, so evaluation did not require tangents.
...
ACameraRig_Rail* UVPCameraBlueprintLibrary::SpawnCameraRigFromPoints(UObject* WorldContextObject, const FTransform& RigTransform, const TArray<FVector>& Points, const FVPCameraRigSpawnParams& Params)
{
	...
	{
		const FSplineCurves CachedLegacySplineData = SplineComponent->GetSplineCurves();
		...
		if (Params.LinearApproximationMode == EVPCameraRigSpawnLinearApproximationMode::IntegrationStep)
		{
			...
			Density = FMath::CeilToFloat(1.f / (CachedLegacySplineData.GetSplineLength() / FMath::Max(KINDA_SMALL_NUMBER, Density)));
		}

		FSplinePositionLinearApproximation::Build(CachedLegacySplineData, OutPoints, Density);

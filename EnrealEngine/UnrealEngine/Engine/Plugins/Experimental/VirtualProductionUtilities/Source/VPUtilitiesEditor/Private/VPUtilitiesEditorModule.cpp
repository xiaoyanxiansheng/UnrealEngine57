...
void FVPUtilitiesEditorModule::RegisterSettings()
{
	...
	if (SettingsModule != nullptr)
	{
		...
		ISettingsSectionPtr EditorUserSettingsSection = SettingsModule->RegisterSettings("Editor", "Plugins", "VirtualProduction",
			LOCTEXT("VirtualProductionEditorUserSettingsName", "Virtual Production"),
			LOCTEXT("VirtualProductionEditorUserSettingsNameDescription", "Configure the Virtual Production Editor user settings"),
			GetMutableDefault<UVPUtilitiesEditorUserSettings>()
		);

		EditorUserSettingsSection->OnModified().BindRaw(this, &FVPUtilitiesEditorModule::OnSettingsModified);
...
void FVPUtilitiesEditorModule::UnregisterSettings()
{
	...
	if (SettingsModule != nullptr)
	{
		SettingsModule->UnregisterSettings("Editor", "Plugins", "VirtualProduction");
...
void FVPUtilitiesEditorModule::InitializeOSCServer()
{
	...
	FString ServerAddress = Settings->OSCServerAddress;
	uint16 ServerPort = Settings->OSCServerPort;

	const UVPUtilitiesEditorUserSettings* UserSettings = GetDefault<UVPUtilitiesEditorUserSettings>();
	const FString& OverrideAddress = UserSettings->OverrideOSCServerAddress;
	uint16 OverridePort = UserSettings->OverrideOSCServerPort;

	if (!OverrideAddress.IsEmpty())
	{
		UE_LOG(LogVPUtilitiesEditor, Display, TEXT("OSC Server address was overridden from %s to %s."), *ServerAddress, *OverrideAddress);
		ServerAddress = OverrideAddress;
	}

	if (OverridePort != 0)
	{
		UE_LOG(LogVPUtilitiesEditor, Display, TEXT("OSC Server port was overridden from %d to %d."), ServerPort, OverridePort);

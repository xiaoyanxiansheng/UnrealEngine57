...
namespace InstallBundleManagerUtil
{
	TSharedPtr<IInstallBundleSource> MakeBundleSource(FInstallBundleSourceType Type)
	{
		...
		{
			if (TSharedPtr<IInstallBundleSource> PlatformSource = MakePlatformBundleSource())
			{
	...
	void LogBundleRequestStats(const InstallBundleUtil::FContentRequestStatsKey& Key, const InstallBundleUtil::FContentRequestStats& RequestStats, ELogVerbosity::Type LogVerbosityOverride)
	{
		FNameBuilder BundleNameBuilder(Key.GetBundleName());
		const TCHAR* BundleName = *BundleNameBuilder;

		LOG_INSTALL_BUNDLE_MAN_OVERRIDE(LogVerbosityOverride, Display, TEXT("------------------------------------------------------"));
		LOG_INSTALL_BUNDLE_MAN_OVERRIDE(LogVerbosityOverride, Display, TEXT("Bundle Request Stats - %s (%p)"), BundleName, Key.GetCookie());
		...
		LOG_INSTALL_BUNDLE_MAN_OVERRIDE(LogVerbosityOverride, Display, TEXT("End Bundle Request Stats - %s (%p)"), BundleName, Key.GetCookie());
		LOG_INSTALL_BUNDLE_MAN_OVERRIDE(LogVerbosityOverride, Display, TEXT("------------------------------------------------------"));
	}
	...
	void FPersistentStatContainer::UpdateForContentState(const FInstallBundleCombinedContentState& ContentState, const FString& SessionName)
	{
		...
		FoundData.bRequiresInstall = ContentState.ContentSize.SpaceRequiredForInstall + ContentState.ContentSize.SpaceRequiredForInstallOtherDirs> 0;
...
namespace InstallBundleManagerAnalytics
{
	...
	void FireEvent_BundleRequestStartedDetailed(IAnalyticsProviderET* AnalyticsProvider,
		const FString& BundleName,
		...
		const FString& CurrentVersion)
	{
		...
		AnalyticsProvider->RecordEvent(TEXT("InstallBundleManager.BundleRequestStartedDetailed"), MakeAnalyticsEventAttributeArray(
			TEXT("BundleName"), BundleName,
			TEXT("PreviousManifest"), PreviousManifest,
	...
	void FireEvent_BundleRequestCompleteDetailed(IAnalyticsProviderET* AnalyticsProvider, 
		const FString& BundleName, 
		bool bDidInstall, 
		const FString& Result, 
		const InstallBundleUtil::FContentRequestStats& TimingStats,
		...
		const TArray<FInstallBundleChunkDownloadInfo>* ChunkDownloadInfos)
	{
		...
		Attributes.Add(FAnalyticsEventAttribute(TEXT("PreviousManfest"), RequestCompleteInfo.PreviousManifest));
		Attributes.Add(FAnalyticsEventAttribute(TEXT("CurrentManifest"), RequestCompleteInfo.CurrentManifest));
		Attributes.Add(FAnalyticsEventAttribute(TEXT("PreviousVersion"), RequestCompleteInfo.OldManifestVersion));
		Attributes.Add(FAnalyticsEventAttribute(TEXT("CurrentVersion"), RequestCompleteInfo.InstallManifestVersion));
		Attributes.Add(FAnalyticsEventAttribute(TEXT("PreviousVersionTimeStamp"), RequestCompleteInfo.OldVersionTimeStamp));
		Attributes.Add(FAnalyticsEventAttribute(TEXT("TotalDownloadedBytes"), RequestCompleteInfo.TotalDownloadedBytes));
		Attributes.Add(FAnalyticsEventAttribute(TEXT("EstimatedFullDownloadBytes"), RequestCompleteInfo.EstimatedFullDownloadBytes));
		Attributes.Add(FAnalyticsEventAttribute(TEXT("BPTErrorCode"), RequestCompleteInfo.BPTErrorCode));
		
		FString ChunckDonwloadInfoString;
		FInstallBundleManagerUtilJsonWriter JsonWriter(&ChunckDonwloadInfoString);
		JsonWriter.WriteArrayStart();
		if (ChunkDownloadInfos)
		{
			for (const auto& ChunkInfo : *ChunkDownloadInfos)
			{
				JsonWriter.WriteObjectStart();

				JsonWriter.WriteValue(TEXT("ChunkName"), ChunkInfo.ChunkName);
				JsonWriter.WriteValue(TEXT("ChunkFileSize"), ChunkInfo.ChunkFileSize);
				JsonWriter.WriteValue(TEXT("ChunkDownloadDuration"), ChunkInfo.ChunkDownloadDuration);
				JsonWriter.WriteValue(TEXT("FetchStartTimeUTC"), ChunkInfo.FetchStartTimeUTC.ToUnixTimestampDecimal());
				JsonWriter.WriteValue(TEXT("FetchEndTimeUTC"), ChunkInfo.FetchEndTimeUTC.ToUnixTimestampDecimal());
		...
		JsonWriter.Close();
		Attributes.Add(FAnalyticsEventAttribute(TEXT("ChunkDownloadCompleteInfos"), FJsonFragment(MoveTemp(ChunckDonwloadInfoString))));

		AnalyticsProvider->RecordEvent(TEXT("InstallBundleManager.BundleRequestCompleteDetailed"), MoveTemp(Attributes));
	}

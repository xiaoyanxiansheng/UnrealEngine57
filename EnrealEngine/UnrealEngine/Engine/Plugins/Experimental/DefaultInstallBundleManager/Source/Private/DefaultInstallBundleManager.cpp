...
FDefaultInstallBundleManager::FDefaultInstallBundleManager(FInstallBundleSourceFactoryFunction InInstallBundleSourceFactory)
	...
	, PersistentStateCache(AsyncIOTasks)
...
FDefaultInstallBundleManager::~FDefaultInstallBundleManager()
{
	...
	FTSTicker::RemoveTicker(ReportingHandle);
	ReportingHandle.Reset();

	InstallBundleUtil::CleanupInstallBundleAsyncIOTasks(AsyncIOTasks);
	
	IInstallBundleManager::InstallBundleChunkDownloadMetricsDelegate.RemoveAll(this);
}
...
void FDefaultInstallBundleManager::Initialize()
{
	...
	ReportingHandle = FTSTicker::GetCoreTicker().AddTicker(FTickerDelegate::CreateRaw(this, &FDefaultInstallBundleManager::TickReporting), 2.0f);
	...
	{
		IInstallBundleManager::InstallBundleChunkDownloadMetricsDelegate.AddSP(this, &FDefaultInstallBundleManager::OnChunkDownloadMetrics);
	}
}
...
void FDefaultInstallBundleManager::TickInit()
{
	...
	while (InitState == EInstallBundleManagerInitState::NotInitialized && InitStepResult == EAsyncInitStepResult::Done)
	{
		...
		case EAsyncInitStep::InitBundleCaches:
			...
			break;
		case EAsyncInitStep::InitPersisentReportCache:
			InitResult = EInstallBundleManagerInitResult::OK;
			AsyncInit_InitPersistentReportCache();
			break;
	...
	while (iRequest < GetContentStateRequests.Num())
	{
		...
		if (Request->Started)
		{
			...
			for (const TPair<FInstallBundleSourceType, FInstallBundleCombinedContentState>& SourcePair : Request->BundleSourceContentStates)
			{
				...
				if (SourceState.IndividualBundleStates.Num()) // FreeSpace is only valid if there's content.
				{
					// Free space should be usually be roughly the same, but Apple platforms can dynamically resize the partition
					// on demand, so we use Max() to ensure we catch the larger size if they've moved on us. It's possible we got
					// less due to another app writing to disk, but in such a case we're already going to be reliant on the in-progress
...
{
	StatsBegin(Request->GetStatsKey(), EContentRequestState::ReservingCache);
...
void FDefaultInstallBundleManager::TryReserveCache(FContentRequestRef Request)
{
	...
	if (!bSuccess)
	{
		...
		StatsEnd(Request->GetStatsKey(), EContentRequestState::ReservingCache);
...
void FDefaultInstallBundleManager::CacheEvictionComplete(TSharedRef<IInstallBundleSource> Source, const FInstallBundleSourceReleaseContentResultInfo& InResultInfo, FCacheEvictionRequestorRef Requestor)
{
	...
	if (Requestor->GetEvictionRequestorType() == ECacheEvictionRequestorType::ContentRequest)
	{
		...
		{
			StatsEnd(ContentRequest->GetStatsKey(), EContentRequestState::ReservingCache);
...
{
	StatsBegin(Request->GetStatsKey(), EContentRequestState::UpdatingBundleSources);
	...
	for (const TSharedPtr<IInstallBundleSource>& Source : EnabledBundleSources)
	{
		...
		Context.StatsCookie = Request->GetStatsCookie();
...
void FDefaultInstallBundleManager::UpdateBundleSourceComplete(TSharedRef<IInstallBundleSource> Source, FInstallBundleSourceUpdateContentResultInfo InResultInfo, FContentRequestRef Request)
{
	...
	if (BundleInfo.bContainsIoStoreOnDemandTocs)
	{
		ensureMsgf(!Request->OnDemandMountArgs.IsEmpty(),
			TEXT("Expected OnDemandMountArgs for bundle %s, check that your bundles sources are configured to match the data being installed. This could also be caused by improper ini settings or CVars."), *Request->BundleName.ToString());
	}
	else
	{
		ensureMsgf(Request->OnDemandMountArgs.IsEmpty(),
			TEXT("Unexpected OnDemandMountArgs for bundle %s, check that your bundles sources are configured to match the data being installed. This could also be caused by improper ini settings or CVars."), *Request->BundleName.ToString());
	}
	...
	StatsEnd(Request->GetStatsKey(), EContentRequestState::UpdatingBundleSources);
...
void FDefaultInstallBundleManager::TickAsyncIOTasks()
{
	CSV_SCOPED_TIMING_STAT(InstallBundleManager, InstallBundleManager_TickAsyncMountTasks);

	InstallBundleUtil::FinishInstallBundleAsyncIOTasks(AsyncIOTasks);
}
...
void FDefaultInstallBundleManager::MountPaks(FContentRequestRef Request)
{
	...
	auto OnPakMountCompleteFunc = [this, WeakThis = AsWeak(), Request](bool bMountedPaks, const TValueOrError<void, UE::UnifiedError::FError>& MountResult) mutable
	{
		...
		if (!MountResult.HasValue())
		{
			...
			StatsEnd(Request->GetStatsKey(), EContentRequestState::Mounting);
	...
	auto OnPakMountingFunc = [this, WeakThis = AsWeak(), Request]() mutable
	{
		TSharedPtr<IInstallBundleManager> PinnedThis = WeakThis.Pin();
		if (!PinnedThis)
		{
		...
		FBundleInfo& BundleInfoLambda = BundleInfoMap.FindChecked(Request->BundleName);
		OnPaksMountingInternal(Request, BundleInfoLambda);
	};
	...
	TUniqueFunction<void()> StartMountPaksFunc = 
		...
		, OnPakMountingFunc = MoveTemp(OnPakMountingFunc)
		, bIsAsyncMount]() mutable
	{
		...
		else
		{
			InstallBundleUtil::StartInstallBundleAsyncIOTask(
				[MountPaksWorkFunc = MoveTemp(MountPaksWorkFunc)
				, OnPakMountCompleteFunc = MoveTemp(OnPakMountCompleteFunc)
				, OnPakMountingFunc = MoveTemp(OnPakMountingFunc)]() mutable
			{
				TValueOrError<void, UE::UnifiedError::FError> MountResult = MakeValue();
				OnPakMountingFunc();
				const bool bMountedPaks = MountPaksWorkFunc(MountResult);
	...
	*/
	...
	if (Request->OnDemandMountArgs.IsEmpty() || BundleInfo.bMountedOnDemandTocs || !AllowIoStoreOnDemandMount(Request, BundleInfo))
	{
		...
		StatsBegin(Request->GetStatsKey(), EContentRequestState::Mounting);
		StartMountPaksFunc();
		return;
	}
			...
			, WeakThis = MoveTemp(WeakThis)]
			{
				...
				for (const UE::IoStore::FOnDemandMountResult& MountResult : Results)
				{
					if (!MountResult.Status.IsOk() && MountResult.Status.GetErrorCode() != EIoErrorCode::PendingEncryptionKey)
					{
						...
						PinnedThis->StatsEnd(Request->GetStatsKey(), EContentRequestState::Mounting);
						TRACE_BOOKMARK(TEXT("Finished Mount Bundle %s"), *Request->BundleName.ToString());
...
void FDefaultInstallBundleManager::WaitForShaderCache(FContentRequestRef Request)
{
	...
	StatsBegin(Request->GetStatsKey(), EContentRequestState::WaitingForShaderCache);
	...
	else
	{
		...
		StatsEnd(Request->GetStatsKey(), EContentRequestState::WaitingForShaderCache);
...
{
	StatsBegin(Request->GetStatsKey(), EContentRequestState::Finishing);
	...
	StatsEnd(Request->GetStatsKey(), EContentRequestState::Finishing);
...
void FDefaultInstallBundleManager::TickContentRequests()
{
	...
	for (FContentRequestRef& Request : ContentRequests[EContentRequestBatch::Requested])
	{
		while (Request->StepResult == EContentRequestStepResult::Done)
		{
			...
			{
				StatsBegin(Request->GetStatsKey());
	...
	while (ContentRequests[EContentRequestBatch::Requested].Num() > 0)
	{
		...
		// If the source supports BundleRequestStartedDelegate (most likely BPS), will skip analytics here, will do it during delegate callback
		bool bSupportsBundleRequestStartedDelegate = false;
		TArray<TSharedPtr<IInstallBundleSource>> EnabledBundleSources = GetEnabledBundleSourcesForRequest(Request);
		for (const TSharedPtr<IInstallBundleSource>& Source : EnabledBundleSources)
		{
			if (Source->SupportsBundleRequestEventDelegates())
			{
				bSupportsBundleRequestStartedDelegate = true;
			}
		}
		if (!bSupportsBundleRequestStartedDelegate)
		{
	...
	for (int i = 0; i < ContentRequests[EContentRequestBatch::Install].Num() && FPlatformTime::Seconds() < ContentInstallEndTime;)
	{
		...
		while (!bRequestComplete && Request->StepResult == EContentRequestStepResult::Done)
		{
			...
			case EContentRequestState::CleaningUp:
				StatsEnd(Request->GetStatsKey());
				LogStats(Request->GetStatsKey(), Request->LogVerbosityOverride);

				CacheHintRequested(Request, false);

				if (Request->bShouldSendAnalytics || Request->Result != EInstallBundleResult::OK)
				{
					// check if source delegate has provided data from BPS, if so send a combined event
					auto Data = BundleRequestCompleteInfoMap.Find(Request->BundleName);
					if (Data)
					{
						InstallBundleManagerAnalytics::FireEvent_BundleRequestCompleteDetailed(AnalyticsProvider.Get(),
							Request->BundleName.ToString(),
							Request->bContentWasInstalled,
							LexToString(Request->Result),
							StatsMap->FindStatsChecked(Request->GetStatsKey()),
							*Data,
							ChunkDownloadInfoMap.Find(Request->BundleName));
						BundleRequestCompleteInfoMap.Remove(Request->BundleName);
						ChunkDownloadInfoMap.Remove(Request->BundleName);
					}
					else
					{
				...
				StatsMap->StatsReset(Request->GetStatsKey());
...
bool FDefaultInstallBundleManager::TickReporting(float dt)
{
	// If it it hasn't changed, don't do anything
	if (bPersistentStateDirty)
	{
		bPersistentStateDirty = false;

		// Update the cache now (so it's saved to disk), and broadcast to listeners
		PersistentStateCache.SetReport(PersistentState);
		ReportingDelegate.Broadcast(PersistentState);
		//FPlatformMisc::LowLevelOutputDebugStringf(TEXT("InstallBundleManager: %s"), *PersistentState.GetLongProgressString());

		// If at any point the "total amount of bytes to download" exceeds 1k, we say we've updated content.
		// Some platforms do a miniature 30-ish byte update after they've been patched from the outside (e.g. EGL), we don't want to count those as "has updated anything".
		if (!bHasEverUpdatedContent && PersistentState.TotalDownloadSize() > 1024)
		{
	...
	InstallBundleUtil::FContentRequestSharedContextPtr RequestSharedContext /*= nullptr*/)
{
	...
	TSet<FName> BundlesToLoad;
	if (EnumHasAnyFlags(Flags, EInstallBundleRequestFlags::ExplicitUpdateList))
	{
		BundlesToLoad.Append(InBundleNames);
	}
	else
	{
		BundlesToLoad = GatherBundlesForRequest(InBundleNames, RetInfo.InfoFlags);
	}
	...
	for (int i = 0; i < SourcesToCreate.Num(); ++i)
	{
		...
		if (!Source)
		{
			...
			if (InitInfo.Result == EInstallBundleManagerInitResult::OK)
			{
				...
				if (Source->SupportsReportingDelegate())
				{
					Source->SetReportingDelegate(FInstallBundleReportingDelegate::CreateLambda([this, WeakSource = Source.ToWeakPtr()](FInstallBundleReport BundleReport)
						{
							checkf(IsInGameThread(), TEXT("ReportingDelegate may only be called on game thread"));
							TSharedPtr<IInstallBundleSource> PinnedSource = WeakSource.Pin();
							if (PinnedSource.IsValid())
							{
								FInstallBundleSourceReport& PersistentSourceReport = PersistentState.FindOrAddBundleSourceReport(PinnedSource->GetSourceType());
								FInstallBundleReport& PersistentReport = PersistentSourceReport.FindOrAddBundleReport(BundleReport.BundleName);

								const int32 PersistentIndex = PersistentReport.State.IsSet() ? PersistentReport.State->GetIndex() : -1;
								const int32 NewIndex = BundleReport.State.IsSet() ? BundleReport.State->GetIndex() : -1;

								if (PersistentIndex != NewIndex)
								{
									LOG_INSTALL_BUNDLE_MAN(Verbose, TEXT("InstallBundleManager: bundle %s changed type index from %d to %d"), *BundleReport.BundleName.ToString(), PersistentIndex, NewIndex);
								}

								// We blindly copy what comes in if there is no state set yet in the persistent data
								if (!PersistentReport.State.IsSet())
								{
									PersistentReport = BundleReport;
									bPersistentStateDirty = true;
								}
								// Otherwise, if the persistent state is not set to FStateUpdated, we take whatever came in, but we add the difference
								// in total to compensate for a source potentially reporting less than what was truly needed
								else if (!PersistentReport.State->IsType<FInstallBundleReport::FStateUpdated>())
								{
									const uint64 TotalPersistent = PersistentReport.TotalDownloadSize();
									const uint64 TotalNew = BundleReport.TotalDownloadSize();
									if (ensureMsgf(TotalNew <= TotalPersistent, TEXT("Bundle %s has TotalPersistent=%" UINT64_FMT ", and TotalNew=%" UINT64_FMT ", which is not expected"), *BundleReport.BundleName.ToString(), TotalPersistent, TotalNew))
									{
										if (const uint64 Discrepancy = TotalPersistent - TotalNew)
										{
											BundleReport.AddDiscrepancy(Discrepancy);
										}
									}
									PersistentReport = BundleReport;
									bPersistentStateDirty = true;
				...
				if (Source->SupportsBundleRequestEventDelegates())
				{
					Source->SetBundleRequestStartedDelegate(FInstallBundleRequestStartedDelegate::CreateLambda([this, WeakSource = Source.ToWeakPtr()]
																												...
																												const FString& ManifestVersion)
					{
						for (EContentRequestBatch b : TEnumRange<EContentRequestBatch>())
						{
							for (const FContentRequestRef& Request : ContentRequests[b])
							{
								if (Request->bShouldSendAnalytics && Request->BundleName.IsEqual(BundleName))
								{
									InstallBundleManagerAnalytics::FireEvent_BundleRequestStartedDetailed(AnalyticsProvider.Get(),
										Request->BundleName.ToString(),
										ManifestName,
										ManifestPath,
										ManifestVersion);
					...
					Source->SetBundleRequestCompletedDelegate(FInstallBundleRequestCompletedDelegate::CreateLambda([this, WeakSource = Source.ToWeakPtr()]
																												   (const FName& BundleName,
																													const FBundleRequestCompleteInfo& AnalyticInfo)
					{
						// Save for later complete event firing
						BundleRequestCompleteInfoMap.Emplace(BundleName, AnalyticInfo);
					}));
				}
...
void FDefaultInstallBundleManager::OnChunkDownloadMetrics(FInstallBundleChunkDownloadInfo ChunkDownloadInfo)
{
	if (auto ChunkDownloadInfos = ChunkDownloadInfoMap.Find(ChunkDownloadInfo.BundleName))
	{
		ChunkDownloadInfos->Add(ChunkDownloadInfo);
	}
	else
	{
		ChunkDownloadInfoMap.Add(ChunkDownloadInfo.BundleName, TArray<FInstallBundleChunkDownloadInfo>{ChunkDownloadInfo});
	}
}
...
void FDefaultInstallBundleManager::AsyncInit_InitPersistentReportCache()
{
	LOG_INSTALL_BUNDLE_MAN(Display, TEXT("Initializing persistent report cache..."));
	InitStepResult = EAsyncInitStepResult::Waiting;
	PersistentStateCache.Load(FInstallManagerBundleReportCacheDoneLoadingDelegate::CreateLambda([this]() {
		LOG_INSTALL_BUNDLE_MAN(Display, TEXT("Persistent report cache context set to [%s]"), *PersistentState.ContextName.ToString());
		PersistentState = PersistentStateCache.GetReport();
		InitStepResult = EAsyncInitStepResult::Done;
	}));
}
...
{
	// no longer do this. We use a default SessionID GUID now when creating this analytics provider, 
	// and no longer need to wait until we have bundle sources to set the session id.
	// We could remove this step, but it would make a more disruptive change.
	InitStepResult = EAsyncInitStepResult::Done;
}
...
void FDefaultInstallBundleManager::StatsBegin(const InstallBundleUtil::FContentRequestStatsKey& Key)
{
	StatsMap->StatsBegin(Key);
}

void FDefaultInstallBundleManager::StatsEnd(const InstallBundleUtil::FContentRequestStatsKey& Key)
{
	StatsMap->StatsEnd(Key);
}

void FDefaultInstallBundleManager::StatsBegin(const InstallBundleUtil::FContentRequestStatsKey& Key, EContentRequestState State)
{
	StatsMap->StatsBegin(Key, LexToString(State));
}

void FDefaultInstallBundleManager::StatsEnd(const InstallBundleUtil::FContentRequestStatsKey& Key, EContentRequestState State, uint64 DataSize /*= 0*/)
{
	StatsMap->StatsEnd(Key, LexToString(State), DataSize);
}

void FDefaultInstallBundleManager::LogStats(const InstallBundleUtil::FContentRequestStatsKey& Key, ELogVerbosity::Type LogVerbosityOverride)
{
	const InstallBundleUtil::FContentRequestStats& RequestStats = StatsMap->FindStatsChecked(Key);
	check(!RequestStats.bOpen);

	InstallBundleManagerUtil::LogBundleRequestStats(Key, RequestStats, LogVerbosityOverride);
}

void FDefaultInstallBundleManager::SetContext(FName NewContextName) {
	LOG_INSTALL_BUNDLE_MAN(Display, TEXT("Changing context to [%s]"), *NewContextName.ToString());

	// Check prerequisites
	checkf(InitState == EInstallBundleManagerInitState::Succeeded, TEXT("Init state must be set to Succeeded when setting a new context"));
	checkf(ContextName == FName() || NewContextName == FName(), TEXT("Cannot have active context when setting a new context"));
	ContextName = NewContextName;

	// If we have the same context as the last time, check if any of our sources are planning to update to a different version 
	if (PersistentState.ContextName == ContextName)
	{
		bool bFoundTargetVersionMismatch = false;
		for (const TPair<FInstallBundleSourceType, TSharedPtr<IInstallBundleSource>>& Pair : BundleSources)
		{
			if (Pair.Value->SupportsReportingDelegate())
			{
				FInstallBundleSourceReport* BundleSourceReport = PersistentState.TryFindBundleSourceReport(Pair.Key);
				if (!BundleSourceReport || !BundleSourceReport->TargetVersion.IsSet() || BundleSourceReport->TargetVersion.GetValue() != Pair.Value->GetContentVersion())
				{
					bFoundTargetVersionMismatch = true;
					break;
				}
			}
		}

		// The context name is the same as last time, and all bundles still updating to the same version, we can continue
		// with this persistent state.
		if (!bFoundTargetVersionMismatch)
		{
			LOG_INSTALL_BUNDLE_MAN(Display, TEXT("Keep persistent state for context [%s]"), *ContextName.ToString());
			return;
		}
	}

	// Rebuild the persistent state from scratch
	LOG_INSTALL_BUNDLE_MAN(Display, TEXT("Resetting persistent state to context [%s]"), *ContextName.ToString());
	PersistentState = {};
	PersistentState.ContextName = ContextName;
	PersistentState.SessionId = FGuid::NewGuid().ToString();
	for (const TPair<FInstallBundleSourceType, TSharedPtr<IInstallBundleSource>>& Pair : BundleSources)
	{
		if (Pair.Value->SupportsReportingDelegate())
		{
			PersistentState.FindOrAddBundleSourceReport(Pair.Key).TargetVersion = Pair.Value->GetContentVersion();
		}
	}

	// NOTE: we don't call FPersistentStateCache::SetReport, so that we don't store the XML file if the system isn't otherwise used
}

bool FDefaultInstallBundleManager::CopyReportTo(FInstallManagerBundleReport& Report)
{
	Report = PersistentState;
	return true;
}

...
private:
	inline void SetProfilerEventStarted() { ensureMsgf(!bProfilerEventStarted, TEXT("Started a new control flow profiler event before previous step completed.")); bProfilerEventStarted = true; }

	void HandleControlFlowNodeCompleted(const TSharedRef<const FControlFlowNode>& NodeCompleted);

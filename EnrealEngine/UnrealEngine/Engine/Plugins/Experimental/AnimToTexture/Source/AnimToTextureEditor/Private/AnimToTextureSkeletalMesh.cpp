...
// NOTE: Unfortunatelly SkeletalMeshDescription is only generated for LOD0. 
	...
	TArray<FIntVector3>& OutTriangles)
{
	...
	for (const FTriangleID& TriangleID : Triangles.GetElementIDs())
	{
		...
		OutTriangles[TriangleIndex].X = TriangleVertices[0].GetValue();
		OutTriangles[TriangleIndex].Y = TriangleVertices[1].GetValue();
		OutTriangles[TriangleIndex].Z = TriangleVertices[2].GetValue();
	}
	...
	TArray<VertexSkinWeightMax>& OutSkinWeights)
{
	...
	const FSkinWeightsVertexAttributesRef SkinWeights = SkeletalMeshAttributes.GetVertexSkinWeights();
	const FSkeletalMeshAttributes::FBoneNameAttributesRef BoneNames = SkeletalMeshAttributes.GetBoneNames();

	const int32 NumBones = SkeletalMeshAttributes.GetNumBones();

	// Allocated SkinWeightData
	OutSkinWeights.SetNumZeroed(NumVertices);

	for (const FVertexID& VertexID : Vertices.GetElementIDs())
	{
		const int32 VertexIndex = VertexID.GetValue();
		const FVertexBoneWeights BoneWeights = SkinWeights.Get(VertexID);
		check(BoneWeights.Num() < MAX_TOTAL_INFLUENCES)

		for (int32 WeightIndex = 0; WeightIndex < BoneWeights.Num(); WeightIndex++)
		{
			// Note: BoneIndex == RawBoneIndex
			const int32 BoneIndex = BoneWeights[WeightIndex].GetBoneIndex();
			const float BoneWeight = BoneWeights[WeightIndex].GetWeight();

			OutSkinWeights[VertexIndex].BoneWeights[WeightIndex] = (uint8)FMath::RoundToInt(BoneWeight * 255.f);
			OutSkinWeights[VertexIndex].MeshBoneIndices[WeightIndex] = BoneIndex;
		}
	}
}
*/
	...
	TArray<FVector3f>& OutPositions)
{
	...
	for (int32 VertexIndex = 0; VertexIndex < NumVertices; VertexIndex++)
	{
		...
		for (int32 Index = 0; Index < MAX_TOTAL_INFLUENCES; Index++)
		{
			...
			// no need to transform if bone doesnt have weights.
			if (BoneWeight > 0) 
			{	
				// get matrix

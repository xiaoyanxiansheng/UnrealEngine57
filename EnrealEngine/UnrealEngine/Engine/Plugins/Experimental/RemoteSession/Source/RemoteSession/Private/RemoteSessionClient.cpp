...
FRemoteSessionClient::FRemoteSessionClient(const TCHAR* InHostAddress, bool bUsingPixelStreaming)
{
	...
	bIsUsingPixelStreaming = bUsingPixelStreaming;
...
void FRemoteSessionClient::AppendExtraInfoToBackChannelPacket(TBackChannelSharedPtr<IBackChannelPacket> Packet)
{
	if (Packet->GetPath() == kHelloEndPoint)
	{
		if (bIsUsingPixelStreaming)
		{
			if (FModuleManager::Get().IsModuleLoaded("PixelStreaming2"))
			{
				Packet->Write(TEXT("PSVersion"), TEXT("PS2"));
			}
			else if (FModuleManager::Get().IsModuleLoaded("PixelStreaming"))
			{
...
void FRemoteSessionClient::OnReceiveHello(IBackChannelPacket& Message)
{

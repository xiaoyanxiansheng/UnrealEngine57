 ...
 */
class FRemoteSessionImageChannel : public IRemoteSessionChannel, FRunnable
{
public:
...
public:
	...
	UE_API FRemoteSessionImageChannel(ERemoteSessionChannelMode InRole, TSharedPtr<IBackChannelConnection, ESPMode::ThreadSafe> InConnection);

	UE_API ~FRemoteSessionImageChannel();

	/** Tick this channel */
	UE_API virtual void Tick(const float InDeltaTime) override;

	/** Get the client Texture2D to display */
	UE_API UTexture2D* GetHostScreen() const;

	/** Set the ImageProvider that will produce the images that will be sent to the client */
	UE_API void SetImageProvider(TSharedPtr<IRemoteSessionImageProvider> ImageProvider);

	/** Sets up an image provider that mirrors the games framebuffer. WIll be the default if no ImageProvider is set. */
	UE_API void SetFramebufferAsImageProvider();

	/** Set the jpg compression quality */
	UE_API void SetCompressQuality(int32 InQuality);
...
protected:
	...
	UE_API void ReceiveHostImage(IBackChannelPacket& Message);

	/** Creates a texture to receive images into */
	UE_API void CreateTexture(const int32 InSlot, const int32 InWidth, const int32 InHeight);
	...
	UE_API void ProcessIncomingTextures();

protected:
	
	/** Background thread for decoding incoming images */
	UE_API uint32 Run();
	UE_API void StartBackgroundThread();
	UE_API void ExitBackgroundThread();

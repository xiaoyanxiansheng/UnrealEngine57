...
public:
	...
	UE_API FRemoteSessionARCameraChannel(ERemoteSessionChannelMode InRole, TSharedPtr<IBackChannelConnection, ESPMode::ThreadSafe> InConnection);

	UE_API ~FRemoteSessionARCameraChannel();
	...
	UE_API UMaterialInterface* GetPostProcessMaterial() const;

protected:
	/** Sends the most recent compressed AR camera image to the remote */
	UE_API void SendARCameraImage();
	/** Compresses in the background and queues for sending */
	UE_API void QueueARCameraImage();

	/** Handles data coming from the client */
	UE_API void ReceiveARCameraImage(IBackChannelPacket& Message);

	/** Updates the next texture to be rendered */
	UE_API void UpdateRenderingTexture();
...
class FRemoteSessionARCameraChannelFactoryWorker : public IRemoteSessionChannelFactoryWorker
{
public:

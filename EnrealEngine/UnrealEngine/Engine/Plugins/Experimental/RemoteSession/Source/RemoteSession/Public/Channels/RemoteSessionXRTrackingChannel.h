...
public:
	...
	UE_API virtual bool EnumerateTrackedDevices(TArray<int32>& OutDevices, EXRTrackedDeviceType Type) override;
	UE_API virtual bool GetCurrentPose(int32 DeviceId, FQuat& OutOrientation, FVector& OutPosition) override;
	UE_API virtual FName GetSystemName() const override;
	virtual int32 GetXRSystemFlags() const override { return 0; }
};

class FRemoteSessionXRTrackingChannel :
	public IRemoteSessionChannel
{
public:

	UE_API FRemoteSessionXRTrackingChannel(ERemoteSessionChannelMode InRole, TSharedPtr<IBackChannelConnection, ESPMode::ThreadSafe> InConnection);

	UE_API virtual ~FRemoteSessionXRTrackingChannel();
	...
	UE_API void SendXRTracking();

	/** Handles data coming from the client */
	UE_API void ReceiveXRTracking(IBackChannelPacket& Message);
...
protected:
	...
	UE_API FRemoteSessionXRTrackingChannel(ERemoteSessionChannelMode InRole, TSharedPtr<IBackChannelConnection, ESPMode::ThreadSafe> InConnection, IARSystemSupport* ARSystemSupport);

...
{
	GENERATED_USTRUCT_BODY()
	DATAFLOW_NODE_DEFINE_INTERNAL(FBoundingBoxDataflowNode, "GetCollectionBoundingBox", "Utilities|Box", "Collection Bounds Size Dimensions Extents Center")
	DATAFLOW_NODE_RENDER_TYPE("SurfaceRender",FName("FBox"), "BoundingBox")

public:
	/** Collection to compute the bouning box from */
	UPROPERTY(meta = (DataflowInput))
	FManagedArrayCollection Collection;

	/** Geometric bounding box of the input collection */
	UPROPERTY(meta = (DataflowOutput))
...
{
	GENERATED_USTRUCT_BODY()
	DATAFLOW_NODE_DEFINE_INTERNAL(FExpandBoundingBoxDataflowNode, "BreakBox", "Utilities|Box", "Bounds Bounding Expand Min Max Center Half Extents Volume")

public:
	UPROPERTY(meta = (DataflowInput, DisplayName = "Box"))
	FBox BoundingBox = FBox(ForceInit);;
...
{
	GENERATED_USTRUCT_BODY()
	DATAFLOW_NODE_DEFINE_INTERNAL(FExpandBoundingSphereDataflowNode, "BreakBoundingSphere", "Utilities|Sphere", "Expand Radius Center Volume")
...
{
	GENERATED_USTRUCT_BODY()
	DATAFLOW_NODE_DEFINE_INTERNAL(FExpandVectorDataflowNode, "BreakVector", "Utilities|Vector", "Expand X Y Z")
...
private:
	...
	/** Control point color by level or setting */
	UPROPERTY(EditAnywhere, Category = "Debug Draw")
	bool bColorByLevel = true;

	/** Point color */
	UPROPERTY(EditAnywhere, Category = "Debug Draw", meta = (EditCondition = "bColorByLevel == false", EditConditionHides))
	FLinearColor Color = FLinearColor::Blue;

	/** Control the point size by level or setting */
	UPROPERTY(EditAnywhere, Category = "Debug Draw")
	bool bSizeByLevel = true;

	/** Point size */
	UPROPERTY(EditAnywhere, Category = "Debug Draw", meta = (EditCondition = "bSizeByLevel == false", EditConditionHides))
	float Size = 8.f;

	/** Float curve to control point size by level */
	UPROPERTY(EditAnywhere, Category = "Debug Draw")
	FRuntimeFloatCurve PointSize;

	/** Output centroids */
	UPROPERTY(meta = (DataflowOutput, DisplayName = "Points"))
	TArray<FVector> Centroids = TArray<FVector>();

	/** Hidden output to store computed level values for centroids */
	UPROPERTY(meta = (DataflowOutput))
	TArray<int32> Levels;

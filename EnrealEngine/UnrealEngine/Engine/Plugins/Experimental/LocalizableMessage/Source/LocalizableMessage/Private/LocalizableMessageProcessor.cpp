...
FText FLocalizableMessageProcessor::Localize(const FLocalizableMessage& Message, const FLocalizationContext& Context)
{
	...
	{
		FTextId TextId;
		if (UE::LocalizableMessageTextInterop::MessageKeyToTextId(Message.Key, TextId))
		{
			LocalizedText = FText::AsLocalizable_Advanced(TextId.GetNamespace(), TextId.GetKey(), Message.DefaultText);
		}
		else
		{
			LocalizedText = FText::AsCultureInvariant(Message.DefaultText);
		}
	}

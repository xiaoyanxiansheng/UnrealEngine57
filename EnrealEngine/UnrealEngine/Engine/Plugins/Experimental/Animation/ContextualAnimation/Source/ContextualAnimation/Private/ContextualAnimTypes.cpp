...
bool FContextualAnimTrack::DoesQuerierPassSelectionCriteria(const FContextualAnimSceneBindingContext& Primary, const FContextualAnimSceneBindingContext& Querier) const
{
	for (const UContextualAnimSelectionCriterion* Criterion : SelectionCriteria)
	{
		...
		{
			UE_LOG(LogContextualAnim, Verbose, TEXT("\t\t\t '%s' condition check failed."), *GetNameSafe(Criterion));
			return false;
		}
		else
		{
			UE_LOG(LogContextualAnim, Verbose, TEXT("\t\t\t '%s' condition check passed."), *GetNameSafe(Criterion));
		}
	}
...
bool FContextualAnimSceneBindings::CheckConditions(const UContextualAnimSceneAsset& SceneAsset, int32 SectionIdx, int32 AnimSetIdx, const TMap<FName, FContextualAnimSceneBindingContext>& Params)
{
	...
	{
		AActor* Actor = Querier.GetActor();
		UContextualAnimSceneActorComponent* SceneActorComp = Actor ? Actor->FindComponentByClass<UContextualAnimSceneActorComponent>() : nullptr;
		if (SceneActorComp == nullptr)
		{
			UE_LOG(LogContextualAnim, Verbose, TEXT("\t\t\t Missing ContextualAnimSceneActorComponent on the Actor."));
			return false;
		}
		...
		{
			UE_LOG(LogContextualAnim, Verbose, TEXT("\t\t\t Missing AnimTrack in SceneAsset for role '%s'."), *Role.ToString());
			return false;
		}

		if (AnimTrack->DoesQuerierPassSelectionCriteria(Primary, Querier) == false)
		{
			return false;
		}
	...
	{
		UE_LOG(LogContextualAnim, Verbose, TEXT("\t\t Missing actor to bind to the primary role '%s'."), *PrimaryRole.ToString());
		return false;
	}
	...
	UE_LOG(LogContextualAnim, Verbose, TEXT("\t\t Checking binding conditions for primary role '%s' (Actor: %s)"), *PrimaryRole.ToString(), *GetNameSafe(PrimaryPtr->GetActor()));

	if (DoCheck(*PrimaryPtr, *PrimaryPtr, PrimaryRole) == false)
	{
		UE_LOG(LogContextualAnim, Verbose, TEXT("\t\t Actor '%s' failed to pass some of the conditions for primary role '%s'."), *GetNameSafe(PrimaryPtr->GetActor()), *PrimaryRole.ToString());
		return false;
	}
	else
	{
		UE_LOG(LogContextualAnim, Verbose, TEXT("\t\t Actor '%s' passed all the conditions for primary role '%s'."), *GetNameSafe(PrimaryPtr->GetActor()), *PrimaryRole.ToString());
	}
	...
	for (const auto& Pair : Params)
	{
		...
		{
			UE_LOG(LogContextualAnim, Verbose, TEXT("\t\t Checking binding conditions for role '%s' (Actor: %s)"), *RoleToBind.ToString(), *GetNameSafe(Pair.Value.GetActor()));
			if (DoCheck(*PrimaryPtr, Pair.Value, RoleToBind) == false)
			{
				UE_LOG(LogContextualAnim, Verbose, TEXT("\t\t Actor '%s' failed to pass some of the conditions for role '%s'."), *GetNameSafe(Pair.Value.GetActor()), *RoleToBind.ToString());
				return false;
			}
			else
			{
				UE_LOG(LogContextualAnim, Verbose, TEXT("\t\t Actor '%s' passed all the conditions for role '%s'."), *GetNameSafe(Pair.Value.GetActor()), *RoleToBind.ToString());	
				ValidEntries++;
			}
		}
	}
	...
	{
		UE_LOG(LogContextualAnim, Verbose, TEXT("\t Missing mandatory roles %d/%d. SceneAsset: %s SectionIdx: %d AnimSetIdx: %d"),
...
int32 FContextualAnimSceneBindings::FindAnimSet(const UContextualAnimSceneAsset& SceneAsset, int32 SectionIdx, const TMap<FName, FContextualAnimSceneBindingContext>& Params)
{
	...
	const int32 NumSets = Section->GetNumAnimSets();
	int32 NumValid = 0;
	
	UE_LOG(LogContextualAnim, Verbose, TEXT("Find AnimSet in Section '%d' ('%s'). NumSets: %d. SceneAsset: %s"), 
		SectionIdx, *Section->GetName().ToString(), NumSets, *GetNameSafe(&SceneAsset));

	if (NumSets == 1)
	{
		const FContextualAnimSet* AnimSet = Section->GetAnimSet(0);
		UE_LOG(LogContextualAnim, Verbose, TEXT("\t Checking binding conditions for AnimSet '0' ('%s')."), *AnimSet->Name.ToString());
		...
		if (CheckConditions(SceneAsset, SectionIdx, 0, Params))
		{
			...
			UE_LOG(LogContextualAnim, Verbose, TEXT("\t AnimSet '0' ('%s') can be selected"), *AnimSet->Name.ToString());
		}
		else
		{
			UE_LOG(LogContextualAnim, Verbose, TEXT("\t AnimSet '0' ('%s') can NOT be selected."), *AnimSet->Name.ToString());
		}
	}
	else if (NumSets > 1)
	{
		...
		for (int32 AnimSetIdx = 0; AnimSetIdx < NumSets; AnimSetIdx++)
		{
			...
			UE_LOG(LogContextualAnim, Verbose, TEXT("\t Checking binding conditions for AnimSet '%d' ('%s')."), AnimSetIdx, *AnimSet->Name.ToString());

			if (CheckConditions(SceneAsset, SectionIdx, AnimSetIdx, Params))
			{
				const float AnimSetRandomWeight = FMath::Max(AnimSet->RandomWeight, 0);
				ValidSets.Add(MakeTuple(AnimSetIdx, AnimSetRandomWeight));
				TotalWeight += AnimSetRandomWeight;

				UE_LOG(LogContextualAnim, Verbose, TEXT("\t AnimSet '%d' ('%s') can be selected. Adding it to the list of valid sets with a random weight of %.2f."), 
					AnimSetIdx, *AnimSet->Name.ToString(), AnimSetRandomWeight);
			}
			else
			{
				UE_LOG(LogContextualAnim, Verbose, TEXT("\t AnimSet '%d' ('%s') can NOT be selected."), 
					AnimSetIdx, *Section->GetAnimSet(AnimSetIdx)->Name.ToString());
			}
		}
		...
		NumValid = ValidSets.Num();
		float RandomValue = FMath::RandRange(0.f, TotalWeight);
		for (int32 Idx = 0; Idx < NumValid; Idx++)
		{
	...
	{
		UE_LOG(LogContextualAnim, Verbose, TEXT("Find AnimSet Result: SUCCESS. %d/%d AnimSets have passed the binding conditions. AnimSet '%d' ('%s') was selected."), 
			NumValid, NumSets, AnimSetIdxSelected, *SceneAsset.GetSection(SectionIdx)->GetAnimSet(AnimSetIdxSelected)->Name.ToString());
	}
	else
	{
		UE_LOG(LogContextualAnim, Verbose, TEXT("Find AnimSet Result: FAILURE. No AnimSet passed the binding conditions."));
	}

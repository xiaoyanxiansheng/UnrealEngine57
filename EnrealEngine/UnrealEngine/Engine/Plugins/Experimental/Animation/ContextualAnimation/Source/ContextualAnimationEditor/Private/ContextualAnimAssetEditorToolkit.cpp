...
TSharedRef<SDockTab> FContextualAnimAssetEditorToolkit::SpawnTab_Timeline(const FSpawnTabArgs& Args)
{
	...
	TSharedRef<SDockTab> SpawnedTab = SNew(SDockTab).Label(LOCTEXT("TimelineTab_Title", "Timeline"));

	TSharedPtr<ISequencer> SequencerPtr = ViewModel ? ViewModel->GetSequencer() : nullptr;
	if (ViewModel && ViewModel->GetSequencer() != nullptr)
	{
		SpawnedTab->SetContent(SequencerPtr->GetSequencerWidget());
	}

...
UFunction* FindMakeBreakFunction(const UScriptStruct* InStruct, const FName& InNativeMetaDataKey, const FName& InScriptDefaultMetaDataKey, const NotFoundFuncT& NotFoundFn)
{
	...
	if (!InStruct->HasMetaData(InScriptDefaultMetaDataKey))
	{
		...
		{
			// Find the function.
			MakeBreakFunc = FindObject<UFunction>(/*Outer*/nullptr, *MakeBreakFunctionName, EFindObjectFlags::ExactClass);
			if (!MakeBreakFunc)
			{
...
void FPyWrapperTypeRegistry::GenerateWrappedTypesForModule(const FName ModuleName)
{
	...
	{
		if (UFunction* MakeFunc = FindObject<UFunction>(nullptr, *Pair.Key, EFindObjectFlags::ExactClass))
		{
	...
	{
		if (UFunction* BreakFunc = FindObject<UFunction>(nullptr, *Pair.Key, EFindObjectFlags::ExactClass))
		{

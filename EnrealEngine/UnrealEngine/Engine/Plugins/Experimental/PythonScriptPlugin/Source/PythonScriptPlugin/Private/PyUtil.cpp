...
void AddSitePackagesPath(const FString& InPath)
{
	...
	// We can safely call site.addsitedir on Python 3.13 as it is now decoded first with utf-8 https://docs.python.org/3/library/site.html
	if (FPyObjectPtr PySiteModule = FPyObjectPtr::StealReference(PyImport_ImportModule("site")))
	{
		...
		if (PyObject* PyAddSiteDirFunc = PyDict_GetItemString(PySiteDict, "addsitedir"))
		{
			...
			if (PyConversion::Pythonize(InPath, PyPath.Get(), PyConversion::ESetErrorState::No))
			{
				...
				if (!PyAddSiteDirResult)
				{
					PyUtil::LogPythonError(); // Ensure the error is logged and cleared
				}
			}
		}
	}
#else
	// Before Python 3.13, we cannot call site.addsitedir directly as it uses a TextIOWrapper with a default "locale" encoding to read .pth files 
	// (which is inconsistent depending on the platform) while we need the .pth to be consistently read as utf-8.
	// As a workaround, we temporarily redefine TextIOWrapper to force a utf-8 encoding
	FString PyCommandStr = FString::Printf(TEXT(
	"import io\n"
	"import site\n"
	
	"old_TextIOWrapper = io.TextIOWrapper\n" // Save the existing io.TextIOWrapper implementation
	"def TextIOWrapper_utf8(buffer, encoding = None):\n"
	"    return old_TextIOWrapper(buffer, encoding='utf-8')\n" // Create a new io.TextIOWrapper forcing the encoding to be utf-8
	
	"io.TextIOWrapper = TextIOWrapper_utf8\n" // Redefine io.TextIOWrapper
	
	"try:\n"
	"    site.addsitedir(r\"%s\")\n" // sets the sitedir
	"finally:\n"
	"    io.TextIOWrapper = old_TextIOWrapper\n" // Reset io.TextIOWrapper to the original implementation
	), *InPath);
	
	ensure(IPythonScriptPlugin::Get()->ExecPythonCommand(*PyCommandStr));
#endif
}

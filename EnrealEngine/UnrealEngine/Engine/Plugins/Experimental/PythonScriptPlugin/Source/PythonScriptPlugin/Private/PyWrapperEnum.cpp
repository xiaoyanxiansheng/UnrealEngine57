...
struct FPyWrapperEnumValueDescrObject
{
	...
	static FPyWrapperEnumValueDescrObject* NewDeprecated(FPyWrapperEnum* InEnumEntry, const char* InEnumEntryName, const char* InEnumEntryDoc)
	{
		FPyWrapperEnumValueDescrObject* Self = (FPyWrapperEnumValueDescrObject*)PyWrapperEnumValueDescrType.tp_alloc(&PyWrapperEnumValueDescrType, 0);
		if (Self)
		{
			...
			new(&Self->DeprecationMessage) TOptional<FString>(FString::Printf(TEXT("Enum entry '%s.%s' is deprecated: Use '%s.%s'"), 
				...
				UTF8_TO_TCHAR(Py_TYPE(Self->EnumEntry)->tp_name), *PyUtil::PyObjectToUEString(Self->EnumEntry->EntryName)
				));
		}
		return Self;
	}
...
FPyWrapperEnum* FPyWrapperEnum::AddDeprecatedEnumEntry(PyTypeObject* InType, FPyWrapperEnum* InEnumEntry, const char* InEnumEntryName, const char* InEnumEntryDoc)
{
	if (!FPyWrapperEnumMetaData::IsEnumFinalized(InType))
	{
		FPyWrapperEnumValueDescrObjectPtr PyEnumValueDescr = FPyWrapperEnumValueDescrObjectPtr::StealReference(FPyWrapperEnumValueDescrObject::NewDeprecated(InEnumEntry, InEnumEntryName, InEnumEntryDoc));
		if (PyEnumValueDescr)
		{
			PyDict_SetItemString(InType->tp_dict, InEnumEntryName, (PyObject*)PyEnumValueDescr.GetPtr());
			return PyEnumValueDescr->EnumEntry;
...
PyTypeObject InitializePyWrapperEnumValueDescrType()
{
	struct FFuncs
	{
		...
		static PyObject* DescrGet(FPyWrapperEnumValueDescrObject* InSelf, PyObject* InObj, PyObject* InType)
		{
			...
			if (InSelf->DeprecationMessage.IsSet() && PyUtil::SetPythonWarning(PyExc_DeprecationWarning, InSelf->EnumEntry, *InSelf->DeprecationMessage.GetValue()) == -1)
			{
				// -1 from SetPythonWarning means the warning should be an exception
				return nullptr;
			}
...
void UPythonGeneratedEnum::AddReferencedObjects(UObject* InThis, FReferenceCollector& Collector)
{
	Super::AddReferencedObjects(InThis, Collector);

	UPythonGeneratedEnum* This = CastChecked<UPythonGeneratedEnum>(InThis);

...
bool InitializePyConstant()
{
	if (!ensure(PyType_Ready(&PyConstantDescrType) == 0))
	{
		PyUtil::LogPythonError();
		UE_LOG(LogPython, Error, TEXT("PyType_Ready(&PyConstantDescrType) failed! Python will be disabled."))
		return false;
	}
	return true;
}

...
bool InitializePyMethodWithClosure()
{
	if (!ensure(PyType_Ready(&PyCFunctionWithClosureType) == 0))
	{
		PyUtil::LogPythonError();
		UE_LOG(LogPython, Error, TEXT("PyType_Ready(&PyCFunctionWithClosureType) failed! Python will be disabled."))
		return false;
	}

	if (!ensure(PyType_Ready(&PyMethodWithClosureDescrType) == 0))
	{
		PyUtil::LogPythonError();
		UE_LOG(LogPython, Error, TEXT("PyType_Ready(&PyMethodWithClosureDescrType) failed! Python will be disabled."))
		return false;
	}
	
	if (!ensure(PyType_Ready(&PyClassMethodWithClosureDescrType) == 0))
	{
		PyUtil::LogPythonError();
		UE_LOG(LogPython, Error, TEXT("PyType_Ready(&PyClassMethodWithClosureDescrType) failed! Python will be disabled."))
		return false;
	}

...
public:
	...
	UPythonGeneratedStruct* Finalize(FPyObjectPtr InPyPostInitFunction)
	{
		...
		NewStruct->StructRedirector = CreatePythonTypeLegacyRedirector(PyUtil::GetCleanTypename(PyType), FTopLevelAssetPath(NewStruct->GetOuter()->GetFName(), *StructName));
...
void UPythonGeneratedStruct::AddReferencedObjects(UObject* InThis, FReferenceCollector& Collector)
{
	Super::AddReferencedObjects(InThis, Collector);

	UPythonGeneratedStruct* This = CastChecked<UPythonGeneratedStruct>(InThis);
...
void UPythonGeneratedStruct::PostRename(UObject* OldOuter, const FName OldName)
{
	...
	if (PyType)
	{
		FPyWrapperTypeRegistry::Get().UnregisterWrappedStructType(FSoftObjectPath::ConstructFromAssetPath(FTopLevelAssetPath(OldOuter->GetFName(), OldName)), PyType, false);

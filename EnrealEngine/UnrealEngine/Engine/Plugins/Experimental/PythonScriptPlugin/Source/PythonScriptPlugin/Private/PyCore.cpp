...
TArray<FTSTicker::FDelegateHandle> PythonTickerCallbackHandles;

FPyTickerDelegateHandle* RegisterTickerCallback(const float InDelay, PyObject* InPyCallable)
{
	// We copy the PyCallable into the lambda to keep the Python object alive as long as the delegate is bound
	FTSTicker::FDelegateHandle TickerDelegateHandle = FTSTicker::GetCoreTicker().AddTicker(TEXT("PythonTicker"), InDelay, [PyCallable = FPyAutoGILPtr(FPyObjectPtr::NewReference(InPyCallable))](const float InDeltaTime) mutable
	{
		if (!PyUtil::CanTickPython())
		{
			return true;
		}
		...
		if (Result)
		{
			PyConversion::Nativize(Result, bContinue, PyConversion::ESetErrorState::No);
		}
		else
		{
	...
	{
		PythonTickerCallbackHandles.Add(TickerDelegateHandle);
	}

	return FPyTickerDelegateHandle::CreateInstance(TickerDelegateHandle);
}

bool UnregisterTickerCallback(PyObject* InCallbackHandle)
{
	FPyTickerDelegateHandlePtr PyTickerDelegateHandle = FPyTickerDelegateHandlePtr::StealReference(FPyTickerDelegateHandle::CastPyObject(InCallbackHandle));
	if (!PyTickerDelegateHandle)
	{
		return false;
	}

	FTSTicker::FDelegateHandle& TickerDelegateHandle = PyTickerDelegateHandle->Value;
	if (TickerDelegateHandle.IsValid())
	{
		PythonTickerCallbackHandles.Remove(TickerDelegateHandle);
...
void ClearTickerCallbacks()
{
	if (PythonTickerCallbackHandles.Num() > 0)
	{
		FTSTicker& CoreTicker = FTSTicker::GetCoreTicker();
		for (const FTSTicker::FDelegateHandle& Handle : PythonTickerCallbackHandles)
		{
			CoreTicker.RemoveTicker(Handle);
		}
		PythonTickerCallbackHandles.Empty();
	}
}
...
UObjectRedirector* CreatePythonTypeLegacyRedirector(const FString& ShortName, const FTopLevelAssetPath& GeneratedPathName)
{
	...
	UObject* ExistingObject = FindObject<UObject>(RedirectorOuter, *ShortName);
	UObjectRedirector* Redirector = Cast<UObjectRedirector>(ExistingObject);
...
FPyTickerDelegateHandle* FPyTickerDelegateHandle::CreateInstance(const FTSTicker::FDelegateHandle& InValue)
{
	FPyTickerDelegateHandlePtr NewInstance = FPyTickerDelegateHandlePtr::StealReference(FPyTickerDelegateHandle::New(&PyTickerDelegateHandleType));
	if (NewInstance)
	{
		if (FPyTickerDelegateHandle::Init(NewInstance, InValue) != 0)
		{
			PyUtil::LogPythonError();
			return nullptr;
		}
...
FPyTickerDelegateHandle* FPyTickerDelegateHandle::CastPyObject(PyObject* InPyObject)
{
	if (PyObject_IsInstance(InPyObject, (PyObject*)&PyTickerDelegateHandleType) == 1)
	{
		Py_INCREF(InPyObject);
		return (FPyTickerDelegateHandle*)InPyObject;
	}
...
PyTypeObject InitializePyScopedSlowTaskType()
{
	...
	struct FMethods
	{
		...
		static PyObject* TickProgress(FPyScopedSlowTask* InSelf)
		{
			if (!FPyScopedSlowTask::ValidateInternalState(InSelf))
			{
				return nullptr;
	...
	static PyMethodDef PyMethods[] = {
		...
		{ "should_cancel", PyCFunctionCast(&FMethods::ShouldCancel), METH_NOARGS, "should_cancel(self) -> bool -- True if the user has requested that the slow task be canceled" },
		{ "tick_progress", PyCFunctionCast(&FMethods::TickProgress), METH_NOARGS, "tick_progress(self) -> None -- Let the UI refresh but doesn't advance progress" },
...
PyTypeObject PyTickerDelegateHandleType = InitializePyWrapperBasicType<FPyTickerDelegateHandle>("_TickerDelegateHandle", "Type for all Unreal exposed FTSTicker::FDelegateHandle instances");
...
PyObject* RegisterTickerCallback(PyObject* InSelf, PyObject* InArgs, PyObject* InKwds)
{
	PyObject* PyCallableObj = nullptr;
	PyObject* PyDelayObj = nullptr;

	static const char* ArgsKwdList[] = { "callable", "delay", nullptr };
	if (!PyArg_ParseTupleAndKeywords(InArgs, InKwds, "O|O:register_ticker_callback", (char**)ArgsKwdList, &PyCallableObj, &PyDelayObj))
	{
		return nullptr;
	}
	check(PyCallableObj);
	...
	float Delay = 0.0f;
	if (PyDelayObj && !PyConversion::Nativize(PyDelayObj, Delay))
	{
		PyUtil::SetPythonError(PyExc_TypeError, TEXT("register_ticker_callback"), *FString::Printf(TEXT("Failed to convert 'delay' (%s) to 'float'"), *PyUtil::GetFriendlyTypename(PyDelayObj)));
		return nullptr;
	}

	return (PyObject*)PyCoreUtil::RegisterTickerCallback(Delay, PyCallableObj);
}

PyObject* UnregisterTickerCallback(PyObject* InSelf, PyObject* InArgs)
{
	PyObject* PyCallableObj = nullptr;
	if (!PyArg_ParseTuple(InArgs, "O:unregister_ticker_callback", &PyCallableObj))
	{
		return nullptr;
	}
	check(PyCallableObj);

	if (!PyCoreUtil::UnregisterTickerCallback(PyCallableObj))
	{
		PyUtil::SetPythonError(PyExc_TypeError, TEXT("unregister_ticker_callback"), *FString::Printf(TEXT("Failed to convert argument '%s' to '_TickerDelegateHandle'"), *PyUtil::GetFriendlyTypename(PyCallableObj)));
		return nullptr;
	}
...
PyMethodDef PyCoreMethods[] = {
	...
	{ "register_ticker_callback", PyCFunctionCast(&RegisterTickerCallback), METH_VARARGS | METH_KEYWORDS, "x.register_ticker_callback(callable: Union[Callable[[float], None]->bool, DelegateBase], delay: float=0.0) -> object -- register the given callable (taking a single float and returning a bool where true will continue ticking) as a ticker callback in FTSTicker, to run at most every delay seconds" },
	{ "unregister_ticker_callback", PyCFunctionCast(&UnregisterTickerCallback), METH_VARARGS, "x.unregister_ticker_callback(handle: object) -> None -- unregister the given handle from a previous call to register_ticker_callback" },
	{ nullptr, nullptr, 0, nullptr }
};
...
void InitializeModule()
{
	...
	PyType_Ready(&PyTickerDelegateHandleType);
...
void ShutdownModule()
{
	PyCoreUtil::ClearTickerCallbacks();
}

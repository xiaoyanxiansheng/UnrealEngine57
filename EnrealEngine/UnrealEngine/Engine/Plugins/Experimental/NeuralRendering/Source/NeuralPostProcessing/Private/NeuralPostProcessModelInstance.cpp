...
TSharedPtr<UE::NNE::IModelInstanceRDG> CreateNNEModelInstance(UNNEModelData* NNEModelData, FString RuntimeName)
{
	...
	if (RuntimeRDG->CanCreateModelRDG(NNEModelData) != INNERuntimeRDG::ECanCreateModelRDGStatus::Ok)
	{
#if WITH_EDITOR
		UE_LOG(LogNeuralPostProcessing, Error, TEXT("Cannot create model RDG for Model = %s, Runtime = %s"), *NNEModelData->GetName(), *RuntimeName);
#endif
		return nullptr;
	}

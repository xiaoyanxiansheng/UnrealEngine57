...
#include UE_INLINE_GENERATED_CPP_BY_NAME(InstanceDataObjectFixupPanel)
...
void FInstanceDataObjectFixupPanel::RedirectProperty(const FPropertyPath& From, const FPropertyPath& To)
{
	...
	for (UObject* Instance : Instances)
	{
		...
		if (SourceProperty->SameType(DestinationProperty))
		{
			...
			FOverridableManager::Get().GetOverriddenProperties(Instance)->RestoreOverriddenPropertyOperation(EOverriddenPropertyOperation::Modified, nullptr, DestinationProperty);
		}
		else
		{
			FString ValueStr;

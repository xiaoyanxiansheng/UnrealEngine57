...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TargetingSelectionTask_AOE)
...
void UTargetingSelectionTask_AOE::ExecuteImmediateTrace(const FTargetingRequestHandle& TargetingHandle) const
{
	...
	if (World && TargetingHandle.IsValid())
	{
		...
		const FQuat SourceRotation = GetSourceRotation(TargetingHandle) * GetSourceRotationOffset(TargetingHandle).Quaternion();
...
void UTargetingSelectionTask_AOE::DebugDrawBoundingVolume(const FTargetingRequestHandle& TargetingHandle, const FColor& Color, const FOverlapDatum* OverlapDatum) const
{
	...
	const FQuat SourceRotation = OverlapDatum ? OverlapDatum->Rot : GetSourceRotation(TargetingHandle) * GetSourceRotationOffset(TargetingHandle).Quaternion();
	const FCollisionShape CollisionShape = GetCollisionShape();
	...
	if (Overlaps.Num() > 0)
	{
		...
		for (const FOverlapResult& OverlapResult : Overlaps)
		{
			...
			if (bAddResult)
			{
				...
				ResultData->HitResult.Item = OverlapResult.GetItemIndex();
...
FVector UTargetingSelectionTask_AOE::GetSourceOffset_Implementation(const FTargetingRequestHandle& TargetingHandle) const
{
	...
	if(bUseRelativeOffset)
	{
		if (const FTargetingSourceContext* SourceContext = FTargetingSourceContext::Find(TargetingHandle))
		{
			if (const AActor* SourceActor = SourceContext->SourceActor)
			{
...
FRotator UTargetingSelectionTask_AOE::GetSourceRotationOffset_Implementation(const FTargetingRequestHandle& TargetingHandle) const
{
	return DefaultSourceRotationOffset;
}

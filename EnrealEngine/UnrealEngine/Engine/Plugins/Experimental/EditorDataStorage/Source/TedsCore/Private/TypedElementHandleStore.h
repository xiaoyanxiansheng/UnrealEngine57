...
namespace UE::Editor::DataStorage
{
	...
	public:
		...
		THandleStore() = default;
		~THandleStore();
		THandleStore(const THandleStore&) = delete;
		THandleStore(THandleStore&&) = delete;
		THandleStore& operator=(const THandleStore&) = delete;
		THandleStore& operator=(THandleStore&&) = delete;
	...
	THandleStore<DataType, ReservationSize>::~THandleStore()
	{
	...
	template<typename DataType, uint32 ReservationSize>
	template<typename... Args>
	auto THandleStore<DataType, ReservationSize>::Emplace(Args... Arguments) -> Handle
	{
	...
	template<typename DataType, uint32 ReservationSize>
	DataType& THandleStore<DataType, ReservationSize>::Get(Handle Entry)
	{
	...
	template<typename DataType, uint32 ReservationSize>
	DataType& THandleStore<DataType, ReservationSize>::GetMutable(Handle Entry)
	{
		checkf(IsAlive(Entry), TEXT("Attempting to retrieve a dead entry from a Typed Element Handle Store."));
		return Data[Entry.Index()];
	}
	...
	template<typename DataType, uint32 ReservationSize>
	const DataType& THandleStore<DataType, ReservationSize>::Get(Handle Entry) const
	{
		checkf(IsAlive(Entry), TEXT("Attempting to retrieve a dead entry from a Typed Element Handle Store."));
		return Data[Entry.Index()];
	}
	...
	template<typename DataType, uint32 ReservationSize>
	void THandleStore<DataType, ReservationSize>::Remove(Handle Entry)
	{
		if (IsAlive(Entry))
		{
	...
	template<typename DataType, uint32 ReservationSize>
	bool THandleStore<DataType, ReservationSize>::IsAlive(Handle Entry) const
	{
		return
	...
	template<typename DataType, uint32 ReservationSize>
	void THandleStore<DataType, ReservationSize>::ListAliveEntries(const ListAliveEntriesConstCallback& Callback) const
	{
		int32 Count = Data.Num();
		const DataType* EntryIt = Data.GetData();
		const FGeneration* GenerationIt = Generations.GetData();
	...
	template<typename DataType, uint32 ReservationSize>
	void THandleStore<DataType, ReservationSize>::ListAliveEntries(const ListAliveEntriesConstCallback& Callback)
	{
		const_cast<const THandleStore<DataType, ReservationSize>*>(this)->ListAliveEntries(Callback);
	}

	template<typename DataType, uint32 ReservationSize>
	void THandleStore<DataType, ReservationSize>::ListAliveEntries(const ListAliveEntriesCallback& Callback)
	{
		int32 Count = Data.Num();

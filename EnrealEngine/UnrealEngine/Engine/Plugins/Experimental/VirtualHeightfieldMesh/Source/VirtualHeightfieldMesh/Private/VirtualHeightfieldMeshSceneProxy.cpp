	...
	, OcclusionGridSize(0, 0)
{
	...
	MaterialRelevance = Material->GetMaterialInterface()->GetRelevance_Concurrent(GetScene().GetShaderPlatform());
...
namespace VirtualHeightfieldMesh
{
	...
	public:
		virtual void InitRHI(FRHICommandListBase& RHICmdList) override
		{
			...
			Initializer.GetTexture2DSubresource(0).WriteColor(FColor(0, 0, 255, 255));
		...
		while (WorkIndex < NumWorkItems && SceneProxies[WorkDescs[WorkIndex].ProxyIndex] == Proxy)
		{
			...
			else
			{
				if (!OcclusionResults->UploadData.IsEmpty())
				{
					...
					OcclusionResults->OcclusionTexture = GraphBuilder.RHICmdList.CreateTexture(Desc);
					...
					{
						uint32 Stride;
						uint8* Dst = (uint8*)GraphBuilder.RHICmdList.LockTexture2D(OcclusionResults->OcclusionTexture, MipIndex, RLM_WriteOnly, Stride, false);
						...
						GraphBuilder.RHICmdList.UnlockTexture2D(OcclusionResults->OcclusionTexture, MipIndex, false);

...
void UNiagaraDataInterfaceChaosDestruction::HandleBreakingEvents(const Chaos::FBreakingEventData& Event)
{
	...
	BreakingEvents.AddZeroed(BreakingDataIn.Num());
...
void UNiagaraDataInterfaceChaosDestruction::HandleTrailingEvents(const Chaos::FTrailingEventData& Event)
{
	...
	const float TrailMinSpeedToSpawnSquared = TrailMinSpeedToSpawn * TrailMinSpeedToSpawn;
	...
	auto GetGeometryCollectionComponentFromProxy = 
		[this](IPhysicsProxyBase* InProxy) -> UGeometryCollectionComponent*
		{
			for (auto& Solver : Solvers)
			{
				if (Solver.PhysScene)
				{
					if (UGeometryCollectionComponent* GeometryCollectionComponent = Solver.PhysScene->GetOwningComponent<UGeometryCollectionComponent>(InProxy))
					{
	...
	const Chaos::FTrailingDataArray& InEvents = Event.TrailingData.AllTrailingsArray;

	for (const Chaos::FTrailingData& InEvent : InEvents)
	{
		if (InEvent.Velocity.SquaredLength() >= TrailMinSpeedToSpawnSquared)
		{
			if (UGeometryCollectionComponent* GeometryCollectionComponent = GetGeometryCollectionComponentFromProxy(InEvent.Proxy))
			{
				if (GeometryCollectionComponent->GetNotifyTrailing())
				{
					if (UPhysicalMaterial* PhysicalMaterial = GeometryCollectionComponent->GetPhysicalMaterial())
					{
						if (!bApplyMaterialsFilter || IsMaterialInFilter(PhysicalMaterial->GetFName()))
						{
							Chaos::FTrailingDataExt TrailingData(InEvent);

							TrailingData.SurfaceType = PhysicalMaterial->SurfaceType;
							const FBox BoundingBox(InEvent.BoundingBox.Min(), InEvent.BoundingBox.Max());
							const FVector Extents = BoundingBox.GetSize();
							TrailingData.BoundingboxVolume = BoundingBox.GetVolume();
							TrailingData.BoundingboxExtentMin = FMath::Min3(Extents[0], Extents[1], Extents[2]);
							TrailingData.BoundingboxExtentMax = FMath::Max3(Extents[0], Extents[1], Extents[2]);

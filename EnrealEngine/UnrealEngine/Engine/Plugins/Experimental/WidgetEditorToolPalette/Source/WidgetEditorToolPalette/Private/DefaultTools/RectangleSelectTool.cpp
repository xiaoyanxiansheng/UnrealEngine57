...
void URectangleSelectTool::SelectWidgetsAffectedByMarquee()
{
	...
	if (PinnedOwningWidget && WidgetBlueprintEditor)
	{
		...
		RectDeltaX.StartPoint = FVector2f(OldEndPoint.X, StartPoint.Y);
		RectDeltaX.EndPoint = FVector2f(NewEndPoint.X, SelectionDeltaY > 0 ? NewEndPoint.Y : OldEndPoint.Y);

		FMarqueeRect RectDeltaY;
		RectDeltaY.StartPoint = FVector2f(StartPoint.X, OldEndPoint.Y);
		RectDeltaY.EndPoint = FVector2f(SelectionDeltaX > 0 ? NewEndPoint.X : OldEndPoint.X, NewEndPoint.Y);

...
bool AlwaysValidValidationFunction(
	const NNERuntimeRDGData::Internal::FAttributeMap& AttributeMap, 
...
//
//
void FAttributeValidator::AddOptional(const FString& Name, ENNERuntimeRDGDataAttributeDataType Type)
{
#if DO_CHECK
...
void FAttributeValidator::AddRequired(const FString& Name, ENNERuntimeRDGDataAttributeDataType Type)
{
#if DO_CHECK
	const bool bIsFoundInOptional = (nullptr != OptionalAttributes.FindByPredicate([Name](const FEntry& Other) { return Other.Name == Name; }));
	const bool bIsFoundInRequired = (nullptr != RequiredAttributes.FindByPredicate([Name](const FEntry& Other) { return Other.Name == Name; }));
...
bool FAttributeValidator::Validate(const NNERuntimeRDGData::Internal::FAttributeMap& AttributesToValidate)
{
	...
	{
		const FNNERuntimeRDGDataAttributeValue* FoundAttribute = AttributesToValidate.GetAttributeValue(RequiredAttributes[Idx].Name);
	...
	{
		const FNNERuntimeRDGDataAttributeValue* FoundAttribute = AttributesToValidate.GetAttributeValue(OptionalAttributes[Idx].Name);

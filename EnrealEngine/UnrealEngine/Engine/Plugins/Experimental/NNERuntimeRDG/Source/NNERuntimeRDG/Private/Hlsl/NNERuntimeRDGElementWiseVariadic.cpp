...
namespace UE::NNERuntimeRDG::Private::Hlsl
{
	...
	using EElementWiseVariadicOperatorType = UE::NNEHlslShaders::Internal::EElementWiseVariadicOperatorType;
	...
	void AddOneVariadicOpPass(FRDGBuilder& GraphBuilder, 
		...
		bool OutputAsInput,
		EElementWiseVariadicOperatorType OpType,
		float Scale)
	{
	 ...
	 */
	template<EElementWiseVariadicOperatorType OpType>
	...
	public:
		...
		virtual void Dispatch(FRDGBuilder& GraphBuilder, TConstArrayView<FTensorRDGRef> InInputTensors, TConstArrayView<FTensorRDGRef> InOutputTensors) override
		{
			...
			for (int32 InputOffset = 0; InputOffset < InInputTensors.Num(); InputOffset += FElementWiseVariadicConstants::MAX_NUM_INPUT)
			{
				...
				if (OpType == EElementWiseVariadicOperatorType::Mean && bIsLastPass)
				{
	...
	bool ValidateElementWiseVariadicOperator(const NNERuntimeRDGData::Internal::FAttributeMap& AttributeMap, TConstArrayView<ENNETensorDataType> InputTypes, TConstArrayView<NNE::FSymbolicTensorShape> InputShapes)
	{
		bool bIsValid = true;
...
#define OP(Name, Version) Registry.OpAdd({{TEXT(#Name), TEXT("Onnx")}, Version}, CreateElementWiseVariadicOperator<EElementWiseVariadicOperatorType::Name>);

...
namespace UE::NNEHlslShaders::Internal
{
	...
	const FString TElementWiseBinaryCS::GetOpFunc(EElementWiseBinaryOperatorType OpType)
	{
		FString OpTable[(int32) EElementWiseBinaryOperatorType::MAX];

		for (int32 Idx = 0; Idx < (int32) EElementWiseBinaryOperatorType::MAX; ++Idx)
		{
			OpTable[Idx] = FString("");
		}

#define OP(OpName, OpFunc) OpTable[(int32) EElementWiseBinaryOperatorType::OpName] = OpFunc
		OP(Add,              TEXT("((X)+(Y))"));
		//OP(And,            TEXT("((X)&&(Y))"));
		OP(Div,              TEXT("((X)/(Y))"));
		//OP(Equal,          TEXT("((X)==(Y))"));

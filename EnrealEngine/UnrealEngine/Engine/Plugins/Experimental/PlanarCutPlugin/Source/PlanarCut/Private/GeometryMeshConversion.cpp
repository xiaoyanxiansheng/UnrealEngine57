...
namespace AugmentedDynamicMesh
{
	...
	void InitializeOverlayToPerVertexColors(FDynamicMesh3& Mesh)
	{
		if (!ensure(Mesh.HasAttributes()))
		{
			return;
		...
		if(Mesh.Attributes())
		{
			...
			for (const int TriangleID : Mesh.TriangleIndicesItr())
			{
				...
				{
					ColorOverlay->SetElement(ColorElementTri[TriVertIndex],
						AugmentedDynamicMesh::GetVertexColor(Mesh, Tri[TriVertIndex]));
				}
			}
		}
	}

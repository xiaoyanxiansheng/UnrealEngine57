...
)
{
	...
	for (int32 Idx = 0; Idx < TransformIndices.Num(); ++Idx)
	{
		...
		if (GeomIdx > -1 && Collection.IsRigid(TransformIdx))
		{
			...
			GeomToParent.Add(GeomIdx, Collection.Parent[Idx]);
		}
	}
	...
	for (int32 SmallIdx : TooSmalls)
	{
		...
		for (int32 NbrIdx : Prox)
		{
			...
			{
				if (bOnlySameParent && GeomToParent[NbrIdx] != GeomToParent[SmallIdx])
				{
					continue;
				}
				double Score;
...
)
{
	...
	for (int32 MeshIdx = 0; MeshIdx < NumMeshes; MeshIdx++)
	{
		...
		AugmentedDynamicMesh::InitializeOverlayToPerVertexColors(Mesh);

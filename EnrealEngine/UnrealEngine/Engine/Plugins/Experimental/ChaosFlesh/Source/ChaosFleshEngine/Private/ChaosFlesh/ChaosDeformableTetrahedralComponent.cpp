...
UDeformablePhysicsComponent::FThreadingProxy* UDeformableTetrahedralComponent::NewProxy()
{
	...
	{
		if (TSharedPtr<const FFleshCollection> Rest = RestAsset->GetFleshCollection())
		{
			if (Rest->NumElements(FGeometryCollection::VerticesGroup))
			{
				...
				GetDynamicCollection()->Reset(Rest.Get());
...
UDeformablePhysicsComponent::FDataMapValue UDeformableTetrahedralComponent::NewDeformableData()
{
	...
	if (GetOwner())
	{
		...
		{
			if (TSharedPtr<const FFleshCollection> Rest = FleshAsset->GetFleshCollection())
			{
...
void UDeformableTetrahedralComponent::UpdateFromSimulation(const FDataMapValue* SimulationBuffer)
{
	...
	if (const FFleshThreadingProxy::FFleshOutputBuffer* FleshBuffer = (*SimulationBuffer)->As<FFleshThreadingProxy::FFleshOutputBuffer>())
	{
		if (GetDynamicCollection())
		{
			...
			const TManagedArray<int32>& Parent = GetRestCollection()->GetFleshCollection()->GetAttribute<int32>(FTransformCollection::ParentAttribute, FTransformCollection::TransformGroup);
			const TManagedArray<FTransform3f>& Transform = GetRestCollection()->GetFleshCollection()->GetAttribute<FTransform3f>(FTransformCollection::TransformAttribute, FTransformCollection::TransformGroup);
			const TManagedArray<int32>& BoneIndex = GetRestCollection()->GetFleshCollection()->GetAttribute<int32>("BoneMap", FGeometryCollection::VerticesGroup);
...
void UDeformableTetrahedralComponent::UpdateLocalBounds()
{
	if (bBoundsNeedsUpdate && RestCollection)
	{
		...
		BoundingBox = RestCollection->GetFleshCollection()->GetBoundingBox();
...
void UDeformableTetrahedralComponent::ResetDynamicCollection()
{
	if (const UFleshAsset* RestAsset = GetRestCollection())
	{
		...
		GetDynamicCollection()->Reset(RestAsset->GetFleshCollection().Get()); //always reset from a new copy to avoid shrinking group size
		...
		if (IsVisible())
		{
...
#endif
			{
				if (Mesh && CVarParams.bDoDrawSimulationMesh)
				{
					...
					if (TSharedPtr<const FFleshCollection> FleshCollection = FleshAsset->GetFleshCollection())
					{
...
TArray<FVector3f> UDeformableTetrahedralComponent::GetGeometryCachePositions(const UStaticMesh* StaticMesh) const
{
	...
	if (RestCollection && DynamicCollection && StaticMesh)
	{
		...
		if (FleshCollection && RestVertices && SimulatedVertices)
		{
			ChaosFlesh::BoundSurfacePositions(StaticMesh, FleshCollection.Get(), RestVertices, SimulatedVertices, Positions);
		}
	}
	return Positions;
}
...
TOptional<TArray<int32>> UDeformableTetrahedralComponent::GetMeshImportVertexMap(const USkinnedAsset& SkinnedMeshAsset) const
{
	...
	{
		ChaosFlesh::BoundSurfacePositions(SkeletalMeshAsset, RestCollection->GetFleshCollection().Get(), RestVertices, RestVertices, Positions);

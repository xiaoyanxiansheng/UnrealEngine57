...
FDIFleshDeformerProviderProxy::IsValid(FValidationData const& InValidationData) const
{
	...
	TSharedPtr<const FFleshCollection> FleshCollection = RestCollectionAsset->GetFleshCollection();
	if (!FleshCollection)
	{
		return false;
	}
...
void
FDIFleshDeformerProviderProxy::AllocateResources(FRDGBuilder& GraphBuilder, FAllocationData const& InAllocationData)
{}

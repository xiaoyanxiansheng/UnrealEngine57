...
public:
	...
	virtual void UpdateFromSimulation(const FDataMapValue* SimulationBuffer) override;
	virtual FDataMapValue WriteRestartDataToSimulation() override;
	...
	virtual TArray<FVector3f> GetGeometryCachePositions(const UStaticMesh* StaticMesh) const override;

	virtual TOptional<TArray<int32>> GetMeshImportVertexMap(const USkinnedAsset& SkinnedMeshAsset) const override;

	virtual bool IsSkeletalMeshAnimationCompatible(const USkeletalMeshComponent* SkeletalMeshComponent) const override;

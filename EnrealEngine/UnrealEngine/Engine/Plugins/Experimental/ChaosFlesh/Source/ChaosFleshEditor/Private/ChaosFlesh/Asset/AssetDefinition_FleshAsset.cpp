		...
		if (UFleshAsset* const FleshAsset = Cast<UFleshAsset>(FleshObjects[0]))
		{
			...
			if (FDataflowEditorToolkit::HasDataflowAsset(FleshAsset))
			{
				...
				if (UDataflowSimulationSettings* SimulationSettings = NewObject<UDataflowSimulationSettings>())
				{
					...
					SimulationSettings->bIsAsyncCachingSupported = true;
					SimulationSettings->bIsAsyncCachingEnabledByDefault = true;
					SimulationSettings->bIsGeometryCacheOutputSupported = true;

...
void UCubeGridTool::AcceptToolAndStartNew()
{
	...
	GetToolManager()->DisplayMessage(CreatingAssetMessage, EToolMessageLevel::UserWarning);
...
void UCubeGridTool::RegisterActions(FInteractiveToolActionSet& ActionSet)
{
	...
	ActionSet.RegisterAction(this, ActionID++,
		TEXT("AcceptAndStartNew"),
		LOCTEXT("AcceptAndStartNewAction", "Accept and Start New"),
		FText(),
		EModifierKey::None, EKeys::N,
		[this]() {RequestAction(ECubeGridToolAction::AcceptAndStartNew); });
...
bool UCubeGridTool::CanApplyPreview() const
{
	return !bPreviewMayDiffer || Preview->HaveValidResult();
}

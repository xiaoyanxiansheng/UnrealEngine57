...
void FApproximateActorsImpl::GenerateApproximationForActorSet(const FInput& Input, const FOptions& Options, FResults& ResultsOut)
{
	...
	UMaterialInterface* UseBaseMaterial = (Options.BakeMaterial != nullptr) ? 
		Options.BakeMaterial : 
		Options.bUsePackedMRS ?
			LoadObject<UMaterial>(nullptr, TEXT("/MeshModelingToolsetExp/Materials/FullMaterialBakeOutputMaterial_PackedMRS")) :
			LoadObject<UMaterial>(nullptr, TEXT("/MeshModelingToolsetExp/Materials/FullMaterialBakeOutputMaterial"));
		...
		FName SwitchName)
	{
		if (Texture)
		{
			...
			TexOptions.NewAssetPath = BaseTexturePath + TEXT("_") + MaterialParamName.ToString();
		...
		if (NewMaterial && SwitchName != NAME_None)
		{
			FGuid SwitchGuid;
			bool bUnusedValue = false;
			if (NewMaterial->GetStaticSwitchParameterValue(SwitchName, bUnusedValue, SwitchGuid))
			{
				const bool bUseSwitchValue = true;
				NewMaterial->SetStaticSwitchParameterValueEditorOnly(SwitchName, bUseSwitchValue);
			}
		}
	};
	...
	{
		WriteTextureLambda(GeneratedTextures.BaseColorMap, FTexture2DBuilder::ETextureType::Color, Options.BaseColorTexParamName, Options.BaseColorSwitchName);
	}
	if (Options.bBakeEmissive)
	{
		WriteTextureLambda(GeneratedTextures.EmissiveMap, FTexture2DBuilder::ETextureType::EmissiveHDR, Options.EmissiveTexParamName, Options.EmissiveSwitchName);
	}
	if (Options.bBakeNormalMap)
	{
		WriteTextureLambda(GeneratedTextures.NormalMap, FTexture2DBuilder::ETextureType::NormalMap, Options.NormalTexParamName, Options.NormalSwitchName);
	}

	if ((Options.bBakeRoughness || Options.bBakeMetallic || Options.bBakeSpecular) && Options.bUsePackedMRS)
	{
		WriteTextureLambda(GeneratedTextures.PackedMRSMap, FTexture2DBuilder::ETextureType::ColorLinear, Options.PackedMRSTexParamName, Options.PackedMRSSwitchName);
	}
	if (Options.bBakeRoughness)
	{
		WriteTextureLambda(GeneratedTextures.RoughnessMap, FTexture2DBuilder::ETextureType::Roughness, Options.RoughnessTexParamName, Options.RoughnessSwitchName);
	}
	if (Options.bBakeMetallic)
	{
		WriteTextureLambda(GeneratedTextures.MetallicMap, FTexture2DBuilder::ETextureType::Metallic, Options.MetallicTexParamName, Options.MetallicSwitchName);
	}
	if (Options.bBakeSpecular)
	{
		WriteTextureLambda(GeneratedTextures.SpecularMap, FTexture2DBuilder::ETextureType::Specular, Options.SpecularTexParamName, Options.SpecularSwitchName);
	}

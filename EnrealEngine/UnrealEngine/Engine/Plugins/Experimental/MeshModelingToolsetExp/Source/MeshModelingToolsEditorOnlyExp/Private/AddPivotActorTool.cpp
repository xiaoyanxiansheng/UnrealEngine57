...
void UAddPivotActorToolBuilder::InitializeNewTool(UMultiSelectionMeshEditingTool* NewTool, const FToolBuilderState& SceneState) const
{
	...
	{
		// When dealing with actor selection, only grab one target per owning actor.
		TSet<AActor*> OwningActors;
		SceneState.TargetManager->EnumerateSelectedAndTargetableComponents(SceneState, Requirements, [this, &SceneState, &Targets, &Requirements, &OwningActors](UActorComponent* Component)
		{
			if (AActor* OwnerActor = Component->GetOwner())
			{
				bool bAlreadyInSet = false;
				OwningActors.Add(OwnerActor, &bAlreadyInSet);
				if (!bAlreadyInSet)
				{

...
public:
	...
	UE_API const TObjectPtr<UTexture2D>& operator[](UE::Geometry::ERenderCaptureType CaptureType) const;
...
public:
	...
	UE_API bool operator==(const URenderCaptureProperties& Other) const;
	UE_API bool operator!=(const URenderCaptureProperties& Other) const;
};
...
protected:
	...
	UE_API void InvalidateResults(UE::Geometry::FRenderCaptureTypeFlags ToInvalidate);
	 ...
	 */
	UE_API void CreateAssets(UWorld* SourceWorld);
	...
	UE_API void InitializePreviewMaterials();
	...
	UE_API void GatherAnalytics(FBakeAnalytics::FMeshSettings& Data);
	UE_API void GatherAnalytics(const UE::Geometry::FMeshMapBaker& Result);
	UE_API void RecordAnalytics() const;
};

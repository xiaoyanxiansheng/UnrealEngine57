 ...
 */
UCLASS(MinimalAPI)
class UMeshTangentsTool : public USingleSelectionMeshEditingTool, public UE::Geometry::IGenericDataOperatorFactory<UE::Geometry::FMeshTangentsd>, public IInteractiveToolManageGeometrySelectionAPI
{
	GENERATED_BODY()
public:
	UE_API UMeshTangentsTool();
	...
	UE_API virtual TUniquePtr<UE::Geometry::TGenericDataOperator<UE::Geometry::FMeshTangentsd>> MakeNewOperator() override;
...
protected:
	...
	UE_API void OnTangentsUpdated(const TUniquePtr<UE::Geometry::FMeshTangentsd>& NewResult);
	UE_API void CopyToOverlays(const UE::Geometry::FMeshTangentsd& Tangents, FDynamicMesh3& Mesh);
	UE_API void UpdateVisualization(bool bThicknessChanged, bool bLengthChanged);
	UE_API TSet<int32> ComputeDegenerateTris() const;
	UE_API void ComputeMikkTDeviations(const TSet<int32>& DegenerateTris);

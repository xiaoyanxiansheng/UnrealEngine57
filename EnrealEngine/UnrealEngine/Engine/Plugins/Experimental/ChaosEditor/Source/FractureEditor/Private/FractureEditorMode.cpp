...
{
	// this is a suggestion has nanite based geometry collection may want to override it 
	bool bShouldDisplayBounds = false;
	if (const UFractureSettings* FractureSettings = GetDefault<UFractureSettings>())
	{
		// early exit if the None option has been selected
		if (FractureSettings->SelectionDisplayMode == EFractureSelectionDisplayMode::None)
		{
			return;
		}

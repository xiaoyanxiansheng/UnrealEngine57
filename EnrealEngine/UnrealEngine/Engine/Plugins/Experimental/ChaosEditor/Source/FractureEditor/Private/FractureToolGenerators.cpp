...
private:
	...
	{
		DataflowAsset = Cast<UDataflow>(InAssetData.GetAsset());
	}
	...
	{
		PhysicalMaterial = Cast<UPhysicalMaterial>(InAssetData.GetAsset());
	}
...
FString UFractureToolGenerateAsset::GetDefaultAssetPath(const TArray<AActor*>& Actors) const
{
	...
	if (bFailedToFindLastUsedFolder || Settings->NewAssetLocation == EFractureModeNewAssetLocation::SourceAssetFolder)
	{
		for (const AActor* Actor : Actors)
		{
			...
			{
				FoundMeshAsset = StaticMeshComponent ? StaticMeshComponent->GetStaticMesh().Get() : nullptr;
				if (FoundMeshAsset)
				{
					break;
				}

...
TArray<FFractureToolContext> UFractureToolCutterBase::GetFractureToolContexts() const
{
	...
	for (UGeometryCollectionComponent* GeometryCollectionComponent : GeomCompSelection)
	{
		...
		if (IsValid(RestCollection))
		{
			...
			int32 FullSelectionSeed = CutterSettings->RandomSeed > -1 ? CutterSettings->RandomSeed : DefaultRandomSeed;
			FullSelection.SetSeed(FullSelectionSeed);
			...
			else
			{
				...
				for (int32 Index : FullSelection.GetSelection())
				{
					...
					FractureContext.SetSeed(FullSelectionSeed + Index);

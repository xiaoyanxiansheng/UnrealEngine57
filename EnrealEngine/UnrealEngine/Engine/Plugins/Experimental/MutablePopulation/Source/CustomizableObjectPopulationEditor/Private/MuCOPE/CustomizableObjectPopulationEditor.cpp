...
void FCustomizableObjectPopulationEditor::TestPopulation()
{
	...
	if (Population && Viewport.IsValid())
	{
		...
		if (ViewportInstances.Num() > 0)
		{
			...
			{
				const UCustomizableObject* CustomizableObject = ViewportInstances[i]->GetCustomizableObject();	
				...
				if (PreviewCustomizableSkeletalComponent && PreviewSkeletalMeshComponent && ColliderComponent)
				{
					...
					// TODO: UE-274335. We want this code to be able to operate with multiple components, not just the first one
					PreviewCustomizableSkeletalComponent->SetComponentName(CustomizableObject->GetComponentName(0));

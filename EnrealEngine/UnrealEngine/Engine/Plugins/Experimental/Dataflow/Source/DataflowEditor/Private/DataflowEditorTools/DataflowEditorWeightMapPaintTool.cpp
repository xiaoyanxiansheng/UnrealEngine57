...
void UDataflowEditorWeightMapPaintTool::SetupBrushEditBehaviorSetup(ULocalTwoAxisPropertyEditInputBehavior& OutBehavior) 
{
...
#if WITH_EDITOR
			FPropertyChangedEvent PropertyChangedEvent(UBrushBaseProperties::StaticClass()->FindPropertyByName(GET_MEMBER_NAME_CHECKED(UDataflowEditorWeightMapPaintBrushFilterProperties, AttributeValue)));
...
bool UDataflowEditorWeightMapPaintTool::UpdateStampPosition(const FRay& WorldRay)
{
	...
	const bool bVolumetric = (FilterProperties->BrushAreaMode == EMeshVertexPaintBrushAreaType::Volumetric);
	if (UseBrushOp->GetAlignStampToView() || bVolumetric)
	{
		AlignBrushToView();
	}
...
int32 UDataflowEditorWeightMapPaintTool::FindHitSculptMeshTriangleConst(const FRay3d& LocalRay) const
{
	if (!IsInBrushSubMode())
	{
...
int32 UDataflowEditorWeightMapPaintTool::FindHitTargetMeshTriangleConst(const FRay3d& LocalRay) const
{
	check(false);
	return IndexConstants::InvalidID;
}

void UDataflowEditorWeightMapPaintTool::UpdateHitSculptMeshTriangle(int32 TriangleID, const FRay3d& LocalRay)
{
...
bool UDataflowEditorWeightMapPaintTool::UpdateBrushPosition(const FRay& WorldRay)
{
	...
	const bool bVolumetric = (FilterProperties->BrushAreaMode == EMeshVertexPaintBrushAreaType::Volumetric);
	if (bHit && (UseBrushOp->GetAlignStampToView() || bVolumetric))
	{
		AlignBrushToView();
	}
...
void UDataflowEditorWeightMapPaintTool::DrawHUD(FCanvas* Canvas, IToolsContextRenderAPI* RenderAPI)
{
	...
	if (Canvas && RenderAPI && BrushIndicator)
	{
		...
		const FVector Offset = (-RenderAPI->GetSceneView()->GetViewUp() * BrushRadius * 1.1f);
		const FVector BrushPos = BrushIndicator->BrushPosition + Offset;
		...
		const bool bShouldShowValueAtBrush = IsInBrushSubMode();
		if (bShouldShowValueAtBrush)
		{
			const FText BaseText = LOCTEXT("DataflowWeightMapPaintTool_ValueAtBrush_TextFormat", "Value: {0}");
			const FText ValueAtBrushText = FText::Format(BaseText, FText::AsNumber(FilterProperties->ValueAtBrush));
...
void UDataflowEditorWeightMapPaintTool::WaitForPendingUndoRedoUpdate()
{
	if (bUndoUpdatePending)
	{
		bUndoUpdatePending = false;

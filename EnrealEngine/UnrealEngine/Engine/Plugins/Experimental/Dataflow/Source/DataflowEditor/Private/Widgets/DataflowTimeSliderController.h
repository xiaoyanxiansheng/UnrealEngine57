...
class FDataflowTimeSliderController : public ITimeSliderController, public TSharedFromThis<FDataflowTimeSliderController>
{
	struct FDrawTickArgs;
	struct FScrubRangeToScreen;

public:
	FDataflowTimeSliderController(const FTimeSliderArgs& InArgs, TWeakPtr<SDataflowSimulationTimeline> DataflowSimulationTimeline);
	~FDataflowTimeSliderController();
	...
	virtual FReply OnMouseButtonDoubleClick(SWidget& OwnerWidget, const FGeometry& MyGeometry, const FPointerEvent& MouseEvent) override;
...
private:
	...
	/** Set the playback range start */
	void SetPlaybackRangeStart(FFrameNumber NewStart);

	/** Set the playback range end */
...
private:
	...
	TWeakPtr<SDataflowSimulationTimeline> SimulationTimeline;

	...
	if (Facade.NumTriangles() > 0)
	{
		...
		for (int i = 0; i < NumGeom; i++)
		{
			if (Facade.GetIndicesCount()[i] != 0)
			{
				...
				MeshBatchData.MeshTransform = Facade.GetGeometryTransform()[i];
			}
		}
...
void FDataflowEngineSceneProxy::GetMeshDynamicMeshElements(int32 ViewIndex, FMeshElementCollector& Collector) const
{
	...
	{
		const FMatrix BatchTransform = GetLocalToWorld() * MeshBatchData.MeshTransform.ToMatrixWithScale();

		FDynamicPrimitiveUniformBuffer& DynamicPrimitiveUniformBuffer = Collector.AllocateOneFrameResource<FDynamicPrimitiveUniformBuffer>();
		DynamicPrimitiveUniformBuffer.Set(Collector.GetRHICommandList(), BatchTransform, BatchTransform, GetBounds(), GetLocalBounds(), true, false, AlwaysHasVelocity());

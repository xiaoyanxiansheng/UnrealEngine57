...
{
public:
	UE_API FMotoSynthEngine();
	UE_API ~FMotoSynthEngine();

	// Queries if the engine is enabled at all. Checks a cvar.
	static UE_API bool IsMotoSynthEngineEnabled();

	//~ Begin IMotoSynthEngine
	UE_API void SetSettings(const FMotoSynthRuntimeSettings& InSettings) final;
	...
	UE_API void Init(int32 InSampleRate);
	UE_API void Reset();

	// Sets all the source data for the moto synth
	UE_API void SetSourceData(uint32 AccelerationSourceID, uint32 DecelerationSourceID);

	// Returns the min and max RPM range, taking into account the acceleration and deceleration data.
	UE_API void GetRPMRange(FVector2D& OutRPMRange);

	// Sets the RPM directly. Used if the engine is in ManualRPM mode. Will be ignored if we're in simulation mode.
	UE_API void SetRPM(float InRPM, float InTimeSec = 0.0f);

	// Sets a pitch scale on the moto synth to scale up or down the pitch of the output
	UE_API void SetPitchScale(float InPitchScale);

private:
	UE_API void GenerateGranularEngine(float* OutAudio, int32 NumSamples);

	UE_API bool NeedsSpawnGrain();
	UE_API void SpawnGrain(int32& StartingIndex, const MotoSynthDataPtr& SynthData);

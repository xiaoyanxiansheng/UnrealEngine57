...
namespace FollowSplineMode::Utils::Private
{
	static float ComputeRangeInputValue(const USplineComponent* ControlSpline, FSplineOffsetRangeInput RangeOffsetInput)
	{
		...
		case ESplineOffsetUnit::DistanceAbsolute:
		{
			...
			return FMath::Clamp(SplineTime, 0.0f, ControlSpline->Duration);
		}
		default:
		{
...
void UFollowSplineMode::SimulationTick_Implementation(const FSimulationTickParams& Params, FMoverTickEndData& OutputState)
{
	...
	FRotator StartingOrientation = StartingMoveState->GetOrientation_WorldSpace();
	...
	if (CanMove(MappedSplineTime))
	{
		...
		const FVector Velocity = DeltaSeconds > UE_SMALL_NUMBER ? MoveDelta / DeltaSeconds : FVector::ZeroVector;
		...
		FRotator NewRotation = MovingComps.UpdatedComponent->GetComponentRotation();
		FVector AngularVelocityDegrees = UMovementUtils::ComputeAngularVelocityDegrees(StartingOrientation, NewRotation, DeltaSeconds);

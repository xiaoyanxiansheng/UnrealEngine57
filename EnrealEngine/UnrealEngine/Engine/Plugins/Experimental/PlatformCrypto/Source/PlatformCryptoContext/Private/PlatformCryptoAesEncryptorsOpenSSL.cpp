...
{
	// GCM always encrypts to the same amount of ciphertext bytes as input plaintext
	return Plaintext.Num();
}

int32 FPlatformCryptoEncryptor_AES_256_GCM_OpenSSL::GetFinalizeBufferSizeBytes() const
{
	// GCM finalization will always output 0 bytes, it only finalizes authentication part
	return 0;
}

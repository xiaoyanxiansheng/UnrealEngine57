...
struct FPoleVectorConstraint 
{
	...
	UE_API bool ApplyConstraint(TArray<FFBIKLinkData>& InOutLinkData, TArray<FTransform>& InOutLocalTransforms);
	UE_API FVector CalculateCurrentPoleVectorDir(const FTransform& ParentTransform, const FTransform& BoneTransform, const FTransform& ChildTransform, const FQuat& LocalFrame) const;
};

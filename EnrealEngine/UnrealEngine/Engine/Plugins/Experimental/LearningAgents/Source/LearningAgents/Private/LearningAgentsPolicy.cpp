...
#include UE_INLINE_GENERATED_CPP_BY_NAME(LearningAgentsPolicy)
...
void ULearningAgentsPolicy::SetupPolicy(
	...
	const bool bReinitializePolicyNetwork,
	const bool bReinitializeDecoderNetwork,
	const FLearningAgentsPolicySettings& InPolicySettings,
	const int32 Seed)
{
	if (IsSetup())
	{
	...
	const int32 MemoryStateSize = PolicySettings.MemoryCell == ELearningAgentsMemoryCell::NoMemoryCell ? 0: PolicySettings.MemoryStateSize;
	...
	if (!PolicyNetwork->NeuralNetworkData || bReinitializePolicyNetwork)
	{
		...
		TArray<uint8> FileData;
		uint32 PolicyInputSize = 0, PolicyOutputSize = 0;

		switch (PolicySettings.MemoryCell)
		{
		case ELearningAgentsMemoryCell::NoMemoryCell:
			Builder.WriteFileDataAndReset(FileData, PolicyInputSize, PolicyOutputSize,
				Builder.MakeSequence({
					Builder.MakeMLP(
						...
						PolicyHiddenLayerNum + 2, // Add 2 to account for input and output layers
			...
			break;
		case ELearningAgentsMemoryCell::LearningAgentsGRU:
			Builder.WriteFileDataAndReset(FileData, PolicyInputSize, PolicyOutputSize,
			...
			break;
		case ELearningAgentsMemoryCell::StandardGRU:
			Builder.WriteFileDataAndReset(FileData, PolicyInputSize, PolicyOutputSize,
				Builder.MakeMemoryBackbone(
					MemoryStateSize,
					Builder.MakeMLP(
...
TSharedRef<FJsonObject> ULearningAgentsPolicy::AsJsonConfig() const
{
	TSharedRef<FJsonObject> ConfigObject = MakeShared<FJsonObject>();

	ConfigObject->SetNumberField(TEXT("HiddenLayerNum"), PolicySettings.HiddenLayerNum);
	ConfigObject->SetNumberField(TEXT("HiddenLayerSize"), PolicySettings.HiddenLayerSize);
	ConfigObject->SetStringField(TEXT("MemoryCell"), StaticEnum<ELearningAgentsMemoryCell>()->GetNameStringByValue(static_cast<int64>(PolicySettings.MemoryCell)));
	ConfigObject->SetNumberField(TEXT("MemoryStateSize"), GetMemoryStateSize());
	ConfigObject->SetNumberField(TEXT("InitialEncodedActionScale"), PolicySettings.InitialEncodedActionScale);
	ConfigObject->SetStringField(TEXT("ActivationFunction"), StaticEnum<ELearningAgentsActivationFunction>()->GetNameStringByValue(static_cast<int64>(PolicySettings.ActivationFunction)));
	return ConfigObject;
}

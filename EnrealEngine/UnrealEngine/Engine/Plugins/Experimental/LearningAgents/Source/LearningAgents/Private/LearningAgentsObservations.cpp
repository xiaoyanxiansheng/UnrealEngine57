...
#include UE_INLINE_GENERATED_CPP_BY_NAME(LearningAgentsObservations)
...
namespace UE::Learning::Agents::Observation::Private
{
		...
		const Learning::Observation::FObjectElement ObjectElement)
	{
		...
		case Learning::Observation::EType::Conv1d:
		{
			const Learning::Observation::FSchemaConv1dParameters SchemaParameters = Schema.GetConv1d(SchemaElement);
			const Learning::Observation::FObjectConv1dParameters ObjectParameters = Object.GetConv1d(ObjectElement);
		...
		case Learning::Observation::EType::Conv2d:
		{
			const Learning::Observation::FSchemaConv2dParameters SchemaParameters = Schema.GetConv2d(SchemaElement);
			const Learning::Observation::FObjectConv2dParameters ObjectParameters = Object.GetConv2d(ObjectElement);
	...
	static inline Learning::Observation::EPaddingMode GetPaddingMode(const ELearningAgentsPaddingMode PaddingMode)
	{
		switch (PaddingMode)
		{
		case ELearningAgentsPaddingMode::Zeros: return Learning::Observation::EPaddingMode::Zeros;
		case ELearningAgentsPaddingMode::Circular: return Learning::Observation::EPaddingMode::Circular;
		default: checkNoEntry(); return Learning::Observation::EPaddingMode::Zeros;
		}
	}
...
FLearningAgentsObservationSchemaElement ULearningAgentsObservations::SpecifyConv1dObservation(ULearningAgentsObservationSchema* Schema, const FLearningAgentsObservationSchemaElement Element, const FConv1dObservationParams& Conv1dParams, const FName Tag)
{
	if (!Schema)
	{
		UE_LOG(LogLearning, Error, TEXT("SpecifyConv1dObservation: Schema is nullptr."));
		return FLearningAgentsObservationSchemaElement();
	}
	if (!Schema->ObservationSchema.IsValid(Element.SchemaElement))
	{
		UE_LOG(LogLearning, Error, TEXT("SpecifyConv1dObservation: Invalid Observation Object."));
		return FLearningAgentsObservationSchemaElement();
	}
	if (Conv1dParams.InputLength < 1)
	{
		UE_LOG(LogLearning, Error, TEXT("SpecifyConv1dObservation: Invalid Observation InputLength '%i' - must be greater than zero."), Conv1dParams.InputLength);
		return FLearningAgentsObservationSchemaElement();
	}
	if (Conv1dParams.InChannels < 1)
	{
		UE_LOG(LogLearning, Error, TEXT("SpecifyConv1dObservation: Invalid Observation InChannels '%i' - must be greater than zero."), Conv1dParams.InChannels);
		return FLearningAgentsObservationSchemaElement();
	}
	if (Conv1dParams.OutChannels < 1)
	{
		UE_LOG(LogLearning, Error, TEXT("SpecifyConv1dObservation: Invalid Observation OutChannels '%i' - must be greater than zero."), Conv1dParams.OutChannels);
		return FLearningAgentsObservationSchemaElement();
	}
	if (Conv1dParams.KernelSize < 1)
	{
		UE_LOG(LogLearning, Error, TEXT("SpecifyConv1dObservation: Invalid Observation KernelSize '%i' - must be greater than zero."), Conv1dParams.KernelSize);
		return FLearningAgentsObservationSchemaElement();
	}
	if (Conv1dParams.Padding < 1)
	{
		UE_LOG(LogLearning, Error, TEXT("SpecifyConv1dObservation: Invalid Observation Padding '%i' - must be greater than zero."), Conv1dParams.Padding);
		return FLearningAgentsObservationSchemaElement();
	}
	if (Conv1dParams.KernelSize > Conv1dParams.InputLength + 2 * Conv1dParams.Padding)
	{
		UE_LOG(LogLearning, Error, TEXT("SpecifyConv1dObservation: Invalid KernelSize '%i' - Kernel size is too large."), Conv1dParams.KernelSize);
		return FLearningAgentsObservationSchemaElement();
	}

	return { Schema->ObservationSchema.CreateConv1d(
	{
		Element.SchemaElement,
		Conv1dParams.InputLength,
		Conv1dParams.InChannels,
		Conv1dParams.OutChannels,
		Conv1dParams.KernelSize,
		Conv1dParams.Padding,
		UE::Learning::Agents::Observation::Private::GetPaddingMode(Conv1dParams.PaddingMode),
		UE::Learning::Agents::Observation::Private::GetEncodingActivationFunction(Conv1dParams.ActivationFunction)
	},
	Tag) };
}

FLearningAgentsObservationSchemaElement ULearningAgentsObservations::SpecifyConv2dObservation(ULearningAgentsObservationSchema* Schema, const FLearningAgentsObservationSchemaElement Element, const FConv2dObservationParams& Conv2dParams, const FName Tag)
{
	if (!Schema)
	{
		UE_LOG(LogLearning, Error, TEXT("SpecifyConv2dObservation: Schema is nullptr."));
		return FLearningAgentsObservationSchemaElement();
	}
	if (!Schema->ObservationSchema.IsValid(Element.SchemaElement))
	{
		UE_LOG(LogLearning, Error, TEXT("SpecifyConv2dObservation: Invalid Observation Object."));
		return FLearningAgentsObservationSchemaElement();
	}
	if (Conv2dParams.InputHeight < 1)
	{
		UE_LOG(LogLearning, Error, TEXT("SpecifyConv2dObservation: Invalid Observation InputHeight '%i' - must be greater than zero."), Conv2dParams.InputHeight);
		return FLearningAgentsObservationSchemaElement();
	}
	if (Conv2dParams.InputWidth < 1)
	{
		UE_LOG(LogLearning, Error, TEXT("SpecifyConv2dObservation: Invalid Observation InputWidth '%i' - must be greater than zero."), Conv2dParams.InputWidth);
		return FLearningAgentsObservationSchemaElement();
	}
	if (Conv2dParams.InChannels < 1)
	{
		UE_LOG(LogLearning, Error, TEXT("SpecifyConv2dObservation: Invalid Observation InChannels '%i' - must be greater than zero."), Conv2dParams.InChannels);
		return FLearningAgentsObservationSchemaElement();
	}
	if (Conv2dParams.OutChannels < 1)
	{
		UE_LOG(LogLearning, Error, TEXT("SpecifyConv2dObservation: Invalid Observation OutChannels '%i' - must be greater than zero."), Conv2dParams.OutChannels);
		return FLearningAgentsObservationSchemaElement();
	}
	if (Conv2dParams.KernelSize < 1)
	{
		UE_LOG(LogLearning, Error, TEXT("SpecifyConv2dObservation: Invalid Observation KernelSize '%i' - must be greater than zero."), Conv2dParams.KernelSize);
		return FLearningAgentsObservationSchemaElement();
	}
	if (Conv2dParams.Stride < 1)
	{
		UE_LOG(LogLearning, Error, TEXT("SpecifyConv2dObservation: Invalid Observation Stride '%i' - must be greater than zero."), Conv2dParams.Stride);
		return FLearningAgentsObservationSchemaElement();
	}
	if (Conv2dParams.Padding < 0)
	{
		UE_LOG(LogLearning, Error, TEXT("SpecifyConv2dObservation: Invalid Observation Padding '%i' - must be greater than zero."), Conv2dParams.Padding);
		return FLearningAgentsObservationSchemaElement();
	}

	return { Schema->ObservationSchema.CreateConv2d(
	{
		Element.SchemaElement,
		Conv2dParams.InputHeight,
		Conv2dParams.InputWidth,
		Conv2dParams.InChannels,
		Conv2dParams.OutChannels,
		Conv2dParams.KernelSize,
		Conv2dParams.Stride,
		Conv2dParams.Padding,
		UE::Learning::Agents::Observation::Private::GetPaddingMode(ELearningAgentsPaddingMode::Zeros),
		UE::Learning::Agents::Observation::Private::GetEncodingActivationFunction(Conv2dParams.ActivationFunction)
	},
	Tag) };
}
...
FLearningAgentsObservationSchemaElement ULearningAgentsObservations::SpecifyLidarObservation(ULearningAgentsObservationSchema* Schema, const FName Tag)
{
	return SpecifyFloatObservation(Schema, 1.0f, Tag);
}
...
FLearningAgentsObservationObjectElement ULearningAgentsObservations::MakeConv1dObservation(ULearningAgentsObservationObject* Object, const FLearningAgentsObservationObjectElement Element, const FName Tag)
{
	if (!Object)
	{
		UE_LOG(LogLearning, Error, TEXT("MakeConv1dObservation: Object is nullptr."));
		return FLearningAgentsObservationObjectElement();
	}

	if (!Object->ObservationObject.IsValid(Element.ObjectElement))
	{
		UE_LOG(LogLearning, Error, TEXT("MakeConv1dObservation: Invalid Observation Object."));
		return FLearningAgentsObservationObjectElement();
	}

	return { Object->ObservationObject.CreateConv1d({ Element.ObjectElement }, Tag) };
}

FLearningAgentsObservationObjectElement ULearningAgentsObservations::MakeConv2dObservation(ULearningAgentsObservationObject* Object, const FLearningAgentsObservationObjectElement Element, const FName Tag)
{
	if (!Object)
	{
		UE_LOG(LogLearning, Error, TEXT("MakeConv2dObservation: Object is nullptr."));
		return FLearningAgentsObservationObjectElement();
	}

	if (!Object->ObservationObject.IsValid(Element.ObjectElement))
	{
		UE_LOG(LogLearning, Error, TEXT("MakeConv2dObservation: Invalid Observation Object."));
		return FLearningAgentsObservationObjectElement();
	}

	return { Object->ObservationObject.CreateConv2d({ Element.ObjectElement }, Tag) };
}
...
FLearningAgentsObservationObjectElement ULearningAgentsObservations::MakeLidarObservation(
	ULearningAgentsObservationObject* Object, 
	const FVector RayStart,
	const FVector RayEnd,
	const FTransform RayTransform, 
	...
	const FLinearColor VisualLoggerColor)
{
	const FVector RayStartWorld = RayTransform.TransformPosition(RayStart);
	const FVector RayEndWorld = RayTransform.TransformPosition(RayEnd);
	const float RayRange = FVector::Dist(RayEndWorld, RayStartWorld);

	if (RayRange <= 0)
	{
		UE_LOG(LogLearning, Error, TEXT("MakeLiDARObservation: Ray range cannot be 0."));
		return FLearningAgentsObservationObjectElement();
	}
	...
	FHitResult TraceHit;
	const bool bHit = Object->GetWorld()->LineTraceSingleByObjectType(TraceHit, RayStartWorld, RayEndWorld, ObjectQueryParams, TraceParams);

	const float Encoded = bHit ? FMath::Clamp(1.0f - TraceHit.Distance / RayRange, 0.0f, 1.0f) : 0.0f;
...
bool ULearningAgentsObservations::GetConv1dObservation(FLearningAgentsObservationObjectElement& OutElement, const ULearningAgentsObservationObject* Object, const FLearningAgentsObservationObjectElement Element, const FName Tag)
{
	if (!Object)
	{
		UE_LOG(LogLearning, Error, TEXT("GetConv1dObservation: Object is nullptr."));
		OutElement = FLearningAgentsObservationObjectElement();
		return false;
	}
	...
	if (!Object->ObservationObject.IsValid(Element.ObjectElement))
	{
		UE_LOG(LogLearning, Error, TEXT("GetConv1dObservation: Invalid Observation Object."));
		OutElement = FLearningAgentsObservationObjectElement();
		return false;
	}
	...
	if (Object->ObservationObject.GetTag(Element.ObjectElement) != Tag)
	{
		UE_LOG(LogLearning, Warning, TEXT("GetConv1dObservation: Observation tag does not match. Observation is '%s' but asked for '%s'."), *Object->ObservationObject.GetTag(Element.ObjectElement).ToString(), *Tag.ToString());
	}

	if (Object->ObservationObject.GetType(Element.ObjectElement) != UE::Learning::Observation::EType::Conv1d)
	{
		UE_LOG(LogLearning, Error, TEXT("GetConv1dObservation: Observation '%s' type does not match. Observation is '%s' but asked for '%s'."),
			*Object->ObservationObject.GetTag(Element.ObjectElement).ToString(),
			UE::Learning::Agents::Observation::Private::GetObservationTypeString(Object->ObservationObject.GetType(Element.ObjectElement)),
			UE::Learning::Agents::Observation::Private::GetObservationTypeString(UE::Learning::Observation::EType::Conv1d));
		OutElement = FLearningAgentsObservationObjectElement();
		return false;
	}

	OutElement = { Object->ObservationObject.GetConv1d(Element.ObjectElement).Element };

	return true;
}

bool ULearningAgentsObservations::GetConv2dObservation(FLearningAgentsObservationObjectElement& OutElement, const ULearningAgentsObservationObject* Object, const FLearningAgentsObservationObjectElement Element, const FName Tag)
{
	if (!Object)
	{
		UE_LOG(LogLearning, Error, TEXT("GetConv2dObservation: Object is nullptr."));
		OutElement = FLearningAgentsObservationObjectElement();
		return false;
	}
	...
	if (!Object->ObservationObject.IsValid(Element.ObjectElement))
	{
		UE_LOG(LogLearning, Error, TEXT("GetConv2dObservation: Invalid Observation Object."));
		OutElement = FLearningAgentsObservationObjectElement();
		return false;
	}
	...
	if (Object->ObservationObject.GetTag(Element.ObjectElement) != Tag)
	{
		UE_LOG(LogLearning, Warning, TEXT("GetConv2dObservation: Observation tag does not match. Observation is '%s' but asked for '%s'."), *Object->ObservationObject.GetTag(Element.ObjectElement).ToString(), *Tag.ToString());
	}

	if (Object->ObservationObject.GetType(Element.ObjectElement) != UE::Learning::Observation::EType::Conv2d)
	{
		UE_LOG(LogLearning, Error, TEXT("GetConv2dObservation: Observation '%s' type does not match. Observation is '%s' but asked for '%s'."),
			*Object->ObservationObject.GetTag(Element.ObjectElement).ToString(),
			UE::Learning::Agents::Observation::Private::GetObservationTypeString(Object->ObservationObject.GetType(Element.ObjectElement)),
			UE::Learning::Agents::Observation::Private::GetObservationTypeString(UE::Learning::Observation::EType::Conv2d));
		OutElement = FLearningAgentsObservationObjectElement();
		return false;
	}

	OutElement = { Object->ObservationObject.GetConv2d(Element.ObjectElement).Element };

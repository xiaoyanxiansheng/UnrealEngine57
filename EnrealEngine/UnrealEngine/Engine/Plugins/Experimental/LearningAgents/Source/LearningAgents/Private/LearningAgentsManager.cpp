...
#include UE_INLINE_GENERATED_CPP_BY_NAME(LearningAgentsManager)
...
void ULearningAgentsManager::RequestAddAgent(UObject* Agent)
{
	if (!IsInGameThread())
	{
		AsyncTask(ENamedThreads::GameThread, [this, Agent]()
			{
				AddAgent(Agent);
			});
		return;
...
void ULearningAgentsManager::RequestRemoveAgent(const int32 AgentId)
{
	if (!IsInGameThread())
	{

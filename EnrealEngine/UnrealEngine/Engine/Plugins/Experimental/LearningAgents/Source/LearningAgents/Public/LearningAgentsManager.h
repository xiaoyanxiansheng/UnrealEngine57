...
public:
	...
	UE_API ULearningAgentsManager();
	UE_API ULearningAgentsManager(FVTableHelper& Helper);
	UE_API virtual ~ULearningAgentsManager();
...
public:
	...
	UE_API int32 GetMaxAgentNum() const;
	...
	UE_API void SetMaxAgentNum(const int32 InMaxAgentNum);
...
public:
	...
	UE_API int32 AddAgent(UObject* Agent);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "LearningAgents")
	UE_API void AddAgents(TArray<int32>& OutAgentIds, const TArray<UObject*>& InAgents);

	/**
	 * Schedules a task to add the given object as an agent to the manager, preventing synchronous mutations of agent arrays on the game thread.
	 * @param Agent The object to be added.
	 * @return The agent's newly assigned id.
	 */
	UFUNCTION(BlueprintCallable, Category = "LearningAgents")
	UE_API void RequestAddAgent(UObject* Agent);
...
public:
	...
	UE_API void RemoveAgent(const int32 AgentId);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "LearningAgents")
	UE_API void RemoveAgents(const TArray<int32>& AgentIds);

	/** Removes all agents from the manager. */
	UFUNCTION(BlueprintCallable, Category = "LearningAgents")
	UE_API void RemoveAllAgents();
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "LearningAgents", meta = (AgentId = "-1"))
	UE_API void RequestRemoveAgent(const int32 AgentId);
...
public:
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "LearningAgents", meta = (AgentId = "-1"))
	UE_API void ResetAgent(const int32 AgentId);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "LearningAgents")
	UE_API void ResetAgents(const TArray<int32>& AgentIds);

	/** Resets all the agents on the manager. */
	UFUNCTION(BlueprintCallable, Category = "LearningAgents")
	UE_API void ResetAllAgents();
...
public:
	...
	UE_API void GetAgents(TArray<UObject*>& OutAgents, const TArray<int32>& AgentIds, const TSubclassOf<UObject> AgentClass) const;
	 ...
	 */
	UFUNCTION(BlueprintPure, Category = "LearningAgents")
	UE_API int32 GetAgentId(UObject* Agent) const;
	...
	UE_API void GetAgentIds(TArray<int32>& OutAgentIds, const TArray<UObject*>& InAgents) const;
...
public:
	...
	UFUNCTION(BlueprintCallable, Category = "LearningAgents")
	UE_API void AddListener(ULearningAgentsManagerListener* Listener);

	/** Removes a listener from being tracked by this manager. */
	UFUNCTION(BlueprintCallable, Category = "LearningAgents")
	UE_API void RemoveListener(ULearningAgentsManagerListener* Listener);
...
public:
	...
	UE_API const TArray<int32>& GetAllAgentIds() const;

	/** Gets the set of agent ids currently added as an FIndexSet */
	UE_API UE::Learning::FIndexSet GetAllAgentSet() const;

	/** Get a const array view of this manager's agent objects. */
	UE_API TConstArrayView<TObjectPtr<UObject>> GetAgents() const;

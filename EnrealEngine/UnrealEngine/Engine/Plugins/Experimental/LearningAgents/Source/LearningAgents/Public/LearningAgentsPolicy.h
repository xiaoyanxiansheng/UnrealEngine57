...
{
	NoMemoryCell UMETA(DisplayName = "No Memory Cell"),
	LearningAgentsGRU UMETA(DisplayName = "Learning Agents GRU"),
	StandardGRU UMETA(DisplayName = "Standard GRU")
};
...
public:
	...
	/** The type of memory cell to use. Setting this to NoMemoryCell is equivalent to setting MemoryStateSize to 0. */
...
public:
	...
	UE_API ULearningAgentsPolicy();
	UE_API ULearningAgentsPolicy(FVTableHelper& Helper);
	UE_API virtual ~ULearningAgentsPolicy();
	...
	UE_API void SetupPolicy(
		...
		const bool bReinitializePolicyNetwork = true,
		const bool bReinitializeDecoderNetwork = true,
		const FLearningAgentsPolicySettings& InPolicySettings = FLearningAgentsPolicySettings(),
		const int32 Seed = 1234);
...
public:
	...
	UE_API void EncodeObservations();
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "LearningAgents")
	UE_API void DecodeAndSampleActions(const float ActionNoiseScale = 1.0f);
	...
	UE_API void GetMemoryState(TArray<float>& OutMemoryState, const int32 AgentId) const;
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "LearningAgents", Meta = (AgentId = -1))
	UE_API void SetMemoryState(const int32 AgentId, const TArray<float>& InMemoryState);

	/** Gets the size of the memory state */
	UFUNCTION(BlueprintPure, Category = "LearningAgents")
	UE_API int32 GetMemoryStateSize() const;
	...
	UFUNCTION(BlueprintPure, Category = "LearningAgents")
	UE_API ULearningAgentsNeuralNetwork* GetPolicyNetworkAsset();
...
public:
	...
	UE_API UE::Learning::FNeuralNetworkPolicy& GetPolicyObject();
	...
	UE_API TLearningArrayView<2, const float> GetPreEvaluationMemoryState() const;

	/** Get a const view to this policy's memory state. */
	UE_API TLearningArrayView<2, const float> GetMemoryState() const;

	/** Get a const view to this policy's encoded observation vectors. */
	UE_API TLearningArrayView<2, const float> GetObservationVectorsEncoded() const;

	/** Returns true if this policy has encoded observations for the given agent id. Otherwise, false. */
	UE_API bool HasEncodedObservationsForAgent(const int32 AgentId) const;

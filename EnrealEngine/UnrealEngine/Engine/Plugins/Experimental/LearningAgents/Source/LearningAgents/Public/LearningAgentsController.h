...
public:
	...
	UE_API ULearningAgentsController();
	UE_API ULearningAgentsController(FVTableHelper& Helper);
	UE_API virtual ~ULearningAgentsController();
	...
	static UE_API ULearningAgentsController* MakeController(
	...
	UE_API void SetupController(UPARAM(ref) ULearningAgentsManager*& InManager, UPARAM(ref) ULearningAgentsInteractor*& InInteractor);
	...
	UE_API void EvaluateAgentController(
	 ...
	 */
	UFUNCTION(BlueprintNativeEvent, Category = "LearningAgents", Meta = (ForceAsFunction))
	UE_API void EvaluateAgentControllers(
...
protected:
	...
	UE_API ULearningAgentsInteractor* GetInteractor(const TSubclassOf<ULearningAgentsInteractor> InteractorClass) const;

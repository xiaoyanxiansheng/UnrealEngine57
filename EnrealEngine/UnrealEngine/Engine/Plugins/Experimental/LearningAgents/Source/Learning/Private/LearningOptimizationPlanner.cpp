...
namespace UE::Learning
{
	...
	namespace OptimizationPlanner
	{
		...
		{
			TRACE_CPUPROFILER_EVENT_SCOPE(Learning::OptimizationPlanner::RunPlan);
		...
		{
			TRACE_CPUPROFILER_EVENT_SCOPE(Learning::OptimizationPlanner::ComputePlanLosses);
		...
		void Plan(
			...
			TAtomic<bool>* bActionVectorsUpdatedFlag)
		{
			TRACE_CPUPROFILER_EVENT_SCOPE(Learning::OptimizationPlanner::Plan);

			checkf(OptimizationPlannerBuffer.Samples.Num<0>() == SampleInstances.Num(),
			...
			checkf(OptimizationPlannerBuffer.Samples.Num<1>() == InOutActionVectors.Num<0>(),
		...
		void PlanWindowed(
			...
			TAtomic<bool>* bActionVectorsUpdatedFlag)
		{
			TRACE_CPUPROFILER_EVENT_SCOPE(Learning::OptimizationPlanner::PlanWindowed);

			checkf(WindowStepNum <= InOutActionVectors.Num<0>(),
			...
			checkf(OptimizationPlannerBuffer.Losses.Num() == SampleInstances.Num(),
				TEXT("Number of samples in OptimizationPlannerBuffer (%i) must match the number of SampleInstances (%i)."),
				OptimizationPlannerBuffer.Losses.Num(), SampleInstances.Num());

			checkf(OptimizationPlannerBuffer.Samples.Num<1>() == WindowStepNum,
			...
			check(!SampleInstances.Contains(PlanInstance));

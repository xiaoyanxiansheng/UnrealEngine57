...
namespace UE::Learning
{
	...
	FPCAResult FPCAEncoder::Fit(
		...
		FProgress* Progress)
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(Learning::FPCAEncoder::Fit);
		...
		if (ensure(Info == Eigen::ComputationInfo::Success))
		{
			...
			if (LogSettings != ELogSetting::Silent)
			{
				UE_LOG(LogLearning, Display, TEXT("--- PCA Cumulative Variance ---"));
			}

			for (int32 ValueIndex = 0; ValueIndex < FMath::Min((int32)VarianceRatio.size(), Settings.MaximumDimensions); ValueIndex++)
			{
				ComponentsToKeepNum = ValueIndex + 1;
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(Learning::FPCAEncoder::Transform);
		...
		{
			TRACE_CPUPROFILER_EVENT_SCOPE(Learning::PCA::Transform::Batch);
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(Learning::FPCAEncoder::Transform);
		check(Data.Num() == DimensionNum());
		check(OutData.Num() == FeatureNum());
	...
	void FPCAEncoder::InverseTransform(
		...
		const TLearningArrayView<1, const float> Data) const
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(Learning::FPCAEncoder::InverseTransform);
		check(Data.Num() == FeatureNum());
		check(OutData.Num() == DimensionNum());

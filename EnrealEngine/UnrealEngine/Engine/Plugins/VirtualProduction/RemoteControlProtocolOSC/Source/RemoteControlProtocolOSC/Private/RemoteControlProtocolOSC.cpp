...
DEFINE_LOG_CATEGORY_STATIC(LogRemoteControlOSC, Log, All);
...
void FRemoteControlProtocolOSC::Bind(FRemoteControlProtocolEntityPtr InRemoteControlProtocolEntityPtr)
{
	...
	if (FRemoteControlOSCProtocolEntity* OSCProtocolEntity = InRemoteControlProtocolEntityPtr->Cast<FRemoteControlOSCProtocolEntity>())
	{
	...
	{
		UE_LOG(LogRemoteControlOSC, Warning, TEXT("Invalid Remote Control Protocol Entity."))
	}
}
		...
		for (const TSharedPtr<TStructOnScope<FRemoteControlProtocolEntity>>& ProtocolEntityPtr : RCProtocolBindingList->GetAwaitingProtocolEntities())
		{
			if (ProtocolEntityPtr.IsValid())
			{
				if ((*ProtocolEntityPtr)->GetBindingStatus() == ERCBindingStatus::Awaiting)
				{
					...
					if (FRemoteControlOSCProtocolEntity* OSCProtocolEntity = ProtocolEntityPtr->Cast<FRemoteControlOSCProtocolEntity>())
					{

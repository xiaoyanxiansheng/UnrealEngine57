...
public:
	 ...
	 *
	 * @param InCameraComponent - The camera to add overscan to
	 ...
	 *	@param bScaleResolution - Indicates if the render resolution should be scaled up so that the original frustum of the camera remains the same resolution
	 */
	UFUNCTION(BlueprintCallable, Category = "VirtualCamera")
	static void SetCameraOverscan(UCineCameraComponent* InCameraComponent, const FVector4f& InAsymmetricOverscan, bool bScaleResolution = false);

	/** Clears any overscan from the camera */
	UFUNCTION(BlueprintCallable, Category = "VirtualCamera")
	static void ClearCameraOverscan(UCineCameraComponent* InCameraComponent);

...
bool FConcertTakeRecorderManager::ShouldIconBeVisible() const
{
	...
	{
		bool bShouldSupressBecauseOfRemoteOpen = false;
		if (TSharedPtr<IConcertSyncClient> ConcertSyncClient = IConcertSyncClientModule::Get().GetClient(TEXT("MultiUser")))
		{
			const SIZE_T Recorders = CanRecord() + RemoteRecorders();
			bShouldSupressBecauseOfRemoteOpen = Recorders > 1 && ConcertSyncClient->GetSequencerManager()->IsSequencerRemoteOpenEnabled();
		}
		...
		if (LevelSequence)
		{
			...
			return !bShouldSupressBecauseOfRemoteOpen && Sources && Sources->GetSources().Num() > 0;
		}
	}
	...
	if (IsTakeSyncEnabled() && !bIsRecording && TakeRecorderState.LastStartedTake != TakeRecorder->GetName())
	{
		if (TSharedPtr<IConcertClientSession> Session = WeakSession.Pin())
		{
			...
			if (TakeMetaData && TakePreset)
			{
				...
				TakeInitializedEvent.Project = GetDefault<UTakeRecorderProjectSettings>()->Settings;
...
void FConcertTakeRecorderManager::OnTakeInitializedEvent(const FConcertSessionContext&, const FConcertTakeInitializedEvent& InEvent)
{
	...
	if (IsTakeSyncEnabled() && CanRecord())
	{
		...
		if (TakePreset && TakePreset->GetLevelSequence())
		{
			...
			DefaultParams.Project = InEvent.Project;
...
{
	const SIZE_T Recorders = CanRecord() + RemoteRecorders();
	if(WeakSession.IsValid() && IsTakeSyncEnabled() && Recorders > 1)
	{
		if (TSharedPtr<IConcertSyncClient> ConcertSyncClient = IConcertSyncClientModule::Get().GetClient(TEXT("MultiUser")))
		{
			const bool bIsRemoteOpenEnabled = ConcertSyncClient->GetSequencerManager()->IsSequencerRemoteOpenEnabled();
			if (bIsRemoteOpenEnabled)
			{
				OutputError = LOCTEXT("ErrorWidget_RemoteOpenEnabled", "Sequencer remote open is enabled. It is recommended you disable this when recording on multiple clients.");
			}
		}
	}
}
...
void FConcertTakeRecorderManager::RecordSettingChange(const FConcertClientRecordSetting& RecordSetting)
{
	...
	if (Session.IsValid())
	{
		...
		if (!bHaveWarned && Recorders > 1)
		{
			...
			// Add a notification to disable remote open feature.
			if (TSharedPtr<IConcertSyncClient> ConcertSyncClient = IConcertSyncClientModule::Get().GetClient(TEXT("MultiUser")))
			{
				const bool bRemoteOpen = ConcertSyncClient->GetSequencerManager() && ConcertSyncClient->GetSequencerManager()->IsSequencerRemoteOpenEnabled();
				if (bRemoteOpen)
				{
					FText ShouldDisableErrorText = LOCTEXT("ShouldDisableSequencerRemoteOpen",
											  "When multiple machines are recording, it is recommended that you disable sequencer remote open.");
					FNotificationInfo SequencerOpenInfo(ShouldDisableErrorText);
					SequencerOpenInfo.ExpireDuration = 10.0f;
					SequencerOpenInfo.HyperlinkText = LOCTEXT("DisableRemoteOpen", "Disable sequencer remote open.");
					SequencerOpenInfo.Hyperlink = FSimpleDelegate::CreateLambda([this, ConcertSyncClient]() {
						if (ConcertSyncClient && ConcertSyncClient->GetSequencerManager())
						{
							ConcertSyncClient->GetSequencerManager()->SetSequencerRemoteOpen(false);
						}
					});

					FSlateNotificationManager::Get().AddNotification(SequencerOpenInfo);
				} 
			}

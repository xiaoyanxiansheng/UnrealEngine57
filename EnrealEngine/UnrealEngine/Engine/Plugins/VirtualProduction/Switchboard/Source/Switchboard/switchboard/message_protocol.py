...
def create_start_process_message(
    ...
    *,
    ...
    combined_stdout_stderr: bool = True,
    ...
    start_cmd = {
        ...
        'bCombinedStdoutStderr': combined_stdout_stderr,
...
def create_kill_process_message(program_id, soft_kill_timeout=None):
    cmd_id = uuid.uuid4()
    kill_cmd = {'command': 'kill', 'id': str(cmd_id), 'uuid': str(program_id)}
    if soft_kill_timeout is not None:
        kill_cmd['soft_kill_timeout'] = soft_kill_timeout

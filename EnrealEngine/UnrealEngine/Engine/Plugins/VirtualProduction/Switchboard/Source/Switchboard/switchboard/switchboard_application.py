...
from . import switchboard_ctypes as sb_ctypes
...
'''
    ...
    def __init__(self):
        ...
        self._cleanup_helper = None
        try:
            self._cleanup_helper = sb_ctypes.CloseSubprocessOnExitHelper()
        except Exception as exc:
            LOGGER.warning('RsyncServer: Error creating cleanup helper')
    ...
    def launch(
        ...
        try:
            if self._cleanup_helper:
                self._cleanup_helper.add_process(self.process.pid)
        except Exception as exc:
            LOGGER.warning('RsyncServer.launch: Error registering process for cleanup', exc_info=exc)

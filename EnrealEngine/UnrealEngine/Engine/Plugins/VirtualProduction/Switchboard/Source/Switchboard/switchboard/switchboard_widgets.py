...
class SearchableComboBox(QtWidgets.QComboBox):
    ...
    def __init__(self, parent):
        ...
        # Maps item text to its index for fast lookup during search filtering
        self._item_index_map: Optional[Dict[str, int]] = None
    ...
    def showPopup(self):
        ...
        # Reset index mapping when popup shows in case items changed
        self._item_index_map = None
    ...
    def text_changed(self, text: str):
        ...
        # Show all and early exit if text is empty
        if not text:
            for i in range(len(self.CB_item_text)):
                self.view().setRowHidden(i, False)
            return
        
        text_lower = text.lower()
        
        # Build index mapping if was reset
        if self._item_index_map is None:
            self._item_index_map = {item: i for i, item in enumerate(self.CB_item_text)}
        
        # Track visibility changes to minimize UI updates
        visibility_changes = []
        
        for item in self.CB_item_text:
            index = self._item_index_map.get(item)

            # This shouldn't happen but let's handle it just in case.
            if index is None:
                LOGGER.warning(f"SearchableComboBox: Item '{item}' unexpectedly not found in index map, rebuilding map")
                self._item_index_map = {item: i for i, item in enumerate(self.CB_item_text)}
                index = self._item_index_map[item]
            
            should_show = text_lower in item.lower()
            current_hidden = self.view().isRowHidden(index)
            
            if should_show and current_hidden:
                visibility_changes.append((index, False))
            elif not should_show and not current_hidden:
                visibility_changes.append((index, True))
        
        # Apply visibility changes in batch
        for index, hidden in visibility_changes:

...
class SwitchboardDialog(QtCore.QObject):
    ...
    def __init__(self, script_manager):
        ...
        # Accept copy-paste, intended for uassets or umaps from Unreal Engine's Content Browser
        QtGui.QShortcut(QtGui.QKeySequence.Paste, self.window).activated.connect(self.on_paste)

        # Avoid other widgets getting focus automaticaly so we can catch the paste shortcut
        self.window.centralwidget.setFocusPolicy(QtCore.Qt.FocusPolicy.ClickFocus)

        # Connect to copy-paste events from handling devices
        DeviceUnreal.static_signals.clipboard_level_added_signal.connect(self.on_clipboard_level_added)
        ...
        self.browse_to_level_button = sb_widgets.ControlQPushButton()
        self.browse_to_level_button.setMaximumSize(18, 22)
        self.browse_to_level_button.setText('...')
        font = self.browse_to_level_button.font()
        font.setBold(True)
        self.browse_to_level_button.setFont(font)
        self.browse_to_level_button.setToolTip("Browse to level")

        self.window.horizontalLayout_7.addWidget(self.level_combo_box)
        self.window.horizontalLayout_7.addWidget(self.browse_to_level_button)
        ...
        self.window.multiuser_session_lineEdit.editingFinished.connect(self.on_multiuser_session_lineEdit_editingFinished)
        self.window.slate_line_edit.editingFinished.connect(
            lambda: self._set_slate(self.window.slate_line_edit.text()))
        # Connect to our custom take widget signal
        self.window.take_spin_box.valueChanged.connect(self._on_take_value_changed)
        ...
        self.browse_to_level_button.clicked.connect(self.browse_to_level_and_add_it)
        ...
        self.set_multiuser_session_name(f'{CONFIG.MUSERVER_SESSION_NAME.get_value()}')
    ...
    def on_paste(self):
        ...
        if mime.hasText():
            DeviceUnreal.handle_clipboard_mime_text(mime.text())

    @QtCore.Slot(object)
    def on_clipboard_level_added(self, gamepath):
        ''' Called when a new level gets added. This will typically happen via a copy-paste operation from
        UE's content browser. When this happens, this dialog needs to refresh the dropdown, and we are also
        automatically selecting it in the dropdown, as that is usually what we expect the user to do when they are
        manually adding a new level.
        '''
        levels = CONFIG.get_levels()
        self.refresh_levels(levels)
        self.level = gamepath
    ...
    def set_config_hooks(self):
        ...
        CONFIG.MUSERVER_SESSION_NAME.signal_setting_changed.connect(lambda: self.refresh_muserver_session_name())
    ...
    def refresh_muserver_session_name(self):
        self.set_multiuser_session_name(CONFIG.MUSERVER_SESSION_NAME.get_value())
    ...
    def set_current_config(self, config_path):
        ...
        self.update_current_config_text()
        self.refresh_muserver_autojoin()
        self.refresh_muserver_session_name()
        self.refresh_trace_settings()
        self.refresh_window_title()
    ...
    def launch_all_button_clicked(self, button_state):
        ...
        if button_state:
            ...
            # Closing devices - group by type and use device specific close_all_devices()
            devices_by_type = {}
            ...
            # Call close_all_devices() for each device type
            for device_class, device_list in devices_by_type.items():
                device_class.close_all_devices(device_list)
    ...
    def on_all_plugin_devices_open_toggled(self, plugin_name, button_state):
        ...
        # Use device specific close_all_devices() method when Stopping.
        if not button_state and devices:
            device_class = type(devices[0])
            device_class.close_all_devices(devices)
        else:
            self.set_device_launch_state(devices, button_state)
    ...
    def on_multiuser_session_lineEdit_editingFinished(self):
        text = self.window.multiuser_session_lineEdit.text()
        self.set_multiuser_session_name(text)

    def _on_take_value_changed(self, value):
        ''' This is called when value actually changes (typed input or button clicks)
        '''
        if value != self._take:
    ...
    def on_multiuser_session_inc(self):
        '''
        Increment the session name by 1, or generate default if empty.
        '''
        current_name = self.multiuser_session_name()
        
        # If the session name is empty or just whitespace, generate a default one
        if not current_name or current_name.strip() == '':
            new_session_name = CONFIG.default_mu_session_name()
            self.set_multiuser_session_name(new_session_name)
            return
        
        # Otherwise, use the existing increment logic
    ...
    def set_multiuser_session_name(self, value):
        ...
        if value != CONFIG.MUSERVER_SESSION_NAME.get_value():
            CONFIG.MUSERVER_SESSION_NAME.update_value(value)
    ...
    def device_status_changed(self, device, previous_status):
        ...
        device.widget.update_status(device.status, previous_status, device)
    ...
    class ChangelistRunnable(QtCore.QRunnable):
        ''' Retrieves a list of recent changelists for the given path(s) on the thread pool. '''

        class Signals(QtCore.QObject):
            result = QtCore.Signal(list) # list[tuple[int, str]]

        def __init__(self, paths: list[str], client: str | None = None):
            super().__init__()
            self.paths = paths
            self.client = client
        ...
        def run(self):
            cl_desc_pairs = p4_utils.p4_latest_changelists(self.paths, client=self.client)
            self.signals.result.emit(cl_desc_pairs)

    _engine_cl_runnable: ChangelistRunnable | None = None
    _project_cl_runnable: ChangelistRunnable | None = None
    ...
    def p4_refresh_project_cl(self):
        ...
        if use_ugs:
            ...
            if changelists := ugs_utils.latest_chagelists(Path(CONFIG.UPROJECT_PATH.get_value()), client=client):
                self._on_project_changelists_refreshed(list([(cl, "") for cl in changelists]))
                return
            else:
                LOGGER.error("UnrealGameSync failed to get the project's latest changelists. "
                             "Falling back to using p4 commands directly.")
        ...
        # Indeterminate state, resolved on runnable completion
        self.window.project_cl_combo_box.clear()
        self._set_project_changelist('...')
        self.window.refresh_project_cl_button.setEnabled(False)
        self.window.sync_all_button.setEnabled(False)
        self.window.sync_and_build_all_button.setEnabled(False)

        self._project_cl_runnable = SwitchboardDialog.ChangelistRunnable(paths, client=client)
        self._project_cl_runnable.signals.result.connect(self._on_project_changelists_refreshed)
        QtCore.QThreadPool.globalInstance().start(self._project_cl_runnable)

    @QtCore.Slot()
    def _on_project_changelists_refreshed(self, cl_desc_pairs: list[tuple[int, str]]):
        if self._project_cl_runnable:
            self._project_cl_runnable.signals.result.disconnect(self._on_project_changelists_refreshed)
            self._project_cl_runnable = None

        self.window.project_cl_combo_box.clear()
        self.window.refresh_project_cl_button.setEnabled(True)
        no_pending_refresh = (self._project_cl_runnable is None) and (self._engine_cl_runnable is None)
        self.window.sync_all_button.setEnabled(no_pending_refresh)
        self.window.sync_and_build_all_button.setEnabled(no_pending_refresh)

        for (idx, (cl, desc)) in zip(count(), cl_desc_pairs):
            self.window.project_cl_combo_box.addItem(str(cl))
            self.window.project_cl_combo_box.setItemData(idx, desc, QtCore.Qt.ItemDataRole.ToolTipRole)
    ...
    def p4_refresh_engine_cl(self):
        ...
        if CONFIG.ENGINE_SYNC_METHOD.get_value() == EngineSyncMethod.Build_Engine.value:
            ...
            if engine_p4_path:
                # Indeterminate state, resolved on runnable completion
                self.window.engine_cl_combo_box.clear()
                self._set_engine_changelist('...')
                self.window.refresh_engine_cl_button.setEnabled(False)
                self.window.sync_all_button.setEnabled(False)
                self.window.sync_and_build_all_button.setEnabled(False)

                client = CONFIG.SOURCE_CONTROL_WORKSPACE.get_value()
                self._engine_cl_runnable = SwitchboardDialog.ChangelistRunnable([engine_p4_path+'/...'], client=client)
                self._engine_cl_runnable.signals.result.connect(self._on_engine_changelists_refreshed)
                QtCore.QThreadPool.globalInstance().start(self._engine_cl_runnable)
            else:
    ...
    @QtCore.Slot()
    def _on_engine_changelists_refreshed(self, cl_desc_pairs: list[tuple[int, str]]):
        if self._engine_cl_runnable:
            self._engine_cl_runnable.signals.result.disconnect(self._on_engine_changelists_refreshed)
            self._engine_cl_runnable = None
        ...
        for (idx, (cl, desc)) in zip(count(), cl_desc_pairs):
            self.window.engine_cl_combo_box.addItem(str(cl))
            self.window.engine_cl_combo_box.setItemData(idx, desc, QtCore.Qt.ItemDataRole.ToolTipRole)
    ...
    def browse_to_level_and_add_it(self) -> Optional[Path]:
        """
        Show a file‐open dialog for .umap files, then add & select the chosen level.

        Returns:
            The gamepath to the chosen .umap if one was picked, otherwise None.
        """

        maps_path = os.path.normpath(
            os.path.join(
                CONFIG.get_project_content_dir(),
                CONFIG.MAPS_PATH.get_value())
        )
        
        file_path, _ = QtWidgets.QFileDialog.getOpenFileName(
            parent=None,
            caption="Browse to Level",
            dir=maps_path,
            filter="Unreal Level Files (*.umap)",
            selectedFilter="",
        ...
        umap_filepath = Path(file_path)
        
        # Check extension
        if umap_filepath.suffix.lower() != ".umap":
            LOGGER.warning(f"Selected file isn’t a .umap: {umap_filepath}")
            return None

        umap_gamepath = CONFIG.deduce_umap_gamepath_from_filepath(umap_filepath)

        if not umap_gamepath:
            return None

        levels = CONFIG.add_single_level(umap_gamepath)

        # Update the UI

        self.refresh_levels(levels)
        self.level = umap_gamepath  # saves the CONFIG if the level changed.

...
class Device(QtCore.QObject):
    ...
    def __init__(self, name: str, address: str, **kwargs):
        ...
        # Runtime tracking
        self._runtime_start_time: Optional[float] = None
    ...
    def status(self, value: DeviceStatus):
        ...
        # Track runtime when device starts/stops running
        if value == DeviceStatus.OPEN and previous_status != DeviceStatus.OPEN:
            self._runtime_start_time = time.time()
        elif previous_status == DeviceStatus.OPEN and value != DeviceStatus.OPEN:
    ...
    def get_runtime_string(self) -> Optional[str]:
        ''' Returns a string representing how long the device has been running.
        If the device is not actually running, returns None.

        Shows hours when >= 1 hour, and days when >= 24 hours.

        e.g.
                  "01:02" (1 min, 2 seconds)
               "01:02:03" (1 hour, 2 minutes, 3 seconds)
            "1d 02:03:04" (1 day, 2 hours, 3 minutes, 4 seconds)
        '''

        if self.status != DeviceStatus.OPEN or self._runtime_start_time is None:
            return None
            
        runtime_seconds = int(time.time() - self._runtime_start_time)
    ...
    def project_changelist(self, value: Optional[int | str]):
    ...
    def engine_changelist(self, value: Optional[int | str]):
    ...
    @classmethod
    def close_all_devices(cls, devices):
        '''
        Handle closing all devices of this type when "Stop all" button is pressed.
        Override this method to customize behavior for specific device types.
        Default implementation closes each device individually.
        '''
        for device in devices:
            try:
                open_button = device.widget.open_button
                if open_button.isEnabled() and open_button.isChecked():

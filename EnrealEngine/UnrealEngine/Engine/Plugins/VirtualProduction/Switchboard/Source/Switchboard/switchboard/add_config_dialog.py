...
class AddConfigDialog(QtWidgets.QDialog):
    ...
    def __init__(self, uproject_search_path, previous_engine_dir, parent):
        ...
        if os.path.exists(previous_engine_dir): # re-use previous engine dir
            normalized_previous_engine_dir = os.path.normpath(str(Path(previous_engine_dir)))
            self.engine_dir_line_edit.setText(normalized_previous_engine_dir)
            self.engine_dir = normalized_previous_engine_dir
    ...
    def populate_best_project_guess(self, overrideIfFound=False):
        ...
        if bUpdateEngine and editorfolder:
            self.engine_dir_line_edit.setText(str(Path(editorfolder)))

        if bUpdateProject and projectpath:
            self.uproject_line_edit.setText(str(Path(projectpath)))
    ...
    def detect_running_projects(self):
        ...
        if sys.platform.startswith('win'):
            for UEname in UEnames:
                ...
                cmd = ['powershell', '-Command', f'Get-WmiObject Win32_Process -Filter "Name=\'{UEname}\'" | Select-Object -ExpandProperty CommandLine']
                ...
                for line in commandlines:
                    ...
                    editorpath = str(Path(argv[0]))
                    projectpath = str(Path(argv[1]))
                    ...
                    if projectpath.lower().endswith('.uproject') and os.path.exists(projectpath):
                        ...
                        # Try to find project by name using *.uprojectdirs files
                        found_project = self._find_project_by_name(editorpath, projectpath)
                        projects.append(found_project if found_project else '')

        return editors,projects

    def _find_project_by_name(self, editorpath, project_name):
        ''' Finds the .uproject path by name using *.uprojectdirs files in the given editor's root folder.
        '''
        try:
            # Look for .uprojectdirs files in tne Engine/'s parent folder.
            current_path = editorpath
            while current_path and current_path != os.path.dirname(current_path):
                current_path = os.path.dirname(current_path)
                if os.path.basename(current_path).lower() == 'engine':
                    engine_parent = os.path.dirname(current_path)
                    break
            else:
                return None
            
            import glob
            uprojectdirs_files = glob.glob(os.path.join(engine_parent, '*.uprojectdirs'))
            
            # Read all search paths from all .uprojectdirs files

            search_paths = []
            for uprojectdirs_file in uprojectdirs_files:
                try:
                    with open(uprojectdirs_file, 'r') as f:
                        for line in f:
                            line = line.strip()
            ...
            # Search for the project in each path

            for search_path in search_paths:
                full_search_path = os.path.join(engine_parent, search_path)
                if os.path.isdir(full_search_path):

                    project_file = os.path.join(full_search_path, f"{project_name}.uproject")
                    if os.path.exists(project_file):
                        return os.path.normpath(project_file)
                    
                    # Also search in subdirectories named after the project
                    subdir_project = os.path.join(full_search_path, project_name, f"{project_name}.uproject")
                    if os.path.exists(subdir_project):
                        return os.path.normpath(subdir_project)
    ...
    def on_uproject_changed(self, text):
        ...
        if not self.engine_dir_line_edit.text():
            try:
                ...
                self.engine_dir_line_edit.setText(str(Path(self.find_upstream_path_with_name(self.uproject, 'Engine', includeSiblings=True))))
            except FileNotFoundError:
                pass
    ...
    def on_browse_engine_dir(self):
        ...
        self.engine_dir = os.path.normpath(str(Path(new_dir)))

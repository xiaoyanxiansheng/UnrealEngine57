...
class ListenerClient(QtCore.QObject):
    ...
    def __init__(self, address, port, buffer_size=1024, parent=None):
        super().__init__(parent)
    ...
    async def _async_main(self, timeout: Optional[float]):
        ...
        async with connect(
            ...
            self.reader._limit = 10 * (2 ** 20)  # 10 MiB max message (default 64 KiB)

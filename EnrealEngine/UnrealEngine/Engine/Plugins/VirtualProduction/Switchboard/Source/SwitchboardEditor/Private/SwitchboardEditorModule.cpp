...
bool FSwitchboardEditorModule::CreateNewConfig(const FSwitchboardNewConfigUserOptions& NewConfigUserOptions)
{
	...
	UE_LOG(LogSwitchboardPlugin, Verbose, TEXT("Will launch Switchboard using script arguments: '%s' which were saved in: '%s'"),
		*ScriptArgsString, *ScriptArgsFilepath
	);
...
bool FSwitchboardEditorModule::RunProcess(const FString& InExe, const FString& InArgs)
{
	...
	UE_LOG(LogSwitchboardPlugin, Display, TEXT("Launching: %s %s"), *InExe, *InArgs);

...
void UActorModifierCoreEditorSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	// Register modifier categories
	{
		const FActorModifierCoreEditorCategoryMetadata DefaultCategory
		{
			.Name = TEXT("Default"),
			.DisplayName = LOCTEXT("DefaultCategoryDisplayName", "Default"),
		};

		RegisterModifierCategory(DefaultCategory);

		const FActorModifierCoreEditorCategoryMetadata LayoutCategory
		{
			.Name = TEXT("Layout"),
			.DisplayName = LOCTEXT("LayoutCategoryDisplayName", "Layout"),
		};

		RegisterModifierCategory(LayoutCategory);

		const FActorModifierCoreEditorCategoryMetadata CameraCategory
		{
			.Name = TEXT("Camera"),
			.DisplayName = LOCTEXT("CameraCategoryDisplayName", "Camera"),
		};

		RegisterModifierCategory(CameraCategory);

		const FActorModifierCoreEditorCategoryMetadata RenderingCategory
		{
			.Name = TEXT("Rendering"),
			.DisplayName = LOCTEXT("RenderingCategoryDisplayName", "Rendering"),
		};

		RegisterModifierCategory(RenderingCategory);

		const FActorModifierCoreEditorCategoryMetadata GeometryCategory
		{
			.Name = TEXT("Geometry"),
			.DisplayName = LOCTEXT("GeometryCategoryDisplayName", "Geometry"),
		};

		RegisterModifierCategory(GeometryCategory);

		const FActorModifierCoreEditorCategoryMetadata ConversionCategory
		{
			.Name = TEXT("Conversion"),
			.DisplayName = LOCTEXT("ConversionCategoryDisplayName", "Conversion"),
		};

		RegisterModifierCategory(ConversionCategory);
	}
}
...
bool UActorModifierCoreEditorSubsystem::RegisterModifierCategory(const FActorModifierCoreEditorCategoryMetadata& InCategoryMetadata)
{
	...
	if (ModifierCategories.Contains(InCategoryMetadata.Name))
	{
		return false;
	}

	ModifierCategories.Emplace(InCategoryMetadata.Name, MakeShared<const FActorModifierCoreEditorCategoryMetadata>(InCategoryMetadata));

	return true;
}

TSharedPtr<const FActorModifierCoreEditorCategoryMetadata> UActorModifierCoreEditorSubsystem::FindModifierCategory(FName InCategoryIdentifier) const
{
	if (const TSharedRef<const FActorModifierCoreEditorCategoryMetadata>* Category = ModifierCategories.Find(InCategoryIdentifier))
	{
		return *Category;
	}

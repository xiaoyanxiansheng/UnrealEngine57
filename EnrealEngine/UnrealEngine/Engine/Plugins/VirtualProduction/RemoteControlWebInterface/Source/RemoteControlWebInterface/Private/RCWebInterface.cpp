...
void FRemoteControlWebInterfaceModule::StartupModule()
{
	...
	FCoreDelegates::OnFEngineLoopInitComplete.AddLambda([WebAppWeak = WebApp.ToWeakPtr()]()
		{
			const TSharedPtr<FRemoteControlWebInterfaceProcess> WebApp = WebAppWeak.Pin();
			if (WebApp.IsValid() && GetDefault<URemoteControlSettings>()->bAutoStartWebSocketServer)
			{
				WebApp->Start();
			}
		});
...
void FRemoteControlWebInterfaceModule::ShutdownModule()
{
	...
	if (IWebRemoteControlModule* WebRemoteControlModule = FModuleManager::GetModulePtr<IWebRemoteControlModule>("WebRemoteControl"))
	{
		...
		WebSocketServerStartedDelegate.Reset();
	}
}

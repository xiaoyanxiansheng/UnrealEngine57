...
public:
	 ...
	 * @param InExtrude - Orthogonal (to front cap) offset value.
	 * @param InBevel - Bevel value (bevel happens before extrude).
	 * @param InBevelType - Defines shape of beveled part.
	 * @param InBevelSegments - Segments count.
	 * @param bInOutline - Front face has outline and is not filled.
	 * @param InOutlineExpand - Offsets the outline by the specified amount.
	 * @param InOutlineType - Defines the outline behavior.
	 */
	void CreateMeshes(const TText3DGlyphContourNodeShared& Root, const float InExtrude, const float InBevel, const EText3DBevelType InBevelType, const int32 InBevelSegments, const bool bInOutline, const float InOutlineExpand, const EText3DOutlineType InOutlineType);
	...
	void BuildMesh(FText3DCachedMesh& InMesh, UMaterialInterface* InDefaultMaterial);
...
private:
	 ...
	 * @param InOutlineType - Defines the outline behavior.
	 */
	void CreateFrontMesh(const TText3DGlyphContourNodeShared& Root, const bool bOutline, const float OutlineExpand, const EText3DOutlineType InOutlineType);
	...
	/**
	 * Create 'Outline' part of glyph. Not compatible with Bevel
	 * @param InOutlineExpand - outline expansion value.
	 */
	void CreateOutlineMesh(float InOutlineExpand);

	/**
	 * Copies and reflect source group into a destination group based on extrusion value
	 */
	 ...
	 * @param bOutline - Front face has outline and is not filled.
	 */
	void TriangulateAndConvert(const TText3DGlyphContourNodeShared& Node, int32& OutVertexIndex, const bool bOutline, const EText3DOutlineType InOutlineType);

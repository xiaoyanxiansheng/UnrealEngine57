...
bool UE::LevelSnapshots::Private::FTakeWorldObjectSnapshotArchive::ShouldSkipProperty(const FProperty* InProperty) const
{
	...
	{
		// The GetSerializedObject() may have overriden UObject::Serialize and may be serializing some custom, non-reflected data.
		// For example, the object may have a non-reflected FMyStruct but manually call SerializeTaggedProperties on the FMyStruct instance.
		// In that case, we cannot call Identical_InContainer with GetSerializedObject() as owning object is not necessarily the container.
		// Solution: Just allow snapshotting all custom properties. Saving the additional property data is the safest thing to do.
		if (!bIsSerializingScriptProperties)
		{
			return false;
		}

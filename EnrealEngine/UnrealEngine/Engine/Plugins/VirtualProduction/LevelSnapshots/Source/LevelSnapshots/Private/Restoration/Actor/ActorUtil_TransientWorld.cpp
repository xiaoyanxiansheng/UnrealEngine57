...
TOptional<TNonNullPtr<UClass>> UE::LevelSnapshots::Private::GetActorClass(const FSoftObjectPath& OriginalActorPath, FWorldSnapshotData& WorldData, bool bWarnIfNotFound)
{
	const FActorSnapshotData* ActorData = WorldData.ActorData.Find(OriginalActorPath);

	if (!ActorData)
	{
		UE_CLOG(bWarnIfNotFound, LogLevelSnapshots, Warning, TEXT("Failed to get the UClass for actor %s due to missing data"), *OriginalActorPath.ToString());
		return {};
	}
	...
	if (!ActorClass)
	{
		UE_LOG(LogLevelSnapshots, Warning, TEXT("Failed to load the UClass for actor %s."), *OriginalActorPath.ToString());
		return {};
	}

	return TOptional<TNonNullPtr<UClass>>(ActorClass);
}

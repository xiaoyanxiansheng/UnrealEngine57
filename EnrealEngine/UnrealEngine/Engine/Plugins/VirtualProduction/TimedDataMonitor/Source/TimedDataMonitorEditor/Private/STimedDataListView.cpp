...
struct FTimedDataInputTableRowData : TSharedFromThis<FTimedDataInputTableRowData>
{
	...
	void UpdateCachedValue()
	{
		...
		if (CachedEnabled == ECheckBoxState::Checked)
		{
			...
			{
				const FTimecode Timecode = NewestDataTime.Timecode.ToTimecode();
				...
				{
					CachedDescription = FText::Format(
						INVTEXT("{0} @ {1}"), 
						FText::FromString(Timecode.ToString()),
						NewestDataTime.Timecode.Rate.ToPrettyText()
					);
				}
			}
			break;
	...
	if (TimedDataListView::HeaderIdName_Icon == ColumnName)
	{
		if (Item->bIsInput)
		{
			return SNew(SHorizontalBox)
				...
				.Padding(6, 0, 0, 0)
				[
					SNew(SExpanderArrow, SharedThis<STimedDataInputTableRow>(this))

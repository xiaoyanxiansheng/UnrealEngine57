...
void UCameraNodalOffsetAlgoOpticalAxis::Initialize(UNodalOffsetTool* InNodalOffsetTool)
{
	...
	FIntPoint OverlaySize = StepsController->GetMediaOverlayResolution();
	if (OverlaySize.SizeSquared() == 0)
	{
		// If the media overlay does not have a valid resolution yet, use a default size
		OverlaySize = FIntPoint(1920, 1080);
	}

	...
	[		
		...
		[ 
			...
			[
				...
				.ViewActor_Lambda([WeakStepsController = CalibrationStepsController]()->ACameraActor*
				{
					if (TSharedPtr<FCameraCalibrationStepsController> PinnedStepsController = WeakStepsController.Pin())
					{
						return PinnedStepsController->GetCamera();
					}

					return nullptr;
				})
				.FilmbackAspectRatio_Lambda([WeakStepsController = CalibrationStepsController]
				{
					if (TSharedPtr<FCameraCalibrationStepsController> PinnedStepsController = WeakStepsController.Pin())
					{
						return PinnedStepsController->GetCameraFeedAspectRatio();
					}

					return 1.0f;
				})
				.MediaOverlayTexture_Lambda([WeakStepsController = CalibrationStepsController]()->UTexture*
				{
					if (TSharedPtr<FCameraCalibrationStepsController> PinnedStepsController = WeakStepsController.Pin())
					{
						return PinnedStepsController->GetMediaOverlayTexture();
					}

					return nullptr;
				})
				.MediaOverlayOpacity_Lambda([WeakStepsController = CalibrationStepsController]
				{
					if (TSharedPtr<FCameraCalibrationStepsController> PinnedStepsController = WeakStepsController.Pin())
					{
						return 1.0f - PinnedStepsController->GetWiperWeight();
					}

					return 0.0f;
				})
				.ToolOverlayMaterial_Lambda([WeakStepsController = CalibrationStepsController]()->UMaterialInterface*
				{
					if (TSharedPtr<FCameraCalibrationStepsController> PinnedStepsController = WeakStepsController.Pin())
					{
						if (PinnedStepsController->IsOverlayEnabled(EOverlayPassType::ToolOverlay))
						{
							return PinnedStepsController->GetOverlayMaterial(EOverlayPassType::ToolOverlay);
						}
					}

					return nullptr;
				})
				.UserOverlayMaterial_Lambda([WeakStepsController = CalibrationStepsController]()->UMaterialInterface*
				{
					if (TSharedPtr<FCameraCalibrationStepsController> PinnedStepsController = WeakStepsController.Pin())
					{
						if (PinnedStepsController->IsOverlayEnabled(EOverlayPassType::UserOverlay))
						{
							return PinnedStepsController->GetOverlayMaterial(EOverlayPassType::UserOverlay);
						}
					}
...
TSharedRef<SWidget> SCameraCalibrationSteps::BuildMediaProfileSourcePicker()
{
	return SAssignNew(MediaProfileSourcesComboBox, SComboBox<TWeakObjectPtr<UMediaSource>>)
		...
		.OnSelectionChanged_Lambda([&](TWeakObjectPtr<UMediaSource> NewValue, ESelectInfo::Type Type) -> void
		{
			...
			CalibrationStepsController.Pin()->SetMediaProfileSource(NewValue.Get());
		})
		...
		[
			...
			.Text_Lambda([&]() -> FText
			{
				...
				UMediaSource* MediaSource = CalibrationStepsController.Pin()->GetMediaProfileSource();
				if (!MediaSource)
				{
					return None;
				}

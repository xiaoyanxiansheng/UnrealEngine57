...
void UDMXControlConsoleFaderGroupController::PostLoad()
{
	...
	// Handle cases where the fixture patch is not present in the DMX Library anymore
	for (auto FaderGroupIt = FaderGroups.CreateIterator(); FaderGroupIt; FaderGroupIt++)
	{
		const TWeakObjectPtr<UDMXControlConsoleFaderGroup> WeakFaderGroup = *FaderGroupIt;
		if (!WeakFaderGroup.IsValid() || !WeakFaderGroup->HasValidData())
		{

...
#if WITH_EDITOR
bool UDMXControlConsoleFaderGroup::HasValidData() const
{
	const bool bHasRawFaders = Elements.ContainsByPredicate(
		[](const TScriptInterface<IDMXControlConsoleFaderGroupElement>& Element)
		{
			const UObject* ElementObject = Element.GetObject();
			return ElementObject && ElementObject->GetClass() == UDMXControlConsoleRawFader::StaticClass();
		});

	const bool bHasFixturePatchFaders = GetFixturePatch() != nullptr && !Elements.IsEmpty();

	return bHasRawFaders || bHasFixturePatchFaders;
}
#endif
...
#if WITH_EDITOR
	// Remove any invalid fader
	for (auto ElementIt = Elements.CreateIterator(); ElementIt; ++ElementIt)
	{
		const TScriptInterface<IDMXControlConsoleFaderGroupElement> Element = *ElementIt;
		if (!Element)
		{
			ElementIt.RemoveCurrent();
		}
	}
#endif
}

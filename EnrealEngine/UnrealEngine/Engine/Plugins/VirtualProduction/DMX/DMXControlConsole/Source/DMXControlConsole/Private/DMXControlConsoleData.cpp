...
#if WITH_EDITOR
	// Remove any fader group row that no longer contains valid data
	for (auto FaderGroupRowIt = FaderGroupRows.CreateIterator(); FaderGroupRowIt; ++FaderGroupRowIt)
	{
		const TObjectPtr<UDMXControlConsoleFaderGroupRow> FaderGroupRow = *FaderGroupRowIt;
		if (!FaderGroupRow || !FaderGroupRow->HasValidData())
		{
			FaderGroupRowIt.RemoveCurrent();
		}
	}

	// Add fixture patches that were added while this control console was not loaded
	TArray<UDMXEntity*> MissingFixturePatches;
	if (UDMXLibrary* DMXLibrary = CachedWeakDMXLibrary.Get())
	{
		...
		for (UDMXEntityFixturePatch* FixturePatch : FixturePatches)
		{
			...
			const bool bFaderGroupRowContainsFixturePatch = FaderGroupRows.ContainsByPredicate(
				[FixturePatch](const UDMXControlConsoleFaderGroupRow* Row)
				{
					const bool bFaderGroupContainsFixturePatch = Row && Row->GetFaderGroups().ContainsByPredicate(
						[FixturePatch](const UDMXControlConsoleFaderGroup* FaderGroup)
						{
							return 
								FaderGroup && 
								FaderGroup->GetFixturePatch() == FixturePatch;
						});

					return bFaderGroupContainsFixturePatch;
				});

			if (!bFaderGroupRowContainsFixturePatch)
			{
				MissingFixturePatches.Add(FixturePatch);
			}
		}

		// Add the missing patches as if they were added to the library while the control console is loaded
		OnFixturePatchAddedToLibrary(DMXLibrary, MissingFixturePatches);
	}
#endif
}

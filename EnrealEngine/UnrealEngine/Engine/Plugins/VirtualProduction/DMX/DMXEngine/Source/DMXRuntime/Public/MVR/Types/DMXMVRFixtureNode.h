...
public:
	...
	void SetUniverseID(int32 UniverseID) { Addresses.Universe = UniverseID - 1; }

	/** Returns the Universe ID, in UE form (Index 0 is ID 1) */
	int32 GetUniverseID() const { return Addresses.Universe + 1; }

	/** Sets the Address of the MVR Fixture */
	void SetStartingChannel(int32 Address) { Addresses.Address = Address; }

	/** Returns the Address of the MVR Fixture */
	int32 GetStartingChannel() const { return Addresses.Address; }

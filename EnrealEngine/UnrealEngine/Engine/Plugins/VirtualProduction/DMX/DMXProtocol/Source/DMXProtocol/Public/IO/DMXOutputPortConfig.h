...
struct DMXPROTOCOL_API FDMXOutputPortDestinationAddress
{
	...
	bool operator==(const FDMXOutputPortDestinationAddress& Other) const { return DestinationAddressString == Other.DestinationAddressString; }
	bool operator!=(const FDMXOutputPortDestinationAddress& Other) const { return !operator==(Other); }
	friend uint32 GetTypeHash(const FDMXOutputPortDestinationAddress& Key) { return GetTypeHash(Key.DestinationAddressString); }
};
...
public:
	...
	const FString& GetPortName() const { return PortName; }
	const FName& GetProtocolName() const { return ProtocolName; }
	EDMXCommunicationType GetCommunicationType() const { return CommunicationType; }
	uint8 GetTimeToLive() const { return TimeToLive; }
	bool IsAutoCompleteDeviceAddressEnabled() const { return bAutoCompleteDeviceAddressEnabled; }
	FString GetAutoCompleteDeviceAddress() const { return AutoCompleteDeviceAddress; }
	FString GetDeviceAddress() const;
	const TArray<FDMXOutputPortDestinationAddress>& GetDestinationAddresses() const { return DestinationAddresses; }
	bool NeedsLoopbackToEngine() const { return bLoopbackToEngine; }
	int32 GetLocalUniverseStart() const { return LocalUniverseStart; }
	int32 GetNumUniverses() const { return NumUniverses; }
	...
	const FFrameRate& GetDelayFrameRate() const { return DelayFrameRate; }
	...
	const FString& GetDestinationAddress() const { return DestinationAddress; }
...
protected:
	...
	/** Sets how many hops multicast DMX packets can take before being discarded. In common network setups this should be set to 1. */
	UPROPERTY()
	uint8 TimeToLive = 1;

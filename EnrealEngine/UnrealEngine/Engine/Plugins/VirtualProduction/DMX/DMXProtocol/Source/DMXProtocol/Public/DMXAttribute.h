...
struct DMXPROTOCOL_API FDMXAttribute
{
	...
	/** Comparison operator */
	inline bool operator==(const FDMXAttribute& Other) const
	{
		return Name.IsEqual(Other.Name) && Keywords.Equals(Other.Keywords);
	}
	inline bool operator!=(const FDMXAttribute& Other) const { return !(*this == Other); }
...
inline uint32 GetTypeHash(const FDMXAttribute& Attribute)
{

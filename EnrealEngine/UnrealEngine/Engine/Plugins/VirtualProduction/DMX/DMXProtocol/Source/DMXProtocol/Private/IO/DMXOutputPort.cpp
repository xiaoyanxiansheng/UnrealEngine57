...
namespace UE::DMX::DMXOutputPort::Private::ConsoleCommands
{
	...
	void LogOutputPortConfiguration(const FDMXOutputPortSharedPtr OutputPort)
	{
		if (OutputPort.IsValid())
		{
			...
			UE_LOG(LogDMXProtocol, Log, TEXT("TimeToLive: %i"), PortConfig.GetTimeToLive());
	...
	static FAutoConsoleCommand GDMXSetTimeToLiveCommand(
		TEXT("DMX.SetOutputPortTimeToLive"),
		TEXT("DMX.SetOutputPortTimeToLive [PortName][TimeToLive (0-255, Default 1)]. Should usually not be changed. Sets how many hops a multicast DMX packet can take before being discarded. Example: DMX.SetOutputPortTimeToLive MyOutputPort 1"),
		FConsoleCommandWithArgsDelegate::CreateStatic(
			[](const TArray<FString>& Args)
			{
				constexpr int32 MinNumExpectedArgs = 2;
				if (!VerifyConsoleCommandArguments(TEXT("DMX.SetOutputPortTimeToLive"), MinNumExpectedArgs, Args))
				{
					return;
				}

				const FString& PortName = Args[0];
				const FString& TTLValueString = Args[1];

				uint8 TTLValue;
				if (LexTryParseString<uint8>(TTLValue, *TTLValueString))
				{
					const FDMXOutputPortSharedPtr OutputPort = FindOutputPortByName(PortName);
					...
					if (OutputPort.IsValid())
					{
						if (const IDMXProtocolPtr Protocol = OutputPort->GetProtocol())
						{
							DMX_OVERRIDE_OUTPUTPORT_VAR(TimeToLive, PortName, TTLValue);
						}
					}
				}
			})
...
{	
	...
	const bool bNeedsUpdateRegistration = [this, &OutputPortConfig, bForceUpdateRegistrationWithProtocol]()
	{
		...
		if (ProtocolName == OutputPortConfig.GetProtocolName() &&
			...
			TimeToLive == OutputPortConfig.GetTimeToLive())
		{
			return false;
		}	
	...
	// Only update on the next tick to give the OS time to fully release previous sockets
	if (bPendingRegistration)
	{
		return;
	}
	bPendingRegistration = true;

	AsyncTask(ENamedThreads::GameThread, [bNeedsUpdateRegistration, WeakThis = AsWeak(), this]()
		{
			if (!WeakThis.IsValid())
			{
				return;

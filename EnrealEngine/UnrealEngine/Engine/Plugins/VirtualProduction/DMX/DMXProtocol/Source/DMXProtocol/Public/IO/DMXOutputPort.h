...
struct FDMXOutputPortCommunicationDeterminator
{
	...
	void SetLoopbackToEngine(bool bInLoopbackToEngine) { bLoopbackToEngine = bInLoopbackToEngine; }

	/** Sets if receive is enabled */
	void SetReceiveEnabled(bool bInReceiveEnabled) { bReceiveEnabled = bInReceiveEnabled; }

	/** Sets if send is enabled */
	void SetSendEnabled(bool bInSendEnabled) { bSendEnabled = bInSendEnabled; }

	/** Sets if there is a valid sender obj */
	void SetHasValidSender(bool bInHasValidSender) { bHasValidSender = bInHasValidSender; }

	/** Determinates if loopback to engine is needed (may be true even is loopback to engine is not enabled) */
	bool NeedsLoopbackToEngine() const { return bLoopbackToEngine || !bReceiveEnabled || !bSendEnabled; }

	/** Determinates if dmx needs to be sent (may be false even if send is enabled) */
	bool NeedsSendDMX() const { return bSendEnabled && bHasValidSender; }

	/** Determinates if send dmx is enabled */
	bool IsSendDMXEnabled() const { return bSendEnabled; }

	/** Returns true if loopback to engine is enabled (only true if loopback to engine is set enabled) */
	bool IsLoopbackToEngineEnabled() const { return bLoopbackToEngine; }
...
public:
	...
	/** Returns the multicast Time-to-Live (TTL), only used when the port is set to multicast */
	uint8 GetTimeToLive() const { return TimeToLive; }

	/** Returns the output port's delay in seconds */
	double GetDelaySeconds() const { return DelaySeconds; }
...
private:
	...
	/** Multicast Time-to-Live (TTL), only used when the port is set to multicast */
	uint8 TimeToLive = 1;
	...
	/** True while registration is pending */

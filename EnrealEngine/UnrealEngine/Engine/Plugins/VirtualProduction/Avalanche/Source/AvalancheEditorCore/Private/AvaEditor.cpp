...
void FAvaEditor::Construct()
{
	ForEachExtension(
		[This = SharedThis(this)](const TSharedRef<IAvaEditorExtension>& InExtension)
		{
			InExtension->Construct(This);
		});
}
...
UObject* FAvaEditor::GetSceneObject(EAvaEditorObjectQueryType InQueryType) const
{
	...
	{
		// Check that the query types from here on are either find or create
		check(InQueryType == EAvaEditorObjectQueryType::SearchOnly || InQueryType == EAvaEditorObjectQueryType::CreateIfNotFound);

		SceneObject = Provider->GetSceneObject(SceneWorld, EAvaEditorObjectQueryType::SearchOnly);

		if (!SceneObject && InQueryType == EAvaEditorObjectQueryType::CreateIfNotFound)
		{
			SceneObject = Provider->GetSceneObject(SceneWorld, EAvaEditorObjectQueryType::CreateIfNotFound);
			bWasCreated = true;
		}
	}
	...
	if (bWasCreated)
	{
		NotifySceneObjectCreated(SceneObject);
	}
	return SceneObject;
}
...
void FAvaEditor::ExtendCreateSceneOptions(UToolMenu* InMenu)
{
	check(InMenu);

	ForEachExtension(
		[InMenu](const TSharedRef<IAvaEditorExtension>& InExtension)
		{
			InExtension->ExtendCreateSceneOptions(InMenu);
		});
}
...
void FAvaEditor::NotifySceneObjectCreated(UObject* InSceneObject) const
{
	ForEachExtension(
		[InSceneObject](const TSharedRef<IAvaEditorExtension>& InExtension)
		{
			InExtension->OnSceneObjectCreated(InSceneObject);
		});
}

...
void AAvaSequencePlaybackActor::PauseSequence(UAvaSequence* InSequence)
{
	if (UAvaSequencePlayer* const Player = GetSequencePlayer(InSequence))
	{
		Player->Pause();
...
void AAvaSequencePlaybackActor::PauseSequencesByLabel(FName InSequenceLabel)
{
	PauseSequencesByLabels({ InSequenceLabel });
}

void AAvaSequencePlaybackActor::PauseSequencesByLabels(const TArray<FName>& InSequenceLabels)
{
	const TArray<UAvaSequencePlayer*> SequencePlayers = GetSequencePlayersByLabels(InSequenceLabels);
	for (UAvaSequencePlayer* SequencePlayer : SequencePlayers)
	{
		SequencePlayer->Pause();
	}
}

void AAvaSequencePlaybackActor::PauseSequencesByTag(const FAvaTagHandle& InTagHandle, bool bInExactMatch)
{
	const TArray<UAvaSequencePlayer*> SequencePlayers = GetSequencePlayersByTag(InTagHandle, bInExactMatch);
	for (UAvaSequencePlayer* SequencePlayer : SequencePlayers)
	{
		SequencePlayer->Pause();
	}
...
void AAvaSequencePlaybackActor::StopSequencesByLabel(FName InSequenceLabel)
{
	StopSequencesByLabels({ InSequenceLabel });
}

void AAvaSequencePlaybackActor::StopSequencesByLabels(const TArray<FName>& InSequenceLabels)
{
	const TArray<UAvaSequencePlayer*> SequencePlayers = GetSequencePlayersByLabels(InSequenceLabels);
	for (UAvaSequencePlayer* SequencePlayer : SequencePlayers)
	{
...
void AAvaSequencePlaybackActor::StopSequencesByTag(const FAvaTagHandle& InTagHandle, bool bInExactMatch)
{
	const TArray<UAvaSequencePlayer*> SequencePlayers = GetSequencePlayersByTag(InTagHandle, bInExactMatch);
	for (UAvaSequencePlayer* SequencePlayer : SequencePlayers)
	{
...
TArray<UAvaSequence*> AAvaSequencePlaybackActor::GetSequencesByTag(const FAvaTagHandle& InTagHandle, bool bInExactMatch) const
{
	...
	for (UAvaSequence* Sequence : AllSequences)
	{
		...
		if (bInExactMatch)
		{
			bTagMatches = Sequence->GetSequenceTag().MatchesExact(InTagHandle);
		}
		else
		{
			bTagMatches = Sequence->GetSequenceTag().Overlaps(InTagHandle);
		}
...
TArray<UAvaSequencePlayer*> AAvaSequencePlaybackActor::GetSequencePlayers(TConstArrayView<const UAvaSequence*> InSequences) const
{
	TArray<UAvaSequencePlayer*> SequencePlayers;
	SequencePlayers.Reserve(InSequences.Num());

	for (UAvaSequencePlayer* SequencePlayer : ActiveSequencePlayers)
	{
		if (SequencePlayer && InSequences.Contains(SequencePlayer->GetSequence()))
		{
			SequencePlayers.Add(SequencePlayer);
		}
	}
...
{
	return GetSequencePlayers(GetSequencesByTag(InTagHandle, bInExactMatch));
}
...
{
	return GetSequencePlayers(GetSequencesByLabel(InSequenceLabels));
}

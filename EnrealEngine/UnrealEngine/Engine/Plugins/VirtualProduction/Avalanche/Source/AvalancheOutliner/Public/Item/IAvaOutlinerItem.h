...
public:
	 ...
	 */
	UE_DEPRECATED(5.6, "Use AddChildren instead")
	virtual bool AddChild(const FAvaOutlinerAddItemParams& InAddItemParams) final
	{
		return AddChildren(InAddItemParams).Num() > 0;
	}

	/**
	 * Adds the items in the add items struct under this item if such items are supported.
	 * Returns the children that were successfully added
	 */
	virtual TArray<FAvaOutlinerItemPtr> AddChildren(const FAvaOutlinerAddItemParams& InAddItemParams) = 0;

	/** Called when children have been added to this item */
	virtual void OnChildrenAdded(TConstArrayView<FAvaOutlinerItemPtr> InAddedChildren, const FAvaOutlinerAddItemParams& InAddItemParams) {}

...
public:
	 ...
	 */
	static void SortItems(TArray<FAvaOutlinerItemPtr>& OutOutlinerItems);
...
private:
	/** Binds a given worlds delegates to the outliner, unbinding the previous bound world's */
	void BindWorldDelegates(UWorld* InWorld);
	/** Unbinds the last bound world's delegates from outliner */
	...
	/** Latest world with bound delegates. */
	TWeakObjectPtr<UWorld> BoundWorldWeak;

...
FAvaOutlinerView::~FAvaOutlinerView()
{
	...
	if (OutlinerWidgetContainer.IsValid())
	{
		OutlinerWidgetContainer->ClearContent();

		const int32 OutlinerWidgetReferenceCount = OutlinerWidget.GetSharedReferenceCount();
		ensureMsgf(OutlinerWidgetReferenceCount == 1, TEXT("Outliner Widget expected to have 1 shared reference count but had %d"), OutlinerWidgetReferenceCount);
	}

	OutlinerWidget.Reset();
...
void FAvaOutlinerView::Init(const TSharedRef<FAvaOutliner>& InOutliner, bool bCreateOutlinerWidget)
{
	...
	{
		OutlinerWidgetContainer = SNew(SBorder)
			.BorderImage(FAppStyle::GetBrush("NoBorder"))
			.Padding(0)
			[
				SAssignNew(OutlinerWidget, SAvaOutliner, SharedThis(this))
			];
...
TSharedRef<SWidget> FAvaOutlinerView::GetOutlinerWidget() const
{
	if (OutlinerWidgetContainer.IsValid())
	{
		return OutlinerWidgetContainer.ToSharedRef();
	}

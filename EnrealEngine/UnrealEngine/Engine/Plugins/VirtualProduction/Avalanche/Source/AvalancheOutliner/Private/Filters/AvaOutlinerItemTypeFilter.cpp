...
#define LOCTEXT_NAMESPACE "AvaOutlinerItemTypeFilter"
...
FText FAvaOutlinerItemTypeFilterData::GetTooltipText() const
{
	...
	TArray<FText> SupportedClasses;
	SupportedClasses.Reserve(FilterClasses.Num());

	for (TSubclassOf<UObject> FilterClass : FilterClasses)
	{
		if (FilterClass)
		{
			SupportedClasses.Add(FilterClass->GetDisplayNameText());
		}
	}

	const_cast<FAvaOutlinerItemTypeFilterData*>(this)->TooltipText = FText::Join(LOCTEXT("CommaDelimiter", ", "), SupportedClasses);
	return TooltipText;
}

...
bool FAvaOutlinerComponent::CanAddChild(const FAvaOutlinerItemPtr& InChild) const
{
	// Super checks InChild.IsValid
	return Super::CanAddChild(InChild) && InChild->IsA<FAvaOutlinerComponent>();
}

void FAvaOutlinerComponent::OnChildrenAdded(TConstArrayView<FAvaOutlinerItemPtr> InAddedChildren, const FAvaOutlinerAddItemParams& InAddItemParams)
{
	Super::OnChildrenAdded(InAddedChildren, InAddItemParams);

	for (const FAvaOutlinerItemPtr& Item : InAddedChildren)
	{
		FAvaOutlinerComponent* ComponentItem = Item->CastTo<FAvaOutlinerComponent>();
		if (!ComponentItem)
		{
			continue;
		}

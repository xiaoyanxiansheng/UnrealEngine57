...
#include "ToolMenus.h"
#include "Editor/LevelEditor/Private/ViewportToolbar/LevelViewportContext.h"
...
FName SAvaLevelViewport::RegisterViewportToolbar() const
{
	{
		// Extend the settings menu with custom visualizer entries
		const FName ParentSettingsName = "LevelEditor.ViewportToolbar.Settings";
		const FName SettingsName = "Ava.ViewportToolbar.Settings";
		
		if (!UToolMenus::Get()->FindMenu(ParentSettingsName))
		{
			UToolMenus::Get()->ExtendMenu(ParentSettingsName);
		}
		
		if (!UToolMenus::Get()->IsMenuRegistered(SettingsName))
		{
			AddVisualizerEntries(UToolMenus::Get()->RegisterMenu(SettingsName, ParentSettingsName));
		}	
	}
	
	const FName ViewportToolbarName = "Ava.ViewportToolbar";
	
	if (!UToolMenus::Get()->IsMenuRegistered(ViewportToolbarName))
	{
		UToolMenu* Menu = UToolMenus::Get()->RegisterMenu(ViewportToolbarName, Super::RegisterViewportToolbar(), EMultiBoxType::SlimHorizontalToolBar);
		Menu->StyleName = "ViewportToolbar";
		Menu->bSeparateSections = false;
		
		{
			FToolMenuSection& LeftSection = Menu->AddSection("Left");
			FToolMenuEntry& ChildActorLock = LeftSection.AddMenuEntry(FAvaLevelViewportCommands::GetInternal().ToggleChildActorLock);
			ChildActorLock.InsertPosition.Position = EToolMenuInsertType::Last;
			ChildActorLock.Icon = FSlateIcon(FAvaLevelViewportStyle::Get().GetStyleSetName(), "Button.PivotMode");
			ChildActorLock.ToolBarData.LabelOverride = FText::GetEmpty();
		}
		
		{
			...
			// Override the "Camera" submenu
			RightSection.AddDynamicEntry("DynamicCamera", FNewToolMenuSectionDelegate::CreateLambda([](FToolMenuSection& Section)
			{
				TSharedPtr<SAvaLevelViewport> Viewport = StaticCastSharedPtr<SAvaLevelViewport>(ULevelViewportContext::GetLevelViewport(Section));
				if (!Viewport)
				{
					return;
				}
			
				FToolMenuEntry& CameraMenu = Section.AddSubMenu(
					"Camera",
					LOCTEXT("CameraMenuLabel", "Camera"),
					FText::GetEmpty(),
					FOnGetContent::CreateSP(Viewport.Get(), &SAvaLevelViewport::GenerateCameraMenu)
				);
				CameraMenu.Icon.Bind(Viewport.Get(), &SAvaLevelViewport::GetCameraIcon);
			}));
		}	
	}
	
	return ViewportToolbarName;
}

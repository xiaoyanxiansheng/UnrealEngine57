...
int32 SAvaLevelViewportActorColorMenu::ActiveThemeIndex = 0;
	...
	FMenuBuilder ThemeMenuBuilder(bInShouldCloseWindowAfterMenuSelection, nullptr);
	{
				...
				.OnTextCommitted(FOnTextCommitted::CreateSPLambda(
					...
					[this](const FText& NewName, ETextCommit::Type CommitType)
					{
						if (CommitType == ETextCommit::OnEnter)
						{
							...
							for (int32 Index = 0; Index < ColorThemes.Num(); ++Index)
							{
									if (Index != ActiveThemeIndex && ColorThemes[Index].GetName().Equals(NewNameString, ESearchCase::IgnoreCase))
								{
									UE_LOG(AvaLevelViewportLog, Error, TEXT("Theme with that name already exists."));
									return;
								}
							}

							ColorThemes[ActiveThemeIndex].SetName(NewNameString);
							SaveColorThemesToIni();
						}
					})
				),
		...
		for (int32 ThemeIdx = 0; ThemeIdx < ColorThemes.Num(); ++ThemeIdx)
		{
			ThemeMenuBuilder.AddSubMenu(
				...
				FNewMenuDelegate::CreateSPLambda(
					this, 
					[this, ThemeIdx](FMenuBuilder& SubMenuBuilder)
					{
			...
			.OnTextCommitted(FOnTextCommitted::CreateSPLambda(this,
				[this](const FText& NewName, ETextCommit::Type CommitType)
				{
					if (CommitType == ETextCommit::OnEnter)
					{
						...
						for (int32 Index = 0; Index < ColorThemes.Num(); ++Index)
						{
							if (ColorThemes[Index].GetName().Equals(NewNameString, ESearchCase::IgnoreCase))
							{
								UE_LOG(AvaLevelViewportLog, Error, TEXT("Theme with that name already exists."));
								return;
							}
						...
						ActiveThemeIndex = ColorThemes.Add(FAvaColorTheme(NewNameString));
						SaveColorThemesToIni();
						UpdateColorPalette();
					}
				})

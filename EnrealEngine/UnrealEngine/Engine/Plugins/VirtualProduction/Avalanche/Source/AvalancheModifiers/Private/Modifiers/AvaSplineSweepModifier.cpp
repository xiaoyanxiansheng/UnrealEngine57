...
{
	InSteps = FMath::Clamp(InSteps, 3, MaxSampleCount);
	if (Steps == InSteps)
	{
		return;
	}
...
#if WITH_EDITOR
	InMetadata.SetDisplayName(LOCTEXT("ModifierDisplayName", "Spline Sweep"));
...
void UAvaSplineSweepModifier::Apply()
{
	...
	const bool bInvalidProgress = ProgressStart > ProgressEnd;
	const bool bOutOfRange = !SplineComponent->IsClosedLoop()
		&& (ProgressStart + ProgressOffset > 1.f || ProgressEnd + ProgressOffset < 0.f);

	if (bOutOfRange || bInvalidProgress)
	{
		// Clear mesh
		TargetMesh->EditMesh([](FDynamicMesh3& InMesh)
		{
	...
	// Samples needed should be >= 3 to avoid any issue during AppendSweepPolygon operation
	if (SplineSamples.Num() < 3)
	{
		Next();
		return;
	}

...
#if WITH_EDITOR
	InMetadata.SetDisplayName(LOCTEXT("ModifierDisplayName", "Translucent Sort Priority"));
...
void UAvaTranslucentPriorityModifier::Apply()
{
	...
	else
	{
		...
		for (const FAvaTranslucentPriorityModifierComponentState& SortedComponentState : CachedSortedComponentStates)
		{
			...
			const bool bHasIteratedOnce = PreviousComponentOwner != nullptr;
			const bool bShouldIncreaseStep = Mode == EAvaTranslucentPriorityModifierMode::AutoCameraDistance || PreviousComponentOwner != Component->GetOwner();

			if (bHasIteratedOnce && bShouldIncreaseStep)
			{
				TranslucentSortPriority += GlobalStep;
			}

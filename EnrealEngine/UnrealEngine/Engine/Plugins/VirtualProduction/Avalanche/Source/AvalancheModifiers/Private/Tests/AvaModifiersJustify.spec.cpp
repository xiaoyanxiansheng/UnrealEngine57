...
BEGIN_DEFINE_SPEC(AvalancheModifiersJustify, "Avalanche.Modifiers.Justify",
	...
	UActorModifierJustifyModifier* JustifyModifier;
...
void AvalancheModifiersJustify::Define()
{
	BeforeEach([this]
	{
		...
		const FName ModifierJustifyName = ModifierTestUtils->GetModifierName(UActorModifierJustifyModifier::StaticClass());
		...
		JustifyModifier = Cast<UActorModifierJustifyModifier>(ModifierSubsystem->InsertModifier(ModifierStack, InsertOp));
		JustifyModifier->SetHorizontalAlignment(EActorModifierJustifyHorizontal::Right);
		JustifyModifier->SetVerticalAlignment(EActorModifierJustifyVertical::Bottom);
		JustifyModifier->SetDepthAlignment(EActorModifierJustifyDepth::Center);

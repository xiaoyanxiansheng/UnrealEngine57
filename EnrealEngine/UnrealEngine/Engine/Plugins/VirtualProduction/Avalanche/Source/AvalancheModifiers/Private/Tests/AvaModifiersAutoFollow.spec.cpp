...
BEGIN_DEFINE_SPEC(AvalancheModifiersAutoFollowTests, "Avalanche.Modifiers.Autofollow",
	...
	UActorModifierAutoFollowModifier* AutoFollowModifier;
...
void AvalancheModifiersAutoFollowTests::Define()
{
	BeforeEach([this]
	{
		...
		ModifierAutoFollowName = ModifierTestUtils->GetModifierName(UActorModifierAutoFollowModifier::StaticClass());
	...
	Describe("Motion Design modifiers. Autofollow modifier application", [this]()
	{
		BeforeEach([this]()
		{
			...
			AutoFollowModifier = Cast<UActorModifierAutoFollowModifier>(ModifierSubsystem->InsertModifier(ActorModifierStack, InsertOp));
		});

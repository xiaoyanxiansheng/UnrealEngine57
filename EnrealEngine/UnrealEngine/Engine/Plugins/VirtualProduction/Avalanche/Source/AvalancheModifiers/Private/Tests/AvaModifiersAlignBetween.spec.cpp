...
BEGIN_DEFINE_SPEC(AvalancheModifiersAlignBetween, "Avalanche.Modifiers.AlignBetween",
	...
	UActorModifierAlignBetweenModifier* AlignBetweenModifier;
...
void AvalancheModifiersAlignBetween::Define()
{
	BeforeEach([this]
	{
		...
		const FName ModifierName = ModifierTestUtils->GetModifierName(UActorModifierAlignBetweenModifier::StaticClass());
		...
		AlignBetweenModifier = Cast<UActorModifierAlignBetweenModifier>(ModifierSubsystem->InsertModifier(ModifierStack, InsertOp));
		check(AlignBetweenModifier);

		// Set up the modifier
		FActorModifierAlignBetweenWeightedActor ReferenceOneActor = FActorModifierAlignBetweenWeightedActor(ReferenceOneMeshActor);
		ReferenceOneActor.bEnabled = true;
		ReferenceOneActor.Weight = ReferenceOneWeight;
		FActorModifierAlignBetweenWeightedActor ReferenceTwoActor = FActorModifierAlignBetweenWeightedActor(ReferenceTwoMeshActor);

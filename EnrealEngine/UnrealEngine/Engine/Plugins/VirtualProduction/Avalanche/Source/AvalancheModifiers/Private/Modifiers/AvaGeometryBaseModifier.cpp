...
void UAvaGeometryBaseModifier::OnModifierEnabled(EActorModifierCoreEnableReason InReason)
{
	Super::OnModifierEnabled(InReason);

	if (UDynamicMeshComponent* const DynMeshComp = GetMeshComponent())
	{
		DynMeshComp->OnMeshChanged.AddUObject(this, &UAvaGeometryBaseModifier::OnMeshChanged);
	}
}

void UAvaGeometryBaseModifier::OnModifierDisabled(EActorModifierCoreDisableReason InReason)
{
	Super::OnModifierDisabled(InReason);

	if (UDynamicMeshComponent* const DynMeshComp = GetMeshComponent())
	{
		DynMeshComp->OnMeshChanged.RemoveAll(this);
	}
}

void UAvaGeometryBaseModifier::OnMeshChanged()
{
	if (const UActorModifierCoreStack* RootStack = GetRootModifierStack())
	{
		if (RootStack->IsModifierIdle()
			&& IsModifierEnabled()
			&& RootStack->IsModifierInitialized()
			&& !RootStack->IsModifierExecutionLocked())
		{
			PreModifierCachedMesh.Reset();
			MarkModifierDirty();
		}
	}
}

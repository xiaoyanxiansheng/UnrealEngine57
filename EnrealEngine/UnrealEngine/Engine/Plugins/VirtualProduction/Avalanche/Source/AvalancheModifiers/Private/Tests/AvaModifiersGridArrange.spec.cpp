...
BEGIN_DEFINE_SPEC(AvalancheModifiersGridArrange, "Avalanche.Modifiers.GridArrange",
	...
	UActorModifierGridArrangeModifier* GridArrangeModifier;
	FIntPoint GridCount;
...
void AvalancheModifiersGridArrange::Define()
{
	BeforeEach([this]
	{
		...
		const FName ModifierGridArrangeName = ModifierTestUtils->GetModifierName(UActorModifierGridArrangeModifier::StaticClass());
		...
		GridArrangeModifier = Cast<UActorModifierGridArrangeModifier>(ModifierSubsystem->InsertModifier(ModifierStack, InsertOp));
	});

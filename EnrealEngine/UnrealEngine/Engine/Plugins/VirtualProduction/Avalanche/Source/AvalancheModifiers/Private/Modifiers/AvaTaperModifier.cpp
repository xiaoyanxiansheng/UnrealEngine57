...
#if WITH_EDITOR
	InMetadata.SetDisplayName(LOCTEXT("ModifierDisplayName", "Taper"));
...
void UAvaTaperModifier::Apply()
{
	...
	if (FDynamicMesh3* const SharedMesh = DynMeshComp->GetMesh())
	{
		...
		TaperSettings.ZAxisResolution = GetSubdividersCuts();
...
int32 UAvaTaperModifier::GetSubdividersCuts() const
{
	...
	{
		FActorModifierCoreStackSearchOp SearchOp;
		SearchOp.bSkipStack = true;
		SearchOp.Position = EActorModifierCoreStackPosition::Before;
		SearchOp.PositionContext = const_cast<UAvaTaperModifier*>(this);
		const TArray<UActorModifierCoreBase*> Modifiers = Stack->FindModifiers(UAvaSubdivideModifier::StaticClass(), SearchOp);

		for (const UActorModifierCoreBase* const Modifier : Modifiers)
		{
			if (const UAvaSubdivideModifier* SubdivideModifier = Cast<UAvaSubdivideModifier>(Modifier))
			{

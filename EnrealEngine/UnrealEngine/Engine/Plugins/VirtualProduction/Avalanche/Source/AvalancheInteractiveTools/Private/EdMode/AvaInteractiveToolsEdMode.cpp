...
void UAvaInteractiveToolsEdMode::BindCommands()
{
	...
	CommandList->MapAction(
		ToolManagerCommands.ToggleViewportToolbar,
		FExecuteAction::CreateWeakLambda(this, [this]()
			{
				if (Toolkit.IsValid())
				{
					FAvaInteractiveToolsEdModeToolkit* AITToolkit = static_cast<FAvaInteractiveToolsEdModeToolkit*>(Toolkit.Get());
					AITToolkit->SetViewportToolbarVisibility(!AITToolkit->GetViewportToolbarVisibility());
				}
			}),
		FCanExecuteAction(),
		FGetActionCheckState(),
	...
	CommandList->MapAction(
		ToolManagerCommands.OpenViewportToolbarSettings,
		FExecuteAction::CreateWeakLambda(this, [this]()
			{
				if (const UAvaInteractiveToolsSettings* Settings = UAvaInteractiveToolsSettings::Get())
				{
					Settings->OpenEditorSettingsWindow();
				}
			}),
		FCanExecuteAction(),
		FGetActionCheckState(),

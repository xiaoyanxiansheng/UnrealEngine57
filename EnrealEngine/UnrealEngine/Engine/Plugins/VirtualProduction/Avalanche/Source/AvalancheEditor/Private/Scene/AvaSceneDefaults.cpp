...
namespace UE::AvaEditor::Private
{
	...
	{
		/** The class of the actor to create/replace/update*/
		TWeakObjectPtr<UClass> ClassWeak;
		/** Describes the actor to create/replace/update */
		FText Description;
		/** Delegate called to initialize the actor */
		FAvaSceneDefaultActorInitDelegate InitDelegate;
		/** Response to apply when there are available actors. */
		EAvaSceneDefaultActorResponse DesiredResponse = EAvaSceneDefaultActorResponse::ReplaceActor;
	};
	...
	static const TMap<FName, FAvaSceneDefaultActor> DefaultSceneActors = {
		{
			DefaultSceneActorNames::DefaultRoot,
			{
				...
				EAvaSceneDefaultActorResponse::CreateNewActor
			}
		},
		{
	...
	TMap<FName, TSharedRef<FAvaSceneDefaultActorResponse>> GeneratorInitialActorResponses(UWorld* InWorld)
	{
		...
		{
			UClass* const Class = DefaultPair.Value.ClassWeak.Get();
			if (!Class)
			{
				continue;
			}
			...
			ActorResponse->ActorClassWeak = Class;
			ActorResponse->Description = DefaultPair.Value.Description;

			const FObjectKey ClassKey = FObjectKey(Class);
			...
			else
			{
				...
				ActorResponse->AvailableActors.Reset();
				...
				if (!ActorResponse->AvailableActors.IsEmpty())
				{
					...
					ActorResponse->Response = DefaultPair.Value.DesiredResponse;
				}

...
{
	return AvaSequencer.IsValid() ? AvaSequencer->GetSequencerPtr() : TSharedPtr<ISequencer>();
}

void FAvaSequencerExtension::OnObjectRenamed(UObject* InRenamedObject, const FText& InDisplayNameText)
{
	if (!InRenamedObject || !AvaSequencer.IsValid())
	{
		return;
	}

	if (const TSharedPtr<ISequencer> Sequencer = AvaSequencer->GetSequencerPtr())
	{
...
void FAvaSequencerExtension::Activate()
{
	if (UWorld* const World = GetWorld())
	{
		if (UAvaSequencerSubsystem* SequencerSubsystem = World->GetSubsystem<UAvaSequencerSubsystem>())
		{
			...
			ConditionallyCreateDefaultSequences();
...
void FAvaSequencerExtension::OnSceneObjectCreated(UObject* InSceneObject)
{
	bCreateDefaultSequences = true;
}
...
void FAvaSequencerExtension::ConditionallyCreateDefaultSequences()
{
	if (!bCreateDefaultSequences || !AvaSequencer.IsValid())
	{
		return;
	}
	...
	// Transition Logic Default Sequences
	...
	{
		const TArray<const FAvaSequencePreset*> SequencePresets = SequencerSettings->GatherPresetsFromGroup(UAvaSequencerSettings::TransitionLogicStarterGroup);
		AvaSequencer->AddSequenceFromPresets(SequencePresets);
	}

	bCreateDefaultSequences = false;
}

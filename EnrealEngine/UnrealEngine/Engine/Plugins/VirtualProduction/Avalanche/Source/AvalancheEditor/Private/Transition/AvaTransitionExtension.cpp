...
void FAvaTransitionExtension::OnSceneObjectCreated(UObject* InSceneObject)
{
	const UAvaTransitionEditorSettings* TransitionEditorSettings = GetDefault<UAvaTransitionEditorSettings>();
	if (!TransitionEditorSettings || !TransitionEditorSettings->ShouldCreateTransitionLogicDefaultScene())
	{
		return;
	}
	...
	UAvaTransitionSubsystem* const TransitionSubsystem = World->GetSubsystem<UAvaTransitionSubsystem>();
	if (!TransitionSubsystem)
	{
		return;
	}

	const IAvaTransitionBehavior* const TransitionBehavior = TransitionSubsystem->GetOrCreateTransitionBehavior();
	if (!TransitionBehavior)
	{
		return;
	}

	// Enable TL if creating the TL default scene
	if (UAvaTransitionTree* const TransitionTree = TransitionBehavior->GetTransitionTree())
	{
		TransitionTree->SetEnabled(true);
		TransitionTree->SetInstancingMode(EAvaTransitionInstancingMode::Reuse);
	}

	// Spawn Layer 1 Content null
	FActorSpawnParameters SpawnParameters;
	SpawnParameters.InitialActorLabel = TEXT("Layer 1 Content");
	AAvaNullActor* const Layer1Actor = World->SpawnActor<AAvaNullActor>(SpawnParameters);

	// Add the Sub Layer Modifier (BP) to this Layer 1 Content null
	UActorModifierCoreStack* ModifierStack;
	if (UActorModifierCoreLibrary::FindModifierStack(Layer1Actor, ModifierStack, /*bCreateIfNone*/true))
	{
		constexpr const TCHAR* SubLayerModifierClassPath = TEXT("/Avalanche/TransitionLogic/SubLayer/BP_SubLayerModifier.BP_SubLayerModifier_C";

		FActorModifierCoreInsertOperation InsertOperation;
		InsertOperation.ModifierClass = StaticLoadClass(UActorModifierCoreBase::StaticClass(), nullptr, SubLayerModifierClassPath));

		UActorModifierCoreBase* Modifier;
		if (UActorModifierCoreLibrary::InsertModifier(ModifierStack, InsertOperation, Modifier))
		{
			auto SetSequenceName =
				[Modifier, ModifierClass = InsertOperation.ModifierClass](FName InPropertyName, FAvaSequenceName InSequenceName)
				{
					FStructProperty* const StructProperty = CastField<FStructProperty>(ModifierClass->FindPropertyByName(InPropertyName));
					if (StructProperty && StructProperty->Struct == FAvaSequenceName::StaticStruct())
					{
						StructProperty->SetValue_InContainer(Modifier, &InSequenceName);
					}
				};

			SetSequenceName(TEXT("Change In Sequence"), FAvaSequenceName(UAvaSequencerSettings::DefaultChangeInName));
			SetSequenceName(TEXT("Change Out Sequence"), FAvaSequenceName(UAvaSequencerSettings::DefaultChangeOutName));
		}
	}
}
...
void FAvaTransitionExtension::ExtendCreateSceneOptions(UToolMenu* InMenu)
{
	auto ToggleCreateTLDefaultScene = []()->void
		{
			if (UAvaTransitionEditorSettings* TransitionEditorSettings = GetMutableDefault<UAvaTransitionEditorSettings>())
			{
				FScopedTransaction Transaction(LOCTEXT("ToggleTransitionTreeEnabled", "Toggle Transition Tree Enabled"));
				TransitionEditorSettings->Modify();
				TransitionEditorSettings->ToggleCreateTransitionLogicDefaultScene();
				TransitionEditorSettings->SaveConfig();
			}
		};

	auto ShouldCreateTLDefaultScene = []()->bool
		{
			const UAvaTransitionEditorSettings* TransitionEditorSettings = GetDefault<UAvaTransitionEditorSettings>();
			return TransitionEditorSettings && TransitionEditorSettings->ShouldCreateTransitionLogicDefaultScene();
		};

	FToolMenuSection& Section = InMenu->FindOrAddSection(DefaultSectionName);

	Section.AddMenuEntry(TEXT("CreateTransitionLogicDefaultScene")
		, LOCTEXT("CreateTransitionLogicDefaultSceneLabel", "Create Transition Logic Default Scene")
		, LOCTEXT("CreateTransitionLogicDefaultSceneTooltip", "Determines whether to create the default setup for Transition Logic when creating the Motion Design Scene")
		, TAttribute<FSlateIcon>()
		, FUIAction(FExecuteAction::CreateLambda(ToggleCreateTLDefaultScene)
			, FCanExecuteAction()
			, FIsActionChecked::CreateLambda(ShouldCreateTLDefaultScene))

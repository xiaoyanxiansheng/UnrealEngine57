...
namespace UE::AvaEditor::Private
{
	...
	ILevelSequenceEditorToolkit* OpenSequenceEditor(ULevelSequence* InSequence)
	{
		...
		IAssetEditorInstance* AssetEditor = AssetEditorSubsystem->FindEditorForAsset(InSequence, bFocusIfOpen);

		// Return early if it's already opened
		if (AssetEditor)
		{
		...
		AssetEditor = AssetEditorSubsystem->FindEditorForAsset(InSequence, bFocusIfOpen);

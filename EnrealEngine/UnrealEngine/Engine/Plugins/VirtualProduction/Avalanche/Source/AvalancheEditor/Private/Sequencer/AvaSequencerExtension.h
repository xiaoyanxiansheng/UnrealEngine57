...
private:
	...
	/** Creates the default sequences gotten from the default preset group to use */
	void ConditionallyCreateDefaultSequences();
	...
	/**
	 * Flag to indicate whether to create the default sequences when activating
	 * Set to true when the scene object is first created, but before the editor itself is activated.
	 */
	bool bCreateDefaultSequences = false;
};

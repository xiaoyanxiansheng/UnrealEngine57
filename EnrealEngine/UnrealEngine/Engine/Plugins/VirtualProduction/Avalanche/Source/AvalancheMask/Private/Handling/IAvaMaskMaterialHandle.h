	...
	, float InInnerFeathering)
{
	...
	{
		UE_LOG(LogAvaMask, Verbose, TEXT("SetParameters: Texture:%s, Channel:%s, Material:%s"), InTexture ? *InTexture->GetName() : TEXT("(None)"), *UE::AvaMask::Internal::MaskChannelEnumToVector[InChannel].ToString(), *MaterialInstance->GetName());

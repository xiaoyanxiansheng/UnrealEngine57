		...
		[
			...
			[
				...
				[
					...
					.OnGetMenuContent(this, &SAvaSequenceTree::CreateAddSequenceMenu)
...
void SAvaSequenceTree::BindCommands(const TSharedRef<FAvaSequencer>& InSequencer)
{
	...
	CommandList->MapAction(AvaSequencerCommands.AddNew
		, FExecuteAction::CreateSP(InSequencer, &FAvaSequencer::AddSequence_Execute)
		, FCanExecuteAction::CreateSP(InSequencer, &FAvaSequencer::AddSequence_CanExecute));
...
void SAvaSequenceTree::CreateAddSequencePresetMenu(UToolMenu* InMenu)
{
	const UAvaSequencerMenuContext* const MenuContext = InMenu->FindContext<UAvaSequencerMenuContext>();
	if (!MenuContext)
	{
		return;
	}

	const TSharedPtr<FAvaSequencer> AvaSequencer = MenuContext->GetAvaSequencer();
	if (!AvaSequencer.IsValid())
	{
		return;
	}
	...
	auto AddGroup = [AvaSequenceRef = AvaSequencer.ToSharedRef()](FToolMenuSection& InSection, const FAvaSequencePresetGroup& InGroup)
		{
			FUIAction Action;
			Action.ExecuteAction.BindSP(AvaSequenceRef, &FAvaSequencer::AddSequencesFromPresetGroup, InGroup.GroupName);
			Action.CanExecuteAction.BindSP(AvaSequenceRef, &FAvaSequencer::CanAddSequence);

			InSection.AddMenuEntry(InGroup.GroupName
				, FText::FromName(InGroup.GroupName)
				, FText::FromName(InGroup.GroupName)
				, FSlateIcon()
				, FToolUIActionChoice(Action));
		};

	// Transition Logic Groups
	FToolMenuSection& TransitionLogicSection = InMenu->FindOrAddSection(TEXT("TransitionLogicGroups"));
	TransitionLogicSection.Label = LOCTEXT("TransitionLogicGroupsLabel", "Transition Logic");
	for (const FAvaSequencePresetGroup& Group : SequencerSettings->GetTransitionLogicPresetGroups())
	{
		AddGroup(TransitionLogicSection, Group);
	}

	// Custom Groups
	if (!SequencerSettings->GetCustomPresetGroups().IsEmpty())
	{
		FToolMenuSection& CustomSection = InMenu->FindOrAddSection(TEXT("CustomGroups"));
		CustomSection.Label = LOCTEXT("CustomGroupsLabel", "Custom");
		for (const FAvaSequencePresetGroup& Group : SequencerSettings->GetCustomPresetGroups())
		{
			AddGroup(CustomSection, Group);
		}
	}
}

TSharedRef<SWidget> SAvaSequenceTree::CreateAddSequenceMenu() const
{
	UToolMenus* ToolMenus = UToolMenus::Get();
	if (!ToolMenus)
	{
	...
	constexpr const TCHAR* ContextMenuName = TEXT("SAvaSequenceAddNewContextMenu");
	...
	if (!ToolMenus->IsMenuRegistered(ContextMenuName))
	{
		...
		Section.AddMenuEntry(FAvaSequencerCommands::Get().AddNew
			, LOCTEXT("AddSequenceLabel", "New sequence")
			, LOCTEXT("AddSequenceTooltip", "Adds a new Motion Design sequence.")
			, FSlateIcon(FAppStyle::GetAppStyleSetName(), TEXT("Icons.Plus")));

		ToolMenu->AddDynamicSection(TEXT("SequencePresets"), FNewToolMenuDelegate::CreateStatic(&SAvaSequenceTree::CreateAddSequencePresetMenu));
	}

	UAvaSequencerMenuContext* MenuContext = NewObject<UAvaSequencerMenuContext>();
	MenuContext->SetAvaSequencer(SequencerWeak);

	FToolMenuContext ToolMenuContext(CommandList, /*Extender*/nullptr, MenuContext);	
	return ToolMenus->GenerateWidget(ContextMenuName, ToolMenuContext);
}

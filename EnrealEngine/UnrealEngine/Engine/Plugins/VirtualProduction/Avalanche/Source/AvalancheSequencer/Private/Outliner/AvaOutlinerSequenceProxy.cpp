...
void FAvaOutlinerSequenceProxy::BindDelegates()
{
	...
	if (TSharedPtr<ISequencer> Sequencer = AvaSequencer->GetSequencerPtr())
	{
		...
		LastBoundSequencerWeak = Sequencer;
	}
}

void FAvaOutlinerSequenceProxy::UnbindDelegates()
{
	if (TSharedPtr<ISequencer> Sequencer = LastBoundSequencerWeak.Pin())
	{

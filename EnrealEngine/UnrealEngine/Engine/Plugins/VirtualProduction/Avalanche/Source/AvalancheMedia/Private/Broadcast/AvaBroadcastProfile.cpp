...
void FAvaBroadcastProfile::SetProfileName(FName InProfileName)
{
	if (ProfileName != InProfileName)
	{
		ProfileName = InProfileName;
		for (FAvaBroadcastOutputChannel& Channel : Channels)
		{
			Channel.SetProfileName(InProfileName);
		}
	}
}
...
void FAvaBroadcastProfile::CopyProfiles(const FAvaBroadcastProfile& InSourceProfile, FAvaBroadcastProfile& OutTargetProfile)
{
	...
	{
		FAvaBroadcastOutputChannel& TargetChannel = OutTargetProfile.Channels.Add_GetRef(FAvaBroadcastOutputChannel(OutTargetProfile.GetName(), &OutTargetProfile.GetBroadcast()));
...
void FAvaBroadcastProfile::PostLoadProfile(bool bInIsProfileActive, UAvaBroadcast* InBroadcast)
{
	...
	for (FAvaBroadcastOutputChannel& Channel : Channels)
	{
		...
		Channel.PostLoadMediaOutputs(bInIsProfileActive, ProfileName, InBroadcast);
	}
...
FAvaBroadcastOutputChannel& FAvaBroadcastProfile::AddChannel(FName InChannelName)
{
	...
	FAvaBroadcastOutputChannel& Channel = Channels.Add_GetRef(FAvaBroadcastOutputChannel(ProfileName, &GetBroadcast()));

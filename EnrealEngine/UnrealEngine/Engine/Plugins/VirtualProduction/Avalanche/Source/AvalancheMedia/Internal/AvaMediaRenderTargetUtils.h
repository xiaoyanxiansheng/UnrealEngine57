...
namespace UE::AvaMediaRenderTargetUtils
{
	 ...
	 * This version of the function doesn't ensure if the render target is not valid.
	 */ 
	AVALANCHEMEDIA_API FIntPoint GetRenderTargetSizeSafe(const UTextureRenderTarget2D* InRenderTarget);
	
	/** 
	 * Returns the size of the render target.
	 * An ensure will be fired if the render target is not valid.
	 */ 

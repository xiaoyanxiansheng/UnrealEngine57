...
int32 FAvaPlaybackGraphBuilder::GetPinIndexForChannel(const FString& InChannelName) const
{
	...
	{
		// For the default channel key, we expect it to not have a broadcast channel, so we don't give the warning.
		if (InChannelName != UE::AvaMedia::Playback::DefaultPreviewChannelName.ToString())
		{
			UE_LOG(LogAvaPlayback, Warning

...
public:
	...
	/**
	 * Push an RC event on the given controllers for the given page.
	 * The propagation to channels in determined by rundown settings.
	 * 
	 * @param InControllerIds List of rc controller ids that should be triggered.
	 * @param InPageId Page id these events apply to.
	 * @param bInPushToProgram if true, the command will be pushed to page's program channel (if currently playing).
	 * @param bInPushToPreview if true, the command will be pushed to preview channel(s).
	 * @param InPreviewChannelName Allow to specify a specific preview channel to push to. If left none, will push to all preview channel this page is playing in.
	 * @return true if the event command was pushed to the corresponding playback instance.
	 */
	bool PushRuntimeRemoteControlEvents(TConstArrayView<FGuid> InControllerIds, int32 InPageId, bool bInPushToProgram, bool bInPushToPreview, FName InPreviewChannelName = NAME_None);

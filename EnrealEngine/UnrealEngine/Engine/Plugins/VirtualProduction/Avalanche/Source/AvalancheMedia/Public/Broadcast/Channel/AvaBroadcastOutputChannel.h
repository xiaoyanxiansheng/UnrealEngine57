...
public:
	...
	UE_DEPRECATED(5.6, "Use constructor with profile name and broadcast.")
	explicit FAvaBroadcastOutputChannel(FAvaBroadcastProfile* InProfile);
	
	explicit FAvaBroadcastOutputChannel(FName InProfileName, UAvaBroadcast* InBroadcast);
...
public:
	...
	void SetProfileName(FName InProfileName);
	
	UE_DEPRECATED(5.6, "Use version with profile name and broadcast.")
	void PostLoadMediaOutputs(bool bInIsProfileActive, FAvaBroadcastProfile* InProfile);
	
	void PostLoadMediaOutputs(bool bInIsProfileActive, FName InProfileName, UAvaBroadcast* InBroadcast);
...
protected:
	...
	*/
	...
	/** Profile name owning this channel. */
	FName ProfileName;

	/** Broadcast object owning this channel. */

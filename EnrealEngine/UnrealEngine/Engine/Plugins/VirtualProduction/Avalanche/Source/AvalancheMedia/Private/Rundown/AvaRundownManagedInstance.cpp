...
namespace UE::AvaMedia::RundownManagedInstance::Private
{
	bool IsEditorTransacting()
	{
#if WITH_EDITOR
...
void FAvaRundownManagedInstance::SetCurrentRundownPageForEvents(UAvaRundown* InRundown, int32 InPageId)
{
	RundownForEvents = InRundown;
	ActivePageId = InPageId;
}
...
void FAvaRundownManagedInstance::RegisterManagedRemoteControlPresetDelegates(URemoteControlPreset* InManagedRemoteControlPreset)
{
	if (::IsValid(InManagedRemoteControlPreset))
	{
		InManagedRemoteControlPreset->OnControllerModified().AddRaw(this, &FAvaRundownManagedInstance::OnManagedRemoteControlControllerModified);
	}
	
}

void FAvaRundownManagedInstance::UnregisterManagedRemoteControlPresetDelegates(URemoteControlPreset* InManagedRemoteControlPreset) const
{
	if (::IsValid(InManagedRemoteControlPreset))
	{
		InManagedRemoteControlPreset->OnControllerModified().RemoveAll(this);
	}
}
...
void FAvaRundownManagedInstance::OnManagedRemoteControlControllerModified(URemoteControlPreset* InPreset, const TSet<FGuid>& InModifiedControllerIds)
{
	using namespace UE::AvaMedia::RundownManagedInstance::Private;
	// Note: Ignore changes from the RCP Transaction listener.
	if (!::IsValid(InPreset) || GetRemoteControlPreset() != InPreset || IsEditorTransacting())
	{
		return;
	}

	UAvaRundown* Rundown = RundownForEvents.Get();

	if (!Rundown || !Rundown->GetPage(ActivePageId).IsValidPage())
	{
		return;
	}
	
	TArray<FGuid, TInlineAllocator<1>> EventControllerIds;

	for (const FGuid& Id : InModifiedControllerIds)
	{
		if (URCVirtualPropertyBase* Controller = InPreset->GetController(Id))
		{
			if (FAvaPlayableRemoteControlValues::IsRuntimeEventController(Controller))
			{
				EventControllerIds.Add(Id);
			}
		}
	}

	if (!EventControllerIds.IsEmpty())
	{
		const bool bPushToProgram = UAvaMediaSettings::Get().bRundownPushControllerEventsToProgram;
		const bool bPushToPreview = UAvaMediaSettings::Get().bRundownPushControllerEventsToPreview;
		Rundown->PushRuntimeRemoteControlEvents(EventControllerIds, ActivePageId, bPushToProgram, bPushToPreview);
	}
}

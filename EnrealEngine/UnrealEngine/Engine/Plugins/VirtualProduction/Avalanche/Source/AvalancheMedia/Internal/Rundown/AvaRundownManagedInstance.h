...
public:
	...
	/** Set the current rundown page for routing controller events from this managed instance. */
	void SetCurrentRundownPageForEvents(UAvaRundown* InRundown, int32 InPageId);

	/** Returns the rundown currently set to receive the RC Controller events from this managed instance. */
	UAvaRundown* GetRundownForEvents() const
	{
		return RundownForEvents.Get();
	}

	/** Returns the rundown page id that is currently set to receive RC Controller events from this managed instance. */
	int32 GetActivePageIdForEvents() const
	{
		return ActivePageId;
	}
...
protected:
	...
	void RegisterManagedRemoteControlPresetDelegates(URemoteControlPreset* InManagedRemoteControlPreset);
	void UnregisterManagedRemoteControlPresetDelegates(URemoteControlPreset* InManagedRemoteControlPreset) const;
	...
	void OnManagedRemoteControlControllerModified(URemoteControlPreset* InPreset, const TSet<FGuid>& InModifiedControllerIds);
...
protected:
	...
	/** Current rundown for routing controller events. */
	TWeakObjectPtr<UAvaRundown> RundownForEvents;

	/** Current active page for routing controller events. */
	int32 ActivePageId;
};
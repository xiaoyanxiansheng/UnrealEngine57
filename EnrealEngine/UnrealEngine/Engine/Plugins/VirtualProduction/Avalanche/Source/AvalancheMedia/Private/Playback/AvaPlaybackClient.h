...
protected:
	...
	void HandlePlaybackTransitionEventMultiMessage(const FAvaPlaybackTransitionEventMulti& InMessage, const TSharedRef<IMessageContext, ESPMode::ThreadSafe>& InContext);
...
private:
	...
	struct FPlaybackAssetInfo
	{
		...
		void RemoveInstanceInfo(const FGuid& InInstanceId)
		{
			InstanceByIds.Remove(InInstanceId);
		}
	...
	struct FPlaybackChannelInfo
	{
		...
		void RemoveInstanceInfo(const FGuid& InInstanceId, const FSoftObjectPath& InAssetPath)
		{
			if (FPlaybackAssetInfo* AssetInfo = GetAssetInfo(InAssetPath))
			{
				AssetInfo->RemoveInstanceInfo(InInstanceId);
			}
		}
	...
	public:
		...
		FPlaybackChannelInfo* GetPlaybackChannelInfo(const FString& InChannelName)
		{
			const TUniquePtr<FPlaybackChannelInfo>* ChannelInfo = PlaybackChannelInfosByName.Find(InChannelName);
			return ChannelInfo ? (*ChannelInfo).Get() : nullptr;
		}
		...
		void RemoveInstanceInfo(const FString& InChannelName, const FGuid& InInstanceId, const FSoftObjectPath& InAssetPath)
		{
			if (FPlaybackChannelInfo* ChannelInfo = GetPlaybackChannelInfo(InChannelName))
			{
				ChannelInfo->RemoveInstanceInfo(InInstanceId, InAssetPath);
			}
		}

...
namespace UE::AvaMedia::BroadcastChannel
{
	...
	{
		using namespace UE::AvaMediaRenderTargetUtils;
		if (!InOutRenderTarget)
		{
			InOutRenderTarget = CreateDefaultRenderTarget(InBaseName);
		}

		UpdateRenderTarget(InOutRenderTarget.Get(), InSize, InFormat, InClearColor);
	}
}
...
void FAvaBroadcastOutputChannel::UpdatePlaceholderRenderTargets()
{
	...
	using namespace UE::AvaMediaRenderTargetUtils;
	const FIntPoint PreviousRenderTargetSize = GetRenderTargetSizeSafe(PlaceholderRenderTarget);
	
	using namespace UE::AvaMedia::BroadcastChannel;
	...
	// Broadcast the render target resize for editor to resize preview brushes.
	if (PreviousRenderTargetSize != GetRenderTargetSizeSafe(PlaceholderRenderTarget))
	{
...
void FAvaBroadcastOutputChannel::UpdateViewportTarget()
{
	...
	if (GetState() == EAvaBroadcastChannelState::Live)
	{
		...
		for (UMediaOutput* const MediaOutput : MediaOutputs)
		{
			...
			UMediaCapture* const MediaCapture = AvaMediaCapture ? AvaMediaCapture->MediaCapture.Get() : nullptr;

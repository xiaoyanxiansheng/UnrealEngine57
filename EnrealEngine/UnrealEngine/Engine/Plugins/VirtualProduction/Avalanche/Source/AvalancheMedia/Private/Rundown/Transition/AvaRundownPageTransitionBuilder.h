...
public:
	...
	/**
	 * @brief Finds a suitable page transition object for the channel.
	 * @param InChannel channel we are requesting the transition object for.
	 * @return the requested page transition object or null if not found.
	 */
	UAvaRundownPageTransition* FindTransition(FName InChannelName) const;
	...
	/**
	 * @brief Retrieves a suitable page transition object for the given channel.
	 * @param InChannel Channel we are requesting the transition object for.
	 * @return the requested page transition object.
	 *
	 * This is where the batching occurs according to criteria.
	 */
	UAvaRundownPageTransition* FindOrAddTransition(FName InChannelName);
...
private:
	UAvaRundownPageTransition* AddTransition(FName InChannelName);

...
void FAvaRundownManagedInstanceCache::Flush()
{
	...
	// Dump post-flush remaining assets to help diagnose which editor keeps the asset alive.
	// Since the cache is shared with all editors (currently), assets can still remain if used by other playlist editors/servers.
	if (UE_LOG_ACTIVE(LogAvaRundown, Verbose))
	{
		if (Instances.Num() > 0)
		{
			UE_LOG(LogAvaRundown, Verbose, TEXT("Managed Instance Cache: Assets remaining in cache after flush:"));
			for (TPair<FSoftObjectPath, TSharedPtr<FAvaRundownManagedInstance>>& Entry : Instances)
			{
				UE_LOG(LogAvaRundown, Verbose, TEXT("- Asset: \"%s\" RefCount: %d"), *Entry.Key.ToString(), Entry.Value.GetSharedReferenceCount());
			}
		}
		else
		{
			UE_LOG(LogAvaRundown, Verbose, TEXT("Managed Instance Cache: No Assets remaining in cache after flush."));
		}
	}
}
...
TArray<TSharedPtr<FAvaRundownManagedInstance>> FAvaRundownManagedInstanceCache::GetManagedInstancesForPage(const UAvaRundown* InRundown, const FAvaRundownPage& InPage)
{
	const TArray<FSoftObjectPath> AssetPaths = InPage.GetAssetPaths(InRundown);
	
	TArray<TSharedPtr<FAvaRundownManagedInstance>> OutInstances;
	OutInstances.Reserve(AssetPaths.Num());
	
	for (const FSoftObjectPath& AssetPath : AssetPaths)
	{		
		if (TSharedPtr<FAvaRundownManagedInstance> Instance = GetOrLoadInstance(AssetPath))
		{
...
FAvaRundownManagedInstanceHandles FAvaRundownManagedInstanceCache::GetManagedHandlesForPage(UAvaRundown* InRundown, const FAvaRundownPage& InPage)
{
	return FAvaRundownManagedInstanceHandles(GetManagedInstancesForPage(InRundown, InPage), InRundown, InPage.GetPageId());
}

...
public:
	...
	virtual FText GetObjectNames(const UAvaRundown* InRundown) const override;
	virtual void OnObjectChanged(const FAssetData& InAssetData) override;

	virtual bool HasCommands(const UAvaRundown* InRundown) const override;
	...
	virtual FText GetPreviewInTooltip() const override;

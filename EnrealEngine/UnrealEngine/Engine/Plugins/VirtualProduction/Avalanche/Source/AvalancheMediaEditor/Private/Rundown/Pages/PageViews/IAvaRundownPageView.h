...
public:
	...
	/** Returns true if the page is a template, and that template is a combo. */
	virtual bool IsComboTemplate() const = 0;
	...
	/** Returns asset name if single asset template. */
	virtual FText GetObjectName(const UAvaRundown* InRundown) const = 0;
	/** Returns list of all assets: single, combo or commands. */
	virtual FText GetObjectNames(const UAvaRundown* InRundown) const = 0;
	virtual void OnObjectChanged(const FAssetData& InAssetData) = 0;

	/** Returns true if the page has commands, either directly or from it's templates. */
	virtual bool HasCommands(const UAvaRundown* InRundown) const = 0;
	...
	virtual FText GetPreviewInTooltip() const = 0;

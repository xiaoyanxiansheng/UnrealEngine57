...
void SAvaRundownPageList::Construct(const FArguments& InArgs, TSharedPtr<FAvaRundownEditor> InRundownEditor, const FAvaRundownPageListReference& InPageListReference)
{
	...
	OnActiveTabChangedDelegateHandle = FGlobalTabmanager::Get()->OnActiveTabChanged_Subscribe(FOnActiveTabChanged::FDelegate::CreateSP(this, &SAvaRundownPageList::OnActiveTabChanged));
	...
	const UAvaRundown* const Rundown = InRundownEditor->GetRundown();
	check(Rundown);
...
SAvaRundownPageList::~SAvaRundownPageList()
{
	...
	OnActiveTabChangedDelegateHandle.Reset();
...
void SAvaRundownPageList::OnActiveTabChanged(TSharedPtr<SDockTab> InPreviouslyActive, TSharedPtr<SDockTab> InNewlyActivated)
{
	if (const TSharedPtr<FAvaRundownEditor> RundownEditor = RundownEditorWeak.Pin())
	{
		if (InNewlyActivated && RundownEditor->GetAssociatedTabManager() == InNewlyActivated->GetTabManagerPtr())
		{
			const TSharedPtr<SWidget> Content = InNewlyActivated->GetContent();
			if (Content.Get() == this)
			{
				// The set of selected pages of this page list becomes the current set edited. Relevant for page detail.
				RundownEditor->GetOnPageEvent().Broadcast(SelectedPageIds, UE::AvaRundown::EPageEvent::SelectionChanged);
			}
		}
	}

...
bool FAvaVisualizerBase::IsMouseOverComponent(const UActorComponent* Component, const FSceneView* InView) const
{
	...
	// Cancel scale out since it's not taken into account in ActorSizeBox
	FTransform ComponentTransform = GetComponentTransform(Component);
	ComponentTransform.SetScale3D(FVector::OneVector);
	const FOrientedBox ActorOrientedBox = FAvaActorUtils::MakeOrientedBox(ActorSizeBox, ComponentTransform);

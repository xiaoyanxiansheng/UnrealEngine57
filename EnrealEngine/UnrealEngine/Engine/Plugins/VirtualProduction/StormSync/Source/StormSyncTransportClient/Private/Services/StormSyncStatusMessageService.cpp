...
{
	FMessageAddress Sender = MessageContext->GetSender();

	UE_LOG(LogStormSyncClient, Display,
		TEXT("FStormSyncStatusMessageService::HandleStatusRequest - Message: %s, Sender: %s"), *InMessage.ToString(), *Sender.ToString());

	const TWeakPtr<FMessageEndpoint> MessageEndpointWeak = GetMessageEndpoint();

	Async(EAsyncExecution::Thread, [Sender, MessageEndpointWeak, MessageId = InMessage.MessageId, PackageNames = InMessage.PackageNames, Dependencies = InMessage.Dependencies]()
	{
		TUniquePtr<FStormSyncTransportStatusResponse> Message(FMessageEndpoint::MakeMessage<FStormSyncTransportStatusResponse>(MessageId));

		UE_LOG(LogStormSyncClient, Display, TEXT("GetSyncFileModifiers for Packages %s"),
			*FString::JoinBy(PackageNames, TEXT(" ,"), [](const FName& InName){return InName.ToString();}));
	
		// Compute the diff
		Message->Modifiers = FStormSyncCoreUtils::GetSyncFileModifiers(PackageNames, Dependencies);
		...
		const TSharedPtr<FMessageEndpoint, ESPMode::ThreadSafe> MessageEndpoint = MessageEndpointWeak.Pin();
		if (!MessageEndpoint.IsValid())
		{
			UE_LOG(LogStormSyncClient, Error, TEXT("FStormSyncStatusMessageService::HandleStatusRequest - Unable to get client endpoint"));
			return;
		}
		
		// Send back result
		MessageEndpoint->Send(Message.Release(), Sender);
	});
}

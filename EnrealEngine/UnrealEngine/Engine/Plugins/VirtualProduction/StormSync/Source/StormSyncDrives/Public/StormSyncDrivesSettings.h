...
struct FStormSyncMountPointConfig
{
	...
	/** The path in MountDirectory should be interpreted as a relative path to the content directory of the active project. */
	UPROPERTY(EditAnywhere, Category = "Mounted Drives")
	bool bResolveAsRelativePath = false;
	
	FStormSyncMountPointConfig() = default;

	FStormSyncMountPointConfig(const FString& InMountPoint, const FDirectoryPath& InMountDirectory, bool bInResolveAsRelativePath = false)
		: MountPoint(InMountPoint)
		, MountDirectory(InMountDirectory)
		, bResolveAsRelativePath(bInResolveAsRelativePath)
	{
	}

	FStormSyncMountPointConfig(const FString& InMountPoint, const FString& InMountDirectory, bool bInResolveAsRelativePath = false)
		: MountPoint(InMountPoint)
		, bResolveAsRelativePath(bInResolveAsRelativePath)
	{

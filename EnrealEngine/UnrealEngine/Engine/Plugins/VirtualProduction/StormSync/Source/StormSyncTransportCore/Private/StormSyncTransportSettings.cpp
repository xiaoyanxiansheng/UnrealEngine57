...
{
	bool bReportUdpSettingError = false;
	FString ServerAddress = TcpServerAddress;
	if (!bOverrideServerAddress)
	{
		...
		{
			ServerAddress = UnicastHostname;
		}
		else
		{
			// Either unable to get the setting or malformed
			bReportUdpSettingError = true;
		}
	}
	
	// Take the command line overrides if they are used and valid (eg. `-StormSyncServerEndpoint=`)
	FString CommandLineOverride;
	if (UE::StormSync::Transport::Private::GetServerEndpointParam(CommandLineOverride, ServerAddress))
	{
		return CommandLineOverride;
	}

	// Report the previous error if we end up using the address from the settings.
	if (bReportUdpSettingError)
	{
		UE_LOG(LogStormSyncTransportCore, Display,
			TEXT("UStormSyncTransportSettings::GetServerEndpoint - Was unable to get UnicastEndpoint from UDPMessagingSettings, using \"%s\" instead.")
			, *ServerAddress);
	}

	return FString::Printf(TEXT("%s:%d"), *ServerAddress, TcpServerPort);
}

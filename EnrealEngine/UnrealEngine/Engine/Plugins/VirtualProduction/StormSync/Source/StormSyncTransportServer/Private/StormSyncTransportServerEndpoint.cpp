...
#include "StormSyncMemoryReader.h"
...
void FStormSyncTransportServerEndpoint::HandleReceivedTcpBuffer(const FIPv4Endpoint& InEndpoint, const TSharedPtr<FSocket>& InSocket, const FStormSyncBufferPtr& InBuffer)
{
	...
	UE_LOG(LogStormSyncServer, Display, TEXT("Handle received tcp buffer via %s:%d (Buffer Size: %lld)"), *InEndpoint.Address.ToString(), InEndpoint.Port, InBuffer->Num());
	...
	AsyncTask(ENamedThreads::GameThread, [InBuffer, Endpoint]()
	{
		...
		const FStormSyncArchivePtr MemoryReader = MakeShared<FStormSyncMemoryReader>(InBuffer);
		FStormSyncCoreDelegates::OnRequestImportBuffer.Broadcast(PackageDescriptor, MemoryReader);
	});
}

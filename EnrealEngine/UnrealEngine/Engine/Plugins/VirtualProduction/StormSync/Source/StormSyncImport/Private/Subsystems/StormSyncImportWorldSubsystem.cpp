...
void UStormSyncImportWorldSubsystem::HandleImportBuffer(const FStormSyncPackageDescriptor& InPackageDescriptor, const FStormSyncArchivePtr& InArchive) const
{
	...
	{
		UE_LOG(LogStormSyncImport, Error, TEXT("UStormSyncImportSubsystem::HandleImportBuffer failed from invalid archive"));
		return;
	}
	
	UE_LOG(LogStormSyncImport, 
		Verbose,
		TEXT("UStormSyncImportWorldSubsystem::HandleImportBuffer InPackageDescriptor: %s, BufferSize: %lld (World: %s)"),

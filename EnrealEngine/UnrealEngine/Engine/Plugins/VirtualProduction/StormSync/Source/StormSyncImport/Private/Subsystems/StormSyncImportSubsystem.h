...
public:
	 ...
	 * @return Operation status, true if successful
	 */
	static bool PerformBufferImport(const FStormSyncPackageDescriptor& InPackageDescriptor, const FStormSyncArchivePtr& InArchive);
	 ...
	 *
	 * @param InPackageDescriptor The package descriptor for this buffer extraction. Only used for UI message logs.
	 * @param InArchive Source archive to be parsed to extract content
	 ...
	 * @return Operation status, true if successful
	 */
	static bool PerformImport(const FStormSyncPackageDescriptor& InPackageDescriptor, const FStormSyncArchivePtr& InArchive, bool bShowWizard = false, bool bDryRun = false);

private:
	/**
	 * Parse the buffer to extract all the file information (but skip content)
	 * @param InArchive Source archive of the pak
	 * @param OutFileInfos File info in the pak.
	 * @return true if parsing was successful.
	 */
	static bool ParsePak(const FStormSyncArchivePtr& InArchive, TArray<FStormSyncImportFileInfo>& OutFileInfos);
...
private:
	 ...
	 */
	static bool ShouldFileBeImported(FStormSyncImportFileInfo& InFileInfo);
	...
	void HandleExistingAssetsPreExtract(TConstArrayView<const FStormSyncImportFileInfo*> InExistingFiles, bool bInShowPrompt);

	/** Event handler for auto-adding new assets post extracting them */
	bool HandleNewAssetsPostExtract(TConstArrayView<const FStormSyncImportFileInfo*> InNewFiles, bool bInShowPrompt);

	/** Event handler for when a file is about to be extracted from an incoming pak */
	bool HandlePakAssetPreExtract(const FStormSyncImportFileInfo& InFileInfo);

	/** Event handler for when a file has been extracted from an incoming pak */
	void HandlePakAssetPostExtract(const FStormSyncImportFileInfo& InFileInfo, bool bInFileWritten);

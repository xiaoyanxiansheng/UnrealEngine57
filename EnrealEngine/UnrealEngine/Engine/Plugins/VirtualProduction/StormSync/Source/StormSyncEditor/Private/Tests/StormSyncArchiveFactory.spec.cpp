...
void FStormSyncArchiveFactorySpec::Define()
{
	...
	Describe(TEXT("FactoryCreateFile"), [this]()
	{
		It(TEXT("Should queue up import next tick but raise error on invalid file"), [this]()
		{
			...
			UPackage* const Package = GetTransientPackage();

			bool bCanceled = false;
			const UObject* Result = Factory->FactoryCreateFile(UStormSyncArchiveData::StaticClass(), Package, NAME_None, RF_Public | RF_Standalone, Filename, nullptr, GWarn, bCanceled);

...
void FStormSyncAssetFolderContextMenu::GetSelectedFilesAndFolders(const UToolMenu* InMenu, TArray<FString>& OutSelectedPackagePaths, TArray<FString>& OutSelectedAssetPackages) const
{
	...
	for (const FContentBrowserItem& SelectedItem : ContextObject->SelectedItems)
	{
		...
		UContentBrowserDataSource* const DataSource = SelectedItemData->GetOwnerDataSource();
		if (!DataSource)
		{
			continue;
		}

		for (const FContentBrowserItemData& Item : SelectedItem.GetInternalItems())
		{
			FName PackagePath;
			FAssetData AssetData;
			if (DataSource->Legacy_TryGetPackagePath(Item, PackagePath))
			{
				OutSelectedPackagePaths.Add(PackagePath.ToString());
			}
			else if (DataSource->Legacy_TryGetAssetData(Item, AssetData))
			{
				OutSelectedAssetPackages.Add(AssetData.PackageName.ToString());
			}
		}
	}
}
...
void FStormSyncAssetFolderContextMenu::GetSelectedFiles(const UToolMenu* InMenu, TArray<FString>& OutSelectedAssetPackages) const
{
	...
	for (const FContentBrowserItem& SelectedItem : ContextObject->SelectedItems)
	{
		...
		{
			FAssetData AssetData;
			if (DataSource->Legacy_TryGetAssetData(Item, AssetData))
			{
				OutSelectedAssetPackages.Add(AssetData.PackageName.ToString());
			}
		}

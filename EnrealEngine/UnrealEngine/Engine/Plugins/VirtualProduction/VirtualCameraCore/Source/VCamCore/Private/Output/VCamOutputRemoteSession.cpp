...
void UVCamOutputRemoteSession::OnImageChannelCreated(TWeakPtr<IRemoteSessionChannel> Instance)
{
	...
	if (ImageChannel)
	{
		...
		if (FinalOutputRenderTarget)
		{
			MediaCapture->CaptureTextureRenderTarget2D(FinalOutputRenderTarget, Options);
			UE_LOG(LogVCamOutputProvider, Log, TEXT("ImageChannel callback - MediaCapture set with TextureRenderTarget2D"));
		}
		else
		{

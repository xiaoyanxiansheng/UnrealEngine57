		...
		for (const UInputMappingContext* MappingContext : AppliedInputContexts)
		{
			...
			for (const FEnhancedActionKeyMapping& Mapping : MappingContext->GetMappings())
			{
				if (Mapping.IsPlayerMappable())
				{
					...
					{
						if (const UEnhancedPlayerMappableKeyProfile* KeyProfile = Settings->GetActiveKeyProfile())
						{
...
void UVCamComponent::ApplyInputProfile()
{
	...
	for (const TPair<FName, FKey>& MappableKeyOverride : InputProfile.MappableKeyOverrides)
	{
		...
		const FPlayerKeyMapping* Mapping = KeyProfile->FindKeyMapping(Args);
		const bool bResetToDefault = Mapping && Mapping->GetDefaultKey() == NewKey;
		...
		UE_CLOG(!FailureReason.IsEmpty(), LogVCamComponent, Warning, TEXT("Failed to map %s. Possible Reasons: "
			"1. The input action is currently not registered: check whether your modifier has registered your input action."
			"2. Settings class is incorrectly specified for the input action. Go into the InputMappingContext, set SettingBehavior to OverrideSettings, specify a PlayerMappableKeySettings, and set the Name to the entry."),
	...
	// RebuildWithFlush because input actions are already mapped by modifiers.
	EnhancedInputSubsystemInterface->RequestRebuildControlMappings(Options, EInputMappingRebuildType::RebuildWithFlush);
}

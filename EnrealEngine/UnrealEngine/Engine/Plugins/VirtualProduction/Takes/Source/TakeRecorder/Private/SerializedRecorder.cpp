...
AActor* FSerializedRecorder::SetActorPossesableOrSpawnable(UMovieSceneSequence* InMovieSceneSequence, UWorld *PlaybackContext, const FActorFileHeader& ActorHeader)
{
	...
	{		
		...
		if (Actor)
		{
			...
			PossessableGuid = MovieScene->AddPossessable(ActorHeader.Label, InitPossessedObjectClass);
			InMovieSceneSequence->BindPossessableObject(PossessableGuid, *Actor, Actor->GetWorld());
		}
	}
	else
	{
		...
		PossessableGuid = MovieScene->AddPossessable(ActorHeader.Label, SpawnableClass);
		...
		CreateBindingParams.ReplacementGuid = PossessableGuid;
	...
	ActorGuidToActorMap.Add(PossessableGuid, Actor);

	// Tag the possessable/spawnable with the original actor label so we can find it later
	if (FMovieScenePossessable* Possessable = MovieScene->FindPossessable(PossessableGuid))
	{
	...
	if (ActorHeader.FolderName.IsValid())
	{
		...
		FolderToUse->AddChildObjectBinding(PossessableGuid);
	}
	return Actor;
}
...
void FSerializedRecorder::SetComponentPossessable(UMovieSceneSequence* InMovieSceneSequence, UWorld *PlaybackContext, AActor* Actor, const FActorFileHeader& ActorHeader, const FActorProperty& ActorProperty)
{
	if (Actor)
	{
		...
		FGuid PossessableGuid = InMovieScene->AddPossessable(ActorProperty.UObjectName, InitPossessedObjectClass);

		// Set up parent/child guids for possessables within spawnables
		FMovieScenePossessable* ChildPossessablePtr = InMovieScene->FindPossessable(PossessableGuid);
		ChildPossessablePtr->SetParent(PossessableGuid, InMovieScene);

		for (UActorComponent* Component : Actor->GetComponents())
		{
			if (Component->GetName() == ActorProperty.UObjectName)
			{
				InMovieSceneSequence->BindPossessableObject(PossessableGuid, *Component, Actor);
				break;
			}
		}
	}

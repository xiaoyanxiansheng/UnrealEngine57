...
struct FTakeRecorderUserParameters
{
	...
	/**
	 * Automatically stop recording when the current time code value loops over to before the start timecode value,
...
struct FTakeRecorderProjectParameters
{
	...
	UPROPERTY(config, EditAnywhere, BlueprintReadWrite, Category = "Take Recorder", meta = (ContentDir, NamingTokens, ForceShowPluginContent))
...
struct FTakeRecorderSequenceParameters
{
	...
	UE_API bool operator==(const FTakeRecorderSequenceParameters& Other) const;
	UE_API bool operator!=(const FTakeRecorderSequenceParameters& Other) const;
};

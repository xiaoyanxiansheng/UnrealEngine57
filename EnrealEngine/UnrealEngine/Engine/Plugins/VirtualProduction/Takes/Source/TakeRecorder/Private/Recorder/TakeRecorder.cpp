...
bool UTakeRecorder::SetupDestinationAsset(const FTakeRecorderParameters& InParameters, ULevelSequence* LevelSequenceBase, UTakeRecorderSources* Sources, UTakeMetaData* MetaData, FText* OutError, bool bOpenSequencer)
{
	...
	SequenceAsset = WeakSequencer.IsValid() ? Cast<ULevelSequence>(WeakSequencer.Pin()->GetFocusedMovieSceneSequence())
		: LevelSequenceBase;
	check(SequenceAsset);
...
FQualifiedFrameTime UTakeRecorder::GetRecordTime() const
{
	...
	if (!WeakSequencer.IsValid())
	{
		...
		if (const AWorldSettings* WorldSettings = RecordingWorld->GetWorldSettings())
		{
			// If we're not using Sequencer we have to apply time dilation ourselves as this normally happens in Sequencer.cpp
			// when the TimeController is ticked.
			TimeDilation = WorldSettings->CinematicTimeDilation;
		}
	}
	
	return TakesUtils::GetRecordTime(WeakSequencer.Pin(), SequenceAsset, TimecodeAtStart,
		Parameters.Project.bStartAtCurrentTimecode, TimeDilation);
}

namespace UE::TakeRecorder
{
static bool operator<(const FTimecode& InLeft, const FTimecode& InRight)
{
	return InLeft.Hours < InRight.Hours
		|| (InLeft.Hours <= InRight.Hours && InLeft.Minutes < InRight.Minutes)
		|| (InLeft.Hours <= InRight.Hours && InLeft.Minutes <= InRight.Minutes && InLeft.Seconds < InRight.Seconds)
		|| (InLeft.Hours <= InRight.Hours && InLeft.Minutes <= InRight.Minutes && InLeft.Seconds <= InRight.Seconds
			&& InLeft.Frames < InRight.Frames)
		|| (InLeft.Hours <= InRight.Hours && InLeft.Minutes <= InRight.Minutes && InLeft.Seconds <= InRight.Seconds
			&& InLeft.Frames <= InRight.Frames && InLeft.Subframe < InRight.Subframe);
}

static void ShowRolloverNotification()
{
	if (FSlateApplication::IsInitialized())
	{
		FNotificationInfo Info(LOCTEXT("RolloverMessage.Title", "Timecode rolled over."));
		Info.SubText = LOCTEXT("RolloverMessage.Subtext", "Recording was stopped because timecode rolled over.");
...
void UTakeRecorder::InternalTick(float DeltaTime)
{
	...
	// We assume timecode providers do not change after recording is started.
	// If no timecode provider is set, then FApp::GetTimecode() == TimecodeAtStart == FTimecode(0,0,0,0).
	if (Parameters.User.bStopOnRollover && FApp::GetTimecode() < TimecodeAtStart)
	{
		ShowRolloverNotification();
		Stop();
		return;
	}
...
void UTakeRecorder::StopInternal(const bool bCancelled)
{
	...
	// Tear down the Sequencer instance before we delete the SequenceAsset.
	Sequencer.Reset();

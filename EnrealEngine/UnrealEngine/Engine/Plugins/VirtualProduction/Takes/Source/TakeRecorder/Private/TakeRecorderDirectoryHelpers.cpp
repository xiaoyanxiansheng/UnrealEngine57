...
FString ResolvePathToProject(const FString& InPath)
{
	...
	const FString FullProjectPath = DefaultProjectPath + FixedPathStr;
	const bool bFullProjectPathExists = FPlatformFileManager::Get().GetPlatformFile().DirectoryExists(*FullProjectPath);
	
	TTuple<FString, FString> Parts = SplitPackagePath(FixedPathStr);
	FString PossibleMountPoint = Parts.Get<0>() + TEXT("/");
	
	if (!bFullProjectPathExists && FPackageName::MountPointExists(PossibleMountPoint))
	{
		return InPath;
	}

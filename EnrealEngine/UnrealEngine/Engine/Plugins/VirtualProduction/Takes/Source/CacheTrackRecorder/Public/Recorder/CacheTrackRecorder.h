...
#define UE_API CACHETRACKRECORDER_API
...
struct FCacheRecorderUserParameters
{
	...
	UE_API FCacheRecorderUserParameters();
...
struct FCacheRecorderProjectParameters
{
	...
	UE_API FCacheRecorderProjectParameters();
...
public:
	 ...
	 */
	static UE_API UCacheTrackRecorder* GetActiveRecorder();

	/**
	 * Utility method to record a single cache track
	 */
	static UE_API void RecordCacheTrack(IMovieSceneCachedTrack* Track, TSharedPtr<ISequencer> Sequencer, FCacheRecorderParameters Parameters = FCacheRecorderParameters());
	
	/**
	 * Utility method to record several cache tracks at once
	 */
	static UE_API void RecordCacheTracks(const TArray<IMovieSceneCachedTrack*>& CacheTracks, TSharedPtr<ISequencer> Sequencer, FCacheRecorderParameters Parameters = FCacheRecorderParameters());

	/**
	 * Utility method to record all the cache tracks selected in sequencer
	 */
	static UE_API void RecordSelectedTracks(TSharedPtr<ISequencer> Sequencer, FCacheRecorderParameters Parameters = FCacheRecorderParameters());
	 ...
	 *
	 */
	UE_API bool Initialize(ULevelSequence* RootLevelSequence, const TArray<IMovieSceneCachedTrack*>& CacheTracks, const FString& Slate, const FCacheRecorderParameters& InParameters, FText* OutError = nullptr);
...
private:
	...
	UE_API void BackupEditorTickState();

	/* Sets the editor up to tick in a fixed time step */
	UE_API void ModifyEditorTickState();

	/* Restores the editor tick state if necessary */
	UE_API void RestoreEditorTickState();
	 ...
	 */
	static UE_API bool SetActiveRecorder(UCacheTrackRecorder* NewActiveRecorder);

...
namespace UE::TakeRecorderSources
{
void FTakeRecorderSourcesModule::StartupModule()
{
	...
	ITakeRecorderModule::FSourceExtensionData ExtensionData;
	ExtensionData.OnAddActorSource = ITakeRecorderModule::FSourceExtensionData::FAddActorSource::CreateStatic(TakeRecorderSourceHelpers::AddActorSources);
	ExtensionData.OnRemoveActorSource = ITakeRecorderModule::FSourceExtensionData::FRemoveActorSource::CreateStatic(TakeRecorderSourceHelpers::RemoveActorSources);
	ExtensionData.OnGetSourceActor = ITakeRecorderModule::FSourceExtensionData::FGetSourceActor::CreateStatic(TakeRecorderSourceHelpers::GetSourceActor);
	TakeRecorderModule.RegisterSourcesExtension(ExtensionData);
...
void FTakeRecorderSourcesModule::ShutdownModule()
{
	...
	if (TakeRecorderModule)
	{
		...
		TakeRecorderModule->UnregisterSourcesExtension();
	}
...
void FTakeRecorderSourcesModule::RegisterCanRecordDelegate(FName HandleId, FCanRecordDelegate InDelegate)
{
	CanRecordDelegates.Add(HandleId, MoveTemp(InDelegate));
}
void FTakeRecorderSourcesModule::UnregisterCanRecordDelegate(FName HandleId)
{
	CanRecordDelegates.Remove(HandleId);
}

bool FTakeRecorderSourcesModule::CanRecord(const FCanRecordArgs& InArgs)
{
	return Algo::AllOf(CanRecordDelegates, [&InArgs](const TPair<FName, FCanRecordDelegate>& Pair)
	{
		return Pair.Value.Execute(InArgs);
	});
}

void FTakeRecorderSourcesModule::RegisterMenuExtensions()
{
#if WITH_EDITOR
	if (GEditor)
	{
...
void FTakeRecorderSourcesModule::UnregisterMenuExtensions()
{
...
void FTakeRecorderSourcesModule::BindCommands()
{
...
TSharedRef<FExtender> FTakeRecorderSourcesModule::ExtendLevelViewportContextMenu(const TSharedRef<FUICommandList> InCommandList, const TArray<AActor*> SelectedActors)
{
...
void FTakeRecorderSourcesModule::ExtendSourcesMenu(TSharedRef<FExtender> Extender, UTakeRecorderSources* Sources)
{
...
void FTakeRecorderSourcesModule::PopulateSourcesMenu(FMenuBuilder& MenuBuilder, UTakeRecorderSources* Sources)
{
...
void FTakeRecorderSourcesModule::PopulateActorSubMenu(FMenuBuilder& MenuBuilder, UTakeRecorderSources* Sources)
{
...
bool FTakeRecorderSourcesModule::Exec_Editor(UWorld* InWorld, const TCHAR* Cmd, FOutputDevice& Ar)
{
#if WITH_EDITOR
...
bool FTakeRecorderSourcesModule::HandleRecordTakeCommand(UWorld* InWorld, const TCHAR* InStr, FOutputDevice& Ar)
{
#if WITH_EDITOR
...
bool FTakeRecorderSourcesModule::HandleStopRecordTakeCommand(UWorld* InWorld, const TCHAR* InStr, FOutputDevice& Ar)
{
#if WITH_EDITOR
...
bool FTakeRecorderSourcesModule::HandleCancelRecordTakeCommand(UWorld* InWorld, const TCHAR* InStr, FOutputDevice& Ar)
{
#if WITH_EDITOR
	if (UTakeRecorder* ActiveRecorder = UTakeRecorder::GetActiveRecorder())
	{
...
void FTakeRecorderSourcesModule::RecordActors(const TArray<AActor*>& ActorsToRecord, TOptional<ULevelSequence*> LevelSequence, TOptional<ULevelSequence*> RootLevelSequence)
{
...
void FTakeRecorderSourcesModule::RecordSelectedActors()
{
	TArray<AActor*> SelectedActors;
	GEditor->GetSelectedActors()->GetSelectedObjects(SelectedActors);
...
void FTakeRecorderSourcesModule::OnSequencerCreated(TSharedRef<ISequencer> Sequencer)
{

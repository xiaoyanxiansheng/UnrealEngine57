...
void UTakeRecorderMicrophoneAudioManager::StartRecording(int32 InChannelCount)
{
	...
	{
		UE_LOG(LogTakesCore, Error, TEXT("Microphone Audio Source will not start. Running in game with audio thread. Audio thread is not supported with take recording set UseAudioThread=false in DefaultEngine.ini to record audio in -game."));
		return;
	}

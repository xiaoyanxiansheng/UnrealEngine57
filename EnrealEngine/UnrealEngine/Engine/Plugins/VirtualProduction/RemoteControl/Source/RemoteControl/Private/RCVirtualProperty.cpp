...
template<typename T>
concept CHasMethodGetPropertyValueInContainer = requires(T && InObject, uint8 * InPtr)
{
	{
		InObject.GetPropertyValue_InContainer(InPtr)
	};
};
...
struct FRCVirtualPropertyCastHelpers
{
	...
	static bool GetPrimitiveValue(const URCVirtualPropertyBase* InVirtualProperty, TValueType& OutValue)
	{
		...
		{
			if constexpr (CHasMethodGetPropertyValueInContainer<TPropertyType>)
			{
			...
			else
			{
				FMemory::Memcpy(
					&OutValue,
					CastProperty->template ContainerPtrToValuePtr<void>(ContainerPtr),
					sizeof(OutValue)
				);
			}
...
bool URCVirtualPropertyBase::GetValueEnum(uint8& OutByte) const
{
	return FRCVirtualPropertyCastHelpers::GetPrimitiveValue<FEnumProperty, uint8&>(this, OutByte);
}
...
FString URCVirtualPropertyBase::GetDisplayValueAsString() const
{
	...
	{
		FString Value;
		StructProperty->ExportTextItem_Direct(
			Value,
			GetValuePtr(),
			/* Default value ptr */ nullptr,
			/* Parent */ nullptr,
			/* Port flags */ 0
		);

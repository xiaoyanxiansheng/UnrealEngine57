...
bool URemoteControlPreset::HasController(const URCVirtualPropertyBase* InController) const
{
	if (!ensure(ControllerContainer))
	{
		return false;
	...
	return ControllerContainer->HasVirtualProperty(InController);
}
...
void URemoteControlPreset::UpdateEntityUsage()
{
	TMap<FGuid, TSet<UObject*>> EntityToBehavior;

	if (ControllerContainer)
	{
		for (URCVirtualPropertyBase* VirtualProperty : ControllerContainer->VirtualProperties)
		{
			VirtualProperty->UpdateEntityUsage(this, EntityToBehavior);
		}
	}

	BoundEntityIds.Empty(EntityToBehavior.Num());

	for (const TPair<FGuid, TSet<UObject*>>& EntityPair : EntityToBehavior)
	{
		BoundEntityIds.Add(EntityPair.Key, EntityPair.Value.Num());
	}
}

bool URemoteControlPreset::IsEntityBound(const FGuid& InEntityGuid) const
{
	if (InEntityGuid.IsValid())
	{
		return BoundEntityIds.FindRef(InEntityGuid) > 0;
	}

	return false;
}

int32 URemoteControlPreset::GetEntityBoundCount(const FGuid& InEntityGuid) const
{
	if (InEntityGuid.IsValid())
	{
		return BoundEntityIds.FindRef(InEntityGuid);
	}

	return 0;
}

int32 URemoteControlPreset::GetNextControllerDisplayIndex() const
{
	int32 HighestDisplayIndex = INDEX_NONE;

	if (ControllerContainer)
	{
		for (const FRCVirtualPropertyCategory& Category : ControllerContainer->Categories)
		{
			HighestDisplayIndex = FMath::Max(HighestDisplayIndex, Category.DisplayIndex);
		}

		for (URCVirtualPropertyBase* Controller : ControllerContainer->VirtualProperties)
		{
			HighestDisplayIndex = FMath::Max(HighestDisplayIndex, Controller->DisplayIndex);
		}
	}

	return HighestDisplayIndex + 1;
}

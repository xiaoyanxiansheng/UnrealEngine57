...
public:
	...
	*/
	...
	/** Returns true if the preset contains this controller. */
	bool HasController(const URCVirtualPropertyBase* InController) const;
...
public:	
	...
	/** Updates the usage of entities by controllers. Specifically the number of controlling behaviors. */
	void UpdateEntityUsage();

	/** Returns a map of the number of times exposed entities are used by behaviors. */
	const TMap<FGuid, int32>& GetEntityUsage() const
	{
		return BoundEntityIds;
	}

	/** Returns true if an entity is bound to a controller/behavior. */
	bool IsEntityBound(const FGuid& InEntityGuid) const;

	/** Returns the number of behaviors an entity is bound to. */
	int32 GetEntityBoundCount(const FGuid& InEntityGuid) const;

	/** The display index to put it at the bottom of the list. */
	int32 GetNextControllerDisplayIndex() const;
...
private:
	...
	/** Transient list of the currently bound entity ids and the number of times they are bound. */
	TMap<FGuid, int32> BoundEntityIds;
...
public:
	...
	/**
	 * Transient value for the default category for new properties.
	 * Not automatically set by the preset. Used when adding controllers.
	 */
	FString NewControllerCategory;

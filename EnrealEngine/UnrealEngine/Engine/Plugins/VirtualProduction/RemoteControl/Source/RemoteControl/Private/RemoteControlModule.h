...
private:
	...
	/** Registers the local forwarding interceptor. */
	void RegisterLocalForwardingInterceptor();

	/** Unregisters the local forwarding interceptor. */
	void UnregisterLocalForwardingInterceptor();

	/** Unregisters the console objects from the console manager. */
	void UnregisterConsoleObjects();
...
private:
	...
	/**
	 * RC Local Forwarding interceptor instance.
	 * When this is enabled it can locally emulate interception and forwarding to the RCI processor.
	 */
	TUniquePtr<IRemoteControlInterceptionFeatureInterceptor> RCILocalForwardingInterceptor;
	...
	/** Keep track of the registered console objects for automatic unregistration on module shutdown. */

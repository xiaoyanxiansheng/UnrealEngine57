...
bool FRemoteControlProtocolEntity::ApplyProtocolValueToProperty(const double InProtocolValue)
{
	...
	for (UObject* Object : RemoteControlProperty->GetBoundObjects())
	{
		...
		if (OverridenMasks == static_cast<uint8>(ERCMask::NoMask))
		{
			MaskingOperation->Masks = RemoteControlProperty->GetActiveMasks();
		}
		else
		{
			MaskingOperation->Masks = static_cast<ERCMask>(OverridenMasks);
		}
...
{
	OverridenMasks &= ~static_cast<uint8>(InMaskBit);
}
...
{
	return (OverridenMasks & static_cast<uint8>(InMaskBit)) != static_cast<uint8>(ERCMask::NoMask);
}

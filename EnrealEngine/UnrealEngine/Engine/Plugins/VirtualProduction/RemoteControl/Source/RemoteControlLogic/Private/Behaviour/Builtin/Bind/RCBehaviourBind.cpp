...
bool URCBehaviourBind::CanHaveActionForField(URCController* Controller, TSharedRef<const FRemoteControlField> InRemoteControlField, const bool bInAllowNumericInputAsStrings)
{
	...
	// BP enum properties are bytes
	else if (const FByteProperty* ByteProperty = CastField<FByteProperty>(RemoteControlProperty))
	{
...
bool URCBehaviourBind::GetPropertyBagTypeFromFieldProperty(const FProperty* InProperty, EPropertyBagPropertyType& OutPropertyBagType, UObject*& OutStructObject)
{
	const static TMap<EPropertyBagPropertyType, TArray<FFieldClass*>> PropertyBagTypesMap =
	{
		...
		{ EPropertyBagPropertyType::Int32,   /* --> */    { FIntProperty::StaticClass(),      FInt64Property::StaticClass(),    FInt16Property::StaticClass(),   FUInt32Property::StaticClass(),
														   FUInt64Property::StaticClass(),   FUInt16Property::StaticClass(),    FByteProperty::StaticClass() } },
		...
		{ EPropertyBagPropertyType::Enum,    /* --> */    { FEnumProperty::StaticClass() } },
	...
	// Handle getting the enum from enum properties
	else if (const FEnumProperty* EnumProperty = CastField<FEnumProperty>(InProperty))
	{
		OutStructObject = EnumProperty->GetEnum();
	}
	// Handle BP-based enums that are stored as byte propertties
	else if (const FByteProperty* ByteProperty = CastField<FByteProperty>(InProperty))
	{
		if (ByteProperty->Enum)
		{
			OutPropertyBagType = EPropertyBagPropertyType::Enum;
			OutStructObject = ByteProperty->Enum;
		}
	}

...
void URCAction::UpdateEntityUsage(const URCBehaviour* InBehavior, TMap<FGuid, TSet<UObject*>>& InEntityToBehavior) const
{
	URCBehaviour* Behavior = GetParentBehaviour();

	if (!Behavior || Behavior != InBehavior || !InBehavior->HasAction(this))
	{
		return;
	}
	...
	InEntityToBehavior.FindOrAdd(ExposedFieldId).Add(Behavior);
}

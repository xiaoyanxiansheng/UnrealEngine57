...
public:
	...
	virtual void InitRHI(FRHICommandListBase& RHICmdList) override
	{
		...
		{
			UpdateRHIFromCacheEntry(RHICmdList);
			return;
		}
	...
	void SetCacheEntry(FRHICommandListBase& RHICmdList, const FString& InNewPath, const TSharedPtr<FResourceCacheEntry>& InNewEntry)
	{
		ReleaseRHI();
	...
	void UpdateRHIFromCacheEntry(FRHICommandListBase& RHICmdList)
	{
		if (CacheEntry)
		{
			TextureRHI = CacheEntry->TextureRHI;
			SamplerStateRHI = CacheEntry->SamplerStateRHI;
			RHICmdList.UpdateTextureReference(RCTextureOwner->TextureReference.TextureReferenceRHI,TextureRHI);
		}
	}
	...
	if (const TSharedPtr<FResourceCacheEntry> ExistingEntry = FResourceCache::Get()->Find(InPath))
	{
		...
		if (FRCExternalTextureResource* TextureResource = static_cast<FRCExternalTextureResource*>(GetResource()))
		{
			ENQUEUE_RENDER_COMMAND(FUpdateSharedTextureEntry)(
				...
				{
					TextureResource->SetCacheEntry(RHICmdList, NewPath, ExistingEntry);
				});
		}
		return;
	}

...
TFunction<bool(const TPair<EPropertyBagPropertyType, UObject*>)> URCBehaviourNode::GetIsSupportedCallback(URCBehaviour* InBehaviour)
{
	return[InBehaviour](const TPair<EPropertyBagPropertyType, UObject*> InPair)
	{
		...
		{
			EPropertyBagPropertyType ValueType = RCController->GetValueType();

			if (ValueType == EPropertyBagPropertyType::Enum && InPair.Key == EPropertyBagPropertyType::Enum)
			{
				return true;
			}

			return ValueType == InPair.Key && RCController->GetValueTypeObjectWeakPtr() == InPair.Value;
		}

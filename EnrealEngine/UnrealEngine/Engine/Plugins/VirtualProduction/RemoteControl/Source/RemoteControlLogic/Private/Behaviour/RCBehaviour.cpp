...
bool URCBehaviour::HasAction(const URCAction* InAction) const
{
	return InAction && ActionContainer->GetActions().Contains(InAction);
}
...
void URCBehaviour::UpdateEntityUsage(const URCController* InController, TMap<FGuid, TSet<UObject*>>& InEntityToBehavior) const
{
	URCController* Controller = ControllerWeakPtr.Get();

	if (!Controller || Controller != InController || !Controller->HasBehavior(this))
	{
		return;
	}
	...
	if (ActionContainer)
	{
		ActionContainer->UpdateEntityUsage(this, InEntityToBehavior);
	}	
}

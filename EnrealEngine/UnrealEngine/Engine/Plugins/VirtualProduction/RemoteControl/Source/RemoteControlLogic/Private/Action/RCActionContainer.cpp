...
void URCActionContainer::UpdateEntityUsage(const URCBehaviour* InBehavior, TMap<FGuid, TSet<UObject*>>& InEntityToBehavior) const
{
	for (URCActionContainer* ActionContainer : ActionContainers)
	{
		ActionContainer->UpdateEntityUsage(InBehavior, InEntityToBehavior);
	}

	for (URCAction* Action : Actions)
	{
		Action->UpdateEntityUsage(InBehavior, InEntityToBehavior);
	}
}

...
TSharedRef<SWidget> UE::RCUIHelpers::GetTypeColorWidget(const FProperty* InProperty)
{
	...
	return GetTypeColorWidget(TypeColor, TooltipText);
}

TSharedRef<SWidget> UE::RCUIHelpers::GetTypeColorWidget(const FLinearColor& InTypeColor, const FText& InTypeDescription)
{
	return SNew(SBox)
		.HeightOverride(5.f)
		.HAlign(HAlign_Left)
		.Padding(FMargin(5.f, 0.f))
		.ToolTipText(InTypeDescription)
		[
			SNew(SBorder)
				.Visibility(EVisibility::Visible)
				.BorderImage(FAppStyle::Get().GetBrush("NumericEntrySpinBox.Decorator"))
				.BorderBackgroundColor(InTypeColor)
...
URCController* UE::RCUIHelpers::CreateControllerFromEntity(URemoteControlPreset* InPreset, const TSharedPtr<const FRemoteControlProperty>& InRemoteControlProperty)
{
	...
	NewController->DisplayIndex = InPreset->GetNextControllerDisplayIndex();
	NewController->Description = GenerateControllerDescriptionFromEntity(InRemoteControlProperty);

	if (!InPreset->NewControllerCategory.IsEmpty())
	{
		using namespace UE::RemoteControl::UI::Private;
		NewController->SetMetadataValue(FRCControllerPropertyInfo::CategoryName, InPreset->NewControllerCategory);
	}

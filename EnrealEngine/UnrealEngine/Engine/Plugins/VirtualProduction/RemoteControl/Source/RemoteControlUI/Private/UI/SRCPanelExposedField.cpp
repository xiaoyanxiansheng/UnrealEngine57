...
TSharedRef<SWidget> SRCPanelExposedField::GetWidget(const FName ForColumnName, const FName InActiveProtocol)
{
	...
	if (ForColumnName == RemoteControlPresetColumns::Reset)
	{
		return SNew(SHorizontalBox)
			...
			[
				SRCPanelTreeNode::GetWidget(ForColumnName, InActiveProtocol)
			]
			+ SHorizontalBox::Slot()
			.AutoWidth()
			.VAlign(VAlign_Center)
		...
		if (TSharedPtr<FRemoteControlFunction> Function = RCPreset->GetExposedEntity<FRemoteControlFunction>(EntityId).Pin())
		{
			for (UObject* Object : Function->GetBoundObjects())
			{
				if (Function->FunctionArguments && Function->FunctionArguments->IsValid())
				{
					...
					CallRef.Function = Function->GetFunction();

					FRCCall Call;
					Call.CallRef = MoveTemp(CallRef);
					Call.TransactionMode = ERCTransactionMode::AUTOMATIC;
					Call.ParamStruct = FStructOnScope(Function->FunctionArguments->GetStruct(), Function->FunctionArguments->GetStructMemory());
					...
					FStructSerializer::Serialize(Function->FunctionArguments->GetStructMemory(), *const_cast<UStruct*>(Function->FunctionArguments->GetStruct()), SerializerBackend);
...
TSharedRef<SWidget> SRCPanelExposedField::CreateInUseWidget()
{
	if (const TSharedPtr<FRemoteControlField> RCField = WeakField.Pin())
	{
		return SNew(SImage)
			...
			.Image_Lambda(
				[ThisWeak = AsWeak()]
				{
					if (TSharedPtr<SRCPanelExposedField> This = StaticCastSharedPtr<SRCPanelExposedField>(ThisWeak.Pin()))
					{
						if (This->GetBoundCount() > 1)
						{
							return FAppStyle::Get().GetBrush("Icons.WarningWithColor");
						}
					}

					return FAppStyle::Get().GetBrush("Icons.Link");
				}
			)
			.ToolTipText_Lambda(
				[ThisWeak = AsWeak()]
				{
					if (TSharedPtr<SRCPanelExposedField> This = StaticCastSharedPtr<SRCPanelExposedField>(ThisWeak.Pin()))
					{
						if (This->GetBoundCount() > 1)
						{
							return LOCTEXT("PropertyBoundMultiple", "WARNING: Property is bound to multiple behaviors!");
						}
					}

					return LOCTEXT("PropertyBound", "Property is bound to a controller.");
				}
			)
			.Visibility_Lambda(
				[ThisWeak = AsWeak()]
...
int32 SRCPanelExposedField::GetBoundCount() const
{
	if (TSharedPtr<FRemoteControlField> Field = WeakField.Pin())
	{
		if (URemoteControlPreset* NodePreset = Field->GetOwner())
		{
			const FGuid Id = Field->GetId();

			if (Id.IsValid())
			{
				return NodePreset->GetEntityBoundCount(Id);
			}
		}
	}

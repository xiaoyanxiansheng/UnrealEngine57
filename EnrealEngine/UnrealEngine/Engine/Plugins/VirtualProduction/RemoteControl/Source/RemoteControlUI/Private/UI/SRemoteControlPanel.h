...
public:
	 ...
	 * @return The preset represented by the panel.
	 */
	const URemoteControlPreset* GetPreset() const
	{
		return PresetWeak.Get();
	}
...
private:
	...
	/** Called at the end of LoadMap to keep the remote control preset panel up-to-date with current PIE world */
...
private:
	...
	/** Holds a weak reference to the preset asset. */
	...
	TSharedPtr<SRCControllerPanel> ControllerPanel;
	/** Behaviour panel UI widget for Remote Control Logic*/
	TSharedPtr<SRCBehaviourPanel> BehaviorPanel;
	/** Action panel UI widget for Remote Control Logic*/
	TSharedPtr<SRCActionPanel> ActionPanel;
	/** Signature panel UI widget */
	TSharedPtr<SRCSignaturePanel> SignaturePanel;

...
int32 SRCLogicPanelListBase::RemoveModel(const TSharedPtr<FRCLogicModeBase> InModel)
{
	if (!InModel.IsValid())
	{
		return 0;
	...
	FScopedTransaction Transaction(LOCTEXT("RemoveModel", "Remove Model"));

	const int32 RemovedCount = InModel->RemoveModel();
...
void SRCLogicPanelListBase::UpdateEntityUsage()
{
	if (TSharedPtr<SRemoteControlPanel> RCPanel = GetRemoteControlPanel())
	{
		if (TSharedPtr<SRCPanelExposedEntitiesList> EntitiesList = RCPanel->GetEntityList())
		{
			EntitiesList->UpdateEntityUsage();
		}
	}
}

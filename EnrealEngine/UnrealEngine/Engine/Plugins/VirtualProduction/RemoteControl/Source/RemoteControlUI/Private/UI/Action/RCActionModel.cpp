...
const FLazyName FRCActionModel::ActionModelName = "ActionModel";
...
void FRCActionModel::AddSpecialContextMenuOptions(FMenuBuilder& MenuBuilder)
{
	if (TSharedPtr<SRemoteControlPanel> RemoteControlPanel = PanelWeakPtr.Pin())
	{
		...
		// 2. Select Source
		MenuBuilder.AddMenuEntry(Commands.SelectSource);
	}
}
...
int32 FRCActionModel::RemoveModel()
{
	URCAction* Action = GetAction();
	...
	TSharedPtr<FRCBehaviourModel> BehaviorModel = GetParentBehaviour();
	...
	URCBehaviour* Behavior = BehaviorModel->GetBehaviour();
	...
	return Behavior->ActionContainer->RemoveAction(Action);
}

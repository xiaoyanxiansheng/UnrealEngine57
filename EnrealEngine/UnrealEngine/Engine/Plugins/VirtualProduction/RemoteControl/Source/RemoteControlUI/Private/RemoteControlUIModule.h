...
public:
	...
	virtual void RegisterPropertyResolver(FName InResolverId, const FRCPropertyResolver& InResolver) override;
	virtual void UnregisterPropertyResolver(FName InResolverId) override;
	//~ End IRemoteControlUIModule interface

	void SelectSource(TSharedPtr<FRemoteControlEntity> InEntity);
...
private:
	...
	void RegisterDefaultPropertyResolvers();

	static bool ResolveDisplayClusterConfigurationDataProperty(const TSharedRef<FRemoteControlProperty>& InProperty, TArray<UObject*>& InOutBoundObjects, TSharedPtr<FPropertyPath>& OutPropertyPath);

	TSharedPtr<FPropertyPath> ResolveProperty(const TSharedRef<FRemoteControlProperty>& InProperty, TArray<UObject*>& InOutBoundObjects);
...
private:
	...
	/** List of property sources resolvers. */
	TMap<FName, FRCPropertyResolver> PropertyResolvers;
};

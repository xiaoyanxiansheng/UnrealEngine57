...
{
	static const FLazyName DisplayClusterConfigDataClassName = "DisplayClusterConfigurationData";
}
...
void FRemoteControlUIModule::StartupModule()
{
	...
	RegisterDefaultPropertyResolvers();
}
...
void FRemoteControlUIModule::UnregisterStructCustomizations()
{
	...
	if (UObjectInitialized())
	{
		...
		PropertyEditorModule.UnregisterCustomPropertyTypeLayout(FRCAssetPathElementNew::StaticStruct()->GetFName());
...
TSharedPtr<SWidget> FRemoteControlUIModule::CreateCustomControllerWidget(URCVirtualPropertyBase* InController, TSharedPtr<IPropertyHandle> InOriginalPropertyHandle) const
{
	...
	{
		CustomControllerWidget = SNew(SCustomTextureControllerWidget, Cast<URCController>(InController), InOriginalPropertyHandle);
	}
	else if (CustomName == UE::RCCustomControllers::CustomStringListControllerName)
	{
		CustomControllerWidget = SNew(SCustomStringListControllerWidget, Cast<URCController>(InController), InOriginalPropertyHandle);
	}
	else if (CustomName == UE::RCCustomControllers::CustomStatelessEventControllerName)
	{
		CustomControllerWidget = SNew(SCustomStatelessEventControllerWidget, Cast<URCController>(InController), InOriginalPropertyHandle);
	}

	return CustomControllerWidget;
}

void FRemoteControlUIModule::RegisterPropertyResolver(FName InResolverId, const FRCPropertyResolver& InResolver)
{
	PropertyResolvers.FindOrAdd(InResolverId, InResolver);
}

void FRemoteControlUIModule::UnregisterPropertyResolver(FName InResolverId)
{
	PropertyResolvers.Remove(InResolverId);
}

void FRemoteControlUIModule::RegisterDefaultPropertyResolvers()
{
	using namespace UE::RemoteControlUI::Private;

	PropertyResolvers.Add(
		DisplayClusterConfigDataClassName,
		FRCPropertyResolver::CreateStatic(&FRemoteControlUIModule::ResolveDisplayClusterConfigurationDataProperty)
	);
}

bool FRemoteControlUIModule::ResolveDisplayClusterConfigurationDataProperty(const TSharedRef<FRemoteControlProperty>& InProperty, TArray<UObject*>& InOutBoundObjects, TSharedPtr<FPropertyPath>& OutPropertyPath)
{
	for (auto It = InOutBoundObjects.CreateIterator(); It; ++It)
	{
		const UObject* BindingObject = *It;

		AActor* OwnerActor = BindingObject->GetTypedOuter<AActor>();
		...
		FProperty* Property = OwnerActor->GetClass()->FindPropertyByName("CurrentConfigData");
		...
		TSharedRef<FPropertyPath> CurrentPropertyPath = InProperty->FieldPathInfo.ToPropertyPath();
		OutPropertyPath = FPropertyPath::Create(Property);

		for (int32 Index = 0; Index < CurrentPropertyPath->GetNumProperties(); Index++)
		{
			OutPropertyPath->AddProperty(CurrentPropertyPath->GetPropertyInfo(Index));
		}
...
TSharedPtr<FPropertyPath> FRemoteControlUIModule::ResolveProperty(const TSharedRef<FRemoteControlProperty>& InProperty, TArray<UObject*>& InOutBoundObjects)
{
	// Resolve it to ensure the property path.
	InProperty->FieldPathInfo.Resolve(InProperty->GetBoundObject());

	if (!InProperty->FieldPathInfo.IsResolved())
	{
		return nullptr;
	}

	TSharedPtr<FPropertyPath> ResolvedPath;

	for (const TPair<FName, FRCPropertyResolver>& ResolverPair : PropertyResolvers)
	{
		const bool bIsResolved = ResolverPair.Value.Execute(InProperty, InOutBoundObjects, ResolvedPath);
	...
	// By default, convert non-actor/actor components to the owning actors.
	for (auto It = InOutBoundObjects.CreateIterator(); It; ++It)
	{
		const UObject* BindingObject = *It;
		...
		{
			AdditionalObjects.Add(OwnerActor);
			It.RemoveCurrent();
		}
	}

	InOutBoundObjects.Append(AdditionalObjects);

	// And then return the default path resolution
	return InProperty->FieldPathInfo.ToPropertyPath();
}

void FRemoteControlUIModule::SelectSource(TSharedPtr<FRemoteControlEntity> InEntity)
{
	const TSharedPtr<FRemoteControlField> RCField = StaticCastSharedPtr<FRemoteControlField>(InEntity);

	if (!RCField.IsValid() || RCField->GetStruct() != FRemoteControlProperty::StaticStruct())
	{
		return;
	}
	...
	TArray<UObject*> BoundObjects = RCProp->GetBoundObjects();
	TSharedPtr<FPropertyPath> PropertyPath = ResolveProperty(RCProp.ToSharedRef(), BoundObjects);

...
struct FRCExposesPropertyArgs
{
	...
	friend inline bool operator==(const FRCExposesPropertyArgs& LHS, const FRCExposesPropertyArgs& RHS)
	{
	...
	friend inline uint32 GetTypeHash(const FRCExposesPropertyArgs& InArgs)
	{
...
/**
 * Callback for custom resolution of exposed property sources.
 * Returns true if the property is resolved / dealt with.
 * When successfully resolving, bound objects can be replaced.
 */
DECLARE_DELEGATE_RetVal_ThreeParams(bool, FRCPropertyResolver, const TSharedRef<FRemoteControlProperty>& /* In Property */, TArray<UObject*>& /* In/Out Bound Objects */, TSharedPtr<FPropertyPath>& /* Out Property Path */)
...
public:	
	...
	/**
	 * Registers a property source resolution callback.
	 */
	virtual void RegisterPropertyResolver(FName InResolverId, const FRCPropertyResolver& InResolver) = 0;

	/**
	 * Unregisters a property source resolution callback.
	 */
	virtual void UnregisterPropertyResolver(FName InResolverId) = 0;

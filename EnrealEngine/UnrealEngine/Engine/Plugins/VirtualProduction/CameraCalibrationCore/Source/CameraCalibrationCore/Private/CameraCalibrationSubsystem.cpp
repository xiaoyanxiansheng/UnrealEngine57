...
#include UE_INLINE_GENERATED_CPP_BY_NAME(CameraCalibrationSubsystem)
...
{
	// Since this may be looked by from a live link stream, we relax the lookup to be case-insensitive

	const FName ModelName = LensModel->GetDefaultObject<ULensModel>()->GetModelName();
	const FString ModelNameStr = ModelName.ToString().ToLower();

	LensModelMap.Add(FName(*ModelNameStr), LensModel);
}
...
void UCameraCalibrationSubsystem::UnregisterDistortionModel(TSubclassOf<ULensModel> LensModel)
{
	...
	LensModelMap.Remove(FName(*ModelNameStr));
}
...
{
	// Keys are stored in lower case to make the lookup case-insensitive.

	const FString ModelNameStr = ModelName.ToString().ToLower();
	const FName LowercaseModelName = FName(*ModelNameStr);
	
	if (LensModelMap.Contains(LowercaseModelName))
	{
		return LensModelMap[LowercaseModelName];
	}
...
const TSharedPtr<FLensDistortionSceneViewExtension, ESPMode::ThreadSafe>& UCameraCalibrationSubsystem::GetSceneViewExtension() const
{

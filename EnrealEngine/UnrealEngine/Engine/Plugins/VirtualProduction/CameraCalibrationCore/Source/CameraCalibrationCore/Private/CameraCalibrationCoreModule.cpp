...
void FCameraCalibrationCoreModule::RegisterDistortionModels()
{
	auto RegisterModels = [this]()
	{
		...
		SubSystem->RegisterDistortionModel(UBrownConradyUDLensModel::StaticClass());
		SubSystem->RegisterDistortionModel(UBrownConradyDULensModel::StaticClass());
	};
...
void FCameraCalibrationCoreModule::UnregisterDistortionModels()
{
	if (GEngine)
	{
		...
		if (UCameraCalibrationSubsystem* SubSystem = GEngine->GetEngineSubsystem<UCameraCalibrationSubsystem>())
		{
			...
			SubSystem->UnregisterDistortionModel(UBrownConradyUDLensModel::StaticClass());
			SubSystem->UnregisterDistortionModel(UBrownConradyDULensModel::StaticClass());
		}
	}
}

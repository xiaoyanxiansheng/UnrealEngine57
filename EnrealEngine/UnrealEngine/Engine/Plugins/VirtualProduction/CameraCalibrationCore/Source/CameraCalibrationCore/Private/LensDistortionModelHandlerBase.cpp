...
#include UE_INLINE_GENERATED_CPP_BY_NAME(LensDistortionModelHandlerBase)
...
EDistortionRenderingMode ULensDistortionModelHandlerBase::GetPreferredRenderingMode() const
{
	// By default, reverse distorting models prefer PPM for accuracy, forward distorting models prefer SVE
	return IsForwardDistorting() ? EDistortionRenderingMode::SceneViewExtension : EDistortionRenderingMode::PostProcessMaterial;
}
...
{
	EShaderPlatform ShaderPlatform = GetWorld() ? GetFeatureLevelShaderPlatform_Checked(GetWorld()->GetFeatureLevel()) : GMaxRHIShaderPlatform;

	if (FMaterialResource* MaterialResource = MID->GetMaterialResource(ShaderPlatform))
	{

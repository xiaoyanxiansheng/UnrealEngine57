...
#include UE_INLINE_GENERATED_CPP_BY_NAME(LensComponent)
...
void ULensComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	if (UCineCameraComponent* const CineCameraComponent = Cast<UCineCameraComponent>(TargetCameraComponent.GetComponent(GetOwner())))
	{
		...
		if (bApplyDistortion)
		{
			...
			{
				const EDistortionRenderingMode ResolvedRenderingMode = GetDistortionRenderingMode();
				
				// Check if the resolved rendering mode has changed (can happen with Preferred mode)
				if (bIsDistortionSetup && ResolvedRenderingMode != LastAppliedRenderingMode)
				{
					// Clean up the previous mode before switching
					CleanupDistortion(CineCameraComponent);
				}
				
				if (ResolvedRenderingMode == EDistortionRenderingMode::PostProcessMaterial)
				{
					...
					LastAppliedRenderingMode = ResolvedRenderingMode;
				}
				else if (ResolvedRenderingMode == EDistortionRenderingMode::SceneViewExtension)
				{
				...
				if (bOverrideCameraOverscan)
				{
					...
					LensDistortionHandler->SetOverscanFactor(CineCameraComponent->Overscan + 1.0f);
 					...
 					CineCameraComponent->bCropOverscan = (ResolvedRenderingMode == EDistortionRenderingMode::SceneViewExtension);
				}
			}
			else
			{
...
void ULensComponent::Serialize(FArchive& Ar)
{
	...
	if (Ar.IsLoading())
	{
		// For backwards compatibility, preserve the old default value
		if ((Ar.CustomVer(FFortniteMainBranchObjectVersion::GUID) < FFortniteMainBranchObjectVersion::LensComponentDefaultToDistortionSVE)
			&& !GetArchetype()->IsInBlueprint())
		{
			DistortionRenderingMode = EDistortionRenderingMode::PostProcessMaterial;
		}
	}
#endif
...
EDistortionRenderingMode ULensComponent::GetDistortionRenderingMode() const
{
	// If not Preferred, return as specified.
	if (DistortionRenderingMode != EDistortionRenderingMode::Preferred)
	{
		return DistortionRenderingMode;
	}

	// Get the handler for the current lens model
	const ULensDistortionModelHandlerBase* LensDistortionHandler = LensDistortionHandlerMap.FindRef(LensModel);
	if (LensDistortionHandler)
	{
		// Let the lens model decide its preferred rendering mode
		const EDistortionRenderingMode PreferredMode = LensDistortionHandler->GetPreferredRenderingMode();
		
		// If the handler unexpectedly returns Preferred, use default behavior based on IsForwardDistorting()
		if (PreferredMode == EDistortionRenderingMode::Preferred)
		{
			// Reverse distorting models prefer PPM for accuracy, forward distorting prefer SVE
			return LensDistortionHandler->IsForwardDistorting() 
				? EDistortionRenderingMode::SceneViewExtension 
				: EDistortionRenderingMode::PostProcessMaterial;
		}
		
		return PreferredMode;
	}

	// Default to SVE if no handler is found
	return EDistortionRenderingMode::SceneViewExtension;
}
...
void ULensComponent::CleanupDistortion(UCineCameraComponent* const CineCameraComponent)
{
	...
	bIsDistortionSetup = false;
	LastAppliedRenderingMode = EDistortionRenderingMode::Preferred;
}

...
private:
	...
	/** Handles received socket data for multicast receiver. */
	void HandleMulticastSocketDataReceived(const TSharedPtr<FArrayReader, ESPMode::ThreadSafe>& Data, const FIPv4Endpoint& Sender);
...
#if PLATFORM_DESKTOP
	/** Logging class to enable inspection of message data. */
	TUniquePtr<FUdpMessageProcessorLogger> MessageLogger;
#endif
};

...
void JoinedToGroup(const FIPv4Endpoint& UnicastEndpoint, const FIPv4Endpoint& MulticastEndpoint, FSocket* MulticastSocket)
{
	...
	if (UnicastEndpoint.Address == FIPv4Address::Any)
	{
		...
		const bool bShouldBindToLocalHostAddr = LocalAdapterAddresses.Num() == 0;
		
		// The loopback address is not included in the call to GetLocalAdapterAddresses. Make sure we add it so that loopback address is included in the group.
		TSharedRef<FInternetAddr> Loopback = ISocketSubsystem::Get(PLATFORM_SOCKETSUBSYSTEM)->CreateInternetAddr();
		Loopback->SetLoopbackAddress();
		LocalAdapterAddresses.Add(Loopback.ToSharedPtr());

		// Now iterate over all addresss and join the multicast group.
		for (const TSharedPtr<FInternetAddr>& LocalIp : LocalAdapterAddresses)
		{
			DoJoinMulticastGroup(MulticastAddr, LocalIp, MulticastSocket);
		}

		if (bShouldBindToLocalHostAddr)
		{
...
bool FUdpMessageTransport::StartTransport(IMessageTransportHandler& Handler)
{
	...
	if (MulticastSocket != nullptr)
	{
		...
		MulticastReceiver->OnDataReceived().BindRaw(this, &FUdpMessageTransport::HandleMulticastSocketDataReceived);
		MulticastReceiver->SetMaxReadBufferSize(2048);
		MulticastReceiver->Start();
	}
	else
	{
		UE_LOG(LogUdpMessaging, Warning, TEXT("Multicast socket was null so there is no multicast receiver."));
	}
	...
	MessageLogger = MakeUnique<FUdpMessageProcessorLogger>(UnicastEndpoint, UnicastSocket ? UnicastSocket->GetPortNo() : 0, MessageProcessor);
#endif
...
void FUdpMessageTransport::HandleProcessorMessageReassembled(FUdpReassembledMessageSharedPtr ReassembledMessagePtr, const TSharedPtr<IMessageAttachment, ESPMode::ThreadSafe>& Attachment, const FGuid& NodeId)
{
	...
	auto DeserializeReassembledMessage = [DeserializedMessage = MoveTemp(DeserializedMessage), TransportHandler = this->TransportHandler, ReassembledMessagePtr = MoveTemp(ReassembledMessagePtr), NodeId](uint32 Slot)
	{
		...
		{
			TWeakObjectPtr<UScriptStruct> TypeInfo = ReassembledMessagePtr->GetMessageTypeInfo();
			FString TypeInfoStr = TypeInfo.IsValid() ? TypeInfo->GetName() : TEXT("UKNOWN TYPE");
			UE_LOG(LogUdpMessaging, Verbose, TEXT("Failed to deserialize message of type %s from %s"), *TypeInfoStr, *NodeId.ToString());
		}
		// We always mark the message as delivered even if we can't deserialize it. There may be some messages that can't be deserialized because of missing type information.
		ReassembledMessagePtr->MarkDelivered();
	};
...
void FUdpMessageTransport::StopAutoRepairRoutine()
{
	...
	{
		FTSTicker::RemoveTicker(AutoRepairHandle);
	}
}
...
void FUdpMessageTransport::HandleMulticastSocketDataReceived(const TSharedPtr<FArrayReader, ESPMode::ThreadSafe>& Data, const FIPv4Endpoint& Sender)
{
	if (MessageProcessor != nullptr)
	{

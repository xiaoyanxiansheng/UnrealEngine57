...
public:
	...
	FUdpReassembledMessage(uint8 InProtocolVersion, EMessageFlags InFlags, int64 MessageSize, uint32 SegmentCount, uint64 InSequence, const FIPv4Endpoint& InSender)
		...
		, ReceivedBytes(0)
		, DeliveryStatus(EDeliveryStatus::NotDelivered)
...
public:
	...
	{
		return DeliveryStatus == EDeliveryStatus::Delivered;
	}

	/**
	 * Get if the message is pending delivery through the transport layer.
	 */
	bool IsPendingDelivery() const
	{
		return DeliveryStatus == EDeliveryStatus::PendingDelivery;
	}
	...
	void MarkDelivered()
	{
		DeliveryStatus = EDeliveryStatus::Delivered;
	}

	/**
	 * Mark the reasembled message as pending delivery.
	 */
	void MarkPendingDelivery()
	{
		DeliveryStatus = EDeliveryStatus::PendingDelivery;
	}
	...
	{
		if (IsPendingDelivery() || IsMalformed() || !IsInitialized() || SegmentData.Num() == 0)
		{
			return;
		}
	...
	/** True if this message is reliably delivered. */
	const bool IsReliable() const
	{
		return EnumHasAnyFlags(GetFlags(), EMessageFlags::Reliable);
	}
...
private:
	...
	/** Holds if the reassembled message delivery status. This may get set from another thread which is why it is atomic. */
	std::atomic<EDeliveryStatus> DeliveryStatus;

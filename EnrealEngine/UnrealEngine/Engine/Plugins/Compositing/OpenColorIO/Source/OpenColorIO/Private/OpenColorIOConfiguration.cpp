...
bool UOpenColorIOConfiguration::GetRenderResources(ERHIFeatureLevel::Type InFeatureLevel, const FOpenColorIOColorConversionSettings& InSettings, FOpenColorIOTransformResource*& OutShaderResource, TArray<TWeakObjectPtr<UTexture>>& OutTextureResources)
{
	TObjectPtr<const UOpenColorIOColorTransform> Transform = FindTransform(InSettings);
	if (IsValid(Transform))
	{
...
void UOpenColorIOConfiguration::ReloadExistingColorspaces(bool bForce)
{
	...
	if (Config && Config->IsValid())
	{
		...
		if (ConfigHash != NewConfigHash || bForce)
		{
			...
			{
				int32 ColorSpaceIndex = Config->GetColorSpaceIndex(*ExistingColorSpace.ColorSpaceName);
				...
				ColorSpace.Description = Config->GetColorSpaceDescription(*ExistingColorSpace.ColorSpaceName);
			...
			for (const FOpenColorIODisplayView& ExistingDisplayView : DisplayViewsToBeReloaded)
			{
				...
				for (int32 ViewIndex = 0; ViewIndex < Config->GetNumViews(*ExistingDisplayView.Display); ++ViewIndex)
				{
					...
					{
						FOpenColorIODisplayView DisplayView = ExistingDisplayView;
						DisplayView.Description = Config->GetDisplayViewTransformDescription(*ExistingDisplayView.Display, *ExistingDisplayView.View);
						
						DesiredDisplayViews.Add(DisplayView);

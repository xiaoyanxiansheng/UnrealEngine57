...
static TAutoConsoleVariable<bool> CVarOpenColorIOAllowAsyncCompilation(
	TEXT("OpenColorIO.AllowAsyncCompilation"),
	true,
	TEXT("When enabled, we enforce synchronous shader compilation.")
);
...
void UOpenColorIOColorTransform::SerializeOpenColorIOShaderMaps(const TMap<const ITargetPlatform*, TArray<FOpenColorIOTransformResource*>>* PlatformColorTransformResourcesToSavePtr, FArchive& Ar, TArray<FOpenColorIOTransformResource>&  OutLoadedResources)
{
	...
	else if (Ar.IsLoading())
	{
		...
		{
			FOpenColorIOTransformResource& LoadedResource = OutLoadedResources.AddDefaulted_GetRef();
...
void UOpenColorIOColorTransform::CacheResourceShadersForCooking(EShaderPlatform InShaderPlatform, const ITargetPlatform* TargetPlatform, const FString& InShaderHash, const FString& InShaderCode, const FString& InRawConfigHash, TArray<FOpenColorIOTransformResource*>& OutCachedResources)
{
	...
	NewResource->SetupResource((ERHIFeatureLevel::Type)TargetFeatureLevel, InShaderHash, InShaderCode, InRawConfigHash, GetTransformFriendlyName(), AssetPath, BuildShaderParamMetadata());
...
void UOpenColorIOColorTransform::ProcessTransformForGPU()
{
	...
	if (GetTransformProcessor(TransformProcessor))
	{
		...
		for (uint32 Index = 0; Index < GPUProcessor.GetNum3DTextures(); ++Index)
		{
			...
			checkf(bSuccess, TEXT("Failed to read OCIO 3D LUT data."));

			LookupTextures.Add(CreateTexture3DLUT(GpuProcessorHash, TextureName, EdgeLength, Filter, TextureValues));
		}
		...
		for (uint32 Index = 0; Index < GPUProcessor.GetNumTextures(); ++Index)
		{
			...
			LookupTextures.Add(CreateTexture1DLUT(GpuProcessorHash, TextureName, TextureWidth, TextureHeight, Filter, bRedChannelOnly, TextureValues));
		}
...
#endif // WITH_EDITOR
				TransformResource->SetupResource(CacheFeatureLevel, ShaderCodeHash, ShaderCode, RawConfigHash, GetTransformFriendlyName(), AssetPath, BuildShaderParamMetadata());
...
{
	if (!CVarOpenColorIOAllowAsyncCompilation.GetValueOnAnyThread())
	{
		bIsSynchronous = true;
	}
...
bool UOpenColorIOColorTransform::GetRenderResources(ERHIFeatureLevel::Type InFeatureLevel, FOpenColorIOTransformResource*& OutShaderResource, TArray<TWeakObjectPtr<UTexture>>& OutTextureResources) const
{
	...
	{
		OutTextureResources.Reserve(LookupTextures.Num());

		for (const TObjectPtr<UTexture>& Texture : LookupTextures)
		{
			OutTextureResources.Add(Texture);
		}
	}
...
{
	TArray<TWeakObjectPtr<UTexture>> OutTextureArray;
	if (GetRenderResources(InFeatureLevel, OutShaderResource, OutTextureArray))
	{
		for (int32 Index = 0; Index < OutTextureArray.Num(); Index++)
		{
			OutTextureResources.Emplace(Index, OutTextureArray[Index]);
		}
...
bool UOpenColorIOColorTransform::GetTransformProcessor(FOpenColorIOWrapperProcessor& OutProcessor, const TMap<FString, FString>& InContextOverride) const
{
#if WITH_OCIO
	...
	{
		OutProcessor = FOpenColorIOWrapperProcessor(
			ConfigWrapper,
			SourceColorSpace,
			Display,
			View,
			CurrentDisplayViewDirection == EOpenColorIOViewTransformDirection::Inverse,
			InContextOverride.IsEmpty() ? ConfigurationOwner->Context : InContextOverride
		);
	}
	else
	{
...
FShaderParametersMetadata* UOpenColorIOColorTransform::BuildShaderParamMetadata()
{
	// We simply use static allocations for resource names, similar to ComputeFramework::AddParamForType.
	static TArray<FString> AllTextureResourceNames;
	static TArray<FString> AllSamplerResourceNames;

	FShaderParametersMetadataBuilder Builder;

	// First, we add our built-in shader paramaters
	Builder.AddIncludedStruct<FOpenColorIOPixelShaderParameters>();

	// Second, we add the dynamic texture & sampler parameters if present in the transform.
	// We always add the texture resource followed by its sampler. The same order is at render-time,
	// in FOpenColorIORendering::AddPass_RenderThread.
	for (const TObjectPtr<UTexture>& Texture : LookupTextures)
	{
		if (IsValid(Texture))
		{
			int32 TexureIndex = AllTextureResourceNames.AddUnique(Texture->GetName());
			int32 SamplerIndex = AllSamplerResourceNames.AddUnique(Texture->GetName() + TEXT("Sampler"));

			Builder.AddTexture(*AllTextureResourceNames[TexureIndex], Texture.IsA(UVolumeTexture::StaticClass()) ? TEXT("TextureCube") : TEXT("Texture2D"));
			Builder.AddSampler(*AllSamplerResourceNames[SamplerIndex], TEXT("SamplerState"));
		}
	}

	return Builder.Build(FShaderParametersMetadata::EUseCase::ShaderParameterStruct, TEXT(""));
}
...
void UOpenColorIOColorTransform::PostLoad()
{
	...
	if (!Textures_DEPRECATED.IsEmpty())
	{
		for (const TPair<int32, TObjectPtr<UTexture>>& TexturePair : Textures_DEPRECATED)
		{
			LookupTextures.Add(TexturePair.Value);
		}
		Textures_DEPRECATED.Empty();
	}

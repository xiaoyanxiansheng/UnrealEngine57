...
static FAutoConsoleVariableRef CVarEnableSeamChecks(TEXT("p.ChaosCloth.EnableSeamChecks"), bEnableSeamChecks, TEXT("Enable seam validation checks"));
	...
	FCollectionClothSeamConstFacade::FCollectionClothSeamConstFacade(const TSharedRef<const FConstClothCollection>& ClothCollection, int32 SeamIndex)
		...
		if (OrigNumSeamStitches > 0)
		{
			...
			{	
				...
				for (int32 AccessoryMeshIndex = 0; AccessoryMeshIndex < Cloth.GetNumSimAccessoryMeshes(); ++AccessoryMeshIndex)
				{
					...
					// Split 3D positions
					SplitCopyVertexData(SplittingGroups, AccessoryMesh.GetSimAccessoryMeshPosition3D());

					// Split normals
					SplitCopyVertexData(SplittingGroups, AccessoryMesh.GetSimAccessoryMeshNormal());

					// Split BoneIndices
					SplitCopyVertexData(SplittingGroups, AccessoryMesh.GetSimAccessoryMeshBoneIndices());

					// Split BoneWeights
					SplitCopyVertexData(SplittingGroups, AccessoryMesh.GetSimAccessoryMeshBoneWeights());
				}
	...
	void FCollectionClothSeamFacade::Initialize(TConstArrayView<FIntVector2> InStitches)
	{
		...
		// Weld accessory mesh data
		for (int32 AccessoryMeshIndex = 0; AccessoryMeshIndex < Cloth.GetNumSimAccessoryMeshes(); ++AccessoryMeshIndex)
		{
			FCollectionClothSimAccessoryMeshFacade AccessoryMesh = Cloth.GetSimAccessoryMesh(AccessoryMeshIndex);

			// Weld 3D positions
			WeldByWeightedAverage(WeldingGroups, AccessoryMesh.GetSimAccessoryMeshPosition3D());

			// Weld normals
			WeldNormals(WeldingGroups, AccessoryMesh.GetSimAccessoryMeshNormal());

			// Weld BoneIndices and Weights
			WeldIndexAndFloatArrays<true, FClothCollection::MaxNumBoneInfluences>(WeldingGroups, AccessoryMesh.GetSimAccessoryMeshBoneIndices(), AccessoryMesh.GetSimAccessoryMeshBoneWeights(),
				[](const TPair<float, int32>& A, const TPair<float, int32>& B) { return A > B; });
		}
	...
	TSharedRef<class FClothCollection> FCollectionClothSeamFacade::GetClothCollection()
	{

...
namespace UE::Chaos::ClothAsset
{
	...
	TConstArrayView<int32> FCollectionClothSimPatternConstFacade::GetSimImportVertexID() const
	{
		return ClothCollection->GetElements(
			ClothCollection->GetSimImportVertexID(),
			ClothCollection->GetSimVertices2DStart(),
			ClothCollection->GetSimVertices2DEnd(),
			GetElementIndex());
	}
	...
	FCollectionClothSimPatternConstFacade::FCollectionClothSimPatternConstFacade(const TSharedRef<const FConstClothCollection>& ClothCollection, int32 InPatternIndex)
	...
	void FCollectionClothSimPatternFacade::Initialize(const FCollectionClothSimPatternConstFacade& Other, const int32 SimVertex3DOffset, const int32 FabricsOffset)
	{
		...
		if (Other.ClothCollection->IsValid(EClothCollectionExtendedSchemas::Import))
		{
			FClothCollection::CopyArrayViewData(GetSimImportVertexID(), Other.GetSimImportVertexID());
		}
		else if (ClothCollection->IsValid(EClothCollectionExtendedSchemas::Import))
		{
			// Initialize to INDEX_NONE
			TArrayView<int32> SimImportVertexID = GetSimImportVertexID();
			for (int32 Index = 0; Index < SimImportVertexID.Num(); ++Index)
			{
				SimImportVertexID[Index] = INDEX_NONE;
			}
		}
	}
	...
	TArrayView<int32> FCollectionClothSimPatternFacade::GetSimImportVertexID()
	{
		if (!GetClothCollection()->IsValid(EClothCollectionExtendedSchemas::Import))
		{
			GetClothCollection()->DefineSchema(EClothCollectionExtendedSchemas::Import);
		}

		return GetClothCollection()->GetElements(
			GetClothCollection()->GetSimImportVertexID(),
			GetClothCollection()->GetSimVertices2DStart(),
			GetClothCollection()->GetSimVertices2DEnd(),
			GetElementIndex());
	}
	...
	FIntVector2 FCollectionClothSimPatternFacade::AppendSimVertices(int32 NumSimVertices)
	{
		...
		const bool bSetImportVertexID = ClothCollection->IsValid(EClothCollectionExtendedSchemas::Import);
		TArrayView<int32> SimImportVertexID = bSetImportVertexID ? GetSimImportVertexID() : TArrayView<int32>();
		for (int32 NewVertexIndex = 0; NewVertexIndex < NumSimVertices; ++NewVertexIndex)
		{
			...
			if (bSetImportVertexID)
			{
				SimImportVertexID[LocalVertex2D] = INDEX_NONE;
			}
		}
	...
	TSharedRef<class FClothCollection> FCollectionClothSimPatternFacade::GetClothCollection()
	{

...
#include UE_INLINE_GENERATED_CPP_BY_NAME(ClothSimulationModel)
...
namespace UE::Chaos::ClothAsset::Private
{
	...
	static int32 CalculateReferenceBoneIndex(const TArray<FChaosClothSimulationLodModel> ClothSimulationLodModels, const FReferenceSkeleton& ReferenceSkeleton, const TArray<int32>& UsedBoneIndices)
	{
		...
		return FClothEngineTools::CalculateReferenceBoneIndex(WeightedBones, ReferenceSkeleton);
	}
	...
	TArray<FChaosClothAssetLodTransitionDataCache>* InOutTransitionCache)
{
	...
	for (int32 LodIndex = 0; LodIndex < NumLods; ++LodIndex)
	{
		...
		{
			// Get reference bone
			const FName ReferenceBoneName = Cloth.GetReferenceBoneName();

			ReferenceBoneIndex = ReferenceSkeleton.FindRawBoneIndex(ReferenceBoneName);
		}
		...
		{
			// Keep all weight maps to allow runtime switching.
		...
		{
			RequiredExtraSimBones.Add(ReferenceBoneIndex);
		}
		...
		// Add any accessory mesh required bones
		for (int32 AccessoryMeshIndex = 0; AccessoryMeshIndex < Cloth.GetNumSimAccessoryMeshes(); ++AccessoryMeshIndex)
		{
			FCollectionClothSimAccessoryMeshConstFacade AccessoryMesh = Cloth.GetSimAccessoryMesh(AccessoryMeshIndex);

			TConstArrayView<TArray<int32>> AccessoryBoneIndices = AccessoryMesh.GetSimAccessoryMeshBoneIndices();
			for (const TArray<int32>& BoneIndices : AccessoryBoneIndices)
			{
				for (const int32 BoneIndex : BoneIndices)
				{
					if (BoneIndex != INDEX_NONE)
					{
						RequiredExtraSimBones.Add(BoneIndex);
					}
				}
			}
		}
	...
	// Initialize Reference bone index if not set in the cloth collection
	if (ReferenceBoneIndex == INDEX_NONE)
	{

...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AnimSequencerController)
...
bool UAnimSequencerController::SetCurveKey(const FAnimationCurveIdentifier& CurveId, const FRichCurveKey& Key, bool bShouldTransact)
{
	...
	if (FRichCurve* RichCurve = Model->GetMutableRichCurve(CurveId))
	{
		...
		auto SetControlCurve = [this, CurveId, Key](bool bUpdate, float& OutKeyTiming)
		{
			// FMovieSceneFloatChannel stores keys along frames rather than time-based keying, this means Key.Time can be different than actual key set on the float-channel (max 240k fps). So adjust the timing using OutKeyTiming to match for undo/redo and rich key mirroring.
			if(CurveId.CurveType == ERawCurveTrackTypes::RCT_Float && !SetCurveControlKey(CurveId.CurveName, Key, bUpdate, OutKeyTiming))
			{
		...
		if (Handle != FKeyHandle::Invalid())
		{
			...
			float MovieSceneKeyTiming = Key.Time;
			if (SetControlCurve(true, MovieSceneKeyTiming))
			{
		...
		else
		{
			...
			float MovieSceneKeyTiming = Key.Time;
			if (SetControlCurve(false, MovieSceneKeyTiming))
			{
				ConditionalAction<UE::Anim::FRemoveRichCurveKeyAction>(bShouldTransact,  CurveId, MovieSceneKeyTiming);
				// Add the new key
				RichCurve->AddKey(MovieSceneKeyTiming, Key.Value);
	...
	if (ModelInterface->HasBeenPopulated())
	{
		OpenBracket(LOCTEXT("SettingNewskeleton", "Updating Skeleton for Animation Data Model"), bShouldTransact);
		{
			...
			if(TargetSkeleton)
			{
			    ...
			    if (UMovieSceneControlRigParameterSection* Section = Model->GetFKControlRigSection())
			    {
			    	// find bones needing to be REMOVED
				    const TArray<FTransformParameterNameAndCurves>& BoneTrackCurves = Section->GetTransformParameterNamesAndCurves();
				    for (const FTransformParameterNameAndCurves& BoneCurve : BoneTrackCurves)
				    {
					    const FName ExpectedBoneName = UFKControlRig::GetControlTargetName(BoneCurve.ParameterName, ERigElementType::Bone);
			    	...
			    	// find bones needing to be ADDED
				    const TArray<FMeshBoneInfo>& RawRefBones = ReferenceSkeleton.GetRawRefBoneInfo();
			    	for (const FMeshBoneInfo& RawRefBone : RawRefBones)
			    	{
			    		bool bFoundBoneTrack = false;
			    		for (const FTransformParameterNameAndCurves& BoneCurve : BoneTrackCurves)
			    		{
			    			if (BoneCurve.ParameterName == RawRefBone.Name)
			    			{
			    				bFoundBoneTrack = true;
			    				break;
			    			}
			    		}
			    		if (!bFoundBoneTrack)
			    		{
			    			BonesToBeAdded.Add(RawRefBone.Name);
			    		}
			    	}

			    	// find curves needing to be REMOVED
			...
			if (BonesToBeAdded.Num() || BonesToBeRemoved.Num() || CurvesToBeRemoved.Num())
			{
...
bool UAnimSequencerController::SetCurveControlKey(const FName& CurveName, const FRichCurveKey& Key, bool bUpdateKey, float& OutKeyTiming) const
{
	if (UMovieSceneControlRigParameterSection* Section = Model->GetFKControlRigSection())
	{
		UControlRig* ControlRig = Section->GetControlRig();
		...
		if (UFKControlRig* FKRig = Cast<UFKControlRig>(ControlRig))
		{
			if(const URigHierarchy* Hierarchy = Model->GetControlRigHierarchy())
			{
				...
				if (bContainsCurve && bContainsCurveControl)
				{
					...
					OutKeyTiming = ParameterCurvePair->ParameterCurve.GetTickResolution().AsSeconds(FrameNumber);

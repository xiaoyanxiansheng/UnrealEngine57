...
TArrayView<const float> FRigUnit_RigLogic::TestAccessor::CreateTwoJointNeutralTransforms(float *InValueArray)
{
	...
	Joint1Transform.SetRotation(Joint1Transform.TransformRotation(FQuat::MakeFromEuler(FVector(90.0f, 0.0f, 0.0f))));
	Joint1Transform.SetTranslation(Joint1Transform.TransformPosition(FVector(1.0f, 0.0f, 0.0f)));
	...
	Joint2Transform.SetRotation(Joint2Transform.TransformRotation(FQuat::MakeFromEuler(FVector(90.0f, 0.0f, 0.0f))));
	Joint2Transform.SetTranslation(Joint2Transform.TransformPosition(FVector(1.0f, 0.0f, 0.0f)));
...
{
	// [hmathee] Disabling this test since it is running into ASAN issues and is no longer officially supported as a test.
	/*
	...
	AddErrorIfFalse(TestHierarchyTwoBones->GetLocalTransform(0).GetTranslation().Equals(FVector(2.f, 0.f, 0.f)), TEXT("UpdateJoints LOD0 Bone 01 Local - unexpected transform"));
	AddErrorIfFalse(TestHierarchyTwoBones->GetLocalTransform(1).GetTranslation().Equals(FVector(2.f, 2.f, 7.f)), TEXT("UpdateJoints LOD0 Bone 02 Local - unexpected transform"));
	AddErrorIfFalse(TestHierarchyTwoBones->GetGlobalTransform(0).GetTranslation().Equals(FVector(2.f, 0.f, 0.f)), TEXT("UpdateJoints LOD0 Bone 01 Global - unexpected transform"));
	AddErrorIfFalse(TestHierarchyTwoBones->GetGlobalTransform(1).GetTranslation().Equals(FVector(4.f, 0.f, -2.f)), TEXT("UpdateJoints LOD0 Bone 02 Global - unexpected transform"));

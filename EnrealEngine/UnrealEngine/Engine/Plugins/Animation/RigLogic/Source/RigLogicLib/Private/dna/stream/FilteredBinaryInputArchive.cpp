...
void FilteredBinaryInputArchive::process(RawMachineLearnedBehavior& dest) {
    ...
    if (contains(layerBitmask, DataLayerBitmask::MachineLearnedBehavior)) {
        ...
        const auto passingIndices2 = dest.lodNeuralNetworkMapping.getCombinedDistinctIndices(memRes);
        UnorderedMap<std::uint16_t, std::uint16_t> remappedIndices2{memRes};
        remap(neuralNetCount, passingIndices2, remappedIndices2);
        dest.lodNeuralNetworkMapping.mapIndices([&remappedIndices2](std::uint16_t value) {
                return remappedIndices2.at(value);
            });
        ...
        for (auto& mesh : dest.neuralNetworkToMeshRegion.indices) {
            for (auto& region : mesh) {
                extd::filter(region, extd::byValue(passingIndices2));
                for (auto& netIndex : region) {
                    netIndex = remappedIndices2.at(netIndex);
                }
            }
        }

        dest.neuralNetworks.reserve(passingIndices2.size());
        for (std::uint16_t i = {}; i < neuralNetCount; ++i) {
            // Check if the neural net indices filtered for the current maxLOD permit loading
            // this neural network
            if (extd::contains(passingIndices2, i)) {
...
void FilteredBinaryInputArchive::process(RawRBFBehavior& dest) {
    ...
    const auto passingIndices2 = dest.lodSolverMapping.getCombinedDistinctIndices(memRes);
    UnorderedMap<std::uint16_t, std::uint16_t> remappedIndices2{memRes};
    remap(solverCount, passingIndices2, remappedIndices2);
    dest.lodSolverMapping.mapIndices([&remappedIndices2](std::uint16_t value) {
            return remappedIndices2.at(value);
        });

    dest.solvers.reserve(passingIndices2.size());

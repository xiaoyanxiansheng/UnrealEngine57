...
static int32 AnimNodeOrientationWarpingDebug = 0;
static FAutoConsoleVariableRef CVarAnimNodeOrientationWarpingDebug(TEXT("a.AnimNode.OrientationWarping.Debug"), AnimNodeOrientationWarpingDebug, TEXT("Turn on visualization debugging for Orientation Warping."));

static int32 AnimNodeOrientationWarpingVerbose = 0;
static FAutoConsoleVariableRef CVarAnimNodeOrientationWarpingVerbose(TEXT("a.AnimNode.OrientationWarping.Verbose"), AnimNodeOrientationWarpingVerbose, TEXT("Turn on verbose graph debugging for Orientation Warping"));

static int32 AnimNodeOrientationWarpingEnable = 1;
static FAutoConsoleVariableRef CVarAnimNodeOrientationWarpingEnable(TEXT("a.AnimNode.OrientationWarping.Enable"), AnimNodeOrientationWarpingEnable, TEXT("Toggle Orientation Warping"));
#endif

#if ENABLE_ANIM_DEBUG || ENABLE_VISUAL_LOG
static int32 AnimNodeOrientationWarpingDebugTransparency = 1;
static FAutoConsoleVariableRef CVarAnimNodeOrientationWarpingDebugTransparency(TEXT("a.AnimNode.OrientationWarping.Debug.Transparency"), AnimNodeOrientationWarpingDebugTransparency, TEXT("Enable transparency for debug draws, based on blend weight"));
#endif
...
#if ENABLE_ANIM_DEBUG
	if (AnimNodeOrientationWarpingVerbose == 1)
	{
...
#if ENABLE_ANIM_DEBUG || ENABLE_VISUAL_LOG
	UE::Anim::FAnimBlendDebugScope* BlendDebugMessage = Context.GetMessage<UE::Anim::FAnimBlendDebugScope>();
	if (BlendDebugMessage)
	{
		DebugColor = BlendDebugMessage->DebugColor;
	}
#endif
...
void FAnimNode_OrientationWarping::EvaluateSkeletalControl_AnyThread(FComponentSpacePoseContext& Output, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	if (bGraphDrivenWarping)
	{
		...
		if (bGraphDrivenWarping)
		{
			...
			if (TargetTime > 0 && CurrentAnimAsset)
			{
				...
				{
					const FAnimExtractContext Context(static_cast<double>(CurrentAnimAssetTime), true, FDeltaTimeRecord(TargetTime), false);
					FTransform PredictedRootMotionTransform = AnimSequence->ExtractRootMotion(Context);
...
#endif
	const int32 DebugIndex = AnimNodeOrientationWarpingDebug;
	...
	if (bDebugging)
	{
		...
		// Draw debug shapes
		{
			...
			Output.AnimInstanceProxy->AnimDrawDebugCircle(
				ComponentTransform.GetLocation() + DebugArrowOffset + ForwardDirection * 100.f * DebugDrawScale,
				4.f * DebugDrawScale, 8, DebugColor.WithAlpha(DebugAlpha), FVector::UpVector, false, 0.f, SDPG_World, 1.f);
			...
			Output.AnimInstanceProxy->AnimDrawDebugDirectionalArrow(
				ComponentTransform.GetLocation() + DebugArrowOffset,
				ComponentTransform.GetLocation() + DebugArrowOffset + RotationDirection * 100.f * DebugDrawScale,
				40.f * DebugDrawScale, FColor::Blue.WithAlpha(DebugAlpha), false, 0.f, 2.f * DebugDrawScale);
			...
			if (bUsedFutureRootMotion && bGraphDrivenWarping)
			{
				...
				Output.AnimInstanceProxy->AnimDrawDebugDirectionalArrow(
					ComponentTransform.GetLocation() + DebugArrowOffset,
					ComponentTransform.GetLocation() + DebugArrowOffset + FutureRotationDirection * 100.f * DebugDrawScale,
					40.f * DebugDrawScale, FColor::Yellow.WithAlpha(DebugAlpha), false, 0.f, 2.f * DebugDrawScale);
			}
			...
			Output.AnimInstanceProxy->AnimDrawDebugDirectionalArrow(
				ComponentTransform.GetLocation() + DebugArrowOffset,
				ComponentTransform.GetLocation() + DebugArrowOffset + WarpedRotationDirection * 100.f * DebugDrawScale,
				40.f * DebugDrawScale, FColor::Green.WithAlpha(DebugAlpha), false, 0.f, 2.f * DebugDrawScale);
		}
...
{
#if ENABLE_ANIM_DEBUG
	if (AnimNodeOrientationWarpingEnable == 0)
	{
		return false;
	}
#endif

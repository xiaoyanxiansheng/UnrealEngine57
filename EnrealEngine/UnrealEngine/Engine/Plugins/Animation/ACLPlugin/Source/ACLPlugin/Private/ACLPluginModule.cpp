...
void FACLPlugin::ListCodecs(const TArray<FString>& Args)
{
	...
	for (const UAnimBoneCompressionSettings* Settings : BoneSettings)
	{
		...
		for (const UAnimSequence* AnimSeq : AnimSequences)
		{
			...
			const SIZE_T Size = GetCompressedBoneSize(CompressedAnimSequence.Get());
	...
	for (const UAnimBoneCompressionCodec* Codec : BoneCodecs)
	{
		...
		for (const UAnimSequence* AnimSeq : AnimSequences)
		{
			UAnimSequence::FScopedCompressedAnimSequence CompressedAnimSequence = AnimSeq->GetCompressedData();
			const SIZE_T Size = GetCompressedBoneSize(CompressedAnimSequence.Get());
			if (CompressedAnimSequence.Get().BoneCompressionCodec == Codec)
			{
				NumReferences++;
				UsedSize += Size;
			}
	...
	for (const UAnimCurveCompressionSettings* Settings : CurveSettings)
	{
		...
		{
			UAnimSequence::FScopedCompressedAnimSequence CompressedAnimSequence = AnimSeq->GetCompressedData();
			const SIZE_T Size = GetCompressedCurveSize(CompressedAnimSequence.Get());
	...
	for (const UAnimCurveCompressionCodec* Codec : CurveCodecs)
	{
		...
		for (const UAnimSequence* AnimSeq : AnimSequences)
		{
			UAnimSequence::FScopedCompressedAnimSequence CompressedAnimSequence = AnimSeq->GetCompressedData();
			const SIZE_T Size = GetCompressedCurveSize(CompressedAnimSequence.Get());
			if (CompressedAnimSequence.Get().CurveCompressionCodec == Codec)
			{
				NumReferences++;
				UsedSize += Size;
			}
	...
	for (const UAnimationCompressionLibraryDatabase* Database : Databases)
	{
		...
		{
			UAnimSequence::FScopedCompressedAnimSequence CompressedAnimSequence = AnimSeq->GetCompressedData();
			UAnimBoneCompressionCodec_ACLDatabase* Codec = Cast<UAnimBoneCompressionCodec_ACLDatabase>(CompressedAnimSequence.Get().BoneCompressionCodec);
...
void FACLPlugin::ListAnimSequences(const TArray<FString>& Args)
{
	...
	for (const UAnimSequence* AnimSeq : AnimSequences)
	{
		...
		UAnimSequence::FScopedCompressedAnimSequence CompressedAnimSequence = AnimSeq->GetCompressedData();
		const FCompressedAnimSequence& CompressedData = CompressedAnimSequence.Get();
		if (CompressedData.BoneCompressionCodec->Description.IsEmpty())
		{
			UE_LOG(LogAnimationCompression, Log, TEXT("    uses bone codec %s"), *CompressedData.BoneCompressionCodec->GetPathName());
		}
		else
		{
			UE_LOG(LogAnimationCompression, Log, TEXT("    uses bone codec %s (%s)"), *CompressedData.BoneCompressionCodec->GetPathName(), *CompressedData.BoneCompressionCodec->Description);
		}

		const SIZE_T BoneDataSize = GetCompressedBoneSize(CompressedData);
		UE_LOG(LogAnimationCompression, Log, TEXT("    has %.2f KB of bone data"), BytesToKB(BoneDataSize));

#if WITH_EDITORONLY_DATA
		if (CompressedData.CompressedDataStructure)
		{
			UE_LOG(LogAnimationCompression, Log, TEXT("    has a bone error of %.4f cm"), CompressedData.CompressedDataStructure->BoneCompressionErrorStats.MaxError);
		}
#endif

		UE_LOG(LogAnimationCompression, Log, TEXT("    uses curve codec %s"), *CompressedData.CurveCompressionCodec->GetPathName());

		const SIZE_T CurveDataSize = GetCompressedCurveSize(CompressedData);

...
void UVertexDeltaModelInstance::CreateNNEModel()
{
	if (!ModelInstanceRDG.IsValid())
	{
		...
		if (VertexDeltaModel)
		{
			const TWeakInterfacePtr<INNERuntime> Runtime = UE::NNE::GetRuntime<INNERuntime>(VertexDeltaModel->GetNNERuntimeName());
			const TWeakInterfacePtr<INNERuntimeRDG> RuntimeRDG = UE::NNE::GetRuntime<INNERuntimeRDG>(VertexDeltaModel->GetNNERuntimeName());
			if (!Runtime.IsValid())
			{
			...
			const TObjectPtr<UNNEModelData> ModelData = VertexDeltaModel->NNEModel;
			if (ModelData && RuntimeRDG.IsValid() && RuntimeRDG->CanCreateModelRDG(ModelData) == INNERuntimeRDG::ECanCreateModelRDGStatus::Ok)
			{
				// Create the model.
				const TSharedPtr<UE::NNE::IModelRDG> ModelRDG = RuntimeRDG->CreateModelRDG(ModelData);
				if (ModelRDG.IsValid())
				{
					...
					{
						// Setup inputs.
						const TConstArrayView<UE::NNE::FTensorDesc> InputTensorDescs = ModelInstanceRDG->GetInputTensorDescs();

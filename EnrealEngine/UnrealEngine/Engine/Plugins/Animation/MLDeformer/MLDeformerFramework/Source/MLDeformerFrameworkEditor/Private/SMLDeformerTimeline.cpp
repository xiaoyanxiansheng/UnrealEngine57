...
namespace UE::MLDeformer
{
							...
							[
								...
								(
									[this]()
									{
										...
										return (VizMode == EMLDeformerVizMode::TrainingData) ? EVisibility::Visible : EVisibility::Hidden;
									}
								)
								[
									SNew(STextBlock)
	...
	void SMLDeformerTimeline::SetTrainingAnimNames(const TArray<TSharedPtr<FMLDeformerTrainingInputAnimName>>& Names)
	{
		...
		// Detect when the selected item index needs to be updated as the position in the name list changed.
		TSharedPtr<FMLDeformerTrainingInputAnimName> PrevSelected;
		if (SelectedTrainingAnimNameIndex != INDEX_NONE)
		{
			PrevSelected = TrainingAnimNames[SelectedTrainingAnimNameIndex];
			if (Names.IsValidIndex(SelectedTrainingAnimNameIndex) && PrevSelected != Names[SelectedTrainingAnimNameIndex])
			{
				for (int32 Index = 0; Index < Names.Num(); ++Index)
				{
					if (Names[Index].IsValid() && Names[Index]->Name == PrevSelected->Name)
					{
						SelectedTrainingAnimNameIndex = Index;
						break;
					}
				}
			}
	...
	void SMLDeformerTimeline::OnSelectTrainingAnim(TSharedPtr<FMLDeformerTrainingInputAnimName> Item, ESelectInfo::Type SelectInfo)
	{
		if (Item.IsValid())
		{
			...
			if (Model.IsValid())
			{
				...
				EditorModel->RequestTriggerInputAssetsChangedNextTick();
			}
		}
	}

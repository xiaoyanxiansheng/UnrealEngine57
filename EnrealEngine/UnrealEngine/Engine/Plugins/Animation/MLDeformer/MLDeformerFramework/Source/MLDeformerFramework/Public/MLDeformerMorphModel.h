...
public:
	UE_API UMLDeformerMorphModel(const FObjectInitializer& ObjectInitializer);
...
#if WITH_EDITOR
	UE_API virtual void UpdateMemoryUsage() override;
	UE_API virtual void FinalizeMorphTargets();
	UE_API bool HasRawMorph() const;
#endif
	...
	UE_API bool CanDynamicallyUpdateMorphTargets() const;
	...
	UE_API virtual void UpdateStatistics();
	...
	UE_API void SetMorphTargetDeltaFloats(const TArray<float>& Deltas);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "MLDeformerMorphModel")
	UE_API void SetMorphTargetDeltas(const TArray<FVector3f>& Deltas);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "MLDeformerMorphModel")
	UE_API void SetMorphTargetsErrorOrder(const TArray<int32>& MorphTargetOrder, const TArray<float>& ErrorValues);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "MLDeformerMorphModel")
	UE_API void SetMorphTargetsMinMaxWeights(const TArray<float>& MinValues, const TArray<float>& MaxValues);
	 ...
	 */
	UE_API void SetMorphTargetsMinMaxWeights(const TArray<FFloatInterval>& MinMaxValues);
	 ...
	 */
	UE_API void ClampMorphTargetWeights(TArrayView<float> WeightsArray);
	...
	TArray<FVector3f>& GetMorphTargetDeltas()							{ return MorphTargetDeltas; }
	...
	UE_API void ClearMorphTargetSets();
	UE_API void AddMorphSets(int32 NumToAdd);
	 ...
	 */
	UE_API int32 GetMorphTargetDeltaStartIndex(int32 MorphTargetIndex) const;
	 ...
	 */
	UE_API TArrayView<const float> GetMorphTargetErrorValues() const;
	 ...
	 */
	UE_API TArrayView<const int32> GetMorphTargetErrorOrder() const;
	 ...
	 */
	UE_DEPRECATED(5.4, "This method will be removed.")
	UE_API TArrayView<const FMLDeformerMorphModelQualityLevel> GetQualityLevels() const;
	 ...
	 */
	UE_DEPRECATED(5.4, "This method will be removed.")
	UE_API TArray<FMLDeformerMorphModelQualityLevel>& GetQualityLevelsArray();
	 ...
	 */
	UE_API float GetMorphTargetError(int32 MorphIndex) const;
	 ...
	 */
	UE_API void SetMorphTargetError(int32 MorphIndex, float Error);

	UE_DEPRECATED(5.4, "Please use the GetNumActiveMorphsForLOD.")
	UE_API int32 GetNumActiveMorphs(int32 QualityLevel) const;

#if WITH_EDITORONLY_DATA
	/** Get the name of the attribute used for the global mask. */
	UE_API FName GetGlobalMaskAttributeName() const;
#endif
...
private:
	 ...
	 * You can get the number of vertices per morph target using Model->GetInputInfo()->GetNumBaseMeshVerts().
	 * This property is Transient, because we don't want it included in undo/redo, as this can be a few hundred megabytes large.
	 * Instead, this data is serialized manually inside the Serialize method.
	 */

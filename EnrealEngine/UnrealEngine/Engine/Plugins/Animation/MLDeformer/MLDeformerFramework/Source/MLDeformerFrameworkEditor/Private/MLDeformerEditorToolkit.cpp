...
namespace UE::MLDeformer
{
		...
		UMLDeformerAsset* InDeformerAsset)
	{
		...
		if (ActiveModel)
		{
			...
			ActiveModel->GetModel()->GetVizSettings()->SetFlags(RF_Transactional);
		}
	...
	TSharedRef<SWidget> FMLDeformerEditorToolkit::GenerateToolsMenuContents(TSharedRef<FUICommandList> InCommandList)
	{
		...
		// Sort the tools menu items before we add them.
		ToolsMenuExtenders.Sort([this](const TUniquePtr<FToolsMenuExtender>& A, const TUniquePtr<FToolsMenuExtender>& B)
			{
				return A->GetMenuEntry(*this).LabelOverride.Get().ToString() < B->GetMenuEntry(*this).LabelOverride.Get().ToString();
			} );
	...
	void FMLDeformerEditorToolkit::OnModelChanged(int Index, bool bForceChange)
	{
		...
		TArray<UClass*> ModelTypes;
		ModelRegistry.GetRegisteredModels().GenerateKeyArray(ModelTypes);
		TObjectPtr<UMLDeformerModel> Model = NewObject<UMLDeformerModel>(DeformerAsset, ModelTypes[Index], NAME_None, RF_Transactional);
		check(Model);
	...
	FSlateIcon FMLDeformerEditorToolkit::GetTrainingStatusIcon() const
	{
		...
		{
			return FSlateIcon(FAppStyle::Get().GetStyleSetName(), CompileStatusBackground, NAME_None);
		}
	...
	void FMLDeformerEditorToolkit::FillToolbar(FToolBarBuilder& ToolbarBuilder)
	{
		...
		ToolbarBuilder.BeginSection("Main");
		{
			ToolbarBuilder.AddToolBarButton(
				FUIAction
				(
					FExecuteAction::CreateLambda([this]() { Train(false); }),
				...
				TAttribute<FSlateIcon>(this, &FMLDeformerEditorToolkit::GetTrainingStatusIcon),
	...
	void FMLDeformerEditorToolkit::PostUndo(bool bSuccess)
	{
		if (ActiveModel && !IsPaintModeActive())
		{
			ActiveModel->TriggerInputAssetChanged(true);
		}
	}

	void FMLDeformerEditorToolkit::PostRedo(bool bSuccess)
	{
		if (ActiveModel && !IsPaintModeActive())
		{
			ActiveModel->TriggerInputAssetChanged(true);
	...
	void FMLDeformerEditorToolkit::AddToolsMenuExtender(TUniquePtr<FToolsMenuExtender> Extender)
	{
		...
		ToolsMenuExtenders.Emplace(MoveTemp(Extender));
	}
	...
	void FMLDeformerEditorToolkit::DisablePaintMode()
	{
		...
		{
			ActiveModel->UpdateLabels();
			ActiveModel->RequestTriggerInputAssetsChangedNextTick();
		}
	}

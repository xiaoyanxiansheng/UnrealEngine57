...
namespace UE::MLDeformer
{
	...
	TSharedPtr<SWidget> SMLDeformerInputCurveListWidget::OnContextMenuOpening() const
	{
		...
		Menu.BeginSection("CopyPasteActions", LOCTEXT("CopyPasteActionsHeading", "Copy/Paste Actions"));
		{
			if (!Elements.IsEmpty())
			{
				Menu.AddMenuEntry(Actions.CopyInputCurves);
			}

			TSharedPtr<FJsonObject> JsonObject;
			if (InputCurvesWidget->GetInputWidget()->HasValidClipBoardData(TEXT("InputCurves"), JsonObject))
			{
				Menu.AddMenuEntry(Actions.PasteInputCurves);
			}
		}
		Menu.EndSection();
	...
	void SMLDeformerInputCurveListWidget::RefreshElements(const TArray<FName>& CurveNames, USkeletalMesh* SkeletalMesh)
	{
		...
		else
		{
			...
			UMLDeformerInputInfo::BuildCurveList(SkeletalMesh, AssetCurveNames);
	...
	void SMLDeformerInputCurvesWidget::BindCommands(TSharedPtr<FUICommandList> CommandList)
	{
		...
		CommandList->MapAction(Commands.CopyInputCurves, FExecuteAction::CreateSP(this, &SMLDeformerInputCurvesWidget::OnCopyInputCurves));
		CommandList->MapAction(Commands.PasteInputCurves, FExecuteAction::CreateSP(this, &SMLDeformerInputCurvesWidget::OnPasteInputCurves));
	}


	void SMLDeformerInputCurvesWidget::OnCopyInputCurves() const
	{
		const TSharedRef<FJsonObject> JsonObject = MakeShared<FJsonObject>();
		JsonObject->SetStringField(TEXT("DataId"), TEXT("InputCurves"));

		// Build the names array.
		TArray<TSharedPtr<FJsonValue>> JsonNameArray;
		for (const FName Name : EditorModel->GetEditorInputInfo()->GetCurveNames())
		{
			if (Name.IsValid() && !Name.IsNone())
			{
				JsonNameArray.Add(MakeShared<FJsonValueString>(Name.ToString()));
			}
		}		
		JsonObject->SetArrayField(TEXT("Names"), JsonNameArray);
		...
		{
			UE_LOG(LogMLDeformer, Error, TEXT("Failed to serialize JSON on Copy action in the Input Curves widget."));
			return;
		}
		FPlatformApplicationMisc::ClipboardCopy(*OutputString);
	}

	void SMLDeformerInputCurvesWidget::OnPasteInputCurves()
	{
		FString Text;
		FPlatformApplicationMisc::ClipboardPaste(Text);

		TSharedPtr<FJsonObject> JsonObject;
		if (GetInputWidget()->HasValidClipBoardData(TEXT("InputCurves"), JsonObject) && JsonObject.IsValid())
		{
			TArray<FString> Names;
			if (!JsonObject->TryGetStringArrayField(TEXT("Names"), Names))
			{
				UE_LOG(LogMLDeformer, Error, TEXT("Failed to get curve names during paste operation."));
			}
				
			FScopedTransaction Transaction(TEXT("SMLDeformerInputCurvesWidget"), LOCTEXT("PasteInputCurvesText", "Paste Input Curves"), EditorModel->GetModel());
			EditorModel->GetModel()->Modify();
		
			TArray<FName> AddedCurves;
			TArray<FMLDeformerCurveReference>& CurrentCurveNames = EditorModel->GetModel()->GetCurveIncludeList();
			for (const FString& CurveNameString : Names)
			{
				if (!CurveNameString.IsEmpty())
				{
					const FName CurveName(CurveNameString);
					CurrentCurveNames.AddUnique(CurveName);
					AddedCurves.Add(CurveName);
				}
			}
			...
			GetInputWidget()->OnAddInputCurves(AddedCurves);
		}
	}
	...
	void FMLDeformerInputCurvesWidgetCommands::RegisterCommands()
	{
		...
		UI_COMMAND(CopyInputCurves, "Copy Input Curves List", "Copies all curves, so that you can paste them into another model's input curves list.", EUserInterfaceActionType::Button, FInputChord(EKeys::C, EModifierKey::Control));
		UI_COMMAND(PasteInputCurves, "Paste Input Curves List", "Pastes a previously copied curves list. This will add curves to the existing list, not replace it fully.", EUserInterfaceActionType::Button, FInputChord(EKeys::V, EModifierKey::Control));
	}

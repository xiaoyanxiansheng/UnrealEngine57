...
namespace UE::MLDeformer
{
	...
	public:
		 ...
		 */
		UE_API const TArray<FName>& GetPickedBoneNames() const;

	private:
		UE_API TSharedRef<SMLDeformerBonePickerTreeWidget> CreateBoneTree();
		UE_API FReply OnKeyDown(const FGeometry& InGeometry, const FKeyEvent& InKeyEvent) override;
		UE_API void OnOkClicked();
		UE_API void OnCancelClicked();
		UE_API void OnFilterTextChanged(const FText& InFilterText);
		UE_API void RefreshBoneTreeElements() const;
		UE_API void SelectInitialItems();
		UE_API void SelectInitialItemsRecursive(const TSharedPtr<FMLDeformerBonePickerTreeElement>& Item);

...
namespace UE::MLDeformer
{
	...
	TSharedPtr<FJsonObject> SMLDeformerInputWidget::GetJsonObject(const FString& JsonString)
	{
	...
	bool SMLDeformerInputWidget::ExtractStringFromJson(const TSharedPtr<FJsonObject>& JsonObject, const FString& Key, FString& OutString)
	{
		if (JsonObject.IsValid())
		{
			if (JsonObject->HasTypedField(Key, EJson::String))
			{
				OutString = JsonObject->GetStringField(Key);
				return true;
			}
		}
		return false;
	...
	bool SMLDeformerInputWidget::HasValidClipBoardData(const FString& DataTypeId, TSharedPtr<FJsonObject>& OutJsonObject)
	{
		// Get the text from the clipboard.
		FString Text;
		FPlatformApplicationMisc::ClipboardPaste(Text);

		const TSharedPtr<FJsonObject> JsonObject = GetJsonObject(Text);
		if (JsonObject.IsValid())
		{
			OutJsonObject = JsonObject;
			
			// Check if we find the "DataTypeId" field, and get it's value.
			// Then compare it to the data type id we are interested in.
			FString DataTypeIdValue;
			if (ExtractStringFromJson(JsonObject, TEXT("DataId"), DataTypeIdValue))
			{
				return DataTypeIdValue.Compare(DataTypeId, ESearchCase::IgnoreCase) == 0;
			}
		}

		// Invalid JSON or no DataTypeId key found, or the value didn't match the one we search for.
		OutJsonObject = JsonObject;
		return false;
	}

...
public:
	 ...
	 * For a more extensive check, please use IsCompatible that takes a USkeletalMeshComponent as parameter.
	 ...
	 * @param InSkeletalMeshComponent The skeletal mesh to check compatibility with. This may not be a nullptr.
	 * @return Returns true when we can safely apply the ML Deformer to a character using this skeletal mesh, otherwise false is returned.
	 * @note Use GenerateCompatibilityErrorString to get the error report.
	 */
	UE_API virtual bool IsCompatible(const USkeletalMeshComponent* InSkeletalMeshComponent) const;
	 ...
	 */
	UE_DEPRECATED(5.7, "Please use the GenerateCompatibilityErrorString that takes a USkeletalMeshComponent as parameter instead.")
	UE_API virtual FString GenerateCompatibilityErrorString(USkeletalMesh* InSkeletalMesh) const;
	
	/**
	 * Get the compatibility error report.
	 * @param InSkeletalMeshComponent The skeletal mesh component to check compatibility with.
	 * @return Returns an empty string in case there are no compatibility issues, otherwise it contains a string that describes the issue(s).
	 *         In case a nullptr is passed as SkeletalMeshComponent parameter, an empty string is returned.
	 */
	UE_API virtual FString GenerateCompatibilityErrorString(const USkeletalMeshComponent* InSkeletalMeshComponent) const;
	...
	/**
	 * Builds a list of available curves on the Skeletal Mesh provided, including its skeleton.
	 * This internally adds a merged set of curves that are found on the skeleton and the skeletal mesh itself.
	 * @param InSkeletalMesh The Skeletal Mesh to get the list of curves for.
	 * @param OutCurveNames The list of curve names that have been found.
	 */
	UE_API static void BuildCurveList(USkeletalMesh* InSkeletalMesh, TArray<FName>& OutCurveNames);

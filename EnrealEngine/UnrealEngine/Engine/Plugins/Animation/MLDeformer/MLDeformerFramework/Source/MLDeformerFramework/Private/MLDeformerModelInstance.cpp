...
{
	TAutoConsoleVariable<bool> CVarMLDeformerDebugDrawInputPoses(
...
FString UMLDeformerModelInstance::CheckCompatibility(USkeletalMeshComponent* InSkelMeshComponent, bool bLogIssues)
{
	...
	if (SkelMesh && !InputInfo->IsCompatible(InSkelMeshComponent) && Model->GetDeformerAsset() && Model->IsTrained())
	{
		ErrorText += InputInfo->GenerateCompatibilityErrorString(InSkelMeshComponent);
		if (bLogIssues)
		{
...
int64 UMLDeformerModelInstance::SetCurveValues(float* OutputBuffer, int64 OutputBufferSize, int64 StartIndex)
{
	...
	for (int32 CurveIndex = 0; CurveIndex < AssetNumCurves; ++CurveIndex)
	{
		...
		float CurveValue;
		SkeletalMeshComponent->GetCurveValue(CurveName, 0.0f, CurveValue);

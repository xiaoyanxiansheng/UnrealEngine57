...
namespace UE::MLDeformer
{
	...
	public:
		...
		UE_API virtual FString GetHeatMapDeformerGraphPath() const override;
		UE_API virtual FString GetHeatMapDeformerGraphDualQuatPath() const override;
		UE_API virtual void OnPreTraining() override;
		UE_API virtual void OnPostTraining(ETrainingResult TrainingResult, bool bUsePartiallyTrainedWhenAborted) override;
		UE_API virtual void OnMaxNumLODsChanged() override;
		 ...
		 */
		UE_API virtual const TArrayView<const float> GetMaskForMorphTarget(int32 MorphTargetIndex) const;
		 ...
		 */
		UE_API virtual bool IsInputMaskingSupported() const;
		 ...
		 */
		UE_API void FillMaskValues(TArrayView<float> ItemMaskBuffer, float Value) const;
		 ...
		 */
		UE_API void ApplyBoneToMask(int32 SkeletonBoneIndex, TArrayView<float> MaskBuffer);

		UE_DEPRECATED(5.4, "Please use the RecursiveAddBoneToMaskUpwards that doesn't take the VirtualParentTable as parameter.")
		UE_API void RecursiveAddBoneToMaskUpwards(const FReferenceSkeleton& RefSkel, int32 SkeletonBoneIndex, int32 MaxHierarchyDepth, const TArray<int32>& VirtualParentTable, TArray<int32>& OutBonesAdded, int32 CurHierarchyDepth = 0);
		 ...
		 */
		UE_API void RecursiveAddBoneToMaskUpwards(const FReferenceSkeleton& RefSkel, int32 SkeletonBoneIndex, int32 MaxHierarchyDepth, TArray<int32>& OutBonesAdded, int32 CurHierarchyDepth = 0);

		UE_DEPRECATED(5.4, "Please use the RecursiveAddBoneToMaskDownwards that doesn't take the VirtualParentTable as parameter.")
		UE_API void RecursiveAddBoneToMaskDownwards(const FReferenceSkeleton& RefSkel, int32 SkeletonBoneIndex, int32 MaxHierarchyDepth, const TArray<int32>& VirtualParentTable, TArray<int32>& OutBonesAdded, int32 CurHierarchyDepth = 0);
		 ...
		 */
		UE_API void RecursiveAddBoneToMaskDownwards(const FReferenceSkeleton& RefSkel, int32 SkeletonBoneIndex, int32 MaxHierarchyDepth, TArray<int32>& OutBonesAdded, int32 CurHierarchyDepth = 0);
		...
		UE_API void AddRequiredBones(const FReferenceSkeleton& RefSkel, int32 SkeletonBoneIndex, const TArray<int32>& VirtualParentTable, TArray<int32>& OutBonesAdded);
		 ...
		 */
		UE_DEPRECATED(5.4, "This method will be removed.")
		UE_API int32 FindVirtualParentIndex(const FReferenceSkeleton& RefSkel, int32 BoneIndex, const TArray<FName>& IncludedBoneNames) const;
		 ...
		 */
		UE_DEPRECATED(5.4, "This method will be removed.")
		UE_API TArray<int32> BuildVirtualParentTable(const FReferenceSkeleton& RefSkel, const TArray<FName>& IncludedBoneNames) const;
		 ...
		 */
		UE_API void ApplyMaskInfoToBuffer(const USkeletalMesh* SkeletalMesh, const FMLDeformerMaskInfo& MaskInfo, TArrayView<float> ItemMaskBuffer);
		 ...
		 */
		UE_API void ApplyGeneratedMaskToVertexAttributes(USkeletalMesh* SkeletalMesh, FMLDeformerMaskInfo& MaskInfo, TVertexAttributesRef<float> AttributeRef);

		// Helpers.
		UE_API UMLDeformerMorphModel* GetMorphModel() const;
		UE_API UMLDeformerMorphModelVizSettings* GetMorphModelVizSettings() const;

	protected:
		UE_API void TransferMorphTargets(TArray<UMorphTarget*> MorphTargetsLODZero);
		 ...
		 */
		UE_API virtual void InitEngineMorphTargets(const TArray<FVector3f>& Deltas);
		 ...
		 */
		UE_API void ClampMorphTargetNumber();
		 ...
		 */
		UE_API void UpdateMorphErrorValues(TArrayView<UMorphTarget*> MorphTargets);
		...
		UE_API virtual void CalcMorphTargetNormals(int32 LOD, USkeletalMesh* SkelMesh, int32 MorphTargetIndex, TArrayView<const FVector3f> Deltas, TArrayView<const FVector3f> BaseVertexPositions, TArrayView<FVector3f> BaseNormals, TArray<FVector3f>& OutDeltaNormals);

		UE_DEPRECATED(5.5, "Please use the CalcMorphTaretNormals that takes an array named GlobalMaskWeights.")
		UE_API virtual void CalcMorphTargetNormals(
			int32 LOD,
		 ...
		 */
		UE_API void DebugDrawMorphTarget(FPrimitiveDrawInterface* PDI, const TArray<FVector3f>& MorphDeltas, float DeltaThreshold, int32 MorphTargetIndex, const FVector& DrawOffset);

		/** Zero all deltas with a length equal to, or smaller than the threshold value. */
		UE_API void ZeroDeltasByLengthThreshold(TArray<FVector3f>& Deltas, float Threshold);
		 ...
		 */
		UE_API void CreateMorphTargets(
		 ...
		 */
		UE_API void CompressMorphTargets(FMorphTargetVertexInfoBuffers& OutMorphBuffers, const TArray<UMorphTarget*>& MorphTargets, int32 LOD = 0, float MorphErrorTolerance = 0.01f);
		 ...
		 */
		UE_API void CalcVertexNormals(TArrayView<const FVector3f> VertexPositions, TArrayView<const uint32> IndexArray, TArrayView<const int32> VertexMap, TArray<FVector3f>& OutNormals) const;
		 ...
		 */
		UE_API bool ProcessVertexDelta(FVector3f& OutScaledDelta, FVector3f& OutScaledDeltaNormal, const FVector3f RawDelta, const FVector3f RawDeltaNormal, float DeltaThreshold, float MorphMaskWeight, float GlobalMaskWeight) const;

		UE_DEPRECATED(5.5, "Please use the CalcGlobalMaskWeights method instead")
		UE_API float CalcGlobalMaskWeight(int32 RenderVertexIndex, const FColorVertexBuffer& ColorBuffer, EMLDeformerMaskChannel MaskChannel, bool bInvertMaskChannel) const;
		 ...
		 */
		UE_API TArray<float> CalcGlobalMaskWeights(const TArray<int32>& VertexMap, const FColorVertexBuffer& ColorBuffer, EMLDeformerMaskChannel MaskChannel, bool bInvertMaskChannel) const;

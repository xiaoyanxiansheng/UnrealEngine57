...
namespace UE::MLDeformer
{
	...
	TSharedPtr<SWidget> SMLDeformerInputBoneTreeWidget::OnContextMenuOpening() const
	{
		...
		Menu.BeginSection("CopyPasteActions", LOCTEXT("CopyPasteActionsHeading", "Copy/Paste Actions"));
		{
			...
			{
				Menu.AddMenuEntry(Actions.CopyInputBones);
			}

			TSharedPtr<FJsonObject> JsonObject;
			if (InputBonesWidget->GetInputWidget()->HasValidClipBoardData(TEXT("InputBones"), JsonObject))
			{
				Menu.AddMenuEntry(Actions.PasteInputBones);
			}
		}
		Menu.EndSection();
	...
	void SMLDeformerInputBonesWidget::BindCommands(TSharedPtr<FUICommandList> CommandList)
	{
		...
		CommandList->MapAction(Commands.CopyInputBones, FExecuteAction::CreateSP(this, &SMLDeformerInputBonesWidget::OnCopyInputBones));
		CommandList->MapAction(Commands.PasteInputBones, FExecuteAction::CreateSP(this, &SMLDeformerInputBonesWidget::OnPasteInputBones));
	}

	void SMLDeformerInputBonesWidget::OnCopyInputBones() const
	{
		const TSharedRef<FJsonObject> JsonObject = MakeShared<FJsonObject>();
		JsonObject->SetStringField(TEXT("DataId"), TEXT("InputBones"));

		// Build the names array.
		TArray<TSharedPtr<FJsonValue>> JsonNameArray;
		for (const FName Name : EditorModel->GetEditorInputInfo()->GetBoneNames())
		{
		...
		{
			UE_LOG(LogMLDeformer, Error, TEXT("Failed to serialize JSON on Copy action in the Input Bones widget."));
			return;
		}
		FPlatformApplicationMisc::ClipboardCopy(*OutputString);
	}

	void SMLDeformerInputBonesWidget::OnPasteInputBones()
	{
		FString Text;
		FPlatformApplicationMisc::ClipboardPaste(Text);

		TSharedPtr<FJsonObject> JsonObject;
		if (GetInputWidget()->HasValidClipBoardData(TEXT("InputBones"), JsonObject) && JsonObject.IsValid())
		{
			TArray<FString> Names;
			if (!JsonObject->TryGetStringArrayField(TEXT("Names"), Names))
			{
				UE_LOG(LogMLDeformer, Error, TEXT("Failed to get bone names during paste operation."));
			}
				
			FScopedTransaction Transaction(TEXT("SMLDeformerInputBonesWidget"), LOCTEXT("PasteInputBonesText", "Paste Input Bones"), EditorModel->GetModel());
			EditorModel->GetModel()->Modify();
		
			TArray<FName> AddedBones;
			TArray<FBoneReference>& CurrentBoneNames = EditorModel->GetModel()->GetBoneIncludeList();
			for (const FString& BoneNameString : Names)
			{
				if (!BoneNameString.IsEmpty())
				{
					const FName BoneName(BoneNameString);
					CurrentBoneNames.AddUnique(BoneName);
			...
			GetInputWidget()->OnAddInputBones(AddedBones);
		}
	}
	...
	void FMLDeformerInputBonesWidgetCommands::RegisterCommands()
	{
		...
		UI_COMMAND(CopyInputBones, "Copy Input Bones List", "Copies all bones, so that you can paste them into another model's input bones list.", EUserInterfaceActionType::Button, FInputChord(EKeys::C, EModifierKey::Control));
		UI_COMMAND(PasteInputBones, "Paste Input Bones List", "Pastes a previously copied bone list. This will add bones to the existing list, not replace it fully.", EUserInterfaceActionType::Button, FInputChord(EKeys::V, EModifierKey::Control));
	}

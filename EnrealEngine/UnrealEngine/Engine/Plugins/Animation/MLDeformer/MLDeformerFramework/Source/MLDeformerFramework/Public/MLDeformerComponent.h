 ...
 */
UCLASS(MinimalAPI, Blueprintable, ClassGroup = Component, BlueprintType, meta = (BlueprintSpawnableComponent))
...
public:
...
#if WITH_EDITOR
	UE_API void PreEditChange(FProperty* Property) override;
	UE_API void PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent) override;
#endif
	// ~END UObject overrides.

	// UActorComponent overrides.
	UE_API void Activate(bool bReset=false) override;
	 ...
	 */
	UE_API void SetupComponent(UMLDeformerAsset* InDeformerAsset, USkeletalMeshComponent* InSkelMeshComponent);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "MLDeformer")
	UE_API USkeletalMeshComponent* FindSkeletalMeshComponent(const UMLDeformerAsset* const Asset) const;
	 ...
	 */
	UE_API float GetFinalMLDeformerWeight() const;
...
protected:
	...
	UE_API virtual void SetWeightInternal(const float NormalizedWeightValue);

	/** Set the ML Deformer asset. */
	UE_API virtual void SetDeformerAssetInternal(UMLDeformerAsset* const InDeformerAsset);
	...
	UE_API void BindDelegates();
	UE_API void UnbindDelegates();
	UE_API void ReleaseModelInstance();

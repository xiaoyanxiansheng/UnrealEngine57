...
public:
	UE_API UNeuralMorphModel(const FObjectInitializer& ObjectInitializer);
	...
	UE_API virtual int32 GetNumFloatsPerCurve() const override;
	UE_API virtual bool IsTrained() const override;
	// ~END UMLDeformerModel overrides.

	/**
	 * Check if only global mode should be supported or not.
	 * Models that inherit from this model can disable this if they only want the global mode.
	 * This will automatically hide the UI elements related to the mode.
	 */
	virtual bool SupportsGlobalModeOnly() const						{ return false; }
	...
	ENeuralMorphMode GetModelMode() const							{ return SupportsGlobalModeOnly() ? ENeuralMorphMode::Global : Mode; }
	...
	void SetSkinningMode(EMLDeformerSkinningMode SkinMode)			{ SkinningMode = SkinMode; }

	UE_API void UpdateMissingGroupNames();
	UE_API void SetNeuralMorphNetwork(UNeuralMorphNetwork* Net);
	void SetNumIterations(int32 InNumIterations)					{ check(InNumIterations > 0); NumIterations = InNumIterations; }

	static FName GetModePropertyName()								{ return GET_MEMBER_NAME_CHECKED(UNeuralMorphModel, Mode); }
	static FName GetSkinningModePropertyName()						{ return GET_MEMBER_NAME_CHECKED(UNeuralMorphModel, SkinningMode); }
...
public:
	 ...
	 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, AdvancedDisplay, Category = "Training Settings", meta = (EditCondition = "Mode == ENeuralMorphMode::Local", EditConditionHides))
	...
	/**
	 * The base skinning mode to use. The learned deltas will be applied on top of this basis.
	 * Please keep in mind that you use the same type of skinning on your character, otherwise the applied deltas are wrong.
	 * If you train using Dual Quaternions you should also run a Dual Quaternion skinning deformer before applying the ML Deltas on top.
	 */
	UPROPERTY(EditAnywhere, AdvancedDisplay, Category = "Training Settings")

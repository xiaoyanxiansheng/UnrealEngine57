...
public:
	 ...
	 */
	UE_API TArray<FNeuralMorphBoneGroup>& GetBoneGroups();
	UE_API const TArray<FNeuralMorphBoneGroup>& GetBoneGroups() const;
	 ...
	 */
	UE_API TArray<FNeuralMorphCurveGroup>& GetCurveGroups();
	UE_API const TArray<FNeuralMorphCurveGroup>& GetCurveGroups() const;
	 ...
	 */
	UE_API int32 CalcMaxNumGroupItems() const;
	 ...
	 */
	UE_API void GenerateBoneGroupIndices(TArray<int32>& OutBoneGroupIndices);
	 ...
	 */
	UE_API void GenerateCurveGroupIndices(TArray<int32>& OutCurveGroupIndices);
	 ...
	 */
	UE_API bool HasInvalidGroups() const;

	/**
	 * Get the skinning mode that we trained with.
	 * @return Returns the skinning mode, such as linear skinned or dual quaternion skinned.
	 */
	UE_API EMLDeformerSkinningMode GetSkinningMode() const;

	/**
	 * Set the skinning mode that was used to train with.
	 */
	UE_API void SetSkinningMode(EMLDeformerSkinningMode Mode);
...
protected:
	...
	/** The skinning mode that was trained for. */
	UPROPERTY()

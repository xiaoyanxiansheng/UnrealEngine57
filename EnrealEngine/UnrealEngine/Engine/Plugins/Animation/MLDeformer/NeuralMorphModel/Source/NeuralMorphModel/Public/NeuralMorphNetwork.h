...
public:
	...
	UE_API UNeuralMorphNetwork();
	UE_API UNeuralMorphNetwork(const FObjectInitializer& ObjectInitializer);
	UE_API UNeuralMorphNetwork(FVTableHelper& Helper);
	UE_API virtual ~UNeuralMorphNetwork();
	 ...
	 */
	UE_API int32 GetNumMorphsPerBone() const;
	 ...
	 */
	UE_API int32 GetNumGroups() const;
	 ...
	 */
	UE_API const TArrayView<const float> GetInputMeans() const;
	 ...
	 */
	UE_API UE::NNE::IModelCPU* GetGroupModel() const;
...
public:
	...
	// These are required because of the use of TSharedPtr
	UE_API UNeuralMorphNetworkInstance();
	UE_API UNeuralMorphNetworkInstance(const FObjectInitializer& ObjectInitializer);
	UE_API UNeuralMorphNetworkInstance(FVTableHelper& Helper);
	UE_API virtual ~UNeuralMorphNetworkInstance();

	/**
	 * Get the network input buffer.
	 * @return The array view of floats that represents the main network inputs.
	 */
	UE_API TArrayView<float> GetInputs();
	UE_API TArrayView<const float> GetInputs() const;

	/**
	 * Get the inputs for the group MLP.
	 * @return The array view of floats that represent the group network inputs.
	 */
	UE_API TArrayView<float> GetGroupInputs();
	UE_API TArrayView<const float> GetGroupInputs() const;

	/**
	 * Get the network output buffer.
	 * @return The array view of floats that represents the network outputs.
	 */
	UE_API TArrayView<float> GetOutputs();
	UE_API TArrayView<const float> GetOutputs() const;

	/**
	 * Get the neural network this is an instance of.
	 * @return A reference to the neural network.
	 */
	UE_API const UNeuralMorphNetwork& GetNeuralNetwork() const;
...
private:
	 ...
	 */
	UE_API void Init(UNeuralMorphNetwork* InNeuralNetwork);

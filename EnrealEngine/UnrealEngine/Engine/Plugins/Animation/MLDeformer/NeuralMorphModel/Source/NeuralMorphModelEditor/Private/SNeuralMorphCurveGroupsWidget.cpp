...
namespace UE::NeuralMorphModel
{
	...
	{
		UI_COMMAND(CreateGroup, "Create New Group", "Create a new curve group.", EUserInterfaceActionType::Button, FInputChord(EKeys::Insert));
		UI_COMMAND(DeleteSelectedItems, "Delete Selected Items", "Deletes the selected curves and/or groups.", EUserInterfaceActionType::Button, FInputChord(EKeys::Delete));
		UI_COMMAND(ClearGroups, "Clear All Groups", "Clears the entire list of curve groups.", EUserInterfaceActionType::Button, FInputChord());
		UI_COMMAND(AddCurveToGroup, "Add Curves To Group", "Add new curves to the group.", EUserInterfaceActionType::Button, FInputChord());
		UI_COMMAND(CopyCurveGroups, "Copy Input Curve Groups List", "Copies all curve groups, so that you can paste them into another model's input curve groups list.", EUserInterfaceActionType::Button, FInputChord(EKeys::C, EModifierKey::Control));
		UI_COMMAND(PasteCurveGroups, "Paste Input Curve Groups List", "Pastes a previously copied curve group list. This will add curve groups to the existing list, not replace it fully.", EUserInterfaceActionType::Button, FInputChord(EKeys::V, EModifierKey::Control));
	}
	...
	void SNeuralMorphCurveGroupsWidget::BindCommands(TSharedPtr<FUICommandList> CommandList)
	{
		...
		CommandList->MapAction(GroupCommands.CopyCurveGroups, FExecuteAction::CreateSP(this, &SNeuralMorphCurveGroupsWidget::OnCopyCurveGroups));
		CommandList->MapAction(GroupCommands.PasteCurveGroups, FExecuteAction::CreateSP(this, &SNeuralMorphCurveGroupsWidget::OnPasteCurveGroups));
	}
	...
	void SNeuralMorphCurveGroupsWidget::OnCopyCurveGroups() const
	{
		const UNeuralMorphModel* NeuralMorphModel = EditorModel->GetNeuralMorphModel();
		if (NeuralMorphModel->GetCurveGroups().IsEmpty())
		{
			return;
		}

		const TSharedRef<FJsonObject> JsonObject = MakeShared<FJsonObject>();
		JsonObject->SetStringField(TEXT("DataId"), TEXT("CurveGroups"));

		// Create each curve group.
		TArray<TSharedPtr<FJsonValue>> CurveGroupsArray;
		for (const FNeuralMorphCurveGroup& CurveGroup : NeuralMorphModel->GetCurveGroups())
		{
			TSharedRef<FJsonObject> JsonCurveroup = MakeShared<FJsonObject>();
			JsonCurveroup->SetStringField(TEXT("GroupName"), CurveGroup.GroupName.ToString());
    
			TArray<TSharedPtr<FJsonValue>> CurveNames;
			for (const FMLDeformerCurveReference& CurveName : CurveGroup.CurveNames)
			{
				if (CurveName.CurveName.IsValid() && !CurveName.CurveName.IsNone())
				{
					CurveNames.Add(MakeShared<FJsonValueString>(CurveName.CurveName.ToString()));
				}
			}
	
			JsonCurveroup->SetArrayField(TEXT("CurveNames"), CurveNames);
			CurveGroupsArray.Add(MakeShared<FJsonValueObject>(JsonCurveroup));
		}

		JsonObject->SetArrayField(TEXT("CurveGroups"), CurveGroupsArray);
		...
		{
			UE_LOG(LogMLDeformer, Error, TEXT("Failed to serialize JSON on Copy action in the Input Curve Groups widget."));
			return;
		}
		FPlatformApplicationMisc::ClipboardCopy(*OutputString);
	}

	void SNeuralMorphCurveGroupsWidget::OnPasteCurveGroups()
	{
		FString Text;
		FPlatformApplicationMisc::ClipboardPaste(Text);

		TSharedPtr<FJsonObject> JsonObject;
		if (GetInputWidget()->HasValidClipBoardData(TEXT("CurveGroups"), JsonObject) && JsonObject.IsValid())
		{
			const TArray<TSharedPtr<FJsonValue>>* CurveGroupsArray;	
			if (JsonObject->TryGetArrayField(TEXT("CurveGroups"), CurveGroupsArray))
			{
				UNeuralMorphModel* NeuralMorphModel = EditorModel->GetNeuralMorphModel();
				FScopedTransaction Transaction(TEXT("SNeuralMorphCurveGroupsWidget"), LOCTEXT("PasteCurveGroupText", "Paste Curve Groups"), NeuralMorphModel);
				NeuralMorphModel->Modify();
	
				for (const TSharedPtr<FJsonValue>& GroupValue : *CurveGroupsArray)
				{
					TSharedPtr<FJsonObject> GroupObject = GroupValue->AsObject();
					if (!GroupObject.IsValid())
					{
						UE_LOG(LogNeuralMorphModel, Error, TEXT("Skipping invalid group object when pasting curve groups."));
						continue;
					}
					...
					{
						UE_LOG(LogNeuralMorphModel, Error, TEXT("Failed to read curve group name when pasting curve groups."));
						continue;
					}
					
					const TArray<TSharedPtr<FJsonValue>>* CurveArray;
					if (!GroupObject->TryGetArrayField(TEXT("CurveNames"), CurveArray))
					{
						UE_LOG(LogNeuralMorphModel, Error, TEXT("Failed to read curve group curve names array when pasting curve groups."));
						continue;
					}

					// Check if the copied group name is unique or if we have to generate a new one.
					bool bIsUniqueName = true;
					for (const FNeuralMorphCurveGroup& Group : NeuralMorphModel->CurveGroups)
					{
					...
					const FName GroupName = bIsUniqueName ? FName(GroupNameString) : EditorModel->GenerateUniqueCurveGroupName();
					FNeuralMorphCurveGroup& CurveGroup = NeuralMorphModel->CurveGroups.AddDefaulted_GetRef();
					CurveGroup.GroupName = GroupName;

					// Add the curves to the newly created group.
					for (const TSharedPtr<FJsonValue>& CurveNameValue : *CurveArray)
					{
						const FString CurveName = CurveNameValue->AsString();
						CurveGroup.CurveNames.AddUnique(FName(CurveName));
					}
				}
			}
	...
	TSharedPtr<SWidget> SNeuralMorphCurveGroupsWidget::CreateContextMenuWidget() const
	{
		...
		Menu.BeginSection("CopyPasteActions", LOCTEXT("CopyPasteActionsHeading", "Copy/Paste Actions"));
		{
			...
			{
				Menu.AddMenuEntry(Actions.CopyCurveGroups);
			}

			TSharedPtr<FJsonObject> JsonObject;
			if (InputWidget->HasValidClipBoardData(TEXT("CurveGroups"), JsonObject))
			{
				Menu.AddMenuEntry(Actions.PasteCurveGroups);
			}
		}
		Menu.EndSection();
	...
	void SNeuralMorphCurveGroupsWidget::OnDeleteSelectedItems()
	{
		...
		// Check if the selection contains curves and/or groups.
	...
	void SNeuralMorphCurveGroupsWidget::Refresh(bool bBroadcastPropertyChanged)
	{ 

...
FVector FControlRigSpline::TangentAtParam(const float InParam) const
{
	...
	const FVector TangentStart = SplineData->SamplesArray[IndexNext].GetTranslation() - SplineData->SamplesArray[IndexPrev].GetTranslation();
	if (IndexNext + 1 < SplineData->SamplesArray.Num())
	{
		const FVector TangentEnd = SplineData->SamplesArray[IndexNext+1].GetTranslation() - SplineData->SamplesArray[IndexNext].GetTranslation();

		const FQuat FullQuat = FRigVMMathLibrary::FindQuatBetweenVectors(TangentStart, TangentEnd);
		const FQuat PartialQuat = FQuat::Slerp(FQuat::Identity, FullQuat, ULocal);

		const FVector Result = PartialQuat.RotateVector(TangentStart);

...
struct FControlRigSplineImpl
{
	...
	UE_API virtual ~FControlRigSplineImpl();
...
struct FControlRigSpline 
{
	...
	UE_API FControlRigSpline(const FControlRigSpline& InOther);
	UE_API FControlRigSpline& operator =(const FControlRigSpline& InOther);
	...
	*/
	UE_API void SetControlPoints(const TArrayView<const FVector>& InPoints, const ESplineType SplineMode = ESplineType::BSpline, const bool bInClosed = false, const int32 SamplesPerSegment = 16, const float Compression = 1.f, const float Stretch = 1.f);
	...
	* @param Stretch The allowed length stretch (1.f being do not allow stretch). If 0, no restriction wil be applied.
	*/
	UE_API void SetControlTransforms(const TArrayView<const FTransform>& InTransforms, const ESplineType SplineMode = ESplineType::BSpline, const bool bInClosed = false, const int32 SamplesPerSegment = 16, const float Compression = 1.f, const float Stretch = 1.f);
	...
	*/
	UE_API FVector PositionAtParam(const float InParam) const;
	...
	*/
	UE_API FTransform TransformAtParam(const float InParam) const;
	...
	*/
	UE_API FVector TangentAtParam(const float InParam) const;
	...
	*/
	UE_API float LengthAtParam(const float InParam) const;
};

...
void ULiveLinkTimecodeProvider::ClearFrameHistory()
{
	FScopeLock ScopeLock(&SubjectFrameLock);
...
void ULiveLinkTimecodeProvider::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyChangedEvent.Property && PropertyChangedEvent.Property->GetFName() == GET_MEMBER_NAME_CHECKED(ULiveLinkTimecodeProvider, SubjectKey))
	{
		...
		if (RegisterForFrameDataReceivedHandle.IsValid() && RegisteredSubjectKey.SubjectName != SubjectKey.SubjectName)
		{
			UnregisterSubject();
			RegisterSubject();
		}
		// If was waiting for the subject to be added to the client
		else if (LiveLinkClient && LiveLinkClient->IsSubjectValid(SubjectKey.SubjectName))
		{
...
void ULiveLinkTimecodeProvider::InitClient()
{
	...
	if (ModularFeatures.IsModularFeatureAvailable(ILiveLinkClient::ModularFeatureName))
	{
		...
		if (LiveLinkClient->IsSubjectValid(SubjectKey.SubjectName))
		{
			OnLiveLinkSubjectAdded(SubjectKey);
		}
	}
...
void ULiveLinkTimecodeProvider::OnLiveLinkSubjectAdded(FLiveLinkSubjectKey InSubjectKey)
{
	...
	{
		// This provider might have been initialized with an invalid key because the subject wasn't live at the time of creation. 
		// So we update the SubjectKey to match the new subject since it's now been added.

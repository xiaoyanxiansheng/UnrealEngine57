 ...
 * 
 */
namespace LiveLinkLocationSwitchPreprocessor
{
	float LiveLinkAxisToVectorMember(ELiveLinkAxis InAxis, const FVector& Origin)
	{
		const uint8 Value = static_cast<uint8>(InAxis) % 3;
	...
	void SwitchTranslation(FVector& Translation, ELiveLinkAxis FrontAxis, ELiveLinkAxis RightAxis, ELiveLinkAxis UpAxis, bool bUseOffsetPosition, FVector OffsetPosition, bool bUseOffsetOrientation, FRotator OffsetOrientation)
	{
		...
		if (bUseOffsetOrientation)
		{
			NewLocation = OffsetOrientation.RotateVector(NewLocation); //Note this is executed after the axes are flipped/swapped
		}
...
/**
 * ULiveLinkTransformAxisSwitchPreProcessor::FLiveLinkLocatorAxisSwitchPreProcessorWorker
 */

TSubclassOf<ULiveLinkRole> ULiveLinkLocatorAxisSwitchPreProcessor::FLiveLinkLocatorAxisSwitchPreProcessorWorker::GetRole() const
	{
		return ULiveLinkLocatorRole::StaticClass();
	}

bool ULiveLinkLocatorAxisSwitchPreProcessor::FLiveLinkLocatorAxisSwitchPreProcessorWorker::PreProcessFrame(FLiveLinkFrameDataStruct& InOutFrame) const
{
	FLiveLinkLocatorFrameData& LocatorData = *InOutFrame.Cast<FLiveLinkLocatorFrameData>();

	for (FVector& Location : LocatorData.Locators)
	{
		LiveLinkLocationSwitchPreprocessor::SwitchTranslation(Location, FrontAxis, RightAxis, UpAxis, bUseOffsetPosition, OffsetPosition, bUseOffsetOrientation, OffsetOrientation);
	}
	return true;
}

ULiveLinkFramePreProcessor::FWorkerSharedPtr ULiveLinkLocatorAxisSwitchPreProcessor::FetchWorker()
{
	if (!Instance.IsValid())
	{
		Instance = MakeShared<FLiveLinkLocatorAxisSwitchPreProcessorWorker, ESPMode::ThreadSafe>();
		Instance->FrontAxis = FrontAxis;
		Instance->RightAxis = RightAxis;
		Instance->UpAxis = UpAxis;
		Instance->bUseOffsetPosition = bUseOffsetPosition;
...
TSubclassOf<ULiveLinkRole> ULiveLinkLocatorAxisSwitchPreProcessor::GetRole() const
{
	return ULiveLinkLocatorRole::StaticClass();
}

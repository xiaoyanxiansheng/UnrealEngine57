...
public:
	...
	virtual void StartupModule() override
	{
		...
		StyleSet->Set("PlacementBrowser.Icons.VirtualProduction", new IMAGE_PLUGIN_BRUSH_SVG(TEXT("Starship/VirtualProduction"), Icon16x16));
...
private:
	...
	void RegisterCustomizations()
	{
		...
		PropertyEditorModule.RegisterCustomClassLayout(ULiveLinkBroadcastComponent::StaticClass()->GetFName(), FOnGetDetailCustomizationInstance::CreateStatic(&FLiveLinkBroadcastComponentDetailCustomization::MakeInstance));
	...
	void UnregisterCustomizations()
	{
		if (UObjectInitialized() && !IsEngineExitRequested())
		{
			...
			if (PropertyEditorModule)
			{
				...
				PropertyEditorModule->UnregisterCustomClassLayout(ULiveLinkBroadcastComponent::StaticClass()->GetFName());
	...
	// Register the livelink data preview actor.
		...
		auto RegisterPlaceActors = [&]() -> void
			{
				...
				if (!Info)
				{
					UE_LOG(LogLiveLinkEditor, Warning, TEXT("Could not find or create VirtualProduction Place Actor Category"));
					return;
				}
	...
	// Unregister the data preview actor placement item.
	...
	// Get the virtual production actor category info.
	const FPlacementCategoryInfo* GetVirtualProductionCategoryRegisteredInfo() const
	{
		...
		else
		{
			FPlacementCategoryInfo Info(
				...
				FSlateIcon(StyleSet->GetStyleSetName(), "PlacementBrowser.Icons.VirtualProduction"),
...
private:
	...
	// Registered livelink data preview actor.

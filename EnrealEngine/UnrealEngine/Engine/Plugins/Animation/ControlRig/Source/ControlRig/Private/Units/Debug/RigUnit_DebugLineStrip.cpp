...
FRigUnit_DebugLineStripItemSpace_Execute()
{
	...
	if (Space.IsValid())
	{
		...
		ExecuteContext.GetDrawInterface()->DrawLineStrip(WorldOffset, PointsTransformed, Color, Thickness, DebugDrawSettings.DepthPriority, DebugDrawSettings.Lifetime);
	}
	else
	{
		ExecuteContext.GetDrawInterface()->DrawLineStrip(WorldOffset, TArrayView<const FVector>(Points.GetData(), Points.Num()), Color, Thickness, DebugDrawSettings.DepthPriority, DebugDrawSettings.Lifetime);
	}
}

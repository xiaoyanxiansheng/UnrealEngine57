...
UControlRig::UControlRig(const FObjectInitializer& ObjectInitializer)
	...
	, bHierarchyInitializedThisFrame(false)
...
void UControlRig::ResetRecordedTransforms(const FName& InEventName)
{
	...
	{
		if(Hierarchy->bRecordInstructionsAtRuntime)
		{
			FControlRigExecuteContext& PublicData = GetRigVMExtendedExecuteContext().GetPublicData<FControlRigExecuteContext>();
			PublicData.ResetInstructionRecords();
		}
	}
}
...
void UControlRig::Evaluate_AnyThread()
{
	if (bIsAdditive)
	{
		...
		Hierarchy->ForEach([Hierarchy](FRigBaseElement* Element) -> bool
		{
			...
			{
				Hierarchy->SetTransform(NullElement, Hierarchy->GetTransform(NullElement, ERigTransformType::InitialLocal), ERigTransformType::CurrentLocal, true);
			}
			return true;
		});
	...
	if (!HasAnyFlags(RF_ClassDefaultObject))
	{
		if (!IsRigModuleInstance())
		{
			...
			// copy hierarchy
			{
				...
				bHierarchyInitializedThisFrame = true;
			}
...
{
	return Super::CanExecute() && GetHierarchy() != nullptr;
}
...
bool UControlRig::Execute(const FName& InEventName)
{
	...
	TConstArrayView<FName> LocalEventQueueToRun = EventQueueToRun.IsEmpty() ? EventQueue : EventQueueToRun;
	...
	if (UControlRig* CDO = GetClass()->GetDefaultObject<UControlRig>())
	{
		...
		DebugInfo.CopyFrom(CDO->DebugInfo);
	}
	...
	if(Hierarchy)
	{
...
#if WITH_EDITOR
		bool bRecordInstructionsAtRuntime = true;
		if(const UObject* Outer = GetOuter())
		{
			if(Outer->IsA<UControlRigComponent>())
			{
				bRecordInstructionsAtRuntime = false;
			}
		}
		...
		// only record instructions when running one of the forward solve events for now
		if (!bIsPreForwardSolve && !bIsForwardSolve && !bIsPostForwardSolve)
		{
			bRecordInstructionsAtRuntime = false;
		}
		
		RecordTransformsPerInstructionGuard = MakeShared<TGuardValue<bool>>(Hierarchy->bRecordInstructionsAtRuntime, bRecordInstructionsAtRuntime);
		if (!bIsEventInQueue || bIsEventFirstInQueue)
		{
	...
	#endif
					if (!bHierarchyInitializedThisFrame)
					{
						if ((bCopyHierarchyBeforeConstruction || bResetInitialTransformsBeforeConstruction))
						{
...
void UControlRig::SwitchToParent(const FRigElementKey& InElementKey, const FRigElementKey& InNewParentKey, bool bInitial, bool bAffectChildren)
{
	...
	{
		// when simply executing space switching we need to make sure it runs through,
		// and during that we'll only respect parent child relationships (while ignoring VM dependencies).
		static const FEmptyRigDependenciesProvider EmptyRigDependenciesProvider;
		DynamicHierarchy->SwitchToParent(InElementKey, InNewParentKey, bInitial, bAffectChildren, EmptyRigDependenciesProvider, nullptr);
	}
}
		...
		if(DynamicHierarchy)
		{
			if (bIsAdditive)
			{
				...
				if (ControlIndex != INDEX_NONE)
				{
					...
					const FTransform OffsetTransform = DynamicHierarchy->GetControlOffsetTransform(ControlElement, ERigTransformType::CurrentLocal);
					const FTransform LocalTransform = OffsetLocalTransform.GetRelativeTransform(OffsetTransform);
...
void UControlRig::PostInitInstance(URigVMHost* InCDO)
{
	...
	// set up the hierarchy
	{
		...
		// did save their hierarchies. If that's the case, let's mark them as garbage and rename them before creating our own hierarchy.
		if (!HasAnyFlags(RF_ClassDefaultObject))
		{
			FGCScopeGuard Guard;
			UObject* ObjectFound = StaticFindObjectFast(URigHierarchy::StaticClass(), this, TEXT("DynamicHierarchy"), EFindObjectFlags::ExactClass, RF_DefaultSubObject);
			if (ObjectFound)
			{
...
void UControlRig::SetDynamicHierarchy(TObjectPtr<URigHierarchy> InHierarchy)
{
	...
	if (DynamicHierarchy && DynamicHierarchy->GetOuter() == this)
	{

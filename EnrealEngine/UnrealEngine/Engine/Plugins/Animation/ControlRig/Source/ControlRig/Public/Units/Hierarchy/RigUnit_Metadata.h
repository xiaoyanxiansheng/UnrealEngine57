...
protected:
	...
	UE_API const TArray<TRigVMTypeIndex>& GetValueTypes(const FRigVMRegistryHandle& InRegistry) const;
	...
	mutable TArray<TRigVMTypeIndex> ValueTypes;
...
protected:
	...
	static void GetMetadataDispatch(FRigVMExtendedExecuteContext& InContext, FRigVMMemoryHandleArray Handles, FRigVMPredicateBranchArray Predicates)
	{
		...
		// unpack the memory
		const FRigElementKey& Item = *reinterpret_cast<const FRigElementKey*>(Handles[Factory->ItemArgIndex].GetInputData());
		const FName& Name = *reinterpret_cast<const FName*>(Handles[Factory->NameArgIndex].GetInputData());
		const ERigMetaDataNameSpace NameSpace = *reinterpret_cast<const ERigMetaDataNameSpace*>(Handles[Factory->NameSpaceArgIndex].GetInputData());
		FCachedRigElement& Cache = *reinterpret_cast<FCachedRigElement*>(Handles[Factory->CacheArgIndex].GetPrivateData(InContext.GetSlice().GetIndex()));
		const ValueType& Default = *reinterpret_cast<const ValueType*>(Handles[Factory->DefaultArgIndex].GetInputData());
		ValueType& Value = *reinterpret_cast<ValueType*>(Handles[Factory->ValueArgIndex].GetOutputData());
		bool& Found = *reinterpret_cast<bool*>(Handles[Factory->FoundArgIndex].GetOutputData());
		...
		if (const MetadataType* Md = Cast<MetadataType>(FindMetadata(InContext, Item, Name, EnumValue, NameSpace, Cache)))
		{
			...
			if (ExecuteContext.Hierarchy->bRecordInstructionsAtRuntime)
			{
				const FName VMName = ExecuteContext.GetRigModuleInstance() ? ExecuteContext.GetRigModuleInstance()->Name : NAME_None;
				const FInstructionRecord Record(ExecuteContext.GetInstructionIndex(), InContext.GetSlice().GetIndex(), Cache.GetIndex(), VMName, Md->GetName());
				ExecuteContext.AddReadInstructionRecord(Record);
			}
#endif			
		}
		else
...
protected:
	...
	static void SetMetadataDispatch(FRigVMExtendedExecuteContext& InContext, FRigVMMemoryHandleArray Handles, FRigVMPredicateBranchArray Predicates)
	{
		...
		const FRigElementKey& Item = *(const FRigElementKey*)Handles[Factory->ItemArgIndex].GetInputData();
		const FName& Name = *(const FName*)Handles[Factory->NameArgIndex].GetInputData();
		const ERigMetaDataNameSpace NameSpace = *(const ERigMetaDataNameSpace*)Handles[Factory->NameSpaceArgIndex].GetInputData();
		FCachedRigElement& Cache = *(FCachedRigElement*)Handles[Factory->CacheArgIndex].GetPrivateData(InContext.GetSlice().GetIndex());
		...
		if (MetadataType* Md = Cast<MetadataType>(FindOrAddMetadata(ExecuteContext, Item, Name, EnumValue, NameSpace, Cache)))
		{
			...
			if (ExecuteContext.Hierarchy->bRecordInstructionsAtRuntime)
			{
				const FName VMName = ExecuteContext.GetRigModuleInstance() ? ExecuteContext.GetRigModuleInstance()->Name : NAME_None;
				const FInstructionRecord Record(ExecuteContext.GetInstructionIndex(), InContext.GetSlice().GetIndex(), Cache.GetIndex(), VMName, Md->GetName());
				ExecuteContext.AddWrittenInstructionRecord(Record);
			}
#endif

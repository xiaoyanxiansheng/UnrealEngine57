...
struct FRegionScaleFactors
{
	...
	FRegionScaleFactors(float InPositiveWidth, float InNegativeWidth, float InPositiveHeight, float InNegativeHeight)
		: PositiveWidth(InPositiveWidth)
		, NegativeWidth(InNegativeWidth)
		, PositiveHeight(InPositiveHeight)
		, NegativeHeight(InNegativeHeight)
	{
	}
...
struct FRigUnit_SphericalPoseReader: public FRigUnit_HighlevelBaseMutable
{
	...
	static UE_API void RemapAndConvertInputs(
	...
	static UE_API void EvaluateInterpolation(FRigUnit_SphericalPoseReader& Node, const FTransform& DriverTransform);
	
	static UE_API void DrawDebug(FRigUnit_SphericalPoseReader& PoseReader);

	static UE_API float CalcOutputParam(
		const FEllipseQuery& InnerEllipseResults,
		const FEllipseQuery& OuterEllipseResults);
	
	static UE_API void DistanceToEllipse(
		const float InX,
		const float InY,
		const float SizeX,
		const float SizeY,
	...
	static UE_API float RemapRange(
		const float T,
		const float AStart,
		const float AEnd,
		const float BStart,

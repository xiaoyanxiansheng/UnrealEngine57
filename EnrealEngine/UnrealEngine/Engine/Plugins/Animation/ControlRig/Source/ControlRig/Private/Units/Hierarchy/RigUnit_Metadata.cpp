...
const TArray<FRigVMTemplateArgumentInfo>& FRigDispatch_MetadataBase::GetArgumentInfos(FRigVMRegistryHandle& InRegistry) const
{
	if(CachedArgumentInfos.IsEmpty())
	{
		ItemArgIndex = CachedArgumentInfos.Emplace(ItemArgName, ERigVMPinDirection::Input, InRegistry->GetTypeIndex_NoLock<FRigElementKey>());
		NameArgIndex = CachedArgumentInfos.Emplace(NameArgName, ERigVMPinDirection::Input, RigVMTypeUtils::TypeIndex::FName);
		NameSpaceArgIndex = CachedArgumentInfos.Emplace(NameSpaceArgName, ERigVMPinDirection::Input, InRegistry->GetTypeIndex_NoLock<ERigMetaDataNameSpace>());
		CacheArgIndex = CachedArgumentInfos.Emplace(CacheArgName, ERigVMPinDirection::Hidden, InRegistry->GetTypeIndex_NoLock<FCachedRigElement>());
	};
...
const TArray<TRigVMTypeIndex>& FRigDispatch_MetadataBase::GetValueTypes(const FRigVMRegistryHandle& InRegistry) const
{
	if(ValueTypes.IsEmpty())
	{
		ValueTypes = {
			...
			InRegistry->GetTypeIndex_NoLock<FRotator>(false),
			InRegistry->GetTypeIndex_NoLock<FQuat>(false),
			InRegistry->GetTypeIndex_NoLock<FTransform>(false),
			InRegistry->GetTypeIndex_NoLock<FLinearColor>(false),
			InRegistry->GetTypeIndex_NoLock<FRigElementKey>(false),
			...
			InRegistry->GetTypeIndex_NoLock<FVector>(true),
			InRegistry->GetTypeIndex_NoLock<FRotator>(true),
			InRegistry->GetTypeIndex_NoLock<FQuat>(true),
			InRegistry->GetTypeIndex_NoLock<FTransform>(true),
			InRegistry->GetTypeIndex_NoLock<FLinearColor>(true),
			InRegistry->GetTypeIndex_NoLock<FRigElementKey>(true)
		};

		// remove invalid indices
		ValueTypes.Remove(TRigVMTypeIndex());
	}
	return ValueTypes;
}
...
const TArray<FRigVMTemplateArgumentInfo>& FRigDispatch_GetMetadata::GetArgumentInfos(FRigVMRegistryHandle& InRegistry) const
{
	if(ValueArgIndex == INDEX_NONE)
	{
		CachedArgumentInfos = Super::GetArgumentInfos(InRegistry); 
		DefaultArgIndex = CachedArgumentInfos.Emplace(DefaultArgName, ERigVMPinDirection::Input, GetValueTypes(InRegistry));
		ValueArgIndex = CachedArgumentInfos.Emplace(ValueArgName, ERigVMPinDirection::Output, GetValueTypes(InRegistry));
...
FRigVMFunctionPtr FRigDispatch_GetMetadata::GetDispatchFunctionImpl(const FRigVMTemplateTypeMap& InTypes, const FRigVMRegistryHandle& InRegistry) const
{
	...
	if(ValueTypeIndex == InRegistry->GetTypeIndex_NoLock<FRotator>(false))
	{
	...
	if(ValueTypeIndex == InRegistry->GetTypeIndex_NoLock<FLinearColor>(false))
	{
		return &FRigDispatch_GetMetadata::GetMetadataDispatch<FLinearColor, FRigLinearColorMetadata, ERigMetadataType::LinearColor>;
	}
	if(ValueTypeIndex == InRegistry->GetTypeIndex_NoLock<FRigElementKey>(false))
	{
	...
	if(ValueTypeIndex == InRegistry->GetTypeIndex_NoLock<FVector>(true))
	{
		return &FRigDispatch_GetMetadata::GetMetadataDispatch<TArray<FVector>, FRigVectorArrayMetadata, ERigMetadataType::VectorArray>;
	}
	if(ValueTypeIndex == InRegistry->GetTypeIndex_NoLock<FRotator>(true))
	{
	...
	if(ValueTypeIndex == InRegistry->GetTypeIndex_NoLock<FQuat>(true))
	{
		return &FRigDispatch_GetMetadata::GetMetadataDispatch<TArray<FQuat>, FRigQuatArrayMetadata, ERigMetadataType::QuatArray>;
	}
	if(ValueTypeIndex == InRegistry->GetTypeIndex_NoLock<FTransform>(true))
	{
	...
	if(ValueTypeIndex == InRegistry->GetTypeIndex_NoLock<FLinearColor>(true))
	{
	...
	if(ValueTypeIndex == InRegistry->GetTypeIndex_NoLock<FRigElementKey>(true))
	{
...
const TArray<FRigVMTemplateArgumentInfo>& FRigDispatch_SetMetadata::GetArgumentInfos(FRigVMRegistryHandle& InRegistry) const
{
	if(ValueArgIndex == INDEX_NONE)
	{
		CachedArgumentInfos = Super::GetArgumentInfos(InRegistry); 
		ValueArgIndex = CachedArgumentInfos.Emplace(ValueArgName, ERigVMPinDirection::Input, GetValueTypes(InRegistry));
...
FRigVMFunctionPtr FRigDispatch_SetMetadata::GetDispatchFunctionImpl(const FRigVMTemplateTypeMap& InTypes, const FRigVMRegistryHandle& InRegistry) const
{
	const TRigVMTypeIndex& ValueTypeIndex = InTypes.FindChecked(TEXT("Value"));
...
const TArray<FRigVMTemplateArgumentInfo>& FRigDispatch_GetModuleMetadata::GetArgumentInfos(FRigVMRegistryHandle& InRegistry) const
{
	if(ValueArgIndex == INDEX_NONE)
	{
		NameArgIndex = CachedArgumentInfos.Emplace(NameArgName, ERigVMPinDirection::Input, RigVMTypeUtils::TypeIndex::FName);
...
FRigVMFunctionPtr FRigDispatch_GetModuleMetadata::GetDispatchFunctionImpl(const FRigVMTemplateTypeMap& InTypes, const FRigVMRegistryHandle& InRegistry) const
{
	const TRigVMTypeIndex& ValueTypeIndex = InTypes.FindChecked(TEXT("Value"));
...
const TArray<FRigVMTemplateArgumentInfo>& FRigDispatch_SetModuleMetadata::GetArgumentInfos(FRigVMRegistryHandle& InRegistry) const
{
	if(ValueArgIndex == INDEX_NONE)
	{
		NameArgIndex = CachedArgumentInfos.Emplace(NameArgName, ERigVMPinDirection::Input, RigVMTypeUtils::TypeIndex::FName);
...
FRigVMFunctionPtr FRigDispatch_SetModuleMetadata::GetDispatchFunctionImpl(const FRigVMTemplateTypeMap& InTypes, const FRigVMRegistryHandle& InRegistry) const
{
	const TRigVMTypeIndex& ValueTypeIndex = InTypes.FindChecked(TEXT("Value"));

...
bool UFKControlRig::Execute_Internal(const FName& InEventName)
{
	...
	if(URigHierarchy* Hierarchy = GetHierarchy())
	{
		...
		Hierarchy->bRecordInstructionsAtRuntime = false;
	}
#endif	
...
void UFKControlRig::CreateRigElements(const FReferenceSkeleton& InReferenceSkeleton, const USkeleton* InSkeleton)
{
	...
	{
		// determine the max element name length
		int32 MaxElementName = GetHierarchy()->GetMaxNameLength();
		for (int32 Index = 0; Index < InitializationOptions.CurveNames.Num(); ++Index)
		{
			MaxElementName = FMath::Max<int32>(MaxElementName, InitializationOptions.CurveNames[Index].GetStringLength() + 14 ); // _CURVE_CONTROL suffix  
		}
		if (InSkeleton)
		{
			InSkeleton->ForEachCurveMetaData([&MaxElementName](const FName& InCurveName, const FCurveMetaData&)
			{
				MaxElementName = FMath::Max<int32>(MaxElementName, InCurveName.GetStringLength() + 14 ); // _CURVE_CONTROL suffix  
			});
		}

		const TArray<FMeshBoneInfo> BoneInfos = InReferenceSkeleton.GetRefBoneInfo();
		for (const FMeshBoneInfo& BoneInfo : BoneInfos)
		{
			MaxElementName = FMath::Max<int32>(MaxElementName, BoneInfo.Name.GetStringLength() + 8 ); // _CONTROL suffix  
		}
		GetHierarchy()->SetMaxNameLength(MaxElementName);

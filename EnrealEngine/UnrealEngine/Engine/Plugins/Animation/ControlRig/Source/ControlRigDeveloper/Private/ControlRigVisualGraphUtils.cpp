...
(
	...
	FConsoleCommandWithWorldAndArgsDelegate::CreateLambda([](const TArray<FString>& InParams, UWorld* InWorld)
	{
		...
		{
			if(UObject* Object = FindObject<UObject>(nullptr, *ObjectPathName, EFindObjectFlags::None))
			{
...
FString FControlRigVisualGraphUtils::DumpRigHierarchyToDotGraph(URigHierarchy* InHierarchy, const FName& InEventName)
{
	...
	if(!InEventName.IsNone())
	{
		...
		{
			const FRigDependenciesProviderForControlRig DependenciesProvider(CR, InEventName);
			TRigHierarchyDependencyMap Dependencies = DependenciesProvider.GetRigHierarchyDependencies();

			for(const TRigHierarchyDependencyMapPair& Dependency : Dependencies)
			{
				if (!Dependency.Key.IsElement())
				{
					continue;
				}
				
				const int32 TargetElementIndex = Dependency.Key.Index;
				
				for(const FRigHierarchyRecord SourceElementRecord : Dependency.Value)
				{
					if (!SourceElementRecord.IsElement())
					{
						continue;
					}
					
					FName EdgeName = *FString::Printf(TEXT("Dependency_%d_%d"), SourceElementRecord.Index, TargetElementIndex);
					...
					const FName SourceNodeName = Local::GetNodeNameForElementIndex(SourceElementRecord.Index);

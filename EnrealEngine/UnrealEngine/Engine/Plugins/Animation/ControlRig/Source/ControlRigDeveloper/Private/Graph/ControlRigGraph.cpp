...
void UControlRigGraph::InitializeFromAsset(FRigVMAssetInterfacePtr InAsset)
{
	DECLARE_SCOPE_HIERARCHICAL_COUNTER_FUNC()

	Super::InitializeFromAsset(InAsset);

	const FControlRigAssetInterfacePtr ControlRigBlueprint(InAsset->GetObject());
	URigHierarchy* Hierarchy = ControlRigBlueprint->GetHierarchy();

	if(UControlRig* ControlRig = Cast<UControlRig>(InAsset->GetObjectBeingDebugged()))
	{
	...
	if(Hierarchy)
	{
		CacheNameLists(Hierarchy, &ControlRigBlueprint->GetDrawContainer(), ControlRigBlueprint->GetShapeLibraries());
	}
}
...
const TArray<TSharedPtr<FRigVMStringWithTag>>* UControlRigGraph::GetElementNameList(ERigElementType InElementType) const
{
	...
	{
		const FControlRigAssetInterfacePtr Blueprint(GetAsset().GetObject());
		...
		MutableThis->CacheNameLists(Hierarchy, &Blueprint->GetDrawContainer(), Blueprint->GetShapeLibraries());
	}
...
const TArray<TSharedPtr<FRigVMStringWithTag>> UControlRigGraph::GetSelectedElementsNameList() const
{
	...
	const UControlRigBlueprint* Blueprint = CastChecked<UControlRigBlueprint>(GetAsset()->GetObject());
	if(Blueprint == nullptr)
	{
		return Result;
	}
...
{
	UControlRigBlueprint* RigVMBlueprint = CastChecked<UControlRigBlueprint>(InEdGraph->GetAsset()->GetObject());

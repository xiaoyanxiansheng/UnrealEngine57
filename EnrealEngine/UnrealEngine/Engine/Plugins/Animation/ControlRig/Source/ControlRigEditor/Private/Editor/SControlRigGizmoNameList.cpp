...
void SControlRigShapeNameList::Construct(const FArguments& InArgs, FRigControlElement* ControlElement, FControlRigAssetInterfacePtr InBlueprint)
{
...
void SControlRigShapeNameList::Construct(const FArguments& InArgs, TArray<FRigControlElement*> ControlElements, FControlRigAssetInterfacePtr InBlueprint)
{
...
FText SControlRigShapeNameList::GetNameListText() const
{
	...
	{
		const int32 ControlIndex = Blueprint->GetHierarchy()->GetIndex(ControlKeys[KeyIndex]);
		if (ControlIndex != INDEX_NONE)
		{
			const FName ShapeName = Blueprint->GetHierarchy()->GetChecked<FRigControlElement>(ControlIndex)->Settings.ShapeName; 
			if(KeyIndex == 0)
			{

...
static FKeyHandle SequencerOpenSpaceSwitchDialog(UControlRig* ControlRig, TArray<FRigElementKey> SelectedControls,ISequencer* Sequencer, FMovieSceneControlRigSpaceChannel* Channel, UMovieSceneSection* SectionToKey, FFrameNumber Time)
{
	...
	if (const TSharedPtr<SWindow> Window = PickerWidget->OpenDialog(true))
	{
		return Handle;
	}
			...
			if (ControlRigParameterTrack && ControlRigParameterTrack->GetControlRig() == ControlRig)
			{
				...
				if (ActiveSection)
				{
					...
					else if (bCreateIfNeeded)
					{
						...
						//important we don't want the adding of the space channel to be in the undo since we want the channel to stick around if we get undone
...
FKeyHandle FControlRigSpaceChannelHelpers::SequencerKeyControlRigSpaceChannel(
	...
	const FRigElementKey& SpaceKey)
{
	...
	UMovieSceneSequence* MovieSceneSequence = Sequencer ? Sequencer->GetFocusedMovieSceneSequence() : nullptr;
	UMovieScene* MovieScene = MovieSceneSequence ? MovieSceneSequence->GetMovieScene() : nullptr;
	if (bDoNotCompensate == true || ControlRig == nullptr || !MovieScene)
	{
		return Handle;
	}
	...
	if (Value != ExistingValue)
	{
		...
		const FMovieSceneSequenceTransform RootToLocalTransform = Sequencer->GetFocusedMovieSceneSequenceTransform();
		const FMovieSceneInverseSequenceTransform LocalToRootTransform = RootToLocalTransform.Inverse();
		
		const FConstraintsManagerController& Controller = FConstraintsManagerController::Get(ControlRig->GetWorld());
		auto EvaluateSequencer = [Sequencer, TickResolution, &Controller](const FFrameTime& Time)
		{
			FMovieSceneContext SceneContext(FMovieSceneEvaluationRange(Time, TickResolution), Sequencer->GetPlaybackStatus());
			SceneContext.SetHasJumped(true);
		...
		CurrentTime = LocalToRootTransform.TryTransformTime(CurrentTime).Get(CurrentTime);

		EvaluateSequencer(CurrentTime);
		...
		GetFramesInThisSpaceAfterThisTime(ControlRig, ControlKey.Name, ExistingValue, Channel, SectionToKey, Time, ExtraFrames);
		SectionToKey->Modify();
		...
		if (ExtraFrames.Num() > 0)
		{
			...
			//if the next space is the same as the new one, then we delete those transform frame and space key if it exists
			FMovieSceneControlRigSpaceBaseKey NextValue;
			using namespace UE::MovieScene;
			FFrameNumber NextSpaceTime = Frames[Frames.Num() - 1] + 1;
			EvaluateChannel(Channel, NextSpaceTime, NextValue);
			if (NextValue == Value)
			{
				int32 ExistingIndex = ChannelInterface.FindKey(NextSpaceTime);
				if (ExistingIndex != INDEX_NONE)
				{
					ChannelInterface.RemoveKey(ExistingIndex);
					if (ControlRigSection)
					{
						FControlRigSpaceChannelHelpers::DeleteTransformKeysAtThisTime(ControlRig, ControlRigSection, ControlKey.Name, NextSpaceTime -1);
						Frames.SetNum(Frames.Num() - 1);
					}
				}
			}
		...
		ControlRigParentWorldTransforms.Init(FTransform::Identity, Frames.Num());
		...
		else //same as previous
		{
			if (ControlRigSection)
			{
				...
				DeleteTransformKeysAtThisTime(ControlRig, ControlRigSection, ControlKey.Name, Time - 1);
				bSetPreviousKey = false; //also don't set previous
			}
		}

		FRigControlModifiedContext Context(EControlRigSetKey::Always);
		constexpr bool bNoUndo = false,  bNoPython = false, bFixEulerFlips = true;
		...
		if (bSetPreviousKey)
		{
			...
			EvaluateSequencer(GlobalTime);

			Context.LocalTime = TickResolution.AsSeconds(FFrameTime(Time - 1));
			ControlRig->SetControlGlobalTransform(ControlKey.Name, ControlWorldTransforms[0], true, Context, bNoUndo, bNoPython, bFixEulerFlips);
		...
		for (const FFrameNumber& Frame : Frames)
		{
			...
			ControlRig->SetControlGlobalTransform(ControlKey.Name, ControlWorldTransforms[FramesIndex], true, Context, bNoUndo, bNoPython, bFixEulerFlips);

	...
	TArray<FFrameNumber>& OutFramesToBake)
{
	...
	TArrayView<const bool> ConstraintValues = ConstraintChannelData.GetValues();

	const TOptional<bool> bDefault = ThisActiveChannel->ActiveChannel.GetDefault();
	const bool bInfinite = bDefault && *bDefault;
	...
	if (!bHasSettings)
	{
		...
		const bool bIsLastKeyInactive = ConstraintValues.IsEmpty() ? false : (ConstraintValues.Last() == false);

...
USkeletalMesh* FControlRigBlueprintActions::GetSkeletalMeshFromControlRigBlueprint(const FAssetData& InAsset)
{
	...
	if (InAsset.GetClass() != UControlRigBlueprint::StaticClass()
		&& InAsset.GetClass() != UControlRigBlueprintGeneratedClass::StaticClass())
	{
		return nullptr;
	}

	if (const UControlRigBlueprintGeneratedClass* Class = Cast<UControlRigBlueprintGeneratedClass>(InAsset.GetAsset()))
	{
		if(Class->PreviewSkeletalMesh.IsValid())
		{
			return Cast<USkeletalMesh>(Class->PreviewSkeletalMesh.TryLoad());
		}
	}
...
void FControlRigBlueprintActions::OnSpawnedSkeletalMeshActorChanged(UObject* InObject, FPropertyChangedEvent& InEvent, UObject* InAsset)
{
	...
	GEditor->GetTimerManager()->SetTimerForNextTick([InObject, InAsset]()
	{
		...
		if (UControlRigBlueprint* RigBlueprint = Cast<UControlRigBlueprint>(InAsset))
		{
			ControlRigClass = RigBlueprint->GeneratedClass;
		}
		else if(UControlRigBlueprintGeneratedClass* GeneratedClass = Cast<UControlRigBlueprintGeneratedClass>(InAsset))
		{

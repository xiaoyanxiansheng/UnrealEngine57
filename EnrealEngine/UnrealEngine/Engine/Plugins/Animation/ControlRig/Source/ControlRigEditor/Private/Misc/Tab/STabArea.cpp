...
void STabArea::Construct(const FArguments& InArgs)
{
	...
	for (const FTabEntry& Entry : InArgs._Tabs)
	{
		...
		Content->AddSlot()
			...
			.FillContentWidth(1.f) // This causes all buttons have the same width and grow until all their desired sizes have been reached.
			.MinWidth(Entry.MinButtonSlotWidth) 
			[
	...
	ChildSlot
		// The buttons should be centered (i.e. padd equally on left and right if there's space left over)

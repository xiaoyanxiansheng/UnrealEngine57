...
public:
	...
	SLATE_ARGUMENT_DEPRECATED(UControlRigBlueprint*, Blueprint, 5.7, "Please use Asset instead")
	SLATE_ARGUMENT(FControlRigAssetInterfacePtr, Asset)
...
private:
	...
	FControlRigAssetInterfacePtr BlueprintBeingCustomized;
...
public:
	...
	struct FPerElementInfo
	{
		...
		UE_DEPRECATED(5.7, "Use GetAsset instead")
		...
		FControlRigAssetInterfacePtr GetAsset() const
		{
			...
			{
				return FControlRigAssetInterfacePtr(ControlRig->GetClass()->ClassGeneratedBy);
			}
			return IControlRigAssetInterface::GetInterfaceOuter(GetDefaultHierarchy());
		}
...
protected:
	...
	TArray<TUniquePtr<FControlRigInteractionScope>> InteractionScopes;
...
private:
		...
		TSharedPtr<SWidget> NameContent = TSharedPtr<SWidget>())
	{
		...
		FControlRigEditorEditMode* EditMode = nullptr;
		...
		auto OnValueChanged = [ValueType, Keys, HierarchyToChange, EditMode, this]
			(TOptional<T> InValue, ETextCommit::Type InCommitType, bool bSetupUndo)
			{
				...
				for(const FRigElementKey& Key : Keys)
				{
					...
					if(!Equals(PreviousValue, Value))
					{
						...
						if (EditMode)
						{
							EditMode->ModifiedRigElements.AddUnique(Key);
						}
					}
				}
...
private:
	...
	TSharedPtr<IPropertyHandle> IsPostConstructionHandle;
};

...
public:
	...
	virtual UClass* GetRigVMAssetClass() const override;
	...
	virtual void GetNodeContextMenuActions(TScriptInterface<IRigVMClientHost> RigVMClientHost, const URigVMEdGraphNode* EdGraphNode, URigVMNode* ModelNode, UToolMenu* Menu) const override;
	virtual void GetPinContextMenuActions(TScriptInterface<IRigVMClientHost> RigVMClientHost, const UEdGraphPin* EdGraphPin, URigVMPin* ModelPin, UToolMenu* Menu) const override;
	...
	CONTROLRIGEDITOR_API void BakeToControlRig(UClass* InClass,UAnimSequence* AnimSequence, USkeletalMesh* SkelMesh,USkeleton* InSkeleton);
...
private:
	...
	TSharedPtr<FRigDependencyGraphPanelNodeFactory> RigDependencyGraphPanelNodeFactory;
};
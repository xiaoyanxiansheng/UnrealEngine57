...
{
	if (FControlRigAssetInterfacePtr InRigBlueprint = Object)
	{
		...
		USkeletalMesh* SkeletalMesh = InRigBlueprint->GetPreviewSkeletalMesh().Get();
...
void UControlRigThumbnailRenderer::AddAdditionalPreviewSceneContent(UObject* Object, UWorld* PreviewWorld)
{
	...
	if (ThumbnailScene->GetPreviewActor() && RigBlueprint && !RigBlueprint->GetShapeLibraries().IsEmpty() && RigBlueprint->GetRigVMAssetInterface()->GetRigVMGeneratedClass())
	{
		...
		UControlRig* CDO = Cast<UControlRig>(RigBlueprint->GetRigVMAssetInterface()->GetRigVMGeneratedClass()->GetDefaultObject(true /* create if needed */));

...
void SControlRigPoseView::Construct(const FArguments& InArgs)
{
	...
	if (OwningWidget.IsValid())
	{
		...
		{
			EditMode->OnControlRigAddedOrRemoved().AddSP(SharedThis(this), &SControlRigPoseView::HandleControlAdded);
...
void SControlRigPoseView::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	SCompoundWidget::Tick(AllottedGeometry, InCurrentTime, InDeltaTime);

	if (bUpdateStatusBlocks)
	{
		UpdateStatusBlocks();
		bUpdateStatusBlocks = false;
	}
}

	...
	if (BlueprintBeingCustomized)
	{
		if(UControlRig* DebuggedRig = Cast<UControlRig>(BlueprintBeingCustomized->GetObjectBeingDebugged()))
		{
			if(FRigVMMemoryStorageStruct* MemoryStorage = DebuggedRig->GetMemoryByType(InMemoryType))
			{
				...
				BlueprintBeingCustomized->GetRigVMAssetInterface()->RequestInspectMemoryStorage(InStructs);
			}
		}
	}
...
FReply FRigVMCompileSettingsDetails::OnCopyASTClicked()
{
	if (BlueprintBeingCustomized)
	{
		if (BlueprintBeingCustomized->GetRigVMAssetInterface()->GetDefaultModel())
		{
			FString DotContent = BlueprintBeingCustomized->GetRigVMAssetInterface()->GetDefaultModel()->GetRuntimeAST()->DumpDot();
...
FReply FRigVMCompileSettingsDetails::OnCopyGeneratedCodeClicked()
{
	if (BlueprintBeingCustomized)
	{
		const FString ClassName = FString::Printf(TEXT("%sVM"), *BlueprintBeingCustomized.GetObject()->GetName());
		if (BlueprintBeingCustomized->GetRigVMAssetInterface()->GetRigVMGeneratedClass())
		{
			if (UControlRig* CDO = Cast<UControlRig>(BlueprintBeingCustomized->GetRigVMAssetInterface()->GetRigVMGeneratedClass()->GetDefaultObject()))
			{
				if(CDO->GetVM())
				{
					...
					FRigVMCodeGenerator CodeGenerator(ClassName,
						TEXT("TestModule"), BlueprintBeingCustomized->GetRigVMAssetInterface()->GetDefaultModel(), CDO->GetVM(), CDOContext,
						CDO->GetPublicContextStruct(), BlueprintBeingCustomized->GetRigVMAssetInterface()->PinToOperandMap);

...
void FControlRigDrawContainerDetails::ImportCurvesFromFBX(const FString& InFilePath, FControlRigAssetInterfacePtr InBlueprint, const FControlRigDrawContainerImportFbxSettings& InSettings)
{
	...
	int32 LastInstructionIndex = INDEX_NONE;
	{
		...
		for (fbxsdk::FbxNode* FbxCurveNode : FbxCurveNodes)
		{
			...
			for (int32 LineIndex = 0;LineIndex < Lines.Num(); LineIndex++)
			{
				...
				{
					InBlueprint->GetDrawContainer().Instructions[LastInstructionIndex].Positions.Append(Instruction.Positions);
					continue;
				}

				LastInstructionIndex = InBlueprint->GetDrawContainer().Instructions.Add(Instruction);
			}
		}

...
UObject* UControlRigBlueprintFactory::FactoryCreateNew(UClass* Class, UObject* InParent, FName Name, EObjectFlags Flags, UObject* Context, FFeedbackContext* Warn, FName CallingContext)
{
	...
	else
	{
		...
		FControlRigEditorModule::Get().CreateRootGraphIfRequired(FRigVMAssetInterfacePtr(ControlRigBlueprint));
		...
		if(ParentClass->IsChildOf(UModularRig::StaticClass()))
		{
			...
			if(const UControlRigBlueprint* DefaultRootModule = Cast<UControlRigBlueprint>(DefaultRootModuleObj))
			{
			...
			{
				static const FName RootName = TEXT("Root");
				ControlRigBlueprint->ModularRigModel.GetController()->AddModule(RootName, DefaultRootModuleGeneratedClass, NAME_None, false);
			}
		}

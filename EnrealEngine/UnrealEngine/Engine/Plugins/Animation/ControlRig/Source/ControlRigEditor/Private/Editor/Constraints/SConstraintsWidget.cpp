...
#include "ConstraintCreationOptions.h"
...
const TArray< UTickableTransformConstraint* >& FConstraintInfo::GetConfigurableConstraints()
{
	using namespace UE::TransformConstraintUtil;
	static TArray< UTickableTransformConstraint* > ConfigurableConstraints({
		GetConfig(UTickableTranslationConstraint::StaticClass()),
		GetConfig(UTickableRotationConstraint::StaticClass()),
		GetConfig(UTickableScaleConstraint::StaticClass()),
		GetConfig(UTickableParentConstraint::StaticClass()),
		GetConfig(UTickableLookAtConstraint::StaticClass()),
	});
	...
	bool CreateConstraint(const TArray<AActor*>& InChildren, const FConstrainable& InParent, const ETransformConstraintType InConstraintType, const bool bUseDefault)
	{
		...
		for (AActor* Child: InChildren)
		{
			if (Child != InParent.Object)
			{
				...
				TUniquePtr<FControlRigDismissDependencyDialogGuard> DependencyDialogGuard;
				TFunction<void()> ValidDependency;
				const AControlRigShapeActor* ChildActor = Cast<AControlRigShapeActor>(Child);
				if (const UControlRig* ControlRig = ChildActor ? ChildActor->ControlRig.Get() : nullptr)
				{
					// enable the dependency dialog when creating a constraint for children that are control rig controls
					DependencyDialogGuard = MakeUnique<FControlRigDismissDependencyDialogGuard>(ControlRig->GetHierarchy());
					ValidDependency = [&DependencyDialogGuard]()
					{
						DependencyDialogGuard.Reset();
					};
				}
				
				UTickableTransformConstraint* Constraint = TransformConstraintUtil::CreateAndAddFromObjects(
					World, Parent, InParent.Socket, Child, NAME_None, InConstraintType, true, bUseDefault, ValidDependency);
				...
				if (Constraint)
				{
					...
					{
						const UConstraintCreationOptions* Options = bUseDefault ? GetDefault<UConstraintCreationOptions>() : nullptr;
						FControlRigParameterTrackEditor::AddConstraintToSequencer(WeakSequencer.Pin(), Constraint, Options);
					}
					else
					{
...
TSharedRef<SWidget> SConstraintMenuEntry::GenerateConstraintDefaultWidget()
{
	...
	if (!ensure(ConfigurableConstraint))
	{
	...
	const auto PropertyVisibility = FIsPropertyVisible::CreateLambda(
		[bIsLookAtConstraint](const FPropertyAndParent& InPropertyAndParent)
		{
			...
			static const FName SequencerOptionsPropName = GET_MEMBER_NAME_CHECKED(UConstraintCreationOptions, SequencerOptions);
			if (PropertyName == SequencerOptionsPropName)
			{
				TWeakPtr<ISequencer> WeakSequencer = UE::Private::GetSequencerChecked();
				return UE::Private::GetSequencerChecked().IsValid();
			}
	...
	TArray<UObject*> Objects(
	{
		...
		GetMutableDefault<UConstraintCreationOptions>()
	});
	...
	return SNew(SBorder)
		...
		[
			...
			[
				SNew(SBox)
				.WidthOverride(350.f) // Make all the menus have a consistent size & prevent details from being too narrow to read the property texts. 
				[
					DetailsView
				]
			]
...
{
	// We need to hide the creation menu. This destroys the menu that is keeping this SConstraintMenuEntry alive.
	// So close only at the end because we're still accessing members in this call (undefined behaviour).
	ON_SCOPE_EXIT { 	FSlateApplication::Get().DismissAllMenus(); };
	
	// use the selected parent if any
	const FConstrainable Parent = OnGetParent.IsBound() ? OnGetParent.Execute() : FConstrainable();
	if (Parent.Object.IsValid())
	{
		const TArray<AActor*> Children = UE::Private::GetCurrentSelection();
		const bool bCreated = UE::Private::CreateConstraint(Children, Parent, ConstraintType, bUseDefault);
		if (!bCreated)
		{
			return FReply::Unhandled();
		}
		...
		// update list
		if (OnConstraintCreated.IsBound())
		{
			OnConstraintCreated.Execute();
		}
...
void FBaseConstraintListWidget::RegisterNotifications()
{
	// selection changed
	{
	...
	// constraints controller updated
 	{
...
void FBaseConstraintListWidget::UnregisterNotifications()
{
	if (OnSelectionChangedHandle.IsValid())
	{
...
void SConstraintsEditionWidget::Construct(const FArguments& InArgs)
{
	...
	auto GetParentWidget = [this, &InArgs]()
	{
		...
		[
			...
			.AutoHeight()
			[
				InArgs._LeftSplitterContent.Widget
			]
		...
		[
			...
			.AutoHeight()
			[
				InArgs._MiddleSplitterContent.Widget
			]
		...
		[
			...
			.AutoHeight()
			[
				InArgs._RightSplitterContent.Widget
			]
	...
	auto GetBakingWidget = [this, &InArgs]()
	{
		...
		[
			...
			.IsEnabled_Lambda([this]() { return ListItems.Num() > 0; })
...
TSharedRef<SWidget> SConstraintsEditionWidget::OnGetParentMenuContent()
{
	...
	FSimpleDelegate UseSelectedDelegate = FSimpleDelegate::CreateLambda([this]
	{
...
TSharedPtr<SWidget> SConstraintsEditionWidget::CreateContextMenu()
{
	...
	FDetailsViewArgs DetailsViewArgs;
	{
		...
		DetailsViewArgs.bShowOptions = false;
		DetailsViewArgs.bShowModifiedPropertiesOption = false;
		DetailsViewArgs.ColumnWidth = 0.35f;
	}
	...
	FIsPropertyVisible PropertyVisibility = FIsPropertyVisible::CreateLambda(
		[bIsLookAtConstraint](const FPropertyAndParent& InPropertyAndParent)
		{
			...
			if (!bIsLookAtConstraint)
			{
				if (PropertyName == GET_MEMBER_NAME_CHECKED(UTickableTransformConstraint, bUseCurrentOffset))
				{
					return false;
				}
				return true;
	...
	MenuBuilder.BeginSection("EditConstraint", LOCTEXT("EditConstraintHeader", "Edit Constraint"));
	{
		MenuBuilder.AddWidget(
			// Summary: This makes sure the menu anchor position is realigned when you open the menu near the edge of your screen, so the menu does
			// not spill over to the next screen.
			// Slate calls Prepass and then uses the widget's desired size. However, the details view always defers until next tick to compute it.
			// The width override forces a desired size.
			SNew(SBox).WidthOverride(480.f) [ DetailsView ],
			FText::GetEmpty(), true
			);
	}
	MenuBuilder.EndSection();
	...
	if (UTickableTransformConstraint* TransformConstraint = Cast<UTickableTransformConstraint>(Constraint))
	{
		...
		MenuBuilder.BeginSection("BakeConstraint", LOCTEXT("BakeConstraintHeader", "Bake Constraint"));
		{
			...
			FUIAction(FExecuteAction::CreateLambda([this, Constraint, World]()
			{
				...
				if (TransformConstraint && Sequencer)
				{

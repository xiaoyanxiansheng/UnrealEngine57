...
struct FDatabaseName : IColumn
{
	...
	virtual FSortPredicate GetSortPredicate() const override
	{
		return [](const FRowDataRef& Row0, const FRowDataRef& Row1) -> bool { return GetNameSafe(Row0->SharedData->SourceDatabase.Get()) < GetNameSafe(Row1->SharedData->SourceDatabase.Get()); };
	}

...
UPoseSearchDatabase* ResolveDatabaseFromId(uint64 DatabaseId)
{
	...
	if (const TraceServices::IAnalysisSession* AnalysisSession = IRewindDebugger::Instance()->GetAnalysisSession())
	{
		...
		if (const IGameplayProvider* GameplayProvider = AnalysisSession->ReadProvider<IGameplayProvider>("GameplayProvider"))
		{
			if (const FObjectInfo* DatabaseInfo = GameplayProvider->FindObjectInfo(DatabaseId))
			{
				Database = TSoftObjectPtr<UPoseSearchDatabase>(FSoftObjectPath(DatabaseInfo->PathName)).LoadSynchronous();
			}
		}
	}
	return Database;
...
public:
	SCostBreakDownData(const TArray<FTraceMotionMatchingStateDatabaseEntry>& DatabaseEntries, bool bIsVerbose)
	{
		...
		for (const FTraceMotionMatchingStateDatabaseEntry& DbEntry : DatabaseEntries)
		{
			...
			if (EAsyncBuildIndexResult::Success == DatabasesManagementCache(Database))
			{
	...
	bool AreLabelsEqualTo(const TConstArrayView<FText> OtherLabels) const
	{
	...
	int32 DbPoseIdx, EPoseCandidateFlags PoseCandidateFlags, TConstArrayView<uint32> PoseToPCAValuesVectorIndexes, const FPoseSearchCost& Cost = FPoseSearchCost())
{
	...
	if (const FSearchIndexAsset* SearchIndexAsset = SearchIndex.GetAssetForPoseSafe(DbPoseIdx))
	{
		...
		if (const FPoseSearchDatabaseAnimationAssetBase* DatabaseAsset = Database->GetDatabaseAnimationAsset(*SearchIndexAsset))
		{
...
void SDebuggerDatabaseView::Update(const FTraceMotionMatchingStateMessage& State)
{
	...
	{
		UPoseSearchDatabase* Database = ResolveDatabaseFromId(DbEntry.DatabaseId);
		if (EAsyncBuildIndexResult::Success == DatabasesManagementCache(Database))
		{
			const FSearchIndex& SearchIndex = Database->GetSearchIndex();
...
void SDebuggerDatabaseView::OnViewSelectionChanged(TSharedPtr<FDebuggerDatabaseRowData> Row, SDebuggerDatabaseView::FTable& Table)
{
	Table.PreviouslySelectedItems = Table.ListView->GetSelectedItems();
...
void SDebuggerDatabaseView::OnActiveViewSelectionChanged(TSharedPtr<FDebuggerDatabaseRowData> Row, ESelectInfo::Type SelectInfo)
{
	OnViewSelectionChanged(Row, ActiveView);
}

void SDebuggerDatabaseView::OnContinuingPoseViewSelectionChanged(TSharedPtr<FDebuggerDatabaseRowData> Row, ESelectInfo::Type SelectInfo)
{
	OnViewSelectionChanged(Row, ContinuingPoseView);
}

void SDebuggerDatabaseView::OnFilteredDatabaseViewSelectionChanged(TSharedPtr<FDebuggerDatabaseRowData> Row, ESelectInfo::Type SelectInfo)
{
	OnViewSelectionChanged(Row, FilteredDatabaseView);
}

void SDebuggerDatabaseView::OnViewMouseButtonClick(const TSharedPtr<FDebuggerDatabaseRowData> Row, SDebuggerDatabaseView::FTable& Table)
{
	const TArray<TSharedRef<FDebuggerDatabaseRowData>> CurrentlySelectedItems = Table.ListView->GetSelectedItems();

	if (CurrentlySelectedItems.Num() == 1 && Table.PreviouslySelectedItems.Num() == 1 &&
		CurrentlySelectedItems[0] == Row && Table.PreviouslySelectedItems[0] == Row)
	{
		Table.ListView->ClearSelection();
	}
}

void SDebuggerDatabaseView::OnActiveViewMouseButtonClick(const TSharedRef<FDebuggerDatabaseRowData> Row)
{
	OnViewMouseButtonClick(Row, ActiveView);
}

void SDebuggerDatabaseView::OnContinuingPoseViewMouseButtonClick(const TSharedRef<FDebuggerDatabaseRowData> Row)
{
	OnViewMouseButtonClick(Row, ContinuingPoseView);
}

void SDebuggerDatabaseView::OnFilteredDatabaseViewMouseButtonClick(const TSharedRef<FDebuggerDatabaseRowData> Row)
{
	OnViewMouseButtonClick(Row, FilteredDatabaseView);
}

void SDebuggerDatabaseView::SortDatabaseRows()
{
	if (!UnfilteredDatabaseRows.IsEmpty() && Columns.Contains(SortColumn))
	{
	...
	if (UnfilteredDatabaseRowsNum > 0)
	{
		...
		if (UPoseSearchDebuggerConfig::Get().bShowOnlyBestAssetPose)
		{
			...
			Algo::Sort(SortIndex, [this](int32 IndexA, int32 IndexB)
			{
				...
				{
					const int32 DatabaseNameCompare = GetNameSafe(RowDataA.SharedData->SourceDatabase.Get()).Compare(GetNameSafe(RowDataB.SharedData->SourceDatabase.Get()));
					if (DatabaseNameCompare < 0)
					{
						return true;
					}

					const int32 DatabasePathCompare = GetPathNameSafe(RowDataA.SharedData->SourceDatabase.Get()).Compare(GetPathNameSafe(RowDataB.SharedData->SourceDatabase.Get()));
					return DatabasePathCompare < 0;
				}
...
void SDebuggerDatabaseView::Construct(const FArguments& InArgs)
{
	...
	ActiveView.ListView = SNew(SListView<TSharedRef<FDebuggerDatabaseRowData>>)
		...
		.OnSelectionChanged(this, &SDebuggerDatabaseView::OnActiveViewSelectionChanged)
		.OnMouseButtonClick(this, &SDebuggerDatabaseView::OnActiveViewMouseButtonClick);
	...
	ContinuingPoseView.ListView = SNew(SListView<TSharedRef<FDebuggerDatabaseRowData>>)
		...
		.OnSelectionChanged(this, &SDebuggerDatabaseView::OnContinuingPoseViewSelectionChanged)
		.OnMouseButtonClick(this, &SDebuggerDatabaseView::OnContinuingPoseViewMouseButtonClick);
	...
	FilteredDatabaseView.ListView = SNew(SListView<TSharedRef<FDebuggerDatabaseRowData>>)
		...
		.OnSelectionChanged(this, &SDebuggerDatabaseView::OnFilteredDatabaseViewSelectionChanged)
		.OnMouseButtonClick(this, &SDebuggerDatabaseView::OnFilteredDatabaseViewMouseButtonClick);

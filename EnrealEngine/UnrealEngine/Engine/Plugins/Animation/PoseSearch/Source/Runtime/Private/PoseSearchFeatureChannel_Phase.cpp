...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PoseSearchFeatureChannel_Phase)
...
bool UPoseSearchFeatureChannel_Phase::IndexAsset(UE::PoseSearch::FAssetIndexer& Indexer) const
{
	...
	if (Indexer.GetNumIndexedPoses() <= 1)
	{
		// We must have at least 2 poses in order to index phase properly.
		UE_LOG(LogPoseSearch, Error, TEXT("UPoseSearchFeatureChannel_Phase::IndexAsset - Attempt to index animation asset with single pose is not supported. Asset name: %s"), *Indexer.GetAssetName());
		return false;
	}

...
UCLASS(MinimalAPI, EditInlineNew, Blueprintable, meta = (DisplayName = "Heading Channel"), CollapseCategories)
...
public:
	...
	UE_API FQuat BP_GetWorldRotation(const UAnimInstance* AnimInstance) const;

	bool bUseBlueprintQueryOverride = false;

	UE_API UPoseSearchFeatureChannel_Heading();
	...
	UE_API FVector GetAxis(const FQuat& Rotation) const;
	static UE_API void FindOrAddToSchema(UPoseSearchSchema* Schema, float SampleTimeOffset, const FName& BoneName, const UE::PoseSearch::FRole& Role, EHeadingAxis HeadingAxis = EHeadingAxis::X, EPermutationTimeType PermutationTimeType = EPermutationTimeType::UseSampleTime);
};

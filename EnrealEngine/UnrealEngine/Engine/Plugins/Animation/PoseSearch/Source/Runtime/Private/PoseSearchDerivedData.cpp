...
static bool InitSearchIndexAssets(FSearchIndexBase& SearchIndex, const UPoseSearchDatabase* DatabaseToLookForAssets, const UPoseSearchSchema* Schema, const FFloatInterval& ExcludeFromDatabaseParameters)
{
	...
	{
		if (const FPoseSearchDatabaseAnimationAssetBase* DatabaseAsset = DatabaseToLookForAssets->GetDatabaseAnimationAsset(AnimationAssetIndex))
		{
...
static bool IndexDatabase(FSearchIndexBase& SearchIndexBase, const UPoseSearchDatabase* DatabaseToLookForAssets, const UPoseSearchSchema* Schema, const FAssetSamplingContext& SamplingContext, const FFloatInterval& AdditionalExtrapolationTime, UE::DerivedData::FRequestOwner& Owner)
{
	...
	for (int32 AssetIdx = 0; AssetIdx != SearchIndexBase.Assets.Num(); ++AssetIdx)
	{
		...
		const FPoseSearchDatabaseAnimationAssetBase* DatabaseAnimationAssetBase = DatabaseToLookForAssets->GetDatabaseAnimationAsset(SearchIndexAsset.GetSourceAssetIdx());
...
static bool ValidateSearchInfoAgainstDatabase(const FSearchIndex& SearchIndex, const UPoseSearchDatabase* Database, const UE::DerivedData::FCacheKey& FullIndexKey)
{
	...
	{
		if (const FPoseSearchDatabaseAnimationAssetBase* DatabaseAsset = Database->GetDatabaseAnimationAsset(AnimationAssetIndex))
		{
...
void FPoseSearchDatabaseAsyncCacheTask::PreCancelIfDependsOn(const UObject* Object)
{
	...
	if (FKeyBuilder::ShouldInclude(Object))
	{
		...
		Owner.LaunchTask(TEXT("PoseSearchDatabaseBuild"), [this, FullIndexKey, bCompareSearchIndex]
			{
				...
				for (int32 IndexBaseIdx = 0; IndexBaseIdx < IndexBaseDatabases.Num(); ++IndexBaseIdx)
				{
					...
					if (!bAllRoledSkeletonHaveMirrorDataTable)
					{
						...
						{
							if (const FPoseSearchDatabaseAnimationAssetBase* DatabaseAsset = DependentDatabase->GetDatabaseAnimationAsset(AnimationAssetIndex))
							{
					...
					for (int32 AnimationAssetIndex = 0; AnimationAssetIndex < DependentDatabase->GetNumAnimationAssets(); ++AnimationAssetIndex)
					{
						if (const FPoseSearchDatabaseAnimationAssetBase* DatabaseAsset = DependentDatabase->GetDatabaseAnimationAsset(AnimationAssetIndex))
						{
							const UAnimationAsset* AnimationAsset = Cast<const UAnimationAsset>(DatabaseAsset->GetAnimationAsset());
							if (DatabaseAsset->GetAnimationAsset() == nullptr)
							{
								UE_LOG(LogPoseSearch, Warning, TEXT("OnGetComplete - - No asset has been selected."));
							}
							else if (!DatabaseAsset->GetAnimationAsset()->GetPackage() || DatabaseAsset->GetAnimationAsset()->HasAnyFlags(RF_MirroredGarbage))
							{
								UE_LOG(LogPoseSearch, Error, TEXT("%s - %s BuildIndex Failed because '%s' referenced asset '%s' is garbage or is not contained in a package"), *LexToString(FullIndexKey.Hash), *MainDatabaseName, *DependentDatabaseName, *DatabaseAsset->GetAnimationAsset()->GetName());
								ResetSearchIndex();
								return;
							}
							else if (AnimationAsset && AnimationAsset->GetSkeleton() == nullptr)
							{
								UE_LOG(LogPoseSearch, Error, TEXT("%s - %s BuildIndex Failed because '%s' referenced asset '%s' which has an invalid (null) Skeleton"), *LexToString(FullIndexKey.Hash), *MainDatabaseName, *DependentDatabaseName, *DatabaseAsset->GetAnimationAsset()->GetName());
								ResetSearchIndex();
								return;
							}
					...
					{
						UE_LOG(LogPoseSearch, Log, TEXT("%s - %s BuildIndex Failed in IndexDatabase"), *LexToString(FullIndexKey.Hash), *MainDatabaseName);
						ResetSearchIndex();
						return;
					}
				...
				const bool bRemoveSearchIndexValues = MainDatabase->PoseSearchMode == EPoseSearchMode::PCAKDTree && MainDatabase->KDTreeQueryNumNeighbors <= 1;
				if (!bRemoveSearchIndexValues)
				{
					// we cannot prune any PCAValues if we need to reconstruct the Values from them!
...
void FAsyncPoseSearchDatabasesManagement::CollectDatabasesToSynchronize(UObject* Object)
{
	...
	if (!Object->IsA<UAnimSequenceBase>() && !Object->IsA<UAnimNotifyState_PoseSearchBranchIn>())
	{
		return;
	}
...
void FAsyncPoseSearchDatabasesManagement::PreModified(UObject* Object)
{
	...
	if (FKeyBuilder::ShouldInclude(Object))
	{
		...
		{
			TaskPtr->PreCancelIfDependsOn(Object);
		}
...
void FAsyncPoseSearchDatabasesManagement::PostModified(UObject* Object)
{
	...
	if (FKeyBuilder::ShouldInclude(Object))
	{
		...
		// promoting the EState::PreCancelled into EState::Cancelled
		for (TUniquePtr<FPoseSearchDatabaseAsyncCacheTask>& TaskPtr : Tasks)
		{
			if (TaskPtr->GetState() == FPoseSearchDatabaseAsyncCacheTask::EState::PreCancelled)
			{
				TaskPtr->Cancel();
			}
		}
...
void FAsyncPoseSearchDatabasesManagement::OnPackageReloaded(const EPackageReloadPhase InPackageReloadPhase, FPackageReloadedEvent* InPackageReloadedEvent)
{
	...
	else if (InPackageReloadPhase == EPackageReloadPhase::PrePackageFixup)
	{
		const TMap<UObject*, UObject*>& RepointedObjects = InPackageReloadedEvent->GetRepointedObjects();
		for (const TPair<UObject*, UObject*>& RepointPair : RepointedObjects)
		{
			PreModified(RepointPair.Key);
		}
	}
...
//////////////////////////////////////////////////////////////////////////
// FAsyncPoseSearchDatabasesManagementCache
EAsyncBuildIndexResult FAsyncPoseSearchDatabasesManagementCache::operator()(const UPoseSearchDatabase* Database, ERequestAsyncBuildFlag RequestAsyncBuildFlag)
{
	if (const EAsyncBuildIndexResult* AsyncBuildIndexResult = CachedRequests.Find(Database))
	{
		return *AsyncBuildIndexResult;
	}

	const EAsyncBuildIndexResult NewAsyncBuildIndexResult = FAsyncPoseSearchDatabasesManagement::RequestAsyncBuildIndex(Database, RequestAsyncBuildFlag);
	CachedRequests.Add(Database) = NewAsyncBuildIndexResult;
	return NewAsyncBuildIndexResult;
}

...
#define LOCTEXT_NAMESPACE "EaseCurveToolCommands"

void FEaseCurveToolCommands::RegisterCommands()
{
	const TSharedRef<FBindingContext> SharedThis = AsShared();

	UI_COMMAND(ToggleToolTabVisible
		, "Ease Curve Tool"
		, "Toggle the visibility of the Ease Curve Tool tab window"
	...
	FUICommandInfo::MakeCommandInfo(SharedThis
		, SaveCurveEditorSelection
		, TEXT("SaveCurveEditorSelection")
		, LOCTEXT("SaveCurveEditorSelection_Label", "Save Curve Editor Selection")
		, LOCTEXT("SaveCurveEditorSelection_ToolTip", "Saves the current curve editor key selection as a new ease curve preset\n\n*NOTE* The selected key must be using broken and weighted tangents")
		, FSlateIcon(FAppStyle::Get().GetStyleSetName(), TEXT("Icons.ChevronLeft"))
		, EUserInterfaceActionType::Button
		, FInputChord());
}

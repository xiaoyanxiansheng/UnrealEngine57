...
#define LOCTEXT_NAMESPACE "SEaseCurvePreview"

namespace UE::EaseCurveTool
{

void SEaseCurvePreview::Construct(const FArguments& InArgs)
{ 
	...
	{
		ToolTipWidget = MakeShared<SEaseCurvePreviewToolTip>(InArgs);
		SetToolTip(ToolTipWidget);
	}
	else
	{
		SetToolTipText(SEaseCurvePreviewToolTip::GetToolTipText(Tangents.Get(FEaseCurveTangents())));
	}

	SetTangents(InArgs._Tangents);
}

FEaseCurveTangents SEaseCurvePreview::GetTangents() const
{
	return Tangents.Get(FEaseCurveTangents());
}

void SEaseCurvePreview::SetTangents(const TAttribute<FEaseCurveTangents>& InTangents)
{
	Tangents = InTangents;
	...
	// Cache motion trails
	const FEaseCurveTangents DrawTangents = Tangents.Get(FEaseCurveTangents());
...
FVector2D SEaseCurvePreview::ComputeDesiredSize(const float InLayoutScaleMultiplier) const
{
...
FReply SEaseCurvePreview::OnMouseButtonDown(const FGeometry& InGeometry, const FPointerEvent& InMouseEvent)
{
	if (InMouseEvent.GetEffectingButton() == EKeys::RightMouseButton)
	{
		FPlatformApplicationMisc::ClipboardCopy(*Tangents.Get(FEaseCurveTangents()).ToJson());
	...
	return SLeafWidget::OnMouseButtonDown(InGeometry, InMouseEvent);
}

void SEaseCurvePreview::Tick(const FGeometry& InAllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
...
TSharedRef<FSlateAccessibleWidget> SEaseCurvePreview::CreateAccessibleWidget()
{
	return MakeShareable<FSlateAccessibleWidget>(new FSlateAccessibleImage(SharedThis(this)));
}
#endif

int32 SEaseCurvePreview::OnPaint(const FPaintArgs& InArgs, const FGeometry& InAllottedGeometry, const FSlateRect& InMyCullingRect, FSlateWindowElementList& OutDrawElements, int32 InLayerId, const FWidgetStyle& InWidgetStyle, bool bInParentEnabled) const
{
...
void SEaseCurvePreview::MakeCurveData(const FGeometry& InGeometry, TArray<FVector2D>& OutCurvePoints, TArray<FLinearColor>& OutCurveColors) const
{
	const FEaseCurveTangents DrawTangents = Tangents.Get(FEaseCurveTangents());
...
float SEaseCurvePreview::WeightedEvalForTwoKeys(const FEaseCurveTangents& InTangents, const float InTime) const
{

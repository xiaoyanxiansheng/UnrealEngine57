...
#include "SEaseCurvePreviewToolTip.h"
...
#define LOCTEXT_NAMESPACE "SEaseCurvePreviewToolTip"

namespace UE::EaseCurveTool
{

SEaseCurvePreviewToolTip::SEaseCurvePreviewToolTip(const SEaseCurvePreview::FArguments& InPreviewArgs, const TSharedPtr<SWidget>& InAdditionalContent)
	: PreviewArgs(InPreviewArgs), AdditionalContent(InAdditionalContent)
{
}

FText SEaseCurvePreviewToolTip::GetToolTipText(const FEaseCurveTangents& InTangents)
{
	return FText::Format(LOCTEXT("FullToolTipText", "{0}\n\nRight click to copy to Json values"), InTangents.ToDisplayText());
}

TSharedRef<SToolTip> SEaseCurvePreviewToolTip::CreateDefaultToolTip(const SEaseCurvePreview::FArguments& InPreviewArgs, const TSharedPtr<SWidget>& InAdditionalContent)
{
	const TSharedRef<SVerticalBox> ContentWidget = SNew(SVerticalBox)
		...
		[
			...
			[
				...
				.Text(InPreviewArgs._Tangents.Get(FEaseCurveTangents()).GetStartTangentText())
			]
			+ SHorizontalBox::Slot()
			.HAlign(HAlign_Right)
			[
				...
				.Text(InPreviewArgs._Tangents.Get(FEaseCurveTangents()).GetEndTangentText())
			]
		]
		+ SVerticalBox::Slot()
		.AutoHeight()
		...
		[
			...
			[
				...
				.Text(InPreviewArgs._Tangents.Get(FEaseCurveTangents()).GetCubicBezierText())
			]
		]
		+ SVerticalBox::Slot()
		.AutoHeight()
...
void SEaseCurvePreviewToolTip::CreateToolTipWidget()
{
...
TSharedRef<SWidget> SEaseCurvePreviewToolTip::AsWidget()
{
	CreateToolTipWidget();
	return ToolTipWidget.ToSharedRef();
}

TSharedRef<SWidget> SEaseCurvePreviewToolTip::GetContentWidget()
{
	CreateToolTipWidget();
	return ToolTipWidget->GetContentWidget();
}

void SEaseCurvePreviewToolTip::SetContentWidget(const TSharedRef<SWidget>& InContentWidget)
{
	CreateToolTipWidget();
	ToolTipWidget->SetContentWidget(InContentWidget);
}

void SEaseCurvePreviewToolTip::InvalidateWidget()
{
	ToolTipWidget.Reset();
}

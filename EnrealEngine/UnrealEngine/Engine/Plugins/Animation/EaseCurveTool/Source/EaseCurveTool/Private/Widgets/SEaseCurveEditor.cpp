...
#include "Widgets/SEaseCurvePreviewToolTip.h"
#include "Widgets/SToolTip.h"
#include "Widgets/Text/STextBlock.h"

#define LOCTEXT_NAMESPACE "SEaseCurveEditor"
...
FVector2d SEaseCurveEditor::CalcTangentDir(const double InTangent)
{
	const double Angle = FMath::Atan(InTangent);
	return FVector2d(FMath::Cos(Angle), FMath::Sin(Angle));
}

float SEaseCurveEditor::CalcTangent(const FVector2d& InHandleDelta)
{
	return InHandleDelta.Y / FMath::Max<double>(InHandleDelta.X, KINDA_SMALL_NUMBER);
}

void SEaseCurveEditor::Construct(const FArguments& InArgs, const TSharedRef<FEaseCurveTool>& InTool)
{
	Tool = InTool;
	EaseCurve = InTool->GetToolCurve();
	...
	bCanEditCurve = InArgs._CanEditCurve;
	ErrorMessage = InArgs._ErrorMessage;
	...
	FCoreUObjectDelegates::OnObjectPropertyChanged.AddSP(this, &SEaseCurveEditor::OnObjectPropertyChanged);
...
FText SEaseCurveEditor::GetCurveToolTipInputText() const
{
	return CurveToolTipInputText;
}

FText SEaseCurveEditor::GetCurveToolTipOutputText() const
{
	return CurveToolTipOutputText;
}

SEaseCurveEditor::~SEaseCurveEditor()
{
	if (GEditor)
	{
...
FVector2D SEaseCurveEditor::ComputeDesiredSize(const float InLayoutScaleMultiplier) const
{
	const TOptional<FVector2D> SizeFromAttribute = DesiredSize.Get(TOptional<FVector2D>());
	return SizeFromAttribute.Get(FVector2D::ZeroVector);
}

int32 SEaseCurveEditor::OnPaint(const FPaintArgs& InArgs
...
{
	InLayerId = SCompoundWidget::OnPaint(InArgs, InGeometry, InCullingRect, OutDrawElements, InLayerId, InWidgetStyle, bInParentEnabled);

	const bool bEnabled = ShouldBeEnabled(bInParentEnabled);
	const ESlateDrawEffect DrawEffects = bEnabled ? ESlateDrawEffect::None : ESlateDrawEffect::DisabledEffect;
	const FTrackScaleInfo ScaleInfo(ViewMinInput, ViewMaxInput, ViewMinOutput, ViewMaxOutput, InGeometry.GetLocalSize());
	...
	// Normalized area background
	{
		...
		FSlateDrawElement::MakeBox(OutDrawElements, ++InLayerId,
			InGeometry.ToPaintGeometry(FVector2D(InputSize, OutputSize), FSlateLayoutTransform(FVector2D(ZeroInputX, OneOutputY))),
			WhiteBrush, DrawEffects, NormalAreaColor);
	}

	auto PaintCenteredText = [&InGeometry, &OutDrawElements, &InLayerId, DrawEffects, &ScaleInfo](const FText& InText)
	{
		const FSlateFontInfo FontInfo = FEaseCurveStyle::Get().GetFontStyle(TEXT("Editor.ErrorFont"));
		...
		const FVector2D HalfTextSize = TextSize * 0.5f;
		const FVector2D ActualOffset(HalfInputX - HalfTextSize.X, HalfOutputY - HalfTextSize.Y);

		FSlateDrawElement::MakeText(OutDrawElements, ++InLayerId
			, InGeometry.ToPaintGeometry(InGeometry.Size, FSlateLayoutTransform(ActualOffset))
			, InText, FontInfo, DrawEffects, FStyleColors::Foreground.GetSpecifiedColor());
	};

	if (CanEditCurve())
	{
		InLayerId = PaintGrid(ScaleInfo, InGeometry, OutDrawElements, ++InLayerId, InCullingRect, DrawEffects);

		if (StartText.IsSet() || EndText.IsSet())
		{
			const FSlateFontInfo FontInfo = FEaseCurveStyle::Get().GetFontStyle(TEXT("Editor.LabelFont"));
		...
		PaintNormalBounds(ScaleInfo, InGeometry, OutDrawElements, ++InLayerId, InCullingRect, DrawEffects, InWidgetStyle);

		PaintCurve(ScaleInfo, InGeometry, OutDrawElements, ++InLayerId, InCullingRect, DrawEffects, InWidgetStyle);

		InLayerId = PaintKeys(ScaleInfo, InGeometry, OutDrawElements, ++InLayerId, DrawEffects, InWidgetStyle);
	}

	if (HasDrawErrorMessage())
	{
		PaintCenteredText(ErrorMessage.Get().GetValue());
	}

	if (Tool->HasCachedKeysToEase())
	{
		PaintSelectionText(ScaleInfo, InGeometry, OutDrawElements, ++InLayerId, DrawEffects);
	}
	...
	if (DragState == EDragState::MarqueeSelect)
	{
		...
		FSlateDrawElement::MakeBox(OutDrawElements, ++InLayerId
			, InGeometry.ToPaintGeometry(MarqueBottomRight - MarqueTopLeft, FSlateLayoutTransform(MarqueTopLeft))
...
int32 SEaseCurveEditor::PaintGrid(const FTrackScaleInfo& InScaleInfo
	, const FGeometry& InGeometry
	, FSlateWindowElementList& OutDrawElements
	, int32 InLayerId
	...
	// Outside normalized area grid lines
	{
		...
		if (ScreenGridStepX >= 1.f)
		{
			for (float ScreenX = StartX; ScreenX < LocalSize.X; ScreenX += ScreenGridStepX)
			{
				...
				FSlateDrawElement::MakeLines(OutDrawElements, InLayerId, InGeometry.ToPaintGeometry(), LinePoints, InDrawEffects, ExtendedGridColor, false);
			}
		}
		...
		if (ScreenGridStepY >= 1.f)
		{
			for (float ScreenY = StartY; ScreenY < LocalSize.Y; ScreenY += ScreenGridStepY)
			{
				...
				FSlateDrawElement::MakeLines(OutDrawElements, InLayerId
					, InGeometry.ToPaintGeometry(), LinePoints, InDrawEffects, ExtendedGridColor, false);
			}
		}
	}
	...
	const int32 NormalizedLayerId = InLayerId + 1;
	{
		...
		for (float X = 0.f; X < 1.f; X += GridStep)
		{
			...
			FSlateDrawElement::MakeLines(OutDrawElements, NormalizedLayerId
				, InGeometry.ToPaintGeometry(), LinePoints, InDrawEffects, GridColor, false);
		}
...
void SEaseCurveEditor::PaintNormalBounds(const FTrackScaleInfo& InScaleInfo
	, const FGeometry& InGeometry
	, FSlateWindowElementList& OutDrawElements
	, int32 InLayerId
	, const FSlateRect& InMyCullingRect
	...
	, const FWidgetStyle& InWidgetStyle) const
{
	...
	ZeroTimeLinePoints.Add(FVector2D(ZeroInputX, InGeometry.GetLocalSize().Y));
	FSlateDrawElement::MakeLines(OutDrawElements, InLayerId, InGeometry.ToPaintGeometry()
	...
	OneTimeLinePoints.Add(FVector2D(OneInputX, InGeometry.GetLocalSize().Y));
	FSlateDrawElement::MakeLines(OutDrawElements, InLayerId, InGeometry.ToPaintGeometry()
	...
	ZeroValueLinePoints.Add(FVector2D(InGeometry.GetLocalSize().X, ZeroOutputY));
	FSlateDrawElement::MakeLines(OutDrawElements, InLayerId, InGeometry.ToPaintGeometry()
	...
	OneValueLinePoints.Add(FVector2D(InGeometry.GetLocalSize().X, OneOutputY));
	FSlateDrawElement::MakeLines(OutDrawElements, InLayerId, InGeometry.ToPaintGeometry()
		, OneValueLinePoints, InDrawEffects, NormalBoundsColor, false, NormalBoundsThickness);
}

void SEaseCurveEditor::PaintCurve(const FTrackScaleInfo& InScaleInfo
	, const FGeometry& InGeometry
	, FSlateWindowElementList& OutDrawElements
	, int32 LayerId
	...
	, const FWidgetStyle& InWidgetStyle) const
{
	...
	for (int32 Index = 0; Index < NumKeys - 1; ++Index)
	{
		...
		FSlateDrawElement::MakeLines(OutDrawElements, LayerId, InGeometry.ToPaintGeometry()
			, LinePoints, LineColors, DrawEffects, FLinearColor::White/*Color*/, true, CurveThickness * InGeometry.Scale);
...
void SEaseCurveEditor::CreateLinesForSegment(const ERichCurveInterpMode InInterpMode
	...
	, const FTrackScaleInfo& InScaleInfo) const
{
	...
	const bool bIsEaseCurve = CanEditCurve();
	...
	case RCIM_Cubic:
	{
		...
		for (float CurrentX = StartX; CurrentX < EndX; CurrentX += StepSize)
		{
			...
			switch (Operation.Get(EEaseCurveToolOperation::InOut))
			{
...
int32 SEaseCurveEditor::PaintKeys(const FTrackScaleInfo& InScaleInfo
	, const FGeometry& InGeometry
	, FSlateWindowElementList& OutDrawElements
	, int32 InLayerId
	, const ESlateDrawEffect InDrawEffects
	, const FWidgetStyle& InWidgetStyle) const
{
	...
	for (auto It(EaseCurve->FloatCurve.GetKeyHandleIterator()); It; ++It)
	{
		...
		FSlateDrawElement::MakeBox(OutDrawElements, LayerToUse,
			InGeometry.ToPaintGeometry(KeyHitSize, FSlateLayoutTransform(KeyIconLocation)),
		...
		if (EaseCurve->FloatCurve.GetKeyInterpMode(KeyHandle) == RCIM_Cubic || LastInterpMode == RCIM_Cubic)
		{
			if (KeyIndex == 0)
			{
				PaintTangentHandle(InGeometry, OutDrawElements, LayerToUse, InDrawEffects, InWidgetStyle
...
int32 SEaseCurveEditor::PaintTangentHandle(const FGeometry& InGeometry
	, FSlateWindowElementList& OutDrawElements
	, int32 InLayerId
	, const ESlateDrawEffect InDrawEffects
	, const FWidgetStyle& InWidgetStyle
	...
	, const bool bInSelected) const
{
	...
	// Draw tangent handle
	{
		...
		FSlateDrawElement::MakeBox(OutDrawElements, InLayerId++
			, InGeometry.ToPaintGeometry(TangentDrawSize, FSlateLayoutTransform(TangentIconLocation))
...
int32 SEaseCurveEditor::PaintSelectionText(const FTrackScaleInfo& InScaleInfo
	, const FGeometry& InGeometry
	, FSlateWindowElementList& OutDrawElements
	, int32 InLayerId
	, const ESlateDrawEffect InDrawEffects) const
{
	const FSlateFontInfo FontInfo = FEaseCurveStyle::Get().GetFontStyle(TEXT("Editor.ErrorFont"));
	const FText SelectionText = Tool->IsCurveEditorSelection()
		? LOCTEXT("CurveEditorSelection", "Curve Editor Selection")
		: LOCTEXT("SequencerSelection", "Sequencer Selection");

	const TSharedRef<FSlateFontMeasure> FontMeasure = FSlateApplication::Get().GetRenderer()->GetFontMeasureService();
	const FVector2D TextSize = FontMeasure->Measure(SelectionText, FontInfo);

	const float HalfInputX = InScaleInfo.InputToLocalX(0.5f);
	const float FullOutputY = InScaleInfo.OutputToLocalY(1.0f);

	const FVector2D HalfTextSize = TextSize * 0.5f;
	const FVector2D ActualOffset(HalfInputX - HalfTextSize.X, FullOutputY - TextSize.Y);

	static const FLinearColor TintColor = FStyleColors::Foreground.GetSpecifiedColor().CopyWithNewOpacity(0.5f);

	FSlateDrawElement::MakeText(OutDrawElements, ++InLayerId
		, InGeometry.ToPaintGeometry(InGeometry.Size, FSlateLayoutTransform(ActualOffset))
		, SelectionText, FontInfo, InDrawEffects, TintColor);

	return InLayerId;
}

void SEaseCurveEditor::SetRequireFocusToZoom(const bool bInRequireFocusToZoom)
{
	bRequireFocusToZoom = bInRequireFocusToZoom;
}

TOptional<bool> SEaseCurveEditor::OnQueryShowFocus(const EFocusCause InFocusCause) const
{
...
FReply SEaseCurveEditor::OnMouseButtonDown(const FGeometry& InMyGeometry, const FPointerEvent& InMouseEvent)
{
	...
	else if (EffectingButton == EKeys::RightMouseButton)
	{
		if (HitTestCurves(InMyGeometry, InMouseEvent))
		{
			...
			FEaseCurveTool::ShowNotificationMessage(LOCTEXT("EaseCurveToolTangentsCopied", "Ease Curve Tool Tangents Copied!"));
		}
...
void SEaseCurveEditor::OnMouseCaptureLost(const FCaptureLostEvent& CaptureLostEvent)
{
...
FReply SEaseCurveEditor::OnMouseButtonUp(const FGeometry& InMyGeometry, const FPointerEvent& InMouseEvent)
{
...
FReply SEaseCurveEditor::OnMouseMove(const FGeometry& InMyGeometry, const FPointerEvent& InMouseEvent)
{
...
TSharedRef<IToolTip> SEaseCurveEditor::CreateCurveToolTip()
{
	const FEaseCurveTangents Tangents = EaseCurve->GetTangents();
	...
	const TSharedRef<SWidget> AdditionalContent = SNew(SVerticalBox)
		...
		[
			SNew(STextBlock)
			.Text(this, &SEaseCurveEditor::GetCurveToolTipInputText)
		]
		+ SVerticalBox::Slot()
		.AutoHeight()
		...
		[
			SNew(STextBlock)
			.Text(this, &SEaseCurveEditor::GetCurveToolTipOutputText)
		];
	...
	return SNew(SToolTip)
		...
		[
			...
			[
				SEaseCurvePreviewToolTip::CreateDefaultToolTip(PreviewArgs, AdditionalContent)
			]
			+ SWidgetSwitcher::Slot()
			[
				SNew(STextBlock)
...
void SEaseCurveEditor::UpdateCurveToolTip(const FGeometry& InMyGeometry, const FPointerEvent& InMouseEvent)
{
...
FReply SEaseCurveEditor::OnMouseWheel(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
void SEaseCurveEditor::ZoomView(const FVector2D& InDelta)
{
...
FReply SEaseCurveEditor::OnKeyDown(const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent)
{
...
void SEaseCurveEditor::TryStartDrag(const FGeometry& InMyGeometry, const FPointerEvent& InMouseEvent)
{
	if (!CanEditCurve())
	{
		return;
...
void SEaseCurveEditor::ProcessDrag(const FGeometry& InMyGeometry, const FPointerEvent& InMouseEvent)
{
	const FTrackScaleInfo ScaleInfo(ViewMinInput, ViewMaxInput, ViewMinOutput, ViewMaxOutput, InMyGeometry.GetLocalSize());
...
void SEaseCurveEditor::EndDrag(const FGeometry& InMyGeometry, const FPointerEvent& InMouseEvent)
{
	if (DragState == EDragState::DragTangent)
	{
		OnDragEnd.ExecuteIfBound();
...
void SEaseCurveEditor::ProcessClick(const FGeometry& InMyGeometry, const FPointerEvent& InMouseEvent)
{
	const FKey EffectingButton = InMouseEvent.GetEffectingButton();
...
void SEaseCurveEditor::ClearSelection()
{
	SelectedTangent = FSelectedTangent();
}

void SEaseCurveEditor::SetDefaultOutput(const float InMinZoomRange)
{
...
void SEaseCurveEditor::ZoomToFit()
{
...
FString SEaseCurveEditor::GetReferencerName() const
{
	return TEXT("SEaseCurveEditor");
}

void SEaseCurveEditor::AddReferencedObjects(FReferenceCollector& Collector)
{
	Collector.AddReferencedObject(EaseCurve);
}

void SEaseCurveEditor::SetInputMinMax(const float InNewMin, const float InNewMax)
{
	ViewMinInput = InNewMin;
	ViewMaxInput = InNewMax;
}

void SEaseCurveEditor::SetOutputMinMax(const float InNewMin, const float InNewMax)
{
	ViewMinOutput = InNewMin;
	ViewMaxOutput = InNewMax;
}

bool SEaseCurveEditor::HitTestCurves(const FGeometry& InMyGeometry, const FPointerEvent& InMouseEvent) const
{
	const FTrackScaleInfo ScaleInfo(ViewMinInput, ViewMaxInput, ViewMinOutput, ViewMaxOutput, InMyGeometry.GetLocalSize());
	const FVector2D MousePosition = InMyGeometry.AbsoluteToLocal(InMouseEvent.GetScreenSpacePosition());
...
SEaseCurveEditor::FSelectedTangent SEaseCurveEditor::HitTestTangentHandles(const FGeometry& InMyGeometry, const FPointerEvent& InMouseEvent) const
{
	const FTrackScaleInfo ScaleInfo(ViewMinInput, ViewMaxInput, ViewMinOutput, ViewMaxOutput, InMyGeometry.GetLocalSize());
	const FVector2D MousePosition = InMyGeometry.AbsoluteToLocal(InMouseEvent.GetScreenSpacePosition());
...
bool SEaseCurveEditor::HitTestKey(const FGeometry& InMyGeometry, const FPointerEvent& InMouseEvent, const FVector2D& InInputPosition) const
{
	const FTrackScaleInfo ScaleInfo(ViewMinInput, ViewMaxInput, ViewMinOutput, ViewMaxOutput, InMyGeometry.GetLocalSize());
	const FVector2D MousePosition = InMyGeometry.AbsoluteToLocal(InMouseEvent.GetScreenSpacePosition());
...
void SEaseCurveEditor::MoveSelectedTangent(const FTrackScaleInfo& InScaleInfo, const FVector2D& InScreenDelta)
{
	...
	FEaseCurveTangents NewTangents = MovingTangents;
...
FVector2d SEaseCurveEditor::GetArriveTangentScreenLocation(const FTrackScaleInfo& InScaleInfo, const FKeyHandle& InKeyHandle) const
{
...
FVector2d SEaseCurveEditor::GetArriveTangentScreenLocation(const FTrackScaleInfo& InScaleInfo
...
FVector2d SEaseCurveEditor::GetLeaveTangentScreenLocation(const FTrackScaleInfo& InScaleInfo, const FKeyHandle& InKeyHandle) const
{
	const FVector2d KeyPosition(EaseCurve->FloatCurve.GetKeyTime(InKeyHandle), EaseCurve->FloatCurve.GetKeyValue(InKeyHandle));
	const FRichCurveKey& RichKey = EaseCurve->FloatCurve.GetKey(InKeyHandle);
...
FVector2D SEaseCurveEditor::GetLeaveTangentScreenLocation(const FTrackScaleInfo& InScaleInfo
	, const FVector2d& InKeyPosition, const float InTangent, const float InWeight) const
{
	// Get direction vector from tangent
...
void SEaseCurveEditor::OnObjectPropertyChanged(UObject* Object, FPropertyChangedEvent& PropertyChangedEvent)
{
...
void SEaseCurveEditor::PostUndo(const bool bInSuccess)
{
	ClearSelection();
}

void SEaseCurveEditor::PostRedo(const bool bInSuccess)
{
	PostUndo(bInSuccess);
}

FVector2D SEaseCurveEditor::SnapLocation(const FVector2D& InLocation) const
{
	if (!GridSize.IsSet())
	{
		return InLocation;
...
bool SEaseCurveEditor::HasDrawErrorMessage() const
{
	return ErrorMessage.IsSet() && ErrorMessage.Get().IsSet();
}

bool SEaseCurveEditor::CanEditCurve() const
{
	return bCanEditCurve.Get(false);
}

...
void FIKRetargetEditor::Tick(float DeltaTime)
{
	...
	// copy the LOD from the source to ensure they are always in-sync for previewing LOD'd ops
	const int32 SourceLOD = EditorController->SourceSkelMeshComponent->GetForcedLOD();
	EditorController->TargetSkelMeshComponent->SetDebugForcedLOD(SourceLOD);
	EditorController->TargetSkelMeshComponent->bTrackAttachedInstanceLOD = false;
}

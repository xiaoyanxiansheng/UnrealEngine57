...
FName FIKRetargetHierarchyElement::GetChainName() const
{
	...
	return Controller->GetChainNameFromBone(Name, Controller->GetSourceOrTarget());
}
...
TSharedRef<SWidget> SIKRetargetHierarchyRow::GenerateWidgetForColumn(const FName& ColumnName)
{
	...
	if (ColumnName == ChainColumnName)
	{
		...
		[
			SNew(STextBlock)
			.Text_Lambda([this](){ return FText::FromName(WeakTreeElement.Pin()->GetChainName()); })
			.Font(TextFont)
			.ColorAndOpacity(TextColor)
		];
...
void SIKRetargetHierarchy::RefreshTreeView(bool IsInitialSetup)
{
	...
	for (const FName BoneName : BoneNames)
	{
		...
		// get the chain this bone belongs to
		...
		if (FilterOptions.bHideBonesNotInChain && ChainName == NAME_None)
		{
			BoneElement->bIsHidden = true;
		}

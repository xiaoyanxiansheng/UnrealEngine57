...
public:
	...
	static UE_API UIKRigController* GetController(const UIKRigDefinition* InIKRigDefinition);
	...
	UE_API bool SetSkeletalMesh(USkeletalMesh* SkeletalMesh) const;
	...
	UE_API bool IsSkeletalMeshCompatible(USkeletalMesh* SkeletalMeshToCheck) const;
	...
	UE_API UIKRigDefinition* GetAsset() const;
	UE_API TObjectPtr<UIKRigDefinition>& GetAssetPtr();  
	
	// force all currently connected processors to reinitialize using latest asset state 
	UE_API void BroadcastNeedsReinitialized() const;
	...
	UE_API const FIKRigSkeleton& GetIKRigSkeleton() const;
	...
	UE_API int32 AddSolver(UScriptStruct* InIKRigSolverType) const;

	// Get access to the given solver. 
	UE_API FIKRigSolverBase* GetSolverAtIndex(int32 Index) const;

	// Get access to the instanced struct for a given solver. 
	UE_API FInstancedStruct* GetSolverStructAtIndex(int32 Index) const;

	// Get access to the given solver. 
	UE_API int32 GetIndexOfSolver(FIKRigSolverBase* Solver) const;
	
	// Get read-only access to the array of solvers. 
	UE_API const TArray<FIKRigSolverBase*> GetSolverArray() const;

	// Get unique label for a given solver. Returns dash separated index and name like so, "0 - SolverName". 
	UE_API FString GetSolverUniqueName(int32 SolverIndex) const;
	...
	UFUNCTION(BlueprintCallable, BlueprintPure=false, Category="IK Rig")
	UE_API FName AddNewGoal(const FName GoalName, const FName BoneName) const;

	// Remove the Goal by name. 
	UFUNCTION(BlueprintCallable, BlueprintPure=false, Category="IK Rig")
	UE_API bool RemoveGoal(const FName GoalName) const;

	// Rename a Goal. Returns new name, which may be different after being sanitized. Returns NAME_None if this fails.
	UFUNCTION(BlueprintCallable, BlueprintPure=false, Category="IK Rig")
	UE_API FName RenameGoal(const FName OldName, const FName PotentialNewName) const;

	// The the Bone that the given Goal should be parented to / associated with. 
	UFUNCTION(BlueprintCallable, BlueprintPure=false, Category="IK Rig")
	UE_API bool SetGoalBone(const FName GoalName, const FName NewBoneName) const;

	// The the Bone associated with the given Goal. 
	UFUNCTION(BlueprintCallable, Category="IK Rig")
	UE_API FName GetBoneForGoal(const FName GoalName) const;

	// Get the Goal associated with the given Bone (may be NAME_None) 
	UFUNCTION(BlueprintCallable, Category="IK Rig")
	UE_API FName GetGoalNameForBone(const FName BoneName) const;

	// Connect the given Goal to the given Solver. This creates an "Effector" with settings specific to this Solver.
	UFUNCTION(BlueprintCallable, BlueprintPure=false, Category="IK Rig")
	UE_API bool ConnectGoalToSolver(const FName GoalName, int32 SolverIndex) const;

	// Disconnect the given Goal from the given Solver. This removes the Effector that associates the Goal with the Solver.
	UFUNCTION(BlueprintCallable, BlueprintPure=false, Category="IK Rig")
	UE_API bool DisconnectGoalFromSolver(const FName GoalToRemove, int32 SolverIndex) const;

	// Returns true if the given Goal is connected to the given Solver. False otherwise. 
	UFUNCTION(BlueprintCallable, Category="IK Rig")
	UE_API bool IsGoalConnectedToSolver(const FName GoalName, int32 SolverIndex) const;

	// Returns true if the given Goal is connected to ANY solver. False otherwise. 
	UFUNCTION(BlueprintCallable, Category="IK Rig")
	UE_API bool IsGoalConnectedToAnySolver(const FName GoalName) const;

	// Get read-write access to the Goal with the given name.
	UFUNCTION(BlueprintCallable, Category="IK Rig")
	UE_API UIKRigEffectorGoal* GetGoal(const FName GoalName) const;

	// Get access to the list of Goals. 
	UFUNCTION(BlueprintCallable, Category="IK Rig")
	UE_API const TArray<UIKRigEffectorGoal*>& GetAllGoals() const;
	...
	UFUNCTION(BlueprintCallable, Category="IK Rig")
	UE_API UObject* GetGoalSettingsForSolver(const FName GoalName, int32 SolverIndex) const;

	// returns true if the goal has custom settings for the given solver
	UFUNCTION(BlueprintCallable, Category="IK Rig")
	UE_API bool DoesSolverHaveCustomGoalSettings(const FName GoalName, int32 SolverIndex) const;
	...
	UE_API int32 GetGoalIndex(const FName InGoalName, const ENameCase CompareMethod = ENameCase::IgnoreCase) const;

	// Get the global-space transform of the given Goal. This may be set by the user in the editor, or at runtime. 
	UE_API FTransform GetGoalCurrentTransform(const FName GoalName) const;

	// Set the Goal to the given transform. 
	UE_API void SetGoalCurrentTransform(const FName GoalName, const FTransform& Transform) const;

	// Reset all Goals back to their initial transforms. 
	UE_API void ResetGoalTransforms() const;

	// Reset initial goal transform 
	UE_API void ResetInitialGoalTransforms() const;

	// Ensure that the given name adheres to required standards for Goal names (no special characters etc..)
	static UE_API void SanitizeGoalName(FString& InOutName);

	// Add a suffix as needed to ensure the Goal name is unique 
	UE_API FName GetUniqueGoalName(const FName NameToMakeUnique) const;

	// Modify a Goal for a transaction. Returns true if Goal found.
	UE_API bool ModifyGoal(const FName GoalName) const;
	...
	UFUNCTION(BlueprintCallable, BlueprintPure=false, Category="IK Rig")
	UE_API bool SetBoneExcluded(const FName BoneName, const bool bExclude) const;

	// Returns true if the given Bone is excluded, false otherwise. 
	UFUNCTION(BlueprintCallable, Category="IK Rig")
	UE_API bool GetBoneExcluded(const FName BoneName) const;
	
	// Add settings to the given Bone/Solver. Does nothing if Bone already has settings in this Solver.
	UFUNCTION(BlueprintCallable, BlueprintPure=false, Category="IK Rig")
	UE_API bool AddBoneSetting(const FName BoneName, int32 SolverIndex) const;

	// Remove settings for the given Bone/Solver. Does nothing if Bone doesn't have setting in this Solver.
	UFUNCTION(BlueprintCallable, BlueprintPure=false, Category="IK Rig")
	UE_API bool RemoveBoneSetting(const FName BoneName, int32 SolverIndex) const;

	// Get the global-space retarget pose transform of the given Bone.
	UFUNCTION(BlueprintCallable, Category="IK Rig")
	UE_API FTransform GetRefPoseTransformOfBone(const FName BoneName) const;
	...
	UFUNCTION(BlueprintCallable, BlueprintPure=false, Category="IK Rig")
	UE_API UObject* GetBoneSettings(const FName BoneName, int32 SolverIndex) const;
	...
	UE_API FIKRigBoneSettingsBase* GetSettingsForBone(const FName BoneName, int32 SolverIndex) const;

	// Returns true if this Bone can have settings in the given Solver. 
	UE_API bool CanAddBoneSetting(const FName BoneName, int32 SolverIndex, FText* OutError = nullptr) const;

	// Returns true if settings for this Bone can be removed from the given Solver. 
	UE_API bool CanRemoveBoneSetting(const FName BoneName, int32 SolverIndex) const;

	// Returns true if the given Bone has any settings in any Solver. 
	UE_API bool DoesBoneHaveSettings(const FName BoneName) const;
	...
	UFUNCTION(BlueprintCallable, BlueprintPure=false, Category="IK Rig")
	UE_API FName AddRetargetChain(const FName ChainName, const FName StartBoneName, const FName EndBoneName, const FName GoalName) const;

	// Remove a Chain with the given name. Returns true if a Chain was removed. 
	UFUNCTION(BlueprintCallable, BlueprintPure=false, Category="IK Rig")
	UE_API bool RemoveRetargetChain(const FName ChainName) const;

	// Renamed the given Chain. Returns the new name (same as old if unsuccessful). 
	UFUNCTION(BlueprintCallable, BlueprintPure=false, Category="IK Rig")
	UE_API FName RenameRetargetChain(const FName ChainName, const FName NewChainName) const;

	// Set the Start Bone for the given Chain. Returns true if operation was successful. 
	UFUNCTION(BlueprintCallable, BlueprintPure=false, Category="IK Rig")
	UE_API bool SetRetargetChainStartBone(const FName ChainName, const FName StartBoneName) const;

	// Set the End Bone for the given Chain. Returns true if operation was successful. 
	UFUNCTION(BlueprintCallable, BlueprintPure=false, Category="IK Rig")
	UE_API bool SetRetargetChainEndBone(const FName ChainName, const FName EndBoneName) const;

	// Set the Goal for the given Chain. Returns true if operation was successful. 
	UFUNCTION(BlueprintCallable, BlueprintPure=false, Category="IK Rig")
	UE_API bool SetRetargetChainGoal(const FName ChainName, const FName GoalName) const;

	// Get the Goal name for the given Chain. 
	UFUNCTION(BlueprintCallable, Category="IK Rig")
	UE_API FName GetRetargetChainGoal(const FName ChainName) const;

	// Get the End Bone name for the given Chain. 
	UFUNCTION(BlueprintCallable, Category="IK Rig")
    UE_API FName GetRetargetChainStartBone(const FName ChainName) const;

	// Get the Start Bone name for the given Chain. 
	UFUNCTION(BlueprintCallable, Category="IK Rig")
    UE_API FName GetRetargetChainEndBone(const FName ChainName) const;

	// Get read-only access to the list of Chains. 
	UFUNCTION(BlueprintCallable, Category="IK Rig")
	UE_API const TArray<FBoneChain>& GetRetargetChains() const;
	...
	UE_API bool SetRetargetRoot(const FName RootBoneName) const;
	...
	UE_API FName GetRetargetRoot() const;
	...
	UFUNCTION(BlueprintCallable, Category="IK Rig")
	UE_API bool ApplyAutoGeneratedRetargetDefinition();
	...
	UE_API void SetRetargetDefinition(const FRetargetDefinition& RetargetDefinition) const;

	// Auto generates a retarget definition and returns the results
	UE_API void AutoGenerateRetargetDefinition(FAutoCharacterizeResults& Results) const;

	// Auto generates an FBIK setup for the current skeletal mesh
	UE_API void AutoGenerateFBIK(FAutoFBIKResults& Results) const;

	// Get read-only access to characterizer
	UE_API const FAutoCharacterizer& GetAutoCharacterizer() const;
	
	// Add a Chain with the given BoneChain settings. Returns newly created chain name.
	UE_API FName AddRetargetChainInternal(const FBoneChain& BoneChain) const;
	
	// Get read-only access to a single retarget chain with the given name 
	UE_API const FBoneChain* GetRetargetChainByName(const FName ChainName) const;
	
	// Get the name of the retarget chain that contains the given Bone. Returns NAME_None if Bone not in a Chain. 
	UE_API FName GetRetargetChainFromBone(const FName BoneName, const FIKRigSkeleton* OptionalSkeleton) const;

	// Get the name of the retarget chain that contains the given Goal. Returns NAME_None if Goal not in a Chain. 
	UE_API FName GetRetargetChainFromGoal(const FName GoalName) const;

	// Sorts the Chains from Root to tip based on the Start Bone of each Chain. 
	UE_API void SortRetargetChains() const;

	// Make unique name for a retargeting bone chain. Adds a numbered suffix to make it unique.
	UE_API FName GetUniqueRetargetChainName(FName NameToMakeUnique) const;
	...
	UE_API bool ValidateChain( const FName ChainName, const FIKRigSkeleton* OptionalSkeleton, TSet<int32>& OutChainIndices) const;

	// returns an array of wrappers to display structs in the details panel
	// NOTE: these are recycled by the editor using this controller (assumes one editor per asset)
	UE_API TArray<UIKRigStructWrapperBase*> GetStructWrappers(const int32 InNum, TSubclassOf<UIKRigStructWrapperBase> InType);
...
private:
	...
	// a pool of objects inheriting from UIKRigStructWrapperBase for editing various IK Rig struct types in a details view
	// NOTE: pool contains a bucket per settings type
	UPROPERTY(transient)
	FIKRigStructWrapperPool StructViewerPool;

	// Static global map of assets to their associated controller
	static UE_API TMap<UIKRigDefinition*, UIKRigController*> Controllers;

	// broadcast changes within the asset goals array
	UE_API void BroadcastGoalsChange() const;

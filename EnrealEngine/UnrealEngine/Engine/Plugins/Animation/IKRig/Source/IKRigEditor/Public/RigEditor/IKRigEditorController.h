...
public:
	...
	UE_API TOptional<FTransform> GetTransform(EIKRigTransformType::Type TransformType) const;
	UE_API bool IsComponentRelative(ESlateTransformComponent::Type Component, EIKRigTransformType::Type TransformType) const;
	UE_API void OnComponentRelativeChanged(ESlateTransformComponent::Type Component, bool bIsRelative, EIKRigTransformType::Type TransformType);
	UE_API void OnCopyToClipboard(ESlateTransformComponent::Type Component, EIKRigTransformType::Type TransformType) const;
	UE_API void OnPasteFromClipboard(ESlateTransformComponent::Type Component, EIKRigTransformType::Type TransformType);
...
private:
	...
	static UE_API bool CurrentTransformRelative[3];
	static UE_API bool ReferenceTransformRelative[3];
};
...
public:
	...
	void ShowDetailsForBoneSettings(const TArray<TSharedPtr<FIKRigTreeElement>>& InItems) const;
	// show GOAL settings in details view 
	void ShowDetailsForGoals(const TArray<FName>& GoalName) const;
	// show EFFECTOR settings in details view 
	void ShowDetailsForGoalSettings(const TArray<TSharedPtr<FIKRigTreeElement>>& InItems) const;
	// show SOLVER settings in details view 
	void ShowDetailsForSolvers(const TArray<int32>& InSolverIndices) const;
	...
	// show the currently configured IKRigStructWrapper objects in the details tab
	void ShowStructWrappersInDetailsView(TArray<UIKRigStructWrapperBase*> InStructWrappers) const;
...
private:
	...
	TArray<TSharedPtr<FIKRigTreeElement>> FilterByFirstItemType(

...
#include UE_INLINE_GENERATED_CPP_BY_NAME(SIKRigHierarchy)
...
FIKRigTreeElement::FIKRigTreeElement(const FText& InKey, EIKRigTreeElementType InType,
...
void SIKRigHierarchyItem::Construct(const FArguments& InArgs)
{
	...
	if (TreeElement->ElementType == EIKRigTreeElementType::BONE)
	{
		RowBox->AddSlot()
        .AutoWidth()
        .VAlign(VAlign_Center)
...
FReply SIKRigSkeletonRow::HandleDragDetected(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	if (DraggedElement->ElementType != EIKRigTreeElementType::GOAL)
	{
		return FReply::Unhandled();
	}
    ...
    TSharedPtr<FIKRigTreeElement> TargetItem)
{
	...
	{
		if (TargetItem.Get()->ElementType == EIKRigTreeElementType::BONE)
        {
    ...
    TSharedPtr<FIKRigTreeElement> TargetItem)
{
	...
	if (DragDropOp->Element.IsValid())
	{
		return FReply::Handled();
	}

	const FIKRigTreeElement& DraggedElement = *DragDropOp.Get()->Element.Pin().Get();
	ensure(DraggedElement.ElementType == EIKRigTreeElementType::GOAL);		// drag only supported for goals
	ensure(TargetItem.Get()->ElementType == EIKRigTreeElementType::BONE);	// drop only supported for bones
...
void SIKRigHierarchy::AddSelectedItemFromViewport(
	...
	EIKRigTreeElementType ItemType,
	const bool bReplace)
{	
	// nothing to add
...
{
	if (TreeElement.Pin()->ElementType != EIKRigTreeElementType::BONE)
	{
		return false;
	}
...
void SIKRigHierarchy::BindCommands()
{
	...
	CommandList->MapAction(Commands.RefreshHierarchy,
	FExecuteAction::CreateSP(this, &SIKRigHierarchy::HandleRefreshHierarchy),
		FCanExecuteAction::CreateSP(this, &SIKRigHierarchy::CanRefreshHierarchy));
...
void SIKRigHierarchy::FillContextMenu(FMenuBuilder& MenuBuilder)
{
	...
	MenuBuilder.BeginSection("Hierarchy", LOCTEXT("HierarchyOperations", "Hierarchy"));
	MenuBuilder.AddMenuEntry(Actions.RefreshHierarchy);
	MenuBuilder.EndSection();
}
...
void SIKRigHierarchy::HandleNewGoal() const
{
	...
	{
		if (Item->ElementType != EIKRigTreeElementType::BONE)
		{
...
bool SIKRigHierarchy::CanDeleteElement() const
{
	...
	for (const TSharedPtr<FIKRigTreeElement>& Item : SelectedItems)
	{
		const bool bIsBone = Item->ElementType == EIKRigTreeElementType::BONE;
		if (bIsBone)
		{
			return false;
		}
...
void SIKRigHierarchy::HandleSetRootBoneOnSolvers()
{
	...
	TArray<TSharedPtr<FSolverStackElement>> SelectedSolvers;
	Controller->GetSelectedSolvers(SelectedSolvers);
	TArray<int32> SolversToShow;
	for (const TSharedPtr<FSolverStackElement>& Solver : SelectedSolvers)
	{
		if (AssetController->SetStartBone(RootBoneName, Solver->IndexInStack))
		{
			SolversToShow.Add(Solver->IndexInStack);
		}
	}

	// show solver that had it's root bone updated
	Controller->ShowDetailsForSolvers(SolversToShow);
...
void SIKRigHierarchy::HandleSetEndBoneOnSolvers()
{
	...
	for (const TSharedPtr<FSolverStackElement>& Solver : SelectedSolvers)
	{
		if (AssetController->SetEndBone(RootBoneName, Solver->IndexInStack))
		{
			SolversToShow.Add(Solver->IndexInStack);
		}
	}
...
void SIKRigHierarchy::HandleAddBoneSettings()
{
	...
	Controller->ShowDetailsForBone(SelectedBones[0]->BoneName);
...
bool SIKRigHierarchy::CanRefreshHierarchy()
{
	FIKRigEditorController* Controller = EditorController.Pin().Get();
	if (!Controller)
	{
	...
	return Controller->AssetController->GetSkeletalMesh() != nullptr;
}

void SIKRigHierarchy::HandleRefreshHierarchy()
{
	FIKRigEditorController* Controller = EditorController.Pin().Get();
	if (!Controller)
	{
	...
	if (USkeletalMesh* SkeletalMesh = Controller->AssetController->GetSkeletalMesh())
	{
		Controller->AssetController->SetSkeletalMesh(SkeletalMesh);
	}
}
...
void SIKRigHierarchy::GetSelectedGoals(TArray<TSharedPtr<FIKRigTreeElement>>& OutSelectedGoals) const
{
	...
	for (const TSharedPtr<FIKRigTreeElement>& Item : SelectedItems)
	{
		if (Item->ElementType == EIKRigTreeElementType::GOAL)
		{
...
void SIKRigHierarchy::RefreshTreeView(bool IsInitialSetup /*=false*/)
{
	...
	for (int32 BoneIndex=0; BoneIndex<Skeleton.BoneNames.Num(); ++BoneIndex)
	{
		...
		TSharedPtr<FIKRigTreeElement> BoneElement = MakeShared<FIKRigTreeElement>(BoneDisplayName, EIKRigTreeElementType::BONE, ControllerRef);
		...
		for (int32 SolverIndex=0; SolverIndex<Solvers.Num(); ++SolverIndex)
		{
			...
			TSharedPtr<FIKRigTreeElement> SettingsItem = MakeShared<FIKRigTreeElement>(BoneSettingDisplayName, EIKRigTreeElementType::BONE_SETTINGS, ControllerRef);
		...
		for (const UIKRigEffectorGoal* Goal : Goals)
		{
			...
			TSharedPtr<FIKRigTreeElement> GoalItem = MakeShared<FIKRigTreeElement>(GoalDisplayName, EIKRigTreeElementType::GOAL, ControllerRef);
			...
			for (int32 SolverIndex=0; SolverIndex<Solvers.Num(); ++SolverIndex)
			{
				...
				TSharedPtr<FIKRigTreeElement> EffectorItem = MakeShared<FIKRigTreeElement>(EffectorDisplayName, EIKRigTreeElementType::GOAL_SETTINGS, ControllerRef);

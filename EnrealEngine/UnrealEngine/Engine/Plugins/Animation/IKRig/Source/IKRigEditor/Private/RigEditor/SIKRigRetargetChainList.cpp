...
void SIKRigRetargetChainList::Construct(const FArguments& InArgs, TSharedRef<FIKRigEditorController> InEditorController)
{
    ...
    [
        ...
        [
			...
			[
				SNew(STextBlock)
				.Text(LOCTEXT("PelvisLabel", "Pelvis:"))
			...
			.Padding(3.0f, 3.0f)
			[
				SAssignNew(PelvisTextBox, SEditableTextBox)
...
void SIKRigRetargetChainList::RefreshView()
{
	...
	// refresh pelvis
	PelvisTextBox.Get()->SetText(FText::FromName(Controller->AssetController->GetRetargetRoot()));
...
void SIKRigRetargetChainList::MirrorSelectedChains() const
{
	...
	for (const FName& SelectedChainName : SelectedChainNames)
	{
		...
		if (GoalOnOldChainName != NAME_None && GoalOnNewChain)
		{
			...
			const TArray<FIKRigSolverBase*> AllSolvers =  AssetController->GetSolverArray();

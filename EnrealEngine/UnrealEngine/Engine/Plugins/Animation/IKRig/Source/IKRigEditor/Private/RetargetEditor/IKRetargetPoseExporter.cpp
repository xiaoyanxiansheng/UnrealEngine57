...
FReply FIKRetargetPoseExporter::ImportPoseAsset() const
{
	...
	for (int32 BoneIndex=0; BoneIndex<RefSkeleton.GetNum(); ++BoneIndex)
	{
		...
		const double DeltaAngle = FMath::RadiansToDegrees(DeltaRotation.GetAngle());
		constexpr double MinAngleThreshold = 0.05;
...
FReply FIKRetargetPoseExporter::OnImportPoseFromSequence()
{
	...
	const FName PelvisBoneName = ControllerPtr->AssetController->GetPelvisBone(ControllerPtr->GetSourceOrTarget());
	for (int32 BoneIndex = 0; BoneIndex < NumBones; BoneIndex++)
	{
		const FName& BoneName = RefSkeleton.GetBoneName(BoneIndex);
		
		// if this is the pelvis, we want to record the translation delta as well
		if (BoneName == PelvisBoneName)
		{
			...
			// rotation offsets are interpreted as relative to the parent (local), but in the case of the pelvis bone,
			...
			// this is a bit crazy, but we have to generate a delta rotation in the local space of the pelvis

...
enum class EIKRigTreeElementType { BONE, GOAL, GOAL_SETTINGS, BONE_SETTINGS };
...
public:
	...
	FIKRigTreeElement(
		const FText& InKey,
		EIKRigTreeElementType InType,
		const TSharedRef<FIKRigEditorController>& InEditorController);

	FText Key;
	EIKRigTreeElementType ElementType;
...
private:
	...
	/** refresh the bone hierarchy */
	bool CanRefreshHierarchy();
	void HandleRefreshHierarchy();

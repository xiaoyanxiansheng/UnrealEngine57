	...
	FVector& OutCurrentCentroid)
{
	...
	// average centroids
	const double InvNumEffectors = 1.0f / static_cast<double>(InInitialPoints.Num());
	...
	for (unsigned int Iter = 0; Iter < MaxIter; Iter++)
	{
		...
		Omega *= 1.0 / (fabs(RCol0.Dot(DX) + RCol1.Dot(DY) + RCol2.Dot(DZ)) + UE_SMALL_NUMBER);
		const double W = Omega.Size();

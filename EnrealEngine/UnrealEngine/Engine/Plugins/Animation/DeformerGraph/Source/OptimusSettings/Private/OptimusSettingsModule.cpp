...
namespace Optimus
{
	...
	static const TCHAR* DeformerGraphEnableAssetValidationCVarName = TEXT("r.DeformerGraph.EnableAssetValidation");
	static TAutoConsoleVariable<int32> CVarDeformerGraphEnableAssetValidation(
	DeformerGraphEnableAssetValidationCVarName,
	1,
	TEXT("Set to 0 to disable Deformer Graph validation that may generate "
	  "extra warnings when Deformer Graph cannot run optimally on certain assets"));

	bool IsAssetValidationEnabled()
	{
		return CVarDeformerGraphEnableAssetValidation.GetValueOnAnyThread() != 0;
	}

	FName GetEnableAssetValidationCVarName()
	{
		return DeformerGraphEnableAssetValidationCVarName;
	}
}

	...
	EditorPtr(InEditorApp)
{
	...
	TabFactories.RegisterFactory(MakeShared<FOptimusEditorFindTabSummoner>(InEditorApp));
...
TSharedRef<FTabManager::FLayout> FOptimusEditorMode::CreatePaneLayout()
{
	...
	TSharedRef<FTabManager::FLayout> Layout = FTabManager::NewLayout("Standalone_OptimusEditor_Layout_v07")
		->AddArea(
			...
			->Split(							// - Main work area
				...
				->Split(						// -- Graph + output
					...
					->Split(					// --- Output
						...
						->AddTab(FOptimusEditorFindTabSummoner::TabId, ETabState::OpenedTab)
						->SetForegroundTab(FOptimusEditorCompilerOutputTabSummoner::TabId)
					)
				)

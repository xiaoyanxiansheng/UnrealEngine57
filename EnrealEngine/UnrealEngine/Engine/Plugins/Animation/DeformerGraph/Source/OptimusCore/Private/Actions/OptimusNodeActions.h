...
private:
	...
	FDeprecateSlateVector2D NewPosition;

	// The node's old position
	FDeprecateSlateVector2D OldPosition;
};

USTRUCT()
struct FOptimusCommentNodeAction_ResizeNode :
	public FOptimusAction
{
	GENERATED_BODY()

public:
	FOptimusCommentNodeAction_ResizeNode() = default;

	FOptimusCommentNodeAction_ResizeNode(
...
private:
	// The path of the node to be resized
	FString NodePath;

	FDeprecateSlateVector2D OldSize;

	FDeprecateSlateVector2D NewSize;
};

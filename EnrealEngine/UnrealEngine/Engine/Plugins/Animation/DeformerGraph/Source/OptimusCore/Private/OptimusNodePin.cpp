...
FText UOptimusNodePin::GetDisplayName() const
{
	...
	const FString PinName = !DisplayName.IsNone() ? DisplayName.ToString() : GetName();
	return FText::FromString(FName::NameToDisplayString(PinName, bIsBool));
}
...
bool UOptimusNodePin::SetDisplayName(const FName& InDisplayName)
{

...
) const
{
	...
	UOptimusComponentSourceBinding* PrimaryBinding = *(PrimaryBindings.CreateIterator());
	...
	bool bSupportUnifiedDispatchOnly = false;
	
	check(InOutKernelDataInterface)
	{
		TOptional<FText> Result = BindKernelDataInterfaceForComputeKernel(
			PrimaryBinding,
			InOutKernelDataInterface,
			KernelSource,
			OutInputDataBindings,
			GeneratedFunctions,
			bSupportUnifiedDispatchOnly
			);
	...
	for (const UOptimusNodePin* Pin: GetPins())
	{
		if (Pin->IsGroupingPin())
		{
			for (const UOptimusNodePin* SubPin: Pin->GetSubPins())
			{
				TOptional<FText> Result = ProcessInputPinForComputeKernel(
					bSupportUnifiedDispatchOnly,
					InTraversalContext, SubPin,
					GroupIndex == 0 ? FString() : Pin->GetName(),
					InKernelInputs, KernelSource,  GeneratedFunctions, OutInputDataBindings, bIsUnifiedDispatch
					);
				if (Result.IsSet())
				{
					return ReturnError(Result.GetValue());
		...
		{
			TOptional<FText> Result = ProcessOutputPinForComputeKernel(
				bSupportUnifiedDispatchOnly,
				InTraversalContext, Pin,
				InKernelOutputs,
				KernelSource, GeneratedFunctions, OutInputDataBindings, OutOutputDataBindings, bIsUnifiedDispatch
				);
	...
	CookedSource += "\n\n";
	CookedSource += GetKernelSourceText(bIsUnifiedDispatch);
...
const TCHAR* UOptimusNode_ComputeKernelBase::GetNumThreadsReservedName()
{
	static const TCHAR* NumThreadsReservedName = TEXT("NumThreads");
	return NumThreadsReservedName;
}


TOptional<FText> UOptimusNode_ComputeKernelBase::ProcessInputPinForComputeKernel(
	bool bSupportUnifiedDispatchOnly,
	const FOptimusPinTraversalContext& InTraversalContext,
	...
	) const
{
	...
	if (const FOptimus_KernelConnection* InputConnection = InKernelInputs.Find(InInputPin))
	{
		...
		if (!DataInterface->CanSupportUnifiedDispatch() )
		{
			...
			if (bSupportUnifiedDispatchOnly)
			{
				return FText::Format(LOCTEXT("InvalidInput_KernelOnlySupportsDataInterfaceUnifiedDispatch", "Cannot connect input binding pin '{0}' to a data interface that does not support unified dispatch, consider using a simple kernel execution domain or inserting a kernel inbetween"), FText::FromName(InInputPin->GetUniqueName()));
			}

			bOutIsUnifiedDispatch = false;
		}
...
TOptional<FText> UOptimusNode_ComputeKernelBase::ProcessOutputPinForComputeKernel(
	bool bSupportUnifiedDispatchOnly,
	const FOptimusPinTraversalContext& InTraversalContext,
	...
	if (const TArray<FOptimus_KernelConnection>* KernelConnections = InKernelOutputs.Find(InOutputPin))
	{
		...
		for (const FOptimus_KernelConnection& KernelConnection: *KernelConnections)
		{
			...
			if (!DataInterface->CanSupportUnifiedDispatch())
			{
				if (bSupportUnifiedDispatchOnly)
				{
					return FText::Format(LOCTEXT("InvalidOutput_KernelOnlySupportsDataInterfaceUnifiedDispatch", "Cannot connect output binding pin '{0}' to a data interface that does not support unified dispatch, consider using to a simple kernel execution domain or inserting a kernel inbetween"), FText::FromName(InOutputPin->GetUniqueName()));
				}
...
TOptional<FText> UOptimusNode_ComputeKernelBase::BindKernelDataInterfaceForComputeKernel(
	const UOptimusComponentSourceBinding* InKernelPrimaryComponentSourceBinding,
	UComputeDataInterface* InOutKernelDataInterface,
	UOptimusKernelSource* InKernelSource,
	FOptimus_InterfaceBindingMap& OutInputDataBindings,
	TArray<FString>& OutGeneratedFunctions,
	bool& bOutUnifiedDispatchOnly
	) const
{
	check(InOutKernelDataInterface);
	...
	check(KernelDataInterface);
	FString NumThreadsExpression = GetExecutionDomain().AsExpression();
	KernelDataInterface->SetExecutionDomain(NumThreadsExpression);
	KernelDataInterface->SetComponentBinding(InKernelPrimaryComponentSourceBinding);
	
	TVariant<bool, FText> UnifiedDispatchResult = Optimus::IsExecutionDomainUnifiedDispatchOnly(NumThreadsExpression,InKernelPrimaryComponentSourceBinding->GetComponentSource());

	if (UnifiedDispatchResult.IsType<FText>())
	{
		return UnifiedDispatchResult.Get<FText>();
	}

	bOutUnifiedDispatchOnly = UnifiedDispatchResult.Get<bool>();
	...
	OutGeneratedFunctions.Add(FString::Printf(
			TEXT("uint3 Read%s() { return %s::%s(); }"),
			GetNumThreadsReservedName(), Optimus::GetKernelInternalNamespaceName(), KernelDataInterface->GetReadNumThreadsFunctionName() ));

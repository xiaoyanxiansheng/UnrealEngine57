...
bool FOptimusComponentBindingAction_AddBinding::Do(IOptimusPathResolver* InRoot)
{
	...
	const UClass* ComponentSourceClass = Cast<UClass>(StaticFindObject(UClass::StaticClass(), ComponentSourceClassPath, EFindObjectFlags::None));
...
{
	const UClass* ComponentSourceClass = Cast<UClass>(StaticFindObject(UClass::StaticClass(), InComponentSourceClassPath, EFindObjectFlags::None));
	if (!ComponentSourceClass)
	{
		return false;
	}

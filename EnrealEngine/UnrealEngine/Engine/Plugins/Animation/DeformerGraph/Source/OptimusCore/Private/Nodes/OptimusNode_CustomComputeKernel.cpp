...
FString UOptimusNode_CustomComputeKernel::GetKernelSourceText(bool bInIsUnifiedDispatch) const
{
	using UKernelDI = UOptimusCustomComputeKernelDataInterface;
	
	return Optimus::GetCookedKernelSource(
		GetPathName(),
	...
	)
{
	...
	if (NewName == GetNumThreadsReservedName())
	{
		NewName.SetNumber(1);
	}
...
void UOptimusNode_CustomComputeKernel::UpdatePreamble()
{
	...
	ShaderSource.Declarations += "uint3 ReadNumThreads();\n";
}
...
bool UOptimusNode_CustomComputeKernel::PostLoadRemoveDeprecatedNumThreadsPin()
{
	...
	const FName NumThreadsPinName = GetNumThreadsReservedName();

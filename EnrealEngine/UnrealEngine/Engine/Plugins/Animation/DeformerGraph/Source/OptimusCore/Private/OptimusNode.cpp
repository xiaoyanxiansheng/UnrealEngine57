...
FText UOptimusNode::GetTooltipText() const
{
	return {};
}
...
UOptimusNodePin* UOptimusNode::GetPinByName(const FName& PinName) const
{
...
void UOptimusNode::PostCreateNode()
{
	...
	// Update the pins display names
	InitializePinsDisplayName();
}

void UOptimusNode::PostDeserializeNode()
{
	TGuardValue<bool> NodeConstructionGuard(bConstructingNode, true);
	InitializeTransientData();
	InitializePinsDisplayName();	
...
void UOptimusNode::PostLoad()
{
	...
	// Initialize transient data for nodes and pins
	PostDeserializeNode();
}

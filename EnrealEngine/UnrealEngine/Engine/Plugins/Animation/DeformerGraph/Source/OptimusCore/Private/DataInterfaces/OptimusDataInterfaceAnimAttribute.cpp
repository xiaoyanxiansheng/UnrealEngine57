	...
	)
{
	...
	for (FOptimusAnimAttributeRuntimeData& Attribute : AttributeRuntimeData)
	{
		...
		if (SkeletalMesh.IsValid() && SkeletalMesh->GetSkeletalMeshAsset())
		{
...
void FOptimusAnimAttributeDataProviderProxy::AllocateResources(FRDGBuilder& GraphBuilder, FAllocationData const& InAllocationData)
{
	ArrayBuffers.Reset();

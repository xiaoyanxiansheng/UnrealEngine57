...
void FSkeletalMeshPoseTrack::BuildContextMenu(FMenuBuilder& MenuBuilder)
{
	...
	if(GameplayProvider)
	{
		...
		const FObjectInfo* ComponentObjectInfo = GameplayProvider->FindObjectInfo(GetGameplayTrack().GetAssociatedObjectId());
		if(ComponentObjectInfo != nullptr)
		{
			...
			const FObjectInfo* ActorObjectInfo = GameplayProvider->FindObjectInfo(ComponentObjectInfo->GetOuterId());
					...
					MenuBuilder.AddMenuEntry(
						...
						FUIAction(
							FExecuteAction::CreateLambda([this, ActorObjectInfo, GameplayProvider]()
							{
								...
								{
									const FObjectInfo* OtherComponentObjectInfo = GameplayProvider->FindObjectInfo(InTrack->GetGameplayTrack().GetAssociatedObjectId());
									if(OtherComponentObjectInfo->GetOuterId() == ActorObjectInfo->GetId())
									{

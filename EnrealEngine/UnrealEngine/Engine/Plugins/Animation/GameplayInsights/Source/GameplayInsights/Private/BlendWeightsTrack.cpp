...
TConstArrayView<TSharedPtr<FRewindDebuggerTrack>> FBlendWeightsTrack::GetChildrenInternal(TArray<TSharedPtr<FRewindDebuggerTrack>>& OutTracks) const
{
...
TConstArrayView<TSharedPtr<FRewindDebuggerTrack>> FBlendWeightTrack::GetChildrenInternal(TArray<TSharedPtr<FRewindDebuggerTrack>>& OutTracks) const
{
	return MakeConstArrayView<TSharedPtr<FRewindDebuggerTrack>>(reinterpret_cast<const TSharedPtr<FRewindDebuggerTrack>*>(Children.GetData()), Children.Num());
}
...
TSharedPtr<RewindDebugger::FRewindDebuggerTrack> FBlendWeightsTrackCreator::CreateTrackInternal(const FObjectId& InObjectId) const
{
 	return MakeShared<RewindDebugger::FBlendWeightsTrack>(InObjectId.GetMainId());
}

bool FBlendWeightsTrackCreator::HasDebugInfoInternal(const FObjectId& InObjectId) const
{
	...
	{
		AnimationProvider->ReadTickRecordTimeline(InObjectId.GetMainId(), [&bHasData](const FAnimationProvider::TickRecordTimeline& InGraphTimeline)
		{
			bHasData = true;
		});
	}

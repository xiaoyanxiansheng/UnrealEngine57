...
public:
	...
	virtual bool ReadObjectEventsTimeline(const RewindDebugger::FObjectId& InObjectId, TFunctionRef<void(const ObjectEventsTimeline&)> Callback) const override;
	virtual bool ReadObjectEvent(const RewindDebugger::FObjectId& InObjectId, uint64 InMessageId, TFunctionRef<void(const FObjectEventMessage&)> Callback) const override;
	...
	virtual void EnumerateSubobjects(const RewindDebugger::FObjectId& InObjectId, TFunctionRef<void(const RewindDebugger::FObjectId& SubObjectId)> Callback) const override;
	...
	virtual const FObjectInfo* FindObjectInfo(const uint64 InObjectId) const override;
	virtual const FObjectInfo* FindObjectInfo(const RewindDebugger::FObjectId& InObjectId) const override;
	...
	virtual const FObjectInfo& GetObjectInfo(const RewindDebugger::FObjectId& InObjectId) const override;
	virtual FOnObjectEndPlay& OnObjectEndPlay() override
	{
		return OnObjectEndPlayDelegate;
	}
	...
	void AppendObject(const RewindDebugger::FObjectId& InObjectId, const RewindDebugger::FObjectId& InOuterId, uint64 InClassId, const TCHAR* InObjectName, const TCHAR* InObjectPathName, EObjectInfoFlags Flags);
	...
	void AppendObjectLifetimeBegin(const RewindDebugger::FObjectId& InObjectId, double InProfileTime, double InRecordingTime);

	/** Add an object destroy message */
	void AppendObjectLifetimeEnd(const RewindDebugger::FObjectId& InObjectId, double InProfileTime, double InRecordingTime);	

	/** Add an object event message */
	void AppendObjectEvent(const RewindDebugger::FObjectId& InObjectId, double InTime, const TCHAR* InEvent);

	/** Add a Controller Attach message */
	void AppendPawnPossess(uint64 InControllerId, uint64 InPawnId, double InTime);

	/** Add a view message */
	void AppendView(uint64 InPlayerId, double InTime, const FVector& InPosition, const FRotator& InRotation, float InFov, float InAspectRatio);

	/** Add a world message */
	void AppendWorld(uint64 InWorldId, int32 InPIEInstanceId, uint8 InType, uint8 InNetMode, bool bInIsSimulating);
	...
	virtual TRange<double> GetObjectTraceLifetime(const RewindDebugger::FObjectId& InObjectId) const override;

	/** find the Recording time range for which an object existed */
	virtual TRange<double> GetObjectRecordingLifetime(const RewindDebugger::FObjectId& InObjectId) const override;
...
private:
	...
	TMap<RewindDebugger::FObjectId, int32> ObjectIdToIndexMap;
	...
	TMap<RewindDebugger::FObjectId, uint32> ObjectIdToEventTimelines;
	...
	TMap<RewindDebugger::FObjectId, uint64> ActiveObjectLifetimes;

	// map from object id to index in ObjectRecordingLifetimes, for lookup
	TMap<RewindDebugger::FObjectId, uint64> ActiveObjectRecordingLifetimes;
	...
	TMultiMap<RewindDebugger::FObjectId, RewindDebugger::FObjectId> ObjectHierarchy;
};

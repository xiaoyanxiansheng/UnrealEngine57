...
TSharedPtr<RewindDebugger::FRewindDebuggerTrack> FNotifiesTrackCreator::CreateTrackInternal(const FObjectId& InObjectId) const
{
	return MakeShared<RewindDebugger::FNotifiesTrack>(InObjectId.GetMainId());
}
...
TConstArrayView<TSharedPtr<FRewindDebuggerTrack>> FNotifiesTrack::GetChildrenInternal(TArray<TSharedPtr<FRewindDebuggerTrack>>& OutTracks) const
{
...
bool FNotifiesTrackCreator::HasDebugInfoInternal(const FObjectId& InObjectId) const
{
	...
	{
		AnimationProvider->ReadNotifyTimeline(InObjectId.GetMainId(), [&bHasData](const FAnimationProvider::AnimNotifyTimeline& InTimeline)
		{
			bHasData = true;
		});
	}

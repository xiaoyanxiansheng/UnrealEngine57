...
namespace RewindDebugger
{
	...
	TConstArrayView<TSharedPtr<FRewindDebuggerTrack>> FPropertiesTrack::GetChildrenInternal(TArray<TSharedPtr<FRewindDebuggerTrack>>& OutTracks) const
	{
		...
		for (const TTuple<uint32, TSharedPtr<FPropertyTrack>>& PropertyWatch : ChildrenTracks)
		{
			...
			{
				OutTracks.Push(PropertyWatch.Value);
			}
		}
	...
	TSharedPtr<FRewindDebuggerTrack> FPropertiesTrackCreator::CreateTrackInternal(const FObjectId& InObjectId) const
	{
		TSharedRef<FPropertiesTrack> NewTrack = MakeShared<FPropertiesTrack>(InObjectId.GetMainId());
	...
	bool FPropertiesTrackCreator::HasDebugInfoInternal(const FObjectId& InObjectId) const
	{
		...
		if (const TraceServices::IAnalysisSession* AnalysisSession = IRewindDebugger::Instance()->GetAnalysisSession())
		{
			...
			{
				GameplayProvider->ReadObjectPropertiesTimeline(InObjectId.GetMainId(), [&bHasData](const IGameplayProvider::ObjectPropertiesTimeline & ObjPropsTimeline)
				{
					bHasData = true;
				});
			}

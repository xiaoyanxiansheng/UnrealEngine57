...
void FObjectEventsTrack::BuildDrawState(ITimingEventsTrackDrawStateBuilder& Builder, const ITimingTrackUpdateContext& Context)
{
	...
	if(GameplayProvider)
	{
		...
		// object events
		GameplayProvider->ReadObjectEventsTimeline(GetGameplayTrack().GetAssociatedObjectId(), [&Context, &Builder](const FGameplayProvider::ObjectEventsTimeline& InTimeline)
		{
	...
	TTimingEventSearch<FObjectEventMessage>::Search(
		...
		[this](TTimingEventSearch<FObjectEventMessage>::FContext& InContext)
		{
			...
			if(GameplayProvider)
			{
				...
				GameplayProvider->ReadObjectEventsTimeline(GetGameplayTrack().GetAssociatedObjectId(), [&InContext](const FGameplayProvider::ObjectEventsTimeline& InTimeline)
				{
...
void FObjectEventsTrack::GetVariantsAtFrame(const TraceServices::FFrame& InFrame, TArray<TSharedRef<FVariantTreeNode>>& OutVariants) const
{
	...
	if(GameplayProvider)
	{
		...
		// object events
		GameplayProvider->ReadObjectEventsTimeline(GetGameplayTrack().GetAssociatedObjectId(), [&InFrame, &Header](const FGameplayProvider::ObjectEventsTimeline& InTimeline)
		{

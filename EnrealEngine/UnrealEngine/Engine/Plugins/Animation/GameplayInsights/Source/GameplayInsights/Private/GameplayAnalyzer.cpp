...
void FGameplayAnalyzer::OnAnalysisBegin(const FOnAnalysisContext& Context)
{
	...
	Builder.RouteEvent(RouteId_Type, "Object", "Class");
	Builder.RouteEvent(RouteId_Type, "Object", "Type");
...
bool FGameplayAnalyzer::OnEvent(uint16 RouteId, EStyle Style, const FOnEventContext& Context)
{
	...
	case RouteId_Type:
	{
		FString TypeName, TypePathName;
		if (EventData.GetString("Name", TypeName))
		{
			EventData.GetString("Path", TypePathName);
		}
		else
		{
			const TCHAR* TypeNameAndPathName = reinterpret_cast<const TCHAR*>(EventData.GetAttachment());
			TypeName = TypeNameAndPathName;
			int32 TypeNameStringLength = EventData.GetValue<int32>("TypeNameStringLength");
			TypePathName = TypeNameAndPathName + TypeNameStringLength;
		}

		uint64 Id = EventData.GetValue<uint64>("Id");
		uint64 SuperId = EventData.GetValue<uint64>("SuperId");
		GameplayProvider.AppendClass(Id, SuperId, *TypeName, *TypePathName);
		break;
	}
	case RouteId_Object:
	{
		...
		EObjectInfoFlags Flags = static_cast<EObjectInfoFlags>(EventData.GetValue<uint8>("Flags", 0));
		GameplayProvider.AppendObject(RewindDebugger::FObjectId{Id}, RewindDebugger::FObjectId{OuterId}, ClassId, *ObjectName, *ObjectPathName, Flags);
		break;
	}
	...
	case RouteId_ObjectEvent:
	{
		...
		GameplayProvider.AppendObjectEvent(RewindDebugger::FObjectId{Id}, Context.EventTime.AsSeconds(Cycle), *Event);
		break;
	}
	case RouteId_ObjectLifetimeBegin:
	{
		uint64 Cycle = EventData.GetValue<uint64>("Cycle");
		uint64 Id = EventData.GetValue<uint64>("Id");
		GameplayProvider.AppendObjectLifetimeBegin(RewindDebugger::FObjectId{Id}, Context.EventTime.AsSeconds(Cycle), 0.0);
		break;
	}
	case RouteId_ObjectLifetimeBegin2:
	{
		...
		double RecordingTime = EventData.GetValue<double>("RecordingTime");
		uint64 Id = EventData.GetValue<uint64>("Id");
		GameplayProvider.AppendObjectLifetimeBegin(RewindDebugger::FObjectId{Id}, Context.EventTime.AsSeconds(Cycle), RecordingTime);
		break;
	}
	case RouteId_ObjectLifetimeEnd:
	{
		uint64 Cycle = EventData.GetValue<uint64>("Cycle");
		uint64 Id = EventData.GetValue<uint64>("Id");
		GameplayProvider.AppendObjectLifetimeEnd(RewindDebugger::FObjectId{Id}, Context.EventTime.AsSeconds(Cycle), 0.0);
		break;
	}
	case RouteId_ObjectLifetimeEnd2:
	{
		...
		double RecordingTime = EventData.GetValue<double>("RecordingTime");
		uint64 Id = EventData.GetValue<uint64>("Id");
		GameplayProvider.AppendObjectLifetimeEnd(RewindDebugger::FObjectId{Id}, Context.EventTime.AsSeconds(Cycle), RecordingTime);
		break;
	}

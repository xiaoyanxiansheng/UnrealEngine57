...
TSharedPtr<RewindDebugger::FRewindDebuggerTrack> FAnimGraphSchematicTrackCreator::CreateTrackInternal(const RewindDebugger::FObjectId& InObjectId) const
{
	return MakeShared<FAnimGraphSchematicTrack>(InObjectId.GetMainId());
}
...
bool FAnimGraphSchematicTrackCreator::HasDebugInfoInternal(const RewindDebugger::FObjectId& InObjectId) const
{
	...
	{
		AnimationProvider->ReadAnimGraphTimeline(InObjectId.GetMainId(), [&bHasData](const FAnimationProvider::AnimGraphTimeline& InGraphTimeline)
		{
			bHasData = true;
		});
	}

...
TSharedRef<FObjectEventsTrack> FGameplaySharedData::GetObjectEventsTrackForId(UE::Insights::Timing::ITimingViewSession& InTimingViewSession, const TraceServices::IAnalysisSession& InAnalysisSession, const FObjectInfo& InObjectInfo)
{
	...
	TSharedPtr<FObjectEventsTrack> LeafObjectEventsTrack = ObjectTracks.FindRef(ObjectId);
	if(!LeafObjectEventsTrack.IsValid())
	{
		LeafObjectEventsTrack = MakeShared<FObjectEventsTrack>(*this, ObjectId, InObjectInfo.Name);
		LeafObjectEventsTrack->SetVisibilityFlag(bObjectTracksEnabled);
		ObjectTracks.Add(ObjectId, LeafObjectEventsTrack);
	...
	if(InObjectInfo.GetOuterId().IsSet())
	{
		...
		uint64 OuterId = InObjectInfo.GetOuterUObjectId();
		while(const FObjectInfo* OuterInfo = GameplayProvider->FindObjectInfo(OuterId))
		{
			...
			OuterId = OuterInfo->GetOuterUObjectId();
		}
	}
		...
		if(GameplayProvider->HasAnyData())
		{
			...
			GameplayProvider->EnumerateObjects([this, &InTimingViewSession, &InAnalysisSession, &GameplayProvider](const FObjectInfo& InObjectInfo)
			{
				if(bObjectTracksEnabled || bObjectPropertyTracksEnabled)
				{
					...
					GameplayProvider->ReadObjectEventsTimeline(InObjectInfo.GetId(), [this, &InTimingViewSession, &InAnalysisSession, &InObjectInfo, &GameplayProvider, &ObjectEventsTrack](const IGameplayProvider::ObjectEventsTimeline& InTimeline)
					{
					...
					GameplayProvider->ReadObjectPropertiesTimeline(InObjectInfo.GetUObjectId(), [this, &InObjectInfo, &ObjectEventsTrack, &InTimingViewSession, &InAnalysisSession, &GameplayProvider](const IGameplayProvider::ObjectPropertiesTimeline& InTimeline)
					{
						...
						TSharedPtr<FObjectPropertiesTrack> ExistingObjectPropertiesTrack = StaticCastSharedPtr<FObjectPropertiesTrack>(ObjectEventsTrack->GetGameplayTrack().FindChildTrack(InObjectInfo.GetUObjectId(), FindObjectProperties));
						if(!ExistingObjectPropertiesTrack.IsValid())
						{
							TSharedPtr<FObjectPropertiesTrack> ObjectPropertiesTrack = MakeShared<FObjectPropertiesTrack>(*this, InObjectInfo.GetUObjectId(), InObjectInfo.Name);

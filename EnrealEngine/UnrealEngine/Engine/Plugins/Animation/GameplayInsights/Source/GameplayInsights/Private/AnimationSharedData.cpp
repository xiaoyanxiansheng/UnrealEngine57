	...
	if(AnimationProvider && GameplayProvider)
	{
		...
		if(AnimationProvider->HasAnyData() && AreAnyAnimationTracksEnabled())
		{
			...
			GameplayProvider->EnumerateObjects([this, &InTimingViewSession, &InAnalysisSession, &AnimationProvider, &GameplayProvider](const FObjectInfo& InObjectInfo)
			{
				...
				const uint64 ObjectId = InObjectInfo.GetUObjectId();
				AnimationProvider->ReadSkeletalMeshPoseTimeline(ObjectId, [this, ObjectId, &InObjectInfo, &ObjectEventsTrack, &InTimingViewSession, &InAnalysisSession](const IAnimationProvider::SkeletalMeshPoseTimeline& InTimeline, bool bInHasCurves)
				{
					...
					TSharedPtr<FSkeletalMeshPoseTrack> ExistingSkeletalMeshPoseTrack = StaticCastSharedPtr<FSkeletalMeshPoseTrack>(ObjectEventsTrack->GetGameplayTrack().FindChildTrack(ObjectId, FindSkeletalMeshPoseTrack));
					if(!ExistingSkeletalMeshPoseTrack.IsValid())
					{
						TSharedPtr<FSkeletalMeshPoseTrack> SkeletalMeshPoseTrack = MakeShared<FSkeletalMeshPoseTrack>(*this, ObjectId, InObjectInfo.Name);
					...
					if(bInHasCurves)
					{
						...
						TSharedPtr<FSkeletalMeshCurvesTrack> ExistingSkeletalMeshCurvesTrack = StaticCastSharedPtr<FSkeletalMeshCurvesTrack>(ObjectEventsTrack->GetGameplayTrack().FindChildTrack(ObjectId, FindSkeletalMeshCurvesTrack));
						if(!ExistingSkeletalMeshCurvesTrack.IsValid())
						{
							TSharedPtr<FSkeletalMeshCurvesTrack> SkeletalMeshCurvesTrack = MakeShared<FSkeletalMeshCurvesTrack>(*this, ObjectId, InObjectInfo.Name);
				...
				AnimationProvider->ReadTickRecordTimeline(ObjectId, [this, ObjectId, &InObjectInfo, &ObjectEventsTrack, &InTimingViewSession, &GameplayProvider, &InAnalysisSession](const IAnimationProvider::TickRecordTimeline& InTimeline)
				{
					if(!ObjectEventsTrack.IsValid())
					{
						ObjectEventsTrack = GameplaySharedData.GetObjectEventsTrackForId(InTimingViewSession, InAnalysisSession, InObjectInfo);
					...
					TSharedPtr<FAnimationTickRecordsTrack> ExistingAnimationTickRecordsTrack = StaticCastSharedPtr<FAnimationTickRecordsTrack>(ObjectEventsTrack->GetGameplayTrack().FindChildTrack(ObjectId, FindTickRecordTrackWithAssetId));
					if(!ExistingAnimationTickRecordsTrack.IsValid())
					{
						TSharedPtr<FAnimationTickRecordsTrack> AnimationTickRecordsTrack = MakeShared<FAnimationTickRecordsTrack>(*this, ObjectId, InObjectInfo.Name);
				...
				AnimationProvider->ReadAnimGraphTimeline(ObjectId, [this, ObjectId, &InObjectInfo, &ObjectEventsTrack, &InTimingViewSession, &InAnalysisSession](const IAnimationProvider::AnimGraphTimeline& InTimeline)
				{
					if(!ObjectEventsTrack.IsValid())
					{
						ObjectEventsTrack = GameplaySharedData.GetObjectEventsTrackForId(InTimingViewSession, InAnalysisSession, InObjectInfo);
					...
					TSharedPtr<FAnimNodesTrack> ExistingAnimNodesTrack = StaticCastSharedPtr<FAnimNodesTrack>(ObjectEventsTrack->GetGameplayTrack().FindChildTrack(ObjectId, FindAnimNodesTrack));
					if(!ExistingAnimNodesTrack.IsValid())
					{
						TSharedPtr<FAnimNodesTrack> AnimNodesTrack = MakeShared<FAnimNodesTrack>(*this, ObjectId, InObjectInfo.Name);
				...
				auto ProcessNotifyTimeline = [this, ObjectId, &InObjectInfo, &ObjectEventsTrack, &InTimingViewSession, &InAnalysisSession]()
				{
					if(!ObjectEventsTrack.IsValid())
					{
						ObjectEventsTrack = GameplaySharedData.GetObjectEventsTrackForId(InTimingViewSession, InAnalysisSession, InObjectInfo);
					...
					TSharedPtr<FAnimNotifiesTrack> ExistingAnimNotifyTrack = StaticCastSharedPtr<FAnimNotifiesTrack>(ObjectEventsTrack->GetGameplayTrack().FindChildTrack(ObjectId, FindAnimNotifyTrack));
					if(!ExistingAnimNotifyTrack.IsValid())
					{
						TSharedPtr<FAnimNotifiesTrack> AnimNotifyTrack = MakeShared<FAnimNotifiesTrack>(*this, ObjectId, InObjectInfo.Name);
				...
				AnimationProvider->ReadNotifyTimeline(ObjectId, [&ProcessNotifyTimeline](const IAnimationProvider::AnimNotifyTimeline& InTimeline)
				{
					ProcessNotifyTimeline();
				});

				AnimationProvider->EnumerateNotifyStateTimelines(ObjectId, [&ProcessNotifyTimeline](uint32 InNotifyNameId, const IAnimationProvider::AnimNotifyTimeline& InTimeline)
				{
					ProcessNotifyTimeline();
				});

				AnimationProvider->ReadMontageTimeline(ObjectId, [this, ObjectId, &InObjectInfo, &ObjectEventsTrack, &InTimingViewSession, &InAnalysisSession](const IAnimationProvider::AnimMontageTimeline& InTimeline)
				{
					if(!ObjectEventsTrack.IsValid())
					{
						ObjectEventsTrack = GameplaySharedData.GetObjectEventsTrackForId(InTimingViewSession, InAnalysisSession, InObjectInfo);
					...
					TSharedPtr<FMontageTrack> ExistingMontageTrack = StaticCastSharedPtr<FMontageTrack>(ObjectEventsTrack->GetGameplayTrack().FindChildTrack(ObjectId, FindMontageTrack));
					if(!ExistingMontageTrack.IsValid())
					{
						TSharedPtr<FMontageTrack> MontageTrack = MakeShared<FMontageTrack>(*this, ObjectId, InObjectInfo.Name);
...
void FAnimationSharedData::ExtendFilterMenu(FMenuBuilder& InMenuBuilder)
{
	InMenuBuilder.BeginSection("AnimationTracks", LOCTEXT("AnimationHeader", "Animation"));
	{
		...
		InMenuBuilder.AddMenuEntry(
			...
			LOCTEXT("ShowTransportControls_Tooltip", "Show the transport controls"),
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateRaw(this, &FAnimationSharedData::ToggleTransportControls),
				FCanExecuteAction(),
				FIsActionChecked::CreateRaw(this, &FAnimationSharedData::ShouldShowTransportControls)),
			NAME_None,
			EUserInterfaceActionType::ToggleButton
		);
	}
...
void FAnimationSharedData::ToggleTransportControls()
{
	bShowTransportControls = !bShowTransportControls;
}

...
TConstArrayView<TSharedPtr<FRewindDebuggerTrack>> FPoseWatchTrack::GetChildrenInternal(TArray<TSharedPtr<FRewindDebuggerTrack>>& OutTracks) const
{
...
TSharedPtr<RewindDebugger::FRewindDebuggerTrack> FPoseWatchesTrackCreator::CreateTrackInternal(const FObjectId& InObjectId) const
{
	return MakeShared<RewindDebugger::FPoseWatchesTrack>(InObjectId.GetMainId());
}
...
TConstArrayView<TSharedPtr<FRewindDebuggerTrack>> FPoseWatchesTrack::GetChildrenInternal(TArray<TSharedPtr<FRewindDebuggerTrack>>& OutTracks) const
{
	return MakeConstArrayView<TSharedPtr<FRewindDebuggerTrack>>(reinterpret_cast<const TSharedPtr<FRewindDebuggerTrack>*>(Children.GetData()), Children.Num());
}

bool FPoseWatchesTrackCreator::HasDebugInfoInternal(const FObjectId& InObjectId) const
{
	...
	{
		AnimationProvider->ReadPoseWatchTimeline(InObjectId.GetMainId(), [&bHasData](const FAnimationProvider::PoseWatchTimeline& InTimeline)
		{
			bHasData = true;
		});
	}

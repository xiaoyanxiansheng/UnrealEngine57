...
DECLARE_LOG_CATEGORY_CLASS(LogSequencer, Log, All);

namespace SequencerToolsPrivate
{
	static TArray<TStrongObjectPtr<const UObject>> DelegateObjectsTempStrongRefs;

	// Will check Condition every Interval seconds (or every frame, if Interval == 0) and execute Callback when true.
	void WaitUntil(const FCheckCanCompleteDelegate Condition, const TFunction<void()> Callback, const UWorld* World, const float Interval = 0.1f)
	{
		if (!Condition.IsBound() || !Callback.IsSet() || !World)
		{
			UE_LOG(LogSequencer, Error, TEXT("WaitUntil() - One of the parameters is invalid."));
			return;
		}

		// Guard the condition delegate from GC.
		DelegateObjectsTempStrongRefs.AddUnique(TStrongObjectPtr<const UObject>(Condition.GetUObject()));

		if (Condition.Execute())
		{
			Callback();
			DelegateObjectsTempStrongRefs.Remove(TStrongObjectPtr<const UObject>(Condition.GetUObject()));
			return;
		}

		FTimerDelegate WaitDelegate = FTimerDelegate::CreateLambda([Condition, Callback, World, Interval]() { SequencerToolsPrivate::WaitUntil(Condition, Callback, World, Interval); });
		FTimerManager& TimerManager = World->GetTimerManager();

		if (Interval == 0.f)
		{
			TimerManager.SetTimerForNextTick(WaitDelegate);
		}
		else
		{
			FTimerHandle Handle; // Unnecessary to keep, but required by SetTimer().
			TimerManager.SetTimer(Handle, WaitDelegate, Interval, false);
		}
	}
}
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	if (LevelSequenceCapture)
	{
		...
		{
			// UE_LOG(LogSequencer, Warning, TEXT("No Level Sequence Asset specified in UAutomatedLevelSequenceCapture."));
...
bool ExportFBXInternal(const FSequencerExportFBXParams& InParams, UMovieSceneSequencePlayer* Player)
{
	...
	{
		FAllSpawnableRestoreState SpawnableRestoreState(Player->GetSharedPlaybackState().ToSharedPtr(), MovieScene);
...
{
	FCheckCanCompleteDelegate EmptyDelegate;
	return ExportAnimSequenceWaitForDelegate(World, Sequence, AnimSequence, ExportOptions, Binding, bCreateLink, EmptyDelegate);
}

bool USequencerToolsFunctionLibrary::ExportAnimSequenceWaitForDelegate(UWorld* World, ULevelSequence* Sequence, UAnimSequence* AnimSequence, UAnimSeqExportOption* ExportOptions, const FMovieSceneBindingProxy& Binding, bool bCreateLink, const FCheckCanCompleteDelegate Delegate)
{
	UMovieScene* MovieScene = Sequence->GetMovieScene();
	...
	auto CompleteExport = [Player, Binding, AnimSequence, RootToLocalTransform, Sequence, ExportOptions, World, OutActor, bCreateLink]() 
	{
		bool bResult = false;

		if (Player && AnimSequence && Sequence && ExportOptions && World)
		{
			...
			{
				UE_LOG(LogSequencer, Warning, TEXT("USequencerToolsFunctionLibrary::ExportAnimSequence: No skel mesh found in Sequencer"));
				bResult = false;
			}
		...
		{
			UE_LOG(LogSequencer, Warning, TEXT("USequencerToolsFunctionLibrary::ExportAnimSequence: One or more of the completion lambda's captured parameters are invalid."));
			bResult = false;
		}
	...
	if (Delegate.IsBound())
	{
		SequencerToolsPrivate::WaitUntil(Delegate, CompleteExport, World);
		return true;
	}
	else
	{
	...
	const FString& ImportFilename)
{
	...
	{
		if (MovieScene->GetObjectDisplayName(Binding.GetObjectGuid()).ToString() == ControlRigTrackName)
		{
	...
	const UMovieSceneUserExportFBXControlRigSettings* ExportFBXControlRigSettings)
{
	...
	for (const FMovieSceneBinding& Binding : Bindings)
	{
		if (MovieScene->GetObjectDisplayName(Binding.GetObjectGuid()).ToString() != ActorWithControlRigTrack)
		{
			continue;
		}

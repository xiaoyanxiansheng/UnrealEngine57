...
#include "KeysAndChannels/MovieSceneScriptingText.h"
...
TArray<UMovieSceneScriptingChannel*> UMovieSceneSectionExtensions::GetAllChannels(UMovieSceneSection* Section)
{
	...
	// types: Bool, Byte, Float, Double, Integer, String, Text, Event, ActorReference
	...
	GetScriptingChannelsForChannel<FMovieSceneTextChannel, UMovieSceneScriptingTextChannel>(ChannelProxy, Sequence, Section, Channels);
...
TArray<UMovieSceneScriptingChannel*> UMovieSceneSectionExtensions::GetChannelsByType(UMovieSceneSection* Section, TSubclassOf<UMovieSceneScriptingChannel> ChannelType)
{
	...
	else if (ChannelType == UMovieSceneScriptingTextChannel::StaticClass())				{ GetScriptingChannelsForChannel<FMovieSceneTextChannel, UMovieSceneScriptingTextChannel>(ChannelProxy, Sequence, Section, Channels); }	
...
UMovieSceneScriptingChannel* UMovieSceneSectionExtensions::GetChannel(UMovieSceneSection* Section, const FName& ChannelName)
{
	...
	else if (ChanneHandle.GetChannelTypeName() == FMovieSceneTextChannel::StaticStruct()->GetFName())
	{
		return GetScriptingChannelForChannel<FMovieSceneTextChannel, UMovieSceneScriptingTextChannel>(MovieSceneChannel, ChannelProxy, ChanneHandle, Sequence, Section, ChannelName);
	}

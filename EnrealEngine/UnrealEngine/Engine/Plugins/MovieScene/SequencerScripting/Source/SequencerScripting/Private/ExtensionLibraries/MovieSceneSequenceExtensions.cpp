...
UMovieSceneClock* UMovieSceneSequenceExtensions::GetCustomClock(UMovieSceneSequence* InSequence)
{
	if (!InSequence)
	{
		FFrame::KismetExecutionMessage(TEXT("Cannot call GetCustomClock on a null sequence"), ELogVerbosity::Error);
		return nullptr;
	}

	UMovieScene* MovieScene = GetMovieScene(InSequence);
	if (MovieScene)
	{
		return MovieScene->GetCustomClock();
	}
...
FMovieSceneBindingProxy UMovieSceneSequenceExtensions::FindBindingByName(UMovieSceneSequence* Sequence, FString Name)
{
	...
	if (MovieScene)
	{
		for (int32 Index = 0; Index < MovieScene->GetPossessableCount(); ++Index)
		{
			...
			if (Possessable.GetName() == Name)
			{
				return FMovieSceneBindingProxy(Possessable.GetGuid(), Sequence);
			}
		}
		...
		for (int32 Index = 0; Index < MovieScene->GetSpawnableCount(); ++Index)
		{
			...
			{
				return FMovieSceneBindingProxy(Spawnable.GetGuid(), Sequence);
			}
		}
	}
	return FMovieSceneBindingProxy();
...
TArray<FMovieSceneBindingProxy> UMovieSceneSequenceExtensions::GetBindings(UMovieSceneSequence* Sequence)
{
	...
	const UMovieScene* MovieScene = GetMovieScene(Sequence);
	if (MovieScene)
	{

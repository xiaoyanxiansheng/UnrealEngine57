...
protected:
	...
	/** Whether translucent objects should be included in the IDs. */
	bool bIncludeTranslucentObjects = false;

	/**
	 * This pass will modify Editor Settings to activate/deactivate translucent object selection (which bIncludeTranslucentObjects relies on). This
	 * is the cached value of the property in Editor Settings (bAllowSelectTranslucent) before modifying it so it can be restored post-render.
	 */
	bool bPrevAllowSelectTranslucent = false;

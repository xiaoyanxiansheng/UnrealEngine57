...
namespace UE::LevelSequenceEditorModule::Private
{
	/** Utility function to make a Menu entry with a custom Slate widget. */
	FNewToolMenuCustomWidget MakeCustomMenuWidgetDelegate(
		const TAttribute<FText>& Label, const TAttribute<FSlateIcon>& Icon, TSharedRef<SWidget> CustomWidget, EHorizontalAlignment CustomWidgetHAlign = HAlign_Fill)
	{
		return FNewToolMenuCustomWidget::CreateLambda([Label, Icon, CustomWidget, CustomWidgetHAlign](const FToolMenuContext& InContext, const FToolMenuCustomWidgetContext& WidgetContext)
			{
...
protected:
	...
	void RegisterMenuExtensions()
	{
		...
		// Create and register context menu extensions
		if (UToolMenu* Menu = UE::ContentBrowser::ExtendToolMenu_AssetContextMenu(ULevelSequence::StaticClass()))
		{
			...
			const FName FavoriteRatingMenuEntry = "LevelSequenceFavoriteRating";
			Section.AddDynamicEntry(FavoriteRatingMenuEntry, FNewToolMenuSectionDelegate::CreateLambda([this](FToolMenuSection& InSection)
			{
				if (UContentBrowserAssetContextMenuContext* Context = InSection.FindContext<UContentBrowserAssetContextMenuContext>())
				{
					// Ensure only a single selected asset.
					const TArray<FAssetData>& SelectedAssets = Context->SelectedAssets;
					if (SelectedAssets.Num() == 1)
					{
						const FAssetData& LevelSequenceData = SelectedAssets[0];
						ULevelSequence* LevelSequence = Cast<ULevelSequence>(LevelSequenceData.GetAsset());

						FText SetFavoriteRatingLabel = LOCTEXT("SetFavoriteRating", "Set Rating");
						FSlateIcon SetFavoriteRatingIcon(FAppStyle::GetAppStyleSetName(), "Icons.Star");

						FToolMenuEntry RatingMenuEntry = FToolMenuEntry::InitMenuEntry(
							"SetFavoriteRating",
							SetFavoriteRatingLabel,
							LOCTEXT("SetFavoriteRatingTooltip", "Set Rating"),
							SetFavoriteRatingIcon,
							FUIAction()
						);

						TSharedRef<SLevelSequenceFavoriteRating> RatingWidget = SNew(SLevelSequenceFavoriteRating)
							.LevelSequence(LevelSequence);

						RatingMenuEntry.MakeCustomWidget = UE::LevelSequenceEditorModule::Private::MakeCustomMenuWidgetDelegate(
							SetFavoriteRatingLabel,
							SetFavoriteRatingIcon,
							RatingWidget,
							HAlign_Right
						);
						InSection.AddEntry(RatingMenuEntry);
					}
				}
			}));
		}

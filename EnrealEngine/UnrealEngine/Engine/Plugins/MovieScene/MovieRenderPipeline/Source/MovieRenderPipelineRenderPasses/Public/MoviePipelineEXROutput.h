...
public:
	...
	/**
	 * Compression method used for the resulting EXR files. Applies to all layers, but per-layer compression can be specified in
	 * `CompressionByLayer` if needed.
	 */
	EEXRCompressionFormat Compression;

	/**
	 * Controls the compression per layer. Only used if `bMultiPart` is true. Takes precedence over `Compression`, but is ignored if this array is
	 * either empty, or its size does not match the number of layers given in `Layers`.
	 */
	TArray<EEXRCompressionFormat> CompressionByLayer;
...
private:
	...
	bool GenerateFilePartsForResolution(const FIntPoint& Resolution, const TArray<FImagePixelData*>& InLayers, const EEXRCompressionFormat CompressionType, const FString& InPartName, FEXRPart& OutPart, TArray<TUniquePtr<FImagePixelData>>& OutQuantizedLayers);
...
#if WITH_UNREALEXR
	MOVIERENDERPIPELINERENDERPASSES_API virtual void OnReceiveImageDataImpl(FMoviePipelineMergerOutputFrame* InMergedOutputFrame) override;
#endif // WITH_UNREALEXR

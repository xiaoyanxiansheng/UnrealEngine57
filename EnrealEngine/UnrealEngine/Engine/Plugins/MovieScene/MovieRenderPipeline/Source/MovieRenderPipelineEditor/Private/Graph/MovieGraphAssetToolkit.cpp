...
#include "Customizations/Graph/MovieGraphMultiLayerEXRNodeCustomization.h"
#include "Customizations/Graph/MovieGraphFormatTokenCustomization.h"
#include "Customizations/Graph/MovieGraphMaterialParameterCollectionModifierNodeCustomization.h"
...
TSharedRef<SDockTab> FMovieGraphAssetToolkit::SpawnTab_RenderGraphDetails(const FSpawnTabArgs& Args)
{
	...
	SelectedGraphObjectsDetailsWidget->RegisterInstancedCustomPropertyLayout(
		UMovieGraphMaterialParameterCollectionModifierNode::StaticClass(),
		FOnGetDetailCustomizationInstance::CreateStatic(&FMovieGraphMaterialParameterCollectionModifierNodeCustomization::MakeInstance));
	...
	SelectedGraphObjectsDetailsWidget->RegisterInstancedCustomPropertyLayout(
		UMovieGraphImageSequenceOutputNode_EXR::StaticClass(),
		FOnGetDetailCustomizationInstance::CreateStatic(&FMovieGraphMultiLayerEXRNodeCustomization::MakeInstance));
		...
		if (!Section.FindEntry("View"))
		{
			Section.AddSubMenu(
				...
				FNewToolMenuDelegate::CreateLambda([](UToolMenu* InMenu)
				{
					...
					ZoomSection.AddMenuEntry(FMoviePipelineCommands::Get().ZoomToWindow);
					ZoomSection.AddMenuEntry(FMoviePipelineCommands::Get().ZoomToSelection);
				})

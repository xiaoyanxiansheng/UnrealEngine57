...
#include "Customizations/PostProcessPassCustomization.h"
...
void SMoviePipelineConfigEditor::UpdateDetails()
{
	...
	for (auto& Pair : ClassToSources)
	{
		...
		else
		{
			...
			Details->RegisterInstancedCustomPropertyTypeLayout(FMoviePipelinePostProcessPass::StaticStruct()->GetFName(), FOnGetPropertyTypeCustomizationInstance::CreateStatic(FPostProcessPassCustomization_MRQ::MakeInstance));

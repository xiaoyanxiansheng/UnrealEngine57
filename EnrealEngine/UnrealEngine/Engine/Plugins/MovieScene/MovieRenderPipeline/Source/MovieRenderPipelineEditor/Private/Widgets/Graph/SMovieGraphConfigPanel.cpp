...
bool SMoviePipelineGraphPanel::CanDeleteSelectedNodes() const
{
	...
	for (UObject* Object : GraphEditorWidget->GetSelectedNodes())
	{
		...
		// If the editor node doesn't derive from UMoviePipelineEdGraphNodeBase, it's a node from outside MRG (eg, a comment node); these are
		// always deletable. They will not have a runtime representation. Ideally we could check to see if this is in CurrentGraph->GetEditorOnlyNodes(),
		// but that array is only updated after a save is performed.
		if (!GraphNode)
		{
			continue;
		}

		// Some nodes in the graph are explicitly marked as non-deletable (eg, inputs/outputs).
		...
		// Editor-only nodes do not have an associated runtime node, but those are accounted for earlier.

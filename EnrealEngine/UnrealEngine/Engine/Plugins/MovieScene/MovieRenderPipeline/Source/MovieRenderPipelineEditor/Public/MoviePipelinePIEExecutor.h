...
public:
	UE_API UMoviePipelinePIEExecutor();
...
public:
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Movie Render Pipeline")
	UE_API void SetAllowUsingUnsavedLevels(const bool bShouldAllow);
...
protected:
	...
	*/
	UE_API void OnIndividualJobFinishedImpl(FMoviePipelineOutputData InOutputData);
	/**
	* This is called after PIE starts up but before UMoviePIpeline::Initialize() is called.
	*/
	UE_API void OnIndividualJobStartedImpl(UMoviePipelineExecutorJob* InJob);
private:
	/** Called when PIE finishes booting up and it is safe for us to spawn an object into that world. */
	UE_API void OnPIEStartupFinished(bool);
	...
	UE_API void OnPIEEnded(bool);
	/** Called when the instance of the pipeline in the PIE world has finished. */
	UE_API void OnPIEMoviePipelineFinished(FMoviePipelineOutputData InOutputData);
	...
	UE_API void DelayedFinishNotification();
private:

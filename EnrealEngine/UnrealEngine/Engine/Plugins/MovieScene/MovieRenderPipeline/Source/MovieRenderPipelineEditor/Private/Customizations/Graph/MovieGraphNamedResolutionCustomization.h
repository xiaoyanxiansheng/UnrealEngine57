...
class FMovieGraphNamedResolutionCustomization : public IPropertyTypeCustomization
{
public:
	...
	UE_API virtual ~FMovieGraphNamedResolutionCustomization() override;
...
protected:
	...
	*/
	UE_API void UpdateComboBoxOptions();
	...
	*/
	UE_API FText GetWidgetTextForOption(const FName InOption, const bool bSuppressResolution) const;
	/** The above, but for the currently selected (ie: "root" of combo box). */
	UE_API FText GetWidgetTextForCurrent() const;
	/** Gets the description property for the specified option */
	UE_API FText GetWidgetTooltipTextForOption(const FName InOption) const;
	/** The above, but for the currently selected (ie: "root" of combo box). */
	UE_API FText GetWidgetTooltipTextForCurrent() const;
	...
	UE_API TSharedRef<SWidget> MakeWidgetForOptionInDropdown(FName InOption) const;
	/** Called to generate the widget for the "root" of the combo box that is always visible. */
	UE_API TSharedRef<SWidget> MakeWidgetForSelectedItem() const;

	/** Create the actual ComboBox Widget */
	UE_API TSharedRef<SWidget> CreateComboBoxWidget();

	/** Binds to the Project Settings to know when a user has added or removed a preset. */
	UE_API void BindToOnProjectSettingsModified();

	/** Get the icon to use for the Aspect Ratio Locked widget, could be a locked or unlocked icon. */
	UE_API const FSlateBrush* GetAspectRatioLockBrush() const;

	/** Get the current tooltip for the aspect ratio lock */
	UE_API FText GetAspectRatioLockTooltipText() const;
	
	//~ Begin IPropertyTypeCustomization interface
	UE_API virtual void CustomizeHeader(
	...
	UE_API virtual void CustomizeChildren(
		TSharedRef<IPropertyHandle> InStructPropertyHandle,
	...
	*/
	...
	UE_API void OnProjectSettingsChanged(UObject*, FPropertyChangedEvent& PropertyChangedEvent);

	/** Called when the user selects a new preset from the combo box dropdown. */
	UE_API void OnComboBoxSelectionChanged(const FName NewValue, ESelectInfo::Type InType);

...
public:
	 ...
	 */
	UFUNCTION(BlueprintPure, Category = "Movie Render Pipeline")
	UE_API bool LoadQueue(UMoviePipelineQueue* QueueToLoad, const bool bPromptOnReplacingDirtyQueue = true);
	 ...
	 */
	UFUNCTION(BlueprintPure, Category = "Movie Render Pipeline")
	UE_API bool IsQueueDirty() const;
	...
	*/
	...
	UE_API UMoviePipelineExecutorBase* RenderQueueWithExecutor(TSubclassOf<UMoviePipelineExecutorBase> InExecutorType);

	/** Like RenderQueueWithExecutor(), but a specific queue instance is used rather than the current queue. */
	UFUNCTION(BlueprintCallable, meta = (DeterminesOutputType = "InExecutorType"), Category = "Movie Render Pipeline|Rendering")
	UE_API UMoviePipelineExecutorBase* RenderQueueInstanceWithExecutor(UMoviePipelineQueue* InQueue, TSubclassOf<UMoviePipelineExecutorBase> InExecutorType);
	...
	*/
	...
	UE_API void RenderQueueWithExecutorInstance(UMoviePipelineExecutorBase* InExecutor);

	/** Like RenderQueueWithExecutorInstance(), but a specific queue instance is used rather than the current queue. */
	UFUNCTION(BlueprintCallable, Category = "Movie Render Pipeline|Rendering")
	UE_API void RenderQueueInstanceWithExecutorInstance(UMoviePipelineQueue* InQueue, UMoviePipelineExecutorBase* InExecutor);
...
private:
	...
	UE_API void OnExecutorFinished(UMoviePipelineExecutorBase* InPipelineExecutor, bool bSuccess);
	UE_API void OnSequencerContextBinding(bool& bAllowBinding);

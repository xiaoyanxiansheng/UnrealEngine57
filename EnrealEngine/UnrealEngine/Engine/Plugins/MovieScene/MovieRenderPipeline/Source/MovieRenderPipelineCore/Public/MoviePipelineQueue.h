...
public:
	...
	*/
	...
	UE_API void SetStatusMessage(const FString& InStatus);
	...
	*/
	...
	UE_API FString GetStatusMessage() const;
	...
	*/
	UFUNCTION(BlueprintCallable, BlueprintNativeEvent, Category = "Movie Render Pipeline")
	UE_API void SetStatusProgress(const float InProgress);
	...
	*/
	UFUNCTION(BlueprintPure, BlueprintNativeEvent, Category = "Movie Render Pipeline")
	UE_API float GetStatusProgress() const;
	...
	UE_API UMoviePipelineShotConfig* AllocateNewShotOverrideConfig(TSubclassOf<UMoviePipelineShotConfig> InConfigType);

	UFUNCTION(BlueprintCallable, Category = "Movie Render Pipeline")
	UE_API void SetShotOverrideConfiguration(UMoviePipelineShotConfig* InPreset);

	UFUNCTION(BlueprintCallable, Category = "Movie Render Pipeline")
	UE_API void SetShotOverridePresetOrigin(UMoviePipelineShotConfig* InPreset);
	 ...
	 */
	UFUNCTION(BlueprintPure, Category = "Movie Render Pipeline")
	UE_API bool IsUsingGraphConfiguration() const;
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Movie Render Pipeline")
	UE_API void SetGraphPreset(const UMovieGraphConfig* InGraphPreset, const bool bUpdateVariableAssignments = true);
	...
	*/
	...
	UE_API UMovieJobVariableAssignmentContainer* GetOrCreateJobVariableAssignmentsForGraph(const UMovieGraphConfig* InGraph, const bool bIsForPrimaryOverrides = false);
	 ...
	 */
	UE_API TArray<TObjectPtr<UMovieJobVariableAssignmentContainer>>& GetGraphVariableAssignments(const bool bUpdateAssignments = true);
	 ...
	 * there is very specific behavior that is needed).
	 */
	UE_API TArray<TObjectPtr<UMovieJobVariableAssignmentContainer>>& GetPrimaryGraphVariableAssignments(const bool bUpdateAssignments = true);
	...
	UE_API void RefreshAllVariableAssignments();
...
protected:
	...
	UE_API void OnGraphPreSave(UObject* InObject, FObjectPreSaveContext InObjectPreSaveContext);
...
public:	
	...
	*/
	UFUNCTION(BlueprintCallable, BlueprintNativeEvent, Category = "Movie Render Pipeline")
	UE_API void SetConsumed(const bool bInConsumed);
	...
	*/
	UFUNCTION(BlueprintCallable, BlueprintNativeEvent, Category = "Movie Render Pipeline")
	UE_API void SetIsEnabled(const bool bInEnabled);
	...
	*/
	UFUNCTION(BlueprintCallable, BlueprintNativeEvent, Category = "Movie Render Pipeline")
	UE_API void OnDuplicated();

	UFUNCTION(BlueprintCallable, Category = "Movie Render Pipeline")
	UE_API void SetPresetOrigin(UMoviePipelinePrimaryConfig* InPreset);
	...
	UFUNCTION(BlueprintCallable, Category = "Movie Render Pipeline")
	UE_API void SetConfiguration(UMoviePipelinePrimaryConfig* InPreset);
	...
	UE_API void SetSequence(FSoftObjectPath InSequence);
	...
	static UE_API FOnSequenceSet& OnSequenceSetEvent();
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "Movie Render Pipeline", DisplayName = "Get or Create Variable Overrides", meta=(ScriptName ="GetOrCreateVariableOverrides"))
	UE_API UMovieJobVariableAssignmentContainer* GetOrCreateJobVariableAssignmentsForGraph(const UMovieGraphConfig* InGraph);

public:
	// UObject Interface
#if WITH_EDITOR
	UE_API void PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent);
#endif
	UE_API virtual void PreSave(FObjectPreSaveContext ObjectSaveContext) override;
	UE_API virtual void PostLoad() override;
	UE_API virtual void BeginDestroy() override;
...
protected:
	...
	UE_API virtual void OnDuplicated_Implementation();
...
public:
	...
	*/
	...
	UE_API UMoviePipelineExecutorJob* AllocateNewJob(TSubclassOf<UMoviePipelineExecutorJob> InJobType);
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "Movie Render Pipeline|Queue")
	UE_API UMoviePipelineExecutorJob* DuplicateJob(UMoviePipelineExecutorJob* InJob);
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "Movie Render Pipeline|Queue")
	UE_API UMoviePipelineQueue* CopyFrom(UMoviePipelineQueue* InQueue);
	
	/* Set the index of the given job */
	UFUNCTION(BlueprintCallable, Category = "Movie Render Pipeline|Queue")
	UE_API void SetJobIndex(UMoviePipelineExecutorJob* InJob, int32 Index);
...
private:
#if WITH_EDITOR
	UE_API void OnAnyObjectModified(UObject* InModifiedObject);
#endif

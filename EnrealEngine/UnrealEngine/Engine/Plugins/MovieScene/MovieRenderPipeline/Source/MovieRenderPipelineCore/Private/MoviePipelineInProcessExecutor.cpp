...
void UMoviePipelineInProcessExecutor::OnMapLoadFinished(UWorld* NewWorld)
{
	...
	// Give the pipeline a chance to shut down properly if the render is interrupted (Alt+F4, closing the preview window, etc). When the render is
	// stopped like this, the engine's shutdown process may kill the player before more common delegates like FCoreDelegates::OnEnginePreExit are
	// called. ApplicationWillTerminateDelegate will be called immediately after an engine shutdown request is received though, and lets the pipeline
	// shut down before trying to access the (now non-existent) player.
	FCoreDelegates::GetApplicationWillTerminateDelegate().AddUObject(this, &UMoviePipelineInProcessExecutor::OnApplicationQuit);
...
void UMoviePipelineInProcessExecutor::OnTick()
{
	...
	if (UGameEngine* GameEngine = Cast<UGameEngine>(GEngine))
	{
		...
		{
			GameViewportWindow->SetAcceptsInput(false);
		}
	}
}
...
void UMoviePipelineInProcessExecutor::RestoreState()
{
	...
	if (UGameEngine* GameEngine = Cast<UGameEngine>(GEngine))
	{
		if (TSharedPtr<SWindow> GameViewportWindow = GameEngine->GameViewportWindow.Pin())
		{
			GameViewportWindow->SetAcceptsInput(true);
		}
	}
}

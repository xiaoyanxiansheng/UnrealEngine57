...
public:
	...
	UE_API UMovieGraphConfig* GetOwningGraph() const;
	...
	UFUNCTION(BlueprintCallable, Category = "Movie Graph")
	UE_API virtual bool SetMemberName(const FString& InNewName);
	...
	UE_API virtual bool CanRename(const FText& InNewName, FText& OutError) const;
...
public:
	...
	UFUNCTION(BlueprintPure, Category = "Movie Graph")
	UE_API bool IsGlobal() const;

	/** Gets the category (if any) assigned to this variable. */
	UFUNCTION(BlueprintPure, Category = "Movie Graph")
	UE_API const FString& GetCategory() const;
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Movie Graph")
	UE_API void SetCategory(const FString& InNewCategory);

	//~ Begin UMovieGraphMember interface
	UE_API virtual bool IsDeletable() const override;
	UE_API virtual bool CanRename(const FText& InNewName, FText& OutError) const override;
	UE_API virtual bool SetMemberName(const FString& InNewName) override;
...
public:
	UE_API UMovieGraphGlobalVariable();
...
struct FMovieGraphEvaluatedBranchConfig
{
	...
	UMovieGraphNode* GetNodeByClassExactMatch(const TSubclassOf<UMovieGraphNode>& InClass, const FString& InName) const
	{
...
public:
	...
	/**
	 * Gets a variable's value via a "value container". This container lets you fetch the variable's value in its native form, rather than being
	 * forced into a string (see GetVariableStringValueByName()). The variable's value is returned from the container via any of its GetValue*()
	 * methods, like GetValueFloat().
	 *
	 * Returns true if a variable with the provided name exists, else false. If the variable does not exist, the value container will be initialized
	 * to an empty one, which defaults to containing a float with value 0. The evaluated graph does not store a reference to this value container, so
	 * if used outside of blueprints, be sure to keep a reference to it in order to prevent it from being garbage collected.
	 */
	UFUNCTION(BlueprintCallable, Category = "Movie Graph", meta=(ReturnDisplayName = "Variable Exists"))
	bool GetVariableValueByName(
		UPARAM(DisplayName = "Variable Name") const FString& InVariableName,
		UPARAM(DisplayName = "Value Container") UMovieGraphValueContainer*& OutVariableValue) const;

	/**
	 * Convenience function to get a variable's string value, instead of its value container (see GetVariableValueByName()). Uses the provided
	 * default value if a variable by the given name does not exist. For variables that aren't string-typed, this provides the serialized
	 * representation of the variable value. Returns true if a variable with the provided name exists, else false.
	 */
	UFUNCTION(BlueprintCallable, Category = "Movie Graph", meta=(ReturnDisplayName = "Variable Exists"))
	bool GetVariableStringValueByName(
		UPARAM(DisplayName = "Variable Name") const FString& InVariableName,
		UPARAM(DisplayName = "Default Value") const FString& InDefaultValue,
		UPARAM(DisplayName = "String Value") FString& OutVariableValue) const;

	/** Gets all of the graphs that were included in the evaluation process. */
	UFUNCTION(BlueprintCallable, Category = "Movie Graph")
	const TArray<UMovieGraphConfig*>& GetEvaluatedGraphs() const;
...
private:
	// Allow the graph traversal/evaluation process to update some private members.
	friend class UMovieGraphConfig;

	/**
	 * The fully-evaluated values of all variables used in all graphs traversed in the evaluated result. The key is the variable name,
	 * and the value is the resolved value of the variable. If there is more than one entry in the array, that means that multiple graphs
	 * used the same variable name. This is not an error, but it makes fetching a variable by name challenging without any additional context.
	 * For future use, all variables with the same name are gathered, where the first in the array is the variable from the parent-most graph,
	 * and so on down the chain of subgraphs.
	 */
	TMap<FString, TArray<UMovieGraphValueContainer*>> VariablesByName;

	/** All of the graphs that were included in the evaluation process. */
	TArray<UMovieGraphConfig*> EvaluatedGraphs;
};
...
public:
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "Movie Graph")
	UE_API bool AddLabeledEdge(UMovieGraphNode* FromNode, const FName& FromPinLabel, UMovieGraphNode* ToNode, const FName& ToPinLabel);

	/**
	* Like AddLabeledEdge, removes the given connection between Node A and Node B (for the specified pins by name).
	*/
	UFUNCTION(BlueprintCallable, Category = "Movie Graph")
	UE_API bool RemoveLabeledEdge(UMovieGraphNode* FromNode, const FName& FromPinName, UMovieGraphNode* ToNode, const FName& ToPinName);
	
	/**
	* Convenience function which removes all Inbound (pins on the left side of a node) edges for the given node.
	*/
	UFUNCTION(BlueprintCallable, Category = "Movie Graph")
	UE_API bool RemoveAllInboundEdges(UMovieGraphNode* InNode);
	
	/**
	* Convenience function which removes all Outbound (pins on the right side of a node) edges for the given node.
	*/
	UFUNCTION(BlueprintCallable, Category = "Movie Graph")
	UE_API bool RemoveAllOutboundEdges(UMovieGraphNode* InNode);
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "Movie Graph")
	UE_API bool RemoveInboundEdges(UMovieGraphNode* InNode, const FName& InPinName);
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "Movie Graph")
	UE_API bool RemoveOutboundEdges(UMovieGraphNode* InNode, const FName& InPinName);
	...
	*/
	...
	UFUNCTION(BlueprintCallable, Category = "Movie Graph")
	UE_API TArray<FName> GetBranchNames() const;
	...
	*/
	...
	UE_API UMovieGraphNode* GetNodeForBranch(UClass* InClass, const FName& InBranchName, bool bExactMatch = false) const;
	...
	*/
	UFUNCTION(BlueprintCallable, meta = (DeterminesOutputType = "InClass"), Category = "Movie Graph")
	UE_API TArray<UMovieGraphNode*> GetNodesForBranch(UClass* InClass, const FName& InBranchName, bool bExactMatch = false) const;
	...
	*/
	...
	UE_API UMovieGraphNode* GetNodeForTag(const FString& ScriptTag, UClass* OptionalClassFilter = nullptr, const FName OptionalBranchFilter = NAME_None, bool bExactMatch = false) const;
	...
	*/
	UFUNCTION(BlueprintCallable, meta = (DeterminesOutputType = "OptionalClassFilter"), Category = "Movie Graph")
	UE_API TArray<UMovieGraphNode*> GetNodesForTag(const FString& ScriptTag, UClass* OptionalClassFilter = nullptr, const FName OptionalBranchFilter = NAME_None, bool bExactMatch = false) const;
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category="Movie Graph")
	UE_API UMovieGraphVariable* AddVariable(const FName InCustomBaseName = NAME_None);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Movie Graph")
	UE_API UMovieGraphInput* AddInput(const FText& InBaseName = FText::GetEmpty());
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Movie Graph")
	UE_API UMovieGraphOutput* AddOutput(const FText& InBaseName = FText::GetEmpty());

	/** Gets the variable in the graph with the specified GUID, else nullptr if one could not be found. */
	UFUNCTION(BlueprintCallable, Category="Movie Graph")
	UE_API UMovieGraphVariable* GetVariableByGuid(const FGuid& InGuid) const;
	
	/** Gets the variable in the graph with the specified name (including global variables), else nullptr if one could not be found. */
	UFUNCTION(BlueprintCallable, Category="Movie Graph")
	UE_API UMovieGraphVariable* GetVariableByName(const FString& InVariableName) const;
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category="Movie Graph")
	UE_API TArray<UMovieGraphVariable*> GetVariables(const bool bIncludeGlobal = false) const;

	/** Updates the values of all global variables. */
	UFUNCTION(BlueprintCallable, Category="Movie Graph")
	UE_API void UpdateGlobalVariableValues(const UMovieGraphPipeline* InPipeline);

	/** Gets all inputs that have been defined on the graph. */
	UFUNCTION(BlueprintCallable, Category = "Movie Graph")
	UE_API TArray<UMovieGraphInput*> GetInputs() const;

	/** Gets all outputs that have been defined on the graph. */
	UFUNCTION(BlueprintCallable, Category = "Movie Graph")
	UE_API TArray<UMovieGraphOutput*> GetOutputs() const;

	/** Remove the specified member (input, output, variable) from the graph. */
	UFUNCTION(BlueprintCallable, Category = "Movie Graph")
	UE_API bool DeleteMember(UMovieGraphMember* MemberToDelete);

	/** Duplicates the provided variable. Returns the new variable on success, else nullptr. */
	UFUNCTION(BlueprintCallable, Category="Movie Graph")
	UE_API UMovieGraphVariable* DuplicateVariable(UMovieGraphVariable* InVariableToDuplicate);
	...
	UE_API void SetEditorOnlyNodes(const TArray<TObjectPtr<const UObject>>& InNodes);
#endif
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category="Movie Graph")
	UE_API UMovieGraphEvaluatedConfig* CreateFlattenedGraph(const FMovieGraphTraversalContext& InContext, FString& OutError);
	...
	static UE_API FBoolProperty* FindOverridePropertyForRealProperty(UClass* InClass, const FProperty* InRealProperty);
	 ...
	 */
	UE_API void VisitUpstreamNodes(UMovieGraphNode* FromNode, const FVisitNodesCallback& VisitCallback) const;

	/**
	 * Visits all nodes downstream from FromNode, running VisitCallback on each one. Note this only follows branch connections,
	 * and does not recurse into subgraphs.
	 */
	UE_API void VisitDownstreamNodes(UMovieGraphNode* FromNode, const FVisitNodesCallback& VisitCallback) const;
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category="Movie Graph")
	UE_API TArray<FString> GetDownstreamBranchNames(UMovieGraphNode* FromNode, const UMovieGraphPin* FromPin, const bool bStopAtSubgraph = false) const;
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category="Movie Graph")
	UE_API TArray<FString> GetUpstreamBranchNames(UMovieGraphNode* FromNode, const UMovieGraphPin* FromPin, const bool bStopAtSubgraph = false) const;

	/** Get all subgraphs that this graph contains, recursively (ie, subgraphs of subgraphs are included, etc). */
	UFUNCTION(BlueprintCallable, Category="Movie Graph")
	UE_API void GetAllContainedSubgraphs(TSet<UMovieGraphConfig*>& OutSubgraphs) const;
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category="Movie Graph")
	UE_API void GetOutputDirectory(FString& OutOutputDirectory) const;
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category="Movie Graph")
	UE_API void MoveVariableBefore(UMovieGraphVariable* InTargetVariable, UMovieGraphVariable* InBeforeVariable);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category="Movie Graph")
	UE_API void MoveVariableToIndex(UMovieGraphVariable* InTargetVariable, int32 NewIndex);

	/** Moves one category (InCategoryToMove) and its variables before another category (InCategoryBefore). */
	UFUNCTION(BlueprintCallable, Category="Movie Graph")
	UE_API void MoveCategoryBefore(const FString& InCategoryToMove, const FString& InCategoryBefore);

protected:
	/** Look for the output directory in the UMovieGraphOutputSettings nodes found upstream of InNode. */
	UE_API void RecurseUpGlobalsBranchToFindOutputDirectory(const UMovieGraphNode* InNode, FString& OutOutputDirectory, TArray<const UMovieGraphConfig*>& VisitedGraphStack) const;
	...
	UE_API void CopyOverriddenProperties(UMovieGraphNode* FromNode, UMovieGraphNode* ToNode, const FMovieGraphEvaluationContext& InEvaluationContext);
	
	/** Find all "Overrideable" marked properties, then find their edit condition properties, then set those to false. */
	UE_API void InitializeFlattenedNode(UMovieGraphNode* InNode);
	 ...
	 */
	UE_API bool CreateFlattenedGraph_Recursive(UMovieGraphEvaluatedConfig* InOwningConfig, FMovieGraphEvaluatedBranchConfig& OutBranchConfig,
		FMovieGraphEvaluationContext& InEvaluationContext, UMovieGraphPin* InPinToFollow);

	/** Recursive helper for VisitUpstreamNodes(). */
	UE_API void VisitUpstreamNodes_Recursive(UMovieGraphNode* FromNode, const FVisitNodesCallback& VisitCallback, TSet<UMovieGraphNode*>& VisitedNodes) const;

	/** Recursive helper for VisitDownstreamNodes(). */
	UE_API void VisitDownstreamNodes_Recursive(UMovieGraphNode* FromNode, const FVisitNodesCallback& VisitCallback, TSet<UMovieGraphNode*>& VisitedNodes) const;
...
private:
	...
	UE_API bool DeleteVariableMember(UMovieGraphVariable* VariableMemberToDelete);
	
	/** Remove the specified input member from the graph. */
	UE_API bool DeleteInputMember(UMovieGraphInput* InputMemberToDelete);

	/** Remove the specified output member from the graph. */
	UE_API bool DeleteOutputMember(UMovieGraphOutput* OutputMemberToDelete);

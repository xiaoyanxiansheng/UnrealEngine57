 ...
 */
UCLASS(MinimalAPI)
class UMovieGraphCommandLineEncoderNode : public UMovieGraphSettingNode, public IMovieGraphPostRenderNode
{
	GENERATED_BODY()

public:
	UE_API UMovieGraphCommandLineEncoderNode();
	...
	UE_API void StartEncodingProcess(TArray<FMovieGraphRenderOutputData>& InGeneratedData, const bool bInIsShotEncode);
	
	// IMovieGraphPostRenderNode interface
	UE_API virtual void BeginExport(UMovieGraphPipeline* InMoviePipeline, TObjectPtr<UMovieGraphEvaluatedConfig>& InPrimaryJobEvaluatedGraph) override;
	UE_API virtual void BeginShotExport(UMovieGraphPipeline* InMoviePipeline) override;
	UE_API virtual bool HasFinishedExporting() override;
	// ~IMovieGraphPostRenderNode interface
...
private:
	 ...
	 */
	UE_API bool AreSettingsValid(TArray<FText>& OutErrors) const;
	 ...
	 */
	UE_API TMap<FMovieGraphRenderDataIdentifier, FEncoderParams> GenerateRenderLayerEncoderParams(TArray<FMovieGraphRenderOutputData>& InGeneratedData, const bool bInIsShotEncode) const;

	/** Gets the path to the file that the encode process should write to. */
	UE_API FString GetResolvedOutputFilename(const FMovieGraphRenderDataIdentifier& RenderIdentifier, const TWeakObjectPtr<UMoviePipelineExecutorShot>& Shot, const FString& FileNameFormatString) const;
	 ...
	 */
	UE_API void GenerateTemporaryEncoderInputFiles(const FEncoderParams& InParams, TArray<FString>& OutVideoInputFilePaths, TArray<FString>& OutAudioInputFilePaths) const;

	/** Generates the encoder command that should be executed. */
	UE_API FString BuildEncoderCommand(const FEncoderParams& InParams, TArray<FString>& InVideoInputFilePaths, TArray<FString>& InAudioInputFilePaths) const;
	...
	UE_API TObjectPtr<UMovieGraphEvaluatedConfig> GetEvaluatedConfig() const;
...
private:
	...
	static UE_API inline TArray<FActiveJob> ActiveEncodeJobs;

	/** The pipeline that started the export. */
	static UE_API inline TWeakObjectPtr<UMovieGraphPipeline> CachedPipeline;
	...
	static UE_API inline TObjectPtr<UMovieGraphEvaluatedConfig> PrimaryJobEvaluatedGraph;
};

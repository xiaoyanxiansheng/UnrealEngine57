...
public:
	 ...
	 */
	UE_API void Init(int32 InSizeX, int32 InSizeY);

	/**
	 * Zeroes the accumulation. Assumes the data is already allocated.
	 */
	UE_API void ZeroPlane();
	 ...
	 */
	UE_API void AccumulateSinglePlane(const TArray64<float>& InRawData, int32 InSizeX, int32 InSizeY, float InSampleWeight, int InSampleOffsetX, int InSampleOffsetY);
 ...
 */
struct FImageTileAccumulator : MoviePipeline::IMoviePipelineAccumulator
{
public:
	/**
	 * Default constructor.
	 ...
	 */
	UE_API void InitMemory(int InTileSizeX, int InTileSizeY, int InNumTilesX, int InNumTilesY, int InNumChannels);
	 ...
	 */
	UE_API int32 GetPlaneIndex(int32 InTileX, int32 InTileY, int32 InChannel) const;
	 ...
	 * @param InChannel - Channel index.
	 */
	UE_API FImageTilePlane& AtPlaneData(int32 InTileX, int32 InTileY, int32 InChannel);
	 ...
	 * @param InChannel - Channel index.
	 */
	UE_API const FImageTilePlane& AtPlaneData(int32 InTileX, int32 InTileY, int32 InChannel) const;
	 ...
	 */
	UE_API void AccumulateTile(const TArray64<float>& InRawData, int32 InRawSizeX, int32 InRawSizeY, int InRawChannel, FVector2D InSubpixelOffset);
	 ...
	 */
	UE_API float CalcSampleWeight(float InDistance) const;
	 ...
	 */
	UE_API void AccumulatePixelData(const FImagePixelData& InPixelData, int32 InTileX, int32 InTileY, FVector2D InSubpixelOffset);
	 ...
	 */
	UE_API void FetchFinalPixelDataByte(TArray64<FColor>& OutPixelData) const;
	 ...
	 */
	UE_API void FetchFinalPlaneScale(TArray<float>& PlaneScale) const;
	 ...
	 */
	UE_API void FetchFullImageValue(float Rgba[4], const TArray<float>& PlaneScale, int32 FullX, int32 FullY) const;

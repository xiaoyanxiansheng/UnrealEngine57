			...
			for (const UMovieSceneSection* Section : Node->MovieScene->GetAllSections())
			{
				...
				if (Section->GetRange().HasLowerBound())
				{
					...
					if (bOverlaps)
					{
						...
						for (const FMovieSceneBinding& Binding : ((const UMovieScene*)(Node->MovieScene.Get()))->GetBindings())
						{
							for (const UMovieSceneTrack* BindingTrack : Binding.GetTracks())
							{
								if (BindingTrack->HasSection(*Section))
								{
									BindingName = Node->MovieScene.Get()->GetObjectDisplayName(Binding.GetObjectGuid()).ToString();
									break;
								}
							}
						}
	...
	TTuple<FString, FString> GetNameForShot(const FMovieSceneSequenceHierarchy& InHierarchy, UMovieSceneSequence* InRootSequence, TSharedPtr<FCameraCutSubSectionHierarchyNode> InSubSectionHierarchy)
	{
		...
		if (InSubSectionHierarchy->CameraCutSection.IsValid())
		{
			...
			if (CameraObjectBindingId.IsValid())
			{
				...
				{
					InnerName = OwningSequence->GetMovieScene()->GetObjectDisplayName(CameraObjectBindingId.GetGuid()).ToString();
				}
			}
		}
...
namespace UE
{
	namespace MoviePipeline
	{
		...
		{
			FString Prefix = FString::Printf(TEXT("unreal/%s/%s"), *InCameraName, *InRenderPassName);
			GetMetadataFromCineCamera(InComponent, Prefix, InOutMetadata);
		}
		
		void GetMetadataFromCineCamera(UCineCameraComponent* InComponent, const FString& InPrefix, TMap<FString, FString>& InOutMetadata)
		{
			if (!InComponent) 
			{ 
				return; 
			...
			InOutMetadata.Add(FString::Printf(TEXT("%s/sensorWidth"), *InPrefix), FString::SanitizeFloat(InComponent->Filmback.SensorWidth));
			InOutMetadata.Add(FString::Printf(TEXT("%s/sensorHeight"), *InPrefix), FString::SanitizeFloat(InComponent->Filmback.SensorHeight));
			InOutMetadata.Add(FString::Printf(TEXT("%s/sensorAspectRatio"), *InPrefix), FString::SanitizeFloat(InComponent->Filmback.SensorAspectRatio));
			InOutMetadata.Add(FString::Printf(TEXT("%s/minFocalLength"), *InPrefix), FString::SanitizeFloat(InComponent->LensSettings.MinFocalLength));
			InOutMetadata.Add(FString::Printf(TEXT("%s/maxFocalLength"), *InPrefix), FString::SanitizeFloat(InComponent->LensSettings.MaxFocalLength));
			InOutMetadata.Add(FString::Printf(TEXT("%s/minFStop"), *InPrefix), FString::SanitizeFloat(InComponent->LensSettings.MinFStop));
			InOutMetadata.Add(FString::Printf(TEXT("%s/maxFStop"), *InPrefix), FString::SanitizeFloat(InComponent->LensSettings.MaxFStop));
			InOutMetadata.Add(FString::Printf(TEXT("%s/dofDiaphragmBladeCount"), *InPrefix), FString::FromInt(InComponent->LensSettings.DiaphragmBladeCount));
			InOutMetadata.Add(FString::Printf(TEXT("%s/focalLength"), *InPrefix), FString::SanitizeFloat(InComponent->CurrentFocalLength));
		}
		...
		void GetMetadataFromCameraLocRot(const FString& InCameraName, const FString& InRenderPassName, const FVector& InCurLoc, const FRotator& InCurRot, const FVector& InPrevLoc, const FRotator& InPrevRot, TMap<FString, FString>& InOutMetadata)
		{
			...
			FString Prefix = FString::Printf(TEXT("unreal/%s"), *CamName);
			GetMetadataFromCameraLocRot(InCurLoc, InCurRot, InPrevLoc, InPrevRot, Prefix, InOutMetadata);
		}
		
		void GetMetadataFromCameraLocRot(const FVector& InCurLoc, const FRotator& InCurRot, const FVector& InPrevLoc, const FRotator& InPrevRot, const FString& InPrefix, TMap<FString, FString>& InOutMetadata)
		{
			InOutMetadata.Add(FString::Printf(TEXT("%s/curPos/x"), *InPrefix), FString::SanitizeFloat(InCurLoc.X));
			InOutMetadata.Add(FString::Printf(TEXT("%s/curPos/y"), *InPrefix), FString::SanitizeFloat(InCurLoc.Y));
			InOutMetadata.Add(FString::Printf(TEXT("%s/curPos/z"), *InPrefix), FString::SanitizeFloat(InCurLoc.Z));
			InOutMetadata.Add(FString::Printf(TEXT("%s/curRot/pitch"), *InPrefix),  FString::SanitizeFloat(InCurRot.Pitch));
			InOutMetadata.Add(FString::Printf(TEXT("%s/curRot/yaw"), *InPrefix), FString::SanitizeFloat(InCurRot.Yaw));
			InOutMetadata.Add(FString::Printf(TEXT("%s/curRot/roll"), *InPrefix), FString::SanitizeFloat(InCurRot.Roll));

			InOutMetadata.Add(FString::Printf(TEXT("%s/prevPos/x"), *InPrefix), FString::SanitizeFloat(InPrevLoc.X));
			InOutMetadata.Add(FString::Printf(TEXT("%s/prevPos/y"), *InPrefix), FString::SanitizeFloat(InPrevLoc.Y));
			InOutMetadata.Add(FString::Printf(TEXT("%s/prevPos/z"), *InPrefix), FString::SanitizeFloat(InPrevLoc.Z));
			InOutMetadata.Add(FString::Printf(TEXT("%s/prevRot/pitch"), *InPrefix), FString::SanitizeFloat(InPrevRot.Pitch));
			InOutMetadata.Add(FString::Printf(TEXT("%s/prevRot/yaw"), *InPrefix), FString::SanitizeFloat(InPrevRot.Yaw));
			InOutMetadata.Add(FString::Printf(TEXT("%s/prevRot/roll"), *InPrefix), FString::SanitizeFloat(InPrevRot.Roll));
		}
		...
		void SetSkeletalMeshClothSubSteps(const int32 InSubdivisionCount, UWorld* InWorld, TMap<TWeakObjectPtr<UObject>, TArray<::MoviePipeline::FClothSimSettingsCache>>& InClothSimCache)
		{
			...
			for (TActorIterator<AActor> ActorIt(InWorld); ActorIt; ++ActorIt)
			{
				...
				if (FoundActor)
				{
					...
					for (const USkeletalMeshComponent* const Component : SkeletalMeshComponents)
					{
						for (const FClothingSimulationInstance& ClothingSimulationInstance : Component->GetClothingSimulationInstances())
						{
							const IClothingSimulationInterface* const ClothingSimulation = ClothingSimulationInstance.GetClothingSimulation();
							UClothingSimulationInteractor* const ClothingSimulationInteractor = ClothingSimulationInstance.GetClothingSimulationInteractor();
							if (ClothingSimulation && ClothingSimulationInteractor)
							{
								constexpr int32 LODIndex = 0;  // There is only a NumSubSteps for LOD 0 in the Skeletal Mesh clothing system
								const TWeakObjectPtr<UObject> WeakPtr(ClothingSimulationInteractor);

								TArray<::MoviePipeline::FClothSimSettingsCache>* ExistingCacheEntry = InClothSimCache.Find(WeakPtr);
								if (!ExistingCacheEntry)
								{
									ExistingCacheEntry = &InClothSimCache.Add(WeakPtr);
									ExistingCacheEntry->SetNumZeroed(1);  // Only store LOD 0
									(*ExistingCacheEntry)[LODIndex] = { ClothingSimulation->GetNumSubsteps(), 0.f };
								}
								ClothingSimulationInteractor->SetNumSubsteps((*ExistingCacheEntry)[LODIndex].NumSubSteps * InSubdivisionCount);
							}
						}
					}
						...
						if (ClothAssetInteractor)
						{
							...
							if (!ExistingCacheEntry)
							{
								...
								for (int32 LODIndex = 0; LODIndex < NumLODs; ++LODIndex)
								{
									...
									const int32 NumSubsteps = FMath::Max(ClothAssetInteractor->GetIntPropertyValue(TEXT("NumSubsteps"), LODIndex, MinNumSubsteps), MinNumSubsteps);
									const float DynamicSubstepDeltaTime = ClothAssetInteractor->GetFloatPropertyValue(TEXT("DynamicSubstepDeltaTime"), LODIndex, MinNumSubsteps);
							...
							for (int32 LODIndex = 0; LODIndex < NumLODs; ++LODIndex)
							{
								...
								{
									ClothAssetInteractor->SetIntPropertyValue(TEXT("NumSubsteps"), LODIndex, (*ExistingCacheEntry)[LODIndex].NumSubSteps * InSubdivisionCount);
									ClothAssetInteractor->SetFloatPropertyValue(TEXT("DynamicSubstepDeltaTime"), LODIndex, 0.f);
								}
							}
						}
					}
			...
			for (const TPair<TWeakObjectPtr<UObject>, TArray<::MoviePipeline::FClothSimSettingsCache>>& Pair : InClothSimCache)
			{
				if (UObject* Object = Pair.Key.Get())
				{
					...
					else if (UChaosClothAssetInteractor* ClothAssetInteractor = Cast<UChaosClothAssetInteractor>(Object))
					{
						...
						{
							ClothAssetInteractor->SetIntPropertyValue(TEXT("NumSubsteps"), LODIndex, Pair.Value[LODIndex].NumSubSteps);
							ClothAssetInteractor->SetFloatPropertyValue(TEXT("DynamicSubstepDeltaTime"), LODIndex, Pair.Value[LODIndex].DynamicSubstepDeltaTime);
						}
					}
				}
			}
...
namespace UE
{
	namespace MoviePipeline
	{
		...
		FString GetMetadataPrefixPath(const FMovieGraphRenderDataIdentifier& Identifier)
		{
			return FString::Printf(TEXT("unreal/%s/%s/%s"), *Identifier.LayerName, *Identifier.RendererName, *Identifier.CameraName);
		}

...
public:
	UE_API UMovieGraphPipeline();

	UFUNCTION(BlueprintCallable, Category = "Movie Graph")
	UE_API void Initialize(UMoviePipelineExecutorJob* InJob, const FMovieGraphInitConfig& InitConfig);
	...
	*/
	...
	UE_API UMovieGraphConfig* GetRootGraphForShot(UMoviePipelineExecutorShot* InShot) const;

	UFUNCTION(BlueprintCallable, Category = "Movie Graph")
	UE_API FMovieGraphTraversalContext GetCurrentTraversalContext(const bool bForShot = true) const;
	...
	UE_API void SetupShot(const TObjectPtr<UMoviePipelineExecutorShot>& InShot);
	...
	UE_API void TeardownShot(const TObjectPtr<UMoviePipelineExecutorShot>& InShot);
	
	/** Used occasionally to cross-reference other components. Don't call this unless you know what you're doing. */
	UFUNCTION(BlueprintCallable, Category = "Movie Graph")
	UE_API UMovieGraphTimeStepBase* GetTimeStepInstance() const;
	...
	UE_API void AddOutputFuture(TFuture<bool>&& InOutputFuture, const UE::MovieGraph::FMovieGraphOutputFutureData& InData);
	 ...
	 */
	UE_API TArray<FMovieGraphRenderOutputData>& GetGeneratedOutputData();
	 ...
	 */
	UE_API void CreateLayersInRenderLayerSubsystem(const UMovieGraphEvaluatedConfig* EvaluatedConfig) const;
	 ...
	 */
	UE_API void UpdateLayerContentsInRenderLayerSubsystem(const UMovieGraphEvaluatedConfig* EvaluatedConfig) const;

protected:
	UFUNCTION(BlueprintCallable, Category = "Movie Render Pipeline")
	UE_API virtual void OnMoviePipelineFinishedImpl();

	UE_API virtual void OnEngineTickBeginFrame();
	UE_API virtual void OnEngineTickEndFrame();
	UE_API virtual void RenderFrame();
	UE_API virtual void BuildShotListFromDataSource();
	 ...
	 */
	UE_API virtual void ProcessOutstandingFinishedFrames();
	 ...
	 */
	UE_API virtual void ProcessOutstandingFutures(const bool bStallAndProcessAllFutures = false);

	UE_API virtual void TickProducingFrames();
	UE_API virtual void TickPostFinalizeExport(const bool bInForceFinish);
	UE_API virtual void TickFinalizeOutputContainers(const bool bInForceFinish);
	UE_API virtual void TransitionToState(const EMovieRenderPipelineState InNewState);
	UE_API virtual const TSet<TObjectPtr<UMovieGraphFileOutputNode>> GetOutputNodesUsed() const;
	UE_API virtual void BeginFinalize();

	/** Begins the export process for a primary job (not called for shot jobs). */
	UE_API virtual void BeginExport();
	
	/** Attempts to start an Unreal Insights capture to a file on disk adjacent to the movie output. */
	UE_API void StartUnrealInsightsCapture(UMovieGraphEvaluatedConfig* EvaluatedConfig);
	/** Attempts to stop an already started Unreal Insights capture. */
	UE_API void StopUnrealInsightsCapture();
	
	UE_API virtual void LoadPreviewWidget();
	UE_API virtual void SetPreviewWidgetVisibleImpl(bool bInIsVisible);

	UE_API virtual void DuplicateJobAndConfiguration();
	UE_API virtual void ExecutePreJobScripts();
	UE_API virtual void ExecutePostJobScripts(const FMoviePipelineOutputData& InData);
	UE_API virtual void ExecutePreShotScripts(const TObjectPtr<UMoviePipelineExecutorShot>& InShot);
	UE_API virtual void ExecutePostShotScripts(const FMoviePipelineOutputData& InData);
	...
	UE_API virtual void SetSoloShot(const TObjectPtr<UMoviePipelineExecutorShot>& InShot);
	...
	*/
	UE_API virtual void ExpandShot(const TObjectPtr<UMoviePipelineExecutorShot>& InShot, const int32 InNumHandleFrames, const bool bInHasMultipleTemporalSamples, const bool bIsPrePass,
	...
	UE_API int32 ResolveVersionForShot(const TObjectPtr<UMoviePipelineExecutorShot>& Shot, const TObjectPtr<UMovieGraphEvaluatedConfig>& EvaluatedConfig);

	// UMoviePipelineBase Interface
	UE_API virtual void RequestShutdownImpl(bool bIsError) override;
	UE_API virtual void ShutdownImpl(bool bIsError) override;
	...
	UE_API virtual bool IsPostShotCallbackNeeded() const override;
	// ~UMoviePipelineBase Interface
...
public:
	static UE_API FString DefaultPreviewWidgetAsset;
};

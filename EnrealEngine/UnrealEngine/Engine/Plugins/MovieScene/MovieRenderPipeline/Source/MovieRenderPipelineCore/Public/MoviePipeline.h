...
public:
	MOVIERENDERPIPELINECORE_API UMoviePipeline();
	...
	*/
	...
	MOVIERENDERPIPELINECORE_API void Initialize(UMoviePipelineExecutorJob* InJob);

	// UMoviePipelineBase Interface
	MOVIERENDERPIPELINECORE_API virtual void RequestShutdownImpl(bool bIsError) override;
	MOVIERENDERPIPELINECORE_API virtual void ShutdownImpl(bool bError ) override;
	...
	*/
	UFUNCTION(BlueprintPure, Category = "Movie Render Pipeline")
	MOVIERENDERPIPELINECORE_API UMoviePipelinePrimaryConfig* GetPipelinePrimaryConfig() const;
...
public:
	...
	MOVIERENDERPIPELINECORE_API FMoviePipelineOutputData GetOutputDataParams();

	MOVIERENDERPIPELINECORE_API void GetSidecarCameraData(UMoviePipelineExecutorShot* InShot, int32 InCameraIndex, FMinimalViewInfo& OutViewInfo, class UCameraComponent** OutCameraComponent) const;
	MOVIERENDERPIPELINECORE_API bool GetSidecarCameraViewPoints(UMoviePipelineExecutorShot* InShot, TArray<FVector>& OutSidecarViewLocations, TArray<FRotator>& OutSidecarViewRotations) const;

	/** Gets any cached overscan for the specified camera, or 0 if no cached overscan was found */
	MOVIERENDERPIPELINECORE_API float GetCachedCameraOverscan(int32 InCameraIndex) const;

	/** Gets whether there is a cached overscan value for the specified camera */
	MOVIERENDERPIPELINECORE_API bool HasCachedCameraOverscan(int32 InCameraIndex) const;

	/** Caches the provided overscan value for the specified camera */
	MOVIERENDERPIPELINECORE_API void CacheCameraOverscan(int32 InCameraIndex, float InCameraOverscan);
	...
	MOVIERENDERPIPELINECORE_API void WarnAboutAnimatedOverscan(float InInitialOverscan);
...
public:
#if WITH_EDITOR
	MOVIERENDERPIPELINECORE_API void AddFrameToOutputMetadata(const FString& ClipName, const FString& ImageSequenceFileName, const FMoviePipelineFrameOutputState& FrameOutputState, const FString& Extension, const bool bHasAlpha);
#endif
	MOVIERENDERPIPELINECORE_API void AddOutputFuture(TFuture<bool>&& OutputFuture, const MoviePipeline::FMoviePipelineOutputFutureData& InData);

	MOVIERENDERPIPELINECORE_API void ProcessOutstandingFinishedFrames();
	MOVIERENDERPIPELINECORE_API void ProcessOutstandingFutures();
	MOVIERENDERPIPELINECORE_API void OnSampleRendered(TUniquePtr<FImagePixelData>&& OutputSample);
...
public:
	...
	MOVIERENDERPIPELINECORE_API TArray<UMoviePipelineSetting*> FindSettingsForShot(TSubclassOf<UMoviePipelineSetting> InSetting, const UMoviePipelineExecutorShot* InShot) const;
	...
	*/
	MOVIERENDERPIPELINECORE_API void ResolveFilenameFormatArguments(const FString& InFormatString, const TMap<FString, FString>& InFormatOverrides, FString& OutFinalPath, FMoviePipelineFormatArgs& OutFinalFormatArgs, const FMoviePipelineFrameOutputState* InOutputState=nullptr, const int32 InFrameNumberOffset=0) const;
...
private:
	...
	MOVIERENDERPIPELINECORE_API void OnEngineTickEndFrame();

	MOVIERENDERPIPELINECORE_API void ValidateSequenceAndSettings() const;


	/** Runs the per-tick logic when doing the ProducingFrames state. */
	MOVIERENDERPIPELINECORE_API void TickProducingFrames();

	MOVIERENDERPIPELINECORE_API void ProcessEndOfCameraCut(UMoviePipelineExecutorShot* InCameraCut);


	/** Called once when first moving to the Finalize state. */
	MOVIERENDERPIPELINECORE_API void BeginFinalize();
	/** Called once when first moving to the Export state. */
	MOVIERENDERPIPELINECORE_API void BeginExport();

	/** Attempts to start an Unreal Insights capture to a file on disk adjacent to the movie output. */
	MOVIERENDERPIPELINECORE_API void StartUnrealInsightsCapture();
	/** Attempts to stop an already started Unreal Insights capture. */
	MOVIERENDERPIPELINECORE_API void StopUnrealInsightsCapture();
	...
	*/
	MOVIERENDERPIPELINECORE_API void TickFinalizeOutputContainers(const bool bInForceFinish);
	/** 
	...
	*/
	MOVIERENDERPIPELINECORE_API void TickPostFinalizeExport(const bool bInForceFinish);
	...
	MOVIERENDERPIPELINECORE_API bool DebugFrameStepPreTick();
	/** Returns true if we are idling because of debug frame stepping. */
	MOVIERENDERPIPELINECORE_API bool IsDebugFrameStepIdling() const;

	/** Debugging/Information. Don't hinge any logic on this as it will get called multiple times per frame in some cases. */
	MOVIERENDERPIPELINECORE_API void OnSequenceEvaluated(const UMovieSceneSequencePlayer& Player, FFrameTime CurrentTime, FFrameTime PreviousTime);

	/** Set up per-shot state for the specific shot, tearing down old state (if it exists) */
	MOVIERENDERPIPELINECORE_API void InitializeShot(UMoviePipelineExecutorShot* InShot);
	MOVIERENDERPIPELINECORE_API void TeardownShot(UMoviePipelineExecutorShot* InShot);
	...
	MOVIERENDERPIPELINECORE_API void SetupRenderingPipelineForShot(UMoviePipelineExecutorShot* InShot);
	/** Deinitialize the rendering pipeline for the given shot. */
	MOVIERENDERPIPELINECORE_API void TeardownRenderingPipelineForShot(UMoviePipelineExecutorShot* InShot);
	...
	MOVIERENDERPIPELINECORE_API void FlushAsyncEngineSystems();

	/** If the log verbosity is high enough, prints out the files specified in the shot output data. */
	MOVIERENDERPIPELINECORE_API void PrintVerboseLogForFiles(const TArray<FMoviePipelineShotOutputData>& InOutputData) const;
	...
	MOVIERENDERPIPELINECORE_API void StartAudioRecording();
	/** Tell our submixes to stop capturing the data, and then store a copy of it. */
	MOVIERENDERPIPELINECORE_API void StopAudioRecording();
	/** Attempt to process the audio thread work. This is complicated by our non-linear time steps */
	MOVIERENDERPIPELINECORE_API void ProcessAudioTick();
	MOVIERENDERPIPELINECORE_API void SetupAudioRendering();
	MOVIERENDERPIPELINECORE_API void TeardownAudioRendering();
	...
	*/
	MOVIERENDERPIPELINECORE_API void RenderFrame();

	/** Allow any Settings to modify the (already duplicated) sequence. This allows inserting automatic pre-roll, etc. */
	MOVIERENDERPIPELINECORE_API void ModifySequenceViaExtensions(ULevelSequence* InSequence);

	/**
	* Should the Progress UI be visible on the player's screen?
	*/
	MOVIERENDERPIPELINECORE_API void SetProgressWidgetVisible(bool bVisible);

	/** Returns list of render passes for a given shot */
	MOVIERENDERPIPELINECORE_API TArray<UMoviePipelineRenderPass*> GetAllRenderPasses(const UMoviePipelineExecutorShot* InShot);


private:
	/** Iterates through the changes we've made to a shot and applies the original settings. */
	MOVIERENDERPIPELINECORE_API void RestoreTargetSequenceToOriginalState();
	...
	MOVIERENDERPIPELINECORE_API void InitializeLevelSequenceActor();
	...
	MOVIERENDERPIPELINECORE_API void BuildShotListFromSequence();
	...
	*/
	MOVIERENDERPIPELINECORE_API void SetSoloShot(UMoviePipelineExecutorShot* InShot);

	/* Expands the specified shot (and contained camera cuts)'s ranges for the given settings. */
	MOVIERENDERPIPELINECORE_API void ExpandShot(UMoviePipelineExecutorShot* InShot, const int32 InNumHandleFrames, const bool bIsPrePass);

	/** Calculates lots of useful numbers used in timing based off of the current shot. These are constant for a given shot. */
	MOVIERENDERPIPELINECORE_API MoviePipeline::FFrameConstantMetrics CalculateShotFrameMetrics(const UMoviePipelineExecutorShot* InShot) const;

	/** It can be useful to know where the data we're generating was relative to the original Timeline, so this calculates that. */
	MOVIERENDERPIPELINECORE_API void CalculateFrameNumbersForOutputState(const MoviePipeline::FFrameConstantMetrics& InFrameMetrics, const UMoviePipelineExecutorShot* InCameraCut, FMoviePipelineFrameOutputState& InOutOutputState) const;
	...
	MOVIERENDERPIPELINECORE_API void TransitionToState(const EMovieRenderPipelineState InNewState);
private:
...
public:
	static MOVIERENDERPIPELINECORE_API FString DefaultDebugWidgetAsset;
};

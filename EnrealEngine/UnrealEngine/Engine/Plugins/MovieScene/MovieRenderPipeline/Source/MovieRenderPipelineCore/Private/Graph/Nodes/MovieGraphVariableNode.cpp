...
#include UE_INLINE_GENERATED_CPP_BY_NAME(MovieGraphVariableNode)
...
FString UMovieGraphVariableNode::GetResolvedValueForOutputPin(const FName& InPinName, const FMovieGraphTraversalContext* InContext) const
{
	if (GraphVariable && (GraphVariable->GetMemberName() == InPinName))
	{
		...
		if (ContextHasEnabledAssignmentForVariable(GraphVariable, InContext, VariableAssignment))
		{
...
{
	return GraphVariable && GraphVariable->IsGlobal();
}

bool UMovieGraphVariableNode::GetResolvedVariableValue(const TObjectPtr<UMovieGraphVariable> InGraphVariable, const FMovieGraphTraversalContext* InContext, TObjectPtr<UMovieGraphValueContainer>& OutValueContainer)
{
	TObjectPtr<UMovieJobVariableAssignmentContainer> VariableAssignment;
	if (ContextHasEnabledAssignmentForVariable(InGraphVariable.Get(), InContext, VariableAssignment))
	{
		return VariableAssignment->GetValueContainer(InGraphVariable.Get(), OutValueContainer);
	}
	
	// No valid variable assignment: just get the value from the variable
	OutValueContainer = InGraphVariable;
	return true;
}
...
bool UMovieGraphVariableNode::ContextHasEnabledAssignmentForVariable(const TObjectPtr<UMovieGraphVariable> InGraphVariable, const FMovieGraphTraversalContext* InContext, TObjectPtr<UMovieJobVariableAssignmentContainer>& OutVariableAssignment)
{
	if (!InContext)
	{
		return false;
	...
	if (Shot)
	{
		ShotVariableAssignments = Shot->GetOrCreateJobVariableAssignmentsForGraph(InGraphVariable->GetOwningGraph());
		...
		if (PrimaryJob)
		{
			...
			ShotVariableAssignments_PrimaryOverrides = Shot->GetOrCreateJobVariableAssignmentsForGraph(InGraphVariable->GetOwningGraph(), bIsForPrimaryOverrides);
		}
	}

	if (PrimaryJob)
	{
		JobVariableAssignments = PrimaryJob->GetOrCreateJobVariableAssignmentsForGraph(InGraphVariable->GetOwningGraph());
	}
	...
	if (ShotVariableAssignments && ShotVariableAssignments->GetVariableAssignmentEnableState(InGraphVariable.Get(), bIsEnabled))
	{
		if (bIsEnabled)
		{
	...
	if (ShotVariableAssignments_PrimaryOverrides && ShotVariableAssignments_PrimaryOverrides->GetVariableAssignmentEnableState(InGraphVariable.Get(), bIsEnabled))
	{
		if (bIsEnabled)
		{
	...
	if (JobVariableAssignments && JobVariableAssignments->GetVariableAssignmentEnableState(InGraphVariable.Get(), bIsEnabled))
	{
		if (bIsEnabled)
		{

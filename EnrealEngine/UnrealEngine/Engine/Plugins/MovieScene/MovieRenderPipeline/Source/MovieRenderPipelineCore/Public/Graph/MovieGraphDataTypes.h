...
public:
	...
	*/
	...
	UE_API UMovieGraphPipeline* GetOwningGraph() const;
};
...
class UMovieGraphEngineTimeStep : public UEngineCustomTimeStep
{
	GENERATED_BODY()
public:
	UE_API UMovieGraphEngineTimeStep();
...
public:
	UE_API void SetCachedFrameTiming(const FTimeStepCache& InTimeCache);
	...
	UE_API virtual bool UpdateTimeStep(UEngine* InEngine) override;
...
public:
	...
	UE_API const MoviePipeline::FAudioState& GetAudioState() const;
...
struct FMovieGraphRenderPassLayerData
{
	...
	/**
	 * The render pass node that's associated with this layer data. WARNING: this node will ONLY be valid during initial pass setup! Post-setup, if
	 * a non-CDO node is needed, it needs to be fetched from the evaluated config.
	 */
	...
	*/
	...
	struct FMovieGraphSampleState : IImagePixelDataPayload, public TSharedFromThis<FMovieGraphSampleState>
	{
		FMovieGraphSampleState()
			...
			, RenderLayerIndex(0)
			, bAllowOCIO(true)
			, bForceLosslessCompression(false)
		{}
		...
		*/
		...
		/** Specifies the filename format that this sample should use when being written to disk. Not used for debug like Debug_OverrideFilename. */
		...
		*/
		...
		bool bForceLosslessCompression;
		...
		/**
		 * Specifies the output node class(es) that can output this sample. If this array is empty, this sample will be processed by all
		 * output nodes (which is typical).
		 */
		TArray<FSoftClassPath> OutputTypeRestrictions;
	};
	...
	*/
	struct FMovieGraphOutputMergerFrame
	{
		...
		*/
		...
		/**
		 * Specifies which payloads will be written from a specific output node instance.
		 * NOTE: This is only updated immediately prior to filename disambiguation and should not be used outside of that.
		 */
		TMap<const UMovieGraphFileOutputNode*, TArray<UE::MovieGraph::FMovieGraphSampleState*>> NodeInstanceToPayloads;

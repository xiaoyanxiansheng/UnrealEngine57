...
#include UE_INLINE_GENERATED_CPP_BY_NAME(MovieGraphPipeline)
...
void UMovieGraphPipeline::Initialize(UMoviePipelineExecutorJob* InJob, const FMovieGraphInitConfig& InitConfig)
{
	...
	else
	{
		...
		UE_LOG(LogMovieRenderPipeline, Error, TEXT("Unable to initialize Movie Pipeline. The following error(s) were encountered: %s"), *OutError);
	}
...
void UMovieGraphPipeline::DuplicateJobAndConfiguration()
{
	...
	// instances of your script, one which receives 4 callbacks (pre/post job, pre/post shot) and one that only receives pre/post shot.
...
void UMovieGraphPipeline::UpdateLayerContentsInRenderLayerSubsystem(const UMovieGraphEvaluatedConfig* EvaluatedConfig) const
{
	...
	for (UMovieGraphRenderLayer* RenderLayer : LayerSubsystem->GetRenderLayers())
	{
		...
		TArray<IMovieGraphModifierNodeInterface*> ModifierNodes =
			EvaluatedConfig->GetSettingsImplementing<IMovieGraphModifierNodeInterface>(UMovieGraphModifierNodeInterface::StaticClass(), LayerName);
		...
		for (const IMovieGraphModifierNodeInterface* ModifierNode : ModifierNodes)
		{
			// For modifiers that don't use collections, just add all of the modifiers unconditionally.
			if (!ModifierNode->SupportsCollections())
			{
				for (UMovieGraphModifierBase* Modifier : ModifierNode->GetAllModifiers())
				{
					RenderLayer->AddLayerModifier(Modifier);
				}
			...
			for (UMovieGraphModifierBase* ModifierInstance : ModifierNode->GetAllModifiers())
			{
				// All of these modifiers should support collections (due to the SupportsCollections() call), but double-check anyway
				UMovieGraphCollectionModifier* CollectionModifierInstance = Cast<UMovieGraphCollectionModifier>(ModifierInstance);
				if (!ensureMsgf(CollectionModifierInstance, TEXT("Found a modifier node which declares it supports collections, but provided modifiers that don't use collections.")))
				{
					continue;
				}
				
				CollectionModifierInstance->SetCollections({});
				TArray<UMovieGraphCollection*> ModifierCollections;

				for (const FName& ModifiedCollectionName : ModifierNode->GetAllCollections())
				{
					...
					if (!bFoundModifiedCollection)
					{
						UE_LOG(LogMovieRenderPipeline, Warning, TEXT("The modifier '%s' specified a collection '%s', but the collection couldn't be found."),
							*CollectionModifierInstance->GetName(), *ModifiedCollectionName.ToString());
					}
				}
				...
				{
					CollectionModifierInstance->SetCollections(ModifierCollections);

					RenderLayer->AddLayerModifier(CollectionModifierInstance);
				}
			}
		}
	}
...
void UMovieGraphPipeline::ProcessOutstandingFinishedFrames()
{
	while (!OutputMerger->GetFinishedFrames().IsEmpty())
	{
		...
		// For each layer in the output merger frame...
		for (TPair<FMovieGraphRenderDataIdentifier, TUniquePtr<FImagePixelData>>& LayerData : OutputFrame.ImageOutputData)
		{
			UE::MovieGraph::FMovieGraphSampleState* LayerPayload = LayerData.Value->GetPayload<UE::MovieGraph::FMovieGraphSampleState>();
			if (!ensure(LayerPayload))
			{
				continue;
			}
			...
			// For each File Output Node CDO in the layer, associate the CDO with the layer...
			//
			...
			constexpr bool bIncludeCDOs = false;
			const FMovieGraphRenderDataIdentifier& LayerIdentifier = LayerData.Key;
			const FName& LayerBranchName = LayerIdentifier.RootBranchName;
			TArray<UMovieGraphFileOutputNode*> OutputNodesForLayer = OutputFrame.EvaluatedConfig->GetSettingsForBranch<UMovieGraphFileOutputNode>(LayerBranchName, bIncludeCDOs);
			for (const UMovieGraphFileOutputNode* LayerOutputNode : OutputNodesForLayer)
			{
				UMovieGraphFileOutputNode* CDO = LayerOutputNode->GetClass()->GetDefaultObject<UMovieGraphFileOutputNode>();
				MaskData.FindOrAdd(CDO).Add(LayerIdentifier);
			}
		}
		...
		{
			UMovieGraphFileOutputNode* OutputCDO = Pair.Key;
			const TSet<FMovieGraphRenderDataIdentifier>& LayerIdentifiers = Pair.Value;

			// Tell the CDO that it's getting data from the output merger frame, and specify which layers it should output data for
			Pair.Key->OnReceiveImageData(this, &OutputFrame, LayerIdentifiers);
			...
			OutputNodesDataSentTo.Add(OutputCDO);
		}
	}
}

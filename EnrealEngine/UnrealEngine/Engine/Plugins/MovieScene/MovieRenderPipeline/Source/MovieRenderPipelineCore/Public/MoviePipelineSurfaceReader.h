	...
	: public TSharedFromThis<FMoviePipelineSurfaceReader, ESPMode::ThreadSafe>
{
	...
	UE_API FMoviePipelineSurfaceReader(EPixelFormat InPixelFormat, bool bInInvertAlpha);

	UE_API ~FMoviePipelineSurfaceReader();
	...
	UE_API void BlockUntilAvailable();
	...
	*/
	UE_API void ResolveSampleToReadbackTexture_RenderThread(const FTextureRHIRef& SourceSurfaceSample);

	/**
	* Maps the ReadbackTexture to the CPU (which should have been resolved to before this point) and copies the data over.
	*/
	UE_API void CopyReadbackTexture_RenderThread(TUniqueFunction<void(TUniquePtr<FImagePixelData>&&)>&& InFunctionCallback, TSharedPtr<FImagePixelDataPayload, ESPMode::ThreadSafe> InFramePayload);

protected:
	friend struct FMoviePipelineSurfaceQueue;
	/** Set up this surface to the specified width/height */
	UE_API void ResizeImpl(uint32 Width, uint32 Height);
...
{
public:
	UE_API FMoviePipelineSurfaceQueue(FIntPoint InSurfaceSize, EPixelFormat InPixelFormat, uint32 InNumSurfaces, bool bInInvertAlpha);
	UE_API ~FMoviePipelineSurfaceQueue();
...
public:
	...
	UE_API void OnRenderTargetReady_RenderThread(const FTextureRHIRef InRenderTarget, TSharedRef<FImagePixelDataPayload, ESPMode::ThreadSafe> InPayload, TUniqueFunction<void(TUniquePtr<FImagePixelData>&&)>&& InFunctionCallback);

	UE_API void BlockUntilAnyAvailable();
	UE_API void Shutdown();

	/** Updates the stored last used frame to be the current frame */
	UE_API void UpdateLastUsedFrame();

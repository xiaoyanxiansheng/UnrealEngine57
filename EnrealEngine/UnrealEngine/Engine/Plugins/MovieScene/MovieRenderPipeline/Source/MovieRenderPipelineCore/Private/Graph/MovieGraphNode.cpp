...
#include UE_INLINE_GENERATED_CPP_BY_NAME(MovieGraphNode)
...
FInstancedPropertyBag* UMovieGraphNode::GetMutableDynamicProperties_Unsafe() const
{
	return const_cast<FInstancedPropertyBag*>(&DynamicProperties);
}
...
bool UMovieGraphNode::GetNodeValidationErrors(const FName& InBranchName, const UMovieGraphEvaluatedConfig* InEvaluatedConfig, TArray<FText>& OutValidationErrors) const
{
...
TArray<TPair<FName, UClass*>> UMovieGraphNode::GetHiddenProperties() const
{
	// By default, no properties are hidden.
	return {};
}
#endif // WITH_EDITOR
...
void UMovieGraphSettingNode::ResolveTokenContainingProperties(TFunction<void(FString&)>& ResolveFunc, const FMovieGraphTokenResolveContext& InContext)
{
	// The default node has no properties that contain {tokens} that need to be resolved.

	// Dev note: Ideally we could use UPROPERTY metadata to define these properties. However, UPROPERTY metadata isn't available in runtime
	// (non-editor) builds, so it's not a viable solution.
}

void IMovieGraphEvaluationNodeInjector::InjectNodesPostEvaluation(const FName& InBranchName, UMovieGraphEvaluatedConfig* InEvaluatedConfig, TArray<UMovieGraphSettingNode*>& OutInjectedNodes)
{

...
public:
	...
	*/
	...
	UE_API void Execute(UMoviePipelineQueue* InPipelineQueue);
...
protected:
	...
	*/
	...
	UE_API virtual void OnBeginFrame_Implementation();
...
protected:
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "Movie Render Pipeline")
	UE_API bool ConnectSocket(const FString& InHostName, const int32 InPort);

	/*
	* Disconnects the socket (if currently connected.)
	*/
	UFUNCTION(BlueprintCallable, Category = "Movie Render Pipeline")
	UE_API void DisconnectSocket();
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "Movie Render Pipeline")
	UE_API int32 SendHTTPRequest(const FString& InURL, const FString& InVerb, const FString& InMessage, const TMap<FString, FString>& InHeaders);
...
private:
	UE_API bool BlockingSendSocketMessageImpl(const uint8* Data, int32 BytesToSend);
	UE_API void OnHttpRequestCompletedImpl(FHttpRequestPtr InRequest, FHttpResponsePtr InResponse, bool bWasSuccessful);
	UE_API bool ProcessIncomingSocketData();

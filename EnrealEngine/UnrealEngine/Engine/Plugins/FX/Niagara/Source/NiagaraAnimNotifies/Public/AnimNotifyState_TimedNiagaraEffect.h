...
public:
	...
	// How/if we should send progress to the FX component
	UPROPERTY(EditAnywhere, Category = NotifyProgress)
	ENiagaraAnimNotifyProgressType NotifyProgressType = ENiagaraAnimNotifyProgressType::Forward;

	// Should we apply the animation rate scale when calculating the elasped time.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = NotifyProgress, meta = (DisplayName = "Notify Progress Apply Rate Scale"))
...
#if WITH_EDITORONLY_DATA
	UPROPERTY()
	bool bEnableNormalizedNotifyProgress_DEPRECATED = true;
#endif

...
void UAnimNotifyState_TimedNiagaraEffectAdvanced::PostLoad()
{
	Super::PostLoad();
	if (!bEnableNormalizedNotifyProgress_DEPRECATED)
	{
		NotifyProgressType = ENiagaraAnimNotifyProgressType::None;
	}
}
#endif
...
void UAnimNotifyState_TimedNiagaraEffectAdvanced::NotifyTick(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation, float FrameDeltaTime, const FAnimNotifyEventReference& EventReference)
{
	...
	if (UFXSystemComponent* FXComponent = GetSpawnedEffect(MeshComp))
	{
		...
		if (NotifyProgressType != ENiagaraAnimNotifyProgressType::None && !NotifyProgressUserParameter.IsNone())
		{
			float NotifyProgress = GetNotifyProgress(MeshComp);
			if (NotifyProgressType == ENiagaraAnimNotifyProgressType::Reverse)
			{

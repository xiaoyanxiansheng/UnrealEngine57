...
#include UE_INLINE_GENERATED_CPP_BY_NAME(NiagaraHlslTranslator)
...
FNiagaraTranslateResults TNiagaraHlslTranslator<GraphBridge>::Translate(const FNiagaraCompileOptions& InCompileOptions, const FHlslNiagaraTranslatorOptions& InTranslateOptions)
{
	...
	if (TranslateResults.bHLSLGenSucceeded)
	{
		...
		// Traverse all the used data interfaces and generate any used structs
		// This step is for template files that might use structs which need defining via the translator
		{
			...
			for (int32 i = 0; i < CompilationOutput.ScriptData.DataInterfaceInfo.Num(); i++)
			{
				...
				if (CDO && CDO->CanExecuteOnTarget(ENiagaraSimTarget::GPUComputeSim))
				{
					...
					{
						TArray<FNiagaraFunctionSignature> FunctionSigs;
						CDO->GetFunctionSignatures(FunctionSigs);
						for (const FNiagaraFunctionSignature& FunctionSig : FunctionSigs)
						{
							if (FunctionSig.bSupportsGPU == false)
							{
								continue;
							}

							for (const FNiagaraVariable& Input : FunctionSig.Inputs)
							{
								AddStructToDefinitionSet(Input.GetType());
							}
							for (const FNiagaraVariableBase& Output : FunctionSig.Outputs)
							{
								AddStructToDefinitionSet(Output.GetType());
							}
						}
					}
				}
...
int32 TNiagaraHlslTranslator<GraphBridge>::RegisterDataInterface(const FNiagaraVariable& Var, const UNiagaraDataInterface* DataInterface, bool bPlaceholder, bool bAddParameterMapRead)
{
	...
	//UE_LOG(LogNiagaraEditor, Log, TEXT("RegisterDataInterface %s %s %s '%s' %s"), *Var.GetName().ToString(), *Var.GetType().GetName(), *PathName,

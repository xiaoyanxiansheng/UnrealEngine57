...
#include UE_INLINE_GENERATED_CPP_BY_NAME(NiagaraComponentDetails)
...
void FNiagaraComponentDetails::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	static const FName ParamUtilitiesName = TEXT("Niagara Utilities");
	...
	IDetailCategoryBuilder& CustomCategory = DetailBuilder.EditCategory(ParamUtilitiesName, FText::GetEmpty(), ECategoryPriority::Important);
...
FReply FNiagaraComponentDetails::OnResetSelectedSystem()
{
	...
	for (int32 Idx = 0; Idx < SelectedObjects.Num(); ++Idx)
	{
		if (SelectedObjects[Idx].IsValid())
		{
			...
			{
				// Note: Must gather then reset as the array we are iterating will be modified with the ReregisterComponent call
...
FReply FNiagaraComponentDetails::OnCaptureSelectedSystem()
{
	...
	if (NiagaraComponentToUse)
	{
		...
		CaptureParameters.NumFrames = FMath::Max(1, NiagaraComponentToUse->SimCacheDebugNumFramesToCapture.Get(GetDefault<UNiagaraSettings>()->QuickSimCacheCaptureFrameCount));

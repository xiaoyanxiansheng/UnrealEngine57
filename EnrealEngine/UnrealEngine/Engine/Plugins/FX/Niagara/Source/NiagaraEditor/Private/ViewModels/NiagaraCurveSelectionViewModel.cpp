...
TSharedPtr<FNiagaraCurveSelectionTreeNode> UNiagaraCurveSelectionViewModel::CreateNodeForEmitter(TArray<TSharedRef<FNiagaraCurveSelectionTreeNode>> OldParentChildNodes, const FNiagaraEmitterHandle& EmitterHandle) const
{
	...
	// Early out for stateless emitters
	FVersionedNiagaraEmitterData* EmitterData = EmitterHandle.GetEmitterData();
	if (EmitterData == nullptr)
	{
		ensureMsgf(OldEmitterNode.IsValid(), TEXT("Old emitter node was valid and new one is not, this is not expected"));
		return NewEmitterNode;
	}

	UNiagaraStackEditorData& StackEditorData = CastChecked<UNiagaraEmitterEditorData>(EmitterData->GetEditorData())->GetStackEditorData();

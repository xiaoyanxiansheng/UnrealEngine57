...
TArray<TSharedPtr<FNiagaraAction_NewNode>> UEdGraphSchema_Niagara::GetGraphActions(const UEdGraph* CurrentGraph, const UEdGraphPin* FromPin, UEdGraph* OwnerOfTemporaries) const
{
	...
	auto AddScriptFunctionAction = [&NewActions, OwnerOfTemporaries](const TArray<FString>& Categories, const FAssetData& ScriptAsset)
	{
		...
		Action->PreviewMoviePath = FNiagaraEditorUtilities::Preview::GetPreviewMovieObjectPath(ScriptAsset);
...
void UEdGraphSchema_Niagara::DroppedAssetsOnGraph(const TArray<FAssetData>& Assets, const FVector2f& GraphPosition, UEdGraph* Graph) const
{
	...
	for (const FAssetData& Data : Assets)
	{
		...
		{
			if (!AssetReferenceFilter.IsValid() || AssetReferenceFilter->PassesFilter(Data))
			{
				Enums.Add(Enum);
			}
		}
...
void UEdGraphSchema_Niagara::GetAssetsGraphHoverMessage(const TArray<FAssetData>&Assets, const UEdGraph* HoverGraph, FString& OutTooltipText, bool& OutOkIcon) const
{
	...
	for (const FAssetData& AssetData : Assets)
	{
		...
		if (Enum)
		{
			...
			OutTooltipText = LOCTEXT("CreateStaticMeshSwitch", "Create a static switch using the selected enum").ToString();
			OutOkIcon = true;
			break;
		}
	}
...
FLinearColor UEdGraphSchema_Niagara::GetPinTypeColor(const FEdGraphPinType& PinType) const
{
	...
	{
		UClass* PinClass = Cast<UClass>(PinType.PinSubCategoryObject.Get());
		// Note: we use the interface color for UObjects as most Niagara work involves DIs not UObjects and we didn't want to confuse users
		return PinClass && PinClass->IsChildOf(UNiagaraDataInterface::StaticClass()) ? Settings->ObjectPinTypeColor : Settings->InterfacePinTypeColor;
	}
...
TSharedPtr<IAssetReferenceFilter> UEdGraphSchema_Niagara::MakeAssetReferenceFilter(const UEdGraph* Graph)
{
	if (Graph)
	{
		if (UNiagaraScript* Script = Graph->GetTypedOuter<UNiagaraScript>())
		{
			TArray<TSharedPtr<FNiagaraScriptViewModel>> ViewModels;
			TNiagaraViewModelManager<UNiagaraScript, FNiagaraScriptViewModel>::GetAllViewModelsForObject(Script, ViewModels);
			if (ViewModels.Num() == 1)
			{
				return ViewModels[0]->MakeAssetReferenceFilter();
			}
			else
			{
				ensureMsgf(false, TEXT("Missing or duplicate script view models detected.  Can not make IAssetReferenceFilter."));
			}
		}
	}

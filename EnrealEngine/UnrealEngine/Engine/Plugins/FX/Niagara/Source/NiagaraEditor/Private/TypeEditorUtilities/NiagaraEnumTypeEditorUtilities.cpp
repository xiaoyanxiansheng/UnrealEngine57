...
int32 FNiagaraEditorEnumTypeUtilities::VariableToSelectNumericValue(const FNiagaraVariable& InVariableValue) const
{
	if (ensureMsgf(InVariableValue.GetType().IsEnum() && InVariableValue.IsDataAllocated(),
		TEXT("InVariableValue must be an enum type and must have its data allocated to convert to a select value.")) == false)
	{
		return INDEX_NONE;
	}

	// We store enums by their value in FNiagaraVariables and parameter store, but the select value is the index.
	UEnum* Enum = InVariableValue.GetType().GetEnum();
	return Enum->GetIndexByValue(InVariableValue.GetValue<FNiagaraInt32>().Value);
}

FName FNiagaraEditorEnumTypeUtilities::GetDebugNameForSelectValue(const FNiagaraTypeDefinition& ValueType, int32 SelectValue) const
{
	UEnum* Enum = ValueType.GetEnum();
	FName Name = Enum->GetNameByIndex(SelectValue);
	FText DisplayName = Enum->GetDisplayNameTextByIndex(SelectValue);
	...
	return NameString.Equals(DisplayNameString)	? Name : *FString::Printf(TEXT("%s (%s)"), *DisplayNameString, *NameString);
}

...
public:
	...
	void GenerateUserParameterRows(IDetailChildrenBuilder& ChildrenBuilder, UHierarchyRoot& UserParameterHierarchyRoot);
	...
	/** Generates a category for a given hierarchy category. */
	void GenerateCategory(IDetailChildrenBuilder& ChildrenBuilder, const UHierarchyCategory& HierarchyCategory);

	void OnActiveSectionChanged(ECheckBoxState State, const UHierarchySection* NewSelection)
	{
		ActiveSection = NewSelection;
		OnRebuildChildren.Execute();
	}

	ECheckBoxState IsSectionActive(const UHierarchySection* Section) const
	{
...
protected:
	...
	const UHierarchySection* ActiveSection = nullptr;
	TArray<UHierarchySection*> AvailableSections;
...
{
public:
	FNiagaraUserParameterCategoryBuilder(const UHierarchyCategory& InNiagaraHierarchyCategory, FName InCustomBuilderRowName, FNiagaraUserParameterNodeBuilder& InNodeBuilder);

...
void UNiagaraStackEmitterPropertiesItem::RefreshIssues(TArray<FStackIssue>& NewIssues)
{
	...
	if (ActualEmitterData)
	{
		...
		if (ActualEmitterData->UsesInterpolatedSpawning() == false)
		{
			const FText InterpolatedSpawningDisabledInfoFix = LOCTEXT("InterpolatedSpawningDisabledInfoFix", "Run update script on spawn");
			FVersionedNiagaraEmitterWeakPtr WeakEmitter = GetEmitterViewModel()->GetEmitter().ToWeakPtr();

			NewIssues.Emplace(
				EStackIssueSeverity::Info,
				LOCTEXT("InterpolatedSpawningDisabledInfo", "Interpolated spawning disabled, particle update will not run on the first frame."),
				LOCTEXT("InterpolatedSpawningDisabledDetailed", "When interpolated spawning is disabled only the spawn script runs on the first frame."),
				GetStackEditorDataKey(),
				true,
				FStackIssueFix(
					InterpolatedSpawningDisabledInfoFix,
					FStackIssueFixDelegate::CreateLambda(
						[WeakEmitter, InterpolatedSpawningDisabledInfoFix]()
						{
							FVersionedNiagaraEmitter PinnedEmitter = WeakEmitter.ResolveWeakPtr();
							if (PinnedEmitter.Emitter)
							{
								FScopedTransaction ScopedTransaction(InterpolatedSpawningDisabledInfoFix);

								PinnedEmitter.Emitter->Modify();
								PinnedEmitter.GetEmitterData()->InterpolatedSpawnMode = ENiagaraInterpolatedSpawnMode::RunUpdateScript;
								UNiagaraSystem::RequestCompileForEmitter(PinnedEmitter);
							}
						}
					)

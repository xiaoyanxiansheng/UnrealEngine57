...
namespace SNiagaraDistributionEditorPrivate
{
	static bool GUseCompactEditor = false;
	static FAutoConsoleVariableRef CVarUseCompactEditor(
		TEXT("fx.Niagara.UI.Distribution.UseCompactEditor"),
		GUseCompactEditor,
		TEXT("When enabled we use the compact editor vs the curve editor.")
	);
}
...
private:
		...
		if (ChannelCount == 2)
		{
			return SNew(SBox)
				...
				[
					...
					.XDisplayName(this, &SNiagaraDistributionValueEditor::GetChannelDisplayName, 0)
					...
					.YDisplayName(this, &SNiagaraDistributionValueEditor::GetChannelDisplayName, 1)
		...
		if (ChannelCount == 3)
		{
			return SNew(SBox)
				...
				[
					...
					.ZDisplayName(this, &SNiagaraDistributionValueEditor::GetChannelDisplayName, 2)
		...
		if (ChannelCount == 4)
		{
			return SNew(SBox)
				...
				[
					...
					.WDisplayName(this, &SNiagaraDistributionValueEditor::GetChannelDisplayName, 3)
	...
	FText GetChannelDisplayName(int32 ChannelIndex) const
	{
		return DistributionAdapter->GetChannelDisplayName(ChannelIndex);
	}
...
public:
	...
	{
		for (const FRichCurveEditInfoConst& ConstCurve: ConstCurveInfos)
		{
			Curves.Add(ConstCurve);
		}
	}
...
TSharedRef<SWidget> SNiagaraDistributionEditor::ConstructContentForMode()
{
	...
	if (FNiagaraDistributionEditorUtilities::IsArray(Mode))
	{
		return SNew(SNiagaraDistributionArrayEditor, DistributionAdapter.ToSharedRef());
	}
	else if (FNiagaraDistributionEditorUtilities::IsBinding(Mode))
	{
	...
	else if (FNiagaraDistributionEditorUtilities::IsCurve(Mode))
	{
		...
		if (SNiagaraDistributionEditorPrivate::GUseCompactEditor)
		{
			...
			for (int32 ChannelIndex = 0; ChannelIndex < NumberOfChannels; ChannelIndex++)
			{
				...
				.Padding(0, 0, 0, 3)
				[
					SNew(SNiagaraDistributionCompactCurveEditor, DistributionAdapter.ToSharedRef(), ChannelIndex)
		...
		else
		{
			...
			.Padding(0, 0, 0, 3)
			[
				SNew(SNiagaraDistributionCurveEditor, DistributionAdapter.ToSharedRef())
			];
		}

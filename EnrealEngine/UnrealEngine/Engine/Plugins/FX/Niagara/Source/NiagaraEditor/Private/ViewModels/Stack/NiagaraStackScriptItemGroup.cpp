...
{
public:
	enum class ESpecificActionType
	{
		NewScratchModule,
		NewSetSpecificParameters,
		ModuleWizard,
		Undefined
	};
	
	struct FCreationArguments
	{
		FText DisplayName;
		...
		TOptional<ESpecificActionType> SpecificActionType;
		FNiagaraActionSourceData SourceData = FNiagaraActionSourceData(EScriptSource::Niagara, FText::FromString("Niagara"), true);
		bool bDisplaySource = true;
		
		bool bRenameParameterOnAdd = false;
	...
	{		
		...
		FCreationArguments CreationArguments;
		CreationArguments.ModuleAssetData = AssetData;
		CreationArguments.DisplayName = FNiagaraEditorUtilities::FormatScriptName(AssetData.AssetName, bIsInLibrary);
		CreationArguments.Categories = { Category.ToString() };
		CreationArguments.Description = FNiagaraEditorUtilities::FormatScriptDescription(AssetDescription, AssetData.GetSoftObjectPath(), bIsInLibrary);
		CreationArguments.Keywords = Keywords;
		CreationArguments.SourceData = FNiagaraActionSourceData(Source.Key, Source.Value, true);
		CreationArguments.FavoritesActionData = FavoritesActionData;
		CreationArguments.bIsInLibrary = bIsInLibrary;
		CreationArguments.PreviewMoviePath = FNiagaraEditorUtilities::Preview::GetPreviewMovieObjectPath(AssetData);
		return MakeShareable(new FScriptGroupAddAction(CreationArguments));
	}
	...
	static TSharedRef<FScriptGroupAddAction> CreateModuleActionFromScratchPadScript(UNiagaraScript* ModuleScript)
	{
		...
		TOptional<FSoftObjectPath> PreviewMoviePath = FNiagaraEditorUtilities::Preview::GetPreviewMovieObjectPath(FAssetData(ModuleScript));
		
		FNiagaraActionSourceData SourceData(EScriptSource::Niagara, FText::FromString("Scratch Pad"), true);

		FCreationArguments CreationArguments;
		CreationArguments.ModuleScript = ModuleScript;
		CreationArguments.DisplayName = DisplayName;
		CreationArguments.Categories = { Category.ToString() };
		CreationArguments.FavoritesActionData = FavoritesActionData;
		CreationArguments.PreviewMoviePath = PreviewMoviePath;
		CreationArguments.SourceData = SourceData;
		CreationArguments.bIsInLibrary = bIsInLibrary;
		CreationArguments.Description = Description;
		CreationArguments.Keywords = Keywords;
		return MakeShareable(new FScriptGroupAddAction(CreationArguments));
	}

	static TSharedRef<FScriptGroupAddAction> CreateSetExistingParameterModuleAction(FNiagaraVariable ParameterVariable)
	{
		...
		CreationArguments.Description = Description;
		CreationArguments.Categories = { Category.ToString() };
		CreationArguments.bIsInLibrary = true;
		CreationArguments.ModuleParameterVariable = ParameterVariable;
		CreationArguments.bRenameParameterOnAdd = false;
		CreationArguments.SpecificActionType = ESpecificActionType::NewSetSpecificParameters;
		return MakeShareable(new FScriptGroupAddAction(CreationArguments));
	}
	...
	static TSharedRef<FScriptGroupAddAction> CreateNewParameterModuleAction(FName NewParameterNamespace, FNiagaraTypeDefinition NewParameterType)
	{
		...
		CreationArguments.Categories = { Category.ToString() };
		CreationArguments.bIsInLibrary = true;
		CreationArguments.bRenameParameterOnAdd = true;
		CreationArguments.ModuleParameterVariable = NewParameter;
		return MakeShareable(new FScriptGroupAddAction(CreationArguments));
	}
	...
	static TSharedRef<FScriptGroupAddAction> CreateNewScratchModuleAction()
	{
		...
		CreationArguments.Description = Description;
		CreationArguments.bIsInLibrary = true;
		CreationArguments.SpecificActionType = ESpecificActionType::NewScratchModule;
		CreationArguments.FavoritesActionData = FavoritesActionData;
		return MakeShareable(new FScriptGroupAddAction(CreationArguments));
	}
	...
	static TSharedRef<FScriptGroupAddAction> CreateWizardAction(const FModuleWizardGenerator::FAction& Action)
	{
		...
		FCreationArguments CreationArguments;
		CreationArguments.DisplayName = Action.DisplayName;
		CreationArguments.Description = Action.Description;
		CreationArguments.Keywords = Action.Keywords;
		CreationArguments.bIsInLibrary = true;
		CreationArguments.ModuleWizardModel = Action.WizardModel;
		CreationArguments.SpecificActionType = ESpecificActionType::ModuleWizard;
		CreationArguments.FavoritesActionData = FavoritesActionData;
		
		TSharedRef<FScriptGroupAddAction> AddAction = MakeShareable(new FScriptGroupAddAction( CreationArguments));
		return AddAction;
	}
	...
	virtual TOptional<FSoftObjectPath> GetPreviewMoviePath() const override
	{
	...
	{
		return SpecificActionType.IsSet() && SpecificActionType.GetValue() == ESpecificActionType::NewScratchModule;
	}

	bool IsNewSetSpecificParametersModuleAction() const
	{
		return SpecificActionType.IsSet() && SpecificActionType.GetValue() == ESpecificActionType::NewSetSpecificParameters;
	}

	bool HasWizardWidget() const
	{
		return SpecificActionType.IsSet() && SpecificActionType.GetValue() == ESpecificActionType::ModuleWizard && ModuleWizardModel.IsValid();
	}
...
private:
	FScriptGroupAddAction(const FCreationArguments& CreationArguments)
		: DisplayName(CreationArguments.DisplayName)
		, Categories(CreationArguments.Categories)
		, Description(CreationArguments.Description)
		, Keywords(CreationArguments.Keywords)
		, FavoritesData(CreationArguments.FavoritesActionData)
		, PreviewMoviePath(CreationArguments.PreviewMoviePath)
		, bIsInLibrary(CreationArguments.bIsInLibrary)
		, ModuleParameterVariable(CreationArguments.ModuleParameterVariable)
		, ModuleAssetData(CreationArguments.ModuleAssetData)
		, ModuleScript(CreationArguments.ModuleScript)
		, ModuleWizardModel(CreationArguments.ModuleWizardModel)
		, SpecificActionType(CreationArguments.SpecificActionType)
		, SourceData(CreationArguments.SourceData)
		, bRenameParameterOnAdd(CreationArguments.bRenameParameterOnAdd)
		{
...
public:
	...
	virtual void ExecuteAddAction(TSharedRef<INiagaraStackItemGroupAddAction> AddAction, int32 TargetIndex) override
	{
		...
		else if (ScriptGroupAddAction->IsNewSetSpecificParametersModuleAction())
		{
			TOptional<const FNiagaraVariable> InitializeVariable;

			if(ScriptGroupAddAction->GetModuleParameterVariable().IsValid())
			{
				InitializeVariable = ScriptGroupAddAction->GetModuleParameterVariable();
			}
			
			NewModuleNode = AddParameterModule(InitializeVariable, ScriptGroupAddAction->GetRenameParameterOnAdd(), TargetIndex);
		}
...
private:
	...
	UNiagaraNodeFunctionCall* AddParameterModule(TOptional<const FNiagaraVariable>& ParameterVariable, bool bRenameParameterOnAdd, int32 TargetIndex)
	{
		TArray<FNiagaraVariable> Vars;
		TArray<FString> DefaultVals;

		if (ParameterVariable.IsSet() && ParameterVariable.GetValue().IsValid())
		{
			Vars.Add(ParameterVariable.GetValue());
			DefaultVals.Add(FNiagaraConstants::GetAttributeDefaultValue(ParameterVariable.GetValue()));
		}
...
TOptional<UNiagaraStackEntry::FDropRequestResponse> UNiagaraStackScriptItemGroup::DropParameterOnTarget(const UNiagaraStackEntry& TargetEntry, const FDropRequest& DropRequest)
{
	...
	int32 TargetIndex = GetTargetIndexForTargetEntry(this, TargetEntry, DropRequest.DropZone);
	TSharedRef<FScriptGroupAddAction> AddAction = FScriptGroupAddAction::CreateSetExistingParameterModuleAction(ParameterAction->GetParameter());
	AddUtilities->ExecuteAddAction(AddAction, TargetIndex);

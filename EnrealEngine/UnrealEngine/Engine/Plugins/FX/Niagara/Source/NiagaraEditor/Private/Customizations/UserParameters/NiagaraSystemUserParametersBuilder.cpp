...
void FNiagaraSystemUserParameterBuilder::GenerateRowForUserParameterInternal(IDetailChildrenBuilder& ChildrenBuilder, FNiagaraVariable UserParameter)
{
	...
	else if(UserParameter.IsUObject())
	{
		...
		[
			...
			.OwnerAssetDataArray({ FAssetData(SystemAsset) })
		];
...
{
	// We re-add the User namespace if necessary
	if(!UserParameter.IsInNameSpace(FNiagaraConstants::UserNamespace))
	{
		UserParameter.SetNamespacedName(FNiagaraConstants::UserNamespaceString, UserParameter.GetName());
	}
	...
	SystemViewModel.Pin()->DeleteParameter(UserParameter);
...
FText FNiagaraSystemUserParameterBuilder::GetParameterAccessLevelTooltip(FNiagaraVariable UserParameter) const
{
	...
	return Enum ? Enum->GetToolTipTextByIndex(static_cast<int32>(GetParameterAccessLevel(UserParameter))) : FText();
}

...
struct FNiagaraEditorSharedTexts
{
	static UE_API const FText DebugDrawUIActionBaseText;
};
...
namespace FNiagaraEditorUtilities
{
	...
	*/
	...
	struct FVariableAttributeBindingInfo
	{
		/** The actual binding that was found during traversal */
		FNiagaraVariableAttributeBinding* Binding = nullptr;
		/** The default binding as provided by the CDO. */
		FNiagaraVariableAttributeBinding* DefaultBinding = nullptr;
		ENiagaraRendererSourceDataMode SourceDataMode = ENiagaraRendererSourceDataMode::Particles;
		/** Immediate owning object, used to call Modify() */
		UObject* Owner = nullptr;
		/** The owning emitter, if it exists. */
		FVersionedNiagaraEmitter OwningEmitter;
	};

	/** Config struct to set what an action should be targeting. Can target the system, all emitters, or a specific emitter.
	 * If SpecificEmitterGuid is set, bIncludeEmitters should be true also.  */
	struct FNiagaraActionScopeConfig
	{
		FNiagaraActionScopeConfig() = default;
		FNiagaraActionScopeConfig(FGuid InSpecificEmitterGuid);
		
		bool bIncludeSystem = true;
		bool bIncludeEmitters = true;
		TOptional<FGuid> SpecificEmitterGuid;
	};
	
	TArray<FVariableAttributeBindingInfo> GetVariableAttributeBindingsForParameter(TSharedRef<FNiagaraSystemViewModel> SystemViewModel, FNiagaraVariable Parameter, const FNiagaraActionScopeConfig& Config);
	...
	/** Test to see id the global filter allows this class or not.   Classes that are not allowed can fail validation. */
	NIAGARAEDITOR_API bool IsClassVisibileInGlobalFilter(UClass* Class);
	...
	namespace UserParameters
	{
		...
		TArray<UNiagaraNodeParameterMapGet*> GetParameterMapGetNodesWithUserParameter(TSharedRef<FNiagaraSystemViewModel> SystemViewModel, FNiagaraVariable UserParameter, const FNiagaraActionScopeConfig& Config);

		struct FUserParameterBindingInfo
		{
			FNiagaraUserParameterBinding* Binding = nullptr;
			UObject* Owner = nullptr;
		};
		
		TArray<FUserParameterBindingInfo> GetUserParameterBindingsForUserParameter(TSharedRef<FNiagaraSystemViewModel> SystemViewModel, FNiagaraVariable UserParameter, const FNiagaraActionScopeConfig& Config);
		TArray<UNiagaraStackFunctionInput*> GetStackFunctionInputsWithLinkedParameter(TSharedRef<FNiagaraSystemViewModel> SystemViewModel, FNiagaraVariable Parameter, const FNiagaraActionScopeConfig& Config);
		...
		void ReplaceUserParameterReferences(TSharedRef<FNiagaraSystemViewModel> SystemViewModel, FNiagaraVariable OldUserParameter, FNiagaraVariable NewUserParameter, const FNiagaraActionScopeConfig& Config);
		void DeleteUserParameterReferences(TSharedRef<FNiagaraSystemViewModel> SystemViewModel, FNiagaraVariable UserParameterToDelete, const FNiagaraActionScopeConfig& Config);
	}

	namespace Preview
	{
		NIAGARAEDITOR_API TOptional<FSoftObjectPath> GetPreviewMovieObjectPath(const FAssetData& AssetData);
	}
	...
	namespace Tooltips
	{
		...
		{
			/** If this is true, PreviewMovieOwner has to be valid as well. If false, will not attempt to create a movie. */
			bool bTryCreatePreviewMovie = false;
			TSharedPtr<FNiagaraPreviewMovieViewModel> PreviewMovieOwner;
		};

		/** An all purpose function to create a tooltip with a text and an optional movie. */
		NIAGARAEDITOR_API TSharedRef<SToolTip> CreateTooltip(FText Description, TOptional<FSoftObjectPath> PreviewMoviePath = TOptional<FSoftObjectPath>(), const FCreateTooltipArguments& CreateTooltipArguments = FCreateTooltipArguments());
		/** Creates a tooltip for a niagara action. Supports preview movies. */
		NIAGARAEDITOR_API TSharedRef<SToolTip> CreateTooltipForNiagaraAction(TSharedRef<FNiagaraMenuAction_Generic> Action, const FCreateTooltipArguments& CreateTooltipArguments = FCreateTooltipArguments());
		/** Creates a tooltip for a script. Supports preview movies.*/
		NIAGARAEDITOR_API TSharedRef<SToolTip> CreateTooltipForScript(UNiagaraScript& Script, FGuid VersionGuid = FGuid(), const FCreateTooltipArguments& CreateTooltipArguments = FCreateTooltipArguments());
...
private:
	...
	friend inline uint32 GetTypeHash(const FEnumIndexPair& EnumIndexPair)
	{

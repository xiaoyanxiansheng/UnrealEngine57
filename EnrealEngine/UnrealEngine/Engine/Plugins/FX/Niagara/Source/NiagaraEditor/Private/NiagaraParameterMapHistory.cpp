	...
	if (FoundIdx == INDEX_NONE)
	{
		if (RegisterReadsAsVariables)
		{
			...
			if (const FGraph* Graph = GraphBridge::GetOwningGraph(GraphBridge::GetOwningNode(InPin)))
			{
				...
				{
					FNiagaraVariable BoundVar = FNiagaraVariable(Var.GetType(), VariableBinding.GetName());
					if (FNiagaraConstants::GetOldPositionTypeVariables().Contains(BoundVar))
					{
						...
						BoundVar.SetType(FNiagaraTypeDefinition::GetPositionDef());
					}

					FNiagaraVariable AliasedBoundVar = BoundVar;
					BoundVar = ResolveAliases(AliasedBoundVar);

					int32 FoundIdxBinding = AddVariableToHistory(History, BoundVar, AliasedBoundVar, nullptr);
...
bool FCompileConstantResolver::ResolveConstant(FNiagaraVariable& OutConstant) const
{
	...
	if (FVersionedNiagaraEmitterData* EmitterData = Emitter.GetEmitterData())
	{
		...
		{
			OutConstant.SetValue(EmitterData->UsesInterpolatedSpawning() ? FNiagaraBool(true) : FNiagaraBool(false));
			return true;
		}
...
uint32 FCompileConstantResolver::BuildTypeHash() const
{
	...
	if (Emitter.Emitter)
	{
		...
		if (FVersionedNiagaraEmitterData* EmitterData = Emitter.GetEmitterData())
		{
			HashUnion.EmitterSimTarget = static_cast<uint8>(EmitterData->SimTarget);
			HashUnion.bEmitterLocalSpace = EmitterData->bLocalSpace;
			HashUnion.bEmitterDeterminism = EmitterData->bDeterminism;
			HashUnion.bEmitterInterpolatedSpawn = EmitterData->UsesInterpolatedSpawning();
		}
	}
	...
	HashUnion.Usage = static_cast<uint8>(Usage);
	HashUnion.DebugState = static_cast<uint8>(DebugState);

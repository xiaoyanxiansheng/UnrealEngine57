...
struct FNiagaraScriptZoomLevelsContainer : public FZoomLevelsContainer
{
	...
	FNiagaraScriptZoomLevelsContainer()
	{
	...
	{
		return GetDefault<UNiagaraEditorSettings>()->GetScriptEditorZoomLimitHandling();
	}
...
TSharedRef<SGraphEditor> SNiagaraScriptGraph::ConstructGraphEditor()
{
			...
			[
				...
				[
					...
					[
						...
						.Style(FNiagaraEditorStyle::Get(), "NiagaraEditor.ScriptGraph.SearchBoxStyle")
	...
	Events.OnSpawnNodeByShortcutAtLocation = SGraphEditor::FOnSpawnNodeByShortcutAtLocation::CreateSP(this, &SNiagaraScriptGraph::OnSpawnGraphNodeByShortcut);
	Events.OnCreateActionMenuAtLocation = SGraphEditor::FOnCreateActionMenuAtLocation::CreateSP(this, &SNiagaraScriptGraph::OnCreateActionMenu);
	...
	CreatedGraphEditor->GetGraphPanel()->SetZoomLevelsContainer<FNiagaraScriptZoomLevelsContainer>();
...
FReply SNiagaraScriptGraph::OnSpawnGraphNodeByShortcut(FInputChord InChord, const FVector2f& InPosition)
{
		...
		if (Settings->GraphCreationShortcuts[i].Input.GetRelationship(InChord) == FInputChord::ERelationshipType::Same)
		{
			...
			for (int32 ActionIdx = 0; ActionIdx < Actions.Num(); ActionIdx++)
			{
				...
				if (bMatch)
				{
					...
					NiagaraAction->CreateNode(Graph, Pins, FDeprecateSlateVector2D(InPosition));
...
FActionMenuContent SNiagaraScriptGraph::OnCreateActionMenu(UEdGraph* Graph, const FVector2f& Position, const TArray<UEdGraphPin*>& DraggedPins, bool bAutoExpandActionMenu, SGraphEditor::FActionMenuClosed OnClosed)
{
...
void SNiagaraScriptGraph::OnCreateComment()
{
	...
	if (CommentAction.PerformAction(ViewModel->GetGraph(), nullptr, GraphEditor->GetPasteLocation2f(), false))
	{
		ViewModel->GetGraph()->NotifyGraphNeedsRecompile();
	}
}

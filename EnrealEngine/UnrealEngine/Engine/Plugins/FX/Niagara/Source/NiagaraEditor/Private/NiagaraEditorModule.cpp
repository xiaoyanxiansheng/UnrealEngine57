...
#include "Customizations/NiagaraEditorPreviewActorCustomization.h"
...
#include "Customizations/SimCache/NiagaraDynamicMeshSimCacheVisualizer.h"
...
static FAutoConsoleCommand LogFolderPermissionsCommand = FAutoConsoleCommand(
	TEXT("fx.Niagara.MigrateInternalTagsToUserAssetTags"),
	TEXT("Finds and resaves Niagara assets within the current Content Browser view (recursive) that were tagged with an internal Niagara Asset Tag (such as Template, Hidden, Deprecated etc.)"),
	FConsoleCommandWithArgsDelegate::CreateLambda(
		[](const TArray<FString>& Args)
		{
			...
			{
				UE_LOG(LogNiagaraEditor, Display, TEXT("Trying to find tagged Niagara assets under path %s"), *CurrentPath.GetInternalPathString());
				...
				Filter.PackagePaths.Add(CurrentPath.GetInternalPathName());
				Filter.bRecursivePaths = true;

PRAGMA_DISABLE_DEPRECATION_WARNINGS
				for(const FNiagaraAssetTagDefinition* Definition : INiagaraModule::Get().GetInternalAssetTagDefinitions())
				{
					Filter.TagsAndValues.Add(FName(Definition->GetGuidAsString()));
				}
PRAGMA_ENABLE_DEPRECATION_WARNINGS
				...
				TArray<UPackage*> PackagesToResave;
				Algo::Transform(OutAssetData, PackagesToResave, [](const FAssetData& AssetData)
				{
					return AssetData.GetPackage();
				});

				for(UPackage* Package : PackagesToResave)
				{
					Package->MarkPackageDirty();
				}

				if(PackagesToResave.Num() > 0)
				{
					UEditorLoadingAndSavingUtils::SavePackagesWithDialog(PackagesToResave, false);
				}
				else
				{
					UE_LOG(LogNiagaraEditor, Warning, TEXT("Didn't find any assets to resave under %s"), *CurrentPath.GetInternalPathString());
				}
			}
			else
			{
				UE_LOG(LogNiagaraEditor, Warning, TEXT("No valid path found"));	
			}
		}));
...
class FNiagaraEditorOnlyDataUtilities : public INiagaraEditorOnlyDataUtilities
{
	...
	virtual bool GetResetSimulationOnParameterChange() const override
	{
		const UNiagaraEditorSettings* NiagaraEditorSettings = GetDefault<UNiagaraEditorSettings>();
		return NiagaraEditorSettings->GetResetSimulationOnChange();
	}

	virtual void MoveUserAssetTagsToMetaData(UObject* NiagaraAsset) const override
	{
		check(NiagaraAsset->IsA<UNiagaraEmitter>() || NiagaraAsset->IsA<UNiagaraSystem>());

		TArray<FNiagaraAssetTagDefinitionReference>* References = nullptr;
		if(UNiagaraEmitter* Emitter = Cast<UNiagaraEmitter>(NiagaraAsset))
		{
			References = &Emitter->AssetTags_DEPRECATED;
		}
		else if(UNiagaraSystem* System = Cast<UNiagaraSystem>(NiagaraAsset))
		{
			References = &System->AssetTags_DEPRECATED;
		}

		if(References && References->Num() > 0)
		{
			TMap<FGuid, const FNiagaraAssetTagDefinition*> AllTags;
			FAssetRegistryModule& AssetRegistryModule = FModuleManager::Get().LoadModuleChecked<FAssetRegistryModule>("AssetRegistry");

			FARFilter Filter;
			Filter.ClassPaths.Add(UDEPRECATED_NiagaraAssetTagDefinitions::StaticClass()->GetClassPathName());

PRAGMA_DISABLE_DEPRECATION_WARNINGS
			for(const FNiagaraAssetTagDefinition* InternalTagDefinition : INiagaraModule::Get().GetInternalAssetTagDefinitions())
			{
				AllTags.Add(InternalTagDefinition->TagGuid, InternalTagDefinition);
			}
PRAGMA_ENABLE_DEPRECATION_WARNINGS
			...
			{
				UDEPRECATED_NiagaraAssetTagDefinitions* TagDefinitionsAsset = Cast<UDEPRECATED_NiagaraAssetTagDefinitions>(AssetTagDefinitionAsset.GetAsset());

				for(const FNiagaraAssetTagDefinition& TagDefinition : TagDefinitionsAsset->GetAssetTagDefinitions())
				{
					AllTags.Add(TagDefinition.TagGuid, &TagDefinition);
				}
			}

			TArray<const FNiagaraAssetTagDefinition*> FoundTagDefinitions;
			for(const FNiagaraAssetTagDefinitionReference& AssetTagDefinitionReference : *References)
			{
				FGuid TagGuid = AssetTagDefinitionReference.GetTagDefinitionReferenceGuid();
				if(AllTags.Contains(TagGuid))
				{
					FoundTagDefinitions.Add(AllTags[TagGuid]);
				}
			}

			for(const FNiagaraAssetTagDefinition* Definition : FoundTagDefinitions)
			{
				...
				UE::UserAssetTags::AddUserAssetTag(NiagaraAsset, FName(Definition->AssetTag.ToString()));
			}
...
{
	UEnum* NiagaraScriptUsageEnum = FindObjectChecked<UEnum>(nullptr, TEXT("/Script/Niagara.ENiagaraScriptUsage"), EFindObjectFlags::ExactClass);
...
void FNiagaraEditorModule::StartupModule()
{
	...
	InitializeTaggedAssetBrowserDetails();
	...
	OnModulesChangedHandle = FModuleManager::Get().OnModulesChanged().AddRaw(this, &FNiagaraEditorModule::OnModulesChanged);
	...
	PropertyModule.RegisterCustomClassLayout(
		FNiagaraEditorPreviewActorCustomization::GetCustomizationTypeName(),
		FOnGetDetailCustomizationInstance::CreateStatic(&FNiagaraEditorPreviewActorCustomization::MakeInstance));
	...
	RegisterDynamicCustomizations();

	// details customizations end.
	...
	RegisterPropertyUtilities(FNiagaraDistributionCurveFloat::StaticStruct(), MakeShared<FNiagaraDistributionPropertyEditorUtilities, ESPMode::ThreadSafe>());
	RegisterPropertyUtilities(FNiagaraDistributionCurveVector3::StaticStruct(), MakeShared<FNiagaraDistributionPropertyEditorUtilities, ESPMode::ThreadSafe>());
	...
	RegisterDataInterfaceCacheVisualizer(UNiagaraDataInterfaceDynamicMesh::StaticClass(), MakeShared<FNiagaraDynamicMeshSimCacheVisualizer>());
...
{
	FNiagaraActorPlacement::Unregister();
...
void FNiagaraEditorModule::OnPostEngineInit()
{
	if (GIsEditor)
	{
		FNiagaraActorPlacement::Register();
...
void FNiagaraEditorModule::RegisterSettings()
{
	...
	if (SettingsModule != nullptr)
	{
		...
		SequencerSettings->SetStopPlayingWhenJumpingToStartOrEnd(true);
...
void FNiagaraEditorModule::InitializeTaggedAssetBrowserDetails()
{	
	// Now we add information on how to display emitters & systems in the tagged asset browser
	// Emitters
	{
		FAssetDetailsDisplayInfo EmitterClassInfo;
		...
		EmitterClassInfo.GetDescriptionDelegate = FAssetDetailsDisplayInfo::FGetDescription::CreateLambda([](const FAssetData& AssetData)
		{
		...
		FTaggedAssetBrowserDetailsDisplayDatabase::RegisterClass(UNiagaraEmitter::StaticClass(), EmitterClassInfo);
	}
	
	// Systems
	{
		FAssetDetailsDisplayInfo SystemClassInfo;
		...
		SystemClassInfo.GetDescriptionDelegate = FAssetDetailsDisplayInfo::FGetDescription::CreateLambda([](const FAssetData& AssetData)
		{
			FName DescriptionTagName("TemplateAssetDescription");
			if(AssetData.FindTag(DescriptionTagName))
			{
		...
		FTaggedAssetBrowserDetailsDisplayDatabase::RegisterClass(UNiagaraSystem::StaticClass(), SystemClassInfo);
	}
}
...
void FNiagaraEditorModule::OnExecParticleInvoked(const TCHAR* Str)
{
	...
	{
		auto ReinitActorComponents =
			[](AActor* Actor)
			{
				TArray<UNiagaraComponent*, TInlineAllocator<1>> ComponentsToReset;
				for (UActorComponent* AC : Actor->GetComponents())
				{
					if (UNiagaraComponent* NiagaraComponent = Cast<UNiagaraComponent>(AC))
					{
						ComponentsToReset.Add(NiagaraComponent);
					}
				}

				for (UNiagaraComponent* NiagaraComponent : ComponentsToReset)
				{
...
void FNiagaraEditorModule::OnModulesChanged(FName Module, EModuleChangeReason Reason)
{	
	//Refresh any dynamic customizations for potentially newly loaded types.
...
void FNiagaraEditorModule::RegisterDynamicCustomizations()
{
	//Register any dynamic/discovered class customizations.
	FPropertyEditorModule& PropertyModule = FModuleManager::LoadModuleChecked<FPropertyEditorModule>("PropertyEditor");
	...
	for (TObjectIterator<UScriptStruct> It; It; ++It)
	{
		...
		//Ensure we don't register the struct again.
		//If the module is unloaded / reloaded we'll still have it's correct customizer in place.
		if(RegisteredDynamicStructs.Contains(Struct->GetFName()))
		{
			continue;
		}

		//Register customizations for Niagara Data Channel Access Context types.
		if (Struct->IsChildOf(FNDCAccessContextBase::StaticStruct()))
		{
			PropertyModule.RegisterCustomPropertyTypeLayout(
				Struct->GetFName(),
				FOnGetPropertyTypeCustomizationInstance::CreateStatic(&FNDCAccessContextDetailsCustomization::MakeInstance));

			RegisteredDynamicStructs.Add(Struct->GetFName());
		}
	}
}

void FNiagaraEditorModule::RegisterMenuExtensions()
{
	RegisterMenuExtension_MigrateToUserAssetTags();
}

void FNiagaraEditorModule::RegisterMenuExtension_MigrateToUserAssetTags()
{
	UToolMenus::RegisterStartupCallback(FSimpleMulticastDelegate::FDelegate::CreateLambda([]()
	{
		UToolMenu* MenuExtension = UToolMenus::Get()->ExtendMenu("ContentBrowser.AssetContextMenu.NiagaraAssetTagDefinitions");
		FToolMenuSection& Section = MenuExtension->FindOrAddSection("GetAssetActions");
		Section.AddDynamicEntry("MigrateToUserAssetTagsDynamic", FNewToolMenuSectionDelegate::CreateLambda([](FToolMenuSection& InSection)
			{
				...
				if (Context)
				{
					if(Context->SelectedAssets.Num() > 0)
					{
						InSection.AddMenuEntry(
							"MigrateToUserAssetTags",
							LOCTEXT("MigrateToUserAssetTags_Label", "Migrate Tagged Assets to User Asset Tags"),
							LOCTEXT("MigrateToUserAssetTags_Tooltip",
								"Migrate the assets that were tagged using tags from this Tag Definitions assets to the new User Asset Tag system."),
							...
							FExecuteAction::CreateLambda([InSelectedAssets = Context->SelectedAssets]()
								{
									...
									{
										UDEPRECATED_NiagaraAssetTagDefinitions* DefinitionsAsset = Cast<UDEPRECATED_NiagaraAssetTagDefinitions>(SelectedObject);
										for(const FNiagaraAssetTagDefinition& Definition : DefinitionsAsset->GetAssetTagDefinitions())
										{
											Filter.TagsAndValues.Add(FName(Definition.GetGuidAsString()));
										}
									}
									...
									{
										UE_LOG(LogNiagaraEditor, Warning, TEXT("Didn't find any assets to resave"));
									}
								}));
					}
				}
...
const FNiagaraEditorModule::FMoviePreviewObjects& FNiagaraEditorModule::FindOrAddMoviePreviewObjects(const FAssetData& AssetData)
{
	if(MoviePreviewObjects.Contains(AssetData))
	{
		return MoviePreviewObjects[AssetData];
	}
	...
	UObject* MoviePreviewObject = PreviewMoviePath.TryLoad();
	UFileMediaSource* MediaSource = Cast<UFileMediaSource>(MoviePreviewObject);
	
	FMoviePreviewObjects& PreviewObjects = MoviePreviewObjects.Add(AssetData);
	PreviewObjects.MediaPlayer.Reset(NewObject<UMediaPlayer>(GetTransientPackage(), UMediaPlayer::StaticClass()));
	PreviewObjects.MediaTexture.Reset(NewObject<UMediaTexture>(GetTransientPackage()));

	PreviewObjects.MediaPlayer->SetLooping(true);
	PreviewObjects.MediaPlayer->AffectedByPIEHandling = false;
	PreviewObjects.MediaPlayer->OpenSource(MediaSource);
	PreviewObjects.MediaTexture->SetMediaPlayer(PreviewObjects.MediaPlayer.Get());
	PreviewObjects.MediaTexture->UpdateResource();
	PreviewObjects.MediaPlayer->Play();
	PreviewObjects.ImageBrush = new FSlateImageBrush(PreviewObjects.MediaTexture.Get(), {80.f, 80.f});

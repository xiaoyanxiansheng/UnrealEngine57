...
void FNiagaraUserParameterNodeBuilder::GenerateUserParameterRows(IDetailChildrenBuilder& ChildrenBuilder, UHierarchyRoot& UserParameterHierarchyRoot)
{
	...
	{		
		if(const UHierarchyCategory* HierarchyCategory = Cast<UHierarchyCategory>(Child))
		{
			...
			if(const FDataHierarchyElementMetaData_SectionAssociation* SectionAssociation = HierarchyCategory->FindMetaDataOfType<FDataHierarchyElementMetaData_SectionAssociation>())
			{
			...
			if((ActiveSection == nullptr || Section == ActiveSection) && HierarchyCategory->DoesOneChildExist<UNiagaraHierarchyUserParameter>(true))
			{

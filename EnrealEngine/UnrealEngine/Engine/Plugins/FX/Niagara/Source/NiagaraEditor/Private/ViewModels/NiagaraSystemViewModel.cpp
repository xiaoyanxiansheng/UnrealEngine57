...
void FNiagaraSystemViewModel::Initialize(UNiagaraSystem& InSystem, FNiagaraSystemViewModelOptions InOptions)
{
	...
	PreviewMovieViewModel = MakeShared<FNiagaraPreviewMovieViewModel>();
	PreviewMovieViewModel->Initialize();
...
TSharedPtr<FNiagaraEmitterHandleViewModel> FNiagaraSystemViewModel::AddStatelessEmitter()
{
	...
	auto SetupStatelessEmitter =
		[](UNiagaraStatelessEmitter* StatelessEmitter, FName EmitterName)
		{
			...
			{
				StatelessEmitter->SetEmitterTemplate(Cast<UNiagaraStatelessEmitterTemplate>(TemplateClass->GetDefaultObject()));
			}
			{
...
void FNiagaraSystemViewModel::DuplicateEmitters(TArray<FEmitterHandleToDuplicate> EmitterHandlesToDuplicate)
{
	...
	for(auto& Element : NewHandleToOriginalSystemMap)
	{
		...
		TArray<FNiagaraVariable> ReferencedUserParameters = FNiagaraEditorUtilities::UserParameters::GetReferencedUserParametersFromEmitter(EmitterHandleViewModel->GetEmitterViewModel());
		...
		for(const FNiagaraVariable& UserParameter : ReferencedUserParameters)
		{
			...
			else if(TargetParameterStore.FindParameterVariable(UserParameter, true) != nullptr)
			{
				...
				if(!bParameterWasRenamed)
				{
					...
					FNiagaraEditorUtilities::FNiagaraActionScopeConfig Config(Element.Key);
					FNiagaraEditorUtilities::UserParameters::ReplaceUserParameterReferences(AsShared(), UserParameter, NewUserParameter, Config);
				...
				else
				{
					...
					FNiagaraEditorUtilities::FNiagaraActionScopeConfig Config(Element.Key);
					FNiagaraEditorUtilities::UserParameters::ReplaceUserParameterReferences(AsShared(), UserParameter, RenamedUserParameter, Config);
				}
			}
...
UNiagaraEditorParametersAdapter* FNiagaraSystemViewModel::GetEditorOnlyParametersAdapter() const
{
	...
	return EmitterData ? CastChecked<UNiagaraEditorParametersAdapter>(EmitterData->GetEditorParameters()) : nullptr;
}
...
bool FNiagaraSystemViewModel::RenameParameter(const FNiagaraVariable TargetParameter, const FName NewName, ENiagaraGetGraphParameterReferencesMode RenameScopeMode)
{
	...
	if (System->GetExposedParameters().IndexOf(TargetParameter) != INDEX_NONE)
	{
		...
		FNiagaraEditorUtilities::FNiagaraActionScopeConfig Config;
		Config.bIncludeSystem = true;
		Config.bIncludeEmitters = true;
		FNiagaraEditorUtilities::UserParameters::ReplaceUserParameterReferences(AsShared(), TargetParameter, NewVariable, Config);
...
bool FNiagaraSystemViewModel::DeleteParameter(const FNiagaraVariableBase Parameter)
{
	FScopedTransaction RenameParameterTransaction(LOCTEXT("DeleteParameter", "Delete parameter"));

	System->Modify();

	// Delete the parameter if it's in the parameter store for user parameters.
	if (System->GetExposedParameters().IndexOf(Parameter) != INDEX_NONE)
	{
		FNiagaraEditorUtilities::FNiagaraActionScopeConfig Config;
		Config.bIncludeSystem = true;
		Config.bIncludeEmitters = true;
		FNiagaraEditorUtilities::UserParameters::DeleteUserParameterReferences(AsShared(), Parameter, Config);
		
		System->GetExposedParameters().RemoveParameter(Parameter);
	}

	System->HandleVariableRemoved(Parameter, true);
...
TArray<UNiagaraGraph*> FNiagaraSystemViewModel::GetAllGraphs()
{
	...
	if (GetEditMode() == ENiagaraSystemViewModelEditMode::SystemAsset)
	{
		...
		for (const TSharedRef<FNiagaraEmitterHandleViewModel>& EmitterHandleViewModel : EmitterHandleViewModels)
		{
			...
			{
				if (UNiagaraGraph* Graph = Cast<UNiagaraScriptSource>(EmitterData->GraphSource)->NodeGraph)
				{
					OutGraphs.Add(Graph);
				}
			}
...
TArray<UNiagaraGraph*> FNiagaraSystemViewModel::GetSelectedGraphs()
{
	...
	{
		if (FVersionedNiagaraEmitterData* EmitterData = GetEmitterHandleViewModels()[0]->GetEmitterHandle()->GetEmitterData())
		{
			SelectedGraphs.Add(CastChecked<UNiagaraScriptSource>(EmitterData->GraphSource)->NodeGraph);
		}
	}
...
void FNiagaraSystemViewModel::RefreshEmitterHandleViewModels()
{
	...
	for (i = 0; i < GetSystem().GetNumEmitters(); ++i)
	{
		...
		else
		{
			...
			// These delegate bindings include the emitter handle id which may have changed since we're reusing existing view models, so remove the binding here.
			ViewModel->OnPropertyChanged().RemoveAll(this);
			ViewModel->GetEmitterViewModel()->OnScriptGraphChanged().RemoveAll(this);
		}

		// Add delegate bindings which require the emitter handle id passed as an additional parameter.

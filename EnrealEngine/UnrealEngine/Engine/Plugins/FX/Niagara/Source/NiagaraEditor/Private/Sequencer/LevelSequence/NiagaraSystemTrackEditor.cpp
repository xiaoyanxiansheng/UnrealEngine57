...
FText FNiagaraSystemTrackEditor::GetDisplayName() const
{
	return LOCTEXT("NiagaraSystemTrackEditor_DisplayName", "Niagara System");
}
	...
	if (ColumnName == FCommonOutlinerNames::Edit)
	{
		return SNew(SBox)
			...
			[
				...
				{
					const TArray<UMovieSceneSection*>& Sections = NiagaraSystemTrack->GetAllSections();
					UMovieSceneNiagaraSystemSpawnSection* Section = Sections.Num() == 1 ? Cast<UMovieSceneNiagaraSystemSpawnSection>(Sections[0]) : nullptr;
					return static_cast<int32>(Section ? Section->GetAgeUpdateMode() : ENiagaraAgeUpdateMode::TickDeltaTime);
				})
				.OnEnumSelectionChanged_Lambda([NiagaraSystemTrack](int32 NewSelection, ESelectInfo::Type)
				{
					...
					if (Section)
					{
						...
						Section->Modify();
						Section->SetAgeUpdateMode(static_cast<ENiagaraAgeUpdateMode>(NewSelection));
					}
				})

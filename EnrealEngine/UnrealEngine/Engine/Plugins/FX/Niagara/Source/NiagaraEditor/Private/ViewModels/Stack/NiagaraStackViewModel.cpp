...
void UNiagaraStackViewModel::Tick()
{
	if (RootEntry)
	{
		if (bRefreshPending)
		{
			...
			bRefreshPending = false;
			FNiagaraStackSearchConfig SearchConfig;
			SearchConfig.bScrollToResult = false;
			RestartSearch(SearchConfig);
...
void UNiagaraStackViewModel::SearchTick()
{
	...
	if (bRestartSearch)
	{
		...
		{
			SearchCompletedDelegate.Broadcast(CurrentSearchConfig.Get(FNiagaraStackSearchConfig()));
		}
		bRestartSearch = false;
	}
...
void UNiagaraStackViewModel::RestartSearch(const FNiagaraStackSearchConfig& InConfig)
{
	if(bRestartSearch == false)
	{
		CurrentSearchConfig = InConfig;
...
UNiagaraStackViewModel::FOnStackEntrySelectionRequested& UNiagaraStackViewModel::OnStackEntrySelectionRequested()
{
	return OnStackEntrySelectionRequestedDelegate;
}
...
void UNiagaraStackViewModel::SelectStackEntry(UNiagaraStackEntry* StackEntryToSelect, FNiagaraStackEntrySelectionConfig Config) const
{
	OnStackEntrySelectionRequestedDelegate.ExecuteIfBound(StackEntryToSelect, Config);
}

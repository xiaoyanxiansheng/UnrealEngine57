...
struct FNiagaraCurveEditorSharedSettings
{
	...
	UPROPERTY(Config)
	bool bCurveInputSnapEnabled = false;

	UPROPERTY(Config)
	bool bCurveOutputSnapEnabled = false;
};

inline uint32 GetTypeHash(const FNiagaraNamespaceMetadata& NamespaceMetaData)
{
...
public:
	...
	/** The configuration used in the Niagara Asset Wizard when creating an new Niagara Emitter or adding an emitter to a system. */
	UPROPERTY(config, EditAnywhere, DisplayName="Emitter Asset Wizard Configuration", Category = Niagara, meta=(AllowedClasses="/Script/UserAssetTagsEditor.TaggedAssetBrowserConfiguration"))
	FSoftObjectPath EmitterAssetWizardConfiguration;

	/** The configuration used in the Niagara Asset Wizard when creating a new Niagara System. */
	UPROPERTY(config, EditAnywhere, DisplayName="System Asset Wizard Configuration", Category = Niagara, meta=(AllowedClasses="/Script/UserAssetTagsEditor.TaggedAssetBrowserConfiguration"))
	FSoftObjectPath SystemAssetWizardConfiguration;
	...
	/** Whether we allow using previews in the editor. */
	UPROPERTY(Config, EditAnywhere, Category = "Preview")
	bool bDisplayPreviewMoviesInTooltips = true;

	/** The size of the movie previews used in tooltips. */
	UPROPERTY(Config, EditAnywhere, Category = "Preview", meta=(EditCondition="bDisplayPreviewMoviesInTooltips==true"))
	FVector2f TooltipPreviewMovieSize = FVector2f(128.f, 128.f);
	...
	/** Module Notes that were defined in a Module Script will expand by default, if true. Consider setting this to false if you are familiar with Niagara and don't need module notes. */
	UPROPERTY(config, EditAnywhere, Category = "Niagara Stack")
	bool bExpandModuleNotesByDefault = true;
	...
	bool GetForceSilentLoadingOfCachedAssets() const { return bForceSilentLoadingOfCachedAssets; }

	NIAGARAEDITOR_API EGraphZoomLimitHandling GetScriptEditorZoomLimitHandling() const;

	NIAGARAEDITOR_API bool IsCurveInputSnapEnabled() const;
	NIAGARAEDITOR_API bool IsCurveOutputSnapEnabled() const;
	NIAGARAEDITOR_API void SetCurveInputSnapEnabled(bool bEnabled);
	NIAGARAEDITOR_API void SetCurveOutputSnapEnabled(bool bEnabled);
...
private:
	...
	UPROPERTY(config)
	FNiagaraCurveEditorSharedSettings CurveEditorSharedSettings;

...
public:
	...
	virtual TSharedPtr<IAssetReferenceFilter> MakeAssetReferenceFilter() const { return {}; }
...
private:
	...
	/** Handles when values in the input parameter collection changes. */
	void InputParameterValueChanged(TConstArrayView<FName> ParameterName);

	/** Handles when values in the output parameter collection changes. */
	void OutputParameterValueChanged(TConstArrayView<FName> ParameterName);

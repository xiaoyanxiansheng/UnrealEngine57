...
void FNiagaraStackGraphUtilities::GetStackFunctionInputs(const UNiagaraNodeFunctionCall& FunctionCallNode, TArray<FNiagaraVariable>& OutInputVariables, TSet<FNiagaraVariable>& OutHiddenVariables, FCompileConstantResolver ConstantResolver, ENiagaraGetStackFunctionInputPinsOptions Options, bool bIgnoreDisabled)
{
	...
	if (GbNiagaraEnableTraversalCache == 0)
	{
...
void FNiagaraStackGraphUtilities::GetEmitterHandleAndCompiledScriptsForStackNode(const UNiagaraSystem& OwningSystem, UNiagaraNode& StackNode, const FNiagaraEmitterHandle*& OutEmitterHandle, TArray<const UNiagaraScript*>& OutCompiledScripts)
{
	...
	if (OutputNode != nullptr)
	{
		...
		{
			OutEmitterHandle = OwningSystem.GetEmitterHandles().FindByPredicate(
				[&StackNode](const FNiagaraEmitterHandle& EmitterHandle)
				{
					FVersionedNiagaraEmitterData* EmitterData = EmitterHandle.GetEmitterData();
					UNiagaraScriptSource* ScriptSource = EmitterData && EmitterData->GraphSource ? CastChecked<UNiagaraScriptSource>(EmitterData->GraphSource) : nullptr;
					return ScriptSource ? ScriptSource->NodeGraph == StackNode.GetNiagaraGraph() : false;
				}
			);
			...
			if (OutEmitterHandle != nullptr)
			{
				switch (OutputNode->GetUsage())
				{
					...
					case ENiagaraScriptUsage::ParticleSimulationStageScript:
					{
						if ( FVersionedNiagaraEmitterData* EmitterData = OutEmitterHandle->GetEmitterData() )
						{
							OutCompiledScripts.Add(EmitterData->GetScript(OutputNode->GetUsage(), OutputNode->GetUsageId()));
							if (EmitterData->SimTarget == ENiagaraSimTarget::GPUComputeSim)
							{
								OutCompiledScripts.Add(EmitterData->GetScript(ENiagaraScriptUsage::ParticleGPUComputeScript, FGuid()));
							}
						}
						break;
					}

...
bool FNiagaraVariableAttributeBindingCustomization::IsValid() const
{
	return TargetVariableBinding && WeakVersionedEmitter.IsValid();
}

FText FNiagaraVariableAttributeBindingCustomization::GetMissingText() const
{
...
{
	return IsValid() ? TargetVariableBinding->GetName() : NAME_None;
}

FText FNiagaraVariableAttributeBindingCustomization::GetCurrentText() const
{
	return IsValid() ? FText::FromName(TargetVariableBinding->GetName()) : GetMissingText();
}
...
FText FNiagaraVariableAttributeBindingCustomization::GetTooltipText() const
{
	if (IsValid())
	{
		FText TooltipDesc = FText::Format(LOCTEXT("AttributeBindingTooltip", "Use the variable \"{0}\" if it exists."), FText::FromName(TargetVariableBinding->GetName()));

		const FString DefaultValueStr = TargetVariableBinding->GetDefaultValueString();
		if (DefaultValueStr.Len() > 0)
		{
			TooltipDesc = FText::Format(LOCTEXT("AttributeBindingTooltipValue", "{0}.  Use the value \"{1}\" if the variable does not exist."), TooltipDesc, FText::FromString(DefaultValueStr));
		}

		return TooltipDesc;
	}
	return GetMissingText();
}
...
TArray<FName> FNiagaraVariableAttributeBindingCustomization::GetNames() const
{
	TArray<FName> Names;
	...
	const FVersionedNiagaraEmitterBase VersionedEmitter = WeakVersionedEmitter.ResolveWeakPtr();
	if (VersionedEmitter.IsValid() == false)
	{
		return Names;
	}

	TArray<UNiagaraGraph*> GraphsToTraverse;
	if ( UNiagaraEmitter* NiagaraEmitter = Cast<UNiagaraEmitter>(VersionedEmitter.Emitter) )
	{
		if (FVersionedNiagaraEmitterData* OwningEmitterData = NiagaraEmitter->GetEmitterData(VersionedEmitter.Version))
		{
	...
	if(UNiagaraSystem* System = VersionedEmitter.Emitter->GetTypedOuter<UNiagaraSystem>())
	{
	...
	UNiagaraRendererProperties* RendererProperties = WeakRendererProperties.Get();
	UNiagaraSimulationStageBase* SimulationStage = WeakSimulationStage.Get();
	...
	for (const FNiagaraVariableBase& Var : Vars)
	{
		...
		{
			if (RendererProperties->IsSupportedVariableForBinding(Var, *PropertyHandle->GetProperty()->GetName()))
			{
...
{
	TArray<FName> EventNames = GetNames();
...
void FNiagaraVariableAttributeBindingCustomization::ChangeSource(FName InVarName)
{
	...
	const FVersionedNiagaraEmitterBase VersionedEmitter(WeakVersionedEmitter);
	const UNiagaraRendererProperties* RendererProperties = WeakRendererProperties.Get();
	const ENiagaraRendererSourceDataMode BindingSourceMode = RendererProperties ? RendererProperties->GetCurrentSourceMode() : ENiagaraRendererSourceDataMode::Emitter;

	PropertyHandle->NotifyPreChange();
	TargetVariableBinding->SetValue(InVarName, VersionedEmitter, BindingSourceMode);
...
FReply FNiagaraVariableAttributeBindingCustomization::OnResetToDefaultsClicked()
{
	...
	PropertyHandle->NotifyPreChange();
	TargetVariableBinding->ResetToDefault(*DefaultVariableBinding, VersionedEmitter, BindingSourceMode);
	PropertyHandle->NotifyPostChange(EPropertyChangeType::ValueSet);
	PropertyHandle->NotifyFinishedChangingProperties();
	return FReply::Handled();
}
...
void FNiagaraVariableAttributeBindingCustomization::CustomizeHeader(TSharedRef<IPropertyHandle> InPropertyHandle, FDetailWidgetRow& HeaderRow, IPropertyTypeCustomizationUtils& CustomizationUtils)
{
	...
	if (Objects.Num() == 1)
	{
		FVersionedNiagaraEmitterBase VersionedEmitter;
		if ( UNiagaraRendererProperties* RenderProperties = Cast<UNiagaraRendererProperties>(Objects[0]) )
		{
			WeakRendererProperties = RenderProperties;
			VersionedEmitter = RenderProperties->GetOuterEmitterBase();
		}
		else if (UNiagaraSimulationStageBase* SimulationStage = Cast<UNiagaraSimulationStageBase>(Objects[0]))
		{
			WeakSimulationStage = SimulationStage;
			VersionedEmitter = SimulationStage->GetOuterEmitterBase();
		}

		WeakVersionedEmitter = FVersionedNiagaraEmitterBaseWeakPtr(VersionedEmitter);

		if (UNiagaraEmitterBase* EmitterBase = VersionedEmitter.Emitter)
		{
			UNiagaraSystem* System = EmitterBase->GetTypedOuter<UNiagaraSystem>();
			...
			if ( SystemViewModel.IsValid() )
			{
				...
				const FName EmitterBaseFName(EmitterBase->GetUniqueEmitterName());
				...
				{
					if ( EmitterHandleViewModel->GetName().IsEqual(EmitterBaseFName) )
					{
...
void FNDCAccessContextDetailsCustomization::CustomizeHeader(TSharedRef<IPropertyHandle> PropertyHandle, FDetailWidgetRow& HeaderRow, IPropertyTypeCustomizationUtils& CustomizationUtils)
{
	HeaderRow.NameContent()
		[
			PropertyHandle->CreatePropertyNameWidget()
...
void FNDCAccessContextDetailsCustomization::CustomizeChildren(TSharedRef<IPropertyHandle> PropertyHandle, IDetailChildrenBuilder& ChildBuilder, IPropertyTypeCustomizationUtils& CustomizationUtils)
{
	bool bShowInputContext = true;
	bool bShowOutputContext = false;

	//Walk up our parent chain until we get to a FNDCAccessContextInst and check it's meta data.
	TSharedPtr<IPropertyHandle> ParentPtr = PropertyHandle->GetParentHandle();
	while(ParentPtr.IsValid())
	{
		if (const FStructProperty* StructProp = CastField<FStructProperty>(ParentPtr->GetProperty()))
		{
			UScriptStruct* ParentStruct = StructProp->Struct;
			if (ParentStruct == FNDCAccessContextInst::StaticStruct())
			{
				bShowInputContext = PropertyHandle->HasMetaData(TEXT("ShowNDCAccessContextInput"));
				bShowOutputContext = PropertyHandle->HasMetaData(TEXT("ShowNDCAccessContextOutput"));

				if (!bShowInputContext && !bShowOutputContext)
				{
					//If neither are set then assume the user has neglected to add the meta tags so just show input only.
					bShowInputContext = true;
					bShowOutputContext = false;
				}
				break;
			}
		}

		ParentPtr = ParentPtr->GetParentHandle();
	}

	uint32 NumAccessContextChildren = 0;
	PropertyHandle->GetNumChildren(NumAccessContextChildren);

	for(uint32 ChildIdx = 0; ChildIdx < NumAccessContextChildren; ++ChildIdx)
	{
		...
		FProperty* ChildProperty = ChildPropertyHandle->GetProperty();
		if(!ChildProperty)
		{
			continue;
		}

		bool bIsInputContext = ChildPropertyHandle->HasMetaData(TEXT("NDCAccessContextInput")) && !ChildPropertyHandle->GetProperty()->HasAnyPropertyFlags(CPF_Transient);//Don't show transients in details panels.
		bool bIsOutputContext = ChildPropertyHandle->HasMetaData(TEXT("NDCAccessContextOutput"));
		if(!bIsInputContext && !bIsOutputContext)
		{
			//If neither are set then assume the user has neglected to add the meta tags so just show everything.
			//A cluttered details display is better than an empty one.
			bIsInputContext = true;
			bIsOutputContext = true;
		}

		if((bShowInputContext && bIsInputContext) || (bShowOutputContext && bIsOutputContext))
		{
...
{
	PropertyPtr = PropertyHandle.ToSharedPtr();
	...
	for (uint32 ChildIdx = 0; ChildIdx < NumChildren; ++ChildIdx)
	{
		...
		ChildPropertyHandle->SetOnPropertyValueChanged(FSimpleDelegate::CreateSP(this, &FNiagaraDataChannelReferenceDetailsCustomization::OnValueChanged));
	}
}

void FNiagaraDataChannelReferenceDetailsCustomization::OnValueChanged()
{
	void* DataPtr = nullptr;
	if (PropertyPtr.IsValid() && PropertyPtr->GetValueData(DataPtr) == FPropertyAccess::Success)
	{
		FNiagaraDataChannelReference* ValuePtr = static_cast<FNiagaraDataChannelReference*>(DataPtr);
		ValuePtr->OnValueChanged();
	}
}

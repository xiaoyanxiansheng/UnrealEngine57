...
namespace NiagaraDebuggerLocal
{
	...
	static void CreateConsoleVariableMenuEntry(FMenuBuilder& MenuBuilder, const FText& Label, const TCHAR* ConsoleVariableName)
	{
		...
		MenuBuilder.AddMenuEntry(
			Label,
			FText::Format(LOCTEXT("ConsoleVariableFormat", "{0}\nSee console variable '{1}'"), ConsoleVariable->GetDetailedHelp(), FText::FromString(ConsoleVariableName)),
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateLambda([ConsoleVariable]() { ConsoleVariable->Set(!ConsoleVariable->GetBool()); }),
				FCanExecuteAction(),
				FIsActionChecked::CreateLambda([ConsoleVariable]() { return ConsoleVariable->GetBool(); })
			),
			NAME_None,
...
namespace NiagaraOutlinerTab
{
	...
	static TSharedRef<SWidget> MakeOutlinerToolbar(TSharedPtr<FNiagaraDebugger>& Debugger)
	{
		...
		// Capture controls
		{
			...
			// Capture on key button
			{
				ToolbarBuilder.AddToolBarButton(
					FUIAction(FExecuteAction::CreateLambda([Debugger]()
						{
							Debugger->ToggleOutlinerCaptureOnKeyPress();
						}),
						FCanExecuteAction(),
						FIsActionChecked::CreateLambda([Debugger]() { return Debugger->IsCapturingOutlinerOnKeyPress(); })),
					NAME_None,
					LOCTEXT("CaptureOnKeyPress", "Capture (Press 'C')"),
					LOCTEXT("CaptureOnKeyPressTooltip", "Triggers a capture of outliner info from the connected session on a key press."),
					FSlateIcon(FNiagaraEditorStyle::Get().GetStyleSetName(), "NiagaraEditor.Debugger.Outliner.Capture"),
					EUserInterfaceActionType::ToggleButton
				);
			}
...
TSharedRef<SWidget> SNiagaraDebugger::MakeModeOptionsMenu()
{
	...
	}
	{
		...
		NiagaraDebuggerLocal::CreateConsoleVariableMenuEntry(MenuBuilder, LOCTEXT("NiagaraComponentsEnabled", "Niagara Components Enabled"), TEXT("fx.NiagaraComponentsEnabled"));

		MenuBuilder.AddSubMenu(
			LOCTEXT("RendererEnabled", "Renderer Enabled"),
			FText(),
			FNewMenuDelegate::CreateLambda(
				[](FMenuBuilder& SubMenuBuilder)
				{
					NiagaraDebuggerLocal::CreateConsoleVariableMenuEntry(SubMenuBuilder, LOCTEXT("LightRenderingEnabled", "Light Renderer Enabled"), TEXT("fx.EnableNiagaraLightRendering"));
					NiagaraDebuggerLocal::CreateConsoleVariableMenuEntry(SubMenuBuilder, LOCTEXT("MeshRenderingEnabled", "Mesh Renderer Enabled"), TEXT("fx.EnableNiagaraMeshRendering"));
					NiagaraDebuggerLocal::CreateConsoleVariableMenuEntry(SubMenuBuilder, LOCTEXT("RibbonRenderingEnabled", "Ribbon Renderer Enabled"), TEXT("fx.EnableNiagaraRibbonRendering"));
					NiagaraDebuggerLocal::CreateConsoleVariableMenuEntry(SubMenuBuilder, LOCTEXT("SpriteRendererEnabled", "Sprite Renderer Enabled"), TEXT("fx.EnableNiagaraSpriteRendering"));
				}
			)
		);
	}

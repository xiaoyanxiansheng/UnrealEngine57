...
public:
	...
	NIAGARAEDITOR_API virtual bool UsesCustomTooltipWidget() const;
	NIAGARAEDITOR_API virtual TSharedPtr<IToolTip> GetCustomTooltipWidget() const;
	...
	virtual void Copy(UNiagaraClipboardContent* ClipboardContent) const { RequestPulse(); }
	...
	virtual void Paste(const UNiagaraClipboardContent* ClipboardContent, FText& OutPasteWarning) { RequestPulse(); }

	void RequestPulse() const { OnPulseRequestedDelegate.ExecuteIfBound(); }
	FSimpleDelegate& OnPulseRequested() { return OnPulseRequestedDelegate; }
	...
	virtual bool SupportsStackNotes() const { return false; }
...
protected:
	...
	FSimpleDelegate OnPulseRequestedDelegate;
private:

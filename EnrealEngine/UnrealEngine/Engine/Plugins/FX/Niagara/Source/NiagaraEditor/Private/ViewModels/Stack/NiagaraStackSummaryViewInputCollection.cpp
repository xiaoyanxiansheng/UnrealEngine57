...
{
	TConstArrayView<const TObjectPtr<UHierarchySection>> HierarchySections = GetHierarchySections();
	...
	{
		if(const FDataHierarchyElementMetaData_SectionAssociation* SectionAssociation = TopLevelHierarchyCategories->FindMetaDataOfType<FDataHierarchyElementMetaData_SectionAssociation>())
		{
			if(SectionAssociation->Section.IsValid())
			{
				SectionToCategoryMap.FindOrAdd(SectionAssociation->Section.Get()).Add(TopLevelHierarchyCategories);	
			}
		}
	}
...
TConstArrayView<const TObjectPtr<UHierarchySection>> UNiagaraStackSummaryViewCollection::GetHierarchySections() const
{
	if (GetEmitterViewModel().IsValid())
	{

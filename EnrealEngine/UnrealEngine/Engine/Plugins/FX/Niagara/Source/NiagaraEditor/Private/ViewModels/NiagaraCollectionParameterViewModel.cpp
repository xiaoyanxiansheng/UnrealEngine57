...
FNiagaraCollectionParameterViewModel::FNiagaraCollectionParameterViewModel(const FNiagaraVariable& Variable, UNiagaraParameterCollectionInstance* CollectionInstance, ENiagaraParameterEditMode ParameterEditMode)
...
{
	return IsProvided() == ECheckBoxState::Checked && !CollectionInst->IsExternallyDriven(Parameter);
}
...
{
	return !CollectionInst->IsExternallyDriven(Parameter) && CollectionInst->IsDefaultInstance();
}

FText FNiagaraCollectionParameterViewModel::GetTooltip() const
{
	if (CollectionInst->IsExternallyDriven(Parameter))
	{
		return FText::Format(LOCTEXT("NPCExternallyDrivenTip", "\"{0}\" can not be edited as it is being driven from the parent MaterialParameterCollection"), GetNameText());
	}

	return FNiagaraParameterViewModel::GetTooltip();
}
...
void FNiagaraCollectionParameterViewModel::SetProvided(ECheckBoxState CheckboxState)
{
	...
	
	{
		...
		else
		{
			...
			CollectionInst->SyncWithCollectionParameters({ Parameter.GetName() });
		}
	}

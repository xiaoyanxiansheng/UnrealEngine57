...
void FNiagaraEditorUtilities::GetFilteredScriptAssets(FGetFilteredScriptAssetsOptions InFilter, TArray<FAssetData>& OutFilteredScriptAssets)
{
	...
	const UEnum* NiagaraScriptUsageEnum = FindObjectChecked<UEnum>(nullptr, TEXT("/Script/Niagara.ENiagaraScriptUsage"), EFindObjectFlags::ExactClass);
	...
	for (TObjectIterator<UNiagaraComponent> ComponentIt; ComponentIt; ++ComponentIt)
	{
		...
		if (Component && Component->GetAsset())
		{
			for (const auto& EmitterHandle : ReferencedSystemViewModel.GetSystem().GetEmitterHandles())
			{
				...
				if (EmitterData && Component->GetAsset()->UsesEmitter(EmitterData->GetParent()))
				{
					ReferencingComponents.Add(Component);
				}
			}
...
const FGuid FNiagaraEditorUtilities::AddEmitterToSystem(UNiagaraSystem& InSystem, UNiagaraEmitter& InEmitterToAdd, FGuid EmitterVersion, bool bCreateCopy)
{
	...
	{
		FVersionedNiagaraEmitterData* EmitterData = EmitterHandle.GetEmitterData();
		UNiagaraEmitterEditorData* EmitterEditorData = EmitterData ? Cast<UNiagaraEmitterEditorData>(EmitterData->GetEditorData()) : nullptr;
		if (EmitterEditorData)
		{
			EmitterEditorData->SetShowSummaryView(InEmitterToAdd.GetEmitterData(EmitterVersion)->AddEmitterDefaultViewState == ENiagaraEmitterDefaultSummaryState::Summary ? true : false);
		}
	}
...
{
	if(Config.SpecificEmitterGuid.IsSet())
	{
		ensureMsgf(Config.bIncludeEmitters, TEXT("If you are searching for a specific emitter, bIncludeEmitters needs to be true"));
	}
	
	TArray<UNiagaraNodeParameterMapGet*> Result;

	TArray<UEdGraph*> GraphsToInclude;
	if(Config.bIncludeSystem)
	{
		GraphsToInclude.Add(SystemViewModel->GetSystemScriptViewModel()->GetGraphViewModel()->GetGraph());
	}

	if(Config.bIncludeEmitters)
	{
		for(TSharedPtr<FNiagaraEmitterHandleViewModel> EmitterHandleViewModel : SystemViewModel->GetEmitterHandleViewModels())
		{
			if(Config.SpecificEmitterGuid.IsSet() && EmitterHandleViewModel->GetId() != Config.SpecificEmitterGuid.GetValue())
			{
				continue;
			}
			
			GraphsToInclude.Add(EmitterHandleViewModel->GetEmitterViewModel()->GetSharedScriptViewModel()->GetGraphViewModel()->GetGraph());
		}
	}

	for(UEdGraph* Graph : GraphsToInclude)
	{
		TArray<UNiagaraNodeParameterMapGet*> MapGetNodes;
		Graph->GetNodesOfClass<UNiagaraNodeParameterMapGet>(MapGetNodes);
...
TArray<FNiagaraEditorUtilities::UserParameters::FUserParameterBindingInfo> FNiagaraEditorUtilities::UserParameters::GetUserParameterBindingsForUserParameter(TSharedRef<FNiagaraSystemViewModel> SystemViewModel, FNiagaraVariable UserParameter, const FNiagaraActionScopeConfig& Config)
{
	if(Config.SpecificEmitterGuid.IsSet())
	{
		ensureMsgf(Config.bIncludeEmitters, TEXT("If you are searching for a specific emitter, bIncludeEmitters needs to be true"));
	...
	TArray<FUserParameterBindingInfo> Bindings;

	if(Config.bIncludeSystem)
	{
		TSharedRef<FNiagaraScriptGraphViewModel> GraphViewModel = SystemViewModel->GetSystemScriptViewModel()->GetGraphViewModel();
				...
				for(TFieldIterator<FStructProperty> It(Input->GetDataInterface()->GetClass()); It; ++It)
				{
					...
					{					
						...
						if(ParameterBinding->Parameter.IsValid())
						{
							...
							{
								Bindings.Emplace(ParameterBinding, Input->GetDataInterface());
							}
						}
					}
				}
	...
	if(Config.bIncludeEmitters)
	{
		for(TSharedPtr<FNiagaraEmitterHandleViewModel> EmitterHandleViewModel : SystemViewModel->GetEmitterHandleViewModels())
		{
			...
			//-TODO:Stateless
			if (FVersionedNiagaraEmitterData* EmitterData = EmitterHandleViewModel->GetEmitterViewModel()->GetEmitter().GetEmitterData())
			{
				const TArray<UNiagaraRendererProperties*>& Renderers = EmitterData->GetRenderers();
				for(UNiagaraRendererProperties* Renderer : Renderers)
				{
					for(TFieldIterator<FStructProperty> It(Renderer->GetClass()); It; ++It)
					{
						...
						{				
							...
							if(ParameterBinding->Parameter.IsValid())
							{
								...
								if(ReferencedParameter.IsUserHandle() && ParameterBinding->Parameter == UserParameter)
								{
									Bindings.Emplace(ParameterBinding, Renderer);
								}
							}
						}
					}
			...
			TSharedRef<FNiagaraScriptGraphViewModel> GraphViewModel = EmitterHandleViewModel->GetEmitterViewModel()->GetSharedScriptViewModel()->GetGraphViewModel();
...
TArray<UNiagaraStackFunctionInput*> FNiagaraEditorUtilities::UserParameters::GetStackFunctionInputsWithLinkedParameter(TSharedRef<FNiagaraSystemViewModel> SystemViewModel, FNiagaraVariable Parameter, const FNiagaraActionScopeConfig& Config)
{
	...
	if(Config.bIncludeSystem)
	{
		SystemViewModel->GetSystemStackViewModel()->GetRootEntry()->GetUnfilteredChildrenOfType(Result, true);
	}
	...
	if(Config.bIncludeEmitters)
	{
		...
		for(TSharedPtr<FNiagaraEmitterHandleViewModel> EmitterHandleViewModel : SystemViewModel->GetEmitterHandleViewModels())
		{
			...
			EmitterHandleViewModel->GetEmitterStackViewModel()->GetRootEntry()->GetUnfilteredChildrenOfType(Result, true);
		}
	}
...
FNiagaraEditorUtilities::FNiagaraActionScopeConfig::FNiagaraActionScopeConfig(FGuid InSpecificEmitterGuid)
{
	ensureMsgf(InSpecificEmitterGuid.IsValid(), TEXT("Invalid emitter ID passed into ctor"));
	
	bIncludeSystem = false;
	bIncludeEmitters = true;
	SpecificEmitterGuid = InSpecificEmitterGuid;
}

TArray<FNiagaraEditorUtilities::FVariableAttributeBindingInfo> FNiagaraEditorUtilities::GetVariableAttributeBindingsForParameter(TSharedRef<FNiagaraSystemViewModel> SystemViewModel, FNiagaraVariable Parameter, const FNiagaraActionScopeConfig& Config)
{
	if(Config.SpecificEmitterGuid.IsSet())
	{
		ensureMsgf(Config.bIncludeEmitters, TEXT("If you are searching for a specific emitter, bIncludeEmitters needs to be true"));
	...
	TArray<FVariableAttributeBindingInfo> Result;

	if(Config.bIncludeSystem)
	{
		// No attribute bindings in systems at this point
	}
	if(Config.bIncludeEmitters)
	{
		//-TODO:Stateless
				...
				for(UNiagaraRendererProperties* Renderer : Renderers)
				{
					for(TFieldIterator<FStructProperty> It(Renderer->GetClass()); It; ++It)
					{
						...
						{				
							...
							{
								FNiagaraVariableAttributeBinding* DefaultParameterBinding = (FNiagaraVariableAttributeBinding*) (*It)->ContainerPtrToValuePtr<void>(Renderer->GetClass()->GetDefaultObject(), 0);
								Result.Emplace(ParameterBinding, DefaultParameterBinding, Renderer->GetCurrentSourceMode(), Renderer, EmitterHandleViewModel->GetEmitterViewModel()->GetEmitter());
							}
						}
					}
				}
...
{
	using namespace FNiagaraEditorUtilities::UserParameters;
	
	for(const FUserParameterBindingInfo& UserParameterBindingInfo : GetUserParameterBindingsForUserParameter(SystemViewModel, OldUserParameter, Config))
	{
		if(UserParameterBindingInfo.Owner)
		{
			UserParameterBindingInfo.Owner->Modify();
		}
		UserParameterBindingInfo.Binding->Parameter = NewUserParameter;
	}

	for(const FVariableAttributeBindingInfo& AttributeBindingInfo : GetVariableAttributeBindingsForParameter(SystemViewModel, OldUserParameter, Config))
	{
		if(AttributeBindingInfo.Owner)
		{
			AttributeBindingInfo.Owner->Modify();
		}
		AttributeBindingInfo.Binding->SetValue(NewUserParameter.GetName(), AttributeBindingInfo.OwningEmitter.ToBase(), AttributeBindingInfo.SourceDataMode);
	}
	
	for(UNiagaraNodeParameterMapGet* MapGet : GetParameterMapGetNodesWithUserParameter(SystemViewModel, OldUserParameter, Config))
	{
		TArray<UEdGraphPin*> OutputPins;
...
void FNiagaraEditorUtilities::UserParameters::DeleteUserParameterReferences(TSharedRef<FNiagaraSystemViewModel> SystemViewModel, FNiagaraVariable UserParameterToDelete, const FNiagaraActionScopeConfig& Config)
{
	...
	for(const FUserParameterBindingInfo& UserParameterBindingInfo : GetUserParameterBindingsForUserParameter(SystemViewModel, UserParameterToDelete, Config))
	{
		if(UserParameterBindingInfo.Owner)
		{
			UserParameterBindingInfo.Owner->Modify();
		}
		UserParameterBindingInfo.Binding->Parameter = FNiagaraVariable(UserParameterBindingInfo.Binding->Parameter.GetType(), NAME_None);
	}

	for(const auto& AttributeBindingInfo : GetVariableAttributeBindingsForParameter(SystemViewModel, UserParameterToDelete, Config))
	{
		if(AttributeBindingInfo.Owner)
		{
			AttributeBindingInfo.Owner->Modify();
		...
		if(AttributeBindingInfo.OwningEmitter.Emitter != nullptr)
		{
			AttributeBindingInfo.Binding->ResetToDefault(*AttributeBindingInfo.DefaultBinding, AttributeBindingInfo.OwningEmitter.ToBase(), AttributeBindingInfo.SourceDataMode);
		}
	}

	for(UNiagaraStackFunctionInput* LinkedStackFunctionInput : GetStackFunctionInputsWithLinkedParameter(SystemViewModel, UserParameterToDelete, Config))
	{
		if(LinkedStackFunctionInput->GetValueMode() == UNiagaraStackFunctionInput::EValueMode::Linked && LinkedStackFunctionInput->GetLinkedParameterValue() == UserParameterToDelete)
		{
			LinkedStackFunctionInput->Reset();
				
			if(LinkedStackFunctionInput->GetInputType().IsDataInterface())
			{
				UNiagaraDataInterface* CurrentUserDataInterface = SystemViewModel->GetSystem().GetExposedParameters().GetDataInterface(UserParameterToDelete);
				if(CurrentUserDataInterface->GetClass() == LinkedStackFunctionInput->GetInputType().GetClass())
				{
					LinkedStackFunctionInput->SetDataInterfaceValue(CurrentUserDataInterface->GetClass());
				}
			}
			else if (LinkedStackFunctionInput->GetInputType().IsUObject() == false)
			{
				const UScriptStruct* LocalValueStruct = LinkedStackFunctionInput->GetInputType().GetScriptStruct();
				if (LocalValueStruct != nullptr)
				{
					TSharedRef<FStructOnScope> LocalValue = MakeShared<FStructOnScope>(LocalValueStruct);
						
					const uint8* Memory = SystemViewModel->GetSystem().GetExposedParameters().GetParameterData(UserParameterToDelete);
					if(Memory)
					{
						FMemory::Memcpy(LocalValue->GetStructMemory(), Memory, LocalValueStruct->GetStructureSize());
						LinkedStackFunctionInput->SetLocalValue(LocalValue);
					}
				}
			}
		}
...
void FNiagaraEditorUtilities::RunPythonUpgradeScripts(UNiagaraNodeFunctionCall* SourceNode,	const TArray<FVersionedNiagaraScriptData*>& UpgradeVersionData, const FNiagaraScriptVersionUpgradeContext& UpgradeContext, FString& OutWarnings)
{
	...
	for (int i = 1; i < UpgradeVersionData.Num(); i++)
	{
		...
		{
			UE_LOG(LogNiagaraEditor, Log, TEXT("Python is not available update script skipped"));
			AddStackWarning(PreviousData->Version, NewData->Version, "Python is not available, upgrade script ignored!", bLoggedWarning, OutWarnings);
			continue;
		}
...
void FNiagaraEditorUtilities::RunPythonUpgradeScripts(UUpgradeNiagaraEmitterContext* UpgradeContext)
{
	...
	for (int32 i = 1; i < UpgradeData.Num(); i++)
	{
		...
		{
			UE_LOG(LogNiagaraEditor, Log, TEXT("Python is not available update script skipped"));
			AddStackWarning(PreviousData->Version, NewData->Version, "Python is not available, upgrade script ignored!", bLoggedWarning, Warnings);
			continue;
		}
...
void FNiagaraEditorUtilities::SwitchParentEmitterVersion(TSharedRef<FNiagaraEmitterViewModel> EmitterViewModel, TSharedRef<FNiagaraSystemViewModel> SystemViewModel, const FGuid& NewVersionGuid)
{
	...
	ensureMsgf(EmitterData, TEXT("EmitterData missing when switching emitter version"));
	if (EmitterData == nullptr)
	{
		return;
	}

	FVersionedNiagaraEmitter Parent = EmitterData->GetParent();
...
bool FNiagaraEditorUtilities::IsClassVisibileInGlobalFilter(UClass* Class)
{
	...
	{
		return GlobalClassFilter->IsClassAllowed(FClassViewerInitializationOptions(), Class, ClassViewerModule->CreateFilterFuncs());
	}
	return true;
}
...
{
	FString PreviewMoviePathString;
	bool bHasMovieSequence = AssetData.GetTagValue("PreviewMoviePath", PreviewMoviePathString);
	FSoftObjectPath PreviewMoviePath(PreviewMoviePathString);

	if(bHasMovieSequence && PreviewMoviePath.IsAsset())
	{
		return PreviewMoviePath;
	}
...
{
	if(CreateTooltipArguments.bTryCreatePreviewMovie == true && CreateTooltipArguments.PreviewMovieOwner.IsValid() == false)
	{
		UE_LOGFMT(LogNiagaraEditor, Error, "Preview Movie Tooltip was requested but no owner was specified.");
	}
	
	SHorizontalBox::FSlot* MoviePreviewSlot;
	SHorizontalBox::FSlot* DescriptionSlot;
	
	TSharedRef<SHorizontalBox> TooltipContent = SNew(SHorizontalBox)
		+ SHorizontalBox::Slot().Expose(MoviePreviewSlot)
		.HAlign(HAlign_Center)
		.VAlign(VAlign_Center)
		.AutoWidth()
		+ SHorizontalBox::Slot().Expose(DescriptionSlot)
		.Padding(4.f, 2.f);

	TSharedPtr<SBox> PreviewMovieContainer = nullptr;
	if(CreateTooltipArguments.bTryCreatePreviewMovie && CreateTooltipArguments.PreviewMovieOwner.IsValid() && PreviewMoviePath.IsSet() && PreviewMoviePath->IsAsset())
	{
		PreviewMovieContainer = SNew(SBox)
			.Visibility_Lambda([]()
			{
				return GetDefault<UNiagaraEditorSettings>()->bDisplayPreviewMoviesInTooltips ? EVisibility::Visible : EVisibility::Collapsed;
			})
			.WidthOverride_Lambda([]()
			{
				return GetDefault<UNiagaraEditorSettings>()->TooltipPreviewMovieSize.X;
			})
			.HeightOverride_Lambda([]()
			{
				return GetDefault<UNiagaraEditorSettings>()->TooltipPreviewMovieSize.Y;
			})
			[
				SNew(SImage)
				// We only have one preview movie brush & one media player per previewmovieviewmodel.
				.Image(CreateTooltipArguments.PreviewMovieOwner->GetMoviePreviewImageBrush())
			];
	}

	if(PreviewMovieContainer.IsValid())
	{
		MoviePreviewSlot->AttachWidget(PreviewMovieContainer.ToSharedRef());
	}

	TSharedRef<STextBlock> DescriptionText = SNew(STextBlock)
		.Text(Description)
		.WrapTextAt(SToolTip::GetToolTipWrapWidth() - 500.f)
		.Font(FCoreStyle::Get().GetFontStyle("ToolTip.Font"))
		.ColorAndOpacity(FLinearColor::Black);
	
	DescriptionSlot->AttachWidget(DescriptionText);
...
{	
	return CreateTooltip(Action->ToolTip, Action->PreviewMoviePath, CreateTooltipArguments);
}
...
{
	FVersionedNiagaraScriptData* ScriptData = nullptr;
	if(VersionGuid.IsValid())
	{
		ScriptData = Script.GetScriptData(VersionGuid);
	}
	else
	{
		ScriptData = Script.GetLatestScriptData(); 
	}

	TOptional<FSoftObjectPath> PreviewMoviePath;
	if(CreateTooltipArguments.bTryCreatePreviewMovie)
	{
		PreviewMoviePath = FNiagaraEditorUtilities::Preview::GetPreviewMovieObjectPath(FAssetData(&Script));
	}

	return CreateTooltip(ScriptData->Description, PreviewMoviePath, CreateTooltipArguments);
}
...
TSharedRef<SWidget> FNiagaraEditorUtilities::HierarchyEditor::Scripts::GenerateRowContentForScriptParameterHierarchyEditor(TSharedRef<FHierarchyElementViewModel> HierarchyItem)
{
	...
	{
		if(ScriptParameter->GetScriptVariable() == nullptr)
		{
			return SNew(STextBlock).Text(LOCTEXT("InvalidHierarchyScriptParameter", "Invalid Script Hierarchy Parameter"));
		}

...
public:
	...
	{
		UE_API FNiagaraAvailableParameterInfo(FNiagaraVariableBase InVariableBase);
		UE_API FNiagaraAvailableParameterInfo(FNiagaraVariableBase InVariableBase, UNiagaraScript* InConversionScript);
...
public:
	...
	struct FGetAvailableParameterArgs
	{
	...
	/** Sets this input's data interface value by creating a new default Data Interface. */
	NIAGARAEDITOR_API void SetDataInterfaceValue(TSubclassOf<UNiagaraDataInterface> InDataInterfaceType);
...
inline uint32 GetTypeHash(const UNiagaraStackFunctionInput::FNiagaraAvailableParameterInfo& ParameterInfo)
{

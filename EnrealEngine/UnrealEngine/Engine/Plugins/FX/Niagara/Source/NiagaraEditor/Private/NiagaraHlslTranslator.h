...
public:
	...
	ENiagaraInterpolatedSpawnMode InterpolatedSpawnMode = ENiagaraInterpolatedSpawnMode::NoInterpolation;
...
protected:
	...
	int32 AddBodyChunk(FStringView SymbolName, FStringView Definition, const FNiagaraTypeDefinition& Type, TArrayView<int32> SourceChunks, bool bDecl = true, bool bIsTerminated = true);
	int32 AddBodyChunk(FStringView SymbolName, FStringView Definition, const FNiagaraTypeDefinition& Type, int32 SourceChunk, bool bDecl = true, bool bIsTerminated = true);
	int32 AddBodyChunk(FStringView SymbolName, FStringView Definition, const FNiagaraTypeDefinition& Type, bool bDecl = true, bool bIsTerminated = true);
	int32 AddBodyComment(FStringView Comment);
	int32 AddBodyChunk(FStringView Definition);
...
public:
	...
	static NIAGARAEDITOR_API FString GenerateFunctionParametersHlsl(const FNiagaraFunctionSignature& FunctionSignature);
...
protected:
	...
	void AppendSanitizedSymbolString(FStringBuilderBase& Builder, FStringView SymbolName, bool bCollapseNamespaces = false) const;
	FString BuildSanitizedSymbolString(FStringView SymbolName, bool bCollapseNamespaces = false) const;
	FString BuildSanitizedSymbolName(FName SymbolName, bool bCollapseNamespaces = false) const;

	void AppendFunctionSignatureString(const FNiagaraFunctionSignature& Sig, FStringBuilderBase& Builder) const;
	FString BuildFunctionSignatureString(const FNiagaraFunctionSignature& Sig) const;

	void AppendFunctionHlslPrototypeString(FStringView InVariableName, const FNiagaraFunctionSignature& FunctionSignature, FStringBuilderBase& Builder) const;
	FString BuildFunctionHlslPrototypeString(FStringView InVariableName, const FNiagaraFunctionSignature& FunctionSignature) const;

	template<typename T>
	void BuildConstantBuffer(ENiagaraCodeChunkMode ChunkMode);

	virtual FNiagaraEmitterID GetEmitterID() const = 0;
	virtual FStringView GetEmitterUniqueName() const = 0;
	...
	// Map of the output variable to the found aliased variable, used to discover the default binding, etc
	TMap<FNiagaraVariableBase, FNiagaraVariableBase> UniqueVarToAliasedVar;
	...
	// local copy of UNiagaraEditorSettings::GetHLSLKeywordReplacmentsMap() for the duration of the translation process
	TMap<FString, FString> KeywordReplacementMap;
};
...
protected:
	...
	virtual FStringView GetEmitterUniqueName() const override;

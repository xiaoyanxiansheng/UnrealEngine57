...
struct FNiagaraStackSearchConfig
{
	bool bScrollToResult = true;
};

struct FNiagaraStackEntrySelectionConfig
{
	bool bScrollToAndExpand = true;
	bool bSelectRow = false;
	bool bPulse = false;
};
...
public:
	...
	DECLARE_DELEGATE_TwoParams(FOnStackEntrySelectionRequested, UNiagaraStackEntry* StackEntry, FNiagaraStackEntrySelectionConfig SelectionConfig);
	...
	DECLARE_MULTICAST_DELEGATE_OneParam(FOnSearchCompleted, const FNiagaraStackSearchConfig& SearchConfig);
...
public:
	...
	NIAGARAEDITOR_API FOnStackEntrySelectionRequested& OnStackEntrySelectionRequested();
	...
	NIAGARAEDITOR_API void SelectStackEntry(UNiagaraStackEntry* StackEntryToSelect, FNiagaraStackEntrySelectionConfig Config) const;
...
private:
	...
	NIAGARAEDITOR_API void RestartSearch(const FNiagaraStackSearchConfig& InConfig);
...
private:
	...
	FOnStackEntrySelectionRequested OnStackEntrySelectionRequestedDelegate;

	// ~Search stuff
	TOptional<FNiagaraStackSearchConfig> CurrentSearchConfig;

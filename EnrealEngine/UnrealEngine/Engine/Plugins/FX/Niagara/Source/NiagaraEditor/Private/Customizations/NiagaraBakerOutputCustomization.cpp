...
TSharedRef<IDetailCustomization> FNiagaraBakerOutputStaticMeshDetails::MakeInstance()
{
	return MakeShared<FNiagaraBakerOutputStaticMeshDetails>();
}

FText FNiagaraBakerOutputStaticMeshDetails::BrowseAssetToolTipText(TWeakObjectPtr<UNiagaraBakerOutputStaticMesh> WeakOutput)
{
	UNiagaraBakerOutputStaticMesh* Output = WeakOutput.Get();
	return FText::Format(LOCTEXT("BrowseAssetToolTipFormat", "Browse to asset '{0}'"), FText::FromString(Output ? Output->GetAssetPath(Output->FramesAssetPathFormat, 0) : FString()));
}

FReply FNiagaraBakerOutputStaticMeshDetails::BrowseToAsset(TWeakObjectPtr<UNiagaraBakerOutputStaticMesh> WeakOutput)
{
	if (UNiagaraBakerOutputStaticMesh* Output = WeakOutput.Get())
	{
...
void FNiagaraBakerOutputStaticMeshDetails::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	// We only support customization on 1 object
	TArray<TWeakObjectPtr<UObject>> ObjectsCustomized;
	DetailBuilder.GetObjectsBeingCustomized(ObjectsCustomized);
	if (ObjectsCustomized.Num() != 1 || !ObjectsCustomized[0]->IsA<UNiagaraBakerOutputStaticMesh>())
	{
		return;
	}
	TWeakObjectPtr<UNiagaraBakerOutputStaticMesh> WeakOutput = CastChecked<UNiagaraBakerOutputStaticMesh>(ObjectsCustomized[0]);
	...
	{
		const FName PropertyName = PropertyHandle->GetProperty() ? PropertyHandle->GetProperty()->GetFName() : NAME_None;
		if (PropertyName == GET_MEMBER_NAME_CHECKED(UNiagaraBakerOutputStaticMesh, FramesAssetPathFormat))
		{
			BuildAssetPathWidget(
				DetailCategory, PropertyHandle,
				TAttribute<FText>::FGetter::CreateStatic(&FNiagaraBakerOutputStaticMeshDetails::BrowseAssetToolTipText, WeakOutput),
				FOnClicked::CreateStatic(&FNiagaraBakerOutputStaticMeshDetails::BrowseToAsset, WeakOutput)
			);
		}
		else
		{

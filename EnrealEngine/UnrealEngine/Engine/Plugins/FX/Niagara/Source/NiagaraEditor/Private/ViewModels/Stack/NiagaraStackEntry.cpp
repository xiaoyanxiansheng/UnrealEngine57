...
bool UNiagaraStackEntry::UsesCustomTooltipWidget() const
{
	return false;
}

TSharedPtr<IToolTip> UNiagaraStackEntry::GetCustomTooltipWidget() const
{
	return nullptr;
}
...
bool UNiagaraStackEntry::HasBaseEmitter() const
{
	...
	{
		const UNiagaraEmitter* BaseEmitter = GetEmitterViewModel().IsValid() ? GetEmitterViewModel()->GetParentEmitter().Emitter.Get() : nullptr;

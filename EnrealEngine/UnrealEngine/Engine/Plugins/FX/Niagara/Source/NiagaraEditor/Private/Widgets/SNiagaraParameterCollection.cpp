...
//#define DEBUG_SORT_ORDER
	...
	TSharedRef<SWidget> ParameterDeleteButton = SNew(SButton)
		.OnClicked(this, &SNiagaraParameterCollection::DeleteParameter, Item)
		.Visibility(this, &SNiagaraParameterCollection::ShowParameterDeleteButton, Item)
		.Content()
		[
			SNew(SImage)
...
EVisibility SNiagaraParameterCollection::ShowParameterDeleteButton(TSharedRef<INiagaraParameterViewModel> Item) const
{
	// if we can add, we can also remove, unless it's an externally provided parameter
	if (Item->IsEditingEnabled())
	{

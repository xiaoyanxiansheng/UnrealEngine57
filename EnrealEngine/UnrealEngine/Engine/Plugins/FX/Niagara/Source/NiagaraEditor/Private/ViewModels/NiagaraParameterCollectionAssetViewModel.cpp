	...
	, Instance(InCollection->GetDefaultInstance())
{
	...
	ExternalChangeHandle = Collection->OnCollectionChangedDelegate.AddRaw(this, &FNiagaraParameterCollectionAssetViewModel::OnCollectionChangedExternally);
	ParameterValuesChangedHandle = Instance->ParameterValuesChanged().AddRaw(this, &FNiagaraParameterCollectionAssetViewModel::RefreshParameterViewModels);
...
FNiagaraParameterCollectionAssetViewModel::~FNiagaraParameterCollectionAssetViewModel()
{
	...
	if (Collection)
	{
		Collection->OnCollectionChangedDelegate.Remove(ExternalChangeHandle);
	}

	Instance->ParameterValuesChanged().Remove(ParameterValuesChangedHandle);
}
...
TSharedPtr<FNiagaraCollectionParameterViewModel> FNiagaraParameterCollectionAssetViewModel::CreateParameterViewModel(const FNiagaraVariable& Var)
{
...
void FNiagaraParameterCollectionAssetViewModel::RefreshParameterViewModels()
{
	...
	for (int32 i = 0; i < Collection->GetParameters().Num(); ++i)
	{
		...
		ParameterViewModels.Add(CreateParameterViewModel(Var).ToSharedRef());
	}

	OnCollectionChangedDelegate.Broadcast();
}

void FNiagaraParameterCollectionAssetViewModel::RefreshParameterViewModels(TConstArrayView<FName> ChangedParameters)
{
	bool bChangeMade = false;

	for (TSharedRef<INiagaraParameterViewModel>& ParameterViewModel : ParameterViewModels)
	{
		FNiagaraVariable Var = ParameterViewModel->GetVariable();
		if (ChangedParameters.Contains(Var.GetName()))
		{
			if (FNiagaraCollectionParameterViewModel* CastParameterViewModel = (FNiagaraCollectionParameterViewModel*)(&ParameterViewModel.Get()))
			{
				CastParameterViewModel->Reset();
			}

			ParameterViewModel = CreateParameterViewModel(Var).ToSharedRef();
...
void FNiagaraParameterCollectionAssetViewModel::OnParameterValueChangedInternal(TSharedRef<FNiagaraCollectionParameterViewModel> ChangedParameter)
{
	...
	if (Collection->GetDefaultInstance() == Instance)
	{
		Collection->OnParameterValuesEdited.Broadcast({ ChangedParameter->GetVariable().GetName() });
	}

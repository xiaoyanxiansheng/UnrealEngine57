...
bool UNiagaraStackModuleItem::UsesCustomTooltipWidget() const
{
	return GetDefault<UNiagaraEditorSettings>()->bDisplayPreviewMoviesInTooltips && FunctionCallNode != nullptr && FunctionCallNode->FunctionScript != nullptr;
}

TSharedPtr<IToolTip> UNiagaraStackModuleItem::GetCustomTooltipWidget() const
{
	using namespace FNiagaraEditorUtilities::Tooltips;
	if(FunctionCallNode != nullptr && FunctionCallNode->FunctionScript != nullptr)
	{
		if (TSharedPtr<FNiagaraSystemViewModel> SystemViewModelPtr = UNiagaraStackEntry::GetSystemViewModelPtr())
		{
			FAssetData AssetData(FunctionCallNode->FunctionScript);
			if (GetDefault<UNiagaraEditorSettings>()->bDisplayPreviewMoviesInTooltips && SystemViewModelPtr->GetPreviewMovieViewModel()->IsPlayingPreviewMovieForAsset(AssetData) == false)
			{
				SystemViewModelPtr->GetPreviewMovieViewModel()->PlayMovieForAsset(AssetData);
			}
			...
			if (TooltipCache.IsValid() == false)
			{
				...
				CreateTooltipArguments.PreviewMovieOwner = GetSystemViewModel()->GetPreviewMovieViewModel();
				TooltipCache = CreateTooltipForScript(*FunctionCallNode->FunctionScript, FunctionCallNode->SelectedScriptVersion, CreateTooltipArguments);
			}
	...
	if (ScriptData != nullptr)
	{
		...
		if (!ScriptData->NoteMessage.IsEmptyOrWhitespace())
		{
			FStackIssue NoteIssue = FStackIssue(
				...
				GetStackEditorDataKey(),
				true);
			NoteIssue.SetIsExpandedByDefault(GetDefault<UNiagaraEditorSettings>()->bExpandModuleNotesByDefault);

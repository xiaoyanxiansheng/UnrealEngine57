...
void UNiagaraEmitterEditorData::PostLoad_TransferSummaryDataToNewFormat()
{
	...
	if(SummarySections_DEPRECATED.Num() > 0)
	{
		for(FNiagaraStackSection& StackSection : SummarySections_DEPRECATED)
		{
			...
			UHierarchySection* Section = NewObject<UHierarchySection>(SummaryViewRoot, UHierarchySection::StaticClass());
			Section->SetSectionNameAsText(SectionDisplayName);
			...
			SummaryViewRoot->GetSectionDataMutable().Insert(Section, 0);
...
TConstArrayView<const TObjectPtr<UHierarchySection>> UNiagaraEmitterEditorData::GetSummarySections() const
{

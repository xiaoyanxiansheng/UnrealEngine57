...
void FNiagaraStaticSwitchNodeDetails::DefaultIntValueCommitted(int32 Value, ETextCommit::Type CommitInfo)
{
	...
	if (ScriptVar)
	{
		const FScopedTransaction Transaction(LOCTEXT("SetStaticSwitchIntValue ", "Set Static Switch Int Value"));
		ScriptVar->Modify();
...
void FNiagaraStaticSwitchNodeDetails::DefaultBoolValueCommitted(ECheckBoxState NewState)
{
	...
	if (ScriptVar)
	{
		const FScopedTransaction Transaction(LOCTEXT("SetStaticSwitchBoolValue ", "Set Static Switch Bool Value"));
		ScriptVar->Modify();
...
void FNiagaraStaticSwitchNodeDetails::ExposeAsPinCommitted(ECheckBoxState NewState)
{
	if (Node.IsValid())
	{
		const FScopedTransaction Transaction(LOCTEXT("SetStaticSwitchExposed", "Set Static Switch Exposed"));
		Node->Modify();
...
void FNiagaraStaticSwitchNodeDetails::OnSelectionChanged(TSharedPtr<DefaultEnumOption> NewValue, ESelectInfo::Type)
{
	...
	const FScopedTransaction Transaction(LOCTEXT("SetStaticSwitchDefaultValue", "Set Static Switch Default Value"));
	ScriptVar->Modify();
...
void FNiagaraStaticSwitchNodeDetails::OnSelectionChanged(TSharedPtr<SwitchDropdownOption> NewValue, ESelectInfo::Type)
{
	...
	const FScopedTransaction Transaction(LOCTEXT("SetStaticSwitchType", "Set Static Switch Type"));
	Node->Modify();
...
void FNiagaraStaticSwitchNodeDetails::OnSelectionChanged(TSharedPtr<ConstantDropdownOption> NewValue, ESelectInfo::Type)
{
	...
	const FScopedTransaction Transaction(LOCTEXT("SetStaticSwitchCompilerConstant", "Set Static Switch Compiler Constant"));
	Node->Modify();
...
void FNiagaraStaticSwitchNodeDetails::OnParameterNameCommited(const FText& InText, ETextCommit::Type InCommitType)
{
	if (Node.IsValid())
	{
		...
		if ( Node->InputParameterName != NewName )
		{
			const FScopedTransaction Transaction(LOCTEXT("SetStaticSwitchName", "Set Static Switch Name"));
			Node->Modify();
			Node->ChangeSwitchParameterName(NewName);
			Node->GetNiagaraGraph()->NotifyGraphNeedsRecompile();
		}
	}
}

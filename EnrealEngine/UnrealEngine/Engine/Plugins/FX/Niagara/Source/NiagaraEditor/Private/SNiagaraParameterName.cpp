...
void SNiagaraParameterName::Construct(const FArguments& InArgs)
{
	...
	DecoratorLocation = InArgs._DecoratorLocation;
...
void SNiagaraParameterName::UpdateContent(FName InDisplayedParameterName, int32 InEditableNamespaceModifierIndex)
{
	...
	auto AddDecorator =
		[this, &ContentBox](bool bShouldAdd)
		{
			if (bShouldAdd && Decorator.IsValid() && Decorator != SNullWidget::NullWidget)
			{
				ContentBox->AddSlot()
					.VAlign(VAlign_Center)
	...
	AddDecorator(DecoratorLocation == ENiagaraParameterDecoratorLocation::BeforeName);
	...
	AddDecorator(DecoratorLocation == ENiagaraParameterDecoratorLocation::AfterName);

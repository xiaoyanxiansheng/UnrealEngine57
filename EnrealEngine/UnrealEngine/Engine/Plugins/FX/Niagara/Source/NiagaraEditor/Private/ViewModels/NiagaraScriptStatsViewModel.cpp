...
namespace NiagaraScriptStatsLocal
{
	...
	static void GetVMScriptStatus(TConstArrayView<UNiagaraScript*> NiagaraScripts, bool& bOutIsCompiling, bool& bOutHasError, FString& OutStatus)
	{
		...
		else
		{
			for (UNiagaraScript* Script : NiagaraScripts)
			{
				...
				{
					OutStatus.Appendf(TEXT("- Op Count = %u"), VMData.LastOpCount);
				}
				else
				{
					OutStatus.Append(TEXT("- Data missing please force a recompile"));
				}
			}
		}
	}
	...
	{
		FNiagaraScriptStats(const FGuid& InGuid, const FNiagaraEmitterHandle& EmitterHandle, EShaderPlatform ShaderPlatform)
		{
			FVersionedNiagaraEmitterData* EmitterData = EmitterHandle.GetEmitterData();

			EmitterId = InGuid;
			bIsGPU = EmitterData && EmitterData->SimTarget == ENiagaraSimTarget::GPUComputeSim;
		}

		void UpdateStatus(EShaderPlatform ShaderPlatform, const FNiagaraEmitterHandle& EmitterHandle)
		{
			if ( bIsCompiling == false )
			{
				return;
			}

			FVersionedNiagaraEmitterData* EmitterData = EmitterHandle.GetEmitterData();
			if (EmitterHandle.GetEmitterMode() != ENiagaraEmitterMode::Standard || EmitterData == nullptr)
			{
				ResultsString = TEXT("N/A");
				bIsCompiling = false;
				return;
			}
			...
			if ( bIsGPU )
			{
				UpdateStatusGPU(ShaderPlatform, EmitterData);
			}
			else
			{
				TArray<UNiagaraScript*> NiagaraScripts;
				EmitterData->GetScripts(NiagaraScripts);
				GetVMScriptStatus(MakeArrayView(NiagaraScripts), bIsCompiling, bHasError, ResultsString);
			}
		}

		void UpdateStatusGPU(EShaderPlatform ShaderPlatform, FVersionedNiagaraEmitterData* EmitterData)
		{
			bHasError = false;
				...
				for (const auto& ShaderScript : ShaderScripts)
				{
					...
					for (int32 iSimStageIndex=0; iSimStageIndex < SimulationStageMetaData.Num(); ++iSimStageIndex)
					{
						...
						if ( Shader.IsValid() )
						{
							...
							{
								ResultsString.Appendf(TEXT("\n- Num Instructions = %d"), Shader->GetNumInstructions());
							}
							...
							{
								ShaderStatsBuilder << TEXT("\n- ") << ShaderStat.StatName << TEXT(" = ");
								Visit([&ShaderStatsBuilder](auto& StoredValue) { ShaderStatsBuilder << StoredValue; }, ShaderStat.Value);
							}
...
FNiagaraScriptStatsViewModel::FGridCellDetails FNiagaraScriptStatsViewModel::GetCellDetails(const FName& ColumnName, const FGuid& RowID)
{
	...
	if (EmitterScriptsPtr == nullptr)
	{
		...
		EmitterScriptsPtr->Reset(new NiagaraScriptStatsLocal::FNiagaraScriptStats(RowID, EmitterHandle, ShaderPlatform));
	}
	NiagaraScriptStatsLocal::FNiagaraScriptStats* EmitterScripts = EmitterScriptsPtr->Get();

	EmitterScripts->UpdateStatus(ShaderPlatform, EmitterHandle);

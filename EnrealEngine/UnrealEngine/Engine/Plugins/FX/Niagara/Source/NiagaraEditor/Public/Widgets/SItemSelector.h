...
public:
	...
	DECLARE_DELEGATE_TwoParams(FOnItemRowHoverEvent, const ItemType&, bool /* bHover */)
...
public:
	...
	{}
		...
		SLATE_EVENT(FOnItemRowHoverEvent, OnItemRowHoverEvent)
...
private:
	...
	public:
		SLATE_BEGIN_ARGS(SItemSelectorItemTableRow)
			...
			SLATE_EVENT(FOnItemRowHoverEvent, OnItemRowHoverEventDelegate)
			SLATE_ARGUMENT(TSharedPtr<FItemSelectorItemContainerViewModel>, ItemViewModel)
		SLATE_END_ARGS();

		void Construct(const FArguments& InArgs, const TSharedRef<STableViewBase>& OwnerTree)
		{
			ItemViewModelWeak = InArgs._ItemViewModel;
			...
			OnItemRowHoverEventDelegate = InArgs._OnItemRowHoverEventDelegate;
			SItemSelectorTableRow::Construct(Arguments, OwnerTree);
		}

		virtual void OnMouseEnter(const FGeometry& InGeometry, const FPointerEvent& PointerEvent) override
		{
			SItemSelectorTableRow::OnMouseEnter(InGeometry, PointerEvent);

			OnItemRowHoverEventDelegate.ExecuteIfBound(ItemViewModelWeak.Pin()->GetItem(), true);
		}

		virtual void OnMouseLeave(const FPointerEvent& PointerEvent) override
		{
			SItemSelectorTableRow::OnMouseLeave(PointerEvent);

			OnItemRowHoverEventDelegate.ExecuteIfBound(ItemViewModelWeak.Pin()->GetItem(), false);
		}

		TWeakPtr<FItemSelectorItemContainerViewModel> ItemViewModelWeak;
		FOnItemRowHoverEvent OnItemRowHoverEventDelegate;
	};

	class SItemSelectorItemCategoryTableRow : public SItemSelectorTableRow
	{
	public:
		SLATE_BEGIN_ARGS(SItemSelectorItemCategoryTableRow)
			...
			SLATE_ARGUMENT(TSharedPtr<FItemSelectorItemCategoryViewModel>, CategoryViewModel)
			SLATE_EVENT(FOnGetCategoryBackgroundImage, OnGetCategoryBackgroundImage)
		SLATE_END_ARGS();

		void Construct(const FArguments& InArgs, const TSharedRef<STableViewBase>& OwnerTree)
		{
			CategoryViewModel = InArgs._CategoryViewModel;
			...
			{
				SItemSelectorTableRow::ChildSlot
	...
	private:
		...
		TWeakPtr<FItemSelectorItemCategoryViewModel> CategoryViewModel;
	};

	class SItemSelectorItemSectionTableRow : public SItemSelectorTableRow
	{
		SLATE_BEGIN_ARGS(SItemSelectorItemSectionTableRow)
			: _Style(&FCoreStyle::Get().GetWidgetStyle<FTableRowStyle>("TableView.Row"))
			, _ShowSelection(false)
			{}
			SLATE_DEFAULT_SLOT(typename SItemSelectorItemSectionTableRow::FArguments, Content)
			SLATE_STYLE_ARGUMENT(FTableRowStyle, Style)
			SLATE_ARGUMENT(bool, ShowSelection)
			SLATE_ARGUMENT(FMargin, Padding)
			SLATE_EVENT(FOnDragDetected, OnDragDetected)
			SLATE_EVENT(FOnItemRowHoverEvent, OnItemRowHoverEventDelegate)
			SLATE_ARGUMENT(TSharedPtr<FSectionViewModel>, SectionViewModel)
		SLATE_END_ARGS()

		void Construct(const FArguments& InArgs, const TSharedRef<STableViewBase>& OwnerTree)
		{
			SectionViewModelWeak = InArgs._SectionViewModel;
		...
		TWeakPtr<FSectionViewModel> SectionViewModelWeak;
	};
public:
	void Construct(const FArguments& InArgs)
	{
		...
		OnItemRowHoverEventDelegate = InArgs._OnItemRowHoverEvent;
	...
	TSharedRef<ITableRow> OnGenerateRow(TSharedRef<FItemSelectorItemViewModel> Item, const TSharedRef<STableViewBase>& OwnerTable)
	{
		...
		case EItemSelectorItemViewModelType::Category:
		{
			...
			return SNew(SItemSelectorItemCategoryTableRow, OwnerTable)
				...
				.CategoryViewModel(ItemCategoryViewModel)
				[
		...
		case EItemSelectorItemViewModelType::Item:
		{
			TSharedRef<FItemSelectorItemContainerViewModel> ItemViewModel = StaticCastSharedRef<FItemSelectorItemContainerViewModel>(Item);
				...
				.OnItemRowHoverEventDelegate(OnItemRowHoverEventDelegate)
		...
		case EItemSelectorItemViewModelType::Section:
		{
			...
			return SNew(SItemSelectorItemSectionTableRow, OwnerTable)

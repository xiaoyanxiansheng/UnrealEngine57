...
void SNiagaraStackFunctionInputName::Construct(const FArguments& InArgs, UNiagaraStackFunctionInput* InFunctionInput, UNiagaraStackViewModel* InStackViewModel)
{
	...
	if (FunctionInput->GetInputType().IsStatic())
	{
		NameBox->AddSlot()
		.VAlign(VAlign_Center)
		.AutoWidth()
		[
			SNew(SBox)
			.ToolTipText(LOCTEXT("TypeModifierTooltip", "This variable is Static and cannot be changed at runtime, or linked to any\n dynamic attributes. Static variables can be used to reduce script instructions & memory."))
			.Padding(3, 2, 1, 2)
			.Content()
			[
				SNew(SImage)
				.ColorAndOpacity(FSlateColor::UseSubduedForeground())
				.Image(FNiagaraEditorStyle::Get().GetBrush("NiagaraEditor.StaticInputIcon"))
			]
		];
	}
}

...
IConsoleVariable* SNiagaraStackRowPerfWidget::StatEnabledVar = IConsoleManager::Get().FindConsoleVariable(TEXT("fx.Niagara.EditorPerformanceStatsEnabled"));

void SNiagaraStackRowPerfWidget::Construct(const FArguments& InArgs, UNiagaraStackEntry* InStackEntry, ENiagaraPerformanceStateScopeMode InStatScopeMode)
{
	StackEntry = InStackEntry;
	StatScopeMode = InStatScopeMode;
		...
		[
            ...
            [
                ...
                [
					...
					.Visibility(this, &SNiagaraStackRowPerfWidget::GetPerfBarVisibility)
                    [
                        SNew(SColorBlock)
...
EVisibility SNiagaraStackRowPerfWidget::GetPerfBarVisibility() const
{
	return StatScopeMode == ENiagaraPerformanceStateScopeMode::Detailed ? EVisibility::Visible : EVisibility::Hidden;
}
...
bool SNiagaraStackRowPerfWidget::HasPerformanceData() const
{
	...
	const bool IsPerfCaptureEnabled = StackEntry->GetSystemViewModel()->IsPerformanceStatsEnabled() && (StackEntry->GetSystemViewModel()->GetPerformanceStateScopeMode() >= StatScopeMode);
...
FText SNiagaraStackRowPerfWidget::GetPerformanceDisplayText() const
{
	...
	TimeFormatOptions.MinimumFractionalDigits = 3;
	TimeFormatOptions.MaximumFractionalDigits = 3;
...
{
	TSharedPtr<FNiagaraSystemViewModel> ViewModel = StackEntry.IsValid() ? StackEntry->GetSystemViewModelPtr() : nullptr;
	return ViewModel.IsValid() ? ViewModel->StatEvaluationType : ENiagaraStatEvaluationType::Average;
}

ENiagaraStatDisplayMode SNiagaraStackRowPerfWidget::GetDisplayMode() const
{
	TSharedPtr<FNiagaraSystemViewModel> ViewModel = StackEntry.IsValid() ? StackEntry->GetSystemViewModelPtr() : nullptr;
	return ViewModel.IsValid() ? ViewModel->StatDisplayMode : ENiagaraStatDisplayMode::Percent;
}
...
bool SNiagaraStackRowPerfWidget::IsInterpolatedSpawnEnabled() const
{
	...
	return EmitterData->UsesInterpolatedSpawning();
}

...
class SNiagaraSystemOverviewEntryListRow : public STableRow<UNiagaraStackEntry*>
{
	...
	void Construct(const FArguments& InArgs, UNiagaraStackViewModel* InStackViewModel, UNiagaraStackEntry* InStackEntry, TSharedRef<FNiagaraStackCommandContext> InStackCommandContext, const TSharedRef<STableViewBase>& InOwnerTableView)
	{
		...
		if(StackEntry->UsesCustomTooltipWidget())
		{
			TooltipBorder->SetToolTip(TAttribute<TSharedPtr<IToolTip>>::Create(
				TAttribute<TSharedPtr<IToolTip>>::FGetter::CreateUObject(StackEntry, &UNiagaraStackEntry::GetCustomTooltipWidget)));
		}
		else
		{
			TooltipBorder->SetToolTipText(TAttribute<FText>::Create(TAttribute<FText>::FGetter::CreateUObject(StackEntry, &UNiagaraStackEntry::GetTooltipText)));
		}
	}

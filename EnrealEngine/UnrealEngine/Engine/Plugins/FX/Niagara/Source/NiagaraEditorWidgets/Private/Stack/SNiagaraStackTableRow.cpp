...
SNiagaraStackTableRow::~SNiagaraStackTableRow()
{
	...
	{
		StackEntry->OnPulseRequested().Unbind();
	}
}
...
void SNiagaraStackTableRow::Construct(const FArguments& InArgs, UNiagaraStackViewModel* InStackViewModel, UNiagaraStackEntry* InStackEntry, TSharedRef<FNiagaraStackCommandContext> InStackCommandContext, const TSharedRef<STreeView<UNiagaraStackEntry*>>& InOwnerTree)
{
	...
	InStackEntry->OnPulseRequested().BindSP(this, &SNiagaraStackTableRow::Pulse);
	...
	if(StackEntry->GetEmitterViewModel().IsValid())
	{
		...
		if(Identity.IsValid())
		{
			...
			else
			{
				...
				UHierarchyElement* Element = StackEntry->GetEmitterViewModel()->GetSummaryHierarchyViewModel()->GetHierarchyRoot()->AddChild<UHierarchyElement>(HierarchyClass);

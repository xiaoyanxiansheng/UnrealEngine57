	...
	if (SystemViewModel.IsValid())
	{
		for (TSharedRef<FNiagaraEmitterHandleViewModel> EmitterModel : SystemViewModel.Pin()->GetEmitterHandleViewModels())
		{
			if (FVersionedNiagaraEmitterData* EmitterData = EmitterModel->GetEmitterHandle()->GetEmitterData())
			{
				...
				{
					bool bHasDeprecatedParentAsset = UE::UserAssetTags::HasUserAssetTag(EmitterData->GetParent().Emitter, "Deprecated");
					
					if (bHasDeprecatedParentAsset || ParentData->bDeprecated)
					{
	...
	if (SystemViewModel.IsValid())
	{
		for (TSharedRef<FNiagaraEmitterHandleViewModel> EmitterModel : SystemViewModel.Pin()->GetEmitterHandleViewModels())
		{
			if (FVersionedNiagaraEmitterData* EmitterData = EmitterModel->GetEmitterHandle()->GetEmitterData())
			{
				...
				{
					bool bHasDeprecatedParentAsset = UE::UserAssetTags::HasUserAssetTag(EmitterData->GetParent().Emitter, "Deprecated");
					return ParentData->bDeprecated || bHasDeprecatedParentAsset;
				}
			}
		}
	}

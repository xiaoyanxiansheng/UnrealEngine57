...
bool FNiagaraStackEditorWidgetsUtilities::AddStackNoteContextMenuActions(FMenuBuilder& MenuBuilder, UNiagaraStackEntry& StackEntry)
{
	if(StackEntry.SupportsStackNotes())
	{
		...
		if(StackEntry.HasStackNoteData() == false)
		{
			...
			FUIAction(FExecuteAction::CreateUObject(&StackEntry, &UNiagaraStackEntry::AddStackNote))
			);
		}
		else
		{				
			...
			FSlateIcon(FAppStyle::GetAppStyleSetName(), "Icons.Comment"),
			FUIAction(FExecuteAction::CreateUObject(&StackEntry, &UNiagaraStackEntry::ToggleStackNoteInlineDisplay))
			);
			...
			FUIAction(FExecuteAction::CreateUObject(&StackEntry, &UNiagaraStackEntry::CopyStackNote))
			);
		}
		...
		if(const UNiagaraClipboardContent* ClipboardContent = FNiagaraEditorModule::Get().GetClipboard().GetClipboardContent())
		{
			if(ClipboardContent->StackNote.IsValid())
			{
				...
				FUIAction(FExecuteAction::CreateUObject(&StackEntry, &UNiagaraStackEntry::PasteStackNote))
				);
			}
		}
		...
		if(StackEntry.HasStackNoteData())
		{
			...
			FUIAction(FExecuteAction::CreateUObject(&StackEntry, &UNiagaraStackEntry::DeleteStackNoteData))
			);
		}

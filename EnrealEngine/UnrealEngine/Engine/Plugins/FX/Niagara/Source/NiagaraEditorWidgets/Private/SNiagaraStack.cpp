...
void SNiagaraStack::Construct(const FArguments& InArgs, UNiagaraStackViewModel* InStackViewModel)
{
	...
	StackViewModel->OnStackEntrySelectionRequested().BindSP(this, &SNiagaraStack::SelectStackEntry);
...
void SNiagaraStack::SelectStackEntry(UNiagaraStackEntry* StackEntry, FNiagaraStackEntrySelectionConfig Config)
{
	if(StackViewModel && StackEntry->IsFinalized() == false)
	{
		if(Config.bScrollToAndExpand)
		{
			TArray<UNiagaraStackEntry*> StackEntryPath;
			StackViewModel->GetPathForEntry(StackEntry, StackEntryPath);
			ExpandAllInPath(StackEntryPath);
			
			StackTree->RequestScrollIntoView(StackEntry);
		}

		if(Config.bSelectRow)
		{
			StackTree->SetSelection(StackEntry, ESelectInfo::Type::Direct);
		}

		if(Config.bPulse)
		{
			
			RegisterActiveTimer(0.f, FWidgetActiveTimerDelegate::CreateLambda([StackEntry](double CurrentTime, float DeltaTime)
			{
				StackEntry->RequestPulse();
...
void SNiagaraStack::OnStackSearchComplete(const FNiagaraStackSearchConfig& SearchConfig)
{
	ExpandSearchResults();

	if(SearchConfig.bScrollToResult)
	{
		ScrollFocusedEntryIntoView();
	}
}

...
namespace NiagaraSimulationTaskPriority
{
	...
	// Priority for work which is not allowed to overlap tick groups
	static int32 GNoneOverlappingPriority = 1;
	static FAutoConsoleVariableRef CVarNoneOverlappingPriority(
		TEXT("fx.Niagara.TaskPriorities.NoneOverlappingPriority"),
		GNoneOverlappingPriority,
		TEXT("Priority to use when we are not allowed to overlap tick groups.")
		TEXT("Default is 1, set to -1 to not override the default priorities."),
		ECVF_Default
	);
			...
			[]()
			{
				...
				for (int32 i = 0; i < UE_ARRAY_COUNT(GTickGroupPriorities); ++i)
				{
					DumpPriority(
						...
						GetTickGroupPriority(ETickingGroup(i), true),
	...
	ENamedThreads::Type GetTickGroupPriority(ETickingGroup TickGroup, bool bCanOverlapTickGroups)
	{
		int32 Priority = 0;
		...
		Priority = bCanOverlapTickGroups || GNoneOverlappingPriority < 0 ? Priority : GNoneOverlappingPriority;

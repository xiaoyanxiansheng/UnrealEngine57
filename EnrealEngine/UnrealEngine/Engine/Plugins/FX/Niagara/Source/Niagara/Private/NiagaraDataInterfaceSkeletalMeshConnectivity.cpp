...
{
	const int32 BufferSize = AdjacencyResource.GetResourceDataSize();

	const FRHIBufferCreateDesc CreateDesc =
		FRHIBufferCreateDesc::CreateVertex(TEXT("FSkeletalMeshConnectivityProxy_AdjacencyBuffer"), BufferSize)
		.AddUsage(EBufferUsageFlags::ShaderResource | EBufferUsageFlags::Static)
		.SetInitActionResourceArray(&AdjacencyResource)
		.DetermineInitialState();

	AdjacencyBuffer = RHICmdList.CreateBuffer(CreateDesc);
	AdjacencySrv = RHICmdList.CreateShaderResourceView(
		AdjacencyBuffer, 

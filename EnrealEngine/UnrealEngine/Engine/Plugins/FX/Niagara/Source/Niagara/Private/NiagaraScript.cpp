...
void UNiagaraScript::ComputeVMCompilationId(FNiagaraVMExecutableDataId& Id, const FGuid& VersionGuid, FNiagaraScriptHashCollector* OutHashCollector) const
{
	...
	Id.AdditionalDefines.Add(FString::Printf(TEXT("EnableTraversalCache: %d"), GbNiagaraEnableTraversalCache));
...
void UNiagaraScript::Serialize(FArchive& Ar)
{
	...
	if (Ar.IsCooking())
	{
		...
		// on changed messages because it will modify the change id on owning objects so we suppress those messages starting here.
		FNiagaraParameterStore::FScopedSuppressOnChanged SuppressOnChanged(RapidIterationParameters);
...
void UNiagaraScript::SetPreviewFeatureLevel(ERHIFeatureLevel::Type InPreviewFeatureLevel)
{
	...
	// We need to invalid IsCooked if we are changing preview level as the scripts can get out of sync
	for (TObjectIterator<UNiagaraScript> It; It; ++It)
	{
		if (UNiagaraScript* Script = *It)
		{
			Script->IsCooked = false;
		}
	}
...
void UNiagaraScript::BuildClassSchema(FAppendToClassSchemaContext& Context)
{
	...
	Context.Update(&GbNiagaraEnableTraversalCache, sizeof(GbNiagaraEnableTraversalCache));

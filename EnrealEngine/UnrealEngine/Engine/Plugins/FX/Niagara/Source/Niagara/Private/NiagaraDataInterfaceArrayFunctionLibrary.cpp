...
void SetNiagaraArray(UNiagaraComponent* NiagaraComponent, FName OverrideName, TConstArrayView<TArrayType> InArray)
{
	if (TDataInterace* ArrayDI = UNiagaraFunctionLibrary::GetDataInterface<TDataInterace>(NiagaraComponent, OverrideName))
	{
		...
		ArrayDI->CreateAndSetVariant(
			NiagaraComponent,
			OverrideName,
			[InArray](UNiagaraDataInterfaceArray* VariantDI)
			{
				CastChecked<TDataInterace>(VariantDI)->SetVariantArrayData(InArray);
			}
		);
	#endif
	}
...
void UNiagaraDataInterfaceArrayFunctionLibrary::SetNiagaraArrayInt32(UNiagaraComponent* NiagaraSystem, FName OverrideName, TConstArrayView<int64> ArrayData)
{
	SetNiagaraArray<int64, UNiagaraDataInterfaceArrayInt32>(NiagaraSystem, OverrideName, ArrayData);
}

void UNiagaraDataInterfaceArrayFunctionLibrary::SetNiagaraArrayFloat(UNiagaraComponent* NiagaraSystem, FName OverrideName, TConstArrayView<double> ArrayData)
{
	SetNiagaraArray<double, UNiagaraDataInterfaceArrayFloat>(NiagaraSystem, OverrideName, ArrayData);
}

...
void FNiagaraParameterStoreBinding::CopyParameters(FNiagaraParameterStore* DestStore, const FNiagaraParameterStore* SrcStore) const
{
	...
	{
		DestStore->SetParameterData(SrcStore->GetParameterData(Binding.SrcOffset, Binding.Size), Binding.DestOffset, Binding.Size);
	}
...
bool FNiagaraParameterStoreBinding::VerifyBinding(const FNiagaraParameterStore* DestStore, const FNiagaraParameterStore* SrcStore)const
{
...
void FNiagaraParameterStoreBinding::Dump(const FNiagaraParameterStore* DestStore, const FNiagaraParameterStore* SrcStore)const
{
#if WITH_EDITORONLY_DATA
	...
	for (const FParameterBinding& Binding : ParameterBindings)
	{
		...
		if (ensure(bFound))
		{
			...
			const uint8* SrcData = SrcStore->GetParameterData(Binding.SrcOffset, Binding.Size);
			const uint8* DestData = DestStore->GetParameterData(Binding.DestOffset, Binding.Size);
	...
	for (const FUObjectBinding& Binding : UObjectBindings)
	{
		...
		if (ensure(bFound))
		{
			...
			//Also ensure the param has been pushed correctly.
			TObjectPtr<const UObject> SrcData = SrcStore->GetUObjects()[Binding.SrcOffset];
			TObjectPtr<const UObject> DestData = DestStore->GetUObjects()[Binding.DestOffset];
			if (!ensure(SrcData == DestData))
			{
				UE_LOG(LogNiagara, Log, TEXT("Data interface parameter in dest store is incorrect!\n"));
			}
...
void FNiagaraParameterStore::CheckForNaNs()const
{
	for (const FNiagaraVariableWithOffset& Var : ReadParameterVariables())
	{
		...
		{
			const float Val = GetParameterValueFromOffset<float>(Offset);
			bContainsNans = FMath::IsNaN(Val) || !FMath::IsFinite(Val);
		}
		else if (Var.GetType() == FNiagaraTypeDefinition::GetVec2Def())
		{
			const FVector2f Val = GetParameterValueFromOffset<FVector2f>(Offset);
			bContainsNans = Val.ContainsNaN();
		}
		else if (Var.GetType() == FNiagaraTypeDefinition::GetVec3Def())
		{
			const FVector3f Val = GetParameterValueFromOffset<FVector3f>(Offset);
			bContainsNans = Val.ContainsNaN();
		}
		else if (Var.GetType() == FNiagaraTypeDefinition::GetVec4Def())
		{
			const FVector4f Val = GetParameterValueFromOffset<FVector4f>(Offset);
			bContainsNans = Val.ContainsNaN();
		}
		else if (Var.GetType() == FNiagaraTypeDefinition::GetMatrix4Def())
		{
			const FMatrix44f Val = GetParameterValueFromOffset<FMatrix44f>(Offset);
			bContainsNans = Val.ContainsNaN();
		}
...
void FNiagaraParameterStore::ConvertParameterType(const FNiagaraVariable& ExistingParam, const FNiagaraTypeDefinition& NewType)
{
	...
	{
		const FVector CurrentValue = FVector(*reinterpret_cast<const FVector3f*>(GetParameterData_Internal(Idx)));
...
void FNiagaraParameterStore::TickBindings() const
{
	...
	for (const TPair<FNiagaraParameterStore*, FNiagaraParameterStoreBinding>& Binding : Bindings)
	{
		...
		if ( bParametersDirty )
		{
			...
			{
				DestStore->SetParameterData(GetParameterData(ParamBinding.SrcOffset, ParamBinding.Size), ParamBinding.DestOffset, ParamBinding.Size);
			}
		}
		if ( bInterfacesDirty )
		{
...
void FNiagaraParameterStore::DumpParameters(bool bDumpBindings) const
{
	for (const FNiagaraVariableWithOffset& VariableBase : ReadParameterVariables())
	{
		...
		Var.SetData(GetParameterData_Internal(VariableBase.Offset));
...
bool FNiagaraParameterStore::AddParameter(const FNiagaraVariable& Param, bool bInitInterfaces, bool bTriggerRebind, int32* OutOffset)
{
	...
	if (ParamType.IsDataInterface())
	{
		...
		const EObjectFlags DataInterfaceObjectFlags = UNiagaraDataInterface::BuildObjectFlagsForOwner(Owner.Get(), RF_Transactional);
		DataInterfaces[Offset] = bInitInterfaces ? NewObject<UNiagaraDataInterface>(Owner.Get(), const_cast<UClass*>(ParamType.GetClass()), NAME_None, DataInterfaceObjectFlags) : nullptr;
		bInterfacesDirty = true;
	}
	...
	else
	{
		...
		if (Param.IsDataAllocated())
		{
			...
			{
				StructConverter.ConvertDataToSimulation(GetMutableParameterData_Internal(Offset), Param.GetData());
			}
			else
			{
				FMemory::Memcpy(GetMutableParameterData_Internal(Offset), Param.GetData(), ParamSize);	
			}			
		}
		else
		{
	...
	for (const FNiagaraVariableWithOffset& Parameter : ReadParameterVariables())
	{
		...
		if (Parameter.IsValid())
		{
			if (Parameter.IsDataInterface())
			{
				...
				{
					const EObjectFlags DataInterfaceObjectFlags = UNiagaraDataInterface::BuildObjectFlagsForOwner(Owner.Get(), RF_Transactional);
					DataInterfaces[SrcIndex] = NewObject<UNiagaraDataInterface>(Owner.Get(), const_cast<UClass*>(Parameter.GetType().GetClass()), NAME_None, DataInterfaceObjectFlags);
...
void FNiagaraParameterStore::ResolvePositions(FNiagaraLWCConverter LwcConverter)
{
	...
	for (const FNiagaraPositionSource& Data : OriginalPositionData)
	{
		...
		if (Offset != INDEX_NONE)
		{
			...
			FNiagaraPosition* ParamData = reinterpret_cast<FNiagaraPosition*>( GetMutableParameterData_Internal(Offset) );
...
void FNiagaraParameterStore::CopyParameterData(FNiagaraParameterStore& DestStore, const FNiagaraVariable& SourceParameter, const FNiagaraVariable& TargetParameter) const
{
	...
	if (DestIndex != INDEX_NONE && SrcIndex != INDEX_NONE)
	{
		...
		{
			DestStore.SetParameterData(GetParameterData_Internal(SrcIndex), DestIndex, SourceParameter.GetSizeInBytes());
		}
	}
}
...
bool FNiagaraParameterStore::CopyParameterData(const FNiagaraVariable& Parameter, uint8* DestinationData) const
{
	...
	{
		const uint8* SourceData = GetParameterData_Internal(NiagaraVariableWithOffset->Offset);
...
void FNiagaraParameterStore::RemoveAllOnChangedHandlers(FDelegateUserObjectConst InUserObject)
{

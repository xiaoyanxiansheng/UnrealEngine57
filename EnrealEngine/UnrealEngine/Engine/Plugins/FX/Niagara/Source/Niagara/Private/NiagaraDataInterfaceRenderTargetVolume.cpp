...
UObject* UNiagaraDataInterfaceRenderTargetVolume::SimCacheBeginWrite(UObject* InSimCache, FNiagaraSystemInstance* NiagaraSystemInstance, const void* OptionalPerInstanceData, FNiagaraSimCacheFeedbackContext& FeedbackContext) const
{
	...
	else if (NDIRenderTargetVolumeLocal::GSimCacheEnabled)
	{
		UNDIRenderTargetSimCacheData* SimCacheData = NewObject<UNDIRenderTargetSimCacheData>(InSimCache);
		...
		if (TextureData.Num() > 0)
		{
			...
			else
			{
				...
				SimCacheData->SetPixelData(
					FrameIndex,
					InstanceData_GT->Size,
					InstanceData_GT->Format,
					TextureData
				);
			}
		}
...
bool UNiagaraDataInterfaceRenderTargetVolume::SimCacheReadFrame(const FNiagaraSimCacheDataInterfaceReadContext& ReadContext)
{
	FRenderTargetVolumeRWInstanceData_GameThread* InstanceData_GT = ReadContext.GetPerInstanceData<FRenderTargetVolumeRWInstanceData_GameThread>();

	if (ReadContext.GetOptionalStorageObject<UVolumeCache>())
	{
	...
	else if (UAnimatedSparseVolumeTexture* SVT = ReadContext.GetOptionalStorageObject<UAnimatedSparseVolumeTexture>())
	{			
		const int32 MipLevel = 0;
		const float FrameRate = 0.0f;
		const bool bBlocking = true;
		const bool bHasFrameRate = false;
		const float SVYFrameIndex = FMath::RoundToFloat(float(ReadContext.GetFrameIndexA()) + ReadContext.GetFrameInterp());
		USparseVolumeTextureFrame *SVTFrame = USparseVolumeTextureFrame::GetFrameAndIssueStreamingRequest(SVT, GetTypeHash(this), FrameRate, SVYFrameIndex, MipLevel, bBlocking, bHasFrameRate);
		...
		const FIntVector VolumeResolution = SVT->GetVolumeResolution();						
	...
	else if (UNDIRenderTargetSimCacheData* SimCacheData = ReadContext.GetOptionalStorageObject<UNDIRenderTargetSimCacheData>())
	{
		...
		InstanceData_GT->Size   = SimCacheData->GetTextureSize(FrameIndex);
		InstanceData_GT->Format = SimCacheData->GetTextureFormat(FrameIndex);
		InstanceData_GT->Filter = TextureFilter::TF_Default;
			...
			(
				[RT_Proxy, RT_InstanceID=SystemInstance->GetId(), RT_TargetTexture, RT_SimCacheData=SimCacheData, RT_FrameIndex=FrameIndex](FRHICommandListImmediate& RHICmdList)
				{
					if (FRenderTargetVolumeRWInstanceData_RenderThread* InstanceData_RT = RT_Proxy->SystemInstancesToProxyData_RT.Find(RT_InstanceID))
					{
						...
						RT_SimCacheData->GetPixelData(RT_FrameIndex, UpdateTexture.Data, UpdateTexture.RowPitch, UpdateTexture.DepthPitch);
...
bool UNiagaraDataInterfaceRenderTargetVolume::PerInstanceTick(void* PerInstanceData, FNiagaraSystemInstance* SystemInstance, float DeltaSeconds)
{
	...
	if (bInheritUserParameterSettings)
	{
		if (UserTargetTexture)
		{
			...
			//}
			InstanceData->Format = InstanceData->TargetTexture->GetFormat();
...
bool UNiagaraDataInterfaceRenderTargetVolume::PerInstanceTickPostSimulate(void* PerInstanceData, FNiagaraSystemInstance* SystemInstance, float DeltaSeconds)
{
	...
	if (InstanceData->TargetTexture)
	{
		...
		if ((InstanceData->TargetTexture->SizeX != InstanceData->Size.X) || (InstanceData->TargetTexture->SizeY != InstanceData->Size.Y) || (InstanceData->TargetTexture->SizeZ != InstanceData->Size.Z) ||
			(InstanceData->TargetTexture->GetFormat() != InstanceData->Format) ||
			(InstanceData->TargetTexture->Filter != InstanceData->Filter) ||

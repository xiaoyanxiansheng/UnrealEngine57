...
void INiagaraParameterDefinitionsSubscriber::SynchronizeWithParameterDefinitions(const FSynchronizeWithParameterDefinitionsArgs Args /*= FSynchronizeWithParameterDefinitionsArgs()*/)
{
	...
	if (AssetRegistryModule.Get().IsLoadingAssets() == false)
	{
		...
		// Since this runs during the cook, we must query for disk-only assets to ensure deterministic results
		// (any asset queries running during cook of a package will affect dependencies)
		FARFilter Filter;
		Filter.ClassPaths.Add(FTopLevelAssetPath(TEXT("/Script/NiagaraEditor"), TEXT("NiagaraParameterDefinitions")));
		Filter.bRecursiveClasses = false;
		Filter.bIncludeOnlyOnDiskAssets = IsRunningCookCommandlet();
		AssetRegistryModule.GetRegistry().GetAssets(Filter, ParameterDefinitionsAssetData);

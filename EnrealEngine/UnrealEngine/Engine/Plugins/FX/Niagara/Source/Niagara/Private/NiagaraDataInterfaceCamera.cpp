...
const FName UNiagaraDataInterfaceCamera::GetSplitScreenName(TEXT("GetSplitscreenInfo"));
...
struct FCameraDataInterface_InstanceDataRT
{
	ESplitScreenType::Type SplitscreenMode = ESplitScreenType::Type::None;
};

struct FNiagaraDataInterfaceProxyCameraQuery : FNiagaraDataInterfaceProxy
{
	virtual int32 PerInstanceDataPassedToRenderThreadSize() const override { return sizeof(FCameraDataInterface_InstanceDataRT); }
	...
	{
		FCameraDataInterface_InstanceDataRT* DataForRenderThread = new (InDataForRenderThread) FCameraDataInterface_InstanceDataRT();
		const FCameraDataInterface_InstanceData* DataFromGameThread = static_cast<FCameraDataInterface_InstanceData*>(InDataFromGameThread);
		
		DataForRenderThread->SplitscreenMode = DataFromGameThread->SplitscreenMode;
	}
	...
	{
		FCameraDataInterface_InstanceDataRT* InstanceDataFromGT = static_cast<FCameraDataInterface_InstanceDataRT*>(PerInstanceData);
		FCameraDataInterface_InstanceDataRT& InstanceData = SystemInstancesToInstanceData_RT.FindOrAdd(InstanceID);
		InstanceData = *InstanceDataFromGT;

		// clean up the GT data
		InstanceDataFromGT->~FCameraDataInterface_InstanceDataRT();
	}

	TMap<FNiagaraSystemInstanceID, FCameraDataInterface_InstanceDataRT> SystemInstancesToInstanceData_RT;
};

UNiagaraDataInterfaceCamera::UNiagaraDataInterfaceCamera(FObjectInitializer const& ObjectInitializer)
	: Super(ObjectInitializer)
{
	Proxy.Reset(new FNiagaraDataInterfaceProxyCameraQuery());
}
...
void UNiagaraDataInterfaceCamera::DestroyPerInstanceData(void* PerInstanceData, FNiagaraSystemInstance* SystemInstance)
{
	...
	(
		[RT_Proxy=GetProxyAs<FNiagaraDataInterfaceProxyCameraQuery>(), InstanceID=SystemInstance->GetId()](FRHICommandListImmediate& CmdList)
		{
			...
			if (PlayerController != nullptr && PlayerController->GetLocalPlayer() != nullptr)
			{
				if (LocalPlayerIndex == PlayerControllerIndex)
				{
					if (PlayerController->PlayerCameraManager)
					{
						...
						{
							PIData->SplitscreenMode = LocalPlayer->ViewportClient ? LocalPlayer->ViewportClient->GetCurrentSplitscreenConfiguration() : ESplitScreenType::Type::None;
						}
						return false;
					}
					else
...
void UNiagaraDataInterfaceCamera::ProvidePerInstanceDataForRenderThread(void* DataForRenderThread, void* PerInstanceData, const FNiagaraSystemInstanceID& SystemInstance)
{
	FNiagaraDataInterfaceProxyCameraQuery::ProvidePerInstanceDataForRenderThread(DataForRenderThread, PerInstanceData, SystemInstance);
}
...
void UNiagaraDataInterfaceCamera::GetFunctionsInternal(TArray<FNiagaraFunctionSignature>& OutFunctions) const
{
	...
	Sig.AddOutput(FNiagaraVariable(FNiagaraTypeDefinition::GetFloatDef(), TEXT("Near Plane")), LOCTEXT("NearPlaneDescription", "Returns the near plane value."));
	Sig.AddOutput(FNiagaraVariable(FNiagaraTypeDefinition::GetVec2Def(), TEXT("Field Of View (Current Frame)")), LOCTEXT("FOVCurrentDescription", "Returns the current camera field of view (X & Y) in radians."));
	Sig.AddOutput(FNiagaraVariable(FNiagaraTypeDefinition::GetVec2Def(), TEXT("Field Of View (Previous Frame)")), LOCTEXT("FOVPreviousDescription", "Returns the previous camera field of view (X & Y) in radians."));
	...
	Sig = FNiagaraFunctionSignature();
	Sig.Name = GetSplitScreenName;
	Sig.Description = NSLOCTEXT("Niagara", "GetSplitScreenNameDescription", "This function returns the currently active splitscreen mode as ESplitScreenType value.");
	Sig.FunctionVersion = FNiagaraCameraDIFunctionVersion::LatestVersion;
	Sig.bMemberFunction = true;
	Sig.Inputs.Add(FNiagaraVariable(FNiagaraTypeDefinition(GetClass()), TEXT("Camera interface")));
	Sig.AddOutput(FNiagaraVariable(FNiagaraTypeDefinition(StaticEnum<ESplitScreenType::Type>()), TEXT("Current Splitscreen Mode")), LOCTEXT("SplitscreenModeOutput", "The splitscreen mode as ESplitScreenType value. A value of 0 means there is no splitscreen active."));
	OutFunctions.Add(Sig);
}
...
bool UNiagaraDataInterfaceCamera::GetFunctionHLSL(const FNiagaraDataInterfaceGPUParamInfo& ParamInfo, const FNiagaraDataInterfaceGeneratedFunction& FunctionInfo, int FunctionInstanceIndex, FString& OutHLSL)
{
	if ((FunctionInfo.DefinitionName == GetViewPropertiesName) ||
		...
		(FunctionInfo.DefinitionName == GetSplitScreenName) ||
...
bool UNiagaraDataInterfaceCamera::UpgradeFunctionCall(FNiagaraFunctionSignature& FunctionSignature)
{
	...
	// LWCConversion + AddGPUNearPlane + AddGPUFOVData
	if (FunctionSignature.FunctionVersion < FNiagaraCameraDIFunctionVersion::LatestVersion)
	{
...
{
	FNiagaraDataInterfaceProxyCameraQuery& DataInterfaceProxy = Context.GetProxy<FNiagaraDataInterfaceProxyCameraQuery>();
	FCameraDataInterface_InstanceDataRT& InstanceData = DataInterfaceProxy.SystemInstancesToInstanceData_RT.FindChecked(Context.GetSystemInstanceID());
	
	FShaderParameters* ShaderParameters = Context.GetParameterNestedStruct<FShaderParameters>();
	ShaderParameters->SplitscreenMode = static_cast<int32>(InstanceData.SplitscreenMode);
}
...
DEFINE_NDI_DIRECT_FUNC_BINDER(UNiagaraDataInterfaceCamera, GetGetSplitScreenInfo);
void UNiagaraDataInterfaceCamera::GetVMExternalFunction(const FVMExternalFunctionBindingInfo& BindingInfo, void* InstanceData, FVMExternalFunction &OutFunc)
{
	...
	else if (BindingInfo.Name == GetSplitScreenName)
	{
		NDI_FUNC_BINDER(UNiagaraDataInterfaceCamera, GetGetSplitScreenInfo)::Bind(this, OutFunc);
	}
	else
	{
...
void UNiagaraDataInterfaceCamera::GetGetSplitScreenInfo(FVectorVMExternalFunctionContext& Context)
{
	VectorVM::FUserPtrHandler<FCameraDataInterface_InstanceData> InstData(Context);
	VectorVM::FExternalFuncRegisterHandler<ESplitScreenType::Type> OutMode(Context);

	ESplitScreenType::Type SplitscreenMode = InstData.Get()->SplitscreenMode;

	for (int32 i = 0; i < Context.GetNumInstances(); ++i)
	{
		*OutMode.GetDestAndAdvance() = SplitscreenMode;
	}
}

		...
		if (Context.IsResourceBound(&Parameters->Texture))
		{
			...
			if (InstanceData && RDGTexture == nullptr)
			{
				...
				if (ResolvedTextureRHI.IsValid())
				{
					...
					{
						InstanceData->TransientRDGTexture = GraphBuilder.RegisterExternalTexture(CreateRenderTarget(ResolvedTextureRHI, TEXT("NiagaraTextureCube")));
			...
			Parameters->Texture = RDGTexture ? RDGTexture : Context.GetComputeDispatchInterface().GetBlackTexture(GraphBuilder, ETextureDimension::TextureCube);
		}
	}
	else
	{
		...
		Parameters->Texture = Context.GetComputeDispatchInterface().GetBlackTexture(GraphBuilder, ETextureDimension::TextureCube);
	}
}

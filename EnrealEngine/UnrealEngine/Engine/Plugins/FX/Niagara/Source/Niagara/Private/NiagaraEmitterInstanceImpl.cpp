	...
	: FNiagaraEmitterInstance(InParentSystemInstance)
{
	...
	if (FNiagaraGpuComputeDispatchInterface* ComputeDispatchInterface = ParentSystemInstance->GetComputeDispatchInterface())
	{
...
void FNiagaraEmitterInstanceImpl::CalculateFixedBounds(const FTransform& ToWorldSpace)
{
	...
	if (EmitterData->SimTarget == ENiagaraSimTarget::GPUComputeSim)
	{
		...
		ScopedGPUReadback.ReadbackData(ComputeDispatchInterfacePtr.Get(), GPUExecContext->MainDataSet);
...
bool FNiagaraEmitterInstanceImpl::WaitForDebugInfo()
{
	...
	{
		FNiagaraGpuComputeDispatchInterface* ComputeDispatchInterface = ComputeDispatchInterfacePtr.Get();
		if (ComputeDispatchInterface)
		{
			ENQUEUE_RENDER_COMMAND(CaptureCommand)([ComputeDispatchInterface](FRHICommandListImmediate& RHICmdList) { ComputeDispatchInterface->ProcessDebugReadbacks(RHICmdList, true); });
		}
		else
		{
			UE_LOG(LogNiagara, Fatal, TEXT("Compute interface is invalid and we have a GPU simulation, this may result in a softlock."));
		}
...
void FNiagaraEmitterInstanceImpl::Tick(float DeltaSeconds)
{
	...
	if (EmitterData->SimTarget == ENiagaraSimTarget::GPUComputeSim && GPUExecContext != nullptr)
	{
		...
		if (ParentSystemInstance->ShouldCaptureThisFrame())
		{
			...
			if (DebugInfo && ComputeDispatchInterfacePtr.IsValid())
			{
				...
				ENQUEUE_RENDER_COMMAND(NiagaraReadbackGpuSim)(
					[RT_ComputeDispatchInterface = ComputeDispatchInterfacePtr.Get(), RT_InstanceID = ParentSystemInstance->GetId(), RT_DebugInfo = DebugInfo, RT_Context = GPUExecContext](FRHICommandListImmediate& RHICmdList)
					{

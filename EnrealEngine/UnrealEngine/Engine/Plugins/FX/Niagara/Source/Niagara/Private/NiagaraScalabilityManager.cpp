...
FNiagaraScalabilityManager::FNiagaraScalabilityManager(FNiagaraWorldManager* InOwner)
	: WorldMan(InOwner)
	, EffectType(nullptr)
	, LastUpdateTime(0.0)
...
void FNiagaraScalabilityManager::ProcessSignificance(UNiagaraSignificanceHandler* SignificanceHandler, FComponentIterationContext& Context)
{
	...
	struct FCullProxyCreateDestroyTask
	{
		UNiagaraComponent* Component = nullptr;
		bool bCreate = false;
	};

	TArray<FCullProxyCreateDestroyTask, TInlineAllocator<64>> CullProxyTasks;
	for (int32 SortedIt = 0; SortedIt < Context.SignificanceIndices.Num(); ++SortedIt)
	{
		...
		//Gather list of cull proxies to create/destroy. Move outside of this loop as in some code paths we can have components register/unregister and invalidate the SortedIdx.
		if (ScalabilitySettings.CullProxyMode != ENiagaraCullProxyMode::None)
		{
			FCullProxyCreateDestroyTask& NewCullProxyTask = CullProxyTasks.AddDefaulted_GetRef();			
			NewCullProxyTask.Component = Component;			
			NewCullProxyTask.bCreate = false;
			if (CompState.bCulled)
			{
				...
				if (bEnableCullProxy)
				{
					...
					NewCullProxyTask.bCreate = true;
				}
			}
		}
	}

	//Process any cull proxy creation / destruction here so as to not invalidate the ManagedComponents/SortedIdx as we process.
	for(FCullProxyCreateDestroyTask& CullProxyTask : CullProxyTasks)
	{
		if(IsValid(CullProxyTask.Component))
		{
			if (CullProxyTask.bCreate)
			{
				CullProxyTask.Component->CreateCullProxy(true);
			}
			else
			{
				CullProxyTask.Component->DestroyCullProxy();
			}
		}
	}
}
...
bool FNiagaraScalabilityManager::ApplyScalabilityState(int32 ComponentIndex, ENiagaraCullReaction CullReaction, bool bNewOnly)
{
	...
	if (UNiagaraComponent* Component = ManagedComponents[ComponentIndex])
	{
		...
		if (CompState.bCulled)
		{
			...
			switch (CullReaction)
			{
			case ENiagaraCullReaction::Deactivate:					Component->DeactivateInternal(false); bKilled = true; break;//We don't increment CompIdx here as this call will remove an entry from ManagedObjects;
			case ENiagaraCullReaction::DeactivateImmediate:			Component->DeactivateImmediateInternal(false); bKilled = true;  break; //We don't increment CompIdx here as this call will remove an entry from ManagedObjects;
...
#if WITH_NIAGARA_DEBUGGER
				WorldMan->GetSystemScalabilityKillCounts(System).Add(Component->DebugCachedScalabilityState);
#endif
			}
		}
		else
...
void FNiagaraScalabilityManager::UpdateInternal(FComponentIterationContext& Context)
{
	int32 UpdateCount = 0;
	if (Context.bProcessAllComponents)
	{
		...
		{
			if (EvaluateCullState(Context, ComponentIt, UpdateCount))
			{
				++ComponentIt;
			}
		}
	...
	else
	{
		for (TConstSetBitIterator<> ComponentIt(Context.ComponentRequiresUpdate); UpdateCount < Context.MaxUpdateCount && ComponentIt;)
		{
			...
			if (EvaluateCullState(Context, ComponentIt.GetIndex(), UpdateCount))
			{
				++ComponentIt;
			}
		}
	...
	if (Context.bProcessAllComponents || !Context.ComponentRequiresUpdate.Contains(true))
	{
		...
		{
			ProcessSignificance(EffectType->SignificanceHandler, Context);
		}

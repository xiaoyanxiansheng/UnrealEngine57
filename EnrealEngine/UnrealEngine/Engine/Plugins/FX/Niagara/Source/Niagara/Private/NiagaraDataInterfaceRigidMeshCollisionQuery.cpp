...
static const FName GetBoxElementsStartIndexName(TEXT("GetBoxElementsStartIndex"));
static const FName GetSphereElementsStartIndexName(TEXT("GetSphereElementsStartIndex"));
static const FName GetCapsuleElementsStartIndexName(TEXT("GetCapsuleElementsStartIndex"));
static const FName GetSphereRadiusName(TEXT("GetSphereRadius"));
static const FName GetCapsuleSizeName(TEXT("GetCapsuleSize"));
static const FName GetBoxSizeName(TEXT("GetBoxSize"));
	...
	for (const FNDIRigidMeshCollisionData::FComponentBodyCount& CurrentCount : CurrentCounts)
	{
		...
		while (PreviousCounts.IsValidIndex(PreviousIndex) && !bPreviousValuesCopied)
		{
			...
			if ((PreviousCount.ComponentHash == CurrentCount.ComponentHash)
				&& (PreviousCount.BoxCount == CurrentCount.BoxCount)
				&& (PreviousCount.SphereCount == CurrentCount.SphereCount)
				&& (PreviousCount.CapsuleCount == CurrentCount.CapsuleCount))
			{
...
void UNiagaraDataInterfaceRigidMeshCollisionQuery::GetFunctionsInternal(TArray<FNiagaraFunctionSignature>& OutFunctions) const
{
	...
	{
		FNiagaraFunctionSignature Sig;
		Sig.Name = GetBoxElementsStartIndexName;
		Sig.SetDescription(LOCTEXT("GetBoxElementsStartIndexNameDescription", "Returns the Index of the first of box primitive for the collection of static meshes the DI represents."));
		Sig.SetFunctionVersion(FNiagaraRigidMeshCollisionDIFunctionVersion::LatestVersion);
		Sig.bSupportsGPU = true;
		Sig.bSupportsCPU = false;
		Sig.bMemberFunction = true;
	...
	{
		FNiagaraFunctionSignature Sig;
		Sig.Name = GetSphereElementsStartIndexName;
		Sig.SetDescription(LOCTEXT("GetSphereElementsStartIndexNameDescription", "Returns the Index of the first of sphere primitive for the collection of static meshes the DI represents."));
		Sig.SetFunctionVersion(FNiagaraRigidMeshCollisionDIFunctionVersion::LatestVersion);
		Sig.bSupportsGPU = true;
		Sig.bSupportsCPU = false;
		Sig.bMemberFunction = true;
	...
	{
		FNiagaraFunctionSignature Sig;
		Sig.Name = GetCapsuleElementsStartIndexName;
		Sig.SetDescription(LOCTEXT("GetCapsuleElementsStartIndexNameDescription", "Returns the Index of the first of capsule primitive for the collection of static meshes the DI represents."));
		Sig.SetFunctionVersion(FNiagaraRigidMeshCollisionDIFunctionVersion::LatestVersion);
		Sig.bSupportsGPU = true;
		Sig.bSupportsCPU = false;
		Sig.bMemberFunction = true;
	...
	{
		FNiagaraFunctionSignature Sig;
		Sig.Name = GetSphereRadiusName;
		Sig.SetDescription(LOCTEXT("GetSphereRadiusNameDescription", "Returns the radius of a sphere primitive in the collection of static meshes the DI represents."));
		Sig.SetFunctionVersion(FNiagaraRigidMeshCollisionDIFunctionVersion::LatestVersion);
		Sig.bSupportsGPU = true;
		Sig.bSupportsCPU = false;
		Sig.bMemberFunction = true;
	...
	{
		FNiagaraFunctionSignature Sig;
		Sig.Name = GetCapsuleSizeName;
		Sig.SetDescription(LOCTEXT("GetCapsuleSizeNameDescription", "Returns the capsule size of a capsule primitive in the collection of static meshes the DI represents."));
		Sig.SetFunctionVersion(FNiagaraRigidMeshCollisionDIFunctionVersion::LatestVersion);
		Sig.bSupportsGPU = true;
		Sig.bSupportsCPU = false;
		Sig.bMemberFunction = true;
		Sig.Inputs.Add(FNiagaraVariable(FNiagaraTypeDefinition(GetClass()), TEXT("Collision DI")));
		Sig.Inputs.Add(FNiagaraVariable(FNiagaraTypeDefinition::GetIntDef(), TEXT("Element Index")));
		Sig.Outputs.Add(FNiagaraVariable(FNiagaraTypeDefinition::GetVec2Def(), TEXT("Size")));

		OutFunctions.Add(Sig);
	}
	{
		FNiagaraFunctionSignature Sig;
		Sig.Name = GetBoxSizeName;
		Sig.SetDescription(LOCTEXT("GetBoxSizeNameNameDescription", "Returns the Extents of a box primitive in the collection of static meshes the DI represents."));
		Sig.SetFunctionVersion(FNiagaraRigidMeshCollisionDIFunctionVersion::LatestVersion);
		Sig.bSupportsGPU = true;
		Sig.bSupportsCPU = false;
		Sig.bMemberFunction = true;
		Sig.Inputs.Add(FNiagaraVariable(FNiagaraTypeDefinition(GetClass()), TEXT("Collision DI")));
		Sig.Inputs.Add(FNiagaraVariable(FNiagaraTypeDefinition::GetIntDef(), TEXT("Element Index")));
		Sig.Outputs.Add(FNiagaraVariable(FNiagaraTypeDefinition::GetVec3Def(), TEXT("Size")));
...
bool UNiagaraDataInterfaceRigidMeshCollisionQuery::GetFunctionHLSL(const FNiagaraDataInterfaceGPUParamInfo& ParamInfo, const FNiagaraDataInterfaceGeneratedFunction& FunctionInfo, int FunctionInstanceIndex, FString& OutHLSL)
{
	...
	if ((FunctionInfo.DefinitionName == GetNumBoxesName) ||
		...
		(FunctionInfo.DefinitionName == GetBoxElementsStartIndexName) ||
		(FunctionInfo.DefinitionName == GetSphereElementsStartIndexName) ||
		(FunctionInfo.DefinitionName == GetCapsuleElementsStartIndexName) ||
		(FunctionInfo.DefinitionName == GetSphereRadiusName) ||
		(FunctionInfo.DefinitionName == GetCapsuleSizeName) ||
		(FunctionInfo.DefinitionName == GetBoxSizeName) ||

...
#define NDIARRAY_GENERATE_BODY(CLASSNAME, TYPENAME, MEMBERNAME) \
	...
	const TArray<TYPENAME>& GetArrayReference() const { return MEMBERNAME; } \
	...
	#define NDIARRAY_GENERATE_BODY_LWC(CLASSNAME, TYPENAME, MEMBERNAME) \
		...
		template<typename TFromArrayType> \
		void SetVariantArrayValue(int Index, const TFromArrayType& Value, bool bSizeToFit); \
		const TArray<TYPENAME>& GetArrayReference() const { return Internal##MEMBERNAME; } \
...
#if WITH_EDITOR
	NIAGARA_API void CreateAndSetVariant(UNiagaraComponent* NiagaraComponent, FName OverrideName, TFunction<void(UNiagaraDataInterfaceArray*)> SetDataDelegate);
#endif

	/** ReadWrite lock to ensure safe access to the underlying array. */
	FTransactionallySafeRWLock ArrayRWGuard;
...
public:
	...
	// Internal use only
	NIAGARA_API int32 FindOrAddData(TConstArrayView<uint8> ArrayData);

protected:
	UE::FMutex FindOrAddGuard;
};

...
namespace NDIRenderTarget2DLocal
{
	...
	static const FName ReleaseResourcesFunctionName("ReleaseResources");
...
struct FNDIRenderTarget2DFunctionVersion
{
	enum Type
	{
		...
		AddedMipLevel = 2,
		AddedOptionalExecuteToSetSize = 3,
...
void UNiagaraDataInterfaceRenderTarget2D::GetFunctionsInternal(TArray<FNiagaraFunctionSignature>& OutFunctions) const
{
	...
	{
		FNiagaraFunctionSignature& Sig = OutFunctions.Add_GetRef(DefaultSig);
		Sig.Name = ReleaseResourcesFunctionName;
		Sig.Inputs.Add_GetRef(FNiagaraVariable(FNiagaraTypeDefinition::GetBoolDef(), TEXT("Execute"))).SetValue(true);
		Sig.ModuleUsageBitmask = EmitterSystemOnlyBitmask;
		Sig.bRequiresExecPin = true;
		Sig.bSupportsGPU = false;
...
bool UNiagaraDataInterfaceRenderTarget2D::UpgradeFunctionCall(FNiagaraFunctionSignature& FunctionSignature)
{
	...
	if (FunctionSignature.FunctionVersion < FNDIRenderTarget2DFunctionVersion::AddedOptionalExecuteToSetSize)
	{
		if (FunctionSignature.Name == SetSizeFunctionName)
		{
			check(FunctionSignature.Inputs.Num() == 3);
			FunctionSignature.Inputs.Add_GetRef(FNiagaraVariable(FNiagaraTypeDefinition::GetBoolDef(), TEXT("Execute"))).SetValue(true);			
			bWasChanged = true;
		}
	}
...
void UNiagaraDataInterfaceRenderTarget2D::GetVMExternalFunction(const FVMExternalFunctionBindingInfo& BindingInfo, void* InstanceData, FVMExternalFunction& OutFunc)
{
	...
	else if (BindingInfo.Name == ReleaseResourcesFunctionName)
	{
		OutFunc = FVMExternalFunction::CreateUObject(this, &UNiagaraDataInterfaceRenderTarget2D::VMReleaseResources);
	}
...
void UNiagaraDataInterfaceRenderTarget2D::VMSetSize(FVectorVMExternalFunctionContext& Context)
{
	...
	for (int32 InstanceIdx = 0; InstanceIdx < Context.GetNumInstances(); ++InstanceIdx)
	{
		...
		const bool bSuccess = (InstData.Get() != nullptr && Context.GetNumInstances() == 1 && SizeX >= 0 && SizeY >= 0) && bExecute;
		OutSuccess.SetAndAdvance(bSuccess);
		if (bSuccess)
		{
			InstData->bReleaseResources = false;
...
void UNiagaraDataInterfaceRenderTarget2D::VMReleaseResources(FVectorVMExternalFunctionContext& Context)
{
	// This should only be called from a system or emitter script due to a need for only setting up initially.
	VectorVM::FUserPtrHandler<FRenderTarget2DRWInstanceData_GameThread> InstData(Context);
	VectorVM::FExternalFuncInputHandler<FNiagaraBool> InExecute(Context);
	...
	for (int32 InstanceIdx = 0; InstanceIdx < Context.GetNumInstances(); ++InstanceIdx)
	{
		...
		if (bExecute)
		{
			InstData->bReleaseResources = true;
			InstData->Size.X = 1;
			InstData->Size.Y = 1;
		}
	}
...
bool UNiagaraDataInterfaceRenderTarget2D::PerInstanceTick(void* PerInstanceData, FNiagaraSystemInstance* SystemInstance, float DeltaSeconds)
{
	...
	FRenderTarget2DRWInstanceData_GameThread* InstanceData = static_cast<FRenderTarget2DRWInstanceData_GameThread*>(PerInstanceData);
	if (InstanceData->bReleaseResources == false)
	{
...
bool UNiagaraDataInterfaceRenderTarget2D::PerInstanceTickPostSimulate(void* PerInstanceData, FNiagaraSystemInstance* SystemInstance, float DeltaSeconds)
{
	...
	if (InstanceData->bReleaseResources)
	{
	...
	if (bUpdateRT)
	{
		...
		ensureMsgf(InstanceData->bReleaseResources || RT_TargetTexture != nullptr, TEXT("NiagaraDataInterfaceRenderTarget2D - null RT for system (%s)"), *GetNameSafe(SystemInstance->GetSystem()));

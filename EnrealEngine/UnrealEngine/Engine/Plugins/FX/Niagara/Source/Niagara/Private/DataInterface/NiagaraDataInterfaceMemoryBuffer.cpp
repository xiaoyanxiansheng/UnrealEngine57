...
namespace NDIMemoryBufferLocal
{
	...
	void VMSetNumElements(FVectorVMExternalFunctionContext& Context)
	{
		...
		const int32 MaxElements = (int32)FMath::Min<int64>(MaxViewDimensionForTypedBuffer, TNumericLimits<int32>::Max());
...
bool UNiagaraDataInterfaceMemoryBuffer::SimCacheReadFrame(const FNiagaraSimCacheDataInterfaceReadContext& ReadContext)
{
	using namespace NDIMemoryBufferLocal;

	UNDIMemoryBufferSimCacheData* CacheData = ReadContext.GetStorageObject<UNDIMemoryBufferSimCacheData>();

	const int FrameIndex = ReadContext.GetFrameIndexA();

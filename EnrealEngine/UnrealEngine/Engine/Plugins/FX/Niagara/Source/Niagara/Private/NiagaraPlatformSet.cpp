...
#if WITH_EDITOR
FCriticalSection FNiagaraPlatformSet::CachedPlatformIniCritSec;
...
{
#if WITH_EDITOR
	FScopeLock Lock(&CachedPlatformIniCritSec);
	...
	if (!ensureMsgf(IsInGameThread(), TEXT("GetActiveQualityMaskForDeviceProfile is attempting to access the config system outside of the game thread, results may be incorrect.")))
	{
		return INDEX_NONE;
	}
...
bool FNiagaraPlatformSetCVarCondition::Evaluate(const UDeviceProfile* DeviceProfile, int32 QualityLevel, bool bCheckCurrentStateOnly, bool& bOutCanEverPass, bool& bOutCanEverFail)const
{
	if (IConsoleVariable* CVar = GetCVar())
	{
		...
		{
			ensureMsgf(false, TEXT("CVar %s is of an unsupported type for FNiagaraPlatformSetCVarCondition. Supported types are Bool, Int and Float. This should not be possible unless the CVar's type has been chagned."), *CVarName.ToString());
		}
	}
	else
	{

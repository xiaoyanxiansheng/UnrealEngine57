...
#if WITH_EDITORONLY_DATA	
	if(NiagaraVer < FNiagaraCustomVersion::Type::UserAssetTagStorageInMetaData && AssetTags_DEPRECATED.Num() > 0)
	{
...
void FVersionedNiagaraEmitterData::PostLoad(UNiagaraEmitter& Emitter, int32 NiagaraVer)
{
	...
	if (SpawnScriptProps.Script)
	{
		const bool bActualInterpolatedSpawning = SpawnScriptProps.Script->IsInterpolatedParticleSpawnScript();
		const bool bUsesInterpolatedSpawning = InterpolatedSpawnMode != ENiagaraInterpolatedSpawnMode::NoInterpolation;
		if (bActualInterpolatedSpawning != bUsesInterpolatedSpawning)
		{
...
#if WITH_EDITORONLY_DATA
void UNiagaraEmitter::OnUniqueEmitterNameChanged(const FString& OldName)
{
...
void UNiagaraEmitter::HandleVariableRenamed(const FNiagaraVariable& InOldVariable, const FNiagaraVariable& InNewVariable, bool bUpdateContexts, FGuid EmitterVersion)
{
	...
	{
		Prop->Modify(false);
		Prop->RenameVariable(InOldVariable, InNewVariable, FVersionedNiagaraEmitterBase(this, EmitterVersion));
	}
	...
	if (bUpdateContexts)
	{
		FNiagaraSystemUpdateContext UpdateCtx(FVersionedNiagaraEmitterBase(this, EmitterVersion), true);
	}
}
...
void UNiagaraEmitter::HandleVariableRemoved(const FNiagaraVariable& InOldVariable, bool bUpdateContexts, FGuid EmitterVersion)
{
	...
	{
		Prop->Modify(false);
		Prop->RemoveVariable(InOldVariable, FVersionedNiagaraEmitterBase(this, EmitterVersion));
	}
...
FNiagaraVariableBase UNiagaraEmitter::GetResolvedDIBinding(const FNiagaraVariableBase& InVariable) const
{
	const FNiagaraVariableBase* ResolvedDIBinding = GetLatestEmitterData()->ResolvedDIBindings.Find(InVariable);
	return ResolvedDIBinding ? *ResolvedDIBinding : InVariable;
}

...
UNiagaraComponentRendererProperties::UNiagaraComponentRendererProperties()
	: ComponentCountLimit(15)
	, bAssignComponentsOnParticleID(true)
	, bOnlyActivateNewlyAquiredComponents(true)
...
{
	// this callback is only registered on the CDO, so we inform all the regular instances like this
	for (TObjectIterator<UNiagaraComponentRendererProperties> Itt; Itt; ++Itt)
	{
		UNiagaraComponentRendererProperties* PropertiesObj = *Itt;
		if (PropertiesObj && PropertiesObj != this)
		{
			PropertiesObj->OnObjectsReplacedInternal(ReplacementsMap);
		}
	}
}

void UNiagaraComponentRendererProperties::OnObjectsReplacedInternal(const TMap<UObject*, UObject*>& ReplacementsMap)
{

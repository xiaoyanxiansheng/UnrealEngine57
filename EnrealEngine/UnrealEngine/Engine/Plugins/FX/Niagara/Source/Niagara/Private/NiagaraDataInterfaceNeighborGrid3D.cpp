...
bool UNiagaraDataInterfaceNeighborGrid3D::InitPerInstanceData(void* PerInstanceData, FNiagaraSystemInstance* SystemInstance)
{
	...
	else if (NumTotalCells > MaxViewDimensionForTypedBuffer)
	{
		UE_LOG(LogNiagara, Error, TEXT("NiagaraDataInterfaceNeighborGrid3D - Invalid NumCells(%dx%dx%dx%d) with total = %u exceeds the platform maximum of %llu for '%s'.  Please reduce the memory usage."), RT_NumCells.X, RT_NumCells.Y, RT_NumCells.Z, RT_MaxNeighborsPerCell, NumTotalCells, MaxViewDimensionForTypedBuffer, *GetFullNameSafe(this));
		return false;
	}
...
void UNiagaraDataInterfaceNeighborGrid3D::SetNumCells(FVectorVMExternalFunctionContext& Context)
{
	...
	for (int32 InstanceIdx = 0; InstanceIdx < Context.GetNumInstances(); ++InstanceIdx)
	{
		...
		{
			UE_LOG(LogNiagara, Error, TEXT("NiagaraDataInterfaceNeighborGrid3D - Invalid NumCells(%dx%dx%dx%d) with total = %u exceeds the platform maximum of %d for '%s'.  Please reduce the memory usage."), NewNumCellsX, NewNumCellsY, NewNumCellsZ, NewMaxNeighborsPerCell, NumTotalCells, MaxViewDimensionForTypedBuffer, *GetFullNameSafe(this));
			bSuccess = false;
		}

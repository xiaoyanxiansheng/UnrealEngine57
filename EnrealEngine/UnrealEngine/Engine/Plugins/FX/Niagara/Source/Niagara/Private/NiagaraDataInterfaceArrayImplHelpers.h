...
struct FNDIArrayImplHelper<float> : public FNDIArrayImplHelperBase<float>
{
	...
	static void CopyCpuToCpuMemory(float* Dest, const float* Src, int32 NumElements)
	{
		FMemory::Memcpy(Dest, Src, NumElements * sizeof(float));
	}

	static void CopyCpuToCpuMemory(float* Dest, const double* Src, int32 NumElements)
	{
...
struct FNDIArrayImplHelper<int32> : public FNDIArrayImplHelperBase<int32>
{
	...
	static void CopyCpuToCpuMemory(int32* Dest, const int32* Src, int32 NumElements)
	{
		FMemory::Memcpy(Dest, Src, NumElements * sizeof(int32));
	}

	static void CopyCpuToCpuMemory(int32* Dest, const int64* Src, int32 NumElements)
	{
		for (int32 i = 0; i < NumElements; ++i)
		{
			Dest[i] = int32(Src[i]);
...
struct FNDIArrayImplHelper<bool> : public FNDIArrayImplHelperBase<bool>
{
	...
	static bool IsNearlyEqual(bool Lhs, bool Rhs, float Tolerance)
	{
		return !!Lhs == !!Rhs;
	}
};

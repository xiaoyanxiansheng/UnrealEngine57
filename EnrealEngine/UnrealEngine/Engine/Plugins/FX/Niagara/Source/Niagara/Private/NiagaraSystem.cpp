...
namespace NiagaraSystemPrivate
{
	...
	bool GCleanStatelessEmitterHandles = true;
	static FAutoConsoleVariableRef CVarCleanStatelessEmitterHandles(
		TEXT("fx.Niagara.CleanStatelessEmitterHandles"),
		GCleanStatelessEmitterHandles,
		TEXT("When enabled stateless emitter handles will clean the stateful emitter during cook to save memory."),
		ECVF_Default
	);
...
static int GNiagaraObjectNeedsLoadMode = 0;
...
void UNiagaraSystem::HandleVariableRenamed(const FNiagaraVariable& InOldVariable, const FNiagaraVariable& InNewVariable, bool bUpdateContexts)
{
	...
	{
		if (UNiagaraEmitterBase* Emitter = Handle.GetEmitterBase())
		{
			FVersionedNiagaraEmitter VersionedEmitter = Handle.GetInstance();
			Emitter->HandleVariableRenamed(InOldVariable, InNewVariable, false, VersionedEmitter.Version);
		}
	}
...
void UNiagaraSystem::HandleVariableRemoved(const FNiagaraVariable& InOldVariable, bool bUpdateContexts)
{
	...
	for (const FNiagaraEmitterHandle& Handle : GetEmitterHandles())
	{
		...
		{
			FVersionedNiagaraEmitter VersionedEmitter = Handle.GetInstance();
			Emitter->HandleVariableRemoved(InOldVariable, false, VersionedEmitter.Version);
		}
	}
	if (bUpdateContexts)
	{
...
bool UNiagaraSystem::ChangeEmitterVersion(const FVersionedNiagaraEmitter& VersionedEmitter, const FGuid& NewVersion)
{
	...
	for (int i = 0; i < EmitterHandles.Num(); i++)
	{
		...
		if (Handle.GetInstance() == VersionedEmitter)
		{
			KillAllActiveCompilations();
			Modify();
			EmitterHandles[i].SetInstanceVersion(NewVersion);
			return true;
		}
	}
	return false;
...
#if WITH_EDITORONLY_DATA
	if (SystemSpawnScript && SystemUpdateScript && !GetOutermost()->bIsCookedForEditor && ensure(SystemSpawnScript->GetLatestSource() != nullptr))
	{
		ensure(SystemSpawnScript->GetLatestSource() == SystemUpdateScript->GetLatestSource());
		SystemSpawnScript->GetLatestSource()->OnChanged().AddUObject(this, &UNiagaraSystem::GraphSourceChanged);
	}
...
void UNiagaraSystem::ReportAnalyticsData(bool bIsCooking)
{
	if (FEngineAnalytics::IsAvailable() && IsAsset() && (bIsCooking == false || GNiagaraReportOnCook))
	{
		...
		FNiagaraDataInterfaceUtilities::ForEachDataInterface(this, [&](const FNiagaraDataInterfaceUtilities::FDataInterfaceUsageContext& UsageContext) -> bool
		{
...
bool UNiagaraSystem::UsesEmitter(UNiagaraEmitterBase* Emitter) const
{
	if (!Emitter)
	{
		return false;
...
bool UNiagaraSystem::UsesEmitter(const FVersionedNiagaraEmitterBase& VersionedEmitter) const
{
	if (!VersionedEmitter.Emitter)
	{
		return false;
...
void UNiagaraSystem::Serialize(FArchive& Ar)
{
	...
	// Temporary hack to remove the UNiagaraEmitter from stateless emitters
	// Currently the UI and various parts of the code do not handle the lack of an UNiagaraEmitter that will be fixed, for the moment we just remove them from cooked
	TArray<FVersionedNiagaraEmitter> TempVersionedEmitterStore;
	if (NiagaraSystemPrivate::GCleanStatelessEmitterHandles && Ar.IsSaving() && Ar.IsCooking() && Ar.CookingTarget() && !Ar.CookingTarget()->HasEditorOnlyData())
	{
		TempVersionedEmitterStore.AddDefaulted(EmitterHandles.Num());
		for (int32 i=0; i < EmitterHandles.Num(); ++i)
		{
			FNiagaraEmitterHandle& Handle = EmitterHandles[i];
			TempVersionedEmitterStore[i] = Handle.GetInstance();
			if ( Handle.GetEmitterMode() == ENiagaraEmitterMode::Stateless )
			{
				Handle.SetInstance(FVersionedNiagaraEmitter());
			}
		}
	}
	...
	if (TempVersionedEmitterStore.Num() > 0)
	{
		for (int32 i=0; i < EmitterHandles.Num(); ++i)
		{
			EmitterHandles[i].SetInstance(TempVersionedEmitterStore[i]);
		}
		TempVersionedEmitterStore.Empty();
	}
...
void UNiagaraSystem::PostLoad()
{
	...
	if (PackageOuter != nullptr && HasAnyFlags(RF_Public | RF_Standalone))
	{
		...
		{
			UNiagaraEmitterBase* Emitter = Cast<UNiagaraEmitterBase>(ObjectInPackage);
			if (Emitter != nullptr)
			{
...
#else
		if (UNiagaraEmitterBase* NiagaraEmitter = EmitterHandle.GetEmitterBase())
		{
...
#if WITH_EDITORONLY_DATA
	if(NiagaraVer < FNiagaraCustomVersion::UserAssetTagStorageInMetaData && AssetTags_DEPRECATED.Num() > 0)
	{
		INiagaraModule::Get().GetEditorOnlyDataUtilities().MoveUserAssetTagsToMetaData(this);
	}
#endif // WITH_EDITORONLY_DATA
...
void UNiagaraSystem::PrecachePSOs()
{
	...
	for (const FNiagaraEmitterHandle& EmitterHandle : GetEmitterHandles())
	{
		...
		EmitterHandle.ForEachEnabledRendererWithIndex(
			[&](const UNiagaraRendererProperties* Properties, int32 EmitterIndex)
			{
...
void UNiagaraSystem::ComputeEmittersExecutionOrder()
{
	...
	for (int32 EmitterIdx = 0; EmitterIdx < NumEmitters; ++EmitterIdx)
	{
		...
		EmitterHandle.ForEachEnabledRendererWithIndex(
			[&](const UNiagaraRendererProperties* RendererProperties, int32 RendererIndex)
			{
				...
				if (RendererProperties->NeedsSystemPostTick())
				{
					RendererPostTickOrder.Add(ExecutionIndex);
				}
				if (RendererProperties->NeedsSystemCompletion())
				{
	...
	{
		const FNiagaraEmitterHandle& EmitterHandle = EmitterHandles[EmitterExecIdx.EmitterIndex];
		return EmitterHandle.GetEmitterMode() == ENiagaraEmitterMode::Stateless ? EmitterHandle.GetStatelessEmitter() == nullptr : EmitterHandle.GetInstance().Emitter == nullptr;
	}));
}
...
void UNiagaraSystem::UpdatePostCompileDIInfo()
{
	...
	ForEachScript([&](UNiagaraScript* Script) 
	{
		for(const FNiagaraScriptResolvedDataInterfaceInfo& ResolvedDIInfo : Script->GetResolvedDataInterfaces())
		{
			...
			{
				ResolvedDIInfo.ResolvedDataInterface->PostCompile(*this);
			}
		}		
	});
}
	...
	if (bDoPostCompile)
	{
		for (FNiagaraEmitterHandle Handle : EmitterHandles)
		{
			if (Handle.GetInstance().Emitter)
			{
				...
				else if (FVersionedNiagaraEmitterData* EmitterData = Handle.GetEmitterData())
				{

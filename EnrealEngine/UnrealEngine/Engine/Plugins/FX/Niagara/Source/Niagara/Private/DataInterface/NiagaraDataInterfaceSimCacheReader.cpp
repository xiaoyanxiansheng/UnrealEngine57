...
namespace NDISimCacheReaderLocal
{
	...
	static const FName GetInstanceID("GetInstanceID");
	...
	struct FShaderStorage : public FNiagaraDataInterfaceParametersCS
	{
		...
		LAYOUT_FIELD(TMemoryImageArray<FMemoryImageName>, AttributeReadNames);
	};
	...
	struct FInstanceData_RenderThread
	{
		...
		TMap<uintptr_t, TArray<int32>>	StorageToComponentOffsets;
	};
	...
	TConstArrayView<FGetFunctionInfo> GetFunctionInfos()
	{
		static const FGetFunctionInfo Infos[] =
		{
			...
			FGetFunctionInfo(GetInstanceID,			FNiagaraTypeDefinition::GetIDDef(),			TEXT("NiagaraID"),	TEXT("GetInstanceID")),
		};
	...
	static int AttributeIndexFromVariable(const UNiagaraSimCache* SimCache, FName FunctionName, FName AttributeName, int EmitterIndex)
	{
	...
	static int ComponentIndexFromVariable(const UNiagaraSimCache* SimCache, FName FunctionName, FName AttributeName, int EmitterIndex)
	{
		const FNiagaraVariableBase Variable(FunctionNameToTypeDef(FunctionName), AttributeName);
		...
		{
			if (Variable.GetType() == FNiagaraTypeDefinition::GetIntDef() || Variable.GetType() == FNiagaraTypeDefinition::GetIDDef())
			{
...
bool UNiagaraDataInterfaceSimCacheReader::PerInstanceTick(void* PerInstanceData, FNiagaraSystemInstance* SystemInstance, float DeltaSeconds)
{
	...
	if (InstanceData_GT->WeakSimCache.Get() != InstanceSimCache)
	{
		...
		(
			[Proxy_RT=GetProxyAs<FNDIProxy>(), InstanceID_RT=SystemInstance->GetId(), EmitterIndex_RT=InstanceData_GT->EmitterIndex, SimCacheResource_RT=MoveTemp(SimCacheResource)](FRHICommandList& CmdList)
			{
...
void UNiagaraDataInterfaceSimCacheReader::GetVMExternalFunction(const FVMExternalFunctionBindingInfo& BindingInfo, void* PerInstanceData, FVMExternalFunction& OutFunc)
{
	...
	else if (BindingInfo.Name == GetInstanceID)
	{
		const int32 AttributeIndex = AttributeIndexFromVariable(InstanceSimCache, BindingInfo.Name, BindingInfo.FunctionSpecifiers[0].Value, InstanceData->EmitterIndex);
		OutFunc = FVMExternalFunction::CreateUObject(this, &UNiagaraDataInterfaceSimCacheReader::VMGetInstanceID, AttributeIndex, false);
	}
}
...
bool UNiagaraDataInterfaceSimCacheReader::AppendCompileHash(FNiagaraCompileHashVisitor* InVisitor) const
{
	...
	InVisitor->UpdateString(TEXT("FShaderStorage"), TEXT("V0"));
	return bSuccess;
}
...
FNiagaraDataInterfaceParametersCS* UNiagaraDataInterfaceSimCacheReader::CreateShaderStorage(const FNiagaraDataInterfaceGPUParamInfo& ParameterInfo, const FShaderParameterMap& ParameterMap) const
{
	using namespace NDISimCacheReaderLocal;

	FShaderStorage* ShaderStorage = new FShaderStorage();
	ShaderStorage->AttributeReadNames.Reserve(ParameterInfo.GeneratedFunctions.Num() * 2);
	for (const FNiagaraDataInterfaceGeneratedFunction& GeneratedFunction : ParameterInfo.GeneratedFunctions)
	{
		if (GeneratedFunction.Specifiers.Num() == 1)
		{
			ShaderStorage->AttributeReadNames.Add(GeneratedFunction.DefinitionName);
			ShaderStorage->AttributeReadNames.Add(GeneratedFunction.Specifiers[0].Value);
		}
		else
		{
			ShaderStorage->AttributeReadNames.Add(NAME_None);
			ShaderStorage->AttributeReadNames.Add(NAME_None);
		}
	}
...
const FTypeLayoutDesc* UNiagaraDataInterfaceSimCacheReader::GetShaderStorageType() const
{
	using namespace NDISimCacheReaderLocal;
...
void UNiagaraDataInterfaceSimCacheReader::SetShaderParameters(const FNiagaraDataInterfaceSetShaderParametersContext& Context) const
{
	...
	FInstanceData_RenderThread* InstanceData_RT = DIProxy.PerInstanceData_RenderThread.Find(Context.GetSystemInstanceID());
	...
	if (InstanceData_RT && InstanceData_RT->SimCacheResource->GetNumFrames() > 0)
	{
		...
		const uint32 NumAttributes4 = FMath::DivideAndRoundUp(FMath::Max(ShaderStorage.AttributeReadNames.Num() / 2, 1), 4);

		TArray<int32>& ComponentOffsets = InstanceData_RT->StorageToComponentOffsets.FindOrAdd(uintptr_t(&ShaderStorage));
		if (ComponentOffsets.IsEmpty())
		{
			const UNiagaraSimCache* InstanceSimCache = InstanceData_RT->SimCacheResource->GetSimCache();

			ComponentOffsets.AddZeroed(NumAttributes4 * 4);
			for (int32 i = 0; i < ShaderStorage.AttributeReadNames.Num() / 2; ++i)
			{
				const FName FunctionName = ShaderStorage.AttributeReadNames[(i * 2) + 0];
				const FName AttributeName = ShaderStorage.AttributeReadNames[(i * 2) + 1];
				const int32 ComponentIndex = FunctionName.IsNone() ? INDEX_NONE : ComponentIndexFromVariable(InstanceSimCache, FunctionName, AttributeName, InstanceData_RT->EmitterIndex);
				ComponentOffsets[i] = ComponentIndex;
			}
		}

		TArrayView<FIntVector4> ShaderComponentOffsets = Context.GetParameterLooseArray<FIntVector4>(NumAttributes4);
		FMemory::Memcpy(ShaderComponentOffsets.GetData(), ComponentOffsets.GetData(), ComponentOffsets.Num() * ComponentOffsets.GetTypeSize());
	}
	else
	{
...
void UNiagaraDataInterfaceSimCacheReader::VMGetInstanceID(FVectorVMExternalFunctionContext& Context, int AttributeIndex, bool bRebase)
{
	using namespace NDISimCacheReaderLocal;
	VMInstanceGetFunction<FNiagaraID>(Context, FNiagaraID(), AttributeIndex, [](FNiagaraSimCacheAttributeReaderHelper& AttributeReader, int32 InstanceIndex) { return AttributeReader.ReadID(InstanceIndex); });
}

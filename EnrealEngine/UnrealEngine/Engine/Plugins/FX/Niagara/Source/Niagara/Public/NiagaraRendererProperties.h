...
struct FNiagaraRendererVariableInfo
{
	...
	NIAGARA_API FNiagaraRendererVariableInfo(const FNiagaraDataSetCompiledData* CompiledData, const FNiagaraVariableBase& Variable);

	inline int32 GetNumComponents() const { return NumComponents; }

	inline int32 GetRawGPUOffset() const { return GPUBufferOffset == kInvalidOffset ? INDEX_NONE : GPUBufferOffset; }
	inline int32 GetGPUOffset() const { return GetRawGPUOffset() | (bHalfType ? (1 << 31) : 0); }

	inline int32 GetRawDatasetOffset() const { return DatasetOffset == kInvalidOffset ? INDEX_NONE : DatasetOffset; }
	inline int32 GetEncodedDatasetOffset() const { return GetRawDatasetOffset() | (bHalfType ? (1 << 31) : 0); }
...
struct FNiagaraRendererMaterialParameters
{
	...
	NIAGARA_API void RenameVariable(const FNiagaraVariableBase& OldVariable, const FNiagaraVariableBase& NewVariable, const FVersionedNiagaraEmitterBase& InEmitter, ENiagaraRendererSourceDataMode SourceMode);
	NIAGARA_API void RemoveVariable(const FNiagaraVariableBase& OldVariable, const FVersionedNiagaraEmitterBase& InEmitter, ENiagaraRendererSourceDataMode SourceMode);
...
public:
	...
	*/
	...
	NIAGARA_API virtual void RenameVariable(const FNiagaraVariableBase& OldVariable, const FNiagaraVariableBase& NewVariable, const FVersionedNiagaraEmitterBase& InEmitter);
	NIAGARA_API virtual void RemoveVariable(const FNiagaraVariableBase& OldVariable, const FVersionedNiagaraEmitterBase& InEmitter);
	...
	/**
	Provides feedback on any material usage issues, i.e. incompatable material assignment or missing usage flags
	Automaticaly called from GetRendererFeedback where OutFeedback results in Errors
	*/
	virtual void GetRendererMaterialUsageFeedback(TArray<FNiagaraRendererFeedback>& OutFeedback) const {}
	...
	NIAGARA_API bool SupportsEmitterMode() const;
	NIAGARA_API ENiagaraRendererSourceDataMode ValidateSourceMode(ENiagaraRendererSourceDataMode InSourceMode) const;
	...
	[[nodiscard]] NIAGARA_API FVersionedNiagaraEmitter GetOuterEmitter() const;
	[[nodiscard]] NIAGARA_API FVersionedNiagaraEmitterBase GetOuterEmitterBase() const;

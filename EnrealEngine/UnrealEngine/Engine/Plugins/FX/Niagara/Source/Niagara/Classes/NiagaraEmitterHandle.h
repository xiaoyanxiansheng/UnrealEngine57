...
public:
	...
	[[nodiscard]] NIAGARA_API const FVersionedNiagaraEmitter GetInstance() const;
	NIAGARA_API void SetInstance(const FVersionedNiagaraEmitter& VersionedData);
	NIAGARA_API void SetInstanceVersion(const FGuid& InVersion);
	...
	/** Iterate over all the enabled renderers on the emitter */
	NIAGARA_API void ForEachEnabledRendererWithIndex(TFunction<void(const class UNiagaraRendererProperties*, int32)> Delegate) const;
	...
	NIAGARA_API bool UsesEmitter(const FVersionedNiagaraEmitterBase& InEmitter) const;
	NIAGARA_API bool UsesEmitter(const UNiagaraEmitterBase& InEmitter) const;

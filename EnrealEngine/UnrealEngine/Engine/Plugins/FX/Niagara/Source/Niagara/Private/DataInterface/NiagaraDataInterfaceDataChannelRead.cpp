...
#include UE_INLINE_GENERATED_CPP_BY_NAME(NiagaraDataInterfaceDataChannelRead)
...
// 		else
	{
		...
		if (DataChannelPtr)
		{
			...
			{
				FNDCAccessContextInst& AccessContext = DataChannelPtr->GetTransientAccessContext();
				if(AccessContext.IsValid() && AccessContext.GetScriptStruct() != FNDCAccessContextLegacy::StaticStruct())
				{
					if(!Interface->bAutoLinkToSpawningNDC)
					{
						//TODO: Copy over the interface custom AccessContext. If this proves slow we can possibly cache in per instance data.
						AccessContext = Interface->AccessContext;
					}
					
					FNDCAccessContextBase& ContextTyped = AccessContext.GetChecked<FNDCAccessContextBase>();
					ContextTyped.OwningComponent = Instance->GetAttachComponent();
					ContextTyped.bIsAutoLinkingSystem = Interface->bAutoLinkToSpawningNDC;
					SetDataChannelData(DataChannelPtr->FindData(AccessContext, ENiagaraResourceAccess::ReadOnly), Interface);//TODO: Maybe should have two paths, one for system instances and another for SceneComponents...
				}
				else
				{
					//Legacy NDC so just use old path.
					FNiagaraDataChannelSearchParameters SearchParams(Instance->GetAttachComponent());
					SetDataChannelData(DataChannelPtr->FindData(SearchParams, ENiagaraResourceAccess::ReadOnly), Interface);
				}
			}	
...
void UNiagaraDataInterfaceDataChannelRead::PostLoad()
{
	Super::PostLoad();
...
#if WITH_EDITOR
void UNiagaraDataInterfaceDataChannelRead::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);

	if (PropertyChangedEvent.GetPropertyName() == GET_MEMBER_NAME_CHECKED(UNiagaraDataInterfaceDataChannelRead, Channel) ||
		PropertyChangedEvent.GetPropertyName() == GET_MEMBER_NAME_CHECKED(UNiagaraDataInterfaceDataChannelRead, bAutoLinkToSpawningNDC))
	{
		InitAccessContext();
	}
}
...
void UNiagaraDataInterfaceDataChannelRead::InitAccessContext()
{
	UNiagaraDataChannel* NDC = Channel ? Channel->Get() : nullptr;
	if (NDC && !bAutoLinkToSpawningNDC)
	{
		...
		else if (AccessContext.IsValid() == false || CtxStruct != AccessContext.GetScriptStruct())
		{
...
void UNiagaraDataInterfaceDataChannelRead::PostCompile(const UNiagaraSystem& OwningSystem)
{
...
bool UNiagaraDataInterfaceDataChannelRead::Equals(const UNiagaraDataInterface* Other)const
{
	...
	if (const UNiagaraDataInterfaceDataChannelRead* OtherTyped = CastChecked<UNiagaraDataInterfaceDataChannelRead>(Other))
	{
		if (Super::Equals(Other) &&
			...
			bAutoLinkToSpawningNDC == OtherTyped->bAutoLinkToSpawningNDC &&
...
bool UNiagaraDataInterfaceDataChannelRead::CopyToInternal(UNiagaraDataInterface* Destination)const
{
	...
	if (UNiagaraDataInterfaceDataChannelRead* DestTyped = CastChecked<UNiagaraDataInterfaceDataChannelRead>(Destination))
	{
		...
		DestTyped->bAutoLinkToSpawningNDC = bAutoLinkToSpawningNDC;
...
void UNiagaraDataInterfaceDataChannelRead::SpawnConditional(FVectorVMExternalFunctionContext& Context, int32 FuncIdx)
{
	...
	FNiagaraDataBuffer* Data = DataChannelData ? DataChannelData->GetCPUData(bReadPrevFrame).GetReference() : nullptr;

...
public:
	...
	[[nodiscard]] const UNiagaraRendererProperties* GetRenderer(int32 i) const
	{
		TConstArrayView<UNiagaraRendererProperties*> LocalRenderers = GetRenderers();
		return LocalRenderers.IsValidIndex(i) ? LocalRenderers[i] : nullptr;
	}
...
public:
	...
	UE_DEPRECATED(5.4, "Please update your code to handle execution path for different emitter types")
	inline  FVersionedNiagaraEmitter GetCachedEmitter() const { return VersionedEmitter; }

	UE_DEPRECATED(5.4, "Please update your code to handle execution path for different emitter types")
	inline FVersionedNiagaraEmitterData* GetCachedEmitterData()const { return VersionedEmitter.GetEmitterData(); }

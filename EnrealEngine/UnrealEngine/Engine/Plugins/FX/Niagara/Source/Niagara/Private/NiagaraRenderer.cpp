...
public:
	...
	virtual void InitRHI(FRHICommandListBase& RHICmdList) override
	{
		FRHITextureCreateDesc Desc = FRHITextureCreateDesc(*DebugName, Dimension)
			.SetExtent(1, 1)
			.SetDepth(1)
			.SetArraySize(1)
			.SetFormat(PixelFormat)
		...
		switch (Dimension)
		{
			...
			{
				FRHITextureInitializer Initializer = RHICmdList.CreateTextureInitializer(Desc.SetInitActionInitializer());
				FRHITextureSubresourceInitializer Subresource = Initializer.GetSubresource(FRHITextureInitializer::FSubresourceIndex());
				FMemory::Memzero(Subresource.Data, Subresource.Size);
				Texture = Initializer.Finalize();
				break;
			}

			case ETextureDimension::Texture3D:
			{
				FTextureRHIRef Texture3D = RHICmdList.CreateTexture(Desc);
				Texture = Texture3D;

				const FPixelFormatInfo& Info = GPixelFormats[PixelFormat];
				TArray<uint8, TInlineAllocator<16>> Data;
				Data.AddZeroed(Info.BlockBytes);
				RHICmdList.UpdateTexture3D(Texture3D, 0, FUpdateTextureRegion3D(0, 0, 0, 0, 0, 0, 1, 1, 1), Info.BlockBytes, Info.BlockBytes, Data.GetData());
				break;
			}
...
class FNiagaraEmptyByteAddressBufferSRV : public FRenderResource
{
public:
	FNiagaraEmptyByteAddressBufferSRV(const FString& InDebugName) : DebugName(InDebugName) {}
	...
	virtual void InitRHI(FRHICommandListBase& RHICmdList) override
	{
		constexpr int32 NumBytes = sizeof(FUintVector4);

		FRHIBufferCreateDesc CreateDesc =
			FRHIBufferCreateDesc::CreateByteAddress(*DebugName, NumBytes, 0)
...
static TGlobalResource<FNiagaraEmptyByteAddressBufferSRV> DummyByteAddressBuffer(TEXT("NiagaraRenderer::DummyByteAddressBuffer"));
...
FRHIShaderResourceView* FNiagaraRenderer::GetDummyByteAddressBuffer()
{
	return DummyByteAddressBuffer.SRV;
}
...
void FNiagaraRenderer::Initialize(const UNiagaraRendererProperties* InProps, const FNiagaraEmitterInstance* Emitter, const FNiagaraSystemInstanceController& InController)
{
	...
	for (UMaterialInterface*& Mat : BaseMaterials_GT)
	{
		...
		if (Mat)
		{
			BaseMaterialRelevance_GT |= Mat->GetRelevance_Concurrent(GetFeatureLevelShaderPlatform_Checked(FeatureLevel));
				...
				if (MatDyn)
				{
					for (const FNiagaraMaterialAttributeBinding& Binding : MaterialParameters.AttributeBindings)
					{
							...
							)
						{
							...
							// Not a valid texture?  Setting nullptr will not remove the override and set to default.
							// We have to do this manually getting the parent material default.
							else if ( const UMaterial* ParentMaterial = MatDyn->GetMaterial() )
							{
								ParentMaterial->GetTextureParameterValue(Binding.MaterialParameterName, Tex, false);
								MatDyn->SetTextureParameterValue(Binding.MaterialParameterName, Tex);
							}
						}
					}

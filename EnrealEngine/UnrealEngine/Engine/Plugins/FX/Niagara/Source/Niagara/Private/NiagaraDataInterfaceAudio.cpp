	...
	if (CastedDestination)
	{
		...
		else
		{
			AsyncTask(
				...
				[WeakSubmixDI=MakeWeakObjectPtr(CastedDestination)]()
				{
					if (UNiagaraDataInterfaceAudioSubmix* SubmixDI = WeakSubmixDI.Get())
					{
						SubmixDI->GetProxyAs<FNiagaraDataInterfaceProxySubmix>()->OnUpdateSubmix(SubmixDI->Submix);
					}
				}
			);
		}

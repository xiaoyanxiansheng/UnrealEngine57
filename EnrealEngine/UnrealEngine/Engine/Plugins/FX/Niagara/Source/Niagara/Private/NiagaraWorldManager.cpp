...
{
	//Rollback gate for new world cleanup flow.
	//Scheduled for removal in UE-347574
	bool NewWorldCleanup = true;
	static FAutoConsoleVariableRef CVarRollback_NewWorldCleanup(
		TEXT("fx.Niagara.Rollback.NewWorldCleanup"),
		NewWorldCleanup,
		TEXT("When true we'll use the new World Manager Cleanup flows. False will use old legacy code."),
		ECVF_Default
	);
	
	//Rollback gate for world initialization changes.
	//Scheduled for removal in UE-348784
	bool WorldManagerRespectsWorldCreateFXSystem = false;
	static FAutoConsoleVariableRef CVarRollback_WorldManagerRespectsWorldCreateFXSystem(
		TEXT("fx.Niagara.Rollback.WorldManagerRespectsWorldCreateFXSystem"),
		WorldManagerRespectsWorldCreateFXSystem,
		TEXT("When true, Niagara world managers will only be created for worlds with bCreateFXSystem==true in it's initialization options."),
		ECVF_Default
	);
	
	//Rollback gate for delaying NDC init. Avoids some cases where UWorlds are created but not "real" UWorlds. 
	//They don't need FX and having NDC managers created that themselves reference the world can cause some GC issues.
	//Scheduled for removal in UE-348786
	bool DelayNDCManagerInitialiation = true;
	static FAutoConsoleVariableRef CVarRollback_DelayNDCManagerInitialiation(
		TEXT("fx.Niagara.Rollback.DelayNDCManagerInitialiation"),
		DelayNDCManagerInitialiation,
		TEXT("When true, Niagara will not initialize it's NDC managers until the first tick of the world manager or first NDC use."),
		ECVF_Default
	);
}
...
FDelegateHandle FNiagaraWorldManager::TickWorldEndHandle;
...
void FNiagaraWorldManager::Init(UWorld* InWorld)
{
	...
	LastRenderThreadUpdateTime = MakeShared<std::atomic<float>>();
	LastRenderThreadUpdateTime->store(InWorld->GetTimeSeconds());
...
{
	if(NiagaraRollbackCVars::DelayNDCManagerInitialiation == false)
	{
...
void FNiagaraWorldManager::OnStartup()
{
	...
	TickWorldEndHandle = FWorldDelegates::OnWorldTickEnd.AddStatic(&FNiagaraWorldManager::TickWorldEnd);
...
void FNiagaraWorldManager::OnShutdown()
{
	...
	FWorldDelegates::OnWorldTickStart.Remove(TickWorldStartHandle);
	FWorldDelegates::OnWorldTickEnd.Remove(TickWorldEndHandle);
...
UNiagaraCullProxyComponent* FNiagaraWorldManager::GetCullProxy(UNiagaraComponent* Component)
{
	...
	//Don't create or return the cull proxy if the world is cleaning up.
	bool bIsWorldCleaningUp = NiagaraRollbackCVars::NewWorldCleanup && (bIsTearingDown || bIsCleaningUp);
	if (ensure(System) && GNiagaraAllowCullProxies && bIsWorldCleaningUp == false)
	{
...
void FNiagaraWorldManager::CleanupTransientObjectsAndManagers()
{
	//TODO: Move as much of cleanup as possible in here.
	//Some world cleanup flows don't call BeginTearDown or even OnWorldCleanup sometimes so anything that could be cleaned up immediately on teardown should go in here.
	if (ComponentPool)
	{
...
{
	bIsTearingDown = true;
	if(NiagaraRollbackCVars::NewWorldCleanup)
	{
		CleanupTransientObjectsAndManagers();
	}
	else
	{
		LegacyOnWorldBeginTearDown();
	}
}
...
void FNiagaraWorldManager::OnWorldCleanup(bool bSessionEnded, bool bCleanupResources)
{
	...
	{
		LegacyOnWorldCleanup(bSessionEnded, bCleanupResources);
	}
}
...
{	
	if(NiagaraRollbackCVars::NewWorldCleanup)
	{
		//Ensure we've already cleaned everything up. In some cases OnWorldCleanup doesn't appear to be called by the world cleanup flows.
		OnWorldCleanup(bSessionEnded, bCleanupResources);
	}
	else
	{
		LegacyOnPostWorldCleanup(bSessionEnded, bCleanupResources);
	}
}
...
//Legacy cleanup code.
//Scheduled for removal in UE-347574
void FNiagaraWorldManager::LegacyOnWorldBeginTearDown()
{
	bIsTearingDown = true;
	if (ComponentPool)
	{
...
void FNiagaraWorldManager::LegacyOnWorldCleanup(bool bSessionEnded, bool bCleanupResources)
{
	bIsCleaningUp = true;
...
void FNiagaraWorldManager::LegacyOnPostWorldCleanup(bool bSessionEnded, bool bCleanupResources)
{
	if (ComponentPool)
	{
		ComponentPool->Cleanup(World);
...
void FNiagaraWorldManager::PostGarbageCollect()
{
	...
	#if WITH_NIAGARA_DEBUGGER
	for (auto It = SystemScalabilityKillCounts.CreateIterator(); It; ++It)
	{
...
{
	if(IVS.bCreateFXSystem || !NiagaraRollbackCVars::WorldManagerRespectsWorldCreateFXSystem)
	{
...
{
	if(FNiagaraWorldManager* WorldMan = Get(World))
	{
		WorldMan->TickStart(DeltaSeconds);
	}	
}

void FNiagaraWorldManager::TickWorldEnd(UWorld* World, ELevelTick TickType, float DeltaSeconds)
{
	if(FNiagaraWorldManager* WorldMan = Get(World))
	{
		WorldMan->TickEnd(DeltaSeconds);
	}
}
...
void FNiagaraWorldManager::TickWorld(UWorld* World, ELevelTick TickType, float DeltaSeconds)
{
	if(FNiagaraWorldManager* WorldMan = Get(World))
	{
		WorldMan->PostActorTick(DeltaSeconds);
	}
}

void FNiagaraWorldManager::OnWorldPreActorTick(UWorld* InWorld, ELevelTick InLevelTick, float InDeltaSeconds)
{
	if(FNiagaraWorldManager* WorldMan = Get(InWorld))
	{
		WorldMan->PreActorTick(InLevelTick, InDeltaSeconds);
	}
}
...
void FNiagaraWorldManager::TickStart(float DeltaSeconds)
{
	...
	//Have the render thread signal the most recent world time it has consumed.
	//This will cause a several frame delay in render based visibility culling.
	//However it will make the culling less sensitive to variable RT times and hitches etc
	//that would otherwise cause systems to be culled in error.
	check(LastRenderThreadUpdateTime.IsValid());
	float WorldTime = GetWorld()->GetTimeSeconds();
	RenderThreadWorldTimeLag = WorldTime - LastRenderThreadUpdateTime->load(std::memory_order_relaxed);
	ENQUEUE_RENDER_COMMAND(FNiagaraWorldManagerRTUpdateTime)(
		[LRT = LastRenderThreadUpdateTime, WorldTime](FRHICommandListImmediate& RHICmdList)
		{
			LRT->store(WorldTime, std::memory_order_relaxed);
		}
	);
...
void FNiagaraWorldManager::TickEnd(float DeltaSeconds)
{
	CSV_SCOPED_TIMING_STAT_EXCLUSIVE(Effects);
	LLM_SCOPE(ELLMTag::Niagara);
	SCOPE_CYCLE_COUNTER(STAT_NiagaraWorldManTick);

	//Do any end of tick bookkeeping that must occur even when paused.	
...
void FNiagaraWorldManager::UpdateScalabilityManagers(float DeltaSeconds, bool bNewSpawnsOnly)
{
	...
	for (auto& Pair : ScalabilityManagers)
	{
		...
		if (bNewSpawnsOnly)
		{
			ScalabilityMan.Update(DeltaSeconds, true);
		}
		else
		{
			ScalabilityMan.Update(DeltaSeconds, false);
		}
	}
}
...
void FNiagaraWorldManager::RegisterWithScalabilityManager(UNiagaraComponent* Component, UNiagaraEffectType* EffectType)
{
	...
	if ( EffectType )
	{
		...
		#endif
			ScalabilityManager = &ScalabilityManagers.Emplace(EffectType, this);
	...
	if (GetScalabilityCullingMode() == ENiagaraScalabilityCullingMode::Enabled && System)
	{
		if (UNiagaraEffectType* EffectType = System->GetEffectType())
		{
			if (CanPreCull(EffectType))
			{
				...
				#if WITH_NIAGARA_DEBUGGER
				GetSystemScalabilityKillCounts(System).Add(State);				
				#endif
...
void FNiagaraWorldManager::CalculateScalabilityState(UNiagaraSystem* System, const FNiagaraSystemScalabilitySettings& ScalabilitySettings, UNiagaraEffectType* EffectType, UNiagaraComponent* Component, bool bIsPreCull, float WorstGlobalBudgetUse, FNiagaraScalabilityState& OutState)
{
	...
	if (GEnableNiagaraVisCulling)
	{
		...
		const float TimeSinceRendered = float(FMath::Max(0.0f, GetWorld()->LastRenderTime - Component->GetLastRenderTime() - World->GetDeltaSeconds() - RenderThreadWorldTimeLag - GLastRenderTimeSafetyBias));

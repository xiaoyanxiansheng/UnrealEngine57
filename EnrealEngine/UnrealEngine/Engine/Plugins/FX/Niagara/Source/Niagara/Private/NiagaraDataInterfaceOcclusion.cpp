...
void UNiagaraDataInterfaceOcclusion::SetShaderParameters(const FNiagaraDataInterfaceSetShaderParametersContext& Context) const
{
	...
	if (Context.IsResourceBound(&ShaderParameters->CloudVolumetricTexture))
	{
		...
		ShaderParameters->CloudVolumetricTextureUVScale = View && View->State ? View->State->GetVolumetricCloudTextureUVScale() : FVector2f(1.0f, 1.0f);
		ShaderParameters->CloudVolumetricTextureUVMax = View && View->State ? View->State->GetVolumetricCloudTextureUVMax() : FVector2f(1.0f, 1.0f);
	}

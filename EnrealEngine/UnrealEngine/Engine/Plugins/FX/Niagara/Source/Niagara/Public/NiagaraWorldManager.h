...
public:
	...
	virtual ~FNiagaraWorldManager() override;
...
public:
	...
	/** Called at the very beginning of world ticking, even when paused. */
	void TickStart(float DeltaSeconds);
	
	/** Called at the very end of world ticking, even when paused. */
	void TickEnd(float DeltaSeconds);
...
public:
	...
	//TODO: Remove - UE-347574
	void LegacyOnWorldBeginTearDown();
	void LegacyOnWorldCleanup(bool bSessionEnded, bool bCleanupResources);
	void LegacyOnPostWorldCleanup(bool bSessionEnded, bool bCleanupResources);
	...
	/*inline*/ void SortedSignificanceCull(UNiagaraEffectType* EffectType, UNiagaraComponent* Component, const FNiagaraSystemScalabilitySettings& ScalabilitySettings, float Significance, int32& EffectTypeInstCount, uint16& SystemInstCount, FNiagaraScalabilityState& OutState);
...
private:
	...
	void CleanupTransientObjectsAndManagers();
...
public:
	...
	FNiagaraSystemScalabilityKillCounts& GetSystemScalabilityKillCounts(UNiagaraSystem* System) { return SystemScalabilityKillCounts.FindOrAdd(System); }
	const TMap<TWeakObjectPtr<UNiagaraSystem>, FNiagaraSystemScalabilityKillCounts>& GetSystemScalabilityKillCounts() const{ return SystemScalabilityKillCounts; }
#endif
...
private:
	...
	// Callback for when a world tick is begun.
	static void TickWorldStart(UWorld* World, ELevelTick TickType, float DeltaSeconds);

	// Callback for when a world tick is ended.
	static void TickWorldEnd(UWorld* World, ELevelTick TickType, float DeltaSeconds);
	...
	static FDelegateHandle TickWorldEndHandle;
	...
	TMap<TWeakObjectPtr<UNiagaraSystem>, FNiagaraSystemScalabilityKillCounts> SystemScalabilityKillCounts;
#endif
	...
	TSharedPtr<std::atomic<float>> LastRenderThreadUpdateTime;
	float RenderThreadWorldTimeLag = 0.0f;
};

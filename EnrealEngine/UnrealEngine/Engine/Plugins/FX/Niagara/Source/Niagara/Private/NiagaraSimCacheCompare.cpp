...
namespace NiagaraSimCacheCompareInternal
{
	...
	bool MatchesWildcardArray(TConstArrayView<FString> Wildcards, FName Name)
	{
		if (Wildcards.Num() == 0)
		{
			return false;
		...
		{
			if (NameString.MatchesWildcard(Wildcard))
			{
				return true;
			}
		}
	...
	TArray<FNiagaraSimCacheVariable> GatherEmitterVariables(const UNiagaraSimCache& SimCache, int32 EmitterIndex, const TSet<FNiagaraVariableBase>& ExcludeVariables, TConstArrayView<FString> WildcardMatches)
	{
		TArray<FNiagaraSimCacheVariable> OutVariables;
		SimCache.ForEachEmitterAttribute(
			EmitterIndex,
			[&OutVariables, &ExcludeVariables, &WildcardMatches](const FNiagaraSimCacheVariable& Variable) -> bool
			{
				if (!ExcludeVariables.Contains(Variable.Variable) && !MatchesWildcardArray(WildcardMatches, Variable.Variable.GetName()))
				{
...
void FNiagaraSimCacheCompare::AddExcludeAttributesByWildcard(FString Wildcard)
{
	AttributesToExclude_ByWildcard.AddUnique(Wildcard);
}
	...
	if (bEqual && bIncludeDataInterfaces)
	{
		...
		if (bEqual)
		{
			for (const FNiagaraVariableBase& DIVariable : DataInterfacesToCompare)
			{
				...
				if ( MatchesWildcardArray(AttributesToExclude_ByWildcard, DIVariable.GetName()) )
				{
					continue;
				}
...
bool FNiagaraSimCacheCompare::CompareEmitter(const UNiagaraSimCache& LhsCache, const UNiagaraSimCache& RhsCache, int EmitterIndex, int FrameIndex, FString& OutDifferences)
{
	...
	const TArray<FNiagaraSimCacheVariable> CompareVariables = GatherEmitterVariables(LhsCache, EmitterIndex, AttributesToExclude, AttributesToExclude_ByWildcard);
	{
		bool bAttributesValid = true;
		const TArray<FNiagaraSimCacheVariable> RhsCompareVariables = GatherEmitterVariables(RhsCache, EmitterIndex, AttributesToExclude, AttributesToExclude_ByWildcard);

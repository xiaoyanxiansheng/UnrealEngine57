	...
	TArray<FNiagaraDataInterfaceFeedback>& OutWarnings, TArray<FNiagaraDataInterfaceFeedback>& OutInfo)
{
	...
	if (SkelMesh && SkelMesh->HasActiveClothingAssets())
	{
		const FText ClothSummary = LOCTEXT("UsingClothSummary", "Active clothing assets found.  Results may be incorrect.");
		if (GetDefault<UNiagaraSettings>()->NDISkelMesh_SupportReadingDeformedGeometry == false)
		{
			OutInfo.Emplace(
				FText::Format(LOCTEXT("UsingClothNeedsProjectSetting", "Sampling from clothing regions on mesh '{0} will be incorrect.  Please enable reading from deformed geometry is disabled in project settings."), FText::FromString(SkelMesh->GetName())),
				ClothSummary,
		...
		{
			OutInfo.Emplace(
				FText::Format(LOCTEXT("UsingClothCheckValid", "Sampling from clothing regions on mesh '{0}' may be incorrect.  Ensure you are using a GPU emitter and the skin cache or deformer graph are enabled."), FText::FromString(SkelMesh->GetName())),
				ClothSummary,
				FNiagaraDataInterfaceFix()
			);
		}

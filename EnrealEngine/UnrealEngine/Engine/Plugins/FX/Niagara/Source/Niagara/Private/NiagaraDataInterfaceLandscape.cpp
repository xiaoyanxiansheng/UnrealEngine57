...
{
	const FName NAME_GetBaseColor("GetBaseColor");
	const FName NAME_GetHeight("GetHeight");
	const FName NAME_GetWorldNormal("GetWorldNormal");
	const FName NAME_GetPhysicalMaterialIndex("GetPhysicalMaterialIndex");
	const FName NAME_GenerateRvtFeedback("GenerateVirtualTextureFeedback");
...
struct FRegionRequest
{
	FBox2D Region = FBox2D(FVector2D::Zero(), FVector2D::One());
	int32 MipLevel = 0;
	bool bIncludeBaseColor = true;
	bool bIncludeHeight = true;
	bool bIncludeNormal = true;
};
...
struct FNDILandscapeData_GameThread
{
	...
	TArray<FRegionRequest> RegionRequests;
	...
	FBox2D RVTLandscapeBounds = FBox2D(EForceInit::ForceInit);
	...
	void Reset()
	{
		...
		RVTLandscapeBounds = FBox2D(EForceInit::ForceInit);
...
struct FNDILandscapeData_RenderThread
{
	...
	struct FVirtualTexturePage
	{
		...
		void Reset()
		{
			...
			AllocatedTexture = nullptr;
			SizeInPages = FUintPoint(0, 0);
		}
		...
		void Update(const URuntimeVirtualTexture* VirtualTexture, uint32 PageTableIndex, bool bIncludeWorldToUv, bool bIncludeHeightUnpack)
		{
			...
			if (VirtualTexture && ::IsValid(VirtualTexture))
			{
				AllocatedTexture = VirtualTexture->GetAllocatedVirtualTexture();
				if (AllocatedTexture)
				{
					...
					SizeInPages = FUintPoint(AllocatedTexture->GetWidthInTiles(), AllocatedTexture->GetHeightInTiles());
				}
			}
		}

		FVector2D WorldToUv(const FVector& WorldPosition) const
		{
			const FVector NormalizedPosition = WorldPosition - WorldToUvParameters[0];
			return FVector2D(
				FVector(WorldToUvParameters[1]).Dot(NormalizedPosition),
				FVector(WorldToUvParameters[2]).Dot(NormalizedPosition));
		}
		...
		IAllocatedVirtualTexture* AllocatedTexture = nullptr;
		FUintPoint SizeInPages = FUintPoint(0, 0);
	};
...
struct FNiagaraDataInterfaceProxyLandscape : public FNiagaraDataInterfaceProxy
{
		...
		ERuntimeVirtualTextureMaterialType NormalVirtualTextureMode)
	{
		if (FNDILandscapeData_RenderThread* Proxy_RT = SystemInstancesToProxyData_RT.Find(ID))
		{
			...
			TArray<FRegionRequest> RegionsToProcess;
			Swap(RegionsToProcess, Proxy_RT->LandscapeData.RegionRequests);

			if (!RegionsToProcess.IsEmpty())
			{
				IRendererModule& RenderModule = GetRendererModule();

				for (const FRegionRequest& RegionRequest : RegionsToProcess)
				{
					const FNDILandscapeData_RenderThread::FVirtualTexturePage* TexturePages[] =
					{
						RegionRequest.bIncludeBaseColor ? &Proxy_RT->BaseColorVirtualPage : nullptr,
						RegionRequest.bIncludeHeight ? &Proxy_RT->HeightVirtualPage : nullptr,
						RegionRequest.bIncludeNormal ? &Proxy_RT->NormalVirtualPage : nullptr,
					};

					for (const FNDILandscapeData_RenderThread::FVirtualTexturePage* TexturePage : TexturePages)
					{
						if (TexturePage && TexturePage->IsValid() && TexturePage->AllocatedTexture)
						{
							const FVector2D UVMin = TexturePage->WorldToUv(FVector(RegionRequest.Region.Min.X, RegionRequest.Region.Min.Y, 0.0));
							const FVector2D UVMax = TexturePage->WorldToUv(FVector(RegionRequest.Region.Max.X, RegionRequest.Region.Max.Y, 0.0));

							const FVector2D ClampedUVMin = FVector2D::Clamp(UVMin, FVector2D::Zero(), FVector2D::One());
							const FVector2D ClampedUVMax = FVector2D::Clamp(UVMax, FVector2D::Zero(), FVector2D::One());

							RenderModule.RequestVirtualTextureTiles(TexturePage->AllocatedTexture, FVector2D::One(), FVector2D::Zero(), FVector2D::One(), ClampedUVMin, ClampedUVMax, RegionRequest.MipLevel);
						}
					}
				}
			}
...
void FLandscapeTextureResource::InitRHI(FRHICommandListBase& RHICmdList)
{
	if (HeightValues.Num())
	{
	...
	if (PhysMatValues.Num())
	{
		...
		PhysMatTexture.Initialize(RHICmdList, TEXT("FLandscapeTextureResource_PhysMatTexture"), sizeof(uint8), CellCount.X, CellCount.Y, EPixelFormat::PF_R8_UINT, FTextureReadBuffer2D::DefaultTextureInitFlag, &BulkData);
	}
...
{
	{
		...
		Sig.Name = NiagaraDataInterfaceLandscape::NAME_GetBaseColor;
	...
	{
		FNiagaraFunctionSignature Sig;
		Sig.Name = NiagaraDataInterfaceLandscape::NAME_GetHeight;
		Sig.bMemberFunction = true;
		Sig.bRequiresContext = false;
		Sig.bSupportsCPU = false;
		Sig.Inputs.Add(FNiagaraVariable(FNiagaraTypeDefinition(GetClass()), TEXT("Landscape")));
	...
	{
		FNiagaraFunctionSignature Sig;
		Sig.Name = NiagaraDataInterfaceLandscape::NAME_GetWorldNormal;
		Sig.bMemberFunction = true;
		Sig.bRequiresContext = false;
		Sig.bSupportsCPU = false;
		Sig.Inputs.Add(FNiagaraVariable(FNiagaraTypeDefinition(GetClass()), TEXT("Landscape")));
	...
	{
		FNiagaraFunctionSignature Sig;
		Sig.Name = NiagaraDataInterfaceLandscape::NAME_GetPhysicalMaterialIndex;
		Sig.bMemberFunction = true;
		Sig.bRequiresContext = false;
		Sig.bSupportsCPU = false;
		Sig.Inputs.Add(FNiagaraVariable(FNiagaraTypeDefinition(GetClass()), TEXT("Landscape")));
	...
	{
		FNiagaraFunctionSignature Sig;
		Sig.Name = NiagaraDataInterfaceLandscape::NAME_GenerateRvtFeedback;
		...
		Sig.Inputs.Add(FNiagaraVariable(FNiagaraTypeDefinition(GetClass()), TEXT("Landscape")));
		Sig.Inputs.Add(FNiagaraVariable(FNiagaraTypeDefinition::GetVec2Def(), TEXT("RegionExtentMin")));
		Sig.Inputs.Add(FNiagaraVariable(FNiagaraTypeDefinition::GetVec2Def(), TEXT("RegionExtentMax")));
		Sig.Inputs.Emplace_GetRef(FNiagaraVariable(FNiagaraTypeDefinition::GetIntDef(), TEXT("MipLevel"))).SetValue(0);
		Sig.Inputs.Emplace_GetRef(FNiagaraVariable(FNiagaraTypeDefinition::GetBoolDef(), TEXT("IncludeBaseColor"))).SetValue(true);
		Sig.Inputs.Emplace_GetRef(FNiagaraVariable(FNiagaraTypeDefinition::GetBoolDef(), TEXT("IncludeHeight"))).SetValue(true);
		Sig.Inputs.Emplace_GetRef(FNiagaraVariable(FNiagaraTypeDefinition::GetBoolDef(), TEXT("IncludeNormal"))).SetValue(true);
		Sig.Inputs.Emplace_GetRef(FNiagaraVariable(FNiagaraTypeDefinition::GetBoolDef(), TEXT("Enabled"))).SetValue(true);
		Sig.SetDescription(LOCTEXT("GenerateRVTFeedback", "Generates a request to refine the RVT in the specified region."));
...
void UNiagaraDataInterfaceLandscape::GetVMExternalFunction(const FVMExternalFunctionBindingInfo& BindingInfo, void* PerInstanceData, FVMExternalFunction& OutFunc)
{
	using namespace NiagaraDataInterfaceLandscape;

	if (BindingInfo.Name == NiagaraDataInterfaceLandscape::NAME_GenerateRvtFeedback)
	{
		OutFunc = FVMExternalFunction::CreateLambda([this](FVectorVMExternalFunctionContext& Context) { VMGenerateRvtFeedback(Context); });
	}
}

void UNiagaraDataInterfaceLandscape::VMGenerateRvtFeedback(FVectorVMExternalFunctionContext& Context)
{
	VectorVM::FUserPtrHandler<FNDILandscapeData_GameThread> InstanceData(Context);
	FNDIInputParam<FVector2f> InExtentMin(Context);
	FNDIInputParam<FVector2f> InExtentMax(Context);
	FNDIInputParam<int32> InMipLevel(Context);
	FNDIInputParam<bool> InIncludeBaseColor(Context);
	FNDIInputParam<bool> InIncludeHeight(Context);
	FNDIInputParam<bool> InIncludeNormal(Context);
	FNDIInputParam<bool> InEnabled(Context);

	const bool bHasValidLandscapeBounds = InstanceData->RVTLandscapeBounds.bIsValid;
	const int32 InstanceCount = Context.GetNumInstances();
	if (InstanceCount)
	{
		if (bHasValidLandscapeBounds)
		{
			InstanceData->RegionRequests.Reserve(InstanceData->RegionRequests.Num() + InstanceCount);
		}

		for (int32 InstanceIdx = 0; InstanceIdx < Context.GetNumInstances(); ++InstanceIdx)
		{
			const FVector2f ExtentMin = InExtentMin.GetAndAdvance();
			const FVector2f ExtentMax = InExtentMax.GetAndAdvance();
			const int32 RequestedMipLevel = InMipLevel.GetAndAdvance();
			const bool bIncludeBaseColor = InIncludeBaseColor.GetAndAdvance();
			const bool bIncludeHeight = InIncludeHeight.GetAndAdvance();
			const bool bIncludeNormal = InIncludeNormal.GetAndAdvance();
			const bool bEnabled = InEnabled.GetAndAdvance();

			if (bEnabled && bHasValidLandscapeBounds)
			{
				const FBox2D RequestBounds = InstanceData->RVTLandscapeBounds.Overlap(FBox2D(FVector2D(ExtentMin), FVector2D(ExtentMax)));
				if (RequestBounds.bIsValid)
				{
					FRegionRequest& Request = InstanceData->RegionRequests.AddDefaulted_GetRef();
					Request.Region = RequestBounds;
					Request.MipLevel = RequestedMipLevel;
					Request.bIncludeBaseColor = bIncludeBaseColor;
					Request.bIncludeHeight = bIncludeHeight;
					Request.bIncludeNormal = bIncludeNormal;
				}
			}
		}
	}
...
{
	FNDILandscapeData_GameThread& SourceData = *reinterpret_cast<FNDILandscapeData_GameThread*>(PerInstanceData);
	...
	TargetData->RegionRequests = MoveTemp(SourceData.RegionRequests);
}
...
{
	if ( (FunctionInfo.DefinitionName == NiagaraDataInterfaceLandscape::NAME_GetBaseColor) ||
		 (FunctionInfo.DefinitionName == NiagaraDataInterfaceLandscape::NAME_GetHeight) ||
		 (FunctionInfo.DefinitionName == NiagaraDataInterfaceLandscape::NAME_GetWorldNormal) ||
		 (FunctionInfo.DefinitionName == NiagaraDataInterfaceLandscape::NAME_GetPhysicalMaterialIndex) )
	{
		return true;
	}
...
bool UNiagaraDataInterfaceLandscape::InitPerInstanceData(void* PerInstanceData, FNiagaraSystemInstance* SystemInstance)
{
	...
	bool SystemRequiresHeightsGpu = false;
	SystemInstance->EvaluateBoundFunction(NiagaraDataInterfaceLandscape::NAME_GetHeight, SystemRequiresHeightsCpu, SystemRequiresHeightsGpu);

	bool SystemRequiresNormalsCpu = false;
	bool SystemRequiresNormalsGpu = false;
	SystemInstance->EvaluateBoundFunction(NiagaraDataInterfaceLandscape::NAME_GetWorldNormal, SystemRequiresNormalsCpu, SystemRequiresNormalsGpu);

	bool SystemRequiresPhysMatCpu = false;
	bool SystemRequiresPhysMatGpu = false;
	SystemInstance->EvaluateBoundFunction(NiagaraDataInterfaceLandscape::NAME_GetPhysicalMaterialIndex, SystemRequiresPhysMatCpu, SystemRequiresPhysMatGpu);

	bool SystemRequiresBaseColorCpu = false;
	bool SystemRequiresBaseColorGpu = false;
	SystemInstance->EvaluateBoundFunction(NiagaraDataInterfaceLandscape::NAME_GetBaseColor, SystemRequiresBaseColorCpu, SystemRequiresBaseColorGpu);
...
bool UNiagaraDataInterfaceLandscape::PerInstanceTick(void* PerInstanceData, FNiagaraSystemInstance* SystemInstance, float DeltaSeconds)
{
	...
	if (FNDILandscapeData_GameThread* InstanceData = reinterpret_cast<FNDILandscapeData_GameThread*>(PerInstanceData))
	{
		...
		// has streamed in for the landscape and we need to update our capture of the data
		ApplyLandscape(*SystemInstance, *InstanceData);
...
void UNiagaraDataInterfaceLandscape::ApplyLandscape(const FNiagaraSystemInstance& SystemInstance, FNDILandscapeData_GameThread& InstanceData) const
{
	...
	InstanceData.RVTLandscapeBounds = FBox2D(EForceInit::ForceInit);
	...
	const bool bHasVirtualTexture = InstanceData.HeightVirtualTextureIndex != INDEX_NONE
		|| InstanceData.NormalVirtualTextureIndex != INDEX_NONE
		|| InstanceData.BaseColorVirtualTextureIndex != INDEX_NONE;

	if (bHasVirtualTexture)
	{
		FVector LandscapeBoundsOrigin;
		FVector LandscapeBoundsExtent;
		Landscape->GetActorBounds(true /*bOnlyCollidingComponents*/, LandscapeBoundsOrigin, LandscapeBoundsExtent);

		InstanceData.RVTLandscapeBounds = FBox2D(FVector2D(LandscapeBoundsOrigin - LandscapeBoundsExtent), FVector2D(LandscapeBoundsOrigin + LandscapeBoundsExtent));
	}

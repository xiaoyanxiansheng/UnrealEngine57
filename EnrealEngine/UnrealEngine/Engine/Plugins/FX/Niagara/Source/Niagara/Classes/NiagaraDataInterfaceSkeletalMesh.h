...
struct FSkeletalMeshSkinningDataUsage
{
	...
	inline bool NeedBoneMatrices()const { return bUsesBoneMatrices || bUsesPreSkinnedVerts; }
	inline bool NeedPreSkinnedVerts()const { return bUsesPreSkinnedVerts; }
	inline int32 GetLODIndex()const { return LODIndex; }
private:
	int32 LODIndex;
...
struct FSkeletalMeshSkinningData
{
	...
	inline int32 GetBoneCount(bool RequiresPrevious) const
	{
	...
	inline FVector3f GetPosition(int32 LODIndex, int32 VertexIndex) const
	{
		return LODData.IsValidIndex(LODIndex) ? LODData[LODIndex].SkinnedCPUPositions[CurrIndex][VertexIndex] : FVector3f::ZeroVector;
	}

	inline FVector3f GetPreviousPosition(int32 LODIndex, int32 VertexIndex) const
	{
		return LODData.IsValidIndex(LODIndex) ? LODData[LODIndex].SkinnedCPUPositions[CurrIndex ^ 1][VertexIndex] : FVector3f::ZeroVector;
	}

	inline void GetTangentBasis(int32 LODIndex, int32 VertexIndex, FVector3f& OutTangentX, FVector3f& OutTangentY, FVector3f& OutTangentZ)
	{
	...
	inline void GetPreviousTangentBasis(int32 LODIndex, int32 VertexIndex, FVector3f& OutTangentX, FVector3f& OutTangentY, FVector3f& OutTangentZ)
	{
		const bool bValidLOD = LODData.IsValidIndex(LODIndex);
...
private:
	inline TArray<FVector3f>& CurrSkinnedPositions(int32 LODIndex)
	{
		return LODData[LODIndex].SkinnedCPUPositions[CurrIndex];
	}

	inline TArray<FVector3f>& PrevSkinnedPositions(int32 LODIndex)
	{
		return LODData[LODIndex].SkinnedCPUPositions[CurrIndex ^ 1];
	}

	inline TArray<FVector3f>& CurrSkinnedTangentBasis(int32 LODIndex)
	{
		return LODData[LODIndex].SkinnedTangentBasis[CurrIndex];
	}

	inline TArray<FVector3f>& PrevSkinnedTangentBasis(int32 LODIndex)
	{
		return LODData[LODIndex].SkinnedTangentBasis[CurrIndex ^ 1];
	}

public:
	inline TArray<FMatrix44f>& CurrBoneRefToLocals()
	{
		return BoneRefToLocals[CurrIndex];
	}

	inline const TArray<FMatrix44f>& CurrBoneRefToLocals() const
	{
		return BoneRefToLocals[CurrIndex];
	}

	inline TArray<FMatrix44f>& PrevBoneRefToLocals()
	{
		return BoneRefToLocals[CurrIndex ^ 1];
	}

	inline const TArray<FMatrix44f>& PrevBoneRefToLocals() const
	{
		return BoneRefToLocals[CurrIndex ^ 1];
	}

	inline TArray<FTransform3f>& CurrComponentTransforms()
	{
		return ComponentTransforms[CurrIndex];
	}

	inline const TArray<FTransform3f>& CurrComponentTransforms() const
	{
		return ComponentTransforms[CurrIndex];
	}

	inline TArray<FTransform3f>& PrevComponentTransforms()
	{
		return ComponentTransforms[CurrIndex ^ 1];
	}

	inline const TArray<FTransform3f>& PrevComponentTransforms() const
	{
		return ComponentTransforms[CurrIndex ^ 1];
	}

	inline bool NeedPreSkinnedVerts() const
	{
...
struct FSkeletalMeshSamplingRegionAreaWeightedSampler : FWeightedRandomSampler
{
	...
	inline bool IsValid() { return TotalWeight > 0.0f; }
...
public:
	...
	inline void Initialise(struct FNDISkeletalMesh_InstanceData* InstData, int32 InLODIndex, const FSkeletalMeshLODRenderData& SkeletalMeshLODRenderData, const FSkeletalMeshSamplingLODBuiltData* SkeletalMeshSamplingLODBuiltData, FNiagaraSystemInstance* SystemInstance);
...
struct FNDISkeletalMesh_InstanceData
{
	...
	inline int32 GetLODIndex()const { return CachedLODIdx; }

	inline const FSkinWeightVertexBuffer* GetSkinWeights() const
	{
...
UCLASS(EditInlineNew, Category = "Meshes", meta = (DisplayName = "Skeletal Mesh Reader"), MinimalAPI)
...
private:
	template<typename FilterMode, typename AreaWeightingMode>
	inline int32 RandomTriIndex(FNDIRandomHelper& RandHelper, FSkeletalMeshAccessorHelper& Accessor, FNDISkeletalMesh_InstanceData* InstData, int32 InstanceIndex);
	...
	template<typename FilterMode, typename AreaWeightingMode>
	inline int32 GetFilteredTriangleCount(FSkeletalMeshAccessorHelper& Accessor, FNDISkeletalMesh_InstanceData* InstData);

	template<typename FilterMode, typename AreaWeightingMode>
	inline int32 GetFilteredTriangleAt(FSkeletalMeshAccessorHelper& Accessor, FNDISkeletalMesh_InstanceData* InstData, int32 FilteredIdx);
...
private:
	template<typename FilterMode>
	inline int32 RandomFilteredVertIndex(FNDIRandomHelper& RandHelper, int32 Instance, FSkeletalMeshAccessorHelper& Accessor, FNDISkeletalMesh_InstanceData* InstData);

	template<typename FilterMode>
	inline int32 GetFilteredVertexCount(FSkeletalMeshAccessorHelper& Accessor, FNDISkeletalMesh_InstanceData* InstData);

	template<typename FilterMode>
	inline int32 GetFilteredVertexAt(FSkeletalMeshAccessorHelper& Accessor, FNDISkeletalMesh_InstanceData* InstData, int32 FilteredIdx);

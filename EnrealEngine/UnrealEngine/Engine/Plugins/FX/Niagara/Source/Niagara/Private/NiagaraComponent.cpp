...
namespace NiagaraComponentPrivate
{
	static bool GParallelGDMEEnabled = true;
	static FAutoConsoleVariableRef CVarNiagaraParallelGDME(
		TEXT("fx.Niagara.ParallelGDME"),
		GParallelGDMEEnabled,
		TEXT("Allow Niagara to run parallel in GDME."),
		ECVF_Default
	);

	static bool GbComponentsEnabled = true;
	...
	static int GCanPreventScalabilityCullingOnPlayerFX = 1;
	static FAutoConsoleVariableRef CVarNiagaraCanPreventScalabilityCullingOnPlayerFX(
		...
		GCanPreventScalabilityCullingOnPlayerFX,
	...
	static void ForEachComponentInWorld(const TArray<FString>& InArgs, UWorld* World, TFunction<void(UNiagaraComponent*)> Callback)
	{
		const FString Command = FString::Join(InArgs, TEXT(" "));

		FString SystemFilter;
		FParse::Value(*Command, TEXT("filter="), SystemFilter);

		for (TObjectIterator<UNiagaraComponent> It; It; ++It)
		{
			UNiagaraComponent* Component = *It;
			if (!IsValid(Component) || Component->GetWorld() != World || !Component->IsActive() || !Component->GetAsset())
			{
				continue;
			}

			UNiagaraSystem* NiagaraSystem = Component->GetAsset();
			if (!SystemFilter.IsEmpty() && !NiagaraSystem->GetName().MatchesWildcard(SystemFilter))
			{
				continue;
			}
	...
	FAutoConsoleCommandWithWorldAndArgs ReinitializeActiveCommand(
		TEXT("fx.Niagara.Component.ReinitializeActive"),
		TEXT("Iterates all activate components and reinitializes them."),
		FConsoleCommandWithWorldAndArgsDelegate::CreateLambda(
			[](const TArray<FString>& InArgs, UWorld* World)
			{
				ForEachComponentInWorld(InArgs, World, [](UNiagaraComponent* Component) { Component->ReinitializeSystem(); });
			}
		)
	);

	FAutoConsoleCommandWithWorldAndArgs DeactivateActiveCommand(
		TEXT("fx.Niagara.Component.DeactivateActive"),
		TEXT("Iterates all activate components and deactivates them."),
		FConsoleCommandWithWorldAndArgsDelegate::CreateLambda(
			[](const TArray<FString>& InArgs, UWorld* World)
			{
				ForEachComponentInWorld(InArgs, World, [](UNiagaraComponent* Component) { Component->DeactivateImmediate(); });
			}
		)
	);
}
	...
	, OcclusionQueryMode(Desc.OcclusionQueryMode)
{
	...
	bSupportsParallelGDME = NiagaraComponentPrivate::GParallelGDMEEnabled;
	...
	, SystemFixedBounds(ForceInit)
{
	...
	PrimaryComponentTick.EndTickGroup = TG_LastDemotable;
	...
	if (IsActive() && SystemInstanceController.Get() && !SystemInstanceController->IsComplete())
	{
		...
		else if(AgeUpdateMode == ENiagaraAgeUpdateMode::DesiredAge)
		{
			...
			if (FMath::Abs(AgeDiff) >= UE_KINDA_SMALL_NUMBER)
			{
				...
				{
					FGraphEventRef CompletionEvent = (ThisTickFunction && ThisTickFunction->IsCompletionHandleValid()) ? ThisTickFunction->GetCompletionHandle() : nullptr;
					...
					if (bLockDesiredAgeDeltaTimeToSeekDelta || AgeDiff > TickDelta)
					{
						...
						const int32 TicksToProcess = FMath::FloorToInt(AgeDiff / TickDelta);
						CompletionEvent = TicksToProcess == 1 ? CompletionEvent : nullptr;

						bIsSeeking = SystemInstanceController->MultiTick(TicksToProcess, TickDelta, MaxSimTime, CompletionEvent);
					}
					else
					{
...
void UNiagaraComponent::ActivateInternal(bool bReset /* = false */, bool bIsScalabilityCull)
{
	...
	if (!NiagaraComponentPrivate::GbComponentsEnabled)
	{
		UnregisterWithScalabilityManager();
	...
	{
		DestroyInstance();
		if (NiagaraComponentPrivate::GWarnNullAsset && !HasAnyFlags(RF_DefaultSubObject | RF_ArchetypeObject | RF_ClassDefaultObject))
		{
	...
	{
		if (bWaitForCompilationOnActivate || NiagaraComponentPrivate::GForceWaitForCompilationOnActivate || GIsAutomationTesting)
		{
	...
	if (SystemInstanceController.IsValid())
	{
		...
		else if (SystemInstanceController->IsSolo())
		{
			...
			PrimaryComponentTick.TickGroup = FMath::Max(TG_PrePhysics, SoloTickGroup);
			PrimaryComponentTick.EndTickGroup = TG_LastDemotable;
...
#if WITH_EDITOR
		// When in the editor do not invoke the system finished delegate if we are reconstrucing blueprint instances as they can have side effects.
		// For example the finished delegate could destroy the actor, attach / remove components / etc
		...
		if (IsRegisteredWithScalabilityManager())
		{
			if (bExternalCompletion == false)
			{
				...
				if (UNiagaraEffectType* EffectType = GetAsset()->GetEffectType())
				{
					...
					{
						if (NiagaraComponentPrivate::GWarnAsleepCullReaction == 1 && GetAsset())
						{
...
bool UNiagaraComponent::IsActiveForUpdateContext() const
{
	return
		// If we are activate and the execution state is also active (anything else means we were winding down / disabled)
		(IsActive() && GetRequestedExecutionState() == ENiagaraExecutionState::Active) ||
		// We were waiting on compilation / DDC request to come back
		(bAwaitingActivationDueToNotReady && PrimaryComponentTick.IsTickFunctionEnabled()) || 
		// We are registered with scalabilty, in which case allow it to take control
...
FPrimitiveSceneProxy* UNiagaraComponent::CreateSceneProxy()
{
	...
	if (Asset != nullptr)
	{
		...
		EPSOPrecachePriority PSOPrecachePriority = NiagaraComponentPrivate::GBoostPSOPrecacheHighest ? EPSOPrecachePriority::Highest : EPSOPrecachePriority::High;
...
bool UNiagaraComponent::ResolveOwnerAllowsScalability(bool bRegister)
{
	...
	if(NiagaraComponentPrivate::GCanPreventScalabilityCullingOnPlayerFX && Asset && Asset->AllowScalabilityForLocalPlayerFX() == false)
	{
...
bool UNiagaraComponent::GetVariableBool(FName InVariableName, bool& bIsValid) const
{
	const FNiagaraVariableBase Variable(FNiagaraTypeDefinition::GetBoolDef(), InVariableName);
	TOptional<FNiagaraBool> Value = OverrideParameters.GetParameterOptionalValue<FNiagaraBool>(Variable);
	bIsValid = Value.IsSet();
	return Value.Get(FNiagaraBool::False).GetValue();
}

int32 UNiagaraComponent::GetVariableInt(FName InVariableName, bool& bIsValid) const
{
	const FNiagaraVariableBase Variable(FNiagaraTypeDefinition::GetIntDef(), InVariableName);
	TOptional<int32> Value = OverrideParameters.GetParameterOptionalValue<int32>(Variable);
	bIsValid = Value.IsSet();
	return Value.Get(0);
}

float UNiagaraComponent::GetVariableFloat(FName InVariableName, bool& bIsValid) const
{
	const FNiagaraVariableBase Variable(FNiagaraTypeDefinition::GetFloatDef(), InVariableName);
	TOptional<float> Value = OverrideParameters.GetParameterOptionalValue<float>(Variable);
	bIsValid = Value.IsSet();
	return Value.Get(0.0f);
}

FVector2D UNiagaraComponent::GetVariableVec2(FName InVariableName, bool& bIsValid) const
{
	const FNiagaraVariableBase Variable(FNiagaraTypeDefinition::GetVec2Def(), InVariableName);
	TOptional<FVector2f> Value = OverrideParameters.GetParameterOptionalValue<FVector2f>(Variable);
	bIsValid = Value.IsSet();
	return FVector2D(Value.Get(FVector2f::ZeroVector));
}

FVector UNiagaraComponent::GetVariableVec3(FName InVariableName, bool& bIsValid) const
{
	const FNiagaraVariableBase Variable(FNiagaraTypeDefinition::GetVec3Def(), InVariableName);
	TOptional<FVector3f> Value = OverrideParameters.GetParameterOptionalValue<FVector3f>(Variable);
	bIsValid = Value.IsSet();
	return FVector(Value.Get(FVector3f::ZeroVector));
}

FVector4 UNiagaraComponent::GetVariableVec4(FName InVariableName, bool& bIsValid) const
{
	const FNiagaraVariableBase Variable(FNiagaraTypeDefinition::GetVec4Def(), InVariableName);
	TOptional<FVector4f> Value = OverrideParameters.GetParameterOptionalValue<FVector4f>(Variable);
	bIsValid = Value.IsSet();
	return FVector4(Value.Get(FVector4f::Zero()));
}

FVector UNiagaraComponent::GetVariablePosition(FName InVariableName, bool& bIsValid) const
{
	const FVector* Position = OverrideParameters.GetPositionParameterValue(InVariableName);
	bIsValid = Position != nullptr;
	return Position ? *Position : FVector::ZeroVector;
}

FLinearColor UNiagaraComponent::GetVariableColor(FName InVariableName, bool& bIsValid) const
{
	const FNiagaraVariableBase Variable(FNiagaraTypeDefinition::GetColorDef(), InVariableName);
	TOptional<FLinearColor> Value = OverrideParameters.GetParameterOptionalValue<FLinearColor>(Variable);
	bIsValid = Value.IsSet();
	return Value.Get(FLinearColor::White);
}

FQuat UNiagaraComponent::GetVariableQuat(FName InVariableName, bool& bIsValid) const
{
	const FNiagaraVariableBase Variable(FNiagaraTypeDefinition::GetQuatDef(), InVariableName);
	TOptional<FQuat4f> Value = OverrideParameters.GetParameterOptionalValue<FQuat4f>(Variable);
	bIsValid = Value.IsSet();
	return FQuat(Value.Get(FQuat4f::Identity));
}

FMatrix UNiagaraComponent::GetVariableMatrix(FName InVariableName, bool& bIsValid) const
{
	const FNiagaraVariableBase Variable(FNiagaraTypeDefinition::GetMatrix4Def(), InVariableName);
	TOptional<FMatrix44f> Value = OverrideParameters.GetParameterOptionalValue<FMatrix44f>(Variable);
	bIsValid = Value.IsSet();
	return FMatrix(Value.Get(FMatrix44f::Identity));
}
...
void UNiagaraComponent::Serialize(FStructuredArchive::FRecord Record)
{
	...
	if (Record.GetUnderlyingArchive().IsLoading())
	{
		...
		// When called from the loader, we know postload is going to be called, so avoid race conditions on the delegate by avoiding registering to it while in the loader.
...
void UNiagaraComponent::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	else if(PropertyName == GET_MEMBER_NAME_CHECKED(UNiagaraComponent, OverrideParameters))
	{
		...
		bNeedsReinitialize = NiagaraModule.GetEditorOnlyDataUtilities().GetResetSimulationOnParameterChange();
	}
	...
	for (const FNiagaraVariable& Key : UserParameters)
	{
		...
		if (OverrideValue.IsValid())
		{
			if (Key.IsDataInterface())
			{
				...
				const UNiagaraDataInterface* VariantDI = OverrideValue.GetDataInterface();
				if (ActualDI != nullptr && VariantDI != nullptr)
				{
					// Null DIs are logged elsewhere so we don't need to raise the issue here also
					ensureAlways(VariantDI->Equals(ActualDI));
				}
			}
...
void UNiagaraComponent::SynchronizeWithSourceSystem()
{
	...
	if (Asset == nullptr)
	{
		...
		EditorOverridesValue_DEPRECATED.Empty();
		CacheAndClearEditorParameterOverrides();
	...
	CopyCachedEditorParameterOverrides();
	ApplyOverridesToParameterStore();
#endif
...
void UNiagaraComponent::FixInvalidUserParameterOverrideData()
{
	...
	for (FNiagaraVariable& ExistingVar : ExistingVars)
	{
		...
		else if (ExistingVar.IsDataInterface())
		{
			...
			if (DataInterface == nullptr)
			{
				UE_LOG(LogNiagara, Log, TEXT("ParameterStore has null data interface '%s' for variable '%s' asset '%s', making a default."), *ExistingVar.GetType().GetClass()->GetName(), *ExistingVar.GetName().ToString(), *GetNameSafe(Asset));
			}
			else if (DataInterface->GetClass() != ExistingVar.GetType().GetClass())
			{
				UE_LOG(LogNiagara, Log, TEXT("ParameterStore has invalid type '%s' data interface '%s' for variable '%s' asset '%s', making a default."), *DataInterface->GetClass()->GetName(), *ExistingVar.GetType().GetClass()->GetName(), *ExistingVar.GetName().ToString(), *GetNameSafe(Asset));
				DataInterface = nullptr;
			}
...
void UNiagaraComponent::CacheAndClearEditorParameterOverrides()
{
	auto CacheOverrides = [](const TMap<FNiagaraVariableBase, FNiagaraVariant>& SourceOverrides, TMap<FNiagaraVariableBase, FNiagaraVariant>& CacheOverrides)
	{
		for (const TPair<FNiagaraVariableBase, FNiagaraVariant>& SourcePair : SourceOverrides)
		{
			CacheOverrides.FindOrAdd(SourcePair.Key) = SourcePair.Value;
		}
	};
	if (IsTemplate())
	{
		CacheOverrides(TemplateParameterOverrides, TemplateParameterOverridesCache);
		TemplateParameterOverrides.Empty();
	}
	else
	{
		CacheOverrides(InstanceParameterOverrides, InstanceParameterOverridesCache);
		InstanceParameterOverrides.Empty();
	}
}

void UNiagaraComponent::CopyCachedEditorParameterOverrides()
{
	auto UpdateOverridesFromCache = [](const TArrayView<const FNiagaraVariableWithOffset>& OverrideParameterVariables, const TMap<FNiagaraVariableBase, FNiagaraVariant>& CacheOverrides, TMap<FNiagaraVariableBase, FNiagaraVariant>& TargetOverrides)
	{
		for (const FNiagaraVariableWithOffset& OverrideParameterVariable : OverrideParameterVariables)
		{
			const FNiagaraVariant* CachedOverrideValue = CacheOverrides.Find(OverrideParameterVariable);
			if (CachedOverrideValue != nullptr)
			{
				if (TargetOverrides.Contains(OverrideParameterVariable) == false)
				{
					TargetOverrides.Add(OverrideParameterVariable, *CachedOverrideValue);
				}
			}
		}
	};
	TArrayView<const FNiagaraVariableWithOffset> OverrideParameterVariables = OverrideParameters.ReadParameterVariables();
	if (IsTemplate())
	{
		UpdateOverridesFromCache(OverrideParameterVariables, TemplateParameterOverridesCache, TemplateParameterOverrides);
	}
	else
	{
		UpdateOverridesFromCache(OverrideParameterVariables, InstanceParameterOverridesCache, InstanceParameterOverrides);
	}
}
...
void UNiagaraComponent::SetOverrideParameterStoreValue(const FNiagaraVariableBase& InKey, const FNiagaraVariant& InValue)
{
	if (InKey.IsDataInterface())
	{
		...
		if (OriginalDI == nullptr)
		{
			// There is an edge case where the data interface inside FNiagaraVariant has been garbage collected, it's unclear on a consistent repro right now.
			// Rather than crashing make a default and log a warning for investigation (see UE-314893).
			UClass* DataInterfaceClass = InKey.GetType().GetClass();
			check(DataInterfaceClass);
			UE_LOG(LogNiagara, Log, TEXT("SetOverrideParameterStoreValue has null data interface '%s' for variable '%s' asset '%s', making a default."), *DataInterfaceClass->GetName(), *InKey.GetName().ToString(), *GetNameSafe(Asset));

			const EObjectFlags DataInterfaceObjectFlags = UNiagaraDataInterface::BuildObjectFlagsForOwner(this, RF_Transactional);
			OriginalDI = NewObject<UNiagaraDataInterface>(this, DataInterfaceClass, NAME_None, DataInterfaceObjectFlags);
		}
...
void UNiagaraComponent::RemoveParameterOverride(const FNiagaraVariableBase& InKey)
{
	...
	if (!IsTemplate())
	{
		...
		InstanceParameterOverridesCache.Remove(UserVariable);
	}
	else
	{
		TemplateParameterOverrides.Remove(UserVariable);
		TemplateParameterOverridesCache.Remove(UserVariable);
...
void UNiagaraComponent::SetPreviewLODDistance(bool bInEnablePreviewLODDistance, float InPreviewLODDistance, float InPreviewMaxDistance)
{
	...
	if(SystemInstanceController)
	{
		if (bEnablePreviewLODDistance)
		{
			SystemInstanceController->SetLODDistance(PreviewLODDistance, InPreviewMaxDistance, true);
		}
		else
		{

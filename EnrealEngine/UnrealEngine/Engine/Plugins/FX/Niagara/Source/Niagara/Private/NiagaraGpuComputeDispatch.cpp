...
namespace FNiagaraGpuComputeDispatchLocal
{
	...
	#if STATS
		static int32 GDumpSortedSimulations = 0;
		static FAutoConsoleVariableRef CVarDumpSortedSimulations(
			TEXT("fx.Niagara.Batcher.DumpSortedSimulations"),
			GDumpSortedSimulations,
			TEXT("Will dump sorted simulations to the log for the provided number of frames."),
			ECVF_Default
		);
	#endif
...
void FNiagaraGpuComputeDispatch::FinishDispatches()
{
	...
	for (int iTickStage = 0; iTickStage < ENiagaraGpuComputeTickStage::Max; ++iTickStage)
	{
		...
		{
			ComputeProxy->ReleaseTicks(MaxTicksToFlush, bIsLastViewFamily);
		}
	}
...
#if STATS
	if (FNiagaraGpuComputeDispatchLocal::GDumpSortedSimulations > 0)
	{
		--FNiagaraGpuComputeDispatchLocal::GDumpSortedSimulations;

		UE_LOG(LogNiagara, Log, TEXT("Niagara SimulationsToSort:"));
		for (const FNiagaraGPUSortInfo& SortInfo : SimulationsToSort)
		{
			TStringBuilder<256> EnabledFeatures;
			if (SortInfo.SortMode != ENiagaraSortMode::None && SortInfo.SortAttributeOffset != INDEX_NONE)
			{
				EnabledFeatures.Append(TEXT("SortingEnabled "));
			}
			if (SortInfo.bEnableCulling)
			{
				EnabledFeatures.Append(TEXT("CullingEnabled "));
				if (SortInfo.CullPlanes.Num() > 0)
				{
					EnabledFeatures.Append(TEXT("FrustumCulling "));
				}
				if (SortInfo.RendererVisTagAttributeOffset != INDEX_NONE)
				{
					EnabledFeatures.Append(TEXT("RendererVisEnabled "));
				}
				if (SortInfo.MeshIndexAttributeOffset != INDEX_NONE)
				{
					EnabledFeatures.Append(TEXT("MeshIndexEnabled "));
				}
			}
			UE_LOG(LogNiagara, Log, TEXT(" - Owner(%s) ParticleCount(%d) %s"), *SortInfo.OwnerName.ToString(), SortInfo.ParticleCount, EnabledFeatures.ToString());
		}
	}
#endif
...
void FNiagaraGpuComputeDispatch::OnMarkPendingKill()
{
...
void FNiagaraGpuComputeDispatch::GenerateSortKeys(FRHICommandListImmediate& RHICmdList, int32 BatchId, int32 NumElementsInBatch, EGPUSortFlags Flags, FRHIUnorderedAccessView* KeysUAV, FRHIUnorderedAccessView* ValuesUAV)
{
	...
	for (const FNiagaraGPUSortInfo& SortInfo : SimulationsToSort)
	{
		...
		{
		#if STATS
			SCOPED_DRAW_EVENTF(RHICmdList, GenerateSortKeys, TEXT("Owner:%s"), SortInfo.OwnerName.ToString());
		#endif

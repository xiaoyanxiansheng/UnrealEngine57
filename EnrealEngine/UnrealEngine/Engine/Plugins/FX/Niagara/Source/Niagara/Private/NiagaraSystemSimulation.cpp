...
static FAutoConsoleVariableRef CVarNiagaraSystemSimulationAllowASync(
	...
	TEXT("If > 0, system simulation tick is parallelized. \n"),
	ECVF_Default
);

static int32 GNiagaraSystemSimulationAllowASyncSimCache = 1;
static FAutoConsoleVariableRef CVarNiagaraSystemSimulationAllowASyncSimCache(
	TEXT("fx.Niagara.SystemSimulation.AllowASyncSimCache"),
	GNiagaraSystemSimulationAllowASyncSimCache,
	TEXT("If > 0, sim cache tick is parallelized. \n"),
	ECVF_Default
);
...
void FNiagaraSystemSimulation::Tick_GameThread(float DeltaSeconds, const FGraphEventRef& MyCompletionGraphEvent)
{
	if (System->HasFixedTickDelta())
	{
		...
		for (int i = 0; i < Ticks; i++)
		{
			...
			Tick_GameThread_Internal(FixedDelta, Ticks == 1 ? MyCompletionGraphEvent : nullptr);
			Budget -= FixedDelta;
		}
...
bool FNiagaraSystemSimulation::MultiTick_GameThread(int32 TicksToProcess, float DeltaSecondsPerTick, float MaxSimTime, const FGraphEventRef& MyCompletionGraphEvent)
{
	...
	return TickInfo.TickNumber != (TicksToProcess - 1);
}
...
void FNiagaraSystemSimulation::SimCacheTick_GameThread(UNiagaraSimCache* SimCache, float DeltaSeconds, const FGraphEventRef& MyCompletionGraphEvent)
{
	check(System != nullptr);
	...
	if (SimCache->CanRead(System) == false)
	{
		for (int32 i=SystemInstances.Num() - 1; i >= 0; --i)
		{
			SystemInstances[i]->Complete(false);
		}
		return;
	}

	// Should we run concurrently or not?
	// Data interfaces are not thread safe for reading, we will need to implement a way to do this....
	if (GNiagaraSystemSimulationAllowASyncSimCache && SimCache->CanRunAsync())
	{
		// Create finalize tick batch information to fit into how things work
		struct FFinalizeTickBatch
		{
			TArray<FNiagaraSystemInstance*, TInlineAllocator<4>> Instances;
		#if DO_CHECK
			std::atomic<int> DebugCounter = 0;
		#endif
		};
		TUniquePtr<FFinalizeTickBatch> FinalizeTickBatch(new FFinalizeTickBatch());

		FinalizeTickBatch->Instances.Reserve(SystemInstances.Num());
		for (FNiagaraSystemInstance* SystemInstance : SystemInstances)
		{
			SystemInstance->ConcurrentTickGraphEvent = ConcurrentTickGraphEvent;
			FinalizeTickBatch->Instances.Add(SystemInstance);

			FNiagaraSystemInstanceFinalizeRef FinalizeRef(&FinalizeTickBatch->Instances.Last());
		#if DO_CHECK
			FinalizeRef.SetDebugCounter(&FinalizeTickBatch->DebugCounter);
		#endif
			SystemInstance->SetPendingFinalize(FinalizeRef);
		}

		// Create events to signal old task system
		ConcurrentTickGraphEvent = FGraphEvent::CreateGraphEvent();
		AllWorkCompleteGraphEvent = FGraphEvent::CreateGraphEvent();

		// Execute concurrent work
		UE::Tasks::FTask ConcurrentTask = UE::Tasks::Launch(
			TEXT("Niagara::SimCacheTick_Concurrent"),
			[this, SimCache, DeltaSeconds]()
			{
				TRACE_CPUPROFILER_EVENT_SCOPE(Niagara::SimCacheTick_Concurrent);

				SimCacheTick_Concurrent(SimCache, DeltaSeconds);
				ConcurrentTickGraphEvent->DispatchSubsequents();
			}
		);

		// Execute finalize work
		UE::Tasks::Launch(
			TEXT("Niagara::SimCacheTick_Finalize"),
			[this, TickBatch=MoveTemp(FinalizeTickBatch)]()
			{
				TRACE_CPUPROFILER_EVENT_SCOPE(Niagara::SimCacheTick_Finalize);

				for (FNiagaraSystemInstance* Instance : TickBatch->Instances)
				{
					if (Instance)
					{
						Instance->SimCacheTickFinalize_GameThread();
					}
				}
			#if DO_CHECK
				ensureMsgf(TickBatch->DebugCounter.load() == 0, TEXT("Finalize batch is complete but counter is %d when it should be zero"), TickBatch->DebugCounter.load());
			#endif
				AllWorkCompleteGraphEvent->DispatchSubsequents();
			},
			ConcurrentTask,
	...
	else
	{
		...
		for ( int32 i=SystemInstances.Num()-1; i >= 0; --i)
		{
			SystemInstances[i]->SimCacheTickFinalize_GameThread();
		}
	}
}

void FNiagaraSystemSimulation::SimCacheTick_Concurrent(UNiagaraSimCache* SimCache, float DeltaSeconds)
{
	// Read running instances
	TArray<FNiagaraSystemInstance*>& SystemInstances = GetSystemInstances(ENiagaraSystemInstanceState::Running);
	for (FNiagaraSystemInstance* SystemInstance : SystemInstances)
	{
		if ( !SimCache->Read(SystemInstance->GetAge(), SystemInstance) )
		{
			SystemInstance->RequestedExecutionState = ENiagaraExecutionState::Complete;
		}
	}

	// Move pending spawn instances into running instances
	{
		TArray<FNiagaraSystemInstance*>& PendingInstances = GetSystemInstances(ENiagaraSystemInstanceState::PendingSpawn);
		for (int32 i=PendingInstances.Num()-1; i >= 0; --i)
		{
			FNiagaraSystemInstance* SystemInstance = PendingInstances[i];
			if (!SimCache->Read(SystemInstance->GetAge(), SystemInstance))
			{
				SystemInstance->RequestedExecutionState = ENiagaraExecutionState::Complete;
			}
			...
			{
				SetInstanceState(SystemInstance, ENiagaraSystemInstanceState::Running);
			}
		}
	}

	// Initialize bindings (if needed)
	if (!bBindingsInitialized && SystemInstances.Num() > 0)
	{
		InitParameterDataSetBindings(SystemInstances[0]);
	}

	// Copy over execution states push to stores, etc
	...
	for ( int32 iSystemInstance=0; iSystemInstance < SystemInstances.Num(); ++iSystemInstance )
	{
		...
		if (SystemInstance.RequestedExecutionState == ENiagaraExecutionState::Complete)
		{
			continue;
		}

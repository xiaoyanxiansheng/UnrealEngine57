...
{
	UE::TWriteScopeLock RWGuard(RWLock);
...
bool UNiagaraSimCache::CanRead(UNiagaraSystem* NiagaraSystem)
{
	...
	if (bNeedsReadComponentMappingRecache)
	{
		// This modifies the component mappings that concurrent reading could be using
		// We should never really hit any concurrent readers while entering here, this is for safety
		UE::TWriteScopeLock RWGuard(RWLock);
		...
		// Determine if we can run async or not
		bCanRunAsync = !HasStoredDataInterfaces();
		if (bCanRunAsync)
		{
			bool bHasAnyPostReadDataInterface = false;
			FNiagaraDataInterfaceUtilities::ForEachDataInterface(
				NiagaraSystem,
				[&bHasAnyPostReadDataInterface](const FNiagaraDataInterfaceUtilities::FDataInterfaceUsageContext& DIContext)->bool
				{
					if (Cast<INiagaraSimCacheCustomStorageInterface>(DIContext.DataInterface) != nullptr)
					{
						bHasAnyPostReadDataInterface = true;
					}
					return !bHasAnyPostReadDataInterface;
				}
			);
			if (bHasAnyPostReadDataInterface)
			{
				bCanRunAsync = false;
			}
		}
...
{
	UE::TReadScopeLock RWGuard(RWLock);
	...
	if ( RelativeTime > DurationSeconds || CacheFrames.Num() == 0 )
	{
		// Complete
		return false;
	}

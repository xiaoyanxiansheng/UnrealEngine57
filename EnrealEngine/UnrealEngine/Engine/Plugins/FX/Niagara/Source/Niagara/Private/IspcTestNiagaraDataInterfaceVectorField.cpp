...
bool FIspcTestNiagaraDataInterfaceVectorField::RunTest(const FString& Parameters)
{
	...
	float ISPCOutZ = 0.f;
	{
		...
		FVectorVMExternalFunctionContext FunctionContext;
		FMemory::Memset(&FunctionContext, 0, sizeof(FunctionContext));
		const int32 RegisterCount = 6;
		FunctionContext.RegReadCount = 0;
		FunctionContext.NumInstances = 1;
		FunctionContext.NumRegisters = RegisterCount;
		FunctionContext.PerInstanceFnInstanceIdx = 0;
		...
		FunctionContext.RegisterData = (uint32**)ValuePtr;
		uint8 IncJunk[RegisterCount] = { 0, 1, 2, 3, 4, 5 };
		uint16 VecJunk[RegisterCount] = { 0, 1, 2, 3, 4, 5 };
		FunctionContext.RegInc = IncJunk;
		VectorField->SampleVectorField(FunctionContext);
	}

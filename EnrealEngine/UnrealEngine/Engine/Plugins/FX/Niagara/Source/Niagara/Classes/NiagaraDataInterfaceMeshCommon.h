...
inline FVector3f RandomBarycentricCoord(FRandomStream& RandStream)
{
...
template<typename T>
inline T BarycentricInterpolate(float BaryX, float BaryY, float BaryZ, T V0, T V1, T V2)
{
...
inline FVector4 BarycentricInterpolate(float BaryX, float BaryY, float BaryZ, const FVector4& V0, const FVector4& V1, const FVector4& V2)
{
	return V0 * BaryX + V1 * BaryY + V2 * BaryZ;
}

template<typename T>
inline T BarycentricInterpolate(FVector3f BaryCoord, T V0, T V1, T V2)
{
	return V0 * BaryCoord.X + V1 * BaryCoord.Y + V2 * BaryCoord.Z;
}

// Overload for FVector4 to work around C2719: (formal parameter with requested alignment of 16 won't be aligned)
inline FVector4 BarycentricInterpolate(FVector3f BaryCoord, const FVector4& V0, const FVector4& V1, const FVector4& V2)
{
	return V0 * BaryCoord.X + V1 * BaryCoord.Y + V2 * BaryCoord.Z;
}

	  ...
	  , FeatureLevel(GMaxRHIFeatureLevel)
{
	...
	{
		ComputeDispatchInterfacePtr = StaticCastSharedRef<FNiagaraGpuComputeDispatchInterface>(ComputeDispatchInterface->AsShared());
	}
...
void FNiagaraSystemInstance::SetGpuComputeDebug(bool bEnableDebug)
{
	...
	if (!ComputeDispatchInterfacePtr.IsValid() || !::IsValid(System))
	{
		return;
	}
	...
	if (bEnableDebug)
	{
		...
		(
			[RT_ComputeDispatchInterface=ComputeDispatchInterfacePtr.Get(), RT_InstanceID = GetId(), RT_SystemName = SystemName](FRHICommandListImmediate& RHICmdList)
			{
	...
	else
	{
		...
		(
			[RT_ComputeDispatchInterface=ComputeDispatchInterfacePtr.Get(), RT_InstanceID = GetId()](FRHICommandListImmediate& RHICmdList)
			{
				if (FNiagaraGpuComputeDebug* GpuComputeDebug = RT_ComputeDispatchInterface->GetGpuComputeDebugPrivate())
				{
...
void FNiagaraSystemInstance::UpdateLWCTile(const FVector3f& InLWCTile, FNiagaraDataSet& EmitterDataSet)
{
	...
	NiagaraLWCHelper::RebaseEmitters(TileShift, Emitters, ComputeDispatchInterfacePtr.Get());
...
bool FNiagaraSystemInstance::MultiTick(int32 TicksToProcess, float DeltaSecondsPerTick, float MaxSimTime, const FGraphEventRef& MyCompletionGraphEvent)
{
	SCOPE_CYCLE_COUNTER(STAT_NiagaraOverview_GT);
	SCOPE_CYCLE_COUNTER(STAT_NiagaraSystemInst_ComponentTickGT);
	CSV_SCOPED_TIMING_STAT_EXCLUSIVE(Effects);
	...
	return SystemSim->MultiTick_GameThread(TicksToProcess, DeltaSecondsPerTick, MaxSimTime, MyCompletionGraphEvent);
}
...
void FNiagaraSystemInstance::SimCacheTick_GameThread(UNiagaraSimCache* SimCache, float DesiredAge, float DeltaSeconds, const FGraphEventRef& MyCompletionGraphEvent)
{
	...
	SystemSimulation->SimCacheTick_GameThread(SimCache, DeltaSeconds, MyCompletionGraphEvent);
}

void FNiagaraSystemInstance::SimCacheTickFinalize_GameThread()
{
	...
	if (RequestedExecutionState == ENiagaraExecutionState::Complete)
	{
		Complete(false);
	}
	else

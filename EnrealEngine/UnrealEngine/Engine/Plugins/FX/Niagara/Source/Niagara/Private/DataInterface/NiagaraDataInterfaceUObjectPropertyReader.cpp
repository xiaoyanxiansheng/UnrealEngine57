...
bool UNiagaraDataInterfaceUObjectPropertyReader::PerInstanceTick(void* PerInstanceData, FNiagaraSystemInstance* SystemInstance, float DeltaSeconds)
{
	...
	{
		// Try Object Binding?
		const bool bTryObjectBinding =
			SourceMode == ENDIObjectPropertyReaderSourceMode::Binding ||
			SourceMode == ENDIObjectPropertyReaderSourceMode::BindingThenAttachParentActor ||
			SourceMode == ENDIObjectPropertyReaderSourceMode::BindingThenAttachParent;
		if (bTryObjectBinding)
		{
			ObjectBinding = InstanceData_GT->UObjectBinding.GetValue();
			ObjectBinding = ObjectBinding ? ObjectBinding : SourceActor.Get();
		}

		// If that failed do we look at the parent / parent actor?
		if (ObjectBinding == nullptr)
		{
			if (SourceMode == ENDIObjectPropertyReaderSourceMode::AttachParent || SourceMode == ENDIObjectPropertyReaderSourceMode::BindingThenAttachParent )
			{
				ObjectBinding = SystemInstance->GetAttachComponent();
			}
			else if (SourceMode == ENDIObjectPropertyReaderSourceMode::AttachParentActor || SourceMode == ENDIObjectPropertyReaderSourceMode::BindingThenAttachParentActor)
			{
...
void UNiagaraDataInterfaceUObjectPropertyReader::BuildShaderParameters(FNiagaraShaderParametersBuilder& ShaderParametersBuilder) const
{
	...
	const int32 PropertyOffsetsSize = FMath::Max(FMath::DivideAndRoundUp(ShaderParametersBuilder.GetGeneratedFunctions().Num(), 4), 1);
	ShaderParametersBuilder.AddLooseParamArray<FUintVector4>(TEXT("PropertyOffsets"), PropertyOffsetsSize);
}
...
void UNiagaraDataInterfaceUObjectPropertyReader::SetShaderParameters(const FNiagaraDataInterfaceSetShaderParametersContext& Context) const
{
	...
	const int32 PropertyOffsetsSize = InstanceData_RT.PropertyOffsets.Num() / 4;
	TArrayView<FUintVector4> PropertyOffsets = Context.GetParameterLooseArray<FUintVector4>(PropertyOffsetsSize);

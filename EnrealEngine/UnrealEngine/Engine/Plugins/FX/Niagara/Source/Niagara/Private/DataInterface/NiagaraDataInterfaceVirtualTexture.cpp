...
void UNiagaraDataInterfaceVirtualTexture::GetFunctionsInternal(TArray<FNiagaraFunctionSignature>& OutFunctions) const
{
...

	{
		...
		Sig.Outputs.Emplace(FNiagaraTypeDefinition::GetBoolDef(), TEXT("DisplacementValid"));
		Sig.Outputs.Emplace(FNiagaraTypeDefinition::GetBoolDef(), TEXT("Mask4Valid"));
	}

	{
		...
		Sig.Outputs.Emplace(FNiagaraTypeDefinition::GetFloatDef(), TEXT("Displacement"));
		Sig.Outputs.Emplace(FNiagaraTypeDefinition::GetVec4Def(), TEXT("Mask4"));
	}
}
#endif
			...
			[RT_Proxy=GetProxyAs<FNDIProxy>(), RT_InstanceID=SystemInstance->GetId(), RT_WeakTexture=TWeakObjectPtr<URuntimeVirtualTexture>(CurrentTexture)](FRHICommandListImmediate&)
			{
				...
				if (VirtualTexture != nullptr)
				{
					...
					for ( int32 iLayer=0; iLayer < MaxRVTLayers; ++iLayer)
					{
						...
						LayerTexture.TextureSRV = VirtualTexture->GetPhysicalTextureSRV(iLayer, RT_Texture->IsLayerSRGB(iLayer));
...
bool UNiagaraDataInterfaceVirtualTexture::UpgradeFunctionCall(FNiagaraFunctionSignature& Sig)
{
	...
	if (Sig.FunctionVersion < FNDIFunctionVersion::AddNewChannels)
	{
		if (Sig.Name == SampleRVTName)
		{
			Sig.Outputs.Emplace(FNiagaraTypeDefinition::GetFloatDef(), TEXT("Displacement"));
		...
		else if (Sig.Name == GetAttributesValidName)
		{
			Sig.Outputs.Emplace(FNiagaraTypeDefinition::GetBoolDef(), TEXT("DisplacementValid"));
			Sig.Outputs.Emplace(FNiagaraTypeDefinition::GetBoolDef(), TEXT("Mask4Valid"));
			bModified = true;

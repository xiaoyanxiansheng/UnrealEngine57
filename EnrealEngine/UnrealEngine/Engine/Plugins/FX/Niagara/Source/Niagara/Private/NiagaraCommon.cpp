...
#include UE_INLINE_GENERATED_CPP_BY_NAME(NiagaraCommon)
...
{
	auto DestroyInstanceNotComponent =
		[this](UNiagaraComponent* Component)
		{
			// Destroy the instance not the component and if we are going to recache the simulation make sure to kill the render state
			// Not killing the render state can result in crashes if it's recached and we tick before flushing EOF updates
			Component->DestroyInstanceNotComponent();
			if (Component->IsRenderStateCreated() && SystemSimsToRecache.Contains(Component->GetAsset()))
			{
				UPrimitiveComponent* PrimitiveComponent = static_cast<UPrimitiveComponent*>(Component);
				PrimitiveComponent->MarkRenderStateDirty();
				PrimitiveComponent->DestroyRenderState_Concurrent();
			}
		};
		...
		for (TObjectIterator<UNiagaraComponent> It; It; ++It)
		{
			...
			if ( ::IsValid(Comp) && SystemSimsToDestroy.Contains(Comp->GetAsset()) )
			{
				...
				if (SystemInstanceController && SystemInstanceController->HasValidSimulation() && SystemInstanceController->IsSolo())
				{
					...
					{
						DestroyInstanceNotComponent(Comp);
	...
	ComponentsToNotifySimDestroy.Empty();
}
...
void FNiagaraSystemUpdateContext::Add(const FVersionedNiagaraEmitterBase& Emitter, bool bReInit)
{
	AddComponentsWithMatchingSystem(bReInit,
...
void FNiagaraSystemUpdateContext::AddInternal(UNiagaraComponent* Comp, bool bReInit, bool bAllowDestroySystemSim)
{
	...
	const bool bIsActive = Comp->IsActiveForUpdateContext();
...
void FNiagaraVariableAttributeBinding::SetValue(const FName& InValue, const FVersionedNiagaraEmitterBase& InVersionedEmitter, ENiagaraRendererSourceDataMode InSourceMode)
{
	UNiagaraEmitterBase* Emitter = InVersionedEmitter.Emitter;
	RootName = InValue;

	FNiagaraVariableBase RootVariable = GetRootVariable();
	...
	RootName = RootVariable.GetName();

	CacheValues(InVersionedEmitter, InSourceMode);
}

void FNiagaraVariableAttributeBinding::SetAsPreviousValue(const FNiagaraVariableBase& Src, const FVersionedNiagaraEmitterBase& InEmitter, ENiagaraRendererSourceDataMode InSourceMode)
{
	static const FString PreviousNamespace = FNiagaraConstants::PreviousNamespace.ToString();

	ParamMapVariable = Src;
	RootName = Src.GetName();
...
void FNiagaraVariableAttributeBinding::SetAsPreviousValue(const FNiagaraVariableAttributeBinding& Src, const FVersionedNiagaraEmitterBase& InEmitter, ENiagaraRendererSourceDataMode InSourceMode)
{
	static const FString PreviousNamespace = FNiagaraConstants::PreviousNamespace.ToString();

	const FNiagaraVariableBase SrcRootVariable = Src.GetRootVariable();

	ParamMapVariable = SrcRootVariable;
	RootName = SrcRootVariable.GetName();
	DataSetName = SrcRootVariable.GetName();

	// Split out the name and it's namespace
	TStringBuilder<128> VarName;
	TArray<FStringView, TInlineAllocator<16>> SplitName;
	SrcRootVariable.GetName().ToString(VarName);
	...
	if (bIsPrev)
	{
		SetValue(SrcRootVariable.GetName(), InEmitter, InSourceMode);
	}
	else
	{
...
{
	RootName = InRootVar.GetName();
	ParamMapVariable.SetType(InRootVar.GetType());
#if WITH_EDITORONLY_DATA
	DefaultValue = InDefaultValue;
#endif
	SetValue(InRootVar.GetName(), FVersionedNiagaraEmitterBase(), InSourceMode);
}
...
FString FNiagaraVariableAttributeBinding::GetDefaultValueString() const
{
	...
	{
		if (DefaultValue.IsDataAllocated())
		{
			DefaultValueStr = DefaultValue.GetType().ToString(DefaultValue.GetData());
			DefaultValueStr.TrimEndInline();
		}
		else
		{
			DefaultValueStr = DefaultValue.GetName().ToString();
		}
	}
...
{
#if WITH_EDITORONLY_DATA
	if (!RootVariable_DEPRECATED.GetName().IsNone())
	{
		RootName = RootVariable_DEPRECATED.GetName();
	}
	if (BoundVariable.IsValid())
	{
		SetValue(BoundVariable.GetName(), FVersionedNiagaraEmitterBase(), InSourceMode);
...
void FNiagaraVariableAttributeBinding::Dump() const
{
	...
	UE_LOG(LogNiagara, Log, TEXT("RootVariable: %s"), *RootName.ToString());
#if WITH_EDITORONLY_DATA
...
void FNiagaraVariableAttributeBinding::ResetToDefault(const FNiagaraVariableAttributeBinding& InOther, const FVersionedNiagaraEmitterBase& InEmitter, ENiagaraRendererSourceDataMode InSourceMode)
{
	if (InOther.BindingSourceMode == ImplicitFromSource || InOther.BindingSourceMode == ENiagaraBindingSource::ExplicitEmitter || InOther.BindingSourceMode == ENiagaraBindingSource::ExplicitParticles)
	{
		...
		FNiagaraVariableBase TempVar = InOther.GetRootVariable();
		...
		SetValue(TempVar.GetName(), FVersionedNiagaraEmitterBase(), InSourceMode);
	}
	else
	{
		SetValue(InOther.RootName, InEmitter, InSourceMode);
	}
}
...
bool FNiagaraVariableAttributeBinding::MatchesDefault(const FNiagaraVariableAttributeBinding& InOther, ENiagaraRendererSourceDataMode InSourceMode) const
{
	if (DataSetName != InOther.DataSetName)
		return false;
	if (RootName != InOther.RootName)
		return false;
	return true;
}


bool FNiagaraVariableAttributeBinding::RenameVariableIfMatching(const FNiagaraVariableBase& OldVariable, const FNiagaraVariableBase& NewVariable, const FVersionedNiagaraEmitterBase& InEmitter, ENiagaraRendererSourceDataMode InSourceMode)
{
...
bool FNiagaraVariableAttributeBinding::Matches(const FNiagaraVariableBase& OldVariable, const FVersionedNiagaraEmitterBase& InEmitter, ENiagaraRendererSourceDataMode InSourceMode) const
{
	// First try a namespace mangling - free match.
	if (OldVariable.GetName() == ParamMapVariable.GetName() && OldVariable.GetType() == ParamMapVariable.GetType())
	{
...
void FNiagaraVariableAttributeBinding::CacheValues(const FVersionedNiagaraEmitterBase& InVersionedEmitter, ENiagaraRendererSourceDataMode InSourceMode)
{
...
void FNiagaraVariableAttributeBinding::SetValue(const FName& InValue, const FVersionedNiagaraEmitter& InEmitter, ENiagaraRendererSourceDataMode InSourceMode)
{
	SetValue(InValue, InEmitter.ToBase(), InSourceMode);
}
...
{
	SetAsPreviousValue(Src, InEmitter.ToBase(), InSourceMode);
}
...
void FNiagaraVariableAttributeBinding::CacheValues(const FVersionedNiagaraEmitter& InEmitter, ENiagaraRendererSourceDataMode InSourceMode)
{
	CacheValues(InEmitter.ToBase(), InSourceMode);
}
...
{
	return RenameVariableIfMatching(OldVariable, NewVariable, InEmitter.ToBase(), InSourceMode);
}

bool FNiagaraVariableAttributeBinding::Matches(const FNiagaraVariableBase& OldVariable, const FVersionedNiagaraEmitter& InEmitter, ENiagaraRendererSourceDataMode InSourceMode) const
{
	return Matches(OldVariable, InEmitter.ToBase(), InSourceMode);
}
...
{
	ResetToDefault(InOther, InEmitter.ToBase(), InSourceMode);
}
...
bool FNiagaraMaterialAttributeBinding::RenameVariableIfMatching(const FNiagaraVariableBase& OldVariable, const FNiagaraVariableBase& NewVariable, const UNiagaraEmitterBase* InEmitter, ENiagaraRendererSourceDataMode InSourceMode)
{
	// First try a namespace mangling - free match.
...
bool FNiagaraMaterialAttributeBinding::Matches(const FNiagaraVariableBase& OldVariable, const UNiagaraEmitterBase* InEmitter, ENiagaraRendererSourceDataMode InSourceMode)
{
	// First try a namespace mangling - free match.
	if (OldVariable.GetName() == NiagaraVariable.GetName() && OldVariable.GetType() == NiagaraVariable.GetType())
	{
...
void FNiagaraMaterialAttributeBinding::CacheValues(const UNiagaraEmitterBase* InEmitter)
{
	if (InEmitter != nullptr)
	{
		...
		{
			ResolvedNiagaraVariable = InEmitter->GetResolvedDIBinding(ResolvedNiagaraVariable);
		}
	}
	else
	{
...
{
	return AreBufferSRVsAlwaysCreatedByDefault(ShaderPlatform);
}

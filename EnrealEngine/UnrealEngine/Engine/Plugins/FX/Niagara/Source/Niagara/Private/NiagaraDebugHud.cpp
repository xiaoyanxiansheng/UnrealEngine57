...
namespace NiagaraDebugLocal
{
	...
	static FAutoConsoleCommandWithWorldAndArgs CmdNiagaraDebugHud(
		...
		FConsoleCommandWithWorldAndArgsDelegate::CreateLambda(
			[](const TArray<FString>& Args, UWorld*)
			{
				if ( Args.Num() == 0 )
				{
					const bool bIsEnabled = Settings.bHudEnabled && Settings.bOverviewEnabled;
					Settings.bHudEnabled = true;
					Settings.bOverviewEnabled = !bIsEnabled;
	...
	const FCachedVariables& GetCachedVariables(UNiagaraSystem* NiagaraSystem)
	{
		...
		if (CachedVariables == nullptr)
		{
			...
			{
				const TConstArrayView<FNiagaraEmitterHandle> EmitterHandles = NiagaraSystem->GetEmitterHandles();
				const TArray<TSharedRef<const FNiagaraEmitterCompiledData>>& AllStatefulEmittersCompiledData = NiagaraSystem->GetEmitterCompiledData();

				CachedVariables->ParticleVariables.AddDefaulted(EmitterHandles.Num());
				CachedVariables->ParticlePositionAccessors.AddDefaulted(EmitterHandles.Num());
				for (int32 iEmitter=0; iEmitter < EmitterHandles.Num(); ++iEmitter)
				{
					const FNiagaraEmitterHandle& EmitterHandle = EmitterHandles[iEmitter];
					...
					const FNiagaraDataSetCompiledData* EmitterCompiledData = nullptr;
					if (EmitterHandle.GetEmitterMode() == ENiagaraEmitterMode::Stateless)
					{
						...
						FNiagaraStatelessEmitterDataPtr EmitterData = StatelessEmitter ? StatelessEmitter->GetEmitterData() : nullptr;
						EmitterCompiledData = EmitterData ? EmitterData->ParticleDataSetCompiledData.Get() : nullptr;
					}
					else
					{
						EmitterCompiledData = AllStatefulEmittersCompiledData.IsValidIndex(iEmitter) ? &AllStatefulEmittersCompiledData[iEmitter]->DataSetCompiledData : nullptr;
					}

					if (EmitterCompiledData == nullptr)
					{
						continue;
					}
...
void FNiagaraDebugHud::GatherSystemInfo()
{
	...
	GlobalScalabiltiyKillCounts.Reset();

	if (FNiagaraWorldManager* WorldMan = FNiagaraWorldManager::Get(World))
	{
		const TMap<TWeakObjectPtr<UNiagaraSystem>, FNiagaraSystemScalabilityKillCounts>& SystemKillCounts = WorldMan->GetSystemScalabilityKillCounts();
		for (auto& KillCountEntry : SystemKillCounts)
		{
			if (UNiagaraSystem* System = KillCountEntry.Key.Get())
			{
				GlobalScalabiltiyKillCounts.Add(KillCountEntry.Value);
				FSystemDebugInfo& SystemDebugInfo = PerSystemDebugInfo.FindOrAdd(System->GetFName());
				SystemDebugInfo.ScalabilityKillCounts = KillCountEntry.Value;
			}
		}
	}
}
...
const FNiagaraDataSet* FNiagaraDebugHud::GetParticleDataSet(FNiagaraSystemInstance* SystemInstance, FNiagaraEmitterInstance* EmitterInstance, int32 iEmitter)
{
	...
	if (EmitterInstance->GetSimTarget() == ENiagaraSimTarget::GPUComputeSim)
	{
...
void FNiagaraDebugHud::DrawOverview(class FNiagaraWorldManager* WorldManager, FCanvas* DrawCanvas)
{
	...
	if (Settings.bOverviewEnabled)
	{
		...
		{
			FString ScalabilitySleepVsKilledFmt(TEXT("%d | %d"));
			OverviewColumns.Emplace(TEXT("TotalActive:"), FString::FromInt(GlobalTotalActive), TEXT("# Active"), ColumnOffset, Font, TEXT("0000"),
				[&DetailColor, &DetailHighlightColor, &fAdvanceHeight](FCanvas* Canvas, UFont* Font, float X, float Y, FOverviewColumn& Col, const FSystemDebugInfo& SystemInfo)
				{
					FLinearColor RowBGColor = SystemInfo.UniqueColor;
			...
			OverviewColumns.Emplace(TEXT("TotalCulled:"), FString::Printf(TEXT("%d | %d"), GlobalTotalCulled, GlobalScalabiltiyKillCounts.Total), TEXT("# Culled (Asleep | Killed)"), ColumnOffset, Font, TEXT("000"),
				[&DetailColor, &DetailHighlightColor, &fAdvanceHeight](FCanvas* Canvas, UFont* Font, float X, float Y, FOverviewColumn& Col, const FSystemDebugInfo& SystemInfo)
				{
					FLinearColor RowBGColor = SystemInfo.UniqueColor;
					RowBGColor.A = Settings.SystemColorTableOpacity;
					Canvas->DrawTile(X, Y, Col.MaxWidth, fAdvanceHeight, 0, 0, 0, 0, RowBGColor);
					const FLinearColor RowColor = SystemInfo.bShowInWorld ? DetailHighlightColor : DetailColor;
					FString ValueStr = FString::Printf(TEXT("%d | %d"), SystemInfo.TotalCulled, SystemInfo.ScalabilityKillCounts.Total);
					Canvas->DrawShadowedString(X, Y, *ValueStr, Font, RowColor);
				});

			OverviewColumns.Emplace(TEXT("Distance:"), FString::Printf(TEXT("%d | %d"), GlobalTotalCulledByDistance, GlobalScalabiltiyKillCounts.Distance), TEXT("# Distance (Asleep | Killed)"), ColumnOffset, Font, TEXT("000"),
				[&DetailColor, &DetailHighlightColor, &fAdvanceHeight](FCanvas* Canvas, UFont* Font, float X, float Y, FOverviewColumn& Col, const FSystemDebugInfo& SystemInfo)
				{
					FLinearColor RowBGColor = SystemInfo.UniqueColor;
					RowBGColor.A = Settings.SystemColorTableOpacity;
					Canvas->DrawTile(X, Y, Col.MaxWidth, fAdvanceHeight, 0, 0, 0, 0, RowBGColor);
					const FLinearColor RowColor = SystemInfo.bShowInWorld ? DetailHighlightColor : DetailColor;
					FString ValueStr = FString::Printf(TEXT("%d | %d"), SystemInfo.TotalCulledByDistance, SystemInfo.ScalabilityKillCounts.Distance);
					Canvas->DrawShadowedString(X, Y, *ValueStr, Font, RowColor);
				});

			OverviewColumns.Emplace(TEXT("Visibility:"), FString::Printf(TEXT("%d | %d"), GlobalTotalCulledByVisibility, GlobalScalabiltiyKillCounts.Visibility), TEXT("# Visibility (Asleep | Killed)"), ColumnOffset, Font, TEXT("000"),
				[&DetailColor, &DetailHighlightColor, &fAdvanceHeight](FCanvas* Canvas, UFont* Font, float X, float Y, FOverviewColumn& Col, const FSystemDebugInfo& SystemInfo)
				{
					FLinearColor RowBGColor = SystemInfo.UniqueColor;
					RowBGColor.A = Settings.SystemColorTableOpacity;
					Canvas->DrawTile(X, Y, Col.MaxWidth, fAdvanceHeight, 0, 0, 0, 0, RowBGColor);
					const FLinearColor RowColor = SystemInfo.bShowInWorld ? DetailHighlightColor : DetailColor;
					FString ValueStr = FString::Printf(TEXT("%d | %d"), SystemInfo.TotalCulledByVisibility, SystemInfo.ScalabilityKillCounts.Visibility);
					Canvas->DrawShadowedString(X, Y, *ValueStr, Font, RowColor);
				});

			OverviewColumns.Emplace(TEXT("InstanceCount:"), FString::Printf(TEXT("%d | %d"), GlobalTotalCulledByInstanceCount, GlobalScalabiltiyKillCounts.InstanceCounts), TEXT("# Inst Count (Asleep | Killed)"), ColumnOffset, Font, TEXT("000"),
				[&DetailColor, &DetailHighlightColor, &fAdvanceHeight](FCanvas* Canvas, UFont* Font, float X, float Y, FOverviewColumn& Col, const FSystemDebugInfo& SystemInfo)
				{
					FLinearColor RowBGColor = SystemInfo.UniqueColor;
					RowBGColor.A = Settings.SystemColorTableOpacity;
					Canvas->DrawTile(X, Y, Col.MaxWidth, fAdvanceHeight, 0, 0, 0, 0, RowBGColor);
					const FLinearColor RowColor = SystemInfo.bShowInWorld ? DetailHighlightColor : DetailColor;
					FString ValueStr = FString::Printf(TEXT("%d | %d"), SystemInfo.TotalCulledByInstanceCount, SystemInfo.ScalabilityKillCounts.InstanceCounts);
					Canvas->DrawShadowedString(X, Y, *ValueStr, Font, RowColor);
				});

			OverviewColumns.Emplace(TEXT("Budget:"), FString::Printf(TEXT("%d | %d"), GlobalTotalCulledByBudget, GlobalScalabiltiyKillCounts.GlobalBudget), TEXT("# Budget (Asleep | Killed)"), ColumnOffset, Font, TEXT("000"),
				[&DetailColor, &DetailHighlightColor, &fAdvanceHeight](FCanvas* Canvas, UFont* Font, float X, float Y, FOverviewColumn& Col, const FSystemDebugInfo& SystemInfo)
				{
					FLinearColor RowBGColor = SystemInfo.UniqueColor;
					RowBGColor.A = Settings.SystemColorTableOpacity;
					Canvas->DrawTile(X, Y, Col.MaxWidth, fAdvanceHeight, 0, 0, 0, 0, RowBGColor);
					const FLinearColor RowColor = SystemInfo.bShowInWorld ? DetailHighlightColor : DetailColor;
					FString ValueStr = FString::Printf(TEXT("%d | %d"), SystemInfo.TotalCulledByBudget, SystemInfo.ScalabilityKillCounts.GlobalBudget);
					Canvas->DrawShadowedString(X, Y, *ValueStr, Font, RowColor);
				});
	...
	// Display overview
	{
		...
		if (Settings.bOverviewEnabled || OverviewString.Len() > 0)
		{
			...
			const UEnum* EnumPtr = FindObjectChecked<UEnum>(nullptr, TEXT("/Script/Niagara.ENiagaraDebugHUDOverviewMode"), EFindObjectFlags::ExactClass);
			...
			{
				const UEnum* PerfGraphModeEnumPtr = FindObjectChecked<UEnum>(nullptr, TEXT("/Script/Niagara.ENiagaraDebugHUDPerfGraphMode"), EFindObjectFlags::ExactClass);
				const UEnum* PerfGraphSampleModeEnumPtr = FindObjectChecked<UEnum>(nullptr, TEXT("/Script/Niagara.ENiagaraDebugHUDPerfSampleMode"), EFindObjectFlags::ExactClass);

...
void FNiagaraRendererMeshes::PrepareParticleMeshRenderData(FRHICommandListBase& RHICmdList, FParticleMeshRenderData& ParticleMeshRenderData, const FSceneViewFamily& ViewFamily, FMeshElementCollector& Collector, FNiagaraDynamicDataBase* InDynamicData, const FNiagaraSceneProxy* SceneProxy, bool bRayTracing, ENiagaraGpuComputeTickStage::Type GpuReadyTickStage) const
{
	...
	ParticleMeshRenderData.bUseGPUScene = FNiagaraMeshVertexFactory::IsGPUSceneEnabled(SceneProxy->GetScene().GetShaderPlatform(), FeatureLevel);
...
FNiagaraMeshCommonParameters FNiagaraRendererMeshes::CreateCommonShaderParams(const FParticleMeshRenderData& ParticleMeshRenderData, const FSceneView& View, const FMeshData& MeshData, const FNiagaraSceneProxy& SceneProxy) const
{
	...
	if (SourceMode == ENiagaraRendererSourceDataMode::Particles)
	{
		...
		{
			Params.PrevScaleDataOffset			= SelectVFComponent(VFVariables, ENiagaraMeshVFLayout::PrevScale, ENiagaraMeshVFLayout::Scale);
			Params.PrevRotationDataOffset 		= SelectVFComponent(VFVariables, ENiagaraMeshVFLayout::PrevRotation, ENiagaraMeshVFLayout::Rotation);
			Params.PrevPositionDataOffset 		= SelectVFComponent(VFVariables, ENiagaraMeshVFLayout::PrevPosition, ENiagaraMeshVFLayout::Position);
			Params.PrevVelocityDataOffset 		= SelectVFComponent(VFVariables, ENiagaraMeshVFLayout::PrevVelocity, ENiagaraMeshVFLayout::Velocity);
			Params.PrevCameraOffsetDataOffset 	= SelectVFComponent(VFVariables, ENiagaraMeshVFLayout::PrevCameraOffset, ENiagaraMeshVFLayout::CameraOffset);
		}
	}
...
void FNiagaraRendererMeshes::GetDynamicRayTracingInstances(FRayTracingInstanceCollector& Collector, const FNiagaraSceneProxy* SceneProxy)
{
	...
	const FSceneView* FirstActiveView = Views[FirstActiveViewIndex]; // TODO: should use view-specific instances?

	const bool bIsInstancedStereo = FirstActiveView->bIsInstancedStereoEnabled && IStereoRendering::IsStereoEyeView(*FirstActiveView);

	FRHICommandListBase& RHICmdList = Collector.GetRHICommandList();

	check(FirstActiveView->Family);
	...
	FParticleMeshRenderData ParticleMeshRenderData;
	PrepareParticleMeshRenderData(RHICmdList, ParticleMeshRenderData, *FirstActiveView->Family, Collector, DynamicDataRender, SceneProxy, true, ENiagaraGpuComputeTickStage::PostInitViews);
	...
	if (ParticleMeshRenderData.bNeedsSort || ParticleMeshRenderData.bNeedsCull)
	{
		InitializeSortInfo(ParticleMeshRenderData, *SceneProxy, *FirstActiveView, FirstActiveViewIndex, bIsInstancedStereo, SortInfo);
	}
	...
	for (int32 MeshIndex = 0; MeshIndex < Meshes.Num(); ++MeshIndex)
	{
		...
		// TODO: Find a way to safely pool these such that they won't be concurrently accessed by multiple views
		FNiagaraMeshVertexFactory& VertexFactory = CollectorResources->VertexFactory;
		if (FirstActiveView->Family->Scene != nullptr)
		{
			VertexFactory.SetNiagaraMeshFeatureLevel(FirstActiveView->Family->Scene->GetFeatureLevel());
		}
		VertexFactory.EnablePrimitiveIDElement(ParticleMeshRenderData.bUseGPUScene);
		VertexFactory.InitResource(RHICmdList);
		...
		// Sort/Cull particles if needed.
		FNiagaraGpuComputeDispatchInterface* ComputeDispatchInterface = SceneProxy->GetComputeDispatchInterface();
		const uint32 NumInstances = PerformSortAndCull(RHICmdList, ParticleMeshRenderData, Collector.GetDynamicReadBuffer(), SortInfo, ComputeDispatchInterface, *FirstActiveView, MeshData);
		if ( NumInstances == 0 )
		{
			continue;
		}

		FNiagaraMeshCommonParameters CommonParameters = CreateCommonShaderParams(ParticleMeshRenderData, *FirstActiveView, MeshData, *SceneProxy);
		FNiagaraMeshUniformBufferRef VFUniformBuffer = CreateVFUniformBuffer(ParticleMeshRenderData, CommonParameters);
		VertexFactory.SetUniformBuffer(VFUniformBuffer);
		CollectorResources->UniformBuffer = VFUniformBuffer;
		...
		for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
		{
			...
			// Need to duplicate instance for each view since AddMesh(...) will cache pointers to entries in RayTracingInstance.Materials
			FRayTracingInstance ViewRayTracingInstance = RayTracingInstance;

			for (FMeshBatch& MeshBatch : ViewRayTracingInstance.Materials)
			{
				Collector.AddMesh(ViewIndex, MeshBatch);
			}

			// Need to use MoveTemp(...) to avoid reallocs that would invalidate mesh batch pointers cached above
			Collector.AddRayTracingInstance(ViewIndex, MoveTemp(ViewRayTracingInstance));
		}
	}
}
#endif

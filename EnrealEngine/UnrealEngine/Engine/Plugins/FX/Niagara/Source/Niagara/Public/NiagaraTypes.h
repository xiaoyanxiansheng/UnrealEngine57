...
struct FNiagaraBool
{
	...
	inline void SetRawValue(int32 RawValue) { Value = RawValue; }

	/** Gets this niagara bools raw integer value expected by the VM and HLSL. */
	inline int32 GetRawValue() const { return Value; }
	...
	inline operator bool() const { return GetValue(); }
...
struct FNiagaraPosition : public FVector3f
{
	...
	explicit inline FNiagaraPosition(EForceInit Init) : Super(Init) {}

	inline FNiagaraPosition(const float& X, const float& Y, const float& Z) : Super(X, Y, Z) {}

	inline FNiagaraPosition(const FVector3f& Other) : Super(Other) {}
	inline FNiagaraPosition(const FVector& Other) : Super(Other) {}
};
...
struct FNiagaraAssetVersion
{
	...
	friend inline uint32 GetTypeHash(const FNiagaraAssetVersion& Version)
	{
...
struct FNiagaraID
{
	...
	*/
	...
	friend inline uint32 GetTypeHash(const FNiagaraID& ID)
	{
...
public:
	...
	inline FNiagaraTypeDefinition(UObject* Object)
	...
	inline FNiagaraTypeDefinition(UClass *ClassDef)
	...
	inline FNiagaraTypeDefinition(UEnum *EnumDef)
	...
	inline FNiagaraTypeDefinition(UScriptStruct* StructDef, EAllowUnfriendlyStruct AllowUnfriendlyStruct)
	...
	inline FNiagaraTypeDefinition(UScriptStruct* StructDef) : FNiagaraTypeDefinition(StructDef, EAllowUnfriendlyStruct::Deny) {}

	inline FNiagaraTypeDefinition(FProperty* Property, EAllowUnfriendlyStruct AllowUnfriendlyStruct)
	{
	...
	inline bool operator !=(const FNiagaraTypeDefinition &Other) const
	{
		return !(*this == Other);
	}
	...
	inline bool IsSameBaseDefinition(const FNiagaraTypeDefinition& Other) const
	{
...
struct FNiagaraVariableBase
{
	...
	inline FNiagaraVariableBase(const FNiagaraTypeDefinition& InType, const FName& InName)
		: Name(InName)
	...
	inline FNiagaraVariableBase(const FNiagaraVariableCommonReference& VariableRef)
	...
	inline void SetName(FName InName)
	{
		Name = InName;
	}
	...
	NIAGARA_API void SetNamespacedName(const FString& InNamespace, FName InVariableName);
	...
	inline bool IsDataInterface()const { return GetType().IsDataInterface(); }
	inline bool IsUObject()const { return GetType().IsUObject(); }
	...
	*/
	...
	inline bool IsInNameSpace(const FStringView& Namespace) const
	{
	...
	inline bool IsInNameSpace(const FName& Namespace) const
	{
	...
	/**
	Check to see if this is an external variable or not.
	An external variable is defined as one that is not contained within the Niagara system, i.e. a user or parameter collection parameter.
	*/
...
protected:
	...
	friend inline uint32 GetTypeHash(const FNiagaraVariableBase& Var)
	{
...
struct FNiagaraVariable : public FNiagaraVariableBase
{
	...
	inline FNiagaraVariable(const FNiagaraTypeDefinition& InType, const FName& InName)
...
// Forward decl for base versioned types
...
struct FVersionedNiagaraEmitter
{
	...
	[[nodiscard]] NIAGARA_API FVersionedNiagaraEmitterData* GetEmitterData() const;
	[[nodiscard]] NIAGARA_API FVersionedNiagaraEmitterWeakPtr ToWeakPtr() const;
	[[nodiscard]] NIAGARA_API FVersionedNiagaraEmitterBase ToBase() const;
...
struct FVersionedNiagaraEmitterWeakPtr
{
	...
	[[nodiscard]] NIAGARA_API bool IsValid() const;
	[[nodiscard]] NIAGARA_API FVersionedNiagaraEmitter ResolveWeakPtr() const;
	[[nodiscard]] NIAGARA_API FVersionedNiagaraEmitterData* GetEmitterData() const;
	[[nodiscard]] NIAGARA_API FVersionedNiagaraEmitterBaseWeakPtr ToWeakBase() const;

...
void UNiagaraDataInterfaceSkeletalMesh::RandomTriangle(FVectorVMExternalFunctionContext& Context)
{
	...
	if (bAreaWeighted)
	{
		...
		const FSkeletalMeshSamplingBuiltData& SamplingInfoBuiltData = SamplingInfo.GetBuiltData();
		if (SamplingInfoBuiltData.WholeMeshBuiltData.IsValidIndex(LODIndex))
		{
			//-TODO: Consider adding a way to warn the user we failed to get the area weighted data
			const FSkeletalMeshSamplingLODBuiltData& WholeMeshBuiltData = SamplingInfoBuiltData.WholeMeshBuiltData[LODIndex];
			if (WholeMeshBuiltData.AreaWeightedTriangleSampler.GetNumEntries() > 0)
			{
				for (int32 i = 0; i < Context.GetNumInstances(); ++i)
				{

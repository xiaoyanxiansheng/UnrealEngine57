	...
	if (InstanceData && InstanceData->TextureReferenceRHI.IsValid())
	{
		...
		if (Context.IsResourceBound(&Parameters->Texture))
		{
			...
			if (InstanceData && RDGTexture == nullptr)
			{
				...
				{
					InstanceData->TransientRDGTexture = GraphBuilder.FindExternalTexture(ResolvedTextureRHI);
					if (InstanceData->TransientRDGTexture == nullptr)
					{
						InstanceData->TransientRDGTexture = GraphBuilder.RegisterExternalTexture(CreateRenderTarget(ResolvedTextureRHI, TEXT("NiagaraTexture2DArray")));
			...
			Parameters->Texture = RDGTexture ? RDGTexture : Context.GetComputeDispatchInterface().GetBlackTexture(GraphBuilder, ETextureDimension::Texture2DArray);
		}
	}
	else
	{
		...
		Parameters->Texture = Context.GetComputeDispatchInterface().GetBlackTexture(GraphBuilder, ETextureDimension::Texture2DArray);
	}
}

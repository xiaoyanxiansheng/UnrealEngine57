...
struct FNiagaraParameterStoreToDataSetBinding
{
	...
	inline void DataSetToParameterStore(FNiagaraParameterStore& ParameterStore, FNiagaraDataSet& DataSet, int32 DataSetInstanceIndex)
	{
	...
	inline void ParameterStoreToDataSet(const FNiagaraParameterStore& ParameterStore, FNiagaraDataSet& DataSet, int32 DataSetInstanceIndex)
	{
...
public:
	...
	bool MultiTick_GameThread(int32 TicksToProcess, float DeltaSecondsPerTick, float MaxSimTime, const FGraphEventRef& MyCompletionGraphEvent);
	...
	/** Game thread portion of sim cache tick.  Must run on GameThread. */
	void SimCacheTick_GameThread(UNiagaraSimCache* SimCache, float DeltaSeconds, const FGraphEventRef& MyCompletionGraphEvent);
	/** Concurrent portion of sim cache tick, can run on any thread. */
	void SimCacheTick_Concurrent(UNiagaraSimCache* SimCache, float DeltaSeconds);
	...
	inline FNiagaraGpuComputeDispatchInterface* GetDispatchInterface() const { return DispatchInterface; }

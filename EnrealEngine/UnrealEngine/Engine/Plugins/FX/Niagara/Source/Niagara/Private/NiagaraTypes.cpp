...
bool FNiagaraVariableBase::IsInExternalNamespace() const
{
	const FNameBuilder NameString(Name);
	const FStringView NameStringView(NameString);

	for (const FString& Namespace : { FNiagaraConstants::UserNamespaceString, FNiagaraConstants::ParameterCollectionNamespaceString })
	{
		if ((NameStringView.Len() > Namespace.Len() + 1) && (NameStringView[Namespace.Len()] == '.') && NameStringView.StartsWith(Namespace))
		{
			return true;
		}
	}
...
FVersionedNiagaraEmitterBase FVersionedNiagaraEmitter::ToBase() const
{
	return FVersionedNiagaraEmitterBase(Emitter, Version);
}
...
FVersionedNiagaraEmitterBaseWeakPtr FVersionedNiagaraEmitterWeakPtr::ToWeakBase() const
{

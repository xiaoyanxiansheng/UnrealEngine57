...
struct FNiagaraSimCacheHelper
{
	...
	void BuildCacheLayoutForSystem(const FNiagaraSimCacheCreateParameters& CreateParameters, FNiagaraSimCacheDataBuffersLayout& CacheLayout)
	{
		...
		if (CreateParameters.bAllowInterpolation)
		{
			for (const FNiagaraVariableBase& Variable : SystemCompileData.Variables)
			{
				if (!CanInterpolateVariable(CreateParameters.bInterpolateAllTypes, Variable) || CreateParameters.InterpolationExcludeAttributes.Contains(Variable.GetName()))
				{
					continue;
				}
		...
		for (const FNiagaraSimCacheVariable& SourceVariable : CacheLayout.Variables)
		{
			...
			if (DestVariableLayout != nullptr)
			{
				...
				if (DestVariableLayout && (bInterpVariable || bRebaseVariable))
				{
					...
					{
						if (SourceVariable.Variable.GetType() == FNiagaraTypeDefinition::GetFloatDef())
						{
							CacheBufferReadInfo.VariableCopyMappingsToDataBuffer.Emplace(SourceVariable.FloatOffset, IntCastChecked<uint16>(DestVariableLayout->GetFloatComponentStart()), &FNiagaraSimCacheHelper::InterpFloats<1>);
							DestVariableLayout = nullptr;
						}
						else if (SourceVariable.Variable.GetType() == FNiagaraTypeDefinition::GetVec2Def())
						{
							CacheBufferReadInfo.VariableCopyMappingsToDataBuffer.Emplace(SourceVariable.FloatOffset, IntCastChecked<uint16>(DestVariableLayout->GetFloatComponentStart()), &FNiagaraSimCacheHelper::InterpFloats<2>);
							DestVariableLayout = nullptr;
						}
						else if (SourceVariable.Variable.GetType() == FNiagaraTypeDefinition::GetVec3Def())
						{
							CacheBufferReadInfo.VariableCopyMappingsToDataBuffer.Emplace(SourceVariable.FloatOffset, IntCastChecked<uint16>(DestVariableLayout->GetFloatComponentStart()), &FNiagaraSimCacheHelper::InterpFloats<3>);
							DestVariableLayout = nullptr;
						}
						else if (SourceVariable.Variable.GetType() == FNiagaraTypeDefinition::GetVec4Def() || SourceVariable.Variable.GetType() == FNiagaraTypeDefinition::GetColorDef())
						{
							CacheBufferReadInfo.VariableCopyMappingsToDataBuffer.Emplace(SourceVariable.FloatOffset, IntCastChecked<uint16>(DestVariableLayout->GetFloatComponentStart()), &FNiagaraSimCacheHelper::InterpFloats<4>);
							DestVariableLayout = nullptr;
						}
					}
				}
	...
	static bool CanInterpolateVariable(bool bInterpolateAllTypes, const FNiagaraVariableBase& Variable)
	{
		if (bInterpolateAllTypes)
		{
			if (
				Variable.GetType() == FNiagaraTypeDefinition::GetFloatDef() ||
				Variable.GetType() == FNiagaraTypeDefinition::GetVec2Def() ||
				Variable.GetType() == FNiagaraTypeDefinition::GetVec3Def() ||
				Variable.GetType() == FNiagaraTypeDefinition::GetVec4Def() ||
				Variable.GetType() == FNiagaraTypeDefinition::GetColorDef())
			{
				return true;
			}
		}
	...
	template<uint32 NumFloats>
	static void InterpFloats(const FNiagaraSimCacheDataBuffersLayout::FVariableCopyContext& CopyDataContext)
	{
		for (uint32 iComponent = 0; iComponent < NumFloats; ++iComponent)
		{
			const float* SrcAFloats = reinterpret_cast<const float*>(CopyDataContext.SourceACurr + (CopyDataContext.SourceAStride * iComponent));
			const float* SrcBFloats = reinterpret_cast<const float*>(CopyDataContext.SourceBCurr + (CopyDataContext.SourceBStride * iComponent));
			float* DstFloats  = reinterpret_cast<float*>(CopyDataContext.DestCurr + (CopyDataContext.DestStride * iComponent));
			for (uint32 iInstanceA=0; iInstanceA < CopyDataContext.NumInstances; ++iInstanceA)
			{
				const uint32 iInstanceB = CopyDataContext.InterpMappings[iInstanceA];
				const float FloatA = SrcAFloats[iInstanceA];
				const float FloatB = iInstanceB == INDEX_NONE ? FloatA : SrcBFloats[iInstanceB];
				DstFloats[iInstanceA] = FMath::Lerp(FloatA, FloatB, CopyDataContext.FrameFraction);
			}
		}
	}

...
namespace NDIDynamicMeshLocal
{
	...
	struct FNDIInstanceData_GameThread
	{
		...
		void GetVertexTangentBasis(uint32 Vertex, FVector3f& OutTangentX, FVector3f& OutTangentY, FVector3f& OutTangentZ)
		{
			...
			OutTangentZ = (OutTangentZ / 127.5f) - 1.0f;
	...
	struct FNDIInstanceData_RenderThread
	{
		...
		void UpdateData(FRHICommandListImmediate& RHICmdList, FGameToRenderData& GameToRenderData)
		{
			...
			VertexBufferSize			= GameToRenderData.VertexBufferSize;
...
UObject* UNiagaraDataInterfaceDynamicMesh::SimCacheBeginWrite(UObject* SimCache, FNiagaraSystemInstance* NiagaraSystemInstance, const void* OptionalPerInstanceData, FNiagaraSimCacheFeedbackContext& FeedbackContext) const
{
	return OptionalPerInstanceData ? NewObject<UNiagaraDataInterfaceDynamicMeshSimCacheData>(SimCache) : nullptr;
}

bool UNiagaraDataInterfaceDynamicMesh::SimCacheWriteFrame(UObject* StorageObject, int FrameIndex, FNiagaraSystemInstance* SystemInstance, const void* OptionalPerInstanceData, FNiagaraSimCacheFeedbackContext& FeedbackContext) const
{
	using namespace NDIDynamicMeshLocal;

	check(OptionalPerInstanceData && StorageObject);
	UNiagaraDataInterfaceDynamicMeshSimCacheData* CacheData = CastChecked<UNiagaraDataInterfaceDynamicMeshSimCacheData>(StorageObject);
	CacheData->ResizeFrames(FrameIndex);

	// Build MeshDesc (shared between RT / GT)
	const FNDIInstanceData_GameThread* InstanceData_GT = reinterpret_cast<const FNDIInstanceData_GameThread*>(OptionalPerInstanceData);

	FNDIDynamicMeshSimCacheFrame MeshDesc;
	MeshDesc.NumTriangles		= InstanceData_GT->NumTriangles;
	MeshDesc.NumVertices		= InstanceData_GT->NumVertices;
	MeshDesc.NumTexCoords		= InstanceData_GT->NumTexCoords;
	MeshDesc.LocalBounds		= InstanceData_GT->LocalBounds;
	MeshDesc.VertexBufferSize	= InstanceData_GT->VertexBufferSize;
	MeshDesc.PositionOffset		= InstanceData_GT->PositionOffset;
	MeshDesc.TangentBasisOffset	= InstanceData_GT->TangentBasisOffset;
	MeshDesc.TexCoordOffset		= InstanceData_GT->TexCoordOffset;
	MeshDesc.ColorOffset		= InstanceData_GT->ColorOffset;
	MeshDesc.Sections.Reserve(InstanceData_GT->Sections.Num());
	for (const FNDIInstanceSection& Section : InstanceData_GT->Sections)
	{
		FNDIDynamicMeshSimCacheSection& CacheSection = MeshDesc.Sections.AddDefaulted_GetRef();
		CacheSection.TriangleOffset		= Section.TriangleOffset;
		CacheSection.MaxTriangles		= Section.MaxTriangles;
		CacheSection.AllocatedTriangles	= Section.AllocatedTriangles;
		CacheSection.Material			= InstanceData_GT->Materials.IsValidIndex(Section.MaterialIndex) ? InstanceData_GT->Materials[Section.MaterialIndex].Material : nullptr;
	}

	// Add to CPU meshes
	CacheData->CpuFrameIndices[FrameIndex] = CacheData->FindOrCreateMesh(MeshDesc, InstanceData_GT->IndexData, InstanceData_GT->VertexData);
	...
	if (IsUsedWithGPUScript())
	{
		...
		ENQUEUE_RENDER_COMMAND(NDIDynamicMesh_SimCacheWrite)(
			[Proxy_RT=GetProxyAs<FNDIProxy>(), InstanceID=SystemInstance->GetId(), CacheData, MeshDesc, FrameIndex, ComputeInterface](FRHICommandListImmediate& RHICmdList) mutable
			{
				const FNDIInstanceData_RenderThread* InstanceData_RT = Proxy_RT->InstanceData_RT.Find(InstanceID);
				if (InstanceData_RT == nullptr)
				{
					return;
				}

				// Right now we are assuming that the GT data must match if it doesn't don't attempt to record the frame
				if ( (MeshDesc.NumTriangles != InstanceData_RT->NumTriangles) ||
					 (MeshDesc.NumVertices != InstanceData_RT->NumVertices) ||
					 (MeshDesc.NumTexCoords != InstanceData_RT->NumTexCoords) ||
					 //(MeshDesc.LocalBounds != InstanceData_RT->LocalBounds) ||
					 (MeshDesc.VertexBufferSize != InstanceData_RT->VertexBufferSize) ||
					 (MeshDesc.PositionOffset != InstanceData_RT->PositionOffset) ||
					 (MeshDesc.TangentBasisOffset != InstanceData_RT->TangentBasisOffset) ||
					 (MeshDesc.TexCoordOffset != InstanceData_RT->TexCoordOffset) ||
					 (MeshDesc.ColorOffset != InstanceData_RT->ColorOffset) ||
					 (MeshDesc.Sections.Num() != InstanceData_RT->NumSections) )
				{
					return;
				}

				if (InstanceData_RT->SectionPooledBuffer == nullptr || InstanceData_RT->IndexPooledBuffer == nullptr || InstanceData_RT->VertexPooledBuffer == nullptr)
				{
					return;
				}
				...
				FRDGBufferRef BuffersToReadback[3];
				BuffersToReadback[0] = GraphBuilder.RegisterExternalBuffer(InstanceData_RT->SectionPooledBuffer);
				BuffersToReadback[1] = GraphBuilder.RegisterExternalBuffer(InstanceData_RT->IndexPooledBuffer);
				BuffersToReadback[2] = GraphBuilder.RegisterExternalBuffer(InstanceData_RT->VertexPooledBuffer);

				TArray<uint32> GpuSectionData;
				TArray<uint8> GpuIndexData;
				TArray<uint8> GpuVertexData;

				FNiagaraGpuReadbackManager* ReadbackManager = ComputeInterface->GetGpuReadbackManager();
				ReadbackManager->EnqueueReadbacks(
					GraphBuilder,
					BuffersToReadback,
					[&](TConstArrayView<TPair<void*, uint32>> ReadbackData)
					{
						GpuSectionData.AddUninitialized(ReadbackData[0].Value / sizeof(uint32));
						FMemory::Memcpy(GpuSectionData.GetData(), ReadbackData[0].Key, GpuSectionData.NumBytes());
			
						GpuIndexData.AddUninitialized(ReadbackData[1].Value);
						FMemory::Memcpy(GpuIndexData.GetData(), ReadbackData[1].Key, ReadbackData[1].Value);

						GpuVertexData.AddUninitialized(ReadbackData[2].Value);
						FMemory::Memcpy(GpuVertexData.GetData(), ReadbackData[2].Key, ReadbackData[2].Value);
					}
				);
				...
				for (int iSection = 0; iSection < MeshDesc.Sections.Num(); ++iSection)
				{
					FNDIDynamicMeshSimCacheSection& Section = MeshDesc.Sections[iSection];
					Section.AllocatedTriangles = GpuSectionData[iSection * 2 + 1];
				}

				CacheData->GpuFrameIndices[FrameIndex] = CacheData->FindOrCreateMesh(MeshDesc, GpuIndexData, GpuVertexData);
			}
		);
...
bool UNiagaraDataInterfaceDynamicMesh::SimCacheReadFrame(const FNiagaraSimCacheDataInterfaceReadContext& ReadContext)
{
	using namespace NDIDynamicMeshLocal;

	const UNiagaraDataInterfaceDynamicMeshSimCacheData* CacheData = ReadContext.GetOptionalStorageObject<UNiagaraDataInterfaceDynamicMeshSimCacheData>();
	const int32 FrameIndex = ReadContext.GetFrameIndexA();

	// Copy data from cache if we have GPU data we take that as the authoritative version
	ENiagaraSimTarget SimTarget = ENiagaraSimTarget::GPUComputeSim;
	const FNDIDynamicMeshSimCacheFrame* CacheFrame = CacheData->GetFrameData(FrameIndex, SimTarget);
	if (CacheFrame == nullptr)
	{
		SimTarget = ENiagaraSimTarget::CPUSim;
		CacheFrame = CacheData->GetFrameData(FrameIndex, SimTarget);
	}

	FNDIInstanceData_GameThread* InstanceData_GT = ReadContext.GetPerInstanceData<FNDIInstanceData_GameThread>();
	if (CacheFrame)
	{
		InstanceData_GT->bHasChanged		= false;
		InstanceData_GT->NumTriangles		= CacheFrame->NumTriangles;
		InstanceData_GT->NumVertices		= CacheFrame->NumVertices;
		InstanceData_GT->NumTexCoords		= CacheFrame->NumTexCoords;
		InstanceData_GT->VertexBufferSize	= CacheFrame->VertexBufferSize;
		InstanceData_GT->PositionOffset		= CacheFrame->PositionOffset;
		InstanceData_GT->TangentBasisOffset	= CacheFrame->TangentBasisOffset;
		InstanceData_GT->TexCoordOffset		= CacheFrame->TexCoordOffset;
		InstanceData_GT->ColorOffset		= CacheFrame->ColorOffset;
		InstanceData_GT->IndexData			= CacheFrame->IndexData;
		InstanceData_GT->VertexData			= CacheFrame->VertexData;
		InstanceData_GT->Sections.SetNum(CacheFrame->Sections.Num());
		InstanceData_GT->Materials.Empty();

		for (int iSection = 0; iSection < CacheFrame->Sections.Num(); ++iSection)
		{
			const FNDIDynamicMeshSimCacheSection& CacheMeshSection = CacheFrame->Sections[iSection];
			FNDIInstanceSection& MeshSection = InstanceData_GT->Sections[iSection];
			UMaterialInterface* SectionMaterial = CacheMeshSection.Material ? CacheMeshSection.Material.Get() : UMaterial::GetDefaultMaterial(MD_Surface);

			MeshSection.TriangleOffset		= CacheMeshSection.TriangleOffset;
			MeshSection.MaxTriangles		= CacheMeshSection.MaxTriangles;
			MeshSection.AllocatedTriangles	= CacheMeshSection.AllocatedTriangles;
			MeshSection.MaterialIndex		= InstanceData_GT->Materials.IndexOfByPredicate([SectionMaterial](const FNDIInstanceMaterial& ExistingMaterial) { return ExistingMaterial.Material == SectionMaterial; });
			if (MeshSection.MaterialIndex == INDEX_NONE)
			{
				MeshSection.MaterialIndex = InstanceData_GT->Materials.AddDefaulted();
				InstanceData_GT->Materials[MeshSection.MaterialIndex].Material = SectionMaterial;
			}
		}

		TUniquePtr<FGameToRenderData> GameToRenderData;
		GameToRenderData.Reset(new FGameToRenderData(*InstanceData_GT));
		if (SimTarget == ENiagaraSimTarget::GPUComputeSim)
		{
			// Clear out the index / vertex data if it came from a GPU simulation
			// Note: We could do something more optimal here and avoid the copy
			InstanceData_GT->IndexData.Empty();
			InstanceData_GT->VertexData.Empty();
		}

		ENQUEUE_RENDER_COMMAND(NDIDynamicMesh_SimCacheRead)(
			[Proxy_RT=GetProxyAs<FNDIProxy>(), InstanceID=ReadContext.GetSystemInstance()->GetId(), GameToRenderData_RT=MoveTemp(GameToRenderData)](FRHICommandListImmediate& RHICmdList)
			{
				FNDIInstanceData_RenderThread& InstanceData_RT = Proxy_RT->InstanceData_RT.FindOrAdd(InstanceID);
				InstanceData_RT.UpdateData(RHICmdList, *GameToRenderData_RT.Get());
			}
		);
	}
	else
	{
		InstanceData_GT->bHasChanged		= false;
		InstanceData_GT->NumTriangles		= 0;
		InstanceData_GT->NumVertices		= 0;
		InstanceData_GT->NumTexCoords		= 0;
		InstanceData_GT->VertexBufferSize	= 0;
		InstanceData_GT->PositionOffset		= INDEX_NONE;
		InstanceData_GT->TangentBasisOffset	= INDEX_NONE;
		InstanceData_GT->TexCoordOffset		= INDEX_NONE;
		InstanceData_GT->ColorOffset		= INDEX_NONE;
		InstanceData_GT->Sections.Empty();
		InstanceData_GT->Materials.Empty();
		InstanceData_GT->IndexData.Empty();
		InstanceData_GT->VertexData.Empty();
	}
...
bool UNiagaraDataInterfaceDynamicMesh::SimCacheCompareFrame(const UObject* LhsStorageObject, const UObject* RhsStorageObject, int FrameIndex, TOptional<float> Tolerance, FString& OutErrors) const
{
	const UNiagaraDataInterfaceDynamicMeshSimCacheData* LhsCacheData = CastChecked<UNiagaraDataInterfaceDynamicMeshSimCacheData>(LhsStorageObject);
	const UNiagaraDataInterfaceDynamicMeshSimCacheData* RhsCacheData = CastChecked<UNiagaraDataInterfaceDynamicMeshSimCacheData>(RhsStorageObject);

	const bool LhsValid = LhsCacheData->CpuFrameIndices.IsValidIndex(FrameIndex) && LhsCacheData->GpuFrameIndices.IsValidIndex(FrameIndex);
	const bool RhsValid = RhsCacheData->CpuFrameIndices.IsValidIndex(FrameIndex) && RhsCacheData->GpuFrameIndices.IsValidIndex(FrameIndex);
	if (LhsValid != RhsValid)
	{
		OutErrors = FString::Format(TEXT("Frame Validity different LhsValid = {0} RhsValid = {1}"), { LhsValid, RhsValid });
		return false;
	}
	...
	{
		const FNDIDynamicMeshSimCacheFrame* LhsMeshData = LhsCacheData->GetFrameData(FrameIndex, SimTarget);
		const FNDIDynamicMeshSimCacheFrame* RhsMeshData = LhsCacheData->GetFrameData(FrameIndex, SimTarget);

		const bool LhsMeshDataValid = LhsMeshData != nullptr;
		const bool RhsMeshDataValid = RhsMeshData != nullptr;
		if (LhsMeshDataValid != RhsMeshDataValid)
		{
			OutErrors = FString::Format(TEXT("Mesh Data Validity different LhsMeshData = {0} RhsMeshData = {1}"), { LhsMeshDataValid, RhsMeshDataValid });
			return false;
		}

		if (LhsMeshDataValid)
		{
			if (!LhsMeshData->Equals(*RhsMeshData))
			{
				OutErrors = TEXT("Mesh Data Different");
				return false;
			}
		}
	}

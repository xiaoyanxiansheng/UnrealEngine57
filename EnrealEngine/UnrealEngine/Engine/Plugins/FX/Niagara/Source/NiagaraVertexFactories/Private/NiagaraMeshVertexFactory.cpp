...
// Main control for GPU scene on all platforms
...
bool FNiagaraMeshVertexFactory::IsGPUSceneEnabled(const FStaticShaderPlatform Platform, const FStaticFeatureLevel FeatureLevel)
{
	...
	return UseGPUScene(Platform, FeatureLevel);
#else
	return false;
#endif
}
...
void FNiagaraMeshVertexFactory::ModifyCompilationEnvironment(const FVertexFactoryShaderPermutationParameters& Parameters, FShaderCompilerEnvironment& OutEnvironment)
{
	...
	const bool bUseGPUScene = IsGPUSceneEnabled(Parameters.Platform, MaxSupportedFeatureLevel);
	...
	OutEnvironment.SetDefine(TEXT("NIAGARA_MESH_WITH_GPU_SCENE"), bUseGPUScene);
...
void FNiagaraMeshVertexFactory::GetPSOPrecacheVertexFetchElements(EVertexInputStreamType VertexInputStreamType, FVertexDeclarationElementList& Elements)
{
	...
	if (IsGPUSceneEnabled(GMaxRHIShaderPlatform, GMaxRHIFeatureLevel) && !PlatformGPUSceneUsesUniformBufferView(GMaxRHIShaderPlatform))
	{

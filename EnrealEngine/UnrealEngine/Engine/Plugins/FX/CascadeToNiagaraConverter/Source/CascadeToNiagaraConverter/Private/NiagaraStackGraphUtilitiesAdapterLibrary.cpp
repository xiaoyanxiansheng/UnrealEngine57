...
FName UFXConverterUtilitiesLibrary::GetNiagaraScriptInputTypeName(ENiagaraScriptInputType InputType)
{
	...
	case ENiagaraScriptInputType::Matrix:
		return FNiagaraTypeDefinition::GetMatrix4Def().GetFName();
...
UNiagaraScriptConversionContextInput* UFXConverterUtilitiesLibrary::CreateScriptInputLinkedEnum(const FString& ParameterNameString, const FString& UserDefinedEnumAssetPath)
{
	UUserDefinedEnum* UserDefinedEnum = LoadObject<UUserDefinedEnum>(nullptr, *UserDefinedEnumAssetPath);
	if (UserDefinedEnum == nullptr)
	{
		return nullptr;
	}

	UNiagaraClipboardFunctionInput* NewInput = UNiagaraClipboardEditorScriptingUtilities::CreateEnumLinkedValueInput(GetTransientPackage(), FName(), false, false, UserDefinedEnum, FName(ParameterNameString));
	if (NewInput != nullptr)
	{
...
{
	ensureMsgf(InputType != ENiagaraScriptInputType::Enum, TEXT("Use CreateEnumLinkedValueInput for Enum linked parameters"));
...
UNiagaraScriptConversionContextInput* UFXConverterUtilitiesLibrary::CreateScriptInputLinearColor(FLinearColor Value)
{
	UNiagaraClipboardFunctionInput* NewInput = UNiagaraClipboardEditorScriptingUtilities::CreateColorLocalValueInput(GetTransientPackage(), FName(), false, false, Value);
	const FNiagaraTypeDefinition& TargetTypeDef = FNiagaraTypeDefinition::GetColorDef();
	UNiagaraScriptConversionContextInput* Input = NewObject<UNiagaraScriptConversionContextInput>();
	Input->Init(NewInput, ENiagaraScriptInputType::LinearColor, TargetTypeDef);
	return Input;
}

UNiagaraScriptConversionContextInput* UFXConverterUtilitiesLibrary::CreateScriptInputMatrix(FMatrix44f Value)
{
	UNiagaraClipboardFunctionInput* NewInput = UNiagaraClipboardEditorScriptingUtilities::CreateMatrixLocalValueInput(GetTransientPackage(), FName(), false, false, Value);
	const FNiagaraTypeDefinition& TargetTypeDef = FNiagaraTypeDefinition::GetMatrix4Def();
	UNiagaraScriptConversionContextInput* Input = NewObject<UNiagaraScriptConversionContextInput>();
	Input->Init(NewInput, ENiagaraScriptInputType::Matrix, TargetTypeDef);
	return Input;
}

UNiagaraScriptConversionContextInput* UFXConverterUtilitiesLibrary::CreateScriptInputQuat(FQuat4f Value)
{
	UNiagaraClipboardFunctionInput* NewInput = UNiagaraClipboardEditorScriptingUtilities::CreateQuatLocalValueInput(GetTransientPackage(), FName(), false, false, Value);
	const FNiagaraTypeDefinition& TargetTypeDef = FNiagaraTypeDefinition::GetQuatDef();
	UNiagaraScriptConversionContextInput* Input = NewObject<UNiagaraScriptConversionContextInput>();
	Input->Init(NewInput, ENiagaraScriptInputType::Quaternion, TargetTypeDef);
	return Input;
}
...
UNiagaraScriptConversionContextInput* UFXConverterUtilitiesLibrary::CreateScriptInputVec4(FVector4f Value)
{
	UNiagaraClipboardFunctionInput* NewInput = UNiagaraClipboardEditorScriptingUtilities::CreateVec4LocalValueInput(GetTransientPackage(), FName(), false, false, Value);
	const FNiagaraTypeDefinition& TargetTypeDef = FNiagaraTypeDefinition::GetVec4Def();
	UNiagaraScriptConversionContextInput* Input = NewObject<UNiagaraScriptConversionContextInput>();
	Input->Init(NewInput, ENiagaraScriptInputType::Vec4, TargetTypeDef);
	return Input;
}
...
{
	return AddTemplateEmitter(NewEmitterNameString, TEXT("/Niagara/DefaultAssets/Templates/CascadeConversion/CompletelyEmpty"));
}

UNiagaraEmitterConversionContext* UNiagaraSystemConversionContext::AddTemplateEmitter(FString NewEmitterNameString, FString Template)
{
	FName NewEmitterName = FNiagaraEditorUtilities::GetUniqueObjectName(System, UNiagaraEmitter::StaticClass(), NewEmitterNameString);
	UNiagaraEmitter* NewEmitter = CastChecked<UNiagaraEmitter>(StaticLoadObject(UNiagaraEmitter::StaticClass(), nullptr, *Template));
...
UNiagaraEmitterConversionContext* UNiagaraSystemConversionContext::FindEmitterConversionContextByName(const FName& EmitterName)
{
	TObjectPtr<UNiagaraEmitterConversionContext>* FoundContext = EmitterNameToConversionContextMap.Find(EmitterName);
	return FoundContext ? FoundContext->Get() : nullptr;
}
...
{
	TObjectPtr<UNiagaraRendererProperties>* PropsPtr = RendererNameToStagedRendererPropertiesMap.Find(RendererNameString);
	return PropsPtr ? PropsPtr->Get() : nullptr;
}

TArray<UNiagaraRendererProperties*> UNiagaraEmitterConversionContext::GetAllRenderers()
{
	TArray<TObjectPtr<UNiagaraRendererProperties>> OutRendererProperties;
...
void UNiagaraEmitterConversionContext::InternalFinalizeEvents(UNiagaraSystemConversionContext* OwningSystemConversionContext)
{
	...
	for (FNiagaraEventHandlerAddAction& EventHandlerAddAction : EventHandlerAddActions)
	{
		...
		if (EventHandlerAddAction.Mode == ENiagaraEventHandlerAddMode::AddEventAndEventGenerator && bFinalizingFromSystemConversionContext)
		{
			...
			if (UNiagaraEmitterConversionContext* SourceEmitterConversionContext = OwningSystemConversionContext->FindEmitterConversionContextByName(SourceEmitterName))
			{
...
void UNiagaraEmitterConversionContext::SetRendererBinding(UNiagaraRendererProperties* InRendererProperties, FName BindingName, FName VariableToBindName, ENiagaraRendererSourceDataMode SourceDataMode)
{
	if (UNiagaraComponentRendererProperties* ComponentRendererProperties = Cast<UNiagaraComponentRendererProperties>(InRendererProperties))
	{
		for (FNiagaraComponentPropertyBinding& ComponentPropertyBinding : ComponentRendererProperties->PropertyBindings)
		{
			...
			{
				ComponentPropertyBinding.AttributeBinding.SetValue(VariableToBindName, Emitter.ToBase(), SourceDataMode);
				return;
			}
		}
	...
	AttributeBinding->SetValue(VariableToBindName, Emitter.ToBase(), SourceDataMode);
}

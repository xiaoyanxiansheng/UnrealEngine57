...
namespace OpImageMipmapInternal
{
	...
	*/
    ...
    void FImageOperator::ImageMipmap_PrepareScratch(const FImage* DestImage, int32 StartLevel, int32 LevelCount, FScratchImageMipmap& Scratch )
    {
		...
		if (UE::Mutable::Private::IsCompressedFormat(DestFormat))
		{
			// Is it a block format?
			if (UE::Mutable::Private::GetImageFormatData(DestFormat).PixelsPerBlockX > 1)
			{
		...
		int32 StartLevel, int32 NumLODs, const FMipmapGenerationSettings& Settings, bool bGenerateOnlyTail)
	{
		...
		const bool bIsBlockCompressedFormat = UE::Mutable::Private::IsBlockCompressedFormat(BaseFormat);
		const bool bIsCompressedFormat = UE::Mutable::Private::IsCompressedFormat(BaseFormat);

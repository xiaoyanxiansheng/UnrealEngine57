...
namespace UE::Mutable::Private
{
	...
	{
		// Index of the shape triangle and the type of binding, Cluster or Vertex. 
		uint32 TriangleIndex_BindingType_NotValid; // the 30 low bits are the triangle index, 30 ith bit is the binding type, 31 ith bit is set if the biding is valid. 
		
		// Barycentric coordinates on the shape triangle
		FVector2f PositionBaryCoords;
		...
		union
		{
			struct
			{
				FVector3f NormalBaryCoords; // NormalBaryCoords.Z is not used.
				FVector3f NormalD;
			} Vertex;
		...
		FORCEINLINE uint32 GetTriangleIndex() const
		{
			return TriangleIndex_BindingType_NotValid & 0x3FFFFFFF;
		}

		FORCEINLINE bool IsClusterBinding() const
		{
			return (TriangleIndex_BindingType_NotValid & 0x40000000) != 0;
		}

		FORCEINLINE bool IsValidBinding() const
		{
			return (TriangleIndex_BindingType_NotValid & 0x80000000) == 0;
		}

		FORCEINLINE void SetTriangleIndex(uint32 Index)
		{	
			check(Index < 0x3FFFFFFF)
			TriangleIndex_BindingType_NotValid = 
					(TriangleIndex_BindingType_NotValid & ~0x3FFFFFFF) | (Index & 0x3FFFFFFF);
		}

		FORCEINLINE void SetClusterBinding(bool Value)
		{
			uint32 ValueBits = uint32(Value) << 30;
			TriangleIndex_BindingType_NotValid = 
					(TriangleIndex_BindingType_NotValid & ~0x40000000) | ValueBits;
		}

		FORCEINLINE void SetBindNotValid(bool Value)
		{
			uint32 ValueBits = uint32(Value) << 31;
			TriangleIndex_BindingType_NotValid = 
					(TriangleIndex_BindingType_NotValid & ~0x80000000) | ValueBits;
		}


	};
	static_assert(sizeof(FReshapeVertexBindingData) == 4*13);
	...
	struct FReshapeVertexBindingDataBufferDescriptor
	{
		...
		constexpr static EMeshBufferSemantic Semantics[Channels] = { EMeshBufferSemantic::TriangleIndex, EMeshBufferSemantic::BarycentricCoords, EMeshBufferSemantic::Distance, EMeshBufferSemantic::Other };
		constexpr static EMeshBufferFormat Formats[Channels] = { EMeshBufferFormat::UInt32, EMeshBufferFormat::Float32, EMeshBufferFormat::Float32, EMeshBufferFormat::Float32 };
		constexpr static int32 Components[Channels] = { 1, 2, 3, 3+3+1 };
		constexpr static int32 Offsets[Channels] = { 0, 1*4, (1+2)*4, (1+2+3)*4 };
			...
			const float ValidityTolerance = UE_KINDA_SMALL_NUMBER)
	{
		...
		FMemory::Memzero(OutBindData);

		float Weight = MaskWeight;

		if (FMath::IsNearlyZero(MaskWeight))
		{
			OutBindData.SetBindNotValid(true);
			return;
		}
		...
		OutBindData.PositionBaryCoords.X = PositionBarycentric.X;
		OutBindData.PositionBaryCoords.Y = PositionBarycentric.Y;
	
		FVector2f NormalBarycentric = ComputeBarycentric((Point + Normal), 
				TriangleA + ShapeMesh.Normals[Triangle.A] * OutBindData.Vertex.NormalD.X, 
				TriangleB + ShapeMesh.Normals[Triangle.B] * OutBindData.Vertex.NormalD.Y, 
				TriangleC + ShapeMesh.Normals[Triangle.C] * OutBindData.Vertex.NormalD.Z); 

		OutBindData.Vertex.NormalBaryCoords.X = NormalBarycentric.X;
		OutBindData.Vertex.NormalBaryCoords.Y = NormalBarycentric.Y;

		check(FoundIndex >= 0);
		OutBindData.SetTriangleIndex((uint32)FoundIndex);
		OutBindData.Weight = FMath::Clamp(MaskWeight, 0.0f, 1.0f);

		OutBindData.SetBindNotValid(FMath::IsNearlyZero(OutBindData.Weight));
	}
			...
			const TArray<int32>& Cluster, FReshapeVertexBindingData& OutBindingData, float BindTolerance)
	{
		...
		if (OutBindingData.IsValidBinding())
		{
			const FShapeMeshDescriptorBind& ShapeMesh = ShapeMeshTree.GetMesh()->Mesh;
			UE::Geometry::FIndex3i Triangle = ShapeMesh.Triangles[OutBindingData.GetTriangleIndex()];

			FVector3f TriangleA = (FVector3f)ShapeMesh.Positions[Triangle.A];
			FVector3f TriangleB = (FVector3f)ShapeMesh.Positions[Triangle.B];
			FVector3f TriangleC = (FVector3f)ShapeMesh.Positions[Triangle.C];

			//TODO: store triangle area for scale.
			//OutBindingData.Cluster.AttachementArea = FVector3f::CrossProduct((TriangleB - TriangleA), (TriangleC - TriangleA)).Size() / 2.0f;
			OutBindingData.Cluster.AttachmentPoint = BoundPoint;
			OutBindingData.Cluster.AttachmentNormal = FPlane4f(TriangleA, TriangleB, TriangleC).GetNormal(); 

			// Mark binding as cluster.
			OutBindingData.SetClusterBinding(true);
		}
	}
			...
			const TArray<FBoneName>& PhysicsToDeform )
	{
		...
		for (int32 PointIndex = 0; PointIndex < TotalNumPoints; ++PointIndex)
		{
			...
			int32 TriangleIndex = VertexBindData.IsValidBinding() 
					? (int32)VertexBindData.GetTriangleIndex() 
					: -1;

			PhysicsBodyBindData[PointIndex] = FReshapePointBindingData
			{ 
				.S = VertexBindData.PositionBaryCoords.X, 
				.T = VertexBindData.PositionBaryCoords.Y, 
				.D = VertexBindData.D, 
				.Triangle = TriangleIndex,
				.Weight = VertexBindData.Weight,
			};
		}
				...
				for (const TArray<int32>& RigidCluster : VertexClusters)
				{
					...
					if (ClusterVertexCount)
					{
						...
						for (int32 I = 0; I < ClusterVertexCount; ++I)
						{
							...
							BindData[VertexIndex].Weight = FMath::Min(ClusterBinding.Weight, MaskWeight);

							// Invalidate binding for very small weights.
							BindData[VertexIndex].SetBindNotValid(FMath::IsNearlyZero(ClusterBinding.Weight));	
						}
					}
				}
			}
			...
			const FMesh* Mesh, FShapeMeshTree& ShapeMeshTree, const TArray<FBoneName>& BonesToDeform )
	{
		...
		for (int32 BoneIndex = 0; BoneIndex < BoneCount; ++BoneIndex)
		{
			...
			if (BindData.Weight > UE_SMALL_NUMBER && BindData.IsValidBinding())
			{
				SkeletonBindDataArray.Emplace(FReshapePointBindingData
				{ 
					.S = BindData.PositionBaryCoords.X, 
					.T = BindData.PositionBaryCoords.Y, 
					.D = BindData.D, 
					.Triangle = (int32)BindData.GetTriangleIndex(), 
					.Weight = BindData.Weight, 
				});

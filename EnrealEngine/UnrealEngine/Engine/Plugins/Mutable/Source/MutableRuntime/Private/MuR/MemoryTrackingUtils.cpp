		...
		static inline FAutoConsoleCommand CmdDumpMutableGlobalMemoryCounter = FAutoConsoleCommand(
			...
			FConsoleCommandDelegate::CreateStatic([]()
			{
				...
				UE_LOG(LogMutableCore, 
					   ...
					   double(UE::Mutable::Private::FGlobalMemoryCounter::GetCounter()) / double(1024*1024),
					   double(UE::Mutable::Private::FGlobalMemoryCounter::GetPeak())    / double(1024*1024)
				);
			})
		);
		...
		static inline FAutoConsoleCommand CmdDumpMutableGlobalMemoryAbsoluteCounter = FAutoConsoleCommand(
			...
			FConsoleCommandDelegate::CreateStatic([]()
			{
				...
				UE_LOG(LogMutableCore, 
					   ...
					   double(UE::Mutable::Private::FGlobalMemoryCounter::GetAbsoluteCounter()) / double(1024*1024),
					   double(UE::Mutable::Private::FGlobalMemoryCounter::GetAbsolutePeak())    / double(1024*1024)
				);
			})
		);

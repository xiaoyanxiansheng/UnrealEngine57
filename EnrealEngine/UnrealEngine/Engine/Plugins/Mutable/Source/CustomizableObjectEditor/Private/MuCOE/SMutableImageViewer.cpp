	...
	[
		...
		[
			...
			[
				...
				.Padding(FMargin(0, 4.0f, 0.0f, 4.0f))
				[
					SNew(STextBlock)
...
void SMutableImageViewer::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	...
	if (bIsPendingUpdate)
	{
		...
		if (MutableImage && MutableImage->GetSizeX()>0 && MutableImage->GetSizeY()>0)
		{
			...
			TSharedPtr<const UE::Mutable::Private::FImage> ImageToConvert = MutableImage;

			// \TODO: This test is a bit weak
			if (MutableImage->GetFormat() >= UE::Mutable::Private::EImageFormat::ASTC_4x4_RGB_LDR
				&&
				MutableImage->GetFormat() <= UE::Mutable::Private::EImageFormat::ASTC_10x10_RG_LDR)
			{
				...
				int32 Quality = 4;
				ImageToConvert = ImOp.ImagePixelFormat(Quality, MutableImage.Get(), UE::Mutable::Private::GetUncompressedFormat(MutableImage->GetFormat()) );
			}
...
void SMutableImageViewer::SetImage(const TSharedPtr<const UE::Mutable::Private::FImage>& InMutableImage, int32 CurrentLOD)
{
...
FText SMutableImageViewer::GetImageDescriptionLabel() const
{
	...
	UE::Mutable::Private::EImageFormat Format = MutableImage->GetFormat();
	FString Label = FString::Printf(TEXT("%s - %d x %d - %d LODs"), 
		StringCast<TCHAR>(UE::Mutable::Private::TypeInfo::s_imageFormatName[(size_t)Format]).Get(),

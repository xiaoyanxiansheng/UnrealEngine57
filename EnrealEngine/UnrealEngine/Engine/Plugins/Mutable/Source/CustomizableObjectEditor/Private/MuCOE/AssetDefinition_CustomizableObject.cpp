...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AssetDefinition_CustomizableObject)
...
EAssetCommandResult UAssetDefinition_CustomizableObject::ActivateAssets(const FAssetActivateArgs& ActivateArgs) const
{
	...
	if ( ActivationType == EAssetActivationMethod::DoubleClicked || ActivationType == EAssetActivationMethod::Opened )
	{
		if ( ActivateArgs.Assets.Num() == 1 )
		{
			...
			GEditor->GetEditorSubsystem<UAssetEditorSubsystem>()->OpenEditorForAsset(UE::Mutable::Private::LoadObject(*FirstAsset));
		}
		else if ( ActivateArgs.Assets.Num() > 1 )
		{
			...
			{
				ObjectsToOpen.Add(UE::Mutable::Private::LoadObject(ToOpen));
			}
...
namespace MenuExtension_CustomizableObject
{
	...
	void RecompileObjects(const FToolMenuContext& InMenuContext, ERecompileCO RecompileType)
	{
		...
		if (RecompileType == ERecompileCO::RCO_Selected || RecompileType == ERecompileCO::RCO_SelectedGatherReferences)
		{
			...
			for (UCustomizableObject* Object : Objects)
			{
				if (Object)
				{
					...
					if (RecompileType == ERecompileCO::RCO_SelectedGatherReferences)
					{
						NewRequest->Options.bGatherReferences = true;
					}
	...
	void GenerateCompilationSubmenu(UToolMenu* InMenu)
	{
		...
		// Recompile Selected Gather References.
		{
			const TAttribute<FText> Label = NSLOCTEXT("UAssetDefinition_CustomizableObject", "CustomizableObject_RecompileSelectedGatherReferences", "Selected and Gather references");
			const TAttribute<FText> ToolTip = NSLOCTEXT("UAssetDefinition_CustomizableObject", "CustomizableObject_RecompileSelectedGatherReferencesTooltip", "Recompile selected Customizable Object and gather references.");
			const FSlateIcon Icon = FSlateIcon();
			
			FToolUIAction UIAction;
			UIAction.ExecuteAction = FToolMenuExecuteAction::CreateStatic(&RecompileObjects,ERecompileCO::RCO_SelectedGatherReferences);
			Section.AddMenuEntry("CustomizableObject_ExecuteCompileSelectedGatherReferences", Label, ToolTip, Icon, UIAction);
		}

...
public:
	...
	TSharedPtr<FString> GetName()
	{
		...
		{
			TSharedPtr<UE::Mutable::Private::FParameters> TempParams = UE::Mutable::Private::FModel::NewParameters(Model);
...
void FCustomizableObjectDetails::CustomizeDetails( IDetailLayoutBuilder& DetailBuilder )
{
	...
	IDetailCategoryBuilder& CompileOptions = DetailBuilder.EditCategory("CompileOptions");
	...
	if (CustomizableObject && CustomizableObject->GetPrivate())
	{
		TArray<TSharedRef<IPropertyHandle>> CompileOptionsProperties;
		CompileOptions.GetDefaultProperties(CompileOptionsProperties);
		for (TSharedRef<IPropertyHandle>& Property : CompileOptionsProperties)
		{
			CompileOptions.AddProperty(Property);
		}

		IDetailPropertyRow* Row = CompileOptions.AddExternalObjectProperty({ CustomizableObject->GetPrivate() }, GET_MEMBER_NAME_CHECKED(UCustomizableObjectPrivate, bDisableTableMissingDataWarning));
	}

...
void SMutableProjectorViewer::SetProjector(const UE::Mutable::Private::FProjector& InMutableProjector)
{
...
UE::Mutable::Private::EProjectorType SMutableProjectorViewer::GetProjectorType() const
{
	const UE::Mutable::Private::EProjectorType ProjectorType = MutableProjector.type;
...
{
	const UE::Mutable::Private::EProjectorType ProjectorType =  GetProjectorType();
	const uint32 TypeIndex = static_cast<uint32> (ProjectorType);

	return FText::FromString(* FString(UE::Mutable::Private::TypeInfo::s_projectorTypeName[TypeIndex]));
}

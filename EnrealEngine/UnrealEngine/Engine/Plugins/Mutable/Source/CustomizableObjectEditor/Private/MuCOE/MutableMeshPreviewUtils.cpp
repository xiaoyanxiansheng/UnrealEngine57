...
namespace MutableMeshPreviewUtils
{
	namespace
	{
		 ...
		 * requires
		 */
		TSharedPtr<const UE::Mutable::Private::FMesh> GenerateCompatibleMutableMesh(const UE::Mutable::Private::FMesh* InMutableMesh)
		{
			// 1) Generate a mutable mesh to be the one hosting the buffer format we desire (it will be required for 
			TSharedPtr<UE::Mutable::Private::FMesh> FormattedMutableMesh = MakeShared<UE::Mutable::Private::FMesh>();
			UE::Mutable::Private::FMeshBufferSet& FormattedVertexBuffers = FormattedMutableMesh->GetVertexBuffers();
			UE::Mutable::Private::FMeshBufferSet& FormattedIndexBuffers = FormattedMutableMesh->GetIndexBuffers();
			...
			// 2) Generate the buffers for that mesh with the format unreal expects
			{
				...
				const UE::Mutable::Private::FMeshBufferSet& OriginVertexBuffers = InMutableMesh->GetVertexBuffers();
				...
				for (int32 BufferIndex = 0; BufferIndex < MutableMeshBuffersCount; BufferIndex++)
				{
					...
					{
						UE::Mutable::Private::EMeshBufferSemantic BufferSemantic = UE::Mutable::Private::EMeshBufferSemantic::None;
						UE::Mutable::Private::EMeshBufferFormat BufferFormat = UE::Mutable::Private::EMeshBufferFormat::None;
						...
						{
							case UE::Mutable::Private::EMeshBufferSemantic::Color:
							{
							...
							case UE::Mutable::Private::EMeshBufferSemantic::TexCoords:
							{
								// Store the texture semantic index to be later used during the buffer formatting process on  UE::Mutable::Private::MeshFormat
							...
							case UE::Mutable::Private::EMeshBufferSemantic::BoneIndices:
							{
								// Store the amount of bones a vertex can be skinned to.
								MaxNumBonesPerVertex = BufferComponentCount;
								BoneIndicesSizeBytes = BufferFormat == UE::Mutable::Private::EMeshBufferFormat::Int16 ? 2 : 1;
								break;
							}

							case UE::Mutable::Private::EMeshBufferSemantic::BoneWeights:
							{
								// Store the amount of bones a vertex can be skinned to.
								BoneWeightsSizeBytes = BufferFormat == UE::Mutable::Private::EMeshBufferFormat::NUInt16 ? 2 : 1;
								break;
							}
			...
			// 3) Call UE::Mutable::Private::MeshFormat to get the mesh with the wanted buffer structure to have the data of the original mesh
			TSharedPtr<UE::Mutable::Private::FMesh> FormatResult = MakeShared<UE::Mutable::Private::FMesh>();

			bool bOutSuccess = false;
			UE::Mutable::Private::MeshFormat(FormatResult.Get(), InMutableMesh, FormattedMutableMesh.Get(),
		 ...
		 */
		bool AreVertexBuffersCorrectlyLocated(const UE::Mutable::Private::FMesh* InMutableMesh, bool bLogFindingsToConsole = false )
		{
			...
			const UE::Mutable::Private::FMeshBufferSet& VertexBuffers = InMutableMesh->GetVertexBuffers();

			int32 Buffer = -1;
			int32 Channel = -1;

			// POSITION BUFFER INDEX CHECK
			VertexBuffers.FindChannel(UE::Mutable::Private::EMeshBufferSemantic::Position, 0, &Buffer, &Channel);
			if (Buffer == -1)
			{
			...
			// SKIN WEIGHTS CHECK
			VertexBuffers.FindChannel(UE::Mutable::Private::EMeshBufferSemantic::BoneIndices, 0, &Buffer, &Channel);
			if (Buffer == -1)
			{
				if (bLogFindingsToConsole)
				{
			...
			VertexBuffers.FindChannel(UE::Mutable::Private::EMeshBufferSemantic::Tangent, 0, &Buffer, &Channel);
			if (Buffer == -1)
			{
				if (bLogFindingsToConsole)
				{
			...
			VertexBuffers.FindChannel(UE::Mutable::Private::EMeshBufferSemantic::TexCoords, 0, &Buffer, &Channel);
			if (Buffer == -1)
			{
				if (bLogFindingsToConsole)
				{
		 ...
		 */
		bool BuildSkeletalMeshSkeletonData(const UE::Mutable::Private::FMesh* InMutableMesh, USkeletalMesh* OutSkeletalMesh)
		{
		 ...
		 */
		void BuildSkeletalMeshElementData(const UE::Mutable::Private::FMesh* InMutableMesh, USkeletalMesh* OutSkeletalMesh)
		{
		 ...
		 * @return True if the operation could be performed successfully, false if not.
		 */
		bool BuildSkeletalMeshRenderData(const UE::Mutable::Private::FMesh* InMutableMesh, USkeletalMesh* OutSkeletalMesh)
		{
	...
	USkeletalMesh* GenerateSkeletalMeshFromMutableMesh(const UE::Mutable::Private::FMesh* InMutableMesh)
	{
		if (!InMutableMesh)
		{
			return nullptr;
		...
		TSharedPtr<const UE::Mutable::Private::FMesh> MeshCopy;

...
UE::Mutable::Private::Ptr<UE::Mutable::Private::NodeExtensionData> UCustomizableObjectNodeExtensionDataVariation::GenerateMutableNode(FExtensionDataCompilerInterface& InCompilerInterface) const
{
	UE::Mutable::Private::Ptr<UE::Mutable::Private::NodeExtensionDataVariation> VariationNode = new UE::Mutable::Private::NodeExtensionDataVariation;
	...
	if (const UEdGraphPin* ConnectedPin = FollowInputPin(*GetDefaultPin()))
	{
		...
		{
			if (UE::Mutable::Private::Ptr<UE::Mutable::Private::NodeExtensionData> DefaultValueExtensionData = ExtensionNode->GenerateMutableNode(InCompilerInterface))
			{
	...
	for (int32 VariationIndex = 0; VariationIndex < Variations.Num(); ++VariationIndex)
	{
		if (const UEdGraphPin* VariationPin = GetVariationPin(VariationIndex))
		{
			...
			if (const UEdGraphPin* ConnectedPin = FollowInputPin(*VariationPin))
			{
				if (const ICustomizableObjectExtensionNode* ExtensionNode = Cast<ICustomizableObjectExtensionNode>(ConnectedPin->GetOwningNode()))
				{
					if (UE::Mutable::Private::Ptr<UE::Mutable::Private::NodeExtensionData> VariationExtensionData = ExtensionNode->GenerateMutableNode(InCompilerInterface))
					{

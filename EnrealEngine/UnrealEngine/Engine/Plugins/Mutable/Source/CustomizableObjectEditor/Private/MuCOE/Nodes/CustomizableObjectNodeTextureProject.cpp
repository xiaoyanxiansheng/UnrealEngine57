...
#include UE_INLINE_GENERATED_CPP_BY_NAME(CustomizableObjectNodeTextureProject)
...
void UCustomizableObjectNodeTextureProject::AllocateDefaultPins(UCustomizableObjectNodeRemapPins* RemapPins)
{
	...
	for ( uint32 LayerIndex = 0; LayerIndex < Textures; ++LayerIndex )
	{
		...
		UEdGraphPin* TexturePin = CustomCreatePin(EGPD_Input, Schema->PC_Texture, FName(*PinName));
	...
	UEdGraphPin* MeshMaskPin = CustomCreatePin(EGPD_Input, Schema->PC_Texture, FName(*PinName));

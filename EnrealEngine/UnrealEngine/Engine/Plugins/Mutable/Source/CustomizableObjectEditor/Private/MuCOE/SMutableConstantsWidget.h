...
{
	TSharedPtr<const UE::Mutable::Private::FImage> ImagePtr;
};
...
{
	TSharedPtr<const UE::Mutable::Private::FMesh> MeshPtr;
};
...
{
	TSharedPtr<const UE::Mutable::Private::FLayout> Layout;
};
...
{
	TSharedPtr<const UE::Mutable::Private::FSkeleton> Skeleton;
};
...
{
	TSharedPtr<const UE::Mutable::Private::FPhysicsBody> Physics;
};
...
{
	const UE::Mutable::Private::FProjector* Projector = nullptr;
};
...
{
	const UE::Mutable::Private::FShape* Shape = nullptr;
};
...
public:
	 ...
	 * @param InMutableProgramPtr - Pointer to the UE::Mutable::Private::FProgram object that holds the constants data.
	 * @param  InMutableCodeViewerPtr - Pointer to the MutableCodeViewer tasked with the previewing of the constant values
	 */
	void Construct(const FArguments& InArgs, const UE::Mutable::Private::FProgram*  InMutableProgramPtr ,  TSharedPtr<SMutableCodeViewer> InMutableCodeViewerPtr);
	 ...
	 * EX : We want to clear all selected elements but not the ones from the constant images. Provide as argument "UE::Mutable::Private::EDataType::Image"
	 * to clear all but the selected item/s of the Image type.
	 */
	void ClearSelectedConstantItems(UE::Mutable::Private::EDataType ExceptionDataType = UE::Mutable::Private::EDataType::None) const;

private:

	/** Mutable object containing the constants data */
	const UE::Mutable::Private::FProgram* MutableProgramPtr = nullptr;
	 ...
	 */
	void SetProgram(const UE::Mutable::Private::FProgram* InProgram);
	...
	*/
	...
	/**
	 * Load up all the elements with the data found on the UE::Mutable::Private::FProgram object onto TArrays after parsing the data found.
	 */

...
void SMutableMeshViewer::SetMesh(const TSharedPtr<const UE::Mutable::Private::FMesh>& InMesh)
{
	if (InMesh != MutableMesh)
	{
		...
		if (MutableMesh)
		{
			...
			if (TSharedPtr<const UE::Mutable::Private::FSkeleton> MutableSkeleton = MutableMesh->GetSkeleton())
			{
...
TArray<FVector2f> SMutableMeshViewer::GetUVs() const
{
	...
	if (MutableMesh && !MutableMesh->VertexBuffers.IsDescriptor())
	{
		...
		UE::Mutable::Private::UntypedMeshBufferIteratorConst ItIndices(MutableMesh->IndexBuffers, UE::Mutable::Private::EMeshBufferSemantic::VertexIndex, 0);
		UE::Mutable::Private::UntypedMeshBufferIteratorConst It(MutableMesh->VertexBuffers, UE::Mutable::Private::EMeshBufferSemantic::TexCoords, UVIndex);
...
{
	// Cache the data accessible from the UE::Mutable::Private::FMesh to be later used by the UI
...
void SMutableMeshViewer::FillTargetBufferSetDataArray(const UE::Mutable::Private::FMeshBufferSet& InMuMeshBufferSet,
                                                      ...
                                                      TSharedPtr<SListView<TSharedPtr<FBufferElement>>>& InHostListView)
{
	...
	for (int32 BufferIndexOnBufferSet = 0; BufferIndexOnBufferSet < BuffersCount; BufferIndexOnBufferSet++)
	{
		...
		for (int32 ChannelIndexOnBuffer = 0; ChannelIndexOnBuffer < ChannelsOnBuffer; ChannelIndexOnBuffer++)
		{
			...
			UE::Mutable::Private::EMeshBufferSemantic BufferChannelSemantic = UE::Mutable::Private::EMeshBufferSemantic::None;
			...
			// Using UE::Mutable::Private::TypeInfo find what is the name of the buffer semantic and the buffer format
			NewChannelDefinition->SemanticIndex = FText::AsNumber(SemanticIndex);
			NewChannelDefinition->BufferSemantic = FText::FromString(
				*FString(UE::Mutable::Private::TypeInfo::s_meshBufferSemanticName[SemanticTypeIndex]));
			NewChannelDefinition->BufferFormat = FText::FromString(
				*FString(UE::Mutable::Private::TypeInfo::s_meshBufferFormatName[BufferFormatIndex]));

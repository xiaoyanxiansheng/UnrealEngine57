...
namespace UE::Mutable::Private { enum class EProjectorType : uint32; }


class SMutableProjectorViewer final : public SCompoundWidget
{
	SLATE_BEGIN_ARGS(SMutableProjectorViewer) {}
		SLATE_ARGUMENT(UE::Mutable::Private::FProjector,MutableProjector);
	SLATE_END_ARGS()
...
public:
	 ...
	 */
	void SetProjector(const UE::Mutable::Private::FProjector& InMutableProjector);

private:

	/** The mutable projector witch data is being exposed on the UI */
	UE::Mutable::Private::FProjector MutableProjector;
	...
	*/		
	 ...
	 */
	UE::Mutable::Private::EProjectorType GetProjectorType() const;

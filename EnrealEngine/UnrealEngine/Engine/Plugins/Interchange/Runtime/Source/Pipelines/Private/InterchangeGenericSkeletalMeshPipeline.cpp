...
void UInterchangeGenericMeshPipeline::ExecutePreImportPipelineSkeletalMesh()
{
		...
		, const TArray<FString>& MeshUids)
		{
			...
			if (MeshUidsPerLodIndex.Num() > 0)
			{
				...
				if (SkeletalMeshFactoryNode != nullptr)
				{
					...
					UpdateAssemblyPartDependencyTable(SkeletalMeshFactoryNode, MeshUidsPerLodIndex);
	...
	if (bCombineSkeletalMeshes)
	{
		...
		PipelineMeshesUtilities->GetCombinedSkinnedMeshInstances(MeshUidsPerSkeletonRootUid, bUseSingleBoneForConvertedMeshes);
		constexpr bool bUseMeshInstance = true;
...
UInterchangeSkeletalMeshFactoryNode* UInterchangeGenericMeshPipeline::CreateSkeletalMeshFactoryNode(const FString& RootJointUid, const TMap<int32, TArray<FString>>& MeshUidsPerLodIndex)
{
	...
	auto HasNaniteAssemblyDependencies = [this, &MeshUidsPerLodIndex](const int32 LodIndex)
	{
		...
		const TArray<FString>* MeshUids = MeshUidsPerLodIndex.Find(LodIndex);
		if (ensure(MeshUids))
		{
			for (const FString& MeshUid : *MeshUids)
			{
				...
				if (const UInterchangeSceneNode* SceneNode = Cast<const UInterchangeSceneNode>(BaseNodeContainer->GetNode(MeshUid)))
				{
					FString MeshNodeUid;
	...
	auto GetFirstNodeInfo = [this, &MeshUidsPerLodIndex](const int32 LodIndex, FString& OutFirstMeshNodeUid, int32& OutSceneNodeCount)->const UInterchangeBaseNode*
	{
		OutSceneNodeCount = 0;
		const TArray<FString>* MeshUids = MeshUidsPerLodIndex.Find(LodIndex);
		if (ensure(MeshUids))
		{
			FString FirstNodeUid = MeshUids->Num() > 0 ? (*MeshUids)[0] : FString();
			for (int32 Index = 1; Index < MeshUids->Num(); ++Index)
			{
				if ((*MeshUids)[Index] < FirstNodeUid)
				{
					FirstNodeUid = (*MeshUids)[Index];
				}
			}
	...
	if (bRecomputeTangents && HasNaniteAssemblyDependencies(BaseLodIndex))
	{
		UE_LOG(LogInterchangePipeline, Log, TEXT("UInterchangeGenericMeshPipeline: Disabling unnecessary tangent recomputation on Nanite skeletalmesh %s"), *DisplayLabel);
...
void UInterchangeGenericMeshPipeline::AddLodDataToSkeletalMesh(const UInterchangeSkeletonFactoryNode* SkeletonFactoryNode, UInterchangeSkeletalMeshFactoryNode* SkeletalMeshFactoryNode, const TMap<int32, TArray<FString>>& NodeUidsPerLodIndex)
{
	...
	for (int32 LodIndex = 0; LodIndex < LodCount; ++LodIndex)
	{
		...
		{
			//Unless ForceAllMeshAsType is set to SkeletalMesh (UE-317102)
			//In this case we need to keep nested static meshes since they might be the only meshes used to generate this skeletal mesh.

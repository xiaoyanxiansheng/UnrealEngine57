...
UInterchangeGenericAssetsPipeline::UInterchangeGenericAssetsPipeline()
{
	...
	GroomPipeline = CreateDefaultSubobject<UInterchangeGenericGroomPipeline>("GroomPipeline");
...
void UInterchangeGenericAssetsPipeline::PreDialogCleanup(const FName PipelineStackName)
{
	...
	if (GroomPipeline)
	{
		GroomPipeline->PreDialogCleanup(PipelineStackName);
	}
...
bool UInterchangeGenericAssetsPipeline::IsSettingsAreValid(TOptional<FText>& OutInvalidReason) const
{
	...
	if (GroomPipeline && !GroomPipeline->IsSettingsAreValid(OutInvalidReason))
	{
		return false;
	}
...
void UInterchangeGenericAssetsPipeline::AdjustSettingsForContext(const FInterchangePipelineContextParams& ContextParams)
{
	...
	if (GroomPipeline)
	{
		GroomPipeline->AdjustSettingsForContext(ContextParams);
	}
...
void UInterchangeGenericAssetsPipeline::FilterPropertiesFromTranslatedData(UInterchangeBaseNodeContainer* InBaseNodeContainer)
{
	...
	if (GroomPipeline)
	{
		GroomPipeline->FilterPropertiesFromTranslatedData(InBaseNodeContainer);
	}
}
...
bool UInterchangeGenericAssetsPipeline::IsPropertyChangeNeedRefresh(const FPropertyChangedEvent& PropertyChangedEvent) const
{
	...
	if ((CommonMeshesProperties && CommonMeshesProperties->IsPropertyChangeNeedRefresh(PropertyChangedEvent))
		...
		|| (GroomPipeline && GroomPipeline->IsPropertyChangeNeedRefresh(PropertyChangedEvent)))
	{
		return true;
	}
...
void UInterchangeGenericAssetsPipeline::GetSupportAssetClasses(TArray<UClass*>& PipelineSupportAssetClasses) const
{
	...
	if (GroomPipeline)
	{
		GroomPipeline->GetSupportAssetClasses(PipelineSupportAssetClasses);
	}
	if (MaterialPipeline)
	{
...
bool UInterchangeGenericAssetsPipeline::GetPropertyPossibleValues(const FName PropertyPath, TArray<FString>& PossibleValues)
{
	...
	if (GroomPipeline && PropertyPathString.StartsWith(UInterchangeGenericGroomPipeline::StaticClass()->GetPathName()))
	{
		if (GroomPipeline->GetPropertyPossibleValues(PropertyPath, PossibleValues))
		{
			return true;
		}
	}
...
TArray<FInterchangeConflictInfo> UInterchangeGenericAssetsPipeline::GetConflictInfos(UObject* ReimportObject, UInterchangeBaseNodeContainer* InBaseNodeContainer, UInterchangeSourceData* SourceData)
{
    ...
    TransientBaseNodeContainer->SetChildrenCache(InBaseNodeContainer->GetChildrenCache());
...
void UInterchangeGenericAssetsPipeline::ExecutePipeline(UInterchangeBaseNodeContainer* InBaseNodeContainer, const TArray<UInterchangeSourceData*>& InSourceDatas, const FString& ContentBasePath)
{
	...
	ensure(Results);
	{
		...
		if (GroomPipeline)
		{
			GroomPipeline->SetResultsContainer(Results);
		}
		if (AnimationPipeline)
		{
	...
	if (GroomPipeline)
	{
		GroomPipeline->ScriptedExecutePipeline(InBaseNodeContainer, InSourceDatas, ContentBasePath);
	}
	if (AnimationPipeline)
	{
	...
	{
		// Update SceneNameFolderPrefix only if it is not a reimport otherwise reimport strategy will be impacted
		// when reimporting with new file which usually has a different name
		if (CacheContextParam.ContextType != EInterchangePipelineContext::AssetReimport && CacheContextParam.ContextType != EInterchangePipelineContext::SceneReimport)
		{
			SceneNameFolderPrefix = FPaths::GetBaseFilename(InSourceDatas[0]->GetFilename());
...
void UInterchangeGenericAssetsPipeline::ExecutePostFactoryPipeline(const UInterchangeBaseNodeContainer* InBaseNodeContainer, const FString& NodeKey, UObject* CreatedAsset, bool bIsAReimport)
{
	...
	if (GroomPipeline)
	{
		GroomPipeline->ScriptedExecutePostFactoryPipeline(InBaseNodeContainer, NodeKey, CreatedAsset, bIsAReimport);
	}
	if (AnimationPipeline)
	{
...
void UInterchangeGenericAssetsPipeline::ExecutePostImportPipeline(const UInterchangeBaseNodeContainer* InBaseNodeContainer, const FString& NodeKey, UObject* CreatedAsset, bool bIsAReimport)
{
	...
	if (GroomPipeline)
	{
		GroomPipeline->ScriptedExecutePostImportPipeline(InBaseNodeContainer, NodeKey, CreatedAsset, bIsAReimport);
	}
	if (AnimationPipeline)
	{
...
namespace UE::InterchangeGenericAssetsPipeline::Private
{
	template<typename T>
	TOptional<FString> GetValueAsString(const UInterchangeBaseNode* Node, const FString& AttributeName)
	{
		TOptional<FString> PayloadKey;
		T Value{};
		if (UInterchangeUserDefinedAttributesAPI::GetUserDefinedAttribute(Node, AttributeName, Value, PayloadKey))
		{
			return UE::Interchange::AttributeValueToString(Value);
		}
		return {};
	}
}
...
void UInterchangeGenericAssetsPipeline::AddMetaData(UObject* CreatedAsset, const UInterchangeBaseNode* Node)
{
...
#endif
	{
		...
		for (const FInterchangeUserDefinedAttributeInfo& UserAttributeInfo : UserAttributeInfos)
		{
			...
			{
				using namespace UE::InterchangeGenericAssetsPipeline::Private;

				case UE::Interchange::EAttributeTypes::Bool:
					MetaDataValue = GetValueAsString<bool>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Color:
					MetaDataValue = GetValueAsString<FColor>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::DateTime:
					MetaDataValue = GetValueAsString<FDateTime>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Double:
					MetaDataValue = GetValueAsString<double>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Enum:
					MetaDataValue = GetValueAsString<uint8>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Float:
					MetaDataValue = GetValueAsString<float>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Guid:
					MetaDataValue = GetValueAsString<FGuid>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Int8:
					MetaDataValue = GetValueAsString<int8>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Int16:
					MetaDataValue = GetValueAsString<int16>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Int32:
					MetaDataValue = GetValueAsString<int32>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Int64:
					MetaDataValue = GetValueAsString<int64>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::IntRect:
					MetaDataValue = GetValueAsString<FIntRect>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::LinearColor:
					MetaDataValue = GetValueAsString<FLinearColor>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Name:
					MetaDataValue = GetValueAsString<FName>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::RandomStream:
					MetaDataValue = GetValueAsString<FRandomStream>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::String:
					MetaDataValue = GetValueAsString<FString>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Timespan:
					MetaDataValue = GetValueAsString<FTimespan>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::TwoVectors:
					MetaDataValue = GetValueAsString<FTwoVectors>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::UInt8:
					MetaDataValue = GetValueAsString<uint8>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::UInt16:
					MetaDataValue = GetValueAsString<uint16>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::UInt32:
					MetaDataValue = GetValueAsString<uint32>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::UInt64:
					MetaDataValue = GetValueAsString<uint64>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Vector2d:
					MetaDataValue = GetValueAsString<FVector2d>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::IntPoint:
					MetaDataValue = GetValueAsString<FIntPoint>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::IntVector:
					MetaDataValue = GetValueAsString<FIntVector>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Vector2DHalf:
					MetaDataValue = GetValueAsString<FVector2DHalf>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Float16:
					MetaDataValue = GetValueAsString<FFloat16>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::OrientedBox:
					MetaDataValue = GetValueAsString<FOrientedBox>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::FrameNumber:
					MetaDataValue = GetValueAsString<FFrameNumber>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::FrameRate:
					MetaDataValue = GetValueAsString<FFrameRate>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::FrameTime:
					MetaDataValue = GetValueAsString<FFrameTime>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::SoftObjectPath:
					MetaDataValue = GetValueAsString<FSoftObjectPath>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Matrix44f:
					MetaDataValue = GetValueAsString<FMatrix44f>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Matrix44d:
					MetaDataValue = GetValueAsString<FMatrix44d>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Plane4f:
					MetaDataValue = GetValueAsString<FPlane4f>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Plane4d:
					MetaDataValue = GetValueAsString<FPlane4d>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Quat4f:
					MetaDataValue = GetValueAsString<FQuat4f>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Quat4d:
					MetaDataValue = GetValueAsString<FQuat4d>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Rotator3f:
					MetaDataValue = GetValueAsString<FRotator3f>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Rotator3d:
					MetaDataValue = GetValueAsString<FRotator3d>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Transform3f:
					MetaDataValue = GetValueAsString<FTransform3f>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Transform3d:
					MetaDataValue = GetValueAsString<FTransform3d>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Vector3f:
					MetaDataValue = GetValueAsString<FVector3f>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Vector3d:
					MetaDataValue = GetValueAsString<FVector3d>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Vector2f:
					MetaDataValue = GetValueAsString<FVector2f>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Vector4f:
					MetaDataValue = GetValueAsString<FVector4f>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Vector4d:
					MetaDataValue = GetValueAsString<FVector4d>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Box2f:
					MetaDataValue = GetValueAsString<FBox2f>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Box2D:
					MetaDataValue = GetValueAsString<FBox2D>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Box3f:
					MetaDataValue = GetValueAsString<FBox3f>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Box3d:
					MetaDataValue = GetValueAsString<FBox3d>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::BoxSphereBounds3f:
					MetaDataValue = GetValueAsString<FBoxSphereBounds3f>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::BoxSphereBounds3d:
					MetaDataValue = GetValueAsString<FBoxSphereBounds3d>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Sphere3f:
					MetaDataValue = GetValueAsString<FSphere3f>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Sphere3d:
					MetaDataValue = GetValueAsString<FSphere3d>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::BoolArray:
					MetaDataValue = GetValueAsString<TArray<bool>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::ColorArray:
					MetaDataValue = GetValueAsString<TArray<FColor>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::DateTimeArray:
					MetaDataValue = GetValueAsString<TArray<FDateTime>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::DoubleArray:
					MetaDataValue = GetValueAsString<TArray<double>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::EnumArray:
					MetaDataValue = GetValueAsString<TArray<uint8>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::FloatArray:
					MetaDataValue = GetValueAsString<TArray<float>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::GuidArray:
					MetaDataValue = GetValueAsString<TArray<FGuid>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Int8Array:
					MetaDataValue = GetValueAsString<TArray<uint8>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Int16Array:
					MetaDataValue = GetValueAsString<TArray<int16>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Int32Array:
					MetaDataValue = GetValueAsString<TArray<int32>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Int64Array:
					MetaDataValue = GetValueAsString<TArray<int64>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::IntRectArray:
					MetaDataValue = GetValueAsString<TArray<FIntRect>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::LinearColorArray:
					MetaDataValue = GetValueAsString<TArray<FLinearColor>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::NameArray:
					MetaDataValue = GetValueAsString<TArray<FName>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::RandomStreamArray:
					MetaDataValue = GetValueAsString<TArray<FRandomStream>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::StringArray:
					MetaDataValue = GetValueAsString<TArray<FString>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::TimespanArray:
					MetaDataValue = GetValueAsString<TArray<FTimespan>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::TwoVectorsArray:
					MetaDataValue = GetValueAsString<TArray<FTwoVectors>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::ByteArray:
					MetaDataValue = GetValueAsString<TArray<uint8>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::ByteArray64:
					MetaDataValue = GetValueAsString<TArray64<uint8>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::UInt16Array:
					MetaDataValue = GetValueAsString<TArray<uint16>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::UInt32Array:
					MetaDataValue = GetValueAsString<TArray<uint32>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::UInt64Array:
					MetaDataValue = GetValueAsString<TArray<uint64>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Vector2dArray:
					MetaDataValue = GetValueAsString<TArray<FVector2d>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::IntPointArray:
					MetaDataValue = GetValueAsString<TArray<FIntPoint>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::IntVectorArray:
					MetaDataValue = GetValueAsString<TArray<FIntVector>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Vector2DHalfArray:
					MetaDataValue = GetValueAsString<TArray<FVector2DHalf>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Float16Array:
					MetaDataValue = GetValueAsString<TArray<FFloat16>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::OrientedBoxArray:
					MetaDataValue = GetValueAsString<TArray<FOrientedBox>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::FrameNumberArray:
					MetaDataValue = GetValueAsString<TArray<FFrameNumber>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::FrameRateArray:
					MetaDataValue = GetValueAsString<TArray<FFrameRate>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::FrameTimeArray:
					MetaDataValue = GetValueAsString<TArray<FFrameTime>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::SoftObjectPathArray:
					MetaDataValue = GetValueAsString<TArray<FSoftObjectPath>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Matrix44fArray:
					MetaDataValue = GetValueAsString<TArray<FMatrix44f>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Matrix44dArray:
					MetaDataValue = GetValueAsString<TArray<FMatrix44d>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Plane4fArray:
					MetaDataValue = GetValueAsString<TArray<FPlane4f>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Plane4dArray:
					MetaDataValue = GetValueAsString<TArray<FPlane4d>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Quat4fArray:
					MetaDataValue = GetValueAsString<TArray<FQuat4f>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Quat4dArray:
					MetaDataValue = GetValueAsString<TArray<FQuat4d>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Rotator3fArray:
					MetaDataValue = GetValueAsString<TArray<FRotator3f>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Rotator3dArray:
					MetaDataValue = GetValueAsString<TArray<FRotator3d>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Transform3fArray:
					MetaDataValue = GetValueAsString<TArray<FTransform3f>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Transform3dArray:
					MetaDataValue = GetValueAsString<TArray<FTransform3d>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Vector3fArray:
					MetaDataValue = GetValueAsString<TArray<FVector3f>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Vector3dArray:
					MetaDataValue = GetValueAsString<TArray<FVector3d>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Vector2fArray:
					MetaDataValue = GetValueAsString<TArray<FVector2f>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Vector4fArray:
					MetaDataValue = GetValueAsString<TArray<FVector4f>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Vector4dArray:
					MetaDataValue = GetValueAsString<TArray<FVector4d>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Box2fArray:
					MetaDataValue = GetValueAsString<TArray<FBox2f>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Box2DArray:
					MetaDataValue = GetValueAsString<TArray<FBox2D>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Box3fArray:
					MetaDataValue = GetValueAsString<TArray<FBox3f>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Box3dArray:
					MetaDataValue = GetValueAsString<TArray<FBox3d>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::BoxSphereBounds3fArray:
					MetaDataValue = GetValueAsString<TArray<FBoxSphereBounds3f>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::BoxSphereBounds3dArray:
					MetaDataValue = GetValueAsString<TArray<FBoxSphereBounds3d>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Sphere3fArray:
					MetaDataValue = GetValueAsString<TArray<FSphere3f>>(Node, UserAttributeInfo.Name);
					break;
				case UE::Interchange::EAttributeTypes::Sphere3dArray:
					MetaDataValue = GetValueAsString<TArray<FSphere3d>>(Node, UserAttributeInfo.Name);
					break;
				default:
...
void UInterchangeGenericAssetsPipeline::SetReimportSourceIndex(UClass* ReimportObjectClass, const int32 SourceFileIndex)
{
	...
	if (GroomPipeline)
	{
		GroomPipeline->ScriptedSetReimportSourceIndex(ReimportObjectClass, SourceFileIndex);
	}
...
void UInterchangeGenericAssetsPipeline::ImplementUseSourceNameForAssetOption(UInterchangeBaseNodeContainer* InBaseNodeContainer, const TArray<UInterchangeSourceData*>& InSourceDatas)
{
	...
	AnimationPipeline->ImplementUseSourceNameForAssetOptionAnimSequence(bUseSourceNameForAsset, OverrideAssetName);
...
{
	bShowSectionFlag[(uint8)EInterchangeSkeletonCompareSection::Skeleton] = true;
	bShowSectionFlag[(uint8)EInterchangeSkeletonCompareSection::References] = true;
...
{
	bShowSectionFlag[(uint8)SectionIndex] = !bShowSectionFlag[(uint8)SectionIndex];
	return FReply::Handled();
}
...
{
	return bShowSectionFlag[(uint8)SectionIndex] ? EVisibility::Visible : EVisibility::Collapsed;
}
...
{
	return bShowSectionFlag[(uint8)SectionIndex] ? FAppStyle::GetBrush("Symbols.DownArrow") : FAppStyle::GetBrush("Symbols.RightArrow");
}

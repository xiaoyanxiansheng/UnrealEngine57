...
static bool GInterchangeEnableOBJLevelImport = false;
static FAutoConsoleVariableRef CCvarInterchangeEnableOBJLevelImport(
	TEXT("Interchange.FeatureFlags.Import.OBJ.ToLevel"),
	GInterchangeEnableOBJLevelImport,
	TEXT("Whether support for OBJ level import is enabled."),
	ECVF_Default);
...
FMeshDescription FObjData::MakeMeshDescriptionForGroup(const FString& GroupName, const FTransform& MeshGlobalTransform) const
{
	...
	// Fill in the object group name onto all the triangles of the mesh.
	TPolygonAttributesRef<FName> PolygonObjectNames = Attributes.GetPolygonObjectNames();
	const FName ObjectName(GroupName);
	for (const FPolygonID PolygonID: MeshDescription.Polygons().GetElementIDs())
	{
		PolygonObjectNames.Set(PolygonID, ObjectName);
	}
...
namespace ObjTranslatorUtils
{
	...
	static FString MakeSceneNodeUid(const FString& Name)
	{
		return TEXT("\\Scene\\") + (Name.IsEmpty() ? FString(TEXT("Null")) : Name);
	}

	static FString MakeActorNodeUid(const FString& Name)
	{
		return TEXT("\\Actor\\") + (Name.IsEmpty() ? FString(TEXT("Null")) : Name);
	}
	 ...
	 */
	static FString MakeMaterialInstanceNodeName(const FString& Name)
	{
	...
	static const UInterchangeTexture2DNode* CreateTexture2DNode(UInterchangeBaseNodeContainer& BaseNodeContainer, const FString& TexturePath, FString& InOutTextureName) 
	{
	...
	static const UInterchangeTexture2DNode* CreateTexture2DNode(UInterchangeBaseNodeContainer& BaseNodeContainer, const FObjData::FTextureParameter& TextureParameter)
	{
		const UInterchangeTexture2DNode* TextureNode = nullptr;
		if (!TextureParameter.IsEmpty())
		{
			FString TextureName = MakeTextureNodeName(FPaths::GetBaseFilename(TextureParameter.Path));
			TextureNode = CreateTexture2DNode(BaseNodeContainer, TextureParameter.Path, TextureName);
		}
		return TextureNode;
	}

	// multiply texture by color, handling cases when any of the arguments is missing
	static bool AddTexturedColoredInput(UInterchangeBaseNodeContainer& BaseNodeContainer, UInterchangeMaterialInstanceNode* MaterialInstanceNode, const FString& InputName, const FVector3f& Color, const FObjData::FTextureParameter& TexturePath)
	{
		if (const UInterchangeTexture2DNode* TextureNode = CreateTexture2DNode(BaseNodeContainer, TexturePath))
		{
			const FString InputNameLabel = InputName + TEXT("Map");
			return MaterialInstanceNode->AddTextureParameterValue(InputNameLabel, TextureNode->GetUniqueID());
		}
		else if (IsColorInitialized(Color))
		{
			return MaterialInstanceNode->AddVectorParameterValue(InputName, Color);
		}
	...
	static bool AddTexturedWeightedInput(UInterchangeBaseNodeContainer& BaseNodeContainer, UInterchangeMaterialInstanceNode* MaterialInstanceNode, const FString& InputName, const float& Weight, const FObjData::FTextureParameter& TexturePath) 
	{
		bool bResult = false;
		if (const UInterchangeTexture2DNode* TextureNode = CreateTexture2DNode(BaseNodeContainer, TexturePath))
		{
			const FString InputNameLabel = InputName + TEXT("MapWeight");
			bResult |= MaterialInstanceNode->AddTextureParameterValue(InputNameLabel, TextureNode->GetUniqueID());

			const float WeightValue = IsScalarInitialized(Weight) ? Weight : 1.0f;
            bResult |= MaterialInstanceNode->AddScalarParameterValue(InputName, Weight);
		}
	...
	static bool HandlePbrAttributes(UInterchangeBaseNodeContainer& BaseNodeContainer, const FObjData::FMaterialData& MaterialData, UInterchangeMaterialInstanceNode* MaterialInstanceNode)
	{
		bool bPbr = !MaterialData.RoughnessTexture.IsEmpty() || IsScalarInitialized(MaterialData.Roughness)
				 ...
				 || IsScalarInitialized(MaterialData.ClearCoatThickness)
			     || IsScalarInitialized(MaterialData.Anisotropy);
		...
		AddTexturedColoredInput(BaseNodeContainer, MaterialInstanceNode, Materials::PBRMR::Parameters::BaseColor.ToString(), MaterialData.DiffuseColor, MaterialData.DiffuseTexture);
		AddTexturedWeightedInput(BaseNodeContainer, MaterialInstanceNode, Materials::PBRMR::Parameters::Metallic.ToString(), MaterialData.Metallic, MaterialData.MetallicTexture);

		const float SpecularValue = (MaterialData.SpecularColor.X + MaterialData.SpecularColor.Y + MaterialData.SpecularColor.Z) / 3;
		AddTexturedWeightedInput(BaseNodeContainer, MaterialInstanceNode, Materials::PBRMR::Parameters::Specular.ToString(), SpecularValue, MaterialData.SpecularTexture);

		// Roughness (Glossiness, Shininess, Specular Power/Exponent in non-PBR terms)
		AddTexturedWeightedInput(BaseNodeContainer, MaterialInstanceNode, Materials::PBRMR::Parameters::Roughness.ToString(), MaterialData.Roughness, MaterialData.RoughnessTexture);

		if (IsScalarInitialized(MaterialData.ClearCoatThickness) && !FMath::IsNearlyZero(MaterialData.ClearCoatThickness))
		{
			MaterialInstanceNode->AddScalarParameterValue(Materials::ClearCoat::Parameters::ClearCoat.ToString(), MaterialData.ClearCoatThickness);

			if (IsScalarInitialized(MaterialData.ClearCoatRoughness))
			{
				MaterialInstanceNode->AddScalarParameterValue(Materials::ClearCoat::Parameters::ClearCoatRoughness.ToString(), MaterialData.ClearCoatRoughness);
			}
		}

		if (IsScalarInitialized(MaterialData.Anisotropy)  && !FMath::IsNearlyZero(MaterialData.Anisotropy))
		{
			MaterialInstanceNode->AddScalarParameterValue(Materials::PBRMR::Parameters::Anisotropy.ToString(), MaterialData.Anisotropy);
		}
...
EInterchangeTranslatorType UInterchangeOBJTranslator::GetTranslatorType() const
{
	return GInterchangeEnableOBJLevelImport ? EInterchangeTranslatorType::Scenes : EInterchangeTranslatorType::Assets;
}
...
bool UInterchangeOBJTranslator::Translate(UInterchangeBaseNodeContainer& BaseNodeContainer) const
{
	...
	if (bSuccess)
	{
		...
		const FString SceneName = FPaths::GetBaseFilename(Filename);
		const FString SceneNodeUid = MakeSceneNodeUid(SceneName);
		BaseNodeContainer.SetupNode(SceneNode, SceneNodeUid, SceneName, EInterchangeNodeContainerType::TranslatedScene);
		...
		for (const TPair<FString, FObjData::FGroupData>& Group : ObjDataPtr->Groups)
		{
			...
			const FString ActorNodeUid = MakeActorNodeUid(GroupName);
			const FString MeshNodeUid = MakeMeshNodeUid(GroupName);

			UInterchangeSceneNode* InterchangeSceneNode = NewObject<UInterchangeSceneNode>(&BaseNodeContainer);
			BaseNodeContainer.SetupNode(InterchangeSceneNode, ActorNodeUid, GroupName, EInterchangeNodeContainerType::TranslatedScene, SceneNodeUid);
			...
			BaseNodeContainer.SetupNode(MeshNode, MeshNodeUid, GroupName, EInterchangeNodeContainerType::TranslatedAsset);
			...
			for (const FObjData::FFaceGroupData& FaceGroup : Group.Value.FaceGroups)
			{
				...
				{					
					const FString MaterialDependencyUid = UInterchangeMaterialInstanceNode::MakeNodeUid(MakeMaterialInstanceNodeName(FaceGroup.MaterialName), TEXT(""));
		...
		for (const TPair<FString, FObjData::FMaterialData>& Material : ObjDataPtr->Materials)
		{
			...
			FString NodeUid = UInterchangeMaterialInstanceNode::MakeNodeUid(MakeMaterialInstanceNodeName(MaterialName), TEXT(""));

			UInterchangeMaterialInstanceNode* MaterialInstanceNode = NewObject<UInterchangeMaterialInstanceNode>(&BaseNodeContainer);
			MaterialInstanceNode->SetCustomParent(TEXT("/InterchangeAssets/Materials/PBRSurfaceMaterial_MR.PBRSurfaceMaterial_MR"));
			BaseNodeContainer.SetupNode(MaterialInstanceNode, NodeUid, MaterialName, EInterchangeNodeContainerType::TranslatedAsset);
			...
			if (!HandlePbrAttributes(BaseNodeContainer, MaterialData, MaterialInstanceNode))
			{
				bPBR = false;
				...
				if(bUnlit)
				{
					MaterialInstanceNode->SetCustomParent(TEXT("/InterchangeAssets/Materials/UnlitMaterial.UnlitMaterial"));
					AddTexturedColoredInput(BaseNodeContainer, MaterialInstanceNode, Materials::Unlit::Parameters::UnlitColor.ToString(), MaterialData.DiffuseColor, MaterialData.DiffuseTexture);
				}
				else
				{
					MaterialInstanceNode->SetCustomParent(TEXT("/InterchangeAssets/Materials/PhongSurfaceMaterial.PhongSurfaceMaterial"));
					AddTexturedColoredInput(BaseNodeContainer, MaterialInstanceNode, Materials::Phong::Parameters::DiffuseColor.ToString(), (MaterialData.DiffuseColor.GetMin() < 0) ? FVector3f(0.8f, 0.8f, 0.8f) : MaterialData.DiffuseColor, MaterialData.DiffuseTexture);

					if (bSpecular)
					{
						AddTexturedColoredInput(BaseNodeContainer, MaterialInstanceNode, Materials::Phong::Parameters::SpecularColor.ToString(), MaterialData.SpecularColor, MaterialData.SpecularTexture);
						...
						if (const UInterchangeTexture2DNode* SpecularExponentTextureNode= CreateTexture2DNode(BaseNodeContainer, MaterialData.SpecularExponentTexture))
						{
							...
							MaterialInstanceNode->AddTextureParameterValue(ShininessParameterName + TEXT("Map"), SpecularExponentTextureNode->GetUniqueID());
							MaterialInstanceNode->AddScalarParameterValue(ShininessParameterName, SpecularExponentScale);
						}
						else if (IsScalarInitialized(MaterialData.SpecularExponent))
						{
							MaterialInstanceNode->AddScalarParameterValue(ShininessParameterName, MaterialData.SpecularExponent);
						}
					}
					...
					if (!bReflectionMetal)
					{
						...
						AddTexturedColoredInput(BaseNodeContainer, MaterialInstanceNode, AmbientColorPropertyName, AmbientColor, MaterialData.AmbientTexture);
					}
				}
			}
			...
			// Opacity
			{
                ...
                if (const UInterchangeTexture2DNode* TransparencyTextureNode = CreateTexture2DNode(BaseNodeContainer, MaterialData.TransparencyTexture))
				{
					bOpacity = true;
                    MaterialInstanceNode->AddTextureParameterValue(OpacityInputName + TEXT("Map"), TransparencyTextureNode->GetUniqueID());
				}
				// Opacity texture is weighted by scalar opacity("d")
				else if (!AddTexturedWeightedInput(BaseNodeContainer, MaterialInstanceNode, OpacityInputName, MaterialData.Opacity, MaterialData.OpacityTexture))
				{
					...
					{
						bOpacity = true;
						MaterialInstanceNode->AddScalarParameterValue(OpacityInputName, OpacityScalar);
					}
				}
				...
				if (bOpacity)
				{
					if (bPBR)
					{
						MaterialInstanceNode->SetCustomParent(TEXT("/InterchangeAssets/Materials/ThinTranslucentMaterial_MR.ThinTranslucentMaterial_MR"));
					}
					else
					{
						MaterialInstanceNode->SetCustomParent(TEXT("/InterchangeAssets/Materials/PhongSurfaceTranslucentMaterial.PhongSurfaceTranslucentMaterial"));
					}
				}
			}
			...
			// Emissive
			AddTexturedColoredInput(BaseNodeContainer, MaterialInstanceNode, Materials::Common::Parameters::EmissiveColor.ToString(), MaterialData.EmissiveColor, MaterialData.EmissiveTexture);
			...
			if (IsScalarInitialized(MaterialData.IndexOfRefraction) || bRefraction)
			{
				...
				MaterialInstanceNode->AddScalarParameterValue(ParameterName, IndexOfRefraction);
			}
			...
			if ((IsColorInitialized(MaterialData.TransmissionFilter) && !(MaterialData.TransmissionFilter - FVector3f::One()).IsNearlyZero()) || bTransmission)
			{
				...
				MaterialInstanceNode->AddVectorParameterValue(ParameterName, TransmissionColor);
			}

			// Normal/bump
			if (const UInterchangeTexture2DNode* NormalmapTextureNode = CreateTexture2DNode(BaseNodeContainer, MaterialData.NormalmapTexture))
			{
				MaterialInstanceNode->AddTextureParameterValue(Materials::Common::Parameters::Normal.ToString() + TEXT("Map"), NormalmapTextureNode->GetUniqueID());
			}
			else if (!MaterialData.BumpmapTexture.IsEmpty())
			{
				...
				if (const UInterchangeTexture2DNode* BumpmapTextureNode = CreateTexture2DNode(BaseNodeContainer, TexturePath, TextureName))
				{
					MaterialInstanceNode->AddTextureParameterValue(Materials::Common::Parameters::Normal.ToString() + TEXT("Map"), BumpmapTextureNode->GetUniqueID());
					MaterialInstanceNode->AddScalarParameterValue(TEXT("NormalMapWeight"), MaterialData.BumpmapTexture.BumpMultiplier);

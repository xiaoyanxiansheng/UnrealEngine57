...
FMaterialXBase::FMaterialXBase(UInterchangeBaseNodeContainer& BaseNodeContainer)
	...
	, bIsSubstrateAdaptiveGBufferEnabled(FMaterialXManager::GetInstance().IsSubstrateAdaptiveGBufferEnabled())
{}
...
void FMaterialXBase::UpdateDocumentRecursively(MaterialX::GraphElementPtr Graph)
{
	...
	ConvertGeomPropValueNodesToTexCoord(Graph);
...
void FMaterialXBase::SetTranslator(const UInterchangeTranslatorBase* InTranslator)
{
...
void FMaterialXBase::ConvertGeomPropValueNodesToTexCoord(MaterialX::GraphElementPtr Graph)
{
	const std::vector<mx::NodePtr> Nodes = Graph->getChildrenOfType<mx::Node>(mx::Category::GeomPropValue);
	for (mx::NodePtr Node : Nodes)
	{
		if (Node->getType() != mx::Type::Vector2)
		{
			continue;
		}

		mx::InputPtr Input = Node->getInput("geomprop");
		if (!(Input && Input->getValueString() == "st"))
		{
			continue;
		}

		// the default UV set is 0 (input "index" value=0)
		Graph->changeChildCategory(Node, mx::Category::TexCoord);
	}
}
...
FLinearColor FMaterialXBase::GetLinearColor(MaterialX::InputPtr Input) const
{
	...
	{
		MTLX_LOG("MaterialXGetLinearColor", "<{0}>-<{1}>: Colorspace {2} is not supported yet, falling back to linear space",

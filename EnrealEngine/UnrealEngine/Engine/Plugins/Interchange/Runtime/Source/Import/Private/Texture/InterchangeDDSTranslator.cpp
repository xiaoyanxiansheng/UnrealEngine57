...
namespace UE::Interchange::Private::InterchangeDDSTranslator
{
	...
	TUniquePtr<UE::DDS::FDDSFile> CreateDDSFileReader(const FString& Filename, bool bHeaderOnly)
	{
		...
		const UE::DDS::EDDSReadMipMode MipReadMode = bHeaderOnly ? UE::DDS::EDDSReadMipMode::HeaderOnly : UE::DDS::EDDSReadMipMode::Full;
		TUniquePtr<UE::DDS::FDDSFile> DDS = TUniquePtr<UE::DDS::FDDSFile>(UE::DDS::FDDSFile::CreateFromDDSInMemory(DDSSourceData.GetData(), DDSSourceData.Num(), &Error, MipReadMode));
		if (!DDS.IsValid())
		{

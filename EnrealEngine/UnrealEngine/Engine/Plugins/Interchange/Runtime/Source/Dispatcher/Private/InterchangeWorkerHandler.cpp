...
namespace UE
{
	namespace Interchange
	{
			...
			, bShouldTerminate(false)
		{
			...
			const int32 UniqueID = (FPlatformTime::Cycles64() & 0x00000000EFFFFFFF);
		...
		void FInterchangeWorkerHandler::StartWorkerProcess()
		{
			...
			FString ProcessorPath = Dispatcher.GetWorkerApplicationPath();
				...
				switch (WorkerState)
				{
					...
					case EWorkerState::Idle:
					{
						...
						else
						{
							...
							// No task to execute, wait a little
							else if (WorkerState == EWorkerState::Idle)
							{
								FPlatformProcess::Sleep(Config::IdleLoopDelay);
							}
						}
						break;
					}
					...
					case EWorkerState::Processing:
					{
						...
						else
						{
							...
							// Task is being executed, sleep before rechecking
							else if (WorkerState == EWorkerState::Processing)
							{
								FPlatformProcess::Sleep(Config::ProcessingLoopDelay);
							}
						}
						break;
					}
		...
		{
			return !EnumHasAnyFlags(EWorkerState(WorkerState), EWorkerState::Terminated);
		}

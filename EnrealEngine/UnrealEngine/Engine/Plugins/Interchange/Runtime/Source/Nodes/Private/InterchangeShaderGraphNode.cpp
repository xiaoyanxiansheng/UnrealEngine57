...
bool UInterchangeShaderPortsAPI::DisconnectInput(UInterchangeBaseNode* InterchangeNode, const FString& InputName)
{
	return InterchangeNode->RemoveAttribute(MakeInputConnectionKey(InputName));
}

bool UInterchangeShaderPortsAPI::DisconnectInputFromOutputNode(UInterchangeBaseNode* InterchangeNode, const FString& InputName, FString& OutExpressionUid, FString& OutputName)
{
	if (GetInputConnection(InterchangeNode, InputName, OutExpressionUid, OutputName))
	{
		return DisconnectInput(InterchangeNode, InputName);
	}
	return false;
}

...
namespace UE
{
	namespace Interchange
	{
		...
		const FAttributeKey& FMeshNodeStaticData::GetAssemblyPartDependenciesKey()
		{
			static FAttributeKey Dependencies_BaseKey(TEXT("__AssemblyPartDependencies__"));
			return Dependencies_BaseKey;
		}
...
UInterchangeMeshNode::UInterchangeMeshNode()
{
	...
	AssemblyPartDependencies.Initialize(Attributes, UE::Interchange::FMeshNodeStaticData::GetAssemblyPartDependenciesKey().ToString());
}
...
void UInterchangeMeshNode::GetAssemblyPartDependencies(TArray<FString>& OutDependencies) const
{
	AssemblyPartDependencies.GetItems(OutDependencies);
}

void UInterchangeMeshNode::GetAssemblyPartDependency(const int32 Index, FString& OutDependency) const
{
	AssemblyPartDependencies.GetItem(Index, OutDependency);
}

bool UInterchangeMeshNode::AddAssemblyPartDependencyUid(const FString& DependencyUid)
{
	return AssemblyPartDependencies.AddItem(DependencyUid);
}

bool UInterchangeMeshNode::RemoveAssemblyPartDependencyUid(const FString& DependencyUid)
{
	return AssemblyPartDependencies.RemoveItem(DependencyUid);
}

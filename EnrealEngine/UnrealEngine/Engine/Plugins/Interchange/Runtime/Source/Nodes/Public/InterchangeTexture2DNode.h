...
public:
	...
	{
		IMPLEMENT_NODE_ATTRIBUTE_SETTER_NODELEGATE(ForceLongLatCubemap, bool);
	}

	UFUNCTION(BlueprintCallable, Category = "Interchange | Node | Texture")
	bool GetForceLongLatCubemap(bool& AttributeValue) const
	{
		IMPLEMENT_NODE_ATTRIBUTE_GETTER(ForceLongLatCubemap, bool);
	}

private:
	IMPLEMENT_NODE_ATTRIBUTE_KEY(WrapU);
	IMPLEMENT_NODE_ATTRIBUTE_KEY(WrapV);
	IMPLEMENT_NODE_ATTRIBUTE_KEY(ForceLongLatCubemap);
};

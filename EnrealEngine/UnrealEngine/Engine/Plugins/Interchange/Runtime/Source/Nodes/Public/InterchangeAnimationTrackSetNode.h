...
enum class EInterchangeAnimationPayLoadType : uint8
{
	...
	GEOMETRY_CACHE_TRANSFORM // Array of time-sampled local transforms used to build a geometry cache
};
...
{
	/** Keep the animated property at the state set at the end of the animation track. */
	KeepState,
	/** Restore the animated property to its state before the start of the animation track. */
	RestoreState,
	/** Restore the animated property to the state set in the project for that property. */
	ProjectDefault,
};

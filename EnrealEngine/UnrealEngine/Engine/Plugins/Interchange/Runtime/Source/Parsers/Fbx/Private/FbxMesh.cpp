...
bool FMeshDescriptionImporter::FillMeshDescriptionFromFbxMesh(FbxMesh* Mesh, const FTransform& MeshGlobalTransform, TArray<FString>& OutJointUniqueNames, EMeshType MeshType)
{
	...
	// We want to store the source mesh name with each triangle so that we can identify individual geometry pieces in the combined static/skeletal mesh.
	...
	// Grab the mesh node's name, we use that to fill the object name attribute. The mesh object name itself is of no use since it's the same
	// for all meshes.
	const FName MeshNodeName(Parser.GetFbxHelper()->GetFbxObjectName(MeshNode));
	...
	bool bHasNonDegeneratePolygons = false;
	{
...

		{
			...
			for (int32 PolygonIndex = 0; PolygonIndex < PolygonCount; PolygonIndex++)
			{
				...
				// Set the name of the object that generated this triangle.
				PolygonObjectNames.Set(MeshDescription->GetTrianglePolygon(NewTriangleID), MeshNodeName);
	...
	for (int32 GeometryIndex = 0; GeometryIndex < GeometryCount; ++GeometryIndex)
	{
		...
		for (int32 MorphTargetIndex = 0; MorphTargetIndex < MorphTargetCount; ++MorphTargetIndex)
		{
			...
			for (int32 ChannelIndex = 0; ChannelIndex < MorphTargetChannelCount; ++ChannelIndex)
			{
				...
				if (CurrentChannelMorphTargetCount == 0)
				{
					continue;
				}
...
bool AcquireBindPoseMatrix(FbxPose* CurrentPose, FbxAMatrix& GlobalBindPoseJointMatrix, FbxNode* Joint)
{
...
bool RetrievePoseFromBindPose(FbxNode* Joint, FbxAMatrix& GlobalBindPoseJointMatrix)
{
	if (!Joint)
	{
		return false;
	...
	FbxScene* SDKScene = Joint->GetScene();
	if (!SDKScene)
	{
		return false;
	}
	...
	bool& bBadBindPoseMessageDisplay, const bool bIsOriginalJoint, const FbxAMatrix& JointOrientationMatrix)
{
	...
	// TODO: Do we expect all Joints to have meshes assigned to them?
	if (!(Joint->GetMesh() && Joint->GetMesh()->GetDeformerCount(FbxDeformer::eSkin) > 0))
	{
		return false;
	}

	bool bRetrieveBindPoseResult = RetrievePoseFromBindPose(Joint, GlobalBindPoseJointMatrix);
	// get bind pose
	if (!bRetrieveBindPoseResult)
	{
		...
		bRetrieveBindPoseResult = RetrievePoseFromBindPose(Joint, GlobalBindPoseJointMatrix);
	}

	//Only joints should have the JointOrientationMatrix applied here.
	if (bRetrieveBindPoseResult && bIsOriginalJoint)
	{
...
FFbxJointMeshBindPoseGenerator::FFbxJointMeshBindPoseGenerator(FbxScene* SDKScene, FFbxParser& Parser)
{
	...
	for (int32 GeometryIndex = 0; GeometryIndex < GeometryCount; ++GeometryIndex)
	{
		...
		{
			FbxSkin* Skin = (FbxSkin*)Geometry->GetDeformer(GeometryDeformerIndex, FbxDeformer::eSkin);
			if (!ensure(Skin))
			{
				continue;
			}
			...
			for (int32 ClusterIndex = 0; ClusterIndex < ClusterCount; ClusterIndex++)
			{
				...
				// Here we try fetching the bind pose only from actual FbxPose objects. Interchange traditionally preferred reading the
				// TransformLink directly from the cluster, but it seems that in some edge cases (see UE-273136) that is not the correct thing to do?
				// By checking the pose first, we mirror the FbxLegacy behavior shown at UnFbx::FFbxImporter::ImportBones, with how it first 
				// checks the PoseArray when filling in GlobalsPerLink
				FbxAMatrix GlobalBindPoseJointMatrixPerGeo;
				bool bRetrieveBindPoseResult = RetrievePoseFromBindPose(Joint, GlobalBindPoseJointMatrixPerGeo);
				if (!bRetrieveBindPoseResult)
				{
					// if failed, delete bind pose, and retry.
					// This is likely not needed as FFbxScene::IsValidBindPose will already create this fallback pose, but just in case
					const int32 PoseCount = SDKScene->GetPoseCount();
					for (int32 PoseIndex = PoseCount - 1; PoseIndex >= 0; --PoseIndex)
					{
						FbxPose* CurrentPose = SDKScene->GetPose(PoseIndex);
					...
					bRetrieveBindPoseResult = RetrievePoseFromBindPose(Joint, GlobalBindPoseJointMatrixPerGeo);
				}
				...
				{
					JointToBindPoseMap.Add(Joint, GlobalBindPoseJointMatrixPerGeo * Parser.JointOrientationMatrix);
				}
				else
				{
					// Convert to FMatrix to check equality with tolerance. 
					const FMatrix ExistingMatrix = FFbxConvert::ConvertMatrix<FMatrix>(JointToBindPoseMap[Joint]);
					const FMatrix NewMatrix = FFbxConvert::ConvertMatrix<FMatrix>(GlobalBindPoseJointMatrixPerGeo * Parser.JointOrientationMatrix);
					ensureMsgf(NewMatrix.Equals(ExistingMatrix, KINDA_SMALL_NUMBER), TEXT("Expected to have same joint node in different clusters to have same Bind Pose matrix!"));
				}

...
namespace GLTF
{
	...
	private:
		...
		void BuildParentJointIndices() const;
		void BuildParentIndices(int32 ParentNodeIndex, int32 DirectParentJointIndex, int32 CurrentNodeIndex) const;
		int32 FindRootJointIndex(int32 CurrentNodeIndex) const;

		//Sets the Node Types to Joint for Nodes that were not marked Joint in Skins, however they have descendant and ancestor both in Skins.
		void FixNodeTypesForIndirectJoints() const;

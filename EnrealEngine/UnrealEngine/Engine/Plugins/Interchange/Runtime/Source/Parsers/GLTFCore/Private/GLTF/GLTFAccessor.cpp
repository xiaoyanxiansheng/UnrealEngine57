...
namespace GLTF
{
	FString ToString(const EMeshAttributeType& Type)
	{
		...
		case GLTF::TEXCOORD_2:
			return TEXT("TEXCOORD_2");
		case GLTF::TEXCOORD_3:
			return TEXT("TEXCOORD_3");
		case GLTF::TEXCOORD_4:
			return TEXT("TEXCOORD_4");
		case GLTF::TEXCOORD_5:
			return TEXT("TEXCOORD_5");
		case GLTF::TEXCOORD_6:
			return TEXT("TEXCOORD_6");
		case GLTF::TEXCOORD_7:
			return TEXT("TEXCOORD_7");
		case GLTF::COLOR_0:
			return TEXT("COLOR_0");
		case GLTF::JOINTS_0:
	...
	bool FAccessor::CheckAccessorTypeForDataType(EMeshAttributeType MeshAttributeType, bool bMorphTargetProperty) const
	{
		switch (MeshAttributeType)
		{
			...
			case GLTF::EMeshAttributeType::TEXCOORD_2:
			case GLTF::EMeshAttributeType::TEXCOORD_3:
			case GLTF::EMeshAttributeType::TEXCOORD_4:
			case GLTF::EMeshAttributeType::TEXCOORD_5:
			case GLTF::EMeshAttributeType::TEXCOORD_6:
			case GLTF::EMeshAttributeType::TEXCOORD_7:
	...
	uint32 FAccessor::GetUnsignedInt(uint32 Index) const
	{
		...
		if (Index < Count)
		{
			...
			{
				uint32 Buffer;
				if (!BufferView.IsValid() || AcquireUnsignedInt(Index, BufferView, ByteOffset, ByteStride, ComponentType, Buffer))
				{
	...
	void FAccessor::GetUnsignedInt16x4(uint32 Index, uint16 Values[4]) const
	{
		...
		if (Index < Count)
		{
			...
			{
				if (!BufferView.IsValid() || AcquireUnsignedInt16x4(Index, BufferView, ByteOffset, ByteStride, ComponentType, Values))
				{
	...
	float FAccessor::GetFloat(uint32 Index) const
	{
		...
		if (Index < Count)
		{
			...
			{
				float Buffer;
				if (!BufferView.IsValid() || AcquireFloat(Index, BufferView, ByteOffset, ByteStride, ComponentType, Buffer))
				{
	...
	FVector2D FAccessor::GetVec2(uint32 Index) const
	{
		...
		if (Index < Count)
		{
			...
			{
				FVector2D Buffer;
				if (!BufferView.IsValid() || AcquireVec2(Index, BufferView, ByteOffset, ByteStride, ComponentType, bNormalized, Buffer))
				{
	...
	FVector FAccessor::GetVec3(uint32 Index) const
	{
		...
		if (Index < Count)
		{
			...
			{
				FVector Buffer;
				if (!BufferView.IsValid() || AcquireVec3(Index, BufferView, ByteOffset, ByteStride, ComponentType, bNormalized, Buffer))
				{
	...
	FVector4 FAccessor::GetVec4(uint32 Index) const
	{
		...
		if (Index < Count)
		{
			...
			{
				FVector4 Buffer;
				if (!BufferView.IsValid() || AcquireVec4(Index, BufferView, ByteOffset, ByteStride, ComponentType, bNormalized, Buffer))
				{
	...
	FMatrix FAccessor::GetMat4(uint32 Index) const
	{
		...
		if (Index < Count)
		{
			...
			{
				FMatrix Buffer;
				if (!BufferView.IsValid() || AcquireMat4(Index, BufferView, ByteOffset, ByteStride, ComponentType, Buffer))
				{
	...
	void FAccessor::GetUnsignedIntArray(uint32* Buffer) const
	{
		if (Type == EType::Scalar && !bNormalized)
		{
			if (!BufferView.IsValid() || FillUnsignedIntArray(BufferView, ByteOffset, ComponentType, Count, Buffer))
			{
	...
	void FAccessor::GetFloatArray(float* Buffer) const
	{
		...
		{
			if (!BufferView.IsValid() || FillFloatArray(bNormalized, BufferView, ByteOffset, ComponentType, ByteStride, Count, Buffer))
			{

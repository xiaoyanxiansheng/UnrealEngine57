...
namespace GLTF
{
	...
	{
		const FName MeshName(Mesh.Name);
		const int32 NumUVs = FMath::Max(1, GetNumUVs(Mesh));

		FStaticMeshAttributes StaticMeshAttributes(*MeshDescription);
		StaticMeshAttributes.Register();
		StaticMeshAttributes.RegisterPolygonObjectNameAttribute();
		...
		TPolygonAttributesRef<FName> PolygonObjectNames = StaticMeshAttributes.GetPolygonObjectNames();
		...
		for (int32 Index = 0; Index < Mesh.Primitives.Num(); ++Index)
		{
			...
			const bool        bHasDegenerateTriangles =
				ImportPrimitive(Primitive, MeshName, Index, NumUVs, Mesh.HasTangents(), Mesh.HasColors(),  //
					VertexInstanceNormals, VertexInstanceTangents, VertexInstanceBinormalSigns, VertexInstanceUVs,
					VertexInstanceColors,  //
					EdgeHardnesses, PolygonObjectNames, MeshGlobalTransform, MeshDescription,
					MorphTargetWeights);
		...
		const TArray<float>& MorphTargetWeights)
	{
		...
		else
		{
			for (uint32 TriangleIndex = 0; TriangleIndex < TriCount; ++TriangleIndex)
			{
				...
				PolygonObjectNames.Set(NewPolygonID, MeshName);
			}
		}

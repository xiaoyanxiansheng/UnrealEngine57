...
namespace GLTF
{
	FAttributeAccessors::FAttributeAccessors(const TArray<TPair<EMeshAttributeType, FAccessor&>>& InAttributeAccessors, bool bInMorphTarget)
	{
		
		for (uint8 AttributeTypeCounter = 0; AttributeTypeCounter < EMeshAttributeType::COUNT; AttributeTypeCounter++)
		{
			EMeshAttributeType AttributeType = EMeshAttributeType(AttributeTypeCounter);

			if (InAttributeAccessors.Num() <= AttributeTypeCounter)
			{
				break;
			}

			if (InAttributeAccessors[AttributeType].Key == AttributeType)
			{
				AttributeAccessors.Emplace(AttributeType, InAttributeAccessors[AttributeType].Value);
			}
			else
			{
				//Expectation mismatch, update InAttributeAccessors array based on EMeshAttributeType.
				ensure(false);

				static FAccessor EmptyAccessor;

				AttributeAccessors.Emplace(AttributeType, EmptyAccessor);
			}
		}
	...
	FMorphTarget::FMorphTarget(const TArray<TPair<EMeshAttributeType, FAccessor&>>& InAttributeAccessors)
		: FAttributeAccessors(InAttributeAccessors, true)
	{
	}
	...
	{
		if (Index < SupportedTexCoordNum)
		{
			const uint32 TexcoordAttributeIndex = EMeshAttributeType::TEXCOORD_0 + Index;
			return HasAttributeAccessor(EMeshAttributeType(TexcoordAttributeIndex));
		}
	...
	void FMorphTarget::GetTexCoordDisplacements(uint32 Index, TArray<FVector2f>& Buffer) const
	{
		...
		{
			const uint32 TexcoordAttributeIndex = EMeshAttributeType::TEXCOORD_0 + Index;
			return GetAttributeAccessor(EMeshAttributeType(TexcoordAttributeIndex)).GetVec2Array(Buffer);
		}
	}
	int32 FMorphTarget::GetNumberOfTexCoordDisplacements(uint32 Index) const
	{
		...
		{
			const uint32 TexcoordAttributeIndex = EMeshAttributeType::TEXCOORD_0 + Index;
			return GetAttributeAccessor(EMeshAttributeType(TexcoordAttributeIndex)).Count;
		}
	...
	FPrimitive::FPrimitive(EMode InMode, int32 InMaterial, const FAccessor& InIndices, const TArray<TPair<EMeshAttributeType, FAccessor&>>& InAttributeAccessors)
	    : FAttributeAccessors(InAttributeAccessors, false)
		, Mode(InMode)

...
#include UE_INLINE_GENERATED_CPP_BY_NAME(EvaluateProxyNode)
...
void UK2Node_EvaluateProxy2::AllocateDefaultPins()
{
	...
	bool bResultIsClass = false;
   	if (Proxy && Proxy->Type)
   	{
   		...
   		// For class results, in Array mode, we can't easily do the CastToClass - we have to just return an Array of Objects
   		bResultIsClass = Proxy->ResultType == EObjectChooserResultType::ClassResult && Mode != EEvaluateProxyMode::AllResults;
   	}
	...
	if (ExecInput->HasAnyConnections())
	{
		...
		if (Proxy) 
		{
			...
			if (UEdGraphPin* ResultIsClassPin = CallFunction->FindPin(TEXT("bResultIsClass")))
			{
				...
				// For class results, in Array mode, we can't easily do the CastToClass - we have to just return an Array of Objects
				if (Proxy->ResultType == EObjectChooserResultType::ClassResult && Mode != EEvaluateProxyMode::AllResults)
				{
					// if it's a class we need to add a CastToClass function to cast it from an Object pointer to a Class pointer

...
UAnimationAsset* FAnimNode_ChooserPlayer::ChooseAsset(const FAnimationUpdateContext& Context)
{
	...
	if (Chooser.IsValid())
	{
		...
		Settings.PlayingAsset = GetAnimAsset();
		Settings.PlayingAssetAccumulatedTime = GetAccumulatedTime();
		Settings.bIsPlayingAssetMirrored = GetMirror();
		Settings.PlayingAssetBlendParameters = GetBlendParameters();
		Settings.AnimationUpdateContext = &Context;
...
void FAnimNode_ChooserPlayer::UpdateAssetPlayer(const FAnimationUpdateContext& Context)
{
	...
	if (Settings.bForceBlendTo || bJustBecameRelevant || NewAsset != CurrentAsset || AnimPlayers.IsEmpty() ||

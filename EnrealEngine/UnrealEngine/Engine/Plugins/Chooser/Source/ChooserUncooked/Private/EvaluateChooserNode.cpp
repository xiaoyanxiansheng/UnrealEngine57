...
#include UE_INLINE_GENERATED_CPP_BY_NAME(EvaluateChooserNode)
	...
	if (Chooser)
	{
		...
		else
		{
			...
			if (Chooser->OutputObjectType)
			{
				...
				bResultIsClass = Chooser->ResultType == EObjectChooserResultType::ClassResult && (Mode != EEvaluateChooserMode::AllResults);
			}
	...
	if (ExecInput->HasAnyConnections())
	{
		...
		if (Chooser) 
		{
			...
			if (UEdGraphPin* ResultIsClassPin = CallFunction->FindPin(TEXT("bResultIsClass")))
			{
				...
				// For class results, in Array mode, we can't easily do the CastToClass - we have to just return an Array of Objects
				if (Chooser->ResultType == EObjectChooserResultType::ClassResult && Mode != EEvaluateChooserMode::AllResults)
				{

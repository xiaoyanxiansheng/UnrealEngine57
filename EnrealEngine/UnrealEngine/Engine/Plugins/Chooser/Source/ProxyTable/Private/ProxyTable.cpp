...
#include UE_INLINE_GENERATED_CPP_BY_NAME(ProxyTable)
...
void UProxyTable::PostLoad()
{
	...
	for (FRuntimeProxyValue& Entry : RuntimeValues)
	{
		...
		{
			UE_LOG(LogChooser, Display, TEXT("ProxyTable has an entry with no ProxyAsset assigned (%s)"), *GetName());
		}
...
FObjectChooserBase::EIteratorStatus UProxyTable::IterateProxyObjects(const FGuid& Key, FChooserEvaluationContext& Context, FObjectChooserBase::FObjectChooserIteratorCallback Callback) const
{
	const int FoundIndex = Algo::BinarySearch(Keys, Key);
	if (FoundIndex != INDEX_NONE)
	{
		...
		FObjectChooserBase::EIteratorStatus Result = EntryValue.IterateObjects(Context, Callback);
		return Result;
	}
...
FObjectChooserBase::EIteratorStatus UProxyTable::IterateProxyObjects(const FGuid& Key, FObjectChooserBase::FObjectChooserIteratorCallback Callback) const
{
	...
	return IterateProxyObjects(Key, Context, Callback);
}

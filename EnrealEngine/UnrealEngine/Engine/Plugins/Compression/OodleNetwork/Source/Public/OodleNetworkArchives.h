...
protected:
	...
	public:
		 ...
		 */
		inline void Set(FOodleNetworkArchiveBase& Ar, Type InValue)
		{
			Value = InValue;
...
public:
	 ...
	 */
	inline friend FArchive& operator<<(FArchive& Ar, FOodleCompressedData& Value)
	{
		Value.Serialize(Ar);
		return Ar;
	}
	 ...
	 * @return			Returns a chained-reference to the archive
	 */
	template<class Type> inline friend FArchive& operator<<(FArchive& Ar, TRewritable<Type>& Value)
	{
		Value.Serialize(Ar);
		return Ar;
	}

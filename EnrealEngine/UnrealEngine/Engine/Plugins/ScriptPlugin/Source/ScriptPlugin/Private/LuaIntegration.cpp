...
struct FLuaUObject
{
	...
	static int32 LuaStaticFindObject(lua_State* LuaState)
	{
		...
		EFindObjectFlags SearchFlags = !!(lua_toboolean(LuaState, 4)) ? EFindObjectFlags::ExactClass : EFindObjectFlags::None;
		UObject* Result = StaticFindObject(Class, Package, *Name, SearchFlags);
	...
	*/
	static int32 LuaStaticFindObjectFast(lua_State* LuaState)
	{
		...
		FString Name = ANSI_TO_TCHAR(luaL_checkstring(LuaState, 3));
		EFindObjectFlags SearchFlags = !!(lua_toboolean(LuaState, 4)) ? EFindObjectFlags::ExactClass : EFindObjectFlags::None;
		UObject* Result = StaticFindObjectFast(Class, Package, *Name, SearchFlags);
		lua_pushlightuserdata(LuaState, Result);
		return 1;
	}

...
#define UE_API SLATESCRIPTINGCOMMANDS_API
...
struct FScriptingCommand
{
	...
	inline bool operator==(const FScriptingCommand& Other) const { return Other.CommandInfo.Name == CommandInfo.Name; }
	inline bool operator!=(const FScriptingCommand& Other) const { return !(*this == Other); }

	inline bool operator==(const FName OtherName) const { return OtherName == CommandInfo.Name; }
	inline bool operator!=(const FName OtherName) const { return !(*this == OtherName); }
...
public:
	 ...
	 */
	UE_API void RegisterCommandListForContext(const FName ContextName, TSharedRef<FUICommandList> CommandList);
	 ...
	 * @param	CommandList		The command list to register in this context
	 */
	UE_API void UnregisterCommandListForContext(const FName ContextName, TSharedRef<FUICommandList> CommandList);

	/**
	 * Unregisters all command lists from the given context.
	 * @param	ContextName		The context name as defined in the binding manager
	 */
	UE_API bool UnregisterContext(const FName ContextName);
...
public:	
	...
	UE_API bool RegisterCommand(FScriptingCommandInfo CommandInfo, FExecuteCommand OnExecuteCommand, bool bOverrideExisting = false);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Editor Scripting | Commands |")
	UE_API bool RegisterCommandChecked(FScriptingCommandInfo CommandInfo, FExecuteCommand OnExecuteCommand,
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Editor Scripting | Commands |")
	UE_API bool UnregisterCommand(FScriptingCommandInfo CommandInfo);

	/** Checks whether the given command is registered within the subsystem. Using name, set and context for comparison */
	UFUNCTION(BlueprintCallable, Category = "Editor Scripting | Commands |")
	UE_API bool IsCommandRegistered(FScriptingCommandInfo CommandInfo, bool bCheckInputChord = true) const;
	...
	UE_API TArray<FScriptingCommandInfo> GetRegisteredCommands() const;

	/** Checks whether commands registered in the subsystem can be executed */
	UFUNCTION(BlueprintCallable, Category = "Editor Scripting | Commands | Global")
	UE_API bool CanExecuteCommands() const;

	/** Sets whether commands registered in the subsystem can be executed */
	UFUNCTION(BlueprintCallable, Category = "Editor Scripting | Commands | Global")
	UE_API void SetCanExecuteCommands(bool bShouldExecuteCommands);
	...
	UE_API bool RegisterCommandSet(FName SetName);

	/** Checks whether the given set is currently registered in the subsystem */
	UFUNCTION(BlueprintCallable, Category = "Editor Scripting | Commands | Sets")
	UE_API bool IsCommandSetRegistered(FName SetName) const;
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Editor Scripting | Commands | Sets")
	UE_API bool UnregisterCommandSet(FName SetName);
	...
	UE_API void SetCanSetExecuteCommands(FName SetName, bool bShouldExecuteCommands);
	...
	UE_API bool CanSetExecuteCommands(const FName SetName) const;
	...
	UE_API TArray<FName> GetAvailableContexts() const;
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Editor Scripting | Commands | Contexts")
	UE_API bool IsContextRegistered(FName ContextName) const;
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Editor Scripting | Commands | Contexts")
	UE_API int GetBindingCountForContext(FName ContextName);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Editor Scripting | Commands | Contexts")
	UE_API bool IsInputChordMapped(FName ContextName, FInputChord InputChord) const;	
	
private:
	/** The internal method to dynamically registers an editor command */
	UE_API bool RegisterNewScriptingCommand(FScriptingCommandInfo CommandInfo, FExecuteCommand OnExecuteCommand,
	...
	UE_API bool RegisterScriptingCommand(const TSharedRef<FScriptingCommand> ScriptingCommand);

	/** Unregisters a context command from its command lists. This assumes the command is already contained in a set */
	UE_API bool UnregisterScriptingCommand(const TSharedRef<FScriptingCommand> ScriptingCommand);

	/** The delegate bound to registered commands. It will broadcast back to the given dynamic delegate */
	UFUNCTION()
	static UE_API void HandleExecuteAction(FExecuteCommand OnExecuteAction, const FScriptingCommandInfo CommandInfo);
	 ...
	 */
	UFUNCTION()
	UE_API bool HandleCanExecuteAction(FCanExecuteCommand OnCanExecuteAction, const FScriptingCommandInfo CommandInfo) const;
	 ...
	 */
	UFUNCTION()
	UE_API bool DefaultCanExecuteAction(FName SetName) const;

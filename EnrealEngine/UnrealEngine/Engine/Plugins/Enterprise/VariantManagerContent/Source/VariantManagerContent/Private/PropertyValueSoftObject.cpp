...
void UPropertyValueSoftObject::SetRecordedData(const uint8* NewDataBytes, int32 NumBytes, int32 Offset /*= 0*/)
{
	...
	{
		UE_LOG(LogVariantContent, Log, TEXT("Attempting to set a UPropertyValueSoftObject with data that is %d bytes long, instead of %" SIZE_T_FMT "! It should receive just an object pointer, and not a full FSoftObjectPtr!"), NumBytes, sizeof(UObject*));
		return;
	}

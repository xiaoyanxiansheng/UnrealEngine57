...
bool FTechSoftInterface::InitializeKernel(const TCHAR* InEngineDir)
{
#ifdef USE_TECHSOFT_SDK
	if (bIsInitialize)
	{
	...
	FString EngineDir(InEngineDir);
	if (EngineDir.IsEmpty())
	{
		EngineDir = FPaths::EngineDir();
	}

	static const TCHAR* ThirdPartyDir = TEXT("Binaries/ThirdParty/TechSoft");
	
	FString TechSoftDllPath = FPaths::Combine(EngineDir, ThirdPartyDir, FPlatformProcess::GetBinariesSubdirectory());
...
{
	return A3DSurfBaseWithDomainGetAsNurbs(SurfacePtr, nullptr, Tolerance, bUseSameParameterization, OutDataPtr);
}

...
void FDatasmithSceneGraphBuilder::FillAnchorActor(const TSharedRef<IDatasmithActorElement>& ActorElement, const FString& CleanFilenameOfCADFile)
{
	...
	CADLibrary::FArchiveReference* Reference = SceneGraph->GetRootReference();
	if (!ensureMsgf(Reference, TEXT("FDatasmithSceneGraphBuilder::FillAnchorActor expected root Reference in the Scene Graph")))
	{
		return;
	}
...
bool FDatasmithSceneBaseGraphBuilder::Build()
{
	FActorData Data(TEXT(""));
	CADLibrary::FArchiveReference* Reference = SceneGraph->GetRootReference();
	if (!ensureMsgf(Reference, TEXT("FDatasmithSceneBaseGraphBuilder::Build: expected root Reference in the Scene Graph")))
	{
		return false;
	}
		...
		if (!UnloadedReference.ExternalFile.GetSourcePath().IsEmpty())
		{
			...
			if (SceneGraph)
			{
				if (AncestorSceneGraphHash.Find(InstanceSceneGraphHash) == INDEX_NONE)
				{
					...
					Reference = SceneGraph->GetRootReference();
					if (ensureMsgf(Reference, TEXT("FDatasmithSceneBaseGraphBuilder::BuildInstance expected root Reference in the Scene Graph of an external reference")))
					{

...
+				 */
	...
	FProperty* GetPropertyFromBinding(FDataprepParameterizationBinding& Binding, void*& OutPropertyValueAddress)
	{
		...
		FProperty* PropertyAtCurrentLevel = nullptr;
		...
		while ( LevelIndex < PropertyChain.Num() && CurrentOuter )
		{
			...
			PropertyAtCurrentLevel = FindFProperty<FProperty>(CurrentOuter.Get<UStruct>(), PropertyLink.PropertyName);
			if (!PropertyAtCurrentLevel)
			{
				if ( UStruct* OuterAsStruct = CurrentOuter.Get<UStruct>() )
				{
					OuterAsStruct = OuterAsStruct->GetSuperStruct();
					while ( !PropertyAtCurrentLevel && OuterAsStruct )
					{
						PropertyAtCurrentLevel = FindFProperty<FProperty>(OuterAsStruct, PropertyLink.PropertyName);
						OuterAsStruct = OuterAsStruct->GetSuperStruct();
					}
				}
			}

			PropertyLink.CachedProperty = PropertyAtCurrentLevel;
			CurrentOuter = GetOuterForPropertyFinding( PropertyAtCurrentLevel);
			LevelIndex++;
		}
		...
		{
			PropertyAtCurrentLevel = PropertyChain.Last().CachedProperty.Get();
			FValueTypeValidationData ValueTypeValidationData;
			PopulateValueTypeValidationData( PropertyAtCurrentLevel, ValueTypeValidationData );
	...
	{
		FFieldClass* PropertyClass = DestinationProperty.GetClass();
		...
		check(PropertyClass == SourceProperty.GetClass() );
...
{
	if ( FProperty* ParameterizationProperty = NameToParameterizationProperty.FindRef( ParameterName ) )
	{
		void* AddressOfParameterValue = DataprepParameterization::GetAddressOf( *ParameterizationProperty, DefaultParameterisation, INDEX_NONE );
			...
			if ( const UDataprepParameterizationBindings::FSetOfBinding* Bindings = BindingsContainer->GetBindingsFromParameter( ParameterName ) )
			{
				...
				for ( const TSharedRef<FDataprepParameterizationBinding>& Binding : *Bindings )
				{
					...
					if ( FProperty* BindingProperty = DataprepParameterization::GetPropertyFromBinding( Binding.Get(), AddressOfBindingValue ) )
					{
						...
						ObjectBinded->Modify();
						DataprepParameterization::CopyValue( *BindingProperty, AddressOfBindingValue, *ParameterizationProperty, AddressOfParameterValue );

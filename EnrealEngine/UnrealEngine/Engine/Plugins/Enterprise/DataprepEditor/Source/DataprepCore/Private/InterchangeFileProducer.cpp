...
bool UInterchangeFileProducer::Execute(TArray< TWeakObjectPtr< UObject > >& OutAssets)
{
	...
	const FString PackagePath = TransientPackage->GetPathName();
	FInterchangeResults ImportResults = InterchangeManager.ImportSceneAsync(PackagePath, SourceData, ImportAssetParameters);
	...
	TSet<UObject*> AssetsToAdd;
	auto AddAssets = [&AssetsToAdd, &PackagePath](const TArray<UObject*>& Objects)
		{
			for (UObject* Object : Objects)
			{
				if (Object && Object->GetPathName().StartsWith(PackagePath) && !Object->IsA<AActor>())
				{
					AssetsToAdd.Add(Object);
				}
			}
		};

	AddAssets(ImportResults.Get<0>()->GetImportedObjects());
	AddAssets(ImportResults.Get<1>()->GetImportedObjects());

	OutAssets.Reserve(OutAssets.Num() + AssetsToAdd.Num());
	OutAssets.Append(AssetsToAdd.Array());

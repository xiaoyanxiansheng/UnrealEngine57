...
void SDataprepGraphTrackNode::Construct(const FArguments& InArgs, UDataprepGraphRecipeNode* InNode)
{
	...
	SGraphNode::MoveTo( UE::Slate::CastToVector2f(TrackAnchor), NodeFilter);
...
void SDataprepGraphTrackNode::MoveTo(const FVector2f& NewPosition, FNodeSet& NodeFilter, bool bMarkDirty)
{
	// Block track node to specific position
	SGraphNode::MoveTo( UE::Slate::CastToVector2f(TrackAnchor), NodeFilter, bMarkDirty);
}
...
FSlateRect SDataprepGraphTrackNode::Update()
{
	...
	{
		const FVector2f WorkingSize = UE::Slate::CastToVector2f(TrackWidget->WorkingSize);
		const FVector2f TrackPosition = GetPosition2f().GetAbs();
		...
		if(TrackWidget->DragIndicatorIndex == INDEX_NONE && !bNodeDragging)
		{
			...
			const FVector2f PanelSize = GraphPanel.GetTickSpaceGeometry().GetLocalSize() / GraphPanel.GetZoomAmount();

			const FVector2f TargetSize = FVector2f(FMath::Max(WorkingSize.X, PanelSize.X), FMath::Max(WorkingSize.Y, PanelSize.Y)) + TrackPosition + 20.f;
		...
		WorkingArea = FSlateRect(FVector2f::ZeroVector, WorkingSize + TrackPosition);
	}

...
public:
	...
	virtual FSlateRect GetTitleRect() const override
	{
		...
		const FVector2f NodeSize = InlineEditableText.IsValid() ? InlineEditableText->GetDesiredSize() : GetDesiredSize();
...
public:
	...
	virtual FReply OnDrop(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent) override
	{
		...
		if (DragActionStepNodeOp.IsValid() && ParentPtr.IsValid())
		{
			...
			{
				const FVector2f NodeAddPosition = ParentPtr.Pin()->NodeCoordToGraphCoord( MyGeometry.AbsoluteToLocal( DragDropEvent.GetScreenSpacePosition() ) );
...
FVector2f SDataprepGraphBaseActionNode::GetNodeMinimumSize2f() const
{
	return FVector2f( SDataprepGraphActionNode::DefaultWidth, SDataprepGraphActionNode::DefaultHeight );
}

FVector2f SDataprepGraphBaseActionNode::GetNodeMaximumSize2f() const
{
	return FVector2f( UserSize.X + 100.0f, UserSize.Y + 0.0f );
}

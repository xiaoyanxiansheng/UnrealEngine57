...
void SDataprepGraphEditor::Construct(const FArguments& InArgs, UDataprepAsset* InDataprepAsset)
{
	...
	Events.OnCreateActionMenuAtLocation = SGraphEditor::FOnCreateActionMenuAtLocation::CreateSP(this, &SDataprepGraphEditor::OnCreateActionMenu);
...
void SDataprepGraphEditor::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	...
	if(bIsComplete)
	{
		...
		UpdateLayout( AllottedGeometry.GetLocalSize(), FDeprecateSlateVector2D(Location), ZoomAmount );
	}

	SGraphEditor::Tick( AllottedGeometry, InCurrentTime, InDeltaTime);
}

void SDataprepGraphEditor::UpdateLayout( const UE::Slate::FDeprecateVector2DParameter& LocalSize, const UE::Slate::FDeprecateVector2DParameter& Location, float ZoomAmount )
{
	if(SDataprepGraphTrackNode* TrackGraphNode = TrackGraphNodePtr.Pin().Get())
	{
		...
		{
			FVector2f ComputedLocation(Location);

			FVector2f PanelSize = LocalSize / ZoomAmount;
			FVector2f WorkingSize = WorkingArea.GetSize();
...
{
	return OnCreateActionMenu(InGraph, UE::Slate::CastToVector2f(InNodePosition), InDraggedPins, bAutoExpand, InOnMenuClosed);
}

FActionMenuContent SDataprepGraphEditor::OnCreateActionMenu(UEdGraph* InGraph, const FVector2f& InNodePosition, const TArray<UEdGraphPin*>& InDraggedPins, bool bAutoExpand, SGraphEditor::FActionMenuClosed InOnMenuClosed)
{
...
FActionMenuContent SDataprepGraphEditor::OnCreateNodeOrPinMenu(UEdGraph* CurrentGraph, const UEdGraphNode* InGraphNode, const UEdGraphPin* InGraphPin, FMenuBuilder* MenuBuilder, bool bIsDebugging)
{
	...
	return OnCreateActionMenu(CurrentGraph, GetPasteLocation2f(), TArray<UEdGraphPin*>(), true, SGraphEditor::FActionMenuClosed());
}

...
class DSAnimBlock:
    ...
    def setName(self, value):
        self.Name = value
    #API V2 to match the getObjectId on vrdNode
...
class DSAnimationLibrary:
    ...
    def catalogClip(self, animClip):
        ...
        Args:
            animClip (vrdNode): Node that is of type AnimClip or AnimWizClip and contains other clips or AnimBlocks
        """
        ...
        def catalogClipInner(animClip, blockDestination):
            ...
            Args:
                animClip (vrdNode): Node that is of type AnimClip or AnimWizClip and contains other clips or AnimBlocks
                blockDestination (list of AnimBlock vrdNode): Where to place the found AnimNode usages into
            """
            for child in animClip.children:
                childName = child.getName()
                #API V2 convert new vrdNode to old vrNodePtr
                APIV1Child = vrNodePtr.toNode(child.getObjectId())
                childType = APIV1Child.getType()
                ...
                #API V2
                childDSAnimBlockUsage.StartSeconds = bb.getMin().x()  # minX
                childDSAnimBlockUsage.EndSeconds = bb.getMax().x()  # maxX
                childDSAnimBlockUsage.IsActive = APIV1Child.getActive()
...
def getAnimRoot(node):
    ...
    Args:
        node (vrdNode): Node to get the animRoot from

    Returns:
        (vrdNode): animationRoot of 'node' or None
    """
    animRoot = None
    #API V2 is there a function to access attachment without converting to a vrNodePtr
    APIV1Node = vrNodePtr.toNode(node.getObjectId())
    if APIV1Node.hasAttachment("AnimAttachment"):
        animAttachment = APIV1Node.getAttachment("AnimAttachment")
        animRootId = vrFieldAccess.vrFieldAccess(animAttachment).getFieldContainerID("animationRoot")
        animRoot = vrNodeService.getNodeFromId(animRootId)
    return animRoot
...
def splitAnimationFromGeometry(node):
    ...
    Args:
        node (vrdNode): Node to split

    Returns:
        (vrdNode): parent node with animations and transformation (whose child is 'node'), or None
    """
    #API V2
    if node.isType(vrKernelServices.vrdGeometryNode) and getAnimRoot(node) is not None:
        parentNode = vrScenegraphService.createNode('Transform3D', node.getParent(), node.getName() + '_Anim')
        ...
        vrScenegraphService.pasteTransformation([parentNode])
...
def getFreeAnimCurves(animRoot):
    ...
    Args:
        animRoot (vrdNode): animationRoot node of an AnimAttachment

    Returns:
        (list of vrdNode): List of 'AnimCurve' nodes
    """
    result = []

    for child in animRoot.children:

        #API V2 convert new vrdNode to old vrNodePtr
        APIV1Child = vrNodePtr.toNode(child.getObjectId())
        childType = APIV1Child.getType()
        if childType == "AnimCurve":
            bb = child.getBoundingBox()
            # bb[0] and bb[3] are minX and maxX respectively, startSeconds and endSeconds of the curve
            #API V2
            if not dsutils.isClose(bb.getMin().x(), bb.getMax().x()):
                result.append(child)
...
def getKeyTime():
    ...
    for node in vrScenegraphService.getAllNodes():
        ...
        for block in animRoot.children:

            for curve in block.children:

                bb = curve.getBoundingBox()
                #API V2
                startTimeSeconds = bb.getMin().x()
...
def catalogSceneAnimations(unitScale, baseTime, playbackSpeed):
    ...
    for node in vrScenegraphService.getAllNodes():
        ...
        numBlocks = animRoot.children.size()
        for i in range(numBlocks-1, -1, -1):
            block = animRoot.children.at(i)
            #API V2 convert new vrdNode to old vrNodePtr
            APIV1Block = vrNodePtr.toNode(block.getObjectId())
            if APIV1Block.getType() == "TimeShapeCache":
    ...
    #API V2 convert old vrNodePtr to new vrdNode as we try to use vrdNode everywhere
    animBlockNodes = [vrNodeService.getNodeFromId(a.getID()) for a in vrAnimWidgets.getAnimBlockNodes(True)]
    dss.renameObjectsAndKeepOriginals(animBlockNodes)
    ...
    for node in vrScenegraphService.getAllNodes():
        ...
        if len(freeCurves) > 0:
            ...
            newAnimBlock = vrScenegraphService.createNode('AnimBlock', animRoot, node.getName() + "_BaseAnimation")
            ...
            library.BlockNameToDSBlock[newAnimBlock.getName()] = DSAnimBlock(library.getObjectId(), newAnimBlock.getName())
    ...
    for node in vrScenegraphService.getAllNodes():
        ...
        if APIV1Node.getID() != node.getObjectId():
            logging.error('######################## vrNodePtr and vrdNode do not share same object ID')
        storage.Translation = APIV1Node.getTranslation()
        storage.Rotation = APIV1Node.getRotation()
        storage.Scale = APIV1Node.getScale()
        storage.Visible = APIV1Node.getActive()
        storage.RotationOrientation = [rotOri.x(), rotOri.y(), rotOri.z()]

        unanimatedTransforms[APIV1Node.getID()] = storage
    ...
    #API V2 convert old vrNodePtr to new vrdNode as we try to use vrdNode everywhere
    animBlockNodes = [vrNodeService.getNodeFromId(a.getID()) for a in vrAnimWidgets.getAnimBlockNodes(True)]
    for block in animBlockNodes :
        library.BlockNameToDSBlock[block.getName()] = DSAnimBlock(library.getObjectId(), block.getName())
    ...
    #API V2 convert old vrNodePtr to new vrdNode as we try to use vrdNode everywhere
    clips = [vrNodeService.getNodeFromId(a.getID()) for a in vrAnimWidgets.getAnimClipNodes()]
    for clip in clips:
    ...
    for node in vrScenegraphService.getAllNodes():
        ...
        for block in animRoot.children:
            #API V2 convert new vrdNode to old vrNodePtr
            APIV1Block = vrNodePtr.toNode(block.getObjectId())
            if APIV1Block.getType() != "AnimBlock":
                continue
            ...
            numCurves = block.children.size()

            for curveIndex in range(numCurves-1, -1, -1):
                curve = block.children.at(curveIndex)    
                #API V2 convert new vrdNode to old vrNodePtr
                APIV1Curve = vrNodePtr.toNode(curve.getObjectId())
                curveType = APIV1Curve.getType()
                if curveType != "AnimCurve":
                    continue

                #move node
                animRoot.children.append(curve)
                block.children.remove(curve)
                ...
                dsid = dsBlock.DSID
                if APIV1Curve.fields() == None or not APIV1Curve.fields().isValid():
                    logging.error("There should be fields in that AnimCurve " + str(APIV1Curve.getName()) + " " + str(APIV1Curve.fields()))
                    continue
                channelName = APIV1Curve.fields().getString('channelName')
                ...
                dsBlock.ChildCurveNamesToCurves[APIV1Curve.fields().getString('channelName') + APIV1Curve.fields().getString('suffix')] = dsCurve
            ...
            for dsCurve in dsBlock.ChildCurveNamesToCurves.values():
                block.children.append(dsCurve.AnimCurve)
                animRoot.children.remove(dsCurve.AnimCurve)

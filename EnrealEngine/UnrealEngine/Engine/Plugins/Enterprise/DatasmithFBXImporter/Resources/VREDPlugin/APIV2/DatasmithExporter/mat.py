...
def extractMaterials(vredMats):
    ...
    for vredMat in vredMats:
        ...
        #API V2
        vredMatId = vredMat.getObjectId()
        if vredMatId in processedMats:
            continue
        processedMats.add(vredMatId)

        #API V2 get old vredMat
        APIV1VredMat = vrMaterialPtr.toMaterial(vredMat.getObjectId())
        mat.name = vredMat.getName()
        mat.matType = APIV1VredMat.getType()
        mat.vredMatId = vredMatId
        if (len(APIV1VredMat.getNodes()) != len(vrMaterialService.findNodesWithMaterial(vredMat))):
            logging.warning("###################   issue get nodes using material ")
        mat.users = [n.getName() for n in vrMaterialService.findNodesWithMaterial(vredMat)]

        # Keep track of submaterials if we're a composite material
        for vredSubmat in APIV1VredMat.getSubMaterials():
        ...
        if fields.hasField('colorComponentData'):
            ...
            if ccd != None and ccd.isValid():
                ...
                logging.error("APIV2 Material cannot extract color component data")

        # If this is a triplanar material, fixup the triplanar mapping info for
        # affected textures
        logging.debug('\tFixup for triplanar')
        if APIV1VredMat.getType() == 'UTriplanarMaterial':

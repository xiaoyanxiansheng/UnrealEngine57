...
UObject* FDatasmithImporterImpl::PublicizeAsset( UObject* SourceAsset, const TCHAR* DestinationPath, UObject* ExistingAsset )
{
	...
	if ( ExistingAsset != nullptr && ExistingAsset != SourceAsset )
	{
		...
		{
			if (DestinationAsset->Rename(*AssetName, DestinationPackage, REN_Test))
			{
				DestinationAsset->Rename(*AssetName, DestinationPackage, REN_DontCreateRedirectors | REN_NonTransactional);
			}
			// Something wrong happened during a previous re-import of this asset
			// There already is a object in DestinationPackage. See UE-306372 for details.
			// Correct things by making package and asset names equal as it should have always been.
			else
			{
				const UObject* ExistingObject = StaticFindObject(nullptr, DestinationPackage, AssetName, EFindObjectFlags::ExactClass);
				UE_LOG(LogDatasmithImport, Warning, TEXT("Renaming %s to %s failed because the latter already exists."), *DestinationAsset->GetFullName(), *ExistingObject->GetFullName());
				
				FAssetToolsModule& AssetToolsModule = FModuleManager::LoadModuleChecked<FAssetToolsModule>("AssetTools");

				FString NewAssetName;
				FString DestinationPackagePath;
				AssetToolsModule.Get().CreateUniqueAssetName(DestinationPackage->GetPathName(), TEXT(""), DestinationPackagePath, NewAssetName);

				DestinationPackage = CreatePackage(*DestinationPackagePath);
				DestinationAsset->Rename(*NewAssetName, DestinationPackage, REN_DontCreateRedirectors | REN_NonTransactional);

				UE_LOG(LogDatasmithImport, Log, TEXT("Asset has been renamed to %s."), *(DestinationPackagePath + TEXT(".") + NewAssetName));
			}
		}
...
{
	// Need exact class (UHierarchicalInstancedStaticMeshComponent should not be reused for UStaticMeshComponent)
	USceneComponent* DestinationComponent = static_cast<USceneComponent*>( StaticFindObjectFastSafe(SourceComponent.GetClass(), &DestinationActor, SourceComponent.GetFName(), EFindObjectFlags::ExactClass) );

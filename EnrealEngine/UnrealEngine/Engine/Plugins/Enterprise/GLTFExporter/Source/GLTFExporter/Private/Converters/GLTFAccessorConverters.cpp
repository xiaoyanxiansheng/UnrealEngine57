...
namespace
{
	...
	struct TGLTFVertexNormalUtilities
	{
		...
		static DestinationType ConvertDelta(const SourceType& NormalDelta)
		{
			return FGLTFCoreUtilities::ConvertNormalDelta(NormalDelta);
		}
	};
	...
	struct TGLTFVertexNormalUtilities<FGLTFVector3, SourceType>
	{
		...
		static FGLTFVector3 ConvertDelta(const SourceType& NormalDelta)
		{
            return FGLTFCoreUtilities::ConvertNormalDelta(NormalDelta.ToFVector3f());
		}
	};
...
FGLTFJsonAccessor* FGLTFPositionDeltaBufferConverter::Convert(const FGLTFMeshSection* MeshSection, const TMap<uint32, FVector3f>* TargetPositionDeltas)
{
    if (!TargetPositionDeltas)
    {
        return nullptr;
    }

    const TArray<uint32>& IndexMap = MeshSection->IndexMap;
    const uint32 VertexCount = IndexMap.Num() > 0 ? IndexMap.Num() : TargetPositionDeltas->Num();
    ...
    if (IndexMap.Num() > 0)
    {
        ...
        {
            const uint32 MappedVertexIndex = IndexMap[VertexIndex];
            const FVector3f* Delta = TargetPositionDeltas->Find(MappedVertexIndex);
            FVector3f Position = Delta ? *Delta : FVector3f();
    ...
    else
    {
        for (uint32 VertexIndex = 0; VertexIndex < VertexCount; ++VertexIndex)
        {
            const FVector3f* Delta = TargetPositionDeltas->Find(VertexIndex);
            FVector3f Position = Delta ? *Delta : FVector3f();
...
FGLTFJsonAccessor* FGLTFNormalDeltaBufferConverter::Convert(const FGLTFMeshSection* MeshSection, const TMap<uint32, FVector3f>* TargetNormalDeltas, bool bHighPrecision)
{
    FGLTFJsonBufferView* BufferView;
    EGLTFJsonComponentType ComponentType;
    ...
    if (bMeshQuantization)
    {
        ...
        {
            ComponentType = EGLTFJsonComponentType::Int16;
            BufferView = ConvertBufferView<FGLTFInt16Vector4, FPackedRGBA16N>(MeshSection, TargetNormalDeltas);
            BufferView->ByteStride = sizeof(FGLTFInt16Vector4);
        }
        else
        {
            ComponentType = EGLTFJsonComponentType::Int8;
            BufferView = ConvertBufferView<FGLTFInt8Vector4, FPackedNormal>(MeshSection, TargetNormalDeltas);
            BufferView->ByteStride = sizeof(FGLTFInt8Vector4);
        }
    }
    else
    {
        ComponentType = EGLTFJsonComponentType::Float;
        BufferView = bHighPrecision
            ? ConvertBufferView<FGLTFVector3, FPackedRGBA16N>(MeshSection, TargetNormalDeltas)
            : ConvertBufferView<FGLTFVector3, FPackedNormal>(MeshSection, TargetNormalDeltas);
    }
...
template <typename DestinationType, typename SourceType>
FGLTFJsonBufferView* FGLTFNormalDeltaBufferConverter::ConvertBufferView(const FGLTFMeshSection* MeshSection, const TMap<uint32, FVector3f>* TargetNormalDeltas)
{
    const TArray<uint32>& IndexMap = MeshSection->IndexMap;
    const uint32 VertexCount = IndexMap.Num();
    ...
    if (TargetNormalDeltas && TargetNormalDeltas->Num())
    {
        for (uint32 VertexIndex = 0; VertexIndex < VertexCount; ++VertexIndex)
        {
            const uint32 MappedVertexIndex = IndexMap[VertexIndex];

            const FVector3f* TargetNormalDelta = TargetNormalDeltas->Find(MappedVertexIndex);
            if (TargetNormalDelta)
            {
                Normals[VertexIndex] = TGLTFVertexNormalUtilities<DestinationType, SourceType>::ConvertDelta(*TargetNormalDelta);
            }
        }
    }
...
FGLTFJsonAccessor* FGLTFNormalBufferConverterRaw::Convert(FGLTFNormalArray NormalsSource, bool bNormalize)
{
    ...
    else
    {
        for (uint32 VertexIndex = 0; VertexIndex < VertexCount; ++VertexIndex)
        {
            Normals[VertexIndex] = FGLTFCoreUtilities::ConvertVector(NormalsSource[VertexIndex]);
        }
    }

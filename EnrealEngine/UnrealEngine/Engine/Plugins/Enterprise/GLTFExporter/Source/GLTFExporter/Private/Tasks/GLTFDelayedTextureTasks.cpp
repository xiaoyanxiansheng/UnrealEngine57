...
{
	FGLTFTextureUtilities::FullyLoad(LightMap);
	LightMap->GetName(JsonTexture->Name);

	const FIntPoint Size = FGLTFTextureUtilities::GetInGameSize(LightMap);
	UTextureRenderTarget2D* RenderTarget = FGLTFTextureUtilities::CreateRenderTarget(Size, false);

	// TODO: preserve maximum image quality (avoid compression artifacts) by copying source data (and adjustments) to a temp texture
	FGLTFTextureUtilities::DrawTexture(RenderTarget, LightMap);
	...
	if (Builder.ExportOptions->bAdjustNormalmaps && LightMap->IsNormalMap())
	{
		// TODO: add support for adjusting normals in GLTFNormalMapPreview instead
		FGLTFTextureUtilities::FlipGreenChannel(*Pixels);
	}

	const bool bIgnoreAlpha = FGLTFTextureUtilities::IsAlphaless(LightMap->GetPixelFormat());

	JsonTexture->Source = Builder.AddUniqueImage(Pixels, Size, bIgnoreAlpha, JsonTexture->Name);
	JsonTexture->Sampler = Builder.AddUniqueSampler(LightMap->GetTextureAddressX(), LightMap->GetTextureAddressX(), LightMap->Filter, LightMap->LODGroup);
}

...
public:
	...
	inline bool IsValid() const { return X != 127 || Y != 127 || Z != 127; }

	inline void SetFromVector(const FVector3f& Normal)
	{
		SetFromFloats(Normal.X, Normal.Y, Normal.Z);
	}
	inline void SetFromFloats(const float& InX, const float& InY, const float& InZ)
	{
	...
	inline FVector3f ToVector() const { return FVector3f(X / 127.5f - 1, Y / 127.5f - 1, Z / 127.5f - 1); }
};
...
public:
	...
	inline void CopyFrom(const FLidarPointCloudPoint& Other)
	{
	...
	inline FLidarPointCloudPoint Transform(const FTransform3f& Transform) const
	{
...
public:
	...
	inline uint8* GetData() { return Data.GetData(); }
	inline bool InUse() const { return bInUse; }
...
public:
	...
	static inline FLidarPointCloudRay FromLocations(const FVector3f& Origin, const FVector3f& Destination)
	{
	...
	inline FLidarPointCloudRay ShiftBy(const FVector3f& Offset) const
	{
		return FLidarPointCloudRay(Origin + Offset, Direction);
	}
	inline FLidarPointCloudRay ShiftBy(const FVector& Offset) const
	{
		return FLidarPointCloudRay(Origin + (FVector3f)Offset, Direction);
	}

	inline FVector3f GetDirection() const { return Direction; }
	inline void SetDirection(const FVector3f& NewDirection)
	{
	...
	inline bool Intersects(const FBox& Box) const
	{
	...
	inline bool Intersects(const FLidarPointCloudPoint* Point, const float& RadiusSq) const
	{
...
struct FLidarPointCloudClippingVolumeParams
{
	...
	inline bool operator<(const FLidarPointCloudClippingVolumeParams& O) const
	{

...
public:
	...
	inline TWeakPtr<FViewportClient> GetOwningViewportClient() const { return OwningViewportClient; }
	inline bool IsOwnedByEditor() const { return OwningViewportClient.IsValid(); }
	...
	UFUNCTION(BlueprintPure, Category = "Lidar Point Cloud")
	TArray<FLidarPointCloudPoint> GetSelectedPointsAsCopies(bool bReturnWorldSpace) const
	{
		TArray<FLidarPointCloudPoint> Points;
#if WITH_EDITOR
		GetSelectedPointsAsCopies(Points, bReturnWorldSpace);
#endif
		return Points;
	}
#if WITH_EDITOR
	void GetSelectedPointsAsCopies(TArray<FLidarPointCloudPoint>& Points, bool bReturnWorldSpace) const
	{
		if(PointCloud)
		{
			PointCloud->GetSelectedPointsAsCopies(Points, bReturnWorldSpace ? GetComponentTransform() : FTransform());
		}
	}
	void GetSelectedPointsAsCopies(TArray64<FLidarPointCloudPoint>& Points, bool bReturnWorldSpace) const
	{
		if(PointCloud)
		{
			PointCloud->GetSelectedPointsAsCopies(Points, bReturnWorldSpace ? GetComponentTransform() : FTransform());
...
public:
	...
	UFUNCTION(BlueprintPure, Category = "Components|LidarPointCloud")
	inline ELidarPointCloudSpriteShape GetPointShape() const { return PointShape; }

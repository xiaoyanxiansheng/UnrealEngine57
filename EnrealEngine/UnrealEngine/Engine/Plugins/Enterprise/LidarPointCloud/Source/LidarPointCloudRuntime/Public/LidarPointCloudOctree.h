...
public:
	inline FLidarPointCloudOctreeNode(FLidarPointCloudOctree* Tree, const uint8& Depth) : FLidarPointCloudOctreeNode(Tree, Depth, 0, FVector3f::ZeroVector) {}
	...
	/** Returns a pointer to the point data */
	inline TSharedPtr<FLidarPointCloudRenderBuffer> GetDataCache() { return DataCache; }

	/** Return a pointer to the vertex factory containing pre-cached geometry */
	inline TSharedPtr<FLidarPointCloudVertexFactory> GetVertexFactory() { return VertexFactory; }
	
	/** Return a pointer to the ray tracing geometry */
	inline TSharedPtr<FLidarPointCloudRayTracingGeometry> GetRayTracingGeometry() { return RayTracingGeometry; }
	...
	inline uint32 GetNumPoints() const { return NumPoints; }
...
public:
	...
	inline FVector3f GetExtent() const { return Extent; }
	...
	/** Returns the total number of nodes. */
	inline int32 GetNumNodesInUse() const { return NodesInUse.Num(); }
	...
	template <typename T>
	void GetSelectedPointsAsCopies_Internal(TArray<FLidarPointCloudPoint, T>& SelectedPoints, const FTransform& Transform) const;
	void GetSelectedPointsAsCopies(TArray<FLidarPointCloudPoint>& SelectedPoints, const FTransform& Transform) const;	
...
struct FLidarPointCloudTraversalOctreeNode
{
	...
	inline bool IsAvailable() const { return bSelected && DataNode->HasRenderData(); }
};

...
#include UE_INLINE_GENERATED_CPP_BY_NAME(SoundHandleSubsystem)
...
Audio::EResult USoundHandleSubsystem::Play(const Audio::FSoundHandleID ID)
{
	if(FSoundHandle* Handle = ActiveHandles.Find(ID))
	{
		...
		const FString* EventLogID = &Audio::Trace::EventLog::ID::PlayRequestSoundHandle;
#else
		const FString* EventLogID = nullptr;
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
		
		AudioDevice->AddNewActiveSound(Handle->ActiveSound, nullptr, EventLogID);
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
		const FString* EventLogID = &Audio::Trace::EventLog::ID::StopRequestedSoundHandle;
#else
		const FString* EventLogID = nullptr;
#endif // UE_AUDIO_PROFILERTRACE_ENABLED

		AudioDevice->StopActiveSound(&Handle->ActiveSound, EventLogID);
	}
}

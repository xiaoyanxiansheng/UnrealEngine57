...
void FCurveEditorMultiScaleTool::OnDragStart()
{
	...
	ActiveTransaction = MakeUnique<FScopedCurveChange>(
		FCurvesSnapshotBuilder(
			CurveEditor, ECurveChangeFlags::KeyAttributes | ECurveChangeFlags::MoveKeysAndRemoveStackedKeys
			).TrackSelectedCurves(),
		LOCTEXT("CurveEditorMultiScaleToolTransaction", "Multi Scale Key(s)")
		);
...
{
	// Call SetKeyPositions again with EPropertyChangeType::ValueSet to trigger any follow up operations, like getting rid of stacked keys...
	ApplyLastKeyPositions();
	// ... then finalize the transaction
	ActiveTransaction.Reset();
	
	KeysByCurve.Reset(); // User might not use our tool anymore - clean-up any cached memory.
...
void FCurveEditorMultiScaleTool::ScaleUnique(const FVector2D& InChangeAmount, const bool bInAffectsX, const bool bInAffectsY)
{
	...
	for (FKeyData& KeyData : KeysByCurve)
	{
		...
		TArray<FKeyPosition>& LastDraggedKeyPositions = KeyData.LastDraggedKeyPositions;
		LastDraggedKeyPositions.Empty(KeyData.StartKeyPositions.Num());
		...
		for (FKeyPosition StartPosition : KeyData.StartKeyPositions)
		{
			...
			LastDraggedKeyPositions.Add(StartPosition);
		}

		CurveModel->SetKeyPositions(KeyData.Handles, LastDraggedKeyPositions, EPropertyChangeType::Interactive);
	}
...
void FCurveEditorMultiScaleTool::ApplyLastKeyPositions()
{

...
void FCurveEditorTransformTool::OnDragStart()
{
	...
	ActiveTransaction = MakeUnique<FScopedCurveChange>(
		FCurvesSnapshotBuilder(CurveEditor,
			ECurveChangeFlags::KeyAttributes | ECurveChangeFlags::MoveKeysAndRemoveStackedKeys
			).TrackSelectedCurves(),
		LOCTEXT("CurveEditorTransformToolTransaction", "Transform Key(s)")
		);
...
{
static bool HaveSameScales(const FCurveEditor& CurveEditor)
{
	...
	// To handle FindFirstInteractiveView returning nullptr, we'll save the first FCurveEditorScreenSpace we encounter and compare against the rest.
	bool bCurvesHaveSameScales = true;
	TOptional<FCurveEditorScreenSpace> FirstCurveViewSpace;
	
	for (int32 ModelIndex = 0; ModelIndex < CurveModelIDs.Num(); ModelIndex++)
	{
		const SCurveEditorView* View = CurveEditor.FindFirstInteractiveView(CurveModelIDs[ModelIndex]);
		if (!ensureMsgf(View, TEXT("No view found for curve. Probably the selection contains stale keys.")))
		{
			continue;
		}
		
		const FCurveEditorScreenSpace CurveViewSpace = View->GetCurveSpace(CurveModelIDs[ModelIndex]);
		if (FirstCurveViewSpace.IsSet())
		{
			bCurvesHaveSameScales &= FMath::IsNearlyEqual(CurveViewSpace.PixelsPerInput(), FirstCurveViewSpace->PixelsPerInput())
				&& FMath::IsNearlyEqual(CurveViewSpace.PixelsPerOutput(), FirstCurveViewSpace->PixelsPerOutput());
		}
		else
		{
			FirstCurveViewSpace = CurveViewSpace;
		}
		
	}

	return bCurvesHaveSameScales;
}
}
...
void FCurveEditorTransformTool::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	...
	bCurvesHaveSameScales = UE::CurveEditorTools::HaveSameScales(*CurveEditor);

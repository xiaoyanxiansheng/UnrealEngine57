...
void FWaveformEditorWaveWriter::ExportTransformedWaveform(bool bPromptUserToSave)
{
	...
	TFunction<void(const USoundWave*)> OnSoundWaveWritten = [bPromptUserToSave, AssetName, AssetPath](const USoundWave* ResultingWave) {
		UE_LOG(LogWaveformEditor, Log, TEXT("Finished Exporting edited soundwave %s/%s"), *AssetPath, *AssetName);
		TArray<UPackage*> PackagesToSave;

		if (ResultingWave == nullptr)
		{
			UE_LOG(LogWaveformEditor, Warning, TEXT("Resulting Exported soundwave is invalid: %s/%s"), *AssetPath, *AssetName);

			return;
		}

		if (UPackage* SoundWavePackage = ResultingWave->GetPackage())
		{
			PackagesToSave.Add(SoundWavePackage);

			SoundWavePackage->SetDirtyFlag(true);
			...
			if (bSuccess)
			{
				UE_LOG(LogWaveformEditor, Log, TEXT("Exported soundwave saving succeeded: % s / % s"), *AssetPath, *AssetName);
			}
			else
			{
				UE_LOG(LogWaveformEditor, Warning, TEXT("Exported soundwave saving failed: %s/%s"), *AssetPath, *AssetName);
			}
		}
	};
	
	if (!WaveWriter->BeginWriteToSoundWave(AssetName, BufferToWrite, AssetPath, OnSoundWaveWritten, SourceSoundWave))
	{

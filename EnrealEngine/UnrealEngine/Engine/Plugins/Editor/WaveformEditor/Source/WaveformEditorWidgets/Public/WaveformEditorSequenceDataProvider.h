...
class FWaveformEditorSequenceDataProvider : public IFixedSampledSequenceViewProvider, public TSharedFromThis<FWaveformEditorSequenceDataProvider>
{
public:

	UE_API FWaveformEditorSequenceDataProvider(TObjectPtr<USoundWave> InSoundWave);
	...
	UE_API void GenerateLayersChain();
	...
	UE_API TArrayView<const FTransformationRenderLayerInfo> GetTransformLayers() const;

	UE_API const TRange<double> GetTransformedWaveformBounds() const;

	const TArray<float> GetTransformedPCMData() const { return TransformedPCMData; };
...
private:
	UE_API void GenerateSequenceDataInternal();

	UE_API void UpdateTransformedWaveformBounds(const uint32 FirstEditedSample, const uint32 LastEditedSample, const uint32 NumOriginalSamples);

	UE_API void CreateDurationHighlightLayer();

	UE_API TArray<Audio::FTransformationPtr> CreateTransformations() const;
	UE_API const bool CanChainChangeFileLength(const TArray<Audio::FTransformationPtr>& TransformationArray) const;

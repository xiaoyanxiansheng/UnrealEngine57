...
class SColorGradingDetailTreeRow : public STableRow<TSharedRef<FColorGradingDetailTreeItem>>
{
public:
	SLATE_BEGIN_ARGS(SColorGradingDetailTreeRow) {}
	SLATE_END_ARGS()

	//~ STableRow Interface
	void Construct(const FArguments& InArgs, const TSharedRef<FColorGradingDetailTreeItem>& InDetailTreeItem, const TSharedRef<STableViewBase>& InOwnerTableView, const FDetailColumnSizeData& InColumnSizeData);
...
private:
	...
	FReply OnRowAcceptDrop(const FDragDropEvent& DragDropEvent, EItemDropZone DropZone, TSharedRef<FColorGradingDetailTreeItem> TargetItem);

	/** Indicates whether the drag drop event can be accepted by this row */
	TOptional<EItemDropZone> CanRowAcceptDrop(const FDragDropEvent& DragDropEvent, EItemDropZone DropZone, TSharedRef<FColorGradingDetailTreeItem> Type);
	...
	void SetExpansionStateRecursive(const TSharedRef<FColorGradingDetailTreeItem>& TreeItem, bool bShouldBeExpanded);

private:
	/** A weak pointer to the detail tree item this row represents */
	TWeakPtr<FColorGradingDetailTreeItem> DetailTreeItem;

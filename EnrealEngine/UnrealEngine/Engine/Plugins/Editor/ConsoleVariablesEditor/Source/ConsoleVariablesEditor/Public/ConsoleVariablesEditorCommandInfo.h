...
struct FConsoleVariablesEditorCommandInfo
{
	 ...
	 */
	UE_API IConsoleCommand* GetConsoleCommandPtr();
 
	static UE_API UWorld* GetCurrentWorld();

	UE_API FString GetHelpText();
 
	UE_API EConsoleVariableFlags GetSource();
 
	UE_API void ClearSourceFlags();

	UE_API void SetSourceFlag(const EConsoleVariableFlags InSource);
 
	UE_API FText GetSourceAsText();

	static UE_API FText ConvertConsoleVariableSetByFlagToText(const EConsoleVariableFlags InFlag);
	 ...
	 */
	UE_API bool GetCurrentValueAsString(FString& OutValueAsString);
 
	UE_API bool IsCurrentValueDifferentFromInputValue(const FString& InValueToCompare);
 
	UE_API void OnConsoleVariableChanged(IConsoleVariable* ChangedVariable);
	...
	static const inline TArray<FStaticConsoleVariableFlagInfo> SupportedFlags =
	{
		...
		{ EConsoleVariableFlags::ECVF_SetByPreview, LOCTEXT("Source_SetByPreview", "Preview") }
	};
};

...
public:
	...
	UE_API void SelectAll(const FDynamicMesh3& Mesh, EType TypeIn);

	UE_API bool IsAllSelected(const FDynamicMesh3& Mesh) const;
	...
	UE_API void SaveStableEdgeIdentifiers(const FDynamicMesh3& Mesh);

	UE_API void RestoreFromStableEdgeIdentifiers(const FDynamicMesh3& Mesh);

	UE_API bool AreElementsPresentInMesh(const FDynamicMesh3& Mesh) const;

	UE_API FAxisAlignedBox3d ToBoundingBox(const FDynamicMesh3& Mesh, const FTransform Transform = FTransform3d::Identity) const;

	/**
	 * Utility method to construct a new selection from an existing reference selection, only with a new geometry type.
	 */
	UE_API FUVToolSelection GetConvertedSelection(const FDynamicMesh3& Mesh, FUVToolSelection::EType ExpectedSelectionType) const;
	...
	*/
	UE_API FUVToolSelection GetConvertedSelectionForUnwrappedMesh() const;
	...
	* If the target for this selection is currently invalid or unset, this will return an empty selection.
	*/
	UE_API FUVToolSelection GetConvertedSelectionForAppliedMesh() const;
	 ...
	 */
	static UE_API TArray<int32> ConvertUnwrappedElementIdsToAppliedElementIds(EType SelectionMode, const FDynamicMesh3& UnwrapMesh,
	 ...
	 */
	static UE_API TArray<int32> ConvertAppliedElementIdsToUnwrappedElementIds(EType SelectionMode, const FDynamicMesh3& AppliedMesh,

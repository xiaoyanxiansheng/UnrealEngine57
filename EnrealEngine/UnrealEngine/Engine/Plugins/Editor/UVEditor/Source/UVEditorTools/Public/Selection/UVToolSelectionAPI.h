...
public:
	 ...
	 */
	UE_API void SetSelections(const TArray<FUVToolSelection>& SelectionsIn, bool bBroadcast, bool bEmitChange);

	/**
	 * Equivalent to calling SetSelections with an empty array.
	 */
	UE_API void ClearSelections(bool bBroadcast, bool bEmitChange);
	 ...
	 */
	UE_API FVector3d GetUnwrapSelectionBoundingBoxCenter(bool bForceRecalculate = false);
	...
	// by some others.
	UE_API bool HaveUnsetElementAppliedMeshSelections() const;
	const TArray<FUVToolSelection>& GetUnsetElementAppliedMeshSelections() { return CurrentUnsetSelections; }
	UE_API void SetUnsetElementAppliedMeshSelections(const TArray<FUVToolSelection>& SelectionsIn, bool bBroadcast, bool bEmitChange);
	UE_API void ClearUnsetElementAppliedMeshSelections(bool bBroadcast, bool bEmitChange);
	...
	UE_API void SetSelectionMechanicEnabled(bool bEnabled);

	struct FSelectionMechanicOptions
	{
		bool bShowHoveredElements = true;
	};
	UE_API void SetSelectionMechanicOptions(const FSelectionMechanicOptions& Options);
	 ...
	 */
	UE_API void SetSelectionMechanicMode(EUVEditorSelectionMode Mode, 
	 ...
	 */
	UE_API void SetHighlightVisible(bool bUnwrapHighlightVisible, bool bAppliedHighlightVisible, bool bRebuild = true);
	...
	UE_API void SetHighlightOptions(const FHighlightOptions& Options);

	UE_API void ClearHighlight(bool bClearForUnwrap = true, bool bClearForAppliedPreview = true);

	/**
	 * Build up a highlight of the current selection in the Unwrap, with a given transform
	 * considered as its start transform.
	 */
	UE_API void RebuildUnwrapHighlight(const FTransform& StartTransform);
	 ...
	 */
	UE_API void SetUnwrapHighlightTransform(const FTransform& NewTransform);

	UE_API FTransform GetUnwrapHighlightTransform();

	UE_API void RebuildAppliedPreviewHighlight();
	 ...
	 */
	UE_API bool EndChangeAndEmitIfModified(bool bBroadcast);

	// Initialization functions:
	UE_API void Initialize(UInteractiveToolManager* ToolManagerIn, UWorld* UnwrapWorld,
	...
	UE_API virtual void OnToolEnded(UInteractiveTool* DeadTool) override;
	...
	UE_API void SetLivePreviewSelectionUXSettings(const FLivePreviewSelectionUXSettings& Settings) const;

...
public:
	...
	UPROPERTY(EditAnywhere, Category = "AdvancedTransform|Scaling", DuplicateTransient, meta = (TransientToolProperty, DisplayName = "Scale"))
	FVector2D Scale = FVector2D(1.0, 1.0); 

	/** Rotation applied to UVs after scaling, specified in degrees */
	UPROPERTY(EditAnywhere, Category = "AdvancedTransform|Rotation", meta = (TransientToolProperty, DisplayName = "Rotation", 
		                                                                        UIMin = "-360", UIMax = "360", ClampMin = "-100000", ClampMax = "100000"))
	float Rotation = 0.0;

	/** Translation applied to UVs, and after scaling and rotation */
	UPROPERTY(EditAnywhere, Category = "AdvancedTransform|Translation", meta = (TransientToolProperty, DisplayName = "Translation"))
	FVector2D Translation = FVector2D(0, 0);

	/** Translation applied to UVs, and after scaling and rotation */
	UPROPERTY(EditAnywhere, Category = "AdvancedTransform|Translation", meta = (TransientToolProperty, DisplayName = "Translation Mode"))
	...
	UPROPERTY(EditAnywhere, Category = "AdvancedTransform|TransformOrigin", meta = (TransientToolProperty, DisplayName = "Mode"))
	EUVEditorPivotType PivotMode = EUVEditorPivotType::BoundingBoxCenter;

	/** Manual Transformation origin point */
	UPROPERTY(EditAnywhere, Category = "AdvancedTransform|TransformOrigin", meta = (TransientToolProperty, DisplayName = "Coords",
...
class FUVEditorUVTransformBaseOp : public FDynamicMeshOperator
{
public:
	...
	UE_API void SetSelection(const TSet<int32>& EdgeSelection, const TSet<int32>& VertexSelection);
...
protected:
	...
	UE_API FVector2f GetAlignmentPointFromBoundingBoxAndDirection(EUVEditorAlignDirectionBackend Direction, const FAxisAlignedBox2d& BoundingBox);
	UE_API FVector2f GetAlignmentPointFromUDIMAndDirection(EUVEditorAlignDirectionBackend Direction, FVector2i UDIMTile);
	UE_API void RebuildBoundingBoxes();
	UE_API void CollectTransformElements();
	UE_API void SortComponentsByBoundingBox(bool bSortXThenY = true);
...
/**
 */
class FUVEditorUVTransformOp : public FUVEditorUVTransformBaseOp
{
public:
...
protected:
	UE_API virtual void HandleTransformationOp(FProgressCancel* Progress) override;
	UE_API FVector2f GetPivotFromMode(int32 ElementID, EUVEditorPivotTypeBackend Mode);
};

class FUVEditorUVAlignOp : public FUVEditorUVTransformBaseOp
{
public:
...
class FUVEditorUVDistributeOp : public FUVEditorUVTransformBaseOp
{
public:
 ...
 */
UCLASS(MinimalAPI)
class UUVEditorUVTransformOperatorFactory : public UObject, public UE::Geometry::IDynamicMeshOperatorFactory
{
	GENERATED_BODY()

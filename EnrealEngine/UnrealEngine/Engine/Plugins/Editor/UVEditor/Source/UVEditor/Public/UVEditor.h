...
public:
	UE_API void Initialize(const TArray<TObjectPtr<UObject>>& InObjects);
	UE_API void Initialize(const TArray<TObjectPtr<UObject>>& InObjects, const TArray<FTransform3d>& InTransforms);

	UE_API void GetWorldspaceRelativeTransforms(TArray<FTransform3d>& OutTransforms);

	/** Returns the asset editor instance interface, so that its window can be focused, for example. */
	UE_API IAssetEditorInstance* GetInstanceInterface();

	// UAssetEditor overrides
	UE_API void GetObjectsToEdit(TArray<UObject*>& OutObjects) override;

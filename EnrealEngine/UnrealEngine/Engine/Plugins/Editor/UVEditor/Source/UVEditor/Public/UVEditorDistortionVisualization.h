 ...
 */
UCLASS(MinimalAPI)
class UUVEditorDistortionVisualizationProperties : public UInteractiveToolPropertySet
{
	GENERATED_BODY()
public:
...
protected:
	...
	UE_API void ConfigureMeshColorsForTarget(int32 TargetIndex);
	UE_API FColor GetDistortionColorForTriangle(const UE::Geometry::FDynamicMesh3& Mesh, int UVChannel, int Tid, double TargetAverageTexelDensity);
	UE_API void ComputeInitialMeshSurfaceAreas();

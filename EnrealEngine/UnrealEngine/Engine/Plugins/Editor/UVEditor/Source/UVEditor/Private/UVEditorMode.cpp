...
void UUVEditorMode::InitializeTargets()
{
	...
	// Nathan - Todo: This is the start of a larger fix for the overall problem of what
	// to do if we're loading more than one object into the UV Editor that's backed by
	// the same asset. Ideally we should show as many copies of this object in the 3D preview
	// pane as we have... but this is a big change.
	// 
	// So for now, we are at least keeping track of, at this stage, which targets map to
	// which object(s) and providing correct transforms for at least one of them.
	typedef TArray<int32> FObjectToTargets;
	FObjectToTargets ObjectToTargetMap;
	...
	UVSubsystem->BuildTargets(OriginalObjectsToEdit, GetToolTargetRequirements(), ToolTargets, &ObjectToTargetMap);
	...
	for (int32 AssetID = 0; AssetID < ToolTargets.Num(); ++AssetID)
	{
		...
		{			
			int32 TargetIndex = ObjectToTargetMap.IndexOfByKey(AssetID);
			if (ensure(TargetIndex != -1))
			{
				ToolInputObject->AppliedPreview->PreviewMesh->SetTransform(Transforms[TargetIndex]);
			}
		}

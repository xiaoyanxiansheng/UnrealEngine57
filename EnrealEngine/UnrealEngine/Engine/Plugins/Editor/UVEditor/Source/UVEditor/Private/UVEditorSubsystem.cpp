...
bool UUVEditorSubsystem::IsObjectValidTarget(const TObjectPtr<UObject> InObject) const
{
	return InObject && ToolTargetManager->CanBuildTarget(InObject, UUVEditorMode::GetToolTargetRequirements());
}
...
bool UUVEditorSubsystem::IsAssetValidTarget(const FAssetData& InAsset) const
{
	...
	return IsValidStaticMeshAsset(InAsset) || IsValidSkeletalMeshAsset(InAsset);
}
...
bool UUVEditorSubsystem::AreAssetsValidTargets(const TArray<FAssetData>& InAssets) const
{
	...
	{
		if (!IsAssetValidTarget(AssetData))
		{
			return false;
		}
	}
...
{
	TargetsOut.Reset();
	if (ObjectToTargetMapOut)
	{
		ObjectToTargetMapOut->SetNum(ObjectsIn.Num());
	}

	// Map from Object to Index into Target Array
	TMap<UObject*, uint32> TargetAssets;

	for (int32 ObjectIndex = 0; ObjectIndex < ObjectsIn.Num(); ++ObjectIndex)
	{
		UObject* Object = ObjectsIn[ObjectIndex];

		if (UToolTarget* Target = ToolTargetManager->BuildTarget(Object, TargetRequirements))
		{
			// If this is an asset backed target, only add it if the asset is not already added to the list.
			if (IAssetBackedTarget* AssetTarget = Cast<IAssetBackedTarget>(Target))
			{
				UObject* SourceData = AssetTarget->GetSourceData();
				if (!TargetAssets.Contains(SourceData))
				{
					TargetsOut.Add(Target);
					TargetAssets.Add(SourceData, TargetsOut.Num()-1);
				}

				if (ObjectToTargetMapOut)
				{
					(*ObjectToTargetMapOut)[ObjectIndex] = TargetAssets[SourceData];
				}
			}
			else
			{
				...
				if (ObjectToTargetMapOut)
				{
					(*ObjectToTargetMapOut)[ObjectIndex] = (TargetsOut.Num() - 1);
				}
			}
		...
		else
		{
			if (ObjectToTargetMapOut)
			{
				(*ObjectToTargetMapOut)[ObjectIndex] = -1;
			}
		}
	}
}

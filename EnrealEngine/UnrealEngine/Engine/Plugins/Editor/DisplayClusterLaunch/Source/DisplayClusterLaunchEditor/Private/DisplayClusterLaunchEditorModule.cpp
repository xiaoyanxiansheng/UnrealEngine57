...
FString EnumToString(const FString EnumName, const int32 EnumValue)
{
	...
	const UEnum* EnumPtr = FindObject<UEnum>(nullptr, *EnumPath, EFindObjectFlags::ExactClass);
...
{		
	...
	if (PackageToSave)
	{
		...
		if (DialogueResponse != FEditorFileUtils::EPromptReturnCode::PR_Success)
		{
			return;
		}
	}
	
	// If the level was renamed (e.g. if it was Untitled), the config actor soft path needs to be updated.
	RefreshSelectedDisplayActor();
	...
	for (const FString& Node : SelectedDisplayClusterConfigActorNodes)
	{
		...
		// Other cvars
		DPCvars.Add(TEXT("LevelInstance.ForceEditorWorldMode=1"));
		DPCvars.Add(TEXT("p.Chaos.Solver.Deterministic=1"));
...
void FDisplayClusterLaunchEditorModule::RefreshSelectedDisplayActor()
{
	if (!SelectedDisplayClusterConfigActor.IsValid() || !Cast<ADisplayClusterRootActor>(SelectedDisplayClusterConfigActor.ResolveObject()))
	{
		for (const TWeakObjectPtr<ADisplayClusterRootActor>& Config : GetAllDisplayClusterConfigsInWorld())
		{

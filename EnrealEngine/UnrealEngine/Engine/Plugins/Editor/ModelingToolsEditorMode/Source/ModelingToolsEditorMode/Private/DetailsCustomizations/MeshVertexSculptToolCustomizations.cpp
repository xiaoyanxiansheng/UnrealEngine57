...
void FSculptBrushPropertiesDetails::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	// If we show falloff, it gets placed on the same line as flow/spacing. Otherwise, flow/spacing will get their own, uncustomized, lines
	if (BrushProperties->bShowFalloff && (BrushProperties->bShowSpacing || BrushProperties->bShowFlowRate))
	{
		...
		// Note: the below assumes that edit conditions are set up in such a way that spacing and flow rate are mutually exclusive,
		//  otherwise falloff (if shown) would show up twice. However, note that bShowSpacing and bShowFlowRate are NOT mutually
		//  exclusive. We customize each one, and then their individual visibility controls the visibility of our customized row. 
		if (BrushProperties->bShowSpacing)
		{
			...
			DetailBuilder.EditDefaultProperty(SpacingHandle)->CustomWidget()
		...
		if (BrushProperties->bShowFlowRate)
		{
...
FMeshSculptLayerPropertiesDetails::FMeshSculptLayerPropertiesDetails()
{
	Controller = MakeShared<FSculptLayersController>();
}

TSharedRef<IDetailCustomization> FMeshSculptLayerPropertiesDetails::MakeInstance()
{
	return MakeShareable(new FMeshSculptLayerPropertiesDetails);
}

void FMeshSculptLayerPropertiesDetails::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	TArray<TWeakObjectPtr<UObject>> ObjectsBeingCustomized;
	DetailBuilder.GetObjectsBeingCustomized(ObjectsBeingCustomized);
	check(ObjectsBeingCustomized.Num() > 0);
	UMeshSculptLayerProperties* SculptLayersProperties = Cast<UMeshSculptLayerProperties>(ObjectsBeingCustomized[0]);

	Controller->SetProperties(SculptLayersProperties);

	TSharedPtr<IPropertyHandle> ActiveLayerHandle = DetailBuilder.GetProperty(GET_MEMBER_NAME_CHECKED(UMeshSculptLayerProperties, ActiveLayer), UMeshSculptLayerProperties::StaticClass());
	ensure(ActiveLayerHandle->IsValidHandle());
	
	TSharedPtr<IPropertyHandle> LayerWeightsHandle = DetailBuilder.GetProperty(GET_MEMBER_NAME_CHECKED(UMeshSculptLayerProperties, LayerWeights), UMeshSculptLayerProperties::StaticClass());
	ensure(LayerWeightsHandle->IsValidHandle());
	LayerWeightsHandle->MarkHiddenByCustomization();
	
	DetailBuilder
	.EditDefaultProperty(ActiveLayerHandle)->CustomWidget()
	.OverrideResetToDefault(FResetToDefaultOverride::Hide())
	.WholeRowContent()
	[
...
void FVertexBrushSculptPropertiesDetails::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	TSharedPtr<SComboPanel> BrushTypeCombo = SNew(SComboPanel)
		...
		.OnSelectionChanged_Lambda([this](TSharedPtr<SComboPanel::FComboPanelItem> NewSelectedItem) {
			...
			{
				if (Tool->CanUpdateBrushType())
				{
					Tool->SetActiveBrushType(NewSelectedItem->Identifier);
				}
			}
		})

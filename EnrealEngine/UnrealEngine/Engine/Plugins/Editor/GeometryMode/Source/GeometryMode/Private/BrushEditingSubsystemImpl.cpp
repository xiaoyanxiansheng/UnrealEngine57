...
bool UBrushEditingSubsystemImpl::ProcessClickOnBrushGeometry(
	FEditorViewportClient* ViewportClient, HHitProxy* InHitProxy, const FViewportClick& Click
)
{
	bool bHandled = false;
	if (InHitProxy->IsA(HGeomPolyProxy::StaticGetType()))
	{
		...
		if (GeomHitProxy->GetGeomObject())
		{
			...
			if (!UE::Editor::ViewportSelectionUtilities::ClickActor(ViewportClient, GeomHitProxy->GetGeomObject()->ActualBrush, Click, false))
			{
	...
	else if (InHitProxy->IsA(HGeomEdgeProxy::StaticGetType()))
	{
		...
		if (GeomHitProxy->GetGeomObject() != nullptr)
		{
			...
			{
				UE::Editor::ViewportSelectionUtilities::ClickActor(ViewportClient, GeomHitProxy->GetGeomObject()->ActualBrush, Click, true);
			}
		}
...
bool UBrushEditingSubsystemImpl::ProcessClickOnGeomPoly(FEditorViewportClient* ViewportClient, HGeomPolyProxy* GeomHitProxy, const FViewportClick& Click)
{
...
bool UBrushEditingSubsystemImpl::ProcessClickOnGeomEdge(FEditorViewportClient* ViewportClient, HGeomEdgeProxy* GeomHitProxy, const FViewportClick& Click)
{
	// Pivot snapping
	if (Click.GetKey() == EKeys::MiddleMouseButton && Click.IsAltDown())
	{
...
bool UBrushEditingSubsystemImpl::ProcessClickOnGeomVertex(FEditorViewportClient* ViewportClient, HGeomVertexProxy* GeomHitProxy, const FViewportClick& Click)
{
	if (GeomHitProxy->GetGeomObject() == nullptr)
	{
		return false;

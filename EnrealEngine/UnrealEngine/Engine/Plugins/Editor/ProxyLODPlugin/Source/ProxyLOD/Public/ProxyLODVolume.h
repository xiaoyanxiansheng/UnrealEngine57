...
#define UE_API PROXYLODMESHREDUCTION_API
...
public:
	...
	struct FVector3i
	{
		...
		UE_API int32 MinIndex() const;
	};

	/** Create OpenVDB volume from input geometry */
	static UE_API TUniquePtr<IProxyLODVolume> CreateSDFVolumeFromMeshArray(const TArray<FMeshMergeData>& Geometry, float Step);
	static UE_API TUniquePtr<IProxyLODVolume> CreateSDFVolumeFromMeshArray(const TArray<FInstancedMeshMergeData>& Geometry, float Step);
...
{
public :
	static UE_API TUniquePtr<IVoxelBasedCSG> CreateCSGTool(float VoxelSize);

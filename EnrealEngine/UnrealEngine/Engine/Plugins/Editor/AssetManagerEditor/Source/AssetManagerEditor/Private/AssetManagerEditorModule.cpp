...
public:
	...
	virtual bool GetReferenceViewerUISelectedNodesAssetData(TArray<FAssetData>& OutAssetDatas) override;
	...
	virtual void AppendRegisteredCommandsToCommandList(TSharedRef<FUICommandList> ListToAppendTo) override;
	virtual FRegisterAdditionalReferenceViewerCommands& OnRegisterAdditionalReferenceViewerCommands() override { return RegisteredCommands; }
	virtual const TArray<TSharedPtr<FUICommandInfo>>& GetRegisteredAdditionalReferenceViewerCommands() override { return AdditionalCommandsInfoList; }
	virtual TSharedRef<SWidget> CreateAssetSourceControlContextMenu(const TArray<FAssetData>& SelectedAssets) override;
...
private:
	...
	/**Delegate for registering additional Commands */
	FRegisterAdditionalReferenceViewerCommands RegisteredCommands;

	TArray<TSharedPtr<FUICommandInfo>> AdditionalCommandsInfoList;
...
void FAssetManagerEditorModule::StartupModule()
{
	...
	// Load the source control module to ensure that it outlives our use of it.
	FModuleManager::Get().LoadModuleChecked<ISourceControlModule>(TEXT("SourceControl"));
...
bool FAssetManagerEditorModule::GetReferenceViewerUISelectedNodesAssetData(TArray<FAssetData>& OutAssetDatas)
{
	bool ValidAssetDataFound = false;
	for (auto TabPair : ReferenceViewerTabs)
	{
		if (TabPair.Value.IsValid())
		{
			const TSharedPtr<SDockTab> Tab = TabPair.Value.Pin();

			// we only want to do this on the actual active tab
			if (Tab->IsForeground())
			{
				TSharedPtr<SWidget> TabContent = Tab->GetContent();

				if (TabContent->GetWidgetClass().GetWidgetType() != SReferenceViewer::StaticWidgetClass().GetWidgetType())
				{
					continue;
				}

				TSharedPtr<SReferenceViewer> ActiveReferenceViewer = StaticCastSharedPtr<SReferenceViewer>(TabContent);

				TArray<FAssetData> SelectedAssets;
				ActiveReferenceViewer->GetSelectedNodeAssetData(SelectedAssets);

				// just in case we can have two valid tabs displayed at once, writing this in such a way that it should still work
				OutAssetDatas.Append(SelectedAssets);
				ValidAssetDataFound = true;
			}
		}
	}

	return ValidAssetDataFound;
}
...
void FAssetManagerEditorModule::AppendRegisteredCommandsToCommandList(TSharedRef<FUICommandList> ListToAppendTo)
{
	// we maintain a separate commands list so we can register these to the menu in the Reference Viewer Schema
	RegisteredCommands.Broadcast(ListToAppendTo, AdditionalCommandsInfoList);
}

TSharedRef<SWidget> FAssetManagerEditorModule::CreateAssetSourceControlContextMenu(const TArray<FAssetData>& SelectedAssets)
{
	UToolMenus* ToolMenus = UToolMenus::Get();
	//Find or add the tool menu extension for this case
	UToolMenu* ContextMenu = ToolMenus->ExtendMenu("GenericContextMenu.AssetSourceControl");
	
	//Find or add the section. We need at least one section in the context menu. We do not need any label since there is only one section.
	FToolMenuSection& Section = ContextMenu->FindOrAddSection("Section");

	//If we did not already add a UContentBrowserAssetContextMenuContext context object we must create one and add it.
	UContentBrowserAssetContextMenuContext* ContentBrowserAssetContextMenuContext = Section.Context.FindContext<UContentBrowserAssetContextMenuContext>();
	if(!ContentBrowserAssetContextMenuContext)
	{
		//Create a content browser asset context menu context. This is required to use the built in source control context menu.
		//The goal of this context is to pass the asset list on which we want to do source control operation.
		ContentBrowserAssetContextMenuContext = NewObject<UContentBrowserAssetContextMenuContext>();
		ContentBrowserAssetContextMenuContext->bCanBeModified = true;
		//Add the context to the section
		Section.Context.AddObject(ContentBrowserAssetContextMenuContext);
	}

	//Set the selected assets
	if(ensure(ContentBrowserAssetContextMenuContext))
	{
		ContentBrowserAssetContextMenuContext->SelectedAssets = SelectedAssets;
	}

	//Build the generic source control context menu
	AssetSourceControlContextMenu.MakeContextMenu(Section);

	//Generate the widget use to show the menu

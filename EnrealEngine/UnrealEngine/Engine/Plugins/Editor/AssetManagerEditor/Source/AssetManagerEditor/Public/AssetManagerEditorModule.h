...
public:
	...
	static UE_API const FString EditorSourceName;

	/** If SourceName matches this, this is the custom source loaded from an arbitrary path */
	static UE_API const FString CustomSourceName;
	...
	UE_API void LoadRegistryTimestamp();
...
public:
	 ...
	 */
	static UE_API TSharedRef<SWidget> MakePrimaryAssetTypeSelector(FOnGetPrimaryAssetDisplayText OnGetDisplayText, FOnSetPrimaryAssetType OnSetType, bool bAllowClear = true, bool bAllowAll = false);
	 ...
	 * @param bAllowClear If true, add None option to top
	 */
	static UE_API TSharedRef<SWidget> MakePrimaryAssetIdSelector(FOnGetPrimaryAssetDisplayText OnGetDisplayText, FOnSetPrimaryAssetId OnSetId, bool bAllowClear = true, 
	...
	static UE_API void GeneratePrimaryAssetTypeComboBoxStrings(TArray< TSharedPtr<FString> >& OutComboBoxStrings, TArray<TSharedPtr<SToolTip>>& OutToolTips, TArray<bool>& OutRestrictedItems, bool bAllowClear, bool bAllowAll);

	/** Called for asset picker to see rather to show asset */
	static UE_API bool OnShouldFilterPrimaryAsset(const FAssetData& InAssetData, TArray<FPrimaryAssetType> AllowedTypes);

	/** Creates fake AssetData to represent chunks */
	static UE_API FAssetData CreateFakeAssetDataFromChunkId(int32 ChunkID);

	/** Returns >= 0 if this is a fake chunk AssetData, otherwise INDEX_NONE */
	static UE_API int32 ExtractChunkIdFromFakeAssetData(const FAssetData& InAssetData);

	/** Creates fake AssetData to represent a primary asset */
	static UE_API FAssetData CreateFakeAssetDataFromPrimaryAssetId(const FPrimaryAssetId& PrimaryAssetId);

	/** Returns valid PrimaryAssetId if this is a fake AssetData for a primary asset */
	static UE_API FPrimaryAssetId ExtractPrimaryAssetIdFromFakeAssetData(const FAssetData& InAssetData);

	/** Parses a list of AssetData and extracts AssetIdentifiers, handles the fake asset data */
	static UE_API void ExtractAssetIdentifiersFromAssetDataList(const TArray<FAssetData>& AssetDataList, TArray<FAssetIdentifier>& OutAssetIdentifiers);

	/** Fake package path used by placeholder asset data, has primary asset ID added to it to set package name */
	static UE_API const FName PrimaryAssetFakeAssetDataPackagePath;

	/** Specific package path inside PrimaryAssetFakeAssetDataPackagePath representing chunks */
	static UE_API const FName ChunkFakeAssetDataPackageName;
	
	/** Fake type returned from type combo box to mean all types */
	static UE_API const FPrimaryAssetType AllPrimaryAssetTypes;

	/** Custom column names */
	static UE_API const FName ResourceSizeName;
	static UE_API const FName DiskSizeName;
	static UE_API const FName ManagedResourceSizeName;
	static UE_API const FName ManagedDiskSizeName;
	static UE_API const FName TotalUsageName;
	static UE_API const FName CookRuleName;
	static UE_API const FName ChunksName;
	static UE_API const FName PluginName;
	...
	virtual bool GetReferenceViewerUISelectedNodesAssetData(TArray<FAssetData>& OutAssetDatas) = 0;
	...
	/** Called by SReferenceViewer to update given list of commands with any of the registered delegates */
	virtual void AppendRegisteredCommandsToCommandList(TSharedRef<FUICommandList> ListToAppendTo) = 0;

	/** Passes commandlist and command info array to be updated by any of the registered delegates. Broadcast from AppendRegisteredCommandsToCommandList */
	DECLARE_MULTICAST_DELEGATE_TwoParams(FRegisterAdditionalReferenceViewerCommands, TSharedRef<FUICommandList>, TArray<TSharedPtr<FUICommandInfo>>&);
	virtual FRegisterAdditionalReferenceViewerCommands &OnRegisterAdditionalReferenceViewerCommands() = 0;
	...
	virtual const TArray<TSharedPtr<FUICommandInfo>>& GetRegisteredAdditionalReferenceViewerCommands() = 0;
	
	/* Return a UToolsMenu generated widget containing the asset source control context menu. */
	virtual TSharedRef<SWidget> CreateAssetSourceControlContextMenu(const TArray<FAssetData>& SelectedAssets) = 0;
};

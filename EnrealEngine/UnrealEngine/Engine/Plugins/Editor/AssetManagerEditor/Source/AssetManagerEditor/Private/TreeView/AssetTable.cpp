...
{
	TSet<int32> PluginAssets = InPlugin.GetPluginAssets();
	for (int32 AssetIndex : PluginAssets)
	{
		Callback(AssetIndex);
	}
}
...
void FAssetTable::PopulateAssetsInPlugins()
{
	for (int32 AssetIndex = 0 ; AssetIndex < Assets.Num() ; AssetIndex++)
	{
		FAssetTablePluginInfo& PluginInfo = GetOrCreatePluginInfo(Assets[AssetIndex].GetPluginName());
		PluginInfo.AddPluginAsset(AssetIndex);
	}
}
...
void FAssetTablePluginInfo::ComputeDependencySizes(const FAssetTable& OwningTable) const
{
	...
	{
		int32 CurrentIndex = DependencyStack.Pop();
		AllDependencies.Add(CurrentIndex, &bAlreadyExists);
		if(!bAlreadyExists)
		{
	...
	TArray<int32> SharedDependencyStack;
	for (int32 DependencyIndex : AllDependencies)
	{
		...
		if (IsShared)
		{
			SharedDependencyStack.Add(DependencyIndex);
		}
	}
	...
	while (SharedDependencyStack.Num() > 0)
	{
		...
		SharedDependencies.Add(CurrentIndex, &bAlreadyExists);
		if (!bAlreadyExists)
		{

...
void FAssetSourceControlContextMenuState::ExecuteSCCRefresh() const
{
	...
	TSharedPtr<FUpdateStatus, ESPMode::ThreadSafe> Operation = ISourceControlOperation::Create<FUpdateStatus>();
	Operation->SetForceUpdate(true);

	ISourceControlModule::Get().GetProvider().Execute(Operation.ToSharedRef(), SourceControlHelpers::PackageFilenames(PackageNames), EConcurrency::Asynchronous);
}
...
void FAssetSourceControlContextMenuState::NextAsyncState()
{
	...
	case EAsyncState::None:
		...
		{
			Async(EAsyncExecution::TaskGraphMainThread, [this, This = AsShared()]()
				{
...
void FAssetSourceControlContextMenuState::TryCacheCanExecuteVars(const TArray<FString>& InSelectedPaths, TArray<FString>* OptionalPathsWithUnknownState)
{
	...
	// Check the SCC state for each package in the selected paths - this needs to happen on the main thread

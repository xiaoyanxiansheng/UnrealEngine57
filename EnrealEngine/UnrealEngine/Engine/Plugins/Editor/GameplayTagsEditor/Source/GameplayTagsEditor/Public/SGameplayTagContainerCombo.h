...
public:
	...
	DECLARE_DELEGATE(FOnTagCleared)
	DECLARE_DELEGATE_OneParam(FOnTagContainerComboClosed, const FGameplayTagContainer& /*TagContainer*/)
	...
	{}
		...
		// Hide Filter from chip display
		SLATE_ARGUMENT(bool, DisplayShortTagNames)
		...
		// Called when the tag container combo selection is closed
		SLATE_EVENT(FOnTagCleared, OnTagCleared)

		// Called when the tag container combo selection is closed
		SLATE_EVENT(FOnTagContainerComboClosed, OnTagContainerComboClosed)
...
private:
	...
	bool bDisplayShortTagNames = false;
	FOnTagCleared OnTagCleared;
	FOnTagContainerComboClosed OnTagContainerComboClosed;

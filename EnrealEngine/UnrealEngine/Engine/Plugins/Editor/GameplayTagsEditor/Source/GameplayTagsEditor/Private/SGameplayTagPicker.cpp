	...
	TSharedRef<SWidget> Picker = 
		...
		[
			...
			[
				...
				.RestrictedTags(bRestrictedTags)
...
TSharedRef<ITableRow> SGameplayTagPicker::OnGenerateRow(TSharedPtr<FGameplayTagNode> InItem, const TSharedRef<STableViewBase>& OwnerTable)
{
	...
	if (InItem.IsValid())
	{
		...
		TooltipText = FText::FromString(MoveTemp(TooltipString));
	}
...
bool SGameplayTagPicker::IsNodeSingleRoot(const TSharedPtr<FGameplayTagNode>& InNode) const
{
...
TSharedRef<SWidget> SGameplayTagPicker::MakeTagActionsMenu(TSharedPtr<FGameplayTagNode> InTagNode, TSharedPtr<SComboButton> ActionsCombo, bool bInShouldCloseWindowAfterMenuSelection)
{
	...
	if (FEditorDelegates::OnOpenReferenceViewer.IsBound())
	{
		MenuBuilder.AddMenuEntry(LOCTEXT("GameplayTagPicker_SearchForReferencesAsChildren", "Search For References (all children)"),
			...
			FSlateIcon(FAppStyle::GetAppStyleSetName(), "Icons.Search"),
			FUIAction(FExecuteAction::CreateSP(this, &SGameplayTagPicker::OnSearchForReferencesAsChildren, InTagNode, ActionsCombo)));
	}
...
void SGameplayTagPicker::OnSearchForReferencesAsChildren(TSharedPtr<FGameplayTagNode> InTagNode, TSharedPtr<SComboButton> OwnerCombo)
{
	if (InTagNode.IsValid())
	{
		UGameplayTagsManager& TagManager = UGameplayTagsManager::Get();
		UScriptStruct* GameplayTagStruct = FGameplayTag::StaticStruct();

		TArray<FAssetIdentifier> AssetIdentifiers;
		AssetIdentifiers.Add(FAssetIdentifier(GameplayTagStruct, InTagNode->GetCompleteTagName()));

		FGameplayTagContainer TagContainer = TagManager.RequestGameplayTagChildrenInDictionary(InTagNode->GetCompleteTag());
		for (FGameplayTag ChildTag : TagContainer.GetGameplayTagArray())
		{
			FAssetIdentifier ChildAssetIdentifier(GameplayTagStruct, ChildTag.GetTagName());
			AssetIdentifiers.Add(ChildAssetIdentifier);
		}
...
void SGameplayTagPicker::SetDefaultTagNodeItemExpansion(TSharedPtr<FGameplayTagNode> Node)
{
	...
	if (Node.IsValid() && TagTreeWidget.IsValid())
	{
		const bool bIsExpanded = CachedExpandedItems.Contains(Node) || IsTagChecked(Node) == ECheckBoxState::Checked || IsNodeSingleRoot(Node);
...
void SGameplayTagPicker::LoadTagNodeItemExpansion(TSharedPtr<FGameplayTagNode> Node)
{
	...
	if (Node.IsValid() && TagTreeWidget.IsValid())
	{
		...
		else if (GConfig->GetBool(*SettingsIniSection, *(SettingsName + Node->GetCompleteTagString() + TEXT(".Expanded")), bIsExpanded, GetGameplayTagsEditorStateIni()))
		{
			TagTreeWidget->SetItemExpansion(Node, bIsExpanded);
			if (bIsExpanded)
			{
...
TSharedRef<SGameplayTagPicker> Create(const FGameplayTagManagerWindowArgs& Args)
{
...
TWeakPtr<SGameplayTagPicker> OpenGameplayTagManager(const FGameplayTagManagerWindowArgs& Args)
{
		...
		|| !GlobalTagWidget.IsValid())
	{
		...
		TagWidget = Create(Args);

		FText Title = Args.Title;
		if (Title.IsEmpty())
		{
			if (Args.bRestrictedTags)
			{
				Title = LOCTEXT("GameplayTagPicker_RestrictedTitle", "Restricted Gameplay Tags");
			}
			else
			{

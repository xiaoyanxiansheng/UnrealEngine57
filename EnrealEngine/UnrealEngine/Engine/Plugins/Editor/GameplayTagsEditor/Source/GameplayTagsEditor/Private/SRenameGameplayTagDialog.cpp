	...
	[
		...
		[
			...
			[
				...
				.Font(FAppStyle::GetFontStyle( TEXT("PropertyWindow.NormalFont")))
				.Text(FText::FromName(GameplayTagNode->GetSimpleTagName()))
...
void SRenameGameplayTagDialog::RenameAndClose()
{
	...
	const FString NewTagName = GameplayTagNode->GetParentTagNode().IsValid() ? GameplayTagNode->GetParentTagNode()->GetCompleteTag().GetTagName().ToString() + '.' + NewTagNameTextBox->GetText().ToString() : NewTagNameTextBox->GetText().ToString();

	if (Module.RenameTagInINI(TagToRename, NewTagName, true))
	{

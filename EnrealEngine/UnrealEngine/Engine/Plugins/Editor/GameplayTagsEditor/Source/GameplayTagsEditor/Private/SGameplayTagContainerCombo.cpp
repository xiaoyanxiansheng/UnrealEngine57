...
void SGameplayTagContainerCombo::Construct(const FArguments& InArgs)
{
	...
	bDisplayShortTagNames = InArgs._DisplayShortTagNames;
	OnTagCleared = InArgs._OnTagCleared;
	OnTagContainerComboClosed = InArgs._OnTagContainerComboClosed;
	...
	if (PropertyHandle.IsValid())
	{
		...
		bDisplayShortTagNames |= PropertyHandle->GetBoolMetaData("DisplayShortTagNames");
	}
	else
	{
...
{
	FString TagName = Item->Tag.GetTagName().ToString();
	if (bDisplayShortTagNames)
	{
		if (TagName.Len() > Filter.Len())
		{
			TagName.RemoveFromStart(Filter + TEXT("."));
		}
	}
...
void SGameplayTagContainerCombo::OnMenuOpenChanged(const bool bOpen)
{
	...
	{
		OnTagContainerComboClosed.ExecuteIfBound(CachedTagContainers[0]);
	}

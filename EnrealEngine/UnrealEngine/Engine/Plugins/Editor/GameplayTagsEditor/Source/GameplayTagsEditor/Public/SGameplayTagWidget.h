...
public:
	 ...
	 */
	static UE_API bool EnumerateEditableTagContainersFromPropertyHandle(const TSharedRef<IPropertyHandle>& PropHandle, TFunctionRef<bool(const FEditableGameplayTagContainerDatum&)> Callback);
	 ...
	 */
	static UE_API bool GetEditableTagContainersFromPropertyHandle(const TSharedRef<IPropertyHandle>& PropHandle, TArray<FEditableGameplayTagContainerDatum>& OutEditableContainers);

	/** Construct the actual widget */
	UE_API void Construct(const FArguments& InArgs, const TArray<FEditableGameplayTagContainerDatum>& EditableTagContainers);
	...
	UE_API bool FilterChildrenCheck( TSharedPtr<FGameplayTagNode>  );

	/** Returns true if we're currently adding a new tag to an INI file */
	UE_API bool IsAddingNewTag() const;
...
private:
	 ...
	 */
	UE_API void SetTagTreeItemExpansion(bool bExpand);
	 ...
	 */
	UE_API void SetTagNodeItemExpansion(TSharedPtr<FGameplayTagNode> Node, bool bExpand);
	...
	UE_API EVisibility DetermineAddNewSourceExpandableUIVisibility() const;

	/** Helper function to determine the visibility of the Add New Tag widget */
	UE_API EVisibility DetermineAddNewTagWidgetVisibility() const;

	/** Helper function to determine the visibility of the Add New Tag Source widget */
	UE_API EVisibility DetermineAddNewSourceWidgetVisibility() const;

	/** Helper function to determine the visibility of the tag tree controls. */
	UE_API EVisibility DetermineTagTreeControlsVisibility() const;

	/** Helper function to determine the visibility of the Add New Subtag widget */
	UE_API EVisibility DetermineAddNewSubTagWidgetVisibility(TSharedPtr<FGameplayTagNode> Node) const;

	/** Helper function to determine the visibility of the Clear Selection button */
	UE_API EVisibility DetermineClearSelectionVisibility() const;
	...
	UE_API FReply OnAddSubtagClicked(TSharedPtr<FGameplayTagNode> InTagNode);

	/** Creates a dropdown menu to provide additional functionality for tags (renaming, deletion, search for references, etc.) */
	UE_API TSharedRef<SWidget> MakeTagActionsMenu(TSharedPtr<FGameplayTagNode> InTagNode);

	/** Attempts to rename the tag through a dialog box */
	UE_API void OnRenameTag(TSharedPtr<FGameplayTagNode> InTagNode);

	/** Attempts to delete the specified tag */
	UE_API void OnDeleteTag(TSharedPtr<FGameplayTagNode> InTagNode);

	/** Attempts to add the exact specified tag*/
	UE_API void OnAddTag(TSharedPtr<FGameplayTagNode> InTagNode);

	/** Attempts to remove the specified tag, but not the children */
	UE_API void OnRemoveTag(TSharedPtr<FGameplayTagNode> InTagNode);

	/** Searches for all references for the selected tag */
	UE_API void OnSearchForReferences(TSharedPtr<FGameplayTagNode> InTagNode);
	...
	UE_API ECheckBoxState GetAddTagSectionExpansionState() const;

	/** Callback for when the state of the expandable UI section changes */
	UE_API void OnAddTagSectionExpansionStateChanged(ECheckBoxState NewState);

	/** Determines if the expandable UI that contains the Add New Tag Source widget should be expanded or collapsed */
	UE_API ECheckBoxState GetAddSourceSectionExpansionState() const;

	/** Callback for when the state of the expandable tag source UI section changes */
	UE_API void OnAddSourceSectionExpansionStateChanged(ECheckBoxState NewState);

	UE_API void SetContainer(FGameplayTagContainer* OriginalContainer, FGameplayTagContainer* EditedContainer, UObject* OwnerObj);

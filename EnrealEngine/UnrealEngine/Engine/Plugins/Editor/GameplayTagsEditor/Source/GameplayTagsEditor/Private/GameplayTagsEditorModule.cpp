...
public:
	...
	virtual bool RenameTagInINI(const FString& TagToRename, const FString& TagToRenameTo, bool bRenameChildren) override
	{
		...
		if (bRenameChildren)
		{
			FGameplayTag ActualTag = Manager.RequestGameplayTag(OldTagName);
			const FGameplayTagContainer Children = Manager.RequestGameplayTagDirectDescendantsInDictionary(ActualTag, EGameplayTagSelectionType::All);

			for (const FGameplayTag& ChildTag : Children.GetGameplayTagArray())
			{
				FString NewTag = ChildTag.ToString().Replace(*(TagToRename + '.'), *(TagToRenameTo + '.'), ESearchCase::CaseSensitive);
				RenameTagInINI(ChildTag.ToString(), NewTag, true);
			}
		}
		...
		UGameplayTagsList* ListToUpdate = (OldTagSource && OldTagSource->SourceTagList) ? OldTagSource->SourceTagList.Get() : GetMutableDefault<UGameplayTagsSettings>();
		check(ListToUpdate);

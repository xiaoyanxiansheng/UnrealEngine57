...
UCLASS(MinimalAPI, config=Game, defaultconfig, meta=(DisplayName="Asset Referencing Policy"))
...
public:
	...
	static UE_API const FString EngineDomainName;
	static UE_API const FString ScriptDomainName;
	static UE_API const FString GameDomainName;
	static UE_API const FString TempDomainName;
	static UE_API const FString NeverCookDomainName;
...
public:
	...
	UE_API UAssetReferencingPolicySettings(const FObjectInitializer& ObjectInitializer);

private:
	UE_API TArray<FString> GetListOfDomains(bool bAllowPlugins, bool bAllowEngine, bool bAllowGame) const;

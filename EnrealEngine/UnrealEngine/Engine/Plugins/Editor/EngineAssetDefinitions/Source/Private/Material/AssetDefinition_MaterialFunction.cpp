...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AssetDefinition_MaterialFunction)
...
EAssetCommandResult UAssetDefinition_MaterialFunction::PerformAssetDiff(const FAssetDiffArgs& DiffArgs) const
{
	UMaterialFunction* OldMaterialFunction = Cast<UMaterialFunction>(DiffArgs.OldAsset);
	UMaterialFunction* NewMaterialFunction = Cast<UMaterialFunction>(DiffArgs.NewAsset);

	if (OldMaterialFunction && NewMaterialFunction)
	{
		...
		OldMaterialGraph->MaterialFunction = OldMaterialFunction;
		OldMaterialGraph->Material = NewObject<UMaterial>();
		{
			...
			OldMaterialGraph->Material->Serialize(DummyArchive);
		}
		OldMaterialGraph->Material->SetShadingModel(MSM_Unlit);
		OldMaterialGraph->Material->AssignExpressionCollection(OldMaterialGraph->MaterialFunction->GetExpressionCollection());

		NewMaterialGraph->MaterialFunction = NewMaterialFunction;
		NewMaterialGraph->Material = NewObject<UMaterial>();
		{
			FArchiveUObject DummyArchive;
			// Hack: serialize the new material with an archive that does nothing so that its material resources are created
			NewMaterialGraph->Material->Serialize(DummyArchive);
		}
		NewMaterialGraph->Material->SetShadingModel(MSM_Unlit);
		NewMaterialGraph->Material->AssignExpressionCollection(NewMaterialGraph->MaterialFunction->GetExpressionCollection());

...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AssetDefinition_StaticMesh)
...
namespace MenuExtension_StaticMesh
{
	...
	void ModifyNaniteEnable(const TArray<UStaticMesh*>& Objects, bool bNaniteEnable)
	{
		...
		{
			if (Mesh && Mesh->GetNaniteSettings().bEnabled != bNaniteEnable)
			{
				Meshes.Add(Mesh);
			}
		}
		...
		{
			Mesh->GetNaniteSettings().bEnabled = bNaniteEnable;
		}
	...
	static void ExecutePasteLODSettings(const FToolMenuContext& InContext)
	{
		...
		LODSettings.AddZeroed(FMath::Min(LODCopyMesh->GetNumSourceModels(), MAX_STATIC_MESH_LODS));
		...
		const bool bAutoComputeLODScreenSize = LODCopyMesh->GetAutoComputeLODScreenSize();
		...
		for (UStaticMesh* Mesh : Context->LoadSelectedObjects<UStaticMesh>())
		{
			...
			Mesh->SetAutoComputeLODScreenSize(bAutoComputeLODScreenSize);

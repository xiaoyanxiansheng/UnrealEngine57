...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AssetDefinition_DataAsset)
...
namespace MenuExtension_DataAsset
{
	...
	public:
		...
		virtual bool IsClassAllowed(const FClassViewerInitializationOptions& InInitOptions, const UClass* InClass, TSharedRef< FClassViewerFilterFuncs > InFilterFuncs) override
		{
			if (InClass != nullptr)
			{
				return InClass->IsChildOf(BaseClass) && !InClass->HasAnyClassFlags(DisallowedClassFlags);
			}
			return false;
		}
		...
		{
			return InUnloadedClassData->IsChildOf(BaseClass) && !InUnloadedClassData->HasAnyClassFlags(DisallowedClassFlags);
		}

...
{
	FEditorModeTools& ModeManager = GLevelEditorModeTools(); //mz need to fix for persona/ animation edit mode
	/* This should work but doesn't. It affects the 'highlighting' correctly but it doesn't render correctly
	#jira UE-306751
	const double Scale = (double)ModeManager.GetWidgetScale();
	const FVector3d ScaleVector(Scale, Scale, Scale);
	FTransform GizmoTransformWithScale(GizmoTransform);
	GizmoTransformWithScale.SetScale3D(ScaleVector);
	*/
	if (TransformGizmo)
	{
		//TransformGizmo->ReinitializeGizmoTransform(GizmoTransformWithScale);
...
void USequencerPivotTool::Render(IToolsContextRenderAPI* RenderAPI)
{
	...
	if (bPickingPivotLocation == false )
	{
		...
		if (RenderAPI->GetSceneView())
		{
			const bool bIsPerspective = (RenderAPI->GetSceneView()->ViewMatrices.GetProjectionMatrix().M[3][3] < 1.0f) ? true : false;
			if (bIsPerspective)
			{
				const float ZoomFactor = FMath::Min<float>(RenderAPI->GetSceneView()->ViewMatrices.GetProjectionMatrix().M[0][0], RenderAPI->GetSceneView()->ViewMatrices.GetProjectionMatrix().M[1][1]);

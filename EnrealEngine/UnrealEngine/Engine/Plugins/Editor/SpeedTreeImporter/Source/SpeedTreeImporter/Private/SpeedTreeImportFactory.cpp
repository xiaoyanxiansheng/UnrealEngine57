...
#include UE_INLINE_GENERATED_CPP_BY_NAME(SpeedTreeImportFactory)
	...
	: Super(ObjectInitializer)
{
	...
	static ConstructorHelpers::FObjectFinder<UMaterialInterface> TemplateMaterialFinder(TEXT("/SpeedTreeImporter/SpeedTree9/SpeedTreeMaster"));
	TemplateMaterial = TemplateMaterialFinder.Object;
	static ConstructorHelpers::FObjectFinder<UMaterialInterface> TemplateBillboardMaterialFinder(TEXT("/SpeedTreeImporter/SpeedTree9/SpeedTreeBillboardMaster"));
	TemplateBillboardMaterial = TemplateBillboardMaterialFinder.Object;
}
...
UMaterialInterface* CreateSpeedTreeMaterial9(UObject* Parent, FString MaterialFullName, GameEngine9::CMaterial& SpeedTreeMaterial, UMaterialInterface* TemplateMaterial, GameEngine9::CWindConfigSDK Wind, bool bHandleCameraFacing, int32 BillboardCount, TSet<UPackage*>& LoadedPackages, FSpeedTreeImportContext& ImportContext)
{
	// Make sure we have a parent
	if ((TemplateMaterial == NULL) || !ensure(Parent))
	{
		return UMaterial::GetDefaultMaterial(MD_Surface);
	}
	...
	UnrealMaterialInstance->SetParentEditorOnly(TemplateMaterial);
...
UObject* USpeedTreeImportFactory::FactoryCreateBinary7(UClass* InClass, UObject* InParent, FName InName, EObjectFlags Flags, UObject* Context, const TCHAR* Type, const uint8*& Buffer, const uint8* BufferEnd, FFeedbackContext* Warn, bool& bOutOperationCanceled)
{
	...
	if (ExistingMesh)
	{
		...
		ExistingImportData = Cast<USpeedTreeImportData>(ExistingMesh->GetAssetImportData());
	}
	...
	if ( !bOutOperationCanceled )
	{
		...
		else
		{
			...
			else
			{
				...
				if (StaticMesh->GetAssetImportData() == nullptr || !StaticMesh->GetAssetImportData()->IsA(USpeedTreeImportData::StaticClass()))
				{
					StaticMesh->SetAssetImportData(NewObject<USpeedTreeImportData>(StaticMesh, NAME_None));
				}

				check(SpeedTreeImportData);
				StaticMesh->GetAssetImportData()->Update(UFactory::GetCurrentFilename());
				Cast<USpeedTreeImportData>(StaticMesh->GetAssetImportData())->CopyFrom(SpeedTreeImportData);
				...
				StaticMesh->SetRequiresLODDistanceConversion(false);
			}
		}
	}
...
UObject* USpeedTreeImportFactory::FactoryCreateBinary8(UClass* InClass, UObject* InParent, FName InName, EObjectFlags Flags, UObject* Context, const TCHAR* Type, const uint8*& Buffer, const uint8* BufferEnd, FFeedbackContext* Warn, bool& bOutOperationCanceled)
{
	...
	else
	{
		...
		{
			//Grab the existing asset data to fill correctly the option with the original import value
			SpeedTreeImportData = Cast<USpeedTreeImportData>(ExistingMesh->GetAssetImportData());
		}
	...
	else
	{
		...
		for (uint32 LODIndex = 0; LODIndex < SpeedTree.Lods().Count(); ++LODIndex)
		{
			...
			for (uint32 DrawCallIndex = 0; DrawCallIndex < LOD.DrawCalls().Count(); ++DrawCallIndex)
			{
				...
				{
					// you could pick different template materials based on SpeedTree.TexturePacker()
					bool bBillboard = (SpeedTree.BillboardInfo().LastLodIsBillboard() && (LODIndex == (SpeedTree.Lods().Count() - 1)));
					UMaterialInterface* Template = (bBillboard ? TemplateBillboardMaterial : TemplateMaterial);

					GameEngine9::CMaterial SpeedTreeMaterial = SpeedTree.Materials()[DrawCall.m_uiMaterialIndex];
					FString MaterialName = FString(SpeedTreeMaterial.Name().Data());
					UMaterialInterface* Material = CreateSpeedTreeMaterial9(InParent, MaterialName, SpeedTreeMaterial, Template, SpeedTree.Wind(), bHasFacingData, SpeedTree.BillboardInfo().SideViewCount(), LoadedPackages, ImportContext);

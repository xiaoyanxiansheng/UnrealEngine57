...
#include UE_INLINE_GENERATED_CPP_BY_NAME(ReimportSpeedTreeFactory)
...
bool UReimportSpeedTreeFactory::CanReimport(UObject* Obj, TArray<FString>& OutFilenames)
{
	...
	{
		const FString FileExtension = FPaths::GetExtension(Mesh->GetAssetImportData()->GetFirstFilename());
		...
		if (bIsSpeedTree)
		{
			...
			Mesh->GetAssetImportData()->ExtractFilenames(OutFilenames);
			return true;
		}		
	}
...
EReimportResult::Type UReimportSpeedTreeFactory::Reimport(UObject* Obj)
{
	...
	const FString Filename = Mesh->GetAssetImportData()->GetFirstFilename();

...
void UWorldPartitionChangelistValidator::ValidateActorsAndDataLayersFromChangeList(UDataValidationChangelist* Changelist)
{
	...
	for (TTuple<FTopLevelAssetPath, TSet<FAssetData>>& It : MapToActorsFiles)
	{
		...
		UWorld* World = FindObject<UWorld>(nullptr, *MapPath.ToString(), EFindObjectFlags::ExactClass);
...
void UWorldPartitionChangelistValidator::OnUnsupportedHLODLayer(const IWorldPartitionActorDescInstanceView& ActorDescView, const UHLODLayer* HLODLayer, EHLODLayerUnsupportedReason Reason)
{
	if (Filter(ActorDescView))
	{
		...
		case EHLODLayerUnsupportedReason::ActorClassUnsupportedByLayerType:
			CurrentWarning = FText::Format(LOCTEXT("DataValidation.Changelist.WorldPartition.UnsupportedHLODLayer", "Actor {0} of class '{1}' is unsupported by HLOD Layer {2} of type '{3}'"),
				FText::FromString(GetFullActorName(ActorDescView)),
				FText::FromString(ActorDescView.GetActorNativeClass()->GetName()),
				FText::FromString(ActorDescView.GetHLODLayer().ToString()),
				LayerTypeText);
			break;
		};

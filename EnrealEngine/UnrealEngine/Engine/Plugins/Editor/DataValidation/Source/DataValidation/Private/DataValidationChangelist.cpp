...
void UDataValidationChangelist::GatherDependencies(const FName& InPackageName, TSet<TPair<FName, FName>>& OutDependencies)
{
	OutDependencies.Add({ InPackageName, InPackageName });
	...
	for (const FName& PackageDependency : Dependencies)
	{
		...
		{
			OutDependencies.Add({ InPackageName, PackageDependency });
		}
	}
}
...
EDataValidationResult UDataValidationChangelist::IsDataValid(FDataValidationContext& Context) const
{
	...
	TArray<TPair<FName, FName>> ExternalDependencies = ExternalDependenciesSet.Array();

	ExternalDependencies.RemoveAll([&FilesInChangelist](TPair<FName, FName>& FilePair) -> bool {
		return Algo::AnyOf(FilesInChangelist, [&FilePair](const auto& FileInChangelist) {
			return FilePair.Value == FileInChangelist;
			});
		});
	...
	Algo::Transform(ExternalDependencies, ExternalDependenciesFilenames, [](const TPair<FName, FName>& InFilenamePair) -> FString
	{
		return USourceControlHelpers::PackageFilename(InFilenamePair.Value.ToString());
	});
	...
	for (int32 i = 0; i < ExternalDependenciesFilenames.Num(); ++i)
	{
		...
		const TPair<FName, FName>& ExternalDependencyPair = ExternalDependencies[i];
		...
		if (FPackageName::IsVersePackage(ExternalDependencyPair.Value.ToString()))
		{
			continue;
		}

		auto GetReferencerText = [&ExternalDependencyPair]() { return FText::FromString(GetPrettyPackageName(ExternalDependencyPair.Key)); };
		auto GetReferenceText = [&ExternalDependencyPair]() { return FText::FromString(GetPrettyPackageName(ExternalDependencyPair.Value)); };
		auto GetExternalPackageFilenameText = [&ExternalPackageFilename]() { return FText::FromString(ExternalPackageFilename); };
		...
		if (ExternalDependencyFileState->IsCheckedOut() || ExternalDependencyFileState->IsAdded())
		{
			...
			FText CurrentError = FText::Format(LOCTEXT("DataValidation.Changelist.Error", "{0} is missing from this changelist (referenced from {1})."), GetReferenceText(), GetReferencerText());
		...
		else if (!ExternalDependencyFileState->IsCurrent())
		{
			...
			{
				FText CurrentWarning = FText::Format(LOCTEXT("DataValidation.Changelist.NotLatest", "{0} is referenced but is not at the latest revision '{1}' (referenced from {2})"), GetReferenceText(), GetExternalPackageFilenameText(), GetReferencerText());
		...
		else if (ExternalDependencyFileState->CanAdd())
		{
			...
			{
				bHasChangelistErrors = true;
				FText CurrentError = FText::Format(LOCTEXT("DataValidation.Changelist.NotInWorkspace", "{0} is referenced and cannot be found in workspace '{1}' (referenced from {2})"), GetReferenceText(), GetExternalPackageFilenameText(), GetReferencerText());
				Context.AddError(CurrentError);
			}
			else
			{
				bHasChangelistErrors = true;
				FText CurrentError = FText::Format(LOCTEXT("DataValidation.Changelist.NotInDepot", "{0} is referenced and must also be added to revision control '{1}' (referenced from {2})"), GetReferenceText(), GetExternalPackageFilenameText(), GetReferencerText());
				Context.AddError(CurrentError);
			}
		}
	}

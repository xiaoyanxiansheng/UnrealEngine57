...
{
	FName FilterGroupLayerName = { PreFilterSettings.FilterGroupLayerName };
	UpdateSourceNodeValue<FNameSourceNode>(*Graph, FilterGroupsLayerNameNode, FilterGroupLayerName);
...
{
	// does nothing if all the node types are already registered.
	{
		UE::GeometryFlow::FCoreNodeRegistration::RegisterNodes();
		UE::GeometryFlow::FMeshProcessingNodeRegistration::RegisterNodes();
	...
	SimplifySettings.SimplifyType = ToUEnum(EMeshSimplifyType::AttributeAware);
	//SimplifySettings.TargetType = EMeshSimplifyTargetType::TriangleCount;
	//SimplifySettings.TargetCount = 500;
	SimplifySettings.TargetType = ToUEnum(EMeshSimplifyTargetType::GeometricDeviation);
	SimplifySettings.GeometricTolerance = 0.5;
	SimplifySettings.TargetCount = 500;
	SimplifySettings.MaterialBorderConstraints = ToUEnum(EEdgeRefineFlags::NoFlip);
	UpdateSimplifySettings(SimplifySettings);

	FMeshNormalsSettings NormalsSettings;
	NormalsSettings.NormalsType = ToUEnum(EComputeNormalsType::FromFaceAngleThreshold);
	NormalsSettings.AngleThresholdDeg = 60.0;
	UpdateNormalsSettings(NormalsSettings);

	FMeshAutoGenerateUVsSettings AutoUVSettings;
	AutoUVSettings.Method = ToUEnum(EAutoUVMethod::PatchBuilder);
	...
	BakeCacheSettings.Dimensions = FIntPoint(1024, 1024);
	...
	GenSimpleCollisionSettings.Type = EGeometryFlow_SimpleCollisionGeometryType::AlignedBoxes;

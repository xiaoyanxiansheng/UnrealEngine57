...
protected:
	...
	/**
	 * When true, texture alpha channel will be used when applying the weight patch. Note that the source data needs to
	 * have an alpha channel for this to have an effect.
	 */
	UPROPERTY(EditAnywhere, Category = WeightPatch)
	bool bUseAlphaChannel = false;

	/** Allows to override the patch component's own blend node */

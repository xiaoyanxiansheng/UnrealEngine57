...
{
	if (InAssetData.AssetClassPath == UVerseClass::VerseClassTopLevelAssetPath)
	{
		// We should not be showing generic compiled Verse classes, only `creative_device` subclasses and other types that 
		// can be drag-and-dropped into the level. We create aliases to display these elsewhere (ie: See FVerseClassAlias)
		return false;
	}
...
{
	if (const UVerseClass* VerseClass = Cast<UVerseClass>(InObject))
	{
		// We should not be showing generic compiled Verse classes, only `creative_device` subclasses and other types that 
		// can be drag-and-dropped into the level. We create aliases to display these elsewhere (ie: See FVerseClassAlias)
		return false;
...
bool DeleteItems(FContentBrowserModule* InContentBrowserModule, IAssetTools* InAssetTools, IAssetRegistry* InAssetRegistry, const UContentBrowserDataSource* InOwnerDataSource, TArrayView<const FContentBrowserItemData> InItems)
{
	...
	{
		bDidDelete |= DeleteAssetFolderItems(InContentBrowserModule, FolderPayloads);
	}
...
bool DeleteAssetFolderItems(FContentBrowserModule* InContentBrowserModule, TArrayView<const TSharedRef<const FContentBrowserAssetFolderItemDataPayload>> InFolderPayloads)
{
	...
	InContentBrowserModule->GetOnDeleteFolders().Broadcast(ItemPathsToDelete);
...
void AppendAssetObjectPath(const FAssetData& AssetData, FString& InOutStr)
{
	...
	AssetData.AppendObjectPath(InOutStr);
}
...
bool GetItemAttribute(FContentBrowserModule* InContentBrowserModule, const UContentBrowserDataSource* InOwnerDataSource, const FContentBrowserItemData& InItem, const bool InIncludeMetaData, const FName InAttributeKey, FContentBrowserItemDataAttributeValue& OutAttributeValue)
{
	// First see if another module hooked this to provide its own customizations. If it did, we can be done here.
	for(const FGetItemDataAttribute& Delegate : InContentBrowserModule->GetAllItemDataAttributeDelegates())
	{
		if (Delegate.Execute(InItem, InIncludeMetaData, InAttributeKey, OutAttributeValue))
		{
			return true;
		}
	}
...
bool GetAssetFileItemAttribute(const FContentBrowserAssetFileItemDataPayload& InAssetPayload, const bool InIncludeMetaData, const FName InAttributeKey, FContentBrowserItemDataAttributeValue& OutAttributeValue)
{
	...
	// Hard-coded attribute keys
	{
		if (InAttributeKey == ContentBrowserItemAttributes::ItemTypeDisplayName)
		{
			if (TSharedPtr<IAssetTypeActions> AssetTypeActions = InAssetPayload.GetAssetTypeActions())
			{
				FText AssetDisplayName = AssetTypeActions->GetDisplayNameFromAssetData(InAssetPayload.GetAssetData());
				if (!AssetDisplayName.IsEmpty())
				{
					OutAttributeValue.SetValue(MoveTemp(AssetDisplayName));
				}
				else
				{
		...
		if (InAttributeKey == ContentBrowserItemAttributes::ItemDescription)
		{
			if (const UAssetDefinition* AssetDefinition = InAssetPayload.GetAssetDefinition())
			{
				...
				FText AssetDescription = AssetDefinition->GetAssetDescription(InAssetPayload.GetAssetData());
				if (!AssetDescription.IsEmpty())
				{
					OutAttributeValue.SetValue(MoveTemp(AssetDescription));
					return true;
				}
			}
			return false;
...
bool GetAssetAccessSpecifier(const UContentBrowserDataSource* InOwnerDataSource, const FContentBrowserItemData& InItem, EAssetAccessSpecifier& OutAssetAccessSpecifier)
{
	if (TSharedPtr<const FContentBrowserAssetFileItemDataPayload> AssetPayload = GetAssetFileItemPayload(InOwnerDataSource, InItem))
	{
		OutAssetAccessSpecifier = AssetPayload->GetAssetData().GetAssetAccessSpecifier();
		return true;
	}
...
bool CanModifyAssetAccessSpecifier(IAssetTools* InAssetTools, const UContentBrowserDataSource* InOwnerDataSource, const FContentBrowserItemData& InItem)
{
	check(InAssetTools);

	if (TSharedPtr<const FContentBrowserAssetFileItemDataPayload> AssetPayload = GetAssetFileItemPayload(InOwnerDataSource, InItem))
	{
		FNameBuilder AssetPath;
		FTopLevelAssetPath(AssetPayload->GetAssetData().PackageName, AssetPayload->GetAssetData().AssetName).AppendString(AssetPath);

		return InAssetTools->CanAssetBePublic(AssetPath);
	}

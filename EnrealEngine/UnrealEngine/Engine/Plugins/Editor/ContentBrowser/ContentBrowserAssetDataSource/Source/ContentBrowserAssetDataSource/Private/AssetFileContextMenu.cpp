...
void FAssetFileContextMenu::MakeAssetActionsSubMenu(UToolMenu* Menu)
{
	...
	// ADVANCED ACTIONS
	{
		...
		if (IsRunningHybridCookedEditor())
		{
			...
			Section.AddMenuEntry(
				"ReloadUncooked",
				LOCTEXT("ReloadUncooked", "Reload Uncooked Version"),
				LOCTEXT("ReloadUncookedTooltip", "Reload the selected assets from their uncooked file on disk."),
				FSlateIcon(),
				FUIAction(
					FExecuteAction::CreateSP(this, &FAssetFileContextMenu::ExecuteReloadUncooked),
					FCanExecuteAction::CreateSP(this, &FAssetFileContextMenu::CanExecuteReloadUncooked)
				)
			);
		}
		...
		Section.AddMenuEntry(
			"Load",
			LOCTEXT("Load", "Load"),
			LOCTEXT("LoadTooltip", "Loads the selected assets into memory without opening any asset editors.\nShift+Click to fully load the selected assets so that UE no longer has a write lock on their on-disk files."),
			FSlateIcon(),
			FUIAction(
		...
		{
			Section.AddMenuEntry(
				"CopyAssetRegistryTags",
				LOCTEXT("CopyAssetRegistryTags", "Copy Asset Registry Tags"),
				LOCTEXT("CopyAssetRegistryTagsTooltip", "Copies to clipboard all asset registry tags that belong to this asset."),
				FSlateIcon(),
				FUIAction(FExecuteAction::CreateSP(this, &FAssetFileContextMenu::ExecuteCopyAssetRegistryTags))
			);
		}
		...
		for (const FAssetData& Asset : SelectedAssets)
		{
			...
			else
			{
				...
				if (FPackageLocalizationUtil::ConvertSourceToLocalized(ObjectPath, CurrentCulture->GetName(), LocalizedObjectPath))
				{
					...
					// Need to check for both UE and Verse style identifiers, as both may be in use
					FAssetData LocalizedAssetData = IAssetRegistry::GetChecked().GetAssetByObjectPath(FSoftObjectPath(LocalizedObjectPath));
					if (!LocalizedAssetData.IsValid())
					{
						if (const FString VerseIdentifier = CurrentCulture->GetVerseIdentifier();
							VerseIdentifier != CurrentCulture->GetName())
						{
							verify(FPackageLocalizationUtil::ConvertSourceToLocalized(ObjectPath, VerseIdentifier, LocalizedObjectPath));
							LocalizedAssetData = IAssetRegistry::GetChecked().GetAssetByObjectPath(FSoftObjectPath(LocalizedObjectPath));
						}
					}
...
void FAssetFileContextMenu::ExecuteCreateLocalizedAsset(TSet<FSoftObjectPath> InSelectedSourceAssets, FLocalizedAssetsState InLocalizedAssetsStateForCulture)
{
	...
	for (const FSoftObjectPath& SourceAsset: InSelectedSourceAssets)
	{
		...
		// Always create new assets using the preferred on-disk culture identifier for this project
		FString LocalizedPackageName;
		if (!FPackageLocalizationUtil::ConvertSourceToLocalized(SourceAssetObject->GetOutermost()->GetPathName(), InLocalizedAssetsStateForCulture.Culture->GetPreferredOnDiskIdentifier(), LocalizedPackageName))
		{
			continue;
		}
...
void FAssetFileContextMenu::ExecuteCopyAssetRegistryTags()
{
	...
	for (const FAssetData& AssetData : SelectedAssets)
	{
		...
		if (SelectedAssets.Num() != 1)
		{
			AssetObject = MakeShared<FJsonObject>();
			JsonObject->SetObjectField(AssetData.PackageName.ToString(), AssetObject);
		}
		else
		{
		...
		AssetData.EnumerateTags([&AssetObject](const TPair<FName, FAssetTagValueRef>& TagPair)
		{
			AssetObject->SetStringField(TagPair.Key.ToString(), TagPair.Value.AsString());
		});
	}
	...
	FNotificationInfo Info(LOCTEXT("CopyAssetRegistryTagsNotification", "Asset Registry Tags have been copied to clipboard."));
	Info.ExpireDuration = 3.0f;
	FSlateNotificationManager::Get().AddNotification(Info);
}
...
{
	const bool bFullyLoad = FSlateApplication::Get().GetModifierKeys().IsShiftDown();
	...
	for (const FAssetData& SelectedAsset : SelectedAssets)
	{
		if (UObject* LoadedAsset = SelectedAsset.GetAsset())
		{
	...
	if (bFullyLoad)
	{
		UEditorLoadingAndSavingUtils::FullyLoadAssets(LoadedAssets);
	}
}
...
static void ProcessObjectForUncook(const UObject* Obj, TArray<UPackage*>& PackagesToReload)
{
	if (Obj == nullptr)
	{
		return;
	...
	// tell package stores to stop serving cooked packages
	FHybridPackageStoreBackend::ForceLoadPackageAsLoose(Obj->GetPackage()->GetPackageId());

	UPackage* Pkg = Obj->GetPackage();
	if (Pkg->HasAnyPackageFlags(PKG_Cooked))
	{
		PackagesToReload.AddUnique(Pkg);
	}

	// if the object needs anymore objects to also be unloaded, call this function recursively
	// here, with those objects
}

static void ReloadPackages(TArray<FAssetData>& Assets, bool bUncookPackages)
{
	...
	if (Assets.Num() > 0)
	{
		...
		for (auto AssetIt = Assets.CreateConstIterator(); AssetIt; ++AssetIt)
		{
			...
			// simple case of just reloading
			if (!bUncookPackages)
			{
				PackagesToReload.AddUnique(AssetData.GetPackage());
			}
			// complexity for uncooked a packages and it's dependencies
			else
			{
				// if the object wasn't loaded, maybe it's because it was cooked, so reload the whole package uncooked,
				// then we can try again for it's related objects
				UObject* AssetObj = AssetData.FastGetAsset();
				if (AssetObj == nullptr)
				{
					UPackageTools::ReloadPackages({ AssetData.GetPackage() } );
				}

				ProcessObjectForUncook(AssetData.FastGetAsset(), PackagesToReload);
			}
		}
...
{
	ReloadPackages(SelectedAssets, /*bUncook*/false);
}

bool FAssetFileContextMenu::CanExecuteReloadUncooked() const
{
	return SelectedAssets.Num() > 0;
}

void FAssetFileContextMenu::ExecuteReloadUncooked()
{
	ReloadPackages(SelectedAssets, /*bUncook*/true);
	OnRefreshView.ExecuteIfBound();
}

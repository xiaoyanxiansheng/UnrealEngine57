...
void UContentBrowserFileDataSource::EnumerateItemsAtUserProvidedPath(const FStringView InPath, const EContentBrowserItemTypeFilter InItemTypeFilter, TFunctionRef<bool(FContentBrowserItemData&&)> InCallback)
{
	TStringBuilder<256> PathBuilder;
	PathBuilder.Append(InPath);
	FPathViews::NormalizeDirectoryName(PathBuilder);

	const FStringView Path(PathBuilder);
	// Find the RegisteredFileMounts the Path is under.
	for (const TPair<FName, FFileMount>& RegisteredFileMount : RegisteredFileMounts)
	{
		if (Path.StartsWith(RegisteredFileMount.Value.DiskPath) &&
			(Path.Len() == RegisteredFileMount.Value.DiskPath.Len() || Path[RegisteredFileMount.Value.DiskPath.Len()] == TEXT('/')))
		{
			TStringBuilder<256> InternalPathBuilder;
			RegisteredFileMount.Key.AppendString(InternalPathBuilder);
			InternalPathBuilder.Append(FStringView(Path).RightChop(RegisteredFileMount.Value.DiskPath.Len()));

			FName InternalPath(InternalPathBuilder, FNAME_Find);
...
bool UContentBrowserFileDataSource::CanRenameItem(const FContentBrowserItemData& InItem, const FString* InNewName, const IContentBrowserHideFolderIfEmptyFilter* HideFolderIfEmptyFilter, FText* OutErrorMsg)
{
	...
	{
		ContentBrowserFileData::SetOptionalErrorMessage(OutErrorMsg, FText::Format(LOCTEXT("Error_FolderIsFileRoot", "Folder '{0}' is a root file path and cannot be renamed"), FText::FromName(InternalPath)));
		return false;
	}
	...
	{
		ContentBrowserFileData::SetOptionalErrorMessage(OutErrorMsg, FText::Format(LOCTEXT("Error_FolderIsNotFilePath", "Folder '{0}' is not a known file path"), FText::FromName(InternalPath)));
		return false;
	}

...
FName FNativeClassHierarchy::GetFolderPathForModule(FName InModuleName, TSet<FName>& InGameModuleCache) const
{
	if (InGameModuleCache.Num() == 0)
	{
		InGameModuleCache = GetGameModules();
	...
	TOptional<EPluginLoadedFrom> WhereLoadedFrom;
	const FName RootNodeName = GetClassPathRootForModule(InModuleName, InGameModuleCache, WhereLoadedFrom);

	FNameBuilder FolderPath;
	FolderPath.AppendChar(TEXT('/'));
	RootNodeName.AppendString(FolderPath);
	FolderPath.AppendChar(TEXT('/'));
	InModuleName.AppendString(FolderPath);

...
void UContentBrowserAliasDataSource::Initialize(const bool InAutoRegister)
{
	...
	if (GIsEditor && !IsRunningCommandlet())
	{
		...
		ContentBrowserModule = &FModuleManager::Get().GetModuleChecked<FContentBrowserModule>(TEXT("ContentBrowser"));
...
void UContentBrowserAliasDataSource::EnumerateItemsAtPath(const FName InPath, const EContentBrowserItemTypeFilter InItemTypeFilter, TFunctionRef<bool(FContentBrowserItemData&&)> InCallback)
{
	...
	{
		if (const FContentBrowserUniqueAlias* Alias = AliasForInternalPath.Find(InternalPath))
		{
			FContentBrowserItemData Item = CreateAssetFileItem(*Alias);
			if (ensureMsgf(Item.IsValid(), TEXT("Could not find alias data for: %s"), *InternalPath.ToString()))
			{
				InCallback(MoveTemp(Item));
			}
		}
	}
}

void UContentBrowserAliasDataSource::EnumerateItemsAtUserProvidedPath(const FStringView InPath, const EContentBrowserItemTypeFilter InItemTypeFilter, TFunctionRef<bool(FContentBrowserItemData&&)> InCallback)
{
	if (EnumHasAnyFlags(InItemTypeFilter, EContentBrowserItemTypeFilter::IncludeFiles) && !AliasesForObjectPath.IsEmpty())
	{
		auto EnumerateItemsAtObjectPath = [this, &InCallback](const FSoftObjectPath& ObjectPath)
		{
			if (ObjectPath.IsValid())
			{
				if (const TArray<FName>* FoundAliases = AliasesForObjectPath.Find(ObjectPath))
				{
					for (const FName Alias : *FoundAliases)
					{
						FContentBrowserItemData Item = CreateAssetFileItem(FContentBrowserUniqueAlias(ObjectPath, Alias));
						if (ensureMsgf(Item.IsValid(), TEXT("Could not find alias data for: %s"), *Alias.ToString()))
						{
							InCallback(MoveTemp(Item));
						}
					}
		...
		// Could be an object path.
		{
			...
			if (EnumerateItemsAtObjectPath(ObjectPath))
			{
				return;
			}
		}
		...
		if (ensure(AssetRegistry))
		{
			...
			if (!PackageName.IsNone())
			{
				...
				if (AssetRegistry->GetAssetsByPackageName(PackageName, Assets))
				{
					...
					{
						EnumerateItemsAtObjectPath(AssetData.GetSoftObjectPath());
					}
				}
			}
		}
...
TArray<FContentBrowserItemPath> UContentBrowserAliasDataSource::GetAliasesForPath(const FSoftObjectPath& ObjectPath) const
{
	...
	if (const TArray<FName>* FoundAliases = AliasesForObjectPath.Find(ObjectPath))
	{
		OutAliases.Reserve(FoundAliases->Num());

		for (const FName Alias : *FoundAliases)
		{
			if (const FAliasData* AliasData = AllAliases.Find(FContentBrowserUniqueAlias(ObjectPath, Alias)))
			{
				OutAliases.Add(FContentBrowserItemPath(AliasData->InternalPath, EContentBrowserPathType::Internal));
			}
		}
	}
...
void UContentBrowserAliasDataSource::AddAliasImpl(const FAssetData& Asset, const AliasType& AliasToAdd, const bool bInIsFromMetaData, const bool bSkipPrimaryAssetValidation)
{
	...
	FAliasData& AliasData = AllAliases.Add(UniqueAlias, FAliasData(Asset, PackagePath, AliasDisplayNameOverride.IsEmpty() ? FText::AsCultureInvariant(*Tokens.Last()) : AliasDisplayNameOverride, bInIsFromMetaData));
	AliasForInternalPath.Add(AliasData.InternalPath, UniqueAlias);
...
void UContentBrowserAliasDataSource::RemoveAlias(const FSoftObjectPath& ObjectPath, const FName Alias)
{
	...
	if (AllAliases.RemoveAndCopyValue(UniqueAlias, AliasData))
	{
		...
		AliasForInternalPath.Remove(AliasData.InternalPath);
...
{
	if (InItem.GetItemType() == EContentBrowserItemFlags::Type_File)
	{
...
bool UContentBrowserAliasDataSource::GetItemAssetAccessSpecifier(const FContentBrowserItemData& InItem, EAssetAccessSpecifier& OutAssetAccessSpecifier)
{
	return ContentBrowserAssetData::GetAssetAccessSpecifier(this, InItem, OutAssetAccessSpecifier);
}

bool UContentBrowserAliasDataSource::CanModifyItemAssetAccessSpecifier(const FContentBrowserItemData& InItem)
{
...
FContentBrowserItemData UContentBrowserAliasDataSource::CreateAssetFileItem(const FContentBrowserUniqueAlias& Alias)
{
	...
	if (AliasData)
	{
		...
		FName VirtualizedPath;
		TryConvertInternalPathToVirtual(AliasData->InternalPath, VirtualizedPath);

...
static TAutoConsoleVariable<bool> CVarAuthRegisterAccountLoggedInThroughOSSEnabled(
	TEXT("Online.AuthRegisterAccountLoggedInThroughOSSEnabled"),
	true,
	TEXT("When enabled, will register account logged in directly through OSSv1 instead of returning error"),
	ECVF_Default);
...
const FExternalAuthTokenTranslationTraits* GetExternalAuthTokenTranslationTraits(FName SubsystemType, EExternalAuthTokenMethod Method)
{
	static const TMap<FName, TArray<FExternalAuthTokenTranslationTraits>> SupportedExternalAuthTranslatorTraits = {
		...
		{ NINTENDO_SUBSYSTEM, { { ExternalLoginType::NintendoNsaIdToken, EExternalAuthTokenMethod::Primary, EExternalAuthTokenTranslationFlags::AuthToken },
...
TOnlineAsyncOpHandle<FAuthLogin> FAuthOSSAdapter::Login(FAuthLogin::Params&& Params)
{
	...
	.Then([this](TOnlineAsyncOp<FAuthLogin>& InAsyncOp)
	{
		...
		if (!PopulateAttributes(*AccountInfoOSSAdapter))
		{
...
TOnlineAsyncOpHandle<FAuthHandleLoginStatusChangedImpl> FAuthOSSAdapter::HandleLoginStatusChangedImplOp(FAuthHandleLoginStatusChangedImpl::Params&& Params)
{
	...
	Op->Then([this](TOnlineAsyncOp<FAuthHandleLoginStatusChangedImpl>& InAsyncOp)
	{
		...
		if (!AccountInfoOSSAdapter)
		{
			if (CVarAuthRegisterAccountLoggedInThroughOSSEnabled.GetValueOnAnyThread())
			{
				// In case of Login get called without going through the OSSv2 OSS Adapter, add logged in user to AccountInfoRegistry as well
				AccountInfoOSSAdapter = MakeShared<FAccountInfoOSSAdapter>();
				AccountInfoOSSAdapter->AccountId = Params.AccountId;
				AccountInfoOSSAdapter->PlatformUserId = Params.PlatformUserId;
				AccountInfoOSSAdapter->LoginStatus = ELoginStatus::NotLoggedIn;
				AccountInfoOSSAdapter->LocalUserNum = FPlatformMisc::GetUserIndexForPlatformUser(Params.PlatformUserId);
				AccountInfoOSSAdapter->UniqueNetId = GetIdentityInterface()->GetUniquePlayerId(AccountInfoOSSAdapter->LocalUserNum);
				if (!PopulateAttributes(*AccountInfoOSSAdapter))
				{
					InAsyncOp.SetError(Errors::InvalidUser());
					return;
				}
				AccountInfoRegistryOSSAdapter.Register(AccountInfoOSSAdapter.ToSharedRef());
			}
			else
			{
				InAsyncOp.SetError(Errors::InvalidUser());
...
bool FAuthOSSAdapter::PopulateAttributes(FAccountInfoOSSAdapter& AccountInfoOSSAdapter) const
{
	if (AccountInfoOSSAdapter.UniqueNetId && AccountInfoOSSAdapter.UniqueNetId->IsValid())
	{
		TSharedPtr<FUserOnlineAccount> UserOnlineAccount = GetIdentityInterface()->GetUserAccount(*AccountInfoOSSAdapter.UniqueNetId);
		if (UserOnlineAccount)
		{
			AccountInfoOSSAdapter.Attributes.Emplace(AccountAttributeData::DisplayName, UserOnlineAccount->GetDisplayName());
			return true;
		}

		FString Displayname = GetIdentityInterface()->GetPlayerNickname(*AccountInfoOSSAdapter.UniqueNetId);

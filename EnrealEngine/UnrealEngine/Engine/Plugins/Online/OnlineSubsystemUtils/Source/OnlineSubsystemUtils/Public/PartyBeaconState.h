...
namespace EPartyReservationResult
{
	enum Type : int
	{
		...
		/** The reservation was rejected because it failed additional, project-specific validation */
		ReservationDenied_ValidationFailed
	};
}
...
struct FPartyReservation
{
	...
	UE_API void RemovePartyMemberAtIndex(int32 Idx);
	UE_API int32 RemoveAllPartyMembers(const FPlayerReservation& OtherRes);

	/** Is this data well formed */
	UE_API bool IsValid(bool bIsValidationStrRequired=true) const;

	/** Dump this reservation to log */
	UE_API void Dump() const;
	UE_API void DumpUniqueIdsOnly() const;
	 ...
	 */
	UE_API bool CanPlayerMigrateFromReservation(const FPartyReservation& Other) const;
};
...
class UPartyBeaconState : public UObject
{
	 ...
	 */
	UE_API virtual bool InitState(int32 InTeamCount, int32 InTeamSize, int32 InMaxReservations, FName InSessionName, int32 InForceTeamNum, bool bEnableRemovalRequests);
	 ...
	 */
	UE_API virtual bool ReconfigureTeamAndPlayerCount(int32 InNumTeams, int32 InNumPlayersPerTeam, int32 InNumReservations);
	 ...
	 */
	UE_API virtual void SetCompetitiveIntegrity(bool bNewCompetitiveIntegrity);
	 ...
	 */
	UE_API virtual void SetTeamAssignmentMethod(FName NewAssignmentMethod);
	 ...
	 */
	UE_API virtual bool AddReservation(const FPartyReservation& ReservationRequest);
	 ...
	 */
	UE_API virtual bool RemoveReservation(const FUniqueNetIdRepl& PartyLeader);
	 ...
	 */
	UE_API void RegisterAuthTicket(const FUniqueNetIdRepl& InPartyMemberId, const FString& InAuthTicket);
	 ...
	 */
	UE_API virtual void UpdatePartyLeader(const FUniqueNetIdRepl& InPartyMemberId, const FUniqueNetIdRepl& NewPartyLeaderId);
	 ...
	 * @return true if successful, false otherwise
	 */
	UE_API virtual bool SwapTeams(const FUniqueNetIdRepl& PartyLeader, const FUniqueNetIdRepl& OtherPartyLeader);
	 ...
	 * @return true if successful, false otherwise
	 */
	UE_API virtual bool ChangeTeam(const FUniqueNetIdRepl& PartyLeader, int32 NewTeamNum);
	 ...
	 */
	UE_API virtual bool RemovePlayer(const FUniqueNetIdRepl& PlayerId);
	 ...
	 */
	UE_API virtual int32 GetExistingReservation(const FUniqueNetIdRepl& PartyLeader) const;
	 ...
	 * @return index of reservation, INDEX_NONE otherwise
	 */
	UE_API virtual int32 GetExistingReservationContainingMember(const FUniqueNetIdRepl& PartyMember) const;
	...
	*/
	 ...
	 */
	UE_API virtual bool DoesReservationFit(const FPartyReservation& ReservationRequest) const;
	 ...
	 */
	UE_API virtual bool DoesModifiedReservationFit(const FPartyReservation& ExistingReservation, const TArray<FPlayerReservation>& NewPlayers) const;
	 ...
	 */
	UE_API virtual int32 GetMaxAvailableTeamSize() const;
	 ...
	 * @return The number of teams.
	 */
	UE_API virtual int32 GetNumPlayersOnTeam(int32 TeamIdx) const;
	 ...
	 */
	UE_API virtual int32 GetTeamForCurrentPlayer(const FUniqueNetId& PlayerId) const;

	/**
	 * Get the amount of teams used by reservations
	 *
	 * @return The number of teams that are used by reservations
	 */
	UE_API virtual int32 GetReservationTeamCount() const;
	 ...
	 */
	UE_API int32 GetPlayersOnTeam(int32 TeamIndex, TArray<FUniqueNetIdRepl>& TeamMembers) const;
	 ...
	 */
	UE_API virtual bool PlayerHasReservation(const FUniqueNetId& PlayerId) const;
	...
	* @return true if a reservation exists, false otherwise
	*/
	UE_API virtual bool UpdateMemberPlatform(const FUniqueNetIdRepl& PartyMember, const FString& PlatformName);
	 ...
	 *
	 */
	UE_API virtual bool GetPlayerValidation(const FUniqueNetId& PlayerId, FString& OutValidation) const;
	 ...
	 */
	UE_API virtual bool GetPartyLeader(const FUniqueNetIdRepl& InPartyMemberId, FUniqueNetIdRepl& OutPartyLeaderId) const;

	/**
	 * Randomly assign a team for the reservation configuring the beacon
	 */
	UE_API virtual void InitTeamArray();
	...
	*/
	UE_API virtual bool AreTeamsAvailable(const FPartyReservation& ReservationRequest) const;
	 ...
	 */
	UE_API virtual bool CrossPlayAllowed(const FPartyReservation& ReservationRequest) const;

	/**
	 * @return true if there are cross play restrictions, false otherwise
	 */
	UE_API virtual bool HasCrossplayOptOutReservation() const;
	 ...
	 */
	UE_API virtual int32 GetReservationPlatformCount(const FString& InPlatform, bool bIncludeMappedPlatforms) const;
	...
	*/
	UE_API virtual int32 GetTeamAssignment(const FPartyReservation& Party);

	/**
	 * Output current state of reservations to log
	 */
	UE_API virtual void DumpReservations() const;
...
protected:
	 ...
	 */
	UE_API virtual void BestFitTeamAssignmentJiggle();
	 ...
	 */
	UE_API void SanityCheckReservations(const bool bIgnoreEmptyReservations) const;

...
{
TAutoConsoleVariable<bool> CVarRemapReservationUpdateToExistingReservation(
	TEXT("beacon.RemapReservationUpdateToExistingReservation"),
	true,
	TEXT("Whether to remap the leader of a reservation update request to the leader of the reservation they appear in.\n")
	TEXT("Enabled"),
	ECVF_Default);
}
					...
					if (bContinue)
					{
						if (!ShouldRespectCompetitiveIntegrity() || State->AreTeamsAvailable(ReservationRequest))
						{
							if (State->CrossPlayAllowed(ReservationRequest))
							{
								...
								{
									Result = EPartyReservationResult::ReservationDenied_ValidationFailed;
								}
							}
							else
							{
...
void APartyBeaconHost::ProcessReservationRequest(APartyBeaconClient* Client, const FString& SessionId, const FPartyReservation& ReservationRequest)
{
	...
	if (Client && Client->GetUniqueId().IsValid() && Client->GetUniqueId() == ReservationRequest.PartyLeader)
	{
	...
	{
		Client->ClientCancelReservationResponse(EPartyReservationResult::ReservationInvalid);
	}
}
...
void APartyBeaconHost::ProcessReservationUpdateRequest(APartyBeaconClient* Client, const FString& SessionId, const FPartyReservation& ReservationUpdateRequest, bool bIsRemovingMember)
{
	...
	if (Client && Client->GetUniqueId().IsValid() && Client->GetUniqueId() == ReservationUpdateRequest.PartyLeader)
	{
		const FPartyReservation* EffectiveReservationUpdateRequest = &ReservationUpdateRequest;
		FPartyReservation RemappedReservationUpdateRequest;
		if (BeaconConsoleVariables::CVarRemapReservationUpdateToExistingReservation.GetValueOnGameThread())
		{
			const int32 ExistingReservationIdx = State ? State->GetExistingReservationContainingMember(ReservationUpdateRequest.PartyLeader) : INDEX_NONE;
			const FPartyReservation* ExistingReservation = ExistingReservationIdx != INDEX_NONE ? &State->GetReservations()[ExistingReservationIdx] : nullptr;
			if (ExistingReservation)
			{
				if (ExistingReservation->PartyLeader != RemappedReservationUpdateRequest.PartyLeader)
				{
					UE_LOG(LogPartyBeacon, Log, TEXT("Remapping reservation update request leader [%s] to leader found in existing reservation [%s]"),
						*ExistingReservation->PartyLeader.ToDebugString(),
						*RemappedReservationUpdateRequest.PartyLeader.ToDebugString());
					RemappedReservationUpdateRequest = ReservationUpdateRequest;
					RemappedReservationUpdateRequest.PartyLeader = ExistingReservation->PartyLeader;
					EffectiveReservationUpdateRequest = &RemappedReservationUpdateRequest;
				}
			}
			else
			{
				// Invalid request, member has no existing reservation to update.
				// Continue with call to UpdatePartyReservation which will return an error.
			}
		}
		...
		if (DoesSessionMatch(SessionId))
		{
			Result = UpdatePartyReservation(*EffectiveReservationUpdateRequest, bIsRemovingMember);
		}
		...
		{
			DumpReservations();
			EffectiveReservationUpdateRequest->Dump();
		}
...
void APartyBeaconHost::ProcessCancelReservationRequest(APartyBeaconClient* Client, const FUniqueNetIdRepl& PartyLeader)
{
	...
	if (Client && Client->GetUniqueId().IsValid() && Client->GetUniqueId() == PartyLeader)
	{

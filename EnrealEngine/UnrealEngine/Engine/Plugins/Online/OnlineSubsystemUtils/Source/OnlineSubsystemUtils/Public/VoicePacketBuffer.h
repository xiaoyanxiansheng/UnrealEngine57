...
public:
	...
	UE_API FVoicePacketBuffer(int32 BufferSize, int32 InNumSamplesUntilIdling, uint64 InStartSample);

	// Push a new packet of decompressed audio onto the buffer to be consumed. This should be called on
	// the voice engine thread.
	UE_API void PushPacket(const void* InBuffer, int32 NumBytes, uint64 InStartSample, EVoipStreamDataFormat Format);
	...
	UE_API int32 PopAudio(float* DestBuffer, uint32 RequestedSamples);
	...
	UE_API int32 GetNumBufferedSamples();
	...
	UE_API uint32 DropOldestAudio(uint32 InNumSamples);

	// Clears the buffer.
	UE_API void Reset(int32 InStartSample);

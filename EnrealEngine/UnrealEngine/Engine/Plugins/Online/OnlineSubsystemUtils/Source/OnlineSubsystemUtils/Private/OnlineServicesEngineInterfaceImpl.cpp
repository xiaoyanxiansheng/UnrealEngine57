...
FString UOnlineServicesEngineInterfaceImpl::GetPIELoginCommandLineArgs(int32 Index)
{
	TArray<UE::Online::FAuthLogin::Params> LoginParams;
	UE::Online::GetServicesEngineUtils()->GetPIELogins(LoginParams);
	if (LoginParams.IsValidIndex(Index))
	{
		const FString& Type = LoginParams[Index].CredentialsType.ToString();
		const FString& Id = LoginParams[Index].CredentialsId;
		const FString& Token = LoginParams[Index].CredentialsToken.IsType<FString>() ? LoginParams[Index].CredentialsToken.Get<FString>() : LoginParams[Index].CredentialsToken.Get<UE::Online::FExternalAuthToken>().Data;

		return FString::Printf(TEXT(" -AUTH_TYPE=%s -AUTH_LOGIN=%s -AUTH_PASSWORD=%s"), *Type, *Id, *Token);
	}
	return TEXT("");
}

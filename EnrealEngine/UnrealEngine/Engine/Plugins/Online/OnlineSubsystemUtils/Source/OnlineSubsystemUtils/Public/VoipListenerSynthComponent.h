...
public:
	UE_API ~UVoipListenerSynthComponent();
	...
	*/
	UE_API void OpenPacketStream(uint64 BeginningSampleCount, int32 BufferSize, float JitterDelay);

	/**
	 * Close an open packet stream and start outputting silence.
	*/
	UE_API void ClosePacketStream();
	 ...
	 * @param JitterDelay The amount of time we wait before we start playing audio from submitted packets.
	*/
	UE_API void ResetBuffer(int32 InStartSample, float JitterDelay);
	...
	*/
	UE_API void SubmitPacket(void* InBuffer, int32 NumBytes, int64 InStartSample, EVoipStreamDataFormat DataFormat);

	/**
	 * Optionally connect this VOIP Synth Component to a splitter.
	 */
	UE_API void ConnectToSplitter(Audio::FPatchMixerSplitter& InSplitter);
	...
	*/
	UE_API uint64 GetSampleCounter();

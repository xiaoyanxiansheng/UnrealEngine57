...
UIpConnection::UIpConnection(const FObjectInitializer& ObjectInitializer) :
	...
	Resolver(MakePimpl<UE::Net::Private::FNetConnectionAddressResolution>())
{
...
void UIpConnection::InitBase(UNetDriver* InDriver, class FSocket* InSocket, const FURL& InURL, EConnectionState InState, int32 InMaxPacket, int32 InPacketOverhead)
{
	...
	{
		SocketPrivate = InSocket->AsShared();
	}
...
void UIpConnection::LowLevelSend(void* Data, int32 CountBits, FOutPacketTraits& Traits)
{
	...
	if (AutoRTFM::IsTransactional() && Driver->ShouldDeferTransactionalSend())
	{
		// Inside an AutoRTFM transaction, we must postpone network sends until commit time, to account for the
		// possibility of aborting the transaction. (An abort will prevent any network traffic from occurring.)
		// We aren't allowed to send packets out of order, so we explicitly jump into the closed here; this ensures
		// that OnCommit takes effect for _all_ transactional packets, not just packets sent in the closed.
		const AutoRTFM::EContextStatus Status = AutoRTFM::Close([&]
		{
			...
			AutoRTFM::OnCommit([this, 
								DataCopy = TArray<uint8>(DataToSend, CountBytes),
								CountBits,
								TraitsCopy = Traits]() mutable
			{
				SendToRemote(DataCopy.GetData(), CountBits, TraitsCopy);
			});
		});
...
void UIpConnection::SendToRemote(uint8* DataToSend, int32 CountBits, FOutPacketTraits& Traits) {
	...
	// Process any packet modifiers, such as compression and encryption.
	if (Handler.IsValid() && !Handler->GetRawSend())
	{
		const ProcessedPacket ProcessedData = Handler->Outgoing(DataToSend, CountBits, Traits);
...
#if !UE_BUILD_SHIPPING
	LowLevelSendDel.ExecuteIfBound(DataToSend, CountBytes, bBlockSend);
#endif
	...
	if (!bBlockSend)
	{
		...
		FPacketAudit::NotifyLowLevelSend(DataToSend, CountBytes, CountBits);

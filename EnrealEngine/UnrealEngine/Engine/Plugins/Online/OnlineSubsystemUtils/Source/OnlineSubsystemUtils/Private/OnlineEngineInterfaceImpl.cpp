...
FString UOnlineEngineInterfaceImpl::GetPIELoginCommandLineArgs(int32 Index)
{
	TArray<FOnlineAccountCredentials> Logins;
	Online::GetUtils()->GetPIELogins(Logins);
	if (Logins.IsValidIndex(Index))
	{
		return FString::Printf(TEXT(" -AUTH_TYPE=%s -AUTH_LOGIN=%s -AUTH_PASSWORD=%s"), *Logins[Index].Type, *Logins[Index].Id, *Logins[Index].Token);
	}
	return TEXT("");
}

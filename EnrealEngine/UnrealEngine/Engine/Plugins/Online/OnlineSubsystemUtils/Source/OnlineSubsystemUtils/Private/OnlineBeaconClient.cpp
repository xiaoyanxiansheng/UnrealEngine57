...
{
	QUICK_SCOPE_CYCLE_COUNTER(STAT_OnlineBeaconClient_InitClient);
		...
		case NMT_BeaconAssignGUID:
			{
				...
				if (FNetControlMessage<NMT_BeaconAssignGUID>::Receive(Bunch, NetID))
				{
					if (NetID.IsValid())
					{
						Connection->Driver->AssignNetIDToObject(NetID, this);

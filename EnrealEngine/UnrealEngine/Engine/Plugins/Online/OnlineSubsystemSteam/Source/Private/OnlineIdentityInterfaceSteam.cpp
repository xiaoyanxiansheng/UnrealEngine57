...
void FOnlineIdentitySteam::GetLinkedAccountAuthToken(int32 LocalUserNum, const FString& InTokenType, const FOnGetLinkedAccountAuthTokenCompleteDelegate& Delegate) const
{
	...
	if (TokenType == AppTokenType)
	{
		...
		*DelegateHandle = SteamSubsystem->GetEncryptedAppTicketInterface()->OnEncryptedAppTicketResultDelegate.AddLambda([this, LocalUserNum, Delegate, DelegateHandle](bool bEncryptedDataAvailable, int32 ResultCode)
		{
			...
			Delegate.ExecuteIfBound(LocalUserNum, ExternalToken.HasTokenData(), ExternalToken);
			SteamSubsystem->GetEncryptedAppTicketInterface()->OnEncryptedAppTicketResultDelegate.Remove(*DelegateHandle);
		});

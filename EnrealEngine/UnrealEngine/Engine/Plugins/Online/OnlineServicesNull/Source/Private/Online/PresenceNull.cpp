...
TOnlineAsyncOpHandle<FPartialUpdatePresence> FPresenceNull::PartialUpdatePresence(FPartialUpdatePresence::Params&& Params)
{
	...
	Op->Then([this](TOnlineAsyncOp<FPartialUpdatePresence>& InAsyncOp) mutable
	{
		...
		if (const TSharedRef<const FUserPresence>* const ExistingPresence = Presences.Find(InAsyncOp.GetParams().LocalAccountId))
		{
			*NewPresence = **ExistingPresence;
		}
		else
		{
			NewPresence->AccountId = InAsyncOp.GetParams().LocalAccountId;
		}

...
FPartyJoinApproval USocialParty::EvaluateJoinRequest(const TArray<IOnlinePartyUserPendingJoinRequestInfoConstRef>& Players, bool bFromJoinRequest) const
{
	...
	auto CheckForSentInvitation = [this, &Players]()
	{
		for (const IOnlinePartyUserPendingJoinRequestInfoConstRef& JoiningPlayer : Players)
		{
			for (USocialToolkit* Toolkit : UE::OnlineFramework::GetLocalPartyMemberToolkits(*this))
			{
				if (USocialUser* JoiningSocialUser = Toolkit->FindUser(JoiningPlayer->GetUserId());
					JoiningSocialUser && JoiningSocialUser->HasBeenInvitedToParty(IOnlinePartySystem::GetPrimaryPartyTypeId()))
				{
					return true;
				}
			}
	...
	if ((GetNumPartyMembers() + Players.Num()) > GetPartyMaxSize())
	{
		...
		SocialUser &&
		(USocialManager::ShouldReadPresenceFromOnlineServices() ? SocialUser->GetOnlineStatusV2() == UE::Online::EUserPresenceStatus::Away : SocialUser->GetOnlineStatus() == EOnlinePresenceState::Away) &&
		(!GetSocialManager().CanAcceptInvitationsFromAwayUsers() || !CheckForSentInvitation()))
	{

...
struct FJoinPartyResult
{
public:
	UE_API FJoinPartyResult(); 
	UE_API FJoinPartyResult(FPartyJoinDenialReason InDenialReason);
	UE_API FJoinPartyResult(EJoinPartyCompletionResult InResult);
	UE_API FJoinPartyResult(EJoinPartyCompletionResult InResult, FPartyJoinDenialReason InDenialReason);
	UE_API FJoinPartyResult(EJoinPartyCompletionResult InResult, int32 InResultSubCode);
	
	UE_API void SetDenialReason(FPartyJoinDenialReason InDenialReason);
	UE_API void SetResult(EJoinPartyCompletionResult InResult);
	
	UE_API bool WasSuccessful() const;
...
protected:
	...
	UE_API void LogSetPropertyFailure(const TCHAR* OwningStructTypeName, const TCHAR* PropertyName) const;
	UE_API void LogPropertyChanged(const TCHAR* OwningStructTypeName, const TCHAR* PropertyName, bool bFromReplication) const;

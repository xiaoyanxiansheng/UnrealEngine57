...
namespace UE::OnlineFramework::CVars
{
	bool bAssignPartyJoinInfoOnPartyJoinFailure = true;
	static FAutoConsoleVariableRef CVarAssignPartyJoinInfoOnPartyJoinFailure(
		TEXT("Party.AssignPartyJoinInfoOnPartyJoinFailure"), bAssignPartyJoinInfoOnPartyJoinFailure,
		TEXT("Temporary cvar (remove after proven stable), whether to set the party join info on the join attempt when ValidateJoinTarget fails"),
		ECVF_Default);
}
//////////////////////////////////////////////////////////////////////////
// FJoinPartyAttempt
...
{
	return FString::Printf(TEXT("TargetUser(%s), PartyId(%s), TypeId(%d), TargetUserPlatformId(%s), JoinMethod(%s)"),
		TargetUser.IsValid() ? *TargetUser->ToDebugString() : TEXT("invalid"),
		JoinInfo.IsValid() ? *JoinInfo->GetPartyId()->ToDebugString() : TEXT("unknown"),
...
bool USocialManager::ShouldReadPresenceFromOnlineServices()
{
	if (IConsoleVariable* CVar = IConsoleManager::Get().FindConsoleVariable(TEXT("onlineframework.party.shouldreadpresencefromonlineservices")))
	{
		return CVar->GetBool();
	}
...
FName USocialManager::GetPresenceFrameworkInstance()
{
	return GetDefault<USocialManager>()->PresenceFrameworkInstance;
}

FName USocialManager::GetPresenceFrameworkAdapterInstance()
{
	return GetDefault<USocialManager>()->PresenceFrameworkAdapterInstance;
}
...
FJoinPartyResult USocialManager::ValidateJoinTarget(const USocialUser& UserToJoin, const FOnlinePartyTypeId& PartyTypeId, bool bCheckPlatformSession) const
{
	...
	else if ((ShouldReadPresenceFromOnlineServices() ? UserToJoin.GetOnlineStatusV2() == UE::Online::EUserPresenceStatus::Away : UserToJoin.GetOnlineStatus() == EOnlinePresenceState::Away) && // Away indicates they do not want to be joined
		(!CanAcceptInvitationsFromAwayUsers() || !UserToJoin.HasSentPartyInvite(PartyTypeId))) // They can still send you invitations while away, should we ignore the Away status if we've received an invite?
	{
...
bool USocialManager::CanAcceptInvitationsFromAwayUsers() const
{
	return bAcceptInvitationsFromAwayUsers;
}
...
void USocialManager::JoinParty(const USocialUser& UserToJoin, const FOnlinePartyTypeId& PartyTypeId, const FOnJoinPartyAttemptComplete& OnJoinPartyComplete, const FName& JoinMethod)
{
	...
	{
		if (UE::OnlineFramework::CVars::bAssignPartyJoinInfoOnPartyJoinFailure)
		{
			NewAttempt.JoinInfo = UserToJoin.GetPartyJoinInfo(PartyTypeId);
		}
...
{
	return bIsConnectedToPartyService 
#if !UE_BUILD_SHIPPING
		&& !CVarForceDisconnectedToPartyService.GetValueOnAnyThread()
#endif
	;
}

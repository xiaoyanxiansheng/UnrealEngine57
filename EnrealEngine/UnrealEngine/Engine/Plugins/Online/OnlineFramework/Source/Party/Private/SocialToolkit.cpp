...
const UE::Online::TOnlineResult<UE::Online::FGetCachedPresence> USocialToolkit::GetPresenceInfoV2(ESocialSubsystem SubsystemType) const
{
	...
	if (IOnlineServicesPtr OnlineServices = CommonConfig.GetServices(USocialManager::GetPresenceFrameworkInstance()))
	{
		FUniqueNetIdRepl LocalUserNetId = GetLocalUserNetId(SubsystemType);
		if(LocalUserNetId.IsValid())
		{
			if (const FCommonAccountPtr LocalUserCommonAccountPtr = GetCommonAccountFromV1(CommonConfig, LocalUserNetId->AsShared(), GetFirstFrameworkInstanceName(LocalUserNetId)))
			{
				...
				{
					Result = Presence->GetCachedPresence({ LocalUserAccountId, LocalUserAccountId });
				}
			}
		}
	}
...
void USocialToolkit::SetLocalUserOnlineStateV2(UE::Online::EUserPresenceStatus OnlineState)
{
	using namespace UE::Online;
	using namespace UE::OnlineFramework;
	...
	if (IOnlineServicesPtr OnlineServices = CommonConfig.GetServices(USocialManager::GetPresenceFrameworkInstance()))
	{
		FUniqueNetIdRepl LocalUserNetId = GetLocalUserNetId(ESocialSubsystem::Primary);
		if(LocalUserNetId.IsValid())
		{
			if (const FCommonAccountPtr LocalUserCommonAccountPtr = GetCommonAccountFromV1(CommonConfig, LocalUserNetId->AsShared(), GetFirstFrameworkInstanceName(LocalUserNetId)))
			{
				if(FAccountId LocalUserAccountId = LocalUserCommonAccountPtr->GetId(USocialManager::GetPresenceFrameworkInstance()))
				{
					if (IPresencePtr Presence = OnlineServices->GetPresenceInterface())
					{
						FPartialUpdatePresence::Params::FMutations Mutations;
						Mutations.Status = OnlineState;

						FPartialUpdatePresence::Params Params = { LocalUserAccountId, Mutations };

						UE_LOG(LogParty, Verbose, TEXT("Updating rich presence with OnlineServices"));
	...
	for (ESocialSubsystem SubsystemType : USocialManager::GetDefaultSubsystems())
	{
		...
		if (LocalUserNetId.IsValid())
		{
			...
			if (IOnlineFriendsPtr FriendsInterface = OSS->GetFriendsInterface())
			{
				...
				FriendsInterface->AddOnFriendsChangeDelegate_Handle(LocalUserNum, FOnFriendsChangeDelegate::CreateUObject(this, &USocialToolkit::HandleFriendsChanged, SubsystemType));
				...
				FriendsInterface->AddOnBlockListChangeDelegate_Handle(LocalUserNum, FOnBlockListChangeDelegate::CreateUObject(this, &USocialToolkit::HandleBlockListChanged, SubsystemType));
			...
			{
				UE::OnlineFramework::FCommonConfig CommonConfig(this);
				if (UE::Online::IOnlineServicesPtr OnlineServices = CommonConfig.GetServices(USocialManager::GetPresenceFrameworkInstance()))
				{
					if (UE::Online::IPresencePtr Presence = OnlineServices->GetPresenceInterface())
					{
						UE::Online::FOnlineEventDelegateHandle OnPresenceUpdatedHandle = Presence->OnPresenceUpdated().Add(this, &USocialToolkit::HandleServicesPresenceUpdated, SubsystemType);
						OnPresenceUpdatedHandles.Emplace(SubsystemType, MoveTemp(OnPresenceUpdatedHandle));
					}
				}
			}
			else
	...
	for (ESocialSubsystem SubsystemType : USocialManager::GetDefaultSubsystems())
	{
		if (IOnlineSubsystem* OSS = GetSocialOss(SubsystemType))
		{
			...
			if (FriendsInterface.IsValid())
			{
				...
				FriendsInterface->ClearOnBlockListChangeDelegates(LocalUserNum, this);
			...
			if (UE::Online::FOnlineEventDelegateHandle* Handle = OnPresenceUpdatedHandles.Find(SubsystemType))
			{
				Handle->Unbind();
			}
		}
...
void USocialToolkit::HandleQueryBlockedPlayersComplete(const FUniqueNetId& UserId, bool bWasSuccessful, const FString& ErrorStr, ESocialSubsystem SubsystemType)
{
	if (UserId == *GetLocalUserNetId(SubsystemType))
	{
		...
		if (bWasSuccessful)
		{
			...
			ProcessOSSBlockedPlayers(SubsystemType, BlockedPlayers);
...
void USocialToolkit::HandleServicesPresenceUpdated(const UE::Online::FPresenceUpdated& Event, ESocialSubsystem SubsystemType)
{
	using namespace UE::Online;
	using namespace UE::OnlineFramework;

	TSharedRef<const FUserPresence> UpdatedPresence = Event.UpdatedPresence;

	FCommonConfig CommonConfig(this);
	
	if (FCommonAccountPtr CommonAccountPtr = GetCommonAccountFromV2(CommonConfig, UpdatedPresence->AccountId, USocialManager::GetPresenceFrameworkInstance()))
	{
		// We need a services instance with an Auth adapter to make this conversion
		if (FUniqueNetIdPtr UserId = GetV1FromCommonAccount(CommonAccountPtr.ToSharedRef(), USocialManager::GetPresenceFrameworkAdapterInstance()))
		{
			QUICK_SCOPE_CYCLE_COUNTER(STAT_USocialToolkit_HandlePresenceReceived);
			if (USocialUser* UpdatedUser = FindUser(UserId.ToSharedRef()))
			{
				UpdatedUser->NotifyPresenceChanged(SubsystemType);

				if (UpdatedUser->IsFriend())
				{
					QueueUserDependentActionInternal(UserId.ToSharedRef(), ESocialSubsystem::Primary,
						[this, UpdatedPresence](USocialUser& SocialUser)
						{
							OnFriendPresenceDidChangeV2(SocialUser, UpdatedPresence, ESocialSubsystem::Primary);
						});
				}
			}
...
void USocialToolkit::HandleFriendsChanged(ESocialSubsystem SubsystemType)
{
	UE_LOG(LogParty, Log, TEXT("SocialToolkit [%d] quietly refreshing Friend list due to an external requery of the friends list."), GetLocalUserNum());

	TArray<TSharedRef<FOnlineFriend>> FriendsList;
	IOnlineFriendsPtr FriendsInterface = Online::GetFriendsInterfaceChecked(GetWorld(), USocialManager::GetSocialOssName(SubsystemType));
	FriendsInterface->GetFriendsList(GetLocalUserNum(), FriendListToQuery, FriendsList);
	ProcessUserList(FriendsList, SubsystemType, OnFriendshipEstablished());
}
...
void USocialToolkit::HandleBlockListChanged(int32 LocalUserNum, const FString& ListName, ESocialSubsystem SubsystemType)
{
	if (LocalUserNum == GetLocalUserNum())
	{
		UE_LOG(LogParty, Log, TEXT("SocialToolkit [%d] Subsystem=[%s] Blocklist changed"), GetLocalUserNum(), ToString(SubsystemType));

		TArray<TSharedRef<FOnlineBlockedPlayer>> BlockedPlayers;
		IOnlineFriendsPtr FriendsInterface = Online::GetFriendsInterfaceChecked(GetWorld(), USocialManager::GetSocialOssName(SubsystemType));
		FriendsInterface->GetBlockedPlayers(*GetLocalUserNetId(SubsystemType), BlockedPlayers);

		ProcessOSSBlockedPlayers(SubsystemType, BlockedPlayers);
	}
}

void USocialToolkit::ProcessOSSBlockedPlayers(ESocialSubsystem SubsystemType, const TArray<TSharedRef<FOnlineBlockedPlayer>>& InBlockedPlayers)
{
	TSet<FUniqueNetIdRepl> InBlockedPlayerNetIds;
	Algo::Transform(InBlockedPlayers, InBlockedPlayerNetIds, [](const TSharedRef<FOnlineBlockedPlayer>& BlockedPlayer) { return BlockedPlayer->GetUserId(); });

	TSet<FUniqueNetIdRepl>& CurrentBlockedUserNetIds = BlockedUsers.FindOrAdd(SubsystemType);
	TSet<FUniqueNetIdRepl> UnblockedNetIds = CurrentBlockedUserNetIds.Difference(InBlockedPlayerNetIds);

	UE_LOG(LogParty, Log, TEXT("SocialToolkit [%d] Subsystem=[%s] BlockedPlayers=[%s] UnblockedPlayers=[%s]"),
		GetLocalUserNum(),
		LexToString(SubsystemType),
		*FString::JoinBy(InBlockedPlayerNetIds, TEXT(","), &FUniqueNetIdRepl::ToDebugString),
		*FString::JoinBy(UnblockedNetIds, TEXT(","), &FUniqueNetIdRepl::ToDebugString));

	CurrentBlockedUserNetIds = MoveTemp(InBlockedPlayerNetIds);

	for (const FUniqueNetIdRepl& UnblockedNetId : UnblockedNetIds)
	{
		if (TWeakObjectPtr<USocialUser>* WeakUnblockedUser = UsersBySubsystemIds.Find(UnblockedNetId))
		{
			if (USocialUser* UnblockedUser = WeakUnblockedUser->Get())
			{
				UnblockedUser->NotifyUserUnblocked(SubsystemType);
			}
		}
	...
	ProcessUserList(InBlockedPlayers, SubsystemType, OnUserBlocked());
}
...
void USocialToolkit::Debug_OnStartRandomizeUserPresence(uint8 NumRandomUser, float TickerTimer)
{
	...
	{
		FTSTicker::RemoveTicker(Debug_PresenceTickerHandle);

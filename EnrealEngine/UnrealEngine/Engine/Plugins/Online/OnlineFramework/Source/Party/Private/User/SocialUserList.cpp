...
bool FSocialUserList::EvaluateUserPresence(const USocialUser& User, ESocialSubsystem SubsystemType)
{
	if (HasPresenceFilters())
	{
		...
		if (USocialManager::ShouldReadPresenceFromOnlineServices())
		{
			...
			const TOnlineResult<FGetCachedPresence>& GetCachedPresenceResult = User.GetFriendPresenceInfoV2(SubsystemType);
			if (GetCachedPresenceResult.IsOk())
			{
				...
				bIsOnline = CachedPresence->Status != UE::Online::EUserPresenceStatus::Offline;
				bIsPlayingThisGame = CachedPresence->GameStatus == UE::Online::EUserPresenceGameStatus::PlayingThisGame;
			}
		}
		else
		{
		...
		if (OwnerToolkit.IsValid())
		{
			...
			if (OwnerToolkit->Debug_IsRandomlyChangingPresence())
			{
				if (USocialManager::ShouldReadPresenceFromOnlineServices())
				{
					bIsOnline = User.GetOnlineStatusV2() != UE::Online::EUserPresenceStatus::Offline;
				}
				else
				{
...
struct FUserSortData
{
	...
	FUserSortData(USocialUser* InUser, UE::Online::EUserPresenceStatus InStatus, bool InPlayingThisGame, FString InDisplayName, TSharedRef<const TArray<int64>> InSortParams)
		: User(InUser)
		, OnlineStatusV2(InStatus)
		, PlayingThisGame(InPlayingThisGame)
		, DisplayName(MoveTemp(InDisplayName))
		, SortParams(InSortParams)
	{ }
	...
	TOptional<EOnlinePresenceState::Type> OnlineStatus;
	TOptional<UE::Online::EUserPresenceStatus> OnlineStatusV2;
	...
	bool operator<(const FUserSortData& OtherSortData) const
	{
		...
		if ((OnlineStatus.IsSet() && *OnlineStatus == *OtherSortData.OnlineStatus) || (OnlineStatusV2.IsSet() && *OnlineStatusV2 == *OtherSortData.OnlineStatusV2))
		{
		...
		else
		{
			...
			return OnlineStatus.IsSet() ? *OnlineStatus < *OtherSortData.OnlineStatus : OnlineStatusV2.IsSet() ? *OnlineStatusV2 < *OtherSortData.OnlineStatusV2 : false;		
		}
	}
};
		...
		if (ListConfig.bSortDuringUpdate)
		{
			...
			if (NumUsers > 1)
			{
				...
				Algo::TransformIf(Users, SortedData,
					...
					[this](const TWeakObjectPtr<USocialUser> UserPtr) -> FUserSortData
					{
						...
						if (USocialManager::ShouldReadPresenceFromOnlineServices())
						{
							return FUserSortData(User, User->GetOnlineStatusV2(), User->IsPlayingThisGame(), User->GetDisplayName(), SortParams);
						}
						else
						{

...
DEFINE_SOCIAL_INTERACTION(AddFriend)
...
DEFINE_SOCIAL_INTERACTION(AddPlatformFriend)
...
DEFINE_SOCIAL_INTERACTION(RemoveFriend)
...
DEFINE_SOCIAL_INTERACTION(AcceptFriendInvite)
...
DEFINE_SOCIAL_INTERACTION(RejectFriendInvite)
...
DEFINE_SOCIAL_INTERACTION(Block)
...
DEFINE_SOCIAL_INTERACTION(Unblock)
...
DEFINE_SOCIAL_INTERACTION(PrivateMessage)
...
bool FSocialInteraction_PrivateMessage::CanExecute(const USocialUser& User)
{
	...
	return !User.GetOwningToolkit().GetChatManager().IsChatRestricted() &&
		(USocialManager::ShouldReadPresenceFromOnlineServices() ? User.GetOnlineStatusV2() != UE::Online::EUserPresenceStatus::Offline : User.GetOnlineStatus() != EOnlinePresenceState::Offline) &&
		User.IsFriend();
}
...
DEFINE_SOCIAL_INTERACTION(ShowPlatformProfile)

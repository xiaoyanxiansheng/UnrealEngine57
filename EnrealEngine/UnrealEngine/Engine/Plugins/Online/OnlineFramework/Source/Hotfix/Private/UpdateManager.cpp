...
static TAutoConsoleVariable<float> CVarUpdateManagerCacheTimeoutSeconds(
	TEXT("UpdateManager.CacheTimeoutSeconds"),
	300,
	TEXT("Timeout in seconds to return a previous update manager response."),
...
void UUpdateManager::RegisterStatsCollector(const TSharedPtr<IUpdateManagerStatsCollector>& InStatsCollector)
{
...
UUpdateManager::EUpdateStartResult UUpdateManager::StartUpdateCheckInternal(const FUpdateContextDefinition& ContextDefinition)
{
	...
	if (!ChecksEnabled() || !ContextDefinition.bEnabled)
	{
		...
		if (TSharedPtr<IUpdateManagerStatsCollector> StrongStatsCollector = StatsCollector.Pin())
		{
			StrongStatsCollector->OnCheckStarted({
				.ContextName = ContextDefinition.Name,
				.bEnabled = false });
		}
		...
		CurrentUpdateState == EUpdateState::UpdateComplete))
	{
		...
		const double CacheTimer = CVarUpdateManagerCacheTimeoutSeconds.GetValueOnGameThread();
		...
		if (bForceCheck || TimeSinceCheck >= CacheTimer)
		{
			if (TSharedPtr<IUpdateManagerStatsCollector> StrongStatsCollector = StatsCollector.Pin())
			{
				StrongStatsCollector->OnCheckStarted({
					.ContextName = ContextDefinition.Name,
					.bCheckAvailabilityOnly = ContextDefinition.bCheckAvailabilityOnly });
			}
		...
		else
		{
			if (TSharedPtr<IUpdateManagerStatsCollector> StrongStatsCollector = StatsCollector.Pin())
			{
				StrongStatsCollector->OnCheckStarted({
					...
					.CacheExpiration = CacheTimer - TimeSinceCheck });
			}

			UE_LOG(LogHotfixManager, Display, TEXT("Returning cached update result %d. Cache timeout in %.2f/%.2f seconds."), (int32)LastResult, CacheTimer - TimeSinceCheck, CacheTimer);
...
void UUpdateManager::CheckComplete(EUpdateCompletionStatus Result, bool bUpdateTimestamp)
{
	...
	auto CompletionDelegate = [this, Result, bUpdateTimestamp]()
	{
		...
		if (TSharedPtr<IUpdateManagerStatsCollector> StrongStatsCollector = StatsCollector.Pin())
		{
			StrongStatsCollector->OnCheckComplete(Result);
		}
...
void UUpdateManager::StartPatchCheck()
{
	...
	if (TSharedPtr<IUpdateManagerStatsCollector> StrongStatsCollector = StatsCollector.Pin())
	{
		StrongStatsCollector->OnCheckStep_PatchCheckStarted();
	}
...
void UUpdateManager::InstallBundlePatchCheckComplete(EInstallBundleManagerPatchCheckResult PatchResult)
{
	if (TSharedPtr<IUpdateManagerStatsCollector> StrongStatsCollector = StatsCollector.Pin())
	{
		StrongStatsCollector->OnCheckStep_PatchCheckComplete(PatchResult);
	}
...
void UUpdateManager::StartHotfixCheck()
{
	if (TSharedPtr<IUpdateManagerStatsCollector> StrongStatsCollector = StatsCollector.Pin())
	{
		StrongStatsCollector->OnCheckStep_HotfixCheckStarted();
	}
...
void UUpdateManager::OnHotfixCheckComplete(EHotfixResult Result)
{
	...
	auto CompletionDelegate = [this, Result]()
	{
		if (TSharedPtr<IUpdateManagerStatsCollector> StrongStatsCollector = StatsCollector.Pin())
		{
			StrongStatsCollector->OnCheckStep_HotfixCheckComplete(Result);
		}
...
void UUpdateManager::StartInitialPreload()
{
	if (TSharedPtr<IUpdateManagerStatsCollector> StrongStatsCollector = StatsCollector.Pin())
	{
		StrongStatsCollector->OnCheckStep_InitialPreloadStarted();
	}
...
void UUpdateManager::InitialPreloadComplete()
{
	if (TSharedPtr<IUpdateManagerStatsCollector> StrongStatsCollector = StatsCollector.Pin())
	{
		StrongStatsCollector->OnCheckStep_InitialPreloadComplete();
	}

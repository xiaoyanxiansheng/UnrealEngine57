...
bool UOnlineHotfixManager::IsMapLoaded(const FString& MapName)
{
	...
	UPackage* MapPackage = FindObject<UPackage>(nullptr, *MapPackageName, EFindObjectFlags::ExactClass);
...
void UOnlineHotfixManager::RestoreBackupIniFiles()
{
	...
	if (ClassesToRestore.Num() > 0)
	{
		...
		{
			UClass* Class = FindObject<UClass>(nullptr, *ClassesToRestore[Index], EFindObjectFlags::ExactClass);
			if (Class != nullptr)
			{
...
void UOnlineHotfixManager::ReloadConfigsFromIniFiles()
{
	...
	if (ClassesToReload.Num() > 0)
	{
		...
		{
			UClass* Class = FindObject<UClass>(nullptr, *ClassesToReload[Index], EFindObjectFlags::ExactClass);
			if (Class != nullptr)
			{
				// Add this to the list to check against
				RestoredClasses.Add(Class);
...
{
	// OwnerWorld may be stale as it is set at the time that ::Get is called.
	if (OwnerWorld.IsValid())
	{
		return OwnerWorld.Get();
	}

	// Try to get the active world from the game instance.
	UGameViewportClient* ActiveViewport = GEngine ? GEngine->GameViewport.Get() : nullptr;
	UGameInstance* GameInstance = ActiveViewport ? ActiveViewport->GetGameInstance() : nullptr;
			...
			if (EndIndex > StartIndex)
			{
				...
				if (PerObjectNameIndex == -1 || PerObjectNameIndex > EndIndex)
				{
					...
					if (FCString::Strnicmp(*IniData + StartIndex, ScriptHeader, FCString::Strlen(ScriptHeader)) == 0)
					{
						...
						UClass* Class = FindObject<UClass>(nullptr, *PackageClassName, EFindObjectFlags::ExactClass);
						if (Class)
						{
							// Add this to the list to check against
							Classes.Add(Class);

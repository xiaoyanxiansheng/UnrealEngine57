...
#endif
{
	...
	if (IEOSSDKManager* EOSSDKManager = IEOSSDKManager::Get())
	{
		NetworkStatusChangedDelegateHandle = EOSSDKManager->OnNetworkStatusChanged.AddRaw(this, &FSocketEOS::OnNetworkStatusChanged);
	}
#endif
}
...
FSocketEOS::~FSocketEOS()
{
	...
	SocketSubsystem.UnbindSocketName(SocketName);
	...
	if (IEOSSDKManager* EOSSDKManager = IEOSSDKManager::Get())
	{
		EOSSDKManager->OnNetworkStatusChanged.Remove(NetworkStatusChangedDelegateHandle);
	}
#endif
}
...
bool FSocketEOS::Close()
{
	...
	FCStringAnsi::Strcpy(SocketId.SocketName, TCHAR_TO_UTF8(*SocketName));
	...
	UE_LOG(LogSocketSubsystemEOS, Log, TEXT("Closing socket (%s) with result (%s)"), *SocketName, *LexToString(Result));
...
bool FSocketEOS::Bind(const FInternetAddr& Addr)
{
	...
	if (!SocketSubsystem.BindSocketName(SocketName))
	{
	...
	UE_LOG(LogSocketSubsystemEOS, Verbose, TEXT("Successfully bound socket (%s) to address (%s)"), *SocketName, *Addr.ToString(true));
	return true;
}
...
bool FSocketEOS::Listen(int32)
{
	...
	ConnectNotifyCallback->CallbackLambda = [this](const EOS_P2P_OnIncomingConnectionRequestInfo* Info)
	{
		...
		if (Info->LocalUserId == SocketSubsystem.GetLocalUserId() && FCStringAnsi::Stricmp(Info->SocketId->SocketName, TCHAR_TO_UTF8(*SocketName)) == 0)
		{
			// In case they disconnected and then reconnected, remove them from our closed list
			FInternetAddrEOS RemoteAddress(Info->RemoteUserId);
...
bool FSocketEOS::HasPendingData(uint32& PendingDataSize)
{
	...
	Options.LocalUserId = SocketSubsystem.GetLocalUserId();
	uint8 Channel = GetTypeHash(SocketName);
	...
	{
		UE_LOG(LogSocketSubsystemEOS, Warning, TEXT("EOS_P2P_GetNextReceivedPacketSize for socket [%s] (Channel [%d]) returned result code = [%s]"), *SocketName, Channel, *LexToString(Result));
...
bool FSocketEOS::SendTo(const uint8* Data, int32 Count, int32& OutBytesSent, const FInternetAddr& Destination)
{
	...
	if (!SocketSubsystem.GetSocketSubsystemEOSUtils()->IsLoggedIn())
	{
		UE_LOG(LogSocketSubsystemEOS, VeryVerbose, TEXT("FSocketEOS::SendTo - Local user is logged out"));
	...
	Options.LocalUserId = SocketSubsystem.GetLocalUserId();
	Options.RemoteUserId = DestinationAddress.GetProductUserId();
	Options.SocketId = &SocketId;
	Options.bAllowDelayedDelivery = EOS_TRUE;
	Options.Reliability = PacketReliability;
	Options.Channel = GetTypeHash(SocketName);
...
bool FSocketEOS::RecvFrom(uint8* Data, int32 BufferSize, int32& BytesRead, FInternetAddr& Source, ESocketReceiveFlags::Type Flags)
{
	...
	if (!SocketSubsystem.GetSocketSubsystemEOSUtils()->IsLoggedIn())
	{
		UE_LOG(LogSocketSubsystemEOS, VeryVerbose, TEXT("FSocketEOS::RecvFrom - Local user is logged out"));
	...
	SourceAddress.SetProductUserId(RemoteUserId);
...
void FSocketEOS::SetSocketName(const FString& InSocketName)
{
...
bool FSocketEOS::Close(const FInternetAddrEOS& RemoteAddress)
{
	...
	Options.LocalUserId = SocketSubsystem.GetLocalUserId();
	Options.RemoteUserId = RemoteAddress.GetProductUserId();
	Options.SocketId = &SocketId;

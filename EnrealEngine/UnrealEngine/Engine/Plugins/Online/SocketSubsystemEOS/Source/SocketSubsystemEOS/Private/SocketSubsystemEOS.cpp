...
TSharedRef<FInternetAddr> FSocketSubsystemEOS::GetLocalBindAddr(const UWorld* const OwningWorld, FOutputDevice& Out)
{
	...
	{
		BoundAddr->SetProductUserId(LocalUserId);
	}
	else
	{
		UE_LOG(LogSocketSubsystemEOS, Error, TEXT("No local user set in LocalBindAddr"));
	}
#endif
...
bool FSocketSubsystemEOS::BindSocketName(const FString& SocketName)
{
	if (BoundSocketNames.Contains(SocketName))
	{
		SetLastSocketError(ESocketErrors::SE_EADDRINUSE);
		return false;
	}

	BoundSocketNames.Add(SocketName);
	return true;
}

bool FSocketSubsystemEOS::UnbindSocketName(const FString& SocketName)
{
	// Remove our port and check if we had it bound
	if (BoundSocketNames.Remove(SocketName) == 0)
	{

...
//~ End ISocketSubsystem Interface
	...
	/**
	 * Bind our socket name and ensure no other connections are using this combination
	 *
	 * @return True if we bound successfully, false if not and LastSocketError will be set with a reason
	 */
	bool BindSocketName(const FString& SocketName);

	/**
	 * Unbind a previously bound socket name.
	 *
	 * @return True if we released the port successfully, false if not and LastSocketError will be set with a reason
	 */
	bool UnbindSocketName(const FString& SocketName);
	...
	/**
	 * Returns the utils for this SocketSubsystemEOS
	 *
	 * @return The ISocketSubsystemEOSUtilsPtr for this SocketSubsystemEOS
	 */
	ISocketSubsystemEOSUtilsPtr GetSocketSubsystemEOSUtils() { return Utils; }
...
private:
	...
	/** All bound socket names */
	TSet<FString> BoundSocketNames;

...
bool FSocketSteam::SendTo(const uint8* Data, int32 Count, int32& BytesSent, const FInternetAddr& Destination) 
{
	...
	if (SteamNetworkingPtr)
	{
		...
		if (SteamDest.GetSteamID() != LocalSteamId)
		{
			if (SteamNetworkingPtr->SendP2PPacket(SteamDest.GetSteamID(), Data, Count, SteamSendMode, SteamDest.GetPort()))
			{
				BytesSent = Count;
				bSuccess = true;
			}
...
bool FSocketSteam::RecvFrom(uint8* Data, int32 BufferSize, int32& BytesRead, FInternetAddr& Source, ESocketReceiveFlags::Type Flags) 
{
	...
	{
		SteamAddr.SetSteamID(SteamId);
		if (SocketSubsystem->P2PTouch(SteamNetworkingPtr, SteamAddr.GetSteamID(), SteamChannel))
		{
	...
	SteamAddr.SetPort(SteamChannel);
...
void FSocketSteam::GetAddress(FInternetAddr& OutAddr) 
{
	...
	SteamAddr.SetSteamID(SteamAddr.GetSteamID());
	SteamAddr.SetPort(SteamAddr.GetPort());
}

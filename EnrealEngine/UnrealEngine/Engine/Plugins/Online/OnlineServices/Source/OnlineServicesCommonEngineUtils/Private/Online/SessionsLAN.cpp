...
FSessionsLAN::~FSessionsLAN()
{
	// Stop hosted LAN session if one is running
	if (HostedLANSession)
	{
		StopLANSession();
	}
...
TFuture<TOnlineResult<FCreateSession>> FSessionsLAN::CreateSessionImpl(const FCreateSession::Params& Params)
{
	...
	{
		FHostSessionResult TryHostLANSessionResult = TryHostLANSession();
		if (TryHostLANSessionResult.HasError())
		{
			Promise.EmplaceValue(TryHostLANSessionResult.StealError());
			return Future;
		}

		UE_LOG(LogOnlineServices, Verbose, TEXT("Hosting LAN session named '%s'"), *(Params.SessionName.ToString()));
		HostedLANSession = AddLANSession(Params);
	}
	else
	{
		AddLANSession(Params);
	}
...
TFuture<TOnlineResult<FUpdateSessionSettings>> FSessionsLAN::UpdateSessionSettingsImpl(const FUpdateSessionSettings::Params& Params)
{
	...
	if (Params.Mutations.JoinPolicy.IsSet())
	{
		...
		if (SessionSettings.JoinPolicy != ESessionJoinPolicy::Public && Params.Mutations.JoinPolicy.GetValue() == ESessionJoinPolicy::Public)
		{
			...
			// This session already exists, we are moving it to be a hosted session
			// So we only need to update our hosting session information
			UE_LOG(LogOnlineServices, Verbose, TEXT("Transitioning session %s to a hosted LAN session."), *Params.SessionName.ToString());
			HostedLANSession = StaticCastSharedRef<FSessionLAN>(FoundSession).ToSharedPtr();
		}
		...
		{
			// Stop hosting the LAN session, since it's no longer public
			LANSessionManager->StopLANSession();
			HostedLANSession.Reset();
			UE_LOG(LogOnlineServices, Verbose, TEXT("No longer hosting LAN session %s as it's no longer publicly accessible."), *Params.SessionName.ToString());
		}
	}
...
TFuture<TOnlineResult<FFindSessions>> FSessionsLAN::FindSessionsImpl(const FFindSessions::Params& Params)
{
	...
	// If we were hosting public sessions before the search, add it to the search results
	if (HostedLANSession)
	{
		UE_LOG(LogOnlineServices, Verbose, TEXT("[FSessionsLAN::FindSessionsImpl] Adding local hosted LAN session with ID %s to search results."), *ToString(HostedLANSession->GetSessionId()));
		SearchResultsUserMap.FindOrAdd(Params.LocalAccountId).Add(HostedLANSession->GetSessionId());
	}
...
FSessionsLAN::FHostSessionResult FSessionsLAN::TryHostLANSession()
{
	...
	if (LANSessionManager->GetBeaconState() == ELanBeaconState::NotUsingLanBeacon)
	{
		...
		else
		{
			...
			return MakeError(Errors::RequestFailure());
		}
	}
	else
	{
		UE_LOG(LogOnlineServices, Warning, TEXT("[FSessionsLAN::TryHostLANSession] LAN Beacon already in use!"));
		return MakeError(Errors::AlreadyPending());
	}
}

TSharedRef<FSessionLAN> FSessionsLAN::AddLANSession(const FCreateSession::Params& Params)
{
...
void FSessionsLAN::FindLANSessions(const FAccountId& LocalAccountId)
{
	...
	if (LANSessionManager->Search(Packet, ResponseDelegate, TimeoutDelegate) == false)
	{
		...
		CurrentSessionSearchPromisesUserMap.FindAndRemoveChecked(LocalAccountId).EmplaceValue(Errors::RequestFailure());
...
void FSessionsLAN::OnLANSearchTimeout(const FAccountId LocalAccountId)
{
	...
	const TArray<FOnlineSessionId>& FoundSessionIds = SearchResultsUserMap.FindChecked(LocalAccountId);
	UE_LOG(LogOnlineServices, Verbose, TEXT("[FSessionsLAN::OnLANSearchTimeout] %d sessions found!"), FoundSessionIds.Num());

	CurrentSessionSearchPromisesUserMap.FindAndRemoveChecked(LocalAccountId).EmplaceValue(FFindSessions::Result{ FoundSessionIds });

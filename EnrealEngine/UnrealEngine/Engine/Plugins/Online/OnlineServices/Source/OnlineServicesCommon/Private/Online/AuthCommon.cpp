...
void FAuthCommon::RegisterCommands()
{
	...
	RegisterCommand(&FAuthCommon::CreateAccount);
...
TOnlineAsyncOpHandle<FAuthCreateAccount> FAuthCommon::CreateAccount(FAuthCreateAccount::Params&& Params)
{
	TOnlineAsyncOpRef<FAuthCreateAccount> Operation = GetOp<FAuthCreateAccount>(MoveTemp(Params));
	Operation->SetError(Errors::NotImplemented());
	return Operation->GetHandle();
}

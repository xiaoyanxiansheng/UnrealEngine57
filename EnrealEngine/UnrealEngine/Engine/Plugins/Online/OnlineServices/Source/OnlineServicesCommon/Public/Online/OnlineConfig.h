...
class FOnlineConfigStructGConfig : public IOnlineConfigStruct
{
public:
	virtual ~FOnlineConfigStructGConfig() = default;

	UE_API virtual bool GetValue(const TCHAR* Key, FString& Value) override;
	UE_API virtual int32 GetValue(const TCHAR* Key, TArray<FString>& Value) override;
	UE_API virtual bool GetValue(const TCHAR * Key, IOnlineConfigStructPtr& Value) override;
	UE_API virtual int32 GetValue(const TCHAR* Key, TArray<IOnlineConfigStructPtr>& Value) override;
...
private:
	...
	static UE_API IOnlineConfigStructPtr CreateStruct(const FString& InConfigValue);
	static UE_API TArray<IOnlineConfigStructPtr> CreateStructArray(const FString& InConfigValue);
	static UE_API const TCHAR* ParseStruct(const TCHAR* InStr, IOnlineConfigStructPtr& OutPtr);
	static UE_API TArray<FString> CreateValueArray(const FString& InConfigValue);
 ...
 */
class FOnlineConfigProviderGConfig : public IOnlineConfigProvider
{
public:
	...
	UE_API virtual bool GetValue(const TCHAR* Section, const TCHAR* Key, FString& Value) override;
	UE_API virtual int32 GetValue(const TCHAR* Section, const TCHAR* Key, TArray<FString>& Value) override;
	UE_API virtual bool GetValue(const TCHAR* Section, const TCHAR* Key, IOnlineConfigStructPtr& Value) override;
	UE_API virtual int32 GetValue(const TCHAR* Section, const TCHAR* Key, TArray<IOnlineConfigStructPtr>& Value) override;
	...
	-> std::enable_if_t<TModels_V<Meta::COnlineMetadataAvailable, T>, bool>
{
	...
	{
		bool bLoadedInCurrentSection = LoadConfig(Provider, Section, Value);
		if (bLoadedInCurrentSection && Section.Contains(" "))
		{
			UE_LOG(LogOnlineServices, Warning, TEXT("Space in online config section has been deprecated, use : instead in [%s]"), *Section);
		}
		bLoadedValue |= bLoadedInCurrentSection;
	}
	return bLoadedValue;
}

...
{
	FLobbyMemberInternal(const TSharedRef<const FSchemaRegistry>& SchemaRegistry, const FSchemaId& DerivedSchemaId = FSchemaId())
		: FLobbyMember()
		, SchemaData(DerivedSchemaId, LobbyBaseSchemaId, LobbyMemberSchemaCategoryId, SchemaRegistry, Attributes)
	{
	}
...
struct FLobbyClientData final
{
public:
	UE_API FLobbyClientData(FLobbyId LobbyId, const TSharedRef<const FSchemaRegistry>& InSchemaRegistry);
	...
	UE_API TSharedPtr<const FLobbyMemberInternal> GetMemberData(FAccountId MemberAccountId) const;
	 ...
	 */
	UE_API TOnlineResult<FLobbyClientDataPrepareServiceSnapshot> PrepareServiceSnapshot(FLobbyClientDataPrepareServiceSnapshot::Params&& Params) const;
	UE_API FLobbyClientDataCommitServiceSnapshot::Result CommitServiceSnapshot(FLobbyClientDataCommitServiceSnapshot::Params&& Params);

	UE_API TOnlineResult<FLobbyClientDataPrepareClientChanges> PrepareClientChanges(FLobbyClientDataPrepareClientChanges::Params&& Params) const;
	UE_API FLobbyClientDataCommitClientChanges::Result CommitClientChanges(FLobbyClientDataCommitClientChanges::Params&& Params);

private:
	UE_API void ResetPreparedChanges() const;
	UE_API void AddMember(const TSharedRef<FLobbyMemberInternal>& Member);
	UE_API void RemoveMember(const TSharedRef<FLobbyMemberInternal>& Member);

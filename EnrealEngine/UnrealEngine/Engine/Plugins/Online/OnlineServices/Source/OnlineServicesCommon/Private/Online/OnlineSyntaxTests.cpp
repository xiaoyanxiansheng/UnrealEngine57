...
struct FTestOp2
{
	static constexpr TCHAR Name[] = TEXT("TestOp2");
	struct Params {};
	struct Result {};
};
...
inline void TestAsyncOpContinuationSyntax(FOnlineServicesCommon& Services)
{
	...
	ChainableOp.Then([](FOnlineAsyncOp&) {})
		...
		.ForEachN(2, [](TOnlineAsyncOp<FTestOp>&, TArrayView<int> Value) { return 1; })
		.ForEachN(2, [](TOnlineAsyncOp<FTestOp>&, TArrayView<int> Value) { return MakeFulfilledPromise<int>(1).GetFuture(); })
		.ForEachN(2, [](TOnlineAsyncOp<FTestOp>&, TArrayView<int> Value, TPromise<int>&& Promise) { Promise.SetValue(1); })
		.ForEachN(2, [](TOnlineAsyncOp<FTestOp>&, TArrayView<int> Value) { TArray<int> IntArray = { 1, 2 }; return TArray<int>(IntArray); })
	...
	Op.GetHandle();
	Op.GetWrappedHandle<FTestOp2>([](const TOnlineResult<FTestOp>& Result) { return TOnlineResult<FTestOp2>(FTestOp2::Result()); });
}

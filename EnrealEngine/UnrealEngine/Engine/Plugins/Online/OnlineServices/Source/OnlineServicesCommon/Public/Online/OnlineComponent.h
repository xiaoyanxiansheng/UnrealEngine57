...
public:
	...
	{
		TOnlineAsyncOpRef<OpType> Op = Services.template GetOp<OpType>(MoveTemp(Params), GetConfigSectionHeirarchy(OpType::Name));
		Op->SetInterfaceName(InterfaceName);
		return Op;
	}
	...
	{
		TOnlineAsyncOpRef<OpType> Op = Services.template GetJoinableOp<OpType, ParamsFuncsType>(MoveTemp(Params), GetConfigSectionHeirarchy(OpType::Name));
		Op->SetInterfaceName(InterfaceName);
		return Op;
	}
	...
	{
		TOnlineAsyncOpRef<OpType> Op = Services.template GetMergeableOp<OpType, ParamsFuncsType>(MoveTemp(Params), GetConfigSectionHeirarchy(OpType::Name));
		Op->SetInterfaceName(InterfaceName);
		return Op;
	}
	...
	TArray<FString> GetConfigSectionHeirarchy(const FString& OperationName = FString())
	{
	...
	void RegisterCommand(T MemberFunction)
	{
		...
		RegisterCommand(static_cast<InterfaceType*>(this), MemberFunction);
	}

	// Default handler: generic parsing of Params, log Result
	template <typename InterfaceType, typename T>
	void RegisterCommand(InterfaceType* Interface, T MemberFunction)
	{
		TOnlineInterfaceOperationExecHandler<InterfaceType, T>* Handler = new TOnlineInterfaceOperationExecHandler<InterfaceType, T>(Interface, MemberFunction);

...
namespace UE::Online {
	...
	void FSessionsCommon::RegisterCommands()
	{
		...
		RegisterCommand(&FSessionsCommon::GetSessionName);
...
#define CHECK_PARAMS_FNAME(Name, MethodName) \
	...
	TOnlineResult<FGetSessionName> FSessionsCommon::GetSessionName(FGetSessionName::Params&& Params) const
	{
		if (const FName* SessionName = LocalSessionsByName.FindKey(Params.SessionId))
		{
			return TOnlineResult<FGetSessionName>({ *SessionName });
		}
		else
		{
			return TOnlineResult<FGetSessionName>(Errors::NotFound());
		}
	}

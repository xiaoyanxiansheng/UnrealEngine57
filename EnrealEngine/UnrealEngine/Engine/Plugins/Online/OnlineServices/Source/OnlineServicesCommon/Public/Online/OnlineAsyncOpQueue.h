...
public:
	...
	void Enqueue(TOnlineAsyncOp<OpType>& Op, double DelayInSeconds = 0.0)
	{
		QueuedOperations.Enqueue(MakeShared<TWrappedOp<OpType>>(Op, DelayInSeconds));
		TryStartOperations();
	}
...
protected:
	...
	public:
		...
		TWrappedOp(TOnlineAsyncOp<OpType>& InOp, double InDelayInSeconds)
			: Op(InOp.AsShared())
			, DelayInSeconds(InDelayInSeconds)
			, EnqueueCycles(InDelayInSeconds > 0.0 ? FPlatformTime::Cycles64() : 0)
		{
		}
		...
		virtual void Start() override final
		{
			...
			{
				const double ElapsedTime = FPlatformTime::ToSeconds64(FPlatformTime::Cycles64() - EnqueueCycles);
				if (DelayInSeconds > ElapsedTime)
				{
					TickerHandle = FTSTicker::GetCoreTicker().AddTicker(TEXT("StartDelayedOnlineAsyncOp"), DelayInSeconds - ElapsedTime,
						[WeakOp = Op.ToWeakPtr()](float)
						{
	...
	private:
		...
		const double DelayInSeconds = 0.0;
		const uint64 EnqueueCycles = 0;
		TOnlineEventCallable<void(void)> OnCompleteEvent;
		FOnlineEventDelegateHandle OnCompleteHandle;
		FTSTicker::FDelegateHandle TickerHandle;
	};

	TTransactionallySafeSpscQueue<TSharedRef<IWrappedOp>> QueuedOperations;
};

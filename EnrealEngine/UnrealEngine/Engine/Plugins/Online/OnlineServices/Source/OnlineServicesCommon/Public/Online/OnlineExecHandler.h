...
/*
	...
	Full command examples:
		OnlineServices Index=0 Auth Login Null username s:pwd s:token []
...
inline bool ParseOnlineExecParams(const TCHAR*& Cmd, FUtf8String& Value, IOnlineServices* Services = nullptr)
{
	FString StringValue;
	if (!ParseOnlineExecParams(Cmd, StringValue))
	{
	...
	Value = StringCast<UTF8CHAR>(*StringValue, StringValue.Len());
	return true;
}
...
template <typename InterfaceType, typename MemberFunctionPtrType>

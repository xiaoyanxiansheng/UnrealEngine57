...
void LogSchemaDefinition(const FSchemaDefinition& Definition)
{
	if (UE_LOG_ACTIVE(LogOnlineSchema, Verbose))
	{
		auto GetParentString = [](const FSchemaDefinition& Definition) -> FString {
			TArray<FString> ParentNames;
			Algo::Transform(Definition.ParentSchemaIds, ParentNames, [](const FSchemaId& ParentSchemaId){ return ParentSchemaId.ToString(); });
			return FString::Join(ParentNames, TEXT(","));
		};

		auto GetCategoryString = [](const FSchemaDefinition& Definition) -> FString {
			TArray<FString> CategoryNames;
			Algo::Transform(Definition.Categories, CategoryNames, [](const TPair<FSchemaCategoryId, FSchemaCategoryDefinition>& CategoryPair){ return CategoryPair.Key.ToString(); });
			return FString::Join(CategoryNames, TEXT(","));
		};

		auto GetFlagsString = [](ESchemaAttributeFlags Flags) -> FString {
			return FString::JoinBy(MakeFlagsRange(Flags), TEXT("|"), [](ESchemaAttributeFlags Flag){ return LexToString(Flag); });
		};

		const FString SchemaName = Definition.Id.ToString();
		const FString ParentSchema = GetParentString(Definition);
		UE_LOG(LogOnlineSchema, Verbose, TEXT("[SchemaDefinition] SchemaName: [%s]"), *SchemaName);
		UE_LOG(LogOnlineSchema, Verbose, TEXT("[SchemaDefinition][%s] CompatabilityId: [0x%08" INT64_X_FMT "]"), *SchemaName, Definition.CompatibilityId);
		if (!ParentSchema.IsEmpty())
		{
			UE_LOG(LogOnlineSchema, Verbose, TEXT("[SchemaDefinition][%s] Parent: [%s]"), *SchemaName, *ParentSchema);
		}
		UE_LOG(LogOnlineSchema, Verbose, TEXT("[SchemaDefinition][%s] Categories: [%s]"), *SchemaName, *GetCategoryString(Definition));
		for (const TPair<FSchemaCategoryId, FSchemaCategoryDefinition>& CategoryPair : Definition.Categories)
		{
			const FSchemaCategoryDefinition& Category = CategoryPair.Value;
			const FString CategoryName = Category.Id.ToString();
			if (Category.SchemaCompatibilityAttributeId != FSchemaId())
			{
				UE_LOG(LogOnlineSchema, Verbose, TEXT("[SchemaDefinition][%s][%s] SchemaCompatibilityAttributeId: [%s]"), *SchemaName, *CategoryName, *Category.SchemaCompatibilityAttributeId.ToString());
			}
			if (Category.SchemaCompatibilityServiceAttributeId != FSchemaId())
			{
				UE_LOG(LogOnlineSchema, Verbose, TEXT("[SchemaDefinition][%s][%s] SchemaCompatibilityServiceAttributeId: [%s]"), *SchemaName, *CategoryName, *Category.SchemaCompatibilityServiceAttributeId.ToString());
			}

			for (const TPair<FSchemaAttributeId, FSchemaAttributeDefinition>& AttributeDefinitionPair : Category.SchemaAttributeDefinitions)
			{
				const FSchemaAttributeDefinition& AttributeDefinition = AttributeDefinitionPair.Value;
				UE_LOG(LogOnlineSchema, Verbose, TEXT("[SchemaDefinition][%s][%s][%s] Type: [%s], Flags: [%s], MaxSize: [%d], ServiceAttributeId: [%s]"),
				*SchemaName, *CategoryName, *AttributeDefinition.Id.ToString(), LexToString(AttributeDefinition.Type), *GetFlagsString(AttributeDefinition.Flags), AttributeDefinition.MaxSize, *AttributeDefinition.ServiceAttributeId.ToString());
			}
		}
	}
}
...
bool FSchemaRegistry::ParseConfig(const FSchemaRegistryDescriptorConfig& Config)
{
	...
	for (const FSchemaServiceAttributeDescriptor& SchemaServiceAttributeDescriptor : Config.ServiceAttributeDescriptors)
	{
		...
		{
			UE_LOG(LogOnlineSchema, Error, TEXT("Duplicate schema service attribute descriptor found: [%s]"), *SchemaServiceAttributeDescriptor.Id.ToString());
	...
	for (const FSchemaServiceDescriptor& SchemaServiceDescriptor : Config.ServiceDescriptors)
	{
		...
		{
			UE_LOG(LogOnlineSchema, Error, TEXT("Duplicate schema service descriptor found: [%s]"), *SchemaServiceDescriptor.Id.ToString());
			ParsedSuccessfully = false;
			continue;
		}
		...
		for (const FSchemaServiceAttributeId& SchemaServiceAttributeId : SchemaServiceDescriptor.AttributeIds)
		{
			...
			if (SeenAttributes.Contains(SchemaServiceAttributeId))
			{
				UE_LOG(LogOnlineSchema, Error, TEXT("Invalid schema service attribute %s.%s: Attribute id has already been used."),
					*SchemaServiceDescriptor.Id.ToString(),
					*SchemaServiceAttributeId.ToString());
				ParsedSuccessfully = false;
				continue;
			}
			...
			if (SupportedTypes == ESchemaServiceAttributeSupportedTypeFlags::None)
			{
				UE_LOG(LogOnlineSchema, Error, TEXT("Invalid schema service attribute %s.%s: A valid supported type must be selected."),
					*SchemaServiceDescriptor.Id.ToString(),
					*SchemaServiceAttributeDescriptor.Id.ToString());
				ParsedSuccessfully = false;
			}
	...
	for (const FSchemaCategoryDescriptor& SchemaCategoryDescriptor : Config.SchemaCategoryDescriptors)
	{
		...
		{
			UE_LOG(LogOnlineSchema, Error, TEXT("Duplicate schema category descriptor found: %s"), *SchemaCategoryDescriptor.Id.ToString());
			ParsedSuccessfully = false;
			continue;
		}
			...
			&& !KnownSchemaServiceDescriptors.Contains(SchemaCategoryDescriptor.ServiceDescriptorId))
		{
			UE_LOG(LogOnlineSchema, Error, TEXT("Invalid schema category %s: Service descriptor id %s not found."),
				*SchemaCategoryDescriptor.Id.ToString(),
				*SchemaCategoryDescriptor.ServiceDescriptorId.ToString());
			ParsedSuccessfully = false;
		}
	}
	...
	for (const FSchemaDescriptor& SchemaDescriptor : Config.SchemaDescriptors)
	{
		...
		{
			UE_LOG(LogOnlineSchema, Error, TEXT("Duplicate schema descriptor found: %s"), *SchemaDescriptor.Id.ToString());
			ParsedSuccessfully = false;
			continue;
		}
	...
	for (const FSchemaAttributeDescriptor& SchemaAttributeDescriptor : Config.SchemaAttributeDescriptors)
	{
		...
		{
			UE_LOG(LogOnlineSchema, Error, TEXT("Duplicate schema attribute descriptor found: %s"), *SchemaAttributeDescriptor.Id.ToString());
			ParsedSuccessfully = false;
			continue;
		}
		...
		if (SchemaAttributeDescriptor.Type == ESchemaAttributeType::None)
		{
			UE_LOG(LogOnlineSchema, Error, TEXT("Invalid schema attribute %s: A valid type must be set."),
				*SchemaAttributeDescriptor.Id.ToString());
			ParsedSuccessfully = false;
		}
	...
	for (const FSchemaCategoryAttributesDescriptor& SchemaCategoryAttributesDescriptor : Config.SchemaCategoryAttributeDescriptors)
	{
		...
		if (FoundSchema && FoundSchema->Contains(SchemaCategoryAttributesDescriptor.CategoryId))
		{
			UE_LOG(LogOnlineSchema, Error, TEXT("Duplicate schema additional category attributes descriptor found: %s.%s"),
				*SchemaCategoryAttributesDescriptor.SchemaId.ToString(),
				*SchemaCategoryAttributesDescriptor.CategoryId.ToString());
			ParsedSuccessfully = false;
			continue;
		}
	...
	for (const TPair<FSchemaId, const FSchemaDescriptor*>& SchemaDescriptorPair : KnownSchemaDescriptors)
	{
		...
		while (TestSchemaDescriptor)
		{
			...
			{
				UE_LOG(LogOnlineSchema, Error, TEXT("Circular parent dependency found in schema: %s"), *SchemaDescriptorToVerify.Id.ToString());
				ParsedSuccessfully = false;
				break;
			}
			...
			if (TestSchemaDescriptor->ParentId != FSchemaId())
			{
				...
				else
				{
					UE_LOG(LogOnlineSchema, Error, TEXT("Invalid schema %s: Parent schema %s does not exist."),
						*SchemaDescriptorToVerify.Id.ToString(),
						*TestSchemaDescriptor->ParentId.ToString());
					ParsedSuccessfully = false;
					break;
				}
			}
					...
					if (EnumHasAllFlags(AttributeFlags, ESchemaAttributeFlags::SchemaCompatibilityId))
					{
						...
						if (SeenSchemaCompatibilityAttributeId != FSchemaId())
						{
							UE_LOG(LogOnlineSchema, Error, TEXT("Invalid schema attribute %s.%s.%s: SchemaCompatibilityId field has already been used by attribute %s."),
								...
								*SchemaCategoryDescriptor.Id.ToString(),
								*SchemaAttributeDescriptor.Id.ToString(),
								*SeenSchemaCompatibilityAttributeId.ToString());
							ParsedSuccessfully = false;
						}
						else
						{
		...
		for (const TPair<FSchemaId, const FSchemaDescriptor*>& SchemaDescriptorPair : KnownSchemaDescriptors)
		{
						...
						for (const FSchemaAttributeId& SchemaAttributeId : *FoundCategoryAttributeIds)
						{
							...
							else
							{
								UE_LOG(LogOnlineSchema, Error, TEXT("Failed to find service attribute to fit schema attribute %s.%s.%s."),
									...
									*SchemaCategoryDefinition.Id.ToString(),
									*SchemaAttributeDefinition.Id.ToString());
								ParsedSuccessfully = false;
							}
						}
					}
			...
			// Build compatibility id from the schema and data CRCs.
			{
				...
				UE_LOG(LogOnlineSchema, Verbose, TEXT("Building compatability id: SchemaId: [%s], SchemaIdCrc: [0x%04X], SchemaDataCrc [0x%04X]."),
					*SchemaDefinition->Id.ToString(), SchemaIdCrc, SchemaDataCrc);
			}
			...
			UE_LOG(LogOnlineSchema, Log, TEXT("Parsed Schema definition: SchemaId: [%s], SchemaCompatabilityId: [0x%08" INT64_X_FMT "]"),
				*SchemaDefinition->Id.ToString(), SchemaDefinition->CompatibilityId);

			LogSchemaDefinition(*SchemaDefinition);
		}
	}
	...
	: SchemaRegistry(SchemaRegistry)
{
	...
	if (!bInitSuccess)
	{
		UE_LOG(LogOnlineSchema, Error, TEXT("[FSchemaCategoryInstance] init failed. DerivedSchema[%s], BaseSchema[%s], CategoryId[%s]"),
			*DerivedSchemaId.ToString(),
			*BaseSchemaId.ToString(),
			*CategoryId.ToString());
	...
	if (Params.ClientChanges.SchemaId)
	{
		...
		else
		{
			UE_LOG(LogOnlineSchema, Warning, TEXT("[PrepareClientChanges] Schema change failed: NewDerivedSchema[%s], BaseSchema[%s]"),
				*Params.ClientChanges.SchemaId->ToString(),
				*BaseSchemaCategoryDefinition->Id.ToString());
		...
		if (AttributeDefinition->ServiceAttributeId != FSchemaServiceAttributeId())
		{
			...
			else
			{
				UE_LOG(LogOnlineSchema, Warning, TEXT("[PrepareClientChanges] %s.%s.%s: Unable to find service attribute definition for service attribute %s."),
					...
					*SchemaCategoryDefinition->Id.ToString(),
					*AttributeDefinition->Id.ToString(),
					*AttributeDefinition->ServiceAttributeId.ToString());
				return TOnlineResult<FSchemaCategoryInstancePrepareClientChanges>(Errors::InvalidState());
			}
		}
	}
	...
	if (SchemaCategoryDefinition->SchemaCompatibilityAttributeId != FSchemaAttributeId() && LastSentSchemaCompatibilityId == 0)
	{
		...
		else
		{
			UE_LOG(LogOnlineSchema, Warning, TEXT("[PrepareClientChanges] %s.%s.%s: Failed to find attribute definition for SchemaCompatibilityId field."),
				*SchemaDefinition->Id.ToString(),
				*SchemaCategoryDefinition->Id.ToString(),
				*SchemaCategoryDefinition->SchemaCompatibilityAttributeId.ToString());
			return TOnlineResult<FSchemaCategoryInstancePrepareClientChanges>(Errors::InvalidState());
		}
	}
	...
	if (SchemaCompatibilityAttributeData)
	{
		...
		else
		{
			UE_LOG(LogOnlineSchema, Warning, TEXT("[PrepareServiceSnapshot] %s.%s.%s: Failed to find valid definition for SchemaCompatibilityId 0x%08" INT64_X_FMT "."),
				*SchemaDefinition->Id.ToString(),
				*SchemaCategoryDefinition->Id.ToString(),
				*SchemaCategoryDefinition->SchemaCompatibilityServiceAttributeId.ToString(),
				SchemaCompatibilityId);
	...
	for (TPair<FSchemaServiceAttributeId, FSchemaVariant>& SchemaServiceAttributeData : Params.ServiceSnapshot.Attributes)
	{
		...
		else
		{
			UE_LOG(LogOnlineSchema, Warning, TEXT("[PrepareServiceSnapshot] %s.%s.%s: Service attribute definition does not exist in schema category."),
				...
				*SchemaCategoryDefinition->Id.ToString(),
				*SchemaServiceAttributeId.ToString());
			// Allow code to continue when an unexpected attribute is received.
			continue;
		}
	}
		...
		if (const FSchemaAttributeDefinition* AttributeDefinition = BaseSchemaCategoryDefinition->SchemaAttributeDefinitions.Find(Id))
		{
			...
			else
			{
				UE_LOG(LogOnlineSchema, Verbose, TEXT("[VerifyBaseAttributeData] Schema attribute %s.%s.%s set with invalid type %s. Expected type %s"),
					*BaseSchemaDefinition->Id.ToString(),
					*BaseSchemaCategoryDefinition->Id.ToString(),
					*AttributeDefinition->Id.ToString(),
	...
	const FSchemaCategoryDefinition** OutSchemaCategoryDefinition) const
{
	...
	if (OptionalBaseDefinition && !SchemaRegistry->IsSchemaChildOf(NewDefinition->Id, OptionalBaseDefinition->Id))
	{
		UE_LOG(LogOnlineSchema, Warning, TEXT("[InitializeSchemaDefinition] Invalid schema definition. New schema %s is not a child of %s."),
			*NewDefinition->Id.ToString(),
			*OptionalBaseDefinition->Id.ToString());
		return false;
	}

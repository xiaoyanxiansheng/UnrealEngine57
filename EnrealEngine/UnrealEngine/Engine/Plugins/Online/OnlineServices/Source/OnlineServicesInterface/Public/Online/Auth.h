...
struct FAuthCreateAccount
{
	static constexpr TCHAR Name[] = TEXT("CreateAccount");
...
public:
	 ...
	 * passed to either CreateAccount or LinkAccount.
	 */
	...
	/**
	* Create a new account and link it to an external auth method using a continuation token from a previous login attempt.
	*/
	virtual TOnlineAsyncOpHandle<FAuthCreateAccount> CreateAccount(FAuthCreateAccount::Params&& Params) = 0;
...
BEGIN_ONLINE_STRUCT_META(FAuthCreateAccount::Params)
	ONLINE_STRUCT_FIELD(FAuthCreateAccount::Params, PlatformUserId),
	ONLINE_STRUCT_FIELD(FAuthCreateAccount::Params, ContinuationId)
END_ONLINE_STRUCT_META()

BEGIN_ONLINE_STRUCT_META(FAuthCreateAccount::Result)
	ONLINE_STRUCT_FIELD(FAuthCreateAccount::Result, AccountInfo)
END_ONLINE_STRUCT_META()

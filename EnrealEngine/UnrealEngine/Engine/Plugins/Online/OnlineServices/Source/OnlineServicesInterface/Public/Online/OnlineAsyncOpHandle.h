...
inline const TCHAR* LexToString(EAsyncOpState Value)
{
	switch (Value)
	{
	case EAsyncOpState::Queued: return TEXT("Queued");
	case EAsyncOpState::Running: return TEXT("Running");
	case EAsyncOpState::RetryQueued: return TEXT("RetryQueued");
	case EAsyncOpState::Complete: return TEXT("Complete");
	case EAsyncOpState::Cancelled: return TEXT("Cancelled");
	default: checkNoEntry(); // Intentional fall through
	case EAsyncOpState::Invalid: return TEXT("Invalid");
	}
}

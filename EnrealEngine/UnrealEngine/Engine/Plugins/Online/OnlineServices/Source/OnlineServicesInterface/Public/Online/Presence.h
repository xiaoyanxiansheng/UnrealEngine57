...
#define SENT_TIME_ATTRIBUTE_KEY FString(TEXT("SentTimeAttributeKey"))
#define SESSION_ID_ATTRIBUTE_KEY FString(TEXT("SessionIdAttributeKey"))

namespace UE::Online {

using FPresenceVariant = FNestedVariantJson;
...
ONLINESERVICESINTERFACE_API FText ToLocText(EUserPresenceStatus EnumVal);
...
using FPresenceProperties = FPresenceVariant::FMap;
using FPresencePropertiesRef = FPresenceVariant::FMapRef;
using FPresencePropertiesPtr = FPresenceVariant::FMapPtr;
using FPresenceProperty = FPresenceProperties::ValueType;
...
struct FUserPresence
{
	...
	EOnlinePlatformType PlatformType = EOnlinePlatformType::Unknown;
...
inline FPartialUpdatePresence::Params::FMutations& FPartialUpdatePresence::Params::FMutations::operator+=(FPartialUpdatePresence::Params::FMutations&& NewParams)
{
	...
	for (TPair<FString, FPresenceProperty>& NewUpdatedProperty : NewParams.UpdatedProperties)
	{
...
inline TSharedRef<FUserPresence> ApplyPresenceMutations(const FUserPresence& BasePresence, const FPartialUpdatePresence::Params::FMutations& Mutations)
{
	...
	*NewPresence = BasePresence;
	...
	{
		NewPresence->RichPresenceString = Mutations.RichPresenceString.GetValue();
	}

	// Insert any updated keys / remove from removed
	for (const TPair<FString, FPresenceProperty>& NewUpdatedProperty : Mutations.UpdatedProperties)
	{
...
BEGIN_ONLINE_STRUCT_META(FUserPresence)
	...
	ONLINE_STRUCT_FIELD(FUserPresence, PlatformType),

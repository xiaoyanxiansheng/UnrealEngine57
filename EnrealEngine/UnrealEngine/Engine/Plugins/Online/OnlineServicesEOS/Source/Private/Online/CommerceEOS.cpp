...
TOnlineAsyncOpHandle<FCommerceQueryEntitlements> FCommerceEOS::QueryEntitlements(FCommerceQueryEntitlements::Params&& Params)
{
	...
	if (!Op->IsReady())
	{
		...
		.Then([this](TOnlineAsyncOp<FCommerceQueryEntitlements>& Op, const EOS_Ecom_QueryEntitlementsCallbackInfo* Data)
		{
			...
			UE_LOG(LogOnlineServices, VeryVerbose, TEXT("Entitlements Count: %d"), Count);
			...
			for (uint32 Index = 0; Index < Count; Index++)
			{
				...
				UE_LOG(LogOnlineServices, VeryVerbose, TEXT("	Entitlement %d:"), Index);
				...
				UE_LOG(LogOnlineServices, VeryVerbose, TEXT("		Entitlement Id: %s"), *Entitlement.EntitlementId);
				UE_LOG(LogOnlineServices, VeryVerbose, TEXT("		Product Id: %s"), *Entitlement.ProductId);

				EOS_Ecom_Entitlement_Release(EosEntitlement);
			}
		})
		.Then([this](TOnlineAsyncOp<FCommerceQueryEntitlements>& Op, TPromise<const EOS_Ecom_QueryOwnershipBySandboxIdsCallbackInfo*>&& Promise)
		{
			const FCommerceQueryEntitlements::Params& Params = Op.GetParams();
			...
			if (IEOSSDKManager* Manager = IEOSSDKManager::Get())
			{
				...
				{
					const FTCHARToUTF8& Converter = FTCHARToUTF8(*Config->SandboxId);
					const char* SandboxId = Converter.Get();
					Options.SandboxIds = &SandboxId;
				}
			}
			
			EOS_Async(EOS_Ecom_QueryOwnershipBySandboxIds, EcomHandle, Options, MoveTemp(Promise));
		})
		.Then([this](TOnlineAsyncOp<FCommerceQueryEntitlements>& Op, const EOS_Ecom_QueryOwnershipBySandboxIdsCallbackInfo* Data)
		{
			const EOS_EResult Result = Data->ResultCode;
			if (Result != EOS_EResult::EOS_Success)
			{
				UE_LOG(LogOnlineServices, Error, TEXT("EOS_Ecom_QueryOwnership: failed with error (%s)"), *LexToString(Data->ResultCode));
				Op.SetError(Errors::FromEOSResult(Result));
				return;
			}
			...
			TArray<FEntitlement>& Entitlements = CachedEntitlements.FindOrAdd(Params.LocalAccountId, TArray<FEntitlement>());
			
			UE_LOG(LogOnlineServices, VeryVerbose, TEXT("Sandbox Id Ownership Count: %d"), Data->SandboxIdItemOwnershipsCount);
			...
			{
				UE_LOG(LogOnlineServices, VeryVerbose, TEXT("	Sandbox Id: %hs"), SandboxOwnershipEntry.SandboxId);
				UE_LOG(LogOnlineServices, VeryVerbose, TEXT("	Ownership Count: %d"), SandboxOwnershipEntry.OwnedCatalogItemIdsCount);
				...
				for (uint32_t CatalogItemIndex = 0; CatalogItemIndex < SandboxOwnershipEntry.OwnedCatalogItemIdsCount; CatalogItemIndex++)
				{
					...
					FEntitlement& Entitlement = Entitlements.Emplace_GetRef();
					Entitlement.bRedeemed = true;
					Entitlement.ProductId = FString(CatalogItemId);
					Entitlement.Quantity = 1;

					UE_LOG(LogOnlineServices, VeryVerbose, TEXT("		Catalog Item %d"), CatalogItemIndex);
					UE_LOG(LogOnlineServices, VeryVerbose, TEXT("			Catalog Item Id: %hs"), CatalogItemId);
				}
			}

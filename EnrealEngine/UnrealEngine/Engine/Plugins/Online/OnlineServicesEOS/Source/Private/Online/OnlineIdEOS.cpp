...
FAccountId FOnlineAccountIdRegistryEOS::FromStringData(const FString& StringData)
{
	FString EpicAccountIdString;
	FString ProductUserIdString;

	StringData.Split(TEXT(":"), &EpicAccountIdString, &ProductUserIdString);

	EOS_EpicAccountId EpicAccountId = nullptr;
	if (!EpicAccountIdString.IsEmpty())
	{
		const auto Utf8EpicAccountId = StringCast<UTF8CHAR>(*EpicAccountIdString);
		EpicAccountId = EOS_EpicAccountId_FromString((const char*)Utf8EpicAccountId.Get());
	}

	EOS_ProductUserId ProductUserId = nullptr;
	if (!ProductUserIdString.IsEmpty())
	{
		const auto Utf8ProductUserId = StringCast<UTF8CHAR>(*ProductUserIdString);
		ProductUserId = EOS_ProductUserId_FromString((const char*)Utf8ProductUserId.Get());
	}

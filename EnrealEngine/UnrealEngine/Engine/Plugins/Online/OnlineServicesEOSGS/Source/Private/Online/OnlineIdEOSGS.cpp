...
FAccountId FOnlineAccountIdRegistryEOSGS::FromStringData(const FString& StringData)
{
	const auto Utf8ProductUserId = StringCast<UTF8CHAR>(*StringData);
	EOS_ProductUserId ProductUserId = EOS_ProductUserId_FromString((const char*)Utf8ProductUserId.Get());
	return FindOrAddAccountId(ProductUserId);
}

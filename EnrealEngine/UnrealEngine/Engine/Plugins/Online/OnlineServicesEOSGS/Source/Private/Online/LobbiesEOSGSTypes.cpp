...
FLobbyBucketIdEOS::FLobbyBucketIdEOS(const FString& InProductName, int32 InProductVersion)
	: ProductName(InProductName.Replace(UE_ONLINE_LOBBIES_BUCKET_ID_EOS_SEPARATOR, TEXT("_")))
...
{
	if (!Prerequisites->ResolverInterface)
	{
		UE_LOG(LogOnlineServices, Warning, TEXT("[FLobbyDetailsEOS::GetLobbySnapshot] Failed: Resolver interface missing. Lobby[%s]"),
	...
	Prerequisites->ResolverInterface->ResolveAccountIds(AssociatedLocalUser, *MemberProductUserIds)

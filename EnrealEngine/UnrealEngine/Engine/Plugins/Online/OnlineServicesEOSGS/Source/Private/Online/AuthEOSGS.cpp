...
TOnlineAsyncOpHandle<FAuthLogout> FAuthEOSGS::Logout(FAuthLogout::Params&& Params)
{
	...
	.Then([this](TOnlineAsyncOp<FAuthLogout>& InAsyncOp)
	{
		...
		if (AccountInfoEOS->ProductUserId)
		{
			...
			.Next([Promise = MoveTemp(Promise)](TDefaultErrorResult<FAuthLogoutConnectImpl>&&) mutable -> void
			{
				Promise.EmplaceValue();
			});
			return Future;
...
ONLINESERVICESEOSGS_API TFuture<TDefaultErrorResult<FAuthLogoutConnectImpl>> FAuthEOSGS::LogoutConnectImpl(const FAuthLogoutConnectImpl::Params& LogoutParams)
{
	TPromise<TDefaultErrorResult<FAuthLogoutConnectImpl>> Promise;
	TFuture<TDefaultErrorResult<FAuthLogoutConnectImpl>> Future = Promise.GetFuture();

	EOS_Connect_LogoutOptions LogoutOptions = {};
	LogoutOptions.ApiVersion = 1; 
	LogoutOptions.LocalUserId = LogoutParams.ProductUserId;
	UE_EOS_CHECK_API_MISMATCH(EOS_CONNECT_LOGOUT_API_LATEST, 1);

	EOS_Async(EOS_Connect_Logout, ConnectHandle, LogoutOptions,
		[Promise = MoveTemp(Promise)](const EOS_Connect_LogoutCallbackInfo* Data) mutable -> void
		{
			if (Data->ResultCode == EOS_EResult::EOS_Success)
			{
				UE_LOG(LogOnlineServices, Verbose, TEXT("[FAuthEOSGS::LogoutConnectImpl] EOS_Connect_Logout Result: [%s]"), *LexToString(Data->ResultCode));
				Promise.SetValue(TDefaultErrorResult<FAuthLogoutConnectImpl>(FAuthLogoutConnectImpl::Result{}));
			}
			else
			{
				UE_LOG(LogOnlineServices, Warning, TEXT("[FAuthEOSGS::LogoutConnectImpl] EOS_Connect_Logout Result: [%s]"), *LexToString(Data->ResultCode));
				Promise.SetValue(TDefaultErrorResult<FAuthLogoutConnectImpl>(Errors::FromEOSResult(Data->ResultCode)));
			}
		});
...
void FAuthEOSGS::InitializeConnectLoginRecoveryTimer(const TSharedRef<FAccountInfoEOS>& AccountInfoEOS)
{
	...
	AccountInfoEOS->RestoreLoginTimer = FTSBackgroundableTicker::GetCoreTicker().AddTicker(
		FTickerDelegate::CreateLambda([this, WeakThis = AsWeak(), EpicAccountId = AccountInfoEOS->EpicAccountId](float) -> bool
		{
			if (WeakThis.IsValid())
			{

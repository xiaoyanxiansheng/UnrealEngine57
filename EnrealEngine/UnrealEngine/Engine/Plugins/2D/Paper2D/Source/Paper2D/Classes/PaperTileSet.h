...
protected:
	...
	UE_API void DestructiveAllocateTileData(int32 NewWidth, int32 NewHeight);

	// Reallocates the per-tile data to match the current (WidthInTiles, HeightInTiles) size, copying over what it can
	UE_API void ReallocateAndCopyTileData();
...
public:
	...
	UE_API int32 GetTerrainMembership(const FPaperTileInfo& TileInfo) const;

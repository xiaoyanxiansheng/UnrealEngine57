...
public:
	...
	UE_API FString RemoveSuffixFromBaseMapName(const FString& InName) const;

	// Generates names to test for a normal map using NormalMapTextureSuffixes
	UE_API void GenerateNormalMapNamesToTest(const FString& InRoot, TArray<FString>& InOutNames) const;

	// Applies the compression settings to the specified texture
	UE_API void ApplyTextureSettings(UTexture2D* Texture) const;
	...
	UE_API void ApplySettingsForSpriteInit(FSpriteAssetInitParameters& InitParams, ESpriteInitMaterialLightingMode LightingMode = ESpriteInitMaterialLightingMode::Automatic, ESpriteInitMaterialType MaterialTypeMode = ESpriteInitMaterialType::Automatic) const;
	...
	UE_API void ApplySettingsForTileMapInit(UPaperTileMap* TileMap, UPaperTileSet* DefaultTileSet = nullptr, ESpriteInitMaterialLightingMode LightingMode = ESpriteInitMaterialLightingMode::Automatic, ESpriteInitMaterialType MaterialTypeMode = ESpriteInitMaterialType::Automatic, bool bCreateEmptyLayer = true) const;
	...
	UE_API ESpriteInitMaterialType AnalyzeTextureForDesiredMaterialType(UTexture* Texture, const FIntPoint& Offset, const FIntPoint& Dimensions) const;

	// Returns the default translucent material
	UE_API UMaterialInterface* GetDefaultTranslucentMaterial(bool bLit) const;

	// Returns the default opaque material
	UE_API UMaterialInterface* GetDefaultOpaqueMaterial(bool bLit) const;

	// Returns the default masked material
	UE_API UMaterialInterface* GetDefaultMaskedMaterial(bool bLit) const;
	...
	UE_API UMaterialInterface* GetDefaultMaterial(ESpriteInitMaterialType MaterialType, bool bUseLitMaterial) const;

...
void TestMetricCollection(UWorld* World, bool bRandomized)
{
	...
	// GetOrAdd
	UMockWorldMetricA* MetricA = Collection.GetOrAdd<UMockWorldMetricA>();
	REQUIRE_MESSAGE(TEXT("Unexpected metric collection 'GetOrAdd<UMockWorldMetricA>' failure."), MetricA != nullptr);	
	UMockWorldMetricA* MetricB = Collection.GetOrAdd<UMockWorldMetricA>();
	REQUIRE_MESSAGE(TEXT("Unexpected metric collection 'GetOrAdd<UMockWorldMetricA>' failure."), MetricB != nullptr);
	REQUIRE_MESSAGE(TEXT("Unexpected different metric instances."), MetricA == MetricB);

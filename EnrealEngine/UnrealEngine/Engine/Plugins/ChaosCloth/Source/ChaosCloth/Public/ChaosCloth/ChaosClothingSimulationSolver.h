...
namespace Chaos
{
	...
	public:
		...
		UE_CHAOS_DECLARE_INDEXLESS_PROPERTYCOLLECTION_NAME(NumSubsteps, int32);
		UE_CHAOS_DECLARE_INDEXLESS_PROPERTYCOLLECTION_NAME(DynamicSubstepDeltaTime, float);
		UE_CHAOS_DECLARE_INDEXLESS_PROPERTYCOLLECTION_NAME(EnableNumSelfCollisionSubsteps, bool);
		UE_CHAOS_DECLARE_INDEXLESS_PROPERTYCOLLECTION_NAME(NumSelfCollisionSubsteps, int32);
	...
	private:
		...
		TArrayCollectionArray<Softs::FSolverVec3> ReferenceSpaceLocations;  // Center of rotations for fictitious forces and local velocity damping in local coordinate to the simulation space location
		TArrayCollectionArray<Softs::FSolverVec3> AppliedReferenceSpaceVelocities; // Used for local velocity damping.
		TArrayCollectionArray<Softs::FSolverVec3> AppliedReferenceSpaceAngularVelocities; // Used for local velocity damping.

...
namespace Chaos
{
	...
	int32 FClothingSimulationSkeletalMesh::GetLODIndex() const
	{
		...
		if (SkeletalMeshComponent)
		{
			...
			LODIndex = GetLODIndexFromOwnerLODIndex(OwnerLODIndex);
		}
		return LODIndex;
	}

	int32 FClothingSimulationSkeletalMesh::GetLODIndexFromOwnerLODIndex(int32 OwnerLODIndex) const
	{
		int32 LODIndex = INDEX_NONE;
	...
	const TArray<FTransform>& FClothingSimulationSkeletalMesh::GetBoneTransforms() const
	{
		...
		return GetContext() ? GetContext()->BoneTransforms : EmptyArray;
	}
	...
	const FClothingSimulationContextCommon* FClothingSimulationSkeletalMesh::GetContext() const
	{
		if (SkeletalMeshComponent)
		{
			...
			{
				if (UClothingSimulationFactory* const ClothingSimulationFactory = ClothingSimulationInstance.GetClothingSimulationFactory())
				{
					if (ClothingSimulationFactory->SupportsAsset(Asset))
					{
						return static_cast<const FClothingSimulationContextCommon*>(ClothingSimulationInstance.GetClothingSimulationContext());
					}
				}
			}
		}

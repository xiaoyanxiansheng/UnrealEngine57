...
{
	return ParticleRangeId == INDEX_NONE ? TConstArrayView<T>() : TConstArrayView<T>(Array.GetData() + ParticleRangeId, PBDEvolution->GetParticleRangeSize(ParticleRangeId));
}
...
{
	return ParticleRangeId == INDEX_NONE ? TArrayView<T>() : TArrayView<T>(Array.GetData() + ParticleRangeId, PBDEvolution->GetParticleRangeSize(ParticleRangeId));
}
...
{
	return ParticleRangeId == INDEX_NONE ? TConstArrayView<T>() : (Evolution ? Evolution->GetSoftBodyParticles(ParticleRangeId).GetConstArrayView(Array) :
...
{
	return ParticleRangeId == INDEX_NONE ? TArrayView<T>() : (Evolution ? Evolution->GetSoftBodyParticles(ParticleRangeId).GetArrayView(Array) :
...
{
	return ParticleRangeId == INDEX_NONE ? TConstArrayView<T>() : TConstArrayView<T>(Array.GetData() + ParticleRangeId, PBDEvolution->GetCollisionParticleRangeSize(ParticleRangeId));
}
...
{
	return ParticleRangeId == INDEX_NONE ? TArrayView<T>() : TArrayView<T>(Array.GetData() + ParticleRangeId, PBDEvolution->GetCollisionParticleRangeSize(ParticleRangeId));
}
...
TConstArrayView<T> GetCollisionParticleConstArrayView(const TUniquePtr<Softs::FEvolution>& Evolution, const TUniquePtr<Softs::FPBDEvolution>& PBDEvolution,
	const int32 ParticleRangeId, const TArray<T>& Array)
{
	return ParticleRangeId == INDEX_NONE ? TConstArrayView<T>() : (Evolution ? Evolution->GetCollisionParticleRange(ParticleRangeId).GetConstArrayView(Array) :
...
TArrayView<T> GetCollisionParticleArrayView(TUniquePtr<Softs::FEvolution>& Evolution, TUniquePtr<Softs::FPBDEvolution>& PBDEvolution,
	const int32 ParticleRangeId, TArray<T>& Array)
{
	return ParticleRangeId == INDEX_NONE ? TArrayView<T>() : (Evolution ? Evolution->GetCollisionParticleRange(ParticleRangeId).GetArrayView(Array) :
	...
	, bEnableSolver(true)
{
	...
	if (!bUseLegacySolver)
	{
		...
		Evolution->AddGroupArray(&AppliedReferenceSpaceVelocities);
		Evolution->AddGroupArray(&AppliedReferenceSpaceAngularVelocities);
	...
	else
	{
		...
		PBDEvolution->AddArray(&AppliedReferenceSpaceVelocities);
		PBDEvolution->AddArray(&AppliedReferenceSpaceAngularVelocities);
...
)
{
	...
	AppliedReferenceSpaceVelocities[GroupId] = DeltaTime > (Softs::FSolverReal)0.f ? FSolverVec3(LocalSpaceScaleInv * InOutAppliedReferenceVelocity) : Softs::FSolverVec3(0.f);
	AppliedReferenceSpaceAngularVelocities[GroupId] = DeltaTime > (Softs::FSolverReal)0.f ? FSolverVec3(InOutAppliedReferenceAngularVelocity) : Softs::FSolverVec3(0.f);
}
	...
	const TMap<FString, TConstArrayView<FRealSingle>>& WeightMaps)
{
	if (Evolution)
	{
		...
		Evolution->SetSoftBodyProperties(ParticleRangeId, InPropertyCollection, WeightMaps, ReferenceSpaceLocations[GroupId], AppliedReferenceSpaceVelocities[GroupId], AppliedReferenceSpaceAngularVelocities[GroupId]);
...
{
	return Config ? GetNumIterations(Config->GetProperties(SolverLOD), ClothingSimulationSolverDefault::NumIterations) :
...
{
	return Config ?
		GetMaxNumIterations(Config->GetProperties(SolverLOD), ClothingSimulationSolverDefault::MaxNumIterations) :
...
{
	return Config ?
		GetNumSubsteps(Config->GetProperties(SolverLOD), ClothingSimulationSolverDefault::NumSubsteps) :
...
void FClothingSimulationSolver::ApplyPreSimulationTransforms()
{
	...
	for (FClothingSimulationCloth* const Cloth : Cloths)
	{
		if (Cloth && PreSimulationTransforms.IsValidIndex(Cloth->GetGroupId()))
		{
			Cloth->ApplyPreSimulationTransforms(this, DeltaLocalSpaceLocation, PreSimulationTransforms[Cloth->GetGroupId()], DeltaTime);
		}
	}
}
...
void FClothingSimulationSolver::PreSubstep( const Softs::FSolverReal InterpolationAlpha, bool bDetectSelfCollisions)
{
	...
	{
		check(Cloth);
		Cloth->PreSubstep(this, InterpolationAlpha);
	}
}
...
void FClothingSimulationSolver::Update(Softs::FSolverReal InDeltaTime)
{
	...
	if (bAdvanceTimeStep)
	{
		...
		if (bEnableSolver)
		{
			...
			const int32 ConfigNumSubsteps = FMath::Max(GetNumSubsteps(Properties, ClothingSimulationSolverDefault::NumSubsteps),
				ClothingSimulationSolverDefault::MinNumSubsteps);

			const Softs::FSolverReal ConfigDynamicSubstepDeltaTime = GetDynamicSubstepDeltaTime(Properties, ClothingSimulationSolverDefault::DynamicSubstepDeltaTime); //  This is in ms
			...
			const bool bConfigEnableNumSelfCollisionSubsteps = GetEnableNumSelfCollisionSubsteps(Properties, ClothingSimulationSolverDefault::bEnableNumSelfCollisionSubsteps);

			const int32 ConfigNumSelfCollisionSubsteps = bConfigEnableNumSelfCollisionSubsteps ? FMath::Clamp(GetNumSelfCollisionSubsteps(Properties, ClothingSimulationSolverDefault::NumSelfCollisionSubsteps), ClothingSimulationSolverDefault::MinNumSubsteps, NumUsedSubsteps) : NumUsedSubsteps;
			...
			else
			{
				const int32 ConfigMaxNumIterations = FMath::Max(
					GetMaxNumIterations(Properties, ClothingSimulationSolverDefault::MaxNumIterations),
					ClothingSimulationSolverDefault::MinNumIterations);

				const int32 ConfigNumIterations = FMath::Clamp(
					GetNumIterations(Properties, ClothingSimulationSolverDefault::NumIterations),
					ClothingSimulationSolverDefault::MinNumIterations,

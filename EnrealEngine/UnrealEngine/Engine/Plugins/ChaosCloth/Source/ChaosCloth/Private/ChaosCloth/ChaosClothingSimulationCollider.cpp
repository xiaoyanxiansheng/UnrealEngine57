...
{
	TAutoConsoleVariable<bool> CVarUseOptimizedTaperedCapsule(TEXT("p.ChaosCloth.UseOptimizedTaperedCapsule"), true, TEXT("Use the optimized TaperedCapsule code instead of using a tapered cylinder and two spheres. Note that only optimized TaperedCapsule supports split tapered capsules"));
}
...
struct FClothingSimulationCollider::FLODData
{
	...
	void Add(
		...
		const FClothCollisionDataExtended& InClothCollisionDataExtended,
		const FReal InScale = 1.f,
		const TArray<int32>& UsedBoneIndices = TArray<int32>(),
		const TArray<int32>& UsedSubBoneIndices = TArray<int32>());
...
void FClothingSimulationCollider::FLODData::Add(
	...
	const TArray<int32>& UsedSubBoneIndices)
{
	check(Solver);
	check(Cloth);
	...
	const int32 NumLevelSets = InClothCollisionDataExtended.LevelSets.Num();
	const int32 NumSkinnedLevelSets = InClothCollisionDataExtended.SkinnedLevelSets.Num();
	const int32 NumMLLevelSets = InClothCollisionDataExtended.MLLevelSets.Num();
	const int32 NumSkinnedTriangleMeshes = InClothCollisionDataExtended.SkinnedTriangleMeshes.Num();

	const bool bSupportsComplexColliders = !Solver->IsLegacySolver();
	const int32 NumSimpleColliders = NumSpheres + NumCapsules + NumConvexes + NumBoxes + NumLevelSets;
	const int32 NumComplexColliders = NumSkinnedLevelSets + NumMLLevelSets + NumSkinnedTriangleMeshes;

	NumGeometries = NumSimpleColliders + (bSupportsComplexColliders ? NumComplexColliders : 0);
	...
	// Must try to add collision particles even when zero, so it will remove any existing collisions
	if (NumGeometries == 0)
	{
		return;
	}
	...
	if (NumCapsules)
	{
		for (int32 Index = CapsuleIndexOffset; Index < CapsuleIndexOffset + NumCapsules; ++Index)
		{
			...
			if (Axis.SizeSquared() < SMALL_NUMBER && Connection.OneSidedPlaneNormal.IsNearlyZero())
			{
				// Sphere
				Solver->SetCollisionGeometry(CollisionRangeId, Index,
					MakeImplicitObjectPtr<FSphere>(Center, MaxRadius));
			}
			else if (MaxRadius - MinRadius < KINDA_SMALL_NUMBER && Connection.OneSidedPlaneNormal.IsNearlyZero())
			{
				// Capsule
				Solver->SetCollisionGeometry(CollisionRangeId, Index,
			...
			else
			{
				...
				{
					Solver->SetCollisionGeometry(CollisionRangeId, Index,
						MakeImplicitObjectPtr<FTaperedCapsule>(P0, P1, Radius0, Radius1, Connection.OneSidedPlaneNormal));
				}
				else
				{
	...
	if (NumLevelSets != 0)
	{
		for (int32 Index = LevelSetIndexOffset, LevelSetIndex = 0; LevelSetIndex < NumLevelSets; ++Index, ++LevelSetIndex)
		{
			...
			BoneIndices[Index] = GetMappedBoneIndex(UsedBoneIndices, InClothCollisionDataExtended.LevelSets[LevelSetIndex].BoneIndex);
			UE_LOG(LogChaosCloth, VeryVerbose, TEXT("Found collision level set on bone index %d."), BoneIndices[Index]);

			// Setup the collision particle geometry
			FImplicitObjectPtr TransformedLevelSet = MakeImplicitObjectPtr<TImplicitObjectTransformed<FReal, 3>>(InClothCollisionDataExtended.LevelSets[LevelSetIndex].LevelSet->DeepCopyGeometry(),
				TRigidTransform<FReal, 3>(InClothCollisionDataExtended.LevelSets[LevelSetIndex].Transform));
	...
	if (bSupportsComplexColliders)
	{
		FClothComplexColliders* const ComplexColliders = Solver->GetComplexColliders(CollisionRangeId);
		check(ComplexColliders);
		ComplexColliders->Reset();
		ComplexColliders->AddSubBoneIndices(UsedSubBoneIndices);

		const int32 SkinnedLevelSetIndexOffset = LevelSetIndexOffset + NumLevelSets;
		for (int32 Index = SkinnedLevelSetIndexOffset, SkinnedLevelSetIndex = 0; SkinnedLevelSetIndex < NumSkinnedLevelSets; ++Index, ++SkinnedLevelSetIndex)
		{
			// Always initialize the collision particle transforms before setting any geometry as otherwise NaNs gets detected during the bounding box updates
			BaseTransforms[Index] = Softs::FSolverRigidTransform3::Identity;

			BoneIndices[Index] = GetMappedBoneIndex(UsedBoneIndices, InClothCollisionDataExtended.SkinnedLevelSets[SkinnedLevelSetIndex].BoneIndex);
			UE_LOG(LogChaosCloth, VeryVerbose, TEXT("Found collision skinned level set on bone index %d."), BoneIndices[Index]);

			FImplicitObjectPtr SkinnedLevelSet = InClothCollisionDataExtended.SkinnedLevelSets[SkinnedLevelSetIndex].WeightedLevelSet->DeepCopyGeometry();
			ComplexColliders->AddSkinnedLevelSet(Index, InClothCollisionDataExtended.SkinnedLevelSets[SkinnedLevelSetIndex].MappedSkinnedBones, SkinnedLevelSet);
			Solver->SetCollisionGeometry(CollisionRangeId, Index, MoveTemp(SkinnedLevelSet));
		}

		const int32 MLLevelSetIndexOffset = SkinnedLevelSetIndexOffset + NumSkinnedLevelSets;
		for (int32 Index = MLLevelSetIndexOffset, MLLevelsetIndex = 0; MLLevelsetIndex < NumMLLevelSets; ++Index, ++MLLevelsetIndex)
		{
			// Always initialize the collision particle transforms before setting any geometry as otherwise NaNs gets detected during the bounding box updates
			BaseTransforms[Index] = Softs::FSolverRigidTransform3::Identity;

			BoneIndices[Index] = GetMappedBoneIndex(UsedBoneIndices, InClothCollisionDataExtended.MLLevelSets[MLLevelsetIndex].BoneIndex);
			UE_LOG(LogChaosCloth, VeryVerbose, TEXT("Found collision ml level set on bone index %d."), BoneIndices[Index]);

			FImplicitObjectPtr MLLevelSet = InClothCollisionDataExtended.MLLevelSets[MLLevelsetIndex].MLLevelSet->CopyGeometry();
			ComplexColliders->AddMLLevelSet(Index, InClothCollisionDataExtended.MLLevelSets[MLLevelsetIndex].MappedActiveBoneIndices, MLLevelSet);
			Solver->SetCollisionGeometry(CollisionRangeId, Index, MoveTemp(MLLevelSet));
		}

		const int32 SkinnedTriMeshIndexOffset = MLLevelSetIndexOffset + NumMLLevelSets;
		for (int32 Index = SkinnedTriMeshIndexOffset, SkinnedTriMeshIndex = 0; SkinnedTriMeshIndex < NumSkinnedTriangleMeshes; ++Index, ++SkinnedTriMeshIndex)
		{
			// Always initialize the collision particle transforms before setting any geometry as otherwise NaNs gets detected during the bounding box updates
			BaseTransforms[Index] = Softs::FSolverRigidTransform3::Identity;

			BoneIndices[Index] = GetMappedBoneIndex(UsedBoneIndices, InClothCollisionDataExtended.SkinnedTriangleMeshes[SkinnedTriMeshIndex].BoneIndex);
			UE_LOG(LogChaosCloth, VeryVerbose, TEXT("Found collision ml level set on bone index %d."), BoneIndices[Index]);

			FImplicitObjectPtr SkinnedTriMesh = InClothCollisionDataExtended.SkinnedTriangleMeshes[SkinnedTriMeshIndex].SkinnedTriangleMesh->CopyGeometry();
			ComplexColliders->AddSkinnedTriangleMesh(Index, InClothCollisionDataExtended.SkinnedTriangleMeshes[SkinnedTriMeshIndex].MappedSkinnedBones, SkinnedTriMesh);
			Solver->SetCollisionGeometry(CollisionRangeId, Index, MoveTemp(SkinnedTriMesh));
		}
	}
}
	...
	const TArray<FTransform>& BoneTransforms)
{
	...
	if (NumGeometries)
	{
		...
		if (FClothComplexColliders* const ComplexColliders = Solver->GetComplexColliders(CollisionRangeId))
		{
			ComplexColliders->Update(ComponentToLocalSpace, BoneTransforms, CollisionTransforms);
		}
	}
}
...
void FClothingSimulationCollider::FLODData::ResetStartPose(FClothingSimulationSolver* Solver, FClothingSimulationCloth* Cloth)
{
	...
	if (NumGeometries)
	{
		...
		if (FClothComplexColliders* const ComplexColliders = Solver->GetComplexColliders(CollisionRangeId))
		{
			ComplexColliders->ResetStartPose();
		}
	}
}
...
{
	FClothCollisionDataExtended ExtendedCollisionData;
	TArray<int32> UsedSubBoneIndices;
	ExtractPhysicsAssetCollision(PhysicsAsset, ReferenceSkeleton, ClothCollisionData, ExtendedCollisionData, UsedBoneIndices, UsedSubBoneIndices, bUseSphylsOnly, bSkipMissingBones);
	LevelSetCollisions = MoveTemp(ExtendedCollisionData.LevelSets);
	SkinnedLevelSetCollisions = MoveTemp(ExtendedCollisionData.SkinnedLevelSets);

	// Append SubBones and reindex
	const int32 Offset = UsedBoneIndices.Num();
	UsedBoneIndices.Append(UsedSubBoneIndices);
	for (Chaos::FSkinnedLevelSetCollisionData& SkinnedLevelSetData : SkinnedLevelSetCollisions)
	{
		for (int32& MappedBone : SkinnedLevelSetData.MappedSkinnedBones)
		{
			MappedBone += Offset;
		}
	}
	...
	bool bSkipMissingBones)
{
	...
	ClothCollisionDataExtended.Reset();
	UsedBoneIndices.Reset();
	UsedSubBoneIndices.Reset();
	...
	if (PhysicsAsset)
	{
		...
		for (const USkeletalBodySetup* BodySetup : PhysicsAsset->SkeletalBodySetups)
		{
			...
			{
				if (TaperedCapsuleElem.Length == 0 && !TaperedCapsuleElem.bOneSidedCollision)
				{
					// Add extracted sphere collision data
					FClothCollisionPrim_Sphere Sphere;
				...
				else
				{
					...
					SphereConnection.SphereIndices[0] = ClothCollisionData.Spheres.Add(Sphere0);
					SphereConnection.SphereIndices[1] = ClothCollisionData.Spheres.Add(Sphere1);
					if (TaperedCapsuleElem.bOneSidedCollision)
					{
						SphereConnection.OneSidedPlaneNormal = TaperedCapsuleElem.Rotation.RotateVector(FVector::ForwardVector);
					}
					ClothCollisionData.SphereConnections.Add(SphereConnection);
				}
			}
			...
			for (const FKLevelSetElem& LevelSetElem : AggGeom.LevelSetElems)
			{
				...
				{
					ClothCollisionDataExtended.LevelSets.Emplace( LevelSetElem.GetLevelSet(), LevelSetElem.GetTransform(), MappedBoneIndex );
				}
			}
			...
			for (const FKSkinnedLevelSetElem& SkinnedLevelSetElem : AggGeom.SkinnedLevelSetElems)
			{
				if (SkinnedLevelSetElem.WeightedLevelSet().IsValid())
				{
					...
					for (const FName& SkinnedBoneName : SkinnedBones)
					{
						...
						const int32 MappedSkinnedBoneIndex = UsedSubBoneIndices.AddUnique(SkinnedBoneIndex);
						MappedSkinnedBones.Add(MappedSkinnedBoneIndex);
					}
					ClothCollisionDataExtended.SkinnedLevelSets.Emplace(SkinnedLevelSetElem.WeightedLevelSet(), MappedBoneIndex, MoveTemp(MappedSkinnedBones));
				}
			}

			for (const FKMLLevelSetElem& MLLevelSetElem : AggGeom.MLLevelSetElems)
			{				
				if (MLLevelSetElem.GetMLLevelSet().IsValid())
				{
					TArray<int32> MappedActiveBones;
					const TArray<FName>& ActiveBoneNames = MLLevelSetElem.GetMLLevelSet()->GetActiveBoneNames();
					MappedActiveBones.Reserve(ActiveBoneNames.Num());
					for (const FName& ActiveBoneName : ActiveBoneNames)
					{
						const int32 ActiveBoneIndex = ReferenceSkeleton ? ReferenceSkeleton->FindBoneIndex(ActiveBoneName) : INDEX_NONE;
						const int32 MappedActiveBoneIndex = UsedSubBoneIndices.AddUnique(ActiveBoneIndex);
						MappedActiveBones.Add(MappedActiveBoneIndex);
					}
					ClothCollisionDataExtended.MLLevelSets.Emplace( MLLevelSetElem.GetMLLevelSet(), MappedBoneIndex, MoveTemp(MappedActiveBones));
				}
			}

			for (const FKSkinnedTriangleMeshElem& SkinnedTriangleMeshElem : AggGeom.SkinnedTriangleMeshElems)
			{
				if (SkinnedTriangleMeshElem.GetSkinnedTriangleMesh().IsValid())
				{
					TArray<int32> MappedSkinnedBones;
					const TArray<FName>& SkinnedBones = SkinnedTriangleMeshElem.GetSkinnedTriangleMesh()->GetUsedBones();
					MappedSkinnedBones.Reserve(SkinnedBones.Num());
					for (const FName& SkinnedBoneName : SkinnedBones)
					{
						const int32 SkinnedBoneIndex = ReferenceSkeleton ? ReferenceSkeleton->FindBoneIndex(SkinnedBoneName) : INDEX_NONE;
					...
					ClothCollisionDataExtended.SkinnedTriangleMeshes.Emplace(SkinnedTriangleMeshElem.GetSkinnedTriangleMesh(), MappedBoneIndex, MoveTemp(MappedSkinnedBones));
				}
			}
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	FClothCollisionDataExtended PhysicsAssetCollisionDataExtended;
	TArray<int32> UsedBoneIndices;
	TArray<int32> UsedSubBoneIndices;
	ExtractPhysicsAssetCollision(PhysicsAsset, ReferenceSkeleton, PhysicsAssetCollisionData, PhysicsAssetCollisionDataExtended, UsedBoneIndices, UsedSubBoneIndices);

	LODData[(int32)ECollisionDataType::LODless]->Add(Solver, Cloth, PhysicsAssetCollisionData, PhysicsAssetCollisionDataExtended, Scale, UsedBoneIndices, UsedSubBoneIndices);
}
...
void FClothingSimulationCollider::PreUpdate(FClothingSimulationSolver* Solver, FClothingSimulationCloth* Cloth)
{
	...
	// TODO: Handle external extended data 
	// Will need to handle refreshing constraints that hold pointers to data in ComplexColliders

	LODData[(int32)ECollisionDataType::External]->Add(Solver, Cloth, CollisionData ? *CollisionData : FClothCollisionData(), FClothCollisionDataExtended());

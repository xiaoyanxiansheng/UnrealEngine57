...
namespace Chaos
{
	...
	FClothingSimulationSolver* FSkeletalMeshCacheAdapter::GetClothSolver(UPrimitiveComponent* InComponent) const
	{
		if(InComponent)
		{
			...
			{
				if (MeshComp->GetClothingSimulationInstances().Num() > 1)
				{
					static bool bErroredOnce = false;
					UE_CLOG(!bErroredOnce, LogChaosCloth, Error,
						TEXT("SkeletalMeshComponent [%s] has more than one clothing simulation running, but cloth caching can only deal with a single one."), *MeshComp->GetName());
					bErroredOnce = true;
				}

				for (FClothingSimulationInstance& ClothingSimulationInstance : MeshComp->GetClothingSimulationInstances())
				{
					if (ClothingSimulationInstance.GetClothingSimulation() && ClothingSimulationInstance.GetClothingSimulationFactory())
					{
						// TODO: Manage multiple solvers and solver types as to process all instances
						if (ClothingSimulationInstance.GetClothingSimulationFactory()->GetClass() == UChaosClothingSimulationFactory::StaticClass())
						{
PRAGMA_DISABLE_DEPRECATION_WARNINGS  // UE_DEPRECATED(5.7, "Use IClothingSimulationInterface instead.")
							return StaticCast<FClothingSimulation*>(ClothingSimulationInstance.GetClothingSimulation())->Solver.Get();

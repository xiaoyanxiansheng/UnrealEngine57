...
static TArray<FInstallInfo> GetInstalledRidersWithMdfind()
{
	...
	FPlatformProcess::ExecProcess(TEXT("/usr/bin/mdfind"), TEXT("\"kMDItemContentType == com.apple.application-bundle && (kMDItemFSName == *Rider*)\""), &ReturnCode, &OutResults, &OutErrors);
	if (ReturnCode != 0)
	{
		return {};
	}
	...
	TArray<FString> RiderPaths;
	OutResults.ParseIntoArray(RiderPaths, TEXT("\n"));

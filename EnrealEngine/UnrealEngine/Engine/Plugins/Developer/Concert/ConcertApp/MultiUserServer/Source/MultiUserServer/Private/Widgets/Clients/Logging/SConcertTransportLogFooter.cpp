...
namespace UE::MultiUserServer
{
	...
	void SConcertTransportLogFooter::Construct(const FArguments& InArgs, TSharedRef<FPagedFilteredConcertLogList> InPagedLogList)
	{
		...
		[
			...
			[
				SNew(SButton)
				.OnClicked_Lambda([](){ ConcertTransportEvents::SetLoggingEnabled(false); return FReply::Handled(); })
				.ToolTipText(LOCTEXT("DisableLoggingVisibility.Button.Tooltip", "Executes Concert.DisableLogging"))
				[
					SNew(STextBlock)
					.Text(LOCTEXT("DisableLoggingVisibility.Button.Text", "Disable Logging"))
				]
			]
		];
	}

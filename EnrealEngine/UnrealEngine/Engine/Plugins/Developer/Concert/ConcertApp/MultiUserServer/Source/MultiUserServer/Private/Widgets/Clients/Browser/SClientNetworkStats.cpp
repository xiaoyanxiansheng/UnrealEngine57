...
namespace UE::MultiUserServer
{
	...
	void SClientNetworkStats::UpdateStatistics(const FMessageTransportStatistics& Statistics) const
	{
		...
		WindowSize->SetText(FText::FromString(NetworkStatistics::FormatWindowSize(Statistics)));
	...
	TSharedRef<SWidget> SClientNetworkStats::CreateStatTable()
	{
		...
		AddStatistic(Content, LOCTEXT("WindowLabel", "WSize"), LOCTEXT("WindowSizeTooltip", "The window size for the end point. This is how big of a window we utilize for sending data. Bigger values are better. Small values indicate poor performance."), WindowSize);

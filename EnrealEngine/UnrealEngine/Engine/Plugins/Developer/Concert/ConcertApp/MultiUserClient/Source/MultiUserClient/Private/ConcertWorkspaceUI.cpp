...
namespace UE::ConcertWorkspaceUI::Private
{
bool ShouldContinueWithSendReceiveState(EConcertSendReceiveState SendReceiveState)
{
	if (SendReceiveState == EConcertSendReceiveState::Default)
	{
		return true;
	}

	FText WarningMsg;
	if (SendReceiveState == EConcertSendReceiveState::SendOnly)
	{
		WarningMsg = LOCTEXT("PersistInNonDefaultStateSendOnly", "You are about to persist a session in send only mode.  Changes from other clients may not be included.  Continue?");
	}
	else if (SendReceiveState == EConcertSendReceiveState::ReceiveOnly)
	{
		WarningMsg = LOCTEXT("PersistInNonDefaultStateReceiveOnly", "You are about to persist a session in receive only mode.  Your changes have not been sent to other clients.  Continue?");
	}
	else
	{
		checkNoEntry();
	}
	EAppReturnType::Type RetType = FMessageDialog::Open(EAppMsgType::YesNo, EAppReturnType::No, WarningMsg);
	
	return RetType == EAppReturnType::Yes;
}
}
...
bool FConcertWorkspaceUI::PromptPersistSessionChanges()
{
	...
	{
		IConcertClientSession& Session = ClientWorkspacePin->GetSession();
		if (!UE::ConcertWorkspaceUI::Private::ShouldContinueWithSendReceiveState(Session.GetSendReceiveState()))
		{
			return false;
		}

...
private:
	...
	void GetPreConnectionTasks(const IConcertClient& InClient, TArray<TUniquePtr<IConcertClientConnectionTask>>& OutTasks)
	{
		...
		bool bSkipSCChecks = FParse::Param(CommandLine, TEXT("-CONCERTSKIPSOURCECONTROLCHECKS"));
		FParse::Bool(CommandLine, TEXT("-CONCERTSKIPSOURCECONTROLCHECKS="), bSkipSCChecks);
		...
		{
			UE_LOG(LogConcert, Display, TEXT("Source control validation task was bypassed."));
		}
	...
	{
		auto GetCleanPath = [](EConcertServerType Type) -> FString
		{
			FString ServerPath = GetMultiUserServerExePathname(Type);
			ServerPath = FPaths::ConvertRelativePathToFull(ServerPath);
			FPaths::NormalizeDirectoryName(ServerPath);
			return ServerPath;
		};
		const FString ConsoleServerPath = GetCleanPath(EConcertServerType::Console);
		const FString SlateServerPath = GetCleanPath(EConcertServerType::Slate);
		...
		while (ProcIter.MoveNext())
		{
			...
			if (Candidate == ConsoleServerPath || Candidate == SlateServerPath)
			{

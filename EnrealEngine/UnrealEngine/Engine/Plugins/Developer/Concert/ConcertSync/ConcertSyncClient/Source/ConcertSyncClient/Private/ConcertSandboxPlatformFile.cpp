...
bool FlushPackageFile(const FString& InFilename, FName* OutPackageName = nullptr, bool bForceBulkDataLoad = true)
{
	FString PackageName;
	if (FPackageName::TryConvertFilenameToLongPackageName(InFilename, PackageName))
	{
		ConcertSyncClientUtil::FlushPackageLoading(PackageName, bForceBulkDataLoad);
		if (OutPackageName)
		{
	...
	FPersistResult& OutResult)
{
	...
	ConcertSandboxPlatformFileUtil::FlushPackageFile(InToFilename, nullptr, false);
...
bool FConcertSandboxPlatformFile::DeleteFileWithSCC(const TCHAR* InFilename, const FPersistParameters& InParam, FPersistResult& OutResult)
{
	...
	ConcertSandboxPlatformFileUtil::FlushPackageFile(InFilename, nullptr, false);

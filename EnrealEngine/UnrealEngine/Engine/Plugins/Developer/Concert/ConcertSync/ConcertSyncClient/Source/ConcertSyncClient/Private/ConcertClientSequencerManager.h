...
private:
	/**
	 * Delegate to provide a hint to the transaction bridge that the given object should be transacted.  We special case LevelSequences
	 * and allow any transaction that has a level sequence as an outermost object to be included because there are many object types
	 * in level sequence transactions that do not share UMovieSceneSignedObject as a base class, which is used by the default transaction
	 * filters.
	 */

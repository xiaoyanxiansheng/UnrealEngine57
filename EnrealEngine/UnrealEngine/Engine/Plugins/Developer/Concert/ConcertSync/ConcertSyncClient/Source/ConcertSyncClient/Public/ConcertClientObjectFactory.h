...
public:
	...
	/**
	 * Should this factory run InitializeObjects before OtherFactory?
	 * @return True if this factory needs to run its InitializeObjects before OtherFactory (to control initialization order between factories), or false if it doesn't matter which order they run.
	 */
	virtual bool ShouldInitializeObjectsBefore(const UConcertClientObjectFactory* OtherFactory) const
	{
		return false;
	}

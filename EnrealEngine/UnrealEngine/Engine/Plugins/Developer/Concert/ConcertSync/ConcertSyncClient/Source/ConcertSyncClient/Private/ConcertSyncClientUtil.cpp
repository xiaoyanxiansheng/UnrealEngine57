...
FGetObjectResult GetObject(const FConcertObjectId& InObjectId, const FName InNewName, const FName InNewOuterPath, const FName InNewPackageName, const FSoftObjectPath& InSourceObject, const bool bAllowCreate)
{
	...
	{
		UObject* ExistingObject = StaticFindObject(ObjectClass, ExistingObjectOuter, *ObjectNameToFind.ToString(), EFindObjectFlags::ExactClass);
		if (!ExistingObject)
		{
		...
		if (ExistingObject)
		{
			...
			if (bIsRename || bIsOuterChange)
			{
				...
				if (UObject* NewObject = StaticFindObject(ObjectClass, NewObjectOuter ? NewObjectOuter : ExistingObjectOuter, *ObjectNameToCreate.ToString(), EFindObjectFlags::ExactClass))
				{
	...
	{
		// Find the new object (in case something already created it)
		if (UObject* NewObject = StaticFindObject(ObjectClass, NewObjectOuter, *ObjectNameToCreate.ToString(), EFindObjectFlags::ExactClass))
		{
			// Update the object flags
		...
		if (bAllowCreate)
		{
			...
			if (Factory)
			{
				if (SourceObject)
				{
					bFactoryHandledCreation = Factory->DuplicateObject(ObjectResult.Obj, SourceObject, NewObjectOuter, ObjectClass, ObjectNameToCreate, (EObjectFlags)InObjectId.ObjectPersistentFlags);
				}
				else
				{
					bFactoryHandledCreation = Factory->CreateObject(ObjectResult.Obj, NewObjectOuter, ObjectClass, ObjectNameToCreate, (EObjectFlags)InObjectId.ObjectPersistentFlags);
				}
			}
			if (!bFactoryHandledCreation)
			{
				...
				else
				{
					if (SourceObject)
					{
						ObjectResult.Obj = DuplicateObject<UObject>(SourceObject, NewObjectOuter, ObjectNameToCreate);
						if (ObjectResult.Obj)
						{
							ObjectResult.Obj->SetFlags((EObjectFlags)InObjectId.ObjectPersistentFlags);
						}
					...
					{
						ObjectResult.Obj = NewObject<UObject>(NewObjectOuter, ObjectClass, ObjectNameToCreate, (EObjectFlags)InObjectId.ObjectPersistentFlags);
					}
...
void PurgePackages(TArrayView<const FName> InPackageNames)
{
	...
	for (const FName& PackageName : InPackageNames)
	{
		...
		if (ExistingPackage)
		{
			...
			// It's unsafe to call CollectObjectToPurge in ForEachObjectWithPackage lambda.
			// CollectObjectToPurge invokes global callbacks (e.g. closing asset editor), which may cause a NewObject call.
			// NewObject calls are illegal while iterating through the global UObject hash map.
			TArray<UObject*> ObjectsInPackage;
			ForEachObjectWithPackage(ExistingPackage, [&ObjectsInPackage](UObject* InObject)
			{
				ObjectsInPackage.Add(InObject);
				return true;
			});
			for (UObject* Subobject : ObjectsInPackage)
			{
				CollectObjectToPurge(Subobject);
			}

...
void ReportCurrentTransactionState()
{
	UE_LOG(LogConcert, Display, TEXT("Undo/Redo GIsTransacting is %s."), GIsTransacting ? TEXT("true") : TEXT("false"));
	UE_LOG(LogConcert, Display, TEXT("GEditor->CanTransact() is %s."), GEditor && GEditor->CanTransact() ? TEXT("true") : TEXT("false"));
	UE_LOG(LogConcert, Display, TEXT("GEditor->IsTransactionActive() is %s."), GEditor && GEditor->IsTransactionActive() ? TEXT("true") : TEXT("false"));
}
...
void ResetTransaction()
{
	if (GEditor)
	{
		GEditor->ResetTransaction(LOCTEXT("ConcertDebugCvarReset", "Reset by console variable command."));
	}
}

FAutoConsoleCommand CVarReportCurrentTransactionState(TEXT("Concert.Debug.ReportTransactionState"),
	TEXT("Report to the log file the current transaction state."),
	FExecuteAction::CreateStatic(&ReportCurrentTransactionState));

FAutoConsoleCommand CVarResetTransaction(TEXT("Concert.Debug.ResetTransaction"),
	TEXT("Reset the current open transaction."),
	FExecuteAction::CreateStatic(&ResetTransaction));
...
void ProcessTransactionEvent(const FConcertTransactionEventBase& InEvent, const FConcertSessionVersionInfo* InVersionInfo, const TArray<FName>& InPackagesToProcess, const FConcertLocalIdentifierTable* InLocalIdentifierTablePtr, const bool bIsSnapshot, const FConcertSyncWorldRemapper& WorldRemapper, const bool bIncludeEditorOnlyProperties, IConcertClientTransactionBridge::FRemoteTransactionResult* ResultInfo)
{
	...
	TArray<ConcertSyncClientUtil::FGetObjectResult, TInlineAllocator<32>> TransactionObjects;
	{
		...
		// Track the extended result info for the objects, if requested
		if (ResultInfo)
		{
			for (int32 ObjectIndex = 0; ObjectIndex < SortedExportedObjects.Num(); ++ObjectIndex)
			{
				...
				if (TransactionObjectRef.Obj)
				{
					IConcertClientTransactionBridge::FRemoteTransactionObjectResult& ObjectResultInfo = ResultInfo->Objects.AddDefaulted_GetRef();
					ObjectResultInfo.ObjectPath = TransactionObjectRef.Obj;
					ObjectResultInfo.ObjectIndex = &ObjectUpdate - InEvent.ExportedObjects.GetData();
					ObjectResultInfo.bNewlyCreated = NewlyCreatedObjects.Contains(TransactionObjectRef.Obj);
				}
			}
		}

		// Run any deferred object factory initialization
		if (ObjectsPendingInitialization.Num() > 0)
		{
			// If factories have initialization order dependencies, then make sure and respect those
			TArray<const UConcertClientObjectFactory*, TInlineAllocator<4>> SortedFactories;
			ObjectsPendingInitialization.GetKeys(SortedFactories);
			Algo::Sort(SortedFactories,
				[](const UConcertClientObjectFactory* ThisFactory, const UConcertClientObjectFactory* OtherFactory)
				{
					return ThisFactory->ShouldInitializeObjectsBefore(OtherFactory);
				});

			for (const UConcertClientObjectFactory* Factory : SortedFactories)
			{
				const TArray<UObject*>& Objects = ObjectsPendingInitialization.FindChecked(Factory);
				Factory->InitializeObjects(Objects);
			}
		}
	}
	...
	for (int32 ObjectIndex = SortedExportedObjects.Num() - 1; ObjectIndex >= 0; --ObjectIndex)
	{
		...
		for (const FConcertSerializedPropertyData& PropertyData : ObjectUpdate.PropertyDatas)
		{
			...
			if (TransactionProp)
			{
				...
				FCoreUObjectDelegates::OnPostObjectPropertyChanged.Broadcast(TransactionObject, PropertyChangedChainEvent);
			}
		}
...
void FConcertClientTransactionBridge::ApplyRemoteTransaction(const FConcertTransactionEventBase& InEvent, const FConcertSessionVersionInfo* InVersionInfo, const TArray<FName>& InPackagesToProcess, const FConcertLocalIdentifierTable* InLocalIdentifierTablePtr, const bool bIsSnapshot, FRemoteTransactionResult* ResultInfo)
{
	ConcertClientTransactionBridgeUtil::ProcessTransactionEvent(InEvent, InVersionInfo, InPackagesToProcess, InLocalIdentifierTablePtr, bIsSnapshot, FConcertSyncWorldRemapper(), bIncludeEditorOnlyProperties, ResultInfo);
}

void FConcertClientTransactionBridge::ApplyRemoteTransaction(const FConcertTransactionEventBase& InEvent, const FConcertSessionVersionInfo* InVersionInfo, const TArray<FName>& InPackagesToProcess, const FConcertLocalIdentifierTable* InLocalIdentifierTablePtr, const bool bIsSnapshot, const FConcertSyncWorldRemapper& ConcertSyncWorldRemapper, FRemoteTransactionResult* ResultInfo)
{
	ConcertClientTransactionBridgeUtil::ProcessTransactionEvent(InEvent, InVersionInfo, InPackagesToProcess, InLocalIdentifierTablePtr, bIsSnapshot, ConcertSyncWorldRemapper, bIncludeEditorOnlyProperties, ResultInfo);
}

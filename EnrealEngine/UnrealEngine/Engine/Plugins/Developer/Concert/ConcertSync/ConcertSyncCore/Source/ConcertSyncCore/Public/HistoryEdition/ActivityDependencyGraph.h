...
namespace UE::ConcertSyncCore
{
	...
	public:
		...
		UE_API FActivityNodeID AddActivity(int64 ActivityIndex, EActivityNodeFlags NodeFlags = EActivityNodeFlags::None);
		
		/** Adds a dependency from an existing node to another existing node */
		UE_API bool AddDependency(FActivityNodeID From, FActivityDependencyEdge To);
		
		/** Tries to find a corresponding node for an activity ID. */
		UE_API TOptional<FActivityNodeID> FindNodeByActivity(int64 ActivityID) const;

		/** Gets the node identified by the given ID, if it is valid. Only valid as long as no new node is added.*/
		UE_API const FActivityNode& GetNodeById(FActivityNodeID ID) const;

		UE_API void ForEachNode(TFunctionRef<void(const FActivityNode&)> ConsumerFunc) const;
		
	private:

		TArray<FActivityNode> Nodes;
		
		UE_API FActivityNode& GetNodeByIdInternal(FActivityNodeID ID);
	};
}

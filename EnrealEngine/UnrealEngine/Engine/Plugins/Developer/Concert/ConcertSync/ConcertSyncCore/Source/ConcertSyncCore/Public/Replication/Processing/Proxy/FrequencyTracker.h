...
namespace UE::ConcertSyncCore
{
	...
	public:
		 ...
		 */
		UE_API ECanProcessObject TrackAndCheckObject(
		 ...
		 * @param NowAsPlatformSeconds The current time - result of FPlatformTime::Seconds(). Pass this in to avoid excessive time spent on timing.
		 */
		UE_API void OnProcessObject(const FConcertObjectInStreamID& Object, const FPlatformSecondsTimestamp NowAsPlatformSeconds = FPlatformTime::Seconds());
		
		/**
		 * Removes all tracked objects were last processed before CutoffTime.
		 * @param CutoffTime A past value of FPlatformTime::Seconds()
		 */
		UE_API void CleanseOutdatedObjects(const FPlatformSecondsTimestamp CutoffTime);
	...
	class FFrequencyTracker_CleanByTick : public FFrequencyTracker
	{
	public:
		...
		UE_API void Tick(float DeltaTime, const FPlatformSecondsTimestamp NowAsPlatformSeconds = FPlatformTime::Seconds());

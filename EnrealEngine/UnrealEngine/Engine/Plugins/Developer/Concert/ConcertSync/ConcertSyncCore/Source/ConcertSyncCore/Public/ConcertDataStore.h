...
public:
	 ...
	 */
	UE_API FConcertDataStoreResult FetchOrAdd(const FName& Key, const FName& TypeName, const FConcertSessionSerializedPayload& SerializedValue);
	 ...
	 */
	UE_API FConcertDataStoreResult Store(const FName& Key, const FName& TypeName, const FConcertSessionSerializedPayload& SerializedValue, const TOptional<uint32>& Version = TOptional<uint32>());
	 ...
	 * @see[The example in the class documentation.](@ref FConcertDataStore)
	 */
	UE_API FConcertDataStoreResult Fetch(const FName& Key, const FName& TypeName) const;

	/**
	 * Visits all the key/value currently stored.
	 * @param Visitor The function invoked for each key/value pair.
	 */
	UE_API void Visit(const TFunctionRef<void(const FName&, const FConcertDataStore_StoreValue&)>& Visitor) const;

	/** Returns the version of the specified key if found. */
	UE_API TOptional<uint32> GetVersion(const FName& Key) const;

	/** Returns the number of key/value pairs currently stored. */
	int32 GetSize() const { return KeyValueMap.Num(); }

private:
	UE_API FConcertDataStoreResult InternalAdd(const FName& Key, const FName& TypeName, const FConcertSessionSerializedPayload& Value, uint32 Version);

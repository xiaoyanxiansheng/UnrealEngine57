...
public:
	...
	UE_API void ReloadActivities(TMap<FGuid, FConcertClientInfo> EndpointClientInfoMap, TArray<FConcertSessionActivity> FetchedActivities);
	
	/** Callback for handling the a new or updated activity item. */ 
	UE_API void HandleActivityAddedOrUpdated(const FConcertClientInfo& InClientInfo, const FConcertSyncActivity& InActivity, const FStructOnScope& InActivitySummary);

	bool IsLastColumn(FName ColumnId) const { return ActivityListView->IsLastColumn(ColumnId); }
	
	UE_API TSet<TSharedRef<FConcertSessionActivity>> GetSelectedActivities() const;
...
private:
	...
	UE_API void OnSearchTextChanged(const FText& InSearchText);
	...
	UE_API FText HighlightSearchedText() const;
...
private:
	...
	UE_API TOptional<FConcertClientInfo> GetClientInfo(FGuid Guid) const;
};

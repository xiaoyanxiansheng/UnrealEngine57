...
private:
	...
	UE_API void HandleUserScrolled(float);

	/** Checks if the scrollbar is currently at the bottom of its track. */
	UE_API bool IsAtBottom();

	/** Callback for scrolling to the start of the list. */
	UE_API FReply HandleScrollToStart();

	/** Callback for scrolling to the end of the list. */
	UE_API FReply HandleScrollToEnd();

	/** Callback for getting the ScrollBar buttons visibility. */
	UE_API EVisibility HandleScrollButtonsVisibility() const;

	/** Creates a button with the FontAwesome font. */
	UE_API TSharedRef<SButton> CreateScrollBarButton(const FText& InToolTip, const FString& InIcon, FOnClicked OnClickedDelegate) const;

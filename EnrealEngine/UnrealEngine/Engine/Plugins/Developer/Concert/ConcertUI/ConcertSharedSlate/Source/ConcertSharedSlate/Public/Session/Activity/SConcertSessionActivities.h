...
public:
	...
	UE_API TSharedPtr<FConcertSessionActivity> GetSelectedActivity() const;
	/** Returns all selected activities */
	UE_API TArray<TSharedPtr<FConcertSessionActivity>> GetSelectedActivities() const;
	...
	UE_API TSharedPtr<FConcertSessionActivity> GetMostRecentActivity() const;

	/** Returns true if the column names is the last one (most right one). */
	UE_API bool IsLastColumn(const FName& ColumnId) const;

	/** Clears all activities displayed. */
	UE_API void ResetActivityList();

	/** Append an activity to the view. Used to populate the view from a live session. */
	UE_API void Append(TSharedPtr<FConcertSessionActivity> Activity);

	/** Request the view to refresh. */
	UE_API void RequestRefresh();

	/** Asks the view to update the text search filter. */
	UE_API FText UpdateTextFilter(const FText& InFilterText);
	
	UE_API void OnColumnVisibilitySettingsChanged(const FColumnVisibilitySnapshot& ColumnSnapshot);
...
private:
	...
	// Widget creation
	UE_API TSharedRef<SHeaderRow> CreateHeaderRow(const FArguments& InArgs);
	UE_API TSharedRef<ITableRow> OnGenerateActivityRowWidget(TSharedPtr<FConcertSessionActivity> Item, const TSharedRef<STableViewBase>& OwnerTable);
	UE_API void FetchActivities();

	// Filtering
	UE_API EConcertActivityFilterFlags QueryActiveActivityFilters() const;
	UE_API void OnActivityFilterUpdated();
	UE_API bool PassesFilters(const FConcertSessionActivity& Activity);

	// List view
	UE_API void OnListViewScrolled(double InScrollOffset);
	UE_API void OnListViewSelectionChanged(TSharedPtr<FConcertSessionActivity> InActivity, ESelectInfo::Type SelectInfo);

	// Search
	UE_API void PopulateSearchStrings(const FConcertSessionActivity& Item, TArray<FString>& OutSearchStrings);

	// Details
	UE_API void OnDetailsAreaExpansionChanged(bool bExpanded);
	UE_API void UpdateDetailArea(TSharedPtr<FConcertSessionActivity> InSelectedActivity);
	UE_API void UpdateDetailsByEventPayload(TSharedPtr<FConcertSessionActivity> InSelectedActivity);
	UE_API void UpdateDetailsByRequest(TSharedPtr<FConcertSessionActivity> InSelectedActivity);
	
	UE_API void SetDetailsPanelVisibility(const SWidget* VisiblePanel);
	UE_API void DisplayTransactionDetails(const FConcertSessionActivity& Activity, const FConcertTransactionEventBase& InTransaction);
	UE_API void DisplayPackageDetails(const FConcertSessionActivity& Activity, int64 PackageRevision, const FConcertPackageInfo& PackageInfo);
	
	UE_API FText GetNoDetailsText() const;
...
public:
	...
	UE_API TSharedRef<SWidget> MakeStatusBar(TAttribute<int32> Total, TAttribute<int32> Displayed);
	
	/** Makes a standard View Options widget, displaying the eye ball icon and showing the possible options. */
	UE_API TSharedRef<SWidget> MakeViewOptionsComboButton(TOptional<FExtendContextMenu> ExtendMenu = {});
	/** Returns a Menu widgets containing the available options. */
	UE_API TSharedRef<SWidget> MakeMenuWidget(TOptional<FExtendContextMenu> ExtendMenu = {});
	...
	UE_API void OnOptionToggled(const FName CheckBoxId);

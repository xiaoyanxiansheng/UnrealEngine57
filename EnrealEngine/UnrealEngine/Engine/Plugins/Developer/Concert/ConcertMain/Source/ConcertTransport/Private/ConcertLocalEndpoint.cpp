...
void FConcertLocalEndpoint::ProcessEndpointDiscovery(const FConcertMessageContext& ConcertContext, const FMessageAddress& InRemoteAddress)
{
	...
	{
		// We received a message from the remote endpoint, make sure we update our last receipt time. 
		RemoteEndpoint->UpdateLastMessageReceivedTime(ConcertContext.UtcNow);
	}
}

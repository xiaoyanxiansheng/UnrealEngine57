...
#define UE_API CONCERTTRANSPORT_API
...
{
public:
	UE_API FConcertIdentifierWriter(FConcertLocalIdentifierTable* InLocalIdentifierTable, TArray<uint8>& InBytes, bool bIsPersistent = false);

	using FMemoryWriter::operator<<; // For visibility of the overloads we don't override

	//~ Begin FArchive Interface
	UE_API virtual FArchive& operator<<(FName& Name) override;
...
{
public:
	UE_API FConcertIdentifierReader(const FConcertLocalIdentifierTable* InLocalIdentifierTable, const TArray<uint8>& InBytes, bool bIsPersistent = false);

	using FMemoryReader::operator<<; // For visibility of the overloads we don't override

	//~ Begin FArchive Interface
	UE_API virtual FArchive& operator<<(FName& Name) override;
	UE_API FArchive& operator<<(FSoftObjectPath& AssetPtr) override;
	UE_API virtual FString GetArchiveName() const override;
	//~ End FArchive Interface
...
{
public:
	UE_API FConcertIdentifierRewriter(const FConcertLocalIdentifierTable* InLocalIdentifierTable, FConcertLocalIdentifierTable* InRewriteIdentifierTable, TArray<uint8>& InBytes, bool bIsPersistent = false);
	...
	UE_API virtual FArchive& operator<<(FName& Name) override;
	UE_API virtual FArchive& operator<<(FSoftObjectPath& AssetPtr) override;
	UE_API virtual int64 TotalSize() override final;
	UE_API virtual void Serialize(void* Data, int64 Num) override final;
	UE_API virtual FString GetArchiveName() const override;
	//~ End FArchive Interface

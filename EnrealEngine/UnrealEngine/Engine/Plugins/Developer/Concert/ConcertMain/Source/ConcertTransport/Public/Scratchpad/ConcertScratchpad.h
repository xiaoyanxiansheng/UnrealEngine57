...
public:
	 ...
	 */
	UE_API bool HasValue(const FName& InId) const;
...
private:
	 ...
	 */
	UE_API void InternalSetValue(const FName& InId, IScratchpadValuePtr&& InValue);

	/**
	 * Get the internal scratchpad value associated with the given key (if any).
	 */
	UE_API IScratchpadValuePtr InternalGetValue(const FName& InId) const;

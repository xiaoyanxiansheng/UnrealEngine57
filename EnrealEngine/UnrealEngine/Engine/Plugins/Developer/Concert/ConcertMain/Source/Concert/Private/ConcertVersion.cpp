...
{
static TAutoConsoleVariable<int32> CVarVersionValidation(
	TEXT("Concert.VersionValidationMode"), 0,
	TEXT("Specifies if we are using default or legacy based version validation. In default mode (0), clients will be allowed to join sessions if patch versions are compatible. For example, 5.4.4 is version compatible with 5.4.3. In legacy mode (1), versions have to match exactly when joining a session."));

bool ValidateVersion(const int32 InCurrent, const int32 InOther, const FText InVersionDisplayName, const EConcertVersionValidationMode InValidationMode, bool bIsVersionPatchValue, FText* OutFailureReason)
{
	// Convert the InValidationMode into something we can handle.  If we are in default version mode then we need to convert patch compatible version to ::Identical or ::Compatible.
	// 
	const EConcertVersionValidationMode Mode = [bIsVersionPatchValue, InValidationMode]()
	{
		const bool bUseDefaultVersionValidation = CVarVersionValidation.GetValueOnAnyThread() == 0;
		if (bUseDefaultVersionValidation && bIsVersionPatchValue && InValidationMode == EConcertVersionValidationMode::PatchCompatible)
		{
			return EConcertVersionValidationMode::Compatible;
		}
		else if (InValidationMode == EConcertVersionValidationMode::PatchCompatible)
		{
			return EConcertVersionValidationMode::Identical;
		}
...
{
	const bool bIsVersionPatchValue = false;
	
	return ConcertVersionUtil::ValidateVersion(FileVersion, InOther.FileVersion, LOCTEXT("UE4PackageVersionName", "UE4 Package Version"), InValidationMode, bIsVersionPatchValue, OutFailureReason)
		&& ConcertVersionUtil::ValidateVersion(FileVersionUE5, InOther.FileVersionUE5, LOCTEXT("UE5PackageVersionName", "UE5 Package Version"), InValidationMode, bIsVersionPatchValue, OutFailureReason)
		&& ConcertVersionUtil::ValidateVersion(FileVersionLicensee, InOther.FileVersionLicensee, LOCTEXT("LicenseePackageVersionName", "Licensee Package Version"), InValidationMode, bIsVersionPatchValue, OutFailureReason);
}
...
{
	return ConcertVersionUtil::ValidateVersion(Major, InOther.Major, LOCTEXT("MajorEngineVersionName", "Major Engine Version"), InValidationMode, false /*bIsVersionPatchValue*/, OutFailureReason)
		&& ConcertVersionUtil::ValidateVersion(Minor, InOther.Minor, LOCTEXT("MinorEngineVersionName", "Minor Engine Version"), InValidationMode, false /*bIsVersionPatchValue*/, OutFailureReason)
		&& ConcertVersionUtil::ValidateVersion(Patch, InOther.Patch, LOCTEXT("PatchEngineVersionName", "Patch Engine Version"), InValidationMode, true /*bIsVersionPatchValue*/, OutFailureReason)
		&& ConcertVersionUtil::ValidateVersion(Changelist, InOther.Changelist, LOCTEXT("ChangelistEngineVersionName", "Changelist Engine Version"), InValidationMode, true /*bIsVersionPatchValue*/, OutFailureReason);
}
...
bool FConcertCustomVersionInfo::Validate(const FConcertCustomVersionInfo& InOther, const EConcertVersionValidationMode InValidationMode, FText* OutFailureReason) const
{
	...
	return ConcertVersionUtil::ValidateVersion(Version, InOther.Version, FText::AsCultureInvariant(FriendlyName.IsNone() ? Key.ToString() : FriendlyName.ToString()), InValidationMode, bIsVersionPatchValue, OutFailureReason);
}

...
namespace PluginUtils
{
			...
			virtual bool Visit(const TCHAR* FilenameOrDirectory, bool bIsDirectory)
			{
				...
				else
				{
					...
					if (!IgnoredFileTypes.Contains(NewExt))
					{
						...
						const bool bIsTagsIni = NewExt.Equals(TEXT("ini")) && NewName.Contains(TEXT("/Config/Tags/"));
						if (NameReplacementFileTypes.Contains(NewExt) || bIsTagsIni)
						{
	...
	void FixupPluginTemplateAssets(const FString& PluginName, const FString& NameToReplace, TMap<FString, FString>& OutModifiedAssetPaths)
	{
		...
		struct FFixupPluginAssets : public IPlatformFile::FDirectoryVisitor
		{
			...
			void ReplaceGameplayTagsInObject(UObject* Object)
			{
				if (!Object)
				{
					return;
				...
				for (TPropertyValueIterator<FProperty> It(Object->GetClass(), Object, EPropertyValueIteratorFlags::FullRecursion); It; ++It)
				{
					if (const FStructProperty* StructProperty = CastField<FStructProperty>(It.Key()))
					{
						if (UScriptStruct* StructType = StructProperty->Struct)
						{
							if (StructType->IsChildOf(FGameplayTag::StaticStruct()))
							{
								const void* PropertyValue = It.Value();

								const FGameplayTag* GameplayTag = reinterpret_cast<const FGameplayTag*>(PropertyValue);
								if (GameplayTag && GameplayTag->IsValid() && GameplayTag->ToString().Contains(PLUGIN_NAME))
								{
									FGameplayTag NewTag = UGameplayTagsManager::Get().RequestGameplayTag(FName(GameplayTag->ToString().Replace(*PLUGIN_NAME, *NameToReplace, ESearchCase::CaseSensitive)));
									It.Key()->CopyCompleteValue(const_cast<void*>(PropertyValue), &NewTag);
								}
							}
						}
					}
				...
				if (FilesToScan.Num() > 0)
				{
					...
					for (const FString& File : FilesToScan)
					{
						...
						for (FAssetData AssetData : Assets)
						{
							...
							const UClass* LoadedClass = AssetData.GetClass(EResolveClass::Yes);
							if (LoadedClass == UBlueprint::StaticClass())
							{
								if (UClass* BPClass = UEditorAssetLibrary::LoadBlueprintClass(AssetData.GetPackage()->GetPathName()))
								{
									LoadedAsset = BPClass->GetDefaultObject();
									...
									if (ensure(DataSubsystem))
									{
										...
										DataSubsystem->GatherSubobjectData(LoadedAsset, SubobjectData);

										for (const FSubobjectDataHandle& Handle : SubobjectData)
										{
											if (UObject* SubObject = const_cast<UObject*>(Handle.GetData()->GetObjectForBlueprint(Cast<UBlueprint>(LoadedAsset))))
											{
												ReplaceGameplayTagsInObject(SubObject);
											}
										}
									}
								}
							...
							ReplaceGameplayTagsInObject(LoadedAsset);

...
ECommandResult::Type FPerforceSourceControlProvider::Execute( const FSourceControlOperationRef& InOperation, FSourceControlChangelistPtr InBaseChangelist, const TArray<FString>& InFiles, EConcurrency::Type InConcurrency, const FSourceControlOperationComplete& InOperationCompleteDelegate )
{
	...
	if(InConcurrency == EConcurrency::Synchronous)
	{
		...
		Command->ContentRoots = ContentRoots;
...
{
	return RegisterStateBranches(BranchNames, TArray<FString> { ContentRootIn });
}

void FPerforceSourceControlProvider::RegisterStateBranches(const TArray<FString>& BranchNames, const TArray<FString>& ContentRootsIn)
{
	StatusBranchNames = BranchNames;
	ContentRoots = ContentRootsIn;
}
...
bool FPerforceSourceControlProvider::GetStateBranchAtIndex(int32 BranchIndex, FString& OutBranchName) const
{
	if (StatusBranchNames.IsValidIndex(BranchIndex))
	{
		OutBranchName = StatusBranchNames[BranchIndex];
		return !OutBranchName.IsEmpty();
	}
	return false;
}

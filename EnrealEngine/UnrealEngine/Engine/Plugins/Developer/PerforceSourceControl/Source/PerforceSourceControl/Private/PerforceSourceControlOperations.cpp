...
void IPerforceSourceControlWorker::RegisterWorkers()
{
	...
	WorkersMap.Add("GetProjectWorkspaces", FGetPerforceSourceControlWorker::CreateStatic(&InstantiateWorker<FPerforceGetProjectWorkspacesWorker>));
...
static void ParseBranchModificationResults(const FP4RecordSet& InRecords, const TArray<FText>& ErrorMessages, const TArray<FString>& ContentRoots, TMap<FString, FBranchModification>& BranchModifications)
{
...
static void ParseUpdateStatusResults(const FP4RecordSet& InRecords, const TArray<FText>& ErrorMessages, TArray<FPerforceSourceControlState>& OutStates, const TArray<FString>& ContentRoots, TMap<FString, FBranchModification>& BranchModifications)
{
...
bool FPerforceUpdateStatusWorker::Execute(FPerforceSourceControlCommand& InCommand)
{
	...
	if (!InCommand.IsCanceled() && ScopedConnection.IsValid())
	{
		...
		if(InCommand.Files.Num())
		{
			...
			const TArray<FString>& ContentRoots = InCommand.ContentRoots;
			...
			if (StatusBranches.Num())
			{
				...
				ParseBranchModificationResults(RevisionRecords, InCommand.ResultInfo.ErrorMessages, ContentRoots, BranchModifications);
			}
			...
			ParseUpdateStatusResults(Records, InCommand.ResultInfo.ErrorMessages, OutStates, ContentRoots, BranchModifications);
			RemoveRedundantErrors(InCommand, TEXT(" - no such file(s)."), false);
			RemoveRedundantErrors(InCommand, TEXT("' is not under client's root '"));
...
bool FPerforceUpdateStatusWorker::UpdateStates() const
{
	...
	auto UpdateDataStorage = [](const FPerforceSourceControlState& State)
	{
		...
		using namespace UE::Editor::RevisionControl;
...
bool FPerforceGetWorkspacesWorker::Execute(FPerforceSourceControlCommand& InCommand)
{
	...
	{
		FPerforceConnection& Connection = ScopedConnection.GetConnection();
		TArray<FScWorkspaceInfo> OutWorkspaces;
		InCommand.bCommandSuccessful = Connection.GetWorkspaces(InCommand.ConnectionInfo, FOnIsCancelled::CreateRaw(&InCommand, &FPerforceSourceControlCommand::IsCanceled), OutWorkspaces, InCommand.ResultInfo);

		check(InCommand.Operation->GetName() == GetName());
		TSharedRef<FGetWorkspaces, ESPMode::ThreadSafe> Operation = StaticCastSharedRef<FGetWorkspaces>(InCommand.Operation);
		Operation->Results = MoveTemp(OutWorkspaces);
	}
	return InCommand.bCommandSuccessful;
}
...
FName FPerforceGetProjectWorkspacesWorker::GetName() const
{
	return "GetProjectWorkspaces";
}

bool FPerforceGetProjectWorkspacesWorker::Execute(FPerforceSourceControlCommand& InCommand)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FPerforceGetProjectWorkspacesWorker::Execute);
	...
	if (!InCommand.IsCanceled() && ScopedConnection.IsValid())
	{
		...
		InCommand.bCommandSuccessful = Connection.GetProjectWorkspaces(InCommand.ConnectionInfo, FOnIsCancelled::CreateRaw(&InCommand, &FPerforceSourceControlCommand::IsCanceled), ClientSpecList, InCommand.ResultInfo);

		check(InCommand.Operation->GetName() == GetName());
		TSharedRef<FGetProjectWorkspaces, ESPMode::ThreadSafe> Operation = StaticCastSharedRef<FGetProjectWorkspaces>(InCommand.Operation);
		Operation->Results = ClientSpecList;
	}
	return InCommand.bCommandSuccessful;
}

bool FPerforceGetProjectWorkspacesWorker::UpdateStates() const
{
	return false;
}
	...
	if (!InCommand.IsCanceled() && ScopedConnection.IsValid())
	{
		...
		else
		{
			...
			for (const FString& TargetFilePath : InCommand.Files)
			{
				...
				if (InCommand.bCommandSuccessful)
				{
					// To avoid Move() logging errors, first test if file exists
					// This ensures we respect what's specified by the operation using ISourceControlOperation::ShouldLogErrors()
					if (IFileManager::Get().FileExists(*TempFilePath))
					{
					...
					{
						// Give error 
						const FString ErrorMsg = FString::Printf(TEXT("Failed to download '%s' to '%s'"), *TargetFilePath, *TempFilePath);
						InCommand.ResultInfo.ErrorMessages.Add(FText::FromString(ErrorMsg));
						InCommand.bCommandSuccessful = false;
					}
				}

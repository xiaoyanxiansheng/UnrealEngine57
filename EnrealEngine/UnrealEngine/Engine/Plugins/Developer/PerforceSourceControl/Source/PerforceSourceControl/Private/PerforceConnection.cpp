...
bool FPerforceConnection::AutoDetectWorkspace(const FPerforceConnectionInfo& InConnectionInfo, FPerforceSourceControlProvider& SCCProvider, FString& OutWorkspaceName, TArray<FText>& OutErrorMessages)
{
	...
	Connection.GetProjectWorkspaces(InConnectionInfo, FOnIsCancelled(), ClientSpecList, ResultInfo);
...
bool FPerforceConnection::GetWorkspaces(const FPerforceConnectionInfo& InConnectionInfo, FOnIsCancelled InOnIsCancelled, TArray<FScWorkspaceInfo>& OutWorkspaceInfos, FSourceControlResultInfo& OutResultInfo)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FPerforceConnection::GetWorkspaces)
	...
	if (bEstablishedConnection)
	{
		...
		bool bCommandOK = RunCommand(TEXT("clients"), Params, Records, OutResultInfo, InOnIsCancelled, bConnectionDropped);
		...
		if (bCommandOK)
		{
			...
			for (const FP4Record& ClientRecord : Records)
			{
				...
				const FString& RootDir = ClientRecord(TEXT("Root"));
				...
				{
					FScWorkspaceInfo& WorkspaceInfo = OutWorkspaceInfos.Emplace_GetRef();
					WorkspaceInfo.Name = ClientName;
					WorkspaceInfo.RootDir = RootDir;
				}
				else
				{
...
bool FPerforceConnection::GetProjectWorkspaces(const FPerforceConnectionInfo& InConnectionInfo, FOnIsCancelled InOnIsCancelled, TArray<FString>& OutWorkspaceNames, FSourceControlResultInfo& OutResultInfo)
{
	TArray<FScWorkspaceInfo> WorkspaceInfos;
	if (GetWorkspaces(InConnectionInfo, MoveTemp(InOnIsCancelled), WorkspaceInfos, OutResultInfo))
	{
		FString ProjectDir = IFileManager::Get().ConvertToAbsolutePathForExternalAppForRead(*ISourceControlModule::Get().GetSourceControlProjectDir()).ToLower();
		ProjectDir = ProjectDir.Replace(TEXT("\\"), TEXT("/"), ESearchCase::CaseSensitive);

		for (FScWorkspaceInfo& WorkspaceInfo : WorkspaceInfos)
		{
			WorkspaceInfo.RootDir.ReplaceInline(TEXT("\\"), TEXT("/"), ESearchCase::CaseSensitive);
			...
			const bool bIsNullClientRootPath = (WorkspaceInfo.RootDir == TEXT("null"));

			if (!WorkspaceInfo.RootDir.EndsWith(TEXT("/")))
			{
				WorkspaceInfo.RootDir += TEXT("/");
			}

			// Only allow paths that are ACTUALLY legit for this application
			if (bIsNullClientRootPath || FPathViews::IsParentPathOf(WorkspaceInfo.RootDir, ProjectDir))
			{
				OutWorkspaceNames.Emplace(MoveTemp(WorkspaceInfo.Name));
			}
			else
			{
				UE_LOG(LogSourceControl, Verbose, TEXT(" %s client specs rejected due to root directory mismatch (%s)"), *WorkspaceInfo.Name, *WorkspaceInfo.RootDir);
			}
		}

...
void FRenderDocPluginModule::OnPostEngineInit()
{
	...
	if (FSlateApplication::IsInitialized() && !IsRunningCommandlet())
	{
		...
		// Cannot register in StartupModule because the RDoc plugin is initialized earlier than the main frame's
		if (IMainFrameModule* MainFrameModule = FModuleManager::GetModulePtr<IMainFrameModule>("MainFrame"))
		{
			MainFrameModule->OnMainFrameCreationFinished().AddRaw(this, &FRenderDocPluginModule::OnMainFrameCreationFinished);
		}
	}
#endif // WITH_EDITOR
}
...
#if WITH_EDITOR
void FRenderDocPluginModule::OnMainFrameCreationFinished(TSharedPtr<SWindow> ParentWindow, bool bIsRunningStartupDialog)
{
	if (ParentWindow.IsValid())
	{
		MainWindowHandle = ParentWindow->GetNativeWindow()->GetOSWindowHandle();
	}
}
#endif // WITH_EDITOR
...
void FRenderDocPluginModule::BeginCapture_RenderThread(FRHICommandListImmediate* InRHICommandList)
{
	...
	{
		RenderDocAPI->StartFrameCapture(Device, MainWindowHandle);
	});
}
...
void FRenderDocPluginModule::EndCapture_RenderThread(FRHICommandListImmediate* InRHICommandList, uint32 InFlags, FString const& InDestFileName)
{
	...
	{
		uint32 Result = RenderDocAPI->EndFrameCapture(Device, MainWindowHandle);
		if (Result == 1)
		{

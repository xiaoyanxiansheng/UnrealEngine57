...
public:
...
#if !(PLATFORM_WINDOWS && PLATFORM_CPU_ARM_FAMILY && !PLATFORM_WINDOWS_ARM64EC) // earlier versions of the DLL are not available on Windows Arm64
		OodleTextureSdkVersionToUseIfNone("2.9.5")
#else	
		OodleTextureSdkVersionToUseIfNone("2.9.14")
#endif
	{
		...
		int32 InMip0NumSlicesNoDepth, int32 InMipIndex, int32 InMipCount, FStringView DebugTexturePathName, const bool bInHasAlpha, FCompressedImage2D& OutImage) const override
	{
		...
		static FName OodleVersion2912("2.9.12");
		if (OodleBCN == OodleTex_BC6U && CompressOodleTextureVersion == OodleVersion2912 )
		{
			// BC6+2.9.12 has a bug, so bump up to 2.9.13
			//	 .13 has the same output as .12 so no DDC key bump needed
			static FName OodleVersion2913("2.9.13");
			CompressOodleTextureVersion = ValidateOodleTextureSdkVersion(OodleVersion2913);
		}
		...
		if (OodleBCN == OodleTex_BC6U)
		{
			...
			*/
			...
			if ( InImage.Format == ERawImageFormat::RGBA32F )
			{
				...
				//  -> Oodle clamp changed/fixed in 2.9.14
			}
			else
			{

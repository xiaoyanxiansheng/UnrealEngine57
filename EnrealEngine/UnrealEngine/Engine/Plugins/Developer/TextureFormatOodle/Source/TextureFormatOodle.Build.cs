...
public class TextureFormatOodle : ModuleRules
{
	...
	protected void SetDynamicLibNameStrings()
	{
		...
		{
			DynamicLibNamePrefix = (Target.Architecture == UnrealArch.Arm64) ? "oo2tex_9_winuwparm64_" : "oo2tex_win64_";
	...
	protected void AddDynamicLibsForVersion(string Version)
	{
		...
		if (Target.IsInPlatformGroup(UnrealPlatformGroup.Windows))
		{
			PlatformDir = (Target.Architecture == UnrealArch.Arm64) ? "WinArm64" : "Win64";
		}
	...
	public TextureFormatOodle(ReadOnlyTargetRules Target) : base(Target)
	{
		...
		bool bIsWinArm64 = Target.IsInPlatformGroup(UnrealPlatformGroup.Windows) && (Target.Architecture == UnrealArch.Arm64); // earlier versions of the DLL are not available on Windows Arm64

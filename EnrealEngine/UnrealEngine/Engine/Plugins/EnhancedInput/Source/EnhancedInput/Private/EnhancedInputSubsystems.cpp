...
void UEnhancedInputLocalPlayerSubsystem::InitalizeUserSettings()
{
	...
	if (!LP)
	{
		UE_LOG(LogEnhancedInput, Error, TEXT("Failed to initialize user settings, there is no valid local player!"));
		return;
	}
	...
	if (ensure(UserSettings))
	{
		...
		OnPostUserSettingsInitialized.Broadcast(UserSettings);
		
		UE_LOG(LogEnhancedInput, Log, TEXT("Enhanced Input local player subsystem has initialized the user settings!"));
	}
}

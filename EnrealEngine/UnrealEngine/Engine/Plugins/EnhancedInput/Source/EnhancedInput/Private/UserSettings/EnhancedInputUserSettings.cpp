...
#include UE_INLINE_GENERATED_CPP_BY_NAME(EnhancedInputUserSettings)
...
namespace UE::EnhancedInput
{
	...
	static void SetActiveKeyProfile(const TArray<FString>& Args)
	{
		if (!Args.IsValidIndex(0))
		{
			UE_LOG(LogEnhancedInput, Error, TEXT("EnhancedInput.SetActiveKeyProfile command failed - you must provide a desired key profile name! Example usage:\n\t EnhancedInput.SetActiveKeyProfile PresetA"));
			return;
		}

		const FString& DesiredKeyProfile = Args[0];
		
		UEnhancedInputLibrary::ForEachSubsystem([DesiredKeyProfile](IEnhancedInputSubsystemInterface* Subsystem)
		{
			if (UEnhancedInputUserSettings* Settings = Subsystem->GetUserSettings())
			{
				Settings->SetActiveKeyProfile(DesiredKeyProfile);
			}
		});
	}
	...
	static FAutoConsoleCommand ConsoleCommandSetKeyProfile(
		TEXT("EnhancedInput.SetActiveKeyProfile"),
		TEXT("Sets the current Enhanced Input user key profile to the one provided."),
		FConsoleCommandWithArgsDelegate::CreateStatic(UE::EnhancedInput::SetActiveKeyProfile));
}
...
void UEnhancedInputUserSettings::Serialize(FArchive& Ar)
{
	...
	if (Ar.IsLoading())
	{
		// When loading, we want to empty the current saved key profile map so that it is populated with 
		// only data from the archive.
		//
		// We only care about the dirty data when serializing. If other profiles currently exist here, they were added 
		// via the Registration of IMC assets and would need to be re-registered. We will re-populate the registered IMCs
		// after we load everything that was dirty.
		...
		for (const UE::EnhancedInput::FMappableKeysHeader& Header : Headers)
		{
			if (const UClass* FoundClass = FindObject<UClass>(nullptr, *Header.ClassPath))
			{
				...
				// Note: The currently registered Input Mapping contexts will be re-registered on these
				// newly created profiles after we are done loading all previous data.
	...
	// Now that we have finished loading all the data that was in the archive, we should re-register all the 
	// mapping contexts to the newly created key profiles.
	if (Ar.IsLoading())
	{
		// This will re-create any profiles which may have existed before this serialize load call was initiated and ensure
		// that each profile has its default key mappings set up for each IMC which has unique defaults for specific profiles
...
void UEnhancedInputUserSettings::MapPlayerKey(const FMapPlayerKeyArgs& InArgs, FGameplayTagContainer& FailureReason)
{
	...
	else if (FKeyMappingRow* MappingRow = KeyProfile->FindKeyMappingRowMutable(InArgs.MappingName))
	{
		...
		{
			const FPlayerKeyMapping* ExistingMapping = MappingRow->Mappings.FindArbitraryElement();
...
bool UEnhancedInputUserSettings::IsKeyProfileAvailable(const FString& ProfileId) const
{
	return SavedKeyProfilesMap.Contains(ProfileId);
}

UEnhancedPlayerMappableKeyProfile* UEnhancedInputUserSettings::GetDefaultKeyProfile() const
{
	if (const TObjectPtr<UEnhancedPlayerMappableKeyProfile>* Found = SavedKeyProfilesMap.Find(UE::EnhancedInput::DefaultProfileIdentifierString))
	{
		return Found->Get();
	}
	return nullptr;
...
bool UEnhancedInputUserSettings::RegisterInputMappingContextInternal(const UInputMappingContext* IMC)
{
	...
	// Create any key profiles for mapping contexts which have specific overrides if we need to
	const TArray<FString> ProfilesSpecifiedInMappingContext = IMC->GetProfilesWithOverridenMappings();
	for (const FString& ProfileId : ProfilesSpecifiedInMappingContext)
	{
		...
		Args.UserId = LP ? LP->GetPlatformUserId() : PLATFORMUSERID_NONE;
		Args.DisplayName = UE::EnhancedInput::DefaultProfileDisplayName;
		Args.bSetAsCurrentProfile = false;
...
bool UEnhancedInputUserSettings::RegisterKeyMappingsToProfile(UEnhancedPlayerMappableKeyProfile& Profile, const UInputMappingContext* IMC)
{
	...
	for (const FEnhancedActionKeyMapping& KeyMapping : IMC->GetMappingsForProfile(Profile.GetProfileIdString()))
	{
		...
		// Only mappings with empty profile or the current profile should be added - same as it's done by IEnhancedInputSubsystemInterface::RebuildControlMappings
		const UPlayerMappableKeySettings* KeySettings = KeyMapping.GetPlayerMappableKeySettings();
		
		// If this mapping has specified a specific key profile, and the current profile isn't it, then don't add this key mapping
		if (KeySettings && !KeySettings->SupportedKeyProfileIds.IsEmpty() && !KeySettings->SupportedKeyProfileIds.Contains(Profile.GetProfileIdString()))
		{
			continue;
		}
		...
		for (const FSetElementId& RegisteredMappingId : MappingsRegistered)
		{
			...
			{
				OnKeyMappingRegisteredToProfile(&Profile, MappingRow.Mappings.Get(RegisteredMappingId), KeyMapping);
			}			
		}
...
void UEnhancedInputUserSettings::OnKeyMappingRegisteredToProfile(UEnhancedPlayerMappableKeyProfile* Profile, FPlayerKeyMapping& RegisteredMapping, const FEnhancedActionKeyMapping& SourceMapping)
{
	// Call the old version for back compat
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	OnKeyMappingRegistered(RegisteredMapping, SourceMapping);
	PRAGMA_ENABLE_DEPRECATION_WARNINGS

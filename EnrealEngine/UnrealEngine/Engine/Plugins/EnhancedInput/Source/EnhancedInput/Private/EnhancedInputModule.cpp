...
void FEnhancedInputModule::OnShowDebugInfo(AHUD* HUD, UCanvas* Canvas, const FDebugDisplayInfo& DisplayInfo, float& YL, float& YPos)
{
	...
	if (Canvas)
	{
		if (HUD->ShouldDisplayDebug(NAME_EnhancedInput))
		{
			...
			bool bFoundValidDebugTarget = false;
			// Checking if the debug target actor is a valid local player, if so then show their debug info
			// If target actor is not a local player then don't display anything
			if (APawn* TargetPawn = Cast<APawn>(HUD->GetCurrentDebugTargetActor()))
			{
				if (APlayerController* PlayerController = Cast<APlayerController>(TargetPawn->GetController()))
				{
					if (const ULocalPlayer* LocalPlayer = Cast<ULocalPlayer>(PlayerController->Player))
					{
						...
						{
							EnhancedInputSubsystem->ShowDebugInfo(Canvas);
							bFoundValidDebugTarget = true;
						}
					}
				}
			}

			if(!bFoundValidDebugTarget)
			{ 
				DisplayDebugManager.SetDrawColor(FColor::Red);
				DisplayDebugManager.DrawString(TEXT("Debug Target is not valid for Enhanced Input Debug Display"));
			}
		}

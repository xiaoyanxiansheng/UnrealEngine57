...
EDataValidationResult FEnhancedActionKeyMapping::IsDataValid(FDataValidationContext& Context) const
{
	...
	for (const UInputTrigger* Trigger : Triggers)
	{
		...
		else
		{
			Result = EDataValidationResult::Invalid;
			FFormatNamedArguments Args;
			Args.Add("InputActionName", FText::FromString(GetNameSafe(Action)));
			Context.AddError(FText::Format(LOCTEXT("NullInputTriggerWithAction", "There cannot be a null Input Trigger on a key mapping for Input Action '{InputActionName}'"), Args));
		}
	}
	...
	for (const UInputModifier* Modifier : Modifiers)
	{
		...
		else
		{
			...
			FFormatNamedArguments Args;
			Args.Add("InputActionName", FText::FromString(GetNameSafe(Action)));
			Context.AddError(FText::Format(LOCTEXT("NullInputModifierWithAction", "There cannot be a null Input Modifier on a key mapping for Input Action '{InputActionName}'"), Args));
		}
	}
	return Result;
}

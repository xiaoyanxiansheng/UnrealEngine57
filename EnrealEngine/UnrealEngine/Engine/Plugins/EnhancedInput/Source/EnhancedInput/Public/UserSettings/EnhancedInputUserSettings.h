...
struct FMapPlayerKeyArgs final
{
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Enhanced Input|User Settings", meta = (AllowPrivateAccess = true, GetOptions = "Engine.InputPlatformSettings.GetAllHardwareDeviceNames"))
...
protected:
	...
	*/
	 ...
	 */
	UE_DEPRECATED(5.7, "Use OnKeyMappingRegisteredToProfile instead.")
	UE_API virtual void OnKeyMappingRegistered(FPlayerKeyMapping& RegisteredMapping, const FEnhancedActionKeyMapping& SourceMapping) final;
	 ...
	 * This does nothing by default.
	 * 
	 * @param Profile				The key profile which this mapping was registered to
	 * @param RegisteredMapping		The newly registered player key mapping
	 * @param SourceMapping			The source mapping that the registered mapping was created from.
	 */
	UE_API virtual void OnKeyMappingRegisteredToProfile(UEnhancedPlayerMappableKeyProfile* Profile, FPlayerKeyMapping& RegisteredMapping, const FEnhancedActionKeyMapping& SourceMapping);
...
public:
	...
	*/
	...
	/** Returns true if the given ProfileId is currently available in the user settings. */
	UE_API bool IsKeyProfileAvailable(const FString& ProfileId) const;

	/**
	* Returns a pointer to the default key profile created by Enhanced Input. 
	* This profile has an ID of "InputUserSettings.Profiles.Default"
	*/
	UE_API UEnhancedPlayerMappableKeyProfile* GetDefaultKeyProfile() const;
...
protected:
	...
	* 
	* If an IMC specified mappings which should have different defaults based on a certain key profile ID,
	* then we will also create the profile here if needed.
	* 
	* This will do so regardless of if the IMC has been registered already. 
	*/
	bool RegisterInputMappingContextInternal(const UInputMappingContext* IMC);

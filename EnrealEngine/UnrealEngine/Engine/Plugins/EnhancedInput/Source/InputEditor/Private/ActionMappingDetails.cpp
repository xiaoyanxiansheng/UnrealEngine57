...
FActionMappingsNodeBuilderEx::FActionMappingsNodeBuilderEx(const TSharedPtr<IPropertyHandle>& InPropertyHandle)
...
void FActionMappingsNodeBuilderEx::ReorderMappings(int32 OriginalIndex, int32 NewIndex, EReorderMode ReorderMode)
{
	...
	if (ReorderMode == EReorderMode::Group)
	{
		...
		TSharedPtr<IPropertyHandle> OuterStructContainerHandle = ActionMappingsPropertyHandle->GetParentHandle();
		if(OuterStructContainerHandle.IsValid())
		{
			void* StructData = nullptr;
			if(FPropertyAccess::Success == OuterStructContainerHandle->GetValueData(StructData))
			{
				FInputMappingContextMappingData* RawData = static_cast<FInputMappingContextMappingData*>(StructData);
				if (RawData)
				{
					RawData->Mappings.StableSort([&Groups](const FEnhancedActionKeyMapping& A, const FEnhancedActionKeyMapping& B) { return Groups[A.Action] < Groups[B.Action]; });
				}
			}
		}
	}
	...
	{
		ActionMappingsArrayHandle->SwapItems(OriginalIndex, NewIndex);
	}
	else
	{
...
void FActionMappingsNodeBuilderEx::AddActionMappingButton_OnClick()
{
	...
	if (OuterObjects.Num() == 1)
	{
		...
		TSharedPtr<IPropertyHandleArray> ArrayHandle = ActionMappingsPropertyHandle->AsArray();
		const FPropertyHandleItemAddResult Res = ArrayHandle->AddItem();
...
void FActionMappingsNodeBuilderEx::OnActionMappingActionChanged(const FAssetData& AssetData, const ActionMappingDetails::FMappingSet MappingSet)
{
	...
	if (SelectedAction != CurrentAction)
	{
		...
		{
			bool bHasExistingAction = false;
				
			for (int32 Index = 0; Index < MappingSet.Mappings.Num(); ++Index)
			{
				const UObject* Action;
				MappingSet.Mappings[Index]->GetChildHandle(GET_MEMBER_NAME_CHECKED(FEnhancedActionKeyMapping, Action))->GetValue(Action);
				if (Action == SelectedAction)
				{
					bHasExistingAction = true;
					break;
				}
			}
			
			if (bHasExistingAction)
			{
				FFormatNamedArguments Args;
...
void FActionMappingsNodeBuilderEx::AddActionMappingToGroupButton_OnClick(const ActionMappingDetails::FMappingSet MappingSet)
{
	...
	if (OuterObjects.Num() == 1)
	{
		...
		// Create a new FEnhancedActionKeyMapping entry in the mappings array, and set its default Input Action
		// value to the UInputAction that we would like.
		{
			// Add the item to the array...
			TSharedPtr<IPropertyHandleArray> ArrayHandle = ActionMappingsPropertyHandle->AsArray();
			const FPropertyHandleItemAddResult Res = ArrayHandle->AddItem();

			// Initalize the UInputAction property...
			TSharedPtr<IPropertyHandle> NewMappingInputActionProperty = ArrayHandle->GetElement(Res.GetIndex())->GetChildHandle(GET_MEMBER_NAME_CHECKED(FEnhancedActionKeyMapping, Action));
			NewMappingInputActionProperty->SetValue(MappingSet.SharedAction);
		}
		ActionMappingsPropertyHandle->NotifyPostChange(EPropertyChangeType::ArrayAdd);
	}
}

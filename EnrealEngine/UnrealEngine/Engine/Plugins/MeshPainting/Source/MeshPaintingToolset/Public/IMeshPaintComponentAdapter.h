...
public:
	...
	virtual void ApplyOrRemoveTextureOverride(UTexture* SourceTexture, UTexture* OverrideTexture, FMaterialUpdateContext& MaterialUpdateContext) const = 0;
	
	UE_DEPRECATED(5.7, "Use the version that takes a FMaterialUpdateContext.")
	void ApplyOrRemoveTextureOverride(UTexture* SourceTexture, UTexture* OverrideTexture) const;
	...
	static UE_API void DefaultQueryPaintableTextures(int32 MaterialIndex, const UMeshComponent* MeshComponent, int32& OutDefaultIndex, TArray<struct FPaintableTexture>& InOutTextureList);
...
namespace UE::MeshPaintingToolset
{
	 ...
	 */
	class FDefaultTextureOverride final
	{
	public:
		UE_API void ApplyOrRemoveTextureOverride(UMeshComponent* InMeshComponent, UTexture* SourceTexture, UTexture* OverrideTexture, FMaterialUpdateContext& MaterialUpdateContext) const;
		UE_DEPRECATED(5.7, "Use the version that takes a FMaterialUpdateContext.")
		UE_API void ApplyOrRemoveTextureOverride(UMeshComponent* InMeshComponent, UTexture* SourceTexture, UTexture* OverrideTexture) const;

		UE_API ~FDefaultTextureOverride();

		FDefaultTextureOverride() = default;

		UE_API FDefaultTextureOverride(const FDefaultTextureOverride& InOther);
		UE_API FDefaultTextureOverride(FDefaultTextureOverride&& InOther);

		UE_API FDefaultTextureOverride& operator=(const FDefaultTextureOverride& InOther);
		UE_API FDefaultTextureOverride& operator=(FDefaultTextureOverride&& InOther);
	};
}

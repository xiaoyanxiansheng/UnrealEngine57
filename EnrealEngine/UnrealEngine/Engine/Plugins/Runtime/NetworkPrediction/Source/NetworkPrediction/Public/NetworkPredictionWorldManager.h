...
public:
	...
	static UE_API UNetworkPredictionWorldManager* ActiveInstance;

	UE_API UNetworkPredictionWorldManager();
	...
	UE_API bool CanPhysicsFixTick() const;

	UE_API ENetworkPredictionTickingPolicy PreferredDefaultTickingPolicy() const;

	UE_API void SyncNetworkPredictionSettings(const UNetworkPredictionSettingsObject* Settings);
...
private:
	...
	UE_API void SetUsingPhysics();
	UE_API void InitPhysicsCapture();
	UE_API void AdvancePhysicsResimFrame(int32& PhysicsFrame);
	UE_API void EnsurePhysicsGTSync(const TCHAR* Context) const;
	...
	UE_API void OnWorldPreTick(UWorld* InWorld, ELevelTick InLevelTick, float InDeltaSeconds);
	UE_API void ReconcileSimulationsPostNetworkUpdate();
	UE_API void BeginNewSimulationFrame(UWorld* InWorld, ELevelTick InLevelTick, float InDeltaSeconds);

	// For ease of unit testing
	friend struct UE::Net::Private::FNetPredictionTestWorld;
	UE_API void OnWorldPreTick_Internal(float InDeltaSeconds, float InFixedFrameRate);
	UE_API void ReconcileSimulationsPostNetworkUpdate_Internal();
	UE_API void BeginNewSimulationFrame_Internal(float InDeltaSeconds);

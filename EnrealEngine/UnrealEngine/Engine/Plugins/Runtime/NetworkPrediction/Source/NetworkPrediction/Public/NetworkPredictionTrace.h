...
public:
	...
	static UE_API void TraceWorldFrameStart(UGameInstance* GameInstance, float DeltaSeconds);
	static UE_API void TraceSimulationConfig(int32 TraceID, ENetRole NetRole, bool bHasNetConnection, const FNetworkPredictionInstanceArchetype& Archetype, const FNetworkPredictionInstanceConfig& Config, int32 ServiceMask);

	static UE_API void TracePIEStart();
	static UE_API void TraceWorldPreInit();
	static UE_API void TraceSystemFault(const TCHAR* Fmt, ...);

	static UE_API void TraceSimulationScope(int32 TraceID);

	static UE_API void TraceTick(int32 StartMS, int32 DeltaMS, int32 OutputFrame);
	static UE_API void TraceSimTick(int32 TraceID);
	// Used to lazy update user state after a simtick
	static UE_API void TraceSimState(int32 TraceID);

	static UE_API void TraceNetRecv(int32 Frame, int32 TimeMS);
	
	static UE_API void TraceReconcile(const FAnsiStringView& StrView); 

	static UE_API void TraceShouldReconcile(int32 TraceID);
	static UE_API void TraceRollbackInject(int32 TraceID);

	static UE_API void TracePushInputFrame(int32 Frame);
	static UE_API void TraceFixedTickOffset(int32 Offset, bool bChanged);
	static UE_API void TraceBufferedInput(int32 NumBufferedFrames, bool bFault);
	static UE_API void TraceProduceInput(int32 TraceID);

	static UE_API void TraceOOBStateMod(int32 SimulationId, int32 Frame, const FAnsiStringView& StrView);
...
private:
	...
	static UE_API void TraceSimulationCreated_Internal(FNetworkPredictionID ID, FStringBuilderBase& Builder);

	// ----------------------------------------------------------------------------------
	

	static UE_API void TraceUserState_Internal(ETraceUserState StateType, FAnsiStringBuilderBase& Builder);

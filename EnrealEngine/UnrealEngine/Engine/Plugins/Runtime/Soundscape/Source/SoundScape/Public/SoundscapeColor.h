...
{
	// Altitude relative to the listener location
	Relative = 0 UMETA(DisplayName = "Relative Altitude"), 

	// Altitude based on world coordinates
	Absolute = 1 UMETA(DisplayName = "Absolute Altitude") 
};
...
struct SOUNDSCAPE_API FSoundscapeColorSpawnSettings
{
	...
	// Include an additional delay for the first spawn attempt between MinFirstSpawnDelay and MaxFirstSpawnDelay times.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "SpawnBehavior")
	bool bDelayFirstSpawn = false;

	// Sets the minimum time delay before the first spawn
	...
	// Sets the maximum time delay before the first spawn
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "SpawnBehavior", meta = (ClampMin = "0.0", UIMin = "0.0", EditCondition = "bDelayFirstSpawn"))
	float MaxFirstSpawnDelay = 3.0f;

	// When true, color will continue to attempt to spawn sounds at an update frequency between the MinSpawnDelay and MaxSpawnDelay times. When false, it will only spawn once.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "SpawnBehavior")
	bool bContinuouslyRespawn = false;

	// Sets the minimum time between respawns
	...
	// Sets the maximum time between respawns
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "SpawnBehavior", meta = (ClampMin = "0.0", UIMin = "0.0", EditCondition = "bContinuouslyRespawn"))
	...
	// When true, spawn locations will clamp the z axis according to the ClampMode rules between the MinSpawnHeightClamp and the MaxSpawnHeightClamp
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "SpawnBehavior")
	...
	// The Minimum normalized directional vector in world orientation for the Trace to fire (default to down, z = -1.0f)
...
struct SOUNDSCAPE_API FSoundscapeColorModulationSettings
{
	...
	// An additional randomization of Volume between VolumeMin and VolumeMax on top of previous volume adjustments
	...
	//  An additional randomization of Pitch between PitchMin and PitchMax on top of previous pitch adjustments
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "ModulationBehavior")
...
private:
	...
	// Evaluate color point density meets threshold, returns true if evaluation passes or if we are not filtering by color point density
	bool EvaluateColorPointDensity();

...
void UActiveSoundscapeColor::Update()
{
	...
	if (Sound && bNeedToSpawnSound && World)
	{
		...
		// Check to see if Position By Trace
		if (SpawnBehavior.bPositionByTrace == false && EvaluateColorPointDensity())
		{
			//Spawn Color Voice
			SpawnSoundscapeColorVoice();
		}
...
bool UActiveSoundscapeColor::EvaluateColorPointDensity()
{
	// Check if we actually want to filter by color point density, otherwise return true
	if (SpawnBehavior.bFilterByColorPointDensity)
	{
		// Get World
		UWorld* World = GetWorld();

		// Return false if failed to get world
		if (!World)
		{
			return false;
		}
		...
		if (USoundscapeSubsystem* SoundscapeSubsystem = GameInstance->GetSubsystem<USoundscapeSubsystem>())
		{
			...
			if (SpawnBehavior.MinColorPointNumber > ColorPointDensity)
			{
				...
				SoundscapeSubsystem->DrawDebugCell(NewSoundParams.Location, false);

				// Failed to meet Color Point Density requirements, early out and do not spawn sound
				return false;
			}
			
			// Attempt Draw Debug Cell
			SoundscapeSubsystem->DrawDebugCell(NewSoundParams.Location, true);

			// We have met the threshold, return success
			return true;
		}

		// If we failed getting the Game Instance, we do not want to return success
		return false;
	}

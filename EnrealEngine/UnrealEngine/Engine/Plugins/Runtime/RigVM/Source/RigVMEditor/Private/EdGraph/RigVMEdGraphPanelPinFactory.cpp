...
{
	return IRigVMAssetInterface::RigVMPanelPinFactoryName;
}
...
TSharedPtr<SGraphPin> FRigVMEdGraphPanelPinFactory::CreatePin(UEdGraphPin* InPin) const
{
	...
	if(const UEdGraphNode* EdGraphNode = InPin->GetOuter())
	{
		...
		{
			if(const FRigVMAssetInterfacePtr Blueprint = EdGraph->GetAssetDefaultObject())
			{
	...
	if (InPin)
	{
		...
		if (URigVMEdGraphNode* RigNode = Cast<URigVMEdGraphNode>(InPin->GetOwningNode()))
		{
			...
			if (ModelPin)
			{
				...
				{
					if (FRigVMAssetInterfacePtr Blueprint = IRigVMAssetInterface::GetInterfaceOuter(RigGraph))
					{

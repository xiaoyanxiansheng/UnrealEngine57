...
void SRigVMEditorSelectedDebugObjectWidget::GenerateDebugObjectInstances(bool bRestoreSelection)
{
	...
	if (!EditorPtr->OnlyShowCustomDebugObjects())
	{
		//if (BlueprintClass)
		{
			...
			TArray<UObject*> BlueprintInstances = GetRigVMAssetInterface()->GetArchetypeInstances(false, bIncludeDerivedClasses);
		...
		// this else doesn't make sense to me, since BlueprintClass doesn't exist, but we check it anyway
		// else
		// {
		...
		// 		const bool bGeneratedByThisBlueprint = bGeneratedByAnyBlueprint && GetBlueprintObj()->GetRigVMGeneratedClass() && TestObject->IsA(GetBlueprintObj()->GetRigVMGeneratedClass());
		//
...
TSharedPtr<FRigVMDebugObjectInstance> SRigVMEditorSelectedDebugObjectWidget::GetDebugObjectInstance() const
{
	...
	const FString& PathToDebug = GetRigVMAssetInterface()->GetObjectBeingDebuggedPath();
...
TSharedPtr<FString> SRigVMEditorSelectedDebugObjectWidget::GetDebugWorldName() const
{
	...
	{
		UWorld* DebugWorld = GetRigVMAssetInterface()->GetWorldBeingDebugged();
...
void SRigVMEditorSelectedDebugObjectWidget::DebugWorldSelectionChanged(TSharedPtr<FString> NewSelection, ESelectInfo::Type SelectInfo)
{
	if (NewSelection != GetDebugWorldName())
	{
		...
		for (int32 WorldIdx = 0; WorldIdx < DebugWorldNames.Num(); ++WorldIdx)
		{
			...
			{
				GetRigVMAssetInterface()->SetWorldBeingDebugged(DebugWorlds[WorldIdx].Get());

				GetRigVMAssetInterface()->SetObjectBeingDebugged(nullptr);
...
void SRigVMEditorSelectedDebugObjectWidget::DebugObjectSelectionChanged(TSharedPtr<FRigVMDebugObjectInstance> NewSelection, ESelectInfo::Type SelectInfo)
{
	if (NewSelection != GetDebugObjectInstance() && NewSelection.IsValid())
	{
		...
		GetRigVMAssetInterface()->SetObjectBeingDebugged(DebugObj);
...
{
	check(GetRigVMAssetInterface());
	if (UObject* DebugObj = GetRigVMAssetInterface()->GetObjectBeingDebugged())
	{
...
FRigVMAssetInterfacePtr SRigVMEditorSelectedDebugObjectWidget::GetRigVMAssetInterface() const
{
	return Editor.Pin()->GetRigVMAssetInterface();
}

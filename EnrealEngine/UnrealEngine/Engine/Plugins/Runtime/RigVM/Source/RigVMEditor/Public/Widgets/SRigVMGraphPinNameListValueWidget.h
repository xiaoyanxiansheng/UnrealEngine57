...
public:
	...
	UE_API void SetSelectedItem(TSharedPtr<FRigVMStringWithTag> InSelectedItem);
	UE_API void SetOptionsSource(const TArray< TSharedPtr<FRigVMStringWithTag> >* InOptionsSource);

	/** @return the item currently selected by the combo box. */
	UE_API TSharedPtr<FRigVMStringWithTag> GetSelectedItem();
...
private:
	...
	UE_API TSharedRef<ITableRow> GenerateMenuItemRow(TSharedPtr<FRigVMStringWithTag> InItem, const TSharedRef<STableViewBase>& OwnerTable);

	//** Called if the menu is closed
	UE_API void OnMenuOpenChanged(bool bOpen);

	/** Invoked when the selection in the list changes */
	UE_API void OnSelectionChanged_Internal(TSharedPtr<FRigVMStringWithTag> ProposedSelection, ESelectInfo::Type SelectInfo, bool bForce = false);

	/** Invoked when the search text changes */
	UE_API void OnSearchTextChanged(const FText& ChangedText);

	/** Invoked when the search is committed*/
	UE_API void OnSearchTextCommitted(const FText& ChangedText, ETextCommit::Type CommitType);

	/** Special case handling for search box key commands */
	UE_API FReply OnSearchTextKeyDown(const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent);

	/** Special case handling for combo list key commands */
	UE_API FReply OnComboListKeyDown(const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent);

	/** Handle clicking on the content menu */
	UE_API virtual FReply OnButtonClicked() override;

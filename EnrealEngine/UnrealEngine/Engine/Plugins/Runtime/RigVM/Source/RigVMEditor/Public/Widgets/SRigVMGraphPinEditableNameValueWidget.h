...
public:
	...
	UE_API void SetSelectedItem(TSharedPtr<FString> InSelectedItem);

	/** @return the item currently selected by the combo box. */
	UE_API TSharedPtr<FString> GetSelectedItem();
...
private:
	...
	UE_API TSharedRef<ITableRow> GenerateMenuItemRow(TSharedPtr<FString> InItem, const TSharedRef<STableViewBase>& OwnerTable);

	//** Called if the menu is closed
	UE_API void OnMenuOpenChanged(bool bOpen);

	/** Invoked when the selection in the list changes */
	UE_API void OnSelectionChanged_Internal(TSharedPtr<FString> ProposedSelection, ESelectInfo::Type SelectInfo);

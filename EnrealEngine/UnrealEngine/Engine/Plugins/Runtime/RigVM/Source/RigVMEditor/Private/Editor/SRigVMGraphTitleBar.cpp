...
void SRigVMGraphTitleBar::Construct( const FArguments& InArgs )
{
	...
	FRigVMAssetInterfacePtr BlueprintObj = FRigVMBlueprintUtils::FindAssetForGraph(this->EdGraphObj);
	if (BlueprintObj)
	{
		BlueprintTitle = FText::FromString(BlueprintObj->GetObject()->GetName());
...
void SRigVMGraphTitleBar::Refresh()
{
	...
	if (EdGraphObj)
	{
		if (FRigVMAssetInterfacePtr BlueprintObj = FRigVMBlueprintUtils::FindAssetForGraph(this->EdGraphObj))
		{
			BlueprintTitle = FText::FromString(BlueprintObj->GetObject()->GetName());
			RebuildBreadcrumbTrail();
		}

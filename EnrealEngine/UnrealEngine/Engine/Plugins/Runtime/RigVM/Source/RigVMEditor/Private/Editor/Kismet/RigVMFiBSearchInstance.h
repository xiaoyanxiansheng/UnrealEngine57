...
public:
	...
	FFindInBlueprintExpressionEvaluator(const ETextFilterExpressionEvaluatorMode InMode, FRigVMFiBSearchInstance* InSearchInstance)
	...
	/** Sets the default function handler, which supports generic functions which are categories or other object based on FRigVMImaginaryFiBData */
...
protected:
	...
	FRigVMFiBSearchInstance* SearchInstance;
...
public:
	 ...
	 */
	FRigVMSearchResult StartSearchQuery(const FString& InSearchString, FRigVMImaginaryFiBDataSharedPtr InImaginaryBlueprintRoot);
	 ...
	 * @param InImaginaryBlueprintRoot		The imaginary Blueprint to search through
	 */
	void MakeSearchQuery(const FString& InSearchString, FRigVMImaginaryFiBDataSharedPtr InImaginaryBlueprintRoot);
	 ...
	 */
	void BuildFunctionTargets(FRigVMImaginaryFiBDataSharedPtr InRootData, ERigVMSearchQueryFilter InSearchQueryFilter, TArray<FRigVMImaginaryFiBDataWeakPtr>& OutTargetPendingSearchables);
	 ...
	 * @param OutTargetPendingSearchables		List of pending imaginary items that will be sub-searched, this gets filled out by the function
	 */
	void BuildFunctionTargetsByName(FRigVMImaginaryFiBDataSharedPtr InRootData, FString InTagName, TArray<FRigVMImaginaryFiBDataWeakPtr>& OutTargetPendingSearchables);
	 ...
	 */
	bool OnFilterFunction(const FTextFilterString& A, ERigVMSearchQueryFilter InSearchQueryFilter);
	...
	void CreateFilteredResultsListFromTree(ERigVMSearchQueryFilter InSearchQueryFilter, TArray<FRigVMImaginaryFiBDataSharedPtr>& InOutValidSearchResults);

	/** Helper function to return search results given an imaginary Blueprint root */
	FRigVMSearchResult GetSearchResults(FRigVMImaginaryFiBDataSharedPtr InImaginaryBlueprintRoot);
public:
	/** Current item being searched in the Imaginary Blueprint. Must be declared as thread-safe because this is a shared object. */
	FRigVMImaginaryFiBDataWeakPtr CurrentSearchable;
	...
	TArray<FRigVMImaginaryFiBDataWeakPtr> PendingSearchables;

	/** When a function returns on an item, this is the list of items that matched the sub-search query */
	TArray< const FRigVMImaginaryFiBData* > LastFunctionResultMatchesSearchQuery;
	...
	TMultiMap< const FRigVMImaginaryFiBData*, FRigVMComponentUniqueDisplay > LastFunctionMatchingSearchComponents;
};
#endif
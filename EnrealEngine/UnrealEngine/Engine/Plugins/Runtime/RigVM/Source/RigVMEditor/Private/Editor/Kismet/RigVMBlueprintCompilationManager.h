...
struct FRigVMBlueprintCompilationManager
{
	static RIGVMEDITOR_API void Initialize();
	static RIGVMEDITOR_API void Shutdown();
	 ...
	 */
	static RIGVMEDITOR_API void FlushCompilationQueue(FUObjectSerializeContext* InLoadContext);
	
	/**
	 * Flushes the compilation queue and finishes reinstancing
	 */
	static RIGVMEDITOR_API void FlushCompilationQueueAndReinstance();
	 ...
	 */
	static RIGVMEDITOR_API void CompileSynchronously(const FRigVMBPCompileRequest& Request);

	/**
	 * Adds a newly loaded blueprint to the compilation queue
	 */
	static RIGVMEDITOR_API void NotifyBlueprintLoaded(UBlueprint* BPLoaded);

	/**
	 * Adds a blueprint to the compilation queue - useful for batch compilation
	 */
	static RIGVMEDITOR_API void QueueForCompilation(UBlueprint* BP);

	/** Returns true when UBlueprint::GeneratedClass members are up to date */
	static RIGVMEDITOR_API bool IsGeneratedClassLayoutReady();
	 ...
	 * value from there
	 */
	static RIGVMEDITOR_API bool GetDefaultValue(const UClass* ForClass, const FProperty* Property, FString& OutDefaultValueAsString);
	 ...
	 */
	static RIGVMEDITOR_API void ReparentHierarchies(const TMap<UClass*, UClass*>& OldClassToNewClass);
	 ...
	 * care has been taken.
	 */
	static RIGVMEDITOR_API void RegisterCompilerExtension(TSubclassOf<UBlueprint> BlueprintType, URigVMBlueprintCompilerExtension* Extension);
private:
	RIGVMEDITOR_API FRigVMBlueprintCompilationManager();
};

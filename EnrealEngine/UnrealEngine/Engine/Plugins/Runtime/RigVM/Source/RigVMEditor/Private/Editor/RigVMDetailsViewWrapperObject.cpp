...
UClass* URigVMDetailsViewWrapperObject::GetClassForStruct(UScriptStruct* InStruct, bool bCreateIfNeeded) const
{
	...
	FField::FLinkedListBuilder PropertyListBuilder(&WrapperClass->ChildProperties);
...
UClass* URigVMDetailsViewWrapperObject::GetClassForNodes(TArray<URigVMNode*> InNodes, bool bCreateIfNeeded) const
{
	...
	for(URigVMPin* Pin : PinsToInspect)
	{
		...
		else if(Pin->GetCPPTypeObject())
		{
			...
			{
				const bool bIsClass = RigVMTypeUtils::IsUClassType(Pin->GetCPPType());
				if(bIsClass)
				{
					FClassProperty* ClassProperty = new FClassProperty(PropertyOwner, Pin->GetFName(), RF_Public);
					ClassProperty->MetaClass = Class; 
					(*ElementProperty) = ClassProperty;
				}
				else
				{
		...
		PropertyListBuilder.AppendNoTerminate(*Property);
	}

...
class SRigVMVariableMappingTreeRow
...
private:
	...
	UE_API FReply OnClearButtonClicked();
	UE_API FText GetFilterText() const;

	UE_API TSharedRef<SWidget> MakeVarOptionComboWidget(TSharedPtr<FString> InItem);
	UE_API void OnVarOptionSourceChanged(TSharedPtr<FString> NewSelection, ESelectInfo::Type SelectInfo);
	UE_API FText GetVarOptionComboBoxContent() const;
	UE_API FText GetVarOptionComboBoxToolTip() const;
	UE_API void OnVarOptionComboOpening();
	UE_API bool IsVarOptionEnabled() const;
	UE_API TSharedPtr<FString> GetVarOptionString(FName VarOptionName) const;

	UE_API ECheckBoxState IsPinChecked() const;
	UE_API void OnPinCheckStatusChanged(ECheckBoxState NewState);
	UE_API bool IsPinEnabled() const;
...
public:
	...
	*
	*/
	UE_API void Construct(const FArguments& InArgs/*, FSimpleMulticastDelegate& InOnPostUndo*/);
	...
	*
	*/
	UE_API void OnFilterTextCommitted(const FText& SearchText, ETextCommit::Type CommitInfo);
	...
	*/
	UE_API TSharedRef<ITableRow> GenerateVariableMappingRow(TSharedPtr<FRigVMVariableMappingInfo> InInfo, const TSharedRef<STableViewBase>& OwnerTable);

	/**
	* Handler for the delete of retarget source
	*/
	UE_API void RefreshVariableMappingList();
...
private:
	...
	*/
	...
	UE_API TSharedRef<ITableRow> MakeTreeRowWidget(TSharedPtr<FRigVMVariableMappingInfo> InInfo, const TSharedRef<STableViewBase>& OwnerTable);

	// Get the children for the provided bone info
	UE_API void GetChildrenForInfo(TSharedPtr<FRigVMVariableMappingInfo> InInfo, TArray< TSharedPtr<FRigVMVariableMappingInfo> >& OutChildren);

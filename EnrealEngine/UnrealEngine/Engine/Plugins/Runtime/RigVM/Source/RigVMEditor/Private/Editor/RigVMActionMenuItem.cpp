...
#include UE_INLINE_GENERATED_CPP_BY_NAME(RigVMActionMenuItem)
...
static void FBlueprintMenuActionItemImpl::DirtyBlueprintFromNewNode(UEdGraphNode* SpawnedNode)
{
	...
	FRigVMAssetInterfacePtr Blueprint = FRigVMBlueprintUtils::FindAssetForGraph(NodeGraph);
	check(Blueprint != nullptr);

	if (!Blueprint || !SpawnedNode)
	{
		return;
	}
	...
	{
		Blueprint->MarkAssetAsStructurallyModified();
	}
	else
	{
		Blueprint->MarkAssetAsModified();
	}
}

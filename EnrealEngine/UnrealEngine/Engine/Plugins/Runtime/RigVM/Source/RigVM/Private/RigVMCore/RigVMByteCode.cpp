...
FRigVMInstructionArray::FRigVMInstructionArray(const FRigVMByteCode& InByteCode, bool bByteCodeIsAligned)
{
	...
	while (ByteIndex < InByteCode.Num())
	{
		...
		const int32 InstructionIndex = Instructions.Num(); 
		Instructions.Add(FRigVMInstruction(OpCode, InstructionIndex, ByteIndex, OperandAlignment));
...
#if WITH_EDITORONLY_DATA
	const FRigVMInstruction Instruction(Op.OpCode, NumInstructions - 1, ByteIndex);
	const FRigVMOperandArray InputOperands(&InSource, 1);
	const FRigVMOperandArray OutputOperands(&InTarget, 1);
	SetOperandsForInstruction(Instruction, InputOperands, OutputOperands);
#endif
...
TArray<int32> FRigVMByteCode::GetInstructionsForInputOperand(const FRigVMOperand& InOperand) const
{
	TArray<int32> InstructionIndices;
	...
	for(int32 InstructionIndex = 0; InstructionIndex < GetNumInstructions(); InstructionIndex++)
	{
		if(InputOperandsPerInstruction[InstructionIndex].Contains(InOperand))
		{
			InstructionIndices.Add(InstructionIndex);
		}
...
TArray<int32> FRigVMByteCode::GetInstructionsForOutputOperand(const FRigVMOperand& InOperand) const
{
	TArray<int32> InstructionIndices;
	...
	for(int32 InstructionIndex = 0; InstructionIndex < GetNumInstructions(); InstructionIndex++)
	{
		if(OutputOperandsPerInstruction[InstructionIndex].Contains(InOperand))
		{
			InstructionIndices.Add(InstructionIndex);
		}
...
void FRigVMByteCode::SetOperandsForInstruction(const FRigVMInstruction& InInstruction, const FRigVMOperandArray& InputOperands,
	const FRigVMOperandArray& OutputOperands)
{
	const FRigVMOperandArray OperandsForInstruction = GetOperandsForOp(InInstruction);
	const int32 InstructionIndex = InInstruction.Index;
	check(InInstruction.Index != INDEX_NONE && InInstruction.Index < NumInstructions);
	
	if (InputOperandsPerInstruction.Num() <= InstructionIndex)
	{
		InputOperandsPerInstruction.AddZeroed(1 + InstructionIndex - InputOperandsPerInstruction.Num());
	}
	InputOperandsPerInstruction[InstructionIndex].Reset();
	InputOperandsPerInstruction[InstructionIndex].Reserve(InputOperands.Num());
	...
	for(int32 OperandIndex = 0; OperandIndex < InputOperands.Num(); OperandIndex++)
	{
		...
		check(OperandsForInstruction.Contains(InputOperands[OperandIndex]));
		InputOperandsPerInstruction[InstructionIndex].Add(InputOperands[OperandIndex]);
	}

	if (OutputOperandsPerInstruction.Num() <= InstructionIndex)
	{
		OutputOperandsPerInstruction.AddZeroed(1 + InstructionIndex - OutputOperandsPerInstruction.Num());
	}
	OutputOperandsPerInstruction[InstructionIndex].Reset();
	OutputOperandsPerInstruction[InstructionIndex].Reserve(OutputOperands.Num());
	...
	for(int32 OperandIndex = 0; OperandIndex < OutputOperands.Num(); OperandIndex++)
	{
		...
		check(OperandsForInstruction.Contains(OutputOperands[OperandIndex]));
		OutputOperandsPerInstruction[InstructionIndex].Add(OutputOperands[OperandIndex]);
	}
}

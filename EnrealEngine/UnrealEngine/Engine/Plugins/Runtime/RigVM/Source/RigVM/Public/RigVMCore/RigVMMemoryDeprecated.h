...
public:
	...
	UE_API FRigVMRegisterOffset(UScriptStruct* InScriptStruct, const FString& InSegmentPath, int32 InInitialOffset = 0, uint16 InElementSize = 0, const FName& InCPPType = NAME_None);

	// returns the data pointer within a container
	UE_API uint8* GetData(uint8* InContainer) const;
	...
	UE_API bool ContainsArraySegment() const;

	UE_API bool operator == (const FRigVMRegisterOffset& InOther) const;
	...
	UE_API uint16 GetElementSize() const;
...
public:
	...
	UE_API FRigVMMemoryContainer(bool bInUseNames = true);
	
	UE_API FRigVMMemoryContainer(const FRigVMMemoryContainer& Other);
	UE_API ~FRigVMMemoryContainer();

	UE_API bool CopyRegisters(const FRigVMMemoryContainer &InOther);
	UE_API FRigVMMemoryContainer& operator= (const FRigVMMemoryContainer &InOther);
...
private:
	...
	uint8* GetDataPtr(const FRigVMRegister& Register, int32 InRegisterOffset = INDEX_NONE, int32 InSliceIndex = 0, bool bArrayContent = false) const
	{
		...
		{
			Ptr = (uint8*)&Data[static_cast<int32>(Register.GetWorkByteIndex())];
		...
		{
			Ptr = (uint8*)&Data[static_cast<int32>(Register.GetWorkByteIndex(InSliceIndex))];
		}
...
public:
	...
	UE_API int32 GetOrAddRegisterOffset(int32 InRegisterIndex, int32 InArrayElement = 0);

	// Adds a register path and returns its index
	UE_API int32 GetOrAddRegisterOffset(int32 InRegisterIndex, const FString& InSegmentPath, int32 InArrayElement = 0);

	// Adds a register path and returns its index
	UE_API int32 GetOrAddRegisterOffset(int32 InRegisterIndex, UScriptStruct* InScriptStruct, const FString& InSegmentPath, int32 InInitialOffset = 0, int32 InElementSize = 0);

	// Updates internal data for topological changes
	UE_API void UpdateRegisters();

	// Allocates a new named register
	UE_API int32 Allocate(const FName& InNewName, int32 InElementSize, int32 InElementCount, int32 InSliceCount, const uint8* InDataPtr = nullptr, bool bUpdateRegisters = true);

	// Allocates a new unnamed register
	UE_API int32 Allocate(int32 InElementSize, int32 InElementCount, int32 InSliceCount, const uint8* InDataPtr = nullptr, bool bUpdateRegisters = true);

	// Performs optional construction of data within a struct register
	UE_API bool Construct(int32 InRegisterIndex, int32 InElementIndex = INDEX_NONE, int32 InSliceIndex = 0);

	// Performs optional destruction of data within a struct register
	UE_API bool Destroy(int32 InRegisterIndex, int32 InElementIndex = INDEX_NONE, int32 InSliceIndex = 0);

	// Ensures to add a script struct to the internal map if needed
	UE_API int32 FindOrAddScriptStruct(UScriptStruct* InScriptStruct);

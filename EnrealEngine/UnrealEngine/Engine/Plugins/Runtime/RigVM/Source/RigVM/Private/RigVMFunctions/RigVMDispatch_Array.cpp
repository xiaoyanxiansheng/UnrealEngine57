...
const TArray<FRigVMTemplateArgumentInfo>& FRigVMDispatch_ArrayMake::GetArgumentInfos(FRigVMRegistryHandle& InRegistry) const
{
	if (CachedArgumentInfos.IsEmpty())
	{
		CachedArgumentInfos.Emplace(CreateArgumentInfo(ValuesName, ERigVMPinDirection::Input));
		CachedArgumentInfos.Emplace(CreateArgumentInfo(ArrayName, ERigVMPinDirection::Output));
	}
	return CachedArgumentInfos;
}

FRigVMTemplateTypeMap FRigVMDispatch_ArrayMake::OnNewArgumentType(const FName& InArgumentName, TRigVMTypeIndex InTypeIndex, FRigVMRegistryHandle& InRegistry) const
{
...
FName FRigVMDispatch_ArrayMake::GetArgumentNameForOperandIndex(int32 InOperandIndex, int32 InTotalOperands, FRigVMRegistryHandle& InRegistry) const
{
...
void FRigVMDispatch_ArrayMake::Execute(FRigVMExtendedExecuteContext& InContext, FRigVMMemoryHandleArray Handles, FRigVMPredicateBranchArray Predicates)
{
	...
	FScriptArrayHelper ArrayHelper(ArrayProperty, Handles.Last().GetOutputData());
	...
	if(InContext.IsValidArraySize(Num))
	{
		...
		{
			const uint8* SourceMemory = Handles[Index].GetInputData();
			uint8* TargetMemory = ArrayHelper.GetRawPtr(Index);
			URigVMMemoryStorage::CopyProperty(ArrayProperty->Inner, TargetMemory, Handles[Index].GetResolvedProperty(), SourceMemory);
		}
	}
	else
	{
...
FName FRigVMDispatch_ArrayReset::GetArgumentNameForOperandIndex(int32 InOperandIndex, int32 InTotalOperands, FRigVMRegistryHandle& InRegistry) const
{
	check(InTotalOperands == 1);
	return ArrayName;
}

const TArray<FRigVMTemplateArgumentInfo>& FRigVMDispatch_ArrayReset::GetArgumentInfos(FRigVMRegistryHandle& InRegistry) const
{
	if(CachedArgumentInfos.IsEmpty())
	{
		CachedArgumentInfos.Emplace(CreateArgumentInfo(ArrayName, ERigVMPinDirection::IO));
	}
	return CachedArgumentInfos;
}

FRigVMTemplateTypeMap FRigVMDispatch_ArrayReset::OnNewArgumentType(const FName& InArgumentName, TRigVMTypeIndex InTypeIndex, FRigVMRegistryHandle& InRegistry) const
{
...
void FRigVMDispatch_ArrayReset::Execute(FRigVMExtendedExecuteContext& InContext, FRigVMMemoryHandleArray Handles, FRigVMPredicateBranchArray Predicates)
{
	...
	FScriptArrayHelper ArrayHelper(ArrayProperty, Handles[0].GetOutputData());
	ArrayHelper.Resize(0);
}
...
FName FRigVMDispatch_ArrayGetNum::GetArgumentNameForOperandIndex(int32 InOperandIndex, int32 InTotalOperands, FRigVMRegistryHandle& InRegistry) const
{
...
const TArray<FRigVMTemplateArgumentInfo>& FRigVMDispatch_ArrayGetNum::GetArgumentInfos(FRigVMRegistryHandle& InRegistry) const
{
	if(CachedArgumentInfos.IsEmpty())
	{
		CachedArgumentInfos.Emplace( CreateArgumentInfo(ArrayName, ERigVMPinDirection::Input) );
		CachedArgumentInfos.Emplace( CreateArgumentInfo(NumName, ERigVMPinDirection::Output) );
	}
	return CachedArgumentInfos;
}

FRigVMTemplateTypeMap FRigVMDispatch_ArrayGetNum::OnNewArgumentType(const FName& InArgumentName, TRigVMTypeIndex InTypeIndex, FRigVMRegistryHandle& InRegistry) const
{
	check(InArgumentName == ArrayName);
	return {
		{ArrayName, InTypeIndex},
...
{
	const FArrayProperty* ArrayProperty = CastFieldChecked<FArrayProperty>(Handles[0].GetResolvedProperty());
	const FScriptArrayHelper ArrayHelper(ArrayProperty, Handles[0].GetInputData());
	int32* Num = (int32*)Handles[1].GetOutputData();
	*Num = ArrayHelper.Num();
}
...
FName FRigVMDispatch_ArraySetNum::GetArgumentNameForOperandIndex(int32 InOperandIndex, int32 InTotalOperands, FRigVMRegistryHandle& InRegistry) const
{
	static const FLazyName ArgumentNames[] = {
		ArrayName,
		NumName
...
const TArray<FRigVMTemplateArgumentInfo>& FRigVMDispatch_ArraySetNum::GetArgumentInfos(FRigVMRegistryHandle& InRegistry) const
{
	if (CachedArgumentInfos.IsEmpty())
	{
		CachedArgumentInfos.Emplace(CreateArgumentInfo(ArrayName, ERigVMPinDirection::IO));
		CachedArgumentInfos.Emplace(CreateArgumentInfo(NumName, ERigVMPinDirection::Input));
	};
	return CachedArgumentInfos;
}

FRigVMTemplateTypeMap FRigVMDispatch_ArraySetNum::OnNewArgumentType(const FName& InArgumentName, TRigVMTypeIndex InTypeIndex, FRigVMRegistryHandle& InRegistry) const
{
	check(InArgumentName == ArrayName);
	return {
		{ArrayName, InTypeIndex},
...
void FRigVMDispatch_ArraySetNum::Execute(FRigVMExtendedExecuteContext& InContext, FRigVMMemoryHandleArray Handles, FRigVMPredicateBranchArray Predicates)
{
	const FArrayProperty* ArrayProperty = CastFieldChecked<FArrayProperty>(Handles[0].GetProperty());
	FScriptArrayHelper ArrayHelper(ArrayProperty, Handles[0].GetOutputData());
	const int32* Num = (const int32*)Handles[1].GetInputData();
	if(InContext.IsValidArraySize(*Num))
	{
		ArrayHelper.Resize(*Num);
	}
...
FName FRigVMDispatch_ArrayInit::GetArgumentNameForOperandIndex(int32 InOperandIndex, int32 InTotalOperands, FRigVMRegistryHandle& InRegistry) const
{
	static const FLazyName ArgumentNames[] = {
		ArrayName,
		NumName,
...
const TArray<FRigVMTemplateArgumentInfo>& FRigVMDispatch_ArrayInit::GetArgumentInfos(FRigVMRegistryHandle& InRegistry) const
{
	if (CachedArgumentInfos.IsEmpty())
	{
		CachedArgumentInfos.Emplace(CreateArgumentInfo(ArrayName, ERigVMPinDirection::IO));
		CachedArgumentInfos.Emplace(CreateArgumentInfo(NumName, ERigVMPinDirection::Input));
		CachedArgumentInfos.Emplace(CreateArgumentInfo(ElementName, ERigVMPinDirection::Input));
	};
	return CachedArgumentInfos;
}

FRigVMTemplateTypeMap FRigVMDispatch_ArrayInit::OnNewArgumentType(const FName& InArgumentName, TRigVMTypeIndex InTypeIndex, FRigVMRegistryHandle& InRegistry) const
{
	check(InArgumentName == ArrayName);
	return {
		{ArrayName, InTypeIndex},
		{NumName, RigVMTypeUtils::TypeIndex::Int32},
		{ElementName, InRegistry->GetBaseTypeFromArrayTypeIndex_NoLock(InTypeIndex)}
	};
}
...
void FRigVMDispatch_ArrayInit::Execute(FRigVMExtendedExecuteContext& InContext, FRigVMMemoryHandleArray Handles, FRigVMPredicateBranchArray Predicates)
{
	...
	const uint8* ValueData = Handles[2].GetInputData();
...
FName FRigVMDispatch_ArrayGetAtIndex::GetArgumentNameForOperandIndex(int32 InOperandIndex, int32 InTotalOperands, FRigVMRegistryHandle& InRegistry) const
{
	static const FLazyName ArgumentNames[] = {
		ArrayName,
		IndexName,
...
const TArray<FRigVMTemplateArgumentInfo>& FRigVMDispatch_ArrayGetAtIndex::GetArgumentInfos(FRigVMRegistryHandle& InRegistry) const
{
	if (CachedArgumentInfos.IsEmpty())
	{
		CachedArgumentInfos.Emplace( CreateArgumentInfo(ArrayName, ERigVMPinDirection::Input) );
		CachedArgumentInfos.Emplace( CreateArgumentInfo(IndexName, ERigVMPinDirection::Input) );
		CachedArgumentInfos.Emplace( CreateArgumentInfo(ElementName, ERigVMPinDirection::Output) );
	}
	return CachedArgumentInfos;
}

FRigVMTemplateTypeMap FRigVMDispatch_ArrayGetAtIndex::OnNewArgumentType(const FName& InArgumentName, TRigVMTypeIndex InTypeIndex, FRigVMRegistryHandle& InRegistry) const
{
	...
	return {
		{ArrayName, InRegistry->GetArrayTypeFromBaseTypeIndex_NoLock(InTypeIndex)},
		{IndexName, RigVMTypeUtils::TypeIndex::Int32},
...
{
	const FArrayProperty* ArrayProperty = CastFieldChecked<FArrayProperty>(Handles[0].GetResolvedProperty());
	FScriptArrayHelper ArrayHelper(ArrayProperty, Handles[0].GetInputData());
	const int32 Index = *(const int32*)Handles[1].GetInputData();
	int32 OutIndex = Index;
	uint8* TargetMemory = Handles[2].GetOutputData();

	if(InContext.IsValidArrayIndex(OutIndex, ArrayHelper))
	{
		const uint8* SourceMemory = ArrayHelper.GetRawPtr(OutIndex);
...
FName FRigVMDispatch_ArraySetAtIndex::GetArgumentNameForOperandIndex(int32 InOperandIndex, int32 InTotalOperands, FRigVMRegistryHandle& InRegistry) const
{
	static const FLazyName ArgumentNames[] = {
		ArrayName,
		IndexName,
...
const TArray<FRigVMTemplateArgumentInfo>& FRigVMDispatch_ArraySetAtIndex::GetArgumentInfos(FRigVMRegistryHandle& InRegistry) const
{
	if (CachedArgumentInfos.IsEmpty())
	{
		CachedArgumentInfos.Emplace( CreateArgumentInfo(ArrayName, ERigVMPinDirection::IO) );
...
FRigVMTemplateTypeMap FRigVMDispatch_ArraySetAtIndex::OnNewArgumentType(const FName& InArgumentName, TRigVMTypeIndex InTypeIndex, FRigVMRegistryHandle& InRegistry) const
{
	check(InArgumentName == ArrayName || InArgumentName == ElementName);
...
void FRigVMDispatch_ArraySetAtIndex::Execute(FRigVMExtendedExecuteContext& InContext, FRigVMMemoryHandleArray Handles, FRigVMPredicateBranchArray Predicates)
{
	...
	const int32 Index = *(const int32*)Handles[1].GetInputData();
	int32 OutIndex = Index;
	const uint8* SourceMemory = Handles[2].GetInputData();

	if(InContext.IsValidArrayIndex(OutIndex, ArrayHelper))
	{
		uint8* TargetMemory = ArrayHelper.GetRawPtr(OutIndex);
		URigVMMemoryStorage::CopyProperty(ArrayProperty->Inner, TargetMemory, Handles[2].GetResolvedProperty(), SourceMemory);
	}
}
...
FName FRigVMDispatch_ArrayAdd::GetArgumentNameForOperandIndex(int32 InOperandIndex, int32 InTotalOperands, FRigVMRegistryHandle& InRegistry) const
{
	static const FLazyName ArgumentNames[] = {
		ArrayName,
		ElementName,
...
const TArray<FRigVMTemplateArgumentInfo>& FRigVMDispatch_ArrayAdd::GetArgumentInfos(FRigVMRegistryHandle& InRegistry) const
{
	if (CachedArgumentInfos.IsEmpty())
	{
		CachedArgumentInfos.Emplace( CreateArgumentInfo(ArrayName, ERigVMPinDirection::IO) );
		CachedArgumentInfos.Emplace( CreateArgumentInfo(ElementName, ERigVMPinDirection::Input) );
		CachedArgumentInfos.Emplace( CreateArgumentInfo(IndexName, ERigVMPinDirection::Output) );
	}
	return CachedArgumentInfos;
}
...
void FRigVMDispatch_ArrayAdd::Execute(FRigVMExtendedExecuteContext& InContext, FRigVMMemoryHandleArray Handles, FRigVMPredicateBranchArray Predicates)
{
	const FArrayProperty* ArrayProperty = CastFieldChecked<FArrayProperty>(Handles[0].GetProperty());
	FScriptArrayHelper ArrayHelper(ArrayProperty, Handles[0].GetOutputData());
	const uint8* SourceMemory = Handles[1].GetInputData();
	int32& Index = *(int32*)Handles[2].GetOutputData();

	if(InContext.IsValidArraySize(ArrayHelper.Num() + 1))
	{
		Index = ArrayHelper.AddValue();
		uint8* TargetMemory = ArrayHelper.GetRawPtr(Index);
		URigVMMemoryStorage::CopyProperty(ArrayProperty->Inner, TargetMemory, Handles[1].GetResolvedProperty(), SourceMemory);
	}
	else
	{
		Index = INDEX_NONE;
...
void FRigVMDispatch_ArrayInsert::Execute(FRigVMExtendedExecuteContext& InContext, FRigVMMemoryHandleArray Handles, FRigVMPredicateBranchArray Predicates)
{
	const FArrayProperty* ArrayProperty = CastFieldChecked<FArrayProperty>(Handles[0].GetProperty());
	FScriptArrayHelper ArrayHelper(ArrayProperty, Handles[0].GetOutputData());
	int32 Index = *(int32*)Handles[1].GetOutputData();
	const uint8* SourceMemory = Handles[2].GetInputData();
...
FName FRigVMDispatch_ArrayRemove::GetArgumentNameForOperandIndex(int32 InOperandIndex, int32 InTotalOperands, FRigVMRegistryHandle& InRegistry) const
{
	static const FLazyName ArgumentNames[] = {
		ArrayName,
		IndexName
...
const TArray<FRigVMTemplateArgumentInfo>& FRigVMDispatch_ArrayRemove::GetArgumentInfos(FRigVMRegistryHandle& InRegistry) const
{
	if (CachedArgumentInfos.IsEmpty())
	{
		CachedArgumentInfos.Emplace( CreateArgumentInfo(ArrayName, ERigVMPinDirection::IO) );
...
FRigVMTemplateTypeMap FRigVMDispatch_ArrayRemove::OnNewArgumentType(const FName& InArgumentName, TRigVMTypeIndex InTypeIndex, FRigVMRegistryHandle& InRegistry) const
{
	check(InArgumentName == ArrayName);
...
void FRigVMDispatch_ArrayRemove::Execute(FRigVMExtendedExecuteContext& InContext, FRigVMMemoryHandleArray Handles, FRigVMPredicateBranchArray Predicates)
{
	...
	if(InContext.IsValidArrayIndex(OutIndex, ArrayHelper))
	{
		ArrayHelper.RemoveValues(OutIndex, 1);
	}
}
...
FName FRigVMDispatch_ArrayFind::GetArgumentNameForOperandIndex(int32 InOperandIndex, int32 InTotalOperands, FRigVMRegistryHandle& InRegistry) const
{
	static const FLazyName ArgumentNames[] = {
		ArrayName,
		ElementName,
...
const TArray<FRigVMTemplateArgumentInfo>& FRigVMDispatch_ArrayFind::GetArgumentInfos(FRigVMRegistryHandle& InRegistry) const
{
	if (CachedArgumentInfos.IsEmpty())
	{
		CachedArgumentInfos.Emplace(CreateArgumentInfo(ArrayName, ERigVMPinDirection::Input));
		CachedArgumentInfos.Emplace(CreateArgumentInfo(ElementName, ERigVMPinDirection::Input));
		CachedArgumentInfos.Emplace(CreateArgumentInfo(IndexName, ERigVMPinDirection::Output));
		CachedArgumentInfos.Emplace(CreateArgumentInfo(SuccessName, ERigVMPinDirection::Output));
	}
	return CachedArgumentInfos;
}

FRigVMTemplateTypeMap FRigVMDispatch_ArrayFind::OnNewArgumentType(const FName& InArgumentName, TRigVMTypeIndex InTypeIndex, FRigVMRegistryHandle& InRegistry) const
{
	check(InArgumentName == ArrayName || InArgumentName == ElementName);
...
void FRigVMDispatch_ArrayFind::Execute(FRigVMExtendedExecuteContext& InContext, FRigVMMemoryHandleArray Handles, FRigVMPredicateBranchArray Predicates)
{
	...
	FScriptArrayHelper ArrayHelper(ArrayProperty, Handles[0].GetInputData());
	int32& Index = *(int32*)Handles[2].GetOutputData();
	bool& bSuccess = *(bool*)Handles[3].GetOutputData();

	Index = INDEX_NONE;
	bSuccess = false;

	const FProperty* PropertyA = Handles[1].GetResolvedProperty();
	const FProperty* PropertyB = ArrayProperty->Inner;

	if(PropertyA->SameType(PropertyB))
	{
		const uint8* MemoryA = Handles[1].GetInputData();
...
FName FRigVMDispatch_ArrayAppend::GetArgumentNameForOperandIndex(int32 InOperandIndex, int32 InTotalOperands, FRigVMRegistryHandle& InRegistry) const
{
	static const FLazyName ArgumentNames[] = {
		ArrayName,
		OtherName
...
const TArray<FRigVMTemplateArgumentInfo>& FRigVMDispatch_ArrayAppend::GetArgumentInfos(FRigVMRegistryHandle& InRegistry) const
{
	if (CachedArgumentInfos.IsEmpty())
	{
		CachedArgumentInfos.Emplace(CreateArgumentInfo(ArrayName, ERigVMPinDirection::IO));
		CachedArgumentInfos.Emplace(CreateArgumentInfo(OtherName, ERigVMPinDirection::Input));
	}
	return CachedArgumentInfos;
}


FRigVMTemplateTypeMap FRigVMDispatch_ArrayAppend::OnNewArgumentType(const FName& InArgumentName, TRigVMTypeIndex InTypeIndex, FRigVMRegistryHandle& InRegistry) const
{
...
{
	const FArrayProperty* SourceArrayProperty = CastFieldChecked<FArrayProperty>(Handles[1].GetResolvedProperty());
	FScriptArrayHelper SourceArrayHelper(SourceArrayProperty, Handles[1].GetInputData());

	if(SourceArrayHelper.Num() > 0)
	{
		const FArrayProperty* TargetArrayProperty = CastFieldChecked<FArrayProperty>(Handles[0].GetProperty());
		FScriptArrayHelper TargetArrayHelper(TargetArrayProperty, Handles[0].GetOutputData());
...
FName FRigVMDispatch_ArrayClone::GetArgumentNameForOperandIndex(int32 InOperandIndex, int32 InTotalOperands, FRigVMRegistryHandle& InRegistry) const
{
	static const FLazyName ArgumentNames[] = {
		ArrayName,
		CloneName
...
const TArray<FRigVMTemplateArgumentInfo>& FRigVMDispatch_ArrayClone::GetArgumentInfos(FRigVMRegistryHandle& InRegistry) const
{
	if (CachedArgumentInfos.IsEmpty())
	{
		CachedArgumentInfos.Emplace(CreateArgumentInfo(ArrayName, ERigVMPinDirection::Input));
		CachedArgumentInfos.Emplace(CreateArgumentInfo(CloneName, ERigVMPinDirection::Output));
	}
	return CachedArgumentInfos;
}

FRigVMTemplateTypeMap FRigVMDispatch_ArrayClone::OnNewArgumentType(const FName& InArgumentName, TRigVMTypeIndex InTypeIndex, FRigVMRegistryHandle& InRegistry) const
{
...
void FRigVMDispatch_ArrayClone::Execute(FRigVMExtendedExecuteContext& InContext, FRigVMMemoryHandleArray Handles, FRigVMPredicateBranchArray Predicates)
{
	...
	FScriptArrayHelper CloneHelper(CloneProperty, Handles[1].GetOutputData());
...
{
	const FArrayProperty* OtherProperty = CastFieldChecked<FArrayProperty>(Handles[1].GetResolvedProperty());
	FScriptArrayHelper OtherHelper(OtherProperty, Handles[1].GetInputData());
...
FName FRigVMDispatch_ArrayDifference::GetArgumentNameForOperandIndex(int32 InOperandIndex, int32 InTotalOperands, FRigVMRegistryHandle& InRegistry) const
{
	static const FLazyName ArgumentNames[] = {
		ArrayName,
		OtherName,
...
const TArray<FRigVMTemplateArgumentInfo>& FRigVMDispatch_ArrayDifference::GetArgumentInfos(FRigVMRegistryHandle& InRegistry) const
{
	if (CachedArgumentInfos.IsEmpty())
	{
		CachedArgumentInfos.Emplace(CreateArgumentInfo(ArrayName, ERigVMPinDirection::Input));
		CachedArgumentInfos.Emplace(CreateArgumentInfo(OtherName, ERigVMPinDirection::Input));
		CachedArgumentInfos.Emplace(CreateArgumentInfo(ResultName, ERigVMPinDirection::Output));
	}
	return CachedArgumentInfos;
}

FRigVMTemplateTypeMap FRigVMDispatch_ArrayDifference::OnNewArgumentType(const FName& InArgumentName, TRigVMTypeIndex InTypeIndex, FRigVMRegistryHandle& InRegistry) const
{
...
void FRigVMDispatch_ArrayDifference::Execute(FRigVMExtendedExecuteContext& InContext, FRigVMMemoryHandleArray Handles, FRigVMPredicateBranchArray Predicates)
{
	...
	FScriptArrayHelper ResultHelper(ResultProperty, Handles[2].GetOutputData());
...
FName FRigVMDispatch_ArrayIterator::GetArgumentNameForOperandIndex(int32 InOperandIndex, int32 InTotalOperands, FRigVMRegistryHandle& InRegistry) const
{
	static const FLazyName ArgumentNames[] = {
		ArrayName,
		ElementName,
...
const TArray<FRigVMTemplateArgumentInfo>& FRigVMDispatch_ArrayIterator::GetArgumentInfos(FRigVMRegistryHandle& InRegistry) const
{
	if (CachedArgumentInfos.IsEmpty())
	{
		CachedArgumentInfos.Emplace(CreateArgumentInfo(ArrayName, ERigVMPinDirection::Input));
		CachedArgumentInfos.Emplace(CreateArgumentInfo(ElementName, ERigVMPinDirection::Output));
		CachedArgumentInfos.Emplace(CreateArgumentInfo(IndexName, ERigVMPinDirection::Output));
		CachedArgumentInfos.Emplace(CreateArgumentInfo(CountName, ERigVMPinDirection::Output));
		CachedArgumentInfos.Emplace(CreateArgumentInfo(RatioName, ERigVMPinDirection::Output));
...
const TArray<FRigVMExecuteArgument>& FRigVMDispatch_ArrayIterator::GetExecuteArguments_Impl(
	const FRigVMDispatchContext& InContext) const
{
	UE_AUTORTFM_DECLARE_THREAD_LOCAL_VAR(TArray<FRigVMExecuteArgument>, ExecuteArguments);
...
void FRigVMDispatch_ArrayIterator::Execute(FRigVMExtendedExecuteContext& InContext, FRigVMMemoryHandleArray Handles, FRigVMPredicateBranchArray Predicates)
{
	...
	FScriptArrayHelper ArrayHelper(ArrayProperty, Handles[0].GetInputData());
	int32& Index = *(int32*)Handles[2].GetOutputData();
	int32& Count = *(int32*)Handles[3].GetOutputData();
	float& Ratio = *(float*)Handles[4].GetOutputData();
	FName& Block = *(FName*)Handles[5].GetOutputData();
	...
	uint8* TargetMemory = Handles[1].GetOutputData();

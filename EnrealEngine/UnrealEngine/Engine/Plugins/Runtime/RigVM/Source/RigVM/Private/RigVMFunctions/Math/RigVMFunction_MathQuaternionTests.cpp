...
namespace FRigVMFunction_MathQuatTest_Utils
{
	...
	bool IsNearlyEqual(const FVector& A, const FVector& B, double Tolerance = 0.001)
	{
	...
	bool IsNearlyEqual(const FQuat& A, const FQuat& B, double Tolerance = 0.001)
	{
		return FMath::IsNearlyEqual(A.X, B.X, Tolerance) &&
			   FMath::IsNearlyEqual(A.Y, B.Y, Tolerance) &&
			   FMath::IsNearlyEqual(A.Z, B.Z, Tolerance) &&
	...
	bool IsNearlyEqual(const FRotator& A, const FRotator& B, double Tolerance = 0.001)
	{
		return FMath::IsNearlyEqual(A.Yaw, B.Yaw, Tolerance) &&
			   FMath::IsNearlyEqual(A.Roll, B.Roll, Tolerance) &&
			   FMath::IsNearlyEqual(A.Pitch, B.Pitch, Tolerance);
	}

	bool IsNearlyEqual(const FQuat& A, const FVector& B, double Tolerance = 0.001)
	{
		const FVector Euler = A.Euler();
...
IMPLEMENT_RIGVMSTRUCT_AUTOMATION_TEST(FRigVMFunction_MathQuaternionDot)
{
	...
	Execute();
	AddErrorIfFalse(FRigVMFunction_MathQuatTest_Utils::IsNearlyEqual(Unit.Result, static_cast<float>(Unit.A | Unit.B)), TEXT("unexpected result"));
	return true;
}

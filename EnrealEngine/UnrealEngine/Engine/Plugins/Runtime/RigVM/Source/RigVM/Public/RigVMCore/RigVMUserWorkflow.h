...
public:
	...
	UE_API bool Perform(const URigVMUserWorkflowOptions* InOptions, UObject* InController) const;

protected:

	UE_API bool ValidateOptions(const URigVMUserWorkflowOptions* InOptions) const;
...
public:
	...
	UFUNCTION(BlueprintCallable, Category=Options)
	UE_API bool RequiresDialog() const;
	...
	UE_API void Report(EMessageSeverity::Type InSeverity, const FString& InMessage) const;

	template <typename... Types>
	void Reportf(EMessageSeverity::Type InSeverity, UE::Core::TCheckedFormatString<FString::FmtCharType, Types...> Fmt, Types... Args) const
	{
...
protected:
	...
	UE_API virtual bool RequiresDialog(const FProperty* InProperty) const;

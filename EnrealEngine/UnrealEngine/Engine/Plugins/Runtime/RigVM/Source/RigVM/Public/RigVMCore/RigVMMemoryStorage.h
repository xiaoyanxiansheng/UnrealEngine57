...
public:
	 ...
	 */
	UE_DEPRECATED(5.7, "Please use GetInputData, GetOutputData or GetPrivateData instead.")
	uint8* GetData(bool bFollowPropertyPath, int32 InSliceIndex)
	{
	 ...
	 */
	UE_DEPRECATED(5.7, "Please use GetInputData, GetOutputData or GetPrivateData instead.")
	const uint8* GetData(bool bFollowPropertyPath, int32 InSliceIndex) const
	{
		return GetData_Internal(bFollowPropertyPath, InSliceIndex);
	}
	 ...
	 * @return The traversed memory cached by this handle.
	 */
	const uint8* GetInputData(int32 InSliceIndex = INDEX_NONE) const
	{
		return GetData_Internal(true, InSliceIndex);
	}
	 ...
	 * @return The traversed memory cached by this handle.
	 */
	uint8* GetOutputData(int32 InSliceIndex = INDEX_NONE)
	{
		return GetData_Internal(false, InSliceIndex);
	}
	 ...
	 * @return The traversed memory cached by this handle.
	 */
	uint8* GetPrivateData(int32 InSliceIndex)
	{
		return GetData_Internal(false, InSliceIndex);
	}
	 ...
	 */
	template<typename ComputedType>
	TRigVMLazyValue<ComputedType> GetDataLazily(int32 InSliceIndex = INDEX_NONE) const
	{
		return GetDataLazily_Internal(false, InSliceIndex);
	}
...
private:
	...
	friend struct FRigVMMemoryStorageStruct;
};

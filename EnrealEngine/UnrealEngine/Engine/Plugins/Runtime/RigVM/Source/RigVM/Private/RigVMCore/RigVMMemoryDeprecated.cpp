	...
	struct FRigVMRegisterOffsetBuilder
	{
		static bool WalkStruct(UStruct* InStruct, const FString& InPath, FRigVMRegisterOffset& Offset)
		{
			...
			else
			{
				...
				Offset.ElementSize = static_cast<uint16>(Property->GetElementSize());
				...
				if (FArrayProperty* ArrayProperty = CastField<FArrayProperty>(Property))
				{
					...
					Offset.ElementSize = static_cast<uint16>(ArrayProperty->Inner->GetElementSize());
				}
		...
		static bool WalkArray(FArrayProperty* InArrayProperty, const FString& InPath, FRigVMRegisterOffset& Offset)
		{
			...
			else
			{
				...
				Offset.ElementSize = static_cast<uint16>(InArrayProperty->Inner->GetElementSize());
...
FRigVMMemoryContainer& FRigVMMemoryContainer::operator= (const FRigVMMemoryContainer &InOther)
{
	...
	for (int32 Index = 0; Index < Registers.Num(); Index++)
	{
		...
		{
			FMemory::Memzero(&Data[static_cast<int32>(Register.GetWorkByteIndex())], sizeof(FRigVMNestedByteArray));
			*(FRigVMNestedByteArray*)&Data[static_cast<int32>(Register.GetWorkByteIndex())] = FRigVMNestedByteArray();
		}
		else if(Register.IsDynamic())
		{
			FMemory::Memzero(&Data[static_cast<int32>(Register.GetWorkByteIndex())], sizeof(FRigVMByteArray));
			*(FRigVMByteArray*)&Data[static_cast<int32>(Register.GetWorkByteIndex())] = FRigVMByteArray();
		}
...
void FRigVMMemoryContainer::Load(FArchive& Ar)
{
	...
	if (!bEncounteredErrorDuringLoad)
	{
		...
		for (int32 RegisterIndex = 0; RegisterIndex < Registers.Num(); RegisterIndex++)
		{
			...
			if (UScriptStruct* ScriptStruct = GetScriptStruct(Register))
			{
				Register.ElementSize = static_cast<uint16>(ScriptStruct->GetStructureSize());
			}
		...
		for (int32 RegisterOffsetIndex = 0; RegisterOffsetIndex < RegisterOffsets.Num(); RegisterOffsetIndex++)
		{
			...
			if (UScriptStruct* ScriptStruct = RegisterOffset.GetScriptStruct())
			{
				RegisterOffset.SetElementSize(static_cast<uint16>(ScriptStruct->GetStructureSize()));
			}
	...
	for (int32 RegisterIndex = 0; RegisterIndex < Registers.Num(); RegisterIndex++)
	{
			...
			switch (Register.Type)
			{
				case ERigVMRegisterType::Plain:
				{
					...
					if (!bEncounteredErrorDuringLoad)
					{
						...
						FMemory::Memcpy(&Data[static_cast<int32>(Register.GetWorkByteIndex())], View.GetData(), View.Num());
					}
					break;
				}
				case ERigVMRegisterType::Name:
				{
					...
					if (!bEncounteredErrorDuringLoad)
					{
						...
						RigVMCopy<FName>(&Data[static_cast<int32>(Register.GetWorkByteIndex())], View.GetData(), View.Num());
					}
					break;
				}
				case ERigVMRegisterType::String:
				{
					...
					if (!bEncounteredErrorDuringLoad)
					{
						...
						RigVMCopy<FString>(&Data[static_cast<int32>(Register.GetWorkByteIndex())], View.GetData(), View.Num());
					}
					break;
				}
				case ERigVMRegisterType::Struct:
				{
					...
					if (!bEncounteredErrorDuringLoad)
					{
						...
						uint8* DataPtr = &Data[static_cast<int32>(Register.GetWorkByteIndex())];
		...
		else
		{
			...
			if (!bEncounteredErrorDuringLoad)
			{
				NestedArrayStorage = (FRigVMNestedByteArray*)&Data[static_cast<int32>(Register.GetWorkByteIndex())];
...
int32 FRigVMMemoryContainer::Allocate(const FName& InNewName, int32 InElementSize, int32 InElementCount, int32 InSliceCount, const uint8* InDataPtr, bool bUpdateRegisters)
{
	...
	if (InDataPtr != nullptr)
	{
		...
		{
			FMemory::Memcpy(&Data[static_cast<int32>(NewRegister.GetWorkByteIndex(SliceIndex))], InDataPtr, NewRegister.GetNumBytesPerSlice());
		}
	}
...
bool FRigVMMemoryContainer::Construct(int32 InRegisterIndex, int32 InElementIndex, int32 InSliceIndex)
{
	...
	uint8* TopDataPtr = &Data[static_cast<int32>(Register.GetWorkByteIndex())];
	if (Register.IsDynamic())
	{
		if (Register.IsNestedDynamic())
		{
...
bool FRigVMMemoryContainer::Destroy(int32 InRegisterIndex, int32 InElementIndex, int32 InSliceIndex)
{
	...
	if (Register.IsDynamic())
	{
		TopDataPtr = &Data[static_cast<int32>(Register.GetWorkByteIndex())];
...
int32 FRigVMMemoryContainer::GetOrAddRegisterOffset(int32 InRegisterIndex, const FString& InSegmentPath, int32 InArrayElement)
{
	...
	UScriptStruct* ScriptStruct = GetScriptStruct(InRegisterIndex);
	int32 ElementSize = Registers[InRegisterIndex].ElementSize;
...
int32 FRigVMMemoryContainer::GetOrAddRegisterOffset(int32 InRegisterIndex, UScriptStruct* InScriptStruct, const FString& InSegmentPath, int32 InInitialOffset, int32 InElementSize)
{
	...
	if (InElementSize == 0)
	{
		...
		InElementSize = Registers[InRegisterIndex].ElementSize;
	}
	...
	FRigVMRegisterOffset Offset(InScriptStruct, InSegmentPath, InInitialOffset, static_cast<uint16>(InElementSize), BaseCPPType);
...
void FRigVMMemoryContainer::UpdateRegisters()
{
	...
	for (int32 RegisterIndex = 0; RegisterIndex < Registers.Num(); RegisterIndex++)
	{
		...
		if (Alignment != 0)
		{
			...
			if (ensure(Data.IsValidIndex(static_cast<int32>(Register.GetWorkByteIndex()))))
			{
				uint8* Pointer = (&(Data[static_cast<int32>(Register.GetWorkByteIndex())]));
			
				if (Register.AlignmentBytes > 0)
				{
					if (!IsAligned(Pointer, Alignment))
					{
						Data.RemoveAt(static_cast<int32>(Register.GetFirstAllocatedByte()), Register.AlignmentBytes);
						AlignmentShift -= Register.AlignmentBytes;
						Register.ByteIndex -= Register.AlignmentBytes;
						Register.AlignmentBytes = 0;
						Pointer = (&(Data[static_cast<int32>(Register.GetWorkByteIndex())]));
					}
				}

				while (!IsAligned(Pointer, Alignment))
				{
					Data.InsertZeroed(static_cast<int32>(Register.GetFirstAllocatedByte()), 1);

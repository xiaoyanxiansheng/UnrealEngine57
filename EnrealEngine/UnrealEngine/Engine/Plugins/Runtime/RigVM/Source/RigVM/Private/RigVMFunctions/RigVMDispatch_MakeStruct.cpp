...
const TArray<FRigVMTemplateArgumentInfo>& FRigVMDispatch_MakeStruct::GetArgumentInfos(FRigVMRegistryHandle& InRegistry) const
{
	if(CachedArgumentInfos.IsEmpty())
	{
		...
		CachedArgumentInfos.Emplace(ElementsName, ERigVMPinDirection::Input, Categories);
		CachedArgumentInfos.Emplace(StructName, ERigVMPinDirection::Output, Categories);
	}
	return CachedArgumentInfos;
}

FRigVMTemplateTypeMap FRigVMDispatch_MakeStruct::OnNewArgumentType(const FName& InArgumentName, TRigVMTypeIndex InTypeIndex, FRigVMRegistryHandle& InRegistry) const
{
	return {
...
{
	const FProperty* SourceProperty = Handles[0].GetResolvedProperty();
	const FProperty* TargetProperty = Handles[1].GetProperty();
	check(SourceProperty->SameType(TargetProperty));
	const uint8* SourceMemory = Handles[0].GetInputData();
...
const TArray<FRigVMTemplateArgumentInfo>& FRigVMDispatch_BreakStruct::GetArgumentInfos(FRigVMRegistryHandle& InRegistry) const
{
	if(CachedArgumentInfos.IsEmpty())
	{
		static const TArray<FRigVMTemplateArgument::ETypeCategory> Categories = {
		...
		CachedArgumentInfos.Emplace(StructName, ERigVMPinDirection::Input, Categories);
		CachedArgumentInfos.Emplace(ElementsName, ERigVMPinDirection::Output, Categories);
	}
	return CachedArgumentInfos;
}

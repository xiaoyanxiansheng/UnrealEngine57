...
FRigVMFunction_MathRayIntersectRay_Execute()
{
	...
	{
		Distance = static_cast<float>((ClosestA - ClosestB).Size());
	}
	RatioA = static_cast<float>(A.GetParameter(ClosestA));	
	RatioB = static_cast<float>(B.GetParameter(ClosestB));	
}
...
FRigVMFunction_MathRayIntersectPlane_Execute()
{
	...
	Ratio = static_cast<float>(FMath::RayPlaneIntersectionParam(Ray.Origin, Ray.Direction, FPlane(PlanePoint, NormalizedPlaneNormal)));
	Result = Ray.PointAt(Ratio);
	Distance = 0;
	if(!Ray.Origin.Equals(Result))
	{
		Distance = static_cast<float>((Ray.Origin - Result).Size());
	}
}

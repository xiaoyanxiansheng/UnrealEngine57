...
FRigVMFunction_TimeOffsetFloat_Execute()
{
	...
	if (ExecuteContext.GetDeltaTime() > UE_SMALL_NUMBER)
	{
		if (UpperBound == 0)
		{
			LastInsertIndex = UpperBound;
			Buffer[UpperBound] = Value;
			DeltaTimes[UpperBound++] = ExecuteContext.GetDeltaTime<float>();
		}
		else
		{
			const float SecondsPerEntry = TimeRange / static_cast<float>(MaxSize - 1);
			if (DeltaTimes[LastInsertIndex] > (SecondsPerEntry - ExecuteContext.GetDeltaTime<float>() * 0.5f))
			{
				...
				else
				{
					...
					DeltaTimes[LastInsertIndex] = ExecuteContext.GetDeltaTime<float>();
				}
			}
			else
			{

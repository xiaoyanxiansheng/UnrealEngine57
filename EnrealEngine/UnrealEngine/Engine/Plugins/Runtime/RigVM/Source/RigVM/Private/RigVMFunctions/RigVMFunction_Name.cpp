...
FRigVMFunction_IsNone_Execute()
{
	Result = Name.IsNone();
}

FRigVMFunction_IsNameValid_Execute()
{
	Result = !Value.IsNone();
}

FRigVMFunction_GetNameNumericSuffix_Execute()
{
	Suffix = INDEX_NONE;
	Success = false;
	...
	{
		Suffix = NAME_INTERNAL_TO_EXTERNAL(Name.GetNumber());
		Success = true;
		return;
	}

	FRigVMFunction_StringToInteger::StaticExecute(
		ExecuteContext, 
		Name.ToString(),
		true, // chop left
		false, // chop right

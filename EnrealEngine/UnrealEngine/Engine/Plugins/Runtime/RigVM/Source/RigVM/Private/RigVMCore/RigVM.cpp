...
void URigVM::Save(FArchive& Ar)
{
	...
	if(Ar.IsPersistent() && CVarRigVMEnableLocalizedRegistry.GetValueOnAnyThread() && LocalizedRegistry.IsValid())
	{
		...
		int64 ArchivePosAfterSerializedRegistry = 0;
		Ar << ArchivePosAfterSerializedRegistry;

		LocalizedRegistry->Serialize_NoLock(Ar);

		// store the archive positions after the registry so we
		// can skip it during load
		ArchivePosAfterSerializedRegistry = Ar.Tell();
...
void URigVM::Load(FArchive& Ar)
{
	...
	if (Ar.CustomVer(FRigVMObjectVersion::GUID) >= FRigVMObjectVersion::LocalizedRegistry)
	{
		...
		if(bStoredLocalizedRegistry)
		{
			...
			{
				// disabling warning since it is causing cooking errors.
				// UE_LOG(LogRigVM, Warning, TEXT("VM '%s' was serialized with a localized registry, but CVarRigVMEnableLocalizedRegistry is disabled."), *GetPathName());
				Ar.Seek(ArchivePosAfterSerializedRegistry);
			}
			else
			{
			...
			else
			{
				...
				if (LocalizedRegistry.IsValid())
				{
					for (int32 FactoryIndex = 0; FactoryIndex < LocalizedRegistry->GetFactories_NoLock().Num(); FactoryIndex++)
					{
						...
						UE_LOG(LogRigVM, Error, TEXT("Available factory [%03d] '%s' for VM '%s'"), FactoryIndex, *AvailableFactory->GetFactoryName().ToString(), *GetPathName());
					}
					for (const FRigVMFunction& AvailableFunction : LocalizedRegistry->GetFunctions_NoLock())
					{
						UE_LOG(LogRigVM, Error, TEXT("Available function [%03d] '%s' for VM '%s'"), AvailableFunction.Index, *AvailableFunction.Name, *GetPathName());
					}
				}
	...
	)>& InOpFunc)
{
	...
	else
	{
		...
		case ERigVMOpCode::Decrement:
		{
			...
			InOpFunc(Context, RegistryHandle, InHandleBaseIndex, {}, Op.Arg);
			break;
		}
		case ERigVMOpCode::Copy:
		{
			const FRigVMCopyOp& Op = ByteCode.GetOpAt<FRigVMCopyOp>(Instructions[InstructionIndex]);
			InOpFunc(Context, RegistryHandle, InHandleBaseIndex + 0, {}, Op.Source);
			InOpFunc(Context, RegistryHandle, InHandleBaseIndex + 1, {}, Op.Target);
			break;
		}
		case ERigVMOpCode::Equals:
		case ERigVMOpCode::NotEquals:
		{
			...
			InOpFunc(Context, RegistryHandle, InHandleBaseIndex + 0, {}, Arg);
			Arg = Op.B;
			InOpFunc(Context, RegistryHandle, InHandleBaseIndex + 1, {}, Arg);
			Arg = Op.Result;
			InOpFunc(Context, RegistryHandle, InHandleBaseIndex + 2, {}, Arg);
			break;
		}
		...
		case ERigVMOpCode::JumpBackwardIf:
		{
			...
			InOpFunc(Context, RegistryHandle, InHandleBaseIndex, {}, Arg);
			break;
		}
		...
		case ERigVMOpCode::BeginBlock:
		{
			...
			InOpFunc(Context, RegistryHandle, InHandleBaseIndex + 0, {}, Op.ArgA);
			InOpFunc(Context, RegistryHandle, InHandleBaseIndex + 1, {}, Op.ArgB);
			break;
		}
...
void URigVM::PrepareMemoryForExecution(FRigVMExtendedExecuteContext& Context)
{
	...
	for (int32 InstructionIndex = 0; InstructionIndex < Instructions.Num(); InstructionIndex++)
	{
		InstructionOpEval(Context, RegistryHandle, InstructionIndex, INDEX_NONE,
			[&HandleCount](FRigVMExtendedExecuteContext&, const FRigVMRegistryHandle&, int32, const FRigVMBranchInfoKey&, const FRigVMOperand&)
			{
				HandleCount++;
			});
...
void URigVM::CacheMemoryHandlesIfRequired(FRigVMExtendedExecuteContext& Context)
{
	...
	const int32 TaskBatchSize = 10;
	ParallelFor(TEXT("CacheMemoryHandlesIfRequired"), Instructions.Num(), TaskBatchSize, [this, &Context, &RegistryHandle](int32 InstructionIndex)
		{
			InstructionOpEval(Context, RegistryHandle, InstructionIndex, FirstHandleForInstruction[InstructionIndex],
				[&](FRigVMExtendedExecuteContext& InContext, const FRigVMRegistryHandle& InRegistry, int32 InHandleIndex, const FRigVMBranchInfoKey& InBranchInfoKey, const FRigVMOperand& InOp)
				{
					CacheSingleMemoryHandle(InContext, InRegistry, InHandleIndex, InBranchInfoKey, InOp);
				});
		}
	);
}
...
ERigVMExecuteResult URigVM::ExecuteVM(FRigVMExtendedExecuteContext& Context, const FName& InEntryName)
{
	...
	FRigVMDebugInfo PreviousDebugInfo;
	if(bIsRootEntry && RigVMDebugInfo)
	{
		PreviousDebugInfo.CopyFrom(*RigVMDebugInfo);
		
		if (!RigVMDebugInfo->IsEmpty())
		{
			// if the instruction we are about to early exit from is outside of the set of instructions
			// we are about to run - disable the debug info.
			const int32 EarlyExistInstructionIndex = RigVMDebugInfo->GetInstructionForEarlyExit();
			if (EarlyExistInstructionIndex != FMath::Clamp(EarlyExistInstructionIndex, ContextPublicData.InstructionIndex, LastInstruction))
			{
				RigVMDebugInfo->Reset();
			}
			else
			{
				LastInstruction = EarlyExistInstructionIndex;
			}
		}
		...
		if (RigVMDebugInfo)
		{
			if (!RigVMDebugInfo->IsEmpty())
			{
				if (Context.CurrentExecuteResult != ERigVMExecuteResult::Failed)
				{
					...
					RigVMDebugInfo->ExecutionHalted().Broadcast(RigVMDebugInfo->GetInstructionForEarlyExit(), nullptr, InEntryName);
				}
			}

			// restore it
			RigVMDebugInfo->CopyFrom(PreviousDebugInfo);
		}
	}
#endif
	...
	while (Instructions.IsValidIndex(ContextPublicData.InstructionIndex))
	{
		...
		switch (Instruction.OpCode)
		{
			...
			case ERigVMOpCode::Zero:
			{
				...
				{
					*((int32*)Context.CachedMemoryHandles[FirstHandleForInstruction[ContextPublicData.InstructionIndex]].GetInputData()) = 0;
				}
				else if(Handle.GetProperty()->IsA<FNameProperty>())
				{
					*((FName*)Context.CachedMemoryHandles[FirstHandleForInstruction[ContextPublicData.InstructionIndex]].GetInputData()) = NAME_None;
				}
#if WITH_EDITOR
				if(GetDebugMemory(Context) && GetDebugMemory(Context)->Num() > 0)
				{
			...
			case ERigVMOpCode::BoolFalse:
			{
				*((bool*)Context.CachedMemoryHandles[FirstHandleForInstruction[ContextPublicData.InstructionIndex]].GetInputData()) = false;
				ContextPublicData.InstructionIndex++;
				break;
			}
			case ERigVMOpCode::BoolTrue:
			{
				*((bool*)Context.CachedMemoryHandles[FirstHandleForInstruction[ContextPublicData.InstructionIndex]].GetInputData()) = true;
				ContextPublicData.InstructionIndex++;
				break;
			}
			case ERigVMOpCode::Copy:
			...
			case ERigVMOpCode::Increment:
			{
				(*((int32*)Context.CachedMemoryHandles[FirstHandleForInstruction[ContextPublicData.InstructionIndex]].GetInputData()))++;
#if WITH_EDITOR
				if(GetDebugMemory(Context) && GetDebugMemory(Context)->Num() > 0)
				{
					const FRigVMUnaryOp& Op = ByteCode.GetOpAt<FRigVMUnaryOp>(Instruction);
			...
			case ERigVMOpCode::NotEquals:
			{
				...
				const bool Result = HandleA.GetProperty()->Identical(HandleA.GetInputData(), HandleB.GetInputData());

				*((bool*)Context.CachedMemoryHandles[FirstHandleForInstruction[ContextPublicData.InstructionIndex]+2].GetOutputData()) = Result;
				ContextPublicData.InstructionIndex++;
				break;
			}
			case ERigVMOpCode::JumpAbsolute:
			...
			case ERigVMOpCode::JumpAbsoluteIf:
			{
				...
				const bool Condition = *(bool*)Context.CachedMemoryHandles[FirstHandleForInstruction[ContextPublicData.InstructionIndex]].GetInputData();
			...
			case ERigVMOpCode::BeginBlock:
			{
				const int32 Count = (*((int32*)Context.CachedMemoryHandles[FirstHandleForInstruction[ContextPublicData.InstructionIndex]].GetInputData()));
				const int32 Index = (*((int32*)Context.CachedMemoryHandles[FirstHandleForInstruction[ContextPublicData.InstructionIndex] + 1].GetInputData()));
			...
			case ERigVMOpCode::JumpToBranch:
			{
				...
				// BranchLabel = Op.Arg
				FName BranchLabel = *(FName*)Context.CachedMemoryHandles[FirstHandleForInstruction[ContextPublicData.InstructionIndex]].GetInputData();
			...
			case ERigVMOpCode::RunInstructions:
			{
				...
				FRigVMInstructionSetExecuteState& ExecutionState = *(FRigVMInstructionSetExecuteState*)Context.CachedMemoryHandles[FirstHandleForInstruction[ContextPublicData.InstructionIndex]].GetPrivateData(INDEX_NONE);
			...
			case ERigVMOpCode::SetupTraits:
			{
				...
				const TArray<int32>& TraitList = *(TArray<int32>*)Context.CachedMemoryHandles[FirstHandleForInstruction[ContextPublicData.InstructionIndex]].GetInputData();
...
bool URigVM::CreateLocalizedRegistryIfRequired(bool bForce)
{
		...
		for(const FRigVMOperand& Operand : Operands)
		{
			...
			switch(Operand.GetMemoryType())
			{
				...
				case ERigVMMemoryType::External:
				{
					...
					{
						Property = ExternalVariables[Operand.GetRegisterIndex()].Property;
					}
					break;
				}
				case ERigVMMemoryType::Debug:
			...
			// just creating the argument will register the type as needed
			FRigVMTemplateArgument::Make_NoLock(const_cast<FProperty*>(Property), TemporaryHandle);
		}
	}
			...
			if(Function && Function->Factory)
			{
				...
				for(const TRigVMTypeIndex& ArgumentTypeIndex : ArgumentTypeIndices)
				{
					if (ArgumentTypeIndex != INDEX_NONE)
					{
						...
						TemporaryRegistry->FindOrAddType_NoLock(Type);
					}
				}
			}
		}
	...
	for(const TPair<const FRigVMDispatchFactory*, TArray<const FRigVMFunction*>>& Pair : RequiredFactories)
	{
		...
		if (Factory == nullptr)
		{
			UE_LOG(LogRigVM, Error, TEXT("Found an unexpected nullptr for required dispatch factories in VM '%s'."), *GetPathName());
			return false;
		}
		...
		{
			UE_LOG(LogRigVM, Error, TEXT("Dispatch factory '%s' has an unexpected nullptr for its script struct in VM '%s'."), *Factory->GetFactoryName().ToString(), *GetPathName());
			return false;
		}
		...
		TArray<FRigVMTemplateArgumentInfo> TemporaryArgumentInfos;
		{
			...
			for(const FRigVMTemplateArgumentInfo& ArgumentInfo : FlattenedArgumentInfos)
			{
				...
				for(int32 TypeIndex = 0; TypeIndex < OriginalTypeIndices.Num(); TypeIndex++)
				{
					...
					{
						UE_LOG(LogRigVM, Error, TEXT("Cannot register dispatch dispatch '%s' for VM '%s'. Invalid types found.\nCannot localize registry."), *Factory->GetFactoryName().ToString(), *GetPathName());
						return false;
					}

					TemporaryTypeIndices.Add(TemporaryRegistry->FindOrAddType_NoLock(OriginalType));
					if (TemporaryTypeIndices.Last() == INDEX_NONE)
					{
						UE_LOG(LogRigVM, Error, TEXT("Cannot register type '%s' in dispatch '%s' for VM '%s'\nCannot localize registry."), *OriginalType.CPPType.ToString(), *Factory->GetFactoryName().ToString(), *GetPathName());
						return false;
					}
		...
		if(FRigVMTemplate* Template = const_cast<FRigVMTemplate*>(TemporaryFactory->GetTemplate_NoLock(TemporaryHandle)))
		{
			...
			for (const FRigVMFunction* OriginalFunction : OriginalFunctionsForFactory)
			{
				...
				if (TemporaryFunctionIndex == INDEX_NONE)
				{
					...
					{
						UE_LOG(LogRigVM, Error, TEXT("Original function [%03d] '%s' of template '%s' for VM '%s'."), OriginalFunctionIndex, *OriginalFunctionsForFactory[OriginalFunctionIndex]->Name, *Template->GetNotation().ToString(), *GetPathName());
					}
					for (TemporaryFunctionIndex = 0; TemporaryFunctionIndex < TemporaryFunctionsForFactory.Num(); TemporaryFunctionIndex++)
					{
						UE_LOG(LogRigVM, Error, TEXT("Temporary function [%03d] '%s' of template '%s' for VM '%s'."), TemporaryFunctionIndex, *TemporaryFunctionsForFactory[TemporaryFunctionIndex]->Name, *Template->GetNotation().ToString(), *GetPathName());
					}
					UE_LOG(LogRigVM, Error, TEXT("Missing expected permutation '%s' of template '%s' for VM '%s'.\nCannot localize registry."), *OriginalFunction->Name, *Template->GetNotation().ToString(), *GetPathName());
					return false;
				}
			}
		}
	...
	for(const FRigVMFunction* Function : FunctionsStorage)
	{
		...
		if(Function->Factory)
		{
			...
			if (!TemporaryRegistry->FindFunction_NoLock(*Function->Name))
			{
				...
				UE_LOG(LogRigVM, Error, TEXT("Cannot find expected permutation '%s' for VM '%s'\nCannot localize registry."), *Function->Name, *GetPathName());
				return false;
			}
		}
		else
	...
	{
		UE_LOG(LogRigVM, Error, TEXT("Cannot localize registry due to missing function."));
...
void URigVM::CacheSingleMemoryHandle(FRigVMExtendedExecuteContext& Context, const FRigVMRegistryHandle& InRegistry, int32 InHandleIndex, const FRigVMBranchInfoKey& InBranchInfoKey, const FRigVMOperand& InArg, bool bForExecute)
{
	...
	if (InArg.GetMemoryType() == ERigVMMemoryType::External)
	{
		...
		check(Context.CachedMemoryHandles[InHandleIndex].GetOutputData() != nullptr);
#endif
		return;
	}
	...
	if(InArg.GetRegisterOffset() != INDEX_NONE)
	{
		...
		{
			UE_LOG(LogRigVM, Error, TEXT("VM '%s' uses an invalid property path."), *GetPathName());
...
#if UE_BUILD_DEBUG
	// make sure the handle points to valid memory
	check(Handle.GetOutputData() != nullptr);
	if(PropertyPath)
	{
		uint8* MemoryPtr = Handle.GetOutputData();

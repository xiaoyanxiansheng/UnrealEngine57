...
FRigVMStructUpgradeInfo FRigVMStructUpgradeInfo::MakeFromStructToFactory(UScriptStruct* InRigVMStruct, UScriptStruct* InFactoryStruct, const TMap<FName,FName>& InArgumentNameMap)
{
	check(InRigVMStruct);
	check(InFactoryStruct);

	FRigVMRegistry& Registry = FRigVMRegistry::Get();
	const FRigVMDispatchFactory* Factory = Registry.FindOrAddDispatchFactory(InFactoryStruct);
	check(Factory);
	FRigVMTemplateTypeMap Types = GetTypeMapFromStruct(InRigVMStruct);
	{
		TArray<FRigVMTemplateArgumentInfo> Infos;
		{
			FRigVMRegistryReadLock ReadLock(Registry);
			Infos = Factory->GetArgumentInfos(ReadLock);
		}

		// if we have only one argument - it's quite possible we'll have to remap the argument name.
		if(Types.Num() == 1 && Infos.Num() == 1)
		{
			FRigVMTemplateTypeMap NewTypes;
			NewTypes.Add(Infos[0].Name, Types.FindArbitraryElement()->Value);
			Swap(NewTypes, Types);
		}

		// if we have more arguments or a mismatch - we need to rely on the name map
		else
		{
			FRigVMTemplateTypeMap NewTypes;
			for(const FRigVMTemplateArgumentInfo& Info : Infos)
			{
				FName ArgumentName = Info.Name;
				if(const FName* MappedArgumentName = InArgumentNameMap.FindKey(ArgumentName))
				{
					ArgumentName = *MappedArgumentName;
				}
				
				if(const TRigVMTypeIndex* TypeIndex = Types.Find(ArgumentName))
				{
					NewTypes.Add(Info.Name, *TypeIndex);
				}
				else
				{
					UE_LOG(
						LogRigVM,
						Error, 
						TEXT("Cannot upgrade node using '%s' to dispatch '%s' - type for argument '%s' cannot be determined.\nYou may need to provide the InArgumentNameMap."),
						*InRigVMStruct->GetName(),
						*InFactoryStruct->GetName(),
						*Info.Name.ToString()
					);
	...
	for(const TPair<FName, FName>& Pair : InArgumentNameMap)
	{
		Info.AddRemappedPin(Pair.Key.ToString(), Pair.Value.ToString());
	}

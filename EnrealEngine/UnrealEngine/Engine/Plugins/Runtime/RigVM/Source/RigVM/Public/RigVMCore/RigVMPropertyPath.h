...
public:
	 ...
	 */
	UE_API FRigVMPropertyPath(const FProperty* InProperty, const FString& InSegmentPath);
	...
	UE_API const FProperty* GetTailProperty() const;

	// Static empty property path (used for comparisons)
	static UE_API FRigVMPropertyPath Empty;

private:

	// Internal traversal method
	UE_API uint8* GetData_Internal(uint8* InPtr, const FProperty* InProperty) const;

...
public:
	...
	UE_API const FRigVMExprAST* GetParent() const;
	...
	UE_API virtual const FRigVMExprAST* GetFirstParentOfType(EType InExprType) const;
	...
	UE_API bool IsParentedTo(const FRigVMExprAST* InParentExpr) const;
	...
	UE_API FRigVMBlockArray GetBlocks(bool bSortByDepth) const;
	...
	UE_API TOptional<uint32> GetBlockCombinationHash() const;
	...
	UE_API const FString& GetBlockCombinationName() const;
	...
	UE_API int32 GetMinChildIndexWithinParent(const FRigVMExprAST* InParentExpr) const;
	...
	UE_API virtual bool IsConstant() const;
	...
	UE_API int32 GetMaximumDepth() const;
	...
	/// @param InExprType The type of expression to look for in the parent tree
	/// @return the first parent in the tree of a given type
	UE_API virtual const FRigVMExprAST* GetFirstChildOfType(EType InExprType) const;
...
protected:
	...
	UE_API FRigVMExprAST(EType InType = EType::Invalid, const FRigVMASTProxy& InProxy = FRigVMASTProxy());
	...
	UE_API void AddParent(FRigVMExprAST* InParent);
	...
	UE_API void RemoveParent(FRigVMExprAST* InParent);
	...
	UE_API void RemoveChild(FRigVMExprAST* InChild);
	...
	// @param OutParentIndices The list of parent indices affected
	UE_API void ReplaceParent(FRigVMExprAST* InCurrentParent, FRigVMExprAST* InNewParent, TArray<int32>* OutParentIndices = nullptr);
	...
	// @param OutChildIndices The list of child indices affected
	UE_API void ReplaceChild(FRigVMExprAST* InCurrentChild, FRigVMExprAST* InNewChild, TArray<int32>* OutChildIndices = nullptr);

	// replaces this expression with another one
	// @param InReplacement the expression to replace this one
	UE_API void ReplaceBy(FRigVMExprAST* InReplacement);

	// computes all of the block this expression is in
	UE_API void GetBlocksImpl(FRigVMBlockArray& InOutBlocks) const;
	...
	UE_API virtual FString DumpText(const FString& InPrefix = FString()) const;

	// Resets the internal caches of the expression
	UE_API void InvalidateCaches();
	UE_API void InvalidateCachesImpl(TArray<bool>& OutProcessed);
...
public:
	...
	UE_API bool Contains(const FRigVMExprAST* InExpression, TMap<const FRigVMExprAST*, bool>* ContainedExpressionsCache = nullptr) const;
...
public:
	...
	UE_API virtual bool IsConstant() const override;
	UE_API const FRigVMExprAST* FindExprWithPinName(const FName& InPinName) const;
	UE_API const FRigVMVarExprAST* FindVarWithPinName(const FName& InPinName) const;
	UE_API const FName& GetPinNameForChildIndex(int32 InChildIndex) const;

	void SetSegmentPathForChild(int32 InChildIndex, const FString& InSegmentPath);
	const FString& GetSegmentPathForChild(int32 InChildIndex) const;

protected:

	// default constructor (protected so that only parser can access it)
	UE_API FRigVMNodeExprAST(EType InType, const FRigVMASTProxy& InNodeProxy);
...
private:
	...
	TMap<int32,FString> SegmentPathForChild;
...
public:
	...
	UE_API FName GetEventName() const;
...
public:
	...
	UE_API ERigVMPinDirection GetPinDirection() const;
...
struct FRigVMParserASTSettings
{
	...
	// fold root pin assignments / copies
	UPROPERTY(EditAnywhere, Category = "AST")
	bool bFoldAssignments = false;
	
   // fold sub pin assignments / copies
   UPROPERTY(EditAnywhere, Category = "AST")
   bool bFoldSubPinCopies = false;
	...
	static FRigVMParserASTSettings Fast(UScriptStruct* InExecuteContextStruct = nullptr)
	{
		...
		Settings.bFoldSubPinCopies = false;
	...
	UE_API void Report(EMessageSeverity::Type InSeverity, UObject* InSubject, const FString& InMessage) const;
...
public:
	...
	UE_API FRigVMParserAST(TArray<URigVMGraph*> InGraphs, URigVMController* InController = nullptr, const FRigVMParserASTSettings& InSettings = FRigVMParserASTSettings::Fast(), const TArray<FRigVMExternalVariable>& InExternalVariables = TArray<FRigVMExternalVariable>());

	// default destructor
	UE_API ~FRigVMParserAST();
	...
	UE_API const FRigVMExprAST* GetExprForSubject(const FRigVMASTProxy& InProxy) const;

	// returns all expressions for a given subject. subjects include nodes and pins.
	// @param InSubject the subject to retrieve the expression for (node or pin)
	// @return all expressions for the given subject
	UE_API TArray<const FRigVMExprAST*> GetExpressionsForSubject(UObject* InSubject) const;
	...
	UE_API void PrepareCycleChecking(URigVMPin* InPin);
	...
	UE_API bool CanLink(URigVMPin* InSourcePin, URigVMPin* InTargetPin, FString* OutFailureReason);

	// returns a string containing an indented tree structure
	// for debugging purposes.
	// @return the simple text representing this abstract syntax tree
	UE_API FString DumpText() const;

	// returns a string containing a dot file notation
	// for debugging purposes.
	// @return the dot file text representing this abstract syntax tree
	UE_API FString DumpDot() const;

	// returns an obsolete block for unmanaged expression
	UE_API FRigVMBlockExprAST* GetObsoleteBlock(bool bCreateIfMissing = true);
	UE_API const FRigVMBlockExprAST* GetObsoleteBlock(bool bCreateIfMissing = true) const;
...
private:
	...
	UE_API void RemoveExpressions(TArray<FRigVMExprAST*> InExprs);
	...
	static UE_API void TraverseParents(const FRigVMExprAST* InExpr, TFunctionRef<bool(const FRigVMExprAST*)> InContinuePredicate);
	...
	// @param InExpr the expression to start the traverse at
	// @param InContinuePredicate the predicate to use to break out of the traverse (return false)
	static UE_API void TraverseChildren(const FRigVMExprAST* InExpr, TFunctionRef<bool(const FRigVMExprAST*)> InContinuePredicate);

	// helper function to fold all entries with the same event name into one block
	UE_API void FoldEntries();

	// helper function to inject an exit expression at the end of every entry expressoin
	UE_API void InjectExitsToEntries();

	// helper function to refresh the expression indices (used after deleting an expression)
	UE_API void RefreshExprIndices();

	// helper function to fold / remove the no op expressions (parameters mostly)
	UE_API void FoldNoOps();

	// helper function to fold / merge redundant literals with the same value
	UE_API void FoldLiterals();

	// helper function to fold / remove obsolete assignments and reduce assignment chains
	UE_API void FoldAssignments();

	// helper function to fold / remove obsolete copies from a sub pin to a root pin
   	UE_API void FoldSubPinCopies();

	// helper function to inline all contributing nodes of the graph
	UE_API void Inline(const TArray<URigVMGraph*>& InGraphs);
	UE_API void Inline(const TArray<URigVMGraph*>& InGraphs, const TArray<FRigVMASTProxy>& InNodeProxies);

	// helper functions to retrieve links for a given pin
	UE_API TArray<int32> GetSourceLinkIndices(const FRigVMASTProxy& InPinProxy, bool bRecursive = false) const;
	UE_API TArray<int32> GetTargetLinkIndices(const FRigVMASTProxy& InPinProxy, bool bRecursive = false) const;
	UE_API TArray<int32> GetLinkIndices(const FRigVMASTProxy& InPinProxy, bool bGetSource, bool bRecursive) const;

	// function to retrieve a link given its index
	UE_API const FRigVMASTLinkDescription& GetLink(int32 InLinkIndex) const;

	// traverse a single mutable node (constructs entry, call extern and other expressions)
	UE_API FRigVMExprAST* TraverseMutableNode(const FRigVMASTProxy& InNodeProxy, FRigVMExprAST* InParentExpr);

	// traverse a single pure node (constructs call extern expressions)
	UE_API FRigVMExprAST* TraverseNode(const FRigVMASTProxy& InNodeProxy, FRigVMExprAST* InParentExpr);

	// returns the expression for a given node
	UE_API FRigVMExprAST* CreateExpressionForNode(const FRigVMASTProxy& InNodeProxy, FRigVMExprAST* InParentExpr);

	// traverse an array of pins for a given node
	UE_API TArray<FRigVMExprAST*> TraversePins(const FRigVMASTProxy& InNodeProxy, FRigVMExprAST* InParentExpr);

	// traverse a single pin (constructs var + literal expressions)
	UE_API FRigVMExprAST* TraversePin(const FRigVMASTProxy& InPinProxy, FRigVMExprAST* InParentExpr);

	// traverse a single link (constructs assign + copy expressions)
	UE_API FRigVMExprAST* TraverseLink(int32 InLinkIndex, FRigVMExprAST* InParentExpr);

	UE_API bool ShouldLinkBeSkipped(const FRigVMASTLinkDescription& InLink) const;

	static UE_API FString GetLinkAsString(const FRigVMASTLinkDescription& InLink);

	UE_API void IncrementCacheVersion() const;

	static bool IsConstantDispatchNode(const URigVMNode* InNode);
	static bool IsConstantDispatchValuePin(const URigVMPin* InPin);
 	static bool IsMakeStructDispatchNode(const URigVMNode* InNode);
	static bool IsMakeStructDispatchElementsPin(const URigVMPin* InPin);
	static bool IsMakeStructDispatchStructPin(const URigVMPin* InPin);
	static bool IsBreakStructDispatchNode(const URigVMNode* InNode);
	static bool IsBreakStructDispatchStructPin(const URigVMPin* InPin);
	static bool IsBreakStructDispatchElementsPin(const URigVMPin* InPin);

	static bool IsFloatOrFloatArrayPinOnVariable(const URigVMPin* InPin);
	...
	static UE_API const TArray<FRigVMASTProxy> EmptyProxyArray;

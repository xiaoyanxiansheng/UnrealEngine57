...
public:
	...
	// Returns the path
	UE_API FString GetCallPath(bool bIncludeLast = true) const;

	// Returns the number of entries on this callstack
	UE_API int32 Num() const;

	// Returns the last object in the callstack
	UE_API const UObject* Last() const;

	// Returns the element at a given index
	UE_API const UObject* operator[](int32 InIndex) const;

	// Returns true if the stack contains a given entry
	UE_API bool Contains(const UObject* InEntry) const;
	...
	UE_API FRigVMCallstack GetCallStackUpTo(int32 InIndex) const;
...
public:
	...
	static UE_API FRigVMASTProxy MakeFromUObject(UObject* InSubject);
	static UE_API FRigVMASTProxy MakeFromCallPath(const FString& InCallPath, UObject* InRootObject);
	static UE_API FRigVMASTProxy MakeFromCallstack(const FRigVMCallstack& InCallstack);
	static UE_API FRigVMASTProxy MakeFromCallstack(const TArray<TWeakObjectPtr<UObject>>* InCallstack);

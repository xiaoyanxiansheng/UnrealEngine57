...
bool URigVMCompiler::TraverseCallExtern(const FRigVMCallExternExprAST* InExpr, FRigVMCompilerWorkData& WorkData)
{
	...
	if (WorkData.bSetupMemory)
	{
		...
		SetupPropertyPathsForFoldedCopies(InExpr, WorkData);
...
bool URigVMCompiler::TraverseInlineFunction(const FRigVMInlineFunctionExprAST* InExpr, FRigVMCompilerWorkData& WorkData)
{
	...
	else
	{
		...
		for (int32 i=InstructionIndexStart; i<=InstructionIndexEnd; ++i)
		{
			...
			const int32 FunctionInstructionIndex = i - InstructionIndexStart;
			...
			const FRigVMOperandArray FunctionOperandArray = FunctionByteCode.GetOperandsForOp(FunctionInstructions[FunctionInstructionIndex]);
			check(FunctionOperandArray.Num() == OperandArray.Num());
			
			int32 OperandsIndex = ByteCode.GetFirstOperandByteIndex(Instruction);
			for (int32 j=0; j<OperandArray.Num(); ++j)
			{
				FRigVMOperand* Operand = const_cast<FRigVMOperand*>(&OperandArray[j]);

				const FRigVMOperand OriginalOperand = *Operand;
				check(OriginalOperand == FunctionOperandArray[j]);
				...
				else if (const int32 FunctionInterfaceParameterIndex = GetOperandFunctionInterfaceParameterIndex(OperandsPinNames, FunctionCompilationData, *Operand); FunctionInterfaceParameterIndex != INDEX_NONE)
				{
					...
					Operand->MemoryType = InterfaceOperand.MemoryType;
					Operand->RegisterIndex = InterfaceOperand.RegisterIndex;
				}
				else
				{
					// Operand is internal
				...
				if (Operand->GetRegisterOffset() != INDEX_NONE || InterfaceOperand.GetRegisterOffset() != INDEX_NONE)
				{
					if (InterfaceOperand.GetRegisterOffset() != INDEX_NONE && Operand->GetRegisterOffset() == INDEX_NONE)
					{
						Operand->RegisterOffset = IntCastChecked<uint16>(InterfaceOperand.GetRegisterOffset());
					}
					else
					{
						FRigVMFunctionCompilationPropertyPath DescriptionFromFunction;
						switch (OriginalOperand.MemoryType)
						{
							case ERigVMMemoryType::Work:
							{
								DescriptionFromFunction = FunctionCompilationData->WorkPropertyPathDescriptions[Operand->GetRegisterOffset()];
								break;
							}
							case ERigVMMemoryType::Literal:
							{
								DescriptionFromFunction = FunctionCompilationData->LiteralPropertyPathDescriptions[Operand->GetRegisterOffset()];
								break;
							}
							case ERigVMMemoryType::External:
							{
								DescriptionFromFunction = FunctionCompilationData->ExternalPropertyPathDescriptions[Operand->GetRegisterOffset()];
								break;
							}
							case ERigVMMemoryType::Debug:
							{
								DescriptionFromFunction = FunctionCompilationData->DebugPropertyPathDescriptions[Operand->GetRegisterOffset()];
								break;
							}
							default:
							{
						...
						{
							Operand->RegisterOffset = IntCastChecked<uint16>(WorkData.FindOrAddPropertyPath(*Operand, DescriptionFromFunction.HeadCPPType, DescriptionFromFunction.SegmentPath));
						}
						else
						{
							...
							Operand->RegisterOffset = IntCastChecked<uint16>(WorkData.FindOrAddPropertyPath(*Operand, Description.HeadCPPType, CombinedSegmentPath));
						}
					}
				}

				// Store correspondence between Function operand and New operands
				check(!FunctionOperandToNewOperand.Contains(OriginalOperand) || FunctionOperandToNewOperand.FindChecked(OriginalOperand) == *Operand);
				FunctionOperandToNewOperand.Add(OriginalOperand, *Operand);

				// Also find the function debug operands and store correspondence to new debug operands
				if (const TArray<FRigVMOperand>* FunctionDebugRegisters = FunctionCompilationData->OperandToDebugRegisters.Find(OriginalOperand))
				{
			...
			if (WorkData.Settings.SetupNodeInstructionIndex)
			{
				if (const TArray<TWeakObjectPtr<UObject>>* Callstack = FunctionByteCode.GetCallstackForInstruction(FunctionInstructionIndex))
				{
					if (Callstack->Num() > 1)
					{
						FRigVMCallstack InstructionCallstack = FuncRefCallstack;
						InstructionCallstack.Stack.Append(&(*Callstack)[1], Callstack->Num()-1);
						ByteCode.SetSubject(i, InstructionCallstack.GetCallPath(), InstructionCallstack.GetStack());
					}
				}

				// also store the instruction for the subject so that profiling can determine the cost of functions.
				ByteCode.AddInstructionForSubject(FunctionReferenceNode, i);
			}

			// remap the input and output operands per instruction
			TArray<FRigVMOperand> InputOperands, OutputOperands;
			if (FunctionByteCode.InputOperandsPerInstruction.IsValidIndex(FunctionInstructionIndex))
			{
				for (const FRigVMOperand& Operand : FunctionByteCode.InputOperandsPerInstruction[FunctionInstructionIndex])
				{
					InputOperands.Add(FunctionOperandToNewOperand.FindChecked(Operand));
				}
			}
			if (FunctionByteCode.OutputOperandsPerInstruction.IsValidIndex(FunctionInstructionIndex))
			{
				for (const FRigVMOperand& Operand : FunctionByteCode.OutputOperandsPerInstruction[FunctionInstructionIndex])
				{
					OutputOperands.Add(FunctionOperandToNewOperand.FindChecked(Operand));
				}
			}
			ByteCode.SetOperandsForInstruction(Instruction, InputOperands, OutputOperands);
		}
...
void URigVMCompiler::SetupPropertyPathsForFoldedCopies(const FRigVMNodeExprAST* InExpr, FRigVMCompilerWorkData& WorkData)
{
	...
	for(const TPair<int32, FString>& SegmentPathPair : InExpr->SegmentPathForChild)
	{
		...
		const FRigVMVarExprAST* SourceVarExpr = GetSourceVarExpr(ChildExpr);
		const FRigVMOperand RootOperand = WorkData.ExprToOperand.FindChecked(SourceVarExpr);
		const FString HeadCPPType = SourceVarExpr->GetPin()->GetRootPin()->GetCPPType();

		WorkData.FindOrAddPropertyPath(RootOperand, HeadCPPType, SegmentPathPair.Value);
	}
}

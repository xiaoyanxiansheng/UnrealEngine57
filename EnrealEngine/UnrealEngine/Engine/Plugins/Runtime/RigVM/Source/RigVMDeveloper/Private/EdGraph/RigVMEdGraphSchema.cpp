...
FRigVMLocalVariableNameValidator::FRigVMLocalVariableNameValidator(const FRigVMAssetInterfacePtr InAsset, const URigVMGraph* Graph, FName InExistingName)
	: FStringSetNameValidator(InExistingName.ToString())
{
	if (const UBlueprint* Blueprint = Cast<UBlueprint>(InAsset.GetObject()))
	{
...
FRigVMNameValidator::FRigVMNameValidator(const FRigVMAssetInterfacePtr Blueprint, const UStruct* ValidationScope, FName InExistingName)
: FStringSetNameValidator(InExistingName.ToString())
{
	if (Blueprint)
	{
		TSet<FName> NamesTemp;
		TArray<FRigVMGraphVariableDescription> Variables = Blueprint->GetAssetVariables();
		...
		{
			NamesTemp.Add(Variable.Name);
		}
		for (const URigVMGraph* Graph : Graphs)
		{
			if (URigVMCollapseNode* Node = Cast<URigVMCollapseNode>(Graph->GetOuter()))
			{
				NamesTemp.Add(Node->GetFName());
			}
			else
			{
				NamesTemp.Add(Graph->GetFName());
			}
		}
...
bool FRigVMEdGraphSchemaAction_LocalVar::IsValidName(const FName& NewName, FText& OutErrorMessage) const
{
	...
	{
		FRigVMLocalVariableNameValidator NameValidator(EdGraphGraph->GetAsset(), EdGraphGraph->GetModel(), GetVariableName());
	...
	const FVector2f& Location, bool bSelectNewNode)
{
	...
	FRigVMAssetInterfacePtr Blueprint = RigGraph->GetAsset();
...
FReply FRigVMFunctionDragDropAction::DroppedOnPanel(const TSharedRef< class SWidget >& Panel, const FVector2f& ScreenPosition, const FVector2f& GraphPosition, UEdGraph& Graph)
{
	...
	{
		if (FRigVMAssetInterfacePtr TargetRigBlueprint = FRigVMBlueprintUtils::FindAssetForGraph(TargetRigGraph))
		{
			if (URigVMGraph* FunctionDefinitionGraph = SourceRigBlueprint->GetModel(SourceRigGraph))
			{
				if (URigVMLibraryNode* FunctionDefinitionNode = Cast<URigVMLibraryNode>(FunctionDefinitionGraph->GetOuter()))
				{
					if(URigVMController* TargetController = TargetRigBlueprint->GetController(TargetRigGraph))
					{
						...
						{
							if(FRigVMAssetInterfacePtr FunctionRigBlueprint = IRigVMAssetInterface::GetInterfaceOuter(FunctionLibrary))
							{
#if WITH_EDITOR
								if(FunctionRigBlueprint != TargetRigBlueprint)
								{
									if(!FunctionRigBlueprint->GetRigVMClientHost()->IsFunctionPublic(FunctionDefinitionNode->GetFName()))
									{
...
TSharedRef<FRigVMFunctionDragDropAction> FRigVMFunctionDragDropAction::New(TSharedPtr<FEdGraphSchemaAction> InAction, FRigVMAssetInterfacePtr InRigBlueprint, URigVMEdGraph* InRigGraph)
{
...
TSharedPtr<INameValidatorInterface> URigVMEdGraphSchema::GetNameValidator(const FRigVMAssetInterfacePtr Asset, const FName& OriginalName, const UStruct* ValidationScope) const
{
	return MakeShareable(new FRigVMNameValidator(Asset, ValidationScope, OriginalName));
}
...
bool URigVMEdGraphSchema::SupportsPreviewFromHereOnNode(const UEdGraphNode* InNode) const
{
	return false;
}
...
FReply URigVMEdGraphSchema::BeginGraphDragAction(TSharedPtr<FEdGraphSchemaAction> InAction, const FPointerEvent& MouseEvent) const
{
	...
	if (InAction->GetTypeId() == FEdGraphSchemaAction_K2Graph::StaticGetTypeId())
	{
		...
		{
			if (FRigVMAssetInterfacePtr RigBlueprint = FRigVMBlueprintUtils::FindAssetForGraph(RigGraph))
			{
...
{
#if WITH_EDITOR
	if (const FRigVMAssetInterfacePtr Blueprint = FRigVMBlueprintUtils::FindAssetForGraph(InGraphObj))
	{
...
void URigVMEdGraphSchema::SetNodePosition(UEdGraphNode* Node, const FVector2D& Position, bool bSetupUndo) const
{
	...
	if (const URigVMEdGraphNode* RigNode = Cast<URigVMEdGraphNode>(Node))
	{
		if (URigVMController* Controller = RigNode->GetController())
		{
			...
			{
				Controller->SetNodePosition(RigNode->GetModelNode(), Position, bSetupUndo, false, false);
			}
		}
	}
	...
	if (const UEdGraphNode_Comment* CommentNode = Cast<UEdGraphNode_Comment>(Node))
	{
		if(const URigVMEdGraph* Graph = CommentNode->GetTypedOuter<URigVMEdGraph>())
		{
			if (URigVMController* Controller = Graph->GetController())
			{
				if  (!URigVMHost::IsGarbageOrDestroyed(Controller))
				{
					Controller->SetNodePositionByName(CommentNode->GetFName(), Position, bSetupUndo, false, false);
				}
			}
		}
	}
...
EGraphType URigVMEdGraphSchema::GetGraphType(const UEdGraph* TestEdGraph) const
{
	...
	{
		if (FRigVMBlueprintUtils::FindAssetForGraph(RigGraph))
		{
			if (const URigVMGraph* Model = RigGraph->GetModel())
			{
				if(Model->IsRootGraph())
...
FReply URigVMEdGraphSchema::TrySetGraphCategory(const UEdGraph* InGraph, const FText& InCategory) const
{
	if (const URigVMEdGraph* RigGraph = Cast<URigVMEdGraph>((UEdGraph*)InGraph))
	{
		if (const FRigVMAssetInterfacePtr RigBlueprint = FRigVMBlueprintUtils::FindAssetForGraph(RigGraph))
		{
			if (const URigVMGraph* Model = RigGraph->GetModel())
			{
						...
						if (URigVMFunctionLibrary* Library = Cast<URigVMFunctionLibrary>(LibraryNode->GetGraph()))
						{
							...
							if (RigBlueprint->IsFunctionPublic(FunctionName))
							{
								for (auto Reference : Library->GetReferencesForFunction(FunctionName))
								{
									...
									{
										const FRigVMAssetInterfacePtr OtherBlueprint = IRigVMAssetInterface::GetInterfaceOuter(Reference.Get());
...
{
	if (FRigVMAssetInterfacePtr RigBlueprint = FRigVMBlueprintUtils::FindAssetForNode(InTargetNode))
	{
...
bool URigVMEdGraphSchema::RequestFunctionDropOnPanel(UEdGraph* InGraph, const FRigVMGraphFunctionIdentifier& InFunction, const FVector2D& InDropPosition, const FVector2D& InScreenPosition) const
{
	if (FRigVMAssetInterfacePtr TargetRigBlueprint = FRigVMBlueprintUtils::FindAssetForGraph(InGraph))
	{
		if(URigVMController* TargetController = TargetRigBlueprint->GetController(InGraph))
		{
			...
			{
				if (TScriptInterface<IRigVMGraphFunctionHost> SourceFunctionHost = SourceClientHost->GetRigVMGraphFunctionHost())
				{
					if (FRigVMGraphFunctionStore* SourceStore = SourceFunctionHost->GetRigVMGraphFunctionStore())
					{
#if WITH_EDITOR
						if(InFunction.HostObject != TargetRigBlueprint->GetRigVMClientHost()->GetRigVMGraphFunctionHost().GetObject())
						{
...
bool URigVMEdGraphSchema::RequestVariableDropOnPanel(UEdGraph* InGraph, FProperty* InVariableToDrop, const FVector2f& InDropPosition, const FVector2f& InScreenPosition) const
{
	...
	{
		FRigVMAssetInterfacePtr RigBlueprint = FRigVMBlueprintUtils::FindAssetForGraph(InGraph);
...
void URigVMEdGraphSchema::EndGraphNodeInteraction(UEdGraphNode* InNode) const
{
	...
	FGuardSkipDirtyBlueprintStatus GuardDirtyBlueprintStatus(Graph->GetAsset(), true);
...
{
	if (FRigVMAssetInterfacePtr RigBlueprint = FRigVMBlueprintUtils::FindAssetForGraph(InGraph))
	{
		return RigBlueprint->GetRigVMClientHost()->GetRigVMEdGraphNodeClass();
	}
	return nullptr;
}

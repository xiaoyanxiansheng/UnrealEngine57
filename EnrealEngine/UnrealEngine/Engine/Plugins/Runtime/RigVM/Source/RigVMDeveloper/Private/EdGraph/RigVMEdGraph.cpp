...
{
	return Cast<URigVMBlueprint>(GetAssetDefaultObject().GetObject());
}

const FRigVMAssetInterfacePtr URigVMEdGraph::GetAssetDefaultObject() const
{
	if(RigVMBlueprintClass)
	{
		UObject* Obj = RigVMBlueprintClass->GetDefaultObject(false);
...
void URigVMEdGraph::SetBlueprintClass(const UClass* InClass)
{
	...
	if(const FRigVMAssetInterfacePtr Blueprint = GetAssetDefaultObject())
	{
		Schema = Blueprint->GetRigVMClientHost()->GetRigVMEdGraphSchemaClass();
	}
}

void URigVMEdGraph::InitializeFromAsset(FRigVMAssetInterfacePtr InBlueprint)
{
	DECLARE_SCOPE_HIERARCHICAL_COUNTER_FUNC()

	SetBlueprintClass(InBlueprint->GetObject()->GetClass());
...
bool URigVMEdGraph::HandleModifiedEvent_Internal(ERigVMGraphNotifType InNotifType, URigVMGraph* InGraph, UObject* InSubject)
{
	...
	switch (InNotifType)
	{
		...
		case ERigVMGraphNotifType::LinkRemoved:
		{
			...
			if (URigVMLink* Link = Cast<URigVMLink>(InSubject))
			{
				...
				if (SourcePin && TargetPin && SourcePin != TargetPin)
				{
					...
					if (const URigVMNode* SourceNode = SourcePin->GetNode())
					{
						if (SourceNode->IsA<URigVMVariableNode>() && TargetRigNode)
						{
							TargetRigNode->ModelPinsChanged(true);
						}
					}
				}
			}
		...
		case ERigVMGraphNotifType::NodeEarlyExitChanged:
		{
			if (URigVMNode* ModelNode = Cast<URigVMNode>(InSubject))
			{
				if (URigVMEdGraphNode* RigNode = Cast<URigVMEdGraphNode>(FindNodeForModelNodeName(ModelNode->GetFName())))
...
int32 URigVMEdGraph::GetInstructionIndex(const URigVMEdGraphNode* InNode, bool bAsInput)
{
	...
	struct Local
	{
		static int32 GetInstructionIndex(URigVMNode* InModelNode, const FRigVMByteCode* InByteCode, TMap<URigVMNode*, TPair<int32, int32>>& Indices, bool bAsInput)
		{
			...
			if(bAsInput)
			{
				Indices.FindOrAdd(InModelNode).Key = INDEX_NONE;
			}
			else
			{
				Indices.FindOrAdd(InModelNode).Value = INDEX_NONE;
			}
	...
	if (const URigVMController* Controller = GetController())
	{
...
void URigVMEdGraph::CacheEntryNameList()
{
	...
	if(const TScriptInterface<IRigVMClientHost> Blueprint = GetAsset()->GetRigVMClientHost())
	{
...
{
	return Cast<URigVMBlueprint>(GetAsset().GetObject());
}
...
FRigVMAssetInterfacePtr URigVMEdGraph::GetAsset() const
{
	if (URigVMEdGraph* OuterGraph = Cast<URigVMEdGraph>(GetOuter()))
	{
		return OuterGraph->GetAsset();
	}
	return GetOuter();
}
...
void URigVMEdGraph::AddNode(UEdGraphNode* NodeToAdd, bool bUserAction, bool bSelectNewNode)
{
	...
	// If a comment node was added the graph needs to be refreshed to re-sort its nodes
	// Otherwise when selecting nodes they won't be depth sorted correctly and the selection can be wrong
	if (bCommentNodeAdded)
	{
		NotifyGraphChanged();
	}
}
...
void URigVMEdGraph::RemoveNode(UEdGraphNode* InNode)
{
	...
	FString DeletedName;
	{
		...
		do
		{
			...
			ExistingObject = StaticFindObject(/*Class=*/ nullptr, this, *DeletedName, EFindObjectFlags::ExactClass);
		}

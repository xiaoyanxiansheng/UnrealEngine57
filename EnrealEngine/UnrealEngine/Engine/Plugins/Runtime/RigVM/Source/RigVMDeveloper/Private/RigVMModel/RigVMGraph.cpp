...
bool URigVMGraph::IsNodeHighlighted(const FName& InNodeName) const
{
	for (const FRigVMGraphSection& MatchingSection : SectionsMatchingTheSelection)
	{
		if (MatchingSection.ContainsNode(InNodeName))
		{
			return true;
		}
	}
...
TArray<FRigVMGraphSection> URigVMGraph::GetMatchingSections(const FRigVMGraphSection& InSection) const
{
	TArray<FRigVMGraphSection> Matches;
	for (const FRigVMGraphSection& Match : SectionsMatchingTheSelection)
	{
		if (Match.Hash == InSection.Hash)
		{
			Matches.Add(Match);
		}
	}
...
void URigVMGraph::UpdateSections(bool bForce)
{
	SelectedSections.Reset();
	SectionsMatchingTheSelection.Reset();
	...
	if (!AreSectionsEnabled() && !bForce)
	{
		return;
	}

	SelectedSections = FRigVMGraphSection::GetSectionsFromSelection(this);
	
	TArray<URigVMNode*> AvailableNodes = Nodes;
	AvailableNodes.RemoveAll([this]( const URigVMNode* Node) -> bool
	{
		return SelectedNodes.Contains(Node->GetFName()) || !FRigVMGraphSection::IsValidNode(Node);
	});

	for (const FRigVMGraphSection& SelectedSection : SelectedSections)
	{
		SectionsMatchingTheSelection.Append(SelectedSection.FindMatches(AvailableNodes));
	}
}
...
bool URigVMGraph::AreSectionsEnabled() const
{
#if WITH_EDITOR
	if (const IRigVMClientHost* ClientHost = GetImplementingOuter<IRigVMClientHost>())
	{
		const URigVMEditorSettings* Settings = GetDefault<URigVMEditorSettings>(ClientHost->GetRigVMEditorSettingsClass());
		if (Settings && Settings->bHighlightSimilarNodes)
		{
			return true;
		}
	}
#endif
	return bSectionsEnabled;
}

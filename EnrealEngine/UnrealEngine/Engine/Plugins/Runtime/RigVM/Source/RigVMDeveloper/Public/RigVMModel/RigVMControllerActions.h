...
{
public:
	RIGVMDEVELOPER_API FRigVMActionWrapper(const FRigVMActionKey& Key);
	RIGVMDEVELOPER_API ~FRigVMActionWrapper();

	RIGVMDEVELOPER_API const UScriptStruct* GetScriptStruct() const;
	RIGVMDEVELOPER_API FRigVMBaseAction* GetAction(URigVMController* InLastController = nullptr) const;
	RIGVMDEVELOPER_API FString ExportText() const;
...
public:
	...
	// Default constructor
	RIGVMDEVELOPER_API FRigVMBaseAction(URigVMController* InController);

	// Default destructor
	virtual ~FRigVMBaseAction() {};

	// Returns the controller of this action
	RIGVMDEVELOPER_API URigVMController* GetController() const;
	...
	RIGVMDEVELOPER_API virtual bool Merge(const FRigVMBaseAction* Other);

	// Returns true if this action makes the other action obsolete.
	RIGVMDEVELOPER_API virtual bool MakesObsolete(const FRigVMBaseAction* Other) const;
	...
	RIGVMDEVELOPER_API virtual bool Undo();

	// Re-does the action and returns true if successfull.
	RIGVMDEVELOPER_API virtual bool Redo();
	...
	RIGVMDEVELOPER_API bool StoreNode(const URigVMNode* InNode, bool bIsPriorChange = true);
	RIGVMDEVELOPER_API bool RestoreNode(const FName& InNodeName, bool bIsUndoing);

#if RIGVM_ACTIONSTACK_VERBOSE_LOG
	
	// Logs the action to the console / output log
	RIGVMDEVELOPER_API void LogAction(const FString& InPrefix) const;
...
protected:
	...
	RIGVMDEVELOPER_API void EnsureControllerValidity() const;
	...
	UPROPERTY()
	FSoftObjectPath ClientHostPath;

	UPROPERTY()
	FString ControllerModelPath;
...
public:
	...
	static RIGVMDEVELOPER_API URigVMActionStack* GetDisabledActionStack();
	...
	UFUNCTION()
	RIGVMDEVELOPER_API bool OpenUndoBracket(URigVMController* InController, const FString& InTitle);

	// Closes an undo bracket / scope.
	// This is primary useful for Python.
	UFUNCTION()
	RIGVMDEVELOPER_API bool CloseUndoBracket(URigVMController* InController);

	// Cancels an undo bracket / scope.
	// This is primary useful for Python.
	UFUNCTION()
	RIGVMDEVELOPER_API bool CancelUndoBracket(URigVMController* InController);
	...
	UFUNCTION()
	RIGVMDEVELOPER_API bool Undo(URigVMController* InController);

	// Pops the last action from the redo stack and perform redo on it.
	// Note: This should really only be used for unit tests,
	// use the GEditor's main Redo method instead.
	UFUNCTION()
	RIGVMDEVELOPER_API bool Redo(URigVMController* InController);

#if WITH_EDITOR
	RIGVMDEVELOPER_API virtual void PostTransacted(const FTransactionObjectEvent& TransactionEvent) override;
#endif
...
private:
	...
	RIGVMDEVELOPER_API void LogAction(const UScriptStruct* InActionStruct, const FRigVMBaseAction& InAction, const FString& InPrefix);
#endif

	RIGVMDEVELOPER_API bool IsDisabled() const;
...
public:
	...
	virtual UScriptStruct* GetScriptStruct() const override { return FRigVMSetPinCategoryExpansionAction::StaticStruct(); }
	virtual bool Undo() override;
	virtual bool Redo() override;

...
FString URigVMUnitNode::GetNodeSubTitle() const
{
	...
	if (StructOnScope.IsValid())
	{
		...
		return StructMemory->GetUnitSubTitle();
	}
...
FText URigVMUnitNode::GetToolTipTextForPin(const URigVMPin* InPin) const
{
	if(UScriptStruct* Struct = GetScriptStruct())
	{
		...
		auto GetTooltipTextFromStruct = [this, InPin, &Parts](UScriptStruct* InStruct, int32 InStartIndex)
		{
			for (int32 PartIndex = InStartIndex; PartIndex < Parts.Num(); PartIndex++)
			{
				FProperty* Property = InStruct->FindPropertyByName(*Parts[PartIndex]);
				if (!Property)
				{
				...
				{
					InStruct = StructProperty->Struct;
				}
			}
		...
		FText TooltipText = GetTooltipTextFromStruct(Struct, 1);
		if (!TooltipText.IsEmpty())
		{
			return TooltipText; 
		}

		// Could be a trait pin
		Struct = InPin->GetTraitScriptStruct();
		if (Struct)
		{
			TooltipText = GetTooltipTextFromStruct(Struct, 2);
			if (!TooltipText.IsEmpty())
			{
				return TooltipText; 
			}
...
void URigVMUnitNode::EnumeratePropertiesOnHostAndStructInstance(URigVMHost* InHost, TSharedPtr<FStructOnScope> InInstance, bool bPreferLiterals, TFunction<void(const URigVMPin*,const FProperty*,uint8*,const FProperty*,uint8*)> InEnumerationFunction, int32 InSliceIndex) const
{
	...
	const IRigVMAssetInterface* Blueprint = GetImplementingOuter<IRigVMAssetInterface>();
	...
	for(int32 Index = 0; Index < InstructionIndices.Num(); Index++)
	{
		...
		for(int32 ArgumentIndex = 0; ArgumentIndex < FunctionArguments.Num(); ArgumentIndex++)
		{
			const FRigVMFunctionArgument& Argument = FunctionArguments[ArgumentIndex];
			...
			if (!Operands.IsValidIndex(ArgumentIndex))
			{
				continue;
			}

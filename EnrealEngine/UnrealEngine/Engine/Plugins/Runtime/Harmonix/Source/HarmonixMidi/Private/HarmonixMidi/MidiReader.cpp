...
void FStdMidiFileReader::ReadTrackHeader()
{
	...
	UE_LOG(LogMIDI, VeryVerbose, TEXT("Reading track header (starting %" INT64_FMT " bytes into file)..."), Archive->Tell());
...
void FStdMidiFileReader::ReadMetaEvent(int32 Tick, uint8 Type)
{
	...
	case GMeta_TrackName:
		...
		else if (CurrentTrackName.IsEmpty())
		{
			CurrentTrackName = FString::Format(TEXT("Track-{0}"), {CurrentTrackIndex - 1});
			UE_LOG(LogMIDI, VeryVerbose, TEXT("TICK %d :: Got track name meta type but an empty string. Using a default track name..."), Tick);
		}
...
FString FStdMidiFileReader::ReadText(int32 Length)
{
	...
	if (Length == 0 || FailIfBytesNotAvailableInTrackData(Length))
	{
		return FString();
	}

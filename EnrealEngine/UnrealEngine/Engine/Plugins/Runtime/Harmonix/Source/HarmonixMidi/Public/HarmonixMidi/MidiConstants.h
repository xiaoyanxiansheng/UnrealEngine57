...
namespace Harmonix::Midi::Constants
{
	...
	inline bool  IsStatus(uint8 Byte)     { return (Byte & GStatusBitMask) > 0;  }
	inline uint8 GetType(uint8 Status)    { check(IsStatus(Status)); return Status & GMessageTypeMask; }

	inline bool  IsNoteOff(uint8 Status)  { return GetType(Status) == GNoteOff;  }
	inline bool  IsNoteOn(uint8 Status)   { return GetType(Status) == GNoteOn;   }
	inline bool  IsPolyPres(uint8 Status) { return GetType(Status) == GPolyPres; }
	inline bool  IsControl(uint8 Status)  { return GetType(Status) == GControl;  }
	inline bool  IsProgram(uint8 Status)  { return GetType(Status) == GProgram;  }
	inline bool  IsChanPres(uint8 Status) { return GetType(Status) == GChanPres; }
	inline bool  IsPitch(uint8 Status)    { return GetType(Status) == GPitch;    }
	inline bool  IsSystem(uint8 Status)   { return GetType(Status) == GSystem;   }

	inline uint8 GetChannel(uint8 Status) { check(IsStatus(Status));	check(!IsSystem(Status)); return Status & GChannelMask; }

	// turns MIDI style tempo (microseconds per quarter note) into BPM...
	inline float MidiTempoToBPM(int32 UsPerQuarterNote) { return UsPerQuarterNote== 0 ? 0 : 60000000.0f/(float)UsPerQuarterNote; }
	inline int32 BPMToMidiTempo(float Bpm)       { return Bpm == 0.0 ? 0 : (int32)(60000000.0f / (float)Bpm); }

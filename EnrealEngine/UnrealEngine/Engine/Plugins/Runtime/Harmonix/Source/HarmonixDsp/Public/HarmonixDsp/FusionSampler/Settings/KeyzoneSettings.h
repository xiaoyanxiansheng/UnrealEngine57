...
public:
	...
	UPROPERTY(EditDefaultsOnly, Category="Settings")
	bool bOverrideAdsr = false;
	
	UPROPERTY(EditDefaultsOnly, Category = "Settings", meta=(EditCondition="bOverrideAdsr", EditConditionHides=true))
	FAdsrSettings Adsr;
	...
	UE_API bool ContainsNoteAndVelocity(uint8 InNote, uint8 InVelocity) const;
	UE_API void SetVolumeDb(float Db);
	UE_API float GetVolumeDb() const;

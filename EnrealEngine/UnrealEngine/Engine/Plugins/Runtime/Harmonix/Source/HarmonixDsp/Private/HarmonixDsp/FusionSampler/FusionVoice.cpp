...
bool FFusionVoice::AssignIDs(FFusionSampler* InSampler, const FKeyzoneSettings* InKeyZone, FMidiVoiceId InVoiceID, FRelinquishHandler Handler, TSharedPtr<IStretcherAndPitchShifter, ESPMode::ThreadSafe> Shifter /*= nullptr*/)
{
	...
	{
		check(KeyZone->TimeStretchConfig.OriginalTempo > 0.0f);
	}
...
void FFusionVoice::SetPitchShiftForMidiNote(uint8 InNote)
{
	...
	double NewPitchOffsetCents = (double)SemitoneOffset * 100.0 + PitchModulation * 100 + PitchOffsetCents;
...
#endif
{
	...
	PitchModulation = 0.0;
	float StartPointMsModulation = 0.0f;
	for (int32 Idx = 0; Idx < FFusionPatchSettings::kNumModulators; ++Idx)
	{
		const FModulatorSettings& Settings = MySampler->RandomizerSettings[Idx];
		if (Settings.Target == EModulatorTarget::Pitch)
		{
			float X = FMath::FRand();
			PitchModulation += FMath::Clamp(FMath::Lerp(-Settings.Range, Settings.Range, X * Settings.Depth), -Settings.Range, Settings.Range);
		}
		else if (Settings.Target == EModulatorTarget::StartPoint)
		{
			float X = FMath::FRand();
			StartPointMsModulation += FMath::Clamp(FMath::Lerp(0, Settings.Range, X * Settings.Depth), 0, Settings.Range);
		}
	}
	...
	PrepareWithPitchOffsetAndGain(SemitoneOffset * 100.0 + PitchModulation * 100.0 + PitchOffsetCents, InGain);

	double OutputSampleOffset = 0.001 * (InStartPointMs + StartPointMsModulation) * MySampler->GetSamplesPerSecond();

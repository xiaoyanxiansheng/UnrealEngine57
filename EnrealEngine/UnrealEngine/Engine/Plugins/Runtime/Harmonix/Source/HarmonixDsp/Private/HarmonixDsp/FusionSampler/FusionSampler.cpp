...
namespace Defaults
{
	...
	constexpr float kMinRenderSpeed = 0.001f;
	constexpr float kMaxRenderSpeed = 10.0f;

	constexpr float kMinPitchShift = 0.0001;
	constexpr float kMaxPitchShift = 10.0f;
...
{
	FScopeLock Lock(&GetBusLock());
	if (InSpeed <= Defaults::kMinRenderSpeed)
	{
		UE_LOG(LogFusionSampler, Warning, TEXT("Speed value cannot be less than %.8f. Clamping"), Defaults::kMinRenderSpeed);
		Speed = Defaults::kMinRenderSpeed;	
	}
	else if (InSpeed > Defaults::kMaxRenderSpeed)
	{
		UE_LOG(LogFusionSampler, Warning, TEXT("Speed value cannot greater than %.2f. Clamping"), Defaults::kMaxRenderSpeed);
		Speed = Defaults::kMaxRenderSpeed;
	}
	else
	{
		Speed = InSpeed;
...
void FFusionSampler::ApplyPatchSettings()
{
	...
	for (int32 Idx = 0; Idx < kNumModulators; ++Idx)
	{
		RandomizerSettings[Idx] = Settings.Randomizer[Idx];
	}

	for (int32 Idx = 0; Idx < kNumModulators; ++Idx)
	{
		VelocityModulatorSettings[Idx] = Settings.VelocityModulator[Idx];
	}
...
void FFusionSampler::ResetPatchRelatedState()
{
	...
	{
		RandomizerSettings[ModulatorIdx] = {};
	}
	
	// set up the default randomizer settings
	for (int32 ModulatorIdx = 0; ModulatorIdx < kNumModulators; ++ModulatorIdx)
	{
		VelocityModulatorSettings[ModulatorIdx] = {};
	}

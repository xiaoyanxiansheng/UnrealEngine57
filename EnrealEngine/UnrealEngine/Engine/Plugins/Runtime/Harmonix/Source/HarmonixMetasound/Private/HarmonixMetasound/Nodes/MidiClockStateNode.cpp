...
namespace HarmonixMetasound
{
	...
	using FMidiClockStateNode = Metasound::TNodeFacade<FMidiClockStateOperator>;
	...
	void FMidiClockStateOperator::Execute()
	{
		...
		if (MidiClockInPin->GetTempoAtEndOfBlock() != *CurrentTempoOutPin)
		{
			HasTempoSpeedOrTimeSigChange = true;
			*CurrentTempoOutPin = FMath::Max(kTinyTempoSpeed, MidiClockInPin->GetTempoAtEndOfBlock());
		}
		if (MidiClockInPin->GetSpeedAtEndOfBlock() != *CurrentSpeedOutPin)
		{
			HasTempoSpeedOrTimeSigChange = true;

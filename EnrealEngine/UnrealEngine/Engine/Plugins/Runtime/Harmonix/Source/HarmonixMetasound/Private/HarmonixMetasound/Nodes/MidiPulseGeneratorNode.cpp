...
namespace HarmonixMetasound::Nodes::MidiPulseGeneratorNode
{
	...
	namespace Inputs
	{
		...
		DEFINE_INPUT_METASOUND_PARAM(DutyCycle, "Duty Cycle", "The pulse width as a percentage of the Interval [0.0, 1.0]")
	...
	public:
		...
		static const Metasound::FVertexInterface& GetVertexInterface()
		{
			...
			static const FVertexInterface Interface
			{
				FInputVertexInterface {
					...
					TInputDataVertex<float>(METASOUND_GET_PARAM_NAME_AND_METADATA(Inputs::DutyCycle), 1.0f),
		...
		struct FInputs
		{
			...
			Metasound::FFloatReadRef DutyCycle;
		...
		static TUniquePtr<IOperator> CreateOperator(const Metasound::FBuildOperatorParams& InParams, Metasound::FBuildResults& OutResults)
		{
			FInputs Inputs
			{
				...
				InParams.InputData.GetOrCreateDefaultDataReadReference<float>(Inputs::DutyCycleName, InParams.OperatorSettings),
		...
		virtual void BindInputs(Metasound::FInputVertexInterfaceData& InVertexData) override
		{
			...
			InVertexData.BindReadVertex(Inputs::DutyCycleName, Inputs.DutyCycle);
	...
	private:
		...
		{
			PulseGenerator.Enable(*Inputs.Enable);
			...
			PulseGenerator.SetDutyCycle(*Inputs.DutyCycle);
		}

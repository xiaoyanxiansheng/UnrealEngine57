...
public:
	...
	UE_API TUniquePtr<Audio::IAnalyzerSettings> GetSettings(const float InSampleRate, const int32 InNumChannels) const;
...
public:
	...
	UE_API void GetCenterFrequencies(UPARAM(DisplayName = "Center Frequencies") TArray<float>& OutCenterFrequencies);

	UFUNCTION(BlueprintCallable, Category = "Audio Analyzer")
	UE_API UPARAM(DisplayName = "Num Center Frequencies") const int32 GetNumCenterFrequencies() const;

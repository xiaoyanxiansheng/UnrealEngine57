...
namespace Audio
{
	...
	{
		public:
			UE_API FOnsetStrengthAnalyzer(const FOnsetStrengthSettings& InSettings, float InSampleRate);

			/** Calculates onset strengths from audio and fills OutOnsetStrengths with generated onset strengths */
			UE_API void CalculateOnsetStrengths(TArrayView<const float> InSamples, TArray<float>& OutOnsetStrengths);

			/** Call when done processing audio for an audio analyzer */
			UE_API void FlushAudio(TArray<float>& OutEnvelopeStrengths);
			
			/** Call to reset internal counters and lag spectra. */
			UE_API void Reset();

			/** Converts an onset strength index into a timestamp. */
			static UE_API float GetTimestampForIndex(const FOnsetStrengthSettings& InSettings, float InSampleRate, int32 InIndex);

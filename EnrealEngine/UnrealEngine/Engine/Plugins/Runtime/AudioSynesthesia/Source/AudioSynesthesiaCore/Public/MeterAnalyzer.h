...
namespace Audio
{
	...
	public:
		...
		/** Construct analyzer */
		UE_API FMeterAnalyzer(float InSampleRate, int32 NumChannels, const FMeterAnalyzerSettings& InSettings);
		 ...
		 */
		UE_API FMeterAnalyzerResults ProcessAudio(TArrayView<const float> InSampleView);

		/**
		 * Return const reference to settings used inside this analyzer.
		 */
		UE_API const FMeterAnalyzerSettings& GetSettings() const;

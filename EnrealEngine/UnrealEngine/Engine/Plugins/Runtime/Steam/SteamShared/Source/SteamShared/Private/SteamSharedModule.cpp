...
/**
 * Write the steam app id to the steam_appid.txt file before initializing the API
 */
static bool WriteSteamAppIdToDisk()
{
	...
	{
		UE_LOG(LogSteamShared, Warning, TEXT("Missing SteamDevAppId key in OnlineSubsystemSteam configuration in .ini file."));
		return false;
	}
	...
	if (SteamDevAppId > 0)
	{
		...
		if (!Handle)
		{
			UE_LOG(LogSteamShared, Warning, TEXT("Failed to create file: %s"), *SteamAppIdFilename);
			return false;
		}
		else
		{
	...
	UE_LOG(LogSteamShared, Warning, TEXT("Steam App Id provided (%d) is invalid, must be greater than 0!"), SteamDevAppId);
	return false;
}

/**
 * Deletes the steam app id file from disk
 */
	...
	FSteamInstanceHandlerBase()
{
	...
	{
#if !UE_BUILD_SHIPPING && !UE_BUILD_SHIPPING_WITH_EDITOR
		if (!WriteSteamAppIdToDisk())
		{
			UE_LOG(LogSteamShared, Warning, TEXT("Failed to create steam_appid.txt. If the file doesn't exist, SteamAPI_Init will fail."));
		}
#endif // !UE_BUILD_SHIPPING && !UE_BUILD_SHIPPING_WITH_EDITOR 

...
public:
	...
	/** Server Unicast unreliable with reference */
	UFUNCTION(Unreliable, Server)
	void ServerUnreliableWithExportRPC(const TArray<uint8>& Payload, UObject* Ref);
	bool bServerUnreliableWithExportRPCCalled = false;
	int32 ServerUnreliableWithExportRPCCallOrder = 0;
	int32 ServerUnreliableWithExportRPCCallCounter = 0;

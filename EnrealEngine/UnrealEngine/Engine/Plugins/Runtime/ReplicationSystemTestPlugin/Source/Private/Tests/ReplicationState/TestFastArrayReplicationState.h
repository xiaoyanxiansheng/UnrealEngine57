...
struct FTestFastArrayReplicationState_FastArrayItem : public FFastArraySerializerItem
{
	...
	UPROPERTY(NotReplicated)
	bool bSkipWriteFastArrayItem = false;
...
struct FTestFastArrayReplicationState_FastArraySerializer : public FIrisFastArraySerializer
{
	...
	// Test of experimental TIrisFastArrayEditor interface
	...
	// Should really never have non replicated elements in a fastarray especially on server, but that does not mean that we should not test it.
	template<typename Type, typename SerializerType>
	bool ShouldWriteFastArrayItem(const Type& Item, const bool bIsWritingOnClient) const
	{
		if (Item.bSkipWriteFastArrayItem)
		{
			return false;
		}

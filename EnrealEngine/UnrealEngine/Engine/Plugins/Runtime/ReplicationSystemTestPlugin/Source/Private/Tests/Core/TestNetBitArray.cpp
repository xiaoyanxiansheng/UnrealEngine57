...
{	
	...
	// Test ForAllSetBitsInRange with ALL bits set
	{
		uint32 SingleWordBuffer = 0xffffffff;
		FNetBitArrayView BitArray(&SingleWordBuffer, 32);

		FCollectSetBitsFunctor FunctorOne;
		BitArray.ForAllSetBitsInRange(0, 31, FunctorOne);

		UE_NET_ASSERT_EQ(32, FunctorOne.Invoked.Num());

		FCollectSetBitsFunctor FunctorTwo;
		BitArray.ForAllSetBitsInRange(0, 15, FunctorTwo);

		UE_NET_ASSERT_EQ(16, FunctorTwo.Invoked.Num());

		FCollectSetBitsFunctor FunctorThree;
		BitArray.ForAllSetBitsInRange(16, 31, FunctorThree);

		UE_NET_ASSERT_EQ(16, FunctorThree.Invoked.Num());
	}

	// Test ForAllSetBitsInRange with NO bits set
	{
		uint32 SingleWordBuffer = 0x00000000;
		FNetBitArrayView BitArray(&SingleWordBuffer, 32);

		FCollectSetBitsFunctor Functor;
		BitArray.ForAllSetBitsInRange(0, 31, Functor);

		UE_NET_ASSERT_EQ(0, Functor.Invoked.Num());
	}

	// Test ForAllSetBitsInRange with HALF bits set
	{
		uint32 SingleWordBuffer = 0xffff0000;
		FNetBitArrayView BitArray(&SingleWordBuffer, 32);

		FCollectSetBitsFunctor Functor;
		BitArray.ForAllSetBitsInRange(0, 31, Functor);

		UE_NET_ASSERT_EQ(16, Functor.Invoked.Num());
	}

	// Test ForAllSetBitsInRange with HALF bits set (interleaved)
	{
		uint32 SingleWordBuffer = 0xaaaaaaaa;
		FNetBitArrayView BitArray(&SingleWordBuffer, 32);
	...
	// Test ForAllSetBitsInRange double word boundary test (HALF bits set, interleaved)
	{
		uint64 DoubleWordBuffer = 0xaaaaaaaaaaaaaaaa;
		FNetBitArrayView BitArray(reinterpret_cast<uint32*>(&DoubleWordBuffer), 64);

		FCollectSetBitsFunctor Functor;
		BitArray.ForAllSetBitsInRange(20, 39, Functor);

		UE_NET_ASSERT_EQ(10, Functor.Invoked.Num());
	}
}

...
#include UE_INLINE_GENERATED_CPP_BY_NAME(ScriptableInteractiveTool)

#define LOCTEXT_NAMESPACE "UScriptableInteractiveTool"

DECLARE_LOG_CATEGORY_CLASS(LogScriptableInteractiveTool, Log, All);
...
UScriptableInteractiveToolPropertySet* UScriptableInteractiveToolPropertySet::SetPropertyAsReadOnly(FString PropertyName, bool bReadOnly)
{
	FProperty* Property = UELocal::FindValidPropertyByTypeAndName<FProperty>(
		this, PropertyName, TEXT("SetReadOnly"));
	if (Property == nullptr)
	{
		return this;
	}
	if (!bReadOnly)
	{
		Property->ClearPropertyFlags(EPropertyFlags::CPF_EditConst);
	}
	else
	{
		Property->SetPropertyFlags(EPropertyFlags::CPF_EditConst);
	}

	if (UScriptableInteractiveTool* OwningTool = ParentTool.Get())
	{
		OwningTool->OnPropertyModifiedDirectlyByTool.Broadcast(this);
	}

	return this;
}

UScriptableInteractiveToolPropertySet* UScriptableInteractiveToolPropertySet::SetPropertyAsHidden(FString PropertyName, bool bHidden)
{
	FProperty* Property = UELocal::FindValidPropertyByTypeAndName<FProperty>(
		this, PropertyName, TEXT("SetHidden"));
	if (Property == nullptr)
	{
		return this;
	}
	if (bHidden)
	{
		Property->ClearPropertyFlags(EPropertyFlags::CPF_Edit);
	}
	else
	{
...
void UScriptableInteractiveTool::AddLogMessage(FText Message, bool bHighlighted)
{
	if (UInteractiveToolManager* ToolManager = GetToolManager())
	{
		if (bHighlighted)
		{
			UE_LOG(LogScriptableInteractiveTool, Warning, TEXT("%s"), *Message.ToString() );
		}
		else
		{
			UE_LOG(LogScriptableInteractiveTool, Display, TEXT("%s"), *Message.ToString() );
		}
	}
}
...
AActor* UScriptableInteractiveTool::GetDrawableGeometryActor() const
{
	return ToolDrawableGeometry ? ToolDrawableGeometry->GetActor() : nullptr;
}
...
void UScriptableInteractiveTool::SetToolPanelToolHeaderWidget(UUserWidget* Widget)
{
	ToolPanelToolHeaderWidget = Widget;

	// Must invoke TakeWidget here to ensure that the underlying Slate widget is created
	// on this call, otherwise calls to set properties on the widget will fail during
	// OnScriptSetup.
	if (ToolPanelToolHeaderWidget)
	{
		ToolPanelToolHeaderWidget->TakeWidget();
	}
}

TSharedRef<SWidget> UScriptableInteractiveTool::GetToolPanelToolHeaderWidget() const
{
	return ToolPanelToolHeaderWidget ? ToolPanelToolHeaderWidget->TakeWidget() : SNullWidget::NullWidget;
}
...
void UScriptableInteractiveTool::SetFocusInViewport()
{
	UContextObjectStore* ContextStore = GetToolManager()->GetContextObjectStore();

	UScriptableToolViewportFocusAPI* ViewportFocusAPI = ContextStore->FindContext< UScriptableToolViewportFocusAPI>();
	if (ViewportFocusAPI)
	{
		ViewportFocusAPI->SetFocusInViewport();
	}
}

...
bool CVar_MVVMViewBindingFilterMode_Enable = false;
FAutoConsoleVariableRef CVar_MVVMViewBindingFilterMode(TEXT("MVVM.ViewBinding.FilterMode.Enable"), CVar_MVVMViewBindingFilterMode_Enable, TEXT(""));
...
void SBindingsPanel::Construct(const FArguments& InArgs, TSharedPtr<FWidgetBlueprintEditor> WidgetBlueprintEditor, bool bInIsDrawerTab)
{
	...
	{	//Make a view available from start without waiting for a view model to be added.
		GEditor->GetEditorSubsystem<UMVVMEditorSubsystem>()->RequestView(WidgetBlueprint);
	}
...
bool SBindingsPanel::IsFilterModeEnabled() const
{
	return CVar_MVVMViewBindingFilterMode_Enable;
}
...
TSharedRef<SWidget> SBindingsPanel::GenerateEditViewWidget()
{
	...
	TSharedRef<SWidget> FilterWidget = SNullWidget::NullWidget;
	if (BindingsList && IsFilterModeEnabled())
	{
		FilterWidget = SNew(SSegmentedControl<SBindingsList::EFilterMode>)
						.Value(BindingsList.ToSharedRef(), &SBindingsList::GetActiveFilterMode)
						.OnValueChanged(BindingsList.ToSharedRef(), &SBindingsList::OnActiveFilterModeChanged)
						+ SSegmentedControl<SBindingsList::EFilterMode>::Slot(SBindingsList::EFilterMode::All)
						.Icon(FAppStyle::GetBrush("Icons.Link"))
						.ToolTip(LOCTEXT("BindingPanelAllFilterModeToolTip", "All bindings"))
						+ SSegmentedControl<SBindingsList::EFilterMode>::Slot(SBindingsList::EFilterMode::Viewmodel)
						.Icon(FMVVMEditorStyle::Get().GetBrush("BlueprintView.TabIcon"))
						.ToolTip(LOCTEXT("BindingPanelViewModelFilterModeToolTip", "ViewModel bindings"))
						+ SSegmentedControl<SBindingsList::EFilterMode>::Slot(SBindingsList::EFilterMode::Verse)
						.Icon(FMVVMEditorStyle::Get().GetBrush("BindingView.VerseBinding"))
						.ToolTip(LOCTEXT("BindingPanelVerseFilterModeToolTip", "Verse bindings"));
	}
...
{
	//Don't display the message about requiring viewmodels if viewmodels are actually not necessary.
	const UMVVMDeveloperProjectSettings* MVVMSettings = GetDefault<UMVVMDeveloperProjectSettings>();
	if (MVVMSettings->bAllowBindingEditingWithoutViewModel)
	{

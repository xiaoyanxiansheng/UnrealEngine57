		...
		[
			...
			[
				...
				[
					...
					.OnDrop(this, &SFunctionParameter::HandleFieldSelectorDrop)
					.OnDragOver(this, &SFunctionParameter::HandleFieldSelectorDragOver)
...
FReply SFunctionParameter::HandleFieldSelectorDrop(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent)
{
	if (const UWidgetBlueprint* WidgetBlueprintPtr = WidgetBlueprint.Get())
	{
		const UMVVMEditorSubsystem* Subsystem = GEditor->GetEditorSubsystem<UMVVMEditorSubsystem>();
		if (UMVVMBlueprintView* View = Subsystem->GetView(WidgetBlueprintPtr))
		{
			if (FMVVMBlueprintViewBinding* Binding = View->GetBinding(BindingId))
			{
				TOptional<FMVVMBlueprintPropertyPath> PropertyPath = BindingEntry::FRowHelper::DropFieldSelector(WidgetBlueprintPtr, DragDropEvent, bSourceToDestination);
				if (PropertyPath.IsSet())
				{
					Subsystem->SetPathForConversionFunctionArgument(WidgetBlueprint.Get(), *Binding, ParameterId, PropertyPath.GetValue(), bSourceToDestination);
				}
...
FReply SFunctionParameter::HandleFieldSelectorDragOver(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent)
{
	UWidgetBlueprint* WidgetBlueprintPtr = WidgetBlueprint.Get();
	if (WidgetBlueprintPtr == nullptr)
	{
	...
	return BindingEntry::FRowHelper::DragOverFieldSelector(WidgetBlueprintPtr, DragDropEvent, bSourceToDestination);
}

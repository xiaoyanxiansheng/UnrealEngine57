...
bool FMVVMViewBlueprintCompiler::IsPropertyPathValid(const UBlueprint* Context, TArrayView<const FMVVMConstFieldVariant> PropertyPath)
{
	...
	for (int32 Index = 0; Index < PathLength; Index++)
	{
		...
		if (Field.IsProperty())
		{
			...
			if (!GetDefault<UMVVMDeveloperProjectSettings>()->IsPropertyAllowed(Context, CurrentContainer, Field.GetProperty(), false))
			{
				return false;
			}
		}
		if (Field.IsFunction())
		{
			...
			if (!GetDefault<UMVVMDeveloperProjectSettings>()->IsFunctionAllowed(Context, CurrentContainerAsClass, Field.GetFunction(), false))
			{
				return false;
			}
		}
...
void FMVVMViewBlueprintCompiler::TestGenerateSetter(const UBlueprint* Context, FStringView ObjectName, FStringView FieldPath, FStringView FunctionName)
{
	...
	{
		UObject* FoundObject = FindObject<UObject>(nullptr, ObjectName.GetData(), EFindObjectFlags::None);

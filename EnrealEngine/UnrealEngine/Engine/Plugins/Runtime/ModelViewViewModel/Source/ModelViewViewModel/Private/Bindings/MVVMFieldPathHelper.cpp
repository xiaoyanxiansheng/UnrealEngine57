...
const UWidgetBlueprintGeneratedClass* GetComponentPropertyPathSource(TArray<UE::MVVM::FMVVMConstFieldVariant> ComponentPaths, const UWidgetBlueprintGeneratedClass* Accessor)
{
	const UWidgetBlueprintGeneratedClass* ResultWidgetBlueprintGeneratedClass = Accessor;

	for (int32 Index = 0; Index < ComponentPaths.Num(); ++Index)
	{
		const UE::MVVM::FMVVMConstFieldVariant& Field = ComponentPaths[Index];
		if (Field.IsProperty())
		{
			if (const FObjectPropertyBase* ObjectProperty = CastField<const FObjectPropertyBase>(Field.GetProperty()))
			{
				if (ObjectProperty->PropertyClass->IsChildOf(UUIComponent::StaticClass()))
				{
					if (const UWidgetBlueprintGeneratedClass* OwnerWidgetBlueprintGeneratedClass = Cast<UWidgetBlueprintGeneratedClass>(ObjectProperty->GetOwnerClass()))
					{
						ResultWidgetBlueprintGeneratedClass = OwnerWidgetBlueprintGeneratedClass;
						break;
					}
				}
			}
	...
	return ResultWidgetBlueprintGeneratedClass;
}

...
bool FMVVMCompiledBindingLibrary::CanAccessLoadedProperiesAtIndex(int16 index) const
{
	if (LoadedProperties.IsValidIndex(index))
	{
		return true;
	}
	FString LoadedPropertiesString;
	for (int32 i = 0; i < LoadedProperties.Num(); ++i)
	{
		LoadedPropertiesString.Append(FString::Format(TEXT("{0}, "), { LoadedProperties[i]->GetNameCPP() }));
	}
	FString DebugLoadedProperties = FString::Format(TEXT("{0} on {1}. LoadedProperties : {2}"), { index, LoadedProperties.Num(), LoadedPropertiesString });
	checkf(false, TEXT("LoadedProperties.IsValidIndex(PathIndex.Index) failed. '%s'"), *DebugLoadedProperties);
	UE_LOG(LogMVVM, Error, TEXT("LoadedProperties.IsValidIndex(PathIndex.Index) failed. '%s'"), *DebugLoadedProperties);
	return false;
}
...
TValueOrError<UE::MVVM::FFieldContext, void> FMVVMCompiledBindingLibrary::EvaluateFieldPath(UObject* InExecutionSource, const FMVVMVCompiledFieldPath& InFieldPath) const
{
	...
	for (int32 Index = InFieldPath.StartIndex; Index < IterationMax; ++Index)
	{
		...
		{
			if (!CanAccessLoadedProperiesAtIndex(PathIndex.Index))
			{
				return MakeError();
			}
...
TValueOrError<UE::MVVM::FMVVMFieldVariant, void> FMVVMCompiledBindingLibrary::GetFinalFieldFromPathImpl(UE::MVVM::FObjectVariant CurrentContainer, const FMVVMVCompiledFieldPath& InFieldPath) const
{
	...
	{
		if (!CanAccessLoadedProperiesAtIndex(FinalPathIndex.Index))
		{
			return MakeError();
		}

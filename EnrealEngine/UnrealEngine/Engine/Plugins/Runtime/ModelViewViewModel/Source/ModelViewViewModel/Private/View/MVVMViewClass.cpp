...
void UMVVMViewClass::Construct(UUserWidget* UserWidget)
{
	...
	if (GEditor)
	{
		...
		Module.OnBlueprintCompiled.AddUObject(this, &UMVVMViewClass::HandleBlueprintCompiled);
	}
#endif
}
...
void UMVVMViewClass::Destruct(UUserWidget* UserWidget)
{
	...
	if (UserWidget)
	{
		bool bIsAnyViewFromThisClass = false;
		for (UUserWidgetExtension* Extension : UserWidget->GetExtensions(UMVVMView::StaticClass()))
		{
			if (UMVVMView* View = Cast<UMVVMView>(Extension))
			{
				if (View->GetViewClass() == this)
				{
					bIsAnyViewFromThisClass = true;
					break;
				}
			}
		}

		if (!bIsAnyViewFromThisClass)
		{
			return;
		}
	}
	...
	{
		FModelViewViewModelModule& Module = FModuleManager::GetModuleChecked<FModelViewViewModelModule>("ModelViewViewModel");
		Module.OnBlueprintCompiled.Remove(BlueprintCompiledHandle);
	}
#endif
}
...
void UMVVMViewClass::HandleBlueprintCompiled()
{
	...
	if (ViewCounter > 0)
	{
		BindingLibraryDirty.store(true, std::memory_order_relaxed);
	}
}

void UMVVMViewClass::ReloadIfDirty() const
{
	if (BindingLibraryDirty.load(std::memory_order_relaxed))
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(UMVVMViewClass::ReloadIfDirty);
		UMVVMViewClass* MutableThis = const_cast<UMVVMViewClass*>(this);
		MutableThis->BindingLibraryDirty.store(false, std::memory_order_relaxed);
		MutableThis->BindingLibrary.Unload();
		MutableThis->BindingLibrary.Load();
	}
}

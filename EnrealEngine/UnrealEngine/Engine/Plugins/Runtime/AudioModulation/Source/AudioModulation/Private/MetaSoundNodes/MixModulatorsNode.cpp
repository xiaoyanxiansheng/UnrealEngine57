...
namespace AudioModulation
{
	...
	public:
		...
		static TUniquePtr<Metasound::IOperator> CreateOperator(const Metasound::FBuildOperatorParams& InParams, Metasound::FBuildResults& OutResults)
		{
			...
			{
				FSoundModulatorAssetReadRef Modulator1ReadRef = InputData.GetOrCreateDefaultDataReadReference<FSoundModulatorAsset>("In1", InParams.OperatorSettings);
				FSoundModulatorAssetReadRef Modulator2ReadRef = InputData.GetOrCreateDefaultDataReadReference<FSoundModulatorAsset>("In2", InParams.OperatorSettings);
				FSoundModulationParameterAssetReadRef ParameterReadRef = InputData.GetOrCreateDefaultDataReadReference<FSoundModulationParameterAsset>("MixParameter", InParams.OperatorSettings);
	...
	using FMixModulatorsNode = Metasound::TNodeFacade<FMixModulatorsNodeOperator>;

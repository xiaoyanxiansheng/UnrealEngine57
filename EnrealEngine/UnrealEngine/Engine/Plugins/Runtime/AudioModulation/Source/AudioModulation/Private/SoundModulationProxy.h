...
namespace AudioModulation
{
	...
	private:
#if !UE_BUILD_SHIPPING
		FString ResolveName(const FString& InNamePath)
		{
			const FSoftObjectPath SoftObjectPath = FSoftObjectPath(InNamePath);
	...
	public:
		...
		TModulatorBase(const FName InPath, const uint32 InId)
			: Id(static_cast<IdType>(InId))
#if !UE_BUILD_SHIPPING
			, Name(ResolveName(InPath.ToString()))
			, Path(InPath)
#endif // !UE_BUILD_SHIPPING
		{
		}

		TModulatorBase(const FString& InPath, const uint32 InId)
			: Id(static_cast<IdType>(InId))
#if !UE_BUILD_SHIPPING
			, Name(ResolveName(InPath))
			, Path(InPath)
#endif // !UE_BUILD_SHIPPING
		{
		}
		...
		// FOR DEBUG USE ONLY (Not available in shipped builds):
		// Provides path of object that generated proxy.
	...
	public:
		...
		TModulatorProxyRefType(const FName& InPath, const uint32 InId, FAudioModulationSystem& OutModSystem)
			: TModulatorBase<IdType>(InPath, InId)
			, RefCount(0)
		...
		TModulatorProxyRefType(const TModulatorProxyRefType& InProxyRef)
			: TModulatorBase<IdType>(InProxyRef.GetPath(), InProxyRef.GetId())

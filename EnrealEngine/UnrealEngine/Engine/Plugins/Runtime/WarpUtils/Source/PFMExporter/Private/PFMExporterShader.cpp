...
)
{
	...
	const FRHIBufferCreateDesc CreateDesc =
		FRHIBufferCreateDesc::CreateVertex<FFilterVertex>(TEXT("FPFMExporterShader"), NumVertices)
		.AddUsage(EBufferUsageFlags::Dynamic)
		.SetInitActionInitializer()
		.DetermineInitialState();

	TRHIBufferInitializer<FFilterVertex> Initializer = RHICmdList.CreateBufferInitializer(CreateDesc);
	{
		//Fill buffer with vertex+selected UV channel:
		for (uint32 i = 0; i < NumVertices; i++)
		{
			FFilterVertex& Vertex = Initializer[i];

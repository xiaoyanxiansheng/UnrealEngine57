...
// ******************************************************************************************************
// Duplicate of definitions found in AudioMixerPlatformXAudio2.h/cpp
// ******************************************************************************************************
...
//     will make the OS automatically load the DLL correctly.s
//   Consoles:
...
#else // USE_REDIST_HEADER
#include <xaudio2.h>
#endif // USE_REDIST_HEADER
...
protected:
	...
	public:
		bool Initialize(const FString& OutputDeviceId, ESonyControllerType ControllerType)
		{
			...
			XAudio2Dll = LoadLibrary(*FPaths::ConvertRelativePathToFull(XAudio2_9Redist));
		...
		void Shutdown()
		{
			...
			if (XAudio2Dll)
			{
				...
				{
					UE_LOG(LogAudioMixer, Warning, TEXT("FXAudio2Device failed to free XAudio2 Dll"));
				}

...
#include UE_INLINE_GENERATED_CPP_BY_NAME(InstancedActorsData)
...
namespace UE::InstancedActors
{
	namespace CVars
	{
		...
		bool bFlagSharedFragmentInsteadOfNullify = true;
		FAutoConsoleVariableRef CVarFlagSharedFragmentInsteadOfNullify(
			TEXT("IA.FlagSharedFragmentInsteadOfNullify"),
			bFlagSharedFragmentInsteadOfNullify,
			TEXT("Enables using the bInstancedActorsDataDespawnedEntities to mark the IAD shared fragment as deinitialized instead of nullifying the IAD reference."),
			ECVF_Cheat);

		bool bReleaseEntityTemplateWhenRemovingVisualization = true;
		FAutoConsoleVariableRef CVarReleaseEntityTemplateWhenRemovingVisualization(
			TEXT("IA.ReleaseEntityTemplateWhenRemovingVisualization"),
			bReleaseEntityTemplateWhenRemovingVisualization,
			TEXT("If enabled (default) instanced actors will release created entity templates when a visualization is removed. This ensures recreating the entities using the newly created visualization"),
			ECVF_Default);
	} // CVars
...
void UInstancedActorsData::DespawnEntities(bool bIsSwitchingSubsystems /*= false*/)
{
	UWorld* World = GetWorld();
	check(World);
	...
	if (ensure(AsSharedManagerFragment) && AsSharedManagerFragment->InstanceData == this)
	{
		...
		if (!bIsSwitchingSubsystems)
		{
			if (UE::InstancedActors::CVars::bFlagSharedFragmentInsteadOfNullify)
			{
				AsSharedManagerFragment->bInstancedActorsDataDespawnedEntities = true;
			}
			else
			{
...
void UInstancedActorsData::RemoveVisualization(uint8 VisualizationIndex)
{
	...
	// Destroying visualizations can leave the FMassRepresentationFragment::StaticMeshDescHandle pointing to stale data. So we need to rebuild the UInstancedActorsVisualizationTrait
	// This is a workaround fix - we are relying on the fact that RemoveVisualization() is only being called when DespawnEntities() is called. This means that either:
	// (1) The parent AInstancedActorsManager::EndPlay() is being called
	// (2) The parent AInstancedActorsManager::OnAddedToSubsystem() is being called with a request to change subsystems -
	//     so we despawn all entities, remove all visualizations, then respawn all entities
	// In that second case, removing the visualizations means it invalidates the template's FMassRepresentationFragment::StaticMeshDescHandle.
	// If we don't force the template to be rebuilt, when this IAD respawns the entities and calls CreateEntityTemplate(), it would just fallback to the previously created template
	// The issue is that the entity template/UInstancedActorsVisualizationTrait treats the FMassRepresentationFragment::StaticMeshDescHandle as invariant
	// A better long-term solve would be to set FMassRepresentationFragment::StaticMeshDescHandle post-entity creation. And possibly error-checking the context of visualizations being removed
	if (UE::InstancedActors::CVars::bReleaseEntityTemplateWhenRemovingVisualization)
	{
		ReleaseEntityTemplate();
	}
}
...
void UInstancedActorsData::OnInstancedActorDestroyed(AActor& DestroyedActor, const FMassEntityHandle EntityHandle)
{
	...
	if (ensureMsgf(DestroyedInstanceIndex.IsValid(), TEXT("Somehow OnInstancedActorDestroyed was received for an actor which we assume was spawned by InstancedActors but %s isn't tracking this entity"), *GetDebugName()))
	{
		...
		// Note that at this point it's possible for retrieved actor to be nullptr if the related actor is already considered destroyed. 
		AActor* UnlinkedActor = UnlinkActor(DestroyedInstanceIndex);
		ensureMsgf(UnlinkedActor == &DestroyedActor || UnlinkedActor == nullptr
			, TEXT("%hs: Unexpected Unlink result. Actor being destroyed: %s, actor unlinked %s")
			, __FUNCTION__, *DestroyedActor.GetName(), *UnlinkedActor->GetName());

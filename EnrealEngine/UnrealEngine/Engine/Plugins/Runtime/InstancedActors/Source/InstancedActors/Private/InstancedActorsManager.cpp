...
#include UE_INLINE_GENERATED_CPP_BY_NAME(InstancedActorsManager)
...
void AInstancedActorsManager::OnReplicationStartedForIris(const FOnReplicationStartedParams& Params)
{
...
void AInstancedActorsManager::OnAddedToSubsystem(UInstancedActorsSubsystem& InInstancedActorSubsystem, FInstancedActorsManagerHandle InManagerHandle)
{
	if (IsValid(InstancedActorSubsystem) && (InstancedActorSubsystem != &InInstancedActorSubsystem))
	{
		...
		DespawnAllEntities(/*bIsSwitchingSubsystems*/ true);
...
void AInstancedActorsManager::InitializeModifyAndSpawnEntities()
{
	...
	// Run virtual callback to let inheritors know that we've initialized/modified/spawned entities
	OnInitializeModifyAndSpawnEntities();
}
...
void AInstancedActorsManager::DespawnAllEntities(bool bIsSwitchingSubsystems /*= false*/)
{
	...
	if (HasSpawnedEntities())
	{
		for (TObjectPtr<UInstancedActorsData> InstanceData : PerActorClassInstanceData)
		{
			...
			InstanceData->DespawnEntities(bIsSwitchingSubsystems);
		}

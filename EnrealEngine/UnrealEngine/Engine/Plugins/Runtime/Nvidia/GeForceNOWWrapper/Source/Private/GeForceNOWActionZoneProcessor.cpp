...
static int GFNActionZonesHeightBuffer = 8;
FAutoConsoleVariableRef CVarGFNActionZonesHeightBuffer(
	TEXT("GFN.GFNActionZonesHeightBuffer"),
	GFNActionZonesHeightBuffer,
	TEXT("Additional buffer zone in pixels, applied to top and down of action zones"));
	...
	if (bIsInteractable)
	{
		if (bRectChanged || !bWasInteractable)
		{
			if (ActionZoneRect.IsValid() && !ActionZoneRect.IsEmpty())
			{
				...
				ActionZoneGFNRect.value2 = ActionZoneRect.Top - GFNActionZonesHeightBuffer;
				ActionZoneGFNRect.value3 = ActionZoneRect.Right;
				ActionZoneGFNRect.value4 = ActionZoneRect.Bottom + GFNActionZonesHeightBuffer;
...
{
	FTSTicker::RemoveTicker(ProcessDelegateHandle);

...
void UReplicationGraph::RemoveClientConnection(UNetConnection* NetConnection)
{
	...
	if (!ActiveGraphConnectionRemoved && !PendingGraphConnectionRemoved)
	{
		...
		UE_LOG(LogReplicationGraph, Warning, TEXT("UReplicationGraph::RemoveClientConnection could not find connection %s in Connections (%d) or PendingConnections (%d) lists"), *GetNameSafe(NetConnection), Connections.Num(), PendingConnections.Num());
	}
}
...
void UReplicationGraph::SetActorDestructionInfoToIgnoreDistanceCulling(AActor* DestroyedActor)
{
	...
	check(NetDriver);
	FNetworkGUID NetGUID = NetDriver->GetNetGuidCache()->GetNetGUID(DestroyedActor);
...
void UNetReplicationGraphConnection::NotifyAddDestructionInfo(FActorDestructionInfo* DestructInfo)
{
	...
	if (!ensureMsgf(ExistingIdx == INDEX_NONE, TEXT("::NotifyAddDestructionInfo already contains DestructInfo: 0x%" INT64_X_FMT " (%s)"), (int64)DestructInfo, *DestructInfo->PathName))
	{
		return;
	}
#endif
...
{
#if WITH_SERVER_CODE
	if (Actor && NetConnection && NetConnection->Driver && NetConnection->Driver->GetNetGuidCache())
	{
		...
		FNetworkGUID NetGUID = NetConnection->Driver->GetNetGuidCache()->GetNetGUID(Actor);

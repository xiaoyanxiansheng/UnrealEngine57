...
public:
	...
	friend bool operator==(const FActorRepListType& RepListActor, TYPE_OF_NULLPTR)
	{
	...
	friend bool operator==(TYPE_OF_NULLPTR, const FActorRepListType& RepListActor)
	{
		return RepListActor == nullptr;
	}
	friend bool operator!=(TYPE_OF_NULLPTR, const FActorRepListType& RepListActor)
...
template< class T > inline T* Cast(const FActorRepListType& Src) { return Cast<T>(Src.GetActor()); }
template< class T > inline T* ExactCast(const FActorRepListType& Src) { return ExactCast<T>(Src.GetActor()); }
template< class T > inline T* CastChecked(const FActorRepListType& Src, ECastCheckedType::Type CheckType = ECastCheckedType::NullChecked)
{
	return CastChecked<T>(Src.GetActor(), CheckType);
}
#endif // UE_ACTOR_REPLIST_TYPE_EXTRA_SAFETY

inline FString GetActorRepListTypeDebugString(const FActorRepListType& In) { return GetNameSafe(In); }
inline UClass* GetActorRepListTypeClass(const FActorRepListType& In) { return In->GetClass(); }
...
inline bool IsActorValidForReplication(const AActor* Actor)
{
	return Actor && !Actor->IsActorBeingDestroyed() && IsValidChecked(Actor) && !Actor->IsUnreachable(); 
}
inline bool IsActorValidForReplication(const FActorRepListType& In)
{
...
inline bool IsActorValidForReplication_LogMoreInfo(const FActorRepListType& In)
{ 
...
inline bool IsActorValidForReplicationGather(const FActorRepListType& In)
{
...
struct TActorRepListViewBase
{
	...
	inline int32 Num() const { return RepList->Num; }
	inline const FActorRepListType& operator[](int32 idx) const  { repCheck(RepList); repCheck(RepList->Max > idx); return RepList->Data[idx]; }

	/** Resets the view to null - meaning it is not pointing to any list (as opposed to an empty list of some preallocated size) */
	inline void ResetToNull() { RepList = nullptr; }
	inline bool IsValid() const { return RepList != nullptr; }
...
private:
	...
	inline friend FActorRepListType* begin(const TActorRepListViewBase<PointerType>& View) { return View.RepList->Data; }
	inline friend FActorRepListType* end(const TActorRepListViewBase<PointerType>& View) { return View.RepList->Data + View.RepList->Num; }
};
...
struct FActorRepListRefView
{
	...
	inline bool RemoveFast(const FActorRepListType& ElementToRemove, bool bAllowShrink)
	{
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	...
	inline const TArray<FActorRepListConstView>& GetLists(EActorRepListTypeFlags ListFlags) const;
PRAGMA_ENABLE_DEPRECATION_WARNINGS

	inline TArrayView<const FActorRepListType> ViewActors(EActorRepListTypeFlags ListFlags) const
	{
		return ReplicationLists[(uint32)ListFlags];
	}

	inline bool ContainsLists(EActorRepListTypeFlags Flags) const
	{
...
struct TClassMap
{
	...
	inline typename TMap<FObjectKey, ValueType>::TIterator CreateIterator() { return Map.CreateIterator(); }
	inline void Reset() { Map.Reset(); }
...
struct FGlobalActorReplicationInfoMap
{
	...
	inline FGlobalActorReplicationInfo* Find(const FActorRepListType& Actor) const 
	{
	...
	/** Returns ClassInfo for a given class. */
	inline FClassReplicationInfo& GetClassInfo(UClass* Class) { return ClassMap.GetChecked(Class); }

	/** Sets class info for a given class and its derived classes if desired. Call this in your Replication Graphs setup */
	inline void SetClassInfo(UClass* InClass, const FClassReplicationInfo& Info) {	ClassMap.Set(InClass, Info); }
	
	inline TMap<FActorRepListType, TUniquePtr<FGlobalActorReplicationInfo>>::TIterator CreateActorMapIterator() { return ActorMap.CreateIterator(); }
	inline TMap<FActorRepListType, TUniquePtr<FGlobalActorReplicationInfo>>::TConstIterator CreateActorMapIterator() const { return ActorMap.CreateConstIterator(); }
	inline TMap<FObjectKey, FClassReplicationInfo>::TIterator CreateClassMapIterator() { return ClassMap.CreateIterator(); }

	int32 Num() const { return ActorMap.Num(); }

	inline void ResetActorMap() { ActorMap.Reset(); }
...
{
	inline FConnectionReplicationActorInfo& FindOrAdd(const FActorRepListType& Actor)
	{
	...
	inline FConnectionReplicationActorInfo* Find(const FActorRepListType& Actor)
	{
		if (TSharedPtr<FConnectionReplicationActorInfo>* ValuePtr = ActorMap.Find(Actor))
		{
			return ValuePtr->Get();
	...
	inline FConnectionReplicationActorInfo* FindByChannel(UActorChannel* Channel)
	{
	...
	inline void AddChannel(const FActorRepListType& Actor, UActorChannel* Channel)
	{
		if (TSharedPtr<FConnectionReplicationActorInfo>* ValuePtr = ActorMap.Find(Actor))
		{
	...
	inline void RemoveChannel(UActorChannel* Channel)
	{
		ChannelMap.Remove(Channel);
	}

	inline void RemoveActor(const FActorRepListType& Actor)
	{
		ActorMap.Remove(Actor);
	}

	inline TMap<FActorRepListType, TSharedPtr<FConnectionReplicationActorInfo>>::TIterator CreateIterator()
	{
		return ActorMap.CreateIterator();
	}

	inline void SetGlobalMap(FGlobalActorReplicationInfoMap* InGlobalMap)
	{
		GlobalMap = InGlobalMap;
	}

	inline TMap<UActorChannel*, TSharedPtr<FConnectionReplicationActorInfo>>::TIterator CreateChannelIterator()
	{
...
#else
inline void ForEachClientPIEWorld(TFunction<void(UWorld*)> Func) { }
#endif

...
bool UGeometryCacheTrackStreamable::EndCoding()
{
	...
	if (ImportVisibilitySamples.Num())
	{
		...
		// Make sure the range of all visibility samples covers the entire track duration
		if (RangeStart < Samples.Last().SampleTime)
		{
			TRange<float> VisibilityRange(RangeStart, Samples.Last().SampleTime);
			FVisibilitySample Sample(bVisible);
			Sample.Range = VisibilityRange;
			VisibilitySamples.Add(Sample);	
		}

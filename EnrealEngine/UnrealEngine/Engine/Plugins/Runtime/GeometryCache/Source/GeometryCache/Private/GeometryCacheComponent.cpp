...
void UGeometryCacheComponent::BeginDestroy()
{
	Super::BeginDestroy();
	ReleaseResources();
	// Make sure to remove the component from the streaming manager
	// There are some flows with Sequencer and PIE where it is removed in OnUnregister but
	// re-added through SetGeometryCache
...
void UGeometryCacheComponent::CollectPSOPrecacheData(const FPSOPrecacheParams& BasePrecachePSOParams, FMaterialInterfacePSOPrecacheParamsList& OutParams)
{	
	FGeometryCacheVertexVertexFactory::FDataType Data;
	FGeomCacheVertexFactory::InitDummyComponentVF(Data);

	FVertexDeclarationElementList Elements;
	FGeometryCacheVertexVertexFactory::GetVertexElements(GMaxRHIFeatureLevel, EVertexInputStreamType::Default, Data, Elements);
	...
	for (UMaterialInterface* Material : GetMaterials())
	{
		if (Material != nullptr && Material->CheckMaterialUsage_Concurrent(EMaterialUsage::MATUSAGE_GeometryCache))
		{
			...
			ComponentParams.MaterialInterface = Material;
			ComponentParams.VertexFactoryDataList.Add(FPSOPrecacheVertexFactoryData(&FGeometryCacheVertexVertexFactory::StaticType, Elements));

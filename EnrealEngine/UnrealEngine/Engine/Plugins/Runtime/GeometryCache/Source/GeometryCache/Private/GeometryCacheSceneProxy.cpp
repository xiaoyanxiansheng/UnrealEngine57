	...
	if (IsRayTracingEnabled())
	{
		...
		for (FGeomCacheTrackProxy* Section : Tracks)
		{
			if (Section != nullptr)
			{
				...
				bRequireRecreate = bRequireRecreate || Section->RayTracingGeometry.Initializer.Segments.Num() != MeshData->BatchesInfo.Num();
				...
				if (!bRequireRecreate)
				{
					...
					{
						const FGeometryCacheMeshBatchInfo& BatchInfo = MeshData->BatchesInfo[SegmentIndex];
						const FRayTracingGeometrySegment& Segment = Section->RayTracingGeometry.Initializer.Segments[SegmentIndex];
				...
				Segments.Reserve(MeshData->BatchesInfo.Num());
				...
				if (bRequireRecreate)
				{
					Section->RayTracingGeometry.ReleaseRHI();
					...
					Section->RayTracingGeometry.Initializer.Segments = MoveTemp(Segments);

					Section->RayTracingGeometry.InitRHI(RHICmdList);
				}
...
void FGeomCacheTrackProxy::InitRenderResources(FRHICommandListBase& RHICmdList, int32 NumVertices, int32 NumIndices)
{
	...
	// Init vertex factory
	FGeometryCacheVertexVertexFactory::FDataType Data;
	FGeomCacheVertexFactory::InitComponentVF(&PositionBuffers[0], &PositionBuffers[1], &TangentXBuffer, &TangentZBuffer, &TextureCoordinatesBuffer, &ColorBuffer, Data);
...
void FGeomCacheVertexFactory::InitDummyComponentVF(FGeometryCacheVertexVertexFactory::FDataType& OutData)
{
	OutData.bIsDummyData = true;
	
	// Provide empty buffers for all entries
	...
	InitComponentVF(&PositionBuffers[0], &PositionBuffers[1], &TangentXBuffer, &TangentZBuffer, &TextureCoordinatesBuffer, &ColorBuffer, OutData);

	// Clear out the pointer entries in OutData again
	OutData.PositionComponent.VertexBuffer = nullptr;
	for (FVertexStreamComponent& TextureStreamComponent : OutData.TextureCoordinates)
	{
		TextureStreamComponent.VertexBuffer = nullptr;
	}
	OutData.TangentBasisComponents[0].VertexBuffer = nullptr;
	OutData.TangentBasisComponents[1].VertexBuffer = nullptr;
	OutData.ColorComponent.VertexBuffer = nullptr;
	OutData.MotionBlurDataComponent.VertexBuffer = nullptr;
}

void FGeomCacheVertexFactory::InitComponentVF(const FVertexBuffer* PositionBuffer, const FVertexBuffer* MotionBlurDataBuffer, const FVertexBuffer* TangentXBuffer, const FVertexBuffer* TangentZBuffer, const FVertexBuffer* TextureCoordinateBuffer, const FVertexBuffer* ColorBuffer, FGeometryCacheVertexVertexFactory::FDataType& OutData)
{
	// Initialize the vertex factory's stream components.
	OutData.PositionComponent = FVertexStreamComponent(PositionBuffer, 0, sizeof(FVector3f), VET_Float3);

	OutData.TextureCoordinates.Empty();
	OutData.TextureCoordinates.Add(FVertexStreamComponent(TextureCoordinateBuffer, 0, sizeof(FVector2f), VET_Float2));
	OutData.TangentBasisComponents[0] = FVertexStreamComponent(TangentXBuffer, 0, sizeof(FPackedNormal), VET_PackedNormal);
	OutData.TangentBasisComponents[1] = FVertexStreamComponent(TangentZBuffer, 0, sizeof(FPackedNormal), VET_PackedNormal);
	OutData.ColorComponent = FVertexStreamComponent(ColorBuffer, 0, sizeof(FColor), VET_Color);
	OutData.MotionBlurDataComponent = FVertexStreamComponent(MotionBlurDataBuffer, 0, sizeof(FVector3f), VET_Float3);
}
...
void FGeomCacheVertexFactory::Init(FRHICommandListBase& RHICmdList, const FVertexBuffer* PositionBuffer, const FVertexBuffer* MotionBlurDataBuffer, const FVertexBuffer* TangentXBuffer, const FVertexBuffer* TangentZBuffer, const FVertexBuffer* TextureCoordinateBuffer, const FVertexBuffer* ColorBuffer)
{
	// Initialize the vertex factory's stream components.
	FDataType NewData;
	InitComponentVF(PositionBuffer, MotionBlurDataBuffer, TangentXBuffer, TangentZBuffer, TextureCoordinateBuffer, ColorBuffer, NewData);
...
void FGeomCacheVertexBuffer::UpdateRaw(FRHICommandListBase& RHICmdList, const void* Data, int32 NumItems, int32 ItemSizeBytes, int32 ItemStrideBytes)
{
	...
	// Preserve SRV format if the SRV needs to be recreated
	EPixelFormat Format = BufferSRV ? BufferSRV->GetDesc().Buffer.SRV.Format : PF_R32_FLOAT;

...
bool FCodecV1Decoder::DecodeFrameData(FBufferReader& Reader, FGeometryCacheMeshData &OutMeshData)
{
	...
	uint32 NumChunks;
	TArray<uint32, TInlineAllocator<8>> ChunkOffsets;
	...
	TArray<uint8>& Bytes = OutMeshData.TempBytes;
	Bytes.SetNumUninitialized(Header.PayloadSize + 16, EAllowShrinking::No); // Overallocate by 16 bytes to ensure BitReader can safely perform uint64 reads.
	...
	// Allocate buffers
	{
		...
		{
			OutMeshData.Indices.SetNumUninitialized(Header.IndexCount, EAllowShrinking::No);
		}

		OutMeshData.Positions.SetNumUninitialized(Header.VertexCount, EAllowShrinking::No);

		OutMeshData.ImportedVertexNumbers.Reset(Header.VertexCount);
		if (OutMeshData.VertexInfo.bHasImportedVertexNumbers)
		{
			OutMeshData.ImportedVertexNumbers.SetNumUninitialized(Header.VertexCount, EAllowShrinking::No);
		}

		if (OutMeshData.VertexInfo.bHasColor0)
		{
			OutMeshData.Colors.SetNumUninitialized(Header.VertexCount, EAllowShrinking::No);
		}
		else
		{
			OutMeshData.Colors.SetNumZeroed(Header.VertexCount, EAllowShrinking::No);
		}	
		
		if (OutMeshData.VertexInfo.bHasTangentX)
		{
			OutMeshData.TangentsX.SetNumUninitialized(Header.VertexCount, EAllowShrinking::No);
		}
		else
		{
			OutMeshData.TangentsX.SetNumZeroed(Header.VertexCount, EAllowShrinking::No);
		}
		
		if (OutMeshData.VertexInfo.bHasTangentZ)
		{
			OutMeshData.TangentsZ.SetNumUninitialized(Header.VertexCount, EAllowShrinking::No);
		}
		else
		{
			OutMeshData.TangentsZ.SetNumZeroed(Header.VertexCount, EAllowShrinking::No);
		}
		
		if (OutMeshData.VertexInfo.bHasUV0)
		{
			OutMeshData.TextureCoordinates.SetNumUninitialized(Header.VertexCount, EAllowShrinking::No);
		}
		else
		{
			OutMeshData.TextureCoordinates.SetNumZeroed(Header.VertexCount, EAllowShrinking::No);
		}	
		
		if (OutMeshData.VertexInfo.bHasMotionVectors)
		{
			OutMeshData.MotionVectors.SetNumUninitialized(Header.VertexCount, EAllowShrinking::No);
		}
		else
		{
			OutMeshData.MotionVectors.SetNumZeroed(Header.VertexCount, EAllowShrinking::No);
		}
	}

...
void FPreLoadMoviePlayerScreenBase::CleanUp()
{
	...
	{
		MovieAttributes = FPreLoadMovieAttributes();
	}
}
...
void FPreLoadMoviePlayerScreenBase::RenderTick(FRHICommandListImmediate& RHICmdList, float DeltaTime)
{
    if (MovieStreamingIsPrepared())
    {
        ...
        const bool bMovieIsDone = MovieStreamer->Tick(RHICmdList, DeltaTime);
        if (bMovieIsDone)
        {
            MovieStreamingIsDone.Set(1);
        }

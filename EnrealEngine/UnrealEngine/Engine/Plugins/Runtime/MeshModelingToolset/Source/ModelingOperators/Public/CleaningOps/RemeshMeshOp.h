...
class FRemeshMeshOp : public FDynamicMeshOperator
{
public:
	...
	UE_API void SetUserSpecifiedConstraints(const FMeshConstraints& Constraints);
...
public:
	static UE_API double CalculateTargetEdgeLength(const FDynamicMesh3* Mesh, int TargetTriCount, double PrecomputedMeshArea = 0.0);


protected:

	UE_API TUniquePtr<FRemesher> CreateRemesher(ERemeshType Type, FDynamicMesh3* TargetMesh);

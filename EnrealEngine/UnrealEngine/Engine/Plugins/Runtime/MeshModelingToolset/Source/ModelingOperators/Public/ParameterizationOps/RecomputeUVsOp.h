...
class FRecomputeUVsOp : public FDynamicMeshOperator
{
public:
...
protected:
	...
	*/
	...
	UE_API void NormalizeUVAreas(const FDynamicMesh3& Mesh, FDynamicMeshUVOverlay* Overlay, const TileConnectedComponents& TileComponents, float GlobalScale = 1.0f);

	UE_API void CollectIslandComponentsPerTile(const FDynamicMeshUVOverlay& UVOverlay, TArray< TileConnectedComponents >& ComponentsPerTile, bool& UseExistingUVTopology);

	UE_API virtual bool CalculateResult_Basic(FProgressCancel* Progress);
	UE_API virtual bool CalculateResult_RegionOptimization(FProgressCancel* Progress);
};
 ...
 */
UCLASS(MinimalAPI)
class URecomputeUVsOpFactory : public UObject, public UE::Geometry::IDynamicMeshOperatorFactory
{
	GENERATED_BODY()

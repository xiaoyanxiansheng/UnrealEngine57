...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TriangulateCurvesOp)
...
void FTriangulateCurvesOp::AddSpline(USplineComponent* Spline, double ErrorTolerance)
{
	...
	// Spline conversion creates a duplicate end/start point, which we don't need for triangulation
	if (Path.bClosed && Path.Vertices.Num() > 1)
	{
		Path.Vertices.Pop();
	}
	if (Paths.Num() == 1)
...
void FTriangulateCurvesOp::AddWorldCurve(TArrayView<const FVector3d> WorldSpaceVertices, bool bClosed, const FTransform& ReferenceTransform)
{
	...
	// Make sure we don't add a duplicate end/start point on closed curves
	if (bClosed && WorldSpaceVertices.Num() > 1 && WorldSpaceVertices[0] == WorldSpaceVertices.Last())
	{
		Path.Vertices.Append(WorldSpaceVertices.Slice(0, WorldSpaceVertices.Num() - 1));
	}
	else
	{
		...
		for (int32 CurveIdx = 0; CurveIdx < ToProcess.Num(); ++CurveIdx)
		{
			...
			if (OffsetClosedMethod != EOffsetClosedCurvesMethod::DoNotOffset)
			{
				PolygonsOffset(
					...
					true,
					MiterLimit,
					ConvertOffsetJoinType(OffsetJoinMethod),

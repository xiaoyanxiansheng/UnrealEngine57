...
public:
	 ...
	 */
	UE_API void Setup(UWorld* InWorld, UE::Geometry::IDynamicMeshOperatorFactory* OpGenerator);

	UE_API void Setup(UWorld* InWorld);

	/**
	 * Terminate any active computation and return the current Preview Mesh/Transform
	 */
	UE_API FDynamicMeshOpResult Shutdown();

	/**
	 * Stops any running computes and swaps in a different op generator. Does not 
	 * update the preview mesh or start a new compute.
	 */
	UE_API void ChangeOpFactory(UE::Geometry::IDynamicMeshOperatorFactory* OpGenerator);


	UE_API void ClearOpFactory();

	/**
	 * Cancel the active computation without returning anything. Doesn't destroy the mesh.
	 */
	UE_API void CancelCompute();
	...
	*/
	 ...
	 */
	UE_API bool GetCurrentResultCopy(FDynamicMesh3& MeshOut, bool bOnlyIfValid = true);
	 ...
	 */
	UE_API bool ProcessCurrentMesh(TFunctionRef<void(const UE::Geometry::FDynamicMesh3&)> ProcessFunc, bool bOnlyIfValid = false);
	 ...
	 */
	UE_API void ConfigureMaterials(UMaterialInterface* StandardMaterial, UMaterialInterface* InProgressMaterial);
	 ...
	 */
	UE_API void ConfigureMaterials(
	 ...
	 */
	UE_API void SetIsMeshTopologyConstant(bool bOn, EMeshRenderAttributeFlags ChangingAttributes = EMeshRenderAttributeFlags::AllVertexAttribs);
	 ...
	 */
	UE_API bool IsUsingWorkingMaterial();

	// Configure the maximum allowed number of background tasks
	UE_API void SetMaxActiveBackgroundTasks(int32 NewMaxTasks);

...
public:
	 ...
	 */
	UE_API void InitializeFromComponent(const UActorComponent* Component, bool bInitializeAggGeom);

	/**
	 * Initialize from the given StaticMesh, and optionally initialize internal geometry members
	 */
	UE_API void InitializeFromStaticMesh(const UStaticMesh* StaticMesh, bool bInitializeAggGeom);

	/**
	 * Initialize 
	 */
	UE_API void InitializeFromExisting(const FPhysicsDataCollection& Other);

	/**
	 * replace our geometry data with that from another FPhysicsDataCollection
	 */
	UE_API void CopyGeometryFromExisting(const FPhysicsDataCollection& Other);


	/**
	 * Empty out our FKAggregateGeom
	 */
	UE_API void ClearAggregate();

	/**
	 * Populate our FKAggregateGeom from our FSimpleShapeSet3d
	 */
	UE_API void CopyGeometryToAggregate();

...
{
	RestoreComponentsMaxDrawDistance();
}
...
void FWorldRenderCapture::RestoreComponentsMaxDrawDistance()
{
	for (UPrimitiveComponent* PrimitiveComponent : ComponentsRestoreMaxDrawDistance)
	{
		PrimitiveComponent->SetCachedMaxDrawDistance(PrimitiveComponent->LDMaxDrawDistance);
	}

	ComponentsRestoreMaxDrawDistance.Reset();
}
	...
	auto AddComponent = [&](UActorComponent* Component)
	{
		auto AddComponentImpl = [&](UActorComponent* Component, auto& AddComponentRef) -> void
		{
			...
			{				
				...
				if (!PrimitiveComponent->bNeverDistanceCull)
				{
					ComponentsRestoreMaxDrawDistance.Add(PrimitiveComponent);
					PrimitiveComponent->SetCachedMaxDrawDistance(0.0f);
				}
...
UTextureRenderTarget2D* FWorldRenderCapture::GetRenderTexture(bool bLinear)
{
	...
	TObjectPtr<UTextureRenderTarget2D>* WhichTexture = (bLinear) ? &LinearRenderTexture : &GammaRenderTexture;
...
void FWorldRenderCapture::AddReferencedObjects(FReferenceCollector& Collector)
{
	Collector.AddReferencedObjects(ComponentsRestoreMaxDrawDistance);
	Collector.AddReferencedObject(LinearRenderTexture);
	Collector.AddReferencedObject(GammaRenderTexture);
	Collector.AddReferencedObject(DepthRenderTexture);
}

FString FWorldRenderCapture::GetReferencerName() const
{
	return TEXT("FWorldRenderCapture");
}

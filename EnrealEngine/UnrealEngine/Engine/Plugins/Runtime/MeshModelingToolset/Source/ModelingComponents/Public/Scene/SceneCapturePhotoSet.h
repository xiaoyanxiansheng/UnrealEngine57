...
public:
	 ...
	 */
	UE_API void SetCaptureSceneActors(UWorld* World, const TArray<AActor*>& Actors);
	UE_API void SetCaptureSceneComponents(UWorld* World, const TArray<UActorComponent*>& Components);
	UE_API void SetCaptureSceneActorsAndComponents(UWorld* World, const TArray<AActor*>& Actors, const TArray<UActorComponent*>& Components);

	UE_API UWorld* GetCaptureTargetWorld();
	 ...
	 */
	UE_API void SetSpatialPhotoParams(const TArray<FSpatialPhotoParams>& SpatialParams);
	UE_API const TArray<FSpatialPhotoParams>& GetSpatialPhotoParams() const;
	 ...
	 */
	UE_API void DisableAllCaptureTypes();

	/**
	 * Enable/Disable a particular capture type
	 * If bEnabled == false the corresponding photo set will be cleared
	 */
	UE_API void SetCaptureTypeEnabled(ERenderCaptureType CaptureType, bool bEnabled);
	...
	UE_API ECaptureTypeStatus GetCaptureTypeStatus(ERenderCaptureType CaptureType) const;
	...
	UE_API FStatus GetSceneCaptureStatus() const;

	/**
	 * Configure the given capture type
	 * If Config != the existing capture config the corresponding photo set will be cleared
	 */
	UE_API void SetCaptureConfig(ERenderCaptureType CaptureType, const FRenderCaptureConfig& Config);
	UE_API FRenderCaptureConfig GetCaptureConfig(ERenderCaptureType CaptureType) const;
	...
	struct FSceneSample
	{
		...
		UE_API FVector3f GetValue3f(ERenderCaptureType CaptureType) const;
		UE_API FVector4f GetValue4f(ERenderCaptureType CaptureType) const;
	};
	 ...
	 */
	UE_API bool ComputeSampleLocation(
		const FVector3d& Position,
		const FVector3d& Normal,
	 ...
	 */
	UE_API void GetSceneSamples(FSceneSamples& OutSamples);
	 ...
	 */
	UE_API void SetEnableWriteDebugImages(bool bEnable, FString FolderName = FString());
	 ...
	 */
	UE_API void SetEnableVisibilityByUnregisterMode(bool bEnable);
...
private:
	...
	UE_API FSpatialPhotoSet1f& GetPhotoSet1f(ERenderCaptureType CaptureType);

	UE_API FSpatialPhotoSet3f& GetPhotoSet3f(ERenderCaptureType CaptureType);

	UE_API void EmptyPhotoSet(ERenderCaptureType CaptureType);

	UE_API void EmptyAllPhotoSets();
};

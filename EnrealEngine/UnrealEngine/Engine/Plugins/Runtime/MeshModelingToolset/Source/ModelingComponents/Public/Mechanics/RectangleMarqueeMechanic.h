...
struct FCameraRectangle
{
	...
	UE_API FRectangleInPlane ProjectSelectionDomain(double OffsetFromCameraPlane = 0.) const;

	// Return true if the given 3D geometry projected to the camera plane is inside or intersecting the rectangle, and
	// false otherwise
	UE_API bool IsProjectedPointInRectangle(const FVector& Point) const;
	UE_API bool IsProjectedSegmentIntersectingRectangle(const FVector& Endpoint1, const FVector& Endpoint2) const;
	// bool IsProjectedTriangleIntersectingRectangle(const FVector& A, const FVector& B, const FVector& C) const;

	UE_API FConvexVolume FrustumAsConvexVolume() const;
 ...
 */
UCLASS(MinimalAPI)
class URectangleMarqueeMechanic : public UInteractionMechanic, public IClickDragBehaviorTarget
{
	GENERATED_BODY()
...
public:
	...
	UE_API void Setup(UInteractiveTool* ParentTool) override;
	...
	UE_API void SetIsEnabled(bool bOn);
 ...
 */
UCLASS(MinimalAPI)
class URectangleMarqueeInteraction : public UObject, public IClickDragBehaviorTarget
{
	GENERATED_BODY()

...
#include UE_INLINE_GENERATED_CPP_BY_NAME(BoundarySelectionMechanic)
...
void UBoundarySelectionMechanic::Initialize(
	...
	EBoundarySelectionType SelectionType)
{
	if (SelectionType == EBoundarySelectionType::Loops)
	{
		TopoSelector = MakeShared<FBoundarySelector>(MeshIn, BoundaryLoopsIn);
	}
	else
	{
		TopoSelector = MakeShared<FBoundaryEdgeSpanSelector>(MeshIn, BoundaryLoopsIn);
	}

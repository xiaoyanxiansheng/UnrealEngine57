...
struct FCameraPolyLasso
{
	...
	FCameraPolyLasso() {};
	UE_API FCameraPolyLasso(const FViewCameraState& CachedCameraState);

	/** Append a point to the PolyLasso polyine using a world-space ray */
	UE_API void AddPoint(const FRay& WorldRay);

	/** Project a world-space point into the same space as the 2D PolyLine */
	UE_API FVector2D GetProjectedPoint(const FVector& Point) const;
 ...
 */
UCLASS(MinimalAPI)
class UPolyLassoMarqueeMechanic : public UInteractionMechanic, public IClickDragBehaviorTarget, public IHoverBehaviorTarget
{
	GENERATED_BODY()
...
public:
	...
	static UE_API bool ApproximateSelfClipPolyline(TArray<FVector2D>& Polyline);
};

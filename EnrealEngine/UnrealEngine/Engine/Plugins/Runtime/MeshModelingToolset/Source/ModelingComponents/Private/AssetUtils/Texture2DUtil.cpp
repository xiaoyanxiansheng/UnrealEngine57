	...
	TImageBuilder<FVector4f>& DestImage)
{
	...
	const FImageDimensions Dimensions = FImageDimensions(Width, Height);
	DestImage.SetDimensions(Dimensions);
	const int64 Num = Dimensions.Num(); //< number of pixels
	...
	{
		check(BytesPerPixel == sizeof(FVector4f));
		for (int64 i = 0; i < Num; ++i)
		{
			const uint8* PixelPtr = SourceDataPtr + (i * BytesPerPixel);
			DestImage.SetPixel(i, *reinterpret_cast<const FVector4f*>(PixelPtr));
		}
		return true;
	}
	else if (SourceFormat == TSF_R32F)
	{
		check(BytesPerPixel == sizeof(float));
		for (int64 i = 0; i < Num; ++i)
		{
			const uint8* PixelPtr = SourceDataPtr + (i * BytesPerPixel);
			const float Value = *reinterpret_cast<const float*>(PixelPtr);
			DestImage.SetPixel(i, FVector4f(Value, Value, Value, 1.f));
		}
		return true;
	}
	...
	ensureAlwaysMsgf(false, TEXT("Texture source format not handled."));

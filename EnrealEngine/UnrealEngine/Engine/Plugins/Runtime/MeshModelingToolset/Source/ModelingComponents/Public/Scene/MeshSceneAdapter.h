...
protected:
	...
	// Spatial data structure query interfaces for spline meshes
	TArray<TSharedPtr<FSpatialWrapperInfo>> SplineMeshSpatialAdapters;

	UE_API void InitializeSpatialWrappers(const TArray<FActorAdapter*>& NewItemsToProcess);

	UE_API void ForEachSpatialWrapper(TFunctionRef<void(TSharedPtr<FSpatialWrapperInfo>)> Func) const;

...
public:
	UE_API FBasePositionSnapSolver3();
	 ...
	 */
	UE_API virtual void AddPointTarget(const FVector3d& Position, int TargetID, int Priority = 100);
	 ...
	 * @param Priority importance of this snap point. Lower priority is more important.
	 */
	UE_API virtual void AddPointTarget(const FVector3d& Position, int TargetID, const FCustomMetric& CustomMetric, int Priority = 100);

	/** Remove any point targets with this TargetID */
	UE_API virtual bool RemovePointTargetsByID(int TargetID);
	 ...
	 */
	UE_API virtual void AddLineTarget(const FLine3d& Line, int TargetID, int Priority = 100);

	/** Remove any line targets with this TargetID */
	UE_API virtual bool RemoveLineTargetsByID(int TargetID);
	 ...
	 */
	UE_API virtual void AddCircleTarget(const FCircle3d& Circle, int TargetID, int Priority = 100);

	/** Remove any line targets with this TargetID */
	UE_API virtual bool RemoveCircleTargetsByID(int TargetID);


	/** Add given TargetID to the ignore list, so any points/lines with that ID will not be snapped-to */
	UE_API virtual void AddIgnoreTarget(int TargetID);
	/** Add given TargetID from the ignore list */
	UE_API virtual void RemoveIgnoreTarget(int TargetID);
	/** @return true if the given TargetID is in the ignore list */
	UE_API virtual bool IsIgnored(int TargetID) const;

	/** Discard the set of snap points and lines and clear the active snap */
	UE_API virtual void Reset();
	/** Clear the active snap */
	UE_API virtual void ResetActiveSnap();
...
protected:
	...
	UE_API virtual void SetActiveSnapData(const FSnapTargetPoint& TargetPoint, const FVector3d& FromPoint, const FVector3d& ToPoint, double Metric);
	UE_API virtual void ClearActiveSnapData();
	...
	UE_API int32 FindIndexOfBestSnapInSet(const TArray<FSnapTargetPoint>& TestTargets, double& MinMetric, int& MinPriority,
		const TFunction<FVector3d(const FVector3d&)>& GetSnapPointFromFunc);
	UE_API const FSnapTargetPoint* FindBestSnapInSet(const TArray<FSnapTargetPoint>& TestTargets, double& MinMetric, int& MinPriority,
		const TFunction<FVector3d(const FVector3d&)>& GetSnapPointFromFunc);

	UE_API bool TestSnapTarget(const FSnapTargetPoint& Target, double MinMetric, int MinPriority,
		const TFunction<FVector3d(const FVector3d&)>& GetSnapPointFromFunc);
};

...
void UGeometrySelectionManager::UnregisterSelectorFactories()
{
	Factories.Empty();
	ResetTargetCache();
}
...
TSharedPtr<UGeometrySelectionManager::FGeometrySelectionTarget> UGeometrySelectionManager::GetCachedTarget(FGeometryIdentifier TargetIdentifier, const IGeometrySelectorFactory* UseFactory)
{
	...
	SelectionTarget->Selector->GetOnSelectableElementsChanged().AddWeakLambda(this, [this]()
	{
	...
	const FRay3d& WorldRay )
{
	...
	Target->Selector->GetConditionalSelectionPreviewForRaycast( RayQueryInfo, NewPreview, &Target->Selection );
...
void UGeometrySelectionManager::ExecuteSelectionCommand(UGeometrySelectionEditCommand* Command)
{
	...
	for (TSharedPtr<FGeometrySelectionTarget> Target : ActiveTargetReferences)
	{
		...
		if (Command->CanExecuteCommand(SelectionArguments))
		{
...
void UGeometrySelectionManager::RebuildSelectableRenderCache()
{
	...
	for (int32 k = 0; k < ActiveTargetReferences.Num(); ++k)
	{
		...
		FGeometrySelection AllElementSelection;
		AllElementSelection.ElementType = GetSelectionElementType();
		AllElementSelection.TopologyType = GetSelectionTopologyType();
			
		// Get full selection, according to the type and topo of AllElementSelection
		Target->Selector->InitializeSelectionFromPredicate(AllElementSelection, [](FGeoSelectionID) -> bool { return true; }, IGeometrySelector::EInitializeSelectionMode::All, &Target->Selection);
		Target->Selector->AccumulateSelectionElements(AllElementSelection, AllElements, true, EEnumerateSelectionMapping::FacesToEdges);

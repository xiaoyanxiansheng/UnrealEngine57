...
public:
	 ...
	 * always be updated in 'Add' mode, and currently no result information will be returned.
	 *
	 * CurrentSelection is provided for selectors which have conditionally selectable geometry.
	 * While not enforced, the expectation is that we add elements to the PreviewEditor only if
	 * those elements are considered selectable with respect to CurrentSelection.
	 */
	UE_EXPERIMENTAL(5.7, "GetConditionalSelectionPreviewForRaycast is experimental while the API is under active development.")
	virtual void GetConditionalSelectionPreviewForRaycast(
		const FWorldRayQueryInfo& RayInfo,
		FGeometrySelectionEditor& PreviewEditor,
		const FGeometrySelection* CurrentSelection 
	) { GetSelectionPreviewForRaycast(RayInfo, PreviewEditor); }
...
public:
	...
	/**
	 * The OnSelectableElementsChanged delegate will fire if the set of selectable elements have changed.
	 * The set of selectable elements is a function of the state of the selection target and the set of
	 * currently selected elements (the selection 'owned' by this selector).
	 */
	UE_EXPERIMENTAL(5.7, "Selectable element invalidation notification is experimental while the API is under active development.")
	FSimpleMulticastDelegate& GetOnSelectableElementsChanged() { return OnSelectableElementsChanged; }

protected:
	FOnGeometryModified OnGeometryModifed;
	void NotifyGeometryModified() { OnGeometryModifed.Broadcast(this); }

	UE_EXPERIMENTAL(5.7, "NotifySelectableElementsChanged is experimental while the API is under active development.")
	void NotifySelectableElementsChanged() const { OnSelectableElementsChanged.Broadcast(); }
	
private:
	FSimpleMulticastDelegate OnSelectableElementsChanged;
};

...
public:
	...
	virtual TUniquePtr<TStaticMeshSpatialWrapperBase<FMeshDescriptionTriangleMeshSurfaceAdapter>> GetIntermediateWrapper()
	{
	...
	virtual bool Build(const FMeshSceneAdapterBuildOptions& BuildOptions) override
	{
		...
		{
			TUniquePtr<TStaticMeshSpatialWrapperBase<FMeshDescriptionTriangleMeshSurfaceAdapter>> TempWrapper = GetIntermediateWrapper();
...
class FSplineStaticMeshSpatialWrapperBase : public TStaticMeshSpatialWrapperBase<FMeshDescriptionTriangleMeshSurfaceAdapter>
{
public:
	FMeshDescription DeformedSourceMeshDescription;
	USplineMeshComponent* SplineMeshComponent = nullptr;

	virtual void InitializeSourceMeshDescription() = 0;

	virtual TUniquePtr<FMeshDescriptionTriangleMeshSurfaceAdapter> MakeMeshAdapter(const FMeshSceneAdapterBuildOptions& BuildOptions) override
	{
#if WITH_EDITOR
		InitializeSourceMeshDescription();
		SplineMeshComponent->PropagateSplineDeformationToMesh(DeformedSourceMeshDescription);

		TUniquePtr<FMeshDescriptionTriangleMeshSurfaceAdapter> SurfaceAdapter = MakeUnique<FMeshDescriptionTriangleMeshSurfaceAdapter>(&DeformedSourceMeshDescription, StaticMesh, BuildOptions.bOnlySurfaceMaterials);
		const FMeshBuildSettings& LODBuildSettings = StaticMesh->GetSourceModel(LODIndex).BuildSettings;
		SurfaceAdapter->SetBuildScale(LODBuildSettings.BuildScale3D, false);
		return SurfaceAdapter;
#else
...
class FSplineStaticMeshSourceDataSpatialWrapper : public FSplineStaticMeshSpatialWrapperBase
{
	virtual void InitializeSourceMeshDescription() override
	{
#if WITH_EDITOR
		StaticMesh->CloneMeshDescription(LODIndex, DeformedSourceMeshDescription);
#endif
	}
};


class FSplineStaticMeshRenderDataSpatialWrapper : public FSplineStaticMeshSpatialWrapperBase
{
	virtual void InitializeSourceMeshDescription() override
	{
#if WITH_EDITOR
		if (const FStaticMeshLODResources* LODResource = StaticMesh->GetRenderData()->GetCurrentFirstLOD(0))
		{
			StaticMesh->ExportStaticMeshLOD(*LODResource, DeformedSourceMeshDescription);
		}
#endif
	}
};
...
class FCompressedSplineStaticMeshSpatialWrapper : public FCompressedStaticMeshSpatialWrapper
{
public:
	USplineMeshComponent* SplineMeshComponent = nullptr;

	virtual TUniquePtr<TStaticMeshSpatialWrapperBase<FMeshDescriptionTriangleMeshSurfaceAdapter>> GetIntermediateWrapper() override
	{
		TUniquePtr<FSplineStaticMeshSourceDataSpatialWrapper> TempWrapper = MakeUnique<FSplineStaticMeshSourceDataSpatialWrapper>();
		TempWrapper->SourceContainer = SourceContainer;
		TempWrapper->StaticMesh = StaticMesh;
		TempWrapper->SplineMeshComponent = SplineMeshComponent;
		return TempWrapper;
	}
};


static TUniquePtr<IMeshSpatialWrapper> SpatialWrapperFactory( const FMeshTypeContainer& MeshContainer, const TArray<FActorChildMesh*>& ParentMeshes, const FMeshSceneAdapterBuildOptions& BuildOptions )
{
...
#endif
			const bool bIsSplineMesh = (ParentMeshes.Num() == 1 && ParentMeshes[0]->ComponentType == EActorMeshComponentType::SplineStaticMesh);
			if (bIsSplineMesh)
			{
				if (bUseRenderMeshes)
				{
					TUniquePtr<FSplineStaticMeshRenderDataSpatialWrapper> SplineSMWrapper = MakeUnique<FSplineStaticMeshRenderDataSpatialWrapper>();
					SplineSMWrapper->SourceContainer = MeshContainer;
					SplineSMWrapper->StaticMesh = StaticMesh;
					SplineSMWrapper->SplineMeshComponent = Cast<USplineMeshComponent>(ParentMeshes[0]->SourceComponent);
					return SplineSMWrapper;
				}
				else if (BuildOptions.bEnableUVQueries == false && BuildOptions.bEnableNormalsQueries == false)
				{
					TUniquePtr<FCompressedSplineStaticMeshSpatialWrapper> SplineSMWrapper = MakeUnique<FCompressedSplineStaticMeshSpatialWrapper>();
					SplineSMWrapper->SourceContainer = MeshContainer;
					SplineSMWrapper->StaticMesh = StaticMesh;
					SplineSMWrapper->SplineMeshComponent = Cast<USplineMeshComponent>(ParentMeshes[0]->SourceComponent);
					return SplineSMWrapper;
				...
				{
					TUniquePtr<FSplineStaticMeshSourceDataSpatialWrapper> SplineSMWrapper = MakeUnique<FSplineStaticMeshSourceDataSpatialWrapper>();
					SplineSMWrapper->SourceContainer = MeshContainer;
					SplineSMWrapper->StaticMesh = StaticMesh;
					SplineSMWrapper->SplineMeshComponent = Cast<USplineMeshComponent>(ParentMeshes[0]->SourceComponent);
					return SplineSMWrapper;
	...
	if (StaticMeshComponent != nullptr)
	{
		...
		if (Mesh != nullptr)
		{
			...
			else
			{
				...
				{
					ChildMesh->ComponentType = EActorMeshComponentType::SplineStaticMesh;
				}
				else
				{
...
{
	auto CreateNewWrapperInfo = [](TUniquePtr<FActorChildMesh>& ChildMesh)
	{
	...
	for (FActorAdapter* Actor : NewItemsToProcess)
	{
		for (TUniquePtr<FActorChildMesh>& ChildMesh : Actor->ChildMeshes)
		{
			...
			if (ChildMesh->ComponentType == EActorMeshComponentType::SplineStaticMesh)
			{
				TSharedPtr<FSpatialWrapperInfo> NewWrapperInfo = CreateNewWrapperInfo(ChildMesh);
				SplineMeshSpatialAdapters.Add(NewWrapperInfo);
...
void FMeshSceneAdapter::ForEachSpatialWrapper(TFunctionRef<void(TSharedPtr<FSpatialWrapperInfo>)> Func) const
{
	...
	for (TSharedPtr<FSpatialWrapperInfo> Adapter : SplineMeshSpatialAdapters)
	{
		Func(Adapter);
	}
}
...
{
	return !SpatialAdapters.IsEmpty() || !SplineMeshSpatialAdapters.IsEmpty();
}
...
void FMeshSceneAdapter::Build(const FMeshSceneAdapterBuildOptions& BuildOptions)
{
	...
	ForEachSpatialWrapper([&BuildOptions](TSharedPtr<FSpatialWrapperInfo> Adapter)
	{
		Adapter->SpatialWrapper = SpatialWrapperFactory(Adapter->SourceContainer, Adapter->ParentMeshes, BuildOptions);
		// populate the MeshSpatial members of the FActorChildMeshes
		for (FActorChildMesh* ParentMesh : Adapter->ParentMeshes)
		{
			ParentMesh->MeshSpatial = Adapter->SpatialWrapper.Get();
		}
	});
	...
	else
	{
		...
		ForEachSpatialWrapper([&ToBuild](TSharedPtr<FSpatialWrapperInfo> Adapter)
		{
			ToBuild.Add(Adapter.Get());
		});
...
void FMeshSceneAdapter::GetGeometryStatistics(FStatistics& StatsOut)
{
	...
	ForEachSpatialWrapper([&StatsOut](TSharedPtr<FSpatialWrapperInfo> Adapter)
	{
		StatsOut.UniqueMeshCount++;
		StatsOut.UniqueMeshTriangleCount += (int64)Adapter->SpatialWrapper->GetTriangleCount();
	});
...
void FMeshSceneAdapter::BuildSpatialEvaluationCache()
{
	...
	ForEachSpatialWrapper([](TSharedPtr<FSpatialWrapperInfo> Adapter)
	{
		FSpatialWrapperInfo& WrapperInfo = *Adapter;

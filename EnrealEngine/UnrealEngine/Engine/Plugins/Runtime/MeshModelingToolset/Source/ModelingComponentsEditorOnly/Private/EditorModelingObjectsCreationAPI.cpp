...
FCreateComponentResult UEditorModelingObjectsCreationAPI::CreateNewComponentOnActor(const FCreateComponentParams& CreateComponentParams)
{
	FCreateComponentParams LocalParams = CreateComponentParams;
	return CreateNewComponentOnActor(MoveTemp(LocalParams));		
}
...
FCreateActorResult UEditorModelingObjectsCreationAPI::CreateNewActor(FCreateActorParams&& CreateActorParams)
{
	...
	// If we don't have an asset template, we're going to just create a blank, empty actor instance.
	if (GEditor && !CreateActorParams.TemplateAsset)
	{
		...
		SpawnParams.Name = FName(CreateActorParams.BaseName);
		SpawnParams.NameMode = FActorSpawnParameters::ESpawnActorNameMode::Requested;
		NewActor = EmptyActorFactory->CreateActor(AssetData.GetAsset(),
			CreateActorParams.TargetWorld->GetCurrentLevel(),
			CreateActorParams.Transform,
			SpawnParams);
...
FCreateComponentResult UEditorModelingObjectsCreationAPI::CreateNewComponentOnActor(FCreateComponentParams&& CreateComponentParams)
{
	FCreateComponentResult Result;

	if (!ensure(CreateComponentParams.HostActor))
	{
		Result.ResultCode = ECreateModelingObjectResult::Failed_InvalidActor;
		return Result;
	}

	if (CreateComponentParams.bTransact)
	{
		CreateComponentParams.HostActor->Modify();
	}

	FName NewComponentName = *FComponentEditorUtils::GenerateValidVariableName(CreateComponentParams.ComponentClass, CreateComponentParams.HostActor);

	if (!CreateComponentParams.BaseName.IsEmpty())
	{
		NewComponentName = FName(CreateComponentParams.BaseName);
	}

	// Note that the RF_Transactional is important here for the spline to undo/redo properly in the future
	UActorComponent* Component = NewObject<UActorComponent>(CreateComponentParams.HostActor, CreateComponentParams.ComponentClass, NewComponentName,
		CreateComponentParams.bTransact ? RF_Transactional : RF_NoFlags);
	
	USceneComponent* SceneComponent = StaticCast<USceneComponent*>(Component);
	if (SceneComponent) {
		if (CreateComponentParams.bSetAsRoot)
		{
			CreateComponentParams.HostActor->SetRootComponent(SceneComponent);
		}
		else
		{
			SceneComponent->SetupAttachment(CreateComponentParams.HostActor->GetRootComponent());
		}
	}

	Component->OnComponentCreated();
	CreateComponentParams.HostActor->AddInstanceComponent(Component);
	Component->RegisterComponent();
	if (SceneComponent) {
		SceneComponent->ResetRelativeTransform();
	}
	CreateComponentParams.HostActor->PostEditChange();

	Result.ResultCode = ECreateModelingObjectResult::Ok;
	Result.NewComponent = Component;

	OnModelingComponentCreated.Broadcast(Result);

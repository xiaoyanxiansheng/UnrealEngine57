...
public:
	...
	UE_API FSubdividePoly(const FGroupTopology& InTopology,
				   const FDynamicMesh3& InOriginalMesh,
				   int InLevel);

	UE_API ~FSubdividePoly();
	...
	UE_API ETopologyCheckResult ValidateTopology();

	UE_API bool ComputeTopologySubdivision();

	UE_API bool ComputeSubdividedMesh(FDynamicMesh3& OutMesh);

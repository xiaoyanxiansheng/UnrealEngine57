	...
	FStaticMeshResults& ResultsOut)
{
	...
	int32 UseNumSourceModels = FMath::Clamp(Options.NumSourceModels, 1, MAX_STATIC_MESH_LODS);
	...
	// Nanite options
	NewStaticMesh->GetNaniteSettings().bEnabled = false;
	if (Options.bGenerateNaniteEnabledMesh)
	{
		NewStaticMesh->SetNaniteSettings(Options.NaniteSettings);
	}

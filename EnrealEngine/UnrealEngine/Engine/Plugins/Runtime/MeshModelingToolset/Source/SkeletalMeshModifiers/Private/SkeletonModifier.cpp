...
#include UE_INLINE_GENERATED_CPP_BY_NAME(SkeletonModifier)

#define LOCTEXT_NAMESPACE "SkeletonModifier"
...
namespace USkeletonModifierLocals
{
	...
	*/
	...
	/**
	* FSkeletonOnlyBones: helps track bones that are in the USkeleton ref skeleton but not in those of the skeletal mesh.
	* This is important because the bone tree must not be recreated in this situation, but rather merged.
	*/
	
	struct FSkeletonOnlyBones
	{
		FSkeletonOnlyBones(const USkeleton* InSkeleton)
			: Skeleton(InSkeleton)
		{
			const FReferenceSkeleton& RefSkeleton = Skeleton->GetReferenceSkeleton();
 
			SkeletonOnlyBones.Reserve(RefSkeleton.GetRawBoneNum());
			for (const FMeshBoneInfo& BoneInfo: RefSkeleton.GetRawRefBoneInfo())
			{
				SkeletonOnlyBones.Add(BoneInfo.Name);
			}
 
			for (const FVirtualBone& VB : InSkeleton->GetVirtualBones())
			{
				SkeletonOnlyBones.Remove(VB.SourceBoneName);
				SkeletonOnlyBones.Remove(VB.TargetBoneName);
			}
		}
		
		void Remove(const FReferenceSkeleton& InRefSkeleton)
		{
			for (const FMeshBoneInfo& BoneInfo: InRefSkeleton.GetRawRefBoneInfo())
			{
				SkeletonOnlyBones.Remove(BoneInfo.Name);
			}
			...
			for (int32 BoneIndex = RefSkeleton.GetRawBoneNum() - 1; BoneIndex >= 0; --BoneIndex)
			{
				...
				if (!SkeletonOnlyBones.Contains(BoneName))
				{
					const int32 ParentIndex = RefSkeleton.GetParentIndex(BoneIndex);
					if (ParentIndex != INDEX_NONE)
					{
						SkeletonOnlyBones.Remove(RefSkeleton.GetBoneName(ParentIndex));
					}
				}
			}
		}
		...
		{
			return SkeletonOnlyBones.Num();
		}
		
	private:
		const USkeleton* Skeleton;
		TArray<FName> SkeletonOnlyBones;
	};
}
...
void USkeletonModifier::ExternalUpdate(const FReferenceSkeleton& InRefSkeleton)
{
	if (!ReferenceSkeleton)
	{
		return;
	...
	*ReferenceSkeleton = InRefSkeleton;
	...
	int32 NumBones = InRefSkeleton.GetRawBoneNum();
	BoneIndexTracker.Reset(NumBones);
...
bool USkeletonModifier::SetDynamicMesh(UDynamicMesh* InDynamicMesh)
{
#if WITH_EDITORONLY_DATA
	...
	ReferenceSkeleton = MakeUnique<FReferenceSkeleton>();
	{
		const FDynamicMesh3& Mesh = InDynamicMesh->GetMeshRef();
		
		const int32 NumBones = Mesh.Attributes()->GetNumBones();
		FReferenceSkeletonModifier RefSkeletonModifier(*ReferenceSkeleton, nullptr);

		using namespace UE::Geometry;
		const FDynamicMeshBoneNameAttribute* BoneNameAttrib = Mesh.Attributes()->GetBoneNames();
		const FDynamicMeshBoneParentIndexAttribute* BoneParentIndexAttrib = Mesh.Attributes()->GetBoneParentIndices();
		const FDynamicMeshBonePoseAttribute* BonePoses = Mesh.Attributes()->GetBonePoses();

		for (int32 BoneIndex = 0; BoneIndex < NumBones; ++BoneIndex)
		{
			const int32 ParentBoneIndex = BoneParentIndexAttrib->GetValue(BoneIndex);
			FName BoneName = BoneNameAttrib->GetValue(BoneIndex);
			FMeshBoneInfo BoneInfo(BoneName, BoneName.ToString(), ParentBoneIndex);

			FTransform Transform = BonePoses->GetValue(BoneIndex);
...
bool USkeletonModifier::CommitSkeletonToDynamicMesh()
{
	if (!WeakDynamicMesh.IsValid())
	{
		return false;
	}
	
	UDynamicMesh* DynamicMesh = WeakDynamicMesh.Get();
	
	TSharedPtr<FDynamicMesh3> OldMeshShared = MakeShared<FDynamicMesh3>();
	DynamicMesh->ProcessMesh([&](const FDynamicMesh3& ReadMesh) { *OldMeshShared = ReadMesh; });
	...
	DynamicMesh->EditMesh([&](FDynamicMesh3& Mesh)
	{
		...
		using namespace UE::Geometry;
		Mesh.Attributes()->EnableBones(NumBones);
		
		FDynamicMeshBoneNameAttribute* BoneNameAttrib = Mesh.Attributes()->GetBoneNames();
		FDynamicMeshBoneParentIndexAttribute* BoneParentIndexAttrib = Mesh.Attributes()->GetBoneParentIndices();
		FDynamicMeshBonePoseAttribute* BonePoses = Mesh.Attributes()->GetBonePoses();

		for (int32 BoneIndex = 0; BoneIndex < NumBones; ++BoneIndex)
		{
			FName BoneName = ReferenceSkeleton->GetBoneName(BoneIndex);
			int32 ParentIndex = ReferenceSkeleton->GetParentIndex(BoneIndex);
			const FTransform& RefBonePose = ReferenceSkeleton->GetRefBonePose()[BoneIndex];

			BoneNameAttrib->SetValue(BoneIndex, BoneName);
			BoneParentIndexAttrib->SetValue(BoneIndex, ParentIndex);
			BonePoses->SetValue(BoneIndex, RefBonePose);
		}	
	});
	...
	DynamicMesh->EditMesh([&](FDynamicMesh3& Mesh)
	{
		...
		TArray<FName> SkinWeightProfiles;
		Mesh.Attributes()->GetSkinWeightsAttributes().GenerateKeyArray(SkinWeightProfiles);

		for (const FName SkinWeightProfile: SkinWeightProfiles)
		{
			FDynamicMeshVertexSkinWeightsAttribute* SkinWeights = Mesh.Attributes()->GetSkinWeightsAttribute(SkinWeightProfile);
			...
			for (int32 VertexID : Mesh.VertexIndicesItr())
			{
				...
				if (const int32 NumBoneWeights = BoneWeights.Num())
				{
					...
					SkinWeights->SetValue(VertexID, FBoneWeights::Create(NewWeights, BoneSettings));
				}
			}
		}	
	});
	...
	TUniquePtr<FMeshReplacementChange> ReplaceChange = MakeUnique<FMeshReplacementChange>(OldMeshShared, NewMeshShared);

	FScopedTransaction Transaction(LOCTEXT("SkeletonModifierCommitDynamicMesh", "Update Skeleton"));
	check(GUndo != nullptr);
	DynamicMesh->Modify();
...
bool USkeletonModifier::CommitSkeletonToSkeletalMesh()
{
	...
	const bool bHasSkeletonOnlyBones = [Modifications, SkeletonModifications, SkeletalMeshPtr = SkeletalMesh.Get()]()
	{
		if (SkeletonModifications == ESkeletonModificationType::SimpleMerge && Modifications == ESkeletalMeshModificationType::BonesAdded)
		{
			USkeletonModifierLocals::FSkeletonOnlyBones SkeletonOnlyBones(SkeletalMeshPtr->GetSkeleton());
			SkeletonOnlyBones.Remove(SkeletalMeshPtr->GetRefSkeleton());
			return SkeletonOnlyBones.NumBones() > 0;
		}
		return false;
	}();
	...
	if (EnumHasAnyFlags(SkeletonModifications, ESkeletonModificationType::DoUpdate))
	{
		...
		auto UpdateSkeleton = [this, &SkeletonModifications, &RetargetingModes, bHasSkeletonOnlyBones]()
		{
			USkeleton* Skeleton = SkeletalMesh->GetSkeleton();
			if (SkeletonModifications == ESkeletonModificationType::SimpleMerge)
			{
				constexpr bool bSkeletalMeshReferencesOnly = true;
				// merge the skeleton if there are several references or if the USkeleton's ref skeleton has bones that did not exist in the
				// non-modified ref skeleton.
				if (HasAnyOtherReferences(bSkeletalMeshReferencesOnly) || bHasSkeletonOnlyBones)
				{
...
void USkeletonModifier::EnsureSingleRoot()
{

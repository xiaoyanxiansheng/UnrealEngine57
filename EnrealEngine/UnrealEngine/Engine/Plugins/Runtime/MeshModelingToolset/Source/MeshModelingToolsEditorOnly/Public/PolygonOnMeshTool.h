...
public:
	...
	UE_API void PostAction(EPolygonOnMeshToolActions Action);
 ...
 */
UCLASS(MinimalAPI)
class UPolygonOnMeshTool : public USingleSelectionMeshEditingTool, public UE::Geometry::IDynamicMeshOperatorFactory, public IClickBehaviorTarget, public IHoverBehaviorTarget
{
	GENERATED_BODY()

public:

	UE_API UPolygonOnMeshTool();
	...
	UE_API virtual void RequestAction(EPolygonOnMeshToolActions ActionType);
...
protected:
	...
	UE_API void UpdatePolygonType();

	UE_API void SetupPreview();
	UE_API void UpdateDrawPlane();

	UE_API void BeginDrawPolygon();
	UE_API void CompleteDrawPolygon();

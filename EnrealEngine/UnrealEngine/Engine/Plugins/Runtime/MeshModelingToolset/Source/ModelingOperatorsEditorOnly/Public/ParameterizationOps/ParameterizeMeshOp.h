...
class FParameterizeMeshOp : public FDynamicMeshOperator
{
public:
...
protected:
	...
	UE_API bool ComputeUVs_UVAtlas(FDynamicMesh3& InOutMesh, TFunction<bool(float)>& Interrupter);
	UE_API bool ComputeUVs_XAtlas(FDynamicMesh3& InOutMesh, TFunction<bool(float)>& Interrupter);
	UE_API bool ComputeUVs_PatchBuilder(FDynamicMesh3& InOutMesh, FProgressCancel* Progress);

	UE_API void CopyNewUVsToMesh(
		FDynamicMesh3& Mesh,
	...
	UE_API void LayoutToUDIMByPolygroup(FDynamicMesh3& InOutMesh, UE::Geometry::FPolygroupSet& InputGroups);
};
 ...
 */
UCLASS(MinimalAPI)
class UParameterizeMeshOperatorFactory : public UObject, public UE::Geometry::IDynamicMeshOperatorFactory
{
	GENERATED_BODY()

...
void UDisplaceMeshTool::StartComputation()
{
	...
	ComputationStartTime = FPlatformTime::Seconds();
	AdvanceComputation();
}
...
void UDisplaceMeshTool::AdvanceComputation()
{
	...
	const float ElapsedTime = (float)(FPlatformTime::Seconds() - ComputationStartTime);
	if (ElapsedTime > 0.75f && DisplaceTask && !DisplaceTask->IsDone())
	{

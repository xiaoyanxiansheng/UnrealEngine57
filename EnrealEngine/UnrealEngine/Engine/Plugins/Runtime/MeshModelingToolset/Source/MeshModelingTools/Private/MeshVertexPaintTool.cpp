...
void UMeshVertexPaintTool::Setup()
{
	...
	TFuture<void> InitializeSymmetry = Async(VertexPaintToolAsyncExecTarget, [Mesh, this]()
	{
	...
	PolygroupLayerProperties->RestoreProperties(this, GetPropertyCacheIdentifier());
	...
	if (ensure(MeshElementsDisplay->Settings))
	{
		...
		MeshElementsDisplay->Settings->RestoreProperties(this, GetPropertyCacheIdentifier());
...
void UMeshVertexPaintTool::Shutdown(EToolShutdownType ShutdownType)
{
	...
	if (ensure(MeshElementsDisplay->Settings))
	{
		MeshElementsDisplay->Settings->SaveProperties(this, GetPropertyCacheIdentifier());
	}
	MeshElementsDisplay->Disconnect();

	BasicProperties->SaveProperties(this);
	FilterProperties->SaveProperties(this);
	PolygroupLayerProperties->SaveProperties(this, GetPropertyCacheIdentifier());
...
void UMeshVertexPaintTool::UpdateROI(const FSculptBrushStamp& BrushStamp)
{
	...
	// With Symmetry or Lazy Brush, GetBrushTriangleID() may not necessarily return a triangle that contains BrushPos,
	...
	if (bApplySymmetry || BrushProperties->Lazyness > 0)
	{
		double NearDistSqr;
	...
	auto FillROI = [this, bShouldFilterTrianglesBySelection, CheckEdgeCriteria, Mesh](TSet<int32>& ResultROI)
	{
		TArray<int32> StartROI;
		for (int32 tid : ResultROI)
		{
			StartROI.Add(tid);
		}
		FMeshConnectedComponents::GrowToConnectedTriangles(Mesh, StartROI, ResultROI, &TempROIBuffer,
														   [&](int t1, int t2)
		{
			if (bShouldFilterTrianglesBySelection && (!SelectionTids.Contains(t1) || !SelectionTids.Contains(t2)))
			{
	...
	auto GroupFillROI = [this, bShouldFilterTrianglesBySelection, CheckEdgeCriteria, Mesh](TSet<int>& ResultROI)
	{
		TArray<int32> StartROI;
	...
	auto PostProcessROI = [this, &FillROI, &GroupFillROI](TSet<int>& ResultROI)
	{
		if (BasicProperties->SubToolType == EMeshVertexPaintInteractionType::Fill)
		{
			FillROI(ResultROI);
		}
		else if (BasicProperties->SubToolType == EMeshVertexPaintInteractionType::GroupFill)
		{
			GroupFillROI(ResultROI);
		}
		...
		if (FilterProperties->VisibilityFilter != EMeshVertexPaintVisibilityType::None)
		{
			...
			ApplyVisibilityFilter(ResultROI, TempROIBuffer, ResultBuffer);
		}
	};
	...
	if (bApplySymmetry)
	{
		...
		for (int32 VertexID : SymmetricVertexROI)
		{
			if (Mesh->IsVertex(VertexID))
			{
				...
				{
					SymmetricTriangleROI.Add(tid);
				});
			}
		}

		PostProcessROI(SymmetricTriangleROI);

		// construct ROI triangle and group buffers
		SymmetricROITriangleBuffer.Reserve(SymmetricTriangleROI.Num());
		SymmetricROITriangleBuffer.SetNum(0, EAllowShrinking::No);
		for (int32 tid : SymmetricTriangleROI)
		{
			SymmetricROITriangleBuffer.Add(tid);
		}
	}

	PostProcessROI(TriangleROI);
...
bool UMeshVertexPaintTool::ApplyStamp()
{
	...
	// TODO: Unify the symmetry and main stamp passes.
	if (bApplySymmetry)
	{
		// convert triangle ROI to element IDs, populating ROIElementSet, ROIElementBuffer, ROIColorBuffer
		{
			TRACE_CPUPROFILER_EVENT_SCOPE(MeshVPaint_ApplyStamp_ElementROI);
			InitializeElementROIFromTriangleROI(SymmetricROITriangleBuffer, true);
		}

		// Temporarily update CurrentStamp to point to the negative side
		FFrame3d PosLocalFrame = CurrentStamp.LocalFrame;
		FFrame3d PosWorldFrame = CurrentStamp.WorldFrame;
		CurrentStamp.LocalFrame.Origin = Symmetry->GetMirroredPosition(CurrentStamp.LocalFrame.Origin);
		CurrentStamp.LocalFrame.Rotation = Symmetry->GetMirroredOrientation(CurrentStamp.LocalFrame.Rotation);
		CurrentStamp.WorldFrame = CurrentStamp.LocalFrame;
		CurrentStamp.WorldFrame.Transform(CurTargetTransform);
		...
		// Restore CurrentStamp
		CurrentStamp.LocalFrame = PosLocalFrame;
		CurrentStamp.WorldFrame = PosWorldFrame;

		bUpdated = SyncMeshWithColorBuffer(Mesh) || bUpdated;
	}
...
void UMeshVertexPaintTool::Render(IToolsContextRenderAPI* RenderAPI)
{
...
FString UMeshVertexPaintTool::GetPropertyCacheIdentifier() const
{
	return TEXT("MeshVertexPaintTool");
}


void UMeshVertexPaintTool::TryToInitializeSymmetry()
{

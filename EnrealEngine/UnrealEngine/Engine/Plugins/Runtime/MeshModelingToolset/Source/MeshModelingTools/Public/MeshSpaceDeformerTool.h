...
public:
	...
	UE_API void PostAction(EMeshSpaceDeformerToolAction Action);
...
UCLASS(MinimalAPI)
class USpaceDeformerOperatorFactory : public UObject, public UE::Geometry::IDynamicMeshOperatorFactory
{
	GENERATED_BODY()
 ...
 */
UCLASS(MinimalAPI)
class UMeshSpaceDeformerTool : public USingleSelectionMeshEditingTool, public IInteractiveToolManageGeometrySelectionAPI
{
	GENERATED_BODY()
public:
	UE_API UMeshSpaceDeformerTool();

	UE_API virtual void Setup() override;
	UE_API virtual void OnShutdown(EToolShutdownType ShutdownType) override;

	UE_API void RequestAction(EMeshSpaceDeformerToolAction ActionType);
	...
	UE_API void UpdateOpParameters(UE::Geometry::FMeshSpaceDeformerOp& MeshSpaceDeformerOp) const;
...
protected:	
	...
	UE_API void TransformProxyChanged(UTransformProxy* Proxy, FTransform Transform);
	UE_API void SetGizmoFrameFromWorldPos(const FVector& Position, const FVector& Normal = FVector(), bool bAlignNormal = false);
	
	UE_API double GetModifierGizmoValue() const;
	UE_API void ApplyModifierGizmoValue(double Value);

	// Apply clicked action
	UE_API void ApplyAction(EMeshSpaceDeformerToolAction Action);

	UE_API void UpdatePreview();

...
void SDisplayClusterConfiguratorViewOutputMapping::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	...
	FVector2f AbsoluteOrigin = MakeGeometryWindowLocal(RootGeometry).LocalToAbsolute(FVector2f::ZeroVector);
	FVector2f AbsoluteOriginWithOffset = AbsoluteOrigin - GraphEditorLocation * GraphEditorZoomAmount * DPIScale;

	TopRuler->SetRuling(FDeprecateSlateVector2D(AbsoluteOriginWithOffset), 1.f / RulerScale);
	SideRuler->SetRuling(FDeprecateSlateVector2D(AbsoluteOriginWithOffset), 1.f / RulerScale);
...
FText SDisplayClusterConfiguratorViewOutputMapping::GetCursorPositionText() const
{
	...
	FGeometry RootGeometry = PreviewSurface->GetCachedGeometry();
	const FVector2f CursorPos = RootGeometry.AbsoluteToLocal(FSlateApplication::Get().GetCursorPos());

	FVector2f GraphEditorLocation = FVector2f::ZeroVector;
	float GraphEditorZoomAmount = 0;
	GraphEditor->GetViewLocation(GraphEditorLocation, GraphEditorZoomAmount);

	FVector2f GraphPosition = ((CursorPos / GraphEditorZoomAmount) + GraphEditorLocation) / ViewScale;

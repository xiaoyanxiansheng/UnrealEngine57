...
bool FDisplayClusterConfiguratorBlueprintEditor::LoadFromFile(const FString& FilePath)
{
	...
	{
		UDisplayClusterBlueprint* const LoadedBlueprintPtr = LoadedBlueprint.Get();
		if (!IsValid(LoadedBlueprintPtr))
		{
			return false;
		}

		LoadedBlueprintPtr->SetConfigData(NewRootActor->GetConfigData(), true);
		UDisplayClusterConfigurationData* ConfigData = LoadedBlueprintPtr->GetConfig();
		check(ConfigData);
		ConfigData->ImportedPath = FilePath;

		FDisplayClusterConfiguratorUtils::AddRootActorComponentsToBlueprint(LoadedBlueprintPtr, NewRootActor, false);
		ClusterChanged();
		FKismetEditorUtilities::CompileBlueprint(LoadedBlueprintPtr);
		UpdatePreviewActor(LoadedBlueprintPtr, true);
		UpdateSubobjectPreview(true);
		SubobjectEditor->UpdateTree();
		LoadedBlueprintPtr->MarkPackageDirty();

...
const FString FPolicyParameterInfoVector::BaseVectorString = DisplayClusterTypesConverter::template ToString<FVector>(FVector::ZeroVector);

FPolicyParameterInfoVector::FPolicyParameterInfoVector(const FString& InDisplayName, const FString& InKey,
	UDisplayClusterBlueprint* InBlueprint,
	const TArray<TWeakObjectPtr<UDisplayClusterConfigurationViewport>>& InConfigurationViewports) :
	FPolicyParameterInfoFloatReference(InDisplayName, InKey, InBlueprint, InConfigurationViewports, &BaseVectorString),
	CachedVectorX(MakeShared<TOptional<float>>()),
	CachedVectorY(MakeShared<TOptional<float>>()),
	CachedVectorZ(MakeShared<TOptional<float>>())
{
	...
	if (!DoParametersMatchForAllViewports())
	{
		for (const TWeakObjectPtr<UDisplayClusterConfigurationViewport>& Viewport : ConfigurationViewports)
		{
			...
			const FText TextValue = FText::FromString(*ParameterValue);
			const FVector Location = DisplayClusterTypesConverter::template FromString<FVector>(TextValue.ToString());

			ResetOrSetCachedValue(CachedVectorX, Location.X);
			ResetOrSetCachedValue(CachedVectorY, Location.Y);
			ResetOrSetCachedValue(CachedVectorZ, Location.Z);
		}
	}
	else
	{
		...
		*CachedVectorX = Location.X;
		*CachedVectorY = Location.Y;
		*CachedVectorZ = Location.Z;
	}
}

void FPolicyParameterInfoVector::CreateCustomRowWidget(IDetailChildrenBuilder& InDetailWidgetRow)
{
	InDetailWidgetRow.AddCustomRow(GetParameterDisplayName())
		.Visibility(MakeAttributeRaw(this, &FPolicyParameterInfoVector::IsParameterVisible))
		.NameContent()
		[
			SNew(STextBlock)
				.Font(IDetailLayoutBuilder::GetDetailFont())
		...
		[
			SNew(SHorizontalBox)
				...
				.Padding(FMargin(0.0f, 2.0f, 3.0f, 2.0f))
				[
					MakeFloatInputWidget(CachedVectorX, LOCTEXT("LocationX", "X"), true, FLinearColor::White, SNumericEntryBox<float>::RedLabelBackgroundColor)
				]
			+ SHorizontalBox::Slot()
				.Padding(FMargin(0.0f, 2.0f, 3.0f, 2.0f))
				[
					MakeFloatInputWidget(CachedVectorY, LOCTEXT("LocationY", "Y"), true, FLinearColor::White, SNumericEntryBox<float>::GreenLabelBackgroundColor)
				]
			+ SHorizontalBox::Slot()
				.Padding(FMargin(0.0f, 2.0f, 3.0f, 2.0f))
				[
					MakeFloatInputWidget(CachedVectorZ, LOCTEXT("LocationZ", "Z"), true, FLinearColor::White, SNumericEntryBox<float>::BlueLabelBackgroundColor)
				]
		];
}

void FPolicyParameterInfoVector::FormatTextAndUpdateParameter()
{
	const bool bAllValuesSet = CachedVectorX->IsSet() && CachedVectorY->IsSet() && CachedVectorZ->IsSet();
	if (!bAllValuesSet)
	{
		return;
	}

	const FVector Location(*CachedVectorX.Get(), *CachedVectorY.Get(), *CachedVectorZ.Get());

	const FString LocationString = DisplayClusterTypesConverter::template ToString(Location);
	UpdateCustomParameterValueText(LocationString);
}

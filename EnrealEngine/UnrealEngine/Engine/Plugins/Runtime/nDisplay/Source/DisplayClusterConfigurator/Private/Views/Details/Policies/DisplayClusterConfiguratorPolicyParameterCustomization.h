...
/**
 * Policy info for vector.
 */
class FPolicyParameterInfoVector final : public FPolicyParameterInfoFloatReference
{
public:
	FPolicyParameterInfoVector(
		const FString& InDisplayName,
		const FString& InKey,
		UDisplayClusterBlueprint* InBlueprint,
		const TArray<TWeakObjectPtr<UDisplayClusterConfigurationViewport>>& InConfigurationViewports);
...
private:
	mutable TSharedRef<TOptional<float>> CachedVectorX;
	mutable TSharedRef<TOptional<float>> CachedVectorY;
	mutable TSharedRef<TOptional<float>> CachedVectorZ;

	static const FString BaseVectorString;
};

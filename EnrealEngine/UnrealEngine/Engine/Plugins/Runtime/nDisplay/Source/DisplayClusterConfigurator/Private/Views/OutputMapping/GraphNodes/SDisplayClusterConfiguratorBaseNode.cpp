...
void SDisplayClusterConfiguratorBaseNode::UpdateGraphNode()
{
	...
	GetOrAddSlot(ENodeZone::BottomRight)
		.SlotSize2f(FVector2f(ResizeHandleSize))
		.SlotOffset2f(TAttribute<FVector2f>(this, &SDisplayClusterConfiguratorBaseNode::GetReizeHandleOffset))
		.VAlign(VAlign_Top)
		.HAlign(HAlign_Left)
		.AllowScaling(false)
		[
...
void SDisplayClusterConfiguratorBaseNode::MoveTo(const FVector2f& NewPosition, FNodeSet& NodeFilter, bool bMarkDirty)
{
	if (!IsNodeUnlocked())
	{
	...
	FVector2f CurrentPosition = FVector2f(GraphNode->NodePosX, GraphNode->NodePosY);
	const FVector2f Offset = NewPosition - CurrentPosition;
	...
	FVector2D BestOffset = FVector2D(Offset);
	if (!bIsNodeFiltered)
	{
	...
	if (CanSnapAlign() && CanNodeBeSnapAligned() && !bIsNodeFiltered)
	{
		...
		AlignmentOffset = UE::Slate::CastToVector2f(Alignments.GetOffset());
	...
	SGraphNode::MoveTo(CurrentPosition + UE::Slate::CastToVector2f(BestOffset) + AlignmentOffset, NodeFilter, bMarkDirty);
...
bool SDisplayClusterConfiguratorBaseNode::CanBeSelected(const FVector2f& MousePositionInNode) const
{
...
TArray<FOverlayWidgetInfo> SDisplayClusterConfiguratorBaseNode::GetOverlayWidgets(bool bSelected, const FVector2f& WidgetSize) const
{
...
void SDisplayClusterConfiguratorBaseNode::SetNodeSize(const FVector2D InLocalSize, bool bFixedAspectRatio)
{
	...
	GraphNode->ResizeNode(UE::Slate::FDeprecateVector2DParameter(BestSize + AlignmentOffset));
...
FVector2f SDisplayClusterConfiguratorBaseNode::GetReizeHandleOffset() const
{
	const FVector2f NodeSize = UE::Slate::CastToVector2f(ComputeDesiredSize(FSlateApplication::Get().GetApplicationScale()));
	const float GraphZoom = GetOwnerPanel()->GetZoomAmount();
	return FVector2f(NodeSize.X, NodeSize.Y) * GraphZoom;
}
...
void SDisplayClusterConfiguratorBaseNode::AddAlignmentRulerToOverlay(TArray<FOverlayWidgetInfo>& OverlayWidgets, TSharedPtr<SAlignmentRuler> RulerWidget, const FAlignmentRulerTarget& Target, const FVector2f& WidgetSize) const
{
	...
	float RulerLength = 0;
	FVector2f RulerOffset = FVector2f::ZeroVector;
	...
	{
		const FVector2f ThisNodePosition = GetPosition2f();
		const FVector2f TargetNodePosition = UE::Slate::CastToVector2f(Target.TargetNode->GetNodePosition());
		const FVector2f TargetNodeSize = UE::Slate::CastToVector2f(Target.TargetNode->GetNodeSize());

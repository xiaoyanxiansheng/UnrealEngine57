...
void UDisplayClusterConfiguratorFactory::SetupNewBlueprint(UDisplayClusterBlueprint* NewBlueprint)
{
	...
	UDisplayClusterConfigurationData* NewConfigData = UDisplayClusterConfigurationData::CreateNewConfigData();

	if (ensure(NewConfigData && NewConfigData->Cluster))
	{
#if PLATFORM_WINDOWS
		// In Windows, the default sync policy should be Nvidia. Linux/other can be Ethernet by default, since Nvidia sync might not be supported.
		NewConfigData->Cluster->Sync.RenderSyncPolicy.Type = DisplayClusterConfigurationStrings::config::cluster::render_sync::Nvidia;
#endif // PLATFORM_WINDOWS

		// Decrease outer viewports screen percentage by default for new configurations
		NewConfigData->RenderFrameSettings.ClusterICVFXOuterViewportBufferRatioMult = 0.66f;
	}

	NewBlueprint->SetConfigData(NewConfigData);

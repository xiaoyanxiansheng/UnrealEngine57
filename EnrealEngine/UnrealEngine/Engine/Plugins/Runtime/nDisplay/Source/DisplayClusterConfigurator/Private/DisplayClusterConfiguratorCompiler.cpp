...
void FDisplayClusterConfiguratorKismetCompilerContext::ValidateConfiguration()
{
	...
	if (DCBlueprint && DCBlueprint->SimpleConstructionScript)
	{
		const TArray<USCS_Node*>& SCSNodes = Blueprint->SimpleConstructionScript->GetAllNodes();

		// Iterate over all SCS nodes
		for (const USCS_Node* const SCSNode : SCSNodes)
		{
			if (SCSNode && SCSNode->ComponentTemplate)
			{
				// Filter ICVFX camera components
				const UActorComponent* const ComponentTemplate = SCSNode->ComponentTemplate;
				if (const UDisplayClusterICVFXCameraComponent* const ICVFXCameraComponent = Cast<UDisplayClusterICVFXCameraComponent>(ComponentTemplate))
				{
					...
					{
						const int32 TilesCount = MediaSettings.TiledSplitLayout.X * MediaSettings.TiledSplitLayout.Y;
						if (TilesCount < 2)
						{
							MessageLog.Error(*LOCTEXT("MediaInvalidTilesLayout", "Camera @@ has invalid split layout.").ToString(), ICVFXCameraComponent);
						}
						else
						{
							for (int32 GroupIdx = 0; GroupIdx < MediaSettings.TiledMediaInputGroups.Num(); ++GroupIdx)
							{
								const FDisplayClusterConfigurationMediaTiledInputGroup& TiledInputGroup = MediaSettings.TiledMediaInputGroups[GroupIdx];
								if (TiledInputGroup.Tiles.Num() > TilesCount)
								{
									MessageLog.Error(*FText::Format(LOCTEXT("MediaInvalidTilesNum", "Camera @@ has invalid amount of tiles in the input group {0}."), FText::AsNumber(GroupIdx)).ToString(), ICVFXCameraComponent);
								}
							}
						}
					}

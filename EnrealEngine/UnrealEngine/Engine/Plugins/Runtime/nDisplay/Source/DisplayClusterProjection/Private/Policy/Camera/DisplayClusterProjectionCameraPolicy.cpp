...
EDisplayClusterProjectionPolicyFlags FDisplayClusterProjectionCameraPolicy::GetProjectionPolicyFlags(const IDisplayClusterViewport* InViewport, const uint32 InContextNum) const
{
	if (InViewport && EnumHasAnyFlags(InViewport->GetRenderSettingsICVFX().RuntimeFlags, EDisplayClusterViewportRuntimeICVFXFlags::InCamera))
	{
		// The ICVFX camera is always in the DCRA area and can follow the camera.
...
bool FDisplayClusterProjectionCameraPolicy::GetProjectionMatrix(IDisplayClusterViewport* InViewport, const uint32 InContextNum, FMatrix& OutPrjMatrix)
{
	...
	{
		// FReversedZPerspectiveMatrix() can't be used because it returns the wrong matrix when ZNear==ZFar (nDisplay encoding for infinite far clipping plane).
		OutPrjMatrix = IDisplayClusterViewport::MakeProjectionMatrix(-HalfFOVH, HalfFOVH, HalfFOVV, -HalfFOVV, ZNear, ZFar);
	}

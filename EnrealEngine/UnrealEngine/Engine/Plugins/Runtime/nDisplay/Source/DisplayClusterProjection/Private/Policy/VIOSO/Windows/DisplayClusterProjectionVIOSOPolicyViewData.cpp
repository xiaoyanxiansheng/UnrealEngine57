...
bool FDisplayClusterProjectionVIOSOPolicyViewData::RenderVIOSO_RenderThread(FRHICommandListImmediate& RHICmdList, FRHITexture* ShaderResourceTexture, FRHITexture* RenderTargetTexture)
{
	...
	if (IsViewDataValid())
	{
		...
		case ERenderDevice::D3D12:
		{
			...
			RHICmdList.Transition(FRHITransitionInfo(RenderTargetTexture, ERHIAccess::Unknown, ERHIAccess::RTV));
			RHICmdList.EnqueueLambda([ViosoViewData = SharedThis(this), RenderTargetTexture = RenderTargetTexture, ShaderResourceTexture= ShaderResourceTexture](FRHICommandList& ExecutingCmdList)
				{
					...
					ViosoViewData->WarperInterface->Render(&RenderInput, VWB_STATEMASK_ALL);
				});
			//experimental: add resource barrier
			RHICmdList.Transition(FRHITransitionInfo(RenderTargetTexture, ERHIAccess::RTV, ERHIAccess::Present));

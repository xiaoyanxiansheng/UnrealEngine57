...
bool FDisplayClusterProjectionEasyBlendPolicyConfiguration::Initialize(const TMap<FString, FString>& InParameters, IDisplayClusterViewport* InViewport)
{
	...
	// Origin offset
	if (DisplayClusterHelpers::map::template ExtractValueFromString(InParameters, DisplayClusterProjectionStrings::cfg::easyblend::OriginOffset, OriginOffset))
	{
		UE_LOG(LogDisplayClusterProjectionEasyBlend, Verbose, TEXT("Viewport <%s>: Projection parameter '%s' - %s"), *InViewportId, DisplayClusterProjectionStrings::cfg::easyblend::OriginOffset, *OriginOffset.ToString());
	}
	else
	{
		if (bEnableExtraLog)
		{
			UE_LOG(LogDisplayClusterProjectionEasyBlend, Log, TEXT("Viewport <%s>: Projection parameter '%s' not found"), *InViewportId, DisplayClusterProjectionStrings::cfg::easyblend::OriginOffset);
		}
	}

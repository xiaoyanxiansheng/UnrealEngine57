...
bool FDisplayClusterProjectionEasyBlendPolicyViewDataDX12::ApplyWarpBlend_RenderThread(FRHICommandListImmediate& RHICmdList, const FDisplayClusterProjectionEasyBlendPolicyViewInfo& InViewInfo, FRHITexture* InputTexture, FRHITexture* OutputTexture, FRHIViewport* InRHIViewport)
{
	...
	[ViewData = SharedThis(this), InputTexture = InputTexture, OutputTexture = OutputTexture, InViewLocation = InViewInfo.ViewLocation](FRHICommandList& ExecutingCmdList)
	{
		...
		// Ensure EasyBlend uses the same GPU device as the source texture to support multi-GPU configurations
		const uint32 DevIndex = D3D12DynamicRHI->RHIGetResourceDeviceIndex(InputTexture);
		ID3D12Device* D3D12Device = D3D12DynamicRHI->RHIGetDevice(DevIndex);
		if (!D3D12Device)
		{
			return;
		}
		...
		{
			const EasyBlendSDKError Result = EasyBlendAPI->EasyBlendSDK_InitializeDX12_Device(ViewData->EasyBlendMeshData.Get(), D3D12Device);
		...
		ID3D12GraphicsCommandList* D3D12GraphicsCommandList = D3D12DynamicRHI->RHIGetGraphicsCommandList(ExecutingCmdList, DevIndex);

		if (SrcTexture && DestTexture && D3D12GraphicsCommandList)
		{
			// Render EasyBlend on proxy
			const EasyBlendSDKError Result2 = EasyBlendAPI->EasyBlendSDK_TransformInputToOutputDX12_CommandList(ViewData->EasyBlendMeshData.Get(), SrcTexture, DestTexture, RTVHandle, D3D12GraphicsCommandList);
		}
	});
	RHICmdList.Transition(FRHITransitionInfo(OutputTexture, ERHIAccess::RTV, ERHIAccess::Present));

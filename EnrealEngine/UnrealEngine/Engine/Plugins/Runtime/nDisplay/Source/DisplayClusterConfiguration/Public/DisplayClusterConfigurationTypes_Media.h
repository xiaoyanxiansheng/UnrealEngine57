...
public:
	/** Media output synchronization policy */
	UE_DEPRECATED(5.6, "This property has been deprecated. Please refer new MediaOutputs property.")
	UPROPERTY(meta = (DeprecatedProperty, DeprecationMessage = "This property has been deprecated. Synchronization policy is bound to the actual media outputs."))
	TObjectPtr<UDisplayClusterMediaOutputSynchronizationPolicy> OutputSyncPolicy;
...
public:
	...
	/** Full-Frame Media Outputs */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Media")
	TArray<FDisplayClusterConfigurationMediaOutput> MediaOutputs;

	/** Split layout */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Media", meta = (ClampMin = 1, ClampMax = 4))
	FIntPoint TiledSplitLayout = { 1, 1 };

	/** Tile mapping. Maps tiles to media outputs. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Media")
	TArray<FDisplayClusterConfigurationMediaUniformTileOutput> TiledMediaOutputs;
...
public:
	...
	/** Allows the receviers to apply their own OCIO transformations (per node OCIO override). Requires the media to support PF_FloatRGBA pixel format if PQ transfer is not used. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Media", meta = (DisplayName = "Receivers Apply OCIO"))
	bool bLateOCIOPass = false;

	/** Use perceptual quantizer (PQ) transfer function to reduce bandwith. Requires PF_A2B10G10R10 pixel format configured on media side. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Media", meta = (DisplayName = "Use PQ-Encoding for Transfer", EditCondition = "bLateOCIOPass"))

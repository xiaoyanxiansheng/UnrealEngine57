...
{
	// Just see if any FullFrame media output instances are assigned
	bool bAnyMediaAssigned = MediaOutputs.ContainsByPredicate([](const FDisplayClusterConfigurationMediaOutput& Item)
		{
			return IsValid(Item.MediaOutput);
		});

	// If no FullFrame, let's see if there are any tiles configured
	if (!bAnyMediaAssigned)
	{
		bAnyMediaAssigned = TiledMediaOutputs.ContainsByPredicate([](const FDisplayClusterConfigurationMediaUniformTileOutput& Item)
			{
				return IsValid(Item.MediaOutput);
			});
	}

...
UDisplayClusterConfigurationClusterNode* UDisplayClusterConfigurationData::GetPrimaryNode() const
{
	if (Cluster)
	{
		return Cluster->GetNode(Cluster->PrimaryNode.Id);
	}

	return nullptr;
}

UDisplayClusterConfigurationClusterNode* UDisplayClusterConfigurationData::GetNode(const FString& NodeId) const
{
	if (Cluster)
	{
...
bool FDisplayClusterConfigurationProjection::ShouldDisplayICVFXCategory() const
{
	return Type.Equals(DisplayClusterProjectionStrings::projection::Mesh, ESearchCase::IgnoreCase)
		|| Type.Equals(DisplayClusterProjectionStrings::projection::MPCDI, ESearchCase::IgnoreCase)
		|| Type.Equals(DisplayClusterProjectionStrings::projection::Reference, ESearchCase::IgnoreCase);
}
...
void UDisplayClusterConfigurationCluster::PostLoad()
{
	Super::PostLoad();

#if WITH_EDITOR
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	if (Failover.FailoverPolicy == EDisplayClusterConfigurationFailoverPolicy::DropSecondaryNodesOnly)
	{
		Failover.bEnabled = true;
	}
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
#endif // WITH_EDITOR
}
...
FDisplayClusterConfigurationNetworkSettings::FDisplayClusterConfigurationNetworkSettings()
	...
	, ConnectRetryDelay        (1000) // 1 second between attempts
	, GameStartBarrierTimeout  (1000 * 60 * 60) // 1 hour
	, FrameStartBarrierTimeout (1000 * 60 * 30) // 30 minutes
	, FrameEndBarrierTimeout   (1000 * 60 * 30) // 30 minutes
	, RenderSyncBarrierTimeout (1000 * 60 * 30) // 30 minutes
{
}

FDisplayClusterConfigurationFailoverSettings::FDisplayClusterConfigurationFailoverSettings()
	: bEnabled(false)
{
}
...
#if WITH_EDITOR
void UDisplayClusterConfigurationData::PostEditChangeChainProperty(struct FPropertyChangedChainEvent& PropertyChangedEvent)
{
	auto ChainContainsProperty = [&PropertyChangedEvent](const FName& InPropertyName)
	{
		TDoubleLinkedList<FProperty*>::TDoubleLinkedListNode* CurrentNode = PropertyChangedEvent.PropertyChain.GetActiveNode();
		while (CurrentNode)
		{
			if (FProperty* Property = CurrentNode->GetValue())
			{
				if (Property->GetFName() == InPropertyName)
				{
					return  true;
				}
			}
	...
	if (PropertyName == GET_MEMBER_NAME_CHECKED(FDisplayClusterConfigurationUpscalerSettings, MethodName)
		&& ChainContainsProperty(GET_MEMBER_NAME_CHECKED(FDisplayClusterConfigurationICVFX_StageSettings, OuterViewportUpscalerSettings)))
	{
		Modify();
		
		// All viewport upscaling methods should match the global outer viewport upscaling method
		for (TTuple<FString, TObjectPtr<UDisplayClusterConfigurationClusterNode>>& Node : Cluster->Nodes)
		{
			for (TTuple<FString, TObjectPtr<UDisplayClusterConfigurationViewport>>& Viewport : Node.Value->Viewports)
			{
				Viewport.Value->Modify();
				
				Viewport.Value->RenderSettings.UpscalerSettings.MethodName = StageSettings.OuterViewportUpscalerSettings.MethodName;
				Viewport.Value->RenderSettings.UpscalerSettings.EditingData = StageSettings.OuterViewportUpscalerSettings.EditingData;
				Viewport.Value->RenderSettings.UpscalerSettings.ParameterOverrideGuids.Empty();
			}
		}
	}
	else if (ChainContainsProperty(GET_MEMBER_NAME_CHECKED(FDisplayClusterConfigurationUpscalerSettings, EditingData))
		&& ChainContainsProperty(GET_MEMBER_NAME_CHECKED(FDisplayClusterConfigurationICVFX_StageSettings, OuterViewportUpscalerSettings)))
	{
		Modify();
		
		// A property in the upscaler settings property bag has changed. Propagate the global property bag values to the viewports, considering viewport overrides
		for (TTuple<FString, TObjectPtr<UDisplayClusterConfigurationClusterNode>>& Node : Cluster->Nodes)
		{
			for (TTuple<FString, TObjectPtr<UDisplayClusterConfigurationViewport>>& Viewport : Node.Value->Viewports)
			{
				...
				Viewport.Value->RenderSettings.UpscalerSettings.EditingData.MigrateToNewBagInstanceWithOverrides(
					StageSettings.OuterViewportUpscalerSettings.EditingData,
					Viewport.Value->RenderSettings.UpscalerSettings.ParameterOverrideGuids);
			}
		}
	}

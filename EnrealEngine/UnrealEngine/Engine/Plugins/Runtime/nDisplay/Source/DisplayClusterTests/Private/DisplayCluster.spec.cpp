...
BEGIN_DEFINE_SPEC(FDisplayClusterSpec, "DisplayCluster", EAutomationTestFlags_ApplicationContextMask | EAutomationTestFlags::EngineFilter)
	...
	TWeakObjectPtr<UOpenColorIOConfiguration> OCIOConfig;
	TWeakObjectPtr<UOpenColorIOConfiguration> AltOCIOConfig;
	...
	FString OwnerName, const TArray<FName>& FieldNames, TFunction<void()> BeforeTestAction, bool bDoInstanceOverrideTest)
{
	...
	BeforeEach([this]()
	{
		...
		FAssetRegistryModule::AssetCreated(OCIOConfig.Get());
		...
		FAssetRegistryModule::AssetCreated(AltOCIOConfig.Get());
	});
	...
	{
		OCIO_CONFIG_TEST_INTERNAL(static_cast<UObject*>(OCIOConfig.Get()), static_cast<UObject*>(AltOCIOConfig.Get()),
	...
	AfterEach([this]()
	{
		DisplayClusterTestUtils::CleanUpAsset(OCIOConfig.Get());
		OCIOConfig.Reset();
		
		DisplayClusterTestUtils::CleanUpAsset(AltOCIOConfig.Get());
		AltOCIOConfig.Reset();
	});
}

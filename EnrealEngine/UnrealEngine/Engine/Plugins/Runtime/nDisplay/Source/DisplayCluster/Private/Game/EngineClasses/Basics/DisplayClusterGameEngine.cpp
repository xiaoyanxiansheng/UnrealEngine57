...
{
	// To prevent static analyzer of reporting unchecked pointers usage
	const IPDisplayCluster* const DCAPI = GDisplayCluster;
	if (!DCAPI)
	{
		UE_LOG(LogDisplayClusterEngine, Warning, TEXT("Could not initialize DC game engine, invalid GDisplayCluster"));
		return false;
	}

	// This function is called after a session had been started so it's safe to get config data from the config manager
	const UDisplayClusterConfigurationData* Config = DCAPI->GetPrivateConfigMgr()->GetConfig();
	check(Config);

	// Store diagnostics settings locally
	Diagnostics = Config->Diagnostics;

	// Listen for JSON sync events
	...
	DCAPI->GetPrivateClusterMgr()->AddClusterEventJsonListener(GameSyncTransition);

	// Enable/disable audio on this node
	const UDisplayClusterConfigurationClusterNode* CfgLocalNode = DCAPI->GetPrivateConfigMgr()->GetLocalNode();

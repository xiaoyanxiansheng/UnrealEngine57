...
namespace UE::DisplayCluster::ViewportPreviewMesh
{
	...
	/** Configure Holdout for the preview mesh
	* 
	* @param Configuration   - current configuration
	* @param InMeshComponent - Mesh for to be configured for Holdout
	* @param InMeshType      - Mesh type
	* @param InMaterialType  - Type of material on mesh
	*/
	static void UpdateHoldoutForMeshComponent(
		...
		const EDisplayClusterDisplayDeviceMaterialType InMaterialType = EDisplayClusterDisplayDeviceMaterialType::PreviewMeshMaterial)
	{
		if (!IsValid(InMeshComponent) || !UCompositeCoreSubsystem::IsProjectSettingsValid())
		{
			return;
		}

		UCompositeCoreSubsystem* CompositeSubsystem = UWorld::GetSubsystem<UCompositeCoreSubsystem>(Configuration.GetCurrentWorld());
		if (!IsValid(CompositeSubsystem))
		{
			return;
		}

		//By default, all materials must support Holdout.
		bool bUseHoldout = true;
		if (InMeshType == EDisplayClusterDisplayDeviceMeshType::DefaultMesh)
		{
			bUseHoldout = false;
		}

		if (bUseHoldout && Configuration.GetRenderFrameSettings().ShouldUseHoldout())
		{
			// Render this mesh using into default (holdout) composite pipeline
			CompositeSubsystem->RegisterPrimitive(InMeshComponent);
		}
		else
		{
			// Use normal rendering
			CompositeSubsystem->UnregisterPrimitive(InMeshComponent);
		}
	}
};
...
void FDisplayClusterViewportPreviewMesh::Update(FDisplayClusterViewport* InViewport, UDisplayClusterDisplayDeviceBaseComponent* InDisplayDeviceComponent, UDisplayClusterCameraComponent* ViewPointComponent)
{
	...
	if (UMeshComponent* MeshComponent = GetMeshComponent())
	{
		...
		// Apply a holdout to this mesh component if possible.
		UpdateHoldoutForMeshComponent(*Configuration, MeshComponent, MeshType, GetCurrentMaterialType());
	}
...
void FDisplayClusterViewportPreviewMesh::ReleaseMeshComponent(FDisplayClusterViewport* InViewport)
{
	...
	if (MeshComponent)
	{
		UpdateHoldoutForMeshComponent(*Configuration, MeshComponent, EDisplayClusterDisplayDeviceMeshType::DefaultMesh);

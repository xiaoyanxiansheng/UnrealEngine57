...
void FDisplayClusterViewportConfiguration_Viewport::PostUpdateViewportInstance(FDisplayClusterViewportConfiguration& InOutConfiguration)
{
	// Call the post-update event for the projection policy
	if (Viewport.IsValid() && Viewport->GetProjectionPolicy().IsValid())
	{
		Viewport->GetProjectionPolicy()->PostUpdateBaseConfiguration(Viewport.Get());
	}
}

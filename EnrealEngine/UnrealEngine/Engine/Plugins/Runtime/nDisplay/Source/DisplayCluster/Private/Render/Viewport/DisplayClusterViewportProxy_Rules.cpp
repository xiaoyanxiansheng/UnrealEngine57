...
bool FDisplayClusterViewportProxy::ShouldApplyWarpBlend_RenderThread() const
{
	...
	return ProjectionPolicy.IsValid() && ProjectionPolicy->IsWarpBlendSupported_RenderThread(this);
}
...
FIntRect FDisplayClusterViewportProxy::GetResourceRect_RenderThread(const EDisplayClusterViewportResourceType InResourceType, const uint32 InContextNum) const
{
	// Rect({0,0}, {0,0} means we are using the entire texture.
	FIntRect OutResourceRect = FIntRect(FIntPoint::ZeroValue, FIntPoint::ZeroValue);
	...
	if (Contexts.IsValidIndex(InContextNum))
	{
		...
		case EDisplayClusterViewportResourceType::InternalRenderTargetResource:
			if (PostRenderSettings.Replace.IsEnabled())
			{
				// Get external texture rect.
				OutResourceRect = PostRenderSettings.Replace.Rect;
			}
			else
			{
				// Get RTT rect
				OutResourceRect = Contexts[InContextNum].RenderTargetRect;

				// Get final rect
				if (InResourceType == EDisplayClusterViewportResourceType::InternalRenderTargetResource)
				{
					// Get the inner region from the overscan texture.
					if (OverscanRuntimeSettings.bIsEnabled && CVarDisplayClusterRenderOverscanResolve.GetValueOnRenderThread() != 0)
					{
						// Apply overscan crop
						OutResourceRect = OverscanRuntimeSettings.OverscanPixels.GetInnerRect(OutResourceRect);
					}
				}
			}
			break;
		...
		case EDisplayClusterViewportResourceType::AdditionalFrameTargetableResource:
			OutResourceRect = Contexts[InContextNum].FrameTargetRect;
			break;

		default:
			// All other resources use the same context size
			OutResourceRect.Max = Contexts[InContextNum].ContextSize;
			break;
		}
	}

	return OutResourceRect;
}
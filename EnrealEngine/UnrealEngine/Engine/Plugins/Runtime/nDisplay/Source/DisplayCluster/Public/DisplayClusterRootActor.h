...
private:
	/**
	* Prepares new preview settings.
	* (Access preview settings as read-only data; does not alter DCRA)
	*
	* @param OutPreviewSettings  Filled with the prepared settings.
	* @return True if preview rendering should proceed, false otherwise.
	*/
	bool PreparePreviewRendererSettings(FDisplayClusterViewport_PreviewSettings& OutPreviewSettings) const;

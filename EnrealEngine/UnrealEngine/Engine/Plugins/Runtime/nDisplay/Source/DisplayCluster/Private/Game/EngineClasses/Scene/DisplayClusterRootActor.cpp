...
bool ADisplayClusterRootActor::IsPrimaryRootActorForPIE() const
{
	...
	if (PreviewNodeId == DisplayClusterConfigurationStrings::gui::preview::PreviewNodeNone)
	{
		return false;
	}
...
{
	// [TEMPORARY FIX]
	// There are various ways we can get to this function while still having an uninitialized 'CurrentConfigData'.
	// Some of them have already been fixed, while others are still unclear how to reproduce.
	// UE-297696, UE-218690, UE-222703
	// Normally, CurrentConfigData should never be nullptr. As a quick work around, we always return
	// some default settings to avoid nullptr crash. Debug builds will keep reporting the issue.
	checkSlow(CurrentConfigData);
	ensureMsgf(IsValid(CurrentConfigData), TEXT("Found invalid DCRA configuration"));
	static const FDisplayClusterConfigurationICVFX_StageSettings DummyStageSettings;
	return IsValid(CurrentConfigData) ? CurrentConfigData->StageSettings : DummyStageSettings;
}
...
const FDisplayClusterConfigurationRenderFrame& ADisplayClusterRootActor::GetRenderFrameSettings() const 
{
	...
	checkSlow(CurrentConfigData);
	ensureMsgf(IsValid(CurrentConfigData), TEXT("Found invalid DCRA configuration"));
	static const FDisplayClusterConfigurationRenderFrame DummyRenderFrameSettings;
	return IsValid(CurrentConfigData) ? CurrentConfigData->RenderFrameSettings : DummyRenderFrameSettings;
}
...
bool ADisplayClusterRootActor::PreparePreviewRendererSettings(FDisplayClusterViewport_PreviewSettings& OutPreviewSettings) const
{
	const bool bIsPrimaryRootActor = IsPrimaryRootActor();
	const bool bIsRunningDisplayCluster = IsRunningDisplayCluster();
	...
	if (bIsRunningPIE)
	{
		...
		if (IsPrimaryRootActorForPIE())
		{
			...
			// Disable frustum rendering in PIE
			OutPreviewSettings.bPreviewICVFXFrustums = false;
		}
		else
		{
			// When running in a game or PIE, a special flag is required to show a preview for PIE/Standalone/Package
			bEnablePreviewInScene = OutPreviewSettings.bPreviewEnable && OutPreviewSettings.bPreviewInGameEnable;
		}
	}
	...
	if (bPreviewInGame && !OutPreviewSettings.bPreviewInGameRenderFrustum)
	{
	...
	if (!bEnablePreviewInScene)
	{
		...
		OutPreviewSettings.bPreviewEnable = false;
	}

	// To use the `HoldoutComposite` plugin, a post-process is required
	if (!OutPreviewSettings.bPreviewEnablePostProcess)
	{
		OutPreviewSettings.bPreviewEnableHoldoutComposite = false;
	}

	// The HoldoutComposite plugin requires special project settings.
	if (OutPreviewSettings.bPreviewEnableHoldoutComposite
		&& !UCompositeCoreSubsystem::IsProjectSettingsValid())
	{
		OutPreviewSettings.bPreviewEnableHoldoutComposite = false;

		// In this particular case, also disable the postprocess.
		OutPreviewSettings.bPreviewEnablePostProcess = false;
	}

	return bEnablePreviewInScene;
}

void ADisplayClusterRootActor::TickPreviewRenderer()
{
	FDisplayClusterViewport_PreviewSettings NewPreviewSettings;
	bool bEnablePreviewInScene = PreparePreviewRendererSettings(NewPreviewSettings);

	if (IDisplayClusterViewportManager* ViewportManager = bEnablePreviewInScene ? GetOrCreateViewportManager() : GetViewportManager())
	{
		// Disable preview rendering if this DCRA is under an exclusive lock.
		if (ViewportManager->GetConfiguration().IsExclusiveLocked())
		{
			// Disable preview in scene

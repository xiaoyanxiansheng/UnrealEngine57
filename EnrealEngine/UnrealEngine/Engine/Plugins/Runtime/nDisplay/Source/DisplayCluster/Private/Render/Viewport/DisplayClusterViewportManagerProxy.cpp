...
void FDisplayClusterViewportManagerProxy::UpdateFrameResources_RenderThread(FRHICommandListImmediate& RHICmdList) const
{
	...
	for (TSharedPtr<FDisplayClusterViewportProxy, ESPMode::ThreadSafe>& ViewportProxyIt : SortedViewportProxy)
	{
		...
		if (ViewportProxyIt.IsValid() && ViewportProxyIt->GetRenderSettings_RenderThread().bVisible)
		{
			...
			// It's possible that some viewports don't need to request scene rendering, but get their final textures
			// from another viewports. In such cases, no rendering callbacks like PostRenderViewFamily get called,
			// therefore we can't media-capture such viewports from normal pipeline. To allow them to be captured,
			// we explicitly trigger another callback for all viewports that are overridden.
			const bool bOverridden = ViewportProxyIt->GetRenderSettings_RenderThread().IsViewportOverridden();
			if (bOverridden)
			{
				IDisplayCluster::Get().GetCallbacks().OnDisplayClusterPostResolveOverridden_RenderThread().Broadcast(RHICmdList, ViewportProxyIt.Get());
			}
		}
	}
...
bool FDisplayClusterViewportManagerProxy::ImplResolveFrameTargetToBackBuffer_RenderThread(FRDGBuilder& GraphBuilder, const uint32 InContextNum, const int32 DestArrayIndex, FRDGTextureRef DestTexture, FVector2f WindowSize) const
{
	...
	if (DestArrayIndex > 0)
	{
		...
		if (!bStereoTexture)
		{
			if (!bHasLoggedBackbufferStereoUnsupported)
			{
				UE_LOG(LogDisplayClusterRender, Error, TEXT("Backbuffer does not support stereo rendering."));
				bHasLoggedBackbufferStereoUnsupported = 1;
			}
	...
	if (GetFrameTargets_RenderThread(FrameResources, TargetOffsets) && FrameResources.IsValidIndex(InContextNum))
	{
		...
		if (FRHITexture* FrameTexture = FrameResources[InContextNum])
		{
			...
			CopyInfo.Size = FIntVector(DestRect.Width(), DestRect.Height(), 1);

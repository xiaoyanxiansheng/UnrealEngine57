...
bool UDisplayClusterStageGeometryComponent::GetStageDistanceAndNormal(const FVector& InDirection, float& OutDistance, FVector& OutNormal)
{
	...
	{
		// Bias ever so slightly to the north to ensure all equator vertices are queried from the same map
		const bool bIsQueryingNorthernHemisphere = InDirection.Z > -0.001;

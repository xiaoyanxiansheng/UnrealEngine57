...
public:
	virtual void StartupModule() override
	{
		...
		ResetMultiUserSyncCommand = MakeUnique<FAutoConsoleCommand>(
			TEXT("nDisplay.MessageBusSync.ResetSyncFlags"),
			TEXT("Reset interceptor sync flags for Multi-user session connections."),
			FConsoleCommandDelegate::CreateRaw(this, &FDisplayClusterMessageInterceptionModule::ResetSyncFlags)
			);
	...
	virtual void ShutdownModule() override
	{
		...
		ResetMultiUserSyncCommand.Reset();
	}

private:
	void ResetSyncFlags()
	{
		// Reset the was ever disconnected flag to allow sync to happen again on when joining a session. 
		bWasEverDisconnected = false;
	}

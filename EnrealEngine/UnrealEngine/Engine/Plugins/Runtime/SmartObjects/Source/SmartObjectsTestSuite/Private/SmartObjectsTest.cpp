...
struct FFindAfterUseSmartObject : FSmartObjectTestBase
{
	virtual bool InstantTest() override
	{
		...
		AITEST_EQUAL("Number of registered smart objects", SOList.Num(), ExpectedNumRegisteredObjects);
...
struct FSlotCustomData : FSmartObjectTestBase
{
	virtual bool InstantTest() override
	{
		...
		FSmartObjectSlotHandle SlotHandleInView;
		const FSmartObjectSlotTestDefinitionData* DefinitionData = nullptr;
		const FSmartObjectSlotTestRuntimeData* RuntimeData = nullptr;

		const bool bSlotViewValid = Subsystem->ReadSlotData(FindResult.SlotHandle
		, [&SlotHandleInView, &DefinitionData, &RuntimeData](FConstSmartObjectSlotView SlotView)
		{
			SlotHandleInView = SlotView.GetSlotHandle();
			DefinitionData = SlotView.GetDefinitionDataPtr<FSmartObjectSlotTestDefinitionData>();
			RuntimeData = SlotView.GetStateDataPtr<FSmartObjectSlotTestRuntimeData>();
		});
		AITEST_TRUE("SlotView.IsValid()", bSlotViewValid);
		AITEST_TRUE("SlotView.SlotHandle.IsValid()", SlotHandleInView.IsValid());
		...
		FSmartObjectSlotTestRuntimeData OutRuntimeDataAfter;
		Subsystem->ReadSlotData(ClaimHandle.SlotHandle
		, [&OutRuntimeDataAfter, &bDataFound](const FConstSmartObjectSlotView SlotView)
		{
			if (const FSmartObjectSlotTestRuntimeData* Data = SlotView.GetStateDataPtr<FSmartObjectSlotTestRuntimeData>())
			{
				OutRuntimeDataAfter = *Data;
				bDataFound = true;
			}
		});

		AITEST_TRUE("Runtime data found", bDataFound);
		AITEST_EQUAL("Runtime data float from SlotView", OutRuntimeDataAfter.SomePerInstanceSharedFloat, SomeFloatConstant);

...
struct FSmartObjectSlotEntranceAnnotation : public FSmartObjectSlotAnnotation
{
	...
	UE_API virtual void AdjustWorldTransform(const FTransform& SlotTransform, const FVector& DeltaTranslation, const FRotator& DeltaRotation) override;
#endif
	
	virtual bool HasTransform() const override { return true; }
	UE_API virtual FTransform GetAnnotationWorldTransform(const FTransform& SlotTransform) const override;
	UE_API virtual FVector GetWorldLocation(const FTransform& SlotTransform) const;
	UE_API virtual FRotator GetWorldRotation(const FTransform& SlotTransform) const;

	/** @returns array of colliders that should free of collisions with world for this entrance to be used. */
	UE_API virtual void GetTrajectoryColliders(const FTransform& SlotTransform, TArray<FSmartObjectAnnotationCollider>& OutColliders) const;

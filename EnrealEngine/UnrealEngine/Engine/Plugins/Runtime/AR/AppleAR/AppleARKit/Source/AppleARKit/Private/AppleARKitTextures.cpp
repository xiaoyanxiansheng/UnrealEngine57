...
public:
	...
	void UpdateCameraImage(CVPixelBufferRef CameraImage, EPixelFormat PixelFormat, const CFStringRef ColorSpace, FImageBlurParams BlurParams)
	{
		...
		([this, CameraImage, PixelFormat, ColorSpace, BlurParams](FRHICommandListImmediate& RHICmdList)
		{
			UpdateCameraImage_RenderThread(RHICmdList, CameraImage, PixelFormat, ColorSpace, BlurParams);
	...
	void UpdateMetalTexture(id<MTLTexture> MetalTexture, EPixelFormat PixelFormat, const CFStringRef ColorSpace)
	{
		...
		{
			UpdateMetalTexture_RenderThread(RHICmdList, MetalTexture, PixelFormat, ColorSpace);
			CFRelease(MetalTexture);
		});
	}
	
protected:
	void ConditionalRecreateTexture(FRHICommandListImmediate& RHICmdList, uint32 Width, uint32 Height, EPixelFormat PixelFormat)
	{
		if (Size.X != Width || Size.Y != Height)
		{
			...
			RHICmdList.UpdateTextureReference(Owner->TextureReference.TextureReferenceRHI, nullptr);
	...
	void UpdateCameraImage_RenderThread(FRHICommandListImmediate& RHICmdList, CVPixelBufferRef CameraImage, EPixelFormat PixelFormat, const CFStringRef ColorSpace, FImageBlurParams BlurParams)
	{
		...
		ConditionalRecreateTexture(RHICmdList, ImageExtent.size.width, ImageExtent.size.height, PixelFormat);
	...
	void UpdateMetalTexture_RenderThread(FRHICommandListImmediate& RHICmdList, id<MTLTexture> MetalTexture, EPixelFormat PixelFormat, const CFStringRef ColorSpace)
	{
		SCOPED_AUTORELEASE_POOL;
		...
		{
			ConditionalRecreateTexture(RHICmdList, DesiredSize.X, DesiredSize.Y, PixelFormat);
		}
...
protected:
	void OnTextureUpdated(FRHICommandListBase& RHICmdList)
	{
		RHICmdList.BindDebugLabelName(TextureRHI, *Owner->GetName());
...
public:
	...
	virtual void InitRHI(FRHICommandListBase& RHICmdList) override
	{
		const FString TextureName = Owner->GetName();
		...
		if (MetalTexture != nullptr)
		{
			...
			const FRHITextureCreateDesc Desc =
				FRHITextureCreateDesc::CreateCube(*TextureName, Size.X, PF_B8G8R8A8)
				.SetFlags(ETextureCreateFlags::SRGB);

			EnvCubemapTextureRHIRef = RHICmdList.CreateTexture(Desc);
		...
		else
		{
			...
			const FRHITextureCreateDesc Desc = FRHITextureCreateDesc::CreateCube(*TextureName, Size.X, PF_B8G8R8A8);
...
public:
	...
	virtual void InitRHI(FRHICommandListBase& RHICmdList) override
	{
		...
		Size.X = Size.Y = 1;
		{
			...
			DecodedTextureRef = RHICmdList.CreateTexture(Desc);
		}
		
		UpdateTextureRHI(RHICmdList);
	}
	...
	void UpdateTextureRHI(FRHICommandListBase& RHICmdList)
	{
	...
	void UpdateVideoTexture(FRHICommandListImmediate& RHICmdList, CVMetalTextureRef CapturedYImage, FIntPoint CapturedYImageSize, CVMetalTextureRef CapturedCbCrImage, FIntPoint CapturedCbCrImageSize, EDeviceScreenOrientation DeviceOrientation)
	{
		...
		
		{
			...
			VideoTextureY = MetalDynamicRHI->RHICreateTexture2DFromCVMetalTexture(PF_B8G8R8A8, ETextureCreateFlags::NoTiling | ETextureCreateFlags::ShaderResource, FClearValueBinding::White, CapturedYImage);
			CFRelease(CapturedYImage);
	
			VideoTextureCbCr = MetalDynamicRHI->RHICreateTexture2DFromCVMetalTexture(PF_B8G8R8A8, ETextureCreateFlags::NoTiling | ETextureCreateFlags::ShaderResource, FClearValueBinding::White, CapturedCbCrImage);
			CFRelease(CapturedCbCrImage);

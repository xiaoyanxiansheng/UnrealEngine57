...
{
	static inline float ToUEScale()
	{
		return 100.f;
	}

	static inline float ToARKitScale()
	{
		return 0.01f;
	}
	 ...
	 */
	static inline FTransform ToFTransform(const matrix_float4x4& RawYUpMatrix, const FRotator& AdjustBy = FRotator::ZeroRotator)
	{
     ...
     * Ignores scale.
     */
    static inline matrix_float4x4 ToARKitMatrix( const FTransform& InTransform, float WorldToMetersScale = 100.0f )
    {
	 ...
	 * 'left handed' coordinate system.
	 */
	static inline FVector ToFVector(const vector_float3& RawYUpVector)
	{
     ...
     * 'left handed' coordinate system.
     */
    static inline vector_float3 ToARKitVector( const FVector& InFVector, float WorldToMetersScale = 100.0f )
    {
		if(!ensure(WorldToMetersScale != 0.f))
		{
			WorldToMetersScale = 100.f;
	...
	static inline FGuid ToFGuid( uuid_t UUID )
	{
	...
	static inline FGuid ToFGuid( NSUUID* Identifier )
	{
		// Get bytes
		uuid_t UUID;
	 ...
	 */
	static inline EARPlaneOrientation ToEARPlaneOrientation(ARPlaneAnchorAlignment Alignment)
	{
	...
	static inline EARObjectClassification ToEARObjectClassification(ARPlaneClassification Classification)
	{
	...
	static inline EARObjectClassification ToEARObjectClassification(ARMeshClassification Classification)
	{
		switch (Classification)
		{

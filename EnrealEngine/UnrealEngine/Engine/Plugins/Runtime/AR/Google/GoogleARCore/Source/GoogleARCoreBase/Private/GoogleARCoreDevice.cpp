...
#if PLATFORM_ANDROID
	UE_LOG(LogGoogleARCore, Display, TEXT("ARCore session using %s."), ShouldUseVulkan() ? TEXT("Vulkan") : TEXT("OpenGL ES"));
#endif
...
void FGoogleARCoreDevice::UpdateGameFrame(UWorld* World)
{
	...
	if (bIsARCoreSessionRunning)
	{
		...
		else if (ARCoreSession->HasNewFrame()) // Only update textures if the session has a new frame
		{
			if (auto Frame = ARCoreSession->GetLatestFrame())
			{
				// With Vulkan update the camera texture with the current frame's hardware buffer.
				if (ShouldUseVulkan())
				{
					const uint32 CurrentFrame = ARCoreSession->GetFrameNum() % PassthroughCameraTextures.Num();
					const uint32 CurrentCameraTextureId = PassthroughCameraTextureKeys[CurrentFrame];

					if (Frame->UpdateCameraTexture(PassthroughCameraTextures[CurrentCameraTextureId]))
					{
						LastCameraTextureId = CurrentCameraTextureId;
					}
				}
				// With OpenGL just use the texture id returned by the frame.
				else
				{
...
void FGoogleARCoreDevice::StartSession()
{
	...
	UE_LOG(LogGoogleARCore, Display, TEXT("SetCameraTextureIds %i textures"), TextureIds.Num());
...
void FGoogleARCoreDevice::AllocatePassthroughCameraTextures()
{
	...
	// Keep an array of the keys to be able circle through the textures frame by frame when using Vulkan.
	PassthroughCameraTextures.GetKeys(PassthroughCameraTextureKeys);
}
...
IYCbCrConversionQuery* FGoogleARCoreDevice::GetLastCameraYCbCrConversionQuery() const
{
	if (auto Record = PassthroughCameraTextures.Find(LastCameraTextureId))
	{
		return *Record;

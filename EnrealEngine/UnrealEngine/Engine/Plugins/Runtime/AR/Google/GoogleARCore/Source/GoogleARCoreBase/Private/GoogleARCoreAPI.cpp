...
DECLARE_CYCLE_STAT(TEXT("Update AR Frame - CameraImage"), STAT_UpdateCameraImage, STATGROUP_ARCore);
...
EGoogleARCoreAPIStatus FGoogleARCoreSession::ConfigSession(const UARSessionConfig& Config)
{
	...
	// Enabling mode to expose Hardware Buffer in Vulkan
	if (FAndroidMisc::ShouldUseVulkan())
	{
		ArConfig_setTextureUpdateMode(SessionHandle, ConfigHandle, AR_TEXTURE_UPDATE_MODE_EXPOSE_HARDWARE_BUFFER);
	}
...
EGoogleARCoreAPIStatus FGoogleARCoreSession::Update(float WorldToMeterScale)
{
	...
	// If the timestamp is the same then the frame contains the same data.
	bHasNewFrame = (LastFrameTimestamp != LatestFrame->GetCameraTimestamp());
...
void FGoogleARCoreSession::SetCameraTextureIds(const TArray<uint32_t>& TextureIds)
{
	...
	// Camera textures names are not used with Vulkan since it relies on getting Hardware Buffer instead.
	if (!FAndroidMisc::ShouldUseVulkan())
	{
...
bool FGoogleARCoreFrame::UpdateCameraTexture(UARCoreCameraTexture* InCameraTexture) const
{
#if PLATFORM_ANDROID
	SCOPE_CYCLE_COUNTER(STAT_UpdateCameraImage);

	if (!VulkanHardwareBuffer || !InCameraTexture)
	{
		return false;
	}

	InCameraTexture->UpdateCameraImage(VulkanHardwareBuffer);
	return true;
#else
	return false;
...
void FGoogleARCoreFrame::Update(float WorldToMeterScale)
{
	...
	
	{
		...
		// With Vulkan obtain the Hardware Buffer used by this frame.
		if (FAndroidMisc::ShouldUseVulkan())
		{
			ArFrame_getHardwareBuffer(SessionHandle, FrameHandle, &VulkanHardwareBuffer);
		}
		// With OpenGL obtain the texture Id used by this frame.
		else
		{

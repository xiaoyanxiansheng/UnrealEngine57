...
#if PLATFORM_ANDROID
	// Culling inversion ignored in Vulkan. Investigate when it's required and approach to apply it in Vulkan.
...
void FGoogleARCoreXRCamera::UpdateCameraYCbCrConversion(IYCbCrConversionQuery* NewYCbCrConversionQuery)
{
	if (PassthroughRenderer)
	{
		PassthroughRenderer->UpdateCameraYCbCrConversion(NewYCbCrConversionQuery);
	}
}

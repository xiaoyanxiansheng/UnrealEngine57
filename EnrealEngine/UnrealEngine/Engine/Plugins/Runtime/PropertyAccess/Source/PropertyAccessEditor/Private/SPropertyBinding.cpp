...
void SPropertyBinding::Construct(const FArguments& InArgs, UBlueprint* InBlueprint, const TArray<FBindingContextStruct>& InBindingContextStructs)
{
	...
	const bool bDisplayLinkIcon = Args.bUseLinkIconStyle && Args.bCanDisplayLinkIcon;
	TSharedRef<SWidget> LinkIcon = bDisplayLinkIcon ?
		SNew(SBox)
	...
	//Adjust padding to avoid non centered content and wasted space.
	const float TextLeftPadding = !bDisplayLinkIcon || Args.bLinkIconStyleCompactMode ? 0.0f : 4.0f;
...
{
	const bool bShouldExecuteCustomGenerateMenu = Args.OnGenerateMenu.IsBound() && (!Args.OnCanUseCustomGenerateMenu.IsBound() || Args.OnCanUseCustomGenerateMenu.Execute());
	if (bShouldExecuteCustomGenerateMenu)
	{
		return Args.OnGenerateMenu.Execute();
	}
...
{
	if (Args.bLinkIconStyleCompactMode)
	{
		return nullptr;
	}

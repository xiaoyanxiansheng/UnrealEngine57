...
namespace MRCaptureComponent_Impl
{
	 ...
	 *
	 */
	static void RemoveAllCVarBindings(FDelegateUserObjectConst BoundObject);
...
//------------------------------------------------------------------------------
static void MRCaptureComponent_Impl::RemoveAllCVarBindings(FDelegateUserObjectConst BoundObject)
{
...
//------------------------------------------------------------------------------
void UMixedRealityCaptureComponent::UpdateSceneCaptureContents(FSceneInterface* Scene, ISceneRenderBuilder& SceneRenderBuilder)
{ 
	if (TextureTarget)
	{
	...
	bool* bPreCommandQueuePtr = SceneRenderBuilder.GetAllocator().Malloc<bool>();

	SceneRenderBuilder.AddCommand([bPreCommandQueuePtr, ViewExtension = ViewExtension, Scene]
	{
		*bPreCommandQueuePtr = ViewExtension->SetupPreCapture(Scene);
	});

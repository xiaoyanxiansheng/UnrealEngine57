...
public:
	...
	static OPENCVHELPER_API void ConvertCoordinateSystem(FTransform& Transform, const EAxis DstXInSrcAxis, const EAxis DstYInSrcAxis, const EAxis DstZInSrcAxis);

	/** Converts in-place an FTransform in Unreal coordinates to OpenCV coordinates */
	static OPENCVHELPER_API void ConvertUnrealToOpenCV(FTransform& Transform);

	/** Converts in-place an FTransform in OpenCV coordinates to Unreal coordinates */
	static OPENCVHELPER_API void ConvertOpenCVToUnreal(FTransform& Transform);

	/** Converts an FVector in Unreal coordinates to OpenCV coordinates */
	static OPENCVHELPER_API FVector ConvertUnrealToOpenCV(const FVector& Transform);

	/** Converts an FTransform in OpenCV coordinates to Unreal coordinates */
	static OPENCVHELPER_API FVector ConvertOpenCVToUnreal(const FVector& Transform);
...
public:
	 ...
	 */
	static OPENCVHELPER_API UTexture2D* TextureFromCvMat(cv::Mat& Mat, const FString* PackagePath = nullptr, const FName* TextureName = nullptr);
	static OPENCVHELPER_API UTexture2D* TextureFromCvMat(cv::Mat& Mat, UTexture2D* InTexture);
	 ...
	 */
	static OPENCVHELPER_API void MakeCameraPoseFromObjectVectors(const cv::Mat& InRotation, const cv::Mat& InTranslation, FTransform& OutTransform);
	 ...
	 */
	static OPENCVHELPER_API void MakeObjectVectorsFromCameraPose(const FTransform& InTransform, cv::Mat& OutRotation, cv::Mat& OutTranslation);
#endif	// WITH_OPENCV
	...
	static OPENCVHELPER_API bool IdentifyArucoMarkers(TArray<FColor>& Image, FIntPoint ImageSize, EArucoDictionary DictionaryName, TArray<FArucoMarker>& OutMarkers);

	/** Draw a debug view of the input aruco markers on top of the input texture */
	static OPENCVHELPER_API bool DrawArucoMarkers(const TArray<FArucoMarker>& Markers, UTexture2D* DebugTexture);
	...
	static OPENCVHELPER_API bool IdentifyCheckerboard(TArray<FColor>& Image, FIntPoint ImageSize, FIntPoint CheckerboardDimensions, TArray<FVector2f>& OutCorners);
	...
	static OPENCVHELPER_API bool IdentifyCheckerboard(TArray<FColor>& Image, FIntPoint ImageSize, FIntRect RegionOfInterest, FIntPoint CheckerboardDimensions, TArray<FVector2f>& OutCorners);

	/** Draw a debug view of the input checkerboard corners on top of the input texture */
	static OPENCVHELPER_API bool DrawCheckerboardCorners(const TArray<FVector2f>& Corners, FIntPoint CheckerboardDimensions, UTexture2D* DebugTexture);

	/** Draw a debug view of the input checkerboard corners on top of the input texture */
	static OPENCVHELPER_API bool DrawCheckerboardCorners(const TArray<FVector2D>& Corners, FIntPoint CheckerboardDimensions, UTexture2D* DebugTexture);

	/** Compute the camera pose that minimizes the reprojection error of the input object points and image points */
	static OPENCVHELPER_API bool SolvePnP(const TArray<FVector>& ObjectPoints, const TArray<FVector2f>& ImagePoints, const FVector2D& FocalLength, const FVector2D& ImageCenter, const TArray<float>& DistortionParameters, FTransform& OutCameraPose);

	/** Project the input object points to the 2D image plane defined by the input camera intrinsics and camera pose */
	static OPENCVHELPER_API bool ProjectPoints(const TArray<FVector>& ObjectPoints, const FVector2D& FocalLength, const FVector2D& ImageCenter, const TArray<float>& DistortionParameters, const FTransform& CameraPose, TArray<FVector2f>& OutImagePoints);

	/** Project the input object points to the 2D image plane defined by the input camera intrinsics and camera pose */
	static OPENCVHELPER_API bool ProjectPoints(const TArray<FVector>& ObjectPoints, const FVector2D& FocalLength, const FVector2D& ImageCenter, const TArray<float>& DistortionParameters, const FTransform& CameraPose, TArray<FVector2D>& OutImagePoints);

	/** Find a 3D fit line that passes through the input points, as well as a point on that line */
	static OPENCVHELPER_API bool FitLine3D(const TArray<FVector>& InPoints, FVector& OutLine, FVector& OutPointOnLine);
	...
	static OPENCVHELPER_API double ComputeReprojectionError(const FTransform& CameraPose, const cv::Mat& CameraIntrinsicMatrix, const std::vector<cv::Point3f>& Points3d, const std::vector<cv::Point2f>& Points2d);
#endif
	...
	static OPENCVHELPER_API double ComputeReprojectionError(const TArray<FVector>& ObjectPoints, const TArray<FVector2f>& ImagePoints, const FVector2D& FocalLength, const FVector2D& ImageCenter, const FTransform& CameraPose);
};
...
public:
	...
	OPENCVHELPER_API cv::Mat ConvertToOpenCVDistortionCoefficients() const;

	/** Convert internal normalized camera matrix to OpenCV pixel scaled matrix representation. */
	OPENCVHELPER_API cv::Mat CreateOpenCVCameraMatrix(const FVector2D& InImageSize) const;
#endif //WITH_OPENCV

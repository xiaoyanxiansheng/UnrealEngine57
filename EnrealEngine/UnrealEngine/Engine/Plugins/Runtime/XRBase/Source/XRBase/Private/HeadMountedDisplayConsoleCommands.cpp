...
static TAutoConsoleVariable<float> CVarSecondaryScreenPercentageHMDRenderTarget(
	...
	100.0f,
	TEXT("Sets the XR render target texture size as a percentage of recommended texture size. Replaces r.SecondaryScreenPercentage.GameViewport when using an HMD.\n")
	TEXT("When using an HMD, secondary spatial scaling is redirected to be performed the HMD runtime by submitting a larger or smaller render target to the runtime.\n")
	TEXT("This saves time by combining secondary scaling with the HMD runtime's compositor's distortion pass, and allows performing secondary scaling even in configurations where it would otherwise be unsupported (e.g. Mobile LDR or Mobile HDR + Tonemap Subpass).\n")
	TEXT("The recommended texture size is the size that will result in no under-sampling in most distorted area of the view when computing the final image to be displayed on the device.\n")
...
static TAutoConsoleVariable<int32> CVarMobilePrimaryScalingMode(
	TEXT("xr.MobilePrimaryScalingMode"),
	1,
	TEXT("Controls whether mobile primary scaling is redirected to be performed by the HMD runtime when it would otherwise be unsupported.\n")
	TEXT("In mobile renderer configurations where normal post-processing is disabled (e.g. Mobile LDR or Mobile HDR + Tonemap Subpass), primary and secondary scaling are normally impossible to perform.\n")
	TEXT("However, when using an HMD, it is possible to redirect primary scaling to the HMD's runtime compositor by rendering the upper left of a fixed size render target and submitting the render target to the runtime along with its view rect.\n")
	TEXT("This approach only supports spatial upscaling (percentages <=100) and leaves some unused space in the RT, but it allows changing the resolution without reallocating the RT, making it suitable for dynamic resolution.\n")
	TEXT("0 - Never redirect primary scaling to the HMD runtime\n")
	TEXT("1 - Redirect primary scaling to the HMD runtime when it would otherwise be unsupported, and dynamic resolution is being used\n")
	TEXT("2 - Redirect primary scaling to the HMD runtime when it would otherwise be unsupported (not recommended, use xr.SecondaryScreenPercentage.HMDRenderTarget for fixed percentages)\n"),
	ECVF_Scalability | ECVF_Default);
static FAutoConsoleVariableDeprecated CVarMobileDynamicResolution_Dep(TEXT("xr.MobileLDRDynamicResolution"), TEXT("xr.MobilePrimaryScalingMode"), TEXT("5.7"));

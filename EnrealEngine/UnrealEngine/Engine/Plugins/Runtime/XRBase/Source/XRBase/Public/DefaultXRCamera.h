 ...
 */
class  FDefaultXRCamera : public IXRCamera, public FHMDSceneViewExtension
{
public:
	UE_API FDefaultXRCamera(const FAutoRegister&, IXRTrackingSystem* InTrackingSystem, int32 InDeviceId);
...
public:
	...
	*/
	 ...
	 */
	UE_API virtual void ApplyHMDRotation(APlayerController* PC, FRotator& ViewRotation) override;

	/**
	 * Apply the orientation and position of the headset to the Camera.
	 */
	UE_API virtual bool UpdatePlayerCamera(FQuat& CurrentOrientation, FVector& CurrentPosition, float DeltaTime) override;
	
	UE_API virtual void OverrideFOV(float& InOutFOV) override;

	/** Setup state for applying the render thread late update */
	UE_API virtual void SetupLateUpdate(const FTransform& ParentToWorld, USceneComponent* Component, bool bSkipLateUpdate) override;

	UE_API virtual void CalculateStereoCameraOffset(const int32 ViewIndex, FRotator& ViewRotation, FVector& ViewLocation) override;

	// ISceneViewExtension interface:
public:
	UE_API virtual void SetupViewFamily(FSceneViewFamily& InViewFamily) override;
	UE_API virtual void SetupView(FSceneViewFamily& InViewFamily, FSceneView& InView) override;
	UE_API virtual void BeginRenderViewFamily(FSceneViewFamily& InViewFamily) override;
	UE_API virtual void PreRenderView_RenderThread(FRDGBuilder& GraphBuilder, FSceneView& InView) override;
	UE_API virtual void PreRenderViewFamily_RenderThread(FRDGBuilder& GraphBuilder, FSceneViewFamily& InViewFamily) override;

	// FWorldSceneViewExtension interface:
	UE_API virtual bool IsActiveThisFrame_Internal(const FSceneViewExtensionContext& Context) const override;

...
public:
	...
	static UE_API FXRTrackingOriginChanged OnXRTrackingOriginChanged;

	DECLARE_MULTICAST_DELEGATE(FXRPlayAreaChanged);
	static UE_API FXRPlayAreaChanged OnXRPlayAreaChanged;

	DECLARE_MULTICAST_DELEGATE(FXRInteractionProfileChanged);
	static UE_API FXRInteractionProfileChanged OnXRInteractionProfileChanged;
};
...
{
public:
	UE_API FXRTrackingSystemBase(IARSystemSupport* InARImplementation);
	UE_API virtual ~FXRTrackingSystemBase();
	 ...
	 */
	UE_API virtual uint32 CountTrackedDevices(EXRTrackedDeviceType Type = EXRTrackedDeviceType::Any) override;
	 ...
	 */
	UE_API virtual bool IsTracking(int32 DeviceId) override;
	 ...
	 */
	UE_API virtual TSharedPtr< class IXRCamera, ESPMode::ThreadSafe > GetXRCamera(int32 DeviceId = HMDDeviceId) override;
	 ...
	 */
	UE_API virtual FTransform GetTrackingToWorldTransform() const override;
	 ...
	 */
	UE_API virtual void UpdateTrackingToWorldTransform(const FTransform& TrackingToWorldOverride) override;
	 ...
	 */
	UE_API virtual void CalibrateExternalTrackingSource(const FTransform& ExternalTrackingTransform) override;
	...
	* @param ExternalTrackingTransform		The transform in world-coordinates, of the reference marker of the external tracking system
	*/
	UE_API virtual void UpdateExternalTrackingPosition(const FTransform& ExternalTrackingTransform) override;
	UE_API virtual class IXRLoadingScreen* GetLoadingScreen() override final;

	UE_API virtual void GetMotionControllerState(UObject* WorldContext, const EXRSpaceType XRSpaceType, const EControllerHand Hand, const EXRControllerPoseType XRControllerPoseType, FXRMotionControllerState& MotionControllerState) override;
	UE_API virtual void GetHandTrackingState(UObject* WorldContext, const EXRSpaceType XRSpaceType, const EControllerHand Hand, FXRHandTrackingState& HandTrackingState) override;
	UE_API virtual bool GetCurrentInteractionProfile(const EControllerHand Hand, FString& InteractionProfile) override;

	UE_API TSharedPtr<FARSupportInterface , ESPMode::ThreadSafe> GetARCompositionComponent() override;
	UE_API const TSharedPtr<const FARSupportInterface , ESPMode::ThreadSafe> GetARCompositionComponent() const override;
...
protected:
	...
	UE_API virtual class IXRLoadingScreen* CreateLoadingScreen();
	 ...
	 */
	UE_API FTransform RefreshTrackingToWorldTransform(FWorldContext& WorldContext);
	UE_API FTransform ComputeTrackingToWorldTransform(FWorldContext& WorldContext) const;

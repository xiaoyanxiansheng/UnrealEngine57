...
void FDefaultXRLoadingScreen::AddReferencedObjects(FReferenceCollector& Collector)
{
	for (FSplashData& Data : Splashes)
	{
		Collector.AddReferencedObject(Data.Desc.TextureObj);
		Collector.AddReferencedObject(Data.Desc.LeftTextureObj);
	}
}
...
void FDefaultXRLoadingScreen::DoShowSplash(FSplashData& Splash)
{
	...
	if (Cast<UTextureCube>(Splash.Desc.TextureObj) != nullptr)
	{
	...
	LayerDesc.TextureObj = Splash.Desc.TextureObj;
	LayerDesc.LeftTextureObj = Splash.Desc.LeftTextureObj;

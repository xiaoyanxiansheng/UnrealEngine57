...
namespace UE::Geometry
{
	...
	/// Populate PathsOut with the intersected SubjPaths with ClipPolygons
	GEOMETRYALGORITHMS_API bool PathsIntersection(const TArray<TArrayView<FVector2d>>& Paths, TArrayView<const FGeneralPolygon2d> ClipPolygons, TArray<TArray<FVector2d>>& PathsOut);

	/// Populate PathsOut with the clipped SubjPaths with ClipPolygons
	GEOMETRYALGORITHMS_API bool PathsDifference(const TArray<TArrayView<FVector2d>>& Paths, TArrayView<const FGeneralPolygon2d> ClipPolygons, TArray<TArray<FVector2d>>& PathsOut);
}

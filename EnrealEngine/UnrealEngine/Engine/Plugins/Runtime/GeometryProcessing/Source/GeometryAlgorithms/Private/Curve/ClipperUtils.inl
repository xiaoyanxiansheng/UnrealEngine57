...
namespace UE::Geometry::Private
{
	...
	template<typename InputType, typename RealType>
	TArray<TVector2<RealType>> ConvertPathToUEPath(const Clipper2Lib::Path<InputType>& InPath, const TVector2<RealType>& InMin, const RealType& InRange)
	{
		TArray<TVector2<RealType>> Path;
		Path.Reserve(InPath.size());
		for (const Clipper2Lib::Point<InputType>& Point : InPath)
		{
			if constexpr (TIsIntegral<InputType>::Value)
			{
				Path.Add(UnpackVector<InputType, RealType>(Point, InMin, InRange));
			}
			else if constexpr (TIsFloatingPoint<InputType>::Value)
			{
				Path.Add({ static_cast<RealType>(Point.x), static_cast<RealType>(Point.y) });
			}
		}
	...
	template<typename InputType, typename RealType>
	TArray<TArray<TVector2<RealType>>> ConvertPathsToUEPaths(const Clipper2Lib::Paths<InputType>& InPaths, const TVector2<RealType>& InMin, const RealType& InRange)
	{
		TArray<TArray<TVector2<RealType>>> Paths;
		Paths.Reserve(InPaths.size());
		for (const Clipper2Lib::Path<InputType>& InPath : InPaths)
		{
			Paths.Add(ConvertPathToUEPath<InputType, RealType>(InPath, InMin, InRange));
		}

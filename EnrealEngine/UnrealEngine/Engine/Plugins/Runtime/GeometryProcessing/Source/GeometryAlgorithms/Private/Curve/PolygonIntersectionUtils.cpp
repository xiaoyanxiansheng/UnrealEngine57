...
namespace UE::Geometry::Private
{
	...
	// Clip paths in the subject array by the polygons in the clip array
	template <typename RealType>
	static bool ClipPaths(
		const Clipper2Lib::ClipType& InClipType,
		const TArray<TArrayView<FVector2D>>& SubjPaths,
		TArrayView<const FGeneralPolygon2d> ClipPolygons,
		TArray<TArray<FVector2D>>& ResultOut)
	{
		TAxisAlignedBox2<RealType> InputBounds;
		for (TArrayView<FVector2D> Path : SubjPaths)
		{
			InputBounds.Contain(Path);
		}
		for (const FGeneralPolygon2d& Polygon : ClipPolygons)
		{
			InputBounds.Contain(Polygon.GetOuter().Bounds());
		...
		Clipper.AddOpenSubject(ConvertPolygonsToPaths<RealType, IntegralType>(SubjPaths, InputBounds.Min, InputRange));
		...
		if (bExecuteResult)
		{
			ResultOut = ConvertPathsToUEPaths(ResultOpenPaths, InputBounds.Min, InputRange);
		}
...
bool PathsIntersection(const TArray<TArrayView<FVector2d>>& SubjPaths, TArrayView<const FGeneralPolygon2d> ClipPolygons, TArray<TArray<FVector2d>>& PathsOut)
{
	return Private::ClipPaths<double>(Clipper2Lib::ClipType::Intersection, SubjPaths, ClipPolygons, PathsOut);
}
bool PathsDifference(const TArray<TArrayView<FVector2d>>& SubjPaths, TArrayView<const FGeneralPolygon2d> ClipPolygons, TArray<TArray<FVector2d>>& PathsOut)
{
	return Private::ClipPaths<double>(Clipper2Lib::ClipType::Difference, SubjPaths, ClipPolygons, PathsOut);
}
} // namespace UE::Geometry

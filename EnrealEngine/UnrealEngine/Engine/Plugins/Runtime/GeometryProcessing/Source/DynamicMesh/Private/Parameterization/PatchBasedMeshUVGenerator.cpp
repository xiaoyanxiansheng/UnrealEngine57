...
bool FPatchBasedMeshUVGenerator::ComputeIslandsByRegionMerging(
	...
	FProgressCancel* Progress)
{
	return ComputeIslandsByRegionMerging(
		TargetMesh,
		TargetUVOverlay,
		ConnectedComponents,
		false,
	...
	FProgressCancel* Progress)
{
	...
	auto KeepRegion = [bOnlyOutputMergedRegions, &ConnectedComponents, &RegionGraph](int32 RID) -> bool
	{
		int32 KeepAboveTriCount = bOnlyOutputMergedRegions ? ConnectedComponents[RID].Indices.Num() : 0;
		return RegionGraph.GetRegionTriCount(RID) > KeepAboveTriCount;
	};

	int32 NumValidRegions = 0;
	for (int32 RID : RemainingRegions)
	{
		NumValidRegions += int32(KeepRegion(RID));
	}

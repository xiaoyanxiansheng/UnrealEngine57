...
namespace FMeshSimpleShapeApproximationLocals
{
	...
	{
		return Box.ReparameterizedCloserToWorldFrame();
	}
}
	...
	ParallelFor(SourceMeshes.Num(), [&](int32 idx)
	{
		...
		PreprocessOptions.CustomPreprocess = [this](FDynamicMesh3& ProcessMesh, const FAxisAlignedBox3d& Bounds) -> void
		{
			...
			if (bDecompositionPreSimplifyWithEdgeLength)
			{
				...
				SimplifyOptions.TargetEdgeLength = DecompositionPreSimplifyEdgeLength;
				// Only consider shape for the decomposition -- do not constrain seams, do not transfer attributes/groups
				SimplifyOptions.bTransferAttributes = false;
				SimplifyOptions.bTransferGroups = false;
				SimplifyOptions.PreserveEdges.SetSeamConstraints(MeshClusterSimplify::FSimplifyOptions::EConstraintLevel::Free);
				FDynamicMesh3 ResultMesh;
				MeshClusterSimplify::Simplify(ProcessMesh, ResultMesh, SimplifyOptions);
				ProcessMesh = ResultMesh;
			}
		};

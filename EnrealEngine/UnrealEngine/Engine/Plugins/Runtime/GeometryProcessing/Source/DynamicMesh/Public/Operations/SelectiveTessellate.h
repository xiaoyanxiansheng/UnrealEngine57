...
public:
	...
	/**
	* @return EOperationValidationResult::Ok if we can perform tessellation on the given mesh with this pattern
	*/
	virtual EOperationValidationResult Validate() const = 0;
...
public:
    ...
    struct FTessellationInformation
    {
		...
		TMap<int32, TArray<int32>>* TriangleIDToNewTriangleIDs = nullptr;
    };
...
public:
	...
	*/
	...
	static UE_API TUniquePtr<FTessellationPattern> CreateRedGreenTessellationPattern(const FDynamicMesh3* InMesh, const TArray<int>& InTriangleTessLevels);

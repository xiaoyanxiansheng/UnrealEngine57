...
bool FPolygroupsGenerator::FindPolygroupsFromUVIslands(int32 UVLayer)
{
	...
	return FindPolygroupsFromConnectedTris([&UV](int32 TriIdx0, int32 TriIdx1)
	{
...
bool FPolygroupsGenerator::FindPolygroupsFromHardNormalSeams()
{
	...
	return FindPolygroupsFromConnectedTris([Normals](int32 TriIdx0, int32 TriIdx1)
	{
		return Normals->AreTrianglesConnected(TriIdx0, TriIdx1);
	});
}

bool FPolygroupsGenerator::FindPolygroupsFromMaterialIDs()
{
	const FDynamicMeshMaterialAttribute* Materials = Mesh->Attributes()->GetMaterialID();
	if (!Materials)
	{
		return false;
	}

	return FindPolygroupsFromConnectedTris([Materials](int32 TriIdx0, int32 TriIdx1)
	{
		return Materials->GetValue(TriIdx0) == Materials->GetValue(TriIdx1);
	});
}

bool FPolygroupsGenerator::FindPolygroupsFromConnectedTris(TFunction<bool(int32, int32)> AreTrisConnected)
{
	FMeshConnectedComponents Components(Mesh);
	Components.FindConnectedTriangles(AreTrisConnected);

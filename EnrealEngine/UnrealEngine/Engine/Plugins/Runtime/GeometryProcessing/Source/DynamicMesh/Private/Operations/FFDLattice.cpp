...
void FFFDLattice::InitLattice(float Padding)
{
	...
	InitialBounds.Max += FVector3d(0.5 * ClampedPadding * MaxDiagonal);
	...
	for (int i = 0; i < Dimensions.X; ++i)
	{
		...
		for (int j = 0; j < Dimensions.Y; ++j)
		{
			...
			for (int k = 0; k < Dimensions.Z; ++k)
			{
				...
				OutLatticePositions[PointID] = Transform.TransformPosition(InitialBounds.Min + FVector3d{ X,Y,Z });
			}
		}
	}
}
...
{
	const FVector3d GridPoint = (QueryPoint - InitialBounds.Min) / CellSize;

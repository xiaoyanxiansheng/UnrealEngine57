 ...
 *
 * Each triangle in the mesh is assigned an initial value via the user-provided `ValueForTriangleFunc`.
 * Triangles that are on the boundary of the mesh or on the boundary of the specified `InsideTriangles` region are assigned a value of zero.
 * Triangle values are then adjusted within the region to ensure that each triangle's value does not exceed one more than the minimum value of its neighboring triangles.
 * This produces a smoothly varying integer field that respects the boundary constraints.
 * Note the field varies with topological distance (triangle connectivity), not distance along the surface or Euclidean distance
 *
 * @param TriangleROI          Triangles to consider
 * @param LevelForTriangleFunc Function that takes a triangle ID and returns the maximum value for that triangle
 * @param OutTriangleLevels    Output array of triangle values
 */
DYNAMICMESH_API void TaperPerTriangleValues(const UE::Geometry::FDynamicMesh3& Mesh,

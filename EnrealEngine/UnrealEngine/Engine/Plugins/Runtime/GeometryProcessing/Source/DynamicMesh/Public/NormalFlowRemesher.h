...
protected:
	...
	UE_API void RemeshWithFaceProjection();

	// Perform face-aligned projection onto the target mesh. Queue edges whose lengths change because of it.
	UE_API void TrackedFaceProjectionPass(double& MaxDistanceMoved, bool bIsTuningIteration);
	UE_API void TrackedFaceProjectionPass_Serial(double& MaxDistanceMoved, bool bIsTuningIteration);
	...
	*/
	UE_API bool EdgeFlipWouldReduceNormalError(int EdgeID, double BadEdgeErrorThreshold = 0.01, double ImprovementRatioThreshold = 0.75) const;

	/** Flip all edges for which EdgeFlipWouldReduceNormalError returns true */
	UE_API void TrackedEdgeFlipPass();
};

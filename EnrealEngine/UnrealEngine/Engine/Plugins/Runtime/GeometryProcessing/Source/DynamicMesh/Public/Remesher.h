...
public:
	...
	UE_API void SetTargetEdgeLength(double fLength);
	 ...
	 */
	UE_API virtual void Precompute();
	 ...
	 */
	UE_API virtual void BasicRemeshPass();
...
protected:
	...
	UE_API virtual EProcessResult ProcessEdge(int edgeID);
	...
	UE_API virtual void UpdateAfterSplit(int edgeID, int va, int vb, const FDynamicMesh3::FEdgeSplitInfo& splitInfo);
	...
	UE_API virtual void InitializeVertexBufferForPass();

	UE_API virtual void ApplyVertexBuffer(bool bParallel);
	...
	UE_API virtual void FullSmoothPass_Buffer(bool bParallel);

	// computes smoothed vertex position w/ proper constraints/etc. Does not modify mesh. 
	UE_API virtual FVector3d ComputeSmoothedVertexPos(int VertexID,
	...
	UE_API virtual void ApplyToSmoothVertices(const TFunction<void(int)>& VertexSmoothFunc);
	...
	UE_API virtual TFunction<FVector3d(const FDynamicMesh3&, int, double)> GetSmoothFunction();


	// Project vertices onto projection target.
	UE_API virtual void FullProjectionPass(bool bParallel);

	// Project a single vertex using the given target
	UE_API virtual void ProjectVertex(int VertexID, IProjectionTarget* UseTarget);

	// used by collapse-edge to get projected position for new vertex
	UE_API virtual FVector3d GetProjectedCollapsePosition(int VertexID, const FVector3d& vNewPos);

	// Apply the given projection function to all vertices in the mesh
	UE_API virtual void ApplyToProjectVertices(const TFunction<void(int)>& VertexProjectFunc);
	...
	UE_API void MoveVerticesParallel(TFunction<FVector3d(int, bool&)> NewVertexPosition);

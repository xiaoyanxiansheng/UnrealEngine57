	...
	{	
		...
		{	
			...
			for (const TTuple<FName, TUniquePtr<FDynamicMeshMorphTargetAttribute>>& AttributeInfo : InAttributes->GetMorphTargetAttributes())
			{
				...
				Generator = FVertexAttributeGenerator<float, 3>::CreateFromVertexAttribute(InMesh, TessellationNum, Progress, bUseParallel, AttributeInfo.Value.Get());
				if (Generator->Generate() == false)
				{
					return false;
				}
				...
				Generator->CopyToAttribute(MorphTargetAttribute);

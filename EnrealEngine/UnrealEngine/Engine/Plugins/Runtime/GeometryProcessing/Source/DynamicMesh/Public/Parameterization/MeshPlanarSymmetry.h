...
public:
	 ...
	 */
	UE_API bool Initialize(FDynamicMesh3* Mesh, FDynamicMeshAABBTree3* Spatial, FFrame3d SymmetryFrameIn);
	/**
	 * Given a Mesh and its bounding box, and a Symmetry Plane/Frame, detect any pairs of vertices with
	 * planar/mirror-symmetry relationships, as well as "on plane" vertices
	 * @return false if any non-on-plane vertex fails to find a match.
	 */
	UE_API bool Initialize(FDynamicMesh3* Mesh, const FAxisAlignedBox3d& Bounds, FFrame3d SymmetryFrameIn);
	 ...
	 * @return false if any non-on-plane vertex fails to find a match.
	 */
	UE_API bool FindPlaneAndInitialize(FDynamicMesh3* Mesh, const FAxisAlignedBox3d& Bounds, FFrame3d& SymmetryFrameOut, TArrayView<const FVector3d> PreferredNormals = TArrayView<const FVector3d>());

	/**
	 * @return the input Point mirrored across the Symmetry plane
	 */
	UE_API FVector3d GetMirroredPosition(const FVector3d& Position) const;

	/**
	 * @return the input Vector/Axis mirrored across the Symmetry plane
	 */
	UE_API FVector3d GetMirroredAxis(const FVector3d& Axis) const;

	/**
	 * @return the input Quaternion mirrored across the Symmetry plane
	 */
	UE_API FQuaterniond GetMirroredOrientation(const FQuaterniond& Orientation) const;

	/**
	 * @return the input Frame mirrored to the "positive" side (unchanged if it is already on the positive side)
	 */
	UE_API FFrame3d GetPositiveSideFrame(FFrame3d FromFrame) const;
	 ...
	 */
	UE_API void FullSymmetryUpdate();


	/** @return true if the TestMesh vertices match the current tracked vertex symmetry correspondence (i.e., the TestMesh vertex IDs match the known correspondence, and have the expected mirror symmetry) */
	UE_API bool ValidateSymmetry(const FDynamicMesh3& TestMesh) const;

	/** @return true if the TestMesh vertices in VertexROI match the current tracked vertex symmetry correspondence (i.e., the vertex IDs match the known correspondence, and have the expected mirror symmetry) */
	UE_API bool ValidateSymmetry(const FDynamicMesh3& TestMesh, TConstArrayView<int32> VertexROI) const;
	 ...
	 */
	UE_API void GetMirrorVertexROI(const TArray<int>& VertexROI, TArray<int>& MirrorVertexROIOut, bool bForceSameSizeWithGaps) const;
	 ...
	 */
	UE_API void ApplySymmetryPlaneConstraints(const TArray<int>& VertexIndices, TArray<FVector3d>& VertexPositionsInOut) const;
	 ...
	 */
	UE_API void ComputeSymmetryConstrainedPositions(
...
protected:
	...
	UE_API void UpdateSourceVertex(int32 VertexID);
	UE_API void UpdatePlaneVertex(int32 VertexID);

...
public:
	...
	UE_API FDirectedSegment GetSegment(int32 SID) const;

	/** @return the segment ID of the {first, last} segment in the path, or InvalidID if the path is empty */
	UE_API int32 GetHeadSegmentID() const;
	UE_API int32 GetTailSegmentID() const;
	...
	UE_API int32 GetPrevSegmentID(int32 SID) const;
	UE_API int32 GetNextSegmentID(int32 SID) const;

	/** @return segment ID of a new segment appended to the tail of the path */
	UE_API int32 AppendSegment(const FDirectedSegment& Segment);
	...
	UE_API int32 InsertSegmentBefore(const FDirectedSegment& DirectedSegment, int32 SegmentIDToInsertBefore = InvalidID);
	
	/** remove specified segment from path. Note SID is added to an internal free list and maybe reused  */
	UE_API void RemoveSegment(int32 SID);
...
public:
	...
	*/
	UE_API FDeformableEdgePath(const FDynamicMesh3& SurfaceMeshIn, const TArray<FEdgePath::FDirectedSegment>& OriginalPathAsDirectedSegments);
	...
	*/ 
	UE_API virtual void Minimize(FEdgePathDeformationInfo&  DeformedPathInfo, const int32 MaxNumIterations = TMathUtilConstants<int32>::MaxReal);
	...
	*/
	UE_API TArray<FSurfacePoint> AsSurfacePoints(double CoalesceThreshold = 0.) const;
...
protected:
	...
	*/
	UE_API void DeformJoint(FPathJoint& PathJoint);
	...
	*/ 
	UE_API bool DeformJoint( const int32 IncomingEID, const int32 OutgoingEID, int32 JointVID, 
	...
	*/ 
	UE_API bool IsJointFlexible(const FPathJoint& PathJoint) const;
	...
	*/ 
	UE_API void UpdateJointAndQueue(int32 OutgoingSegmentID);
	...
	*/ 
	UE_API void ReplaceJointWithPath(const FPathJoint PathJoint, const TArray<FEdgePath::FDirectedSegment>& PathAsDirectedEdges);
	...
	*       
	*/
	UE_API bool OuterArcFlipEdges(int32 StartEID, int32 EndEID, int32 CenterVID, bool bClockwise = false);
	...
	* It is assumed the specified edges do in fact have vertex CVID in common, but this is not checked.
	*/
	UE_API void ComputeWedgeAngles(int32 IncomingEID, int32 OutgoingEID, int32 CenterVID, 

	...
	TFunctionRef<FVector3d(const FVector3d&)> TransformFunc) const
{
	...
	auto ContainVertex = [MeshPtr = Mesh, &Bounds, TransformFunc](int VertexID)
	{
		if (MeshPtr->IsVertex(VertexID))
		{
			Bounds.Contain( TransformFunc(MeshPtr->GetVertex(VertexID)) );
		}		
	};

	for (int32 CornerID : Selection.SelectedCornerIDs)
	{
		if (Corners.IsValidIndex(CornerID))
		{
			ContainVertex(GetCornerVertexID(CornerID));
		}
	}
	...
	for (int32 GroupID : Selection.SelectedGroupIDs)
	{
		for (int32 TriangleID : GetGroupTriangles(GroupID))
		{
			if (Mesh->IsTriangle(TriangleID))
			{
				...
				ContainVertex(TriVerts.A);
				ContainVertex(TriVerts.B);
				ContainVertex(TriVerts.C);
			}
		}
	}

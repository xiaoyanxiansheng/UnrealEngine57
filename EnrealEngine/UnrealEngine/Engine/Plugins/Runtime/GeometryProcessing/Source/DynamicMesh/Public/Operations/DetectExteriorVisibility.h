...
struct FExteriorVisibilitySampling
{
	...
	// Compute per-triangle visibility array for a triangle mesh, with support for transparent (non-occluding) triangles, and with custom sampling directions (ignoring the NumSearchDirections parameter)
	// @param IsTriTransparent		Indicator function returns true if a triangle is 'transparent' (should not occlude, but can still be occluded)
	// @param OutTriOccluded		For each valid triangle ID, OutTriOccluded[ID] will be true if that triangle is hidden, false if it is visible
	// @param InitialViewTestDirections Initial directions to test for triangle visibility/occlusion. Note: directions are from mesh surface toward outside.
	//									Use this to specify a small set of directions (e.g., cardinal directions) to test per triangle before using the full set, which can help improve the algorithm performance.
	// @param FullViewTestDirections Full set of directions to test for triangle visibility/occlusion. Note: directions are from mesh surface toward outside
	// @param bAlsoSampleTriangleNormalDirections Whether to also sample in the normal direction for each triangle. Can be disabled for greater control over the sampling directions.
	// @param bPerPolyGroup			If true, visiblity will be determined on a per group basis: If any triangle in a group is visible, the whole group will be marked as not-occluded
	// @param SamplingParameters	Parameters controlling how exterior visiblity is tested
	// @param SkipTris				True for triangles do not need visibility testing (will be marked as not occluded). If empty, no triangles will be skipped.
	DYNAMICMESH_API static void ComputePerTriangleOcclusionFromDirections(const FDynamicMesh3& Mesh, TFunctionRef<bool(int32)> IsTriTransparent, TArray<bool>& OutTriOccluded, bool bPerPolyGroup, 
		TConstArrayView<FVector3d> InitialTestDirections, TConstArrayView<FVector3d> FullTestDirections, bool bAlsoSampleTriangleNormalDirections,
		const FExteriorVisibilitySampling& SamplingParameters,
		TArrayView<const bool> SkipTris = TArrayView<const bool>());

	DYNAMICMESH_API static TArray<FVector3d> GetRestrictedViewDirections(TConstArrayView<FVector3d> RestrictViewDirections, double RestrictViewAngle, int32 NumSphereSamples);
};

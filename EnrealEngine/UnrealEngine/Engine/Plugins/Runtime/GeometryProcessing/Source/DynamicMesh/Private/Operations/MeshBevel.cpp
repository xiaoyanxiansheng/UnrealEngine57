...
void FMeshBevel::BuildTerminatorVertex(FBevelVertex& Vertex, const FDynamicMesh3& Mesh)
{
	...
	// NOTE: code below follows polygroups on the mesh when available; otherwise, just tries to pick a 'furthest' edge
	...
	// Logic to find the RingSplitEdgeID (in its own control block just to clarify local variables logical scope)
	{
		...
		// If we didn't find other groups at all, then pick the outgoing edge
		// that is most aligned to the incoming edge
		if (OtherGroups.Num() == 0)
		{
			FIndex2i IncomingEdgeV = Mesh.GetEdgeV(IncomingEdgeID);
			int32 OtherVID = IncomingEdgeV.OtherElement(Vertex.VertexID);
			if (!ensure(OtherVID != IndexConstants::InvalidID))
			{
				// incoming edge is not attached to the expected vertex, fail out
				return;
			}
			FVector3d CenterPos = Mesh.GetVertex(Vertex.VertexID);
			FVector3d IncomingPos = Mesh.GetVertex(OtherVID);
			FVector3d IncomingDir = (CenterPos - IncomingPos);
			IncomingDir.Normalize();
			// Note: If the incoming edge is degenerate, this will just pick the first different edge.
			// We could instead try to use a different heuristic in this case.
			double BestAlignmentScore = -FMathd::MaxReal;
			Mesh.EnumerateVertexEdges(Vertex.VertexID, 
				[&BestAlignmentScore, &RingSplitEdgeID, &Mesh, &IncomingDir, &CenterPos, &Vertex, IncomingEdgeID]
				(int32 EID)
				{
					...
					FIndex2i OutEdgeV = Mesh.GetEdgeV(EID);
					int32 OutVID = OutEdgeV.OtherElement(Vertex.VertexID);
					if (ensure(OutVID != IndexConstants::InvalidID))
					{
						FVector3d OutgoingPos = Mesh.GetVertex(OutVID);
						FVector3d OutgoingDir = OutgoingPos - CenterPos;
						OutgoingDir.Normalize();
						double AlignmentScore = OutgoingDir.Dot(IncomingDir);
		...
		// If we found edges on other groups, use them to guide the split --
...
void FMeshBevel::AppendTerminatorVertexPairQuad(FDynamicMesh3& Mesh, FBevelVertex& Vertex0, FBevelVertex& Vertex1)
{
	...
	if (!MESH_BEVEL_DEBUG_ENSURE(
		Mesh.IsEdge(QuadEdgeID0) && Mesh.IsEdge(QuadEdgeID1) &&
		Mesh.IsBoundaryEdge(QuadEdgeID0) && Mesh.IsBoundaryEdge(QuadEdgeID1)))
	{
		// if we failed to find an open boundary on edge, we can't build the connecting quad
		return;
	}
...
void FMeshBevel::AppendLoopQuads(FDynamicMesh3& Mesh, FBevelLoop& Loop)
{
	...
	for (int32 k = 0; k < NumEdges; ++k)
	{
		...
		if (EdgeID0 != EdgeID1 && Mesh.IsEdge(EdgeID1))
		{
			...
			if (MESH_BEVEL_DEBUG_ENSURE(!EdgeV1.Contains(EdgeV0.B)))
			{
...
void FMeshBevel::AppendEdgeQuads_Multi(FDynamicMesh3& Mesh, FBevelEdge& Edge)
{
	...
	for (int32 k = 0; k < NumEdges; ++k)
	{
		for ( int32 j = 0; j < NumStrips; ++j)
		{
			...
			bSuccess &= (QuadTris.A >= 0 && QuadTris.B >= 0);
	...
	if (MESH_BEVEL_DEBUG_ENSURE(bSuccess))
	{
...
void FMeshBevel::AppendTerminatorVertexTriangles_Multi(FDynamicMesh3& Mesh, FBevelVertex& Vertex)
{
	...
	if (Mesh.IsEdge(RingSplitEdgeID))
	{
		...
		// If we failed to create a strip quad patch (due to failure earlier in the pipeline)
		// try to append a non-multi terminator instead
		if (IncomingEdge.StripQuadPatch.IsEmpty())
		{
			AppendTerminatorVertexTriangle(Mesh, Vertex);
			return;
		}
		...
		if (!MESH_BEVEL_DEBUG_ENSURE(ColumnIndex == 0 || ColumnIndex == IncomingEdge.StripQuadPatch.NumVertexCols() - 1))
		{
			return;
		}
		TArray<int32> QuadStripEdgeVerts;
		IncomingEdge.StripQuadPatch.GetVertexColumn(ColumnIndex, QuadStripEdgeVerts);
		if (!MESH_BEVEL_DEBUG_ENSURE(QuadStripEdgeVerts.Contains(WedgeVertexB)))
		{
			return;
		}
		...
		if (!MESH_BEVEL_DEBUG_ENSURE(FirstEdgeID != IndexConstants::InvalidID && Mesh.IsBoundaryEdge(FirstEdgeID)))
		{
			return;
		}
...
void FMeshBevel::AppendTerminatorVertexPairQuad_Multi(FDynamicMesh3& Mesh, FBevelVertex& Vertex0, FBevelVertex& Vertex1)
{
	...
	bool bFallbackToQuad = false, bInvalidInfo = false;
	auto CollectTerminatorVtxInfo = [this, &bFallbackToQuad, &bInvalidInfo](FDynamicMesh3& Mesh, FBevelVertex& Vertex, TArray<int32>& QuadStripEdgeVertsOut) -> void
	{
		int32 BevelEdgeIndex = Vertex.IncomingBevelEdgeIndices[0];
		const FBevelEdge& IncomingEdge = Edges[BevelEdgeIndex];
		if (IncomingEdge.StripQuadPatch.IsEmpty())
		{
			bFallbackToQuad = true;
			return;
		}

		int32 ColumnIndex = IncomingEdge.StripQuadPatch.FindColumnIndex(Vertex.Wedges[0].WedgeVertex);
		if (!(ColumnIndex == 0 || ColumnIndex == IncomingEdge.StripQuadPatch.NumVertexCols() - 1))
		{
			bInvalidInfo = true;
			return;
		}
		IncomingEdge.StripQuadPatch.GetVertexColumn(ColumnIndex, QuadStripEdgeVertsOut);
		if (!(QuadStripEdgeVertsOut.Contains(Vertex.Wedges[1].WedgeVertex)))
		{
			bInvalidInfo = true;
			return;
		}
	...
	if (bFallbackToQuad)
	{
		// no quad-strip fallback case
		AppendTerminatorVertexPairQuad(Mesh, Vertex0, Vertex1);
		return;
	}

...
public:
	 ...
	 */
	UE_API virtual ESolveStatus Solve(ScalarType& OutEigenValue, RealVectorType& OutEigenVector, bool bComputeLargest = false);
...
protected:
	...
	UE_API virtual bool Converged(ScalarType Lambda, const RealVectorType& EVector);
	
	/** Compute the eigenvalue corresponding to the given eigenvector using the Rayleigh Quotient: lambda = x'*A*x/x'*B*x */
	UE_API virtual ScalarType RayleighQuotient(const RealVectorType& NormalizedEVector) const;
	...
	UE_API virtual bool Iteration(RealVectorType& EVector, bool bComputeLargest);
	 ...
	 */
	UE_API virtual bool SetupSolver(bool bComputeLargest);

	/** Check that the input user parameters are valid and matrix operators are set */
	UE_API virtual bool VerifyUserParameters(bool bComputeLargest) const;
 ...
 */
class FSparsePowerMethod : public FPowerMethod
{
public:
     ...
     */
	static UE_API void CreateSparseMatrixOperator(const FSparseMatrixD& InMatrix, 

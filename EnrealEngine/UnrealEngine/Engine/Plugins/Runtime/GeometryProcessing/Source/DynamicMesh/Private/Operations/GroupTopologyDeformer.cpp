	...
	for (int fi = 0; fi < NumROIFaces; ++fi)
	{
		...
		for (int vi = 0; vi < NumInteriorV; ++vi)
		{
			...
			for (int k = 0; k < NumBoundaryV; ++k)
			{
				...
				double DeltaSquaredLength = DeltaVec.SquaredLength();
				if (DeltaSquaredLength == 0)
				{
					// This weight should be 1, and everything else should be zero.
					for (int32 i = 0; i < VtxEncoding.Weights.Num(); ++i)
					{
						VtxEncoding.Weights[i] = 0;
					}
					VtxEncoding.Weights[k] = 1;
					WeightSum = 1;
					break;
				}

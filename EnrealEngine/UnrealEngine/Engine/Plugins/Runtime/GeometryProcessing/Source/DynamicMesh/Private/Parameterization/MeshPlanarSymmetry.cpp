...
namespace MeshPlanarSymmetryLocals
{
	template<bool bSkipNonSourceVertices>
	inline bool ValidateVertexSymmetry(const FDynamicMesh3& TestMesh, int32 Idx, 
		const bool bOnPlane, const bool bIsSourceVertex, const int32 PairedVertex,
		const FVector3d& Origin, const FVector3d& Normal, double OnPlaneTolerance, double MatchVertexToleranceSq)
	{
		FVector3d Pos = TestMesh.GetVertex(Idx);
		if (bOnPlane)
		{
			const double SignedDistance = (Pos - Origin).Dot(Normal);
			if (FMath::Abs(SignedDistance) > OnPlaneTolerance)
			{
				return false;
			}
		}
		...
		{
			if constexpr (bSkipNonSourceVertices)
			{
				if (!bIsSourceVertex)
				{
					return true;
			...
			const int32 PairedVertexIdx = PairedVertex;
			if (!TestMesh.IsVertex(PairedVertexIdx))
			{
				return false;
			}
			FVector3d PairedPos = TestMesh.GetVertex(PairedVertexIdx);
			const double SignedDistance = (Pos - Origin).Dot(Normal);
			FVector3d MirrorPos = Pos - (2 * SignedDistance * Normal);
			if (FVector3d::DistSquared(MirrorPos, PairedPos) > MatchVertexToleranceSq)
			{
				return false;
			}
		}
...
bool FMeshPlanarSymmetry::ValidateSymmetry(const FDynamicMesh3& TestMesh) const
{
	if (TestMesh.MaxVertexID() != Vertices.Num())
	{
		return false;
	}

	const double OnPlaneTolerance = OnPlaneToleranceFactor * ErrorScale;
	const double MatchVertexToleranceSq = (MatchVertexToleranceFactor * ErrorScale) * (MatchVertexToleranceFactor * ErrorScale);

	for (int32 Idx = 0; Idx < Vertices.Num(); ++Idx)
	{
		if (!TestMesh.IsVertex(Idx))
		{
			continue;
		}
		const FSymmetryVertex& SV = Vertices[Idx];
		if (!MeshPlanarSymmetryLocals::ValidateVertexSymmetry<true>(TestMesh, Idx, SV.bOnPlane, SV.bIsSourceVertex, SV.PairedVertex, 
			SymmetryFrame.Origin, CachedSymmetryAxis, OnPlaneTolerance, MatchVertexToleranceSq))
		{
			return false;
		}
	}
...
bool FMeshPlanarSymmetry::ValidateSymmetry(const FDynamicMesh3& TestMesh, TConstArrayView<int32> VertexROI) const
{
	const double OnPlaneTolerance = OnPlaneToleranceFactor * ErrorScale;
	const double MatchVertexToleranceSq = (MatchVertexToleranceFactor * ErrorScale) * (MatchVertexToleranceFactor * ErrorScale);

	for (int32 Idx : VertexROI)
	{
		// All ROI vertices must be valid
		if (!TestMesh.IsVertex(Idx) || !Vertices.IsValidIndex(Idx))
		{
			return false;
		}
		const FSymmetryVertex& SV = Vertices[Idx];
		if (!MeshPlanarSymmetryLocals::ValidateVertexSymmetry<false>(TestMesh, Idx, SV.bOnPlane, SV.bIsSourceVertex, SV.PairedVertex, 
			SymmetryFrame.Origin, CachedSymmetryAxis, OnPlaneTolerance, MatchVertexToleranceSq))
		{
			return false;
		}

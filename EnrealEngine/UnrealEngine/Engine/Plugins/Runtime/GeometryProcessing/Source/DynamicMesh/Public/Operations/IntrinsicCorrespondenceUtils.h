...
namespace IntrinsicCorrespondenceUtils
{
	...
	struct FSurfacePoint
	{
		...
		UE_API FSurfacePoint(); // defaults to invalid vertex position
		UE_API FSurfacePoint(const int32 VID);
		UE_API FSurfacePoint(const int32 EdgeID, const double Alpha);
		UE_API FSurfacePoint(const int32 TriID, const FVector3d& BaryCentrics);
	...
	*/
	...
	struct FMeshConnection
	{
		...
		UE_API FMeshConnection(const FDynamicMesh3& SurfMesh);

		/** Reset the connection to the given (not owned) surface mesh*/
		UE_API void Reset(const FDynamicMesh3& SurfMesh);     

		/** Reset the connection to used a internally owned surface mesh*/
		UE_API void Reset(TUniquePtr<FDynamicMesh3>& SurfMesh);
	...
	*/
	struct FNormalCoordinates : public FMeshConnection
	{
		typedef FMeshConnection  MyBase;

		FNormalCoordinates(): MyBase() {}

		UE_API FNormalCoordinates(const FDynamicMesh3& SurfMesh);
		...
		*/
		UE_API bool  OnFlipEdge(const int32 T0ID, const FIndex3i& BeforeT0EIDs, const int32 T0OppVID,
		...
		*/
		UE_API int32 GetNthEdgeID(const int32 VID, const int32 N) const;
		...
		*/
		UE_API int32 GetEdgeOrder(const int32 VID, const int32 SurfaceEID) const;
	...
	protected:
		...
		UE_API void RebuildNormalCoordinates(const FDynamicMesh3& SurfMesh);
	};
	...
	* is initialized with the same connectivity as the surface mesh.  Note, it is assumed that the surface mesh has no bow-ties.
	*/
	struct FSignpost : public FMeshConnection
	{
		typedef FMeshConnection  MyBase;
		UE_API FSignpost(const FDynamicMesh3& SurfaceMesh);
		...
		*/
		UE_API void OnFlipEdge(const int32 EID, const FIndex2i AdjTris, const FIndex2i OpposingVerts, const FIndex2i PreFlipIndexOf, 

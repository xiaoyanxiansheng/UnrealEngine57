...
void MirrorAndAppendVerticesWithWelding(FDynamicMesh3& Mesh, const FVector3d& PlaneNormal, const FVector3d& PlaneOrigin,
	TArray<int32>& OutVidToMirrorMap, bool bAllowBowtieVertexCreation, double PlaneTolerance, EMeshMirrorNormalMode WeldNormalMode);
...
void FMeshMirror::MirrorAndAppend(FProgressCancel* Progress)
{
	...
	if (bWeldAlongPlane)
	{
		MirrorAndAppendVerticesWithWelding(*Mesh, PlaneNormal, PlaneOrigin, VidToMirrorMap,
			bAllowBowtieVertexCreation, PlaneTolerance, WeldNormalMode);
	}
	else
	{
	...
	TArray<int32>& OutVidToMirrorMap, bool bAllowBowtieVertexCreation, double PlaneTolerance, EMeshMirrorNormalMode WeldNormalMode)
{
	...
	const bool bHasAttributeNormals = Mesh.Attributes() && Mesh.Attributes()->PrimaryNormals();
	for (int32 Vid : VerticesOnPlane)
	{
		...
		else
		{
			...
			if (WeldNormalMode == EMeshMirrorNormalMode::AverageMirrorNormals)
			{
				auto ComputeVertexNormal = [&PlaneNormal](const FVector3f& OldNormal, FVector3f& NewNormal)
				{
					NewNormal = (OldNormal - OldNormal.Dot((FVector3f)PlaneNormal) * (FVector3f)PlaneNormal); // Not yet normalized
				...
				if (bHasAttributeNormals)
				{
					...
					NormalOverlay->EnumerateVertexElements(Vid,[NormalOverlay, &ComputeVertexNormal](int, int ElementId, const FVector3f& OldNormal)
					{
						FVector3f NewNormal;
						if (ComputeVertexNormal(OldNormal, NewNormal))
						{

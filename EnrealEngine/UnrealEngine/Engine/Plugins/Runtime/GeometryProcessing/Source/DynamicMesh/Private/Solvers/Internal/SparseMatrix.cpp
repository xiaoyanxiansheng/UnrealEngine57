                       ...
                       FSparseMatrixD& OutSlicedMatrix)
{
    ...
    const float NonZeroProbabilityPerEntry = static_cast<float>(NumNonZeros) / static_cast<float>(NumRows) / static_cast<float>(NumCols);	// ~= NumNonZeros / (NumRows*NumCols) but 
																																			// won't hit int32 overflow on large systems

    const float SlicedNumEntries = static_cast<float>(NumRowsSliced) * static_cast<float>(NumColsSliced);
    const int SlicedNonZeroEstimate = static_cast<int>(NonZeroProbabilityPerEntry * SlicedNumEntries);
	if (!ensure(SlicedNonZeroEstimate >= 0))
	{
		return false;
	}

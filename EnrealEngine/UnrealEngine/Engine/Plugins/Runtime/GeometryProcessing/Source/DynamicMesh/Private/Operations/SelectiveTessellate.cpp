...
{	
	...
	public:
		...
		{
			if (TrianglesOffsets.Contains(TriangleID))
			{
								  ...
								  FSelectiveTessellate::FTessellationInformation& TessInfo)
	{
		...
		for (const int TriangleID : Mesh->TriangleIndicesItr())
		{
			if (TessData.TrianglesToTessellate.Contains(TriangleID) == false)
			{
				...
				const int NewTriID = ResultMesh->AppendTriangle(MappedTri, TriGrp);
				CompactInfo.SetTriangleMapping(TriangleID, NewTriID);

				if (TessInfo.TriangleIDToNewTriangleIDs)
				{
					TessInfo.TriangleIDToNewTriangleIDs->FindOrAdd(TriangleID).Add(NewTriID);
				}
			}
		}
		...
		for (const int TriangleID : TessData.TrianglesToTessellate) 
		{
			...
			for (int Index = 0; Index < TriangleIDBlock.Num(); ++Index)
			{
				...
				const FIndex3i MappedTri = CompactInfo.GetVertexMapping(Tri);
				const int MappedTriId = ResultMesh->AppendTriangle(MappedTri, TriGrp);
				CompactInfo.SetTriangleMapping(NewTriangleID, MappedTriId);

				if (TessInfo.TriangleIDToNewTriangleIDs)
				{
					TessInfo.TriangleIDToNewTriangleIDs->FindOrAdd(TriangleID).Add(MappedTriId);
				}
			}	 
		}
					...
					FSelectiveTessellate::FTessellationInformation& TessInfo)
	{
		...
		{	
			...
			for (const TTuple<FName, TUniquePtr<FDynamicMeshMorphTargetAttribute>>& AttributeInfo : InAttributes->GetMorphTargetAttributes())
			{
				FDynamicMeshMorphTargetAttribute* MorphTargetAttribute = new FDynamicMeshMorphTargetAttribute(OutMesh);
				ConstructDynamicMeshVertexAttribute<float, 3>(InMesh, AttributeInfo.Value.Get(), TessData, bUseParallel, CompactInfo, MorphTargetAttribute);
				MorphTargetAttribute->SetName(AttributeInfo.Value.Get()->GetName());
				OutAttributes->AttachMorphTargetAttribute(AttributeInfo.Key, MorphTargetAttribute);
			}
...
TUniquePtr<FTessellationPattern> FSelectiveTessellate::CreateRedGreenTessellationPattern(const FDynamicMesh3* InMesh, const TArray<int>& InTriangleTessLevels)
{
	TUniquePtr<FRedGreenTessellationPattern> Pattern = MakeUnique<FRedGreenTessellationPattern>(InMesh, InTriangleTessLevels);
	return Pattern;
}

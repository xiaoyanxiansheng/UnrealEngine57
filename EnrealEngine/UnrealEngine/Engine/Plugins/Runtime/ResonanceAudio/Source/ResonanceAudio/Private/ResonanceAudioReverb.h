...
public:
	...
	UE_API void SetEnableRoomEffects(bool bInEnableRoomEffects);

	// Sets room position in 3D space
	UFUNCTION(BlueprintCallable, Category = "Resonance Audio Room Effect Settings")
	UE_API void SetRoomPosition(const FVector& InPosition);

	// Sets room rotation in 3D space
	UFUNCTION(BlueprintCallable, Category = "Resonance Audio Room Effect Settings")
	UE_API void SetRoomRotation(const FQuat& InRotation);

	// Sets room dimensions in 3D space
	UFUNCTION(BlueprintCallable, Category = "Resonance Audio Room Effect Settings")
	UE_API void SetRoomDimensions(const FVector& InDimensions);

	// Sets Resonance Audio room's acoustic materials
	UFUNCTION(BlueprintCallable, Category = "Resonance Audio Room Effect Settings")
	UE_API void SetRoomMaterials(const TArray<ERaMaterialName>& InMaterials);

	// Sets early reflections gain multiplier
	UFUNCTION(BlueprintCallable, Category = "Resonance Audio Room Effect Settings")
	UE_API void SetReflectionScalar(float InReflectionScalar);

	// Sets reverb gain multiplier
	UFUNCTION(BlueprintCallable, Category = "Resonance Audio Room Effect Settings")
	UE_API void SetReverbGain(float InReverbGain);
	...
	UFUNCTION(BlueprintCallable, Category = "Resonance Audio Room Effect Settings")
	UE_API void SetReverbTimeModifier(float InReverbTimeModifier);

	// Increases high frequency reverberation times when positive, decreases when negative.
	// Has no effect when set to 0.0
	UFUNCTION(BlueprintCallable, Category = "Resonance Audio Room Effect Settings")
	UE_API void SetReverbBrightness(float InReverbBrightness);
};

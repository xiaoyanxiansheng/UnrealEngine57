...
struct FZoneGraphAStarWrapper
{
	...
	inline bool IsValidRef(const FNodeRef NodeRef) const
	{
...
struct FZoneGraphAStarNode : public FGraphAStarDefaultNode<FZoneGraphAStarWrapper>
{
	...
	inline FZoneGraphAStarNode(const FNodeRef InNodeRef = INDEX_NONE, const FVector InPosition = FVector(TNumericLimits<FVector::FReal>::Max()))
		: Super(InNodeRef)
		, Position(InPosition)
	{}
...
struct FZoneGraphPathFilter
{
	...
	inline FVector::FReal GetHeuristicScale() const { return 1.; }
	...
	inline bool IsTraversalAllowed(const FNodeRef StartNodeRef, const FNodeRef& Neighbour) const
	{
		const FZoneGraphTagMask& LaneTagMask = ZoneStorage.Lanes[Neighbour].Tags;
		return ZoneTagFilter.Pass(LaneTagMask);
	}

	inline bool WantsPartialSolution() const { return false; }
	inline bool ShouldIncludeStartNodeInPath() const { return true; }

...
const TCHAR* GetHairAttributeText(EHairAttribute In, uint32 InFlags)
{
	...
	switch (In)
	{
	case EHairAttribute::Width:						return TEXT("Width");
...
{
	if (In == EHairAttribute::Width)		{ return PointsRadius.Num() > 0; }
...
uint32 FHairStrandsCommonDatas::GetAttributes() const
{
	uint32 Out = 0;
...
uint32 FHairStrandsCommonDatas::GetAttributeFlags() const
{
...
void FHairStrandsDatas::CopyPoint(const FHairStrandsDatas& In, FHairStrandsDatas& Out, uint32 InAttributes, uint32 InIndex, uint32 OutIndex)
{
	...
	if(!Out.StrandsPoints.PointsMapping.IsEmpty())
	{ 
		Out.StrandsPoints.PointsMapping[OutIndex] = In.StrandsPoints.PointsMapping.IsValidIndex(InIndex) ? In.StrandsPoints.PointsMapping[InIndex] : InIndex;
	}
}
...
void FHairStrandsDatas::CopyPointLerp(const FHairStrandsDatas& In, FHairStrandsDatas& Out, uint32 InAttributes, uint32 InIndex0, uint32 InIndex1, float InAlpha, uint32 OutIndex)
{
	...
	if (!Out.StrandsPoints.PointsMapping.IsEmpty())
	{
		const int32 PointIndex = (InAlpha < 0.5) ? InIndex0 : InIndex1;
		Out.StrandsPoints.PointsMapping[OutIndex] = In.StrandsPoints.PointsMapping.IsValidIndex(PointIndex) ? In.StrandsPoints.PointsMapping[PointIndex] : PointIndex;
	}
}
...
void FHairStrandsDatas::CopyCurve(const FHairStrandsDatas& In, FHairStrandsDatas& Out, uint32 InAttributes, uint32 InIndex, uint32 OutIndex)
{
	...
	if (!Out.StrandsCurves.CurvesMapping.IsEmpty())
	{
		Out.StrandsCurves.CurvesMapping[OutIndex] = In.StrandsCurves.CurvesMapping.IsValidIndex(InIndex) ? In.StrandsCurves.CurvesMapping[InIndex] : InIndex;
	}
}
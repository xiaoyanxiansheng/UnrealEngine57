...
void FGroomCacheStreamingData::UpdateStreamingStatus(bool bAsyncDeletionAllowed)
{
	...
	for (int32 NeededIndex : ChunksNeeded)
	{
		if (!ChunksAvailable.Contains(NeededIndex))
		{
			...
			if (!ChunksRequested.Contains(NeededIndex) && GroomCache->GetChunks().IsValidIndex(NeededIndex))
			{
				...
				TRACE_IOSTORE_METADATA_SCOPE_TAG("GroomCache");
...
void FGroomCacheStreamingData::UnmapAnimationData(uint32 ChunkIndex)
{
	...
	FResidentGroomCacheChunk* ResidentChunk = Chunks.Find(ChunkIndex);
	// If the unmap is done from RT and GT at the same time (init resource...) even if the critical section is preventing a race condition
	// we could end up with already a refcount = 0 in that function
	if (ResidentChunk != nullptr && ResidentChunk->Refcount > 0)
	{

...
void InternalBuildRenderCurveResourceBulkData(const FHairStrandsDatas& In, FRenderCurveResourceData& Out)
{
	...
	static_assert(CurvePerCluster == RENDERCURVE_CURVE_PER_CLUSTER);
	...
	for (uint32 ClusterIndex = 0; ClusterIndex < ClusterCount; ++ClusterIndex)
	{
		...
		PackedCluster.Header.CurveCount       = Cluster.CurveCount;
		PackedCluster.Header.CurvePerCluster  = Cluster.CurvePerCluster;
		...
		PackedCluster.Data.PackedPoints.SetNum(PackedCluster.Header.CurvePerCluster * PackedCluster.Header.PointPerCurve);

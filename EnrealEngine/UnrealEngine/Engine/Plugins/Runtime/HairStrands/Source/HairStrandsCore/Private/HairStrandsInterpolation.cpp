...
public:
	...
	static bool DoesSupportsWaveOps(EShaderPlatform InPlatform, uint32 InPointPerCurve)
	{
		...
		return FDataDrivenShaderPlatformInfo::GetSupportsWaveOperations(InPlatform) != ERHIFeatureSupport::Unsupported && FDataDrivenShaderPlatformInfo::GetMinimumWaveSize(InPlatform) <= 32 && FDataDrivenShaderPlatformInfo::GetMaximumWaveSize(InPlatform) >= 32;
	}
	static bool ShouldCompilePermutation(const FGlobalShaderPermutationParameters& Parameters) 
	{ 
		FPermutationDomain PermutationVector(Parameters.PermutationId);
		if (PermutationVector.Get<FWaveOps>() && !DoesSupportsWaveOps(Parameters.Platform, PermutationVector.Get<FPointPerCurve>()))
		{
			return false;
		}
	...
	{ 
		...
		// single guide can't be disabled when GetHairStrandsForceSingleGuideInterpolation() is set
		if (!PermutationVector.Get<FSingleGuide>() && GetHairStrandsForceSingleGuideInterpolation())
		{
	...
	const FHairStrandsLODDeformedRootResource::EFrameType DeformedFrame)
{
	...
	const bool bWaveOps = FHairInterpolationCS::DoesSupportsWaveOps(InPlatform, DispatchInfo.PointPerCurve);

...
void FHairStrandsCurves::SetNum(const uint32 NumCurves, uint32 InAttributes, bool bHasMapping)
{
	...
	if(bHasMapping)
	{
		CurvesMapping.SetNum(NumCurves);
	}
}
...
void FHairStrandsPoints::SetNum(const uint32 NumPoints, uint32 InAttributes, bool bHasMapping)
{
	...
	if(bHasMapping)
	{
		PointsMapping.SetNum(NumPoints);
	}
}

void FHairStrandsRawDatas::Reset()
{
	StrandsCurves.Reset();
	StrandsPoints.Reset();
}
...
void FHairStrandsBulkData::SerializeHeader(FArchive& Ar, UObject* Owner)
{
	...
	Ar << Header.SourceCurveCount;
	Ar << Header.SourcePointCount;
	...
	Ar << Header.Strides.CurveMappingStride;
	Ar << Header.Strides.PointMappingStride;
...
void FHairStrandsBulkData::GetResources(FHairStrandsBulkCommon::FQuery& Out)
{
	...
	if (!!(Header.Flags & DataFlags_HasData))
	{
		...
		if(Header.Flags & DataFlags_HasPointMapping)
		{
			Out.Add(Data.PointMapping, TEXT("_PointMapping"), Data.PointMapping.LoadedSize, PointCount * Header.Strides.PointMappingStride);
		}
		if(Header.Flags & DataFlags_HasCurveMapping)
		{
			Out.Add(Data.CurveMapping, TEXT("_CurveMapping"), Data.CurveMapping.LoadedSize, CurveCount * Header.Strides.CurveMappingStride);
		}
	}
}
...
uint32 FHairStrandsBulkData::GetSize() const
{
	...
	Out += Data.CurveMapping.GetBulkDataSize();
	Out += Data.PointMapping.GetBulkDataSize();
	return Out;
}
...
void FHairStrandsBulkData::Reset()
{
	...
	Data.CurveMapping.RemoveBulkData();
	Data.PointMapping.RemoveBulkData();
	...
	Data.CurveMapping	= FHairBulkContainer();
	Data.PointMapping	= FHairBulkContainer();
}
...
void FHairStrandsBulkData::ResetLoadedSize()
{
	...
	Data.CurveMapping.LoadedSize	= 0;
	Data.PointMapping.LoadedSize	= 0;
}
...
// Relies on bulk metadata being populated, which is only in cooked builds
#if !WITH_EDITORONLY_DATA
bool FHairStrandsBulkData::IsCookedBulkDataLoadedForCPUAccess() const
{
	bool bIsAllBulkDataLoaded = true;

	bIsAllBulkDataLoaded &= Data.Positions.IsBulkDataEmptyOrLoaded();
	bIsAllBulkDataLoaded &= Data.TranscodedPositions.IsBulkDataEmptyOrLoaded();
	bIsAllBulkDataLoaded &= Data.CurveAttributes.IsBulkDataEmptyOrLoaded();
	bIsAllBulkDataLoaded &= Data.PointAttributes.IsBulkDataEmptyOrLoaded();
	bIsAllBulkDataLoaded &= Data.PointToCurve.IsBulkDataEmptyOrLoaded();
	bIsAllBulkDataLoaded &= Data.Curves.IsBulkDataEmptyOrLoaded();
	bIsAllBulkDataLoaded &= Data.CurveMapping.IsBulkDataEmptyOrLoaded();
	bIsAllBulkDataLoaded &= Data.PointMapping.IsBulkDataEmptyOrLoaded();

	return bIsAllBulkDataLoaded;
}
#endif
...
// Relies on bulk metadata being populated, which is only in cooked builds
#if !WITH_EDITORONLY_DATA
bool FHairStrandsInterpolationBulkData::IsCookedBulkDataLoadedForCPUAccess() const
{
	bool bIsAllBulkDataLoaded = true;

	bIsAllBulkDataLoaded &= Data.CurveInterpolation.IsBulkDataEmptyOrLoaded();
	bIsAllBulkDataLoaded &= Data.PointInterpolation.IsBulkDataEmptyOrLoaded();
...
// Relies on bulk metadata being populated, which is only in cooked builds
#if !WITH_EDITORONLY_DATA
bool FHairStrandsClusterBulkData::IsCookedBulkDataLoadedForCPUAccess() const
{
	bool bIsAllBulkDataLoaded = true;

	bIsAllBulkDataLoaded &= Data.PackedClusterInfos.IsBulkDataEmptyOrLoaded();
	bIsAllBulkDataLoaded &= Data.CurveToClusterIds.IsBulkDataEmptyOrLoaded();
	bIsAllBulkDataLoaded &= Data.PointLODs.IsBulkDataEmptyOrLoaded();
...
// Relies on bulk metadata being populated, which is only in cooked builds
#if !WITH_EDITORONLY_DATA
bool FHairStrandsRootBulkData::IsCookedBulkDataLoadedForCPUAccess() const
{
	bool bIsAllBulkDataLoaded = true;

	bIsAllBulkDataLoaded &= Data.RootBarycentricBuffer.IsBulkDataEmptyOrLoaded();
	bIsAllBulkDataLoaded &= Data.RootToUniqueTriangleIndexBuffer.IsBulkDataEmptyOrLoaded();
	bIsAllBulkDataLoaded &= Data.UniqueTriangleIndexBuffer.IsBulkDataEmptyOrLoaded();
	bIsAllBulkDataLoaded &= Data.RestUniqueTrianglePositionBuffer.IsBulkDataEmptyOrLoaded();
	bIsAllBulkDataLoaded &= Data.MeshInterpolationWeightsBuffer.IsBulkDataEmptyOrLoaded();
	bIsAllBulkDataLoaded &= Data.MeshSampleIndicesAndSectionsBuffer.IsBulkDataEmptyOrLoaded();
	bIsAllBulkDataLoaded &= Data.RestSamplePositionsBuffer.IsBulkDataEmptyOrLoaded();

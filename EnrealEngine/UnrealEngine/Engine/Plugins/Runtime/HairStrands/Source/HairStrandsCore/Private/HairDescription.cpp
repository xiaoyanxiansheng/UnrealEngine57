...
bool FHairDescription::HasAttribute(EHairAttribute InAttribute) const
{
	...
	switch (InAttribute)
	{
	case EHairAttribute::Width:			return VertexAttributes().GetAttributesRef<float>(HairAttribute::Vertex::Width).IsValid() || StrandAttributes().GetAttributesRef<float>(HairAttribute::Strand::Width).IsValid() || GroomAttributes().GetAttributesRef<float>(HairAttribute::Groom::Width).IsValid();

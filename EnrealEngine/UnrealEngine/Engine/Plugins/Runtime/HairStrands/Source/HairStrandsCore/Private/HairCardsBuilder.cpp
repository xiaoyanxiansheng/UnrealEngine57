...
#define HAIR_CARD_UV_ROOTUV 0
#define HAIR_CARD_UV_CARDUV 1
#define HAIR_CARD_UV_COUNT  2

// Example of layout with 3 texture coords
//  -----------------------------------------------  -----------------------------------------------
// |          Vertex0          ||          Vertex1          ||          Vertex2          ||
// | 0.xy   0.zw   1.xy   1.zw || 0.xy   0.zw   1.xy   1.zw || 0.xy   0.zw   1.xy   1.zw ||
// | UV0  | UV1  | UV2  | ---  || UV0  | UV1  | UV2  | ---  || UV0  | UV1  | UV2  | ---  ||
//  -----------------------------------------------  -----------------------------------------------
static void SetPackedUV(uint32 VertexIndex, uint32 UVIndex, uint32 UVCount, const FVector2f& InUV, TArray<FVector4f>& OutPackedUVs)
{
	const uint32 StrideInUint = FMath::DivideAndRoundUp(UVCount, 2u);
	uint32 UVVertexIndex = VertexIndex * StrideInUint + (UVIndex / 2);
	if (UVIndex % 2 == 0)
	{
		OutPackedUVs[UVVertexIndex].X = InUV.X; 
		OutPackedUVs[UVVertexIndex].Y = InUV.Y; 
	}
	else
	{
		OutPackedUVs[UVVertexIndex].Z = InUV.X; 
		OutPackedUVs[UVVertexIndex].W = InUV.Y; 
	}
}
static FVector2f GetPackedUV(uint32 VertexIndex, uint32 UVIndex, uint32 UVCount, const TArray<FVector4f>& InPackedUVs)
{
	const uint32 StrideInUint = FMath::DivideAndRoundUp(UVCount, 2u);
	const uint32 UVVertexIndex = VertexIndex * StrideInUint + (UVIndex / 2);
	const FVector4f PackedUV = InPackedUVs[UVVertexIndex];
	return (UVIndex % 2 == 0) ? FVector2f(PackedUV.X, PackedUV.Y) : FVector2f(PackedUV.Z, PackedUV.W);
}

static uint32 GetNumPackedUVs(uint32 PointCount, uint32 NumUVs)
{
	static_assert(FHairCardsGeometry::FUVChannel::GetCardUVIndex() == HAIR_CARD_UV_CARDUV);

	// Packed 2 UVs into a single FVector4f
	const uint32 StrideInUint = FMath::DivideAndRoundUp(NumUVs, 2u);
	return PointCount * StrideInUint;
}
		...
		for (uint32 CardIt = 0; CardIt < NumCards; ++CardIt)
		{
			...
			for (uint32 TriangleIt = 0; TriangleIt < NumTriangles; ++TriangleIt)
			{
				...
				for (uint32 VertexIt = 0; VertexIt < 3; ++VertexIt)
				{
					...
					const FVector2f UV = InCards.UVs[HAIR_CARD_UV_CARDUV].UV[VertexIndex];
	...
	FHairCardsInterpolationBulkData& OutInterpolationBulkData)
{
	...
	const uint32 MeshDescriptionNumChannels = MeshDescription->GetNumUVElementChannels();
	const uint32 NumUVSource = MeshDescriptionNumChannels;
	const uint32 NumUVs = NumUVSource + 1 /*RootUV*/;
	...
	Out.Cards.UVs.SetNum(NumUVs);
	for (uint32 UVIt = 0;UVIt<NumUVs; ++UVIt)
	{
		Out.Cards.UVs[UVIt].UV.Init(FVector2f::ZeroVector, VertexCount);
	}
	...
	for (const FVertexID VertexId : MeshDescription->Vertices().GetElementIDs())
	{
		...
		for (uint32 UVIt = 0; UVIt < NumUVSource; ++UVIt)
		{
			Out.Cards.UVs[HAIR_CARD_UV_CARDUV + UVIt].UV[VertexIndex] = VertexInstanceUVs.Get(VertexInstanceId0, UVIt);
		}
	...
	OutBulk.UVs.SetNum(GetNumPackedUVs(PointCount, NumUVs));
	...
	if (InStrandsData.IsValid())
	{
		...
		for (uint32 CardRootIt = 0, CardRootCount = CardsRoots.Num(); CardRootIt < CardRootCount; ++CardRootIt)
		{
			...
			for (uint32 IndexIt = 0; IndexIt < CardsIndexCount; ++IndexIt)
			{
				...
				Out.Cards.UVs[HAIR_CARD_UV_ROOTUV].UV[VertexIndex] = RootUV;
			}
		}
	...
	for (uint32 PointIt = 0; PointIt < PointCount; ++PointIt)
	{
		for (uint32 UVIt = 0; UVIt < NumUVs; ++UVIt)
		{
			SetPackedUV(PointIt, UVIt, NumUVs, Out.Cards.UVs[UVIt].UV[PointIt], OutBulk.UVs);
		}
	}
	...
	FHairCardsInterpolationBulkData& OutInterpolationBulkData)
{
	...
	const bool bHasVertexColor = VertexColors.GetNumElements() > 0;
	const uint32 NumSourceUVs = VertexInstanceUVs.GetNumChannels();
	const uint32 NumUVs = NumSourceUVs + 1u /*RootUV*/;
	...
	for (const FVertexID VertexId : MeshDescription->Vertices().GetElementIDs())
	{
		...
		// UVs are packed 2 channels into a single float4
		// * UVs[0]   -> Root UV - Not set here, but will be 'patched' later once guides & interpolation data are built
		// * UVs[1-N] -> Card UV & else
		for (uint32 UVIndex=0;UVIndex<NumSourceUVs;++UVIndex)
		{
			const FVector2f UV = VertexInstanceUVs.Get(VertexInstanceId0, UVIndex);

			SetPackedUV(VertexIndex, HAIR_CARD_UV_CARDUV + UVIndex, NumUVs, UV, OutBulk.UVs);
		}
	...
	{		
		...
		for (uint32 PointIt = 0; PointIt < PointCount; ++PointIt)
		{
			...
			// Root UV bulk data
			{
				...
				SetPackedUV(PointIt, HAIR_CARD_UV_ROOTUV, NumUVs, RootUV, OutBulk.UVs);
			}
		}
	}
	...
	FHairMeshesBulkData& OutBulk)
{
	...
	Out.Meshes.UVs.SetNum(HAIR_CARD_UV_COUNT);
	Out.Meshes.UVs[HAIR_CARD_UV_CARDUV].UV.SetNum(TotalPointCount);
	Out.Meshes.UVs[HAIR_CARD_UV_ROOTUV].UV.Init(FVector2f(0,0), TotalPointCount);
	...
	Out.Meshes.UVs[HAIR_CARD_UV_CARDUV].UV[0] = FVector2f(0, 0);
	Out.Meshes.UVs[HAIR_CARD_UV_CARDUV].UV[1] = FVector2f(1, 0);
	Out.Meshes.UVs[HAIR_CARD_UV_CARDUV].UV[2] = FVector2f(1, 1);
	Out.Meshes.UVs[HAIR_CARD_UV_CARDUV].UV[3] = FVector2f(0, 1);
	Out.Meshes.UVs[HAIR_CARD_UV_CARDUV].UV[4] = FVector2f(0, 0);
	Out.Meshes.UVs[HAIR_CARD_UV_CARDUV].UV[5] = FVector2f(1, 0);
	Out.Meshes.UVs[HAIR_CARD_UV_CARDUV].UV[6] = FVector2f(1, 1);
	Out.Meshes.UVs[HAIR_CARD_UV_CARDUV].UV[7] = FVector2f(0, 1);
	...
	// Fill in render resources (do we need to keep it separated? e.g, format compression, packing)
	const uint32 NumUVs = 2; // RootUV+CardUV
	const uint32 PointCount = Out.Meshes.Positions.Num();
	OutBulk.Positions.SetNum(PointCount);
	OutBulk.Normals.SetNum(PointCount * FHairCardsNormalFormat::ComponentCount);
	OutBulk.UVs.SetNum(FHairCardsBuilder::GetNumPackedUVs(PointCount, NumUVs));
	OutBulk.VertexColors.Empty();
	for (uint32 PointIt = 0; PointIt < PointCount; ++PointIt)
	{
		...
		FHairCardsBuilder::SetPackedUV(PointIt, HAIR_CARD_UV_CARDUV, NumUVs, Out.Meshes.UVs[HAIR_CARD_UV_CARDUV].UV[PointIt], OutBulk.UVs);
		FHairCardsBuilder::SetPackedUV(PointIt, HAIR_CARD_UV_ROOTUV, NumUVs, FVector2f::ZeroVector, OutBulk.UVs);
	...
	FHairMeshesBulkData& OutBulk)
{
	...
	const uint32 NumUVsSource = LODData.VertexBuffers.StaticMeshVertexBuffer.GetNumTexCoords();
	const uint32 NumUVs = NumUVsSource + 1; // +1 for RootUV
	...
	Out.Meshes.UVs.SetNum(NumUVs);
	for (uint32 UVIt = 0; UVIt < NumUVs; ++UVIt)
	{
		Out.Meshes.UVs[UVIt].UV.Init(FVector2f::ZeroVector, VertexCount);
	}
	...
	for (uint32 VertexIt = 0; VertexIt < VertexCount; ++VertexIt)
	{
		...
		for (uint32 UVIt=0; UVIt<NumUVsSource; ++UVIt)
		{
			const FVector2f UV = LODData.VertexBuffers.StaticMeshVertexBuffer.GetVertexUV(VertexIt, UVIt);
			Out.Meshes.UVs[HAIR_CARD_UV_CARDUV + UVIt].UV[VertexIt] = UV;
		}
	...
	for (uint32 PointIt = 0; PointIt < PointCount; ++PointIt)
	{
		...
		for (uint32 UVIt=0; UVIt<NumUVs; ++UVIt)
		{
			const FVector2f CardUVs = Out.Meshes.UVs[UVIt].UV[PointIt];
			FHairCardsBuilder::SetPackedUV(PointIt, UVIt, NumUVs, CardUVs, OutBulk.UVs);
		}
	}
...
void ConvertCardsGeometryToMeshDescription(const FHairCardsGeometry& In, FMeshDescription& Out)
{
	...
	// Do not write out RootUV
	const uint32 NumUVs = FMath::Max(1, In.UVs.Num()-1);

	FMeshDescriptionBuilder Builder;
	Builder.SetMeshDescription(&Out);
	Builder.EnablePolyGroups();
	Builder.SetNumUVLayers(NumUVs);
	...
	for (uint32 VIndex=0; VIndex<VertexCount; ++VIndex)
	{
		...
		for (uint32 UVIt = 0; UVIt < NumUVs; ++UVIt)
		{
			Builder.SetInstanceUV(InstanceID, In.UVs[HAIR_CARD_UV_CARDUV + UVIt].UV[VIndex], UVIt);
		}

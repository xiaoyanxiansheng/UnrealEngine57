...
class FHairProjectionHairDebugCS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_RDG_BUFFER_SRV(StructuredBuffer<float4>, RestPositionOffset)
...
static void AddDebugProjectionHairPass(
	...
	const FHairStrandsRestResource* RestResources,
	const FHairStrandsDeformedResource* DeformedResources,
	...
	const HairStrandsTriangleType PoseType)
{
	...
	Parameters->RestPositionOffset = RegisterAsSRV(GraphBuilder, RestResources->PositionOffsetBuffer);	
	Parameters->DeformedPositionOffset = RegisterAsSRV(GraphBuilder, DeformedResources->GetPositionOffsetBuffer(FHairStrandsDeformedResource::EFrameType::Current));
	...
	const FHairInstanceCullingResults& CullingResults)
{
	...
	{
		const EHairInstanceVisibilityType VisibilityType = CullingResults.Is(View, *Instances[InstanceIndex]);
		InstanceCountPerType[uint32(VisibilityType)]++;
	}
	...
	for (uint32 InstanceIndex = 0; InstanceIndex < InstanceCount; ++InstanceIndex)
	{
		...
		const EHairInstanceVisibilityType VisibilityType = CullingResults.Is(View,*Instance);
		...
		case EHairGeometryType::Strands:
			if (Instance->Strands.IsValid())
			{
				...
				{
					D.Data1.X = Instance->Strands.RestRootResource->GetLOD(MeshLODIndex)->GetBulkData().Header.UniqueSectionIndices.Num();
					D.Data1.Y = Instance->Strands.RestRootResource->GetLOD(MeshLODIndex)->GetBulkData().Header.UniqueTriangleCount;
					D.Data1.Z = Instance->Strands.RestRootResource->GetLOD(MeshLODIndex)->GetBulkData().Header.RootCount;
					D.Data1.W = Instance->Strands.RestRootResource->GetLOD(MeshLODIndex)->GetBulkData().Header.PointCount;
				}
				...
				D.Data3.W |= Instance->HairGroupPublicData->VFInput.Strands.Common.bCurveRemapping ? (1u << 17u) : 0;
				D.Data3.W |= Instance->HairGroupPublicData->VFInput.Strands.Common.bPointRemapping ? (1u << 18u) : 0;
	...
	const TUniformBufferRef<FViewUniformShaderParameters>& ViewUniformBuffer)
{
	...
	{
		AddHairDebugPrintInstancePass(GraphBuilder, ShaderMap, View, TransientResources, ShaderPrintData, Instances, CullingResults);
	}
	...
	{
		{
			...
			auto RenderProjectionData = [&GraphBuilder, LocalView, ShaderMap, Viewport, &ViewUniformBuffer, Instances, ShaderPrintData, &DrawIndex](bool bSim)
			{
				...
				for (FHairStrandsInstance* AbstractInstance : Instances)
				{
					...
					if (bSim)
					{
						RestResource 	 = Instance->Guides.RestResource;
						DeformedResource = Instance->Guides.DeformedResource;
					...
					{
						RestResource 	 = Instance->Strands.RestResource;
						DeformedResource = Instance->Strands.DeformedResource;
					...
					else if (Instance->GetHairGeometry() == EHairGeometryType::Cards)
					{
						...
						if (Instance->Cards.IsValid(LODIndex))
						{
							RestResource 	 = Instance->Cards.LODs[LODIndex].Guides.RestResource;
							DeformedResource = Instance->Cards.LODs[LODIndex].Guides.DeformedResource;
					...
					if (RestRootResource == nullptr || DeformedRootResource == nullptr || RestResource == nullptr || DeformedResource == nullptr)
						continue;

					const int32 MeshLODIndex = Instance->Debug.MeshLODIndex;

					AddDebugProjectionHairPass(GraphBuilder, LocalView, ShaderMap, ShaderPrintData, Viewport, ViewUniformBuffer, MeshLODIndex, RestResource, DeformedResource, RestRootResource, DeformedRootResource, Instance->HairGroupPublicData->VFInput.LocalToWorldTransform, DrawIndex++, bSim, EDebugProjectionHairType::HairRoots,    HairStrandsTriangleType::RestPose);
					AddDebugProjectionHairPass(GraphBuilder, LocalView, ShaderMap, ShaderPrintData, Viewport, ViewUniformBuffer, MeshLODIndex, RestResource, DeformedResource, RestRootResource, DeformedRootResource, Instance->HairGroupPublicData->VFInput.LocalToWorldTransform, DrawIndex++, bSim, EDebugProjectionHairType::HairSamples,  HairStrandsTriangleType::RestPose);
					AddDebugProjectionHairPass(GraphBuilder, LocalView, ShaderMap, ShaderPrintData, Viewport, ViewUniformBuffer, MeshLODIndex, RestResource, DeformedResource, RestRootResource, DeformedRootResource, Instance->HairGroupPublicData->VFInput.LocalToWorldTransform, DrawIndex++, bSim, EDebugProjectionHairType::HairRoots,    HairStrandsTriangleType::DeformedPose);
					AddDebugProjectionHairPass(GraphBuilder, LocalView, ShaderMap, ShaderPrintData, Viewport, ViewUniformBuffer, MeshLODIndex, RestResource, DeformedResource, RestRootResource, DeformedRootResource, Instance->HairGroupPublicData->VFInput.LocalToWorldTransform, DrawIndex++, bSim, EDebugProjectionHairType::HairSamples,  HairStrandsTriangleType::DeformedPose);
				}
			};

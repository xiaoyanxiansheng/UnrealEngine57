...
void UGroomAsset::Serialize(FArchive& Ar)
{
	...
	if (Ar.CustomVer(FPhysicsObjectVersion::GUID) >= FPhysicsObjectVersion::GroomWithDescription)
	{
		...
		{
			auto SerializeHairDescription = [this, &Ar](const EHairDescriptionType DescriptionType)
			{
				const uint8 DescriptionIndex = FMath::Clamp(static_cast<uint8>(DescriptionType), 0, 1);
				...
				if (!HairDescriptionBulkData[DescriptionIndex])
				{
					// When loading, bulk data can be null so instantiate a new one to serialize into
					HairDescriptionBulkData[DescriptionIndex] = MakeUnique<FHairDescriptionBulkData>();
				}

				// We don' serialize the hair description if it is a transactional object (i.e., for undo/redo)
				if (!Ar.IsTransacting())
				{
					HairDescriptionBulkData[DescriptionIndex]->Serialize(Ar, this);
				}
			};

			SerializeHairDescription(EHairDescriptionType::Source);
			
			if (GetHairDescriptionType() == EHairDescriptionType::Edit)
			{
				SerializeHairDescription(EHairDescriptionType::Edit);
			}
		}
...
UGroomAsset::UGroomAsset(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer), DataflowSettings(this, nullptr, TEXT("GroomAssetTerminal"))
{
...
void UGroomAsset::UpdateHairGroupsInfo()
{
	...
	for (FHairGroupPlatformData& Data: GetHairGroupsPlatformData())
	{
		...
		if (const FHairDescriptionGroups* LocalHairDescriptionGroups = CachedHairDescriptionGroups[GetHairDescriptionIndex()].Get())
		{
...
static void SyncHairGroupsName(UGroomAsset* In)
{
#if WITH_EDITORONLY_DATA
	check(In);
	...
	{
		if (In->GetHairGroupsPhysics().IsValidIndex(GroupIndex))
		{
			In->GetHairGroupsPhysics()[GroupIndex].GroupName = Info.GroupName;
		}

		if (In->GetHairGroupsRendering().IsValidIndex(GroupIndex))
		{
			In->GetHairGroupsRendering()[GroupIndex].GroupName = Info.GroupName;
		}

		if (In->GetHairGroupsInterpolation().IsValidIndex(GroupIndex))
		{
			In->GetHairGroupsInterpolation()[GroupIndex].GroupName = Info.GroupName;
		}
...
void UGroomAsset::PostLoad()
{
	...
	if (bIsLegacyAsset)
	{
		if (HairDescriptionBulkData[GetHairDescriptionIndex()])
		{
...
{
#if WITH_EDITORONLY_DATA
	return HairDescriptionBulkData[GetHairDescriptionIndex()].IsValid() && !HairDescriptionBulkData[GetHairDescriptionIndex()]->IsEmpty();
#else
	return false;
#endif
}
...
FString UGroomAsset::GetDerivedDataKeyForStrands(uint32 GroupIndex)
{
	...
	const FString KeySuffix = GroomDerivedDataCacheUtils::BuildStrandsDerivedDataKeySuffix(GroupIndex, InterpolationSettings, LODSettings, PhysicsSettings, bNeedInterpolationData, HairDescriptionBulkData[GetHairDescriptionIndex()].Get());
	const FString DerivedDataKey = GroomDerivedDataCacheUtils::BuildGroomDerivedDataKey(KeySuffix);
...
void UGroomAsset::CommitHairDescription(FHairDescription&& InHairDescription, EHairDescriptionType Type)
{
	...
	SetHairDescriptionType(Type);

	UE::TUniqueLock LockScope(InternalLock);

	CachedHairDescription[GetHairDescriptionIndex()] = MakeUnique<FHairDescription>(InHairDescription);

	if (!HairDescriptionBulkData[GetHairDescriptionIndex()])
	{
		HairDescriptionBulkData[GetHairDescriptionIndex()] = MakeUnique<FHairDescriptionBulkData>();
	}

	// Update the cached hair description groups with the new hair description data
	CachedHairDescriptionGroups[GetHairDescriptionIndex()] = MakeUnique<FHairDescriptionGroups>();
	FGroomBuilder::BuildHairDescriptionGroups(*CachedHairDescription[GetHairDescriptionIndex()], *CachedHairDescriptionGroups[GetHairDescriptionIndex()]);

	HairDescriptionBulkData[GetHairDescriptionIndex()]->SaveHairDescription(*CachedHairDescription[GetHairDescriptionIndex()]);
}

const FHairDescriptionGroups& UGroomAsset::GetHairDescriptionGroups() const
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UGroomAsset::GetHairDescriptionGroups);

	check(HairDescriptionBulkData[GetHairDescriptionIndex()]);
	
	{
		UE::TUniqueLock LockScope(InternalLock);

		if (!CachedHairDescription[GetHairDescriptionIndex()])
		{
			TUniquePtr<FHairDescription> HairDescription = MakeUnique<FHairDescription>();
			HairDescriptionBulkData[GetHairDescriptionIndex()]->LoadHairDescription(*HairDescription);
			...
			CachedHairDescription[GetHairDescriptionIndex()] = MoveTemp(HairDescription);
		}

		if (!CachedHairDescriptionGroups[GetHairDescriptionIndex()])
		{
			TUniquePtr<FHairDescriptionGroups> HairDescriptionGroups = MakeUnique<FHairDescriptionGroups>();
			FGroomBuilder::BuildHairDescriptionGroups(*CachedHairDescription[GetHairDescriptionIndex()], *HairDescriptionGroups, true /*bAllowAddEndControlPoint*/);
			
			// Since we don't take read-lock, we need to make sure all memory is visible inside newly created object before publishing it.
			FPlatformMisc::MemoryBarrier();

			CachedHairDescriptionGroups[GetHairDescriptionIndex()] = MoveTemp(HairDescriptionGroups);
		}
	}

	return *CachedHairDescriptionGroups[GetHairDescriptionIndex()];
}

FHairDescription UGroomAsset::LoadHairDescription(const EHairDescriptionType DescriptionType) const
{
	const uint8 DescriptionIndex = FMath::Clamp(static_cast<uint8>(DescriptionType), 0, 1);
	...
	if (HairDescriptionBulkData[DescriptionIndex])
	{
		HairDescriptionBulkData[DescriptionIndex]->LoadHairDescription(OutHairDescription);
	}
...
{
	return LoadHairDescription(GetHairDescriptionType());
}
...
{
	if (!HairDescriptionBulkData[GetHairDescriptionIndex()] || HairDescriptionBulkData[GetHairDescriptionIndex()]->IsEmpty())
	{
...
bool UGroomAsset::GetHairStrandsDatas(
	...
	FHairStrandsDatas& OutGuidesData) const
{
...
void UGroomAsset::WriteDataflowContent(const TObjectPtr<UDataflowBaseContent>& DataflowContent) const
{
	if(const TObjectPtr<UDataflowGroomContent> GroomContent = Cast<UDataflowGroomContent>(DataflowContent))
	{
		...
		GroomContent->SetDataflowTerminal(DataflowSettings.GetDataflowTerminal().ToString());

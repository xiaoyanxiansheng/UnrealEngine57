...
static EHairMaterialCompatibility IsHairMaterialCompatible(UMaterialInterface* MaterialInterface, EShaderPlatform InShaderPlatform, EHairGeometryType GeometryType)
{
...
public:
	...
	virtual void GetDynamicRayTracingInstances(FRayTracingInstanceCollector& Collector) override
	{
		...
		const EShaderPlatform Platform = FirstActiveView->GetShaderPlatform();
		...
		const bool bWireframe = AllowDebugViewmodes() && FirstActiveView->Family->EngineShowFlags.Wireframe;
		const EHairViewRayTracingMask ViewRayTracingMask = FirstActiveView->Family->EngineShowFlags.PathTracing ? EHairViewRayTracingMask::PathTracing : EHairViewRayTracingMask::RayTracing;
		if (bWireframe)
			return;
		...
		for (uint32 GroupIt = 0, GroupCount = HairGroupInstances.Num(); GroupIt < GroupCount; ++GroupIt)
		{
			...
			if (RTGeometry && RTGeometry->RayTracingGeometry.IsValid())
			{
				...
				if (FMeshBatch* MeshBatch = CreateMeshBatch(*FirstActiveView->Family, Collector, EHairMeshBatchType::Raytracing, Instance, GroupIt, nullptr))
				{
					...
					for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
					{
						...
						{
							Collector.AddRDGPooledBuffer(ViewIndex, RTGeometry->PositionBuffer.Buffer);
						}
						if (RTGeometry->IndexBuffer.Buffer)
						{
							Collector.AddRDGPooledBuffer(ViewIndex, RTGeometry->IndexBuffer.Buffer);
						}
					}
				}
			}
		...
		for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
		{
			...
			if ((IsShadowCast(View) || IsShown(View)) && (VisibilityMap & (1 << ViewIndex)))
			{
				for (uint32 GroupIt = 0; GroupIt < GroupCount; ++GroupIt)
				{
					...
					if (FMeshBatch* MeshBatch = CreateMeshBatch(ViewFamily, Collector, EHairMeshBatchType::Raster, HairGroupInstances[GroupIt], GroupIt, Debug_MaterialProxy))
					{
...
void UGroomComponent::ReleaseHairSimulation(const int32 GroupIndex)
{
	if (GroupIndex < NiagaraComponents.Num())
	{
		...
		if (NiagaraComponent && !NiagaraComponent->IsBeingDestroyed())
		{
			...
			NiagaraComponent->MarkAsGarbage();
			NiagaraComponent->Rename(nullptr, GetTransientPackage(), REN_NonTransactional | REN_DoNotDirty | REN_DontCreateRedirectors);
...
void UGroomComponent::CreateHairSimulation(const int32 GroupIndex, const int32 LODIndex)
{
	if (GroupIndex < NiagaraComponents.Num())
	{
		...
		if (GroomAsset && (GroupIndex < GroomAsset->GetHairGroupsPhysics().Num()) && (LODIndex < GroomAsset->GetHairGroupsLOD()[GroupIndex].LODs.Num()) && IsSimulationEnable(GroupIndex, LODIndex))
		{
			...
			{
				FNameBuilder NSName;
				NSName.Append(TEXT("NiagaraComponent_"));
				GetFName().AppendString(NSName);
				NSName.Appendf(TEXT("_%d_%d"), GroupIndex, LODIndex);

				NiagaraComponent = NewObject<UNiagaraComponent>(this, NSName.ToString(), RF_Transient);
...
UMaterialInterface* UGroomComponent::GetMaterial(int32 ElementIndex, EHairGeometryType GeometryType, bool bUseDefaultIfIncompatible) const
{
	...
	if (bUseDefaultIfIncompatible)
	{
		...
		if (ShaderPlatform != EShaderPlatform::SP_NumPlatforms && IsHairMaterialCompatible(OverrideMaterial, ShaderPlatform, GeometryType) != EHairMaterialCompatibility::Valid)
		{
			bUseHairDefaultMaterial = true;
		}
	}
	...
	for (int32 GroupIt = 0, GroupCount = GroomAsset->GetHairGroupsPlatformData().Num(); GroupIt < GroupCount; ++GroupIt)
	{
		...
		for (uint32 CardLODIt=0;CardLODIt<CardLODCount;++CardLODIt)
		{
			...
			if (LODData.HasValidData())
			{
				...
				#if RHI_RAYTRACING
				if (IsHairRayTracingEnabled() && IsRayTracingAllowed() && bVisibleInRayTracing)
				{
					if (bNeedDeformedPositions)
					{
						// Allocate dynamic raytracing resources (owned by the groom component/instance)
...
void UGroomComponent::ValidateMaterials(bool bMapCheck) const
{
	...
	for (uint32 MaterialIt = 0, MaterialCount = GetNumMaterials(); MaterialIt < MaterialCount; ++MaterialIt)
	{
		...
		const EHairMaterialCompatibility Result = IsHairMaterialCompatible(OverrideMaterial, ShaderPlatform, GeometryType);
		switch (Result)
		{

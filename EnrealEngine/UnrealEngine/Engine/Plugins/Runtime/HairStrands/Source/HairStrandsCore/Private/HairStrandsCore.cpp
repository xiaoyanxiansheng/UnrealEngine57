...
{
	return CreateGroomBindingAsset(EGroomBindingMeshType::SkeletalMesh, InPackageName, InParent, GroomAsset, SourceSkelMesh, TargetSkelMesh, NumInterpolationPoints, MatchingSection, NAME_None);
}

UGroomBindingAsset* FHairStrandsCore::CreateGroomBindingAsset(EGroomBindingMeshType BindingType, const FString& InPackageName, UObject* InParent, UGroomAsset* GroomAsset, UObject* Source, UObject* Target, const int32 NumInterpolationPoints, const int32 MatchingSection, const FName TargetMeshAttribute)
{
	if (!GroomAsset ||
	...
	if (UGroomBindingAsset* Out = NewObject<UGroomBindingAsset>(Package, *Name, RF_Public | RF_Standalone | RF_Transactional))
	{
		...
		Out->SetTargetBindingAttribute(TargetMeshAttribute);
		Out->MarkPackageDirty();
		AssetHelper().RegisterAsset(Out);
		return Out;
	}
...
UGroomBindingAsset* FHairStrandsCore::CreateGroomBindingAsset(EGroomBindingMeshType BindingType, UGroomAsset* GroomAsset, UObject* Source, UObject* Target, const int32 NumInterpolationPoints, const int32 MatchingSection, const FName TargetMeshAttribute)
{
	return CreateGroomBindingAsset(BindingType, FString(), nullptr, GroomAsset, Source, Target, NumInterpolationPoints, MatchingSection, TargetMeshAttribute);
}
...
{
	return CreateGroomBindingAsset(EGroomBindingMeshType::SkeletalMesh, FString(), nullptr, GroomAsset, SourceSkelMesh, TargetSkelMesh, NumInterpolationPoints, MatchingSection, NAME_None);
}

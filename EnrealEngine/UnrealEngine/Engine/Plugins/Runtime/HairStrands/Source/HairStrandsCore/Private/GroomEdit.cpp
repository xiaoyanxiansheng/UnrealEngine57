...
void ConvertFromGroomAsset(UGroomAsset* InGroom, FEditableGroom* OutGroom, const bool bAllowCurveReordering, const bool bApplyDecimation, const bool bAllowAddEndControlPoint)
{
	...
	FGroomBuilder::BuildHairDescriptionGroups(In.LoadHairDescription(EHairDescriptionType::Source), HairDescriptionGroups, bAllowAddEndControlPoint);

	const uint32 GroupCount = In.GetNumHairGroups();
	if(GroupCount == HairDescriptionGroups.HairGroups.Num() && GroupCount == In.GetHairGroupsInterpolation().Num())
	{ 
...
void ConvertToGroomAsset(UGroomAsset* OutGroom, const FEditableGroom* InGroom, uint32 Operations)
{
	...
	OutGroom->CommitHairDescription(MoveTemp(HairDescription), EHairDescriptionType::Edit);

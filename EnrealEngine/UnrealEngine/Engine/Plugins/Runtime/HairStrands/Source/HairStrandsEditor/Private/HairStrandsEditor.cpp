...
void FGroomEditor::StartupModule()
{
	...
	// default no dataflow template
	UE::Groom::RegisterGroomDataflowTemplate({ "SelectNoDataflowTemplate", "No Dataflow", "Don't add a Dataflow to this Groom Asset. Useful when a Dataflow already exists.", "", false });
	UE::Groom::RegisterGroomDataflowTemplatePath(TEXT("/HairStrands/Dataflow/Templates"));
}
...
void FGroomEditor::ShutdownModule()
{
	...
	if (UObjectInitialized())
	{
		...
		if (PropertyModule)
		{
			...
			PropertyModule->UnregisterCustomClassLayout(UGroomHairGroupsMapping::StaticClass()->GetFName());
	...
	UE::Groom::UnregisterGroomDataflowTemplate("SelectNoDataflowTemplate");
	UE::Groom::UnregisterGroomDataflowTemplatePath(TEXT("/HairStrands/Dataflow/Templates"));
}

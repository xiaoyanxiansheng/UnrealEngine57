...
void FSourceEffectChorus::Init(const FSoundEffectSourceInitData& InitData)
{
	...
	if (USourceEffectChorusPreset* ProcPreset = Cast<USourceEffectChorusPreset>(Preset.Get()))
	{
		...
		SetDepthModulators(TransformObjectPtrSet(ProcPreset->Settings.DepthModulation.Modulators));
		SetFeedbackModulators(TransformObjectPtrSet(ProcPreset->Settings.FeedbackModulation.Modulators));
		SetFrequencyModulators(TransformObjectPtrSet(ProcPreset->Settings.FrequencyModulation.Modulators));
		SetWetModulators(TransformObjectPtrSet(ProcPreset->Settings.WetModulation.Modulators));
		SetDryModulators(TransformObjectPtrSet(ProcPreset->Settings.DryModulation.Modulators));
		SetSpreadModulators(TransformObjectPtrSet(ProcPreset->Settings.SpreadModulation.Modulators));
	}

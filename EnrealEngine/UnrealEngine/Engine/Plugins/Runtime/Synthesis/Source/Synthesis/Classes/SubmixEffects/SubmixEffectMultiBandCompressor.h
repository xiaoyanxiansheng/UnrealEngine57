...
{
public:
	UE_API FSubmixEffectMultibandCompressor();

	UE_API virtual ~FSubmixEffectMultibandCompressor();
	...
	UE_API void Initialize(FSubmixEffectMultibandCompressorSettings& Settings);

protected:
	// ISubmixBufferListener interface
	UE_API void ResetKey();
	UE_API void UpdateKeyFromSettings(const FSubmixEffectMultibandCompressorSettings& InSettings);
	UE_API bool UpdateKeySourcePatch();

	UE_API void OnDeviceCreated(Audio::FDeviceId InDeviceId);
	UE_API void OnDeviceDestroyed(Audio::FDeviceId InDeviceId);

	UE_API Audio::FMixerDevice* GetMixerDevice();
...
public:
...
#if WITH_EDITOR
	UE_API virtual void PostEditChangeChainProperty(struct FPropertyChangedChainEvent& InChainEvent) override;
#endif // WITH_EDITOR
	...
	// to effect's input.
	UFUNCTION(BlueprintCallable, Category = "Audio|Effects")
	UE_API void SetAudioBus(UAudioBus* AudioBus);

	// Sets the source key input as the provided Submix's output.  If no object is provided, key is set
	// to effect's input.
	UFUNCTION(BlueprintCallable, Category = "Audio|Effects")
	UE_API void SetExternalSubmix(USoundSubmix* Submix);

	UFUNCTION(BlueprintCallable, Category = "Audio|Effects")
	UE_API void SetSettings(const FSubmixEffectMultibandCompressorSettings& InSettings);

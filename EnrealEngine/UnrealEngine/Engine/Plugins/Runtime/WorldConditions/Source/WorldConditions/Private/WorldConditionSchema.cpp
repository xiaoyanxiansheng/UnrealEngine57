...
const FWorldConditionContextDataDesc* UWorldConditionSchema::GetContextDataDescByName(const FName DataName, const TArray<const UStruct*>& Structs) const
{
	return ContextDataDescs.FindByPredicate([DataName, &Structs](const FWorldConditionContextDataDesc& Desc)
	{
		return Desc.Name == DataName && Algo::AnyOf(Structs, [&Desc] (const UStruct* Struct)
			{
				return Desc.Struct->IsChildOf(Struct);
			});
	});
}

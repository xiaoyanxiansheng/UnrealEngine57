...
struct FWorldConditionDataView
{
    ...
    typename TEnableIf<!TIsDerivedFrom<T, UObject>::IsDerived, T*>::Type GetPtr() const
	{
		// If Memory is set, expect Struct too. Otherwise, let nulls pass through.
		check(!Memory || (Memory && Struct));
		check(!Struct || Struct->IsChildOf(TBaseStructure<T>::Get()));
		return ((T*)Memory);
	}

	/** @return Pointer to object stored in the view if the type matches correctly, else returns nullptr. */
	template <typename T>
	typename TEnableIf<TIsDerivedFrom<T, UObject>::IsDerived, T*>::Type TryGetPtr() const
	{
		// If Memory is set, expect Struct too. Otherwise, let nulls pass through.
		check(!Memory || (Memory && Struct));
		if (Struct && Struct->IsChildOf(T::StaticClass()))
		{
			return ((T*)Memory);
		}
	...
	/** @return Pointer to struct stored in the view if the type matches correctly, else returns nullptr. */
	template <typename T>
	typename TEnableIf<!TIsDerivedFrom<T, UObject>::IsDerived, T*>::Type TryGetPtr() const
	{
		// If Memory is set, expect Struct too. Otherwise, let nulls pass through.
		check(!Memory || (Memory && Struct));
		if (Struct && Struct->IsChildOf(TBaseStructure<T>::Get()))
		{
			return ((T*)Memory);
		}

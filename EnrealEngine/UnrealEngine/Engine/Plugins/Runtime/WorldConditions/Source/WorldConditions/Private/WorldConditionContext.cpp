...
bool FWorldConditionContext::Activate() const
{
	...
	{
		UE_VLOG_UELOG(QueryState.GetOwner(), LogWorldCondition, Warning, TEXT("Conditions already actived. Validate the call site to avoid redundant activation."));
		return true;
	}
	...
	for (int32 Index = 0; Index < Conditions.Num(); Index++)
	{
		...
		{
			UE_VLOG_UELOG(QueryState.GetOwner(), LogWorldCondition, Error, TEXT("Failed to activate condition at index %d"), Index);
			bSuccess = false;
		}
	}

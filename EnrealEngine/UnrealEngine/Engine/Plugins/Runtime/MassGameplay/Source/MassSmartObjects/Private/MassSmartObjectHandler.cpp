...
FMassSmartObjectRequestID FMassSmartObjectHandler::FindCandidatesAsync(const FMassEntityHandle RequestingEntity, const FGameplayTagContainer& UserTags, const FGameplayTagQuery& ActivityRequirements, const FVector& Location) const
{
	...
	return FindCandidatesAsync(RequestingEntity, MoveTemp(Parameters));
}
...
FMassSmartObjectRequestID FMassSmartObjectHandler::FindCandidatesAsync(const FMassEntityHandle RequestingEntity, UE::Mass::SmartObject::FFindCandidatesParameters&& Parameters) const
{
	...
	if (Parameters.LaneLocation.LaneHandle.IsValid())
	{
		FMassSmartObjectLaneLocationRequestFragment RequestFragment;
		RequestFragment.CompactLaneLocation = Parameters.LaneLocation;
		RequestFragment.RequestingEntity = RequestingEntity;
		RequestFragment.UserTags = Parameters.UserTags;
		RequestFragment.ActivityRequirements = Parameters.ActivityRequirements;

		if (Parameters.MRUSlots.NumSlots > 0)
		{
			UE::Mass::SmartObject::FMRUSlotsFragment MRUSlotsFragment{.Slots = Parameters.MRUSlots};
			ExecutionContext.Defer().PushCommand<FMassCommandAddFragmentInstances>(ReservedEntity, RequestFragment, MRUSlotsFragment, ResultFragment);
		}
		else
		{
	...
	else
	{
		...
		RequestFragment.SearchOrigin = Parameters.Location;
		RequestFragment.RequestingEntity = RequestingEntity;
		RequestFragment.UserTags = Parameters.UserTags;
		RequestFragment.ActivityRequirements = Parameters.ActivityRequirements;

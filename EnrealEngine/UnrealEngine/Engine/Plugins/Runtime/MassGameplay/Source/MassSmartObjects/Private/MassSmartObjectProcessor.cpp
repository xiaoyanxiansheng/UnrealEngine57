...
#include UE_INLINE_GENERATED_CPP_BY_NAME(MassSmartObjectProcessor)
void UMassSmartObjectCandidatesFinderProcessor::ConfigureQueries(const TSharedRef<FMassEntityManager>& EntityManager)
{
	...
	WorldRequestQuery.AddRequirement<UE::Mass::SmartObject::FMRUSlotsFragment>(EMassFragmentAccess::ReadOnly, EMassFragmentPresence::Optional);
	...
	LaneRequestQuery.AddRequirement<UE::Mass::SmartObject::FMRUSlotsFragment>(EMassFragmentAccess::ReadOnly, EMassFragmentPresence::Optional);
...
void UMassSmartObjectCandidatesFinderProcessor::Execute(FMassEntityManager& EntityManager, FMassExecutionContext& ExecutionContext)
{
	...
	WorldRequestQuery.ForEachEntityChunk(ExecutionContext, [this, &Filter, &EntitiesToSignal, &BeginRequestProcessing, &EndRequestProcessing](FMassExecutionContext& Context)
	{
		...
		const TConstArrayView<UE::Mass::SmartObject::FMRUSlotsFragment> MRUSlotsList = Context.GetFragmentView<UE::Mass::SmartObject::FMRUSlotsFragment>();
		...
		for (FMassExecutionContext::FEntityIterator EntityIt = Context.CreateEntityIterator(); EntityIt; ++EntityIt)
		{
			...
			// Gather MRU slots to ignore if the settings are set to use them
			TArray<FSmartObjectSlotHandle> SlotsToIgnore;
			if (MRUSlotsList.Num() > 0)
			{
				MRUSlotsList[EntityIt].Slots.AppendTo(SlotsToIgnore);

				const UE::Mass::SmartObject::FMRUSlots& Slots = MRUSlotsList[EntityIt].Slots;
				SlotsToIgnore.Reserve(Slots.NumSlots);

				for (int32 SlotIndex = Slots.NumSlots - 1; SlotIndex >= 0; SlotIndex--)
				{
					SlotsToIgnore.Push(Slots.Slots[SlotIndex].Slot);
				}
			}

			for (const FSmartObjectRequestResult& QueryResult : QueryResults)
			{
				if (SlotsToIgnore.Contains(QueryResult.SlotHandle))
				{
					continue;
				}
		...
		[&AnnotationSubsystem, Annotations, &Filter, SmartObjectTag, &EntitiesToSignal, &BeginRequestProcessing, &EndRequestProcessing](FMassExecutionContext& Context)
		{
			...
			for (FMassExecutionContext::FEntityIterator EntityIt = Context.CreateEntityIterator(); EntityIt; ++EntityIt)
			{
				...
				for (const int32 Index : SmartObjectList->SmartObjectLaneLocationIndices)
				{
					...
					{
						if (SlotsToIgnore.Contains(SlotHandle))
						{
							continue;
						}
...
// UMRUSlotsProcessor
//----------------------------------------------------------------------//
void UMRUSlotsProcessor::ConfigureQueries(const TSharedRef<FMassEntityManager>& EntityManager)
{
	EntityQuery.AddRequirement<FMassSmartObjectUserFragment>(EMassFragmentAccess::ReadOnly);
	EntityQuery.AddRequirement<FMRUSlotsFragment>(EMassFragmentAccess::ReadWrite);
}

UMRUSlotsProcessor::UMRUSlotsProcessor()
	: EntityQuery(*this)
{
	ExecutionOrder.ExecuteInGroup = ProcessorGroupNames::SyncWorldToMass;
}

void UMRUSlotsProcessor::Execute(FMassEntityManager& EntityManager, FMassExecutionContext& Context)
{
	QUICK_SCOPE_CYCLE_COUNTER(UMassProcessor_SmartObjecMRUSlots);

	const UMassSmartObjectSettings* Settings = GetDefault<UMassSmartObjectSettings>();
	if (Settings && Settings->bUseCooldownForMRUSlots == false)
	{
		return;
	}
	...
	{
		const float DT = Context.GetDeltaTimeSeconds();
		const TArrayView<FMRUSlotsFragment> MRUsList = Context.GetMutableFragmentView<FMRUSlotsFragment>();

		for (FMassExecutionContext::FEntityIterator EntityIt = Context.CreateEntityIterator(); EntityIt; ++EntityIt)
		{
			FMRUSlotsFragment& MRUs = MRUsList[EntityIt];
			FMRUSlots UpdatedSlots;
			for (int32 Index = MRUs.Slots.NumSlots - 1; Index >= 0; Index--)
			{
				float& Time = MRUs.Slots.Slots[Index].RemainingTime;
				Time = FMath::Max(Time - DT, 0.0f);
				if (Time > 0.f)
				{
					UpdatedSlots.Slots[UpdatedSlots.NumSlots] = MRUs.Slots.Slots[Index];
					UpdatedSlots.NumSlots++;
				}
			}

			MRUs.Slots = MoveTemp(UpdatedSlots);
		}
	});
}

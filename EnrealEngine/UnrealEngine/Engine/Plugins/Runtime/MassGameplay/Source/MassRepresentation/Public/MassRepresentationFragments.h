...
struct FMassRepresentationParameters : public FMassConstSharedFragment
{
	...
	// -- all propeties comparable with Memcpy go above this line. 
	// -- all other properties need to be manually handled in Identical implementation (as long as they are not "transient")
	...
	/** 
	 * The operation by design ignores CachedDefaultRepresentationType and CachedRepresentationActorManagement,
	 * since those two properties are fully rutime and are derived from the others.
	 */
	bool Identical(const FMassRepresentationParameters* InOtherValue, uint32 /*PortFlags*/) const
	{
		constexpr size_t NumBytesToCompare = offsetof(FMassRepresentationParameters, NotVisibleUpdateRate) + sizeof(NotVisibleUpdateRate);
		const int32 Result = FMemory::Memcmp(this, InOtherValue, NumBytesToCompare);

		return Result == 0
			// comparing bitfields separately since the padding might contain garbage
			&& bForceActorRepresentationForExternalActors == InOtherValue->bForceActorRepresentationForExternalActors
			&& bKeepLowResActors == InOtherValue->bKeepLowResActors
			&& bKeepActorExtraFrame == InOtherValue->bKeepActorExtraFrame
			&& bSpreadFirstVisualizationUpdate == InOtherValue->bSpreadFirstVisualizationUpdate
#if WITH_EDITORONLY_DATA
			&& bCanModifyRepresentationActorManagementClass == InOtherValue->bCanModifyRepresentationActorManagementClass
#endif // WITH_EDITORONLY_DATA
		;
	}
};
...
template<>
struct TStructOpsTypeTraits<FMassRepresentationParameters> : public TStructOpsTypeTraitsBase2<FMassRepresentationParameters>
{
	enum 
	{
		WithIdentical = true,

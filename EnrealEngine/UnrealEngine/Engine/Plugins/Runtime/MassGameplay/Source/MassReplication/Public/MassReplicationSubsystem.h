...
public:
	...
	UE_API UMassReplicationSubsystem();
	...
	UE_API FMassNetworkID GetNetIDFromHandle(const FMassEntityHandle Handle) const;
	...
	UE_API void SynchronizeClientsAndViewers();
	 ...
	 */
	UE_API FMassBubbleInfoClassHandle RegisterBubbleInfoClass(const TSubclassOf<AMassClientBubbleInfoBase>& BubbleInfoClass);
	...
	UE_API FMassBubbleInfoClassHandle GetBubbleInfoClassHandle(const TSubclassOf<AMassClientBubbleInfoBase>& BubbleInfoClass) const;
	 ...
	 */
	UE_API void SetEntity(const FMassNetworkID NetworkID, const FMassEntityHandle Entity);
	 ...
	 */
	UE_API FMassEntityHandle ResetEntityIfValid(const FMassNetworkID NetworkID, int32 ReplicationID);

	UE_API void RemoveFromEntityInfoMap(const FMassNetworkID NetworkID);
	 ...
	 */
	UE_API EFindOrAddMassEntityInfo FindAndUpdateOrAddMassEntityInfo(const FMassNetworkID NetworkID, int32 ReplicationID, const FMassReplicationEntityInfo*& OutMassEntityInfo);
	 ...
	 */
	UE_API const FMassReplicationEntityInfo* FindMassEntityInfo(const FMassNetworkID NetworkID) const;

	UE_API FMassEntityHandle FindEntity(const FMassNetworkID NetworkID) const;
...
protected:
...
#if UE_REPLICATION_COMPILE_CLIENT_CODE
	UE_API FMassReplicationEntityInfo* FindMassEntityInfoMutable(const FMassNetworkID NetworkID);
#endif // UE_REPLICATION_COMPILE_CLIENT_CODE
	 ...
	 */
	UE_API void AddClient(FMassViewerHandle ViewerHandle, APlayerController& InController);
	 ...
	 */
	UE_API void RemoveClient(FMassClientHandle ClientHandle);

	/** @return true if we should shrink the number of handles. */
	UE_API bool SynchronizeClients(const TArray<FViewerInfo>& Viewers);
	UE_API void SynchronizeClientViewers(const TArray<FViewerInfo>& Viewers);

	UE_API void DebugCheckArraysAreInSync();

protected:
	static UE_API uint32 CurrentNetMassCounter;

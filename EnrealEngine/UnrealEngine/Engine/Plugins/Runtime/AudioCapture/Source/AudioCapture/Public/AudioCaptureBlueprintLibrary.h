...
struct FAudioInputDeviceInfo
{
	...
	UE_API FAudioInputDeviceInfo(const Audio::FCaptureDeviceInfo& InDeviceInfo);
...
UCLASS(MinimalAPI, meta = (ScriptName = "AudioCaptureLibrary"))
class UAudioCaptureBlueprintLibrary : public UBlueprintFunctionLibrary
{
	...
	static UE_API FString Conv_AudioInputDeviceInfoToString(const FAudioInputDeviceInfo& info);
	...
	static UE_API void GetAvailableAudioInputDevices(const UObject* WorldContextObject, const FOnAudioInputDevicesObtained& OnObtainDevicesEvent);
};

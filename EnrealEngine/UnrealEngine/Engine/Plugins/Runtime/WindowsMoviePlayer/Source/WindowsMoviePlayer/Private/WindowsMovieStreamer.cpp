...
void FMediaFoundationMovieStreamer::ConvertSample(FRHICommandList& RHICmdList)
{
	...
	RHICmdList.BeginRenderPass(RPInfo, TEXT("WindowsMovieConvertSample"));
	{
		...
		default:
			bValidFormat = false; // unsupported format
		}
...
bool FMediaFoundationMovieStreamer::Tick(FRHICommandListBase& RHICmdList, float DeltaTime)
{
	FSlateTexture2DRHIRef* CurrentTexture = Texture.Get();
	...
	if (CurrentTexture && SampleGrabberCallback->GetIsSampleReadyToUpdate())
	{
		...
		{
			uint32 Stride;
			uint8* DestTextureData = (uint8*)RHICmdList.GetAsImmediate().LockTexture2D( CurrentTexture->GetTypedResource(), 0, RLM_WriteOnly, Stride, false );
			FMemory::Memcpy( DestTextureData, TextureData.GetData(), TextureData.Num());
			RHICmdList.GetAsImmediate().UnlockTexture2D( CurrentTexture->GetTypedResource(), 0, false );
		}
		else
		{
			ConvertSample(RHICmdList.GetAsImmediate());
		}

...
public:
	...
	UE_API void Apply(float& InOutValue, bool bInBipolar = false) const;

	float GetDuration() const { return Duration; }

#if WITH_EDITOR
	UE_DEPRECATED(5.3, "Use FWaveTableData version instead")
	UE_API void CopyToWaveTable(TArrayView<float> InOutTable, float& OutFinalValue, bool bInBipolar) const;

	UE_DEPRECATED(5.3, "Use FWaveTableData version instead")
	UE_API void CreateWaveTable(TArray<float>& InOutTable, float& OutFinalValue, bool bInBipolar) const;
	...
	UE_API void CopyToWaveTable(FWaveTableData& InOutTableData, bool bInBipolar) const;
	...
	UE_API void CreateWaveTable(FWaveTableData& InOutTableData, bool bInBipolar) const;
	...
	/** Caches curve data.  Should a shared curve be selected, curve points are copied locally
	  ...
	  */
	UE_API void CacheCurve();

	UE_API const FWaveTableData& GetTableData() const;
	...
	UE_API void SetFinalValue(float InValue);
...
private:
	...
	UE_API void SampleCurveBipolar(FWaveTableData& InOutTableData) const;

	/** Clamps & applies transform to provided values as unipolar signal */
	UE_API void SampleCurveUnipolar(FWaveTableData& InOutTableData) const;

	/** Converts internal TableData to float bit rate if not and re-samples to fit into provided wavetable data. */
	UE_API void SampleWaveTableData(FWaveTableData& InOutTableData) const;

...
public:
	UE_API UUsdAssetCache();

#if WITH_EDITOR
	UE_API virtual void PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent) override;
#endif

	UE_API void CacheAsset(const FString& Hash, UObject* Asset, const FString& PrimPath = FString());
	UE_API void DiscardAsset(const FString& Hash);
	...
	UE_API void LinkAssetToPrim(const FString& PrimPath, UObject* Asset);
	UE_API void RemoveAssetPrimLink(const FString& PrimPath);
	UE_API UObject* GetAssetForPrim(const FString& PrimPath) const;

	/** Returns the first prim path associated with an asset (see TMap::FindKey) */
	UE_API FString GetPrimForAsset(UObject* Asset) const;

	/**
	 * Returns the hash used as a key for an asset, in case we own it.
	 * Returns the empty string otherwise.
	 */
	UE_API FString GetHashForAsset(UObject* Asset) const;

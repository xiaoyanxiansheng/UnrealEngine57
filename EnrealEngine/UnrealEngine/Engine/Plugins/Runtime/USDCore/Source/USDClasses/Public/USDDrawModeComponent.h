 ...
 * applied.
 */
UCLASS(MinimalAPI, ClassGroup = (USD), meta = (BlueprintSpawnableComponent))
...
public:
	...
	UE_API void SetBoundsMin(const FVector& NewMin);

	UFUNCTION(BlueprintSetter, Category = "USD")
	UE_API void SetBoundsMax(const FVector& NewMax);

	UFUNCTION(BlueprintSetter, Category = "USD")
	UE_API void SetDrawMode(EUsdDrawMode NewDrawMode);

	UFUNCTION(BlueprintSetter, Category = "USD")
	UE_API void SetBoundsColor(FLinearColor NewColor);

	UFUNCTION(BlueprintSetter, Category = "USD")
	UE_API void SetCardGeometry(EUsdModelCardGeometry NewGeometry);

	UFUNCTION(BlueprintSetter, Category = "USD|Cards")
	UE_API void SetCardTextureXPos(UTexture2D* NewTexture);

	UFUNCTION(BlueprintSetter, Category = "USD|Cards")
	UE_API void SetCardTextureYPos(UTexture2D* NewTexture);

	UFUNCTION(BlueprintSetter, Category = "USD|Cards")
	UE_API void SetCardTextureZPos(UTexture2D* NewTexture);

	UFUNCTION(BlueprintSetter, Category = "USD|Cards")
	UE_API void SetCardTextureXNeg(UTexture2D* NewTexture);

	UFUNCTION(BlueprintSetter, Category = "USD|Cards")
	UE_API void SetCardTextureYNeg(UTexture2D* NewTexture);

	UFUNCTION(BlueprintSetter, Category = "USD|Cards")
	UE_API void SetCardTextureZNeg(UTexture2D* NewTexture);

	UFUNCTION(BlueprintPure, Category = "USD|Cards")
	UE_API UTexture2D* GetTextureForFace(EUsdModelCardFace Face) const;

	UFUNCTION(BlueprintCallable, Category = "USD|Cards")
	UE_API void SetTextureForFace(EUsdModelCardFace Face, UTexture2D* Texture);

public:
	UE_API UUsdDrawModeComponent();
	...
	UE_API virtual HHitProxy* CreateMeshHitProxy(int32 SectionIndex, int32 MaterialIndex) const override;
#endif
	// End UPrimitiveComponent interface

	UE_API EUsdModelCardFace GetAuthoredFaces() const;
	UE_API void SetAuthoredFaces(EUsdModelCardFace NewAuthoredFaces);

private:
	UE_API UMaterialInstance* GetOrCreateTextureMaterial(EUsdModelCardFace Face);
	UE_API void RefreshMaterialInstances();
};

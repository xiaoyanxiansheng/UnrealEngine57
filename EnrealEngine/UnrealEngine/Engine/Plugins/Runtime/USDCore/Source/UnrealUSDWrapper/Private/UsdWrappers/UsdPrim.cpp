...
namespace UE
{
	namespace Internal
	{
		...
		public:
			...
			{
#if defined(ENABLE_USD_DEBUG_PATH) && ENABLE_USD_DEBUG_PATH
				DebugPath = FString{UTF8_TO_TCHAR(PxrUsdPrim.Get().GetPrimPath().GetString().c_str())};
#endif
			}
	...
	TMap<FString, UE::FVtValue> FUsdPrim::GetAllMetadata() const
	{
		// Note that in USD the key types are pxr::TfToken, and so traditionally we should be using FNames instead of FStrings here.
		// That could cause trouble however, due to how pxr::TfToken is case sensitive and FNames are "case preserving" (sometimes).
		// Also, it's a bit annoying how pxr::VtDictionary (in case of nested metadata) does have std::string as keys... So for overall
		// convenience we use FString here so that we can use the same types for both this result and nested metadata maps, and can
		// avoid issues due to casing differences
		TMap<FString, UE::FVtValue> Result;
#if USE_USD_SDK
		FScopedUsdAllocs Allocs;

		std::map<pxr::TfToken, pxr::VtValue, pxr::TfDictionaryLessThan> UsdMetadata = Impl->PxrUsdPrim.Get().GetAllMetadata();
		Result.Reserve(UsdMetadata.size());

		std::map<pxr::TfToken, pxr::VtValue, pxr::TfDictionaryLessThan>::iterator Iter = UsdMetadata.begin();
		for (; Iter != UsdMetadata.end(); ++Iter)
		{
			Result.Add(UTF8_TO_TCHAR(Iter->first.GetString().c_str()), UE::FVtValue{std::move(Iter->second)});
		}
#endif	  // #if USE_USD_SDK
		return Result;
	}

	TMap<FString, UE::FVtValue> FUsdPrim::GetAllAuthoredMetadata() const
	{
		TMap<FString, UE::FVtValue> Result;
#if USE_USD_SDK
		FScopedUsdAllocs Allocs;

		std::map<pxr::TfToken, pxr::VtValue, pxr::TfDictionaryLessThan> UsdMetadata = Impl->PxrUsdPrim.Get().GetAllAuthoredMetadata();
		Result.Reserve(UsdMetadata.size());
	...
	FString FUsdPrim::GetDisplayName() const
	{
		FString Result;

#if USE_USD_SDK
		Result = UTF8_TO_TCHAR(Impl->PxrUsdPrim.Get().GetDisplayName().c_str());
#endif	  // #if USE_USD_SDK

		return Result;
	}

	bool FUsdPrim::SetDisplayName(const FString& Name) const
	{
#if USE_USD_SDK
		return Impl->PxrUsdPrim.Get().SetDisplayName(TCHAR_TO_UTF8(*Name));
#else
		return false;
#endif	  // #if USE_USD_SDK
	}

	bool FUsdPrim::ClearDisplayName() const
	{
#if USE_USD_SDK
		return Impl->PxrUsdPrim.Get().ClearDisplayName();
#else
		return false;
#endif	  // #if USE_USD_SDK
	}

	bool FUsdPrim::HasAuthoredDisplayName() const
	{
#if USE_USD_SDK
		return Impl->PxrUsdPrim.Get().HasAuthoredDisplayName();
#else
		return false;
#endif	  // #if USE_USD_SDK
	}
	...
	TArray<FSdfPrimSpec> FUsdPrim::GetPrimStack() const
	{
		TArray<FSdfPrimSpec> Result;

#if USE_USD_SDK
		TUsdStore<std::vector<pxr::SdfPrimSpecHandle>> UsdSpecStack = Impl->PxrUsdPrim.Get().GetPrimStack();

		Result.Reserve(UsdSpecStack->size());
		for (const pxr::SdfPrimSpecHandle& Handle : *UsdSpecStack)
		{
			Result.Add(FSdfPrimSpec{Handle});
		}
#endif	  // USE_USD_SDK
	...
	FPcpPrimIndex FUsdPrim::GetPrimIndex() const
	{
#if USE_USD_SDK
		return FPcpPrimIndex{Impl->PxrUsdPrim.Get().GetPrimIndex()};
#else
		return {};
#endif	  // #if USE_USD_SDK
	}

	FPcpPrimIndex FUsdPrim::ComputeExpandedPrimIndex() const
	{
#if USE_USD_SDK
		return FPcpPrimIndex{Impl->PxrUsdPrim.Get().ComputeExpandedPrimIndex()};
#else
		return {};
#endif	  // #if USE_USD_SDK
	}

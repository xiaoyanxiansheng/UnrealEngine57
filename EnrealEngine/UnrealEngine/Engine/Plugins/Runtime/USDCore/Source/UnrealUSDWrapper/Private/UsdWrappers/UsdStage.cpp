...
namespace UE
{
	namespace Internal
	{
		...
		// The only purpose of this class is so that we can cast the UsdStage impl to this type in the Unreal.natvis debug
		// visualizers, allowing us to inspect these FUsdStage and FUsdStageWeak variables in the debugger.
		//
		// Unlike the other wrapper types, we need these for FUsdStage because it is templated, and we can't express templated
		// type cast on the .natvis files very well.
		class UNREALUSDWRAPPER_API FUsdStageDebugImpl
		{
			FString DebugRootLayerIdentifier;
			FString DebugSessionLayerIdentifier;
			TUsdStore<int> FakePointer;
		};
#endif	  // ENABLE_USD_DEBUG_PATH
		...
		public:
			...
			{
#if defined(ENABLE_USD_DEBUG_PATH) && ENABLE_USD_DEBUG_PATH
				if (const PtrType& Ptr = GetInner())
				{
					DebugRootLayerIdentifier = FString{UTF8_TO_TCHAR(Ptr->GetRootLayer()->GetIdentifier().c_str())};
					DebugSessionLayerIdentifier = FString{UTF8_TO_TCHAR(Ptr->GetSessionLayer()->GetIdentifier().c_str())};
				}
#endif
			}
	...
	template UNREALUSDWRAPPER_API bool FUsdStage::operator==(const FUsdStage& Other) const;
	template UNREALUSDWRAPPER_API bool FUsdStage::operator==(const FUsdStageWeak& Other) const;
	template UNREALUSDWRAPPER_API bool FUsdStageWeak::operator==(const FUsdStage& Other) const;
	template UNREALUSDWRAPPER_API bool FUsdStageWeak::operator==(const FUsdStageWeak& Other) const;
...
#if USE_USD_SDK
	template class FUsdStageBase<pxr::UsdStageRefPtr>;
	template class FUsdStageBase<pxr::UsdStageWeakPtr>;
#else
	template class FUsdStageBase<FDummyRefPtrType>;
	template class FUsdStageBase<FDummyWeakPtrType>;
#endif	  // #if USE_USD_SDK
}	 // namespace UE

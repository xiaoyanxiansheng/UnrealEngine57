...
#include UE_INLINE_GENERATED_CPP_BY_NAME(UnrealUSDWrapper)
...
namespace UnrealIdentifiers
{
	...
	const TfToken SubdivisionAPI("SubdivisionAPI");
	const TfToken SubdivisionLevel = TfToken("unreal:subdivisionLevel");
	...
	const TfToken NaniteAssemblyRootAPI = TfToken("NaniteAssemblyRootAPI");
	const TfToken NaniteAssemblyExternalRefAPI = TfToken("NaniteAssemblyExternalRefAPI");
	const TfToken NaniteAssemblySkelBindingAPI = TfToken("NaniteAssemblySkelBindingAPI");
	const TfToken UnrealNaniteAssemblyMeshType = TfToken("unreal:naniteAssembly:meshType");
	const TfToken UnrealNaniteAssemblyMeshAssetPath = TfToken("unreal:naniteAssembly:meshAssetPath");
	const TfToken UnrealNaniteAssemblySkeleton = TfToken("unreal:naniteAssembly:skeleton");
	const TfToken PrimvarsUnrealNaniteAssemblyBindJoints = TfToken("primvars:unreal:naniteAssembly:bindJoints");
	const TfToken PrimvarsUnrealNaniteAssemblyBindJointWeights = TfToken("primvars:unreal:naniteAssembly:bindJointWeights");
	const TfToken NaniteAssemblyStaticMesh = TfToken("staticMesh");
	const TfToken NaniteAssemblySkeletalMesh = TfToken("skeletalMesh");
...
FSdfTimeCode::FSdfTimeCode(double InTime)
	: Time(InTime)
{
}

FSdfTimeCode::operator double() const
{
	return Time;
}

FSdfAssetPath::FSdfAssetPath(const FString& InAssetPath, const FString& InResolvedPath)
	: AssetPath(InAssetPath)
	, ResolvedPath(InResolvedPath)
{
}

FSdfAssetPath::operator FString() const
{
	return AssetPath;
}

FQuat4h::FQuat4h(float InX, float InY, float InZ, float InW)
	: X(InX)
	, Y(InY)
	, Z(InZ)
	, W(InW)
...
FQuat4h::FQuat4h(const FQuat4f& InQuat)
	: X(InQuat.X)
	, Y(InQuat.Y)
	, Z(InQuat.Z)
	, W(InQuat.W)
...
FQuat4h::operator FQuat4f() const
{
	return FQuat4f{X, Y, Z, W};
}

FVector3h::FVector3h(float InX, float InY, float InZ)
	: X(InX)
	, Y(InY)
	, Z(InZ)
{
...
FVector3h::FVector3h(const FVector3f& InVector)
	: X(InVector.X)
	, Y(InVector.Y)
	, Z(InVector.Z)
{
...
FVector3h::operator FVector3f() const
{
	return FVector3f{X, Y, Z};
}

FVector4h::FVector4h(float InX, float InY, float InZ, float InW)
	: X(InX)
	, Y(InY)
	, Z(InZ)
	, W(InW)
...
FVector4h::FVector4h(const FVector4f& InVector)
	: X(InVector.X)
	, Y(InVector.Y)
	, Z(InVector.Z)
	, W(InVector.W)
...
FVector4h::operator FVector4f() const
{
...
namespace UE::UnrealUSDWrapper::Private
{
	...
	)
	{
		...
		if (bForceReloadLayersFromDisk && Stage)
		{
			...
			// GetUsedLayers also returns layers from external references and such
			pxr::SdfLayerHandleVector StageLayers = Stage->GetUsedLayers();
...
UE::FUsdStage UnrealUSDWrapper::GetClipboardStage(bool bCreateIfNeeded)
{
	...
	if (!ClipboardStage && bCreateIfNeeded)
	{
...
// Memory wrapper is declared in ModuleBoilerPlate.h
// Needed for FMemory::Malloc to fit the signature of std::malloc
void* FMemory_Malloc(size_t Size, size_t Alignment);
// Needed for FMemory::Free to fit the signature of std::free
void FMemory_Free(void* Data);
...
public:
	virtual void StartupModule() override TSAN_SAFE
	{
...
#if !USE_USD_MEMORY_MANAGER
		pxr::MemoryOverloads::RegisterMallocFree(FMemory_Malloc, FMemory_Free);
#endif
		...
		UsdPluginsPath /= FPaths::Combine(TEXT("Win64"), TEXT("Arm64"), TEXT("plugins"));
#else
		UsdPluginsPath /= FPaths::Combine(TEXT("Win64"), TEXT("X64"), TEXT("plugins"));
#endif
#elif PLATFORM_LINUX
#if PLATFORM_CPU_ARM_FAMILY
		UsdPluginsPath /= FPaths::Combine(TEXT("Linux"), TEXT("Arm64"), TEXT("plugins"));
#else
		UsdPluginsPath /= FPaths::Combine(TEXT("Linux"), TEXT("X64"), TEXT("plugins"));
#endif
#elif PLATFORM_MAC
		...
		// e.g. "../../../Engine/Plugins/Runtime/USDCore/Source/ThirdParty/USD"
...
#if PLATFORM_WINDOWS
		// VS2015 aligns with UnrealBuildTool's
		// Target.WindowsPlatform.GetVisualStudioCompilerVersionName()
#if PLATFORM_CPU_ARM_FAMILY
		TargetDllFolder /= FPaths::Combine(TEXT("VS2015"), TEXT("ARM64"), TEXT("bin"));
#else
		TargetDllFolder /= FPaths::Combine(TEXT("VS2015"), TEXT("x64"), TEXT("bin"));
#endif
#elif PLATFORM_LINUX
#if PLATFORM_CPU_ARM_FAMILY
		TargetDllFolder /= FPaths::Combine(TEXT("Unix"), TEXT("aarch64-unknown-linux-gnueabi"), TEXT("lib"));
#else
		TargetDllFolder /= FPaths::Combine(TEXT("Unix"), TEXT("x86_64-unknown-linux-gnu"), TEXT("lib"));
#endif
#elif PLATFORM_MAC
		TargetDllFolder /= FPaths::Combine(TEXT("Mac"), TEXT("lib"));
#endif	  // PLATFORM_WINDOWS

#else
		FString TargetDllFolder = FPlatformProcess::ExecutablePath();
		FPaths::NormalizeFilename(TargetDllFolder);
		TargetDllFolder = FPaths::GetPath(TargetDllFolder);
		...
		TArray<FDirectoryPath> SearchPath = ProjectSettings->DefaultResolverSearchPath;

		FString ArSearchPathEnvVar = FPlatformMisc::GetEnvironmentVariable(TEXT("PXR_AR_DEFAULT_SEARCH_PATH"));
		if (!ArSearchPathEnvVar.IsEmpty())
		{
			TArray<FString> SearchPaths;
			ArSearchPathEnvVar.ParseIntoArray(SearchPaths, FPlatformMisc::GetPathVarDelimiter());
			SearchPath.Append(SearchPaths);
		}

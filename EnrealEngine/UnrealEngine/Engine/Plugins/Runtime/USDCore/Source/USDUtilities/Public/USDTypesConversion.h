...
struct FUsdStageInfo
{
	...
	USDUTILITIES_API explicit FUsdStageInfo(const pxr::UsdStageRefPtr& Stage);
};
...
namespace UsdUtils
{
	...
	/** Produces a transform that can be applied to a point from USD space to convert it into UE space */
	USDUTILITIES_API FTransform GetUsdToUESpaceTransform(const FUsdStageInfo& StageInfo);

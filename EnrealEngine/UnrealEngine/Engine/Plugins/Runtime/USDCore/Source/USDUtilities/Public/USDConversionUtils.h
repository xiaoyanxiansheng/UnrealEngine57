...
namespace UsdUtils
{
	...
	/** Returns all of the Prim attributes that contain timeSamples */
	USDUTILITIES_API TArray<UE::FUsdAttribute> GetAnimatedAttributes(const pxr::UsdPrim& Prim);
	...
	// Whether or not instancing-aware translation is enabled
	USDUTILITIES_API bool IsInstancingAwareTranslationEnabled();

	// Retrieves the equivalent of this prim in the prototype it's referenced from, where applicable
	USDUTILITIES_API UE::FUsdPrim GetPrototypePrim(const UE::FUsdPrim& Prim);
	
	// Retrieves the prototype path for the prim
	USDUTILITIES_API UE::FSdfPath GetPrototypePrimPath(const UE::FUsdPrim& Prim);

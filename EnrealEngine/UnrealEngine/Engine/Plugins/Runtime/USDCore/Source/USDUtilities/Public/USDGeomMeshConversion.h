...
namespace UsdToUnreal
{
	...
	struct FUsdMeshConversionOptions
	{
		...
		USDUTILITIES_API PRAGMA_ENABLE_DEPRECATION_WARNINGS

		const static FUsdMeshConversionOptions DefaultOptions;
		USDUTILITIES_API FUsdMeshConversionOptions();
		...
		EImportPrimvar ImportPrimvars;

		// Whether to add an additional offset to vertex join indices if the skinned mesh's skeleton has multiple root bones
		bool bOffsetJointIndicesForMultipleRootBones;
	};
...
namespace UsdUtils
{
	...
	struct FUsdPointInstancerInstancesData
	{
		struct FUsdPrototypeInstances
		{
			FString PathString;
			TArray<FTransform> InstanceTransforms;
		};

		TArray<FUsdPrototypeInstances> Prototypes;
	};

	/** Acquires the Prototypes from the PointInstancer and the related InstanceTransforms. */
	USDUTILITIES_API void GetPointInstancerInstancesData(const pxr::UsdPrim& Prim, FUsdPointInstancerInstancesData& InstancesData);
	...
	/** Returns all of the Prim UsdGeomMesh-related attributes (points, faceVertexIndices, etc.) that contain timeSamples */
	USDUTILITIES_API TArray<UE::FUsdAttribute> GetAnimatedMeshAttributes(const pxr::UsdPrim& Prim);

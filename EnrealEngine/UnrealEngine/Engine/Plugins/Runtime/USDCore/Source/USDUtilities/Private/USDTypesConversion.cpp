...
namespace UsdUtils
{
	...
	FTransform GetUsdToUESpaceTransform(const FUsdStageInfo& StageInfo)
	{
		FTransform MetersPerUnitTransform;
		const double UEMetersPerUnit = 0.01;
		if (!FMath::IsNearlyEqual(StageInfo.MetersPerUnit, UEMetersPerUnit))
		{
			const double ScaleFactor = StageInfo.MetersPerUnit / UEMetersPerUnit;
			MetersPerUnitTransform.SetScale3D(FVector{ScaleFactor, ScaleFactor, ScaleFactor});
		}

		FTransform UpAxisTransform;
		UpAxisTransform.SetScale3D(FVector{1.0, -1.0, 1.0});
		if (StageInfo.UpAxis == EUsdUpAxis::YAxis)
		{
			UpAxisTransform.SetRotation(FQuat::MakeFromEuler(FVector{90, 0, 0}));
		}

		return MetersPerUnitTransform * UpAxisTransform;
	}
}	 // namespace UsdUtils

...
namespace UsdUtils
{
	...
	struct FConvertedVtValue
	{
		...
		FString ExtraTypeInfo;	  // Used so that we can serialize something like SdfListOp<Token> as basically a token array, but still get the same
								  // type when converting back on the other end.

...
// 				}
			...
			, GetValueStringFunction(MoveTemp(InGetValueStringFunction))
		{
			...
			FConstOutputHandle ReroutedOutputHandle = GetConstReroutedOutputHandle();
		...
		void FMetasoundNumericDebugLineItem::OnValueCommitted(const FText& InValueText, FMetasoundFrontendLiteral& OutNewLiteral, int32 InIndex)
		{
			...
			FConstOutputHandle OutputHandle = GetConstReroutedOutputHandle();
		...
		const FGraphConnectionManager* FMetasoundNumericDebugLineItem::GetConnectionManager() const
		{
			using namespace Frontend;
		...
		void FMetasoundNumericDebugLineItem::Update()
		{
			...
			if (GetNodeIDAndOutputName(NodeID, OutputName))
			{
				...
				if (const FGraphConnectionManager* ConnectionManager = GetConnectionManager())
				{
		...
		TSharedRef<SWidget> FMetasoundNumericDebugLineItem::GenerateValueWidget(TSharedPtr<FString> InSearchString)
		{
			...
			const FName DataType = GetConstReroutedOutputHandle()->GetDataType();

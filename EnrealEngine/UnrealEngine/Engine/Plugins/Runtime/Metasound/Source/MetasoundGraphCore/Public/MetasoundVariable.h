...
namespace Metasound
{
	...
	struct TVariable
	{
		...
		void BindDataReference(FOutputVertexInterfaceData& InVertexData, const FVertexName& InVertexName)
		{
			checkf(DataReference.IsSet(), TEXT("InitDataReference must be called before accessing"));
			InVertexData.BindReadVertex(InVertexName, *DataReference);
		}

		void BindDelayedDataReference(FOutputVertexInterfaceData& InVertexData, const FVertexName& InVertexName)
		{
			checkf(DelayedDataReference.IsSet(), TEXT("InitDelayedDataReference must be called before accessing"));
			InVertexData.BindReadVertex(InVertexName, *DelayedDataReference);
		}

...
namespace Metasound
{
	...
	class FNodeFacade : public FBasicNode
	{
		...
		{
			using FCreateOperatorFunction = TUniquePtr<IOperator>(*)(const FBuildOperatorParams& InParams, FBuildResults& OutResults);
			friend class FNodeFacade;
	...
	public: 
		...
		: FNodeFacade(InInstanceName, InInstanceID, OperatorType::GetNodeInfo(), &OperatorType::CreateOperator)
		{
		}

		template<typename OperatorType>
		FNodeFacade(const FNodeInitData& InInitData, TFacadeOperatorClass<OperatorType> OperatorClass)
		: FNodeFacade(InInitData.InstanceName, InInitData.InstanceID, OperatorType::GetNodeInfo(), &OperatorType::CreateOperator)
		{
		}

		template<typename OperatorType>
		FNodeFacade(const FNodeData& InNodeData, const TSharedRef<const FNodeClassMetadata>& InClassMetadata, TFacadeOperatorClass<OperatorType> OperatorClass)
		: FNodeFacade(InNodeData, InClassMetadata, &OperatorType::CreateOperator)
		{
		}
	...
	private:
		FORCENOINLINE FNodeFacade(const FVertexName& InInstanceName, const FGuid& InInstanceID, const FNodeClassMetadata& InNodeInfo, FFactory::FCreateOperatorFunction InCreate)
		: FBasicNode(FNodeData(InInstanceName, InInstanceID, InNodeInfo.DefaultInterface), MakeShared<FNodeClassMetadata>(InNodeInfo))
		, Factory(MakeShared<FFactory, ESPMode::ThreadSafe>(InCreate))
		{
		}

		FORCENOINLINE FNodeFacade(const FNodeData& InNodeData, const TSharedRef<const FNodeClassMetadata>& InClassMetadata, FFactory::FCreateOperatorFunction InCreate)
	...
	public:
		...
		TNodeFacade(const FNodeInitData& InInitData)
			: FNodeFacade(InInitData, TFacadeOperatorClass<OperatorType>())
		{
		}

		TNodeFacade(const FNodeData& InNodeData, const TSharedRef<const FNodeClassMetadata>& InClassMetadata)
			: FNodeFacade(InNodeData, InClassMetadata, TFacadeOperatorClass<OperatorType>())
		{
		}

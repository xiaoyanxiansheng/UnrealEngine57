...
namespace Metasound
{
	...
	namespace MetasoundVariableNodesPrivate
	{
		...
		public:
			...
			virtual void BindOutputs(FOutputVertexInterfaceData& InVertexData) override
			{
				...
				InVertexData.BindWriteVertex<FVariable>(METASOUND_GET_PARAM_NAME(OutputVariable), Variable);
				Variable->BindDelayedDataReference(InVertexData, METASOUND_GET_PARAM_NAME(OutputData));
			}
		...
		public:
			...
			virtual void BindOutputs(FOutputVertexInterfaceData& InVertexData) override
			{
				...
				InVertexData.BindWriteVertex<FVariable>(METASOUND_GET_PARAM_NAME(OutputVariable), Variable);
				Variable->BindDataReference(InVertexData, METASOUND_GET_PARAM_NAME(OutputData));
			}
	 ...
	 * is a TVariable.  */
	template<typename DataType>
	class TVariableNode : public FBasicNode
	{
		using FVariable = TVariable<DataType>;
	...
	public:
		...
		: TVariableNode(MoveTemp(InLiteral), FNodeData{InNodeName, InInstanceID, DeclareVertexInterface()}, MakeShared<const FNodeClassMetadata>(CreateNodeClassMetadata()))
		{
		}

		TVariableNode(FLiteral InLiteral, FNodeData InNodeData, TSharedRef<const FNodeClassMetadata> InClassMetadata)
	...
	template<typename DataType>
	using TVariableMutatorNode = TNodeFacade<MetasoundVariableNodesPrivate::TVariableMutatorOperator<DataType>>;
	 ...
	 */
	template<typename DataType>
	using TVariableDeferredAccessorNode = TNodeFacade<MetasoundVariableNodesPrivate::TVariableDeferredAccessorOperator<DataType>>;

	/** FGetVariable node provides delay free, cpu free access to a set variable. */
	template<typename DataType>
	using TVariableAccessorNode = TNodeFacade<MetasoundVariableNodesPrivate::TVariableAccessorOperator<DataType>>;
}

...
namespace Metasound
{
	...
	TUniquePtr<IOperator> FTriggerRepeatOperator::CreateOperator(const FBuildOperatorParams& InParams, FBuildResults& OutResults)
	{
		...
		FTriggerReadRef TriggerEnable = InputData.GetOrCreateDefaultDataReadReference<FTrigger>(METASOUND_GET_PARAM_NAME(InputStart), InParams.OperatorSettings);
		FTriggerReadRef TriggerDisable = InputData.GetOrCreateDefaultDataReadReference<FTrigger>(METASOUND_GET_PARAM_NAME(InputStop), InParams.OperatorSettings);
	...
	FTriggerRepeatNode::FTriggerRepeatNode(FNodeData InNodeData, TSharedRef<const FNodeClassMetadata> InMetadata)
	:	FNodeFacade(MoveTemp(InNodeData), MoveTemp(InMetadata), TFacadeOperatorClass<FTriggerRepeatOperator>())
	{
	}

	FNodeClassMetadata FTriggerRepeatNode::CreateNodeClassMetadata()
	{
		return FTriggerRepeatOperator::GetNodeInfo();
	}
}

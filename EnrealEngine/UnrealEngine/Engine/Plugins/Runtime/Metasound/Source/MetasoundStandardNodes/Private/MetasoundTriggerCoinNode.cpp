...
namespace Metasound
{
	...
	namespace TriggerCoinVertexNames
	{
		...
		METASOUND_PARAM(InputProbability, "Probability", "Probability that the output trigger will get called. 1.0 is always heads, 0.0 is always tails, and 0.5 is a 50% chance for either.");
		METASOUND_PARAM(OutputTrueTrigger, "Heads", "The first possible output trigger, chosen with Probability chance.");
		METASOUND_PARAM(OutputFalseTrigger, "Tails", "The second possible output trigger, chosen with (1-Probability) chance).");
	}
	...
	const FVertexInterface& FTriggerCoinOperator::GetVertexInterface()
	{
		...
		static const FVertexInterface Interface(
			FInputVertexInterface(
				...
				TInputDataVertex<FTrigger>(METASOUND_GET_PARAM_NAME_AND_METADATA_ADVANCED(InputReset)),
				TInputDataVertex<int32>(METASOUND_GET_PARAM_NAME_AND_METADATA_ADVANCED(InputSeed), -1),
			...
			FOutputVertexInterface(
				...
				TOutputDataVertex<FTrigger>(METASOUND_GET_PARAM_NAME_AND_METADATA_ADVANCED(OutputFalseTrigger))
			)
		);
	...
	const FNodeClassMetadata& FTriggerCoinOperator::GetNodeInfo()
	{
		auto InitNodeInfo = []() -> FNodeClassMetadata
		{
			...
			Info.MajorVersion = 1;
			Info.MinorVersion = 0;
			Info.DisplayName = METASOUND_LOCTEXT("Metasound_TriggerCoinNodeDisplayName", "Trigger Chance");
			Info.Description = METASOUND_LOCTEXT("Metasound_TriggerCoinNodeDescription", "When triggered, Heads has the given probability chance to trigger, otherwise it's Tail.");
			Info.Author = PluginAuthor;
			Info.PromptIfMissing = PluginNodeMissingPrompt;
			Info.DefaultInterface = GetVertexInterface();
			Info.CategoryHierarchy.Emplace(NodeCategories::Trigger);
			Info.Keywords = {
				METASOUND_LOCTEXT("TriggerFilterRandomKeyword", "Random"),
				METASOUND_LOCTEXT("TriggerFilterProbabilityKeyword", "Probability"),
				METASOUND_LOCTEXT("TriggerFilterChanceKeyword", "Chance"),
				METASOUND_LOCTEXT("TriggerFilterCoinKeyword", "Coin"),
				METASOUND_LOCTEXT("TriggerFilterBernoulliKeyword", "Bernoulli Gate")
			};

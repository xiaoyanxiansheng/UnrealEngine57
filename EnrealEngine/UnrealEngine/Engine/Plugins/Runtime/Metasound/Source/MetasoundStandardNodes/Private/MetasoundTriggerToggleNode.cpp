...
namespace Metasound
{
	...
	TUniquePtr<IOperator> FTriggerToggleOperator::CreateOperator(const FBuildOperatorParams& InParams, FBuildResults& OutResults)
	{
		...
		FTriggerReadRef InTriggerOn = InputData.GetOrCreateDefaultDataReadReference<FTrigger>(METASOUND_GET_PARAM_NAME(InputOnTrigger), InParams.OperatorSettings);
		FTriggerReadRef InTriggerOff = InputData.GetOrCreateDefaultDataReadReference<FTrigger>(METASOUND_GET_PARAM_NAME(InputOffTrigger), InParams.OperatorSettings);
	...
	FTriggerToggleNode::FTriggerToggleNode(FNodeData InNodeData, TSharedRef<const FNodeClassMetadata> InMetadata)
	:	FNodeFacade(MoveTemp(InNodeData), InMetadata, TFacadeOperatorClass<FTriggerToggleOperator>())
	{
	}

	FNodeClassMetadata FTriggerToggleNode::CreateNodeClassMetadata()
	{
		return FTriggerToggleOperator::GetNodeInfo();
	}

...
namespace Metasound
{
	...
	TUniquePtr<IOperator> FTriggerOnNanOperator::CreateOperator(const FBuildOperatorParams& InParams, FBuildResults& OutResults)
	{
		...
		FAudioBufferReadRef AudioInput = InputData.GetOrCreateDefaultDataReadReference<FAudioBuffer>(METASOUND_GET_PARAM_NAME(AudioInput), InParams.OperatorSettings);
	...
	// Node Class
	using FTriggerOnNanNode = TNodeFacade<FTriggerOnNanOperator>;

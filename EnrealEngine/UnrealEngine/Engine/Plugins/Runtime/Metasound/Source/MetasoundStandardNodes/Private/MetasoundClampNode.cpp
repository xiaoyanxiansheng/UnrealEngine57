	...
	namespace MetasoundClampNodePrivate
	{
		...
		struct TClamp<FAudioBuffer>
		{
			...
			static TDataReadReference<FAudioBuffer> CreateInMin(const FBuildOperatorParams& InParams)
			{
				using namespace ClampVertexNames;
				const FInputVertexInterfaceData& InputData = InParams.InputData;
				return InputData.GetOrCreateDefaultDataReadReference<FAudioBuffer>(METASOUND_GET_PARAM_NAME(InputMinValue), InParams.OperatorSettings);
			}
			...
			static TDataReadReference<FAudioBuffer> CreateInMax(const FBuildOperatorParams& InParams)
			{
				using namespace ClampVertexNames;
				const FInputVertexInterfaceData& InputData = InParams.InputData;
				return InputData.GetOrCreateDefaultDataReadReference<FAudioBuffer>(METASOUND_GET_PARAM_NAME(InputMaxValue), InParams.OperatorSettings);
			}
		};
	}
 ...
 */
	template<typename ValueType>
	using TClampNode = TNodeFacade<TClampNodeOperator<ValueType>>;

...
namespace Metasound
{
	...
	void FAsyncMetaSoundBuilderBase::SetGraphOnGenerator(FOperatorAndInputs&& InOpAndInputs, FBuildResults&& InBuildResults)
	{
		...
		const FGeneratorInitParams& InitParams = GetGeneratorParams();
		...
		{
			UE_LOG(LogMetaSound, Error, TEXT("Failed to build Metasound operator from graph in MetasoundSource [%s]"), *InitParams.AssetPath.ToString());
	...
	FAsyncMetaSoundBuilder::FAsyncMetaSoundBuilder(FMetasoundConstGraphGenerator* InGenerator, FGeneratorInitParams&& InInitParams, bool bInTriggerGenerator)
	...
	void FAsyncMetaSoundBuilder::DoWork()
	{
		...
		METASOUND_TRACE_CPUPROFILER_EVENT_SCOPE_TEXT(*FString::Printf(TEXT("AsyncMetaSoundBuilder::DoWork %s"), *InitParams.AssetPath.GetAssetName().ToString()));
		...
		GeneratorBuilder::LogBuildErrors(InitParams.AssetPath, BuildResults);

		SetGraphOnGenerator(MoveTemp(GraphOperatorAndInputs), MoveTemp(BuildResults));

		FGeneratorInitParams::Reset(InitParams);
	}

	const FGeneratorInitParams& FAsyncMetaSoundBuilder::GetGeneratorParams() const
	{
		return InitParams;
	}
	...
	const FGeneratorInitParams& FAsyncDynamicMetaSoundBuilder::GetGeneratorParams() const
	{
		return InitParams;
	}
	...
	{
		TArray<FAudioBufferReadRef> FindOutputAudioBuffers(const TArray<FVertexName>& InAudioVertexNames, const FVertexInterfaceData& InVertexData, const FOperatorSettings& InOperatorSettings, const FTopLevelAssetPath& InAssetPath)
		{
			...
			for (const FVertexName& AudioOutputName : InAudioVertexNames)
			{
				...
				{
					UE_LOG(LogMetaSound, Warning, TEXT("MetasoundSource [%s] does not contain audio output [%s] in output"), *InAssetPath.ToString(), *AudioOutputName.ToString());
				}
		...
		void LogBuildErrors(const FTopLevelAssetPath& InAssetPath, const FBuildResults& InBuildResults)
		{
			// Log build errors
			for (const IOperatorBuilder::FBuildErrorPtr& Error : InBuildResults.Errors)
			{
				if (Error.IsValid())
				{
					UE_LOG(LogMetaSound, Warning, TEXT("MetasoundSource [%s] build error [%s] \"%s\""), *InAssetPath.ToString(), *(Error->GetErrorType().ToString()), *(Error->GetErrorDescription().ToString()));
				}
			}
		}
		...
		FInputVertexInterfaceData BuildGraphOperatorInputs(const FOperatorSettings& InOperatorSettings, FGeneratorInitParams& InInitParams)
		{
			...
			{
				UE_LOG(LogMetaSound, Error, TEXT("Unable to build graph operator inputs for null graph in MetaSoundSource [%s]"), *InInitParams.AssetPath.ToString());
			...
			for (FAudioParameter& Parameter : InInitParams.DefaultParameters)
			{
				...
				if (const FInputDataVertex* InputVertex = InputInterface.Find(ParamName))
				{
					...
					{
						UE_LOG(LogMetaSound, Warning, TEXT("Failed to create initial input data reference from parameter %s of type %s on graph in MetaSoundSource [%s]"), *ParamName.ToString(), *InputVertex->DataTypeName.ToString(), *InInitParams.AssetPath.ToString());
					}
				}
				else if(MetaSoundParameterEnableWarningOnIgnoredParameterCVar)
				{
					UE_LOG(LogMetaSound, Warning, TEXT("Failed to set initial input parameter %s on graph in MetaSoundSource [%s]"), *ParamName.ToString(), *InInitParams.AssetPath.ToString());
				}
			}
		...
		FOperatorAndInputs BuildGraphOperator(const FOperatorSettings& InOperatorSettings, FGeneratorInitParams& InInitParams, FBuildResults& OutBuildResults)
		{
			...
			if (!InInitParams.Graph)
			{
				UE_LOG(LogMetaSound, Error, TEXT("Unable to build graph operator for null graph in MetaSoundSource [%s]"), *InInitParams.AssetPath.ToString());
				return OpAndInputs;
			}
			...
			METASOUND_DEBUG_SET_ACTIVE_ASSET_SCOPE(InInitParams.AssetPath.GetAssetName(), InInitParams.ClassName, InInitParams.AssetPath.GetPackageName());
			
			OpAndInputs.Operator = Builder.BuildGraphOperator(BuildParams, OutBuildResults);
			OpAndInputs.RenderCost = InInitParams.GraphRenderCost;
			return OpAndInputs;
		}
		...
		MetasoundGeneratorPrivate::FMetasoundGeneratorData BuildGeneratorData(const FOperatorSettings& InOperatorSettings, const FGeneratorInitParams& InInitParams, FOperatorAndInputs&& InGraphOperatorAndInputs, TUniquePtr<Frontend::FGraphAnalyzer> InAnalyzer)
		{
			using namespace Audio;
			using namespace Frontend;
			using namespace MetasoundGeneratorPrivate;
			...
			// Get outputs
			TArray<FAudioBufferReadRef> OutputBuffers = FindOutputAudioBuffers(InInitParams.AudioOutputNames, VertexData, InOperatorSettings, InInitParams.AssetPath);

...
namespace Metasound
{
	namespace Frontend
	{
		namespace SearchEngineQuerySteps
		{
			...
			{
			public:
				FRemoveDeprecatedClasses() = default;
		...
		//		}
		// };
		template<typename QueryPolicy, typename... ArgTypes>
		struct TSearchEngineQuery : public ISearchEngineQuery
		{
			using ResultType = typename QueryPolicy::ResultType;

			TSearchEngineQuery(ArgTypes&&... Args)
			: Query(QueryPolicy::CreateQuery(Forward<ArgTypes>(Args)...))
			{
			}

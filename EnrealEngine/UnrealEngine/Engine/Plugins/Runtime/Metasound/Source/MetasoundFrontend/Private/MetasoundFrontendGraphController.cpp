...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
				PageOrder = DocRegistry->GetPageOrder();
PRAGMA_ENABLE_DEPRECATION_WARNINGS
			}
			return GetDefaultInput(InVertexID, PageOrder);
		}

		FMetasoundFrontendLiteral FGraphController::GetDefaultInput(const FGuid& InVertexID, TArrayView<const FGuid> InPageOrder) const
		{
			if (const FMetasoundFrontendClassInput* Desc = FindInputDescriptionWithVertexID(InVertexID))
			{
				if (const FMetasoundFrontendClassInputDefault* Default = FindPreferredPage(Desc->GetDefaults(), InPageOrder))
				{
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
			...
			return SetDefaultInput(InVertexID, InLiteral, PageOrder);
		}

		bool FGraphController::SetDefaultInput(const FGuid& InVertexID, const FMetasoundFrontendLiteral& InLiteral, TArrayView<const FGuid> InPageOrder)
		{
			if (FMetasoundFrontendClassInput* Desc = FindInputDescriptionWithVertexID(InVertexID))
			{
				if (ensure(IDataTypeRegistry::Get().IsLiteralTypeSupported(Desc->TypeName, InLiteral.GetType())))
				{
					if (const FMetasoundFrontendClassInputDefault* Default = FindPreferredPage(Desc->GetDefaults(), InPageOrder))
					{
						Desc->FindDefaultChecked(Default->PageID) = InLiteral;
						return true;
					}
				}
				else
			...
			if (FMetasoundFrontendClassInput* Desc = FindInputDescriptionWithName(InName))
			{
				...
				if (const FMetasoundFrontendGraphClass* GraphClass = GraphClassPtr.Get())
				{
					for (const FMetasoundFrontendClassInput& Input : GraphClass->GetDefaultInterface().Inputs)
					{
						...
						{
							if (const IDocumentBuilderRegistry* BuilderRegistry = IDocumentBuilderRegistry::Get())
							{
								if (FMetaSoundFrontendDocumentBuilder* Builder = BuilderRegistry->FindBuilder(GraphClass->Metadata.GetClassName(), { }))
								{
									const int32 Index = Builder->GetConstDocumentChecked().RootGraph.GetDefaultInterface().Inputs.Find(Input);
									FDocumentModifyDelegates& DocumentDelegates = Builder->GetDocumentDelegates();
					...
					for (const FMetasoundFrontendClassOutput& Output : GraphClass->GetDefaultInterface().Outputs)
					{
						if (Output.Name == InName)
						{
							if (const IDocumentBuilderRegistry* BuilderRegistry = IDocumentBuilderRegistry::Get())
							{
								if (FMetaSoundFrontendDocumentBuilder* Builder = BuilderRegistry->FindBuilder(GraphClass->Metadata.GetClassName(), { }))
								{
									const int32 Index = Builder->GetConstDocumentChecked().RootGraph.GetDefaultInterface().Outputs.Find(Output);
									FDocumentModifyDelegates& DocumentDelegates = Builder->GetDocumentDelegates();
		...
		FNodeHandle FGraphController::AddNode(FConstClassAccessPtr InExistingDependency, FGuid InNodeGuid)
		{
			if (FMetasoundFrontendGraphClass* GraphClass = GraphClassPtr.Get())
			{
				if (const FMetasoundFrontendClass* NodeClass = InExistingDependency.Get())
				{
					...
					{
						if (const INodeTemplate* Template = INodeTemplateRegistry::Get().FindTemplate(FNodeClassRegistryKey(NodeClass->Metadata)))
						{
							Configuration = Template->CreateFrontendTemplateNodeConfiguration();
						}
					}
					else if (NodeClass->Metadata.GetType() != EMetasoundFrontendClassType::Graph) // Subgraphs do not have extensions
					{

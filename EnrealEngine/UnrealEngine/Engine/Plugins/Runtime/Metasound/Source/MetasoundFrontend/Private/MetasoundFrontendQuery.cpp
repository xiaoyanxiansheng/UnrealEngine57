...
#include UE_INLINE_GENERATED_CPP_BY_NAME(MetasoundFrontendQuery)
...
namespace Metasound::Frontend
{
	...
	{
		static const FLazyName AccessFlags = "AssetFlags";
...
namespace Metasound
{
	namespace Frontend
	{
			...
			: bIsValid(0)
		{
			...
			{
				int32 AccessFlagsValue = 0;
				const bool bFoundFlags = InAssetData.GetTagValue(ClassTagsPrivate::AccessFlags, AccessFlagsValue);
				if (bFoundFlags)
				{
					AccessFlags = static_cast<EMetasoundFrontendClassAccessFlags>((uint16)(AccessFlagsValue));
				}
				else
				{
					// When the referenceable flag was added, the prior document version assumed everything was referenceable.
					// Therefore, we can reliably set this flag as that is the default that the document version process inevitably
					// will upgrade the flag to.  Setting here enables caller to at least know it should be shown to end users or
					// is available by default for referencing if out-of-date.
					AccessFlags = EMetasoundFrontendClassAccessFlags::Referenceable;
				}
				bSuccess &= bFoundFlags;
			}
		...
		void FMetaSoundClassInfo::ExportToContext(FAssetRegistryTagsContext& OutContext) const
		{
			...
			OutContext.AddTag(FAssetRegistryTag(ClassTagsPrivate::AccessFlags, FString::Printf(TEXT("%i"), (int32)(AccessFlags)), FAssetRegistryTag::TT_Hidden));
		...
		void FMetaSoundClassInfo::InitFromDocument(const IMetaSoundDocumentInterface& InDocInterface)
		{
			...
			AccessFlags = Document.RootGraph.Metadata.GetAccessFlags();

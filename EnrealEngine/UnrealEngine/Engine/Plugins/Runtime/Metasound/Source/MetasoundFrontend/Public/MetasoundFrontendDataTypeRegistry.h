...
namespace Metasound
{
	...
	namespace Frontend
	{
		...
		struct IPolymorphicDataTypeInterface;
		...
		public:
			...
			UE_EXPERIMENTAL(5.7, "Metasound Polymorphic Datatypes are still in devleopement")
			virtual TSharedPtr<IPolymorphicDataTypeInterface> GetPolymorphicInterface() const = 0;
		...
		/**
		 * IPolymorphicDataTypeInterface.
		 * Experimental API. 
		 */	
		struct IPolymorphicDataTypeInterface
		{
			virtual ~IPolymorphicDataTypeInterface() = default;
			UE_EXPERIMENTAL(5.7, "Metasound Polymorphic Datatypes are still in devleopement")
			virtual bool IsA(const FName InDataTypeName, const IDataTypeRegistry& InRegistry = IDataTypeRegistry::Get() ) const = 0;
		};

		/**
		 * FPolymorphicDataTypeBase 
		 * Experimental API. 
		 */
		class FPolymorphicDataTypeBase : public IPolymorphicDataTypeInterface
		{
		public:
			UE_API FPolymorphicDataTypeBase(const FName InDataTypeName, const FName InParentName = NAME_None, const bool InbIsAbstract = false);
			virtual ~FPolymorphicDataTypeBase() override = default;

			UE_EXPERIMENTAL(5.7, "Metasound Polymorphic Datatypes are still in developement")			
			UE_API virtual bool IsA(const FName InDataTypeName, const IDataTypeRegistry& InRegistry) const override;
		private:
			void ResolveParent(const IDataTypeRegistry& InRegistry) const;
			FName DataTypeName = NAME_None;
			FName ParentTypeName = NAME_None;
			bool bIsAbstract = false;
			mutable TWeakPtr<const IPolymorphicDataTypeInterface> ParentDataType;
		};

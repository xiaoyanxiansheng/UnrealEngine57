...
namespace Metasound
{
	...
	public:
		...
		static UE_API FSendAddress CreateSendAddressFromEnvironment(const FMetasoundEnvironment& InEnvironment, const FVertexName& InVertexName, const FName& InTypeName);
		
		/** Creates a unique send address using the given InstanceID. */
		UE_DEPRECATED(5.3, "This function is no longer used. Send addresses are no longer used when communicating with MetaSound inputs")
		static UE_API FSendAddress CreateSendAddressFromInstanceID(uint64 InInstanceID, const FVertexName& InVertexName, const FName& InTypeName);

		UE_API FMetaSoundParameterTransmitter(FMetaSoundParameterTransmitter::FInitParams&& InInitParams);
		virtual ~FMetaSoundParameterTransmitter() = default;

		UE_API void AddAvailableParameter(FName InName);
		UE_API void RemoveAvailableParameter(FName InName);

		/** Sets parameters using array of AudioParameter structs
		 *
		 * @param InParameter - Parameter to set.
		 */
		UE_API virtual bool SetParameters(TArray<FAudioParameter>&& InParameters) override;
		 ...
		 */
		UE_API bool SetParameterWithLiteral(FName InParameterName, const FLiteral& InValue);
	...
	private:
		 ...
		 */
		UE_API bool SetVirtualizedParameters(TArray<FAudioParameter>&& InParameters);

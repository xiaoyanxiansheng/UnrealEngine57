...
namespace Metasound
{
	...
	class FLiteralNode : public FNode
	{
	protected:
		static UE_API FVertexInterface CreateVertexInterface(const FName& InDataTypeName, EVertexAccessType InAccessType);

	public:

		static UE_API FNodeClassMetadata GetNodeMetadata(const FName& InDataTypeName, EVertexAccessType InAccessType);

		UE_API FLiteralNode(const FVertexName& InInstanceName, const FGuid& InInstanceID, const FName& InDataTypeName, EVertexAccessType InAccessType, FOperatorFactorySharedRef InFactory);

...
namespace Metasound
{
	namespace Frontend
	{
		...
		namespace MetasoundFrontendDocumentAccessPtrPrivate
		{
			...
			{
			 	TArray<FMetasoundFrontendClassInput>& operator()(FMetasoundFrontendClass& InClass) const { return InClass.GetDefaultInterface().Inputs; }
			 	const TArray<FMetasoundFrontendClassInput>& operator()(const FMetasoundFrontendClass& InClass) const { return InClass.GetDefaultInterface().Inputs; }
			};

			/** A callable object for accessing arrays of outputs from a class. */
			struct FGetClassOutputArray 
			{
			 	TArray<FMetasoundFrontendClassOutput>& operator()(FMetasoundFrontendClass& InClass) const { return InClass.GetDefaultInterface().Outputs; }
			 	const TArray<FMetasoundFrontendClassOutput>& operator()(const FMetasoundFrontendClass& InClass) const { return InClass.GetDefaultInterface().Outputs; }
			};

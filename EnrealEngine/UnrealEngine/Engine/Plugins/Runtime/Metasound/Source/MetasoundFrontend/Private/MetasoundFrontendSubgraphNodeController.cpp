	...
	namespace Frontend
	{
		...
		void FSubgraphNodeController::ConformNodeInterfaceToClassInterface() 
		{
			...
			if (FMetasoundFrontendNode* Node = NodePtr.Get())
			{
				...
				{
					ConformNodeVertexArrayToClassVertexArray(Node->Interface.Inputs, ClassInterface->Inputs);
					ConformNodeVertexArrayToClassVertexArray(Node->Interface.Outputs, ClassInterface->Outputs);

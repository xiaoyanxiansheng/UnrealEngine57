...
namespace Metasound::Frontend
{
	...
	FDocumentGraphInterfaceCache::FDocumentGraphInterfaceCache(TSharedRef<const FDocumentCache> ParentCache)
		: Parent(ParentCache)
	{
		const FMetasoundFrontendGraphClass& GraphClass = ParentCache->GetDocument().RootGraph;
	...
	const FMetasoundFrontendClassInput* FDocumentGraphInterfaceCache::FindInput(FName InputName) const
	{
		...
		{
			if (TSharedPtr<const FDocumentCache> ParentCache = Parent.Pin())
			{
				const FMetasoundFrontendDocument& Doc = ParentCache->GetDocument();
	...
	void FDocumentGraphInterfaceCache::Init(FInterfaceModifyDelegates& OutDelegates)
	{
		...
		OutDelegates.OnInputInheritsDefaultChanged.AddSP(this, &FDocumentGraphInterfaceCache::OnInputInheritsDefaultChanged);

#if WITH_EDITOR
		OutDelegates.OnInputDisplayNameChanged.AddSP(this, &FDocumentGraphInterfaceCache::OnInputDisplayNameChanged);
		OutDelegates.OnInputDescriptionChanged.AddSP(this, &FDocumentGraphInterfaceCache::OnInputDescriptionChanged);
		OutDelegates.OnInputSortOrderIndexChanged.AddSP(this, &FDocumentGraphInterfaceCache::OnInputSortOrderIndexChanged);
		OutDelegates.OnInputIsAdvancedDisplayChanged.AddSP(this, &FDocumentGraphInterfaceCache::OnInputIsAdvancedDisplayChanged);
#endif // WITH_EDITOR
...
#if WITH_EDITOR
		OutDelegates.OnOutputDisplayNameChanged.AddSP(this, &FDocumentGraphInterfaceCache::OnOutputDisplayNameChanged);
		OutDelegates.OnOutputDescriptionChanged.AddSP(this, &FDocumentGraphInterfaceCache::OnOutputDescriptionChanged);
		OutDelegates.OnOutputSortOrderIndexChanged.AddSP(this, &FDocumentGraphInterfaceCache::OnOutputSortOrderIndexChanged);
		OutDelegates.OnOutputIsAdvancedDisplayChanged.AddSP(this, &FDocumentGraphInterfaceCache::OnOutputIsAdvancedDisplayChanged);
#endif // WITH_EDITOR
	}
	...
	void FDocumentGraphInterfaceCache::OnInputAdded(int32 NewIndex)
	{
		if (TSharedPtr<const FDocumentCache> ParentCache = Parent.Pin())
		{
			const FMetasoundFrontendDocument& Document = ParentCache->GetDocument();
	...
	void FDocumentGraphInterfaceCache::OnInputInheritsDefaultChanged(int32 NewIndex)
	{
		++TransactionCount;
	}

#if WITH_EDITOR
	void FDocumentGraphInterfaceCache::OnInputDescriptionChanged(int32 Index)
	{
		++TransactionCount;
	}

	void FDocumentGraphInterfaceCache::OnInputSortOrderIndexChanged(int32 Index)
	{
		++TransactionCount;
	}

	void FDocumentGraphInterfaceCache::OnInputIsAdvancedDisplayChanged(int32 Index)
	{
		++TransactionCount;
	}

	void FDocumentGraphInterfaceCache::OnOutputDisplayNameChanged(int32 Index)
	{
		++TransactionCount;
	}
#endif // WITH_EDITOR
...
#if WITH_EDITOR
	void FDocumentGraphInterfaceCache::OnInputDisplayNameChanged(int32 Index)
	{
		++TransactionCount;
	}

	void FDocumentGraphInterfaceCache::OnOutputDescriptionChanged(int32 Index)
	{
		++TransactionCount;
	}

	void FDocumentGraphInterfaceCache::OnOutputSortOrderIndexChanged(int32 Index)
	{
		++TransactionCount;
	}

	void FDocumentGraphInterfaceCache::OnOutputIsAdvancedDisplayChanged(int32 Index)
	{
		++TransactionCount;
	}
#endif // WITH_EDITOR
	...
	FDocumentGraphNodeCache::FDocumentGraphNodeCache(TSharedRef<const FDocumentCache> ParentCache, const FGuid& InPageID)
		...
		, PageID(InPageID)
	{
		const FMetasoundFrontendGraph& Graph = ParentCache->GetDocument().RootGraph.FindConstGraphChecked(InPageID);
		const TArray<FMetasoundFrontendNode>& Nodes = Graph.Nodes;
		for (int32 Index = 0; Index < Nodes.Num(); ++Index)
		{
			const FMetasoundFrontendNode& Node = Nodes[Index];
			const FMetasoundFrontendClass* Class = ParentCache->FindDependency(Node.ClassID);
			check(Class);
	...
	TSharedRef<FDocumentGraphNodeCache> FDocumentGraphNodeCache::Create(TSharedRef<const FDocumentCache> ParentCache, const FGuid& InPageID, FDocumentModifyDelegates& OutDelegates)
	{
		...
		Cache->Init(OutDelegates.FindGraphDelegatesChecked(InPageID).NodeDelegates);
		return Cache;
	}
		...
		if (const FMetasoundFrontendNode* Node = FindNode(InNodeID))
		{
			if (TSharedPtr<const FDocumentCache> ParentCache = Parent.Pin())
			{
				...
				if (Class->Metadata.GetClassName() == FRerouteNodeTemplate::ClassName)
				{
					...
					const IDocumentGraphEdgeCache& EdgeCache = ParentCache->GetEdgeCache(PageID);
	...
	FDocumentGraphEdgeCache::FDocumentGraphEdgeCache(TSharedRef<const FDocumentCache> ParentCache, const FGuid& InPageID)
		...
		, PageID(InPageID)
	{
		const TArray<FMetasoundFrontendEdge>& Edges = ParentCache->GetDocument().RootGraph.FindConstGraphChecked(InPageID).Edges;
	...
	TSharedRef<FDocumentGraphEdgeCache> FDocumentGraphEdgeCache::Create(TSharedRef<const FDocumentCache> ParentCache, const FGuid& InPageID, FDocumentModifyDelegates& OutDelegates)
	{
		...
		Cache->Init(OutDelegates.FindGraphDelegatesChecked(InPageID).EdgeDelegates);
		return Cache;
	}
...
#if !NO_LOGGING
			if (IDToIndex.Contains(Class.ID))
			{
				UE_LOG(LogMetaSound, Display, TEXT("MetaSound document with class name %s found containing multiple dependencies with the same ID while initializing builder cache. Document may be currently versioning or require versioning past version 1.15"), *InDocument.RootGraph.Metadata.GetClassName().ToString());
			}
#endif // !NO_LOGGING

...
#define UE_API METASOUNDENGINETEST_API
...
namespace Metasound::Test
{
	...
	public:
		...
		*/
		UE_API Frontend::FNodeHandle AddNode(const FNodeClassName& ClassName, int32 MajorVersion) const;
		...
		*/
		UE_API Frontend::FNodeHandle GetInput(const FName& Name) const;
		...
		* @return				NodeHandle, which can be invalid if an input with the given name doesn't exist in the graph
		*/
		UE_API Frontend::FNodeHandle GetOutput(const FName& Name) const;
		...
		*/
		UE_API Frontend::FNodeHandle AddInput(
		...
		* @return				NodeHandle of the added node, can be invalid if failed to add
		*/
		UE_API Frontend::FNodeHandle AddOutput(const FName& OutputName, const FName& TypeName);
		...
		*/
		UE_API bool AddAndConnectDataReferenceInputs(const Frontend::FNodeHandle& NodeHandle);
		...
		* @return				NodeHandle of the added node, can be invalid if failed to add
		*/
		UE_API Frontend::FNodeHandle AddAndConnectDataReferenceInput(const Frontend::FNodeHandle& NodeToConnect, const FName& InputName, const FName& TypeName, const FName& NodeName = NAME_None) const;
		...
		* @return				NodeHandle of the added node, can be invalid if failed to add
		*/
		UE_API Frontend::FNodeHandle AddAndConnectDataReferenceOutput(const Frontend::FNodeHandle& NodeToConnect, const FName& OutputName, const FName& TypeName, const FName& NodeName = NAME_None);
		...
		*/
		UE_API TUniquePtr<FMetasoundGenerator> BuildGenerator(FSampleRate SampleRate = 48000, int32 SamplesPerBlock = 256) const;
		...
		* @return					A unique pointer to the Metasound Generator created
		*/
		static UE_API TUniquePtr<FMetasoundGenerator> MakeSingleNodeGraph(
		...
		*/
		static UE_API bool ConnectNodes(const Frontend::FNodeHandle& LeftNode, const FName& OutputName, const Frontend::FNodeHandle& RightNode, const FName& InputName);
		...
		* @bool						true if able to connect the nodes (ie. all nodes were valid, all input/outputs were valid)
		*/
		static UE_API bool ConnectNodes(const Frontend::FNodeHandle& LeftNode, const Frontend::FNodeHandle& RightNode, const FName& InputOutputName);

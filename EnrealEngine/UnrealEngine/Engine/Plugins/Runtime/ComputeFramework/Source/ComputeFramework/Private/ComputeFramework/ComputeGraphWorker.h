...
public:
	...
	void SubmitWork(FComputeContext& Context) override;
...
private:
	...
	struct FGraphInvocation
	{
		...
		/** Whether render captures are enabled. */
	...
	struct FSubmitDescription
	{
		 ...
		 * In future we may want to add a limit to the number of graphs in flight to avoid memory pressure.
		 */
		union
		{
		...
		uint32 bIsUnified : 1;
		/** Whether a render capture should be triggered in the scope around this submit. */
		uint32 bTriggerRenderCapture : 1;
	};

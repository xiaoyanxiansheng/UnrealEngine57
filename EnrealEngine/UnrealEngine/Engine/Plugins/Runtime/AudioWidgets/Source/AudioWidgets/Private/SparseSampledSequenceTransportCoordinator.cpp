...
void FSparseSampledSequenceTransportCoordinator::MoveWaveformView(float MoveAmount)
{
	double MinDisplayRangeValue = DisplayRange.GetLowerBoundValue() + (MoveAmount * ZoomRatio);
	double MaxDisplayRangeValue = DisplayRange.GetUpperBoundValue() + (MoveAmount * ZoomRatio);

	//Clamp zoom bounds while maintaining same width if bounds go outside 0-1 range
	if (MinDisplayRangeValue < 0.0)
	{
		const double ExceedingDelta = FMath::Abs(MinDisplayRangeValue);
		MaxDisplayRangeValue = FMath::Clamp(MaxDisplayRangeValue + ExceedingDelta, 0.0, 1.0);
		MinDisplayRangeValue = 0.0;
	}

	if (MaxDisplayRangeValue > 1.0)
	{
		const double ExceedingDelta = MaxDisplayRangeValue - 1.0;

		MinDisplayRangeValue = FMath::Clamp(MinDisplayRangeValue - ExceedingDelta, 0.0, 1.0);
		MaxDisplayRangeValue = 1.0;
	}

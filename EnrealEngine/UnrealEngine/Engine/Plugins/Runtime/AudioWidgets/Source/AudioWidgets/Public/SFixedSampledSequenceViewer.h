...
class SFixedSampledSequenceViewer : public SLeafWidget
{
public:
	...
	UE_API void Construct(const FArguments& InArgs, TArrayView<const float> InSampleData, const uint8 InNumChannels, TSharedRef<IFixedSampledSequenceGridService> InGridService);
	UE_API void UpdateView(TArrayView<const float> InSampleData, const uint8 InNumChannels);
	
	UE_API void OnStyleUpdated(const FSampledSequenceViewerStyle UpdatedStyle);

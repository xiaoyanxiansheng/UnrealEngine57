...
void SAudioSliderBase::SetSliderValue(TAttribute<float> InSliderValue)
{
	SliderValueAttribute = MoveTemp(InSliderValue);
	if (Slider.IsValid())
	{
		Slider->SetValue(SliderValueAttribute);
	}

	const float Current = SliderValueAttribute.Get();
	Label->SetValueText(GetOutputValueForText(Current));
}
...
void SAudioSliderBase::SetOutputRange(const FVector2D Range)
{
	...
	// Cache these values to handle case where we have a bound slider value attribute
	const float OldOutputValueNormalized = SliderValueAttribute.Get();
	const float OldOutputValueMapped = GetOutputValue(OldOutputValueNormalized);

	const float NewClampedOutputValue = FMath::Clamp(OldOutputValueMapped, OutputRange.X, OutputRange.Y);
	const float NewNormalizedValue = GetSliderValue(NewClampedOutputValue);

	Label->SetValueText(GetOutputValueForText(NewNormalizedValue));
	Label->UpdateValueTextWidth(OutputRange);

	if (!FMath::IsNearlyEqual(OldOutputValueNormalized, NewNormalizedValue))
	{
		// Avoid mutating the SliderValueAttribute if we're bound to avoid severing the binding
		if (SliderValueAttribute.IsBound())
		{
			OnValueChanged.ExecuteIfBound(NewNormalizedValue);
			OnValueCommitted.ExecuteIfBound(NewNormalizedValue);
		}
		else
		{
			SliderValueAttribute.Set(NewNormalizedValue);
			if (Slider.IsValid())
			{
				Slider->SetValue(SliderValueAttribute);
			}
...
// SAudioVolumeSlider
const float SAudioVolumeSlider::MinDbValue = MIN_VOLUME_DECIBELS;

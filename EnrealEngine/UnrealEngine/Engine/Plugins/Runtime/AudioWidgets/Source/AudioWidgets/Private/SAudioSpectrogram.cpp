...
void SAudioSpectrogram::Construct(const FArguments& InArgs)
{
	...
	OnColorMapMenuEntryClicked = InArgs._OnColorMapMenuEntryClicked;
	OnOrientationMenuEntryClicked = InArgs._OnOrientationMenuEntryClicked;
...
int32 SAudioSpectrogram::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
	...
	{
		.NumRows = FMath::Max((bRotateViewport) ? RenderWidth : RenderHeight, 1),
	...
	if (bFillBackground.Get())
	{
		// Fill allotted geometry with colormap minimum value:
		const FSlateBrush BackgroundBrush;
		const FLinearColor BoxColor = FAudioColorMapper(0.0f, 1.0f, RenderParams.ColorMap).GetColorFromValue(0.0f);
...
TSharedRef<SWidget> SAudioSpectrogram::BuildDefaultContextMenu()
{
	...
	if (OnColorMapMenuEntryClicked.IsBound() || !ColorMap.IsBound())
	{
		MenuBuilder.AddSubMenu(
	...
	if (OnOrientationMenuEntryClicked.IsBound() || !Orientation.IsBound())
	{
		MenuBuilder.AddSubMenu(
				...
				FExecuteAction::CreateSPLambda(this, [this, EnumValue]()
					{
						...
						OnColorMapMenuEntryClicked.ExecuteIfBound(EnumValue);
					}),
				FCanExecuteAction(),
				...
				FExecuteAction::CreateSPLambda(this, [this, EnumValue]()
					{
						...
						OnOrientationMenuEntryClicked.ExecuteIfBound(EnumValue);
					}),
				FCanExecuteAction(),

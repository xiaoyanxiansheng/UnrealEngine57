...
#include UE_INLINE_GENERATED_CPP_BY_NAME(SAudioOscilloscopePanelWidget)
...
namespace AudioOscilloscopePanelWidgetPrivate
{
	...
	TFunction<FText(const double)> YAxisValueGeneratorDb = [](const double InLabelValue)
	{
		...
		const FText GridLineValueText = (GridLineValue <= MIN_VOLUME_DECIBELS) ? FText::FromString("-inf") : FText::AsNumber(GridLineValue, &Formatting);

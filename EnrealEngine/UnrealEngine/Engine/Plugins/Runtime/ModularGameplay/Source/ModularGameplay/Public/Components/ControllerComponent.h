...
public:
	...
	UE_API UControllerComponent(const FObjectInitializer& ObjectInitializer);
	...
	UE_API bool IsLocalController() const;

	/** Returns the point of view for either a player or controlled pawn */
	UE_API void GetPlayerViewPoint(FVector& Location, FRotator& Rotation) const;

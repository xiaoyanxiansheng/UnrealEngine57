...
bool UAITask_UseGameplayBehaviorSmartObject::StartInteraction()
{
	...
	const UGameplayBehaviorConfig* GameplayBehaviorConfig = SmartObjectGameplayBehaviorDefinition != nullptr ? SmartObjectGameplayBehaviorDefinition->GameplayBehaviorConfig.Get() : nullptr;
...
void UAITask_UseGameplayBehaviorSmartObject::OnDestroy(const bool bInOwnerFinished)
{
	...
	{
		if (ensureMsgf(IsValid(OwnerController), TEXT("OwnerController is expected to always be valid")))
		{
			USmartObjectSubsystem* SmartObjectSubsystem = USmartObjectSubsystem::GetCurrent(OwnerController->GetWorld());
...
void UAITask_UseGameplayBehaviorSmartObject::Abort()
{
	...
	else if (!bBehaviorFinished)
	{
		if (GameplayBehavior != nullptr)
		{
			...
			// Check for a valid pawn first - this is used by the 'shutdown' path so pawn is not guaranteed to still be valid.
			if (APawn* OwnerPawn = OwnerController->GetPawn())
			{
				GameplayBehavior->AbortBehavior(*OwnerPawn);
			}
		}
	}
...
{
	ensureMsgf(IsValid(OwnerController), TEXT("OwnerController is expected to always be valid"));

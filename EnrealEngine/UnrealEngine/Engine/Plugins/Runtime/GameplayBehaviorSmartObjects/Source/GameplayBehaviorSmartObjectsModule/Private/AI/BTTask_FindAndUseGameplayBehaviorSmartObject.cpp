...
EBTNodeResult::Type UBTTask_FindAndUseGameplayBehaviorSmartObject::AbortTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	...
	if (MyMemory->EQSRequestID != INDEX_NONE)
	{
		// Clear before calling AbortQuery since it will call us back through OnTaskFinished
		// which uses a check to make sure that the request ID was cleared before reaching that point.
		const int32 EQSRequestID = MyMemory->EQSRequestID;
		MyMemory->EQSRequestID = INDEX_NONE;
		...
		if (UWorld* World = OwnerComp.GetWorld())
		{
			...
			{
				EnvQueryManager->AbortQuery(EQSRequestID);
			}
		}
	}

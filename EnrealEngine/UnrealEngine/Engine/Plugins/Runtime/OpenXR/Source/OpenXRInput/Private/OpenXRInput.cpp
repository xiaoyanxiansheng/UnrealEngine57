...
bool FOpenXRInputPlugin::FOpenXRInput::BuildActions(XrSession Session)
{
	...
	{
		auto AddProfile = [&Profiles](const char* const Profile, const char* const PathChars, bool HasHaptics)
			{
				FOpenXRPath Path = FOpenXRPath(PathChars);
				if (XR_NULL_PATH == Path.ToXRPath())
				{
					UE_LOG(LogHMD, Warning, TEXT("BuildActions tried to AddProfile: '%s' Path: '%s', but it looks like the path creation failed. Profile not added. Look above for path related error messages."), ANSI_TO_TCHAR(Profile), ANSI_TO_TCHAR(PathChars));
					return;
				}
				Profiles.Add(Profile, FInteractionProfile(Path, HasHaptics));
			};
		//AddProfile("MalformedPathTest", "MALFORMEDPATH *!,/", false); // malformed path, should fail with XR_ERROR_PATH_FORMAT_INVALID = -21
		AddProfile("SimpleController", "/interaction_profiles/khr/simple_controller", true);
		AddProfile("Vive", "/interaction_profiles/htc/vive_controller", true);
		AddProfile("MixedReality", "/interaction_profiles/microsoft/motion_controller", true);
		AddProfile("OculusGo", "/interaction_profiles/oculus/go_controller", false);
		AddProfile("OculusTouch", "/interaction_profiles/oculus/touch_controller", true);
		AddProfile("ValveIndex", "/interaction_profiles/valve/index_controller", true);
	}
	...
	{
		UE_LOG(LogHMD, Warning, TEXT("No mapping context provided in the OpenXR Input project settings, action bindings will not be visible to the OpenXR runtime. Action/Axis mappings from the Input configuration are deprecated for XR in favor of Enhanced Input."));

...
#include UE_INLINE_GENERATED_CPP_BY_NAME(GooglePADFunctionLibrary)
...
void UGooglePADFunctionLibrary::Initialize()
{
	...
	if (UseGooglePAD)
	{
		AssetPackErrorCode result = AssetPackManager_init(GJavaVM, FJavaWrapper::GameActivityThis);
...
EGooglePADErrorCode UGooglePADFunctionLibrary::ShowConfirmationDialog()
{
	EGooglePADErrorCode result = EGooglePADErrorCode::AssetPack_UNAVAILABLE;
#if SUPPORTED_PLATFORM
	AssetPackErrorCode ErrorCode = AssetPackManager_showConfirmationDialog(FJavaWrapper::GameActivityThis);
	result = ConvertErrorCode(ErrorCode);
#endif
	return result;
}

...
bool FAVPlayerMovieStreamer::Tick(FRHICommandListBase& RHICmdList, float DeltaTime)
{
    ...
    if( bVideoTracksLoaded )
    {
		...
		CheckForNextFrameAndCopy(RHICmdList);
...
bool FAVPlayerMovieStreamer::CheckForNextFrameAndCopy(FRHICommandListBase& RHICmdList)
{
    ...
    if( SyncStatus == Ready )
    {
		...
		FTextureRHIRef MediaWrappedTexture = MetalDynamicRHI->RHICreateTexture2DFromCVMetalTexture(PF_B8G8R8A8, ETextureCreateFlags::NoTiling | ETextureCreateFlags::ShaderResource, FClearValueBinding::White, TextureCacheRef);

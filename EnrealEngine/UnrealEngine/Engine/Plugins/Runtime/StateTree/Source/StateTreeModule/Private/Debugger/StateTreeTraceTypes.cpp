...
#include UE_INLINE_GENERATED_CPP_BY_NAME(StateTreeTraceTypes)
...
{
	return FString::Printf(TEXT("%s '%s': %s"), *UEnum::GetDisplayValueAsText(EventType).ToString(), *GetTypeString(StateTree), *GetValueString(StateTree));
}
...
FString FStateTreeTraceStateEvent::GetValueString(const UStateTree& StateTree) const
{
	...
	if (const FCompactStateTreeState* CompactState = StateTree.GetStateFromHandle(StateHandle))
	{
		TArray<FString, TInlineAllocator<FStateTreeActiveStates::MaxStates>> StateHierarchy;
		StateHierarchy.Push(UE::StateTreeTrace::GetStateName(StateTree, CompactState));

		// Climb hierarchy
		for (FStateTreeStateHandle ParentHandle = CompactState->Parent; ParentHandle.IsValid(); ParentHandle = CompactState->Parent)
		{
			if (CompactState = StateTree.GetStateFromHandle(ParentHandle); CompactState != nullptr)
			{
				StateHierarchy.Push(UE::StateTreeTrace::GetStateName(StateTree, CompactState));
			}
			else

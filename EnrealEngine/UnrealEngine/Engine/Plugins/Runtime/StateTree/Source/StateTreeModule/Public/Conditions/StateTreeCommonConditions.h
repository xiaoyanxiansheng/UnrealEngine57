...
STATETREE_POD_INSTANCEDATA(FStateTreeCompareNameConditionInstanceData);

/**
 * Condition comparing two FNames.
 */
USTRUCT(DisplayName = "Name Compare")
struct FStateTreeCompareNameCondition : public FStateTreeConditionCommonBase
{
	GENERATED_BODY()

	using FInstanceDataType = FStateTreeCompareNameConditionInstanceData;

	FStateTreeCompareNameCondition() = default;
	explicit FStateTreeCompareNameCondition(const EStateTreeCompare InInverts)
	: bInvert(InInverts == EStateTreeCompare::Invert)
	{}

	FStateTreeCompareNameCondition(const bool bInInverts)
	: bInvert(bInInverts)
	{}

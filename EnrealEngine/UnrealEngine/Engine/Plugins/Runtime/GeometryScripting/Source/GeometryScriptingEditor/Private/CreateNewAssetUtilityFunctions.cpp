...
namespace UE::CreateNewAssetUtilityFunctionsLocals
{
	static bool IsAssetPathValid(const FString& Path, bool bLogFailureReason = true)
	{
		FText BadPackageNameReason;
		if (!FPackageName::IsValidTextForLongPackageName(Path, &BadPackageNameReason))
		{
			if (bLogFailureReason)
			{
				MakeScriptError(EGeometryScriptErrorType::InvalidInputs,
					FText::Format(LOCTEXT("GeometryScript_BadPackageName", "Invalid Package Name '{0}' (Reason: {1})"),
						FText::FromString(Path), BadPackageNameReason)
				);
			}
			return false;
		}
	...
	UGeometryScriptDebug* Debug)
{
	...
	while (Attempts++ < 10)
	{
		...
		if (!UE::CreateNewAssetUtilityFunctionsLocals::IsAssetPathValid(UniqueAssetPathAndName))
		{
			// path has invalid format; early out here b/c changing the random ID won't fix this
			return;
		}
	...
	UGeometryScriptDebug* Debug)
{
	...
	if (!UE::CreateNewAssetUtilityFunctionsLocals::IsAssetPathValid(AssetPathAndName))
	{
		return nullptr;
	}
	// todo: other safety checks

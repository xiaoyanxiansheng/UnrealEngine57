...
void UEditorGeometryGenerationManager::Tick(float DeltaTime)
{
	...
	// note: we have a separate to-rebuild array in case the process of the rebuild modifies the ActiveGeneratedActors array
	TArray<AGeneratedDynamicMeshActor*> ToRebuild;
	for (AGeneratedDynamicMeshActor* Actor : ActiveGeneratedActors)
	{
		if (Actor->IsValidLowLevel() && IsValid(Actor) && !Actor->IsUnreachable() && Actor->GetLevel() != nullptr && Actor->WantsRebuild())
		{
			ToRebuild.Add(Actor);
		}
	}
	for (AGeneratedDynamicMeshActor* Actor : ToRebuild)
	{

...
UDynamicMesh* UGeometryScriptLibrary_MeshRepairFunctions::SelectHiddenTrianglesFromOutside(
	UDynamicMesh* TargetMesh,
	...
	UGeometryScriptDebug* Debug)
{
	if (TargetMesh == nullptr)
	{
		UE::Geometry::AppendError(Debug, EGeometryScriptErrorType::InvalidInputs, LOCTEXT("SelectHiddenTrianglesFromOutside_InvalidInput", "SelectHiddenTrianglesFromOutside: TargetMesh is Null"));
		return TargetMesh;
	}

	TargetMesh->ProcessMesh([&Options, &IgnoreSelection, &TransparentSelection, &ResultSelection](const FDynamicMesh3& Mesh)
		{
			FExteriorVisibilitySampling SamplingParameters;
			SamplingParameters.bDoubleSided = Options.bDoubleSided;
			SamplingParameters.NumSearchDirections = Options.NumSearchDirections;
			SamplingParameters.SamplingDensity = Options.SampleSpacing;
			const bool bPerPolyGroup = Options.bPerPolyGroup;

			TSet<int32> TransparentTriangles, IgnoreTriangles;
			TransparentSelection.ProcessByTriangleID(Mesh, [&TransparentTriangles](int32 TriangleID)
				{
					TransparentTriangles.Add(TriangleID);
				});
			IgnoreSelection.ProcessByTriangleID(Mesh, [&IgnoreTriangles](int32 TriangleID)
				{
					IgnoreTriangles.Add(TriangleID);
				});
			auto IsTriTransparent = [&TransparentTriangles](int32 TID)
				{
					return TransparentTriangles.Contains(TID);
				};
			TArray<bool> SkipTris;
			if (!IgnoreTriangles.IsEmpty())
			{
				SkipTris.SetNumZeroed(Mesh.MaxTriangleID());
				for (int32 IgnoreTID : IgnoreTriangles)
				{
					SkipTris[IgnoreTID] = true;
				}
			}

			TArray<bool> ResultTriOccluded;

			if (!Options.RestrictToViewDirections.IsEmpty())
			{
				TArray<FVector3d> NormalizedRestrictDirections = Options.RestrictToViewDirections;
				for (FVector3d& V : NormalizedRestrictDirections)
				{
					V = Normalized(V);
				}

				TArray<FVector3d> FullTestDirections = FExteriorVisibilitySampling::GetRestrictedViewDirections(NormalizedRestrictDirections, Options.ViewDirectionAngleRange, Options.NumSearchDirections);
				FExteriorVisibilitySampling::ComputePerTriangleOcclusionFromDirections(Mesh, IsTriTransparent, ResultTriOccluded, bPerPolyGroup, NormalizedRestrictDirections, FullTestDirections, false, SamplingParameters, SkipTris);
			}
			else
			{
				FExteriorVisibilitySampling::ComputePerTriangleOcclusion(Mesh, IsTriTransparent, ResultTriOccluded, bPerPolyGroup, SamplingParameters, SkipTris);
			}

			
			// transfer boolean array to a geometry script selection result
			FGeometrySelection NewSelection;
			for (int32 TID = 0; TID < ResultTriOccluded.Num(); ++TID)
			{
				if (ResultTriOccluded[TID])
				{
					NewSelection.Selection.Add(FGeoSelectionID::MeshTriangle(TID).Encoded());
				}
			}
			ResultSelection.SetSelection(MoveTemp(NewSelection));
		}
	);

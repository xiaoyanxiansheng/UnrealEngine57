	...
	UGeometryScriptDebug* Debug)
{
	...
	// Get the transform from the component (if applicable)
	LocalToWorld = FTransform::Identity;
	if (USceneComponent* SceneComponent = Cast<USceneComponent>(FromObject))
	{
		LocalToWorld = SceneComponent->GetComponentTransform();
	}

	if (bUseComplexCollision)
	{
		// find the Complex Collision mesh interface
		IInterface_CollisionDataProvider* CollisionProvider = Cast<IInterface_CollisionDataProvider>(FromObject);
		// For static and skeletal mesh cases, try pulling from the asset if the component didn't give us collision data
		if (!CollisionProvider || !CollisionProvider->ContainsPhysicsTriMeshData(true))
		{
			if (USkeletalMeshComponent* SkeletalMeshComp = Cast<USkeletalMeshComponent>(FromObject))
			{
				USkeletalMesh* SkeletalMesh = SkeletalMeshComp->GetSkeletalMeshAsset();
				CollisionProvider = Cast<IInterface_CollisionDataProvider>(SkeletalMesh);
			}
			else if (UStaticMeshComponent* StaticMeshComp = Cast<UStaticMeshComponent>(FromObject))
			{
				UStaticMesh* StaticMesh = StaticMeshComp->GetStaticMesh();
				CollisionProvider = Cast<IInterface_CollisionDataProvider>(StaticMesh);
			}
		}

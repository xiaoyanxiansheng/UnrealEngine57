	...
	UGeometryScriptDebug* Debug)
{
	...
	TargetMesh->EditMesh([&](FDynamicMesh3& EditMesh) 
	{
		...
		if (EditMesh.Attributes()->PrimaryNormals()->ElementCount() == 0)
		{
			...
			EditMesh.Attributes()->PrimaryNormals()->CreatePerVertex(0.0f);
		}
	...
	UGeometryScriptDebug* Debug)
{
	...
	TargetMesh->EditMesh([&](FDynamicMesh3& EditMesh) 
	{
		...
		if (Options.Type == EGeometryScriptTangentTypes::StandardMikkT)
		{
			if (DynamicMeshMikkTWrapper::IsSupported())
			{
				FMeshTangentsd Tangents(&EditMesh);
				if (DynamicMeshMikkTWrapper::ComputeTangents(Tangents, Options.UVLayer))
				{
					Tangents.CopyToOverlays(EditMesh);
				}
				else
				{
					UE::Geometry::AppendError(Debug, EGeometryScriptErrorType::OperationFailed, LOCTEXT("ComputeTangents_MikkTFailed", "ComputeTangents: Standard MikkT failed to compute a result"));
				}
				return;
			}
			else
			{
				UE::Geometry::AppendWarning(Debug, EGeometryScriptErrorType::InvalidInputs, LOCTEXT("ComputeTangents_StandardMikkTEditorOnly", "ComputeTangents: Standard MikkT not supported by current platform; falling back to Fast MikkT."));
				Options.Type = EGeometryScriptTangentTypes::FastMikkT;
				// continue through below to compute FastMikkT
			}
		}
	...
	bool bDeferChangeNotifications)
{
	...
	auto UpdateOverlay = [VertexID, &TriVtxElements, &UniqueElementIDs, bMergeSplitNormals](FDynamicMesh3& EditMesh, FDynamicMeshNormalOverlay* Overlay, FVector NewValue)
	{
		...
		{
			// Note the UniqueElementIDs array must not be empty, since TriVtxElements was not empty
			Overlay->SetElement(UniqueElementIDs[0], (FVector3f)NewValue); //-V557
			for (FIndex3i TriInfo : TriVtxElements)
			{
				FIndex3i OverlayTri = Overlay->GetTriangle(TriInfo.A);
				OverlayTri[TriInfo.B] = UniqueElementIDs[0]; //-V557

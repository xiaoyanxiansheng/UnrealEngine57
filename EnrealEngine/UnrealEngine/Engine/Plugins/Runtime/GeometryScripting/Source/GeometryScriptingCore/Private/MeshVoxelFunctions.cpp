	...
	UGeometryScriptDebug* Debug)
{
	...
	TargetMesh->EditMesh([&](FDynamicMesh3& EditMesh)
	{
		...
		const bool bUseCustomBounds = (bool)Options.CustomBounds.IsValid;
		FAxisAlignedBox3d UseBounds = bUseCustomBounds ?
			FAxisAlignedBox3d(Options.CustomBounds) : EditMesh.GetBounds(true);

		if (bUseCustomBounds && UseBounds.Diagonal().GetMin() <= 0)
		{
			UE::Geometry::AppendError(Debug, EGeometryScriptErrorType::InvalidInputs, LOCTEXT("ApplyMeshSolidify_EmptyCustomBounds", "ApplyMeshSolidify: Empty custom bounds"));
			EditMesh.Clear();
			return;
		}
		...
		if (Options.bThickenShells)
		{
			...
			double CellSize = (Options.GridParameters.SizeMethod == EGeometryScriptGridSizingMethod::GridResolution) ?
				(UseBounds.MaxDim() / Options.GridParameters.GridResolution) : Options.GridParameters.GridCellSize;
		...
		Solidify.bUseCustomBounds = bUseCustomBounds;
		Solidify.CustomBounds = UseBounds;
		...
		if (Options.GridParameters.SizeMethod == EGeometryScriptGridSizingMethod::GridResolution)
		{
			...
			Solidify.SetCellSizeAndExtendBounds(UseBounds, Options.ExtendBounds, ClampResolution);
		}
		else
		{
			double UseCellSize = GetClampedCellSize(Options.GridParameters.GridCellSize, UseBounds, TEXT("ApplyMeshSolidify"), Debug);
	...
	UGeometryScriptDebug* Debug)
{
	...
	TargetMesh->EditMesh([&](FDynamicMesh3& EditMesh) 
	{
		...
		const bool bUseCustomBounds = (bool)Options.CustomBounds.IsValid;
		FAxisAlignedBox3d UseBounds = bUseCustomBounds ?
			FAxisAlignedBox3d(Options.CustomBounds) : Spatial.GetBoundingBox();

		if (bUseCustomBounds && UseBounds.Diagonal().GetMin() <= 0)
		{
			UE::Geometry::AppendError(Debug, EGeometryScriptErrorType::InvalidInputs, LOCTEXT("ApplyMeshMorphology_EmptyCustomBounds", "ApplyMeshMorphology: Empty custom bounds"));
			EditMesh.Clear();
			return;
		}
		
		Morphology.MorphologyOp = static_cast<TImplicitMorphology<FDynamicMesh3>::EMorphologyOp>(Options.Operation);
		Morphology.bUseCustomBounds = bUseCustomBounds;
		Morphology.CustomBounds = UseBounds;
		...
		if (SDFGridParameters.SizeMethod == EGeometryScriptGridSizingMethod::GridResolution)
		{
			...
			Morphology.SetGridCellSize(UseBounds, Options.Distance, ClampResolution);
		}
		else
		{
			double UseCellSize = GetClampedCellSize(SDFGridParameters.GridCellSize, UseBounds, TEXT("ApplyMeshMorphology"), Debug);
		...
		if (MeshGridParameters.SizeMethod == EGeometryScriptGridSizingMethod::GridResolution)
		{
			...
			Morphology.SetMeshCellSize(UseBounds, Options.Distance, ClampResolution);
		}
		else
		{
			double UseCellSize = GetClampedCellSize(MeshGridParameters.GridCellSize, UseBounds, TEXT("ApplyMeshMorphology"), Debug);

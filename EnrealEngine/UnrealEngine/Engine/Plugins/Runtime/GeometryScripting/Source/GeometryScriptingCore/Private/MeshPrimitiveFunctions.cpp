...
namespace UE::Private::AppendMeshHelper
{
	static UDynamicMesh* AppendBox(UDynamicMesh* TargetMesh,
		...
		const UE::Geometry::FOrientedBox3d& OrientedBox,
		int32 StepsX,
		int32 StepsY,
		int32 StepsZ,
		UGeometryScriptDebug* Debug)
	{
		...
		GridBoxGenerator.Box = OrientedBox;
		...
		if (SimpleCollision)
		{
			FVector3d Dims = Transform.GetScale3D() * 2 * OrientedBox.Extents;
			FKBoxElem BoxElem(Dims.X, Dims.Y, Dims.Z);
			BoxElem.Center = Transform.TransformPosition(OrientedBox.Center());
			BoxElem.Rotation = (Transform.GetRotation() * FQuat(OrientedBox.Frame.Rotation)).Rotator();
		...
		UGeometryScriptDebug* Debug)
	{
		UE::Geometry::FOrientedBox3d OrientedBox(
			FVector3d(0, 0, Origin == EGeometryScriptPrimitiveOriginMode::Base ? DimensionZ * .5f : 0.f),
			FVector3d(DimensionX * .5f, DimensionY * .5f, DimensionZ * .5f)); // Extents are half-dimensions
		return UE::Private::AppendMeshHelper::AppendBox(TargetMesh, SimpleCollision, PrimitiveOptions, Transform, OrientedBox, StepsX, StepsY, StepsZ, Debug);
	}
...
UDynamicMesh* UGeometryScriptLibrary_MeshPrimitiveFunctions::AppendBoxWithCollision(
	...
	UGeometryScriptDebug* Debug)
{
	SimpleCollision.AggGeom.EmptyElements(); // Clear the existing elements since BP can re-use these output structs
...
UDynamicMesh* UGeometryScriptLibrary_MeshPrimitiveFunctions::AppendOrientedBox(
	UDynamicMesh* TargetMesh,
	FGeometryScriptPrimitiveOptions PrimitiveOptions,
	FTransform Transform,
	...
	UGeometryScriptDebug* Debug)
{
	return UE::Private::AppendMeshHelper::AppendBox(TargetMesh, nullptr, PrimitiveOptions, Transform, 
		UE::Geometry::FOrientedBox3d(OrientedBox), StepsX, StepsY, StepsZ, Debug);
}

UDynamicMesh* UGeometryScriptLibrary_MeshPrimitiveFunctions::AppendOrientedBoxWithCollision(
	UDynamicMesh* TargetMesh,
	FGeometryScriptSimpleCollision& SimpleCollision,
	FGeometryScriptPrimitiveOptions PrimitiveOptions,
	FTransform Transform,
...
{
	SimpleCollision.AggGeom.EmptyElements(); // Clear the existing elements since BP can re-use these output structs
	return UE::Private::AppendMeshHelper::AppendBox(TargetMesh, &SimpleCollision, PrimitiveOptions, Transform,
		UE::Geometry::FOrientedBox3d(OrientedBox), StepsX, StepsY, StepsZ, Debug);
}

...
FOrientedBox UGeometryScriptLibrary_OrientedBoxFunctions::MakeOrientedBoxFromBox(FBox Box, FTransform Transform)
{
	FOrientedBox OrientedBox;
	FVector BoxCenter, BoxExtents;
	Box.GetCenterAndExtents(BoxCenter, BoxExtents);
	OrientedBox.Center = Transform.TransformPosition(BoxCenter);
	FVector OrientedExtents = (Transform.GetScale3D() * BoxExtents).GetAbs();
	OrientedBox.ExtentX = OrientedExtents.X;
	OrientedBox.ExtentY = OrientedExtents.Y;
	OrientedBox.ExtentZ = OrientedExtents.Z;
	OrientedBox.AxisX = Transform.GetUnitAxis(EAxis::X);
	OrientedBox.AxisY = Transform.GetUnitAxis(EAxis::Y);
	OrientedBox.AxisZ = Transform.GetUnitAxis(EAxis::Z);
	return OrientedBox;
}

FRotator UGeometryScriptLibrary_OrientedBoxFunctions::GetOrientedBoxOrientation(FOrientedBox Box)
{
	return FRotator(FQuat(FQuaterniond(FMatrix3d(Box.AxisX, Box.AxisY, Box.AxisZ, false))));
}


FVector UGeometryScriptLibrary_OrientedBoxFunctions::GetOrientedBoxCorner(FOrientedBox Box, int CornerIndex)
{
	CornerIndex = FMath::Clamp(CornerIndex, 0, 7);
	
	// matches Box.GetVertices() / GetBoxCorner() (and not the FOrientedBox3d GetCorner ordering)
	auto GetCornerDirection = [](int32 CornerIndex)
		{
			switch (CornerIndex)		
			{
			default:
			case 0: return FVector(-1.);
			case 1: return FVector(-1., -1., 1.);
			case 2: return FVector(-1., 1., -1.);
			case 3: return FVector(1., -1., -1.);
			case 4: return FVector(1., 1., -1.);
			case 5: return FVector(1., -1., 1.);
			case 6: return FVector(-1., 1., 1.);
			case 7: return FVector(1.);
			}
		};
	FVector Directions = GetCornerDirection(CornerIndex);
	return Box.Center
		+ Box.AxisX * Box.ExtentX * Directions.X
		+ Box.AxisY * Box.ExtentY * Directions.Y
		+ Box.AxisZ * Box.ExtentZ * Directions.Z;
}

FVector UGeometryScriptLibrary_OrientedBoxFunctions::GetOrientedBoxFaceCenter(FOrientedBox Box, int FaceIndex, FVector& FaceNormal)
{
	FaceIndex = FMath::Clamp(FaceIndex, 0, 5);
	switch (FaceIndex)
	{
	...
	case 0:
		...
		return Box.Center + FaceNormal * Box.ExtentZ;
	case 1:
		FaceNormal = Box.AxisZ;
		return Box.Center + FaceNormal * Box.ExtentZ;
	case 2:
		FaceNormal = -Box.AxisY;
		return Box.Center + FaceNormal * Box.ExtentY;
	case 3:
		FaceNormal = Box.AxisY;
		return Box.Center + FaceNormal * Box.ExtentY;
	case 4:
		FaceNormal = -Box.AxisX;
		return Box.Center + FaceNormal * Box.ExtentX;
	case 5:
		FaceNormal = Box.AxisX;
		return Box.Center + FaceNormal * Box.ExtentX;
	}
...
void UGeometryScriptLibrary_OrientedBoxFunctions::GetOrientedBoxVolumeArea(FOrientedBox Box, double& Volume, double& SurfaceArea)
{
	FVector Dimensions = FVector(Box.ExtentX, Box.ExtentY, Box.ExtentZ) * 2.;
	double AreaXY = Dimensions.X * Dimensions.Y;
	double AreaXZ = Dimensions.X * Dimensions.Z;
	double AreaYZ = Dimensions.Y * Dimensions.Z;
	SurfaceArea = 2.0 * AreaXY + 2.0 * AreaXZ + 2.0 * AreaYZ;
...
FOrientedBox UGeometryScriptLibrary_OrientedBoxFunctions::GetExpandedOrientedBox(FOrientedBox Box, FVector ExpandBy, double MinExtent)
{
	// Min Extents cannot be negative
	MinExtent = FMath::Max(0, MinExtent);

	// Expand extents by the provided vector, clamping at MinExtent in case of a negative ExpandBy amount
	Box.ExtentX = FMath::Max(MinExtent, Box.ExtentX + ExpandBy.X);
	Box.ExtentY = FMath::Max(MinExtent, Box.ExtentY + ExpandBy.Y);
	Box.ExtentZ = FMath::Max(MinExtent, Box.ExtentZ + ExpandBy.Z);
	return Box;
}

FOrientedBox UGeometryScriptLibrary_OrientedBoxFunctions::GetTransformedOrientedBox(FOrientedBox OrientedBox, FTransform Transform)
{
	FOrientedBox Result;
	Result.Center = Transform.TransformPosition(OrientedBox.Center);
	FVector ScaledExtents = (Transform.GetScale3D() * FVector(OrientedBox.ExtentX, OrientedBox.ExtentY, OrientedBox.ExtentZ)).GetAbs();
	Result.ExtentX = ScaledExtents.X;
	Result.ExtentY = ScaledExtents.Y;
	Result.ExtentZ = ScaledExtents.Z;
	Result.AxisX = Transform.TransformVectorNoScale(OrientedBox.AxisX);
	Result.AxisY = Transform.TransformVectorNoScale(OrientedBox.AxisY);
	Result.AxisZ = Transform.TransformVectorNoScale(OrientedBox.AxisZ);
	return Result;
}

FBox UGeometryScriptLibrary_OrientedBoxFunctions::ConvertOrientedBoxToAxisAlignedBox(FOrientedBox Box)
{
	FBox Result;

	FVector Size = FVector(Box.ExtentX, Box.ExtentY, Box.ExtentZ) * 2.;
	FVector Corners[8];
	Corners[0] = Box.Center - Box.AxisX * Box.ExtentX - Box.AxisY * Box.ExtentY - Box.ExtentZ * Box.AxisZ;
	Corners[7] = Box.Center + Box.AxisX * Box.ExtentX + Box.AxisY * Box.ExtentY + Box.ExtentZ * Box.AxisZ;
	Corners[1] = Corners[0] + Size.Z * Box.AxisZ;
	Corners[2] = Corners[0] + Size.Y * Box.AxisY;
	Corners[3] = Corners[0] + Size.X * Box.AxisX;
	Corners[4] = Corners[7] - Size.X * Box.AxisX;
	Corners[5] = Corners[7] - Size.Y * Box.AxisY;
	Corners[6] = Corners[7] - Size.Z * Box.AxisZ;
...
bool UGeometryScriptLibrary_OrientedBoxFunctions::TestPointInsideOrOnOrientedBox(FOrientedBox Box, FVector Point)
{
	UE::Geometry::FOrientedBox3d OBox(Box);
	return OBox.Contains(Point);
}

FVector UGeometryScriptLibrary_OrientedBoxFunctions::FindClosestPointOnOrientedBox(FOrientedBox Box, FVector Point, bool& bIsInside)
{
	UE::Geometry::FOrientedBox3d OBox(Box);
	bIsInside = OBox.Contains(Point);
	return OBox.ClosestPoint(Point);
}

bool UGeometryScriptLibrary_OrientedBoxFunctions::TestOrientedBoxOrientedBoxIntersection(FOrientedBox Box1, FOrientedBox Box2)
{
	UE::Geometry::FOrientedBox3d OBox1(Box1), OBox2(Box2);
	return OBox1.Intersects(OBox2);
}

bool UGeometryScriptLibrary_OrientedBoxFunctions::TestBoxOrientedBoxIntersection(FBox Box1, FOrientedBox OrientedBox2)
{
	UE::Geometry::FOrientedBox3d OBox2(OrientedBox2);
	UE::Geometry::FAxisAlignedBox3d ABox1(Box1);
	return OBox2.Intersects(ABox1);
}

...
public:
	 ...
	 */
	UFUNCTION(BlueprintPure, Category = "GeometryScript|Shapes|OrientedBox", meta = (KeyWords = "Transform"))
	static UE_API UPARAM(DisplayName="OrientedBox") FOrientedBox 
	MakeOrientedBoxFromBox(FBox Box, FTransform Transform);

	/**
	 * Get the orientation of an Oriented Box
	 */
	UFUNCTION(BlueprintPure, Category = "GeometryScript|Shapes|OrientedBox", meta=(ScriptMethod, KeyWords = "Rotation"))
	static UE_API UPARAM(DisplayName = "Orientation") FRotator GetOrientedBoxOrientation(FOrientedBox Box);

	/**
	 * Get the position of a corner of the Oriented Box. Corners are indexed from 0 to 7:
	 * Corner 0 is in the negative direction along each axis, 1/2/3 are along +Z/+Y/+X from the corner 0,
	 * 7 is the all-positive direction corner, and 4/5/6 are along -Z/-Y/-X from the corner 7.
	 */
	UFUNCTION(BlueprintPure, Category = "GeometryScript|Shapes|OrientedBox", meta=(ScriptMethod))
	static UE_API UPARAM(DisplayName="Corner Point") FVector 
	GetOrientedBoxCorner(FOrientedBox Box, int CornerIndex);

	/**
	 * Get the position of the center of a face of the Oriented Box. Faces are indexed from 0 to 5,
	 * using an ordering where 0/1 are the negative then positive Z-axis faces, 2/3 are Y-axis faces, and 4/5 are X-axis faces
	 * @param FaceNormal returned Normal vector of the identified face
	 */
	UFUNCTION(BlueprintPure, Category = "GeometryScript|Shapes|OrientedBox", meta=(ScriptMethod))
	static UE_API UPARAM(DisplayName="Center Point") FVector 
	GetOrientedBoxFaceCenter(FOrientedBox Box, int FaceIndex, FVector& FaceNormal);

	/**
	 * Get the Volume and Surface Area of an Oriented Box
	 */
	UFUNCTION(BlueprintPure, Category = "GeometryScript|Shapes|OrientedBox", meta=(ScriptMethod))
	static UE_API void GetOrientedBoxVolumeArea(FOrientedBox Box, double& Volume, double& SurfaceArea);

	/**
	 * Get the Oriented Box with extents expanded by the ExpandBy vector
	 * Can also shrink the box if negative ExpandBy values are provided, but will clamp to Max(0, MinExtent)
	 */
	UFUNCTION(BlueprintPure, Category = "GeometryScript|Shapes|OrientedBox", meta=(ScriptMethod, KeyWords = "Shrink"))
	static UE_API UPARAM(DisplayName="Expanded Oriented Box") FOrientedBox 
	GetExpandedOrientedBox(FOrientedBox OrientedBox, FVector ExpandBy, double MinExtent = 0);

	/**
	 * Apply the Transform to the Oriented Box
	 */
	UFUNCTION(BlueprintPure, Category = "GeometryScript|Shapes|OrientedBox", meta=(ScriptMethod))
	static UE_API UPARAM(DisplayName="Transformed Box") FOrientedBox 
	GetTransformedOrientedBox(FOrientedBox OrientedBox, FTransform Transform);

	/**
	 * Find the bounding Box enclosing the input Oriented Box.
	 */
	UFUNCTION(BlueprintPure, Category = "GeometryScript|Shapes|OrientedBox", meta=(ScriptMethod, KeyWords = "Bounding"))
	static UE_API UPARAM(DisplayName="Bounding Box") FBox 
	ConvertOrientedBoxToAxisAlignedBox(FOrientedBox Box);
	...
	UFUNCTION(BlueprintPure, Category = "GeometryScript|Shapes|OrientedBox", meta = (ScriptMethod))
	static UE_API UPARAM(DisplayName = "Is Inside") bool
	TestPointInsideOrOnOrientedBox(FOrientedBox Box, FVector Point);

	/**
	 * Find the point on the faces of the Box that is closest to the input Point.
	 * If the Point is inside the Box, it is returned, ie points Inside do not project to the Box Faces
	 * @param bIsInsideOrOn if the Point is inside or on the Box, this will return as true, otherwise false
	 */
	UFUNCTION(BlueprintPure, Category = "GeometryScript|Shapes|OrientedBox", meta = (ScriptMethod, KeyWords = "Project"))
	static UE_API UPARAM(DisplayName = "Closest Point") FVector
	FindClosestPointOnOrientedBox(FOrientedBox Box, FVector Point, bool& bIsInsideOrOn);

	/**
	 * Test if two oriented boxes intersect
	 */
	UFUNCTION(BlueprintPure, Category = "GeometryScript|Shapes|OrientedBox", meta = (ScriptMethod, KeyWords = "Overlaps"))
	static UE_API UPARAM(DisplayName = "Intersects") bool
	TestOrientedBoxOrientedBoxIntersection(FOrientedBox Box1, FOrientedBox Box2);
	...
	UFUNCTION(BlueprintPure, Category = "GeometryScript|Shapes|OrientedBox", meta = (ScriptMethod, KeyWords = "Overlaps"))
	static UE_API UPARAM(DisplayName = "Intersects") bool
	TestBoxOrientedBoxIntersection(FBox Box, FOrientedBox OrientedBox);
};

...
namespace UE::MeshQueryFunctionLocals
{
	static bool ValidateMeshVertexID(const FDynamicMesh3& Mesh, int32 VID, bool bLogError = true)
	{
		if (!Mesh.IsVertex(VID))
		{
			if (bLogError)
			{
				UE::Geometry::MakeScriptError(EGeometryScriptErrorType::InvalidInputs, 
					FText::Format(LOCTEXT("MeshQueryInvalidVertexID", "Mesh queried with invalid vertex ID: {0} (Max ID: {1})"),
						VID, Mesh.MaxVertexID()));
			}
			return false;
		}
		return true;
	...
	static bool ValidateMeshTriangleID(const FDynamicMesh3& Mesh, int32 TID, bool bLogError = true)
	{
		if (!Mesh.IsTriangle(TID))
		{
			...
			{
				UE::Geometry::MakeScriptError(EGeometryScriptErrorType::InvalidInputs,
					FText::Format(LOCTEXT("MeshQueryInvalidTriangleID", "Mesh queried with invalid triangle ID: {0} (Max ID: {1})"),
						TID, Mesh.MaxTriangleID()));
			}
			return false;
		}
		return true;
...
UDynamicMesh* UGeometryScriptLibrary_MeshQueryFunctions::GetVertexConnectedTriangles(UDynamicMesh* TargetMesh, int32 VertexID, TArray<int32>& Triangles)
{
	SimpleMeshQuery<int32>(TargetMesh, 0, [&](const FDynamicMesh3& Mesh) {
		if (!UE::MeshQueryFunctionLocals::ValidateMeshVertexID(Mesh, VertexID))
		{
			return 0;
		}
...
UDynamicMesh* UGeometryScriptLibrary_MeshQueryFunctions::GetTriangleNormals(UDynamicMesh* TargetMesh, int32 TriangleID, FVector& Normal1, FVector& Normal2, FVector& Normal3, bool& bTriHasValidNormals)
{
	...
	{
		if (!UE::MeshQueryFunctionLocals::ValidateMeshTriangleID(Mesh, TriangleID))
		{
			return false;
		}

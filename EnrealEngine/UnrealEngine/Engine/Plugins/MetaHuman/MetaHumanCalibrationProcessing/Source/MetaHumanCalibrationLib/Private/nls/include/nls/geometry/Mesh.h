...
// forward decs for friend declaration below
...
public:
    ...
    void SetVertices(Eigen::Matrix<T, 3, -1>&& vertices) { m_vertices = std::move(vertices); ClearVertexNormals(); }
    ...
    void SetTriangles(Eigen::Matrix<int, 3, -1>&& tris) { m_tris = std::move(tris); ClearVertexNormals(); }
    ...
    void SetQuads(Eigen::Matrix<int, 4, -1>&& quads) { m_quads = std::move(quads); ClearVertexNormals(); }
     ...
     * @returns for each triangle, the index of the quad (bool = true) from which it was triangulated or the triangle id (bool = false)
     */
    enum class TriangulationMethod {
        Simple,
        Quality
    };
    std::vector<std::pair<int, bool>> Triangulate(TriangulationMethod triangulationMethod = TriangulationMethod::Quality, TaskThreadPool* taskThreadPool = nullptr);
    ...
    const Eigen::Matrix<T, 3, -1>& CalculateVertexNormals(bool recompute = false,
                                                          VertexNormalComputationType vertexNormalComputationType = VertexNormalComputationType::VoronoiAreaWeighted,
                                                          bool stableNormalize = true,
                                                          TaskThreadPool* taskThreadPool = nullptr);
    ...
    void CalculateVertexNormals(const Eigen::Matrix<T, 3, -1>& vertices,
                                ...
                                TaskThreadPool* taskThreadPool = nullptr) const;
     ...
     * @param[in] accurate  If True, it uses the mixed area term (see above), otherwise it uses a faster version by weighing using the triangle area.
     */
    void CalculateVertexNormalsRef(Eigen::Ref<const Eigen::Matrix<T, 3, -1>> vertices,
    ...
    void SetVertexNormals(Eigen::Matrix<T, 3, -1>&& vertexNormals) { m_normals = std::move(vertexNormals); }
    ...
    void SetTexcoords(Eigen::Matrix<T, 2, -1>&& texcoords) { m_texcoords = std::move(texcoords); }
    ...
    void SetTexTriangles(Eigen::Matrix<int, 3, -1>&& tris) { m_tex_tris = std::move(tris); }
    ...
    void SetTexQuads(Eigen::Matrix<int, 4, -1>&& quads) { m_tex_quads = std::move(quads); }
    ...
    /**
     * @brief Resample the mesh such that new vertex i will correspond to previous vertex newToOldMap[i]. In the current implementation texture coordinates and
     * texture faces are discarded.
    ...
    //! Calculate and return the triangle normals
    Eigen::Matrix<T, 3, -1> CalculateTriangleNormals() const;

    //! Remove all triangles and quads where any vertex is mask[vID] = 0.
    void Mask(const Eigen::VectorX<T>& mask);

    //! Keep only triangles and quads that contain vIDs
    void Mask(const std::vector<int>& vIDs);

    template <class U> 
    friend bool TITAN_NAMESPACE::io::ToBinaryFile(FILE* pFile, const Mesh<U>& mesh);

    template <class U> 
    friend bool TITAN_NAMESPACE::io::FromBinaryFile(FILE* pFile, Mesh<U>& mesh);

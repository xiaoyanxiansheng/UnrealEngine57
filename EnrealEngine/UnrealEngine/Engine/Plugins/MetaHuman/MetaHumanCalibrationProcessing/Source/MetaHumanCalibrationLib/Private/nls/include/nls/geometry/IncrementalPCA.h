...
public:
     ...
     */
    void AddTrainingData(const Eigen::Matrix<T, -1, -1>& dataMatrix, T forgetFactor, int maxModes, T maxVariance, DataOrder order)
    {
    ...
    {
        assert(IsTrained() &&
               ((order == DataOrder::ColsAreExamples && parameterMatrix.rows() == NumVariables()) ||
                (order == DataOrder::RowsAreExamples && parameterMatrix.cols() == NumVariables())));

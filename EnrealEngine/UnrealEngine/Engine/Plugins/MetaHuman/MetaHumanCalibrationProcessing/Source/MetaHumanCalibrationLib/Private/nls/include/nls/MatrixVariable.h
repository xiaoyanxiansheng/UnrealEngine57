...
public:
    ...
    DiffDataMatrix<T, R, C> EvaluateMatrix(Context<T>* context = nullptr) { return DiffDataMatrix<T, R, C>(Rows(), Cols(), this->Evaluate(context)); }

    //! convenience function to set the identity matrix
    void SetIdentity() { SetMatrix(Eigen::Matrix<T, R, C>::Identity(Rows(), Cols())); }

    //! convenience function to set a random matrix
    void SetRandom() { SetMatrix(Eigen::Matrix<T, R, C>::Random(Rows(), Cols())); }
...
protected:
     ...
     */
    void SetMatrix(const Eigen::Matrix<T, R, C>& M, bool projectToManifold) { this->SetValue(M.data(), M.size(), projectToManifold); }

    //! @see SetMatrix above
    void SetMatrix(const Eigen::Map<const Eigen::Matrix<T, R, C>>& M, bool projectToManifold) { this->SetValue(M.data(), M.size(), projectToManifold); }

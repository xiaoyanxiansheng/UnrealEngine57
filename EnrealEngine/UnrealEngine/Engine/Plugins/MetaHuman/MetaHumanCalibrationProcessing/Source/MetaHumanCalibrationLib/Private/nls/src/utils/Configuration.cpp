...
{
    std::transform(configParameters.begin(), configParameters.end(), std::back_inserter(m_paramOrder), [](const auto& pair) -> std::string {
            return pair.first;
        });
}
...
void Configuration::AddConfiguration(const Configuration& config) { AddConfiguration(config.Name(), config); }
...
JsonElement Configuration::ToJson() const
{
    ...
    auto toJsonElement = [](const ConfigurationParameter& param) {
            ...
            {
                return JsonElement(param.Value<bool>());
            }
            else if (param.IsType<int>())
            {
                return JsonElement(param.Value<int>());
            }
            else if (param.IsType<double>())
            {
                return JsonElement(param.Value<double>());
            }
            else if (param.IsType<float>())
            {
                return JsonElement(param.Value<float>());
            }
            ...
            {
                return JsonElement(param.AsString());
            }
        };
...
void Configuration::FromJson(const JsonElement& json, std::vector<std::string>& unspecifiedKeys, std::vector<std::string>& unknownKeys)
{
    FromJson(json, unspecifiedKeys, unknownKeys, "");
}

void Configuration::FromJson(const JsonElement& json,
                             ...
                             const std::string& prefix)
{
    for (auto&& [key, value] : json.Object())
    {
        ...
        if (it != Parameters().end())
        {
            ...
            switch (value.Type())
            {
                case JsonElement::JsonType::True: param.Set(true);
                    break;
                case JsonElement::JsonType::False: param.Set(false);
                    break;
                case JsonElement::JsonType::String: param.Set(value.String());
                    break;

...
public:
    ...
    void SetMatrix(const Eigen::Matrix<T, R + 1, C + 1>& mat) { m_affineMatrix = mat; }
    ...
    static Affine FromLinear(const Eigen::Matrix<T, R, C>& linear)
    {
        Affine aff;
        aff.SetLinear(linear);
        return aff;
    }

    static inline Affine FromEulerXYZ(const Eigen::Vector3f& eulerXYZ, const Eigen::Vector3f& translation, bool radians = true)
    {
        Affine<float, 3, 3> aff;
        if (radians)
        {
            aff.SetLinear(EulerXYZ(eulerXYZ[0], eulerXYZ[1], eulerXYZ[2]));
        }
        else
        {
            aff.SetLinear(EulerXYZ(degree2rad(eulerXYZ[0]), degree2rad(eulerXYZ[1]), degree2rad(eulerXYZ[2])));
        }
        aff.SetTranslation(translation);
        return aff;
    }
    ...
    void SetIdentity() { m_affineMatrix.setIdentity(); }

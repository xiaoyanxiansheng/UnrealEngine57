...
public:
    ...
    void AddInfluenceWeights(const std::string& geometryName, const std::map<std::string, InfluenceWeights<T>>& jointInfluenceWeights, int numVertices = -1)
    {
        ...
        {
            LOG_WARNING("Geometry \"{}\" has {} vertices that are not influenced by any rig joint and are therefore connected to the root.",
        ...
        for (int i = 0; i < int(m_jointsVector.size()); i++)
        {
            ...
            if (it != jointInfluenceWeights.end())
            {
                ...
                {
                    mat.insertBackByOuterInner(i, it->second.indices[k]) = it->second.weights[k];
                }
            }
            if (m_jointsVector[i]->IsRoot())
            {
                ...
                {
                    mat.insertBackByOuterInner(i, vID) = T(1);
                }
            }
        }
        mat.finalize();
     ...
     */
    Eigen::Matrix<T, 3, -1> EvaluateGeometry(const std::string& geometryName, const Eigen::Matrix<T, 3, -1>& restVertices) const;
     ...
     */
    DiffDataMatrix<T, 3, -1> EvaluateGeometry(const std::string& geometryName, const DiffDataMatrix<T, 3, -1>& restVertices) const;
    ...
    void CheckValidity() { CheckSingleRoot(); }

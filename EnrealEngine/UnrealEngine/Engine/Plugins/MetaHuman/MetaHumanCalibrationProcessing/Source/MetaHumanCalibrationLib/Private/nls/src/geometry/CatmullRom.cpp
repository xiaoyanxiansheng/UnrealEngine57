...
Eigen::Vector<T, D> CatmullRomPointOnCurve(const Eigen::Vector<T, D>& a,
                                           const Eigen::Vector<T, D>& b,
                                           const Eigen::Vector<T, D>& c,
                                           const Eigen::Vector<T, D>& d,
                                           T t)
{
...
template <typename T, int D>
Eigen::Matrix<T, D, -1> GenerateCatmullRomSpline(const Eigen::Matrix<T, D, -1>& points, const std::vector<int>& n_points_per_section, bool closed)
{
    static_assert(std::is_same<T, float>::value || std::is_same<T, double>::value, "T must be a float or double type");
    ...
    CARBON_ASSERT(((static_cast<int>(n_points_per_section.size()) == (N_points - 1)) && (closed == false)) ||
                  ((static_cast<int>(n_points_per_section.size()) == N_points) && (closed == true)),
                  "vector of number of points of sections does not match the requested number of control points");
    ...
    if (points.cols() == 2) // Straight line
    {
        ...
        Eigen::Vector<T, D> step = delta / T(n_points_per_section[0]);
...
template <class T, int D>
Eigen::Vector<T, -1> CatmullRom<T, D>::LinearResampling(const Eigen::Vector<T, -1>& values) const
{
    const int newSize =
        (IsClosed() &&
         m_controlPoints.NumControlPoints() > 2) ? int(values.size() * PointsPerSegment() + 1) : int((values.size() - 1) * PointsPerSegment() + 1);
    Eigen::Vector<T, -1> newValues(newSize);
    const T step = T(1) / T(PointsPerSegment());
    newValues[0] = values[0];
    for (int i = 0; i < int(values.size()) - 1; ++i)
    {
        const T delta = values[i + 1] - values[i];
        for (int k = 0; k < PointsPerSegment(); ++k)
        {
            newValues[1 + PointsPerSegment() * i + k] = values[i] + step * T(1 + k) * delta;
        }
    }
    if (IsClosed() && (m_controlPoints.NumControlPoints() > 2))
    {
        const int lastIndex = int(values.size()) - 1;
        const T delta = values[0] - values[lastIndex];
        for (int k = 0; k < PointsPerSegment(); ++k)
        {
            newValues[1 + PointsPerSegment() * lastIndex + k] = values[lastIndex] + step * T(1 + k) * delta;
        }
    }
    return newValues;
}

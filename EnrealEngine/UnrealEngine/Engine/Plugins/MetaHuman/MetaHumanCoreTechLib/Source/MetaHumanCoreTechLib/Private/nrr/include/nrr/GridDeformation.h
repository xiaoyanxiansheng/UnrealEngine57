...
public:
    ...
    GridDeformation(int gridSizeX,
                    int gridSizeY,
                    int gridSizeZ,
                    const Eigen::Vector3<T>& minGridPoint,
                    const Eigen::Vector3<T>& maxGridPoint,
    ...
    void Init(Eigen::Ref<const Eigen::Matrix<T, 3, -1>> srcVertices)
    {
    ...
    Eigen::Vector3<T> EvaluateGridPosition(const Eigen::Vector3<T>& pt) { return EvaluateGridPosition(GetGridPosition(pt)); }

    Affine<T, 3, 3> EvaluateGridTransform(const Eigen::Vector3<T>& pt) { return EvaluateGridTransform(GetGridPosition(pt)); }
               ...
               const Eigen::VectorX<T>& areaPerVertex = Eigen::VectorX<T>())
    {
        ...
        Eigen::SparseMatrix<T, Eigen::RowMajor> A(numRows, GridSizeX() * GridSizeY() * GridSizeZ());
        A.setFromTriplets(triplets.begin(), triplets.end());

        Eigen::SparseMatrix<T, Eigen::RowMajor> AtA = A.transpose() * A;
        ...
        for (int dim = 0; dim < 3; ++dim)
        {
            ...
            m_vertexOffsets.row(dim) = solver.Solve(maxIterations, AtA, A.transpose() * b, Eigen::VectorX<T>()).transpose();

            LOG_VERBOSE("estimated grid solve err for dim {}: {}", dim, solver.TolError());
        }
    }

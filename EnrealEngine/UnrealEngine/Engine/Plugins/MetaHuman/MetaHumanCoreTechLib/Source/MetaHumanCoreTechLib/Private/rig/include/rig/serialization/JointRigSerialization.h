...
void JointRigFromJson(const JsonElement& j, JointRig<AffineType>& jointRig)
{
    ...
    using Joint = TITAN_NAMESPACE::Joint<AffineType>;
    using JointPtr = std::shared_ptr<Joint>;

    std::map<std::string, JointPtr> joints;

    // create all joints
    const JsonElement& jointsDict = j["joints"];
    ...
    for (const auto& [jointName, jointDict] : jointsDict.Map())
    {
        ...
        joint->SetLocalMatrix(Affine<T, 3, 3>(io::FromJson<Eigen::Matrix<T, 4, 4>>(jointDict["local"])));
        joint->SetBindMatrix(Affine<T, 3, 3>(io::FromJson<Eigen::Matrix<T, 4, 4>>(jointDict["world"])));
    ...
    for (const auto& [jointName, jointDict] : jointsDict.Map())
    {
        const JsonElement& influencesDict = jointDict["influence"];
        ...
        for (const auto& [geometryName, influenceDict] : influencesDict.Map())
        {
            ...
            const Eigen::VectorX<T> weights = io::FromJson<Eigen::VectorX<T>>(influenceDict["vertex weights"]);
            const Eigen::VectorX<int> indices = io::FromJson<Eigen::VectorX<int>>(influenceDict["vertex indices"]);
 ...
 */
template <class AffineType>
void JointRigStateFromJson(const JsonElement& j, JointRig<AffineType>& jointRig)
{
    const JsonElement& jointsDict = j["joints"];
    ...
    for (const auto& [jointName, jointDict] : jointsDict.Map())
    {
        rigState[jointName] = Affine<T, 3, 3>(io::FromJson<Eigen::Matrix<T, 4, 4>>(jointDict["local"]));
    }
 ...
 */
template <class AffineType>
JsonElement JointRigStateToJson(const JointRig<AffineType>& jointRig)
{
    using T = typename JointHelper<AffineType>::SCALAR;
    std::map<std::string, Affine<T, 3, 3>> rigState = jointRig.GetState();

    JsonElement states(JsonElement::JsonType::Object);
    for (auto&& [jointName, aff] : rigState)
    {
        JsonElement state(JsonElement::JsonType::Object);
        state.Insert("local", ToJson(aff));

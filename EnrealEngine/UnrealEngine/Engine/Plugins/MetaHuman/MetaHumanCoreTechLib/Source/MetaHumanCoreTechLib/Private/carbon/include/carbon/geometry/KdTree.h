...
public:
    ...
    template <typename ... Types>
    std::pair<int64_t, scalar_t> getClosestPoint(const Eigen::Ref<const vector_t>& point, const scalar_t& distance, Types&& ... args) const
    {
    ...
    template <typename ... Types>
    std::pair<std::vector<int64_t>, std::vector<scalar_t>> getPointsInRadius(const Eigen::Ref<const vector_t>& point, const scalar_t& distance,
                                                                             Types&& ... args) const
    {
    ...
    Eigen::VectorXi Search(const scalar_t* qrs, int numPts, const scalar_t maxDistance = std::numeric_limits<scalar_t>::max()) const
    {
...
private:
    ...
    void splitNode(size_t index)
    {
        ...
        auto splitIndex = it - indices.begin();

        if (node.bbox.first[node.dim] == node.bbox.second[node.dim])
        {
            // all points in the node are coincident; in this case, set the split index to the 
            // halfway point through the range to avoid all points being placed into one of the child nodes

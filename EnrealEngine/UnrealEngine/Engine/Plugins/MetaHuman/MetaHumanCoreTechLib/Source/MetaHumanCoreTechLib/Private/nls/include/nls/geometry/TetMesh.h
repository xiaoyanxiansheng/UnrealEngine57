...
public:
    ...
    //! Sets the boundary visualization mesh: one triangle per boundary face. tetPresent can flag tets to be ignored. Vertex indices are the same as for tets.
    void BoundaryMesh(Eigen::Matrix<int, 3, -1>& visTriangles, std::vector<bool>* tetPresent = nullptr) const;

    //! @return all boundary tets
    std::vector<int> BoundaryTets() const;

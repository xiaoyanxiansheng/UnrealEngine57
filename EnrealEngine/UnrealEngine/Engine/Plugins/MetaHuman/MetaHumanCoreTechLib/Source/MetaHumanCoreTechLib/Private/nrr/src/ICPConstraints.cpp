...
struct ICPConstraints<T>::Private
{
    ...
    Configuration config = { ICPConstraints<T>::ConfigName(), {
                                 ...
                                 //!< flag whether to dynamically adjust geometryWeight using actor's distance from camera
                                 { "useActorDistanceWeight", ConfigurationParameter(false) },
                                 //!< threshold for minimum distance of actor from camera used to calculate dynamic geometryWeight
                                 { "minimumCameraRangeToUseDepth", ConfigurationParameter(T(15), T(5), T(20)) },
                                 //!< threshold for maximum distance of actor from camera where beyond sets geometryWeight=0.0
                                 { "maximumCameraRangeToUseDepth", ConfigurationParameter(T(30), T(25), T(40)) }
                             } };
};


template <class T>
ICPConstraints<T>::ICPConstraints() : m(new Private)
{}
...
template <class T>
void ICPConstraints<T>::SetConfiguration(const Configuration& config, bool ignoreUnknownKeys) { m->config.Set(config, ignoreUnknownKeys); }
...
template <class T>
void ICPConstraints<T>::SetTargetWeights(const Eigen::Vector<T, -1>& targetWeights) { m->correspondenceSearch.SetTargetWeights(targetWeights); }

template <class T>
void ICPConstraints<T>::AddTargetDepthAndNormals(const std::shared_ptr<const DepthmapData<T>>& depthmapData) { m->depthmapData.push_back(depthmapData); }

template <class T>
void ICPConstraints<T>::ClearPreviousCorrespondences() { m->correspondences = typename MeshCorrespondenceSearch<T>::Result(); }
    ...
    if ((geometryWeight > 0) && (m->correspondences.srcIndices.size() > 0))
    {
        ...
        if (point2pointWeight > T(0))
        {
            ...
            DiffData<T> residual = PointPointConstraintFunction<T, 3>::Evaluate(vertices,
                                                                                m->correspondences.srcIndices,
                                                                                m->correspondences.targetVertices,
                                                                                m->correspondences.weights,
                                                                                geometryWeight * point2pointWeight);
            cost.Add(std::move(residual), T(1), "icp_p2p");
        }
        if (point2surfaceWeight > T(0))
        {
            ...
            DiffData<T> residual = PointSurfaceConstraintFunction<T, 3>::Evaluate(vertices,
                                                                                  m->correspondences.srcIndices,
                                                                                  m->correspondences.targetVertices,
                                                                                  m->correspondences.targetNormals,
                                                                                  m->correspondences.weights,
                                                                                  geometryWeight * point2surfaceWeight);
            cost.Add(std::move(residual), T(1), "icp_p2s");
        }
    }
...
template <class T>
void ICPConstraints<T>::FindCorrespondences(const Eigen::Matrix<T, 3, -1>& vertices,
                                            ...
                                            typename MeshCorrespondenceSearch<T>::Result& correspondences) const
{
    ...
    {
        m->correspondenceSearch.Search(vertices,
                                       normals,
                                       correspondences,
                                       (m->correspondenceSearchWeights.NumVertices() > 0 ? &m->correspondenceSearchWeights.Weights() : nullptr),
                                       normalIncompatibilityThreshold);

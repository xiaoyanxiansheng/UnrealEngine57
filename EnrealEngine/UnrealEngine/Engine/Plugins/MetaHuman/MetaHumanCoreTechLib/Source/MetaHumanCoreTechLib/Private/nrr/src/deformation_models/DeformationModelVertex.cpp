...
std::pair<DiffDataMatrix<T, 3, -1>, DiffDataMatrix<T, 3, -1>> DeformationModelVertex<T>::EvaluateBothStabilizedAndTransformedVertices(Context<T>* context)
{
    ...
    DiffDataMatrix<T, 3, -1> vertices = baseVertices + offsets;
    const bool optimizePose = m->config["optimizePose"].template Value<bool>();
    DiffDataMatrix<T, 3, -1> transformedVertices = m->defModelRigid.EvaluateAffine(optimizePose ? context : nullptr).Transform(vertices);
    return { std::move(vertices), std::move(transformedVertices) };
}
...
template <class T>
void DeformationModelVertex<T>::SetConfiguration(const Configuration& config) { m->config.Set(config); }
...
template <class T>
void DeformationModelVertex<T>::SetRigidTransformation(const Affine<T, 3, 3>& affine) { m->defModelRigid.SetRigidTransformation(affine); }

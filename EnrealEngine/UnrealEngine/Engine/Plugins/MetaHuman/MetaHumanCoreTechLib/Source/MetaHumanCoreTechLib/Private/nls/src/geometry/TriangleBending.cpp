...
template <class T>
void TriangleBending<T>::SetTopology(const Eigen::Matrix<int, 3, -1>& triangles) { m_triangles = triangles; }
...
void TriangleBending<T>::SetRestPose(const Eigen::Matrix<T, 3, -1>& vertices)
{
    ...
    for (int i = 0; i < int(m_triangles.cols()); i++)
    {
        ...
        {
            CARBON_CRITICAL("Bending energy is not possible if triangles have degenerate triangles.");
        }

        if ((vID0 < 0) || (vID0 >= m_numVertices) || (vID1 < 0) || (vID1 >= m_numVertices) || (vID2 < 0) || (vID2 > m_numVertices))
        {
            CARBON_CRITICAL("Triangle index out of boudns.");
        }
    }
...
template <class T>
DiffData<T> TriangleBending<T>::EvaluateQuadratic(const DiffDataMatrix<T, 3, -1>& vertices, const T bendingWeight) const
{
    Vector<T> outputData(NumBendingTerms() * 3);
...
template <class T>
DiffData<T> TriangleBending<T>::EvaluateDihedral(const DiffDataMatrix<T, 3, -1>& vertices, const T bendingWeight) const
{
    Vector<T> outputData(NumBendingTerms());
    ...
    for (int i = 0; i < NumBendingTerms(); i++)
    {
        ...
        if (vertices.HasJacobian())
        {
            ...
            for (int k = 0; k < 3; k++)
            {
                triplets.push_back(Eigen::Triplet<T>(i, 3 * vID0 + k, ddx1[k]));
            }
            for (int k = 0; k < 3; k++)
            {
                triplets.push_back(Eigen::Triplet<T>(i, 3 * vID1 + k, ddx2[k]));
            }
            for (int k = 0; k < 3; k++)
            {
                triplets.push_back(Eigen::Triplet<T>(i, 3 * vID2 + k, ddx3[k]));
            }
            for (int k = 0; k < 3; k++)
            {
                triplets.push_back(Eigen::Triplet<T>(i, 3 * vID3 + k, ddx4[k]));
            }
        }
    }

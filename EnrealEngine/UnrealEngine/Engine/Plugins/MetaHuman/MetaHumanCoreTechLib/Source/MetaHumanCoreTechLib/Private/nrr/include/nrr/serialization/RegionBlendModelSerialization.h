...
template <class T>
void RegionBlendModelFromJson(const JsonElement& j, RegionBlendModel<T>& regionBlendModel)
{
    Eigen::MatrixX<T> archetype;
    io::FromJson(j["archetype"], archetype);
    const int numVertices = int(archetype.cols());

    const JsonElement& jRegions = j["regions"];
    ...
    for (const auto& region : jRegions.Array())
    {
        ...
        const JsonElement& regionData = region.Map().begin()->second;

        Eigen::VectorX<T> weights;
        io::FromJson(regionData, weights);
    ...
    const JsonElement& jCharacters = j["characters"];
    ...
    for (const auto& character : jCharacters.Array())
    {
        ...
        const JsonElement& charData = character.Map().begin()->second;

        Eigen::MatrixX<T> vertices;
        io::FromJson(charData, vertices);

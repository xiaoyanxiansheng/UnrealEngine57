...
SparseMatrix<T> VertexLaplacian<T>::LaplacianMatrix(const Mesh<T>& mesh, Type type, int dim)
{
    ...
    for (const Eigen::Triplet<T>& triplet : triplets)
    {
        ...
        // weight the laplacian by the vertex area - in the least square sense this means that Ldx is scaled appropriately and therefore global scale changes do
...
template <class T>
void VertexLaplacian<T>::SetRestPose(const Mesh<T>& mesh, Type type) { m_laplacianMatrix = LaplacianMatrix(mesh, type, 3); }
...
DiffDataMatrix<T, 3, -1> VertexLaplacian<T>::EvaluateLaplacianOnOffsets(const DiffDataMatrix<T, 3, -1>& vertexOffsets, const T laplacianWeight) const
{
    ...
    Vector<T> outputData = laplacianWeightSqrt * (m_laplacianMatrix * vertexOffsets.Value());
    ...
    return DiffDataMatrix<T, 3, -1>(3, vertexOffsets.Cols(), DiffData<T>(std::move(outputData), Jacobian));
}

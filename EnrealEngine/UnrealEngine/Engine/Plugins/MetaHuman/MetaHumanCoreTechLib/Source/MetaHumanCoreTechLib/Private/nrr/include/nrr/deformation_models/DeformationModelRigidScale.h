...
public:
    ...
    /**
     * @returns the current rigid transformation either assuming the vertices are first scaled and then transformed, or alternatively first transformed and then
     * scaled.
     ...
     * transformation and scale, the customer needs to determine whether first to transform and then scale (preScale=false) or scale and then transform
     *(preScale=true)
     */
    Affine<T, 3, 3> RigidTransformation(bool preScale) const;

    /**
     * Set the current rigid transformation either assuming the vertices are first scaled and then transformed, or alternatively first transformed and then
     * scaled.
     */

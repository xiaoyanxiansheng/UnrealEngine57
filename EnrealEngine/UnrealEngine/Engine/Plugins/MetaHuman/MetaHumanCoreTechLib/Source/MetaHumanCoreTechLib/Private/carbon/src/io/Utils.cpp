...
FILE* OpenUtf8File(const std::string& utf8Path, const char* mode) {
    std::u8string u8str(utf8Path.begin(), utf8Path.end());
    std::filesystem::path path(u8str);
...
std::string ReadFile(const std::string& utf8Path) {
    FILE* file = OpenUtf8File(utf8Path, "rb");
    if (!file)
        CARBON_CRITICAL("Failed to open file: " + utf8Path);
    ...
    if (fread(data.data(), 1, size, file) != static_cast<size_t>(size)) {
        fclose(file);
        CARBON_CRITICAL("Failed to read file: " + utf8Path);
    }

...
template <typename T>
constexpr T rad2degreeScale() { return T(180.0 / CARBON_PI); }

//! convert from radian to degree
template <typename T>
T rad2degree(T deg) { return deg * rad2degreeScale<T>(); }

//! convert from degree to radian
template <typename T>
constexpr T degree2radScale() { return T(CARBON_PI / 180.0); }

template <typename T>
T degree2rad(T rad) { return rad * degree2radScale<T>(); }

inline int RoundUp(int value, int multiple)
{
    const int offset = value % multiple;
    return (offset > 0) ? (value - offset + multiple) : value;
}

inline int RoundDown(int value, int multiple) { return value - (value % multiple); }
 ...
 */
template <class T>
SparseMatrix<T> AddSparseMatricesAndPadColumns(const SparseMatrix<T>& A, const SparseMatrix<T>& B, bool squeeze)
{
 ...
 */
template <class T>
void RepeatRowsOfSparseMatrix(const SparseMatrix<T>& A, SparseMatrix<T>& output, int n)
{
    static_assert(SparseMatrix<T>::IsRowMajor);
    CARBON_PRECONDITION(A.isCompressed(), "A needs to be compressed");
 ...
 */
template <class T>
void ConcatenateSparseMatricesAlongRowDimension(const std::vector<std::reference_wrapper<const SparseMatrix<T>>>& inputs, SparseMatrix<T>& output)
{
    static_assert(SparseMatrix<T>::IsRowMajor);
...
/**
 * Gathers columns from the sparse matrix.
 * @param blockIndices  The indices indicate which rows to select from the sparse matrix. rowIndex = blockIdx * blockSize
 * @param blockSize		The size of each row block i.e. for blockSize = 1, the blockIndices are equivalent to rowIndices.
 */
template <class T>
SparseMatrix<T> ColGather(const SparseMatrix<T>& inSparse, const Eigen::VectorX<int>& blockIndices, int blockSize = 1)
{
    static_assert(SparseMatrix<T>::IsRowMajor);
    CARBON_PRECONDITION(inSparse.isCompressed(), "input matrix needs to be compressed");

    // TODO: make efficient
    SparseMatrix<T> inSparseT = inSparse.transpose();
    SparseMatrix<T> outSparse = RowGather(inSparseT, blockIndices, blockSize).transpose();
    return outSparse;
}
 ...
 */
template <class T>
SparseMatrix<T> ColScatter(const SparseMatrix<T>& inSparse, const int outputCols, const Eigen::VectorX<int>& blockIndices, int blockSize = 1)
{
    static_assert(SparseMatrix<T>::IsRowMajor);
    CARBON_PRECONDITION(inSparse.isCompressed(), "input matrix needs to be compressed");
    ...
    // TODO: make efficient
    SparseMatrix<T> inSparseT = inSparse.transpose();
    SparseMatrix<T> outSparse = RowScatter(inSparseT, outputCols, blockIndices, blockSize).transpose();
    return outSparse;
}
 ...
 * @param colOffset  Offsets the dense matrix keeping the first colOffset colums of the sparse matrix empty
 */
template <class T>
void Dense2Sparse(const Eigen::Matrix<T, -1, -1, Eigen::RowMajor>& dense, SparseMatrix<T>& sparse, int rowOffset, int colOffset)
{
    sparse.resize(dense.rows() + rowOffset, dense.cols() + colOffset);
    sparse.resizeNonZeros(dense.rows() * dense.cols());
    for (int i = 0; i < int(dense.rows()); ++i)
    {
        sparse.outerIndexPtr()[i + rowOffset] = i * int(dense.cols());
    }
    sparse.outerIndexPtr()[dense.rows() + rowOffset] = int(dense.rows() * dense.cols());
    for (int i = 0; i < int(dense.cols()); ++i)
    {
        sparse.innerIndexPtr()[i] = colOffset + i;
    }
    for (int i = 0; i < int(dense.rows()); ++i)
    {
...
template <class T>
inline int NumNonzerosForRow(const SparseMatrix<T>& smat, int row) { return (smat.outerIndexPtr()[row + 1] - smat.outerIndexPtr()[row]); }

...
public:
    ...
    virtual Cost<T> Evaluate(const DiffDataMatrix<T, 3, -1>& vertices, const Eigen::Matrix<T, 3, -1>& normals,
                             bool enforceConsistentSparsityPattern = true) const override;
...
public:
    ...
    virtual Cost<T> EvaluateLandmarks(const DiffDataMatrix<T, 3, -1>& vertices,
                                      ...
                                      LandmarkConstraintsData<T>* debugInfo = nullptr) const override;
    ...
    /**
     * Evaluates the contour point2surface constraint for all current camera/curve pairs (does *not* scale it by the configuration weight, use Evaluate()
     * instead)
     ...
     */
    Cost<T> EvaluateContours(const DiffDataMatrix<T, 3, -1>& vertices,
                             const Eigen::Matrix<T, 3, -1>& normals,
                             MeshType meshType = MeshType::Face,
                             LandmarkConstraintsData<T>* debugInfo = nullptr) const;

    /**
     * Evaluates the inner lip point2surface constraint for all current camera/inner lip contour curve pairs (does *not* scale it by the configuration weight,

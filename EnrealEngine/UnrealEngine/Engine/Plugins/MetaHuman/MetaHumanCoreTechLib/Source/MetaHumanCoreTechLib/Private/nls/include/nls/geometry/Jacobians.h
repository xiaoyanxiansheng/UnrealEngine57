...
template <class T, int R, int K, int C>
Eigen::Matrix<T, R* C, K* C, Eigen::RowMajor> JacobianOfPremultipliedMatrixDense(const Eigen::Matrix<T, R, K>& M)
{
    Eigen::Matrix<T, R* C, K* C, Eigen::RowMajor> dAdX = Eigen::Matrix<T, R* C, K* C, Eigen::RowMajor>::Zero();
    for (int c = 0; c < C; c++)
    {
        for (int r = 0; r < R; r++)
        {

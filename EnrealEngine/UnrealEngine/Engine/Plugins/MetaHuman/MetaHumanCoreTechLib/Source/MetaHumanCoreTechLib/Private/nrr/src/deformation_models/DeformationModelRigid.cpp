...
template <class T>
DeformationModelRigid<T>::DeformationModelRigid(const DeformationModelRigid& other)
    : m(new Private(*other.m.Get()))
{}
template <class T> DeformationModelRigid<T>& DeformationModelRigid<T>::operator=(const DeformationModelRigid& other)
{
    if (&other != this)
    {
...
template <class T>
DiffDataMatrix<T, 3, -1> DeformationModelRigid<T>::EvaluateVertices(Context<T>* context) { return EvaluateAffine(context).Transform(m->vertices); }
...
template <class T>
void DeformationModelRigid<T>::SetConfiguration(const Configuration& config) { m->config.Set(config); }

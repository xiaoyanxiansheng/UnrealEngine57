...
struct JointHelper<Eigen::Matrix<T, R, C>>
{
    using SCALAR = T;
};

template <class T, int R, int C>
struct JointHelper<DiffDataAffine<T, R, C>>
{
    using SCALAR = T;
};
template <class T, int R, int C>
struct JointHelper<Affine<T, R, C>>
{
    using SCALAR = T;
};
...
public:
    ...
    const AffineType& LocalMatrix() { return m_localMatrix; }
    ...
    void SetLocalMatrix(AffineType&& localMatrix)
    {
        m_localMatrix = std::move(localMatrix);
        InvalidateWorldMatrix();
    }
    ...
    void SetBindMatrix(const Affine<T, 3, 3>& bindMatrix)
    {
        ...
        m_bindMatrixInverse = bindMatrix.Inverse();
        InvalidateWorldMatrix();
    }
    ...
    const AffineType& SkinningMatrix()
    {
        ...
        {
            const AffineType& worldMatrix = WorldMatrix();
...
private:
    ...
    {
        if (IsRoot() || m_worldMatrix.has_value())
        {
            // only call the children if the world matrix was valid before, otherwise the children will be invalid anyway
            // for the root we always call invalidate as the world matrix is equivalent to the local matrix
            m_worldMatrix.reset();

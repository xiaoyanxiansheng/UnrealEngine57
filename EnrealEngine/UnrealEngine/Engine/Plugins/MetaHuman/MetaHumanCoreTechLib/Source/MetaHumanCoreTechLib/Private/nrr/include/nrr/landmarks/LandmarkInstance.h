...
public:
    ...
    std::shared_ptr<LandmarkInstance<T, D>> Clone()const
    {
        auto clone = std::make_shared<LandmarkInstance<T, D>>(m_points, m_confidence);

        clone->m_frameNumber = m_frameNumber;
        clone->m_landmarkConfiguration = m_landmarkConfiguration;
    ...
    bool LoadFromJson(const JsonElement& json);

    //! Save the landmark instance to json
    JsonElement SaveToJson() const;

    /**
     * Make a deep copy of the LandmarkInstance only copying the specified landmarks and curves
     */
    std::shared_ptr<LandmarkInstance<T, D>> CloneSubSet(const std::vector<std::string>  &landmarks, const std::vector<std::string>  &curves)const;

    //! Sets the points for a curve. The number of points can be different from the existing points
    void UpdatePointsForCurve(const std::string & curveName, const Eigen::Matrix<T, D, -1>& newPoints);

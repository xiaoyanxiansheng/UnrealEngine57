...
std::pair<std::vector<MeshContourPoint<T>>,
          std::vector<MeshContourPoint<T>>> MeshContourPoint<T>::FindContourChangesAndOcclusions(const std::vector<std::vector<int>>& vIDsForContourA,
...
void MeshContourPoint<T>::UpdateContourBasedOnOcclusions(std::vector<MeshContourPoint<T>>& contourA, const std::vector<MeshContourPoint<T>>& contourB)
{
    ...
    if (curveB.Valid())
    {
        for (auto& contourAPoint : contourA)
        {
            ...
            contourAPoint.occlusionDepthOffset = destPix[2] - contourAPoint.pix[2];
            // use the normal of the curve to determine wether there is an occlusion (whether it is an occluder, or the point is occluded can be determined
            // by the occlusionDepthOffset
            if ((contourAPoint.pix - destPix).template head<2>().dot(destNormal) < 0)
            {
    ...
    {
        LOG_WARNING("contour curve of size {} is not valid", contourB.size());
    }
}

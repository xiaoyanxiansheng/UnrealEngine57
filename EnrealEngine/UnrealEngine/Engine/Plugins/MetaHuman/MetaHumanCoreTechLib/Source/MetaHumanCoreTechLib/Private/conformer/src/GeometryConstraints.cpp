...
struct GeometryConstraints<T>::Private
{
    ...
    std::shared_ptr<TITAN_NAMESPACE::AABBTree<T>> aabbTree;
    std::shared_ptr<TITAN_NAMESPACE::KdTree<T>> kdTree;
    ...
    Configuration config = { std::string("ICP Constraints Configuration"), {
                                 ...
                                 //!< normal incompatibility threhold. remove correspondences where the dot product of source and target normal is smaller than
...
template <class T>
GeometryConstraints<T>::GeometryConstraints() : m(new Private)
{}
...
template <class T>
const std::shared_ptr<const Mesh<T>> GeometryConstraints<T>::TargetMesh() const
{
    return m->targetMesh;
}
...
template <class T>
void GeometryConstraints<T>::SetConfiguration(const Configuration& config, bool ignoreUnknownKeys) { m->config.Set(config, ignoreUnknownKeys); }
...
void GeometryConstraints<T>::SetTargetMesh(const std::shared_ptr<const Mesh<T>>& targetMesh)
{
    ...
    if (m->targetMesh != targetMesh)
    {
        ...
        ClearPreviousCorrespondences();
        m->aabbTree = std::make_shared<TITAN_NAMESPACE::AABBTree<T>>(m->targetMesh->Vertices().transpose(), m->targetMesh->Triangles().transpose());
        m->kdTree = std::make_shared<TITAN_NAMESPACE::KdTree<T>>(m->targetMesh->Vertices().transpose());
...
template <class T>
void GeometryConstraints<T>::AddTargetDepthAndNormals(const std::shared_ptr<const DepthmapData<T>>& depthData) { m->depthData.push_back(depthData); }

template <class T>
void GeometryConstraints<T>::SetTargetWeights(const Eigen::Vector<T, -1>& targetWeights) { m->targetWeights = targetWeights; }
...
Cost<T> GeometryConstraints<T>::EvaluateICP(const DiffDataMatrix<T, 3, -1>& vertices)
{
    ...
    if ((geometryWeight > 0) && (m->source2TargetCorrespondences.srcIndices.size() > 0))
    {
        ...
        if (point2pointWeight > T(0))
        {
            ...
            DiffData<T> residual = PointPointConstraintFunction<T, 3>::Evaluate(vertices,
                                                                                m->source2TargetCorrespondences.srcIndices,
                                                                                m->source2TargetCorrespondences.targetVertices,
                                                                                m->source2TargetCorrespondences.weights,
                                                                                geometryWeight * point2pointWeight);
            cost.Add(std::move(residual), T(1));
        }
        if (point2surfaceWeight > T(0))
        {
            // point2surface (tangential motion)
            DiffData<T> residual = PointSurfaceConstraintFunction<T, 3>::Evaluate(vertices,
                                                                                  m->source2TargetCorrespondences.srcIndices,
                                                                                  m->source2TargetCorrespondences.targetVertices,
                                                                                  m->source2TargetCorrespondences.targetNormals,
                                                                                  m->source2TargetCorrespondences.weights,
                                                                                  geometryWeight * point2surfaceWeight);
            cost.Add(std::move(residual), T(1));
        }
    }
    ...
    if ((geometryWeight > 0) && (m->target2SourceCorrespondences.srcBcs.size() > 0))
    {
        ...
        if (point2pointWeight > T(0))
        {
            ...
            DiffData<T> residual = PointPointConstraintFunction<T, 3>::Evaluate(srcVertices,
                                                                                m->target2SourceCorrespondences.targetVertices,
                                                                                m->target2SourceCorrespondences.weights,
                                                                                geometryWeight * point2pointWeight);
            cost.Add(std::move(residual), T(1));
        }
        if (point2surfaceWeight > T(0))
        {
            // point2surface (tangential motion)
            DiffData<T> residual = PointSurfaceConstraintFunction<T, 3>::Evaluate(srcVertices,
                                                                                  m->target2SourceCorrespondences.targetVertices,
                                                                                  m->target2SourceCorrespondences.targetNormals,
                                                                                  m->target2SourceCorrespondences.weights,
                                                                                  geometryWeight * point2surfaceWeight);
            cost.Add(std::move(residual), T(1));
        }
...
template <class T>
void GeometryConstraints<T>::FindCorrespondences(const Eigen::Matrix<T, 3, -1>& vertices,
                                                 const Eigen::Matrix<T, 3, -1>& normals,
                                                 typename MeshCorrespondenceSearch<T>::Result& correspondences) const
{
    ...
    if (m->targetMesh)
    {
        ...
        m->threadPool->AddTaskRangeAndWait(numQueries, [&](int start, int end) {
                ...
                for (int k = start; k < end; ++k)
                {
                    ...
                    if (tID >= 0)
                    {
                        bcs[k] = BarycentricCoordinates<T>(m->targetMesh->Triangles().col(tID), bc.transpose());
                    }
                    else
                    {
                        hasBadInput = true;
        ...
        if (hasBadInput)
        {
            CARBON_CRITICAL("ICP cannot collect correspondances, please check input data.");
        }
        ...
        for (int i = 0; i < numQueries; i++)
        {
            ...
            const Eigen::Vector3<T>& targetNormal = bc.template Evaluate<3>(m->targetMesh->VertexNormals());
            const Eigen::Vector3<T>& targetVertex = bc.template Evaluate<3>(m->targetMesh->Vertices());
            const T normalCompatibilityWeight = std::max<T>(0,
                                                            (normals.col(i).dot(
                                                                 targetNormal) - normalIncompatibilityThreshold) * normalIncompatibilityMultiplier);
            T weight = normalCompatibilityWeight * normalCompatibilityWeight;
            if (m->targetWeights.size() > 0)
            {
                weight *= bc.template Evaluate<1>(m->targetWeights.transpose())[0];
            }
    ...
    if (m->depthData.size() > 0)
    {
        ...
        for (int i = 0; i < int(m->depthData.size()); ++i)
        {
            ...
            for (auto&& [srcIndex, weight] : m->nonzeroSourceWeights)
            {
                ...
                if ((x >= 0) && (x < camera.Width()) && (y >= 0) && (y < camera.Height()))
                {
                    ...
                    {
                        targetVertex =
                            camera.Unproject(Eigen::Vector2<T>(x, y) + Eigen::Vector2<T>(T(0.5), T(0.5)), depthAndNormal[0], /*withExtrinsics=*/true);
        ...
        for (int k = 0; k < int(depthmapCorrespondences.size()); ++k)
        {
            ...
            const T distanceWeight = useDistanceTreshold ? std::max<T>(T(0), (distanceThreshold - depthmapCorrespondences[k].distance) / distanceThreshold) : T(
                1);
...
void GeometryConstraints<T>::FindTarget2SourceCorrespondences(const Mesh<T>& sourceMesh)
{
    ...
    TITAN_NAMESPACE::AABBTree<T> srcAabbTree(sourceMesh.Vertices().transpose(), sourceMesh.Triangles().transpose());
    ...
    m->threadPool->AddTaskRangeAndWait(numQueries, [&](int start, int end){
            for (int k = start; k < end; ++k)
            {
                ...
                bcs[k] = BarycentricCoordinates<T>(sourceMesh.Triangles().col(tID), bc.transpose());
            }
        });
        ...
        if (point2pointWeight > T(0))
        {
            ...
            for (int i = 0; i < correspondences.srcIndices.size(); ++i)
            {
                ...
                if (weight > 0)
                {
                    ...
                    point2PointVertexConstraints.AddConstraint(vID, residual, drdV);
                }
            }
        }
        if (point2surfaceWeight > T(0))

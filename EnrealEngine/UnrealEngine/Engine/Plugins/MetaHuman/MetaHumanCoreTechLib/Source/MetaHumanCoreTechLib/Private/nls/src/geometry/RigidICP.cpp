                             ...
                             const int numIterations)
{
    ...
    for (int iter = 0; iter < numIterations; ++iter)
    {
        ...
        for (std::ptrdiff_t vID = 0; vID < numPts; ++vID)
        {
            ...
            if ((x >= 0) && (x < targetCamera.Width()) && (y >= 0) && (y < targetCamera.Height()))
            {
                ...
                if (depthAndNormal[0] > 0)
                {
                    ...
                    const float normalCompatibilityWeight = std::max<float>(0,
                                                                            (normal.dot(
                                                                                 targetNormal) - normalIncompatibilityThreshold) *
        ...
        {
            const QRigidMotion<float> deltaRigidMotion = SolveRigid<float>(sampledPts,
                                                                           targetPts,
                                                                           targetNormals,
                                                                           weights,
                                                                           rigidMotion,
                                                                           /*outputDeltaTransform=*/true);
                                 ...
                                 const int numIterations)
{
    ...
    for (int iter = 0; iter < numIterations; ++iter)
    {
        ...
        if (iter > 0) // ignore first iteration to get a better distance threshold
        {
            const QRigidMotion<float> deltaRigidMotion = SolveRigid(sampledPtsX.data(),
                                                                    sampledPtsY.data(),
                                                                    sampledPtsZ.data(),
                                                                    targetPtsX.data(),
                                                                    targetPtsY.data(),
                                                                    ...
                                                                    targetNormalsZ.data(),
                                                                    weights.data(),
                                                                    static_cast<int>(numPaddedPts / SimdType::Size()),
                                                                    rigidMotion,
                                                                    /*outputDeltaTransform=*/true);

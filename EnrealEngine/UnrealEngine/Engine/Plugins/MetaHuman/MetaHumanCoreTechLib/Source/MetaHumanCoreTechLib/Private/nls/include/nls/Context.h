...
public:
    ...
    void Set(const Vector<T>& x)
    {
        ...
        {
            CARBON_CRITICAL("x size and value size do not match: {} vs {}", x.size(), m_valueSize);
        }
    ...
    void Update(const Vector<T>& dx)
    {
        ...
        {
            CARBON_CRITICAL("dx size and update size do not match: {} vs {}", dx.size(), UpdateSize());
        }
    ...
    const VariableInfo& GetVariableInfo(const Variable<T>* var) const
    {
        ...
        {
            CARBON_CRITICAL("Variable is not mapped by context");
        }
    }
...
private:
    JacobianConstPtr<T> LocalToGlobalJacobian(const SparseMatrixConstPtr<T>& localJacobian, int columnIndex)
    {
        ...
        // resize jacobian to fit max column (note that this is for free as m_size > cols)
        globalJacobian->conservativeResize(localJacobian->rows(), columnIndex + localJacobian->cols());
        int* columnIndices = globalJacobian->innerIndexPtr();
        for (int i = 0; i < globalJacobian->nonZeros(); i++)
        {
            columnIndices[i] += columnIndex;
        }
        return std::make_shared<SparseJacobian<T>>(globalJacobian, columnIndex);
    }

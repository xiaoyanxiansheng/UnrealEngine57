...
DiffDataMatrix<T, 3, -1> DeformationModelRigidScale<T>::EvaluateVertices(Context<T>* context)
{
    ...
    // apply scale
    DiffDataMatrix<T, 1, 1> scaleMatrix(m->varScale.Evaluate(context));
    ...
    DiffDataMatrix<T, 3, -1> scaledAndCenteredVertices(3, static_cast<int>(centeredVertices.cols()), std::move(scaledAndCenteredVerticesFlattenedVertices));
...
template <class T>
void DeformationModelRigidScale<T>::SetConfiguration(const Configuration& config) { m->config.Set(config); }
...
Affine<T, 3, 3> DeformationModelRigidScale<T>::RigidTransformation(bool preScale) const
{
    ...
    Affine<T, 3,
           3> affine = Affine<T, 3, 3>::FromTranslation(m->centerOfGravity) * m->varAffine.Affine() * Affine<T, 3, 3>::FromTranslation(
        Scale() * (-m->centerOfGravity));
    if (!preScale)
    {

...
namespace core
{
	...
	void BurnLineIntoImage(const FVector2D& InStartPoint, const FVector2D& InEndPoint, int32 InWidth, int32 InHeight, TArray<uint8>& OutImgData, uint8 InRed, uint8 InGreen, uint8 InBlue, int32 InLineWidth, bool bInUseAntiAliasing)
	{
		TArray<FVector2D> Points;
		FVector2D CurPoint = InStartPoint;
		FVector2D Diff = InEndPoint - InStartPoint;
		double Length = Diff.Length();
		int32 NumPts = FMath::FloorToDouble(Length);
		FVector2D Step = Diff / NumPts;
		for (int32 Pt = 0; Pt < NumPts; ++Pt)
		{
			Points.Push(CurPoint);
			CurPoint += Step;
		}
		Points.Push(InEndPoint);

		BurnPointsIntoImage(Points, InWidth, InHeight, OutImgData, InRed, InGreen, InBlue, InLineWidth, bInUseAntiAliasing);
	}

	void BurnLineIntoImage(const FVector2D& InStartPoint, const FVector2D& InEndPoint, int32 InWidth, int32 InHeight, FColor* OutImgData, uint8 InRed, uint8 InGreen, uint8 InBlue, int32 InLineWidth, bool bInUseAntiAliasing)
	{
		TArray<FVector2D> Points;
		FVector2D CurPoint = InStartPoint;
		FVector2D Diff = InEndPoint - InStartPoint;

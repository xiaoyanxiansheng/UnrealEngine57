...
/**
 * Class used to determine and validate the image resolution for a set of Cubic camera data.
 *
...
public:
	...
	TValueOrError<FIntPoint, EAddError> Add(const FCubicTakeInfo::FCamera& Camera);
	TValueOrError<FIntPoint, EResolveError> Resolve() const;

private:
	TValueOrError<FIntPoint, EAddError> GetCameraResolution(const FCubicTakeInfo::FCamera& Camera) const;

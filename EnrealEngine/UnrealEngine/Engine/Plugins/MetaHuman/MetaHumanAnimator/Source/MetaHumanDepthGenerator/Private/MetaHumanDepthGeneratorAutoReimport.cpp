...
DEFINE_LOG_CATEGORY_STATIC(LogDepthGeneratorAutoReimport, Log, All);

namespace UE::MetaHuman
{
static bool DepthGeneratorDirectoryConfigIsEqual(const FAutoReimportDirectoryConfig& Lhs, const FAutoReimportDirectoryConfig& Rhs)
{
...
)
{
	...
	for (int32 DirectoryConfigIndex = 0; DirectoryConfigIndex < DirectoryConfigs.Num(); ++DirectoryConfigIndex)
	{
		...
		for (const int32 IndexToRemove : WildcardIndicesToRemove)
		{
			...
			UE_LOG(LogDepthGeneratorAutoReimport, Display, TEXT("Removed redundant auto reimport wildcard \"%s\" from source directory %s"), *RemovedWildcard, *DirectoryConfig.SourceDirectory);
		}
	...
	for (const int32 IndexToRemove : DirectoryConfigIndicesToRemove)
	{
		...
		UE_LOG(LogDepthGeneratorAutoReimport, Display, TEXT("Removed empty auto reimport directory config: %s"), *RemovedSourceDirectory);
	}
...
bool DepthGeneratorDirectoryConfigsAreDifferent(const TArray<FAutoReimportDirectoryConfig>& Lhs, const TArray<FAutoReimportDirectoryConfig>& Rhs)
{
	if (Lhs.Num() != Rhs.Num())
	{
		return true;
	...
	for (int32 DirectoryIndex = 0; DirectoryIndex < Lhs.Num(); ++DirectoryIndex)
	{
		...
		if (!DepthGeneratorDirectoryConfigIsEqual(LhsConfig, RhsConfig))
		{
			return true;
		}
	}

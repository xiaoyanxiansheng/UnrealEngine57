...
class FAnimSrcNode : public FNode
{
public:

	FAnimSrcNode(const FString& InName);

	virtual bool Process(const TSharedPtr<FPipelineData>& InPipelineData) override;

	int32 NumberOfFrames = 1;
	TMap<FString, float> AnimationData;
	TArray<TMap<FString, float>> AnimationDataPerFrame;
};



class FDepthSrcNode : public FNode
{
public:

	FDepthSrcNode(const FString& InName);

	virtual bool Process(const TSharedPtr<FPipelineData>& InPipelineData) override;

	FDepthDataType DepthData;
};

...
public:
	...
	UE_API bool				IsScalar() const; // float or int
	UE_API bool				IsBool() const; // boolean
	UE_API bool				IsString() const; // string  but NOT FName
	UE_API bool				IsColor() const; // FlinearColor
	UE_API bool				IsVector() const; // FLinearColor or ...
	UE_API bool				IsTexture() const; // FTG_Texture or ...
	UE_API bool				IsArray() const; // FTG_TextureArray or ...
	UE_API bool				IsVariant() const; // FTG_Variant
	UE_API bool				IsObject(UClass* InClass) const; // check that the Argument is a UObject and of baseType InClass
	UE_API bool				GetMetaData(const FName KeyToFind, FString& OutValue);
	
    // Hashing
    static UE_API FTG_Hash Hash(const FTG_Argument& Argument);
    FTG_Hash Hash() const { return Hash(*this); }

    UE_API const static FTG_Argument Invalid;
...
public:
    ...
    UE_API FTG_Index FindName(FTG_Name& Name) const;

    // Find the Index of a argument at a particular hash.
    // Return INVALID_INDEX if not found
    UE_API FTG_Index FindHash(FTG_Hash Hash) const;

    // Hashing
    static UE_API FTG_Hash Hash(const FTG_ArgumentSet& Set);
    FTG_Hash Hash() const { return Hash(*this); }
...
public:
	...
	UE_API FTG_Signature(const FInit& Init);
    FTG_Signature() {}
	...
	UE_API const FTG_Argument& GetArgument(int32 ArgIdx);
	...
	UE_API IndexAccess FindArgumentIndexFromName(FTG_Name& InName) const;
	UE_API const FTG_Argument& FindArgumentFromName(FTG_Name& InName) const;

    // Hashing
    static UE_API FTG_Hash Hash(const FTG_Signature& Type);
    FTG_Hash Hash() const { return Hash(*this); }

	UE_API const FTG_Argument& FindArgumentFromHash(FTG_Hash Hash) const;
	UE_API IndexAccess FindArgumentIndexFromHash(FTG_Hash Hash) const;
	...
	static UE_API FTG_Indices GenerateMappingArgIdxTable(const FTG_Signature& FromSignature, const FTG_Signature& ToSignature);
};

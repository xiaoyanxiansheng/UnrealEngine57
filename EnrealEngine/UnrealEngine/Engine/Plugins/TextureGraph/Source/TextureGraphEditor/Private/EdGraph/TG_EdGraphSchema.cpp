...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TG_EdGraphSchema)
...
//	bool bPreventInvalidConnections = CVarPreventInvalidMaterialConnections.GetValueOnGameThread() != 0;
	...
	{
		const EMaterialValueType InputType = MCT_Float1; //GetMaterialValueType(InputPin);
	...
	{
		const EMaterialValueType OutputType = MCT_Float1; //GetMaterialValueType(InputPin);
...
void UTG_EdGraphSchema::DroppedAssetsOnNode(const TArray<FAssetData>& Assets, const FVector2f& GraphPosition, UEdGraphNode* Node) const
{
	...
	if(EdNode)
	{
		const FAssetData& AssetData = Assets.Last();
		UObject* Asset = AssetData.GetAsset();
		UTG_Expression* Expression = EdNode->GetNode()->GetExpression();

		if(Expression->CanHandleAsset(Asset))
		{
			const TSharedPtr<IAssetReferenceFilter> AssetReferenceFilter = MakeAssetReferenceFilter(EdNode->GetGraph());
			if (!AssetReferenceFilter.IsValid() || AssetReferenceFilter->PassesFilter(AssetData))
			{
	...
	FString& OutTooltipText, bool& OutOkIcon) const
{
	...
	for (const FAssetData& AssetData : Assets)
	{
		...
		if (TextureHelper::CanSupportTexture(Cast<UTexture>(Asset)) ||
			Cast<UMaterialInterface>(Asset) != nullptr ||
			Cast<UMaterialFunctionInterface>(Asset) != nullptr ||
			Cast<UTextureGraph>(Asset) != nullptr)
		{
...
TSharedPtr<IAssetReferenceFilter> UTG_EdGraphSchema::MakeAssetReferenceFilter(const UEdGraph* Graph)
{
	if (const UTG_EdGraph* EdNode = Cast<const UTG_EdGraph>(Graph))
	{
		if (TSharedPtr<FTG_Editor> TGEditor = EdNode->TGEditor.Pin())
		{
			return TGEditor->MakeAssetReferenceFilter();
		}
	}

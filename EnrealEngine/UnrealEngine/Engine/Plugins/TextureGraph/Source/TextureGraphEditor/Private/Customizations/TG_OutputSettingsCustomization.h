...
public:
	...
	virtual void CustomizeChildren(TSharedRef<IPropertyHandle> PropertyHandle, IDetailChildrenBuilder& ChildBuilder, IPropertyTypeCustomizationUtils& CustomizationUtils) override
	{
		...
		UTextureGraphBase* TextureGraph = CastChecked<UTextureGraphBase>(OutermostObject);
		...
		TObjectPtr<UTG_Pin> TGPin = OutputExpression->GetParentNode()->GetPin(GET_MEMBER_NAME_CHECKED(UTG_Expression_Output, OutputSettings));
		FProperty* Property = TGPin->GetExpressionProperty();
		bool bCanEditChange = OutputExpression->CanEditChange(Property);

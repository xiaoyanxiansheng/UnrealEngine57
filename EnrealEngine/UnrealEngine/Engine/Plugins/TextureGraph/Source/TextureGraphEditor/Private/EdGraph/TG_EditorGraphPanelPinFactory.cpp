...
#include "Pins/STG_GraphPinTextureDescriptor.h"
#include "Pins/STG_GraphPinString.h"
#include "Pins/STG_GraphPinMaterial.h"
...
TSharedPtr<class SGraphPin> FTG_EditorGraphPanelPinFactory::CreatePin(class UEdGraphPin* InPin) const
{
	if (InPin->GetSchema()->IsA<UTG_EdGraphSchema>())
	{
		...
		else if (InPin->PinType.PinCategory == UTG_EdGraphSchema::PC_Struct)
		{
			...
			UScriptStruct* TextureDescSettingsStruct = TBaseStructure<FTG_TextureDescriptor>::Get();
			UScriptStruct* TG_Material = TBaseStructure<FTG_Material>::Get();
			...
			else if (InPin->PinType.PinSubCategoryObject == TextureDescSettingsStruct)
			{
				return SNew(STG_GraphPinTextureDescriptor, InPin);
			}
			else if (InPin->PinType.PinSubCategoryObject == TG_Material)
			{
				return SNew(STG_GraphPinMaterial, InPin);
			}
		}
...
class TG_GraphPinEnum : public SGraphPinEnum
{
public:
...
{
	TSharedPtr<SGraphPin> PinEnum = SNew(TG_GraphPinEnum, InPin);

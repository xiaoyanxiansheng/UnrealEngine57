		...
		if (ParameterInfoPtr != nullptr)
		{
			...
			if (PinPtr != nullptr)
			{
				if (PinPtr->IsInput())
				{
					...
					FName PinName = PinPtr->GetArgumentName();
					...
					{
						if (PinName == GET_MEMBER_NAME_CHECKED(UTG_Expression_Texture, Source))
						{
							// skip the Source pin to be shown directly
							return;
						}
						else if (PinName == GET_MEMBER_NAME_CHECKED(UTG_Expression_Texture, Texture))
						{
							// allow the Texture pin instead with the correct parameter alias name but internally use the Source pin.

...
public:
	...
	static UE_API const DeviceBufferRecord null;
...
public:
	...
	static UE_API const BlobRecord			null;
...
public:
	// adding a default constructor to enforce this to be a non POD class to successfully initialize
	JobRecord() {}
	static UE_API const JobRecord	GNull;
...
public:
	...
	static UE_API const BatchRecord null;
...
public:
	// adding a default constructor to enforce this to be a non POD class to successfully initialize
	ActionRecord() {}
	static UE_API const ActionRecord null;
...
public:
	...
	static UE_API const MixRecord null;
...
public:
	...
	// Batch records
	UE_API RecordID						FindBatchRecord(BatchID InBatchID) const;
	...
	UE_API RecordID						NewBatchRecord(BatchID InBatchID, BatchRecordConstructor Constructor);
	UE_API bool							EditBatchRecord(RecordID RecId, BatchRecordEditor Editor);

	// Device Buffer records
	UE_API std::tuple<RecordID, bool>		NewDeviceBufferRecord(DeviceType DevType, DeviceBufferID BufferID, const DeviceBufferRecord& Record);
	UE_API RecordID						EraseDeviceBufferRecord(DeviceType DevType, DeviceBufferID BufferID); /// Assuming the db is in the current map, then move its key in the erased map and tag it erased

	/// Find the records from the array of bufferIDs & output the leaked buffers
	UE_API RecordIDArray    				FindActiveDeviceBufferRecords(DeviceType DevType, const DeviceBufferIDArray& BufferIDs, RecordIDArray& LeakedBuffers);
	
	UE_API RecordID						FindDeviceBufferRecord(DeviceBufferID BufferID) const;
	UE_API RecordIDArray					FindDeviceBufferRecords(const DeviceBufferIDArray& BufferIDs) const;
	UE_API RecordIDArray					FetchActiveDeviceBufferIDs(DeviceType DevType) const;

	UE_API RecordID						FindDeviceBufferRecordFromHash(HashType InHash) const;

	// Blob records
	UE_API RecordID						FindBlobRecord(BlobID InBlobID) const;
	using BlobRecordConstructor		= std::function<BlobRecord(RecordID RecId)>;
	using BlobRecordEditor			= std::function<bool(BlobRecord& r)>;
	UE_API RecordID						NewBlobRecord(BlobID InBlobID, BlobRecordConstructor Constructor);
	UE_API bool							EditBlobRecord(RecordID RecId, BlobRecordEditor Editor);

	UE_API RecordID						NewActionRecord(ActionID InActionID, const ActionRecord& Record);
	UE_API RecordID						FindActionRecord(ActionID InActionID) const;

	// Mix records
	UE_API RecordID						FindMixRecord(MixID MixID) const;
	using MixRecordConstructor		= std::function<MixRecord(RecordID RecId)>;
	using MixRecordEditor			= std::function<bool(MixRecord& r)>;
	UE_API RecordID						NewMixRecord(BatchID InBatchID, MixRecordConstructor constructor);
	UE_API bool							EditMixRecord(RecordID RecId, MixRecordEditor editor);

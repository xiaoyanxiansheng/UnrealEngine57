...
{
	TOptional<DrawTilesSettings> DrawSettingsCopy;
	if (DrawSettings != nullptr)
	{
		DrawSettingsCopy = *DrawSettings;
	}

	TOptional<FLinearColor> ClearColorCopy;
	if (ClearColor != nullptr)
	{
		ClearColorCopy = *ClearColor;
	}

	DeviceNativeTaskPtr Task = DeviceNativeTask_Lambda::Create(this, (int32)E_Priority::kSystem, TEXT("DrawTilesToBuffer"), [this, Buffer, Tiles, DrawSettingsCopy, ClearColorCopy]() mutable -> int32
		{
			...
			if (ClearColorCopy.IsSet())
				DrawClearQuad(DevRHI, *ClearColorCopy);

			{
				...
				bool bUseInputPositions = DrawSettingsCopy.IsSet();

				/// Verify that there are either no DrawSettings or they are setup correctly to match the input array dimensions
				check(!DrawSettingsCopy.IsSet() || (DrawSettingsCopy->Position.Rows() == Tiles.Rows() && DrawSettingsCopy->Position.Cols() == Tiles.Cols()));
				check(!DrawSettingsCopy.IsSet() || (DrawSettingsCopy->Size.Rows() == Tiles.Rows() && DrawSettingsCopy->Size.Cols() == Tiles.Cols()));
				...
				for (int32 RowId = 0; RowId < Tiles.Rows(); RowId++)
				{
					...
					for (int32 ColId = 0; ColId < Tiles.Cols(); ColId++)
					{
						...
						{
							const FIntPoint& TilePos = DrawSettingsCopy->Position(RowId, ColId);
							X1 = (uint32)TilePos.X;
							Y1 = (uint32)TilePos.Y;

							const FIntPoint& TileSize = DrawSettingsCopy->Size(RowId, ColId);
...
int32 Device_FX::InitRTResource(TexPtr TextureObj, UTextureRenderTarget* RT)
{
	...
	UE_LOG(LogDevice, Verbose, TEXT("New RT allocation: %s %llu [Ptr: 0x%p, Size: %ux%u]"), *TextureObj->GetDescriptor().Name,
		TextureObj->GetDescriptor().Format_HashValue(), RT, RTRes->GetSizeX(), RTRes->GetSizeY());
	
	FTextureRHIRef RHITexture = RTRes->GetShaderResourceTexture();
	check(RHITexture);

...
class BlobberObserverSource
{
public:
	using HashArray = std::vector<HashType>;

	UE_API void AddHash(HashType Hash);
	UE_API void RemapHash(HashType OldHash, HashType NewHash);
...
public:
	UE_API void							AddReferencedBlob(Blob* BlobObj, JobArg* Arg);
	UE_API void							RemoveReferencedBlob(Blob* BlobObj, JobArg* Arg);
	UE_API bool							IsBlobReferenced(Blob* BlobObj);
private:
#endif 
	...
	UE_API void							AddHashMapping(HashType LHS, CHashPtr RHS);
	UE_API void							AddBlobEntry(HashType Hash, BlobPtr BlobObj, BlobCacheOptions Options);
	UE_API void							AddBlobEntryThreadSafe(HashType Hash, BlobPtr BlobObj, BlobCacheOptions Options);
	UE_API void							RemoveHashMapping(HashType Hash);

	UE_API void							PrintStats();
	UE_API BlobRef							AddInternal(BlobPtr BlobObj, BlobCacheOptions Options);
	UE_API BlobPtr							Remove(HashType Hash);

	UE_API CHashPtr						AddGloballyUniqueHashInternal(CHashPtr Hash);
	UE_API void							UpdateGloballyUniqueHashInternal(HashType OldValue);
	UE_API void							UpdateMappingInternal(HashType OldValue, CHashPtr NewValue);
	UE_API void							UpdateBlobLookupInternal(HashType OldValue, BlobPtr BlobObj);
	UE_API void							Touch(const Blob* BlobObj);
	UE_API BlobCacheEntryPtr				FindInternal(HashType Hash);
	UE_API BlobRef							AddResult(CHashPtr LHash, BlobRef Result, BlobCacheOptions Options);
	UE_API void							UpdateTransient();
	UE_API void							UpdateTouchedHashes();
	UE_API void							UpdateBlobCache();

public:
									UE_API Blobber();
									UE_API ~Blobber();

	UE_API BlobRef							FindSingle(HashType Hash);
	UE_API TiledBlobRef					FindTiled(HashType Hash);

	UE_API BlobRef							Create(const BufferDescriptor& Desc);
	UE_API BlobRef							Create(Device* Dev, RawBufferPtr Raw);
	UE_API BlobRef							Create(DeviceBufferRef DevBuffer, bool NoCache = false);

	UE_API BlobRef							AddResult(CHashPtr LHash, BlobPtr InBlob, BlobCacheOptions Options = {});
	...
	UE_API TiledBlobRef					AddTiledResult(CHashPtr LHash, TiledBlobPtr Result, BlobCacheOptions Options = {});
	UE_API TiledBlobRef					AddTiledResult(TiledBlobPtr Result, BlobCacheOptions Options = {});

	UE_API void							UpdateBlobHash(HashType OldHash, BlobRef InBlob);
	UE_API void							UpdateHash(HashType OldHash, CHashPtr NewHash);
	UE_API void							Update(float DT);
	UE_API AsyncJobResultPtr				UpdateIdle();
	UE_API bool							IsBlobCached(HashType Hash);

	UE_API void							RegisterObserverSource(const BlobberObserverSourcePtr& observerSource); ///

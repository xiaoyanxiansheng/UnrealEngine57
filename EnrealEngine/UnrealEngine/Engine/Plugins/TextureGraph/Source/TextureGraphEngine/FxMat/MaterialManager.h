...
UCLASS(MinimalAPI)
class UMaterialManager : public UModelObject
{
	GENERATED_BODY()
...
private:
	...
	UE_API void							InitEssentialMaterials();
	

public:
									UE_API UMaterialManager();
	UE_API virtual							~UMaterialManager() override;
	
	UE_API class UMaterial*				LoadMaterial(const FString& Path);	

	UE_API RenderMaterial_BPPtr			CreateMaterial_BP(FString Name, const FString& Path, int32 InNumWarmupFrames = 0);
	UE_API RenderMaterial_BPPtr			CreateMaterial_BP(FString InName, UMaterial* InMaterial, int32 InNumWarmupFrames = 0);
	UE_API RenderMaterial_BP_NoTilePtr		CreateMaterial_BP_NoTile(FString Name, const FString& Path, int32 InNumWarmupFrames = 0);
	UE_API RenderMaterial_ThumbPtr			CreateMaterial_Thumbnail(FString Name, const FString& Path);
	UE_API RenderMaterial_BP_TileArgsPtr	CreateMaterialOfType_BP_TileArgs(FString Name, const FString& Path, int32 InNumWarmupFrames = 0);

...
class UMixSettings : public UObject
{
	GENERATED_BODY()
...
private:
	...
	UE_API void							SetMesh(RenderMeshPtr mesh);
	
protected:

#if WITH_EDITOR
	UE_API virtual void					PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent) override;
	UE_API virtual void					PostEditChangeChainProperty(FPropertyChangedChainEvent& PropertyChangedEvent) override; // For TArray type properties in the component.
#endif

public:
	UE_API virtual							~UMixSettings() override;

	UE_API void							FreeTargets();
	UE_API void							InitTargets(size_t count);
	UE_API void							SetTarget(size_t index, TargetTextureSetPtr& target);
	UE_API virtual void					Free();
	
#if WITH_EDITOR
	UE_API AsyncActionResultPtr			SetEditorMesh(AActor* actor);
	UE_API AsyncActionResultPtr			SetEditorMesh(UStaticMeshComponent* meshComponent,UWorld* world);
#endif

	UE_API RenderMeshPtr					GetMesh();
	UE_API UMixInterface*					Mix() const;
	
	UE_API FViewportSettings&				GetViewportSettings();

...
static TAutoConsoleVariable<bool> CVarAllowTextureGraphInCommandlets(
	TEXT("TextureGraph.AllowCommandlets"),
	false,
	TEXT("Allow TextureGraph engine to be initialized when running via Commandlets")
	);
...
#if WITH_EDITOR
	// Early out if commandlet execution is not allowed
...
bool TextureGraphEngine::BlockCommandlets()
{
	return !CVarAllowTextureGraphInCommandlets.GetValueOnAnyThread()
	#if WITH_EDITOR
		&& !(GEditor && FSlateApplication::IsInitialized() && FApp::CanEverRender())
	#endif
	;
}

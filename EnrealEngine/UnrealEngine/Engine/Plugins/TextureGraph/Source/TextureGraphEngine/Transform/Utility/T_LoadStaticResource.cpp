...
FString Job_LoadStaticImageResource::GetReferencerName() const
{
	if (Source)
		return Source->GetAssetUUID();
	return TEXT("Job_LoadStaticImageResource");
}

void Job_LoadStaticImageResource::AddReferencedObjects(FReferenceCollector& Collector)
{
	if (Source)
		Collector.AddReferencedObject(Source);
}

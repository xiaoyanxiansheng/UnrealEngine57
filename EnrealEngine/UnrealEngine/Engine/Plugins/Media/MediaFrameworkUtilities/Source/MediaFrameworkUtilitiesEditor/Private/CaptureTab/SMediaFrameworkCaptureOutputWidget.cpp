...
#include "SMediaFrameworkCapture.h"
...
namespace MediaFrameworkUtilities
{
	...
	constexpr float ViewportBoxDesiredSizeY = 200.f;
	 ...
	 */
	FIntPoint GetTargetSize(const TWeakObjectPtr<UMediaOutput>& InMediaOutputWeak)
	{
		if (const UMediaOutput* MediaOutputPtr = InMediaOutputWeak.Get())
		{
			const FIntPoint TargetSize = MediaOutputPtr->GetRequestedSize();
	...
	float GetWidthOverride(const TWeakObjectPtr<UMediaOutput> InMediaOutputWeak)
	{
		float Ratio = 1.77777777f;
		const FIntPoint TargetSize = GetTargetSize(InMediaOutputWeak);
...
void SMediaFrameworkCaptureCameraViewportWidget::StartOutput()
{
	...
	if (MediaOutputPtr && SceneViewport.IsValid())
	{
		...
		if (MediaCapture.IsValid())
		{
			const FIntPoint TargetSize = MediaFrameworkUtilities::GetTargetSize(MediaOutput);

...
public:
	...
	/**
	 * This is the resolution to use if it is not specified by the media output.
	 */
	UPROPERTY(EditAnywhere, config, Category="Media Viewport Capture")
	FIntPoint DefaultViewportCaptureSize = FIntPoint(1920, 1080);
};
...
private:
	...
	TOptional<int32> GetDefaultViewportSize(int32 InValueIndex) const;
	void DefaultViewportSizeChanged(int32 InValue, int32 InValueIndex);
	void DefaultViewportSizeCommitted(int32 InValue, ETextCommit::Type InCommitInfo, int32 InValueIndex);

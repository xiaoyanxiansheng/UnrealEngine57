...
public:
	...
	/** Fills the list of proxy sources with the default proxy assets stored in the media framework plugin */
	void FillDefaultMediaSourceProxies(int32 NumSources, bool bSilent = false);
	
	/** Set the media proxies used by the engine. */
	void SetMediaOutputProxy(const TArray<UProxyMediaOutput*>& Proxies);

	/** Fills the list of proxy outputs with the default proxy assets stored in the media framework plugin */
	void FillDefaultMediaOutputProxies(int32 NumOutputs, bool bSilent = false);

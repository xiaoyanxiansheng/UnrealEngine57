...
FString UProxyMediaOutput::GetDescriptionString() const
{
	...
	{
		UE_LOG(LogMediaFrameworkUtilities, Warning, TEXT("UProxyMediaOutput::GetConfigurationDisplayText - Reentrant calls are not supported. Asset: %s"), *GetPathName());
		return FString();
	}
...
void UProxyMediaOutput::GetDetailsPanelInfoElements(TArray<FInfoElement>& OutInfoElements) const
{
	...
	{
		UE_LOG(LogMediaFrameworkUtilities, Warning, TEXT("UProxyMediaOutput::GetDetailsPanelInfoElements - Reentrant calls are not supported. Asset: %s"), *GetPathName());
		return;
	}

...
public:
	...
	UE_API UMediaCapture* CreateMediaCapture();
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Media|Output")
	UE_API virtual bool Validate(FString& OutFailureReason) const;

public:

	static UE_API const FIntPoint RequestCaptureSourceSize;
...
#if WITH_EDITOR
	/** Gets a string that sums up the media output's description for UI purposes */
	virtual FString GetDescriptionString() const { return FString(); }

	typedef TTuple<FText, FText, FText> FInfoElement;
	
	/**
	 * Gets a list of text containing the media source's configuration and description for UI purposes
	 * @param OutInfoElements A list of elements, where an element is constructed from a group, a label, and a value
	 */
	virtual void GetDetailsPanelInfoElements(TArray<FInfoElement>& OutInfoElements) const { }

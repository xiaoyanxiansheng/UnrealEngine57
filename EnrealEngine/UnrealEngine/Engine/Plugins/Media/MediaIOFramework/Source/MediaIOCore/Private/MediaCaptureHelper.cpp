...
bool FMediaCaptureHelper::AreInputsValid(const UE::MediaCaptureData::FCaptureFrameArgs& Args)
{
	...
	if (Args.MediaCapture->DesiredCaptureOptions.ResizeMethod == EMediaCaptureResizeMethod::ResizeInCapturePass &&
		Args.MediaCapture->GetConversionOperation() != EMediaCaptureConversionOperation::CUSTOM)
	{
		UE_LOG(LogMediaIOCore, Error, TEXT("The capture will stop for '%s'. The resize method of ResizeInCapturePass expects GetConversionOperation() to return EMediaCaptureConversionOperation::CUSTOM")
			, *Args.MediaCapture->MediaOutputName);
	...
	if (Args.MediaCapture->DesiredCaptureOptions.ResizeMethod != EMediaCaptureResizeMethod::ResizeInRenderPass && 
		Args.MediaCapture->DesiredCaptureOptions.ResizeMethod != EMediaCaptureResizeMethod::ResizeInCapturePass)
	{

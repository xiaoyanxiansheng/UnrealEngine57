...
FMediaIOCoreEncodeTime::FMediaIOCoreEncodeTime(EMediaIOCoreEncodePixelFormat InFormat, void* InBuffer, uint32 InPitch, uint32 InWidth, uint32 InHeight, uint32 XOffset, uint32 YOffset)
	: Format(InFormat)
	, Buffer(InBuffer)
	, Pitch(InPitch)
	, Width(InWidth)
...
void FMediaIOCoreEncodeTime::SetPixelScaled(uint32 InX, uint32 InY, bool InSet, uint32 InScale) const
{
	for (uint32 ScaleY = 0; ScaleY < InScale; ScaleY++)
	{
		...
		{
			SetPixel((InX+XPixelOffset) * InScale + ScaleX, (InY+YPixelOffset) * InScale + ScaleY, InSet);
		}
	}
}

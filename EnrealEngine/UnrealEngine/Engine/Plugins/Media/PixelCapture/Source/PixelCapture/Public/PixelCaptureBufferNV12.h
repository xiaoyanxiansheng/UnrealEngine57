...
public:
	 ...
	 */
	UE_API FPixelCaptureBufferNV12(int InWidth, int InHeight);
	 ...
	 */
	UE_API const uint8_t* GetDataUV() const;
	 ...
	 */
	UE_API uint8_t* GetMutableDataUV();

	/**
	 * Gets the size of the Y plane in bytes.
	 */
	UE_API int GetDataSizeY() const;

	/**
	 * Gets the size of both the UV plane in bytes.
	 */
	UE_API int GetDataSizeUV() const;

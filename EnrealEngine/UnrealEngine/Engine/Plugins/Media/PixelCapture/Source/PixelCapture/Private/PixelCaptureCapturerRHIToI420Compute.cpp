...
TSharedPtr<FPixelCaptureCapturerRHIToI420Compute> FPixelCaptureCapturerRHIToI420Compute::Create(FPixelCaptureCapturerConfig Config)
{
	return TSharedPtr<FPixelCaptureCapturerRHIToI420Compute>(new FPixelCaptureCapturerRHIToI420Compute(Config));
}

FPixelCaptureCapturerRHIToI420Compute::FPixelCaptureCapturerRHIToI420Compute(FPixelCaptureCapturerConfig& Config)
...
void FPixelCaptureCapturerRHIToI420Compute::Initialize(int32 InputWidth, int32 InputHeight)
{
	...
	PlaneYDimensions = { Config.OutputResolution.X, Config.OutputResolution.Y };
	PlaneUVDimensions = { (Config.OutputResolution.X + 1) / 2, (Config.OutputResolution.Y + 1) / 2 }; // UV is halved and rounded up

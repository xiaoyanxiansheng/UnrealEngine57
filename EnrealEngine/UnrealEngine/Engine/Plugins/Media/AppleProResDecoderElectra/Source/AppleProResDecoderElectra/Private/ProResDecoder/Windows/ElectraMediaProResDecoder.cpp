...
#include "Utilities/UtilitiesMP4.h"	// to read the ProRes header, which is given in the form of an MP4 Atom
...
public:
	...
	FVideoDecoderProResElectra(const TMap<FString, FVariant>& InOptions);
...
public:
	...
	{
		return MakeShared<FVideoDecoderProResElectra, ESPMode::ThreadSafe>(InOptions);
	}
...
FVideoDecoderProResElectra::FVideoDecoderProResElectra(const TMap<FString, FVariant>& InOptions)
...
bool FVideoDecoderProResElectra::ParseHeader(const void* InData, int32 InNumData)
{
	...
	CurrentHeader.Reset();
	Electra::UtilitiesMP4::FMP4AtomReader Reader(MakeConstArrayView<const uint8>(reinterpret_cast<const uint8*>(InData), InNumData));
	Reader.Read(CurrentHeader.FrameSizeInBytes);
	Reader.Read(CurrentHeader.FrameType);
	if (CurrentHeader.FrameType != Electra::UtilitiesMP4::MakeBoxAtom('i','c','p','f'))
	{
		return false;
	}
	...
	if (InInputAccessUnit.Data && InInputAccessUnit.DataSize)
	{
		...
		switch(OutputPixelFormat)
		{
			...
			case kPRFormat_r4fl:
			{
				...
				NewOutput->BufferFormat = EPixelFormat::PF_A32B32G32R32F;
				NewOutput->BufferEncoding = EElectraTextureSamplePixelEncoding::YCbCr_Alpha;
				NewOutput->Pitch *= 16;
				break;
			}
			...
			case kPRFormat_b64a:
			{
				NewOutput->NumBits = 16;
				NewOutput->BufferFormat = EPixelFormat::PF_A16B16G16R16;
				NewOutput->BufferEncoding = EElectraTextureSamplePixelEncoding::ARGB_BigEndian;
				NewOutput->Pitch *= 8;
				break;
			}
			case kPRFormat_R10k:
		...
		if (bUseGPUBuffers)
		{
			...
			uint32 BufferPitch;
			D3D12ResourcePool->AllocateOutputDataAsBuffer(Result, ResultMsg, NewOutput->GPUBuffer, BufferPitch);
			if (NewOutput->GPUBuffer.Resource)
			{
				// Correct output pitch to what the resource is setup for
				NewOutput->Pitch = BufferPitch;

				void* BufferAddr;
				Result = NewOutput->GPUBuffer.Resource->Map(0, nullptr, &BufferAddr);
				if (SUCCEEDED(Result))
				{
				...
				{
					PostError((int32)Result, TEXT("Failed to map DX12 texture buffer"), ERRCODE_INTERNAL_FAILED_TO_DECODE_INPUT);
					return IElectraDecoder::EDecoderError::Error;
				}
			}
			else
			{
				PostError((int32)Result, FString::Printf(TEXT("Failed to allocate DX12 texture buffer (%s)"), *ResultMsg), ERRCODE_INTERNAL_FAILED_TO_DECODE_INPUT);
				return IElectraDecoder::EDecoderError::Error;
			}
		}
		else

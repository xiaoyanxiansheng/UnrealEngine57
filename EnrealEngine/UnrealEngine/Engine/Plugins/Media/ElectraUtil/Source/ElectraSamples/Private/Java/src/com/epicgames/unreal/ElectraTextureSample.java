	...
	*/
	class FBitmapRendererNew extends FBitmapRenderer implements ImageReader.OnImageAvailableListener
	{
		...
		private FImageResources getFrameTextureInfo()
		{
			...
			{
				Log.d(TAG, String.format("getFrameTextureInfo: ImageReader.acquireNextImage() threw: %s", e.toString()));
				return null;
			}

			if (Image == null)
			{
				Log.d(TAG, "getFrameTextureInfo requested, but no images queued...");
				return null;
			}

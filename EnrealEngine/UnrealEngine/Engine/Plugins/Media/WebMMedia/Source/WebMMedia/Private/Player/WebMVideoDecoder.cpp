		...
		while (const vpx_image_t* Image = vpx_codec_get_frame(&Context, &ImageIter))
		{
			...
			if (!bTexturesCreated)
			{
				...
				ENQUEUE_RENDER_COMMAND(WebMMediaPlayerCreateTextures)(
					...
					{
						Self->CreateTextures(RHICmdList, Image);
					});
			}
			...
			ENQUEUE_RENDER_COMMAND(WebMMediaPlayerConvertYUVToRGB)(
				...
				{
					Self->ConvertYUVToRGBAndSubmit(RHICmdList, Params);
				});
		}
	}
}

void FWebMVideoDecoder::CreateTextures(FRHICommandListBase& RHICmdList, const vpx_image_t* Image)
{
	...
	DecodedY = RHICmdList.CreateTexture(DecodedYDesc);
	DecodedU = RHICmdList.CreateTexture(DecodedUDesc);
	DecodedV = RHICmdList.CreateTexture(DecodedVDesc);
}
...
void FWebMVideoDecoder::ConvertYUVToRGBAndSubmit(FRHICommandListImmediate& CommandList, const FConvertParams& Params)
{
	...
	VideoSample->CreateTexture(CommandList);

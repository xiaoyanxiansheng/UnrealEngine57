...
public:
	...
	TSharedPtr<FElectraDecodersPlatformResources::FAsyncConsecutiveTaskSync, ESPMode::ThreadSafe> GPUBuffer_TaskSync;
#endif
};
...
public:
	...
	FVideoDecoderAvidDNxHDElectra(const TMap<FString, FVariant>& InOptions);
...
public:
	...
	{
		return MakeShared<FVideoDecoderAvidDNxHDElectra, ESPMode::ThreadSafe>(InOptions);
	}
...
FVideoDecoderAvidDNxHDElectra::FVideoDecoderAvidDNxHDElectra(const TMap<FString, FVariant>& InOptions)
{
...
IElectraDecoder::EDecoderError FVideoDecoderAvidDNxHDElectra::DecodeAccessUnit(const FInputAccessUnit& InInputAccessUnit, const TMap<FString, FVariant>& InAdditionalOptions)
{
	...
	if (InInputAccessUnit.Data && InInputAccessUnit.DataSize)
	{
		...
		switch(Decoder.CurrentUncompressedParams.compType)
		{
			case DNX_CT_UCHAR:
			{
				switch(Decoder.CurrentUncompressedParams.compOrder)
				{
					...
					{
						NewOutput->BufferFormat = EPixelFormat::PF_B8G8R8A8;
						NewOutput->BufferEncoding = (Decoder.CurrentUncompressedParams.compOrder == DNX_CCO_CbYCrY_NoA) ?  EElectraTextureSamplePixelEncoding::CbY0CrY1 : EElectraTextureSamplePixelEncoding::Y0CbY1Cr;
						NewOutput->Width /= 2;
						NewOutput->Pitch *= 2;
						break;
					}
					...
					{
						NewOutput->BufferFormat = EPixelFormat::PF_A8R8G8B8;
						NewOutput->BufferEncoding = EElectraTextureSamplePixelEncoding::Native;
						NewOutput->Pitch *= 4;
						break;
					}
					case DNX_CCO_RGBA_Interleaved:
					{
						NewOutput->BufferFormat = EPixelFormat::PF_R8G8B8A8;
						NewOutput->BufferEncoding = EElectraTextureSamplePixelEncoding::Native;
						NewOutput->Pitch *= 4;
						break;
					}
					...
					{
						NewOutput->BufferFormat = EPixelFormat::PF_B8G8R8A8;
						NewOutput->BufferEncoding = EElectraTextureSamplePixelEncoding::YCbCr_Alpha;
						NewOutput->Pitch *= 4;
						break;
					}
					case DNX_CCO_YCbCr_Planar:
					{
						NewOutput->BufferFormat = EPixelFormat::PF_NV12;
						NewOutput->BufferEncoding = EElectraTextureSamplePixelEncoding::Native;
						if (!bUseGPUBuffers)
						{
					...
					default:
					{
						NewOutput->BufferFormat = EPixelFormat::PF_Unknown;
						NewOutput->BufferEncoding = EElectraTextureSamplePixelEncoding::Native;
					}
				}
				break;
			...
			case DNX_CT_USHORT_12_4:
			{
				switch(Decoder.CurrentUncompressedParams.compOrder)
				{
					...
					case DNX_CCO_CbYCrY_NoA:
					{
						NewOutput->BufferFormat = EPixelFormat::PF_A16B16G16R16;
						NewOutput->BufferEncoding = (Decoder.CurrentUncompressedParams.compOrder == DNX_CCO_CbYCrY_NoA) ? EElectraTextureSamplePixelEncoding::CbY0CrY1 : EElectraTextureSamplePixelEncoding::Y0CbY1Cr;
						NewOutput->Width /= 2;
						NewOutput->Pitch *= 4;
						break;
					...
					case DNX_CCO_RGBA_Interleaved:
					{
						NewOutput->BufferFormat = EPixelFormat::PF_R16G16B16A16_UNORM;
						NewOutput->BufferEncoding = EElectraTextureSamplePixelEncoding::Native;
						NewOutput->Pitch *= 8;
						break;
					}
					...
					case DNX_CCO_YCbCr_Planar:
					{
						NewOutput->BufferFormat = EPixelFormat::PF_P010;
						NewOutput->BufferEncoding = EElectraTextureSamplePixelEncoding::Native;
						if (!bUseGPUBuffers)
						{
							NewOutput->Height = (NewOutput->Height * 3) / 2;
			...
			case DNX_CT_V210:
			{
				NewOutput->BufferFormat = EPixelFormat::PF_A2B10G10R10;
				NewOutput->BufferEncoding = EElectraTextureSamplePixelEncoding::CbY0CrY1;
				break;
			}
			default:
			{
		...
		if (bUseGPUBuffers)
		{
			...
			if (D3D12ResourcePool->AllocateOutputDataAsBuffer(HResult, HResultMsg, NewOutput->GPUBuffer, ResourcePitch))
			{
				...
				else
				{
					...
					HResult = NewOutput->GPUBuffer.Resource->Map(0, nullptr, (void**)&OutBufferData);
					if (FAILED(HResult))
					{
						PostError((int32)HResult, FString::Printf(TEXT("Failed to map D3D12 buffer")), ERRCODE_INTERNAL_FAILED_TO_DECODE_INPUT);
						return IElectraDecoder::EDecoderError::Error;
					}
			...
			{
				PostError((int32)HResult, FString::Printf(TEXT("Failed to allocate D3D12 buffer (%s)"), *HResultMsg), ERRCODE_INTERNAL_FAILED_TO_DECODE_INPUT);
				return IElectraDecoder::EDecoderError::Error;
			}
		}
		else
#endif
		{
			NewOutput->Buffer = MakeShared<TArray64<uint8>, ESPMode::ThreadSafe>();
		...
		if (bUseGPUBuffers)
		{
			if (Result == 0)
			{
				...
				{
					if (FElectraDecodersPlatformResources::RunCodeAsync([BufferResource = NewOutput->GPUBuffer.Resource, Width = NewOutput->Width, Height = NewOutput->Height, ResourcePitch, BPP, TempBuffer, BufferFence = NewOutput->GPUBuffer.Fence, BufferFenceValue = NewOutput->GPUBuffer.FenceValue]()
						{
...
bool FVideoDecoderAvidDNxHDElectra::FDecoderHandle::SetupDecompressStruct()
{
	...
	if (CurrentCompressedParams.colorFormat == DNX_ColorFormat_t::DNX_CF_YCbCr)
	{
		switch(CurrentCompressedParams.depth)
		{
			...
			case 12:
			{
				CurrentUncompressedParams.compType = DNX_ComponentType_t::DNX_CT_USHORT_12_4;
				// FIXME: What output format would the alpha channel require us to use?
				if (DecodeOperationParams.decodeAlpha)
				{
					UE_LOG(LogAvidDNxHDElectraDecoder, Log, TEXT("Compressed alpha with YCbYCr is not currently handled"));

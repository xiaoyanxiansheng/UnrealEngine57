...
static BINKTEXTURES * Create_textures(BINKSHADERS * pshaders, HBINK bink, void * user_ptr)
{
	...
	for (int i = 0; i < bb->TotalFrames; ++i)
	{
		...
		{
			textures->Ytexture[i] = RHICmdList.CreateTexture(YADesc);
#if PLATFORM_HAS_DIRECT_TEXTURE_MEMORY_ACCESS 
		...
		{
			textures->cRtexture[i] = RHICmdList.CreateTexture(cRcBDesc);
#if PLATFORM_HAS_DIRECT_TEXTURE_MEMORY_ACCESS 
		...
		{
			textures->cBtexture[i] = RHICmdList.CreateTexture(cRcBDesc);
#if PLATFORM_HAS_DIRECT_TEXTURE_MEMORY_ACCESS 
		...
		{
			textures->Atexture[i] = RHICmdList.CreateTexture(YADesc);
#if PLATFORM_HAS_DIRECT_TEXTURE_MEMORY_ACCESS 
		...
		{
			textures->Htexture[i] = RHICmdList.CreateTexture(YADesc);
#if PLATFORM_HAS_DIRECT_TEXTURE_MEMORY_ACCESS 

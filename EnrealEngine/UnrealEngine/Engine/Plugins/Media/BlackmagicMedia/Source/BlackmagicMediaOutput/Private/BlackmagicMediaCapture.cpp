...
namespace BlackmagicMediaCaptureHelpers
{
	...
	public:
		...
		bool ShouldCaptureThisFrame(FTimecode Timecode, uint32 FrameNumber) const
		{
			return BlackmagicDesign::ShouldCaptureThisFrame(ChannelInfo, Timecode, FrameNumber);
		}
...
bool UBlackmagicMediaCapture::ShouldCaptureThisFrame(const FCaptureBaseData& InBaseData) const
{
	if (EventCallback)
	{
		return EventCallback->ShouldCaptureThisFrame(InBaseData.SourceFrameTimecode, InBaseData.SourceFrameNumber);
	}
...
bool UBlackmagicMediaCapture::InitBlackmagic(UBlackmagicMediaOutput* InBlackmagicMediaOutput)
{
	...
	ChannelOptions.bOutputInterlaceOnEvenFrames = InBlackmagicMediaOutput->bOutputInterlaceOnEvenFrames;
	...
	ChannelOptions.bEncodeTimecodeInTexel = InBlackmagicMediaOutput->bEncodeTimecodeInTexel;

...
namespace BlackmagicDesign
{
	namespace Private
	{
		...
		bool FDevice::ShouldCaptureThisFrame(const FChannelInfo& InChannelInfo, ::FTimecode Timecode, uint32 FrameNumber)
		{
			if (FOutputChannel* FoundChannel = FindChannel(InChannelInfo))
			{
				return FoundChannel->ShouldCaptureThisFrame(Timecode, FrameNumber);
			}
			return false;
		}

...
namespace BlackmagicDesign
{
	namespace Private
	{
		...
		void EncodeTimecodeInTexel(BMDPixelFormat PixelFormat, BlackmagicDesign::FTimecode Timecode, void* Buffer, int32 Width, int32 Height, int32 XOffset = 0, int32 YOffset = 0)
		{
			switch (PixelFormat)
			{
			case bmdFormat8BitYUV:
			{
				FMediaIOCoreEncodeTime EncodeTime(EMediaIOCoreEncodePixelFormat::CharUYVY, Buffer, Width * 4, Width * 2, Height, XOffset, YOffset);
				EncodeTime.Render(Timecode.Hours, Timecode.Minutes, Timecode.Seconds, Timecode.Frames);
				break;
			}
			case bmdFormat8BitBGRA:
			{
				FMediaIOCoreEncodeTime EncodeTime(EMediaIOCoreEncodePixelFormat::CharBGRA, Buffer, Width * 4, Width, Height, XOffset, YOffset);
				EncodeTime.Render(Timecode.Hours, Timecode.Minutes, Timecode.Seconds, Timecode.Frames);
				break;
			}
			case bmdFormat10BitYUV:
			{
				FMediaIOCoreEncodeTime EncodeTime(EMediaIOCoreEncodePixelFormat::YUVv210, Buffer, Width * 16, Width * 6, Height, XOffset, YOffset);
				EncodeTime.Render(Timecode.Hours, Timecode.Minutes, Timecode.Seconds, Timecode.Frames);
				break;
			}
			default:
		...
		*****************************************************************************/
		...
		bool FOutputChannel::ScheduleOutputFrame(FOutputFrame* InFrame)
		{
			...
			const int32_t VideoHeight = ChannelOptions.FormatInfo.Height;
			const uint32_t Stride = InFrame->BlackmagicFrame->GetRowBytes();
			const uint32_t VideoWidth = ChannelOptions.PixelFormat == EPixelFormat::pf_8Bits ? Stride / 4 : Stride / 16;
			
			if (ChannelOptions.FormatInfo.FieldDominance == EFieldDominance::Progressive)
			{
				if (ChannelOptions.bEncodeTimecodeInTexel)
				{
					EncodeTimecodeInTexel(PixelFormat, InFrame->Timecode, DestinationBuffer, VideoWidth, VideoHeight);
				}
			}
			else
			{
				...
				{
					EncodeTimecodeInTexel(PixelFormat, InFrame->Timecode, DestinationBuffer, VideoWidth, VideoHeight);
					EncodeTimecodeInTexel(PixelFormat, InFrame->Timecode2, DestinationBuffer, VideoWidth, VideoHeight, 0, 20);
				}

				if (InFrame->Timecode.Frames % 2 == 0)
				{
					UE_LOG(LogBlackmagicCore, VeryVerbose, TEXT("Scheduled frame starting with even timecode. (%d)"), InFrame->Timecode.Frames);
				}
				else
				{
					UE_LOG(LogBlackmagicCore, VeryVerbose, TEXT("Scheduled frame starting with odd timecode. (%d)"), InFrame->Timecode.Frames);
				}
			}
		...
		bool FOutputChannel::ShouldCaptureThisFrame(::FTimecode Timecode, uint32 FrameNumber)
		{
			// In interlace, don't capture for an odd frame if we don't find an even frame for it.
			if (ChannelOptions.IsInterlaced() && Timecode.Frames % 2 != 0 && ChannelOptions.bOutputInterlaceOnEvenFrames)
			{
				...
				for (FOutputFrame* Frame : FrameReadyToWrite)
				{
					if (Frame->FrameIdentifier != FOutputFrame::InvalidFrameIdentifier && Frame->FrameIdentifierF2 == FOutputFrame::InvalidFrameIdentifier)
					{
						if (Frame->FrameIdentifier + 1 == FrameNumber)
						{
							return true;
						}
					}
				...
				{
					UE_LOG(LogBlackmagicCore, Verbose, TEXT("Skipping output because no writable frame is available."));
				}
				else
				{
					for (FOutputFrame* Frame : FrameReadyToWrite)
					{
						UE_LOG(LogBlackmagicCore, VeryVerbose, TEXT("\t F1: %d, F2: %d"), Frame->FrameIdentifier, Frame->FrameIdentifierF2);
					}
				}

				// Dont interlace starting with an odd frame.
				return false;
			}
			else
			{
		...
		BlackmagicDesign::Private::FOutputChannel::FOutputFrame* FOutputChannel::FetchAvailableWritingFrameInterlaced(const FBaseFrameData& InFrame)
		{
			auto InitializeFrame = [&InFrame](FOutputFrame* Frame) -> FOutputFrame*
			{
				...
				else
				{
					...
					Frame->Timecode2 = InFrame.Timecode;
				}
			...
			if (!ChannelOptions.bOutputInterlacedFieldsTimecodeNeedToMatch)
			{
				// We don't use this path when using bOutputInterlaceOnEvenFrames, since frames that land on an odd 
				// frame counter in the render thread may actually be even timecode frames.
				if (!ChannelOptions.bOutputInterlaceOnEvenFrames)
				{ 

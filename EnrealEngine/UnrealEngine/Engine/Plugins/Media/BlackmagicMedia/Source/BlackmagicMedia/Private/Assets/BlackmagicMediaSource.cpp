...
#if WITH_EDITOR
FString UBlackmagicMediaSource::GetDescriptionString() const
{
...
void UBlackmagicMediaSource::GetDetailsPanelInfoElements(TArray<FInfoElement>& OutInfoElements) const
{
#define LOCTEXT_NAMESPACE "BlackmagicMediaSource"
	IMediaIOCoreDeviceProvider* DeviceProviderPtr = IMediaIOCoreModule::Get().GetDeviceProvider(FBlackmagicDeviceProvider::GetProviderName());
	if (!DeviceProviderPtr)
	{
		return;
	...
	const FText AudioText = FText::Format(LOCTEXT("AudioFormat", "{0}, {1} buffered frames"),
		UEnum::GetDisplayValueAsText(AudioChannels), FText::AsNumber(MaxNumAudioFrameBuffer));

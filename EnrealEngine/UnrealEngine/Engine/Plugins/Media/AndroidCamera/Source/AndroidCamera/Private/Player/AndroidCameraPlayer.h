...
class FAndroidCameraPlayer
	...
	, public TSharedFromThis<FAndroidCameraPlayer>
{
public:
...
private:
	...
	TAtomic<EMediaState> CurrentState;
	...
	/** Identify the game is in background */
	bool IsInBackground;

private:
	/** The struct to pass a shared pointer of this object to DeferredSetRateThread */
	struct SetRateThreadArg
	{
		TSharedPtr<FAndroidCameraPlayer> ThisPtr;
	};

	/** The thread of delaying the SetRate until the camera is prepared */
	pthread_t DeferredSetRateThread;
	TAtomic<bool> IsSetRateThreadRunning;
	
	/** The flag for exiting the SetRate thread */
	TAtomic<bool> StopSetRateThread;
};

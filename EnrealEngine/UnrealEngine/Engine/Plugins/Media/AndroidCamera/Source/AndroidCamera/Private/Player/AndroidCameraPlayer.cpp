...
FAndroidCameraPlayer::FAndroidCameraPlayer(IMediaEventSink& InEventSink)
	...
	, DeferredSetRateThread()
	, IsSetRateThreadRunning(false)
	, StopSetRateThread(false)
{
...
static void ConditionalInitializeVideoTexture_RenderThread(FRHICommandListBase& RHICmdList, TWeakPtr<FJavaAndroidCameraPlayer, ESPMode::ThreadSafe> JavaCameraPlayerPtr, FGuid PlayerGuid)
{
	...
	if (VideoTexture == nullptr)
	{
		...
		if (VideoTexture == nullptr)
		{
			UE_LOG(LogAndroidCamera, Warning, TEXT("CreateTexture failed!"));
			return;
		}
...
#if ANDROIDCAMERAPLAYER_USE_NATIVELOGGING
		FPlatformMisc::LowLevelOutputDebugStringf(TEXT("Fetch RT: Created VideoTexture: %s"), *PlayerGuid.ToString());
#endif
	}
}
...
static void ConditionalInitializeVideoSample_RenderThread(FRHICommandListBase& RHICmdList, TSharedRef<FAndroidCameraTextureSample, ESPMode::ThreadSafe> VideoSample)
{
	if (VideoSample->GetTexture() == nullptr || VideoSample->GetTexture()->GetSizeXY() != VideoSample->GetOutputDim())
	{
		VideoSample->InitializeTexture(RHICmdList, FTimespan::Zero());
	}
}

static void DoUpdateTextureCameraSampleExecute(FRHICommandListBase& RHICmdList, TWeakPtr<FJavaAndroidCameraPlayer, ESPMode::ThreadSafe> JavaCameraPlayerPtr, TWeakPtr<FMediaSamples, ESPMode::ThreadSafe> SamplesPtr,
	TSharedRef<FAndroidCameraTextureSample, ESPMode::ThreadSafe> VideoSample, FTimespan SampleTime)
{
	...
	FRHITexture* Texture = VideoSample->InitializeTexture(RHICmdList, SampleTime);
...
struct FRHICommandUpdateTextureCameraSample final : public FRHICommand<FRHICommandUpdateTextureCameraSample>
{
	...
	void Execute(FRHICommandListBase& RHICmdList)
	{
		...
		DoUpdateTextureCameraSampleExecute(RHICmdList, JavaCameraPlayerPtr, SamplesPtr, VideoSample, SampleTime);
	}
};
...
static void DoUpdateMediaFrameDataCameraSampleExecute(TWeakPtr<FJavaAndroidCameraPlayer, ESPMode::ThreadSafe> JavaCameraPlayerPtr, TWeakPtr<FMediaSamples, ESPMode::ThreadSafe> SamplesPtr,
	TSharedRef<FAndroidCameraTextureSample, ESPMode::ThreadSafe> VideoSample, FTimespan SampleTime)
{
	auto PinnedJavaCameraPlayer = JavaCameraPlayerPtr.Pin();
	auto PinnedSamples = SamplesPtr.Pin();
	...
	jobject MediaFrameData = PinnedJavaCameraPlayer->GetVideoLastMediaFrameData(CurrentFramePosition, bRegionChanged);
	if (MediaFrameData == nullptr)
	{
		return;
	}
	...
	if (PinnedJavaCameraPlayer->IsActive())
	{
		VideoSample->InitializeMediaFrameData(MediaFrameData, SampleTime);
...
struct FRHICommandUpdateMediaFrameDataCameraSample final : public FRHICommand<FRHICommandUpdateMediaFrameDataCameraSample>
{
	TWeakPtr<FJavaAndroidCameraPlayer, ESPMode::ThreadSafe> JavaCameraPlayerPtr;
	TWeakPtr<FMediaSamples, ESPMode::ThreadSafe> SamplesPtr;
	TSharedRef<FAndroidCameraTextureSample, ESPMode::ThreadSafe> VideoSample;
	...
	FORCEINLINE_DEBUGGABLE FRHICommandUpdateMediaFrameDataCameraSample(TWeakPtr<FJavaAndroidCameraPlayer, ESPMode::ThreadSafe> InJavaCameraPlayerPtr, TWeakPtr<FMediaSamples, ESPMode::ThreadSafe> InSamplesPtr,
		TSharedRef<FAndroidCameraTextureSample, ESPMode::ThreadSafe> InVideoSample, FTimespan InSampleTime)
		: JavaCameraPlayerPtr(InJavaCameraPlayerPtr)
		, SamplesPtr(InSamplesPtr)
		, VideoSample(InVideoSample)
	...
	{
		QUICK_SCOPE_CYCLE_COUNTER(STAT_FRHICommandUpdateBufferCameraSample_Execute);
		DoUpdateMediaFrameDataCameraSampleExecute(JavaCameraPlayerPtr, SamplesPtr, VideoSample, SampleTime);
	}
};
...
void FAndroidCameraPlayer::TickFetch(FTimespan DeltaTime, FTimespan Timecode)
{
	...
	if (FAndroidMisc::ShouldUseVulkan())
	{
		...
		if (JavaCameraPlayer->IsUsingBitmapRendererImageReader())
		{
			ENQUEUE_RENDER_COMMAND(AndroidCameraPlayerWriteVideoSample)(
				[Params](FRHICommandListImmediate& RHICmdList)
				{
					DoUpdateMediaFrameDataCameraSampleExecute(Params.JavaCameraPlayerPtr, Params.SamplesPtr, Params.VideoSample, Params.SampleTime);
				});
			return;
		}
	...
	else if (GSupportsImageExternal)
	{
		...
		FWriteVideoSampleParams Params = { JavaCameraPlayer, PlayerGuid, VideoSample, Samples, CurrentTime };
		...
		if (JavaCameraPlayer->IsUsingBitmapRendererImageReader())
		{
			...
			ENQUEUE_RENDER_COMMAND(AndroidCameraPlayerWriteVideoSample)(
				[Params](FRHICommandListImmediate& RHICmdList)
				{
					...
					{
						new (RHICmdList.AllocCommand<FRHICommandUpdateMediaFrameDataCameraSample>()) FRHICommandUpdateMediaFrameDataCameraSample(Params.JavaCameraPlayerPtr, Params.Samples, Params.VideoSample, Params.SampleTime);
						RHICmdList.ImmediateFlush(EImmediateFlushType::FlushRHIThread);
					}
					else
					{
						DoUpdateMediaFrameDataCameraSampleExecute(Params.JavaCameraPlayerPtr, Params.Samples, Params.VideoSample, Params.SampleTime);
					}
				});
			return;
		}
		...
		ENQUEUE_RENDER_COMMAND(AndroidCameraPlayerWriteVideoSample)(
			[Params](FRHICommandListImmediate& RHICmdList)
			{
				ConditionalInitializeVideoTexture_RenderThread(RHICmdList, Params.JavaCameraPlayerPtr, Params.PlayerGuid);
				if (IsRunningRHIInSeparateThread())
				{
				...
				Params.VideoSample->InitializeExternalTexture(Params.SampleTime);
				
				// Update the associated data of the registered external texture
				FTextureRHIRef SampleTexture;
				FSamplerStateRHIRef SamplerState;
				if (FExternalTextureRegistry::Get().GetExternalTexture(nullptr, Params.PlayerGuid, SampleTexture, SamplerState))
				{
					FVector4 ScaleRotation = PinnedJavaCameraPlayer->GetScaleRotation();
					FVector4 Offset = PinnedJavaCameraPlayer->GetOffset();
					FExternalTextureRegistry::Get().RegisterExternalTexture(Params.PlayerGuid, SampleTexture, SamplerState,
						FLinearColor(ScaleRotation.X, ScaleRotation.Y, ScaleRotation.Z, ScaleRotation.W),
						FLinearColor(Offset.X, Offset.Y, Offset.Z, Offset.W));
					PinnedJavaCameraPlayer->SetVideoTextureValid(true);
				}
	...
	else
	{
		...
		ENQUEUE_RENDER_COMMAND(AndroidCameraPlayerWriteVideoSample)(
			[Params](FRHICommandListImmediate& RHICmdList)
			{
				ConditionalInitializeVideoSample_RenderThread(RHICmdList, Params.VideoSample);
				if (IsRunningRHIInSeparateThread())
				{
				...
				{
					DoUpdateTextureCameraSampleExecute(RHICmdList, Params.JavaCameraPlayerPtr, Params.SamplesPtr, Params.VideoSample, Params.SampleTime);
				}
			});
	}
	...
	if (JavaCameraPlayer->IsUsingBitmapRendererImageReader())
	{
		ENQUEUE_RENDER_COMMAND(AndroidCameraPlayerWriteVideoSample)(
			[Params](FRHICommandListImmediate& RHICmdList)
			{
				if (IsRunningRHIInSeparateThread())
				{
					new (RHICmdList.AllocCommand<FRHICommandUpdateMediaFrameDataCameraSample>()) FRHICommandUpdateMediaFrameDataCameraSample(Params.JavaCameraPlayerPtr, Params.SamplesPtr, Params.VideoSample, Params.SampleTime);
					RHICmdList.ImmediateFlush(EImmediateFlushType::FlushRHIThread);
				}
				else
				{
					DoUpdateBufferCameraSampleExecute(Params.JavaCameraPlayerPtr, Params.SamplesPtr, Params.VideoSample, Params.SampleTime);
				}
			});
		return;
	}
...
void FAndroidCameraPlayer::TickInput(FTimespan DeltaTime, FTimespan Timecode)
{
	...
	{
		// remove delegates if registered
		if (ResumeHandle.IsValid() && !IsInBackground)
		{
			FCoreDelegates::ApplicationHasEnteredForegroundDelegate.Remove(ResumeHandle);
			ResumeHandle.Reset();
		}
		...
		if (CurrentState == EMediaState::Preparing && JavaCameraPlayer->IsPrepared())
		{
			InitializePlayer();
		}
...
bool FAndroidCameraPlayer::SetRate(float Rate)
{
	...
	if (CurrentState == EMediaState::Preparing && Rate == 1.0f)
	{
		if (IsSetRateThreadRunning)
		{
			UE_LOG(LogAndroidCamera, Warning, TEXT("It's unnecessary to Play the camera player repeatedly."));
			return true;
		}

		UE_LOG(LogAndroidCamera, Log, TEXT("Camera is under preparation."));

		StopSetRateThread = false;

		SetRateThreadArg* ThreadArg = new SetRateThreadArg();
		ThreadArg->ThisPtr = StaticCastSharedRef<FAndroidCameraPlayer>(AsShared());

		int ret = pthread_create(&DeferredSetRateThread, NULL, [] (void* arg) -> void* {
			SetRateThreadArg* Arg = static_cast<SetRateThreadArg*>(arg);
			if (Arg->ThisPtr.IsValid())
			{
				Arg->ThisPtr->IsSetRateThreadRunning = true;

				UE_LOG(LogAndroidCamera, Log, TEXT("Waiting for camera to become ready..."));

				float ExitTime = FPlatformTime::Seconds() + 3.0f;
				while (true)
				{
					if (FPlatformTime::Seconds() > ExitTime)
					{
						UE_LOG(LogAndroidCamera, Log, TEXT("Timed out after waiting for camera preparation for 3 seconds."));
						break;
					}
					if (Arg->ThisPtr->StopSetRateThread)
					{
						UE_LOG(LogAndroidCamera, Log, TEXT("Stopped waiting for camera preparation."));
						break;
					}
					if ((Arg->ThisPtr->CurrentState != EMediaState::Preparing) &&
						(Arg->ThisPtr->CurrentState != EMediaState::Error) &&
						(Arg->ThisPtr->CurrentState != EMediaState::Closed))
					{
						UE_LOG(LogAndroidCamera, Log, TEXT("Camera is ready."));
						Arg->ThisPtr->SetRate(1.0f);
						break;
					}
					usleep(100);
				}

				Arg->ThisPtr->IsSetRateThreadRunning = false;
			}
			else
			{
				UE_LOG(LogAndroidCamera, Error, TEXT("The camera player object has been deleted."));
			}
	...
	if (Rate == 0.0f)
	{
		if (IsSetRateThreadRunning)
		{
			StopSetRateThread = true;
			pthread_join(DeferredSetRateThread, nullptr);
			StopSetRateThread = false;
			IsSetRateThreadRunning = false;
		}
...
{
	// prevent the HandleApplicationHasEnteredForeground from removed in TickInput()
	IsInBackground = true;
...
void FAndroidCameraPlayer::HandleApplicationHasEnteredForeground()
{
	...
	// check state in case changed before ticked
	if (JavaCameraPlayer.IsValid() && CurrentState == EMediaState::Closed)
	{
		if (JavaCameraPlayer->RestoreCamera())
		{
			CurrentState = EMediaState::Preparing;
			SetRate(1.0f);
		}
		...
		{
			UE_LOG(LogAndroidCamera, Warning, TEXT("HandleApplicationHasEnteredForeground - RestoreCamera failed."));
		}
	}
}

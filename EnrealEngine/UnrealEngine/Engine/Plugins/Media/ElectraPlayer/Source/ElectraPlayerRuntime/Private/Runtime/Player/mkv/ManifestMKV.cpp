 ...
 */
void FManifestMKVInternal::FPlayPeriodMKV::GetAverageSegmentDuration(FTimeValue& OutAverageSegmentDuration, const FString& AdaptationSetID, const FString& RepresentationID)
{
	TSharedPtrTS<FTimelineAssetMKV> ma = MediaAsset.Pin();
	if (ma.IsValid())
	{
...
IManifest::FResult FManifestMKVInternal::FTimelineAssetMKV::GetStartingSegment(TSharedPtrTS<IStreamSegment>& OutSegment, const FPlayerSequenceState& InSequenceState, FString InSelectedStreamID, const FPlayStartPosition& StartPosition, ESearchType SearchType, int64 AtAbsoluteFilePos)
{
	...
	if (Repr.IsValid())
	{
		...
		if (Track)
		{
			...
			{
				firstTimestamp = StreamDuration;
				Error = UEMEDIA_ERROR_END_OF_STREAM;
			}
			else
			{
				Error = TrackIt->StartAtTime(StartPosition.Time, SearchMode);
				firstTimestamp = Error == UEMEDIA_ERROR_OK ? TrackIt->GetTimestamp() : StreamDuration;
			}
...
void FManifestMKVInternal::FTimelineAssetMKV::GetAverageSegmentDuration(FTimeValue& OutAverageSegmentDuration, const FString& AdaptationSetID, const FString& RepresentationID)
{

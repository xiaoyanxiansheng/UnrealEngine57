...
FAdaptiveStreamingPlayer::FAdaptiveStreamingPlayer(const IAdaptiveStreamingPlayer::FCreateParam& InCreateParameters)
{
	...
	VideoOutput.OutputHandler = InCreateParameters.VideoOutputHandler;
	AudioOutput.OutputHandler = InCreateParameters.AudioOutputHandler;
...
{
	return MediaInfoDictionary.GetValue(InKey);	// thread safe
}
...
{
	PlaybackState.RequestLooping(InLoopParams);
}
...
void FAdaptiveStreamingPlayer::GetReplayEventState(FEventReplayState& OutReplayEventState) const
{
	OutReplayEventState.bIsReplayEvent = Manifest.IsValid() ? Manifest->GetReplayType() != IManifest::EReplayEventType::NoReplay : EventReplayParams.HaveReplayParams();
}
	...
	if (OutStreamBufferInfo.bIsBufferActive)
	{
		switch(InStreamType)
		{
			...
			case EStreamType::Audio:
			{
				...
				TArray<IOutputHandlerBase::FEnqueuedSampleInfo> Enqueued;
				if (InStreamType == EStreamType::Video && VideoOutput.OutputHandler.IsValid())
				{
					VideoOutput.OutputHandler->GetEnqueuedSampleInfo(Enqueued);
				}
				else if (InStreamType == EStreamType::Audio && AudioOutput.OutputHandler.IsValid())
				{
					AudioOutput.OutputHandler->GetEnqueuedSampleInfo(Enqueued);
				}
				if (Enqueued.Num())
				{
					// TBD: Do we need to break them down into separate ranges based on the sequence index?
					FTimeRange tr;
					tr.Start.SetFromTimespan(Enqueued[0].Timestamp.GetTime());
					tr.End.SetFromTimespan(Enqueued.Last().Timestamp.GetTime() + Enqueued.Last().Duration);
...
void FAdaptiveStreamingPlayer::StartRendering()
{
	...
	if (VideoOutput.OutputHandler)
	{
		VideoOutput.OutputHandler->StartOutput();
	}
	if (AudioOutput.OutputHandler)
	{
		AudioOutput.OutputHandler->StartOutput();
	}
}
...
void FAdaptiveStreamingPlayer::StopRendering()
{
	...
	if (VideoOutput.OutputHandler)
	{
		VideoOutput.OutputHandler->StopOutput();
	}
	if (AudioOutput.OutputHandler)
	{
		AudioOutput.OutputHandler->StopOutput();
	}
}
...
int32 FAdaptiveStreamingPlayer::CreateRenderers()
{
	...
	if (VideoOutput.OutputHandler)
	{
		VideoOutput.OutputHandler->SetClock(RenderClock);
	}
	if (AudioOutput.OutputHandler)
	{
		AudioOutput.OutputHandler->SetClock(RenderClock);
		AudioOutput.OutputHandler->SetPlaybackRate(CurrentRate, IntendedRate, bCurrentlyPaused);
	}
...
IPlayerABRLiveControl* FAdaptiveStreamingPlayer::GetPlayerARBControl()
{
	return this;
}
...
TSharedPtrTS<FCMCDHandler> FAdaptiveStreamingPlayer::GetCMCDHandler()
{
	return CMCDHandler;
}
...
FParamDictTS& FAdaptiveStreamingPlayer::GetMediaInfoDictionary()
{
	return MediaInfoDictionary;
}
...
void FAdaptiveStreamingPlayer::InternalHandleOnce()
{
	if (!bIsClosing)
	{
		...
		// Handle CMCD
		if (CMCDHandler.IsValid())
		{
			CMCDHandler->PeriodicHandle();
		}
	}
}
				...
				if (pRequest.IsValid() && pRequest->GetPlaybackSequenceID() == CurrentPlaybackSequenceID[StreamTypeToArrayIndex(reqType)])
				{
					...
					if (ev.Request->GetType() == EStreamType::Video)
					{
						...
						if (SegmentBitrate != CurrentVideoStreamBitrate.Bitrate)
						{
							...
							GetMediaInfoDictionary().Set(MediaInfoVideoBitrateBuffering.Resolve(), FVariantValue((int64)SegmentBitrate));
						}
					}
					...
					else if (ev.Request->GetType() == EStreamType::Audio)
					{
						...
						if (SegmentBitrate != CurrentAudioStreamBitrate.Bitrate)
						{
							...
							GetMediaInfoDictionary().Set(MediaInfoAudioBitrateBuffering.Resolve(), FVariantValue((int64)SegmentBitrate));
						}
					}
				}
				break;
...
void FAdaptiveStreamingPlayer::HandlePlayStateChanges()
{
	...
	// Handle pending loop type changes.
	InternalHandlePendingLoopRequest();
...
void FAdaptiveStreamingPlayer::InternalHandlePendingStartRequest(const FTimeValue& CurrentTime)
{
	...
	if (PendingStartRequest.IsValid())
	{
		...
		if (Manifest.IsValid())
		{
			...
			// and is also not a replay event.
			if (Manifest->GetPresentationType() != IManifest::EType::OnDemand && Manifest->GetReplayType() == IManifest::EReplayEventType::NoReplay)
			{
				...
				if (!StartAt.Time.IsValid())
				{
					...
					else
					{
						...
						*/
						...
						// If this is a looping replay event and our start time is not set then we are to join 'now'.
						if (CurrentLoopParam.bEnableLooping && Manifest->GetReplayType() != IManifest::EReplayEventType::NoReplay)
						{
							StartAt.Time = GetSynchronizedUTCTime()->GetTime();
							FTimeValue LiveLatency = Manifest->GetDesiredLiveLatency();
							if (LiveLatency.IsValid())
							{
								StartAt.Time -= LiveLatency;
							}
						}
						else
						{
			...
			if (!InitialPlayPeriod.IsValid())
			{
				...
				switch(Result.GetType())
				{
					...
					case IManifest::FResult::EType::PastEOS:
					{
						...
						// but looping is enabled we need to handle this accordingly.
						bool bEnded = !CurrentLoopParam.bEnableLooping;
						if (CurrentLoopParam.bEnableLooping)
						{
							if (Manifest->GetReplayType() == IManifest::EReplayEventType::NoReplay)
							{
								// Not a replay, start at the beginning.
								FTimeRange Seekable = Manifest->GetSeekableTimeRange();
							...
							{
								// For a looping replay we need to check if the start is beyond the event range and
								// then adjust the timeline accordingly.
								FTimeRange TotalRange = Manifest->GetTotalTimeRange();
								if (TotalRange.IsValid())
								{
									const FTimeValue RangeDuration = TotalRange.End - TotalRange.Start;
									const FTimeValue OverTime = StartAt.Time - TotalRange.Start;
									int32 NumWraps = OverTime.GetAsHNS() / RangeDuration.GetAsHNS();
									// Apply a batch of wraps only when there is more than one to avoid inaccuracies
									// that may result in slightly overshooting the range.
									if (NumWraps > 1)
									{
										Manifest->PrepareForLooping(NumWraps - 1);
									}
									// Advance single iterations (zero or one)
									while(StartAt.Time >= Manifest->GetTotalTimeRange().End)
									{
										Manifest->PrepareForLooping(1);
									}
								}
								else
								{
	...
	if (CompletedSegmentRequests.Num())
	{
		...
		if (bAllAtEOS)
		{
			...
			if (CurrentLoopParam.bEnableLooping)
			{
				if (Manifest.IsValid())
				{
					// We will be looping now. If the manifest needs internal updating, let it know to do so now.
					Manifest->PrepareForLooping(1);
...
void FAdaptiveStreamingPlayer::InternalStop(bool bHoldCurrentFrame)
{
	...
	AudioOutput.Flush();
	VideoOutput.Flush(bHoldCurrentFrame);
...
void FAdaptiveStreamingPlayer::InternalInitialize()
{
	...
	HttpResponseCache = IHTTPResponseCache::Create(GetOptionValue(OptionKeyResponseCacheMaxByteSize).SafeGetInt64(0), (int32)GetOptionValue(OptionKeyResponseCacheMaxEntries).SafeGetInt64(8192), nullptr);

	// Create the ABR stream selector.
	StreamSelector = IAdaptiveStreamSelector::Create(this);
	...
	// Create the CMCD handler.
	CMCDHandler = MakeSharedTS<FCMCDHandler>(this);
	CMCDHandler->Initialize(GetOptionValue(OptionKeyContentID).SafeGetFString(), GetOptionValue(OptionKeySessionID).SafeGetFString(), GetOptionValue(OptionKeyCMCDConfiguration).SafeGetFString(FString()));
	AddMetricsReceiver(CMCDHandler.Get());
...
void FAdaptiveStreamingPlayer::InternalClose()
{
	...
	RemoveMetricsReceiver(CMCDHandler.Get());
	CMCDHandler.Reset();
...
void FAdaptiveStreamingPlayer::InternalHandlePendingLoopRequest()
{
	TUniquePtr<FLoopParam> NewLoopParam(PlaybackState.GetPendingLoopRequest());
	if (NewLoopParam.IsValid())
	{
		...
		if (!Manifest || Manifest->GetPresentationType() == IManifest::EType::OnDemand || Manifest->GetReplayType() == IManifest::EReplayEventType::IsReplay)
		{
			bool bStartOver = false;
			// When looping is to be enabled we need to check if we can do this without having to start over.
			if (Manifest && !CurrentLoopParam.bEnableLooping && NewLoopParam->bEnableLooping)
			{
			...
			CurrentLoopParam = *NewLoopParam;
			// For the lack of better knowledge update the current state immediately.
			CurrentLoopState.bIsEnabled = NewLoopParam->bEnableLooping;
			PlaybackState.SetLoopStateEnable(NewLoopParam->bEnableLooping);
			if (bStartOver)
			{
...
void FAdaptiveStreamingPlayer::ABRGetStreamBufferStats(IPlayerABRLiveControl::FABRBufferStats& OutBufferStats, EStreamType ForStream)
{
	UpdateDiagnostics();
	DiagnosticsCriticalSection.Lock();
	switch(ForStream)
	...
	DiagnosticsCriticalSection.Unlock();
	if (ForStream == EStreamType::Video && VideoOutput.OutputHandler.IsValid())
	{
		OutBufferStats.PlayableContentDuration += FTimeValue().SetFromTimespan(VideoOutput.OutputHandler->GetEnqueuedSampleDuration());
	}
	else if (ForStream == EStreamType::Audio && AudioOutput.OutputHandler.IsValid())
	{
		OutBufferStats.PlayableContentDuration += FTimeValue().SetFromTimespan(AudioOutput.OutputHandler->GetEnqueuedSampleDuration());
	}
}
void FAdaptiveStreamingPlayer::ABRSetRenderRateScale(double InRenderRateScale)
{
	RenderRateScale = InRenderRateScale;
	if (AudioOutput.OutputHandler.IsValid())
	{
		AudioOutput.OutputHandler->SetPlayRateScale(InRenderRateScale);
	}
}
FTimeRange FAdaptiveStreamingPlayer::ABRGetSupportedRenderRateScale()
{
	...
	if (AudioOutput.OutputHandler.IsValid())
	{
		Range = AudioOutput.OutputHandler->GetSupportedRenderRateScale();
	}
	return Range;
}
...
void FAdaptiveStreamingPlayer::ABRTriggerClockSync(IPlayerABRLiveControl::EClockSyncType InClockSyncType)
{
	if (Manifest.IsValid())
	{
		Manifest->TriggerClockSync(InClockSyncType == IPlayerABRLiveControl::EClockSyncType::Required ?
...
void FAdaptiveStreamingPlayer::DebugPrint(void* pPlayer, void (*pPrintFN)(void* pPlayer, const char *pFmt, ...))
{
	...
	if(1)
	{
		...
		pPrintFN(pPlayer, "Video renderer: %#7.4fs enqueued", VideoOutput.OutputHandler.IsValid() ? VideoOutput.OutputHandler->GetEnqueuedSampleDuration().GetTotalSeconds() : 0.0);
	}
	if(1)
	{
		...
		pPrintFN(pPlayer, "Audio renderer: %#7.4fs enqueued", AudioOutput.OutputHandler.IsValid() ? AudioOutput.OutputHandler->GetEnqueuedSampleDuration().GetTotalSeconds() : 0.0);
	}
	if(1)
	{

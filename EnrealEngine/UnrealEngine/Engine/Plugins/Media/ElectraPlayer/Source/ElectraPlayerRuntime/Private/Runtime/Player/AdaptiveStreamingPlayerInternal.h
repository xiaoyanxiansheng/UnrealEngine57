...
struct FPlaybackState
{
	...
	TUniquePtr<IAdaptiveStreamingPlayer::FLoopParam>	PendingLoopParams;
	...
	void GetLoopState(IAdaptiveStreamingPlayer::FLoopState& OutLoopState) const
	{
		...
		// If there is a pending loop change request we return the requested state
		// until we have handled the request.
		if (PendingLoopParams.IsValid())
		{
			OutLoopState.bIsEnabled = PendingLoopParams->bEnableLooping;
		}
	}
	...
	void RequestLooping(const IAdaptiveStreamingPlayer::FLoopParam& InLoopParams)
	{
		FScopeLock lock(&Lock);
		PendingLoopParams.Reset(new IAdaptiveStreamingPlayer::FLoopParam(InLoopParams));
	}
	TUniquePtr<IAdaptiveStreamingPlayer::FLoopParam> GetPendingLoopRequest()
	{
		FScopeLock lock(&Lock);
		return MoveTemp(PendingLoopParams);
	}
...
public:
	...
	void GetReplayEventState(FEventReplayState& OutReplayEventState) const override;
...
private:
	...
	IPlayerABRLiveControl* GetPlayerARBControl() override;
	...
	TSharedPtrTS<FCMCDHandler> GetCMCDHandler() override;
	...
	FParamDictTS& GetMediaInfoDictionary() override;
	...
	// Methods from IPlayerABRLiveControl
	...
	void ABRGetStreamBufferStats(IPlayerABRLiveControl::FABRBufferStats& OutBufferStats, EStreamType ForStream) override;
	...
	void ABRTriggerClockSync(IPlayerABRLiveControl::EClockSyncType InClockSyncType) override;
	...
	void InternalClose();
	void InternalHandlePendingLoopRequest();
	...
	FParamDictTS														MediaInfoDictionary;
	...
	Playlist::FReplayEventParams										EventReplayParams;
	...
	TSharedPtrTS<FCMCDHandler>											CMCDHandler;
	...
	FVideoOutput														VideoOutput;
	FAudioOutput														AudioOutput;

...
/**
 * DASH-IF proposed CMCD reporting type
 */
class FDashMPD_CMCDParametersType : public IDashMPDElement
{
	typedef IDashMPDElement Super;
public:
	FDashMPD_CMCDParametersType(const TCHAR* Name, const TCHAR* Data) : Super(Name, Data)
	{
		Mode = TEXT("query");
		// Add default value. It will be removed when the @includeInRequests attribute is encountered.
		// This is valid even for the base UrlQueryInfo which _only_ applies to segments.
	...
	virtual ~FDashMPD_CMCDParametersType() = default;
	EType GetElementType() const override
	{ return EType::CMCDParameters; }
	...
	const FString& GetContentID() const					{ return ContentID; }
...
class FDashMPD_ClientDataReportingType : public IDashMPDElement
{
	typedef IDashMPDElement Super;
public:
	FDashMPD_ClientDataReportingType(const TCHAR* Name, const TCHAR* Data) : Super(Name, Data)
	{ }
	virtual ~FDashMPD_ClientDataReportingType() = default;
	EType GetElementType() const override
	{ return EType::ClientDataReporting; }

	const TArray<FString>& GetServiceLocations() const	{ return ServiceLocations; }
	const TArray<FString>& GetAdaptationSets() const	{ return AdaptationSets; }

	const TArray<TSharedPtrTS<FDashMPD_CMCDParametersType>>& GetCMCDParameters_Elements() const	{ return CMCDParameters_Elements; }
private:
	bool ProcessElement(FManifestParserDASH* Builder, const TCHAR* ElementName, const TCHAR* ElementData, int32 XmlFileLineNumber) override;
	bool ProcessAttribute(FManifestParserDASH* Builder, const TCHAR* AttributeName, const TCHAR* AttributeValue) override;
	TArray<TSharedPtrTS<FDashMPD_CMCDParametersType>> CMCDParameters_Elements;
	// Attributes
	TArray<FString> ServiceLocations;
	TArray<FString> AdaptationSets;
};
...
public:
	...
	const TArray<TSharedPtrTS<FDashMPD_ContentSteeringType>>& GetContentSteerings() const			{ return ContentSteerings; }
	const TArray<TSharedPtrTS<FDashMPD_ClientDataReportingType>>& GetClientDataReportings() const	{ return ClientDataReportings; }
...
private:
	...
	TArray<TSharedPtrTS<FDashMPD_ContentSteeringType>> ContentSteerings;
	TArray<TSharedPtrTS<FDashMPD_ClientDataReportingType>> ClientDataReportings;

		...
		if (VideoDecoder.Decoder == nullptr)
		{
			...
			if (AccessUnit)
			{
				...
				if (AccessUnit->AUCodecData.IsValid())
				{
					...
					GetMediaInfoDictionary().Set(MediaInfoVideoBitrateCurrent.Resolve(), FVariantValue((int64)VideoDecoder.CurrentCodecInfo.GetBitrate()));
				...
				if (VideoDecoder.Decoder)
				{
					...
					// Where the video decoder shall send its output to
					VideoDecoder.Decoder->SetOutputHandler(VideoOutput.OutputHandler);
		...
		if (AudioDecoder.Decoder == nullptr)
		{
			...
			if (AccessUnit)
			{
				...
				if (AccessUnit->AUCodecData.IsValid())
				{
					...
					GetMediaInfoDictionary().Set(MediaInfoAudioBitrateCurrent.Resolve(), FVariantValue((int64)AudioDecoder.CurrentCodecInfo.GetBitrate()));
				...
				if (AudioDecoder.Decoder)
				{
					...
					// Where the audio decoder shall send its output to
					AudioDecoder.Decoder->SetOutputHandler(AudioOutput.OutputHandler);
					// Open the decoder after having set all listeners.
	...
	if (!bIsDeselected)
	{
		...
		if (bHandleUnderrun && RebufferCause == ERebufferCause::None && CurrentState == EPlayerState::eState_Playing && StreamState == EStreamState::eStream_Running && PipelineState == EPipelineState::ePipeline_Running)
		{
			...
			if (!bEODSet && !bEOTSet && FromMultistreamBuffer->Num() == 0 && PushedDuration >= MinPushedDuration)
			{
				...
				if (Type == EStreamType::Video && VideoOutput.OutputHandler.IsValid())
				{
					TArray<IOutputHandlerBase::FEnqueuedSampleInfo> sampleInfos;
					VideoOutput.OutputHandler->GetEnqueuedSampleInfo(sampleInfos);
					NumEnqueuedSamples = sampleInfos.Num();
				}
				else if (Type == EStreamType::Audio && AudioOutput.OutputHandler.IsValid())
				{
					TArray<IOutputHandlerBase::FEnqueuedSampleInfo> sampleInfos;
					AudioOutput.OutputHandler->GetEnqueuedSampleInfo(sampleInfos);
					NumEnqueuedSamples = sampleInfos.Num();
				}
		...
		if (PeekedAU)
		{
			...
			else
			{
				...
				if (AccessUnit->AUCodecData.IsValid() && LastSentAUCodecData && LastSentAUCodecData->IsValid())
				{
					const bool bBitrateChanged = AccessUnit->AUCodecData->ParsedInfo.GetBitrate() != (*LastSentAUCodecData)->ParsedInfo.GetBitrate();
					// If there is any pertinent format change, emit an event.
					if (*LastSentAUCodecData != AccessUnit->AUCodecData)
					{
						bool bFormatChanged = bBitrateChanged || !AccessUnit->AUCodecData->ParsedInfo.Equals((*LastSentAUCodecData)->ParsedInfo);
						if (bFormatChanged)
						{
					...
					// Record the currently active bitrate.
					if (bBitrateChanged)
					{
						if (Type == EStreamType::Video)
						{
							GetMediaInfoDictionary().Set(MediaInfoVideoBitrateCurrent.Resolve(), FVariantValue((int64)((*LastSentAUCodecData)->ParsedInfo.GetBitrate())));
						}
						else if (Type == EStreamType::Audio)
						{
							GetMediaInfoDictionary().Set(MediaInfoAudioBitrateCurrent.Resolve(), FVariantValue((int64)((*LastSentAUCodecData)->ParsedInfo.GetBitrate())));
						}
					}
				}
...
void FAdaptiveStreamingPlayer::Deprecate_InternalInitializeDecoderLimits()
{
	...
	if (PlayerOptions.HaveKey(OptionKeyMaxVerticalResolution))
	{
		PlayerConfig.H264LimitUpto30fps.MaxResolution.Height =
		PlayerConfig.H264LimitAbove30fps.MaxResolution.Height = (int32)PlayerOptions.GetValue(OptionKeyMaxVerticalResolution).GetInt64();
	}
	// A limit in vertical resolution for streams with more than 30fps?
	if (PlayerOptions.HaveKey(OptionKeyMaxVerticalResolutionAbove30fps))
	{
		PlayerConfig.H264LimitAbove30fps.MaxResolution.Height = (int32)PlayerOptions.GetValue(OptionKeyMaxVerticalResolutionAbove30fps).GetInt64();
	}
}

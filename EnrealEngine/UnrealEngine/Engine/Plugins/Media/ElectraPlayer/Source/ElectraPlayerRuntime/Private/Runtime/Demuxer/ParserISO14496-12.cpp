...
namespace Electra
{
	...
	{
	public:
		FDataBufferReader(const TArray<uint8>& InDataBufferToReadFrom, int64 InBaseOffset=0) : DataBufferRef(InDataBufferToReadFrom), BaseOffset(InBaseOffset)
		{}
...
#define RETURN_IF_ERROR(expr)											\
	...
	public:
		...
		void PushReader(FMP4BoxReader* InNewReader)
		{
			BoxReaders.Push(BoxReader);
			BoxReader = InNewReader;
		}

		void PopReader()
		{
			BoxReader = BoxReaders.Pop();
		}
	...
	private:
		...
		TArray<FMP4BoxReader*> BoxReaders;
	...
	*  The box type is the reference type (See kTrackRef_...), along with a list of TrackIds for that relation type.
	...
	protected:
		UEMediaError ReadAndParseAttributes(FMP4ParseInfo* ParseInfo) override
		{
			...
			// The 'meta' box is a full box under ISO and a basic box in QuickTime.
			// Some tools write a QT 'meta' box even for ISO which is then missing the version number and flags!
			// To work around this issue we read the box here as a whole and locate the mandatory 'hdlr' box
			// that has to be the first box and based on its position handle this as a basic or a full box.
			TArray<uint8> BoxData;
			uint32 BytesRemaining = (uint32)(BoxSize - (ParseInfo->Reader()->GetCurrentReadOffset() - StartOffset));
			if (BytesRemaining)
			{
				BoxData.AddUninitialized(BytesRemaining);
				RETURN_IF_ERROR(ParseInfo->Reader()->ReadBytes(BoxData.GetData(), BytesRemaining));
			...
			{
				bIsFullBox = BoxData[8]=='h' && BoxData[9]=='d' && BoxData[10]=='l' && BoxData[11]=='r';
			}

			FDataBufferReader BufferReader(BoxData, StartOffset);
			FMP4BoxReader Reader(&BufferReader, nullptr);
			struct FAutoPopReader
			{
				FAutoPopReader(FMP4ParseInfo* InParseInfo, FMP4BoxReader* InReader) : PInfo(InParseInfo)
				{ PInfo->PushReader(InReader); }
				~FAutoPopReader()
				{ Detach(); }
				void Detach()
				{
			...
			FAutoPopReader BoxReader(ParseInfo, &Reader);
			if (bIsFullBox)
			{
				RETURN_IF_ERROR(FMP4BoxFull::ReadAndParseAttributes(ParseInfo));
				BytesRemaining -= 4;
			...
			{
				bIsLeafBox = false;
				UEMediaError Err = ParseInfo->ReadAndParseNextBox(this, ParseInfo->Reader()->GetCurrentReadOffset() + BytesRemaining);
				if (Err != UEMEDIA_ERROR_OK && Err != UEMEDIA_ERROR_END_OF_STREAM)
				{
					return Err;
				}
			}
	...
	UEMediaError FParserISO14496_12::ReadTMCDSample(IPlayerSessionServices* InPlayerSession, FTrack* InTrack, const FMP4Box* InSampleBox, FCancellationCheckDelegate InCheckCancellationDelegate)
	{
		...
		if (const TSharedPtrTS<FWaitableBuffer> ChunkData = ChunkLoader.LoadChunk(ChunkOffset, ChunkSize, InPlayerSession->GetHTTPManager(), InPlayerSession->GetHTTPResponseCache(), ManifestURL, CancellationCheckDelegate))
		{
			if (ChunkData.IsValid() && ChunkData->Num() >= SampleSizeInBytes)
			{
				...
				InTrack->CodecInformation.GetExtras().Set(MediaInfoStartTimecodeValue, StartTimecodeValue);
				InTrack->CodecInformation.GetExtras().Set(MediaInfoStartTimecodeFrameRate, StartTimecodeFrameRate);
				return UEMEDIA_ERROR_OK;
			}
		}
		return UEMEDIA_ERROR_NOT_SUPPORTED;
					...
					case FMP4Box::kBox_trak:
					{
						...
						switch(STSDFirstChildBox->GetType())
						{
							...
							case FMP4Box::kSample_tmcd:
							{
								...
								if (Error == UEMEDIA_ERROR_NOT_SUPPORTED) //-V547
								{
									bIsSupported = false;
								}
								else if (Error != UEMEDIA_ERROR_OK) //-V547
	...
	UEMediaError FParserISO14496_12::ResolveTimecodeTracks(IPlayerSessionServices* InPlayerSession, FCancellationCheckDelegate InCancellationCheckDelegate)
	{
				...
				if (!TimecodeTrackIds.IsEmpty())
				{
					if (const FTrack* TimecodeTrack = ParsedTrackInfo->GetTrackByID(TimecodeTrackIds[0]))
					{
						if (TimecodeTrack->StartTimecode.IsSet())
						{
							...
							TrackExtras.SetValueFrom(MediaInfoStartTimecodeValue, TimecodeTrack->CodecInformation.GetExtras());
							TrackExtras.SetValueFrom(MediaInfoStartTimecodeFrameRate, TimecodeTrack->CodecInformation.GetExtras());
						}
					}
				}
			}
		...
		for(int32 TrackIndex = 0; TrackIndex < ParsedTrackInfo->GetNumberOfTracks(); ++TrackIndex)
		{
			...
			if (Track && Track->CodecInformation.GetCodec4CC() == FMP4Box::kSample_tmcd && Track->StartTimecode.IsSet())
			{
				...
				{
					InPlayerSession->GetMediaInfoDictionary().SetValueFrom(MediaInfoStartTimecodeValue, Track->CodecInformation.GetExtras());
					InPlayerSession->GetMediaInfoDictionary().SetValueFrom(MediaInfoStartTimecodeFrameRate, Track->CodecInformation.GetExtras());
					break;
				}
			}
		}

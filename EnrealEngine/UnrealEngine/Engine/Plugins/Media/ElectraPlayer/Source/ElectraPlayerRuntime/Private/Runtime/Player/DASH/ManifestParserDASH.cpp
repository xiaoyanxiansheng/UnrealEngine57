...
DEF_ELEMENT(ClientDataReporting);
DEF_ELEMENT(CMCDParameters);
...
DEF_ATTR(serviceLocations);
DEF_ATTR(adaptationSets);
DEF_ATTR(version);
DEF_ATTR(mode);
DEF_ATTR(keys);
DEF_ATTR(contentID);
...
bool IDashMPDElement::ProcessAttribute(FManifestParserDASH* Builder, const TCHAR* AttributeName, const TCHAR* AttributeValue)
{
	...
	if (GetElementType() == EType::Descriptor || GetElementType() == EType::ContentSteering || GetElementType() == EType::ClientDataReporting || GetElementType() == EType::Other)
	{
		bEmitWarning = false;
	}
		...
		for(int32 i=0; i<Periods.Num(); ++i)
		{
			if (Periods[i] == Element)
			{
				...
				if (NewRootEntities.Periods.Num())
				{
					...
					{
						NewRootEntities.Periods[j]->SetParentElement(AsShared());
		...
		for(int32 i=0; i<InitializationSets.Num(); ++i)
		{
			if (InitializationSets[i] == Element)
			{
				...
				if (NewRootEntities.InitializationSets.Num())
				{
					...
					{
						NewRootEntities.InitializationSets[j]->SetParentElement(AsShared());
		...
		for(int32 i=0; i<WellKnownDescriptors.Num(); ++i)
		{
			if (WellKnownDescriptors[i] == Element)
			{
				...
				if (NewRootEntities.UrlQueries.Num())
				{
					...
					{
						NewRootEntities.UrlQueries[j]->SetParentElement(AsShared());
		...
		for(int32 i=0; i<AdaptationSets.Num(); ++i)
		{
			if (AdaptationSets[i] == Element)
			{
				...
				if (NewRootEntities.AdaptationSets.Num())
				{
					...
					{
						NewRootEntities.AdaptationSets[j]->SetParentElement(AsShared());
		...
		for(int32 i=0; i<InbandEventStreams.Num(); ++i)
		{
			if (InbandEventStreams[i] == Element)
			{
				...
				if (NewRootEntities.EventStreams.Num())
				{
					...
					for(int32 j=0; j<NewRootEntities.EventStreams.Num(); ++j)
					{
						NewRootEntities.EventStreams[j]->SetParentElement(AsShared());
						NewRootEntities.EventStreams[j]->GetXLink().LastResolveID = NewResolveID;
						NewRootEntities.EventStreams[j]->GetXLink().bWasRemoteEntity = true;
					}
	...
	if (Element->GetElementType() == IDashMPDElement::EType::SegmentList)
	{
		...
		if (SegmentList == Element)
		{
			...
			if (NewRootEntities.SegmentLists.Num())
			{
				...
				for(int32 j=0; j<NewRootEntities.SegmentLists.Num(); ++j)
				{
					NewRootEntities.SegmentLists[j]->SetParentElement(AsShared());
					NewRootEntities.SegmentLists[j]->GetXLink().LastResolveID = NewResolveID;
					NewRootEntities.SegmentLists[j]->GetXLink().bWasRemoteEntity = true;
				}
				SegmentList = NewRootEntities.SegmentLists[0];
...
bool FDashMPD_ClientDataReportingType::ProcessElement(FManifestParserDASH* Builder, const TCHAR* ElementName, const TCHAR* ElementData, int32 XmlFileLineNumber)
{
	if (StringHelpers::StringEquals(ElementName, ELEMENT(CMCDParameters)))
	{
		CMCDParameters_Elements.Emplace(MakeSharedTS<FDashMPD_CMCDParametersType>(ElementName, ElementData));
		return Builder->PushNewElement(CMCDParameters_Elements.Last(), XmlFileLineNumber);
	}
	else
	{
		return Super::ProcessElement(Builder, ElementName, ElementData, XmlFileLineNumber);
...
bool FDashMPD_ClientDataReportingType::ProcessAttribute(FManifestParserDASH* Builder, const TCHAR* AttributeName, const TCHAR* AttributeValue)
{
	if (AttrEquals(serviceLocations))
	{
		return ParseStringArray(ServiceLocations, TEXT(" "), Builder, AttributeName, AttributeValue, GetXMLLineNumber());
	}
	else if (AttrEquals(adaptationSets))
	{
		return ParseStringArray(AdaptationSets, TEXT(" "), Builder, AttributeName, AttributeValue, GetXMLLineNumber());
	}
	else
	{
		return Super::ProcessAttribute(Builder, AttributeName, AttributeValue);
...
bool FDashMPD_CMCDParametersType::ProcessAttribute(FManifestParserDASH* Builder, const TCHAR* AttributeName, const TCHAR* AttributeValue)
{
	if (AttrEquals(version))
	{
		return ParseIntAttribute(Version, Builder, AttributeName, AttributeValue, GetXMLLineNumber());
	}
	else if (AttrEquals(includeInRequests))
	{
		return ParseStringArray(IncludeInRequests, TEXT(" "), Builder, AttributeName, AttributeValue, GetXMLLineNumber());
	}
	else if (AttrEquals(keys))
	{
		return ParseStringArray(Keys, TEXT(" "), Builder, AttributeName, AttributeValue, GetXMLLineNumber());
	}
	else if (AttrEquals(mode))
	{
		return ParseStringAttribute(Mode, Builder, AttributeName, AttributeValue, GetXMLLineNumber());
	}
	else if (AttrEquals(contentID))
	{
		return ParseStringAttribute(ContentID, Builder, AttributeName, AttributeValue, GetXMLLineNumber());
	}
	else if (AttrEquals(sessionID))
	{
		return ParseStringAttribute(SessionID, Builder, AttributeName, AttributeValue, GetXMLLineNumber());
	}
	else if (AttrEquals(schemeIdUri))
	{
		return ParseStringAttribute(SchemeIdUri, Builder, AttributeName, AttributeValue, GetXMLLineNumber());
...
bool FDashMPD_ServiceDescriptionType::ProcessElement(FManifestParserDASH* Builder, const TCHAR* ElementName, const TCHAR* ElementData, int32 XmlFileLineNumber)
{
	...
	// Content steering and client data reportings are now provided in the service description.
	else if (StringHelpers::StringEquals(ElementName, ELEMENT(ContentSteering)))
	{
		ContentSteerings.Emplace(MakeSharedTS<FDashMPD_ContentSteeringType>(ElementName, ElementData));
		return Builder->PushNewElement(ContentSteerings.Last(), XmlFileLineNumber);
	}
	else if (StringHelpers::StringEquals(ElementName, ELEMENT(ClientDataReporting)))
	{
		ClientDataReportings.Emplace(MakeSharedTS<FDashMPD_ClientDataReportingType>(ElementName, ElementData));
		return Builder->PushNewElement(ClientDataReportings.Last(), XmlFileLineNumber);
	}
	else
	{
		return Super::ProcessElement(Builder, ElementName, ElementData, XmlFileLineNumber);

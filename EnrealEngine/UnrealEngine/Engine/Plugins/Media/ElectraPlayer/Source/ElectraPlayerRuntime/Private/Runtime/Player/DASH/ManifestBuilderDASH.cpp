...
void FManifestDASHInternal::InjectEpicTimingSources()
{
	...
	{
		if (frgs[i].Name.Equals(CustomOptions::Custom_EpicUTCUrl))
		{
			MPDRoot->AddUTCTimingElement(FString(), DASH::Schemes::TimingSources::Scheme_urn_mpeg_dash_utc_httpxsdate2014, FString(), frgs[i].Value);
			bDidInjectUTCTimingElements = true;
		}
		else if (frgs[i].Name.Equals(CustomOptions::Custom_EpicUTCNow))
		{
			MPDRoot->AddUTCTimingElement(FString(), DASH::Schemes::TimingSources::Scheme_urn_mpeg_dash_utc_direct2014, FString(), frgs[i].Value);
			bDidInjectUTCTimingElements = true;
		}
	}
}
...
void FManifestDASHInternal::TransformIntoEpicEvent()
{
	...
	if (PresentationType == EPresentationType::Static)
	{
		...
		if (Params.Num())
		{
			OriginalPresentationDuration = GetDuration();
...
void FManifestDASHInternal::PrepareEpicEventForLooping(int32 InNumLoopsToAdd)
{
	if (OriginalPresentationDuration.IsValid())
	{
		FTimeValue ast = GetAnchorTime();
		FTimeValue AddDur = OriginalPresentationDuration * InNumLoopsToAdd;
		FTimeValue Start = ast + AddDur;
		MPDRoot->SetAvailabilityStartTime(Start);
		if (EpicEventType == EEpicEventType::Dynamic)
		{
			MPDRoot->SetPublishTime(Start);
		...
		for(auto &Period : GetPeriods())
		{
			Period->StartAST += AddDur;
			Period->EndAST += AddDur;
		}
	}
}

...
#define MAX_NEXT_SEGMENTS_FOR_CMCD	3
...
IManifest::EReplayEventType FManifestDASH::GetReplayType() const
{
	TSharedPtrTS<FManifestDASHInternal> Manifest(CurrentManifest);
	if (Manifest.IsValid())
	{
		return Manifest->IsEpicEvent() ? IManifest::EReplayEventType::IsReplay : IManifest::EReplayEventType::NoReplay;
	}
...
void FManifestDASH::PrepareForLooping(int32 InNumLoopsToAdd)
{
	TSharedPtrTS<FManifestDASHInternal> Manifest = CurrentManifest;
	if (Manifest.IsValid() && Manifest->IsEpicEvent())
	{
		Manifest->PrepareEpicEventForLooping(InNumLoopsToAdd);
	}
}
	...
	for(auto& pl : InitSegmentsToPreload)
	{
		...
		if (!is.bRequested)
		{
			...
			if (!PlayerSessionServices->GetEntityCache()->GetCachedEntity(CachedItem, is.InitSegmentInfo.InitializationURL.Url.URL, is.InitSegmentInfo.InitializationURL.Range))
			{
				...
				is.LoadRequest->LoadType = FMPDLoadRequestDASH::ELoadType::InitSegment;
		...
		if (RepresentationIndexSegmentURLRange.Len() || RepresentationIndexSegmentURL.Len())
		{
			...
			else
			{
				...
				if (GetStreamContainerType() == EStreamContainerType::ISO14496_12)
				{
					...
					LoadReq->LoadType = FMPDLoadRequestDASH::ELoadType::IndexSegment;
...
bool FManifestDASHInternal::FRepresentation::PrepareDownloadURLs(IPlayerSessionServices* InPlayerSessionServices, FSegmentInformationCommon& InOutSegmentInfo, const TArray<TSharedPtrTS<FDashMPD_SegmentTemplateType>>& SegmentTemplate)
{
	...
	FString MediaTemplateURL = ApplyTemplateStrings(MediaTemplate, InOutSegmentInfo.Number, InOutSegmentInfo.Time, InOutSegmentInfo.SubIndex);
	FString InitTemplateURL = ApplyTemplateStrings(InitializationTemplate, InOutSegmentInfo.Number, InOutSegmentInfo.Time, InOutSegmentInfo.SubIndex);
	...
	// If there are future segments to be requested to be conveyed via CMCD `nor`, prepare their URLs now.
	for(int32 nf=0; nf<InOutSegmentInfo.NextMediaCMCDNumberTimes.Num(); ++nf)
	{
		FString NextMediaURL = ApplyTemplateStrings(MediaTemplate, InOutSegmentInfo.NextMediaCMCDNumberTimes[nf].Number, InOutSegmentInfo.NextMediaCMCDNumberTimes[nf].Time, 0);

		TArray<FContentSteeringHandler::FCandidateURL> NextSteeringCandidates;
		PrepareSteeringCandidates(NextSteeringCandidates, InPlayerSessionServices, RootDocument.URL, CandidateURLs, UrlQueries, NextMediaURL);
		FString DummyMessage;
		FContentSteeringHandler::FSelectedCandidateURL NextChosenMedia = InPlayerSessionServices->GetContentSteeringHandler()->SelectBestCandidateFrom(DummyMessage, FContentSteeringHandler::ESelectFor::Segment, NextSteeringCandidates);
		if (NextChosenMedia.MediaURL.URL.IsEmpty())
		{
			break;
		}
		InOutSegmentInfo.NextMediaURLS.Emplace_GetRef().Url.URL = NextChosenMedia.MediaURL.URL;
	}
...
FString FManifestDASHInternal::FRepresentation::ApplyTemplateStrings(FString TemplateURL, int64 InNumber, int64 InTime, int64 InSubIndex)
{
	...
	while(!TemplateURL.IsEmpty())
	{
		...
		else
		{
			...
			if (token2Pos != INDEX_NONE)
			{
				...
				{
					NewURL.Append(PrintWithWidth(InNumber, GetFormatWidth(token.Mid(6))));
				}
				// $Bandwidth$ ?
				...
				{
					NewURL.Append(PrintWithWidth(InTime, GetFormatWidth(token.Mid(4))));
				}
				// $SubNumber$ ?
				else if (token.StartsWith(TEXT("SubNumber")))
				{
					NewURL.Append(PrintWithWidth(InSubIndex, GetFormatWidth(token.Mid(9))));
				}
				else
				{
...
FManifestDASHInternal::FRepresentation::ESearchResult FManifestDASHInternal::FRepresentation::FindSegment_Base_MP4(IPlayerSessionServices* InPlayerSessionServices, FSegmentInformationCommon& OutSegmentInfo, TArray<TWeakPtrTS<FMPDLoadRequestDASH>>& OutRemoteElementLoadRequests, const FSegmentSearchOption& InSearchOptions, const TSharedPtrTS<FDashMPD_RepresentationType>& MPDRepresentation, const TArray<TSharedPtrTS<FDashMPD_SegmentBaseType>>& SegmentBase)
{
	...
	if (bFound && CurrentT < MediaLocalEndTime)
	{
		...
		// Look at the next n segment ranges for CMCD `nrr`
		if (!OutSegmentInfo.bIsLastInPeriod && InPlayerSessionServices->GetCMCDHandler()->IsEnabled())
		{
			int64 cbo = OutSegmentInfo.FirstByteOffset + OutSegmentInfo.NumberOfBytes;
			int64 nlt = CurrentT + CurrentD;
			for(int32 jj=CurrentN+1; jj<EndNumber && nlt<MediaLocalEndTime; ++jj)
			{
				int64 nss = Sidx->GetEntry(jj).Size;
				int64 nsd = Sidx->GetEntry(jj).SubSegmentDuration;
				int64 NextSegStart = cbo;
				int64 NextSegEnd = cbo + nss - 1;
				OutSegmentInfo.NextMediaURLS.Emplace_GetRef().Range = FString::Printf(TEXT("%lld-%lld"), (long long int)NextSegStart, (long long int)NextSegEnd);
				if (OutSegmentInfo.NextMediaURLS.Num() >= MAX_NEXT_SEGMENTS_FOR_CMCD)
				{
					break;
				}
				cbo += nss;
...
FManifestDASHInternal::FRepresentation::ESearchResult FManifestDASHInternal::FRepresentation::FindSegment_Template(IPlayerSessionServices* InPlayerSessionServices, FSegmentInformationCommon& OutSegmentInfo, TArray<TWeakPtrTS<FMPDLoadRequestDASH>>& OutRemoteElementLoadRequests, const FSegmentSearchOption& InSearchOptions, const TSharedPtrTS<FDashMPD_RepresentationType>& MPDRepresentation, const TArray<TSharedPtrTS<FDashMPD_SegmentTemplateType>>& SegmentTemplate)
{
	...
	if ((OutSegmentInfo.MediaLocalLastAUTime = MediaLocalEndTime + PTO) < 0)
	{
		OutSegmentInfo.MediaLocalLastAUTime = TNumericLimits<int64>::Max();
	}
	...
	// Look at the next n segment numbers for CMCD `nro` construction
	if (!OutSegmentInfo.bIsLastInPeriod && InPlayerSessionServices->GetCMCDHandler()->IsEnabled())
	{
		for(uint32 jj=SegmentNum+1; jj<MaxSegmentsInPeriod; ++jj)
		{
			auto& nxt = OutSegmentInfo.NextMediaCMCDNumberTimes.Emplace_GetRef();
			nxt.Number = StartNumber + jj;
			nxt.Time = PTO + EPTdelta + jj * (int64)SegmentDuration;
			if (OutSegmentInfo.NextMediaCMCDNumberTimes.Num() >= MAX_NEXT_SEGMENTS_FOR_CMCD)
			{
				break;
			}
		}
...
FManifestDASHInternal::FRepresentation::ESearchResult FManifestDASHInternal::FRepresentation::FindSegment_Timeline(IPlayerSessionServices* InPlayerSessionServices, FSegmentInformationCommon& OutSegmentInfo, TArray<TWeakPtrTS<FMPDLoadRequestDASH>>& OutRemoteElementLoadRequests, const FSegmentSearchOption& InSearchOptions, const TSharedPtrTS<FDashMPD_RepresentationType>& MPDRepresentation, const TArray<TSharedPtrTS<FDashMPD_SegmentTemplateType>>& SegmentTemplate, const TSharedPtrTS<FDashMPD_SegmentTimelineType>& SegmentTimeline)
{
	...
	bool bFound = false;
	bool bFirstContentMissing = false;
	...
	else
	{
		...
		for(nIndex=0; nIndex<Selements.Num(); ++nIndex)
		{
	...
	if (bFound && CurrentT < MediaLocalEndTime)
	{
		...
		// Look at the next n segment numbers for CMCD `nro` construction
		if (!bFirstContentMissing && !OutSegmentInfo.bIsLastInPeriod && InPlayerSessionServices->GetCMCDHandler()->IsEnabled())
		{
			bool bAllDone = false;
			while(!bAllDone)
			{
				...
				if (++nIndex >= Selements.Num())
				{
					bAllDone = true;
					break;
				}
				CurrentD = Selements[nIndex].D;
				CurrentN = Selements[nIndex].HaveN ? Selements[nIndex].N - 1 : CurrentN;		// -1 since n is incremented right at the start of the loop above
				CurrentR = (Selements[nIndex].HaveR ? Selements[nIndex].R : 0) + 1;				// +1 due to the predecrement at the start of the loop
				CurrentT = Selements[nIndex].HaveT ? Selements[nIndex].T - CurrentD : CurrentT;	// subtract one d from the new time as it is added at the start of the loop
				if (CurrentD == 0)
				{
					break;
				}
	...
	if (bSuccess)
	{
		...
		else if (GetStreamContainerType() == EStreamContainerType::Matroska)
		{
			if (LoadRequest->NumRemainingInChain)
			{
				...
				NextChainedReq->LoadType = FMPDLoadRequestDASH::ELoadType::InitSegment;

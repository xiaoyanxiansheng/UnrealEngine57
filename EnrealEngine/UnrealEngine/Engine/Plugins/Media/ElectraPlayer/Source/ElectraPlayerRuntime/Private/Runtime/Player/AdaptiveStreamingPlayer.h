...
namespace Electra
{
class FOutputHandlerVideo;
class FOutputHandlerAudio;
...
public:
	...
	struct FEventReplayState
	{
		FTimeValue ProposedJoinTime;
		bool bIsReplayEvent = false;
	};
	//! Returns if the media is set to be a replay (configured via URL fragment parameters)
	virtual void GetReplayEventState(FEventReplayState& OutReplayEventState) const = 0;

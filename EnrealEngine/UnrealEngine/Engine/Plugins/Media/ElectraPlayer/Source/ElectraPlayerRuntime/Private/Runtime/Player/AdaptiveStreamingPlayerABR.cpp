...
namespace Electra
{
	...
	{
	public:
		FAdaptiveStreamSelector(IPlayerSessionServices* PlayerSessionServices, IPlayerABRLiveControl* InPlayerLiveControl);
	...
	private:
		...
		IPlayerABRLiveControl*								PlayerLiveControl;
	 ...
	 * @return
	 */
	TSharedPtrTS<IAdaptiveStreamSelector> IAdaptiveStreamSelector::Create(IPlayerSessionServices* InPlayerSessionServices)
	{
		return TSharedPtrTS<IAdaptiveStreamSelector>(new FAdaptiveStreamSelector(InPlayerSessionServices, InPlayerSessionServices->GetPlayerARBControl()));
	}
	 ...
	 */
	FAdaptiveStreamSelector::FAdaptiveStreamSelector(IPlayerSessionServices* InPlayerSessionServices, IPlayerABRLiveControl* InPlayerLiveControl)
				...
				if (!CurrentVideoAdaptationSetID.IsEmpty())
				{
					for(int32 nA=0, maxA=Asset->GetNumberOfAdaptationSets(EStreamType::Video); nA<maxA; ++nA)
					{
						...
						if (CurrentVideoAdaptationSetID.Equals(Adapt->GetUniqueIdentifier()))
						{
							...
							// Set the lowest and highest bitrate with the CMCD handler
							PlayerSessionServices->GetCMCDHandler()->SetPlayableBitrateRange(EStreamType::Video, StreamInformationVideo.Num() ? StreamInformationVideo[0]->Bitrate : 0, StreamInformationVideo.Num() ? StreamInformationVideo.Last()->Bitrate : 0);
							break;
						}
					}
				}
				...
				if (!CurrentAudioAdaptationSetID.IsEmpty())
				{
					for(int32 nA=0, maxA=Asset->GetNumberOfAdaptationSets(EStreamType::Audio); nA<maxA; ++nA)
					{
						...
						if (CurrentAudioAdaptationSetID.Equals(Adapt->GetUniqueIdentifier()))
						{
							...
							// Set the lowest and highest bitrate with the CMCD handler
							PlayerSessionServices->GetCMCDHandler()->SetPlayableBitrateRange(EStreamType::Audio, StreamInformationAudio.Num() ? StreamInformationAudio[0]->Bitrate : 0, StreamInformationAudio.Num() ? StreamInformationAudio.Last()->Bitrate : 0);
							break;
						}
					}
				}

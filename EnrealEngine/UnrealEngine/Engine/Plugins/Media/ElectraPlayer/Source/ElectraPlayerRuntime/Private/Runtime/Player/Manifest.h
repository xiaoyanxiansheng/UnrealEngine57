...
namespace Electra
{
	...
	public:
		...
		//! Returns the type of replay event.
		virtual EReplayEventType GetReplayType() const = 0;
		...
		/**
		 * Prepares the manifest for looping. This does not normally do anything except for when this is a replay,
		 * in which case the internal timestamps may be modified such that the entire presentation is shifted into
		 * the future by one presentation duration.
		 */
		virtual void PrepareForLooping(int32 InNumLoopsToAdd) = 0;

...
private:
	...
	{
		IPlayerABRLiveControl::FABRBufferStats bs;
...
void FABRLiveStream::PrepareLatencyConfiguration()
{
	...
	// The target latency should be at least half the maximum permitted latency.
	// When the maximum latency is reached a jump to the Live edge may be performed, so
	// some wiggle room is needed to avoid the target to be close to the maximum, especially
	// if the media segment duration is quite large and starting playback may put us further
	// back and possibly over the permitted maximum.
	if (TargetLatency * 2.0 > LatencyConfig.MaxLatency)
	{
		LatencyConfig.MaxLatency = TargetLatency * 2.0;
	}
	if (lld.IsValid())
	{
		...
		if (CurrentStreamInfo.IsValid())
		{
			...
			if (!Stats.bWasSuccessful)
			{
				...
				if (Stats.AvailibilityDelay && Utils::AbsoluteValue(Stats.AvailibilityDelay) < 0.5)
				{
					...
					if (WorkVars->bIsLowLatencyEnabled)
					{
						Info->ABRTriggerClockSync(IPlayerABRLiveControl::EClockSyncType::Required);
					}
					else
					{

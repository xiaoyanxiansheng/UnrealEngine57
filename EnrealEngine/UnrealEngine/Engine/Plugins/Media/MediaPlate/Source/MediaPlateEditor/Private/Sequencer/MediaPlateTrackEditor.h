...
private:
	...
	/**
	 * Information on the last dialog response.
	 */
	struct FDialogResponseInfo
	{
		/** Keep track of the context id for the response to all during multiple operations. */
		FGuid ContextId;
		/** True if the response applies to all in multiple operations under the same context id. */
		bool bAll = false;
		/** True if the operation should be done. */
		bool bApply = false;
	};

	FDialogResponseInfo LastAutoPlayDisableResponse;
	FDialogResponseInfo LastAudioDisableResponse;
};

...
void FMediaPlateTrackEditor::HandleAddMediaTrackToObjectBindingMenuEntryExecute(TArray<FGuid> InObjectBindingIDs)
{
	...
	for (FGuid InObjectBindingID : InObjectBindingIDs)
	{
		...
		{
			// Add media track.			
			if (UMovieSceneMediaTrack* NewObjectTrack = FocusedMovieScene->AddTrack<UMovieSceneMediaTrack>(InObjectBindingID))
			{
...
void FMediaPlateTrackEditor::AddTrackForComponent(UMediaPlateComponent* Component, FGuid TargetObjectGuid)
{
	...
	// Helper function to implement the "apply to all" logic and response tracking over multiple operations.
	auto AskForDisable = [](const FText& InMessage, FDialogResponseInfo& InResponseInfo)
	{
		// We are using the current transaction id to establish a context for our response info.
		const FGuid CurrentContextId = GUndo ? GUndo->GetContext().TransactionId : FGuid();

		// See if we need to ask again or can reuse previous answer.	
		if (!InResponseInfo.ContextId.IsValid() || InResponseInfo.ContextId != CurrentContextId || !InResponseInfo.bAll)
		{
			const EAppReturnType::Type Reply = FMessageDialog::Open(EAppMsgType::YesNoYesAllNoAll, InMessage);
			InResponseInfo.ContextId = CurrentContextId;
			InResponseInfo.bAll = Reply == EAppReturnType::YesAll || Reply == EAppReturnType::NoAll;
			InResponseInfo.bApply = Reply == EAppReturnType::Yes || Reply == EAppReturnType::YesAll;
		}
		return InResponseInfo.bApply;
	};
	...
	if (Component->bAutoPlay)
	{
		...
		if (AskForDisable(LOCTEXT("Prompt_DisableAutoplay", "Would you like to disable autoplay on the MediaPlate actor (recommended)?"), LastAutoPlayDisableResponse))
		{
			Component->Modify();
	...
	{
		// Ask the user if it can be disabled.
		if (AskForDisable(LOCTEXT("Prompt_DisableAudio", "Would you like to disable audio on the MediaPlate actor (recommended)?"), LastAudioDisableResponse))
		{
			Component->Modify();

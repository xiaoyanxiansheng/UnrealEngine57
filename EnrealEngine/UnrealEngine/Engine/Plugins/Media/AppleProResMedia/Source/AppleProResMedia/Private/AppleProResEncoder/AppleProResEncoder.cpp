...
{
	// Manually loading the toolbox library, on demand only.
	if (!UE::AppleProResMedia::LoadProResToolbox())
	{
	...
	ON_SCOPE_EXIT
	{
		// Clear any allocated resources if initialization failed.
		if (!bInitialized)
		{
			ReleaseAndClearResources();
		}
	};
...
void FAppleProResEncoder::ReleaseAndClearResources()
{
...
bool FAppleProResEncoder::WriteAudioSample(const TArrayView<int16>& InAudioSamples)
{
	...
	if (!ensureMsgf(Options.AudioChannelCount != 0, TEXT("AudioChannelCount can't be zero.")))
	{
		return false;
	}

	if (!ensureMsgf(AudioSamplesPerFrame != 0, TEXT("AudioSamplesPerFrame can't be zero.")))
	{
		return false;
	}

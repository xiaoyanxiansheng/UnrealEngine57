...
struct FImgMediaFrameInfo
{
	...
	/** Frame rate. */
	/*
		Note that the frame rate here is not necessarily correct if the sequence uses a frame rate override.
		The override value is not always accessible from where this member gets set, and it might also not be
		all that useful since it is conceivable that the images end up in the image cache and are either
		referenced by different media sources with different frame rate overrides, or the frame rate override
		is changed after the images have been added to the cache.
	*/

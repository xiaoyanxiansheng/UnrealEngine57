...
public:
	 ...
	 */
	UE_API void AddFrame(const FString& FileName, const FName& Sequence, int32 Index, const TSharedPtr<FImgMediaFrame, ESPMode::ThreadSafe>& Frame, bool bAdjustSizeForMipMaps = false);
	 ...
	 */
	UE_API bool Contains(const FName& Sequence, int32 Index);
	 ...
	 * @param	Index			Index of frame to touch.
	 */
	UE_API TSharedPtr<FImgMediaFrame, ESPMode::ThreadSafe>* FindAndTouch(const FName& Sequence, int32 Index);
	 ...
	 */
	UE_API void GetIndices(const FName& Sequence, TArray<int32>& OutIndices) const;

	/**
	 * Empties the cache.
	 */
	UE_API void EmptyCache();
...
private:
	 ...
	 */
	UE_API void EnforceMaxSize(SIZE_T Extra);
	 ...
	 */
	UE_API void Remove(const FName& Sequence, FImgMediaGlobalCacheEntry& Entry);
	 ...
	 */
	UE_API void MarkAsRecent(const FName& Sequence, FImgMediaGlobalCacheEntry& Entry);
	 ...
	 * @param	Entry			Entry to remove.
	 */
	UE_API void Unlink(const FName& Sequence, FImgMediaGlobalCacheEntry& Entry);
	 ...
	 */
	UE_API void UpdateSettings(const UImgMediaSettings* Settings);
	 ...
	 */
	UE_API void OnDirectoryChanged(const TArray<FFileChangeData>& InFileChanges);

...
static TAutoConsoleVariable<int32> CVarImgMediaMaxFrameSequenceGap(
	TEXT("ImgMedia.MaxFrameSequenceGap"),
	10000,
	TEXT("Maximum number of frames that will be filled by empty frames in a sequence when there is a gap in the frame indices of the filenames."));
	...
	if (IsInitialized() && InTimeRange.HasLowerBound() && InTimeRange.HasUpperBound())
	{
		...
		if (Frame)
		{
			...
			*/
			...
			QueuedSampleFetch.LastDuration = FrameLength;
		...
		else
		{
			...
			// Could we have lost a frame that we previously loaded due to the global cache being full?
			if (UseGlobalCache)
			{
				HandleGlobalCacheFrameEviction();
			}
		}
...
void FImgMediaLoader::HandleGlobalCacheFrameEviction()
{
...
bool FImgMediaLoader::LoadSequence(const FString& SequencePath, const FFrameRate& FrameRateOverride, bool Loop, FImgMediaFrameInfo& FirstFrameInfo)
{
	...
	{
		FirstFrameInfo.FrameRate = FrameRateOverride;
...
void FImgMediaLoader::FindFiles(const FString& SequencePath, TArray<FString>& OutputPaths)
{
	...
	int32 LastIndex = -1;
	const int32 MaxSequenceGap = CVarImgMediaMaxFrameSequenceGap.GetValueOnAnyThread();
	for (const auto& Pair : SortedFoundFiles)
	{
		...
		if (bFillGapsInSequence)
		{
			...
			{
				// Protect against large gaps that could lead to out of memory allocation.
				const int32 SequenceGap = (ThisIndex - 1) - LastIndex;
				if (SequenceGap < MaxSequenceGap)
				{
					for (int32 Index = 0; Index < SequenceGap; ++Index)
					{
				...
				{
					UE_LOG(LogImgMedia, Warning,
						TEXT("Loader %p, sequence \"%s\": Ignoring large frame gap (%d frames) from frame %d to %d.")
						TEXT(" If this is expected, adjust cvar \"ImgMedia.MaxFrameSequenceGap\" (current: %d) accordingly."),
						this, *SequencePath, SequenceGap, LastIndex, ThisIndex, MaxSequenceGap);
				}
			}

...
void FAvfMediaCapturePlayer::ProcessSampleBufferVideo(CMSampleBufferRef SampleBuffer)
{
	...
	if (PixelBuffer)
	{
		...
		ENQUEUE_RENDER_COMMAND(FAvfMediaCapturePlayer_ProcessSampleBufferVideo_CreateTexture)(
            [WeakPlayer = this->AsWeak(), Dim, SampleDuration, TextureRef](FRHICommandListImmediate& RHICmdList)
            {
            	if (const TSharedPtr<FAvfMediaCapturePlayer> Player = WeakPlayer.Pin())
            	{
					IMetalDynamicRHI* MetalDynamicRHI = GetIMetalDynamicRHI();
										
					TRefCountPtr<FRHITexture> ShaderResource = MetalDynamicRHI->RHICreateTexture2DFromCVMetalTexture(PF_B8G8R8A8, ETextureCreateFlags::NoTiling | ETextureCreateFlags::ShaderResource, FClearValueBinding::White, TextureRef);

...
TFuture<void> UAjaMediaCapture::StopCapture_Future()
{
	...
	if (OutputChannel)
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(UAjaMediaCapture::StopCapture_Future);

		TPromise<void> CompletionPromise;
		Future = CompletionPromise.GetFuture();

		if (TexturesToRelease.Num())
		{
			ENQUEUE_RENDER_COMMAND(AjaMediaCaptureInitialize)(
			[this, TexturesToUnregister = TexturesToRelease](FRHICommandListImmediate& RHICmdList) mutable
			{
				// Unregistering them from the game thread would cause a crash in the GPUDirect SDK.
				for (FTextureRHIRef& Texture : TexturesToUnregister)
				{
		...
		OutputChannel->Uninitialize(MoveTemp(CompletionPromise));
	}

	if (!Future.IsValid())
	{
		Future = MakeFulfilledPromise<void>().GetFuture();
	}
...
bool UAjaMediaCapture::ShouldCaptureThisFrame(const FCaptureBaseData& InBaseData) const
{
	if (OutputChannel)
	{
		return OutputChannel->ShouldCaptureThisFrame(InBaseData.SourceFrameTimecode, InBaseData.SourceFrameNumber);
	}
	return false;
}
...
bool UAjaMediaCapture::InitAJA(UAjaMediaOutput* InAjaMediaOutput)
{
	...
	ChannelOptions.bOutputInterlaceOnEvenFrames = InAjaMediaOutput->bOutputInterlaceOnEvenFrames;
	...
	ChannelOptions.bStopOnTimeout = InAjaMediaOutput->bStopOutputOnCardTimeout;
...
void UAjaMediaCapture::PostCaptureCleanupImpl()
{
	OutputChannel.Reset();
	OutputCallback.Reset();

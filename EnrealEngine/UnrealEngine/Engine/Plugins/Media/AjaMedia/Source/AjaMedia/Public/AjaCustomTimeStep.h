...
public:
	...
	/**
	 * If this is disabled, genlock will attempt to recover if the SDK timeouts while waiting for an interrupt signal from the card. It will remain in the synchronizing state until the card receives a signal.
	 * Note: In editor outside of PIE and -game, the CustomTimeStep will try to restart itself automatically regardless of this setting.
	 */
	UPROPERTY(EditAnywhere, Category = "Genlock")
	bool bStopOnCardTimeout = true;

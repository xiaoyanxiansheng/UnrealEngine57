...
static void ConditionalInitializeVideoTexture_RenderThread(FRHICommandListBase& RHICmdList, TWeakPtr<FJavaAndroidMediaPlayer, ESPMode::ThreadSafe> JavaMediaPlayerPtr, FGuid PlayerGuid)
{
	...
	if (VideoTexture == nullptr)
	{
		...
		if (VideoTexture == nullptr)
		{
			UE_LOG(LogAndroidMedia, Warning, TEXT("CreateTexture failed!"));
			return;
		}
...
static void ConditionalInitializeVideoSample_RenderThread(FRHICommandListBase& RHICmdList, TSharedRef<FAndroidMediaTextureSample, ESPMode::ThreadSafe> VideoSample)
{
...
static void DoUpdateTextureMediaSampleExecute(FRHICommandListBase& RHICmdList, TWeakPtr<FJavaAndroidMediaPlayer, ESPMode::ThreadSafe> JavaMediaPlayerPtr, TWeakPtr<FMediaSamples, ESPMode::ThreadSafe> SamplesPtr,
	TSharedRef<FAndroidMediaTextureSample, ESPMode::ThreadSafe> VideoSample)
{
	...
	FRHITexture* Texture = VideoSample->InitializeTexture(RHICmdList, Time);
...
struct FRHICommandUpdateTextureMediaSample final : public FRHICommand<FRHICommandUpdateTextureMediaSample>
{
	...
	void Execute(FRHICommandListBase& RHICmdList)
	{
		...
		DoUpdateTextureMediaSampleExecute(RHICmdList, JavaMediaPlayerPtr, SamplesPtr, VideoSample);
	}
};
	...
	else if (GSupportsImageExternal)
	{
		...
		if (!JavaMediaPlayer->IsUsingBitmapRendererImageReader())
		{
			ENQUEUE_RENDER_COMMAND(AndroidMediaPlayerWriteVideoSample)(
				[Params](FRHICommandListImmediate& RHICmdList)
				{
					ConditionalInitializeVideoTexture_RenderThread(RHICmdList, Params.JavaMediaPlayerPtr, Params.PlayerGuid);
		...
		else
		{
			ENQUEUE_RENDER_COMMAND(AndroidMediaPlayerWriteVideoSample)(
				[Params](FRHICommandListImmediate& RHICmdList)
				{
					if (IsRunningRHIInSeparateThread())
					{
						new (RHICmdList.AllocCommand<FRHICommandUpdateMediaFrameDataMediaSample>()) FRHICommandUpdateMediaFrameDataMediaSample(Params.JavaMediaPlayerPtr, Params.Samples, Params.VideoSample);
						// wait for DoUpdateExternalMediaSampleExecute to complete.
						RHICmdList.ImmediateFlush(EImmediateFlushType::FlushRHIThread);
					}
					else
	...
	else
	{
		...
		ENQUEUE_RENDER_COMMAND(AndroidMediaPlayerWriteVideoSample)(
			[Params](FRHICommandListImmediate& RHICmdList)
			{
				...
				{
					DoUpdateTextureMediaSampleExecute(RHICmdList, Params.JavaMediaPlayerPtr, Params.SamplesPtr, Params.VideoSample);
				}
			});
	}
	...
	else
	{
		ENQUEUE_RENDER_COMMAND(AndroidMediaPlayerWriteVideoSample)(
			[Params](FRHICommandListImmediate& RHICmdList)
			{
				if (IsRunningRHIInSeparateThread())
				{
					new (RHICmdList.AllocCommand<FRHICommandUpdateMediaFrameDataMediaSample>()) FRHICommandUpdateMediaFrameDataMediaSample(Params.JavaMediaPlayerPtr, Params.SamplesPtr, Params.VideoSample);
				}
				else
				{
					DoUpdateMediaFrameDataMediaSampleExecute(Params.JavaMediaPlayerPtr, Params.SamplesPtr, Params.VideoSample);

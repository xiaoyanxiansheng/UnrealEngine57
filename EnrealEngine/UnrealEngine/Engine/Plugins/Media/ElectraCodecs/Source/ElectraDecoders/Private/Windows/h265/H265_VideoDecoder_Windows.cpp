...
public:
	...
	{
		return IElectraVideoDecoderH265_DX::Create(InOptions);
	}
...
IElectraDecoder::FError IElectraVideoDecoderH265_DX::PlatformCreateMFDecoderTransform(IPlatformHandle** OutPlatformHandle, const TMap<FString, FVariant>& InOptions)
{
	IElectraDecoder::FError Error;
	check(OutPlatformHandle);
	...
	if (OutPlatformHandle)
	{
		...
		if (FPlatformMisc::VerifyWindowsVersion(6, 2))
		{
			...
			if (IsDX11Aware)
			{
				...
				if (FElectraDecodersPlatformResources::GetD3DDeviceAndVersion(&ApplicationD3DDevice, &ApplicationD3DDeviceVersion))
				{

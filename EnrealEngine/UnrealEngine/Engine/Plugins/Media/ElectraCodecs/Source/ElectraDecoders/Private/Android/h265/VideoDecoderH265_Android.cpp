...
{
public:
	FElectraVideoDecoderH265_Android(const TMap<FString, FVariant>& InOptions);
...
private:
	class FSurfaceRequestCallback : public FElectraDecodersPlatformResources::IDecoderResourceInstance::ISurfaceRequestCallback
	{
	public:
		...
		{ return Surface; }
	private:
		void OnNewSurface(ESurfaceType InSurfaceType, jobject InSurface)
		{
	...
	FElectraDecodersPlatformResources::IDecoderResourceInstance* ResourceInstance = nullptr;
...
TSharedPtr<IElectraDecoder, ESPMode::ThreadSafe> IElectraVideoDecoderH265_Android::Create(const TMap<FString, FVariant>& InOptions)
{
	TSharedPtr<FElectraVideoDecoderH265_Android, ESPMode::ThreadSafe> New = MakeShared<FElectraVideoDecoderH265_Android>(InOptions);
	return New;
}


FElectraVideoDecoderH265_Android::FElectraVideoDecoderH265_Android(const TMap<FString, FVariant>& InOptions)
	: InitialCreationOptions(InOptions)
{
	ResourceInstance = FElectraDecodersPlatformResources::CreateResourceInstance(ElectraDecodersUtil::GetVariantValueSafeI64(InOptions, FElectraDecodersPlatformResources::OptionKeyOutputTexturePoolID().ToString(), 0));
...
FElectraVideoDecoderH265_Android::~FElectraVideoDecoderH265_Android()
{
	...
	FElectraDecodersPlatformResources::ReleaseResourceInstance(ResourceInstance);
}
...
IElectraDecoder::EDecoderError FElectraVideoDecoderH265_Android::DecodeAccessUnit(const FInputAccessUnit& InInputAccessUnit, const TMap<FString, FVariant>& InAdditionalOptions)
{
	...
	if (DecoderCreatedAtResumeCount != CurrentResumeCount)
	{
		bTriggerReplay = true;
		CurrentOutput.Reset();
		InDecoderInput.Empty();
...
bool FElectraVideoDecoderH265_Android::InternalHandleDecoderCreate()
{
	...
	else
	{
		...
		ResourceInstance->RequestSurface(CurrentSurfaceRequest);
		return true;
	}
	return true;
}
...
FElectraVideoDecoderH265_Android::EConvertResult FElectraVideoDecoderH265_Android::ConvertDecoderOutput(const IElectraH265VideoDecoderAndroidJava::FOutputBufferInfo& InInfo)
{
	...
	if (DecoderInstance.IsValid())
	{
		int32 Result = DecoderInstance->ReleaseOutputBuffer(InInfo.BufferIndex, InInfo.ValidCount, true, -1);
		if (Result)
		{

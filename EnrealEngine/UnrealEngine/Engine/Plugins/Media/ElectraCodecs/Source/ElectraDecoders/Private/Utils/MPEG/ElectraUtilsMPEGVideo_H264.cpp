...
namespace ElectraDecodersUtil
{
	namespace MPEG
	{
		namespace H264
		{
			...
			bool ParseScalingLists(const FSequenceParameterSet* InSPS, const FPictureParameterSet* InPPS, uint8 scaling_list_present_flag[12], uint8 ScalingList4x4[6][16], uint8 ScalingList8x8[6][64], FBitstreamReaderH264& br)
			{
				struct sl
				{
					static bool scaling_list(uint8* ScalingList, int32 sizeOfScalingList, const uint8* DefaultScalingMatrix, FBitstreamReaderH264& br)
					{
					...
					static bool parse_scaling_list(uint8* ScalingList, int32 sizeOfScalingList, const uint8* DefaultScalingMatrix, const uint8* FallbackScalingMatrix, FBitstreamReaderH264& br)
					{
						if (br.GetBits(1))
						{
			...
			bool ParseSequenceParameterSet(TMap<uint32, FSequenceParameterSet>& InOutSequenceParameterSets, const uint8* InBitstream, uint64 InBitstreamLenInBytes)
			{
				...
				FBitstreamReaderH264 br(RBSP->Data.GetData(), RBSP->Data.Num());
			...
			bool ParseSliceHeader(TUniquePtr<FRBSP>& OutRBSP, FBitstreamReaderH264& OutRBSPReader, FSliceHeader& OutSlice, const TMap<uint32, FSequenceParameterSet>& InSequenceParameterSets, const TMap<uint32, FPictureParameterSet>& InPictureParameterSets, const uint8* InBitstream, uint64 InBitstreamLenInBytes)
			{
				#define RANGE_CHECK_FAILURE(expr, elem)															\
					if (!(expr))																				\
					{																							\
				...
				FBitstreamReaderH264& br(OutRBSPReader);
			...
			bool FAVCDecoderConfigurationRecord::Parse(const TConstArrayView<uint8>& InDCR)
			{
				...
				RawData = InDCR;
				Electra::FElectraByteReader ByteReader(InDCR.GetData(), InDCR.Num());

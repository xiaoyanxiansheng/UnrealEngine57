...
public:
	...
	void* GetPlatformOutputHandle(EElectraDecoderPlatformOutputHandleType InTypeOfHandle) const override
	{
		switch(InTypeOfHandle)
		{
			...
			{
				return !MFSample.IsValid() ? static_cast<IElectraDecoderVideoOutputImageBuffers*>(const_cast<FElectraVideoDecoderOutputH264_DX*>(this)) : nullptr;
			}
#endif
...
{
public:
	FElectraVideoDecoderH264_DX(const TMap<FString, FVariant>& InOptions);
...
TSharedPtr<IElectraDecoder, ESPMode::ThreadSafe> IElectraVideoDecoderH264_DX::Create(const TMap<FString, FVariant>& InOptions)
{
	TSharedPtr<FElectraVideoDecoderH264_DX, ESPMode::ThreadSafe> New = MakeShared<FElectraVideoDecoderH264_DX>(InOptions);
	return New;
}


FElectraVideoDecoderH264_DX::FElectraVideoDecoderH264_DX(const TMap<FString, FVariant>& InOptions)
...
void FElectraVideoDecoderH264_DX::CopyBufferData(TRefCountPtr<IMF2DBuffer> Buffer2D, TRefCountPtr<ID3D12Resource> BufferResource, TRefCountPtr<ID3D12Fence> BufferFence, uint64 BufferFenceValue, uint32 BufferPitch, int32 DecodedHeight)
{
	...
	if (FAILED(Res))
	{
		D3D12RHI->RHIVerifyResult(Device, Res, "FElectraVideoDecoderH264_DX::CopyBufferData Map", __FILE__, __LINE__);
	}
	...
	if (FAILED(Res))
	{
		D3D12RHI->RHIVerifyResult(Device, Res, "FElectraVideoDecoderH264_DX::CopyBufferData Signal", __FILE__, __LINE__);
	}
};
#endif
...
bool FElectraVideoDecoderH264_DX::ConvertDecoderOutput()
{
	...
	*/
	...
	NewOutput->ExtraValues.Emplace(TEXT("pixfmt"), FVariant((int64)EPixelFormat::PF_NV12));
	...
	if (CVarElectraWindowsH264UseOldOutputPath.GetValueOnAnyThread() == 0)
	{

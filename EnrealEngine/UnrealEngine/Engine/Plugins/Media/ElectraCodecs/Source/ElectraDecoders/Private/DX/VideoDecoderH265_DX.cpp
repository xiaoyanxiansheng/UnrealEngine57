...
public:
	...
	void* GetPlatformOutputHandle(EElectraDecoderPlatformOutputHandleType InTypeOfHandle) const override
	{
		switch (InTypeOfHandle)
		{
			...
			{
				return !MFSample.IsValid() ? static_cast<IElectraDecoderVideoOutputImageBuffers*>(const_cast<FElectraVideoDecoderOutputH265_DX*>(this)) : nullptr;
			}
#endif
	...
	EPixelFormat GetBufferFormatByIndex(int32 InBufferIndex) const override
	{
		if (InBufferIndex == 0)
		{
			return (EPixelFormat)ElectraDecodersUtil::GetVariantValueSafeU64(ExtraValues, TEXT("pixfmt"), (uint64)EPixelFormat::PF_Unknown);
		}
	...
	EElectraTextureSamplePixelEncoding GetBufferEncodingByIndex(int32 InBufferIndex) const override
	{
		if (InBufferIndex == 0)
		{
			return (EElectraTextureSamplePixelEncoding)ElectraDecodersUtil::GetVariantValueSafeU64(ExtraValues, TEXT("pixenc"), (uint64)EElectraTextureSamplePixelEncoding::Native);
		}
...
public:
	...
	TSharedPtr<FElectraDecodersPlatformResources::FAsyncConsecutiveTaskSync, ESPMode::ThreadSafe> Buffer_TaskSync;
#endif
};
...
{
public:
	FElectraVideoDecoderH265_DX(const TMap<FString, FVariant>& InOptions);
...
private:
	...
	mutable TSharedPtr<FElectraDecodersPlatformResources::FAsyncConsecutiveTaskSync, ESPMode::ThreadSafe> TaskSync;
#endif
...
TSharedPtr<IElectraDecoder, ESPMode::ThreadSafe> IElectraVideoDecoderH265_DX::Create(const TMap<FString, FVariant>& InOptions)
{
	TSharedPtr<FElectraVideoDecoderH265_DX, ESPMode::ThreadSafe> New = MakeShared<FElectraVideoDecoderH265_DX>(InOptions);
	return New;
}


FElectraVideoDecoderH265_DX::FElectraVideoDecoderH265_DX(const TMap<FString, FVariant>& InOptions)
...
bool FElectraVideoDecoderH265_DX::InternalDecoderCreate(const TMap<FString, FVariant>& InAdditionalOptions)
{
	...
	LastError = PlatformCreateMFDecoderTransform(&DecoderPlatformHandle, NoOptions);
	if (LastError.IsSet())
	{
		return false;
	}
...
void FElectraVideoDecoderH265_DX::CopyBufferData(TRefCountPtr<IMF2DBuffer> Buffer2D, TRefCountPtr<ID3D12Resource> BufferResource, TRefCountPtr<ID3D12Fence> BufferFence, uint64 BufferFenceValue, uint32 BufferPitch, int32 DecodedHeight)
{
	...
	ID3D12DynamicRHI* const D3D12RHI = GetID3D12DynamicRHI();
	ID3D12Device* const Device = static_cast<ID3D12Device*>(D3D12RHI->RHIGetNativeDevice());

	void* BufferAddr;
	Res = BufferResource->Map(0, nullptr, &BufferAddr);
	if (FAILED(Res))
	{
		D3D12RHI->RHIVerifyResult(Device, Res, "FElectraVideoDecoderH265_DX::CopyBufferData Map", __FILE__, __LINE__);
	}
	...
	Res = BufferFence->Signal(BufferFenceValue);
	if (FAILED(Res))
	{
		D3D12RHI->RHIVerifyResult(Device, Res, "FElectraVideoDecoderH265_DX::CopyBufferData Signal", __FILE__, __LINE__);
	}
};
#endif
...
bool FElectraVideoDecoderH265_DX::ConvertDecoderOutput()
{
	...
	*/
	...
	if (CurrentVideoFormatGUID == MFVideoFormat_NV12)
	{
		...
		PixFmt = EPixelFormat::PF_NV12;
	}
	else
	{
		NewOutput->NumBits = 10;
		NewOutput->PixelFormat = 1;
		PixFmt = EPixelFormat::PF_P010;
	}
	...
	if (CVarElectraWindowsH265UseOldOutputPath.GetValueOnAnyThread() == 0)
	{
	...
	if (bUseGPUBuffers)
	{
		...
		{
			PostError(Result, TEXT("Failed to get buffer count"), ERRCODE_INTERNAL_FAILED_TO_CONVERT_OUTPUT_SAMPLE);
			return false;
		}
		if (BuffersNum != 1)
		{
			PostError(0, TEXT("Invalid buffer count"), ERRCODE_INTERNAL_FAILED_TO_CONVERT_OUTPUT_SAMPLE);
			return false;
		}
		TRefCountPtr<IMFMediaBuffer> Buffer;
		if ((Result = DecodedOutputSample->GetBufferByIndex(0, Buffer.GetInitReference())) != S_OK)
		{
			PostError(Result, TEXT("Failed to get buffer by index"), ERRCODE_INTERNAL_FAILED_TO_CONVERT_OUTPUT_SAMPLE);
			return false;
		}
		...
		if (!D3D12ResourcePool || !D3D12ResourcePool->IsCompatibleAsBuffer(MaxOutputBuffers, MaxWidth, MaxHeight * 3 / 2, (PixFmt == EPixelFormat::PF_NV12) ? 1 : 2))
		{
			D3D12ResourcePool = MakeShared<FElectraMediaDecoderOutputBufferPool_DX12, ESPMode::ThreadSafe>(D3D12Device, MaxOutputBuffers, MaxWidth, MaxHeight * 3 / 2, (PixFmt == EPixelFormat::PF_NV12) ? 1 : 2);
		}

		// Create a tasksync instance so we can have our own async jobs run in consecutive order
		if (!TaskSync.IsValid())
		{
			TaskSync = FElectraDecodersPlatformResources::CreateAsyncConsecutiveTaskSync();
		}
		...
		{
			PostError(Result, TEXT("Failed to get 2d buffer from buffer"), ERRCODE_INTERNAL_FAILED_TO_CONVERT_OUTPUT_SAMPLE);
			return false;
		}

		// Request resource and fence...
		uint32 BufferPitch;
		if (!D3D12ResourcePool->AllocateOutputDataAsBuffer(Result, ResultMsg, NewOutput->Buffer, BufferPitch))
		{
			PostError(Result, FString::Printf(TEXT("Failed to allocate D3D12 data buffer (%s)"), *ResultMsg), ERRCODE_INTERNAL_FAILED_TO_CONVERT_OUTPUT_SAMPLE);
			return false;
		}

		NewOutput->Pitch = BufferPitch;

		if (FElectraDecodersPlatformResources::RunCodeAsync([DecodedOutputSample, Buffer2D, BufferResource = NewOutput->Buffer.Resource, BufferFence = NewOutput->Buffer.Fence, BufferFenceValue = NewOutput->Buffer.FenceValue, BufferPitch, DecodedHeight = NewOutput->DecodedHeight]()
			{

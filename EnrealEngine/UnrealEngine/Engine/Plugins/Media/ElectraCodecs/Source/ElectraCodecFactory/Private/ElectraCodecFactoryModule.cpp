...
public:
	...
	TSharedPtr<IElectraCodecFactory, ESPMode::ThreadSafe> GetBestFactoryForFormat(TMap<FString, FVariant>& OutFormatInfo, const FString& InCodecFormat, bool bInEncoder, const TMap<FString, FVariant>& InOptions) override;
...
TSharedPtr<IElectraCodecFactory, ESPMode::ThreadSafe> FElectraCodecFactoryModule::GetBestFactoryForFormat(TMap<FString, FVariant>& OutFormatInfo, const FString& InCodecFormat, bool bInEncoder, const TMap<FString, FVariant>& InOptions)
{
	...
	for(int32 i=0; i<Factories.Num(); ++i)
	{
		...
		for(int32 j=0; j<FormatFactories.Num(); ++j)
		{
			...
			int32 SupportedPriority = FormatFactory.IsValid() ? FormatFactory->SupportsFormat(FormatInfo, InCodecFormat, bInEncoder, InOptions) : 0;
			if (SupportedPriority > BestPriority)
			{
				BestPriority = SupportedPriority;
				BestFactory = FormatFactory;
				OutFormatInfo = MoveTemp(FormatInfo);
			}
		}
	}

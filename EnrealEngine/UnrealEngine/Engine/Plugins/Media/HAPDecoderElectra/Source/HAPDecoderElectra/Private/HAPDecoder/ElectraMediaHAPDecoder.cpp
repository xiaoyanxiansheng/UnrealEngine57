...
public:
	...
	FVideoDecoderHAPElectra(const TMap<FString, FVariant>& InOptions);
...
private:
	...
	FBufferInstanceInfo InstanceBufferData[FVideoDecoderOutputHAPElectra::Buffer_Max];
#if ELECTRA_MEDIAGPUBUFFER_DX12
...
public:
	...
	{
		return MakeShared<FVideoDecoderHAPElectra, ESPMode::ThreadSafe>(InOptions);
	}
...
FVideoDecoderHAPElectra::FVideoDecoderHAPElectra(const TMap<FString, FVariant>& InOptions)
{
...
static bool ConvertHapTextureFormat(HapTextureFormat HapFmt, EPixelFormat& Format, EElectraTextureSamplePixelEncoding& Encoding)
{
	switch (HapFmt)
	{
		case HapTextureFormat_RGB_DXT1:
		{
			...
			Encoding = EElectraTextureSamplePixelEncoding::Native;
			break;
		}
		...
		{
			Format = EPixelFormat::PF_DXT5;
			Encoding = EElectraTextureSamplePixelEncoding::YCoCg;
			break;
		}
	...
	return Format != EPixelFormat::PF_Unknown;
}
...
// These actually depend on the format, but we don't support the latest HAP formats at the moment.
		...
		for (uint32 BufIdx = 0; BufIdx < NumBuffers; ++BufIdx)
		{
			...
			if (bUseGPUBuffers)
			{
				...
				if (InstanceBufferData[BufIdx].D3D12ResourcePool->AllocateOutputDataAsBuffer(HResult, HResultMsg, BufferInfo.GPUBuffer, ResourcePitch))
				{
				...
				{
					PostError(Result, FString::Printf(TEXT("Could not allocate D3D12 buffer (%s)"), *HResultMsg), ERRCODE_INTERNAL_FAILED_TO_DECODE_INPUT);
					return IElectraDecoder::EDecoderError::Error;
				}
			}
			else
#endif
			{
				BufferInfo.Buffer = MakeShared<TArray64<uint8>, ESPMode::ThreadSafe>();
			...
			if (bUseGPUBuffers)
			{
				if (Result == HapResult_No_Error)
				{
					if (FElectraDecodersPlatformResources::RunCodeAsync([BufferResource = BufferInfo.GPUBuffer.Resource, BufferFence = BufferInfo.GPUBuffer.Fence, BufferFenceValue = BufferInfo.GPUBuffer.FenceValue, ResourcePitch, BufferAddr, BufferPitch, DecodedPitch = DecodedHeight / BlockSizeY, TempBuffer]()
						{

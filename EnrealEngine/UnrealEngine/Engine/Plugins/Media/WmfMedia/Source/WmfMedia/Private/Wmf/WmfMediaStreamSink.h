...
public:
	 ...
	 */
	void SetMediaSamplePoolAndQueue(TSharedPtr<FWmfMediaHardwareVideoDecodingTextureSamplePool>& InVideoSamplePool, TMediaSampleQueue<IMediaTextureSample>* InVideoSampleQueue, TFunction<FMediaTimeStamp(FTimespan, EMediaTrackType)>&& InAdjustTimeStamp, TFunction<bool()>&& InIsSampleStillDesired);
...
private:
	...
	/** Callback to check if a newly decoded sample is still desired or should be discarded. */
	TFunction<bool()> IsSampleStillDesired;

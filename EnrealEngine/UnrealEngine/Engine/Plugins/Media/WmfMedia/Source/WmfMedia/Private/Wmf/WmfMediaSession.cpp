...
{
	// Are we waiting for the end (of non-looping playback)?
	if (bIsWaitingForEnd)
	{
		/*
			Realistically we cannot check the tracks to have delivered all samples, because
			tracks may have different durations. If audio is shorter then the facade will not
			pull the remaining video samples. Also, WmfMedia delivers samples outside the movie's
			duration (with mp4/mov the duration is reported as specified in the movie box, but
			the tracks and the delivered samples may be longer, so we end up getting extra
			samples at the end that are outside the duration and will not be consumed, thus
			sticking around in the sample queues that will not empty.
		*/
#if 0
...
bool FWmfMediaSession::Seek(const FTimespan& Time, const FMediaSeekParams& InAdditionalParams)
{
	if (MediaSession == nullptr)
	{
		return false;
	...
	UE_LOG(LogWmfMedia, Verbose, TEXT("Session %p: Seeking to %s with sequence index %d"), this, *Time.ToString(), InAdditionalParams.NewSequenceIndex.GetValue());
	...
	++NumSeeksInWorkQueue;
	NextSequenceIndex = InAdditionalParams.NewSequenceIndex;
...
STDMETHODIMP FWmfMediaSession::Invoke(IMFAsyncResult* AsyncResult)
{
	...
	case MEBufferingStopped:
		Status &= ~EMediaStatus::Buffering;
		DeferredEvents.Enqueue(EMediaEvent::MediaBufferingComplete);
		break;
...
bool FWmfMediaSession::CommitTime(FTimespan Time, bool bIsSeek)
{
	...
	if (bCanSeek && bIsSeek)
	{
		...
		if (TracksPinned.IsValid())
		{
			check(NextSequenceIndex.IsSet());
			TracksPinned->SeekStarted(OriginalTime, NextSequenceIndex.GetValue());

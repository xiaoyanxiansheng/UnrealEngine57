...
public:
	 ...
	 */
	void SeekStarted(const FTimespan& InTime, int32 InNewSequenceIndex);
...
private:
	...
	/** Check if the current sequence index to apply is still the one that is wanted. */
	bool IsSampleStillDesired();

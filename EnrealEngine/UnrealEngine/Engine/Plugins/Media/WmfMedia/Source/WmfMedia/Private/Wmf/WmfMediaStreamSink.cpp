...
STDMETHODIMP FWmfMediaStreamSink::ProcessSample(__RPC__in_opt IMFSample* pSample)
{
	...
	NumRequestsInFlight = FMath::Max(NumRequestsInFlight-1, 0);
	NumRequestsInFlightWithSystem = FMath::Max(NumRequestsInFlightWithSystem-1, 0);
...
void FWmfMediaStreamSink::CopyTextureAndEnqueueSample(IMFSample* pSample)
{
	...
	Result = CurrentMediaType->GetBlob(MF_MT_MINIMUM_DISPLAY_APERTURE, (UINT8*)&videoArea, sizeof(MFVideoArea), nullptr);
	if (FAILED(Result))
	{
		Result = CurrentMediaType->GetBlob(MF_MT_GEOMETRIC_APERTURE, (UINT8*)&videoArea, sizeof(MFVideoArea), nullptr);
		if (FAILED(Result))
		{
			...
			videoArea.Area.cx = DimX;
			videoArea.Area.cy = DimY;
		}
	}
		...
		else
		{
			...
			if (RHIGetInterfaceType() == ERHIInterfaceType::D3D11)
			{
				...
				ID3D11Texture2D* SharedTexture = TextureSample->InitializeSourceTexture(
					...
					AdjustTimeStamp(FTimespan::FromMicroseconds(SampleTime / 10), EMediaTrackType::Video),
					FTimespan::FromMicroseconds(SampleDuration / 10),
					FIntPoint(videoArea.Area.cx, videoArea.Area.cy),
					PixelFormat,
				...
				SrcBox.top = 0;
				SrcBox.front = 0;
				SrcBox.right = videoArea.Area.cx;
				SrcBox.bottom = videoArea.Area.cy;
				SrcBox.back = 1;
			...
			else
			{
				...
				if (pBuffer->Lock(&Data, NULL, NULL) == S_OK)
				{
					...
					TextureSample->Initialize(Data, BufferSize, FIntPoint(DimX, SampleDimY), FIntPoint(videoArea.Area.cx, videoArea.Area.cy), MediaTextureSampleFormat, Pitch, AdjustTimeStamp(FTimespan::FromMicroseconds(SampleTime / 10), EMediaTrackType::Video), FTimespan::FromMicroseconds(SampleDuration / 10));
					pBuffer->Unlock();

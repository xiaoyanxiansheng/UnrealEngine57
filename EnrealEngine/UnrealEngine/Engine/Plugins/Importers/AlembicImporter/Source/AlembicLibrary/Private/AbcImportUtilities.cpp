...
bool AbcImporterUtilities::GenerateAbcMeshSampleDataForFrame(const Alembic::AbcGeom::IPolyMeshSchema &Schema, const Alembic::Abc::ISampleSelector FrameSelector, FAbcMeshSample* &Sample, const ESampleReadFlags ReadFlags, const bool bFirstFrame)
{
	...
	if (EnumHasAnyFlags(ReadFlags, ESampleReadFlags::Positions))
	{
		...
		if (!bRetrievalResult)
		{
			TSharedRef<FTokenizedMessage> Message = FTokenizedMessage::Create(EMessageSeverity::Error, LOCTEXT("NoVertices", "Could not retrieve mesh vertices."));
	...
	if (EnumHasAnyFlags(ReadFlags, ESampleReadFlags::Indices | ESampleReadFlags::UVs | ESampleReadFlags::Normals | ESampleReadFlags::Colors | ESampleReadFlags::MaterialIndices))
	{
		...
		if (!bRetrievalResult)
		{
			TSharedRef<FTokenizedMessage> Message = FTokenizedMessage::Create(EMessageSeverity::Error, LOCTEXT("NoFaceCounts", "Could not retrieve mesh face counts."));
			FAbcImportLogger::AddImportMessage(Message);
			return false;
		}
	}
	...
	if (EnumHasAnyFlags(ReadFlags, ESampleReadFlags::Indices))
	{
		...
		{
			TSharedRef<FTokenizedMessage> Message = FTokenizedMessage::Create(EMessageSeverity::Error, LOCTEXT("NoIndices", "Could not retrieve mesh indices."));
			FAbcImportLogger::AddImportMessage(Message);
			return false;
		}
...
FAbcMeshSample* AbcImporterUtilities::MergeMeshSamples(const TArray<const FAbcMeshSample*>& Samples)
{
	...
	for (const FAbcMeshSample* Sample : Samples)
	{
		...
		ensureMsgf(MaterialIndicesOffset == SmoothingGroupIndicesOffset, TEXT("Material and smoothing group indices count should match (%d != %d))"), MaterialIndicesOffset, SmoothingGroupIndicesOffset);
		...
		// Remap material and smoothing group indices in separate loops since they might not have the same number of elements
		...
		const uint32 NumSmoothingGroupIndices = MergedSample->SmoothingGroupIndices.Num();
		for (uint32 IndiceIndex = SmoothingGroupIndicesOffset; IndiceIndex < NumSmoothingGroupIndices; ++IndiceIndex)
		{
...
void AbcImporterUtilities::AppendMeshSample(FAbcMeshSample* MeshSampleOne, const FAbcMeshSample* MeshSampleTwo)
{
	...
	const uint32 NumSmoothingGroupIndices = MeshSampleOne->SmoothingGroupIndices.Num();
	for (uint32 IndiceIndex = SmoothingGroupIndicesOffset; IndiceIndex < NumSmoothingGroupIndices; ++IndiceIndex)
	{
	...
	const uint32 MaterialOffset, const float SecondsPerFrame, const bool bUseVelocitiesAsMotionVectors, const bool bStoreImportedVertexNumbers)
{
	...
	if (MeshSample->Vertices.Num() == 0)
	{
		// Somehow the MeshSample can have Indices but no Vertices so just skip the whole thing if there are no vertices
		// Only the BoundingBox needs to be initialized to avoid asserts when rendering
		return;
	}
	...
	FGeometryCacheMeshData& MeshData, int32& PreviousNumVertices, bool& bConstantTopology, bool bStoreImportedVertexNumbers)
{
	...
	// For some reason, merged meshes (especially with varying topologies) can end up with an invalid number of material indices.
	// Here, the material indices is filled up with 0 (the first material) to match the number of expected triangles so that the
	// generated geometry cache mesh data from GeometryCacheDataForMeshSample is complete.
	const int32 NumTriangles = MergedSample.Indices.Num() / 3;
	const int32 NumMatIndices = MergedSample.MaterialIndices.Num();
	if (!ensureMsgf(NumMatIndices >= NumTriangles, TEXT("Patching merged meshes (%d) since there are fewer material indices than triangles (%d < %d)."), PolyMeshes.Num(), NumMatIndices, NumTriangles))
	{
		const int32 Delta = NumTriangles - NumMatIndices;
		MergedSample.MaterialIndices.AddZeroed(Delta);
	}

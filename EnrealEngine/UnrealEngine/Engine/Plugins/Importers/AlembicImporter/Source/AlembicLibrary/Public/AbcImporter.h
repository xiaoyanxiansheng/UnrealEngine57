...
{
public:
	ALEMBICLIBRARY_API FAbcImporter();
	ALEMBICLIBRARY_API ~FAbcImporter();
public:
	/**
	...
	*/
	ALEMBICLIBRARY_API const EAbcImportError OpenAbcFileForImport(const FString InFilePath);
	...
	* @return - Possible error code
	*/
	ALEMBICLIBRARY_API const EAbcImportError ImportTrackData(const int32 InNumThreads, UAbcImportSettings* ImportSettings);
	...
	* @return FStaticMesh*
	*/
	ALEMBICLIBRARY_API const TArray<UStaticMesh*> ImportAsStaticMesh(UObject* InParent, EObjectFlags Flags);
	...
	*/
	ALEMBICLIBRARY_API UGeometryCache* ImportAsGeometryCache(UObject* InParent, EObjectFlags Flags);

	ALEMBICLIBRARY_API TArray<UObject*> ImportAsSkeletalMesh(UObject* InParent, EObjectFlags Flags);	
	...
	* @return FStaticMesh*
	*/
	ALEMBICLIBRARY_API const TArray<UStaticMesh*> ReimportAsStaticMesh(UStaticMesh* Mesh);
	...
	* @return UGeometryCache*
	*/
	ALEMBICLIBRARY_API UGeometryCache* ReimportAsGeometryCache(UGeometryCache* GeometryCache);
	...
	*/
	ALEMBICLIBRARY_API TArray<UObject*> ReimportAsSkeletalMesh(USkeletalMesh* SkeletalMesh);

	/** Returns the array of imported PolyMesh objects */
	ALEMBICLIBRARY_API const TArray<class FAbcPolyMesh*>& GetPolyMeshes() const;
	
	/** Returns the lowest frame index containing data for the imported Alembic file */
	ALEMBICLIBRARY_API const uint32 GetStartFrameIndex() const;

	/** Returns the highest frame index containing data for the imported Alembic file */
	ALEMBICLIBRARY_API const uint32 GetEndFrameIndex() const;

	/** Returns the number of tracks found in the imported Alembic file */
	ALEMBICLIBRARY_API const uint32 GetNumMeshTracks() const;

	ALEMBICLIBRARY_API void UpdateAssetImportData(UAbcAssetImportData* AssetImportData);
	ALEMBICLIBRARY_API void RetrieveAssetImportData(UAbcAssetImportData* ImportData);
private:
	/**
	...
	* @return UStaticMesh*
	*/
	ALEMBICLIBRARY_API UStaticMesh* CreateStaticMeshFromSample(UObject* InParent, const FString& Name, EObjectFlags Flags, const TArray<FString>& UniqueFaceSetNames, const TArray<int32>& LookupMaterialSlot, const FAbcMeshSample* Sample);

	/** Generates and populate a FMeshDescription instance from the given sample*/
	ALEMBICLIBRARY_API void GenerateMeshDescriptionFromSample(const TArray<FString>& UniqueFaceSetNames, const TArray<int32>& LookupMaterialSlot, const FAbcMeshSample* Sample, FMeshDescription* MeshDescription);
	
	/** Compresses the imported animation data, returns true if compression was successful and compressed data was populated */
	ALEMBICLIBRARY_API const bool CompressAnimationDataUsingPCA(const FAbcCompressionSettings& InCompressionSettings, const bool bRunComparison = false);	
	/** Performs the actual SVD compression to retrieve the bases and weights used to set up the Skeletal mesh's morph targets */
	ALEMBICLIBRARY_API const int32 PerformSVDCompression(const TArray64<float>& OriginalMatrix, const TArray64<float>& OriginalNormalsMatrix, const uint32 NumSamples, const float InPercentage, const int32 InFixedNumValue,
	...
	ALEMBICLIBRARY_API void CompareCompressionResult(const TArray64<float>& OriginalMatrix, const uint32 NumSamples, const uint32 NumUsedSingularValues, const TArrayView64<float>& OutU, const TArray64<float>& OutV, const float Tolerance);
	
	/** Build a skeletal mesh from the PCA compressed data */
	ALEMBICLIBRARY_API bool BuildSkeletalMesh(FSkeletalMeshLODModel& LODModel, const FReferenceSkeleton& RefSkeleton, FAbcMeshSample* Sample, 
	...
	ALEMBICLIBRARY_API void GenerateMorphTargetVertices(FAbcMeshSample* BaseSample, TArray<FMorphTargetDelta> &MorphDeltas, FAbcMeshSample* AverageSample, uint32 WedgeOffset, const TArray<int32>& RemapIndices, const TArray<int32>& UsedVertexIndicesForMorphs, const uint32 VertexOffset, const uint32 IndexOffset);
	
	/** Set up correct morph target weights from the PCA compressed data */
	ALEMBICLIBRARY_API void SetupMorphTargetCurves(USkeleton* Skeleton, FName ConstCurveName, UAnimSequence* Sequence, const TArray<float> &CurveValues, const TArray<float>& TimeValues, IAnimationDataController& Controller, bool bShouldTransact);
	
	/** Set the Alembic archive metadata on the given objects */
	ALEMBICLIBRARY_API void SetMetaData(const TArray<UObject*>& Objects);

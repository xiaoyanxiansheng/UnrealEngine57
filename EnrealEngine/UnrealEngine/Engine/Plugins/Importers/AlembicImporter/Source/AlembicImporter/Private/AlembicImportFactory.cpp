	...
	if (NumMeshes > 0)
	{
		...
		for (UStaticMesh* StaticMesh : StaticMeshes)
		{
			if (StaticMesh)
			{
				...
				if (!StaticMesh->GetAssetImportData() || !StaticMesh->GetAssetImportData()->IsA<UAbcAssetImportData>())
				{
					StaticMesh->SetAssetImportData(NewObject<UAbcAssetImportData>(StaticMesh));
				}
				StaticMesh->GetAssetImportData()->Update(UFactory::CurrentFilename);

				UAbcAssetImportData* AssetImportData = Cast<UAbcAssetImportData>(StaticMesh->GetAssetImportData());
				if (AssetImportData)
				{
...
EReimportResult::Type UAlembicImportFactory::Reimport(UObject* Obj)
{
	...
	if (Obj->GetClass() == UStaticMesh::StaticClass())
	{
		...
		CurrentFilename = Mesh->GetAssetImportData()->GetFirstFilename();
...
EReimportResult::Type UAlembicImportFactory::ReimportStaticMesh(UStaticMesh* Mesh)
{
	...
	if (Mesh->GetAssetImportData() && Mesh->GetAssetImportData()->IsA<UAbcAssetImportData>())
	{
		UAbcAssetImportData* ImportData = Cast<UAbcAssetImportData>(Mesh->GetAssetImportData());
		PopulateOptionsWithImportData(ImportData);
		Importer.RetrieveAssetImportData(ImportData);
	}

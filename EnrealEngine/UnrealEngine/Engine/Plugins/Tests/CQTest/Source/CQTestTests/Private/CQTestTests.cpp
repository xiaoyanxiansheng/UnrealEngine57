...
namespace CQTestTests
{
	...
	TEST_CLASS_WITH_FLAGS_AND_TAGS(OverrideFixtureTestFlagsAndTags, "TestFramework.CQTest.Core.Tags", EAutomationTestFlags_ApplicationContextMask | EAutomationTestFlags::EngineFilter, "[CQFlagsTest][FlagsExtraTag][CQTaggedTests]")
	{
		TEST_METHOD(GetTestFlags_ReturnsSetAutomationTestFlags_HasExpectedTags)
		{
			ASSERT_THAT(AreEqual(EAutomationTestFlags_ApplicationContextMask | EAutomationTestFlags::EngineFilter, TestRunner->GetTestFlags()));

			const FString TestTags = TestRunner->GetTestTags();
			ASSERT_THAT(IsTrue(TestTags.Contains(TEXT("[CQFlagsTest]"))));
			ASSERT_THAT(IsTrue(TestTags.Contains(TEXT("[FlagsExtraTag]"))));
		}

		TEST_METHOD_WITH_TAGS(GetTestFlags_HasExpectedMethodTags, "[CQClassWithFlagsAndTagsMethod]")
		{
			const FString ThisMethodName = TEXT("GetTestFlags_HasExpectedMethodTags");
			const FString TestTags = TestRunner->GetTestMethodTags(ThisMethodName);
			ASSERT_THAT(IsTrue(TestTags.Contains(TEXT("[CQFlagsTest]"))));
			ASSERT_THAT(IsTrue(TestTags.Contains(TEXT("[FlagsExtraTag]"))));
			ASSERT_THAT(IsTrue(TestTags.Contains(TEXT("[CQClassWithFlagsAndTagsMethod]"))));
		}
	};
	...
	TEST_CLASS_WITH_TAGS(CQTags, "TestFramework.CQTest.Core.Tags", "[CQTest][ExtraTag][CQTaggedTests]")
	{
		TEST_METHOD(CheckTagsAreValid)
		{
			const FString TestTags = TestRunner->GetTestTags();
			ASSERT_THAT(IsTrue(TestTags.Contains(TEXT("[CQTest]"))));
			ASSERT_THAT(IsTrue(TestTags.Contains(TEXT("[ExtraTag]"))));
		}

		TEST_METHOD_WITH_TAGS(CheckTagsAreValid_HasExpectedMethodTags, "[CQClassWithTagsMethod]")
		{
			const FString ThisMethodName = TEXT("CheckTagsAreValid_HasExpectedMethodTags");
			const FString TestTags = TestRunner->GetTestMethodTags(ThisMethodName);
			ASSERT_THAT(IsTrue(TestTags.Contains(TEXT("[CQTest]"))));
			ASSERT_THAT(IsTrue(TestTags.Contains(TEXT("[ExtraTag]"))));
			ASSERT_THAT(IsTrue(TestTags.Contains(TEXT("[CQClassWithTagsMethod]"))));
		}

		TEST_METHOD_WITH_TAGS(CheckTagsAreValid_HasExpectedMethodTagsNoLeak, "[CQClassWithTagsNoLeakMethod]")
		{
			const FString ThisMethodName = TEXT("CheckTagsAreValid_HasExpectedMethodTagsNoLeak");
			const FString TestTags = TestRunner->GetTestMethodTags(ThisMethodName);
			ASSERT_THAT(IsTrue(TestTags.Contains(TEXT("[CQClassWithTagsNoLeakMethod]"))));
			ASSERT_THAT(IsFalse(TestTags.Contains(TEXT("[CQClassWithTagsMethod]"))));
		}
	};
} // namespace CQTestTests

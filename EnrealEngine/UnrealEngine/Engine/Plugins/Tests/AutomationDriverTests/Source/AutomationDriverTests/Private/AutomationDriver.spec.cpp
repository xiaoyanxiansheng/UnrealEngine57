...
void FAutomationDriverSpec::Define()
{
	...
	Describe("By::TextFilter", [this]()
	{
		Describe("Equals", [this]()
		{
			static const FString ElementText = "Bb/A#";
			static const FString ElementSubstringText = ElementText.LeftChop(1);
			static const FString ElementWrongCaseText = ElementText.ToLower();

			It("should fail to locate an element if none exist with the specified text", EAsyncExecution::ThreadPool, [this]()
			{
				TEST_FALSE(Driver->FindElement(By::TextFilter::Equals(By::Path("#Piano//<SButton>"), "NotDefined"))->Exists());
			});

			It("should fail to locate an element containing the specified substring in the text", EAsyncExecution::ThreadPool, [this]()
			{
				TEST_FALSE(Driver->FindElement(By::TextFilter::Equals(By::Path("#Piano//<SButton>"), ElementSubstringText))->Exists());
			});

			Describe("IgnoreCase", [this]()
			{
				It("should locate an element with the specified text in a different case", EAsyncExecution::ThreadPool, [this]()
				{
					TEST_TRUE(Driver->FindElement(By::TextFilter::Equals(By::Path("#Piano//<SButton>"), ElementWrongCaseText, ESearchCase::IgnoreCase))->Exists());
				});

				It("should locate an element with the specified text in the proper case", EAsyncExecution::ThreadPool, [this]()
				{
					TEST_TRUE(Driver->FindElement(By::TextFilter::Equals(By::Path("#Piano//<SButton>"), ElementText, ESearchCase::IgnoreCase))->Exists());
				});
			});

			Describe("CaseSensitive", [this]()
			{
				It("should fail to locate an element with the specified text in a different case", EAsyncExecution::ThreadPool, [this]()
				{
					TEST_FALSE(Driver->FindElement(By::TextFilter::Equals(By::Path("#Piano//<SButton>"), ElementWrongCaseText, ESearchCase::CaseSensitive))->Exists());
				});

				It("should locate an element with the specified text in the proper case", EAsyncExecution::ThreadPool, [this]()
				{
					TEST_TRUE(Driver->FindElement(By::TextFilter::Equals(By::Path("#Piano//<SButton>"), ElementText, ESearchCase::CaseSensitive))->Exists());
				});
			});

			Describe("Default", [this]()
			{
				It("should use case sensitive comparison by default", EAsyncExecution::ThreadPool, [this]()
				{
					TEST_TRUE(Driver->FindElement(By::TextFilter::Equals(By::Path("#Piano//<SButton>"), ElementText))->Exists());
					TEST_FALSE(Driver->FindElement(By::TextFilter::Equals(By::Path("#Piano//<SButton>"), ElementWrongCaseText))->Exists());
				});
			});
			
			It("should be capable of locating multiple elements", EAsyncExecution::ThreadPool, [this]()
			{
				TEST_EQUAL(Driver->FindElements(By::TextFilter::Equals(By::Path("Documents//<SButton>"), "Document 1", ESearchCase::CaseSensitive))->GetElements().Num(), 3);
			});
		});
		
		Describe("Contains", [this]()
		{
			static const FString ElementText = "Bb/A#";
			static const FString ElementSubstringText = ElementText.LeftChop(1);
			static const FString ElementWrongCaseSubstringText = ElementSubstringText.ToLower();

			static const FString ElementLeftSubstringText = ElementText.LeftChop(1);
			static const FString ElementRightSubstringText = ElementText.RightChop(1);
			static const FString ElementCenterSubstringText = ElementText.LeftChop(1).RightChop(1);

			It("should fail to locate an element if none exist with the specified text", EAsyncExecution::ThreadPool, [this]()
			{
				TEST_FALSE(Driver->FindElement(By::TextFilter::Contains(By::Path("#Piano//<SButton>"), "NotDefined"))->Exists());
			});

			It("should locate an element containing the specified substring at any position in the text", EAsyncExecution::ThreadPool, [this]()
			{
				TEST_TRUE(Driver->FindElement(By::TextFilter::Contains(By::Path("#Piano//<SButton>"), ElementLeftSubstringText))->Exists());
				TEST_TRUE(Driver->FindElement(By::TextFilter::Contains(By::Path("#Piano//<SButton>"), ElementCenterSubstringText))->Exists());
				TEST_TRUE(Driver->FindElement(By::TextFilter::Contains(By::Path("#Piano//<SButton>"), ElementRightSubstringText))->Exists());
			});

			Describe("IgnoreCase", [this]()
			{
				It("should locate an element containing the specified substring in a different case", EAsyncExecution::ThreadPool, [this]()
				{
					TEST_TRUE(Driver->FindElement(By::TextFilter::Contains(By::Path("#Piano//<SButton>"), ElementWrongCaseSubstringText, ESearchCase::IgnoreCase))->Exists());
				});

				It("should locate an element containing the specified substring in the proper case", EAsyncExecution::ThreadPool, [this]()
				{
					TEST_TRUE(Driver->FindElement(By::TextFilter::Contains(By::Path("#Piano//<SButton>"), ElementSubstringText, ESearchCase::IgnoreCase))->Exists());
				});
			});

			Describe("CaseSensitive", [this]()
			{
				It("should fail to locate an element containing the specified substring in a different case", EAsyncExecution::ThreadPool, [this]()
				{
					TEST_FALSE(Driver->FindElement(By::TextFilter::Contains(By::Path("#Piano//<SButton>"), ElementWrongCaseSubstringText, ESearchCase::CaseSensitive))->Exists());
				});

				It("should locate an element containing the specified substring in the proper case", EAsyncExecution::ThreadPool, [this]()
				{
					TEST_TRUE(Driver->FindElement(By::TextFilter::Contains(By::Path("#Piano//<SButton>"), ElementSubstringText, ESearchCase::CaseSensitive))->Exists());
				});
			});

			Describe("SearchDir", [this]()
			{
				It("should locate an element containing the specified substring by searching in both directions", EAsyncExecution::ThreadPool, [this]()
				{
					TEST_TRUE(Driver->FindElement(By::TextFilter::Contains(By::Path("#Piano//<SButton>"), ElementLeftSubstringText, ESearchCase::CaseSensitive, ESearchDir::FromStart))->Exists());
					TEST_TRUE(Driver->FindElement(By::TextFilter::Contains(By::Path("#Piano//<SButton>"), ElementLeftSubstringText, ESearchCase::CaseSensitive, ESearchDir::FromEnd))->Exists());
					TEST_TRUE(Driver->FindElement(By::TextFilter::Contains(By::Path("#Piano//<SButton>"), ElementRightSubstringText, ESearchCase::CaseSensitive, ESearchDir::FromStart))->Exists());
					TEST_TRUE(Driver->FindElement(By::TextFilter::Contains(By::Path("#Piano//<SButton>"), ElementRightSubstringText, ESearchCase::CaseSensitive, ESearchDir::FromEnd))->Exists());
				});
			});

			Describe("Default", [this]()
			{
				It("should ignore case by default", EAsyncExecution::ThreadPool, [this]()
				{
					TEST_TRUE(Driver->FindElement(By::TextFilter::Contains(By::Path("#Piano//<SButton>"), ElementSubstringText))->Exists());
					TEST_TRUE(Driver->FindElement(By::TextFilter::Contains(By::Path("#Piano//<SButton>"), ElementWrongCaseSubstringText))->Exists());
				});
			});
			
			It("should be capable of locating multiple elements", EAsyncExecution::ThreadPool, [this]()
			{
				const FString ElementsSubstringText = "ocument 2";
				const int32 ElementsNum = 12; // "Document 2", "Document 20", ..., "Document 29", "Document 200"
				TEST_EQUAL(Driver->FindElements(By::TextFilter::Contains(By::Path("#Documents//<SScrollBox>//<SButton>"), ElementsSubstringText, ESearchCase::CaseSensitive))->GetElements().Num(), ElementsNum);
			});
		});
	});

...
bool FRHIReadbackTests::Test_BufferReadback(FRHICommandListImmediate& RHICmdList)
{
	...
	//
	{
		const EBufferUsageFlags TypedBufferUsage = EBufferUsageFlags::UnorderedAccess | EBufferUsageFlags::SourceCopy;
		const FRHIBufferCreateDesc CreateDesc =
			FRHIBufferCreateDesc::Create(TEXT("TypedBuffer"), TypedBufferSize, 0, TypedBufferUsage)
			.SetInitialState(ERHIAccess::UAVCompute);
		TypedBuffer = RHICmdList.CreateBuffer(CreateDesc);
	...
	{
		const EBufferUsageFlags ByteAddressBufferUsage = EBufferUsageFlags::UnorderedAccess | EBufferUsageFlags::SourceCopy | EBufferUsageFlags::ByteAddressBuffer | EBufferUsageFlags::StructuredBuffer;
		const FRHIBufferCreateDesc CreateDesc =
			FRHIBufferCreateDesc::Create(TEXT("ByteAddressBuffer"), ByteAddressBufferSize, sizeof(uint32), ByteAddressBufferUsage)
			.SetInitialState(ERHIAccess::UAVCompute);
		ByteAddressBuffer = RHICmdList.CreateBuffer(CreateDesc);
	...
	{
		const EBufferUsageFlags StructuredBufferUsage = EBufferUsageFlags::UnorderedAccess | EBufferUsageFlags::SourceCopy | EBufferUsageFlags::StructuredBuffer;
		const FRHIBufferCreateDesc CreateDesc =
			FRHIBufferCreateDesc::Create(TEXT("StructuredBuffer"), StructuredBufferSize, sizeof(FStructuredBufferElem), StructuredBufferUsage)
			.SetInitialState(ERHIAccess::UAVCompute);
		StructuredBuffer = RHICmdList.CreateBuffer(CreateDesc);

...
void FAutomationRHITest::Define()
{
	...
	Describe("Test RHI Allocators", [this]()
	{
		It("LockBuffer 16 Byte Alignment", [this]()
		{
			bool bResult = RunOnRenderThreadSynchronous(FRHIAllocatorTests::Test_LockBuffer16ByteAlignment);
			TestEqual("LockBuffer 16 Byte Alignment Failed", bResult, 1);
		});
	});

	Describe("Test RHI Bindless", [this]()
	{
		It("RHI Resource Collection", [this]()
		{
			bool bResult = RunOnRenderThreadSynchronous(RHIBindlessTests::Test_ResourceCollection);
			TestEqual("Resource Collection failed", bResult, 1);
		});
	});
}

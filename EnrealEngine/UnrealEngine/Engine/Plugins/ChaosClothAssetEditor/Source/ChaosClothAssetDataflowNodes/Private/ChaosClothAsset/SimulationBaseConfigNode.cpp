...
#include UE_INLINE_GENERATED_CPP_BY_NAME(SimulationBaseConfigNode)
	...
	ECollectionPropertyFlags PropertyFlags) const
{
	...
	int32 KeyIndex = Properties.GetKeyNameIndex(PropertyName);
	if (KeyIndex == INDEX_NONE)
	{
		KeyIndex = Properties.AddProperty(PropertyName);
	}
	...
	{
		const int32 SimilarPropertyKeyIndex = Properties.GetKeyNameIndex(SimilarPropertyName);
...
void FChaosClothAssetSimulationBaseConfigNode::FPropertyHelper::OverridePropertiesBool(const TArray<FName>& PropertyNames, bool bPropertyValue)
{
	...
	{
		const int32 PropertyKeyIndex = Properties.GetKeyNameIndex(PropertyName);

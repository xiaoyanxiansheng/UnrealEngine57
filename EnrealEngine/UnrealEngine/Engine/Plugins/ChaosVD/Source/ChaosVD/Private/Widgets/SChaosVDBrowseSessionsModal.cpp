...
void SChaosVDBrowseSessionsModal::Construct(const FArguments& InArgs)
{
	...
	ConnectionMode = EChaosVDLoadRecordedDataMode::SingleSource;
		...
		[
			...
			[
				...
				[
					...
					[
						SNew(STextBlock)
						.Text(LOCTEXT("LiveConnectionMode", "Connection Mode"))
						.Font(FCoreStyle::GetDefaultFontStyle("Regular", 12))
					]
					+SHorizontalBox::Slot()
					.HAlign(HAlign_Right)
...
TSharedPtr<SWidget> SChaosVDBrowseSessionsModal::GenerateConnectionModeWidget()
{
	TAttribute<int32> GetCurrentMode;
	GetCurrentMode.BindSPLambda(this, [this]()
	{
		return static_cast<int32>(ConnectionMode);
	});
	...
	{
		ConnectionMode = static_cast<EChaosVDLoadRecordedDataMode>(NewValue);
	});
...
void SChaosVDBrowseSessionsModal::UpdateCurrentSessionInfoMap()
{
	...
	for (uint32 SessionIndex = 0; SessionIndex < SessionCount; SessionIndex++)
	{
		if (const FStoreClient::FSessionInfo* SessionInfo = StoreClient->GetSessionInfo(SessionIndex))
		{
			...
			FStringFormatOrderedArguments Args {IP.ToString(), FString::FromInt(SessionInfo->GetTraceId()), FString::FromInt(SessionIndex)};
			FString SessionDisplayNameString = FString::Format(TEXT("IP: {0} + {1} + {2}"), Args);

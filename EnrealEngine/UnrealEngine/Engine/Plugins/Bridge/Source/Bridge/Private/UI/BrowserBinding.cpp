...
void UBrowserBinding::ShowLoginDialog(FString LoginUrl, FString ResponseCodeUrl) 
{
	...
	AsyncTask(ENamedThreads::Type::GameThread, [this, LoginUrl, ResponseCodeUrl]() 
	{
						...
						.OnUrlChanged_Lambda([ResponseCodeUrl](const FText& Url) 
									{
										...
										{
											// we are constructing this on the game thread, but this lambda is not necessairly 
											// running there and we need this to run on the GameThread because it interacts with
											// Slate.
											AsyncTask(ENamedThreads::Type::GameThread, [RedirectedUrl, ResponseCodeUrl] ()
										    {
...
void UBrowserBinding::DragStarted(TArray<FString> ImageUrls, TArray<FString> IDs, TArray<FString> Types)
{
	...
	AsyncTask(ENamedThreads::Type::GameThread, [this, ImageUrls, IDs, ActorFactory] () 
	{

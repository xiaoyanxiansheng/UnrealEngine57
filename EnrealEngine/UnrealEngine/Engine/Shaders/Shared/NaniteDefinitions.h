...
#define NANITE_USE_ANALYTIC_SGGX							0
...
#define NANITE_CONSTRAINED_CLUSTER_CACHE_SIZE				32
...
#define NANITE_MAX_VOXEL_ANIMATION_BONE_INFLUENCES			1
...
#define NANITE_MAX_CLUSTER_HIERARCHY_DEPTH					32
...
#define NANITE_PAGE_RANGE_KEY_COUNT_BITS					(NANITE_MAX_GROUP_PARTS_BITS) // can be no more than max group parts
#define NANITE_PAGE_RANGE_KEY_COUNT_MASK					((1u << NANITE_PAGE_RANGE_KEY_COUNT_BITS) - 1u)
#define NANITE_PAGE_RANGE_KEY_MAX_COUNT						(NANITE_PAGE_RANGE_KEY_COUNT_MASK)
#define NANITE_PAGE_RANGE_KEY_INDEX_BITS					25u
#define NANITE_PAGE_RANGE_KEY_INDEX_MASK					((1u << NANITE_PAGE_RANGE_KEY_INDEX_BITS) - 1u)
#define NANITE_PAGE_RANGE_KEY_MAX_INDEX						(NANITE_PAGE_RANGE_KEY_INDEX_MASK - 1u) // page range key of ~0u has special meaning
#define NANITE_PAGE_RANGE_KEY_FLAG_MULTI_RANGE				(1u << 31u)
#define NANITE_PAGE_RANGE_KEY_FLAG_HAS_STREAMING_PAGES		(1u << 30u) // used by the GPU to prevent superfluous streaming requests
#define NANITE_PAGE_RANGE_KEY_EMPTY_RANGE					(0u)
...
#define NANITE_HIERARCHY_ASSEMBLY_TRANSFORM_INDEX_BITS		16
#define NANITE_HIERARCHY_MAX_ASSEMBLY_TRANSFORMS			((1 << NANITE_HIERARCHY_ASSEMBLY_TRANSFORM_INDEX_BITS) - 1)

#define NANITE_ASSEMBLY_TRANSFORM_INDEX_BITS				24
...
#define NANITE_RENDER_FLAG_DRAW_ONLY_RAYTRACING_FAR_FIELD	0x20000
...
#define NANITE_MATERIAL_FLAG_VOXEL							0x800
#define NANITE_MATERIAL_FLAG_FIRST_PERSON_LERP				0x1000

#define NANITE_TRANSCODE_PASS_INDEPENDENT					0
#define NANITE_TRANSCODE_PASS_PARENT_DEPENDENT				1

#define NANITE_MATERIAL_VERTEX_PROGRAMMABLE_FLAGS			(NANITE_MATERIAL_FLAG_WORLD_POSITION_OFFSET | NANITE_MATERIAL_FLAG_DISPLACEMENT | NANITE_MATERIAL_FLAG_VERTEX_UVS | NANITE_MATERIAL_FLAG_FIRST_PERSON_LERP)
...
#define NANITE_DEBUG_FLAG_DISABLE_CULL_MIN_LOD						0x10
#define NANITE_DEBUG_FLAG_DISABLE_WPO_DISABLE_DISTANCE				0x20
#define NANITE_DEBUG_FLAG_DISABLE_PIXEL_PROGRAMMABLE_DISTANCE		0x40
#define NANITE_DEBUG_FLAG_DISABLE_DISPLACEMENT_FADE_OUT_DISTANCE	0x80
#define NANITE_DEBUG_FLAG_DRAW_ONLY_ROOT_DATA						0x100
#define NANITE_DEBUG_FLAG_HIDE_ASSEMBLY_PARTS						0x200
#define NANITE_DEBUG_FLAG_WRITE_ASSEMBLY_META						0x400

#define NANITE_INVALID_RASTER_BIN		0xFFFFu
...
#define NANITE_SHADING_BIN_CMASK_CLEAR	4
...
#define NANITE_VISUALIZE_SHADOW_CASTERS						26u
...
#define NANITE_VISUALIZE_SKINNING							39u

#define NANITE_VISUALIZE_FLAG_ENABLE_OUTLINE		(1u << 0u)
#define NANITE_VISUALIZE_FLAG_ENABLE_DEBUG_SHADING	(1u << 1u)
...
INLINE_ATTR FNaniteMaterialFlags UnpackNaniteMaterialFlags(UINT_TYPE Packed)
{
	...
	MaterialFlags.bVoxel = (Packed & NANITE_MATERIAL_FLAG_VOXEL) != 0u;
	MaterialFlags.bFirstPersonLerp = (Packed & NANITE_MATERIAL_FLAG_FIRST_PERSON_LERP) != 0u;
...
{
	return MaterialFlags.bWorldPositionOffset || MaterialFlags.bDisplacement || MaterialFlags.bVertexUVs || MaterialFlags.bFirstPersonLerp;
}
...
INLINE_ATTR UINT_TYPE PackNaniteMaterialBitFlags(FNaniteMaterialFlags Flags)
{
	...
	if (Flags.bVoxel)
	{
		MaterialBitFlags |= NANITE_MATERIAL_FLAG_VOXEL;
	}

	if (Flags.bFirstPersonLerp)
	{
		MaterialBitFlags |= NANITE_MATERIAL_FLAG_FIRST_PERSON_LERP;
	}
...
struct FNaniteStats
{
	...
	UINT_TYPE NumAssemblyTransforms;
	UINT_TYPE Debug[4];
};
...
struct FNaniteShadingBinScatterCounters
{
	UINT_TYPE FullTileElementCount;
	UINT_TYPE LooseElementCount;
	UINT_TYPE Pad0[30];				// Pad to 128b to better distribute the atomics operations across banks.
};

// Intentionally separate from counters, so binning pass can use scalar SRV reads.
struct FNaniteShadingBinScatterRanges
{
	UINT_TYPE RangeStart;
	UINT_TYPE RangeEnd;
};

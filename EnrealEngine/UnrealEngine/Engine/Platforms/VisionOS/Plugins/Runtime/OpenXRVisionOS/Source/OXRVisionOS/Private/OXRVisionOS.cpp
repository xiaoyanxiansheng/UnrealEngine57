...
void FOXRVisionOS::OnBeginRendering_GameThread(XrSession InSession, FSceneViewFamily& InViewFamily, TArrayView<const uint32> VisibleLayers)
{
...
void FOXRVisionOS::OnBeginRendering_RenderThread_PreDeviceLocationUpdate(XrSession InSession, FRDGBuilder& GraphBuilder)
{
	if (!FOXRVisionOSInstance::CheckSession(InSession))
	{
		return;
	...
	FOXRVisionOSSession* Session = (FOXRVisionOSSession*)InSession;
	Session->OnBeginRendering_RenderThread_PreDeviceLocationUpdate();
}

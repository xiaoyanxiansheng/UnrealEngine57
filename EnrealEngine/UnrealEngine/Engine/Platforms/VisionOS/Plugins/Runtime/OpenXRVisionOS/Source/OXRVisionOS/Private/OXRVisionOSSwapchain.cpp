...
FOXRVisionOSSwapchain::FOXRVisionOSSwapchain(const XrSwapchainCreateInfo* createInfo, uint32 SwapchainLength, FOXRVisionOSSession* InSession)
{
...

	{
		...
		{
			const FString Name = FString::Printf(TEXT("%s_SwapChain%d"), (CreateInfo.usageFlags & XR_SWAPCHAIN_USAGE_COLOR_ATTACHMENT_BIT) ? TEXT("Color") : TEXT("Depth"), BufferIndex);

			FRHITextureCreateDesc CurrentBackBufferDesc = BackBufferDesc;
			CurrentBackBufferDesc.DebugName = *Name;

			FTextureRHIRef Tex = RHICmdList.CreateTexture(CurrentBackBufferDesc);

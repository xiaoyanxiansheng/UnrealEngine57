...
void FOXRVisionOSSession::StartSubmission()
{
	...
	if (FrameState.SwiftFrame)
	{
		...
		{
			UE_LOG(LogOXRVisionOS, Log, TEXT("%s FC_%i SLF_0x%x SD_0x%8x FOXRVisionOSSession::StartSubmission doing nothing because SwiftDrawable is null.  This is OK if the app is being paused or exiting."), VOSThreadString(), FrameState.FrameCounter, FrameState.SwiftFrame, FrameState.SwiftDrawable);			
		}
	}
	else
	{

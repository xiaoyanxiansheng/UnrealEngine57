...
### 先把 Engine 基础 Shape 转换为项目 Mesh

前面通过 **Add > Shapes > Cube** 创建的墙和切割体是 Level 中的 `StaticMeshActor`，但它们的 `StaticMeshComponent` 默认仍引用 `/Engine/BasicShapes/Cube`。Actor 的 Transform 和命名属于当前 Level，Cube Mesh Asset 的所有权却仍属于 Engine Content；复制 Actor 只会复制这条引用，不会把 Cube Asset 复制进项目。

进入 Modeling Mode 并同时选中 `Room_Wall_Right_Window` 与 `Cut_Window` 后，如果顶部出现红字：

```text
Selection includes Engine assets, which cannot be modified.
```

这不是 Boolean 算法失败，而是输入资产的所有权不满足编辑条件。此时 `Model > Boolean` 会显示为灰色。先建立项目自有的输入 Mesh：

1. 保持 `Room_Wall_Right_Window` 与 `Cut_Window` 同时选中。
2. 在 Modeling Mode 左侧选择 **XForm**。
3. 点击 **Convert**。
4. 在 Convert 工具中把 **Output Type** 设为 **Static Mesh**。
5. 保持 **Transfer Materials** 开启；本次把 **Transfer Collision** 关闭，因为切割输入不需要碰撞，最终窗墙会在第 11 节重新建立明确的碰撞方案。
6. 点击 **Accept**。

Convert 会读取两个 Engine Cube 实例经过 Transform 后的几何，创建项目自有的 Static Mesh Asset 与替代 Actor，并删除被转换的两个旧 Actor。它修改的是输入对象的资产归属和表达方式，不应改变墙体与切割体在 World 中的可见尺寸和位置。生成资产的 Package 位置由 Modeling Mode 的 Generated Assets 设置决定，默认可能位于地图附近的 `_GENERATED` 目录；这些只是本次 Boolean 的中间输入，最终结果稳定后可以清理。

转换后重新检查：

- 顶部红色 Engine Asset 警告消失。
- Outliner 中仍有一面完整墙和一个穿透墙体的切割体。
- 两个对象的可见位置与尺寸没有变化。
- 重新选择两个对象后，**Model > Boolean** 不再是灰色。

如果 Convert 后名称被自动调整，先在 Outliner 恢复 `Room_Wall_Right_Window` 和 `Cut_Window`，再继续下一节。这里需要的是可保存、可进入版本管理的项目资产，不能靠复制 Actor 来改变 Engine Cube 的资产归属。

## 10. 使用 Modeling Mode 执行 Boolean

1. 在模式下拉菜单选择 **Modeling**。
2. 先选中 `Room_Wall_Right_Window`，再按住 `Ctrl` 选中 `Cut_Window`。
3. 在左侧工具分类选择 **Model**；不要停留在 **Create**。
4. 点击 **Boolean**。UE 5.7 当前这套 Modeling Mode 工具面板没有常驻工具搜索框，因此应使用明确的 **Model > Boolean** 入口。
5. **Operation** 选择 **Difference A - B**。
6. **Write To** 选择 **New Object**，不要覆盖任一中间输入。
7. **Name** 输入 `SM_Room_Wall_Window`。
8. **Output Type** 选择 **Static Mesh**，而不是临时 Dynamic Mesh Actor。
9. 检查预览正确后点击 **Accept**。
10. 在 Content Drawer 找到生成的 `SM_Room_Wall_Window`，移动到：

    `Content/RenderingPractice/Environment/Architecture`

    如果 Modeling Mode 的 **Asset Generation Location** 已设为 **Current Asset Browser Path If Available**，并且启动工具前 Content Drawer 正停在该目录，结果可能已直接生成在这里；否则它通常会先出现在配置的 `_GENERATED` 目录。移动后如产生 Redirector，在确认所有引用已保存且 Source Control 状态干净时，再对原目录执行 **Fix Up Redirectors**。

成功后：

- Content Drawer 中出现 `SM_Room_Wall_Window`。
- Level 中出现使用该 Mesh 的 Actor。
- Boolean 的中间墙体、切割体 Actor 依照工具的输入处理设置被隐藏或删除；确认最终 Actor 正确后，可清理 Convert 生成且已无引用的中间 Mesh Asset。
- 窗洞边缘应完整，没有封闭面残留。

这里必须区分三个状态：Convert 生成的项目 Mesh 是 Boolean 的可编辑输入，Boolean 预览是尚未提交的计算结果，`SM_Room_Wall_Window` 才是后续章节依赖的正式输出。不要把 `_GENERATED` 中间输入误当作最终窗墙，也不要在最终 Actor 仍引用它们时提前删除。
...
1. 检查窗洞方向和表面是否正常，并查看 Viewport 左上角统计中的 **Num Collision Primitives**。这个数只统计 Box、Sphere、Capsule 和 Convex 等简单碰撞体，不统计渲染三角形。
2. 点击 Viewport 工具栏中 **Lit** 右侧的眼睛图标 **Show**，展开 **Mesh Components**，启用 **Simple Collision**。这才是碰撞可视化入口；窗口顶部的 **Collision** 菜单主要用于生成、复制和删除简单碰撞，不是显示开关。
3. 如果 **Num Collision Primitives** 大于 `0`，并且 Simple Collision 显示出一个封住整个窗洞的 Box，选择窗口顶部 **Collision > Remove Collision**。该命令会删除这个 Asset 上的全部简单碰撞体；若统计为 `0` 且命令为灰色，说明当前没有旧 Box，直接跳过。
4. 在右侧 **Details > Collision** 中找到 **Collision Complexity**。点击当前的 **Project Default** 下拉框，选择 **Use Complex Collision As Simple**。该选项不在窗口顶部的 Collision 菜单中。
5. 再次通过 **Show > Mesh Components > Simple Collision** 检查。由于当前 Asset 已把 Complex 当作 Simple 查询几何，Viewport 应沿窗墙三角形显示碰撞线，窗洞区域不应出现一整块封闭 Box。也可以临时启用同一 Show 菜单中的 **Complex Collision**，单独观察复杂三角形表示。
6. 在 Details 中看到 **Complex Collision Mesh = None** 不代表没有复杂碰撞。这个字段用于指定一份独立的复杂碰撞 Mesh；保持 `None` 时，当前 Static Mesh 自身的三角形仍可提供 Complex Collision。
7. 确认至少有一个 Material Slot。
8. 按 `Ctrl+S` 保存 Static Mesh，确认 Asset 标签后的星号消失。
...
Complex As Simple 复用渲染三角形进行查询，能准确保留 Boolean 开口，但查询数据更复杂，也不能把该 Mesh 当作需要物理模拟的普通动态刚体方案。它是有明确期限的课程过渡，不是“更精确所以永远更好”。`Num Collision Primitives: 0` 在此方案下是正常结果：它证明没有独立简单凸体，不是否定三角形查询碰撞。成功检查还应回到 Level，使用 Pawn 或 Trace 验证墙面能阻挡、窗洞区域不命中整块 Box；只看到碰撞线并不足以证明实际查询路径正确。
...
先看顶部是否出现 `Selection includes Engine assets, which cannot be modified.`。出现该红字表示所选 Actor 仍引用 `/Engine/` 下的基础 Mesh；按第 9 节执行 **XForm > Convert**，输出项目自有的 Static Mesh，再重新按 A、B 顺序选择两个 Mesh Actor。还要确认当前位于左侧 **Model** 分类、恰好选择两个对象，并且没有处于 PIE 或 Simulate 状态。成功证据是红字消失且 Boolean 按钮恢复可点击。

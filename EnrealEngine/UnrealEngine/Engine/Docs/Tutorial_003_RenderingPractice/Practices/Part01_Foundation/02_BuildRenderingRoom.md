...
5. 切割 Cube 要完全穿过墙厚，并形成约 120 × 150 cm 的窗口区域。
...
3. 如果 Boolean 后没有合适碰撞，使用 **Collision > Add Box Simplified Collision**，或后续按实际需求重新生成。
4. 确认至少有一个 Material Slot。
5. 保存 Static Mesh。
...
简单 Box Collision 成本低但不能精确贴合窗洞；复杂碰撞能更准确，却增加碰撞数据和查询成本。当前章节以可继续学习为准，不在这里完成最终碰撞方案。
